...
install_qwen_code() {
    ...
    # Capture build-time info from source directory
    local build_hash build_ver
    build_hash=$(cd "${SCRIPT_DIR}" && git rev-parse --short HEAD 2>/dev/null || echo "N/A")
    build_ver=$(cd "${SCRIPT_DIR}" && node -p "require('./package.json').version" 2>/dev/null || echo "UNKNOWN")
    ...
    pack_output=$(cd "${work_dir}" && GIT_COMMIT_HASH="${build_hash}" CLI_VERSION="${build_ver}" npm pack 2>&1 | tee /dev/stderr)
