...
## Branch: v0.11.0-preview.0-no-telemetry
...
### 1. Branch Creation
```bash
# Start from v0.11.0-preview.0
git checkout -b v0.11.0-preview.0-no-telemetry origin/v0.11.0-preview.0
```
...
**Root Cause:** v0.11.0-preview.0 had a modified version of `scripts/telemetry_utils.js` while the no-telemetry commit wanted to delete it entirely.
...
**Issue:** v0.11.0-preview.0 uses multi-stage build, v0.10.6-no-telemetry uses single-stage build with direct `npm pack` from GitHub.

**Resolution:**
- Used v0.10.6-no-telemetry's single-stage Dockerfile as base
- Updated only `QWEN_REF` from `"release/v0.10.6-no-telemetry"` to `"v0.11.0-preview.0-no-telemetry"`
...
ARG QWEN_REF="v0.11.0-preview.0-no-telemetry"
...
### Summary
- **Total:** 3523 tests passed, 2 failed (same as v0.11.0-preview.0 base)
...
# Create base branch from target version (e.g., v0.12.0-preview.0)
