...
#
# Examples:
#   bash install.sh v0.11.0-no-telemetry
#   bash install.sh v0.11.0-no-telemetry --source github
...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  bash install.sh v0.11.0-no-telemetry"
    echo "  bash install.sh v0.11.0-no-telemetry --source github"
    echo ""
    exit 1
}
...
install_qwen_code() {
    ...
    pack_output=$(cd "${work_dir}" && npm pack "${pkg_ref}" 2>&1 | tee /dev/stderr)
