...
```bash
curl -fsSL https://raw.githubusercontent.com/undici77/qwen-code-no-telemetry/v0.13.0-no-telemetry/install.sh \
    | bash -s v0.13.0-no-telemetry
```
...
When a new upstream version is released, the "no-telemetry" dummy layer
and configuration must be applied to the new tag.
