...
import { checkI18n, findForbiddenZhTwPatterns, printCheckI18nResult, shouldWriteUnusedKeysJson, } from '../check-i18n.js';
vi.unmock('fs');
vi.unmock('node:fs');
const tempDirs = [];
...
function writeLocale(localesDir, code, entries) {
    const lines = Object.entries(entries).map(([key, value]) => `  ${JSON.stringify(key)}: ${JSON.stringify(value)},`);
    writeFileSync(path.join(localesDir, `${code}.js`), `export default {\n${lines.join('\n')}\n};\n`);
}
function writeSource(sourceDir, content) {
    writeFileSync(path.join(sourceDir, 'fixture.ts'), content);
}
function languages(...definitions) {
    return definitions.map((definition) => {
        const language = typeof definition === 'string' ? { code: definition } : definition;
        const strictParity = language.strictParity ?? ['zh', 'zh-TW'].includes(language.code);
        return {
...
describe('checkI18n', () => {
    it('records locale load failures and continues checking other locales', async () => {
        ...
        expect(result.errors).toContainEqual(expect.stringContaining('Failed to load broken.js: Invalid locale module'));
    ...
    it('returns a failed empty result when en.js is missing', async () => {
        ...
        expect(result.errors).toContainEqual(expect.stringContaining('Failed to load en.js:'));
    });
    it('enforces strict key parity for zh and zh-TW', async () => {
        ...
        expect(result.errors).toContain('Missing translation in zh.js: "MissingInStrict"');
        expect(result.errors).toContain('Extra key in zh.js (not in en.js): "ExtraStrict"');
        expect(result.errors).toContain('Missing translation in zh-TW.js: "MissingInStrict"');
        expect(result.errors).toContain('Extra key in zh-TW.js (not in en.js): "ExtraStrict"');
    });
    it('uses language metadata to decide strict key parity', async () => {
        ...
        expect(result.errors).toContain('Missing translation in fr.js: "MissingInStrict"');
        expect(result.errors).toContain('Extra key in fr.js (not in en.js): "ExtraStrict"');
    });
    it('warns for non-strict optional missing keys and errors for required keys', async () => {
        ...
        expect(result.errors).toContain('Missing required translation in fr.js: "Required"');
        expect(result.warnings).toContain('fr.js is missing 1 non-required translation keys');
    });
    it('warns instead of errors for extra keys in non-strict locales', async () => {
        ...
        expect(result.errors).not.toContain('Extra key in fr.js (not in en.js): "ExtraLoose"');
    ...
    it('errors when a required translation still falls back to English', async () => {
        ...
        expect(result.errors).toContain('Required translation still falls back to English in fr.js: "Required"');
    });
    it('allows the AUTO mode entry notice semantic key in en.js while preserving mismatch errors', async () => {
        ...
        writeSource(sourceDir, "t('auto_mode.entry_notice');\nt('OrdinaryMismatch');\n");
        const result = await checkI18n({
            localesDir,
            sourceDir,
            supportedLanguages: languages('en', 'fr'),
        ...
        expect(result.errors).not.toContain('Key-value mismatch in en.js: "auto_mode.entry_notice" !== "Auto mode enabled."');
        expect(result.errors).toContain('Key-value mismatch in en.js: "OrdinaryMismatch" !== "Different English copy"');
    });
    ...
    it('flags Mainland-Chinese vocabulary and variant Traditional chars in zh-TW values', async () => {
        ...
        expect(result.errors).toContain('Non-Taiwan vocabulary in zh-TW.js at "Open": "啓" should be "啟"');
        expect(result.errors).toContain('Non-Taiwan vocabulary in zh-TW.js at "Server": "服務器" should be "伺服器"');
        expect(result.errors).toContain('Non-Taiwan vocabulary in zh-TW.js at "Menu": "菜單" should be "選單"');
        expect(result.errors).not.toContainEqual(expect.stringContaining('at "Disable"'));
        expect(result.errors).not.toContainEqual(expect.stringContaining('at "Config"'));
    });
...
//# sourceMappingURL=check-i18n.test.js.map