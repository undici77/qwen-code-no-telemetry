...
import { chmodSync, mkdtempSync, mkdirSync, readdirSync, rmSync, utimesSync, writeFileSync, } from 'node:fs';
import { extname, join } from 'node:path';
import { tmpdir } from 'node:os';
import { fileURLToPath } from 'node:url';
import { BUNDLED_SKILL_TEST_FILE_RE, copyBundleAssets, reviewSourceDigestForBuild, } from '../copy_bundle_assets.js';
import { isAllowedDistEntry } from '../create-standalone-package.js';
import { DIGESTED_EXTENSIONS, DIGEST_FILE, NOT_BUNDLED_SKILL_FILE, reviewSourceRoots, reviewSourcesDigest, } from '../../packages/cli/src/commands/review/lib/stale-bundle.js';
 ...
 */
function stampNameWrittenByBuild() {
...
describe('the build stamp and the staleness check agree', () => {
    it('hashes this repository to the same digest', () => {
        const fromCheck = reviewSourcesDigest(repoRoot, reviewSourceRoots(repoRoot));
    ...
    it('agrees on a synthetic tree too, including a file-shaped root', () => {
        ...
        try {
            ...
            mkdirSync(join(root, 'packages', 'core', 'src', 'skills', 'bundled', 'review'), { recursive: true });
            ...
            writeFileSync(join(services, 'review-worktree-lease.ts'), 'leases the worktree');
            ...
            expect(reviewSourceDigestForBuild(root).digest).toBe(reviewSourcesDigest(root, reviewSourceRoots(root)));
            ...
            writeFileSync(join(cli, 'review', '__snapshots__', 'snapshot-helper.ts'), 'exports[`a`] = `b`;');
            mkdirSync(join(cli, 'review', '__fixtures__'), { recursive: true });
            writeFileSync(join(cli, 'review', '__fixtures__', 'responder.mjs'), 'export const a = 1;');
    ...
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('neither side measures a tree whose subdirectory cannot be listed', () => {
        ...
        try {
            ...
            try {
                ...
                expect(reviewSourcesDigest(root, reviewSourceRoots(root))).toBeUndefined();
            }
            finally {
                chmodSync(lib, 0o755);
            }
    ...
    it('the skill allowlist covers everything the copier would ship', () => {
        ...
        const skillDir = join(repoRoot, 'packages', 'core', 'src', 'skills', 'bundled', 'review');
        const shipped = [];
        const admitted = [];
        const walk = (dir) => {
            for (const e of readdirSync(dir, { withFileTypes: true })) {
                ...
                if (e.isDirectory())
                    ...
                    if (e.name !== '.DS_Store' &&
                        ...
                        shipped.push(full);
                    if (DIGESTED_EXTENSIONS.skill.has(extname(e.name)) &&
...
//# sourceMappingURL=review-source-digest.test.js.map