...
function parseArgs(argv) {
    const overlayPaths = new Set(parsePathList(process.env.OPENWORK_OVERLAY_PATHS || 'README.md'));
    let mode = 'auto';
    let openworkDir = process.env.OPENWORK_DIR?.trim();
    let openworkRef = process.env.OPENWORK_REF?.trim() ||
    ...
    if (!openworkDir) {
        throw new Error('Missing OpenWork checkout. Pass --openwork-dir or set OPENWORK_DIR.');
    }
    return {
        mode,
...
async function run(cmd, cwd, options = {}) {
    ...
    const stdoutPromise = options.capture && proc.stdout
        ? new Response(proc.stdout).text()
        : Promise.resolve('');
    const stderrPromise = options.capture && proc.stderr
    ...
    if (exitCode !== 0 && !options.allowFailure) {
        ...
        throw new Error(`${cmd.join(' ')} failed with exit code ${exitCode}${detail ? `\n${detail}` : ''}`);
    }
...
async function localBranchExists(cwd, branch) {
    const result = await git(cwd, ['show-ref', '--verify', `refs/heads/${branch}`], { allowFailure: true, capture: true });
    return result.exitCode === 0;
}
...
async function ensureCleanWorktree(cwd, label, paths = []) {
    ...
    if (status.stdout.trim()) {
        throw new Error(`${label} must be clean before syncing:\n${status.stdout.trim()}`);
    }
}
...
async function findLatestTrailer(cwd, ref, trailer) {
    ...
    const pattern = new RegExp(`^${escapeRegExp(trailer)}:\\s*([^\\s]+)\\s*$`, 'im');
...
async function findTrailerValues(cwd, ref, trailer) {
    ...
    const pattern = new RegExp(`^${escapeRegExp(trailer)}:\\s*([^\\s]+)\\s*$`, 'gim');
...
async function resolveSourceBase(sourceRepo, explicitBase, targetRepo, targetRef, trailer) {
    const base = explicitBase ?? (await findLatestTrailer(targetRepo, targetRef, trailer));
    if (!base) {
        throw new Error(`Missing source base. Pass --source-base or create a prior sync commit ` +
...
async function createImportPatch(openworkRoot, base, source, overlayPaths) {
    const result = await git(openworkRoot, [
        'diff',
        '--binary',
        '--full-index',
...
async function getMergeIntroducedCommits(cwd, firstParent, mergeCommit, pathspecs) {
    const result = await git(cwd, [
        'log',
        '--reverse',
        '--topo-order',
...
async function ensureSimpleMergeCommit(params) {
    ...
    const introducedCommits = await getMergeIntroducedCommits(params.sourceRepo, firstParent, params.commit, params.pathspecs);
    const missingCommits = [];
    for (const commit of introducedCommits) {
        if (params.handledCommits.has(commit) ||
    ...
    if (missingCommits.length > 0) {
        throw new Error(`Merge commit introduced unhandled commits: ${params.commit}\n` +
            missingCommits.map((commit) => `  ${commit}`).join('\n'));
    }
    const mergeTree = await git(params.sourceRepo, ['merge-tree', '--write-tree', firstParent, secondParent], { allowFailure: true, capture: true });
    if (mergeTree.exitCode !== 0) {
        throw new Error(`Merge commit requires manual resolution: ${params.commit}`);
    }
    const tree = mergeTree.stdout.trim().split(/\s+/)[0];
    const diff = await git(params.sourceRepo, ['diff', '--quiet', tree, params.commit, '--', ...params.pathspecs], { allowFailure: true });
    if (diff.exitCode === 0)
        return;
    if (diff.exitCode === 1) {
        throw new Error(`Merge commit has manual resolution changes: ${params.commit}`);
    }
...
async function migrateCommits(params) {
    ...
    const targetTrailer = params.mode === 'import' ? 'OpenWork-Commit' : 'Qwen-Code-Commit';
    const targetSyncedCommits = await findTrailerValues(params.targetRepo, 'HEAD', targetTrailer);
    for (const commit of params.commits) {
        ...
        const targetReason = targetAlreadySyncedCommit(commit, targetTrailer, targetSyncedCommits);
        if (targetReason) {
            console.log(`Skipping ${commit.slice(0, 12)}; ${targetReason}.`);
            handledCommits.add(commit);
            continue;
        }
        const skipReason = await shouldSkipSyncedCommit(params.sourceRepo, commit, params.mode);
        if (skipReason) {
        ...
        if (await commitChanges(params.targetRepo, subject, [
...
async function runExport(options) {
    ...
    const base = await resolveSourceBase(repoRoot, options.sourceBase, openworkRoot, options.openworkRef, 'Qwen-Code-Commit');
    ...
    const count = await migrateCommits({
        ...
        commits,
        pathspecs,
        createPatch: (parent, commit) => createExportPatch(parent, commit, options.overlayPaths),
...
async function runImport(options) {
    ...
    const base = await resolveSourceBase(openworkRoot, options.sourceBase, repoRoot, options.qwenBase, 'OpenWork-Commit');
    ...
    const count = await migrateCommits({
        ...
        commits,
        pathspecs,
        createPatch: (parent, commit) => createImportPatch(openworkRoot, parent, commit, options.overlayPaths),
        trailers: (parent, commit) => [
...
async function runAuto() {
    throw new Error('Auto mode is intentionally conservative. Use --mode export or --mode ' +
