...
if (!existsSync(cliBundle)) {
    throw new Error('Java daemon E2E requires dist/cli.js; run `npm run build && npm run bundle` from the repository root first.');
}
...
writeFileSync(path.join(testHome, '.qwen', 'settings.json'), JSON.stringify({ ui: { enableFollowupSuggestions: false } }));
const fake = await startFakeOpenAIServer(({ body }) => {
    ...
    if (latestUserText.includes(deadlinePrompt) ||
...
const cleanEnvironment = Object.fromEntries(Object.entries(process.env).filter(([key]) => !/^(https?|all)_proxy$/i.test(key) &&
...
try {
    ...
    const result = await Promise.race([
        ...
        new Promise((_resolve, reject) => {
            timeout = setTimeout(() => reject(new Error(`Java daemon E2E Maven test timed out after ${javaTestTimeoutMs}ms`)), javaTestTimeoutMs);
        }),
        signalFailure,
    ]).finally(() => {
    ...
    if (result !== 0) {
        const logPath = path.join(testHome, '.qwen', 'debug', 'daemon', 'daemon.log');
        let daemonLog = '';
        try {
        ...
        throw new Error(`Java E2E failed with ${result}; fake requests=${fake.requests.length}\n${stderr}\n${daemonLog}`);
    }
    if (fake.requests.length === 0) {
        throw new Error('Java E2E completed without exercising the daemon model path; the integration tests may have been skipped');
    }
}
catch (error) {
    runFailure = error;
...
if (runFailure !== undefined && cleanupFailures.length > 0) {
    throw new AggregateError([runFailure, ...cleanupFailures], 'Java daemon E2E and cleanup both failed');
}
...
//# sourceMappingURL=run-java-daemon-sdk-e2e.js.map