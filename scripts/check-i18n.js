...
import { MUST_TRANSLATE_KEYS, SUPPORTED_LANGUAGES, } from '../packages/cli/src/i18n/index.js';
import { getTranslationModuleExport, isTranslationDict, } from '../packages/cli/src/i18n/translationDict.js';
...
const EN_SEMANTIC_KEY_EXCEPTIONS = new Set([
...
export function shouldWriteUnusedKeysJson() {
    return (process.argv.includes(WRITE_UNUSED_KEYS_FLAG) ||
 ...
 */
const ZH_TW_FORBIDDEN_PATTERNS_RAW = [
...
const ZH_TW_FORBIDDEN_PATTERNS = [...ZH_TW_FORBIDDEN_PATTERNS_RAW].sort((a, b) => b.pattern.length - a.pattern.length);
/**
 ...
 */
const ZH_TW_ALLOWED_EXCEPTIONS = new Set([
 ...
 */
export function findForbiddenZhTwPatterns(translations) {
    const findings = [];
    for (const [key, value] of Object.entries(translations)) {
        if (ZH_TW_ALLOWED_EXCEPTIONS.has(key))
            continue;
...
function translationValuesMatch(left, right) {
    return JSON.stringify(left) === JSON.stringify(right);
}
function countTranslatedKeys(enTranslations, localeTranslations) {
...
export async function checkI18n(options = {}) {
    const errors = [];
    const warnings = [];
    const localesDir = options.localesDir ??
        path.join(__dirname, '../packages/cli/src/i18n/locales');
    const sourceDir = options.sourceDir ?? path.join(__dirname, '../packages/cli/src');
    ...
    const strictKeyParityLocales = options.strictKeyParityLocales ??
    ...
    for (const key of EN_SEMANTIC_KEY_EXCEPTIONS) {
        if (!builtinMustTranslateKeySet.has(key)) {
            errors.push(`English semantic key exception must be listed in MUST_TRANSLATE_KEYS: "${key}"`);
        }
    }
    ...
    for (const locale of localeDefinitions) {
        try {
            localeTranslations.set(locale.code, await loadTranslationsFile(locale.path));
        }
        catch (error) {
            errors.push(`Failed to load ${locale.code}.js: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    for (const locale of localeDefinitions) {
        ...
        if (missingKeys.length > 0) {
            if (requiresStrictKeyParity) {
                ...
                const missingRequiredKeys = missingKeys.filter((key) => mustTranslateKeySet.has(key));
                const missingOptionalKeyCount = missingKeys.length - missingRequiredKeys.length;
                for (const key of missingRequiredKeys) {
                    errors.push(`Missing required translation in ${locale.code}.js: "${key}"`);
                }
                if (missingOptionalKeyCount > 0) {
                    warnings.push(`${locale.code}.js is missing ${missingOptionalKeyCount} non-required translation keys`);
                }
            }
        }
        if (extraKeys.length > 0) {
            if (requiresStrictKeyParity) {
                for (const key of extraKeys) {
                    errors.push(`Extra key in ${locale.code}.js (not in en.js): "${key}"`);
                }
            }
            else {
                warnings.push(`${locale.code}.js has ${extraKeys.length} keys not present in en.js`);
            }
        }
        for (const key of untranslatedMustKeys) {
            errors.push(`Required translation still falls back to English in ${locale.code}.js: "${key}"`);
        }
    }
    ...
    if (zhTWTranslations) {
        for (const { key, pattern, preferred } of findForbiddenZhTwPatterns(zhTWTranslations)) {
            errors.push(`Non-Taiwan vocabulary in zh-TW.js at "${key}": "${pattern}" should be "${preferred}"`);
        }
    }
...
export function printCheckI18nResult(result, options = {}) {
    console.log('\n=== i18n Check Results ===\n');
    ...
    for (const locale of result.stats.locales) {
        const coverage = locale.totalKeys > 0
            ...
            : '0.0';
        console.log(`  - ${locale.id} (${locale.code}): ${locale.translatedKeys}/${locale.totalKeys} translated (${coverage}%)`);
    }
    console.log();
    if (result.warnings.length > 0) {
        ...
        if (result.stats.unusedKeysOnlyInLocales &&
            ...
            if (options.writeUnusedKeysJson) {
                const outputPath = options.unusedKeysOutputPath ??
