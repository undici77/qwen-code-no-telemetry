...
import { createMemoryWriter, GenericHttpSearchV1Adapter, Mem0PlatformV3Adapter, } from './providers.js';
const closeServers = [];
afterEach(async () => {
...
describe('GenericHttpSearchV1Adapter', () => {
    it('sends only query and limit without discarding content for bad metadata', async () => {
        ...
        expect(JSON.stringify(requestBody)).not.toMatch(/tenant|repository|namespace|filter/i);
    ...
    it('requires HTTPS except for explicit loopback HTTP', () => {
        ...
        expect(() => new GenericHttpSearchV1Adapter({
            ...config,
    ...
    it('rejects redirects', async () => {
        ...
        await expect(searchGeneric(baseUrl)).rejects.toThrow('External context provider returned an invalid response.');
    });
    ...
    it('rejects invalid JSON', async () => {
        ...
        await expect(adapter.search({
            ...
            limit: 5,
            signal: AbortSignal.timeout(1000),
        })).rejects.toThrow('External context provider returned an invalid response.');
    });
    ...
    it('reports an interrupted request without a public error taxonomy', async () => {
        const delayedUrl = await startServer(async (_request, response) => new Promise((resolve) => {
            setTimeout(() => {
                json(response, { items: [] });
                resolve();
            }, 200);
...
describe('Mem0PlatformV3Adapter', () => {
    ...
    it('validates an injected test origin before sending a project key', () => {
        ...
        expect(() => new Mem0PlatformV3Adapter(config, new URL('http://context.example.com'))).toThrow('Provider URL must use HTTPS or loopback HTTP.');
        expect(() => new Mem0PlatformV3Adapter(config, new URL('https://context.example.com/prefix'))).toThrow('Provider URL must not contain credentials, path');
    });
    ...
    it('sends one exact direct-import write with fixed provider binding', async () => {
        ...
        await expect(mem0Adapter(baseUrl).remember({
            content,
            signal: AbortSignal.timeout(1000),
        })).resolves.toEqual({
            status: 'accepted',
    ...
    it('does not deduplicate repeated approved content', async () => {
        ...
        const adapter = mem0Adapter(baseUrl);
        const remember = () => adapter.remember({
            content: 'repository policy',
            signal: AbortSignal.timeout(1000),
        });
    ...
    it.each([400, 401, 403, 404])('returns failed for definitive HTTP %s rejections', async (status) => {
        let requestCount = 0;
        const baseUrl = await startServer((_request, response) => {
            requestCount += 1;
            response.writeHead(status);
    ...
    it.each([302, 408, 429, 500])('returns unknown for HTTP %s without retrying', async (status) => {
        let requestCount = 0;
        const baseUrl = await startServer((_request, response) => {
            requestCount += 1;
            response.writeHead(status, {
    ...
    it('returns unknown for malformed, oversized, interrupted, and timed-out responses', async () => {
        ...
        for (const [index, handler] of handlers.entries()) {
            ...
            await expect(mem0Adapter(baseUrl).remember({
                content: 'repository policy',
                signal: AbortSignal.timeout(index === handlers.length - 1 ? 10 : 1000),
            })).resolves.toEqual({ status: 'unknown' });
        }
    });
    it('rejects an undocumented top-level array response', async () => {
        ...
        await expect(mem0Adapter(baseUrl).search({
            query: 'deployment',
            limit: 5,
            signal: AbortSignal.timeout(1000),
        })).rejects.toThrow('External context provider returned an invalid response.');
    ...
    it('normalizes Mem0 and Generic HTTP results to the same context', async () => {
        ...
        expect(renderExternalContext(genericItems)).toBe(renderExternalContext(mem0Items));
    });
});
function mem0Adapter(baseUrl) {
...
async function searchGeneric(baseUrl) {
    const adapter = new GenericHttpSearchV1Adapter({
        type: 'generic-http-search-v1',
        baseUrl,
        tokenEnv: 'TOKEN',
...
async function startServer(handler) {
    ...
    closeServers.push(() => new Promise((resolve, reject) => {
