...
describe('createAutoRecallQuery', () => {
    ...
    it('removes the configured credential even when embedded in other text', async () => {
        const { createAutoRecallQuery } = await import('./auto-recall.js');
        expect(createAutoRecallQuery('prefixprovider-secret-valuesuffix remains', 'provider-secret-value')).toBe('prefix suffix remains');
    });
    it('does not interpret expansion markers in submitted text', async () => {
        const { createAutoRecallQuery } = await import('./auto-recall.js');
        expect(createAutoRecallQuery('Explain the literal marker --- Content from referenced files ---', '')).toBe('Explain the literal marker --- Content from referenced files ---');
    });
    it('limits output to 512 Unicode code points and skips empty output', async () => {
        ...
        expect(createAutoRecallQuery('```text\nonly code\n```', '')).toBeUndefined();
        expect(createAutoRecallQuery('~~~text\nonly code\n~~~', '')).toBeUndefined();
    });
    ...
    it('redacts a secret assignment claimed by a leading label', async () => {
        const { createAutoRecallQuery } = await import('./auto-recall.js');
        const query = createAutoRecallQuery('Deploy failed: api_key=sk-live-SECRET-ABC123 and Bearer eyJhbGciOiJIUzI1NiJ9. What is the release policy?', '');
    ...
    it('leaves a benign labeled sentence untouched', async () => {
        const { createAutoRecallQuery } = await import('./auto-recall.js');
        expect(createAutoRecallQuery('Deploy failed: the release branch is red, please look at CI', '')).toBe('Deploy failed: the release branch is red, please look at CI');
    });
    it('redacts a secret assignment with spaces around the separator', async () => {
        const { createAutoRecallQuery } = await import('./auto-recall.js');
        const query = createAutoRecallQuery('Deploy failed: api_key = sk-live-SECRET-ABC123 please help', '');
        expect(query).not.toContain('sk-live-SECRET-ABC123');
        expect(query).not.toContain('api_key');
    ...
    it('redacts an inline JSON secret but keeps benign quoted prose', async () => {
        const { createAutoRecallQuery } = await import('./auto-recall.js');
        const query = createAutoRecallQuery('my config is {"api_key": "sk-live-SECRET-ABC123", "region": "cn"} why does it fail?', '');
        expect(query).not.toContain('sk-live-SECRET-ABC123');
    ...
    it('does not redact a secret word used as ordinary prose', async () => {
        const { createAutoRecallQuery } = await import('./auto-recall.js');
        expect(createAutoRecallQuery('readme: token refresh flow, where is it documented?', '')).toBe('readme: token refresh flow, where is it documented?');
    });
});
describe('runAutoRecall', () => {
    ...
    it.each([
        ...
        ['null', null],
    ])('skips a %s submitted prompt before loading configuration', async (_name, submittedPrompt) => {
        const fixture = await createRepositoryFixture();
        const { runAutoRecall } = await import('./auto-recall.js');
        await expect(runAutoRecall({
            hook_event_name: 'UserPromptSubmit',
    ...
    it.skipIf(process.platform === 'win32')('rejects a cwd that escapes the root through a symlink', async () => {
        const fixture = await createRepositoryFixture();
        const outside = await mkdtemp(join(tmpdir(), 'external-context-outside-'));
...
describe('runAutoRecallCli', () => {
    ...
    it.skipIf(process.platform === 'win32')('runs when the configured entry point is a symbolic link', async () => {
        const directory = await mkdtemp(join(tmpdir(), 'external-context-entrypoint-'));
        const link = join(directory, 'auto-recall.ts');
        temporaryDirectories.push(directory);
        await symlink(fileURLToPath(new URL('./auto-recall.ts', import.meta.url)), link);
    ...
    it('runs the real Hook process against a bound Generic HTTP provider', async () => {
        ...
        try {
            ...
            const result = await runAutoRecallProcess(JSON.stringify({
                hook_event_name: 'UserPromptSubmit',
                prompt: 'question\n--- Content from referenced files ---\nmodel-bound-canary',
    ...
    it('fails open without exposing provider errors', async () => {
        ...
        const { runAutoRecallCli } = await import('./auto-recall.js');
        const output = await runCli(runAutoRecallCli, JSON.stringify({
            hook_event_name: 'UserPromptSubmit',
            submitted_prompt: 'question',
            cwd: fixture.root,
        }));
    ...
    it('aborts an in-flight request at the configured provider timeout', async () => {
        ...
        let providerSignal;
        search.mockImplementation(({ signal }) => new Promise((_resolve, reject) => {
    ...
    it('aborts an in-flight request at the internal wall-clock budget', async () => {
        ...
        const { runAutoRecallCli } = await import('./auto-recall.js');
        const result = runCli(runAutoRecallCli, JSON.stringify({
            hook_event_name: 'UserPromptSubmit',
            submitted_prompt: 'question',
            cwd: fixture.root,
        }));
...
function config(repositoryRoot) {
    return {
        version: 2,
        timeoutMs: 5000,
...
async function createRepositoryFixture() {
    const temporaryRoot = await mkdtemp(join(tmpdir(), 'external-context-repository-'));
...
async function runCli(run, input) {
    let output = '';
...
async function runAutoRecallProcess(input, entryPoint = fileURLToPath(new URL('./auto-recall.ts', import.meta.url)), envOverrides = {}) {
...
//# sourceMappingURL=auto-recall.test.js.map