...
import { runWriteConfirmation, runWriteConfirmationCli, } from './write-confirmation.js';
describe('write confirmation Hook', () => {
    it.each(['default', 'auto', 'auto_edit', 'auto-edit', 'yolo'])('asks in %s mode and displays a reversible escaped representation', (permissionMode) => {
        const content = '  quote " slash \\ newline\nnull \u0000 c1 \u0085 line \u2028 paragraph \u2029 zero \u200b bidi \u202e supplemental \u{110bd}  ';
        const result = runWriteConfirmation(validInput({ permission_mode: permissionMode, content }));
    ...
    it.each([
        ...
        expect(result.hookSpecificOutput).toEqual({
            ...
            permissionDecisionReason: 'External context memory write confirmation request is invalid.',
        });
    });
    it.each([
        ...
        const result = runWriteConfirmation(validInput({ permission_mode: permissionMode }));
        expect(result.hookSpecificOutput).toEqual({
            hookEventName: 'PreToolUse',
            permissionDecision: 'deny',
    ...
    it('ignores extra tool arguments that the MCP schema strips', () => {
        ...
        const result = runWriteConfirmation(validInput({ content, extraToolInput: true }));
        expect(result.hookSpecificOutput.permissionDecision).toBe('ask');
        const serialized = result.hookSpecificOutput.permissionDecisionReason.slice(result.hookSpecificOutput.permissionDecisionReason.indexOf('\n') + 1);
        expect(JSON.parse(serialized)).toBe(content);
    });
    it('accepts exactly 4000 Unicode code points', () => {
        ...
        expect(result.hookSpecificOutput.permissionDecision).toBe('ask');
        expect(JSON.parse(result.hookSpecificOutput.permissionDecisionReason.split('\n')[1] ??
            'null')).toBe(content);
    });
    it.each([
    ...
    it('propagates output failures to the process entry point', async () => {
        const input = Readable.from([JSON.stringify(validInput())]);
        await expect(runWriteConfirmationCli(input, {
            write: () => {
...
//# sourceMappingURL=write-confirmation.test.js.map