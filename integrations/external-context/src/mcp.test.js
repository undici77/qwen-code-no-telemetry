...
describe('external context MCP server', () => {
    it('registers only a provider-bound retrieval tool', async () => {
        ...
        expect(tools.tools[0]?.inputSchema).not.toHaveProperty('properties.tenantId');
        expect(tools.tools[0]?.inputSchema).not.toHaveProperty('properties.repositoryId');
        expect(tools.tools[0]?.inputSchema).not.toHaveProperty('properties.filters');
    });
    ...
    it('accepts 2000 astral Unicode characters and rejects 2001', async () => {
        ...
        expect(JSON.stringify(rejected.content)).toContain('Search query must contain at most 2000');
    ...
    it('aborts the provider when the client cancels a tool request', async () => {
        ...
        const search = vi.fn(({ signal }) => new Promise((_resolve, reject) => {
    ...
    it('reports an empty normalized query without calling the provider', async () => {
        ...
        expect(result.isError).toBe(true);
        expect(JSON.stringify(result.content)).toContain('Search query must not be empty.');
    ...
    it('registers the write tool only for an enabled Mem0 writer', async () => {
        ...
        expect(JSON.stringify(remember?.inputSchema.properties)).not.toMatch(/tenant|repository|namespace|filter|metadata|app.?id/i);
    });
    it('stores exact validated content without forwarding model selectors', async () => {
        ...
        expect(JSON.stringify(remember.mock.calls)).not.toMatch(/model-controlled|tenant|filter|metadata/i);
        const text = result.content[0];
        expect(text).toMatchObject({ type: 'text' });
    ...
    it('returns a stable error for a definitive provider rejection', async () => {
        ...
        expect(body.message).toContain('Do not retry without changing the content or configuration.');
    });
    it('returns a stable memory error without provider details', async () => {
        ...
        expect(result.isError).toBe(true);
        expect(JSON.stringify(result)).toContain('External context memory write failed.');
        expect(JSON.stringify(result)).not.toContain('secret upstream');
    });
    it('aborts the memory writer when the client cancels the request', async () => {
        ...
        const remember = vi.fn(({ signal }) => new Promise((_resolve, reject) => {
            writerSignal = signal;
            signalReceived?.();
            signal.addEventListener('abort', () => reject(signal.reason), {
                once: true,
    ...
    it('aborts the memory writer at the configured timeout', async () => {
        let writerSignal;
        const remember = vi.fn(({ signal }) => new Promise((resolve) => {
            writerSignal = signal;
            signal.addEventListener('abort', () => resolve({ status: 'unknown' }), { once: true });
        }));
        const client = await connect({
...
describe('runMcp', () => {
    ...
    it('rejects a non-version-1 config at startup', async () => {
        ...
        await expect(runMcp()).rejects.toThrow('External context MCP server requires a version 1 configuration.');
    });
});
function config() {
    return {
...
function writeConfig(timeoutMs = 1000) {
    return {
        version: 1,
        timeoutMs,
        write: { enabled: true },
