...
describe('external context startup', () => {
    ...
    it('prints a sanitized error for invalid proxy configuration', async () => {
        ...
        await import('./main.js');
        expect(write).toHaveBeenCalledWith('[external-context] Proxy environment configuration is invalid. Check HTTP_PROXY, HTTPS_PROXY, and NO_PROXY.\n');
        expect(runMcp).not.toHaveBeenCalled();
        expect(process.exitCode).toBe(1);
    });
    it('prints sanitized configuration errors', async () => {
        const { ConfigurationError } = await import('./config.js');
        runMcp.mockRejectedValue(new ConfigurationError('External context config is not valid JSON.'));
        const write = vi.spyOn(process.stderr, 'write').mockReturnValue(true);
        await import('./main.js');
        expect(write).toHaveBeenCalledWith('[external-context] External context config is not valid JSON.\n');
        expect(process.exitCode).toBe(1);
    });
    it('keeps unexpected startup errors opaque', async () => {
        ...
        const write = vi.spyOn(process.stderr, 'write').mockReturnValue(true);
        await import('./main.js');
        expect(write).toHaveBeenCalledWith('[external-context] External context startup failed.\n');
    });
});
