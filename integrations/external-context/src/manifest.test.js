...
describe('extension manifest', () => {
    it('exposes only the retrieval tool', async () => {
        const manifest = JSON.parse(await readFile(new URL('../qwen-extension.json', import.meta.url), 'utf8'));
    ...
    it.each([
        {
            platform: 'posix',
            command: "exec '/absolute/path/to/node' '/administrator/path/to/qwen-code/integrations/external-context/dist/auto-recall.js'",
            shell: undefined,
        },
        {
            platform: 'windows',
    ...
    ])('keeps the managed auto-recall $platform profile Hook-only', async ({ platform, command, shell }) => {
        const settings = await readJson(`../examples/managed-auto-recall-user-settings-${platform}.json`);
    ...
    it.each(['generic-http', 'mem0'])('uses v2 for the managed auto-recall %s provider', async (provider) => {
    ...
    it('disables local persistence and native memory in the auto profile', async () => {
        const settings = await readJson('../examples/managed-auto-recall-system-settings.json');
        ...
        expect(settings.slashCommands?.disabled).toEqual(expect.arrayContaining(['memory', 'remember', 'forget', 'dream', 'cd']));
        expect(settings.hooks).toBeUndefined();
    });
    it.each([
        {
            platform: 'posix',
            command: "exec '/absolute/path/to/node' '/administrator/path/to/qwen-code/integrations/external-context/dist/write-confirmation.js'",
            shell: undefined,
        },
        {
            platform: 'windows',
    ...
    ])('pins the managed Mem0 write $platform confirmation Hook', async ({ platform, command, shell }) => {
        const settings = await readJson(`../examples/managed-mem0-write-user-settings-${platform}.json`);
        const events = Object.keys(settings.hooks ?? {});
    ...
    it('pins write confirmation and disables local persistence', async () => {
        const settings = await readJson('../examples/managed-mem0-write-system-settings.json');
        expect(settings).toMatchObject({
            $version: 4,
            disableAllHooks: false,
            general: { chatRecording: false },
