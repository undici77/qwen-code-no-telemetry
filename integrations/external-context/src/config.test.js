...
describe('loadConfig', () => {
    ...
    it('rejects writes for Generic HTTP and v2 configurations', async () => {
        ...
        })).rejects.toThrow('External context memory writes require a Mem0 provider.');
        await expect(loadConfig({
            QWEN_EXTERNAL_CONTEXT_CONFIG: fixture.config,
        })).rejects.toThrow('External context memory writes require a Mem0 provider.');
        await writeConfig(fixture, {
    ...
    it.each([{ enabled: false }, { enabled: true, mode: 'unsafe' }, {}])('strictly validates the v1 write block %#', async (write) => {
        const fixture = await createFixture();
        await writeConfig(fixture, {
            version: 1,
            write,
    ...
    it.skipIf(process.platform === 'win32')('resolves a symlinked v2 repository root', async () => {
        const fixture = await createFixture();
        const repositoryRoot = join(fixture.root, 'repository');
    ...
    it('requires an absolute config path', async () => {
        ...
        })).rejects.toThrow('QWEN_EXTERNAL_CONTEXT_CONFIG must name an absolute file path.');
    });
    it('does not read an inherited config path', async () => {
        ...
        await expect(loadConfig(env)).rejects.toThrow('QWEN_EXTERNAL_CONTEXT_CONFIG must name an absolute file path.');
    });
    it('does not expose a missing credential name or secret config data', async () => {
        ...
        expect(message).toBe('Configured external context credential is unavailable.');
    });
    it.each(['__proto__', 'constructor', 'toString'])('does not treat inherited %s as a credential', async (tokenEnv) => {
        const fixture = await createFixture();
        await writeConfig(fixture, {
            version: 1,
            provider: {
...
async function writeConfig(fixture, value) {
