...
import { postJson, ProviderHttpStatusError, validateProviderBaseUrl, } from './http-client.js';
...
export function createMemoryWriter(config) {
    switch (config.type) {
        case 'mem0-platform-v3':
            return new Mem0PlatformV3Adapter(config);
        case 'generic-http-search-v1':
...
export class GenericHttpSearchV1Adapter {
    config;
    searchUrl;
    constructor(config) {
        this.config = config;
...
export class Mem0PlatformV3Adapter {
    config;
    baseUrl;
    constructor(config, baseUrl = MEM0_BASE_URL) {
        this.config = config;
    ...
    async remember(input) {
        ...
        try {
            ...
            if (error instanceof ProviderHttpStatusError &&
...
function parseMem0RememberResult(response) {
...
function parseGenericItems(response) {
...
function parseGenericItem(value) {
    if (!isRecord(value)) {
        return undefined;
    }
    return parseItemFields(value, 'content');
}
function parseMem0Items(response) {
    const values = isRecord(response) && Array.isArray(response['results'])
        ? response['results']
        : undefined;
    if (!values) {
        throw new Error('External context provider returned an invalid response.');
    ...
    return values
        .map((value) => isRecord(value) ? parseItemFields(value, 'memory') : undefined)
        .filter((item) => item !== undefined)
        .slice(0, MAX_PROVIDER_ITEMS);
}
function parseItemFields(value, contentKey) {
    const id = value['id'];
...
function validateConfiguredBaseUrl(value) {
    try {
        return validateProviderBaseUrl(value);
    }
    catch (error) {
        throw new ConfigurationError(error instanceof Error
            ? error.message
