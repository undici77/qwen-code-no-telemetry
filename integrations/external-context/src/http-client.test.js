...
describe('postJson bounded body reading', () => {
    ...
    it('accepts a body of exactly MAX_RESPONSE_BYTES', async () => {
        ...
        vi.stubGlobal('fetch', vi.fn(async () => streamingResponse([new TextEncoder().encode(exact)], vi.fn())));
        await expect(postJson(requestArgs())).resolves.toEqual([
    ...
    it('rejects an over-budget stream AND cancels it', async () => {
        ...
        await expect(postJson(requestArgs())).rejects.toThrow('External context provider returned an invalid response.');
    ...
    it('holds the oversize reject until a deferred cancellation settles', async () => {
        ...
        let resolveCancel;
        const cancelBarrier = new Promise((resolve) => {
        ...
        let rejected = false;
        let rejection;
        const pending = postJson(requestArgs()).catch((error) => {
            rejected = true;
            rejection = error;
        });
        await cancelCalled;
        ...
        expect(rejection.message).toBe('External context provider returned an invalid response.');
    });
    it('maps a mid-stream read failure after partial data to a transport error', async () => {
        ...
        vi.stubGlobal('fetch', vi.fn(async () => new Response(body, { status: 200 })));
        await expect(postJson(requestArgs())).rejects.toThrow('External context provider request did not complete.');
