...
- When releasing a new version (e.g., bumping from v0.14.5-no-telemetry to v0.15.10-no-telemetry), ALWAYS update these files with the new version number:
...
1. **Dockerfile**: `ARG QWEN_REF="v[version]-no-telemetry"`
2. **install.sh**: All example version references and usage docs
3. **README.md**: Install script URLs/examples (e.g., `v0.15.10-no-telemetry`) AND the "original README" link version (e.g., `v0.15.10`)
