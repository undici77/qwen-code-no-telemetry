...
import { DEFAULT_REPO_ROOT, gitHead, LISTENING_LINE_RE, makeTempWorkspace, percentiles, sleep, } from './_daemon-harness.js';
import { collectPlatformInfo, formatPercentiles, resolveOutputDir, writeSnapshotArtifacts, } from './_daemon-perf-report.js';
const SANDBOX_ENABLED = Boolean(process.env['QWEN_SANDBOX'] &&
    process.env['QWEN_SANDBOX'].toLowerCase() !== 'false');
const SKIP = process.env['QWEN_BENCHMARK_ENABLED'] !== '1' ||
    SANDBOX_ENABLED ||
    process.platform === 'win32';
const ITERATIONS = Number(process.env['BENCHMARK_ITERATIONS'] ?? 5);
const WARMUP_ITERATIONS = Number(process.env['BENCHMARK_WARMUP_ITERATIONS'] ?? 1);
const BOOT_TIMEOUT_MS = Number(process.env['BENCHMARK_BOOT_TIMEOUT_MS'] ?? 15_000);
const EXTERNAL_P99_MAX_MS = Number(process.env['BENCHMARK_DAEMON_STARTUP_P99_MAX_MS'] ?? 15_000);
const PROCESS_P99_MAX_MS = Number(process.env['BENCHMARK_PROCESS_P99_MAX_MS'] ?? 5_000);
const RUN_SERVE_P99_MAX_MS = Number(process.env['BENCHMARK_RUN_SERVE_P99_MAX_MS'] ?? 5_000);
const STATUS_FETCH_TIMEOUT_MS = Number(process.env['BENCHMARK_DAEMON_STATUS_TIMEOUT_MS'] ?? 5_000);
const STDERR_TIMING_TIMEOUT_MS = Number(process.env['BENCHMARK_STDERR_TIMING_TIMEOUT_MS'] ??
...
const CLI_BIN = process.env['TEST_CLI_PATH'] ??
    path.resolve(DEFAULT_REPO_ROOT, 'dist/cli.js');
const PREHEAT_STATUS_SET = {
...
const PREHEAT_STATUSES = Object.keys(PREHEAT_STATUS_SET);
const snapshot = {
    version: 1,
...
const STDERR_TIMING_RE = /qwen serve: startup timing: processToListenMs=(\d+) runQwenServeToListenMs=(\d+)/;
...
async function waitForStderrTiming(stderr) {
    ...
    if (match)
        return match;
    throw new Error(`startup timing line missing from stderr after ` +
        `${STDERR_TIMING_TIMEOUT_MS}ms:\n${stderr()}`);
}
async function runStartupIteration(iteration, measured) {
    const workspace = makeTempWorkspace(`${iteration}`, 'qwen-daemon-startup-benchmark');
    ...
    try {
        ...
        const listening = await new Promise((resolve, reject) => {
            ...
            const onStdout = (chunk) => {
                ...
                resolve({
                    line,
                    port: Number(port),
                    externalCommandToListeningMs: Math.round(performance.now() - startedAt),
                });
            };
            const onExit = (code, signal) => {
                fail(new Error(`daemon exited before listening: code=${code} signal=${signal}\n` +
                    `stdout=${stdout}\nstderr=${stderr}`));
            };
            const timer = setTimeout(() => {
                fail(new Error(`daemon startup timed out after ${BOOT_TIMEOUT_MS}ms\n` +
                    `stdout=${stdout}\nstderr=${stderr}`));
            }, BOOT_TIMEOUT_MS);
        ...
        try {
            ...
            throw new Error(`/daemon/status fetch failed on iteration ${iteration} ` +
                `(port ${listening.port}): ` +
        ...
        if (measured) {
            ...
            try {
                ...
                console.warn(`[daemon-startup] warmup ${iteration}: startup validation failed: ` +
                    `${err instanceof Error ? err.message : String(err)}`);
            }
        }
        return {
...
(SKIP ? describe.skip : describe)('qwen serve daemon startup benchmark', { retry: 0 }, () => {
    it('measures built CLI startup to listener readiness', async () => {
        ...
        for (let i = 0; i < total; i++) {
            ...
            console.log(`[daemon-startup] ${measured ? 'run' : 'warmup'} ${i}: ` +
        ...
        snapshot.externalCommandToListening = percentiles(measuredRuns.map((run) => run.externalCommandToListeningMs));
        snapshot.processToListen = percentiles(measuredRuns.map((run) => run.processToListenMs));
        snapshot.runQwenServeToListen = percentiles(measuredRuns.map((run) => run.runQwenServeToListenMs));
        expect(snapshot.externalCommandToListening.count).toBe(ITERATIONS);
        expect(snapshot.externalCommandToListening.p99).toBeLessThan(EXTERNAL_P99_MAX_MS);
        expect(snapshot.processToListen.p99).toBeLessThan(PROCESS_P99_MAX_MS);
        expect(snapshot.runQwenServeToListen.p99).toBeLessThan(RUN_SERVE_P99_MAX_MS);
        ...
        (BOOT_TIMEOUT_MS +
            ...
            5_000));
    afterAll(() => {
        if (SKIP || snapshot.runs.length === 0)
            return;
        console.log('\n[daemon-startup] ---- summary ----');
        console.log(`  External command -> listening: ${formatPercentiles(snapshot.externalCommandToListening)}`);
        console.log(`  process -> listening:          ${formatPercentiles(snapshot.processToListen)}`);
        console.log(`  runQwenServe -> listening:     ${formatPercentiles(snapshot.runQwenServeToListen)}`);
        console.log('[daemon-startup] ---- end summary ----\n');
        writeSnapshotArtifacts(OUTPUT_DIR, 'daemon-startup-benchmark', snapshot, renderMarkdown(snapshot), 'daemon-startup');
    });
});
function renderMarkdown(s) {
    const lines = [
        ...
        '## Summary',
        '',
        `- External command -> listening: ${formatPercentiles(s.externalCommandToListening)}`,
    ...
    for (const run of s.runs) {
        lines.push(`| ${run.iteration} | ${run.measured ? 'yes' : 'warmup'} | ` +
            `${run.externalCommandToListeningMs}ms | ${run.processToListenMs}ms | ` +
            `${run.runQwenServeToListenMs}ms | ${run.preheatStatus} |`);
    }
    lines.push('', '## Scope', '', 'This benchmark stops at the stdout listening line and does not create a daemon session. It records the preheat state visible at listen time, but preheat completion is not part of the primary metric. Use `qwen-daemon-vs-cli-benchmark.test.ts` for first-session, memory, and prompt metrics.', '');
    return lines.join('\n');
}
//# sourceMappingURL=qwen-daemon-startup-benchmark.test.js.map