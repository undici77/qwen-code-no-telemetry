...
const expectReadThenNotebookEdit = (rig, result) => {
    ...
    const notebookEditIndex = foundTools.findIndex((name) => name === 'notebook_edit');
    if (readIndex === -1 || notebookEditIndex === -1) {
        printDebugInfo(rig, result, { foundTools });
    }
    expect(readIndex, createToolCallErrorMessage('read_file', foundTools, result)).toBeGreaterThanOrEqual(0);
    expect(notebookEditIndex, createToolCallErrorMessage('notebook_edit', foundTools, result)).toBeGreaterThan(readIndex);
};
...
describe('notebook_edit integration', () => {
    ...
    it('replaces a code cell after reading the notebook and clears stale outputs', async () => {
        ...
        const notebookPath = rig.createFile(fileName, JSON.stringify(baseNotebook([
            {
                id: 'intro',
                cell_type: 'markdown',
                metadata: {},
...
Do not change any other cell.`;
        ...
        expect(sourceText(target.source).trimEnd()).toBe('result = 41 + 1\nprint(result)');
...
Do not change the calculate code cell.`;
        ...
        const successfulNotebookEdits = rig
            .readToolLogs()
            .filter((log) => log.toolRequest.name === 'notebook_edit' && log.toolRequest.success);
        expect(successfulNotebookEdits.length).toBeGreaterThanOrEqual(2);
        const notebook = readNotebook(rig, fileName);
        const cellIds = notebook.cells.map((cell) => cell.id);
        const insertedCell = notebook.cells.find((cell) => sourceText(cell.source).includes('Inserted Note'));
