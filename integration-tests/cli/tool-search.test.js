...
describe('tool-search / deferred tools', () => {
    ...
    it('reveals a deferred tool via select: and lets the model invoke it', async () => {
        ...
        expect(foundList, 'cron_list must succeed after tool_search reveals it').toBeTruthy();
    ...
    it('finds deferred tools via keyword search', async () => {
        ...
        const result = await rig.run('Use the tool_search tool with the keyword query "cron schedule" ' +
        ...
        expect(usedKeyword, `expected at least one keyword tool_search; saw args: ${searchCalls
