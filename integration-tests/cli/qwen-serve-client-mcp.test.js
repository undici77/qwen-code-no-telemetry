...
const SKIP_PROMPTED_MODEL_TEST = Boolean(SANDBOX_MODE && SANDBOX_MODE !== 'false' && SANDBOX_MODE !== '0');
...
beforeAll(async () => {
    ...
    daemon.stderr?.on('data', (c) => {
        stderr += c.toString();
        if (process.env['DEBUG_CLIENT_MCP'])
            process.stderr.write(c);
    });
    port = await new Promise((resolve, reject) => {
        let buf = '';
        const bootTimer = setTimeout(() => reject(new Error(`daemon boot timeout\nstderr=${stderr}`)), 15_000);
        const onData = (chunk) => {
            buf += chunk.toString();
 ...
 * agent forwarded.
 */
function answerHandshakeFrame(frame, onReadPageCall) {
    const { payload } = frame;
    ...
    it('discovers a client-hosted tool end-to-end via the ACP child', async () => {
        ...
        ws.on('message', (data) => {
            ...
            if (typeof msg['id'] === 'number') {
                acpReplies.set(msg['id'], msg);
            }
        });
        const waitForAcp = (id, timeoutMs = 20_000) => new Promise((resolve, reject) => {
        ...
        const sessionId = sessionReply['result']
            ?.sessionId;
        ...
        expect(registeredAck, `expected mcp_registered, got ${JSON.stringify(registeredAck)}`).toMatchObject({ type: 'mcp_registered', server: 'chrome-tools' });
        ...
        const hasReadPage = (toolsBody.tools ?? []).some((t) => t.serverToolName === 'chrome_read_page' ||
    ...
    itPromptedModelMaybe('drives a model→agent tools/call of chrome_read_page over the reverse WS channel and consumes the result', async () => {
        ...
        ws.on('message', (data) => {
            ...
            if (msg['type'] === 'mcp_message') {
                ...
                const reply = answerHandshakeFrame(msg, (args) => readPageCalls.push(args));
                if (reply)
                    ws.send(JSON.stringify(reply));
                return;
            }
        ...
        const { promptId, lastEventId } = (await promptRes.json());
            ...
            try {
                for (;;) {
                    ...
                    while ((sep = buf.indexOf('\n\n')) !== -1) {
                        ...
                        const isTurnComplete = (env.type ?? evType) === 'turn_complete' &&
                            env.data?.promptId === promptId;
                        if (isTurnComplete)
                            return;
                    }
                }
        ...
        if (timedOut) {
            ...
            throw new Error(`timeout waiting for turn_complete; fakeReqs=${fakeServer.requests.length} ` +
                `readPageCalls=${readPageCalls.length} ` +
                `events=${JSON.stringify(events.map((e) => ({
                    t: e.type,
                    u: e.data?.update?.sessionUpdate,
                    s: e.data?.update
                        ?.status,
                })))}`);
        }
        // Collect the tool-call lifecycle the agent surfaced for THIS turn.
        const toolCallUpdates = events.filter((e) => e.type === 'session_update' &&
            (e.data?.update
        ...
        const readPageUpdate = toolCallUpdates.find((e) => {
            ...
            return (meta?.toolName === 'chrome_read_page' ||
        ...
        expect(readPageUpdate, `expected a tool_call(_update) naming chrome_read_page; ` +
            `events=${JSON.stringify(events.map((e) => e.type))}`).toBeDefined();
        ...
        expect(readPageCalls.length, `expected the reverse tools/call to reach the ws client; ` +
            `updates=${JSON.stringify(toolCallUpdates.map((e) => e.data?.update?.status))}`).toBeGreaterThanOrEqual(1);
        ...
        const completed = toolCallUpdates.some((e) => e.data?.update?.status ===
            'completed');
        expect(completed, `expected a completed tool_call_update for chrome_read_page; ` +
            `statuses=${JSON.stringify(toolCallUpdates.map((e) => e.data?.update?.status))}`).toBe(true);
        ...
        const turnComplete = events.find((e) => e.type === 'turn_complete' &&
            e.data?.promptId === promptId);
        expect(turnComplete, 'expected a turn_complete for this prompt').toBeDefined();
        ws.close();
    }, 90_000);
});
//# sourceMappingURL=qwen-serve-client-mcp.test.js.map