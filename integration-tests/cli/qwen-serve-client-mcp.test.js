...
describeMaybe(
  ...
  () => {
    it('discovers a client-hosted tool end-to-end via the ACP child', async () => {
      ...
      const waitForAcp = (id, timeoutMs = 20_000) =>
      ...
      const sessionId = sessionReply['result']?.sessionId;
      ...
      async () => {
        ...
        ws.on('message', (data) => {
          ...
          if (msg['type'] === 'mcp_message') {
            ...
            const reply = answerHandshakeFrame(msg, (args) =>
              readPageCalls.push(args),
            );
            if (reply) ws.send(JSON.stringify(reply));
            return;
          }
        ...
        expect(
          ...
          `expected the reverse tools/call to reach the ws client; ` +
            `updates=${JSON.stringify(toolCallUpdates.map((e) => e.data?.update?.status))}`,
        ...
        const completed = toolCallUpdates.some(
          (e) => e.data?.update?.status === 'completed',
        );
        expect(
          completed,
          `expected a completed tool_call_update for chrome_read_page; ` +
            `statuses=${JSON.stringify(toolCallUpdates.map((e) => e.data?.update?.status))}`,
        ).toBe(true);
        ...
        const turnComplete = events.find(
          (e) => e.type === 'turn_complete' && e.data?.promptId === promptId,
        );
        expect(
          turnComplete,
