...
import { spawnDaemon, percentiles, gitHead, makeTempWorkspace, sleep, } from './_daemon-harness.js';
import { spawnDaemonWithTime, spawnCliWithTime, measureProcessTreeRss, measureCliStartupWithProfiler, } from './_daemon-benchmark-helpers.js';
import { resolveOutputDir, formatPercentiles, writeSnapshotArtifacts, collectPlatformInfo, } from './_daemon-perf-report.js';
// ---------------------------------------------------------------------------
// Skip logic
// ---------------------------------------------------------------------------
const SKIP = process.env['QWEN_BENCHMARK_ENABLED'] !== '1' ||
...
const ITERATIONS = Number(process.env['BENCHMARK_ITERATIONS'] ?? (HEAVY ? 20 : 5));
const CONCURRENT_SESSIONS = Number(process.env['BENCHMARK_CONCURRENT_SESSIONS'] ?? (HEAVY ? 10 : 5));
const THROUGHPUT_WINDOW_S = Number(process.env['BENCHMARK_THROUGHPUT_WINDOW_S'] ?? (HEAVY ? 30 : 10));
const CHURN_ROUNDS = Number(process.env['BENCHMARK_CHURN_ROUNDS'] ?? ITERATIONS * 4);
...
const HAS_MODEL_KEY = PROMPT_CREDENTIAL_ENV_KEYS.some((k) => Boolean(process.env[k])) ||
    Object.entries(process.env).some(([k, v]) => k.startsWith('QWEN_CUSTOM_API_KEY_') && Boolean(v));
...
// Tests
// ---------------------------------------------------------------------------
(SKIP ? describe.skip : describe)('daemon vs CLI benchmark (POSIX-only, QWEN_BENCHMARK_ENABLED=1)', { retry: 0 }, () => {
    // -----------------------------------------------------------------------
    // Phase 1: no-model metrics
    // -----------------------------------------------------------------------
    describe('Phase 1: no-model metrics', () => {
        it('CLI cold start latency (full init via startup profiler)', async () => {
            ...
            try {
                ...
                const cliTimeResult = await spawnCliWithTime(['-p', 'x', '--output-format', 'text'], { cwd: ws });
        ...
        it('daemon boot latency (including first session)', async () => {
            const latencies = [];
            let daemonResourceMetrics = null;
            for (let i = 0; i < ITERATIONS; i++) {
        ...
        it('warm session creation latency', async () => {
        ...
        it('memory baseline (process tree RSS)', async () => {
            ...
            const cliResult = await spawnCliWithTime(['-p', 'x', '--output-format', 'text'], { cwd: ws });
            ...
            try {
                ...
                const rss10 = daemon.daemon.pid
                    ? measureProcessTreeRss(daemon.daemon.pid)
                    : null;
                const growthPerSession = rss0 && rss10
                ...
                if (rss0) {
                    expect(rss0.totalRssMB).toBeLessThan(THRESH.daemonBaselineTreeRssMaxMB);
                }
            }
            finally {
                if (daemon) {
    ...
    describe('Phase 2: model-dependent metrics', () => {
        if (!SKIP_PROMPT) {
            it('single prompt latency — CLI vs daemon', async () => {
                ...
                try {
                    ...
                    expect(snapshot.singlePromptLatency.cli.p99).toBeLessThan(THRESH.promptP99MaxMs);
                    expect(snapshot.singlePromptLatency.daemon.p99).toBeLessThan(THRESH.promptP99MaxMs);
                }
                finally {
                    if (daemon)
                        await daemon.dispose();
            ...
            it('concurrent queuing latency (daemon only)', async () => {
                ...
                try {
                    ...
                    snapshot.concurrentQueueingLatency = {
                        ...
                        wallClockMs,
                        promptsPerSec: Math.round((successCount / (wallClockMs / 1000)) * 100) / 100,
                        perPromptLatency: percentiles(perPromptLatencies),
                        successCount,
                        failureCount,
                        skipped: false,
                    };
                    expect(wallClockMs).toBeLessThan(CONCURRENT_SESSIONS * THRESH.promptP99MaxMs);
                }
                finally {
                    if (daemon) {
                        for (const sid of sessionIds) {
        ...
        if (SKIP_PROMPT) {
            it('prompt tests skipped (no model credential env)', () => {
                snapshot.singlePromptLatency = {
                    ...
                    daemon: null,
                    skipped: true,
                    skipReason: 'No recognized model credential env var is set. ' +
    ...
    describe('Phase 3: stress tests', () => {
        it('concurrent burst — N simultaneous session creations (daemon)', async () => {
            ...
            try {
                ...
                const latencies = [];
                const results = await Promise.allSettled(Array.from({ length: CONCURRENT_SESSIONS }, async () => {
                    const t0 = performance.now();
                    const s = await daemon.client.createOrAttachSession({
                        workspaceCwd: ws,
                        sessionScope: 'thread',
        ...
        it('sustained throughput — session create+close ops/sec (daemon)', async () => {
            ...
            try {
                ...
                snapshot.throughputStress = {
                    anchored: {
                        opsPerSec: Math.round((anchoredOps / THROUGHPUT_WINDOW_S) * 100) / 100,
                        totalOps: anchoredOps,
                    },
                    unanchored: {
                        opsPerSec: Math.round((unanchoredOps / THROUGHPUT_WINDOW_S) * 100) / 100,
        ...
        it('session churn + leak detection (daemon only)', async () => {
            ...
            try {
                ...
                const rssAfter = daemon.daemon.pid
                    ? measureProcessTreeRss(daemon.daemon.pid)
                    : null;
                const rssDrift = rssBefore && rssAfter
                    ? Math.round((rssAfter.totalRssMB - rssBefore.totalRssMB) * 10) / 10
                    : 0;
        ...
        it('SSE connection flood (daemon only)', async () => {
            ...
            try {
                ...
                const connectionResults = await Promise.allSettled(Array.from({ length: N }, async () => {
                    const ac = new AbortController();
                    abortControllers.push(ac);
                    const timer = setTimeout(() => ac.abort(), 10_000);
                    try {
                        for await (const ev of daemon.client.subscribeEvents(sessionId, { signal: ac.signal, lastEventId: 0 })) {
                ...
                const allConnected = connectionResults.filter((r) => r.status === 'fulfilled').length ===
                    N;
    ...
    afterAll(() => {
        if (SKIP)
            return;
        // Console summary
        const fmtP = (p) => p && p.count > 0
            ? `p50=${p.p50.toFixed(0)}ms  p90=${p.p90.toFixed(0)}ms  p99=${p.p99.toFixed(0)}ms  (n=${p.count})`
            : 'n/a';
        console.log('\n[benchmark] ---- daemon vs CLI summary ----');
        console.log(`  CLI cold start:      ${fmtP(snapshot.cliColdStart)}${snapshot.cliColdStart?.peakRssMB
            ? `  RSS=${snapshot.cliColdStart.peakRssMB}MB`
            : ''}`);
        const sp = snapshot.cliColdStart?.startupPhases;
        if (sp) {
            console.log(`    phases:            module=${sp.moduleLoadMs ?? '?'}ms  configInit=${sp.configInitMs ?? '?'}ms  mcpSettled=${sp.mcpSettledMs ?? '?'}ms`);
        }
        console.log(`  Daemon boot+1st:     ${fmtP(snapshot.daemonBootLatency)}`);
        console.log(`  Warm session create: ${fmtP(snapshot.warmSessionCreation)}`);
        if (snapshot.memoryBaseline) {
            const mb = snapshot.memoryBaseline;
            const d0 = mb.daemon0Sessions;
            const d10 = mb.daemon10Sessions;
            console.log(`  Memory (daemon 0s):  ${d0 ? `total=${Math.round(d0.totalRssMB * 10) / 10}MB (daemon=${d0.daemonRssMB} acp=${d0.acpChildRssMB} mcp=${d0.mcpChildrenRssMB})` : 'n/a'}`);
            console.log(`  Memory (daemon 10s): ${d10 ? `total=${Math.round(d10.totalRssMB * 10) / 10}MB (+${mb.growthPerSessionMB}MB/session)` : 'n/a'}`);
            console.log(`  Memory (CLI -p init):  ${mb.cliVersionPeakRssMB ? `${mb.cliVersionPeakRssMB}MB` : 'n/a'}`);
        }
        ...
        if (cql && !cql.skipped) {
            console.log(`  Concurrent (${cql.sessionCount}x):   ${cql.promptsPerSec} prompts/sec  wall=${cql.wallClockMs.toFixed(0)}ms  success=${cql.successCount}/${cql.totalPrompts}`);
        }
        // Phase 3 stress tests
        const burst = snapshot.burstStress;
        if (burst) {
            console.log(`  Burst (${burst.concurrency}x):      ${fmtP(burst.latency)}  success=${(burst.successRate * 100).toFixed(0)}%`);
        }
        const tp = snapshot.throughputStress;
        if (tp) {
            console.log(`  Throughput anchored: ${tp.anchored.opsPerSec} ops/sec (${tp.anchored.totalOps} ops in ${tp.windowSeconds}s)`);
            console.log(`  Throughput cold:     ${tp.unanchored.opsPerSec} ops/sec (${tp.unanchored.totalOps} ops in ${tp.windowSeconds}s, incl. ACP respawn)`);
        }
        const churn = snapshot.sessionChurn;
        if (churn) {
            console.log(`  Session churn:       ${fmtP(churn.latency)}  RSS drift=${churn.rssDriftMB}MB (${churn.rounds} rounds)`);
        }
        const lim = snapshot.sessionLimitSaturation;
        if (lim) {
            console.log(`  Limit saturation:    enforced=${lim.limitEnforced} recovery=${lim.recoverySucceeded} code=${lim.errorCode}`);
        }
        const sse = snapshot.sseConnectionFlood;
        if (sse) {
            console.log(`  SSE flood:           ${sse.connectionsOpened} connections  allConnected=${sse.allConnected}  healthy=${sse.daemonHealthyAfter}`);
        }
        const rp = snapshot.resourceProfile;
        if (rp) {
            const fmtRes = (label, m) => {
                if (!m)
                    return;
                const parts = [
        ...
        writeSnapshotArtifacts(OUTPUT_DIR, 'daemon-vs-cli-benchmark', snapshot, renderMarkdown(snapshot), 'benchmark');
    });
});
// ---------------------------------------------------------------------------
// Markdown renderer
...
function renderMarkdown(s) {
    ...
    const lines = [
        ...
        s.cliColdStart
            ? (() => {
                ...
                if (sp2) {
                    lines2.push(`- Phase breakdown: module_load=${sp2.moduleLoadMs ?? '?'}ms, config_init=${sp2.configInitMs ?? '?'}ms, mcp_settled=${sp2.mcpSettledMs ?? '?'}ms`);
                }
                lines2.push(`- *Measures full CLI init: Node startup + ESM + config + MCP discovery + auth (via startup profiler)*`);
                return lines2.join('\n');
            })()
            : 'not run',
        ``,
    ...
    if (s.memoryBaseline) {
        ...
        lines.push(`- CLI peak RSS (full init): ${mb.cliVersionPeakRssMB ?? 'n/a'} MB`, `- Daemon at 1 session: ${fmtTree(mb.daemon0Sessions)}`, `- Daemon at 5 sessions: ${fmtTree(mb.daemon5Sessions)}`, `- Daemon at 10 sessions: ${fmtTree(mb.daemon10Sessions)}`, `- Growth per session: ${mb.growthPerSessionMB ?? 'n/a'} MB`);
    }
    else {
        lines.push('not run');
    }
    ...
    if (spl) {
        if (spl.skipped) {
            ...
            lines.push(`| Metric | CLI | Daemon |`, `|--------|-----|--------|`, `| p50    | ${spl.cli?.p50.toFixed(0) ?? '-'}ms | ${spl.daemon?.p50.toFixed(0) ?? '-'}ms |`, `| p90    | ${spl.cli?.p90.toFixed(0) ?? '-'}ms | ${spl.daemon?.p90.toFixed(0) ?? '-'}ms |`, `| p99    | ${spl.cli?.p99.toFixed(0) ?? '-'}ms | ${spl.daemon?.p99.toFixed(0) ?? '-'}ms |`, `| mean   | ${spl.cli?.mean.toFixed(0) ?? '-'}ms | ${spl.daemon?.mean.toFixed(0) ?? '-'}ms |`, ``, `*CLI = end-to-end (spawn+init+model+exit). Daemon = HTTP round-trip+model. Difference ≈ CLI startup amortization.*`);
        }
    }
    else {
        lines.push('not run');
    ...
    if (cql) {
        if (cql.skipped) {
            ...
            lines.push(`- Sessions: ${cql.sessionCount}`, `- Wall clock: ${cql.wallClockMs.toFixed(0)}ms`, `- Throughput: ${cql.promptsPerSec} prompts/sec`, `- Success: ${cql.successCount}/${cql.totalPrompts}`, `- Per-prompt latency: ${fmtP(cql.perPromptLatency)}`, ``, `*Stage 1 single-ACP-child mode — prompts queue serially at the ACP level.*`);
        }
    }
    else {
        lines.push('not run');
    ...
    if (burst) {
        lines.push(`- Concurrency: ${burst.concurrency}`, `- Latency: ${fmtP(burst.latency)}`, `- Success rate: ${(burst.successRate * 100).toFixed(0)}%`, `- *Measures ${burst.concurrency} simultaneous session creations on a warm daemon (ACP child already running)*`);
    }
    else {
        lines.push('not run');
    }
    ...
    if (tp) {
        lines.push(`| Mode | ops/sec | total ops | window |`, `|------|---------|-----------|--------|`, `| Anchored (ACP stays alive) | ${tp.anchored.opsPerSec} | ${tp.anchored.totalOps} | ${tp.windowSeconds}s |`, `| Unanchored (ACP respawns each cycle) | ${tp.unanchored.opsPerSec} | ${tp.unanchored.totalOps} | ${tp.windowSeconds}s |`, ``, `*Anchored: 3 sessions keep ACP child alive during create+close cycles (steady-state). Unanchored: each close kills ACP child, next create respawns it (cold-cycle cost).*`);
    }
    else {
        lines.push('not run');
    }
    ...
    if (churn) {
        lines.push(`- Rounds: ${churn.rounds}`, `- Latency: ${fmtP(churn.latency)}`, `- RSS drift: ${churn.rssDriftMB} MB`, `- *Drift < 100MB is normal V8 fragmentation; > 100MB indicates potential leak*`);
    }
    else {
        lines.push('not run');
    }
    ...
    if (lim) {
        lines.push(`- Limit enforced: ${lim.limitEnforced}`, `- Error code: ${lim.errorCode}`, `- Recovery after close: ${lim.recoverySucceeded}`);
    }
    else {
        lines.push('not run');
    }
    ...
    if (sse) {
        lines.push(`- Connections opened: ${sse.connectionsOpened}`, `- All connected: ${sse.allConnected}`, `- Daemon healthy after: ${sse.daemonHealthyAfter}`);
    }
    else {
        lines.push('not run');
    }
...
//# sourceMappingURL=qwen-daemon-vs-cli-benchmark.test.js.map