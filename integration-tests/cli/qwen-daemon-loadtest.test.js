...
import { spawnDaemon, percentiles, consumeSseEvents, gitHead, makeTempWorkspace, sleep, } from './_daemon-harness.js';
...
// Skip logic
// ---------------------------------------------------------------------------
const SKIP = process.env['QWEN_LOADTEST_ENABLED'] !== '1' ||
...
// Helpers
// ---------------------------------------------------------------------------
function mockDaemonEnv(mode = 'echo') {
    return {
...
async function withDaemon(opts, fn) {
    ...
    try {
        ...
        if (d) {
            console.error(`[loadtest:${opts.label}] stdout:\n${d.stdoutBuf.value}\nstderr:\n${d.stderrBuf.value}`);
        }
        throw err;
    }
    finally {
...
// Tests
// ---------------------------------------------------------------------------
(SKIP ? describe.skip : describe).sequential('daemon connection stress test (mock ACP, POSIX-only)', { retry: 0 }, () => {
    afterEach(() => {
        if (activeDaemon?.daemon.exitCode === null) {
            try {
    ...
    it('SSE slow consumer triggers eviction through HTTP', async () => {
        ...
        let evicted = false;
        let received = 0;
        await withDaemon({ label: 'sse-eviction', extraArgs: ['--event-ring-size', '32'] }, async (d) => {
            const session = await d.client.createOrAttachSession({
                sessionScope: 'thread',
            });
    ...
    it('Last-Event-ID reconnect under concurrent load', async () => {
        ...
        await withDaemon({ label: 'reconnect' }, async (d) => {
            ...
            if (initial.lastSeenId !== undefined) {
                const reconnect = await consumeSseEvents(d.client, session.sessionId, {
                    maxEvents: 5,
                    timeoutMs: 10_000,
    ...
    it('ACP child crash → session error recovery', async () => {
        ...
        await withDaemon({ label: 'crash-recovery', mode: 'crash-on-prompt', skipWarmup: true }, async (d) => {
            const session = await d.client.createOrAttachSession({
                sessionScope: 'thread',
            });
            try {
    ...
    it('burst: concurrent sessions with mock prompts', async () => {
        ...
        await withDaemon({ label: 'burst' }, async (d) => {
            const results = await Promise.allSettled(Array.from({ length: BURST_SESSIONS }, async (_, i) => {
                const t = performance.now();
                const session = await d.client.createOrAttachSession({
                    sessionScope: 'thread',
                });
    ...
    afterAll(() => {
        if (SKIP)
            return;
        writeSnapshotArtifacts(OUTPUT_DIR, 'loadtest-report', snapshot, renderMarkdown(snapshot), 'loadtest');
    });
});
// ---------------------------------------------------------------------------
// Markdown renderer
...
function renderMarkdown(s) {
    ...
    for (const sc of s.scenarios) {
        lines.push(`### ${sc.name}`, `- Status: ${sc.status}`, `- Duration: ${sc.durationMs.toFixed(0)}ms`);
        if (sc.error) {
        ...
        if (sc.metrics) {
            ...
            if ('p50' in m &&
                ...
                lines.push(`- Latency: ${formatPercentiles({
                    count: m['count'],
                    p50: m['p50'],
                    p90: m['p90'],
                    p99: m['p99'],
...
//# sourceMappingURL=qwen-daemon-loadtest.test.js.map