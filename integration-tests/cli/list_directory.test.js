...
import { fakeServerHostOptions, IS_CONTAINER_SANDBOX, CONTAINER_SANDBOX_NO_PROXY, TestRig, } from '../test-helper.js';
...
describe('list_directory', () => {
    ...
    it('should be able to list a directory', async () => {
        ...
        try {
            ...
            await rig.run(prompt, '--auth-type', 'openai', '--model', 'fake-model', '--openai-base-url', fakeServer.baseUrl, '--openai-api-key', 'fake-key');
            ...
            const toolResultRequest = fakeServer.requests.find(({ body }) => {
                ...
                return (Array.isArray(messages) &&
                    messages.some((message) => typeof message === 'object' &&
                        message !== null &&
                        'role' in message &&
                        message.role === 'tool'));
            });
            expect(toolResultRequest, 'Expected a model request containing the list_directory result').toBeDefined();
            const messages = toolResultRequest?.body['messages'];
            const toolResultContent = JSON.stringify(messages?.find((message) => message.role === 'tool')?.content ?? '');
