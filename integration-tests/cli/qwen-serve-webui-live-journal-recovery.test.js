...
beforeAll(async () => {
    ...
    ({ DaemonSessionProvider, useTranscriptBlocks } = await import('@qwen-code/webui/daemon-react-sdk'));
});
afterAll(() => {
    dom.window.close();
    ...
    it('repairs once after terminal without another model request', async () => {
        ...
        try {
            ...
            const sawLastChunk = (async () => {
                for await (const event of activeDaemon.client.subscribeEvents(created.sessionId, { signal: observerAbort.signal })) {
                    if (eventText(event) === 'chunk-19')
                        return;
                }
            })();
            ...
            await waitFor(() => blocks.some((block) => 'source' in block && block.source === 'history_truncated'), 'visible live journal marker');
            await act(async () => {
                await prompt;
            });
            await waitFor(() => JSON.stringify(blocks).includes('chunk-0') &&
                JSON.stringify(blocks).includes('chunk-19') &&
                !blocks.some((block) => 'source' in block && block.source === 'history_truncated'), 'atomically repaired complete turn');
            expect(requestPaths.filter((pathname) => pathname.endsWith('/load'))).toHaveLength(2);
            expect(requestPaths.filter((pathname) => pathname.endsWith('/prompt'))).toHaveLength(1);
...
//# sourceMappingURL=qwen-serve-webui-live-journal-recovery.test.js.map