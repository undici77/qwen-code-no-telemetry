...
(IS_SANDBOX ? describe.skip : describe)('acp cron integration', () => {
    it('cron job fires and streams results via sessionUpdate after prompt returns', async () => {
        ...
        const { sendRequest, cleanup, stderr, 
        // sessionUpdates available for debugging
        waitForSessionUpdate, } = setupAcpCronTest(rig);
        try {
            ...
            const cronUserMsg = await waitForSessionUpdate((u) => u.update?.sessionUpdate === 'user_message_chunk' &&
                ...
                u.receivedAt > promptDoneAt, 'cron-fired user_message_chunk with CRONFIRE7742', 75_000);
            ...
            const cronAgentMsg = await waitForSessionUpdate((u) => u.update?.sessionUpdate === 'agent_message_chunk' &&
