...
import { startFakeOpenAIServer, fakeToolCall, } from '../fake-openai-server.js';
 ...
 */
function setupAcpCronTest(rig, fakeServer) {
    ...
    const qwenHome = join(dirname(rig.testDir), `${basename(rig.testDir)}-home`);
    rmSync(qwenHome, { recursive: true, force: true });
    mkdirSync(qwenHome, { recursive: true });
    const agent = spawn('node', [rig.bundlePath, '--acp', '--no-chat-recording'], {
        cwd: rig.testDir,
     ...
     */
    const waitForSessionUpdate = async (predicate, description, timeoutMs) => {
        ...
        throw new Error(`Timed out waiting for sessionUpdate: ${description} (after ${timeoutMs}ms, ` +
...
async function initSession(sendRequest, testDir) {
...
(IS_SANDBOX ? describe.skip : describe)('acp cron integration', () => {
    it('cron job fires and streams results via sessionUpdate after prompt returns', async () => {
        ...
        try {
            const { sendRequest, cleanup, stderr, waitForSessionUpdate } = setupAcpCronTest(rig, fakeServer);
            try {
                ...
                expect(JSON.stringify(fakeServer.requests[0]?.body['messages']), 'requestIndex 0 was not the cron_create prompt — dispatch shifted').toContain('CRONFIRE7742');
                ...
                const cronUserMsg = await waitForSessionUpdate((u) => u.update?.sessionUpdate === 'user_message_chunk' &&
                    u.update?._meta?.source === 'cron', "cron-sourced user_message_chunk (_meta.source === 'cron')", 75_000);
                ...
                await waitForSessionUpdate((u) => u.update?.sessionUpdate === 'agent_message_chunk' &&
                    u.receivedAt > cronUserMsg.receivedAt, 'agent_message_chunk after cron fire', 15_000);
            }
            catch (e) {
                if (stderr.length)
                    console.error('Agent stderr:', stderr.join(''));
                console.error('Fake server requests:', fakeServer.requests.map((r, i) => ({
                    index: i,
                    model: r.body['model'],
...
//# sourceMappingURL=acp-cron.test.js.map