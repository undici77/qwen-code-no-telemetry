...
describe('qwen serve WebUI transactional same-session refresh', () => {
        ...
        try {
            globalThis.fetch = async (input, init) => {
                ...
                if (request.method === 'POST' &&
                    new URL(request.url).pathname.endsWith(`/session/${encodeURIComponent(state.source.sessionId)}/load`)) {
            ...
            await waitFor(() => JSON.stringify(state.getBlocks()).includes('live during refresh'), 'live source tail');
            await act(async () => {
    ...
    it('preserves the transcript across a controlled clientId rebind', async () => {
        const state = await setup();
        const rebound = await activeDaemon.client.resumeSession(state.source.sessionId, { workspaceCwd: state.workspace });
        const reboundClientId = rebound.clientId;
        expect(reboundClientId).toBeTruthy();
        await state.render(reboundClientId);
        await waitFor(() => state.getConnection()?.clientId === reboundClientId, 'clientId rebind');
        expect(JSON.stringify(state.getBlocks())).toContain('source transcript');
        await act(async () => {
            await state.getActions().sendPrompt('prompt after rebind');
        });
        await waitFor(() => JSON.stringify(state.getBlocks()).includes('prompt after rebind'), 'prompt after clientId rebind');
    }, 30_000);
});
//# sourceMappingURL=qwen-serve-webui-same-session-refresh.test.js.map