...
function updateKind(event) {
    if (event.type !== 'session_update' || !event.data)
        return undefined;
...
describe('qwen serve live journal recovery', () => {
    it('keeps a compatible live chunk stream within the replay entry cap', async () => {
        ...
        try {
            ...
            while (Date.now() < deadline) {
                const loaded = await DaemonSessionClient.load(activeDaemon.client, created.sessionId, {}, 'live-journal-observer');
            ...
            expect(duringTurn.replaySnapshot.liveJournal).not.toContainEqual(expect.objectContaining({ type: 'history_truncated' }));
    ...
    it('attributes a truncated live tail and exposes the complete turn after terminal', async () => {
        ...
        try {
            ...
            while (Date.now() < deadline) {
                const loaded = await DaemonSessionClient.load(activeDaemon.client, created.sessionId, {}, 'live-journal-observer');
                if (loaded.replaySnapshot.liveJournal.some((event) => event.type === 'history_truncated')) {
                    duringTurn = loaded;
                    break;
                }
                await new Promise((resolve) => setTimeout(resolve, 25));
            ...
            expect(duringTurn).toBeDefined();
            const marker = duringTurn.replaySnapshot.liveJournal.find((event) => event.type === 'history_truncated');
            ...
            expect(marker?.data
                ?.truncatedEvents).toBeGreaterThan(0);
            ...
            expect(JSON.stringify(duringTurn.replaySnapshot.liveJournal)).not.toContain('chunk-0');
            await prompt;
            const repaired = await DaemonSessionClient.load(activeDaemon.client, created.sessionId, {}, 'live-journal-observer');
            const completeReplay = repaired.replaySnapshot.compactedReplay;
            expect(repaired.replaySnapshot.liveJournal).toEqual([]);
            expect(completeReplay.find((event) => updateKind(event) === 'user_message_chunk')).toMatchObject({ promptId: marker?.promptId });
            expect(completeReplay).toContainEqual(expect.objectContaining({
                type: 'turn_complete',
            ...
            expect(replayText).toContain('chunk-0');
            expect(replayText).toContain('chunk-19');
            expect(completeReplay).not.toContainEqual(expect.objectContaining({ type: 'history_truncated' }));
        }
        finally {
            await activeDaemon?.dispose();
            activeDaemon = undefined;
...
//# sourceMappingURL=qwen-serve-live-journal-recovery.test.js.map