...
import { SESSION_TRANSCRIPT_MAX_INDEX_BYTES, Storage, } from '@qwen-code/qwen-code-core';
...
function chatRecord(sessionId, uuid, parentUuid, text) {
...
beforeAll(async () => {
    ...
    port = await new Promise((resolve, reject) => {
        ...
        const bootTimer = setTimeout(() => reject(new Error('daemon boot timeout')), 25_000);
        const onData = (chunk) => {
            buf += chunk.toString();
...
describe('qwen serve — transcript paging route', () => {
    const getTranscript = (sessionId, query = '') => fetch(`${base}/session/${sessionId}/transcript${query}`, {
        headers: { Authorization: `Bearer ${TOKEN}` },
    });
    it('serves persisted transcript pages through the SDK helper', async () => {
        ...
        expect(first.events.every((event) => event.type === 'session_update')).toBe(true);
        ...
        expect(second.events.every((event) => event.type === 'session_update')).toBe(true);
    ...
    it('maps transcript request validation errors through the real daemon', async () => {
        ...
        const invalidCursor = await getTranscript(sessionId, '?cursor=not-a-cursor');
    ...
    it('maps archived, conflicting, and unavailable transcript snapshots to 409', async () => {
        ...
        const archivedRecord = chatRecord(archivedId, 'u1', null, 'archived transcript');
        ...
        const conflictRecord = chatRecord(conflictId, 'u1', null, 'conflicting transcript');
        ...
        const unavailable = await getTranscript('99999999-aaaa-bbbb-cccc-666666666666', '?cursor=stale');
...
describe('qwen serve — POST /session validation + concurrent coalescing', () => {
    ...
    it('honors and reserves a normalized caller-supplied session ID', async () => {
        ...
        try {
            ...
            await client.closeSession(normalizedId);
            const acpTransport = new AcpWsTransport(`ws://127.0.0.1:${port}/acp`, TOKEN);
...
describe('qwen serve — SSE Content-Type guard (SDK side)', () => {
    it('throws DaemonHttpError when upstream returns 200 + JSON', async () => {
        const ghostFetch = async () => new Response(JSON.stringify({ ok: true }), {
            status: 200,
        ...
        expect(threw.message).toMatch(/text\/event-stream/);
    });
});
...
describe('qwen serve — DELETE /session/:id', () => {
    it('204 on explicit close', async () => {
        ...
        await client.closeSession(session.sessionId);
        const sessions = await client.listWorkspaceSessions(REPO_ROOT);
        expect(sessions.some((s) => s.sessionId === session.sessionId)).toBe(false);
    });
...
describe('qwen serve — prompt clientId admission', () => {
    ...
    it('rejects an unregistered prompt clientId and re-registers via resume', async () => {
        ...
        const rejected = await client
            .promptNonBlocking(session.sessionId, prompt, undefined, 'client-never-registered')
            .catch((err) => err);
        ...
        const accepted = await client.promptNonBlocking(session.sessionId, prompt, undefined, reattached.clientId);
...
//# sourceMappingURL=qwen-serve-routes.test.js.map