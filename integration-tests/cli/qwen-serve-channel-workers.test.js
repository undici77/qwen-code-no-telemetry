...
import { createMockServer, } from '../../packages/channels/plugin-example/src/index.js';
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = path.resolve(__dirname, '../..');
const CLI_BIN = process.env['TEST_CLI_PATH'] ?? path.join(REPO_ROOT, 'dist', 'cli.js');
const TOKEN = 'multi-workspace-channel-test-token';
let daemon;
let primaryServer;
let secondaryServer;
let testRoot;
async function createClosedWebSocketUrl() {
...
async function waitForRunningWorkers(baseUrl) {
    ...
    throw new Error(`workers did not reach running state: ${JSON.stringify(lastStatus)}`);
}
async function waitForRunningControl(baseUrl) {
    const deadline = Date.now() + 15_000;
    let lastState;
    while (Date.now() < deadline) {
    ...
    throw new Error(`runtime channel did not reach running state: ${JSON.stringify(lastState)}`);
}
afterEach(async () => {
...
describe('qwen serve multi-workspace channel workers', () => {
    it('controls a real mock-plugin worker after a channel-less boot', async () => {
        ...
        const spawnDaemon = () => spawn(process.execPath, [
            CLI_BIN,
            'serve',
            '--hostname',
            '127.0.0.1',
    ...
    it('returns partial startup failures from control and daemon status', async () => {
        ...
        expect(control.workers[0]?.startupFailures).toEqual(enabled.state.workers[0]?.startupFailures);
    ...
    it('returns attempted failures on 502 and does not retain them after rollback', async () => {
        ...
        const failed = (await enable.json());
    ...
    it('starts real workers for primary and secondary workspaces', async () => {
        ...
        try {
            ...
            throw new Error(`workers did not connect (daemon exitCode=${daemon.exitCode}, signal=${daemon.signalCode})`, { cause: error });
        }
        const status = await waitForRunningWorkers(baseUrl);
        expect(status.runtime?.channelWorkers).toEqual(expect.arrayContaining([
            expect.objectContaining({
...
//# sourceMappingURL=qwen-serve-channel-workers.test.js.map