...
import { makeTempWorkspace, spawnDaemon, } from './_daemon-harness.js';
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const MOCK_AGENT_PATH = path.resolve(__dirname, '../fixtures/mock-acp-child/agent.mjs');
let activeDaemon;
let root;
let dom;
let createRoot;
...
let useActions;
let useConnection;
let useTranscriptBlocks;
let restoreSessionRequestRecorder;
const originalGlobalDescriptors = new Map(['window', 'document', 'navigator', 'IS_REACT_ACT_ENVIRONMENT'].map((key) => [key, Object.getOwnPropertyDescriptor(globalThis, key)]));
beforeAll(async () => {
...
function installSessionRequestRecorder(respond) {
...
async function waitFor(condition, description, timeoutMs = 8_000) {
...
describe('qwen serve WebUI transactional session switching', () => {
    async function setup() {
        ...
        await act(async () => {
            root?.render(createElement(DaemonSessionProvider, {
                autoConnect: true,
        ...
        await waitFor(() => connection?.status === 'connected' &&
            ...
            connection.capabilities?.features.includes('client_identity') ===
                true &&
            JSON.stringify(blocks).includes('source transcript'), 'source session bootstrap');
        return {
            ...
            getActions: () => {
                if (!actions)
                    throw new Error('session actions unavailable');
                return actions;
            },
    ...
    async function setupActiveSource(options) {
        ...
        const recorder = installSessionRequestRecorder((request) => {
            if (options?.targetLoadTimeout === true &&
                request.method === 'POST' &&
                new URL(request.url).pathname.endsWith(`/session/${encodeURIComponent(target.sessionId)}/load`)) {
        ...
        await waitFor(() => connection?.status === 'connected' &&
            connection.sessionId === source.sessionId &&
            connection.capabilities?.features.includes('client_identity') === true, 'active source bootstrap');
        ...
        expect((await activeDaemon.client.sessionStatus(source.sessionId))
    ...
    async function cleanupActiveSource(state) {
        state.recorder.stop();
        try {
            await act(async () => {
    ...
    function executionRequests(requests) {
    ...
    it('keeps an active source running across A to B to A navigation', async () => {
        ...
        try {
            ...
            expect((await activeDaemon.client.sessionStatus(state.source.sessionId))
                .hasActivePrompt).toBe(true);
            await act(async () => {
        ...
        try {
            globalThis.fetch = async (input, init) => {
                ...
                if (request.method === 'POST' &&
                    new URL(request.url).pathname.endsWith(`/session/${encodeURIComponent(state.target.sessionId)}/load`)) {
            ...
            await waitFor(() => JSON.stringify(state.getBlocks()).includes('source remains live'), 'source event while target response is held');
            await act(async () => {
            ...
            expect(JSON.stringify(state.getBlocks())).not.toContain('source remains live');
        }
        finally {
            globalThis.fetch = originalFetch;
            releaseResponse.resolve();
    ...
    it('serializes non-equivalent load and resume requests for one target', async () => {
        ...
        try {
            ...
            await waitFor(() => JSON.stringify(state.getBlocks()).includes('source remains live while queued'), 'source event while resume is queued');
            await act(async () => {
                releaseLoadResponse.resolve();
                await resumeOutcome;
            });
            expect(resumeRequests).toBe(1);
            await waitFor(() => targetDetachRequests === 1, 'stale target attachment cleanup');
    ...
    it('preserves the source after a structured target timeout', async () => {
        ...
        try {
            ...
            await waitFor(() => JSON.stringify(state.getBlocks()).includes('source after timeout'), 'source event after target timeout');
        }
        finally {
            globalThis.fetch = originalFetch;
            if (root) {
...
//# sourceMappingURL=qwen-serve-webui-session-switching.test.js.map