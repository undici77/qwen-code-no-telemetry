...
import { DEFAULT_BOOTSTRAP_ITERATIONS, FIRST_OUTPUT_BENCHMARK_VERSION, FirstOutputTracker, classifyFirstOutputEvent, computeFirstOutputSessionTimings, evaluateSingleBundlePrototypeGate, findInvalidTimings, measuredPairCountForDwell, nullablePercentiles, parseBenchmarkPostSessionDwell, pairedCandidateControlStats, percentiles, renderFirstOutputBenchmarkMarkdown, validateExpectedFinalText, validatePromptAcceptance, } from './_first-output-benchmark.js';
describe('measuredPairCountForDwell', () => {
    ...
    it('rejects sample counts that could mislabel a diagnostic or decision run', () => {
        expect(() => measuredPairCountForDwell(0, '10')).toThrow('0ms dwell requires exactly 30 measured pairs');
        expect(() => measuredPairCountForDwell(500, '30')).toThrow('500ms dwell requires exactly 10 measured pairs');
        expect(() => measuredPairCountForDwell(100, '20')).toThrow('expected exactly 10 or 30');
    });
});
describe('parseBenchmarkPostSessionDwell', () => {
    ...
    it('rejects unsupported and timer-overflow dwell values', () => {
        expect(() => parseBenchmarkPostSessionDwell('1')).toThrow('expected exactly 0, 100, or 500');
        expect(() => parseBenchmarkPostSessionDwell('2147483648')).toThrow('expected exactly 0, 100, or 500');
    });
});
function sessionUpdate(promptId, update, id = 1, meta) {
    return {
        id,
        v: 1,
        type: 'session_update',
...
function answer(promptId, text, id = 1) {
    return sessionUpdate(promptId, {
        sessionUpdate: 'agent_message_chunk',
        content: { type: 'text', text },
    }, id);
}
function thought(promptId, text, id = 1) {
    return sessionUpdate(promptId, {
        sessionUpdate: 'agent_thought_chunk',
        content: { type: 'text', text },
    }, id);
}
function userEcho(promptId, id = 1) {
    return sessionUpdate(promptId, {
        sessionUpdate: 'user_message_chunk',
        content: { type: 'text', text: 'benchmark prompt' },
    }, id);
}
function toolCall(promptId, id = 1) {
    return sessionUpdate(promptId, {
...
function turnComplete(promptId, id = 10) {
    return {
        id,
        v: 1,
        type: 'turn_complete',
...
function turnError(promptId, message, id = 10) {
    return {
        id,
        v: 1,
        type: 'turn_error',
...
describe('classifyFirstOutputEvent', () => {
    ...
    it('classifies thought text and an initial tool call as output', () => {
        expect(classifyFirstOutputEvent(thought('prompt-1', 'thinking'), 'prompt-1')).toMatchObject({
            type: 'output',
            kind: 'thought_text',
            text: 'thinking',
        });
        expect(classifyFirstOutputEvent(toolCall('prompt-1'), 'prompt-1')).toMatchObject({
            type: 'output',
            kind: 'tool_call',
            toolCallId: 'call-1',
        });
    ...
    it('excludes empty, replayed, diagnostic, user, and update-only frames', () => {
        const excluded = [
            ...
            answer('prompt-1', 'IMPORTANT: This conversation approached the input token limit. ' +
    ...
    it('requires exact prompt correlation for output and terminal events', () => {
        expect(classifyFirstOutputEvent(answer('other-prompt', 'wrong'), 'prompt-1')).toEqual({ type: 'ignore', reason: 'wrong_prompt_id' });
        const wrongNestedPrompt = turnComplete('prompt-1');
        wrongNestedPrompt.data['promptId'] =
            'other-prompt';
...
describe('FirstOutputTracker', () => {
    ...
    it('records the matching relayed user echo without counting it as model output', () => {
        ...
        const updateLevelReplayedEcho = sessionUpdate('prompt-1', {
            sessionUpdate: 'user_message_chunk',
            content: { type: 'text', text: 'benchmark prompt' },
            _meta: { qwenTranscript: { sourceRecordIds: ['record-1'] } },
        }, 6);
...
describe('computeFirstOutputSessionTimings', () => {
    it('splits prompt acceptance from Provider arrival on the parent clock', () => {
        ...
        expect(timings.promptToAcceptanceMs +
            timings.acceptanceToProviderRequestArrivalMs).toBe(timings.promptToProviderRequestArrivalMs);
    });
...
describe('runner decision contracts', () => {
    it('distinguishes accepted, legacy, and malformed prompt responses', () => {
        expect(validatePromptAcceptance({ promptId: 'prompt-1', lastEventId: 0 })).toEqual({
            kind: 'accepted',
            promptId: 'prompt-1',
            lastEventId: 0,
        });
    ...
    it('applies the absolute and relative prototype gates only to complete baselines', () => {
        const tightAround = (centre) => Array.from({ length: 30 }, (_, index) => centre - 1 + (index % 3));
        expect(evaluateSingleBundlePrototypeGate({
            complete: true,
    ...
    it('fails the prototype gate when a passing point estimate has a CI that crosses the threshold', () => {
        ...
        const noisy = Array.from({ length: 30 }, (_, index) => index % 2 === 0 ? -40 : 100);
    ...
    it('produces a deterministic prototype-gate interval for a fixed seed', () => {
        ...
        expect(differentSeed.bootstrapMedianCi95).not.toEqual(first.bootstrapMedianCi95);
    });
    it('rejects invalid prototype-gate inputs before sampling', () => {
        ...
        expect(() => evaluateSingleBundlePrototypeGate({ ...validInput, seed: Number.NaN })).toThrow('seed must be finite');
        expect(() => evaluateSingleBundlePrototypeGate({
            ...validInput,
...
describe('artifact rendering', () => {
    ...
    it('renders a fatal configuration artifact', () => {
        ...
        expect(renderFirstOutputBenchmarkMarkdown(artifact)).toContain('Failure: invalid_configuration');
    });
});
//# sourceMappingURL=_first-output-benchmark.test.js.map