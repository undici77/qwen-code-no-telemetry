...
describeLocal('GenAI telemetry fields', () => {
    it('exports aligned LLM and tool fields across a complete tool turn', async () => {
        server = await startFakeOpenAIServer(({ requestIndex }) => requestIndex === 0
            ? {
                model: 'provider-model-tool',
                toolCalls: [
                    fakeToolCall('run_shell_command', { command: 'pwd' }, 'provider-call-123'),
                ],
                usage: {
                    prompt_tokens: 20,
                    completion_tokens: 4,
        ...
        try {
            await rig.run('Run the requested tool and then report completion.', '--output-format', 'json');
        }
        finally {
            restoreEnvironment();
        }
        const records = parseTelemetry(rig.readFile('telemetry.log'));
        const llmSpans = records.filter((record) => record.name === 'qwen-code.llm_request');
        expect(llmSpans).toHaveLength(2);
        const firstLlm = llmSpans[0].attributes;
        const secondLlm = llmSpans[1].attributes;
        const interactionSpan = records.find((record) => record.name === 'qwen-code.interaction');
        expect(interactionSpan?.attributes?.['gen_ai.user.id']).toBe('integration-user-079458');
        ...
        expect(firstLlm['gen_ai.conversation.id']).toEqual(secondLlm['gen_ai.conversation.id']);
        expect(firstLlm['gen_ai.conversation.id']).toEqual(expect.any(String));
        const firstRequest = server.requests[0].body;
        const firstInput = JSON.parse(firstLlm['gen_ai.input.messages']);
        const secondInput = JSON.parse(secondLlm['gen_ai.input.messages']);
        expect(firstInput.map((message) => message['role'])).toEqual(firstRequest['messages'].map((message) => message['role']));
        expect(JSON.stringify(firstInput).includes('Run the requested tool and then report completion.')).toBe(true);
        expect(secondInput.some((message) => message['role'] === 'tool' &&
            JSON.stringify(message).includes('provider-call-123'))).toBe(true);
        ...
        const rawTools = firstRequest['tools'];
        const toolDefinitions = JSON.parse(firstLlm['gen_ai.tool.definitions']);
        expect(toolDefinitions).toEqual(rawTools.map((tool) => ({
            type: tool.type,
        ...
        expect(JSON.parse(firstLlm['gen_ai.output.messages'])).toEqual([
            {
                role: 'assistant',
                parts: [
                    {
        ...
        expect(JSON.parse(secondLlm['gen_ai.output.messages'])).toEqual([
            {
                role: 'assistant',
        ...
        for (const attributes of [firstLlm, secondLlm]) {
            ...
            expect(attributes).not.toHaveProperty('gen_ai.server.time_to_first_token');
        ...
        expect(records.flatMap((record) => record.events ?? [])).not.toContainEqual(expect.objectContaining({ name: 'tool_schema' }));
        ...
        const toolSpan = records.find((record) => record.name === 'qwen-code.tool' &&
        ...
        expect(toolSpan?.attributes?.['gen_ai.tool.description']).toEqual(expect.any(String));
        expect(JSON.parse(toolSpan?.attributes?.['gen_ai.tool.call.arguments'])).toEqual({ command: 'pwd' });
        expect(JSON.parse(toolSpan?.attributes?.['gen_ai.tool.call.result'])).toMatchObject({ output: expect.any(String) });
        ...
        for (const request of server.requests) {
            ...
            expect(String(request.headers['baggage'] ?? '')).not.toContain('integration-user-079458');
        }
    });
    it('omits the ARMS user ID when it is not configured', async () => {
        ...
        const records = parseTelemetry(rig.readFile('telemetry.log'));
        const interactionSpan = records.find((record) => record.name === 'qwen-code.interaction');
        const llmSpan = records.find((record) => record.name === 'qwen-code.llm_request');
        expect(interactionSpan?.attributes ?? {}).not.toHaveProperty('gen_ai.user.id');
    ...
    it('omits the default choice count and sensitive tool payloads', async () => {
        server = await startFakeOpenAIServer(({ requestIndex }) => requestIndex === 0
            ? {
                model: 'provider-model-tool',
                toolCalls: [
                    fakeToolCall('run_shell_command', { command: 'pwd' }, 'provider-call-sensitive-off'),
                ],
                usage: {
                    prompt_tokens: 10,
                    completion_tokens: 2,
        ...
        try {
            await rig.run('Run the requested tool and reply with done.', '--output-format', 'json');
        }
        finally {
            restoreEnvironment();
        }
        ...
        for (const llmSpan of llmSpans) {
            expect(llmSpan.attributes).not.toHaveProperty('gen_ai.request.choice.count');
            ...
            expect(llmSpan.attributes).not.toHaveProperty('gen_ai.system_instructions');
...
//# sourceMappingURL=gen-ai-telemetry.test.js.map