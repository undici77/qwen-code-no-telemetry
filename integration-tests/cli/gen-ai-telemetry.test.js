...
describeLocal('GenAI telemetry fields', () => {
  it('exports aligned LLM and tool fields across a complete tool turn', async () => {
    ...
    expect(firstInput.map((message) => message['role'])).toEqual(
      firstRequest['messages'].map((message) => message['role']),
    );
    expect(
    ...
    expect(
      JSON.parse(toolSpan?.attributes?.['gen_ai.tool.call.arguments']),
    ).toEqual({ command: 'pwd' });
    expect(
      JSON.parse(toolSpan?.attributes?.['gen_ai.tool.call.result']),
