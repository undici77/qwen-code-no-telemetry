...
import { DEFAULT_BOOTSTRAP_ITERATIONS, DEFAULT_FIRST_OUTPUT_EVENT_BUFFER_LIMIT, DEFAULT_MATERIAL_THRESHOLD_MS, DEFAULT_ORDER_SENSITIVITY_THRESHOLD_MS, FIRST_OUTPUT_BENCHMARK_VERSION, FirstOutputTracker, computeFirstOutputSessionTimings, evaluateSingleBundlePrototypeGate, findInvalidTimings, measuredPairCountForDwell, nullablePercentiles, pairedCandidateControlStats, parseBenchmarkPostSessionDwell, renderFirstOutputBenchmarkMarkdown, validateExpectedFinalText, validatePromptAcceptance, } from './_first-output-benchmark.js';
import { countDescendants, LISTENING_LINE_RE, sleep, } from './_daemon-harness.js';
...
const SESSION_TIMEOUT_BUDGET_MS = SDK_FETCH_TIMEOUT_MS * 2 +
    TURN_TIMEOUT_MS * 3 +
    PROCESS_EXIT_TIMEOUT_MS +
    MAX_POST_SESSION_DWELL_MS;
const PROCESS_TIMEOUT_BUDGET_MS = (sessionCount) => BOOT_TIMEOUT_MS +
    sessionCount * SESSION_TIMEOUT_BUDGET_MS +
    PROCESS_EXIT_TIMEOUT_MS * 3 +
    DESCENDANT_EXIT_TIMEOUT_MS * 2;
const SINGLE_RUN_TIMEOUT_BUDGET_MS = (SINGLE_WARMUPS + SINGLE_MEASURED) *
    PROCESS_TIMEOUT_BUDGET_MS(SINGLE_SESSIONS_PER_PROCESS);
const COMPARE_RUN_TIMEOUT_BUDGET_MS = (COMPARE_WARMUP_PAIRS + 30) *
    2 *
    PROCESS_TIMEOUT_BUDGET_MS(COMPARE_SESSIONS_PER_PROCESS);
const BENCHMARK_TEST_TIMEOUT_MS = Math.ceil(Math.max(SINGLE_RUN_TIMEOUT_BUDGET_MS, COMPARE_RUN_TIMEOUT_BUDGET_MS) *
    1.25) +
    5 * 60_000;
...
const OUTPUT_DIR = path.join(REPOSITORY_ROOT, '.qwen', 'investigations', 'daemon-first-output-benchmark', new Date().toISOString().replace(/[:.]/g, '').replace(/Z$/, ''));
const BOOTSTRAP_SEED = 0x51f02026;
const activeDaemons = new Set();
const activeProcessGroups = new Set();
const activeFakeProviders = new Set();
const activeTempDirs = new Set();
const activeCompileCacheDirs = new Set();
...
class BenchmarkSpawnFailure extends BenchmarkFailure {
    ...
    safeToContinue;
    daemonExited;
    constructor(primary, pid, stdout, stderr, processStartedAtMs, cleanupFailure, residualProcessCount, safeToContinue, daemonExited) {
...
function firstOutputFailure(error, fallback = 'harness_error') {
...
function readBenchmarkConfig() {
    if (!SANDBOX_DISABLED) {
        throw new Error('Invalid first-output benchmark config: QWEN_SANDBOX must be exactly false.');
    }
    ...
    if (hasSingle && (hasControl || hasCandidate)) {
        throw new Error('Invalid first-output benchmark config: BENCHMARK_CLI_PATH cannot be ' +
            'combined with BENCHMARK_CONTROL_CLI_PATH or ' +
            'BENCHMARK_CANDIDATE_CLI_PATH.');
    }
    if (hasControl !== hasCandidate) {
        throw new Error('Invalid first-output benchmark config: compare mode requires both ' +
            'BENCHMARK_CONTROL_CLI_PATH and BENCHMARK_CANDIDATE_CLI_PATH.');
    }
    if (!hasSingle && !hasControl) {
        throw new Error('Invalid first-output benchmark config: set BENCHMARK_CLI_PATH for ' +
    ...
    if (hasSingle) {
        if (process.env['BENCHMARK_POST_SESSION_DWELL_MS'] !== undefined) {
            throw new Error('Invalid first-output benchmark config: ' +
    ...
    const dwell = parseBenchmarkPostSessionDwell(process.env['BENCHMARK_POST_SESSION_DWELL_MS']);
    ...
    if (control.realPath === candidate.realPath) {
        throw new Error('Invalid first-output benchmark config: control and candidate resolve ' +
            `to the same bundle (${control.realPath}).`);
    }
    if (control.sha256 === candidate.sha256) {
        throw new Error('Invalid first-output benchmark config: control and candidate bundle ' +
    ...
    return {
        ...
        measuredPairs: measuredPairCountForDwell(dwell, process.env['BENCHMARK_MEASURED_PAIRS']),
    };
}
function resolveBundle(input, role) {
    ...
    try {
        ...
        throw new Error(`Invalid ${role} bundle path ${JSON.stringify(input)}: ${errorMessage(error)}`);
    }
    const stat = fs.statSync(realPath);
    if (!stat.isFile()) {
        throw new Error(`Invalid ${role} bundle path ${JSON.stringify(input)}: expected a file.`);
    }
    const compileCacheDir = fs.mkdtempSync(path.join(os.tmpdir(), `qwen-first-output-cache-${role}-`));
...
async function spawnBenchmarkDaemon(bundle, workspace, env) {
    ...
    try {
        ...
        throw new BenchmarkSpawnFailure(primary, pid, tailUtf8(stdout.value), tailUtf8(stderr.value), processStartedAtMs, cleanupFailure, residualProcessCount, safeToContinue, hasChildExited(child));
    }
    const daemon = {
...
function waitForListening(child, stdout, stderr) {
    return new Promise((resolve, reject) => {
        ...
        const onExit = (code, signal) => {
            fail(new BenchmarkFailure('daemon_exited_before_listen', `Daemon exited before listening (code=${code}, signal=${signal}).\n` +
                `stdout:\n${tailUtf8(stdout.value)}\nstderr:\n${tailUtf8(stderr.value)}`));
        };
        const onError = (error) => {
            fail(new BenchmarkFailure('daemon_exited_before_listen', `Daemon spawn failed: ${error.message}`, { cause: error }));
        };
        const timer = setTimeout(() => {
            fail(new BenchmarkFailure('daemon_boot_timeout', `Daemon did not listen within ${BOOT_TIMEOUT_MS}ms.\n` +
                `stdout:\n${tailUtf8(stdout.value)}\nstderr:\n${tailUtf8(stderr.value)}`));
        }, BOOT_TIMEOUT_MS);
...
async function terminateBenchmarkDaemon(daemon) {
...
function benchmarkDescendantPids(pid) {
    const descendants = benchmarkDescendants(pid);
    return [
        ...new Set([...descendants.acpChildren, ...descendants.mcpGrandchildren]),
    ];
}
function benchmarkDescendants(pid) {
    return countDescendants(pid, {
        acpFilter: '(^|[[:space:]])--acp([[:space:]]|$)',
    });
}
function rememberDescendantPids(processGroup, descendantPids) {
    for (const descendantPid of descendantPids) {
        processGroup.descendantPids.add(descendantPid);
    }
}
async function terminateTrackedProcessGroup(processGroup) {
    let enumerationFailure = null;
    if (!hasChildExited(processGroup.child)) {
        try {
            rememberDescendantPids(processGroup, benchmarkDescendantPids(processGroup.pid));
    ...
    const remainingPids = await terminateProcessGroup(processGroup.child, trackedPids);
    ...
    if (remainingPids.length > 0) {
        throw new ResidualProcessFailure(`Tracked descendants survived teardown: ${remainingPids.join(', ')}.`, remainingPids.length);
    }
    if (!processGroup.descendantSnapshotComplete) {
        throw new BenchmarkFailure('cleanup_timeout', enumerationFailure?.message ??
            'Descendant enumeration did not complete before the process-group leader exited.');
    }
}
async function terminateProcessGroup(child, descendantPids) {
    const pid = child.pid;
    if (!pid)
    ...
    let remaining = await waitForDescendantsExit(descendantPids, DESCENDANT_EXIT_TIMEOUT_MS);
    if (!hasChildExited(child)) {
        signalProcessGroup(pid, 'SIGKILL');
        await waitForChildExit(child, PROCESS_EXIT_TIMEOUT_MS);
        remaining = await waitForDescendantsExit(remaining, DESCENDANT_EXIT_TIMEOUT_MS);
    }
    if (!hasChildExited(child)) {
        throw new BenchmarkFailure('cleanup_timeout', `Daemon process group ${pid} did not exit after SIGKILL.`);
    }
    return remaining;
}
async function waitForDescendantsExit(descendantPids, timeoutMs) {
...
function fakeProviderPort(server) {
    const port = Number(new URL(server.baseUrl).port);
    if (!Number.isSafeInteger(port) || port <= 0) {
        throw new Error(`Fake provider returned an invalid base URL: ${server.baseUrl}`);
    }
    return port;
}
function isolatedDaemonEnv(home, qwenHome, tempDir, workspace, fakeServer, bundle) {
    const inherited = {};
    if (process.env['PATH'])
        inherited['PATH'] = process.env['PATH'];
    const trustedFoldersPath = path.join(qwenHome, 'trustedFolders.json');
    fs.writeFileSync(trustedFoldersPath, JSON.stringify({ [fs.realpathSync(workspace)]: 'TRUST_FOLDER' }));
    return {
        ...inherited,
        HOME: home,
        QWEN_HOME: qwenHome,
...
function prepareIsolatedDirectories(sampleId) {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), `qwen-first-output-${sampleId}-`));
...
function startTimedFakeProvider(expectedRequests, failureState) {
    return startFakeOpenAIServer(async ({ body }) => {
        if (body['stream'] !== true || body['model'] !== 'fake-model') {
            const message = 'Benchmark Provider requires stream=true and model=fake-model.';
            failureState.value ??= {
                code: 'harness_error',
                message,
            };
        ...
        const matches = [...expectedRequests.values()].filter(({ prompt }) => messages.includes(prompt));
        if (matches.length !== 1) {
...
async function runSession(daemon, workspace, ordinal, postSessionDwellMs, expectedRequests) {
    const prompt = makePrompt();
    const expectedRequest = { prompt, count: 0 };
    ...
    let failure = null;
    let cleanupFailure = null;
    let daemonPromptQueueWaitMs = null;
    ...
    try {
        try {
            ...
            throw new BenchmarkFailure('session_create_failed', `Session creation failed: ${errorMessage(error)}`, { cause: error });
        }
        if (!session.sessionId || !session.clientId) {
            throw new BenchmarkFailure('session_create_failed', 'Session creation returned a malformed response.');
        }
        ...
        streamTask = (async () => {
            try {
                for await (const event of daemon.client.subscribeEvents(session.sessionId, {
                    signal: abort.signal,
                    onEpoch: () => {
                    ...
                    tracker.push(event, performance.now());
                    if (tracker.snapshot().terminal !== null) {
                        terminalReady.resolve();
                        return;
                    }
                }
                const streamEnded = new BenchmarkFailure('sse_stream_ended', 'SSE stream ended before the matching prompt terminal.');
                ...
                if (!abort.signal.aborted) {
                    ...
                    terminalReady.reject(new BenchmarkFailure('sse_stream_ended', `SSE stream failed: ${errorMessage(error)}`, { cause: error }));
                }
            }
        })();
        try {
            await withTimeout(epochReady.promise, TURN_TIMEOUT_MS, 'SSE epoch readiness');
        }
        catch (error) {
            throw new BenchmarkFailure(error instanceof BenchmarkTimeoutError
                ? 'sse_connect_timeout'
                : 'sse_stream_ended', `SSE connection failed: ${errorMessage(error)}`, { cause: error });
        }
        ...
        if (timestamps.sseReady === null) {
            throw new BenchmarkFailure('harness_error', 'SSE readiness resolved without recording a timestamp.');
        }
        ...
        let accepted;
        try {
            accepted = await withTimeout(daemon.client.promptNonBlocking(session.sessionId, {
                prompt: [{ type: 'text', text: prompt }],
            }, promptAbort.signal, session.clientId), TURN_TIMEOUT_MS, 'prompt acceptance');
        }
        catch (error) {
            promptAbort.abort();
            const snapshot = tracker.snapshot();
            if (snapshot.failureCode !== null) {
                throw new BenchmarkFailure(snapshot.failureCode, snapshot.failureMessage ?? snapshot.failureCode);
            }
            throw new BenchmarkFailure(error instanceof BenchmarkTimeoutError
                ? 'prompt_accept_timeout'
                : 'prompt_rejected', `Prompt acceptance failed: ${errorMessage(error)}`, { cause: error });
        }
        const acceptance = validatePromptAcceptance(accepted);
        if (acceptance.kind === 'failure') {
            throw new BenchmarkFailure(acceptance.failure.code, acceptance.failure.message);
        }
        ...
        if (acceptedSnapshot.terminal !== null ||
            acceptedSnapshot.failureCode !== null) {
            terminalReady.resolve();
        }
        try {
            await withTimeout(terminalReady.promise, TURN_TIMEOUT_MS, 'matching prompt terminal');
        }
        catch (error) {
            const snapshot = tracker.snapshot();
            if (snapshot.failureCode !== null) {
            ...
            if (error instanceof BenchmarkFailure)
                throw error;
            throw new BenchmarkFailure(snapshot.firstOutput === null
                ? 'first_output_timeout'
                : 'terminal_timeout', errorMessage(error), { cause: error });
        }
        const tracked = tracker.snapshot();
        if (tracked.failureCode !== null) {
            throw new BenchmarkFailure(tracked.failureCode, tracked.failureMessage ?? tracked.failureCode);
        }
        if (!tracked.runEligible || !tracked.firstOutput || !tracked.terminal) {
            throw new BenchmarkFailure('harness_error', `Prompt ${promptId} completed without an eligible output and terminal.`);
        }
        if (!tracked.userEcho) {
            throw new BenchmarkFailure('harness_error', `Prompt ${promptId} completed without a matching relayed user echo.`);
        }
        if (tracked.firstOutput.kind !== 'answer_text') {
            throw new BenchmarkFailure('unexpected_output_kind', `Expected answer_text as first output, received ${tracked.firstOutput.kind}.`);
        }
        const finalTextFailure = validateExpectedFinalText(tracked.finalAnswerText, EXPECTED_ANSWER);
        if (finalTextFailure) {
            throw new BenchmarkFailure(finalTextFailure.code, finalTextFailure.message);
        }
        if (expectedRequest.count !== 1 || !expectedRequest.timing) {
            throw new BenchmarkFailure('provider_request_count_mismatch', `Expected exactly one target generation request, observed ${expectedRequest.count}.`);
        }
        const daemonStatus = await daemon.client.daemonStatus();
        const queueWait = daemonStatus.runtime.perf?.promptQueueWait;
        if (queueWait?.count !== ordinal ||
            queueWait.lastMs === null ||
            !Number.isFinite(queueWait.lastMs) ||
            queueWait.lastMs < 0) {
            throw new BenchmarkFailure('harness_error', `Expected isolated daemon prompt queue-wait sample ${ordinal}, ` +
                `received count=${String(queueWait?.count)} lastMs=${String(queueWait?.lastMs)}.`);
        }
    ...
    const timings = computeFirstOutputSessionTimings(timestamps, daemon.processStartedAtMs, daemonPromptQueueWaitMs);
    ...
    return {
        ...
        diagnostics: {
            ...
            sseReadyBeforePrompt: timestamps.sseReady !== null &&
                timestamps.promptStart !== null &&
                timestamps.sseReady <= timestamps.promptStart,
            actualProviderDelayMs: duration(timestamps.providerReady, timestamps.providerRequestArrival),
        },
    };
}
...
async function runProcessSample(options) {
    ...
    const providerFailureState = { value: null };
    let fakeServer;
    let daemon;
    let spawnFailure;
    const sessions = [];
    ...
    let cleanupFailure = null;
    let residualProcessCount = 0;
    let firstSessionAcpPids = null;
    let safeToContinue = true;
    try {
        dirs = prepareIsolatedDirectories(options.sampleId);
        fakeServer = await startTimedFakeProvider(expectedRequests, providerFailureState);
        activeFakeProviders.add(fakeServer);
        daemon = await spawnBenchmarkDaemon(options.bundle, dirs.workspace, isolatedDaemonEnv(dirs.home, dirs.qwenHome, dirs.tempDir, dirs.workspace, fakeServer, options.bundle));
        for (let ordinal = 1; ordinal <= options.sessionCount; ordinal += 1) {
            const session = await runSession(daemon, dirs.workspace, ordinal, options.postSessionDwellMs, expectedRequests);
            ...
            if (options.sessionCount === 2 && session.failure === null) {
                ...
                const acpPids = descendants.acpChildren.sort((left, right) => left - right);
                session.diagnostics = { ...session.diagnostics, acpPids };
                if (ordinal === 1) {
                    firstSessionAcpPids = acpPids;
                }
                else if (firstSessionAcpPids === null ||
                    firstSessionAcpPids.length !== 1 ||
                    acpPids.length !== 1 ||
                    firstSessionAcpPids[0] !== acpPids[0]) {
                    failure = {
                        code: 'harness_error',
                        message: 'Cold and warm sessions did not use the same single ACP child process.',
                    };
                }
            }
        ...
        if (failure === null &&
            ...
            if (!first ||
                ...
                failure = {
                    code: 'harness_error',
                    message: 'The two sessionScope=thread benchmark turns did not retain distinct session and prompt ids.',
                };
            }
        }
    }
        ...
        if (fakeServer &&
            ...
            failure = {
                code: 'provider_request_count_mismatch',
                message: 'Provider request-count invariant failed after daemon exit: ' +
        ...
        if (fakeServer) {
            try {
                await withTimeout(fakeServer.close(), PROCESS_EXIT_TIMEOUT_MS, 'Fake Provider shutdown');
    ...
    return {
        ...
        processStartedAtMs: daemon?.processStartedAtMs ?? spawnFailure?.processStartedAtMs ?? null,
...
async function runComparisonPairs(config) {
    const warmups = [];
    const measured = [];
    const warmupOrders = ['AB', 'BA'];
    for (let index = 1; index <= COMPARE_WARMUP_PAIRS; index++) {
        const order = warmupOrders[index - 1];
        logProgress(`compare warmup pair ${index}/${COMPARE_WARMUP_PAIRS} (${order})`);
        const pair = await runComparisonPair(config, index, order, true);
        warmups.push(pair);
        if (!isSuccessfulRawPair(pair))
            return { warmups, measured };
    }
    for (let index = 1; index <= config.measuredPairs; index++) {
        const order = index % 2 === 1 ? 'AB' : 'BA';
        logProgress(`compare measured pair ${index}/${config.measuredPairs} (${order})`);
...
async function runComparisonPair(config, pairIndex, order, warmup) {
    const sequence = order === 'AB'
        ? [config.control, config.candidate]
        : [config.candidate, config.control];
    const samples = [];
    for (const [position, bundle] of sequence.entries()) {
        const sampleId = `${warmup ? 'warmup' : 'measured'}-pair-${pairIndex}-` +
        ...
        if (!sample.safeToContinue && position === 0) {
            ...
            samples.push(skippedProcessSample(`${warmup ? 'warmup' : 'measured'}-pair-${pairIndex}-2-${skippedBundle.role}`, skippedBundle, order, pairIndex, warmup));
            break;
        }
    }
...
function isSuccessfulProcessSample(sample, expectedSessions) {
...
function isSuccessfulRawPair(pair) {
    return (pair.safeToContinue &&
...
function skippedProcessSample(sampleId, bundle, order, pairIndex, warmup) {
    return {
        sampleId,
        role: bundle.role,
        ...
        failure: {
            code: 'harness_error',
            message: "Arm was not started because the preceding arm's owned resources could not be verified as stopped.",
        },
        safeToContinue: false,
        stdout: '',
        stderr: '',
...
function toProcessRun(sample, sampleIndex, orderPosition) {
    return {
        variant: sample.role,
        sampleIndex,
        ...
        processToListenMs: duration(sample.daemonReadyAtMs, sample.processStartedAtMs),
...
function toPairResult(pair, sampleIndex) {
    const controlPosition = pair.order === 'AB' ? 1 : 2;
    const candidatePosition = pair.order === 'AB' ? 2 : 1;
    const control = toProcessRun(pair.control, sampleIndex, controlPosition);
    const candidate = toProcessRun(pair.candidate, sampleIndex, candidatePosition);
    const invalidReasons = [control, candidate].flatMap((run) => {
        const reasons = [];
        if (run.failure)
            reasons.push(`${run.variant}:${run.failure.code}`);
...
function buildSingleMetricSummary(runs, metric) {
    if (metric === 'processToListenMs') {
        return {
            all: nullablePercentiles(runs.map((run) => isSuccessfulRun(run) ? run.processToListenMs : null)),
            bySession: {},
        };
    }
    const bySession = {};
    for (const ordinal of [1, 2]) {
        bySession[`session_${ordinal}`] = nullablePercentiles(runs.map((run) => {
            if (!isSuccessfulRun(run))
                return null;
            const session = run.sessions.find((candidate) => candidate.ordinal === ordinal);
...
function buildPairedMetricSummary(pairs, metric, seedOffset) {
    if (metric === 'processToListenMs') {
        return {
            all: pairedCandidateControlStats(pairs.map((pair) => ({
                order: pair.order,
                valid: pair.valid,
    ...
    const sessionStats = pairedCandidateControlStats(pairs.map((pair) => ({
        order: pair.order,
        valid: pair.valid,
...
function metricForOrdinal(run, ordinal, metric) {
    const session = run.sessions.find((candidate) => candidate.ordinal === ordinal);
    return session ? sessionMetric(session, metric) : null;
}
function pairedOptions(seedOffset) {
    return {
...
function commonConfig(postSessionDwellMs) {
    return {
        seed: BOOTSTRAP_SEED,
        providerDelayMs: PROVIDER_DELAY_MS,
        providerConnection: 'close-per-response',
        postSessionDwellMs,
        promptShape: `Unique fixed-width ${PROMPT_LENGTH}-character prompt with a ` +
...
function variantDescriptor(bundle) {
    return {
...
function buildSingleArtifact(config, raw) {
    const warmupRuns = raw.warmups.map((sample, index) => toProcessRun(sample, index + 1, null));
    const runs = raw.measured.map((sample, index) => toProcessRun(sample, index + 1, null));
    const metrics = {};
    ...
    const coldProviderP50 = metrics['promptToProviderRequestArrivalMs']?.bySession['session_1']
        ?.distribution?.p50 ?? null;
    const warmProviderP50 = metrics['promptToProviderRequestArrivalMs']?.bySession['session_2']
        ?.distribution?.p50 ?? null;
    const coldFirstOutputP50 = metrics['promptToFirstModelOutputMs']?.bySession['session_1']?.distribution
        ?.p50 ?? null;
        ...
        summary: {
            ...
            decision: {
                ...
                reasons: [
                    ...
                    measuredValid
                        ? `Paired cold-minus-warm Provider-arrival median: ${gate.pairedMedianDeltaMs === null
                            ? 'n/a'
                            : `${gate.pairedMedianDeltaMs.toFixed(1)}ms`}, 95% CI ${gate.bootstrapMedianCi95 === null
                            ? 'n/a'
...
function buildPairedArtifact(config, raw) {
    const warmups = raw.warmups.map((pair, index) => toPairResult(pair, index + 1));
    const pairs = raw.measured.map((pair, index) => toPairResult(pair, index + 1));
    const metrics = {};
    for (const [index, metric] of METRICS.entries()) {
        metrics[metric] = buildPairedMetricSummary(pairs, metric, index * 10);
    }
    const primary = metrics['processToFirstModelOutputMs']?.bySession['session_1'];
    if (!primary) {
    ...
    const warmupsValid = warmups.length === COMPARE_WARMUP_PAIRS &&
        warmups.every((pair) => pair.valid);
    const diagnosticOnly = config.measuredPairs === 10;
    const comparisonOutcome = measuredValid && warmupsValid
        ? diagnosticOnly
            ? 'inconclusive'
            : primary.decision
        : 'invalid';
        ...
        summary: {
            ...
            decision: {
                ...
                reasons: [
                    ...
                    `Session 1 median candidate-control delta: ${primary.medianDeltaMs === null
                        ? 'n/a'
...
function coldWarmProviderDeltas(runs) {
    const deltas = [];
...
function countRunFailures(runs) {
    const counts = {};
...
function countPairFailures(pairs) {
    return countRunFailures(pairs.flatMap((pair) => [pair.control, pair.candidate]));
}
function buildFatalArtifact(failure) {
    const hasSingle = Boolean(process.env['BENCHMARK_CLI_PATH']?.trim());
    const hasPaired = Boolean(process.env['BENCHMARK_CONTROL_CLI_PATH']?.trim() ||
        process.env['BENCHMARK_CANDIDATE_CLI_PATH']?.trim());
    return {
        ...commonArtifactFields(),
        mode: 'failed',
        config: {
            requestedMode: hasSingle === hasPaired ? 'unknown' : hasSingle ? 'single' : 'paired',
        },
        failure,
        summary: {
...
async function cleanupActiveResources() {
    ...
    for (const fakeProvider of [...activeFakeProviders]) {
        try {
            await withTimeout(fakeProvider.close(), PROCESS_EXIT_TIMEOUT_MS, 'Fake Provider emergency shutdown');
    ...
    if (failures.length > 0) {
        throw new AggregateError(failures, 'One or more benchmark-owned resources survived emergency cleanup.');
    }
}
(SKIP ? describe.skip : describe).sequential('daemon first-output benchmark (opt-in, POSIX-only)', { retry: 0 }, () => {
    afterAll(cleanupActiveResources, EMERGENCY_CLEANUP_TIMEOUT_MS);
    it('records a single baseline or balanced paired comparison', async () => {
        let artifact;
        let runError;
        ...
        try {
            ...
            runError = error;
            artifact = buildFatalArtifact(firstOutputFailure(error, configResolved ? 'harness_error' : 'invalid_configuration'));
        }
        finally {
            if (artifact) {
                writeSnapshotArtifacts(OUTPUT_DIR, 'daemon-first-output-benchmark-v2', artifact, renderFirstOutputBenchmarkMarkdown(artifact), 'first-output-benchmark');
            }
        }
        ...
        if (artifact.mode === 'failed' ||
            ...
            throw new Error('First-output benchmark produced an invalid artifact.');
        }
    }, BENCHMARK_TEST_TIMEOUT_MS);
});
//# sourceMappingURL=qwen-daemon-first-output-benchmark.test.js.map