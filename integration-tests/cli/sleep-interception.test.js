...
describe('sleep-interception', () => {
     ...
     */
    const waitForShellCall = (predicate) => rig.poll(() => rig
        .readToolLogs()
        .some((log) => log.toolRequest.name === 'run_shell_command' &&
        predicate(log.toolRequest)), rig.getDefaultTimeout(), 100);
    const shellCalls = () => rig
        .readToolLogs()
    ...
    it('should block sleep >= 2s and mention Monitor in guidance', async () => {
        ...
        const result = await rig.run('Use the run_shell_command tool to run this exact command in the ' +
        ...
        const foundBlockedCall = await waitForShellCall((call) => !!call.args?.includes('sleep 5') && call.success === false);
        if (!foundBlockedCall) {
            printDebugInfo(rig, result, {
                'Shell calls': JSON.stringify(shellCalls()),
            });
        }
        expect(foundBlockedCall, 'Expected a blocked (success: false) run_shell_command call for sleep 5').toBeTruthy();
        ...
        const blockedCall = shellCalls().find((call) => !!call.args?.includes('sleep 5') && call.success === false);
    ...
    it('should allow sleep < 2s', async () => {
        ...
        const result = await rig.run('Use the run_shell_command tool to run this exact command: sleep 1. ' +
            'You must actually call run_shell_command with that command — do ' +
            'not skip it. After it completes, say "DONE".');
        const foundSuccessfulCall = await waitForShellCall((call) => !!call.args?.includes('sleep 1') && call.success === true);
        if (!foundSuccessfulCall) {
            printDebugInfo(rig, result, {
                'Shell calls': JSON.stringify(shellCalls()),
            });
        }
        expect(foundSuccessfulCall, 'Expected a successful run_shell_command call for sleep 1').toBeTruthy();
    ...
    it('should allow retrying blocked sleep with an intentional sleep comment', async () => {
        ...
        const foundIntentionalCall = await waitForShellCall((call) => !!call.args?.includes('intentional-sleep') && call.success === true);
        if (!foundIntentionalCall) {
            printDebugInfo(rig, result, {
                'Shell calls': JSON.stringify(shellCalls()),
            });
        }
        expect(foundIntentionalCall, 'Expected a successful run_shell_command call with an intentional-sleep comment').toBeTruthy();
    ...
    it('should block sleep >= 2s even when followed by a trailing comment', async () => {
        ...
        expect(foundBlockedCall, 'Expected a blocked (success: false) run_shell_command call for sleep 5 with trailing comment').toBeTruthy();
        validateModelOutput(result, 'blocked', 'sleep blocked with trailing comment');
    });
});
//# sourceMappingURL=sleep-interception.test.js.map