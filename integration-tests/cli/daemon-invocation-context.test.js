...
import { approveWorkspaceMcpServers, spawnDaemon, writeWorkspaceSettings, } from './_daemon-harness.js';
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = path.resolve(__dirname, '../..');
const ECHO_SERVER = path.join(REPO_ROOT, 'integration-tests', 'fixtures', 'invocation-context-echo.mjs');
const TSX_BIN = path.join(REPO_ROOT, 'node_modules', 'tsx', 'dist', 'cli.mjs');
const SERVE_BRIDGE_SOURCE = path.join(REPO_ROOT, 'packages', 'sdk-typescript', 'src', 'daemon-mcp', 'serve-bridge', 'bin.ts');
...
function pendingSentinel(body) {
    const messages = body['messages'];
    if (!Array.isArray(messages))
        return false;
    const userIndex = messages.findLastIndex((message) => typeof message === 'object' &&
        message !== null &&
        message['role'] === 'user');
    if (userIndex === -1)
        return false;
...
describe('trusted daemon invocation context', () => {
    it('reaches a Qwen-launched local stdio MCP tool without leaking the private capability', async ({ skip, }) => {
        ...
        const fakeModel = await startFakeOpenAIServer(({ body }) => pendingSentinel(body)
        ...
        try {
            ...
            const session = await callBridgeTool(bridgeClient, 'session_create', { workspace_cwd: workspace, session_scope: 'thread' });
            ...
            expect(records[0]?.metadata?.[INVOCATION_META_KEY]?.['originatorClientId']).toBeUndefined();
...
//# sourceMappingURL=daemon-invocation-context.test.js.map