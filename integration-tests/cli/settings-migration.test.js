...
describe('settings-migration', () => {
  ...
  describe('V2 settings migration', () => {
    it('should migrate V2 settings forward through the chain on CLI startup', async () => {
      ...
      expect(
        migratedSettings['ui']?.['accessibility']?.['enableLoadingPhrases'],
      ).toBe(true);
      expect(migratedSettings['general']?.['enableAutoUpdate']).toBe(true);
      expect(
        migratedSettings['context']?.['fileFiltering']?.['enableFuzzySearch'],
      ).toBe(false);
      // Verify old disable* keys are removed
      expect(
        migratedSettings['general']?.['disableAutoUpdate'],
      ).toBeUndefined();
      expect(migratedSettings['general']?.['disableUpdateNag']).toBeUndefined();
      expect(
        migratedSettings['ui']?.['accessibility']?.['disableLoadingPhrases'],
      ).toBeUndefined();
      expect(
        migratedSettings['context']?.['fileFiltering']?.['disableFuzzySearch'],
      ).toBeUndefined();
    });
    ...
    it('should coerce valid string booleans and remove invalid deprecated keys while bumping V2 forward through the chain', async () => {
      ...
      expect(
        migratedSettings['model']?.['generationConfig']?.[
          'disableCacheControl'
        ],
      ).toBeUndefined();
      expect(
        migratedSettings['model']?.['generationConfig']?.['enableCacheControl'],
      ).toBeUndefined();
    });
  ...
  describe('V3 settings handling', () => {
    it('should handle V3 settings with legacy disable* keys', async () => {
      ...
      expect(
        finalSettings['ui']?.['accessibility']?.['disableLoadingPhrases'],
      ).toBe(false);
      // Existing enable* keys should be preserved
      expect(finalSettings['general']?.['enableAutoUpdate']).toBe(false);
      expect(
        finalSettings['ui']?.['accessibility']?.['enableLoadingPhrases'],
      ).toBe(true);
    ...
    it('should expand legacy boolean general.gitCoAuthor: false through V3 → V4', async () => {
      ...
      const finalSettings = readSettingsFile(rig);
      expect(finalSettings['$version']).toBe(CURRENT_SETTINGS_VERSION);
      expect(finalSettings['general']?.['gitCoAuthor']).toEqual({
        commit: false,
        pr: false,
      });
