...
function findResultMessage(parsed) {
    if (!Array.isArray(parsed))
        return undefined;
    return parsed.find((msg) => typeof msg === 'object' &&
        msg !== null &&
        msg.type === 'result');
}
describe('--json-schema headless structured output', () => {
    ...
    it('emits structured_result when the model fills the schema', async () => {
        ...
        const stdout = await rig.run('What is 2 + 2? Submit it via the structured_output tool.', '--output-format', 'json', '--json-schema', schema);
        const parsed = JSON.parse(stdout);
        const result = findResultMessage(parsed);
        expect(result, 'expected a result message').toBeDefined();
        expect(result.is_error).toBe(false);
        expect(result, 'expected structured_result on success').toHaveProperty('structured_result');
        ...
        const found = toolLogs.find((l) => l.toolRequest.name === 'structured_output');
        expect(found, `expected structured_output tool call, saw: ${toolLogs.map((l) => l.toolRequest.name).join(', ')}`).toBeTruthy();
    ...
    it('loads a schema from disk via the @path syntax', async () => {
        ...
        const stdout = await rig.run('What is the capital of France and what country is it in? Submit via structured_output.', '--output-format', 'json', '--json-schema', `@${schemaPath}`);
    ...
    it('fails fast at CLI parse time on invalid JSON Schema', async () => {
        ...
        let thrown;
        try {
            await rig.run('hi', '--json-schema', JSON.stringify({
                type: 'object',
        ...
        expect(thrown).toBeDefined();
        expect(thrown.message).toMatch(/--json-schema is not a valid JSON Schema/i);
    });
    ...
    it('exits 1 with is_error=true when the model emits plain text instead of calling structured_output', async () => {
        ...
        let thrown;
        try {
            await rig.run('Reply with the literal text "ok".', '--output-format', 'json', '--json-schema', schema, '--exclude-tools', 'structured_output');
        ...
        const stdoutMatch = thrown.message.match(/Stdout:\n([\s\S]*?)(?:\n\nStderr:|$)/);
        expect(stdoutMatch, `expected JSON stdout in error body, got: ${thrown.message.slice(0, 400)}`).toBeTruthy();
