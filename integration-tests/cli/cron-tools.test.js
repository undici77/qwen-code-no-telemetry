...
describe('cron-tools', () => {
    ...
    it('should have cron tools registered when enabled via settings', async () => {
        ...
        const result = await rig.run('Do you have access to tools called cron_create, cron_list, and cron_delete? Reply with just "yes" or "no".');
    ...
    (IS_SANDBOX ? it.skip : it)('should have cron tools registered when enabled via env var', async () => {
        rig = new TestRig();
    ...
    it('should NOT have cron tools by default', async () => {
        ...
        const result = await rig.run('Try to create a cron job with cron_create using cron "*/5 * * * *", prompt "disabled test", and recurring true. If you cannot call that tool, say so briefly.');
        validateModelOutput(result, null, 'cron disabled by default');
        const toolLogs = rig.readToolLogs();
        expect(toolLogs.some((log) => log.toolRequest.name === 'cron_create'), 'cron_create should not be callable when cron is disabled').toBe(false);
    });
    it('should create, list, and delete a cron job in a single turn', async () => {
        ...
        const result = await rig.run('Call cron_create with cron "*/5 * * * *", prompt "test ping", recurring true. Then call cron_list. Then delete that job using cron_delete. Then call cron_list again. How many jobs remain? Reply with just the number.');
    ...
    it('should create a one-shot (non-recurring) job', async () => {
        ...
        const result = await rig.run('Do these steps: (1) Call cron_create with cron "*/5 * * * *", prompt "one-shot test", recurring false. (2) Call cron_list. Is the job marked as recurring or one-shot? Remember the answer. (3) Delete all cron jobs. Reply with just "recurring" or "one-shot".');
        const foundCreate = await rig.waitForToolCall('cron_create');
        const foundList = await rig.waitForToolCall('cron_list');
...
//# sourceMappingURL=cron-tools.test.js.map