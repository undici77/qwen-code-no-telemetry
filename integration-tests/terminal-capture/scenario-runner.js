...
export async function runScenario(config, basedir) {
    ...
    try {
                    ...
                    if (step.streaming) {
                        ...
                        for (let j = 0; j < count; j++) {
                            ...
                            if (curOutputLen === prevOutputLen) {
                                stableCount++;
                                if (stableCount >= 3) {
                                    console.log(`         ⏹️  streaming stopped early: output stable for ${stableCount} intervals`);
                                    break;
                                }
                            ...
                            console.log(`         📸 streaming [${shotNum}/${count}]: ${shotName}`);
        ...
        if (config.gif !== false) {
            const gifFrames = screenshots.filter((s) => !s.endsWith('full-flow.png') && !s.includes('-full-'));
...
function generateGif(frames, outputDir) {
    if (frames.length === 0)
        return null;
    ...
    try {
        ...
        execSync(`ffmpeg -y -f concat -safe 0 -i "${listFile}" -vf "split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 "${gifPath}"`, { stdio: 'pipe' });
        return gifPath;
    }
    catch {
