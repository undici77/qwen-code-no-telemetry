...
export class TerminalCapture {
    ...
    async init() {
        ...
        await this.page.waitForFunction(() => window['termReady'] === true);
    }
    /**
    ...
    async spawn(command, args = []) {
        if (!this.page) {
            throw new Error('Not initialized. Use TerminalCapture.create() factory method.');
        }
    ...
    async waitFor(text, options) {
        ...
        while (Date.now() - start < timeout) {
            if (stripAnsi(this.rawOutput).toLowerCase().includes(text.toLowerCase())) {
                return;
            }
            await this.sleep(200);
        }
        throw new Error(`Timeout (${timeout}ms) waiting for text: "${text}"\n` +
     ...
     * ```
     */
    async idle(stableMs = 500, timeout = 30000) {
        const start = Date.now();
     ...
     */
    async waitForAndIdle(text, options) {
     ...
     * ```
     */
    async capture(filename, outputDir) {
        if (!this.page) {
     ...
     * ```
     */
    async captureFull(filename, outputDir) {
        if (!this.page) {
            throw new Error('Not initialized');
        }
        // 1. Flush all accumulated PTY data to xterm.js
    ...
    resolveXtermDir() {
        try {
            ...
            throw new Error('@xterm/xterm is not installed.\n' +
...
<body>
  <div id="capture-root">
    ...
    lighten(hex, factor) {
        const h = hex.replace('#', '');
        const r = Math.min(255, parseInt(h.slice(0, 2), 16) + Math.round(255 * factor));
        const g = Math.min(255, parseInt(h.slice(2, 4), 16) + Math.round(255 * factor));
        const b = Math.min(255, parseInt(h.slice(4, 6), 16) + Math.round(255 * factor));
