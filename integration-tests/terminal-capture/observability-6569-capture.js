...
const LONG_COMMAND = 'git log --format="%h %an %ad %s" --date=iso --since="6 months ago" -- ' +
    'packages/core/src/agents packages/cli/src/ui/components/background-view ' +
    '> /dev/null && sleep 45 && echo LONG_COMMAND_FINISHED';
function subagentRead(packageJsonPath, index) {
    return {
        toolCalls: [
            fakeToolCall('read_file', { file_path: packageJsonPath, offset: index * 3, limit: 5 }, `call_sub_read_${index}`),
        ],
        usage: { prompt_tokens: 50, completion_tokens: 12, total_tokens: 62 },
    };
}
function subagentShell() {
    return {
        toolCalls: [
            fakeToolCall('run_shell_command', { command: LONG_COMMAND, description: 'Scan recent agent history' }, 'call_sub_shell'),
        ],
...
async function main() {
    ...
    if (scenario !== 'detail' && scenario !== 'approval') {
        throw new Error('usage: observability-6569-capture.ts <detail|approval> <outputDir> <label>');
    }
    ...
    const fakeServer = await startFakeOpenAIServer(({ body }) => {
        ...
        const isSubagent = rc.includes('general-purpose agent') && rc.includes(SUBAGENT_MARKER);
        if (isSubagent) {
            subTurns += 1;
            if (subTurns <= readCalls)
                return subagentRead(packageJsonPath, subTurns);
            if (subTurns === readCalls + 1)
    ...
    try {
        ...
        await terminal.type('Use the general-purpose subagent to audit the agent runtime sources', { slow: true, delay: 8 });
        ...
        if (scenario === 'detail') {
            ...
            try {
                ...
                console.error(`[debug] server requests=${fakeServer.requests.length} mainTurns=${mainTurns} subTurns=${subTurns}`);
                console.error('[debug] screen:\n' + (await terminal.getScreenText()));
                throw err;
            }
            await terminal.idle(800, 5000);
            await terminal.capture(`${label}-panel.png`);
            writeFileSync(join(outputDir, `${label}-panel.txt`), await terminal.getScreenText());
            ...
            writeFileSync(join(outputDir, `${label}-detail.txt`), await terminal.getScreenText());
        }
        else {
            ...
            try {
                ...
                console.error(`[debug] requests=${fakeServer.requests.length} mainTurns=${mainTurns} subTurns=${subTurns}`);
                console.error('[debug] screen:\n' + (await terminal.getScreenText()));
                throw err;
            }
            await terminal.idle(800, 5000);
            await terminal.capture(`${label}-approval.png`);
            writeFileSync(join(outputDir, `${label}-approval.txt`), await terminal.getScreenText());
        }
...
//# sourceMappingURL=observability-6569-capture.js.map