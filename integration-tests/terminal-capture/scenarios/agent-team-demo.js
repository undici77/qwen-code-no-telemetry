...
const PROMPT = 'Use the agent team tools to create a team "explorers" with two teammates ' +
...
async function main() {
    ...
    const seen = (marker) => stripAnsi(terminal.getRawOutput())
        .toLowerCase()
     ...
     */
    const stream = async (marker, opts) => {
        let prevLen = -1;
        let captured = 0;
 ...
 */
function generateGif(frames, dir) {
    if (frames.length === 0)
        return null;
...
//# sourceMappingURL=agent-team-demo.js.map