...
const WORDS = 'lorem ipsum dolor sit amet consectetur adipiscing elit sed do eiusmod tempor incididunt ut labore et dolore magna aliqua enim ad minim veniam quis nostrud exercitation ullamco laboris nisi aliquip ex ea commodo consequat duis aute irure reprehenderit voluptate velit esse cillum'.split(' ');
/** A deterministic ~200-char cell that wraps across several lines. */
function longCell(seed) {
    let s = '';
    let i = seed;
    while (s.length < 190) {
...
function buildMarkdown() {
    const lines = ['Here are the requested tables:', ''];
    for (let t = 1; t <= NUM_TABLES; t++) {
        ...
        for (let r = 0; r < 2; r++) {
            const cells = [];
            for (let c = 0; c < 7; c++)
                cells.push(longCell(t * 100 + r * 7 + c));
...
function startChunkedServer(content) {
    let requests = 0;
...
async function main() {
    ...
    const repoRoot = resolve(process.env['QWEN_TUI_E2E_REPO'] ?? resolve(scriptDir, '../..'));
...
//# sourceMappingURL=table-pending-height-scroll-lock-regression.js.map