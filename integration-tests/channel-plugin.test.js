...
describe('Channel Plugin (Mock WebSocket E2E)', () => {
    ...
    let testDir;
    const setup = async () => {
        const baseDir = process.env['INTEGRATION_TEST_FILE_DIR'] ||
    ...
    it('should send a message through WebSocket and receive a real agent response', async () => {
        await setup();
        // This goes: server → WS → MockPluginChannel → ChannelBase → AcpBridge → agent → back
        const response = await server.sendMessage('What is 2+2? Reply with ONLY the number, nothing else.');
    ...
    it('should handle a different sender through the same WebSocket pipeline', async () => {
        const response = await server.sendMessage('What is 10 * 5? Reply with ONLY the number, nothing else.', {
...
//# sourceMappingURL=channel-plugin.test.js.map