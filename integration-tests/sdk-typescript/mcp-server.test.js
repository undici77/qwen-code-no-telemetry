...
import { query, isSDKAssistantMessage, isSDKResultMessage, isSDKSystemMessage, } from '@qwen-code/sdk';
import { SDKTestHelper, createMCPServer, extractText, findToolUseBlocks, findToolResult, createSharedTestOptions, createResultWaiter, } from './test-helper.js';
...
describe('MCP Server Integration (E2E)', () => {
        ...
        it('should support multi-turn asyncGenerator prompt with MCP tools', async () => {
            ...
            async function* createMultiTurnPrompt() {
                ...
                yield {
                    ...
                    message: {
                        role: 'user',
                        content: 'Now use the multiply tool to calculate 5 * 4. Give me the result.',
                    },
                    parent_tool_use_id: null,
                };
        ...
        it('should support multi-turn MCP tools with canUseTool', async () => {
            ...
            async function* createMultiTurnPrompt() {
                ...
                yield {
                    ...
                    message: {
                        role: 'user',
                        content: 'Now use the multiply tool to calculate 4 * 3. Give me the result.',
                    },
                    parent_tool_use_id: null,
                };
                await resultWaiter.waitForResult(1);
            ...
            try {
                ...
                expect(toolCalls).toContain(MCP_ADD_TOOL);
                expect(toolCalls).toContain(MCP_MULTIPLY_TOOL);
                expect(canUseToolCalls.map((call) => call.toolName)).toEqual(expect.arrayContaining([MCP_ADD_TOOL, MCP_MULTIPLY_TOOL]));
    ...
    describe('MCP Tool Message Flow', () => {
        it('should receive proper message sequence for MCP tool usage', async () => {
            const q = query({
                prompt: 'Use the add tool to calculate 2 + 3. You must call the tool. Just give me the result.',
                options: {
                    ...SHARED_TEST_OPTIONS,
                    cwd: testDir,
                    debug: false,
