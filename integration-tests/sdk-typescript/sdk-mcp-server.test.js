...
import { z } from 'zod';
import { query, tool, createSdkMcpServer, isSDKAssistantMessage, isSDKResultMessage, isSDKSystemMessage, } from '@qwen-code/sdk';
        ...
        it('should use SDK MCP tool with string operations', async () => {
            ...
            try {
                for await (const message of q) {
                    messages.push(message);
                    if (isSDKAssistantMessage(message)) {
                        const toolUseBlocks = findToolUseBlocks(message, MCP_REVERSE_STRING);
                        if (toolUseBlocks.length > 0) {
                            foundToolUse = true;
                        }
                        assistantText += extractText(message.message.content);
        ...
        it('should handle async tool handlers with delays', async () => {
            ...
            try {
                for await (const message of q) {
                    messages.push(message);
                    if (isSDKAssistantMessage(message)) {
                        const toolUseBlocks = findToolUseBlocks(message, MCP_DELAYED_RESPONSE);
                        if (toolUseBlocks.length > 0) {
                            foundToolUse = true;
                        }
                        assistantText += extractText(message.message.content);
