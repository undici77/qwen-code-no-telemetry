...
import { z } from 'zod';
import { query, tool, createSdkMcpServer, isSDKSystemMessage, } from '@qwen-code/sdk';
import { SDKTestHelper, findToolResults, createSharedTestOptions, assertSuccessfulCompletion, } from './test-helper.js';
...
function advertisedToolNames(fakeServer, requestIndex) {
    const tools = fakeServer.requests.filter(({ body }) => body['stream'] === true)[requestIndex]?.body['tools'];
...
describe('SDK MCP Server Integration (E2E)', () => {
    ...
    beforeEach(async () => {
        ...
        fakeServer = await startFakeOpenAIServer((context) => fakeResponse(context), FAKE_SERVER_OPTIONS);
    });
    afterEach(async () => {
    ...
    describe('Basic SDK MCP Tool Usage', () => {
        it('routes multiple tools from one SDK MCP server', async () => {
            ...
            const stringTool = tool('reverse_string', 'Reverse a string', { text: z.string().describe('The text to reverse') }, async (args) => ({
                content: [
            ...
            try {
                ...
                expect(advertisedToolNames(fakeServer, 1)).toEqual(expect.arrayContaining([MCP_CALCULATE_SUM, MCP_REVERSE_STRING]));
                ...
                expect(systemMessage?.mcp_servers?.some((server) => server.name === 'sdk-calculator')).toBe(true);
        ...
        it('keeps previously used MCP tools available when resuming a session', async () => {
            ...
            try {
                ...
                expect(findToolResults(firstMessages, MCP_CALCULATE_SUM)[0]?.content).toContain('42');
            ...
            try {
                ...
                const resumedResults = findToolResults(resumedMessages, MCP_CALCULATE_SUM);
    ...
    describe('Async Tool Handlers', () => {
        it('should handle async tool handlers with delays', async () => {
            ...
            try {
                ...
                expect(advertisedToolNames(fakeServer, 1)).toContain(MCP_DELAYED_RESPONSE);
