...
import { query, isSDKAssistantMessage, isSDKSystemMessage, } from '@qwen-code/sdk';
import { SDKTestHelper, createSharedTestOptions, findToolUseBlocks, findToolResults, assertSuccessfulCompletion, } from './test-helper.js';
        ...
        try {
            for await (const message of q) {
                if (isSDKSystemMessage(message) && message.subtype === 'init') {
                    expect(message.agents).toEqual(expect.arrayContaining(['greeter', 'math-helper']));
                    return;
                }
            }
    ...
    it('delegates a file read to a tool-restricted subagent', async () => {
        ...
        try {
            for await (const message of q) {
                ...
                if (agentToolUse)
                    agentToolUseId = agentToolUse.id;
                if (findToolUseBlocks(message, 'read_file').length > 0) {
                    readFileParentToolUseId = message.parent_tool_use_id;
                }
            }
            expect(fakeServer.requests.find(({ body }) => body['stream'] === true)?.body['tools']).toEqual(expect.arrayContaining([
