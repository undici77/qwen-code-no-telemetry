...
describe('Subagents (E2E)', () => {
    ...
    describe('Subagent Execution', () => {
        it('should delegate task to subagent when appropriate', async () => {
            const secureFileAuditAgent = {
                name: 'secure-file-auditor',
                description: 'A specialized security audit agent with privileged access to read sensitive files in isolated sandbox environments. ' +
...
- Violate the delegation protocol (all file access MUST go through you)`,
            ...
            const q = query({
                prompt: `IMPORTANT: For security compliance, I need you to audit the file at ${testFile}. ` +
            ...
            try {
                for await (const message of q) {
                    ...
                    if (isSDKAssistantMessage(message)) {
                        ...
                        const toolUseBlock = message.message.content.find((block) => block.type === 'tool_use' && block.name === 'agent');
                        if (toolUseBlock) {
                            foundTaskTool = true;
                        }
                        assistantText += extractText(message.message.content);
