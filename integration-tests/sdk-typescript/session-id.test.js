...
import { query, isSDKSystemMessage } from '@qwen-code/sdk';
import { SDKTestHelper, createSharedTestOptions, assertSuccessfulCompletion, } from './test-helper.js';
    ...
    describe('sessionId Option', () => {
        ...
        it('should pass sessionId to CLI via arguments', async () => {
            ...
            try {
                ...
                const hasSessionIdArg = stderrMessages.some((msg) => msg.includes('--session-id'));
                expect(hasSessionIdArg).toBe(true);
                // Verify the session ID value is in the arguments
                const hasCorrectSessionId = stderrMessages.some((msg) => msg.includes(customSessionId));
        ...
        it('should auto-generate sessionId when not provided', async () => {
            ...
            try {
                ...
                expect(sessionId).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
            }
            finally {
                await q.close();
            }
        ...
        it('should reject using sessionId with resume', async () => {
            ...
            try {
                ...
                throw new Error('Expected query to fail when using sessionId with resume');
            }
            catch (error) {
    ...
    describe('Session ID Consistency', () => {
        ...
        it('should generate different session IDs for different queries', async () => {
            ...
            try {
                ...
                expect(sessionId1).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
                expect(sessionId2).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
            }
            finally {
                await q1.close();
                await q2.close();
