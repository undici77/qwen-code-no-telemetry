...
describe('fake OpenAI server', () => {
    ...
    it('can close response connections for isolated latency measurements', async () => {
        server = await startFakeOpenAIServer(() => ({ content: 'no connection reuse' }), { keepAlive: false });
    ...
    it('closes the response when streaming fails after headers are sent', async () => {
        ...
        await expect(fetch(`${server.baseUrl}/chat/completions`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({
                model: 'fake-model',
...
//# sourceMappingURL=fake-openai-server.test.js.map