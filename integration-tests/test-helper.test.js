...
describe('TestRig', () => {
    ...
    it.each([
        [
            'telemetry events',
            (rig) => rig.waitForTelemetryEvent('tool_call'),
        ],
        ['tool calls', (rig) => rig.waitForToolCall('read_file')],
        [
            'any tool call',
            (rig) => rig.waitForAnyToolCall(['read_file', 'write_file']),
        ],
    ])('keeps polling for %s when telemetry is not ready yet', async (_label, waitFor) => {
        const rig = new TestRig();
