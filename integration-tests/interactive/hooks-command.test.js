...
describe('/hooks command', () => {
    ...
    it('should display hooks dialog when /hooks command is entered', async () => {
        ...
        const showedHooksDialog = await rig.poll(() => output.includes('Hooks') || output.includes('hooks'), 5000, 200);
        // Print output for debugging
        console.log('Output after /hooks command:');
        console.log(output);
        expect(showedHooksDialog, `Hooks dialog not shown. Output: ${output}`).toBe(true);
...
//# sourceMappingURL=hooks-command.test.js.map