...
import { TestRig, type, applyContainerSandboxNoProxy, fakeServerHostOptions, } from '../test-helper.js';
describe('Interactive protocol tag retry guard', () => {
    ...
    it.skipIf(process.platform === 'win32')('retries protocol leaks across SSE disconnect and completed streams', async () => {
        ...
        const { ptyProcess, promise } = rig.runInteractive('--auth-type', 'openai', '--openai-api-key', 'fake-key', '--openai-base-url', fakeServer.baseUrl, '--model', 'fake-model');
        try {
            const isReady = await rig.poll(() => /YOLO (模式|mode)/i.test(stripAnsi(rig._interactiveOutput)), 30000, 200);
            ...
            const sawVisibleSummary = await rig.waitForText('VISIBLE_TMUX_RETRY_RESPONSE_DONE', 15000);
            expect(sawVisibleSummary, 'Expected visible summary marker after HTTP retries').toBe(true);
            expect(fakeServer.requests).toHaveLength(3);
            expect(fakeServer.requests[1].body).toEqual(fakeServer.requests[0].body);
            expect(fakeServer.requests[2].body).toEqual(fakeServer.requests[0].body);
...
//# sourceMappingURL=protocol-tags-interactive.test.js.map