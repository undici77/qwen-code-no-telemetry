...
(IS_SANDBOX ? describe.skip : describe)('external context auto recall', () => {
    ...
    it('retrieves from the submitted prompt and adds results to the model request', async () => {
        ...
        closeContextProvider = () => new Promise((resolve, reject) => {
        ...
        const hookPath = join(import.meta.dirname, '..', '..', 'integrations', 'external-context', 'dist', 'auto-recall.js');
        const systemSettingsSourcePath = join(import.meta.dirname, '..', '..', 'integrations', 'external-context', 'examples', 'managed-auto-recall-system-settings.json');
        ...
        rig.createFile('.qwen-home/system-settings.json', await readFile(systemSettingsSourcePath, 'utf8'));
        rig.createFile('.qwen-home/trustedFolders.json', JSON.stringify({ [rig.testDir]: 'TRUST_FOLDER' }));
        const address = contextProvider.address();
        ...
        const { ptyProcess, promise } = rig.runInteractive('--auth-type', 'openai', '--openai-api-key', 'fake-key', '--openai-base-url', fakeModel.baseUrl, '--model', 'fake-model');
        try {
            ...
            expect(await rig.waitForText('AUTO_RECALL_E2E_DONE', 30_000), 'Fake model turn did not complete').toBe(true);
            ...
            const userMessages = messages.filter((message) => isRecord(message) && message['role'] === 'user');
            const systemMessages = messages.filter((message) => isRecord(message) && message['role'] === 'system');
...
//# sourceMappingURL=external-context-auto-recall.test.js.map