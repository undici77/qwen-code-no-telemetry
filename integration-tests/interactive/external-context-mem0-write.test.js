...
const IS_SANDBOX = Boolean(SANDBOX_MODE && SANDBOX_MODE !== 'false' && SANDBOX_MODE !== '0');
...
const LONG_CONFIRMATION_CONTENT = [
    ...
    ...Array.from({ length: 140 }, (_, index) => `repository-policy-line-${index.toString().padStart(3, '0')}`),
    'CONFIRM_TAIL',
].join('\n');
...
(IS_SANDBOX ? describe.skip : describe)('external context Mem0 write', () => {
    ...
    beforeEach(() => {
        rig = new TestRig();
        savedEnvironment = new Map(ENVIRONMENT_KEYS.map((key) => [key, process.env[key]]));
    });
    afterEach(async () => {
    ...
    it.each([
        {
            ...
            content: 'LINK [visible label](https://hidden.example/secret-target) BOLD **bold-value** CODE `code-value` UNDER <u>under-value</u>',
        ...
        closeMem0 = () => new Promise((resolve, reject) => {
        ...
        fakeModel = await startFakeOpenAIServer(({ requestIndex }) => requestIndex === 0
            ? {
                toolCalls: [
                    fakeToolCall('mcp__external-context__context_remember', { content }, 'call_external_context_remember'),
                ],
            }
            : { content: 'MEM0_WRITE_E2E_DONE' });
        const { ptyProcess, promise, screen } = runInteractive(rig, '--approval-mode', scenario.approvalMode, '--mcp-config', paths.mcpConfigPath, '--auth-type', 'openai', '--openai-api-key', 'fake-key', '--openai-base-url', fakeModel.baseUrl, '--model', 'fake-model');
        try {
            expect(await rig.waitForText('Type your message', 30_000), 'CLI did not start in interactive mode').toBe(true);
            await type(ptyProcess, 'Remember the repository policy.');
            await type(ptyProcess, '\r');
            if (scenario.expectsMcpConfirmation) {
                expect(await rig.waitForText('Allow execution of MCP tool', 30_000), 'ordinary MCP confirmation did not appear').toBe(true);
                await type(ptyProcess, '\r');
            }
            if (scenario.verifiesLongConfirmation) {
                const constrainedScreen = await waitForScreen(screen, (value) => value.includes('CONFIRM_TOP') &&
                    value.includes('lines hidden') &&
                    value.includes('Press ctrl-s to show more lines'), 'bounded literal content confirmation');
                const constrainedConfirmation = constrainedScreen.slice(constrainedScreen.lastIndexOf('Save this exact content to the bound Mem0 repository memory?'));
                expect(constrainedConfirmation).toContain('[visible](https://hidden.example/target)');
                ...
                const expandedScreen = await waitForScreen(screen, (value) => value.includes('CONFIRM_TAIL') && !value.includes('lines hidden'), 'expanded complete content confirmation');
                ...
                const screenWithConfirmation = await waitForScreen(screen, (value) => confirmationSection(value).includes('hidden.example/secret-target'), 'short literal content confirmation');
                ...
                expect(await rig.waitForText('Save this exact content to the bound Mem0 repository memory?', 30_000), 'content-visible Hook confirmation did not appear').toBe(true);
            ...
            if (scenario.approveWrite) {
                expect(await rig.waitForText('MEM0_WRITE_E2E_DONE', 30_000), 'fake model turn did not complete').toBe(true);
            }
            else {
            ...
            if (scenario.expectedRequests === 1) {
                ...
                const toolResult = toolResultText(fakeModel.requests[1]?.body['messages']);
...
async function configureManagedWrite(rig, mem0) {
    ...
    const integrationRoot = join(import.meta.dirname, '..', '..', 'integrations', 'external-context');
    ...
    rig.mkdir('.qwen-home');
    rig.createFile('.qwen-home/settings.json', JSON.stringify({
        hooks: {
            PreToolUse: [
                {
    ...
    const systemSettingsSource = join(integrationRoot, 'examples', 'managed-mem0-write-system-settings.json');
    rig.createFile('.qwen-home/system-settings.json', await readFile(systemSettingsSource, 'utf8'));
    rig.createFile('.qwen-home/trustedFolders.json', JSON.stringify({ [rig.testDir]: 'TRUST_FOLDER' }));
    rig.createFile('.qwen-home/external-context.json', JSON.stringify({
        version: 1,
        timeoutMs: 1000,
        write: { enabled: true },
        provider: {
    ...
    rig.createFile('.qwen-home/mcp.json', JSON.stringify({ mcpServers: { 'external-context': serverConfig } }));
    rig.createFile('.qwen-home/mcpApprovals.json', JSON.stringify({
        [rig.testDir]: {
            'external-context': {
...
function fakeMem0McpSource(integrationRoot) {
    const moduleUrl = (path) => pathToFileURL(join(integrationRoot, path)).href;
    const stdioUrl = pathToFileURL(join(import.meta.dirname, '..', '..', 'node_modules', '@modelcontextprotocol', 'sdk', 'dist', 'esm', 'server', 'stdio.js')).href;
    return [
...
function runInteractive(rig, ...args) {
    ...
    const ptyProcess = pty.spawn(process.execPath, [rig.bundlePath, '--no-chat-recording', ...args], {
        name: 'xterm-color',
        cols: 110,
        rows: 38,
        cwd: rig.testDir,
...
async function waitForScreen(screen, predicate, description, timeoutMs = 30_000) {
...
function confirmationSection(screen) {
    const heading = 'Save this exact content to the bound Mem0 repository memory?';
...
//# sourceMappingURL=external-context-mem0-write.test.js.map