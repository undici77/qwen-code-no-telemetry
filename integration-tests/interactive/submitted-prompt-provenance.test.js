...
describe('submitted prompt provenance', () => {
    ...
    it('keeps expanded file content out of submitted_prompt and omits it on tool continuation', async () => {
        ...
        rig.createFile('capture-hook-input.mjs', [
        ...
        fakeServer = await startFakeOpenAIServer(({ requestIndex }) => requestIndex === 0
            ? {
                toolCalls: [
                    fakeToolCall('read_file', { file_path: toolFile }, 'call_submitted_prompt_e2e'),
                ],
            }
            : { content: 'PROVENANCE_E2E_DONE' }, fakeServerHostOptions());
        ...
        try {
            ...
            expect(inputs[0]?.submitted_prompt).not.toEqual(expect.stringContaining(fileCanary));
...
//# sourceMappingURL=submitted-prompt-provenance.test.js.map