...
export class InteractiveSession {
    ...
    constructor(ptyProcess, terminal) {
        this.ptyProcess = ptyProcess;
        this.terminal = terminal;
        ptyProcess.onData((data) => {
            this.rawOutput += data;
            // Chain writes so flush() can await all pending data
            this.pendingWrite = this.pendingWrite.then(() => new Promise((resolve) => {
    ...
    async waitFor(text, timeout = 120_000) {
    ...
    async idle(stableMs = 5000, timeout = 120_000) {
        const start = Date.now();
     ...
     */
    async waitForScreen(predicate, description, timeout = 120_000) {
        const start = Date.now();
        while (Date.now() - start < timeout) {
            await sleep(3000);
