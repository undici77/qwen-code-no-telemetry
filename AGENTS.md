...
Verbatim in `.qwen/rules/web-shell.md`, path-gated on `packages/web-shell/**` and
auto-injected on first touch of that package. Read it before editing there.
...
Project review rules live verbatim in **`.qwen/review-rules.md`**, which `/review`
reads into every review agent's brief. Keep them there. Do not restore a
`## Code Review` heading to this file: `qwen review` extracts that heading from
here as well, so a second copy would be injected twice and paid for twice.
