...
git checkout v0.13.1-no-telemetry
git pull origin v0.13.1-no-telemetry

# 2. Create new branch for changes
git checkout -b v0.13.1-no-telemetry

# 3. Find merge base and cherry-pick commits
MERGE_BASE=$(git merge-base v0.13.1-no-telemetry main)
...
git diff v0.13.1-no-telemetry..v0.13.1-no-telemetry --name-status
```
