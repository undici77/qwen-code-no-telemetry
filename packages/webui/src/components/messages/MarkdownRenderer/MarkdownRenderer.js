 ...
 */
const createMarkdownInstance = () => new MarkdownIt({
 ...
 */
export const MarkdownRenderer = ({ content, onFileClick, enableFileLinks = true, }) => {
    ...
    const processFilePaths = (html) => {
        ...
        const FILE_PATH_WITH_LINES_NO_G = new RegExp(FILE_PATH_WITH_LINES_REGEX.source, FILE_PATH_WITH_LINES_REGEX.flags.replace('g', ''));
        ...
        const union = new RegExp(`${FILE_PATH_WITH_LINES_REGEX.source}|${FILE_PATH_REGEX.source}|${BARE_FILE_REGEX.source}`, 'gi');
        ...
        const upgradeAnchorIfFilePath = (a) => {
            ...
            if (FILE_PATH_WITH_LINES_NO_G.test(candidate) ||
    ...
    const handleContainerClick = useCallback((e) => {
        ...
        const anchor = (target.closest &&
            target.closest('a.file-path-link'));
        if (anchor) {
        ...
        const isRelativeFile = !isAbsolutePath &&
    ...
    return (_jsx("div", { className: "markdown-content", onClick: handleContainerClick, dangerouslySetInnerHTML: { __html: renderedHtml }, style: {
            wordWrap: 'break-word',
