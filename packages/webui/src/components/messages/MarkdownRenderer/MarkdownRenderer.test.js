...
let root = null;
let container = null;
function renderMarkdown(content, onFileClick = vi.fn(), enableFileLinks = false) {
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);
    flushSync(() => {
        root?.render(_jsx(MarkdownRenderer, { content: content, onFileClick: onFileClick, enableFileLinks: enableFileLinks }));
    });
    return { onFileClick };
}
afterEach(() => {
...
describe('MarkdownRenderer explicit file links', () => {
    it('opens markdown file links with decoded absolute paths', () => {
        const { onFileClick } = renderMarkdown('Saved: [export.html](/tmp/my%20dir/export.html)');
