...
export const WithSearch = {
    args: {
        visible: true,
        sessions: mockSessions.filter((s) => s.title.toLowerCase().includes('debug')),
...
export const ManySessions = {
    args: {
        ...
        sessions: Array.from({ length: 30 }, (_, i) => ({
            id: String(i + 1),
            title: `Session ${i + 1}`,
            lastUpdated: new Date(now.getTime() - i * 24 * 60 * 60 * 1000).toISOString(),
        })),
        currentSessionId: '5',
        searchQuery: '',
        hasMore: true,
...
//# sourceMappingURL=SessionSelector.stories.js.map