...
describe('CollapsibleOutput', () => {
    ...
    it('toggles the collapsed height while preserving all children', () => {
        act(() => {
            root?.render(_jsx(CollapsibleOutput, { isCollapsible: true, children: "full content with __TAIL__" }));
        });
        expect(container?.textContent).toContain('__TAIL__');
        const content = container?.querySelector('.toolcall-collapsible-output-content');
    ...
    it('does not clip or show a toggle when content is not collapsible', () => {
        act(() => {
            root?.render(_jsx(CollapsibleOutput, { isCollapsible: false, children: "short content" }));
        });
        const content = container?.querySelector('.toolcall-collapsible-output-content');
        expect(content.style.maxHeight).toBe('');
...
//# sourceMappingURL=CollapsibleOutput.test.js.map