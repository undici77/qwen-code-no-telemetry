 ...
 * ```
 */
export const extractCommandOutput = (text) => {
    ...
    const outputMatch = text.match(/Output:[ \t]{0,20}(.{0,1000}?)(?=\nError:|$)/i);
 ...
 */
export const shouldShowToolCall = (kind) => !kind.includes('internal');
/**
 ...
 */
export const groupContent = (content) => {
    const textOutputs = [];
    const errors = [];
    const diffs = [];
    const otherData = [];
    content?.forEach((item) => {
        if (item.type === 'diff') {
            ...
            const isErrorType = contentObj.type === 'error' &&
 ...
 */
export const hasToolCallOutput = (toolCall) => {
    ...
    if (toolCall.content && toolCall.content.length > 0) {
        ...
        if (grouped.textOutputs.length > 0 ||
 ...
 * - default fallback
 */
export const mapToolStatusToContainerStatus = (status) => {
    switch (status) {
        case 'pending':
        case 'in_progress':
            return 'loading';
