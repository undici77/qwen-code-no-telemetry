...
describe('ShellToolCall collapsible output', () => {
    ...
    const renderToolCall = (kind, output, openTempFile = vi.fn()) => {
        ...
        act(() => {
            root?.render(_jsx(PlatformProvider, { value: {
                    ...
                    onMessage: () => () => { },
                    openTempFile,
                }, children: _jsx(ShellToolCall, { toolCall: {
                        toolCallId: `${kind}-1`,
                        kind,
                        title: 'Run command',
                        status: 'completed',
    ...
    it.each(['bash', 'execute'])('preserves and expands the full %s output without opening a temp file', (kind) => {
        ...
        const toggle = container?.querySelector('button[aria-label="Expand output"]');
        expect(toggle).not.toBeNull();
        expect(toggle.getAttribute('aria-expanded')).toBe('false');
        const collapsibleContent = container?.querySelector('.toolcall-collapsible-output-content');
        ...
        expect(collapsibleContent.style.maxHeight).toBe('60px');
        expect(openTempFile).not.toHaveBeenCalled();
        const outRow = Array.from(container?.querySelectorAll(`.${kind}-toolcall-row`) ?? []).find((row) => row.textContent?.includes('OUT'));
        expect(outRow).toBeDefined();
        act(() => {
            outRow?.dispatchEvent(new MouseEvent('click', { bubbles: true }));
        });
        expect(openTempFile).toHaveBeenCalledWith(output, `${kind}-output-${kind}-1`);
    });
    it('does not add a toggle at the 500-character boundary', () => {
        renderToolCall('bash', 'x'.repeat(500));
        expect(container?.querySelector('button[aria-label="Expand output"]')).toBeNull();
    });
});
//# sourceMappingURL=ShellToolCall.test.js.map