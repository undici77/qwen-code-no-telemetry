...
describe('getToolDisplayLabel', () => {
    ...
    it('normalizes todo write labels even when older titles are still present', () => {
        expect(getToolDisplayLabel({ kind: 'todo_write', title: 'Updated Plan' })).toBe('TodoWrite');
        expect(getToolDisplayLabel({ kind: 'update_todos', title: 'Update Todos' })).toBe('TodoWrite');
        expect(getToolDisplayLabel({ kind: 'updated_plan', title: 'Updated Plan' })).toBe('TodoWrite');
    });
