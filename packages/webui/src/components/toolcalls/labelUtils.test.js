...
describe('getToolDisplayLabel', () => {
    ...
    it('normalizes todo write labels even when older titles are still present', () => {
        expect(getToolDisplayLabel({ kind: 'todo_write', title: 'Updated Plan' })).toBe('TodoList');
        expect(getToolDisplayLabel({ kind: 'update_todos', title: 'Update Todos' })).toBe('TodoList');
        expect(getToolDisplayLabel({ kind: 'updated_plan', title: 'Updated Plan' })).toBe('TodoList');
    });
    it('uses core names for read-family tools by kind', () => {
        expect(getToolDisplayLabel({ kind: 'read_many_files' })).toBe('ReadManyFiles');
    ...
    it('capitalizes generic label mappings that still fall through generic rendering', () => {
        ...
        expect(getToolDisplayLabel({ kind: 'exit_plan_mode' })).toBe('ExitPlanMode');
        expect(getToolDisplayLabel({ kind: 'switch_mode' })).toBe('ExitPlanMode');
    });
    it('returns EnterPlanMode for enter_plan_mode kind', () => {
        expect(getToolDisplayLabel({ kind: 'enter_plan_mode' })).toBe('EnterPlanMode');
    });
...
//# sourceMappingURL=labelUtils.test.js.map