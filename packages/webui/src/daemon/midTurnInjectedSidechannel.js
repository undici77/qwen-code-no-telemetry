...
export function subscribeSidechannelMidTurnInjected(listener) {
    listeners.add(listener);
    return () => {
        listeners.delete(listener);
    };
}
export function publishSidechannelMidTurnInjected(data) {
    ...
    if (appended.length > MAX_PENDING_BATCHES) {
        ...
        console.debug(`[mid-turn] sidechannel buffer over ${MAX_PENDING_BATCHES}; evicting ${dropped} oldest batch(es)`);
 ...
 */
export function consumeSidechannelMidTurnInjected(handled) {
    if (handled.length === 0 || pending.length === 0)
        return;
    const handledSet = new Set(handled);
    const next = pending.filter((batch) => !handledSet.has(batch));
    if (next.length === pending.length)
        return; // nothing matched — no notify
 ...
 */
export function parseSidechannelMidTurnInjected(event) {
    if (!event || typeof event !== 'object')
        return undefined;
    const record = event;
    if (record['type'] !== MID_TURN_MESSAGE_INJECTED_EVENT)
        return undefined;
    ...
    const stringMessages = messages.filter((message) => typeof message === 'string' && message.length > 0);
    if (stringMessages.length === 0)
        return undefined;
    const messageIds = dataRecord['messageIds'];
    const stringMessageIds = Array.isArray(messageIds) &&
...
//# sourceMappingURL=midTurnInjectedSidechannel.js.map