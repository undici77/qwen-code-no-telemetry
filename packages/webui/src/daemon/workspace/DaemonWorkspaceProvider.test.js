...
import { DaemonWorkspaceProvider, useDaemonWorkspace, useOptionalDaemonWorkspace, } from './DaemonWorkspaceProvider.js';
...
describe('DaemonWorkspaceProvider', () => {
    ...
    function renderWithProvider(children) {
        ...
        return new Promise((resolve) => {
            act(() => {
                root?.render(_jsx(DaemonWorkspaceProvider, { baseUrl: "http://127.0.0.1:4170", children: children }));
            });
            resolve();
        });
    }
    ...
    it('does not let the initial request overwrite a newer refresh', async () => {
        let resolveInitial;
        sdkMocks.capabilities.mockImplementationOnce(() => new Promise((resolve) => (resolveInitial = resolve)));
        let context;
        function Harness() {
            context = useOptionalDaemonWorkspace();
            return null;
    ...
    it('throws when useDaemonWorkspace is used without provider', async () => {
        ...
        expect(error?.message).toContain('useDaemonWorkspace must be used within DaemonWorkspaceProvider');
    });
    it('exposes workspace actions', async () => {
        ...
        expect(sdkMocks.setWorkspaceSkillEnabled).toHaveBeenCalledWith('review', false);
    });
    ...
    it('returns MCP tools fallback for older daemons', async () => {
        sdkMocks.workspaceMcpTools.mockRejectedValueOnce(new Error('missing route'));
        let actions;
        function Harness() {
            const workspace = useOptionalDaemonWorkspace();
            actions = workspace?.actions;
    ...
    it('loads MCP resources for a server', async () => {
        ...
        const workspaceActions = actions;
        await act(async () => {
            await expect(workspaceActions.loadMcpResources('docs')).resolves.toMatchObject({
                serverName: 'docs',
                resources: [{ uri: 'file:///docs/intro.md', name: 'Intro' }],
            });
        });
    ...
    it('returns MCP resources fallback for older daemons', async () => {
        sdkMocks.workspaceMcpResources.mockRejectedValueOnce(new Error('missing route'));
        let actions;
        function Harness() {
            const workspace = useOptionalDaemonWorkspace();
            actions = workspace?.actions;
        ...
        const workspaceActions = actions;
        await act(async () => {
            await expect(workspaceActions.loadMcpResources('server-a')).resolves.toEqual({
                v: 1,
                workspaceCwd: '',
                serverName: 'server-a',
                initialized: false,
    ...
    it('loads workspace glob matches', async () => {
        const fetchMock = vi.fn(async (_input, _init) => new Response(JSON.stringify({ matches: ['src/App.tsx', 42, 'src/index.ts'] }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
    ...
    it('actions.deleteSession throws when result has errors', async () => {
        ...
        if (!actions)
            throw new Error('actions not defined');
        await act(async () => {
            await expect(actions.deleteSession('session-456')).rejects.toThrow('invalid client id');
        });
    });
    ...
    it('useDaemonSessions exposes exportSession', async () => {
        ...
        if (!exportSession)
            throw new Error('exportSession not defined');
    ...
    it('useDaemonSessions exposes session list page metadata', async () => {
        ...
        expect(sdkMocks.listWorkspaceSessionsPage).toHaveBeenCalledWith('/mock-workspace', {
            pageSize: 10,
            cursor: 'cursor-1',
            view: 'organized',
            group: 'all',
...
//# sourceMappingURL=DaemonWorkspaceProvider.test.js.map