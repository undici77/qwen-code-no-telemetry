...
describe('scheduled-tasks workspace actions', () => {
    ...
    const initOf = (call) => call[1];
    const headersOf = (init) => (init.headers ?? {});
    it('lists tasks with a GET and returns the tasks array', async () => {
        fetchMock.mockResolvedValue(ok({ v: 1, tasks: [{ id: 'a' }, { id: 'b' }] }));
    ...
    it('throws with the server error message on a non-ok response', async () => {
        fetchMock.mockResolvedValue(fail(400, { error: 'bad cron', code: 'invalid_cron' }));
        await expect(makeActions().createScheduledTask({ cron: 'x', prompt: 'p' })).rejects.toThrow(/bad cron/);
    });
    describe('workspace-qualified targeting', () => {
        it('lists a named workspace via the qualified route', async () => {
            ...
            expect(fetchMock.mock.calls[0][0]).toBe('/workspaces/ws-2/scheduled-tasks');
        });
        it('creates in a named workspace via the qualified route', async () => {
            fetchMock.mockResolvedValue(ok({ id: 'x' }));
            await makeActions().createScheduledTask({ cron: '0 9 * * *', prompt: 'p' }, 'ws-2');
            expect(fetchMock.mock.calls[0][0]).toBe('/workspaces/ws-2/scheduled-tasks');
        ...
        it('patches / runs / deletes a task in a named workspace', async () => {
            ...
            expect(fetchMock.mock.calls[0][0]).toBe('/workspaces/ws-2/scheduled-tasks/a');
            expect(fetchMock.mock.calls[1][0]).toBe('/workspaces/ws-2/scheduled-tasks/a/run');
            expect(fetchMock.mock.calls[2][0]).toBe('/workspaces/ws-2/scheduled-tasks/a');
        });
        it('url-encodes both the workspace selector and the task id', async () => {
            fetchMock.mockResolvedValue(ok({ id: 'a' }));
            await makeActions().updateScheduledTask('a/b', { enabled: false }, '/abs/path');
            expect(fetchMock.mock.calls[0][0]).toBe('/workspaces/%2Fabs%2Fpath/scheduled-tasks/a%2Fb');
        });
...
//# sourceMappingURL=scheduledTasks.actions.test.js.map