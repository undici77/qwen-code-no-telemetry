...
export function useDaemonAuth(options = {}) {
    const workspaceActions = useDaemonWorkspaceActions();
    const load = useCallback(() => workspaceActions.getAuthStatus(), [workspaceActions]);
    const result = useDaemonResource(load, options);
    const signals = useDaemonWorkspaceEventSignals();
    useWorkspaceEventReload(signals?.authVersion, result.reload, options.autoLoad === true || result.data !== undefined);
    return {
        ...result,
        status: result.data,
...
//# sourceMappingURL=useDaemonAuth.js.map