...
export function useDaemonAgents(options = {}) {
    const workspaceActions = useDaemonWorkspaceActions();
    const load = useCallback(() => workspaceActions.listAgents(), [workspaceActions]);
    const result = useDaemonResource(load, options);
    const signals = useDaemonWorkspaceEventSignals();
    useWorkspaceEventReload(signals?.agentsVersion, result.reload, options.autoLoad === true || result.data !== undefined);
    return {
        ...result,
        status: result.data,
...
//# sourceMappingURL=useDaemonAgents.js.map