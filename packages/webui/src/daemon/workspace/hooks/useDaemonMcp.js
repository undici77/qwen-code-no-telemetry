...
export function useDaemonMcp(options = {}) {
    const workspaceActions = useDaemonWorkspaceActions();
    const load = useCallback(() => workspaceActions.loadMcpStatus(), [workspaceActions]);
    const result = useDaemonResource(load, options);
    const signals = useDaemonWorkspaceEventSignals();
    useWorkspaceEventReload(signals?.mcpVersion, result.reload, options.autoLoad === true || result.data !== undefined);
    return {
        ...result,
        status: result.data,
...
//# sourceMappingURL=useDaemonMcp.js.map