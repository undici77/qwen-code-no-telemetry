...
export function useDaemonMemory(options = {}) {
    const workspaceActions = useDaemonWorkspaceActions();
    const load = useCallback(() => workspaceActions.loadMemoryStatus(), [workspaceActions]);
    const result = useDaemonResource(load, options);
    const signals = useDaemonWorkspaceEventSignals();
    useWorkspaceEventReload(signals?.memoryVersion, result.reload, options.autoLoad === true || result.data !== undefined);
    return {
        ...result,
        status: result.data,
...
//# sourceMappingURL=useDaemonMemory.js.map