...
const DaemonWorkspaceContext = createContext(undefined);
...
export function DaemonWorkspaceProvider({ baseUrl, token, workspaceCwd, autoConnect = true, transport, children, }) {
    const client = useMemo(() => autoConnect ? new DaemonClient({ baseUrl, token, transport }) : undefined, [autoConnect, baseUrl, token, transport]);
    const clientRef = useRef(client);
    clientRef.current = client;
    const capabilitiesClientRef = useRef(undefined);
    const capabilitiesPromiseRef = useRef(undefined);
    ...
    const [status, setStatus] = useState(autoConnect ? 'connecting' : 'idle');
    ...
    const getCapabilities = useCallback(() => {
        ...
        if (!capabilitiesPromiseRef.current) {
            const promise = client.capabilities().catch((error) => {
    ...
    const refreshCapabilities = useCallback(() => {
        ...
        const followAcceptedSuccessor = () => {
            ...
            return Promise.reject(new Error('Capabilities refresh was superseded by a client change'));
        };
        const acceptedPromise = client.capabilities().then((caps) => {
            if (capabilitiesClientRef.current !== client ||
                capabilitiesGenerationRef.current !== generation) {
    ...
    const workspaceActions = useMemo(() => createDaemonWorkspaceActions({
    ...
    return (_jsx(DaemonWorkspaceContext.Provider, { value: contextValue, children: children }));
}
export function useDaemonWorkspace() {
...
//# sourceMappingURL=DaemonWorkspaceProvider.js.map