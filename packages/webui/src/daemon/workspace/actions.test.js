...
describe('workspace actions', () => {
    ...
    it('forwards workspace updates to the daemon client', async () => {
        ...
        const actions = createDaemonWorkspaceActions({
            getClient: () => ({ updateWorkspace }),
            getWorkspaceCwd: () => '/ws',
            baseUrl: '',
        });
        await expect(actions.updateWorkspace('secondary', { displayName: 'Payments' })).resolves.toEqual(workspace);
    ...
    it('allows the SDK archive timeout to run before the wrapper timeout', async () => {
        vi.useFakeTimers();
        const installExtensionArchive = vi.fn(() => new Promise(() => { }));
        const actions = createDaemonWorkspaceActions({
            getClient: () => ({ installExtensionArchive }),
            getWorkspaceCwd: () => '/ws',
            baseUrl: '',
        });
    ...
    it('applies the action timeout to workspace removal', async () => {
        ...
        const actions = createDaemonWorkspaceActions({
            getClient: () => ({ workspaceById: () => ({ remove }) }),
            getWorkspaceCwd: () => '/ws',
            baseUrl: '',
        });
        const result = actions
    ...
    it('forwards successful workspace removal results', async () => {
        ...
        const actions = createDaemonWorkspaceActions({
            getClient: () => ({ workspaceById }),
            getWorkspaceCwd: () => '/ws',
            baseUrl: '',
        });
        await expect(actions.removeWorkspace('secondary', { force: false })).resolves.toEqual(removal);
    ...
    it('rejects workspace removal without a connected client', async () => {
        ...
        await expect(actions.removeWorkspace('secondary')).rejects.toThrow('Remove workspace failed: DaemonClient is not connected');
    });
    it('preserves zero as the disabled timeout sentinel', async () => {
        ...
        await expect(actions.removeWorkspace('secondary', { timeoutMs: 0 })).resolves.toEqual(removal);
    ...
    it('does not preempt SDK timeouts for channel mutations', async () => {
        ...
        let resolveUpsert;
        let resolveApproval;
        const upsertWorkspaceChannel = vi.fn(() => new Promise((resolve) => {
            resolveUpsert = resolve;
        }));
        const approveWorkspaceChannelPairing = vi.fn(() => new Promise((resolve) => {
    ...
    it('loads active extension operations from the daemon client', async () => {
        ...
        const actions = createDaemonWorkspaceActions({
            getClient: () => ({ activeExtensionOperations }),
            getWorkspaceCwd: () => '/workspace',
            baseUrl: 'http://daemon',
        });
    ...
    it('forwards an extension interaction response to the daemon client', async () => {
        ...
        const actions = createDaemonWorkspaceActions({
            getClient: () => ({ respondToExtensionInteraction }),
            getWorkspaceCwd: () => '/workspace',
            baseUrl: 'http://daemon',
        });
        await expect(actions.respondToExtensionInteraction('op-1', 'interaction-1', { value: 'answer' }, 'client-1')).resolves.toEqual({ accepted: true });
        expect(respondToExtensionInteraction).toHaveBeenCalledWith('op-1', 'interaction-1', { value: 'answer' }, 'client-1');
    });
    it('rejects when no daemon client is connected', async () => {
        ...
        await expect(actions.respondToExtensionInteraction('op-1', 'interaction-1', {
            cancelled: true,
    ...
    it('routes Channel management and pairing through the current workspace', async () => {
        ...
        await expect(actions.channelPairing.approve('bot', 'abcdefgh')).resolves.toBe(approval);
        await expect(actions.channelPairing.approvals('bot')).resolves.toBe(pairingApprovals);
        await expect(actions.channelPairing.revoke('bot', { senderId: 'sender-1' })).resolves.toBe(pairingRevocation);
        await expect(actions.channelPairing.revoke('bot', { groupId: 'group-1' })).resolves.toBe(pairingRevocation);
        ...
        expect(workspace.approveWorkspaceChannelPairing).toHaveBeenCalledWith('bot', { code: 'abcdefgh' });
        expect(workspace.workspaceChannelPairingApprovals).toHaveBeenCalledWith('bot');
        expect(workspace.revokeWorkspaceChannelPairingApproval).toHaveBeenCalledWith('bot', { senderId: 'sender-1' });
        expect(workspace.revokeWorkspaceChannelPairingApproval).toHaveBeenCalledWith('bot', { groupId: 'group-1' });
    });
    it('rejects Channel management without a selected workspace', async () => {
        ...
        await expect(actions.loadChannels()).rejects.toThrow('Daemon workspace is not connected');
    ...
    it('forwards the directory picker to the daemon client', async () => {
        ...
        const actions = createDaemonWorkspaceActions({
            getClient: () => ({ workspaceDirectoryPicker }),
            getWorkspaceCwd: () => '/ws',
            baseUrl: '',
        });
        await expect(actions.pickWorkspaceDirectory()).resolves.toEqual(pickerResult);
        expect(workspaceDirectoryPicker).toHaveBeenCalledOnce();
    });
    it('applies the 320s timeout to the directory picker', async () => {
        vi.useFakeTimers();
        const workspaceDirectoryPicker = vi.fn(() => new Promise(() => { }));
        const actions = createDaemonWorkspaceActions({
            getClient: () => ({ workspaceDirectoryPicker }),
            getWorkspaceCwd: () => '/ws',
            baseUrl: '',
        });
        const result = actions.pickWorkspaceDirectory().then(() => undefined, (error) => error);
    ...
    it('rejects the directory picker without a connected client', async () => {
        ...
        await expect(actions.pickWorkspaceDirectory()).rejects.toThrow('Open directory picker failed: DaemonClient is not connected');
    });
});
