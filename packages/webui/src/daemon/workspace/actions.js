 ...
 */
import { EXTENSION_ARCHIVE_UPLOAD_TIMEOUT_MS, } from '@qwen-code/sdk/daemon';
import { withActionTimeout } from '../timing.js';
const AGENT_GENERATE_TIMEOUT_MS = 330_000;
export function createDaemonWorkspaceActions({ getClient, getWorkspaceCwd, baseUrl, token, }) {
    return {
        async listSessions(options) {
            ...
            if (!cwd)
                return [];
            return withActionTimeout(client
                .listWorkspaceSessionsPage(cwd, options)
                .then((page) => page.sessions), 'List sessions timed out');
        },
        async listSessionsPage(options) {
            ...
            if (!cwd)
                return { sessions: [] };
            return withActionTimeout(client.listWorkspaceSessionsPage(cwd, options), 'List sessions timed out');
        },
        async listSessionGroups() {
            ...
            return withActionTimeout(client.listSessionGroups(cwd), 'List session groups timed out');
        },
        async createSessionGroup(input) {
            ...
            return withActionTimeout(client.createSessionGroup(cwd, input), 'Create session group timed out');
        },
        async updateSessionGroup(groupId, update) {
            const client = requireClient(getClient, 'Update session group failed');
            const cwd = requireWorkspaceCwd(getWorkspaceCwd);
            return withActionTimeout(client.updateSessionGroup(cwd, groupId, update), 'Update session group timed out');
        },
        async deleteSessionGroup(groupId) {
            const client = requireClient(getClient, 'Delete session group failed');
            const cwd = requireWorkspaceCwd(getWorkspaceCwd);
            return withActionTimeout(client.deleteSessionGroup(cwd, groupId), 'Delete session group timed out');
        },
        async updateSessionOrganization(sessionId, update) {
            const client = requireClient(getClient, 'Update session organization failed');
            return withActionTimeout(client.updateSessionOrganization(sessionId, update), 'Update session organization timed out');
        },
        async deleteSession(sessionId) {
            const client = requireClient(getClient, 'Delete session failed');
            const result = await withActionTimeout(client.deleteSessionsData([sessionId]), 'Delete session timed out');
        ...
        async deleteSessions(sessionIds) {
            ...
            return withActionTimeout(client.deleteSessionsData(sessionIds), 'Delete sessions timed out');
        },
        async exportSession(sessionId, format = 'html') {
            const client = requireClient(getClient, 'Export session failed');
            return withActionTimeout(client.exportSession(sessionId, { format }), 'Export session timed out');
        },
        async archiveSession(sessionId) {
            const client = requireClient(getClient, 'Archive session failed');
            const result = await withActionTimeout(client.archiveSessionsData([sessionId]), 'Archive session timed out');
            if (result.errors.length > 0) {
                throw new Error(result.errors[0].error);
            }
        ...
        async unarchiveSession(sessionId) {
            ...
            const result = await withActionTimeout(client.unarchiveSessionsData([sessionId]), 'Unarchive session timed out');
            if (result.errors.length > 0) {
                throw new Error(result.errors[0].error);
            }
        ...
        async loadChannels() {
            const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Load channels failed');
            const [catalog, snapshot] = await withActionTimeout(Promise.all([
        ...
        async upsertChannel(name, request) {
            const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Update channel failed');
            return workspace.upsertWorkspaceChannel(name, request);
        },
        async removeChannel(name, request) {
            const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Remove channel failed');
            return workspace.deleteWorkspaceChannel(name, request);
        },
        async setChannelStartup(name, request) {
            const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Update channel startup failed');
            return workspace.setWorkspaceChannelStartup(name, request);
        },
        async startChannel(name) {
            const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Start channel failed');
            return workspace.startWorkspaceChannel(name);
        },
        async stopChannel(name) {
            const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Stop channel failed');
            return workspace.stopWorkspaceChannel(name);
        },
        async restartChannel(name) {
            const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Restart channel failed');
            return workspace.restartWorkspaceChannel(name);
        },
        channelPairing: {
            async list(name) {
                const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Load channel pairing requests failed');
                return withActionTimeout(workspace.workspaceChannelPairingRequests(name), 'Load channel pairing requests timed out');
            },
            async approve(name, code) {
                const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Approve channel pairing failed');
                return workspace.approveWorkspaceChannelPairing(name, { code });
            },
            async approvals(name) {
                const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Load channel pairing approvals failed');
                return withActionTimeout(workspace.workspaceChannelPairingApprovals(name), 'Load channel pairing approvals timed out');
            },
            async revoke(name, request) {
                const workspace = requireWorkspaceClient(getClient, getWorkspaceCwd, 'Revoke channel pairing approval failed');
        ...
        async loadMcpStatus() {
            ...
            return withActionTimeout(client.workspaceMcp(), 'Load MCP status timed out');
        },
        async initializeMcp() {
            const client = requireClient(getClient, 'Initialize MCP failed');
            return withActionTimeout(client.initializeWorkspaceMcp(), 'Initialize MCP timed out');
        },
        async reloadMcp() {
            const client = requireClient(getClient, 'Reload MCP failed');
            return withActionTimeout(client.reloadWorkspaceMcp(), 'Reload MCP timed out');
        },
        async loadMcpTools(serverName) {
            const client = requireClient(getClient, 'Load MCP tools failed');
            try {
                return await withActionTimeout(client.workspaceMcpTools(serverName), 'Load MCP tools timed out');
            }
            catch {
                return {
                    v: 1,
        ...
        async loadMcpResources(serverName) {
            ...
            try {
                return await withActionTimeout(client.workspaceMcpResources(serverName), 'Load MCP resources timed out');
            }
            catch {
        ...
        async restartMcpServer(serverName) {
            ...
            return withActionTimeout(client.restartMcpServer(serverName), 'Restart MCP server timed out', 5 * 60_000);
        },
        async manageMcpServer(serverName, action) {
            ...
            return withActionTimeout(client.manageMcpServer(serverName, action), 'Manage MCP server timed out', timeoutMs);
        },
        async addRuntimeMcpServer(request) {
            const client = requireClient(getClient, 'Add MCP server failed');
            return withActionTimeout(client.addRuntimeMcpServer(request), 'Add MCP server timed out', 5 * 60_000);
        },
        async removeRuntimeMcpServer(name) {
            const client = requireClient(getClient, 'Remove MCP server failed');
            return withActionTimeout(client.removeRuntimeMcpServer(name), 'Remove MCP server timed out', 5 * 60_000);
        },
        async loadDaemonStatus(detail) {
            const client = requireClient(getClient, 'Load daemon status failed');
            return withActionTimeout(client.daemonStatus(detail), 'Load daemon status timed out');
        },
        async loadUsageDashboard(opts) {
            const client = requireClient(getClient, 'Load usage dashboard failed');
            return withActionTimeout(client.usageDashboard(opts), 'Load usage dashboard timed out');
        },
        async loadSkillsStatus() {
            const client = requireClient(getClient, 'Load skills failed');
            return withActionTimeout(client.workspaceSkills(), 'Load skills timed out');
        },
        async setWorkspaceSkillEnabled(skillName, enabled) {
            const client = requireClient(getClient, 'Set skill enabled failed');
            return withActionTimeout(client.setWorkspaceSkillEnabled(skillName, enabled), 'Set skill enabled timed out');
        },
        async installWorkspaceSkill(request) {
            const client = requireClient(getClient, 'Install skill failed');
            return withActionTimeout(client.installWorkspaceSkill(request), 'Install skill timed out');
        },
        async deleteWorkspaceSkill(skillName, scope) {
            const client = requireClient(getClient, 'Delete skill failed');
            return withActionTimeout(client.deleteWorkspaceSkill(skillName, scope), 'Delete skill timed out');
        },
        async loadExtensionsStatus() {
            const client = requireClient(getClient, 'Load extensions failed');
            return withActionTimeout(client.workspaceExtensions(), 'Load extensions timed out');
        },
        ...
        async preheatAcp(timeoutMs) {
            ...
            return withActionTimeout(client.workspaceAcpPreheat(timeoutMs), 'Preheat ACP timed out', timeoutMs === undefined ? undefined : timeoutMs + 2_000);
        },
        async setWorkspaceToolEnabled(toolName, enabled) {
            const client = requireClient(getClient, 'Set tool enabled failed');
            return withActionTimeout(client.setWorkspaceToolEnabled(toolName, enabled), 'Set tool enabled timed out');
        },
        async loadSettingsStatus() {
            const client = requireClient(getClient, 'Load settings failed');
            return withActionTimeout(client.workspaceSettings(), 'Load settings timed out');
        },
        async setWorkspaceSetting(scope, key, value, options) {
            const client = requireClient(getClient, 'Set setting failed');
            return withActionTimeout(client.setWorkspaceSetting(scope, key, value, options), 'Set setting timed out');
        },
        async loadMemoryStatus() {
            const client = requireClient(getClient, 'Load memory failed');
            return withActionTimeout(client.workspaceMemory(), 'Load memory timed out');
        },
        async readWorkspaceFile(filePath) {
            const client = requireClient(getClient, 'Read workspace file failed');
            return withActionTimeout(client.readWorkspaceFile(filePath), 'Read workspace file timed out');
        },
        async writeMemory(req) {
            const client = requireClient(getClient, 'Write memory failed');
            return withActionTimeout(client.writeWorkspaceMemory(req), 'Write memory timed out');
        },
        ...
        async listAgents() {
            ...
            return withActionTimeout(client.listWorkspaceAgents(), 'List agents timed out');
        },
        async getAgent(agentType, scope) {
            const client = requireClient(getClient, 'Get agent failed');
            return withActionTimeout(client.getWorkspaceAgent(agentType, scope ? { scope } : {}), 'Get agent timed out');
        },
        async createAgent(req) {
            const client = requireClient(getClient, 'Create agent failed');
            return withActionTimeout(client.createWorkspaceAgent(req), 'Create agent timed out');
        },
        async generateAgent(description) {
            const client = requireClient(getClient, 'Generate agent failed');
            return withActionTimeout(client.generateWorkspaceAgent(description), 'Generate agent timed out', AGENT_GENERATE_TIMEOUT_MS);
        },
        async deleteAgent(agentType, scope) {
            const client = requireClient(getClient, 'Delete agent failed');
            return withActionTimeout(client.deleteWorkspaceAgent(agentType, scope ? { scope } : {}), 'Delete agent timed out');
        },
        ...
        async globWorkspace(pattern, opts) {
            ...
            const res = await withActionTimeout(fetch(serializeDaemonRequestUrl(url, baseUrl), {
            ...
            return {
                matches: Array.isArray(data.matches)
                    ? data.matches.filter((match) => typeof match === 'string')
                    : [],
            };
        },
        async loadProviders() {
            const client = requireClient(getClient, 'Load providers failed');
            return withActionTimeout(client.workspaceProviders(), 'Load providers timed out');
        },
        async readFileBytes(filePath, opts) {
            const client = requireClient(getClient, 'Read file bytes failed');
            return withActionTimeout(client.readWorkspaceFileBytes(filePath, opts ?? {}), 'Read file bytes timed out');
        },
        async writeFile(req) {
            const client = requireClient(getClient, 'Write file failed');
            return withActionTimeout(client.writeWorkspaceFile(req), 'Write file timed out');
        },
        async editFile(req) {
            const client = requireClient(getClient, 'Edit file failed');
            return withActionTimeout(client.editWorkspaceFile(req), 'Edit file timed out');
        },
        async stat(filePath) {
        ...
        async updateScheduledTask(id, patch, workspaceId) {
            ...
            const path = scheduledTasksPath(workspaceId, `/${encodeURIComponent(id)}`);
            const url = createDaemonRequestUrl(baseUrl, path);
            const res = await withActionTimeout(fetch(serializeDaemonRequestUrl(url, baseUrl), {
                method: 'PATCH',
                headers: createDaemonJsonHeaders(token),
        ...
        async runScheduledTask(id, workspaceId) {
            ...
            const path = scheduledTasksPath(workspaceId, `/${encodeURIComponent(id)}/run`);
            const url = createDaemonRequestUrl(baseUrl, path);
            const res = await withActionTimeout(fetch(serializeDaemonRequestUrl(url, baseUrl), {
                method: 'POST',
                headers: createDaemonJsonHeaders(token),
        ...
        async listGoals() {
            ...
            return {
                ...
                droppedCount: typeof data.droppedCount === 'number' && data.droppedCount > 0
                    ? data.droppedCount
                    : 0,
            };
        },
        async clearGoal(sessionId) {
            ...
            const url = createDaemonRequestUrl(baseUrl, `/session/${encodeURIComponent(sessionId)}/goal/clear`);
            const res = await withActionTimeout(fetch(serializeDaemonRequestUrl(url, baseUrl), {
                method: 'POST',
                headers: createDaemonJsonHeaders(token),
                body: '{}',
            ...
            if (!res.ok) {
                throw new Error(await readDaemonError(res, `POST /session/${sessionId}/goal/clear`));
            }
            return (await res.json());
        },
        async loadEnv() {
        ...
        async loadPreflight() {
            ...
            return withActionTimeout(client.workspacePreflight(), 'Load preflight timed out');
        },
        async initWorkspace(opts) {
            const client = requireClient(getClient, 'Init workspace failed');
            return withActionTimeout(client.initWorkspace(opts), 'Init workspace timed out');
        },
        async updateAgent(agentType, req, scope) {
            const client = requireClient(getClient, 'Update agent failed');
            return withActionTimeout(client.updateWorkspaceAgent(agentType, req, scope ? { scope } : {}), 'Update agent timed out');
        },
        async installExtension(params, clientId) {
            const client = requireClient(getClient, 'Install extension failed');
            return withActionTimeout(client.installExtension(params, clientId), 'Install extension timed out');
        },
        async installExtensionArchive(params, clientId) {
            const client = requireClient(getClient, 'Install extension failed');
            return withActionTimeout(client.installExtensionArchive(params, clientId), 'Install extension timed out', EXTENSION_ARCHIVE_UPLOAD_TIMEOUT_MS + 10_000);
        },
        async extensionOperationStatus(operationId) {
            const client = requireClient(getClient, 'Load extension operation failed');
            return withActionTimeout(client.extensionOperationStatus(operationId), 'Load extension operation timed out');
        },
        async activeExtensionOperations() {
            const client = requireClient(getClient, 'Load active extension operations failed');
            return withActionTimeout(client.activeExtensionOperations(), 'Load active extension operations timed out');
        },
        async respondToExtensionInteraction(operationId, interactionId, response, clientId) {
            const client = requireClient(getClient, 'Respond to extension interaction failed');
            return withActionTimeout(client.respondToExtensionInteraction(operationId, interactionId, response, clientId), 'Respond to extension interaction timed out');
        },
        async checkExtensionUpdates(clientId) {
            const client = requireClient(getClient, 'Check extension updates failed');
            return withActionTimeout(client.checkExtensionUpdates(clientId), 'Check extension updates timed out');
        },
        async refreshExtensions(clientId) {
            const client = requireClient(getClient, 'Refresh extensions failed');
            return withActionTimeout(client.refreshExtensions(clientId), 'Refresh extensions timed out');
        },
        async enableExtension(name, params, clientId) {
            const client = requireClient(getClient, 'Enable extension failed');
            return withActionTimeout(client.enableExtension(name, params, clientId), 'Enable extension timed out');
        },
        async disableExtension(name, params, clientId) {
            const client = requireClient(getClient, 'Disable extension failed');
            return withActionTimeout(client.disableExtension(name, params, clientId), 'Disable extension timed out');
        },
        async updateExtension(name, clientId) {
            const client = requireClient(getClient, 'Update extension failed');
            return withActionTimeout(client.updateExtension(name, clientId), 'Update extension timed out');
        },
        async uninstallExtension(name, clientId) {
            const client = requireClient(getClient, 'Uninstall extension failed');
            return withActionTimeout(client.uninstallExtension(name, clientId), 'Uninstall extension timed out');
        },
        async startDeviceFlow(providerId) {
            const client = requireClient(getClient, 'Start device flow failed');
            return withActionTimeout(client.startDeviceFlow({ providerId }), 'Start device flow timed out');
        },
        async getDeviceFlow(deviceFlowId, opts) {
            const client = requireClient(getClient, 'Get device flow failed');
            return withActionTimeout(client.getDeviceFlow(deviceFlowId, opts), 'Get device flow timed out');
        },
        async cancelDeviceFlow(deviceFlowId) {
            const client = requireClient(getClient, 'Cancel device flow failed');
            return withActionTimeout(client.cancelDeviceFlow(deviceFlowId), 'Cancel device flow timed out');
        },
        async getAuthStatus() {
            const client = requireClient(getClient, 'Get auth status failed');
            return withActionTimeout(client.getAuthStatus(), 'Get auth status timed out');
        },
        async getAuthProviders() {
            const client = requireClient(getClient, 'Get auth providers failed');
            return withActionTimeout(client.getAuthProviders(), 'Get auth providers timed out');
        },
        async installAuthProvider(req) {
            const client = requireClient(getClient, 'Install auth provider failed');
            return withActionTimeout(client.installAuthProvider(req), 'Install auth provider timed out');
        },
        async deleteModel(target) {
            const client = requireClient(getClient, 'Delete model failed');
            return withActionTimeout(client.deleteModel(target), 'Delete model timed out');
        },
        async addWorkspace(cwd, options) {
            const client = requireClient(getClient, 'Add workspace failed');
            return withActionTimeout(client.addWorkspace(cwd, options), 'Add workspace timed out');
        },
        ...
        async addScratchWorkspace() {
            ...
            return withActionTimeout(client.addScratchWorkspace(), 'Add scratch workspace timed out');
        },
        async suggestWorkspacePaths(prefix) {
            const client = requireClient(getClient, 'Suggest workspace paths failed');
            const result = await withActionTimeout(client.workspacePathSuggestions(prefix), 'Suggest workspace paths timed out');
            return result;
        },
        async pickWorkspaceDirectory() {
            const client = requireClient(getClient, 'Open directory picker failed');
            const result = await withActionTimeout(client.workspaceDirectoryPicker(), 'Open directory picker timed out', 320_000);
            return result;
        },
        async updateWorkspace(workspaceSelector, update) {
            const client = requireClient(getClient, 'Update workspace failed');
            return withActionTimeout(client.updateWorkspace(workspaceSelector, update), 'Update workspace timed out');
        },
        async removeWorkspace(workspaceId, options) {
            ...
            if (options?.timeoutMs === 0)
                return removal;
            return withActionTimeout(removal, 'Remove workspace timed out', options?.timeoutMs);
        },
    };
}
// ── Helpers ─────────────────────────────────────────────────────────
