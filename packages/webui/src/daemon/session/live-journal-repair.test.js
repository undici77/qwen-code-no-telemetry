...
import { findLiveJournalRepairSuffix, findLiveJournalRepairTarget, } from './live-journal-repair.js';
...
const update = (id, promptId, sessionUpdate) => ({
    id,
    v: 1,
    type: 'session_update',
    promptId,
...
describe('live journal repair validation', () => {
    it('prefers marker promptId and falls back only to a unique retained prompt', () => {
        expect(findLiveJournalRepairTarget('session-1', [
            marker('prompt-authoritative'),
            update(5, 'prompt-other', 'agent_message_chunk'),
        ], 5, false)?.promptId).toBe('prompt-authoritative');
        expect(findLiveJournalRepairTarget('session-1', [marker(), update(5, 'prompt-fallback', 'agent_message_chunk')], 5, false)?.promptId).toBe('prompt-fallback');
        expect(findLiveJournalRepairTarget('session-1', [
            marker(),
            update(5, 'prompt-a', 'agent_message_chunk'),
            update(6, 'prompt-b', 'agent_message_chunk'),
        ], 6, false)).toBeUndefined();
    });
    it('rejects degraded or non-recoverable live markers', () => {
        expect(findLiveJournalRepairTarget('session-1', [marker('prompt-1')], 5, true)).toBeUndefined();
        ...
        expect(findLiveJournalRepairTarget('session-1', [unavailableMarker], 5, false)).toBeUndefined();
        for (const malformedData of [
            ...
            expect(findLiveJournalRepairTarget('session-1', [malformedMarker], 5, false)).toBeUndefined();
        }
    });
    it('requires the target user input and matching formal terminal', () => {
        ...
        expect(findLiveJournalRepairSuffix(replay.slice(1), 'prompt-1')).toBeUndefined();
        expect(findLiveJournalRepairSuffix(replay.slice(0, 2), 'prompt-1')).toBeUndefined();
        expect(findLiveJournalRepairSuffix(replay, 'prompt-2')).toBeUndefined();
    });
});
//# sourceMappingURL=live-journal-repair.test.js.map