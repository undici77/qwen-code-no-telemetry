...
import { DaemonClient, DaemonHttpError, DaemonSessionClient, createDaemonTranscriptStore, extractServerTimestamp, matchTurnEvent, normalizeDaemonEvent, } from '@qwen-code/sdk/daemon';
import { createDaemonSessionActions, getPromptSettledKey, normalizeWorkspaceIdentity, resolveSessionRestoreTimeouts, } from './actions.js';
import { eventPromptId, findLiveJournalRepairSuffix, findLiveJournalRepairTarget, } from './live-journal-repair.js';
import { detachDaemonClient, getStableClientId, persistStableClientId, } from './clientLifecycle.js';
...
import { getCurrentMode, getSessionDisplayName, getReplayTokenUsage, getTokenCountFromUsage, mapProviderStatus, mapSessionContextModels, mapSessionContextReasoning, mapSupportedCommands, mapWorkspaceSkills, updateConnectionFromDaemonEvent, } from './mappers.js';
import { selectDaemonActiveTodoList, selectDaemonPendingPermissions, selectDaemonStreamingState, } from './selectors.js';
import { clearPassiveAssistantDoneTimer, delay, getReconnectDelayMs, schedulePassiveAssistantDone, } from '../timing.js';
import { clearSidechannelFollowupSuggestion, parseSidechannelFollowupSuggestion, publishSidechannelFollowupSuggestion, } from '../followupSidechannel.js';
import { parseSidechannelMidTurnInjected, publishSidechannelMidTurnInjected, } from '../midTurnInjectedSidechannel.js';
import { isPendingPromptEvent, publishPendingPromptEvent, } from '../pendingPromptVersion.js';
import { MISSING_SESSION_HTTP_STATUSES, isMissingSessionHttpStatus, resolveConnectionErrorStatus, } from './status.js';
...
function crossSessionKey(sessionId, workspaceCwd, mode, historyPageSize, clientId) {
    const replayShape = mode === 'resume'
        ? 'resume:none'
...
function transitionState(target, phase, error) {
    return {
        phase,
...
function settleCrossSessionIntent(intent, error) {
...
function findFirstPersistedRecordId(session) {
...
function stageCrossSession(input) {
    ...
    const notices = [];
    const dismissNoticeIds = new Set();
    let noticeId = 0;
    const addStagedNotice = (notice) => {
        const stagedNotice = {
            ...notice,
            id: notice.id ?? `staged-daemon-notice-${++noticeId}`,
            createdAt: notice.createdAt ?? Date.now(),
        };
        const existingIndex = notices.findIndex((existing) => existing.id === stagedNotice.id);
        if (!dismissNoticeIds.delete(stagedNotice.id) && existingIndex >= 0)
            return stagedNotice;
        if (existingIndex >= 0)
            notices.splice(existingIndex, 1);
    ...
    let transcriptBatch = [];
    const repairTarget = findLiveJournalRepairTarget(session.sessionId, session.replaySnapshot.liveJournal, session.lastEventId, session.replayDegraded === true);
    let repairCheckpoint;
    const midTurnEvents = [];
    const pendingPromptEvents = [];
    let followupSuggestion;
    const observedSnapshotEventIds = new Set();
    const flush = () => {
    ...
    const enqueueTranscript = (events) => {
        for (const uiEvent of events) {
            transcriptBatch.push(uiEvent);
            if (transcriptBatch.length === STAGING_BATCH_SIZE)
                flush();
        }
    };
    const firstPersistedRecordId = findFirstPersistedRecordId(session);
    const replayWasTruncated = session.replaySnapshot.compactedReplay.some(hasFullTranscriptBeforeReplay) || session.replaySnapshot.liveJournal.some(hasFullTranscriptBeforeReplay);
    const historyHasMore = capabilities.features.includes(SESSION_TRANSCRIPT_PAGINATION_FEATURE) &&
    ...
    const consume = (event) => {
        try {
            const normalized = normalizeAndFilterEvent(event, session.clientId, replayOpts, updateConnection, { suppressLog: true });
            bumpWorkspaceEventSignals(normalized, updateSignals);
            let transcript = filterDaemonUiEventsForTranscript(event, normalized, addStagedNotice, (id) => dismissNoticeIds.add(id), { hideHistoryTruncation: historyHasMore, suppressLogs: true });
            ...
            if (event.type === 'turn_complete') {
                enqueueTranscript([
                    assistantDoneFromTurnEvent(event, event.data?.stopReason ??
                        'end_turn'),
                ]);
            }
            ...
            if (isPendingPromptEvent(event)) {
                ...
                if (pendingPromptEvents.length > 200)
                    pendingPromptEvents.shift();
            }
        }
        catch (error) {
            addStagedNotice({
    ...
    for (const event of session.replaySnapshot.compactedReplay)
        consume(event);
    for (const event of session.replaySnapshot.liveJournal) {
        if (event.id !== undefined)
            observedSnapshotEventIds.add(event.id);
    ...
    for (const event of input.additionalEvents ?? [])
        consume(event);
    flush();
    const replayTokenUsage = getReplayTokenUsage(session.replaySnapshot.liveJournal) ??
    ...
    if (repairCheckpoint)
        repairCheckpoint.maxBlocks = transcript.maxBlocks;
    const repair = repairTarget && repairCheckpoint
        ? {
            sessionId: session.sessionId,
            target: repairTarget,
...
function hasFullTranscriptBeforeReplay(event) {
    return (event.type === 'history_truncated' &&
        isRecord(event.data) &&
        event.data['fullTranscriptAvailable'] === true);
}
function isHistoricalReplayMarker(event) {
    return (hasFullTranscriptBeforeReplay(event) &&
        isRecord(event.data) &&
        event.data['scope'] === undefined);
}
function materializeTranscriptHistory(current, events, maxBlocks) {
    ...
    const freshEvents = displayedRecordIds.size === 0
        ? events
        : events.filter((event) => !event.sourceRecordIds?.some((recordId) => displayedRecordIds.has(recordId)));
...
function projectMainTranscriptEvents(events) {
    const projected = [];
    for (const event of events) {
        if ('parentToolCallId' in event &&
            event.parentToolCallId &&
            event.type !== 'assistant.usage') {
            continue;
        }
        projected.push(event.type === 'tool.update' ? projectSubagentToolUpdate(event) : event);
    }
    return projected;
}
export const projectMainTranscriptEventsForTesting = projectMainTranscriptEvents;
const DaemonStoreContext = createContext(undefined);
const DaemonConnectionContext = createContext(undefined);
const DaemonActionsContext = createContext(undefined);
const DaemonTranscriptHistoryContext = createContext(undefined);
const DaemonPromptStatusContext = createContext(undefined);
const DaemonSessionNoticesContext = createContext(undefined);
const DaemonWorkspaceEventSignalsContext = createContext(undefined);
const DaemonSessionOwnerGuardContext = createContext(undefined);
/**
...
export function DaemonSessionProvider(props) {
    const { baseUrl, token, workspaceCwd, sessionId, clientId, createSessionRequest, maxQueued = 1024, maxBlocks = DEFAULT_MAX_BLOCKS, historyPageSize, subagentTranscriptMode = 'full', suppressOwnUserEcho = true, includeRawEvent = false, autoConnect = true, autoReconnect = true, restartEventStreamOnPrompt = false, reconnectDelayMs = 1_000, maxReconnectDelayMs = 10_000, heartbeatIntervalMs = 30_000, heartbeatFailureThreshold = 3, loadWarnings, onSessionTransitionCommit, children, } = props;
    ...
    const sessionCapabilitiesRef = useRef(workspace?.capabilities);
    ...
    if (environmentRef.current.baseUrl !== resolvedBaseUrl ||
    ...
    const shouldDeferInitialSessionCreation = initialRestoreSessionId === undefined;
    ...
    const runnerControlRef = useRef(undefined);
    const preparedRunnerRef = useRef(undefined);
    const desiredTransitionRef = useRef(undefined);
    if (!sessionRef.current &&
        !desiredTransitionRef.current &&
        resolvedWorkspaceCwd) {
        activeWorkspaceCwdRef.current = resolvedWorkspaceCwd;
    }
    const rawTransitionRef = useRef(undefined);
    const pumpTransitionRef = useRef(() => undefined);
    const lifecycleRef = useRef(0);
    const sourceBoundOperationCountRef = useRef(0);
    const sessionConfigGenerationRef = useRef(new WeakMap());
    const controlledRetryPendingRef = useRef(false);
    const cancelTransitionRef = useRef(() => undefined);
    ...
    const [transcriptHistoryState, setTranscriptHistoryState] = useState(transcriptHistoryRef.current);
    const [controlledRetryNonce, setControlledRetryNonce] = useState(0);
    const eventStreamRef = useRef(undefined);
    const lastSessionIdRef = useRef(undefined);
    const activePromptsRef = useRef(new Map());
    const settledPromptsRef = useRef(new Map());
    const pendingSessionLoadRef = useRef(undefined);
    const pendingSessionLoadIdRef = useRef(0);
    const liveJournalRepairRef = useRef(undefined);
    const repairReloadRef = useRef(undefined);
    const tryLiveJournalRepairRef = useRef(undefined);
    const passiveAssistantDoneTimerRef = useRef(undefined);
    ...
    const skipNextCleanupDetachSessionRef = useRef(undefined);
    const settledRestoredActivePromptSessionsRef = useRef(new WeakSet());
    ...
    const [restoreSessionId, setRestoreSessionId] = useState(initialRestoreSessionId);
    const [restoreWorkspaceCwd, setRestoreWorkspaceCwd] = useState(undefined);
    ...
    const knownCapabilities = workspace?.capabilities ??
        sessionCapabilitiesRef.current ??
        connection.capabilities;
    const legacyClientIdDependency = knownCapabilities &&
    ...
    const setConnectionSynchronous = useCallback((update) => {
    ...
    useEffect(() => {
        if (!workspace?.capabilities)
            return;
        setConnection((current) => current.capabilities === workspace.capabilities
            ? current
    ...
    const addNotice = useCallback((input) => {
        ...
        setNotices((current) => current.some((existing) => existing.id === notice.id)
            ? current
    ...
    useEffect(() => {
        ...
        let disposed = false;
        const preservingTranscriptDuringLoad = restoreMode === 'load' &&
        ...
        const runTranscriptFlush = (force = false) => {
            ...
            try {
                ...
                console.error('[DaemonSessionProvider] batched transcript dispatch failed', { eventCount: batch.length, error });
            }
        };
        ...
        const enqueueTranscriptEvents = (events) => {
            if (events.length === 0)
                return;
            for (const event of events)
        ...
        const dispatchTranscriptNow = (events) => {
            flushTranscriptSync();
        ...
        const captureSourceEvent = (event) => {
            const capture = runnerControl.capture;
            if (!capture || capture.invalidReason)
                return;
            if (runnerControl.session !== capture.source ||
            ...
            if (event.id <= capture.lastCapturedEventId)
                return;
            ...
            if (capture.events.length >= maxQueued ||
        ...
        const markSourceEventProcessed = (event) => {
        ...
        const tryLiveJournalRepair = () => {
            ...
            void reload(controller.signal, { replaySource: 'memory' }).catch((error) => {
                if (liveJournalRepairRef.current !== repair)
                    return;
                addNotice({
        ...
        const run = async () => {
            ...
            let prepared = preparedRunnerRef.current?.session === sessionRef.current
            ...
            let shouldCreateFreshSession = !manualSessionClearRef.current &&
                !restoreSessionId &&
                newSessionNonce > 0;
            ...
            while (!disposed && !abort.signal.aborted) {
                ...
                try {
                    ...
                    if (!session) {
                        if (!preservingTranscriptDuringLoad) {
                            setConnection((current) => ({
                                ...
                                status: 'connecting',
                                error: undefined,
                                errorStatus: resolveConnectionErrorStatus(undefined, current.errorStatus),
                            }));
                        }
                        const getWorkspaceCapabilities = workspaceGetCapabilitiesRef.current;
                        const caps = workspaceCapabilitiesRef.current ??
                        ...
                        const historyPaginationSupported = Array.isArray(caps.features) &&
                        ...
                        const effectWorkspaceCwd = restoreWorkspaceCwd ??
                        ...
                        const capabilityFeatures = Array.isArray(caps.features)
                            ? caps.features
                            : [];
                        const canPreheatPrimaryWorkspace = effectWorkspaceCwd === caps.workspaceCwd &&
                            capabilityFeatures.includes(WORKSPACE_ACP_PREHEAT_FEATURE);
                        const canReadPrimaryAcpStatus = canPreheatPrimaryWorkspace &&
                            capabilityFeatures.includes(WORKSPACE_ACP_STATUS_FEATURE);
                        if ((shouldDeferInitialSessionCreation ||
                            ...
                            // creates a session.
                            const [providerResult, skillsResult, acpStatusResult, gitResult] = await Promise.allSettled([
                            ...
                            if (providerResult.status === 'rejected') {
                                console.warn('[DaemonSessionProvider] workspaceProviders failed in deferred connect:', providerResult.reason);
                            }
                            if (skillsResult.status === 'rejected') {
                                console.warn('[DaemonSessionProvider] workspaceSkills failed in deferred connect:', skillsResult.reason);
                            }
                            if (canReadPrimaryAcpStatus &&
                                acpStatusResult.status === 'rejected') {
                                console.warn('[DaemonSessionProvider] workspaceAcpStatus failed in deferred connect:', acpStatusResult.reason);
                            }
                            const providers = providerResult.status === 'fulfilled'
                                ? providerResult.value
                                : undefined;
                            const providerModelStatus = mapProviderStatus(providers);
                            const { commands: deferredSkillCommands, skills: deferredSkills, } = mapWorkspaceSkills(skillsResult.status === 'fulfilled'
                                ? skillsResult.value
                                : undefined);
                            const preserveClearedSessionCommands = skillsResult.status === 'rejected' ||
                                (manualSessionClearRef.current &&
                            ...
                            setConnection((current) => ({
                                ...
                                gitBranch: gitResult.status === 'fulfilled'
                            ...
                            if (canPreheatPrimaryWorkspace &&
                                ...
                                void (async () => {
                                    try {
                                        ...
                                        console.warn('[DaemonSessionProvider] ACP preheat for workspace skills failed:', error);
                                    }
                                    finally {
                        ...
                        const legacyClientRebind = targetSessionId !== undefined &&
                        ...
                        const restoreMethod = restoreSessionId &&
                            restoreMode === 'resume' &&
                            !legacyClientRebind
                        ...
                        const attemptedLoad = pendingSessionLoadRef.current?.sessionId === targetSessionId
                            ? pendingSessionLoadRef.current
                            : undefined;
                        const restoreRequestTimeoutMs = attemptedLoad?.requestTimeoutMs ??
                            resolveSessionRestoreTimeouts(capabilities).requestTimeoutMs;
                        const nextSession = restoreSessionId
                            ? await restoreMethod(client, restoreSessionId, {
                                workspaceCwd: effectWorkspaceCwd,
                        ...
                        if (disposed || abort.signal.aborted) {
                            ...
                            }).catch((err) => console.warn('[DaemonSessionProvider] detach failed:', err));
                            return;
                        }
                        ...
                        if (preservingTranscriptDuringLoad &&
                            ...
                            if (nextSession !== previousSession) {
                                await nextSession.detach().catch((error) => {
                                    console.warn('[DaemonSessionProvider] detach cancelled reload failed:', error);
                                });
                            }
                            if (pendingSessionLoadRef.current === attemptedLoad) {
                                ...
                                attemptedLoad.reject(new DOMException('Session load cancelled', 'AbortError'));
                            }
                        ...
                        if (attemptedLoad?.replaySource === 'memory') {
                            ...
                            const suffix = episode
                                ? findLiveJournalRepairSuffix(freshReplayEvents, episode.target.promptId)
                                : undefined;
                            if (!episode ||
                                ...
                                if (nextSession !== previousSession) {
                                    await nextSession.detach().catch((error) => {
                                        console.warn('[DaemonSessionProvider] detach rejected repair load failed:', error);
                                    });
                                }
                                if (pendingSessionLoadRef.current === attemptedLoad) {
                                    pendingSessionLoadRef.current = undefined;
                                    ...
                                    attemptedLoad.reject(new Error(nextSession.replayDegraded === true
                        ...
                        if (previousSessionId !== undefined &&
                            ...
                            const replaySnapshotEventCount = nextSession.replaySnapshot.compactedReplay.length +
                    ...
                    let restoredActivePrompt = activeSession.hasActivePrompt === true &&
                    ...
                    const hasSessionActivePrompt = () => restoredActivePrompt ||
                    ...
                    const pendingLoadToResolve = pendingLoad?.sessionId === activeSession.sessionId
                        ? pendingLoad
                        : undefined;
                    ...
                    const markerStillVisible = repairingEpisode?.markerBlockId !== undefined &&
                        store
                            .getSnapshot()
                            .blocks.some((block) => block.id === repairingEpisode?.markerBlockId);
                    ...
                    const firstPersistedRecordId = replayEvents
                        ...
                        .find((recordId) => recordId !== undefined) ??
                        replayEvents
                            .filter((e) => e.type === 'history_truncated')
                            .map(getPersistedReplayRecordId)
                            .find((recordId) => recordId !== undefined) ??
                        activeSession.historyAnchorRecordId;
                    const replayHistoryWasTruncated = replayEvents.some(hasFullTranscriptBeforeReplay);
                    ...
                    if (replayInjected) {
                        ...
                        const sourceEvents = repairingEpisode && repairSuffix && markerStillVisible
                            ? repairSuffix.events
                            : replayEvents;
                        const replayTarget = findLiveJournalRepairTarget(activeSession.sessionId, liveJournal, activeSession.lastEventId, activeSession.replayDegraded);
                        const markerIndex = replayTarget
                            ? sourceEvents.indexOf(replayTarget.marker)
                            : -1;
                        const eventGroups = [];
                        for (const replayEvent of sourceEvents) {
                            const isNewRepairEvent = repairingEpisode !== undefined &&
                                replayEvent.id !== undefined &&
                                !repairingEpisode.observedSnapshotEventIds.has(replayEvent.id) &&
                                !(replayEvent.id > repairingEpisode.snapshotLastEventId &&
                                    replayEvent.id <= repairingEpisode.lastObservedEventId);
                            try {
                                const replayUiEvents = normalizeAndFilterEvent(replayEvent, activeSession.clientId, replayOpts, setConnection, {
                                    updateConnection: repairingEpisode !== undefined && isNewRepairEvent,
                                    suppressLog: repairingEpisode !== undefined && !isNewRepairEvent,
                                });
                                const transcriptEvents = filterDaemonUiEventsForTranscript(replayEvent, replayUiEvents, addNotice, dismissNotice, {
                                    hideHistoryTruncation: historyHasMore,
                                    suppressSideEffects: repairingEpisode !== undefined && !isNewRepairEvent,
                                });
                                const projectedEvents = subagentTranscriptModeRef.current === 'summary'
                                    ? projectMainTranscriptEvents(transcriptEvents)
                                    : transcriptEvents;
                                const groupEvents = [...projectedEvents];
                                if (replayEvent.type === 'turn_complete') {
                                    const stopReason = replayEvent.data
                                        ?.stopReason ?? 'end_turn';
                                    groupEvents.push(assistantDoneFromTurnEvent(replayEvent, stopReason));
                                }
                                else if (replayEvent.type === 'turn_error') {
                                    groupEvents.push(assistantDoneFromTurnEvent(replayEvent, 'error'));
                                }
                                eventGroups.push({
                                    transcript: groupEvents,
                                    sideEffects: repairingEpisode === undefined
                                        ? projectedEvents
                                        : isNewRepairEvent
                                            ? replayUiEvents
                                            : [],
                                ...
                                if (repairingEpisode === undefined || isNewRepairEvent) {
                                    ...
                                    console.warn('[DaemonSessionProvider] skipped malformed replay event:', error);
                                }
                                eventGroups.push({ transcript: [], sideEffects: [] });
                            }
                        }
                        const allUiEvents = eventGroups.flatMap((group) => group.transcript);
                        let replayExceededCapacity = false;
                        const rebuildReplay = repairingEpisode !== undefined ||
                        ...
                        if (rebuildReplay) {
                            ...
                            const replayStore = createDaemonTranscriptStore(repairingEpisode && markerStillVisible
                                ? {
                            ...
                            if (replayTarget && nextCheckpoint) {
                                const markerBlock = store
                                    .getSnapshot()
                                    .blocks.find((block) => block.kind === 'status' &&
                                    ...
                                    existingRepair?.target.signature === replayTarget.signature &&
                                        ...
                                        : {
                                            ...
                                            ...(markerBlock
                                                ...
                                                : {}),
                                            observedSnapshotEventIds: new Set(liveJournal.flatMap((event) => event.id === undefined ? [] : [event.id])),
                        ...
                        const sideEffectEvents = eventGroups.flatMap((group) => group.sideEffects);
                        if (sideEffectEvents.length > 0) {
                            bumpWorkspaceEventSignals(sideEffectEvents, setWorkspaceEventSignals);
                        }
                        ...
                        for (const replayEvent of replayEvents) {
                            settleActivePromptFromTurnEvent(activePromptsRef.current, settledPromptsRef.current, activeSession.sessionId, replayEvent, store, setPromptStatus, passiveAssistantDoneTimerRef, { requireBoundPromptId: true });
                        }
                    ...
                    setConnection((current) => ({
                        ...
                        displayName: getSessionDisplayName(activeSession.state) ??
                            (current.sessionId === activeSession.sessionId
                                ? current.displayName
                                : undefined),
                        tokenUsage: replayTokenUsage !== undefined
                            ? replayTokenUsage
                            : current.sessionId === activeSession.sessionId
                                ? current.tokenUsage
                                : undefined,
                        tokenCount: replayTokenCount !== undefined
                            ? replayTokenCount
                            : current.sessionId === activeSession.sessionId
                    ...
                    const canReuseSessionMetadata = skipMetadataRefreshThisIteration ||
                    ...
                    const gitPromise = skipMetadataRefreshThisIteration
                        ...
                        : activeSession.workspaceCwd
                            ...
                            : client.workspaceGit();
                    const [providerResult, commandResult, contextResult, gitResult] = await Promise.allSettled([
                    ...
                    const providers = providerResult?.status === 'fulfilled'
                        ? providerResult.value
                        : undefined;
                    const supportedCommands = commandResult?.status === 'fulfilled'
                        ? commandResult.value
                        : undefined;
                    const context = contextResult?.status === 'fulfilled'
                        ? contextResult.value
                        : undefined;
                    const gitBranch = gitResult?.status === 'fulfilled'
                        ? (gitResult.value.branch ?? undefined)
                        : undefined;
                    ...
                    const sessionModels = contextModelStatus && contextModelStatus.models.length > 0
                        ? contextModelStatus.models
                        : providerModelStatus.models;
                    const sessionCurrentModel = contextModelStatus?.currentModel ??
                        providerModelStatus.currentModel;
                    const providerContextWindow = sessionCurrentModel === providerModelStatus.currentModel
                        ? providerModelStatus.contextWindow
                        : providerModelStatus.models.find((model) => model.id === sessionCurrentModel)?.contextWindow;
                    const sessionContextWindow = contextModelStatus?.contextWindow ??
                    ...
                    setConnection((current) => {
                        ...
                        const configSnapshotCurrent = configGeneration % 2 === 0 &&
                        ...
                        return {
                            ...
                            commands: supportedCommands !== undefined ? commands : current.commands,
                            ...
                            reasoning: configSnapshotCurrent && context !== undefined
                                ? mapSessionContextReasoning(context, current.reasoning?.effort)
                                : current.reasoning,
                            displayName: getSessionDisplayName(activeSession.state) ??
                                current.displayName,
                    ...
                    if (loadWarningTexts.length > 0) {
                        const existingWarningTexts = repairingEpisode
                            ? new Set(store
                                .getSnapshot()
                                .blocks.flatMap((block) => block.kind === 'status' ? [block.text] : []))
                            : undefined;
                    ...
                    const requestEpochResetReload = () => {
                        cancelTransitionRef.current('Session transition cancelled by state resync');
                        ...
                        const active = activePromptsRef.current.get(activeSession.sessionId);
                    ...
                    for await (const event of activeSession.events({
                        ...
                        if (currentRepair?.sessionId === activeSession.sessionId &&
                            event.id !== undefined) {
                            currentRepair.lastObservedEventId = Math.max(currentRepair.lastObservedEventId, event.id);
                        }
                        try {
                            try {
                                ...
                                const normalizedUiEvents = normalizeAndFilterEvent(event, activeSession.clientId, eventOptionsRef.current, (update) => {
                                ...
                                const uiEvents = filterDaemonUiEventsForTranscript(event, normalizedUiEvents, addNotice, dismissNotice);
                                const transcriptUiEvents = subagentTranscriptModeRef.current === 'summary'
                                ...
                                if (uiEvents.length > 0) {
                                    ...
                                    if (hasGenerationSignal)
                                        runnerActiveTurn = true;
                                    setPromptStatus((current) => current === 'waiting' ||
                                ...
                                const activePromptSettled = settleActivePromptFromTurnEvent(activePromptsRef.current, settledPromptsRef.current, activeSession.sessionId, event, store, setPromptStatus, passiveAssistantDoneTimerRef);
                                let restoredPromptSettled = false;
                                if (!activePromptSettled &&
                                    ...
                                    clearPassiveAssistantDoneTimer(passiveAssistantDoneTimerRef);
                                    const stopReason = event.type === 'turn_complete'
                                        ? (event.data
                                            ?.stopReason ?? 'end_turn')
                                        : 'error';
                                    dispatchTranscriptNow(assistantDoneFromTurnEvent(event, stopReason));
                                ...
                                const shouldGuardAssistant = !hasSessionActivePrompt() &&
                                ...
                                for (const uiEvent of uiEvents) {
                                    if (uiEvent.type === 'prompt.cancelled' &&
                                        (restoredActivePrompt ||
                                            uiEvent.originatorClientId !== activeSession.clientId)) {
                                        dispatchTranscriptNow(assistantDoneFromTurnEvent(event, 'cancelled'));
                                ...
                                const isObserver = !activePromptSettled &&
                                    !restoredPromptSettled &&
                                    !hasSessionActivePrompt();
                                if (isObserver) {
                                    const hasUserMsg = uiEvents.some((e) => e.type === 'user.text.delta');
                                    if (hasUserMsg) {
                                        runnerActiveTurn = true;
                                        setPromptStatus('waiting');
                                    }
                                    else if (hasActiveGenerationSignal(uiEvents)) {
                                        setPromptStatus((current) => current === 'idle' ? 'streaming' : current);
                                    }
                                }
                                if (isObserver && event.type === 'turn_complete') {
                                    clearPassiveAssistantDoneTimer(passiveAssistantDoneTimerRef);
                                    const stopReason = event.data
                                        ?.stopReason ?? 'end_turn';
                                    dispatchTranscriptNow(assistantDoneFromTurnEvent(event, stopReason));
                                    setPromptStatus('idle');
                                }
                                    ...
                                    clearPassiveAssistantDoneTimer(passiveAssistantDoneTimerRef);
                                    dispatchTranscriptNow(assistantDoneFromTurnEvent(event, 'error'));
                                    setPromptStatus('idle');
                                }
                                else if (isObserver && hasActiveGenerationSignal(uiEvents)) {
                                    schedulePassiveAssistantDone(store, passiveAssistantDoneTimerRef, 'passive_observer', 3000, () => setPromptStatus('idle'));
                                }
                                if (event.id !== undefined &&
                                    (event.type === 'turn_complete' ||
                                        event.type === 'turn_error' ||
                                        uiEvents.some((uiEvent) => uiEvent.type === 'prompt.cancelled'))) {
                                    lastPromptTerminalEventId = event.id;
                                    runnerActiveTurn = false;
                                    queueMicrotask(pumpTransitionRef.current);
                                }
                                const pendingRepair = liveJournalRepairRef.current;
                                if (pendingRepair?.sessionId === activeSession.sessionId &&
                                    (event.type === 'turn_complete' ||
                                        event.type === 'turn_error') &&
                                ...
                                if (event.type === 'state_resync_required') {
                                    ...
                                    if (reason !== 'epoch_reset') {
                                        ...
                                        // window.
                                        console.warn('[DaemonSessionProvider] ring eviction detected, reloading session (sessionId=%s)', activeSession.sessionId);
                                        resyncRequested = true;
                                        nextSseConnectReason = 'state_resync';
                                        session = undefined;
                                        sessionRef.current = undefined;
                                ...
                                console.warn('[DaemonSessionProvider] skipped malformed daemon event:', error);
                            }
                        }
                        finally {
                    ...
                    if (autoReconnect &&
                        ...
                        isClosingSessionLoadError(error, !capabilities?.features.includes(CLIENT_IDENTITY_FEATURE))) {
                        reconnectAttempt += 1;
                        const reconnectConfig = reconnectConfigRef.current;
                        await delay(getReconnectDelayMs(reconnectAttempt, reconnectConfig.reconnectDelayMs, reconnectConfig.maxReconnectDelayMs), abort.signal);
                    ...
                    if (pendingLoad &&
                        ...
                        if (skipNextCleanupDetachSessionRef.current?.sessionId ===
                    ...
                    if (isAuthFailure || isTerminal) {
                        ...
                        if (isAuthFailure) {
                            setConnection((current) => ({
                                ...
                                sessionId: undefined,
                                error: message,
                                errorStatus: resolveConnectionErrorStatus(errorStatus, current.errorStatus),
                                missingSession: false,
                                capabilities: capabilities ?? current.capabilities,
                                loadingTranscript: undefined,
                        ...
                        const missingLoadedSession = loadingRequestedSession &&
                            isMissingSessionHttpStatus(errorStatus);
                        console.warn('[DaemonSessionProvider] terminal session error (sessionId=%s, status=%d, message=%s)', failedSessionId, errorStatus, message);
                        setConnection((current) => ({
                            ...current,
                            status: 'disconnected',
                            sessionId: undefined,
                    ...
                    else if (preservingTranscriptDuringLoad &&
                        session === undefined &&
                        pendingLoad?.sessionId === restoreSessionId &&
                        ...
                        console.debug('[DaemonSessionProvider] retriable SSE error, preserving session for delta resume (sessionId=%s)', session?.sessionId);
                        if (eventStream) {
                ...
                reconnectAttempt += 1;
                const reconnectConfig = reconnectConfigRef.current;
                const delayMs = getReconnectDelayMs(reconnectAttempt, reconnectConfig.reconnectDelayMs, reconnectConfig.maxReconnectDelayMs);
                setConnection((current) => ({
                    ...current,
                    status: 'disconnected',
                    error: undefined,
        ...
        return () => {
            ...
            disposed = true;
            abort.abort();
            const ownsCurrentSession = session !== undefined && sessionRef.current === session;
            const ownsEmptyState = session === undefined && sessionRef.current === undefined;
            ...
            const releaseOwnedSession = (unmounting) => {
                const ownedSession = unmounting ? sessionRef.current : session;
                const stillOwnsSession = ownedSession !== undefined && sessionRef.current === ownedSession;
                const stillOwnsEmptyState = session === undefined && sessionRef.current === undefined;
                if (!stillOwnsSession && !stillOwnsEmptyState)
                    return;
                const keepSessionForNextEffect = !unmounting &&
                    stillOwnsSession &&
                ...
                if (pendingLoad &&
                    ...
                    pendingLoad.reject(new DOMException('Session load interrupted by cleanup', 'AbortError'));
                    pendingSessionLoadRef.current = undefined;
                }
                if (stillOwnsSession) {
    ...
    useEffect(() => {
        if (!heartbeatSupportedRef.current ||
        ...
        const timer = setInterval(() => {
            ...
            session
                ...
                if (heartbeatFailureState.consecutiveFailures >=
                    heartbeatFailureThreshold) {
                    setConnection((current) => current.sessionId === session.sessionId
                        ? {
                            ...current,
                            status: 'connected',
                            error: undefined,
                ...
                if (thisErrorStatus !== undefined) {
                    ...
                    heartbeatFailureState.lastHttpError = {
                        status: resolveConnectionErrorStatus(thisErrorStatus, lastStatus) ??
                            thisErrorStatus,
                ...
                const effectiveMessage = heartbeatFailureState.lastHttpError?.message ?? message;
                const authFailure = errorStatus !== undefined &&
                ...
                if (authFailure || missingSession) {
                    ...
                    if (missingSession) {
                        console.warn('[DaemonSessionProvider] heartbeat detected missing session (sessionId=%s, status=%d)', deadSessionId, errorStatus);
                    }
                    else {
                        console.warn('[DaemonSessionProvider] heartbeat auth failure (sessionId=%s, status=%d)', deadSessionId, errorStatus);
                    }
    ...
    const publishCrossSessionFailure = useCallback((target, error) => {
        const message = error instanceof Error ? error.message : String(error);
        const status = extractHttpStatus(error);
        const code = error instanceof DaemonHttpError &&
            isRecord(error.body) &&
    ...
    const retireAttachment = useCallback((session, intent) => {
    ...
    const cleanupTransitionArtifacts = useCallback((intent, options = {}) => {
        const candidate = intent.candidate;
        intent.candidate = undefined;
        if (candidate)
            retireAttachment(candidate, intent);
        if (options.preserveInFlightCapture)
            return;
    ...
    const exposeCrossSessionFailure = useCallback((intent, error, options) => {
        if (desiredTransitionRef.current !== intent)
            return;
        cleanupTransitionArtifacts(intent, options);
        desiredTransitionRef.current = undefined;
        if (mountedRef.current)
            publishCrossSessionFailure(intent, error);
        settleCrossSessionIntent(intent, error);
    }, [cleanupTransitionArtifacts, publishCrossSessionFailure]);
    const armTransitionDeadline = useCallback((intent, capabilities) => {
        if (intent.deadlineStarted)
            return;
        intent.deadlineStarted = true;
        const timeoutMs = resolveSessionRestoreTimeouts(capabilities).watchdogTimeoutMs;
        if (timeoutMs === undefined)
            return;
        intent.deadlineAt = Date.now() + timeoutMs;
        intent.timeout = setTimeout(() => {
            exposeCrossSessionFailure(intent, new Error('Session transition timed out'), {
                preserveInFlightCapture: rawTransitionRef.current === intent,
            });
        }, timeoutMs);
    }, [exposeCrossSessionFailure]);
    const commitCrossSession = useCallback((intent, staged) => {
        if (!mountedRef.current ||
        ...
        clearNotices();
        for (const notice of staged.notices)
            addNotice(notice);
        for (const id of staged.dismissNoticeIds)
            dismissNotice(id);
        setWorkspaceEventSignals((currentSignals) => ({
            memoryVersion: currentSignals.memoryVersion + staged.signals.memoryVersion,
            agentsVersion: currentSignals.agentsVersion + staged.signals.agentsVersion,
            toolsVersion: currentSignals.toolsVersion + staged.signals.toolsVersion,
            settingsVersion: currentSignals.settingsVersion + staged.signals.settingsVersion,
            mcpVersion: currentSignals.mcpVersion + staged.signals.mcpVersion,
            extensionsVersion: currentSignals.extensionsVersion + staged.signals.extensionsVersion,
            artifactsVersion: currentSignals.artifactsVersion + staged.signals.artifactsVersion,
        ...
        for (const event of staged.midTurnEvents) {
            ...
            if (injected)
                publishSidechannelMidTurnInjected(injected);
        }
        for (const event of staged.pendingPromptEvents) {
            publishPendingPromptEvent(event);
        }
        const active = activePromptsRef.current.get(sourceToRetire.sessionId);
        active?.controller.abort(new DOMException('Session switch interrupted prompt wait', 'AbortError'));
        activePromptsRef.current.delete(sourceToRetire.sessionId);
        settledPromptsRef.current.clear();
        hasCurrentSessionActivePromptRef.current = () => staged.session.hasActivePrompt === true;
        clearPassiveAssistantDoneTimer(passiveAssistantDoneTimerRef);
    ...
    const commitSameSession = useCallback((intent, candidate, capabilities, capture) => {
        const control = runnerControlRef.current;
        if (!control)
            return false;
        ...
        if (intent.mode === 'load') {
            ...
            const tail = capture.events.filter((event) => event.id !== undefined &&
                event.id > watermark &&
            ...
            if (staged.repair ||
                staged.notices.some((notice) => notice.code === 'daemon.replay_event_malformed')) {
                return false;
            }
        }
    ...
    const pumpCrossSessionTransition = useCallback(() => {
        ...
        if (!intent)
            return;
        if (intent.lifecycle !== lifecycleRef.current ||
            intent.environmentGeneration !== environmentRef.current.generation) {
            exposeCrossSessionFailure(intent, new DOMException('Session transition environment changed', 'AbortError'));
            return;
        }
        if (intent.deadlineAt !== undefined && Date.now() >= intent.deadlineAt) {
            exposeCrossSessionFailure(intent, new Error('Session transition timed out before restore started'), {
                preserveInFlightCapture: rawTransitionRef.current === intent,
            });
            return;
        }
        const capabilities = workspaceCapabilitiesRef.current ??
            sessionCapabilitiesRef.current ??
        ...
        if (intent.sameLogical && intent.candidate) {
            ...
            if (intent.capture?.invalidReason ||
                sessionRef.current !== intent.source ||
                intent.source.clientId !== intent.sourceClientId ||
                snapshot?.session !== intent.source ||
                snapshot.clientId !== intent.sourceClientId ||
                snapshot.eventEpoch !== candidate.eventEpoch) {
                exposeCrossSessionFailure(intent, new Error(intent.capture?.invalidReason ??
            ...
            if (!commitSameSession(intent, candidate, intent.candidateCapabilities ?? capabilities, intent.capture)) {
                exposeCrossSessionFailure(intent, new Error('Session refresh failed integrity validation'));
            }
            return;
        }
        if (rawTransitionRef.current)
            return;
        if (intent.sameLogical) {
            const snapshot = runnerControlRef.current?.snapshot();
            if (snapshot?.session !== intent.source ||
                snapshot.clientId !== intent.sourceClientId ||
                snapshot.eventEpoch === undefined ||
                snapshot.processedEventId === undefined ||
                !Number.isSafeInteger(snapshot.processedEventId) ||
                snapshot.processedEventId < 0) {
                exposeCrossSessionFailure(intent, new Error('Current attachment cursor is unavailable; session was preserved'));
                return;
            }
            if (!snapshot.ready ||
                snapshot.activeTurn ||
        ...
        if (!requestClientId) {
            exposeCrossSessionFailure(intent, new Error('Session restore client identity is unavailable'));
            return;
        }
        ...
        const client = workspaceClientRef.current ??
            new DaemonClient({ baseUrl: resolvedBaseUrl, token: resolvedToken });
        const remaining = intent.deadlineAt === undefined
            ? undefined
            : Math.max(1, intent.deadlineAt - Date.now());
        const requestBudget = resolveSessionRestoreTimeouts(capabilities).requestTimeoutMs;
        const timeoutMs = requestBudget === 0
            ? (remaining ?? 0)
            : remaining === undefined
                ? requestBudget
                : Math.min(requestBudget, remaining);
        const restore = intent.mode === 'resume'
            ? DaemonSessionClient.resume
            : DaemonSessionClient.load;
        let retryScheduled = false;
        void restore(client, intent.sessionId, {
            ...
            if (!candidate.clientId ||
                ...
                retireAttachment(candidate, intent);
                if (latest === intent) {
                    exposeCrossSessionFailure(latest, new Error('Session restore returned an invalid owner identity'));
                }
                return;
            }
            if (intent.sameLogical &&
                ...
                if (latest?.key === intent.key) {
                    exposeCrossSessionFailure(latest, new Error('Session refresh returned an incomplete snapshot'));
                }
                return;
            }
            if (intent.resultSuperseded === true ||
            ...
            if (!commitCrossSession(latest, staged)) {
                retireAttachment(candidate, intent);
                exposeCrossSessionFailure(latest, new DOMException('Session transition became stale before commit', 'AbortError'));
            }
        })
            .catch((error) => {
            const latest = desiredTransitionRef.current;
            if (autoReconnect &&
                ...
                const reconnectConfig = reconnectConfigRef.current;
                setTimeout(pumpTransitionRef.current, getReconnectDelayMs(intent.retryAttempt, reconnectConfig.reconnectDelayMs, reconnectConfig.maxReconnectDelayMs));
            }
    ...
    const cancelCrossSessionTransition = useCallback((reason) => {
        ...
        if (intent) {
            ...
            settleCrossSessionIntent(intent, new DOMException(reason, 'AbortError'));
        }
        setConnectionSynchronous((current) => {
            if (!current.sessionTransition)
                return current;
            const next = { ...current };
            delete next.sessionTransition;
            return next;
    ...
    const beginCrossSessionTransition = useCallback((request, startLegacy) => {
        const capabilities = workspaceCapabilitiesRef.current ??
            sessionCapabilitiesRef.current ??
            connectionRef.current.capabilities;
        const rejectPreflight = (error) => {
            publishCrossSessionFailure(request, error);
            return Promise.reject(error);
        };
        if (!capabilities) {
            return rejectPreflight(new Error('Daemon capabilities are unavailable; current session was preserved'));
        }
        if (sourceBoundOperationCountRef.current > 0) {
            return rejectPreflight(new DOMException('Another session operation is already in progress', 'InvalidStateError'));
        }
        if (!capabilities.features.includes(CLIENT_IDENTITY_FEATURE)) {
            return startLegacy();
        }
        if (!resolvedBaseUrl) {
            return rejectPreflight(new Error('Daemon endpoint is unavailable; current session was preserved'));
        }
        const source = sessionRef.current;
        if (!source?.clientId) {
            return rejectPreflight(new Error('The daemon advertises client identity but the current session has no clientId'));
        }
        if (pendingSessionLoadRef.current) {
            return rejectPreflight(new DOMException('Another session restore is already in progress', 'InvalidStateError'));
        }
        const pending = desiredTransitionRef.current;
        if (request.sameLogical && pending && !pending.sameLogical) {
            return Promise.reject(new DOMException('A session switch is still preparing', 'InvalidStateError'));
        }
        const effectiveHistoryPageSize = request.mode === 'load' &&
            historyPageSizeRef.current !== undefined &&
            capabilities.features.includes(SESSION_TRANSCRIPT_PAGINATION_FEATURE)
            ? historyPageSizeRef.current
            : undefined;
        const requestShape = crossSessionKey(request.sessionId, request.workspaceCwd, request.mode, effectiveHistoryPageSize, undefined);
        const raw = rawTransitionRef.current;
        const rawShape = raw
            ? crossSessionKey(raw.sessionId, raw.workspaceCwd, raw.mode, raw.effectiveHistoryPageSize, undefined)
            : undefined;
        ...
        const key = crossSessionKey(request.sessionId, request.workspaceCwd, request.mode, effectiveHistoryPageSize, targetClientId);
        if (raw && raw.key !== key)
            raw.resultSuperseded = true;
        ...
        if (current) {
            ...
            settleCrossSessionIntent(current, new DOMException('Session transition superseded by a newer request', 'AbortError'));
        }
        let resolve;
        let reject;
        ...
        if (request.signal) {
            ...
            intent.removeAbortListener = () => request.signal?.removeEventListener('abort', abort);
            if (request.signal.aborted) {
                abort();
                return promise;
            }
        }
        if (!request.sameLogical)
            armTransitionDeadline(intent, capabilities);
        setConnectionSynchronous((connectionState) => ({
            ...connectionState,
            sessionTransition: transitionState(request, adoptingRaw
                ? 'preparing'
    ...
    const actions = useMemo(() => createDaemonSessionActions({
        store,
        sessionRef,
        activePromptsRef,
        settledPromptsRef,
        ...
        getCreateSessionRequest: () => ({
            ...
            sessionScope: 'thread',
            workspaceCwd: activeWorkspaceCwdRef.current ?? sessionRef.current?.workspaceCwd,
        }),
        createDetachedSession: (workspaceCwd, overrides) => {
            const client = workspaceClientRef.current ??
                new DaemonClient({
                    baseUrl: resolvedBaseUrl,
                    token: resolvedToken,
            ...
            return DaemonSessionClient.createOrAttach(client, request, requestClientId);
        },
        ...
        isCrossSessionTransitionPending: () => desiredTransitionRef.current !== undefined,
        isDifferentLogicalTransitionPending: () => desiredTransitionRef.current?.sameLogical === false,
        isSourceBoundOperationInFlight: () => sourceBoundOperationCountRef.current > 0,
    ...
    const loadMoreTranscript = useCallback(async (options) => {
        ...
        try {
            const page = await activeSession.client.getSessionTranscriptPage(activeSession.sessionId, {
            ...
            if (page.partial || page.replayError) {
                ...
                throw new Error(page.replayError ??
            ...
            for (const replayEvent of page.events) {
                try {
                    const transcriptEvents = filterDaemonUiEventsForTranscript(replayEvent, normalizeAndFilterEvent(replayEvent, activeSession.clientId, replayOpts, setConnection, { updateConnection: false }), addNotice, dismissNotice);
                    uiEvents.push(...(subagentTranscriptModeRef.current === 'summary'
                        ? projectMainTranscriptEvents(transcriptEvents)
                        : transcriptEvents));
                }
                catch (error) {
                    ...
                    console.warn('[DaemonSessionProvider] skipped malformed history event:', error);
                }
            }
            const historyMaterialization = uiEvents.length > 0
                ? materializeTranscriptHistory(store.getSnapshot(), uiEvents, maxBlocks)
                : undefined;
            ...
            if (historyMaterialization) {
                store.reset(applyTranscriptHistory(store.getSnapshot(), historyMaterialization));
                const repair = liveJournalRepairRef.current;
                if (repair?.sessionId === activeSession.sessionId) {
                    repair.checkpoint = applyTranscriptHistory(repair.checkpoint, historyMaterialization);
                }
            }
            ...
            const retryable = !terminalFailure &&
    ...
    const transcriptHistoryValue = useMemo(() => {
        const active = connection.sessionId === transcriptHistoryRef.current.sessionId &&
    ...
    const lastHandledWorkspaceRef = useRef(undefined);
    const lastHandledClientIdRef = useRef(undefined);
    useEffect(() => {
        const targetWorkspaceCwd = resolvedWorkspaceCwd ?? connectionRef.current.workspaceCwd;
        ...
        lastHandledClientIdRef.current = clientId;
        if (sessionId === undefined && previousSessionId === undefined)
            return;
        const pending = desiredTransitionRef.current;
        if (pending &&
            ...
            cancelTransitionRef.current('Session transition cancelled by controlled target change');
        }
        const currentSessionId = connectionRef.current.sessionId;
        const currentSession = sessionRef.current;
        const sameLogicalTarget = sessionId === currentSessionId &&
            normalizeWorkspaceIdentity(targetWorkspaceCwd) ===
                normalizeWorkspaceIdentity(connectionRef.current.workspaceCwd);
        const clientIdChanged = sameLogicalTarget &&
            currentSession !== undefined &&
            clientId !== undefined &&
            currentSession.clientId !== clientId;
        const controlledCapabilities = workspaceCapabilitiesRef.current ??
            sessionCapabilitiesRef.current ??
            connectionRef.current.capabilities;
        const needsTransactionalClientRebind = clientIdChanged &&
        ...
        if (sameLogicalTarget && !needsTransactionalClientRebind) {
            if (connectionRef.current.sessionTransition?.phase === 'failed') {
                setConnectionSynchronous((current) => {
                    if (current.sessionTransition?.phase !== 'failed')
                        return current;
                    const next = { ...current };
                    delete next.sessionTransition;
                    return next;
        ...
        if (sessionId)
            controlledTransitionOriginRef.current = true;
        ...
        void request.catch((error) => {
            console.warn('[DaemonSessionProvider] controlled session transition failed:', error);
        });
    }, [
        actions,
        clientId,
    ...
    return (_jsx(DaemonStoreContext.Provider, { value: store, children: _jsx(DaemonConnectionContext.Provider, { value: connection, children: _jsx(DaemonPromptStatusContext.Provider, { value: promptStatus, children: _jsx(DaemonSessionNoticesContext.Provider, { value: noticesValue, children: _jsx(DaemonWorkspaceEventSignalsContext.Provider, { value: workspaceEventSignals, children: _jsx(DaemonActionsContext.Provider, { value: actions, children: _jsx(DaemonSessionOwnerGuardContext.Provider, { value: ownerGuardValue, children: _jsx(DaemonTranscriptHistoryContext.Provider, { value: transcriptHistoryValue, children: children }) }) }) }) }) }) }) }));
}
/**
...
function logSettingsReloadEvent(event) {
    if (event.type !== 'settings_reloaded')
        return;
    console.debug('[DaemonSessionProvider] settings reloaded:', getSettingsReloadLogData(event));
}
function getSettingsReloadLogData(event) {
    ...
    if (typeof childReloaded === 'boolean')
        log['childReloaded'] = childReloaded;
    if (sessionsRefreshed)
        log['sessionsRefreshed'] = sessionsRefreshed;
    if (sessionsSkipped)
        log['sessionsSkipped'] = sessionsSkipped;
    if (childError)
        log['childError'] = childError;
    return log;
}
...
function filterDaemonUiEventsForTranscript(sourceEvent, events, addNotice, dismissNotice, behavior = {}) {
    if (behavior.hideHistoryTruncation && isHistoricalReplayMarker(sourceEvent)) {
        return [];
    }
    if (!behavior.suppressSideEffects &&
    ...
    for (const event of events) {
        ...
        if (behavior.suppressSideEffects)
            continue;
        const notice = addNotice(daemonErrorEventToNotice(sourceEvent, event));
        if (!behavior.suppressLogs &&
...
export function useDaemonTranscriptStore() {
    const store = useContext(DaemonStoreContext);
    if (!store) {
        throw new Error('useDaemonTranscriptStore must be used within DaemonSessionProvider');
    }
    return store;
}
export function useDaemonTranscriptHistory() {
    const history = useContext(DaemonTranscriptHistoryContext);
    if (!history) {
        throw new Error('useDaemonTranscriptHistory must be used within DaemonSessionProvider');
    }
    return history;
}
export function useDaemonTranscriptState() {
    const store = useDaemonTranscriptStore();
    return useSyncExternalStore(store.subscribe, store.getSnapshot, store.getSnapshot);
}
export function useDaemonTranscriptBlocks() {
    const store = useDaemonTranscriptStore();
...
export function useDaemonStreamingState() {
    ...
    const getStreamingState = useCallback(() => selectDaemonStreamingState(store.getSnapshot().blocks, promptStatus), [promptStatus, store]);
    return useSyncExternalStore(store.subscribe, getStreamingState, getStreamingState);
}
...
export function useOptionalDaemonActions() {
    return useContext(DaemonActionsContext);
}
export function useDaemonSessionOwnerGuard() {
    const guard = useContext(DaemonSessionOwnerGuardContext);
    if (!guard) {
        throw new Error('useDaemonSessionOwnerGuard must be used within DaemonSessionProvider');
    }
    return guard;
}
...
export function useDaemonPromptStatus() {
    const promptStatus = useContext(DaemonPromptStatusContext);
    if (!promptStatus) {
        throw new Error('useDaemonPromptStatus must be used within DaemonSessionProvider');
    }
    return promptStatus;
}
export function useDaemonConnection() {
    ...
    if (!connection) {
        throw new Error('useDaemonConnection must be used within DaemonSessionProvider');
    }
    return connection;
}
export function useDaemonSessionNotices() {
    ...
    if (!value) {
        throw new Error('useDaemonSessionNotices must be used within DaemonSessionProvider');
    }
    return value;
}
function hasActiveGenerationSignal(events) {
...
function bumpWorkspaceEventSignals(events, setSignals) {
    let memory = 0;
    let agents = 0;
    let tools = 0;
    let settings = 0;
...
function isClosingSessionLoadError(error, allowLegacyMessage = false) {
    ...
    return (body?.['code'] === 'session_closing' ||
        (allowLegacyMessage &&
            typeof body?.['error'] === 'string' &&
            body['error'].endsWith('The session is closing; retry after close completes')));
}
//# sourceMappingURL=DaemonSessionProvider.js.map