...
import { extractDaemonTodosFromToolBlock, isDaemonSubAgentToolBlock, selectDaemonActiveTodoList, selectDaemonLatestTodoList, selectDaemonPendingPermissions, selectDaemonSubAgentToolBlocks, selectDaemonStreamingState, selectDaemonTodoLists, selectDaemonTranscriptStreamingState, } from './selectors.js';
describe('daemon selectors', () => {
    ...
    it('ignores earlier active todo lists when the latest list is complete', () => {
        ...
        expect(selectDaemonActiveTodoList([earlierActive, latestCompleted])).toBeUndefined();
    });
    ...
    it('derives transcript streaming state from active text and tool blocks', () => {
        expect(selectDaemonTranscriptStreamingState([
    ...
    it('falls back to prompt status when transcript is idle', () => {
        ...
        expect(selectDaemonStreamingState([block({ kind: 'tool', status: 'running' })])).toBe('responding');
        expect(selectDaemonStreamingState([block({ kind: 'tool', status: 'running' })], 'idle')).toBe('idle');
        expect(selectDaemonStreamingState([block({ kind: 'thought', text: 'thinking', streaming: true })], 'waiting')).toBe('thinking');
    });
});
