...
import { detachDaemonClient, getPersistedClientId, getStableClientId, persistStableClientId, } from './clientLifecycle.js';
describe('getStableClientId', () => {
    ...
    it('generates a client ID without session storage compatibility fallback', () => {
        ...
        expect(window.sessionStorage.getItem('qwen-code-webui-client-id')).toBeNull();
    });
...
describe('detachDaemonClient', () => {
    ...
    it('sends POST with keepalive: true', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('http://localhost:3000/session/sess-1/detach', expect.objectContaining({
            method: 'POST',
            keepalive: true,
    ...
    it('strips trailing slashes from baseUrl', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('http://localhost:3000/session/sess-1/detach', expect.anything());
    });
    it('throws on non-204/non-404 response', async () => {
        fetch.mockResolvedValue({ status: 500 });
        await expect(detachDaemonClient({
            baseUrl: 'http://localhost:3000',
            sessionId: 'sess-1',
            clientId: 'client-1',
...
//# sourceMappingURL=clientLifecycle.test.js.map