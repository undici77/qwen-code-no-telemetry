 ...
 */
export function mapProviderStatus(status, preferredCurrentModel) {
    if (!status)
        return { models: [] };
    const seen = new Set();
    const models = [];
...
export function mapSessionContextModels(status) {
    const modelState = getRecord(status?.state?.models);
    if (!modelState)
        return undefined;
    const currentModel = getString(modelState, 'currentModelId') ??
    ...
    if (Array.isArray(availableModels)) {
        for (const rawModel of availableModels) {
            ...
            const modelContextWindow = getNumber(meta, 'contextLimit') ??
            ...
            models.push({
                id: modelId,
                baseModelId: getString(model, 'baseModelId') ?? stripAcpAuthSuffix(modelId),
...
export function mapReasoningControls(configOptions, fallbackEffort) {
    ...
    if (!option || !Array.isArray(rawOptions))
        return undefined;
    ...
    const effort = [currentValue, fallbackEffort, defaultEffort].find((value) => typeof value === 'string' && efforts.includes(value)) ?? efforts[0];
    return { enabled: currentValue !== 'none', effort, efforts };
}
export function mapSessionContextReasoning(status, fallbackEffort) {
    return mapReasoningControls(status?.state?.configOptions, fallbackEffort);
}
export function mapSupportedCommands(status) {
    if (!status)
 ...
 */
export function mapWorkspaceSkills(status) {
    if (!status)
        return { commands: [], skills: [] };
    const availableSkills = status.skills.filter((skill) => skill.status === 'ok');
...
export function updateConnectionFromDaemonEvent(event, setConnection) {
    ...
    switch (event.type) {
        case 'git_branch_changed': {
            ...
            setConnection((current) => workspaceCwd && workspaceCwd !== current.workspaceCwd
                ? current
        ...
        case 'model_switched': {
            ...
            if (modelId) {
                setConnection((current) => ({
                    ...
                    reasoning: current.currentModel === modelId ? current.reasoning : undefined,
                }));
            }
            break;
        }
 ...
 * count available.
 */
export function getReplayTokenCount(events) {
...
export function getTokenCountFromUsage(usage) {
    ...
    if (!usage)
        return undefined;
    const total = Object.values(usage).reduce((sum, value) => sum + (typeof value === 'number' ? value : 0), 0);
    return total > 0 ? total : undefined;
}
export function getReplayTokenUsage(events) {
...
function mapAvailableCommandsUpdate(update) {
    ...
    const skills = Array.isArray(update['availableSkills'])
        ? update['availableSkills'].filter((skill) => typeof skill === 'string')
        : [];
