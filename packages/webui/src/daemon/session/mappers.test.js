...
import { getReplayTokenCount, getReplayTokenUsage, mapWorkspaceSkills, updateConnectionFromDaemonEvent, } from './mappers.js';
function availableCommandsEvent(availableCommands, availableSkills) {
    return {
        id: 1,
        v: 1,
        type: 'session_update',
...
function usageEvent(id, usage, text = '') {
    return {
        id,
        v: 1,
        type: 'session_update',
...
describe('getReplayTokenCount', () => {
    ...
    it('prefers inputTokens over totalTokens and falls back to totalTokens', () => {
        ...
        expect(getReplayTokenCount([usageEvent(1, { totalTokens: 7_500 })])).toBe(7_500);
    });
    ...
    it('skips events with non-record payloads and keeps scanning', () => {
        ...
        expect(getReplayTokenCount([usageEvent(1, { inputTokens: 500 }), nullData])).toBe(500);
    });
    it('skips events whose payload getter throws and keeps scanning', () => {
        ...
        expect(getReplayTokenCount([usageEvent(1, { inputTokens: 500 }), throwing])).toBe(500);
    });
});
...
describe('updateConnectionFromDaemonEvent', () => {
    it('updates and clears the current git branch', () => {
        const changed = applyEvent({ status: 'connected', workspaceCwd: '/workspace', gitBranch: 'main' }, {
            v: 1,
    ...
    it('stores the enriched git status pushed for the current workspace', () => {
        const next = applyEvent({ status: 'connected', workspaceCwd: '/workspace' }, {
            v: 1,
    ...
    it('replaces commands and skills from an available_commands_update', () => {
        const next = applyEvent({ status: 'connected', workspaceCwd: '/workspace' }, availableCommandsEvent([{ name: 'review', description: 'Review a PR', input: null }], ['review']));
