...
import { createDaemonSessionActions, getConnectionAfterSessionClear, resolveSessionRestoreTimeouts, } from './actions';
 ...
 */
function advertisingRestoreBudget(sessionRestoreTimeoutMs) {
    return {
        v: 1,
        mode: 'http-bridge',
        features: [],
...
describe('resolveSessionRestoreTimeouts', () => {
    ...
    it('derives both client budgets from the advertised server timeout', () => {
        expect(resolveSessionRestoreTimeouts(advertisingRestoreBudget(90_000))).toEqual({
            requestTimeoutMs: 100_000,
            watchdogTimeoutMs: 105_000,
        });
    });
    it('disables derived timers that exceed the JavaScript timer ceiling', () => {
        expect(resolveSessionRestoreTimeouts(advertisingRestoreBudget(2_147_483_647))).toEqual({ requestTimeoutMs: 0, watchdogTimeoutMs: undefined });
    });
});
describe('createDaemonSessionActions', () => {
    it('rejects a concurrent source-bound branch request', async () => {
        ...
        const setSourceBoundOperationInFlight = vi.fn((inFlight) => {
    ...
    it('does not open a branch that resolves after its source is cleared', async () => {
        ...
        expect(source.client.detachSession).toHaveBeenCalledWith('session-b', 'client-b');
    });
    ...
    it('blocks a restore while active-session creation is in flight', async () => {
        ...
        existingSession.client.createOrAttachSession.mockReturnValueOnce(created.promise);
        let sourceBoundOperationCount = 0;
        const setSourceBoundOperationInFlight = vi.fn((inFlight) => {
            sourceBoundOperationCount += inFlight ? 1 : -1;
        });
        const beginCrossSessionTransition = vi.fn(async () => {
            if (sourceBoundOperationCount > 0) {
                throw new DOMException('Another session operation is already in progress', 'InvalidStateError');
            }
        });
        const { actions } = createActionsHarness({
            beginCrossSessionTransition,
    ...
    it('keeps restore blocked after create times out until the raw request settles', async () => {
        ...
        try {
            ...
            created.resolve(nextSession);
            await Promise.resolve();
            expect(existingSession.client.detachSession).toHaveBeenCalledWith(nextSession.sessionId, nextSession.clientId);
    ...
    it('consumes a controlled origin when a source-bound operation blocks restore', async () => {
        ...
        const beginCrossSessionTransition = vi.fn(async (_request) => {
            if (sourceBound) {
                throw new DOMException('Another session operation is already in progress', 'InvalidStateError');
            }
        });
        ...
        expect(beginCrossSessionTransition).toHaveBeenCalledWith(expect.objectContaining({ origin: 'action' }), expect.any(Function));
        expect(beginCrossSessionTransition.mock.calls.map(([request]) => request.origin)).toEqual(['controlled', 'action']);
    });
    ...
    it('merges options.workspaceCwd into the active session request', async () => {
        ...
        await actions.createSession({ workspaceCwd: '/ws/secondary' });
        expect(existingSession.client.createOrAttachSession).toHaveBeenCalledWith(expect.objectContaining({ workspaceCwd: '/ws/secondary' }));
    });
    it('folds options.approvalMode into the active session request', async () => {
        ...
        await actions.createSession({ approvalMode: 'yolo' });
        expect(existingSession.client.createOrAttachSession).toHaveBeenCalledWith(expect.objectContaining({ approvalMode: 'yolo' }));
    });
    it('folds options.sourceType into the active session request', async () => {
        ...
        await actions.createSession({ sourceType: 'default' });
        expect(existingSession.client.createOrAttachSession).toHaveBeenCalledWith(expect.objectContaining({ sourceType: 'default' }));
    });
    ...
    it('clears the active session while a session switch is loading', async () => {
        const existingSession = createMockSession('session-a');
        const { actions, getConnection, pendingSessionLoadRef, sessionRef } = createActionsHarness({
            connection: { status: 'connected', sessionId: 'session-a' },
            session: existingSession,
        });
    ...
    it('detaches the old same-session attachment after its replacement loads', async () => {
        const existingSession = createMockSession('session-a');
        const { actions, getConnection, pendingSessionLoadRef, sessionRef, store } = createActionsHarness({
            connection: { status: 'connected', sessionId: 'session-a' },
            session: existingSession,
        });
    ...
    it('does not start a session reload with an aborted signal', async () => {
        const existingSession = createMockSession('session-a');
        const { actions, pendingSessionLoadRef, sessionRef, store } = createActionsHarness({
            connection: { status: 'connected', sessionId: 'session-a' },
            session: existingSession,
        });
        const controller = new AbortController();
        controller.abort();
        await expect(actions.reloadSession(controller.signal)).rejects.toMatchObject({ name: 'AbortError' });
    ...
    it('keeps the reload abort signal with the pending load', () => {
        ...
        pendingSessionLoadRef.current?.reject(new DOMException('Test cleanup', 'AbortError'));
        pendingSessionLoadRef.current = undefined;
    });
    ...
    it('does not restart the current session while a target switch is preparing', async () => {
        ...
        await expect(actions.reloadSession(new AbortController().signal, {
    ...
    it('consumes the controlled origin when a switch uses the legacy path', () => {
        const getTransitionOrigin = vi.fn(() => 'controlled');
        const { actions, pendingSessionLoadRef } = createActionsHarness({
            getTransitionOrigin,
        });
        void actions.loadSession('session-b').catch(() => undefined);
    ...
    it('reports attach timeouts as attach session failures', async () => {
        ...
        try {
            ...
            expect(addNotice).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('rejects attachSession when no session exists', async () => {
        const { actions } = createActionsHarness();
        await expect(actions.attachSession()).rejects.toThrow('Daemon session is not connected');
    });
    ...
    it('suppresses notices for silent transient getTasks failures', async () => {
        ...
        session.tasks.mockRejectedValueOnce(new Error('Failed to fetch'));
        const { actions } = createActionsHarness({ addNotice, session });
        await expect(actions.getTasks({ silent: true })).rejects.toThrow('Failed to fetch');
        expect(addNotice).not.toHaveBeenCalled();
    });
    it.each(['Request timed out', 'Network error', 'NetworkError'])('suppresses notices for silent %s getTasks failures', async (message) => {
        const session = createMockSession('session-a');
        const addNotice = vi.fn((notice) => notice);
    ...
    it.each([500, 408, 429])('suppresses notices for silent retryable HTTP %i getTasks failures', async (status) => {
        const session = createMockSession('session-a');
        const addNotice = vi.fn((notice) => notice);
        session.tasks.mockRejectedValueOnce(new DaemonHttpError(status, undefined, 'Retryable failure'));
        const { actions } = createActionsHarness({ addNotice, session });
        await expect(actions.getTasks({ silent: true })).rejects.toBeInstanceOf(DaemonHttpError);
        expect(addNotice).not.toHaveBeenCalled();
    });
    it('suppresses notices for silent abort getTasks failures', async () => {
        const session = createMockSession('session-a');
        const addNotice = vi.fn((notice) => notice);
        session.tasks.mockRejectedValueOnce(new DOMException('Aborted', 'AbortError'));
        const { actions } = createActionsHarness({ addNotice, session });
    ...
    it('reports silent hard HTTP getTasks failures once', async () => {
        const session = createMockSession('session-a');
        const addNotice = vi.fn((notice) => notice);
        session.tasks.mockRejectedValueOnce(new DaemonHttpError(403, undefined, 'Forbidden'));
        const { actions } = createActionsHarness({ addNotice, session });
        await expect(actions.getTasks({ silent: true })).rejects.toBeInstanceOf(DaemonHttpError);
    ...
    it('reports silent hard getTasks failures once', async () => {
        ...
        const { actions } = createActionsHarness({ addNotice, session });
        await expect(actions.getTasks({ silent: true })).rejects.toThrow('Malformed response');
        await expect(actions.getTasks({ silent: true })).rejects.toThrow('Malformed response');
        expect(addNotice).toHaveBeenCalledOnce();
        expect(addNotice).toHaveBeenCalledWith(expect.objectContaining({
            code: 'daemon.load_tasks.failed',
    ...
    it('rejects getTasks silently when no session exists', async () => {
        ...
        await expect(actions.getTasks()).rejects.toThrow('Daemon session is not connected');
        expect(addNotice).not.toHaveBeenCalled();
    });
    it('aborts active prompts and rejects pending session loads when clearing', async () => {
        ...
        const { actions } = createActionsHarness({
            activePrompts: new Map([['session-a', { controller }]]),
    ...
    it('starts admission only after local prompt guards pass', async () => {
        ...
        await expect(actions.sendPrompt('hello', { onAdmissionStarted })).rejects.toThrow('A prompt is already in progress');
    ...
    it('preserves ambiguous stable-id admission failures for reconciliation', async () => {
        ...
        await expect(actions.enqueueMidTurnMessage('follow up', { messageId: 'stable-id' })).rejects.toThrow('response lost');
    ...
    it('settles a prompt after a same-logical attachment replacement', async () => {
        ...
        expect(store.dispatch).toHaveBeenCalledWith(expect.objectContaining({ type: 'assistant.done' }));
    });
    it('keeps a replacement active prompt streaming when the old attachment aborts', async () => {
        ...
        admitted.reject(new DOMException('source retired', 'AbortError'));
        await expect(pending).resolves.toEqual({ stopReason: 'cancelled' });
        expect(store.dispatch).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'assistant.done' }));
    ...
    it('reports a shell failure after a same-logical attachment replacement', async () => {
        ...
        expect(addNotice).toHaveBeenCalledWith(expect.objectContaining({ operation: 'send_shell_command' }));
        expect(setPromptStatus).toHaveBeenLastCalledWith('idle');
    });
    it('does not apply a late model update to a replacement attachment', async () => {
        ...
        const { actions, getConnection, replaceConnection, sessionRef } = createActionsHarness({
            connection: {
                status: 'connected',
    ...
    it('keeps refresh blocked after model context times out until it settles', async () => {
        ...
        try {
            ...
            await expect(actions.loadSession(session.sessionId)).rejects.toMatchObject({ name: 'InvalidStateError' });
            context.resolve(contextStatus(session.sessionId));
            await vi.waitFor(() => expect(sourceBoundOperationCount).toBe(0));
            await expect(actions.loadSession(session.sessionId)).resolves.toBeUndefined();
            expect(setSourceBoundOperationInFlight.mock.calls).toEqual([
                [true],
                [false],
            ]);
    ...
    it('does not apply context captured before a reasoning update', async () => {
        ...
        const sessionConfigGeneration = new WeakMap();
        const { actions, getConnection } = createActionsHarness({
            connection: {
                status: 'connected',
                sessionId: session.sessionId,
        ...
        await expect(staleRead).resolves.toEqual(reasoningContext(session.sessionId, 'xhigh'));
        expect(getConnection()).toMatchObject({
    ...
    it('does not apply late commands to a replacement attachment', async () => {
        ...
        const { actions, getConnection, replaceConnection, sessionRef } = createActionsHarness({ session: source });
...
function createActionsHarness(opts = {}) {
    let connection = opts.connection ?? {
        status: 'connected',
        workspaceCwd: '/workspace',
    };
    const replaceConnection = (next) => {
        connection = next;
    };
    const sessionRef = {
    ...
    const activePromptsRef = {
        current: opts.activePrompts ?? new Map(),
    };
    const pendingSessionLoadRef = opts.pendingSessionLoadRef ??
        { current: undefined };
    const store = {
        reset: vi.fn(),
    ...
    const actions = createDaemonSessionActions({
        ...
        createDetachedSession: (opts.createDetachedSession ??
            vi.fn(async () => createMockSession('detached-session'))),
        ...
        isDifferentLogicalTransitionPending: opts.isDifferentLogicalTransitionPending,
...
function createMockSession(sessionId, clientId = `client-${sessionId}`) {
    return {
        sessionId,
        workspaceCwd: '/workspace',
        clientId,
        ...
        tasks: vi.fn(async () => ({ v: 1, sessionId, tasks: [] })),
    };
}
...
function supportedCommandsStatus(sessionId, ...names) {
    return {
        v: 1,
        sessionId,
...
function contextStatus(sessionId) {
    return {
        v: 1,
        sessionId,
        workspaceCwd: '/workspace',
...
function reasoningContext(sessionId, effort) {
    return {
        ...contextStatus(sessionId),
        state: { configOptions: reasoningConfigOptions(effort) },
    };
}
function reasoningConfigOptions(effort) {
    return [
        {
            id: 'reasoning_effort',
            currentValue: effort,
