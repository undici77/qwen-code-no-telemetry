 ...
 */
import { asKnownDaemonEvent } from '@qwen-code/sdk/daemon';
import { isRecord } from './httpErrors.js';
export function findLiveJournalRepairTarget(sessionId, liveJournal, lastEventId, replayDegraded) {
    if (replayDegraded)
        return undefined;
...
export function findLiveJournalRepairSuffix(replayEvents, promptId) {
    const start = replayEvents.findIndex((event) => eventPromptId(event) === promptId && isUserMessageChunk(event));
    if (start < 0)
        return undefined;
    const terminal = replayEvents.find((event, index) => index >= start &&
...
function isRecoverableLiveJournalMarker(event) {
    ...
    return (knownEvent?.type === 'history_truncated' &&
...
//# sourceMappingURL=live-journal-repair.js.map