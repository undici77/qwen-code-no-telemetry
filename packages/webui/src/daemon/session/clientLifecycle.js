...
export function getStableClientId(clientId, sessionId) {
    if (clientId)
        return clientId;
    ...
    try {
        if (sessionId) {
            const existingSessionClientId = window.sessionStorage.getItem(sessionClientIdKey(sessionId));
            if (existingSessionClientId)
                return existingSessionClientId;
        }
        return createWebuiClientId();
    }
    catch {
 ...
 */
export function getPersistedClientId(sessionId) {
    if (typeof window === 'undefined')
        return undefined;
    try {
        return (window.sessionStorage.getItem(sessionClientIdKey(sessionId)) ?? undefined);
    }
    catch {
        return undefined;
    }
}
export function persistStableClientId(clientId, sessionId) {
    if (!clientId || !sessionId || typeof window === 'undefined')
        return;
    try {
...
export async function detachDaemonClient(opts) {
    ...
    const url = `${stripTrailingSlashes(opts.baseUrl)}/session/${encodeURIComponent(opts.sessionId)}/detach`;
...
//# sourceMappingURL=clientLifecycle.js.map