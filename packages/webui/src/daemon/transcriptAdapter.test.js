...
describe('daemonTranscriptToUnifiedMessages', () => {
    ...
    it('truncates deeply nested daemon values instead of returning raw subtrees', () => {
        ...
        expect(JSON.stringify(messages[0]?.toolCall?.rawOutput)).toContain('[truncated]');
...
function createToolBlock(toolCallId, status) {
    return {
        id: toolCallId,
        kind: 'tool',
        toolCallId,
...
function createPermissionBlock(requestId, resolved) {
    return {
        id: requestId,
        kind: 'permission',
        requestId,
...
describe('previewMarkdown preserves rawOutput (wenshao R3 qwen3.7-max)', () => {
    it('keeps rawOutput as original object and exposes previewMarkdown when enrich enabled', () => {
        ...
        expect(tc['previewMarkdown'].includes('/x.ts')).toBe(true);
    });
});
//# sourceMappingURL=transcriptAdapter.test.js.map