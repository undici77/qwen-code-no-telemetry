...
function pendingEvent(type, promptId = 'p-1', state) {
    return {
        v: 1,
        type,
        data: {
...
describe('pending prompt sidechannel', () => {
    it('bumps queue version for pending-prompt queue events', async () => {
        ...
        sidechannel.publishPendingPromptEvent(pendingEvent('pending_prompt_started'));
        sidechannel.publishPendingPromptEvent(pendingEvent('pending_prompt_completed', 'p-1', 'completed'));
        expect(listener).toHaveBeenCalledTimes(3);
        expect(sidechannel.getPendingPromptVersion()).toBe(3);
    });
    it.each(['turn_complete', 'turn_error'])('notifies event subscribers without bumping version for %s events', async (type) => {
        const sidechannel = await loadModule();
    ...
    it('stops notifying unsubscribed listeners', async () => {
        ...
        const versionListener = vi.fn();
        const eventListener = vi.fn();
        const unsubscribeVersion = sidechannel.subscribePendingPromptVersion(versionListener);
        const unsubscribeEvents = sidechannel.subscribePendingPromptEvents(eventListener);
        unsubscribeVersion();
        unsubscribeEvents();
        sidechannel.publishPendingPromptEvent({
            v: 1,
    ...
    it('rejects malformed pending prompt events', async () => {
        const sidechannel = await loadModule();
        expect(sidechannel.isPendingPromptEvent({ type: 'pending_prompt_added' })).toBe(false);
        expect(sidechannel.isPendingPromptEvent({
            type: 'pending_prompt_added',
            data: { sessionId: 's-1' },
        })).toBe(false);
        expect(sidechannel.isPendingPromptEvent({
    ...
    it('caps the event buffer and evicts oldest events', async () => {
        const sidechannel = await loadModule();
        for (let i = 0; i < 205; i++) {
            sidechannel.publishPendingPromptEvent(pendingEvent('pending_prompt_added', `p-${i}`));
        }
...
//# sourceMappingURL=pendingPromptVersion.test.js.map