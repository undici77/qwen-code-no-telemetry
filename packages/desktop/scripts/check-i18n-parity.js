...
const LOCALES_DIR = resolve(import.meta.dir ?? new URL('.', import.meta.url).pathname, '..', 'packages', 'shared', 'src', 'i18n', 'locales');
const PLURAL_SUFFIX = /_(?:zero|one|two|few|many|other)$/;
const pluralBase = (key) => key.replace(PLURAL_SUFFIX, '');
const isPluralKey = (key) => PLURAL_SUFFIX.test(key);
const loadLocale = (path) => JSON.parse(readFileSync(path, 'utf-8'));
...
for (const file of localeFiles.sort()) {
    ...
    if (missing.length) {
        errors.push(`${lang}.json: ${missing.length} keys missing (e.g. ${missing.slice(0, 3).join(', ')})`);
    }
    if (extra.length) {
        errors.push(`${lang}.json: ${extra.length} extra keys (e.g. ${extra.slice(0, 3).join(', ')})`);
    }
}
if (errors.length) {
...
//# sourceMappingURL=check-i18n-parity.js.map