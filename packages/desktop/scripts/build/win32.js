 ...
 */
async function safeRmDir(dir, maxRetries = 5) {
    if (!existsSync(dir))
        return;
    let lastError = null;
 ...
 */
function buildMainProcess(config) {
 ...
 */
export async function buildElectronAppWindows(config) {
    ...
    run('node ./node_modules/esbuild/bin/esbuild apps/electron/src/preload/bootstrap.ts --bundle --platform=node --format=cjs --outfile=apps/electron/dist/bootstrap-preload.cjs --external:electron', rootDir);
 ...
 */
export async function packageWindows(config) {
