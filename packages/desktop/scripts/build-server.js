...
import { existsSync, mkdirSync, rmSync, copyFileSync, cpSync, lstatSync, readdirSync, readFileSync, writeFileSync, chmodSync, symlinkSync, } from 'fs';
import { $ } from 'bun';
import { BUN_VERSION, UV_VERSION, downloadBun, downloadUv, buildMcpServers, getPlatformKey, } from './build/common';
// ---------------------------------------------------------------------------
// CLI
// ---------------------------------------------------------------------------
function showHelp() {
...
// Resource assembly
// ---------------------------------------------------------------------------
function assembleResources(config) {
...
// ---------------------------------------------------------------------------
async function downloadUvForServer(config) {
...
// ---------------------------------------------------------------------------
async function downloadBunForServer(config) {
    const { platform, arch, outputDir, skipDownload } = config;
 ...
 */
function copyDependencyTree(dep, srcModules, destModules, visited) {
    if (visited.has(dep))
        return;
    visited.add(dep);
        ...
        for (const entry of readdirSync(d, { withFileTypes: true })) {
            ...
            if (entry.isDirectory() && entry.name !== 'node_modules' && entry.name !== 'tests' && entry.name !== '__tests__') {
                ...
                while ((match = importRe.exec(content)) !== null) {
                    ...
                    if (spec.startsWith('.') || spec.startsWith('node:') || spec.startsWith('@craft-agent/'))
                        continue;
...
function copyProductionDeps(config) {
...
// Workspace packages
// ---------------------------------------------------------------------------
function copyWorkspacePackages(config) {
...
// ---------------------------------------------------------------------------
function createRootConfig(config) {
...
// Entry scripts
// ---------------------------------------------------------------------------
function createEntryScripts(config) {
...
// Docker
// ---------------------------------------------------------------------------
function createDockerFiles(config) {
    const { outputDir, version } = config;
