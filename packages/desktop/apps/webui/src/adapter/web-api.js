...
export function createWebApi(options) {
    ...
    const baseApi = buildClientApi(client, CHANNEL_MAP, (ch) => client.isChannelAvailable(ch));
    ...
    const webOverrides = {
        ...
        openWorkspace: async () => { },
        openSessionInNewWindow: async (_wsId, sessionId) => {
            // Open in new tab
        ...
        checkForUpdates: () => Promise.resolve({ available: false, currentVersion: client.getServerVersion() ?? '' }),
        getUpdateInfo: () => Promise.resolve({ available: false, currentVersion: client.getServerVersion() ?? '' }),
        ...
        checkGitBash: () => Promise.resolve({ available: true }),
        ...
        // Transport state
        getTransportConnectionState: () => Promise.resolve(client.getConnectionState()),
        onTransportConnectionStateChanged: (cb) => {
            return client.onConnectionStateChanged(cb);
        },
        reconnectTransport: () => { client.reconnectNow(); return Promise.resolve(); },
        isChannelAvailable: (ch) => client.isChannelAvailable(ch),
