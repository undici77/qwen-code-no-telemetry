...
import { LIVE_HOST_BUNDLE_ID, LIVE_PROTOCOL_VERSION, MAX_CONTROL_FRAME_BYTES, MAX_INPUT_AUDIO_WIRE_FRAME_BYTES, MAX_OUTPUT_AUDIO_FRAME_BYTES, MAX_SOCKET_BUFFERED_BYTES, encodeInputAudioFrame, encodeHostControlMessage, isValidOutputAudioFrame, parseDaemonControlMessage, } from '../shared/protocol.ts';
import { buildHostWebSocketUrl, buildWebShellSessionUrl, DiscoveryMonitor, resolveDiscoveryPath, } from './discovery.ts';
import { BoundedReconnectPolicy } from './reconnect-policy.ts';
const HANDSHAKE_TIMEOUT_MS = 5_000;
const DEFAULT_HEARTBEAT_INTERVAL_MS = 5_000;
const EXHAUSTED_RETRY_DELAY_MS = 30_000;
export function canSendHostControlMessage(message, socketOpen, welcomed, bufferedAmount) {
    return (socketOpen &&
        (message.type === 'host.hello' || welcomed) &&
        bufferedAmount <= MAX_SOCKET_BUFFERED_BYTES);
}
const MAX_APPSHOT_ERROR_CHARS = 1_024;
function screenContextResultMessage(requestId, result) {
    const message = (accessibilityText) => ({
    ...
    const fits = (candidate) => Buffer.byteLength(JSON.stringify(candidate), 'utf8') <=
...
export class LiveDaemonConnection {
    ...
    callbacks;
    retryOptions;
    hostInstanceNonce = randomBytes(24).toString('base64url');
    reconnectPolicy;
    discovery;
    currentRecord;
    currentSignature = '';
    ...
    constructor(hostVersion, callbacks, discoveryPath = resolveDiscoveryPath(), retryOptions = {}) {
    ...
    handleDiscovery(result) {
        ...
        if (result.signature === this.currentSignature && this.socket)
            return;
    ...
    connect(record) {
        ...
        socket.on('message', (data, isBinary) => {
            ...
            if (isBinary) {
                ...
                this.callbacks.onOutputAudio(new Uint8Array(frame.buffer, frame.byteOffset, frame.byteLength));
                return;
            }
            ...
            switch (message.type) {
                case 'host.welcome':
                    ...
                    if (!this.nonceMatches(message.daemonInstanceNonce, record.instanceNonce)) {
                        this.intentionalClose = true;
        ...
        socket.on('error', () => {
            if (socket !== this.socket || this.intentionalClose)
                return;
    ...
    scheduleReconnect() {
        if (!this.currentRecord || this.reconnectTimer)
            return;
        ...
        if (delay === undefined) {
            ...
            this.reconnectTimer = setTimeout(() => {
                ...
                this.reconnectPolicy.reset();
                this.connect(this.currentRecord);
            }, Math.max(1, this.retryOptions.exhaustedRetryDelayMs ?? EXHAUSTED_RETRY_DELAY_MS));
    ...
    sendControl(message) {
        const socket = this.socket;
        if (!socket ||
            !canSendHostControlMessage(message, socket.readyState === WebSocket.OPEN, this.welcomed, socket.bufferedAmount)) {
            return false;
        }
        socket.send(encodeHostControlMessage(message));
        return true;
    }
    async captureScreenContext(requestId, epoch) {
        ...
        try {
            ...
            if (!this.welcomed || epoch !== this.epoch)
                return;
    ...
    nonceMatches(received, expected) {
