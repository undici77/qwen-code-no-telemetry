...
export function validateNativeCapture(value) {
    if (!value ||
        typeof value !== 'object' ||
    ...
    const windowTitle = typeof value.windowTitle === 'string' && value.windowTitle.trim()
    ...
    return {
        ...
        accessibilityText: boundedText(value.accessibilityText, MAX_ACCESSIBILITY_TEXT_CHARS, 'accessibility tree'),
...
export class AppshotCaptureService {
    ...
    constructor(captureDirectory = join(tmpdir(), 'qwen-live-appshot'), native = loadNativeAppshot) {
    ...
    async capture() {
        ...
        try {
            ...
            const capture = validateNativeCapture(await this.native().captureAppshot());
...
//# sourceMappingURL=appshot-capture.js.map