...
import { AppshotCaptureService, validateNativeCapture, } from '../appshot-capture.ts';
const cleanup = [];
const PNG = Buffer.from([137, 80, 78, 71, 13, 10, 26, 10, 1]);
afterEach(async () => {
    await Promise.all(cleanup.splice(0).map((path) => rm(path, { recursive: true, force: true })));
});
function fakeNative(captureAppshot) {
    return {
        getPermissionState: () => ({
            accessibility: true,
...
describe('AppshotCaptureService', () => {
    ...
    it('rejects malformed native results before writing them', () => {
        assert.throws(() => validateNativeCapture({
            appName: 'Safari',
            windowId: 0,
            accessibilityText: '- AXWindow',
            screenshot: PNG,
...
//# sourceMappingURL=appshot-capture.test.js.map