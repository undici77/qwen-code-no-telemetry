...
import { buildHostWebSocketUrl, buildWebShellSessionUrl, DiscoveryMonitor, readDiscoveryFile, } from '../discovery.ts';
...
async function discoveryFile(mode = 0o600) {
...
describe('Live daemon discovery', () => {
    ...
    it('forces the fixed host route and rejects non-loopback URLs', () => {
        assert.equal(buildHostWebSocketUrl('http://127.0.0.1:9527/private?token=nope'), 'ws://127.0.0.1:9527/live/host');
        assert.equal(buildHostWebSocketUrl('http://127.23.45.67:9527'), 'ws://127.23.45.67:9527/live/host');
    ...
    it('builds a scoped WebShell link without putting auth in the query', () => {
        const url = new URL(buildWebShellSessionUrl({
        ...
        assert.equal(new URLSearchParams(url.hash.slice(1)).get('token'), 'secret-not-logged');
    });
    it('coalesces overlapping polls so an older read cannot win', async () => {
        let resolveRead;
        let reads = 0;
        const observed = [];
        const monitor = new DiscoveryMonitor('/unused', (result) => observed.push(result), 1_000, async () => {
            reads += 1;
    ...
    it('suppresses an in-flight result after the monitor stops', async () => {
        let resolveRead;
        const observed = [];
        const monitor = new DiscoveryMonitor('/unused', (result) => observed.push(result), 60_000, async () => await new Promise((resolve) => {
            resolveRead = resolve;
        }));
        monitor.start();
        monitor.stop();
