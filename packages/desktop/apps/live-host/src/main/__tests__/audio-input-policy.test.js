...
import { audioInputConstraints, hasAudioInputDevice, isUnavailableDevicePreference, shouldRecheckAudioInput, } from '../../preload/audio-input-policy.ts';
describe('Live Host audio input policy', () => {
    ...
    it('falls back only when a preferred device is unavailable', () => {
        assert.equal(isUnavailableDevicePreference(new DOMException('missing', 'NotFoundError')), true);
        assert.equal(isUnavailableDevicePreference(new DOMException('constraint', 'OverconstrainedError')), true);
        assert.equal(isUnavailableDevicePreference(new DOMException('denied', 'NotAllowedError')), false);
    });
    ...
    it('checks input availability without opening a microphone stream', () => {
        ...
        assert.equal(hasAudioInputDevice([{ kind: 'audiooutput' }, { kind: 'audioinput' }]), true);
    });
});
//# sourceMappingURL=audio-input-policy.test.js.map