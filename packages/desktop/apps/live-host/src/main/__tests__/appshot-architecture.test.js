...
const BUILDER_CONFIG = new URL('../../../electron-builder.yml', import.meta.url);
...
describe('built-in Appshot architecture', () => {
    it('has no external capture backend or process-launch path', async () => {
        ...
        assert.doesNotMatch(source, /@modelcontextprotocol|node:child_process|NSTask|posix_spawn|execFile(?:Sync)?\s*\(|spawn(?:Sync)?\s*\(|popen\s*\(|system\s*\(/u);
    });
    it('owns one fixed in-process macOS capture implementation', async () => {
        ...
        assert.match(builderConfig, /from: 'dist\/native\/qwen-live-appshot\.node'/u);
    ...
    it('locks down Electron runtime escape hatches in packaged builds', async () => {
        ...
        assert.match(builderConfig, /enableNodeOptionsEnvironmentVariable:\s*false/u);
        ...
        assert.match(builderConfig, /enableEmbeddedAsarIntegrityValidation:\s*true/u);
    });
});
//# sourceMappingURL=appshot-architecture.test.js.map