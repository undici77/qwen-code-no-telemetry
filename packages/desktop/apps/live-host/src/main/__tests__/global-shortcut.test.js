...
function fixture(registerResult = true) {
    const registered = [];
    const unregistered = [];
    ...
    const backend = {
        register: (accelerator, callback) => {
            ...
            const registeredSuccessfully = typeof registerResult === 'function'
                ? registerResult()
                : registerResult;
    ...
    let toggles = 0;
    const shortcut = new LiveGlobalShortcut(backend, () => {
        toggles += 1;
...
//# sourceMappingURL=global-shortcut.test.js.map