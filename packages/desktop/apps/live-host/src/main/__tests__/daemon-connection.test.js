...
import { canSendHostControlMessage, LiveDaemonConnection, } from '../daemon-connection.ts';
import { BoundedReconnectPolicy } from '../reconnect-policy.ts';
import { INPUT_AUDIO_EPOCH_BYTES, LIVE_PROTOCOL_VERSION, MAX_CONTROL_FRAME_BYTES, MAX_SOCKET_BUFFERED_BYTES, } from '../../shared/protocol.ts';
const cleanup = [];
afterEach(async () => {
...
function nextMessage(socket) {
    return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => reject(new Error('Timed out waiting for WebSocket frame')), 3_000);
...
describe('LiveDaemonConnection', () => {
    it('authenticates, handshakes, handles heartbeat, and enforces binary ownership', async () => {
        ...
        const shortcuts = [];
        let captureCalls = 0;
        const connection = new LiveDaemonConnection('0.0.6', {
            getReadiness: () => ({
                permissions: {
                    microphone: 'granted',
            ...
            captureScreenContext: async () => {
                ...
                return {
                    ...
                    accessibilityText: captureCalls === 1
                        ? 'AXWindow LIVE_APP_A'
                        : '\u0001'.repeat(32_000),
        ...
        const hello = JSON.parse(helloFrame.data.toString('utf8'));
        ...
        for (const action of requiredActions) {
            ...
            assert.equal(canSendHostControlMessage(action, true, true, MAX_SOCKET_BUFFERED_BYTES + 1), false);
        }
        ...
        const boundedCapture = JSON.parse(boundedCaptureFrame.data.toString('utf8'));
        assert.equal(boundedCapture.success, true);
        assert.equal(Buffer.byteLength(boundedCaptureFrame.data.toString('utf8'), 'utf8') <=
        ...
        const failedCapture = JSON.parse(failedCaptureFrame.data.toString('utf8'));
        assert.equal(failedCapture.success, false);
        assert.equal(failedCapture.error.length, 1_024);
        assert.equal(Buffer.byteLength(failedCaptureFrame.data.toString('utf8'), 'utf8') <=
            MAX_CONTROL_FRAME_BYTES, true);
        ...
        assert.deepEqual(inputFrame.data.subarray(INPUT_AUDIO_EPOCH_BYTES), Buffer.alloc(640));
    ...
    it('keeps retrying the same discovery identity slowly after the fast budget', async () => {
        ...
        await new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error('Timed out waiting for slow retry')), 2_000);
            const interval = setInterval(() => {
                if (connection.getSnapshot().phase !== 'ready')
                    return;
                clearTimeout(timeout);
...
//# sourceMappingURL=daemon-connection.test.js.map