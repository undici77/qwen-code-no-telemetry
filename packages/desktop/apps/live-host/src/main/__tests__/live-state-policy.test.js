...
import { canToggleLive, isActiveLiveCall, projectLiveStatusForCapture, shouldCaptureLiveAudio, shouldRenderSetup, shouldStopLiveOnToggle, } from '../live-state-policy.ts';
function status(state, available) {
...
describe('Live Host state policy', () => {
    ...
    it('treats a second shortcut press as stop while start is pending', () => {
        ...
        assert.equal(shouldStopLiveOnToggle(status('listening', true), false), true);
        assert.equal(shouldStopLiveOnToggle(status('idle', true), false), false);
    });
});
//# sourceMappingURL=live-state-policy.test.js.map