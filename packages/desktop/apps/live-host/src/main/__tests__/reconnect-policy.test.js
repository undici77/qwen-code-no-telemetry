...
describe('BoundedReconnectPolicy', () => {
    ...
    it('keeps jitter bounded', () => {
        assert.equal(new BoundedReconnectPolicy([100], 0.2, () => 0).nextDelayMs(), 80);
        assert.equal(new BoundedReconnectPolicy([100], 0.2, () => 1).nextDelayMs(), 120);
    });
});
//# sourceMappingURL=reconnect-policy.test.js.map