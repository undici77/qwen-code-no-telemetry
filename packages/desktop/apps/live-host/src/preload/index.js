...
const inputLevelListeners = new Set();
const diagnosticsEnabled = process.env['QWEN_LIVE_DIAGNOSTICS'] === '1';
const audio = new HostAudioEngine((level) => {
    for (const listener of inputLevelListeners)
        listener(level);
}, (event, details) => {
...
const api = {
    ...
    requestPermission: (permission) => invoke('live:request-permission', permission),
    ...
    getState: () => ipcRenderer.invoke('live:get-state'),
    onState: (listener) => {
...
ipcRenderer.on('live:audio:initialize', (_event, microphoneAllowed) => {
    void audio.initialize(microphoneAllowed);
});
ipcRenderer.on('live:audio:deactivate', () => {
    void audio.dispose();
});
ipcRenderer.on('live:audio:recheck', (_event, reason) => {
    void audio.recheck(reason);
});
ipcRenderer.on('live:audio:set-capture', (_event, value) => {
    void audio
...
ipcRenderer.on('live:audio:set-output-muted', (_event, muted) => {
    audio.setOutputMuted(muted);
});
ipcRenderer.on('live:audio:play', (_event, frame) => {
...
window.addEventListener('mousemove', (event) => {
    ...
    requestAnimationFrame(() => {
        ...
        if (interactive === lastPointerInteractive)
            return;
