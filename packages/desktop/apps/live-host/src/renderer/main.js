...
if (!appRoot)
    throw new Error('Missing Live Host root');
const app = appRoot;
...
function permissionRow(label, permission, state) {
    ...
    if (state === 'granted')
        ...
        row.append(title, button('授权', () => void window.qwenLiveHost.requestPermission(permission)));
    return row;
}
...
function liveStatus(state) {
    if (state.live.statusText)
        return state.live.statusText;
...
function controlButton(label, icon, action, options) {
...
function renderSetup(state) {
    ...
    if (state.connection === 'ready') {
        ...
        permissions.append(permissionRow('麦克风', 'microphone', state.permissions.microphone), permissionRow('辅助功能（Appshot）', 'accessibility', state.permissions.accessibility), permissionRow('屏幕录制（Appshot）', 'screenRecording', state.permissions.screenRecording));
...
function render(state) {
    ...
    orb.addEventListener('click', () => {
        if (active)
            ...
            void window.qwenLiveHost.toggle();
    });
    stage.append(orb);
    if (state.live.pendingPermission) {
        const openWebShell = button('等待授权 · 前往 WebShell', () => void window.qwenLiveHost.openWebShellForPermission(), { className: 'web-shell-permission' });
    ...
    if (active) {
        actions.append(controlButton(state.live.outputMuted ? '取消语音静音' : '语音静音', state.live.outputMuted ? 'speakerOff' : 'speaker', () => {
            void window.qwenLiveHost.setOutputMuted(!state.live.outputMuted);
        }), controlButton('停止语音对话', 'stop', () => {
            void window.qwenLiveHost.stop();
        }, { primary: true }), controlButton(state.live.inputMuted ? '取消麦克风静音' : '麦克风静音', state.live.inputMuted ? 'microphoneOff' : 'microphone', () => {
...
window.qwenLiveHost.onInputLevel((level) => {
    const orb = document.querySelector('.voice-orb.listening');
    if (!orb)
        return;
