 ...
 */
export async function connectToRemote(url, token, workspaceId) {
...
export function registerWorkspaceGuiHandlers(server, deps) {
    ...
    server.handle(RPC_CHANNELS.remote.TEST_CONNECTION, async (_ctx, url, token) => {
        ...
        try {
            ...
            const workspaces = await client.invoke(RPC_CHANNELS.server.GET_WORKSPACES);
    ...
    server.handle(RPC_CHANNELS.window.OPEN_WORKSPACE, async (_ctx, workspaceId) => {
        if (!windowManager)
            return;
        windowManager.focusOrCreateWindow(workspaceId);
    });
    // Open a session in a new window
    server.handle(RPC_CHANNELS.window.OPEN_SESSION_IN_NEW_WINDOW, async (_ctx, workspaceId, sessionId) => {
        if (!windowManager)
            return;
    ...
    server.handle(RPC_CHANNELS.window.SET_TRAFFIC_LIGHTS, (ctx, visible) => {
        if (!windowManager)
            return;
