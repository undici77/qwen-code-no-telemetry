...
import { requestClientOpenExternal, requestClientOpenPath, requestClientShowInFolder, requestClientOpenFileDialog, } from '@craft-agent/server-core/transport';
...
export function registerSystemCoreHandlers(server, deps) {
    ...
    server.handle(RPC_CHANNELS.releaseNotes.GET, async () => {
        const { getCombinedReleaseNotes } = require('@craft-agent/shared/release-notes');
        return getCombinedReleaseNotes();
    });
    server.handle(RPC_CHANNELS.releaseNotes.GET_LATEST_VERSION, async () => {
        const { getLatestReleaseVersion } = require('@craft-agent/shared/release-notes');
        return getLatestReleaseVersion();
    });
    // Get git branch for a directory (returns null if not a git repo or git unavailable)
    server.handle(RPC_CHANNELS.git.GET_BRANCH, async (_ctx, dirPath) => {
        try {
    ...
    server.handle(RPC_CHANNELS.gitbash.SET_PATH, async (_ctx, bashPath) => {
    ...
    server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url) => {
        ...
        try {
            ...
            if (parsed.protocol === 'craftagents:') {
                ...
                const resolver = (wcId) => windowManager.getClientIdForWindow(wcId);
    ...
    server.handle(RPC_CHANNELS.shell.OPEN_FILE, async (ctx, path) => {
        try {
    ...
    server.handle(RPC_CHANNELS.shell.SHOW_IN_FOLDER, async (ctx, path) => {
        try {
            const expanded = path.startsWith('~') ? path.replace(/^~/, homedir()) : path;
            const absolutePath = resolve(expanded);
            const workspaceId = ctx.workspaceId ?? deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId);
...
export function registerSystemGuiHandlers(server, deps) {
    ...
    server.handle(RPC_CHANNELS.update.DISMISS, async (_ctx, version) => {
    ...
    // Notifications
    server.handle(RPC_CHANNELS.notification.SHOW, async (_ctx, title, body, workspaceId, sessionId) => {
    ...
    server.handle(RPC_CHANNELS.notification.SET_ENABLED, async (_ctx, enabled) => {
    ...
    server.handle(RPC_CHANNELS.badge.SET_ICON, async (_ctx, dataUrl) => {
...
export function registerSystemHandlers(server, deps) {
