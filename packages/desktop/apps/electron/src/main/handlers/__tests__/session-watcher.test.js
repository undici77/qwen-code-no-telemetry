...
function createTestHarness(sessionPaths) {
    ...
    const deps = {
        sessionManager: {
            getSessionPath: (sessionId) => sessionPaths.get(sessionId) ?? null,
            waitForInit: async () => { },
            getSessions: () => [],
        },
        platform: {
...
function makeCtx(clientId, workspaceId = 'ws-1') {
...
describe('session file watcher isolation', () => {
    ...
    it('cleans up previous watcher when same client watches a different session', async () => {
        ...
        await new Promise(r => setTimeout(r, 300));
        const s1Pushes = pushCalls.filter(p => p.args[0] === 's1' && p.channel === RPC_CHANNELS.sessions.FILES_CHANGED);
        expect(s1Pushes.length).toBe(0);
        // Write to s2 — should trigger notification
        writeFileSync(join(dir2, 'new.txt'), 'fresh');
        await new Promise(r => setTimeout(r, 300));
        const s2Pushes = pushCalls.filter(p => p.args[0] === 's2' && p.channel === RPC_CHANNELS.sessions.FILES_CHANGED);
