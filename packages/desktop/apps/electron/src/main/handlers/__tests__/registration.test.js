...
async function getExpectedChannels() {
    // Core handler channels (now in server-core)
    const [auth, automations, files, labels, llm, oauth, sessions, coreSettings, skills, sources, statuses, coreSystem, coreWorkspace, onboarding, resources,] = await Promise.all([
    ...
    const [browser, guiSystem, guiWorkspace, guiSettings, guiWindowDrag, guiPetWindow,] = await Promise.all([
        import('../browser'),
        import('../system'),
        import('../workspace'),
        import('../settings'),
