 ...
 */
export function registerPetWindowGuiHandlers(server, deps) {
    server.handle(RPC_CHANNELS.window.PET_SET_ENABLED, (ctx, enabled) => {
    ...
    server.handle(RPC_CHANNELS.window.PET_SET_IGNORE_MOUSE, (_ctx, ignore) => {
    ...
    server.handle(RPC_CHANNELS.window.PET_FOCUS_SESSION, async (ctx, sessionId) => {
        const wm = deps.windowManager;
        if (!wm || !sessionId || ctx.webContentsId == null)
            return;
...
//# sourceMappingURL=pet-window.js.map