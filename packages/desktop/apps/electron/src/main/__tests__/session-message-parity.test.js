...
function createMinimalMessage(role = 'user') {
    return {
        id: 'msg-minimal',
        role,
        content: 'Hello',
...
describe('messageToStored/storedToMessage round-trip', () => {
    ...
    it('role ↔ type mapping works for each MessageRole', () => {
        const roles = ['user', 'assistant', 'tool', 'error', 'info', 'warning', 'plan', 'auth-request'];
...
describe('persistence pipeline filtering', () => {
    function createMessageWithRole(role, extra = {}) {
...
//# sourceMappingURL=session-message-parity.test.js.map