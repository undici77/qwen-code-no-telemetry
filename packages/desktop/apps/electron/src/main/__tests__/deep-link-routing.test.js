...
function createMockWindow(webContentsId) {
    return {
        isMinimized: () => false,
        restore: () => { },
        focus: () => { },
...
describe('handleDeepLink routing', () => {
    it('prefers resolved target client over preferred caller client', async () => {
        ...
        const windowManager = {
            ...
            getWorkspaceForWindow: (webContentsId) => webContentsId === 22 ? 'ws-target' : 'ws-other',
        };
        const sent = [];
        const sink = (channel, target, ...args) => {
            sent.push({ channel, target, args });
        };
        await handleDeepLink('craftagents://workspace/ws-target/allSessions', windowManager, sink, (wcId) => wcId === 22 ? 'client-target' : undefined, 'client-caller');
    ...
    it('uses preferred client only when no resolver is provided', async () => {
        ...
        await handleDeepLink('craftagents://workspace/ws-target/allSessions', windowManager, sink, undefined, 'client-caller');
        expect(sent.length).toBe(1);
...
//# sourceMappingURL=deep-link-routing.test.js.map