...
// Mock Helpers
// ============================================================================
function createMockWebContents(sendCommandImpl) {
    const listeners = {};
    return {
        debugger: {
            attach: mock((_version) => { }),
            detach: mock(() => { }),
        ...
        _triggerDetach: () => {
            for (const cb of listeners['detach'] || [])
                cb();
        },
    };
}
...
describe('BrowserCDP', () => {
    describe('ensureAttached', () => {
        ...
        it('registers detach listener only once', async () => {
            ...
            const detachCalls = wc.debugger.on.mock.calls.filter((call) => call[0] === 'detach');
