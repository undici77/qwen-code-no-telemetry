...
let emptyStateLoadError = null;
const pageLoadErrorsByUrl = new Map();
...
function createMockWebContents() {
    ...
    return {
        ...
        loadFile: mock(async (_path, _opts) => {
        ...
        insertCSS: mock(async (_css, _opts) => 'mock-css-key'),
        removeInsertedCSS: mock(async (_key) => { }),
        goBack: mock(() => { }),
        goForward: mock(() => { }),
        reload: mock(() => { }),
        stop: mock(() => { }),
        ...
        capturePage: mock(async () => {
            const img = {
                ...
                toJPEG: (_quality) => Buffer.from('fake-jpeg'),
            };
            return img;
        }),
        executeJavaScript: mock(async (expr) => eval(expr)),
        focus: mock(() => { }),
        setWindowOpenHandler: mock((_handler) => { }),
        send: mock((_channel, _payload) => { }),
        debugger: {
            attach: mock(() => { }),
            detach: mock(() => { }),
...
function createMockWindow(opts) {
    ...
    const win = {
        ...
        show: mock(() => { }),
        showInactive: mock(() => { }),
        setWindowButtonVisibility: mock((_visible) => { }),
        hide: mock(() => {
            win._emit('hide');
        }),
        focus: mock(() => { }),
...
mock.module('electron', () => ({
    ...
    session: {
        fromPartition: mock(() => ({
            ...
            webRequest: {
                onBeforeRequest: mock((_cb) => { }),
                onCompleted: mock((_cb) => { }),
                onErrorOccurred: mock((_cb) => { }),
            },
            on: mock((_event, _cb) => { }),
        })),
    },
...
describe('BrowserPaneManager', () => {
    ...
    it('docks an instance into a host window', () => {
        ...
        const instance = manager.instances.get('dock-1');
        ...
        manager.dock('dock-1', hostWindow, {
            x: 360,
            y: 0,
            width: 640,
            height: 800,
    ...
    it('does not blank the page when empty-state loading is aborted by navigation', async () => {
        ...
        const instance = manager.instances.get('dock-race');
    ...
    it('retries aborted navigation when reusing the docked browser', async () => {
        ...
        const instance = manager.instances.get('dock-reuse');
        ...
        expect(instance.pageView.webContents.loadURL.mock.calls.filter(([url]) => url === secondUrl)).toHaveLength(2);
    ...
    it('allows http(s) popups with shared browser partition', () => {
        ...
        const instance = manager.instances.get('popup-allow');
    ...
    it('denies app deep-link popups and forwards to deep-link handler', async () => {
        ...
        const instance = manager.instances.get('popup-deeplink');
        const openHandler = instance.pageView.webContents.setWindowOpenHandler.mock.calls[0][0];
        const result = openHandler({
    ...
    it('destroys child popups when parent instance is destroyed', () => {
        ...
        const instance = manager.instances.get('popup-parent');
        ...
        expect(manager.popupWindowsByParentInstanceId.get('popup-parent')?.size).toBe(1);
        manager.destroyInstance('popup-parent');
        expect(popupWindow.destroy).toHaveBeenCalledTimes(1);
        expect(manager.popupWindowsByParentInstanceId.has('popup-parent')).toBe(false);
    });
    ...
    it('destroys instance via toolbar destroy IPC handler', async () => {
        ...
        const destroyRegistration = mockIpcMainHandle.mock.calls.find(([channel]) => channel === 'browser-toolbar:destroy');
        expect(destroyRegistration).toBeTruthy();
        if (!destroyRegistration)
            throw new Error('Expected browser-toolbar:destroy IPC registration');
    ...
    it('ignores late state events after instance was removed', () => {
        ...
        const instance = manager.instances.get('d-late-state');
        states.length = 0;
    ...
    it('navigate normalizes hostnames to https', async () => {
        ...
        const instance = manager.instances.get('nav-1');
    ...
    it('navigate recognizes host-like inputs with a query', async () => {
        ...
        const instance = manager.instances.get('nav-query');
        expect(instance.pageView.webContents.loadURL).toHaveBeenCalledWith('https://example.com?q=1');
    });
    it('navigate recognizes host-like inputs with a fragment', async () => {
        ...
        const instance = manager.instances.get('nav-fragment');
        expect(instance.pageView.webContents.loadURL).toHaveBeenCalledWith('https://localhost:3000#docs');
    });
    it('navigate searches invalid host-like inputs instead of loading them', async () => {
        ...
        const instance = manager.instances.get('nav-invalid');
        expect(instance.pageView.webContents.loadURL).toHaveBeenCalledWith('https://duckduckgo.com/?q=192.168.1.1%3A70000');
    });
    it('navigate treats plain text as search query', async () => {
        ...
        const instance = manager.instances.get('nav-2');
        expect(instance.pageView.webContents.loadURL).toHaveBeenCalledWith('https://duckduckgo.com/?q=craft%20agents%20browser%20tools');
    });
    it('navigate searches free text with lone surrogates using replacement characters', async () => {
        ...
        const instance = manager.instances.get('nav-surrogate');
        expect(instance.pageView.webContents.loadURL).toHaveBeenCalledWith('https://duckduckgo.com/?q=search%20%EF%BF%BD%20term');
    });
    ...
    it('clears navigation timeout timer on success', async () => {
        ...
        const clearTimeoutSpy = mock((handle) => originalClearTimeout(handle));
    ...
    it('focus brings the instance window to front', () => {
        ...
        manager.focus('f1');
        const instance = manager.instances.get('f1');
    ...
    it('dedupes repeated focus calls before ready-to-show', () => {
        ...
        manager.focus('f2');
        manager.focus('f2');
        const instance = manager.instances.get('f2');
        instance.window._emit('ready-to-show');
    ...
    it('cancels deferred pre-ready focus when hide happens first', () => {
        ...
        const instance = manager.instances.get('f-hide-race');
    ...
    it('user close hides window and keeps instance alive', () => {
        ...
        const instance = manager.instances.get('h1');
    ...
    it('does not intercept close when destroy is explicit', () => {
        ...
        const instance = manager.instances.get('h-explicit-destroy');
        manager.destroyingIds.add('h-explicit-destroy');
        const closeEvent = { preventDefault: mock(() => { }) };
        instance.window._emit('close', closeEvent);
    ...
    it('still destroys instance when cleanup throws', () => {
        ...
        const instance = manager.instances.get('destroy-cleanup-throw');
        manager.updateNativeOverlayState = () => {
    ...
    it('emits removed callback when window closes', () => {
        ...
        const instance = manager.instances.get('r1');
    ...
    it('retries toolbar load and recovers', async () => {
        ...
        const toolbarUrlAttempts = toolbarWindow.webContents.loadURL.mock.calls
            .filter((args) => args[0]?.includes('browser-toolbar.html')).length;
    ...
    it('captures and filters console entries', () => {
        ...
        const instance = manager.instances.get('console-1');
    ...
    it('applies observer theme signal and skips regular console logging for it', () => {
        ...
        const instance = manager.instances.get('theme-signal');
    ...
    it('dedupes repeated observer theme signals', () => {
        ...
        const instance = manager.instances.get('theme-dedupe');
    ...
    it('ignores observer theme signals from stale token', () => {
        ...
        const instance = manager.instances.get('theme-stale-token');
    ...
    it('clears theme on explicit null sentinel signal', () => {
        ...
        const instance = manager.instances.get('theme-null');
    ...
    it('replays toolbar state with theme color when window is shown', () => {
        ...
        const instance = manager.instances.get('theme-show-replay');
    ...
    it('replays full toolbar state when toolbar renderer finishes loading', () => {
        ...
        const instance = manager.instances.get('toolbar-finish-load-replay');
    ...
    it('does not mark toolbar ready for about:blank did-finish-load', () => {
        ...
        const instance = manager.instances.get('toolbar-ignore-about-blank');
    ...
    it('marks toolbar ready for fallback data page did-finish-load', () => {
        ...
        const instance = manager.instances.get('toolbar-fallback-ready');
    ...
    it('keeps focus deferred until a valid toolbar document loads', () => {
        ...
        const instance = manager.instances.get('toolbar-focus-guard');
    ...
    it('runs early theme extraction shortly after navigation', async () => {
        ...
        const instance = manager.instances.get('theme-early');
    ...
    it('clears pending in-page theme timer on full navigation', async () => {
        ...
        const instance = manager.instances.get('theme-timer-clear');
    ...
    it('throws when screenshot capture returns empty NativeImage', async () => {
        ...
        const instance = manager.instances.get('screenshot-empty-image');
    ...
    it('throws when screenshot capture returns empty PNG buffer', async () => {
        ...
        const instance = manager.instances.get('screenshot-empty-png');
        instance.pageView.webContents.capturePage = mock(async () => ({
            isEmpty: () => false,
            getSize: () => ({ width: 2400, height: 1800 }),
            resize: function () { return this; },
    ...
    it('recovers screenshot via non-disruptive inactive reveal and restores hidden state', async () => {
        ...
        const instance = manager.instances.get('screenshot-rescue-success');
        let captureCalls = 0;
        instance.pageView.webContents.capturePage = mock(async () => {
            captureCalls += 1;
            if (captureCalls <= 3) {
            ...
            const img = {
                ...
                toJPEG: (_q) => Buffer.from('rescued-jpeg'),
            };
            return img;
        });
        ...
        expect(instance.window.hide).toHaveBeenCalled();
        expect(result.metadata?.warnings?.some((w) => w.includes('temporary inactive reveal'))).toBe(true);
    });
    it('throws when region screenshot capture returns empty NativeImage', async () => {
        manager.createInstance('region-empty-image');
        const instance = manager.instances.get('region-empty-image');
        instance.pageView.webContents.capturePage = mock(async () => ({
            isEmpty: () => true,
            getSize: () => ({ width: 0, height: 0 }),
            resize: function () { return this; },
        ...
        await expect(manager.screenshotRegion('region-empty-image', { x: 10, y: 20, width: 120, height: 80 })).rejects.toThrow('Failed to capture region screenshot: empty image buffer');
    });
    it('throws when region screenshot capture returns empty PNG buffer', async () => {
        manager.createInstance('region-empty-png');
        const instance = manager.instances.get('region-empty-png');
        instance.pageView.webContents.capturePage = mock(async () => ({
            isEmpty: () => false,
            getSize: () => ({ width: 2400, height: 1800 }),
            resize: function () { return this; },
        ...
        await expect(manager.screenshotRegion('region-empty-png', { x: 10, y: 20, width: 120, height: 80 })).rejects.toThrow('Failed to capture region screenshot: empty image buffer');
    });
    ...
    it('throws for ambiguous screenshot region target modes', async () => {
        ...
        await expect(manager.screenshotRegion('region-ambiguous', { ref: '@e1', selector: 'div.card' })).rejects.toThrow('Region screenshot target is ambiguous');
    });
    it('throws when selector target cannot be resolved', async () => {
        manager.createInstance('region-selector-missing');
        const instance = manager.instances.get('region-selector-missing');
        instance.cdp.getElementGeometryBySelector = mock(async () => {
            throw new Error('No element found for selector "div.missing"');
        });
        await expect(manager.screenshotRegion('region-selector-missing', { selector: 'div.missing' })).rejects.toThrow('No element found for selector "div.missing"');
    });
    it('throws when resolved region is outside viewport', async () => {
        manager.createInstance('region-oob');
        await expect(manager.screenshotRegion('region-oob', { x: 5000, y: 5000, width: 100, height: 100 })).rejects.toThrow('Resolved screenshot region is outside the current viewport');
    });
    it('resizes browser window viewport and returns effective applied size', () => {
        ...
        const instance = manager.instances.get('resize-1');
    ...
    describe('agent control overlay', () => {
        it('setAgentControl activates native overlay on bound instance', async () => {
            ...
            await Promise.resolve();
            const instance = manager.instances.get('ac-1');
        ...
        it('keeps native overlay visible for active session control', async () => {
            ...
            await Promise.resolve();
            const instance = manager.instances.get('ac-idle');
        ...
        it('reapplies native overlay after did-stop-loading while control is active', async () => {
            ...
            await Promise.resolve();
            const instance = manager.instances.get('ac-reapply');
        ...
        it('reapplies native overlay after hide/show while control is active', async () => {
            ...
            await Promise.resolve();
            const instance = manager.instances.get('ac-show-reapply');
            const callCountAfterSet = instance.nativeOverlayView.webContents.executeJavaScript.mock.calls.length;
            instance.window._emit('hide');
            instance.window._emit('show');
            await Promise.resolve();
        ...
        it('setAgentControl uses fallback label when no intent', async () => {
            ...
            await Promise.resolve();
            const instance = manager.instances.get('ac-2');
        ...
        it('setAgentControl uses default label when no metadata', async () => {
            ...
            await Promise.resolve();
            const instance = manager.instances.get('ac-3');
            const calls = instance.nativeOverlayView.webContents.executeJavaScript.mock.calls;
            expect(calls.length).toBeGreaterThan(0);
        ...
        it('clearAgentControl dismisses native overlay', () => {
            ...
            const instance = manager.instances.get('ac-4');
        ...
        it('clearAgentControl is a no-op when not active', () => {
            ...
            const instance = manager.instances.get('ac-5');
        ...
        it('clearVisualsForSession resets agent control state', async () => {
            ...
            const instance = manager.instances.get('ac-6');
            expect(instance.agentControl).toBeNull();
            expect(instance.nativeOverlayView.setBounds).toHaveBeenCalledWith({ x: 0, y: 0, width: 0, height: 0 });
        });
        it('setAgentControl ignores unbound sessions', () => {
            ...
            const instance = manager.instances.get('ac-7');
            expect(instance.agentControl).toBeNull();
            expect(instance.nativeOverlayView.webContents.executeJavaScript).not.toHaveBeenCalled();
        });
        it('navigate does not trigger overlay by itself', async () => {
            ...
            const instance = manager.instances.get('ac-8');
            expect(instance.agentControl).toBeNull();
            expect(instance.nativeOverlayView.webContents.executeJavaScript).not.toHaveBeenCalled();
        });
    });
    describe('failed interaction tracking', () => {
        it('clickElement records failed lastAction on error', async () => {
            ...
            const instance = manager.instances.get('fail-click');
        ...
        it('fillElement records failed lastAction on error', async () => {
            ...
            const instance = manager.instances.get('fail-fill');
        ...
        it('selectOption records failed lastAction on error', async () => {
            ...
            const instance = manager.instances.get('fail-select');
...
//# sourceMappingURL=browser-pane-manager.test.js.map