...
function summarizeTopCounts(map, maxEntries = 8) {
...
export class BrowserCDP {
    ...
    async getAccessibilitySnapshot() {
        ...
        const droppedReasonCounts = new Map();
        const seenBackendNodeIds = new Set();
        const pushAccessNode = (entry) => {
            if (result.length >= MAX_AX_SNAPSHOT_NODES)
                return false;
            ...
            if (entry.value !== undefined)
                accessNode.value = entry.value;
            if (entry.description)
                accessNode.description = entry.description;
            if (entry.focused)
                accessNode.focused = true;
            if (entry.checked)
                accessNode.checked = true;
            if (entry.disabled)
                accessNode.disabled = true;
            ...
            if (props) {
                for (const prop of props) {
                    ...
                    if (prop.name === 'checked' && prop.value?.value !== 'false')
                        checked = prop.value?.value === true || prop.value?.value === 'true';
        ...
        if (result.length === 0 && fallbackCandidates.length > 0) {
            ...
            mainLog.info(`[browser-cdp] snapshot fallback engaged url=${this.webContents.getURL()} raw=${nodes.length} kept=${result.length} fallbackKept=${fallbackKept} roles=[${summarizeTopCounts(rawRoleCounts)}] dropped=[${summarizeTopCounts(droppedReasonCounts)}]`);
        }
        if (result.length === 0 && nodes.length > 0) {
            mainLog.warn(`[browser-cdp] snapshot produced zero nodes url=${this.webContents.getURL()} raw=${nodes.length} roles=[${summarizeTopCounts(rawRoleCounts)}] dropped=[${summarizeTopCounts(droppedReasonCounts)}]`);
        }
        return {
    ...
    async getElementGeometryBySelector(selector) {
     ...
     */
    generateTrajectory(fromX, fromY, toX, toY, steps) {
        const points = [];
    ...
    sendMouseEvent(type, x, y, button, clickCount) {
        const event = { type, x: Math.round(x), y: Math.round(y) };
        if (button)
            event.button = button;
    ...
    async setFileInputFiles(ref, filePaths) {
        const backendNodeId = this.refMap.get(ref);
        if (!backendNodeId) {
            throw new Error(`Element ${ref} not found. Run browser_snapshot first to get current element refs.`);
        }
...
//# sourceMappingURL=browser-cdp.js.map