...
import { getRendererDevOrigin, isTrustedRendererFrameUrl, } from '../frame-trust';
...
describe('getRendererDevOrigin', () => {
    ...
    it('derives the origin from a valid dev server url', () => {
        ...
        expect(getRendererDevOrigin('http://localhost:5173/index.html?x=1')).toBe(DEV_URL);
    });
...
describe('isTrustedRendererFrameUrl', () => {
    it('(a) trusts a file:// frame in production', () => {
        expect(isTrustedRendererFrameUrl('file:///app/index.html', undefined)).toBe(true);
    });
    it('(b) trusts a frame whose origin matches the dev server', () => {
        expect(isTrustedRendererFrameUrl('http://localhost:5173/index.html', DEV_URL)).toBe(true);
        // Origin match only — a different path on the same origin is still trusted.
        expect(isTrustedRendererFrameUrl('http://localhost:5173/deep/path?q=1#h', DEV_URL)).toBe(true);
    });
    it('(c) does NOT trust a cross-origin frame', () => {
        expect(isTrustedRendererFrameUrl('https://evil.example.com/x', DEV_URL)).toBe(false);
        // Same host, different port/scheme is still a different origin.
        expect(isTrustedRendererFrameUrl('http://localhost:6006/index.html', DEV_URL)).toBe(false);
        expect(isTrustedRendererFrameUrl('https://localhost:5173/index.html', DEV_URL)).toBe(false);
    });
    ...
    it('(f) flows VITE_DEV_SERVER_URL through: a dev-origin frame is trusted only when the dev url is set', () => {
        ...
        expect(isTrustedRendererFrameUrl('http://localhost:5173/index.html', undefined)).toBe(false);
        // A malformed dev url falls back to no trusted dev origin.
        expect(isTrustedRendererFrameUrl('http://localhost:5173/index.html', 'not-a-url')).toBe(false);
    });
});
//# sourceMappingURL=frame-trust.test.js.map