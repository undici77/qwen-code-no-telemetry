 ...
 */
export function parseNoProxyRules(noProxy) {
    if (!noProxy)
        return [];
...
export function shouldBypassProxy(url, rules) {
    if (rules.length === 0)
        return false;
    ...
    for (const rule of rules) {
        ...
        if (host.endsWith(`.${rule.host}`))
            return true;
    }
    return false;
}
//# sourceMappingURL=network-proxy-utils.js.map