...
import { startVoiceServer, resolveDesktopVoiceConfig } from '@craft-agent/server-core/voice';
import { createMessagingBootstrap } from '@craft-agent/messaging-gateway';
...
let browserPaneManager = null;
let oauthFlowStore = null;
let moduleSink = null;
let moduleClientResolver = null;
...
function normalizeOriginForCert(urlStr) {
...
async function createInitialWindows(options) {
    if (!windowManager)
        return;
...
app.whenReady().then(async () => {
    ...
    const getMediaTypes = (details) => {
        if (!details || typeof details !== 'object')
            return [];
        const mediaDetails = details;
        if (Array.isArray(mediaDetails.mediaTypes)) {
            return mediaDetails.mediaTypes.filter((mediaType) => typeof mediaType === 'string');
        }
        return typeof mediaDetails.mediaType === 'string'
            ? [mediaDetails.mediaType]
            : [];
    };
    const isAudioOnlyMediaRequest = (permission, details) => {
    ...
    const canUseVoicePermission = (wc, permission, details) => Boolean(wc &&
        getVoiceEnabled() &&
    ...
    try {
        ...
        ipcMain.on('__transport:status', (_event, payload) => {
        ...
        if (!isClientOnly) {
            ...
            const resolveClientId = (wcId) => clientMap.get(wcId);
            ...
            const instance = await bootstrapServer({
                ...
                createHandlerDeps: ({ sessionManager: sm, platform: p, oauthFlowStore: ofs }) => {
                    ...
                    messagingHandle = createMessagingBootstrap({
                        ...
                        getMessagingDir: (wsId) => join(homedir(), '.craft-agent', 'workspaces', wsId, 'messaging'),
                        getLegacyMessagingDir: (wsId) => {
            ...
            try {
                ...
                voiceServer = await startVoiceServer({
                    ...
                    allowedOrigins: [getRendererDevOrigin()].filter((origin) => Boolean(origin)),
            ...
            ipcMain.handle('workspace:remove', async (_event, workspaceId) => {
            ...
            ipcMain.handle('workspace:pinned:set', async (_event, workspaceId, pinned) => {
            ...
            ipcMain.handle('workspace:reorder', async (_event, orderedIds) => {
            ...
            ipcMain.handle('server:invokeOnServer', async (_event, url, token, channel, ...args) => {
            ...
            ipcMain.handle('session:transferToRemoteWorkspace', async (_event, sessionId, targetWorkspaceId, sessionIndex, sessionCount) => {
                ...
                if (!targetWorkspace?.remoteServer)
                    throw new Error(`Workspace ${targetWorkspaceId} has no remote server`);
                if (!sessionManager)
                    throw new Error('Session manager not initialized');
                const sourceWorkspaceLocalId = windowManager?.getWorkspaceForWindow(_event.sender.id);
                if (!sourceWorkspaceLocalId)
                    throw new Error('Unable to resolve source workspace for transfer');
                const sourceWorkspace = getWorkspaceByNameOrId(sourceWorkspaceLocalId);
                if (!sourceWorkspace)
                    throw new Error(`Source workspace ${sourceWorkspaceLocalId} not found`);
                let bundle = null;
                if (sourceWorkspace.remoteServer) {
                    ...
                    if (!sourceClient)
                        throw new Error(sourceError ?? 'Connection failed to source remote server');
                    try {
                ...
                if (!client)
                    throw new Error(error ?? 'Connection failed to target remote server');
                ...
                try {
                    ...
                    const emitProgress = (chunkSent, chunkTotal) => {
                        try {
                            _event.sender.send('transfer:progress', { sessionIndex: idx, sessionCount: count, chunkSent, chunkTotal });
                        }
                    ...
                    return await invokeChunked(client, 'sessions:import', [remoteWorkspaceId, bundle, 'fork'], 1, emitProgress, preparedBundle);
                }
                finally {
                    client.destroy();
                }
            ...
            ipcMain.handle('i18n:changeLanguage', async (_event, lang) => {
            ...
            instance.wsServer.handle(RPC_CHANNELS.settings.SET_SERVER_CONFIG, async (_ctx, config) => {
            ...
            instance.wsServer.handle(RPC_CHANNELS.settings.GET_SERVER_STATUS, async () => {
                ...
                const needsRestart = saved.enabled !== runningServerState.enabled
                    || ((saved.enabled || runningServerState.enabled) && (saved.port !== runningServerState.port
            ...
            if (isInsecureBind) {
                mainLog.warn('[server-mode] WARNING: Listening on a network address without TLS. ' +
        ...
        mainLog.error('Failed to initialize app:', error instanceof Error ? error.message : error, error?.stack);
...
app.on('window-all-closed', () => {
    if (process.env.CRAFT_HEADLESS)
        return; // headless server stays alive
