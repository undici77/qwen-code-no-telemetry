...
import { getDismissedUpdateVersion, clearDismissedUpdateVersion, } from '@craft-agent/shared/config';
...
let updaterCacheDirName = null;
function getAutoUpdateCapability() {
 ...
 */
export function setAutoUpdateEventSink(sink) {
    eventSink = sink;
}
/**
...
autoUpdater.logger = {
    info: (msg) => mainLog.info('[electron-updater]', msg),
    warn: (msg) => mainLog.warn('[electron-updater]', msg),
    error: (msg) => mainLog.error('[electron-updater]', msg),
    debug: (msg) => mainLog.info('[electron-updater:debug]', msg),
};
// ─── Event handlers ───────────────────────────────────────────────────────────
 ...
 */
function checkElectronUpdaterState() {
    try {
 ...
 */
function checkForExistingDownload() {
    try {
        ...
        const downloadFile = files.find(f => f.endsWith('.zip') ||
            f.endsWith('.exe') ||
            f.endsWith('.AppImage') ||
            f.endsWith('.dmg') ||
            f.endsWith('.nupkg'));
