...
import { getPetWindowBounds, setPetWindowBounds, } from '@craft-agent/shared/config/storage';
...
export class WindowManager {
    windows = new Map(); // webContents.id → ManagedWindow
    petWindow = null; // floating desktop-pet window (not a managed workspace window)
    petWorkspaceId = ''; // workspace the pet window subscribes to for activity
    focusedModeWindows = new Set(); // webContents.id of windows in focused mode
    pendingCloseTimeouts = new Map(); // Fallback timeouts for window close
    eventSink = null;
    clientResolver = null;
    keyboardCloseIntents = new Set(); // webContents.id flagged by Cmd/Ctrl+W before close
    keyboardCloseIntentTimeouts = new Map(); // Auto-clear stale keyboard-close intents
    isAppQuitting = false; // Skip layered close interception during app quit
    /**
    ...
    getClientIdForWindow(webContentsId) {
     ...
     */
    forceCloseWindow(webContentsId) {
     ...
     */
    cancelPendingClose(webContentsId) {
        const timeout = this.pendingCloseTimeouts.get(webContentsId);
        if (timeout) {
            clearTimeout(timeout);
            this.pendingCloseTimeouts.delete(webContentsId);
     ...
     */
    closeWindowForWorkspace(workspaceId) {
     ...
     */
    updateWindowWorkspace(webContentsId, workspaceId) {
        const managed = this.windows.get(webContentsId);
        if (managed) {
     ...
     */
    registerWindow(window, workspaceId) {
        const webContentsId = window.webContents.id;
        this.windows.set(webContentsId, { window, workspaceId });
    ...
    setPetWindowIgnoreMouse(ignore) {
     ...
     */
    setPetWindowEnabled(enabled, workspaceId) {
        if (!enabled) {
    ...
    loadPetWindow(window, workspaceId) {
        if (VITE_DEV_SERVER_URL) {
            const params = new URLSearchParams({ workspaceId }).toString();
    ...
    defaultPetPosition(width, height) {
     ...
     */
    setTrafficLightsVisible(webContentsId, visible) {
        if (process.platform !== 'darwin')
            return;
        const managed = this.windows.get(webContentsId);
        if (managed && !managed.window.isDestroyed()) {
