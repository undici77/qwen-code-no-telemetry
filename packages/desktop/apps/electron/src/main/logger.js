...
function ensureMessagingLogDir() {
    mkdirSync(dirname(messagingGatewayLogPath), { recursive: true });
}
function rotateMessagingLogIfNeeded(nextLineBytes) {
    if (!existsSync(messagingGatewayLogPath))
        return;
    try {
        const currentSize = statSync(messagingGatewayLogPath).size;
        if (currentSize + nextLineBytes <= MESSAGING_LOG_MAX_BYTES)
            return;
...
function normalizeLogValue(value, depth = 0) {
    ...
    if (value instanceof Error) {
        ...
        if (cause !== undefined)
            out.cause = normalizeLogValue(cause, depth + 1);
        if (value.stack)
...
function writeMessagingGatewayLog(level, context, message, meta) {
    const entry = {
...
class StructuredMessagingGatewayLogger {
    context;
    constructor(context = {}) {
...
export const messagingGatewayLog = new StructuredMessagingGatewayLogger({
    component: 'root',
});
/**
...
export function getMessagingGatewayLogFilePath() {
