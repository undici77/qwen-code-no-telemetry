 ...
 */
function parseWindowMode(parsed) {
 ...
 */
function parseRightSidebar(parsed) {
...
export function parseDeepLink(url) {
    try {
        ...
        if (host === 'workspace') {
            ...
            if (!workspaceId)
                return null;
            const result = { workspaceId, windowMode, rightSidebar };
 ...
 */
function buildDeepLinkWithoutWindowParam(url) {
    const parsed = new URL(url);
 ...
 */
export async function handleDeepLink(url, windowManager, sink, resolveClientId, preferredClientId) {
