 ...
 */
export function isCompoundRoute(route) {
 ...
 */
export function parseCompoundRoute(route) {
    ...
    if (first === 'sources') {
        ...
        if (validSourceTypes.includes(segments[1])) {
            ...
            const sourceFilter = { kind: 'type', sourceType };
    ...
    if (first === 'automations') {
        ...
        if (validAutomationTypes.includes(segments[1])) {
            const automationType = segments[1];
 ...
 */
export function buildCompoundRoute(parsed) {
 ...
 */
function convertCompoundToViewRoute(compound) {
    // Settings
 ...
 */
export function parseRouteToNavigationState(route, sidebarParam) {
 ...
 */
function convertCompoundToNavigationState(compound) {
    // Settings
    if (compound.navigator === 'settings') {
 ...
 */
function convertParsedRouteToNavigationState(parsed) {
 ...
 */
function navigationStateToCompoundRoute(state) {
    ...
    if (state.navigator === 'skills') {
        return {
            navigator: 'skills',
            details: state.details?.type === 'skill'
    ...
    if (state.navigator === 'skillMarketplace') {
        return {
            navigator: 'skillMarketplace',
            details: state.details?.type === 'marketplaceSkill'
 ...
 */
export function buildRouteFromNavigationState(state) {
 ...
 *   'none' -> { type: 'none' }
 */
export function parseRightSidebarParam(sidebarStr) {
    if (!sidebarStr)
        return undefined;
 ...
 */
export function buildRightSidebarParam(panel) {
