...
import { formatSessionLoadFailure, hasSessionContentHint, shouldShowForegroundMessageLoading, shouldShowMissingSessionState, } from '@/lib/session-load';
import { ensureSessionMessagesLoadedAtom, loadedSessionsAtom, sessionMetaMapAtom, } from '@/atoms/sessions';
import { getSessionTitle } from '@/utils/session';
// Model resolution: connection.defaultModel (no hardcoded defaults)
import { resolveEffectiveConnectionSlug, isSessionConnectionUnavailable, } from '@config/llm-connections';
const MISSING_SESSION_CONFIRMATION_DELAY_MS = 250;
function getConnectionModelIds(connection) {
    return (connection?.models ?? [])
        .map((model) => (typeof model === 'string' ? model : model.id))
        .filter(Boolean);
}
function resolveDisplayModel(sessionModel, connection) {
...
const ChatPage = React.memo(function ChatPage({ sessionId }) {
    const { t } = useTranslation();
    // Diagnostic: mark when component runs
    React.useLayoutEffect(() => {
        rendererPerf.markSessionSwitch(sessionId, 'panel.mounted');
    }, [sessionId]);
    const { activeWorkspaceId, llmConnections, workspaceDefaultLlmConnection, onOptimisticDefaultModelChange, onSendMessage, onOpenFile, onOpenUrl, workspaces, onRespondToPermission, onRespondToCredential, onMarkSessionRead, onMarkSessionUnread, onSetActiveViewingSession, getDraft, hydrateDraftAttachments, onInputChange, onAttachmentsChange, enabledSources, skills, getQwenCapabilitySnapshot, labels, onSessionLabelsChange, enabledModes, sessionStatuses, onSessionSourcesChange, onRenameSession, onFlagSession, onUnflagSession, onArchiveSession, onUnarchiveSession, onSessionStatusChange, onDeleteSession, leadingAction, isCompactMode, sessionListSearchQuery, isSearchModeActive, chatDisplayRef, onChatMatchInfoChange, isFocusedPanel, } = useAppShellContext();
    ...
    const [missingSessionStartedAt, setMissingSessionStartedAt] = React.useState(null);
    const [missingSessionCheckAt, setMissingSessionCheckAt] = React.useState(() => Date.now());
    React.useEffect(() => {
    ...
    const [messageLoadError, setMessageLoadError] = React.useState(null);
    ...
    const sessionLoadedMarkedRef = React.useRef(null);
    React.useLayoutEffect(() => {
    ...
    React.useEffect(() => {
        const handler = (e) => {
            const { sessionId: targetId, text } = e.detail ?? {};
    ...
    React.useEffect(() => {
        const handleAdd = (e) => {
            const { sessionId: targetId, message, optimisticMessageId, } = e.detail ?? {};
            if (targetId !== sessionId || !message)
                return;
        ...
        const handleRemove = (e) => {
            const { sessionId: targetId, messageId, optimisticMessageId, } = e.detail ?? {};
            if (targetId !== sessionId)
                return;
            setQueuedInputMessages((prev) => prev.filter((item) => item.id !== messageId &&
                (!optimisticMessageId || item.id !== optimisticMessageId)));
        };
    ...
    React.useEffect(() => {
        ...
        hydrateDraftAttachments(sessionId).then((atts) => {
            if (!cancelled)
                setAttachmentsValue(atts);
        });
        return () => {
            cancelled = true;
        };
    }, [sessionId, hydrateDraftAttachments]);
    const handleAttachmentsChange = React.useCallback((attachments) => {
        setAttachmentsValue(attachments);
        onAttachmentsChange(sessionId, attachments);
    }, [sessionId, onAttachmentsChange]);
    const handleSendChatMessage = React.useCallback((message, attachments, skillSlugs) => {
    ...
    const handleModelChange = React.useCallback((model, connection) => {
    ...
    const handleConnectionChange = React.useCallback(async (connectionSlug) => {
        try {
    ...
    const connectionUnavailable = React.useMemo(() => isSessionConnectionUnavailable(session?.llmConnection, llmConnections), [session?.llmConnection, llmConnections]);
    // Effective model for this session (session-specific or global fallback)
    const effectiveConnectionSlug = React.useMemo(() => {
        if (connectionUnavailable)
            return session?.llmConnection;
    ...
    const displaySession = React.useMemo(() => {
        if (!session || !qwenCapabilitySnapshot)
            return session;
        ...
        const availableSkills = cachedSkills !== undefined ? cachedSkills : session.availableSkills;
        const availableSkillDetails = cachedSkillDetails !== undefined
            ? cachedSkillDetails
            : session.availableSkillDetails;
        if (availableCommands === session.availableCommands &&
    ...
    const activeWorkspace = React.useMemo(() => workspaces.find((w) => w.id === activeWorkspaceId) || null, [workspaces, activeWorkspaceId]);
    const handleWorkingDirectoryChange = React.useCallback(async (path) => {
        if (!session)
            return;
    ...
    const handleOpenFile = React.useCallback(async (path) => {
    ...
    const dataReadyMarkedRef = React.useRef(null);
    React.useLayoutEffect(() => {
    ...
    const messagesLoading = !messageLoadError &&
        shouldShowForegroundMessageLoading(messagesLoaded, session?.messages?.length, expectedMessageCount, hasExistingConversationHint);
    ...
    const isAsyncOperationOngoing = session?.isAsyncOperationOngoing ||
    ...
    const shouldShowMissingSession = shouldShowMissingSessionState({
        ...
        missingForMs: missingSessionStartedAt === null
            ? 0
    ...
    const handleSessionStatusChange = React.useCallback((state) => {
        onSessionStatusChange(sessionId, state);
    }, [sessionId, onSessionStatusChange]);
    const handleLabelsChange = React.useCallback((newLabels) => {
    ...
    const compactInfoButton = React.useMemo(() => {
        if (!isCompactMode || !sessionMeta)
            return undefined;
        return (_jsx(SessionInfoPopover, { sessionId: sessionId, sessionFolderPath: session?.sessionFolderPath, presentation: "drawer", trigger: _jsx(PanelHeaderCenterButton, { icon: _jsx(Info, { className: "h-4 w-4" }), "aria-label": t('chat.sessionInfo') }) }));
    ...
    const titleBadge = isFlagged ? (_jsx(Flag, { className: "h-3.5 w-3.5 shrink-0 text-info" })) : undefined;
    const messageLoadErrorView = messageLoadError ? (_jsxs("div", { className: "flex-1 flex flex-col items-center justify-center gap-3 px-6 text-center text-muted-foreground", children: [_jsx(AlertCircle, { className: "h-9 w-9 text-destructive/70" }), _jsxs("div", { className: "space-y-1", children: [_jsx("p", { className: "text-sm font-medium text-foreground", children: t('common.errorLoadingContent') }), _jsx("p", { className: "max-w-md text-xs text-muted-foreground break-words", children: messageLoadError })] }), _jsx("button", { type: "button", onClick: retryMessageLoad, className: "mt-1 inline-flex h-8 items-center justify-center rounded-[8px] bg-foreground px-3 text-sm font-medium text-background transition-opacity hover:opacity-90", children: t('common.retry') })] })) : null;
    // Build title menu content for chat sessions using shared SessionMenu
    const titleMenu = React.useMemo(() => sessionMeta ? (_jsx(SessionMenu, { item: sessionMeta, hideShareAction: true, hideMessagingAction: true, hideStatusAction: true, sessionStatuses: sessionStatuses ?? [], labels: labels ?? [], onLabelsChange: handleLabelsChange, onRename: handleRename, onFlag: handleFlag, onUnflag: handleUnflag, onArchive: handleArchive, onUnarchive: handleUnarchive, onMarkUnread: handleMarkUnread, onSessionStatusChange: handleSessionStatusChange, onOpenInNewWindow: handleOpenInNewWindow, onDelete: handleDelete })) : null, [
        sessionMeta,
        sessionStatuses,
        labels,
        handleLabelsChange,
    ...
    if (!session) {
        if (sessionMeta) {
            ...
            return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "h-full flex flex-col", children: [_jsx(PanelHeader, { title: displayTitle, badge: titleBadge, titleMenu: titleMenu, leadingAction: leadingAction, actions: headerActions, isRegeneratingTitle: isAsyncOperationOngoing }), _jsx("div", { className: "flex-1 flex flex-col min-h-0", children: messageLoadErrorView ?? (_jsx(ChatDisplay, { ref: chatDisplayRef, session: skeletonSession, onSendMessage: handleNoopSendMessage, onOpenFile: handleOpenFile, onOpenUrl: handleOpenUrl, currentModel: effectiveModel, onModelChange: handleModelChange, onConnectionChange: handleConnectionChange, pendingPermission: undefined, onRespondToPermission: onRespondToPermission, pendingCredential: undefined, onRespondToCredential: onRespondToCredential, thinkingLevel: sessionOpts.thinkingLevel, onThinkingLevelChange: (level) => setOption('thinkingLevel', level), permissionMode: sessionOpts.permissionMode, onPermissionModeChange: setPermissionMode, enabledModes: enabledModes, inputValue: inputValue, onInputChange: handleInputChange, queuedInputMessages: queuedInputMessages, attachmentsValue: attachmentsValue, onAttachmentsChange: handleAttachmentsChange, sources: enabledSources, skills: skills, sessionStatuses: sessionStatuses, onSessionStatusChange: handleSessionStatusChange, workspaceId: activeWorkspaceId || undefined, onSourcesChange: (slugs) => onSessionSourcesChange?.(sessionId, slugs), workingDirectory: sessionMeta.workingDirectory, onWorkingDirectoryChange: handleWorkingDirectoryChange, messagesLoading: shouldShowForegroundMessageLoading(messagesLoaded, 0, sessionMeta.messageCount, hasSessionContentHint(sessionMeta)), searchQuery: sessionListSearchQuery, isSearchModeActive: isSearchModeActive, onMatchInfoChange: onChatMatchInfoChange, connectionUnavailable: connectionUnavailable, compactMode: !!isCompactMode })) })] }), _jsx(RenameDialog, { open: renameDialogOpen, onOpenChange: setRenameDialogOpen, title: t('chat.renameSession'), value: renameName, onValueChange: setRenameName, onSubmit: handleRenameSubmit, placeholder: t('chat.enterSessionName') })] }));
        }
        if (!shouldShowMissingSession) {
            return (_jsxs("div", { className: "h-full flex flex-col", children: [_jsx(PanelHeader, { title: displayTitle, leadingAction: leadingAction }), _jsx("div", { className: "flex-1" })] }));
        }
        // Session truly doesn't exist
        return (_jsxs("div", { className: "h-full flex flex-col", children: [_jsx(PanelHeader, { title: t('chat.session'), leadingAction: leadingAction }), _jsxs("div", { className: "flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground", children: [_jsx(AlertCircle, { className: "h-10 w-10" }), _jsx("p", { className: "text-sm", children: t('chat.sessionNoLongerExists') })] })] }));
    }
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "h-full flex flex-col", children: [_jsx(PanelHeader, { title: displayTitle, badge: titleBadge, titleMenu: titleMenu, leadingAction: leadingAction, actions: headerActions, isRegeneratingTitle: isAsyncOperationOngoing }), _jsx("div", { className: "flex-1 flex flex-col min-h-0", children: messageLoadError && session.messages.length === 0 ? (messageLoadErrorView) : (_jsx(ChatDisplay, { ref: chatDisplayRef, session: displaySession ?? session, onSendMessage: handleSendChatMessage, onOpenFile: handleOpenFile, onOpenUrl: handleOpenUrl, currentModel: effectiveModel, onModelChange: handleModelChange, onConnectionChange: handleConnectionChange, pendingPermission: pendingPermission, onRespondToPermission: onRespondToPermission, pendingCredential: pendingCredential, onRespondToCredential: onRespondToCredential, thinkingLevel: sessionOpts.thinkingLevel, onThinkingLevelChange: (level) => setOption('thinkingLevel', level), permissionMode: sessionOpts.permissionMode, onPermissionModeChange: setPermissionMode, enabledModes: enabledModes, inputValue: inputValue, onInputChange: handleInputChange, queuedInputMessages: queuedInputMessages, attachmentsValue: attachmentsValue, onAttachmentsChange: handleAttachmentsChange, sources: enabledSources, skills: skills, labels: labels, onLabelsChange: (newLabels) => onSessionLabelsChange?.(sessionId, newLabels), sessionStatuses: sessionStatuses, onSessionStatusChange: handleSessionStatusChange, workspaceId: activeWorkspaceId || undefined, onSourcesChange: (slugs) => onSessionSourcesChange?.(sessionId, slugs), workingDirectory: workingDirectory, onWorkingDirectoryChange: handleWorkingDirectoryChange, sessionFolderPath: session?.sessionFolderPath, messagesLoading: messagesLoading, searchQuery: sessionListSearchQuery, isSearchModeActive: isSearchModeActive, onMatchInfoChange: onChatMatchInfoChange, connectionUnavailable: connectionUnavailable, compactMode: !!isCompactMode })) })] }), _jsx(RenameDialog, { open: renameDialogOpen, onOpenChange: setRenameDialogOpen, title: t('chat.renameSession'), value: renameName, onValueChange: setRenameName, onSubmit: handleRenameSubmit, placeholder: t('chat.enterSessionName') })] }));
});
