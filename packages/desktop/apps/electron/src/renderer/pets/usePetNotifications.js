...
export function usePetNotifications() {
    ...
    const dismiss = useCallback((sessionId) => {
        setBySession((prev) => {
    ...
    useEffect(() => {
        if (!window.electronAPI?.onSessionEvent)
            return;
        const setState = (sessionId, kind, titleKey) => {
            setBySession((prev) => {
                const cur = prev.get(sessionId);
                if (cur && cur.kind === kind)
                    return prev; // no transition -> no churn
                seq += 1;
                const next = new Map(prev);
    ...
    const items = useMemo(() => Array.from(bySession.values()).sort((a, b) => b.seq - a.seq), [bySession]);
    return { items, dismiss, clear };
}
//# sourceMappingURL=usePetNotifications.js.map