...
function buildRow(rowIndex, count, normalMs, lastMs) {
...
const IDLE_SETTLED = IDLE_FRAMES.map((f) => ({
    ...f,
    durationMs: f.durationMs * IDLE_SLOWDOWN,
}));
export const PET_STATE_FRAMES = {
    idle: IDLE_FRAMES,
 ...
 */
export function buildSequence(state, reducedMotion) {
    ...
    const action = [];
    for (let i = 0; i < ACTION_REPEATS; i += 1)
        action.push(...base);
    return {
...
export function backgroundPositionFor(frame) {
...
//# sourceMappingURL=pet-animation.js.map