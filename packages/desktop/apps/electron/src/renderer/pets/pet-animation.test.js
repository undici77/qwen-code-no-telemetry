...
import { PET_BACKGROUND_SIZE, PET_STATE_FRAMES, PET_STATES, backgroundPositionFor, buildSequence, } from './pet-animation';
describe('pet-animation frame tables', () => {
    ...
    it('matches the atlas row/frame contract', () => {
        const counts = Object.fromEntries(PET_STATES.map((s) => [s, PET_STATE_FRAMES[s].length]));
...
describe('backgroundPositionFor', () => {
    it('maps the first cell to the top-left', () => {
        expect(backgroundPositionFor({ rowIndex: 0, columnIndex: 0, durationMs: 1 })).toBe('0% 0%');
    });
    it('maps the last cell to the bottom-right', () => {
        expect(backgroundPositionFor({ rowIndex: 8, columnIndex: 7, durationMs: 1 })).toBe('100% 100%');
    });
...
//# sourceMappingURL=pet-animation.test.js.map