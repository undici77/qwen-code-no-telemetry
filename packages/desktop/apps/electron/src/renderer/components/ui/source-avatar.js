 ...
 */
export function getSourceFallbackIcon(type) {
 ...
 */
function useFaviconFallback(source, primaryIcon) {
    ...
    React.useEffect(() => {
        ...
        let serviceUrl = null;
        if (sourceType === 'mcp' && mcpUrl)
            serviceUrl = mcpUrl;
        else if (sourceType === 'api' && apiBaseUrl)
...
// Component
// ============================================================================
export function SourceAvatar({ source, size = 'md', fluid, showStatus, className }) {
    ...
    const finalIcon = icon.kind === 'fallback' && faviconUrl
        ? { kind: 'file', value: faviconUrl, colorable: false }
        : icon;
    const FallbackIcon = SOURCE_FALLBACKS[source.config.type] ?? Plug;
    const entityIcon = (_jsx(EntityIcon, { icon: finalIcon, size: size, fallbackIcon: FallbackIcon, alt: source.config.name, className: className, containerClassName: fluid ? 'h-full w-full' : undefined }));
    ...
    if (showStatus) {
        ...
        return (_jsxs("span", { className: "relative inline-flex shrink-0", children: [entityIcon, connectionStatus && (_jsx("span", { className: "absolute -bottom-0.5 -right-0.5", children: _jsx(SourceStatusIndicator, { status: connectionStatus, errorMessage: source.config.connectionError, size: statusSize }) }))] }));
    }
    return entityIcon;
}
//# sourceMappingURL=source-avatar.js.map