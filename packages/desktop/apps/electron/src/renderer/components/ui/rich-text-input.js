...
import { findMentionMatches, parseMentions, } from '@/lib/mentions';
import { findSlashCommandMatches, } from '@/lib/slash-command-badges';
import { loadSourceIcon, loadSkillIcon, getSourceIconSync, getSkillIconSync, EMOJI_ICON_PREFIX, } from '@/lib/icon-cache';
// ============================================================================
// Types
// ============================================================================
 ...
 */
export function isEscapeDuringComposition(event, isComposingRefActive) {
    if (event.key !== 'Escape')
        return false;
...
function renderBadgeHTML(type, label, skill, source, workspaceId, tooltip) {
...
function getTextFromElement(element) {
    ...
    if (meaningfulChildren.length === 1 &&
        onlyMeaningfulChild?.tagName === 'BR') {
        return '';
    }
        ...
        if (node.nodeType === Node.TEXT_NODE) {
            ...
            if (el.tagName === 'BR') {
                ...
                if (isTopLevel) {
                    ...
                    const isBrowserWrapper = nextSibling?.getAttribute?.('data-mention') ===
                        'true';
                    if (!isBrowserWrapper) {
                        text += '\n';
                    }
...
// ============================================================================
function getCursorPosition(element, fallback = 0) {
...
// ============================================================================
function textToHTML(text, skills, sources, workspaceId, slashCommandNames = []) {
    if (!text)
        return '';
    ...
    for (const match of matches) {
        ...
        const withMentionText = badgeHtml.replace('data-mention="true"', `data-mention="true" data-mention-text="${match.fullMatch.replace(/"/g, '&quot;')}"`);
...
// ============================================================================
function getBadgeSignature(text, skillSlugs, sourceSlugs, slashCommandNames) {
    const matches = [
        ...findMentionMatches(text, skillSlugs, sourceSlugs),
        ...findSlashCommandMatches(text, slashCommandNames),
    ].sort((a, b) => a.startIndex - b.startIndex);
...
export function shouldSyncRenderedValue(lastValue, nextValue, lastBadgeSignature, nextBadgeSignature) {
    return lastValue !== nextValue || lastBadgeSignature !== nextBadgeSignature;
}
function RotatingPlaceholder({ placeholders, intervalMs = 5000, className, }) {
    ...
    return (_jsx("div", { className: cn('transition-opacity duration-300 ease-in-out', className), style: { opacity }, children: placeholders[currentIndex] }));
}
// ============================================================================
// RichTextInput Component
// ============================================================================
export const RichTextInput = React.forwardRef(({ value, onChange, placeholder, skills = [], sources = [], workspaceId, slashCommandNames = [], disabled = false, className, style, onFocus, onBlur, onKeyDown, onInput, onPaste, onLongTextPaste, ...restProps }, forwardedRef) => {
    ...
    const pendingCursorRef = React.useRef(null);
    ...
    const handleKeyDownInternal = React.useCallback((e) => {
    ...
    const handlePasteInternal = React.useCallback((e) => {
    ...
    React.useEffect(() => {
        ...
        if (isInternalUpdate.current)
            return;
        const nextMentionSignature = getBadgeSignature(safeValue, skillSlugs, sourceSlugs, slashCommandNames);
        if (!shouldSyncRenderedValue(lastValueRef.current, safeValue, lastMentionSignatureRef.current, nextMentionSignature)) {
            return;
        }
    ...
    React.useEffect(() => {
        ...
        const handleSelectionChange = () => {
            ...
            const badges = divRef.current.querySelectorAll('.mention-badge');
            badges.forEach((badge) => {
                // Check if badge is within selection range
                const badgeRange = document.createRange();
                badgeRange.selectNode(badge);
                const isSelected = range.compareBoundaryPoints(Range.START_TO_END, badgeRange) > 0 &&
    ...
    const placeholderArray = React.useMemo(() => {
        if (placeholder == null)
            return [t('chatInput.placeholder.typeMessage')];
        if (Array.isArray(placeholder)) {
            const entries = placeholder.filter((entry) => entry.trim().length > 0);
            return entries.length > 0
                ? entries
                : [t('chatInput.placeholder.typeMessage')];
        }
    ...
    return (_jsxs("div", { className: "relative", children: [_jsx("div", { ref: divRef, contentEditable: !disabled, suppressContentEditableWarning: true, tabIndex: disabled ? -1 : 0, className: cn('outline-none text-sm whitespace-pre-wrap break-words', 'min-h-[1.5em]', disabled && 'opacity-50 cursor-not-allowed', 
                // Make text transparent when showing placeholder (so caret is still visible)
                showPlaceholder && 'text-transparent caret-foreground', className), 
                // Use inline style for line-height to override text-sm's built-in line-height.
                style: { ...style, lineHeight: hasMentions ? 1.45 : 1.25 }, onInput: handleInput, onKeyDown: handleKeyDownInternal, onFocus: handleFocus, onBlur: handleBlur, onPaste: handlePasteInternal, onCompositionStart: handleCompositionStart, onCompositionEnd: handleCompositionEnd, "aria-disabled": disabled, "aria-placeholder": Array.isArray(placeholder) ? placeholder[0] : placeholder, role: "textbox", "aria-multiline": "true", ...restProps }), showPlaceholder && (_jsx(RotatingPlaceholder, { placeholders: placeholderArray, intervalMs: 5000, className: cn('absolute inset-0 text-sm text-muted-foreground pointer-events-none select-none', className) }))] }));
});
RichTextInput.displayName = 'RichTextInput';
//# sourceMappingURL=rich-text-input.js.map