...
import { isEscapeDuringComposition, shouldSyncRenderedValue, } from '../rich-text-input';
describe('isEscapeDuringComposition', () => {
    ...
    it('returns true for Escape when nativeEvent.isComposing is true', () => {
        expect(isEscapeDuringComposition({ key: 'Escape', nativeEvent: { isComposing: true } }, false)).toBe(true);
    });
...
describe('shouldSyncRenderedValue', () => {
    ...
    it('returns true when mention availability changes badge rendering', () => {
        expect(shouldSyncRenderedValue('[skill:pptx] inspect this deck', '[skill:pptx] inspect this deck', '', 'skill:pptx:0')).toBe(true);
    });
    it('returns false when neither text nor badge rendering changed', () => {
        expect(shouldSyncRenderedValue('[skill:pptx] inspect this deck', '[skill:pptx] inspect this deck', 'skill:pptx:0', 'skill:pptx:0')).toBe(false);
    });
});
//# sourceMappingURL=rich-text-input.test.js.map