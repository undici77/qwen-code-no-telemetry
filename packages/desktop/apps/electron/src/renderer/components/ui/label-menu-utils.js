...
export function compareLabelMenuItems(a, b) {
 ...
 */
export function createLabelMenuItems(labels, excludedLabelIds = []) {
 ...
 * 0 = no match
 */
export function segmentScore(part, segment) {
    const lower = part.toLowerCase();
    if (lower.startsWith(segment))
        return 3;
    if (new RegExp(`[\\s\\-_]${segment.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`).test(lower))
        return 2;
...
export function filterItems(items, filter) {
    if (!filter)
        return [...items].sort(compareLabelMenuItems);
 ...
 */
export function filterSessionStatuses(states, filter, getLabel = (state) => state.label) {
    if (!filter)
        return states;
    const segments = filter.toLowerCase().split('/').map(s => s.trim()).filter(Boolean);
    if (segments.length === 0)
...
//# sourceMappingURL=label-menu-utils.js.map