...
const HOST_PATTERN = /^(localhost|\d{1,3}(?:\.\d{1,3}){3}|[\w-]+(?:\.[\w-]+)+)(?::\d+)?(?:[/?#]|$)/i;
function shouldUseBuiltInBrowser(trimmedUrl) {
    return (HOST_PATTERN.test(trimmedUrl) ||
        !EXPLICIT_SCHEME_PATTERN.test(trimmedUrl) ||
        /^https?:\/\//i.test(trimmedUrl));
}
function normalizeExternalUrl(trimmedUrl) {
    ...
    if (EXPLICIT_SCHEME_PATTERN.test(trimmedUrl))
        return trimmedUrl;
 ...
 */
export async function openUrlInBuiltInBrowser(url, { browserPaneApi, isChannelAvailable, openExternal }) {
    ...
    if (isChannelAvailable && !isChannelAvailable(RPC_CHANNELS.browserPane.CREATE)) {
        console.info('[openUrlInBuiltInBrowser] Browser pane channel unavailable, falling back to default browser:', trimmedUrl);
        openExternal(externalUrl);
        return;
    }
    try {
        ...
        console.warn('[openUrlInBuiltInBrowser] Failed to open URL in built-in browser, falling back to default browser:', trimmedUrl, error);
        openExternal(externalUrl);
    }
}
//# sourceMappingURL=open-url-in-built-in-browser.js.map