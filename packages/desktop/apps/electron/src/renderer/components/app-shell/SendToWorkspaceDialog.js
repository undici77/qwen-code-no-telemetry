...
export function SendToWorkspaceDialog({ open, onOpenChange, sessionIds, workspaces, activeWorkspaceId, onTransferComplete, }) {
    ...
    const handleTransfer = useCallback(async () => {
        if (!selectedWorkspaceId || sessionIds.length === 0)
            return;
    ...
    return (_jsx(Dialog, { open: open, onOpenChange: (isOpen) => {
            if (!isTransferring) {
                ...
                if (!isOpen)
                    setSelectedWorkspaceId(null);
        ...
        }, children: _jsxs(DialogContent, { className: "sm:max-w-sm", children: [_jsxs(DialogHeader, { children: [_jsxs(DialogTitle, { className: "flex items-center gap-2", children: [_jsx(Send, { className: "h-4 w-4" }), t("sendToWorkspace.title")] }), _jsx(DialogDescription, { children: t("sendToWorkspace.description", { count }) })] }), _jsx("div", { className: "flex flex-col gap-1 max-h-64 overflow-y-auto py-1", children: remoteWorkspaces.length === 0 ? (_jsx("p", { className: "text-sm text-muted-foreground px-2 py-4 text-center", children: t("sendToWorkspace.noRemoteWorkspaces") })) : (remoteWorkspaces.map(workspace => {
                        ...
                        return (_jsxs("button", { type: "button", disabled: isTransferring || isDisconnected, onClick: () => setSelectedWorkspaceId(workspace.id), className: cn('flex items-center gap-2 w-full px-2 py-2 rounded-md text-left text-sm transition-colors', 'hover:bg-foreground/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring', isSelected && 'bg-foreground/10 ring-1 ring-foreground/15', isDisconnected && 'opacity-50 cursor-not-allowed hover:bg-transparent'), children: [_jsx(CrossfadeAvatar, { src: workspaceIconMap.get(workspace.id), alt: displayName, className: "h-5 w-5 rounded-full ring-1 ring-border/50 shrink-0", fallbackClassName: "bg-muted text-[10px] rounded-full", fallback: getWorkspaceInitial(workspace, t) }), _jsx("span", { className: "flex-1 truncate", children: displayName }), isDisconnected ? (_jsx(CloudOff, { className: "h-3.5 w-3.5 text-muted-foreground/50 shrink-0" })) : (_jsx(Cloud, { className: cn('h-3.5 w-3.5 shrink-0', isChecking ? 'text-muted-foreground/30 animate-pulse' : 'text-muted-foreground') }))] }, workspace.id));
                    })) }), _jsxs(DialogFooter, { children: [_jsx(Button, { variant: "outline", onClick: () => onOpenChange(false), disabled: isTransferring, children: "Cancel" }), _jsx(TransferButton, { onClick: handleTransfer, disabled: !selectedWorkspaceId || isTransferring, isTransferring: isTransferring, progress: overallProgress })] })] }) }));
}
...
function TransferButton({ onClick, disabled, isTransferring, progress }) {
    ...
    return (_jsxs("div", { ref: wrapperRef, className: "relative", children: [_jsx(Button, { onClick: onClick, disabled: disabled, children: isTransferring ? 'Sending...' : 'Send' }), isTransferring && (_jsx("svg", { className: "absolute pointer-events-none", style: { inset: '-3px', width: 'calc(100% + 6px)', height: 'calc(100% + 6px)', overflow: 'visible' }, children: _jsx("rect", { ref: rectRef, x: "1.5", y: "1.5", width: "calc(100% - 3px)", height: "calc(100% - 3px)", rx: "10", ry: "10", fill: "none", stroke: "#8B5CF6", strokeWidth: "2", strokeDasharray: perim > 0 ? `${progress * perim} ${perim}` : '0 999', style: {
                        transition: 'stroke-dasharray 0.2s ease-out',
                        filter: 'drop-shadow(0 0 3px #8B5CF6) drop-shadow(0 0 6px rgba(139,92,246,0.3))',
                    } }) }))] }));
}
//# sourceMappingURL=SendToWorkspaceDialog.js.map