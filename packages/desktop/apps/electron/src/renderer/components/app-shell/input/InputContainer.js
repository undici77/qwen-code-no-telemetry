 ...
 */
export function InputContainer({ structuredInput, onStructuredResponse, textareaRef, compactMode, isProcessing, onAnimatedHeightChange, ...freeFormProps }) {
    const appShellContext = useOptionalAppShellContext();
    const isFocusedPanel = appShellContext?.isFocusedPanel ?? true;
    const mode = structuredInput ? 'structured' : 'freeform';
    ...
    const [freeformHeight, setFreeformHeight] = React.useState(compactMode ? FALLBACK_HEIGHTS['freeform-compact'] : FALLBACK_HEIGHTS.freeform);
    const [structuredHeight, setStructuredHeight] = React.useState(null);
    ...
    const handleFreeformHeightChange = React.useCallback((height) => {
    ...
    const handleFocusChange = React.useCallback((focused) => {
    ...
    const renderContent = (forMeasuring) => {
        if (mode === 'freeform') {
            return (_jsx(FreeFormInput, { ...freeFormProps, compactMode: compactMode, isProcessing: isProcessing, inputRef: forMeasuring ? undefined : textareaRef, onHeightChange: forMeasuring ? undefined : handleFreeformHeightChange, onFocusChange: forMeasuring ? undefined : handleFocusChange, unstyled: true }));
        }
        return (_jsx(StructuredInput, { state: structuredInput, onResponse: forMeasuring ? () => { } : handleStructuredResponse, unstyled: true }));
    };
    return (_jsxs("div", { className: "relative", children: [mode !== 'freeform' && (_jsx("div", { ref: measureRef, className: "absolute top-0 left-0 right-0 invisible pointer-events-none", "aria-hidden": "true", children: _jsx("div", { className: "rounded-[8px] bg-background overflow-hidden", children: renderContent(true) }) })), _jsx(motion.div, { className: cn("input-container relative rounded-[12px] overflow-hidden transition-colors", isFocusedPanel ? "shadow-middle" : "shadow-minimal", "bg-background"), style: { height: heightMotionValue }, children: _jsx(AnimatePresence, { mode: "sync", initial: false, children: _jsx(motion.div, { className: mode === 'freeform' ? "absolute bottom-0 left-0 right-0" : "absolute inset-0", initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 }, transition: { duration: TRANSITION_DURATION, ease: TRANSITION_EASE }, children: renderContent(false) }, contentKey) }) })] }));
}
