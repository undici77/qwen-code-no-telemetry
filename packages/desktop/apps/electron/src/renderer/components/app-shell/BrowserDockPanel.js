...
import { activeBrowserInstanceIdAtom, browserInstancesAtom, removeBrowserInstanceAtom, setBrowserInstancesAtom, updateBrowserInstanceAtom, } from '@/atoms/browser-pane';
...
export function BrowserDockPanel({ expandedLeft, autoHideKey = null, isCompact = false, }) {
    ...
    const dockedInstance = React.useMemo(() => {
        return browserInstances.find((instance) => instance.presentation === 'docked' && instance.isVisible) ?? null;
    }, [browserInstances]);
    const panelRef = React.useRef(null);
    const viewportRef = React.useRef(null);
    const lastBoundsKeyRef = React.useRef(null);
    const lastBoundsInstanceIdRef = React.useRef(null);
    const autoHideKeyRef = React.useRef(autoHideKey);
    ...
    React.useEffect(() => {
        ...
        if (previousKey === autoHideKey || !dockedInstance?.isVisible)
            return;
        void window.electronAPI?.browserPane
            ?.hide(dockedInstance.id)
            .catch((error) => {
    ...
    React.useLayoutEffect(() => {
        ...
        const browserPaneApi = window.electronAPI?.browserPane;
        if (!browserPaneApi?.dock)
            return;
        if (fullscreenOverlayOpen) {
            const suspendedBoundsKey = 'suspended';
            if (lastBoundsKeyRef.current === suspendedBoundsKey)
                return;
    ...
    if (!dockedInstance || fullscreenOverlayOpen)
        return null;
    const expandLabel = expanded ? 'Restore panel width' : 'Expand panel';
    const tooltipLabel = hoveredAction === 'expand'
        ? expandLabel
        : hoveredAction === 'close'
            ? 'Close side panel'
            : null;
    return (_jsxs("div", { ref: panelRef, className: cn('shrink-0 overflow-hidden bg-background shadow-middle', 'border-l border-foreground/10', expanded ? 'absolute z-fullscreen' : 'relative z-panel'), style: expanded
            ? {
                top: 0,
                bottom: 0,
            ...
            : {
                width: DOCK_WIDTH,
                borderBottomRightRadius: RADIUS_EDGE,
            }, children: [_jsxs("div", { className: cn('relative flex items-center justify-between gap-2 px-3', 'border-b border-foreground/10 bg-background/95'), style: { height: DOCK_HEADER_HEIGHT }, children: [_jsxs("div", { className: "flex min-w-0 flex-1 items-center gap-2", children: [_jsx("span", { className: cn('flex h-3.5 w-3.5 shrink-0 items-center justify-center', 'transition-opacity duration-150', dockedInstance.isLoading ? 'opacity-100' : 'opacity-0'), "aria-hidden": !dockedInstance.isLoading, children: _jsx(Spinner, { className: "text-[10px] text-foreground/60" }) }), _jsx("div", { className: "min-w-0 flex-1 truncate text-sm font-medium text-foreground/80", children: dockedInstance.title || 'Browser' })] }), _jsxs("div", { className: "relative flex shrink-0 items-center gap-1", children: [tooltipLabel && (_jsx("div", { className: "pointer-events-none absolute right-full top-1/2 mr-2 -translate-y-1/2 whitespace-nowrap rounded-[6px] bg-popover px-2 py-1 text-xs text-popover-foreground shadow-modal-small", children: tooltipLabel })), _jsx(HeaderIconButton, { icon: expanded ? (_jsx(Minimize2, { className: "h-3.5 w-3.5" })) : (_jsx(Maximize2, { className: "h-3.5 w-3.5" })), "aria-label": expandLabel, onMouseEnter: () => setHoveredAction('expand'), onMouseLeave: () => setHoveredAction(null), onFocus: () => setHoveredAction('expand'), onBlur: () => setHoveredAction(null), onClick: handleToggleExpanded }), _jsx(HeaderIconButton, { icon: _jsx(X, { className: "h-3.5 w-3.5" }), "aria-label": "Close side panel", onMouseEnter: () => setHoveredAction('close'), onMouseLeave: () => setHoveredAction(null), onFocus: () => setHoveredAction('close'), onBlur: () => setHoveredAction(null), onClick: handleClose })] }), dockedInstance.isLoading && (_jsx("div", { className: "absolute inset-x-0 bottom-0 h-[2px] overflow-hidden", role: "progressbar", "aria-label": "Loading page", children: _jsx("div", { className: "h-full w-full animate-shimmer-loading", style: {
                                background: 'linear-gradient(90deg, transparent 0%, var(--accent) 50%, transparent 100%)',
                            } }) }))] }), _jsx("div", { ref: viewportRef, className: "absolute inset-x-0 bottom-0 overflow-hidden", style: {
                    top: DOCK_HEADER_HEIGHT,
                    borderBottomRightRadius: RADIUS_EDGE,
                } })] }));
}
//# sourceMappingURL=BrowserDockPanel.js.map