...
import { openUrlInBuiltInBrowser, } from '../open-url-in-built-in-browser';
function makeBrowserPaneApi(overrides = {}) {
    return {
...
describe('openUrlInBuiltInBrowser', () => {
    it('opens http(s) URLs in the built-in browser via create, navigate and focus', async () => {
        ...
        expect(browserPaneApi.navigate).toHaveBeenCalledWith('built-in-browser', 'https://github.com/QwenLM/qwen-code');
    ...
    it('opens host-like URLs in the built-in browser', async () => {
        ...
        expect(browserPaneApi.navigate).toHaveBeenCalledWith('built-in-browser', 'example.com:8443/docs');
        expect(openExternal).not.toHaveBeenCalled();
    });
    it('opens host-like URLs with a fragment in the built-in browser', async () => {
        ...
        expect(browserPaneApi.navigate).toHaveBeenCalledWith('built-in-browser', 'localhost:3000#docs');
        expect(openExternal).not.toHaveBeenCalled();
    });
    it('opens bare host-like URLs with a query in the built-in browser', async () => {
        ...
        expect(browserPaneApi.navigate).toHaveBeenCalledWith('built-in-browser', 'example.com?q=1');
        expect(openExternal).not.toHaveBeenCalled();
    });
    ...
    it('falls back to the system browser when the browser pane API is missing', async () => {
        ...
        expect(openExternal).toHaveBeenCalledTimes(1);
        expect(openExternal).toHaveBeenCalledWith('https://duckduckgo.com/?q=search%20%EF%BF%BD%20term');
    });
    it('falls back to the system browser when browser-pane channels are unavailable', async () => {
        ...
        expect(infoSpy).toHaveBeenCalledWith('[openUrlInBuiltInBrowser] Browser pane channel unavailable, falling back to default browser:', '127.0.0.1:3000/docs');
        expect(openExternal).toHaveBeenCalledTimes(1);
    ...
    it('searches scheme-less free text in the built-in browser', async () => {
        ...
        expect(browserPaneApi.navigate).toHaveBeenCalledWith('built-in-browser', 'qwen code docs');
        expect(openExternal).not.toHaveBeenCalled();
    });
    it.each([
    ...
    ])('searches invalid host-like input when falling back: %s', async (url, query) => {
        const openExternal = mock(() => { });
        await openUrlInBuiltInBrowser(url, { openExternal });
        expect(openExternal).toHaveBeenCalledWith(`https://duckduckgo.com/?q=${query}`);
    });
    it('falls back to the system browser when create fails', async () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith('[openUrlInBuiltInBrowser] Failed to open URL in built-in browser, falling back to default browser:', 'localhost:3000/docs', expect.any(Error));
        expect(openExternal).toHaveBeenCalledTimes(1);
...
//# sourceMappingURL=open-url-in-built-in-browser.test.js.map