...
import { closePanelAtom, focusedPanelIdAtom, } from '@/atoms/panel-stack';
...
export function PanelSlot({ entry, isOnly, isFocusedPanel, isSidebarAndNavigatorHidden, isAtLeftEdge, isAtRightEdge, proportion, sash, isCompact, }) {
    ...
    const closeButton = useMemo(() => (_jsx(PanelHeaderCenterButton, { icon: _jsx(X, { className: "h-4 w-4" }), onClick: handleClose, tooltip: t('common.close') })), [handleClose, t]);
    ...
    const backButton = useMemo(() => {
        if (!isCompact)
            return undefined;
        return (_jsx(PanelHeaderCenterButton, { icon: _jsx(ChevronLeft, { className: "h-4 w-4" }), onClick: handleClose, tooltip: t('common.backToList') }));
    ...
    return (_jsxs(_Fragment, { children: [sash, _jsx("div", { onPointerDown: handlePointerDown, "data-panel-role": "content", "data-compact": isCompact || undefined, className: cn('h-full overflow-hidden relative @container/panel', !isOnly && isFocusedPanel
                    ? 'shadow-panel-focused z-[1]'
                    : 'shadow-middle z-0', 'bg-foreground-2'), style: {
                ...
                }, children: _jsx("div", { className: "h-full flex flex-col", children: _jsx(AppShellProvider, { value: contextOverride, children: _jsx(MainContentPanel, { navStateOverride: navState, isSidebarAndNavigatorHidden: isSidebarAndNavigatorHidden || isAtLeftEdge }) }) }) })] }));
}
//# sourceMappingURL=PanelSlot.js.map