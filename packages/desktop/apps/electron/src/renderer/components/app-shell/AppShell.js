...
import { Archive, Settings, ChevronRight, ChevronDown, MoreHorizontal, RotateCw, Flag, ListFilter, Tag, Check, X, Search, Plus, Trash2, Zap, Inbox, Calendar, Layers, ListTodo, Clock, Radio, Bot, Info, Store, } from 'lucide-react';
...
import { Collapsible, CollapsibleTrigger, AnimatedCollapsibleContent, springTransition as collapsibleSpring, } from '@/components/ui/collapsible';
...
import { areSessionMetaListsEquivalent, compareSessionsByActivityDesc, getWorkspaceSessionMetas, mergeStableSessionMetaList, sessionMetaMapAtom, sendToWorkspaceAtom, workspaceSessionMetaCacheAtom, workspaceSessionsAtom, } from '@/atoms/sessions';
...
import { createLabelMenuItems, filterItems as filterLabelMenuItems, } from '@/components/ui/label-menu-utils';
...
import { getQwenCapabilityCacheKey, getWorkspaceSkillsCacheKey, providerSkillsFromQwenCapabilities, qwenCapabilitiesFromSkills, } from '@/lib/qwen-capability-cache';
import { useNavigation, useNavigationState, isSessionsNavigation, isSourcesNavigation, isSettingsNavigation, isSkillsNavigation, isSkillMarketplaceNavigation, isAutomationsNavigation, } from '@/contexts/NavigationContext';
import { SourcesListPanel } from './SourcesListPanel';
import { SkillsListPanel } from './SkillsListPanel';
import { SkillMarketplacePanel } from './SkillMarketplacePanel';
import { AutomationsListPanel } from '../automations/AutomationsListPanel';
import { APP_EVENTS, AGENT_EVENTS, AUTOMATION_TYPE_TO_FILTER_KIND, } from '../automations/types';
...
import { PANEL_GAP, PANEL_EDGE_INSET, PANEL_SASH_HALF_HIT_WIDTH, PANEL_SASH_HIT_WIDTH, PANEL_SASH_LINE_WIDTH, PANEL_STACK_VERTICAL_OVERFLOW, RADIUS_EDGE, RADIUS_INNER, } from './panel-constants';
...
const EMPTY_SKILLS = [];
const EMPTY_SESSION_ATOM = atom(null);
function SidebarSessionSearch({ workspaces, workspaceSessions, activeWorkspaceId, selectedSessionId, onSelectSession, onRevealSession, }) {
    const { t } = useTranslation();
    const [open, setOpen] = React.useState(false);
    const [query, setQuery] = React.useState('');
    const inputRef = React.useRef(null);
    const normalizedQuery = query.trim().toLocaleLowerCase();
    const workspacesById = React.useMemo(() => new Map(workspaces.map((workspace) => [workspace.id, workspace])), [workspaces]);
    const results = React.useMemo(() => {
        ...
        for (const [workspaceId, sessions] of workspaceSessions) {
            ...
            for (const session of sessions) {
                if (session.hidden || session.isArchived)
                    continue;
    ...
    const handleSelectResult = React.useCallback((result) => {
    ...
    return (_jsxs(Dialog, { open: open, onOpenChange: handleOpenChange, children: [_jsx(DialogTrigger, { asChild: true, children: _jsxs("button", { type: "button", className: cn('group flex w-full items-center gap-2 rounded-[6px] px-2 py-[5px]', 'text-[13px] font-normal select-none outline-none titlebar-no-drag', 'hover:bg-sidebar-hover data-[state=open]:bg-sidebar-hover', 'focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring'), children: [_jsx(Search, { className: "h-3.5 w-3.5 shrink-0", style: {
                                color: 'color-mix(in oklch, var(--foreground) 60%, transparent)',
                            } }), _jsx("span", { className: "min-w-0 flex-1 truncate text-left", children: t('sidebar.search') })] }) }), _jsxs(DialogContent, { showCloseButton: false, overlayClassName: "bg-black/25", className: cn('titlebar-no-drag w-[440px] max-w-[calc(100vw-32px)] overflow-hidden p-0 gap-0', 'border border-foreground/10 bg-popover/92 text-popover-foreground shadow-modal-small backdrop-blur-xl'), onOpenAutoFocus: (event) => {
                    event.preventDefault();
                    inputRef.current?.focus();
                }, children: [_jsx(DialogTitle, { className: "sr-only", children: t('sidebar.search') }), _jsx("div", { className: "border-b border-foreground/8 px-4 py-3", children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Search, { className: "h-4 w-4 shrink-0 text-muted-foreground" }), _jsx("input", { ref: inputRef, value: query, onChange: (event) => setQuery(event.target.value), placeholder: t('session.searchSessionsPlaceholder'), className: "h-7 min-w-0 flex-1 bg-transparent text-[14px] font-medium text-foreground outline-none placeholder:text-muted-foreground" })] }) }), _jsxs("div", { className: "px-2 py-2", children: [_jsx("div", { className: "px-2 pb-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground/70", children: normalizedQuery
                                    ? t('session.results', { count: results.length })
                                    : t('session.recentSessions') }), _jsx("div", { className: "max-h-[360px] overflow-y-auto pr-1", children: results.length > 0 ? (_jsx("div", { className: "grid gap-0.5", children: results.map((result) => {
                                        const title = getSessionTitle(result.session);
                                        const isActive = result.workspace.id === activeWorkspaceId &&
                                            result.session.id === selectedSessionId;
                                        return (_jsxs("button", { type: "button", onClick: () => handleSelectResult(result), className: cn('grid h-9 min-w-0 grid-cols-[1rem_minmax(0,1fr)_auto] items-center gap-2 rounded-[7px] px-2 text-left', 'text-[13px] text-foreground/86 transition-colors hover:bg-foreground/[0.055]', 'focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring', isActive && 'bg-foreground/[0.07] text-foreground'), children: [_jsx("span", { "aria-hidden": "true", className: cn('h-2.5 w-2.5 rounded-full border border-foreground/35', result.session.isProcessing &&
                                                        'border-accent/80 bg-accent/20') }), _jsx("span", { className: "min-w-0 truncate font-medium", children: title }), _jsx("span", { className: "min-w-0 max-w-[8rem] truncate text-[12px] text-muted-foreground", children: result.workspaceName })] }, `${result.workspace.id}:${result.session.id}`));
                                    }) })) : (_jsx("div", { className: "flex h-24 items-center justify-center rounded-[8px] text-[13px] font-medium text-muted-foreground", children: t('session.noSessionsFound') })) })] })] })] }));
}
...
function AltExcludeTooltip({ show, children, }) {
    if (!show)
        return children;
    return (_jsxs(Tooltip, { delayDuration: 0, children: [_jsx(TooltipTrigger, { asChild: true, children: children }), _jsx(TooltipContent, { side: "right", className: "text-xs", children: altClickTooltipLabel })] }));
}
/**
 ...
 */
function FilterModeBadge({ mode }) {
    return (_jsx("span", { className: cn('flex items-center justify-center h-5 w-5 rounded-[4px] -mr-1', mode === 'include'
            ? 'bg-background text-foreground shadow-minimal'
            : 'bg-destructive/10 text-destructive shadow-tinted'), style: mode === 'exclude'
            ? {
                '--shadow-color': 'var(--destructive-rgb)',
            }
            : undefined, children: mode === 'include' ? (_jsx(Check, { className: "!h-2.5 !w-2.5" })) : (_jsx(X, { className: "!h-2.5 !w-2.5" })) }));
}
/**
 ...
 */
function FilterModeSubMenuItems({ mode, onChangeMode, onRemove, }) {
    const { t } = useTranslation();
    return (_jsxs(_Fragment, { children: [_jsxs(StyledDropdownMenuItem, { onClick: (e) => {
                    e.preventDefault();
                    onChangeMode('include');
                }, className: cn(mode === 'include' && 'bg-foreground/[0.03]'), children: [_jsx(Check, { className: "h-3.5 w-3.5 shrink-0" }), _jsx("span", { className: "flex-1", children: t('filter.include') })] }), _jsxs(StyledDropdownMenuItem, { onClick: (e) => {
                    e.preventDefault();
                    onChangeMode('exclude');
                }, className: cn(mode === 'exclude' && 'bg-foreground/[0.03]'), children: [_jsx(X, { className: "h-3.5 w-3.5 shrink-0" }), _jsx("span", { className: "flex-1", children: t('filter.exclude') })] }), _jsx(StyledDropdownMenuSeparator, {}), _jsxs(StyledDropdownMenuItem, { onClick: (e) => {
                    e.preventDefault();
                    onRemove();
                }, children: [_jsx(Trash2, { className: "h-3.5 w-3.5 shrink-0" }), _jsx("span", { className: "flex-1", children: t('common.clear') })] })] }));
}
/**
 * FilterMenuRow - Consistent layout for filter menu items.
 * Enforces: [icon 14px box] [label flex] [accessory 12px box]
 */
function FilterMenuRow({ icon, label, accessory, iconClassName, iconStyle, noIconContainer, }) {
    return (_jsxs(_Fragment, { children: [noIconContainer ? (
            // Wrapper for color inheritance. Clone icon to add bare prop (removes EntityIcon container).
            _jsx("span", { style: iconStyle, children: React.isValidElement(icon)
                    ? React.cloneElement(icon, { bare: true })
                    : icon })) : (_jsx("span", { className: cn('h-3.5 w-3.5 flex items-center justify-center shrink-0', iconClassName), style: iconStyle, children: icon })), _jsx("span", { className: "flex-1", children: label }), _jsx("span", { className: "shrink-0", children: accessory })] }));
}
/**
 ...
 */
function FilterLabelItems({ labels, labelFilter, setLabelFilter, pinnedLabelId, altHeld, }) {
    /** Toggle a label filter: if active → remove, if inactive → add as 'include' (or 'exclude' with Alt) */
    const toggleLabel = (id, altKey = false) => {
        setLabelFilter((prev) => {
            const next = new Map(prev);
            if (next.has(id))
                next.delete(id);
            else
                next.set(id, altKey ? 'exclude' : 'include');
            return next;
        });
    };
    /** Build callbacks for changing/removing a label's filter mode */
    const makeModeCallbacks = (id) => ({
    ...
    return (_jsx(_Fragment, { children: labels.map((label) => {
            ...
            if (hasChildren) {
                ...
                return (_jsxs(DropdownMenuSub, { children: [_jsx(StyledDropdownMenuSubTrigger, { children: _jsx(FilterMenuRow, { icon: _jsx(LabelIcon, { label: label, size: "lg", hasChildren: true }), label: label.name, accessory: showIndicator ? (_jsx(Check, { className: "h-3 w-3 text-muted-foreground" })) : undefined }) }), _jsx(StyledDropdownMenuSubContent, { minWidth: "min-w-[160px]", children: isActive ? (
                            // Active group: group title as nested sub-trigger for mode options, then children
                            _jsxs(_Fragment, { children: [_jsxs(DropdownMenuSub, { children: [_jsx(StyledDropdownMenuSubTrigger, { onClick: (e) => {
                                                    e.preventDefault();
                                                    toggleLabel(label.id, e.altKey);
                                                }, children: _jsx(FilterMenuRow, { icon: _jsx(LabelIcon, { label: label, size: "lg", hasChildren: true }), label: label.name, accessory: _jsx(FilterModeBadge, { mode: mode }) }) }), _jsx(StyledDropdownMenuSubContent, { minWidth: "min-w-[140px]", children: _jsx(FilterModeSubMenuItems, { mode: mode, ...makeModeCallbacks(label.id) }) })] }), _jsx(StyledDropdownMenuSeparator, {}), _jsx(FilterLabelItems, { labels: label.children, labelFilter: labelFilter, setLabelFilter: setLabelFilter, pinnedLabelId: pinnedLabelId, altHeld: altHeld })] })) : (
                            // Inactive group: self-toggle item, then children
                            _jsxs(_Fragment, { children: [_jsx(AltExcludeTooltip, { show: !!altHeld && !isPinned, children: _jsx(StyledDropdownMenuItem, { disabled: isPinned, onClick: (e) => {
                                                if (isPinned)
                                                    return;
                                                e.preventDefault();
                                                toggleLabel(label.id, e.altKey);
                                            }, children: _jsx(FilterMenuRow, { icon: _jsx(LabelIcon, { label: label, size: "lg", hasChildren: true }), label: label.name, accessory: isPinned ? (_jsx(Check, { className: "h-3 w-3 text-muted-foreground" })) : undefined }) }) }), _jsx(StyledDropdownMenuSeparator, {}), _jsx(FilterLabelItems, { labels: label.children, labelFilter: labelFilter, setLabelFilter: setLabelFilter, pinnedLabelId: pinnedLabelId, altHeld: altHeld })] })) })] }, label.id));
            }
            // --- Active leaf label → DropdownMenuSub with mode options ---
            if (isActive) {
                return (_jsxs(DropdownMenuSub, { children: [_jsx(StyledDropdownMenuSubTrigger, { onClick: (e) => {
                                e.preventDefault();
                                toggleLabel(label.id, e.altKey);
                            }, children: _jsx(FilterMenuRow, { icon: _jsx(LabelIcon, { label: label, size: "lg" }), label: label.name, accessory: _jsx(FilterModeBadge, { mode: mode }) }) }), _jsx(StyledDropdownMenuSubContent, { minWidth: "min-w-[140px]", children: _jsx(FilterModeSubMenuItems, { mode: mode, ...makeModeCallbacks(label.id) }) })] }, label.id));
            }
            // --- Inactive / pinned leaf label → simple toggleable item ---
            return (_jsx(AltExcludeTooltip, { show: !!altHeld && !isPinned, children: _jsx(StyledDropdownMenuItem, { disabled: isPinned, onClick: (e) => {
                        if (isPinned)
                            return;
                        e.preventDefault();
                        toggleLabel(label.id, e.altKey);
                    }, children: _jsx(FilterMenuRow, { icon: _jsx(LabelIcon, { label: label, size: "lg" }), label: label.name, accessory: isPinned ? (_jsx(Check, { className: "h-3 w-3 text-muted-foreground" })) : undefined }) }) }, label.id));
        }) }));
}
/**
...
export function AppShell(props) {
    ...
    return (_jsx(EscapeInterruptProvider, { children: _jsx(AppShellContent, { ...props }) }));
}
/**
 ...
 */
function AppShellContent({ contextValue, defaultLayout = [20, 32, 48], defaultCollapsed = false, menuNewChatTrigger, isFocusedMode = false, isSessionListLoading = false, onProjectSessionSnapshotsReadyChange, }) {
    ...
    const { workspaces, activeWorkspaceId, llmConnections, workspaceDefaultLlmConnection, sessionOptions, onSelectWorkspace, onRefreshWorkspaces, onDeleteSession, onFlagSession, onUnflagSession, onArchiveSession, onUnarchiveSession, onMarkSessionRead, onMarkSessionUnread, onSessionStatusChange, onRenameSession, onOpenSettings, onOpenKeyboardShortcuts, onOpenStoredUserPreferences, onReset, onSendMessage, openNewChat, pendingPermissions, } = contextValue;
    const { t } = useTranslation();
    ...
    const [projectDraftTargetWorkspaceId, setProjectDraftTargetWorkspaceId] = React.useState(null);
    ...
    const [sidebarHandleY, setSidebarHandleY] = React.useState(null);
    const [sessionListHandleY, setSessionListHandleY] = React.useState(null);
    const resizeHandleRef = React.useRef(null);
    const sessionListHandleRef = React.useRef(null);
    ...
    const navigateToSessionInPanel = useCallback((sessionId) => {
    ...
    const shouldCollapseProjectDraftNavigator = collapseSessionNavigatorForProjectDraft &&
        isSessionsNavigation(navState) &&
        !sessionsContext?.sessionId;
    const isAllSessionsNavigatorHidden = isSessionsNavigation(navState) &&
        sessionsContext?.filter.kind === 'allSessions';
    const isSessionNavigatorCollapsed = !isAutoCompact &&
        isSessionsNavigation(navState) &&
    ...
    const sourceFilter = isSourcesNavigation(navState)
        ? (navState.filter ?? null)
        : null;
    // Derive automation filter from navigation state (only when in automations navigator)
    const automationFilter = isAutomationsNavigation(navState)
        ? (navState.filter ?? null)
        : null;
    ...
    const [viewFiltersMap, setViewFiltersMap] = React.useState(() => {
        ...
        if (!saved.allSessions) {
            const oldStatuses = storage.get(storage.KEYS.listFilter, []);
            const oldLabels = storage.get(storage.KEYS.labelFilter, []);
            if (oldStatuses.length > 0 || oldLabels.length > 0) {
                const statuses = {};
                for (const id of oldStatuses)
                    statuses[id] = 'include';
                const labels = {};
                for (const id of oldLabels)
                    labels[id] = 'include';
    ...
    const setListFilter = useCallback((updater) => {
        if (!FEATURE_FLAGS.sessionLabelsUi)
            return;
        setViewFiltersMap((prev) => {
            if (!sessionFilterKey)
                return prev;
            const current = new Map(Object.entries(prev[sessionFilterKey]?.statuses ?? {}));
    ...
    const setLabelFilter = useCallback((updater) => {
        setViewFiltersMap((prev) => {
            if (!sessionFilterKey)
                return prev;
            const current = new Map(Object.entries(prev[sessionFilterKey]?.labels ?? {}));
            const next = typeof updater === 'function' ? updater(current) : updater;
            return {
                ...prev,
                [sessionFilterKey]: {
    ...
    const chatGroupingMode = viewFiltersMap[sessionFilterKey ?? '']?.groupingMode ?? 'none';
    const setChatGroupingMode = useCallback((mode) => {
        setViewFiltersMap((prev) => {
            if (!sessionFilterKey)
                return prev;
    ...
    const [chatMatchInfo, setChatMatchInfo] = React.useState({ sessionId: null, count: 0, index: 0 });
    ...
    const handleChatMatchInfoChange = React.useCallback((info) => {
    ...
    const [expandedFolders, setExpandedFolders] = React.useState(() => {
        const saved = storage.get(storage.KEYS.expandedFolders, []);
        return new Set(saved);
    });
    const [focusedSidebarItemId, setFocusedSidebarItemId] = React.useState(null);
    const sidebarItemRefs = React.useRef(new Map());
    ...
    const [collapsedItems, setCollapsedItems] = React.useState(() => new Set(['nav:automations']));
    const isExpanded = React.useCallback((id) => !collapsedItems.has(id), [collapsedItems]);
    ...
    const activeSessionMeta = session.selected
        ...
        : undefined;
    const activeSessionBelongsToActiveWorkspace = Boolean(activeSessionMeta &&
        activeWorkspaceId &&
    ...
    const scopedActiveSessionMeta = activeSessionBelongsToActiveWorkspace
        ? activeSessionMeta
        : undefined;
    const activeSessionWorkingDirectory = scopedActiveSessionMeta?.workingDirectory;
    const [workspaceSkillWorkingDirectory, setWorkspaceSkillWorkingDirectory] = React.useState(undefined);
    const activeSkillsWorkingDirectory = activeSessionWorkingDirectory ??
    ...
    const [installingMarketplaceSkillIds, setInstallingMarketplaceSkillIds] = React.useState(() => new Set());
    const skillsRequestIdRef = React.useRef(0);
    const activeSkillsScopeKey = getWorkspaceSkillsCacheKey(activeWorkspaceId, activeSkillsWorkingDirectory);
    ...
    const handleTransferComplete = useCallback((targetWorkspaceId, _newSessionIds) => {
        onSelectWorkspace(targetWorkspaceId);
    }, [onSelectWorkspace]);
    const { automations, automationTestResults, automationPendingDelete, pendingDeleteAutomation, setAutomationPendingDelete, handleTestAutomation, handleToggleAutomation, handleDuplicateAutomation, handleDeleteAutomation, confirmDeleteAutomation, getAutomationHistory, handleReplayAutomation, } = useAutomations(activeWorkspaceId);
    // Whether local MCP servers are enabled (affects stdio source status)
    const [localMcpEnabled, setLocalMcpEnabled] = React.useState(true);
    // Enabled permission modes for Shift+Tab cycling (min 2 modes)
    const [enabledModes, setEnabledModes] = React.useState([
    ...
    React.useEffect(() => {
        ...
        window.electronAPI
            ...
            if (settings) {
                ...
                setWorkspaceSkillWorkingDirectory(settings.workingDirectory ?? activeWorkspace?.rootPath);
                // Load cyclablePermissionModes from workspace settings
                if (settings.cyclablePermissionModes &&
    ...
    React.useEffect(() => {
        ...
        if (previousWorkspaceId !== activeWorkspaceId) {
            const newViewFilters = storage.get(storage.KEYS.viewFilters, {}, activeWorkspaceId);
            setViewFiltersMap(newViewFilters);
            const newExpandedFolders = storage.get(storage.KEYS.expandedFolders, [], activeWorkspaceId);
    ...
    const handleSessionSourcesChange = React.useCallback(async (sessionId, sourceSlugs) => {
        try {
    ...
    const handleSessionLabelsChange = React.useCallback(async (sessionId, labels) => {
        try {
            await window.electronAPI.sessionCommand(sessionId, {
                type: 'setLabels',
                labels,
    ...
    const [sessionStatuses, setSessionStatuses] = React.useState([]);
    ...
    const [optimisticStatusOrder, setOptimisticStatusOrder] = React.useState(null);
    ...
    const effectiveSessionStatuses = React.useMemo(() => {
        ...
        for (const state of sessionStatuses) {
            if (!optimisticStatusOrder.includes(state.id))
                reordered.push(state);
        }
        return reordered;
    ...
    const displayLabelConfigs = useMemo(() => FEATURE_FLAGS.sessionLabelsUi ? sortLabelsForDisplay(labelConfigs) : [], [labelConfigs]);
    ...
    const flatLabelMenuItems = useMemo(() => createLabelMenuItems(displayLabelConfigs), [displayLabelConfigs]);
    ...
    const filterDropdownListRef = React.useRef(null);
    const filterDropdownInputRef = React.useRef(null);
    ...
    React.useEffect(() => {
        if (!filterDropdownListRef.current)
            return;
    ...
    const handleSourceSelect = React.useCallback((source) => {
        if (!activeWorkspaceId)
            return;
    ...
    const handleAutomationSelect = React.useCallback((automationId) => {
    ...
    const focusChatInputForSession = useCallback((targetSessionId) => {
        if (!targetSessionId)
            return;
    ...
    useAction('chat.cyclePermissionMode', () => {
        if (effectiveSessionId) {
            ...
            const currentMode = currentOptions?.permissionMode ?? contextValue.globalPermissionMode;
    ...
    const firstActiveWorkspaceSessionMeta = React.useMemo(() => {
        if (!activeWorkspaceId)
            return undefined;
        const orderedWorkspaceMetas = getWorkspaceSessionMetas(workspaceSessions, activeWorkspaceId);
        const liveWorkspaceMetas = Array.from(sessionMetaMap.values()).filter((s) => s.workspaceId === activeWorkspaceId ||
            (remoteWorkspaceId && s.workspaceId === remoteWorkspaceId));
        const workspaceMetas = liveWorkspaceMetas.length === 0 && orderedWorkspaceMetas.length > 0
    ...
    const hasPendingPrompt = React.useCallback((sessionId) => {
    ...
    const skillDiscoverySessionMeta = scopedActiveSessionMeta ?? firstActiveWorkspaceSessionMeta;
    const activeEffectiveConnectionSlug = React.useMemo(() => resolveEffectiveConnectionSlug(skillDiscoverySessionMeta?.llmConnection, workspaceDefaultLlmConnection, llmConnections), [
        skillDiscoverySessionMeta?.llmConnection,
        llmConnections,
        workspaceDefaultLlmConnection,
    ]);
    const activeEffectiveConnection = React.useMemo(() => activeEffectiveConnectionSlug
        ? llmConnections.find((connection) => connection.slug === activeEffectiveConnectionSlug)
        : undefined, [activeEffectiveConnectionSlug, llmConnections]);
    const shouldUseQwenAcpSkills = activeEffectiveConnection?.providerType === 'qwen';
    const activeQwenSessionId = shouldUseQwenAcpSkills
        ? activeSessionBelongsToActiveWorkspace
            ? (session.selected ?? null)
            : (firstActiveWorkspaceSessionMeta?.id ?? null)
        : null;
    const activeQwenCapabilityCacheKey = getQwenCapabilityCacheKey(activeWorkspaceId, activeSkillsWorkingDirectory, activeEffectiveConnectionSlug);
    const activeQwenCapabilitySnapshot = activeQwenCapabilityCacheKey
        ? qwenCapabilityCache[activeQwenCapabilityCacheKey]
        : undefined;
    const selectedSessionAtom = React.useMemo(() => session.selected
        ? sessionAtomFamily(session.selected)
        : EMPTY_SESSION_ATOM, [session.selected]);
    const selectedSession = useAtomValue(selectedSessionAtom);
    const getQwenCapabilitySnapshot = React.useCallback((workspaceId, workingDirectory, connectionSlug) => {
        const key = getQwenCapabilityCacheKey(workspaceId, workingDirectory, connectionSlug);
        if (key && qwenCapabilityCache[key])
            return qwenCapabilityCache[key];
        ...
        const defaultDirectories = [
            ...
            workspaceSkillWorkingDirectory,
            activeWorkspace?.rootPath,
        ].filter((directory) => Boolean(directory && directory.trim()));
        const fallbackDirectories = new Set();
        if (!requestedDirectory) {
            for (const directory of defaultDirectories) {
                fallbackDirectories.add(directory);
            }
        }
        else if (defaultDirectories.some((directory) => directory.trim() === requestedDirectory)) {
            fallbackDirectories.add(null);
        }
        for (const fallbackDirectory of fallbackDirectories) {
            const fallbackKey = getQwenCapabilityCacheKey(workspaceId, fallbackDirectory, connectionSlug);
    ...
    const shouldLoadSkills = shouldLoadWorkspaceSkills({
        isSkillsNavigation: isSkillsNavigation(navState) || isSkillMarketplaceNavigation(navState),
        llmConnectionCount: llmConnections.length,
        providerType: activeEffectiveConnection?.providerType,
    });
    const skillsLoading = shouldLoadSkills &&
        (!activeSkillsState ||
    ...
    React.useEffect(() => {
        ...
        setQwenCapabilityCache((prev) => {
            ...
            if (current?.availableCommands === availableCommands &&
            ...
            const nextSkills = availableSkills?.length || availableSkillDetails?.length
    ...
    const reloadSkills = React.useCallback(async (options) => {
        ...
        try {
            const loaded = await window.electronAPI.getSkills(workspaceId, activeSkillsWorkingDirectory, activeQwenSessionId ?? undefined);
            ...
            setSkillsByScopeKey((prev) => {
                if (prev[scopeKey]?.requestId !== requestId)
                    return prev;
                return {
                    ...prev,
                    [scopeKey]: {
    ...
    const handleMarketplaceSkillInstallStart = React.useCallback((skillId) => {
        setInstallingMarketplaceSkillIds((ids) => new Set(ids).add(skillId));
    }, []);
    const handleMarketplaceSkillInstallFinish = React.useCallback((skillId) => {
    ...
    const workspaceSessionMetas = useMemo(() => {
        ...
        if (!activeWorkspaceId)
            return liveMetas.filter((s) => !s.hidden);
        const activeWorkspaceMetas = getWorkspaceSessionMetas(workspaceSessions, activeWorkspaceId);
        const liveWorkspaceMetas = liveMetas.filter((s) => s.workspaceId === activeWorkspaceId ||
            (remoteWorkspaceId && s.workspaceId === remoteWorkspaceId));
    ...
    const [workspaceSessionSnapshotLoadingIds, setWorkspaceSessionSnapshotLoadingIds,] = useState(new Set());
    const [sessionListRefreshWorkspaceIds, setSessionListRefreshWorkspaceIds] = useState(new Set());
    const workspaceSessionSnapshotRetryAttemptsRef = useRef(new Map());
    const workspaceSessionSnapshotRetryTimersRef = useRef(new Map());
    const scheduleWorkspaceSessionSnapshotRetry = useCallback((workspaceId) => {
    ...
    useEffect(() => {
        ...
        const loadWorkspaceSessionSnapshots = async () => {
            ...
            setWorkspaceSessionSnapshotLoadingIds(new Set(snapshotWorkspaces
                .filter((workspace) => !previousCache.has(workspace.id))
                .map((workspace) => workspace.id)));
            let hasPendingRetry = false;
            const pendingRetryWorkspaceIds = new Set();
    ...
    const projectTreeWorkspaceSessions = useMemo(() => {
        ...
        if (activeWorkspaceId) {
            next.set(activeWorkspaceId, mergeStableSessionMetaList(next.get(activeWorkspaceId), workspaceSessionMetas));
        }
        return next;
    ...
    const [projectSessionRevealRequest, setProjectSessionRevealRequest] = React.useState(null);
    const handleRevealProjectSession = useCallback((workspaceId, sessionId) => {
    ...
    const isMetaDone = (s) => s.sessionStatus === 'done' || s.sessionStatus === 'cancelled';
    ...
    const labelCounts = useMemo(() => {
        ...
        for (const label of allLabels) {
            ...
            const directCount = activeSessionMetas.filter((s) => s.labels?.some((l) => extractLabelId(l) === label.id)).length;
        ...
        for (const label of allLabels) {
            ...
            if (descendants.length > 0) {
                const descendantCount = activeSessionMetas.filter((s) => s.labels?.some((l) => descendants.includes(extractLabelId(l)))).length;
    ...
    const sessionStatusCounts = useMemo(() => {
        ...
        for (const s of activeSessionMetas) {
            const state = (s.sessionStatus || 'todo');
    ...
    const automationTypeCounts = useMemo(() => {
        ...
        for (const automation of automations) {
            if (automation.event === 'SchedulerTick')
                counts.scheduled++;
            else if (APP_EVENTS.includes(automation.event))
                counts.event++;
            else if (AGENT_EVENTS.includes(automation.event))
                counts.agentic++;
        }
        return counts;
    }, [automations]);
    ...
    const filteredSessionMetas = useMemo(() => {
        ...
        switch (sessionFilter.kind) {
            ...
            case 'label': {
                if (sessionFilter.labelId === '__all__') {
                    ...
                    result = activeSessionMetas.filter((s) => s.labels?.some((l) => matchIds.has(extractLabelId(l))));
                }
                break;
            }
            case 'view': {
        ...
        if (listFilter.size > 0) {
            const statusIncludes = new Set();
            const statusExcludes = new Set();
            for (const [id, mode] of listFilter) {
                if (mode === 'include')
                    statusIncludes.add(id);
                else
                    statusExcludes.add(id);
            }
            if (statusIncludes.size > 0) {
                result = result.filter((s) => statusIncludes.has((s.sessionStatus || 'todo')));
            }
            if (statusExcludes.size > 0) {
                result = result.filter((s) => !statusExcludes.has((s.sessionStatus || 'todo')));
            }
        }
        ...
        if (labelFilter.size > 0) {
            ...
            for (const [id, mode] of labelFilter) {
                ...
                for (const expandedId of ids) {
                    if (mode === 'include')
                        labelIncludes.add(expandedId);
                    else
                        labelExcludes.add(expandedId);
                }
            }
            if (labelIncludes.size > 0) {
                result = result.filter((s) => s.labels?.some((l) => labelIncludes.has(extractLabelId(l))));
            }
            if (labelExcludes.size > 0) {
                result = result.filter((s) => !s.labels?.some((l) => labelExcludes.has(extractLabelId(l))));
            }
        }
        return result;
    }, [
    ...
    const pinnedFilters = useMemo(() => {
        ...
        switch (sessionFilter.kind) {
            ...
            case 'label':
                ...
                return {
                    pinnedStatusId: null,
                    pinnedLabelId: sessionFilter.labelId !== '__all__' ? sessionFilter.labelId : null,
                    pinnedFlagged: false,
                };
            case 'flagged':
                return {
    ...
    const handleDeleteSession = useCallback(async (sessionId, skipConfirmation, displayTitle) => {
    ...
    const appShellContextValue = React.useMemo(() => ({
        ...contextValue,
    ...
    const handleSessionStatusClick = useCallback((stateId) => {
        setCollapseSessionNavigatorForProjectDraft(false);
        setProjectDraftTargetWorkspaceId(null);
    ...
    const handleStatusReorder = useCallback((orderedIds) => {
        if (!activeWorkspaceId)
            return;
    ...
    const handleMarketplaceSkillSelect = useCallback((skillId) => {
    ...
    const handleSettingsClick = useCallback((subpage) => {
    ...
    const editPopoverAnchorY = useRef(120);
    ...
    const editLabelTargetId = useRef(undefined);
    ...
    const editPopoverTriggerRef = useRef(null);
    ...
    const openConfigureLabels = useCallback((labelId) => {
    ...
    const handleAddLabel = useCallback((parentId) => {
    ...
    const openAddSource = useCallback((sourceType) => {
        captureContextMenuPosition();
    ...
    const handleNewChat = useCallback((newPanel = false) => {
        ...
        navigate(routes.action.newSession(), newPanel ? { newPanel: true, targetLaneId: 'main' } : undefined);
        // Focus the chat input after navigation completes
        setTimeout(() => focusZone('chat', { intent: 'programmatic' }), 50);
    }, [activeWorkspace, focusZone]);
    const handleNewProjectSession = useCallback(async (workspaceId) => {
        setSearchActive(false);
        setSearchQuery('');
        ...
        if (workspaceId !== activeWorkspaceId) {
            await Promise.resolve(onSelectWorkspace(workspaceId, false, {
    ...
    const handleSelectProjectSession = useCallback(async (workspaceId, sessionId) => {
        setCollapseSessionNavigatorForProjectDraft(false);
        setProjectDraftTargetWorkspaceId(null);
        setSearchActive(false);
        setSearchQuery('');
    ...
    const handleDeleteSource = useCallback(async (sourceSlug) => {
        if (!activeWorkspace)
            return;
        try {
    ...
    // Delete Skill
    const handleDeleteSkill = useCallback(async (skillSlug) => {
        if (!activeWorkspace)
            return;
        try {
            await window.electronAPI.deleteSkill(activeWorkspace.id, skillSlug, activeSkillsWorkingDirectory, activeQwenSessionId ?? undefined);
    ...
    const handleSetSkillEnabled = useCallback(async (skill, enabled) => {
        if (!activeWorkspace)
            return;
        const scope = skill.providerLevel === 'project' ? 'project' : 'global';
        try {
            await window.electronAPI.setSkillEnabled(activeWorkspace.id, skill.slug, enabled, activeSkillsWorkingDirectory, activeQwenSessionId ?? undefined, scope);
            await reloadSkills({ force: true });
        }
        catch (error) {
    ...
    useEffect(() => {
        // Skip initial render
        if (menuTriggerRef.current === menuNewChatTrigger)
            return;
    ...
    const handleToggleFolder = React.useCallback((path) => {
    ...
    const getSidebarItemProps = React.useCallback((id) => ({
    ...
    const handleSidebarKeyDown = React.useCallback((e) => {
    ...
    const browserDockExpandedLeft = effectiveSidebarAndNavigatorHidden
        ...
        : isSidebarVisible
            ...
            : PANEL_EDGE_INSET;
    return (_jsxs(AppShellProvider, { value: appShellContextValue, children: [_jsx(TopBar, { workspaces: workspaces, activeWorkspaceId: activeWorkspaceId, onSelectWorkspace: onSelectWorkspace, workspaceUnreadMap: workspaceUnreadMap, onWorkspaceCreated: () => onRefreshWorkspaces?.(), onWorkspaceRemoved: () => onRefreshWorkspaces?.(), onNewChat: () => handleNewChat(), onNewWindow: () => window.electronAPI.menuNewWindow(), onOpenSettings: onOpenSettings, onOpenSettingsSubpage: handleSettingsClick, onOpenKeyboardShortcuts: onOpenKeyboardShortcuts, onShowAbout: BRAND.creditsEntries.length > 0
                    ? () => setShowAboutDialog(true)
                    : undefined, onBack: goBack, onForward: goForward, canGoBack: canGoBack, canGoForward: canGoForward, onToggleSidebar: handleToggleSidebar, onToggleFocusMode: () => setIsSidebarAndNavigatorHidden((prev) => !prev), isCompact: isAutoCompact }), _jsx(AboutDialog, { open: showAboutDialog, onOpenChange: setShowAboutDialog }), _jsxs("div", { ref: shellRef, className: "flex items-stretch relative", style: {
                    height: '100%',
                    paddingRight: 0,
                    paddingBottom: 0,
                    paddingLeft: 0,
                    gap: PANEL_GAP,
                }, children: [_jsx(PanelStackContainer, { sidebarSlot: _jsx("div", { ref: sidebarRef, style: { width: sidebarWidth }, className: "h-full font-sans relative", "data-focus-zone": "sidebar", tabIndex: sidebarFocused ? 0 : -1, onKeyDown: handleSidebarKeyDown, children: _jsx("div", { className: "flex h-full flex-col select-none pt-[48px]", children: _jsxs("div", { className: "flex-1 flex flex-col min-h-0", children: [_jsxs("div", { className: "px-2 pb-0 shrink-0 grid gap-0.5", children: [_jsxs(ContextMenu, { modal: true, children: [_jsx(ContextMenuTrigger, { asChild: true, children: _jsxs("button", { type: "button", onClick: (e) => handleNewChat(e.metaKey || e.ctrlKey), className: cn('group flex w-full items-center gap-2 rounded-[6px] px-2 py-[5px]', 'text-[13px] font-normal select-none outline-none titlebar-no-drag', 'hover:bg-sidebar-hover data-[state=open]:bg-sidebar-hover', 'focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring'), "data-tutorial": "new-chat-button", children: [_jsx(SquarePenRounded, { className: "h-3.5 w-3.5 shrink-0", style: {
                                                                            color: 'color-mix(in oklch, var(--foreground) 60%, transparent)',
                                                                        } }), _jsx("span", { className: "min-w-0 flex-1 truncate text-left", children: t('session.newSession') }), newChatHotkey && (_jsx("span", { className: cn('ml-auto shrink-0 text-xs text-foreground/30 opacity-0 transition-opacity', 'group-hover:opacity-100 group-focus-visible:opacity-100 group-data-[state=open]:opacity-100'), children: newChatHotkey }))] }) }), _jsx(StyledContextMenuContent, { children: _jsx(ContextMenuProvider, { children: _jsx(SidebarMenu, { type: "newSession" }) }) })] }), _jsx(SidebarSessionSearch, { workspaces: workspaces, workspaceSessions: projectTreeWorkspaceSessions, activeWorkspaceId: activeWorkspaceId, selectedSessionId: effectiveSessionId, onSelectSession: handleSelectProjectSession, onRevealSession: handleRevealProjectSession })] }), _jsx("div", { className: "shrink-0 border-b border-foreground/5 pt-0 pb-2", children: _jsx(LeftSidebar, { isCollapsed: false, className: "py-0 mt-0.5", getItemProps: getSidebarItemProps, focusedItemId: focusedSidebarItemId, links: [
                                                    {
                                                        id: 'nav:skills',
                                                    ...
                                                    },
                                                    {
                                                        ...
                                                        icon: ListTodo,
                                                        variant: isAutomationsNavigation(navState) &&
                                                            !automationFilter
                                                            ? 'default'
                                                            : 'ghost',
                                                        ...
                                                        items: [
                                                            {
                                                                ...
                                                                icon: Clock,
                                                                variant: automationFilter?.kind === 'type' &&
                                                ...
                                                ] }) }), _jsx(WorkspaceProjectTree, { workspaces: workspaces, activeWorkspaceId: activeWorkspaceId, selectedSessionId: effectiveSessionId, workspaceSessions: projectTreeWorkspaceSessions, loadingWorkspaceSessionIds: projectTreeLoadingWorkspaceSessionIds, workspaceUnreadMap: workspaceUnreadMap, revealRequest: projectSessionRevealRequest, onSelectWorkspace: onSelectWorkspace, onSelectSession: handleSelectProjectSession, onNewSession: handleNewProjectSession, onWorkspaceCreated: () => onRefreshWorkspaces?.(), onWorkspaceChanged: () => onRefreshWorkspaces?.(), sessionStatuses: effectiveSessionStatuses, labels: displayLabelConfigs, onDeleteSession: handleDeleteSession, onFlagSession: onFlagSession, onUnflagSession: onUnflagSession, onArchiveSession: onArchiveSession, onUnarchiveSession: onUnarchiveSession, onMarkSessionUnread: onMarkSessionUnread, onSessionStatusChange: onSessionStatusChange, onRenameSession: onRenameSession, onSessionLabelsChange: handleSessionLabelsChange })] }) }) }), sidebarWidth: effectiveSidebarAndNavigatorHidden
                            ? 0
                            : isSidebarVisible
                                ? sidebarWidth
                                : 0, navigatorSlot: isAllSessionsNavigatorHidden ||
                            isSessionNavigatorCollapsed ? null : (_jsxs("div", { style: { width: isAutoCompact ? '100%' : sessionListWidth }, className: "h-full flex flex-col min-w-0 relative z-panel", children: [_jsx(PanelHeader, { title: isSidebarVisible ? listTitle : undefined, compensateForStoplight: !isSidebarVisible, badge: automationFilter?.automationType === 'scheduled' ? (_jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx("span", { className: "text-muted-foreground/50 cursor-default flex items-center titlebar-no-drag", children: _jsx(Info, { className: "h-3 w-3" }) }) }), _jsx(TooltipContent, { side: "bottom", className: "max-w-[220px]", children: "Scheduling requires your machine to be running. It can be locked, but must be powered on." })] })) : undefined, actions: _jsxs(_Fragment, { children: [isSessionsNavigation(navState) && (_jsxs(DropdownMenu, { onOpenChange: (open) => {
                                                    if (!open) {
                                                        setFilterDropdownQuery('');
                                                        setFilterAltHeld(false);
                                                    }
                                                }, children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx(HeaderIconButton, { icon: _jsx(ListFilter, { className: "h-4 w-4" }), className: listFilter.size > 0 || labelFilter.size > 0
                                                                ? 'bg-accent/5 text-accent rounded-[8px] shadow-tinted'
                                                                : 'rounded-[8px]', style: listFilter.size > 0 || labelFilter.size > 0
                                                                ? {
                                                                    '--shadow-color': 'var(--accent-rgb)',
                                                                }
                                                                : undefined }) }), _jsxs(StyledDropdownMenuContent, { align: "end", light: true, minWidth: "min-w-[200px]", onKeyDown: (e) => {
                                                            if (e.key === 'Alt')
                                                                setFilterAltHeld(true);
                                                            ...
                                                            if (e.key === 'Alt')
                                                                setFilterAltHeld(false);
                                                        }, children: [_jsxs("div", { className: "flex items-center justify-between px-2 py-1.5", children: [_jsx("span", { className: "text-xs font-medium text-muted-foreground", children: t('sidebar.filterChats') }), (listFilter.size > 0 ||
                                                                        labelFilter.size > 0) && (_jsx("button", { onClick: (e) => {
                                                                            e.preventDefault();
                                                                            setListFilter(new Map());
                                                                            setLabelFilter(new Map());
                                                                        }, className: "text-xs text-muted-foreground hover:text-foreground", children: "Clear" }))] }), _jsx("div", { className: "px-1 pb-3 border-b border-foreground/5", children: _jsx("div", { className: "bg-background rounded-[6px] shadow-minimal px-2 py-1.5", children: _jsx("input", { ref: filterDropdownInputRef, type: "text", value: filterDropdownQuery, onChange: (e) => setFilterDropdownQuery(e.target.value), onKeyDown: (e) => {
                                                                        ...
                                                                        }, placeholder: t(FEATURE_FLAGS.sessionLabelsUi
                                                                            ? 'sidebar.searchStatusesLabels'
                                                                            : 'sidebar.searchStatuses'), className: "w-full bg-transparent text-sm text-foreground placeholder:text-muted-foreground outline-none", autoFocus: true }) }) }), filterDropdownQuery.trim() === '' ? (_jsxs(_Fragment, { children: [(pinnedFilters.pinnedFlagged ||
                                                                        pinnedFilters.pinnedStatusId ||
                                                                        pinnedFilters.pinnedLabelId ||
                                                                        listFilter.size > 0 ||
                                                                        labelFilter.size > 0) && (_jsxs(_Fragment, { children: [pinnedFilters.pinnedFlagged && (_jsx(StyledDropdownMenuItem, { disabled: true, children: _jsx(FilterMenuRow, { icon: _jsx(Flag, { className: "h-3.5 w-3.5" }), label: t('sidebar.flagged'), accessory: _jsx(Check, { className: "h-3 w-3 text-muted-foreground" }) }) })), (() => {
                                                                                if (!pinnedFilters.pinnedStatusId)
                                                                                    return null;
                                                                                const state = effectiveSessionStatuses.find((s) => s.id ===
                                                                                    pinnedFilters.pinnedStatusId);
                                                                                if (!state)
                                                                                    return null;
                                                                                return (_jsx(StyledDropdownMenuItem, { disabled: true, children: _jsx(FilterMenuRow, { icon: state.icon, label: state.label, accessory: _jsx(Check, { className: "h-3 w-3 text-muted-foreground" }), iconStyle: state.iconColorable
                                                                                            ? { color: state.resolvedColor }
                                                                                            : undefined, noIconContainer: true }) }, `pinned-status-${state.id}`));
                                                                            })(), (() => {
                                                                                ...
                                                                                if (!label)
                                                                                    return null;
                                                                                return (_jsx(StyledDropdownMenuItem, { disabled: true, children: _jsx(FilterMenuRow, { icon: _jsx(LabelIcon, { label: label, size: "lg" }), label: label.name, accessory: _jsx(Check, { className: "h-3 w-3 text-muted-foreground" }) }) }, `pinned-label-${label.id}`));
                                                                            })(), effectiveSessionStatuses
                                                                                ...
                                                                                return (_jsxs(DropdownMenuSub, { children: [_jsx(StyledDropdownMenuSubTrigger, { onClick: (e) => {
                                                                                            ...
                                                                                            }, children: _jsx(FilterMenuRow, { icon: state.icon, label: state.label, accessory: _jsx(FilterModeBadge, { mode: mode }), iconStyle: applyColor
                                                                                                    ? {
                                                                                                        color: state.resolvedColor,
                                                                                                    }
                                                                                                    : undefined, noIconContainer: true }) }), _jsx(StyledDropdownMenuSubContent, { minWidth: "min-w-[140px]", children: _jsx(FilterModeSubMenuItems, { mode: mode, onChangeMode: (newMode) => setListFilter((prev) => {
                                                                                                    const next = new Map(prev);
                                                                                ...
                                                                                return (_jsxs(DropdownMenuSub, { children: [_jsx(StyledDropdownMenuSubTrigger, { onClick: (e) => {
                                                                                            ...
                                                                                            }, children: _jsx(FilterMenuRow, { icon: _jsx(LabelIcon, { label: label, size: "lg" }), label: label.name, accessory: _jsx(FilterModeBadge, { mode: mode }) }) }), _jsx(StyledDropdownMenuSubContent, { minWidth: "min-w-[140px]", children: _jsx(FilterModeSubMenuItems, { mode: mode, onChangeMode: (newMode) => setLabelFilter((prev) => {
                                                                                                    const next = new Map(prev);
                                                                            ...
                                                                            }), _jsx(StyledDropdownMenuSeparator, {})] })), _jsxs(DropdownMenuSub, { children: [_jsxs(StyledDropdownMenuSubTrigger, { children: [_jsx(Inbox, { className: "h-3.5 w-3.5" }), _jsx("span", { className: "flex-1", children: t('sidebar.statuses') })] }), _jsx(StyledDropdownMenuSubContent, { minWidth: "min-w-[180px]", children: effectiveSessionStatuses.map((state) => {
                                                                                    const applyColor = state.iconColorable;
                                                                                    const isPinned = state.id ===
                                                                                        pinnedFilters.pinnedStatusId;
                                                                                    ...
                                                                                    if (isActive) {
                                                                                        return (_jsxs(DropdownMenuSub, { children: [_jsx(StyledDropdownMenuSubTrigger, { onClick: (e) => {
                                                                                                    ...
                                                                                                    }, children: _jsx(FilterMenuRow, { icon: state.icon, label: state.label, accessory: _jsx(FilterModeBadge, { mode: currentMode }), iconStyle: applyColor
                                                                                                            ? {
                                                                                                                color: state.resolvedColor,
                                                                                                            }
                                                                                                            : undefined, noIconContainer: true }) }), _jsx(StyledDropdownMenuSubContent, { minWidth: "min-w-[140px]", children: _jsx(FilterModeSubMenuItems, { mode: currentMode, onChangeMode: (newMode) => setListFilter((prev) => {
                                                                                                            const next = new Map(prev);
                                                                                                            next.set(state.id, newMode);
                                                                                                            return next;
                                                                                                        }), onRemove: () => setListFilter((prev) => {
                                                                                    ...
                                                                                    return (_jsx(AltExcludeTooltip, { show: filterAltHeld && !isPinned, children: _jsx(StyledDropdownMenuItem, { disabled: isPinned, onClick: (e) => {
                                                                                                if (isPinned)
                                                                                                    return;
                                                                                                e.preventDefault();
                                                                                                setListFilter((prev) => {
                                                                                                    ...
                                                                                                    if (next.has(state.id))
                                                                                                        ...
                                                                                                        next.set(state.id, e.altKey
                                                                                                            ? 'exclude'
                                                                                                            : 'include');
                                                                                                    return next;
                                                                                                });
                                                                                            }, children: _jsx(FilterMenuRow, { icon: state.icon, label: state.label, accessory: isPinned ? (_jsx(Check, { className: "h-3 w-3 text-muted-foreground" })) : null, iconStyle: applyColor
                                                                                                    ? {
                                                                                                        color: state.resolvedColor,
                                                                                                    }
                                                                                                    : undefined, noIconContainer: true }) }) }, state.id));
                                                                                }) })] }), FEATURE_FLAGS.sessionLabelsUi && (_jsxs(DropdownMenuSub, { children: [_jsxs(StyledDropdownMenuSubTrigger, { children: [_jsx(Tag, { className: "h-3.5 w-3.5" }), _jsx("span", { className: "flex-1", children: t('sidebar.labels') })] }), _jsx(StyledDropdownMenuSubContent, { minWidth: "min-w-[180px]", children: labelConfigs.length === 0 ? (_jsx(StyledDropdownMenuItem, { disabled: true, children: _jsx("span", { className: "text-muted-foreground", children: t('table.noLabelsConfigured') }) })) : (_jsx(FilterLabelItems, { labels: displayLabelConfigs, labelFilter: labelFilter, setLabelFilter: setLabelFilter, pinnedLabelId: pinnedFilters.pinnedLabelId, altHeld: filterAltHeld })) })] })), !isStateSubView && (_jsxs(_Fragment, { children: [_jsx(StyledDropdownMenuSeparator, {}), _jsxs(DropdownMenuSub, { children: [_jsxs(StyledDropdownMenuSubTrigger, { children: [_jsx(Layers, { className: "h-3.5 w-3.5" }), _jsx("span", { className: "flex-1", children: t('sidebar.group') })] }), _jsxs(StyledDropdownMenuSubContent, { minWidth: "min-w-[140px]", children: [_jsxs(StyledDropdownMenuItem, { onClick: () => setChatGroupingMode('none'), children: [_jsx(ListTodo, { className: "h-3.5 w-3.5" }), _jsx("span", { className: "flex-1", children: t('sidebar.groupNone', 'No grouping') }), chatGroupingMode === 'none' && (_jsx(Check, { className: "h-3 w-3 text-muted-foreground" }))] }), _jsxs(StyledDropdownMenuItem, { onClick: () => setChatGroupingMode('date'), children: [_jsx(Calendar, { className: "h-3.5 w-3.5" }), _jsx("span", { className: "flex-1", children: t('sidebar.groupByDate') }), chatGroupingMode === 'date' && (_jsx(Check, { className: "h-3 w-3 text-muted-foreground" }))] }), _jsxs(StyledDropdownMenuItem, { onClick: () => setChatGroupingMode('status'), children: [_jsx(Inbox, { className: "h-3.5 w-3.5" }), _jsx("span", { className: "flex-1", children: t('sidebar.groupByStatus') }), chatGroupingMode === 'status' && (_jsx(Check, { className: "h-3 w-3 text-muted-foreground" }))] })] })] })] })), _jsx(StyledDropdownMenuSeparator, {}), _jsxs(StyledDropdownMenuItem, { onClick: () => {
                                                                            setSearchActive(true);
                                                                        }, children: [_jsx(Search, { className: "h-3.5 w-3.5" }), _jsx("span", { className: "flex-1", children: t('sidebar.search') })] })] })) : (_jsx(_Fragment, { children: filterDropdownResults.states.length === 0 &&
                                                                    filterDropdownResults.labels.length === 0 ? (_jsx("div", { className: "px-3 py-4 text-center text-xs text-muted-foreground", children: FEATURE_FLAGS.sessionLabelsUi
                                                                        ? 'No matching statuses or labels'
                                                                        : 'No matching statuses' })) : (_jsxs("div", { ref: filterDropdownListRef, className: "max-h-[240px] overflow-y-auto py-1", children: [filterDropdownResults.states.length >
                                                                            0 && (_jsxs(_Fragment, { children: [_jsx("div", { className: "px-3 pt-1.5 pb-1 text-[11px] font-medium text-muted-foreground/60 uppercase tracking-wider", children: "Statuses" }), filterDropdownResults.states.map((state, index) => {
                                                                                    const applyColor = state.iconColorable;
                                                                                    const isPinned = state.id ===
                                                                                        pinnedFilters.pinnedStatusId;
                                                                                    const currentMode = listFilter.get(state.id);
                                                                                    ...
                                                                                    if (isActive) {
                                                                                        return (_jsxs(DropdownMenuSub, { children: [_jsx(StyledDropdownMenuSubTrigger, { "data-filter-selected": isHighlighted, onMouseEnter: () => setFilterDropdownSelectedIdx(index), className: cn('mx-1', isHighlighted &&
                                                                                                        'bg-foreground/5'), onClick: (e) => {
                                                                                                        e.preventDefault();
                                                                                                        setListFilter((prev) => {
                                                                                                            const next = new Map(prev);
                                                                                                            next.delete(state.id);
                                                                                                            ...
                                                                                                            next.delete(state.id);
                                                                                                            return next;
                                                                                                        }) }) })] }, `flat-status-${state.id}`));
                                                                                    }
                                                                                    // Inactive / pinned status → plain div with click-to-toggle
                                                                                    return (_jsx(AltExcludeTooltip, { show: filterAltHeld && !isPinned, children: _jsx("div", { "data-filter-selected": isHighlighted, onMouseEnter: () => setFilterDropdownSelectedIdx(index), onClick: (e) => {
                                                                                                if (isPinned)
                                                                                                    return;
                                                                                                e.preventDefault();
                                                                                                setListFilter((prev) => {
                                                                                            ...
                                                                                            "flex cursor-pointer select-none items-center gap-2 rounded-[4px] mx-1 px-2 py-1.5 text-sm [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0", isHighlighted &&
                                                                                                'bg-foreground/5', isPinned &&
                                                                                                'opacity-50 pointer-events-none'), children: _jsx(FilterMenuRow, { icon: state.icon, label: state.label, accessory: isPinned ? (_jsx(Check, { className: "h-3 w-3 text-muted-foreground" })) : null, iconStyle: applyColor
                                                                                                    ? {
                                                                                                        color: state.resolvedColor,
                                                                                                    }
                                                                                                    : undefined, noIconContainer: true }) }) }, `flat-status-${state.id}`));
                                                                                })] })), filterDropdownResults.states.length > 0 &&
                                                                            filterDropdownResults.labels.length >
                                                                                0 && (_jsx("div", { className: "my-1 mx-2 border-t border-border/40" })), filterDropdownResults.labels.length >
                                                                            0 && (_jsxs(_Fragment, { children: [_jsx("div", { className: "px-3 pt-1.5 pb-1 text-[11px] font-medium text-muted-foreground/60 uppercase tracking-wider", children: "Labels" }), filterDropdownResults.labels.map((item, index) => {
                                                                                    // Offset by state count for unified index
                                                                                    const flatIndex = filterDropdownResults.states
                                                                                        .length + index;
                                                                                    const isPinned = item.id ===
                                                                                        pinnedFilters.pinnedLabelId;
                                                                                    const currentMode = labelFilter.get(item.id);
                                                                                    const isHighlighted = flatIndex ===
                                                                                        filterDropdownSelectedIdx;
                                                                                    const isActive = !!currentMode && !isPinned;
                                                                                    const labelDisplay = item.parentPath ? (_jsxs(_Fragment, { children: [_jsx("span", { className: "text-muted-foreground", children: item.parentPath }), item.label] })) : (item.label);
                                                                                    // Active label → DropdownMenuSub with mode options
                                                                                    if (isActive) {
                                                                                        return (_jsxs(DropdownMenuSub, { children: [_jsx(StyledDropdownMenuSubTrigger, { "data-filter-selected": isHighlighted, onMouseEnter: () => setFilterDropdownSelectedIdx(flatIndex), className: cn('mx-1', isHighlighted &&
                                                                                                        'bg-foreground/5'), onClick: (e) => {
                                                                                                        e.preventDefault();
                                                                                                        setLabelFilter((prev) => {
                                                                                                            const next = new Map(prev);
                                                                                                    ...
                                                                                                    }, children: _jsx(FilterMenuRow, { icon: _jsx(LabelIcon, { label: item.config, size: "lg" }), label: labelDisplay, accessory: _jsx(FilterModeBadge, { mode: currentMode }) }) }), _jsx(StyledDropdownMenuSubContent, { minWidth: "min-w-[140px]", children: _jsx(FilterModeSubMenuItems, { mode: currentMode, onChangeMode: (newMode) => setLabelFilter((prev) => {
                                                                                                            const next = new Map(prev);
                                                                                    ...
                                                                                    return (_jsx(AltExcludeTooltip, { show: filterAltHeld && !isPinned, children: _jsx("div", { "data-filter-selected": isHighlighted, onMouseEnter: () => setFilterDropdownSelectedIdx(flatIndex), onClick: (e) => {
                                                                                                if (isPinned)
                                                                                                    return;
                                                                                                e.preventDefault();
                                                                                                setLabelFilter((prev) => {
                                                                                            ...
                                                                                            "flex cursor-pointer select-none items-center gap-2 rounded-[4px] mx-1 px-2 py-1.5 text-sm [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0", isHighlighted &&
                                                                                                'bg-foreground/5', isPinned &&
                                                                                                'opacity-50 pointer-events-none'), children: _jsx(FilterMenuRow, { icon: _jsx(LabelIcon, { label: item.config, size: "lg" }), label: labelDisplay, accessory: isPinned ? (_jsx(Check, { className: "h-3 w-3 text-muted-foreground" })) : null }) }) }, `flat-label-${item.id}`));
                                                                                })] }))] })) }))] })] })), isSourcesNavigation(navState) && activeWorkspace && (_jsx(EditPopover, { trigger: _jsx(HeaderIconButton, { icon: _jsx(Plus, { className: "h-4 w-4" }), tooltip: t('sidebarMenu.addSource'), "data-tutorial": "add-source-button" }), ...getEditConfig(sourceFilter?.kind === 'type'
                                                    ? `add-source-${sourceFilter.sourceType}`
                                                    : 'add-source', activeWorkspace.rootPath) })), isAutomationsNavigation(navState) && activeWorkspace && (_jsx(EditPopover, { trigger: _jsx(HeaderIconButton, { icon: _jsx(Plus, { className: "h-4 w-4" }), tooltip: t('sidebarMenu.addAutomation') }), ...getEditConfig('automation-config', activeWorkspace.rootPath) }))] }) }), isSourcesNavigation(navState) && (
                                /* Sources List - filtered by type if sourceFilter is active */
                                _jsx(SourcesListPanel, { sources: sources, sourceFilter: sourceFilter, workspaceRootPath: activeWorkspace?.rootPath, onDeleteSource: handleDeleteSource, onSourceClick: handleSourceSelect, selectedSourceSlug: isSourcesNavigation(navState) && navState.details
                                        ? navState.details.sourceSlug
                                        : null, localMcpEnabled: localMcpEnabled })), isSkillsNavigation(navState) && activeWorkspaceId && (
                                /* Skills List */
                                _jsx(SkillsListPanel, { skills: skills, workspaceId: activeWorkspaceId, workspaceRootPath: activeWorkspace?.rootPath, isLoading: skillsLoading, onSkillClick: handleSkillSelect, onDeleteSkill: handleDeleteSkill, onSetSkillEnabled: handleSetSkillEnabled, selectedSkillSlug: isSkillsNavigation(navState) &&
                                        navState.details?.type === 'skill'
                                        ? navState.details.skillSlug
                                        : null })), isSkillMarketplaceNavigation(navState) &&
                                    activeWorkspaceId && (_jsx(SkillMarketplacePanel, { workspaceId: activeWorkspaceId, workingDirectory: activeSkillsWorkingDirectory, activeSessionId: activeQwenSessionId, selectedSkillId: navState.details?.type === 'marketplaceSkill'
                                        ? navState.details.skillId
                                        : null, onSkillSelect: handleMarketplaceSkillSelect, onInstalled: reloadSkills, installingSkillIds: installingMarketplaceSkillIds, onInstallStart: handleMarketplaceSkillInstallStart, onInstallFinish: handleMarketplaceSkillInstallFinish })), isAutomationsNavigation(navState) && (
                                /* Automations List - filtered by type if automationFilter is active */
                                _jsx(AutomationsListPanel, { automations: automations, automationFilter: automationFilter
                                        ? {
                                            kind: AUTOMATION_TYPE_TO_FILTER_KIND[automationFilter.automationType] ?? 'all',
                                        }
                                        : undefined, onAutomationClick: handleAutomationSelect, onTestAutomation: handleTestAutomation, onToggleAutomation: handleToggleAutomation, onDuplicateAutomation: handleDuplicateAutomation, onDeleteAutomation: handleDeleteAutomation, selectedAutomationId: isAutomationsNavigation(navState) && navState.details
                                        ? navState.details.automationId
                                        : null, workspaceRootPath: activeWorkspace?.rootPath })), isSettingsNavigation(navState) && (
                                /* Settings Navigator */
                                _jsx(SettingsNavigator, { selectedSubpage: navState.subpage, onSelectSubpage: (subpage) => handleSettingsClick(subpage) })), isSessionsNavigation(navState) && (
                                /* Sessions List */
                                _jsx(_Fragment, { children: _jsx(SessionList, { items: searchActive
                                            ? workspaceSessionMetas
                                            : filteredSessionMetas, onDelete: handleDeleteSession, onFlag: onFlagSession, onUnflag: onUnflagSession, onArchive: onArchiveSession, onUnarchive: onUnarchiveSession, onMarkUnread: onMarkSessionUnread, onSessionStatusChange: onSessionStatusChange, onRename: onRenameSession, onFocusChatInput: (targetSessionId) => {
                                            focusChatInputForSession(targetSessionId ??
                                                focusedSessionId ??
                                                session.selected);
                                        ...
                                        }, sessionOptions: sessionOptions, searchActive: searchActive, searchQuery: searchQuery, onSearchChange: setSearchQuery, onSearchClose: () => {
                                            setSearchActive(false);
                                            setSearchQuery('');
                                        }, sessionStatuses: effectiveSessionStatuses, evaluateViews: evaluateViews, labels: displayLabelConfigs, onLabelsChange: handleSessionLabelsChange, groupingMode: chatGroupingMode, workspaceId: activeWorkspaceId ?? undefined, statusFilter: listFilter, labelFilterMap: labelFilter, focusedSessionId: panelCount === 0
                                            ? null
                                            : panelCount > 1
                                                ? focusedSessionId
                                                : undefined, onNavigateToSession: panelCount > 1 ? navigateToSessionInPanel : undefined, hasPendingPrompt: hasPendingPrompt, activeChatMatchInfo: chatMatchInfo, isLoading: isSessionListLoading }, sessionFilter?.kind) }))] })), navigatorWidth: effectiveNavigatorWidth, isSidebarAndNavigatorHidden: effectiveSidebarAndNavigatorHidden, isRightSidebarVisible: false, isCompact: isAutoCompact, isResizing: !!isResizing }), _jsx(BrowserDockPanel, { expandedLeft: browserDockExpandedLeft, autoHideKey: focusedPanelRoute, isCompact: isAutoCompact }), isSidebarResizeAvailable && (_jsx("div", { ref: resizeHandleRef, onMouseDown: (e) => {
                            e.preventDefault();
                            ...
                            if (!isResizing)
                                setSidebarHandleY(null);
                        }, className: "absolute cursor-col-resize z-panel flex justify-center", style: {
                        ...
                        }, children: _jsx("div", { className: "h-full", style: {
                                ...getResizeGradientStyle(sidebarHandleY, resizeHandleRef.current?.clientHeight ?? null),
                                width: PANEL_SASH_LINE_WIDTH,
                            } }) })), isNavigatorResizeAvailable && (_jsx("div", { ref: sessionListHandleRef, onMouseDown: (e) => {
                            e.preventDefault();
                            ...
                            transition: isResizing === 'session-list'
                                ? undefined
                                : 'left 0.15s ease-out',
                        }, children: _jsx("div", { className: "h-full", style: {
                                ...getResizeGradientStyle(sessionListHandleY, sessionListHandleRef.current?.clientHeight ?? null),
                                width: PANEL_SASH_LINE_WIDTH,
                            } }) }))] }), activeWorkspace && (_jsxs(_Fragment, { children: [_jsx(EditPopover, { open: editPopoverOpen === 'statuses', onOpenChange: (isOpen) => setEditPopoverOpen(isOpen ? 'statuses' : null), modal: true, trigger: _jsx("div", { className: "fixed w-0 h-0 pointer-events-none", style: {
                                left: sidebarWidth + 20,
                                top: editPopoverAnchorY.current,
                            }, "aria-hidden": "true" }), side: "bottom", align: "start", secondaryAction: {
                            label: 'Edit File',
                            filePath: `${activeWorkspace.rootPath}/statuses/config.json`,
                        }, ...getEditConfig('edit-statuses', activeWorkspace.rootPath) }), FEATURE_FLAGS.sessionLabelsUi && (_jsx(EditPopover, { open: editPopoverOpen === 'labels', onOpenChange: (isOpen) => setEditPopoverOpen(isOpen ? 'labels' : null), modal: true, trigger: _jsx("div", { className: "fixed w-0 h-0 pointer-events-none", style: {
                                left: sidebarWidth + 20,
                                top: editPopoverAnchorY.current,
                            }, "aria-hidden": "true" }), side: "bottom", align: "start", secondaryAction: {
                            label: 'Edit File',
                        ...
                        })() })), _jsx(EditPopover, { open: editPopoverOpen === 'views', onOpenChange: (isOpen) => setEditPopoverOpen(isOpen ? 'views' : null), modal: true, trigger: _jsx("div", { className: "fixed w-0 h-0 pointer-events-none", style: {
                                left: sidebarWidth + 20,
                                top: editPopoverAnchorY.current,
                            }, "aria-hidden": "true" }), side: "bottom", align: "start", secondaryAction: {
                            label: 'Edit File',
                        ...
                        'add-source-mcp',
                        'add-source-local',
                    ].map((variant) => (_jsx(EditPopover, { open: editPopoverOpen === variant, onOpenChange: (isOpen) => setEditPopoverOpen(isOpen ? variant : null), modal: true, trigger: _jsx("div", { className: "fixed w-0 h-0 pointer-events-none", style: {
                                left: sidebarWidth + 20,
                                top: editPopoverAnchorY.current,
                            }, "aria-hidden": "true" }), side: "bottom", align: "start", ...getEditConfig(variant, activeWorkspace.rootPath) }, variant))), _jsx(EditPopover, { open: editPopoverOpen === 'add-skill', onOpenChange: (isOpen) => setEditPopoverOpen(isOpen ? 'add-skill' : null), modal: true, trigger: _jsx("div", { className: "fixed w-0 h-0 pointer-events-none", style: {
                                left: sidebarWidth + 20,
                                top: editPopoverAnchorY.current,
                            }, "aria-hidden": "true" }), side: "bottom", align: "start", ...getEditConfig('add-skill', activeWorkspace.rootPath) }), _jsx(EditPopover, { open: editPopoverOpen === 'automation-config', onOpenChange: (isOpen) => setEditPopoverOpen(isOpen ? 'automation-config' : null), modal: true, trigger: _jsx("div", { className: "fixed w-0 h-0 pointer-events-none", style: {
                                left: sidebarWidth + 20,
                                top: editPopoverAnchorY.current,
                            }, "aria-hidden": "true" }), side: "bottom", align: "start", ...getEditConfig('automation-config', activeWorkspace.rootPath) }), _jsx(EditPopover, { open: editPopoverOpen === 'add-label', onOpenChange: (isOpen) => setEditPopoverOpen(isOpen ? 'add-label' : null), modal: true, trigger: _jsx("div", { className: "fixed w-0 h-0 pointer-events-none", style: {
                                left: sidebarWidth + 20,
                                top: editPopoverAnchorY.current,
                            }, "aria-hidden": "true" }), side: "bottom", align: "start", secondaryAction: {
                            label: 'Edit File',
                        ...
                        })() })] })), _jsx(Dialog, { open: !!automationPendingDelete, onOpenChange: (open) => {
                    if (!open)
                        setAutomationPendingDelete(null);
                }, children: _jsxs(DialogContent, { showCloseButton: false, children: [_jsxs(DialogHeader, { children: [_jsx(DialogTitle, { children: t('dialog.deleteAutomation.title') }), _jsx(DialogDescription, { children: _jsx(Trans, { i18nKey: "dialog.deleteAutomation.description", values: { name: pendingDeleteAutomation?.name }, components: { strong: _jsx("strong", {}) } }) })] }), _jsxs(DialogFooter, { children: [_jsx(Button, { variant: "outline", onClick: () => setAutomationPendingDelete(null), children: t('common.cancel') }), _jsx(Button, { variant: "destructive", onClick: confirmDeleteAutomation, children: t('common.delete') })] })] }) }), _jsx(SendToWorkspaceDialog, { open: sendToWorkspaceIds.length > 0, onOpenChange: (open) => {
                    if (!open)
                        setSendToWorkspaceIds([]);
                }, sessionIds: sendToWorkspaceIds, workspaces: workspaces, activeWorkspaceId: activeWorkspaceId, onTransferComplete: handleTransferComplete }), _jsx(MessagingDialogHost, {})] }));
}
