 ...
 */
export function isVoiceInitExhausted(wsUrl, retryCount) {
    return !wsUrl && retryCount >= MAX_VOICE_URL_RETRIES;
}
/** Diagnostic logged once when voice-server URL resolution is abandoned. */
export function formatVoiceInitFailureWarning() {
    const seconds = Math.round((MAX_VOICE_URL_RETRIES * VOICE_URL_RETRY_INTERVAL_MS) / 1000);
    return (`[voice] voice server URL unavailable after ${MAX_VOICE_URL_RETRIES} retries ` +
        `(~${seconds}s); dictation will not be available this session.`);
}
export function useVoiceDictation(options) {
    const [wsUrl, setWsUrl] = useState(() => window.electronAPI?.getVoiceStreamUrl?.() ?? null);
    ...
    useEffect(() => {
        ...
        const id = setInterval(() => setElapsedMs(performance.now() - startedAtRef.current), 1000);
