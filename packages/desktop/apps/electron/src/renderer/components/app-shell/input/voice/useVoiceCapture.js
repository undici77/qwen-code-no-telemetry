...
export function resampleToSampleRate(input, inputSampleRate, outputSampleRate = SAMPLE_RATE) {
...
export function useVoiceCapture(options) {
    ...
    const teardownAudio = useCallback(() => {
        const res = resourcesRef.current;
        if (res.processor)
            res.processor.onaudioprocess = null;
    ...
    const fail = useCallback((message, generation) => {
    ...
    const finishWith = useCallback((text, generation) => {
    ...
    const armTranscribeTimeout = useCallback((generation) => {
        clearTranscribeTimeout();
        resourcesRef.current.transcribeTimeout = setTimeout(() => {
            if (statusRef.current === 'recording') {
                fail('No response from server. Check that the voice model is running.', generation);
            }
    ...
    const start = useCallback(() => {
        ...
        const isStale = () => !mountedRef.current || captureGenerationRef.current !== generation;
        void (async () => {
            try {
                ...
                processor.onaudioprocess = (event) => {
                    const input = resampleToSampleRate(event.inputBuffer.getChannelData(0), context.sampleRate);
                    ...
                    droppedFrames = sendVoicePcmFrame(ws, pcm, droppedFrames, () => fail('Voice connection lost. Audio is not being sent.', generation));
                };
                ws.onopen = () => {
                    if (isStale()) {
                ...
                ws.onmessage = (event) => {
                    ...
                    if (msg.type === 'ready') {
                        ...
                        fail(msg.message ?? msg.text ?? 'Voice transcription failed.', generation);
                    }
                };
                ws.onerror = () => {
                ...
                ws.onclose = (event) => {
                    ...
                    if (mountedRef.current &&
                        ...
                        fail(`Voice connection closed (code=${code}, reason=${reason}).`, generation);
                    }
                };
            }
            catch (error) {
