 ...
 */
const MemoizedBlock = React.memo(function Block({ content, mode, onUrlClick, onFileClick, }) {
    return (_jsx(Markdown, { mode: mode, onUrlClick: onUrlClick, onFileClick: onFileClick, children: content }));
}, (prev, next) => {
 ...
 */
export function StreamingMarkdown({ content, isStreaming, mode = 'minimal', onUrlClick, onFileClick, }) {
    // Split into blocks - memoized to avoid recomputation
    // Must be called unconditionally to satisfy Rules of Hooks
    const blocks = React.useMemo(() => (isStreaming ? splitIntoBlocks(content) : []), [content, isStreaming]);
    // Not streaming - use simple Markdown (no block splitting needed)
    if (!isStreaming) {
        return (_jsx(Markdown, { mode: mode, onUrlClick: onUrlClick, onFileClick: onFileClick, children: content }));
    }
    return (_jsx(_Fragment, { children: blocks.map((block, i) => {
            ...
            return (_jsx(MemoizedBlock, { content: block.content, mode: mode, onUrlClick: onUrlClick, onFileClick: onFileClick }, key));
        }) }));
}
