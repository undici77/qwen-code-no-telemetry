 ...
 */
import { createContext, useContext, useCallback, useEffect, useLayoutEffect, useRef, useState, useMemo, } from 'react';
import { useTranslation } from 'react-i18next';
import { toast } from 'sonner';
import { useAtomValue, useSetAtom, useStore } from 'jotai';
import { useSession } from '@/hooks/useSession';
import { parseRoute, parseRouteToNavigationState, buildRouteFromNavigationState, buildRightSidebarParam, } from '../../shared/route-parser';
import { routes } from '../../shared/routes';
...
import { isSessionsNavigation, isSourcesNavigation, isSettingsNavigation, isSkillsNavigation, isSkillMarketplaceNavigation, isAutomationsNavigation, DEFAULT_NAVIGATION_STATE, } from '../../shared/types';
import { DEFAULT_SETTINGS_SUBPAGE, isValidSettingsSubpage, isVisibleSettingsSubpage, } from '../../shared/settings-registry';
import { compareSessionsByActivityDesc, sessionMetaMapAtom, updateSessionMetaAtom, } from '@/atoms/sessions';
import { sourcesAtom } from '@/atoms/sources';
import { skillsAtom } from '@/atoms/skills';
import { panelStackAtom, pushPanelAtom, reconcilePanelStackAtom, focusedPanelIdAtom, focusedPanelRouteAtom, focusedPanelIndexAtom, updateFocusedPanelRouteAtom, parseSessionIdFromRoute, } from '@/atoms/panel-stack';
...
function applyFocusedRouteToSearchParams(params, route) {
...
export function NavigationProvider({ children, workspaceId, workspaceSlug, onSwitchWorkspaceBySlug, onCreateSession, getDraft, onAutoDeleteEmptySession, isReady = true, isSessionsReady = true, remoteWorkspaceId, consumeWorkspaceSwitchRoute, }) {
     ...
     */
    const syncUrl = useCallback((push = false) => {
        const panels = store.get(panelStackAtom);
        const focusedIdx = store.get(focusedPanelIndexAtom);
        if (panels.length === 0)
            return;
    ...
    const maybePushHistoryForSemanticChange = useCallback(() => {
        ...
        if (currentSemanticKey === lastSemanticHistoryKeyRef.current)
            return;
    ...
    useEffect(() => {
        if (!initialRouteRestoredRef.current)
            return;
        if (getSemanticHistoryKey() !== lastSemanticHistoryKeyRef.current)
            return;
    ...
    useEffect(() => {
        ...
        const unsub = store.sub(panelStackAtom, () => {
            ...
            if (currRoutes.length !== prevRoutes.length ||
    ...
    useEffect(() => {
        if (rightSidebar?.type === prevSidebarTypeRef.current)
            return;
     ...
     */
    const reconcileFromUrlParams = useCallback((params) => {
        ...
        if (panelsParam) {
            ...
            if (navState) {
                const finalRoute = 'details' in navState && navState.details
                    ? initialRoute
                    : buildRouteFromNavigationState(resolveAutoSelectionRef.current(navState));
    ...
    const prevVisibleSessionIdsRef = useRef(new Set());
    useEffect(() => {
    ...
    const filterSessionsByFilter = useCallback((filter) => {
        ...
        return visibleSessions.filter((session) => {
            switch (filter.kind) {
                ...
                case 'archived':
                    return session.isArchived === true;
                case 'state':
                    return (session.sessionStatus === filter.stateId &&
    ...
    const getFirstSessionId = useCallback((filter) => {
    ...
    const getLastSelectedSessionId = useCallback((filter) => {
        if (!workspaceId)
            return null;
        const storedId = storage.get(storage.KEYS.lastSelectedSessionId, null, workspaceId);
        if (!storedId)
            return null;
        const filtered = filterSessionsByFilter(filter);
    ...
    const getFirstSourceSlug = useCallback((filter) => {
        if (!filter) {
     ...
     */
    const resolveAutoSelection = useCallback((newState, options) => {
        ...
        if (isSessionsNavigation(nextState) &&
            !nextState.details &&
        ...
        if (isSourcesNavigation(nextState) &&
            !nextState.details &&
            !options?.skipAutoSelect) {
        ...
        if (isSkillsNavigation(nextState) &&
            !nextState.details &&
            !options?.skipAutoSelect) {
    ...
    // ACTION NAVIGATION
    // =========================================================================
    const handleActionNavigation = useCallback(async (parsed, options) => {
        if (!workspaceId)
            return;
        switch (parsed.name) {
            case 'new-session': {
                ...
                if (parsed.params.workdir) {
                    createOptions.workingDirectory = parsed.params.workdir;
                }
                if (parsed.params.model) {
                    createOptions.model = parsed.params.model;
                }
                if (parsed.params.systemPrompt) {
                    createOptions.systemPromptPreset = parsed.params.systemPrompt;
                }
                ...
                if (parsed.params.badges) {
                    try {
                        badges = JSON.parse(parsed.params.badges);
                    }
                    catch (e) {
                ...
                if (shouldSend) {
                    ...
                    setTimeout(() => {
                        window.electronAPI.sendMessage(session.id, parsed.params.input, undefined, undefined, badges
                            ? {
                ...
                const hasExplicitDraftParams = parsed.params.input !== undefined ||
                ...
                const draftRoute = buildRouteFromNavigationState(draftState);
                if (options?.newPanel) {
                    pushPanel({
                        route: draftRoute,
                        targetLaneId: options.targetLaneId,
    ...
    // NAVIGATE
    // =========================================================================
    const navigate = useCallback(async (route, options) => {
        ...
        if (newNavState && isSettingsNavigation(newNavState)) {
            ...
            if (isBareSettingsRoute) {
                const savedSubpage = storage.get(storage.KEYS.lastSettingsSubpage, 'app');
        ...
        if (newNavState) {
            ...
            const finalRoute = buildRouteFromNavigationState(resolvedState);
            // Persist last selected session for auto-select on next visit
            if (isSessionsNavigation(resolvedState) &&
    ...
    // WORKSPACE SWITCH
    // =========================================================================
    const previousWorkspaceSlugRef = useRef(null);
    useLayoutEffect(() => {
        if (!workspaceId || !workspaceSlug || !isSessionsReady)
            return;
        ...
        if (previousWorkspaceSlugRef.current === workspaceSlug)
            return;
        previousWorkspaceSlugRef.current = workspaceSlug;
        ...
        if (isPopstateSwitchRef.current) {
            ...
            const savedSearch = storage.get(storage.KEYS.workspaceUrl, '', workspaceSlug);
    ...
    useEffect(() => {
        if (isReady && pendingNavigationRef.current) {
            ...
            if (navState) {
                ...
                const finalRoute = buildRouteFromNavigationState(resolved);
                store.set(updateFocusedPanelRouteAtom, finalRoute);
            }
        }
    ...
    // SIDEBAR HELPERS
    // =========================================================================
    const updateRightSidebar = useCallback((panel) => {
        setRightSidebar(panel);
        // pushState handled by the rightSidebar change effect
    }, []);
    const toggleRightSidebar = useCallback((panel) => {
    ...
    // =========================================================================
    const navigateToSource = useCallback((sourceSlug) => {
        if (isSourcesNavigation(navigationState) &&
    ...
    useEffect(() => {
        ...
        if (store.get(panelStackAtom).length === 0)
            return;
        if (!isSessionsNavigation(navigationState) || navigationState.details)
            return;
