...
import { ShikiThemeProvider, PlatformProvider, ImagePreviewOverlay, PDFPreviewOverlay, CodePreviewOverlay, DocumentFormattedMarkdownOverlay, JSONPreviewOverlay, } from '@craft-agent/ui';
...
function skillsForBadgeExtraction(skills, skillSlugs) {
 ...
 */
function handleBackgroundTaskEvent(store, sessionId, event, agentEvent) {
    ...
    if (event.type === 'task_backgrounded' && 'taskId' in evt && 'toolUseId' in evt) {
        ...
        const currentTasks = store.get(backgroundTasksAtom);
        store.set(backgroundTasksAtom, currentTasks.map(t => t.toolUseId === evt.toolUseId
            ? { ...t, elapsedSeconds: evt.elapsedSeconds }
            : t));
    }
        ...
        const isBackgroundingResult = result && (/agentId:\s*[a-zA-Z0-9_-]+/.test(result) ||
...
function SessionLoadErrorScreen({ message, onRetry, }) {
    const { t } = useTranslation();
    return (_jsx("div", { className: "flex h-full items-center justify-center p-6", children: _jsxs("div", { className: "max-w-lg rounded-xl border border-border/50 bg-background shadow-minimal p-6 text-center", children: [_jsx("h2", { className: "text-lg font-semibold text-foreground", children: t("errors.failedToLoadSessions") }), _jsx("p", { className: "mt-2 text-sm text-foreground/60", children: t("errors.failedToLoadSessionsDesc") }), _jsx("p", { className: "mt-3 rounded-lg bg-foreground/5 px-3 py-2 text-left text-xs text-foreground/70 break-words", children: message }), _jsx("button", { type: "button", onClick: onRetry, className: "mt-4 inline-flex h-8 items-center justify-center rounded-[8px] bg-foreground text-background px-3 text-sm font-medium hover:opacity-90 transition-opacity", children: t("errors.retryLoadingSessions") })] }) }));
}
export default function App() {
    ...
    const [setupNeeds, setSetupNeeds] = useState(null);
    ...
    const workspaceSwitchChainRef = useRef(Promise.resolve());
    const pendingWorkspaceSwitchRouteRef = useRef(null);
    useEffect(() => {
    ...
    const consumePendingWorkspaceSwitchRoute = useCallback((workspaceId) => {
    ...
    const [workspaceDefaultLlmConnection, setWorkspaceDefaultLlmConnection] = useState();
    // Global default LLM connection slug (from app config)
    const [defaultLlmConnectionSlug, setDefaultLlmConnectionSlug] = useState();
    ...
    const [pendingCredentials, setPendingCredentials] = useState(new Map());
    ...
    const [sessionOptions, setSessionOptions] = useState(new Map());
    const [globalPermissionMode, setGlobalPermissionMode] = useState(defaultSessionOptions.permissionMode);
    ...
    const applyPermissionModeState = useCallback((sessionId, state, source) => {
        setSessionOptions(prev => {
            const next = new Map(prev);
            const current = {
            ...
            if (state.modeVersion < currentVersion) {
                window.electronAPI.debugLog('[ModeSync] Ignoring stale permission mode update', { sessionId, source, incoming: state.modeVersion, current: currentVersion });
                return prev;
            }
            if (state.modeVersion === currentVersion &&
                current.permissionMode !== state.permissionMode) {
                window.electronAPI.debugLog('[ModeSync] Equal modeVersion with differing mode detected, applying and requesting reconciliation', {
                    sessionId,
                    source,
    ...
    const reconcilePermissionModeState = useCallback(async (sessionId) => {
        try {
    ...
    const syncSessionOptionsFromSession = useCallback((session) => {
        setSessionOptions(prev => {
            const next = new Map(prev);
    ...
    const refreshSessionFromServer = useCallback(async (sessionId) => {
        try {
            const fresh = await window.electronAPI.getSessionMessages(sessionId);
            if (!fresh)
                return 'failed';
            const prevSession = store.get(sessionAtomFamily(sessionId));
            const { session: nextSession, preservedExistingMessages, } = mergeSessionRefreshResult(prevSession, fresh);
    ...
    const cacheWorkspaceSessionMetas = useCallback((workspaceId, sessions) => {
    ...
    const applyLoadedSessions = useCallback((loadedSessions, workspaceId, remoteWorkspaceId) => {
    ...
    const reconcileLoadedSessionPermissionModes = useCallback((loadedSessions) => {
        return Promise.allSettled(loadedSessions.map((s) => reconcilePermissionModeState(s.id)));
    }, [reconcilePermissionModeState]);
    const applyFastLocalSessionSnapshot = useCallback(async (workspaceId, requestSeq) => {
        if (!workspaceId)
            return;
        ...
        if (!targetWorkspace || targetWorkspace.remoteServer)
            return;
        try {
    ...
    const loadSessionsFromServer = useCallback(async () => {
        ...
        const fullSessionsPromise = window.electronAPI.getSessions().then((sessions) => ({ ok: true, sessions }), (error) => ({ ok: false, error }));
        try {
    ...
    const refreshChangedWorkspaceSessions = useCallback(async (workspaceId) => {
        try {
            ...
            if (workspaceId !== windowWorkspaceIdRef.current)
                return;
    ...
    const qwenModelRefreshInFlightRef = useRef(null);
    const qwenModelRefreshAttemptedRef = useRef(new Set());
    const qwenModelRefreshCompletedRef = useRef(new Set());
    const resolveDefaultConnectionSlug = useCallback((connections) => {
        return connections.find(c => c.isDefault)?.slug ?? connections[0]?.slug;
    }, []);
    const handleOptimisticDefaultModelChange = useCallback((model, connectionSlug) => {
    ...
    const refreshLlmConnections = useCallback(async () => {
        ...
        const qwenConnectionToRefresh = connections.find(connection => connection.providerType === 'qwen'
    ...
    useEffect(() => {
        ...
        const handleEffects = (effects, sessionId, eventType) => {
        ...
        const cleanup = window.electronAPI.onSessionEvent((event) => {
            ...
            if (event.type === 'session_created') {
                window.electronAPI.getSessionMessages(sessionId)
                    ...
                    if (createdSession) {
                        ...
                        if (workspaceId) {
                            cacheWorkspaceSessionMetas(workspaceId, Array.from(store.get(sessionMetaMapAtom).values())
                    ...
                    .catch((error) => console.error('Failed to handle session_created event:', error));
                return;
            }
            ...
            if (event.type === 'session_id_changed') {
                ...
                setSessionOptions(prev => {
                    if (!prev.has(previousId))
                        return prev;
                    const next = new Map(prev);
                    const opts = next.get(previousId);
                    next.delete(previousId);
                    if (opts && !next.has(nextId))
                        next.set(nextId, opts);
                    return next;
                });
                ...
                window.electronAPI.getSessionMessages(nextId)
                    ...
                    .catch((error) => console.error('Failed to handle session_id_changed event:', error));
                return;
            }
            ...
            const eventWorkspaceId = event.workspaceId ?? atomSession?.workspaceId ?? metaSession?.workspaceId ?? workspaceId;
            ...
            if (isStreaming || isHandoff) {
                ...
                // Process the event
                const { session: updatedSession, effects } = processAgentEvent(agentEvent, currentSession, eventWorkspaceId);
                ...
                if (isHandoff) {
                    ...
                    if (event.type === 'complete' && !updatedSession.hidden) {
                        ...
                        const lastMessage = updatedSession.messages.findLast(m => (m.role === 'assistant' || m.role === 'plan') && !m.isIntermediate);
    ...
    useEffect(() => {
        const cleanup = window.electronAPI.onReconnected(async (isStale) => {
            if (!isStale) {
    ...
    const handleCreateSession = useCallback(async (workspaceId, options) => {
    ...
    const handleDeleteSession = useCallback(async (sessionId, skipConfirmation = false, displayTitle) => {
        if (!skipConfirmation) {
            const metaMap = store.get(sessionMetaMapAtom);
            const meta = metaMap.get(sessionId);
            const confirmed = await window.electronAPI.showDeleteSessionConfirmation(displayTitle || (meta ? getSessionTitle(meta) : 'Untitled'));
            if (!confirmed)
                return false;
        }
    ...
    const handleAutoDeleteEmptySession = useCallback((sessionId) => {
        if (sessionSelection.selected === sessionId) {
            setSession({ selected: null });
        }
     ...
     */
    const handleSetActiveViewingSession = useCallback((sessionId) => {
    ...
    const handleMarkSessionRead = useCallback((sessionId) => {
        ...
        updateSessionById(sessionId, (s) => {
            const lastFinalId = s.messages.findLast(m => (m.role === 'assistant' || m.role === 'plan') && !m.isIntermediate)?.id;
            return {
                hasUnread: false,
    ...
    const handleMarkSessionUnread = useCallback((sessionId) => {
    ...
    const handleSendMessage = useCallback(async (sessionId, message, attachments, skillSlugs, externalBadges) => {
        ...
        try {
            ...
            if (attachments?.length) {
                ...
                processedAttachments = await Promise.all(successfulAttachments.map(async (att, i) => {
            ...
            const mentionBadges = windowWorkspaceSlug
                ...
                : [];
            const badges = [...(externalBadges || []), ...mentionBadges];
     ...
     */
    const handleSessionOptionsChange = useCallback((sessionId, updates) => {
        setSessionOptions(prev => {
            const next = new Map(prev);
            const current = {
                ...defaultSessionOptions,
    ...
    const draftSaveTimeoutRef = useRef(new Map());
    ...
    const getDraftAttachmentRefs = useCallback((sessionId) => {
    ...
    const hydrateDraftAttachments = useCallback(async (sessionId) => {
        const attachments = sessionDraftsRef.current.get(sessionId)?.attachments;
    ...
    const schedulePersistDraft = useCallback((sessionId) => {
    ...
    const handleInputChange = useCallback((sessionId, value) => {
    ...
    const handleAttachmentsChange = useCallback((sessionId, attachments) => {
        const existing = sessionDraftsRef.current.get(sessionId);
        const refs = [];
    ...
    const openNewChat = useCallback(async (params = {}) => {
        if (!windowWorkspaceId) {
            console.warn('[App] Cannot open new chat: no workspace ID');
            return;
        }
        navigate(routes.action.newSession(params.input || params.name
            ? {
    ...
    const handleRespondToPermission = useCallback(async (sessionId, requestId, allowed, alwaysAllow, options) => {
    ...
    const handleRespondToCredential = useCallback(async (sessionId, requestId, response) => {
    ...
    const handleOpenUrlInBuiltInBrowser = useCallback((url) => {
    ...
    const handleSelectWorkspace = useCallback(async (workspaceId, openInNewWindow = false, options) => {
        ...
        const runSwitch = async () => {
            ...
            flushSync(() => {
                ...
                applyLoadedSessions(loadedSessions, workspaceId, targetWorkspace?.remoteServer?.remoteWorkspaceId);
                setSessionsLoaded(true);
                setSessionListLoading(false);
            });
            void reconcileLoadedSessionPermissionModes(loadedSessions);
    ...
    const handleSwitchWorkspaceBySlug = useCallback((slug) => {
    ...
    const platformActions = useMemo(() => ({
        ...
        onReadFileDataUrl: (path) => window.electronAPI.readFileDataUrl(path),
        // Read file as binary Uint8Array (used by PDF preview blocks)
        onReadFileBinary: (path) => window.electronAPI.readFileBinary(path),
        ...
        onSetTrafficLightsVisible: (visible) => {
    ...
    if (appState === 'loading') {
        return _jsx(SplashScreen, { isExiting: false });
    }
    // Reauth state - session expired, need to re-login
    // ModalProvider + WindowCloseHandler ensures X button works on Windows
    if (appState === 'reauth') {
        return (_jsx(DismissibleLayerProvider, { children: _jsxs(ModalProvider, { children: [_jsx(WindowCloseHandler, {}), _jsx(ReauthScreen, { onLogin: handleReauthLogin, onReset: handleReauthReset }), _jsx(ResetConfirmationDialog, { open: showResetDialog, onConfirm: executeReset, onCancel: () => setShowResetDialog(false) })] }) }));
    }
    // Onboarding state
    // ModalProvider + WindowCloseHandler ensures X button works on Windows
    // (without this, the close IPC message has no listener and window stays open)
    if (appState === 'onboarding') {
        return (_jsx(DismissibleLayerProvider, { children: _jsxs(ModalProvider, { children: [_jsx(WindowCloseHandler, {}), _jsx(OnboardingWizard, { state: onboarding.state, onContinue: onboarding.handleContinue, onBack: onboarding.handleBack, onSelectProvider: onboarding.handleSelectProvider, onSkipSetup: onboarding.handleSkipSetup, onSelectApiSetupMethod: onboarding.handleSelectApiSetupMethod, onSubmitCredential: onboarding.handleSubmitCredential, onFinish: onboarding.handleFinish, onBrowseGitBash: onboarding.handleBrowseGitBash, onUseGitBashPath: onboarding.handleUseGitBashPath, onRecheckGitBash: onboarding.handleRecheckGitBash, onClearError: onboarding.handleClearError })] }) }));
    }
    // Workspace picker — thin client with no workspace selected
    if (appState === 'workspace-picker') {
        return (_jsx(DismissibleLayerProvider, { children: _jsxs(ModalProvider, { children: [_jsx(WindowCloseHandler, {}), _jsx(WorkspacePicker, { onSelectWorkspace: async (id) => {
    ...
    const isActiveSessionListLoading = sessionListLoading ||
    ...
    return (_jsx(PlatformProvider, { actions: platformActions, children: _jsx(ShikiThemeProvider, { shikiTheme: shikiTheme, children: _jsx(ActionRegistryProvider, { children: _jsx(FocusProvider, { children: _jsx(DismissibleLayerProvider, { children: _jsx(ModalProvider, { children: _jsx(TooltipProvider, { delayDuration: 0, children: _jsxs(NavigationProvider, { workspaceId: windowWorkspaceId, workspaceSlug: windowWorkspaceSlug, onSwitchWorkspaceBySlug: handleSwitchWorkspaceBySlug, onCreateSession: handleCreateSession, onInputChange: handleInputChange, getDraft: getDraft, onAutoDeleteEmptySession: handleAutoDeleteEmptySession, isReady: appState === 'ready', isSessionsReady: sessionsLoaded, remoteWorkspaceId: windowRemoteWorkspaceId, consumeWorkspaceSwitchRoute: consumePendingWorkspaceSwitchRoute, children: [_jsx(WindowCloseHandler, {}), showSplash && (_jsx(SplashScreen, { isExiting: splashExiting, onExitComplete: handleSplashExitComplete })), _jsxs("div", { className: "h-full flex flex-col text-foreground", children: [showTransportConnectionBanner && connectionState && (_jsx(TransportConnectionBanner, { state: connectionState, onRetry: handleReconnectTransport })), _jsxs("div", { className: "flex-1 min-h-0 flex flex-row", children: [_jsx("div", { className: "flex-1 min-w-0 h-full", children: sessionLoadError ? (_jsx(SessionLoadErrorScreen, { message: sessionLoadError, onRetry: () => { void loadSessionsFromServer(); } })) : (_jsx(AppShell, { contextValue: appShellContextValue, defaultLayout: [20, 32, 48], menuNewChatTrigger: menuNewChatTrigger, isFocusedMode: isFocusedMode, isSessionListLoading: isActiveSessionListLoading, onProjectSessionSnapshotsReadyChange: setProjectSessionSnapshotsReady })) }), linkInterceptor.previewState && (_jsx(FilePreviewPanel, { children: _jsx(FilePreviewRenderer, { state: linkInterceptor.previewState, onClose: linkInterceptor.closePreview, loadDataUrl: linkInterceptor.readFileDataUrl, loadPdfData: linkInterceptor.readFileBinary, isDark: isDark, embedded: true }) }))] }), _jsx(ResetConfirmationDialog, { open: showResetDialog, onConfirm: executeReset, onCancel: () => setShowResetDialog(false) })] })] }) }) }) }) }) }) }) }));
}
/**
 ...
 */
function FilePreviewRenderer({ state, onClose, loadDataUrl, loadPdfData, isDark, embedded = false, }) {
    const theme = isDark ? 'dark' : 'light';
    switch (state.type) {
        case 'image':
            return (_jsx(ImagePreviewOverlay, { isOpen: true, onClose: onClose, filePath: state.filePath, loadDataUrl: loadDataUrl, theme: theme, embedded: embedded }));
        case 'pdf':
            return (_jsx(PDFPreviewOverlay, { isOpen: true, onClose: onClose, filePath: state.filePath, loadPdfData: loadPdfData, theme: theme, embedded: embedded }));
        case 'code':
        case 'text':
            return (_jsx(CodePreviewOverlay, { isOpen: true, onClose: onClose, filePath: state.filePath, content: state.content ?? '', language: state.type === 'code' ? state.language : 'plaintext', mode: "read", theme: theme, error: state.error, embedded: embedded }));
        case 'markdown': {
            // Show PLAN header for .md files in plans folder (handles both absolute and relative paths)
            const isPlanFile = (state.filePath.includes('/plans/') || state.filePath.startsWith('plans/')) &&
                state.filePath.endsWith('.md');
            return (_jsx(DocumentFormattedMarkdownOverlay, { isOpen: true, onClose: onClose, content: state.content ?? '', filePath: state.filePath, variant: isPlanFile ? 'plan' : 'response', embedded: embedded }));
        }
        case 'json': {
            ...
            try {
                if (state.content)
                    parsedData = JSON.parse(state.content);
            }
            catch {
                // If parsing fails, fall back to showing as code
                return (_jsx(CodePreviewOverlay, { isOpen: true, onClose: onClose, filePath: state.filePath, content: state.content ?? '', language: "json", mode: "read", theme: theme, error: state.error, embedded: embedded }));
            }
            ...
            return (_jsx(JSONPreviewOverlay, { isOpen: true, onClose: onClose, filePath: state.filePath, title: state.filePath.split('/').pop() ?? 'JSON', data: parsedData, theme: theme, error: state.error, embedded: embedded }));
        }
        default:
            return null;
    }
