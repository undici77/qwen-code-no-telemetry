...
export function MessagingSettingsPagePreview({ telegramConnected, whatsappConnected, bindings, }) {
    ...
    React.useEffect(() => {
        setSessionMetaMap((prev) => {
            ...
            for (const meta of Object.values(MOCK_SESSION_META))
        ...
        return () => {
            setSessionMetaMap((prev) => {
                const next = new Map(prev);
                for (const id of Object.keys(MOCK_SESSION_META))
                    next.delete(id);
                return next;
            });
        };
    ...
    return _jsx(MessagingSettingsPage, {});
}
//# sourceMappingURL=MessagingSettingsPagePreview.js.map