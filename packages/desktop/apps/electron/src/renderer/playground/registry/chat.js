...
import { TurnCard } from '@craft-agent/ui';
...
import { ensureMockElectronAPI, mockInputCallbacks, mockAttachmentCallbacks, mockSources, sampleImageAttachment, samplePdfAttachment, } from '../mock-utils';
import { mockAdminApprovalRequest } from '../adapters/input-adapters';
import { getRecentDirsForScenario } from '../recent-working-dirs';
const sampleCodeAttachment = {
    type: 'text',
...
const veryLongPermissionRequest = {
    requestId: 'perm-3',
    sessionId: 'session-1',
    toolName: 'bash',
...
const inputContainerSampleLabels = [
...
const inputContainerSampleStatuses = [
    {
        id: 'todo',
        label: 'Todo',
        resolvedColor: 'var(--muted-foreground)',
        icon: _jsx(Circle, { className: "h-3.5 w-3.5", strokeWidth: 1.5 }),
        iconColorable: true,
        category: 'open',
    },
    {
...
const nestedActivitiesCompleted = [
    {
        id: 'task-1',
        type: 'tool',
        status: 'completed',
...
const nestedActivitiesInProgress = [
    {
        id: 'task-2',
        type: 'tool',
        status: 'running',
...
function InputContainerPlayground({ disabled = false, isProcessing = false, placeholder = 'Message Qwen Code...', currentModel = 'qwen3-coder-plus', permissionMode = 'ask', workingDirectory = '/Users/demo/projects/craft-agent', inputMode = 'freeform', compactMode = false, showOptionBadges = true, showTasks = true, showLabels = true, showStatuses = true, labelCount = 3, showSources = true, sourceCount = 2, showWorkingDirectory = true, seedRecentDirs = true, recentDirScenario = 'few', showAttachments = false, attachmentCount = 2, showFollowUps = false, followUpCount = 2, }) {
    const playgroundSessionId = 'playground-session';
    const [model, setModel] = React.useState(currentModel);
    const [mode, setMode] = React.useState(permissionMode);
    ...
    const [sessionLabels, setSessionLabels] = React.useState(['bug', 'priority::2', 'sprint::Q1-S3']);
    React.useEffect(() => {
        const next = labels.map((label) => {
            if (label.id === 'priority')
                return 'priority::2';
            if (label.id === 'due-date')
                return 'due-date::2026-03-15';
            if (label.id === 'sprint')
                return 'sprint::Q1-S3';
            return label.id;
        });
    ...
    const [enabledSourceSlugs, setEnabledSourceSlugs] = React.useState(defaultEnabled);
    React.useEffect(() => {
    ...
    React.useEffect(() => {
        if (!showAttachments || attachmentFiles.length === 0)
            return;
    ...
    return (_jsx(ModalProvider, { children: _jsx(AppShellProvider, { value: playgroundAppShellContext, children: _jsxs("div", { className: "w-full h-full flex flex-col bg-background", children: [_jsx("div", { className: "flex-1" }), _jsx(ChatInputZone, { compactMode: compactMode, showOptionBadges: showOptionBadges, permissionMode: mode, onPermissionModeChange: setMode, tasks: showTasks ? sampleBackgroundTasks : [], sessionId: playgroundSessionId, onKillTask: (taskId) => console.log('[Playground] Kill task:', taskId), onInsertMessage: setInputValue, sessionLabels: showLabels ? sessionLabels : [], labels: showLabels ? labels : [], onLabelsChange: setSessionLabels, sessionStatuses: showStatuses ? inputContainerSampleStatuses : [], currentSessionStatus: showStatuses ? currentSessionStatus : undefined, onSessionStatusChange: setCurrentSessionStatus, inputProps: {
                            placeholder,
                            disabled,
                            isProcessing,
                            structuredInput,
                        ...
                        } }, `input:${inputMode}:${compactMode ? 'compact' : 'full'}:${showAttachments ? attachmentSeedKey : 'none'}:${showFollowUps ? followUpCount : 0}:${seedRecentDirs ? recentDirScenario : 'unseeded'}`)] }) }) }));
}
function ActiveTasksBarContext({ tasks = sampleBackgroundTasks }) {
    const [permissionMode, setPermissionMode] = React.useState('ask');
    // Inject mock electronAPI for file attachments
    React.useEffect(() => {
        ensureMockElectronAPI();
    }, []);
    return (_jsxs("div", { className: "w-full max-w-[960px] h-full flex flex-col", children: [_jsxs("div", { className: "flex-1 overflow-auto px-5 py-8 space-y-2.5", children: [_jsx("div", { className: "pt-3 flex justify-end", children: _jsx("div", { className: "max-w-[80%] rounded-2xl bg-foreground text-background px-4 py-2", children: _jsx("p", { className: "text-sm", children: "Can you explore the codebase structure and analyze the API endpoints?" }) }) }), _jsx("div", { className: "flex justify-start", children: _jsx("div", { className: "max-w-[80%] rounded-2xl bg-muted px-4 py-2", children: _jsx("p", { className: "text-sm", children: "I'll explore the codebase and analyze the API endpoints. Let me start by running a background task to search for API route definitions..." }) }) })] }), _jsxs("div", { className: "mx-auto w-full px-4 pb-4 mt-1", style: { maxWidth: 'var(--content-max-width, 960px)' }, children: [_jsx(ActiveOptionBadges, { permissionMode: permissionMode, onPermissionModeChange: setPermissionMode, tasks: tasks, sessionId: "playground-session", onKillTask: (taskId) => console.log('[Playground] Kill task:', taskId) }), _jsx(InputContainer, { placeholder: "Message Qwen Code...", disabled: false, isProcessing: false, currentModel: "qwen3-coder-plus", permissionMode: permissionMode, onPermissionModeChange: setPermissionMode, sources: mockSources, enabledSourceSlugs: ['github-api', 'local-files'], workingDirectory: "/Users/demo/projects/craft-agent", sessionId: "playground-session", onSubmit: mockInputCallbacks.onSubmit, onModelChange: mockInputCallbacks.onModelChange, onInputChange: mockInputCallbacks.onInputChange, onHeightChange: mockInputCallbacks.onHeightChange, onFocusChange: mockInputCallbacks.onFocusChange, onSourcesChange: mockInputCallbacks.onSourcesChange, onWorkingDirectoryChange: mockInputCallbacks.onWorkingDirectoryChange, onStop: mockInputCallbacks.onStop })] })] }));
}
function PermissionInputToggle({ autoToggle = false, autoToggleInterval = 3000, useLongCommand = false }) {
    ...
    return (_jsxs("div", { className: "w-full max-w-[960px] h-full flex flex-col px-4 pb-4", children: [_jsx("div", { className: "flex-1" }), _jsxs("div", { className: "flex items-center gap-2 mb-20", children: [_jsxs(Button, { variant: showPermission ? 'default' : 'outline', size: "sm", onClick: () => setShowPermission(true), className: "gap-1.5", children: [_jsx(Sparkles, { className: "h-3.5 w-3.5" }), "Show Permission"] }), _jsx(Button, { variant: !showPermission ? 'default' : 'outline', size: "sm", onClick: () => setShowPermission(false), className: "gap-1.5", children: "Show Input" }), _jsx("div", { className: "flex-1" }), _jsxs("span", { className: "text-xs text-muted-foreground", children: ["Current: ", _jsx("span", { className: "font-medium", children: showPermission ? 'Permission Banner' : 'Input View' })] })] }), _jsx(ActiveOptionBadges, { permissionMode: permissionMode, onPermissionModeChange: setPermissionMode }), _jsx(InputContainer, { placeholder: "Message Qwen Code...", disabled: false, isProcessing: false, currentModel: "qwen3-coder-plus", permissionMode: permissionMode, onPermissionModeChange: setPermissionMode, sources: mockSources, enabledSourceSlugs: ['github-api', 'local-files'], workingDirectory: "/Users/demo/projects/craft-agent", sessionId: "playground-session", structuredInput: structuredInput, onStructuredResponse: handlePermissionResponse, onSubmit: mockInputCallbacks.onSubmit, onModelChange: mockInputCallbacks.onModelChange, onInputChange: mockInputCallbacks.onInputChange, onHeightChange: mockInputCallbacks.onHeightChange, onFocusChange: mockInputCallbacks.onFocusChange, onSourcesChange: mockInputCallbacks.onSourcesChange, onWorkingDirectoryChange: mockInputCallbacks.onWorkingDirectoryChange, onStop: mockInputCallbacks.onStop })] }));
}
...
export const chatComponents = [
    ...
    },
    {
        ...
        mockData: () => ({
            ...
            onPermissionModeChange: (mode) => console.log('[Playground] Permission mode changed:', mode),
