...
import { TurnCard, UserMessageBubble, } from '@craft-agent/ui';
// Import sample workflows
import { incidentResponseActivities, incidentResponseResponse, } from './samples/incident-response';
import { deploymentActivities, deploymentResponse, } from './samples/deployment';
import { customerSupportActivities, customerSupportResponse, } from './samples/customer-support';
...
function TurnCardModesDemo({ activities, response, userMessage, initialMode = 'detailed', }) {
    const [mode, setMode] = useState(initialMode);
    // Playback state
    const [isPlaying, setIsPlaying] = useState(false);
    const [playbackActivities, setPlaybackActivities] = useState([]);
    const [showResponse, setShowResponse] = useState(false);
    const playbackRef = useRef({ cancel: false });
    ...
    const startPlayback = useCallback(async () => {
        ...
        for (let i = 0; i < activities.length; i++) {
            ...
            if (playbackRef.current.cancel)
                return;
            // Mark as "completed"
            setPlaybackActivities(prev => prev.map((a, idx) => idx === prev.length - 1 ? { ...a, status: 'completed' } : a));
    ...
    return (_jsxs("div", { className: "space-y-4", children: [_jsxs("div", { className: "flex items-center gap-2 p-3 bg-muted/50 rounded-lg", children: [_jsx("button", { onClick: hasPlaybackStarted ? resetPlayback : startPlayback, className: "flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-md bg-background shadow-minimal text-foreground hover:bg-foreground/5 transition-colors", children: hasPlaybackStarted ? (_jsxs(_Fragment, { children: [_jsx(RotateCcw, { className: "w-3.5 h-3.5" }), "Reset"] })) : (_jsxs(_Fragment, { children: [_jsx(Play, { className: "w-3.5 h-3.5" }), "Play"] })) }), _jsx("div", { className: "w-px h-5 bg-border mx-2" }), _jsx("span", { className: "text-sm font-medium text-muted-foreground mr-2", children: "Display Mode:" }), _jsx("button", { onClick: () => setMode('informative'), className: `px-3 py-1.5 text-sm rounded-md transition-colors ${mode === 'informative'
                            ? 'bg-background shadow-minimal text-foreground'
                            : 'text-muted-foreground hover:text-foreground'}`, children: "Informative" }), _jsx("button", { onClick: () => setMode('detailed'), className: `px-3 py-1.5 text-sm rounded-md transition-colors ${mode === 'detailed'
                            ? 'bg-background shadow-minimal text-foreground'
                            : 'text-muted-foreground hover:text-foreground'}`, children: "Detailed" }), _jsx("span", { className: "ml-4 text-xs text-muted-foreground", children: mode === 'informative'
                            ? 'Hides MCP/API tool names and params, shows only source + intent'
                            : 'Shows full tool names, params, and all metadata' })] }), userMessage && (_jsx("div", { className: "pt-4", children: _jsx(UserMessageBubble, { content: userMessage }) })), _jsx(TurnCard, { sessionId: "playground-modes", turnId: "playground-turn", activities: displayActivities, response: displayResponse, isStreaming: isPlaying, isComplete: isComplete, defaultExpanded: true, displayMode: mode, animateResponse: hasPlaybackStarted, onOpenFile: (path) => console.log('[Playground] Open file:', path), onOpenUrl: (url) => console.log('[Playground] Open URL:', url), onOpenActivityDetails: (activity) => console.log('[Playground] Open activity details:', activity.id, activity.toolName) })] }));
}
// ============================================================================
// Component Registry
// ============================================================================
export const turnCardModesComponents = [
    {
...
//# sourceMappingURL=turn-card-modes.js.map