...
function CollapsibleWrapper({ children }) {
    return _jsx(CollapsibleMarkdownProvider, { children: children });
}
...
function MarkdownImageBlockWrapper({ children }) {
    return (_jsx(PlatformProvider, { actions: {
            onReadFileDataUrl: async (path) => {
...
function ImageCardStackPlayground({ items, maxRotate, }) {
    ...
    return (_jsxs("div", { className: "h-full w-full p-4 flex flex-col gap-3", children: [_jsxs("div", { className: "text-xs text-muted-foreground flex items-center justify-between", children: [_jsxs("span", { children: ["Active card: ", Math.min(currentIndex + 1, Math.max(parsedItems.length, 1)), " / ", parsedItems.length] }), _jsx("span", { children: "Swipe left/right on top card" })] }), _jsx("div", { className: "flex-1 min-h-0 rounded-md border border-border/60 bg-muted/20 p-4 flex items-center justify-center", children: parsedItems.length > 0 ? (_jsx("div", { className: "h-[320px] w-full", children: _jsx(ImageCardStack, { items: parsedItems, currentIndex: currentIndex, onIndexChange: setCurrentIndex, maxRotate: maxRotate, maxHeight: 320 }) })) : (_jsx("div", { className: "h-full w-full flex items-center justify-center text-sm text-muted-foreground", children: "Invalid or empty items JSON" })) })] }));
}
export const markdownComponents = [
    {
        ...
        mockData: () => ({
            onUrlClick: (url) => console.log('[Playground] URL clicked:', url),
            onFileClick: (path) => console.log('[Playground] File clicked:', path),
        }),
