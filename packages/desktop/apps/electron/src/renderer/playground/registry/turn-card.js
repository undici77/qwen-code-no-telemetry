...
import { TurnCard, DocumentFormattedMarkdownOverlay, CodePreviewOverlay, MultiDiffPreviewOverlay, TerminalPreviewOverlay, JSONPreviewOverlay, GenericOverlay, DataTableOverlay, } from '@craft-agent/ui';
/** Wrapper with padding for playground preview */
function PaddedWrapper({ children }) {
    return _jsx("div", { className: "p-8", children: children });
}
// ============================================================================
 ...
 */
function useStreamingSimulation(fullText, speed = 'normal') {
...
function StreamingSimulationTurnCard({ activities, intent, simulationSpeed = 'normal', }) {
    const { streamedText, isComplete } = useStreamingSimulation(streamingTextSample, simulationSpeed);
    const response = {
        text: streamedText,
        isStreaming: !isComplete,
    };
    return (_jsx(TurnCard, { sessionId: "playground-session", turnId: "playground-turn", activities: activities, response: response, intent: intent, isStreaming: !isComplete, isComplete: isComplete, onOpenFile: (path) => console.log('[Playground] Open file:', path), onOpenUrl: (url) => console.log('[Playground] Open URL:', url) }));
}
// ============================================================================
// Sample Data
// ============================================================================
...
const intermediateMessageRunning = {
...
const intermediateMessageStreaming = {
...
const toolVariety = [
    { tool: 'Read', getInput: (i) => ({ file_path: `/src/components/feature-${i}.tsx` }) },
    { tool: 'Grep', getInput: (i) => ({ pattern: `pattern${i}`, path: 'src/' }) },
    { tool: 'Glob', getInput: (i) => ({ pattern: `**/*${i}*.ts` }) },
    { tool: 'Bash', getInput: (i) => ({ command: `npm test -- file${i}` }) },
    { tool: 'Write', getInput: (i) => ({ file_path: `/src/utils/helper-${i}.ts` }) },
    { tool: 'Edit', getInput: (i) => ({ file_path: `/src/lib/module-${i}.ts` }) },
];
 ...
 */
function generateManyActivities(count) {
    const activities = [];
...
// Component Entry
// ============================================================================
export const turnCardComponents = [
    {
        id: 'turn-card',
        name: 'TurnCard',
        category: 'Turn Cards',
...
function DocumentFormattedMarkdownOverlayPlayground({ content, variant, }) {
    const [isOpen, setIsOpen] = useState(true);
    return (_jsxs("div", { className: "p-8", children: [_jsx("button", { onClick: () => setIsOpen(true), className: "px-4 py-2 bg-accent text-accent-foreground rounded-lg text-sm font-medium hover:bg-accent/90", children: "Open Document Overlay" }), _jsx(DocumentFormattedMarkdownOverlay, { content: content, isOpen: isOpen, onClose: () => setIsOpen(false), variant: variant, onOpenUrl: (url) => console.log('[Playground] Open URL:', url), onOpenFile: (path) => console.log('[Playground] Open file:', path) })] }));
}
// ============================================================================
...
function CodePreviewOverlayPlayground({ content, filePath, mode, startLine, totalLines, numLines, error, }) {
    return (_jsx("div", { className: "h-[500px]", children: _jsx(CodePreviewOverlay, { isOpen: true, onClose: () => { }, embedded: true, content: content, filePath: filePath, mode: mode, startLine: startLine, totalLines: totalLines, numLines: numLines, error: error }) }));
}
// ============================================================================
// Multi-Diff Preview Overlay - Playground Wrapper
// ============================================================================
const sampleMultiDiffChanges = [
    {
        id: 'change-1',
...
function MultiDiffPreviewOverlayPlayground({ changes, consolidated, focusedChangeId, }) {
    return (_jsx("div", { className: "h-[500px]", children: _jsx(MultiDiffPreviewOverlay, { isOpen: true, onClose: () => { }, embedded: true, changes: changes, consolidated: consolidated, focusedChangeId: focusedChangeId }) }));
}
// ============================================================================
...
function TerminalPreviewOverlayPlayground({ command, output, exitCode, toolType, description, error, }) {
    return (_jsx("div", { className: "h-[500px]", children: _jsx(TerminalPreviewOverlay, { isOpen: true, onClose: () => { }, embedded: true, command: command, output: output, exitCode: exitCode, toolType: toolType, description: description, error: error }) }));
}
// ============================================================================
...
function JSONPreviewOverlayPlayground({ data, title, error, }) {
    return (_jsx("div", { className: "h-[500px]", children: _jsx(JSONPreviewOverlay, { isOpen: true, onClose: () => { }, embedded: true, data: data, title: title, error: error }) }));
}
// ============================================================================
...
function GenericOverlayPlayground({ content, title, }) {
    return (_jsx("div", { className: "h-[500px]", children: _jsx(GenericOverlay, { isOpen: true, onClose: () => { }, embedded: true, content: content, title: title }) }));
}
// ============================================================================
...
function SampleDataTable() {
    ...
    return (_jsxs("table", { className: "w-full text-sm", children: [_jsx("thead", { children: _jsxs("tr", { className: "border-b border-border", children: [_jsx("th", { className: "text-left py-2 px-4 font-medium text-muted-foreground", children: "Tool" }), _jsx("th", { className: "text-left py-2 px-4 font-medium text-muted-foreground", children: "Permission" }), _jsx("th", { className: "text-left py-2 px-4 font-medium text-muted-foreground", children: "Description" })] }) }), _jsx("tbody", { children: rows.map((row) => (_jsxs("tr", { className: "border-b border-border/50", children: [_jsx("td", { className: "py-2 px-4 font-mono text-xs", children: row.tool }), _jsx("td", { className: "py-2 px-4", children: _jsx("span", { className: `text-xs px-2 py-0.5 rounded-full ${row.permission === 'allowed' ? 'bg-green-500/10 text-green-600' :
                                    row.permission === 'ask' ? 'bg-amber-500/10 text-amber-600' :
                                        'bg-red-500/10 text-red-600'}`, children: row.permission }) }), _jsx("td", { className: "py-2 px-4 text-muted-foreground", children: row.description })] }, row.tool))) })] }));
}
/** Wrapper with button to open DataTableOverlay */
function DataTableOverlayPlayground({ title, subtitle, }) {
    const [isOpen, setIsOpen] = useState(true);
    return (_jsxs("div", { className: "p-8", children: [_jsx("button", { onClick: () => setIsOpen(true), className: "px-4 py-2 bg-accent text-accent-foreground rounded-lg text-sm font-medium hover:bg-accent/90", children: "Open Data Table" }), _jsx(DataTableOverlay, { isOpen: isOpen, onClose: () => setIsOpen(false), title: title, subtitle: subtitle, children: _jsx(SampleDataTable, {}) })] }));
}
// ============================================================================
// Fullscreen Overlay Component Registry
// ============================================================================
/** Export all fullscreen overlay components */
export const fullscreenOverlayComponents = [
    {
        id: 'document-overlay',
        ...
        variants: [
            ...
            },
            {
                ...
                props: {
                    ...
                    mode: 'read',
                    error: "Bash command `python3 -c import re; f=open('./node_modules/@qwen-code/qwen-code/dist/cli.js','r'); content=f.read(); f.close(); [print(m.group(),'---') for m in list(re.finditer(r'.{0,150}AbortError.{0,150}', content))[:5]]` is not in the read-only allowlist.\n\nMatched: `python3 -` (9 chars)\nFailed at: `c` (position 9)\n\nPattern: Python 3 version",
                },
            },
        ],
        mockData: () => ({
...
//# sourceMappingURL=turn-card.js.map