...
import { getEventDisplayName } from '@/components/automations/types';
// ============================================================================
// Wrappers
// ============================================================================
function PaddedWrapper({ children }) {
    return _jsx("div", { className: "p-6", children: children });
}
/** Stateful wrapper for AutomationsListPanel */
function AutomationsListPanelPlayground({ automations, selectedAutomationId: initialSelectedId, }) {
    const [selectedId, setSelectedId] = useState(initialSelectedId ?? null);
    const [automationList, setAutomationList] = useState(automations);
    return (_jsx(AutomationsListPanel, { automations: automationList, selectedAutomationId: selectedId, onAutomationClick: (id) => setSelectedId(id), onDeleteAutomation: (id) => {
            setAutomationList(prev => prev.filter(h => h.id !== id));
            if (selectedId === id)
                setSelectedId(null);
        }, onToggleAutomation: (id) => {
            setAutomationList(prev => prev.map(h => h.id === id ? { ...h, enabled: !h.enabled } : h));
        }, onTestAutomation: (id) => console.log('[Playground] Test automation:', id), onDuplicateAutomation: (id) => console.log('[Playground] Duplicate automation:', id) }));
}
/** Stateful wrapper for AutomationInfoPage with test simulation */
function AutomationInfoPagePlayground({ automation, executions, }) {
    const [currentAutomation, setCurrentAutomation] = useState(automation);
    const [testResult, setTestResult] = useState({ state: 'idle' });
    const handleTest = () => {
    ...
    return (_jsx(AutomationInfoPage, { automation: currentAutomation, executions: executions, testResult: testResult, onToggleEnabled: () => setCurrentAutomation(prev => ({ ...prev, enabled: !prev.enabled })), onTest: handleTest, onDuplicate: () => console.log('[Playground] Duplicate'), onDelete: () => console.log('[Playground] Delete') }));
}
/** Stateful wrapper for CronBuilder */
function CronBuilderPlayground({ initialValue, timezone, }) {
    const [value, setValue] = useState(initialValue ?? '0 9 * * 1-5');
    return (_jsx(CronBuilder, { value: value, onChange: setValue, timezone: timezone }));
}
...
function AutomationAvatarGallery() {
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { children: [_jsx("h4", { className: "text-xs font-medium text-muted-foreground uppercase tracking-wider mb-3", children: "Sizes" }), _jsx("div", { className: "flex items-end gap-4", children: ['xs', 'sm', 'md', 'lg'].map(size => (_jsxs("div", { className: "flex flex-col items-center gap-1", children: [_jsx(AutomationAvatar, { event: "SchedulerTick", size: size }), _jsx("span", { className: "text-[10px] text-muted-foreground", children: size })] }, size))) })] }), _jsxs("div", { children: [_jsx("h4", { className: "text-xs font-medium text-muted-foreground uppercase tracking-wider mb-3", children: "All Events" }), _jsx("div", { className: "grid grid-cols-4 gap-3", children: events.map(event => (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(AutomationAvatar, { event: event, size: "md" }), _jsx("span", { className: "text-xs text-foreground/70 truncate", children: getEventDisplayName(event) })] }, event))) })] })] }));
}
/** Stateful wrapper for AutomationCard */
function AutomationCardPlayground({ automation, defaultExpanded, }) {
    const [currentAutomation, setCurrentAutomation] = useState(automation);
    return (_jsx(AutomationCard, { automation: currentAutomation, defaultExpanded: defaultExpanded, onToggleEnabled: (enabled) => setCurrentAutomation(prev => ({ ...prev, enabled })), onTest: () => console.log('[Playground] Test automation:', currentAutomation.id) }));
}
// ============================================================================
// Mock Data
// ============================================================================
...
const mockWebhookExecutions = mockExecutions.filter(e => !!e.webhookDetails);
        ...
        variants: [
            ...
            },
            {
                ...
                props: {
                    ...
                    onReplay: (automationId, event) => console.log('[Playground] Replay:', automationId, event),
                },
            },
            {
                name: 'All Success',
