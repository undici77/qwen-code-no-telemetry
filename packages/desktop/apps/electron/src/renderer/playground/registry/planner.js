...
import { CalendarDays, CheckCircle2, Circle, Clock3, CloudAlert, CloudCheck, CloudOff, CloudUpload, History, MoreHorizontal, Link2, ListTodo, User, } from 'lucide-react';
...
import { PANEL_GAP, PANEL_SASH_HALF_HIT_WIDTH, PANEL_SASH_HIT_WIDTH, PANEL_SASH_LINE_WIDTH, } from '@/components/app-shell/panel-constants';
import './planner.css';
const projects = [
...
const events = [
    ...
    { id: 'e4', taskId: 't1', type: 'task.session_snapshot_updated', at: 'Today · 18:42', actor: 'Qwen Code', payloadSummary: 'Refreshed snapshot summary' },
];
const stateStyles = {
...
function buildFlatOrder(projectHeadings, tasks) {
    const rows = [];
...
function parseHeadingIdFromKey(rowKey) {
    return rowKey.startsWith('heading:') ? rowKey.slice('heading:'.length) : null;
}
function parseTaskIdFromKey(rowKey) {
...
function PlannerBoard() {
    ...
    const [headingsState, setHeadingsState] = React.useState(initialHeadings);
    const [tasksState, setTasksState] = React.useState(initialTasks);
    ...
    const [navigatorHandleY, setNavigatorHandleY] = React.useState(null);
    const sidebarHandleRef = React.useRef(null);
    const navigatorHandleRef = React.useRef(null);
    const [flatOrder, setFlatOrder] = React.useState(() => {
    ...
    const sortableRegistryRef = React.useRef(new Map());
    ...
    const projectHeadings = React.useMemo(() => headingsState.filter(h => h.projectId === project.id).sort((a, b) => a.sortOrder - b.sortOrder), [headingsState, project.id]);
    ...
    const flatRows = React.useMemo(() => {
        ...
        return flatOrder
            ...
            if (headingId) {
                if (!headingById.has(headingId))
                    return null;
                return { kind: 'heading', headingId, key: rowKey };
            }
            ...
            if (taskId) {
                ...
                if (!task)
                    return null;
                if (!headingById.has(task.headingId))
                    return null;
                return { kind: 'task', taskId, key: rowKey };
            }
            return null;
        })
    ...
    const applyFlatOrderToState = React.useCallback((orderedKeys) => {
        ...
        const projectHeadingIds = new Set(headings
            .filter(heading => heading.projectId === activeProject)
            .map(heading => heading.id));
        const orderedHeadingIds = uniqueOrdered(normalizedOrderedKeys
            .map(parseHeadingIdFromKey)
            .filter((headingId) => Boolean(headingId && projectHeadingIds.has(headingId))));
        if (orderedHeadingIds.length === 0)
            return;
        const orderedTaskIds = [];
        const seenTaskIds = new Set();
        const nextHeadingByTaskId = new Map();
        ...
        normalizedOrderedKeys.forEach((rowKey) => {
            ...
            if (!task)
                return;
            if (!projectHeadingIds.has(task.headingId))
                return;
        ...
        setHeadingsState((prev) => {
            ...
            return prev.map((heading) => {
                if (heading.projectId !== activeProject)
                    return heading;
        ...
        setTasksState((prev) => {
            const updated = prev.map((task) => {
                ...
                if (!nextHeadingId || task.headingId === nextHeadingId)
                    return task;
                return { ...task, headingId: nextHeadingId };
            });
            const byId = new Map(updated.map(task => [task.id, task]));
            const ordered = orderedTaskIds.map(taskId => byId.get(taskId)).filter((task) => Boolean(task));
    ...
    React.useEffect(() => {
        ...
        const unsubDragEnd = manager.monitor.addEventListener('dragend', (event) => {
            ...
            if (!sourceId.startsWith('heading:') && !sourceId.startsWith('task:'))
                return;
            const list = flatListRef.current;
            if (!list)
                return;
            const orderedKeys = uniqueOrdered(Array.from(list.children)
    ...
    const updateTaskTitle = React.useCallback((taskId, newTitle) => {
        const trimmed = newTitle.trim();
        if (!trimmed)
            return;
        setTasksState(prev => prev.map(t => t.id === taskId ? { ...t, title: trimmed } : t));
    }, []);
    const updateTaskNotes = React.useCallback((taskId, newNotes) => {
    ...
    const addTask = React.useCallback(() => {
        ...
        if (!title || projectHeadings.length === 0)
            return;
        const task = {
            id: `t-${Date.now()}`,
    ...
    return (_jsxs("div", { className: "w-full h-full flex relative", style: { gap: 6, padding: 4 }, children: [_jsxs("aside", { className: "p-3 shrink-0 overflow-hidden", style: { width: sidebarWidth }, children: [_jsxs("div", { className: "mb-3 flex items-center gap-2 px-2 py-1", children: [_jsx(ListTodo, { className: "h-4 w-4 text-foreground/60" }), _jsx("span", { className: "text-sm font-semibold", children: "Planner" })] }), _jsx("div", { className: "space-y-1", children: projects.map(p => (_jsxs("button", { onClick: () => setActiveProjectId(p.id), className: cn('w-full rounded-[10px] px-2.5 py-2 text-left transition-colors', p.id === project.id
                                ? 'bg-foreground/10 text-foreground'
                                : 'text-foreground/65 hover:bg-foreground/5 hover:text-foreground'), children: [_jsx("div", { className: "text-sm font-medium", children: p.name }), _jsx("div", { className: "mt-0.5 text-[11px] text-foreground/45", children: p.installationHint })] }, p.id))) })] }), _jsxs("section", { className: "bg-background shadow-middle rounded-[10px] overflow-hidden shrink-0 flex flex-col", style: { width: navigatorWidth }, children: [_jsxs("div", { className: "border-b border-border/60 px-5 py-3", children: [_jsx("h3", { className: "mb-2 text-base font-semibold", children: project.name }), _jsxs("div", { className: "flex gap-2", children: [_jsx(Input, { value: quickAdd, onChange: (e) => setQuickAdd(e.target.value), onKeyDown: (e) => {
                                            if (e.key === 'Enter')
                                                addTask();
                                        }, placeholder: "Quick add task\u2026", className: "h-8 text-sm" }), _jsx(Button, { size: "sm", className: "h-8", onClick: addTask, children: "Add" })] })] }), _jsx(ScrollArea, { className: "flex-1", onPointerDown: (e) => {
                            const target = e.target;
                            if (target.closest('[data-row-key]'))
                                return;
                            setSelectedTaskId('');
                        }, children: _jsxs("div", { className: "px-5 py-4 min-h-full flex flex-col", children: [_jsx("div", { ref: flatListRef, className: "flex flex-col gap-1.5 rounded-[10px] p-1", children: flatRows.map((row, index) => {
                                        if (row.kind === 'heading') {
                                            const heading = projectHeadings.find(h => h.id === row.headingId);
                                            if (!heading)
                                                return null;
                                            return (_jsx("div", { "data-row-key": row.key, ref: (el) => {
                                                    if (el)
                                                        rowRefs.current.set(row.key, el);
                                                    else
                                                        rowRefs.current.delete(row.key);
                                                }, className: cn('w-full select-none', index === 0 ? 'pt-1' : 'pt-3'), children: _jsxs("div", { className: "flex items-center justify-between gap-2 border-b border-border/70 pb-1.5 px-1", children: [_jsx("div", { className: "text-[13px] font-semibold text-foreground", children: heading.title }), _jsxs(DropdownMenu, { modal: false, children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx("button", { type: "button", "data-no-dnd": "true", onPointerDown: (e) => e.stopPropagation(), onClick: (e) => e.stopPropagation(), className: "h-6 w-6 inline-flex items-center justify-center rounded-[6px] hover:bg-foreground/5 data-[state=open]:bg-foreground/5", "aria-label": `Open ${heading.title} menu`, children: _jsx(MoreHorizontal, { className: "h-4 w-4 text-foreground/45" }) }) }), _jsxs(StyledDropdownMenuContent, { align: "end", minWidth: "min-w-44", children: [_jsx(StyledDropdownMenuItem, { onClick: (e) => e.preventDefault(), children: _jsx("span", { className: "flex-1", children: "Rename section" }) }), _jsx(StyledDropdownMenuItem, { onClick: (e) => e.preventDefault(), children: _jsx("span", { className: "flex-1", children: "Add task below" }) }), _jsx(StyledDropdownMenuItem, { onClick: (e) => e.preventDefault(), children: _jsx("span", { className: "flex-1", children: "Delete section" }) })] })] })] }) }, row.key));
                                        }
                                        ...
                                        return (_jsx("div", { "data-row-key": row.key, ref: (el) => {
                                                if (el)
                                                    ...
                                                    rowRefs.current.delete(row.key);
                                            }, onClick: () => { if (!isDraggingRef.current)
                                                setSelectedTaskId(task.id); }, className: cn('planner-sortable-item w-full rounded-[8px] px-3 py-2 text-left select-none', selectedTaskId === task.id
                                                ? 'planner-sortable-item--selected bg-background'
                                                : 'bg-background shadow-none'), children: _jsxs("div", { className: "flex items-center gap-2", children: [task.state === 'done' ? (_jsx(CheckCircle2, { className: cn('h-4 w-4', stateStyles[task.state]) })) : (_jsx(Circle, { className: cn('h-4 w-4', stateStyles[task.state]) })), _jsx("span", { className: cn('min-w-0 flex-1 truncate text-sm', task.state === 'done' && 'line-through text-foreground/45'), children: task.title }), _jsx("span", { className: "text-[11px] text-foreground/45", children: task.due })] }) }, row.key));
                                    }) }), _jsx("div", { className: "flex-1 min-h-12", "aria-hidden": "true" })] }) })] }), _jsx("aside", { className: "bg-foreground-2 shadow-middle rounded-[10px] overflow-hidden flex-1 min-w-0 flex flex-col", children: !selectedTask ? (_jsx("div", { className: "p-5 text-sm text-foreground/50", children: "Select a task to inspect details." })) : (_jsx(ScrollArea, { className: "flex-1", children: _jsxs("div", { className: "w-full max-w-[720px] mx-auto px-6 pt-8 pb-5", children: [_jsx("div", { contentEditable: true, suppressContentEditableWarning: true, onBlur: (e) => updateTaskTitle(selectedTask.id, e.currentTarget.textContent ?? ''), className: "text-xl font-bold leading-snug outline-none mb-5", children: selectedTask.title }, selectedTask.id + '-title'), _jsxs("div", { className: "flex items-center gap-2 flex-wrap mb-5", children: [_jsx(MetadataBadge, { interactive: false, icon: selectedTask.state === 'done'
                                            ? _jsx(CheckCircle2, { className: cn('h-3.5 w-3.5', stateStyles[selectedTask.state]) })
                                            : _jsx(Circle, { className: cn('h-3.5 w-3.5', stateStyles[selectedTask.state]) }), label: stateLabels[selectedTask.state] }), _jsx(MetadataBadge, { interactive: false, icon: _jsx(CalendarDays, { className: "h-3.5 w-3.5 text-foreground/55" }), label: "Due", value: selectedTask.due }), selectedHeading && (_jsx(MetadataBadge, { interactive: false, label: "Section", value: selectedHeading.title })), _jsx(MetadataBadge, { interactive: false, icon: _jsx(Link2, { className: "h-3.5 w-3.5 text-foreground/55" }), label: "Sessions", value: String(selectedLinks.length) })] }), _jsx(TiptapMarkdownEditor, { content: selectedTask.notes, onUpdate: (md) => updateTaskNotes(selectedTask.id, md), placeholder: "Add notes...", markdownEngine: "official", className: "text-sm leading-relaxed text-foreground/75" }, selectedTask.id + '-notes'), _jsx("div", { className: "border-t border-border/60 mt-5 mb-1" }), _jsx("div", { className: "mb-1 text-sm font-semibold", children: "Attached Sessions" }), _jsx("div", { className: "space-y-2 mt-3", children: selectedLinks.length === 0 ? (_jsx("div", { className: "text-xs text-foreground/40", children: "No sessions linked." })) : (selectedLinks.map(link => {
                                    const snap = snapshots.find(s => s.id === link.snapshotId);
                                    if (!snap)
                                        return null;
                                    const meta = syncMeta[link.syncState];
                                    const Icon = meta.icon;
                                    return (_jsxs("div", { className: "rounded-lg border border-border/60 p-2.5 bg-foreground/[0.01]", children: [_jsxs("div", { className: "mb-1 flex items-center justify-between gap-2", children: [_jsxs("div", { className: "min-w-0 flex items-center gap-1.5", children: [_jsx(Link2, { className: "h-3.5 w-3.5 text-foreground/45" }), _jsx("span", { className: "truncate text-xs font-medium", children: snap.title })] }), _jsxs("span", { className: cn('inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px]', meta.cls), children: [_jsx(Icon, { className: "h-3 w-3" }), meta.label] })] }), _jsx("p", { className: "text-[11px] text-foreground/60 leading-relaxed", children: snap.summary }), _jsxs("div", { className: "mt-1 text-[10px] text-foreground/45", children: ["Updated ", snap.lastUpdated] })] }, link.id));
                                })) }), _jsx("div", { className: "border-t border-border/60 my-5" }), _jsx("div", { className: "mb-3 text-sm font-semibold", children: "Activity" }), _jsx("div", { className: "space-y-3", children: selectedEvents.map(ev => (_jsxs("div", { className: "flex items-start gap-2.5", children: [_jsx("div", { className: "mt-0.5 h-5 w-5 rounded-full bg-foreground/8 flex items-center justify-center shrink-0", children: _jsx(History, { className: "h-3 w-3 text-foreground/45" }) }), _jsxs("div", { className: "min-w-0 flex-1", children: [_jsx("div", { className: "text-xs text-foreground/75", children: ev.payloadSummary }), _jsxs("div", { className: "mt-0.5 text-[11px] text-foreground/40", children: [ev.actor, " \u00B7 ", ev.at] })] })] }, ev.id))) })] }) })) }), _jsx("div", { ref: sidebarHandleRef, onMouseDown: (e) => { e.preventDefault(); setIsResizing('sidebar'); }, onMouseMove: (e) => {
                    if (sidebarHandleRef.current) {
                        const rect = sidebarHandleRef.current.getBoundingClientRect();
                        setSidebarHandleY(e.clientY - rect.top);
                    }
                }, onMouseLeave: () => { if (isResizing !== 'sidebar')
                    setSidebarHandleY(null); }, className: "absolute top-0 h-full cursor-col-resize z-10 flex justify-center", style: {
                    ...
                    } }) }), _jsx("div", { ref: navigatorHandleRef, onMouseDown: (e) => { e.preventDefault(); setIsResizing('navigator'); }, onMouseMove: (e) => {
                    if (navigatorHandleRef.current) {
                        const rect = navigatorHandleRef.current.getBoundingClientRect();
                        setNavigatorHandleY(e.clientY - rect.top);
                    }
                }, onMouseLeave: () => { if (isResizing !== 'navigator')
                    setNavigatorHandleY(null); }, className: "absolute top-0 h-full cursor-col-resize z-10 flex justify-center", style: {
                    width: PANEL_SASH_HIT_WIDTH,
...
function PlannerSyncStatePalette() {
    return (_jsxs("div", { className: "w-[820px] rounded-[14px] border border-border bg-background p-4", children: [_jsxs("div", { className: "mb-3 flex items-center gap-2 text-sm font-semibold", children: [_jsx(CalendarDays, { className: "h-4 w-4 text-foreground/60" }), "Sync States (task_session_links_local)"] }), _jsx("div", { className: "grid grid-cols-2 gap-2.5", children: Object.keys(syncMeta).map((state) => {
                    const meta = syncMeta[state];
                    const Icon = meta.icon;
                    return (_jsxs("div", { className: "rounded-[10px] border border-border/60 bg-foreground/[0.015] p-3", children: [_jsxs("div", { className: "mb-1.5 flex items-center gap-1.5 text-sm font-medium", children: [_jsx(Icon, { className: cn('h-4 w-4', meta.cls.split(' ')[0]) }), meta.label] }), _jsx("div", { className: "text-xs text-foreground/60", children: "Snapshot card always visible; live session resolution is optional enhancement." })] }, state));
                }) })] }));
}
