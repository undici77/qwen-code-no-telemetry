...
const noopKeyDown = () => { };
function SessionListSearchPreview({ searchQuery = '', showSearchInput = true, isSearching = false, selectedIndex = 0, chatMatchCount, showNoResults = false, resultCount, }) {
    ...
    const contentSearchResults = new Map();
    ...
    return (_jsx(ActionRegistryProvider, { children: _jsx(SessionListProvider, { value: ctx, children: _jsxs("div", { className: "w-[320px] h-[480px] flex flex-col border border-border rounded-lg overflow-hidden bg-background", children: [showSearchInput && (_jsx(SessionSearchHeader, { searchQuery: searchQuery, isSearching: isSearching, resultCount: displayCount, readOnly: true })), _jsx("div", { className: "flex-1 overflow-auto", children: showNoResults ? (_jsxs("div", { className: "flex flex-col items-center justify-center py-12 px-4", children: [_jsx("p", { className: "text-sm text-muted-foreground", children: "No conversations found" }), _jsx("p", { className: "text-xs text-muted-foreground/60 mt-0.5", children: "Searched titles and message content" }), _jsx("button", { className: "text-xs text-foreground hover:underline mt-2", children: "Clear search" })] })) : (_jsxs("div", { className: "flex flex-col pb-4", children: [_jsx("div", { className: "px-4 py-2", children: _jsx("span", { className: "text-[11px] font-medium text-muted-foreground uppercase tracking-wider", children: "Today" }) }), filteredSessions.map((session, index) => (_jsx(SessionItem, { item: session, index: index, itemProps: { tabIndex: 0, onKeyDown: noopKeyDown }, isSelected: session.id === selectedSessionId, isFirstInGroup: index === 0, isInMultiSelect: false, onSelect: () => { } }, session.id)))] })) })] }) }) }));
}
function SessionItemPreview({ item = sampleSessions[0], isSelected = false, searchQuery = '', chatMatchCount, state = 'none', flagged = false, }) {
    const resolvedItem = React.useMemo(() => {
        const base = { ...item, isProcessing: false, hasUnread: false, lastMessageRole: undefined, isFlagged: flagged };
        switch (state) {
            case 'loading':
                base.isProcessing = true;
                break;
            case 'plan':
                base.lastMessageRole = 'plan';
                break;
            case 'new':
                base.hasUnread = true;
                break;
    ...
    return (_jsx(ActionRegistryProvider, { children: _jsx(SessionListProvider, { value: ctx, children: _jsx(SessionItem, { item: resolvedItem, index: 0, itemProps: { tabIndex: 0, onKeyDown: noopKeyDown }, isSelected: isSelected, isFirstInGroup: true, isInMultiSelect: false, onSelect: () => { } }) }) }));
}
// ============================================================================
// Registry Entries
// ============================================================================
export const sessionListComponents = [
    {
