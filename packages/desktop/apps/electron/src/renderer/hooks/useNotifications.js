 ...
 */
function drawBadgeOnIcon(iconDataUrl, count) {
 ...
 */
function drawWindowsBadgeOverlay(count) {
    const canvas = document.createElement('canvas');
    const size = 32;
    canvas.width = size;
    canvas.height = size;
...
export function useNotifications({ workspaceId, onNavigateToSession, enabled = true, }) {
    ...
    const hasGuiChannels = useMemo(() => window.electronAPI.isChannelAvailable(RPC_CHANNELS.notification.SHOW), []);
    // Keep ref updated
    useEffect(() => {
    ...
    const showSessionNotification = useCallback((session, messagePreview) => {
        ...
        let body = messagePreview || 'Qwen Code has a new message for you';
        if (body.length > 100) {
