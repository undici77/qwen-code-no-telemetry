export function hasSessionContentHint(session) {
    if (!session)
        return false;
...
export function shouldShowForegroundMessageLoading(messagesLoaded, visibleMessageCount, expectedMessageCount, hasContentHint = false) {
    if ((visibleMessageCount ?? 0) > 0)
        return false;
    if (expectedMessageCount === 0 && (messagesLoaded || !hasContentHint))
        return false;
    if (messagesLoaded)
        return hasContentHint && expectedMessageCount !== 0;
    return true;
}
export function shouldShowMissingSessionState({ hasSession, hasSessionMeta, missingForMs, confirmationDelayMs, }) {
    if (hasSession || hasSessionMeta)
        return false;
    return missingForMs >= confirmationDelayMs;
}
export function shouldTreatSessionLoadFailureAsTransportFallback(state) {
    if (!state || state.mode !== 'remote')
        return false;
...
export function formatSessionLoadFailure(error) {
...
function mergeMessageById(existingMessage, freshMessage) {
...
function mergeMessagesById(existingMessages, freshMessages) {
...
export function mergeSessionRefreshResult(existingSession, freshSession) {
    ...
    const shouldMergeProcessingSnapshot = freshMessages.length <= existingSession.messages.length
...
//# sourceMappingURL=session-load.js.map