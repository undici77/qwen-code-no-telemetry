...
const mockReadWorkspaceImage = mock((workspaceId, path) => Promise.resolve(null));
// We need to mock the window object before importing the module
const originalWindow = globalThis.window;
beforeEach(() => {
    // Reset mock
    mockReadWorkspaceImage.mockReset();
    mockReadWorkspaceImage.mockImplementation((_workspaceId, _path) => Promise.resolve(null));
    globalThis.window = {
        electronAPI: {
...
describe('icon-cache null handling', () => {
    describe('when IPC returns null for missing file', () => {
        ...
        it('IPC returning null should not throw', async () => {
            ...
            await expect(mockReadWorkspaceImage('workspace-id', 'sources/test/icon.svg')).resolves.toBeNull();
        });
    });
    ...
    describe('error scenarios', () => {
        it('should handle IPC errors gracefully', async () => {
            ...
            await expect(mockReadWorkspaceImage('workspace-id', 'icon.svg')).rejects.toThrow('IPC failed');
        });
    });
});
describe('remote icon URLs', () => {
    ...
    it('returns skill icon URLs with uppercase schemes directly', async () => {
        ...
        await expect(loadSkillIcon({
            slug: 'skill',
            metadata: { icon },
        }, 'workspace-id')).resolves.toBe(icon);
        expect(mockReadWorkspaceImage).not.toHaveBeenCalled();
    });
});
// ============================================================================
...
//# sourceMappingURL=icon-cache.test.js.map