...
import { formatSessionLoadFailure, hasSessionContentHint, mergeSessionRefreshResult, shouldShowMissingSessionState, shouldShowForegroundMessageLoading, shouldTreatSessionLoadFailureAsTransportFallback, } from '../session-load';
...
function message(id, content = id) {
    return {
        id,
        role: 'assistant',
        content,
...
describe('shouldTreatSessionLoadFailureAsTransportFallback', () => {
    it('returns true for remote reconnecting state', () => {
        expect(shouldTreatSessionLoadFailureAsTransportFallback(createState({ status: 'reconnecting' }))).toBe(true);
    });
    it('returns true for remote auth/network/timeout failures', () => {
        expect(shouldTreatSessionLoadFailureAsTransportFallback(createState({
            status: 'connected',
            lastError: { kind: 'auth', message: 'Bad token' },
        }))).toBe(true);
    });
    it('returns false for remote connected state without transport errors', () => {
        expect(shouldTreatSessionLoadFailureAsTransportFallback(createState({ status: 'connected' }))).toBe(false);
    });
    it('returns false for local transport state', () => {
        expect(shouldTreatSessionLoadFailureAsTransportFallback(createState({ mode: 'local', status: 'failed' }))).toBe(false);
    });
});
...
//# sourceMappingURL=session-load.test.js.map