 ...
 */
export const logoUrlCache = new Map();
// ============================================================================
...
export const sourceIconCache = {
    get: (key) => iconCache.get(`source:${key}`),
    set: (key, value) => iconCache.set(`source:${key}`, value),
    has: (key) => iconCache.has(`source:${key}`),
    delete: (key) => iconCache.delete(`source:${key}`),
    clear: () => {
...
export const skillIconCache = {
    get: (key) => iconCache.get(`skill:${key}`),
    set: (key, value) => iconCache.set(`skill:${key}`, value),
    has: (key) => iconCache.has(`skill:${key}`),
    delete: (key) => iconCache.delete(`skill:${key}`),
    clear: () => {
 ...
 * @deprecated Will be removed once rich-text-input.tsx is migrated to useEntityIcon.
 */
export function clearSkillIconCaches() {
 ...
 */
export async function loadSourceIcon(source) {
 ...
 */
async function loadWorkspaceIcon(workspaceId, relativePath) {
    try {
 ...
 */
export function getSourceIconSync(workspaceId, slug) {
 ...
 */
export async function loadSkillIcon(skill, workspaceId) {
 ...
 */
export function getSkillIconSync(workspaceId, slug) {
    const cacheKey = `${workspaceId}:${slug}`;
 ...
 */
export function themeSvgContent(svgContent, foregroundColor) {
 ...
 */
export function svgToThemedDataUrl(svgContent, foregroundColor) {
 ...
 */
function deriveServiceUrl(config) {
 ...
 */
export function useEntityIcon(opts) {
 ...
 */
async function loadIconFile(workspaceId, relativePath) {
    try {
 ...
 */
function sanitizeSvgForInline(svg) {
    return svg
 ...
 */
async function discoverIconFile(workspaceId, iconDir, fileName) {
    const name = fileName ?? 'icon';
    // Probe all extensions in parallel — reduces round-trips from N to 1
    const results = await Promise.allSettled(ICON_FILE_EXTENSIONS.map(ext => loadIconFile(workspaceId, `${iconDir}/${name}${ext}`)));
