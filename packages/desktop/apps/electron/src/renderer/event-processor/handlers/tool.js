...
import { findToolMessage, updateMessageAt, appendMessage, generateMessageId } from '../helpers';
/**
 ...
 */
export function handleToolStart(state, event) {
 ...
 */
export function handleToolResult(state, event) {
    const { session, streaming } = state;
    ...
    if (toolIndex !== -1) {
        ...
        const isPersistedOutput = inferredError && (event.result?.includes('Output has been saved to') ||
        ...
        if (completedTool && (isParentTaskTool(completedTool.toolName || '') || completedTool.toolName === 'TaskOutput')) {
            const hasOrphanedChildren = updatedSession.messages.some(m => m.parentToolUseId === event.toolUseId
                && m.toolStatus !== 'completed'
                && m.toolStatus !== 'error');
            if (hasOrphanedChildren) {
                const updatedMessages = updatedSession.messages.map(m => {
                    if (m.parentToolUseId === event.toolUseId
                        && m.toolStatus !== 'completed'
                        && m.toolStatus !== 'error') {
                        return { ...m, toolStatus: 'completed', toolResult: m.toolResult || '' };
                    }
                    return m;
                });
 ...
 */
export function handleTaskBackgrounded(state, event) {
    const { session, streaming } = state;
    const toolIndex = findToolMessage(session.messages, event.toolUseId);
    if (toolIndex !== -1) {
 ...
 */
export function handleShellBackgrounded(state, event) {
    const { session, streaming } = state;
    const toolIndex = findToolMessage(session.messages, event.toolUseId);
    if (toolIndex !== -1) {
 ...
 */
export function handleTaskProgress(state, event) {
    const { session, streaming } = state;
    const toolIndex = findToolMessage(session.messages, event.toolUseId);
    if (toolIndex !== -1) {
 ...
 * and result summary.
 */
export function handleTaskCompleted(state, event) {
    const { session, streaming } = state;
