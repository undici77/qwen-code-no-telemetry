...
import { sessionAtomFamily, sessionMetaMapAtom, sessionIdsAtom, loadedSessionsAtom, ensureSessionMessagesLoadedAtom, forceSessionMessagesReloadAtom, refreshSessionsMetadataAtom, initializeSessionsAtom, initializeWorkspaceSessionsAtom, addSessionAtom, updateSessionMetaAtom, removeSessionAtom, extractSessionMeta, compareSessionsByFlaggedThenActivityDesc, getWorkspaceSessionMetas, mergeStableSessionMetaList, workspaceSessionMetaCacheAtom, workspaceSessionsAtom, } from '../sessions';
function msg(id, role = 'user') {
    return {
        id,
        role,
        content: `content:${id}`,
...
describe('session message loading atoms', () => {
    ...
    it('does not let a shorter processing response replace existing history', async () => {
        ...
        globalThis.window = {
            electronAPI: {
                getSessionMessages: async (id) => makeSession({
                    id,
                    isProcessing: true,
                    messages: [
...
describe('refreshSessionsMetadataAtom', () => {
    ...
    it('preserves existing messages even when loaded tracking is stale', () => {
        ...
        const existingMessages = [msg('m1'), msg('m2', 'assistant')];
        store.set(sessionAtomFamily('s1'), makeSession({ id: 's1', messages: existingMessages }));
        store.set(loadedSessionsAtom, new Set());
        store.set(refreshSessionsMetadataAtom, {
