...
import { browserInstancesAtom, DEFAULT_DOCKED_BROWSER_INSTANCE_ID, removeBrowserInstanceAtom, setBrowserInstancesAtom, updateBrowserInstanceAtom, } from '../browser-pane';
...
describe('browser pane atoms', () => {
    ...
    it('allows the fixed docked browser ID to reopen after removal', () => {
        ...
        store.set(updateBrowserInstanceAtom, makeInstance(DEFAULT_DOCKED_BROWSER_INSTANCE_ID));
        expect(store.get(browserInstancesAtom).map((i) => i.id)).toEqual([
            DEFAULT_DOCKED_BROWSER_INSTANCE_ID,
        ]);
    });
});
//# sourceMappingURL=browser-pane.test.js.map