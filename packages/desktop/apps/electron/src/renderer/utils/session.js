 ...
 */
export function getSessionPreviewText(session, maxLength = 88) {
 ...
 */
export function getLastFinalAssistantMessageId(session) {
 ...
 */
export function hasUnreadMessages(session) {
    const lastFinalId = getLastFinalAssistantMessageId(session);
    if (!lastFinalId)
        return false; // No final assistant message yet
 ...
 */
export function countUnreadMessages(session) {
    // Helper to check if message is a final response (assistant or plan)
    const isFinalResponse = (msg) => (msg.role === 'assistant' || msg.role === 'plan') && !msg.isIntermediate;
...
export function hasUnreadMeta(session) {
    return session.hasUnread === true;
}
export function hasMessagesMeta(session) {
...
export function formatSessionRelativeTime(timestamp) {
...
export function highlightMatch(text, query) {
    ...
    return React.createElement(React.Fragment, null, before, React.createElement('span', { className: 'bg-yellow-300/30 rounded-[2px]' }, match), highlightMatch(after, query));
}
