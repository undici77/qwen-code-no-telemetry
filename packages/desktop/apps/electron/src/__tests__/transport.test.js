...
import { EVENT_BUFFER_MAX_SIZE } from '@craft-agent/shared/protocol';
...
let servers = [];
let clients = [];
function trackServer(s) { servers.push(s); return s; }
function trackClient(c) { clients.push(c); return c; }
afterEach(() => {
...
async function waitConnected(client, timeoutMs = 2000) {
...
async function waitForStatus(client, predicate, timeoutMs = 2000) {
    const start = Date.now();
...
async function createPair(serverOpts, clientOpts) {
    ...
    while (!connectedClientId) {
        if (Date.now() - start > 2000)
            throw new Error('No clientId received');
        await new Promise(r => setTimeout(r, 10));
    }
...
describe('RPC', () => {
    test('simple invoke returns result', async () => {
        ...
        server.handle('greet', async (_ctx, name) => {
            return `Hello, ${name}!`;
        });
    ...
    test('handler receives correct args', async () => {
        const { server, client } = await createPair();
        server.handle('add', async (_ctx, a, b) => a + b);
    ...
    test('handler with custom error code', async () => {
        ...
        server.handle('export', async () => {
            const err = new Error('Session is active');
    ...
    test('multiple concurrent requests resolve independently', async () => {
        const { server, client } = await createPair();
        server.handle('delay', async (_ctx, ms, value) => {
    ...
    test('Uint8Array response payload roundtrips intact', async () => {
        ...
        expect(Array.from(result)).toEqual([37, 80, 68, 70, 45]);
    });
    test('Uint8Array request args decode correctly in handler', async () => {
        const { server, client } = await createPair();
        let seen = null;
        server.handle('binary:arg', async (_ctx, bytes) => {
            seen = bytes;
...
describe('push events', () => {
    ...
    test('workspace-targeted push only reaches matching clients', async () => {
        ...
        const received1 = [];
        const received2 = [];
        client1.on('update', (v) => received1.push(v));
        client2.on('update', (v) => received2.push(v));
        await new Promise(r => setTimeout(r, 50));
    ...
    test('unsubscribe stops receiving events', async () => {
        const { server, client } = await createPair();
        const received = [];
        const unsub = client.on('test:event', (data) => {
            received.push(data);
        });
        await new Promise(r => setTimeout(r, 50));
...
describe('reliable delivery', () => {
    test('manual reconnect preserves reconnect identity and replays missed events', async () => {
        ...
        client.on('stream', (value) => received.push(value));
        client.on('__transport:reconnected', (isStale) => reconnectStates.push(isStale));
        client.connect();
        await waitConnected(client);
    ...
    test('reconnect replay ignores events targeted at other clients', async () => {
        ...
        const receivedB = [];
        const reconnectStates = [];
        clientA.on('stream', (value) => receivedA.push(value));
        clientB.on('stream', (value) => receivedB.push(value));
        clientA.on('__transport:reconnected', (isStale) => reconnectStates.push(isStale));
        clientA.connect();
        clientB.connect();
        ...
        expect(clientA.lastSeenSeq).toBe(2);
        expect(clientB.lastSeenSeq).toBe(1);
    });
    ...
    test('sequence_ack evicts acknowledged buffered events', async () => {
        ...
        await waitUntil(() => client.lastSeenSeq === 2);
        const ack = {
            id: randomUUID(),
            type: 'sequence_ack',
            lastSeq: 2,
        };
        client.ws.send(serializeEnvelope(ack));
        await waitUntil(() => (server.clients.get(clientId)?.eventBuffer.length ?? -1) === 0);
        expect(server.clients.get(clientId)?.lastAckedSeq).toBe(2);
    });
    test('safe send skips non-open sockets', () => {
        ...
        const sent = client.trySendEnvelope(fakeWs, {
            id: randomUUID(),
            type: 'sequence_ack',
            lastSeq: 1,
        });
...
describe('edge cases', () => {
    ...
    test('invoke on disconnected client throws', async () => {
        ...
        try {
            ...
            expect(err.message.includes('Connection') || err.message.includes('WebSocket error during connection setup')).toBe(true);
        }
    });
...
describe('invokeClient', () => {
    test('server invokes client capability and receives result', async () => {
        const { server, client, clientId } = await createPair({}, { clientCapabilities: ['client:openExternal'] });
        client.handleCapability('client:openExternal', (url) => {
            return `opened: ${url}`;
        });
    ...
    test('invokeClient on missing capability returns CAPABILITY_UNAVAILABLE immediately', async () => {
        const { server, clientId } = await createPair({}, { clientCapabilities: [] });
        try {
    ...
    test('client handler error propagates to server', async () => {
        const { server, client, clientId } = await createPair({}, { clientCapabilities: ['client:failing'] });
    ...
    test('disconnect mid-flight rejects pending invoke with CLIENT_DISCONNECTED', async () => {
        const { server, client, clientId } = await createPair({}, { clientCapabilities: ['client:slow'] });
    ...
    test('handshake with capabilities stores them on server', async () => {
        const { server, client, clientId } = await createPair({}, { clientCapabilities: ['client:openExternal', 'client:notify'] });
    ...
    test('async client handler works', async () => {
        const { server, client, clientId } = await createPair({}, { clientCapabilities: ['client:async'] });
        client.handleCapability('client:async', async (ms) => {
            await new Promise(r => setTimeout(r, ms));
            return 'done';
        });
