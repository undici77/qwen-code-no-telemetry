...
// Proxy builder
// ---------------------------------------------------------------------------
export function buildClientApi(client, channelMap, isChannelAvailable) {
    const api = {};
    const nested = {};
    for (const [key, entry] of Object.entries(channelMap)) {
        let fn;
        if (entry.type === 'listener') {
            fn = (cb) => client.on(entry.channel, cb);
        }
        else if (entry.transform) {
            const t = entry.transform;
            fn = async (...args) => t(await client.invoke(entry.channel, ...args));
        }
        else {
            fn = (...args) => client.invoke(entry.channel, ...args);
        }
