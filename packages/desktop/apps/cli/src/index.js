...
export function parseArgs(argv) {
    ...
    if (!tlsCa)
        tlsCa = process.env.CRAFT_TLS_CA;
    if (!provider)
...
// ---------------------------------------------------------------------------
async function resolveWorkspace(client, explicit) {
    if (explicit) {
...
const c = {
    dim: (s) => _useColor ? `\x1b[2m${s}\x1b[22m` : s,
    green: (s) => _useColor ? `\x1b[32m${s}\x1b[39m` : s,
    red: (s) => _useColor ? `\x1b[31m${s}\x1b[39m` : s,
    cyan: (s) => _useColor ? `\x1b[36m${s}\x1b[39m` : s,
    bold: (s) => _useColor ? `\x1b[1m${s}\x1b[22m` : s,
    yellow: (s) => _useColor ? `\x1b[33m${s}\x1b[39m` : s,
    blue: (s) => _useColor ? `\x1b[34m${s}\x1b[39m` : s,
};
// ---------------------------------------------------------------------------
...
// Commands
// ---------------------------------------------------------------------------
async function cmdPing(client, args) {
    const start = performance.now();
    const clientId = await client.connect();
    const latency = Math.round(performance.now() - start);
    out(args.json
        ? { clientId, latencyMs: latency }
        : `Connected: clientId=${clientId} latency=${latency}ms`, args.json);
}
...
async function cmdVersions(client, args) {
    await client.connect();
    const result = await client.invoke('system:versions');
    out(result, args.json);
}
async function cmdWorkspaces(client, args) {
    await client.connect();
    const result = (await client.invoke('workspaces:get'));
    if (args.json) {
        out(result, true);
    }
    else {
...
async function cmdSessions(client, args) {
    await client.connect();
    ...
    const result = (await client.invoke('sessions:get', workspaceId));
    if (args.json) {
        out(result, true);
    }
    else {
...
async function cmdConnections(client, args) {
    await client.connect();
    const result = await client.invoke('LLM_Connection:list');
    out(result, args.json);
}
async function cmdSources(client, args) {
    await client.connect();
    const workspaceId = await resolveWorkspace(client, args.workspace);
    if (!workspaceId) {
        err('No workspace available. Use --workspace <id>');
...
async function cmdSessionCreate(client, args) {
    await client.connect();
    const workspaceId = await resolveWorkspace(client, args.workspace);
    if (!workspaceId) {
        err('No workspace available. Use --workspace <id>');
    ...
    if (name)
        opts.name = name;
    if (args.mode)
        opts.permissionMode = args.mode;
    const result = await client.invoke('sessions:create', workspaceId, opts);
    out(result, args.json);
}
async function cmdSessionMessages(client, args) {
...
async function cmdSessionDelete(client, args) {
    const sessionId = args.rest[0];
    if (!sessionId) {
 ...
 */
async function readPrompt(words, restArgs) {
 ...
 */
async function sendAndStream(client, sessionId, message, args) {
    let exitCode = 0;
    let finished = false;
    const streamJson = args.outputFormat === 'stream-json';
    const unsub = client.on('session:event', (event) => {
        const ev = event;
...
async function cmdSend(client, args) {
    const sessionId = args.rest[0];
    if (!sessionId) {
...
async function spawnLocalServer(args, opts) {
...
export function resolveApiKey(_provider, explicit) {
    return explicit;
}
export function shouldSetupLlmConnection(existingConnectionCount, _args) {
    return existingConnectionCount === 0;
}
async function setupLlmConnection(client, _args) {
    const provider = 'qwen';
    ...
    const setupResult = await client.invoke('settings:setupLlmConnection', { slug: connectionSlug });
...
async function cmdRun(args) {
    ...
    try {
        ...
        if (args.workspaceDir) {
            ...
            const ws = (await client.invoke('workspaces:create', absPath, 'ci-workspace'));
        ...
        const connections = (await client.invoke('LLM_Connection:list'));
        let connectionSlug;
...
async function cmdInvoke(client, args) {
...
async function cmdListen(client, args) {
    const channel = args.rest[0];
    if (!channel) {
 ...
 */
async function waitForSendEvents(client, sessionId, message, timeoutMs, expectTool, sendOptions, onEvent, expectToolName) {
    const seen = new Set();
    let textChunks = 0;
    let toolName = '';
    let finished = false;
    ...
    try {
        await client.invoke('sessions:sendMessage', sessionId, message, undefined, undefined, sendOptions);
        ...
        if (seen.has('error') && !seen.has('complete'))
            throw new Error('Session returned an error event');
        if (expectTool) {
            if (!seen.has('tool_start'))
                throw new Error('No tool_start event received');
            if (!seen.has('tool_result'))
                throw new Error('No tool_result event received');
        ...
        if (!seen.has('text_delta'))
            throw new Error('No text_delta events received');
 ...
 */
async function cleanupAutomationArtifacts(client, ctx) {
    const cleaned = [];
    ...
    for (const key of ['automationTestSessionId', 'automationBlockedSessionId']) {
        const id = ctx[key];
        if (!id || !client.isConnected)
            continue;
        try {
...
export function getValidateSteps() {
    return [
        {
        ...
        },
        {
            ...
            fn: async (client) => {
                const r = (await client.invoke('credentials:healthCheck'));
        ...
        },
        {
            ...
            fn: async (client) => {
                const r = (await client.invoke('system:versions'));
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                ...
                const r = (await client.invoke('workspaces:get'));
                if (r?.length > 0) {
                ...
                const ws = (await client.invoke('workspaces:create', tmpDir, 'validate-workspace'));
                ctx.workspaceId = ws.id;
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                ...
                const r = (await client.invoke('sessions:get', ctx.workspaceId));
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                const r = (await client.invoke('LLM_Connection:list'));
                ...
                const result = await client.invoke('settings:setupLlmConnection', { slug });
                if (!result?.success)
                    return `setup failed: ${result?.error ?? 'unknown'}`;
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.workspaceId)
                    return 'skipped (no workspace)';
                const r = (await client.invoke('sources:get', ctx.workspaceId));
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.createdSessionId)
                    return 'skipped (no session)';
                return await waitForSendEvents(client, ctx.createdSessionId, 'Reply with exactly: VALIDATION_OK', 60_000, false, undefined, ctx.onEvent);
            },
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.createdSessionId)
                    return 'skipped (no session)';
                return await waitForSendEvents(client, ctx.createdSessionId, 'Use the Bash tool to run: echo TOOL_VALIDATION_OK', 90_000, true, undefined, ctx.onEvent);
            },
        },
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.createdSessionId)
                    return 'skipped (no session)';
                if (!ctx.e2eTestLabelId)
                    return 'skipped (no e2e-test label)';
                const result = await waitForSendEvents(client, ctx.createdSessionId, 'Use the set_session_labels tool to set labels: ["e2e-test"] on the current session. Do NOT use any other tool.', 90_000, true, undefined, ctx.onEvent, 'set_session_labels');
                // Verify labels were actually applied
                const sessions = (await client.invoke('sessions:get', ctx.workspaceId));
                const session = sessions?.find((s) => s.id === ctx.createdSessionId);
                const labels = session?.labels ?? session?.labelIds ?? [];
                if (!labels.length)
                    throw new Error('Labels not applied to session');
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.createdSessionId)
                    return 'skipped (no session)';
                return await waitForSendEvents(client, ctx.createdSessionId, 'Use the get_session_info tool to get info about the current session. Do NOT use any other tool.', 90_000, true, undefined, ctx.onEvent, 'get_session_info');
            },
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.createdSessionId)
                    return 'skipped (no session)';
                return await waitForSendEvents(client, ctx.createdSessionId, 'Use the list_sessions tool to list all sessions. Do NOT use any other tool.', 90_000, true, undefined, ctx.onEvent, 'list_sessions');
            },
        },
        // ----- Session branching -----
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.createdSessionId || !ctx.workspaceId)
                    return 'skipped (no session)';
                const r = (await client.invoke('sessions:getMessages', ctx.createdSessionId));
                const messages = r?.messages ?? r?.conversation ?? [];
                const firstAssistant = messages.find((m) => m.role === 'assistant');
                if (!firstAssistant?.id)
                    throw new Error('No assistant message found to branch from');
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.branchedSessionId)
                    return 'skipped (no branch)';
                const r = (await client.invoke('sessions:getMessages', ctx.branchedSessionId));
                const messages = r?.messages ?? r?.conversation ?? [];
                const hasAssistant = messages.some((m) => m.role === 'assistant');
                if (!hasAssistant)
                    throw new Error('Branch missing assistant message');
                const origR = (await client.invoke('sessions:getMessages', ctx.createdSessionId));
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.branchedSessionId)
                    return 'skipped (no branch)';
                return await waitForSendEvents(client, ctx.branchedSessionId, 'Reply with exactly: BRANCH_OK', 60_000, false, undefined, ctx.onEvent);
            },
        },
        // ----- Source lifecycle -----
        {
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.createdSessionId || !ctx.createdSourceSlug)
                    return 'skipped (no session or source)';
                ...
                return await waitForSendEvents(client, ctx.createdSessionId, `[source:${ctx.createdSourceSlug}] Get me a cat fact`, 90_000, false, undefined, ctx.onEvent);
            },
        },
        // ----- MCP source validation (pre-committed in .github/agents/sources/) -----
        {
            ...
            fn: async (client, ctx) => {
                ...
                const enableSlugs = [ctx.createdSourceSlug, 'craft-public'].filter(Boolean);
                await client.invoke('sessions:command', ctx.createdSessionId, {
                    type: 'setSources',
                    sourceSlugs: enableSlugs,
                });
                return await waitForSendEvents(client, ctx.createdSessionId, `[source:craft-public] List the documents under the "CraftAgents E2E Test" folder inside the "CraftAgents" folder. Just list their names.`, 180_000, false, undefined, ctx.onEvent);
            },
        },
        {
            ...
            fn: async (client, ctx) => {
                ...
                if (!apiKey)
                    return 'skipped (no STITCH_API_KEY)';
                ...
                const enableSlugs = [ctx.createdSourceSlug, 'craft-public', 'stitch-mcp'].filter(Boolean);
                await client.invoke('sessions:command', ctx.createdSessionId, {
                    type: 'setSources',
                    sourceSlugs: enableSlugs,
                });
                return await waitForSendEvents(client, ctx.createdSessionId, `Use the source_test tool to test the stitch-mcp source. Report the result.`, 90_000, false, undefined, ctx.onEvent);
            },
        },
        // ----- Skill lifecycle -----
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.createdSessionId || !ctx.workspaceRootPath)
                    return 'skipped (no session or workspace)';
                ...
                return await waitForSendEvents(client, ctx.createdSessionId, `Use the Bash tool to run this exact command:
...
SKILLEOF`, 90_000, true, undefined, ctx.onEvent);
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.workspaceId || !ctx.createdSkillSlug)
                    return 'skipped (no skill)';
                const r = (await client.invoke('skills:get', ctx.workspaceId));
                const found = r?.find((s) => s.slug === ctx.createdSkillSlug);
                if (!found)
                    throw new Error(`Skill '${ctx.createdSkillSlug}' not found in skills list`);
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.createdSessionId || !ctx.createdSkillSlug)
                    return 'skipped (no session or skill)';
                return await waitForSendEvents(client, ctx.createdSessionId, `[skill:${ctx.createdSkillSlug}] Run the skill`, 120_000, false, { skillSlugs: [ctx.createdSkillSlug] }, ctx.onEvent);
            },
        },
        {
            name: 'skills:delete',
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                ...
                const statuses = (await client.invoke('statuses:list', ctx.workspaceId));
                const inProgress = statuses?.find((s) => (s.id ?? '').toLowerCase().includes('in-progress') ||
                ...
                while (Date.now() < deadline) {
                    ...
                    const sessions = (await client.invoke('sessions:get', ctx.workspaceId));
                    const blockedSession = sessions?.find((s) => s.name === ctx.automationBlockedName && s.id !== ctx.createdSessionId);
                    if (blockedSession) {
                    ...
                    const passSession = sessions?.find((s) => s.name === ctx.automationName && s.id !== ctx.createdSessionId);
                    if (passSession) {
                        ctx.automationTestSessionId = passSession.id;
                        // Guard against delayed blocked-automation session creation.
                        await new Promise((r) => setTimeout(r, 2000));
                        const sessionsAfter = (await client.invoke('sessions:get', ctx.workspaceId));
                        const blockedAfter = sessionsAfter?.find((s) => s.name === ctx.automationBlockedName && s.id !== ctx.createdSessionId);
                        if (blockedAfter) {
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.automationTestSessionId)
                    return 'skipped (no automation session)';
                // Wait for the automation session to complete
                let delay = 1000;
                const deadline = Date.now() + 90_000;
                while (Date.now() < deadline) {
                    const session = (await client.invoke('sessions:getMessages', ctx.automationTestSessionId));
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.automationTestSessionId || !ctx.workspaceId)
                    return 'skipped (no automation session)';
                // Verify label was auto-created
                const labels = (await client.invoke('labels:list', ctx.workspaceId));
                const found = labels?.find((l) => (l.id ?? l.name ?? '') === 'cli-validate-label');
                if (!found)
                    throw new Error('Label cli-validate-label was not auto-created');
                ...
                const sessionLabels = automationSession?.labels ?? [];
                const hasLabel = sessionLabels.some((l) => l.includes('cli-validate-label'));
                if (!hasLabel)
                    throw new Error(`Automation session missing label (has: ${sessionLabels.join(', ')})`);
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.workspaceId)
                    return 'skipped (no workspace)';
                const history = (await client.invoke('automations:getLastExecuted', ctx.workspaceId));
                const entries = Object.entries(history);
                if (entries.length === 0)
                    throw new Error('No automation execution history found');
                ...
                if (!recent)
                    throw new Error(`No recent automation execution (latest: ${Math.max(...entries.map(([, ts]) => ts))})`);
        ...
        // ----- Webhook validation -----
        {
            ...
            fn: async (client, ctx) => {
                ...
                if (result?.success)
                    throw new Error('Expected webhook to fail (nothing listening)');
                if (!result?.error && result?.statusCode !== 0)
                    throw new Error('Expected error or statusCode 0 in result');
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.workspaceRootPath)
                    return 'skipped (no workspace root)';
                ...
                while (Date.now() < deadline) {
                    ...
                    if (webhookEntries.length > 0) {
                        const recentThreshold = Date.now() - 120_000;
                        const recentFailed = webhookEntries.find((e) => !e.ok && e.ts > recentThreshold && e.webhook?.method === 'POST');
                        if (recentFailed) {
                            return `webhook failure recorded: method=${recentFailed.webhook.method}, url=${recentFailed.webhook.url?.slice(0, 50)}`;
                        }
                        const latest = webhookEntries[webhookEntries.length - 1];
                ...
                throw new Error(`No recent failed POST webhook history entry after ${waitedMs}ms (lines=${lastLineCount}, webhookEntries=${lastWebhookCount}, ${lastSummary})`);
            },
        },
        {
        ...
        },
        {
            name: 'sources:delete',
            fn: async (client, ctx) => {
                if (!ctx.workspaceId || !ctx.createdSourceSlug)
        ...
        },
        {
            ...
            fn: async (client, ctx) => {
                if (!ctx.workspaceId || !ctx.e2eTestLabelId)
                    return 'skipped (no e2e-test label)';
...
export async function runValidation(client, jsonMode, noSpinner, workspaceDir, validateOptions) {
            ...
            ctx.onEvent = (ev) => {
                switch (ev.type) {
                    ...
                    case 'text_delta':
                        ...
                        if (!textFlushed && accText.length > 40)
                            flushText();
                        break;
                    case 'text_complete':
                        ensureHeader();
