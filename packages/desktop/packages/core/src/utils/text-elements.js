 ...
 */
export function utf16IndexToByteOffset(text, index) {
 ...
 */
export function byteOffsetToUtf16Index(text, byteOffset) {
    const boundedOffset = Math.max(0, byteOffset);
    let bytes = 0;
    let index = 0;
    for (const char of text) {
        if (bytes >= boundedOffset)
            return index;
...
function badgeTarget(badge) {
    ...
    if (badge.type === 'command')
        return badge.label.replace(/^\/+/, '').trim() || undefined;
    return undefined;
}
export function contentBadgeToTextElement(content, badge) {
...
export function contentBadgesToTextElements(content, badges) {
    if (!badges?.length)
        return undefined;
    return badges.map((badge) => contentBadgeToTextElement(content, badge));
}
function metadataString(element, key) {
    const value = element.metadata?.[key];
    return typeof value === 'string' ? value : undefined;
}
export function textElementToContentBadge(content, element) {
...
export function textElementsToContentBadges(content, textElements) {
