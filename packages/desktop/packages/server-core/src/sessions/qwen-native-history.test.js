...
import { loadSession, saveSession, sessionPersistenceQueue, } from '@craft-agent/shared/sessions';
...
describe('Qwen native history loading', () => {
    ...
    it('deduplicates concurrent agent creation for one managed session', async () => {
        ...
        let createCalls = 0;
        const managerInternals = manager;
    ...
    it('hides unresolved stripped Qwen canonical mirrors from session lists', () => {
        ...
        manager.sessions.set(unresolved.id, unresolved);
        manager.sessions.set(placeholder.id, placeholder);
        manager.sessions.set(resolved.id, resolved);
        expect(manager.getSessions(workspace.id).map((session) => session.id)).toEqual([resolvedSdkSessionId]);
    });
    it('lists provider-native sessions from the workspace default working directory', async () => {
        ...
        await manager.doRefreshExternalSessionsForWorkspace(workspace);
        ...
        const header = JSON.parse(readFileSync(join(workspaceRoot, 'sessions', sessionId, 'session.jsonl'), 'utf8').split('\n')[0]);
    ...
    it('removes provider-native mirrors once a Craft session owns the same SDK session ID', async () => {
        ...
        manager.sessions.set(craftSessionId, craftManaged);
        manager.sessions.set(sdkSessionId, mirrorManaged);
        await manager.doRefreshExternalSessionsForWorkspace(workspace);
        ...
        expect(loadSession(workspaceRoot, sdkSessionId)?.sdkSessionId).toBe(sdkSessionId);
    });
    it('syncs Craft session titles to Qwen custom titles through the provider rename hook', async () => {
        ...
        const manager = new SessionManager({
            createExternalSessionAgent: () => ({
                renameBackendSession: async (sessionId, title, options) => {
        ...
        await manager.syncExternalBackendTitleIfSupported(managed, managed.name);
    ...
    it('skips placeholder provider-native sessions with no renderable history', async () => {
        ...
        const manager = new SessionManager({
            createExternalSessionAgent: () => ({
                ...
                loadSessionMessages: async (requestedSessionId, options) => {
                    loadCalls += 1;
        ...
        expect(manager
            ...
            .some((session) => session.id === sessionId)).toBe(false);
    });
    ...
    it('uses workspace default cwd when opening a stripped Qwen canonical session', async () => {
        ...
        const manager = new SessionManager({
            createExternalSessionAgent: () => ({
                loadSessionMessages: async (_sessionId, options) => {
    ...
    it('persists only local visual overlays for Qwen canonical sessions', async () => {
        ...
        const attachment = {
            ...
            mimeType: 'image/png',
            size: 1024,
            storedPath: join(workspaceRoot, 'sessions', sessionId, 'attachments', 'rabbit.png'),
    ...
    it('flushes local visual overlays when sending Qwen messages with attachments', async () => {
        ...
        const attachment = {
            ...
            mimeType: 'image/jpeg',
            size: 1024,
            storedPath: join(workspaceRoot, 'sessions', sessionId, 'attachments', 'subscription.jpg'),
    ...
    it('flushes queued local visual overlays for Qwen sessions with attachments', async () => {
        ...
        const managed = createManagedSession({
            ...
            llmConnection: 'qwen-code',
            lastMessageAt: timestamp,
        }, workspace, { isProcessing: true, messagesLoaded: true });
        const manager = new SessionManager();
        manager.sessions.set(sessionId, managed);
    ...
    it('offers live image attachments to Qwen mid-turn injection', async () => {
        ...
        const manager = new SessionManager();
        manager.sessions.set(sessionId, managed);
        await manager.sendMessage(sessionId, '这个是什么图片', [liveAttachment], [storedAttachment], { optimisticMessageId: 'optimistic-1' });
    ...
    it('acknowledges Qwen mid-turn queued messages by messageId', () => {
        ...
        const onMidTurnMessagesDrained = manager.createMidTurnMessagesDrainedCallback(managed);
    ...
    it('offers plain text follow-ups to Qwen mid-turn injection after visual messages', async () => {
        ...
        const manager = new SessionManager();
        manager.sessions.set(sessionId, managed);
        await manager.sendMessage(sessionId, 'also summarize the visible text', undefined, undefined, { optimisticMessageId: 'optimistic-text' });
        expect(enqueueCalls).toHaveLength(1);
        expect(enqueueCalls[0]).toEqual({
    ...
    it('keeps stored-only image attachments queued for the next turn', async () => {
        ...
        const manager = new SessionManager();
        manager.sessions.set(sessionId, managed);
        await manager.sendMessage(sessionId, '这个是什么图片', undefined, [storedAttachment], { optimisticMessageId: 'optimistic-1' });
    ...
    it('repairs legacy Qwen sessions with orphaned image attachments', async () => {
        ...
        const attachmentsDir = join(workspaceRoot, 'sessions', sessionId, 'attachments');
        mkdirSync(attachmentsDir, { recursive: true });
        const attachmentId2 = '22222222-2222-4222-8222-222222222222';
        const imagePath2 = join(attachmentsDir, `${attachmentId2}_screenshot-2.jpg`);
        ...
        const imagePath1 = join(attachmentsDir, `${attachmentId1}_screenshot-1.jpg`);
        ...
        expect(loadSession(workspaceRoot, sessionId)?.messages[0]?.attachments).toHaveLength(2);
    });
    ...
    it('backfills a lazy-loaded empty local session from provider-native history', async () => {
        ...
        expect(session?.lastUsedAt).toBe(timestamp + 3);
    ...
    it('does not reload Qwen native history between repeated edits of the latest user message', async () => {
        ...
        let chatCalls = 0;
        const activeAgent = {
            rewindToUserTurn: async (targetTurnIndex) => {
                rewindCalls += 1;
...
//# sourceMappingURL=qwen-native-history.test.js.map