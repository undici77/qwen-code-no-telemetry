...
import { createScopedLogger, CONSOLE_LOGGER, } from '@craft-agent/server-core/runtime';
import { basename, extname, join } from 'path';
import { existsSync, readdirSync, readFileSync, statSync } from 'fs';
import { mkdir, readFile, rename, writeFile } from 'fs/promises';
import { randomUUID } from 'node:crypto';
import { setPermissionMode, hydratePreviousPermissionMode, getPermissionModeDiagnostics, unregisterSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, AbortReason, generateConversationSummary, } from '@craft-agent/shared/agent';
import { resolveSessionConnection, createBackendFromConnection, resolveBackendContext, createBackendFromResolvedContext, cleanupSourceRuntimeArtifacts, providerTypeToAgentProvider, resolveModelForProvider, } from '@craft-agent/shared/agent/backend';
import { getLlmConnection, getLlmConnections, getDefaultLlmConnection, getDefaultThinkingLevel, QWEN_CODE_CONNECTION_SLUG, } from '@craft-agent/shared/config';
import { PrivilegedExecutionBroker } from '@craft-agent/server-core/services';
import { isValidWorkingDirectory } from '../utils/path-validation';
import { InitGate } from '@craft-agent/server-core/domain';
import { i18n, LOCALE_REGISTRY, } from '@craft-agent/shared/i18n';
import { getWorkspaces, getWorkspaceByNameOrId, loadConfigDefaults, migrateLegacyCredentials, migrateLegacyLlmConnectionsConfig, migrateOrphanedDefaultConnections, MODEL_REGISTRY, } from '@craft-agent/shared/config';
import { loadWorkspaceConfig } from '@craft-agent/shared/workspaces';
import { 
// Session persistence functions
listSessions as listStoredSessions, loadSession as loadStoredSession, saveSession as saveStoredSession, createSession as createStoredSession, deleteSession as deleteStoredSession, updateSessionMetadata, setPendingPlanExecution as setStoredPendingPlanExecution, markCompactionComplete as markStoredCompactionComplete, markPendingPlanExecutionDispatched as markStoredPendingPlanExecutionDispatched, clearPendingPlanExecution as clearStoredPendingPlanExecution, getPendingPlanExecution as getStoredPendingPlanExecution, getSessionPath as getSessionStoragePath, ensureSessionDir, getSessionFilePath, generateSessionId, sessionPersistenceQueue, getHeaderMetadataSignature, createSessionHeader, writeSessionJsonl, serializeSession, validateBundle, pickSessionFields, } from '@craft-agent/shared/sessions';
import { loadWorkspaceSources, loadAllSources, getSourcesBySlugs, isSourceUsable, getSourceCredentialManager, getSourceServerBuilder, isApiOAuthProvider, hasRenewEndpoint, SERVER_BUILD_ERRORS, TokenRefreshManager, createTokenGetter, } from '@craft-agent/shared/sources';
...
import { RPC_CHANNELS, generateMessageId, } from '@craft-agent/shared/protocol';
...
import { loadAllSkills, loadSkillBySlug, invalidateSkillsCache, } from '@craft-agent/shared/skills';
...
import { AutomationSystem, createPromptHistoryEntry, appendAutomationHistoryEntry, } from '@craft-agent/shared/automations';
...
let sessionRuntimeHooks = defaultSessionRuntimeHooks;
export function setSessionRuntimeHooks(hooks) {
    sessionRuntimeHooks = {
        ...sessionRuntimeHooks,
        ...hooks,
    };
...
function buildBackendHostRuntimeContext() {
    if (!_platform)
        throw new Error('setSessionPlatform() must be called before session creation');
    return {
...
function canOfferMidTurnAttachments(attachments) {
 ...
 */
async function buildServersFromSources(sources, sessionPath, tokenRefreshManager, summarize) {
    ...
    const sourcesWithCreds = await Promise.all(sources.map(async (source) => ({
        source,
    ...
    const getTokenForSource = (source) => {
 ...
 */
async function refreshOAuthTokensIfNeeded(agent, sources, sessionPath, tokenRefreshManager, options) {
    ...
    if (refreshed.length > 0) {
        ...
        const { mcpServers, apiServers } = await buildServersFromSources(enabledSources, sessionPath, tokenRefreshManager, agent.getSummarizeCallback());
 ...
 */
async function applyBridgeUpdates(agent, sessionPath, enabledSources, mcpServers, sessionId, workspaceRootPath, context, poolServerUrl) {
...
let browserToolIconDataUrlCache;
async function getBrowserToolIconDataUrl() {
...
async function resolveToolDisplayMeta(toolName, toolInput, workspaceRootPath, sources) {
    ...
    if (toolName === 'Skill' && toolInput) {
        ...
        const skillParam = toolInput.skill;
        if (skillParam) {
...
function canRewindToUserTurn(agent) {
    return typeof agent.rewindToUserTurn === 'function';
}
function canManagePermissionSettings(agent) {
    const candidate = agent;
    return (typeof candidate.getPermissionSettings === 'function' &&
        typeof candidate.setPermissionRules === 'function');
}
function canManageQwenSettings(agent) {
    const candidate = agent;
    return (typeof candidate.getCoreSettings === 'function' &&
...
function canManageQwenProviders(agent) {
    const candidate = agent;
    return (typeof candidate.listProviders === 'function' &&
        typeof candidate.connectProvider === 'function');
}
function isQwenNativeHistoryMessageId(messageId, sdkSessionId) {
...
const LEGACY_IMAGE_MIME_BY_EXTENSION = {
    '.avif': 'image/avif',
    '.bmp': 'image/bmp',
    '.gif': 'image/gif',
    '.heic': 'image/heic',
...
function legacyStoredAttachmentName(fileName) {
    return fileName.replace(/^[0-9a-f-]{36}_/i, '');
}
function hasImagePreviewContent(message) {
    return message.content.includes('```image-preview');
}
function stripImagePreviewBlocks(content) {
    return content.replace(IMAGE_PREVIEW_BLOCK_PATTERN, '');
}
function normalizeQwenVisualMatchContent(content) {
    return stripImagePreviewBlocks(content).replace(/\s+/g, ' ').trim();
}
function extractImagePreviewBlocks(content) {
    return content.match(IMAGE_PREVIEW_BLOCK_PATTERN) ?? [];
}
function hasQwenCanonicalLocalVisualState(message) {
    return ((message.attachments?.length ?? 0) > 0 || hasImagePreviewContent(message));
}
function qwenCanonicalLocalVisualMessages(messages) {
    return messages.filter((message) => message.role !== 'status' && hasQwenCanonicalLocalVisualState(message));
}
function findQwenCanonicalVisualOverlayMatch(message, overlays, usedOverlayIndexes) {
    let bestIndex = -1;
    let bestScore = 0;
    const normalized = normalizeQwenVisualMatchContent(message.content);
    overlays.forEach((overlay, index) => {
        if (usedOverlayIndexes.has(index) || overlay.role !== message.role)
            return;
        ...
        let score = 0;
        if (overlay.id === message.id)
            score += 1000;
        if (overlay.content === message.content)
            score += 500;
        if (overlayNormalized && overlayNormalized === normalized)
            score += 400;
        ...
        if (timeDiff <= 30_000)
            score += 100 - Math.floor(timeDiff / 1_000);
        if (score > bestScore) {
            bestScore = score;
            bestIndex = index;
        }
...
function mergeQwenCanonicalVisualOverlay(message, overlay) {
    const next = { ...message };
...
function mergeQwenCanonicalLocalVisualMessages(messages, localMessages) {
    const overlays = qwenCanonicalLocalVisualMessages(localMessages);
    if (overlays.length === 0)
        return messages;
    const usedOverlayIndexes = new Set();
    const merged = messages.map((message) => {
        const overlayIndex = findQwenCanonicalVisualOverlayMatch(message, overlays, usedOverlayIndexes);
        if (overlayIndex === -1)
            return message;
 ...
 */
export function createManagedSession(source, workspace, overrides) {
    const s = source;
    const sourceFields = Object.fromEntries(Object.entries(s).filter(([, v]) => v !== undefined));
    ...
    if (managed.branchContextStrategy === 'seeded-fresh-session' &&
...
function normalizeBackendSessionMessages(payload) {
    if (!payload)
        return undefined;
 ...
 */
function resolveSupportsBranching(managed) {
 ...
 */
function managedToSession(m, overrides) {
    const persistableMessages = m.messages.filter((message) => message.role !== 'status');
    const derivedHeader = persistableMessages.length > 0 && !isQwenCanonicalManagedSession(m)
        ? createSessionHeader({
            ...pickSessionFields(m),
    ...
    return {
        ...
        lastFinalMessageId: derivedHeader?.lastFinalMessageId ?? m.lastFinalMessageId,
        // Runtime-only fields
...
function getManagedSessionOrderTime(session) {
    return session.lastMessageAt ?? session.lastUsedAt ?? session.createdAt ?? 0;
}
function compareManagedSessionsByActivityDesc(a, b) {
...
function mapPermissionModeToQwenApprovalMode(mode) {
    switch (mode) {
        case 'allow-all':
            return 'yolo';
        case 'safe':
...
export class SessionManager {
    createExternalSessionAgentOverride;
    sessions = new Map();
    ...
    getAdminRememberKey(sessionId, commandHash) {
    ...
    storeAdminRememberApproval(sessionId, commandHash, sourceRequestId, rememberForMinutes) {
    ...
    clearAdminRememberApprovalsForSession(sessionId) {
    ...
    clearPendingPermissionRequestsForSession(sessionId) {
    ...
    applyExternalSessionMetadata(managed, header) {
        ...
        if (!(header.name === undefined && this.isQwenCanonicalMessageSession(managed)) &&
     ...
     */
    setupConfigWatcher(workspaceRootPath, workspaceId) {
        ...
        const callbacks = {
            ...
            onSessionMetadataChange: (sessionId, header) => {
                ...
                if (!isSelfWrite) {
                    ...
                    const hasWriteGuard = managed._metadataWriteGuardUntil &&
        ...
        if (!this.automationSystems.has(workspaceRootPath)) {
            const automationSystem = new AutomationSystem({
                ...
                onPromptsReady: async (prompts) => {
                    ...
                    const settled = await Promise.allSettled(prompts.map((pending) => this.executePromptAutomation(workspaceId, workspaceRootPath, pending.prompt, pending.labels, pending.permissionMode, pending.mentions, pending.llmConnection, pending.model, pending.automationName)));
                    ...
                    for (const [idx, result] of settled.entries()) {
                        ...
                        const entry = createPromptHistoryEntry({
                            ...
                            sessionId: result.status === 'fulfilled'
     ...
     */
    notifyConfigFileChange(workspaceRootPath, relativePath) {
     ...
     */
    async reloadSourcesForWorkspace(workspaceRootPath) {
    ...
    broadcastAutomationsChanged(workspaceId) {
        if (!this.eventSink)
            return;
    ...
    broadcastAppThemeChanged(theme) {
        if (!this.eventSink)
            return;
    ...
    updateQwenConnectionModels(slug, models, currentModelId) {
    ...
    updateQwenConnectionDefault(slug, model) {
        const connection = getLlmConnection(slug);
        if (!connection || connection.providerType !== 'qwen')
            return;
        try {
            const previous = getModelRefreshService().getRuntimeModelState(slug);
    ...
    broadcastDefaultPermissionsChanged() {
        if (!this.eventSink)
            return;
     ...
     */
    async reloadSessionSources(managed) {
        if (!managed.agent)
            return; // No agent = nothing to update (fresh build on next message)
        ...
        const enabledSources = allSources.filter((s) => enabledSlugs.includes(s.config.slug) && isSourceUsable(s));
     ...
     */
    async reinitializeAuth(connectionSlug) {
        try {
    ...
    async refreshExternalSessions(workspaceId) {
        const workspaces = getWorkspaces().filter((workspace) => !workspaceId || workspace.id === workspaceId);
        await Promise.all(workspaces.map((workspace) => this.refreshExternalSessionsForWorkspace(workspace)));
    }
    async refreshExternalSessionsForWorkspace(workspace) {
        const lastSync = this.externalSessionListSyncAt.get(workspace.id) ?? 0;
        if (Date.now() - lastSync < EXTERNAL_SESSION_LIST_SYNC_INTERVAL_MS)
            return;
    ...
    async doRefreshExternalSessionsForWorkspace(workspace) {
        ...
        if (!backendContext.capabilities.listsSessions ||
        ...
        const seenSdkSessionIds = new Set();
        for (const info of listedSessions) {
            const didUpsert = await this.upsertExternalListedSession({
                ...
                defaultThinkingLevel: normalizeThinkingLevel(workspaceConfig?.defaults?.thinkingLevel) ??
                    getDefaultThinkingLevel(),
                loadMessages: agent.loadSessionMessages
                    ? (sessionInfo) => agent.loadSessionMessages(sessionInfo.sessionId, {
                        cwd: sessionInfo.cwd,
                    })
                    : undefined,
            });
            if (didUpsert)
                seenSdkSessionIds.add(info.sessionId);
        }
    ...
    createExternalSessionListAgent(workspace, backendContext) {
    ...
    externalSessionAgentCacheKey(workspace, backendContext) {
    ...
    getExternalSessionAgent(workspace, backendContext) {
        ...
        agent.onDebug = (msg) => sessionLog.debug(msg);
        this.externalSessionAgents.set(key, agent);
        return agent;
    }
    async withExternalSessionAgent(managed, callback) {
    ...
    resolveExternalSessionConnectionSlug(managed) {
        if (managed.llmConnection)
            return managed.llmConnection;
    ...
    async canonicalizeQwenManagedSessionId(workspace, managed, sdkSessionId) {
        if (managed.id === sdkSessionId)
            return managed;
        if (this.resolveExternalSessionConnectionSlug(managed) !==
        ...
        else if (existsSync(getSessionStoragePath(workspace.rootPath, sdkSessionId))) {
        ...
        await sessionPersistenceQueue.flush(previousId).catch((error) => {
            sessionLog.debug(`Failed to flush session ${previousId} before Qwen id canonicalization:`, error);
        });
    ...
    syncExternalBackendTitleIfSupported(managed, title) {
    ...
    async deleteExternalBackendSessionIfSupported(managed) {
        if (!managed.sdkSessionId)
            return;
        try {
            await this.withExternalSessionAgent(managed, async (agent) => {
                if (!agent.deleteBackendSession)
                    return false;
    ...
    findManagedSessionsBySdkSessionId(workspaceId, sdkSessionId) {
        return Array.from(this.sessions.values()).filter((managed) => managed.workspace.id === workspaceId &&
            (managed.sdkSessionId === sdkSessionId || managed.id === sdkSessionId));
    }
    selectManagedSessionBySdkSessionId(workspaceId, sdkSessionId) {
        const matches = this.findManagedSessionsBySdkSessionId(workspaceId, sdkSessionId);
        if (matches.length === 0)
            return undefined;
        const byActivityDesc = (a, b) => Math.max(b.lastMessageAt ?? 0, b.lastUsedAt ?? 0) -
    ...
    parseExternalSessionTimestamp(updatedAt) {
        if (!updatedAt)
            return Date.now();
        const parsed = Date.parse(updatedAt);
        return Number.isFinite(parsed) ? parsed : Date.now();
    }
    isExternalSessionPlaceholderTitle(title) {
        return EXTERNAL_SESSION_PLACEHOLDER_TITLES.has(title.trim());
    }
    hasNoRenderableLocalMessages(managed) {
    ...
    isUnresolvedQwenCanonicalMirror(managed) {
        if (managed.isProcessing)
            return false;
        ...
        if (this.resolveExternalSessionConnectionSlug(managed) !==
            QWEN_CODE_CONNECTION_SLUG)
            return false;
        if (!this.hasNoRenderableLocalMessages(managed))
            return false;
        const title = typeof managed.name === 'string' ? managed.name : undefined;
        if (title && !this.isExternalSessionPlaceholderTitle(title))
            return false;
        if (managed.preview || managed.lastMessageRole)
            return false;
        return true;
    }
    shouldInspectExternalPlaceholderSession(managed) {
        if (!managed)
            return true;
        if (this.hasNoRenderableLocalMessages(managed))
            return true;
        if (managed.preview)
            return false;
        const hasLoadedUserMessage = managed.messages.some((message) => message.role === 'user' && message.content.trim());
        if (hasLoadedUserMessage)
            return false;
    ...
    async loadExternalListedMessages(info, loadMessages) {
        if (!loadMessages)
            return undefined;
        try {
    ...
    removeExternalListedLocalMirror(workspace, managed) {
    ...
    applyAvailableCommandsSnapshot(managed, snapshot, source) {
        if (snapshot.availableCommands.length === 0 &&
    ...
    applyAvailableCommandsFromMessagesResult(managed, result, source) {
        if (!result ||
    ...
    applyTokenUsageFromMessagesResult(managed, result) {
        if (!result?.tokenUsage)
            return;
        managed.tokenUsage = { ...result.tokenUsage };
    }
    applyLoadedExternalMessages(managed, messages) {
    ...
    inferLegacyQwenCanonicalAttachmentOverlays(managed, messages) {
        ...
        if (userMessages.length === 0)
            return [];
        const attachmentsDir = join(getSessionStoragePath(managed.workspace.rootPath, managed.id), 'attachments');
        ...
        try {
            const legacyAttachments = readdirSync(attachmentsDir)
                ...
                .sort((a, b) => LEGACY_ATTACHMENT_NAME_COLLATOR.compare(a.name, b.name) ||
                a.stats.mtimeMs - b.stats.mtimeMs ||
                a.fileName.localeCompare(b.fileName));
            for (const { fileName, mimeType, name, stats, storedPath, } of legacyAttachments) {
                ...
                if (!matchedMessage)
                    continue;
                const attachmentId = fileName.match(/^([0-9a-f-]{36})_/i)?.[1] ?? randomUUID();
            ...
            sessionLog.warn(`Failed to infer legacy Qwen attachments for session ${managed.id}:`, error);
            return [];
        }
        const overlays = [];
    ...
    async upsertExternalListedSession(args) {
        ...
        const createdTimestamp = parseOptionalTimestamp(info.createdAt) ??
            parseOptionalTimestamp(info.startTime);
        let title = info.title?.trim() || EXTERNAL_SESSION_PLACEHOLDER_TITLE;
        let managed = this.selectManagedSessionBySdkSessionId(workspace.id, info.sessionId);
        if (managed) {
            this.removeDuplicateExternalListedMirrors(workspace, managed, info.sessionId);
            if (connectionSlug === QWEN_CODE_CONNECTION_SLUG) {
                managed = await this.canonicalizeQwenManagedSessionId(workspace, managed, info.sessionId);
            }
        }
        let inspectedResult;
        if (this.isExternalSessionPlaceholderTitle(title) &&
        ...
        if (managed) {
            ...
            if (!managed.model && model)
                managed.model = model;
            if (!managed.thinkingLevel)
                managed.thinkingLevel = defaultThinkingLevel;
            if (connectionSlug === QWEN_CODE_CONNECTION_SLUG) {
        ...
        const createdAt = firstInspectedMessage?.timestamp ?? createdTimestamp ?? timestamp;
        const inMemoryLastMessageAt = lastInspectedMessage?.timestamp ?? timestamp;
        const storedSessionBase = {
            id: info.sessionId,
        ...
        const storedMetadata = pickSessionFields(storedSession);
        ...
        if (inspectedResult?.availableCommands?.length ||
    ...
    async removeMissingExternalListedSessions(workspace, connectionSlug, seenSdkSessionIds) {
        let removed = false;
        for (const managed of Array.from(this.sessions.values())) {
            ...
            if (!managed.sdkSessionId || managed.id !== managed.sdkSessionId)
                continue;
            if (seenSdkSessionIds.has(managed.sdkSessionId))
                continue;
            if (managed.isProcessing)
                continue;
            this.sessions.delete(managed.id);
    ...
    async persistSessionMetadataUpdate(managed, updates) {
     ...
     */
    getAuthRequestDescription(request) {
        switch (request.type) {
            case 'credential':
     ...
     */
    formatAuthResultMessage(result) {
        if (result.success) {
            let msg = `Authentication completed for ${result.sourceSlug}.`;
            if (result.email)
                msg += ` Signed in as ${result.email}.`;
            if (result.workspace)
     ...
     */
    async completeAuthRequest(sessionId, result) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
        ...
        const authMessage = managed.messages.find((m) => m.role === 'auth-request' &&
     ...
     */
    async handleCredentialInput(sessionId, requestId, response) {
        const managed = this.sessions.get(sessionId);
    ...
    getActiveSessionCount(workspaceId) {
        let count = 0;
        for (const managed of this.sessions.values()) {
            if (workspaceId && managed.workspace.id !== workspaceId)
                continue;
            if (managed.isProcessing)
                count++;
        }
    ...
    getWorkspaceAutomationSummary(workspaceId) {
    ...
    emitSessionListChanged(workspaceId) {
        if (!this.eventSink)
            return;
        this.eventSink(RPC_CHANNELS.sessions.LIST_CHANGED, { to: 'all' }, workspaceId);
    }
    emitSessionListRefreshStateChanged(workspaceId, isRefreshing) {
        if (!this.eventSink)
            return;
        this.eventSink(RPC_CHANNELS.sessions.LIST_REFRESH_STATE_CHANGED, { to: 'all' }, workspaceId, isRefreshing);
    }
    /**
     ...
     */
    async ensureMessagesLoaded(managed) {
     ...
     */
    async loadMessagesFromDisk(managed) {
        ...
        if (storedSession) {
            ...
            if (storedSession.name !== undefined ||
            ...
            const orphanedQueued = usesQwenCanonicalMessages
                ? []
                : managed.messages.filter((m) => m.role === 'user' && m.isQueued === true);
    ...
    shouldAttemptExternalMessageLoad(managed) {
        if (this.isQwenCanonicalMessageSession(managed)) {
            if (managed.isProcessing)
                return false;
    ...
    async loadExternalMessagesForEmptyLoadedSession(managed) {
        if (!this.shouldAttemptExternalMessageLoad(managed))
            return;
        const externalLoadResult = await this.loadExternalSessionMessages(managed);
        managed.externalMessagesLoadAttempted =
    ...
    markExternalMessagesLoadedThrough(managed) {
        managed.externalMessagesLoadedThroughAt = Math.max(managed.externalMessagesLoadedThroughAt ?? 0, managed.lastMessageAt ?? 0, managed.lastUsedAt ?? 0);
    }
    findMessageForContentUpdate(managed, messageId) {
        ...
        if (this.isQwenCanonicalMessageSession(managed) &&
            ...
            if (latestUserIndex >= 0) {
                sessionLog.info(`Mapped stale Qwen history message id ${messageId} to latest user message ${managed.messages[latestUserIndex].id} in session ${managed.id}`);
                return {
    ...
    async createSession(workspaceId, options) {
        ...
        const defaultThinkingLevel = normalizeThinkingLevel(options?.thinkingLevel) ??
        ...
        if (resolvedModelOption === 'fast' || resolvedModelOption === 'default') {
            const tierConnection = resolveSessionConnection(options?.llmConnection, wsConfig?.defaults?.defaultLlmConnection);
            if (tierConnection) {
                resolvedModelOption =
        ...
        else if (options?.workingDirectory === 'user_default' ||
        ...
        if (options?.branchFromSessionId || options?.branchFromMessageId) {
            ...
            const branchContextStrategy = 'sdk-fork';
        ...
        if (validatedBranch) {
            ...
            if (sourceDir !== branchDir) {
                branchedStored.messages = sourceMessages.map((m) => {
                    ...
                    if (!json.includes(sourceDir))
                        return m;
                    return JSON.parse(json.replaceAll(sourceDir, branchDir));
                });
            }
            else {
     ...
     */
    async getOrCreateAgent(managed) {
        if (managed.agent)
    ...
    createMidTurnMessagesDrainedCallback(managed) {
        return (messageIds) => {
            ...
            if (drainedEntries.length < messageIds.length) {
                sessionLog.warn(`Mid-turn drain acknowledgement matched ${drainedEntries.length}/${messageIds.length} entries for session ${managed.id}`);
            }
    ...
    async createAgentForManagedSession(managed) {
        if (!managed.agent) {
            ...
            managed.agentReady = new Promise((r) => {
            ...
            const sessionConfig = {
                ...
                workspaceRootPath: managed.workspace.rootPath,
                sdkSessionId: managed.sdkSessionId,
                branchFromSdkSessionId: managed.branchContextStrategy === 'sdk-fork'
                    ? managed.branchFromSdkSessionId
                    : undefined,
                branchFromSessionPath: managed.branchContextStrategy === 'sdk-fork'
                    ? managed.branchFromSessionPath
                    : undefined,
                branchFromSdkCwd: managed.branchContextStrategy === 'sdk-fork'
                    ? managed.branchFromSdkCwd
                    : undefined,
                branchFromSdkTurnId: managed.branchContextStrategy === 'sdk-fork'
            ...
            const onSdkSessionIdUpdate = (sdkSessionId) => {
                managed.sdkSessionId = sdkSessionId;
            ...
            const markTransferredSessionSummaryApplied = () => {
                if (managed.transferredSessionSummaryApplied ||
                    !managed.transferredSessionSummary)
                    return;
            ...
            const onMidTurnMessagesDrained = this.createMidTurnMessagesDrainedCallback(managed);
            // ============================================================
            ...
            managed.agent = createBackendFromResolvedContext({
                ...
                coreConfig: {
                    ...
                    onImageResize: async (filePath, maxSizeBytes) => {
                        try {
            ...
            managed.agent.onBackendAuthRequired = (reason) => {
            ...
            if (this.browserPaneManager) {
                ...
                const resolveSessionBrowserInstance = (toolName, options) => {
                ...
                const resolveLifecycleWindowTarget = (command, requestedInstanceId) => {
            ...
            managed.agent.onPlanSubmitted = async (planPath) => {
                ...
                try {
                    ...
                    const submitPlanMsg = managed.messages.find((m) => m.toolName?.includes('SubmitPlan') &&
            ...
            mergeSessionScopedToolCallbacks(managed.id, {
                setSessionLabelsFn: (sessionId, labels) => {
                    this.setSessionLabels(sessionId ?? managed.id, labels);
                },
                setSessionStatusFn: async (sessionId, status) => {
                    await this.setSessionStatus(sessionId ?? managed.id, status);
                },
                getSessionInfoFn: (sessionId) => {
                ...
                sendAgentMessageFn: async (sessionId, message, attachments) => {
                    ...
                    if (attachments?.length) {
                        ...
                        if (builtAttachments.length > 0)
                            fileAttachments = builtAttachments;
                    }
            ...
            managed.agent.onSourceActivationRequest = async (sourceSlug) => {
    ...
    async setSessionStatus(sessionId, sessionStatus) {
        const managed = this.sessions.get(sessionId);
        if (managed) {
     ...
     */
    async setSessionConnection(sessionId, connectionSlug) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
     ...
     */
    async setPendingPlanExecution(sessionId, planPath, draftInputSnapshot) {
        const managed = this.sessions.get(sessionId);
        if (managed) {
     ...
     */
    async markCompactionComplete(sessionId) {
        const managed = this.sessions.get(sessionId);
        if (managed) {
     ...
     */
    async markPendingPlanExecutionDispatched(sessionId) {
        const managed = this.sessions.get(sessionId);
        if (managed) {
     ...
     */
    async clearPendingPlanExecution(sessionId) {
        const managed = this.sessions.get(sessionId);
        if (managed) {
     ...
     * path.
     */
    async acceptPlan(sessionId, _planPath) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
     ...
     */
    async setSessionSources(sessionId, sourceSlugs) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
     ...
     */
    getLastFinalAssistantMessageId(messages) {
     ...
     */
    setActiveViewingSession(sessionId, workspaceId) {
        if (sessionId) {
     ...
     */
    clearActiveViewingSession(workspaceId) {
        this.activeViewingSession.delete(workspaceId);
    }
    /**
     * Check if a session is currently being viewed by the user
     */
    isSessionBeingViewed(sessionId, workspaceId) {
     ...
     */
    async markAllSessionsRead(workspaceId) {
        const updates = [];
        for (const managed of this.sessions.values()) {
            if (managed.workspace.id !== workspaceId)
                continue;
            if (managed.hidden || managed.isArchived)
                continue;
            if (managed.isProcessing)
                continue;
            if (!managed.hasUnread)
                continue;
            managed.hasUnread = false;
            updates.push(this.persistSessionMetadataUpdate(managed, { hasUnread: false }));
        }
        if (updates.length > 0) {
    ...
    async refreshTitle(sessionId) {
        ...
        const titleLangCode = (i18n.resolvedLanguage ?? 'en');
    ...
    updateWorkingDirectory(sessionId, path) {
        ...
        if (managed) {
            ...
            const shouldUpdateSdkCwd = managed.messages.length === 0 &&
                !managed.sdkSessionId &&
                !managed.agent;
     ...
     */
    async updateSessionModel(sessionId, workspaceId, model, connection) {
        ...
        const nextConnectionSlug = connection && !managed.connectionLocked
            ? connection
        ...
        const provider = sessionConn
            ...
            : 'qwen';
        const resolveModel = (candidate) => resolveModelForProvider(provider, candidate, sessionConn);
        ...
        const updates = { model: persistedModel };
        await this.persistSessionMetadataUpdate(managed, updates);
        ...
        if (managed.agent) {
            ...
            const effectiveModel = resolveModel(persistedModel ??
    ...
    async cleanupDraftAgent(agent, workspace, workingDirectory, reason) {
    ...
    async persistDraftModelSelection(workspaceId, model, connection) {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace) {
    ...
    async refreshQwenConnectionDefault(slug, reason) {
        try {
     ...
     */
    async updateMessageContent(sessionId, messageId, content) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
        ...
        this.sendMessage(sessionId, nextContent, attachments, storedAttachments, undefined, message.id).catch((error) => {
    ...
    rehydrateStoredAttachments(storedAttachments) {
     ...
     */
    updateMessageAnnotation(sessionId, messageId, annotationId, patch) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
        ...
        if (patch.target?.source?.messageId &&
    ...
    async deleteSession(sessionId) {
        ...
        if (managed.sharedId) {
            try {
                const { VIEWER_URL } = await import('@craft-agent/shared/branding');
                const response = await fetch(`${VIEWER_URL}/s/api/${managed.sharedId}`, { method: 'DELETE', signal: AbortSignal.timeout(5000) });
                if (!response.ok) {
    ...
    async sendMessage(sessionId, message, attachments, storedAttachments, options, existingMessageId, eventClientId, _isAuthRetry) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
        ...
        if (managed.isProcessing) {
            ...
            const hasStoredAttachmentsWithoutLivePayload = (storedAttachments?.length ?? 0) > 0 && !attachments?.length;
            const canInjectMidTurn = !hasStoredAttachmentsWithoutLivePayload &&
        ...
        if (shouldPreEnableLocalSkillSources) {
            try {
                ...
                if (requiredSources.size > 0) {
                    ...
                    const usableSources = new Set(loadedSources
                        .filter(isSourceUsable)
                        .map((source) => source.config.slug));
                    for (const srcSlug of candidateSlugs) {
                        if (currentSlugs.has(srcSlug))
                            continue;
        ...
        if (managed.enabledSourceSlugs?.length) {
            ...
            if (managed.tokenRefreshManager) {
                const refreshResult = await refreshOAuthTokensIfNeeded(agent, sources, sessionPath, managed.tokenRefreshManager, {
                    sessionId,
                    workspaceRootPath,
    ...
    async cancelProcessing(sessionId, silent = false) {
        const managed = this.sessions.get(sessionId);
        if (!managed?.isProcessing) {
        ...
        const queuedMessageIds = new Set(managed.messageQueue
            .map((q) => q.messageId)
            .filter((id) => !!id));
     ...
     */
    attemptAuthRetry(sessionId, managed, workspaceId, failureErrorCode) {
        if (managed.authRetryAttempted || !managed.lastSentMessage)
            return false;
        ...
        setImmediate(async () => {
            try {
                ...
                if (retryMessage) {
                    ...
                    await this.sendMessage(sessionId, retryMessage, retryAttachments, retryStoredAttachments, retryOptions, undefined, // existingMessageId
     ...
     */
    async onProcessingStopped(sessionId, reason) {
        const managed = this.sessions.get(sessionId);
        if (!managed)
            return;
        ...
        const didReceiveNewFinalMessage = !!currentFinalMessageId &&
    ...
    processNextQueuedMessage(sessionId) {
        const managed = this.sessions.get(sessionId);
        if (!managed || managed.messageQueue.length === 0)
            return;
        ...
        setImmediate(() => {
            this.sendMessage(sessionId, next.message, next.attachments, next.storedAttachments, next.options, next.messageId, next.eventClientId).catch((err) => {
    ...
    async killShell(sessionId, shellId) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            return { success: false, error: 'Session not found' };
        }
     ...
     */
    respondToPermission(sessionId, requestId, allowed, alwaysAllow, options) {
        const managed = this.sessions.get(sessionId);
        if (managed?.agent) {
     ...
     */
    async respondToCredential(sessionId, requestId, response) {
    ...
    async applyGlobalPermissionMode(mode, options = {}) {
        this.currentGlobalPermissionMode = mode;
        const changedBy = options.changedBy ?? 'user';
        let changedCount = 0;
        for (const managed of this.sessions.values()) {
            if (this.applyPermissionModeToManagedSession(managed, mode, changedBy, {
                suppressLog: true,
            })) {
                changedCount += 1;
            }
     ...
     */
    async setGlobalPermissionMode(mode, options = {}) {
        await this.applyGlobalPermissionMode(mode, options.changedBy ? { changedBy: options.changedBy } : {});
    ...
    applyPermissionModeToManagedSession(managed, mode, changedBy, options = {}) {
        const sessionId = managed.id;
    ...
    async persistQwenApprovalMode(mode, preferredSessionId) {
        ...
        for (const managed of candidates) {
            const agent = managed.agent;
            if (!agent || !canManageQwenSettings(agent))
                continue;
            try {
                await agent.setCoreSetting('user', 'tools.approvalMode', mapPermissionModeToQwenApprovalMode(mode));
                return;
            }
            catch (error) {
    ...
    async getSessionPermissionSettings(sessionId) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async setSessionPermissionRules(sessionId, scope, ruleType, rules) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async getSessionQwenCoreSettings(sessionId) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async listSessionQwenProviders(sessionId) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async connectSessionQwenProvider(sessionId, params) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async setSessionQwenCoreSetting(sessionId, scope, key, value) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async setSessionQwenMcpServer(sessionId, scope, name, server) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async removeSessionQwenMcpServer(sessionId, scope, name) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async setSessionQwenHook(sessionId, scope, event, index, hook) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async removeSessionQwenHook(sessionId, scope, event, index) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    async setSessionQwenExtensionSetting(sessionId, extensionId, settingKey, scope, value) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            throw new Error(`Session ${sessionId} not found`);
        }
     ...
     */
    getSessionPermissionModeState(sessionId) {
        const managed = this.sessions.get(sessionId);
        if (!managed)
            return null;
        ...
        if (diagnostics.permissionMode !== globalPermissionMode) {
            sessionLog.warn('Permission mode diagnostics mismatch, reconciling to global mode', {
                sessionId,
     ...
     */
    setSessionLabels(sessionId, labels) {
        const managed = this.sessions.get(sessionId);
        if (managed) {
    ...
    async refreshDraftAvailableCommands(options) {
        ...
        const workingDirectory = options.workingDirectory ??
            workspaceConfig?.defaults?.workingDirectory ??
            undefined;
        const permissionMode = options.permissionMode ?? this.currentGlobalPermissionMode;
    ...
    async installDraftQwenSkill(skill, options) {
        if (!options.workspaceId) {
            return {
                success: false,
        ...
        try {
            ...
            if (agent.refreshAvailableCommands) {
                try {
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    sessionLog.warn(`installQwenSkill: post-install command refresh failed: ${message}`);
                }
            }
            ...
            if (agent) {
                await this.cleanupDraftAgent(agent, workspace, workingDirectory, 'draft skill installation complete');
            }
        }
    }
    async deleteDraftQwenSkill(skill, options) {
        if (!options.workspaceId) {
            return {
                success: false,
        ...
        try {
            ...
            const snapshot = agent.refreshAvailableCommands
                ? await agent.refreshAvailableCommands().catch((error) => {
                    const message = error instanceof Error ? error.message : String(error);
                    sessionLog.warn(`deleteQwenSkill: post-delete command refresh failed: ${message}`);
                    return null;
                })
                : null;
            return {
            ...
            if (agent) {
                await this.cleanupDraftAgent(agent, workspace, workingDirectory, 'draft skill deletion complete');
            }
        }
    }
    async setDraftQwenSkillEnabled(skill, options) {
        if (!options.workspaceId) {
            return {
                success: false,
        ...
        try {
            ...
            const snapshot = agent.refreshAvailableCommands
                ? await agent.refreshAvailableCommands().catch((error) => {
                    const message = error instanceof Error ? error.message : String(error);
                    sessionLog.warn(`setQwenSkillEnabled: post-update command refresh failed: ${message}`);
                    return null;
                })
                : null;
            return {
            ...
            if (agent) {
                await this.cleanupDraftAgent(agent, workspace, workingDirectory, 'draft skill update complete');
            }
        }
    }
    async refreshAvailableCommands(sessionId, options) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            if (options?.workspaceId) {
    ...
    async installQwenSkill(sessionId, skill, options) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            if (options?.workspaceId) {
        ...
        try {
            ...
            if (agent.refreshAvailableCommands) {
                snapshot = await agent.refreshAvailableCommands();
                if (snapshot) {
                    this.applyAvailableCommandsSnapshot(managed, snapshot, 'skill install refresh');
                }
            }
            return {
                success: true,
            ...
            const message = error instanceof Error ? error.message : String(error);
            sessionLog.warn(`installQwenSkill failed for session ${sessionId}: ${message}`);
            return { success: false, error: message };
        }
    }
    async deleteQwenSkill(sessionId, skill, options) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            if (options?.workspaceId) {
        ...
        try {
            ...
            if (agent.refreshAvailableCommands) {
                snapshot = await agent.refreshAvailableCommands();
                if (snapshot) {
                    this.applyAvailableCommandsSnapshot(managed, snapshot, 'skill delete refresh');
                }
            }
            return {
                success: true,
            ...
            const message = error instanceof Error ? error.message : String(error);
            sessionLog.warn(`deleteQwenSkill failed for session ${sessionId}: ${message}`);
            return { success: false, error: message };
        }
    }
    async setQwenSkillEnabled(sessionId, skill, options) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
            if (options?.workspaceId) {
        ...
        try {
            ...
            if (agent.refreshAvailableCommands) {
                snapshot = await agent.refreshAvailableCommands();
                if (snapshot) {
                    this.applyAvailableCommandsSnapshot(managed, snapshot, 'skill enabled refresh');
                }
            }
            return {
                success: true,
            ...
            const message = error instanceof Error ? error.message : String(error);
            sessionLog.warn(`setQwenSkillEnabled failed for session ${sessionId}: ${message}`);
            return { success: false, error: message };
        }
    }
    /**
     ...
     */
    async generateTitle(managed, userMessage) {
        ...
        try {
            const genLangCode = (i18n.resolvedLanguage ?? 'en');
    ...
    async processEvent(managed, event) {
        const sessionId = managed.id;
        ...
        switch (event.type) {
            ...
            case 'tool_start': {
                ...
                if (existingStartMsg) {
                    ...
                    if (formattedToolInput &&
                        ...
                        const hadInputBefore = existingStartMsg.toolInput &&
                ...
                const shouldActivateOverlay = shouldActivateBrowserOverlay(event.toolName, formattedToolInput);
            ...
            case 'tool_result': {
                ...
                const inferredError = event.isError === true ||
                ...
                if (isParentTaskTool(toolName) || toolName === 'TaskOutput') {
                    const pendingChildren = managed.messages.filter((m) => m.parentToolUseId === event.toolUseId &&
            ...
            case 'error': {
                ...
                const isPlainAuthError = lowerErr.includes('token is expired') ||
            ...
            case 'typed_error': {
                ...
                if (typedErrorMsg.includes('aborted') ||
    ...
    sendEvent(event, workspaceId) {
        ...
        const eventWithContext = { ...event, workspaceId };
        const eventClientId = this.sessionEventClientIds.get(event.sessionId);
        if (eventClientId) {
            this.eventSink(RPC_CHANNELS.sessions.EVENT, { to: 'workspace', workspaceId, exclude: eventClientId }, eventWithContext);
            this.eventSink(RPC_CHANNELS.sessions.EVENT, { to: 'client', clientId: eventClientId }, eventWithContext);
            return;
        }
        this.eventSink(RPC_CHANNELS.sessions.EVENT, { to: 'workspace', workspaceId }, eventWithContext);
    }
    /**
     ...
     */
    queueDelta(sessionId, workspaceId, delta, turnId) {
     ...
     */
    flushDelta(sessionId, workspaceId) {
        // Clear the timer
        const timer = this.deltaFlushTimers.get(sessionId);
        if (timer) {
            clearTimeout(timer);
     ...
     */
    async executePromptAutomation(workspaceId, workspaceRootPath, prompt, labels, permissionMode, mentions, llmConnection, model, automationName) {
     ...
     */
    resolveAutomationMentions(workspaceRootPath, mentions) {
        const sources = loadWorkspaceSources(workspaceRootPath);
        const skills = loadAllSkills(workspaceRootPath);
        const sourceSlugs = [];
        const skillSlugs = [];
    ...
    // ============================================
    async generateRemoteTransferSummary(managed) {
        await this.ensureMessagesLoaded(managed);
    ...
    async exportRemoteSessionTransfer(sessionId, workspaceId) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
    ...
    async importRemoteSessionTransfer(workspaceId, payload) {
        if (!payload ||
     ...
     */
    async exportSession(sessionId, workspaceId) {
        const managed = this.sessions.get(sessionId);
        if (!managed) {
     ...
     */
    async importSession(workspaceId, bundle, mode) {
     ...
     */
    findCompatibleLlmConnection(workspaceRootPath, providerType) {
        const wsConfig = loadWorkspaceConfig(workspaceRootPath);
