...
import { isAlwaysBlockedVoiceAddress, isLoopbackHost, isLoopbackVoiceAddress, isPrivateNetworkIp, } from './net-guard';
const DEFAULT_DASHSCOPE_BASE_URL = 'https://dashscope.aliyuncs.com/compatible-mode/v1';
const NO_CREDENTIALS_ERROR = 'Voice dictation needs Qwen credentials. Sign in to Qwen Code (or set a DashScope API key), then try again.';
 ...
 * Exported for tests.
 */
export function getQwenConfigDir() {
...
async function readQwenJsonFromDisk(file) {
    return readJsonFileFromDisk(join(getQwenConfigDir(), file));
}
async function readJsonFileFromDisk(file) {
    try {
        ...
        if (error.code !== 'ENOENT') {
            voiceConfigLogger.warn(`[voice] Failed to parse trusted settings file ${file}:`, error instanceof Error ? error.message : String(error));
        }
        return undefined;
    }
}
async function readEnvFileFromDisk(file) {
    try {
        return await readFile(file, 'utf-8');
    }
    catch (error) {
        if (error.code !== 'ENOENT') {
            voiceConfigLogger.warn(`[voice] Failed to read home .env file ${file}:`, error instanceof Error ? error.message : String(error));
        }
        return undefined;
    }
}
...
// home .env.
async function getHomeEnvFallback(env, readHomeEnvFile) {
...
function getSystemSettingsPath(env, currentPlatform) {
...
function getSystemDefaultsPath(env, systemSettingsPath, currentPlatform) {
...
async function fromOAuth(deps) {
    const creds = await deps.readQwenJson('oauth_creds.json');
...
function resolveSettingsEnvVars(value, env, homeEnvFallback) {
    const resolveValue = (input) => {
        if (typeof input === 'string') {
            return input.replace(/\$(?:(\w+)|{([^}]+)})/g, (placeholder, plainName, bracedName) => {
...
function unwrapWrappedProviderConfigs(modelProviders) {
    if (!modelProviders) {
    ...
    for (const [key, value] of Object.entries(modelProviders)) {
        ...
        unwrapped[key] = Array.isArray(models) ? models : [];
    }
...
function omitUnsafeKeys(source) {
    if (!source) {
        return source;
    }
...
function mergeModelProviders(...scopes) {
    let merged;
...
function mergeTrustedQwenSettings(systemDefaults, user, system) {
    return {
        env: {
        ...
        modelProviders: mergeModelProviders(systemDefaults?.modelProviders, user?.modelProviders, system?.modelProviders),
        security: {
...
function normalizeAllowedVoiceBaseUrl(raw) {
    try {
...
export function isDashscopeCompatible(url) {
    try {
        const u = new URL(url);
        return (/(^|\.)dashscope(-intl|-us)?\.aliyuncs\.com$/.test(u.hostname) &&
...
function readProviderApiKey(provider, settings, envSource) {
...
const PROVIDER_ENTRY_REMEDY = 'Remove or complete this provider entry to fall back to your Qwen sign-in.';
/**
 ...
 * fall-through.
 */
function classifyVoiceProviderEntry(provider, settings, voiceModel) {
    if (provider.baseUrl != null && typeof provider.baseUrl !== 'string') {
        throw new Error(`Voice model '${voiceModel}' baseUrl must be a string. ${PROVIDER_ENTRY_REMEDY}`);
    }
    ...
    try {
        ...
        voiceConfigLogger.warn(`[voice] Provider baseUrl for voice model '${voiceModel}' is not a valid URL (${rawBaseUrl}); ignoring the entry and falling back to the next credential source.`);
        return undefined;
    }
    if (parsedBaseUrl.username || parsedBaseUrl.password) {
        throw new Error(`Voice model '${voiceModel}' baseUrl must not contain embedded credentials. ${PROVIDER_ENTRY_REMEDY}`);
    }
    ...
    const isPublicHttps = parsedBaseUrl.protocol === 'https:' &&
        !isLoopback &&
...
function needsVoicePolicyDecision(entry) {
 ...
 */
function fromExactModelProvider(settings, envSource, voiceModel) {
    if (!settings)
        return undefined;
    ...
    const matches = providers.filter((provider) => provider !== null &&
    ...
    const classified = distinct
        .map((provider) => classifyVoiceProviderEntry(provider, settings, voiceModel))
    ...
    const { provider, parsedBaseUrl, baseUrl, allowInsecureBaseUrl, isLoopback } = entry;
    if (parsedBaseUrl.protocol !== 'http:' &&
        parsedBaseUrl.protocol !== 'https:') {
        throw new Error(`Voice model '${voiceModel}' must use an http or https baseUrl. ${PROVIDER_ENTRY_REMEDY}`);
    }
    ...
    if (parsedBaseUrl.protocol === 'http:' &&
        !isLoopback &&
        !allowInsecureBaseUrl) {
        throw new Error(`Voice model '${voiceModel}' must use an https baseUrl. Voice audio must not be transmitted in cleartext. To trust this managed endpoint, add its exact complete normalized URL (${baseUrl}) to security.allowedInsecureVoiceBaseUrls. ${PROVIDER_ENTRY_REMEDY}`);
    }
    if (!isLoopback &&
        !allowInsecureBaseUrl &&
        isPrivateNetworkIp(parsedBaseUrl.hostname)) {
        throw new Error(`Voice model '${voiceModel}' must not use a private-network baseUrl. To trust this managed endpoint, add its exact complete normalized URL (${baseUrl}) to security.allowedInsecureVoiceBaseUrls. ${PROVIDER_ENTRY_REMEDY}`);
    }
    if (provider.envKey != null && typeof provider.envKey !== 'string') {
        throw new Error(`Voice model '${voiceModel}' envKey must be a string. ${PROVIDER_ENTRY_REMEDY}`);
    }
    const envKey = provider.envKey?.trim();
    if (!envKey) {
    ...
    if (!apiKey) {
        throw new Error(`Voice model '${voiceModel}' requires ${envKey}. ${PROVIDER_ENTRY_REMEDY}`);
    }
 ...
 */
function fromLegacyDashscopeProvider(settings, envSource) {
    if (!settings)
        return undefined;
    const providers = Object.values(settings.modelProviders ?? {}).flat();
...
function fromEnv(env) {
    ...
    if (dashscopeKey) {
        return {
            apiKey: dashscopeKey,
            baseUrl: normalizeBaseUrl(env['DASHSCOPE_PROXY_BASE_URL']?.trim() || DEFAULT_DASHSCOPE_BASE_URL),
        };
    }
    ...
    if (openaiKey) {
        throw new Error('Set OPENAI_BASE_URL alongside OPENAI_API_KEY to use it for voice dictation.');
    }
    return undefined;
}
export async function resolveDesktopVoiceConfig(deps = {}) {
    const resolvedDeps = {
    ...
    const systemSettingsPath = deps.systemSettingsPath ??
        getSystemSettingsPath(resolvedDeps.env, resolvedDeps.platform);
    const systemDefaultsPath = deps.systemDefaultsPath ??
        getSystemDefaultsPath(resolvedDeps.env, systemSettingsPath, resolvedDeps.platform);
    const [rawSystemDefaults, rawUserSettings, rawSystemSettings] = await Promise.all([
        resolvedDeps.readSystemJson(systemDefaultsPath),
        resolvedDeps.readQwenJson('settings.json'),
        resolvedDeps.readSystemJson(systemSettingsPath),
    ]);
    const homeEnvFallback = await getHomeEnvFallback(resolvedDeps.env, resolvedDeps.readHomeEnvFile);
    ...
    const lookupEnv = { ...homeEnvFallback };
    for (const [key, value] of Object.entries(resolvedDeps.env)) {
        if (value !== '' || !Object.hasOwn(homeEnvFallback, key)) {
            lookupEnv[key] = value;
        }
    }
    const systemDefaults = resolveSettingsEnvVars(rawSystemDefaults, resolvedDeps.env, homeEnvFallback);
    const userSettings = resolveSettingsEnvVars(rawUserSettings, resolvedDeps.env, homeEnvFallback);
    const systemSettings = resolveSettingsEnvVars(rawSystemSettings, resolvedDeps.env, homeEnvFallback);
    const qwenSettings = mergeTrustedQwenSettings(systemDefaults, userSettings, systemSettings);
    const creds = fromExactModelProvider(qwenSettings, lookupEnv, voiceModel) ??
    ...
    const allowInsecureBaseUrl = isInsecureVoiceBaseUrlAllowed(qwenSettings, creds.baseUrl);
    ...
    if (!isLoopbackHost(parsed.hostname) &&
        ...
        throw new Error(isLoopbackVoiceAddress(parsed.hostname)
    ...
    if (parsed.protocol === 'http:' &&
        ...
        !allowInsecureBaseUrl) {
        throw new Error(`Voice endpoint must use an https baseUrl, or its exact complete normalized URL (${creds.baseUrl}) must be listed in security.allowedInsecureVoiceBaseUrls.`);
    }
    if (!isLoopbackHost(parsed.hostname) &&
        !allowInsecureBaseUrl &&
        isPrivateNetworkIp(parsed.hostname)) {
        throw new Error(`Voice endpoint must not use a private-network baseUrl. To trust this managed endpoint, add its exact complete normalized URL (${creds.baseUrl}) to security.allowedInsecureVoiceBaseUrls.`);
    }
    return {
        model: voiceModel,
