...
import { isRetryableVoiceStreamError, openVoiceStreamWithRetry, } from './voice-stream-retry';
...
describe('openVoiceStreamWithRetry', () => {
    ...
    it('gives up after the single retry when the retry also fails', async () => {
        ...
        try {
            ...
            const settled = promise.then(() => ({ ok: true }), (error) => ({ ok: false, error }));
            await flushMicrotasks();
            jest.advanceTimersByTime(200);
...
//# sourceMappingURL=voice-stream-retry.test.js.map