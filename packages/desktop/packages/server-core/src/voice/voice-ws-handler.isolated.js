...
const { createVoiceConnectionHandler, toStreamConfig } = await import('./voice-ws-handler');
...
class FakeWebSocket {
    ...
    emitMessage(data, isBinary = false) {
        for (const cb of this.handlers.get('message') ?? [])
            cb(data, isBinary);
    }
    emitClose() {
        for (const cb of this.handlers.get('close') ?? [])
            cb();
    }
    sentJson() {
...
describe('createVoiceConnectionHandler', () => {
    ...
    it('reaches the default batch transport when the private-network opt-in is set', async () => {
        ...
        try {
            ...
            expect(ws.sentJson().some((message) => message.type === 'error')).toBe(false);
        }
        finally {
            restore();
        }
    ...
    it('blocks the default batch transport for a private gateway without the opt-in', async () => {
        ...
        try {
            ...
            expect(ws.sentJson().some((message) => message.type === 'error' && /private-network/.test(message.message))).toBe(true);
        }
        finally {
            restore();
        }
    ...
    it('redacts credentials from streaming session errors before sending them to the renderer', async () => {
        ...
        onError?.(new Error('Bearer leaked-token connecting to wss://host/api-ws?apikey=leaked-token'));
        await flush();
...
//# sourceMappingURL=voice-ws-handler.isolated.js.map