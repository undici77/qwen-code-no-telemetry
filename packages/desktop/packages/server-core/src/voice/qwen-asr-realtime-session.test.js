...
const { deriveQwenRealtimeUrl, openQwenAsrRealtimeStream } = await import('./qwen-asr-realtime-session');
class FakeSocket {
    OPEN = 1;
...
describe('openQwenAsrRealtimeStream', () => {
    it('opens a Qwen realtime session and returns committed transcripts', async () => {
        ...
        expect(socket.url).toBe(deriveQwenRealtimeUrl('https://dashscope.aliyuncs.com/compatible-mode/v1', 'qwen3-asr-flash-realtime'));
        ...
        expect(socket.sent
            .slice(-2)
            .map((message) => JSON.parse(String(message)).type)).toEqual(['input_audio_buffer.commit', 'session.finish']);
    ...
    it('redacts credentials from realtime server errors', async () => {
        ...
        socket.emit('message', JSON.stringify({
            ...
            error: {
                code: 'InvalidApiKey',
                message: 'Authorization Bearer sk-secret-token was rejected for sk-secret-token',
            },
        }));
    ...
    it('does not double-fire onError when a close is followed by another terminal event', async () => {
        ...
        const streamPromise = openQwenAsrRealtimeStream({
            baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
            model: 'qwen3-asr-flash-realtime',
        }, { onError: (error) => errors.push(error) }, { createWebSocket: () => socket });
        socket.emit('message', JSON.stringify({ type: 'session.created' }));
        socket.emit('message', JSON.stringify({ type: 'session.updated' }));
        await streamPromise;
