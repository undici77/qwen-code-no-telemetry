...
import { classifyVoiceUpgrade, closeVoiceClients, closeVoiceServerResources, isAllowedVoiceOrigin, startVoiceServer, terminateDisabledVoiceClients, terminateVoiceClients, tokenMatches, } from './voice-server';
...
describe('isAllowedVoiceOrigin', () => {
    it('allows app origins and rejects browser origins', () => {
        ...
        expect(isAllowedVoiceOrigin('http://localhost:5173', [
            'http://localhost:5173',
        ])).toBe(true);
...
describe('closeVoiceServerResources', () => {
    ...
    it('logs shutdown start (with client count), force-terminate, and completion', async () => {
        ...
        logger);
        expect(logger.infos.some((m) => m.includes('shutting down stream server (1 active client(s))'))).toBe(true);
        // The fake client never removes itself from the set, so it is a straggler.
        expect(logger.warnings.some((m) => m.includes('force-terminated 1 straggling client(s)'))).toBe(true);
        expect(logger.infos.some((m) => m.includes('stream server shutdown complete'))).toBe(true);
    });
});
...
describe('terminateDisabledVoiceClients', () => {
    it('logs the straggler count when clients are force-terminated', () => {
        ...
        expect(firstTerminated).toBe(true);
        expect(secondTerminated).toBe(true);
        expect(logger.warnings.some((m) => m.includes('force-terminated 2 straggling client(s) after disable-grace period'))).toBe(true);
    });
    it('does not log when there are no straggling clients', () => {
        const logger = createFakeLogger();
        const terminated = terminateDisabledVoiceClients({ clients: new Set() }, logger);
...
describe('classifyVoiceUpgrade', () => {
    ...
    it('rejects a wrong path with 404 before any other guard', () => {
        expect(classifyVoiceUpgrade({ ...base, pathname: '/nope', token: 'wrong' })).toEqual({ status: 404, statusText: 'Not Found', reason: 'bad-path' });
    });
    it('rejects a disabled server with 403', () => {
        expect(classifyVoiceUpgrade({ ...base, isEnabled: () => false })).toEqual({ status: 403, statusText: 'Forbidden', reason: 'disabled' });
    });
    it('rejects a disallowed origin with 403', () => {
        expect(classifyVoiceUpgrade({ ...base, origin: 'https://evil.example' })).toEqual({ status: 403, statusText: 'Forbidden', reason: 'bad-origin' });
    });
...
describe('startVoiceServer upgrade rejection guards (integration)', () => {
    it('rejects each guard without upgrading, logging the matching guard', async () => {
        ...
        try {
            ...
            expect(await attemptOpen({ port: server.port, path: '/nope?token=voice-token' })).toBe('rejected');
            expect(logger.warnings.some((w) => w.includes('rejected upgrade for path'))).toBe(true);
            // Disallowed origin -> rejected (403).
            expect(await attemptOpen({
                port: server.port,
                path: validPath,
            ...
            expect(await attemptOpen({ port: server.port, path: '/voice/stream?token=wrong' })).toBe('rejected');
...
//# sourceMappingURL=voice-server.test.js.map