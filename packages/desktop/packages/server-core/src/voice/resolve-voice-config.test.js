...
import { getQwenConfigDir, normalizeBaseUrl, parseEnvFileContent, resolveDesktopVoiceConfig, } from './resolve-voice-config';
...
describe('resolveDesktopVoiceConfig', () => {
    ...
    it('prefers fresh OAuth credentials over settings and env keys', async () => {
        const config = await resolveDesktopVoiceConfig({
            ...
            readQwenJson: async (file) => (file === 'oauth_creds.json'
                ? {
                    access_token: 'oauth-token',
                    resource_url: 'dashscope.aliyuncs.com/compatible-mode',
                    expiry_date: future,
    ...
    it('throws without credentials and rejects cleartext non-loopback endpoints', async () => {
        await expect(resolveDesktopVoiceConfig({
            getVoiceModel: () => 'qwen3-asr-flash',
            env: {},
    ...
    it('falls through and warns when an exact model provider baseUrl is incomplete', async () => {
        ...
        try {
            await expect(resolveDesktopVoiceConfig({
                getVoiceModel: () => 'qwen3-asr-flash',
                env: {},
                readQwenJson: async (file) => (file === 'settings.json'
                    ? {
                        env: { DASHSCOPE_API_KEY: 'settings-key' },
                        modelProviders: {
                            openai: [
    ...
    it('defers an unlisted public HTTPS exact provider to the legacy chain', async () => {
        const baseUrl = 'https://voice.example.com/openai/v1';
        const settingsWithEntry = (allowlisted) => ({
        ...
        const readSettings = (allowlisted) => (async (file) => (file === 'settings.json'
    ...
    it('deep-merges modelProviders across scopes like the CLI', async () => {
        ...
        const config = await resolveDesktopVoiceConfig({
            ...
            readSystemJson: async (file) => (file.endsWith('system-defaults.json')
                ? {
                    modelProviders: {
    ...
    it('lets a higher scope replace the same provider-group key like the CLI merge', async () => {
        ...
        await expect(resolveDesktopVoiceConfig({
            ...
            systemDefaultsPath: '/managed/system-defaults.json',
            systemSettingsPath: '/managed/settings.json',
            readSystemJson: async (file) => (file.endsWith('settings.json')
                ? {
                    modelProviders: {
    ...
    it('skips prototype-pollution keys when merging trusted settings like the CLI', async () => {
        ...
        const settings = JSON.parse('{"env":{"VOICE_KEY":"settings-key"},' +
        ...
        await expect(resolveDesktopVoiceConfig({
            getVoiceModel: () => 'qwen3-asr-flash',
            env: {},
            readQwenJson: async (file) => (file === 'settings.json' ? settings : undefined),
            readSystemJson: async () => undefined,
            readHomeEnvFile: async () => undefined,
        })).rejects.toThrow('Voice dictation needs Qwen credentials');
    });
    ...
    it('reports the complete normalized URL including query and fragment', async () => {
        ...
        const normalizedBaseUrl = 'http://10.0.0.8/v1?api-version=2#voice';
        const readQwenJson = async (file) => (file === 'settings.json'
            ? {
                security: {
                ...
                ? {
                    ...
                    modelProviders: {
                        openai: [
                            {
                                id: 'qwen3-asr-flash',
                                baseUrl: 'http://voice.region-b.internal.example/v1',
                                envKey: 'PRIVATE_ASR_KEY',
                            },
                        ],
                    },
        ...
        for (const baseUrl of [
            ...
            for (const allowlisted of [false, true]) {
                await expect(resolveDesktopVoiceConfig({
                    getVoiceModel: () => 'qwen3-asr-flash',
                    env: { OPENAI_API_KEY: 'env-key', OPENAI_BASE_URL: baseUrl },
                    readQwenJson: async (file) => (file === 'settings.json' && allowlisted
                        ? { security: { allowedInsecureVoiceBaseUrls: [baseUrl] } }
                        : undefined),
                    readSystemJson: async () => undefined,
                    readHomeEnvFile: async () => undefined,
                })).rejects.toThrow('Voice endpoint must not use a private-network baseUrl.');
            }
        }
    });
    it('reports the missing key for an exact model provider', async () => {
        ...
        await expect(resolveDesktopVoiceConfig({
            ...
            readSystemJson: async () => undefined,
            readHomeEnvFile: async () => undefined,
        })).rejects.toThrow(/Voice model 'qwen3-asr-flash' requires PRIVATE_ASR_KEY\. Remove or complete this provider entry/);
    });
    ...
    it('reports embedded credentials in an exact model provider baseUrl', async () => {
        await expect(resolveDesktopVoiceConfig({
                ...
                ? {
                    modelProviders: {
                        openai: [
                            {
                                id: 'qwen3-asr-flash',
                                baseUrl: 'http://user:pass@voice.internal.example/v1',
                                envKey: 'PRIVATE_ASR_KEY',
                            },
                        ],
                    },
            ...
            readSystemJson: async () => undefined,
            readHomeEnvFile: async () => undefined,
        })).rejects.toThrow("Voice model 'qwen3-asr-flash' baseUrl must not contain embedded credentials");
    });
    ...
    it('rejects unallowlisted private-network HTTPS endpoints at config time', async () => {
        ...
        await expect(resolveDesktopVoiceConfig({
            ...
            readSystemJson: async () => undefined,
            readHomeEnvFile: async () => undefined,
        })).rejects.toThrow(`Voice endpoint must not use a private-network baseUrl. To trust this managed endpoint, add its exact complete normalized URL (${baseUrl}) to security.allowedInsecureVoiceBaseUrls.`);
        const allowlisted = await resolveDesktopVoiceConfig({
            getVoiceModel: () => 'qwen3-asr-flash',
            env: { OPENAI_API_KEY: 'env-key', OPENAI_BASE_URL: baseUrl },
            readQwenJson: async (file) => (file === 'settings.json'
    ...
    it('honors explicit System and SystemDefaults path overrides', async () => {
        ...
        expect(readPaths).toEqual(new Set(['/managed/defaults.json', '/managed/settings.json']));
    });
    it('parses JSONC and resolves env placeholders in managed settings', async () => {
        ...
        try {
            await writeFile(systemSettingsPath, `{
    ...
    it('reads only user and managed system settings, never workspace settings', async () => {
        ...
        expect(systemFiles).toEqual(new Set([
            '/etc/qwen-code/system-defaults.json',
            '/etc/qwen-code/settings.json',
        ]));
    });
    it('resolves settings placeholders and provider keys from the home .env fallback', async () => {
        const baseUrl = 'http://voice.region-a.internal.example/v1';
        const envFileContent = 'VOICE_GW=http://voice.region-a.internal.example\nVOICE_KEY=home-env-key\n';
        const readHomeEnvFile = async (file) => file === join(getQwenConfigDir(), '.env') ? envFileContent : undefined;
        const config = await resolveDesktopVoiceConfig({
            getVoiceModel: () => 'qwen3-asr-flash',
            env: {},
            readQwenJson: async (file) => (file === 'settings.json'
    ...
    it('fills empty process env values from the home .env fallback like the CLI', async () => {
        const readHomeEnvFile = async (file) => file === join(getQwenConfigDir(), '.env')
    ...
    it('reports a non-string provider baseUrl with the remediation error', async () => {
        await expect(resolveDesktopVoiceConfig({
            ...
            readSystemJson: async () => undefined,
            readHomeEnvFile: async () => undefined,
        })).rejects.toThrow("Voice model 'qwen3-asr-flash' baseUrl must be a string. Remove or complete this provider entry to fall back to your Qwen sign-in.");
    });
    it('reports a non-string provider envKey with the remediation error', async () => {
        ...
        await expect(resolveDesktopVoiceConfig({
            ...
            readSystemJson: async () => undefined,
            readHomeEnvFile: async () => undefined,
        })).rejects.toThrow("Voice model 'qwen3-asr-flash' envKey must be a string. Remove or complete this provider entry to fall back to your Qwen sign-in.");
    });
    ...
    it('names the accepted loopback spellings for a non-canonical loopback exact provider', async () => {
        ...
        await expect(resolveDesktopVoiceConfig({
            ...
            readSystemJson: async () => undefined,
            readHomeEnvFile: async () => undefined,
        })).rejects.toThrow("Voice model 'qwen3-asr-flash' uses a loopback address outside the accepted spellings. To use a local ASR endpoint, set the baseUrl to http://localhost, http://127.0.0.1, or http://[::1]. Remove or complete this provider entry to fall back to your Qwen sign-in.");
    });
    it('names the accepted loopback spellings for non-canonical loopback environment base URLs', async () => {
        ...
        for (const baseUrl of [
            ...
            await expect(resolveDesktopVoiceConfig({
                ...
                readSystemJson: async () => undefined,
                readHomeEnvFile: async () => undefined,
            })).rejects.toThrow('Voice endpoint uses a loopback address outside the accepted spellings. To use a local ASR endpoint, set the baseUrl to http://localhost, http://127.0.0.1, or http://[::1].');
        }
    });
    ...
    it('reports the post-/v1 URL in the remedy for DashScope-compatible entries', async () => {
        ...
        await expect(resolveDesktopVoiceConfig({
            ...
            readSystemJson: async () => undefined,
            readHomeEnvFile: async () => undefined,
        })).rejects.toThrow('add its exact complete normalized URL (http://dashscope.aliyuncs.com/compatible-mode/v1) to security.allowedInsecureVoiceBaseUrls');
    });
});
describe('getQwenConfigDir', () => {
    ...
    afterEach(() => {
        if (original === undefined)
            ...
            process.env.QWEN_HOME = original;
    });
...
describe('normalizeBaseUrl', () => {
    it('does not append a second /v1 when proxy paths already contain it', () => {
        expect(normalizeBaseUrl('https://proxy.example.com/v1/dashscope')).toBe('https://proxy.example.com/v1/dashscope');
    });
    ...
    it('rejects base URLs that embed credentials instead of stripping them', () => {
        expect(() => normalizeBaseUrl('https://dashscope.aliyuncs.com@evil.com/compatible-mode')).toThrow('must not contain embedded credentials');
        expect(() => normalizeBaseUrl('https://user:pass@proxy.example.com/v1')).toThrow('must not contain embedded credentials');
    });
});
describe('parseEnvFileContent (dotenv@17 parity)', () => {
    ...
    it('supports the export prefix, single quotes, and colon separators', () => {
        expect(parseEnvFileContent("export A='plain'\nB: colon-value\n")).toEqual({ A: 'plain', B: 'colon-value' });
    });
