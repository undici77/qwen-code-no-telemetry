...
describe('transcribeQwenAsrBatch', () => {
    it('posts audio chat completions and returns trimmed text', async () => {
        ...
        await expect(transcribeQwenAsrBatch({ data: new Uint8Array([1, 2, 3]), mimeType: 'audio/wav' }, {
            baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1/',
            model: 'qwen3-asr-flash',
            apiKey: 'secret-key',
        }, { language: 'en' }, fetchFn)).resolves.toBe('hello desktop');
    ...
    it('redacts credentials from failed provider responses', async () => {
        const fetchFn = async () => new Response('Bearer leaked-token and explicit leaked-token should not escape', {
            status: 500,
            statusText: 'Internal Server Error',
        });
        const promise = transcribeQwenAsrBatch({ data: new Uint8Array([1]), mimeType: 'audio/wav' }, {
            baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
            model: 'qwen3-asr-flash',
            apiKey: 'leaked-token',
        }, {}, fetchFn);
