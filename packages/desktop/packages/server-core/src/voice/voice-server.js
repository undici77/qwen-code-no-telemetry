...
export function isAllowedVoiceOrigin(origin, allowedOrigins = []) {
...
export function terminateVoiceClients(wss) {
    let terminated = 0;
 ...
 */
export function terminateDisabledVoiceClients(wss, log) {
    const terminated = terminateVoiceClients(wss);
    if (terminated > 0) {
        log?.warn(`voice: force-terminated ${terminated} straggling client(s) after disable-grace period`);
    }
    return terminated;
}
export function closeVoiceClients(wss, code = 1000, reason = 'voice disabled') {
    let closed = 0;
    for (const client of wss.clients) {
        try {
...
export function closeVoiceServerResources(httpServer, wss, timeoutMs = CLOSE_TIMEOUT_MS, graceMs = SHUTDOWN_GRACE_MS, log) {
    return new Promise((resolve) => {
        ...
        const teardown = () => {
            ...
            const terminated = terminateVoiceClients(wss);
            if (terminated > 0) {
                log?.warn(`voice: force-terminated ${terminated} straggling client(s) after grace period`);
            }
    ...
    return {
        ...
        close: () => {
            if (!closePromise) {
                ...
                closePromise = closeVoiceServerResources(httpServer, wss, undefined, undefined, log);
            }
            return closePromise;
        },
    };
}
//# sourceMappingURL=voice-server.js.map