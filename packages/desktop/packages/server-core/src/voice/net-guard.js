...
export function isPrivateNetworkIp(hostname) {
...
export function isAlwaysBlockedVoiceAddress(address) {
...
function isBlockedResolvedIp(address, allowInsecureBaseUrl) {
    return (isAlwaysBlockedVoiceAddress(address) ||
        (!allowInsecureBaseUrl && isPrivateNetworkIp(address)));
}
export function isLoopbackVoiceAddress(address) {
    const host = normalizeIpAddress(address);
    if (isLoopbackHost(host))
        return true;
    const step = unwrapIpv6TransitionStep(host);
...
export async function assertVoiceBaseUrlNetworkAllowed(voiceConfig, lookupHost) {
    ...
    const host = normalizeHostname(hostname);
    if (isIP(host) !== 0) {
        if (isAlwaysBlockedVoiceAddress(host) ||
            (!allowInsecureBaseUrl && isPrivateNetworkIp(host))) {
            throw new Error(isLoopbackVoiceAddress(host)
    ...
    try {
        ...
        throw new Error(`Voice model '${model}': DNS lookup failed for ${hostname}. Cannot verify network safety.`);
    }
    const records = Array.isArray(result) ? result : [result];
    if (records.some((record) => isBlockedResolvedIp(record.address, allowInsecureBaseUrl))) {
