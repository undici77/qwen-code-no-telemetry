...
import { assertVoiceBaseUrlNetworkAllowed, isLoopbackHost, isPrivateNetworkIp, } from './net-guard';
...
describe('net-guard host classification', () => {
    ...
    it('flags private IPv4/IPv6 ranges but not public IPs', () => {
        ...
        expect(isPrivateNetworkIp('2001:0:4136:e378:8000:63bf:3fff:fdd2')).toBe(true);
...
describe('normalizeBaseUrl', () => {
    it('prepends https:// when no scheme and appends /v1', () => {
        expect(normalizeBaseUrl('dashscope.aliyuncs.com/compatible-mode')).toBe('https://dashscope.aliyuncs.com/compatible-mode/v1');
        expect(normalizeBaseUrl('https://x.example/openai')).toBe('https://x.example/openai/v1');
    });
    it('leaves an existing /v1 and strips trailing slashes', () => {
        expect(normalizeBaseUrl('https://dashscope.aliyuncs.com/compatible-mode/v1')).toBe('https://dashscope.aliyuncs.com/compatible-mode/v1');
        expect(normalizeBaseUrl('https://dashscope.aliyuncs.com/compatible-mode/v1/')).toBe('https://dashscope.aliyuncs.com/compatible-mode/v1');
    });
...
describe('isDashscopeCompatible', () => {
    it('matches DashScope compatible-mode endpoints (incl. intl/us)', () => {
        expect(isDashscopeCompatible('https://dashscope.aliyuncs.com/compatible-mode/v1')).toBe(true);
        expect(isDashscopeCompatible('https://dashscope-intl.aliyuncs.com/compatible-mode/v1')).toBe(true);
    });
    it('rejects non-compatible-mode and unrelated hosts', () => {
        expect(isDashscopeCompatible('https://dashscope.aliyuncs.com/apps/anthropic')).toBe(false);
        expect(isDashscopeCompatible('https://idealab.alibaba-inc.com/api/openai/v1')).toBe(false);
...
describe('openVoiceStream', () => {
    it('does not expose stream URLs or task IDs in server failure errors', async () => {
        ...
        await expect(finishPromise).rejects.toThrow('Voice stream failed (InvalidParameter): provider rejected audio');
...
describe('assertVoiceBaseUrlNetworkAllowed', () => {
    it('rejects private IP-literal hosts without DNS lookup', async () => {
        ...
        await expect(assertVoiceBaseUrlNetworkAllowed({ baseUrl: 'https://10.0.0.1:443', model: 'm' }, lookup)).rejects.toThrow(/private-network/);
        await expect(assertVoiceBaseUrlNetworkAllowed({ baseUrl: 'https://169.254.169.254', model: 'm' }, lookup)).rejects.toThrow(/private-network/);
        await expect(assertVoiceBaseUrlNetworkAllowed({ baseUrl: 'https://[::a9fe:a9fe]', model: 'm' }, lookup)).rejects.toThrow(/private-network/);
        expect(called).toBe(false);
    });
    it('rejects a hostname that resolves to a private address', async () => {
        await expect(assertVoiceBaseUrlNetworkAllowed({ baseUrl: 'https://evil.example', model: 'm' }, async () => [{ address: '10.1.2.3' }])).rejects.toThrow(/private-network/);
        for (const address of [
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed({ baseUrl: 'https://evil.example', model: 'm' }, async () => [{ address }])).rejects.toThrow(/private-network/);
        }
    });
    it('rejects loopback DNS results even when trusted, with loopback guidance', async () => {
        for (const address of [
            ...
            }, async () => [{ address }])).rejects.toThrow(/loopback/);
        }
    });
    it('allows a hostname that resolves to a public address', async () => {
        await expect(assertVoiceBaseUrlNetworkAllowed({ baseUrl: 'https://api.example', model: 'm' }, async () => [{ address: '93.184.216.34' }])).resolves.toBeUndefined();
    });
    it('allows an explicitly trusted private address', async () => {
        ...
        await expect(assertVoiceBaseUrlNetworkAllowed({
            ...
            model: 'm',
            allowInsecureBaseUrl: true,
        }, async () => [{ address: '10.0.0.9' }])).resolves.toBeUndefined();
        await expect(assertVoiceBaseUrlNetworkAllowed({
            baseUrl: 'http://voice.internal.example/v1',
            model: 'm',
            allowInsecureBaseUrl: true,
        }, async () => [{ address: '::ffff:a00:9' }])).resolves.toBeUndefined();
        await expect(assertVoiceBaseUrlNetworkAllowed({
            baseUrl: 'http://voice.internal.example/v1',
            model: 'm',
            allowInsecureBaseUrl: true,
        }, async () => [{ address: '64:ff9b::a00:9' }])).resolves.toBeUndefined();
    });
    it('keeps metadata and link-local addresses blocked when trusted', async () => {
        ...
        for (const address of [
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed({
                ...
                model: 'm',
                allowInsecureBaseUrl: true,
            })).rejects.toThrow('uses a loopback address outside the accepted spellings. To use a local ASR endpoint, set the baseUrl to http://localhost, http://127.0.0.1, or http://[::1].');
        }
        await expect(assertVoiceBaseUrlNetworkAllowed({
            baseUrl: 'http://voice.internal.example/v1',
            model: 'm',
            allowInsecureBaseUrl: true,
        }, async () => [{ address: '169.254.169.254' }])).rejects.toThrow('resolved to an address that is always blocked');
        for (const address of [
            '0.0.0.0',
            '::',
            'fe80::1',
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed({
                ...
                model: 'm',
                allowInsecureBaseUrl: true,
            }, async () => [{ address }])).rejects.toThrow('resolved to an address that is always blocked');
        }
        await expect(assertVoiceBaseUrlNetworkAllowed({
            baseUrl: 'http://voice.internal.example/v1',
            model: 'm',
    ...
    it('skips DNS for IP-literal and loopback hosts', async () => {
        ...
        for (const baseUrl of [
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed({ baseUrl, model: 'm' }, lookup)).resolves.toBeUndefined();
        }
        expect(called).toBe(false);
    });
    it('rejects when the DNS lookup fails (cannot verify network safety)', async () => {
        ...
        await expect(assertVoiceBaseUrlNetworkAllowed({ baseUrl: 'https://voice.example', model: 'm' }, lookup)).rejects.toThrow("Voice model 'm': DNS lookup failed for voice.example. Cannot verify network safety.");
    });
    it('rejects a multi-record DNS answer when any record is blocked', async () => {
        ...
        await expect(assertVoiceBaseUrlNetworkAllowed({
            ...
            model: 'm',
            allowInsecureBaseUrl: true,
        }, async () => [{ address: '10.0.0.9' }, { address: '169.254.169.254' }])).rejects.toThrow('resolved to an address that is always blocked');
        await expect(assertVoiceBaseUrlNetworkAllowed({ baseUrl: 'https://voice.example', model: 'm' }, async () => [{ address: '93.184.216.34' }, { address: '10.1.2.3' }])).rejects.toThrow(/private-network/);
        await expect(assertVoiceBaseUrlNetworkAllowed({
            baseUrl: 'http://voice.internal.example/v1',
            model: 'm',
            allowInsecureBaseUrl: true,
        }, async () => [{ address: '10.0.0.9' }, { address: '10.0.0.10' }])).resolves.toBeUndefined();
    });
});
