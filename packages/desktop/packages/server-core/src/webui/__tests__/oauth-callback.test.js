...
describe('WebUI /api/oauth/callback', () => {
    it('completes OAuth when the relay forwards the inner flow state', async () => {
        ...
        try {
            const response = await handler.fetch(new Request('http://127.0.0.1/api/oauth/callback?code=auth-code-123&state=inner-state-123'));
    ...
    it('renders an OAuth failure page when the relay forwards provider errors', async () => {
        ...
        try {
            const response = await handler.fetch(new Request('http://127.0.0.1/api/oauth/callback?error=access_denied&error_description=User%20denied&state=inner-state-456'));
            expect(response.status).toBe(200);
