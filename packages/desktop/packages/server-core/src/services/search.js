...
let handlerLog = createScopedLogger(CONSOLE_LOGGER, 'handler');
let searchLog = createScopedLogger(CONSOLE_LOGGER, 'search');
export function setSearchPlatform(platform) {
    _platform = platform;
...
function getRipgrepPath() {
    if (!_platform)
        throw new Error('setSearchPlatform() must be called before search');
 ...
 */
function extractSnippetFast(rawLine, matchText, maxLength = 150) {
    try {
 ...
 */
export async function searchSessions(query, sessionsDir, options = {}) {
    const { timeout = 5000, maxMatchesPerSession = 3, maxSessions = 50, ignoreCase = true, searchId = Date.now().toString(36), } = options;
    if (!query.trim()) {
        return [];
    }
        ...
        rg.stdout.on('data', (chunk) => {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    const jsonlIndex = pathParts.findIndex((p) => p === 'session.jsonl');
                    if (jsonlIndex < 1)
                        continue;
                    ...
                    if (rawLine.includes('"isIntermediate":true'))
                        continue;
