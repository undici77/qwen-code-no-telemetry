...
import { validatePathFormat, isValidWorkingDirectory, isValidWorkspaceRootPath, } from './path-validation';
...
describe('isValidWorkspaceRootPath', () => {
    ...
    it('accepts a non-existent path when an ancestor directory exists', () => {
        const statFn = (path) => {
            if (path === '/workspace/nested/new-root')
                throw new Error('missing');
