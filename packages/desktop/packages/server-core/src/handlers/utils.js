 ...
 */
export function getWorkspaceOrThrow(workspaceId) {
...
export function buildBackendHostRuntimeContext(platform) {
    return {
 ...
 */
export function getWorkspaceAllowedDirs(workspaceId) {
    if (!workspaceId)
        return [];
    const workspace = getWorkspaceByNameOrId(workspaceId);
    if (!workspace)
        return [];
    const dirs = [workspace.rootPath];
 ...
 */
export async function validateFilePath(filePath, additionalAllowedDirs) {
