...
import { __resetTransferStateForTests, registerTransferHandlers, setTransferableHandler, } from './transfer';
...
function encodeParts(value, splitAt) {
...
describe('chunked transfer handlers', () => {
    ...
    it('rejects checksum mismatches', async () => {
        ...
        if (worldIndex === -1)
            throw new Error('Expected payload to contain "world"');
