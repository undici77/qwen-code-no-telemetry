...
const transferableHandlers = new Map();
function getTransferTtlMs() {
...
function rescheduleTransferCleanup(transfer) {
    if (transfer.timer)
        clearTimeout(transfer.timer);
...
function assertTransferOwner(ctx, transfer) {
    if (ctx.clientId !== transfer.ownerClientId) {
        throw new Error('Transfer owned by different client');
    }
}
export function setTransferableHandler(channel, handler) {
    transferableHandlers.set(channel, handler);
}
export function __resetTransferStateForTests() {
...
export function registerTransferHandlers(server) {
    ...
    server.handle(RPC_CHANNELS.transfer.ABORT, async (ctx, opts) => {
        const transfer = activeTransfers.get(opts.transferId);
        if (!transfer) {
