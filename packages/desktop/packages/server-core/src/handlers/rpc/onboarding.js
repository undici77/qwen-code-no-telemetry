...
import { getQwenWorkspacePreflightViaAcp, listQwenProvidersViaAcp, } from '@craft-agent/shared/agent';
...
async function getQwenSetupNeeds(deps) {
    if (isSetupDeferred()) {
    ...
    try {
        ...
        deps.platform.logger?.warn(`Qwen setup preflight failed; continuing to main UI: ${message}`);
        return completeSetupNeeds();
    }
}
export function registerOnboardingHandlers(server, deps) {
    ...
    server.handle(RPC_CHANNELS.onboarding.VALIDATE_MCP, async (_ctx, mcpUrl, accessToken) => {
        try {
    ...
    server.handle(RPC_CHANNELS.onboarding.START_MCP_OAUTH, async (_ctx, mcpUrl, callbackPort) => {
