...
export function registerResourcesHandlers(server, deps) {
    // Export workspace resources to a portable bundle
    server.handle(RPC_CHANNELS.resources.EXPORT, async (_ctx, workspaceId, options) => {
        ...
        deps.platform.logger?.info(`RESOURCES_EXPORT: Exported from ${workspaceId}: ` +
    ...
    server.handle(RPC_CHANNELS.resources.IMPORT, async (_ctx, workspaceId, bundle, mode) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error(`Workspace not found: ${workspaceId}`);
        ...
        const result = await importResources(workspace.rootPath, bundle, mode, {
            ...
            clearSourceCredentials: async (wsId, sourceSlug) => {
        ...
        deps.platform.logger?.info(`RESOURCES_IMPORT: Imported into ${workspaceId} (mode=${mode}): ` +
