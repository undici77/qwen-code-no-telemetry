...
describe('workspace image path boundaries', () => {
    ...
    it.skipIf(process.platform === 'win32')('rejects image reads that escape through a symlink', async () => {
        ...
        const { readImage } = createWorkspaceImageHandlers();
        await expect(readImage({ clientId: 'c1', workspaceId: null, webContentsId: null }, 'workspace-1', 'icon.svg')).rejects.toThrow('outside workspace directory');
    });
    it('rejects image writes that escape through a symlinked parent directory', async () => {
        ...
        await expect(writeImage({ clientId: 'c1', workspaceId: null, webContentsId: null }, 'workspace-1', 'linked-outside/icon.svg', Buffer.from('<svg></svg>').toString('base64'), 'image/svg+xml')).rejects.toThrow('outside workspace directory');
    ...
    it.skipIf(process.platform === 'win32')('rejects image writes through a broken final symlink', async () => {
        ...
        symlinkSync(outsideImage, join(workspaceRoot, 'icon.svg'), 'file');
        const { writeImage } = createWorkspaceImageHandlers();
        await expect(writeImage({ clientId: 'c1', workspaceId: null, webContentsId: null }, 'workspace-1', 'icon.svg', Buffer.from('<svg></svg>').toString('base64'), 'image/svg+xml')).rejects.toThrow('outside workspace directory');
        expect(existsSync(outsideImage)).toBe(false);
    });
    it('allows overwriting an image when the workspace root is a symlink', async () => {
        ...
        const { writeImage } = createWorkspaceImageHandlers();
        await writeImage({ clientId: 'c1', workspaceId: null, webContentsId: null }, 'workspace-1', 'icon.svg', Buffer.from('<svg>new</svg>').toString('base64'), 'image/svg+xml');
        expect(readFileSync(realImage, 'utf8')).toBe('<svg>new</svg>');
    });
});
//# sourceMappingURL=workspace.image-path.test.js.map