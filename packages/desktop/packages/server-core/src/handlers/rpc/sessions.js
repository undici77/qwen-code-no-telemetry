...
export async function waitForSessionListExternalRefresh(refreshPromise, { log, timeoutMs = SESSION_LIST_EXTERNAL_REFRESH_WAIT_MS, workspaceId, }) {
    if (!refreshPromise)
        return;
    let timeout;
    ...
    try {
        const result = await Promise.race([
            guardedRefresh,
            new Promise((resolve) => {
                timeout = setTimeout(() => resolve(EXTERNAL_REFRESH_TIMED_OUT), timeoutMs);
            }),
        ]);
        if (result === EXTERNAL_REFRESH_TIMED_OUT) {
            log.warn(`Session list external refresh for workspace ${workspaceId ?? '(all)'} is still running after ${timeoutMs}ms; returning cached sessions`);
        }
    }
    finally {
        if (timeout)
...
async function scanSessionDirectory(dirPath) {
    const { readdir, stat } = await import('fs/promises');
    const entries = await readdir(dirPath, { withFileTypes: true });
    const files = [];
    for (const entry of entries) {
        // Skip internal and hidden files
        if (entry.name === 'session.jsonl' || entry.name.startsWith('.'))
            continue;
...
export function registerSessionsHandlers(server, deps) {
    ...
    server.handle(RPC_CHANNELS.sessions.GET, async (ctx) => {
        ...
        await waitForSessionListExternalRefresh(sessionManager.refreshExternalSessions?.(workspaceId ?? undefined), { log, workspaceId: workspaceId ?? undefined });
    ...
    server.handle(RPC_CHANNELS.sessions.GET_FOR_WORKSPACE, async (_ctx, workspaceId, options) => {
        try {
            await sessionManager.waitForInit();
        }
        catch (error) {
        ...
        if (options?.refreshExternal !== false) {
            await waitForSessionListExternalRefresh(sessionManager.refreshExternalSessions?.(workspaceId), { log, workspaceId });
        }
    ...
    server.handle(RPC_CHANNELS.sessions.MARK_ALL_READ, async (_ctx, workspaceId) => sessionManager.markAllSessionsRead(workspaceId));
    // Get a single session with messages (for lazy loading)
    server.handle(RPC_CHANNELS.sessions.GET_MESSAGES, async (_ctx, sessionId) => {
    ...
    server.handle(RPC_CHANNELS.sessions.CREATE, async (_ctx, workspaceId, options) => {
    ...
    // Delete a session
    server.handle(RPC_CHANNELS.sessions.DELETE, async (_ctx, sessionId) => sessionManager.deleteSession(sessionId));
    ...
    server.handle(RPC_CHANNELS.sessions.SEND_MESSAGE, async (ctx, sessionId, message, attachments, storedAttachments, options) => {
        ...
        sessionManager
            .sendMessage(sessionId, message, attachments, storedAttachments, options, undefined, callerClientId)
            .catch((err) => {
    ...
    // Cancel processing
    server.handle(RPC_CHANNELS.sessions.CANCEL, async (_ctx, sessionId, silent) => sessionManager.cancelProcessing(sessionId, silent));
    // Kill background shell
    server.handle(RPC_CHANNELS.sessions.KILL_SHELL, async (_ctx, sessionId, shellId) => sessionManager.killShell(sessionId, shellId));
    // Get background task output
    server.handle(RPC_CHANNELS.tasks.GET_OUTPUT, async (_ctx, taskId) => {
        try {
    ...
    server.handle(RPC_CHANNELS.sessions.RESPOND_TO_PERMISSION, async (_ctx, sessionId, requestId, allowed, alwaysAllow, options) => sessionManager.respondToPermission(sessionId, requestId, allowed, alwaysAllow, options));
    // Respond to a credential request (secure auth input)
    // Returns true if the response was delivered, false if agent/session is gone
    server.handle(RPC_CHANNELS.sessions.RESPOND_TO_CREDENTIAL, async (_ctx, sessionId, requestId, response) => sessionManager.respondToCredential(sessionId, requestId, response));
    // ==========================================================================
    // Consolidated Command Handlers
    // ==========================================================================
    // Session commands - consolidated handler for session operations
    server.handle(RPC_CHANNELS.sessions.COMMAND, async (_ctx, sessionId, command) => {
        switch (command.type) {
            case 'flag':
            ...
            case 'setQwenPermissionRules':
                ...
                return sessionManager.setSessionPermissionRules(sessionId, command.scope, command.ruleType, command.rules);
            case 'getQwenCoreSettings':
                return sessionManager.getSessionQwenCoreSettings(sessionId);
            case 'setQwenCoreSetting':
                return sessionManager.setSessionQwenCoreSetting(sessionId, command.scope, command.key, command.value);
            case 'setQwenMcpServer':
                return sessionManager.setSessionQwenMcpServer(sessionId, command.scope, command.name, command.server);
            case 'removeQwenMcpServer':
                return sessionManager.removeSessionQwenMcpServer(sessionId, command.scope, command.name);
            case 'setQwenHook':
                return sessionManager.setSessionQwenHook(sessionId, command.scope, command.event, command.index, command.hook);
            case 'removeQwenHook':
                return sessionManager.removeSessionQwenHook(sessionId, command.scope, command.event, command.index);
            case 'setQwenExtensionSetting':
                return sessionManager.setSessionQwenExtensionSetting(sessionId, command.extensionId, command.settingKey, command.scope, command.value);
    ...
    server.handle(RPC_CHANNELS.sessions.GET_PENDING_PLAN_EXECUTION, async (_ctx, sessionId) => sessionManager.getPendingPlanExecution(sessionId));
    // Get authoritative permission mode diagnostics for renderer reconciliation
    server.handle(RPC_CHANNELS.sessions.GET_PERMISSION_MODE_STATE, async (_ctx, sessionId) => sessionManager.getSessionPermissionModeState(sessionId));
    // ============================================================
    // Session Content Search
    // ============================================================
    // Search session content using ripgrep
    server.handle(RPC_CHANNELS.sessions.SEARCH_CONTENT, async (_ctx, workspaceId, query, searchId) => {
        ...
        const hiddenSessionIds = new Set(allSessions.filter((s) => s.hidden).map((s) => s.id));
    ...
    server.handle(RPC_CHANNELS.sessions.GET_FILES, async (_ctx, sessionId) => {
        const sessionPath = sessionManager.getSessionPath(sessionId);
        if (!sessionPath)
            return [];
        try {
    ...
    server.handle(RPC_CHANNELS.sessions.WATCH_FILES, async (ctx, sessionId) => {
    ...
    server.handle(RPC_CHANNELS.sessions.GET_NOTES, async (_ctx, sessionId) => {
        const sessionPath = sessionManager.getSessionPath(sessionId);
        if (!sessionPath)
            return '';
        try {
    ...
    server.handle(RPC_CHANNELS.sessions.SET_NOTES, async (_ctx, sessionId, content) => {
        const sessionPath = sessionManager.getSessionPath(sessionId);
        if (!sessionPath) {
    ...
    server.handle(RPC_CHANNELS.sessions.EXPORT, async (ctx, sessionId) => {
        await sessionManager.waitForInit();
        const workspaceId = ctx.workspaceId ??
            deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId);
        if (!workspaceId)
    ...
    const importHandler = async (_ctx, targetWorkspaceId, bundle, mode) => {
        await sessionManager.waitForInit();
        ...
        return sessionManager.importSession(targetWorkspaceId, bundle, mode);
    };
    ...
    server.handle(RPC_CHANNELS.sessions.EXPORT_REMOTE_TRANSFER, async (ctx, sessionId) => {
        await sessionManager.waitForInit();
        const workspaceId = ctx.workspaceId ??
            deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId);
        if (!workspaceId)
        ...
        if (!payload)
            throw new Error(`Failed to export remote transfer for session ${sessionId}`);
        return payload;
    });
    // Import a summarized remote-transfer payload into a target workspace.
    server.handle(RPC_CHANNELS.sessions.IMPORT_REMOTE_TRANSFER, async (_ctx, targetWorkspaceId, payload) => {
        await sessionManager.waitForInit();
        if (!targetWorkspaceId || typeof targetWorkspaceId !== 'string')
            throw new Error('targetWorkspaceId is required');
