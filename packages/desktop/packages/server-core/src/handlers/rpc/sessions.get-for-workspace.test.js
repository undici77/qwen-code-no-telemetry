...
import { registerSessionsHandlers, waitForSessionListExternalRefresh, } from './sessions';
function deferred() {
    let resolve;
    let reject;
...
function createTestHarness(options) {
    ...
    const sessionManager = {
        ...
        refreshExternalSessions: async (workspaceId) => {
...
describe('waitForSessionListExternalRefresh', () => {
    ...
    it('logs refresh failures without failing the session list request', async () => {
        const warnings = [];
        await waitForSessionListExternalRefresh(Promise.reject(new Error('provider failed')), {
            log: { warn: (...args) => warnings.push(args) },
            timeoutMs: 50,
            workspaceId: 'current-workspace',
        });
...
//# sourceMappingURL=sessions.get-for-workspace.test.js.map