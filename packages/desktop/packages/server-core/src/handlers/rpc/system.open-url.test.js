...
describe('registerSystemCoreHandlers OPEN_URL', () => {
    ...
    it('rejects blocked URL schemes', async () => {
        const { openUrl, ctx, invokeClientCalls } = createTestHarness();
        await expect(openUrl(ctx, 'file:///tmp/test.txt')).rejects.toThrow('Failed to open URL: Refused to open URL with blocked scheme: file:');
        expect(invokeClientCalls).toHaveLength(0);
    });
});
//# sourceMappingURL=system.open-url.test.js.map