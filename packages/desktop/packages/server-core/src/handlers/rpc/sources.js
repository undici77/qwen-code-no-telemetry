...
function isInvalidSourceSlugError(error) {
...
export function registerSourcesHandlers(server, deps) {
    const log = deps.platform.logger;
    // Get all sources for a workspace
    server.handle(RPC_CHANNELS.sources.GET, async (_ctx, workspaceId) => {
    ...
    // Create a new source
    server.handle(RPC_CHANNELS.sources.CREATE, async (_ctx, workspaceId, config) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
    ...
    // Delete a source
    server.handle(RPC_CHANNELS.sources.DELETE, async (_ctx, workspaceId, sourceSlug) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error(`Workspace not found: ${workspaceId}`);
    ...
    server.handle(RPC_CHANNELS.sources.SAVE_CREDENTIALS, async (_ctx, workspaceId, sourceSlug, credential) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error(`Workspace not found: ${workspaceId}`);
    ...
    server.handle(RPC_CHANNELS.sources.GET_PERMISSIONS, async (_ctx, workspaceId, sourceSlug) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            return null;
    ...
    server.handle(RPC_CHANNELS.workspace.GET_PERMISSIONS, async (_ctx, workspaceId) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            return null;
        const { existsSync, readFileSync } = await import('fs');
    ...
    server.handle(RPC_CHANNELS.sources.GET_MCP_TOOLS, async (_ctx, workspaceId, sourceSlug) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
        ...
        try {
            ...
            if (source.config.type !== 'mcp')
                return { success: false, error: 'Source is not an MCP server' };
            ...
            if (source.config.mcp.transport === 'stdio') {
                ...
                if (source.config.mcp.authType === 'oauth' || source.config.mcp.authType === 'bearer') {
                    ...
                    const credentialId = source.config.mcp.authType === 'oauth'
                        ? { type: 'source_oauth', workspaceId: source.workspaceId, sourceId: sourceSlug }
                        : { type: 'source_bearer', workspaceId: source.workspaceId, sourceId: sourceSlug };
