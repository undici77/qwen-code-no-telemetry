...
function getWorktreeDirName(branchName) {
    return branchName
        .replace(/[\\/]+/g, '--')
...
async function ensureWorktreesGitignored(repoRoot) {
    const qwenDir = join(repoRoot, '.qwen');
    await mkdir(qwenDir, { recursive: true });
    await writeFile(join(qwenDir, '.gitignore'), `# Auto-generated by qwen-code.\n${WORKTREES_DIR}/\n`, { encoding: 'utf8', flag: 'wx' }).catch((error) => {
...
export function registerWorkspaceCoreHandlers(server, deps) {
    ...
    server.handle(RPC_CHANNELS.workspaces.CREATE, async (_ctx, folderPath, name, remoteServer) => {
    ...
    server.handle(RPC_CHANNELS.workspaces.CREATE_PERMANENT_WORKTREE, async (_ctx, workspaceId, branchNameInput) => {
        ...
        await ensureWorktreesGitignored(repoRoot).catch((error) => {
    ...
    server.handle(RPC_CHANNELS.workspaces.CHECK_SLUG, async (_ctx, slug) => {
    ...
    server.handle(RPC_CHANNELS.workspaces.UPDATE_REMOTE, async (_ctx, workspaceId, remoteServer) => {
    ...
    server.handle(RPC_CHANNELS.window.SWITCH_WORKSPACE, async (ctx, workspaceId) => {
    ...
    server.handle(RPC_CHANNELS.workspace.READ_IMAGE, async (_ctx, workspaceId, relativePath) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error('Workspace not found');
    ...
    server.handle(RPC_CHANNELS.workspace.WRITE_IMAGE, async (_ctx, workspaceId, relativePath, base64, mimeType) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error('Workspace not found');
    ...
    server.handle(RPC_CHANNELS.theme.LOAD_PRESET, async (_ctx, themeId) => {
    ...
    server.handle(RPC_CHANNELS.theme.SET_COLOR_THEME, async (_ctx, themeId) => {
    ...
    server.handle(RPC_CHANNELS.theme.GET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId) => {
    ...
    server.handle(RPC_CHANNELS.theme.SET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId, themeId) => {
        const { getWorkspaces } = await import('@craft-agent/shared/config/storage');
    ...
    server.handle(RPC_CHANNELS.theme.BROADCAST_WORKSPACE_THEME, async (ctx, workspaceId, themeId) => {
    ...
    server.handle(RPC_CHANNELS.views.LIST, async (_ctx, workspaceId) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error('Workspace not found');
    ...
    server.handle(RPC_CHANNELS.views.SAVE, async (_ctx, workspaceId, views) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error('Workspace not found');
    ...
    server.handle(RPC_CHANNELS.logo.GET_URL, async (_ctx, serviceUrl, provider) => {
