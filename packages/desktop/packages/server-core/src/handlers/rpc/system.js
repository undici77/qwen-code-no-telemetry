...
function collectDeepLinkParams(parsed, pathId) {
    const params = {};
    if (pathId)
        params.id = pathId;
...
function parseInternalCraftAgentsDeepLink(parsed) {
...
function assertLocalWorkspace(ctx, action) {
            ...
            if (parsed.protocol === 'craftagents:') {
                ...
                if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
                    const target = deepLink.workspaceId && deepLink.workspaceId !== ctx.workspaceId
                        ? { to: 'workspace', workspaceId: deepLink.workspaceId }
