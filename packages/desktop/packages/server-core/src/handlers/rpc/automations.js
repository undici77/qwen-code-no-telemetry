...
const configMutexes = new Map();
function withConfigMutex(workspaceRoot, fn) {
...
async function withAutomationMatcher(workspaceId, eventName, matcherIndex, mutate) {
...
export function registerAutomationsHandlers(server, deps) {
    const log = deps.platform.logger;
    // Get automations config for a workspace (read-only, resolves path server-side)
    server.handle(RPC_CHANNELS.automations.GET, async (_ctx, workspaceId) => {
    ...
    server.handle(RPC_CHANNELS.automations.TEST, async (_ctx, payload) => {
        ...
        for (const action of payload.actions) {
            ...
            if (action.type === 'webhook') {
                ...
                const result = await executeWebhookRequest(action);
            ...
            try {
                const { sessionId } = await deps.sessionManager.executePromptAutomation(payload.workspaceId, workspace.rootPath, action.prompt, payload.labels, payload.permissionMode, references.mentions, action.llmConnection, action.model, payload.automationName);
                results.push({
                    type: 'prompt',
                    success: true,
                    sessionId,
                ...
                if (payload.automationId) {
                    const entry = createPromptHistoryEntry({ matcherId: payload.automationId, ok: false, error: err.message, prompt: action.prompt });
                    try {
                        await appendAutomationHistoryEntry(workspace.rootPath, entry);
                    }
                    catch (e) {
    ...
    server.handle(RPC_CHANNELS.automations.SET_ENABLED, async (_ctx, workspaceId, eventName, matcherIndex, enabled) => {
    ...
    server.handle(RPC_CHANNELS.automations.DUPLICATE, async (_ctx, workspaceId, eventName, matcherIndex) => {
    ...
    server.handle(RPC_CHANNELS.automations.DELETE, async (_ctx, workspaceId, eventName, matcherIndex) => {
    ...
    server.handle(RPC_CHANNELS.automations.GET_HISTORY, async (_ctx, workspaceId, automationId, limit = AUTOMATION_HISTORY_MAX_RUNS_PER_MATCHER) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error('Workspace not found');
    ...
    server.handle(RPC_CHANNELS.automations.REPLAY, async (_ctx, workspaceId, automationId, eventName) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error('Workspace not found');
        const { resolveAutomationsConfigPath } = await import('@craft-agent/shared/automations/resolve-config-path');
        ...
        if (webhookActions.length === 0)
            throw new Error('No webhook actions to replay');
        const { executeWebhookRequest, createWebhookHistoryEntry } = await import('@craft-agent/shared/automations/webhook-utils');
        const results = await Promise.all(webhookActions.map(a => executeWebhookRequest(a)));
    ...
    server.handle(RPC_CHANNELS.automations.GET_LAST_EXECUTED, async (_ctx, workspaceId) => {
        const workspace = getWorkspaceByNameOrId(workspaceId);
        if (!workspace)
            throw new Error('Workspace not found');
        const historyPath = join(workspace.rootPath, HISTORY_FILE);
