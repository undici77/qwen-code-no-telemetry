...
export function registerFilesHandlers(server, deps) {
    // Read a file (with path validation to prevent traversal attacks)
    server.handle(RPC_CHANNELS.file.READ, async (ctx, path) => {
        try {
    ...
    server.handle(RPC_CHANNELS.file.READ_DATA_URL, async (ctx, path) => {
        try {
            const workspaceId = ctx.workspaceId ?? deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId);
            const safePath = await validateFilePath(path, getWorkspaceAllowedDirs(workspaceId));
    ...
    server.handle(RPC_CHANNELS.file.READ_PREVIEW_DATA_URL, async (ctx, path, maxSize = 64) => {
        try {
            const workspaceId = ctx.workspaceId ?? deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId);
            const safePath = await validateFilePath(path, getWorkspaceAllowedDirs(workspaceId));
    ...
    server.handle(RPC_CHANNELS.file.READ_BINARY, async (ctx, path) => {
        try {
            const workspaceId = ctx.workspaceId ?? deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId);
            const safePath = await validateFilePath(path, getWorkspaceAllowedDirs(workspaceId));
            const buffer = await readFile(safePath);
    ...
    server.handle(RPC_CHANNELS.file.READ_ATTACHMENT, async (ctx, path) => {
        try {
            const workspaceId = ctx.workspaceId ?? deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId);
            const safePath = await validateFilePath(path, getWorkspaceAllowedDirs(workspaceId));
    ...
    server.handle(RPC_CHANNELS.file.READ_USER_ATTACHMENT, async (_ctx, path) => {
        try {
    ...
    server.handle(RPC_CHANNELS.file.GENERATE_THUMBNAIL, async (_ctx, base64, _mimeType) => {
        try {
    ...
    server.handle(RPC_CHANNELS.file.STORE_ATTACHMENT, async (ctx, sessionId, attachment) => {
    ...
    server.handle(RPC_CHANNELS.fs.SEARCH, async (_ctx, basePath, query) => {
        ...
        try {
            ...
            while (queue.length > 0 && results.length < MAX_RESULTS) {
                ...
                const nextQueue = [];
                const dirResults = await Promise.all(queue.map(async (relDir) => {
    ...
    server.handle(RPC_CHANNELS.fs.LIST_DIRECTORY, async (_ctx, dirPath) => {
