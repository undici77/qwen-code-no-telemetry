...
export function parseServerPort(name, value, defaultPort) {
 ...
 */
function isLockFromPreviousBoot(startedAt) {
    if (startedAt <= 0)
        return false; // legacy lock without timestamp — can't tell
    const bootTime = Date.now() - osUptime() * 1000;
    return startedAt < bootTime;
}
function acquireServerLock(logger) {
    ...
    if (existsSync(LOCK_FILE)) {
        try {
            ...
            if (lock) {
                ...
                if (lock.pid === process.pid) {
                    ...
                    if (isLockFromPreviousBoot(lock.startedAt)) {
                        ...
                        throw new Error(`Another server instance is already running (PID ${lock.pid}). ` +
            ...
            if (err instanceof Error && err.message.includes('Another server instance'))
                throw err;
            logger.warn('[bootstrap] Could not read lock file, overwriting');
        }
    }
    const payload = { pid: process.pid, startedAt: Date.now() };
...
// Config artifacts
// ---------------------------------------------------------------------------
function bootstrapConfigArtifacts(platform) {
    ensureConfigDir();
    platform.logger.info('[bootstrap] Config artifacts initialized');
}
function ensureGlobalConfigExists(platform) {
...
export async function bootstrapServer(options) {
    ...
    platform.logger.info(`Qwen Code server listening on ${wsServer.protocol}://${rpcHost}:${wsServer.port}`);
    let stopped = false;
    const stop = async () => {
        if (stopped)
            return;
 ...
 */
export async function startHealthHttpServer(options) {
    if (options.port <= 0)
        return null;
    // Dynamic import — getHealthCheck uses HandlerDeps shape
    const { getHealthCheck } = await import('../handlers/rpc/server');
    const depsLike = { sessionManager: options.deps.sessionManager };
...
//# sourceMappingURL=headless-start.js.map