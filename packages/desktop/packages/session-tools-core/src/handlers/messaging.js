...
export async function handleListMessagingChannels(ctx, args) {
    ...
    try {
        ...
        const lines = bindings.map((b) => `- ${b.platform}: ${b.channelName || b.channelId} (${b.enabled ? 'active' : 'disabled'})`);
        return successResponse(`Messaging bindings for session ${sessionId}:\n${lines.join('\n')}`);
    }
    catch (error) {
...
export async function handleUnbindMessagingChannel(ctx, args) {
