...
import { validateJsonFileHasFields, validateSourceConfigBasic, } from '../validation.ts';
import { sourceExists, getSourceConfigPath, getSourceGuidePath, getSourcePath, } from '../source-helpers.ts';
/**
 ...
 */
export async function handleSourceTest(ctx, args) {
...
// Icon Handling
// ============================================================
async function handleIconCheck(ctx, sourcePath, sourceSlug, source) {
    const lines = [];
    ...
    const hasLocalIcon = ctx.fs.exists(iconPngPath) ||
...
// Completeness Check
// ============================================================
function checkCompleteness(ctx, sourceSlug, source) {
    const lines = [];
    let hasWarning = false;
    // Check guide.md
...
// Connection Test
// ============================================================
async function testConnection(ctx, source, sourceSlug) {
    const lines = [];
    let success = false;
    let hasError = false;
    let error;
...
async function testApiConnection(ctx, source, sourceSlug) {
    const lines = [];
    let success = false;
    let hasError = false;
    let error;
 ...
 */
async function testApiConnectionWithAuth(ctx, source, sourceSlug, testUrl) {
    const lines = [];
    ...
    switch (source.api.authType) {
        ...
        case 'header':
            ...
            if (source.api.headerName) {
                ...
                try {
                    const headerValues = JSON.parse(token);
 ...
 */
async function testApiConnectionBasic(source, testUrl) {
    const lines = [];
    let success = false;
    let hasError = false;
    let error;
...
function testLocalConnection(ctx, source) {
    const lines = [];
    let success = false;
    let hasError = false;
    let error;
...
// Auth Status Check
// ============================================================
async function checkAuthStatus(ctx, source, sourceSlug) {
    const lines = [];
    let hasWarning = false;
    if (source.isAuthenticated) {
