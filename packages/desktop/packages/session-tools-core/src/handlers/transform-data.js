...
function formatIsolationContext(filesystemIsolation, networkIsolation) {
    const commandLine = [
...
function killSandboxProcess(child) {
    if (!child.pid) {
        child.kill('SIGKILL');
        return;
    }
 ...
 */
export async function handleTransformData(ctx, args) {
    ...
    if (!isPathWithinDirectoryForCreation(resolvedOutput, dataDir)) {
        return errorResponse(`outputFile must be within the session data directory. Got: ${args.outputFile}`);
    }
    ...
    for (const inputFile of args.inputFiles) {
        ...
        if (!isAllowed) {
            return errorResponse(`inputFile must be within the session or skills directory. Got: ${inputFile}`);
        }
    ...
    try {
        ...
        filesystemIsolation = applyFilesystemIsolation(runtime.command, runtimeArgs, dataDir, {
            includeNetworkDeny: true,
            isolateIpc: true,
        });
        networkIsolation = {
            status: filesystemIsolation.status,
            backend: filesystemIsolation.status === 'enforced'
        ...
        if (filesystemIsolation.status !== 'enforced') {
            return errorResponse('transform_data requires filesystem isolation in all permission modes, but no supported isolation backend is available on this platform/runtime.');
        }
        ...
        const result = await new Promise((resolvePromise, reject) => {
            const child = spawn(enforcedFilesystemIsolation.command, enforcedFilesystemIsolation.args, {
                cwd: dataDir,
                env,
        ...
        if (result.code !== 0) {
            ...
            return errorResponse(`Script failed (exit code ${result.code}):\n${errorOutput.slice(0, 2000)}${formatIsolationContext(enforcedFilesystemIsolation, enforcedNetworkIsolation)}`);
        }
        // Verify output file was created
        if (!existsSync(resolvedOutput)) {
            return errorResponse(`Script completed but output file was not created: ${args.outputFile}\n\nStdout: ${result.stdout.slice(0, 500)}`);
        }
        ...
        lines.push(`Network isolation: ${enforcedNetworkIsolation.status} (${enforcedNetworkIsolation.backend})`);
        lines.push(`Filesystem isolation: ${enforcedFilesystemIsolation.status} (${enforcedFilesystemIsolation.backend})`);
        lines.push('');
        ...
        const isolationContext = filesystemIsolation && networkIsolation
