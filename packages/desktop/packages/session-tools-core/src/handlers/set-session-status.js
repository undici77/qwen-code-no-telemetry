...
export async function handleSetSessionStatus(ctx, args) {
    ...
    try {
        ...
        if (ctx.resolveStatus) {
            ...
            if (!resolved) {
                return errorResponse(`Unknown status: "${status}". Available status IDs: ${available.join(', ')}`);
            }
            status = resolved;
        }
...
//# sourceMappingURL=set-session-status.js.map