...
import { formatValidationResult, validateJsonFileHasFields, mergeResults, } from '../validation.ts';
 ...
 */
export async function handleConfigValidate(ctx, args) {
    ...
    if (ctx.validators) {
        try {
            ...
            return errorResponse(`Config validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    // Fallback: basic validation
    switch (target) {
        case 'config': {
            const result = validateJsonFileHasFields(join(craftAgentRoot, 'config.json'), ['workspaces']);
            return successResponse(formatValidationResult(result));
        }
        case 'sources': {
            if (sourceSlug) {
                ...
                for (const entry of entries) {
                    ...
                    if (ctx.fs.isDirectory(entryPath)) {
                        ...
                        const sourceResult = validateJsonFileHasFields(join(entryPath, 'config.json'), ['slug', 'name', 'type']);
        ...
        case 'statuses': {
            const result = validateJsonFileHasFields(join(ctx.workspacePath, 'statuses', 'config.json'), ['statuses']);
            return successResponse(formatValidationResult(result));
        }
        case 'preferences': {
            const result = validateJsonFileHasFields(join(craftAgentRoot, 'preferences.json'), []);
            return successResponse(formatValidationResult(result));
        }
        case 'permissions': {
        ...
        case 'tool-icons': {
            const result = validateJsonFileHasFields(join(craftAgentRoot, 'tool-icons', 'tool-icons.json'), ['version', 'tools']);
            return successResponse(formatValidationResult(result));
        }
        case 'all': {
            const configResult = validateJsonFileHasFields(join(craftAgentRoot, 'config.json'), ['workspaces']);
            const prefsResult = validateJsonFileHasFields(join(craftAgentRoot, 'preferences.json'), []);
            const merged = mergeResults(configResult, prefsResult);
            return successResponse(formatValidationResult(merged));
        }
        default:
            return errorResponse(`Unknown validation target: ${target}. Valid targets: config, sources, statuses, preferences, permissions, automations, tool-icons, all`);
    }
}
