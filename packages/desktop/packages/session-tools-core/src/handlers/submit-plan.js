 ...
 */
export async function handleSubmitPlan(ctx, args) {
    const { planPath } = args;
    if (!isPathWithinDirectory(planPath, ctx.plansFolderPath)) {
        return errorResponse(`Plan file must be inside the session plans directory: ${ctx.plansFolderPath}`);
    }
    // Verify the file exists
    if (!ctx.fs.exists(planPath)) {
        return errorResponse(`Plan file not found at ${planPath}. Please write the plan file first using the Write tool.`);
    }
    ...
    try {
        ...
        return errorResponse(`Failed to read plan file: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
...
//# sourceMappingURL=submit-plan.js.map