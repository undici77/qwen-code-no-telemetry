 ...
 */
export async function handleSourceOAuthTrigger(ctx, args) {
    ...
    if (!isMcpOAuth && !isApiOAuth) {
        return errorResponse(`Source '${sourceSlug}' is not configured for OAuth authentication.`);
    }
    ...
    if (source.isAuthenticated && ctx.credentialManager) {
        ...
        if (refreshedToken) {
            return successResponse(`Token for source '${sourceSlug}' has been refreshed successfully. The source is ready — proceed with using its tools directly.`);
        }
    }
    // Build auth request
    const authRequest = {
    ...
    return successResponse(`OAuth authentication requested for '${source.name}'. Opening browser for authentication.`);
}
/**
 * Handle the source_google_oauth_trigger tool call.
 * Triggers Google OAuth for Gmail, Calendar, Drive, etc.
 */
export async function handleGoogleOAuthTrigger(ctx, args) {
    const { sourceSlug } = args;
    // Load source config
    const source = ctx.loadSourceConfig(sourceSlug);
    if (!source) {
    ...
    if (source.provider !== 'google') {
        ...
        return errorResponse(`Source '${sourceSlug}' is not configured as a Google API source. ${hint}`);
    }
    ...
    if (ctx.isGoogleOAuthConfigured) {
        ...
        if (!ctx.isGoogleOAuthConfigured(api?.googleOAuthClientId, api?.googleOAuthClientSecret)) {
            return errorResponse(`Google OAuth credentials not configured for source '${sourceSlug}'.
...
\`\`\``);
    ...
    ctx.callbacks.onAuthRequest(authRequest);
    return successResponse(`Google OAuth requested for '${source.name}'. Opening browser for authentication.`);
}
/**
 * Handle the source_slack_oauth_trigger tool call.
 * Triggers Slack OAuth for workspace access.
 */
export async function handleSlackOAuthTrigger(ctx, args) {
    const { sourceSlug } = args;
    // Load source config
    const source = ctx.loadSourceConfig(sourceSlug);
    if (!source) {
    ...
    if (source.provider !== 'slack') {
        ...
        return errorResponse(`Source '${sourceSlug}' is not configured as a Slack API source. ${hint}`);
    }
    ...
    if (source.type !== 'api') {
        ...
        return errorResponse(`source_slack_oauth_trigger only works with API sources (type: "api"), not ${source.type} sources. ${hint}`);
    }
    // Check if already authenticated (with valid token)
    if (source.isAuthenticated && ctx.credentialManager) {
        const workspaceId = basename(ctx.workspacePath) || '';
    ...
    // Trigger auth request
    ctx.callbacks.onAuthRequest(authRequest);
    return successResponse(`Slack OAuth requested for '${source.name}'. Opening browser for authentication.`);
}
/**
 * Handle the source_microsoft_oauth_trigger tool call.
 * Triggers Microsoft OAuth for Outlook, OneDrive, Teams, etc.
 */
export async function handleMicrosoftOAuthTrigger(ctx, args) {
    const { sourceSlug } = args;
    // Load source config
    const source = ctx.loadSourceConfig(sourceSlug);
    if (!source) {
    ...
    if (source.provider !== 'microsoft') {
        const hint = !source.provider
            ...
            : `This source has provider '${source.provider}'.`;
        return errorResponse(`Source '${sourceSlug}' is not configured as a Microsoft API source. ${hint}`);
    }
    // Check if already authenticated (with valid token)
    if (source.isAuthenticated && ctx.credentialManager) {
        const workspaceId = basename(ctx.workspacePath) || '';
    ...
    if (!service) {
        return errorResponse(`Cannot determine Microsoft service for source '${sourceSlug}'. Set microsoftService ('outlook', 'microsoft-calendar', 'onedrive', 'teams', or 'sharepoint') in api config.`);
    }
    // Build auth request
    const authRequest = {
...
//# sourceMappingURL=source-oauth.js.map