...
export async function handleScriptSandbox(ctx, args) {
    ...
    const tempScript = join(sandboxScriptDir, `craft-sandbox-${randomUUID()}${ext}`);
    ...
    try {
        ...
        if (process.platform === 'darwin') {
            ...
            if (networkIsolation.status !== 'enforced') {
                return errorResponse('script_sandbox requires network isolation in all permission modes, but no supported isolation backend is available on this platform/runtime.');
            }
            filesystemIsolation = applyFilesystemIsolation(networkIsolation.command, networkIsolation.args, sessionDir);
        }
        if (networkIsolation.status !== 'enforced') {
            return errorResponse('script_sandbox requires network isolation in all permission modes, but no supported isolation backend is available on this platform/runtime.');
        }
        if (filesystemIsolation.status !== 'enforced') {
            return errorResponse('script_sandbox requires filesystem isolation in all permission modes, but no supported isolation backend is available on this platform/runtime.');
        }
...
//# sourceMappingURL=script-sandbox.js.map