 ...
 */
export async function handleRenderTemplate(ctx, args) {
    if (!ctx.dataPath) {
    ...
    if (!existsSync(sourcePath)) {
        return errorResponse(`Source "${args.source}" not found at ${sourcePath}`);
    }
    // Load template
    const template = loadTemplate(sourcePath, args.template);
    if (!template) {
        return errorResponse(`Template "${args.template}" not found for source "${args.source}".\n\nExpected file: ${join(sourcePath, 'templates', `${args.template}.html`)}`);
    }
    // Soft validation
