...
describe('buildDarwinSandboxProfile', () => {
    ...
    it('falls back to resolved path when realpathSync fails', () => {
        ...
        expect(profile).toContain(`(allow file-write* (subpath "${resolvedPath}"))`);
    });
    it('includes logical and real write roots when the session path crosses a symlink', () => {
        ...
        try {
            ...
            expect(profile).toContain(`(allow file-write* (subpath "${logicalRoot}"))`);
            expect(profile).toContain(`(allow file-write* (subpath "${realSession}"))`);
        }
        finally {
...
//# sourceMappingURL=filesystem-isolation.test.js.map