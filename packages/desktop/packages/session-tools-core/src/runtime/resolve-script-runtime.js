...
function inferPackagedMode(ctx) {
    if (typeof ctx?.isPackaged === 'boolean')
...
function resolveResourcesBase(ctx) {
...
function resolveBundledNode(ctx) {
...
function resolveBundledBun(ctx) {
...
function validatePackagedEnvRuntime(command, label) {
    const hasPathSeparator = command.includes('/') || command.includes('\\');
    if (!isAbsolute(command) && !hasPathSeparator) {
        throw new Error(`${label} runtime from env is not an absolute/bundled path (${command}). ` +
    ...
    if (!existsSync(resolvedCommand)) {
        throw new Error(`${label} runtime from env does not exist: ${resolvedCommand}. ` +
 ...
 */
export function resolveScriptRuntime(language, ctx) {
...
//# sourceMappingURL=resolve-script-runtime.js.map