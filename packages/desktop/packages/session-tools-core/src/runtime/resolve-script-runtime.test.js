...
describe('resolveScriptRuntime', () => {
    ...
    it('blocks PATH fallback in packaged mode', () => {
        ...
        try {
            expect(() => resolveScriptRuntime('python3', { isPackaged: true })).toThrow('packaged app');
        }
        finally {
            if (prevUv === undefined)
                delete process.env.CRAFT_UV;
            ...
            if (prevBase === undefined)
                ...
                process.env.CRAFT_RESOURCES_BASE = prevBase;
            if (prevRoot === undefined)
                delete process.env.CRAFT_APP_ROOT;
            else
                process.env.CRAFT_APP_ROOT = prevRoot;
        }
    });
    it('rejects bare CRAFT_NODE command in packaged mode', () => {
        ...
        try {
            expect(() => resolveScriptRuntime('node', { isPackaged: true })).toThrow('do not allow PATH-based runtime resolution');
        }
        finally {
            if (prev === undefined)
                delete process.env.CRAFT_NODE;
            else
                process.env.CRAFT_NODE = prev;
        }
    });
    it('prefers CRAFT_BUN for bun in dev', () => {
        ...
        try {
            ...
            if (prev === undefined)
                ...
                process.env.CRAFT_BUN = prev;
        }
    });
});
//# sourceMappingURL=resolve-script-runtime.test.js.map