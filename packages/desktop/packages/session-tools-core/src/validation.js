 ...
 */
export function invalidResult(path, message, suggestion) {
    return {
        valid: false,
 ...
 */
export function validateJsonFileHasFields(filePath, requiredFields) {
 ...
 */
export function zodErrorToIssues(error, filePath) {
...
export function validateSlug(slug) {
    if (!SLUG_REGEX.test(slug)) {
        const suggestedSlug = slug
            ...
            .replace(/^-+|-+$/g, '')
            .replace(/-+/g, '-');
        return invalidResult('slug', 'Slug must be lowercase alphanumeric with hyphens', `Suggested: '${suggestedSlug || 'valid-slug-name'}'`);
    }
    return validResult();
}
// ============================================================
...
export function validateSkillContent(markdownContent, slug) {
    ...
    try {
        ...
        return invalidResult('frontmatter', `Invalid YAML frontmatter: ${e instanceof Error ? e.message : 'Unknown error'}`, 'Check YAML syntax in frontmatter section');
    }
...
export function validateSourceConfigBasic(config) {
    ...
    if ('type' in data && !SOURCE_TYPES.includes(data.type)) {
        errors.push({
            path: 'type',
