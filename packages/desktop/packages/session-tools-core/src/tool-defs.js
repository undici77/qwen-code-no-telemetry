...
import { handleSourceOAuthTrigger, handleGoogleOAuthTrigger, handleSlackOAuthTrigger, handleMicrosoftOAuthTrigger, } from './handlers/source-oauth.ts';
...
export const SourceTestSchema = z.object({
    ...
    autoEnable: z
        .boolean()
        .optional()
        .describe('Automatically enable and activate the source in the current session on successful validation. Defaults to true. Pass false to keep pure validation behavior.'),
});
...
export const UpdatePreferencesSchema = z.object({
    ...
    includeCoAuthoredBy: z.boolean().optional().describe("Whether to include 'Co-Authored-By: Qwen Code' trailer on git commits. Defaults to true."),
});
...
- Do NOT include any text or tool calls after SubmitPlan - they will not be executed`,
    config_validate: `Validate Qwen Code configuration files.
...
Only use 'attachments' for existing file paths on disk — the tool reads them automatically.`,
    send_developer_feedback: `Send freeform feedback to the Qwen Code development team.
 ...
 */
export function getSessionToolDefs(options) {
 ...
 */
export function getSessionBackendToolNames(options) {
 ...
 */
export function getSessionRegistryToolNames(options) {
 ...
 */
export function getSessionSafeAllowedToolNames(options) {
    const prefix = options?.prefix ?? '';
    return new Set(getSessionToolDefs(options)
 ...
 */
export function getSessionSafeBlockedToolNames(options) {
    const prefix = options?.prefix ?? '';
    return new Set(getSessionToolDefs(options)
...
export const SESSION_BACKEND_TOOL_NAMES = new Set(SESSION_TOOL_DEFS.filter(d => d.executionMode === 'backend').map(d => d.name));
/** Session tool names that are always executable from the canonical registry. */
export const SESSION_REGISTRY_TOOL_NAMES = new Set(SESSION_TOOL_DEFS.filter(d => d.executionMode === 'registry').map(d => d.name));
/** Session tool names allowed in Explore/Safe mode (unfiltered canonical set). */
export const SESSION_SAFE_ALLOWED_TOOL_NAMES = new Set(SESSION_TOOL_DEFS.filter(d => d.safeMode === 'allow').map(d => d.name));
/** Session tool names blocked in Explore/Safe mode (unfiltered canonical set). */
export const SESSION_SAFE_BLOCKED_TOOL_NAMES = new Set(SESSION_TOOL_DEFS.filter(d => d.safeMode === 'block').map(d => d.name));
...
export function getToolDefsAsJsonSchema(opts) {
    ...
    return defs.map(def => {
        ...
        const jsonSchema = zodToJsonSchema(def.inputSchema, { $refStrategy: 'none' });
