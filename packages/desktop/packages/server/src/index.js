...
function parseOptionalBooleanEnv(name, value) {
...
function parseOptionalWebSocketUrl(name, value) {
    if (value == null || value.trim() === '')
        return undefined;
    try {
...
function parseServerPortOrExit(name, value, defaultPort) {
    try {
...
// ---------------------------------------------------------------------------
let webuiHandler = null;
let webuiNodeHandler;
...
if (webuiHandler) {
    ...
    const depsLike = { sessionManager: instance.sessionManager };
