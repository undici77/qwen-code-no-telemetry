...
export class Commands {
    ...
    pairingConsumer;
    log;
    constructor(sessionManager, bindingStore, workspaceId, pairingConsumer, logger = NOOP_LOGGER) {
        this.sessionManager = sessionManager;
        this.bindingStore = bindingStore;
        this.workspaceId = workspaceId;
        this.pairingConsumer = pairingConsumer;
        this.log = logger;
    }
    async handle(adapter, msg) {
        ...
        if (text.startsWith('/new')) {
            ...
            await adapter.sendText(msg.channelId, 'No session bound to this chat.\n\n' +
    ...
    async handleCommand(adapter, msg) {
        const text = msg.text.trim();
        if (!text.startsWith('/'))
            return false;
    ...
    async handleNew(adapter, msg) {
        ...
        try {
            ...
            await adapter.sendText(msg.channelId, `Created "${displayName}" — you're connected. Just type to start.`);
    ...
    async handleBind(adapter, msg) {
        ...
        await adapter.sendText(msg.channelId, 'Recent sessions:\n' + lines.join('\n') + '\n\nUse /bind <number> to connect, or /bind <session-id> if you already know it.');
    }
    async handlePair(adapter, msg) {
        ...
        await adapter.sendText(msg.channelId, `✅ Paired with "${session.name || session.id}". You can start chatting now.`);
    }
    async handleUnbind(adapter, msg) {
    ...
    async handleStatus(adapter, msg) {
        ...
        await adapter.sendText(msg.channelId, `Bound to "${name}"\nApproval: ${mode}\nResponse mode: ${responseMode}`);
    }
    async handleStop(adapter, msg) {
        const binding = this.bindingStore.findByChannel(adapter.platform, msg.channelId);
        if (!binding) {
    ...
    async resolveBindTarget(bindArg, recent) {
        if (/^\d+$/.test(bindArg)) {
