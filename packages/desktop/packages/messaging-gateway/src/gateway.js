...
export class MessagingGateway {
    ...
    renderer;
    planTokens;
    planMessages = new Map();
    pendingCompactAccepts = new Map();
    adapters = new Map();
    log;
    started = false;
    constructor(opts) {
        ...
        this.bindingStore = new BindingStore(opts.storageDir, opts.legacyStorageDir, this.log.child({ component: 'binding-store' }));
        if (opts.onBindingChanged) {
            this.bindingStore.onChange(opts.onBindingChanged);
        }
        this.commands = new Commands(opts.sessionManager, this.bindingStore, opts.workspaceId, opts.pairingConsumer, this.log.child({ component: 'commands' }));
        this.router = new Router(opts.sessionManager, this.bindingStore, this.commands, this.log.child({ component: 'router' }));
    ...
    async unregisterAdapter(platform) {
    ...
    wireAdapter(adapter) {
        ...
        adapter.onButtonPress(async (press) => {
    ...
    // -------------------------------------------------------------------------
    onSessionEvent(channel, _target, ...args) {
        ...
        if (event.type === 'info' &&
            event.statusType === 'compaction_complete') {
    ...
    // Button handling
    // -------------------------------------------------------------------------
    async handleButtonPress(platform, press) {
        const adapter = this.adapters.get(platform);
        if (!adapter)
            return;
        if (press.buttonId.startsWith('bind:')) {
            ...
            this.bindingStore.bind(this.workspaceId, session.id, platform, press.channelId, undefined);
            await adapter.sendText(press.channelId, `Bound to "${session.name || session.id}"`);
            return;
        }
        if (press.buttonId.startsWith('perm:')) {
            if (platform === 'whatsapp') {
                ...
                await adapter.sendText(press.channelId, '⏸ Permission required. Approve it in the desktop app to continue.');
                return;
            }
            ...
            this.sessionManager.respondToPermission(binding.sessionId, requestId, allowed, false);
    ...
    async handlePlanButton(platform, adapter, press) {
        const parts = press.buttonId.split(':');
        const action = parts[1];
        const token = parts[2];
        if (!token || (action !== 'accept' && action !== 'compact'))
            return;
        const entry = this.planTokens.resolve(token);
        if (!entry) {
            await adapter.sendText(press.channelId, '⚠️ This plan has expired. Retry from the desktop app.');
            return;
        }
        ...
        if (action === 'accept') {
            try {
                ...
                await adapter.sendText(press.channelId, '❌ Couldn\'t accept the plan. Check the desktop app.');
            }
            return;
        }
        ...
        try {
            ...
            await adapter.sendText(press.channelId, '♻️ Compacting conversation, then executing the plan…');
        }
        catch (err) {
            ...
            await adapter.sendText(press.channelId, '❌ Couldn\'t start compaction. Check the desktop app.');
        }
    }
    async finishPendingCompactAccept(sessionId) {
        ...
        try {
            ...
            if (adapter?.isConnected()) {
                await adapter.sendText(entry.channelId, '❌ Compaction finished but the plan couldn\'t execute. Check the desktop app.');
            }
        }
    }
    // -------------------------------------------------------------------------
