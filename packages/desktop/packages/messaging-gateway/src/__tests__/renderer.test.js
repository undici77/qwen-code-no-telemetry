...
function makeAdapter(capabilities = {}) {
    ...
    const adapter = {
        ...
        async sendButtons(channelId, text) {
            const messageId = String(nextId++);
...
async function play(renderer, binding, adapter, events) {
    for (const ev of events) {
        await renderer.handle(ev, binding, adapter);
    }
}
const ev = {
    delta: (s) => ({ type: 'text_delta', sessionId: 's', delta: s }),
    intermediate: (text) => ({
        type: 'text_complete',
        sessionId: 's',
        text,
    ...
    complete: () => ({ type: 'complete', sessionId: 's' }),
};
// ---------------------------------------------------------------------------
...
describe('Renderer — legacy config coercion', () => {
    it('streamResponses=true with no responseMode → streaming behaviour', async () => {
        ...
        const adapter = makeAdapter();
        const binding = makeBinding();
        binding.config.responseMode = undefined;
...
describe('Renderer — WhatsApp desktop-only approvals', () => {
    ...
    it('plan_submitted on WhatsApp sends an informational desktop-only message', async () => {
        ...
        await renderer.handle({
            type: 'plan_submitted',
            sessionId: 's',
            message: { id: 'm1', role: 'assistant', content: 'Plan ready' },
        }, binding, adapter);
        const sends = adapter.calls.filter((c) => c.kind === 'sendText');
        expect(sends).toHaveLength(1);
