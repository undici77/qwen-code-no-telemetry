...
function withTimeout(p, ms, label) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error(`[telegram] ${label} timed out after ${ms}ms`)), ms);
...
function describeError(err, depth = 0) {
    ...
    if (err instanceof Error) {
        ...
        if (code !== undefined)
            out.code = code;
        const grammyInner = err.error;
        if (grammyInner !== undefined)
            out.error = describeError(grammyInner, depth + 1);
        const cause = err.cause;
        if (cause !== undefined)
            out.cause = describeError(cause, depth + 1);
        if (err.stack)
            out.stack = err.stack.split('\n').slice(0, 4).join('\n');
        return out;
    }
    if (err && typeof err === 'object')
        return { value: String(err), raw: err };
...
export class TelegramAdapter {
     ...
     */
    logNonPrivateDropped(handler, ctx) {
    ...
    async initialize(config) {
        ...
        this.bot.on('message:text', async (ctx) => {
            if (!this.messageHandler || !ctx.message || !ctx.chat)
                return;
        ...
        this.bot.on('callback_query:data', async (ctx) => {
            if (!this.buttonHandler || !ctx.callbackQuery)
                return;
            if (!isPrivateChat(ctx)) {
        ...
        // saved the token.
        try {
            await withTimeout(this.bot.api.deleteWebhook({ drop_pending_updates: false }), 10_000, 'deleteWebhook');
        ...
        this.bot.start({
            onStart: () => {
                ...
                this.bot?.api.getWebhookInfo().then((info) => this.log.info('[telegram] webhook state after start:', {
                    url: info.url || null,
     ...
     */
    async emitAttachmentMessage(ctx, meta) {
        if (!this.messageHandler || !ctx.message || !ctx.chat || !this.bot)
            return;
        ...
        if (meta.fileSize !== undefined && meta.fileSize > MAX_ATTACHMENT_BYTES) {
            ...
            await ctx.reply(`Attachment too large (>${Math.round(MAX_ATTACHMENT_BYTES / 1024 / 1024)}MB). Not forwarded.`).catch(() => { });
            return;
        }
        let downloaded;
        try {
            downloaded = await this.downloadToTemp(meta.fileId, meta.fileName ?? `${meta.type}-${Date.now()}`, meta.mimeType);
        }
        catch (err) {
            this.log.error('[telegram] attachment download failed:', describeError(err));
            await ctx.reply('Failed to download your attachment. Please try again.').catch(() => { });
            return;
        }
        const attachment = {
            type: meta.type,
     ...
     */
    async downloadToTemp(fileId, fallbackName, mimeType) {
        if (!this.bot)
        ...
        const localPath = join(tmpdir(), `craft-agent-messaging-${randomBytes(8).toString('hex')}${ext}`);
    ...
    async sendFile(channelId, file, filename, caption) {
        if (!this.bot)
            throw new Error('Telegram adapter not initialized');
