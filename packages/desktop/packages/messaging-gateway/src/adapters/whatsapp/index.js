...
import { encodeMessage, parseFrames, } from '@craft-agent/messaging-whatsapp-worker';
const NOOP_LOGGER = {
    info: () => { },
    warn: () => { },
    error: () => { },
...
export class WhatsAppAdapter {
    ...
    async initialize(config) {
        const cfg = config;
        if (!cfg.workerEntry)
            throw new Error('WhatsApp: workerEntry path is required');
        if (!cfg.authStateDir)
            throw new Error('WhatsApp: authStateDir is required');
        if (this.proc) {
        ...
        this.proc.stdout?.on('data', (chunk) => {
            ...
            const { messages, rest } = parseFrames(this.stdoutBuffer);
            this.stdoutBuffer = rest;
            for (const ev of messages)
                this.onWorkerEvent(ev);
        });
    ...
    async editMessage(_channelId, _messageId, _text) {
