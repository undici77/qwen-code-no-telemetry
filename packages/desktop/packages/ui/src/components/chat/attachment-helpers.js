 ...
 */
export function getFileTypeLabel(type, mimeType, fileName) {
 ...
 */
export function FileTypeIcon({ type, mimeType, className }) {
    const baseClass = cn("h-4 w-4", className);
    // Images get dedicated icon
    if (type === 'image') {
        return _jsx(ImageIcon, { className: cn(baseClass, "text-accent") });
    }
    // Everything else gets generic file icon with color tint
    const colorClass = getFileColor(type, mimeType);
    return _jsx(File, { className: cn(baseClass, colorClass) });
}
function getFileColor(type, mimeType) {
...
//# sourceMappingURL=attachment-helpers.js.map