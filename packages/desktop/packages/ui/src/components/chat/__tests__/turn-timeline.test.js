...
describe('buildTurnTimelineItems', () => {
    it('interleaves commentary, activity sections, and final response chronologically', () => {
        ...
        expect(timeline[1]?.type === 'activity-section'
            ? timeline[1].activities.map((item) => item.id)
            : []).toEqual(['tool-1']);
        expect(timeline[3]?.type === 'activity-section'
    ...
    it('keeps adjacent tool activities in the same activity section', () => {
        ...
        expect(timeline[0]?.type === 'activity-section'
...
describe('processed duration', () => {
    ...
    it('handles details and final response with identical timestamps', () => {
        const timeline = buildTurnTimelineItems([activity({ id: 'tool', toolName: 'Read', timestamp: 1000 })], {
            text: 'Done',
            isStreaming: false,
            timestamp: 1000,
            messageId: 'final',
        ...
        const split = splitTimelineAtFinalResponse(timeline);
        expect(formatProcessedDuration(getProcessedDurationMs(split.detailItems, split.finalResponseItem))).toBe('<1s');
    });
});
//# sourceMappingURL=turn-timeline.test.js.map