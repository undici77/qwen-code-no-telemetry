...
import { groupActivitiesByParent, groupMessagesByTurn, computeLastChildSet, isActivityGroup, } from '../turn-utils';
// ============================================================================
// Test Helpers
// ============================================================================
 ...
 */
function createTaskActivity(description, overrides = {}) {
 ...
 */
function createChildActivity(parentToolUseId, overrides = {}) {
    return createActivity({
 ...
 */
function createTaskOutputActivity(taskId, data, overrides = {}) {
 ...
 */
function createTodoWriteActivity(todos, overrides = {}) {
    return createActivity({
        toolName: 'TodoWrite',
        toolInput: { todos },
...
describe('groupActivitiesByParent', () => {
    ...
    describe('mixed orphans and Task groups', () => {
        it('preserves orphan activities alongside Task groups in chronological order', () => {
            ...
            expect(result[1].parent.toolName).toBe('Task');
    ...
    describe('TaskOutput data attachment', () => {
        ...
        it('hides TaskOutput activities from result', () => {
            ...
            expect(result.length).toBe(2);
            expect(result.some(item => !isActivityGroup(item) && item.toolName === 'TaskOutput')).toBe(false);
        });
    });
    describe('edge cases', () => {
...
describe('TodoWrite extraction', () => {
    ...
    it('includes TodoWrite activities in flat list (not grouped)', () => {
        ...
        expect(result.length).toBe(1);
        expect(isActivityGroup(result[0])).toBe(false);
        expect(result[0].toolName).toBe('TodoWrite');
    });
    it('handles TodoWrite as child of Task', () => {
        resetCounters();
        const task = createTaskActivity('Plan implementation');
        const todoChild = createTodoWriteActivity([{ content: 'Step 1', status: 'pending' }], { parentId: task.toolUseId, depth: 1 });
...
//# sourceMappingURL=turn-utils-grouping.test.js.map