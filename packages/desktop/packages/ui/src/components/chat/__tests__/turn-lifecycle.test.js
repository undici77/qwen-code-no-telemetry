...
import { deriveTurnPhase, groupMessagesByTurn, } from '../turn-utils';
// ============================================================================
// Test Helpers
// ============================================================================
...
function createToolMessage(status, name = 'Read', turnId) {
    return {
...
function createAssistantMessage(isStreaming, isIntermediate = false, turnId) {
    return {
...
function updateMessage(messages, id, updates) {
...
describe('turn lifecycle scenarios', () => {
    describe('simple response flow', () => {
        it('pending → streaming → complete (no tools)', () => {
            resetCounters();
            // 1. User message
            let messages = [createUserMessage()];
    ...
    describe('tool with error', () => {
        ...
        it('missing historical tool result marks turn complete', () => {
            resetCounters();
            turnIdCounter++;
            const messages = [
                createUserMessage(),
                {
                    ...createToolMessage('running', 'Bash'),
                    toolStatus: 'error',
    ...
    describe('interruption', () => {
        it('user message during tool_active marks turn complete', () => {
            ...
            const firstAssistantTurn = turns.find((t) => t.type === 'assistant');
...
//# sourceMappingURL=turn-lifecycle.test.js.map