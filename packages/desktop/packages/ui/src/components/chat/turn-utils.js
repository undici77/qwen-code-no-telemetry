 ...
 */
function stripErrorTags(content) {
    if (!content)
        return content;
    return content
        .replace(/<\/?error>/gi, '')
...
function hasTerminalMissingToolResult(turn) {
    return turn.activities.some(activity => activity.type === 'tool' &&
 ...
 */
export function getAssistantTurnUiKey(turn, index) {
 ...
 */
export function deriveTurnPhase(turn) {
 ...
 */
export function shouldShowThinkingIndicator(phase, isBuffering) {
...
function getToolStatus(message) {
    ...
    if (message.errorCode === 'response_too_large')
        return 'completed';
    if (message.isError)
        return 'error';
    ...
    if (message.toolStatus === 'backgrounded')
        return 'backgrounded';
 ...
 */
function messageToActivity(message, existingActivities = []) {
    const activity = {
        id: message.id,
        type: 'tool',
 ...
 */
function calculateActivityDepths(activities) {
    // Build a map of toolUseId -> activity for fast parent lookup
    const toolIdToActivity = new Map();
 ...
 */
function extractTodosFromActivities(activities) {
 ...
 */
export function groupMessagesByTurn(messages) {
    ...
    const flushCurrentTurn = (interrupted = false) => {
        if (currentTurn) {
            ...
            if (interrupted) {
                currentTurn.activities = currentTurn.activities.map(activity => activity.status === 'running'
                    ? { ...activity, status: 'error', error: 'Interrupted' }
                    : activity);
                if (currentTurn.todos) {
                    currentTurn.todos = currentTurn.todos.map(todo => todo.status === 'in_progress'
    ...
    for (const message of sortedMessages) {
        ...
        if (message.role === 'info' && message.statusType === 'compaction_complete') {
            if (currentTurn) {
                const statusIdx = currentTurn.activities.findIndex(a => a.type === 'status' && a.statusType === 'compacting');
...
function findSingleChangedMessageIndex(previousMessages, nextMessages) {
...
function hasOnlyContentChanged(previousMessage, nextMessage) {
    const previousRecord = previousMessage;
    const nextRecord = nextMessage;
...
function isStreamingAssistantContentUpdate(previousMessage, nextMessage) {
...
function patchAssistantTurnForStreamingMessage(turn, messageId, nextContent) {
 ...
 */
export function updateGroupedTurnsForStreamingMessage(previousMessages, nextMessages, previousTurns) {
    ...
    if (!previousMessage || !nextMessage)
        return undefined;
    if (!isStreamingAssistantContentUpdate(previousMessage, nextMessage))
        return undefined;
 ...
 */
export function getTurnIntent(turn) {
 ...
 */
export function hasPendingActivities(turn) {
 ...
 */
export function formatTurnAsMarkdown(turn) {
    const lines = [];
 ...
 */
export function formatActivityAsMarkdown(activity) {
    const lines = [];
    if (activity.type === 'intermediate') {
        // Commentary/thinking
 ...
 */
export function getLastAssistantTurn(turns) {
 ...
 */
export function getLastUserMessageTime(turns) {
    for (let i = turns.length - 1; i >= 0; i--) {
        const turn = turns[i];
        if (turn?.type === 'user') {
 ...
 */
export function isLastTurnStreaming(turns) {
 ...
 */
export function computeLastChildSet(activities) {
 ...
 */
export function isActivityGroup(item) {
 ...
 */
function extractTaskOutputData(activity) {
    if (!activity.content)
        return undefined;
    try {
 ...
 */
export function groupActivitiesByParent(activities) {
    ...
    const taskOutputByAgentId = new Map();
    for (const activity of activities) {
 ...
 */
export function countTotalActivities(items) {
    let count = 0;
...
//# sourceMappingURL=turn-utils.js.map