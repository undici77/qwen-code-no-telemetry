...
import { parsePatchFiles, DIFFS_TAG_NAME, registerCustomTheme, resolveTheme } from '@pierre/diffs';
 ...
 */
function parseUnifiedDiff(unifiedDiff, filePath) {
 ...
 */
export function UnifiedDiffViewer({ unifiedDiff, filePath = 'file', diffStyle = 'unified', theme = 'light', shikiTheme, disableBackground = false, disableFileHeader = true, onFileHeaderClick, onReady, className, }) {
    ...
    useEffect(() => {
        if (!onFileHeaderClick || disableFileHeader)
            return;
        ...
        const timer = setTimeout(() => {
            ...
            header.__craftClickCleanup = () => header.removeEventListener('click', handleClick);
        }, 150);
        return () => {
            clearTimeout(timer);
            const diffsContainer = containerRef.current?.querySelector(DIFFS_TAG_NAME);
    ...
    if (!fileDiff) {
        return (_jsx("div", { ref: containerRef, className: cn('h-full w-full overflow-auto p-4', className), style: {
                backgroundColor,
                fontFamily: '"JetBrains Mono", monospace',
                fontSize: 13,
                lineHeight: 1.6,
            }, children: _jsx("pre", { className: "text-foreground/70 whitespace-pre-wrap", children: unifiedDiff || '(empty diff)' }) }));
    }
    return (_jsx("div", { ref: containerRef, className: cn('h-full w-full overflow-auto transition-opacity duration-200', className), style: {
            backgroundColor,
            fontFamily: '"JetBrains Mono", monospace',
            fontSize: 13,
            lineHeight: 1.6,
        }, children: _jsx(FileDiff, { fileDiff: fileDiff, options: options, className: "min-h-full h-full" }) }));
}
/**
 ...
 */
export function getUnifiedDiffStats(unifiedDiff, filePath = 'file') {
...
//# sourceMappingURL=UnifiedDiffViewer.js.map