...
const FILE_PATH_TARGET_REGEX = new RegExp(`^(?!https?://|mailto:|ftp://|data:)(?:/|~/|\\./|\\.\\./|${PATH_START_CHAR}${PATH_CHAR}*)${PATH_CHAR}*\\.(?:${FILE_EXTENSIONS_PATTERN})$`, 'iu');
/**
 ...
 */
function isInsideMarkdownLink(pos, ranges) {
    return ranges.some(r => pos >= r.start && pos < r.end);
}
/**
 ...
 */
function stripPlaceholderLinks(text) {
    const codeRanges = findCodeRanges(text);
    // Match markdown links [text](url) where url contains placeholder patterns
    return text.replace(/\[([^\[\]]*)\]\(([^)]*)\)/g, (fullMatch, linkText, url, offset) => {
        // Don't modify links inside code blocks
        if (isInsideCode(offset, codeRanges))
            return fullMatch;
 ...
 */
export function preprocessLinks(text) {
    ...
    for (const link of links) {
        ...
        if (isInsideCode(link.start, codeRanges))
            continue;
        // Skip if inside an existing markdown link (text or href portion)
        if (isInsideMarkdownLink(link.start, markdownLinkRanges))
            continue;
