...
export function tableToCsv(columns, rows) {
    ...
    const dataRows = rows.map((row) => columns.map((col) => escapeField(row[col.key])).join(','));
...
function getCellStyle(col, val) {
    switch (col.type) {
        ...
        case 'badge': {
            ...
            if (s === 'active' || s === 'passing' || s === 'success' || s === 'done')
                return S.BADGE_SUCCESS;
            if (s === 'revoked' || s === 'failed' || s === 'error')
                return S.BADGE_ERROR;
...
export function tableToXlsx(columns, rows, filename) {
    ...
    const blob = new Blob([zipData.buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
