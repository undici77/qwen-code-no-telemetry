...
export function MarkdownMermaidBlock({ code, className, showExpandButton = true, tapToOpen = true, minHeight }) {
    ...
    if (error) {
        return _jsx(CodeBlock, { code: code, language: "mermaid", mode: "full", className: className });
    }
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: cn('relative group', className), style: minHeightStyle, children: [showExpandButton && (_jsx("button", { onClick: () => setIsFullscreen(true), className: cn("absolute top-2 right-2 p-1 rounded-[6px] transition-all z-10 select-none", "opacity-0 group-hover:opacity-100", "bg-background shadow-minimal", "text-muted-foreground/50 hover:text-foreground", "focus:outline-none focus-visible:ring-1 focus-visible:ring-ring focus-visible:opacity-100"), title: t('common.viewFullscreen'), children: _jsx(Maximize2, { className: "w-3.5 h-3.5" }) })), _jsx("div", { ref: scrollRef, style: {
                            overflowX: 'auto',
                            overflowY: 'hidden',
                            maskImage,
                            ...
                            }, onClick: tapToOpen ? () => setIsFullscreen(true) : undefined, role: tapToOpen ? 'button' : undefined, "aria-label": tapToOpen ? 'Open Mermaid diagram fullscreen' : undefined, tabIndex: tapToOpen ? 0 : undefined, onKeyDown: tapToOpen ? (e) => {
                                if (e.key === 'Enter' || e.key === ' ') {
                                    e.preventDefault();
                                    setIsFullscreen(true);
                                }
                            } : undefined, children: _jsx("div", { dangerouslySetInnerHTML: { __html: svg }, style: {
                                    transformOrigin: 'top left',
                                    transform: scaledDims && scaledDims.scale !== 1 ? `scale(${scaledDims.scale})` : undefined,
                                } }) }) })] }), _jsx(MermaidPreviewOverlay, { isOpen: isFullscreen, onClose: () => setIsFullscreen(false), svg: svg, code: code })] }));
}
//# sourceMappingURL=MarkdownMermaidBlock.js.map