...
const lastCodeLanguageByEditor = new WeakMap();
function findLastCodeLanguageInDoc(editor) {
...
function resolvePreferredCodeLanguage(editor) {
    ...
    if (editor.isActive('codeBlock')) {
        ...
        if (current && !VISUAL_LANGUAGES.has(current))
            return current;
    }
...
function rememberCodeLanguage(editor, language) {
    const normalized = String(language).trim().toLowerCase();
    if (!normalized || VISUAL_LANGUAGES.has(normalized))
        return;
    lastCodeLanguageByEditor.set(editor, normalized);
}
function insertCodeBlockWithPlaceholder(editor, insertPos) {
    const targetPos = insertPos ?? editor.state.selection.from;
...
export function createSlashCommandItems(_editor) {
    return [
        {
            id: 'paragraph',
            title: 'Text',
...
export function filterSlashCommandItems(items, query) {
...
function renderMenuItems(container, items, selectedIndex) {
...
class SlashMenuView {
    ...
    constructor(props) {
        this.props = props;
        this.surface = new InlineMenuSurface({
...
//# sourceMappingURL=TiptapSlashMenu.js.map