...
const RICH_BLOCK_SELECTION_HIGHLIGHT_KEY = new PluginKey('richBlockSelectionHighlight');
function isRichBlockNode(node) {
    return node?.type?.name != null && RICH_BLOCK_NODES.has(node.type.name);
}
function isEditableRichBlockNode(node) {
    return node?.type?.name != null && EDITABLE_RICH_BLOCK_NODES.has(node.type.name);
}
function syncImageLoadingState(root) {
    ...
    images.forEach((img) => {
        ...
        if (image.dataset.loadTrackedSrc === src)
            return;
        ...
        return [
            new Plugin({
                ...
                state: {
                    ...
                    apply(tr, prev) {
                        const meta = tr.getMeta(RICH_BLOCK_SELECTION_HIGHLIGHT_KEY);
                ...
                view: (view) => {
                    ...
                    const setSuspendDuringPointerDrag = (suspendDuringPointerDrag) => {
                        const pluginState = RICH_BLOCK_SELECTION_HIGHLIGHT_KEY.getState(view.state);
                        if (pluginState?.suspendDuringPointerDrag === suspendDuringPointerDrag)
                            return;
                ...
                props: {
                    decorations(state) {
                        const pluginState = RICH_BLOCK_SELECTION_HIGHLIGHT_KEY.getState(state);
                        if (pluginState?.suspendDuringPointerDrag)
                    ...
                    handleTextInput: (view) => {
                        ...
                        if (!(selection instanceof NodeSelection))
                            return false;
                        if (!isEditableRichBlockNode(selection.node))
                            return false;
                        this.editor.emit(RICH_BLOCK_EDIT_EVENT);
                        return true;
                    },
                    handleKeyDown: (view, event) => {
                        if (event.key === 'Enter') {
                            ...
                            if (selection instanceof NodeSelection && selection.node.type.name === 'inlineMath') {
                                ...
                                this.editor.emit(INLINE_MATH_EDIT_EVENT);
                                return true;
                            }
                        ...
                        if (event.key !== 'ArrowDown' && event.key !== 'ArrowUp')
                            return false;
                        if (isSlashSuggestionActive(this.editor))
                            return false;
                        ...
                        if ($head.depth < 1)
                            return false;
                        if (!view.endOfTextblock(down ? 'down' : 'up'))
                            return false;
                        if (down) {
...
//# sourceMappingURL=RichBlockInteractions.js.map