 ...
 * - latexBlock
 */
function TiptapCodeBlockView({ node, updateAttributes }) {
    ...
    const languageFilterInputRef = React.useRef(null);
    const languageOptionRefs = React.useRef([]);
    const currentLanguage = normalizeLanguage(node.attrs.language ?? null);
    ...
    const handleLanguageSelect = React.useCallback((language) => {
    ...
    React.useEffect(() => {
        if (highlightedLanguageIndex < filteredLanguageOptions.length)
            return;
    ...
    const handleLanguageFilterKeyDown = React.useCallback((event) => {
    ...
    return (_jsxs(NodeViewWrapper, { className: "tiptap-code-block-node tiptap-hover-actions-host", "data-actions-open": languageDropdownOpen ? 'true' : 'false', children: [_jsxs(TiptapHoverActions, { children: [_jsxs(SimpleDropdown, { align: "end", className: "min-w-[220px] max-w-[320px]", keyboardNavigation: false, onOpenChange: setLanguageDropdownOpen, trigger: (_jsxs(TiptapHoverActionButton, { active: languageDropdownOpen, title: t('editor.codeLanguage'), "aria-label": "Select code language", className: "tiptap-code-block-language-trigger", children: [_jsx("span", { className: "tiptap-code-block-language-label", children: languageLabel }), _jsx(ChevronDown, { className: "w-3 h-3" })] })), children: [_jsx("div", { className: "px-3 pt-1.5 pb-1", children: _jsx("input", { ref: languageFilterInputRef, value: languageFilter, onChange: (event) => setLanguageFilter(event.target.value), onKeyDown: handleLanguageFilterKeyDown, placeholder: t('editor.searchLanguages'), className: "w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground placeholder:select-none" }) }), _jsx("div", { className: "h-px bg-foreground/5 -mx-1" }), _jsx("div", { className: "max-h-[240px] overflow-y-auto p-1", children: filteredLanguageOptions.length === 0 ? (_jsx("div", { className: "px-2.5 py-2 text-xs text-foreground/50 select-none", children: "No languages found" })) : (filteredLanguageOptions.map((option, index) => {
                                    const isActive = option.value === currentLanguage;
                                    const isHighlighted = index === highlightedLanguageIndex;
                                    return (_jsx(SimpleDropdownItem, { onClick: () => handleLanguageSelect(option.value), buttonRef: (el) => {
                                            languageOptionRefs.current[index] = el;
                                        }, onMouseEnter: () => setHighlightedLanguageIndex(index), className: cn('pl-2.5', isActive && 'text-accent', isHighlighted && 'bg-foreground/[0.05]'), children: _jsxs("span", { className: "flex w-full items-center justify-between gap-2", children: [_jsx("span", { children: option.label }), isActive ? _jsx(Check, { className: "w-3.5 h-3.5 shrink-0" }) : _jsx("span", { className: "w-3.5 h-3.5 shrink-0" })] }) }, option.value ?? 'plain-text'));
                                })) })] }), _jsx(TiptapHoverActionButton, { title: t("common.copyCode"), "aria-label": t("common.copyCode"), onClick: handleCopy, children: copied ? _jsx(Check, { className: "w-3.5 h-3.5" }) : _jsx(Copy, { className: "w-3.5 h-3.5" }) })] }), _jsx("pre", { children: _jsx(NodeViewContent, { as: "code" }) })] }));
}
/**
...
export const tiptapCodeBlock = CodeBlockShiki.extend({
    ...
    parseMarkdown: (token, helpers) => {
        ...
        return helpers.createNode('codeBlock', { language: token.lang ?? null }, token.text
            ? [
                {
                    type: 'text',
                    text: token.text,
...
//# sourceMappingURL=TiptapCodeBlockView.js.map