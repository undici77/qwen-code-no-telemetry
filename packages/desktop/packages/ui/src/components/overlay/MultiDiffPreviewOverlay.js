 ...
 */
function createFileSections(changes, consolidated) {
    if (!consolidated) {
...
function computeChangeStats(change) {
    ...
    const oldFile = { name: change.filePath, contents: change.original, lang: lang };
    const newFile = { name: change.filePath, contents: change.modified, lang: lang };
...
// Main Component
// ============================================
export function MultiDiffPreviewOverlay({ isOpen, onClose, changes, consolidated = true, focusedChangeId, theme = 'light', embedded, diffViewerSettings, onDiffViewerSettingsChange, }) {
    ...
    const [diffStyle, setDiffStyleInternal] = useState(diffViewerSettings?.diffStyle ?? 'unified');
    const [disableBackground, setDisableBackgroundInternal] = useState(diffViewerSettings?.disableBackground ?? false);
    ...
    const setDisableBackground = useCallback((disabled) => {
    ...
    const headerActions = (_jsx(DiffViewerControls, { additions: totalStats.additions, deletions: totalStats.deletions, diffStyle: diffStyle, onDiffStyleChange: setDiffStyle, disableBackground: disableBackground, onBackgroundChange: setDisableBackground }));
    // Ref callback to register each change element for scroll-to support
    const setChangeRef = useCallback((changeId, el) => {
        if (el) {
    ...
    return (_jsx(PreviewOverlay, { isOpen: isOpen, onClose: onClose, theme: theme, typeBadge: typeBadge, filePath: headerFilePath, title: headerTitle, headerActions: headerActions, embedded: embedded, className: "bg-foreground-3", children: _jsx("div", { className: cn("flex px-6 min-h-full", animateReveal && "transition-opacity duration-150"), style: { opacity: contentVisible ? 1 : 0 }, children: _jsx("div", { className: "m-auto", style: { width: 'max-content', maxWidth: '100%', minWidth: 'min(850px, 100%)' }, children: _jsx("div", { className: "flex flex-col gap-4", children: fileSections.map((section) => (_jsx("div", { className: "flex flex-col gap-4", children: section.changes.map((change) => (_jsx("div", { ref: (el) => setChangeRef(change.id, el), className: "rounded-xl overflow-hidden bg-background shadow-minimal", style: { minHeight: change.error ? undefined : 200, borderRadius: 12 }, children: change.error ? (
                            // Errored change — tinted error banner
                            _jsx("div", { className: "px-4 py-4", children: _jsxs("div", { className: "flex items-start gap-3 px-4 py-3 rounded-[8px] bg-[color-mix(in_oklab,var(--destructive)_5%,var(--background))] shadow-tinted", style: { '--shadow-color': 'var(--destructive-rgb)' }, children: [_jsx(X, { className: "w-4 h-4 text-destructive shrink-0 mt-0.5" }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "text-xs font-semibold text-destructive/70 mb-0.5", children: [change.toolType, " Failed"] }), _jsx("p", { className: "text-sm text-destructive whitespace-pre-wrap break-words", children: change.error })] })] }) })) : change.unifiedDiff ? (
                            // Codex format: pre-computed unified diff
                            _jsx(UnifiedDiffViewer, { unifiedDiff: change.unifiedDiff, filePath: change.filePath, diffStyle: diffStyle, disableBackground: disableBackground, disableFileHeader: false, onFileHeaderClick: onOpenFileExternal, theme: theme, onReady: handleDiffReady })) : (
                            // original/modified string format
                            _jsx(ShikiDiffViewer, { original: change.original, modified: change.modified, filePath: change.filePath, diffStyle: diffStyle, disableBackground: disableBackground, disableFileHeader: false, onFileHeaderClick: onOpenFileExternal, theme: theme, onReady: handleDiffReady })) }, change.id))) }, section.key))) }) }) }) }));
}
//# sourceMappingURL=MultiDiffPreviewOverlay.js.map