...
export function HTMLPreviewOverlay({ isOpen, onClose, html, items, contentCache: externalCache, onLoadContent, initialIndex = 0, title, theme, }) {
    ...
    const [internalCache, setInternalCache] = React.useState({});
    ...
    const mergedCache = React.useMemo(() => {
        const merged = { ...internalCache };
        if (externalCache)
            Object.assign(merged, externalCache);
        if (html)
    ...
    const processedHtml = React.useMemo(() => activeContent ? injectBaseTarget(activeContent) : null, [activeContent]);
    ...
    const headerActions = (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(ItemNavigator, { items: resolvedItems, activeIndex: activeIdx, onSelect: setActiveIdx, size: "md" }), _jsx(CopyButton, { content: activeContent || '', label: "Copy HTML", className: "bg-background shadow-minimal" })] }));
    return (_jsx(PreviewOverlay, { isOpen: isOpen, onClose: onClose, theme: theme, typeBadge: {
            icon: Globe,
            label: 'HTML',
            variant: 'blue',
        }, title: title || activeItem?.label || 'HTML Preview', headerActions: headerActions, children: _jsxs("div", { className: "px-6 pb-6", children: [loadingItem && !activeContent && (_jsx("div", { className: "py-12 text-center text-muted-foreground text-sm", children: t('common.loading') })), loadError && !activeContent && (_jsx("div", { className: "py-12 text-center text-destructive/70 text-sm", children: loadError })), processedHtml && (_jsx("div", { className: "bg-white rounded-[12px] overflow-hidden shadow-minimal mx-auto", style: {
                        maxWidth: contentSize?.width ? `${contentSize.width + 128}px` : undefined,
                        padding: '24px 64px 36px',
                        opacity: measured ? 1 : 0,
                        transition: 'opacity 200ms ease-in',
                    }, children: _jsx("iframe", { ref: iframeRef, sandbox: "allow-same-origin allow-top-navigation-by-user-activation", srcDoc: processedHtml, onLoad: handleLoad, title: activeItem?.label || title || 'HTML Preview', className: "w-full border-0", style: { height: iframeHeight, minHeight: '400px' } }) }))] }) }));
}
//# sourceMappingURL=HTMLPreviewOverlay.js.map