...
import { ANNOTATION_PREFIX_SUFFIX_WINDOW, SELECTION_POINTER_MAX_AGE_MS, clamp, hasExistingTextRangeAnnotation, createSelectionPreviewAnnotation, createTextSelectionAnnotation, getCanonicalText, resolveNodeOffset, } from '../annotations/annotation-core';
import { getAnnotationNoteText, formatAnnotationFollowUpTooltipText, } from '../annotations/follow-up-state';
...
export function AnnotatableMarkdownDocument({ content, messageId, sessionId, annotations, onAddAnnotation, onRemoveAnnotation, onUpdateAnnotation, onOpenUrl, onOpenFile, sendMessageKey = 'enter', islandZIndex = 'var(--z-island, 400)', openAnnotationRequest, isStreaming = false, }) {
    ...
    const [selectionMenuTransitionConfig, setSelectionMenuTransitionConfig] = React.useState(buildAnnotationChipEntryTransition());
    const [annotationOverlay, setAnnotationOverlay] = React.useState({ rects: [], chips: [] });
    const { renderAnchor: selectionMenuRenderAnchor, renderSourceKey: selectionMenuRenderSourceKey, isVisible: isSelectionMenuVisible, openedAtRef: selectionMenuOpenedAtRef, handleExitComplete: handleSelectionMenuExitComplete, } = useAnnotationIslandPresentation({
        anchor: selectionMenuAnchor,
        sourceKey: selectionMenuSourceKey,
    });
    const contentLayerRef = React.useRef(null);
    const lastPointerRef = React.useRef(null);
    const dragStartPointerRef = React.useRef(null);
    const selectionStartedInContentRef = React.useRef(false);
    const closeSelectionMenu = React.useCallback(() => {
        closeAll();
    }, [closeAll]);
    const isTargetInsideAnnotationIsland = React.useCallback((target) => {
        if (!target)
            return false;
    ...
    const handleSubmitFollowUp = React.useCallback((note) => {
    ...
    const handleDeleteActiveAnnotation = React.useCallback(() => {
        if (!onRemoveAnnotation || !activeAnnotationDetail)
            return;
    ...
    const handleOpenAnnotationDetail = React.useCallback((annotationId, index, anchorX, anchorY, mode = 'view') => {
    ...
    const handleSelectionPointerDown = React.useCallback((event) => {
    ...
    const handleTextSelection = React.useCallback((event) => {
        if (!canAnnotate)
            return;
    ...
    const handleSelectionMenuRequestBack = React.useCallback(() => {
    ...
    const selectionMenu = (_jsx(AnnotationIslandMenu, { anchor: selectionMenuRenderAnchor, sourceKey: selectionMenuRenderSourceKey, replayNonce: selectionMenuShowNonce, isVisible: isSelectionMenuVisible, activeView: selectionMenuView, mode: followUpMode, draft: followUpDraft, onDraftChange: setFollowUpDraft, onOpenFollowUp: handleOpenFollowUpView, onCancel: handleCancelFollowUp, onRequestBack: handleSelectionMenuRequestBack, onRequestEdit: handleRequestFollowUpEdit, onSubmit: handleSubmitFollowUp, onDelete: activeAnnotationDetail ? handleDeleteActiveAnnotation : undefined, sendMessageKey: sendMessageKey, transitionConfig: selectionMenuTransitionConfig, onExitComplete: handleSelectionMenuExitComplete, zIndex: islandZIndex, usePortal: shouldRenderAnnotationIslandInPortal('fullscreen') }));
    return (_jsxs(_Fragment, { children: [_jsxs("div", { ref: contentLayerRef, className: "relative", onMouseDown: handleSelectionPointerDown, onMouseUp: handleTextSelection, children: [_jsx(Markdown, { mode: "minimal", onUrlClick: onOpenUrl, onFileClick: onOpenFile, hideFirstMermaidExpand: false, children: content }), _jsx(AnnotationOverlayLayer, { rects: annotationOverlay.rects, chips: annotationOverlay.chips, annotations: renderedAnnotations, getTooltipText: (annotation) => formatAnnotationFollowUpTooltipText(annotation), onChipOpen: ({ annotationId, index, anchorX, anchorY, mode }) => {
                            handleOpenAnnotationDetail(annotationId, index, anchorX, anchorY, mode);
                        } })] }), selectionMenu] }));
}
//# sourceMappingURL=AnnotatableMarkdownDocument.js.map