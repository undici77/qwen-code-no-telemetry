...
export function zoomStepScale(current, direction, factor, min, max) {
    const nextFactor = direction === 'in' ? factor : 1 / factor;
    return clampScale(current * nextFactor, min, max);
}
export function cursorAnchoredTranslate(translate, cursor, scaleRatio) {
    return {
        x: cursor.x - scaleRatio * (cursor.x - translate.x),
        y: cursor.y - scaleRatio * (cursor.y - translate.y),
    };
}
export function computeFitScale(container, content, min, max) {
...
export function useRichBlockInteractions({ isOpen, containerRef, minScale = RICH_BLOCK_DEFAULTS.minScale, maxScale = RICH_BLOCK_DEFAULTS.maxScale, zoomStepFactor = RICH_BLOCK_DEFAULTS.zoomStepFactor, wheelSensitivity = RICH_BLOCK_DEFAULTS.wheelSensitivity, keyboardShortcuts = true, }) {
    ...
    const zoomToPreset = useCallback((percent) => {
        setIsAnimating(true);
        setScale(clampScale(percent / 100, minScale, maxScale));
        setTranslate({ x: 0, y: 0 });
    }, [minScale, maxScale]);
    const zoomToFit = useCallback((content) => {
...
//# sourceMappingURL=useRichBlockInteractions.js.map