...
import { createSelectionPreviewAnnotation, createTextSelectionAnnotation, } from '../annotation-core';
...
describe('annotation core helpers', () => {
    it('creates text selection annotation with follow-up metadata and session id', () => {
        const annotation = createTextSelectionAnnotation('msg-1', {
            start: 2,
            end: 9,
            selectedText: 'example',
            prefix: 'pre',
            suffix: 'suf',
        }, 'Need follow-up', 'session-42');
        ...
        const followUp = annotation.meta?.followUp;
    ...
    it('creates preview annotation marked ephemeral with explicit source', () => {
        const preview = createSelectionPreviewAnnotation('msg-2', {
            start: 0,
            end: 4,
            selectedText: 'test',
            prefix: '',
        ...
        expect(preview.meta?.ephemeral).toBe(true);
        expect(preview.meta?.source).toBe('follow-up-selection-preview');
    });
...
//# sourceMappingURL=annotation-core.test.js.map