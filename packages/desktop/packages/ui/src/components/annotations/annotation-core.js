...
export function hasExistingTextRangeAnnotation(annotations, start, end) {
    return (annotations ?? []).some(annotation => {
        const pos = annotation.target.selectors.find(s => s.type === 'text-position');
        return pos?.start === start && pos?.end === end;
    });
}
export function createSelectionPreviewAnnotation(messageId, selection, sessionId = '') {
    return {
...
export function createTextSelectionAnnotation(messageId, selection, followUpNote, sessionId = '') {
...
export function collectTextSegments(root) {
...
export function getCanonicalText(root) {
    return collectTextSegments(root)
        .map(segment => segment.node.nodeValue || '')
        .join('');
}
export function resolveNodeOffset(root, targetNode, nodeOffset) {
...
export function resolveRangeFromOffsets(root, start, end) {
    if (end <= start)
        return null;
    const segments = collectTextSegments(root);
...
export function getClientRectsForOffsets(root, start, end) {
    if (end <= start)
        return [];
    const segments = collectTextSegments(root);
    const rects = [];
...
export function consolidateRectsByLine(rects) {
    if (rects.length <= 1)
        return rects;
...
//# sourceMappingURL=annotation-core.js.map