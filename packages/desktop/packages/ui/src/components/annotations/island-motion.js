...
export function clampIslandAnchorX(anchorX, islandWidth) {
...
export function getDefaultIslandWidthEstimate() {
    return DEFAULT_WIDTH_ESTIMATE;
}
export function buildSelectionEntryTransition(from, to) {
    if (!from || !to) {
        return {
            entryAngleDeg: 90,
    ...
    const derivedDistancePx = clamp(speedPxPerSec * SPEED_TO_DISTANCE_FACTOR, MIN_ENTRY_DISTANCE, MAX_ENTRY_DISTANCE);
    return {
...
export function buildAnnotationChipEntryTransition() {
    return {
        entryAngleDeg: 90,
        entryDistancePx: 64,
        entryStartScale: DEFAULT_START_SCALE,
...
//# sourceMappingURL=island-motion.js.map