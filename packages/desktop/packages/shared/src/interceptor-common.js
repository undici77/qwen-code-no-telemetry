...
let _sessionDir = process.env.CRAFT_SESSION_DIR || null;
// ============================================================================
// LOGGING
// ============================================================================
    ...
    const message = `${timestamp} [interceptor] ${args.map((a) => {
        if (typeof a === 'object') {
            try {
                ...
                const keys = a && typeof a === 'object' ? Object.keys(a).join(', ') : 'unknown';
...
function getInterceptorConfig() {
    const now = Date.now();
    if (_cachedConfig && (now - _cacheTimestamp) < CONFIG_CACHE_TTL_MS)
 ...
 */
export function isRichToolDescriptionsEnabled() {
 ...
 */
export function isExtendedPromptCacheEnabled() {
    const config = getInterceptorConfig();
 ...
 */
export function is1MContextEnabled() {
    const config = getInterceptorConfig();
...
export function setStoredError(error) {
...
export function getLastApiError(sessionDir) {
...
function readMetadataFileFromDir(dir) {
    try {
...
function writeMetadataFile(allMetadata) {
