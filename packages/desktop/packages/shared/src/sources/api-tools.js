 ...
 */
export function buildAuthorizationHeader(authScheme, token) {
 ...
 */
function isBasicAuthCredential(cred) {
 ...
 */
function buildUrl(baseUrl, path, method, params, auth, credential) {
 ...
 */
export function createApiTool(config, credential, sessionPath, summarize) {
    ...
    return localTool(toolName, description, {
        ...
        try {
            ...
            if (method !== 'GET' && params && Object.keys(params).length > 0) {
                ...
                if (typeof params._rawBody === 'string') {
                    ...
                    debug(`[api-tools] ${config.name}: raw body (${fetchOptions.headers['Content-Type']}): ${params._rawBody.substring(0, 200)}`);
                }
                else {
 ...
 */
export function createApiServer(config, credential, sessionPath, summarize) {
