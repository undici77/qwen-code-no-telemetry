 ...
 */
import { inferGoogleServiceFromUrl, inferSlackServiceFromUrl, inferMicrosoftServiceFromUrl, isApiOAuthProvider, hasRenewEndpoint, } from './types.ts';
import { buildAuthorizationHeader } from './api-tools.ts';
import { getCredentialManager } from '../credentials/index.ts';
import { CraftOAuth, getMcpBaseUrl, prepareMcpOAuth, exchangeMcpOAuth } from '../auth/oauth.ts';
import {} from '../auth/types.ts';
import { OAUTH_RELAY_CALLBACK_URL, wrapPreparedOAuthFlowForRelay } from '../auth/oauth-relay.ts';
import { startGoogleOAuth, prepareGoogleOAuth, exchangeGoogleOAuth, refreshGoogleToken, } from '../auth/google-oauth.ts';
import { startSlackOAuth, prepareSlackOAuth, exchangeSlackOAuth, refreshSlackToken, } from '../auth/slack-oauth.ts';
import { startMicrosoftOAuth, prepareMicrosoftOAuth, exchangeMicrosoftOAuth, refreshMicrosoftToken, } from '../auth/microsoft-oauth.ts';
import { prepareGenericOAuth, exchangeGenericOAuth, refreshGenericOAuthToken, } from '../auth/generic-oauth.ts';
 ...
 */
export function isMultiHeaderCredential(cred) {
...
export class SourceCredentialManager {
     ...
     */
    async prepareOAuth(source, options) {
        ...
        switch (provider) {
            case 'google': {
                ...
                if (api?.googleScopes && api.googleScopes.length > 0) {
                    ...
                    if (!service) {
                        throw new Error(`Cannot determine Google service for source '${source.config.slug}'. ` +
            ...
            case 'microsoft': {
                ...
                if (api?.microsoftScopes && api.microsoftScopes.length > 0) {
                    ...
                    if (!service) {
                        throw new Error(`Cannot determine Microsoft service for source '${source.config.slug}'. ` +
     ...
     */
    async exchangeAndStore(source, provider, params) {
        let result;
        switch (provider) {
            case 'google':
     ...
     */
    async authenticate(source, callbacks, sessionContext) {
     ...
     */
    async authenticateMcp(source, callbacks, sessionContext) {
        if (!source.config.mcp?.url) {
            return { success: false, error: 'MCP URL not configured' };
        }
        try {
            const oauth = new CraftOAuth({ mcpUrl: source.config.mcp.url }, callbacks, sessionContext);
     ...
     */
    async authenticateGoogle(source, callbacks, sessionContext) {
        try {
            ...
            const result = await startGoogleOAuth(options);
            if (!result.success) {
     ...
     */
    async authenticateSlack(source, callbacks, sessionContext) {
        try {
            // Determine service/scopes from config
            const api = source.config.api;
            let service;
            ...
            const result = await startSlackOAuth(options);
            if (!result.success) {
     ...
     */
    async authenticateMicrosoft(source, callbacks, sessionContext) {
        try {
            // Determine service/scopes from config
            const api = source.config.api;
            let service;
            ...
            const result = await startMicrosoftOAuth(options);
            if (!result.success) {
    ...
    async doRefresh(source) {
        ...
        if (source.config.api?.authType === 'oauth') {
            ...
            if (source.config.api?.baseUrl && cred.clientId) {
                return this.refreshMcp({ ...source, config: { ...source.config, type: 'mcp', mcp: { url: source.config.api.baseUrl, authType: 'oauth' } } }, cred);
            }
            return null;
        }
        // MCP refresh
     ...
     */
    async refreshGoogle(source, cred) {
        try {
            // Pass stored credentials (or fall back to env vars via undefined)
            const result = await refreshGoogleToken(cred.refreshToken, cred.clientId, cred.clientSecret);
     ...
     */
    async refreshSlack(source, cred) {
        try {
     ...
     */
    async refreshMicrosoft(source, cred) {
        try {
     ...
     */
    async authenticateGeneric(source, _cb, _sessionContext) {
        const oauthConfig = source.config.api?.oauth;
        if (!oauthConfig) {
     ...
     */
    async refreshGeneric(source, cred) {
        const oauthConfig = source.config.api?.oauth;
        ...
        try {
            const result = await refreshGenericOAuthToken(cred.refreshToken, oauthConfig.tokenUrl, cred.clientId || oauthConfig.clientId, cred.clientSecret || oauthConfig.clientSecret);
            await this.save(source, {
                ...cred,
                value: result.accessToken,
                refreshToken: result.refreshToken || cred.refreshToken,
    ...
    async refreshMcp(source, cred) {
        ...
        try {
            ...
            const oauth = new CraftOAuth({ mcpUrl: source.config.mcp.url }, {
                onStatus: () => { },
                onError: () => { },
            });
...
function substituteTokenInBody(obj, token) {
    ...
    for (const [key, value] of Object.entries(obj)) {
        if (typeof value === 'string') {
            ...
            result[key] = value.map(item => typeof item === 'string' ? item.replace(/\{\{token\}\}/g, token) :
                (item && typeof item === 'object' ? substituteTokenInBody(item, token) : item));
        }
        else if (value && typeof value === 'object') {
            result[key] = substituteTokenInBody(value, token);
        }
        else {
            result[key] = value;
        }
 ...
 */
export function sourceNeedsAuthentication(source) {
    const mcp = source.config.mcp;
    const api = source.config.api;
 ...
 */
export function getSourcesNeedingAuth(sources) {
 ...
 */
export function getSourceCredentialManager() {
    if (!instance) {
