...
import { deleteSource, generateSourceSlug, getSourcePath, loadSourceConfig, loadSourceGuide, loadWorkspaceSources, sourceExists, } from '../storage.ts';
describe('source storage slug validation', () => {
    it('resolves valid source slugs under the workspace sources directory', () => {
        ...
        expect(getSourcePath(workspaceRoot, 'craft-kb')).toBe(join(workspaceRoot, 'sources', 'craft-kb'));
        expect(getSourcePath(workspaceRoot, 'source2')).toBe(join(workspaceRoot, 'sources', 'source2'));
        expect(getSourcePath(workspaceRoot, 'my-source-123')).toBe(join(workspaceRoot, 'sources', 'my-source-123'));
    });
    it('rejects traversal and malformed source slugs before path construction', () => {
        ...
        for (const slug of unsafeSlugs) {
            expect(() => getSourcePath(workspaceRoot, slug)).toThrow(`Invalid source slug: ${JSON.stringify(slug)}`);
        }
    });
    it('does not delete sibling directories when given an unsafe slug', () => {
        ...
        for (const slug of ['../sessions', '..\\sessions']) {
            expect(() => deleteSource(workspaceRoot, slug)).toThrow(`Invalid source slug: ${JSON.stringify(slug)}`);
        }
    ...
    it('preserves null-return and source listing behavior for legacy invalid source directories', () => {
        ...
        writeFileSync(join(validDir, 'config.json'), JSON.stringify({
...
//# sourceMappingURL=storage-source-slug.test.js.map