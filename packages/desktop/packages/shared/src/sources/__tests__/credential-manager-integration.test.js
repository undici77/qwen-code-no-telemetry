...
describe('getApiCredential with multi-header sources', () => {
    ...
    test('should return MultiHeaderCredential when source has headerNames and credential is valid JSON', async () => {
        ...
        expect(result['DD-API-KEY']).toBe('test-api-key');
        expect(result['DD-APPLICATION-KEY']).toBe('test-app-key');
        loadSpy.mockRestore();
    });
...
//# sourceMappingURL=credential-manager-integration.test.js.map