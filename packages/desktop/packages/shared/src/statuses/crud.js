 ...
 */
function generateStatusSlug(label) {
    return label
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, '')
 ...
 */
export function createStatus(workspaceRootPath, input) {
 ...
 */
export function updateStatus(workspaceRootPath, statusId, updates) {
    const config = loadStatusConfig(workspaceRootPath);
    ...
    if (updates.label !== undefined)
        status.label = updates.label;
    if (updates.color !== undefined)
        status.color = updates.color;
    if (updates.icon !== undefined)
        status.icon = updates.icon;
    if (updates.category !== undefined)
        status.category = updates.category;
    saveStatusConfig(workspaceRootPath, config);
    return status;
}
/**
 ...
 */
export function deleteStatus(workspaceRootPath, statusId) {
    const config = loadStatusConfig(workspaceRootPath);
    const status = config.statuses.find(s => s.id === statusId);
    if (!status) {
        throw new Error(`Status '${statusId}' not found`);
 ...
 * Reorder statuses
 */
export function reorderStatuses(workspaceRootPath, orderedIds) {
    const config = loadStatusConfig(workspaceRootPath);
 ...
 */
export function resetToDefaults(workspaceRootPath) {
 ...
 */
function migrateSessionsFromDeletedStatus(workspaceRootPath, deletedStatusId) {
