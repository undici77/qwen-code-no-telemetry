...
import { loadStoredConfig } from './storage.ts';
import { validateConfig, validatePreferences, validateSource, } from './validators.ts';
import { loadSource, loadWorkspaceSources, loadSourceGuide, sourceNeedsIconDownload, downloadSourceIcon, } from '../sources/storage.ts';
...
import { loadStatusConfig, statusNeedsIconDownload, downloadStatusIcon, } from '../statuses/storage.ts';
 ...
 */
const activeWatchers = new Map(); // workspaceDir → creator workspaceId
/** Exported for testing only */
export function _getActiveWatchers() {
...
export class ConfigWatcher {
    ...
    lastLlmConnectionsHash = '';
    // Computed paths
    workspaceDir;
    sourcesDir;
    skillsDir;
    constructor(workspaceIdOrPath, callbacks) {
     ...
     */
    handleWorkspaceFileChange(relativePath, eventType) {
     ...
     */
    debounce(key, handler, delayMs = DEBOUNCE_MS) {
    ...
    handleSourcesDirChange() {
        ...
        try {
            ...
            this.callbacks.onError?.('sources/', error);
        }
    }
    /**
     ...
     */
    handleSourcePermissionsChange(slug) {
    ...
    handleSkillsDirChange() {
        ...
        try {
            ...
            this.callbacks.onError?.('skills/', error);
        }
    }
    /**
     ...
     */
    handleWorkspacePermissionsChange() {
     ...
     */
    handleStatusIconChange(iconFilename) {
     ...
     */
    handleSessionMetadataChange(sessionId) {
     ...
     */
    handleDefaultPermissionsChange() {
     ...
     */
    handlePresetThemeChange(themeId) {
 ...
 */
export function createConfigWatcher(workspaceId, callbacks) {
