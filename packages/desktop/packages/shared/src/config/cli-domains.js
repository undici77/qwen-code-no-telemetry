 ...
 */
export const CRAFT_AGENTS_CLI_OWNED_WORKSPACE_PATH_SCOPES = dedupeScopes(Object.values(POLICIES).flatMap(policy => policy.workspacePathScopes));
/**
 * Canonical workspace-relative path scopes guarded for direct Bash operations.
 */
export const CRAFT_AGENTS_CLI_OWNED_BASH_GUARD_PATH_SCOPES = dedupeScopes(Object.values(POLICIES).flatMap(policy => policy.bashGuardPaths ?? []));
/**
 * Namespace-aware workspace scope entries for craft-agent CLI owned paths.
 */
export const CRAFT_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES = Object.values(POLICIES)
 ...
 */
export const CRAFT_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES = Object.values(POLICIES)
 ...
 */
export function getCraftAgentReadOnlyBashPatterns() {
    const namespaces = Object.keys(POLICIES);
    const namespaceAlternation = namespaces.join('|');
    const rules = namespaces.map((namespace) => {
    ...
    rules.push({ pattern: '^craft-agent\\s*$', comment: 'craft-agent bare invocation (prints help)' }, { pattern: `^craft-agent\\s+(${namespaceAlternation})\\s*$`, comment: 'craft-agent entity help' }, { pattern: `^craft-agent\\s+(${namespaceAlternation})\\s+--help\\b`, comment: 'craft-agent entity help flags' }, { pattern: '^craft-agent\\s+--(help|version|discover)\\b', comment: 'craft-agent global flags' });
    return rules;
}
export function getCliDomainPolicy(namespace) {
    return POLICIES[namespace];
}
//# sourceMappingURL=cli-domains.js.map