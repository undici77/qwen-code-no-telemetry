...
export const StoredConfigSchema = z.object({
    ...
    defaultThinkingLevel: z.enum([...THINKING_LEVEL_IDS, 'think']).transform(v => v === 'think' ? 'medium' : v).optional(),
    ...
    if (!result.success) {
        ...
        if (config.llmConnections) {
            ...
            for (const [i, conn] of config.llmConnections.entries()) {
                ...
                if (!isValidProviderAuthCombination(conn.providerType, conn.authType)) {
                    warnings.push({
                        file: 'config.json',
 ...
 */
export function validateAll(workspaceId, workspaceRoot) {
    const results = [
        validateConfig(),
        validatePreferences(),
        validateToolIcons(),
...
function getSourceValidationFile(slug) {
    try {
...
function getSourcePermissionsValidationFile(sourceSlug) {
    try {
...
export const FolderSourceConfigSchema = z.object({
    id: z.string().min(1),
    name: z.string().min(1),
    slug: z.string().regex(SOURCE_SLUG_REGEX, 'Slug must be lowercase alphanumeric segments separated by single hyphens'),
    enabled: z.boolean(),
 ...
 */
export function validateSourceConfigContent(jsonString) {
    let content;
    try {
 ...
 */
export function validateSource(workspaceId, slug) {
 ...
 */
export function validateAllSources(workspaceId) {
    const sourcesDir = getWorkspaceSourcesPath(workspaceId);
    const errors = [];
    const warnings = [];
 ...
 */
function findSkillIconForValidation(skillDir) {
 ...
 */
export function validateSkill(workspaceRoot, slug) {
 ...
 */
export function validateSkillContent(markdownContent, slug) {
    const file = `skills/${slug}/SKILL.md`;
    const errors = [];
 ...
 * @param workspaceRoot - Absolute path to workspace root folder
 */
export function validateAllSkills(workspaceRoot) {
    const skillsDir = getWorkspaceSkillsPath(workspaceRoot);
    const errors = [];
    const warnings = [];
...
const REQUIRED_FIXED_STATUS_IDS = ['todo', 'done', 'cancelled'];
/**
 ...
 * @param workspaceRoot - Absolute path to workspace root folder
 */
export function validateStatuses(workspaceRoot) {
 ...
 */
export function validateStatusesContent(jsonString) {
    ...
    for (const status of config.statuses) {
        const shouldBeFixed = REQUIRED_FIXED_STATUS_IDS.includes(status.id);
...
const BaseLabelConfigSchema = z.object({
    id: z.string().regex(/^[a-z0-9]([a-z0-9-]*[a-z0-9])?$/, 'Label ID must be a simple slug (lowercase alphanumeric + hyphens, e.g., "bug", "frontend")'),
...
const LabelConfigSchema = BaseLabelConfigSchema.extend({
 ...
 * @param workspaceRoot - Absolute path to workspace root folder
 */
export function validateLabels(workspaceRoot) {
 ...
 */
export function validateLabelsContent(jsonString) {
    ...
    const seenIds = new Set();
    function checkUniqueIds(labels, path) {
    ...
    function checkDepth(labels, depth, path) {
    ...
    function checkAutoRules(labels, path) {
        for (let i = 0; i < labels.length; i++) {
            const label = labels[i];
...
import { validatePermissionsConfig, getWorkspacePermissionsPath, getSourcePermissionsPath, getAppPermissionsDir, } from '../agent/permissions-config.ts';
 ...
 */
function validatePermissionsFile(filePath, displayFile) {
 ...
 */
export function validatePermissionsContent(jsonString, displayFile = 'permissions.json') {
    const errors = [];
    // Parse JSON
    let content;
    try {
 ...
 * @param workspaceRoot - Absolute path to workspace root folder
 */
export function validateWorkspacePermissions(workspaceRoot) {
 ...
 */
export function validateSourcePermissions(workspaceRoot, sourceSlug) {
 ...
 */
export function validateDefaultPermissions() {
 ...
 */
export function validateAllPermissions(workspaceRoot) {
    const errors = [];
    const warnings = [];
    // Validate app-level default permissions
 ...
 */
export function isValidPermissionsFile(filePath) {
    try {
...
export const ThemeOverrideSchema = z.object({
    ...
    .refine((data) => data.mode !== 'scenic' || Boolean(data.backgroundImage), { message: 'backgroundImage is required when mode is scenic', path: ['backgroundImage'] });
/**
 ...
 */
export function validateThemeContent(jsonString, displayFile = 'theme.json') {
    const errors = [];
    // Parse JSON
    let content;
    try {
 ...
 */
export function validateThemeOverrideContent(jsonString, displayFile = 'theme.json') {
    const errors = [];
    // Parse JSON
    let content;
    try {
 ...
 * Returns true if the file exists and passes schema validation.
 */
export function isValidThemeFile(filePath) {
    try {
        const content = readFileSync(filePath, 'utf-8');
...
const ToolIconEntrySchema = z.object({
    id: z.string().min(1, 'Tool ID is required').regex(/^[a-z0-9-]+$/, 'ID must be lowercase alphanumeric with hyphens (e.g., "my-tool")'),
 ...
 */
export function validateToolIconsContent(jsonString) {
 ...
 */
export function detectConfigFileType(filePath, workspaceRootPath) {
 ...
 */
export function detectAppConfigFileType(filePath) {
    const normalizedPath = filePath.replace(/\\/g, '/');
 ...
 */
export function validateConfigFileContent(detection, content) {
