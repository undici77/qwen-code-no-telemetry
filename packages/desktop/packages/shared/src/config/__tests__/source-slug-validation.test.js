...
describe('source slug validation in config validators', () => {
    ...
    it('skips legacy invalid source directories during permissions validation', () => {
        ...
        expect(result.warnings.some(warning => warning.file === 'sources/legacy-source-/permissions.json' &&
            warning.message.includes('invalid slug format'))).toBe(true);
    });
    it('skips legacy invalid source directories during all source validation', () => {
        ...
        expect(result.errors).toHaveLength(0);
        expect(result.warnings.some(warning => warning.file === 'sources/legacy-source-/config.json' &&
            warning.message.includes('invalid slug format'))).toBe(true);
    });
...
//# sourceMappingURL=source-slug-validation.test.js.map