...
describe('project-root workspace storage', () => {
    it('stores new workspaces for existing projects under managed config storage', () => {
        ...
        writeFileSync(join(configDir, 'config.json'), JSON.stringify({ workspaces: [], activeWorkspaceId: null, activeSessionId: null }, null, 2), 'utf-8');
        const output = runEval(configDir, `const ws = addWorkspace({ name: 'qwen-code', rootPath: ${JSON.stringify(projectDir)} }); console.log(JSON.stringify(ws));`);
    ...
    it('stores newly-created external project folders under managed config storage', () => {
        ...
        writeFileSync(join(configDir, 'config.json'), JSON.stringify({ workspaces: [], activeWorkspaceId: null, activeSessionId: null }, null, 2), 'utf-8');
        const output = runEval(configDir, `const ws = addWorkspace({ name: 'fresh-project', rootPath: ${JSON.stringify(projectDir)} }); console.log(JSON.stringify(ws));`);
        const workspace = JSON.parse(output);
...
//# sourceMappingURL=workspace-project-root-migration.test.js.map