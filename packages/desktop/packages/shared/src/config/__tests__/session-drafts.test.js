...
function runEval(configDir, code) {
...
describe('session draft storage', () => {
    ...
    it('round-trips a draft with attachment refs', () => {
        const configDir = makeConfigDir();
        runEval(configDir, "setSessionDraft('s1', { text: 'caption', attachments: [{ path: '/tmp/a.png', name: 'a.png' }, { path: '/tmp/b.pdf', name: 'Report.pdf' }] })");
        const output = runEval(configDir, "console.log(JSON.stringify(getSessionDraft('s1')))");
    ...
    it('strips extra FileAttachment fields when persisting path-only attachments', () => {
        ...
        runEval(configDir, "setSessionDraft('s1', { text: '', attachments: [{ path: '/tmp/a.png', name: 'a.png', base64: 'AAAA', size: 4, type: 'image', mimeType: 'image/png' }] })");
    ...
    it('round-trips a draft with a content-backed attachment (paste / web-drag path)', () => {
        const configDir = makeConfigDir();
        runEval(configDir, "setSessionDraft('s1', { text: 'note', attachments: [{ path: 'pasted-image-1.png', name: 'pasted-image-1.png', content: { type: 'image', mimeType: 'image/png', size: 4, base64: 'AAAA', thumbnailBase64: 'BBBB' } }] })");
        const output = runEval(configDir, "console.log(JSON.stringify(getSessionDraft('s1')))");
        expect(JSON.parse(output)).toEqual({
            text: 'note',
            attachments: [{
    ...
    it('round-trips a text-content attachment without base64', () => {
        const configDir = makeConfigDir();
        runEval(configDir, "setSessionDraft('s1', { text: '', attachments: [{ path: 'pasted.txt', name: 'pasted.txt', content: { type: 'text', mimeType: 'text/plain', size: 5, text: 'hello' } }] })");
        const output = runEval(configDir, "console.log(JSON.stringify(getSessionDraft('s1')))");
        expect(JSON.parse(output)).toEqual({
            text: '',
            attachments: [{
...
//# sourceMappingURL=session-drafts.test.js.map