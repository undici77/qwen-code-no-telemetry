...
function runEval(configDir, defaultWorkspaceDir, code) {
...
describe('default conversation workspace', () => {
    it('creates a protected Qwen conversation workspace outside managed project storage', () => {
        ...
        const output = runEval(configDir, defaultWorkspaceDir, `const ws = ensureDefaultConversationWorkspace(); console.log(JSON.stringify({ ws, all: getWorkspaces() }));`);
    ...
    it('does not remove or reorder protected conversation workspaces', () => {
        const configDir = setupConfigDir();
        const defaultWorkspaceDir = join(tmpdir(), `qwen-documents-${crypto.randomUUID()}`, 'Qwen');
        const output = runEval(configDir, defaultWorkspaceDir, `const ws = ensureDefaultConversationWorkspace(); const removed = await removeWorkspace(ws.id); const reordered = reorderWorkspaces([ws.id]); console.log(JSON.stringify({ removed, reordered, all: getWorkspaces() }));`);
...
//# sourceMappingURL=default-conversation-workspace.test.js.map