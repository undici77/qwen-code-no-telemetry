...
import { getOrCreateLatestSession } from '../sessions/index.ts';
import { discoverWorkspacesInDefaultLocation, ensureDefaultWorkspacesDir, generateSlug, generateUniqueWorkspacePath, getDefaultWorkspacesDir, loadWorkspaceConfig, saveWorkspaceConfig, createWorkspaceAtPath, isValidWorkspace, } from '../workspaces/storage.ts';
...
import {} from './config-defaults-schema.ts';
...
import { QWEN_CODE_CONNECTION_SLUG } from './llm-connections.ts';
...
export const DEFAULT_CONVERSATION_WORKSPACE_KIND = 'conversation';
...
export function loadConfigDefaults() {
    ...
    const defaults = readJsonFileSync(CONFIG_DEFAULTS_FILE);
    const parsedPermissionMode = typeof defaults.workspaceDefaults?.permissionMode === 'string'
    ...
    defaults.workspaceDefaults.cyclablePermissionModes = normalizeCyclablePermissionModes(defaults.workspaceDefaults?.cyclablePermissionModes);
    return defaults;
}
/**
...
function isManagedWorkspacePath(rootPath) {
...
function looksLikeUserProjectDirectory(rootPath) {
    if (!isExistingDirectory(rootPath))
        return false;
    return PROJECT_DIRECTORY_MARKERS.some(marker => existsSync(join(rootPath, marker)));
}
function allocateManagedWorkspacePath(workspace, reservedPaths) {
...
function migrateExternalProjectWorkspace(workspace, reservedPaths, forceProjectWorkingDirectory) {
    ...
    const shouldMoveToManagedStorage = !isValidWorkspace(workspace.rootPath) ||
        looksLikeUserProjectDirectory(workspace.rootPath) ||
        forceProjectWorkingDirectory;
    if (!shouldMoveToManagedStorage)
        return false;
 ...
 * Defaults to true if not set.
 */
export function getAutoCapitalisation() {
    const config = loadStoredConfig();
 ...
 */
export function setAutoCapitalisation(enabled) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 */
export function getSendMessageKey() {
    const config = loadStoredConfig();
 ...
 */
export function setSendMessageKey(key) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 */
export function setVoiceModel(model) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 */
export function getKeepAwakeWhileRunning() {
    const config = loadStoredConfig();
 ...
 */
export function setKeepAwakeWhileRunning(enabled) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 * Defaults to true if not set.
 */
export function getRichToolDescriptions() {
    const config = loadStoredConfig();
 ...
 */
export function setRichToolDescriptions(enabled) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 */
export function setPetSize(size) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 */
export function setPetWindowBounds(bounds) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 * Defaults to false if not set.
 */
export function getExtendedPromptCache() {
    const config = loadStoredConfig();
 ...
 */
export function setExtendedPromptCache(enabled) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 * Defaults to true if not set.
 */
export function getBrowserToolEnabled() {
    const config = loadStoredConfig();
 ...
 */
export function setBrowserToolEnabled(enabled) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 */
export function getEnable1MContext() {
    const config = loadStoredConfig();
 ...
 */
export function setEnable1MContext(enabled) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 */
export function setGitBashPath(path) {
    const config = loadStoredConfig();
    if (!config) {
 ...
 */
export function clearGitBashPath() {
    const config = loadStoredConfig();
    if (!config || !config.gitBashPath)
        return;
...
export function ensureDefaultConversationWorkspace() {
    ...
    if (!existingConfig) {
        createWorkspaceAtPath(rootPath, DEFAULT_CONVERSATION_WORKSPACE_NAME, { workingDirectory: rootPath }, { kind: DEFAULT_CONVERSATION_WORKSPACE_KIND, isProtected: true });
    }
    else {
 ...
 */
export function findWorkspaceIcon(rootPath) {
 ...
 */
export function getWorkspaceByNameOrId(nameOrId) {
    const workspaces = getWorkspaces();
    return workspaces.find(w => w.id === nameOrId ||
        w.name.toLowerCase() === nameOrId.toLowerCase()) || null;
}
export function updateWorkspaceRemoteServer(workspaceId, remoteServer) {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 */
export async function switchWorkspaceAtomic(workspaceId) {
    const config = loadStoredConfig();
    if (!config)
        return null;
    const workspace = config.workspaces.find(w => w.id === workspaceId);
...
export function addWorkspace(workspace) {
    ...
    const reservedPaths = new Set(config.workspaces.map(w => resolve(w.rootPath)));
    const shouldStoreInManagedWorkspace = !isManagedWorkspacePath(newWorkspace.rootPath) &&
...
export function saveWorkspaceConversation(workspaceId, messages, tokenUsage) {
...
export function loadWorkspaceConversation(workspaceId) {
...
export function clearWorkspaceConversation(workspaceId) {
    const filePath = join(WORKSPACES_DIR, workspaceId, 'conversation.json');
    if (existsSync(filePath)) {
 ...
 */
export function saveWorkspacePlan(workspaceId, plan) {
    const dir = ensureWorkspaceDir(workspaceId);
 ...
 */
export function loadWorkspacePlan(workspaceId) {
 ...
 */
export function clearWorkspacePlan(workspaceId) {
    const filePath = join(WORKSPACES_DIR, workspaceId, 'plan.json');
    if (existsSync(filePath)) {
        rmSync(filePath);
    }
...
function isAbsoluteDraftPath(p) {
    if (!p)
        return false;
    if (p.startsWith('/'))
        return true;
...
function isDraftAttachmentContent(value) {
    if (!value || typeof value !== 'object')
        return false;
    const c = value;
    if (!ATTACHMENT_CONTENT_TYPES.has(c.type))
        return false;
    if (typeof c.mimeType !== 'string')
        return false;
    if (typeof c.size !== 'number')
        return false;
    if (c.base64 !== undefined && typeof c.base64 !== 'string')
        return false;
    if (c.text !== undefined && typeof c.text !== 'string')
        return false;
    if (c.thumbnailBase64 !== undefined && typeof c.thumbnailBase64 !== 'string')
        return false;
    return true;
}
function isDraftAttachmentRef(value) {
    if (!value || typeof value !== 'object')
        return false;
    const ref = value;
    if (typeof ref.path !== 'string' || typeof ref.name !== 'string')
        return false;
    if (ref.content !== undefined && !isDraftAttachmentContent(ref.content))
        return false;
    ...
    if (ref.content === undefined && !isAbsoluteDraftPath(ref.path))
        return false;
    return true;
}
function isSessionDraft(value) {
    if (!value || typeof value !== 'object')
        return false;
    ...
    if (candidate.attachments !== undefined) {
        if (!Array.isArray(candidate.attachments))
            return false;
        if (!candidate.attachments.every(isDraftAttachmentRef))
            return false;
    }
    return true;
}
...
function loadDraftsData() {
    try {
        ...
        const raw = readJsonFileSync(DRAFTS_FILE);
        const drafts = {};
...
function normalizeDraftAttachment(ref) {
    const base = { path: ref.path, name: ref.name };
...
export function deleteSessionDraft(sessionId) {
    const data = loadDraftsData();
    delete data.drafts[sessionId];
    saveDraftsData(data);
}
 ...
 */
export function getAllSessionDrafts() {
    const data = loadDraftsData();
    return data.drafts;
}
 ...
 */
export function getAppThemePath() {
 ...
 */
export function getAppThemesDir() {
...
export function loadAppTheme() {
    try {
        ...
        return readJsonFileSync(APP_THEME_FILE);
    }
    catch {
        return null;
    }
 ...
 */
export function ensurePresetThemes() {
 ...
 */
export function loadPresetThemes() {
    ensurePresetThemes();
    const themesDir = getAppThemesDir();
    if (!existsSync(themesDir)) {
        return [];
    ...
    try {
        ...
        for (const file of files) {
            ...
            try {
                const theme = readJsonFileSync(path);
 ...
 */
function resolveThemeBackgroundImage(theme, themePath) {
 ...
 */
export function loadPresetTheme(id) {
    const themesDir = getAppThemesDir();
 ...
 */
export function getPresetThemesDir() {
 ...
 */
export function resetPresetTheme(id) {
    // Resolve bundled themes directory via shared asset resolver
    const bundledThemesDir = getBundledAssetsDir('themes');
    if (!bundledThemesDir) {
        return false;
...
function stripQwenRuntimeModelFields(connection) {
...
export function inferModelSelectionMode(connection, providerDefaultModelIds) {
    const currentIds = normalizeModelIds(connection.models);
    if (currentIds.length === 0)
        return 'automaticallySyncedFromProvider';
    return modelSetEquals(currentIds, providerDefaultModelIds)
        ? 'automaticallySyncedFromProvider'
        : 'userDefined3Tier';
}
function migrateModelDefaultsToConnections(config) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const configAny = config;
    if (!configAny.modelDefaults)
        return false;
    delete configAny.modelDefaults;
    return true;
}
function createQwenCodeConnection(existing) {
    return {
...
function cleanupRemovedConnectionCredentials(removedConnectionSlugs) {
...
function clearWorkspaceDefaultConnectionReferences() {
    let changed = false;
    try {
        const workspaces = getWorkspaces();
...
function normalizeToQwenCodeOnly(target) {
 ...
 */
export function migrateLegacyLlmConnectionsConfig() {
    const config = loadStoredConfig();
    if (!config)
        return;
    let needsSave = false;
 ...
 */
export function migrateOrphanedDefaultConnections() {
    const config = loadStoredConfig();
    if (!config)
        return;
 ...
 */
function ensureDefaultLlmConnection(config) {
    if (!config.llmConnections || config.llmConnections.length === 0) {
        return false;
    }
 ...
 */
export function getLlmConnection(slug) {
 ...
 */
export function addLlmConnection(connection) {
    const config = loadStoredConfig();
    if (!config)
        return false;
 ...
 */
export function updateLlmConnection(slug, updates) {
    const config = loadStoredConfig();
    if (!config)
        return false;
 ...
 */
export function deleteLlmConnection(slug) {
    const config = loadStoredConfig();
    if (!config)
        return false;
 ...
 */
export function getDefaultLlmConnection() {
    const config = loadStoredConfig();
    if (!config)
        return null;
 ...
 */
export function setDefaultLlmConnection(slug) {
    const config = loadStoredConfig();
    if (!config)
        return false;
 ...
 */
export function setDefaultThinkingLevel(level) {
    const config = loadStoredConfig();
    if (!config)
        return false;
 ...
 * @param slug - Connection slug
 */
export function touchLlmConnection(slug) {
    const config = loadStoredConfig();
    if (!config)
        return;
...
function normalizeProxyString(value) {
    const trimmed = value?.trim();
    return trimmed || undefined;
}
function normalizeNetworkProxySettings(settings) {
    return {
 ...
 */
export function setNetworkProxySettings(settings) {
    const config = loadStoredConfig();
    if (!config)
        return;
...
// ============================================
export function isSetupDeferred() {
    return loadStoredConfig()?.setupDeferred === true;
}
export function setSetupDeferred(deferred) {
    const config = loadStoredConfig();
    if (!config)
        return;
    if (deferred) {
 ...
 */
export function getToolIconsDir() {
 ...
 */
export function ensureToolIcons() {
