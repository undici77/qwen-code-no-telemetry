...
function isValidCredentialType(type) {
    return VALID_CREDENTIAL_TYPES.includes(type);
}
...
function isMessagingCredential(type) {
    return MESSAGING_CREDENTIAL_TYPES.includes(type);
}
...
function isSourceCredential(type) {
    return SOURCE_CREDENTIAL_TYPES.includes(type);
}
/** Check if type is an LLM connection credential */
function isLlmCredential(type) {
    return LLM_CREDENTIAL_TYPES.includes(type);
}
/** Convert CredentialId to credential store account string */
export function credentialIdToAccount(id) {
    const parts = [id.type];
...
export function accountToCredentialId(account) {
