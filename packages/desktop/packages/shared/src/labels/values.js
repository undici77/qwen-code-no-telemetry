 ...
 */
export function parseLabelEntry(entry) {
 ...
 */
export function formatLabelEntry(id, value) {
    if (value === undefined) {
        return id;
    }
 ...
 * "bug"         → "bug"
 */
export function extractLabelId(entry) {
    const separatorIndex = entry.indexOf(VALUE_SEPARATOR);
 ...
 */
export function validateLabelValue(rawValue, valueType) {
    switch (valueType) {
        case 'string':
            return true;
        case 'number':
 ...
 */
export function formatDisplayValue(rawValue, valueType) {
    if (valueType === 'date') {
