 ...
 */
function generateLabelSlug(name) {
    return name
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, '')
 ...
 */
export function createLabel(workspaceRootPath, input) {
 ...
 */
export function ensureLabelsExist(workspaceRootPath, labels) {
    return labels.map(label => {
        const { id: labelId, rawValue } = parseLabelEntry(label);
        if (isValidLabelId(workspaceRootPath, labelId))
            return label;
        if (!isValidLabelIdFormat(labelId))
            return label;
 ...
 */
export function updateLabel(workspaceRootPath, labelId, updates) {
    const config = loadLabelConfig(workspaceRootPath);
    ...
    if (updates.valueType !== undefined)
        label.valueType = updates.valueType || undefined;
    saveLabelConfig(workspaceRootPath, config);
    return label;
}
/**
 ...
 */
export function deleteLabel(workspaceRootPath, labelId) {
    const config = loadLabelConfig(workspaceRootPath);
 ...
 */
export function reorderLabels(workspaceRootPath, parentId, orderedIds) {
    const config = loadLabelConfig(workspaceRootPath);
    ...
    if (parentId) {
        ...
        if (!parent)
            throw new Error(`Parent label '${parentId}' not found`);
        if (!parent.children)
            throw new Error(`Parent label '${parentId}' has no children`);
 ...
 */
export function moveLabel(workspaceRootPath, labelId, newParentId) {
    const config = loadLabelConfig(workspaceRootPath);
 ...
 */
function removeNodeFromTree(labels, targetId) {
    // Check root level
 ...
 */
function stripLabelFromSessions(workspaceRootPath, deletedLabelId) {
    ...
    for (const session of sessions) {
        ...
        if (session.labels && session.labels.some((entry) => extractLabelId(entry) === deletedLabelId)) {
            const updatedLabels = session.labels.filter((entry) => extractLabelId(entry) !== deletedLabelId);
