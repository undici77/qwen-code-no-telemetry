...
describe('resolveSessionLabels', () => {
    ...
    describe('valued labels (oss#566 scenarios)', () => {
        it('resolves string-typed values', () => {
            const r = resolveSessionLabels(['parent-task::TASK-123', 'subtask-id::SUB-001'], LABELS);
            expect(r.resolved).toEqual(['parent-task::TASK-123', 'subtask-id::SUB-001']);
            expect(r.unknown).toEqual([]);
        });
        it('resolves a mixed batch (the exact #566 repro)', () => {
            const r = resolveSessionLabels(['subagent', 'parent-task::TASK-123', 'subtask-id::SUB-001'], LABELS);
    ...
    describe('rejections', () => {
        it('rejects valued input on a boolean label', () => {
            ...
            expect(r.reasons['subagent::hello']).toBe('label "subagent" doesn\'t accept a value (no valueType configured)');
        });
        it('rejects number values that are not valid numbers', () => {
            ...
            expect(r.reasons['priority::high']).toBe('label "priority" expects a number value, got "high"');
        });
        ...
        it('rejects unknown base ID even when input has a ::value', () => {
            ...
            expect(r.reasons['nonexistent::42']).toBe('label "nonexistent" is not configured');
        });
    });
    describe('mixed batches', () => {
        it('partially resolves — valid entries pass, invalid entries reported', () => {
            const r = resolveSessionLabels(['bug', 'priority::high', 'parent-task::TASK-1', 'unknown'], LABELS);
