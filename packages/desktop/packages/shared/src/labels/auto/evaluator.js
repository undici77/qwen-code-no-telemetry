 ...
 */
export function collectAutoLabelRules(labels) {
    const result = [];
    function walk(nodes) {
 ...
 */
export function evaluateAutoLabels(message, labels) {
    ...
    for (const { label, rule } of rules) {
        ...
        if (matches.length >= MAX_MATCHES_PER_MESSAGE)
            break;
 ...
 */
function evaluateRegexRule(message, label, rule) {
    const matches = [];
    try {
