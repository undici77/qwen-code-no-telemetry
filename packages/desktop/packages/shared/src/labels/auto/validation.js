 ...
 */
export function validateAutoLabelRule(pattern, flags) {
    const errors = [];
    const warnings = [];
    ...
    for (const badPattern of CATASTROPHIC_BACKTRACKING_PATTERNS) {
        if (badPattern.test(pattern)) {
            errors.push(`Pattern contains nested quantifiers which can cause catastrophic backtracking (ReDoS). ` +
    ...
    if (!pattern.includes('(') || pattern.replace(/\(\?[:<!=]/g, '').indexOf('(') === -1) {
        warnings.push('Pattern has no capture groups. The entire match will be used as the label value. ' +
