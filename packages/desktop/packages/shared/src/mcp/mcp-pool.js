...
import { saveBinaryResponse, detectExtensionFromMagic, sanitizeFilename, } from '../utils/binary-detection.ts';
/**
 * Convert SdkMcpServerConfig (used by backend types) to CraftMcpClient config.
 */
function sdkConfigToClientConfig(config) {
 ...
 */
function mcpConfigChanged(oldConfig, newConfig) {
    if (oldConfig.type !== newConfig.type)
        return true;
    if ((oldConfig.type === 'http' || oldConfig.type === 'sse') &&
...
export class McpClientPool {
     ...
     */
    async registerClient(slug, client) {
    ...
    async connect(slug, config) {
        ...
        if (!clientConfig) {
            this.debug(`Unknown MCP server type for ${slug}: ${config.type}`);
            return;
        }
     ...
     */
    async connectInProcess(slug, mcpServer) {
        if (this.clients.has(slug))
            return;
     ...
     */
    async sync(mcpServers, apiServers = {}) {
...
//# sourceMappingURL=mcp-pool.js.map