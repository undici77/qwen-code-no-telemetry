...
export function expandSessionPath(jsonLine, sessionDir) {
    if (!jsonLine.includes(SESSION_PATH_TOKEN))
        return jsonLine;
    return jsonLine.replaceAll(SESSION_PATH_TOKEN, normalizePath(sessionDir));
}
function normalizeHeaderPermissionModes(header) {
    delete header.permissionMode;
    delete header.previousPermissionMode;
    return header;
}
function normalizeSessionHeader(header) {
    delete header.model;
...
export function readSessionHeader(sessionFile) {
    try {
        ...
        const parsed = safeJsonParse(expandSessionPath(firstLine, dirname(sessionFile)));
 ...
 */
export function readSessionJsonl(sessionFile) {
    try {
        ...
        const header = normalizeSessionHeader(safeJsonParse(expandSessionPath(firstLine, sessionDir)));
 ...
 */
export function writeSessionJsonl(sessionFile, session) {
    const headerOptionsSource = session;
    const header = createSessionHeader(session, {
        omitMessageDerivedFields: headerOptionsSource.omitMessageDerivedHeaderFields,
        omitTranscriptDerivedFields: headerOptionsSource.omitTranscriptDerivedHeaderFields,
 ...
 */
export function createSessionHeader(session, options = {}) {
    const header = {
 ...
 */
function extractLastMessageRole(messages) {
 ...
 */
function extractLastFinalMessageId(messages) {
 ...
 */
export async function readSessionHeaderAsync(sessionFile) {
    try {
 ...
 */
function parseMessagesResilient(lines) {
    const messages = [];
