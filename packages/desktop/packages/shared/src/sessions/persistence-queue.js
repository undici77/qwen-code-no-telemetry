...
function getHeaderMetadataSignature(header) {
    const signature = {
        name: header.name,
...
function mergeHeaderWithExternalMetadata(localHeader, diskHeader) {
    return {
        ...localHeader,
...
class SessionPersistenceQueue {
    pending = new Map();
    writeInProgress = new Map();
    lastWrittenHeaderSignature = new Map();
    debounceMs;
    ...
    async write(sessionId) {
        ...
        try {
            const { data } = entry;
            const headerOptionsSource = data;
...
//# sourceMappingURL=persistence-queue.js.map