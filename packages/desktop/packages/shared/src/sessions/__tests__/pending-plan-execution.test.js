...
import { getPendingPlanExecution, markCompactionComplete, markPendingPlanExecutionDispatched, saveSession, setPendingPlanExecution, } from '../storage.ts';
function makeTmpDir() {
    const dir = join(tmpdir(), `pending-plan-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    mkdirSync(dir, { recursive: true });
    return dir;
}
function makeStoredSession(workspaceRootPath) {
    return {
        id: 'session-1',
        workspaceRootPath,
        createdAt: 1000,
...
//# sourceMappingURL=pending-plan-execution.test.js.map