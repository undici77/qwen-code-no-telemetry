 ...
 */
export function ensureSessionsDir(workspaceRootPath) {
 ...
 */
export function getSessionPath(workspaceRootPath, sessionId) {
 ...
 */
export function getSessionFilePath(workspaceRootPath, sessionId) {
 ...
 */
export function ensureSessionDir(workspaceRootPath, sessionId) {
 ...
 */
export function getSessionAttachmentsPath(workspaceRootPath, sessionId) {
    return join(getSessionPath(workspaceRootPath, sessionId), 'attachments');
}
/**
 * Get the plans directory for a session
 */
export function getSessionPlansPath(workspaceRootPath, sessionId) {
 ...
 */
export function getSessionDataPath(workspaceRootPath, sessionId) {
 ...
 */
export function getSessionDownloadsPath(workspaceRootPath, sessionId) {
 ...
 */
function getExistingSessionIds(workspaceRootPath) {
 ...
 * adjective-noun slug.
 */
export function generateSessionId(workspaceRootPath, slugHint) {
 ...
 */
export async function createSession(workspaceRootPath, options) {
 ...
 */
export async function getOrCreateSessionById(workspaceRootPath, sessionId) {
 ...
 */
export function loadSession(workspaceRootPath, sessionId) {
 ...
 */
export function listSessions(workspaceRootPath) {
    ...
    const sorted = sessions.sort((a, b) => (b.lastUsedAt ?? b.lastMessageAt ?? b.createdAt ?? 0) -
 ...
 */
function headerToMetadata(header, workspaceRootPath) {
    try {
        // Migration: accept old 'todoState' field from pre-rename session files
        const rawStatus = header.sessionStatus ?? header.todoState;
        ...
        const { enabledSourceSlugs: _es, pendingPlanExecution: _pp, sessionStatus: _ss, workingDirectory: _wd, sdkCwd: _sc, model: _model, workspaceRootPath: _wrp, ...headerFields } = header;
        return {
            ...headerFields,
            workspaceRootPath,
 ...
 */
export function deleteSession(workspaceRootPath, sessionId) {
    try {
 ...
 */
export async function clearSessionMessages(workspaceRootPath, sessionId) {
 ...
 */
export async function updateSessionSdkId(workspaceRootPath, sessionId, sdkSessionId) {
    const session = loadSession(workspaceRootPath, sessionId);
    if (session) {
 ...
 */
export function canUpdateSdkCwd(session) {
    // Safe to update if:
 ...
 */
export async function updateSessionMetadata(workspaceRootPath, sessionId, updates) {
    const session = loadSession(workspaceRootPath, sessionId);
    if (!session)
        return;
    if (updates.isFlagged !== undefined)
        session.isFlagged = updates.isFlagged;
    if (updates.name !== undefined)
        session.name = updates.name;
    if (updates.sessionStatus !== undefined)
        session.sessionStatus = updates.sessionStatus;
    if (updates.labels !== undefined)
        session.labels = updates.labels;
    if (updates.enabledSourceSlugs !== undefined)
        session.enabledSourceSlugs = updates.enabledSourceSlugs;
    if (updates.workingDirectory !== undefined)
        session.workingDirectory = updates.workingDirectory;
    if (updates.sdkCwd !== undefined)
        session.sdkCwd = updates.sdkCwd;
    if ('lastReadMessageId' in updates)
        session.lastReadMessageId = updates.lastReadMessageId;
    if ('hasUnread' in updates)
        session.hasUnread = updates.hasUnread;
    if ('sharedUrl' in updates)
        session.sharedUrl = updates.sharedUrl;
    if ('sharedId' in updates)
        session.sharedId = updates.sharedId;
    if ('model' in updates)
        session.model = updates.model;
    if (updates.llmConnection !== undefined)
        session.llmConnection = updates.llmConnection;
    if (updates.isArchived !== undefined)
        session.isArchived = updates.isArchived;
 ...
 * Flag a session
 */
export async function flagSession(workspaceRootPath, sessionId) {
    await updateSessionMetadata(workspaceRootPath, sessionId, { isFlagged: true });
}
/**
 * Unflag a session
 */
export async function unflagSession(workspaceRootPath, sessionId) {
    await updateSessionMetadata(workspaceRootPath, sessionId, { isFlagged: false });
}
/**
 * Set session status
 */
export async function setSessionStatus(workspaceRootPath, sessionId, sessionStatus) {
    await updateSessionMetadata(workspaceRootPath, sessionId, { sessionStatus });
}
/**
 * Set labels for a session
 */
export async function setSessionLabels(workspaceRootPath, sessionId, labels) {
    await updateSessionMetadata(workspaceRootPath, sessionId, { labels });
}
/**
 * Archive a session
 */
export async function archiveSession(workspaceRootPath, sessionId) {
 ...
 * Unarchive a session
 */
export async function unarchiveSession(workspaceRootPath, sessionId) {
    await updateSessionMetadata(workspaceRootPath, sessionId, {
        isArchived: false,
 ...
 */
export async function setPendingPlanExecution(workspaceRootPath, sessionId, planPath, draftInputSnapshot) {
    const session = loadSession(workspaceRootPath, sessionId);
    if (!session)
        return;
 ...
 */
export async function markCompactionComplete(workspaceRootPath, sessionId) {
    const session = loadSession(workspaceRootPath, sessionId);
    if (!session?.pendingPlanExecution)
        return;
 ...
 */
export async function markPendingPlanExecutionDispatched(workspaceRootPath, sessionId) {
    const session = loadSession(workspaceRootPath, sessionId);
    if (!session?.pendingPlanExecution)
        return;
 ...
 */
export async function clearPendingPlanExecution(workspaceRootPath, sessionId) {
    const session = loadSession(workspaceRootPath, sessionId);
    if (!session)
        return;
 ...
 */
export function getPendingPlanExecution(workspaceRootPath, sessionId) {
    const session = loadSession(workspaceRootPath, sessionId);
    if (!session?.pendingPlanExecution)
        return null;
    return {
 ...
 */
export function listFlaggedSessions(workspaceRootPath) {
 ...
 */
export function listCompletedSessions(workspaceRootPath) {
 ...
 * Excludes archived sessions
 */
export function listInboxSessions(workspaceRootPath) {
    return listActiveSessions(workspaceRootPath).filter(s => {
        const category = getStatusCategory(workspaceRootPath, s.sessionStatus || 'todo');
 ...
 */
export function listArchivedSessions(workspaceRootPath) {
    return listSessions(workspaceRootPath).filter(s => s.isArchived === true);
}
/**
 * List active (non-archived) sessions
 */
export function listActiveSessions(workspaceRootPath) {
    return listSessions(workspaceRootPath).filter(s => s.isArchived !== true);
}
/**
 ...
 */
export function deleteOldArchivedSessions(workspaceRootPath, retentionDays) {
 ...
 */
function generatePlanFileName(plan, plansDir) {
 ...
 */
function ensurePlansDir(workspaceRootPath, sessionId) {
 ...
 */
export function parsePlanFromMarkdown(content, planId) {
    try {
 ...
 */
export function savePlanToFile(workspaceRootPath, sessionId, plan, fileName) {
 ...
 */
export function loadPlanFromFile(workspaceRootPath, sessionId, fileName) {
    const plansDir = getSessionPlansPath(workspaceRootPath, sessionId);
 ...
 */
export function loadPlanFromPath(filePath) {
    if (!existsSync(filePath)) {
        return null;
    }
    try {
 ...
 */
export function listPlanFiles(workspaceRootPath, sessionId) {
    const plansDir = getSessionPlansPath(workspaceRootPath, sessionId);
    if (!existsSync(plansDir)) {
        return [];
    }
 ...
 * Delete a plan file
 */
export function deletePlanFile(workspaceRootPath, sessionId, fileName) {
    const plansDir = getSessionPlansPath(workspaceRootPath, sessionId);
    const filePath = join(plansDir, `${fileName}.md`);
    if (existsSync(filePath)) {
        unlinkSync(filePath);
 ...
 */
export function getMostRecentPlanFile(workspaceRootPath, sessionId) {
 ...
 */
export function ensureAttachmentsDir(workspaceRootPath, sessionId) {
