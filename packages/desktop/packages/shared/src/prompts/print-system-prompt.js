...
function printHeader(title, bgColor = colors.bgBlue) {
...
const systemPrompt = getSystemPrompt(undefined, // No pinned preferences (use current from disk)
...
const systemPromptWithDebug = getSystemPrompt(undefined, { enabled: true, logFilePath: '~/Library/Logs/@craft-agent/electron/main.log' }, '/Users/example/.craft-agent/workspaces/abc123');
...
const sessionState = formatSessionState('260121-example-session', {
    plansFolderPath: '/Users/example/.craft-agent/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printSection('4. WORKSPACE CAPABILITIES - formatWorkspaceCapabilities()', exampleCapabilities, colors.magenta);
printAnnotation('Shows whether local MCP stdio servers are enabled for the workspace');
// 5. Working Directory Context
const workingDirContext = getWorkingDirectoryContext('/Users/example/projects/my-app', false, // Not session root
undefined // No bash cwd mismatch
);
printSection('5. WORKING DIRECTORY - getWorkingDirectoryContext()', workingDirContext || '(empty - no working directory)', colors.magenta);
...
printSection('6. RECOVERY CONTEXT - buildRecoveryContext() [only on resume]', exampleRecoveryContext, colors.magenta);
...
`);
//# sourceMappingURL=print-system-prompt.js.map