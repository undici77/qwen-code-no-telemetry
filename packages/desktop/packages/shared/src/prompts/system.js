 ...
 */
function findFileCaseInsensitive(directory, pattern) {
    try {
 ...
 * Returns the actual filename if found, null otherwise.
 */
export function findProjectContextFile(directory) {
...
export function invalidateContextFileCache(directory) {
    if (directory) {
 ...
 */
export function findAllProjectContextFiles(directory) {
    // Check cache first
    const now = Date.now();
 ...
 */
export function readProjectContextFile(directory) {
    for (const pattern of CONTEXT_FILE_PATTERNS) {
 ...
 */
export function getWorkingDirectoryContext(workingDirectory, isSessionRoot, bashCwd) {
    if (!workingDirectory) {
        return '';
    }
    const parts = [];
 ...
 */
export function getProjectContextFilesPrompt(workingDirectory) {
    if (!workingDirectory) {
        return '';
    }
 ...
 */
export function getMiniAgentSystemPrompt(workspaceRootPath) {
 ...
 */
export function getSystemPrompt(pinnedPreferencesPrompt, debugMode, workspaceRootPath, workingDirectory, preset, backendName, includeCoAuthoredBy) {
 ...
 */
function formatDebugModeContext(logFilePath) {
    if (!logFilePath) {
        return '';
    }
    return `
 ...
 */
function getCraftAgentEnvironmentMarker() {
 ...
 */
function getCraftAssistantPrompt(workspaceRootPath, backendName = BRAND.selfReferName, includeCoAuthoredBy = true) {
