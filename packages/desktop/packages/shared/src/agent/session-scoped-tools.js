...
import { SESSION_BACKEND_TOOL_NAMES, SESSION_TOOL_REGISTRY, getSessionToolDefs, isPathInsideOrEqual, TOOL_DESCRIPTIONS as BASE_DESCRIPTIONS, } from '@craft-agent/session-tools-core';
...
export { registerSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, unregisterSessionScopedToolCallbacks, getSessionScopedToolCallbacks, } from './session-scoped-tool-callback-registry.ts';
...
export const BACKEND_SESSION_TOOL_NAMES = new Set([
    'call_llm',
    'spawn_session',
    'browser_tool',
]);
 ...
 */
function assertBackendSessionToolParity() {
    const missing = [...SESSION_BACKEND_TOOL_NAMES].filter((name) => !BACKEND_SESSION_TOOL_NAMES.has(name));
    if (missing.length > 0) {
        throw new Error(`Session tools missing backend adapter implementations: ${missing.join(', ')}`);
    }
}
// ============================================================
// Plan State Management
 ...
 */
export function getSessionPlansDir(workspacePath, sessionId) {
    return getSessionPlansPath(workspacePath, sessionId);
}
/**
 * Check if a path is within a session's plans directory
 */
export function isPathInPlansDir(path, workspacePath, sessionId) {
 ...
 */
export function invalidateAllSessionToolsCaches() {
    sessionToolsCache.clear();
}
/**
 * Clean up cached tools for a session
 */
export function cleanupSessionScopedTools(sessionId) {
 ...
 */
export function getSessionScopedTools(sessionId, workspaceRootPath, workspaceId) {
    ...
    let tools = sessionToolsCache.get(cacheKey);
    if (!tools) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        function registryTool(name, schema) {
            const def = SESSION_TOOL_REGISTRY.get(name);
            return localTool(name, TOOL_DESCRIPTIONS[name] || def.description, schema, async (args) => {
        ...
        tools.push(createSpawnSessionTool({
            sessionId,
            getSpawnSessionFn: () => {
                const callbacks = getSessionScopedToolCallbacks(sessionId);
...
//# sourceMappingURL=session-scoped-tools.js.map