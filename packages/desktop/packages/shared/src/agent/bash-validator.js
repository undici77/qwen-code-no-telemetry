 ...
 */
export function validateBashCommand(command, patterns) {
    // Parse the command into an AST
    let ast;
    try {
        ast = bashParser(command);
    }
    catch (error) {
...
function validateNode(node, patterns, results) {
    switch (node.type) {
        case 'Script':
            return validateScript(node, patterns, results);
        case 'Command':
            return validateCommand(node, patterns, results);
        case 'LogicalExpression':
            return validateLogicalExpression(node, patterns, results);
        case 'Pipeline':
        ...
        case 'Subshell':
            return validateSubshell(node, patterns, results);
        case 'CompoundList':
            return validateCompoundList(node, patterns, results);
        default:
 ...
 */
function validateScript(node, patterns, results) {
        ...
        for (const item of node.prefix) {
            ...
            if (item.type === 'AssignmentWord') {
                return {
                    ...
                    reason: {
                        type: 'env_assignment',
                        explanation: `Environment variable assignment "${item.text}" modifies command behavior (e.g., PATH hijacking, LD_PRELOAD injection)`,
                    },
                };
            }
        }
 ...
 */
function validateLogicalExpression(node, patterns, results) {
    // Validate left side
 ...
 */
function checkWordForExpansions(word) {
    if (!word.expansion) {
        return null;
    }
 ...
 */
function isRedirectSafe(redirect) {
 ...
 */
function getRedirectExplanation(op) {
    const explanations = {
 ...
 */
export function hasControlCharacters(command) {
    const dangerous = {
...
//# sourceMappingURL=bash-validator.js.map