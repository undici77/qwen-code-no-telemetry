...
export const MINI_AGENT_TOOLS = ['Read', 'Edit', 'Write', 'Glob', 'Grep', 'Bash'];
/** MCP servers for mini agents - minimal set (docs tools are now bundled in session) */
export const MINI_AGENT_MCP_KEYS = ['session'];
// ============================================================
// BaseAgent Abstract Class
// ============================================================
/**
...
export class BaseAgent {
    ...
    _supportsBranching = true;
    get supportsBranching() { return this._supportsBranching; }
    // ============================================================
    ...
    // ============================================================
    _pendingSourceActivationRestart = null;
    _currentTurnUserMessage = null;
    ...
    setCurrentTurnUserMessage(message) {
    ...
    onAuthRequest = null;
    onSourceChange = null;
    onSourcesListChange = null;
    onConfigValidationError = null;
    ...
    // Constructor
    // ============================================================
    constructor(config, defaultModel, contextWindow) {
        this.config = config;
     ...
     */
    async emitAutomationEvent(event, input, signal) {
        try {
     ...
     */
    handleSessionMcpToolCompletion(toolName, args) {
        ...
        if (toolName === 'SubmitPlan' && args.planPath) {
            ...
            this.onPlanSubmitted?.(args.planPath);
            return;
        }
    ...
    async setSourceServers(mcpServers, apiServers, intendedSlugs) {
        ...
        this.sourceManager.updateActiveState(Object.keys(mcpServers), Object.keys(apiServers), intendedSlugs);
     ...
     */
    setTemporaryClarifications(text) {
     ...
     */
    filterMcpServersForMiniAgent(servers, allowedKeys) {
        const filtered = {};
...
`;
     ...
     */
    buildBranchSeedContext(messages) {
        if (!messages || messages.length === 0)
            return null;
...
${formattedMessages}
     ...
     */
    async applyBridgeUpdates(_context) {
        // No-op by default
    }
    /**
     ...
     */
    formatSkillDirective(skillPaths) {
     ...
     */
    async preExecuteSpawnSession(input) {
     ...
     */
    async generateTitle(message, options) {
        try {
     ...
     * @returns Generated title (2-5 words), or null if generation fails
     */
    async regenerateTitle(recentUserMessages, lastAssistantResponse, options) {
        try {
