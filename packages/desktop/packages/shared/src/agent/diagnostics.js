...
async function withTimeout(promise, timeoutMs, defaultValue) {
    const timeoutPromise = new Promise((resolve) => setTimeout(() => resolve(defaultValue), timeoutMs));
    return Promise.race([promise, timeoutPromise]);
}
async function checkCapturedApiError(providerLabel) {
...
export async function runErrorDiagnostics(config) {
    const providerLabel = config.providerType === 'qwen' ? 'Qwen Code' : 'Backend';
    const details = [];
    const defaultResult = { ok: true, detail: 'Check timed out' };
