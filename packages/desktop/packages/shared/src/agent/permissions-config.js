...
import { SAFE_MODE_CONFIG, PermissionsConfigSchema, } from './mode-types.ts';
// ============================================================
 ...
 */
export function getAppPermissionsDir() {
...
export function ensureDefaultPermissions() {
    ...
    try {
        ...
        const installed = safeJsonParse(installedContent);
        const bundled = safeJsonParse(bundledContent);
 ...
 */
function migratePermissions(installed, bundled) {
    // Get existing pattern strings for deduplication
    const getPatternString = (p) => typeof p === 'string' ? p : p.pattern;
    const existingBashPatterns = new Set((installed.allowedBashPatterns || []).map(getPatternString));
    const existingMcpPatterns = new Set((installed.allowedMcpPatterns || []).map(getPatternString));
    // Find new patterns not already in installed
    const newBashPatterns = (bundled.allowedBashPatterns || []).filter(p => !existingBashPatterns.has(getPatternString(p)));
    const newMcpPatterns = (bundled.allowedMcpPatterns || []).filter(p => !existingMcpPatterns.has(getPatternString(p)));
    // Merge blocked command hints (dedupe by command + whenNotMatching + reason)
    const installedHints = installed.blockedCommandHints || [];
    const installedHintKeys = new Set(installedHints.map(h => `${h.command}::${h.whenNotMatching || ''}::${h.reason}`));
    const newBlockedCommandHints = (bundled.blockedCommandHints || []).filter(h => !installedHintKeys.has(`${h.command}::${h.whenNotMatching || ''}::${h.reason}`));
...
export { PermissionsConfigSchema, };
// ============================================================
// JSON Parser
// ============================================================
/**
 ...
 */
export function parsePermissionsJson(content) {
    const emptyConfig = {
        allowedBashPatterns: [],
        allowedMcpPatterns: [],
        allowedApiEndpoints: [],
    ...
    try {
        ...
        const normalizeBashPatterns = (patterns) => {
            if (!patterns)
                return [];
...
function compileBlockedCommandHint(hint) {
    const command = hint.command.trim().toLowerCase();
    if (!command)
        return null;
    let whenNotMatchingRegex;
...
function shouldCompileBashPattern(pattern) {
 ...
 */
export function validatePermissionsConfig(config) {
    const errors = [];
    // Validate regex patterns
    const checkPatterns = (patterns, name) => {
        if (!patterns)
            return;
 ...
 */
export function getWorkspacePermissionsPath(workspaceRootPath) {
    return join(workspaceRootPath, 'permissions.json');
}
/**
 * Get path to source permissions.json
 */
export function getSourcePermissionsPath(workspaceRootPath, sourceSlug) {
    return join(getSourcePath(workspaceRootPath, sourceSlug), 'permissions.json');
}
/**
 * Load workspace-level permissions config
 */
export function loadWorkspacePermissionsConfig(workspaceRootPath) {
 ...
 */
export function loadSourcePermissionsConfig(workspaceRootPath, sourceSlug) {
    try {
 ...
 */
export function loadRawWorkspacePermissions(workspaceRootPath) {
 ...
 * Returns null if the file doesn't exist.
 */
export function loadRawSourcePermissions(workspaceRootPath, sourceSlug) {
    try {
 ...
 */
export function saveWorkspacePermissions(workspaceRootPath, config) {
    const filePath = getWorkspacePermissionsPath(workspaceRootPath);
 ...
 */
export function saveSourcePermissions(workspaceRootPath, sourceSlug, config) {
    const filePath = getSourcePermissionsPath(workspaceRootPath, sourceSlug);
 ...
 */
export function isApiEndpointAllowed(method, path, config) {
...
class PermissionsConfigCache {
    ...
    defaultConfig = undefined; // undefined = not loaded yet
    /**
     ...
     */
    getWorkspaceConfig(workspaceRootPath) {
     ...
     */
    getSourceConfig(workspaceRootPath, sourceSlug) {
     ...
     */
    invalidateWorkspace(workspaceRootPath) {
     ...
     */
    invalidateSource(workspaceRootPath, sourceSlug) {
     ...
     */
    applyDefaultConfig(merged, config) {
    ...
    applyCustomConfig(merged, custom) {
     ...
     */
    applySourceConfig(merged, custom, sourceSlug) {
