 ...
 */
export async function buildCallLlmRequest(input, options) {
    ...
    if (attachments?.length) {
        for (let i = 0; i < attachments.length; i++) {
            ...
            if (result.type === 'image') {
                throw new Error(`Attachment ${i + 1}: Image attachments are not supported in ${options.backendName} mode. Use text files only.`);
            }
            if (result.type === 'text') {
    ...
    else if (outputFormat && OUTPUT_FORMATS[outputFormat]) {
        schema = OUTPUT_FORMATS[outputFormat];
    }
    if (schema) {
...
// ============================================================================
function summarizeSection(lines, start, end) {
...
export async function processAttachment(input, index, basePath) {
    // Normalize input to AttachmentInput format
    const attachment = typeof input === 'string'
        ? { path: input }
        : input;
...
export function createLLMTool(options) {
    ...
    return localTool('call_llm', `Invoke a secondary LLM for focused subtasks. Use for:
...
For large files (>2000 lines), use {path, startLine, endLine} to select a portion.`, {
        ...
        if (args.outputFormat && args.outputSchema) {
            return errorResponse('Cannot use both outputFormat and outputSchema.\n\n' +
                'Options:\n' +
            ...
            for (let i = 0; i < args.attachments.length; i++) {
                ...
                if (result.type === 'text') {
                    ...
                    if (totalContentBytes > MAX_TOTAL_CONTENT_BYTES) {
                        return errorResponse(`Total attachment size exceeds ${MAX_TOTAL_CONTENT_BYTES / 1_000_000}MB limit.\n\n` +
                            'Options:\n' +
        ...
        try {
            const result = await queryFn({
                ...
                outputSchema: schema ? schema : undefined,
            });
            if (!result.text && !result.warning) {
                return { content: [{ type: 'text', text: '(Model returned empty response)' }] };
            }
...
//# sourceMappingURL=llm-tool.js.map