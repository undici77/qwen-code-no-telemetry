 ...
 */
export function parseReadCommand(command) {
    ...
    if (looksLikePowerShell(command)) {
        ...
        if (filePath)
            return { filePath, originalCommand: command };
    }
    try {
 ...
 */
function extractSimpleCommand(ast) {
 ...
 */
function parseDirectReadCommand(cmd, original) {
    const name = cmd.name?.text;
    if (!name || !READ_COMMANDS.has(name))
        return null;
    const args = getArgs(cmd);
    switch (name) {
        case 'cat':
 ...
 */
function parseCatCommand(args, original) {
 ...
 */
function parseSedCommand(args, original) {
    const flag = args[0];
 ...
 * - head -50 file.ts
 */
function parseHeadCommand(args, original) {
    const firstArg = args[0];
 ...
 */
function parseTailCommand(args, original) {
    const firstArg = args[0];
    const secondArg = args[1];
    const thirdArg = args[2];
    // tail -n 50 file.ts
...
//# sourceMappingURL=read-patterns.js.map