export function getBrowserToolHelp() {
    return [
...
function formatNodeLine(node, options) {
...
async function safeEvaluate(fns, expression) {
    try {
...
async function getPageMetrics(fns) {
    return safeEvaluate(fns, `(() => {
...
function describeActive(metrics) {
    if (!metrics)
        return 'unknown';
...
function countActionableNodes(nodes) {
...
function getOpenVisibilitySettleTimeoutMs(override) {
...
function getOpenVisibilitySettlePollMs(override) {
    if (typeof override === 'number' && Number.isFinite(override)) {
...
function formatLifecycleResultLine(result) {
...
function decodeEscapes(input) {
    return input.replace(/\\(.)/g, (_match, ch) => {
        if (ch === 'n')
            return '\n';
        if (ch === 't')
            return '\t';
...
function splitBatchCommands(input) {
    const commands = [];
    let current = '';
    let inSingle = false;
    let inDouble = false;
...
function parseSelectCommand(parts) {
    const ref = parts[1];
    if (!ref)
        throw new Error('select requires ref and value. Example: select @e3 optionValue');
    ...
    for (let i = 0; i < tokens.length; i += 1) {
        ...
        if (token === '--assert-text') {
            ...
            if (!next)
                throw new Error('select --assert-text requires a value. Example: select @e3 CNAME --assert-text Target');
            assertText = next;
            i += 1;
            continue;
        }
        if (token === '--assert-value') {
            const next = tokens[i + 1];
            if (!next)
                throw new Error('select --assert-value requires a value. Example: select @e3 CNAME --assert-value CNAME');
            assertValue = next;
            i += 1;
            continue;
        }
        if (token === '--timeout') {
            const next = tokens[i + 1];
            if (!next)
                throw new Error('select --timeout requires milliseconds. Example: select @e3 CNAME --timeout 3000');
            const parsed = Number(next);
            if (Number.isNaN(parsed))
                throw new Error(`Invalid select timeout "${next}". Expected a number.`);
...
async function verifySelectResult(args) {
    ...
    while (Date.now() - started <= timeoutMs) {
        ...
        if (assertText) {
            assertTextMatched = snapshot.nodes.some((n) => includesNormalized(n.name, assertText)
        ...
        if (assertValue) {
            ...
            const anyNodeMatches = snapshot.nodes.some((n) => includesNormalized(n.value, assertValue)
...
async function executeSingleCommand(args) {
    ...
    if (cmd === 'open') {
        ...
        if (win) {
            lines.push(`Visible: ${win.isVisible}, ownerType: ${win.ownerType}, boundSessionId: ${win.boundSessionId ?? 'none'}`);
        }
    ...
    if (cmd === 'navigate') {
        ...
        if (!url)
            throw new Error('navigate requires a URL. Example: navigate https://example.com');
    ...
    if (cmd === 'find') {
        ...
        if (!query)
            throw new Error('find requires a search query. Example: find login button');
        const snapshot = await fns.snapshot();
        ...
        const scored = snapshot.nodes
            ...
            for (const kw of keywords) {
                if ((node.name ?? '').toLowerCase().includes(kw))
                    score += 3;
                if ((node.role ?? '').toLowerCase().includes(kw))
                    score += 2;
                if ((node.value ?? '').toLowerCase().includes(kw))
                    score += 1;
                if ((node.description ?? '').toLowerCase().includes(kw))
                    score += 1;
            }
            return { node, score };
        })
    ...
    if (cmd === 'click') {
        const ref = parts[1];
        if (!ref)
            throw new Error('click requires a ref. Example: click @e1');
        const waitForRaw = parts[2];
    ...
    if (cmd === 'click-at') {
        ...
        const yRaw = parts[2];
        if (!xRaw || !yRaw)
            throw new Error('click-at requires x and y coordinates. Example: click-at 350 200');
        const x = Number(xRaw);
        const y = Number(yRaw);
    ...
    if (cmd === 'fill') {
        ...
        if (!ref || value === undefined)
            throw new Error('fill requires ref and value. Example: fill @e5 hello world');
        const before = await getPageMetrics(fns);
    ...
    if (cmd === 'type') {
        ...
        if (!text)
            throw new Error('type requires text. Example: type Hello World');
        const before = await getPageMetrics(fns);
        await fns.type(text);
        const after = await getPageMetrics(fns);
        return {
    ...
    if (cmd === 'set-clipboard') {
        const text = parts.slice(1).join(' ');
        if (!text)
            throw new Error('set-clipboard requires text. Example: set-clipboard Hello World');
    ...
    if (cmd === 'paste') {
        const text = parts.slice(1).join(' ');
        if (!text)
            throw new Error('paste requires text. Example: paste Hello World');
        await fns.setClipboard(text);
    ...
    if (cmd === 'screenshot') {
        ...
        const mimeType = result.imageFormat === 'jpeg' ? 'image/jpeg' : 'image/png';
        const lines = [
            annotate
    ...
    if (cmd === 'screenshot-region') {
        ...
        if (cleaned[0] === '--ref') {
            const ref = cleaned[1];
            if (!ref)
                throw new Error('screenshot-region --ref requires a ref value.');
            ...
            if (!selector)
                throw new Error('screenshot-region --selector requires a CSS selector value.');
    ...
    if (cmd === 'window-resize') {
        ...
        if (!widthRaw || !heightRaw)
            throw new Error('window-resize requires width and height. Example: window-resize 1280 720');
        const width = Number(widthRaw);
        const height = Number(heightRaw);
    ...
    if (cmd === 'wait') {
        ...
        if (kind === 'network-idle') {
            ...
            value = parts[2];
            if (!value)
                throw new Error(`wait ${kind} requires a value.`);
            const timeoutRaw = parts[3];
            timeoutMs = timeoutRaw ? Number(timeoutRaw) : undefined;
            if (timeoutRaw && Number.isNaN(timeoutMs)) {
                throw new Error(`Invalid wait timeout "${timeoutRaw}". Expected a number.`);
    ...
    if (cmd === 'key') {
        const key = parts[1];
        if (!key)
            throw new Error('key requires key value. Example: key Enter');
        const modifiers = (parts[2] ? parts[2].split('+') : []);
    ...
    if (cmd === 'downloads') {
        ...
        for (const entry of entries) {
            ...
            lines.push(`[${new Date(entry.timestamp).toISOString()}] [${entry.state}] ${entry.filename} (${formatBytes(entry.bytesReceived)}/${formatBytes(entry.totalBytes)})${savePathSuffix}`);
        }
        return { output: lines.join('\n'), appendReleaseHint: true };
    }
    if (cmd === 'scroll') {
    ...
    if (cmd === 'evaluate') {
        ...
        if (!expression)
            throw new Error('evaluate requires an expression. Example: evaluate document.title');
    ...
    if (cmd === 'windows') {
        ...
        const lines = [`Browser windows (${windows.length}) — ${summarizeWindows(windows)}, availableToSession=${available}`];
        for (const w of windows) {
            ...
            lines.push('', `- ${w.id}`, `  title: ${w.title || 'New Tab'}`, `  url: ${w.url || 'about:blank'}`, `  visible: ${w.isVisible}`, `  ownerType: ${w.ownerType}`, `  ownerSessionId: ${w.ownerSessionId ?? 'none'}`, `  boundSessionId: ${w.boundSessionId ?? 'none'}`, `  lockState: ${lockState}`, `  availableToSession: ${availableToSession}`, `  agentControlActive: ${!!w.agentControlActive}`);
        }
        return { output: lines.join('\n'), appendReleaseHint: false };
    }
    if (cmd === 'release') {
...
//# sourceMappingURL=browser-tool-runtime.js.map