...
function extractErrorMessages(error) {
    ...
    if (error instanceof Error) {
        ...
        if (typeof anyError.stdout === 'string')
            messages.push(anyError.stdout);
        if (typeof anyError.stderr === 'string')
            messages.push(anyError.stderr);
        if (typeof anyError.output === 'string')
            messages.push(anyError.output);
    }
    else {
...
const HTML_DOC_HINTS = ['<html', '<!doctype html', '<head', '<body', '<title', '<h1'];
...
function looksLikeHtmlPayload(textLower) {
...
function hasHtmlErrorPageSignals(textLower) {
    const hasKnownHttpTitle = textLower.includes('bad request') ||
...
function isLikelyProxyInterception(textLower) {
...
function buildProxyErrorMessage(errorMessage, fullErrorText) {
 ...
 */
export function parseError(error, providerContext) {
    ...
    else if (lowerMessage.includes('no endpoints found that support tool use') ||
    ...
    const providerInfo = providerContext
        ? getProviderMetadata(providerContext.providerType ?? 'qwen') ?? undefined
        : undefined;
 ...
 */
export function parseSDKErrorText(text) {
 ...
 */
export function isSDKErrorText(text) {
