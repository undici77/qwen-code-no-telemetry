...
import { registerSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, getSessionScopedToolCallbacks, unregisterSessionScopedToolCallbacks, } from '../session-scoped-tools.ts';
describe('session-scoped tool callback merge', () => {
    ...
    it('preserves existing browserPaneFns when merging turn-level callbacks', () => {
        const browserPaneFns = {
            ...
            screenshot: async () => ({ imageBuffer: Buffer.from('png'), imageFormat: 'png' }),
            screenshotRegion: async () => ({ imageBuffer: Buffer.from('png'), imageFormat: 'png' }),
            getConsoleLogs: async () => [],
            windowResize: async () => ({ width: 1280, height: 720 }),
            getNetworkLogs: async () => [],
            waitFor: async () => ({ ok: true, kind: 'network-idle', elapsedMs: 0, detail: 'ok' }),
            sendKey: async () => { },
            getDownloads: async () => [],
            upload: async () => { },
            scroll: async () => { },
            ...
            releaseControl: async () => ({ action: 'released', affectedIds: [] }),
            closeWindow: async () => ({ action: 'closed', affectedIds: [] }),
            hideWindow: async () => ({ action: 'hidden', affectedIds: [] }),
            listWindows: async () => [],
...
//# sourceMappingURL=session-scoped-tools-merge.test.js.map