...
function createAgent(cwd, onSdkSessionIdUpdate, onMidTurnMessagesDrained) {
...
function writeQwenTranscript(runtimeRoot, cwd, sessionId, records) {
    const projectId = resolve(cwd).replace(/[^a-zA-Z0-9]/g, '-');
    const transcriptDir = join(runtimeRoot, 'projects', projectId, 'chats');
    mkdirSync(transcriptDir, { recursive: true });
    writeFileSync(join(transcriptDir, `${sessionId}.jsonl`), records.map((record) => JSON.stringify(record)).join('\n') + '\n');
}
function readQwenTranscript(runtimeRoot, cwd, sessionId) {
    const projectId = resolve(cwd).replace(/[^a-zA-Z0-9]/g, '-');
...
async function readNextQueuedEvent(agent) {
    const queue = agent.eventQueue;
...
describe('QwenAgent slash command history', () => {
    ...
    it('projects Qwen user transcript records without hook context', () => {
        const extractQwenRecordText = QwenAgent.prototype.extractQwenRecordText;
        const hookContext = '<qwen:user-prompt-submit-context>\ntrusted context\n</qwen:user-prompt-submit-context>';
        expect(extractQwenRecordText({
            type: 'user',
            message: {
    ...
    it('sends slash commands as raw ACP prompts', () => {
        const blocks = QwenAgent.prototype.buildPromptBlocks('  /context  ');
        expect(blocks).toEqual([{ type: 'text', text: '/context' }]);
    });
    it('starts Qwen ACP with the desktop channel', () => {
        const command = QwenAgent.prototype.buildSpawnCommand('/opt/qwen/dist/cli.js', '/usr/local/bin/node');
    ...
    it('does not prepend Craft context to Qwen prompts while disabled', () => {
        const blocks = QwenAgent.prototype.buildPromptBlocks('hello');
    ...
    it('logs attachments skipped while building prompt blocks', () => {
        ...
        const blocks = agent.buildPromptBlocks('hello', [attachment]);
        expect(blocks).toEqual([{ type: 'text', text: 'hello' }]);
        expect(debugMessages).toContain('[QwenAgent] Skipping attachment empty.bin while building prompt blocks: no readable content');
        agent.destroy();
    });
    it('drains queued mid-turn messages through the ACP extension handler', async () => {
        ...
        expect(agent.enqueueMidTurnMessage('please also inspect tests', undefined, {
            messageId: 'queued-1',
        })).toBe(true);
        await expect(internals.handleExtMethod('craft/drainMidTurnQueue', {
        ...
        expect(agent.enqueueMidTurnMessage('and summarize findings', undefined, {
            messageId: 'queued-2',
        })).toBe(true);
        await expect(internals.handleExtMethod('craft/drainMidTurnQueue', {
    ...
    it('claims Todo Stop Guard continuations only for the current session owner', async () => {
        ...
        const internals = agent;
        internals.qwenSessionId = 'sdk-session-qwen';
        await expect(internals.handleExtMethod('craft/claimTodoStopGuardContinuation', {
            sessionId: 'sdk-session-qwen',
        })).resolves.toEqual({
            claimed: true,
            hasQueuedPrompt: false,
    ...
    it('acknowledges metadata-free image-only mid-turn messages by empty text', async () => {
        const cwd = mkdtempSync(join(tmpdir(), 'qwen-cwd-'));
        tempRoots.push(cwd);
        const onMidTurnMessagesDrained = mock((_messageIds) => { });
        const agent = createAgent(cwd, undefined, onMidTurnMessagesDrained);
        const internals = agent;
        internals.qwenSessionId = 'sdk-session-qwen';
        internals._isProcessing = true;
    ...
    it('drains queued mid-turn image attachments as ACP content blocks', async () => {
        ...
        expect(agent.enqueueMidTurnMessage('please inspect this image', [attachment], {
    ...
    it('retries and falls back when mid-turn attachment messages fail to build', async () => {
        ...
        const agent = createAgent(cwd, undefined, onMidTurnMessagesDrained);
        const internals = agent;
        const promptInternals = agent;
        const originalBuildPromptBlocks = promptInternals.buildPromptBlocks.bind(agent);
        ...
        expect(agent.enqueueMidTurnMessage('bad image', [attachment], {
            messageId: 'bad-image',
        })).toBe(true);
        expect(agent.enqueueMidTurnMessage('good image', [attachment], {
    ...
    it('acknowledges image-only mid-turn messages by optimistic id', async () => {
        ...
        expect(agent.enqueueMidTurnMessage('', [attachment], {
    ...
    it('drains mixed text and image mid-turn messages as ACP items', async () => {
        ...
        expect(agent.enqueueMidTurnMessage('first text only', undefined, {
            messageId: 'queued-text',
        })).toBe(true);
        expect(agent.enqueueMidTurnMessage('then inspect image', [attachment], {
            messageId: 'queued-image',
        })).toBe(true);
        await expect(internals.handleExtMethod('craft/drainMidTurnQueue', {
            sessionId: 'sdk-session-qwen',
    ...
    it('adds only matched slash command invocations when results produce output', () => {
        ...
        const messages = agent.mergeSlashCommandInvocationMessages(sessionId, acpMessages, cwd);
        agent.destroy();
    ...
    it('stops orphan result lookup at its user turn while preserving multi-hop results', () => {
        ...
        const agent = createAgent(cwd);
        const messages = agent.mergeSlashCommandInvocationMessages(sessionId, [], cwd);
        agent.destroy();
        expect(messages.map((message) => [
            message.role,
            message.content,
    ...
    it('does not derive text elements from Qwen user history without metadata', () => {
        ...
        tempRoots.push(cwd);
        const agent = createAgent(cwd);
        const messages = agent.buildHistoryMessages('session-with-files', [
            {
    ...
    it('marks replayed pre-tool assistant text as commentary, not thought', () => {
        ...
        tempRoots.push(cwd);
        const agent = createAgent(cwd);
        const messages = agent.buildHistoryMessages('session-with-commentary', [
            {
    ...
    it('records slash command text elements through ACP without rewriting the transcript', async () => {
        ...
        agent.callAcp = async (_method, execute) => await execute({ extMethod });
        await agent.persistQwenTranscriptTextElements(sessionId, cwd, [
            {
                type: 'slash_command',
    ...
    it('keeps text-element persistence and replay on the pre-cd transcript root', async () => {
        ...
        const extMethod = mock(async () => ({}));
        internals.callAcp = async (_method, execute) => await execute({ extMethod });
        ...
        ], sessionId, logicalCwd);
        agent.destroy();
        expect(extMethod).toHaveBeenCalledWith('qwen/session/recordTextElements', expect.objectContaining({ sessionId, content: '/qc-helper' }));
    ...
    it('does not complete a newer turn when prior text-element persistence resumes', async () => {
        ...
        internals.callAcp = (async (method) => {
    ...
    it('records skill text elements through ACP', async () => {
        ...
        agent.destroy();
        expect(extMethod).toHaveBeenCalledWith('qwen/session/recordTextElements', expect.objectContaining({
            sessionId,
            content: '@qc-helper',
            textElements: [
                {
    ...
    it('loads text elements back from the Qwen transcript', () => {
        ...
        const agent = createAgent(cwd);
        const messages = agent.applyQwenTranscriptTextElements([
            {
                id: 'message-1',
                role: 'user',
                content: '@qc-helper',
    ...
    it('does not send Craft context while Qwen prompt context is disabled', () => {
        ...
        tempRoots.push(cwd);
        const agent = createAgent(cwd);
        const blocks = agent.buildPromptBlocks('Fix session names');
        agent.destroy();
    ...
    it('returns available commands captured while loading Qwen history', async () => {
        ...
        expect(result.messages.map((message) => [message.role, message.content])).toEqual([['user', 'hello']]);
    });
    ...
    it('does not re-add hook-expanded transcript users after ACP history replay', async () => {
        ...
        const internals = agent;
        internals.ensureProcess = async () => { };
        internals.callAcp = async (_method, execute) => execute({
            extMethod: async () => ({
                updates: [
                    {
                        sessionUpdate: 'user_message_chunk',
        ...
        agent.destroy();
        expect(result.messages.map((message) => [message.role, message.content])).toEqual([['user', 'original prompt']]);
    });
    ...
    it('closes dangling Qwen transcript tool calls as terminal errors', async () => {
        ...
        expect(result.messages.some((message) => message.role === 'info')).toBe(false);
    });
    it('supplements Qwen history with transcript subagent telemetry', async () => {
        ...
        const result = await agent.loadSessionMessages(sessionId, { cwd });
        agent.destroy();
        const parent = result.messages.find((message) => message.toolUseId === parentToolUseId);
        const child = result.messages.find((message) => message.toolUseId === 'child-read-1');
    ...
    it('shares concurrent Qwen ACP process startup for one agent instance', async () => {
        ...
        let startCalls = 0;
        let releaseStart;
        const startStarted = new Promise((resolve) => {
...
//# sourceMappingURL=qwen-agent-slash-history.test.js.map