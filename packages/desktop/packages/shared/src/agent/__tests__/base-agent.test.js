...
import { TestAgent, createMockBackendConfig, createMockSource, collectEvents, } from './test-utils.ts';
describe('BaseAgent', () => {
    let agent;
    beforeEach(() => {
    ...
    describe('Source Management', () => {
        ...
        it('should track source servers', async () => {
            await agent.setSourceServers({ 'source-1': { type: 'http', url: 'http://test' } }, { 'source-2': {} }, ['source-1', 'source-2']);
        ...
        it('should check if source is active', async () => {
            await agent.setSourceServers({ 'active-source': { type: 'http', url: 'http://test' } }, {}, ['active-source']);
