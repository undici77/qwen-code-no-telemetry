...
function withDeveloperFeedbackEnabled(run) {
...
describe('session tool safe-mode classification', () => {
    ...
    it('allows developer feedback in safe mode when the feature is enabled', () => {
        withDeveloperFeedbackEnabled(() => {
            const result = shouldAllowToolInMode('mcp__session__send_developer_feedback', {}, 'safe');
            expect(result.allowed).toBe(true);
        });
    });
...
//# sourceMappingURL=session-tool-safe-mode-permissions.test.js.map