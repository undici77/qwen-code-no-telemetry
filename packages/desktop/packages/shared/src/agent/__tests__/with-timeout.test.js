...
describe('withTimeout()', () => {
    ...
    it('rejects with timeout message when promise takes too long', async () => {
        ...
        await expect(withTimeout(neverResolves, 50, 'custom timeout message')).rejects.toThrow('custom timeout message');
    });
    ...
    it('clears timer after resolution (no leaked timer)', async () => {
        ...
        const quick = new Promise((resolve) => setTimeout(resolve, 10));
