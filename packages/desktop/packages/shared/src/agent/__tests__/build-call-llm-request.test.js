 ...
 * used by backend PreToolUse adapters.
 *
...
describe('buildCallLlmRequest()', () => {
    ...
    it('throws on empty prompt', async () => {
        await expect(buildCallLlmRequest({ prompt: '' }, { backendName: 'Test' })).rejects.toThrow('Prompt is required');
    });
    it('throws on whitespace-only prompt', async () => {
        await expect(buildCallLlmRequest({ prompt: '   ' }, { backendName: 'Test' })).rejects.toThrow('Prompt is required');
    });
    it('throws on missing prompt', async () => {
        await expect(buildCallLlmRequest({}, { backendName: 'Test' })).rejects.toThrow('Prompt is required');
    });
    // --- Basic request ---
    it('builds request from prompt-only input', async () => {
        const result = await buildCallLlmRequest({ prompt: 'Summarize this' }, { backendName: 'Test' });
        expect(result.prompt).toBe('Summarize this');
        expect(result.systemPrompt).toBeUndefined();
        expect(result.model).toBeUndefined();
    });
    it('passes through systemPrompt', async () => {
        const result = await buildCallLlmRequest({ prompt: 'test', systemPrompt: 'Be concise' }, { backendName: 'Test' });
        expect(result.systemPrompt).toContain('Be concise');
    });
    it('passes through maxTokens and temperature', async () => {
        const result = await buildCallLlmRequest({ prompt: 'test', maxTokens: 1000, temperature: 0.5 }, { backendName: 'Test' });
        expect(result.maxTokens).toBe(1000);
        expect(result.temperature).toBe(0.5);
    });
    // --- Attachments ---
    it('processes text file attachments', async () => {
        const result = await buildCallLlmRequest({ prompt: 'Summarize', attachments: [join(TMP_DIR, 'test.ts')] }, { backendName: 'Test' });
    ...
    it('rejects image attachments', async () => {
        await expect(buildCallLlmRequest({ prompt: 'test', attachments: [join(TMP_DIR, 'image.png')] }, { backendName: 'Codex' })).rejects.toThrow('Image attachments are not supported in Codex mode');
    });
    it('rejects missing file attachments', async () => {
        await expect(buildCallLlmRequest({ prompt: 'test', attachments: ['/nonexistent/file.ts'] }, { backendName: 'Test' })).rejects.toThrow('File not found');
    });
    // --- Schema injection ---
    it('injects outputFormat schema into systemPrompt', async () => {
        const result = await buildCallLlmRequest({ prompt: 'test', outputFormat: 'summary' }, { backendName: 'Test' });
    ...
    it('injects custom outputSchema into systemPrompt', async () => {
        ...
        const result = await buildCallLlmRequest({ prompt: 'test', outputSchema: schema }, { backendName: 'Test' });
        expect(result.systemPrompt).toContain('"name"');
        expect(result.systemPrompt).toContain('JSON');
    });
    it('appends schema to existing systemPrompt', async () => {
        const result = await buildCallLlmRequest({ prompt: 'test', systemPrompt: 'Custom instructions', outputFormat: 'classification' }, { backendName: 'Test' });
    ...
    it('does not include outputSchema when no schema specified', async () => {
        const result = await buildCallLlmRequest({ prompt: 'test' }, { backendName: 'Test' });
        expect(result.outputSchema).toBeUndefined();
    });
    // --- validateModel callback ---
    it('calls validateModel with resolved model ID', async () => {
        let receivedModel = '';
        await buildCallLlmRequest({ prompt: 'test', model: 'some-model-id' }, {
            backendName: 'Test',
    ...
    it('clears model when validateModel returns undefined', async () => {
        const result = await buildCallLlmRequest({ prompt: 'test', model: 'some-model-id' }, {
            backendName: 'Test',
...
//# sourceMappingURL=build-call-llm-request.test.js.map