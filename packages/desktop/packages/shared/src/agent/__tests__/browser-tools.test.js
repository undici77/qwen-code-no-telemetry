...
function createMockFns() {
    return {
        ...
        navigate: async (url) => ({ url: `https://${url}`, title: 'Test Page' }),
        snapshot: async () => ({
        ...
        click: async (_ref) => { },
        clickAt: async (_x, _y) => { },
        drag: async (_x1, _y1, _x2, _y2) => { },
        fill: async (_ref, _value) => { },
        type: async (_text) => { },
        select: async (_ref, _value) => { },
        setClipboard: async (_text) => { },
        getClipboard: async () => 'clipboard content',
        screenshot: async () => ({ imageBuffer: Buffer.from('fake-png-data'), imageFormat: 'png' }),
        screenshotRegion: async () => ({ imageBuffer: Buffer.from('fake-png-data'), imageFormat: 'png' }),
        ...
        waitFor: async (args) => ({ ok: true, kind: args.kind, elapsedMs: 123, detail: 'condition met' }),
        sendKey: async (_args) => { },
        getDownloads: async () => ([
        ...
        upload: async (_ref, _filePaths) => { },
        scroll: async (_dir, _amount) => { },
        goBack: async () => { },
        goForward: async () => { },
        evaluate: async (expr) => eval(expr),
        focusWindow: async (instanceId) => ({ instanceId: instanceId ?? 'browser-1', title: 'Example Domain', url: 'https://example.com' }),
        releaseControl: async (_instanceId) => ({ action: 'released', resolvedInstanceId: 'browser-1', affectedIds: ['browser-1'] }),
        closeWindow: async (_instanceId) => ({ action: 'closed', resolvedInstanceId: 'browser-1', affectedIds: ['browser-1'] }),
        hideWindow: async (_instanceId) => ({ action: 'hidden', resolvedInstanceId: 'browser-1', affectedIds: ['browser-1'] }),
        listWindows: async () => ([
            {
                id: 'browser-1',
...
describe('createBrowserTools', () => {
    ...
    describe('browser_tool', () => {
        ...
        it('releases control when navigate lands on a security challenge', async () => {
            ...
            mockFns.releaseControl = async (_instanceId) => {
                releaseCalls += 1;
                return { action: 'released', resolvedInstanceId: 'browser-1', affectedIds: ['browser-1'] };
            };
            const result = await executeTool(tools, 'browser_tool', { command: 'navigate example.com' });
        ...
        it('uses focus fallback when foreground open visibility does not settle in wait loop', async () => {
            ...
            try {
                ...
                mockFns.focusWindow = async (instanceId) => {
                    focusCalls += 1;
                    return {
                ...
                if (previousTimeout === undefined)
                    delete process.env.CRAFT_BROWSER_OPEN_SETTLE_TIMEOUT_MS;
                else
                    process.env.CRAFT_BROWSER_OPEN_SETTLE_TIMEOUT_MS = previousTimeout;
                if (previousPoll === undefined)
                    delete process.env.CRAFT_BROWSER_OPEN_SETTLE_POLL_MS;
                else
                    process.env.CRAFT_BROWSER_OPEN_SETTLE_POLL_MS = previousPoll;
            }
        });
        ...
        it('still reports challenge when screenshot capture fails or is empty', async () => {
            ...
            mockFns.screenshot = async () => ({ imageBuffer: Buffer.alloc(0), imageFormat: 'png' });
            const result = await executeTool(tools, 'browser_tool', { command: 'snapshot' });
            expect(result.content[0].text).toContain('Security verification detected (cloudflare).');
        ...
        it('routes annotated screenshot and passes annotate flag', async () => {
            ...
            mockFns.screenshot = async (args) => {
                screenshotArgs = args;
                return { imageBuffer: Buffer.from('fake-png-data'), imageFormat: 'png' };
            };
        ...
        it('returns error for screenshot-region when PNG is empty', async () => {
            mockFns.screenshotRegion = async () => ({ imageBuffer: Buffer.alloc(0), imageFormat: 'png' });
            const result = await executeTool(tools, 'browser_tool', { command: 'screenshot-region 10 20 100 80' });
            expect(result.isError).toBe(true);
            expect(result.content[0].text).toContain('empty image data');
        });
        ...
        it('parses quoted wait text values with spaces', async () => {
            ...
            mockFns.waitFor = async (args) => {
                waitArgs = args;
                return { ok: true, kind: args.kind, elapsedMs: 42, detail: 'condition met' };
            };
            const result = await executeTool(tools, 'browser_tool', {
        ...
        it('routes release command and calls releaseControl without hint', async () => {
            let requestedId;
            mockFns.releaseControl = async (instanceId) => {
        ...
        it('routes close command and calls closeWindow without hint', async () => {
            let requestedId;
            mockFns.closeWindow = async (instanceId) => {
                requestedId = instanceId;
                return { action: 'closed', resolvedInstanceId: 'browser-1', affectedIds: ['browser-1'] };
            };
        ...
        it('routes hide command and calls hideWindow without hint', async () => {
            let requestedId;
            mockFns.hideWindow = async (instanceId) => {
                requestedId = instanceId;
                return { action: 'hidden', resolvedInstanceId: 'browser-1', affectedIds: ['browser-1'] };
            };
        ...
        it('routes close command with explicit window id', async () => {
            ...
            mockFns.closeWindow = async (instanceId) => {
                requestedId = instanceId;
                return { action: 'closed', requestedInstanceId: instanceId, resolvedInstanceId: instanceId, affectedIds: instanceId ? [instanceId] : [] };
            };
