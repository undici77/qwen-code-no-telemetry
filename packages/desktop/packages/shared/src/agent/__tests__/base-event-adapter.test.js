...
class TestEventAdapter extends BaseEventAdapter {
    ...
    testConsumeBlockReason(...keys) {
        return this.consumeBlockReason(...keys);
    }
    testClassifyReadCommand(id, command) {
        return this.classifyReadCommand(id, command);
    }
    testConsumeReadCommand(id) {
        return this.consumeReadCommand(id);
    }
    testConsumeOutput(id) {
        return this.consumeOutput(id);
    }
    testCreateToolStart(id, toolName, input, intent, displayName, parentToolUseId) {
        return this.createToolStart(id, toolName, input, intent, displayName, parentToolUseId);
    }
    testCreateToolResult(id, toolName, result, isError, parentToolUseId) {
        return this.createToolResult(id, toolName, result, isError, parentToolUseId);
    }
    testCreateReadToolStart(id, readInfo, intent, displayName, parentToolUseId) {
...
describe('BaseEventAdapter', () => {
    ...
    describe('Event Construction', () => {
        it('should create tool_start events', () => {
            ...
            const event = adapter.testCreateToolStart('tool-123', 'Bash', { command: 'ls' }, 'List files', 'List Directory', 'parent-456');
        ...
        it('should create tool_result events', () => {
            adapter.startTurn('turn-1');
            const event = adapter.testCreateToolResult('tool-123', 'Bash', 'file1.ts\nfile2.ts', false, 'parent-456');
            expect(event).toEqual({
                type: 'tool_result',
                toolUseId: 'tool-123',
                toolName: 'Bash',
        ...
        it('should create Read-classified tool_start events', () => {
            adapter.startTurn();
            const event = adapter.testCreateReadToolStart('tool-123', {
        ...
        it('should use custom displayName for Read tool start', () => {
            adapter.startTurn();
            const event = adapter.testCreateReadToolStart('tool-123', { filePath: '/src/index.ts', originalCommand: 'head -20 /src/index.ts' }, undefined, 'Read Source');
            expect(event).toHaveProperty('displayName', 'Read Source');
        });
    });
});
//# sourceMappingURL=base-event-adapter.test.js.map