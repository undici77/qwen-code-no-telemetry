...
import { QwenAgent, extractQwenParentToolUseId, formatQwenAcpErrorMessage, resolveQwenParentToolUseId, } from '../qwen-agent.ts';
function createAgent(cwd, onAvailableModelsUpdate) {
...
describe('QwenAgent ACP error formatting', () => {
    it('includes ACP internal error details in user-visible messages', () => {
        ...
        expect(formatQwenAcpErrorMessage(error)).toBe('Internal error: 401 Unauthorized');
    });
    it('uses nested provider messages from ACP internal error data', () => {
        ...
        expect(formatQwenAcpErrorMessage(error)).toBe('Internal error: Model access denied');
    });
});
describe('QwenAgent model metadata', () => {
    it('extracts subagent parent metadata from Qwen ACP updates', () => {
        expect(extractQwenParentToolUseId({
            _meta: {
    ...
    it('falls back to the only active parent tool without self-referencing', () => {
        expect(resolveQwenParentToolUseId({
            update: {},
    ...
    it('uses ACP total tokens for context usage without double-counting cached tokens', () => {
        ...
        const usage = agent.extractUsage({
            _meta: {
                usage: {
                    inputTokens: 100,
                    outputTokens: 50,
    ...
    it('extracts latest replay usage for loaded Qwen native history', () => {
        const cwd = mkdtempSync(join(tmpdir(), 'qwen-cwd-'));
        const agent = createAgent(cwd, () => { });
        const tokenUsage = agent.extractLatestTokenUsage([
            {
...
//# sourceMappingURL=qwen-agent-models.test.js.map