...
function writeDefaultPermissions(configDir) {
    const permissionsDir = join(configDir, 'permissions');
    mkdirSync(permissionsDir, { recursive: true });
    writeFileSync(join(permissionsDir, 'default.json'), JSON.stringify({
        version: '2026-03-07',
        allowedBashPatterns: [
...
afterEach(() => {
    ...
    if (originalCliFlag === undefined)
        ...
        process.env.CRAFT_FEATURE_CRAFT_AGENTS_CLI = originalCliFlag;
});
...
//# sourceMappingURL=permissions-config-craft-cli-flag.test.js.map