...
import { ToolIndex, extractToolStarts, extractToolResults, serializeResult, isToolResultError, } from '../tool-matching';
...
function makeToolUseBlock(name, input = {}, id) {
    return {
        type: 'tool_use',
...
function makeToolResultBlock(toolUseId, content = 'success', isError = false) {
    return {
        type: 'tool_result',
...
describe('extractToolStarts', () => {
    ...
    it('sets parentToolUseId from SDK parent_tool_use_id (top-level = undefined)', () => {
        const blocks = [makeToolUseBlock('Grep', {}, 'toolu_1')];
        const events = extractToolStarts(blocks, null, toolIndex, emittedIds);
        expect(events[0]).toMatchObject({
            parentToolUseId: undefined,
        });
    ...
    it('sets parentToolUseId from SDK parent_tool_use_id (subagent child)', () => {
        const blocks = [makeToolUseBlock('Grep', {}, 'toolu_child1')];
    ...
    it('passes turnId through to events', () => {
        const blocks = [makeToolUseBlock('Read', {}, 'toolu_1')];
...
describe('extractToolResults', () => {
    ...
    it('does NOT emit task_backgrounded for a foreground non-fork Agent with a launch-shaped result', () => {
        ...
        const blocks = [
            makeToolResultBlock('toolu_agent', 'Background agent launched successfully.\nagentId: fg_agent_xyz'),
        ];
        const events = extractToolResults(blocks, null, undefined, toolIndex);
    ...
    it('detects a registry-backed headless fork from the runtime result', () => {
        ...
        const blocks = [
            makeToolResultBlock('toolu_fork_agent', 'Background agent launched successfully.\nagentId: fork_agent_xyz'),
        ];
        const events = extractToolResults(blocks, null, undefined, toolIndex);
        expect(events).toHaveLength(2);
        expect(events[1]).toMatchObject({
    ...
    it.each([undefined, true])('does NOT emit task_backgrounded for a nested Agent when the flag is %s', (runInBackground) => {
        ...
        const blocks = [
            makeToolResultBlock('toolu_nested_agent', 'Found auth code\nagentId: nested_agent_xyz'),
        ];
        const events = extractToolResults(blocks, 'parent_agent', undefined, toolIndex);
        expect(events).toHaveLength(1);
        expect(events[0]).toMatchObject({
            type: 'tool_result',
...
describe('Determinism property', () => {
    it('same tool_starts in different order produce same events (modulo emission order)', () => {
        ...
        const sortById = (events) => [...events].sort((a, b) => a.toolUseId.localeCompare(b.toolUseId));
    ...
    it('out-of-order results always match to correct tool by ID', () => {
        ...
        const result1 = [makeToolResultBlock('toolu_bash', 'output1')];
        const result2 = [makeToolResultBlock('toolu_read', 'output2')];
        const result3 = [makeToolResultBlock('toolu_grep', 'output3')];
...
//# sourceMappingURL=tool-matching.test.js.map