...
import { buildConversationSummaryPrompt, buildConversationSummaryTranscript, buildTransferredSessionContext, generateConversationSummary, } from '../conversation-summary.ts';
describe('conversation-summary helpers', () => {
    it('bounds individual messages and total transcript length', () => {
        const transcript = buildConversationSummaryTranscript(Array.from({ length: 40 }, (_, index) => ({
    ...
    it('delegates summary generation to the provided mini completion callback', async () => {
        const runMiniCompletion = mock(async (prompt) => {
...
//# sourceMappingURL=conversation-summary.test.js.map