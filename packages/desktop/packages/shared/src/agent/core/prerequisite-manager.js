...
const RULES = [
    // MCP source tools: mcp__{slug}__* format
    {
        ...
        resolveRequiredPath: (toolName, workspaceRootPath) => {
            const parts = toolName.split('__');
            const slug = parts[1];
            const guidePath = resolve(workspaceRootPath, 'sources', slug, 'guide.md');
            return existsSync(guidePath) ? guidePath : null;
        },
        blockMessage: 'You must read the source guide before using this tool. Please read the file at {filePath} first, then retry.',
    },
    ...
    // Browser tools and command wrapper: require browser-tools.md first
    {
        ...
        blockMessage: 'You must read the browser tools guide before using browser automation. Please read the file at {filePath} first, then retry.',
        strict: true,
    },
];
// ============================================================
...
export class PrerequisiteManager {
    ...
    static MAX_REJECTIONS = 1;
    readFiles = new Set();
    rejectionCounts = new Map();
    pendingSkillPaths = new Set();
    workspaceRootPath;
    onDebug;
    constructor(config) {
    ...
    checkSkillPrerequisites(toolName) {
        if (this.pendingSkillPaths.size === 0)
            return { allowed: true };
    ...
    trackBashSkillRead(input) {
        ...
        if (!command || this.pendingSkillPaths.size === 0)
            return false;
        let matched = false;
...
//# sourceMappingURL=prerequisite-manager.js.map