...
import { CLI_DOMAIN_POLICIES, CRAFT_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES, CRAFT_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES, } from '../../config/cli-domains.ts';
...
import { shouldAllowToolInMode, isApiEndpointAllowed, isReadOnlyBashCommandWithConfig, getPermissionModeDiagnostics, PERMISSION_MODE_CONFIG, } from '../mode-manager.ts';
 ...
 */
export function expandToolPaths(toolName, input, onDebug) {
 ...
 */
export function qualifySkillName(input, workspaceSlug, workspaceRootPath, workingDirectory, onDebug) {
 ...
 */
function resolveSkillPlugin(bareSlug, workspaceSlug, workspaceRootPath, workingDirectory) {
 ...
 */
export function stripToolMetadata(toolName, input, onDebug) {
 ...
 */
export function validateConfigWrite(toolName, input, workspaceRootPath, onDebug) {
    ...
    const detection = detectConfigFileType(filePath, workspaceRootPath) ?? detectAppConfigFileType(filePath);
    if (!detection) {
        // Not a config file - allow
        return { valid: true };
    }
    let contentToValidate = null;
    if (toolName === 'Write') {
    ...
    if (validationResult && !validationResult.valid) {
        onDebug?.(`Config validation blocked ${toolName} to ${detection.displayFile}: ${validationResult.errors.length} errors`);
        return {
            valid: false,
...
function buildCliDomainBlockMessage(namespace, context) {
...
function getWorkspaceRelativePath(filePath, workspaceRootPath, workingDirectory) {
...
function matchesPathScope(relativePath, scope) {
    if (scope.endsWith('/**')) {
...
function detectCliNamespaceFromConfigDetection(detection) {
    if (detection.type === 'labels')
        return 'label';
    if (detection.type === 'automations')
        return 'automation';
    if (detection.type === 'source')
        return 'source';
 ...
 */
export function getConfigCliRedirect(toolName, input, workspaceRootPath, workingDirectory) {
    const filePath = input.file_path;
    if (filePath && LABELS_BLOCKED_FILE_TOOLS.has(toolName)) {
        ...
        if (relativePath) {
            const labelsScopeMatch = CRAFT_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES.find(entry => entry.namespace === 'label' && matchesPathScope(relativePath, entry.scope));
            if (labelsScopeMatch) {
                return {
                    message: buildCliDomainBlockMessage('label', `Direct ${toolName} operations in labels/ are blocked.`),
                };
            }
        }
    }
    ...
    if (!namespace)
        return null;
    return {
        message: buildCliDomainBlockMessage(namespace, `Direct ${toolName} operations in ${detection.displayFile} are blocked.`),
    };
}
/**
 ...
 */
export function getConfigDomainBashRedirect(input, workspaceRootPath, workingDirectory) {
 ...
 */
function withPermissionModeContext(reason, sessionId, effectiveMode) {
    if (reason.includes('Effective mode:'))
        return reason;
...
export function runPreToolUseChecks(ctx) {
    const { toolName, input, sessionId, permissionMode, workspaceRootPath, workspaceId, plansFolderPath, dataFolderPath, workingDirectory, activeSourceSlugs, allSourceSlugs, hasSourceActivation, permissionManager, prerequisiteManager, backendMetadata, onDebug, } = ctx;
    ...
    if (permissionMode !== effectivePermissionMode) {
        onDebug?.(`[ModeSync] sessionId=${sessionId} incomingMode=${permissionMode} effectiveMode=${effectivePermissionMode} ` +
    ...
    // ============================================================
    const modeResult = shouldAllowToolInMode(toolName, input, effectivePermissionMode, { plansFolderPath, dataFolderPath, permissionsContext });
    ...
    if (toolName === 'Skill') {
        const skillResult = qualifySkillName(currentInput, workspaceId, workspaceRootPath, workingDirectory, onDebug);
    ...
    if (effectivePermissionMode === 'ask' || effectivePermissionMode === 'auto-edit') {
        const promptInfo = shouldPromptInAskMode(toolName, input, // Use original input for permission decisions (before stripping)
        permissionManager, permissionsContext, plansFolderPath, onDebug);
        const shouldPrompt = effectivePermissionMode === 'auto-edit' && promptInfo?.promptType === 'file_write'
            ? null
            : promptInfo;
        if (shouldPrompt) {
            const adminWrappedInput = shouldPrompt.promptType === 'admin_approval' &&
...
function toDisplayName(token) {
    return token.replace(/[-_]/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
}
function classifyAdminApproval(command) {
...
function wrapCommandForMacAdminPrompt(command) {
 ...
 * backends.
 */
export function shouldPromptInAskMode(toolName, input, permissionManager, permissionsContext, plansFolderPath, onDebug) {
    // --- File writes ---
    if (FILE_WRITE_TOOLS.has(toolName)) {
        ...
        const filePath = input.file_path || input.notebook_path || 'unknown';
        return {
    ...
    if (toolName.startsWith('mcp__')) {
        ...
        const safeModeResult = shouldAllowToolInMode(toolName, input, 'safe', { plansFolderPath });
