 ...
 * which are shared by agent backends.
 */
import { describe, it, expect, beforeEach, mock } from 'bun:test';
// ============================================================
// Module mocks (must be before imports of the module under test)
// ============================================================
let mockShouldAllowToolInMode = mock((_toolName, _input, _mode, _opts) => ({ allowed: true, reason: '' }));
let mockIsApiEndpointAllowed = mock((_method, _path, _ctx) => false);
let mockIsReadOnlyBashCommandWithConfig = mock((_command, _config) => false);
let mockEffectivePermissionMode = 'safe';
// Paths resolve from THIS file's location (core/__tests__/)
mock.module('../../mode-manager.ts', () => ({
    shouldAllowToolInMode: (a, b, c, d) => mockShouldAllowToolInMode(a, b, c, d),
    isApiEndpointAllowed: (a, b, c) => mockIsApiEndpointAllowed(a, b, c),
    isReadOnlyBashCommandWithConfig: (a, b) => mockIsReadOnlyBashCommandWithConfig(a, b),
...
mock.module('../../../utils/paths.ts', () => ({
    expandPath: (p) => p.replace(/^~/, '/Users/test'),
}));
...
let mockDetectConfigFileType = mock((_path, _workspaceRootPath) => null);
let mockDetectAppConfigFileType = mock((_path) => null);
let mockValidateConfigFileContent = mock((_type, _content) => null);
mock.module('../../../config/validators.ts', () => ({
    detectConfigFileType: (a, b) => mockDetectConfigFileType(a, b),
    detectAppConfigFileType: (a) => mockDetectAppConfigFileType(a),
    validateConfigFileContent: (a, b) => mockValidateConfigFileContent(a, b),
...
// ============================================================
import { runPreToolUseChecks, shouldPromptInAskMode, } from '../pre-tool-use.ts';
// ============================================================
// Test helpers
// ============================================================
function createMockPermissionManager(overrides) {
    return {
...
function createMockPrerequisiteManager(overrides) {
    return {
...
describe('runPreToolUseChecks', () => {
    ...
    describe('step 1: permission mode check', () => {
        ...
        it('passes correct args to shouldAllowToolInMode', () => {
            ...
            expect(mockShouldAllowToolInMode).toHaveBeenCalledWith('Bash', { command: 'ls' }, 'safe', {
                plansFolderPath: '/test/plans',
                dataFolderPath: '/test/data',
                permissionsContext: {
                    workspaceRootPath: '/test/workspace',
        ...
        it('uses effective mode from mode-manager diagnostics when incoming mode is stale', () => {
            ...
            expect(mockShouldAllowToolInMode).toHaveBeenCalledWith('Bash', { command: 'ls' }, 'safe', expect.any(Object));
        });
    });
    // ============================================================
...
describe('shouldPromptInAskMode', () => {
    ...
    describe('MCP mutations', () => {
        it('prompts for MCP mutations (blocked in safe mode)', () => {
            mockShouldAllowToolInMode.mockImplementation((_tool, _input, mode) => mode === 'safe' ? { allowed: false, reason: 'mutation' } : { allowed: true, reason: '' });
...
//# sourceMappingURL=pre-tool-use-checks.isolated.js.map