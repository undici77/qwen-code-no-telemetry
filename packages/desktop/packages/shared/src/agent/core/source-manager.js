...
export class SourceManager {
     ...
     */
    updateActiveState(mcpServerNames, apiServerNames, intendedSlugs) {
    ...
    formatSourceState() {
        ...
        const inactiveSources = this.allSources.filter((s) => !this.intendedSlugs.has(s.config.slug));
        // Find sources not yet seen this session
        const unseenSources = this.allSources.filter((s) => !this.knownSlugs.has(s.config.slug));
        // Find active sources that need attention (needs_auth or failed status)
        const activeSources = this.allSources.filter((s) => this.intendedSlugs.has(s.config.slug));
        ...
        const activeSourcesWithGuides = activeSources.filter((s) => s.guide?.raw && !GUIDE_EXEMPT_SLUGS.has(s.config.slug));
     ...
     */
    detectInactiveSourceToolError(toolName, errorMessage) {
        // Extract tool name from error message patterns
        let extractedToolName = toolName;
        ...
        if (!extractedToolName.startsWith('mcp__'))
            return null;
     ...
     */
    sourceNeedsAuthentication(source) {
