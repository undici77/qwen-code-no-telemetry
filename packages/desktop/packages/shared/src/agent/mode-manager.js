...
import { validatePowerShellCommand, looksLikePowerShell, isPowerShellAvailable, extractPowerShellWriteTarget, } from './powershell-validator.ts';
import { PERMISSION_MODE_ORDER, PERMISSION_MODE_CONFIG, SAFE_MODE_CONFIG, toCanonicalPermissionMode, parsePermissionMode, } from './mode-types.ts';
...
export { PERMISSION_MODE_ORDER, PERMISSION_MODE_CONFIG, SAFE_MODE_CONFIG, toCanonicalPermissionMode, parsePermissionMode, };
// Re-export PowerShell validator types
export { looksLikePowerShell, isPowerShellAvailable, };
// ============================================================
 ...
 */
function matchesAllowedWritePath(filePath, allowedPaths) {
 ...
 */
function isPathWithinDirectory(targetPath, baseDir) {
...
class ModeManager {
     ...
     */
    setPreviousPermissionMode(sessionId, previousPermissionMode) {
     ...
     */
    registerCallbacks(sessionId, callbacks) {
 ...
 */
export function getPermissionMode(sessionId) {
 ...
 * @returns true when state changed, false when mode was unchanged
 */
export function setPermissionMode(sessionId, mode, metadata) {
 ...
 */
export function consumeUserModeSignal(sessionId) {
 ...
 */
export function cyclePermissionMode(sessionId, enabledModes) {
 ...
 * Returns an unsubscribe function
 */
export function subscribeModeChanges(sessionId, callback) {
 ...
 */
export function hydratePreviousPermissionMode(sessionId, previousPermissionMode) {
 ...
 */
export function getPermissionModeDiagnostics(sessionId) {
    ...
    const transitionDisplay = state.previousPermissionMode
        ...
        : undefined;
    const userModeSignalPending = state.lastChangedBy === 'user' &&
        state.modeVersion > 0 &&
 ...
 */
export function initializeModeState(sessionId, initialMode, callbacks) {
    let mode;
 ...
 */
export function cleanupModeState(sessionId) {
 ...
 */
export function hasDangerousControlChars(command) {
 ...
 */
export function hasDangerousSubstitution(command) {
 ...
 */
const CONTROL_CHAR_EXPLANATIONS = {
 ...
 */
function findDangerousControlChar(command) {
    for (const char of command) {
        if (DANGEROUS_CONTROL_CHARS.has(char)) {
 ...
 */
function findDangerousSubstitution(command) {
    let inSingleQuote = false;
    let escaped = false;
    for (let i = 0; i < command.length; i++) {
        const char = command[i];
 ...
 */
function findRelevantPatterns(command, patterns) {
    ...
    for (const pattern of patterns) {
        ...
        if (sourceLower.includes(firstWord) ||
 ...
 */
function findBlockedCommandHint(command, config) {
 ...
 */
function analyzePatternMismatch(command, patterns) {
    ...
    const suggestion = generateMismatchSuggestion(trimmedCommand, bestMatch.matchedPrefix, failedToken, bestMatch.pattern);
    return {
 ...
 */
function generateMismatchSuggestion(command, matchedPrefix, failedToken, pattern) {
    ...
    if (commandsWithSubcommands.includes(firstWord || '') &&
 ...
 */
function looksLikeCmdBuiltin(command) {
 ...
 */
export function normalizeWindowsPathsForBashParser(command) {
    let result = '';
    let i = 0;
 ...
 */
export function getBashRejectionReason(command, config) {
    const trimmedCommand = command.trim();
 ...
 */
function getPowerShellRejectionReason(command, config) {
 ...
 */
function formatPermissionGuidance(config) {
    const lines = [];
 ...
 */
function isKnownBashParserTokenizerBug(error) {
 ...
 */
export function formatBashRejectionMessage(reason, config) {
 ...
 */
export function isReadOnlyBashCommandWithConfig(command, config) {
 ...
 */
export function extractBashWriteTarget(command) {
    ...
    const shellExecMatch = command.match(/(?:\/bin\/)?(?:zsh|bash|sh)\s+(?:-\w+\s+)*["'].*?>\s*([^\s'"\\]+)/);
 ...
 */
export function looksLikePotentialWrite(command) {
 ...
 */
export function getPathHint(targetPath, plansFolderPath, dataFolderPath) {
 ...
 */
function isReadOnlyMcpToolWithConfig(toolName, config) {
 ...
 */
function isApiCallAllowedWithConfig(method, path, config) {
 ...
 */
export function isApiEndpointAllowed(method, path, permissionsContext) {
    let config;
 ...
 */
export function shouldAllowToolInMode(toolName, toolInput, mode, options) {
    ...
    if (toolName === 'Bash') {
        ...
        if (typeof command === 'string') {
            ...
            const likelyWriteAttempt = (rejection.type === 'dangerous_operator' && rejection.operatorType === 'redirect') ||
            ...
            if (likelyWriteAttempt && (options?.plansFolderPath || options?.dataFolderPath)) {
                ...
                if (targetPath) {
                    ...
                    lines.push(``, `Allowed paths in Plan mode:`, ...[plansHint, dataHint].filter(Boolean).map(p => `• ${p}`), `• Or ask the user to switch to Ask before edits, Edit automatically, or YOLO (${config.shortcutHint}) to enable writes anywhere`);
                    return {
                        allowed: false,
                        reason: lines.join('\n'),
                    };
 ...
 */
function getBlockReasonWithConfig(toolName, config) {
 ...
 */
export function formatSessionState(sessionId, options) {
