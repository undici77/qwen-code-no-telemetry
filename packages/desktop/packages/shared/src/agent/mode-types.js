 ...
 */
export const PERMISSION_MODE_ORDER = ['allow-all', 'safe', 'ask', 'auto-edit'];
/**
 * Internal -> canonical mapping.
 */
export const PERMISSION_MODE_TO_CANONICAL = {
    'allow-all': 'execute',
    safe: 'explore',
    ask: 'ask',
    'auto-edit': 'auto-edit',
 ...
 */
export const CANONICAL_TO_PERMISSION_MODE = {
    explore: 'safe',
    ask: 'ask',
    execute: 'allow-all',
    'auto-edit': 'auto-edit',
 ...
 */
export function toCanonicalPermissionMode(mode) {
 ...
 */
export function parsePermissionMode(mode) {
    ...
    if (normalized === 'explore')
        return 'safe';
    if (normalized === 'plan' || normalized === 'plan-mode' || normalized === 'plan mode')
        return 'safe';
    ...
    if (normalized === 'default')
        return 'ask';
    if (normalized === 'ask-to-edit' || normalized === 'ask_to_edit' || normalized === 'ask to edit')
        return 'ask';
    if (normalized === 'ask-before-edits' || normalized === 'ask_before_edits' || normalized === 'ask before edits')
        return 'ask';
    if (normalized === 'edit-automatically' || normalized === 'edit_automatically' || normalized === 'edit automatically')
        return 'auto-edit';
    return null;
}
const LEGACY_CYCLABLE_PERMISSION_MODE_SETS = [
    ['safe', 'allow-all'],
    ['safe', 'ask', 'allow-all'],
];
function isLegacyCyclablePermissionModeSet(modes) {
    return LEGACY_CYCLABLE_PERMISSION_MODE_SETS.some(legacy => legacy.length === modes.length && legacy.every((mode, index) => modes[index] === mode));
}
export function normalizeCyclablePermissionModes(modes) {
    const normalized = [];
...
//# sourceMappingURL=mode-types.js.map