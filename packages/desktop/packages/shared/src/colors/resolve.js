 ...
 */
export function resolveEntityColor(color, isDark) {
 ...
 */
export function parseSystemColor(value) {
    const slashIndex = value.indexOf('/');
    const name = slashIndex === -1 ? value : value.slice(0, slashIndex);
    if (!isSystemColorName(name))
        return null;
    if (slashIndex === -1) {
        return { name };
    }
    ...
    if (!opacityStr || !/^\d+$/.test(opacityStr))
        return null;
 ...
 */
export function isSystemColorName(value) {
    return SYSTEM_COLOR_NAMES.includes(value);
}
/**
 ...
 */
export function deriveDarkVariant(lightColor) {
