 ...
 */
export function isValidSystemColor(value) {
    const match = SYSTEM_COLOR_PATTERN.exec(value);
    if (!match)
        return false;
    const name = match[1];
    if (!SYSTEM_COLOR_NAMES.includes(name))
        return false;
 ...
 */
export function isValidEntityColor(value) {
    if (typeof value === 'string') {
        return isValidSystemColor(value);
    }
    if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
        const obj = value;
        if (typeof obj.light !== 'string' || !isValidCSSColor(obj.light))
            return false;
        if (obj.dark !== undefined && (typeof obj.dark !== 'string' || !isValidCSSColor(obj.dark)))
            return false;
        return true;
    }
    return false;
