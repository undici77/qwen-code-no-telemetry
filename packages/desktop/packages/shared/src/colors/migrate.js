 ...
 */
const TAILWIND_TO_ENTITY_COLOR = {
    'text-accent': 'accent',
    'text-info': 'info',
 ...
 */
export function migrateColorValue(oldColor) {
    ...
    if (fgOpacityMatch) {
        ...
        return { migrated: `foreground/${opacity}`, changed: true };
    }
 ...
 */
export function migrateStatusColors(config) {
    let changed = false;
 ...
 * Mutates the config in place and returns whether any changes were made.
 */
export function migrateLabelColors(config) {
    let changed = false;
    // Recursively migrate colors in the label tree
    function migrateTree(labels) {
