...
function isAbsoluteMentionPath(path) {
 ...
 */
export function parseMentions(text, availableSkillSlugs, availableSourceSlugs) {
    const result = {
        skills: [],
        invalidSkills: [],
        sources: [],
 ...
 */
export function stripAllMentions(text) {
    return text
 ...
 */
export function resolveSkillMentions(text, skillNames) {
    return text.replace(new RegExp(`\\[skill:(?:${WS_ID_CHARS}+:)?([\\w-]+)\\]`, 'g'), (_match, slug) => {
 ...
 */
export function resolveSourceMentions(text) {
    return text.replace(/\[source:([\w-]+)\]/g, (_match, slug) => `[Mentioned source: ${slug}]`);
}
/**
 ...
 */
export function resolveFileMentions(text, workingDirectory) {
    return text
        .replace(/\[file:([^\]]+)\]/g, (_match, filePath) => {
        ...
        .replace(/\[folder:([^\]]+)\]/g, (_match, folderPath) => {
