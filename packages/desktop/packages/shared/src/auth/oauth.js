...
import { buildOAuthDeeplinkUrl } from './types.ts';
...
export class CraftOAuth {
    ...
    callbacks;
    sessionContext;
    constructor(config, callbacks, sessionContext) {
        this.config = config;
    ...
    async getServerMetadata() {
        const metadata = await discoverOAuthMetadata(this.config.mcpUrl, (msg) => this.callbacks.onStatus(msg));
        if (!metadata) {
    ...
    async registerClient(registrationEndpoint, port) {
    ...
    async exchangeCodeForTokens(tokenEndpoint, code, codeVerifier, clientId, port) {
        const redirectUri = `http://localhost:${port}${CALLBACK_PATH}`;
    ...
    async authenticate() {
        ...
        const tokens = await this.exchangeCodeForTokens(metadata.token_endpoint, authCode, pkce.verifier, clientId, port);
    ...
    async startCallbackServer(expectedState) {
        ...
        // All ports exhausted
        clearTimeout(timeout);
        throw new Error(`All OAuth callback ports (${CALLBACK_PORT_START}-${CALLBACK_PORT_END}) are in use. Please restart the application.`);
    }
    stopServer() {
        if (this.server) {
            this.server.close();
...
function shouldFallbackToDefaultMcpClient(error) {
    return error instanceof McpClientRegistrationError && (error.status === 401 || error.status === 403);
}
async function registerMcpOAuthClient(registrationEndpoint, redirectUri) {
    let response;
    try {
 ...
 */
async function exchangeMcpCodeForTokens(tokenEndpoint, code, codeVerifier, clientId, redirectUri) {
    const params = new URLSearchParams({
        grant_type: 'authorization_code',
        code,
        redirect_uri: redirectUri,
 ...
 */
export async function prepareMcpOAuth(mcpUrl, options) {
 ...
 */
export async function exchangeMcpOAuth(params) {
    try {
        const tokens = await exchangeMcpCodeForTokens(params.tokenEndpoint, params.code, params.codeVerifier, params.clientId, params.redirectUri);
        return {
            success: true,
 ...
 */
export function getMcpBaseUrl(mcpUrl) {
    try {
 ...
 */
async function tryFetchAuthServerMetadata(url, onLog) {
    try {
        onLog?.(`  Trying: ${url}`);
 ...
 */
function isProtectedResourceMetadata(data) {
    ...
    if (obj.authorization_servers !== undefined) {
        if (!Array.isArray(obj.authorization_servers))
            return false;
        if (!obj.authorization_servers.every(s => typeof s === 'string'))
            return false;
    }
    return true;
}
 ...
 */
async function fetchWithTimeout(url, options = {}, timeoutMs = DISCOVERY_TIMEOUT_MS) {
 ...
 */
function parseResourceMetadataFromHeader(wwwAuthenticate) {
    if (!wwwAuthenticate)
        return null;
 ...
 */
async function fetchProtectedResourceMetadata(metadataUrl, onLog) {
 ...
 */
async function discoverViaProtectedResource(mcpUrl, onLog) {
    try {
 ...
 */
export async function discoverOAuthMetadata(mcpUrl, onLog) {
    let url;
    try {
        url = new URL(mcpUrl);
    }
