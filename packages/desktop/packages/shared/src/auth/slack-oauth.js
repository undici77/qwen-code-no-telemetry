 ...
 */
export const SLACK_SERVICE_SCOPES = {
 ...
 */
export async function refreshSlackToken(refreshToken, clientId) {
    const authHeader = Buffer.from(`${clientId || SLACK_CLIENT_ID}:${SLACK_CLIENT_SECRET}`).toString('base64');
    const params = new URLSearchParams({
 ...
 */
export function getSlackScopes(options) {
 ...
 */
export function prepareSlackOAuth(options) {
    if (!isSlackOAuthConfigured()) {
        throw new Error('Slack OAuth not configured. Set SLACK_OAUTH_CLIENT_ID and SLACK_OAUTH_CLIENT_SECRET environment variables.');
    }
 ...
 */
export async function exchangeSlackOAuth(params) {
    try {
 ...
 * });
 */
export async function startSlackOAuth(options = {}) {
    try {
