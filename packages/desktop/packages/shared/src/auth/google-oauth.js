...
export async function refreshGoogleToken(refreshToken, clientId, clientSecret) {
    ...
    if (!id || !secret) {
        throw new Error('Google OAuth credentials not available for token refresh. ' +
 ...
 */
export function isGoogleOAuthConfigured(clientId, clientSecret) {
    const id = clientId || GOOGLE_CLIENT_ID_ENV;
    const secret = clientSecret || GOOGLE_CLIENT_SECRET_ENV;
 ...
 */
export function getGoogleScopes(options) {
 ...
 */
export function prepareGoogleOAuth(options) {
    ...
    if (!isGoogleOAuthConfigured(clientId, clientSecret)) {
        throw new Error('Google OAuth not configured. Provide clientId and clientSecret in source config, ' +
 ...
 */
export async function exchangeGoogleOAuth(params) {
    try {
        const tokens = await exchangeCodeForTokens(params.code, params.codeVerifier, params.redirectUri, params.clientId, params.clientSecret || '');
...
export async function startGoogleOAuth(options = {}) {
    try {
        ...
        if (!isGoogleOAuthConfigured(clientId, clientSecret)) {
            return {
                success: false,
                error: 'Google OAuth not configured. Provide clientId and clientSecret in source config, ' +
                    'or set GOOGLE_OAUTH_CLIENT_ID and GOOGLE_OAUTH_CLIENT_SECRET environment variables.',
            };
        }
        ...
        if (callback.query.error) {
            const isAccessBlocked = callback.query.error === 'access_denied' &&
