...
function mockTokenResponse(body, contentType = 'application/json') {
    globalThis.fetch = (async () => new Response(typeof body === 'string' ? body : JSON.stringify(body), { status: 200, headers: { 'Content-Type': contentType } }));
}
describe('generic OAuth token responses', () => {
    ...
    it('exchanges form-encoded token responses with string expires_in', async () => {
        mockTokenResponse('access_token=access-token&refresh_token=refresh-token&expires_in=3600', 'application/x-www-form-urlencoded');
        const result = await exchangeGenericOAuth(exchangeParams);
        expect(result.success).toBe(true);
        expect(result.accessToken).toBe('access-token');
        expect(result.refreshToken).toBe('refresh-token');
    ...
    it('rejects refresh responses without access_token', async () => {
        mockTokenResponse({ refresh_token: 'refresh-token', expires_in: 3600 });
        await expect(refreshGenericOAuthToken('refresh-token', 'https://auth.example.com/oauth/token', 'client-123', 'secret-123')).rejects.toThrow('missing access_token');
    });
    it('refreshes JSON token responses with refresh_token and numeric expires_in', async () => {
        ...
        const result = await refreshGenericOAuthToken('old-refresh-token', 'https://auth.example.com/oauth/token', 'client-123', 'secret-123');
    ...
    it('rejects refresh responses with malformed expires_in', async () => {
        ...
        await expect(refreshGenericOAuthToken('refresh-token', 'https://auth.example.com/oauth/token', 'client-123', 'secret-123')).rejects.toThrow('invalid expires_in');
    });
});
//# sourceMappingURL=generic-oauth.test.js.map