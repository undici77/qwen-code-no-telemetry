...
import { OAUTH_RELAY_CALLBACK_URL, decodeOAuthRelayState, encodeOAuthRelayState, isOAuthRelayState, wrapPreparedOAuthFlowForRelay, } from '../oauth-relay.ts';
describe('oauth relay state', () => {
    it('round-trips the relay callback target and inner state', () => {
        const encoded = encodeOAuthRelayState('https://ghalmos.craftdocs-cf-t1.com/api/oauth/callback', 'inner-state-123');
...
describe('wrapPreparedOAuthFlowForRelay', () => {
    it('keeps the inner flow state but rewrites auth URL state and redirect_uri', () => {
        ...
        const wrapped = wrapPreparedOAuthFlowForRelay(prepared, 'https://ghalmos.craftdocs-cf-t1.com/api/oauth/callback');
...
//# sourceMappingURL=oauth-relay.test.js.map