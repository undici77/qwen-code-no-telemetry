 ...
 */
export async function appendAutomationHistoryEntry(workspaceRootPath, entry) {
 ...
 */
export async function compactAutomationHistory(workspaceRootPath, maxPerMatcher = AUTOMATION_HISTORY_MAX_RUNS_PER_MATCHER, maxTotal = AUTOMATION_HISTORY_MAX_ENTRIES) {
    const historyPath = join(workspaceRootPath, AUTOMATIONS_HISTORY_FILE);
 ...
 */
export function compactAutomationHistorySync(workspaceRootPath, maxPerMatcher = AUTOMATION_HISTORY_MAX_RUNS_PER_MATCHER, maxTotal = AUTOMATION_HISTORY_MAX_ENTRIES) {
    const historyPath = join(workspaceRootPath, AUTOMATIONS_HISTORY_FILE);
...
// ============================================================================
function recoverHistoryObjectsFromLine(line) {
    const recovered = [];
    let depth = 0;
    let start = -1;
    let cursor = 0;
...
function isValidHistoryObject(value) {
    return value !== null && typeof value === 'object' && !Array.isArray(value);
}
function getHistoryObjectId(value) {
...
function compactEntries(content, maxPerMatcher, maxTotal) {
    ...
    if (!needsRewrite && trimmed.length === originalLineCount)
        return null;
