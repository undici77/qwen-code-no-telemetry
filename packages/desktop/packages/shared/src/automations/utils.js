 ...
 */
export function expandEnvVars(str, env) {
    return str
        // Replace ${VAR} syntax
 ...
 */
export function getMatchValue(event, data) {
    switch (event) {
        case 'LabelAdd':
        case 'LabelRemove':
 ...
 */
export function getMatchValueForSdkInput(event, input) {
    switch (event) {
        case 'PreToolUse':
        case 'PostToolUse':
 ...
 */
export function matcherMatchesWithContext(matcher, event, context) {
    if (!matchesBasePredicate(matcher, event, context.matchValue))
        return false;
 ...
 */
export function matcherMatches(matcher, event, data) {
 ...
 */
export function matcherMatchesSdk(matcher, event, input) {
    return matcherMatchesWithContext(matcher, event, {
...
export function cleanEnv() {
    return Object.fromEntries(Object.entries(process.env).filter((e) => e[1] !== undefined));
}
...
function buildBaseEventEnv(event, payload) {
    ...
    if (payload.sessionId)
        env.CRAFT_SESSION_ID = payload.sessionId;
    if (payload.sessionName)
        env.CRAFT_SESSION_NAME = payload.sessionName;
    if (payload.workspaceId)
        env.CRAFT_WORKSPACE_ID = payload.workspaceId;
    // Session metadata as JSON
    const sessionMetadata = {};
    if (payload.sessionId)
        sessionMetadata.id = payload.sessionId;
    if (payload.sessionName)
        sessionMetadata.name = payload.sessionName;
 ...
 */
export function buildEnvFromPayload(event, payload) {
    const base = buildBaseEventEnv(event, payload);
    const env = { ...cleanEnv(), ...base };
    // Sanitize session name for shell context
    if (payload.sessionName)
        env.CRAFT_SESSION_NAME = sanitizeForShell(payload.sessionName);
 ...
 */
export function buildWebhookEnv(event, payload) {
