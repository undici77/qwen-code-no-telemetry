...
import { PromptHandler, EventLogHandler, WebhookHandler } from './handlers/index.ts';
import {} from './types.ts';
import { validateAutomationsConfig } from './validation.ts';
import { matcherMatchesSdk } from './utils.ts';
import { SchedulerService } from '../scheduler/scheduler-service.ts';
...
export class AutomationSystem {
    ...
    getActionCount() {
        if (!this.config)
            return 0;
        return Object.values(this.config.automations).reduce((sum, matchers) => sum + (matchers?.reduce((s, m) => s + m.actions.length, 0) ?? 0), 0);
    }
    // ============================================================================
    ...
    async executeAgentEvent(event, input, signal) {
        ...
        for (const matcher of matchers) {
            if (!matcherMatchesSdk(matcher, event, input))
                continue;
            matchedCount++;
...
//# sourceMappingURL=automation-system.js.map