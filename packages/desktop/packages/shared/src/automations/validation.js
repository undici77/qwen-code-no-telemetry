 ...
 */
export function validateAutomationsConfig(content) {
 ...
 */
export function validateAutomationsContent(jsonString, fileName) {
    ...
    const matcherCount = Object.values(config.automations).reduce((sum, matchers) => sum + (matchers?.length ?? 0), 0);
    if (matcherCount === 0) {
        warnings.push({
            file,
            path: 'automations',
 ...
 */
export function validateAutomations(workspaceRoot) {
                        ...
                        for (const action of actions) {
                            ...
                            if (action.llmConnection) {
                                ...
                                if (!connection) {
                                    ...
                                    // Check exact match, provider-prefixed match, or short alias match.
                                    const modelValue = action.model;
                                    const isAvailable = modelIds.some(id => id === modelValue || id.endsWith(`/${modelValue}`) ||
