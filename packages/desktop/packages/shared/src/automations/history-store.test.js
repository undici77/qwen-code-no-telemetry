...
describe('history-store', () => {
    ...
    describe('compactAutomationHistory', () => {
        it('should keep only last N entries per automation ID', async () => {
            const lines = Array.from({ length: 30 }, (_, i) => JSON.stringify(makeEntry('a1', i))).join('\n') + '\n';
        ...
        it('should preserve nested and 3+ glued object records while rewriting the file', async () => {
            const gluedLine = JSON.stringify(makeEntry('a1', 2)) +
        ...
        it('should preserve complete glued records before trailing garbage', async () => {
            ...
            writeFileSync(join(tempDir, AUTOMATIONS_HISTORY_FILE), JSON.stringify(first) + JSON.stringify(second) + 'partial-write\n');
            await compactAutomationHistory(tempDir, 20, 1000);
            const rewritten = readFileSync(join(tempDir, AUTOMATIONS_HISTORY_FILE), 'utf-8');
        ...
        it('should recover valid objects before a stray closing brace', async () => {
            const first = makeEntry('a1', 1);
            const second = makeEntry('a1', 2);
            writeFileSync(join(tempDir, AUTOMATIONS_HISTORY_FILE), JSON.stringify(first) + '}garbage\n' + JSON.stringify(second) + '\n');
            await compactAutomationHistory(tempDir, 20, 1000);
            const entries = readHistory(tempDir);
            expect(entries.map((entry) => entry.ts)).toEqual([1, 2]);
        });
        it('should rewrite glued records when recovered count matches physical line count', async () => {
            const first = makeEntry('a1', 1);
            const second = makeEntry('a1', 2);
            writeFileSync(join(tempDir, AUTOMATIONS_HISTORY_FILE), JSON.stringify(first) + JSON.stringify(second) + '\nnot-json{{{\n');
            await compactAutomationHistory(tempDir, 20, 1000);
            const rewritten = readFileSync(join(tempDir, AUTOMATIONS_HISTORY_FILE), 'utf-8');
            expect(rewritten).not.toContain('}{');
            expect(rewritten.trim().split('\n')).toHaveLength(2);
        ...
        it('should rewrite non-object JSON history lines out of the file', async () => {
            writeFileSync(join(tempDir, AUTOMATIONS_HISTORY_FILE), [
                JSON.stringify(makeEntry('a1', 1)),
                'null',
                '42',
                '[]',
        ...
        it('should skip invalid recovered object segments while preserving valid glued records', async () => {
            const first = makeEntry('a1', 1);
            const second = makeEntry('a1', 2);
            writeFileSync(join(tempDir, AUTOMATIONS_HISTORY_FILE), JSON.stringify(first) + '{"id":}' + JSON.stringify(second) + '\n');
            await compactAutomationHistory(tempDir, 20, 1000);
            const entries = readHistory(tempDir);
            expect(entries.map((entry) => entry.ts)).toEqual([1, 2]);
        });
        ...
        it('should no-op when already within limits', async () => {
            const lines = Array.from({ length: 5 }, (_, i) => JSON.stringify(makeEntry('a1', i))).join('\n') + '\n';
            writeFileSync(join(tempDir, AUTOMATIONS_HISTORY_FILE), lines);
            await compactAutomationHistory(tempDir, 20, 1000);
            const entries = readHistory(tempDir);
            expect(entries).toHaveLength(5);
...
//# sourceMappingURL=history-store.test.js.map