...
describe('evaluateConditions', () => {
    ...
    describe('state condition', () => {
        ...
        it('should fail with no operator', () => {
            const conditions = [
                { condition: 'state', field: 'something' },
            ];
    ...
    describe('logical conditions', () => {
        ...
        describe('nesting', () => {
            ...
            it('should fail at max nesting depth', () => {
                ...
                let inner = { condition: 'state', field: 'x', value: 1 };
                for (let i = 0; i < 9; i++) {
...
describe('matcherMatches with conditions', () => {
    ...
    it('should work with time conditions', () => {
        const matcher = {
            conditions: [
                { condition: 'time', after: '09:00', before: '17:00', weekday: ['fri'] },
            ],
            actions: [{ type: 'prompt', prompt: 'test' }],
        };
...
describe('matcherMatchesSdk with conditions', () => {
    it('should pass when SDK matcher and conditions both pass', () => {
        const matcher = {
            matcher: '^Bash$',
            conditions: [
                { condition: 'state', field: 'hook_event_name', value: 'PreToolUse' },
            ],
            actions: [{ type: 'prompt', prompt: 'test' }],
        };
    ...
    it('should fail when SDK matcher passes but conditions fail', () => {
        const matcher = {
            matcher: '^Bash$',
            conditions: [
                { condition: 'state', field: 'hook_event_name', value: 'PostToolUse' },
            ],
            actions: [{ type: 'prompt', prompt: 'test' }],
        };
        expect(matcherMatchesSdk(matcher, 'PreToolUse', {
    ...
    it('should fail when SDK matcher does not match regardless of conditions', () => {
        const matcher = {
            matcher: '^Read$',
            conditions: [
                { condition: 'state', field: 'tool_name', value: 'Read' },
            ],
            actions: [{ type: 'prompt', prompt: 'test' }],
        };
        expect(matcherMatchesSdk(matcher, 'PreToolUse', {
