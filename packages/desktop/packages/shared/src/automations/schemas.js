...
export const WebhookActionSchema = z.object({
    ...
    url: z.string().min(1, 'URL cannot be empty').refine((url) => {
...
export const StateConditionSchema = z.object({
    ...
    const operatorCount = (hasValue ? 1 : 0) +
        (hasFromOrTo ? 1 : 0) +
        (hasContains ? 1 : 0) +
        (hasNotValue ? 1 : 0);
    if (operatorCount === 0) {
...
export const AutomationConditionSchema = z.lazy(() => z.discriminatedUnion('condition', [
    TimeConditionSchema,
 ...
 */
export function zodErrorToIssues(error, file) {
