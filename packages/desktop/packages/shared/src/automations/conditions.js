...
export function evaluateConditions(conditions, context) {
    ...
    for (const condition of conditions) {
        if (!evaluateCondition(condition, context, 0))
            return false;
    }
    return true;
}
...
function evaluateStateCondition(condition, context) {
    ...
    if (hasFrom || hasTo) {
        ...
        if (hasTo && payload[toKey] !== condition.to)
            return false;
        if (hasFrom && payload[fromKey] !== condition.from)
            return false;
        return true;
    }
    ...
    switch (condition.condition) {
        case 'and':
            for (const sub of conditions) {
                if (!evaluateCondition(sub, context, depth + 1))
                    return false;
            }
            return true;
        case 'or':
