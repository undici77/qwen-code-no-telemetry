...
describe('cron-matcher', () => {
    ...
    it('should match with timezone conversion', () => {
        ...
        // That's 15:15 UTC
        jest.useFakeTimers();
        jest.setSystemTime(new Date('2026-02-09T15:15:30Z'));
    ...
    it('should match every 5 minutes pattern', () => {
        jest.useFakeTimers();
        jest.setSystemTime(new Date(2026, 1, 10, 10, 15, 0));
        expect(matchesCron('*/5 * * * *')).toBe(true);
    });
    it('should not match between 5-minute intervals', () => {
        jest.useFakeTimers();
        jest.setSystemTime(new Date(2026, 1, 10, 10, 13, 0));
        expect(matchesCron('*/5 * * * *')).toBe(false);
    });
});
//# sourceMappingURL=cron-matcher.test.js.map