...
function normalizeRequiredSources(value) {
    ...
    if (!asArray)
        return undefined;
    const normalized = Array.from(new Set(asArray
...
function loadSkillsFromDirs(skillsDirs, source) {
 ...
 * @param slug - Skill directory name
 */
export function loadSkill(workspaceRoot, slug) {
 ...
 * @param workspaceRoot - Absolute path to workspace root
 */
export function loadWorkspaceSkills(workspaceRoot) {
    const skillsDir = getWorkspaceSkillsPath(workspaceRoot);
 ...
 */
export function loadAllSkills(workspaceRoot, projectRoot) {
 ...
 */
export function loadSkillBySlug(workspaceRoot, slug, projectRoot) {
 ...
 * @param slug - Skill directory name
 */
export function getSkillIconPath(workspaceRoot, slug) {
    const skillsDir = getWorkspaceSkillsPath(workspaceRoot);
    const skillDir = join(skillsDir, slug);
 ...
 * @param slug - Skill directory name
 */
export function deleteSkill(workspaceRoot, slug) {
    const skillsDir = getWorkspaceSkillsPath(workspaceRoot);
    const skillDir = join(skillsDir, slug);
    if (!existsSync(skillDir)) {
        return false;
 ...
 * @param slug - Skill directory name
 */
export function skillExists(workspaceRoot, slug) {
    const skillsDir = getWorkspaceSkillsPath(workspaceRoot);
    const skillDir = join(skillsDir, slug);
    const skillFile = join(skillDir, 'SKILL.md');
 ...
 * @param workspaceRoot - Absolute path to workspace root
 */
export function listSkillSlugs(workspaceRoot) {
    const skillsDir = getWorkspaceSkillsPath(workspaceRoot);
    if (!existsSync(skillsDir)) {
        return [];
    }
 ...
 */
export async function downloadSkillIcon(skillDir, iconUrl) {
 ...
 */
export function skillNeedsIconDownload(skill) {
