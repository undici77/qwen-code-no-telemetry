...
import { loadAllSkills, loadWorkspaceSkills, loadSkill, skillExists, listSkillSlugs, deleteSkill, } from '../storage.ts';
// ============================================================
...
function createSkill(skillsDir, slug, opts = {}) {
...
function createInvalidSkill(skillsDir, slug) {
    const skillDir = join(skillsDir, slug);
    mkdirSync(skillDir, { recursive: true });
...
function createEmptySkillDir(skillsDir, slug) {
    const skillDir = join(skillsDir, slug);
    mkdirSync(skillDir, { recursive: true });
    return skillDir;
}
/** Get the set of slugs currently in the real global skills directory */
function getExistingGlobalSlugs() {
