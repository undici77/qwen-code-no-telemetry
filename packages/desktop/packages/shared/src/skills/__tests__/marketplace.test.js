...
import { getSkillMarketplaceDefinition, SKILL_MARKETPLACE_DEFINITIONS, } from '../marketplace.ts';
describe('skill marketplace definitions', () => {
    ...
    it('uses installable GitHub SKILL.md URLs for modelstudioai skills', () => {
        const modelstudioSkills = SKILL_MARKETPLACE_DEFINITIONS.filter((skill) => skill.sourceUrl.includes('github.com/modelstudioai/skills/'));
    ...
    it('looks up spark-video by the installable skill name', () => {
        ...
        expect(skill?.sourceUrl).toBe('https://github.com/modelstudioai/skills/blob/main/skills/spark-video/SKILL.md');
    });
});
