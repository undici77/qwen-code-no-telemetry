 ...
 */
export function getDefaultWorkspacesDir() {
    return DEFAULT_WORKSPACES_DIR;
}
/**
 * Ensure default workspaces directory exists
 */
export function ensureDefaultWorkspacesDir() {
 ...
 */
export function getWorkspaceSourcesPath(rootPath) {
 ...
 * @param rootPath - Absolute path to workspace root folder
 */
export function getWorkspaceSessionsPath(rootPath) {
 ...
 * @param rootPath - Absolute path to workspace root folder
 */
export function getWorkspaceSkillsPath(rootPath) {
 ...
 * @param rootPath - Absolute path to workspace root folder
 */
export function loadWorkspaceConfig(rootPath) {
 ...
 * @param rootPath - Absolute path to workspace root folder
 */
export function saveWorkspaceConfig(rootPath, config) {
 ...
 */
function listSubdirNames(dirPath) {
    if (!existsSync(dirPath))
        return [];
    try {
 ...
 * @param rootPath - Absolute path to workspace root folder
 */
export function loadWorkspace(rootPath) {
 ...
 * @param rootPath - Absolute path to workspace root folder
 */
export function getWorkspaceSummary(rootPath) {
    const config = loadWorkspaceConfig(rootPath);
    if (!config)
        return null;
    return {
 ...
 */
export function generateUniqueWorkspacePath(name, baseDir) {
 ...
 */
export function createWorkspaceAtPath(rootPath, name, defaults, metadata) {
    const now = Date.now();
    const slug = generateSlug(name);
 ...
 * @param rootPath - Absolute path to workspace root folder
 */
export function deleteWorkspaceFolder(rootPath) {
    if (!existsSync(rootPath))
        return false;
    try {
 ...
 */
export function isValidWorkspace(rootPath) {
 ...
 */
export function renameWorkspaceFolder(rootPath, newName) {
    const config = loadWorkspaceConfig(rootPath);
    if (!config)
        return false;
 ...
 */
export function discoverWorkspacesInDefaultLocation() {
    const discovered = [];
    if (!existsSync(DEFAULT_WORKSPACES_DIR)) {
        return discovered;
    }
 ...
 */
export function getWorkspaceColorTheme(rootPath) {
    const config = loadWorkspaceConfig(rootPath);
 ...
 */
export function setWorkspaceColorTheme(rootPath, themeId) {
    const config = loadWorkspaceConfig(rootPath);
    if (!config)
        return;
 ...
 */
export function isLocalMcpEnabled(rootPath) {
