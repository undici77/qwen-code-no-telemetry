...
// Helpers
// ============================================================
function createTestWorkspace(rootDir) {
...
function createTestSource(wsDir, slug, config) {
...
function createTestSkill(wsDir, slug, extraFiles) {
...
function makeBundleFile(path, content) {
...
function createTestAutomations(wsDir, automations, version = 2) {
    writeFileSync(join(wsDir, 'automations.json'), JSON.stringify({ version, automations }, null, 2));
}
function makeAutomationEntry(overrides) {
    return {
        matcher: {
            id: overrides.id,
            name: overrides.name,
...
describe('resource-bundle', () => {
    ...
    describe('exportResources', () => {
        ...
        it('sanitizes webhook auth on export', () => {
            ...
            const action = bundle.resources.automations[0].matcher.actions[0];
    ...
    describe('importResources', () => {
        ...
        it('calls clearSourceCredentials on source overwrite', async () => {
            ...
            const cleared = [];
            const deps = {
                clearSourceCredentials: async (_wsId, slug) => {
                    cleared.push(slug);
                },
            };
            const bundle = {
        ...
        it('overwrites automation by ID in overwrite mode', async () => {
            ...
            const config = JSON.parse(readFileSync(join(wsDir, 'automations.json'), 'utf-8'));
            // Replaced automation
            const names = config.automations.UserPromptSubmit.map((m) => m.name);
        ...
        it('rejects invalid bundle with error in result', async () => {
            const wsDir = createTestWorkspace(tmpDir);
            const result = await importResources(wsDir, { version: 99 }, 'skip', noopDeps);
...
//# sourceMappingURL=resource-bundle.test.js.map