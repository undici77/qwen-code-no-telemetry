 ...
 */
export function toPortableRelPath(relPath) {
    return relPath.split(sep).join('/');
}
/**
 * Convert a portable forward-slash path to OS-native form for filesystem writes.
 */
export function fromPortableRelPath(portablePath) {
    return portablePath.split('/').join(sep);
}
function hasParentTraversalSegment(portablePath) {
 ...
 */
export function collectDirectoryFiles(dir, options) {
    const files = [];
 ...
 */
export function restoreFiles(targetDir, files) {
...
//# sourceMappingURL=bundle-files.js.map