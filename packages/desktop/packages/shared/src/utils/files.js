 ...
 */
export function validateImageForAgent(size, width, height) {
 ...
 */
export function readFileAttachment(filePath) {
    try {
 ...
 */
export function processInputWithFiles(input) {
 ...
 */
function readClipboardImageDataWindows() {
 ...
 */
function readClipboardImageDataLinux() {
    const tempFile = join(tmpdir(), `craft-clipboard-${Date.now()}.png`);
 ...
 */
function readClipboardImageDataMacOS() {
    const tempFile = join(tmpdir(), `craft-clipboard-${Date.now()}.png`);
 ...
 */
function readImageFile(tempFile) {
    try {
        const stats = statSync(tempFile);
 ...
 */
export function formatSinglePathToRelative(absolutePath, cwd) {
 ...
 */
export function formatPathsToRelative(text, cwd) {
 ...
 */
export function formatToolInputPaths(input, cwd) {
    if (!input)
        return input;
    const result = {};
