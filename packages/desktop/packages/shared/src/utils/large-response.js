...
import { looksLikeBinary, extractBase64Binary, detectExtensionFromMagic, saveBinaryResponse, sanitizeFilename, formatBytes, } from './binary-detection.ts';
// ============================================================
 ...
 */
export function saveLargeResponse(sessionPath, toolName, label, content) {
...
function saveJsonArtifact(sessionPath, toolName, suffix, content) {
    const responsesDir = join(sessionPath, LONG_RESPONSES_DIR);
    try {
        mkdirSync(responsesDir, { recursive: true });
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 23);
...
function inferMimeFromContext(container) {
...
function saveExtractedAsset(sessionPath, toolName, buffer, ext, mimeType, jsonPath, source) {
    try {
...
function extractAssetsFromStructuredJson(text, sessionPath, toolName) {
    ...
    const linked = JSON.parse(JSON.stringify(parsed));
    const assets = [];
    const walk = (node, path, parent, key) => {
        if (typeof node === 'string') {
            const extraction = extractBase64Binary(node);
            if (!extraction || !parent || key === null)
                return;
    ...
    if (!originalArtifact || !linkedArtifact)
        return null;
    return {
...
function formatStructuredMediaExtractionMessage(result) {
 ...
 */
export function buildSummarizationPrompt(text, context) {
 ...
 */
export function formatLargeResponseMessage(opts) {
 ...
 * - string: binary detection via Buffer conversion. Used by MCP pool and backend adapters
 *   where data is already a string.
 *
 * Pipeline: binary check → (if text) size check → save + summarize.
 *
 * Shared by McpClientPool.callTool(), api-tools.ts, and backend agents.
 */
export async function guardLargeResult(input, opts) {
 ...
 */
export async function handleLargeResponse(opts) {
    ...
    const saveResult = saveLargeResponse(sessionPath, context.toolName, context.path || '', text);
    if (!saveResult) {
...
//# sourceMappingURL=large-response.js.map