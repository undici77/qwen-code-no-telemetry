 ...
 * Maps provider names to their canonical domain for proper favicon resolution.
 */
const STATIC_PROVIDER_DOMAINS = Object.freeze({
 ...
 */
function loadProviderDomainsCache() {
    try {
        if (!existsSync(PROVIDER_DOMAINS_CACHE_PATH))
            return {};
        const cache = readJsonFileSync(PROVIDER_DOMAINS_CACHE_PATH);
 ...
 */
let _mergedProviderDomains = null;
/**
 ...
 */
export function getProviderDomain(provider) {
 ...
 */
export function _resetProviderDomainCache() {
 ...
 */
async function parseFaviconsFromHtml(url) {
    try {
 ...
 */
export async function getHighQualityLogoUrl(serviceUrl, provider) {
    ...
    async function tryFaviconPaths(domain) {
 ...
 * @param provider - Optional provider name (e.g., 'gmail') to use canonical domain mapping
 */
export function getLogoUrl(serviceUrl, provider) {
    // Check if provider has a direct icon URL (highest priority)
    if (provider) {
        const directIconUrl = PROVIDER_ICON_URLS[provider.toLowerCase()];
        if (directIconUrl) {
