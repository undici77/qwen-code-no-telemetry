...
export function sliceAtWord(text, max) {
    if (text.length <= max)
        return text;
...
export function truncateTitle(title, max = MAX_TITLE_LENGTH) {
 ...
 */
function isPreamblePrefix(text) {
    const lower = text.trim().toLowerCase();
    // Exact single-word preamble
    if (/^(?:title|topic|sure|okay|ok)$/.test(lower))
        return true;
    // Starts with a known opener and optionally references title/topic
    if (/^(?:sure|okay|ok|here(?:'s| is))\b/.test(lower))
        return true;
    // "the title/topic is" or similar
    if (/^the\s+(?:title|topic)\b/.test(lower))
        return true;
    return false;
}
/**
 ...
 */
export function buildTitlePrompt(message, options) {
 ...
 */
export function isLowSignal(message) {
    const trimmed = message.trim();
    if (trimmed.length > LOW_SIGNAL_MAX_CHARS)
        return false;
    if (trimmed.split(/\s+/).length > LOW_SIGNAL_MAX_WORDS)
        return false;
 ...
 */
export function selectSpreadMessages(allUserMessages) {
...
function messagesSectionLabel(count) {
    if (count === 1)
        return 'User message:';
    if (count === 2)
        return 'User messages (first, last):';
 ...
 * @returns Formatted prompt string
 */
export function buildRegenerateTitlePrompt(recentUserMessages, lastAssistantResponse, options) {
    ...
    lines.push('', messagesSectionLabel(recentUserMessages.length), userContext, '', 'Latest assistant response:', assistantSnippet, '', 'Topic:');
    return lines.join('\n');
}
