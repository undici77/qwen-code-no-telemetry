...
function shouldColorDebugOutput() {
    if (typeof process === 'undefined')
        return false;
...
function colorizeLevel(level, levelStr, enabled) {
    switch (level) {
        case 'error':
 ...
 */
function formatMessage(scope, message, args, options) {
 ...
 */
function output(formatted, consoleFormatted = formatted) {
...
export function debug(message, ...args) {
    if (!isDebugEnabled())
        return;
    const date = new Date();
    output(formatMessage(undefined, message, args, { date }), formatMessage(undefined, message, args, { color: true, date }));
}
/**
...
export function createLogger(scope) {
    const logWithLevel = (level, message, args) => {
        if (!isDebugEnabled())
            return;
        const date = new Date();
        output(formatMessage(scope, message, args, { date, level }), formatMessage(scope, message, args, { color: true, date, level }));
    };
    return {
