 ...
 */
export function detectExtensionFromMagic(buffer) {
    if (buffer.length < 8)
        return '';
 ...
 */
export function getMimeExtension(mimeType, buffer) {
    if (mimeType) {
 ...
 */
export function extractBase64Binary(text) {
    ...
    if (dataUrlMatch) {
        ...
        try {
            ...
            if (!isBinaryMime(mime) && !looksLikeBinary(decoded))
                return null;
    ...
    if (trimmed.length < MIN_BASE64_LENGTH)
        return null;
    // Quick reject: structured data delimiters
    const firstChar = trimmed.charCodeAt(0);
    if (firstChar === 0x7B || firstChar === 0x5B || firstChar === 0x3C)
        return null; // { [ <
 ...
 */
export function saveBinaryResponse(sessionPath, filename, buffer, mimeType) {
    ...
    try {
        ...
        return {
            ...
            error: `Failed to create downloads directory: ${err.message}`,
        };
    }
...
//# sourceMappingURL=binary-detection.js.map