 ...
 */
export function configurePerfTracking(options) {
 ...
 */
function updateAggregatedStats(metric) {
    if (metric.duration === undefined)
        return;
 ...
 */
export function start(name, metadata) {
 ...
 */
export async function measure(name, fn, metadata) {
...
export function span(name, metadata) {
    const startTime = performance.now();
    const marks = [];
...
export function formatStatsSummary() {
    ...
    lines.push('Operation'.padEnd(40) +
        'Count'.padStart(8) +
        'Avg'.padStart(10) +
        'P50'.padStart(10) +
        'P95'.padStart(10));
