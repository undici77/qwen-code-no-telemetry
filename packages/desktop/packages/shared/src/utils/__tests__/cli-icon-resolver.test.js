...
import { splitCommands, extractCommandName, extractCommandNames, resolveToolIcon, loadToolIconConfig, } from '../cli-icon-resolver.ts';
// ============================================
// splitCommands
// ============================================
...
describe('extractCommandNames', () => {
    ...
    test('complex mixed command', () => {
        expect(extractCommandNames('NODE_ENV=prod sudo docker-compose up && npm run migrate')).toEqual(['docker-compose', 'npm']);
    });
    test('triple chain', () => {
        expect(extractCommandNames('git add . && git commit -m "msg" && git push')).toEqual(['git', 'git', 'git']);
    });
    test('pipe chain', () => {
        expect(extractCommandNames('cat package.json | jq .scripts | head')).toEqual(['cat', 'jq', 'head']);
    });
});
// ============================================
// loadToolIconConfig
...
//# sourceMappingURL=cli-icon-resolver.test.js.map