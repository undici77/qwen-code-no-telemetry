...
describe('getHighQualityLogoUrl', () => {
    ...
    it('keeps uppercase absolute favicon hrefs from HTML', async () => {
        ...
        const fetchMock = mock((input, init) => {
            const url = String(input);
            if (init?.method === 'GET' && url === 'https://example.com') {
                return Promise.resolve(new Response(`<html><head><link rel="icon" href="${iconUrl}" sizes="any"></head></html>`, { status: 200 }));
            }
        ...
        await expect(getHighQualityLogoUrl('https://example.com')).resolves.toBe(iconUrl);
    });
});
