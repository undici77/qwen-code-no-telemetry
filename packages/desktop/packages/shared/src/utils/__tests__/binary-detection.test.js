...
import { looksLikeBinary, extractBase64Binary, detectExtensionFromMagic, getMimeExtension, formatBytes, MAX_DOWNLOAD_SIZE, } from '../binary-detection.ts';
...
const riffBuffer = (form) => Buffer.concat([
...
const LARGE_PDF_BINARY = (() => {
    ...
    for (let i = 0; i < 300; i++)
        junk[i] = i % 256; // includes 0x00 at i=0, 256, etc.
...
const TEXT_AS_BASE64 = Buffer.from('Hello, this is a plain text message that is not binary at all. '.repeat(10)).toString('base64');
...
describe('guardLargeResult (structured JSON media extraction)', () => {
    test('extracts screenshot asset from pretty JSON and writes original + linked JSON artifacts', async () => {
        ...
        const linked = JSON.parse(readFileSync(join(longResponsesDir, linkedJson), 'utf-8'));
        const imageBlock = linked[1];
...
//# sourceMappingURL=binary-detection.test.js.map