...
const en = locales["en"];
if (!en)
    throw new Error("en.json is required as the source-of-truth locale");
...
describe("i18n locale parity", () => {
    ...
    for (const [lang, translations] of Object.entries(locales)) {
        it(`${lang} keys are sorted alphabetically`, () => {
            ...
            if (firstUnsorted !== -1) {
                expect.unreachable(`Key "${keys[firstUnsorted]}" at index ${firstUnsorted} is out of order ` +
                    `(expected "${sorted[firstUnsorted]}")`);
            }
        });
    }
});
//# sourceMappingURL=locale-parity.test.js.map