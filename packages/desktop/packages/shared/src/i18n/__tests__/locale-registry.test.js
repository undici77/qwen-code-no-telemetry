...
describe("locale registry completeness", () => {
    ...
    it("every locale file on disk is registered in LOCALE_REGISTRY", () => {
        const unregistered = localeFilesOnDisk.filter((code) => !registryCodes.includes(code));
        expect(unregistered).toEqual([]);
    });
    it("every registry entry has a locale file on disk", () => {
        const missingFiles = registryCodes.filter((code) => !localeFilesOnDisk.includes(code));
...
describe("derived exports", () => {
    it("SUPPORTED_LANGUAGE_CODES matches registry keys", () => {
        const registryCodes = Object.keys(LOCALE_REGISTRY).sort();
        const supported = [...SUPPORTED_LANGUAGE_CODES].sort();
    ...
    it("LANGUAGES nativeNames match registry", () => {
        for (const code of SUPPORTED_LANGUAGE_CODES) {
            expect(LANGUAGES[code].nativeName).toBe(LOCALE_REGISTRY[code].nativeName);
        }
    });
});
// ---------------------------------------------------------------------------
...
//# sourceMappingURL=locale-registry.test.js.map