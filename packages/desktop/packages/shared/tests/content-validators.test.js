...
import { validateSourceConfigContent, validateSkillContent, validateStatusesContent, validatePermissionsContent, detectConfigFileType, validateConfigFileContent, } from '../src/config/validators.ts';
// ============================================================
...
describe('validateStatusesContent', () => {
    ...
    it('warns when fixed status does not have isFixed flag', () => {
        const config = {
            ...validStatuses,
            statuses: validStatuses.statuses.map(s => s.id === 'todo' ? { ...s, isFixed: false } : s),
        };
        const result = validateStatusesContent(JSON.stringify(config));
...
describe('detectConfigFileType', () => {
    ...
    it('detects source config files', () => {
        const result = detectConfigFileType(`${workspaceRoot}/sources/github/config.json`, workspaceRoot);
    ...
    it('detects skill SKILL.md files', () => {
        const result = detectConfigFileType(`${workspaceRoot}/skills/commit/SKILL.md`, workspaceRoot);
        expect(result).not.toBeNull();
    ...
    it('detects statuses config', () => {
        const result = detectConfigFileType(`${workspaceRoot}/statuses/config.json`, workspaceRoot);
        expect(result).not.toBeNull();
    ...
    it('detects workspace-level permissions.json', () => {
        const result = detectConfigFileType(`${workspaceRoot}/permissions.json`, workspaceRoot);
        expect(result).not.toBeNull();
    ...
    it('detects source-level permissions.json', () => {
        const result = detectConfigFileType(`${workspaceRoot}/sources/linear/permissions.json`, workspaceRoot);
        expect(result).not.toBeNull();
        expect(result.type).toBe('permissions');
    ...
    it('returns null for files outside workspace root', () => {
        const result = detectConfigFileType('/some/other/path/config.json', workspaceRoot);
        expect(result).toBeNull();
    });
    it('returns null for non-config files in workspace', () => {
        const result = detectConfigFileType(`${workspaceRoot}/sources/github/guide.md`, workspaceRoot);
        expect(result).toBeNull();
    });
    it('returns null for nested non-matching paths', () => {
        const result = detectConfigFileType(`${workspaceRoot}/sources/github/deep/config.json`, workspaceRoot);
        expect(result).toBeNull();
    });
});
// ============================================================
...
describe('validateConfigFileContent', () => {
    it('dispatches to source validator', () => {
        const detection = { type: 'source', slug: 'test', displayFile: 'sources/test/config.json' };
    ...
    it('dispatches to skill validator', () => {
        const detection = { type: 'skill', slug: 'my-skill', displayFile: 'skills/my-skill/SKILL.md' };
        const content = `---
name: Test
description: Test skill
---
...
`;
    ...
    it('dispatches to statuses validator', () => {
        const detection = { type: 'statuses', displayFile: 'statuses/config.json' };
        const config = JSON.stringify({
    ...
    it('dispatches to permissions validator', () => {
        const detection = { type: 'permissions', displayFile: 'permissions.json' };
        const result = validateConfigFileContent(detection, '{}');
        expect(result).not.toBeNull();
        expect(result.valid).toBe(true);
    });
    it('returns validation errors for invalid content', () => {
        const detection = { type: 'source', slug: 'bad', displayFile: 'sources/bad/config.json' };
        const result = validateConfigFileContent(detection, '{ not valid json }');
        expect(result).not.toBeNull();
        expect(result.valid).toBe(false);
    });
});
//# sourceMappingURL=content-validators.test.js.map