...
function createTestSession(id, workspaceRootPath, sdkSessionId) {
    return {
        id,
        workspaceRootPath,
        createdAt: Date.now(),
...
describe('SessionPersistenceQueue', () => {
    ...
    it('allows parallel writes to different sessions', async () => {
        ...
        const contentA = readFileSync(join(testDir, 'sessions', 'session-a', 'session.jsonl'), 'utf-8');
        const contentB = readFileSync(join(testDir, 'sessions', 'session-b', 'session.jsonl'), 'utf-8');
        expect(JSON.parse(contentA.split('\n')[0]).sdkSessionId).toBe('id-a');
        expect(JSON.parse(contentB.split('\n')[0]).sdkSessionId).toBe('id-b');
    });
});
//# sourceMappingURL=persistence-queue.test.js.map