...
import { hasDangerousSubstitution, hasDangerousControlChars, isReadOnlyBashCommand, isReadOnlyBashCommandWithConfig, getBashRejectionReason, formatBashRejectionMessage, shouldAllowToolInMode, extractBashWriteTarget, looksLikePotentialWrite, SAFE_MODE_CONFIG, } from '../src/agent/mode-manager.ts';
// ============================================================
// Test Configuration
// ============================================================
...
describe('grep with regex patterns containing shell metacharacters', () => {
    ...
    describe('unquoted patterns with operators should be detected', () => {
        it('should detect pipeline when | is unquoted in grep pattern', () => {
            ...
            const reason = getBashRejectionReason('grep model.*selector|ModelSelector /Users/test', grepConfig);
            expect(reason).not.toBeNull();
...
describe('shouldAllowToolInMode - Bash plans folder exception', () => {
    ...
    describe('should allow bash writes to plans folder in safe mode', () => {
        ...
        it.skipIf(!isWindows)('should allow PowerShell Out-File to plans folder', () => {
            ...
            const result = shouldAllowToolInMode('Bash', { command }, 'safe', { plansFolderPath: windowsPlansFolderPath });
            expect(result.allowed).toBe(true);
        });
    ...
    describe('should allow Write/Edit to plans folder with case-insensitive paths', () => {
        it.skipIf(!isWindows)('should allow Write with different case in path (Windows compatibility)', () => {
            ...
            const plansFolderPath = 'C:\\Users\\Balin\\.craft-agent\\workspaces\\ws\\sessions\\s1\\plans';
            const result = shouldAllowToolInMode('Write', { file_path: 'C:\\Users\\balin\\.craft-agent\\workspaces\\ws\\sessions\\s1\\plans\\plan.md', content: '# Plan' }, 'safe', { plansFolderPath });
            expect(result.allowed).toBe(true);
        });
        it.skipIf(!isWindows)('should allow Edit with different case in path (Windows compatibility)', () => {
            const plansFolderPath = 'C:\\Users\\Balin\\.craft-agent\\plans';
            const result = shouldAllowToolInMode('Edit', { file_path: 'C:\\Users\\balin\\.craft-agent\\plans\\plan.md', old_string: 'old', new_string: 'new' }, 'safe', { plansFolderPath });
            expect(result.allowed).toBe(true);
        });
    });
...
//# sourceMappingURL=mode-manager.test.js.map