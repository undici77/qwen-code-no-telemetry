...
import { buildClassifierContents, MAX_TRANSCRIPT_MESSAGES, } from './classifier-transcript.js';
import { DeclarativeTool, } from '../tools/tools.js';
import { Kind } from '../tools/tools.js';
class StubTool extends DeclarativeTool {
    projection;
...
describe('buildClassifierContents', () => {
    ...
    it('strips model text parts (anti self-injection) and renders historical functionCalls as user-role text', () => {
        ...
        const priorActionTurn = result.find((c) => ((c.parts?.[0]).text ?? '').startsWith('Prior action:'));
        expect(priorActionTurn).toBeDefined();
        const priorText = (priorActionTurn.parts?.[0]).text;
    ...
    it('projects historical functionCall args through tool.toAutoClassifierInput', () => {
        ...
        const priorText = (result[0].parts?.[0]).text;
    ...
    it('drops empty historical user turns but keeps the pending-action user turn', () => {
        ...
        expect((userTurns[0].parts?.[0]).text).toBe('real message');
        expect((userTurns[1].parts?.[0]).text).toContain('read_file');
    });
...
//# sourceMappingURL=classifier-transcript.test.js.map