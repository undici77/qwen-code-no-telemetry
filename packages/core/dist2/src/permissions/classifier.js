...
export async function classifyAction(input) {
    ...
    try {
        ...
        return failClosed('Classifier prompt construction failed', err, 'fast', overallStart, input.config);
    }
    ...
    try {
        ...
        if (input.signal.aborted)
            throw err;
        return failClosed('Classifier stage 1 unavailable', err, 'fast', overallStart, input.config);
    }
...
function failClosed(baseMessage, err, stage, startedAt, config) {
    ...
    debugLogger.warn(`failClosed stage=${stage} durationMs=${Date.now() - startedAt} ` +
