...
import { findDangerousAllowRules, isDangerousAgentRule, isDangerousAllowRule, isDangerousBashRule, isDangerousSkillRule, } from './dangerousRules.js';
import { ToolNames } from '../tools/tool-names.js';
function bashRule(specifier) {
    return {
...
describe('isDangerousBashRule', () => {
    ...
    it.each(['npx *', 'uvx *', 'pipx *', 'go run *', 'go install *'])('flags modern-runner wildcard %s — common attack pattern via fetch-and-execute', (s) => {
        expect(isDangerousBashRule(bashRule(s))).toBe(true);
    });
    it.each(['python:*', 'node:*', 'bash:*'])('flags interpreter prefix-form %s', (s) => {
        expect(isDangerousBashRule(bashRule(s))).toBe(true);
    });
    it.each(['python*', 'node*', 'bash*'])('flags interpreter wildcard-form %s', (s) => {
        expect(isDangerousBashRule(bashRule(s))).toBe(true);
    });
    ...
    it('does NOT flag specific colon-form rules (concrete suffix is a user-allowed concrete command)', () => {
        ...
        expect(isDangerousBashRule(bashRule('python:./scripts/build.py'))).toBe(false);
...
describe('isDangerousAgentRule', () => {
    ...
    it('returns false for non-Agent tools', () => {
        expect(isDangerousAgentRule({ raw: 'Bash', toolName: ToolNames.SHELL })).toBe(false);
    });
});
describe('isDangerousSkillRule', () => {
    it('flags any Skill allow rule', () => {
        expect(isDangerousSkillRule({ raw: 'Skill', toolName: ToolNames.SKILL })).toBe(true);
        expect(isDangerousSkillRule({
            raw: 'Skill(pdf)',
...
describe('isDangerousAllowRule (aggregate)', () => {
    it('returns true for any dangerous category', () => {
        ...
        expect(isDangerousAllowRule({ raw: 'Agent', toolName: ToolNames.AGENT })).toBe(true);
        expect(isDangerousAllowRule({ raw: 'Skill', toolName: ToolNames.SKILL })).toBe(true);
    });
    it('returns false for safe rules', () => {
        expect(isDangerousAllowRule({
            raw: 'Read',
            toolName: ToolNames.READ_FILE,
        })).toBe(false);
...
describe('findDangerousAllowRules', () => {
    it('returns only the dangerous subset', () => {
        ...
        expect(dangerous.map((r) => r.toolName).sort()).toEqual([ToolNames.AGENT, ToolNames.SHELL].sort());
    });
...
//# sourceMappingURL=dangerousRules.test.js.map