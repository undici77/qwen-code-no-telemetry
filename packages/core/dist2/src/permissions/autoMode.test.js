...
import { SAFE_TOOL_ALLOWLIST, evaluateAutoMode, formatClassifierBlockMessage, isInSafeToolAllowlist, passesAcceptEditsFastPath, shouldRunAutoModeForCall, } from './autoMode.js';
 ...
 */
function makeConfig(workspaceRoots) {
    return {
        getWorkspaceContext: () => ({
            ...
            isPathWithinWorkspace: (p) => workspaceRoots.some((root) => p === root || p.startsWith(root + '/')),
        }),
    };
}
function ctx(over) {
...
describe('passesAcceptEditsFastPath', () => {
    ...
    it('allows EDIT targeting a path inside cwd', () => {
        expect(passesAcceptEditsFastPath(ctx({ toolName: ToolNames.EDIT, filePath: `${cwd}/src/foo.ts` }), config)).toBe(true);
    });
    it('allows WRITE_FILE targeting a path inside cwd', () => {
        expect(passesAcceptEditsFastPath(ctx({ toolName: ToolNames.WRITE_FILE, filePath: `${cwd}/x.ts` }), config)).toBe(true);
    });
    it('rejects EDIT targeting a path outside the workspace', () => {
        expect(passesAcceptEditsFastPath(ctx({
            toolName: ToolNames.EDIT,
            filePath: '/Users/test/other-project/x.ts',
        }), config)).toBe(false);
    });
    it('rejects WRITE_FILE targeting /etc/hosts', () => {
        expect(passesAcceptEditsFastPath(ctx({ toolName: ToolNames.WRITE_FILE, filePath: '/etc/hosts' }), config)).toBe(false);
    });
    it('rejects when filePath is missing', () => {
        expect(passesAcceptEditsFastPath(ctx({ toolName: ToolNames.EDIT, filePath: undefined }), config)).toBe(false);
    });
    ...
    it('rejects allowlisted read-only tools', () => {
        expect(passesAcceptEditsFastPath(ctx({ toolName: ToolNames.READ_FILE, filePath: `${cwd}/x.ts` }), config)).toBe(false);
    });
    ...
    it('calls workspace context isPathWithinWorkspace for the actual path check', () => {
        ...
        passesAcceptEditsFastPath(ctx({ toolName: ToolNames.EDIT, filePath: '/some/path/x.ts' }), cfg);
...
describe('formatClassifierBlockMessage', () => {
    ...
    it('renders a policy-block message including the reason', () => {
        expect(formatClassifierBlockMessage({
            ...baseDecision,
    ...
    it('renders an unavailable message with cause when reason is present', () => {
        expect(formatClassifierBlockMessage({
            ...
            unavailable: true,
        })).toBe('Auto mode classifier unavailable (Conversation transcript exceeds classifier context window); action blocked for safety');
    });
...
describe('shouldRunAutoModeForCall', () => {
    ...
    it('excludes ASK_USER_QUESTION even under AUTO — must always reach the user', () => {
        expect(shouldRunAutoModeForCall(ApprovalMode.AUTO, ToolNames.ASK_USER_QUESTION)).toBe(false);
    });
    it('excludes EXIT_PLAN_MODE even under AUTO — plan exits are operator-driven', () => {
        expect(shouldRunAutoModeForCall(ApprovalMode.AUTO, ToolNames.EXIT_PLAN_MODE)).toBe(false);
    });
    it('returns false for unknown tool names when not in AUTO', () => {
        expect(shouldRunAutoModeForCall(ApprovalMode.DEFAULT, 'unknown_tool')).toBe(false);
    });
});
//# sourceMappingURL=autoMode.test.js.map