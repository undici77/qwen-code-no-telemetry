...
describe('auto-memory storage scaffold', () => {
    ...
    it('builds stable auto-memory paths under project .qwen directory', () => {
        expect(getAutoMemoryRoot(projectRoot)).toBe(path.join(projectRoot, '.qwen', 'memory'));
        expect(getAutoMemoryIndexPath(projectRoot)).toBe(path.join(projectRoot, '.qwen', 'memory', 'MEMORY.md'));
        expect(getAutoMemoryMetadataPath(projectRoot)).toBe(path.join(projectRoot, '.qwen', 'meta.json'));
        expect(getAutoMemoryExtractCursorPath(projectRoot)).toBe(path.join(projectRoot, '.qwen', 'extract-cursor.json'));
        expect(getAutoMemoryConsolidationLockPath(projectRoot)).toBe(path.join(projectRoot, '.qwen', 'consolidation.lock'));
        expect(getAutoMemoryTopicPath(projectRoot, 'feedback')).toBe(path.join(projectRoot, '.qwen', 'memory', 'feedback.md'));
    });
