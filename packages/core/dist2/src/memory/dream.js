...
async function runDreamByAgent(projectRoot, config, abortSignal) {
    const result = await planManagedAutoMemoryDreamByAgent(config, projectRoot, abortSignal);
...
export async function runManagedAutoMemoryDream(projectRoot, now = new Date(), config, abortSignal) {
