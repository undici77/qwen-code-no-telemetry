...
import { AGENT_CONTEXT_FILENAME, DEFAULT_CONTEXT_FILENAME, MEMORY_SECTION_HEADER, setGeminiMdFilename, } from './const.js';
...
describe('writeWorkspaceContextFile', () => {
    ...
    it('creates QWEN.md with a fresh section header on first append', async () => {
        ...
        expect(result.filePath).toBe(path.join(workspace, DEFAULT_CONTEXT_FILENAME));
    ...
    it('appends under existing section header', async () => {
        ...
        expect(written).toBe(`# project notes\n\n${MEMORY_SECTION_HEADER}\n- first entry\n- second entry\n`);
    });
    ...
    it('writes to the global ~/.qwen directory when scope=global', async () => {
        ...
        expect(result.filePath).toBe(path.join(globalDir, DEFAULT_CONTEXT_FILENAME));
    ...
    it('creates the parent directory when missing', async () => {
        ...
        const created = await fs.readFile(path.join(nested, DEFAULT_CONTEXT_FILENAME), 'utf8');
        expect(created).toContain('- entry');
    });
    it('rejects non-absolute projectRoot', async () => {
        await expect(writeWorkspaceContextFile({
            scope: 'workspace',
            mode: 'append',
            content: 'x',
    ...
    it('serializes concurrent appends so no entry is lost', async () => {
        ...
        const PARALLEL = 10;
        const writes = Array.from({ length: PARALLEL }, (_, i) => writeWorkspaceContextFile({
            scope: 'workspace',
            mode: 'append',
            content: `- entry ${i}`,
            projectRoot: workspace,
    ...
    it('marks `changed: false` for a no-op append against a missing file', async () => {
        ...
        expect(result.changed).toBe(false);
        expect(result.bytesWritten).toBe(0);
        await expect(fs.access(path.join(workspace, DEFAULT_CONTEXT_FILENAME))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    ...
    it('honors setGeminiMdFilename overrides so POST targets the same file GET surfaces', async () => {
        ...
        try {
            ...
            expect(result.filePath).toBe(path.join(workspace, AGENT_CONTEXT_FILENAME));
            const written = await fs.readFile(result.filePath, 'utf8');
...
//# sourceMappingURL=writeContextFile.test.js.map