...
describe('MemoryManager', () => {
    ...
    describe('scheduleSkillReview()', () => {
        ...
        it('skips second call while first is still in-flight (already_running)', async () => {
            let resolveReview;
            vi.mocked(runSkillReviewByAgent).mockReturnValueOnce(new Promise((resolve) => {
        ...
        it('schedules skill review at threshold', async () => {
            ...
            expect(mgr.listTasksByType('skill-review', '/project')[0]?.status).toBe('completed');
        });
    });
    // ─── listTasksByType() ────────────────────────────────────────────────────
    ...
    describe('cancelTask()', () => {
        ...
        it('aborts the dream fork agent and marks the record cancelled', async () => {
            ...
            let capturedSignal;
            let resolveDreamStarted;
            const dreamStarted = new Promise((r) => {
                resolveDreamStarted = r;
            });
            vi.mocked(runManagedAutoMemoryDream).mockImplementation(async (_root, _now, _config, signal) => {
        ...
        it('keeps the record cancelled even when runManagedAutoMemoryDream resolves successfully after abort', async () => {
            ...
            const metaRaw = await fs.readFile(getAutoMemoryMetadataPath(projectRoot), 'utf-8');
