...
import { buildRelevantAutoMemoryPrompt, resolveRelevantAutoMemoryPromptForQuery, selectRelevantAutoMemoryDocuments, } from './recall.js';
...
describe('auto-memory relevant recall', () => {
    ...
    it('selects the most relevant documents for a query', () => {
        const selected = selectRelevantAutoMemoryDocuments('check the dashboard reference for latency', docs);
    ...
    it('uses model-driven selection when config is provided', async () => {
        ...
        const result = await resolveRelevantAutoMemoryPromptForQuery('/tmp/project', 'check the dashboard reference for latency', {
            config: {},
        });
    ...
    it('falls back to heuristic selection when model-driven selection fails', async () => {
        vi.mocked(scanAutoMemoryTopicDocuments).mockResolvedValue(docs);
        vi.mocked(selectRelevantAutoMemoryDocumentsByModel).mockRejectedValue(new Error('selector failed'));
        const result = await resolveRelevantAutoMemoryPromptForQuery('/tmp/project', 'check the dashboard reference for latency', {
            config: {},
            excludedFilePaths: ['/tmp/user.md'],
        });
        expect(result.strategy).toBe('heuristic');
        expect(result.selectedDocs.map((doc) => doc.filePath)).toContain('/tmp/reference.md');
        expect(result.selectedDocs.map((doc) => doc.filePath)).not.toContain('/tmp/user.md');
    });
});
