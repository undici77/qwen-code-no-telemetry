...
describe('managed auto-memory prompt helpers', () => {
    ...
    it('embeds the current MEMORY.md index content', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', '- [User Memory](user/terse.md) — User prefers terse responses.');
    ...
    it('appends managed auto-memory after existing hierarchical memory', () => {
        const result = appendManagedAutoMemoryToUserMemory('--- Context from: QWEN.md ---\nProject rules', '/tmp/project/.qwen/memory', '- [Project Memory](project/release-freeze.md) — Release freeze starts Friday.');
        expect(result).toContain('Project rules');
        expect(result).toContain('\n\n---\n\n');
        expect(result).toContain('# auto memory');
    });
    it('returns only managed auto-memory when hierarchical memory is empty', () => {
        const result = appendManagedAutoMemoryToUserMemory('   ', '/tmp/project/.qwen/memory', '- [Reference](reference/grafana.md) — Grafana dashboard link.');
        expect(result).toContain('# auto memory');
    ...
    it('truncates oversized managed auto-memory index content', () => {
        ...
        const result = buildManagedAutoMemoryPrompt('/tmp/project/.qwen/memory', oversizedIndex);
