...
import { escapePath, resolvePath, validatePath, resolveAndValidatePath, unescapePath, isSubpath, shortenPath, tildeifyPath, expandHomeDir, getProjectHash, _resetValidatePathCacheForTest, } from './paths.js';
...
describe('expandHomeDir', () => {
    ...
    it('should return regular absolute path unchanged (but normalized)', () => {
        expect(expandHomeDir('/absolute/path')).toBe(path.normalize('/absolute/path'));
    });
    it('should return relative path unchanged (but normalized)', () => {
        expect(expandHomeDir('relative/path')).toBe(path.normalize('relative/path'));
    });
});
