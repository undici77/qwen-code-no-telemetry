...
describe('truncateAndSaveToFile', () => {
    ...
    it('should return content unchanged if below both threshold and line limit', async () => {
        ...
        const result = await truncateAndSaveToFile(content, fileName, projectTempDir, THRESHOLD, TRUNCATE_LINES);
    ...
    it('should truncate when line limit exceeded even if under character threshold', async () => {
        ...
        const result = await truncateAndSaveToFile(content, fileName, projectTempDir, THRESHOLD, TRUNCATE_LINES);
        expect(result.outputFile).toBe(path.join(projectTempDir, `${fileName}.output`));
    ...
    it('should reduce effective lines when line content would exceed character threshold', async () => {
        ...
        const truncatedPart = result.content.split('Truncated part of the output:\n')[1];
    ...
    it('should truncate content by lines when line limit is the binding constraint', async () => {
        ...
        expect(mockWriteFile).toHaveBeenCalledWith(path.join(projectTempDir, `${fileName}.output`), content);
    ...
    it('should sanitize fileName to prevent path traversal', async () => {
        ...
        mockWriteFile.mockResolvedValue(undefined);
        await truncateAndSaveToFile(content, fileName, projectTempDir, THRESHOLD, TRUNCATE_LINES);
