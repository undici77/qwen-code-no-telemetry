...
describe('runForkedAgent (cache path)', () => {
    ...
    it('routes a cross-auth fast model through a runtime content-generator view', async () => {
        ...
        saveCacheSafeParams({
            systemInstruction: 'You are helpful',
        }, [{ role: 'user', parts: [{ text: 'hello' }] }], 'claude-main');
        const mockSendMessageStream = vi.fn((_model, _params, _promptId) => {
            async function* generate() {
                yield {
                    type: StreamEventType.CHUNK,
                    value: {
        ...
        const mockConfig = {
            ...
            getAllConfiguredModels: vi.fn((authTypes) => authTypes?.includes(AuthType.USE_OPENAI)
                ? [
                    {
                        id: fastModel,
                        label: fastModel,
        ...
        expect(result.text).toBe('commit this');
        expect(createRuntimeContentGeneratorView).toHaveBeenCalledWith(mockConfig, mockConfig, fastModel, { authType: AuthType.USE_OPENAI });
        expect(mockSendMessageStream).toHaveBeenCalledWith(fastModel, expect.objectContaining({
            message: [{ text: 'suggest something' }],
        }), 'forked_query');
    });
    it('routes a same-auth fast model through a runtime content-generator view when the model differs', async () => {
        ...
        saveCacheSafeParams({
            systemInstruction: 'You are helpful',
        }, [{ role: 'user', parts: [{ text: 'hello' }] }], 'gpt-4');
        const mockSendMessageStream = vi.fn((_model, _params, _promptId) => {
            async function* generate() {
                yield {
                    type: StreamEventType.CHUNK,
    ...
    it('falls back to the parent model when `fast` cannot resolve (no fast model configured)', async () => {
        ...
        saveCacheSafeParams({ systemInstruction: 'You are helpful' }, [{ role: 'user', parts: [{ text: 'hello' }] }], 'parent-model');
        let capturedModel;
        const mockSendMessageStream = vi.fn((model, _params, _promptId) => {
            capturedModel = model;
            async function* generate() {
                yield {
                    type: StreamEventType.CHUNK,
