 ...
 */
export async function truncateAndSaveToFile(content, fileName, projectTempDir, threshold, truncateLines) {
 ...
 */
export async function truncateToolOutput(config, toolName, content) {
    ...
    const result = await truncateAndSaveToFile(content, fileName, config.storage.getProjectTempDir(), threshold, lines);
    if (result.outputFile) {
        logToolOutputTruncated(config, new ToolOutputTruncatedEvent('', {
            toolName,
