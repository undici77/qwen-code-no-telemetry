...
describe('normalizeProxyUrl', () => {
    ...
    it('should not modify URL that already has http:// prefix', () => {
        expect(normalizeProxyUrl('http://127.0.0.1:7860')).toBe('http://127.0.0.1:7860');
    });
    it('should not modify URL that already has https:// prefix', () => {
        expect(normalizeProxyUrl('https://proxy.example.com:443')).toBe('https://proxy.example.com:443');
    });
    ...
    it('should handle proxy URL with authentication', () => {
        expect(normalizeProxyUrl('user:pass@proxy.example.com:8080')).toBe('http://user:pass@proxy.example.com:8080');
    });
    it('should handle proxy URL with authentication and http:// prefix', () => {
        expect(normalizeProxyUrl('http://user:pass@proxy.example.com:8080')).toBe('http://user:pass@proxy.example.com:8080');
    });
    ...
    it('should throw error for socks:// proxy URL', () => {
        expect(() => normalizeProxyUrl('socks://proxy.example.com:1080')).toThrow('SOCKS proxy is not supported');
    });
    it('should throw error for socks4:// proxy URL', () => {
        expect(() => normalizeProxyUrl('socks4://proxy.example.com:1080')).toThrow('SOCKS proxy is not supported');
    });
    it('should throw error for socks5:// proxy URL', () => {
        expect(() => normalizeProxyUrl('socks5://proxy.example.com:1080')).toThrow('SOCKS proxy is not supported');
    });
    it('should throw error for SOCKS5:// proxy URL (case insensitive)', () => {
        expect(() => normalizeProxyUrl('SOCKS5://proxy.example.com:1080')).toThrow('SOCKS proxy is not supported');
    });
});
//# sourceMappingURL=proxyUtils.test.js.map