...
Use hooks.`);
        ...
        it('formats rules with source markers', async () => {
            await createTestFile(path.join(projectRoot, QWEN_DIR, 'rules', 'test.md'), 'Content.');
            const result = await loadRules(projectRoot, true);
            expect(result.content).toContain(`--- Rule from: ${QWEN_DIR}/rules/test.md ---`);
        });
        it('reads global rules from QWEN_HOME when set', async () => {
            ...
            try {
                await createTestFile(path.join(customQwenHome, 'rules', 'fromCustomHome.md'), 'CustomHome rule.');
                // A stale rule in the legacy ~/.qwen/rules location should NOT be
                // loaded once QWEN_HOME points elsewhere.
                await createTestFile(path.join(homedir, QWEN_DIR, 'rules', 'fromLegacyHome.md'), 'LegacyHome rule.');
                const result = await loadRules(projectRoot, true);
