...
describe('memoryImportProcessor', () => {
    beforeEach(() => {
        ...
        mockedFs.lstat.mockRejectedValue(Object.assign(new Error('ENOENT'), { code: 'ENOENT' }));
    });
    afterEach(() => {
    ...
    describe('processImports', () => {
        ...
        it('should silently preserve content when file not found (ENOENT)', async () => {
            ...
            mockedFs.access.mockRejectedValue(Object.assign(new Error('ENOENT: no such file or directory'), {
                code: 'ENOENT',
            }));
            const result = await processImports(content, basePath);
        ...
        it('should log error for non-ENOENT file access errors', async () => {
            ...
            mockedFs.access.mockRejectedValue(Object.assign(new Error('Permission denied'), { code: 'EACCES' }));
            const result = await processImports(content, basePath);
            // Should show error comment for non-ENOENT errors
            expect(result.content).toContain('<!-- Import failed: ./permission-denied.md - Permission denied -->');
            expect(console.error).not.toHaveBeenCalled();
        });
        ...
        it('should ignore imports inside code blocks', async () => {
            ...
            const result = await processImports(content, basePath, undefined, projectRoot);
        ...
        it('should still import valid paths while ignoring non-existent paths', async () => {
            ...
            mockedFs.access
                ...
                .mockRejectedValueOnce(Object.assign(new Error('ENOENT'), { code: 'ENOENT' })); // 中文路径 doesn't exist
            mockedFs.readFile.mockResolvedValue(importedContent);
            const result = await processImports(content, basePath);
        ...
        it('should produce flat output in Claude-style with unique files in order', async () => {
            ...
            mockedFs.readFile
                ...
                .mockResolvedValueOnce(simpleContent)
                .mockResolvedValueOnce(innerContent);
            const result = await processImports(content, basePath, undefined, projectRoot, 'flat');
        ...
        it('should not duplicate files in flat output if imported multiple times', async () => {
            ...
            const result = await processImports(content, basePath, undefined, // allowedPaths
            projectRoot, 'flat');
