...
describe('debugLogger', () => {
    ...
    describe('createDebugLogger', () => {
        ...
        it('uses real OTel span context when an active span exists', async () => {
            ...
            await vi.runAllTimersAsync();
            expect(fs.appendFile).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('[trace_id=realtraceidddddddddddddddddddddd span_id=realspanid111111]'), 'utf8');
        });
        it('omits trace context when active span is noop and telemetry context is unset', async () => {
            ...
            await vi.runAllTimersAsync();
            expect(fs.appendFile).toHaveBeenCalledWith(expect.any(String), expect.not.stringContaining('trace_id='), 'utf8');
        });
        ...
        it('uses the session root span context for fallback trace context', async () => {
            const sessionRootContext = { root: true };
            setSessionContext(sessionRootContext, 'test-session');
            vi.mocked(trace.getSpan).mockImplementation((ctx) => ctx === sessionRootContext
                ? {
                    spanContext: () => ({
            ...
            await vi.runAllTimersAsync();
            expect(fs.appendFile).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('[trace_id=cccccccccccccccccccccccccccccccc span_id=dddddddddddddddd]'), 'utf8');
        });
