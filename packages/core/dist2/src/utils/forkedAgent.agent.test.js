...
import { hasRebuiltToolRegistry, TOOL_REGISTRY_REBUILT, } from '../tools/agent/agent.js';
...
function makeRuntimeView(model) {
    return {
        contentGenerator: {},
...
describe('runForkedAgent (AgentHeadless path) bound-tool isolation', () => {
    ...
    function captureAgentHeadlessConfig() {
        const captured = { config: undefined };
        const spy = vi
            .spyOn(AgentHeadless, 'create')
            .mockImplementation(async (_name, config, ..._rest) => {
    ...
    it('passes a Config with the rebuilt-registry marker and YOLO approval mode to AgentHeadless.create', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(captured.config[TOOL_REGISTRY_REBUILT]).toBe(true);
    ...
    it('binds EditTool from the wrapper registry to the wrapper Config (not the parent)', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(editTool.config).toBe(captured.config);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const boundConfig = editTool.config;
        expect(boundConfig.getApprovalMode()).toBe(ApprovalMode.YOLO);
        expect(boundConfig.getFileReadCache()).toBe(captured.config.getFileReadCache());
    ...
    it('preserves an upstream getPermissionManager override (memory-scoped composition)', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        parent.toolRegistry = parentRegistry;
        const scopedPm = { id: 'scoped-pm-marker' };
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const scopedConfig = Object.create(parent);
    ...
    it('stops the per-fork ToolRegistry even when headless.execute rejects', async () => {
        ...
        const createSpy = vi.spyOn(AgentHeadless, 'create').mockImplementation(async (..._args) => ({
            execute: vi
                .fn()
    ...
    it('uses a runtime content-generator view for cross-auth fast models', async () => {
        ...
        vi.spyOn(parent, 'getFastModel').mockReturnValue(`${AuthType.USE_OPENAI}:${fastModel}`);
        vi.spyOn(parent, 'getAllConfiguredModels').mockImplementation((authTypes) => authTypes?.includes(AuthType.USE_OPENAI)
            ? [
                {
                    id: fastModel,
                    label: fastModel,
        ...
        const createSpy = vi
            .spyOn(AgentHeadless, 'create')
            .mockImplementation(async (_name, config, _promptConfig, modelConfig, _runConfig, _toolConfig, _eventEmitter, _hooks, runtimeViewArg) => {
            captured.config = config;
        ...
        expect(createRuntimeContentGeneratorView).toHaveBeenCalledWith(parent, captured.config, fastModel, { authType: AuthType.USE_OPENAI });
    });
});
//# sourceMappingURL=forkedAgent.agent.test.js.map