...
import { crawl, __setCommandRunnerForTests, __resetCrawlerStateForTests, } from './crawler.js';
...
async function runExecFile(command, args, cwd) {
    await new Promise((resolve, reject) => {
        childProcess.execFile(command, args, { cwd, windowsHide: true }, (error) => {
            if (error) {
                reject(error);
                return;
            }
...
describe('crawler', () => {
    ...
    describe('with in-memory cache', () => {
        ...
        it('should hit the cache for subsequent crawls', async () => {
            ...
            const canonicalDir = path
                ...
                .split(path.sep)
                .join('/');
            const cacheKey = cache.getCacheKey(canonicalDir, options.ignore.getFingerprint(), undefined);
    ...
    describe('two-tier strategy: git ls-files + ripgrep fallback', () => {
        it('should use git ls-files in a git repo', async () => {
            ...
            expect(results).toEqual(expect.arrayContaining(['.', 'src/', 'file1.js', 'src/file2.js']));
        });
        ...
        it('should fall back to fdir when not in a git repo and ripgrep unavailable', async () => {
            ...
            expect(results).toEqual(expect.arrayContaining(['.', 'lib/', 'index.js', 'lib/util.js']));
            const degradationWarns = warnSpy.mock.calls.filter((c) => typeof c[0] === 'string' &&
                c[0].startsWith('[crawler] falling back to'));
        ...
        it('should respect maxDepth on git ls-files path', async () => {
            ...
            expect(results).toEqual(expect.arrayContaining(['.', 'root/', 'nested/']));
        ...
        it('should not run git ls-files --cached on second crawl when throttled and working tree unchanged', async () => {
            ...
            expect(gitCalls.some((a) => a.includes('-z') &&
                a.includes('ls-files') &&
                (a.includes('--others') || a.includes('--deleted')))).toBe(true);
            await crawl(options);
        ...
        it('should fall back to ripgrep when git ls-files --cached fails inside a git repo', async () => {
            ...
            const degradationCalls = warnSpy.mock.calls
                .map((c) => c[0])
                .filter((m) => typeof m === 'string' && m.startsWith('[crawler] falling back to'));
            expect(degradationCalls).toContain('[crawler] falling back to ripgrep (git ls-files unavailable)');
            warnSpy.mockRestore();
        });
    ...
    describe('mtime-based change detection', () => {
        ...
        it('should re-crawl when git index mtime changes', async () => {
            ...
            await fs.utimes(path.join(tmpDir, '.git', 'index'), futureTime, futureTime);
            const results2 = await crawl(options);
        ...
        it('should re-crawl git worktrees when the gitdir index changes', async () => {
            ...
            __setCommandRunnerForTests(async (command, args, cwd) => {
                if (command !== 'git') {
                    return { success: false, lines: [] };
                }
                if (args.includes('rev-parse') && args.includes('--show-toplevel')) {
            ...
            const second = await crawl(options);
            expect(second).toEqual(expect.arrayContaining(['.', 'tracked.txt', 'new-file.txt']));
        });
    });
});
