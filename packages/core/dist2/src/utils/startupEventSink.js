 ...
 */
export function setStartupEventSink(handler) {
    sink = handler;
}
/**
...
export function recordStartupEvent(name, attrs) {
    if (sink) {
        try {
            ...
            debugLogger.error(`startup event sink threw for '${name}': ${err instanceof Error ? err.message : String(err)}`);
        }
    }
}
//# sourceMappingURL=startupEventSink.js.map