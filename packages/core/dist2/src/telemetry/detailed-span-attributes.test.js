...
import { truncateContent, addUserPromptAttributes, addSystemPromptAttributes, addToolSchemaAttributes, addModelOutputAttributes, addToolInputAttributes, addToolResultAttributes, clearDetailedSpanState, } from './detailed-span-attributes.js';
function createMockConfig() {
    return {
        getTelemetryIncludeSensitiveSpanAttributes: () => mockState.sensitiveEnabled,
    };
}
...
describe('detailed-span-attributes', () => {
    ...
    describe('addToolSchemaAttributes', () => {
        it('sets tools summary and count', () => {
            ...
            const toolsSummary = JSON.parse(span.attrs['tools']);
        ...
        it('flattens functionDeclarations wrapper (Gemini API shape)', () => {
            ...
            expect(span.attrs['tools_count']).toBe(2);
            const toolsSummary = JSON.parse(span.attrs['tools']);
            expect(toolsSummary.map((t) => t.name)).toEqual([
                'Read',
                'Bash',
            ]);
    ...
    describe('addToolInputAttributes', () => {
        it('sets tool_input with prefix', () => {
            ...
            expect(span.attrs['tool_input']).toBe('[TOOL INPUT: Bash]\n{"command":"ls"}');
        });
        it('no-ops when flag is disabled', () => {
            mockState.sensitiveEnabled = false;
            const config = createMockConfig();
    ...
    describe('addToolResultAttributes', () => {
        it('sets tool_result with prefix', () => {
            ...
            expect(span.attrs['tool_result']).toBe('[TOOL RESULT: Read]\nfile contents here');
        });
...
//# sourceMappingURL=detailed-span-attributes.test.js.map