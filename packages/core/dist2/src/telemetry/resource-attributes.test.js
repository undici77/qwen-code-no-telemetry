...
import { RESERVED_RESOURCE_ATTRIBUTE_KEYS, coerceStringResourceAttributes, parseOtelResourceAttributes, stripReservedResourceAttributes, } from './resource-attributes.js';
...
describe('stripReservedResourceAttributes', () => {
    ...
    it('drops reserved keys and warns for env source', () => {
        ...
        const out = stripReservedResourceAttributes(attrs, 'OTEL_RESOURCE_ATTRIBUTES');
    ...
    it('drops reserved keys and warns for settings source', () => {
        ...
        const out = stripReservedResourceAttributes(attrs, 'settings.telemetry.resourceAttributes');
        expect(out).toEqual({});
        expect(warnSpy).toHaveBeenCalledTimes(1);
        expect(warnSpy.mock.calls[0][0]).toContain('settings.telemetry.resourceAttributes');
    });
    ...
    it('mutates the input object', () => {
        const attrs = { 'service.version': 'x', a: '1' };
        const out = stripReservedResourceAttributes(attrs, 'OTEL_RESOURCE_ATTRIBUTES');
...
describe('warnings accumulator', () => {
    ...
    it('parseOtelResourceAttributes pushes diagnostic strings into the accumulator', () => {
        ...
        expect(warnings.some((w) => w.includes('Invalid percent-encoding'))).toBe(true);
    });
    it('stripReservedResourceAttributes pushes diagnostic for each reserved drop', () => {
        const warnings = [];
        stripReservedResourceAttributes({ 'service.version': 'x', 'session.id': 'y', team: 'z' }, 'OTEL_RESOURCE_ATTRIBUTES', warnings);
        expect(warnings).toHaveLength(2);
    });
    it('coerceStringResourceAttributes pushes diagnostic for empty key + non-string value', () => {
        const warnings = [];
        coerceStringResourceAttributes({ '': 'empty', team: 'ok', count: 42 }, warnings);
        expect(warnings).toHaveLength(2);
    });
    it('accumulator is opt-in (helpers work without one)', () => {
        expect(() => parseOtelResourceAttributes('a=1,bogus')).not.toThrow();
        expect(() => stripReservedResourceAttributes({ 'service.version': 'x' }, 'OTEL_RESOURCE_ATTRIBUTES')).not.toThrow();
        expect(() => coerceStringResourceAttributes({ team: 'ok', count: 1 })).not.toThrow();
    });
});
//# sourceMappingURL=resource-attributes.test.js.map