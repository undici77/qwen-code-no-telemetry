...
import { parseNpmPackageSource, isScopedNpmPackage, resolveNpmRegistry, checkNpmUpdate, } from './npm.js';
...
describe('parseNpmPackageSource', () => {
    ...
    it('should throw for invalid source', () => {
        expect(() => parseNpmPackageSource('not-scoped')).toThrow('Invalid scoped npm package source');
    });
    it('should throw for unscoped package', () => {
        expect(() => parseNpmPackageSource('some-package')).toThrow('Invalid scoped npm package source');
    });
});
...
describe('resolveNpmRegistry', () => {
    ...
    it('should return CLI override when provided', () => {
        const result = resolveNpmRegistry('@ali', 'https://registry.npmmirror.com/');
        expect(result).toBe('https://registry.npmmirror.com');
    });
    it('should return scoped registry from .npmrc', () => {
        vi.mocked(fs.readFileSync).mockReturnValueOnce('@ali:registry=https://registry.npmmirror.com/\nregistry=https://custom.registry.com/');
        const result = resolveNpmRegistry('@ali');
        expect(result).toBe('https://registry.npmmirror.com');
    });
    it('should return default registry from .npmrc when no scoped match', () => {
        vi.mocked(fs.readFileSync).mockReturnValueOnce('registry=https://custom.registry.com/');
...
function mockNpmRegistryResponse(data) {
