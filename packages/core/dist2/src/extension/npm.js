 ...
 */
function getNpmAuthToken(registryUrl) {
 ...
 */
function fetchNpmJson(url, authToken) {
    const headers = {
        ...
        client
            ...
            if (res.statusCode === 301 || res.statusCode === 302) {
                if (res.headers.location) {
                    ...
                    const redirectToken = redirectHost === originalHost ? authToken : undefined;
                    fetchNpmJson(res.headers.location, redirectToken)
                        .then(resolve)
                        .catch(reject);
                    return;
                }
            ...
            if (res.statusCode !== 200) {
                return reject(new Error(`npm registry request failed with status ${res.statusCode}: ${url}`));
            }
            const chunks = [];
 ...
 */
function downloadNpmFile(url, dest, authToken) {
    const headers = {};
    if (authToken) {
        headers['Authorization'] = `Bearer ${authToken}`;
    }
    ...
    return new Promise((resolve, reject) => {
        client
            ...
            if (res.statusCode !== 200) {
                return reject(new Error(`Failed to download npm tarball: status ${res.statusCode}`));
            }
 ...
 */
export async function downloadFromNpmRegistry(installMetadata, destination) {
    const { name, version: requestedVersion } = parseNpmPackageSource(installMetadata.source);
    const scope = name.split('/')[0];
    const registryUrl = installMetadata.registryUrl || resolveNpmRegistry(scope, undefined);
    ...
    const metadata = await fetchNpmJson(metadataUrl, authToken);
    // Resolve version
    let resolvedVersion;
    ...
    debugLogger.debug(`Downloading ${name}@${resolvedVersion} from ${tarballUrl}`);
 ...
 */
export async function checkNpmUpdate(installMetadata) {
    try {
        ...
        debugLogger.error(`Failed to check npm update for "${installMetadata.source}": ${error}`);
        return ExtensionUpdateState.ERROR;
    }
}
