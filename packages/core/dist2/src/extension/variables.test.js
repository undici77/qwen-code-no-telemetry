...
describe('substituteHookVariables', () => {
    ...
    it('should handle multiple event types with hooks', () => {
        ...
        expect(result).toBeDefined();
        expect(result['PreToolUse']).toHaveLength(1);
        expect(result['PreToolUse'][0].hooks[0].command).toBe('/home/user/.qwen/extensions/my-extension/scripts/pre-start.sh');
        expect(result['UserPromptSubmit']).toHaveLength(1);
        expect(result['UserPromptSubmit'][0].hooks[0]
            .command).toBe('/home/user/.qwen/extensions/my-extension/setup/install.py');
    });
