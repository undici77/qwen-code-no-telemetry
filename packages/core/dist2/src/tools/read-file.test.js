...
describe('ReadFileTool', () => {
    ...
    describe('build', () => {
        ...
        it('should reject offset or limit for notebook files', () => {
            ...
            expect(() => tool.build(params)).toThrow('offset and limit are not supported for Jupyter notebook (.ipynb) files');
        });
        it('should reject pages for notebook files', () => {
            ...
            expect(() => tool.build(params)).toThrow('pages is not supported for Jupyter notebook (.ipynb) files');
        });
    });
    ...
    describe('execute', () => {
        ...
        describe('with FileReadCache', () => {
            ...
            it('re-emits bytes (no placeholder) after the read was evicted from history by microcompaction (issue #4239)', async () => {
                ...
                expect(third.llmContent).toMatch(/unchanged since last read in this session/);
            });
            ...
            it('records an auto-memory read in the cache so a follow-up Edit can pass enforcement', async () => {
                ...
                try {
                    const { getAutoMemoryRoot, clearAutoMemoryRootCache } = await import('../memory/paths.js');
            ...
            it('records SVG-as-text reads with cacheable=true so a follow-up Edit passes enforcement', async () => {
                ...
                await fsp.writeFile(svgPath, '<svg xmlns="http://www.w3.org/2000/svg"></svg>\n', 'utf-8');
