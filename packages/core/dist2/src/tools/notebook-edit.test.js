...
describe('NotebookEditTool', () => {
    ...
    function writeNotebook(name, notebook) {
        const filePath = path.join(tempDir, name);
        fs.writeFileSync(filePath, JSON.stringify(notebook, null, 1), 'utf-8');
        return filePath;
    }
    function seedNotebookRead(filePath) {
    ...
    it.skipIf(process.platform === 'win32')('rejects non-regular notebook paths with a dedicated error type', async () => {
    ...
    it('returns structured errors for missing cells and invalid JSON', async () => {
        ...
        expect(missingCellResult.error?.type).toBe(ToolErrorType.NOTEBOOK_CELL_NOT_FOUND);
    ...
    it('keeps invalid original notebook errors structured for user-modified content', async () => {
        ...
        const updatedParams = modifyContext.createUpdatedParams(currentContent, proposedContent, originalParams);
        fs.writeFileSync(invalidPath, 'not json', 'utf-8');
        seedNotebookRead(invalidPath);
        const result = await buildInvocation(structuredClone(updatedParams)).execute(abortSignal);
    ...
    it('returns a notebook diff for confirmation', async () => {
        ...
        expect(editDetails.originalContent).toBe(fs.readFileSync(filePath, 'utf-8'));
    });
    it('applies IDE or inline modified full-notebook content instead of the original cell proposal', async () => {
        ...
        const updatedParams = modifyContext.createUpdatedParams(currentContent, modifiedContent, originalParams);
        const result = await buildInvocation(structuredClone(updatedParams)).execute(abortSignal);
        expect(result.error).toBeUndefined();
        const updated = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
