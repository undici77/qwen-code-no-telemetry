...
import { getCommandRoot, getCommandRoots, getShellConfiguration, splitCommands, stripShellWrapper, } from '../utils/shell-utils.js';
...
export class ShellToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal, updateOutput, shellExecutionConfig, setPidCallback, setPromoteAbortControllerCallback) {
        ...
        if (result.aborted) {
            ...
            const wasTimeout = effectiveTimeout &&
                combinedSignal.aborted &&
                !signal.aborted &&
                !promoteAbortController.signal.aborted;
            const wasPromoteRefused = promoteAbortController.signal.aborted && !signal.aborted;
            if (wasTimeout) {
        ...
        promoteArtifacts.onSettleWired = (info) => {
            ...
            try {
                ...
                const flushTimer = setTimeout(() => {
                    debugLogger.warn(`promote: output stream flush timed out for ${shellId} after ${PROMOTE_FLUSH_TIMEOUT_MS}ms — transitioning registry without flush confirmation`);
                    finalize();
                ...
                debugLogger.warn(`promote: closing output stream on settle threw: ${getErrorMessage(closeErr)}`);
                transitionRegistry(info);
            }
        };
    ...
    async runGitCount(cwd, args) {
        return new Promise((resolve) => {
            const child = childProcess.execFile('git', args, { cwd, timeout: 2000 }, (error, stdout) => {
                if (error) {
                    resolve(0);
                    return;
                }
    ...
    async getGitHead(cwd) {
        return new Promise((resolve) => {
            const child = childProcess.execFile('git', ['rev-parse', 'HEAD'], { cwd, timeout: 2000 }, (error, stdout) => {
                if (error) {
                    resolve(null);
                    return;
                }
    ...
    async attachCommitAttribution(cwd, preHead, isAmend) {
        ...
        try {
            ...
            const { exitCode, output, timedOut } = await new Promise((resolve) => {
                const child = childProcess.execFile(notesCommand.command, notesCommand.args, { cwd, timeout: 5000 }, (error, stdout, stderr) => {
