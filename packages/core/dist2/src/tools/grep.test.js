...
describe('GrepTool', () => {
    ...
    describe('execute', () => {
        ...
        it('normalizes CRLF fallback grep output without dropping result paths', () => {
            ...
            const matches = invocationForPrivateMethod.parseGrepOutput(`crlf.txt:1:hello world\r${os.EOL}`, tempRootDir);
        ...
        it('includes result paths for partially rendered match lines', async () => {
            ...
            await fs.writeFile(path.join(tempRootDir, 'partial.ts'), 'partial marker');
        ...
        it('only reports result paths for matches visible before character truncation', async () => {
            ...
            expect(result.returnDisplay).toContain('truncated');
            expect(result.resultFilePaths?.length).toBeLessThan(allResultPaths.length);
