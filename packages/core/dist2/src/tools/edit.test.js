...
describe('EditTool', () => {
     ...
     */
    function seedPriorRead(filePath) {
    ...
    describe('execute', () => {
        ...
        it('completes the edit even when trackEdit throws', async () => {
            ...
            fs.writeFileSync(filePath, initialContent, 'utf8');
            seedPriorRead(filePath);
            mockFileHistoryService.trackEdit.mockRejectedValueOnce(new Error('disk full'));
            const params = {
                file_path: filePath,
                old_string: 'old',
                new_string: 'new',
        ...
        describe('commit-attribution wiring', () => {
            ...
            it('records AI-originated edits in the attribution service', async () => {
                ...
                const attribution = CommitAttributionService.getInstance().getFileAttribution(filePath);
            ...
            it('skips attribution when the edit is modified_by_user', async () => {
                ...
                await invocation.execute(new AbortController().signal);
                expect(CommitAttributionService.getInstance().getFileAttribution(filePath)).toBeUndefined();
            });
        });
    ...
    describe('Error Scenarios', () => {
        ...
        it('should return FILE_WRITE_FAILURE when fsService.writeTextFile fails', async () => {
            fs.writeFileSync(filePath, 'content', 'utf8');
            seedPriorRead(filePath);
            vi.spyOn(fsService, 'writeTextFile').mockRejectedValueOnce(new Error('Simulated write error'));
            const params = {
                file_path: filePath,
                old_string: 'content',
                new_string: 'new content',
    ...
    describe('prior-read enforcement', () => {
        ...
        it('rejects an edit when the file has not been read in this session', async () => {
            ...
            expect(result.error?.message).toMatch(/has not been read in this session/);
        ...
        it('rejects an edit when the previous read was non-cacheable (binary / pdf / image)', async () => {
            ...
            expect(result.error?.message).toMatch(/binary \/ image \/ audio \/ video \/ PDF \/ notebook payload/);
            ...
            expect(result.error?.message).not.toMatch(/if you need to overwrite it\./);
        });
        ...
        it('rejects an edit with a stat failure other than ENOENT (fail-closed)', async () => {
            ...
            const result = await tool
                ...
                .execute(abortSignal);
            expect(result.error?.type).toBe(ToolErrorType.PRIOR_READ_VERIFICATION_FAILED);
        ...
        it('does not let an unread file be probed via NO_OCCURRENCE_FOUND', async () => {
            ...
            const result = await tool
                ...
                .execute(abortSignal);
            expect(result.error?.type).toBe(ToolErrorType.EDIT_REQUIRES_PRIOR_READ);
            expect(result.error?.type).not.toBe(ToolErrorType.EDIT_NO_OCCURRENCE_FOUND);
        });
        it('rejects confirmation requests on an unread file before showing a diff', async () => {
            ...
            await expect(invocation.getConfirmationDetails(abortSignal)).rejects.toThrow(/has not been read in this session/);
        });
        ...
        it('allows Edit after Write→partial-Read', async () => {
            ...
            const edited = await tool
                ...
                .execute(abortSignal);
            expect(edited.error).toBeUndefined();
            expect(fs.readFileSync(newPath, 'utf8')).toBe('line one\nsecond line\nline three\n');
        });
        ...
        it('bypasses enforcement entirely when fileReadCacheDisabled is true', async () => {
            ...
            mockConfig.getFileReadCacheDisabled.mockReturnValue(true);
            const params = {
                file_path: filePath,
                old_string: 'untouched',
                new_string: 'modified',
        ...
        it('attaches a structured ToolErrorType when getConfirmationDetails rejects', async () => {
            ...
            expect(caught?.errorType).toBe(ToolErrorType.EDIT_REQUIRES_PRIOR_READ);
        });
    });
