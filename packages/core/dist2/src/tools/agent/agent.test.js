...
describe('AgentTool', () => {
    ...
    describe('isolation — round-7 parent-dirty guard', () => {
        it('refuses isolation when parent has uncommitted edits', async () => {
            ...
            const repo = await fs.mkdtemp(pathMod.join(os.tmpdir(), 'qwen-iso-dirty-'));
            try {
        ...
        it('would allow isolation when parent is clean (sanity)', async () => {
            ...
            const os = await import('node:os');
            const { execFileSync } = await import('node:child_process');
            const repo = await fs.mkdtemp(pathMod.join(os.tmpdir(), 'qwen-iso-clean-'));
            try {
                execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: repo });
                execFileSync('git', ['config', 'user.email', 't@e.com'], {
                    cwd: repo,
    ...
    describe('AgentToolInvocation', () => {
        ...
        it('should execute subagent successfully', async () => {
            ...
            expect(mockAgent.execute).toHaveBeenCalledWith(mockContextState, expect.any(AbortSignal));
    ...
    describe('Fork dispatch (subagent_type omitted)', () => {
        ...
        it('stops the per-subagent ToolRegistry after the fork body finishes', async () => {
            ...
            stubReg);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            vi.mocked(config.createToolRegistry).mockResolvedValue(
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            stubReg);
            const params = {
                description: 'fork task',
        ...
        it('routes owned monitor notifications and cleanup for implicit forks', async () => {
            let releaseExecute;
            vi.mocked(mockAgent.execute).mockImplementation(() => new Promise((resolve) => {
            ...
            const callback = monitorRegistry.setAgentNotificationCallback.mock
                .calls[0][1];
            const provider = mockAgent.setExternalMessageProvider.mock.calls[0][0];
            const waiter = mockAgent.setExternalMessageWaiter.mock.calls[0][0];
            ...
            lifecycleCallback();
            await expect(Promise.all([firstOverlapWait, secondOverlapWait])).resolves.toEqual([[], []]);
            releaseExecute?.();
            await vi.runAllTimersAsync();
            expect(monitorRegistry.setAgentNotificationCallback).toHaveBeenCalledWith(agentId, undefined);
            expect(monitorRegistry.setAgentLifecycleCallback).toHaveBeenCalledWith(agentId, undefined);
            expect(monitorRegistry.cancelRunningForOwner).toHaveBeenCalledWith(agentId, { notify: false });
        });
    });
    describe('SubagentStart hook integration', () => {
        ...
        it('should call fireSubagentStartEvent before execution', async () => {
            ...
            const invocation = agentTool.createInvocation(params);
            await invocation.execute();
            expect(mockHookSystem.fireSubagentStartEvent).toHaveBeenCalledWith(expect.stringContaining('file-search-'), 'file-search', PermissionMode.AutoEdit, 
    ...
    describe('SubagentStop hook integration', () => {
        ...
        it('should call fireSubagentStopEvent after execution', async () => {
            ...
            const invocation = agentTool.createInvocation(params);
            await invocation.execute();
            expect(mockHookSystem.fireSubagentStopEvent).toHaveBeenCalledWith(expect.stringContaining('file-search-'), 'file-search', '/test/transcript', 'Task completed successfully', false, PermissionMode.AutoEdit, 
        ...
        it('should re-execute subagent when stop hook returns blocking decision', async () => {
            ...
            expect(mockHookSystem.fireSubagentStopEvent).toHaveBeenNthCalledWith(2, expect.stringContaining('file-search-'), 'file-search', '/test/transcript', 'Task completed successfully', true, PermissionMode.AutoEdit, 
            // Foreground subagents now run with a composed signal.
            expect.any(AbortSignal));
        });
        ...
        it('uses the configured SubagentStop blocking cap', async () => {
            config.getStopHookBlockingCap.mockReturnValue(2);
            const mockBlockOutput = {
            ...
            expect(mockHookSystem.fireSubagentStopEvent).toHaveBeenCalledTimes(2);
            expect(mockAgent.execute).toHaveBeenCalledTimes(2);
            expect(partToString(result.llmContent)).toContain('SubagentStop hook blocked continuation 2 consecutive times; overriding and ending the turn.');
        });
    ...
    describe('Agent-level background: true', () => {
        ...
        it('routes owned monitor notifications into a background agent external input queue', async () => {
            ...
            const invocation = agentTool.createInvocation(params);
            await invocation.execute();
            const agentId = mockRegistry.register.mock.calls[0][0].agentId;
            const monitorRegistry = config.getMonitorRegistry();
            const callback = monitorRegistry.setAgentNotificationCallback.mock.calls.find(([id, cb]) => id === agentId && typeof cb === 'function')?.[1];
            ...
            const lifecycleCallback = monitorRegistry.setAgentLifecycleCallback.mock.calls.find(([id, cb]) => id === agentId && typeof cb === 'function')?.[1];
            expect(lifecycleCallback).toBeDefined();
            lifecycleCallback?.();
            expect(mockRegistry.wakeExternalInputWaiters).toHaveBeenCalledWith(agentId);
        });
        ...
        it('should run in foreground when neither flag is set', async () => {
            ...
            expect(mockRegistry.unregisterForeground).toHaveBeenCalledWith(expect.stringContaining('file-search-'));
            expect(mockAgent.setExternalMessageProvider).toHaveBeenCalled();
            expect(mockAgent.setExternalMessageWaiter).toHaveBeenCalled();
            expect(mockAgent.setExternalMessageWaitPredicate).toHaveBeenCalled();
        });
        ...
        it('foreground subagent reserves a JSONL+meta path on the registry entry', async () => {
            ...
            expect(mockRegistry.register).toHaveBeenCalledWith(expect.objectContaining({
                isBackgrounded: false,
                outputFile: expect.stringMatching(/subagents[\\/]test-session-id[\\/]agent-file-search-.*\.jsonl$/),
                metaPath: expect.stringMatching(/subagents[\\/]test-session-id[\\/]agent-file-search-.*\.meta\.json$/),
            }));
            ...
            expect(writeMetaSpy).toHaveBeenCalledWith(expect.stringMatching(/agent-file-search-.*\.meta\.json$/), expect.objectContaining({
                status: 'running',
            ...
            // frozen at `running`.
            expect(patchMetaSpy).toHaveBeenCalledWith(expect.stringMatching(/agent-file-search-.*\.meta\.json$/), expect.objectContaining({ status: 'completed' }));
        ...
        it.each([
        ...
        ])('foreground %s terminate mode patches meta as %s', async (mode, expectedStatus) => {
            ...
            const invocation = agentTool.createInvocation(params);
            await invocation.execute();
            expect(patchMetaSpy).toHaveBeenCalledWith(expect.stringMatching(/agent-file-search-.*\.meta\.json$/), expect.objectContaining({ status: expectedStatus }));
            patchMetaSpy.mockRestore();
        });
        it('foreground CANCELLED prefixes the partial result so the parent sees the cancel', async () => {
            ...
            vi.mocked(mockAgent.getTerminateMode).mockReturnValue(AgentTerminateMode.CANCELLED);
            const params = {
                description: 'Search files',
                prompt: 'Find all TypeScript files',
                subagent_type: 'file-search',
