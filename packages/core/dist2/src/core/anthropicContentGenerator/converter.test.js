    ...
    describe('enableCacheControl', () => {
        ...
        describe('per-call options override constructor default', () => {
            ...
            it('overrides constructor false → true for system + messages + tools', async () => {
                const constructedWithCacheOff = new AnthropicContentConverter('test-model', 'auto', false);
                const { system, messages } = constructedWithCacheOff.convertGeminiRequestToAnthropic({
                    model: 'models/test',
                    contents: 'Hello',
                    config: { systemInstruction: 'sys' },
                ...
                const result = await constructedWithCacheOff.convertGeminiToolsToAnthropic(tools, {
            ...
            it('overrides constructor true → false (cache fully off)', async () => {
                ...
                const constructedWithCacheOn = new AnthropicContentConverter('test-model', 'auto', true);
                const { system, messages } = constructedWithCacheOn.convertGeminiRequestToAnthropic({
                    model: 'models/test',
                    contents: 'Hello',
                    config: { systemInstruction: 'sys' },
                ...
                const result = await constructedWithCacheOn.convertGeminiToolsToAnthropic(tools, {
                    enableCacheControl: false,
                });
                expect(result[0]).not.toHaveProperty('cache_control');
            });
            it('honors useGlobalCacheScope independently of enableCacheControl source', async () => {
                ...
                const converterDefault = new AnthropicContentConverter('test-model', 'auto');
                ...
                const result = await converterDefault.convertGeminiToolsToAnthropic(tools, { enableCacheControl: true });
