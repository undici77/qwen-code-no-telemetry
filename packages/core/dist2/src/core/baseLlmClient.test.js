...
vi.mock('./contentGenerator.js', async (importOriginal) => {
    ...
    return {
        ...actual,
        createContentGenerator: (...args) => mockCreateContentGenerator(...args),
    };
});
const mockBuildAgentContentGeneratorConfig = vi.fn();
vi.mock('../models/content-generator-config.js', () => ({
    buildAgentContentGeneratorConfig: (...args) => mockBuildAgentContentGeneratorConfig(...args),
}));
...
const createMockTextResponse = (text) => ({
    candidates: [
        {
            content: {
                role: 'model',
...
describe('BaseLlmClient', () => {
    ...
    describe('generateJson - Success Scenarios', () => {
        ...
        it('should reject loose JSON arrays', async () => {
            mockGenerateContent.mockResolvedValue(createMockTextResponse('[{"color":"blue"}]'));
            vi.mocked(getFunctionCalls).mockReturnValue(undefined);
            const result = await client.generateJson(defaultOptions);
            expect(result).toEqual({});
        });
    ...
    describe('generateJson - Error Handling', () => {
        it('should throw and report generic API errors', async () => {
            ...
            await expect(client.generateJson(defaultOptions)).rejects.toThrow('Failed to generate JSON content (test-prompt-id): Service Unavailable (503)');
    ...
    describe('per-model resolution', () => {
        ...
        let crossProviderConfig;
        beforeEach(() => {
            vi.mocked(retryWithBackoff).mockImplementation(async (fn) => await fn());
            ...
            crossProviderConfig = {
                ...
                getAllConfiguredModels: vi.fn((authTypes) => authTypes?.includes(AuthType.QWEN_OAUTH)
                    ? []
                    : [
                        {
                            id: fastModel,
        ...
        it('builds a per-model generator when model differs and is registered under another authType', async () => {
            ...
            getResolvedModel.mockImplementation((authType, model) => {
            ...
            expect(mockBuildAgentContentGeneratorConfig).toHaveBeenCalledWith(crossProviderConfig, fastModel, expect.objectContaining({
                authType: AuthType.USE_ANTHROPIC,
                baseUrl: 'https://api.anthropic.com',
            }));
        ...
        it('falls back to the main generator when createContentGenerator throws', async () => {
            ...
            mockCreateContentGenerator.mockRejectedValue(new Error('SDK init failed'));
            const c = new BaseLlmClient(mockContentGenerator, crossProviderConfig);
            const resolved = await c.resolveForModel(fastModel);
            expect(resolved.contentGenerator).toBe(mockContentGenerator);
        ...
        it('generateJson routes through the per-model generator and forwards retry authType', async () => {
            ...
            fastGenerateContent.mockResolvedValue(createMockResponseWithFunctionCall({ ok: true }));
            vi.mocked(getFunctionCalls).mockReturnValue([
            ...
            expect(retryWithBackoff).toHaveBeenCalledWith(expect.any(Function), expect.objectContaining({ authType: AuthType.USE_ANTHROPIC }));
        });
        it('generateJson accepts authType-qualified selectors and sends the bare model id', async () => {
            ...
            expect(getResolvedModel).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'shared-model');
            expect(mockBuildAgentContentGeneratorConfig).toHaveBeenCalledWith(crossProviderConfig, 'shared-model', expect.objectContaining({ authType: AuthType.USE_OPENAI }));
            expect(fastGenerateContent).toHaveBeenCalledWith(expect.objectContaining({ model: 'shared-model' }), 'test');
        });
