...
// Services
import { COMPRESSION_PRESERVE_THRESHOLD, COMPRESSION_TOKEN_THRESHOLD, } from '../services/chatCompressionService.js';
import { LoopDetectionService } from '../services/loopDetectionService.js';
import { CommitAttributionService } from '../services/commitAttribution.js';
import { AUTO_SKILL_THRESHOLD } from '../memory/manager.js';
import { DEFAULT_AUTO_SKILL_MAX_TURNS, DEFAULT_AUTO_SKILL_TIMEOUT_MS, } from '../memory/skillReviewAgentPlanner.js';
import { isProjectSkillPath } from '../skills/skill-paths.js';
import { ToolNames } from '../tools/tool-names.js';
// Telemetry
import { NextSpeakerCheckEvent, logNextSpeakerCheck, startInteractionSpan, endInteractionSpan, getActiveInteractionSpan, addUserPromptAttributes, } from '../telemetry/index.js';
...
export class GeminiClient {
    ...
    async initialize(sessionStartSource) {
        ...
        if (resumedSessionData) {
            ...
            this.getChat().setLastPromptTokenCount(uiTelemetryService.getLastPromptTokenCount());
    ...
    async setTools() {
        ...
        this.getChat().setSystemInstruction(this.getMainSessionSystemInstruction(deferredTools));
        ...
        if (this.lastSessionStartContext && this.lastSessionStartSource) {
            this.getChat().applySessionStartContext(this.lastSessionStartContext, this.lastSessionStartSource);
        }
     ...
     */
    async tryConsumeMemoryPrefetch() {
        const handle = this.pendingMemoryPrefetch;
    ...
    getMainSessionSystemInstruction(deferredTools) {
        ...
        if (overrideSystemPrompt) {
            const base = getCustomSystemPrompt(overrideSystemPrompt, userMemory, appendSystemPrompt, deferredTools);
            return gitStatus ? base + '\n\n' + gitStatus : base;
        }
        const base = getCoreSystemPrompt(userMemory, this.config.getModel(), appendSystemPrompt, deferredTools);
        return gitStatus ? base + '\n\n' + gitStatus : base;
    }
    /**
    ...
    async refreshSystemInstruction() {
        ...
        const deferredTools = this.resolveDeferredToolsForSystemPrompt();
        this.chat.setSystemInstruction(this.getMainSessionSystemInstruction(deferredTools));
        if (this.lastSessionStartContext && this.lastSessionStartSource) {
     ...
     * load on demand.
     */
    resolveDeferredToolsForSystemPrompt() {
        const toolRegistry = this.config.getToolRegistry();
    ...
    async startChat(extraHistory, sessionStartSource = extraHistory
        ...
        try {
            ...
            if (history.length > 0) {
                const deferredNames = new Set(toolRegistry.getDeferredToolSummary().map((t) => t.name));
            ...
            const deferredTools = this.resolveDeferredToolsForSystemPrompt();
            const systemInstruction = this.getMainSessionSystemInstruction(deferredTools);
            ...
            if (sessionStartAdditionalContext) {
                this.chat.applySessionStartContext(sessionStartAdditionalContext, sessionStartSource);
            }
            await this.setTools();
            return this.chat;
        }
    ...
    async *sendMessageStream(request, signal, prompt_id, options, turns = MAX_TURNS) {
        ...
        const isTopLevelInteraction = messageType === SendMessageType.UserQuery ||
            messageType === SendMessageType.Cron ||
            messageType === SendMessageType.Notification;
        ...
        try {
            if (messageType === SendMessageType.UserQuery ||
                ...
                const mcResult = microcompactHistory(this.getHistoryShallow(), this.lastApiCompletionTimestamp, this.config.getClearContextOnIdle());
                if (mcResult.meta) {
                    ...
                    if (m.unresolvedEvictedReads > 0) {
                        ...
                        const statResults = await Promise.all(m.evictedReadPaths.map((p) => fsPromises.stat(p).catch(() => undefined)));
            ...
            if (hooksEnabled &&
                ...
                const activeGoalAfterStopHook = getActiveGoal(this.config.getSessionId());
                ...
                if (stopOutput?.isBlockingDecision() ||
                    ...
                    const hookTurn = yield* this.sendMessageStream(continueRequest, signal, prompt_id, {
            ...
            if (!turn.pendingToolCalls.length && signal && !signal.aborted) {
                ...
                try {
                    ...
                    const cachedHistory = this.getHistoryTailShallow(maxHistoryForCache, true);
                ...
                if (nextSpeakerCheck?.next_speaker === 'model') {
                    ...
                    const continueTurn = yield* this.sendMessageStream(nextRequest, signal, prompt_id, { ...options, type: SendMessageType.Hook }, boundedTurns - 1);
                    if (isTopLevelInteraction)
                        endInteractionSpan(signal.aborted ? 'cancelled' : 'ok');
    ...
    async generateContent(contents, generationConfig, abortSignal, model, promptIdOverride) {
        ...
        try {
            ...
            const { contentGenerator, retryAuthType, model: requestModel, } = await this.config.getBaseLlmClient().resolveForModel(model);
            const apiCall = () => {
            ...
            const result = await retryWithBackoff(apiCall, {
                ...
                signal: abortSignal,
                heartbeatFn: (info) => {
                    process.stderr.write(`[qwen-code] Waiting for API capacity... attempt ${info.attempt}, retry in ${Math.ceil(info.remainingMs / 1000)}s\n`);
                },
            });
            return result;
        }
    ...
    async tryCompressChat(prompt_id, force = false, signal) {
        ...
        const info = await this.getChat().tryCompress(prompt_id, this.config.getModel(), force, signal);
