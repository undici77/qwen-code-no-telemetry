...
export function convertOpenAIChunkToGemini(chunk, requestContext) {
    ...
    if (choice) {
        ...
        if (!requestContext.responseParsingOptions?.taggedThinkingTags) {
            ...
            if (reasoningText) {
                const normalizedReasoningText = normalizeStreamingTextDelta(reasoningText, (requestContext.reasoningDeltaState ??= {
                    emittedText: '',
                    emittedLength: 0,
        ...
        if (typeof choice.delta?.content === 'string') {
            ...
            if (normalizedContent || choice.finish_reason) {
                parts.push(...convertOpenAITextToParts(normalizedContent, requestContext, Boolean(choice.finish_reason)));
            }
        }
