...
describe('DashScopeOpenAICompatibleProvider', () => {
    ...
    describe('isDashScopeProvider', () => {
        ...
        it('should return true for internal alibaba-inc.com subdomain', () => {
            ...
            const result = DashScopeOpenAICompatibleProvider.isDashScopeProvider(config);
            expect(result).toBe(true);
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('DashScope provider activated via internal origin: gateway.alibaba-inc.com');
        });
        ...
        it('should debug log when baseUrl does not match DASHSCOPE_PROXY_BASE_URL', () => {
            ...
            const result = DashScopeOpenAICompatibleProvider.isDashScopeProvider(config);
            expect(result).toBe(false);
            expect(mockDebugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('DASHSCOPE_PROXY_BASE_URL is configured but the request baseUrl does not match'));
        });
        it('should log internal-origin activation instead of proxy mismatch for internal domains', () => {
            ...
            expect(result).toBe(true);
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('DashScope provider activated via internal origin: gateway.alibaba-inc.com');
            expect(mockDebugLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('DASHSCOPE_PROXY_BASE_URL is configured but the request baseUrl does not match'));
        });
