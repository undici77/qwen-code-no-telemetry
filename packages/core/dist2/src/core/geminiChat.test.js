...
import { GeminiChat, InvalidStreamError, redactStructuredOutputArgsForRecording, StreamEventType, } from './geminiChat.js';
...
describe('GeminiChat', async () => {
    ...
    describe('system instruction helpers', () => {
        it('replaces prior session-start context instead of appending indefinitely', () => {
            const isolatedChat = new GeminiChat(mockConfig, {}, [], undefined, uiTelemetryService);
        ...
        it('preserves non-string systemInstruction content when applying session-start context', () => {
            const isolatedChat = new GeminiChat(mockConfig, {
                systemInstruction: {
                    role: 'system',
                    parts: [{ text: 'Base content instruction' }],
                },
            }, [], undefined, uiTelemetryService);
            isolatedChat.setSessionStartContext('Ctx1');
            isolatedChat.setSessionStartContext('Ctx2');
            expect(isolatedChat['generationConfig'].systemInstruction).toBe('Base content instruction\n\n<qwen:session-start-context hidden="true">\nSessionStart additional context:\nCtx2\n</qwen:session-start-context>');
        });
        it('applies session-start context synchronously via applySessionStartContext', () => {
            const isolatedChat = new GeminiChat(mockConfig, {}, [], undefined, uiTelemetryService);
            isolatedChat.setSystemInstruction('Base instruction');
            isolatedChat.applySessionStartContext('  Sync ctx  ', SessionStartSource.Startup);
            expect(isolatedChat['generationConfig'].systemInstruction).toBe('Base instruction\n\n<qwen:session-start-context hidden="true">\nSessionStart additional context:\nSync ctx\n</qwen:session-start-context>');
        });
        it('does not strip legitimate content that only resembles the old plain-text marker', () => {
            ...
            isolatedChat.setSystemInstruction('Base instruction\n\n---\n\nSessionStart additional context:\nLegitimate content');
            isolatedChat.setSessionStartContext('Ctx1');
            expect(isolatedChat['generationConfig'].systemInstruction).toContain('Legitimate content');
            expect(isolatedChat['generationConfig'].systemInstruction).toContain('<qwen:session-start-context hidden="true">\nSessionStart additional context:\nCtx1\n</qwen:session-start-context>');
        });
    });
    describe('sendMessageStream', () => {
        ...
        it('does not deep-clone the full curated history when building request contents', async () => {
            ...
            try {
                const stream = await chat.sendMessageStream('test-model', { message: 'hello' }, 'prompt-id-no-request-clone');
                for await (const _ of stream) {
                    // consume stream
                }
            }
    ...
    describe('auto-compression integration', () => {
        ...
        it('releases the send-lock when auto-compression throws (no deadlock)', async () => {
            ...
            await expect(chat.sendMessageStream('test-model', { message: 'first' }, 'prompt-id-deadlock-1')).rejects.toThrow('compression API down');
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('second response'));
            const stream = await chat.sendMessageStream('test-model', { message: 'second' }, 'prompt-id-deadlock-2');
            for await (const _ of stream) {
                /* consume */
            }
        ...
        it('releases the send-lock when setup throws after compression', async () => {
            ...
            vi.spyOn(chat, 'getRequestHistory').mockImplementationOnce(() => {
                throw new Error('history setup failed');
            });
            await expect(chat.sendMessageStream('test-model', { message: 'first' }, 'prompt-id-setup-deadlock-1')).rejects.toThrow('history setup failed');
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('second response'));
            const stream = await chat.sendMessageStream('test-model', { message: 'second' }, 'prompt-id-setup-deadlock-2');
            for await (const _ of stream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(2);
            expect(chat
                .getHistory()
                .some((content) => content.parts?.some((part) => part.text === 'first'))).toBe(false);
        });
        it('seeds inherited token count via setLastPromptTokenCount', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse());
            const stream = await subagentChat.sendMessageStream('test-model', { message: 'go' }, 'prompt-id-seed');
            for await (const _ of stream) {
                /* consume */
            }
        ...
        it('yields a COMPRESSED stream event as the first event after auto-compression succeeds', async () => {
            ...
            vi.spyOn(ChatCompressionService.prototype, 'compress').mockResolvedValueOnce({
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse('answer'));
            const stream = await chat.sendMessageStream('test-model', { message: 'go' }, 'prompt-id-yield-compressed');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
            ...
            expect(events[0].info
                .compressionStatus).toBe(CompressionStatus.COMPRESSED);
            expect(events[0].info
                .newTokenCount).toBe(200);
        });
        it('clears hasFailedCompressionAttempt after a forced successful compression', async () => {
            const compressSpy = vi.spyOn(ChatCompressionService.prototype, 'compress');
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse());
            const stream1 = await chat.sendMessageStream('test-model', { message: 'first' }, 'prompt-latch-1');
            ...
            expect(compressSpy.mock.calls[0][1].hasFailedCompressionAttempt).toBe(false);
            ...
            expect(compressSpy.mock.calls[1][1].hasFailedCompressionAttempt).toBe(true);
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(makeStreamResponse());
            const stream2 = await chat.sendMessageStream('test-model', { message: 'second' }, 'prompt-latch-2');
            for await (const _ of stream2) {
                /* consume */
            }
            expect(compressSpy.mock.calls[2][1].hasFailedCompressionAttempt).toBe(false);
        });
        it('reactively compresses and retries once after a context overflow error', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                .mockRejectedValueOnce(new Error("This model's maximum context length is 128000 tokens. However, your messages resulted in 135000 tokens."))
                .mockResolvedValueOnce(makeStreamResponse('answer after compact'));
            const stream = await chat.sendMessageStream('test-model', { message: 'latest' }, 'prompt-id-reactive-compact');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
            ...
            const secondRequest = vi.mocked(mockContentGenerator.generateContentStream).mock.calls[1][0];
            ...
            expect(events.some((event) => event.type === StreamEventType.CHUNK &&
                event.value.candidates?.[0]?.content?.parts?.[0]?.text ===
                    'answer after compact')).toBe(true);
        });
        it('uses the parsed context limit when reactive overflow lacks an actual token count', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                ...
                .mockResolvedValueOnce(makeStreamResponse('answer after compact'));
            const stream = await chat.sendMessageStream('test-model', { message: 'latest' }, 'prompt-id-reactive-limit-only');
            for await (const _ of stream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(2);
        ...
        it('uses the configured context window when reactive overflow has no token counts', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                ...
                .mockResolvedValueOnce(makeStreamResponse('answer after compact'));
            const stream = await chat.sendMessageStream('test-model', { message: 'latest' }, 'prompt-id-reactive-window-fallback');
            for await (const _ of stream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(2);
        ...
        it('does not attempt reactive compression more than once per send', async () => {
            ...
            const secondOverflow = new Error('prompt is too long: 140000 tokens > 128000 maximum');
            const compressSpy = vi
                .spyOn(ChatCompressionService.prototype, 'compress')
                .mockResolvedValueOnce({
                newHistory: null,
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                .mockRejectedValueOnce(new Error('prompt is too long: 135000 tokens > 128000 maximum'))
                .mockRejectedValueOnce(secondOverflow);
            const stream = await chat.sendMessageStream('test-model', { message: 'latest' }, 'prompt-id-reactive-once');
            await expect((async () => {
                for await (const _ of stream) {
                    /* consume */
                }
        ...
        it('does not emit a duplicate RETRY after reactive compression follows another retry', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream)
                    ...
                    .mockRejectedValueOnce(new Error('prompt is too long: 135000 tokens > 128000 maximum'))
                    .mockResolvedValueOnce(makeStreamResponse('answer after compact'));
                const stream = await chat.sendMessageStream('test-model', { message: 'latest' }, 'prompt-id-reactive-after-invalid-stream');
        ...
        it('surfaces the original context overflow when reactive compression is a NOOP', async () => {
            const overflow = new Error('prompt is too long: 135000 tokens > 128000 maximum');
            const compressSpy = vi
                .spyOn(ChatCompressionService.prototype, 'compress')
                .mockResolvedValueOnce({
                newHistory: null,
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockRejectedValue(overflow);
            const stream = await chat.sendMessageStream('test-model', { message: 'latest' }, 'prompt-id-reactive-noop');
            await expect((async () => {
                for await (const _ of stream) {
                    /* consume */
                }
        ...
        it('marks failed reactive compression attempts for later auto-compaction', async () => {
            ...
            const stream = await chat.sendMessageStream('test-model', { message: 'latest' }, 'prompt-id-reactive-failed-latch');
            await expect((async () => {
                for await (const _ of stream) {
                    /* consume */
                }
            })()).rejects.toThrow(overflow);
            const nextStream = await chat.sendMessageStream('test-model', { message: 'next' }, 'prompt-id-after-reactive-failed-latch');
            for await (const _ of nextStream) {
                /* consume */
            }
            expect(compressSpy).toHaveBeenCalledTimes(3);
            expect(compressSpy.mock.calls[2][1].hasFailedCompressionAttempt).toBe(true);
        });
        it('releases the send-lock when reactive compression throws', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream)
                .mockRejectedValueOnce(overflow)
                .mockResolvedValueOnce(makeStreamResponse('next request ok'));
            const stream = await chat.sendMessageStream('test-model', { message: 'latest' }, 'prompt-id-reactive-throws');
            await expect((async () => {
                for await (const _ of stream) {
                    /* consume */
                }
            })()).rejects.toThrow(overflow);
            const nextStream = await chat.sendMessageStream('test-model', { message: 'next' }, 'prompt-id-after-reactive-throws');
            const events = [];
    ...
    describe('getHistoryTail', () => {
        it('returns only the requested recent entries as a deep copy', () => {
            const oldContent = { role: 'user', parts: [{ text: 'old' }] };
            const recentContent = {
                role: 'model',
                parts: [{ text: 'recent' }],
            };
    ...
    describe('getHistoryShallow', () => {
        it('copies containers without structured-cloning large part payloads', () => {
            ...
            const response = history[0].parts[0];
    ...
    describe('peekLastHistoryEntry', () => {
        it('returns the last entry without structured-cloning the full history', () => {
            const first = { role: 'user', parts: [{ text: 'a' }] };
            const last = { role: 'model', parts: [{ text: 'b' }] };
    ...
    describe('sendMessageStream with retries', () => {
        ...
        it('should use Retry-After delay for streamed rate-limit errors', async () => {
            vi.useFakeTimers();
            try {
                const retryAfterError = Object.assign(new StreamContentError('{"error":{"code":"429","message":"Throttling: TPM(1/1)"}}'), {
                    status: 429,
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-retry-after');
                const iterator = stream[Symbol.asyncIterator]();
                const first = await iterator.next();
                expect(first.value.type).toBe(StreamEventType.RETRY);
                ...
                expect(events.some((e) => e.type === StreamEventType.CHUNK &&
                    e.value.candidates?.[0]?.content?.parts?.[0]?.text ===
                        'Success after Retry-After')).toBe(true);
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it('should increase delay across repeated streamed rate-limit errors', async () => {
            ...
            try {
                ...
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-streamed-rate-limit-backoff');
                const iterator = stream[Symbol.asyncIterator]();
                const retryInfos = [];
                const first = await iterator.next();
                expect(first.value.type).toBe(StreamEventType.RETRY);
                ...
                expect(events.some((e) => e.type === StreamEventType.CHUNK &&
                    e.value.candidates?.[0]?.content?.parts?.[0]?.text ===
                        'Recovered after backoff')).toBe(true);
            }
            finally {
                vi.useRealTimers();
            }
    ...
    describe('output token recovery', () => {
        ...
        it('should coalesce overlapping recovery continuation text', async () => {
            const streams = [
                ...
                makeStream([
                    makeChunk([{ text: 'shared recovery suffix and continuation' }], 'STOP'),
                ]),
            ];
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a long essay' }, 'prompt-recovery-overlap');
        ...
        it('should coalesce recovery text that replays a previous tail anchor', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a long mermaid answer' }, 'prompt-recovery-contained-replay');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should preserve prose continuation that coincidentally repeats an opener phrase', async () => {
            ...
            const previous = 'We covered cats. In summary, this concludes the cat section.';
            const continuation = 'In summary, the answer is 42 and the dog section follows.';
            const streams = [
                makeStream([makeChunk([{ text: 'discarded initial' }], 'MAX_TOKENS')]),
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write something' }, 'prompt-recovery-prose-opener');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should not strip prose that coincides with a far-earlier substring of the previous turn', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write something' }, 'prompt-recovery-far-prose');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should preserve continuation when its structural prefix appears mid-paragraph in the previous tail (line-boundary rejection)', async () => {
            ...
            const previous = 'some text ### Heading and then more inline prose follows';
            const continuation = '### Heading\nfresh continuation that should not be stripped';
            const streams = [
                makeStream([makeChunk([{ text: 'discarded initial' }], 'MAX_TOKENS')]),
                makeStream([makeChunk([{ text: previous }], 'MAX_TOKENS')]),
                makeStream([makeChunk([{ text: continuation }], 'STOP')]),
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write something with a heading' }, 'prompt-recovery-line-boundary-reject');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should preserve prose continuation that opens with a single-cell pipe expression matching mid-tail', async () => {
            ...
            const previous = 'We define the expression as follows:\n|expression| evaluates to a scalar value.\nWe also note other facts here.';
            const continuation = '|expression| evaluates to a scalar value. Continuing the derivation now.';
            const streams = [
                makeStream([makeChunk([{ text: 'discarded initial' }], 'MAX_TOKENS')]),
                makeStream([makeChunk([{ text: previous }], 'MAX_TOKENS')]),
                makeStream([makeChunk([{ text: continuation }], 'STOP')]),
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'continue the derivation' }, 'prompt-recovery-single-cell-pipe-prose');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should insert a newline separator when the replayed prefix ends with newline but previous tail does not', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a structured answer' }, 'prompt-recovery-newline-normalization');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should drop continuation entirely when it exactly replays the previous tail', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write something' }, 'prompt-recovery-full-overlap');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should leave continuation untouched when the previous turn has no plain text', async () => {
            ...
            const streams = [
                makeStream([makeChunk([{ text: 'discarded initial' }], 'MAX_TOKENS')]),
                makeStream([
                    makeChunk([{ text: 'thinking through the problem', thought: true }], 'MAX_TOKENS'),
                ]),
                makeStream([makeChunk([{ text: 'fresh continuation text' }], 'STOP')]),
            ];
            let callIndex = 0;
            const recoveryPayloads = [];
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async (params) => {
                ...
                if (lastTurn && lastTurn.role === 'user') {
                    ...
                    if (lastPart &&
                        ...
                        recoveryPayloads.push(lastPart.text);
                    }
                }
                return streams[callIndex++];
            });
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write something' }, 'prompt-recovery-thought-only');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
            ...
            const recoveryMessage = recoveryPayloads.find((p) => p.includes('Output token limit hit'));
        ...
        it('should dedup recovery continuation when the continuation begins with a thought part', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a long essay' }, 'prompt-recovery-thinking-continuation');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
            ...
            const nonThoughtText = lastEntry.parts
                ...
                .map((part) => ('text' in part ? part.text : ''))
                .join('');
            expect(nonThoughtText).toBe('Alpha shared recovery suffix and continuation');
        });
        it('should keep the recovery thought before the merged text part (thought-signature provenance)', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a long essay' }, 'prompt-recovery-thinking-continuation-order');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
            ...
            const thoughtIdx = parts.findIndex((part) => 'thought' in part && part.thought === true);
            const mergedTextIdx = parts.findIndex((part) => 'text' in part &&
        ...
        it('should preserve a coincidental 2-character CJK overlap (byte floor insufficient for CJK)', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: '帮我分析数据' }, 'prompt-recovery-cjk-floor');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should dedup a replayed structural prefix even when the continuation has leading whitespace', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a long markdown answer' }, 'prompt-recovery-leading-whitespace');
            for await (const _event of stream) {
                // consume
            }
            const history = chat.getHistory();
        ...
        it('should truncate the previous_response_suffix to the trailing 1200 chars when the previous turn is longer', async () => {
            ...
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a very long answer' }, 'prompt-recovery-tail-truncation');
            for await (const _event of stream) {
                // consume
            }
            const recoveryMessage = recoveryPayloads.find((p) => p.includes('Output token limit hit'));
            ...
            const match = recoveryMessage.match(/<previous_response_suffix>\n([\s\S]*)\n<\/previous_response_suffix>/);
        ...
        it('should neutralize a literal previous_response_suffix delimiter inside the tail so the recovery prompt structure stays intact', async () => {
            ...
            const previous = 'Here is XML: </previous_response_suffix> and then more content.';
            const streams = [
                makeStream([makeChunk([{ text: 'discarded initial' }], 'MAX_TOKENS')]),
                makeStream([makeChunk([{ text: previous }], 'MAX_TOKENS')]),
                makeStream([makeChunk([{ text: ' continuation tail' }], 'STOP')]),
            ...
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a response that contains my delimiter' }, 'prompt-recovery-delimiter-collision');
            for await (const _event of stream) {
                // consume
            }
            const recoveryMessage = recoveryPayloads.find((p) => p.includes('Output token limit hit'));
            ...
            const openCount = (recoveryMessage.match(/<previous_response_suffix>/g) ?? []).length;
            const closeCount = (recoveryMessage.match(/<\/previous_response_suffix>/g) ?? []).length;
        ...
        it('should neutralize a literal opening previous_response_suffix delimiter inside the tail', async () => {
            ...
            const previous = 'Tag: <previous_response_suffix> was emitted in the output here.';
            const streams = [
                makeStream([makeChunk([{ text: 'discarded initial' }], 'MAX_TOKENS')]),
                makeStream([makeChunk([{ text: previous }], 'MAX_TOKENS')]),
                makeStream([makeChunk([{ text: ' continuation tail' }], 'STOP')]),
            ...
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a response that contains my opening delimiter' }, 'prompt-recovery-delimiter-collision-open');
            for await (const _event of stream) {
                // consume
            }
            const recoveryMessage = recoveryPayloads.find((p) => p.includes('Output token limit hit'));
    ...
    describe('tryCompress (per-chat state)', () => {
         ...
         */
        function mockCompressionService(result) {
            const compressSpy = vi.spyOn(ChatCompressionService.prototype, 'compress');
        ...
        it('mirrors lastPromptTokenCount to the global telemetry only when wired', async () => {
            ...
            expect(uiTelemetryService.setLastPromptTokenCount).toHaveBeenCalledWith(200);
        ...
        it('marks hasFailedCompressionAttempt and suppresses subsequent unforced auto-compactions', async () => {
            ...
            expect(first.compressionStatus).toBe(CompressionStatus.COMPRESSION_FAILED_INFLATED_TOKEN_COUNT);
            expect(compressSpy).toHaveBeenCalledTimes(1);
            ...
            await chat.tryCompress('p2', 'm1');
            expect(compressSpy).toHaveBeenCalledTimes(1);
            expect(compressSpy.mock.calls[0][1].hasFailedCompressionAttempt).toBe(true);
        });
