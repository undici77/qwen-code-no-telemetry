...
const loggingSpanNamesWithSetStatusFailure = vi.hoisted(() => new Set());
vi.mock('@opentelemetry/api', async (importOriginal) => {
    const actual = await importOriginal();
    function runWithActive(label, fn) {
    ...
    return {
        ...
        trace: {
            ...
            getSpan: (ctx) => typeof ctx === 'object' && ctx !== null && 'span' in ctx
                ? ctx.span
                : undefined,
        },
    };
...
vi.mock('../../telemetry/index.js', () => {
    ...
    return {
        startLLMRequestSpan: vi.fn((model, promptId) => createSpan('qwen-code.llm_request', {
            model,
            prompt_id: promptId,
        })),
        endLLMRequestSpan: vi.fn((span, metadata) => {
            ...
            const record = loggingSpanRecords.find((r) => r.name === span.__spanName && r.endMetadata === undefined);
            if (record) {
...
const getStreamSpanRecord = () => {
    const spanRecord = loggingSpanRecords.find((record) => record.name === 'qwen-code.llm_request');
    if (!spanRecord) {
...
describe('LoggingContentGenerator', () => {
    ...
    it('logs request/response, normalizes thought parts, and logs OpenAI interaction', async () => {
        const wrapped = createWrappedGenerator(vi.fn().mockResolvedValue(createResponse('resp-1', 'model-v2', [{ text: 'ok' }, { text: 'hidden thought', thought: true }], {
            promptTokenCount: 3,
        ...
        const generator = new LoggingContentGenerator(wrapped, createConfig({ authType: AuthType.USE_ANTHROPIC }), generatorConfig);
        const request = {
            model: 'test-model',
            contents: [
                {
    ...
    it('creates and closes the non-stream API span on success', async () => {
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-span', 'test-model', [{ text: 'ok' }])), vi.fn());
    ...
    it('marks non-stream LLM span with llm_request.stream=false', async () => {
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp', 'test-model', [{ text: 'ok' }])), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: false,
    ...
    it('marks streaming LLM span with llm_request.stream=true', async () => {
        const streamFn = vi.fn().mockResolvedValue((async function* () {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-stream-attr');
    ...
    it('forwards token counts and duration to endLLMRequestSpan on non-stream success', async () => {
        const wrapped = createWrappedGenerator(vi.fn().mockResolvedValue(createResponse('resp', 'test-model', [{ text: 'ok' }], {
            promptTokenCount: 42,
    ...
    it('forwards error metadata to endLLMRequestSpan on non-stream failure', async () => {
        const wrapped = createWrappedGenerator(vi.fn().mockRejectedValue(new Error('upstream-down')), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: false,
        ...
        await expect(generator.generateContent(request, 'prompt-err')).rejects.toThrow('upstream-down');
        const spanRecord = getGenerateContentSpanRecord();
        expect(spanRecord.endMetadata).toMatchObject({
            success: false,
    ...
    it('preserves non-stream success when response and OpenAI logging fail', async () => {
        ...
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-safe', 'test-model', [{ text: 'ok' }])), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: true,
        ...
        const openaiLoggerInstance = vi.mocked(OpenAILogger).mock.results.at(-1)
            ?.value;
        openaiLoggerInstance.logInteraction.mockRejectedValueOnce(new Error('openai-log-fail'));
        const request = {
            model: 'test-model',
            contents: 'Hello',
        };
    ...
    it('truncates long response text in API response telemetry', async () => {
        ...
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-long', 'test-model', [{ text: longText }])), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: false,
        ...
        expect(responseEvent.response_text).toBe(`${longText.slice(0, MAX_RESPONSE_TEXT_LENGTH - RESPONSE_TEXT_TRUNCATION_SUFFIX.length)}${RESPONSE_TEXT_TRUNCATION_SUFFIX}`);
    });
    it.each([
        ...
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-empty', 'test-model', parts)), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: false,
    ...
    it('sanitizes non-stream request logging errors in span status', async () => {
        ...
        await expect(generator.generateContent(request, 'prompt-log-prep')).rejects.toThrow('request-log-secret');
        ...
        expect(JSON.stringify(spanRecord.statuses)).not.toContain('request-log-secret');
        expect(spanRecord.ended).toBe(true);
    });
    ...
    it('preserves stream success when response and OpenAI logging fail', async () => {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-safe-stream');
        const seen = [];
        for await (const item of stream) {
            seen.push(item);
        }
    ...
    it('preserves stream success when the OK status update fails', async () => {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-status');
        const seen = [];
        for await (const item of stream) {
            seen.push(item);
        }
    ...
    it('activates the stream span while the wrapped generator creates the stream', async () => {
        ...
        const wrapped = createWrappedGenerator(vi.fn(), vi.fn().mockImplementation(async () => {
    ...
    it('logs stream setup errors before leaving the stream span context', async () => {
        ...
        const wrapped = createWrappedGenerator(vi.fn(), vi.fn().mockRejectedValue(setupError));
        const generator = new LoggingContentGenerator(wrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: false,
        ...
        await expect(generator.generateContentStream(request, 'prompt-setup-error')).rejects.toThrow('setup-fail');
        expect(logApiError).toHaveBeenCalledTimes(1);
    ...
    it('skips success api_response log when stream span is ended by idle timeout (#4212)', async () => {
        ...
        const setTimeoutSpy = vi.spyOn(global, 'setTimeout').mockImplementation(((...args) => {
            const [cb, ms] = args;
        ...
        try {
            ...
            const stream = await generator.generateContentStream(request, 'prompt-idle-timeout');
            ...
            expect(spanRecord.endMetadata?.error).toBe('Stream span timed out (idle)');
            expect(spanRecord.ended).toBe(true);
            releaseStream?.();
    ...
    it('skips api_error log when stream throws after idle timeout already closed the span (#4302)', async () => {
        ...
        try {
            ...
            const stream = await generator.generateContentStream(request, 'prompt-throw-after-timeout');
            const iterator = stream[Symbol.asyncIterator]();
            const first = await iterator.next();
            expect(first.done).toBe(false);
            expect(idleCallback).toBeDefined();
    ...
    it('preserves stream errors when the error status update fails', async () => {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-error-status');
        await expect(async () => {
            for await (const _item of stream) {
                // Consume stream to trigger error.
            }
    ...
    it('logs the captured wire request including provider-injected fields (generateContent)', async () => {
        ...
        const wrapped = createWrappedGenerator(vi.fn().mockImplementation(async () => {
    ...
    it('logs the captured wire request for streaming requests (generateContentStream)', async () => {
        ...
        const stream = await generator.generateContentStream(request, 'prompt-stream-cap');
        for await (const _ of stream) {
            // drain
        }
        const openaiLoggerInstance = vi.mocked(OpenAILogger).mock.results[0]
    ...
    it('falls back to synthetic request when the wrapped generator does not capture', async () => {
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-fallback', 'test-model', [{ text: 'ok' }])), vi.fn());
        const generator = new LoggingContentGenerator(wrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: true,
        ...
        const [loggedRequest] = openaiLoggerInstance.logInteraction.mock
            .calls[0];
        expect(loggedRequest).toEqual(expect.objectContaining({
            model: 'test-model',
            temperature: 0.4,
        }));
    });
    it('does not propagate logging-side throws (success and error paths)', async () => {
        ...
        const successWrapped = createWrappedGenerator(vi.fn().mockResolvedValue(successResponse), vi.fn());
        const successGen = new LoggingContentGenerator(successWrapped, createConfig(), {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: true,
            openAILoggingDir: 'logs',
        ...
        await expect(successGen.generateContent(request, 'prompt-safe-success')).resolves.toBe(successResponse);
        const apiError = new Error('api-boom');
        const errorWrapped = createWrappedGenerator(vi.fn().mockRejectedValue(apiError), vi.fn());
        ...
        await expect(errorGen.generateContent(request, 'prompt-safe-error')).rejects.toThrow('api-boom');
    });
    it('does not propagate logging-side throws on a successful stream', async () => {
        ...
        const openaiLoggerInstance = vi.mocked(OpenAILogger).mock.results[0]
            ?.value;
        openaiLoggerInstance.logInteraction.mockRejectedValueOnce(new Error('log-fail-on-stream-success'));
        const request = {
            model: 'test-model',
            contents: [{ role: 'user', parts: [{ text: 'hi' }] }],
        };
        const stream = await generator.generateContentStream(request, 'prompt-stream-safe-success');
        const seen = [];
        for await (const item of stream) {
            seen.push(item);
        }
    ...
    it('does not let logging-side throws replace the original stream error', async () => {
        ...
        const openaiLoggerInstance = vi.mocked(OpenAILogger).mock.results[0]
            ?.value;
        openaiLoggerInstance.logInteraction.mockRejectedValueOnce(new Error('log-fail-on-stream-error'));
        const request = {
            model: 'test-model',
            contents: [{ role: 'user', parts: [{ text: 'hi' }] }],
        };
        const stream = await generator.generateContentStream(request, 'prompt-stream-safe-error');
        await expect(async () => {
            for await (const _item of stream) {
                // drain
            }
    ...
    it.each([
    ...
    ])('skips logApiRequest but writes tagged OpenAI logging for internal promptId %s (generateContent)', async (promptId) => {
        const mockResponse = {
        ...
        const [openaiRequest, openaiResponse, openaiError, options] = loggerInstance.logInteraction.mock.calls[0];
        expect(openaiRequest).toEqual(expect.objectContaining({
            model: 'test-model',
            messages: [{ role: 'user', content: 'converted' }],
        }));
        expect(openaiResponse).toEqual(expect.objectContaining({ id: 'openai-response' }));
        expect(openaiError).toBeUndefined();
    ...
    it.each([
        ...
        'speculation',
        'side-query:session-title',
    ])('skips logApiRequest but writes tagged OpenAI logging for internal promptId %s (generateContentStream)', async (promptId) => {
        const mockChunk = {
