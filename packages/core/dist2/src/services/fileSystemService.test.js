...
import { StandardFileSystemService, encodeTextFileContent, needsUtf8Bom, resetUtf8BomCache, detectLineEnding, ensureCrlfLineEndings, } from './fileSystemService.js';
...
vi.mock('../utils/atomicFileWrite.js', () => ({
    atomicWriteFile: vi.fn(async (filePath, data, options) => {
...
describe('StandardFileSystemService', () => {
    ...
    describe('readTextFile', () => {
        ...
        it('should propagate readFileWithLineAndLimit errors', async () => {
            ...
            await expect(fileSystem.readTextFile({ path: '/test/file.txt' })).rejects.toThrow('ENOENT: File not found');
        });
    });
    describe('writeTextFile', () => {
        ...
        it('should write file as UTF-8 when encoding is utf-8', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith('/test/file.txt', 'Hello', 'utf-8');
        });
    ...
    describe('writeTextFile with lineEnding preservation', () => {
        ...
        it('should preserve CRLF for non-bat files on non-Windows when lineEnding is crlf', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith('/test/file.cs', 'using System;\r\nclass Foo {}\r\n', 'utf-8');
        });
    });
