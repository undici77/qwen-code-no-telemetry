...
describe('SessionService', () => {
    ...
    describe('removeSessions', () => {
        ...
        it('should keep going when one removal fails', async () => {
            ...
            unlinkSyncSpy.mockImplementation((p) => {
    ...
    describe('countSessionMessages', () => {
        ...
        const stubCreateReadStream = (lines) => vi
            .spyOn(fs, 'createReadStream')
            .mockImplementation(() => Readable.from([lines.join('\n')]));
        ...
        it('should return 0 when the session file is missing (ENOENT)', async () => {
            ...
            vi.mocked(jsonl.readLines).mockRejectedValue(Object.assign(new Error('ENOENT'), { code: 'ENOENT' }));
            const count = await sessionService.countSessionMessages(sessionIdA);
            expect(count).toBe(0);
        });
    ...
    describe('getResumePromptTokenCount', () => {
        ...
        it('should prefer promptTokenCount over totalTokenCount when both are present', () => {
            ...
            expect(getResumePromptTokenCount(makeConversation([compressionRecord, assistant]))).toBe(200);
        });
    ...
    describe('buildApiHistoryFromConversation', () => {
        ...
        it('does not deep-clone stored messages when rebuilding resume API history', () => {
            ...
            const response = history[1].parts[0];
    ...
    describe('forkSession', () => {
        ...
        beforeEach(async () => {
            ...
            realPath = await vi.importActual('node:path');
            const actualPaths = await vi.importActual('../utils/paths.js');
            const actualJsonl = await vi.importActual('../utils/jsonl-utils.js');
            vi.mocked(path.join).mockImplementation(realPath.join);
            vi.mocked(path.dirname).mockImplementation(realPath.dirname);
            ...
            vi.mocked(path.isAbsolute).mockImplementation(realPath.isAbsolute);
            ...
            const mockedPaths = (await import('../utils/paths.js'));
            ...
            realTmpDir = fs.mkdtempSync(realPath.join(realOs.tmpdir(), 'fork-session-'));
        ...
        const seedSession = (sessionId) => {
            const chatsDir = realPath.join(service['storage'].getProjectDir(), 'chats');
        ...
        it('throws when the target session file already exists', async () => {
            ...
            await expect(service.forkSession(oldId, newId)).rejects.toThrow(/already exists/);
        });
        it('throws when the source session belongs to a different project', async () => {
            ...
            const chatsDir = realPath.join(service['storage'].getProjectDir(), 'chats');
            fs.mkdirSync(chatsDir, { recursive: true });
            fs.writeFileSync(realPath.join(chatsDir, `${oldId}.jsonl`), JSON.stringify({
                uuid: 'u1',
                parentUuid: null,
                sessionId: oldId,
                type: 'user',
            ...
            await expect(service.forkSession(oldId, newId)).rejects.toThrow(/does not belong to current project/);
        });
        it('rejects invalid sessionId patterns before touching disk', async () => {
            const valid = '99999999-9999-9999-9999-999999999999';
            await expect(service.forkSession('bogus', valid)).rejects.toThrow(/Invalid source sessionId/);
            await expect(service.forkSession(valid, 'bogus')).rejects.toThrow(/Invalid new sessionId/);
        });
    });
    describe('findSessionTitlesByPrefix', () => {
        ...
        beforeEach(async () => {
            ...
            vi.mocked(statSyncSpy).mockRestore?.();
            vi.mocked(unlinkSyncSpy).mockRestore?.();
            realTmpDir = fs.mkdtempSync(realPath.join(realOs.tmpdir(), 'find-titles-prefix-'));
            process.env['QWEN_RUNTIME_DIR'] = realTmpDir;
            cwd = process.cwd();
            service = new SessionService(cwd);
        });
        ...
        const seedSessionWithTitle = (sessionId, title, sessionCwd = cwd) => {
            const chatsDir = realPath.join(service['storage'].getProjectDir(), 'chats');
            fs.mkdirSync(chatsDir, { recursive: true });
            const file = realPath.join(chatsDir, `${sessionId}.jsonl`);
            const lines = [
        ...
        it('returns titles whose custom_title starts with the prefix (case-insensitive)', async () => {
            seedSessionWithTitle('11111111-1111-1111-1111-111111111111', 'my-branch (Branch)');
            seedSessionWithTitle('22222222-2222-2222-2222-222222222222', 'My-Branch (Branch 2)');
            seedSessionWithTitle('33333333-3333-3333-3333-333333333333', 'unrelated session');
            const titles = await service.findSessionTitlesByPrefix('my-branch (Branch');
            expect(new Set(titles)).toEqual(new Set(['my-branch (Branch)', 'My-Branch (Branch 2)']));
        });
        ...
        it('skips sessions from other projects (collisions are project-scoped)', async () => {
            seedSessionWithTitle('11111111-1111-1111-1111-111111111111', 'shared (Branch)', cwd);
            ...
            seedSessionWithTitle('22222222-2222-2222-2222-222222222222', 'shared (Branch 2)', '/some/other/project');
