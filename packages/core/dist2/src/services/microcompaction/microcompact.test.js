...
import { evaluateTimeBasedTrigger, microcompactHistory, MICROCOMPACT_CLEARED_MESSAGE, MICROCOMPACT_CLEARED_IMAGE_PREFIX, } from './microcompact.js';
function makeInlineImage(mimeType = 'image/png', data = 'AAAA') {
    return {
        role: 'user',
...
describe('microcompactHistory', () => {
    ...
    it('should clear old inline image parts and keep recent ones', () => {
        ...
        expect(result.history[1].parts[0].text).toBe(`${MICROCOMPACT_CLEARED_IMAGE_PREFIX} image/png]`);
    ...
    it('does not reclear an already-cleared image part', () => {
        ...
        expect(result.history[0].parts[0].text).toBe(`${MICROCOMPACT_CLEARED_IMAGE_PREFIX} image/png]`);
    });
    it('uses per-kind keepRecent budgets (tools and media counted independently)', () => {
        ...
        expect(result.history[5].parts[0].functionResponse.response['output']).toBe('recent output');
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        // Only-media keeps its slot under the separate media budget.
        expect(result.history[3].parts[0].inlineData?.data).toBe('OLDIMAGEOLDIMAGE');
        expect(result.meta.toolsCleared).toBe(1);
        expect(result.meta.mediaCleared).toBe(0);
    });
    it('clears older media when there are more than keepRecent of them', () => {
        ...
        expect(result.history[1].parts[0].text).toBe(`${MICROCOMPACT_CLEARED_IMAGE_PREFIX} image/png]`);
        expect(result.history[3].parts[0].text).toBe(`${MICROCOMPACT_CLEARED_IMAGE_PREFIX} image/jpeg]`);
    ...
    it('clears stale fileData parts (not just inlineData)', () => {
        ...
        expect(result.history[1].parts[0].text).toBe(`${MICROCOMPACT_CLEARED_IMAGE_PREFIX} image/png]`);
        expect(result.history[3].parts[0].fileData?.fileUri).toBe('gs://b/new.png');
    });
    ...
    it('strips nested media from non-compactable tool results (preserves text output)', () => {
        ...
        const oldNonCompactableWithImage = {
            role: 'user',
            parts: [
                {
                    functionResponse: {
        ...
        const recentNonCompactableWithImage = {
            role: 'user',
            parts: [
                {
                    functionResponse: {
        ...
        const result = microcompactHistory(history, twoHoursAgo, DEFAULT_SETTINGS);
        expect(result.meta).toBeDefined();
        const cleared = result.history[1].parts[0].functionResponse;
        ...
        const recent = result.history[3].parts[0].functionResponse;
...
describe('microcompactHistory evictedReadPaths (issue #4239)', () => {
    ...
    function fileCall(id, name, filePath) {
        return {
            role: 'model',
            parts: [{ functionCall: { id, name, args: { file_path: filePath } } }],
        };
    }
    function fileResult(id, name, output) {
        return {
            role: 'user',
