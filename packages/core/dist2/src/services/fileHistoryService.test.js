...
describe('FileHistoryService', () => {
    ...
    describe('trackEdit', () => {
        ...
        it('heals a failed entry on the next trackEdit attempt', async () => {
            ...
            expect(service.getSnapshots()[1].trackedFileBackups['a.txt'].failed).toBe(true);
            ...
            const backupPath = join(storageDir, 'file-history', 'test-session', p2Backup.backupFileName);
    ...
    describe('makeSnapshot', () => {
        ...
        it('should inherit version for unchanged files', async () => {
            ...
            expect(snapshots[1].trackedFileBackups[p2Key].backupFileName).toBe(snapshots[0].trackedFileBackups[p1Key].backupFileName);
        });
    ...
    describe('rewind', () => {
        ...
        it('should return filesFailed when backup file is missing on disk', async () => {
            ...
            const backupFileName = snapshots[0].trackedFileBackups[key].backupFileName;
            expect(backupFileName).not.toBeNull();
            const backupPath = join(storageDir, 'file-history', 'test-session', backupFileName);
        ...
        it('should report filesFailed when both backup and working file are gone', async () => {
            ...
            await service.makeSnapshot('p2');
            const snapshots = service.getSnapshots();
            const backupName = snapshots[0].trackedFileBackups['a.txt'].backupFileName;
        ...
        it('should throw when snapshot not found', async () => {
            await service.makeSnapshot('p1');
            await expect(service.rewind('nonexistent')).rejects.toThrow('The selected snapshot was not found');
        });
        it('should not truncate snapshot timeline when restore has failures', async () => {
            ...
            const key = Object.keys(snapshots[0].trackedFileBackups)[0];
            const backupFileName = snapshots[0].trackedFileBackups[key].backupFileName;
            await rm(join(storageDir, 'file-history', 'test-session', backupFileName), { force: true });
    ...
    describe('snapshot eviction', () => {
        const backupPath = (name) => join(storageDir, 'file-history', 'test-session', name);
        ...
        it('should delete orphaned backup files on overflow', async () => {
            ...
            evictedNames.push(service.getSnapshots()[0].trackedFileBackups['a.txt'].backupFileName);
            // 104 more snapshots, each with new content → fresh backup per snapshot.
            for (let i = 1; i < 105; i++) {
                await writeFile(file, `v${i}`);
                await service.makeSnapshot(`p${i}`);
                if (i < 5) {
                    evictedNames.push(service.getSnapshots()[i].trackedFileBackups['a.txt']
                        .backupFileName);
                }
            }
            ...
            for (const s of survivors) {
                ...
                if (bn)
                    expect(existsSync(backupPath(bn))).toBe(true);
            }
        });
        it('should preserve deduplicated backup files referenced by survivors', async () => {
            ...
            await service.makeSnapshot('p0');
            await service.trackEdit(file);
            const sharedName = service.getSnapshots()[0].trackedFileBackups['a.txt'].backupFileName;
    ...
    describe('rewind cleanup', () => {
        it('should delete backups orphaned by truncation', async () => {
            ...
            await service.makeSnapshot('p1');
            await service.trackEdit(file);
            const v1 = service.getSnapshots()[0].trackedFileBackups['a.txt'].backupFileName;
            await writeFile(file, 'v1');
            await service.makeSnapshot('p2');
            const v2 = service.getSnapshots()[1].trackedFileBackups['a.txt'].backupFileName;
            await writeFile(file, 'v2');
            await service.makeSnapshot('p3');
            const v3 = service.getSnapshots()[2].trackedFileBackups['a.txt'].backupFileName;
            await service.rewind('p1', true);
    ...
    describe('getTurnDiff', () => {
        ...
        it('compares the latest turn against the live worktree', async () => {
            ...
            expect(turn.files[0].linesAdded + turn.files[0].linesRemoved).toBeGreaterThan(0);
        });
...
//# sourceMappingURL=fileHistoryService.test.js.map