...
import { CommitAttributionService, computeCharContribution, } from './commitAttribution.js';
function makeStagedInfo(files, diffSizes, deleted, renamed) {
    return {
        files,
...
describe('CommitAttributionService', () => {
    ...
    it('should return defensive copies', () => {
        ...
        expect(service.getFileAttribution('/project/f.ts').aiContribution).not.toBe(99999);
    });
    ...
    describe('snapshot / restore', () => {
        it('should serialize and restore state', () => {
            ...
            expect(restored.getFileAttribution('/project/f.ts').aiContribution).toBe(5);
        });
    });
    describe('generateNotePayload', () => {
        it('should compute real AI/human percentages', () => {
            ...
            const note = service.generateNotePayload(staged, '/project', 'Qwen-Coder');
        ...
        it('should exclude generated files', () => {
            ...
            const staged = makeStagedInfo(['src/main.ts', 'package-lock.json', 'dist/bundle.js'], {
                'src/main.ts': 100,
        ...
        it('should sanitize internal model codenames', () => {
            ...
            expect(service.generateNotePayload(staged, '/project', 'qwen-72b').generator).toBe('Qwen-Coder');
            expect(service.generateNotePayload(staged, '/project', 'CustomAgent')
                .generator).toBe('CustomAgent');
        });
    ...
    describe('symlink-aware path canonicalisation', () => {
        beforeEach(() => {
            ...
            vi.mocked(fs.realpathSync).mockImplementation(((input) => {
                ...
                if (s.startsWith('/var/'))
                    return s.replace('/var/', '/private/var/');
                if (s === '/var')
                    return '/private/var';
                return s;
            }));
        ...
        it('records and looks up under the canonical path', () => {
            ...
            expect(service.getFileAttribution('/private/var/repo/src/main.ts')).toBeDefined();
        });
        ...
        it('clearAttributedFiles deletes by canonical key without realpath-ing the leaf', () => {
            ...
            expect(service.getFileAttribution('/var/repo/src/deleted.ts')).toBeDefined();
            ...
            service.clearAttributedFiles(new Set(['/private/var/repo/src/deleted.ts']));
            expect(service.getFileAttribution('/var/repo/src/deleted.ts')).toBeUndefined();
        });
        it('moves attribution across committed renames before payload generation', () => {
            ...
            service.applyCommittedRenames(new Map([['src/old.ts', 'src/new.ts']]), '/private/var/repo');
            expect(service.getFileAttribution('/var/repo/src/old.ts')).toBeUndefined();
        ...
        it('merges old-path attribution into an existing destination entry', () => {
            ...
            expect(attr.aiContribution).toBe('old ai text'.length + 'new ai text'.length);
            expect(service.getFileAttribution('/var/repo/src/old.ts')).toBeUndefined();
        });
        it('canonicalises keys on snapshot restore', () => {
            ...
            expect(service.getFileAttribution('/private/var/repo/src/legacy.ts')
                .aiContribution).toBe(99);
        });
        ...
        it('merges duplicate entries collapsed by canonicalisation', () => {
            ...
            const restored = service.getFileAttribution('/private/var/repo/src/dup.ts');
        ...
        it.each([
            ...
            ['null', null],
            ['empty string', ''],
        ])('falls back to client surface when snapshot.surface is non-string (%s)', (_label, badValue) => {
            const service = CommitAttributionService.getInstance();
            service.restoreFromSnapshot({
                type: 'attribution-snapshot',
                surface: badValue,
        ...
        ])('resets to fresh state when snapshot envelope is malformed (%s)', (_label, badPayload) => {
            const service = CommitAttributionService.getInstance();
            // Seed some pre-existing state to confirm the reset clears it.
            service.recordEdit('/project/preexisting.ts', null, 'hello');
            expect(service.getFileAttribution('/project/preexisting.ts')).toBeDefined();
            service.restoreFromSnapshot(badPayload);
            expect(service.getFileAttribution('/project/preexisting.ts')).toBeUndefined();
            expect(service.getSurface()).toBe('cli');
            expect(service.getPromptsSinceLastCommit()).toBe(0);
        });
        ...
        it.each([
            ...
            ['number', 42],
            ['null', null],
        ])('ignores non-object fileStates (%s) without polluting attribution map', (_label, badFileStates) => {
            const service = CommitAttributionService.getInstance();
            service.restoreFromSnapshot({
                type: 'attribution-snapshot',
                surface: 'cli',
                fileStates: badFileStates,
                promptCount: 0,
                promptCountAtLastCommit: 0,
            });
            expect(service.hasAttributions()).toBe(false);
...
//# sourceMappingURL=commitAttribution.test.js.map