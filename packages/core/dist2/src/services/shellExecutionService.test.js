...
import { getShellAbortReasonKind, ShellExecutionService, } from './shellExecutionService.js';
const { Terminal } = pkg;
// Hoisted Mocks
...
describe('ShellExecutionService', () => {
    ...
    const simulateExecution = async (command, simulation, config = shellExecutionConfig, options = {}) => {
        const abortController = new AbortController();
        const handle = await ShellExecutionService.execute(command, '/test/dir', onOutputEventMock, abortController.signal, true, config, options);
    ...
    describe('Aborting Commands', () => {
        ...
        it('signal.reason = { kind: "cancel" } still tree-kills (same as default)', async () => {
            const { result } = await simulateExecution('sleep 10', (pty, abortController) => {
                abortController.abort({ kind: 'cancel' });
                pty.onExit.mock.calls[0][0]({ exitCode: 1, signal: null });
            });
            expect(result.aborted).toBe(true);
            ...
            expect(mockProcessKill).toHaveBeenCalledWith(-mockPtyProcess.pid, 'SIGTERM');
        });
        it('signal.reason = { kind: "background" } skips kill and resolves with promoted: true (and aborted: false per design question 7)', async () => {
            ...
            const { result } = await simulateExecution('tail -f /tmp/never.log', (_pty, abortController) => {
                abortController.abort({
                    kind: 'background',
                    shellId: 'bg_test123',
                });
            ...
            expect(mockProcessKill).not.toHaveBeenCalledWith(-mockPtyProcess.pid, 'SIGTERM');
            expect(mockProcessKill).not.toHaveBeenCalledWith(-mockPtyProcess.pid, 'SIGKILL');
        });
        it('post-promotion: PTY data is no longer routed to onOutputEvent (handoff boundary)', async () => {
            ...
            const { result } = await simulateExecution('tail -f /tmp/never.log', (pty, ac) => {
        ...
        it('PR-2.5: post-promote bytes route to postPromote.onData when callback provided', async () => {
            ...
            const postPromoteHandler = onDataRegistrations[onDataRegistrations.length - 1][0];
        ...
        it('PR-2.5: postPromote.onSettle fires on natural child exit after promote', async () => {
            ...
            const settleCalls = [];
            const { result } = await simulateExecution('long-running-command', (pty, ac) => {
                ac.abort({
                    kind: 'background',
            ...
            const postPromoteExitHandler = onExitRegistrations[onExitRegistrations.length - 1][0];
        ...
        it('PR-2.5 wave-2 (C2): unexpected post-promote PTY error routes to onSettle as failure (does NOT crash the CLI)', async () => {
            ...
            const settleCalls = [];
            const { result } = await simulateExecution('long-running-with-error', (pty, ac) => {
                ac.abort({
                    kind: 'background',
            ...
            mockPtyProcess.emit('error', Object.assign(new Error('read EIO'), { code: 'EIO' }));
        ...
        it('PR-2.5 wave-3 (T6): post-promote IDisposables and error listener are released on settle (no GC roots dangling)', async () => {
            ...
            const settleCalls = [];
            const { result } = await simulateExecution('long-running-disposable', (pty, ac) => {
                ac.abort({
                    kind: 'background',
            ...
            const errorRemoves = removeListenerSpy.mock.calls.filter((args) => args[0] === 'error');
        ...
        it('PR-2.5: onData-only PTY caller has post-promote error + exit listeners (no crash, listeners disposed on exit)', async () => {
            const dataChunks = [];
            const { result } = await simulateExecution('tail -f /dev/null', (pty, ac) => {
                ac.abort({
                    kind: 'background',
            ...
            expect(() => mockPtyProcess.emit('error', new Error('post-promote pty err'))).not.toThrow();
        ...
        it('post-exit race: PTY background-promote refuses if process.kill(pid, 0) reports the pid is gone', async () => {
            ...
            const { result } = await simulateExecution('fast-and-cancelled', (pty, abortController) => {
                abortController.abort({
                    kind: 'background',
                    shellId: 'bg_test123',
                });
        ...
        it("post-promotion: ptyProcess error listener is removed via 'removeListener', NOT 'off' (regression guard for @lydell/node-pty)", async () => {
            ...
            const offErrorCalls = offSpy.mock.calls.filter(([event]) => event === 'error');
...
describe('ShellExecutionService child_process fallback', () => {
    ...
    // Helper function to run a standard execution simulation
    const simulateExecution = async (command, simulation, options = {}) => {
        const abortController = new AbortController();
        const handle = await ShellExecutionService.execute(command, '/test/dir', onOutputEventMock, abortController.signal, true, shellExecutionConfig, options);
        await new Promise((resolve) => process.nextTick(resolve));
    ...
    describe('Aborting Commands', () => {
        ...
        it('signal.reason = { kind: "background" } skips kill and resolves with promoted: true (and aborted: false per design question 7)', async () => {
            ...
            const { result } = await simulateExecution('tail -f /tmp/never.log', (cp, abortController) => {
            ...
            expect(mockProcessKill).not.toHaveBeenCalledWith(-mockChildProcess.pid, 'SIGTERM');
            expect(mockProcessKill).not.toHaveBeenCalledWith(-mockChildProcess.pid, 'SIGKILL');
        ...
        it('post-exit race: background-promote refuses if child is already terminal (exitCode/signalCode non-null)', async () => {
            ...
            mockPlatform.mockReturnValue('linux');
            const { result } = await simulateExecution('fast-and-cancelled', (cp, abortController) => {
        ...
        it('PR-2.5 child_process: post-promote stdout/stderr forward to postPromote.onData with SEPARATE decoders', async () => {
            ...
            mockPlatform.mockReturnValue('linux');
            const events = [];
            const { result } = await simulateExecution('tail -f', (cp, ac) => {
                ac.abort({
                    kind: 'background',
                    shellId: 'bg_cp_data',
                });
        ...
        it('PR-2.5 child_process: onSettle fires on `close` (NOT `exit`) so late chunks land before the registry transitions', async () => {
            ...
            const events = [];
            const settles = [];
            const { result } = await simulateExecution('cmd', (cp, ac) => {
                ac.abort({
                    kind: 'background',
                    shellId: 'bg_cp_close',
                });
        ...
        it('PR-2.5 wave-4 (T3): onData-only caller still gets decoder flush on close (no trailing multibyte loss)', async () => {
            ...
            const joined = dataChunks
                .map((d) => d.type === 'data' && typeof d.chunk === 'string' ? d.chunk : '')
                .join('');
        ...
        it('PR-2.5 wave-4 (T6): onData-only caller has post-promote `error` listener (does not crash CLI)', async () => {
            ...
            const { result } = await simulateExecution('cmd', (cp, ac) => {
                ...
                expect(() => cp.emit('error', new Error('post-promote err'))).not.toThrow();
        ...
        it('PR-2.5 wave-4 (T7): onSettle-only caller has stdout/stderr resumed (child does not block on full pipes)', async () => {
            ...
            const { result } = await simulateExecution('cmd', (cp, ac) => {
                ...
                if (cp.stdout)
                    cp.stdout.resume = stdoutResumeSpy;
                if (cp.stderr)
                    cp.stderr.resume = stderrResumeSpy;
                ac.abort({
                    kind: 'background',
                    shellId: 'bg_cp_t7',
                });
...
describe('getShellAbortReasonKind (defensive abort-reason read)', () => {
    ...
    it("returns 'background' for the canonical happy-path reason", () => {
        ...
        expect(getShellAbortReasonKind({ kind: 'background', shellId: 'bg_x' })).toBe('background');
    });
