...
import { DEFAULT_STOP_HOOK_BLOCK_CAP, MAX_STOP_HOOK_BLOCK_CAP, STOP_HOOK_BLOCK_CAP_ENV, appendStopHookBlockingCapWarning, formatStopHookBlockingCapWarning, normalizeStopHookBlockingCap, resolveStopHookBlockingCap, } from './stopHookCap.js';
describe('stop hook blocking cap', () => {
    afterEach(() => {
        delete process.env[STOP_HOOK_BLOCK_CAP_ENV];
    });
    it('normalizes invalid values to the default cap', () => {
        expect(normalizeStopHookBlockingCap(undefined)).toBe(DEFAULT_STOP_HOOK_BLOCK_CAP);
        ...
        expect(normalizeStopHookBlockingCap(Number.NaN)).toBe(DEFAULT_STOP_HOOK_BLOCK_CAP);
    });
    ...
    it('formats warnings for the relevant hook event', () => {
        expect(formatStopHookBlockingCapWarning('Stop', 8)).toBe('Stop hook blocked continuation 8 consecutive times; overriding and ending the turn.');
        expect(formatStopHookBlockingCapWarning('SubagentStop', 2)).toContain('SubagentStop hook blocked continuation 2 consecutive times');
        expect(formatStopHookBlockingCapWarning('Stop', 1)).toBe('Stop hook blocked continuation 1 consecutive time; overriding and ending the turn.');
    });
    it('appends cap warnings to visible subagent output', () => {
        ...
        expect(appendStopHookBlockingCapWarning('done', 'warning')).toBe('done\n\nwarning');
    });
});
//# sourceMappingURL=stopHookCap.test.js.map