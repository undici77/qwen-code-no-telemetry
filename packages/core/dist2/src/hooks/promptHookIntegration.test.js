...
describe('Prompt Hook Integration', () => {
    ...
    const createPreToolUseInput = (toolName, toolInput) => ({
    ...
    const createPromptHookConfig = (prompt, overrides = {}) => ({
    ...
    describe('HookRunner with Prompt Hook', () => {
        it('should execute prompt hook for PreToolUse event', async () => {
            ...
            const hookConfig = createPromptHookConfig('Evaluate this tool use: $ARGUMENTS. Allow if safe.');
        ...
        it('should block dangerous Bash command via prompt hook', async () => {
            ...
            const hookConfig = createPromptHookConfig('Analyze this Bash command for safety risks: $ARGUMENTS. Block dangerous commands like rm -rf.', { name: 'bash-security-check' });
    ...
    describe('HookSystem with Prompt Hooks', () => {
        it('should process hook definitions with prompt hooks', async () => {
            ...
            const result = await hookRunner.executeHook(hookDefinition.hooks[0], HookEventName.PreToolUse, input);
    ...
    describe('Hook Configuration Validation', () => {
        it('should return error result when Config not provided for prompt hooks', async () => {
            ...
            const hookConfig = createPromptHookConfig('Check: $ARGUMENTS');
            const input = createPreToolUseInput('Bash', { command: 'ls' });
            const result = await runnerWithoutConfig.executeHook(hookConfig, HookEventName.PreToolUse, input);
    ...
    describe('$ARGUMENTS Placeholder', () => {
        it('should properly substitute tool input in prompt', async () => {
            ...
            const hookConfig = createPromptHookConfig('Tool: $ARGUMENTS. Analyze the tool_name and tool_input fields.');
            const input = createPreToolUseInput('Bash', {
                command: 'git status',
...
//# sourceMappingURL=promptHookIntegration.test.js.map