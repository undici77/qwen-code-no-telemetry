...
import { clearActiveGoal, clearGoalTerminalObserver, getActiveGoal, notifyGoalTerminal, recordGoalIteration, setActiveGoal, } from './activeGoalStore.js';
...
const GOAL_JUDGE_TIMEOUT_REASON = 'Goal judge timed out; continue working toward the goal and run `/goal clear` to stop early.';
        ...
        return await Promise.race([
            ...
            new Promise((resolve) => {
                timeoutId = setTimeout(() => {
                    debugLogger.debug(`Goal judge exceeded ${GOAL_JUDGE_TIMEOUT_MS}ms; defaulting to not-met`);
...
export function createGoalStopHookCallback(args) {
    ...
    return async (input, context) => {
        ...
        if (verdict.impossible) {
            debugLogger.debug(`Impossible goal verdict suppressed: iterations=${latest.iterations} < MIN_IMPOSSIBLE_GOAL_ITERATIONS=${MIN_IMPOSSIBLE_GOAL_ITERATIONS}; continuing.`);
        }
