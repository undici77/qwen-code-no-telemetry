...
import { parseSkillContent, loadSkillsFromDir, validateConfig, parsePriorityField, normalizeSkillPriority, } from './skill-load.js';
...
Body.
`;
            const config = parseSkillContent(markdownWithInvalidPriority, testFilePath);
...
`)
        ...
        it('should load skills from symlinked directories', async () => {
            ...
            vi.mocked(fs.realpath).mockResolvedValue('/elsewhere/skills-repo/symlinked-skill');
...
`);
        ...
        it('should skip symlinks that do not point to a directory', async () => {
            ...
            vi.mocked(fs.realpath).mockResolvedValue('/elsewhere/skills-repo/some-file');
    ...
    describe('parsePriorityField', () => {
        ...
        it('rejects strings, including numeric-looking strings', () => {
            expect(parsePriorityField({ priority: 'high' }, filePath)).toBeUndefined();
        ...
        it('rejects NaN and Infinity', () => {
            expect(parsePriorityField({ priority: Number.NaN }, filePath)).toBeUndefined();
            expect(parsePriorityField({ priority: Number.POSITIVE_INFINITY }, filePath)).toBeUndefined();
            expect(parsePriorityField({ priority: Number.NEGATIVE_INFINITY }, filePath)).toBeUndefined();
        });
        it('rejects objects and arrays', () => {
            expect(parsePriorityField({ priority: { level: 1 } }, filePath)).toBeUndefined();
