...
export class AgentCore {
    ...
    async prepareTools() {
        ...
        const recursionGuardOnly = new Set([ToolNames.AGENT]);
        if (this.toolConfig) {
            ...
            toolsList.push(...onlyInlineDecls.filter((d) => !(d.name && recursionGuardOnly.has(d.name))));
        }
        else {
                    ...
                    try {
                        ...
                        this.eventEmitter?.emit(AgentEventType.TOOL_WAITING_APPROVAL, {
                            ...
                            respond: async (outcome, payload) => {
                                ...
                                await this.runInAgentFrames(() => waiting.confirmationDetails.onConfirm(outcome, payload), inheritedView, inheritedAgentId ?? undefined);
                            },
                            timestamp: Date.now(),
                        });
                    }
