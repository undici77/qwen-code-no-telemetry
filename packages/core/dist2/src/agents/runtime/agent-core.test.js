...
import { getCurrentAgentId, getRuntimeContentGenerator, runWithAgentContext, runWithRuntimeContentGenerator, } from './agent-context.js';
...
describe('AgentCore.runInAgentFrames', () => {
    ...
    function makeCore(name, runtimeView) {
        const promptConfig = { systemPrompt: '' };
        const modelConfig = { model: 'test-model' };
        const runConfig = { max_turns: 1 };
        return new AgentCore(name, {}, promptConfig, modelConfig, runConfig, undefined, undefined, undefined, runtimeView);
    }
    ...
    it('uses inheritedView for deferred-approval continuation when the agent owns no view', async () => {
        ...
        await runWithRuntimeContentGenerator(parentView, async () => {
            ...
            respondClosure = () => inheritingCore.runInAgentFrames(onConfirm, inheritedView);
        });
    ...
    it('restores the logical agent id for deferred-approval continuations', async () => {
        ...
        await runWithAgentContext('agent-123', async () => {
            ...
            respondClosure = () => core.runInAgentFrames(onConfirm, undefined, inheritedAgentId ?? undefined);
        });
...
describe('AgentCore.prepareTools', () => {
    ...
    function buildAgentForTools(toolConfig, fnDeclarations) {
        ...
        const core = new AgentCore('test-subagent', config, { systemPrompt: '' }, { model: 'test-model' }, { max_turns: 1 }, toolConfig);
        return { core, getFunctionDeclarationsSpy };
    }
    it('wildcard tools:["*"] inherits deferred tools (passes includeDeferred: true)', async () => {
        const fnDecls = [
            { name: 'core_tool', description: 'core' },
            {
                name: 'mcp__github__create_issue',
                description: 'mcp deferred',
            },
        ];
        const { core, getFunctionDeclarationsSpy } = buildAgentForTools({ tools: ['*'] }, fnDecls);
        ...
        expect(tools.map((t) => t.name)).toEqual(expect.arrayContaining(['core_tool', 'mcp__github__create_issue']));
    });
    it('absent toolConfig also inherits deferred tools (default = wildcard)', async () => {
        const fnDecls = [
            { name: 'lsp', description: 'language server' },
        ];
        const { core, getFunctionDeclarationsSpy } = buildAgentForTools(undefined, fnDecls);
    ...
    it('explicit tools list does NOT use the wildcard inherit path', async () => {
        ...
        const { core, getFunctionDeclarationsSpy } = buildAgentForTools({ tools: ['read_file', 'edit'] }, []);
        await core.prepareTools();
        expect(getFunctionDeclarationsSpy).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=agent-core.test.js.map