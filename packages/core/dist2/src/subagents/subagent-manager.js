...
export class SubagentManager {
    ...
    async createAgentHeadless(config, runtimeContext, options) {
        try {
            ...
            const runtimeView = await this.buildRuntimeContentGeneratorView(config, runtimeContext);
            const subagentContext = await this.buildSubagentContextOverride(runtimeContext);
            return await AgentHeadless.create(config.name, subagentContext, promptConfig, modelConfig, runConfig, toolConfig, options?.eventEmitter, options?.hooks, runtimeView);
        }
        catch (error) {
     ...
     * view is built here.
     */
    async buildRuntimeContentGeneratorView(config, base) {
        const resolvedModel = this.resolveModelOverride(config.model, base);
        if (!resolvedModel) {
            return undefined;
        }
        const authType = resolvedModel.authType ?? base.getContentGeneratorConfig().authType;
        const authOverrides = {
            authType: authType,
        };
        const view = await createRuntimeContentGeneratorView(base, base, resolvedModel.modelId, authOverrides);
