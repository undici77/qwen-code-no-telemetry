...
import { applyProviderInstallPlan, ProviderInstallError, } from '../index.js';
function createAdapter(modelProviders = {}) {
    const adapter = {
        getValue: vi.fn(),
        setValue: vi.fn(),
...
describe('applyProviderInstallPlan', () => {
    ...
    it('refuses an install plan that sets a reserved env var (NODE_OPTIONS)', async () => {
        ...
        await expect(applyProviderInstallPlan(plan, { settings: adapter })).rejects.toThrow(/reserved environment variable: NODE_OPTIONS/);
        ...
        expect(adapter.setValue).not.toHaveBeenCalledWith('env.NODE_OPTIONS', expect.anything());
    });
    it('matches the env denylist case-insensitively (Path)', async () => {
        ...
        await expect(applyProviderInstallPlan(plan, { settings: adapter })).rejects.toThrow(/reserved environment variable: Path/);
    });
    it.each(['TMP', 'TEMP', 'tmp'])('rejects the Windows temp-redirect env var %s', async (key) => {
        const adapter = createAdapter();
        const plan = {
            providerId: 'evil',
            authType: AuthType.USE_OPENAI,
        ...
        await expect(applyProviderInstallPlan(plan, { settings: adapter })).rejects.toThrow(/reserved environment variable/);
    });
    it('persists env, auth selection, selected model, and merged model providers', async () => {
        ...
        expect(adapter.setValue).toHaveBeenCalledWith('env.TEST_API_KEY', 'sk-test');
        ...
        expect(syncAuthState).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'new-model');
    ...
    it('writes provider state and legacy credentials', async () => {
        ...
        await applyProviderInstallPlan(plan, { settings: adapter });
        expect(adapter.setValue).toHaveBeenCalledWith('security.auth.apiKey', 'legacy-key');
        expect(adapter.setValue).toHaveBeenCalledWith('security.auth.baseUrl', 'https://example.com/v1');
        expect(adapter.setValue).toHaveBeenCalledWith('codingPlan.baseUrl', 'https://coding.example.com/v1');
    ...
    it('rolls back process.env on error', async () => {
        ...
        await expect(applyProviderInstallPlan(plan, { settings: adapter, refreshAuth })).rejects.toThrow('network error');
    ...
    it('deletes env var on rollback if it did not exist before', async () => {
        ...
        await expect(applyProviderInstallPlan(plan, { settings: adapter, refreshAuth })).rejects.toThrow('fail');
    ...
    it('restores runtime model providers when refreshAuth rejects after reloadModelProviders ran', async () => {
        ...
        await expect(applyProviderInstallPlan(plan, {
            settings: adapter,
            reloadModelProviders,
            refreshAuth,
    ...
    it('still rolls back env vars when backup() throws before persist', async () => {
        ...
        await expect(applyProviderInstallPlan(plan, { settings: adapter })).rejects.toThrow('backup failed');
    ...
    it('continues env rollback even when settings.restore itself throws', async () => {
        ...
        await expect(applyProviderInstallPlan(plan, { settings: adapter, refreshAuth })).rejects.toThrow('original error');
    ...
    it('annotates the rethrown error with the failing step and preserves the original cause', async () => {
        ...
        expect(err.cause.message).toBe('endpoint unreachable');
    });
