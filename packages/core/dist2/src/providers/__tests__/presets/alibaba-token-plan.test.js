...
import { AuthType, TOKEN_PLAN_ENV_KEY, TOKEN_PLAN_BASE_URL, tokenPlanProvider, buildInstallPlan, buildProviderTemplate, computeModelListVersion, getDefaultModelIds, resolveBaseUrl, providerMatchesCredentials, } from '@qwen-code/qwen-code-core';
describe('token plan provider', () => {
    ...
    it('matches Token Plan credentials', () => {
        expect(providerMatchesCredentials(tokenPlanProvider, TOKEN_PLAN_BASE_URL, TOKEN_PLAN_ENV_KEY)).toBe(true);
        expect(providerMatchesCredentials(tokenPlanProvider, 'https://custom.example.com/v1', 'CUSTOM_API_KEY')).toBe(false);
    });
});
//# sourceMappingURL=alibaba-token-plan.test.js.map