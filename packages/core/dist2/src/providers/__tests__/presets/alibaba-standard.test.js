...
import { AuthType, alibabaStandardProvider, buildInstallPlan, resolveBaseUrl, providerMatchesCredentials, } from '@qwen-code/qwen-code-core';
describe('alibabaStandardProvider', () => {
    ...
    it('offers multiple region endpoints', () => {
        ...
        const urls = alibabaStandardProvider.baseUrl.map((o) => o.url);
    ...
    it('resolves baseUrl for known region', () => {
        const url = resolveBaseUrl(alibabaStandardProvider, 'https://dashscope-intl.aliyuncs.com/compatible-mode/v1');
    ...
    it('matches credentials for all base URL options', () => {
        ...
        for (const url of urls) {
            expect(providerMatchesCredentials(alibabaStandardProvider, url, 'DASHSCOPE_API_KEY')).toBe(true);
        }
        expect(providerMatchesCredentials(alibabaStandardProvider, 'https://unknown.com', 'DASHSCOPE_API_KEY')).toBe(false);
    });
});
//# sourceMappingURL=alibaba-standard.test.js.map