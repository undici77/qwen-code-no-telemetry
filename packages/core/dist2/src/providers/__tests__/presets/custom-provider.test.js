...
import { AuthType, customProvider, CUSTOM_API_KEY_ENV_PREFIX, buildInstallPlan, shouldShowStep, } from '@qwen-code/qwen-code-core';
...
describe('generateCustomEnvKey', () => {
    it('produces a deterministic URL-based key with a stable hash suffix', () => {
        const key1 = generateCustomEnvKey(AuthType.USE_OPENAI, 'https://api.example.com/v1');
        const key2 = generateCustomEnvKey(AuthType.USE_OPENAI, 'https://api.example.com/v1');
        expect(key1).toBe(key2);
        // Readable prefix + 6-hex-char SHA-256 suffix.
        expect(key1).toMatch(new RegExp(`^${CUSTOM_API_KEY_ENV_PREFIX}OPENAI_HTTPS_API_EXAMPLE_COM_V1_[0-9A-F]{12}$`));
    });
    it('produces different keys for different protocols', () => {
        const k1 = generateCustomEnvKey(AuthType.USE_OPENAI, 'https://api.example.com');
        const k2 = generateCustomEnvKey(AuthType.USE_ANTHROPIC, 'https://api.example.com');
    ...
    it('disambiguates structurally distinct URLs that normalize identically', () => {
        ...
        const dotted = generateCustomEnvKey(AuthType.USE_OPENAI, 'https://api.example.com');
        const dashed = generateCustomEnvKey(AuthType.USE_OPENAI, 'https://api-example.com');
        const underscored = generateCustomEnvKey(AuthType.USE_OPENAI, 'https://api_example.com');
    ...
    it('normalizes special characters to underscores in the readable part', () => {
        ...
        expect(k1).toMatch(new RegExp(`^${CUSTOM_API_KEY_ENV_PREFIX}OPENAI_HTTP_API_A_B_COM_[0-9A-F]{12}$`));
    });
    it('handles empty strings', () => {
        const key = generateCustomEnvKey('', '');
...
//# sourceMappingURL=custom-provider.test.js.map