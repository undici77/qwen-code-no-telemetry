...
import { DefaultHookOutput, HookPhase } from '../hooks/types.js';
...
describe('TodoWriteTool', () => {
    ...
    describe('execute', () => {
        ...
        it('should block todo creation when validation hook returns block', async () => {
            ...
            const invocation = tool.build(params);
            const result = await invocation.execute(mockAbortSignal);
            expect(mockHookSystem.fireTodoCreatedEvent).toHaveBeenCalledWith('1', 'Task 1', 'pending', params.todos, HookPhase.Validation, mockAbortSignal);
            expect(mockFs.writeFile).not.toHaveBeenCalled();
            expect(result.llmContent).toContain('Todo creation blocked: Creation denied');
            expect(result.returnDisplay).toBe('Todo creation blocked: Creation denied');
        });
        it('should block todo completion when validation hook returns block', async () => {
            ...
            const invocation = tool.build(params);
            const result = await invocation.execute(mockAbortSignal);
            expect(mockHookSystem.fireTodoCompletedEvent).toHaveBeenCalledWith('1', 'Task 1', 'in_progress', params.todos, HookPhase.Validation, mockAbortSignal);
            expect(mockFs.writeFile).not.toHaveBeenCalled();
            expect(result.llmContent).toContain('Todo completion blocked: Completion denied');
            expect(result.returnDisplay).toBe('Todo completion blocked: Completion denied');
        });
        it('should ignore postWrite block decisions after persistence', async () => {
            ...
            expect(mockHookSystem.fireTodoCreatedEvent).toHaveBeenNthCalledWith(1, '1', 'Task 1', 'pending', params.todos, HookPhase.Validation, mockAbortSignal);
            expect(mockHookSystem.fireTodoCreatedEvent).toHaveBeenNthCalledWith(2, '1', 'Task 1', 'pending', params.todos, HookPhase.PostWrite, mockAbortSignal);
            expect(result.llmContent).toContain('Todos have been modified successfully');
        });
        ...
        it('should report success when postWrite hooks fail after persistence', async () => {
            ...
            expect(mockFs.writeFile).toHaveBeenCalled();
            expect(result.llmContent).toContain('Todos have been modified successfully');
            expect(result.llmContent).toContain('Todos were persisted successfully, but post-write hooks failed with error: Hook timeout.');
            expect(result.returnDisplay).toMatchObject({
                type: 'todo_list',
                todos: params.todos,
            });
