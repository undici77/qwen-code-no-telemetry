...
describe('scoreTool', () => {
    it('gives higher score on exact name match than substring', () => {
        ...
        expect(scoreTool(exactTool, ['grep'])).toBeGreaterThan(scoreTool(substringTool, ['grep']));
    });
    it('boosts MCP tools above built-in tools with equal match type', () => {
        ...
        const mcpCallable = {};
        const mcp = new DiscoveredMCPTool(mcpCallable, 'slack', 'send_message', 'an action', {});
    ...
    it('MCP tools with `mcp__server__name` format get exact-suffix score on the trailing toolname', () => {
        ...
        const mcpCallable = {};
        const mcp = new DiscoveredMCPTool(mcpCallable, 'github', 'create_issue', 'create a github issue', {});
    ...
    it('scores searchHint word matches', () => {
        ...
        expect(scoreTool(withHint, ['schedule'])).toBeGreaterThan(scoreTool(withoutHint, ['schedule']));
    });
...
describe('ToolSearchTool', () => {
    ...
    it('enforces max_results cap — schema rejects values above HARD_MAX_RESULTS', () => {
        ...
        expect(() => tool.build({ query: 'slack', max_results: 100 })).toThrow(/max_results must be <= 20/);
    });
    ...
    it('caps select: mode by max_results and surfaces dropped names', async () => {
        ...
        for (let i = 0; i < 10; i++) {
            registry.registerTool(new MockTool({ name: `tool_${i}`, shouldDefer: true }));
        }
        const tool = new ToolSearchTool(config);
    ...
    it('rejects empty query at build time via schema (minLength)', () => {
        ...
        const tool = new ToolSearchTool(config);
        expect(() => tool.build({ query: '' })).toThrow(/must NOT have fewer than 1 character/i);
    });
    ...
    it('select: mode dedupes repeated names', async () => {
        registry.registerTool(new MockTool({ name: 'cron_create', shouldDefer: true }));
        const tool = new ToolSearchTool(config);
        const invocation = tool.build({
    ...
    it('select: mode still works for non-deferred tools (e.g. re-inspect schema)', async () => {
        registry.registerTool(new MockTool({ name: 'core_tool', shouldDefer: false }));
        const tool = new ToolSearchTool(config);
    ...
    it("rolls back this call's reveals when setTools() throws", async () => {
        ...
        registry.registerTool(new MockTool({ name: 'cron_create', shouldDefer: true }));
        registry.registerTool(new MockTool({ name: 'cron_list', shouldDefer: true }));
    ...
    it('treats a null GeminiClient identically to setTools() throwing', async () => {
        ...
        registry.registerTool(new MockTool({ name: 'cron_create', shouldDefer: true }));
        vi.spyOn(config, 'getGeminiClient').mockReturnValue(null);
        const tool = new ToolSearchTool(config);
        const result = await tool
            .build({ query: 'select:cron_create' })
            .execute(new AbortController().signal);
...
describe('ToolRegistry.clearRevealedDeferredTools', () => {
    it('empties the revealed set so new sessions start clean', async () => {
        ...
        expect(registry.getFunctionDeclarations().map((d) => d.name)).not.toContain('cron_create');
    });
});
//# sourceMappingURL=tool-search.test.js.map