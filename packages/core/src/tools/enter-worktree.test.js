...
function makeMockConfig(targetDir = '/tmp/mock-repo') {
    return {
...
describe('GitWorktreeService.validateUserWorktreeSlug', () => {
    it('accepts simple slugs', () => {
        expect(GitWorktreeService.validateUserWorktreeSlug('my-feature')).toBeNull();
        expect(GitWorktreeService.validateUserWorktreeSlug('foo123')).toBeNull();
        expect(GitWorktreeService.validateUserWorktreeSlug('foo.bar_baz-1')).toBeNull();
    });
    it('rejects empty', () => {
        expect(GitWorktreeService.validateUserWorktreeSlug('')).toMatch(/non-empty/i);
    });
    it('rejects path-traversal patterns', () => {
        expect(GitWorktreeService.validateUserWorktreeSlug('../etc/passwd')).not.toBeNull();
        expect(GitWorktreeService.validateUserWorktreeSlug('a/b')).not.toBeNull();
        expect(GitWorktreeService.validateUserWorktreeSlug('foo..bar')).toMatch(/must not.*\.\./i);
        expect(GitWorktreeService.validateUserWorktreeSlug('.hidden')).toMatch(/must not start/i);
        expect(GitWorktreeService.validateUserWorktreeSlug('-leadingdash')).toMatch(/must not start/i);
    });
    ...
    it('rejects strings longer than 64 chars', () => {
        expect(GitWorktreeService.validateUserWorktreeSlug('a'.repeat(65))).toMatch(/64/);
        expect(GitWorktreeService.validateUserWorktreeSlug('a'.repeat(64))).toBeNull();
    });
    it('reserves the `agent-` prefix for ephemeral agent worktrees', () => {
        ...
        // sweep.
        expect(GitWorktreeService.validateUserWorktreeSlug('agent-feature')).toMatch(/reserved/i);
        expect(GitWorktreeService.validateUserWorktreeSlug('agent-1234567g')).toMatch(/reserved/i);
        expect(GitWorktreeService.validateUserWorktreeSlug('agent-12345678')).toMatch(/reserved/i);
        ...
        expect(GitWorktreeService.validateUserWorktreeSlug('agent-aabbccd')).toBeNull();
        expect(GitWorktreeService.validateUserWorktreeSlug('agent-1234567')).toBeNull();
    ...
    it('round-trip: every generated agent slug passes user validation', async () => {
        ...
        const { generateAgentWorktreeSlug } = await import('../services/gitWorktreeService.js');
        for (let i = 0; i < 50; i++) {
...
describe('generateAgentWorktreeSlug', () => {
    it('produces slugs that match AGENT_WORKTREE_SLUG_PATTERN', async () => {
        const { generateAgentWorktreeSlug, AGENT_WORKTREE_SLUG_PATTERN } = await import('../services/gitWorktreeService.js');
        for (let i = 0; i < 50; i++) {
            const slug = generateAgentWorktreeSlug();
...
describe('worktreeBranchForSlug', () => {
    it('prefixes the slug with WORKTREE_BRANCH_PREFIX', async () => {
        const { worktreeBranchForSlug, WORKTREE_BRANCH_PREFIX } = await import('../services/gitWorktreeService.js');
        expect(worktreeBranchForSlug('feat-x')).toBe(`${WORKTREE_BRANCH_PREFIX}feat-x`);
        expect(worktreeBranchForSlug('agent-aabbccd')).toBe(`${WORKTREE_BRANCH_PREFIX}agent-aabbccd`);
    });
});
...
describe('session marker round-trip', () => {
    it('write then read returns the same session id', async () => {
        ...
        const os = await import('node:os');
        const { writeWorktreeSessionMarker, readWorktreeSessionMarker, WORKTREE_SESSION_FILE, } = await import('../services/gitWorktreeService.js');
        ...
        try {
            ...
            const onDisk = await fs.readFile(path.join(tmp, WORKTREE_SESSION_FILE), 'utf8');
    ...
    it('returns null when the marker file is missing', async () => {
        ...
        const path = await import('node:path');
        const os = await import('node:os');
        const { readWorktreeSessionMarker } = await import('../services/gitWorktreeService.js');
        const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-wt-session-'));
        try {
    ...
    it('returns null when the marker file is empty / whitespace', async () => {
        ...
        const pathMod = await import('node:path');
        const os = await import('node:os');
        const { readWorktreeSessionMarker, WORKTREE_SESSION_FILE } = await import('../services/gitWorktreeService.js');
        const tmp = await fs.mkdtemp(pathMod.join(os.tmpdir(), 'qwen-wt-session-'));
        try {
            await fs.writeFile(pathMod.join(tmp, WORKTREE_SESSION_FILE), '   \n  \n', 'utf8');
            expect(await readWorktreeSessionMarker(tmp)).toBeNull();
        }
        finally {
            await fs.rm(tmp, { recursive: true, force: true });
...
describe('GitWorktreeService.getUserWorktreesDir / getUserWorktreePath', () => {
    it('uses .qwen/worktrees under the project root', () => {
        ...
        // separator is `\`, not `/`.
        expect(service.getUserWorktreesDir()).toBe(path.join(root, '.qwen', 'worktrees'));
        expect(service.getUserWorktreePath('feat-x')).toBe(path.join(root, '.qwen', 'worktrees', 'feat-x'));
    });
});
...
describe('ExitWorktreeTool metadata and validation', () => {
    ...
    it('requires action to be keep or remove', () => {
        ...
        expect(tool.validateToolParams({ name: 'foo', action: 'remove' })).toBeNull();
    });
    it('rejects invalid name slugs', () => {
        const tool = new ExitWorktreeTool(makeMockConfig());
        expect(tool.validateToolParams({ name: 'a/b', action: 'remove' })).not.toBeNull();
    });
});
//# sourceMappingURL=enter-worktree.test.js.map