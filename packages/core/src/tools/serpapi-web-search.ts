...
// No-telemetry fork patch — see NO_TELEMETRY_GUIDELINES.md §1.5. This is the
// fork-owned SerpApi backend for the built-in web_search tool. It lives here
// rather than in `web-search.ts` so upstream's DashScope/ModelStudio rewrite
// of that file can never conflict with it: `web-search.ts` is a re-export
// shim onto this module and is merged with `merge=ours`.
//
// The exported contract (names, shapes, opt-out gate semantics) matches
// upstream's `web-search.ts` exactly, so upstream's own consumers — the
// registry block in `config/config.ts` and the re-export in `index.ts` — work
// against this backend without a single fork edit.
 ...
 * loader from `tools.webSearch` in settings.json merged with env overrides
 * (`ENABLE_WEB_SEARCH`, `SERPAPI_API_KEY`).
 *
 * This is a superset of upstream's shape. Upstream's `model`, `webExtractor`,
 * `baseUrl` and `apiKeyEnv` are accepted and deliberately ignored: the fork's
 * only search backend is SerpApi, so a DashScope endpoint or model can never
 * be reached. Keeping the keys declared upstream-shaped is what lets
 * `cli/src/config/config.ts` and `settingsSchema.ts` stay upstream-shaped and
 * conflict-free — see NO_TELEMETRY_GUIDELINES.md §1.5.
 */
export interface WebSearchSettings {
   ...
   */
  gl?: string;
  /** Accepted and ignored — upstream selects a DashScope search model. */
  model?: string;
  /** Accepted and ignored — upstream's page-opening search agent. */
...
/** Upstream's name for the resolved backend; the fork serves SerpApi only. */
export type WebSearchBackendConfig = SerpApiBackendConfig;
...
export type WebSearchGateResult =
  ...
  | {
      ...
      notice: string;
      /**
       * True when the user never asked for web search — `enabled` was left
       * unset and no SerpApi key resolves. Upstream's opt-out registry keeps
       * the tool off without a startup notice when this is set, so an
       * unconfigured install stays quiet instead of nagging on every start.
       */
      silent?: boolean;
    };
 ...
 * notice) and re-checked per invocation. A SerpApi API key is the only thing
 * that can turn the tool on; nothing here can resolve a non-SerpApi backend.
 */
