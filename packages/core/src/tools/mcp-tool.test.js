...
describe('DiscoveredMCPTool', () => {
    ...
    describe('execute', () => {
        ...
        it('should handle empty result from getDisplayFromParts', async () => {
            ...
            const invocation = tool.build(params);
            const toolResult = await invocation.execute(new AbortController().signal);
            expect(toolResult.returnDisplay).toBe('[Error: Could not parse tool response]');
            expect(toolResult.llmContent).toEqual([
        ...
        it('should handle an AudioBlock response', async () => {
            ...
            expect(toolResult.returnDisplay).toBe(`[Tool '${serverToolName}' provided the following audio data with mime-type: audio/mp3]\n[audio/mp3]`);
        });
        it('should handle a ResourceLinkBlock response', async () => {
            ...
            expect(toolResult.returnDisplay).toBe('Resource Link: My Resource at file:///path/to/thing');
        });
        ...
        it('should handle an embedded binary ResourceBlock response', async () => {
            ...
            expect(toolResult.returnDisplay).toBe(`[Tool '${serverToolName}' provided the following embedded resource with mime-type: application/octet-stream]\n[application/octet-stream]`);
        });
    ...
    describe('getDefaultPermission and getConfirmationDetails', () => {
        ...
        it('should have onConfirm as a no-op', async () => {
            ...
            if ('onConfirm' in confirmation &&
                ...
                await confirmation.onConfirm(ToolConfirmationOutcome.ProceedAlwaysProject);
    ...
    describe('output truncation for large MCP results', () => {
        ...
        it('should truncate large text results from direct client execution', async () => {
            ...
            const truncTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, true, // trust
            undefined, mockConfigWithTruncation, mockMcpClient);
            const invocation = truncTool.build({ param: 'test' });
            const result = await invocation.execute(new AbortController().signal);
            // The text part in llmContent should be truncated
            const textParts = result.llmContent.filter((p) => p.text);
        ...
        it('should truncate large text results from callable tool execution', async () => {
            ...
            mockCallTool.mockResolvedValue(mockMcpToolResponseParts);
            const truncTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, true, undefined, mockConfigWithTruncation);
            const invocation = truncTool.build({ param: 'test' });
            const result = await invocation.execute(new AbortController().signal);
            const textParts = result.llmContent.filter((p) => p.text);
            const combinedText = textParts.map((p) => p.text).join('');
        ...
        it('should not truncate small text results', async () => {
            ...
            const truncTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, true, undefined, mockConfigWithTruncation, mockMcpClient);
            const invocation = truncTool.build({ param: 'test' });
            const result = await invocation.execute(new AbortController().signal);
        ...
        it('should not truncate non-text content (images, audio)', async () => {
            ...
            const inlineDataParts = result.llmContent.filter((p) => p.inlineData);
        ...
        it('should truncate only text parts in mixed content', async () => {
            ...
            const invocation = truncTool.build({ param: 'test' });
            const result = await invocation.execute(new AbortController().signal);
            const parts = result.llmContent;
            // Text should be truncated
            const textPart = parts.find((p) => p.text && !p.text.startsWith('[Tool'));
        ...
        it('should not truncate when config is not provided', async () => {
            ...
            const truncTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, undefined, undefined, // no config
            mockMcpClient);
            const invocation = truncTool.build({ param: 'test' });
            const result = await invocation.execute(new AbortController().signal);
    ...
    describe('streaming progress for long-running MCP tools', () => {
        ...
        it('should forward MCP progress notifications to updateOutput callback during execution', async () => {
            ...
            const streamingTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, // trust
            undefined, // nameOverride
            undefined, // cliConfig
            mockMcpClient);
            const invocation = streamingTool.build(params);
            const updateOutputSpy = vi.fn();
            const result = await invocation.execute(new AbortController().signal, updateOutputSpy);
            ...
            expect(updateOutputSpy).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should show incremental progress for multi-step browser automation', async () => {
            ...
            const streamingTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, undefined, undefined, mockMcpClient);
            const invocation = streamingTool.build(params);
            const receivedUpdates = [];
            const updateOutputCallback = (output) => {
                receivedUpdates.push(output);
            };
            await invocation.execute(new AbortController().signal, updateOutputCallback);
    ...
    describe('auto-reconnect on connection error', () => {
        it('should attempt reconnect and retry on connection error', async () => {
            ...
            const newMockMcpClient = {
                callTool: vi.fn().mockResolvedValueOnce(successResult),
            };
            const newTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, undefined, undefined, newMockMcpClient);
            ...
            mockMcpClient.callTool.mockRejectedValueOnce(connectionError);
            const reconnectTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, undefined, mockConfig, mockMcpClient);
        ...
        it('should not retry on non-connection errors', async () => {
            ...
            const reconnectTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, undefined, mockConfig, mockMcpClient);
            const invocation = reconnectTool.build(params);
            await expect(invocation.execute(new AbortController().signal)).rejects.toThrow('Invalid parameters');
            expect(mockMcpClient.callTool).toHaveBeenCalledTimes(1);
        });
        it('should not retry after reconnection attempt fails', async () => {
            ...
            const secondMockMcpClient = {
                callTool: vi.fn().mockRejectedValue(new Error('ECONNREFUSED')),
            };
            const secondTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, undefined, undefined, secondMockMcpClient);
            const discoverToolsForServer = vi.fn().mockResolvedValue(undefined);
            const mockConfig = {
                isTrustedFolder: () => true,
                getToolRegistry: () => ({
            ...
            const reconnectTool = new DiscoveredMCPTool(mockCallableToolInstance, serverName, serverToolName, baseDescription, inputSchema, undefined, undefined, mockConfig, mockMcpClient);
            const invocation = reconnectTool.build(params);
            await expect(invocation.execute(new AbortController().signal)).rejects.toThrow('ECONNREFUSED');
            expect(mockMcpClient.callTool).toHaveBeenCalledTimes(1);
        ...
        it('should reconnect when MCP error occurs and server is disconnected', async () => {
            ...
            const mockMcpClient = {
                callTool: vi
                    .fn()
                    .mockRejectedValueOnce(new Error('MCP error -32602: Invalid request')),
            };
            const newMockMcpClient = {
                callTool: vi
                    .fn()
