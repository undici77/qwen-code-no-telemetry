...
import { createApprovalModeOverride, hasRebuiltToolRegistry, rebuildToolRegistryOnOverride, TOOL_REGISTRY_REBUILT, } from './agent.js';
...
describe('createApprovalModeOverride bound-tool isolation', () => {
    ...
    it('returns a Config whose registry is a distinct instance from the parent', async () => {
        ...
        parent.toolRegistry = parentRegistry;
        const { config: child } = await createApprovalModeOverride(parent, ApprovalMode.AUTO_EDIT);
    ...
    it('binds Edit / WriteFile / ReadFile on the override registry to the override Config, not the parent', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(parentEdit.config).toBe(parent);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(childEdit.config).toBe(child);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(parentRead.config).toBe(parent);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(childRead.config).toBe(child);
    });
    it('routes child tools through the child FileReadCache, not the parent', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const boundConfig = childEdit.config;
    ...
    it('preserves the override approval mode on the bound tools', async () => {
        ...
        const { config: child } = await createApprovalModeOverride(parent, ApprovalMode.YOLO);
    ...
    it('copies discovered tools from the parent registry without re-discovering', async () => {
        ...
        const parentNonBareRegistry = await parentNonBare.createToolRegistry(undefined, { skipDiscovery: true });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        parentNonBare.toolRegistry = parentNonBareRegistry;
        const { config: childNonBare } = await createApprovalModeOverride(parentNonBare, ApprovalMode.AUTO_EDIT);
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(childNonBareWrite.config).toBe(childNonBare);
    });
    describe('TOOL_REGISTRY_REBUILT marker propagation', () => {
        ...
        it('hasRebuiltToolRegistry returns true even when checked on an Object.create wrapper above the rebuilt Config', async () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            parent.toolRegistry = parentRegistry;
            const { config: upstream } = await createApprovalModeOverride(parent, ApprovalMode.AUTO_EDIT);
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const bgWrapper = Object.create(upstream);
            bgWrapper.getShouldAvoidPermissionPrompts = () => true;
            // The plain own-property check would miss this — Symbol lookup
            // doesn't.
            expect(Object.prototype.hasOwnProperty.call(bgWrapper, 'getToolRegistry')).toBe(false);
            expect(hasRebuiltToolRegistry(bgWrapper)).toBe(true);
        });
        it('hasRebuiltToolRegistry returns false on a fresh Config and on a wrapper that was not rebuilt', () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const plainWrapper = Object.create(parent);
            plainWrapper.getApprovalMode = () => ApprovalMode.AUTO_EDIT;
            expect(hasRebuiltToolRegistry(plainWrapper)).toBe(false);
        });
        it('rebuildToolRegistryOnOverride installs the marker and an own getToolRegistry', async () => {
            ...
            await rebuildToolRegistryOnOverride(override, parent);
            expect(Object.prototype.hasOwnProperty.call(override, 'getToolRegistry')).toBe(true);
            expect(Object.prototype.hasOwnProperty.call(override, TOOL_REGISTRY_REBUILT)).toBe(true);
            expect(override[TOOL_REGISTRY_REBUILT]).toBe(true);
            expect(hasRebuiltToolRegistry(override)).toBe(true);
        });
    });
});
//# sourceMappingURL=agent-override.test.js.map