...
import { FORK_AGENT, FORK_PLACEHOLDER_RESULT, buildForkedMessages, buildChildMessage, buildWorktreeNotice, isInForkExecution, runInForkContext, } from './fork-subagent.js';
...
import { getCurrentAgentId, runWithAgentContext, } from '../../agents/runtime/agent-context.js';
 ...
 */
export const TOOL_REGISTRY_REBUILT = Symbol.for('qwen-code:tool-registry-rebuilt');
/**
 ...
 */
export async function createApprovalModeOverride(base, mode) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
class AgentToolInvocation extends BaseToolInvocation {
    ...
    async createForkSubagent(agentConfig, eventEmitter = this.eventEmitter) {
        ...
        if (generationConfig?.systemInstruction) {
            ...
            const parentToolDecls = generationConfig.tools
                ?.flatMap((t) => t.functionDeclarations ?? [])
                .filter((d) => !(d.name && EXCLUDED_TOOLS_FOR_SUBAGENTS.has(d.name))) ?? [];
            promptConfig = {
    ...
    async execute(signal, updateOutput) {
        ...
        try {
            ...
            if (worktreeIsolation) {
                const notice = buildWorktreeNotice(this.config.getTargetDir(), worktreeIsolation.path);
                taskPrompt = `${notice}\n\n${taskPrompt}`;
            }
            ...
            if (shouldRunInBackground) {
                ...
                const bgBody = async () => {
                    try {
                        ...
                        const finalText = appendStopHookBlockingCapWarning(bgSubagent.getFinalText(), stopHookWarning) + wtSuffix;
                ...
                const framedBgBody = () => runWithAgentContext(hookOpts.agentId, bgBody);
            ...
            if (isFork) {
                ...
                const runFramedFork = () => runWithAgentContext(hookOpts.agentId, async () => {
                    try {
            ...
            const runFramed = () => runWithAgentContext(hookOpts.agentId, () => this.runSubagentWithHooks(subagent, contextState, fgHookOpts));
            ...
            const fgJsonlPath = getAgentJsonlPath(fgProjectDir, fgSessionId, hookOpts.agentId);
            ...
            try {
                ({ cleanup: cleanupFgJsonl } = attachJsonlTranscriptWriter(this.eventEmitter, fgJsonlPath, {
                    agentId: hookOpts.agentId,
                    agentName: subagentConfig.name,
                    agentColor: subagentConfig.color,
