...
import { GitWorktreeService, WORKTREE_SESSION_FILE, worktreeBranchForSlug, writeWorktreeSessionMarker, } from '../services/gitWorktreeService.js';
function makeMockConfig(targetDir = process.cwd()) {
...
describe('ExitWorktreeTool', () => {
    ...
    describe('validateToolParams', () => {
        it('requires a non-empty name', () => {
            const tool = new ExitWorktreeTool(makeMockConfig());
            expect(tool.validateToolParams({ name: '', action: 'keep' })).toMatch(/non-empty/i);
        });
        ...
        it('rejects slugs that would resolve outside the worktrees dir', () => {
            ...
            expect(tool.validateToolParams({ name: '../etc', action: 'remove' })).not.toBeNull();
        });
    ...
    describe('confirmation type — round-7 AUTO_EDIT bypass guard', () => {
        ...
        it("returns type 'exec' for action=remove (NOT auto-approved by AUTO_EDIT)", async () => {
            const tool = new ExitWorktreeTool(makeMockConfig());
            const inv = tool.build({ name: 'foo', action: 'remove' });
            const details = await inv.getConfirmationDetails(new AbortController().signal);
    ...
    describe('execute() — session ownership & lifecycle', () => {
        ...
        async function provisionWorktree(slug) {
        ...
        it('refuses remove when the marker names a different session', async () => {
            ...
            const marker = await fs.readFile(path.join(wtPath, WORKTREE_SESSION_FILE), 'utf8');
            ...
            expect(result.error?.message).toMatch(/different session.*owner=session-creator/i);
        ...
        it('allows removal when the worktree predates the session-marker guard', async () => {
            ...
            await expect(fs.access(path.join(wtPath, WORKTREE_SESSION_FILE))).rejects.toBeDefined();
            const cfg = {
                getTargetDir: () => repoRoot,
                getSessionId: () => 'session-stranger',
            };
        ...
        it('marker also written by writeWorktreeSessionMarker survives round-trip', async () => {
            ...
            const re = await fs.readFile(path.join(wtPath, WORKTREE_SESSION_FILE), 'utf8');
            expect(re.trim()).toBe('rewritten-id');
        });
    });
});
//# sourceMappingURL=exit-worktree.test.js.map