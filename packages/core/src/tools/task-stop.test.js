...
describe('TaskStopTool', () => {
    ...
    describe('dream task support', () => {
        it('cancels a running dream by routing through MemoryManager.cancelTask', async () => {
            ...
            const memoryManager = {
                getTask: vi.fn((id) => id === 'dream-running-1' ? dreamRecord : undefined),
                cancelTask,
            };
            const localConfig = {
                getBackgroundTaskRegistry: () => registry,
            ...
            const result = await localTool.validateBuildAndExecute({ task_id: 'dream-running-1' }, new AbortController().signal);
        ...
        it('returns NOT_RUNNING when the dream is already terminal', async () => {
            ...
            const localTool = new TaskStopTool(localConfig);
            const result = await localTool.validateBuildAndExecute({ task_id: 'dream-done-1' }, new AbortController().signal);
        ...
        it('returns NOT_CANCELLABLE when the task id resolves to an extract record', async () => {
            ...
            const localTool = new TaskStopTool(localConfig);
            const result = await localTool.validateBuildAndExecute({ task_id: 'extract-running-1' }, new AbortController().signal);
            expect(cancelTask).not.toHaveBeenCalled();
        ...
        it('returns an error when cancelTask returns false (missing AbortController)', async () => {
            ...
            const localTool = new TaskStopTool(localConfig);
            const result = await localTool.validateBuildAndExecute({ task_id: 'dream-broken-1' }, new AbortController().signal);
