...
function getCommandParameterDescription(shellTool) {
    return shellTool.schema.parametersJsonSchema
...
describe('ShellTool', () => {
    ...
    describe('execute', () => {
        ...
        it('runs background commands as managed pool entries (no & / pgrep wrap)', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith('npm start', '/test/dir', expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), { streamStdout: true });
        ...
        it('preserves a trailing && (logical AND would be syntactically broken otherwise)', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith('npm run dev &&', expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), { streamStdout: true });
        });
        it('preserves an escaped trailing \\& (literal &)', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith('echo foo \\&', expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), { streamStdout: true });
        });
        it('preserves quoted trailing ampersands', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith(`printf '&'`, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), { streamStdout: true });
        });
        it('preserves ampersands inside double-quoted script arguments', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith(`node -e "console.log('&')"`, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), { streamStdout: true });
        });
        it('preserves ampersands inside command substitutions', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith(`echo $(printf '&')`, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), { streamStdout: true });
        });
        ...
        it('should not add ampersand when is_background is false', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith('npm test', expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
        });
        it('preserves shell wrapper environment and flags during foreground execution', async () => {
            ...
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith(command, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
        });
        it('preserves shell wrapper environment and flags during background execution', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith(command, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), { streamStdout: true });
        });
        it('should use the provided directory as cwd', async () => {
            ...
            await promise;
            // Foreground commands should not be wrapped with pgrep
            expect(mockShellExecutionService).toHaveBeenCalledWith('ls', '/test/dir/subdir', expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
        });
        it('should not wrap command on windows', async () => {
            ...
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith('dir', '/test/dir', expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
        });
        ...
        describe('Streaming to `updateOutput`', () => {
            ...
            it('should clean up a pending trailing flush if execute() rejects', async () => {
                ...
                await expect(invocation.execute(ac.signal, updateOutputMock)).rejects.toThrow('pty-import-failed');
        ...
        describe('addCoAuthorToGitCommit', () => {
            it('should add co-author to git commit with double quotes', async () => {
                ...
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Co-authored-by: Qwen-Coder <qwen-coder@alibabacloud.com>'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            ...
            it('should not modify non-git commands', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('npm install'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            it('should not modify git commands without -m flag', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('git commit'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            ...
            it('should not add co-author when only pr is enabled (commit off)', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.not.stringContaining('Co-authored-by:'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            it('should not add co-author when disabled in config', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('git commit -m "Initial commit"'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            it('should use custom name and email from config', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Co-authored-by: Custom Bot <custom@example.com>'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            ...
            it('should add co-author for cd subdir && git commit (relative same-repo)', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Co-authored-by:'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            ...
            it('should add co-author when git commit is wrapped in env KEY=VAL', async () => {
                const command = 'env GIT_COMMITTER_DATE=now git commit -m "Test commit"';
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
            ...
            ])('should NOT add co-author for repo-redirecting %s assignment', async (_label, command) => {
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
            ...
            it('should still add co-author with benign GIT_COMMITTER_DATE assignment', async () => {
                const command = 'GIT_COMMITTER_DATE="2026-01-01T00:00:00Z" git commit -m "Test commit"';
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
            ...
            ])('should NOT add co-author for cd %s && git commit (env-var target)', async (_label, command) => {
                const invocation = shellTool.build({
                    command,
                    is_background: false,
                });
            ...
            ])('should NOT add co-author for %s (env-var/empty target)', async (_label, command) => {
                const invocation = shellTool.build({
                    command,
                    is_background: false,
                });
            ...
            it('should add Co-authored-by trailer to the LAST -m when multiple are present', async () => {
                ...
                expect(observed).toMatch(/-m\s+"Body line 1\s+Co-authored-by: Qwen-Coder <qwen-coder@alibabacloud\.com>"/s);
            ...
            it('should not be fooled by a literal -m token inside the quoted message body', async () => {
                const command = 'git commit -m "docs mention -m \'flag\' for completeness"';
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
                ...
                expect(observed).toContain("-m \"docs mention -m 'flag' for completeness");
                // The trailer must land AFTER the original body, just before
                // the message's outer closing quote.
                expect(observed).toMatch(/docs mention -m 'flag' for completeness\s+Co-authored-by:[^"]+"/s);
            });
            ...
            it('should target the commit message, not a later git tag -m in the same chain', async () => {
                const command = 'git commit -m "fix" && git tag -a v1 -m "release notes"';
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
            ...
            it("should append trailer after the final ' in -m 'don'\\''t' apostrophe-escape", async () => {
                ...
                // middle of `'\''`.
                expect(observed).toMatch(/git commit -m 'don'\\''t[\s\S]*Co-authored-by:[^']*'/);
            });
 ...
 spanning multiple lines"`;
        ...
        describe('addAttributionToPR', () => {
            ...
            ])('should leave gh pr create %s unchanged (non-inline-body flow)', async (_label, command) => {
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
            ...
            it('should append attribution to `gh pr new --body "..."` (alias form)', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Generated with Qwen Code'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            ...
            it('should NOT match -b in earlier non-gh segments of a compound', async () => {
                const command = 'curl -b "session=abc" https://example.com && gh pr create --title "x" --body "summary"';
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
                ...
                expect(observed).toMatch(/gh pr create --title "x" --body "summary[\s\S]*Generated with Qwen Code"/);
            });
            ...
            it('should pick the OUTER --body when an inner -b appears in body text', async () => {
                const command = 'gh pr create --title "x" --body "docs mention -b \'flag\' here"';
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
                ...
                expect(cmd).toMatch(/--body "docs mention -b 'flag' here[\s\S]*Generated with Qwen Code"/);
                expect(cmd).not.toMatch(/-b 'flag[\s\S]*Generated with Qwen Code[\s\S]*' here"/);
            });
            ...
            it('should target the LAST --body when gh pr create has multiple', async () => {
                const command = 'gh pr create --title "x" --body "ignored" --body "real summary"';
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
                ...
                const calls = mockShellExecutionService.mock.calls;
                const cmd = calls[calls.length - 1]?.[0];
                expect(cmd).toMatch(/--body "ignored" --body "real summary[\s\S]*Generated with Qwen Code/);
                // The trailer must NOT be inside the first --body.
                expect(cmd).not.toMatch(/--body "ignored[\s\S]*Generated with Qwen Code[\s\S]*" --body/);
            });
            ...
            it('should append attribution when gh has global flags before pr create', async () => {
                const command = 'gh --repo owner/repo pr create --title "x" --body "Summary"';
                const invocation = shellTool.build({ command, is_background: false });
                const promise = invocation.execute(mockAbortSignal);
                resolveExecutionPromise({
                    rawOutput: Buffer.from(''),
            ...
            it('should NOT rewrite when gh pr create appears only inside quoted text', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.not.stringContaining('Generated with Qwen Code'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
            });
            ...
            it("should match the full body across '\\\\'' apostrophe escapes", async () => {
                ...
                // middle of it.
                expect(observed).toMatch(/don'\\''t break me[\s\S]*Generated with Qwen Code/);
            });
        });
        describe('foreground → background promote (#3831 PR-2)', () => {
            it("exposes a promote AbortController whose signal is wired into ShellExecutionService.execute's combined signal", async () => {
                ...
                const promise = invocation.execute(mockAbortSignal, undefined, {}, undefined, setPromoteAc);
                resolveShellExecution({ pid: 12345 });
                await promise;
                expect(setPromoteAc).toHaveBeenCalledTimes(1);
                const passedAc = setPromoteAc.mock.calls[0][0];
            ...
            it('registers a bg_xxx entry on `result.promoted: true` and returns promote-flavored ToolResult', async () => {
                ...
                const streamMock = fs.createWriteStream.mock.results[0]
                    ?.value;
                expect(streamMock.write).toHaveBeenCalledWith('partial output before promote');
                // Model-facing copy points at /tasks / dialog / task_stop.
                expect(result.llmContent).toContain(`promoted to background as ${entry.shellId}`);
                expect(result.llmContent).toContain(`PID: 99999`);
                expect(result.llmContent).toContain('/tasks');
                expect(result.llmContent).toContain(`task_stop({ task_id: '${entry.shellId}'`);
                expect(result.returnDisplay).toContain(`Promoted to background: ${entry.shellId}`);
            ...
            it('aborting entry.abortController kills the child via SIGTERM/SIGKILL and marks the registry entry cancelled', async () => {
                ...
                try {
                    ...
                    expect(registry.cancel).toHaveBeenCalledWith(entry.shellId, expect.any(Number));
                }
                finally {
            ...
            it('promote-refused race (aborted: true, promoted: false after promote signal) is reported as benign race, not "Command timed out"', async () => {
                ...
                await Promise.resolve();
                const promoteAc = setPromoteAc.mock.calls[0]?.[0];
                ...
                expect(String(result.llmContent)).toContain('Command finished before the background-promote');
        ...
        describe('foreground → background promote PR-2.5 (post-promote stream + natural-exit settle)', () => {
            it('post-promote bytes APPEND to bg_xxx.output via write stream (do NOT overwrite snapshot)', async () => {
                ...
                vi.mocked(fs.createWriteStream).mockReturnValueOnce(writeStreamMock);
                const registry = mockConfig.getBackgroundShellRegistry();
                const invocation = shellTool.build({
                    command: 'tail -f /tmp/never.log',
                    is_background: false,
                ...
                expect(writeStreamMock.write).toHaveBeenNthCalledWith(1, 'initial-snapshot');
            });
            it('natural child exit transitions the registry entry to "completed" (exitCode 0)', async () => {
                ...
                const entry = registry.register.mock.calls[0][0];
                expect(registry.complete).toHaveBeenCalledWith(entry.shellId, 0, 1700000000000);
            ...
            it('non-zero exit / signal / error all transition entry to "failed" with descriptive message', async () => {
                ...
                await promise;
                const serviceCall = mockShellExecutionService.mock.calls[0];
                const onSettle = serviceCall[6].postPromote.onSettle;
                const entry = registry.register.mock.calls[0][0];
                // Non-zero exitCode → fail with "Exited with code N".
                onSettle({ exitCode: 137, signal: null, endTime: 1 });
                expect(registry.fail).toHaveBeenCalledWith(entry.shellId, 'Exited with code 137', 1);
                // signal-killed (no exitCode) → fail with "Terminated by signal N".
                onSettle({ exitCode: null, signal: 15, endTime: 2 });
                expect(registry.fail).toHaveBeenCalledWith(entry.shellId, 'Terminated by signal 15', 2);
            ...
            it('queued-settle race: onSettle fires BEFORE handlePromotedForeground completes — entry settles + llmContent reflects final status', async () => {
                ...
                mockShellExecutionService.mockImplementationOnce((...args) => {
                    const opts = args[6];
                ...
                expect(registry.complete).toHaveBeenCalledWith(entry.shellId, 0, 1700000000123);
            ...
            it('queued-settle race with non-zero exit code: llmContent reflects failed status', async () => {
                ...
                const result = await invocation.execute(mockAbortSignal);
                const entry = registry.register.mock.calls[0][0];
                expect(registry.fail).toHaveBeenCalledWith(entry.shellId, 'Exited with code 1', 1700000000456);
            ...
            it("wave-2 (C3): llmContent reflects 'completed' even when stream.once('finish') fires asynchronously after the queued-settle drain", async () => {
                ...
                let capturedFinishHandler = null;
                const writeStreamMock = {
                    write: vi.fn(),
                    end: vi.fn(),
                    on: vi.fn(),
                ...
                expect(registry.complete).toHaveBeenCalledWith(entry.shellId, 0, 1700000000999);
            });
            it('wave-2 (C1): stream open async error transitions registry — does not hang waiting on `finish`', async () => {
                ...
                errorListeners[0](Object.assign(new Error('disk full'), { code: 'ENOSPC' }));
                ...
                const entry = registry.register.mock.calls[0][0];
                expect(registry.complete).toHaveBeenCalledWith(entry.shellId, 0, 1700000111111);
            });
            it('stream open async error writes diagnostic marker via appendFileSync', async () => {
                ...
                await promise;
                errorListeners[0](Object.assign(new Error('disk full'), { code: 'ENOSPC' }));
                expect(fs.appendFileSync).toHaveBeenCalledWith(expect.stringContaining('bg_'), expect.stringContaining('[WARNING: post-promote output lost'));
            });
            it('flush timeout transitions registry when stream.finish never fires', async () => {
                ...
                try {
                    ...
                    const entry = registry.register.mock.calls[0][0];
                    expect(registry.complete).toHaveBeenCalledWith(entry.shellId, 0, 1700000222222);
                }
                finally {
                    vi.useRealTimers();
                }
            ...
            it('wave-3 (T2): onSettleWired drains pre-settle buffer AND latches streamClosed so post-end chunks drop instead of leaking the buffer', async () => {
                ...
                // third branch drops).
                const writeCalls = writeStreamMock.write.mock.calls.map((c) => c[0]);
            ...
            it('wave-3 (T3): catch-path clears the buffered chunks and falls back to writeFileSync(snapshot)', async () => {
                ...
                // fallback).
                expect(writeFileSyncSpy).toHaveBeenCalledWith(expect.any(String), 'snap');
                ...
                const entry = registry.register.mock.calls[0][0];
                expect(registry.complete).toHaveBeenCalledWith(entry.shellId, 0, 1700002222222);
            });
            it('wave-4 (T4): post-promote `onData` chunks have ANSI stripped before write (matches executeBackground file format)', async () => {
                ...
                const ansiChunk = '\x1b[31mFAILED\x1b[0m: 3 tests\n\x1b[2K\x1b[1Aprogress: 50%';
                ...
                const writeCalls = writeStreamMock.write.mock.calls.map((c) => c[0]);
                const post = writeCalls.find((c) => typeof c === 'string' && c.includes('FAILED'));
    ...
    describe('getDescription', () => {
        ...
        it('should return the windows description when on windows', async () => {
            ...
            expect(shellTool.description).toContain("Use '&' only when you need to run commands sequentially");
            expect(shellTool.description).toContain("DO NOT use ';' or newlines to separate commands in cmd.exe.");
            expect(getCommandParameterDescription(shellTool)).toBe('Exact cmd.exe command to execute as `cmd.exe /d /s /c <command>`');
        });
        it('should return the non-windows description when not on windows', async () => {
            vi.mocked(os.platform).mockReturnValue('linux');
            const shellTool = new ShellTool(mockConfig);
            expect(shellTool.description).toMatchSnapshot();
            expect(getCommandParameterDescription(shellTool)).toBe('Exact bash command to execute as `bash -c <command>`');
        });
        it('should describe PowerShell when ComSpec points to powershell.exe', async () => {
            ...
            delete process.env['TERM'];
            const shellTool = new ShellTool(mockConfig);
            expect(shellTool.description).toContain('`powershell.exe -NoProfile -Command <command>`');
            expect(shellTool.description).toContain('The active shell is PowerShell.');
            expect(shellTool.description).toContain('Do NOT use Bash-only forms such as ANSI-C quoting');
            expect(shellTool.description).toContain("Windows PowerShell does not support '&&'.");
            expect(shellTool.description).not.toContain("use a single run_shell_command call with '&&'");
            expect(getCommandParameterDescription(shellTool)).toBe('Exact PowerShell command to execute as `powershell.exe -NoProfile -Command <command>`');
        });
        it('should describe pwsh when ComSpec points to pwsh.exe', async () => {
            ...
            delete process.env['TERM'];
            const shellTool = new ShellTool(mockConfig);
            expect(shellTool.description).toContain('`pwsh.exe -NoProfile -Command <command>`');
            expect(shellTool.description).toContain("use a single run_shell_command call with '&&'");
            expect(getCommandParameterDescription(shellTool)).toBe('Exact PowerShell command to execute as `pwsh.exe -NoProfile -Command <command>`');
        });
        it('should describe bash when Windows is running in Git Bash', async () => {
            ...
            expect(shellTool.description).not.toContain('Command process group can be terminated');
            expect(getCommandParameterDescription(shellTool)).toBe('Exact bash command to execute as `bash -c <command>`');
        });
    });
    describe('timeout parameter', () => {
        ...
        it('should create combined signal with timeout for foreground execution', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(expect.any(String), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({}), expect.objectContaining({ postPromote: expect.any(Object) }));
