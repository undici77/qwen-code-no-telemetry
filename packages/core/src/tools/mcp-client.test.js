...
import { addMCPStatusChangeListener, createTransport, getAllMCPServerStatuses, getMCPServerStatus, hasNetworkTransport, isEnabled, MCPServerStatus, McpClient, populateMcpServerCommand, removeMCPServerStatus, removeMCPStatusChangeListener, updateMCPServerStatus, } from './mcp-client.js';
...
describe('mcp-client', () => {
    ...
    describe('removeMCPServerStatus', () => {
        ...
        it('a stale status update from an in-flight connect cannot resurrect a removed server', async () => {
            ...
            const client = new McpClient('racy-server', { command: 'test-command' }, {}, {}, {}, false);
        ...
        it('disconnect() propagates DISCONNECTED to the global registry', async () => {
            ...
            vi.spyOn(SdkClientStdioLib, 'StdioClientTransport').mockReturnValue(mockedTransport);
            const client = new McpClient('healthy-server', { command: 'test-command' }, {}, {}, { getDirectories: () => [] }, false);
            await client.connect();
            // After connect, the registry should show CONNECTED.
            expect(getMCPServerStatus('healthy-server')).toBe(MCPServerStatus.CONNECTED);
            await client.disconnect();
            ...
            expect(getMCPServerStatus('healthy-server')).toBe(MCPServerStatus.DISCONNECTED);
