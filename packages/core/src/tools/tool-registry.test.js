...
import { getAllMCPServerStatuses, MCPServerStatus, removeMCPServerStatus, updateMCPServerStatus, } from './mcp-client.js';
...
describe('ToolRegistry', () => {
    ...
    describe('registerTool', () => {
        ...
        it('renames an MCP tool whose name shadows a registered lazy factory', async () => {
            ...
            toolRegistry.registerFactory('structured_output', async () => new MockTool({ name: 'structured_output' }));
            const mockCallable = {};
            const collidingMcp = new DiscoveredMCPTool(mockCallable, 'rogue-server', 'structured_output', 'description', {});
            ...
            const renamed = toolRegistry.getTool('mcp__rogue-server__structured_output');
        ...
        it('skips tools whose name is in Config.disabledTools (#4175 Wave 4 PR 17)', () => {
            ...
            registry.registerTool(new MockTool({ name: 'mcp__github__create_issue' }));
        ...
        it('skips lazy factories whose name is in Config.disabledTools', async () => {
            ...
            const registry = new ToolRegistry(disabledConfig);
            registry.registerFactory('structured_output', async () => new MockTool({ name: 'structured_output' }));
            registry.registerFactory('sequential_thinking', async () => new MockTool({ name: 'sequential_thinking' }));
        ...
        it('honors disabledTools against the renamed name when an MCP tool collides with a lazy factory (#4282 fold-in 2 CV3)', async () => {
            ...
            const registry = new ToolRegistry(disabledConfig);
            registry.registerFactory('structured_output', async () => new MockTool({ name: 'structured_output' }));
            const collidingMcp = new DiscoveredMCPTool({}, 'rogue-server', 'structured_output', 'description', {});
            registry.registerTool(collidingMcp);
            // The renamed MCP tool must NOT have been inserted.
            expect(registry.getTool('mcp__rogue-server__structured_output')).toBeUndefined();
    ...
    describe('deferred tool filtering', () => {
        it('excludes shouldDefer tools from getFunctionDeclarations by default', () => {
            ...
            toolRegistry.registerTool(new MockTool({ name: 'hidden', shouldDefer: true }));
        ...
        it('includes revealed deferred tools in getFunctionDeclarations', () => {
            toolRegistry.registerTool(new MockTool({ name: 'hidden', shouldDefer: true }));
            toolRegistry.registerTool(new MockTool({ name: 'other-hidden', shouldDefer: true }));
        ...
        it('removeMcpToolsByServer also drops revealedDeferred entries', async () => {
            ...
            const mcpCallable = {};
            const tool = new DiscoveredMCPTool(mcpCallable, 'slack', 'send_message', 'send a message', {});
            toolRegistry.registerTool(tool);
    ...
    describe('disableMcpServer', () => {
        ...
        it('still removes the registry entry and updates the exclusion list when disconnect throws', async () => {
            ...
            const setExcludedSpy = vi
                ...
                .mockImplementation(() => { });
            vi.spyOn(McpClientManager.prototype, 'disconnectServer').mockRejectedValue(new Error('boom'));
            await expect(toolRegistry.disableMcpServer('flaky-server')).rejects.toThrow('boom');
        ...
        it('still removes the registry entry when the exclusion-list update throws', async () => {
            ...
            vi.spyOn(McpClientManager.prototype, 'disconnectServer').mockResolvedValue(undefined);
            await expect(toolRegistry.disableMcpServer('flaky-server')).rejects.toThrow('config write failed');
            expect(getAllMCPServerStatuses().has('flaky-server')).toBe(false);
        });
        ...
        it('updates the exclusion list before dropping the status entry', async () => {
            ...
            const callOrder = [];
            vi.spyOn(config, 'setExcludedMcpServers').mockImplementation(() => {
                callOrder.push(`setExcludedMcpServers:hasStatus=${getAllMCPServerStatuses().has('flaky-server')}`);
            });
            await toolRegistry.disableMcpServer('flaky-server');
