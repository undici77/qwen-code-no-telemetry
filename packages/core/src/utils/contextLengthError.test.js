...
import { getContextLengthExceededInfo, isContextLengthExceededError, } from './contextLengthError.js';
describe('contextLengthError', () => {
    ...
    it('parses prompt-too-long actual and limit token counts', () => {
        const info = getContextLengthExceededInfo(new Error('prompt is too long: 137500 tokens > 135000 maximum'));
    ...
    it('parses OpenAI-style maximum context length token counts', () => {
        const info = getContextLengthExceededInfo(new Error("This model's maximum context length is 128000 tokens. However, your messages resulted in 135000 tokens."));
        expect(info.isExceeded).toBe(true);
    ...
    it('parses maximum context length limits without actual token counts', () => {
        const info = getContextLengthExceededInfo(new Error("This model's maximum context length is 128000 tokens."));
        expect(info.isExceeded).toBe(true);
        expect(info.actualTokens).toBeUndefined();
        expect(info.limitTokens).toBe(128000);
    });
    it('extracts nested JSON error messages from strings', () => {
        const info = getContextLengthExceededInfo(new Error('HTTP 400 {"error":{"code":"context_length_exceeded","message":"prompt is too long: 137500 tokens > 135000 maximum"}}'));
        expect(info.isExceeded).toBe(true);
        expect(info.actualTokens).toBe(137500);
        expect(info.limitTokens).toBe(135000);
    });
...
//# sourceMappingURL=contextLengthError.test.js.map