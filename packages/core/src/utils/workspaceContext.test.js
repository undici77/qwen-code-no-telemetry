...
describe('WorkspaceContext removeDirectory', () => {
    ...
    let anotherDir;
    beforeEach(() => {
        tempDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'workspace-context-remove-')));
        cwd = path.join(tempDir, 'project');
...
describe('WorkspaceContext isInitialDirectory', () => {
    ...
    let runtimeDir;
    beforeEach(() => {
        tempDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'workspace-context-initial-')));
        cwd = path.join(tempDir, 'project');
