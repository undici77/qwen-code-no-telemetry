...
import { partToString, getResponseText, flatMapTextParts, appendToLastTextPart, prependToFirstTextPart, } from './partUtils.js';
...
describe('partUtils', () => {
    ...
    describe('prependToFirstTextPart', () => {
        ...
        it('should prepend to a prompt with a text part object', () => {
            expect(prependToFirstTextPart([{ text: 'first part' }], 'new text')).toEqual([{ text: 'new text\n\nfirst part' }]);
        });
        ...
        it('should use a custom separator', () => {
            expect(prependToFirstTextPart(['first part'], 'new text', '---')).toEqual(['new text---first part']);
        });
    });
});
