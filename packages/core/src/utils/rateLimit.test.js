...
import { getRateLimitErrorDetails, getRateLimitRetryDelayMs, isRateLimitError, } from './rateLimit.js';
...
describe('rate-limit retry diagnostics', () => {
    ...
    it('should extract nested JSON provider details from error messages', () => {
        const error = new Error('{"error":{"code":"429","message":"Throttling: TPM limit reached"}}');
        expect(getRateLimitErrorDetails(error)).toEqual({
            providerCode: '429',
    ...
    it('should extract request id from top-level nested JSON error messages', () => {
        const error = new Error('{"request_id":"req-123","error":{"code":"429","message":"Throttling: TPM limit reached"}}');
        expect(getRateLimitErrorDetails(error)).toEqual({
            providerCode: '429',
            providerMessage: 'Throttling: TPM limit reached',
            requestId: 'req-123',
    ...
    it('should increase retry delay by attempt and cap at the maximum', () => {
        expect(getRateLimitRetryDelayMs(0, {
            initialDelayMs: 60_000,
            maxDelayMs: 300_000,
        })).toBe(60_000);
        expect(getRateLimitRetryDelayMs(1, {
            initialDelayMs: 60_000,
            maxDelayMs: 300_000,
        })).toBe(60_000);
        expect(getRateLimitRetryDelayMs(2, {
            initialDelayMs: 60_000,
            maxDelayMs: 300_000,
        })).toBe(120_000);
        expect(getRateLimitRetryDelayMs(10, {
            initialDelayMs: 60_000,
            maxDelayMs: 300_000,
        })).toBe(300_000);
    });
    ...
    it('should read Retry-After from Headers-like objects', () => {
        const error = Object.assign(new Error('Too many requests'), {
            status: 429,
            headers: {
                get: (name) => (name === 'retry-after' ? '180' : null),
            },
        });
        expect(getRateLimitRetryDelayMs(1, {
            initialDelayMs: 60_000,
