...
import { _recoverObjectsFromLine, _resetEnsuredDirsCacheForTest, countLines, parseLineTolerant, read, readLines, } from './jsonl-utils.js';
let tmpRoot;
beforeAll(() => {
...
async function waitForStreamClosed(getStream) {
...
async function withCapturedReadStream(operation) {
    let capturedStream;
...
describe('parseLineTolerant', () => {
    it('returns the parsed object for a well-formed line', () => {
        expect(parseLineTolerant('{"a":1}', '/tmp/x.jsonl')).toEqual([{ a: 1 }]);
    });
    it('recovers both records from a `}{`-glued line', () => {
        expect(parseLineTolerant('{"uuid":"a"}{"uuid":"b"}', '/tmp/x.jsonl')).toEqual([{ uuid: 'a' }, { uuid: 'b' }]);
    });
...
describe('read() / readLines() with malformed lines', () => {
    ...
    it('drops scalar / array lines so callers do not see non-object records', async () => {
        ...
        const file = tmpFile('{"a":1}\nnull\n42\n"a string"\n[1,2,3]\n{"a":2}\n');
        expect(await read(file)).toEqual([{ a: 1 }, { a: 2 }]);
        expect(await readLines(file, 10)).toEqual([
            { a: 1 },
            { a: 2 },
        ]);
    });
...
describe('reader resource cleanup', () => {
    it('closes the file stream after readLines stops at the requested limit', async () => {
        const file = tmpFile('{"i":1}\n{"i":2}\n{"i":3}\n');
        const result = await withCapturedReadStream(() => readLines(file, 1));
        expect(result).toEqual([{ i: 1 }]);
    });
    it('closes the file stream after read consumes all lines', async () => {
        const file = tmpFile('{"i":1}\n{"i":2}\n');
        const result = await withCapturedReadStream(() => read(file));
