...
import { buildRuntimeFetchOptions, extractHostnameFromProxyUrl, getOrCreateSharedDispatcher, redactProxyCredentials, redactProxyError, resetDispatcherCache, } from './runtimeFetchOptions.js';
describe('buildRuntimeFetchOptions (node runtime)', () => {
    ...
    it('pins fetch to undici when proxy is set so dispatcher and fetch share a version', () => {
        ...
        // dispatcher.
        const openaiResult = buildRuntimeFetchOptions('openai', 'http://proxy.local');
        expect(openaiResult.fetch).toBe(mockUndiciFetch);
        const anthropicResult = buildRuntimeFetchOptions('anthropic', 'http://proxy.local');
        expect(anthropicResult.fetch).toBe(mockUndiciFetch);
    });
    it('does not inject a fetch override when no proxy is set', () => {
        ...
        expect(anthropicResult.fetch).toBeUndefined();
    });
    it('returns undefined for OpenAI when dispatcher creation fails', () => {
        ...
        expect(mockWarn).toHaveBeenCalledWith(expect.stringContaining('Failed to create proxy dispatcher'));
        // Should also log to console.error for production visibility
        expect(mockConsoleError).toHaveBeenCalledOnce();
        expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('[RUNTIME_FETCH]'));
    });
    it('returns empty object for Anthropic when dispatcher creation fails', () => {
        const result = buildRuntimeFetchOptions('anthropic', 'http://invalid-proxy');
    ...
    it('redacts credentials from proxy URL in error message', () => {
        ...
        expect(mockWarn).toHaveBeenCalledWith(expect.stringContaining('<redacted>'));
        expect(mockWarn).not.toHaveBeenCalledWith(expect.stringContaining('secret'));
        expect(mockConsoleError).toHaveBeenCalledWith(expect.stringContaining('<redacted>'));
        expect(mockConsoleError).not.toHaveBeenCalledWith(expect.stringContaining('secret'));
    });
    it('logs hostname (without credentials) in failure message', () => {
        buildRuntimeFetchOptions('openai', 'http://invalid-proxy');
        expect(mockWarn).toHaveBeenCalledWith(expect.stringContaining('invalid-proxy'));
        expect(mockWarn).not.toHaveBeenCalledWith(expect.stringContaining('secret'));
    });
    it('logs each failure separately (no deduplication)', () => {
        ...
        expect(mockWarn).toHaveBeenNthCalledWith(1, expect.stringContaining('(first failure)'));
        expect(mockWarn).toHaveBeenNthCalledWith(2, expect.stringContaining('(failure #2)'));
        expect(mockWarn).toHaveBeenNthCalledWith(3, expect.stringContaining('(failure #3)'));
    });
});
...
describe('redactProxyCredentials', () => {
    it('redacts credentials from a single proxy URL', () => {
        ...
        expect(redactProxyCredentials(msg)).toBe('Failed to connect: http://<redacted>@proxy.local');
    });
    it('redacts every credential occurrence in a multi-URL error message', () => {
        const msg = 'Failed: http://a:b@p1; cause: http://c:d@p2';
        expect(redactProxyCredentials(msg)).toBe('Failed: http://<redacted>@p1; cause: http://<redacted>@p2');
    });
    it('does not over-redact non-userinfo @ characters past the hostname', () => {
        ...
        expect(redactProxyCredentials(msg)).toBe('http://<redacted>@proxy.local/contact@example.com');
    });
    ...
    it('redacts credentials in Node.js native error format (no scheme)', () => {
        ...
        expect(redactProxyCredentials(msg)).toBe('connect ECONNREFUSED <redacted>@proxy.local:8080');
    });
    ...
    it('redacts token-only credentials for localhost proxy endpoints', () => {
        ...
        expect(redactProxyCredentials(msg)).toBe('connect ECONNREFUSED <redacted>@localhost:8080');
    });
    it('redacts token-only credentials for IP proxy endpoints', () => {
        const msg = 'connect ECONNREFUSED token@10.0.0.5:8080';
        expect(redactProxyCredentials(msg)).toBe('connect ECONNREFUSED <redacted>@10.0.0.5:8080');
    });
    it('redacts token-only credentials for corporate proxy endpoints', () => {
        ...
        expect(redactProxyCredentials(msg)).toBe('connect ECONNREFUSED <redacted>@gateway.corp.local:8080');
    });
    ...
    it('redacts token-only credentials for public hosts in network error context', () => {
        ...
        expect(redactProxyCredentials(msg)).toBe('connect ECONNREFUSED <redacted>@public.example.com:8080');
    });
    ...
    it('preserves labels and delimiters around bare proxy credentials', () => {
        ...
        expect(redactProxyCredentials(msg)).toBe('cause=(<redacted>@proxy.local:8080)');
    });
    it('does not double-redact when both patterns are present', () => {
        const msg = 'http://user:pass@proxy.local — cause: connect ECONNREFUSED user:pass@proxy.local:8080';
...
describe('redactProxyError', () => {
    it('redacts proxy credentials from Error message and stack in-place', () => {
        ...
        expect(error.message).toBe('connect ECONNREFUSED <redacted>@proxy.local:8080');
    ...
    it('redacts proxy credentials from string errors', () => {
        expect(redactProxyError('407 via http://user:pass@proxy.local')).toBe('407 via http://<redacted>@proxy.local');
    });
    it('preserves SDK error metadata while redacting nested causes', () => {
        ...
        const error = Object.assign(new Error('request failed via http://user:pass@proxy.local'), { status: 407, code: 'proxy_auth_required', cause });
        const result = redactProxyError(error);
        expect(result).toBe(error);
        expect(error.status).toBe(407);
        expect(error.code).toBe('proxy_auth_required');
        expect(error.message).toBe('request failed via http://<redacted>@proxy.local');
        expect(cause.message).toBe('connect ECONNREFUSED <redacted>@localhost:8080');
    });
    ...
    it('returns a redacted clone when an error-like object has read-only fields', () => {
        ...
        expect(result.message).toBe('connect ECONNREFUSED <redacted>@proxy.local:8080');
    ...
    it('preserves Error subclass prototype when cloning read-only fields', () => {
        ...
        const error = new ProxySdkError('connect ECONNREFUSED token@proxy.local:8080');
        Object.defineProperty(error, 'message', {
            value: error.message,
            writable: false,
            configurable: false,
    ...
    it('redacts nested AggregateError errors', () => {
        const nestedError = new Error('connect ECONNREFUSED token@proxy.local:8080');
        const aggregateError = new AggregateError([nestedError], 'fetch failed via http://user:pass@proxy.local');
        const result = redactProxyError(aggregateError);
        const [redactedNestedError] = result.errors;
        expect(result).toBe(aggregateError);
        expect(result.message).toBe('fetch failed via http://<redacted>@proxy.local');
        expect(redactedNestedError).toBe(nestedError);
        expect(redactedNestedError.message).toBe('connect ECONNREFUSED <redacted>@proxy.local:8080');
    });
});
describe('extractHostnameFromProxyUrl', () => {
    it('extracts host and port from a valid credentialed proxy URL', () => {
        expect(extractHostnameFromProxyUrl('http://user:secret@proxy.local:8080')).toBe('proxy.local:8080');
    });
    it('extracts host and port from a scheme-less credentialed proxy value', () => {
        expect(extractHostnameFromProxyUrl('user:secret@proxy.local:8080')).toBe('proxy.local:8080');
    });
    it('redacts fallback output when no safe hostname can be extracted', () => {
        expect(extractHostnameFromProxyUrl('http://user:secret@')).toBe('http://<redacted>@');
    });
});
