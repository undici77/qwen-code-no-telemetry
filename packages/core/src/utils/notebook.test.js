...
import { findCellIndex, getCellDisplayId, hasStableCellIds, inferInsertedCellSourceArrayStyle, inferNotebookJsonFormat, isAmbiguousCellId, makeCellId, parseCellId, parseNotebook, readNotebook, readNotebookWithMetadata, serializeNotebook, toNotebookSource, } from './notebook.js';
...
describe('notebook utilities', () => {
    ...
    it('should parse notebooks with a leading UTF-8 BOM', async () => {
        ...
        expect(parseNotebook(`\ufeff${JSON.stringify(notebook)}`).cells).toHaveLength(1);
    ...
    it('should reject non-object notebook cells', () => {
        expect(() => parseNotebook(JSON.stringify({ cells: [null], metadata: {} }))).toThrow('cell at index 0 is not an object');
        expect(() => parseNotebook(JSON.stringify({ cells: ['not a cell'], metadata: {} }))).toThrow('cell at index 0 is not an object');
    });
});
