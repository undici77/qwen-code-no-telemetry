 ...
 */
const canMergeAnsiTokens = (left, right) => left.bold === right.bold &&
    ...
    left.fg === right.fg &&
    left.bg === right.bg;
const appendAnsiLineTokens = (target, source) => {
...
export function serializeTerminalToText(terminal) {
...
export function serializeTerminalToObject(terminal, scrollOffset = 0, options = {}) {
    const buffer = terminal.buffer.active;
    const defaultFg = '';
    const defaultBg = '';
