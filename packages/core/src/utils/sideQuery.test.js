...
describe('runSideQuery', () => {
    ...
    describe('JSON mode (schema present)', () => {
        ...
        it('prefers fastModel when available', async () => {
            ...
            expect(mockBaseLlmClient.generateJson).toHaveBeenCalledWith(expect.objectContaining({ model: 'fast-model' }));
        });
        it('explicit model override beats fastModel', async () => {
            ...
            expect(mockBaseLlmClient.generateJson).toHaveBeenCalledWith(expect.objectContaining({ model: 'override-model' }));
        });
        ...
        it('preserves caller-supplied promptId when provided', async () => {
            ...
            expect(mockBaseLlmClient.generateJson).toHaveBeenCalledWith(expect.objectContaining({ promptId: 'legacy-id' }));
        });
        ...
        it('forwards maxAttempts when provided, omits it otherwise', async () => {
            ...
            expect(mockBaseLlmClient.generateJson).toHaveBeenLastCalledWith(expect.objectContaining({ maxAttempts: 1 }));
            await runSideQuery(mockConfig, baseOptions);
            const lastCall = vi
                .mocked(mockBaseLlmClient.generateJson)
                .mock.calls.at(-1)?.[0];
            expect(lastCall).toBeDefined();
            expect(lastCall).not.toHaveProperty('maxAttempts');
        });
    ...
    describe('text mode (no schema)', () => {
        function mockTextResult(text, withUsage = true) {
        ...
        it('routes through BaseLlmClient.generateText and returns { text, usage }', async () => {
            ...
            expect(mockBaseLlmClient.generateText).toHaveBeenCalledWith(expect.objectContaining({
                contents: [{ role: 'user', parts: [{ text: 'recap please' }] }],
                model: 'main-model',
                systemInstruction: 'You are a recap generator.',
                abortSignal: abortController.signal,
        ...
        it('prefers fastModel when available', async () => {
            ...
            expect(mockBaseLlmClient.generateText).toHaveBeenCalledWith(expect.objectContaining({ model: 'fast-model' }));
        });
        ...
        it('passes validate the text and throws on failure', async () => {
            ...
            const validate = vi.fn((text) => text.length < 20 ? 'too short' : null);
            await expect(runSideQuery(mockConfig, {
                purpose: 'p',
                contents: [{ role: 'user', parts: [{ text: 'q' }] }],
                abortSignal: abortController.signal,
        ...
        it('forwards maxAttempts when provided, omits it otherwise', async () => {
            ...
            expect(mockBaseLlmClient.generateText).toHaveBeenLastCalledWith(expect.objectContaining({ maxAttempts: 1 }));
            await runSideQuery(mockConfig, baseOptions);
            const lastCall = vi
