...
describe('collectMemoryDiagnostics', () => {
    ...
    it('captures memory, V8, resource, handle, fd, smaps, and risk data', async () => {
        ...
        expect(diagnostics.analysis.risks).toEqual(expect.arrayContaining([
        ...
        expect(diagnostics.analysis.recommendation).toBe('5 potential leak indicator(s) found.');
    ...
    it('does not flag native pressure when malloced memory is below the absolute floor', async () => {
        ...
        expect(diagnostics.analysis.risks).not.toEqual(expect.arrayContaining([
            expect.objectContaining({ type: 'native-memory-pressure' }),
        ]));
    });
    ...
    it('treats unsupported optional probes as unavailable instead of failing', async () => {
        ...
        expect(diagnostics.analysis.recommendation).toBe('No obvious leak indicators detected.');
        expect(diagnostics.analysis.recommendation).not.toContain('heap snapshot');
        expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('heapSpaceStatistics'), expect.any(Error));
        expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('openFileDescriptors'), expect.any(Error));
        expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('smapsRollup'), expect.any(Error));
    });
    ...
    it('logs unavailable Node.js internal active probes before returning zero counts', async () => {
        ...
        try {
            ...
            expect(diagnostics.activeHandles).toBe(0);
            expect(diagnostics.activeRequests).toBe(0);
            expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('activeHandles'), expect.any(Error));
            expect(debugLogger.debug).toHaveBeenCalledWith(expect.stringContaining('activeRequests'), expect.any(Error));
        }
        finally {
    ...
    it('starts independent optional probes before awaiting slow probes', async () => {
        let resolveFileDescriptors;
        const fileDescriptors = new Promise((resolve) => {
