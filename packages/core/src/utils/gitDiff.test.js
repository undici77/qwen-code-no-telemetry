...
import { fetchGitDiff, fetchGitDiffHunks, MAX_DIFF_SIZE_BYTES, MAX_FILES, MAX_LINES_PER_FILE, parseDeletedFromNameStatus, parseGitDiff, parseGitNumstat, parseShortstat, resolveGitDir, } from './gitDiff.js';
...
describe('parseDeletedFromNameStatus', () => {
    it('extracts D-status paths and ignores M/A entries', () => {
        ...
        expect(parseDeletedFromNameStatus(out)).toEqual(new Set(['gone.txt', 'also-gone.txt']));
    });
    it('skips both halves of rename and copy entries', () => {
        ...
        // the new name.
        const out = 'R100\0old.txt\0new.txt\0' + 'C75\0src.txt\0copy.txt\0' + 'D\0gone.txt\0';
    ...
    it('preserves NUL-safe paths (tabs, non-ASCII)', () => {
        ...
        expect(parseDeletedFromNameStatus(out)).toEqual(new Set(['tab\there.txt', '日本語.txt']));
    });
...
describe('parseShortstat', () => {
    it('parses the full form', () => {
        expect(parseShortstat(' 3 files changed, 42 insertions(+), 7 deletions(-)')).toEqual({ filesCount: 3, linesAdded: 42, linesRemoved: 7 });
    });
...
`;
        const body = Array.from({ length: MAX_LINES_PER_FILE + 50 }, (_, i) => ` line${i}`).join('\n');
...
describe('fetchGitDiff', () => {
    ...
    it('flags untracked binary files without counting lines', async () => {
        ...
        await fs.writeFile(path.join(repo, 'blob.bin'), Buffer.from([0x89, 0x00, 0xff, 0x10]));
        const result = await fetchGitDiff(repo);
        expect(result).not.toBeNull();
    ...
    it('returns null during a transient merge state', async () => {
        ...
        await fs.writeFile(path.join(repo, '.git', 'MERGE_HEAD'), '0000000000000000000000000000000000000000\n');
...
describe('fetchGitDiffHunks', () => {
    ...
    it('returns hunks for modified tracked files', async () => {
        ...
        expect(fileHunks[0].lines.some((l) => l.startsWith('-two'))).toBe(true);
        expect(fileHunks[0].lines.some((l) => l.startsWith('+TWO'))).toBe(true);
    });
    it('preserves content lines that start with --- / +++ / index', async () => {
        ...
        expect(fileHunks).toBeDefined();
        const removed = fileHunks.flatMap((h) => h.lines.filter((l) => l.startsWith('-')));
        expect(removed).toEqual(expect.arrayContaining(['----a/foo', '-+++b/bar', '-index deadbeef']));
    });
    it('keys hunks by the real path for files with tabs in the name (C-quoted in diff output)', async () => {
        ...
        expect(hunks.get(weirdName)[0].lines.some((l) => l.startsWith('-x'))).toBe(true);
        expect(hunks.get(weirdName)[0].lines.some((l) => l.startsWith('+y'))).toBe(true);
    });
...
describe('resolveGitDir', () => {
    ...
    it('follows the gitdir pointer for linked worktrees', async () => {
        ...
        try {
            ...
            await execFileAsync('git', ['worktree', 'add', '-q', wtPath, '-b', 'side'], { cwd: main });
            ...
            await fs.writeFile(path.join(resolved, 'MERGE_HEAD'), '0000000000000000000000000000000000000000\n');
...
describe('fetchGitDiff transient-state detection', () => {
    ...
    it.each([
        ...
        ['rebase-merge', 'dir'],
        ['rebase-apply', 'dir'],
    ])('short-circuits when %s is present (%s)', async (name, kind) => {
...
describe('parseShortstat ReDoS guard', () => {
    it('runs in bounded time on pathological input', () => {
        ...
        const adversarial = `${'0'.repeat(100_000)} files changed, ${'0'.repeat(100_000)} insertions(+)`;
...
describe('fetchGitDiffHunks ignores external diff drivers', () => {
        ...
        try {
            ...
            try {
                ...
                if (prev === undefined)
                    ...
                    process.env['GIT_EXTERNAL_DIFF'] = prev;
            }
    ...
    it('does not invoke textconv drivers when reading hunks', async () => {
        ...
        try {
            ...
            await execFileAsync('git', ['config', 'diff.evil.textconv', driverScript], {
                cwd: repo,
            });
            await fs.writeFile(path.join(repo, '.gitattributes'), '*.pdf diff=evil\n');
...
describe('fetchGitDiff deletion detection', () => {
    ...
    it('marks tracked files removed from the worktree as isDeleted', async () => {
        ...
        await fs.writeFile(path.join(repo, 'gone.bin'), Buffer.from([0x89, 0x00, 0xff]));
        await git(repo, 'add', '.');
        await git(repo, 'commit', '-q', '-m', 'init');
...
describe('fetchGitDiff special filetypes among untracked files', () => {
    it('marks untracked symlinks as binary and never follows them', async () => {
        ...
        try {
            ...
            try {
                await fs.writeFile(path.join(outside, 'secret.txt'), 'one\ntwo\nthree\n');
...
describe('fetchGitDiff untracked counting', () => {
    ...
    it('aggregates untracked line counts into linesAdded even when the per-file map is full of tracked entries', async () => {
        ...
        expect(result.stats.linesAdded).toBe(trackedLinesAdded + untrackedCount * linesPerFile);
    });
