...
describe('isToolEnabled', () => {
    ...
    it('disables tool when excluded by canonical tool name', () => {
        expect(isToolEnabled(ToolNames.SHELL, undefined, ['run_shell_command'])).toBe(false);
    });
    ...
    it('supports class names with leading underscores', () => {
        expect(isToolEnabled(ToolNames.SHELL, ['__ShellTool'], undefined)).toBe(true);
    });
    it('enables tool when coreTools contains a legacy tool name alias', () => {
        expect(isToolEnabled(ToolNames.GREP, ['search_file_content'], undefined)).toBe(true);
    });
    ...
    it('enables tool when coreTools contains an argument-specific pattern', () => {
        expect(isToolEnabled(ToolNames.SHELL, ['Shell(git status)'], undefined)).toBe(true);
    });
    ...
    it('uses legacy display name aliases when excluding tools', () => {
        expect(isToolEnabled(ToolNames.GREP, undefined, ['SearchFiles'])).toBe(false);
    });
    it('does not treat argument-specific exclusions as matches', () => {
        expect(isToolEnabled(ToolNames.SHELL, undefined, ['Shell(git status)'])).toBe(true);
    });
    it('considers excludeTools even when tool is explicitly enabled', () => {
        expect(isToolEnabled(ToolNames.SHELL, ['Shell'], ['ShellTool'])).toBe(false);
    });
});
