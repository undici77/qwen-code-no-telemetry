...
describe('reportError', () => {
    ...
    it('should not throw when called with a standard error', async () => {
        ...
        await expect(reportError(error, baseMessage, context, type)).resolves.not.toThrow();
        expect(debugLoggerSpy.error).toHaveBeenCalled();
        expect(debugLoggerSpy.error).toHaveBeenCalledWith(`${baseMessage} [${type}]`, expect.any(String));
    });
    it('should handle errors that are plain objects with a message property', async () => {
        ...
        const type = 'general';
        await expect(reportError(error, baseMessage, undefined, type)).resolves.not.toThrow();
        expect(debugLoggerSpy.error).toHaveBeenCalledWith(`${baseMessage} [${type}]`, expect.any(String));
    });
    ...
    it('should handle stringification failure of report content (e.g. BigInt in context)', async () => {
        ...
        await expect(reportError(error, baseMessage, context, 'bigint-fail')).resolves.not.toThrow();
        expect(debugLoggerSpy.error).toHaveBeenCalledWith(`${baseMessage} [bigint-fail] Could not stringify report content (likely due to context):`, expect.any(TypeError), error);
        expect(debugLoggerSpy.error).toHaveBeenCalledWith(`${baseMessage} [bigint-fail]`, expect.any(String));
    });
