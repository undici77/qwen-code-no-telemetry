...
describe('atomicWriteFile', () => {
    let tmpDir;
    beforeEach(async () => {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'atomic-write-file-test-'));
    });
    afterEach(async () => {
        await fs.rm(tmpDir, { recursive: true, force: true });
    });
    ...
    it.skipIf(process.platform === 'win32')('should preserve existing file permissions', async () => {
        const filePath = path.join(tmpDir, 'test.txt');
    ...
    it.skipIf(process.platform === 'win32')('should apply explicit mode option for new files', async () => {
    ...
    it('should resolve relative symlink targets through directory symlinks', async () => {
        ...
        await atomicWriteFile(path.join(linkDir, 'file.txt'), 'updated via dir symlink');
        ...
        expect(path.normalize(await fs.readlink(linkDir))).toBe(path.normalize(realDir));
        expect(path.normalize(await fs.readlink(linkInRealDir))).toBe(path.normalize('../otherDir/target.txt'));
    });
});
