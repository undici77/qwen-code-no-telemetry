...
describe('sessionStorageUtils', () => {
    ...
    describe('readLastJsonStringFieldSync', () => {
        ...
        it('falls back to head window when tail has no match', () => {
            ...
            const titleLine = '{"subtype":"custom_title","customTitle":"in-head-window"}';
            ...
            expect(readLastJsonStringFieldSync(p, 'customTitle', 'custom_title')).toBe('in-head-window');
        });
        it('returns undefined when title is buried beyond both head and tail windows', () => {
            ...
            const padTo = (label, byteCount) => {
                const filler = '{"type":"user","message":"' +
            ...
            const titleLine = '{"subtype":"custom_title","customTitle":"buried-out-of-reach"}';
            const content = padTo('{"type":"user"}', buryWindow) +
                titleLine +
                '\n' +
        ...
        it('does not pick up a customTitle from a partial trailing line in the head window', () => {
            ...
            const line1 = '{"type":"system","subtype":"custom_title","customTitle":"complete"}\n';
            const line2Prefix = '{"type":"system","subtype":"custom_title","customTitle":"phantom","filler":"';
            // Make line2 long enough that its closing `"}\n` is past the head
            // window (LITE_READ_BUF_SIZE = 64KB). 70KB of `x` guarantees that.
            const line2 = line2Prefix + 'x'.repeat(LITE_READ_BUF_SIZE + 8 * 1024) + '"}\n';
            // Push file size past 2 × LITE_READ_BUF_SIZE so listSessions-style
            // callers go through head fallback (tail has no match).
            const tailFiller = '{"type":"user","message":"' +
                'a'.repeat(LITE_READ_BUF_SIZE + 4 * 1024) +
                '"}\n';
            const p = writeFile('partial-line-head.jsonl', line1 + line2 + tailFiller);
            ...
            expect(readLastJsonStringFieldSync(p, 'customTitle', 'custom_title')).toBe('complete');
        });
        it('reuses a caller-provided scratch buffer across tail and head reads', () => {
            ...
            const big = writeFile('big.jsonl', '{"subtype":"custom_title","customTitle":"big-file"}\n');
            ...
            scratch.fill(0x55);
            expect(readLastJsonStringFieldSync(big, 'customTitle', 'custom_title', scratch)).toBe('big-file');
            expect(readLastJsonStringFieldSync(small, 'customTitle', 'custom_title', scratch)).toBe('x');
        });
        it('re-reads the latest tail once when the file grows during a tail miss', () => {
            const p = writeFile('grows-during-tail-miss.jsonl', '{"subtype":"custom_title","customTitle":"old"}\n' +
    ...
    describe('readLastJsonStringFieldsSync', () => {
        ...
        it('returns all-undefined when the pair is buried beyond both head and tail windows', () => {
            ...
            const buryWindow = LITE_READ_BUF_SIZE + 16 * 1024;
            const buriedPair = '{"subtype":"custom_title","customTitle":"buried","titleSource":"auto"}';
            const p = writeFile('buried-pair.jsonl', padTo('{"type":"user"}', buryWindow) +
                buriedPair +
                '\n' +
                padTo('{"type":"user"}', buryWindow));
            expect(readLastJsonStringFieldsSync(p, 'customTitle', ['titleSource'], 'custom_title')).toEqual({ customTitle: undefined, titleSource: undefined });
        ...
        it('reuses a caller-provided scratch buffer across tail and head reads', () => {
            ...
            const tailHit = writeFile('tail-pair.jsonl', '{"subtype":"custom_title","customTitle":"big","titleSource":"manual"}\n');
            const header = '{"subtype":"custom_title","customTitle":"x","titleSource":"auto"}\n';
            const filler = '{"type":"user","message":"' + 'y'.repeat(LITE_READ_BUF_SIZE) + '"}\n';
            const headFallback = writeFile('head-pair.jsonl', header + filler);
            const scratch = Buffer.alloc(LITE_READ_BUF_SIZE);
            scratch.fill(0x55);
            expect(readLastJsonStringFieldsSync(tailHit, 'customTitle', ['titleSource'], 'custom_title', scratch)).toEqual({ customTitle: 'big', titleSource: 'manual' });
            expect(readLastJsonStringFieldsSync(headFallback, 'customTitle', ['titleSource'], 'custom_title', scratch)).toEqual({ customTitle: 'x', titleSource: 'auto' });
        });
        it('re-reads the latest tail once when the file grows during a tail miss', () => {
            const p = writeFile('grows-during-tail-miss-pair.jsonl', '{"subtype":"custom_title","customTitle":"old","titleSource":"manual"}\n' +
                'x'.repeat(LITE_READ_BUF_SIZE + 16 * 1024) +
                '\n');
            ...
            expect(readLastJsonStringFieldsSync(p, 'customTitle', ['titleSource'], 'custom_title')).toEqual({ customTitle: 'new', titleSource: 'auto' });
        });
    });
});
