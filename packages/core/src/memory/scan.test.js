...
import { parseAutoMemoryTopicDocument, scanAutoMemoryTopicDocuments, } from './scan.js';
...
describe('auto-memory topic scanning', () => {
    ...
    it('parses a managed auto-memory topic document', () => {
        const parsed = parseAutoMemoryTopicDocument('/tmp/project.md', [
            '---',
            'type: project',
    ...
    it('scans existing auto-memory files from nested topic folders', async () => {
        const referencePath = getAutoMemoryFilePath(projectRoot, path.join('reference', 'grafana.md'));
