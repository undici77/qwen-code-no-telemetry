...
describe('selectRelevantAutoMemoryDocumentsByModel', () => {
    ...
    it('returns documents chosen by the side-query selector', async () => {
        ...
        const result = await selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'check preferences', docs, 2, []);
    ...
    it('forwards caller abort signal to runSideQuery combined with timeout', async () => {
        ...
        await selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'check preferences', docs, 2, [], callerController.signal);
    ...
    it('uses timeout-only abort signal when no caller signal provided', async () => {
        ...
        await selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'check preferences', docs, 2);
        expect(runSideQuery).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
    ...
    it('lets runSideQuery choose the default side-query model when fast model is configured', async () => {
        ...
        await selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'check the latency dashboard', docs, 2);
        expect(runSideQuery).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
            purpose: 'auto-memory-recall',
            config: { temperature: 0 },
        }));
        expect('model' in vi.mocked(runSideQuery).mock.calls[0][1]).toBe(false);
    });
