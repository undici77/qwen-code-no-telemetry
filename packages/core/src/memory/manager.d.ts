...
import { type BuildMemoryPromptOptions, type UserAutoMemorySection, type TeamAutoMemorySection } from './prompt.js';
...
export interface SkillReviewScheduleResult {
    ...
    taskId?: string;
    skippedReason?: 'below_threshold' | 'skills_modified_in_session' | 'disabled' | 'already_running' | 'memory_pressure';
...
export interface DreamScheduleResult {
    status: 'scheduled' | 'skipped';
    taskId?: string;
    skippedReason?: 'disabled' | 'same_session' | 'min_hours' | 'min_sessions' | 'scan_throttled' | 'locked' | 'running' | 'memory_pressure';
    promise?: Promise<MemoryTaskRecord>;
}
...
export declare class MemoryManager {
    ...
    acceptPendingSkillFromTask(taskId: string, skillName: string): Promise<void>;
    /** Discard one staged skill (by dir name) for the given skill-review task. */
    rejectPendingSkillFromTask(taskId: string, skillName: string): Promise<void>;
    ...
    forgetMatches(projectRoot: string, matches: AutoMemoryForgetMatch[], now?: Date, options?: {
        abortSignal?: AbortSignal;
     ...
     */
    buildAutoMemoryPrompt(memoryDir: string, indexContent?: string | null, userSection?: UserAutoMemorySection, teamSection?: TeamAutoMemorySection, options?: BuildMemoryPromptOptions): string;
    /**
