...
describe('GeminiContentGenerator', () => {
    ...
    it("maps reasoning effort 'max' to HIGH (Gemini has no higher tier)", async () => {
        ...
        await generatorWithMax.generateContent({ model: 'gemini-2.5-pro', contents: [] }, 'prompt-id');
        expect(mockGoogleGenAI.models.generateContent).toHaveBeenCalledWith(expect.objectContaining({
            config: expect.objectContaining({
                thinkingConfig: {
                    includeThoughts: true,
    ...
    it('should strip displayName from inlineData and fileData before sending to API', async () => {
        ...
        expect(calledWith.contents[0].parts[0].inlineData.displayName).toBeUndefined();
        expect(calledWith.contents[0].parts[1].inlineData).toEqual({
            mimeType: 'application/pdf',
            data: 'base64pdfdata',
        });
        expect(calledWith.contents[0].parts[1].inlineData.displayName).toBeUndefined();
        // Verify displayName is stripped from fileData
        expect(calledWith.contents[0].parts[2].fileData).toEqual({
            mimeType: 'application/pdf',
            fileUri: 'gs://bucket/file.pdf',
        });
        expect(calledWith.contents[0].parts[2].fileData.displayName).toBeUndefined();
    });
    it('should strip displayName from functionResponse parts', async () => {
        ...
        await generator.generateContent(request, 'prompt-id');
        const calledWith = mockGoogleGenAI.models.generateContent.mock.calls[0][0];
        const functionResponseParts = calledWith.contents[0].parts[0].functionResponse.parts;
    ...
    it('should convert audio and video to text in functionResponse parts', async () => {
        ...
        expect(functionResponseParts[1].text).toBe('Unsupported media type for Gemini: audio/wav (recording.wav).');
        expect(functionResponseParts[2].text).toBe('Unsupported media type for Gemini: video/mp4.');
    });
});
