...
export class BaseLlmClient {
    ...
    async generateJson(options) {
        ...
        const { contentGenerator, retryAuthType, model: requestModel, } = await this.resolveForModel(model);
        try {
    ...
    async generateText(options) {
        const { contents, model, abortSignal, systemInstruction, promptId, maxAttempts, } = options;
        const requestConfig = {
            abortSignal,
            ...options.config,
            ...(systemInstruction && { systemInstruction }),
        ...
        try {
            ...
            await reportError(error, 'Error generating text content via API.', contents, 'generateText-api');
    ...
    async resolveForModel(model) {
        ...
        const contentGenerator = await this.createContentGeneratorForModel(model, selector);
     ...
     */
    resolveModelAcrossAuthTypes(model, selector) {
    ...
    async createContentGeneratorForModel(model, selector) {
