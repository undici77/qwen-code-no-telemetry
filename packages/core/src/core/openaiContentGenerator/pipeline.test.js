...
describe('ContentGenerationPipeline', () => {
    ...
    describe('execute', () => {
        ...
        it('should redact proxy credentials before request errors reach the error handler', async () => {
            ...
            const userPromptId = 'test-prompt-id';
            const testError = new Error('connect ECONNREFUSED token@proxy.local:8080');
            mockConverter.convertGeminiRequestToOpenAI.mockReturnValue([]);
            mockClient.chat.completions.create.mockRejectedValue(testError);
            await expect(pipeline.execute(request, userPromptId)).rejects.toThrow('connect ECONNREFUSED <redacted>@proxy.local:8080');
    ...
    describe('executeStream', () => {
        ...
        it('should redact proxy credentials from stream creation errors', async () => {
            ...
            mockConverter.convertGeminiRequestToOpenAI.mockReturnValue([]);
            mockClient.chat.completions.create.mockRejectedValue(testError);
            await expect(pipeline.executeStream(request, userPromptId)).rejects.toThrow('407 via http://<redacted>@proxy.local');
            expect(mockErrorHandler.handle).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('should redact proxy credentials from StreamContentError messages', async () => {
            ...
            mockConverter.convertGeminiRequestToOpenAI.mockReturnValue([]);
            mockClient.chat.completions.create.mockResolvedValue(mockStream);
            const resultGenerator = await pipeline.executeStream(request, 'prompt-id');
            await expect(async () => {
                for await (const _ of resultGenerator) {
                    // consume stream
                }
    ...
    describe('openaiRequestCaptureContext integration', () => {
        it('forwards the provider-enhanced request to the active capture', async () => {
            ...
            expect(mockClient.chat.completions.create).toHaveBeenCalledWith(captured, expect.anything());
        ...
        it('captures the streaming request including stream/stream_options', async () => {
            ...
            mockClient.chat.completions.create.mockResolvedValue(fakeStream);
            mockProvider.buildRequest.mockImplementation((req) => ({
                ...req,
