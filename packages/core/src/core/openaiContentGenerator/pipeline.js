...
export class ContentGenerationPipeline {
    ...
    _getMockToolCalls(content) {
        const toolCalls = [];
        const lowerContent = content.toLowerCase();
        if (lowerContent.includes('add 5 and 10') ||
        ...
        else if (lowerContent.includes('list_directory') ||
        ...
        else if (lowerContent.includes('run_shell_command') ||
        ...
        else if (lowerContent.includes('todo_write') ||
        ...
        else if (lowerContent.includes('cron_create') ||
    ...
    getMockResponse(openaiRequest, context) {
        ...
        if (content.toLowerCase().includes('cron tools') &&
        ...
        return OpenAIContentConverter.convertOpenAIResponseToGemini(response, context);
    }
    async *getMockStream(openaiRequest, context) {
        const lastMessage = openaiRequest.messages[openaiRequest.messages.length - 1];
        const content = typeof lastMessage.content === 'string' ? lastMessage.content : '';
        const toolCalls = this._getMockToolCalls(content);
