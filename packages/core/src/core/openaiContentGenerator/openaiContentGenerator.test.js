...
describe('OpenAIContentGenerator (Refactored)', () => {
    ...
    describe('embedContent', () => {
        ...
        it('should redact proxy credentials from embedding request errors', async () => {
            const generatorWithClient = generator;
            generatorWithClient.pipeline.client.embeddings.create.mockRejectedValue(new Error('connect ECONNREFUSED token@proxy.local:8080'));
            await expect(generator.embedContent({
                model: 'text-embedding-ada-002',
                contents: 'hello',
            })).rejects.toThrow('OpenAI API error: connect ECONNREFUSED <redacted>@proxy.local:8080');
        });
    });
