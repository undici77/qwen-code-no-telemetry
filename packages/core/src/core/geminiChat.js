...
import { DEFAULT_TOKEN_LIMIT, ESCALATED_MAX_TOKENS, tokenLimit, } from './tokenLimits.js';
...
function buildOutputRecoveryMessage(previousModelTurn) {
    const previousText = previousModelTurn?.role === 'model'
...
export class GeminiChat {
     ...
     * telemetry singleton.
     */
    async tryCompress(promptId, model, force = false, signal, options) {
        ...
        const { newHistory, info, summary } = await service.compress(this, {
            ...
            originalTokenCount: options?.originalTokenCountOverride ?? this.lastPromptTokenCount,
    ...
    async sendMessageStream(model, params, prompt_id) {
        ...
        try {
            ...
            compressionInfo = await this.tryCompress(prompt_id, model, false, params.config?.abortSignal);
            ...
            try {
                    ...
                    try {
                        ...
                        if (contextOverflow.isExceeded) {
                            if (!reactiveCompressionAttempted) {
                                ...
                                try {
                                    const reactiveInfo = await self.tryCompress(prompt_id, model, true, params.config?.abortSignal, {
                ...
                if (lastError === null &&
                    ...
                    while (escalatedFinishReason === FinishReason.MAX_TOKENS &&
                        ...
                        self.history.push(createUserContent([
