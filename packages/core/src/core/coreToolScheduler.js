...
import { applyAutoModeDecision, evaluateAutoMode, shouldRunAutoModeForCall, } from '../permissions/autoMode.js';
import { MAX_TRANSCRIPT_MESSAGES } from '../permissions/classifier-transcript.js';
import { isApproveOutcome, recordAllow, recordFallbackApprove, shouldFallback, } from '../permissions/denialTracking.js';
...
import { startToolSpan, endToolSpan, runInToolSpanContext, startToolExecutionSpan, endToolExecutionSpan, startToolBlockedOnUserSpan, endToolBlockedOnUserSpan, startHookSpan, endHookSpan, addToolInputAttributes, addToolResultAttributes, truncateSpanError, } from '../telemetry/index.js';
...
async function safelyFirePostToolUseFailureHook(messageBus, toolUseId, toolName, toolInput, errorMessage, isInterrupt, permissionMode) {
    try {
        return await firePostToolUseFailureHook(messageBus, toolUseId, toolName, toolInput, errorMessage, isInterrupt, permissionMode);
    }
    catch (error) {
...
function isConcurrencySafe(call) {
    ...
    if (canonicalToolName(call.request.name) === ToolNames.AGENT)
        return true;
...
export class CoreToolScheduler {
        ...
        try {
            ...
            for (const toolCall of newToolCalls) {
                ...
                try {
                    ...
                    const flowResult = await evaluatePermissionFlow(this.config, invocation, canonicalName, toolParams);
                    ...
                    if (shouldRunAutoModeForCall(approvalMode, canonicalName)) {
                        ...
                        switch (outcome.kind) {
                            case 'approved':
                                ...
                                continue;
                            case 'blocked':
                                this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, new Error(outcome.errorMessage), ToolErrorType.EXECUTION_DENIED));
                                continue;
                            case 'fallback':
                                ...
                                if (fallback.fallback) {
                                    debugLogger.warn(`Auto mode fallback to manual approval (${fallback.reason}): consecutiveBlock=${denialState.consecutiveBlock}, consecutiveUnavailable=${denialState.consecutiveUnavailable}`);
                                }
                                break;
                            default: {
                    ...
                    if (!needsConfirmation(finalPermission, approvalMode, canonicalName)) {
                        ...
                        if (hooksEnabled && messageBus) {
                            ...
                            const hookResult = await firePermissionRequestHook(messageBus, canonicalName, reqInfo.args || {}, permissionMode);
                        ...
                        if (hooksEnabled && messageBus) {
                            fireNotificationHook(messageBus, `Qwen Code needs your permission to use ${reqInfo.name}`, NotificationType.PermissionPrompt, 'Permission needed').catch((error) => {
                    ...
                    this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, error instanceof Error ? error : new Error(String(error)), explicitErrorType ?? ToolErrorType.UNHANDLED_EXCEPTION));
    ...
    async _handleConfirmationResponseInner(callId, toolCall, originalOnConfirm, outcome, signal, payload) {
        ...
        if (this.config.getApprovalMode() === ApprovalMode.AUTO &&
            ...
            this.config.setAutoModeDenialState(recordFallbackApprove(this.config.getAutoModeDenialState()));
        }
    ...
    async executeSingleToolCall(toolCall, signal) {
        ...
        try {
            ...
            this.setStatusInternal(callId, 'error', createErrorResponse(scheduledCall.request, error instanceof Error ? error : new Error(errorMessage), ToolErrorType.UNHANDLED_EXCEPTION));
            throw error;
        }
        finally {
    ...
    async _executeToolCallBody(scheduledCall, signal, span) {
        ...
        if (hooksEnabled && messageBus) {
            ...
            const preHookResult = await this.withHookSpan({ hookEvent: 'PreToolUse', toolName: canonicalName, toolUseId }, () => firePreToolUseHook(messageBus, canonicalName, toolInput, toolUseId, permissionMode), (r) => r.hookError
                ? {
                    success: false,
                    error: r.hookError,
            ...
            if (!preHookResult.shouldProceed) {
                ...
                addToolResultAttributes(this.config, span, toolName, `BLOCKED: ${blockMessage}`);
        ...
        try {
            ...
            if (invocation instanceof ShellToolInvocation) {
                ...
                promise = invocation.execute(signal, liveOutputCallback, shellExecutionConfig, setPidCallback, setPromoteAbortControllerCallback);
            }
            else {
            ...
            if (aborted) {
                ...
                if (hooksEnabled && messageBus) {
                    const failureHookResult = await this.withHookSpan({
                        ...
                        toolUseId,
                        isInterrupt: true,
                    }, () => safelyFirePostToolUseFailureHook(messageBus, toolUseId, canonicalName, toolInput, cancelMessage, true, this.config.getApprovalMode()), this.postToolUseFailureEndMeta);
                    // Append additional context from hook if provided
                    if (failureHookResult.additionalContext) {
                        cancelMessage += `\n\n${failureHookResult.additionalContext}`;
                    }
                }
                addToolResultAttributes(this.config, span, toolName, `CANCELLED: ${cancelMessage}`);
                this.setStatusInternal(callId, 'cancelled', cancelMessage);
                setToolSpanCancelled(span);
            ...
            if (toolResult.error === undefined) {
                ...
                if (hooksEnabled && messageBus) {
                    ...
                    const permissionMode = this.config.getApprovalMode();
                    const postHookResult = await this.withHookSpan({ hookEvent: 'PostToolUse', toolName: canonicalName, toolUseId }, () => firePostToolUseHook(messageBus, canonicalName, toolInput, toolResponse, toolUseId, permissionMode), (r) => r.hookError
                        ? {
                            success: false,
                            error: r.hookError,
                    ...
                    if (postHookResult.shouldStop) {
                        ...
                        addToolResultAttributes(this.config, span, toolName, `STOPPED: ${stopMessage}`);
                        this.setStatusInternal(callId, 'error', errorResponse);
                ...
                if (this.config.getTelemetryIncludeSensitiveSpanAttributes?.()) {
                    addToolResultAttributes(this.config, span, toolName, typeof content === 'string'
                        ? content
                ...
                if (hooksEnabled && messageBus) {
                    const failureHookResult = await this.withHookSpan({
                        ...
                        toolUseId,
                        isInterrupt: false,
                    }, () => safelyFirePostToolUseFailureHook(messageBus, toolUseId, canonicalName, toolInput, toolResult.error.message, false, this.config.getApprovalMode()), this.postToolUseFailureEndMeta);
                    // Append additional context from hook if provided
                    if (failureHookResult.additionalContext) {
                        errorMessage += `\n\n${failureHookResult.additionalContext}`;
                    }
                }
                addToolResultAttributes(this.config, span, toolName, `ERROR: ${errorMessage}`);
            ...
            if (aborted) {
                ...
                if (hooksEnabled && messageBus) {
                    const failureHookResult = await this.withHookSpan({
                        ...
                        toolUseId,
                        isInterrupt: false,
                    }, () => safelyFirePostToolUseFailureHook(messageBus, toolUseId, canonicalName, toolInput, errorMessage, false, this.config.getApprovalMode()), this.postToolUseFailureEndMeta);
                    // Append additional context from hook if provided
                    if (failureHookResult.additionalContext) {
                        exceptionErrorMessage += `\n\n${failureHookResult.additionalContext}`;
                    }
                }
                addToolResultAttributes(this.config, span, toolName, `EXCEPTION: ${exceptionErrorMessage}`);
