...
const mockInvocation = (overrides = {}) => ({
...
describe('evaluatePermissionFlow', () => {
    it('should return deny result with correct message when defaultPermission is deny', async () => {
        ...
        const result = await evaluatePermissionFlow(mockConfig(), invocation, 'shell', { command: 'rm -rf /' });
...
describe('needsConfirmation', () => {
    it('should return false for YOLO mode non-ask_user_question tools', () => {
        ...
        expect(needsConfirmation('default', ApprovalMode.YOLO, 'read_file')).toBe(false);
    });
    it('should return true for ask_user_question in YOLO mode', () => {
        expect(needsConfirmation('ask', ApprovalMode.YOLO, ToolNames.ASK_USER_QUESTION)).toBe(true);
    });
    it('should return true when finalPermission is ask or default', () => {
        ...
        expect(needsConfirmation('default', ApprovalMode.DEFAULT, 'shell')).toBe(true);
    });
    it('should return false when finalPermission is allow or deny', () => {
        expect(needsConfirmation('allow', ApprovalMode.DEFAULT, 'shell')).toBe(false);
        expect(needsConfirmation('deny', ApprovalMode.DEFAULT, 'shell')).toBe(false);
    });
});
describe('isPlanModeBlocked', () => {
    const mockConfirmationDetails = (type) => ({ type });
    it('should block non-info tools in plan mode', () => {
        expect(isPlanModeBlocked(true, false, false, mockConfirmationDetails('exec'))).toBe(true);
        expect(isPlanModeBlocked(true, false, false, mockConfirmationDetails('edit'))).toBe(true);
    });
    it('should not block info-type tools in plan mode', () => {
        expect(isPlanModeBlocked(true, false, false, mockConfirmationDetails('info'))).toBe(false);
    });
    it('should not block exit_plan_mode tool', () => {
        expect(isPlanModeBlocked(true, true, false, mockConfirmationDetails('exec'))).toBe(false);
    });
    it('should not block ask_user_question tool', () => {
        expect(isPlanModeBlocked(true, false, true, mockConfirmationDetails('exec'))).toBe(false);
    });
    it('should not block when not in plan mode', () => {
        expect(isPlanModeBlocked(false, false, false, mockConfirmationDetails('exec'))).toBe(false);
    });
});
describe('isAutoEditApproved', () => {
    const mockConfirmationDetails = (type) => ({ type });
    it('should auto-approve edit-type tools in AUTO_EDIT mode', () => {
        expect(isAutoEditApproved(ApprovalMode.AUTO_EDIT, mockConfirmationDetails('edit'))).toBe(true);
    });
    it('should auto-approve info-type tools in AUTO_EDIT mode', () => {
        expect(isAutoEditApproved(ApprovalMode.AUTO_EDIT, mockConfirmationDetails('info'))).toBe(true);
    });
    it('should not auto-approve exec-type tools in AUTO_EDIT mode', () => {
        expect(isAutoEditApproved(ApprovalMode.AUTO_EDIT, mockConfirmationDetails('exec'))).toBe(false);
    });
    it('should not auto-approve in non-AUTO_EDIT mode', () => {
        expect(isAutoEditApproved(ApprovalMode.DEFAULT, mockConfirmationDetails('edit'))).toBe(false);
    });
});
//# sourceMappingURL=permissionFlow.test.js.map