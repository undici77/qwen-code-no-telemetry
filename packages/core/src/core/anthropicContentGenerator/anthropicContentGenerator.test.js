...
describe('AnthropicContentGenerator', () => {
    ...
    describe('env back-fill suppression and baseURL env resolution', () => {
        ...
        it('suppresses ANTHROPIC_API_KEY back-fill on the proxy branch (prevents credential leak)', async () => {
            ...
            expect(anthropicState.constructorOptions?.['apiKey']).toBeNull();
            expect(anthropicState.constructorOptions?.['authToken']).toBe('idealab-token');
        });
        it('suppresses ANTHROPIC_AUTH_TOKEN back-fill on the Anthropic-native branch', async () => {
            ...
            expect(anthropicState.constructorOptions?.['apiKey']).toBe('config-api-key');
            expect(anthropicState.constructorOptions?.['authToken']).toBeNull();
        });
    ...
    describe('per-request anthropic-beta header', () => {
        ...
        it('drops only the cache-scope beta when enableCacheControl is false but reasoning is on', async () => {
            ...
            expect(headers['anthropic-beta']).toContain('interleaved-thinking-2025-05-14');
            expect(headers['anthropic-beta']).toContain('effort-2025-11-24');
            expect(headers['anthropic-beta']).not.toContain('prompt-caching-scope-2026-01-05');
        });
        it('reflects hot enableCacheControl flips between requests (no stale converter cache)', async () => {
            ...
            let [req, options] = anthropicState.lastCreateArgs;
            let reqHeaders = (options
                ?.headers || {});
            expect(reqHeaders['anthropic-beta']).toBe('prompt-caching-scope-2026-01-05');
            ...
            const reqMessages = req
                .messages;
            const userBlocks = reqMessages?.[0]?.content;
            ...
            await generator.generateContent(requestWithTool);
            [req, options] = anthropicState.lastCreateArgs;
            reqHeaders = (options
                ?.headers || {});
            ...
            const reqMessages2 = req
                .messages;
            const userBlocks2 = reqMessages2?.[0]?.content;
        ...
        it('suppresses the cache-scope beta when the body has no scope field (empty system + no tools)', async () => {
            ...
            const generator = new AnthropicContentGenerator({ ...baseConfig, reasoning: false }, mockConfig);
            await generator.generateContent({
                model: 'models/ignored',
                contents: 'Hi',
        ...
        it('keeps customHeaders + User-Agent in defaultHeaders while sending computed anthropic-beta per-request', async () => {
            ...
            expect(reqHeaders['anthropic-beta']).toContain('prompt-caching-scope-2026-01-05');
        });
    ...
    describe('generateContent', () => {
        it('redacts proxy credentials from request-time SDK errors', async () => {
            const { AnthropicContentGenerator } = await importGenerator();
            anthropicState.createImpl.mockRejectedValue(new Error('connect ECONNREFUSED token@proxy.local:8080'));
            const generator = new AnthropicContentGenerator({
                model: 'claude-test',
                apiKey: 'test-key',
                timeout: 10_000,
        ...
        describe('adaptive thinking (Claude 4.6+ models)', () => {
            ...
            async function thinkingFor(model, reasoningOverride) {
                const { AnthropicContentGenerator } = await importGenerator();
                anthropicState.createImpl.mockResolvedValue({
                    id: 'anthropic-1',
                    model,
                ...
                const [req] = anthropicState.lastCreateArgs;
                return req.thinking;
            }
            ...
            it('honors explicit reasoning.budget_tokens before falling back to adaptive', async () => {
                ...
                // run first.
                expect(await thinkingFor('claude-opus-4-7', {
                    effort: 'medium',
            ...
            it('still ships adaptive (no output_config, no effort beta) when reasoning is undefined on a 4.6+ model', async () => {
                ...
                expect(req).toEqual(expect.not.objectContaining({ output_config: expect.anything() }));
                const headers = (options
                    ?.headers || {});
                expect(headers['anthropic-beta']).toContain('interleaved-thinking-2025-05-14');
    ...
    describe('generateContentStream', () => {
        it('redacts proxy credentials from stream creation errors', async () => {
            const { AnthropicContentGenerator } = await importGenerator();
            anthropicState.createImpl.mockRejectedValue(new Error('407 via http://user:pass@proxy.local'));
            const generator = new AnthropicContentGenerator({
                model: 'claude-test',
                apiKey: 'test-key',
                timeout: 10_000,
            ...
            await expect(generator.generateContentStream({
                model: 'models/ignored',
                contents: 'Hello',
        ...
        it('redacts proxy credentials from stream iteration errors', async () => {
            ...
            anthropicState.createImpl.mockResolvedValue({
                [Symbol.asyncIterator]: () => ({
                    next: vi
                        .fn()
                        .mockRejectedValue(new Error('connect ECONNREFUSED token@proxy.local:8080')),
                }),
            });
            const generator = new AnthropicContentGenerator({
                model: 'claude-test',
