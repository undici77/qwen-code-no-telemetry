 ...
 */
export function getCustomSystemPrompt(customInstruction, userMemory, appendInstruction, deferredTools) {
 ...
 */
export function buildDeferredToolsSection(deferredTools) {
    ...
    const lines = deferredTools.map(({ name, description }) => {
        ...
        const truncated = firstLine.length > MAX_DESC_LEN
    ...
    const exampleName = deferredTools.find((t) => !t.name.includes('`'))?.name ?? '<tool_name>';
    return `
...
export function getCoreSystemPrompt(userMemory, model, appendInstruction, deferredTools) {
