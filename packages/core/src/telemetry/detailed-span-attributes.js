...
export function addUserPromptAttributes(config, span, promptText) {
    if (!isEnabled(config) || !promptText)
        return;
...
export function addSystemPromptAttributes(config, span, systemInstruction) {
    if (!isEnabled(config) || !systemInstruction)
        return;
...
export function addToolSchemaAttributes(config, span, tools) {
    if (!isEnabled(config) || !tools?.length)
        return;
...
export function addModelOutputAttributes(config, span, responseText) {
    if (!isEnabled(config) || !responseText)
        return;
...
// --- Tool Span: Input ---
export function addToolInputAttributes(config, span, toolName, toolInput) {
    if (!isEnabled(config))
        return;
...
// --- Tool Span: Result ---
export function addToolResultAttributes(config, span, toolName, toolResult) {
    if (!isEnabled(config))
        return;
