...
import { __resetActiveGoalStoreForTests, activeGoalEquals, clearActiveGoal, getActiveGoal, recordGoalIteration, setActiveGoal, } from './activeGoalStore.js';
...
describe('activeGoalStore', () => {
    ...
    it('compares active goal snapshots by value', () => {
        ...
        expect(activeGoalEquals(makeGoal(), makeGoal({ lastReason: undefined }))).toBe(true);
        expect(activeGoalEquals(makeGoal({ iterations: 1 }), makeGoal({ iterations: 2 }))).toBe(false);
        expect(activeGoalEquals(makeGoal(), undefined)).toBe(false);
    });
});
//# sourceMappingURL=activeGoalStore.test.js.map