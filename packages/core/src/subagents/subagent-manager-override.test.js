...
describe('SubagentManager.buildSubagentContextOverride bound-tool isolation', () => {
    ...
    function callBuildOverride(manager, base) {
        const fn = manager.buildSubagentContextOverride.bind(manager);
        return fn(base);
    }
    ...
    it('skips rebuild and inherits registry via prototype when an upstream wrapper has already rebuilt the registry (real-world chained-override case)', async () => {
        ...
        const { config: upstreamWrapper } = await createApprovalModeOverride(parent, ApprovalMode.AUTO_EDIT);
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const bgWrapper = Object.create(upstreamWrapper);
        bgWrapper.getShouldAvoidPermissionPrompts = () => true;
        const manager = new SubagentManager(parent);
        const child = await callBuildOverride(manager, bgWrapper);
        ...
        expect(childEdit).toBeInstanceOf(EditTool);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        expect(childEdit.config).toBe(upstreamWrapper);
    });
...
//# sourceMappingURL=subagent-manager-override.test.js.map