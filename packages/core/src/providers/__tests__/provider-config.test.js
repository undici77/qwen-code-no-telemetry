...
import { AuthType, buildInstallPlan, buildProviderTemplate, computeModelListVersion, findProviderByCredentials, getAllProviderBaseUrls, getDefaultModelIds, resolveBaseUrl, shouldShowStep, providerMatchesCredentials, } from '@qwen-code/qwen-code-core';
...
describe('buildInstallPlan', () => {
    ...
    it('throws when models list is empty', () => {
        const config = makeConfig({ models: undefined, modelNamePrefix: '' });
        expect(() => buildInstallPlan(config, {
            baseUrl: 'https://custom.com/v1',
            apiKey: 'sk-custom',
            modelIds: [],
    ...
    it('resolves envKey from function', () => {
        const config = makeConfig({
            envKey: (protocol, baseUrl) => `CUSTOM_${protocol}_${baseUrl.replace(/\W+/g, '_')}`,
            models: undefined,
            modelNamePrefix: '',
        });
        const plan = buildInstallPlan(config, {
...
describe('resolveOwnsModel (via buildInstallPlan)', () => {
    it('auto-derives ownership from string envKey + prefix', () => {
        ...
        expect(ownsModel?.({ id: 'x', envKey: 'TEST_API_KEY', name: '[Pfx] x' })).toBe(true);
        expect(ownsModel?.({ id: 'x', envKey: 'OTHER_KEY', name: '[Pfx] x' })).toBe(false);
        expect(ownsModel?.({ id: 'x', envKey: 'TEST_API_KEY', name: 'no prefix' })).toBe(false);
    });
    ...
    it('uses custom ownsModel when provided', () => {
        const customOwns = (model) => model.id === 'special';
...
describe('resolveBaseUrl', () => {
    it('returns fixed string baseUrl', () => {
        ...
        expect(resolveBaseUrl(config, 'https://ignored.com')).toBe('https://fixed.com');
    });
    ...
    it('returns selectedBaseUrl for undefined config.baseUrl', () => {
        ...
        expect(resolveBaseUrl(config, 'https://typed.com')).toBe('https://typed.com');
...
describe('shouldShowStep', () => {
    ...
    it('shows baseUrl step when undefined or array', () => {
        expect(shouldShowStep(makeConfig({ baseUrl: undefined }), 'baseUrl')).toBe(true);
        expect(shouldShowStep(makeConfig({
            baseUrl: [{ id: 'a', label: 'A', url: 'https://a.com' }],
        }), 'baseUrl')).toBe(true);
        expect(shouldShowStep(makeConfig({ baseUrl: 'https://fixed.com' }), 'baseUrl')).toBe(false);
    });
    ...
    it('shows models step only when editable or undefined', () => {
        expect(shouldShowStep(makeConfig({ models: undefined }), 'models')).toBe(true);
        expect(shouldShowStep(makeConfig({ modelsEditable: true }), 'models')).toBe(true);
        expect(shouldShowStep(makeConfig({ modelsEditable: false }), 'models')).toBe(false);
    });
    it('shows advancedConfig step only when enabled', () => {
        expect(shouldShowStep(makeConfig({ showAdvancedConfig: true }), 'advancedConfig')).toBe(true);
...
describe('providerMatchesCredentials', () => {
    it('matches by string envKey and string baseUrl', () => {
        const config = makeConfig();
        expect(providerMatchesCredentials(config, 'https://api.test.com/v1', 'TEST_API_KEY')).toBe(true);
    });
    it('rejects mismatched envKey', () => {
        const config = makeConfig();
        expect(providerMatchesCredentials(config, 'https://api.test.com/v1', 'OTHER')).toBe(false);
    });
    it('rejects mismatched baseUrl', () => {
        const config = makeConfig();
        expect(providerMatchesCredentials(config, 'https://other.com', 'TEST_API_KEY')).toBe(false);
    });
    it('matches against BaseUrlOption array', () => {
        ...
        expect(providerMatchesCredentials(config, 'https://b.com', 'TEST_API_KEY')).toBe(true);
        expect(providerMatchesCredentials(config, 'https://c.com', 'TEST_API_KEY')).toBe(false);
    });
    it('matches when function-typed envKey derives a matching key', () => {
        ...
        expect(providerMatchesCredentialsSrc(config, 'https://api.test.com/v1', 'DYNAMIC')).toBe(true);
    });
});
describe('computeModelListVersion', () => {
    ...
    it('produces different hashes for different models', () => {
        expect(computeModelListVersion([{ id: 'a' }])).not.toBe(computeModelListVersion([{ id: 'b' }]));
    });
});
describe('buildProviderTemplate', () => {
    ...
    it('uses function-typed modelNamePrefix', () => {
        const config = makeConfig({
            baseUrl: undefined,
            modelNamePrefix: (baseUrl) => baseUrl.includes('intl') ? 'Intl' : 'Default',
            models: [{ id: 'm' }],
        });
...
describe('findProviderByCredentials', () => {
    it('finds a preset by its env key + base URL', () => {
        const found = findProviderByCredentials('https://api.deepseek.com', 'DEEPSEEK_API_KEY');
        expect(found?.id).toBe('deepseek');
    });
    it('returns undefined for an unknown env key', () => {
        expect(findProviderByCredentials('https://api.deepseek.com', 'NOT_A_REAL_KEY')).toBeUndefined();
    });
    it('returns undefined for a known env key but mismatched base URL', () => {
        expect(findProviderByCredentials('https://wrong.example.com/v1', 'DEEPSEEK_API_KEY')).toBeUndefined();
    });
    it('matches a multi-baseUrl preset against any of its registered URLs', () => {
        ...
        const intl = findProviderByCredentials('https://coding-intl.dashscope.aliyuncs.com/v1', 'BAILIAN_CODING_PLAN_API_KEY');
...
import { resolveBaseUrl as resolveBaseUrlSrc, providerMatchesCredentials as providerMatchesCredentialsSrc, } from '../provider-config.js';
describe('resolveBaseUrl edge cases', () => {
    it('does not crash on an empty baseUrl array — falls back to selected or ""', () => {
        ...
        expect(resolveBaseUrlSrc(config, 'https://api.user.com/v1')).toBe('https://api.user.com/v1');
    });
});
describe('providerMatchesCredentials with function envKey (custom provider)', () => {
    ...
    it('matches a custom-style provider whose envKey is a function deriving from baseUrl', () => {
        const derivedFor = (_protocol, baseUrl) => `QWEN_CUSTOM_${Buffer.from(baseUrl).toString('hex').slice(0, 8)}`;
        const config = makeConfig({
            id: 'custom-like',
            envKey: derivedFor,
    ...
    it('does not match when the derived key differs from the supplied envKey', () => {
        const derivedFor = (_protocol, baseUrl) => `QWEN_CUSTOM_${baseUrl.length}`;
        const config = makeConfig({
            id: 'custom-like',
            envKey: derivedFor,
            baseUrl: undefined,
        ...
        expect(providerMatchesCredentialsSrc(config, 'https://api.example.com/v1', 'WRONG_ENV_KEY')).toBe(false);
    });
    it('returns false (not crash) when the function envKey itself throws', () => {
        ...
        expect(providerMatchesCredentialsSrc(config, 'https://api.example.com/v1', 'ANY')).toBe(false);
    });
    it('iterates protocolOptions and matches when any one derives the env key', () => {
        ...
        const derivedFor = (protocol, baseUrl) => `QWEN_CUSTOM_${protocol.toUpperCase()}_${baseUrl.length}`;
        const config = makeConfig({
            id: 'custom-like',
            envKey: derivedFor,
            baseUrl: undefined,
