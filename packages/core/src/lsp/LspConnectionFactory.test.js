...
describe('LspConnectionFactory', () => {
    it('captures stderr and exit code when stdio server closes during initialize', async () => {
        const connection = await LspConnectionFactory.createStdioConnection(process.execPath, [
            '-e',
            'process.stderr.write("clangd failed before initialize\\n"); process.exit(7);',
        ]);
        await expect(connection.connection.initialize({})).rejects.toThrow('LSP connection closed');
...
//# sourceMappingURL=LspConnectionFactory.test.js.map