...
import { DEFAULT_IMAGE_TOKEN_ESTIMATE, estimateContentChars, resolveSlimmingConfig, slimCompactionInput, } from './compactionInputSlimming.js';
/**
...
const hasFunctionCall = (content) => !!content?.parts?.some((part) => !!part.functionCall);
const hasFunctionResponse = (content) => !!content?.parts?.some((part) => !!part.functionResponse);
/**
 ...
 */
function splitPointRetainingTrailingPairs(contents, retainCount) {
    let pairsFound = 0;
 ...
 */
export function findCompressSplitPoint(contents, fraction, retainCount = TOOL_ROUND_RETAIN_COUNT, precomputedCharCounts) {
    ...
    const charCounts = precomputedCharCounts ??
        contents.map((content) => estimateContentChars(content, DEFAULT_IMAGE_TOKEN_ESTIMATE));
    ...
    if (lastContent?.role === 'model') {
        if (!hasFunctionCall(lastContent))
...
export class ChatCompressionService {
    async compress(chat, opts) {
        const { promptId, force, model, config, hasFailedCompressionAttempt, originalTokenCount, trigger, signal, } = opts;
        const compactTrigger = trigger ?? (force ? 'manual' : 'auto');
        const chatCompressionSettings = config.getChatCompression();
        const threshold = chatCompressionSettings?.contextPercentageThreshold ??
        ...
        const charCounts = historyForSplit.map((c) => estimateContentChars(c, slimmingConfig.imageTokenEstimate));
        const splitPoint = findCompressSplitPoint(historyForSplit, 1 - COMPRESSION_PRESERVE_THRESHOLD, TOOL_ROUND_RETAIN_COUNT, charCounts);
        ...
        for (let i = 0; i < splitPoint; i++)
            compressCharCount += charCounts[i];
        const totalCharCount = charCounts.reduce((a, b) => a + b, 0);
        if (totalCharCount > 0 &&
