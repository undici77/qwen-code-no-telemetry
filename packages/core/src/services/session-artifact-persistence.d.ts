...
export declare const WORKSPACE_CONTENT_SHA256_METADATA_KEY =
  'qwen.workspace.sha256';
export declare const WORKSPACE_CONTENT_MTIME_MS_METADATA_KEY =
  'qwen.workspace.mtimeMs';
...
export declare function isSessionArtifactRecord(
  record: SessionArtifactChatRecordLike,
): boolean;
export declare function stableSessionArtifactId(
  sessionId: string,
  identityKey: string,
): string;
export declare function sessionArtifactIdentityKey(
  artifact: Pick<
    PersistedSessionArtifact,
    'workspacePath' | 'managedId' | 'url'
  >,
): string | undefined;
export declare function rebuildSessionArtifactSnapshot(
  records: readonly SessionArtifactChatRecordLike[],
  fallbackSessionId?: string,
): RebuiltSessionArtifactSnapshot | undefined;
export declare function remapSessionArtifactPayloadForFork(
  payload: unknown,
  sourceSessionId: string,
  newSessionId: string,
  remappedArtifactIds?: Map<string, string>,
): unknown;
export declare function normalizeSnapshotPayload(
  value: unknown,
  warnings: string[],
): SessionArtifactSnapshotRecordPayload | undefined;
export declare function normalizeEventPayload(
  value: unknown,
  warnings: string[],
...
export declare function metadataBudgetBytes(
  metadata: Record<string, string | number | boolean | null>,
  budget?: 'user' | 'persisted',
): number;
export declare function isWorkspaceContentMetadataEntry(
  key: string,
