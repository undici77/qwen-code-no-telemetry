...
import { readWorktreeSession, writeWorktreeSession, clearWorktreeSession, restoreWorktreeContext, } from './worktreeSessionService.js';
const sample = {
    slug: 'my-feature',
...
describe('readWorktreeSession', () => {
    ...
    it('returns null when sidecar is missing required fields', async () => {
        ...
        await fs.writeFile(filePath, JSON.stringify({ slug: 'x', worktreePath: '/p' }), // missing 4 fields
        'utf-8');
        expect(await readWorktreeSession(filePath)).toBeNull();
    });
    it('returns null when a required field has the wrong type', async () => {
        await fs.writeFile(filePath, JSON.stringify({ ...sample, slug: 42 }), 'utf-8');
        expect(await readWorktreeSession(filePath)).toBeNull();
    });
});
...
describe('restoreWorktreeContext', () => {
    ...
    it('rejects and clears a sidecar whose worktreePath escapes the managed subtree', async () => {
        ...
        const warnings = [];
        const result = await restoreWorktreeContext(filePath, (e) => warnings.push(e));
        expect(result.session).toBeNull();
        expect(result.contextMessage).toBeNull();
    ...
    it('treats a regular file at worktreePath as not-a-worktree', async () => {
        ...
        const bogus = { ...sample, worktreePath: filePathTarget };
    ...
    it('cleans up sidecar with valid JSON but missing required fields', async () => {
        ...
        await fs.writeFile(filePath, JSON.stringify({ slug: 'incomplete' }), 'utf-8');
        const result = await restoreWorktreeContext(filePath);
        expect(result.session).toBeNull();
        expect(await fs
            .stat(filePath)
...
//# sourceMappingURL=worktreeSessionService.test.js.map