...
import { ChatCompressionService, findCompressSplitPoint, TOOL_ROUND_RETAIN_COUNT, } from './chatCompressionService.js';
...
describe('findCompressSplitPoint', () => {
    ...
    it('honors precomputedCharCounts when provided', () => {
        ...
        expect(findCompressSplitPoint(history, 0.7, TOOL_ROUND_RETAIN_COUNT, inflated)).toBe(4);
        // Same history with even weights yields the standard split.
        const even = [1, 1, 1, 1, 1];
        expect(findCompressSplitPoint(history, 0.7, TOOL_ROUND_RETAIN_COUNT, even)).toBe(4);
    });
});
...
describe('ChatCompressionService', () => {
    ...
    beforeEach(() => {
        ...
        mockChat = {
            getHistory: vi.fn(),
            getHistoryShallow: vi.fn((curated) => mockChat.getHistory(curated)),
    ...
    describe('tool-loop subagent absorption', () => {
        ...
        it('compresses by absorbing older tool rounds when fresh-user split is too small', async () => {
            ...
            const buildHistory = (completePairs) => {
                const h = [
            ...
            vi.mocked(uiTelemetryService.getLastPromptTokenCount).mockReturnValue(80_000);
            vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
                model: 'gemini-pro',
        ...
        it('NOOPs when the keep slice has too few tool rounds to absorb', async () => {
            ...
            // the cheap-gate.
            vi.mocked(uiTelemetryService.getLastPromptTokenCount).mockReturnValue(22_000);
            vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
                model: 'gemini-pro',
