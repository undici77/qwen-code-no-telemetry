...
describe('ChatRecordingService', () => {
    ...
    describe('recordUserMessage', () => {
        ...
        it('should record mid-turn user messages with a mergeable subtype', async () => {
            const modelFacingParts = [
                {
                    text: '\n[User message received during tool execution]: save logs',
                },
            ];
            chatRecordingService.recordMidTurnUserMessage(modelFacingParts, 'save logs');
            await chatRecordingService.flush();
            expect(jsonl.writeLine).toHaveBeenCalledTimes(1);
            const record = vi.mocked(jsonl.writeLine).mock.calls[0][1];
            expect(record.type).toBe('user');
    ...
    describe('recordToolResult', () => {
        ...
        it('should keep small file diff resultDisplay unchanged', async () => {
            ...
            expect((record.toolCallResult?.resultDisplay).truncatedForSession).toBeUndefined();
        });
        it('should shrink large file diff resultDisplay without mutating input', async () => {
            ...
            await chatRecordingService.flush();
            const record = vi.mocked(jsonl.writeLine).mock.calls[0][1];
            const savedDisplay = record.toolCallResult?.resultDisplay;
            ...
            expect(savedDisplay.fileDiff).toContain('Full diff omitted from saved session history');
            expect(savedDisplay.fileDiff).not.toBe(largeDiff);
            expect(savedDisplay.originalContent?.length).toBeLessThanOrEqual(16_000);
            expect(savedDisplay.originalContent).toContain('truncated for saved session preview');
            expect(savedDisplay.newContent.length).toBeLessThanOrEqual(16_000);
            expect(savedDisplay.newContent).toContain('truncated for saved session preview');
    ...
    describe('recordAttributionSnapshot', () => {
        ...
        it('should re-write an identical snapshot after rewindRecording', async () => {
            ...
            expect(vi.mocked(jsonl.writeLine).mock.calls.length).toBe(beforeRewind + 2);
        });
        ...
        it('should retry an identical snapshot after a write failure', async () => {
            ...
            expect(vi.mocked(jsonl.writeLine).mock.calls.length).toBe(afterFailure + 1);
        });
