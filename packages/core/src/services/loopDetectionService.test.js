...
describe('LoopDetectionService', () => {
    ...
    describe('Repetitive Thoughts Detection', () => {
        it('should detect repetitive thoughts pattern', () => {
            service.reset('');
            for (let i = 0; i < 3; i++) {
                service.addAndCheck(createThoughtEvent('Plan', 'Inspect the migration script.'));
            }
        ...
        it('should not detect loop with varied thoughts', () => {
            ...
            service.addAndCheck(createThoughtEvent('Analysis', 'Check migration risks.'));
            service.addAndCheck(createThoughtEvent('Plan', 'Evaluate rollout alternatives.'));
            const isLoop = service.addAndCheck(createThoughtEvent('Next', 'Draft the fix.'));
            expect(isLoop).toBe(false);
            expect(loggers.logLoopDetected).not.toHaveBeenCalled();
        });
        it('should not detect a loop when an earlier thought reappears after progress', () => {
            ...
            service.addAndCheck(createThoughtEvent('Plan', 'Inspect the schema.'));
            service.addAndCheck(createThoughtEvent('Analysis', 'Consider migration.'));
            service.addAndCheck(createThoughtEvent('Analysis', 'Review indexes.'));
            service.addAndCheck(createThoughtEvent('Plan', 'Inspect the schema.'));
            service.addAndCheck(createThoughtEvent('Analysis', 'Consider rollout risks.'));
            const isLoop = service.addAndCheck(createThoughtEvent('Plan', 'Inspect the schema.'));
            expect(isLoop).toBe(false);
            expect(loggers.logLoopDetected).not.toHaveBeenCalled();
        });
        it('clears thought history across tool-call roundtrips within a turn', () => {
            ...
            service.addAndCheck(createThoughtEvent('Plan', 'Inspect the schema.'));
            service.addAndCheck(createToolCallRequestEvent('read_file', { path: 'a.sql' }));
            service.addAndCheck(createThoughtEvent('Plan', 'Inspect the schema.'));
            service.addAndCheck(createToolCallRequestEvent('read_file', { path: 'b.sql' }));
            const isLoop = service.addAndCheck(createThoughtEvent('Plan', 'Inspect the schema.'));
            expect(isLoop).toBe(false);
            expect(loggers.logLoopDetected).not.toHaveBeenCalled();
        });
        it('ignores hedge phrases in Content events (thought detection is Thought-only)', () => {
            ...
            for (let i = 0; i < 5; i++) {
                service.addAndCheck(createContentEvent('I should check the config, maybe it helps.'));
            }
            expect(loggers.logLoopDetected).not.toHaveBeenCalledWith(mockConfig, expect.objectContaining({ loop_type: 'repetitive_thoughts' }));
        });
    });
    describe('Read File Loop Detection', () => {
        ...
        const primeNonReadTool = () => {
            service.addAndCheck(createToolCallRequestEvent('write_file', {
                path: 'prime.txt',
                content: '',
            }));
        };
        ...
        it('should exempt opening exploration from READ_FILE_LOOP (cold start)', () => {
            ...
            for (let i = 0; i < 20; i++) {
                ...
                const isLoop = service.addAndCheck(createToolCallRequestEvent(name, { path: `f${i}` }));
                expect(isLoop).toBe(false);
            }
            expect(loggers.logLoopDetected).not.toHaveBeenCalledWith(mockConfig, expect.objectContaining({ loop_type: 'read_file_loop' }));
        });
        it('should activate READ_FILE_LOOP once a non-read tool lands mid-prompt', () => {
            service.reset('');
            // No firing before the cold-start gate flips.
            for (let i = 0; i < 7; i++) {
                service.addAndCheck(createToolCallRequestEvent('read_file', { path: `pre${i}.txt` }));
            }
            expect(loggers.logLoopDetected).not.toHaveBeenCalled();
            ...
            for (let i = 0; i < FILE_READ_WINDOW + 2 && !detected; i++) {
                detected = service.addAndCheck(createToolCallRequestEvent('read_file', { path: `post${i}.txt` }));
            }
            expect(detected).toBe(true);
            expect(loggers.logLoopDetected).toHaveBeenCalledWith(mockConfig, expect.objectContaining({ loop_type: 'read_file_loop' }));
        });
        it('should detect other read-like operations (exact names + read_/list_ prefixes)', () => {
            ...
            service.addAndCheck(createToolCallRequestEvent('read_many_files', {
                paths: ['file1.txt'],
            }));
            service.addAndCheck(createToolCallRequestEvent('list_directory', { path: '.' }));
            service.addAndCheck(createToolCallRequestEvent('read_resource', { uri: 'a' }));
            ...
            service.addAndCheck(createToolCallRequestEvent('read_file', { path: 'file5.txt' }));
            service.addAndCheck(createToolCallRequestEvent('read_many_files', {
                paths: ['file6.txt'],
            }));
            const isLoop = service.addAndCheck(createToolCallRequestEvent('list_directory', { path: 'nested' }));
            expect(isLoop).toBe(true);
            expect(loggers.logLoopDetected).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
                loop_type: 'read_file_loop',
            }));
        ...
        it('should not treat tools that merely contain read-like substrings as file reads', () => {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                const isLoop = service.addAndCheck(createToolCallRequestEvent(name, { i }));
                expect(isLoop).toBe(false);
            }
            expect(loggers.logLoopDetected).not.toHaveBeenCalledWith(mockConfig, expect.objectContaining({ loop_type: 'read_file_loop' }));
        });
    ...
    describe('Action Stagnation Detection', () => {
        ...
        it('should detect action stagnation when the same tool is repeated with varying args', () => {
            ...
            for (let i = 0; i < 7; i++) {
                const isLoop = service.addAndCheck(createToolCallRequestEvent('search_code', { query: `term${i}` }));
                expect(isLoop).toBe(false);
            }
            // THRESHOLD-th consecutive same-name call triggers stagnation
            const isLoop = service.addAndCheck(createToolCallRequestEvent('search_code', { query: 'term7' }));
            expect(isLoop).toBe(true);
            expect(loggers.logLoopDetected).toHaveBeenCalledWith(mockConfig, expect.objectContaining({ loop_type: 'action_stagnation' }));
        });
        it('should reset stagnation streak when a different tool is called', () => {
            ...
            for (let i = 0; i < 5; i++) {
                service.addAndCheck(createToolCallRequestEvent('search_code', { query: `a${i}` }));
            }
            ...
            for (let i = 0; i < 5; i++) {
                const isLoop = service.addAndCheck(createToolCallRequestEvent('search_code', { query: `b${i}` }));
                expect(isLoop).toBe(false);
            }
        });
    });
