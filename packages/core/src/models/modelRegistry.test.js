...
import { ModelRegistry, QWEN_OAUTH_MODELS, modelRegistryKey, } from './modelRegistry.js';
...
describe('ModelRegistry', () => {
    describe('initialization', () => {
        ...
        it('should initialize with empty config', () => {
            const registry = new ModelRegistry();
            expect(registry.getModelsForAuthType(AuthType.QWEN_OAUTH).length).toBe(QWEN_OAUTH_MODELS.length);
    ...
    describe('getDefaultModelForAuthType', () => {
        it('should return coder-model for qwen-oauth', () => {
            const registry = new ModelRegistry();
            const defaultModel = registry.getDefaultModelForAuthType(AuthType.QWEN_OAUTH);
    ...
    describe('duplicate model id handling', () => {
        ...
        it('should retrieve model by id and baseUrl precisely', () => {
            ...
            const direct = registry.getModel(AuthType.USE_OPENAI, 'gpt-4', 'https://api.openai.com/v1');
            expect(direct?.name).toBe('GPT-4 Direct');
            const proxy = registry.getModel(AuthType.USE_OPENAI, 'gpt-4', 'https://proxy.example.com/v1');
        ...
        it('should handle hasModel with and without baseUrl', () => {
            ...
            expect(registry.hasModel(AuthType.USE_OPENAI, 'gpt-4')).toBe(true);
            expect(registry.hasModel(AuthType.USE_OPENAI, 'gpt-4', 'https://api.openai.com/v1')).toBe(true);
            expect(registry.hasModel(AuthType.USE_OPENAI, 'gpt-4', 'https://proxy.example.com/v1')).toBe(true);
            expect(registry.hasModel(AuthType.USE_OPENAI, 'gpt-4', 'https://unknown.example.com/v1')).toBe(false);
        });
    ...
    describe('reloadModels', () => {
        ...
        it('should preserve hard-coded qwen-oauth models after reload', () => {
            ...
            expect(registry.getModelsForAuthType(AuthType.QWEN_OAUTH).length).toBe(QWEN_OAUTH_MODELS.length);
            expect(registry.getModel(AuthType.QWEN_OAUTH, 'coder-model')).toBeDefined();
        });
        ...
        it('should correctly reload same-id different-baseUrl models', () => {
            ...
            const models = registry.getModelsForAuthType(AuthType.USE_OPENAI);
            expect(models.length).toBe(2);
            expect(registry.getModel(AuthType.USE_OPENAI, 'gpt-4', 'https://api.openai.com/v1')?.name).toBe('New Direct');
            expect(registry.getModel(AuthType.USE_OPENAI, 'gpt-4', 'https://proxy.example.com/v1')?.name).toBe('New Proxy');
        });
        ...
        it('should handle reload replacing same-id entries when baseUrls change', () => {
            ...
            const models = registry.getModelsForAuthType(AuthType.USE_OPENAI);
            expect(models.length).toBe(2);
            expect(registry.getModel(AuthType.USE_OPENAI, 'gpt-4', 'https://old-proxy.example.com/v1')).toBeUndefined();
            expect(registry.getModel(AuthType.USE_OPENAI, 'gpt-4', 'https://new-proxy.example.com/v1')?.name).toBe('GPT-4 New Proxy');
        });
