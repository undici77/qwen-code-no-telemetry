...
describe('PromptHookRunner', () => {
    ...
    const createMockResponse = (text, overrides = {}) => ({
        candidates: [
            {
                content: {
                    parts: [{ text }],
    ...
    describe('execute', () => {
        it('should execute prompt hook successfully with ok:true', async () => {
            ...
            const result = await promptRunner.execute(config, HookEventName.PreToolUse, input);
        ...
        it('should execute prompt hook with blocking decision ok:false', async () => {
            const mockResponse = createMockResponse('{"ok": false, "reason": "Security violation"}');
            mockGenerateContent.mockResolvedValue(mockResponse);
            const config = createMockConfig();
            const input = createMockInput();
            const result = await promptRunner.execute(config, HookEventName.PreToolUse, input);
        ...
        it('should handle response with additionalContext', async () => {
            const mockResponse = createMockResponse('{"ok": true, "additionalContext": "Some useful info"}');
            mockGenerateContent.mockResolvedValue(mockResponse);
            const config = createMockConfig();
            const input = createMockInput();
            const result = await promptRunner.execute(config, HookEventName.PreToolUse, input);
            expect(result.success).toBe(true);
            expect(result.output?.hookSpecificOutput?.['additionalContext']).toBe('Some useful info');
        });
        it('should handle blocking response with additionalContext', async () => {
            const mockResponse = createMockResponse('{"ok": false, "reason": "Blocked", "additionalContext": "Context info"}');
            mockGenerateContent.mockResolvedValue(mockResponse);
            const config = createMockConfig();
            const input = createMockInput();
            const result = await promptRunner.execute(config, HookEventName.PreToolUse, input);
            ...
            expect(result.output?.hookSpecificOutput?.['additionalContext']).toBe('Context info');
        });
        ...
        it('should treat truncated MAX_TOKENS response as non-blocking error', async () => {
            ...
            expect(result.outcome).toBe('non_blocking_error');
            expect(result.output?.continue).toBe(true);
            expect(result.error?.message).toContain('Response truncated due to token limit');
        });
        ...
        it('should handle abort signal (already aborted)', async () => {
            ...
            const config = createMockConfig();
            const input = createMockInput();
            const result = await promptRunner.execute(config, HookEventName.PreToolUse, input, controller.signal);
            expect(result.success).toBe(false);
            expect(result.outcome).toBe('cancelled');
            expect(mockGenerateContent).not.toHaveBeenCalled();
        });
        it('should include hook name in error when blocking', async () => {
            const mockResponse = createMockResponse('{"ok": false, "reason": "Blocked"}');
            mockGenerateContent.mockResolvedValue(mockResponse);
        ...
        it('should track duration correctly', async () => {
            ...
            const input = createMockInput();
            try {
                const execution = promptRunner.execute(config, HookEventName.PreToolUse, input);
...
//# sourceMappingURL=promptHookRunner.test.js.map