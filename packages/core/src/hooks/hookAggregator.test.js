...
describe('HookAggregator', () => {
    ...
    describe('Todo events - mergeWithOrLogic', () => {
        it('should block TodoCreated when any hook blocks', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.TodoCreated);
            expect(result.finalOutput?.decision).toBe('block');
        ...
        it('should block TodoCompleted when a later hook allows', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.TodoCompleted);
            expect(result.finalOutput?.decision).toBe('block');
            expect(result.finalOutput?.reason).toBe('already completed elsewhere\ncompletion approved');
        });
    });
