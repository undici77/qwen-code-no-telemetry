...
import type { ContextUsageData, DefaultHookOutput, HookPhase } from './types.js';
import { PermissionMode } from './types.js';
import type { SessionStartSource, SessionEndReason, AgentType, PostToolBatchToolCall, PreCompactTrigger, PostCompactTrigger, NotificationType, PermissionDeniedReason, PermissionSuggestion, HookEventName, FunctionHookCallback, CommandHookConfig, HttpHookConfig, PendingAsyncHook, PendingAsyncOutput, MessagesProvider, StopFailureErrorType, TodoItem, TodoStatus, InstructionMemoryType, InstructionLoadReason } from './types.js';
...
export declare class HookSystem {
    ...
    fireUserPromptSubmitEvent(prompt: string, signal?: AbortSignal, submittedPrompt?: string): Promise<DefaultHookOutput | undefined>;
    fireInstructionsLoadedEvent(filePath: string, memoryType: InstructionMemoryType, loadReason: InstructionLoadReason, options?: {
        triggerFilePath?: string;
        parentFilePath?: string;
    }, signal?: AbortSignal): Promise<DefaultHookOutput | undefined>;
    /**
     ...
     */
    fireUserPromptExpansionEvent(commandName: string, commandArgs: string, prompt: string, signal?: AbortSignal): Promise<DefaultHookOutput | undefined>;
    fireStopEvent(stopHookActive?: boolean, lastAssistantMessage?: string, contextUsage?: ContextUsageData, signal?: AbortSignal): Promise<AggregatedHookResult>;
    /**
     * Fire a MessageDisplay event - called repeatedly as the assistant's reply streams
     */
    fireMessageDisplayEvent(messageId: string, displayedText: string, isFinal: boolean, signal?: AbortSignal): Promise<AggregatedHookResult>;
    ...
    fireSessionDeleteEvent(deletedSessionId: string, signal?: AbortSignal): Promise<DefaultHookOutput | undefined>;
    /**
     * Fire a PreToolUse event - called before tool execution
     */
    firePreToolUseEvent(toolName: string, toolInput: Record<string, unknown>, toolUseId: string, permissionMode: PermissionMode, signal?: AbortSignal, tool_call_id?: string): Promise<DefaultHookOutput | undefined>;
    /**
     * Fire a PostToolUse event - called after successful tool execution
     */
    firePostToolUseEvent(toolName: string, toolInput: Record<string, unknown>, toolResponse: Record<string, unknown>, toolUseId: string, permissionMode: PermissionMode, signal?: AbortSignal, tool_call_id?: string): Promise<DefaultHookOutput | undefined>;
    /**
     * Fire a PostToolUseFailure event - called when tool execution fails
     */
    firePostToolUseFailureEvent(toolUseId: string, toolName: string, toolInput: Record<string, unknown>, errorMessage: string, isInterrupt?: boolean, permissionMode?: PermissionMode, signal?: AbortSignal, tool_call_id?: string): Promise<DefaultHookOutput | undefined>;
    /**
     * Fire a PostToolBatch event - called once after a tool-call batch resolves
     */
    firePostToolBatchEvent(toolCalls: PostToolBatchToolCall[], permissionMode?: PermissionMode, signal?: AbortSignal): Promise<DefaultHookOutput | undefined>;
    /**
     ...
     */
    firePermissionDeniedEvent(toolName: string, toolInput: Record<string, unknown>, toolUseId: string, reason: PermissionDeniedReason, signal?: AbortSignal, tool_call_id?: string): Promise<DefaultHookOutput | undefined>;
    /**
