...
Skill body.
`;
            const config = manager.parseSkillContent(markdownWithPriority, validSkillConfig.filePath, 'project');
...
Skill body.
`;
            const config = manager.parseSkillContent(markdownWithInvalidPriority, validSkillConfig.filePath, 'project');
...
`;
    ...
    describe('listSkills', () => {
        beforeEach(() => {
            ...
            const projectQwenSkillsDir = path.join('/test/project', '.qwen', 'skills');
...
Skill 3 content`);
        ...
        it('should return a stable alphabetical order regardless of priority (priority only affects the /skills display layer)', async () => {
            ...
            mockParseYaml.mockImplementation((yamlString) => yaml.parse(yamlString));
            const projectQwenSkillsDir = path.join('/test/project', '.qwen', 'skills');
            vi.mocked(fs.readdir).mockImplementation((dirPath) => {
                if (String(dirPath) === projectQwenSkillsDir) {
                    return Promise.resolve(['high', 'unset-beta', 'unset-alpha', 'negative'].map((name) => ({
                        name,
                        isDirectory: () => true,
                        isFile: () => false,
                        isSymbolicLink: () => false,
...
`);
    ...
    describe('symlink support', () => {
        it('should load skills from symlinked directories', async () => {
            ...
            vi.mocked(fs.realpath).mockResolvedValue('/elsewhere/skills-repo/symlink-skill');
