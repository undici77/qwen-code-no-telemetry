...
describe('validateSymlinkTarget', () => {
    ...
    it('accepts a target that resolves outside the skills directory', async () => {
        ...
        vi.mocked(fs.realpath).mockResolvedValue('/Users/me/projects/skills-repo/skills/auto-pr');
        vi.mocked(fs.stat).mockResolvedValue({
            isDirectory: () => true,
        });
        const result = await validateSymlinkTarget('/Users/me/.qwen/skills/auto-pr');
        expect(result.ok).toBe(true);
    });
