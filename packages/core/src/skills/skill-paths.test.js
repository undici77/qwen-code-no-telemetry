...
import { assertProjectSkillPath, assertRealProjectSkillPath, getProjectSkillsRoot, isProjectSkillPath, sanitizeSkillName, } from './skill-paths.js';
describe('skill project paths', () => {
    const projectRoot = '/tmp/project';
    it('resolves the project skills root', () => {
        expect(getProjectSkillsRoot(projectRoot)).toBe(path.join(projectRoot, '.qwen', 'skills'));
    });
    it('allows paths inside project .qwen/skills', () => {
        const skillPath = path.join(projectRoot, '.qwen', 'skills', 'my-skill', 'SKILL.md');
    ...
    it('rejects sibling paths that merely share the prefix', () => {
        ...
        expect(() => assertProjectSkillPath(sibling, projectRoot)).toThrow('Skills writes are restricted to');
    });
...
describe('assertRealProjectSkillPath – symlink traversal', () => {
    ...
    it('accepts a legitimate path inside skills dir', async () => {
        ...
        await expect(assertRealProjectSkillPath(target, projectRoot)).resolves.toBeUndefined();
    });
    it('rejects a path whose parent is a symlink pointing outside skills dir', async () => {
        ...
        await expect(assertRealProjectSkillPath(target, projectRoot)).rejects.toThrow('symlink traversal detected');
    });
...
//# sourceMappingURL=skill-paths.test.js.map