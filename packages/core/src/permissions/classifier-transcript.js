 ...
 */
export function buildClassifierContents(messages, toolRegistry, pendingAction) {
    const transcript = [];
    ...
    for (const msg of recent) {
        if (msg.role === 'user') {
            const textParts = (msg.parts ?? []).filter((p) => typeof p.text === 'string');
            ...
            for (const part of msg.parts ?? []) {
                ...
                if (fc && typeof fc.name === 'string') {
                    transcript.push({
                        ...
                        parts: [
                            {
                                text: formatHistoricalActionPrompt(fc.name, fc.args, toolRegistry),
                            },
                        ],
                    });
                }
    ...
    transcript.push({
        ...
        parts: [
            {
                text: formatPendingActionPrompt(pendingAction.toolName, pendingAction.toolParams, toolRegistry),
            },
        ],
    });
    return transcript;
 ...
 */
function formatHistoricalActionPrompt(toolName, toolArgs, toolRegistry) {
 ...
 * still redacted.
 */
function formatPendingActionPrompt(toolName, toolParams, toolRegistry) {
 ...
 */
function projectFunctionArgs(name, args, toolRegistry) {
    const tool = toolRegistry.getTool(name);
    const rawArgs = args && typeof args === 'object' ? args : {};
    let projected;
    if (tool) {
        try {
            projected = tool.toAutoClassifierInput(rawArgs);
        }
        catch {
            projected = undefined;
        }
...
//# sourceMappingURL=classifier-transcript.js.map