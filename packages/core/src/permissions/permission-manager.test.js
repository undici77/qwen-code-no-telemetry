...
describe('matchesRule', () => {
    ...
    it('Edit path specifier matches notebook_edit too', async () => {
        ...
        const pathCtx = { projectRoot: '/project', cwd: '/project' };
        expect(matchesRule(rule, 'notebook_edit', undefined, '/project/src/analysis.ipynb', undefined, pathCtx)).toBe(true);
        expect(matchesRule(rule, 'notebook_edit', undefined, '/project/docs/analysis.ipynb', undefined, pathCtx)).toBe(false);
    });
...
describe('PermissionManager — strip/restore for AUTO mode', () => {
    it('strips Bash interpreter wildcards and stashes them', () => {
        ...
        return expect(pm.evaluate({ toolName: 'run_shell_command', command: 'git status' })).resolves.toBe('allow');
    });
    it('strips bare tool-level Bash allow', async () => {
        const pm = new PermissionManager(makeConfig({ permissionsAllow: ['Bash'] }));
        pm.initialize();
    ...
    it('strips Agent / Skill any-allow rules', () => {
        ...
        expect(stash.persistent.map((r) => r.toolName).sort()).toEqual(['agent', 'skill'].sort());
    ...
    it('is idempotent — second strip returns the same stash without re-removal', () => {
        const pm = new PermissionManager(makeConfig({ permissionsAllow: ['Bash(python:*)'] }));
        pm.initialize();
    ...
    it('never strips deny rules — user intent for deny is honored', () => {
        ...
        return expect(pm.evaluate({ toolName: 'run_shell_command', command: 'git log' })).resolves.toBe('deny');
    });
