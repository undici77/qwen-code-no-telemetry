...
import { type AutoModeDenialState, type DenialFallbackReason } from './denialTracking.js';
 ...
 */
export declare function shouldClassifyAllShellForAutoMode(toolName: string, config: Config): boolean;
/**
 ...
 */
export declare function shouldForceAutoModeReviewForAllow(ctx: PermissionCheckContext, cwdFallback?: string): boolean;
/**
 ...
 */
export type AutoModeUnavailableReason = 'circuit-breaker' | 'disabled' | 'policy';
/**
 ...
 */
export type FallbackToAskReason = 'safety_check' | 'ask_rule' | 'plan_mode_floor' | 'org_ask_ceiling' | 'classifier_unavailable' | DenialFallbackReason;
...
export declare function shouldFirePermissionDeniedForAutoMode(decision: AutoModeDecision, outcome: AutoModeOutcome): decision is Extract<AutoModeDecision, {
    via: 'classifier';
}>;
export declare function getAutoModePermissionDeniedReason(decision: Extract<AutoModeDecision, {
    via: 'classifier';
 ...
 */
export declare const AUTO_MODE_DENIAL_GUIDANCE = "Do not try to complete the denied action through another tool, shell indirection, generated script, alias, symlink, config change, hook, command file, MCP configuration, encoded payload, or equivalent path. If that action is required, stop and ask the user for explicit approval. You may continue with unrelated safe work or a genuinely safer alternative that does not accomplish the denied action.";
export declare function formatClassifierUnavailableFallbackMessage(decision: Extract<AutoModeDecision, {
    via: 'classifier';
}>): string;
export declare function decorateClassifierUnavailableConfirmation(confirmation: ToolCallConfirmationDetails, message: string): ToolCallConfirmationDetails;
/**
