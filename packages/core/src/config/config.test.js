...
import { Config, ApprovalMode, APPROVAL_MODES, APPROVAL_MODE_INFO, MCPServerConfig, } from './config.js';
import { Storage } from './storage.js';
import * as fs from 'node:fs';
import * as path from 'node:path';
import { setGeminiMdFilename as mockSetGeminiMdFilename } from '../memory/const.js';
import { DEFAULT_TELEMETRY_TARGET, DEFAULT_OTLP_ENDPOINT, QwenLogger, isTelemetrySdkInitialized, shutdownTelemetry, refreshSessionContext, } from '../telemetry/index.js';
import { DEFAULT_DASHSCOPE_BASE_URL } from '../core/openaiContentGenerator/constants.js';
import { createContentGenerator, createContentGeneratorConfig, resolveContentGeneratorConfigWithSources, } from '../core/contentGenerator.js';
...
vi.mock('../tools/tool-registry', () => {
    ...
    ToolRegistryMock.mockImplementation(function () {
    ...
    ToolRegistryMock.prototype.getMcpClientManager = function () {
...
describe('Server Config (config.ts)', () => {
    ...
    beforeEach(() => {
        ...
        fs.copyFileSync.mockImplementation(() => undefined);
    ...
    describe('initialize', () => {
        ...
        it('should skip implicit startup discovery in bare mode', async () => {
            ...
            expect(ToolRegistry.prototype.registerFactory.mock.calls.map((call) => call[0])).toEqual([
                ToolNames.READ_FILE,
                ToolNames.EDIT,
        ...
        it('honors QWEN_CODE_LEGACY_MCP_BLOCKING=1 by running MCP discovery inline', async () => {
            ...
            try {
                ...
                expect(ToolRegistry.prototype.discoverAllTools).toHaveBeenCalledTimes(1);
            }
            finally {
    ...
    describe('Usage Statistics', () => {
        ...
        it.each([{ enabled: true }, { enabled: false }])('always returns false for usage statistics (input: $enabled)', ({ enabled }) => {
            const config = new Config({
                ...baseParams,
    ...
    describe('GitCoAuthor Settings', () => {
        ...
        it.each([true, false])('coerces legacy boolean %s to { commit, pr } with the same value', (value) => {
        ...
        ])('parses %s as %s for both commit and pr', (_label, badValue, expected) => {
            const config = new Config({
                ...baseParams,
                gitCoAuthor: {
                    commit: badValue,
    ...
    describe('Telemetry Settings', () => {
        ...
        it('should default includeSensitiveSpanAttributes to false', () => {
            ...
            expect(configWithTelemetry.getTelemetryIncludeSensitiveSpanAttributes()).toBe(false);
            const paramsWithoutTelemetry = { ...baseParams };
            delete paramsWithoutTelemetry.telemetry;
            const configWithoutTelemetry = new Config(paramsWithoutTelemetry);
            expect(configWithoutTelemetry.getTelemetryIncludeSensitiveSpanAttributes()).toBe(false);
        });
    ...
    describe('createToolRegistry', () => {
        it('should ignore coreTools overrides in bare mode', async () => {
            ...
            expect(registerToolMock.mock.calls.map((call) => call[0])).toEqual([
                ToolNames.READ_FILE,
                ToolNames.EDIT,
                ToolNames.NOTEBOOK_EDIT,
                ToolNames.SHELL,
    ...
    describe('setMcpBudgetEventCallback handoff to McpClientManager', () => {
        it('applies pending callback when registry is created during initialize()', async () => {
            ...
            expect(registry.__mcpManagerMock.setOnBudgetEvent).toHaveBeenCalledWith(cb);
            // Exactly once — the apply path fires only once per
            // `createToolRegistry` invocation.
            expect(registry.__mcpManagerMock.setOnBudgetEvent.mock.calls).toHaveLength(1);
        });
        it('applies callback directly to existing manager when called after initialize()', async () => {
            ...
            expect(registry.__mcpManagerMock.setOnBudgetEvent).toHaveBeenLastCalledWith(undefined);
        });
        it('does NOT stash the callback when called after initialize() (codex round 7 fix — subagent isolation)', async () => {
            ...
            expect(subagentRegistry.__mcpManagerMock.setOnBudgetEvent).not.toHaveBeenCalled();
        });
    });
});
describe('setApprovalMode with folder trust', () => {
    ...
    describe('AUTO mode', () => {
        it('should throw an error when setting AUTO mode in an untrusted folder', () => {
            const config = new Config(baseParams);
            vi.spyOn(config, 'isTrustedFolder').mockReturnValue(false);
            expect(() => config.setApprovalMode(ApprovalMode.AUTO)).toThrow('Cannot enable privileged approval modes in an untrusted folder.');
        });
    ...
    describe('plan file persistence', () => {
        it('should save plan to disk atomically', () => {
            ...
            expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('.tmp'), '# My Plan\n1. Step one\n2. Step two', 'utf-8');
            // Then atomically renames to final path
            expect(fs.renameSync).toHaveBeenCalledWith(expect.stringContaining('.tmp'), expect.stringContaining('.md'));
        });
        ...
        it('should sanitize session ID when building plan file path', () => {
            ...
            expect(config.getPlanFilePath()).toBe(path.join(path.resolve(baseParams.targetDir), 'project-plans', 'escape.md'));
        });
        it('should use configured plansDirectory for plan file path', () => {
            ...
            expect(config.getPlansDir()).toBe(path.join(path.resolve(baseParams.targetDir), 'project-plans'));
            expect(config.getPlanFilePath()).toBe(path.join(path.resolve(baseParams.targetDir), 'project-plans', 'test-session-123.md'));
        });
        it('should save and load plan from configured plansDirectory', () => {
            ...
            fs.writeFileSync.mockImplementation((pathToWrite, contents) => {
                storedFiles.set(pathToWrite.toString(), contents.toString());
            });
            fs.renameSync.mockImplementation((fromPath, toPath) => {
            ...
            fs.readFileSync.mockImplementation((pathToRead) => {
            ...
            expect(fs.writeFileSync).toHaveBeenCalledWith(tmpPath, '# My Plan', 'utf-8');
        ...
        it('should fall back to copyFileSync when renameSync hits EXDEV', () => {
            ...
            config.savePlan('# My Plan');
            expect(fs.copyFileSync).toHaveBeenCalledWith(expect.stringContaining('.tmp'), expect.stringContaining('project-plans'));
        ...
        it('should remove plan file when post-write containment check fails', () => {
            ...
            const plansDir = path.join(targetDir, 'project-plans');
            const filePath = path.join(plansDir, 'test-session-123.md');
            const outsideFilePath = path.resolve(path.dirname(targetDir), 'outside-plans', 'test-session-123.md');
            ...
            try {
                expect(() => config.savePlan('# My Plan')).toThrow('plansDirectory must resolve within the project root');
        ...
        it('should reject loading a plan when final file path escapes targetDir', () => {
            ...
            try {
                expect(() => config.loadPlan()).toThrow('plansDirectory must resolve within the project root');
        ...
        it('should warn when configured plansDirectory hides a legacy plan file', () => {
            ...
            fs.readdirSync.mockImplementation((pathToCheck) => {
            ...
            try {
                ...
                expect(config.getWarnings()).toContainEqual(expect.stringContaining(legacyPlansDir));
                expect(config.getWarnings()).toContainEqual(expect.stringContaining('plansDirectory is configured'));
            }
            finally {
                fs.readdirSync.mockReturnValue([]);
            }
        ...
        it('should surface legacy plan directory read failures as warnings', () => {
            const legacyError = new Error('EACCES');
            ...
            try {
                ...
                expect(config.getWarnings()).toContainEqual(expect.stringContaining('Failed to read plan directory'));
            }
            finally {
                fs.readdirSync.mockReturnValue([]);
            }
        ...
        it('should revalidate configured plansDirectory before plan I/O', () => {
            ...
            const targetDir = path.resolve(baseParams.targetDir);
            const plansDir = path.join(targetDir, 'project-plans');
            const outsidePlansDir = path.resolve(path.dirname(targetDir), 'outside-plans');
            vi.mocked(fs.realpathSync).mockImplementation((pathToResolve) => {
                const resolvedPath = pathToResolve.toString();
...
describe('Model Switching and Config Updates', () => {
    ...
    describe('hasHooksForEvent', () => {
        ...
        it('should delegate to hookSystem.hasHooksForEvent when hookSystem exists', () => {
            ...
            expect(mockHasHooksForEvent).toHaveBeenCalledWith('UserPromptSubmit', expect.any(String));
        });
        it('should return false when hookSystem has no hooks for the event', () => {
            ...
            config['hookSystem'] = mockHookSystem;
            expect(config.hasHooksForEvent('Stop')).toBe(false);
            expect(mockHasHooksForEvent).toHaveBeenCalledWith('Stop', expect.any(String));
        });
    });
    describe('runtime ContentGenerator view (AsyncLocalStorage)', () => {
        ...
        function setInstanceFields(config, contentGenerator, generatorConfig) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            config.contentGenerator = contentGenerator;
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            config.contentGeneratorConfig = generatorConfig;
        }
        it('resolves getters to the runtime view inside the frame, instance fields outside', async () => {
            const { runWithRuntimeContentGenerator } = await import('../agents/runtime/agent-context.js');
            const config = new Config(baseParams);
            const parentGenerator = {
                generateContentStream: vi.fn(),
            };
            const parentGeneratorConfig = {
                model: 'parent-model',
                authType: AuthType.QWEN_OAUTH,
                apiKey: 'parent-key',
            };
        ...
        it('falls back to the parent model id when the runtime view config has no model', async () => {
            const { runWithRuntimeContentGenerator } = await import('../agents/runtime/agent-context.js');
            const config = new Config(baseParams);
            setInstanceFields(config, { generateContentStream: vi.fn() }, {
                model: 'parent-model',
                authType: AuthType.QWEN_OAUTH,
            });
            await runWithRuntimeContentGenerator({
