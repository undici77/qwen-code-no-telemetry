...
import { generatePromptSuggestion, shouldFilterSuggestion, } from './suggestionGenerator.js';
...
describe('generatePromptSuggestion', () => {
    ...
    it('passes cache-safe model in cache mode when no explicit or fast model exists', async () => {
        ...
        await generatePromptSuggestion(config, conversationHistory, new AbortController().signal, { enableCacheSharing: true });
        expect(mockRunForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ model: 'main-model' }));
    });
    it('passes the fast model in cache mode when one is configured', async () => {
        ...
        await generatePromptSuggestion(config, conversationHistory, new AbortController().signal, { enableCacheSharing: true });
        expect(mockRunForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ model: 'openai:fast-model' }));
    });
});
