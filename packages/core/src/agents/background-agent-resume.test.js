...
describe('BackgroundAgentResumeService', () => {
    ...
    it('appends a warning when resumed SubagentStop hooks reach the blocking cap', async () => {
        ...
        expect(registry.get(agentId)?.result).toContain('SubagentStop hook blocked continuation 2 consecutive times; overriding and ending the turn.');
    });
    ...
    it('routes owned monitor notifications into a resumed agent queue', async () => {
        ...
        await vi.waitFor(() => {
            expect(monitorRegistry.setAgentNotificationCallback).toHaveBeenCalledWith(agentId, expect.any(Function));
        });
        ...
        const waitPromise = registry.waitForMessages(agentId, new AbortController().signal);
        lifecycleCallback();
        ...
        await vi.waitFor(() => {
            ...
            expect(monitorRegistry.cancelRunningForOwner).toHaveBeenCalledWith(agentId, {
                notify: false,
            });
        });
    });
    it('cleans up owned monitor callbacks when resume setup fails before execution', async () => {
        ...
        const { service, subagentManager, monitorRegistry } = createService();
        subagentManager.createAgentHeadless.mockResolvedValue(subagent);
        await expect(service.resumeBackgroundAgent(agentId, 'continue')).resolves.toBeUndefined();
        expect(subagent.execute).not.toHaveBeenCalled();
        expect(monitorRegistry.setAgentNotificationCallback).toHaveBeenCalledWith(agentId, expect.any(Function));
        expect(monitorRegistry.setAgentLifecycleCallback).toHaveBeenCalledWith(agentId, expect.any(Function));
        expect(monitorRegistry.setAgentNotificationCallback).toHaveBeenCalledWith(agentId, undefined);
        expect(monitorRegistry.setAgentLifecycleCallback).toHaveBeenCalledWith(agentId, undefined);
        expect(monitorRegistry.cancelRunningForOwner).toHaveBeenCalledWith(agentId, {
            notify: false,
