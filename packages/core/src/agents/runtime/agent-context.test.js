...
describe('agent-context (runtimeView)', () => {
    ...
    it('exposes the view to code running inside the frame', async () => {
        ...
        const inner = await runWithRuntimeContentGenerator(view, async () => getRuntimeContentGenerator());
    ...
    it('isolates sibling runs', async () => {
        ...
        const [seen1, seen2] = await Promise.all([
            runWithRuntimeContentGenerator(v1, async () => getRuntimeContentGenerator()),
            runWithRuntimeContentGenerator(v2, async () => getRuntimeContentGenerator()),
        ]);
    ...
    it('lets a nested run shadow the outer view', async () => {
        ...
        const [seenOuter, seenInner] = await runWithRuntimeContentGenerator(outer, async () => {
            const before = getRuntimeContentGenerator();
            const after = await runWithRuntimeContentGenerator(inner, async () => getRuntimeContentGenerator());
            return [before, after];
        });
