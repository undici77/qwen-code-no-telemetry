    ...
    describe('AgentHeadless', () => {
        ...
        describe('execute - Execution and Tool Use', () => {
            ...
            it('should wait for external notification after a no-tool response', async () => {
                const { config } = await createMockConfig();
                mockSendMessageStream.mockImplementation(createMockStream(['stop', 'stop']));
                let resolveWait;
                const waitForExternalMessages = vi.fn((_signal) => new Promise((resolve) => {
                    resolveWait = resolve;
                }));
                let shouldWait = true;
                const scope = await AgentHeadless.create('test-agent', config, promptConfig, defaultModelConfig, defaultRunConfig);
                ...
                await vi.waitFor(() => expect(waitForExternalMessages).toHaveBeenCalled());
                shouldWait = false;
                resolveWait?.([
                    {
                        kind: 'notification',
            ...
            it('should not idle-wait when max turns prevents another round', async () => {
                const { config } = await createMockConfig();
                const runConfig = { ...defaultRunConfig, max_turns: 1 };
                const waitForExternalMessages = vi.fn(async () => []);
                mockSendMessageStream.mockImplementation(createMockStream(['stop']));
