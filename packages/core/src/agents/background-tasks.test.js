...
import { BackgroundTaskRegistry, MAX_RETAINED_TERMINAL_AGENTS, } from './background-tasks.js';
...
describe('BackgroundTaskRegistry', () => {
    ...
    describe('terminal-entry retention cap', () => {
        function makeRegisteredEntry(id, startTime) {
            return {
                agentId: id,
                description: id,
                status: 'running',
        ...
        it('retains only a bounded number of fully-finalized terminal entries', () => {
            ...
            expect(registry.get(`a-${MAX_RETAINED_TERMINAL_AGENTS + 1}`)).toBeDefined();
        });
    ...
    describe('waitForMessages', () => {
        it('resolves with queued input when a running agent is notified', async () => {
            ...
            const waitPromise = registry.waitForMessages('test-1', new AbortController().signal);
    ...
    describe('foreground flavor', () => {
        ...
        it('unregisterForeground throws if asked to remove a background entry', () => {
            ...
            expect(() => registry.unregisterForeground('bg-1')).toThrow(/non-foreground entry bg-1/);
        ...
        it('unregisterForeground emits status change after removing the entry', () => {
            ...
            let observedFromCallback;
            let snapshotDuringCallback = [];
            registry.setStatusChangeCallback((entry) => {
