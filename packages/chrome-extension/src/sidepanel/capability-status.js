...
export function deriveCapabilityStatus(daemonReachable, features, mcpSnapshot, baseUrl) {
    if (!daemonReachable) {
    ...
    if (mcpSnapshot) {
        ...
        if ((mcpSnapshot.initialized === false ||
        ...
        const server = mcpSnapshot.servers?.find((candidate) => candidate.name === CHROME_DEVTOOLS_SERVER_NAME);
        if (!server) {
            return {
        ...
        const usesTunnel = server.config?.args?.some((arg) => {
            ...
            if (!CDP_TUNNEL_ENDPOINT_PATTERN.test(candidate))
                return false;
            if (!baseUrl)
                return true;
            try {
                ...
                return (loopback(argUrl.hostname) === loopback(daemonUrl.hostname) &&
...
//# sourceMappingURL=capability-status.js.map