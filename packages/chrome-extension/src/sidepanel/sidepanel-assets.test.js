...
describe('side panel capability status assets', () => {
    ...
    it('loads the generated capability model before the panel host', () => {
        const html = readFileSync(path.join(packageRoot, 'public/sidepanel.html'), 'utf8');
        expect(html).toContain('src="sidepanel/capability-status.js"');
        expect(html.indexOf('sidepanel/capability-status.js')).toBeLessThan(html.indexOf('src="sidepanel.js"'));
    });
    ...
    it('requests only permissions used by the extension', () => {
        const manifest = JSON.parse(readFileSync(path.join(packageRoot, 'public/manifest.json'), 'utf8'));
    ...
    `;
        ...
        let poll;
        vi.stubGlobal('setInterval', (handler) => {
            poll = handler;
            return 1;
        });
        const script = readFileSync(path.join(packageRoot, 'public/sidepanel.js'), 'utf8');
        Function(script)();
        await vi.waitFor(() => expect(document.getElementById('welcome-title')?.textContent).toBe('Start qwen serve'));
        daemonState = 'chat-only';
        await poll?.();
        expect(document.getElementById('ui')?.classList.contains('hidden')).toBe(false);
        expect(document.getElementById('capability-warning')?.textContent).toBe('Browser bridge is disabled for this daemon.');
        daemonState = 'down';
        await poll?.();
        await poll?.();
        await poll?.();
    ...
    `;
        ...
        await vi.waitFor(() => expect(document.getElementById('ui')?.classList.contains('hidden')).toBe(false));
    ...
    `;
        ...
        vi.stubGlobal('fetch', vi.fn(async (input) => {
            ...
            if (url.endsWith('/workspace/mcp')) {
                return {
                    ok: mcpResponse.ok,
                    json: async () => mcpResponse.ok ? mcpResponse.value : { error: 'unavailable' },
                };
            }
        ...
        const pollUntil = (assertion) => vi.waitFor(async () => {
            await poll?.();
            assertion();
        });
        await vi.waitFor(() => expect(document.getElementById('capability-warning')?.textContent).toBe('Browser tools status could not be verified.'));
        mcpResponse = {
            ok: true,
            value: { initialized: false, discoveryState: 'not_started', servers: [] },
        };
        await pollUntil(() => expect(document.getElementById('capability-warning')?.textContent).toBe('Browser tools status is unknown until a chat session starts.'));
        mcpResponse = { ok: true, value: { servers: [] } };
        await pollUntil(() => expect(document.getElementById('capability-warning')?.textContent).toBe('Browser tools are configured but the adapter is not connected.'));
        mcpResponse = {
            ok: true,
            value: {
                servers: [
        ...
        await pollUntil(() => expect(document.getElementById('capability-warning')?.textContent).toBe('An existing chrome-devtools MCP configuration is taking precedence. Disable or rename it to use the extension tunnel.'));
        mcpResponse = {
            ok: true,
            value: {
                servers: [
        ...
        await pollUntil(() => expect(document
            .getElementById('capability-warning')
            ?.classList.contains('hidden')).toBe(true));
    });
    ...
    `;
        ...
        const script = readFileSync(path.join(packageRoot, 'public/sidepanel.js'), 'utf8');
        Function(script)();
        await vi.waitFor(() => expect(document.getElementById('capability-warning')?.textContent).toBe('Browser tools are configured but the adapter is not connected.'));
        mcpOk = false;
        // Drive past the next MCP re-probe tick (MCP_POLL_EVERY = 5).
        for (let i = 0; i < 6; i++)
            await poll?.();
        expect(document.getElementById('capability-warning')?.textContent).toBe('Browser tools are configured but the adapter is not connected.');
    });
    ...
    `;
        ...
        await vi.waitFor(async () => {
            await poll?.();
            expect(document.getElementById('capability-warning')?.textContent).toBe('Browser tools status could not be verified.');
        });
    });
});
//# sourceMappingURL=sidepanel-assets.test.js.map