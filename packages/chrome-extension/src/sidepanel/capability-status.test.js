...
import { CDP_TUNNEL_ENDPOINT_PATTERN, CHROME_DEVTOOLS_SERVER_NAME, deriveCapabilityStatus, } from './capability-status.js';
describe('deriveCapabilityStatus', () => {
    ...
    it('warns when the CDP tunnel is ready without an automation adapter', () => {
        expect(deriveCapabilityStatus(true, ['allow_origin', 'cdp_tunnel_over_ws'])).toEqual({
            state: 'tunnel-only',
            shellReady: true,
            warning: 'Browser tools are unavailable. They require QWEN_CDP_MCP_COMMAND and an auth-free loopback daemon.',
        });
    });
    it('reports configured, not pending, when the ACP child has not started discovery', () => {
        expect(deriveCapabilityStatus(true, ['allow_origin', 'cdp_tunnel_over_ws', 'browser_automation_mcp'], { initialized: false, discoveryState: 'not_started', servers: [] })).toEqual({
    ...
    it('reports configured when the MCP snapshot argument is omitted', () => {
        expect(deriveCapabilityStatus(true, [
            'allow_origin',
            'cdp_tunnel_over_ws',
    ...
    it('warns when runtime MCP status cannot be read', () => {
        expect(deriveCapabilityStatus(true, ['allow_origin', 'cdp_tunnel_over_ws', 'browser_automation_mcp'], null)).toEqual({
    ...
    it('reports a connected runtime MCP that targets the extension tunnel', () => {
        expect(deriveCapabilityStatus(true, ['allow_origin', 'cdp_tunnel_over_ws', 'browser_automation_mcp'], {
            servers: [
                {
    ...
    it('warns while the configured runtime MCP is not connected', () => {
        expect(deriveCapabilityStatus(true, ['allow_origin', 'cdp_tunnel_over_ws', 'browser_automation_mcp'], { servers: [] })).toEqual({
    ...
    it('warns when an existing chrome-devtools configuration shadows the tunnel', () => {
        expect(deriveCapabilityStatus(true, ['allow_origin', 'cdp_tunnel_over_ws', 'browser_automation_mcp'], {
            ...
            shellReady: true,
            warning: 'An existing chrome-devtools MCP configuration is taking precedence. Disable or rename it to use the extension tunnel.',
        });
    });
...
describe('cross-package contracts', () => {
    ...
    it('keeps the runtime MCP server name aligned with the daemon', () => {
        const daemonSource = readFileSync(path.join(repoRoot, 'packages/cli/src/serve/acp-http/index.ts'), 'utf8');
        const declaration = daemonSource.match(/const CHROME_DEVTOOLS_MCP_SERVER_NAME = '([^']+)'/);
    ...
    it('keeps the tunnel endpoint pattern aligned with the acceptance runner', () => {
        const runnerSource = readFileSync(path.join(repoRoot, 'packages/cli/src/serve/cdp-tunnel/acceptance/run-real-chrome.mjs'), 'utf8');
        expect(runnerSource).toContain(String(CDP_TUNNEL_ENDPOINT_PATTERN));
    });
});
//# sourceMappingURL=capability-status.test.js.map