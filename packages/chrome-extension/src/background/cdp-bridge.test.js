...
function installChromeHarness(options) {
    ...
    const attach = vi.fn((_target, _version, callback) => {
        if (options?.deferAttach)
            attachCallback = callback;
        else
            callback();
    });
    const detach = vi.fn((_target, callback) => callback?.());
    const sendCommand = vi.fn((_target, _method, _params, callback) => callback({ value: 'ok' }));
    globalThis.chrome = {
        debugger: {
            attach,
            detach,
            ...
            onDetach: {
                addListener: vi.fn((listener) => debuggerDetachListeners.push(listener)),
                removeListener: vi.fn(),
            },
        },
        tabs: {
...
describe('CDP bridge', () => {
    ...
    it('attaches the active tab and returns its metadata', async () => {
        ...
        expect(chromeHarness.attach).toHaveBeenCalledWith({ tabId: 7 }, '1.3', expect.any(Function));
    ...
    it('forwards commands to the attached tab', async () => {
        ...
        expect(chromeHarness.sendCommand).toHaveBeenCalledWith({ tabId: 7 }, 'Runtime.evaluate', { expression: 'document.title' }, expect.any(Function));
        bridge.shutdownCdpBridge();
    });
    it('notifies the daemon when Chrome detaches the debugger', async () => {
        ...
        bridge.handleCdpFrame(frame({ type: 'cdp_attach', id: 1 }), send);
        await vi.waitFor(() => expect(send).toHaveBeenCalledTimes(1));
        chromeHarness.debuggerDetachListeners[0]?.({ tabId: 7 }, 'canceled_by_user');
    ...
    it('forwards debugger events for the attached tab', async () => {
        ...
        bridge.handleCdpFrame(frame({ type: 'cdp_attach', id: 1 }), send);
        await vi.waitFor(() => expect(send).toHaveBeenCalledTimes(1));
        chromeHarness.debuggerEventListeners[0]?.({ tabId: 7 }, 'Runtime.consoleAPICalled', { type: 'log' });
        expect(send).toHaveBeenCalledWith({
            type: 'cdp_event',
    ...
    it('ignores debugger events from a different tab', async () => {
        ...
        bridge.handleCdpFrame(frame({ type: 'cdp_attach', id: 1 }), send);
        await vi.waitFor(() => expect(send).toHaveBeenCalledTimes(1));
        chromeHarness.debuggerEventListeners[0]?.({ tabId: 999 }, 'Runtime.consoleAPICalled', { type: 'log' });
        expect(send).toHaveBeenCalledTimes(1);
    });
    it('rejects a CDP command when chrome.runtime.lastError is set', async () => {
        const chromeHarness = installChromeHarness();
        chromeHarness.sendCommand.mockImplementation((_target, _method, _params, callback) => {
...
//# sourceMappingURL=cdp-bridge.test.js.map