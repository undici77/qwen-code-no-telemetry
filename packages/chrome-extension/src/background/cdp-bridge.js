...
export function isCdpBridgeFrame(type) {
    return (type === 'cdp_command' || type === 'cdp_attach' || type === 'cdp_release');
}
/**
...
function onDebuggerEvent(source, method, params) {
    if (attachedTabId === null || source.tabId !== attachedTabId)
        return;
    if (!activeSend)
        return;
    activeSend({
...
async function handleAttach(frame, send) {
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            chrome.debugger.attach({ tabId }, CDP_PROTOCOL_VERSION, () => {
                ...
                const ownAlreadyAttached = /already attached/i.test(err?.message ?? '') &&
                    attachedTabId === tabId;
 ...
 */
async function handleCommand(frame, send) {
    if (attachedTabId === null) {
        send({
            type: 'cdp_result',
            id: frame.id,
    ...
    try {
        const result = await sendDebuggerCommand(attachedTabId, frame.method, frame.params);
 ...
 */
export function handleCdpFrame(frame, send) {
    activeSend = send;
    if (frame.type === 'cdp_attach') {
        void handleAttach(frame, send);
    }
    else if (frame.type === 'cdp_command') {
        void handleCommand(frame, send);
    }
 ...
 */
export function shutdownCdpBridge() {
