 ...
 * moment.
 */
import { isCdpBridgeFrame, handleCdpFrame, shutdownCdpBridge, } from './cdp-bridge';
...
function onWsMessage(ws, data) {
    ...
    if (msg['id'] === ACP_INIT_ID && ('result' in msg || 'error' in msg)) {
        if (msg['error']) {
            ...
            console.warn(LOG_PREFIX, 'ACP initialize failed; closing socket:', msg['error']);
            ws.close();
        }
        else {
    ...
    if (isCdpBridgeFrame(msg['type'])) {
        handleCdpFrame(msg, (frame) => sendRaw(ws, frame));
        return;
    }
...
async function connect() {
    ...
    ws.onmessage = (event) => onWsMessage(ws, event.data);
    ws.onerror = (event) => {
...
async function start() {
    ...
    try {
        ...
        if (!health.reachable) {
            console.log(LOG_PREFIX, 'Daemon not reachable; CDP client idle:', health.error);
            return;
        }
