...
export function ShareCard({ data, theme = 'light', }) {
    ...
    if (heatmapKeys.length > 0) {
        ...
        const timestamps = heatmapKeys.map((d) => parseDayKey(d).getTime());
    ...
    return (_jsxs("div", { id: "share-card", style: {
            ...
            boxSizing: 'border-box',
                    ...
                    flexShrink: 0,
                }, children: [_jsxs("div", { style: { fontSize: '12px', color: t.textMuted }, children: ["Generated by Qwen Code \u00B7 ", dayKey(new Date())] }), _jsx("div", { style: { fontSize: '12px', color: t.textMuted }, children: "github.com/QwenLM/qwen-code" })] })] }));
}
...
function buildMiniHeatmap(heatmap, theme) {
    ...
    while (d <= endDate) {
        // Local key: the cursor walks local days, and storage keys are local
        // (#6835) — a UTC key here shifted the whole strip for non-UTC viewers.
        const key = dayKey(d);
