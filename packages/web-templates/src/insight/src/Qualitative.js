...
export function AtAGlance({ qualitative, targetSections, }) {
    ...
    if (sections.length === 0)
        return null;
    return (_jsxs("div", { className: "at-a-glance", children: [_jsx("div", { className: "glance-title", children: "At a Glance" }), _jsx("div", { className: "glance-sections", children: sections.map((section) => (_jsxs("div", { className: "glance-section", children: [_jsx("strong", { children: section.label }), ' ', _jsx(MarkdownText, { children: section.body }), section.showLink && (_jsx("a", { href: section.href, className: "see-more", children: section.link }))] }, section.key))) })] }));
}
export function NavToc({ sections }) {
    if (sections.length === 0)
        return null;
    return (_jsx("nav", { className: "nav-toc", children: sections.map((section) => (_jsx("a", { href: section.href, children: section.label }, section.href))) }));
}
...
export function ImpressiveWorkflows({ qualitative, primarySuccess, outcomes, }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("h2", { id: "section-wins", className: "text-xl font-semibold text-slate-900 mt-8 mb-4", children: "Impressive Things You Did" }), impressiveWorkflows?.intro && (_jsx("p", { className: "section-intro", children: _jsx(MarkdownText, { children: impressiveWorkflows.intro }) })), impressiveWorkflows && (_jsx("div", { className: "big-wins", children: Array.isArray(impressiveWorkflows.impressive_workflows) &&
...
function HorizontalBarChart({ data, title, color = '#3b82f6', allowedKeys = null, }) {
    ...
    let entries = Object.entries(data).filter(([, count]) => Number.isFinite(count) && count !== 0);
    if (allowedKeys) {
...
export function Improvements({ qualitative, }) {
    ...
    if (!improvements)
        return null;
    const hasFeatureSuggestions = (Array.isArray(improvements.Qwen_md_additions) &&
        improvements.Qwen_md_additions.length > 0) ||
        (Array.isArray(improvements.features_to_try) &&
            improvements.features_to_try.length > 0);
    const hasUsagePatterns = Array.isArray(improvements.usage_patterns) &&
        improvements.usage_patterns.length > 0;
    if (!hasFeatureSuggestions && !hasUsagePatterns)
        return null;
    return (_jsxs(_Fragment, { children: [hasFeatureSuggestions && (_jsxs(_Fragment, { children: [_jsx("h2", { id: "section-features", className: "text-xl font-semibold text-slate-900 mt-8 mb-4", children: "Existing Qwen Code Features to Try" }), Array.isArray(improvements.Qwen_md_additions) &&
