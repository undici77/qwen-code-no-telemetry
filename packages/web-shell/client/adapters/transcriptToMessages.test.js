...
function textBlock(id, kind, text, createdAt, streaming = false, overrides = {}) {
    return {
        id,
        kind,
        text,
...
function statusBlock(id, text, createdAt, overrides = {}) {
    return {
        id,
        kind: 'status',
        text,
...
function promptCancelledBlock(id, createdAt) {
    return {
        id,
...
function shellBlock(id, text, createdAt, overrides = {}) {
    return {
        id,
        kind: 'shell',
        text,
...
function userShellBlock(id, text, command, createdAt, overrides = {}) {
    return {
        id,
        kind: 'user_shell',
        text,
...
function toolBlock(id, toolCallId, status, createdAt, overrides = {}) {
    return {
        id,
        kind: 'tool',
        toolCallId,
...
describe('transcriptBlocksToDaemonMessages', () => {
    ...
    it('renders live background task notifications as system messages', () => {
        const messages = transcriptBlocksToDaemonMessages([
            textBlock('bg-1', 'assistant', 'Background agent "general-purpose: 查询百度云活动信息" completed.', 1, false, {
                meta: {
        ...
        expect(messages).toEqual([
            {
                id: 'bg-1',
                role: 'system',
                content: 'Background agent "general-purpose: 查询百度云活动信息" completed.',
                variant: 'info',
                source: 'background_notification',
    ...
    it.each([
        ...
        ['failed', 'failed'],
        ['cancelled', 'completed'],
    ])('projects a background agent %s notification onto its tool', (notificationStatus, expectedStatus) => {
        const messages = transcriptBlocksToDaemonMessages([
            ...
            textBlock('agent-terminal', 'assistant', 'background agent finished', 2, false, {
                meta: {
                    source: 'background_notification',
                    qwenDiscreteMessage: true,
                    backgroundTask: {
        ...
        if (notificationStatus === 'cancelled') {
            expect(messages[0]?.role === 'tool_group'
                ? messages[0].tools[0]?.rawOutput
                : undefined).toMatchObject({ status: 'cancelled' });
        }
    });
    it('projects a replayed user background agent notification onto its tool', () => {
        const messages = transcriptBlocksToDaemonMessages([
            ...
            textBlock('agent-terminal', 'user', 'background agent finished', 2, false, {
                meta: {
                    source: 'background_notification',
                    qwenDiscreteMessage: true,
                    backgroundTask: {
    ...
    it('localizes structured session branch status blocks', () => {
        const messages = transcriptBlocksToDaemonMessages([
            ...
            labels: {
                branchSuccess: (name) => `已复制会话，新会话名称为： "${name}"，当前已切换到新的会话。`,
            },
        });
        expect(messages).toEqual([
            expect.objectContaining({
                role: 'system',
                content: '已复制会话，新会话名称为： "support-branch-new3 (Branch 2)"，当前已切换到新的会话。',
                source: 'session_branched',
                data: {
                    sourceSessionId: 'source',
                    newSessionId: 'new',
    ...
    it('keeps merged groups intact when a member tool completes later', () => {
        ...
        const tools = messages[0].role === 'tool_group' ? messages[0].tools : undefined;
    ...
    it('renders insight progress messages', () => {
        const messages = transcriptBlocksToDaemonMessages([
            textBlock('insight-1', 'assistant', '{"insight_progress":{"stage":"scan","progress":0.5,"detail":"reading"}}', 1),
        ]);
        expect(messages).toEqual([
            {
                id: 'insight-1-ip',
    ...
    it('renders terminal insight messages with surrounding text', () => {
        const messages = transcriptBlocksToDaemonMessages([
            textBlock('insight-1', 'assistant', 'before {"insight_ready":{"path":"/tmp/report.md"}} middle {"insight_error":{"error":"boom"}} after', 1),
        ]);
        expect(messages).toEqual([
            {
                id: 'insight-1-t-0',
    ...
    it('keeps parented compacted replay subagent content nested', () => {
        ...
        expect(messages.some((message) => message.role === 'assistant' &&
            message.content.includes('subagent answer'))).toBe(false);
    });
    it('keeps parallel top-level subagents as sibling tool messages', () => {
        ...
        expect(messages[0]?.role === 'tool_group' && messages[0].tools[0]).not.toHaveProperty('subTools');
        expect(messages[1]?.role === 'tool_group' && messages[1].tools[0]).not.toHaveProperty('subTools');
        expect(messages[2]?.role === 'tool_group' && messages[2].tools[0]).not.toHaveProperty('subTools');
    });
    ...
    it('keeps unparented assistant text in the main transcript', () => {
        ...
        expect(messages[1]?.role === 'tool_group' && messages[1].tools[0]).not.toHaveProperty('subContent');
    });
        ...
        const messages = transcriptBlocksToDaemonMessages([
            {
                ...
                text: 'language_changed (unrecognized daemon event): ' +
    ...
    it('filters legacy unrecognized-event blocks that carry no debugReason', () => {
        ...
        const messages = transcriptBlocksToDaemonMessages([
            legacy('legacy-1', 'language_changed (unrecognized daemon event): {"language":"en"}'),
            legacy('legacy-2', 'session_cwd_changed (unrecognized daemon event): {"cwd":"/work"}'),
            legacy('legacy-3', 'some_future_event (unrecognized daemon event): {"a":1}'),
        ]);
        expect(messages).toEqual([]);
    });
    ...
    it('keys the filter off the unrecognized_ category prefix, not the enum', () => {
        ...
        const block = (id, debugReason, text) => ({
            id,
            kind: 'debug',
            debugReason,
            text,
        ...
        const messages = transcriptBlocksToDaemonMessages([
            block('future-unrecognized', 'unrecognized_tool_frame', 'tool_frame: {"frameId":"f1"}'),
            block('future-malformed', 'malformed_tool_frame', 'tool_frame: broken frame payload'),
        ]);
    ...
    it('only matches the legacy shape, never a quoted marker or a status block', () => {
        ...
        const messages = transcriptBlocksToDaemonMessages([
            ...
            block('status-1', 'status', 'peer reported (unrecognized daemon event): malformed frame'),
            // A legacy malformed payload relaying an upstream message.
            block('legacy-malformed', 'debug', 'permission_request: {"message":"peer said (unrecognized daemon event): x"}'),
            // A client-dispatched summary that happens to quote it.
            block('client-1', 'debug', 'Model switch failed: upstream said (unrecognized daemon event): x', { source: 'model_switch_summary' }),
    ...
    it('keeps shell output attached when the tool later completes', () => {
        ...
        const tool = messages[0].role === 'tool_group' ? messages[0].tools[0] : undefined;
    ...
    it('preserves title, args, and rawOutput on completed parallel agents', () => {
        ...
        expect(messages).toHaveLength(2);
        const tool1 = messages[0].role === 'tool_group' ? messages[0].tools[0] : undefined;
        const tool2 = messages[1].role === 'tool_group' ? messages[1].tools[0] : undefined;
                ...
                tools: [
                    {
                        ...
                        status: 'failed',
                        rawOutput: 'Question 1: "options" must contain between 2 and 4 options.',
                    },
                ],
            },
        ]);
    ...
    it('merges the real subagent tool update after a permission block', () => {
        ...
        expect(messages).toHaveLength(1);
        const agent = messages[0].role === 'tool_group' ? messages[0].tools[0] : undefined;
    ...
    it('keeps approved regular tool permission visible until tool update arrives', () => {
        const pendingMessages = transcriptBlocksToDaemonMessages([
            {
                ...
                requestId: 'req-1',
                sessionId: 'sess-1',
                title: 'Fetching content from https://www.aliyun.com/activity (format: markdown)',
        ...
        const pendingTool = pendingMessages[0].role === 'tool_group'
    ...
    ])('does not overwrite a completed tool with a later %s permission block', (resolved) => {
        const messages = transcriptBlocksToDaemonMessages([
    ...
    it('handles nested subagent via parentToolCallId', () => {
        ...
        expect(messages).toHaveLength(1);
        const parentTool = messages[0].role === 'tool_group' ? messages[0].tools[0] : undefined;
    ...
    it('keeps status blocks in the main transcript while subagent is active', () => {
        ...
        expect(messages).toHaveLength(2);
        const agentTool = messages[0].role === 'tool_group' ? messages[0].tools[0] : undefined;
    ...
    it('renders localized prompt cancellation messages', () => {
        const messages = transcriptBlocksToDaemonMessages([promptCancelledBlock('cancel-1', 20)], { labels: { promptCancelled: '请求已取消。' } });
        expect(messages).toEqual([
            {
                id: 'cancel-1',
                role: 'system',
    ...
    it('status mapping covers running, pending, failed, and canceled (alternate spelling)', () => {
        ...
        expect(messages).toHaveLength(2);
        const tools = messages[0].role === 'tool_group' ? messages[0].tools : undefined;
        const tool4 = messages[1].role === 'tool_group' ? messages[1].tools[0] : undefined;
    ...
    it('identifies subAgent by toolName task and rawOutput.type task_execution', () => {
        ...
        expect(messages).toHaveLength(1);
        const taskTool = messages[0].role === 'tool_group' ? messages[0].tools[0] : undefined;
        ...
        const execTool = taskExecByRawOutput[0].role === 'tool_group'
    ...
    it('two parallel Agents from permission_request each nest their own sub-tools', () => {
        ...
        expect(messages).toHaveLength(2);
        const agentA = messages[0].role === 'tool_group' ? messages[0].tools[0] : undefined;
        const agentB = messages[1].role === 'tool_group' ? messages[1].tools[0] : undefined;
    ...
    it('nests parented text inside the matching parallel subagent', () => {
        ...
        expect(messages.find((message) => message.role === 'assistant')).toBe(undefined);
    });
...
//# sourceMappingURL=transcriptToMessages.test.js.map