...
const batch = (sessionId, ...messages) => ({
    sessionId,
    messages,
});
const batchFrom = (sessionId, originatorClientId, ...messages) => ({ sessionId, originatorClientId, messages });
const batchWithIds = (sessionId, messages, messageIds) => ({ sessionId, messages, messageIds });
describe('removeInjectedFromQueue', () => {
    ...
    it('reconciles ACROSS multiple accumulated batches (the #439 regression)', () => {
        ...
        const next = removeInjectedFromQueue(prompts, [batch('s', 'first'), batch('s', 'second')], 's');
    ...
    it('is count-based: removes one queued entry per injected occurrence', () => {
        ...
        expect(removeInjectedFromQueue(prompts, [batch('s', 'dup')], 's')?.map((p) => p.text)).toEqual(['dup', 'other']);
        // two injections (across batches) -> both removed
        expect(removeInjectedFromQueue(prompts, [batch('s', 'dup'), batch('s', 'dup')], 's')?.map((p) => p.text)).toEqual(['other']);
    });
    ...
    it('does not remove an ordinary queued prompt with the same text', () => {
        ...
        const next = removeInjectedFromQueue([ordinary, inserted], [batch('s', 'same')], 's');
        expect(next).toEqual([ordinary]);
    });
    it('uses message ids instead of text when the daemon provides them', () => {
        const first = q('same');
        const second = q('same');
        const next = removeInjectedFromQueue([first, second], [batchWithIds('s', ['same'], [second.midTurnMessageId])], 's');
    ...
    it('matches a submitting prompt before its admission response provides the id', () => {
        ...
        const next = removeInjectedFromQueue([submitting], [batchWithIds('s', ['early injection'], ['mid-early'])], 's');
        expect(next).toEqual([]);
    });
    it('removes the id-matched row, not an earlier same-text row still submitting', () => {
        ...
        const queued = q('x');
        const next = removeInjectedFromQueue([submitting, queued], [batchWithIds('s', ['x'], [queued.midTurnMessageId])], 's');
    ...
    describe('originator (clientId) filtering', () => {
        it('dedupes a batch whose originator matches our client id', () => {
            ...
            const next = removeInjectedFromQueue(prompts, [batchFrom('s', 'me', 'mine')], 's', 'me');
        ...
        it('skips a batch originated by a DIFFERENT client (no spurious dedupe)', () => {
            ...
            expect(removeInjectedFromQueue(prompts, [batchFrom('s', 'peer', 'shared')], 's', 'me')).toBeNull();
        });
        it('matches a stable id even when another client receives the echo', () => {
            const prompt = q('adopted');
            const next = removeInjectedFromQueue([prompt], [
                {
        ...
        it('dedupes an anonymous batch (no originator) regardless of our client id', () => {
            ...
            const next = removeInjectedFromQueue(prompts, [batch('s', 'anon')], 's', 'me');
            expect(next?.map((p) => p.text)).toEqual(['keep']);
        });
        it('routes a mixed-originator set: ours dedupes, the peer’s is skipped', () => {
            const prompts = [q('mine'), q('theirs'), q('keep')];
            const next = removeInjectedFromQueue(prompts, [batchFrom('s', 'me', 'mine'), batchFrom('s', 'peer', 'theirs')], 's', 'me');
        ...
        it('skips our OWN-tagged batch when no client id is supplied (regression guard)', () => {
            ...
            expect(removeInjectedFromQueue(prompts, [batchFrom('s', 'me', 'mine')], 's')).toBeNull();
        });
    });
});
//# sourceMappingURL=midTurnDedup.test.js.map