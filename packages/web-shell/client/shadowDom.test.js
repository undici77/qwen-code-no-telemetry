...
describe('isPluginShadowPanel', () => {
    it.each(['plugins', 'extensions', 'mcp', 'skills', 'agents', 'channels'])('includes the %s management surface', (panel) => {
        expect(isPluginShadowPanel(panel)).toBe(true);
    });
    it.each([null, 'settings', 'status', 'sessions'])('excludes the %s non-plugin surface', (panel) => {
...
describe('resolveWebShellShadowDom', () => {
    ...
    it('resolves plugin and portal scenes independently', () => {
        expect(resolveWebShellShadowDom({
            plugins: true,
            portals: false,
            styles: '.custom {}',
        })).toEqual({
...
describe('installWebShellShadowStyles', () => {
    it('copies package CSS before consumer CSS and cleans up both', () => {
        ...
        const cleanup = installWebShellShadowStyles(shadowRoot, '.consumer-content { color: rebeccapurple; }');
        expect(Array.from(shadowRoot.querySelectorAll('style')).map((style) => style.textContent)).toEqual([
...
//# sourceMappingURL=shadowDom.test.js.map