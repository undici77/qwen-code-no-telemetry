...
function hasModernWorkspaceFeature(features) {
    return features.some((feature) => MODERN_WORKSPACE_FEATURES.has(feature) ||
...
export function resolveVoiceWorkspaceTarget({ capabilities, intendedCwd, sessionId, workspaces, }) {
    if (!capabilities)
        return undefined;
    const registered = workspaces ?? capabilities.workspaces;
    const primaryCwd = registered?.find((workspace) => workspace.primary)?.cwd ??
    ...
    if (registered) {
        ...
        if (workspace.primary)
            return legacyTarget(workspace.cwd, sessionId);
        if (!workspace.trusted)
            return undefined;
        const usableId = workspace.id.trim() ? workspace.id : undefined;
        const idIsUnique = usableId !== undefined &&
    ...
    if (capabilities.workspaceCwd) {
        if (cwd !== capabilities.workspaceCwd)
            return undefined;
...
export function supportsVoiceCapture(target, features) {
    if (!target)
        return false;
    return features.includes(target.route === 'legacy-primary'
...
export async function loadVoiceStatus(client, target) {
    const status = target.route === 'legacy-primary'
    ...
    if (target.cwd && status.workspaceCwd !== target.cwd) {
        throw new Error('Voice status workspace does not match the selected target.');
    }
    return status;
}
export async function loadVoiceProviders(client, target) {
    ...
    if (target.cwd && status.workspaceCwd !== target.cwd) {
        throw new Error('Voice provider workspace does not match the selected target.');
    }
    return status;
}
...
export async function setVoiceModelSetting(client, target, scope, value) {
    if (target.route === 'workspace-qualified' && scope === 'workspace') {
        return await qualifiedClient(client, target).setWorkspaceSetting('workspace', 'voiceModel', value);
    }
