...
const mounted = [];
function voiceStatus(enabled, workspaceCwd = '/tmp/workspace', mode = 'hold') {
    return {
        v: 1,
        workspaceCwd,
        enabled,
...
function mount(disabled, target = legacyTarget, onActiveChange) {
    const container = document.createElement('div');
    document.body.appendChild(container);
    const root = createRoot(container);
    act(() => {
        root.render(_jsx(VoiceButton, { disabled: disabled, onInsert: () => { }, target: target, onActiveChange: onActiveChange }));
    });
...
async function render(disabled) {
...
const pointer = (button, type, pointerId = 1, mouseButton = 0, timeStamp) => {
...
beforeEach(() => {
    ...
    mocks.qualifiedWorkspaceVoice.mockResolvedValue(voiceStatus(true, '/tmp/secondary'));
...
describe('VoiceButton', () => {
    ...
    it('stays hidden while the workspace voice request is pending or fails', async () => {
        ...
        let rejectVoice = () => undefined;
        mocks.workspaceVoice.mockReturnValue(new Promise((_, reject) => {
            rejectVoice = reject;
        }));
        const { container } = mount(false);
        expect(container.querySelector('button')).toBeNull();
        ...
        expect(container.querySelector('button')).toBeNull();
        expect(warn).toHaveBeenCalledWith('[web-shell] Voice status probe failed:', expect.any(Error));
        warn.mockRestore();
    });
    ...
    it('reloads workspace voice when settings change', async () => {
        ...
        mocks.workspaceVoice.mockResolvedValue(voiceStatus(true));
        act(() => {
            root.render(_jsx(VoiceButton, { disabled: false, onInsert: () => { }, target: legacyTarget }));
        });
        expect(container.querySelector('button')).toBeNull();
        await flush();
        expect(container.querySelector('button')).not.toBeNull();
    ...
    it('ignores a stale workspace voice response', async () => {
        ...
        mocks.workspaceVoice.mockReturnValueOnce(new Promise((resolve) => {
    ...
    it('waits for the current workspace client voice status', async () => {
        ...
        await flush();
        expect(container.querySelector('button')).not.toBeNull();
        let resolveNext = () => undefined;
        const workspaceVoice = vi.fn(() => new Promise((resolve) => {
            resolveNext = resolve;
        }));
    ...
    it('ignores a completed capability refresh from a previous owner', async () => {
        mocks.workspace.capabilities.features = ['workspace_qualified_voice'];
        mocks.qualifiedWorkspaceVoice.mockResolvedValue(voiceStatus(true, '/tmp/secondary-a'));
        let resolveRefresh = () => undefined;
        mocks.workspace.refreshCapabilities.mockReturnValue(new Promise((resolve) => {
        ...
        mocks.qualifiedWorkspaceVoice.mockResolvedValue(voiceStatus(true, '/tmp/secondary-b'));
        act(() => {
            root.render(_jsx(VoiceButton, { disabled: false, onInsert: () => { }, target: targetB }));
        });
        await flush();
        expect(container.querySelector('button')).not.toBeNull();
        mocks.capture.status = 'recording';
        ...
        expect(container.querySelector('button')).not.toBeNull();
        expect(mocks.capture.abort).not.toHaveBeenCalled();
        expect(mocks.qualifiedWorkspaceVoice).toHaveBeenCalledTimes(statusRequestCount);
        expect(mocks.captureOptions?.target).toEqual({
    ...
    it('revalidates the same owner when a 1012 refresh fails', async () => {
        const warn = vi.spyOn(console, 'warn').mockImplementation(() => undefined);
        mocks.workspace.refreshCapabilities.mockRejectedValue(new Error('refresh failed'));
        const { root, container } = mount(false);
        await flush();
        expect(container.querySelector('button')).not.toBeNull();
        mocks.workspaceVoice.mockRejectedValueOnce(new Error('voice status still unavailable'));
        act(() => {
            mocks.captureOptions?.onUnexpectedClose?.({
                code: 1012,
                reason: 'workspace removed',
    ...
    it('lets a disabled composer stop active dictation', async () => {
        mocks.workspaceVoice.mockResolvedValue(voiceStatus(true, '/tmp/workspace', 'tap'));
        mocks.capture.status = 'recording';
    ...
    it('reports whether voice capture is active', async () => {
        ...
        mocks.capture.status = 'connecting';
        act(() => {
            root.render(_jsx(VoiceButton, { disabled: false, onInsert: () => { }, target: legacyTarget, onActiveChange: onActiveChange }));
        });
    ...
    it('clears the no-speech notice after two seconds', async () => {
        ...
        expect(container.querySelector('[role="status"]')?.textContent).toBe('voice.noSpeech');
        act(() => {
