...
import { useConnection, useWorkspace, useWorkspaceEventSignals, } from '@qwen-code/webui/daemon-react-sdk';
import { useI18n } from '../i18n';
import { useVoiceCapture } from './useVoiceCapture';
import { loadVoiceStatus, supportsVoiceCapture, } from './voice-workspace-target';
...
export function VoiceButton({ onInsert, onActiveChange, disabled, target, statusRevision = { user: 0, workspace: 0 }, }) {
    ...
    const settingsVersion = target?.sessionId &&
    ...
    const voiceEnabled = captureSupported &&
        !capabilityRefreshFailed &&
    ...
    useEffect(() => {
        if (!noticeMessage)
            return undefined;
        const timer = window.setTimeout(() => setNoticeMessage(undefined), NOTICE_TIMEOUT_MS);
    ...
    useLayoutEffect(() => {
        if (previousOwnerRef.current === target?.ownerKey)
            return;
    ...
    const handlePointerUp = (event) => {
        ...
        if (event.timeStamp - holdStartedAtRef.current >= HOLD_THRESHOLD_MS &&
    ...
    let control;
    if (isRecording) {
        control = (_jsxs("button", { type: "button", className: styles.pill, onClick: handleClick, onPointerDown: handlePointerDown, onPointerUp: handlePointerUp, onPointerCancel: handlePointerCancel, "aria-label": label, title: label, children: [_jsx("span", { className: styles.recDot, "aria-hidden": "true" }), _jsx("span", { className: styles.wave, "aria-hidden": "true", children: levels.map((lvl, i) => (_jsx("span", { className: styles.bar, style: { height: `${2 + Math.round(lvl * 14)}px` } }, i))) }), _jsx("span", { className: styles.time, children: formatElapsed(elapsedMs) }), _jsx("span", { className: styles.stop, "aria-hidden": "true", children: _jsx(StopIcon, {}) })] }));
    }
        ...
        const iconClass = [
            ...
            .filter(Boolean)
            .join(' ');
        control = (_jsx("button", { type: "button", className: iconClass, onClick: handleClick, onPointerDown: handlePointerDown, onPointerUp: handlePointerUp, onPointerCancel: handlePointerCancel, disabled: Boolean(disabled) && !canCancel, "aria-label": label, title: errorMessage ?? noticeMessage ?? label, children: _jsx(MicIcon, {}) }));
    }
