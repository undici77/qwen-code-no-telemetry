...
function decodeBearerSubprotocol(proto) {
    const b64 = proto
...
describe('useVoiceCapture', () => {
    it('preserves reverse-proxy base paths in the websocket URL', async () => {
        ...
        expect(MockWebSocket.latest?.url).toBe('wss://example.test/qwen/voice/stream');
    });
    it('uses a workspace-qualified stream path without double encoding', async () => {
        ...
        expect(MockWebSocket.latest?.url).toBe('wss://example.test/qwen/workspaces/%2Frepo%2F%E4%BA%8C%20%E6%AC%A1/voice/stream');
    });
    ...
    it('times out and cleans up when mobile audio resume stalls', async () => {
        ...
        class StalledAudioContext extends MockAudioContext {
            state = 'suspended';
            resume = vi.fn(() => new Promise(() => undefined));
            close = close;
        }
        Object.defineProperty(window, 'AudioContext', {
        ...
        expect(onError).toHaveBeenCalledWith('Voice capture timed out while starting.');
    ...
    it('fails instead of staying transcribing when the socket closes early', async () => {
        ...
        expect(onError).toHaveBeenCalledWith('Voice connection closed (code=1006, reason=none).');
        expect(capture?.status).toBe('error');
    });
    ...
    it('flushes buffered audio before a pending stop when the socket opens', async () => {
        ...
        expect(Array.from(new Int16Array(ws.sent[1]))).toEqual([
            16383, -16384,
        ]);
    ...
    it('fails and stops buffering when pre-open audio exceeds the limit', async () => {
        ...
        expect(onError).toHaveBeenCalledWith('Voice capture buffer limit reached while starting.');
        expect(capture?.status).toBe('error');
        expect(processor.onaudioprocess).toBeNull();
    ...
    it('fails when the server sends no response after recording starts', async () => {
        ...
        expect(onError).toHaveBeenCalledWith('No response from server. Check that the voice model is running.');
        expect(capture?.status).toBe('error');
    });
    it('ignores stale socket callbacks after a new capture starts', async () => {
        ...
        if (!firstWs?.onmessage)
            throw new Error('first WebSocket was not ready');
    ...
    it('aborts and drops an old final when the owner changes', async () => {
        ...
        const firstWs = MockWebSocket.latest;
        const staleMessage = firstWs?.onmessage;
        if (!firstWs || !staleMessage)
            throw new Error('socket was not ready');
...
//# sourceMappingURL=useVoiceCapture.test.js.map