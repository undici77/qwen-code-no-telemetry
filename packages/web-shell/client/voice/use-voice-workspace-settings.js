...
import { loadVoiceSettings, } from './voice-workspace-target';
export function useVoiceWorkspaceSettings(client, target, enabled, revisionKey) {
    const requestRef = useRef(0);
    const ownerKey = enabled && target?.route === 'workspace-qualified'
        ? target.ownerKey
        : undefined;
    ...
    const reload = useCallback(async () => {
        ...
        if (currentRequestKeyRef.current !== key)
            return undefined;
    ...
    return {
        descriptor: current?.status?.settings.find((setting) => setting.key === 'voiceModel'),
        reload,
    };
}
//# sourceMappingURL=use-voice-workspace-settings.js.map