...
const MAX_BUFFERED_PCM_BYTES = SAMPLE_RATE * Int16Array.BYTES_PER_ELEMENT * (START_TIMEOUT_MS / 1000);
export function toVoiceWebSocketUrl(baseUrl, streamPath) {
    const base = new URL(baseUrl);
    const basePath = base.pathname.replace(/\/?$/, '/');
    const url = new URL(streamPath.replace(/^\/+/, ''), `${base.origin}${basePath}`);
...
export function useVoiceCapture({ baseUrl, token, target, onFinal, onError, onUnexpectedClose, }) {
    ...
    const clearTranscribeTimeout = useCallback(() => {
        ...
        if (!resources.transcribeTimeout)
            return;
    ...
    const snapshotIsCurrent = useCallback((snapshot) => mountedRef.current &&
        generationRef.current === snapshot.generation &&
        snapshotRef.current === snapshot, []);
    const fail = useCallback((message, snapshot) => {
    ...
    const finishWith = useCallback((text, snapshot) => {
        if (!snapshotIsCurrent(snapshot))
            return;
        cleanup();
        if (!mountedRef.current)
    ...
    const finalize = useCallback((ws, snapshot) => {
        teardownAudio();
        setAudioLevel(0);
    ...
    const start = useCallback(() => {
        ...
        if (!navigator.mediaDevices?.getUserMedia) {
            fail(window.isSecureContext
                ...
                : 'Microphone needs a secure context — open the Web Shell via localhost/127.0.0.1 or https.', snapshot);
            return;
        }
        let context;
        let contextReady;
        try {
            ...
            if (context.sampleRate !== SAMPLE_RATE) {
                throw new Error(`Browser audio rate ${context.sampleRate} Hz is not the required ${SAMPLE_RATE} Hz.`);
            }
            contextReady =
        ...
        void (async () => {
            try {
                ...
                const ws = new WebSocket(toVoiceWebSocketUrl(baseUrl, snapshot.streamPath), token ? [WS_AUTH_SUBPROTOCOL, bearerSubprotocol(token)] : undefined);
                ...
                processor.onaudioprocess = (event) => {
                    if (!snapshotIsCurrent(snapshot))
                        return;
                    const { pcm, level } = floatToPcm16(event.inputBuffer.getChannelData(0));
                    setAudioLevel(level);
                    if (streamStarted) {
                        ...
                        if (bufferedPcmBytes + pcm.byteLength > MAX_BUFFERED_PCM_BYTES) {
                            fail('Voice capture buffer limit reached while starting.', snapshot);
                            return;
                        }
                ...
                ws.onopen = () => {
                    ...
                    resourcesRef.current.transcribeTimeout = setTimeout(() => {
                        if (snapshotIsCurrent(snapshot) &&
                            ...
                            fail('No response from server. Check that the voice model is running.', snapshot);
                        }
                    }, TRANSCRIPTION_TIMEOUT_MS);
                ...
                ws.onmessage = (event) => {
                    ...
                    if (message.type === 'interim') {
                        ...
                        fail(message.message ?? message.text ?? 'Voice transcription failed.', snapshot);
                    }
                };
                ws.onerror = () => {
                ...
                ws.onclose = (event) => {
                    ...
                    fail(code === 1013
                        ? reason === 'none'
                            ...
                            : reason
                        : `Voice connection closed (code=${code}, reason=${reason}).`, snapshot);
                };
            }
            catch (error) {
                fail(error instanceof Error ? error.message : String(error), snapshot);
    ...
    const stop = useCallback(() => {
        ...
        if (!snapshot || !ws || ws.readyState !== WebSocket.OPEN) {
            if (snapshot && statusRef.current === 'connecting') {
                if (resourcesRef.current.processor)
                    teardownAudio();
