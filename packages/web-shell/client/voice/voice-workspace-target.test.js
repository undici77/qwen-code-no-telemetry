...
import { loadVoiceProviders, loadVoiceStatus, resolveVoiceWorkspaceTarget, setVoiceModelSetting, supportsVoiceCapture, supportsVoiceModelSettings, } from './voice-workspace-target';
...
describe('resolveVoiceWorkspaceTarget', () => {
    ...
    it('fails closed for an untrusted secondary target', () => {
        expect(resolveVoiceWorkspaceTarget({
            capabilities: capabilities({
...
describe('Voice target operation gates', () => {
    ...
    it('separates dictation from secondary model/settings support', () => {
        ...
        expect(supportsVoiceCapture(qualified, ['workspace_qualified_voice'])).toBe(true);
        expect(supportsVoiceModelSettings(qualified, ['workspace_qualified_voice'])).toBe(false);
        expect(supportsVoiceModelSettings(qualified, [
            'workspace_qualified_voice',
            'workspace_qualified_rest_core',
            'workspace_settings',
        ])).toBe(true);
...
describe('Voice target route adapters', () => {
    it('uses qualified reads and workspace writes without primary fallback', async () => {
        ...
        if (!target)
            throw new Error('expected qualified target');
        await expect(loadVoiceStatus(client, target)).resolves.toMatchObject({ workspaceCwd: secondary.cwd });
        await expect(loadVoiceProviders(client, target)).resolves.toMatchObject({ workspaceCwd: secondary.cwd });
        await setVoiceModelSetting(client, target, 'workspace', 'voice-1');
        ...
        expect(setWorkspaceSetting).toHaveBeenCalledWith('workspace', 'voiceModel', 'voice-1');
    ...
    it('rejects a response for the wrong workspace', async () => {
        ...
        if (!target)
            throw new Error('expected qualified target');
        await expect(loadVoiceStatus(client, target)).rejects.toThrow('does not match');
    });
});
//# sourceMappingURL=voice-workspace-target.test.js.map