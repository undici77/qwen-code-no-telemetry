...
import { DAEMON_APPROVAL_MODES, useActions, useConnection, useDaemonFollowupSuggestion, useSettings, useProviders, useSessionNotices, useDaemonSessionOwnerGuard, useStreamingState, useTranscriptHistory, useTranscriptStore, useWorkspace, useWorkspaceActions, useWorkspaceEventSignals, } from '@qwen-code/webui/daemon-react-sdk';
import { DaemonHttpError, isDaemonTurnError } from '@qwen-code/sdk/daemon';
import {} from './components/GitModePopover';
import { SESSION_LIST_PAGE_SIZE, SESSION_MONITOR_TOOL_CORRELATION_FEATURE, SESSION_SIDE_TASK_FEATURE, SESSION_TRANSCRIPT_PAGINATION_FEATURE, WEB_SHELL_SIDE_TASK_SOURCE_TYPE, } from './constants/sessions';
...
import { loadVoiceProviders, resolveVoiceWorkspaceTarget, setVoiceModelSetting, supportsVoiceModelSettings, } from './voice/voice-workspace-target';
...
import { loadSessionCatalogOnce, SESSION_CATALOG_TRAILING_REFRESH_MS, } from './session-catalog/session-catalog-store';
...
import { displayPath, getFileChangePreviewContent, TURN_OUTPUT_KINDS, } from './components/artifacts/TurnOutputs';
import { useArtifactWorkspaceTarget } from './components/artifacts/useArtifactWorkspaceTarget';
import { getArtifactsByTurn, getFileChangesByTurn, getScheduledTasksByTurn, } from './components/artifacts/turnOutputSelectors';
import { useIsLargeScreen } from './hooks/useIsLargeScreen';
import { usePrefersReducedMotion } from './hooks/usePrefersReducedMotion';
import { clearSplitSessions, loadSplitSessions, MAX_SPLIT_PANES, parseSplitSessionIds, saveSplitSessions, } from './utils/splitUrl';
...
import { goalArgOf, isGoalClearCommand, isGoalClearKeyword, } from './utils/goalCondition';
...
import { isPluginShadowPanel, installWebShellShadowStyles, resolveWebShellShadowDom, } from './shadowDom';
...
import { I18nProvider, getTranslator, languageSettingToWebShellLanguage, languageLabel, normalizeLanguage, } from './i18n';
import { copyFromLastAssistantMessage, COPY_MESSAGES, } from './utils/copyCommand';
import { getShadowAwareActiveElement, isEditableTarget } from './utils/dom';
import { invokeSlashCommandHandler, SLASH_COMMAND_PATTERN, } from './utils/slash-command-action';
...
import { decodeVisionModelForPicker, encodeVisionModelForSetting, extractBareModelId, } from './utils/modelEncoding';
...
import { GOAL_STATUS_ACTIVE_EVENT, parseGoalStatusMessage, serializeGoalStatusMessage, } from './components/messages/GoalStatusMessage';
import { BtwMessage } from './components/messages/BtwMessage';
import { createAndAttachSessionForPrompt, isDaemonApprovalMode, } from './utils/sessionPreparation';
import { getComposerPlaceholderKey, getComposerPlaceholderState, shouldBlockComposerSubmit, shouldDisableComposerInput, } from './utils/composerInputState';
...
import { computeTodoDetails, computeTodoTimeline, getAgentToolsForPlan, getFloatingTodos, getActiveTodosForPlanRevision, isExitPlanApprovalRequest, todoDetailSignature, todoTimelineSignature, } from './utils/todos';
import { ThemeProvider } from './themeContext';
import { InteractionBlockContext } from './interactionBlockContext';
import { WebShellThemeId, THEME_SETTING_KEY, LANGUAGE_SETTING_KEY, themeSettingToWebShellTheme, webShellThemeToSettingValue, } from './themeContext';
...
function TodoContextsProvider({ timeline, details, children, }) {
    return (_jsx(TodoTimelineContext.Provider, { value: timeline, children: _jsx(TodoDetailContext.Provider, { value: details, children: children }) }));
}
...
function getFullscreenSurfaceTabEdges(container) {
    ...
    const visible = candidates.filter((element) => typeof element.checkVisibility === 'function'
...
function retryOwnerMatchesCurrent(owner, sessionId, workspaceCwd, sourceVersion) {
    const workspaceMatches = (owner.workspaceCwd !== undefined && owner.workspaceCwd === workspaceCwd) ||
...
function mergeCancelledRetryEntries(current, incoming) {
    return incoming.reduce((merged, candidate) => {
        const existing = merged.find((entry) => entry.state.kind === candidate.state.kind);
...
function matchesUserMessageIdentity(block, identity, allowLocalId = false) {
    ...
    if (block === identity.block)
        return true;
    if (allowLocalId && block.id === identity.block.id)
        return true;
    const expectedRecords = identity.block.sourceRecordIds;
    const currentRecords = block.sourceRecordIds;
    return (expectedRecords !== undefined &&
...
function getRetryableTurnError(blocks) {
    for (let i = blocks.length - 1; i >= 0; i--) {
        ...
        if (block?.kind === 'user') {
            if (block.meta?.['source'] === 'background_notification')
                continue;
            break;
        }
...
function matchesTurnErrorIdentity(block, identity) {
    ...
    if (expectedPromptId)
        return block.promptId === expectedPromptId;
    return (identity.block.eventId !== undefined &&
        block.eventId === identity.block.eventId);
}
function retryTranscriptIdentityMatches(blocks, transcriptIdentity, allowLocalUserId = false) {
    return transcriptIdentity.kind === 'failed-prompt'
        ? matchesUserMessageIdentity(getLatestUserBlock(blocks), transcriptIdentity.identity, allowLocalUserId)
        : matchesTurnErrorIdentity(getRetryableTurnError(blocks), transcriptIdentity.identity);
}
function parseGoalStatusFromBlock(block) {
    const statusBlock = block;
    if (statusBlock.source !== 'goal')
        return null;
    return (parseGoalStatusMessage(statusBlock.data) ??
...
const DEFAULT_CHAT_HEADER_ITEMS = [
    'title',
    'environment',
    'rightPanel',
];
const DEFAULT_RIGHT_PANEL_ITEMS = [
    'review',
    'sideTask',
];
...
function shouldToastNotice(notice) {
    return (notice.category === 'validation' ||
...
function parseRenameArgument(raw) {
    ...
    if (!trimmed)
        return { type: 'auto' };
    if (trimmed === '--')
        return { type: 'manual', displayName: '' };
...
function isBackgroundTaskToolCall(tool) {
    ...
    if (name === 'monitor')
        return true;
    if (tool.args?.is_background !== true)
        return false;
    return (name === 'shell' ||
        name === 'bash' ||
...
export function mergeSideTaskCatalog(catalog, parentSessionId, listedItems, optimisticIds) {
    ...
    return {
        ...
        items: [
            ...listedItems,
            ...catalog.items.filter((item) => !listedIds.has(item.sessionId) && optimisticIds.has(item.sessionId)),
        ],
    };
}
function agentStatusFromTool(tool) {
    ...
    if (rawOutput?.['status'] === 'cancelled')
        return 'cancelled';
    return rawOutput?.['status'] === 'failed' ? 'failed' : 'completed';
}
function agentTaskAsToolCall(task) {
    const status = task.status === 'running' || task.status === 'paused'
        ? 'in_progress'
        : task.status === 'failed'
            ? 'failed'
            : 'completed';
...
function isEnvironmentAgentToolCall(tool) {
    ...
    if (typeof tool.args?.subagent_type === 'string')
        return true;
    return (isRecord(tool.rawOutput) && tool.rawOutput['type'] === 'task_execution');
}
function derivedTaskIdForTool(tool) {
    const rawOutput = isRecord(tool.rawOutput) ? tool.rawOutput : undefined;
    const subagentName = typeof rawOutput?.['subagentName'] === 'string'
...
export function getEnvironmentAgentTasks(messages, sessionTasks) {
    ...
    for (const message of messages) {
        if (message.role !== 'system' || !isRecord(message.data))
            continue;
    ...
    const collectPreciseLinks = (tools) => {
        for (const tool of tools) {
            if (isEnvironmentAgentToolCall(tool) &&
                !envToolCallIds.has(tool.callId)) {
                envToolCallIds.add(tool.callId);
                const derivedTaskId = derivedTaskIdForTool(tool);
                if (derivedTaskId)
                    preciselyClaimedTaskIds.add(derivedTaskId);
            }
            if (tool.subTools)
                collectPreciseLinks(tool.subTools);
        }
    };
    for (const message of messages) {
        if (message.role === 'tool_group')
            collectPreciseLinks(message.tools);
    }
    const isPreciselyClaimed = (task) => (task.toolUseId != null && envToolCallIds.has(task.toolUseId)) ||
    ...
    const visit = (tools) => {
        for (const tool of tools) {
            if (isEnvironmentAgentToolCall(tool) &&
                ...
                const rawOutput = isRecord(tool.rawOutput) ? tool.rawOutput : undefined;
                const color = typeof rawOutput?.['subagentColor'] === 'string'
                    ? rawOutput['subagentColor']
                    : undefined;
                const description = typeof tool.args?.description === 'string'
                    ? tool.args.description
                    : undefined;
                const prompt = typeof tool.args?.prompt === 'string' ? tool.args.prompt : undefined;
                const subagentType = typeof tool.args?.subagent_type === 'string'
                    ? tool.args.subagent_type
                    : undefined;
                const subagentName = typeof rawOutput?.['subagentName'] === 'string'
                ...
                const liveTask = liveAgents.find((task) => task.toolUseId === tool.callId ||
                    task.id === taskId ||
                ...
                const meaningfulTitle = title && title.toLowerCase() !== 'agent' ? title : undefined;
                const label = meaningfulTitle ??
                    description ??
                    prompt ??
                    subagentName ??
                    subagentType ??
                    '';
                const taskDescription = description ?? prompt ?? subagentName ?? subagentType ?? '';
                const startTime = tool.startTime ?? 0;
                agents.push(liveTask
                    ? {
                        ...liveTask,
                        label,
                    ...
                    : {
                        ...
                        ...(tool.endTime !== undefined
                            ...
                            : {}),
                        runtimeMs: Math.max(0, (tool.endTime ?? tool.startTime ?? startTime) - startTime),
                        ...(subagentType ? { subagentType } : {}),
                        ...(color ? { color } : {}),
    ...
    for (const task of liveAgents) {
        ...
        const alreadyListed = agents.some((a) => (a.toolUseId != null && a.toolUseId === task.toolUseId) ||
...
function translateCopyMessage(message, t) {
    if (message === COPY_MESSAGES.NO_OUTPUT)
        return t('copy.noOutput');
    if (message === COPY_MESSAGES.NO_TEXT)
        return t('copy.noText');
    if (message === COPY_MESSAGES.CODE_MISSING)
        return t('copy.codeMissing');
    if (message === COPY_MESSAGES.LATEX_MISSING)
        return t('copy.latexMissing');
    if (message === COPY_MESSAGES.INLINE_LATEX_MISSING) {
        return t('copy.inlineLatexMissing');
    }
    if (message === COPY_MESSAGES.OUTPUT_COPIED)
...
export function App({ desiredSessionTargetPending = false, onSessionIdChange, onSessionCreated, theme: providedTheme, onThemeChange, language: providedLanguage, onLanguageChange, className: externalClassName, style: externalStyle, shadowDom, onConnectionChange, onStreamingStateChange, onError, onBugReport, hiddenSlashCommands, slashCommandCategoryOrder, onSlashCommand, builtinAtProviders, atProviders, composerTagIcons, fileUploadEnabled, renderToolHeaderExtra, renderWelcomeHeader, renderWelcomeFooter, mobileWelcomeFooterMiddle = false, parseUserMessageContent, renderUserMessageContent, renderComposerTag, renderComposerTagTooltip, onComposerTagClick, renderAssistantTurnFooter, renderComposerToolbarStart, renderComposerToolbarEnd, renderComposerToolbarRight, renderComposerHeader, renderComposerFooter, renderChatHeader, renderFooter, bottomStatusItems, chatMaxWidth, sidebar, header, rightPanel, environmentPanel, splitSessionIds: externalSplitSessionIds, onSplitSessionIdsChange, renderPaneHeaderActions, onRightPanelOpen, messageTurnOutputs, shellRef, composerToolbarActions, composerPlaceholders, compactThinking = false, collapseCompletedTurns = true, markdownTableMode = 'basic', virtualScrollThreshold, markdown, loadingPhrases, onTranscriptChange, onToast, composerRef, onComposerReady, composerInput, composerInputVersion, onSessionChange, onSubmitBefore, restartSseOnPrompt, historyPageSize, initialSelectedWorkspaceCwd, lockedWorkspaceCwd, lockedWorkspaceCapability, } = {}) {
    const [chatWidthMode, setChatWidthMode] = useState(readChatWidthMode);
    const [selectedLanguage, setSelectedLanguage] = useState(() => providedLanguage === undefined
        ? getInitialLanguage()
        : normalizeLanguage(providedLanguage));
    const t = useMemo(() => getTranslator(selectedLanguage), [selectedLanguage]);
    const shadowDomOptions = useMemo(() => resolveWebShellShadowDom(shadowDom), [shadowDom]);
    const sidebarOptions = useMemo(() => resolveSidebarOptions(sidebar), [sidebar]);
    ...
    const environmentPanelItems = environmentPanel?.items ?? DEFAULT_ENVIRONMENT_PANEL_ITEMS;
    ...
    // the legacy entries.
    const environmentPanelReachable = chatHeaderEnabled &&
        environmentHeaderItemVisible &&
        (!renderChatHeader || Boolean(header));
    const environmentGitReplacementEnabled = environmentPanelReachable && environmentPanelItems.includes('environment');
    const environmentTasksReplacementEnabled = environmentPanelReachable &&
        environmentPanelItems.includes('backgroundTasks');
    const [sidebarCollapsed, setSidebarCollapsed] = useState(() => readSidebarCollapsed(sidebarOptions.defaultCollapsed));
    const [sidebarSwitchingSessionId, setSidebarSwitchingSessionId] = useState(null);
    ...
    useEffect(() => {
        ...
        const onKey = (e) => {
            ...
            if (artifactPanelFullscreenRef.current)
                return;
        ...
        const preventScroll = (e) => {
            ...
            const el = e.target;
            if (el?.closest('[data-sidebar-shell]') &&
    ...
    const logicalSessionKey = getLogicalSessionKey(connection.sessionId, connection.workspaceCwd);
    const sessionWriteBlocked = desiredSessionTargetPending ||
    ...
    const workspaces = useMemo(() => {
        ...
        if (lockedWorkspaceCapability &&
            !capabilityWorkspaces.some((entry) => entry.cwd === lockedWorkspaceCapability.cwd)) {
    ...
    const nextComposerWorkspaces = !lockedWorkspaceCwd
        ? workspaces.map((entry) => ({
            id: entry.id,
            cwd: entry.cwd,
            label: entry.kind === 'live' ? t('sidebar.live') : workspaceLabel(entry),
            primary: entry.primary,
            trusted: entry.trusted,
        }))
        : undefined;
    const currentComposerWorkspaces = composerWorkspacesRef.current;
    if (currentComposerWorkspaces?.length !== nextComposerWorkspaces?.length ||
    ...
    const visibleWorkspaces = useMemo(() => lockedWorkspaceCwd
        ? workspaces.filter((entry) => entry.cwd === lockedWorkspaceCwd)
        : workspaces, [lockedWorkspaceCwd, workspaces]);
    ...
    const monitorDetailsSupported = connection.capabilities?.features.includes(SESSION_MONITOR_TOOL_CORRELATION_FEATURE) === true;
    const { notices, dismissNotice } = useSessionNotices();
    const workspaceActions = useWorkspaceActions();
    const artifactWorkspaceTarget = useArtifactWorkspaceTarget(connection.workspaceCwd);
    const dynamicWorkspaceRegistrationSupported = workspace.capabilities?.features?.includes('dynamic_workspace_registration') === true;
    const persistentWorkspaceRegistrationSupported = workspace.capabilities?.features?.includes('persistent_workspace_registration') === true;
    const scratchWorkspaceRegistrationSupported = workspace.capabilities?.features?.includes('scratch_workspace_registration') === true;
    const workspaceDisplayNameSupported = workspace.capabilities?.features?.includes('workspace_display_name') ===
        true;
    const gitHubPrsSupported = workspace.capabilities?.features?.includes('workspace_github_prs') === true;
    ...
    const [selectedWorkspaceCwd, setSelectedWorkspaceCwd] = useState(initialSelectedWorkspaceCwd);
    const selectedWorkspaceCwdRef = useRef(selectedWorkspaceCwd);
    selectedWorkspaceCwdRef.current = selectedWorkspaceCwd;
    const [selectedWorkspaceGitStatus, setSelectedWorkspaceGitStatus] = useState(undefined);
    useEffect(() => {
        if (!workspace.capabilities || !selectedWorkspaceCwd)
            return;
        const selected = workspaces.find((entry) => entry.cwd === selectedWorkspaceCwd);
        if (selected?.trusted)
            return;
    ...
    const [sessionStatusDisplayName, setSessionStatusDisplayName] = useState(undefined);
    ...
    useEffect(() => {
        ...
        workspace.client
            ...
            return loadSessionCatalogOnce(workspace.client, {
                routeKind: 'legacy',
                workspaceCwd: summary.workspaceCwd,
                options: { pageSize: 200 },
            }, { fresh: true })
                .then((page) => {
                if (worktreeSessionKeyRef.current !== sessionKey ||
                    !owner.isCurrent()) {
                    return;
                }
                const listedSession = page.sessions.find((session) => session.sessionId === sid);
                setSessionStatusDisplayName(listedSession?.displayName ?? summary.displayName);
            })
                .catch(() => undefined);
        })
            .catch(() => {
    ...
    const activeWorkspaceCwd = useMemo(() => connection.sessionId
        ? connection.workspaceCwd
        : (lockedWorkspaceCwd ??
    ...
    useEffect(() => {
        ...
        const fetchStatus = () => {
            ...
            void git
                ...
                .then((status) => {
                if (!cancelled) {
                    setSelectedWorkspaceGitStatus((current) => isSameGitStatus(current, status) ? current : status);
                }
            })
                .catch(() => {
                if (!cancelled)
    ...
    useEffect(() => {
        ...
        if (!status || sessionWorktree)
            return;
        if (status.workspaceCwd !== activeWorkspaceCwd)
            return;
    ...
    const pushToast = useCallback((tone, message) => {
        ...
        setToasts((current) => {
            const withoutDuplicate = current.filter((item) => item.tone !== tone || item.message !== message);
    ...
    useLayoutEffect(() => {
        ...
        if (previousOwner.sessionId === connection.sessionId &&
    ...
    const displayMessages = useMemo(() => {
        ...
        for (const localMessage of localMessages.sort((a, b) => a.anchorIndex - b.anchorIndex)) {
            const anchorIndex = localMessage.anchorAfterId
                ? result.findIndex((message) => message.id === localMessage.anchorAfterId)
                : -1;
    ...
    useEffect(() => {
        ...
        if (!failed)
            return;
        const currentFailedBlock = findUserMessageByIdentity(store.getSnapshot().blocks, failed.identity, failed.owner.snapshot.isCurrent());
        const failedBlock = findUserMessageByIdentity(blocks, failed.identity, failed.owner.snapshot.isCurrent());
        ...
        if (!failedBlock)
            return;
        const failedIndex = displayMessages.findIndex((message) => message.id === failedBlock.id);
        if (failedIndex < 0)
            return;
        if (displayMessages
            .slice(failedIndex + 1)
        ...
        if (failed.messageId !== failedBlock.id ||
    ...
    const { artifacts, loading: artifactsLoading, error: artifactsError, } = useSessionArtifacts();
    const [artifactPanelExtraArtifacts, setArtifactPanelExtraArtifacts] = useState([]);
    const [paneArtifactSnapshots, setPaneArtifactSnapshots] = useState(() => new Map());
    ...
    useEffect(() => {
        ...
        const openArtifactIds = new Set(artifactPanelTabs
            .filter((tab) => tab.kind === 'artifact')
            .map((tab) => (tab.kind === 'artifact' ? tab.artifactId : '')));
        setArtifactPanelExtraArtifacts((previous) => {
            const next = previous.filter((artifact) => !artifactIds.has(artifact.id) || openArtifactIds.has(artifact.id));
            return next.length === previous.length ? previous : next;
        });
    }, [artifacts, artifactPanelExtraArtifacts.length, artifactPanelTabs]);
    const paneArtifactExtras = useMemo(() => Array.from(paneArtifactSnapshots.values()).flatMap((snapshot) => [
        ...snapshot.artifacts,
    ]), [paneArtifactSnapshots]);
    const artifactPanelArtifacts = useMemo(() => {
        if (artifactPanelExtraArtifacts.length === 0 &&
    ...
    const handlePaneArtifactsChange = useCallback((paneSessionId, paneArtifacts) => {
        if (paneArtifacts.length > 0) {
            const paneArtifactIds = new Set(paneArtifacts.map((artifact) => artifact.id));
            ...
            const openArtifactIds = new Set(artifactPanelTabsRef.current
                .filter((tab) => tab.kind === 'artifact')
                .map((tab) => (tab.kind === 'artifact' ? tab.artifactId : '')));
            setArtifactPanelExtraArtifacts((current) => {
                const next = current.filter((artifact) => !paneArtifactIds.has(artifact.id) ||
        ...
        setPaneArtifactSnapshots((current) => {
            ...
            const unchanged = previous?.artifacts.length === paneArtifacts.length &&
                previous.artifacts.every((artifact, index) => {
                    ...
                    return (nextArtifact?.id === artifact.id &&
    ...
    const artifactsByTurn = useMemo(() => getArtifactsByTurn(displayMessages, artifacts, connection.workspaceCwd || ''), [displayMessages, artifacts, connection.workspaceCwd]);
    const fileChangesByTurn = useMemo(() => getFileChangesByTurn(displayMessages, artifactsByTurn, connection.workspaceCwd || ''), [displayMessages, artifactsByTurn, connection.workspaceCwd]);
    ...
    const scheduledTasksByTurn = useMemo(() => getScheduledTasksByTurn(displayMessages), [displayMessages]);
    ...
    const [activeArtifactPanelTabId, setActiveArtifactPanelTabId] = useState(null);
    ...
    const [artifactPanelWidth, setArtifactPanelWidth] = useState(DEFAULT_REVIEW_PANEL_WIDTH);
    ...
    // drawer handing back).
    const [suppressArtifactDockOpenAnimation, setSuppressArtifactDockOpenAnimation,] = useState(false);
    const [waitForSubagentPanelAnimation, setWaitForSubagentPanelAnimation] = useState(false);
    ...
    useEffect(() => {
        ...
        if (previousSessionId) {
            ...
            if (currentState) {
                artifactPanelStateBySessionRef.current.set(previousSessionId, currentState);
                if (artifactPanelStateBySessionRef.current.size >
    ...
    const sideTasksAvailable = Boolean(connection.sessionId && connection.workspaceCwd) &&
    ...
    const visibleSideTasks = sideTaskCatalog.parentSessionId === connection.sessionId
        ? sideTaskCatalog.items
        : [];
    const sideTasksLoading = visibleSideTasks.length === 0 &&
        (sideTaskCatalog.parentSessionId !== connection.sessionId ||
            !sideTaskCatalog.loaded);
    const nextSideTaskTabIdRef = useRef(0);
    const createSideTask = useCallback((initialPrompt) => {
        const parentSessionId = connection.sessionId;
        if (!parentSessionId || !sideTasksAvailable)
            return false;
        const tab = {
    ...
    const createSideTaskSession = useCallback(async (_tabId, parentSessionId, title) => {
        const ownerCwd = connection.workspaceCwd;
        const parentClientId = connection.sessionId === parentSessionId
            ? connection.clientId
            : undefined;
        const session = await workspace.client.createSideTaskSession(parentSessionId, {
            name: title,
        }, parentClientId);
        if (ownerCwd) {
    ...
    const handleSideTaskCreated = useCallback((tabId, sessionId) => {
        let createdTab = artifactPanelTabsRef.current.find((candidate) => candidate.id === tabId);
        setArtifactPanelTabs((tabs) => tabs.map((tab) => tab.id === tabId && tab.kind === 'side_task'
            ? { ...tab, sessionId }
            : tab));
        if (!createdTab) {
            ...
            for (const state of artifactPanelStateBySessionRef.current.values()) {
                const candidate = state.tabs.find((bucketTab) => bucketTab.id === tabId);
                if (!candidate)
                    continue;
                createdTab = candidate;
                state.tabs = state.tabs.map((bucketTab) => bucketTab.id === tabId && bucketTab.kind === 'side_task'
                    ? { ...bucketTab, sessionId }
                    : bucketTab);
                break;
            }
        }
        const sideTaskTab = createdTab?.kind === 'side_task' ? createdTab : undefined;
        if (!sideTaskTab)
            return;
    ...
    const handleSideTaskTitleChange = useCallback((tabId, title, fromFirstPrompt = false) => {
        const sideTaskTab = artifactPanelTabsRef.current.find((tab) => tab.id === tabId && tab.kind === 'side_task');
        const sessionId = sideTaskTab?.kind === 'side_task' ? sideTaskTab.sessionId : undefined;
        setArtifactPanelTabs((tabs) => tabs.map((tab) => {
            if (tab.id !== tabId || tab.kind !== 'side_task')
                return tab;
            if (!fromFirstPrompt && tab.title === title)
                return tab;
            return {
                ...tab,
                title,
        ...
        if (sessionId) {
            setSideTaskCatalog((catalog) => ({
                ...catalog,
                items: catalog.items.map((item) => item.sessionId === sessionId ? { ...item, title } : item),
            }));
        }
    }, []);
    const openSideTask = useCallback((sideTask) => {
        const parentSessionId = connection.sessionId;
        if (!parentSessionId)
            return;
        const tab = {
        ...
        setArtifactPanelTabs((tabs) => tabs.some((item) => item.kind === 'side_task' && item.sessionId === sideTask.sessionId)
            ? tabs
            : [...tabs, tab]);
        const existingTab = artifactPanelTabsRef.current.find((item) => item.kind === 'side_task' && item.sessionId === sideTask.sessionId);
    ...
    useEffect(() => {
        ...
        if (!artifactPanelOpen)
            return;
        setSideTaskCatalog((catalog) => catalog.parentSessionId === parentSessionId
            ? { ...catalog, loaded: false }
            : { parentSessionId, items: [], loaded: false });
        let cancelled = false;
        void loadSessionCatalogOnce(workspace.client, {
            routeKind: 'legacy',
            workspaceCwd,
            options: {
            ...
            const listedItems = page.sessions.map((session) => ({
                sessionId: session.sessionId,
                title: session.displayName?.trim() ||
            ...
            setSideTaskCatalog((catalog) => mergeSideTaskCatalog(catalog, parentSessionId, listedItems, optimisticSideTaskIdsRef.current));
        })
            .catch(() => {
            if (cancelled)
                return;
    ...
    const getMaxArtifactPanelWidth = useCallback(() => {
        ...
        if (!chatPaneWidth) {
            return Math.max(MIN_ARTIFACT_PANEL_WIDTH, window.innerWidth - MIN_CHAT_PANE_WIDTH_WITH_ARTIFACT_PANEL);
        }
        return Math.max(MIN_ARTIFACT_PANEL_WIDTH, artifactPanelWidth +
            chatPaneWidth -
            MIN_CHAT_PANE_WIDTH_WITH_ARTIFACT_PANEL);
    }, [artifactPanelWidth]);
    const getDefaultReviewPanelWidth = useCallback(() => {
        const chatPaneWidth = chatPaneRef.current?.getBoundingClientRect().width ?? window.innerWidth;
        const maxWidth = Math.max(MIN_ARTIFACT_PANEL_WIDTH, chatPaneWidth - MIN_CHAT_PANE_WIDTH_WITH_ARTIFACT_PANEL);
        return Math.min(maxWidth, Math.max(DEFAULT_REVIEW_PANEL_WIDTH, Math.round(chatPaneWidth * 0.56)));
    }, []);
    const openArtifactPanel = useCallback((artifactId, previewContent) => {
        if (!artifactId)
            return;
        const artifact = artifactPanelArtifacts.find((item) => item.id === artifactId);
        const tab = {
        ...
        setArtifactPanelTabs((tabs) => tabs.some((item) => item.id === tab.id)
            ? tabs.map((item) => item.id === tab.id ? { ...item, ...tab } : item)
            : [...tabs, tab]);
        setActiveArtifactPanelTabId(tab.id);
    ...
    const openReviewPanel = useCallback((changes, selectedPath, reviewWorkspaceCwd, reviewWorkspaceId, tabId = 'review') => {
        const reviewTab = {
            id: tabId,
            kind: 'review',
        ...
        setArtifactPanelTabs((tabs) => tabs.some((item) => item.id === reviewTab.id)
    ...
    const openLatestReviewPanel = useCallback(() => {
        if (latestReviewChanges.length === 0)
            return;
        openReviewPanel(latestReviewChanges, undefined, connection.workspaceCwd, artifactWorkspaceTarget?.workspaceId);
    }, [
        artifactWorkspaceTarget?.workspaceId,
        connection.workspaceCwd,
        latestReviewChanges,
    ...
    const openScheduledTaskPanel = useCallback((task, tabWorkspaceCwd, tabWorkspaceId, sourceSessionId) => {
        const tab = {
            id: sourceSessionId
    ...
    const openMonitorPanel = useCallback((task, sourceSessionId, sourceSessionActions) => {
        const tab = {
            id: sourceSessionId
        ...
        setArtifactPanelTabs((tabs) => tabs.some((item) => item.id === tab.id)
            ? tabs.map((item) => {
                if (item.id !== tab.id || item.kind !== 'monitor')
                    return item;
    ...
    const openImagePanel = useCallback((src, alt) => {
        const tab = {
            id: imageTabId(src),
            kind: 'image',
    ...
    const openShellPanel = useCallback((task, sourceSessionId, sourceSessionActions) => {
        const tab = {
            id: sourceSessionId
        ...
        setArtifactPanelTabs((tabs) => tabs.some((item) => item.id === tab.id)
            ? tabs.map((item) => {
                if (item.id !== tab.id || item.kind !== 'shell')
                    return item;
    ...
    const openSubagentPanelForSession = useCallback((tool, sessionId, workspaceCwd) => {
    ...
    const openSubagentPanel = useCallback((tool) => {
        if (!connection.sessionId)
            return;
        openSubagentPanelForSession(tool, connection.sessionId, connection.workspaceCwd);
    }, [
        connection.sessionId,
        connection.workspaceCwd,
        openSubagentPanelForSession,
    ]);
    const openEnvironmentAgent = useCallback((task) => {
        if (!connection.sessionId)
            return;
        if (!artifactPanelOpenRef.current) {
        ...
        const tool = task.toolUseId
            ...
            : undefined;
        openSubagentPanelForSession(tool ?? agentTaskAsToolCall(task), connection.sessionId, connection.workspaceCwd);
    }, [
        connection.sessionId,
        connection.workspaceCwd,
        messages,
    ...
    const handleTurnOutputOpen = useCallback((request) => {
        if (onRightPanelOpen) {
            onRightPanelOpen(request);
            return;
        }
        if (request.kind === 'review') {
            openReviewPanel(request.changes, request.selectedPath, request.workspaceCwd, request.workspaceId, request.sourceSessionId
                ? `review:${request.sourceSessionId}:${request.turnId}`
                : undefined);
            return;
        }
        if (request.kind === 'scheduled_task') {
            openScheduledTaskPanel(request.task, request.workspaceCwd, request.workspaceId, request.sourceSessionId);
            return;
        }
        ...
        if (request.kind === 'subagent') {
            openSubagentPanelForSession(request.tool, request.sessionId, request.workspaceCwd);
            return;
        }
        ...
        setArtifactPanelExtraArtifacts((current) => {
            const index = current.findIndex((artifact) => artifact.id === request.artifact.id);
            if (index < 0)
                return [...current, request.artifact];
            const next = [...current];
    ...
    const openFilePreview = useCallback((change, previewWorkspaceCwd, previewWorkspaceId) => {
    ...
    const closeArtifactPanel = useCallback(() => {
        ...
        setSideTaskCatalog((catalog) => catalog.items.length === 0 ? { ...catalog, loaded: false } : catalog);
    }, []);
    useLayoutEffect(() => {
        ...
        if (!artifactPanelOpen || artifactPanelFullscreen)
            return;
        const clampWidth = () => {
            setArtifactPanelWidth((width) => {
                const chatPaneWidth = chatPaneRef.current?.getBoundingClientRect().width ??
                    window.innerWidth - width;
                const maxWidth = Math.max(MIN_ARTIFACT_PANEL_WIDTH, width + chatPaneWidth - MIN_CHAT_PANE_WIDTH_WITH_ARTIFACT_PANEL);
    ...
    const closeArtifactPanelTab = useCallback((tabId) => {
        setArtifactPanelTabs((tabs) => {
            ...
            if (activeArtifactPanelTabIdRef.current === tabId) {
                ...
                const nextActive = nextTabs[Math.min(closedIndex, nextTabs.length - 1)] ?? nextTabs[0];
                setActiveArtifactPanelTabId(nextActive.id);
            }
            return nextTabs;
        });
    }, []);
    const handleArtifactPanelResizeStart = useCallback((event) => {
        ...
        const handlePointerMove = (moveEvent) => {
            pendingWidth = Math.min(maxWidth, Math.max(MIN_ARTIFACT_PANEL_WIDTH, startWidth - (moveEvent.clientX - startX)));
    ...
    const rawPendingApproval = useMemo(() => extractPendingPermission(blocks), [blocks]);
    const pendingApproval = useShallowMemo(rawPendingApproval);
    const canActOnPendingApproval = !(connection.catchingUp && sidebarSwitchingSessionId !== null);
    const pendingAskUserApproval = isAskUserPermission(pendingApproval)
        ? canActOnPendingApproval
            ? pendingApproval
            : null
        : null;
    const pendingToolApproval = pendingApproval && !isAskUserPermission(pendingApproval)
        ? canActOnPendingApproval
            ? pendingApproval
            : null
        : null;
    ...
    // dialogOpen prop.
    const approvalOverlayActive = pendingToolApproval !== null || pendingAskUserApproval !== null;
    ...
    const floatingTodosState = useMemo(() => getFloatingTodos(messages), [messages]);
    const approvalPlanTodos = useMemo(() => isExitPlanApprovalRequest(pendingToolApproval)
    ...
    const floatingTodos = useStableArray(floatingTodosState.todos, (todo) => JSON.stringify([todo.id, todo.status, todo.content, todo.blockedBy ?? []]));
    const floatingTodosAllCompleted = floatingTodosState.allCompleted;
    const [todoPanelMode, setTodoPanelMode] = useState('hidden');
    const nextTodoPanelMode = connection.catchingUp ||
    ...
    const floatingBottomStatusItems = bottomStatusItems ?? EMPTY_BOTTOM_STATUS_ITEMS;
    const showBottomPanels = showFloatingTodos || floatingBottomStatusItems.length > 0;
    ...
    useLayoutEffect(() => {
        ...
        const updateInset = () => {
            ...
            setBottomPanelInset(Math.max(BOTTOM_PANEL_FALLBACK_INSET_PX, Math.ceil(overlapAboveFooter)));
        };
        updateInset();
    ...
    const taskActivityKey = useMemo(() => getTaskActivityKey(messages), [messages]);
    const [backgroundTasksRefreshTrigger, setBackgroundTasksRefreshTrigger] = useState(0);
    const sessionTasks = useBackgroundTasks(connection.sessionId, taskActivityKey, connection.status === 'connected', backgroundTasksRefreshTrigger);
    const environmentAgentTasks = useMemo(() => getEnvironmentAgentTasks(messages, sessionTasks), [messages, sessionTasks]);
    const backgroundTasks = useMemo(() => sessionTasks.filter((task) => task.kind !== 'agent'), [sessionTasks]);
    const monitorDetailsSessionIdRef = useRef(connection.sessionId);
    monitorDetailsSessionIdRef.current = connection.sessionId;
    const openMonitorPanelFromTool = useCallback(async (tool) => {
    ...
    useEffect(() => {
        const monitors = new Map(backgroundTasks
    ...
    useEffect(() => {
        const shellTasks = new Map(backgroundTasks
    ...
    const footerTasks = useMemo(() => (renderFooter ? backgroundTasks.map(mapToWebShellTaskInfo) : []), [backgroundTasks, renderFooter]);
    const statusBarRef = useRef(null);
    const messageListRef = useRef(null);
    const editorRef = useRef(null);
    const notifiedComposerReadyRef = useRef(null);
    const [canScrollMessageListToBottom, setCanScrollMessageListToBottom] = useState(false);
    ...
    const setEditorHandle = useCallback((handle) => {
    ...
    const [isCreatingMissingSession, setIsCreatingMissingSession] = useState(false);
    ...
    const [isStartingNewSessionSuggestion, setIsStartingNewSessionSuggestion] = useState(false);
    const streamingState = useStreamingState();
    const failedPromptRetryIsCurrent = Boolean(failedPromptRetry &&
        retryOwnerMatchesCurrent(failedPromptRetry.owner, connection.sessionId, connection.workspaceCwd, composerSourceVersionRef.current) &&
        retryTranscriptIdentityMatches(blocks, failedPromptRetry.transcriptIdentity));
    useEffect(() => {
        if (failedPromptRetry &&
    ...
    const suppressFailedPromptRetryStreaming = Boolean(failedPromptRetry &&
        failedPromptRetryIsCurrent &&
    ...
    if (previousStreamingStateRef.current === 'idle' &&
        streamingState !== 'idle') {
    ...
    const lastSubmittedSourceVersionRef = useRef(composerSourceVersionRef.current);
    ...
    const rearmFailedTurnErrorRetry = useCallback((retryableTurnError, currentBlocks) => {
        const failedRetry = failedTurnErrorRetryRef.current;
        if (!failedRetry || retryableTurnError.id === failedRetry.errorId) {
            return;
        }
        if (!retryOwnerMatchesCurrent(failedRetry.owner, connectionRef.current.sessionId, connectionRef.current.workspaceCwd, composerSourceVersionRef.current)) {
            failedTurnErrorRetryRef.current = null;
            return;
        }
        if (!currentBlocks.some((block) => block.kind === 'error' &&
            block.source === 'turn_error' &&
    ...
    useLayoutEffect(() => {
        ...
        const workspaceBecameKnown = previousOwner.sessionId !== undefined &&
        ...
        if (workspaceBecameKnown && logicalSessionKey) {
            ...
            const previousStates = previousSessionKey !== undefined
                ? cancelledRetryStatesRef.current.get(previousSessionKey)
                : undefined;
            if (previousSessionKey &&
                previousStates &&
                previousSessionKey !== logicalSessionKey) {
                const currentStates = cancelledRetryStatesRef.current.get(logicalSessionKey) ?? [];
                cancelledRetryStatesRef.current.set(logicalSessionKey, mergeCancelledRetryEntries(currentStates, previousStates.map((previous) => ({ state: previous.state }))));
                cancelledRetryStatesRef.current.delete(previousSessionKey);
            }
            return;
        }
        if (retryOwnerMatchesCurrent(previousOwner, connection.sessionId, connection.workspaceCwd, composerSourceVersionRef.current)) {
            return;
        }
    ...
    const applyCancelledRetryState = useCallback((state) => {
        ...
        if (state.kind === 'failed-prompt') {
            ...
            if (!matchesUserMessageIdentity(latestUserMessage, failed.identity)) {
                if (!matchesUserMessageIdentity(latestUserMessage, failed.previousIdentity)) {
                    return 'invalid';
                }
                store.appendLocalUserMessage(failed.text, failed.images?.map((image) => ({
                    data: image.data,
                    mimeType: image.media_type,
                })), failed.inputAnnotations?.length
                    ? { inputAnnotations: failed.inputAnnotations }
                    : undefined);
                const rehydratedMessage = getLatestUserBlock(store.getSnapshot().blocks);
            ...
            if (!displayMessages.some((message) => message.id === failed.messageId)) {
                return 'pending';
            }
            failed = {
                ...failed,
    ...
    const restoreOrDeferCancelledRetry = useCallback((owner, state) => {
        ...
        const states = mergeCancelledRetryEntries(cancelledRetryStatesRef.current.get(resolvedSessionKey) ?? [], [
            {
    ...
    useLayoutEffect(() => {
        ...
        const exactStates = cancelledRetryStatesRef.current.get(logicalSessionKey) ?? [];
        ...
        if (pendingExactStates.length > 0) {
            cancelledRetryStatesRef.current.set(logicalSessionKey, pendingExactStates);
        }
        else {
    ...
    const reloadLoadedSkills = useCallback(async (workspaceCwd) => {
        const request = ++loadedSkillsRequestRef.current;
        try {
            const status = workspaceCwd && workspace.client
                ? await workspace.client
    ...
    const useFloatingArtifactPanel = !canDockArtifactPanel || mainView === 'split';
    const deferSubagentMount = waitForSubagentPanelAnimation &&
        artifactPanelOpen &&
    ...
    useEffect(() => {
        if (!deferSubagentMount)
            return;
        const timeout = window.setTimeout(() => setWaitForSubagentPanelAnimation(false), SUBAGENT_PANEL_ANIMATION_FALLBACK_MS);
    ...
    const dockedFullscreenActive = artifactPanelOpen && !useFloatingArtifactPanel && artifactPanelFullscreen;
    ...
    useLayoutEffect(() => {
        ...
        if (!slot || !dockedFullscreenActive)
            return;
    ...
    useLayoutEffect(() => {
        ...
        if (!dockedFullscreenActive || !surface)
            return;
        const hidden = [];
        let node = surface;
        while (node && node !== document.body) {
            ...
            for (const sibling of Array.from(parent.children)) {
                ...
                if (sibling.matches('[data-web-shell-toast-host]'))
                    continue;
                hidden.push({
                    element: sibling,
        ...
        if (!surface.contains(surfaceActive))
            surface.focus();
    ...
    const handleArtifactPanelSurfaceKeyDown = useCallback((event) => {
        if (!artifactPanelFullscreen)
            return;
        if (event.key !== 'Tab' ||
            event.altKey ||
        ...
        if (!first || !last) {
            if (focused === event.currentTarget)
                event.preventDefault();
            return;
        }
    ...
    useEffect(() => {
        if (!artifactPanelOpen || !useFloatingArtifactPanel)
            return;
        const preserveImeEscape = (event) => {
            ...
            document.addEventListener('keydown', (currentEvent) => {
    ...
    const handleUseSkill = useCallback((name) => {
        closePanel();
        window.setTimeout(() => {
    ...
    const openSplitView = useCallback((sessionIds) => {
        setActivePanel(null);
    ...
    const externalSplitSignature = useMemo(() => {
        const requested = Array.from(new Set((externalSplitSessionIds ?? []).filter(Boolean))).slice(0, MAX_SPLIT_PANES);
    ...
    const requestOpenSplitView = useCallback(() => {
        ...
        const requested = splitSessionIds.length > 0
            ? splitSessionIds
            : connection.sessionId
                ? [connection.sessionId]
                : [];
    ...
    useEffect(() => {
        ...
        const requested = externalSplitSignature
            ...
            : [];
        setSplitSessionIds((prev) => areSessionIdsEqual(prev, requested) ? prev : requested);
        if (requested.length > 0) {
    ...
    const handleSplitPanesChange = useCallback((sessionIds) => {
        if (!externalSplitControlled) {
    ...
    useEffect(() => {
        if (mainView !== 'chat' || !focusComposerAfterSplitCloseRef.current)
            return;
        focusComposerAfterSplitCloseRef.current = false;
        if (!activePanel && !approvalOverlayActive)
            editorRef.current?.focus();
    ...
    const toolApprovalOverlayVisible = pendingToolApproval !== null &&
        !activePanel &&
        modelDialogMode === null &&
        !showApprovalModeDialog &&
        mainView === 'chat' &&
        !artifactPanelFullscreen;
    const askUserOverlayVisible = pendingAskUserApproval !== null &&
        !activePanel &&
        modelDialogMode === null &&
        !showApprovalModeDialog &&
        mainView === 'chat' &&
    ...
    const [externalInteractionBlockCount, setExternalInteractionBlockCount] = useState(0);
    ...
    const extensionsVersionRef = useRef(workspaceEventSignals?.extensionsVersion ?? 0);
    useEffect(() => {
        ...
        if (current !== extensionsVersionRef.current) {
            ...
            if (change?.status === 'installed') {
                ...
                const name = change.name ?? change.source ?? t('extensions.label');
                const key = change.status === 'updated' && change.version
    ...
    const planAgentTools = useMemo(() => tasksDialogMessage
        ? getAgentToolsForPlan(messages, floatingTodosState)
        : [], [floatingTodosState, messages, tasksDialogMessage]);
    const handleOpenMonitorDetails = useCallback((task) => {
        setTasksDialogMessage(null);
        setBackgroundTasksRefreshTrigger((value) => value + 1);
        openMonitorPanel(task);
    }, [openMonitorPanel]);
    const handleOpenShellDetails = useCallback((task) => {
        setTasksDialogMessage(null);
        setBackgroundTasksRefreshTrigger((value) => value + 1);
        openShellPanel(task);
    }, [openShellPanel]);
    const [selectedTheme, setSelectedTheme] = useState(providedTheme ?? WebShellThemeId.Dark);
    ...
    const refreshActiveSessionDisplayName = useCallback(async () => {
        ...
        if (!activeConnection.sessionId || !activeConnection.workspaceCwd)
            return;
        const owner = sessionOwnerGuard.capture();
        try {
            const page = await loadSessionCatalogOnce(workspace.client, {
                routeKind: 'legacy',
            ...
            const displayName = page.sessions.find((session) => session.sessionId === activeConnection.sessionId)?.displayName;
            if (displayName?.trim())
    ...
    const refreshActiveSessionDisplayNameRef = useRef(refreshActiveSessionDisplayName);
    ...
    const scheduleDelayedActiveSessionDisplayNameRefresh = useCallback((sessionId, workspaceCwd) => {
    ...
    const finishPromptPreparation = useCallback((owner) => {
        if (!owner || promptPreparationOwnerRef.current !== owner)
            return;
    ...
    const ensureSessionForPrompt = useCallback(() => {
        ...
        const promise = (async () => {
            ...
            const modelId = currentModelRef.current || connectionRef.current.currentModel;
            const modeId = currentModeRef.current || connectionRef.current.currentMode;
            ...
            const acceptedWorkspaceCwd = requestedWorkspaceCwd
                ? workspaces.find((entry) => entry.cwd === requestedWorkspaceCwd && entry.trusted === true)?.cwd
                : undefined;
            const targetWorkspaceCwd = lockedWorkspaceCwd ?? acceptedWorkspaceCwd ?? primaryWorkspaceCwd;
            const catalogWorkspaceCwd = targetWorkspaceCwd ??
            ...
            try {
                await createAndAttachSessionForPrompt({
                    ...
                    worktree: gitModeIntentRef.current.mode === 'worktree'
                        ? { slug: gitModeIntentRef.current.slug }
                        : undefined,
                    branch: gitModeIntentRef.current.mode === 'branch'
                    ...
                    onSessionAllocated: (sessionId) => {
                        ...
                        if (catalogWorkspaceCwd) {
                            ...
                            sessionCatalogController.sessionCreated(catalogWorkspaceCwd, sessionId);
                        }
                    },
    ...
    const retryOwnerIsCurrent = useCallback((owner) => retryOwnerMatchesCurrent(owner, connectionRef.current.sessionId, getComposerWorkspaceCwd(), composerSourceVersionRef.current), [getComposerWorkspaceCwd]);
    const dispatchSessionChange = useCallback((event) => {
    ...
    const sendPrompt = useCallback(async (text, images, opts) => {
        if (sessionWriteBlockedRef.current) {
            throw new DOMException('Session switch is still preparing', 'InvalidStateError');
        }
        ...
        const admissionOwnerIsCurrent = (allocatedSessionId) => {
            if (!appMountedRef.current)
                return false;
            const currentSessionId = connectionRef.current.sessionId;
            const sessionMatches = currentSessionId === admissionSource.sessionId ||
                (admissionSource.sessionId === undefined &&
                    (currentSessionId === undefined ||
                        (allocatedSessionId !== undefined &&
                            currentSessionId === allocatedSessionId)));
            const ownAllocationSucceeded = admissionSource.sessionId === undefined &&
                allocatedSessionId !== undefined &&
                (currentSessionId === undefined ||
                    currentSessionId === allocatedSessionId);
            return ((admissionSource.owner.isCurrent() || ownAllocationSucceeded) &&
                sessionMatches &&
        ...
        const admissionSourceIsCurrent = (allocatedSessionId) => admissionOwnerIsCurrent(allocatedSessionId) &&
        ...
        if (submitBefore) {
            ...
            try {
                ...
                if (!appMountedRef.current)
                    return;
                console.warn('[web-shell] onSubmitBefore rejected, prompt cancelled', err);
        ...
        const sessionIdAfterEnsure = connectionRef.current.sessionId ?? allocatedSessionId;
        ...
        if (!opts?.retry &&
            ...
            retryOwnerRef.current = {
                ...
                sessionKey: getLogicalSessionKey(sessionIdAfterEnsure, promptWorkspaceCwd),
                sourceVersion: composerSourceVersionRef.current,
                snapshot: sessionOwnerGuard.capture(),
            };
        }
        ...
        finishPreparing();
        if (opts?.ownerRef)
            opts.ownerRef.current = sessionOwnerGuard.capture();
        clearFollowup();
        ...
        const promptOptions = {
            ...
            onAdmissionStarted: () => {
                ...
                opts?.onAdmissionStarted?.(connectionRef.current.sessionId ?? allocatedSessionId);
            },
            onAdmitted: () => {
                admitted = true;
                if (sessionIdAfterEnsure && promptWorkspaceCwd) {
                    sessionCatalogController.promptAdmitted(promptWorkspaceCwd, sessionIdAfterEnsure);
                }
                opts?.onAdmitted?.();
            },
        };
    ...
    const resolveSessionForWorkspace = useCallback(async (cwd, forceCreate) => {
        try {
            if (!forceCreate) {
                ...
                if (page.sessions.length > 0)
                    return page.sessions[0].sessionId;
            }
            // No session exists or forced: create one.
            const result = await sessionActions.createSession({ workspaceCwd: cwd });
    ...
    const availableModels = useMemo(() => (connection.models ?? []).filter(isVisibleComposerModel).map((m) => ({
        id: m.id,
    ...
    const gitModeEligible = Boolean(!connection.sessionId &&
    ...
    const dialogOpen = showResumeDialog ||
        showDeleteDialog ||
        showReleaseDialog ||
        showRewindDialog ||
        showHelpDialog ||
    ...
    const interactionBlocked = dialogOpen || mainView !== 'chat' || artifactPanelFullscreen;
    const mainVoiceTarget = useMemo(() => resolveVoiceWorkspaceTarget({
        capabilities: workspace.capabilities,
        intendedCwd: activeWorkspaceCwd,
        sessionId: connection.sessionId,
        workspaces: workspace.capabilities?.workspaces || lockedWorkspaceCapability
            ? workspaces
            : undefined,
    }), [
        activeWorkspaceCwd,
    ...
    const voiceModelSettingsSupported = supportsVoiceModelSettings(mainVoiceTarget, workspace.capabilities?.features ?? []);
    const matchingVoiceSettingsVersion = mainVoiceTarget?.sessionId === connection.sessionId &&
        (!mainVoiceTarget?.cwd || mainVoiceTarget.cwd === connection.workspaceCwd)
        ? workspaceEventSignals?.settingsVersion
        : undefined;
    const { descriptor: qualifiedVoiceSetting, reload: reloadQualifiedVoiceSettings, } = useVoiceWorkspaceSettings(workspace.client, mainVoiceTarget, voiceModelSettingsSupported && mainView !== 'split', JSON.stringify([
        voiceUserRevision,
    ...
    const handleFailedPromptRetry = useCallback(() => {
        ...
        if (!currentFailedBlock ||
            !matchesUserMessageIdentity(currentFailedBlock, failed.identity, retryOwner.snapshot.isCurrent())) {
            updateFailedPrompt(null);
            return;
        }
        if (currentFailedBlock !== failed.identity.block ||
        ...
        const retryTranscriptIsCurrent = () => retryTranscriptIdentityMatches(store.getSnapshot().blocks, retryTranscriptIdentity, retryOwner.snapshot.isCurrent());
        ...
        sendPrompt(failed.text, failed.images, {
            ...
            onAdmitted: () => {
                ...
                if (!retryOwnerIsCurrent(retryOwner))
                    return;
                setFailedPromptRetry((current) => current?.transcriptIdentity === retryTranscriptIdentity
            ...
            if (admissionStarted && !admitted && !definitelyRejected) {
                ...
                console.warn('[WebShell] prompt retry admission outcome is unknown', error);
                return;
            }
            if (!admitted) {
                restoreOrDeferCancelledRetry(retryOwner, {
    ...
    const { queuedPrompts, queuedTexts, enqueuePrompt: rawEnqueuePrompt, removeQueuedPrompt, editQueuedPrompt, editLastQueuedPrompt, clearQueuedPrompts, restoreUnknownQueuedPrompt, discardUnknownQueuedPrompt, } = useQueuedPrompts({
        connected,
    ...
    const enqueuePrompt = useCallback((text, images, onComplete, commitComposerAccepted, inputAnnotations) => {
        if (onSubmitBeforeRef.current) {
            ...
            onSubmitBeforeRef
                ...
                const result = rawEnqueuePrompt(text, images, onComplete, inputAnnotations);
                if (result !== false) {
                ...
                if (sourceSessionId && (text.trim() || (images?.length ?? 0) > 0)) {
                    ...
                    if (sourceWorkspaceCwd) {
                        sessionCatalogController.invalidateWorkspace(sourceWorkspaceCwd);
                    }
                }
            })
                .catch((err) => {
                console.warn('[web-shell] onSubmitBefore rejected queued prompt, cancelled', err);
            });
            return false;
        }
        const result = rawEnqueuePrompt(text, images, onComplete, inputAnnotations);
    ...
    const runVisibleRecap = useCallback(() => {
        if (sessionWriteBlocked)
            return;
        if (!requireActiveSessionForLocalCommand())
            return;
        ...
        sessionActions.recapSession().then((result) => {
    ...
    const runVisibleBtw = useCallback((rawQuestion) => {
        if (sessionWriteBlocked)
            return;
        ...
        sessionActions
            ...
            if (currentSessionIdRef.current !== sessionId ||
                connectionRef.current.workspaceCwd !== workspaceCwd)
                return;
            if (btwAbortControllerRef.current !== abortController)
                return;
            btwAbortControllerRef.current = null;
            setBtwMessage({
                id: messageId,
    ...
    useEffect(() => {
        const onBtwShortcut = (e) => {
            if (interactionBlocked || pendingApproval)
                return;
            const message = btwMessage;
            if (!message || message.role !== 'btw')
                return;
            const key = e.key.toLowerCase();
            const isPlainEscape = e.key === 'Escape' &&
                !e.ctrlKey &&
                !e.metaKey &&
                !e.altKey &&
                !e.shiftKey;
            const isCtrlCancel = e.ctrlKey &&
                !e.metaKey &&
                !e.altKey &&
                !e.shiftKey &&
                (key === 'c' || key === 'd');
            if (message.isPending) {
                if (!isPlainEscape && !isCtrlCancel)
                    return;
            }
            else {
                const editorHasText = editorRef.current?.hasInput() ?? false;
                const isPlainDismiss = !e.ctrlKey &&
                    !e.metaKey &&
                    !e.altKey &&
                    !e.shiftKey &&
                    (e.key === 'Escape' ||
    ...
    const echoOrDeferLocalCommand = useCallback((text, images) => appendOrDeferLocalUserMessage(streamingStateRef.current !== 'idle', text, images, {
    ...
    const echoLocalCommandIfIdle = useCallback((text) => {
    ...
    // usage frames.
    const dispatchReadOnlyStatus = useCallback((text) => {
    ...
    const { settings: workspaceSettings, setValue: setWorkspaceSetting, reload: reloadWorkspaceSettings, } = workspaceSettingsState;
    const liveSetup = useLiveVoiceSetup(workspaceSettings.some((setting) => setting.key === 'experimental.liveVoice.enabled'));
    const sessionWorkflowEnabled = workspaceSettings.find((setting) => setting.key === 'experimental.sessionWorkflow')?.values.effective === true;
    ...
    const targetedWorkspaceSettings = useMemo(() => {
        const withoutVoice = workspaceSettings.filter((setting) => setting.key !== 'voiceModel');
        if (mainView === 'split')
            return withoutVoice;
        if (mainVoiceTarget?.route === 'legacy-primary')
            return workspaceSettings;
        if (!voiceModelSettingsSupported || !qualifiedVoiceSetting) {
            return withoutVoice;
        }
        const voiceIndex = workspaceSettings.findIndex((setting) => setting.key === 'voiceModel');
        if (voiceIndex < 0) {
            return [...withoutVoice, qualifiedVoiceSetting];
        }
        return workspaceSettings.map((setting) => setting.key === 'voiceModel' ? qualifiedVoiceSetting : setting);
    }, [
        mainView,
        mainVoiceTarget?.route,
    ...
    const themeSetting = workspaceSettings.find((setting) => setting.key === THEME_SETTING_KEY);
    const hideTipsSetting = workspaceSettings.find((setting) => setting.key === HIDE_TIPS_SETTING_KEY);
    const languageSetting = workspaceSettings.find((setting) => setting.key === LANGUAGE_SETTING_KEY);
    const compactModeSetting = workspaceSettings.find((setting) => setting.key === COMPACT_MODE_SETTING_KEY);
    const currentVoiceModel = (() => {
        const value = readScopedModelSetting(targetedWorkspaceSettings, modelSettingScope, 'voiceModel');
        return typeof value === 'string' && value.trim() ? value.trim() : undefined;
    })();
    const currentVisionModel = (() => {
        const value = readScopedModelSetting(workspaceSettings, modelSettingScope, 'visionModel');
        if (typeof value !== 'string' || !value.trim())
            return undefined;
        return decodeVisionModelForPicker(value.trim());
    })();
    const currentFastModel = (() => {
        const value = readScopedModelSetting(workspaceSettings, modelSettingScope, 'fastModel');
        return typeof value === 'string' && value.trim() ? value.trim() : undefined;
    })();
    const currentModelFallbacks = useMemo(() => {
        const value = readScopedModelSetting(workspaceSettings, modelSettingScope, 'modelFallbacks');
    ...
    const writeVoiceModelForTarget = useCallback(async (value, scope, target = mainVoiceTarget) => {
        if (!target ||
            !supportsVoiceModelSettings(target, workspace.capabilities?.features ?? [])) {
            throw new Error('Voice model settings are unavailable for this workspace.');
        }
        try {
    ...
    const openVoiceModelPicker = useCallback(async (scope, source) => {
        const target = mainVoiceTargetRef.current;
        if (!target ||
            !supportsVoiceModelSettings(target, workspace.capabilities?.features ?? [])) {
            reportError(new Error('Voice model settings are unavailable for this workspace.'), t('model.setVoice'));
            return;
        }
        ...
        const intentIsCurrent = () => request === voicePickerRequestRef.current &&
            mainVoiceTargetRef.current?.ownerKey === target.ownerKey &&
            supportsVoiceModelSettings(mainVoiceTargetRef.current, voiceFeaturesRef.current) &&
            (source === 'settings'
    ...
    useEffect(() => {
        if (pendingVoicePickerSourceRef.current &&
    ...
    useEffect(() => {
        ...
        const sourceSurfaceIsCurrent = source === 'settings' ? activePanel === 'settings' : activePanel === null;
    ...
    useEffect(() => {
        ...
        const settingTheme = themeSettingToWebShellTheme(themeSetting?.values.effective);
        if (settingTheme) {
    ...
    useEffect(() => {
        ...
        const settingLanguage = languageSettingToWebShellLanguage(languageSetting?.values.effective);
        if (settingLanguage) {
            setSelectedLanguage(settingLanguage);
        }
    }, [providedLanguage, languageSetting?.values.effective]);
    const handleSettingsLanguageChange = useCallback((nextLanguage, scope = 'user') => {
        if (sessionWriteBlocked)
            return;
    ...
    const handleToggleCompact = useCallback(() => {
        ...
        setCompactMode(next);
        setWorkspaceSetting('workspace', COMPACT_MODE_SETTING_KEY, next).catch((error) => {
            setCompactMode(previous);
            reportError(error, t('compact.saveFailed'));
        });
    }, [reportError, setWorkspaceSetting, t]);
    const handleSetMode = useCallback((modeId) => {
        if (sessionWriteBlocked)
            return;
        if (!isDaemonApprovalMode(modeId)) {
            reportError(new Error(`Unsupported approval mode: ${modeId}`), t('local.approvalMode'));
            return;
        }
        if (!connectionRef.current.sessionId) {
        ...
        sessionActions
            ...
            if (!approval)
                return;
            const shouldAutoApprove = modeId === 'yolo' ||
                (modeId === 'auto-edit' && isEditToolPermission(approval));
            if (shouldAutoApprove) {
                const allowOnce = approval.options.find((o) => o.kind === 'allow_once');
                if (allowOnce) {
    ...
    useEffect(() => {
        if (prevQueueSessionIdRef.current === logicalSessionKey)
            return;
                ...
                while (batch.length > 0) {
                    for (let i = 0; i < batch.length; i++) {
                        ...
                        if (generationChanged ||
                            ...
                            console.warn('[web-shell] dropping %d queued shell command(s)', dropped);
                            pushToast('warning', t('queue.shellDropped', { count: dropped }));
                            return;
                        }
                        try {
                            ...
                            reportError(error, `Failed to execute shell command: !${batch[i]}`);
                        }
                        finally {
    ...
    useEffect(() => {
        ...
        const identityMatches = previousIdentity === undefined ||
    ...
    useEffect(() => {
        ...
        if (streamingState !== 'idle') {
            ...
            else if (connection.sessionId === streamingSessionIdRef.current &&
        ...
        if (prev !== 'idle' && streamingState === 'idle') {
            ...
            const turnError = retryableTurnErrorIdRef.current != null
            ...
            if (workspaceCwd) {
                ...
                if (!connectionRef.current.displayName) {
                    scheduleDelayedActiveSessionDisplayNameRefresh(sessionId, workspaceCwd);
                    if (typeof document !== 'undefined' && document.hidden) {
    ...
    useEffect(() => {
        ...
        const reportedWorkspaceCwd = connection.sessionId
            ? connection.workspaceCwd
            : activeWorkspaceCwd;
        const activeWorkspace = workspaces.find((entry) => entry.cwd === reportedWorkspaceCwd);
        if (connection.sessionId && !workspace.capabilities)
            return;
        const workspaceId = activeWorkspace && !activeWorkspace.primary
            ? activeWorkspace.id
            : undefined;
        if (lastNotifiedSessionIdRef.current === connection.sessionId &&
        ...
        lastNotifiedWorkspaceIdRef.current = workspaceId;
        lastNotifiedWorkspaceCwdRef.current = reportedWorkspaceCwd;
        onSessionIdChange?.(connection.sessionId, workspaceId, reportedWorkspaceCwd);
    }, [
    ...
    const reconcileCatalogRename = useCallback((workspaceCwd, sessionId, displayName) => {
    ...
    useEffect(() => {
        ...
        if (displayName === lastRenameNameRef.current)
            return;
        lastRenameNameRef.current = displayName;
        const reconciled = lastReconciledRenameRef.current;
        lastReconciledRenameRef.current = undefined;
        const alreadyReconciled = reconciled !== undefined &&
        ...
        if (!alreadyReconciled) {
            if (connection.workspaceCwd) {
                sessionCatalogController.renamed(connection.workspaceCwd, sessionId, displayName);
            }
        }
        dispatchSessionChangeRef.current?.({
            type: 'rename',
    ...
    useEffect(() => {
        ...
        setActiveGoal((current) => {
            ...
            if (current?.condition === nextGoal.condition &&
    ...
    useEffect(() => {
        ...
        function onVisibilityChange() {
            ...
            sessionActions.recapSession().then((result) => {
                const currentUserBlockId = getLatestUserBlockId(store.getSnapshot().blocks);
                ...
                if (autoRecapVersionRef.current !== version ||
                    !owner.isCurrent() ||
    ...
    const showContextUsage = useCallback((commandText, detail) => {
    ...
    const handleShowContextUsage = useCallback(() => showContextUsage('/context', false), [showContextUsage]);
    ...
    const branchCurrentSession = useCallback((name) => {
        if (sessionWriteBlocked)
            return;
        if (!requireActiveSessionForLocalCommand())
            return;
    ...
    const scheduleComposerFocus = useCallback((sessionId) => {
        ...
        window.setTimeout(() => {
            if (request !== composerFocusRequestRef.current ||
                approvalOverlayActiveRef.current ||
                (sessionId !== undefined &&
                    connectionRef.current.sessionId !== sessionId)) {
                return;
    ...
    const createNewSession = useCallback(async (workspaceCwd, 
    /**
     ...
     */
    const switchWorkspace = useCallback(async (workspaceCwd, acceptedWorkspaces = workspacesRef.current) => {
        if (workspaceSwitchTokenRef.current)
            return;
        const token = Symbol('workspace-switch');
        workspaceSwitchTokenRef.current = token;
        try {
            const primaryCwd = acceptedWorkspaces.find((entry) => entry.primary)?.cwd;
            const targetCwd = workspaceCwd ?? primaryCwd;
            if (workspaceCwd) {
                const target = acceptedWorkspaces.find((entry) => entry.cwd === workspaceCwd);
                if (!target?.trusted)
                    return;
            }
            if (connectionRef.current.sessionId) {
                if (connectionRef.current.workspaceCwd === targetCwd)
                    return;
    ...
    const reconcileAddedWorkspace = useCallback(async (canonicalCwd) => {
        try {
            const capabilities = await refreshWorkspaceCapabilities?.();
            const acceptedWorkspaces = capabilities?.workspaces ?? [];
            const added = acceptedWorkspaces.find((entry) => entry.cwd === canonicalCwd);
            if (added?.trusted) {
                await switchWorkspace(added.primary ? undefined : added.cwd, acceptedWorkspaces);
            }
    ...
    const handleAddWorkspace = useCallback(async (cwd, persist, displayName) => {
        ...
        try {
            const effectivePersist = persistentWorkspaceRegistrationSupported === true && persist;
    ...
    const refreshScratchOutcome = useCallback(async () => {
        ...
        try {
            ...
            if (committedCwd) {
                const added = acceptedWorkspaces.find((entry) => entry.cwd === committedCwd);
                if (added?.trusted) {
                    await switchWorkspace(added.primary ? undefined : added.cwd, acceptedWorkspaces);
                }
    ...
    const handleCreateScratchWorkspace = useCallback(async () => {
        if (scratchOutcomeUnknownRef.current !== 'clear' ||
        ...
        try {
            ...
            const definitelyRejected = error instanceof DaemonHttpError &&
    ...
    const handleSelectComposerWorkspace = useCallback((cwd) => {
    ...
    const generateSuggestionContent = useCallback((prompt, options) => {
        void logicalSessionKey;
        return sessionActions.generateSessionContent(prompt, options);
    }, [logicalSessionKey, sessionActions]);
    const { suggestion: newSessionSuggestion, updateInput: updateNewSessionSuggestionInput, dismiss: dismissNewSessionSuggestion, suppress: suppressNewSessionSuggestion, } = useNewSessionSuggestion({
        enabled: connection.capabilities?.features.includes('session_generation') === true,
        messages,
        sessionId: connection.sessionId,
        contextUsageRatio: (connection.contextWindow ?? 0) > 0
    ...
    const flushPendingNewSessionSuggestionSubmit = useCallback((expectedToken) => {
    ...
    const handleAcceptNewSessionSuggestion = useCallback(() => {
        ...
        if (!draft || isStartingNewSessionSuggestion)
            return;
        if (newSessionSuggestion?.suggestion !== 'new_session' ||
    ...
    const handleMissingSessionNewSession = useCallback(async () => {
        if (creatingMissingSessionRef.current)
            return;
    ...
    const loadSidebarSession = useCallback(async (sessionId, workspaceCwd) => {
    ...
    const handleOpenSessionFromOverview = useCallback((sessionId, workspaceCwd) => {
        setMainView('chat');
    ...
    useEffect(() => {
        const handler = (e) => {
            ...
            const workspaceCwd = typeof detail === 'object' &&
                detail !== null &&
    ...
    useEffect(() => {
        if (sidebarSwitchingSessionId !== null &&
    ...
    // session.
    const enqueueManualRun = useCallback((prompt) => new Promise((resolve, reject) => {
        let admitted = false;
        const admit = () => {
            if (admitted)
                return;
    ...
    const tryFireBoundRun = useCallback(() => {
        ...
        if (pending.owner && !pending.owner.isCurrent()) {
            clearPendingBoundRun(pending.sessionId);
            pending.reject(new DOMException('Bound run session was replaced', 'AbortError'));
            return;
        }
        if (pending.timer !== undefined)
        ...
        enqueueManualRun(pending.prompt).then(() => pending.resolve(), (error) => pending.reject(error));
    }, [clearPendingBoundRun, enqueueManualRun]);
    const runTaskManually = useCallback((prompt, sessionId) => {
        setMainView('chat');
        if (!sessionId) {
        ...
        return new Promise((resolve, reject) => {
            ...
            loadSidebarSession(sessionId)
                ...
                .then(() => {
                if (pendingBoundRunRef.current !== pending)
                    return;
    ...
    const openEnvironmentTask = useCallback((task) => {
    ...
    const dispatchGoalSet = useCallback((condition, setAt) => {
    ...
    const dispatchGoalCleared = useCallback((goal) => {
        if (!goal)
            return;
        store.dispatch([
            {
    ...
    const handleBusyGoalClear = useCallback((text) => {
        if (sessionWriteBlocked)
            return false;
        if (!requireActiveSessionForLocalCommand())
            return false;
    ...
    const loadRewindSnapshots = useCallback(() => sessionActions.getRewindSnapshots(), [sessionActions]);
    const rewindConversationOnly = useCallback((promptId) => sessionActions
        .rewindSession(promptId, { rewindFiles: false })
        .then(() => undefined), [sessionActions]);
    const handleRewindError = useCallback((error) => {
        if (isAlreadyDispatched(error))
            return;
        const reason = error instanceof Error ? error.message : String(error);
        pushToast('error', t('rewind.failed', { reason }));
    }, [pushToast, t]);
    const handleGoalSlashCommand = useCallback((text, images, opts) => {
        ...
        const sendGoalPrompt = () => {
            ...
            const deferComposerCommit = Boolean(onSubmitBeforeRef.current) ||
                createSessionPromiseRef.current !== null;
            const clearComposerOnPromptStart = !connectionRef.current.sessionId || deferComposerCommit;
    ...
    const hiddenCommands = useMemo(() => new Set((hiddenSlashCommands ?? []).map(normalizeHiddenCommand).filter(Boolean)), [hiddenSlashCommands]);
    const hideSettings = hiddenCommands.has('settings');
    const handleSubmit = useCallback((text, images, commitComposerAccepted, metadata) => {
        if (sessionWriteBlockedRef.current)
            return false;
        if (unknownPromptAdmissionRef.current?.payloadAvailable &&
            unknownPromptAdmissionRef.current.sessionId ===
                connectionRef.current.sessionId) {
            return false;
        }
        if (invokeSlashCommandHandler(text, onSlashCommandRef.current, reportError)) {
            return true;
        }
        ...
        const submitPromptFromEditor = (promptText, promptImages, errorMessage, opts) => {
            ...
            const admissionOwnerIsCurrent = () => admissionAttachment.current.isCurrent() &&
            ...
            sendPrompt(promptText, promptImages, {
                ...
                if (admissionStarted && !admitted && !definitelyRejected) {
                    updateFailedPrompt(null);
                    const uncertainSessionId = failedMessage?.sessionId ??
                        admissionSessionId ??
                        connectionRef.current.sessionId;
                    ...
                    pushToast('warning', t('queue.admissionUnknown'));
                    console.warn('[WebShell] prompt admission outcome is unknown', error);
                    return;
                }
                if (trackSendFailure &&
                    !admitted &&
                    ...
                    matchesUserMessageIdentity(store
                        .getSnapshot()
                        .blocks.find((block) => block.kind === 'user' &&
                        block.id === failedMessage.messageId), failedMessage.identity, failedMessage.owner.snapshot.isCurrent())) {
                    updateFailedPrompt({
                        ...failedMessage,
                        text: promptText,
                        images: promptImages,
        ...
        if (text.startsWith('/')) {
            ...
            if (match) {
                ...
                if (hiddenCommands.has(normalizeHiddenCommand(cmd))) {
                    if (promptBlocked) {
                        return enqueuePrompt(text, images, undefined, commitComposerAccepted, metadata?.inputAnnotations);
                    }
                    return submitPromptFromEditor(text, images, 'Failed to send hidden slash command', { inputAnnotations: metadata?.inputAnnotations });
                }
                if (cmd === 'help') {
                    setShowHelpDialog(true);
                    return true;
                ...
                if (cmd === 'language') {
                    ...
                    if (subCommand?.toLowerCase() === 'ui') {
                        ...
                        const valid = ['en', 'zh', 'zh-cn', 'zh_cn'].includes(normalizedArg);
                        if (!valid) {
                ...
                if (cmd === 'fork') {
                    ...
                    sessionActions
                        ...
                        setBackgroundTasksRefreshTrigger((value) => value + 1);
                        pushToast('success', t('fork.started', { name: result.description }));
                    })
                        .catch((error) => {
                        if (!owner.isCurrent())
                            return;
                ...
                if (cmd === 'model') {
                    ...
                    if (modelArg.startsWith('--fast ')) {
                        ...
                        return submitPromptFromEditor(text, images, 'Failed to send /model --fast', { inputAnnotations: metadata?.inputAnnotations });
                    }
                    if (modelArg === '--voice') {
                        if (echoOrDeferLocalCommand(text, images))
                            return true;
                    ...
                    if (modelArg.startsWith('--voice ')) {
                        ...
                        void writeVoiceModelForTarget(voiceModelId, 'workspace').catch((error) => reportError(error, t('model.setVoice')));
                        return true;
                    }
                    if (modelArg === '--vision') {
                    ...
                    if (modelArg.startsWith('--vision ')) {
                        ...
                        setWorkspaceSetting('workspace', 'visionModel', visionModelId).catch((error) => reportError(error, t('model.setVision')));
                        return true;
                    }
                    if (modelArg) {
                        if (!connectionRef.current.sessionId) {
                ...
                if (cmd === 'plan') {
                    ...
                    if (!connectionRef.current.sessionId) {
                        setPendingMode('plan');
                        if (prompt) {
                            return submitPromptFromEditor(prompt, images, 'Failed to send plan prompt', { inputAnnotations: metadata?.inputAnnotations });
                        }
                        return true;
                    }
                    ...
                    sessionActions
                        ...
                        if (prompt &&
                            ...
                            }).catch((error) => reportError(error, 'Failed to send plan prompt'));
                        }
                    })
                        .catch((error) => {
                        if (!owner.isCurrent())
                ...
                if (cmd === 'skills') {
                    ...
                    if (!skillArg || skillArg === 'detail' || skillArg === 'details') {
                        ...
                        if (promptBlocked) {
                            return enqueuePrompt(skillPrompt, images, undefined, commitComposerAccepted, metadata?.inputAnnotations);
                        }
                        return submitPromptFromEditor(skillPrompt, images, 'Failed to send /skills command', { inputAnnotations: metadata?.inputAnnotations });
                    }
                    return true;
                }
                if (cmd === 'tools') {
                ...
                if (cmd === 'context') {
                    ...
                    if (contextArg === '' ||
                        contextArg === 'detail' ||
                        contextArg === '-d') {
                        showContextUsage(text, contextArg === 'detail' || contextArg === '-d');
                        return true;
                    }
                }
                if (cmd === 'memory') {
                    ...
                    if (memoryArg === 'refresh') {
                        ...
                        setMemoryAddScope(addTarget === 'user' || addTarget === 'global'
                            ? 'global'
                            : 'workspace');
                ...
                if (cmd === 'agents') {
                    ...
                    else if (subCommand === 'create project' ||
                ...
                if (cmd === 'rename') {
                    ...
                    if (renameArg.type === 'auto' || renameArg.type === 'delegate') {
                        ...
                        return submitPromptFromEditor(text, images, 'Failed to send /rename command', { inputAnnotations: metadata?.inputAnnotations });
                    }
                    ...
                    sessionActions
                        ...
                        .then(() => {
                        if (renamedSessionId) {
                            reconcileCatalogRename(renamedWorkspaceCwd, renamedSessionId, displayName);
                        }
                        if (!owner.isCurrent())
                            return;
                        store.dispatch([
                        ...
                        if (renamedWorkspaceCwd) {
                            sessionCatalogController.invalidateWorkspace(renamedWorkspaceCwd);
                        }
                        if (!owner.isCurrent())
                            return;
                ...
                if (cmd === 'stats') {
                    ...
                    sessionActions
                        ...
                        if (!owner.isCurrent())
                            return;
                        dispatchReadOnlyStatus(serializeStatsMessage(result, statsView));
                    })
                        .catch((error) => {
                        if (!owner.isCurrent())
                            return;
                ...
                if (cmd === 'status' || cmd === 'about') {
                    ...
                    Promise.all([
                        ...
                        if (sys.npmVersion)
                            runtimeParts.push(`npm ${sys.npmVersion}`);
                        let formattedAuth = '';
                        if (authSource) {
                            if (authSource.startsWith('oauth') ||
                        ...
                        const info = {
                            ...
                            model: curModel ||
                                ...
                                '',
                            fastModel: providers?.current?.fastModelId ||
                                curModel ||
                                conn.currentModel ||
                                providers?.current?.modelId ||
                                '',
                ...
                if (cmd === 'bug') {
                    ...
                    Promise.all([
                        ...
                        .then(([preflight, env]) => {
                        const sys = collectSystemInfo(preflight, env);
                        const qwenCodeVersion = connectionRef.current.capabilities?.qwenCodeVersion || '';
                        const sysInfo = {};
                        if (qwenCodeVersion)
                            sysInfo.cliVersion = qwenCodeVersion;
                        if (sys.nodeVersion)
                            sysInfo.nodeVersion = sys.nodeVersion;
                        if (sys.npmVersion)
                            sysInfo.npmVersion = sys.npmVersion;
                        if (sys.platform)
                            sysInfo.platform = sys.platform;
                        if (sys.arch)
                            sysInfo.arch = sys.arch;
                        if (sys.sandbox)
                            sysInfo.sandbox = sys.sandbox;
                        if (sys.memoryUsage)
                            sysInfo.memoryUsage = sys.memoryUsage;
            ...
            if (needsSession) {
                if (shellSubmitInFlightRef.current)
                    return false;
            ...
            void ensureSessionForPrompt()
                ...
                if (drainGenerationRef.current !== generationAtSubmit)
                    return;
                ...
                .catch((error) => {
                reportError(error, needsSession && !sessionCreated
    ...
    const handleEditorSubmit = useCallback((text, images, commitComposerAccepted, metadata) => {
        const accepted = handleSubmitRef.current(text, images, commitComposerAccepted, metadata);
        if (accepted !== false) {
            resumeChatBottomFollow('smooth');
        }
        return accepted;
    ...
    const handleAskUserConfirm = useCallback((id, selectedOption, answers) => sessionActions.submitPermission(id, selectedOption, answers), [sessionActions]);
    ...
    const restoreUnknownPromptPayload = useCallback(() => {
        ...
        const restoredText = current.text?.trim()
            ? draft.trim()
                ...
                : current.text
            : draft;
        if (restoredText !== draft)
            editor.setText(restoredText);
        if (current.images?.length)
            editor.restoreImages(current.images);
    ...
    const handleRetry = useCallback(() => {
        ...
        if (showRetryHintRef.current &&
            ...
            const currentRetryError = getRetryableTurnError(store.getSnapshot().blocks);
            if (!savedRetryErrorIdentity ||
                !currentRetryError ||
            ...
            const retryTranscriptIsCurrent = () => retryTranscriptIdentityMatches(store.getSnapshot().blocks, retryTranscriptIdentity);
            setFailedPromptRetry({
            ...
            sendPrompt(retryText, retryImages, {
                ...
                if (admissionStarted && !admitted && !definitelyRejected) {
                    ...
                    pushToast('warning', t('queue.admissionUnknown'));
                    console.warn('[WebShell] post-turn retry admission outcome is unknown', error);
                    return;
                }
                if (!admitted) {
                    restoreOrDeferCancelledRetry(retryOwner, {
        ...
        const onKeyDown = (e) => {
            ...
            switch (intent.kind) {
                ...
                case 'clear':
                    ...
                    break;
                case 'arm':
                    armEscape(intent.action, intent.action === 'cancel'
    ...
    const isDisabled = sessionWriteBlocked ||
    ...
    const showCurrentRetryHint = Boolean(showRetryHint &&
        !isPreparingPrompt &&
        retryableTurnErrorIdentity &&
        matchesTurnErrorIdentity(getRetryableTurnError(blocks), retryableTurnErrorIdentity) &&
        retryOwnerIsCurrent(retryOwnerRef.current));
    const latestUserBlock = getLatestUserBlock(blocks);
    const visibleFailedPromptBlock = failedPrompt &&
        latestUserBlock &&
    ...
    const composerPlaceholderState = getComposerPlaceholderState(composerPlaceholderInputState);
    const customComposerPlaceholder = composerPlaceholders?.[composerPlaceholderState];
    ...
    const handleReasoningEffort = useCallback((value) => {
    ...
    const handleDeleteModel = useCallback((target) => {
        const modelActionToken = ++modelActionTokenRef.current;
        setModelActionBusy(true);
        workspaceActions
            .deleteModel(target)
            ...
            reloadProviders().catch((err) => {
                console.warn('[web-shell] failed to reload providers after delete', err);
            });
            reloadWorkspaceSettings().catch((err) => {
                console.warn('[web-shell] failed to reload settings after delete', err);
            });
        })
            .catch((error) => {
    ...
    const handleCloseAuthDialog = useCallback(() => {
        ...
        reloadProviders().catch((err) => {
            console.warn('[web-shell] failed to reload providers after auth dialog close', err);
        });
    }, [reloadProviders]);
    const handleFallbacksConfirm = useCallback((baseIds) => {
        setShowFallbacksDialog(false);
        setWorkspaceSetting(modelSettingScope, 'modelFallbacks', baseIds.join(','))
            .then((result) => {
            ...
            reloadWorkspaceSettings().catch((err) => {
                console.warn('[web-shell] failed to reload settings after fallbacks save', err);
            });
        })
            .catch((error) => reportError(error, t('settings.models.fallbacks.saveFailed')));
    }, [
        modelSettingScope,
        setWorkspaceSetting,
        reloadWorkspaceSettings,
    ...
    const handleFastModelSelect = useCallback((modelId) => {
        if (streamingState !== 'idle') {
            blockLocalCommandDuringTurn();
            return;
        }
        ...
        const scopeFlag = modelSettingScope === 'user' ? ' --global' : ' --project';
        const owner = { current: sessionOwnerGuard.capture() };
        sendPrompt(`/model --fast ${modelId}${scopeFlag}`, undefined, {
            ...
            reloadWorkspaceSettings().catch((err) => {
                console.warn('[web-shell] failed to reload workspace settings after fast-model switch', err);
            });
        })
            .catch((error) => {
            if (!owner.current.isCurrent())
    ...
    const handleVoiceModelSelect = useCallback((modelId) => {
        ...
        if (!target || target.ownerKey !== mainVoiceTargetRef.current?.ownerKey) {
            reportError(new Error('The Voice workspace changed before the selection applied.'), t('model.setVoice'));
            return;
        }
        void writeVoiceModelForTarget(bareModelId, modelSettingScope, target).catch((error) => reportError(error, t('model.setVoice')));
    }, [modelSettingScope, reportError, t, writeVoiceModelForTarget]);
    const handleVisionModelSelect = useCallback((modelId) => {
        ...
        setWorkspaceSetting(modelSettingScope, 'visionModel', encoded).catch((error) => reportError(error, t('model.setVision')));
    ...
    const commands = useMemo(() => {
        const previousSkillNames = new Set((connection.skills ?? []).map((skill) => skill.toLowerCase()));
        const retainedCommands = loadedSkillsReady
            ? (connection.commands ?? []).filter((command) => command.source !== 'skill' &&
        ...
        const refreshedSkillCommands = loadedSkillsReady
            ...
            : [];
        return localizeBuiltinDescriptions(mergeCommands(retainedCommands, refreshedSkillCommands, getLocalCommands(t, { sideTaskAvailable: sideTasksAvailable })), t)
            .filter((command) => !hiddenCommands.has(normalizeHiddenCommand(command.name)))
            .map((command) => {
    ...
    const welcomeHeader = useMemo(() => (_jsx(_Fragment, { children: renderWelcomeHeader ? (renderWelcomeHeader(welcomeHeaderProps)) : (_jsx(WelcomeHeader, { ...welcomeHeaderProps })) })), [renderWelcomeHeader, welcomeHeaderProps]);
    const welcomeFooter = useMemo(() => renderWelcomeFooter?.(welcomeHeaderProps), [renderWelcomeFooter, welcomeHeaderProps]);
    const isChatEmptyState = !connection.sessionId &&
        displayMessages.length === 0 &&
        !showFloatingTodos &&
        !pendingApproval &&
        !btwMessage;
    const useMobileWelcomeMiddleLayout = isChatEmptyState && mobileWelcomeFooterMiddle;
    const showMobileWelcomeFooterMiddle = useMobileWelcomeMiddleLayout && Boolean(welcomeFooter);
    const hasWelcomeMiddle = isChatEmptyState && showMobileWelcomeFooterMiddle;
    const hasMobileComposerBottom = isChatEmptyState && useMobileWelcomeMiddleLayout;
    const missingSession = connection.status !== 'connecting' &&
        !connection.sessionId &&
        connection.missingSession === true;
    const showMissingSessionState = missingSession && !activePanel && mainView === 'chat';
    ...
    const environmentPanelCanDock = contextBodyWidth === null ||
        contextBodyWidth >=
            MIN_DOCKED_MESSAGE_AREA_WIDTH + DOCKED_ENVIRONMENT_PANEL_WIDTH;
    const environmentPanelFits = chatWidthMode !== 'wide' && environmentPanelCanDock;
    const environmentPanelVisible = environmentPanelOpen &&
        !isChatEmptyState &&
        !activePanel &&
        mainView === 'chat';
    ...
    useLayoutEffect(() => {
        ...
        const updateWidth = () => {
            ...
            if (availableWidth <= 0)
                return;
            setContextBodyWidth((current) => current === availableWidth ? current : availableWidth);
        };
    ...
    useLayoutEffect(() => {
        const crossedDockBreakpoint = previousEnvironmentCanDockRef.current && !environmentPanelCanDock;
        previousEnvironmentCanDockRef.current = environmentPanelCanDock;
        if (crossedDockBreakpoint &&
    ...
    useLayoutEffect(() => {
        const artifactPanelJustOpened = !previousArtifactPanelOpenForEnvironmentRef.current && artifactPanelOpen;
        previousArtifactPanelOpenForEnvironmentRef.current = artifactPanelOpen;
        if (!artifactPanelOpen) {
            preserveEnvironmentPanelOnArtifactOpenRef.current = false;
            return;
        }
        if (!artifactPanelJustOpened)
            return;
        const preserveEnvironmentPanel = preserveEnvironmentPanelOnArtifactOpenRef.current;
        if (!preserveEnvironmentPanel && !environmentPanelFits) {
            setEnvironmentPanelOpen(false);
        }
    }, [artifactPanelOpen, environmentPanelFits]);
    const environmentPanelMounted = !isChatEmptyState && !activePanel && mainView === 'chat';
    ...
    useLayoutEffect(() => {
        ...
        if (host && shadowRoot) {
            ...
            host.style.setProperty('z-index', 'var(--web-shell-portal-root-z-index, 1000)', 'important');
            const removeStyles = installWebShellShadowStyles(shadowRoot, shadowDomOptions.styles);
    ...
    useLayoutEffect(() => {
        ...
        const portalHost = portalRootNode instanceof ShadowRoot
            ? portalRootNode.host
            : null;
        let frameId = null;
        const syncVariables = () => {
            ...
            portalRoot.classList.toggle('dark', selectedTheme === WebShellThemeId.Dark);
    ...
    return (_jsx(ThemeProvider, { value: selectedTheme, children: _jsx(I18nProvider, { language: selectedLanguage, children: _jsx(WebShellPortalRootContext.Provider, { value: portalRoot, children: _jsxs("div", { ref: appRootRef, className: appClassName, style: appStyle, "data-web-shell-root": true, "data-web-shell-shadcn": true, lang: selectedLanguage, children: [!onToast && (_jsx(ToastHost, { toasts: toasts, onDismiss: dismissToast, elevated: artifactPanelFullscreen })), showResumeDialog && (_jsx(DialogShell, { title: t('resume.title'), size: "lg", onClose: () => setShowResumeDialog(false), children: _jsx(ResumeDialog, { workspaceCwd: lockedWorkspaceCwd, onSelect: (sessionId) => {
                                    loadSidebarSession(sessionId).catch((error) => {
                                        reportError(error, 'Failed to load session');
                                    });
                                }, onClose: () => setShowResumeDialog(false) }) })), modelDialogMode && (_jsx(DialogShell, { title: t(MODE_TITLE_KEY[modelDialogMode]), size: "lg", onClose: () => setModelDialogMode(null), children: _jsx(ModelDialog, { mode: modelDialogMode, models: modelDialogMode === 'voice' ? voiceModels : undefined, currentModelId: modelDialogMode === 'voice'
                                    ? currentVoiceModel
                                    : modelDialogMode === 'vision'
                                        ? currentVisionModel
                                        : modelDialogMode === 'fast'
                                            ? currentFastModel
                                            : undefined, onSelect: (modelId) => {
                                    if (modelDialogMode) {
                                        modelHandlers[modelDialogMode](modelId);
                                    }
                                    setModelDialogMode(null);
                                } }) })), showApprovalModeDialog && (_jsx(DialogShell, { title: t('mode.select'), size: "sm", onClose: () => setShowApprovalModeDialog(false), children: _jsx(ApprovalModeDialog, { currentMode: currentMode, sessionWorkflowEnabled: sessionWorkflowEnabled, onSelect: (modeId) => {
                                    handleSetMode(modeId);
                                    setShowApprovalModeDialog(false);
                                } }) })), showToolsDialog && (_jsx(DialogShell, { title: t('tools.title'), size: "lg", onClose: () => setShowToolsDialog(false), children: _jsx(ToolsDialog, {}) })), gitDialog && (_jsx(GitDialog, { workspaceCwd: gitDialog.workspaceCwd, gitCwd: gitDialog.gitCwd, initialView: gitDialog.view, sessionId: connection.sessionId, resolveSessionForWorkspace: resolveSessionForWorkspace, onClose: () => setGitDialog(undefined) }, `${gitDialog.workspaceCwd}:${gitDialog.gitCwd ?? ''}:${gitDialog.view}`)), tasksDialogMessage && (_jsx(DialogShell, { title: sessionWorkflowEnabled && floatingTodos.length > 0
                                ? t('planExecution.dialogTitle')
                                : t('tasks.title'), size: "lg", onClose: () => setTasksDialogMessage(null), children: _jsx(TasksStatusMessage, { message: tasksDialogMessage, embedded: true, manageActiveEvent: false, onClose: () => setTasksDialogMessage(null), planTodos: sessionWorkflowEnabled ? floatingTodos : [], agentTools: sessionWorkflowEnabled ? planAgentTools : [], onOpenSubagent: (tool) => {
                                    setTasksDialogMessage(null);
                                    openSubagentPanel(tool);
                                }, onOpenMonitor: handleOpenMonitorDetails }) })), showMemoryDialog && (_jsx(DialogShell, { title: t('memory.menu'), size: "lg", onClose: () => setShowMemoryDialog(false), children: _jsx(MemoryMessage, { refreshSignal: memoryRefreshSignal, addSignal: memoryAddSignal, addScope: memoryAddScope, onMessage: (text, type = 'status') => {
                                    store.dispatch([{ type, text }]);
                                } }) })), showHelpDialog && (_jsx(DialogShell, { title: t('help.title'), size: "md", onClose: () => setShowHelpDialog(false), children: _jsx(HelpDialog, { commands: commands }) })), showThemeDialog && (_jsx(DialogShell, { title: t('theme.title'), size: "sm", onClose: () => setShowThemeDialog(false), children: _jsx(ThemeDialog, { currentTheme: selectedTheme, onSelect: handleThemeChange, onClose: () => setShowThemeDialog(false) }) })), showAuthDialog && (_jsx(DialogShell, { title: t('auth.title'), size: "lg", onClose: handleCloseAuthDialog, children: _jsx(AuthMessage, { onMessage: (text, type = 'status') => {
                                    store.dispatch([
                                        type === 'error'
                                            ? { type: 'error', text }
                                            : { type: 'status', text },
                                    ]);
                                }, onClose: handleCloseAuthDialog }) })), showFallbacksDialog && (_jsx(DialogShell, { title: t('settings.models.fallbacks.title'), size: "md", onClose: () => setShowFallbacksDialog(false), children: _jsx(ModelFallbacksDialog, { models: fallbackModelOptions, current: currentModelFallbacks, max: 3, onConfirm: handleFallbacksConfirm, onClose: () => setShowFallbacksDialog(false) }) })), showDeleteDialog && (_jsx(DialogShell, { title: t('delete.title'), size: "lg", onClose: () => setShowDeleteDialog(false), children: _jsx(DeleteSessionDialog, { workspaceCwd: lockedWorkspaceCwd, onDeleted: (sessionIds) => {
                                    store.dispatch([
                                        {
                                            type: 'status',
                                            text: sessionIds.length === 1
                                ...
                                }, onClose: () => setShowDeleteDialog(false) }) })), showReleaseDialog && (_jsx(DialogShell, { title: t('release.title'), size: "lg", onClose: () => setShowReleaseDialog(false), children: _jsx(ReleaseSessionDialog, { workspaceCwd: lockedWorkspaceCwd, onReleased: (sessionId) => {
                                    store.dispatch([
                                        {
                                            type: 'status',
                                ...
                                }, onClose: () => setShowReleaseDialog(false) }) })), showRewindDialog && (_jsx(DialogShell, { title: t('rewind.title'), subtitle: t('rewind.subtitle'), size: "lg", onClose: () => setShowRewindDialog(false), children: _jsx(RewindDialog, { blocks: blocks, loadSnapshots: loadRewindSnapshots, rewind: rewindConversationOnly, onError: handleRewindError, onClose: () => setShowRewindDialog(false) }) })), !lockedWorkspaceCwd && showAddWorkspaceDialog && (_jsx(AddWorkspaceDialog, { onClose: () => setShowAddWorkspaceDialog(false), onAdd: handleAddWorkspace, onSuggest: workspaceActions.suggestWorkspacePaths, onPick: async () => {
                                const result = await workspaceActions.pickWorkspaceDirectory();
                                return result.selected ? result.path : undefined;
                            }, persistenceSupported: persistentWorkspaceRegistrationSupported, displayNameEnabled: workspaceDisplayNameSupported })), scratchOutcomeUnknown !== 'clear' && (_jsx(DialogShell, { title: t('sidebar.scratchOutcomeUnknownTitle'), size: "md", dismissible: false, onClose: () => undefined, children: _jsxs("div", { className: "flex flex-col gap-4", children: [_jsx("p", { children: t('sidebar.scratchOutcomeUnknown') }), _jsx("ul", { className: "max-h-48 overflow-y-auto text-sm text-muted-foreground", children: workspaces.map((entry) => (_jsx("li", { children: entry.cwd }, entry.id))) }), _jsx("div", { className: "flex justify-end", children: scratchOutcomeUnknown === 'refreshing' ? (_jsx(Button, { type: "button", onClick: () => void refreshScratchOutcome(), children: t('sidebar.scratchOutcomeRefresh') })) : (_jsx(Button, { type: "button", onClick: () => setScratchOutcome('clear'), children: t('sidebar.scratchOutcomeAcknowledge') })) })] }) })), _jsxs("div", { className: styles.appShell, children: [sidebarOptions.enabled && (_jsxs("div", { "data-sidebar-shell": "", ...(mobileDrawerOpen
                                        ? { role: 'dialog', 'aria-modal': 'true' }
                                        : {}), "aria-label": t('sidebar.label'), "aria-hidden": artifactPanelFullscreen || undefined, className: [
                                        styles.mobileDrawer,
                                        ...
                                        .filter(Boolean)
                                        .join(' '), children: [_jsx("div", { className: styles.mobileBackdrop, onClick: closeMobileDrawer, "aria-hidden": "true" }), _jsx(WebShellSidebar, { collapsed: (sidebarCollapsed ||
                                                (mainView === 'split' && !splitSidebarHasRoom)) &&
                                                !mobileDrawerOpen, onCollapsedChange: handleSidebarCollapsedChange, onOpenSettings: () => {
                                                closeMobileDrawer();
                                                openPanel('settings');
                                            }, onOpenPlugins: () => {
                                                closeMobileDrawer();
                                                ...
                                                closeMobileDrawer();
                                                openPanel('sessions');
                                            }, canOpenSessionsOverview: isLargeScreen, onOpenSplitView: () => {
                                                closeMobileDrawer();
                                                openSplitView();
                                            }, canOpenSplitView: isLargeScreen, theme: selectedTheme, onThemeChange: (theme) => {
                                                handleThemeChange(theme);
                                                void setWorkspaceSetting('workspace', THEME_SETTING_KEY, webShellThemeToSettingValue(theme));
                                            }, onNewSession: (workspaceCwd) => createNewSession(workspaceCwd), onLoadSession: (sessionId, workspaceCwd) => {
                                                setMainView('chat');
                                                ...
                                                setMainView('chat');
                                                closePanel();
                                            }, onSessionRenameConfirmed: reconcileCatalogRename, onError: reportError, mobileOpen: mobileDrawerOpen, selectedWorkspaceCwd: selectedWorkspaceCwd, onSelectWorkspace: setSelectedWorkspaceCwd, onOpenGitDiff: (workspaceCwd) => setGitDialog({
                                                workspaceCwd,
                                                gitCwd: workspaceCwd === activeWorkspaceCwd
                                                    ? sessionWorktree?.path
                                                    : undefined,
                                                view: 'diff',
                                            }), onOpenCommit: (workspaceCwd) => setGitDialog({
                                                workspaceCwd,
                                                ...
                                                gitCwd: workspaceCwd === activeWorkspaceCwd
                                                    ...
                                                    : undefined,
                                                view: 'commit',
                                            }), onOpenAddWorkspace: dynamicWorkspaceRegistrationSupported
                                                ? () => setShowAddWorkspaceDialog(true)
                                                : undefined, workspaces: workspaces, lockedWorkspaceCwd: lockedWorkspaceCwd, lockedWorkspace: sidebarOptions.lockedWorkspace, branding: sidebarOptions.branding, primaryNav: sidebarOptions.primaryNav, hideProjectHeader: sidebarOptions.hideProjectHeader, sessionActions: sidebarOptions.sessionActions, footer: sidebarOptions.footer })] })), _jsxs("div", { className: [
                                        styles.contextShell,
                                        artifactPanelFullscreen ? styles.chatViewHidden : undefined,
                                    ]
                                        .filter(Boolean)
                                        .join(' '), "aria-hidden": artifactPanelFullscreen || undefined, children: [chatHeaderEnabled &&
                                            !isChatEmptyState &&
                                            !activePanel &&
                                            mainView === 'chat' && (_jsxs("div", { className: styles.chatHeaderRow, children: [sidebarOptions.enabled &&
                                                    sidebarOptions.showCompactToggle && (_jsx("button", { type: "button", className: styles.hamburgerButton, onClick: () => {
                                                        setForceMobileDrawer(false);
                                                        setMobileDrawerOpen((open) => !open);
                                                    }, "aria-label": t('sidebar.toggleMenu'), "aria-expanded": mobileDrawerOpen, children: _jsxs("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true", children: [_jsx("line", { x1: "3", y1: "6", x2: "21", y2: "6" }), _jsx("line", { x1: "3", y1: "12", x2: "21", y2: "12" }), _jsx("line", { x1: "3", y1: "18", x2: "21", y2: "18" })] }) })), renderChatHeader ? (_jsx("div", { className: styles.customChatHeader, children: renderChatHeader({
                                                        sessionId: connection.sessionId,
                                                        ...
                                                        onEnvironmentPanelOpenChange: handleEnvironmentPanelOpenChange,
                                                        onRightPanelOpenChange: handleRightPanelOpenChange,
                                                    }) })) : (_jsx(ChatContextHeader, { content: titleHeaderItemVisible
                                                        ? (sessionDisplayName ?? t('session.new'))
                                                        : null, environmentOpen: environmentPanelVisible, environmentAvailable: environmentHeaderItemVisible, rightPanelOpen: artifactPanelOpen, rightPanelAvailable: rightPanelHeaderItemVisible && !artifactPanelOpen, onToggleEnvironment: () => handleEnvironmentPanelOpenChange(!environmentPanelVisible), onToggleRightPanel: () => handleRightPanelOpenChange(!artifactPanelOpen) }))] })), _jsxs("div", { ref: contextBodyRef, "data-testid": "context-body", className: [
                                                styles.contextBody,
                                                ...
                                                .filter(Boolean)
                                                .join(' '), children: [_jsxs("div", { ref: chatPaneRef, "data-testid": "chat-pane-container", className: [
                                                        styles.chatPane,
                                                        ...
                                                        .filter(Boolean)
                                                        .join(' '), children: [isChatEmptyState &&
                                                            !showMissingSessionState &&
                                                            !activePanel &&
                                                            mainView === 'chat' && _jsx(NewSessionDotField, {}), sidebarOptions.enabled &&
                                                            sidebarOptions.showCompactToggle &&
                                                            (!chatHeaderEnabled || isChatEmptyState) &&
                                                            !activePanel &&
                                                            mainView === 'chat' && (_jsx("button", { type: "button", className: [
                                                                ...
                                                                setForceMobileDrawer(false);
                                                                setMobileDrawerOpen((open) => !open);
                                                            }, "aria-label": t('sidebar.toggleMenu'), "aria-expanded": mobileDrawerOpen, children: _jsxs("svg", { viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true", children: [_jsx("line", { x1: "3", y1: "6", x2: "21", y2: "6" }), _jsx("line", { x1: "3", y1: "12", x2: "21", y2: "12" }), _jsx("line", { x1: "3", y1: "18", x2: "21", y2: "18" })] }) })), activePanel && (_jsxs("section", { className: styles.panelHost, role: "region", "data-testid": "inline-panel", "aria-label": activePanel === 'settings'
                                                                ? t('settings.title')
                                                                : activePanel === 'status'
                                                                    ? t('daemon.title')
                                                                                ...
                                                                                : activePanel === 'agents'
                                                                                    ...
                                                                                    : activePanel === 'plugins'
                                                                                        ...
                                                                                        : activePanel === 'channels'
                                                                                            ? t('channels.title')
                                                                                            : t('sessionsOverview.title'), children: [activePanel !== 'extensions' &&
                                                                    activePanel !== 'mcp' &&
                                                                    activePanel !== 'skills' &&
                                                                    activePanel !== 'agents' &&
                                                                    activePanel !== 'plugins' &&
                                                                    activePanel !== 'channels' && (_jsxs("div", { className: styles.panelHeader, children: [_jsx("button", { ref: panelBackRef, type: "button", className: styles.panelBack, "data-testid": "panel-back", onClick: closePanel, "aria-label": t('common.back'), title: t('common.back'), children: _jsx("svg", { viewBox: "0 0 24 24", "aria-hidden": "true", children: _jsx("path", { d: "M15 5l-7 7 7 7", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round" }) }) }), _jsx("div", { className: styles.panelTitle, children: activePanel === 'settings'
                                                                                ? t('settings.title')
                                                                                : activePanel === 'status'
                                                                                    ? t('daemon.title')
                                                                                    : t('sessionsOverview.title') })] })), _jsx("div", { className: styles.panelBody, children: _jsx(ShadowDomBoundary, { enabled: shadowDomOptions.plugins &&
                                                                            isPluginShadowPanel(activePanel), language: selectedLanguage, themeClassName: [
                                                                            selectedTheme === WebShellThemeId.Light
                                                                                ? styles.themeLight
                                                                                : styles.themeDark,
                                                                            ...
                                                                            .filter(Boolean)
                                                                            .join(' '), styles: shadowDomOptions.styles, initialFocusRef: activePanel === 'plugins'
                                                                            ? pluginTabRef
                                                                            : activePanel === 'extensions' ||
                                                                                activePanel === 'channels'
                                                                                ? panelHeadingRef
                                                                                : undefined, children: activePanel === 'settings' ? (_jsx(SettingsMessage, { settingsState: targetedWorkspaceSettingsState, embedded: true, onLanguageChange: handleSettingsLanguageChange, onThemeChange: handleThemeChange, chatWidthMode: chatWidthMode, onChatWidthModeChange: handleChatWidthModeChange, modelManagement: {
                                                                            ...
                                                                            } })) : activePanel === 'status' ? (_jsx(DaemonStatusDialog, {})) : activePanel === 'extensions' ? (_jsx(ExtensionsManagerPage, { onClose: closePanel, initialFocusRef: panelHeadingRef })) : activePanel === 'mcp' && mcpDialogMessage ? (_jsx(McpManagerPage, { message: mcpDialogMessage, onClose: () => {
                                                                                setMcpDialogMessage(null);
                                                                                closePanel();
                                                                            } })) : activePanel === 'skills' ? (_jsx(SkillsManagerPage, { onClose: closePanel, onUseSkill: handleUseSkill })) : activePanel === 'agents' ? (_jsx(AgentsManagerPage, { onClose: () => {
                                                                                setAgentsCreateScope(null);
                                                                                closePanel();
                                                                            }, initialCreateScope: agentsCreateScope })) : activePanel === 'plugins' ? (_jsx(PluginManagerPage, { mcpMessage: mcpDialogMessage, loadMcpMessage: async () => {
                                                                                try {
                                                                            ...
                                                                            }, onClose: closePanel, onUseSkill: handleUseSkill, initialFocusRef: pluginTabRef })) : activePanel === 'channels' ? (_jsx(ChannelsManagerPage, { onClose: closePanel, initialFocusRef: panelHeadingRef })) : (_jsx(SessionOverviewPanel, { onOpenSession: handleOpenSessionFromOverview, onOpenSplit: openSplitView, includeOtherWorkspaces: !lockedWorkspaceCwd, workspaceCwd: lockedWorkspaceCwd })) }) }, activePanel)] })), mainView === 'scheduledTasks' && (_jsxs("div", { className: styles.fullPage, "data-testid": "scheduled-tasks-page", children: [_jsxs("div", { className: styles.fullPageHeader, children: [_jsx("button", { type: "button", className: styles.fullPageBack, onClick: () => setMainView('chat'), "aria-label": t('common.back'), title: t('common.back'), children: _jsx("svg", { viewBox: "0 0 24 24", width: "18", height: "18", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true", children: _jsx("path", { d: "M15 18l-6-6 6-6" }) }) }), _jsx("div", { className: styles.fullPageTitle, children: t('scheduledTasks.title') })] }), _jsx("div", { className: styles.fullPageBody, children: _jsx(ScheduledTasksDialog, { onRunPrompt: runTaskManually, 
                                                                        ...
                                                                        workspaces: lockedWorkspaceCwd
                                                                            ? visibleWorkspaces
                                                                            : workspaces, lockedWorkspace: lockedWorkspaceCapability, onCreateViaChat: () => {
                                                                            ...
                                                                            void createNewSession().then((created) => {
                                                                                ...
                                                                                onSessionIdChange?.(undefined);
                                                                                window.setTimeout(() => {
                                                                                    editorRef.current?.insertText(t('scheduledTasks.chatStarter'), { mode: 'replace' });
                                                                                    editorRef.current?.focus();
                                                                                }, 0);
                                                                            });
                                                                        ...
                                                                        }, onError: reportError }) })] })), mainView === 'goals' && (_jsxs("div", { className: styles.fullPage, "data-testid": "goals-page", children: [_jsxs("div", { className: styles.fullPageHeader, children: [_jsx("button", { type: "button", className: styles.fullPageBack, onClick: () => setMainView('chat'), "aria-label": t('common.back'), title: t('common.back'), children: _jsx("svg", { viewBox: "0 0 24 24", width: "18", height: "18", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true", children: _jsx("path", { d: "M15 18l-6-6 6-6" }) }) }), _jsx("div", { className: styles.fullPageTitle, children: t('goals.title') })] }), _jsx("div", { className: styles.fullPageBody, children: _jsx(GoalsDialog, { onCreateGoal: async (condition) => {
                                                                            ...
                                                                            const canReuseStranded = stranded !== undefined &&
                                                                        ...
                                                                        }, onError: reportError }) })] })), mainView === 'split' && (_jsxs("div", { className: styles.fullPage, "data-testid": "split-view-page", children: [approvalOverlayActive && (_jsxs("div", { className: styles.splitApprovalNotice, role: "status", "data-testid": "split-approval-notice", children: [_jsx("span", { children: t('splitView.outerApprovalPending') }), _jsx("button", { type: "button", onClick: () => setMainView('chat'), children: t('splitView.goToApproval') })] })), _jsx(WebShellCustomizationProvider, { value: customization, children: _jsx(CompactModeContext.Provider, { value: compactMode, children: _jsx(SplitView, { sessionIds: splitSessionIds, 
                                                                            ...
                                                                            // effect.
                                                                            onPanesChange: handleSplitPanesChange, includeOtherWorkspaces: !lockedWorkspaceCwd, workspaceCwd: lockedWorkspaceCwd, 
                                                                            // Back returns to the Session Overview (the hub the split
                                                                            // is launched from), not the single-session chat.
                                                                            onExit: handleSplitExit, onError: reportError, onImageIngestionNotice: pushToast, onSlashCommand: onSlashCommand, onRightPanelOpen: handleTurnOutputOpen, onOpenMonitor: openMonitorPanel, onPaneArtifactsChange: handlePaneArtifactsChange, messageTurnOutputs: messageTurnOutputs, restartSseOnPrompt: restartSseOnPrompt, historyPageSize: historyPageSize, renderPaneHeaderActions: renderPaneHeaderActions, voiceUserRevision: voiceUserRevision, voiceWorkspaceRevisions: voiceWorkspaceRevisions, voiceWorkspaces: workspace.capabilities?.workspaces ||
                                                                                lockedWorkspaceCapability
                                                                                ? workspaces
                                                                                : undefined, sessionWorkflowEnabled: sessionWorkflowEnabled }) }) })] })), _jsxs("div", { className: [
                                                                styles.chatViewWrap,
                                                                hasMobileComposerBottom
                                                                ...
                                                                .filter(Boolean)
                                                                .join(' '), "aria-hidden": activePanel ||
                                                                mainView !== 'chat' ||
                                                                artifactPanelFullscreen
                                                                ? true
                                                                : undefined, children: [showMissingSessionState && (_jsxs("div", { className: styles.missingSessionState, children: [_jsx("div", { className: styles.missingSessionMessage, children: t('session.missing') }), _jsx("button", { type: "button", className: styles.missingSessionButton, disabled: isCreatingMissingSession, onClick: handleMissingSessionNewSession, children: t('session.new') })] })), _jsx("div", { className: showMissingSessionState
                                                                        ? styles.chatSubtreeHidden
                                                                        : styles.chatSubtree, children: _jsxs(WebShellCustomizationProvider, { value: customization, children: [_jsx(CompactModeContext.Provider, { value: compactMode, children: _jsx(TodoContextsProvider, { timeline: todoTimeline, details: todoDetails, children: _jsx(InteractionBlockContext.Provider, { value: registerInteractionBlocker, children: (() => {
                                                                                            const contentClassName = [
                                                                                                ...
                                                                                                .filter(Boolean)
                                                                                                .join(' ');
                                                                                            const messageListContent = (_jsx(MessageList, { ref: messageListRef, messages: displayMessages, pendingApproval: pendingToolApproval, onShowContextDetail: handleShowContextDetail, loadingTranscript: connection.loadingTranscript, catchingUp: connection.catchingUp, hasOlderHistory: transcriptHistory.hasMore, loadingOlderHistory: transcriptHistory.loading, historyCapacityReached: transcriptHistory.capacityReached, historyPaginationError: transcriptHistory.paginationError, onLoadOlderHistory: transcriptHistory.loadMore, transcriptBlockCount: blocks.length, transcriptActivity: store, onReloadTranscript: transcriptReloadSupported
                                                                                                    ? reloadTranscript
                                                                                                    : undefined, isResponding: streamingState !== 'idle' &&
                                                                                                    !suppressFailedPromptRetryStreaming, activeTurnStartedAt: suppressFailedPromptRetryStreaming
                                                                                                    ? undefined
                                                                                                    : (failedPromptRetry?.startedAt ??
                                                                                                        activeTurnStartedAt), workspaceCwd: connection.workspaceCwd || '', hideSessionTimeline: effectiveChatWidthMode === 'wide', showRetryHint: showCurrentRetryHint, onRetryClick: handleRetry, failedPromptMessageId: isPreparingPrompt
                                                                                                    ? undefined
                                                                                                    : visibleFailedPromptBlock?.id, onRetryFailedPrompt: handleFailedPromptRetry, onBranchSession: handleBranchCurrentSession, bottomOverlayInset: bottomPanelInset, welcomeHeader: isChatEmptyState ? welcomeHeader : undefined, centerWelcomeHeader: showMobileWelcomeFooterMiddle || undefined, tailContent: undefined, tailKey: undefined, onCanScrollToBottomChange: handleCanScrollToBottomChange, virtualScrollThreshold: virtualScrollThreshold, turnFileChanges: visibleTurnOutputKinds.has('file')
                                                                                                    ? fileChangesByTurn
                                                                                                    ...
                                                                                                    : undefined, onTurnOutputOpen: handleTurnOutputOpen, onImagePreview: openImagePanel, onReviewChanges: openReviewPanel, onOpenArtifact: openArtifactPanel, onOpenScheduledTask: openScheduledTaskPanel, onError: reportError, generateContent: connection.capabilities?.features.includes('session_generation')
                                                                                                    ? sessionActions.generateSessionContent
                                                                                                    : undefined }));
                                                                                            const messageListWithSubagentDetails = (_jsx(SubagentDetailsProvider, { onOpen: openSubagentPanel, children: messageListContent }));
                                                                                            const messageList = monitorDetailsSupported ? (_jsx(MonitorDetailsProvider, { onOpen: openMonitorPanelFromTool, children: messageListWithSubagentDetails })) : (messageListWithSubagentDetails);
                                                                                            const btwPanel = !showMobileWelcomeFooterMiddle &&
                                                                                                btwMessage?.role === 'btw' ? (_jsx("div", { className: styles.btwPanel, children: _jsx(BtwMessage, { question: btwMessage.question, answer: btwMessage.answer, isPending: btwMessage.isPending }) })) : null;
                                                                                            if (showMobileWelcomeFooterMiddle) {
                                                                                                return (_jsxs("div", { className: styles.mobileWelcomeGroup, children: [_jsxs("div", { style: contentStyle, className: contentClassName, children: [messageList, btwPanel] }), _jsx("div", { className: styles.mobileWelcomeFooterMiddle, children: welcomeFooter })] }));
                                                                                            }
                                                                                            return (_jsxs("div", { style: contentStyle, className: contentClassName, children: [messageList, btwPanel] }));
                                                                                        })() }) }) }), _jsxs("div", { ref: footerRef, style: contentStyle, className: CustomFooter
                                                                                    ? `${styles.footer} ${styles.footerWithCustomFooter}`
                                                                                    : styles.footer, children: [canScrollMessageListToBottom && (_jsx("div", { className: showBottomPanels
                                                                                            ? `${styles.scrollToBottomLayer} ${styles.scrollToBottomLayerWithTodos}`
                                                                                            : styles.scrollToBottomLayer, children: _jsx("button", { type: "button", className: styles.scrollToBottomButton, "aria-label": t('chat.scrollToBottom'), onClick: () => resumeChatBottomFollow('smooth'), children: _jsx("svg", { className: styles.scrollToBottomIcon, viewBox: "0 0 24 24", "aria-hidden": "true", children: _jsx("path", { d: "M12 5v13M6.5 12.5 12 18l5.5-5.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }) }) }) })), showBottomPanels && (_jsx("div", { ref: bottomPanelsRef, className: styles.bottomPanels, children: _jsx(TodoPanel, { todos: showFloatingTodos ? floatingTodos : [], statusItems: floatingBottomStatusItems, onOpen: sessionWorkflowEnabled && showFloatingTodos
                                                                                                ? openTasksPanel
                                                                                                : undefined }) })), pendingToolApproval && mainView === 'chat' && (_jsx("div", { "data-testid": "approval-overlay", className: styles.approvalOverlay, children: _jsx(ToolApproval, { request: pendingToolApproval, onConfirm: handleConfirm, variant: "floating", keyboardActive: toolApprovalOverlayVisible, planTodos: sessionWorkflowEnabled ? approvalPlanTodos : [] }) })), pendingAskUserApproval && mainView === 'chat' && (_jsx("div", { "data-testid": "approval-overlay", className: styles.approvalOverlay, children: _jsx(AskUserQuestion, { request: pendingAskUserApproval, onConfirm: handleAskUserConfirm, onError: reportError, variant: "floating", keyboardActive: askUserOverlayVisible }) })), _jsxs("div", { className: styles.composer, children: [streamingState !== 'idle' ? (suppressFailedPromptRetryStreaming ? null : (_jsx(StreamingStatus, { startedAt: failedPromptRetry?.startedAt ??
                                                                                                    activeTurnStartedAt }))) : newSessionSuggestion ? (_jsxs("div", { className: styles.composerActionTip, role: "status", "data-testid": newSessionSuggestion.suggestion === 'btw'
                                                                                                    ? 'btw-suggestion'
                                                                                                    : 'new-session-suggestion', children: [_jsx("span", { className: styles.composerActionTipIcon, "aria-hidden": "true", children: "\u2726" }), _jsx("span", { className: styles.composerActionTipText, children: newSessionSuggestion.suggestion === 'btw'
                                                                                                            ? t('editor.btwSuggestionTitle')
                                                                                                            : t('editor.newSessionSuggestionTitle') }), _jsx("div", { className: styles.composerActionTipActions, children: _jsx("button", { type: "button", className: `${styles.composerActionTipButton} ${styles.composerActionTipButtonPrimary}`, "data-testid": newSessionSuggestion.suggestion === 'btw'
                                                                                                                ? 'btw-suggestion-send'
                                                                                                                : 'new-session-suggestion-start', onMouseDown: (event) => event.preventDefault(), onClick: newSessionSuggestion.suggestion === 'btw'
                                                                                                                ? handleAcceptBtwSuggestion
                                                                                                                : handleAcceptNewSessionSuggestion, children: newSessionSuggestion.suggestion === 'btw'
                                                                                                                ? t('editor.btwSuggestionSend')
                                                                                                                : t('editor.newSessionSuggestionStart') }) })] })) : escapeHintVisible && streamingState === 'idle' ? (_jsx("div", { className: styles.escClearStatus, role: "status", children: t('editor.escClearHint') })) : null, unknownPromptAdmission &&
                                                                                                unknownPromptAdmission.sessionId ===
                                                                                                    connection.sessionId && (_jsxs("div", { className: styles.composerActionTip, role: "status", "data-testid": "prompt-admission-unknown", children: [_jsx("span", { className: styles.composerActionTipIcon, "aria-hidden": "true", children: "!" }), _jsx("span", { className: styles.composerActionTipText, children: t('queue.admissionUnknown') }), unknownPromptAdmission.payloadAvailable && (_jsxs("div", { className: styles.composerActionTipActions, children: [_jsx("button", { type: "button", className: `${styles.composerActionTipButton} ${styles.composerActionTipButtonPrimary}`, onClick: restoreUnknownPromptPayload, children: t('queue.restoreUnknown') }), _jsx("button", { type: "button", className: styles.composerActionTipButton, onClick: discardUnknownPromptPayload, children: t('queue.discardUnknown') })] }))] })), _jsx(QueuedPromptDisplay, { prompts: queuedPrompts, t: t, canMutateMidTurn: canMutateMidTurn, onDelete: removeQueuedPrompt, onEdit: editQueuedPrompt, onRestoreUnknown: restoreUnknownQueuedPrompt, onDiscardUnknown: discardUnknownQueuedPrompt }), CustomComposerHeader && (_jsx("div", { className: styles.composerHeader, children: _jsx(CustomComposerHeader, { disabled: isDisabled ||
                                                                                                        unknownPromptAdmission?.payloadAvailable === true, isRunning: streamingState !== 'idle', currentMode: currentMode, currentModel: currentModel, sessionName: sessionDisplayName }) })), _jsx(ChatEditor, { ref: setEditorHandle, onSubmit: handleEditorSubmit, onInputTextChange: handleComposerTextChange, onAttachmentsChange: handleComposerAttachmentsChange, onImageIngestionNotice: pushToast, onImagePreview: openImagePanel, onCycleMode: handleCycleMode, onToggleShortcuts: handleToggleShortcuts, onCancel: handleCancel, isRunning: streamingState !== 'idle', isPreparing: isPreparingPrompt || isStartingNewSessionSuggestion, cancelArmed: cancelArmed, disabled: isDisabled ||
                                                                                                    isStartingNewSessionSuggestion ||
                                                                                                    interactionBlocked ||
                                                                                                    approvalOverlayActive ||
                                                                                                    unknownPromptAdmission?.payloadAvailable === true, commands: commands, skills: loadedSkills, slashCommandCategoryOrder: slashCommandCategoryOrder, builtinAtProviders: builtinAtProviders, atProviders: atProviders, composerTagIcons: composerTagIcons, voiceTarget: activePanel !== null || mainView !== 'chat'
                                                                                                    ? undefined
                                                                                                    : mainVoiceTarget, voiceStatusRevision: mainVoiceStatusRevision, queuedMessages: queuedTexts, onFocusFooter: handleFocusTaskPill, onPopQueuedMessages: editLastQueuedPrompt, onClearQueuedMessages: clearQueuedPrompts, currentMode: currentMode, sessionWorkflowEnabled: sessionWorkflowEnabled, currentModel: currentModel, gitBranch: activeGitBranch, gitWorktree: Boolean(sessionWorktree), gitCwd: sessionWorktree?.path, gitModeIntent: gitModeEligible ? gitModeIntent : undefined, onGitModeIntentChange: gitModeEligible ? setGitModeIntent : undefined, gitStatus: selectedWorkspaceGitStatus, onOpenGitDiff: gitDiffWorkspaceCwd
                                                                                                    ? handleOpenGitDiff
                                                                                                    : undefined, onOpenCommit: gitDiffWorkspaceCwd
                                                                                                    ? handleOpenCommit
                                                                                                    : undefined, chatWidthMode: chatWidthMode, showChatWidthToggle: !isChatEmptyState, chatWidthToggleMin: chatWidthToggleMin, visibleToolbarActions: composerToolbarActions ??
                                                                                                    (isChatEmptyState ||
                                                                                                        !environmentGitReplacementEnabled
                                                                                                        ? DEFAULT_EMPTY_COMPOSER_TOOLBAR_ACTIONS
                                                                                                        : DEFAULT_COMPOSER_TOOLBAR_ACTIONS), tokenCount: connection.tokenCount ?? 0, contextWindow: connection.contextWindow ?? 0, onShowContextUsage: handleShowContextUsage, availableModels: availableModels, onSelectMode: handleSetMode, onSelectModel: handleModelSelect, reasoning: connection.reasoning, onSelectReasoningEffort: handleReasoningEffort, workspaces: composerWorkspaces, selectedWorkspaceCwd: connection.sessionId
                                                                                                    ? workspaces.find((entry) => entry.cwd === connection.workspaceCwd)?.primary
                                                                                                        ? undefined
                                                                                                        : connection.workspaceCwd
                                                                                                    : selectedWorkspaceCwd, workspaceSelectionDisabled: false, atWorkspaceCwd: lockedWorkspaceCwd ??
                                                                                                    (connection.sessionId
                                                                                                        ? connection.workspaceCwd
                                                                                                        : (selectedWorkspaceCwd ??
                                                                                                            workspaces.find((entry) => entry.primary)?.cwd)), onSelectWorkspace: handleSelectComposerWorkspace, scratchWorkspaceSupported: scratchWorkspaceRegistrationSupported, existingFolderWorkspaceSupported: dynamicWorkspaceRegistrationSupported, workspaceMutationBusy: workspaceMutationBusy, onCreateScratchWorkspace: handleCreateComposerScratchWorkspace, onOpenExistingWorkspace: handleOpenExistingWorkspace, onChatWidthModeChange: handleChatWidthModeChange, sessionId: connection.sessionId, sessionName: sessionDisplayName, dialogOpen: interactionBlocked || approvalOverlayActive, followupState: followupState, onAcceptFollowup: onAcceptFollowup, onDismissFollowup: onDismissFollowup, composerInput: composerInput, composerInputVersion: composerInputVersion, placeholderText: composerPlaceholderText, animatePlaceholder: isChatEmptyState }), CustomComposerFooter && (_jsx(CustomComposerFooter, { disabled: isDisabled, isRunning: streamingState !== 'idle', currentMode: currentMode, currentModel: currentModel, sessionName: sessionDisplayName }))] }), CustomFooter ? (hasMobileComposerBottom ? (_jsx("div", { className: styles.customFooter, children: _jsx(CustomFooter, { connected: connected, mode: currentMode, model: currentModel, streamingState: streamingState, contextUsageRatio: (connection.contextWindow ?? 0) > 0
                                                                                                ? (connection.tokenCount ?? 0) /
                                                                                                    (connection.contextWindow ?? 0)
                                                                                                : 0, activeGoal: activeGoal, tasks: footerTasks, availableModes: MODES_CYCLE, availableModels: (connection.models ?? [])
                                                                                                .filter(isVisibleComposerModel)
                                                                                                .map((m) => ({
                                                                                                id: m.id,
                                                                                                label: getModelDisplayName(m.label || m.id),
                                                                                                contextWindow: m.contextWindow,
                                                                                            })), skills: loadedSkills, onSelectMode: handleSetMode, onSelectModel: handleModelSelect }) })) : (_jsx(CustomFooter, { connected: connected, mode: currentMode, model: currentModel, streamingState: streamingState, contextUsageRatio: (connection.contextWindow ?? 0) > 0
                                                                                            ? (connection.tokenCount ?? 0) /
                                                                                                (connection.contextWindow ?? 0)
                                                                                            : 0, activeGoal: activeGoal, tasks: footerTasks, availableModes: MODES_CYCLE, availableModels: (connection.models ?? [])
                                                                                            .filter(isVisibleComposerModel)
                                                                                            ...
                                                                                            label: getModelDisplayName(m.label || m.id),
                                                                                            contextWindow: m.contextWindow,
                                                                                        })), skills: loadedSkills, onSelectMode: handleSetMode, onSelectModel: handleModelSelect }))) : (_jsx(StatusBar, { onSelectMode: () => setShowApprovalModeDialog((v) => !v), onSelectModel: () => setModelDialogMode((v) => (v ? null : 'main')), onShowContext: () => showContextUsage('/context', false), onOpenSettings: () => openPanel('settings'), ref: statusBarRef, onOpenTasks: () => openTasksPanel(), onReturnToInput: handleReturnToEditor, tasks: isChatEmptyState ||
                                                                                            !environmentTasksReplacementEnabled
                                                                                            ? backgroundTasks
                                                                                            : [], activeGoal: activeGoal, onOpenGoals: openGoals, hideSettings: hideSettings, onToggleShortcuts: handleToggleShortcuts, compact: true })), isChatEmptyState && welcomeFooter && (_jsx("div", { className: [
                                                                                            ...
                                                                                            .filter(Boolean)
                                                                                            .join(' '), children: welcomeFooter }))] })] }) })] })] }), environmentPanelMounted && (_jsx(EnvironmentPanel, { floating: !environmentPanelFits, hidden: !environmentPanelVisible || artifactPanelFullscreen, workspaceCwd: sessionWorktree?.path ?? activeWorkspaceCwd, gitWorkspaceCwd: connection.sessionId ? gitDiffWorkspaceCwd : undefined, gitCwd: sessionWorktree?.path, branch: activeGitBranch, gitStatus: selectedWorkspaceGitStatus, tasks: sessionTasks, agentTasks: environmentAgentTasks, items: environmentPanelItems, onOpenGitDiff: gitDiffWorkspaceCwd ? handleOpenGitDiff : undefined, onOpenGitCommit: gitDiffWorkspaceCwd ? handleOpenCommit : undefined, onOpenAgent: openEnvironmentAgent, onOpenTask: openEnvironmentTask, onDismiss: dismissEnvironmentPanel })), artifactPanelOpen && useFloatingArtifactPanel && (_jsx(Drawer, { open: true, direction: "right", shouldScaleBackground: false, onOpenChange: (open) => {
                                                        if (!open)
                                                            closeArtifactPanel();
                                                    }, children: _jsxs(DrawerContent, { onAnimationEnd: (event) => {
                                                            ...
                                                            : 'data-[vaul-drawer-direction=right]:w-[min(520px,calc(100vw-16px))] data-[vaul-drawer-direction=right]:sm:max-w-[520px]', onEscapeKeyDown: (event) => {
                                                            ...
                                                            event.preventDefault();
                                                            setArtifactPanelFullscreen(false);
                                                        }, children: [_jsx(DrawerTitle, { className: "sr-only", children: "Right panel" }), _jsx(ArtifactPanel, { ...artifactPanelSharedProps, variant: "drawer" })] }) }))] })] }), _jsx("div", { ref: setArtifactPanelSlotEl, className: styles.artifactPanelSlot }), artifactPanelOpen &&
                                    !useFloatingArtifactPanel &&
                                    artifactPanelSlotEl &&
                                    createPortal(_jsxs("div", { ref: artifactPanelFullscreenSurfaceRef, tabIndex: -1, onKeyDown: handleArtifactPanelSurfaceKeyDown, onAnimationEnd: (event) => {
                                            if (event.target === event.currentTarget) {
                                                setWaitForSubagentPanelAnimation(false);
                                            }
                                        }, ...(artifactPanelFullscreen
                                        ...
                                        }, children: [!artifactPanelFullscreen && (_jsx("div", { className: styles.artifactResizeHandle, role: "separator", "aria-orientation": "vertical", "aria-valuemin": MIN_ARTIFACT_PANEL_WIDTH, "aria-valuemax": getMaxArtifactPanelWidth(), "aria-valuenow": artifactPanelWidth, onPointerDown: handleArtifactPanelResizeStart })), _jsx("div", { className: styles.artifactPanelClip, children: _jsx(ArtifactPanel, { ...artifactPanelSharedProps, panelWidth: artifactPanelWidth }) })] }), artifactPanelSlotEl)] })] }) }) }) }));
}
