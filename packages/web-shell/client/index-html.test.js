...
function installMeasureGuard(measure) {
    const html = readFileSync(resolve(__dirname, 'index.html'), 'utf8');
    const script = Array.from(html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g))
        .map((match) => match[1] ?? '')
        .find((source) => source.includes('performance.measure ='));
    if (!script)
        throw new Error('Performance measure guard not found');
...
describe('React performance measure guard', () => {
    it('removes React component detail before calling the native measure', () => {
        ...
        const forwardedOptions = measure.mock.calls[0]?.[1];
