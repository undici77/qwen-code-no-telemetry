...
export function WebShellWithProviders(props) {
    const { baseUrl, token, sessionId, workspaceId, workspaceCwd, lockWorkspaceCwd, clientId, restartSseOnPrompt, historyPageSize, ...webShellProps } = props;
    const resolvedBaseUrl = resolveBaseUrl(baseUrl);
    return (_jsx(RootBoundary, { language: webShellProps.language
            ? normalizeLanguage(webShellProps.language)
            : undefined, children: _jsx(DaemonWorkspaceProvider, { baseUrl: resolvedBaseUrl, token: token, children: _jsx(WorkspaceSessionProvider, { sessionId: sessionId, workspaceId: workspaceId, workspaceCwd: workspaceCwd, lockWorkspaceCwd: lockWorkspaceCwd, clientId: clientId, restartSseOnPrompt: restartSseOnPrompt, historyPageSize: historyPageSize, webShellProps: webShellProps }) }) }));
}
