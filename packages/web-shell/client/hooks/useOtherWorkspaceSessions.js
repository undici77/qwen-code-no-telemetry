...
import { SESSION_LIST_PAGE_SIZE, WEB_SHELL_SESSION_SOURCE_TYPE, } from '../constants/sessions';
import { useSessionCatalogQueries } from '../session-catalog/session-catalog-hooks';
import { getSessionCatalogStore, } from '../session-catalog/session-catalog-store';
const EMPTY = [];
export function useOtherWorkspaceSessions(enabled = true, pollIntervalMs) {
    const workspace = useWorkspace();
    const client = workspace.client;
    const targetsKey = JSON.stringify(enabled
        ? (workspace.capabilities?.workspaces ?? [])
            .filter((entry) => !entry.primary && entry.trusted)
            .map((entry) => entry.cwd)
        : []);
    const queries = useMemo(() => JSON.parse(targetsKey).map((workspaceCwd) => ({
        routeKind: 'legacy',
        workspaceCwd,
        options: {
    ...
    useEffect(() => {
        ...
        for (const cwd of reportedErrors.current.keys()) {
            if (!activeCwds.has(cwd))
                reportedErrors.current.delete(cwd);
        }
        snapshots.forEach((snapshot, index) => {
            const error = snapshot.error;
            const cwd = queries[index]?.workspaceCwd;
            if (!error || !cwd || reportedErrors.current.get(cwd) === error)
                return;
            reportedErrors.current.set(cwd, error);
            console.warn(`[useOtherWorkspaceSessions] failed to list sessions for ${cwd}:`, error);
        });
    }, [queries, snapshots]);
    const sessions = useMemo(() => {
        if (snapshots.length === 0)
            return EMPTY;
        const merged = snapshots.flatMap((snapshot) => snapshot.page?.sessions ?? []);
...
//# sourceMappingURL=useOtherWorkspaceSessions.js.map