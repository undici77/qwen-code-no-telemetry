...
import { useNewSessionSuggestion, } from './useNewSessionSuggestion';
...
describe('useNewSessionSuggestion', () => {
    ...
    async function classify(decisionText, inputText = NEW_TASK_DRAFT, messages = CONTEXT_MESSAGES) {
        vi.useFakeTimers();
    ...
    it('classifies a side question after only one prior exchange', async () => {
        ...
        await classify(JSON.stringify({ suggestion: 'btw', confidence: 0.92 }), sideQuestion);
        expect(testState.generateContent).toHaveBeenCalledOnce();
        expect(latestSuggestion).toEqual({
            suggestion: 'btw',
    ...
    it('lets common side-question wording reach the classifier', async () => {
        ...
        await classify(JSON.stringify({ suggestion: 'btw', confidence: 0.9 }), sideQuestion);
        expect(testState.generateContent).toHaveBeenCalledOnce();
        expect(latestSuggestion?.suggestion).toBe('btw');
    });
    it('does not surface new_session from the relaxed BTW context floor', async () => {
        await classify(JSON.stringify({ suggestion: 'new_session', confidence: 0.96 }), '这里的 confidence 阈值为什么是 0.75？');
        expect(testState.generateContent).toHaveBeenCalledOnce();
        expect(latestSuggestion).toBeNull();
    });
    it('does not classify BTW with less than one prior exchange', async () => {
        await classify(JSON.stringify({ suggestion: 'btw', confidence: 0.96 }), '这里的 confidence 阈值为什么是 0.75？', CONTEXT_MESSAGES.slice(0, 1));
        expect(testState.generateContent).not.toHaveBeenCalled();
        expect(latestSuggestion).toBeNull();
    });
    it.each([true, null])('does not classify a low-context side question when attachment presence is %s', async (hasAttachments) => {
    ...
    it('recovers a positive decision wrapped in prose (observed live)', async () => {
        ...
        await classify('The user is explicitly switching to a completely new task, which is ' +
    ...
    it('keeps the banner hidden for a valid none decision', async () => {
        await classify('This is a follow-up on the same topic.\n\n' +
    ...
    it.each([true, null])('does not suggest BTW when attachment presence is %s', async (hasAttachments) => {
        testState.hasAttachments = hasAttachments;
        await classify(JSON.stringify({ suggestion: 'btw', confidence: 0.92 }));
        expect(latestSuggestion).toBeNull();
    });
    ...
    it('stays fail-closed on prose with no recoverable JSON object', async () => {
        await classify('I think {this draft} switches topics, but here is no JSON to parse.');
        expect(testState.generateContent).toHaveBeenCalledOnce();
        expect(latestSuggestion).toBeNull();
    });
});
//# sourceMappingURL=useNewSessionSuggestion.test.js.map