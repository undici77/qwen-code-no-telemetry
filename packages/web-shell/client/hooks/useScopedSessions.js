...
import { useSessionCatalogController, useSessionCatalogQuery, useWebShellSessions, } from '../session-catalog/session-catalog-hooks';
export function useScopedSessions(workspaceCwd, options = {}) {
    const { autoLoad = false, enabled = true, maxAgeMs, pageSize, archiveState, view, group, pollIntervalMs, } = options;
    ...
    const query = useMemo(() => workspaceCwd
        ...
        : undefined, [archiveState, group, pageSize, view, workspaceCwd]);
    ...
    const deleteSessions = useCallback(async (sessionIds) => {
        if (!workspaceCwd)
            return primaryDeleteSessions(sessionIds);
        try {
    ...
    const deleteSession = useCallback(async (sessionId) => {
        if (!workspaceCwd)
            return primaryDeleteSession(sessionId);
