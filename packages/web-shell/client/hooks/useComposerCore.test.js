...
import { buildComposerPrompt, buildComposerPromptWithInlineTagPlacements, getComposerTagDisplay, getComposerTagLabel, getComposerTagValue, getFollowupCompletion, replaceInlineTagPlacements, serializeComposerTag, } from './useComposerCore';
describe('follow-up completion helpers', () => {
    ...
    it('keeps a suggestion available while the user types a matching prefix', () => {
        expect(getFollowupCompletion('show', 'show me tests')).toBe('show me tests');
    });
...
describe('composer tag serialization', () => {
    ...
    it('builds prompts with tags before user text', () => {
        expect(buildComposerPrompt('do it', [
            { id: 'file', label: 'File', value: 'src/a.ts' },
            { id: 'goal', label: 'Goal' },
        ])).toBe('src/a.ts\nGoal\n\ndo it');
        expect(buildComposerPrompt('', [{ id: 'file', value: 'src/a.ts' }])).toBe('src/a.ts');
        expect(buildComposerPrompt('do it', [])).toBe('do it');
    });
    it('keeps inline tags at their editor positions', () => {
        expect(buildComposerPromptWithInlineTagPlacements('explain @orders now', [{ id: 'top', serialized: '<top />' }], [
            {
                start: 8,
                end: 15,
    ...
    it('ignores invalid and overlapping inline tag placements', () => {
        expect(replaceInlineTagPlacements('a @one and @two', [
            {
                start: -1,
                end: 2,
...
//# sourceMappingURL=useComposerCore.test.js.map