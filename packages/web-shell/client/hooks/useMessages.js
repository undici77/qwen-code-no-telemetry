...
import { useConnection, useTranscriptBlocks, useWorkspace, } from '@qwen-code/webui/daemon-react-sdk';
import { transcriptBlocksToDaemonMessages } from '../adapters/transcriptToMessages';
import { isActiveToolStatus, isBackgroundSubAgentToolCall, } from '../adapters/toolClassification';
...
export function reconcileBackgroundAgentResolutions(messages, resolutions) {
    ...
    const reconciled = messages.map((message) => {
        ...
        const tools = message.tools.map((tool) => {
            ...
            toolsChanged = true;
            const cancelled = resolution.status === 'cancelled' || resolution.status === 'canceled';
...
export function useMessagesFromBlocks(t, blocks) {
    ...
    const pendingBackgroundAgentKey = useMemo(() => getPendingBackgroundAgentKey(messages), [messages]);
    const backgroundAgentNotificationKey = useMemo(() => getBackgroundAgentNotificationKey(blocks), [blocks]);
    ...
    useEffect(() => {
        ...
        const request = existingRequest?.key === requestKey
            ...
            : Promise.allSettled(callIds.map(async (callId) => {
                const resolution = await workspace.client.resolveSubagentSession(sessionId, callId);
    ...
    return useMemo(() => {
        ...
        return reconcileBackgroundAgentResolutions(messages, resolutionSnapshot.resolutions);
