...
const sdkMock = vi.hoisted(() => {
    const pendingEventListeners = new Set();
    const mock = {
        actions: {
        ...
        publishPendingEvents: (events) => {
            mock.pendingEvents = events;
            for (const listener of [...pendingEventListeners])
                listener();
        },
    };
    return mock;
...
vi.mock('@qwen-code/webui/daemon-react-sdk', async () => {
    const actual = await vi.importActual('@qwen-code/webui/daemon-react-sdk');
    ...
    return {
        ...
        subscribePendingPromptEvents: (listener) => {
        ...
        consumePendingPromptEvents: (handled) => {
            ...
            const next = sdkMock.pendingEvents.filter((event) => !handledSet.has(event));
            if (next.length === sdkMock.pendingEvents.length)
                return;
...
function createHarness() {
    ...
    const stableEditorRef = { current: stableEditor };
    const stableT = ((key) => key);
    const stableReportError = vi.fn();
    function TestComponent(opts) {
        latest = useQueuedPrompts({
            ...
            sessionActions: sdkMock.actions,
            store: stableStore,
    ...
    const render = async (opts) => {
        await act(async () => {
            root.render(_jsx(TestComponent, { ...opts }));
        });
...
describe('useQueuedPrompts mid-turn reconciliation (session_mid_turn_message_query)', () => {
    beforeEach(() => {
        ...
        sdkMock.actions.enqueueMidTurnMessage.mockImplementation((_message, opts) => Promise.resolve({ accepted: true, messageId: opts?.messageId }));
    ...
    it('keeps a connect snapshot across active streaming substates', async () => {
        let resolveSnapshot;
        sdkMock.actions.getMidTurnMessages.mockImplementation(() => new Promise((resolve) => {
    ...
    it('prunes a stale queued row whose id was already injected (no resend)', async () => {
        ...
        sdkMock.actions.enqueueMidTurnMessage.mockImplementation((_message, opts) => {
            sdkMock.actions.getMidTurnMessages.mockResolvedValue({
                messages: [],
    ...
    it('never queries when the daemon lacks the capability (degraded)', async () => {
        ...
        try {
            ...
            expect(sdkMock.actions.submitPrompt).toHaveBeenCalledWith('note', expect.objectContaining({ sessionId: 'session-a' }));
        }
        finally {
            await harness.dispose();
        }
    ...
    it('falls back when a legacy admission response reaches an idle turn', async () => {
        ...
        sdkMock.actions.enqueueMidTurnMessage.mockReturnValue(new Promise((resolve) => {
        ...
        try {
            ...
            expect(sdkMock.actions.submitPrompt).toHaveBeenCalledWith('legacy late response', expect.objectContaining({ sessionId: 'session-a' }));
        }
        finally {
            await harness.dispose();
        }
    ...
    it('does not resubmit when an accepted response arrives after idle', async () => {
        ...
        try {
            ...
            const messageId = sdkMock.actions.enqueueMidTurnMessage.mock.calls[0]?.[1]?.messageId;
            await harness.render({ streamingState: 'idle' });
            sdkMock.actions.getMidTurnMessages.mockResolvedValue({
                messages: [],
                settledMessageIds: [],
    ...
    it('reconciles an ambiguous admission without retrying or falling back', async () => {
        sdkMock.actions.enqueueMidTurnMessage.mockRejectedValueOnce(new Error('response lost'));
    ...
    it('does not report an admission failure after the user switches sessions', async () => {
        let rejectAdmission;
        sdkMock.actions.enqueueMidTurnMessage.mockReturnValueOnce(new Promise((_resolve, reject) => {
    ...
    it('keeps a promoted row visible when pending-prompt refresh fails', async () => {
        ...
        try {
            ...
            sdkMock.actions.getPendingPrompts.mockRejectedValue(new Error('pending snapshot unavailable'));
            await harness.render({ streamingState: 'responding', connected: false });
            await harness.render({ streamingState: 'responding', connected: true });
            expect(harness.result().queuedPrompts[0]).toMatchObject({
    ...
    it('does not restore a deleted row from an older snapshot', async () => {
        ...
        try {
            ...
            const row = harness.result().queuedPrompts[0];
            let resolveSnapshot;
            sdkMock.actions.getMidTurnMessages.mockImplementationOnce(() => new Promise((resolve) => {
                resolveSnapshot = resolve;
            }));
            await harness.render({ streamingState: 'idle' });
            await act(async () => {
    ...
    it('does not create local state while daemon admission is pending', async () => {
        ...
        try {
            await harness.render({ streamingState: 'responding' });
            sdkMock.actions.enqueueMidTurnMessage.mockReturnValue(new Promise(() => { }));
            await act(async () => {
                harness.result().enqueuePrompt('note');
            });
            expect(harness.result().queuedPrompts).toEqual([]);
    ...
    it('preserves an ambiguous stable-id admission across later reattachment', async () => {
        let rejectAdmission;
        sdkMock.actions.enqueueMidTurnMessage.mockReturnValue(new Promise((_resolve, reject) => {
            rejectAdmission = reject;
        }));
        const harness = createHarness();
        try {
    ...
    it('does not resurrect an admission after authoritative settlement', async () => {
        ...
        try {
            ...
            if (!messageId)
                throw new Error('missing stable message id');
            sdkMock.actions.getMidTurnMessages.mockResolvedValue({
                messages: [],
                settledMessageIds: [messageId],
                promotedMessageIds: [],
    ...
    it('does not fall back after an idle reconciliation is blocked', async () => {
        ...
        try {
            ...
            sdkMock.actions.getMidTurnMessages.mockImplementationOnce((opts) => new Promise((resolve) => {
    ...
    it('materializes the queued row mid-turn after an accepted admission', async () => {
        ...
        try {
            ...
            expect(harness.result().queuedPrompts[0]).toMatchObject({
                text: 'mid-turn note',
                midTurnState: 'queued',
                midTurnMessageId: sdkMock.actions.enqueueMidTurnMessage.mock.calls[0]?.[1]?.messageId,
            });
            expect(sdkMock.actions.submitPrompt).not.toHaveBeenCalled();
        }
        finally {
    ...
    it('aborts an in-flight reconcile when the session changes', async () => {
        const signals = [];
        sdkMock.actions.getMidTurnMessages.mockImplementation((opts) => {
    ...
    it('renders a stable-id message the daemon promoted and started immediately', async () => {
        ...
        try {
            ...
            expect(harness.store.appendLocalUserMessage).toHaveBeenCalledWith('settled late', undefined, undefined);
        }
        finally {
            await harness.dispose();
        }
...
//# sourceMappingURL=useQueuedPrompts.midTurnReconcile.test.js.map