...
function createBuiltinProviderCache() {
    return {
        directories: new Map(),
        globResults: new Map(),
...
function shallowEqualMenuState(current, next) {
    if (current === next)
        return true;
    if (!current || !next)
        return false;
...
function equalProviderViews(current, next) {
    return (current.length === next.length &&
        current.every((provider, index) => {
            const nextProvider = next[index];
            return (nextProvider !== undefined &&
...
function isBuiltinProviderEnabled(providerId, config) {
    if (config === undefined || config === true)
        return true;
    if (config === false)
        return false;
    if (Array.isArray(config)) {
        return config.includes(providerId);
    }
    const options = config;
    if (options.enabled === false)
        return false;
    if (options.include && !options.include.includes(providerId))
        return false;
    if (options.exclude?.includes(providerId))
        return false;
    return true;
}
function getRegisteredCustomProviders(customProviders) {
    const registeredIds = new Set(BUILTIN_PROVIDER_IDS);
    const accepted = [];
    for (const provider of customProviders) {
        if (registeredIds.has(provider.id)) {
            console.error(`[@mention] duplicate provider id="${provider.id}" ignored`);
            continue;
        }
...
function getProviderTextValue(provider) {
    return ((provider.textValue === undefined
        ? undefined
...
function joinWorkspacePath(dirPath, name) {
    if (dirPath === '.' || dirPath === '')
        return name;
...
function escapeGlobQuery(query) {
...
function fileSearchGlobPattern(query) {
...
function directoryInsertText(path) {
...
function unescapeAtReferenceText(ref) {
    return ref.replace(/\\(.)/g, '$1');
}
function isSafeTextChar(char) {
    const code = char.charCodeAt(0);
    return code > 0x1f && code !== 0x7f && (code < 0x80 || code > 0x9f);
}
function mcpResourceInsertText(serverName, uri) {
    return `@${escapeAtReferenceText(buildMcpResourceRef(sanitizeInsertText(serverName), sanitizeInsertText(uri)))} `;
}
function splitInsertedReferenceQuery(query, lastSelectedProviderId, lastSelectedMcpServerName) {
    const unescapedQuery = unescapeAtReferenceText(query);
    if (lastSelectedProviderId === MCP_RESOURCES_PROVIDER_ID &&
...
function getProviderQueryFromMention(providerId, parsedQuery, mcpServerName) {
...
async function isEnabledMcpServer(actions, serverName, signal, loadStatus) {
    if (signal?.aborted)
        return false;
    ...
    try {
        ...
        if (signal?.aborted)
            return false;
        return status.servers.some((server) => server.name === serverName && !server.disabled);
    }
    catch (error) {
...
function splitFileQuery(query, fallbackDir) {
    ...
    const dirPath = fallback === '.' ? dirQuery : joinWorkspacePath(fallback, dirQuery);
    return {
        dirPath: normalizeDirectoryPath(dirPath),
        entryQuery: normalizedQuery.slice(slashIndex + 1),
    };
}
function parseAtMention(view) {
    if (!view)
        return null;
 ...
 */
export function fileReferenceInsertText(filePath) {
    return `@${escapeAtReferenceText(sanitizeInsertText(filePath))} `;
}
function safeDisplayText(raw) {
    if (raw === undefined)
        return SAFE_DISPLAY_FALLBACK;
    return sanitizeDisplayText(raw) ?? SAFE_DISPLAY_FALLBACK;
}
function sanitizeOptionalInsertText(raw) {
    if (raw === undefined)
        return undefined;
    return sanitizeInsertText(raw).trim();
}
function sanitizeComposerTag(tag) {
    if (!tag)
        return undefined;
    const kind = tag.kind === undefined ? undefined : sanitizeDisplayText(tag.kind);
    return {
...
function sanitizeAtMentionItem(item, options) {
    const sanitized = {
        ...item,
        ...
        detail: item.detail === undefined ? undefined : sanitizeDisplayText(item.detail),
        icon: item.icon,
        iconTooltip: item.iconTooltip === undefined
            ? undefined
            : sanitizeDisplayText(item.iconTooltip),
        insertText: item.insertText === undefined
            ? undefined
...
function createComposerTagForItem(providerId, item, insert) {
...
function createFileProvider(getActions, getCurrentDir, getCache, label, description, getUploadItem) {
    return {
        id: FILE_PROVIDER_ID,
        label,
        description,
        ...
        async search({ query, signal }) {
            ...
            if (listDirectory && (!query || !actions?.globWorkspace)) {
                try {
                    ...
                    const listing = await getCached(getCache().directories, dirPath, () => listDirectory(dirPath, { signal }));
                    if (signal.aborted)
                        return [];
                    ...
                    return [
                        ...
                        ...entries.slice(0, ITEM_LIMIT).map((entry) => {
            ...
            if (!globWorkspace) {
                console.warn('[@mention] file provider unavailable: workspace actions are not configured');
                return [];
            }
            try {
                const pattern = fileSearchGlobPattern(query);
                const result = await getCached(getCache().globResults, pattern, () => globWorkspace(pattern, { maxResults: ITEM_LIMIT, signal }));
                if (signal.aborted)
                    return [];
...
function createExtensionProvider(getActions, getCache, label, description) {
    return {
        ...
        async search({ query, signal }) {
            ...
            try {
                ...
                return status.extensions
                    ...
                    const insertName = escapeAtReferenceText(sanitizeInsertText(ext.name));
                    ...
                    return {
                        id: ext.name,
                        label,
                        description: displayName && displayName !== label
                            ? displayName
                            : description,
...
function createMcpResourcesProvider(getActions, getCache, label, description, formatResourceCount) {
    return {
        ...
        async search({ query, signal }) {
            ...
            try {
                ...
                return status.servers
                    ...
                    const count = server.resourceCount === undefined
                        ? undefined
...
function nextSelectionIndex(current, total, direction) {
    if (total <= 0)
        return null;
    if (direction === 'up')
...
export function useAtMentionMenu({ viewRef, disabledRef, shellModeRef, workspaceActionsRef, workspaceKey, builtinProviders, providers = EMPTY_PROVIDERS, createInlineTagEffect, onUploadRequest, }) {
    const { t } = useI18n();
    ...
    const builtinCacheRef = useRef(createBuiltinProviderCache());
    const lastSelectedProviderIdRef = useRef(null);
    const lastSelectedMcpServerNameRef = useRef(null);
    const preserveProviderSelectionRef = useRef(false);
    const allProviders = useMemo(() => {
        const builtinAtProviders = [
            createFileProvider(() => workspaceActionsRef.current, () => fileDirectoryRef.current, () => builtinCacheRef.current, t('at.category.files'), t('at.category.files.description'), () => onUploadRequest
                ? {
                    id: 'upload-file',
                ...
                : null),
            createExtensionProvider(() => workspaceActionsRef.current, () => builtinCacheRef.current, t('at.category.extensions'), t('at.category.extensions.description')),
            createMcpResourcesProvider(() => workspaceActionsRef.current, () => builtinCacheRef.current, t('at.category.mcpResources'), t('at.category.mcpResources.description'), (count) => t('mcp.resourceCount', { count })),
        ].filter((provider) => isBuiltinProviderEnabled(provider.id, builtinProviders));
        return [
            ...builtinAtProviders,
    ...
    const providerViews = useMemo(() => allProviders.map((provider) => ({
        id: provider.id,
        provider,
        textValue: getProviderTextValue(provider),
        label: typeof provider.label === 'string'
    ...
    const setMenu = useCallback((next) => {
        ...
        if (shallowEqualMenuState(stateRef.current, resolved))
            return;
    ...
    const close = useCallback((options = {}) => {
        ...
        clearPendingLoad();
        builtinCacheRef.current = createBuiltinProviderCache();
        const preserveSelection = options.preserveProviderSelection ||
            preserveProviderSelectionRef.current;
    ...
    const getPreviousProviderItems = useCallback((providerId, query, baseState) => {
        const current = stateRef.current;
        if (current?.level !== 'items' ||
    ...
    const hasCachedProviderData = useCallback((providerId, query) => {
        ...
        if (providerId === FILE_PROVIDER_ID) {
            ...
            if (actions?.listDirectory) {
                const { dirPath } = splitFileQuery(query, normalizeDirectoryPath(fileDirectoryRef.current));
                return cache.directories.has(dirPath);
            }
            const pattern = fileSearchGlobPattern(query);
            return (Boolean(actions?.globWorkspace) && cache.globResults.has(pattern));
        }
        if (providerId === EXTENSIONS_PROVIDER_ID) {
    ...
    const loadItems = useCallback((providerId, query, baseState, options = {}) => {
        const provider = allProvidersRef.current.find((item) => item.id === providerId);
        if (!provider) {
            console.warn(`[@mention] provider="${providerId}" not found (may have been removed)`);
            setMenu(null);
            return;
        }
        abortRef.current?.abort();
        ...
        const previousItems = getPreviousProviderItems(providerId, query, baseState);
        if (!options.loadingAlreadySet) {
            setMenu({ ...baseState, items: previousItems, loading: true });
        }
        Promise.resolve()
            .then(() => provider.search({
            query,
            signal: abort.signal,
            ...
            setMenu((prev) => {
                ...
                const maxItems = providerId === FILE_PROVIDER_ID
                    ? FILE_ROOT_ITEM_LIMIT
                    : ITEM_LIMIT;
                return {
                    ...prev,
                    items: items.slice(0, maxItems).map((item) => sanitizeAtMentionItem(item, {
            ...
            console.warn(`[@mention] provider="${providerId}" query=<redacted> failed`, error);
            setMenu((prev) => {
                if (!prev || prev.level !== 'items')
                    return prev;
                return {
    ...
    const scheduleLoadItems = useCallback((providerId, query, baseState) => {
        if (searchTimerRef.current) {
            clearTimeout(searchTimerRef.current);
        }
        abortRef.current?.abort();
    ...
    const loadMcpResourceItems = useCallback((serverName, query, baseState, options = {}) => {
        const actions = workspaceActionsRef.current;
        const loadMcpResources = actions?.loadMcpResources;
        if (!loadMcpResources) {
            console.warn(`[@mention] loadMcpResources not available for server="${serverName}"`);
        ...
        const requestId = requestIdRef.current + 1;
        requestIdRef.current = requestId;
        const previousItems = stateRef.current?.level === 'items' &&
        ...
        const enabledPromise = options.validateServer
            ? isEnabledMcpServer(actions, serverName, abort.signal, loadMcpStatus
                ? () => {
                    const cache = builtinCacheRef.current;
                    cache.mcpStatus ??= loadMcpStatus().catch((error) => {
                        cache.mcpStatus = undefined;
        ...
        enabledPromise
            ...
            return getCached(builtinCacheRef.current.mcpResources, serverName, () => loadMcpResources(serverName, { signal: abort.signal }));
        })
            .then((status) => {
            if (abort.signal.aborted || requestIdRef.current !== requestId) {
                return;
            ...
            const items = status.resources
                .map((resource) => {
                const label = sanitizeDisplayText(resource.title ?? '') ??
                ...
                .filter((resource) => {
                return matchesQuery(query, resource.label, resource.description, resource.detail);
            })
                .sort((a, b) => a.label.localeCompare(b.label))
                .slice(0, ITEM_LIMIT);
            setMenu((prev) => {
    ...
    const scheduleLoadMcpResourceItems = useCallback((serverName, query, baseState, options = {}) => {
        if (searchTimerRef.current) {
            clearTimeout(searchTimerRef.current);
        }
        abortRef.current?.abort();
    ...
    const refreshForView = useCallback((view) => {
        ...
        const current = stateRef.current;
        const keepItemsLevel = current?.level === 'items' &&
            ...
            providerViewsRef.current.some((provider) => provider.id === current.selectedProviderId);
        if (keepItemsLevel) {
            if (!current.selectedProviderId)
                return true;
            const providerId = current.selectedProviderId;
            const query = current.inputMode === 'context'
                ? getProviderQueryFromMention(providerId, parsed.query, current.mcpServerName)
                : current.query;
            if (query !== current.query) {
                ...
                if (providerId === MCP_RESOURCES_PROVIDER_ID &&
                    ...
                    scheduleLoadMcpResourceItems(current.mcpServerName, query, nextState, { validateServer: current.validateMcpServer });
                    return true;
                }
        ...
        const filteredProviders = providerViewsRef.current
            .filter((provider) => {
            return matchesQuery(parsed.query, provider.textValue, provider.description);
        })
            .slice(0, ITEM_LIMIT);
        ...
        if (filteredProviders.length === 0 && parsed.query) {
            const insertedReference = splitInsertedReferenceQuery(parsed.query, lastSelectedProviderIdRef.current, lastSelectedMcpServerNameRef.current);
            if (insertedReference &&
                providerViewsRef.current.some((provider) => provider.id === insertedReference.providerId)) {
                if (insertedReference.providerId === MCP_RESOURCES_PROVIDER_ID &&
                    insertedReference.serverName) {
                    lastSelectedMcpServerNameRef.current = insertedReference.serverName;
                    scheduleLoadMcpResourceItems(insertedReference.serverName, insertedReference.itemQuery, {
                        from: parsed.from,
                        to: parsed.to,
                ...
                scheduleLoadItems(insertedReference.providerId, insertedReference.itemQuery, {
                    from: parsed.from,
                    to: parsed.to,
                    query: insertedReference.itemQuery,
                    level: 'items',
                    ...
                    itemMode: 'default',
                    mcpServerName: undefined,
                    fileDirectory: insertedReference.providerId === FILE_PROVIDER_ID
                        ? fileDirectoryRef.current
                        : undefined,
                    inputMode: 'context',
                });
            ...
            const prefixedProvider = providerViewsRef.current.find((provider) => !isBuiltinProviderId(provider.id) &&
            ...
            if (providerViewsRef.current.some((provider) => provider.id === FILE_PROVIDER_ID)) {
                fileDirectoryRef.current = '.';
    ...
    const moveSelection = useCallback((direction) => {
        ...
        if (!current)
            return false;
        const total = current.level === 'categories'
            ? current.providers.length
            : current.items.length;
        if (total <= 0)
            return true;
        const nextIndex = nextSelectionIndex(current.selectedIndex, total, direction);
    ...
    const enterCategory = useCallback((index) => {
        const current = stateRef.current;
        if (!current || current.level !== 'categories')
            return false;
        ...
        scheduleLoadItems(provider.id, current.query, {
            ...
            tabs: provider.tabs,
            selectedTabId,
            itemMode: provider.id === MCP_RESOURCES_PROVIDER_ID ? 'mcpServers' : 'default',
            mcpServerName: undefined,
            fileDirectory: provider.id === FILE_PROVIDER_ID
                ? fileDirectoryRef.current
                : undefined,
            inputMode: 'search',
        });
    ...
    const updateSearch = useCallback((query) => {
        ...
        if (current.selectedProviderId === MCP_RESOURCES_PROVIDER_ID &&
            current.itemMode === 'mcpResources' &&
            current.mcpServerName) {
    ...
    const backToCategories = useCallback(() => {
        const current = stateRef.current;
        if (!current || current.level !== 'items')
            return false;
        if (current.selectedProviderId === MCP_RESOURCES_PROVIDER_ID &&
            current.itemMode === 'mcpResources') {
        ...
        if (current.selectedProviderId === FILE_PROVIDER_ID) {
            const currentDir = normalizeDirectoryPath(current.fileDirectory ?? fileDirectoryRef.current);
            if (currentDir !== '.') {
    ...
    const accept = useCallback((index) => {
        ...
        if (item.kind === 'upload') {
            ...
            const { dirPath } = splitFileQuery(current.query, fileDirectoryRef.current);
        ...
        const rawInsert = item.insertText ??
            `@${escapeAtReferenceText(sanitizeInsertText(item.label))} `;
        const insert = item.composerTag && !/\s$/.test(rawInsert)
            ? `${rawInsert} `
            : rawInsert;
        ...
        const currentMention = view.state.doc.sliceString(current.from, current.to);
        ...
        const tag = createComposerTagForItem(current.selectedProviderId, item, insert);
        const tagText = insert.trimEnd();
        const effects = tag && createInlineTagEffect
            ? [
...
//# sourceMappingURL=useAtMentionMenu.js.map