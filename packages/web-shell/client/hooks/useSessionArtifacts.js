...
import { useActions, useConnection, usePromptStatus, useDaemonSessionOwnerGuard, useWorkspaceEventSignals, } from '@qwen-code/webui/daemon-react-sdk';
...
export function useSessionArtifacts() {
    ...
    if (!ownerRef.current?.isCurrent())
        ownerRef.current = ownerGuard.capture();
    ...
    const supportsArtifacts = connection.capabilities?.features?.includes(SESSION_ARTIFACTS_FEATURE) ??
        false;
    const sessionId = connection.sessionId;
    const [artifacts, setArtifacts] = useState([]);
    const [loading, setLoading] = useState(false);
    const requestIdRef = useRef(0);
    const loadedOwnerRef = useRef(undefined);
    const loadingOwnerRef = useRef(undefined);
    ...
    const artifactById = useMemo(() => new Map((loadedOwnerRef.current === owner ? artifacts : []).map((artifact) => [
        artifact.id,
        artifact,
    ])), [artifacts, owner]);
