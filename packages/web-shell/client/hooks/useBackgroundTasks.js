...
import { useActions, useDaemonSessionOwnerGuard, } from '@qwen-code/webui/daemon-react-sdk';
...
function hasActiveTask(tasks) {
    return tasks.some((task) => task.status === 'running' || task.status === 'paused');
}
export function useBackgroundTasks(sessionId, taskActivityKey, connected, refreshTrigger = 0) {
    ...
    useEffect(() => {
        if (tasksPanelActive)
            return;
        if (!connected || !sessionId || !pollingActive)
            return;
        let disposed = false;
        const refresh = () => {
    ...
    useEffect(() => {
        const onTasksPanelActive = (event) => {
        ...
        return () => window.removeEventListener(TASKS_STATUS_ACTIVE_EVENT, onTasksPanelActive);
    }, []);
