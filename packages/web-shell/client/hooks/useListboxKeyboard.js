...
import { DialogShellIdContext, isTopDialogShellId, } from '../components/dialogs/DialogShell';
function clamp(index, itemCount) {
    if (itemCount <= 0)
        return 0;
    if (index < 0)
        return 0;
 ...
 * dismissal.
 */
export function useListboxKeyboard({ itemCount, activeIndex, onActiveIndexChange, onConfirm, enabled = true, }) {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (event) => {
            ...
            if (!isTopDialogShellId(dialogShellId))
                return;
            ...
            switch (event.key) {
                ...
                case ' ': {
                    ...
                    if (focusOwnsEnter() || focusOwnsHomeEnd())
                        return;
                    if (active < 0)
                        return;
                    event.preventDefault();
...
//# sourceMappingURL=useListboxKeyboard.js.map