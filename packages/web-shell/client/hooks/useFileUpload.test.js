...
function Probe({ client, maxBytes, targetKey }) {
    latest = useFileUpload({ client, maxBytes, targetKey });
    probeRenderCount += 1;
    return null;
}
function render(props) {
    act(() => root.render(_jsx(Probe, { ...props })));
}
function makeFile(name, size = 3) {
...
describe('useFileUpload', () => {
    ...
    it('rejects an oversized file locally without an HTTP request', async () => {
        ...
        const client = { uploadWorkspaceFile };
        render({ client, maxBytes: MAX, targetKey: 'ws:/a' });
        const onUploaded = vi.fn();
        act(() => {
    ...
    it('continues the batch after a local oversize rejection', async () => {
        const uploadWorkspaceFile = vi.fn(async (req) => ({
            kind: 'file_upload',
            path: req.path,
            sizeBytes: 3,
            hash: `sha256:${'b'.repeat(64)}`,
        ...
        render({ client, maxBytes: MAX, targetKey: 'ws:/a' });
        act(() => {
            latest.uploadFiles([makeFile('big.bin', MAX + 1), makeFile('small.txt')], '.');
        });
        await act(async () => { });
        expect(uploadWorkspaceFile).toHaveBeenCalledTimes(1);
    ...
    it('keeps a successful upload done when onUploaded throws', async () => {
        ...
        expect(consoleError).toHaveBeenCalledWith('Failed to handle uploaded file', expect.objectContaining({ message: 'insert failed' }));
    ...
    it('a failed upload does not block the next item', async () => {
        ...
        const onUploaded = vi.fn();
        act(() => {
            latest.uploadFiles([makeFile('bad.txt'), makeFile('good.txt')], '.', onUploaded);
        });
        await act(async () => { });
        expect(latest.uploads[0]).toMatchObject({
            status: 'error',
    ...
    it('invalidates the generation before the new client can serve the old queue', async () => {
        ...
        act(() => {
            latest.uploadFiles([makeFile('one.txt'), makeFile('two.txt')], '.', onUploaded);
        });
        ...
        try {
            ...
            root.render(_jsx(Probe, { client: clientB, maxBytes: MAX, targetKey: "ws:/b" }));
            ...
            for (let tasks = 0; probeRenderCount <= rendersBefore && tasks < 1000; tasks += 1) {
                await nextMacrotask();
            }
    ...
    it('never uploads a queued item after unmount', async () => {
        ...
        render({ client, maxBytes: MAX, targetKey: 'ws:/a' });
        act(() => {
            latest.uploadFiles([makeFile('a.txt'), makeFile('b.txt')], '.', onUploaded);
        });
    ...
    it('drains a file queued while a stale-generation loop winds down', async () => {
        ...
        await act(async () => {
            gates[0].reject(new DOMException('This operation was aborted', 'AbortError'));
        });
    ...
    it('is not busy when every item failed locally', () => {
        const client = { uploadWorkspaceFile: vi.fn() };
        render({ client, maxBytes: MAX, targetKey: 'ws:/a' });
        act(() => {
...
//# sourceMappingURL=useFileUpload.test.js.map