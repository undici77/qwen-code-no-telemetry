...
import { getBackgroundAgentNotificationKey, getPendingBackgroundAgentKey, reconcileBackgroundAgentResolutions, transcriptBlocksToLocalizedMessages, useMessages, } from './useMessages';
...
function backgroundAgentMessage(status = 'pending') {
    return {
        id: 'agent-block',
        role: 'tool_group',
        tools: [
...
function backgroundAgentResolution(status) {
    return {
        status,
        durationMs: 20,
    };
}
function backgroundAgentBlock(toolCallId) {
    return baseBlock({
...
describe('background agent task reconciliation', () => {
    it('uses terminal agent notifications as a reconciliation trigger without requiring toolUseId', () => {
        expect(getBackgroundAgentNotificationKey([
            baseBlock({
                id: 'notification',
                kind: 'assistant',
                text: '',
    ...
    it('only requests reconciliation for an active background agent', () => {
        expect(getPendingBackgroundAgentKey([backgroundAgentMessage()])).toBe('agent-call');
        expect(getPendingBackgroundAgentKey([backgroundAgentMessage('completed')])).toBe('');
    });
    it.each([
        ['completed', 'completed', undefined],
        ['failed', 'failed', undefined],
        ['cancelled', 'completed', 'cancelled'],
    ])('restores a %s background agent from the task snapshot', (taskStatus, expectedStatus, expectedRawStatus) => {
        const [message] = reconcileBackgroundAgentResolutions([backgroundAgentMessage()], new Map([['agent-call', backgroundAgentResolution(taskStatus)]]));
    ...
    it('does not complete the card from a running task snapshot', () => {
        expect(reconcileBackgroundAgentResolutions([backgroundAgentMessage()], new Map([['agent-call', backgroundAgentResolution('running')]]))).toMatchObject([{ role: 'tool_group', tools: [{ status: 'pending' }] }]);
    });
    it('queries once for a pending card, ignores streaming, and retries after reconnect', async () => {
        ...
        hookState.resolveSubagentSession.mockResolvedValue(backgroundAgentResolution('completed'));
        const container = document.createElement('div');
        const root = createRoot(container);
        const t = (key) => key;
        function Consumer() {
            const messages = useMessages(t);
            const status = messages[0]?.role === 'tool_group'
                ? messages[0].tools[0]?.status
                : undefined;
            return createElement('div', null, status);
        }
        const renderConsumer = () => root.render(createElement(StrictMode, null, createElement(Consumer)));
        await act(async () => renderConsumer());
        await vi.waitFor(() => {
            expect(hookState.resolveSubagentSession).toHaveBeenCalledTimes(1);
            expect(hookState.resolveSubagentSession).toHaveBeenCalledWith('session-1', 'agent-call');
        ...
        await act(async () => renderConsumer());
        await vi.waitFor(() => expect(hookState.resolveSubagentSession).toHaveBeenCalledTimes(2));
    ...
    it('ignores an older response after the pending Agent set expands', async () => {
        ...
        const t = (key) => key;
        function Consumer() {
            const statuses = useMessages(t).flatMap((message) => message.role === 'tool_group'
    ...
    it('applies successful resolutions when another pending Agent fails', async () => {
        ...
        hookState.resolveSubagentSession.mockReset();
        hookState.resolveSubagentSession.mockImplementation((_sessionId, callId) => callId === 'agent-a'
        ...
        hookState.resolveSubagentSession.mockImplementation((_sessionId, callId) => callId === 'agent-b'
            ? Promise.resolve(backgroundAgentResolution('completed'))
            : Promise.reject(new Error('not found')));
        hookState.blocks = [
            ...hookState.blocks,
    ...
    it('ignores an older response after switching sessions', async () => {
        ...
        await act(async () => renderConsumer());
        expect(hookState.resolveSubagentSession).toHaveBeenCalledTimes(2);
        await act(async () => sessionB.resolve(backgroundAgentResolution('completed')));
        expect(container.textContent).toBe('completed');
        await act(async () => sessionA.resolve(backgroundAgentResolution('running')));
        expect(container.textContent).toBe('completed');
        await act(async () => root.unmount());
        hookState.connection.sessionId = 'session-1';
    });
