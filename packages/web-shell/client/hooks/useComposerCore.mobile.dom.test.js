...
function Harness({ composerInput, onSubmit, onInputTextChange, sessionId, atWorkspaceCwd, }) {
    ...
    return (_jsx("div", { ...composer.imageTransferHandlers, "data-web-shell-composer-surface": true, children: composer.mobileComposer ? (_jsx("textarea", { ref: composer.mobileComposer.textareaRef, value: composer.mobileComposer.value, onChange: composer.mobileComposer.onChange, onBlur: composer.mobileComposer.onBlur, placeholder: composer.mobileComposer.placeholder, "data-web-shell-composer-editor": true })) : (_jsx("div", { ref: composer.containerRef, "data-web-shell-composer-editor": true })) }));
}
async function mount({ composerInput, onSubmit = vi.fn(), onInputTextChange, sessionId, atWorkspaceCwd, } = {}) {
    container = document.createElement('div');
    document.body.append(container);
    root = createRoot(container);
    await act(async () => {
        root.render(_jsx(WebShellPortalRootContext.Provider, { value: null, children: _jsx(I18nProvider, { language: "en", children: _jsx(Harness, { composerInput: composerInput, onSubmit: onSubmit, onInputTextChange: onInputTextChange, sessionId: sessionId, atWorkspaceCwd: atWorkspaceCwd }) }) }));
    });
    return { onSubmit };
}
...
afterEach(() => {
    ...
    for (let index = localStorage.length - 1; index >= 0; index--) {
        ...
        if (key?.startsWith('qwen-web-shell-session-draft:') ||
...
describe('useComposerCore mobile textarea backend', () => {
    ...
    it('falls back to the top placement for inline tags', async () => {
        mockTouchDevice();
        await mount();
        act(() => latest.addTags([{ id: 'file', value: 'a.ts' }], { placement: 'inline' }));
    ...
    it('collects pasted images and lets plain text paste natively', async () => {
        ...
        const imageItem = {
            kind: 'file',
            type: 'image/png',
            getAsFile: () => new File([new Uint8Array([137, 80, 78, 71])], 'x.png', {
                type: 'image/png',
            }),
        };
    ...
    it('saves the draft immediately on blur before the debounce timer fires', async () => {
        ...
        expect(localStorage.getItem('qwen-web-shell-session-draft:' + encodeURIComponent('mobile-session'))).toBeNull();
        act(() => {
            container
                .querySelector('textarea')
                .dispatchEvent(new FocusEvent('focusout', { bubbles: true }));
        });
        expect(localStorage.getItem('qwen-web-shell-session-draft:' + encodeURIComponent('mobile-session'))).toBe('mobile draft text');
        vi.useRealTimers();
    });
});
//# sourceMappingURL=useComposerCore.mobile.dom.test.js.map