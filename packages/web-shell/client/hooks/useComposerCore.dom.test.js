...
let root = null;
let latest = null;
function Harness({ composerInput, onSubmit, renderComposerTag, renderComposerTagTooltip, parseUserMessageContent, followupState, sessionId, atWorkspaceCwd, commands, onImageIngestionNotice, workspaceUploadBusy, }) {
    ...
    latest = composer;
    return (_jsx("div", { "data-web-shell-composer-surface": true, ...composer.imageTransferHandlers, children: _jsx("div", { ref: composer.containerRef }) }));
}
async function mount({ composerInput, onSubmit = vi.fn(), renderComposerTag, renderComposerTagTooltip, parseUserMessageContent, followupState, sessionId, atWorkspaceCwd, commands, onImageIngestionNotice, workspaceUploadBusy, } = {}) {
    container = document.createElement('div');
    document.body.append(container);
    root = createRoot(container);
    let currentPortalRoot = null;
    let currentSessionId = sessionId;
    let currentWorkspaceCwd = atWorkspaceCwd;
    const render = () => {
        root.render(_jsx(WebShellPortalRootContext.Provider, { value: currentPortalRoot, children: _jsx(I18nProvider, { language: "en", children: _jsx(Harness, { composerInput: composerInput, onSubmit: onSubmit, renderComposerTag: renderComposerTag, renderComposerTagTooltip: renderComposerTagTooltip, parseUserMessageContent: parseUserMessageContent, followupState: followupState, sessionId: currentSessionId, atWorkspaceCwd: currentWorkspaceCwd, commands: commands, onImageIngestionNotice: onImageIngestionNotice, workspaceUploadBusy: workspaceUploadBusy }) }) }));
    };
    await act(async () => {
        render();
    });
    return {
        ...
        switchSession(nextSessionId, nextWorkspaceCwd) {
...
afterEach(() => {
    ...
    for (let index = localStorage.length - 1; index >= 0; index--) {
        ...
        if (key === 'qwen-web-shell-command-history' ||
...
function getSessionDraftKey(sessionId) {
    return `qwen-web-shell-session-draft:${encodeURIComponent(sessionId)}`;
}
function getPendingTaskDraftKey(workspaceCwd) {
...
describe('useComposerCore tooltip portal', () => {
    it('moves the CodeMirror tooltip portal and styles into the shared shadow root', async () => {
        ...
        const tooltipPortal = document.querySelector('[data-web-shell-tooltip-portal]');
        ...
        expect(shadowRoot.getElementById('web-shell-tooltip-styles')).not.toBeNull();
        host.remove();
    });
});
describe('useComposerCore history and drafts', () => {
    it('does not serialize the whole document again for a slash menu refresh', async () => {
        ...
        const textPrototype = Object.getPrototypeOf(Object.getPrototypeOf(doc));
        const toString = vi.spyOn(textPrototype, 'toString');
        act(() => latest.setText(`${Array.from({ length: 10_000 }, () => 'draft').join('\n')}\n/`));
    ...
    it('blocks history retry and search-match submit while an upload is busy', async () => {
        ...
        localStorage.setItem(getPromptHistoryStorageKey(workspaceCwd), JSON.stringify(['previous prompt']));
        const onSubmit = vi.fn();
        await mount({
            onSubmit,
    ...
    it('falls back to legacy prompt history until the workspace has its own history', async () => {
        localStorage.setItem(getPromptHistoryStorageKey(), JSON.stringify(['legacy prompt']));
        await mount({
            sessionId: 'session-a',
            atWorkspaceCwd: '/workspace/a',
        });
        ...
        expect(JSON.parse(localStorage.getItem(getPromptHistoryStorageKey('/workspace/a')) ??
    ...
    it('isolates prompt history by workspace and reloads it when the workspace changes', async () => {
        ...
        expect(JSON.parse(localStorage.getItem(getPromptHistoryStorageKey('/workspace/b')) ??
    ...
    it('restores an unsent new-task draft after leaving and returning', async () => {
        ...
        expect(localStorage.getItem(getPendingTaskDraftKey(workspaceCwd))).toBeNull();
        act(() => vi.advanceTimersByTime(1999));
        expect(localStorage.getItem(getPendingTaskDraftKey(workspaceCwd))).toBeNull();
        act(() => vi.advanceTimersByTime(1));
        expect(localStorage.getItem(getPendingTaskDraftKey(workspaceCwd))).toBe('new task draft still typing');
        act(() => latest.setText('draft flushed on switch'));
        mounted.switchSession('session-a', workspaceCwd);
        expect(latest.getText()).toBe('');
        expect(localStorage.getItem(getPendingTaskDraftKey(workspaceCwd))).toBe('draft flushed on switch');
        ...
        act(() => root?.unmount());
        expect(localStorage.getItem(getPendingTaskDraftKey(workspaceCwd))).toBe('draft flushed on unmount');
        container?.remove();
        root = null;
        container = null;
        latest = null;
    ...
    it('keeps an unscoped new-task draft in memory until its workspace resolves', async () => {
        localStorage.setItem('qwen-web-shell-pending-task-draft:', 'draft from another workspace');
        const mounted = await mount();
        expect(latest.getText()).toBe('');
        ...
        expect(localStorage.getItem(getPendingTaskDraftKey('/workspace/current'))).toBe('current pending draft');
        expect(localStorage.getItem('qwen-web-shell-pending-task-draft:')).toBe('draft from another workspace');
    });
    it('keeps newer unscoped input when the resolved workspace has an older draft', async () => {
        const workspaceCwd = '/workspace/current';
        localStorage.setItem(getPendingTaskDraftKey(workspaceCwd), 'older persisted draft');
        const mounted = await mount();
        act(() => latest.setText('newer in-memory draft'));
        mounted.switchSession(undefined, workspaceCwd);
        expect(latest.getText()).toBe('newer in-memory draft');
        expect(localStorage.getItem(getPendingTaskDraftKey(workspaceCwd))).toBe('newer in-memory draft');
    });
    it('restores an older workspace draft after a programmatic unscoped reset', async () => {
        const workspaceCwd = '/workspace/current';
        localStorage.setItem(getPendingTaskDraftKey(workspaceCwd), 'persisted workspace draft');
        const mounted = await mount({
            sessionId: 'session-a',
            atWorkspaceCwd: '/workspace/a',
        });
    ...
    it('commits a new-task prompt after its session is allocated', async () => {
        let commitPrompt;
        const onSubmit = vi.fn((_text, _images, commitAccepted) => {
        ...
        expect(latest.getText()).toBe('');
        act(() => commitPrompt?.());
        expect(JSON.parse(localStorage.getItem(getPromptHistoryStorageKey(workspaceCwd)) ?? '[]')).toEqual(['first prompt']);
        expect(localStorage.getItem(getPendingTaskDraftKey(workspaceCwd))).toBeNull();
    });
    it('commits a delayed queued prompt to history exactly once', async () => {
        let commitQueuedPrompt;
        const onSubmit = vi.fn((_text, _images, commitAccepted) => {
        ...
        act(() => blurEditor());
        expect(localStorage.getItem(getSessionDraftKey('queued-session'))).toBe('stale saved draft');
        act(() => {
            latest.setText('queued prompt');
            latest.submitText();
        });
        expect(latest.getText()).toBe('queued prompt');
        expect(localStorage.getItem(getPromptHistoryStorageKey('/workspace/queue'))).toBeNull();
        act(() => {
            commitQueuedPrompt?.();
            commitQueuedPrompt?.();
        });
        expect(latest.getText()).toBe('');
        expect(JSON.parse(localStorage.getItem(getPromptHistoryStorageKey('/workspace/queue')) ??
            '[]')).toEqual(['queued prompt']);
        expect(localStorage.getItem(getSessionDraftKey('queued-session'))).toBeNull();
    });
    it('does not clear newer input when a prompt is accepted in the same session', async () => {
        ...
        expect(localStorage.getItem(getSessionDraftKey('queued-session'))).toBe('newer draft');
    });
    ...
    it('does not clear the next session when a delayed prompt is accepted', async () => {
        ...
        expect(localStorage.getItem(getSessionDraftKey('session-b'))).toBeNull();
        act(() => blurEditor());
        expect(localStorage.getItem(getSessionDraftKey('session-b'))).toBe('draft b');
        expect(JSON.parse(localStorage.getItem(getPromptHistoryStorageKey('/workspace/a')) ??
            '[]')).toEqual(['queued in a']);
        expect(localStorage.getItem(getPromptHistoryStorageKey('/workspace/b'))).toBeNull();
    });
    ...
    it('flushes the draft to localStorage on visibilitychange and pagehide', async () => {
        ...
        expect(localStorage.getItem(getSessionDraftKey('vis-session'))).toBe('draft before hide');
        act(() => latest.setText('draft before pagehide'));
        act(() => {
            window.dispatchEvent(new PageTransitionEvent('pagehide'));
        });
        expect(localStorage.getItem(getSessionDraftKey('vis-session'))).toBe('draft before pagehide');
        Object.defineProperty(document, 'visibilityState', {
...
describe('useComposerCore paste', () => {
    ...
    it('claims image drops, blocks submit while reading, and submits image-only', async () => {
        ...
        act(() => latest.submitText());
        expect(onSubmit).toHaveBeenCalledWith('', [expect.objectContaining({ media_type: 'image/png' })], expect.any(Function), undefined);
    });
    it('keeps drag feedback across composer children and clears it globally', async () => {
        ...
        const dispatchDrag = (target, type, relatedTarget = null) => {
    ...
    it('keeps batch order, normalizes BMP, and aggregates rejected drops', async () => {
        ...
        expect(onImageIngestionNotice).toHaveBeenCalledWith('warning', expect.stringContaining('1'));
    });
...
describe('useComposerCore tags', () => {
    it('resubmits restored input annotations with the draft', async () => {
        ...
        expect(onSubmit).toHaveBeenCalledWith('@file.ts\n\nfix it', undefined, expect.any(Function), { inputAnnotations });
    });
    it('keeps earlier annotations when another restored prompt is prepended', async () => {
        ...
        expect(onSubmit).toHaveBeenCalledWith('@b new\n@a old', undefined, expect.any(Function), {
            inputAnnotations: [
    ...
    it('maps restored annotations through edits before their range', async () => {
        ...
        expect(onSubmit).toHaveBeenCalledWith('please @file.ts\n\nfix it', undefined, expect.any(Function), {
            inputAnnotations: [
    ...
    it('drops a restored annotation when its range is edited', async () => {
        ...
        expect(onSubmit).toHaveBeenCalledWith('@Xile.ts\n\nfix it', undefined, expect.any(Function), undefined);
    });
    ...
    it('falls back when inline custom tag rendering throws', async () => {
        ...
        expect(warn).toHaveBeenCalledWith('[WebShell] inline tag renderContent failed', error);
    ...
    it('falls back when inline custom tag tooltip rendering throws', async () => {
        ...
        expect(warn).toHaveBeenCalledWith('[WebShell] inline tag tooltip render failed', error);
        expect(document.body.textContent).toContain('orders');
        warn.mockRestore();
    });
    it('uses a custom inline tooltip without a native title', async () => {
        ...
        expect(tooltip?.parentElement?.getAttribute('aria-describedby')).toBe(tooltip?.id);
    });
    it('falls back to a native title when attaching an inline tooltip fails', async () => {
        const error = new Error('append failed');
        const appendChild = HTMLElement.prototype.appendChild;
        let failingTooltip = null;
        let readFailingChipTitle = null;
        let dispatchOnFailingChip = null;
    ...
    it('guards inline mask icon sources', async () => {
        ...
        expect(document.body.querySelector('[style*="--composer-tag-icon-url"]')).toBeNull();
    });
    it('renders built-in icons for inline composer tags', async () => {
        ...
        expect(document.body.querySelectorAll('[style*="--composer-tag-icon-url"]')).toHaveLength(kinds.length);
    });
    it('reports inline composer tags as attachments', async () => {
        ...
        act(() => {
            latest.addTags([{ id: 'orders', value: 'orders', serialized: '@orders' }], { placement: 'inline' });
        });
    ...
    it('preserves surrounding text when an inline tag chip is removed', async () => {
        ...
        expect(latest.hasAttachments).toBe(true);
        const removeButton = document.body.querySelector('button[aria-label="Remove orders"]');
    ...
    it('appends end-placed inline tags without stealing focus', async () => {
        ...
        act(() => {
            latest.addTags([{ id: 'orders', value: 'orders', serialized: '@orders' }], { placement: 'inline', position: 'end' });
        });
    ...
    it('keeps inline tags after trimming leading whitespace on submit', async () => {
        ...
        act(() => {
            latest.setText('  ');
            latest.addTags([{ id: 'orders', value: 'orders', serialized: '<table />' }], { placement: 'inline' });
            latest.insertText('explain');
            latest.submitText();
        });
        expect(onSubmit).toHaveBeenCalledWith('<table /> explain', undefined, expect.any(Function), {
            inputAnnotations: [
                {
                    end: 9,
                    reference: {
    ...
    it('restores parsed inline tags when arrow keys browse prompt history', async () => {
        ...
        act(() => latest.submitText());
        expect(onSubmit).toHaveBeenLastCalledWith(prompt, undefined, expect.any(Function), {
            inputAnnotations: [
                expect.objectContaining({
                    start: 8,
    ...
    it('restores shell history as raw command text without parsing tags', async () => {
        ...
        const parseUserMessageContent = vi.fn((content) => content === command
            ? [
                { type: 'text', text: 'echo ' },
                {
                    type: 'tag',
        ...
        expect(editor.querySelectorAll('button[aria-label^="Remove "]')).toHaveLength(0);
    ...
    it('restores tags when Tab or a search-result selection recalls history', async () => {
        ...
        const prompt = `inspect ${serialized} now`;
        const parseUserMessageContent = (content) => content === prompt
            ? [
                { type: 'text', text: 'inspect ' },
                {
                    type: 'tag',
    ...
    it('submits the selected plain Ctrl-R history text instead of a visible followup', async () => {
        ...
        localStorage.setItem('qwen-web-shell-history', JSON.stringify([historyText]));
        const { onSubmit } = await mount({
            followupState: {
                isVisible: true,
                shownAt: Date.now(),
        ...
        expect(onSubmit).toHaveBeenLastCalledWith(historyText, undefined, expect.any(Function), undefined);
    });
    it('keeps history source text and skips annotations for mismatched parser output', async () => {
        ...
        act(() => latest.submitText());
        expect(onSubmit).toHaveBeenLastCalledWith(prompt, undefined, expect.any(Function), undefined);
    });
...
//# sourceMappingURL=useComposerCore.dom.test.js.map