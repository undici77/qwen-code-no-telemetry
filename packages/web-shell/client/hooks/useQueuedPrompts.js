...
export function useQueuedPrompts({
  ...
  const isCurrentOwnerTokenRef = useRef(
    (token) => ownerTokenRef.current === token && token.snapshot.isCurrent(),
  );
  ...
  const applyMidTurnSnapshot = useCallback(
    (snapshot, targetSessionId, applyPromoted) => {
  ...
  const enqueuePrompt = useCallback(
    (text, images, onComplete, inputAnnotations, onAdmitted) => {
  ...
  const restoreUnknownQueuedPrompt = useCallback(
    (id) => resolveUnknownQueuedPrompt(id, true),
    [resolveUnknownQueuedPrompt],
  );
  const discardUnknownQueuedPrompt = useCallback(
    (id) => resolveUnknownQueuedPrompt(id, false),
    [resolveUnknownQueuedPrompt],
  );
