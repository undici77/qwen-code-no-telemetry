...
import { useStreamingState, useTranscriptBlocks, } from '@qwen-code/webui/daemon-react-sdk';
/**
    ...
    useEffect(() => {
        ...
        const id = setInterval(() => {
            ...
            if (gap <= 0) {
                ...
                setMetrics((prev) => {
                    const next = {
                        estimatedOutputTokens: agentTokens + Math.round(displayRef.current / 4),
                        isReceivingContent: isReceiving,
                    };
                    if (prev.estimatedOutputTokens === next.estimatedOutputTokens &&
...
function getTaskExecutionTokenCount(rawOutput) {
...
//# sourceMappingURL=useStreamingLoadingMetrics.js.map