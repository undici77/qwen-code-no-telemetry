...
function Probe({ cwd }) {
    ...
    return (_jsxs("div", { children: [_jsx("span", { "data-testid": "sessions", children: sessions.map((session) => session.sessionId).join(',') }), _jsx("button", { "data-action": "delete", onClick: () => void deleteSessions(['secondary']), children: "delete" }), _jsx("button", { "data-action": "release", onClick: () => void releaseSession?.('secondary'), children: "release" })] }));
}
function render(cwd) {
