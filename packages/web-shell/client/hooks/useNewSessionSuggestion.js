...
function hasExplicitNewTaskCue(text) {
    const trimmed = text.trim();
...
export function useNewSessionSuggestion({ enabled, messages, sessionId, contextUsageRatio, isRunning, dialogOpen, hasAttachments, generateContent, }) {
    ...
    const scheduleInput = useCallback((inputText) => {
        clearPending();
        const { enabled: currentEnabled, recentMessages: currentRecentMessages, sessionId: currentSessionId, contextUsageRatio: currentContextUsageRatio, isRunning: currentIsRunning, dialogOpen: currentDialogOpen, hasAttachments: currentHasAttachments, generateContent: currentGenerateContent, } = optionsRef.current;
        ...
        const allowBtw = currentHasAttachments === false &&
            currentRecentMessages.length >= MIN_BTW_MESSAGE_COUNT;
        const allowNewSession = !isFollowupLike(trimmed) &&
            (explicitNewTaskCue
    ...
    const updateInput = useCallback((inputText) => {
...
//# sourceMappingURL=useNewSessionSuggestion.js.map