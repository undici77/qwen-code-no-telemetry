...
function Harness({ onKeyboardMode, ...props }) {
    const { keyboardMode } = useListboxKeyboard(props);
    onKeyboardMode?.(keyboardMode);
    return null;
}
function ScopedHarness({ name, onKeyboardMode, ...props }) {
    const { keyboardMode } = useListboxKeyboard(props);
    onKeyboardMode?.(keyboardMode);
    return (_jsx("div", { "data-scope": name, tabIndex: -1, children: name }));
}
let container = null;
let root = null;
function mount(props) {
    ...
    act(() => {
        root.render(_jsx(Harness, { ...props }));
    });
}
...
describe('useListboxKeyboard', () => {
    ...
    it('only the active scope handles keys when two scoped hooks are mounted', () => {
        ...
        mountNode(_jsx(I18nProvider, { language: "en", children: _jsxs(ThemeProvider, { value: "dark", children: [_jsx(DialogShell, { title: "Bottom", onClose: vi.fn(), children: _jsx(ScopedHarness, { name: "bottom", itemCount: 3, activeIndex: 0, onActiveIndexChange: onBottomMove, onConfirm: vi.fn() }) }), _jsx(DialogShell, { title: "Top", onClose: vi.fn(), children: _jsx(ScopedHarness, { name: "top", itemCount: 3, activeIndex: 0, onActiveIndexChange: onTopMove, onConfirm: vi.fn() }) })] }) }));
...
//# sourceMappingURL=useListboxKeyboard.test.js.map