...
function Harness() {
    latestTasks = useBackgroundTasks(sessionId, taskActivityKey, true, refreshTrigger);
    return null;
}
...
describe('useBackgroundTasks', () => {
    ...
    it('stops polling when the session is disconnected', async () => {
        sdkMock.actions.getTasks.mockRejectedValue(new Error('Get tasks failed: Daemon session is not connected'));
        await renderHarness();
        expect(sdkMock.actions.getTasks).toHaveBeenCalledTimes(1);
        await act(async () => {
    ...
    it('starts polling when an out-of-band task triggers a refresh', async () => {
        ...
        sdkMock.actions.getTasks.mockResolvedValue(snapshot('session-a', [runningFork]));
        await renderHarness();
    ...
    it('ignores an old response and starts polling the new session immediately', async () => {
        ...
        await act(async () => {
            sessionA.resolve(snapshot('session-a', [monitor('monitor-a', 'completed')]));
    ...
    it('ignores an old attachment response when the session id is unchanged', async () => {
        ...
        sdkMock.ownerVersion += 1;
        await act(async () => {
            request.resolve(snapshot('session-a', [monitor('stale-monitor', 'running')]));
    ...
    it('starts polling a replacement attachment while the old request hangs', async () => {
        ...
        expect(latestTasks).toEqual([runningMonitor]);
        await act(async () => {
            oldRequest.resolve(snapshot('session-a', [monitor('stale-monitor', 'completed')]));
            await oldRequest.promise;
        });
        expect(latestTasks).toEqual([runningMonitor]);
    });
});
//# sourceMappingURL=useBackgroundTasks.test.js.map