...
function mount(streamingState, sessionActions, canMutateMidTurn = true, connected = false, writeBlocked = false) {
    const editor = {
        getText: vi.fn(() => ''),
        setText: vi.fn(),
        focus: vi.fn(),
    ...
    function Harness({ state, activeSessionId, blocked, }) {
    ...
    const render = (state, nextSessionId = activeSessionId, replaceOwner = false, nextWriteBlocked = blocked) => {
        if (replaceOwner)
            sdk.ownerVersion += 1;
        activeSessionId = nextSessionId;
        blocked = nextWriteBlocked;
        act(() => root.render(_jsx(Harness, { state: state, activeSessionId: activeSessionId, blocked: blocked })));
    };
...
describe('useQueuedPrompts default mid-turn insertion', () => {
    it('restores an unaccepted mid-turn prompt when its owner is replaced', () => {
        ...
        vi.mocked(actions.enqueueMidTurnMessage).mockReturnValue(new Promise(() => undefined));
        ...
        expect(editor.setText).toHaveBeenCalledWith('belongs to the source attachment');
    ...
    it('queues and submits an image-only prompt without using mid-turn text insertion', () => {
        ...
        expect(actions.submitPrompt).toHaveBeenCalledWith('', expect.objectContaining({
            images,
    ...
    it('buffers an image-only started event until its exact response binds', async () => {
        ...
        expect(store.appendLocalUserMessage).toHaveBeenCalledWith('', [{ data: 'Ym1w', mimeType: 'image/bmp' }], undefined);
        expect(latest.queuedPrompts).toMatchObject([
            {
                text: '',
                images,
    ...
    it('keeps another prompt started buffer when one admission is unknown', async () => {
        ...
        expect(store.appendLocalUserMessage).toHaveBeenCalledOnce();
        expect(store.appendLocalUserMessage).toHaveBeenCalledWith('', [{ data: 'c2Vjb25k', mimeType: 'image/png' }], undefined);
    });
    it('binds an image prompt when its terminal event precedes admission response', async () => {
        ...
        expect(store.appendLocalUserMessage).toHaveBeenCalledOnce();
        expect(store.appendLocalUserMessage).toHaveBeenCalledWith('', [{ data: 'dGVybWluYWw=', mimeType: 'image/png' }], undefined);
    });
    ...
    it('ignores an old submit response after an S1 to S2 to S1 owner change', async () => {
        const { actions, pendingSubmit } = createActions();
        const { render, store } = mount('responding', actions);
        act(() => latest.enqueuePrompt('', [{ data: 'b2xk', media_type: 'image/png' }]));
    ...
    it('does not duplicate attachments when restored text is already present', async () => {
        ...
        act(() => latest.enqueuePrompt('describe', images, undefined, inputAnnotations));
        await act(async () => {
            pendingSubmit.reject(new DaemonHttpError(413, undefined, 'Too large'));
            await Promise.resolve();
        });
    ...
    it('restores an uncertain local payload only after an explicit action', async () => {
        ...
        expect(reportError).toHaveBeenCalledWith(expect.any(TypeError), t('queue.admissionUnknown'));
        act(() => {
    ...
    it('restores queued input annotations with the local payload', async () => {
        ...
        act(() => latest.enqueuePrompt('@file.ts\n\nfix it', undefined, undefined, inputAnnotations));
        await act(async () => {
        ...
        expect(editor.restoreInputAnnotations).toHaveBeenCalledWith(inputAnnotations);
    });
    ...
    it('keeps an accepted message queued until its injection event', async () => {
        ...
        await act(async () => { });
        expect(actions.enqueueMidTurnMessage).toHaveBeenCalledWith('补充信息', expect.objectContaining({ signal: expect.any(AbortSignal) }));
    ...
    it('does not resend when injection arrives before admission resolves', async () => {
        ...
        render('responding');
        expect(latest.queuedPrompts).toEqual([]);
        await act(async () => admission.resolve({ accepted: true, messageId: 'mid-early' }));
        render('idle');
        expect(latest.queuedPrompts).toEqual([]);
        expect(actions.submitPrompt).not.toHaveBeenCalled();
    });
    it('falls back to one ordinary submission when mid-turn admission fails', async () => {
        ...
        await act(async () => { });
        expect(actions.submitPrompt).toHaveBeenCalledTimes(1);
        expect(actions.submitPrompt).toHaveBeenCalledWith('下一步', expect.objectContaining({ optimisticUserMessage: false }));
        expect(latest.queuedPrompts).toMatchObject([
    ...
    it('ignores a late admission result after idle fallback claimed the prompt', async () => {
        ...
        render('idle');
        render('responding');
        await act(async () => admission.resolve({ accepted: true, messageId: 'mid-late' }));
        expect(actions.submitPrompt).toHaveBeenCalledTimes(1);
        expect(latest.queuedPrompts).toMatchObject([
            {
                text: '不要重复',
    ...
    it('keeps idle, image, and command submissions on the ordinary path', () => {
        ...
        act(() => latest.enqueuePrompt('@file.ts fix', undefined, undefined, [
            {
                type: 'reference',
                start: 0,
                end: 7,
    ...
    it('clears a submitting row after the server confirms abort cleanup', async () => {
        const { actions, pendingSubmit } = createActions();
        mount('responding', actions);
        act(() => latest.enqueuePrompt('clear me', [
...
//# sourceMappingURL=useQueuedPrompts.dom.test.js.map