...
function makeViewAt(doc, anchor) {
    return {
...
function setViewState(view, doc, anchor = doc.length) {
    Object.defineProperty(view, 'state', {
        value: EditorState.create({ doc, selection: { anchor } }),
        configurable: true,
    });
}
function Harness({ actions, disabled = false, builtinProviders, providers, shellMode = false, view, createInlineTagEffect, onUploadRequest, }) {
...
function mount({ actions, disabled, builtinProviders, providers, shellMode, view, createInlineTagEffect, onUploadRequest, } = {}) {
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);
    act(() => {
        root.render(_jsx(Harness, { actions: actions, disabled: disabled, builtinProviders: builtinProviders, providers: providers, shellMode: shellMode, view: view, createInlineTagEffect: createInlineTagEffect, onUploadRequest: onUploadRequest }));
    });
}
...
describe('useAtMentionMenu', () => {
    ...
    it('restores the removed mention through the restore callback', async () => {
        ...
        let restoreQuery;
        const onUploadRequest = vi.fn((_targetDir, restore) => {
    ...
    it('hides the upload item while filtering files with an entry query', async () => {
        ...
        await runDebounce();
        expect(latest.state?.items.some((item) => item.id === 'upload-file')).toBe(false);
        expect(latest.state?.items.map((item) => item.label)).toContain('file-0.ts');
    });
    ...
    it('keeps the typed query when a stale upload item outlives the handler', async () => {
        ...
        act(() => {
            root.render(_jsx(Harness, { view: view, builtinProviders: ['files'], onUploadRequest: undefined, actions: actions }));
        });
        act(() => expect(latest.accept(0)).toBe(true));
        expect(view.dispatch).not.toHaveBeenCalled();
        expect(onUploadRequest).not.toHaveBeenCalled();
    ...
    it('rejects custom providers that reuse built-in ids', () => {
        ...
        expect(error).toHaveBeenCalledWith('[@mention] duplicate provider id="extensions" ignored');
        error.mockRestore();
    });
    it('rejects custom providers that reuse disabled built-in ids', () => {
        ...
        act(() => latest.refreshForView(makeView('@')));
        expect(latest.state?.providers).toEqual([]);
        expect(error).toHaveBeenCalledWith('[@mention] duplicate provider id="files" ignored');
        error.mockRestore();
    });
    it('rejects duplicate custom provider ids', () => {
        ...
        expect(error).toHaveBeenCalledWith('[@mention] duplicate provider id="custom" ignored');
        error.mockRestore();
    });
    ...
    it('opens MCP resource items using the inserted ref suffix as search text', async () => {
        ...
        await runDebounce();
        act(() => latest.accept());
        act(() => latest.refreshForView(makeView('@docs:https://example.com/doc')));
        await runDebounce();
    ...
    it('searches matching files across the workspace', async () => {
        ...
        await runDebounce();
        expect(globWorkspace).toHaveBeenCalledWith('**/*[cC][oO][nN][fF][iI][gG]*', {
            maxResults: 50,
            signal: expect.any(AbortSignal),
        });
    ...
    ])('decorates accepted extension refs using the %s', async (_case, displayName, expectedValue) => {
        vi.useFakeTimers();
    ...
    it('shows an empty MCP resource list when resource loading is unavailable', async () => {
        ...
        await runDebounce();
        expect(warn).toHaveBeenCalledWith('[@mention] loadMcpResources not available for server="docs"');
        expect(latest.state).toMatchObject({
            level: 'items',
            selectedProviderId: 'mcp-resources',
            itemMode: 'mcpResources',
    ...
    it('escapes glob metacharacters in workspace file search', async () => {
        ...
        await runDebounce();
        expect(globWorkspace).toHaveBeenCalledWith('**/*[fF][oO][oO]\\*[bB][aA][rR]\\?*', {
            maxResults: 50,
            signal: expect.any(AbortSignal),
        });
    });
    it('normalizes escaped paths and literalizes extglob operators', async () => {
        ...
        await runDebounce();
        expect(globWorkspace).toHaveBeenCalledWith('**/*[fF][oO][oO] [bB][aA][rR]\\+\\([tT][eE][sS][tT]\\)*', { maxResults: 50, signal: expect.any(AbortSignal) });
    });
    it('recovers from file provider list failures', async () => {
        ...
        expect(warn).toHaveBeenCalledWith('Failed to load @ file suggestions', expect.any(Error));
        warn.mockRestore();
    });
    ...
    it('selects enabled provider tabs and ignores disabled or same-tab choices', async () => {
        vi.useFakeTimers();
        const search = vi.fn(({ tabId }) => Promise.resolve([{ id: tabId ?? 'none', label: tabId ?? 'none' }]));
        mount({
            providers: [
                {
                    id: 'custom',
        ...
        act(() => latest.enterCategory(0));
        await runDebounce();
        expect(search).toHaveBeenLastCalledWith(expect.objectContaining({ query: '', tabId: 'open' }));
        expect(latest.state).toMatchObject({
            selectedTabId: 'open',
        ...
        expect(search).toHaveBeenLastCalledWith(expect.objectContaining({ query: '', tabId: 'all' }));
    ...
    it('unescapes parser delimiters when reopening a file mention', async () => {
        ...
        mount({ actions: { listDirectory } });
        act(() => latest.refreshForView(makeView('@space\\ name\\(1\\)\\#plus\\+.md')));
        await runDebounce();
        expect(latest.state).toMatchObject({
            level: 'items',
            selectedProviderId: 'files',
    ...
    it('inserts MCP resources with parser-safe escaping', async () => {
        ...
        const expectedInsert = '@docs\\:res\\://doc\\?version\\=1\\&tag\\=a\\+b\\ path\\@x. ';
        expect(view.dispatch).toHaveBeenCalledWith({
            changes: {
                from: 0,
                to: 1,
    ...
    it('recovers from MCP resource load failures', async () => {
        ...
        expect(warn).toHaveBeenCalledWith('Failed to load @ MCP resources', expect.any(Error));
        warn.mockRestore();
    });
    ...
    it('recovers from provider search failures', async () => {
        ...
        expect(warn).toHaveBeenCalledWith('[@mention] provider="broken" query=<redacted> failed', expect.any(Error));
        warn.mockRestore();
    });
    it('recovers from built-in provider search failures', async () => {
        ...
        expect(warn).toHaveBeenCalledWith('Failed to load @ extension suggestions', expect.any(Error));
        warn.mockRestore();
    });
...
//# sourceMappingURL=useAtMentionMenu.test.js.map