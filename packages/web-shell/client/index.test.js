...
vi.mock('@qwen-code/webui/daemon-react-sdk', async () => {
    ...
    return {
        DaemonWorkspaceProvider: ({ children }) => {
            if (workspaceShouldThrow)
...
vi.mock('./components/WebShellTranscript', async () => {
    const React = await import('react');
    return {
        WebShellTranscript: () => React.createElement('div', { 'data-testid': 'transcript-ok' }),
    };
});
...
describe('WebShellWithProviders top-level boundary', () => {
    it('exports the readonly transcript entry', () => {
        const container = render(_jsx(WebShellTranscript, { blocks: [] }));
        expect(container.querySelector('[data-testid="transcript-ok"]')).not.toBeNull();
    });
    it('renders normally when the providers are healthy', () => {
        const container = render(_jsx(WebShellWithProviders, {}));
        expect(container.querySelector('[data-testid="app-ok"]')).not.toBeNull();
        expect(container.querySelector('[role="alert"]')).toBeNull();
    });
    it('starts on an empty session by default', () => {
        render(_jsx(WebShellWithProviders, {}));
    ...
    it('passes controlled sessionId to the daemon session provider', () => {
        render(_jsx(WebShellWithProviders, { sessionId: "session-2" }));
        expect(sessionProviderProps[0]).toMatchObject({
            sessionId: 'session-2',
        });
    });
    it('passes explicit undefined sessionId to the daemon session provider', () => {
        render(_jsx(WebShellWithProviders, { sessionId: undefined }));
    ...
    it('enables prompt SSE restarts only when explicitly requested', () => {
        render(_jsx(WebShellWithProviders, { restartSseOnPrompt: true }));
        expect(sessionProviderProps[0]).toMatchObject({
    ...
    it('selects a registered workspace by path without locking the UI', () => {
        ...
        render(_jsx(WebShellWithProviders, { workspaceId: "primary", workspaceCwd: "/work/secondary" }));
    ...
    it('initializes an unlocked workspace selector from workspace id', () => {
        ...
        render(_jsx(WebShellWithProviders, { workspaceId: "secondary" }));
        expect(sessionProviderProps[0]).toMatchObject({
            workspaceCwd: '/work/secondary',
        });
    ...
    it('does not register an unknown unlocked workspace path', () => {
        const container = render(_jsx(WebShellWithProviders, { workspaceCwd: "/work/missing" }));
        expect(addWorkspace).not.toHaveBeenCalled();
    ...
    it('locks directly to an already registered workspace path', () => {
        ...
        render(_jsx(WebShellWithProviders, { lockWorkspaceCwd: "/work/secondary" }));
        expect(addWorkspace).not.toHaveBeenCalled();
        expect(sessionProviderProps[0]).toMatchObject({
            workspaceCwd: '/work/secondary',
        });
    ...
    it('locks to the primary workspace without registering it again', () => {
        render(_jsx(WebShellWithProviders, { lockWorkspaceCwd: "/workspace" }));
        expect(addWorkspace).not.toHaveBeenCalled();
        expect(sessionProviderProps[0]).toMatchObject({
            workspaceCwd: '/workspace',
        });
    ...
    it('selects the primary path without locking when workspaces are omitted', () => {
        workspaceCapabilities = { features: [], workspaceCwd: '/workspace' };
        render(_jsx(WebShellWithProviders, { workspaceCwd: "/workspace" }));
        expect(addWorkspace).not.toHaveBeenCalled();
        expect(sessionProviderProps[0]).toMatchObject({
            workspaceCwd: '/workspace',
        });
    ...
    it('registers a missing workspace persistently before rendering', async () => {
        ...
        const container = render(_jsx(WebShellWithProviders, { lockWorkspaceCwd: "/work/../work/secondary" }));
    ...
    it('ignores an older registration response after the locked path changes', async () => {
        let resolveA;
        let resolveB;
        addWorkspace.mockImplementation((cwd) => {
        ...
        const root = createRoot(container);
        mounted.push({ root, container });
        act(() => root.render(_jsx(WebShellWithProviders, { lockWorkspaceCwd: "/a" })));
        act(() => root.render(_jsx(WebShellWithProviders, { lockWorkspaceCwd: "/b" })));
        await act(async () => {
