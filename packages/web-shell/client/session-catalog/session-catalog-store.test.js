...
import { SESSION_CATALOG_RETENTION_MS, SessionCatalogStore, getSessionCatalogQueryKey, } from './session-catalog-store';
function deferred() {
    let resolve;
    let reject;
...
function page(sessionId, workspaceCwd = '/work') {
    return {
...
function query(workspaceCwd, routeKind = 'legacy') {
    return {
        routeKind,
        workspaceCwd,
...
describe('SessionCatalogStore', () => {
    ...
    it('keys every wire-affecting option and route kind', () => {
        ...
        expect(getSessionCatalogQueryKey(base)).toBe(getSessionCatalogQueryKey({ ...base, options: { ...base.options } }));
        expect(getSessionCatalogQueryKey(base)).not.toBe(getSessionCatalogQueryKey({ ...base, routeKind: 'qualified' }));
        expect(getSessionCatalogQueryKey(base)).not.toBe(getSessionCatalogQueryKey({ ...base, workspaceCwd: '/other' }));
        expect(getSessionCatalogQueryKey(base)).not.toBe(getSessionCatalogQueryKey({
            ...base,
    ...
    it('does not attach a command load to an already failed revision', async () => {
        ...
        const unsubscribe = store.subscribe(target, () => {
    ...
    it('discards an in-flight response invalidated by a fresh reload', async () => {
        ...
        expect(store.getSnapshot(target).page?.sessions[0]?.sessionId).toBe('fresh');
        unsubscribe();
    });
    ...
    it('patches only the owning workspace without changing page metadata', async () => {
        legacy.mockImplementation(async (cwd) => page(cwd, cwd));
        ...
        expect(store.getSnapshot(query('/b')).page?.sessions[0]?.displayName).toBe(undefined);
    });
    it('uses the qualified client only for qualified queries', async () => {
        ...
        expect(qualified).toHaveBeenCalledWith(expect.objectContaining({ archiveState: 'active' }));
        expect(legacy).not.toHaveBeenCalled();
        expect(store.getSnapshot(query('/work', 'qualified')).page?.sessions[0]
            ?.workspaceCwd).toBe('/work');
    });
...
//# sourceMappingURL=session-catalog-store.test.js.map