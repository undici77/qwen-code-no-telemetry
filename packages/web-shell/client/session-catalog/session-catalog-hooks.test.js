...
const { useSessionCatalogQuery, useWebShellSessions } = await import('./session-catalog-hooks');
let root;
let container;
let legacy;
beforeEach(() => {
...
function QueryProbe({ id }) {
    ...
    const result = useSessionCatalogQuery(mocks.workspace.client, query, { autoLoad: true });
    return _jsx("span", { "data-testid": id, children: result.sessions[0]?.sessionId });
}
describe('session catalog hooks', () => {
    it('shares one request across StrictMode consumers and remounts', async () => {
        ...
        act(() => {
            root.render(_jsxs(StrictMode, { children: [_jsx(QueryProbe, { id: "first" }), _jsx(QueryProbe, { id: "second" })] }));
        });
        ...
        root = createRoot(container);
        act(() => {
            root.render(_jsx(StrictMode, { children: _jsx(QueryProbe, { id: "remounted" }) }));
        });
        expect(legacy).toHaveBeenCalledTimes(1);
    ...
    it('provides the legacy Web Shell facade without dropping page metadata', async () => {
        ...
        function FacadeProbe() {
            ...
            return (_jsxs("span", { children: [result.sessions[0]?.sessionId, ":", result.nextCursor, ":", String(result.liveMergeFailed), ":", String(result.truncated)] }));
        }
        await act(async () => {
    ...
    it('runs mutations directly and performs only the Store resynchronization', async () => {
        ...
        expect(mocks.workspace.actions.deleteSession).toHaveBeenCalledWith('primary');
        expect(mocks.useSessions.mock.results.at(-1)?.value.deleteSession).not.toHaveBeenCalled();
