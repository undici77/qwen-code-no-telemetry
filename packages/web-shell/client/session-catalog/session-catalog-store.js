...
function hasAutoLoadSubscriber(entry) {
    return Array.from(entry.subscribers).some((subscriber) => subscriber.autoLoad);
}
...
export class SessionCatalogStore {
    ...
    subscribe(query, listener, options = {}) {
        ...
        const retainedPageExpired = subscriber.autoLoad &&
    ...
    async loadOnce(query, options = {}) {
        ...
        let request;
        const reusableRunningRequest = entry.runningRevision !== undefined && entry.snapshot.loading;
    ...
    invalidateWorkspace(workspaceCwd, options = {}) {
        ...
        for (const entry of this.entries.values()) {
            if (entry.query.workspaceCwd !== workspaceCwd)
                continue;
            ...
            if (entry.waiters.length > 0) {
                ...
                this.ensureScheduled(entry, background ? PRIORITY.initial : PRIORITY.interactive, background);
            }
        }
    }
    ...
    scheduleWorkspaceRefresh(workspaceCwd, delayMs = SESSION_CATALOG_TRAILING_REFRESH_MS) {
    ...
    ensureScheduled(entry, priority, background) {
        if (entry.runningRevision !== undefined) {
            entry.trailingPriority = Math.min(entry.trailingPriority ?? priority, priority);
    ...
    drainQueue() {
        while (this.activeRequests < 2) {
            const index = this.queue.findIndex((job) => !job.background || this.activeBackgroundRequests < 1);
            if (index < 0)
                return;
    ...
    async fetchPage(query) {
        const page = await (query.routeKind === 'qualified'
            ...
            : this.client.listWorkspaceSessionsPage(query.workspaceCwd, query.options));
        return {
            ...page,
            sessions: page.sessions.map((session) => session.workspaceCwd
                ? session
    ...
    resolveWaiters(entry, page) {
        const pending = [];
        for (const waiter of entry.waiters) {
            if (waiter.revision <= entry.acceptedRevision)
                waiter.resolve(page);
            else
                pending.push(waiter);
        }
    ...
    schedulePollFromNow(entry) {
        ...
        const delay = Math.max(0, entry.retryAt !== undefined && entry.retryAt > now
            ? entry.retryAt - now
            : (interval ?? SESSION_CATALOG_ERROR_RETRY_MS));
        entry.nextPollAt = now + delay;
        if (!this.isHidden())
            this.scheduleEntryTimer(entry, delay);
    }
    scheduleEntryTimer(entry, delay) {
        ...
        entry.pollTimer = setTimeout(() => {
            ...
            else if (hasAutoLoadSubscriber(entry) &&
                (entry.snapshot.page === undefined || entry.snapshot.stale)) {
                this.requestBackground(entry, 'initial');
            }
        }, Math.max(0, delay));
    ...
    updateVisibilityListener() {
        const needsListener = Array.from(this.entries.values()).some((entry) => entry.subscribers.size > 0);
        if (needsListener &&
    ...
    removeVisibilityListener() {
        if (!this.visibilityListening || typeof document === 'undefined')
            return;
    ...
    onVisibilityChange = () => {
        ...
        for (const entry of this.entries.values()) {
            ...
            if (hasAutoLoadSubscriber(entry) &&
                (entry.snapshot.page === undefined || entry.snapshot.stale)) {
                this.requestBackground(entry, 'initial');
                continue;
            }
            ...
            if (entry.nextPollAt !== undefined && entry.nextPollAt <= now) {
                ...
                this.scheduleEntryTimer(entry, Math.max(0, (entry.nextPollAt ?? now + interval) - now));
            }
        }
    };
}
