...
import { getDaemonBaseUrl, getDaemonToken, removeDaemonTokenFromUrl, waitForDaemonTokenMessage, } from './config/daemon';
...
function getWorkspaceIdFromUrl() {
    return (new URLSearchParams(window.location.search).get('workspace') || undefined);
}
...
export function StandaloneApp({ daemonToken }) {
    ...
    const handleSessionIdChange = useCallback((nextSessionId, nextWorkspaceId) => {
        setSessionId(nextSessionId);
        setWorkspaceId(nextWorkspaceId);
        replaceStandaloneSessionUrl(nextSessionId, nextWorkspaceId);
    }, []);
    return (_jsx(ErrorBoundary, { label: "web-shell-root", fallback: (error, reset) => (_jsx(RootErrorFallback, { error: error, onRetry: reset, language: language })), children: _jsx(DaemonWorkspaceProvider, { baseUrl: baseUrl, token: daemonToken, children: _jsx(WorkspaceSessionProvider, { sessionId: sessionId, workspaceId: workspaceId, webShellProps: {
                    theme,
...
async function main() {
    ...
    ReactDOM.createRoot(document.getElementById('root')).render(_jsx(React.StrictMode, { children: _jsx(StandaloneApp, { daemonToken: daemonToken }) }));
}
void main();
