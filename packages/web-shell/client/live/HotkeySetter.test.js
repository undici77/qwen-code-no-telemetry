...
function render(onChange) {
    ...
    roots.push(root);
    act(() => {
        root.render(_jsx(HotkeySetter, { accelerator: "Command+E", disabled: false, captureLabel: "Press shortcut", clearLabel: "Clear", offLabel: "Off", onChange: onChange }));
    });
    return container;
}
describe('HotkeySetter', () => {
    it('captures an ordinary Electron accelerator', async () => {
        ...
        const button = container.querySelector('button[aria-label="Press shortcut"]');
        act(() => button?.click());
    ...
    it('cancels capture with Escape and clears to Off explicitly', async () => {
        const onChange = vi.fn(async () => { });
        const container = render(onChange);
        const capture = container.querySelector('button[aria-label="Press shortcut"]');
        act(() => capture?.click());
        act(() => {
...
//# sourceMappingURL=HotkeySetter.test.js.map