...
import { useLiveVoiceSetup, } from './useLiveVoiceSetup';
...
describe('useLiveVoiceSetup', () => {
    it('does not let an older status poll overwrite a completed mutation', async () => {
        let resolveStatus;
        mocks.client.liveSetupStatus.mockReturnValue(new Promise((resolve) => {
        ...
        function Harness() {
            ...
            return _jsx("span", { children: setup.status?.enabled ? 'enabled' : 'disabled' });
        }
        await act(async () => {
    ...
    it('keeps mutation errors across successful status refreshes', async () => {
        ...
        function Harness() {
            setup = useLiveVoiceSetup(true);
            return _jsx("span", { children: setup.error?.message ?? 'ok' });
        }
        await act(async () => {
            root.render(_jsx(Harness, {}));
            await Promise.resolve();
    ...
    it('does not contact setup routes when the daemon hides Live', async () => {
        ...
        const root = createRoot(container);
        function Harness() {
            return _jsx(_Fragment, { children: String(useLiveVoiceSetup(false).supported) });
        }
        await act(async () => {
            root.render(_jsx(Harness, {}));
            await Promise.resolve();
...
//# sourceMappingURL=useLiveVoiceSetup.test.js.map