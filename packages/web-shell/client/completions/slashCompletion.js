...
export function getSlashCommandCompletionResult(
  ...
  language = 'en',
  translate = (key) => key,
  categoryOrder = DEFAULT_COMMAND_CATEGORY_ORDER,
) {
  const {
    lineStart,
    lineEnd,
  ...
  if (subMatch) {
    ...
    const items = filteredNodes.slice(0, COMPLETION_ITEM_LIMIT).map((node) => {
...
export function slashCompletionSource(
  ...
  getLanguage = () => 'en',
  translate = (key) => key,
  getCategoryOrder = () => DEFAULT_COMMAND_CATEGORY_ORDER,
) {
  return (context) => {
