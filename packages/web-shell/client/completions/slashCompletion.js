import { Fzf } from 'fzf';
import { compareCommandsByCategory, DEFAULT_COMMAND_CATEGORY_ORDER, getCategoryRank, getCommandDisplayCategory, } from '../utils/commandDisplay';
const COMMAND_NAME_PATTERN = String.raw `([^\s/]+)`;
const SUBCOMMAND_TREE_ZH = {
    agents: [
...
const IMPLICIT_SUBCOMMAND_TREE_ZH = {
...
const IMPLICIT_SUBCOMMAND_TREE_EN = {
...
function resolveSubcommands(cmdName, parts, dynamicSkills, language, commandSubcommands) {
    ...
    if (!nodes) {
        const implicitTree = language === 'zh-CN'
...
function comparePrefixFirst(a, b, query) {
...
function compareSlashCommands(a, b, query, categoryOrder) {
...
function renderCommandSectionHeader(section) {
...
function getCommandSection(command, translate, categoryOrder) {
...
export function getMissingSlashPrefixCompletion(text, commands) {
...
export function getImplicitTabCompletion(text, commands, language) {
...
export function getSlashCommandArgumentHint(text, commands, language) {
    const match = text.match(new RegExp(`^/${COMMAND_NAME_PATTERN}(\\s*)$`));
    if (!match)
        return null;
    const cmdName = match[1];
...
function getLineBounds(text, cursor) {
...
function getCommandFzf(commands) {
...
function fuzzyRankCommands(commands, query) {
    try {
        ...
        console.warn('[web-shell] slash fuzzy search failed, falling back to substring match:', error);
        const lp = query.toLowerCase();
        return commands.filter((command) => command.name.toLowerCase().includes(lp));
    }
}
export function getSlashCommandCompletionResult(text, cursor, commands, skills = [], language = 'en', translate = (key) => key, categoryOrder = DEFAULT_COMMAND_CATEGORY_ORDER) {
    const { lineStart, lineEnd, lineText, textBefore, cursor: safeCursor, } = getLineBounds(text, cursor);
    const subMatch = textBefore.match(new RegExp(`^/${COMMAND_NAME_PATTERN}\\s+(.*)$`));
    if (subMatch) {
        ...
        const nodes = resolveSubcommands(cmdName, completedParts, skills, language, cmd?.subcommands);
        if (!nodes)
            return null;
        ...
        const filteredNodes = nodes
            ...
            .sort((a, b) => currentTyping ? comparePrefixFirst(a.name, b.name, lp) : 0);
    ...
    const filteredCommands = isBrowsing
        ? [...commands].sort((a, b) => compareSlashCommands(a, b, '', categoryOrder))
...
export function slashCompletionSource(getCommands, getSkills = () => [], getLanguage = () => 'en', translate = (key) => key, getCategoryOrder = () => DEFAULT_COMMAND_CATEGORY_ORDER) {
    return (context) => {
        ...
        if (subMatch) {
            ...
            const nodes = resolveSubcommands(cmdName, completedParts, getSkills(), language, cmd?.subcommands);
            if (!nodes)
                return null;
            const lp = currentTyping.toLowerCase();
            const prefix = `/${cmdName} ${completedParts.length > 0 ? completedParts.join(' ') + ' ' : ''}`;
            ...
            const maxNameLength = isSkillList
                ...
                : 0;
            const options = filteredNodes.map((n) => {
        ...
        const options = filteredCommands.map((c) => {
...
//# sourceMappingURL=slashCompletion.js.map