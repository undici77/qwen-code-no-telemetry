...
import { getSlashCommandCompletionResult, getSlashCommandArgumentHint, slashCompletionSource, } from './slashCompletion';
describe('getSlashCommandArgumentHint', () => {
    it('returns a command argument hint for a bare slash command', () => {
        ...
        expect(getSlashCommandArgumentHint('/stats', commands, 'en')).toBe('[model|tools]');
        expect(getSlashCommandArgumentHint('/stats ', commands, 'en')).toBe('[model|tools]');
    });
    it('falls back to implicit subcommands when no argument hint is provided', () => {
        ...
        expect(getSlashCommandArgumentHint('/context', commands, 'en')).toBe('[detail]');
    });
    ...
    it('returns argument hints for Chinese command names', () => {
        ...
        expect(getSlashCommandArgumentHint('/学生信息', commands, 'zh-CN')).toBe('<姓名>');
    });
});
describe('getSlashCommandCompletionResult', () => {
    it('returns top-level command items for the custom slash panel', () => {
        ...
        const result = getSlashCommandCompletionResult('/', 1, commands, [], 'en', getTranslator('en'));
    ...
    it('returns explicit subcommands after accepting a parent command', () => {
        ...
        const result = getSlashCommandCompletionResult('/agents ', 8, commands, [], 'en', getTranslator('en'));
    ...
    it('returns command-provided subcommands when no built-in tree exists', () => {
        ...
        const result = getSlashCommandCompletionResult('/custom t', 9, commands, [], 'en', getTranslator('en'));
    ...
    it('returns dynamic skills for /skills', () => {
        ...
        const result = getSlashCommandCompletionResult('/skills re', 10, commands, [{ name: 'review', description: 'Review current code' }], 'en', getTranslator('en'));
    ...
    it('fuzzy-ranks top-level commands for an abbreviated query', () => {
        ...
        const mdl = getSlashCommandCompletionResult('/mdl', 4, commands, [], 'en', getTranslator('en'));
        expect(mdl?.items[0]?.label).toBe('/model');
        const arf = getSlashCommandCompletionResult('/arf', 4, commands, [], 'en', getTranslator('en'));
        expect(arf?.items.map((item) => item.label)).toContain('/agent-reproduce-feature');
    });
    it('drops section headers while searching but keeps them while browsing', () => {
        ...
        const browsing = getSlashCommandCompletionResult('/', 1, commands, [], 'en', getTranslator('en'));
        expect(browsing?.items.every((item) => Boolean(item.section))).toBe(true);
        const searching = getSlashCommandCompletionResult('/c', 2, commands, [], 'en', getTranslator('en'));
        expect(searching?.items.map((item) => item.label)).toEqual(['/clear']);
        expect(searching?.items.every((item) => item.section === undefined)).toBe(true);
    });
    it('returns null when fuzzy search matches nothing', () => {
        const commands = [
            { name: 'clear', description: 'Clear', source: 'builtin-command' },
        ];
        const result = getSlashCommandCompletionResult('/zzzzzzz', 8, commands, [], 'en', getTranslator('en'));
    ...
    it('honors panel category order and filtered commands', () => {
        ...
        const result = getSlashCommandCompletionResult('/', 1, commands, [], 'en', getTranslator('en'), ['system', 'custom', 'skill']);
        expect(result?.items.map((item) => item.label)).toEqual([
            '/clear',
            '/demo:ping',
        ]);
    ...
    it('limits the rendered command menu while keeping search available', () => {
        ...
        const searching = getSlashCommandCompletionResult('/command-999', 12, commands);
...
describe('slashCompletionSource', () => {
    it('completes a top-level slash command from any cursor position in the command', () => {
        ...
        const source = slashCompletionSource(() => commands, () => [], () => 'en', getTranslator('en'));
    ...
    it('orders slash commands by custom, skill, then system categories', () => {
        ...
        expect(result?.options.map((option) => {
            const section = typeof option.section === 'string' ? undefined : option.section;
    ...
    it('localizes slash command category section titles', () => {
        ...
        const source = slashCompletionSource(() => commands, () => [], () => 'zh-CN', getTranslator('zh-CN'));
        const state = EditorState.create({ doc: '/' });
        const result = source(new CompletionContext(state, 1, true));
        expect(result?.options.map((option) => {
            const section = typeof option.section === 'string' ? undefined : option.section;
    ...
    it('supports custom slash command category order', () => {
        ...
        const source = slashCompletionSource(() => commands, () => [], () => 'en', getTranslator('en'), () => ['system', 'custom', 'skill']);
        const state = EditorState.create({ doc: '/' });
        const result = source(new CompletionContext(state, 1, true));
        expect(result?.options.map((option) => option.label)).toEqual([
            '/clear',
...
//# sourceMappingURL=slashCompletion.test.js.map