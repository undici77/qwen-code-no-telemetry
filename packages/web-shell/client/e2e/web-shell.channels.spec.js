...
import { createWebShellDaemonScenario, installMockDaemon, replayCompleteEvent, } from './utils/mockDaemon';
test('shows channel sessions in the sidebar channel catalog', async ({ page, }, testInfo) => {
    ...
    await daemon.sendEvent(replayCompleteEvent({ sessionId: connection.sessionId }));
    ...
    await expect(page.getByText('DingTalk conversation', { exact: true })).toBeVisible();
    ...
    await expect(page.getByRole('region', { name: 'Feishu' })).toContainText('Feishu conversation');
    await expect(page.getByRole('region', { name: 'Other channels' })).toContainText('Legacy channel conversation');
    ...
    await expect(page.getByText('DingTalk conversation', { exact: true })).toHaveCount(0);
    await dingTalkToggle.click();
    await expect(dingTalkToggle).toHaveAttribute('aria-expanded', 'true');
    ...
    await expect(page.getByText('New DingTalk conversation', { exact: true })).toBeVisible({ timeout: 5_000 });
    await expect(dingTalkGroup).toContainText('New DingTalk conversation');
});
test('creates and deletes a typed Channel configuration', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario({
        capabilities: {
            features: [
                'session_events',
    ...
    await expect(page.getByRole('heading', { name: 'Configure DingTalk' })).toHaveCount(0);
    ...
    await expect(page.getByRole('heading', { name: 'Edit DingTalk' })).toBeVisible();
    ...
    await expect
        .poll(() => daemon.requests.filter((request) => request.method === 'POST' &&
        request.path.endsWith('/channels/release-bot/pairing-requests/approve')))
        .toEqual([
        expect.objectContaining({
            body: { code: 'ABCD1234' },
        }),
    ...
    await expect(page.getByRole('button', { name: 'Revoke Group: group-9' })).toBeVisible();
    await page.getByRole('button', { name: 'Revoke Group: group-9' }).click();
    const groupRevokeConfirmation = page.getByRole('alertdialog');
    await expect(groupRevokeConfirmation).toContainText('Only the approval created through pairing will be removed.');
    await groupRevokeConfirmation
        .getByRole('button', { name: 'Revoke approval' })
        .click();
    await expect(page.getByRole('button', { name: 'Revoke user-42' })).toBeVisible();
    await page.getByRole('button', { name: 'Revoke user-42' }).click();
    const revokeConfirmation = page.getByRole('alertdialog');
    await expect(revokeConfirmation).toContainText('Only the approval created through pairing will be removed.');
    await revokeConfirmation
        .getByRole('button', { name: 'Revoke approval' })
        .click();
    await expect(page.getByText('No pairing approvals')).toBeVisible();
    await expect
        .poll(() => daemon.requests.filter((request) => request.method === 'DELETE' &&
...
//# sourceMappingURL=web-shell.channels.spec.js.map