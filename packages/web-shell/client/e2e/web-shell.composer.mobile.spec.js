...
import { assistantTextEvent, createWebShellDaemonScenario, installMockDaemon, replayCompleteEvent, turnCompleteEvent, } from './utils/mockDaemon';
import { emptyMobileComposerLayout, expectEmptyMobileComposerAnchored, expectEmptyMobileWelcomeChromeVisible, gotoEmptyMobileWelcomeHarness, } from './utils/emptyMobileComposer';
const COMPOSER_TEXTAREA = 'textarea[data-web-shell-composer-editor]';
test('renders the textarea backend instead of CodeMirror on touch devices', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    await gotoSession(page, scenario, daemon);
    await expect(page.locator(COMPOSER_TEXTAREA)).toBeVisible();
    await expect(page.locator('.cm-editor')).toHaveCount(0);
    await expect(page.locator('[data-web-shell-composer-typewriter]')).toHaveCount(0);
});
test('anchors the empty mobile composer with the textarea backend', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    ...
    await expect(textarea).toHaveValue('Composer remains interactive on touch devices');
});
test('keeps the composer usable after WebGL context loss', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    await gotoSession(page, scenario, daemon);
...
test('keeps voice controls reachable on an extra-narrow touch viewport', async ({ page, }, testInfo) => {
...
test('tap, type, and Send submit through the shared prompt pipeline', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    await gotoSession(page, scenario, daemon);
    const textarea = page.locator(COMPOSER_TEXTAREA);
    ...
    expectPromptBodyToContainText(firstRequestBody(daemon.promptRequests()), 'Ping from mobile');
...
test('Enter inserts a newline and does not submit', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    await gotoSession(page, scenario, daemon);
    const textarea = page.locator(COMPOSER_TEXTAREA);
...
test('keeps the textarea scrollable once content exceeds the height cap', async ({ page, }, testInfo) => {
    ...
    expect(metrics.scrollTop).toBeGreaterThanOrEqual(metrics.scrollHeight - metrics.clientHeight - 2);
});
test('slash commands typed as text still execute as commands', async ({ page, }, testInfo) => {
...
test('?composer=codemirror escape hatch forces the CodeMirror path', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    await page.goto(`/session/${encodeURIComponent(scenario.sessionId)}?composer=codemirror`);
...
function firstRequestBody(requests) {
...
function expectPromptBodyToContainText(body, text) {
    ...
    const blocks = prompt;
    expect(blocks.some((block) => typeof block === 'object' &&
        block !== null &&
        block['type'] === 'text' &&
        block['text'] === text)).toBe(true);
}
//# sourceMappingURL=web-shell.composer.mobile.spec.js.map