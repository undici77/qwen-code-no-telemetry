...
import { assistantTextEvent, createWebShellDaemonScenario, turnCompleteEvent, } from '../utils/mockDaemon';
import { beat, fillComposer, gotoSession, installScenario, recordFlow, resolveBaseURL, } from './harness';
...
test('flow: open the slash menu and switch model', async ({ browser, }, testInfo) => {
...
test('flow: submit a prompt and watch the reply stream in', async ({ browser, }, testInfo) => {
    const url = resolveBaseURL(testInfo);
    await recordFlow(browser, url, 'prompt-stream', async (page) => {
        ...
        await daemon.sse.split(assistantTextEvent('Streaming a reply from the mock daemon', { id: 10 }));
        await beat(page);
        ...
        await expect(page.locator('[data-web-shell-message-list]')).toContainText('Streaming a reply');
        await beat(page, 900);
    });
});
...
test('flow: a drive error propagates instead of being masked', async ({ browser, }, testInfo) => {
    const url = resolveBaseURL(testInfo);
    await expect(recordFlow(browser, url, 'drive-error', async () => {
