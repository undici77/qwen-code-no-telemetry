...
import { installMockDaemon, replayCompleteEvent, } from '../utils/mockDaemon';
 ...
 */
export async function gotoSession(page, scenario, daemon, theme) {
    await primeTheme(page, theme);
    await page.goto(`/session/${encodeURIComponent(scenario.sessionId)}?theme=${theme}`);
 ...
 */
export async function gotoNewSession(page, theme) {
    await primeTheme(page, theme);
    await page.goto(`/?theme=${theme}`);
    await expect(page.locator('[data-web-shell-root]')).toBeVisible();
    await expect(page.locator('html')).toHaveClass(new RegExp(`theme-${theme}`));
}
export async function completeReplay(page, daemon, sessionId, replayedCount = 0) {
    const connection = await daemon.sse.waitForConnection(sessionId);
    await daemon.sendEvent(replayCompleteEvent({ sessionId: connection.sessionId, replayedCount }));
 ...
 */
export async function recordFlow(browser, baseURL, name, drive) {
