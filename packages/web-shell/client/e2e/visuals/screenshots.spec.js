...
import { assistantTextEvent, createWebShellDaemonScenario, permissionRequestEvent, turnCompleteEvent, userTextEvent, } from '../utils/mockDaemon';
import { captureScreenshot, completeReplay, fillComposer, gotoNewSession, gotoSession, installScenario, resolveBaseURL, submitLocalCommand, VISUAL_VIEWPORT, } from './harness';
...
for (const theme of THEMES) {
    test.describe(`web-shell screenshots (${theme})`, () => {
        test(`session transcript`, async ({ page }, testInfo) => {
            const scenario = createWebShellDaemonScenario({
                events: [
                    ...
                    assistantTextEvent('Here is a **streamed** reply with a code block:\n\n```ts\nexport const greeting = "hello from web-shell";\n```', { id: 2 }),
                    turnCompleteEvent('prompt-visual', { id: 3 }),
                ],
            });
            const daemon = await installScenario(page, scenario, resolveBaseURL(testInfo));
            await gotoSession(page, scenario, daemon, theme);
            await expect(page.locator('[data-web-shell-message-list]')).toContainText('Here is a');
            ...
            await expect(page.locator('[data-web-shell-message-list] pre.shiki').first()).toBeVisible();
        ...
        test(`parallel agents group`, async ({ page }, testInfo) => {
            ...
            const agentToolCallEvent = (id, toolCallId, description) => ({
                id,
                v: 1,
                type: 'session_update',
                data: {
            ...
            const scenario = createWebShellDaemonScenario({
                events: [
                    ...
                    agentToolCallEvent(2, 'call-agent-schema-audit', 'Audit the schema drift between services'),
                    agentToolCallEvent(3, 'call-agent-backfill-plan', 'Draft the backfill plan for the users table'),
            ...
            await summary.click();
            await expect(messageList.getByText('Audit the schema drift between services')).toBeVisible();
        ...
        test(`extensions manager`, async ({ page }, testInfo) => {
                    ...
                    extensions: [
                        {
                            ...
                            description: 'Up-to-date library docs injected into your prompts.',
                            version: '1.4.0',
                            isActive: true,
                            path: '/ext/context7',
                            source: 'marketplace',
            ...
            await expect(page.getByRole('heading', { name: 'Manage Extensions' })).toBeVisible();
            await expect(page.getByRole('button', { name: 'Context7' })).toBeVisible();
            await expect(page.getByRole('button', { name: 'Local Notes' })).toBeVisible();
        ...
        test(`Channel manager`, async ({ page }, testInfo) => {
            ...
            await expect(configuredChannels.getByText('dingtalk', { exact: true })).toBeVisible();
            await expect(configuredChannels.getByText('release-notifier', { exact: true })).toBeVisible();
            ...
            await expect(page.getByRole('heading', { name: 'Configure DingTalk' })).toBeVisible();
        ...
        test(`GitHub channel editor`, async ({ page }, testInfo) => {
            ...
            await expect(page.getByText('Enter a token or enable local GitHub CLI authentication.')).toBeVisible();
            await captureScreenshot(page, `github-channel-editor-credential-${theme}`);
            await page
                .getByRole('switch', {
                name: 'Use local GitHub CLI authentication',
            })
                .click();
            await page.getByRole('button', { name: 'Save' }).click();
            await expect(page.getByText('Enter a token or enable local GitHub CLI authentication.')).toHaveCount(0);
            await expect(page.getByRole('heading', { name: 'Configure GitHub' })).toHaveCount(0);
            await expect
                .poll(() => daemon.requests.filter((request) => request.method === 'PUT' &&
        ...
        test(`mermaid diagram`, async ({ page }, testInfo) => {
            const scenario = createWebShellDaemonScenario({
                events: [
                    ...
                    assistantTextEvent('Here is the tool-approval flow:\n\n' +
                        '```mermaid\n' +
                        'flowchart LR\n' +
            ...
            await expect(page.locator('[data-web-shell-message-list] svg[id^="mermaid-"]')).toBeVisible();
        ...
        test(`split view`, async ({ page }, testInfo) => {
            ...
            const secondSessionId = scenario.sessions.find((s) => s.sessionId !== scenario.sessionId)?.sessionId;
            if (!secondSessionId) {
                throw new Error('split view scenario expects a second session in the list');
            }
            const daemon = await installScenario(page, scenario, resolveBaseURL(testInfo));
            ...
            await expect(page.getByRole('button', { name: 'Maximize pane' }).first()).toBeVisible();
            ...
            await expect(page.getByRole('button', { name: 'Restore pane' })).toBeVisible();
        ...
        test(`workspace sidebar`, async ({ page }, testInfo) => {
            ...
            const sidebar = page.getByRole('complementary');
            await expect(sidebar.getByText('qwen-web-shell-e2e', { exact: true })).toBeVisible();
            await expect(sidebar.getByText('qwen-api-service', { exact: true })).toBeVisible();
        ...
        test(`git mode selector`, async ({ page }, testInfo) => {
            ...
            // reviewed screenshot.
            await chip.click();
            await expect(page.getByText('Current branch', { exact: true })).toBeVisible();
            ...
            await expect(branchInput).toBeVisible();
            await expect(page.locator('[data-testid="git-mode-confirm-branch"]')).toBeEnabled();
        ...
        test(`code review artifact`, async ({ page }, testInfo) => {
                ...
                findings: [
                    {
                        ...
                        confidence: 'high',
                        source: 'review',
                        summary: 'Review verdict is reported even when the child process times out',
                        shortSummary: 'timeout treated as success',
                        failureScenario: 'When `review run` times out, the CLI still prints a verdict as if the review completed.',
                        suggestedFix: 'Fail closed when timedOut is true instead of reporting the verdict.',
                    ...
                    },
                    {
                        ...
                        confidence: 'low',
                        source: 'lint',
                        summary: 'Artifact evidence links should render their file name only',
                        shortSummary: 'verbose evidence labels',
                        failureScenario: 'Long asset URLs overflow the finding card in narrow panels.',
                        locations: [
                            {
            ...
            await expect(page.getByText('Verdict: Request changes (1 Critical, 1 Suggestion)')).toBeVisible();
            await expect(page.getByText('Review verdict is reported even when the child process times out')).toBeVisible();
            ...
            await page
                ...
                .click();
            await expect(page.locator('[class*="artifactPanelFullscreen"]')).toBeVisible();
            ...
            await page.keyboard.press('Escape');
            await expect(page.locator('[class*="artifactPanelFullscreen"]')).toHaveCount(0);
        ...
        test(`drawer fullscreen`, async ({ page }, testInfo) => {
            ...
            await page
                ...
                .getByRole('button', { name: 'Open', exact: true })
                .click();
            const drawerAside = page.locator('[data-web-shell-portal-root] aside[aria-label="Right panel"]');
            await expect(drawerAside).toBeVisible();
            await drawerAside
                .getByRole('button', { name: 'Fullscreen', exact: true })
                .click();
            await expect(page.locator('aside[class*="panelFullscreen"]')).toBeVisible();
            await captureScreenshot(page, `drawer-fullscreen-${theme}`);
            // Escape shrinks the surface back to the drawer width; the drawer
            // itself stays open.
            await page.keyboard.press('Escape');
            await expect(page.locator('aside[class*="panelFullscreen"]')).toHaveCount(0);
            await expect(drawerAside).toBeVisible();
        });
    });
}
