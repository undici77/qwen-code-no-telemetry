...
test('restores the split across a reload and isolates it per tab @smoke', async ({ page, context, }, testInfo) => {
    ...
    await expect
        .poll(async () => page.evaluate((key) => window.sessionStorage.getItem(key), STORAGE_KEY))
...
test('leaving the split clears storage so a refresh does not restore it', async ({ page, }, testInfo) => {
    await page.setViewportSize({ width: 1440, height: 900 });
    const scenario = createSplitScenario();
    await installScenario(page, scenario, testInfo);
    await page.goto(`/?split=${SESSION_A},${SESSION_B}`);
...
//# sourceMappingURL=web-shell.split-persist.spec.js.map