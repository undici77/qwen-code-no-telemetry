...
import { createWebShellDaemonScenario, installMockDaemon, } from './utils/mockDaemon';
const WORKSPACE_CWD = '/tmp/qwen-web-shell-e2e';
function createGitWorkspaceScenario(overrides = {}) {
...
async function installScenario(page, scenario, baseURL) {
...
function sessionCreateBody(daemon) {
    const record = daemon.requests.find((r) => r.method === 'POST' && r.path === '/session');
    return record?.body;
}
test('git mode chip shows popover with three modes and captures screenshots', async ({ page, }, testInfo) => {
    const scenario = createGitWorkspaceScenario();
    const daemon = await installScenario(page, scenario, String(testInfo.project.use.baseURL));
    await page.goto('/');
...
test('git mode chip worktree mode sends worktree intent', async ({ page, }, testInfo) => {
    const scenario = createGitWorkspaceScenario();
    const daemon = await installScenario(page, scenario, String(testInfo.project.use.baseURL));
    await page.goto('/');
    const chip = page.locator('[data-testid="git-mode-chip"]');
...
test('git mode chip default current-branch mode sends neither branch nor worktree', async ({ page, }, testInfo) => {
    const scenario = createGitWorkspaceScenario();
    const daemon = await installScenario(page, scenario, String(testInfo.project.use.baseURL));
    await page.goto('/');
    const chip = page.locator('[data-testid="git-mode-chip"]');
...
test('git mode chip clear button resets to current branch', async ({ page, }, testInfo) => {
    const scenario = createGitWorkspaceScenario();
    const daemon = await installScenario(page, scenario, String(testInfo.project.use.baseURL));
    await page.goto('/');
    const chip = page.locator('[data-testid="git-mode-chip"]');
...
test('git mode chip is hidden when workspace is not a git repo', async ({ page, }, testInfo) => {
    const scenario = createGitWorkspaceScenario({ gitStatus: undefined });
    const daemon = await installScenario(page, scenario, String(testInfo.project.use.baseURL));
    await page.goto('/');
    // Wait for git status request to complete
    await expect
        .poll(() => daemon.requests.some((r) => /^\/workspaces\/.+\/git/.test(r.path)))
        .toBe(true);
...
//# sourceMappingURL=web-shell.git-mode.spec.js.map