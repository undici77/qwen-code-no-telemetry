...
export async function installMockDaemon(page, scenario, options = {}) {
    ...
    return {
        ...
        configOptionRequests: () => requests.filter((request) => /\/session\/[^/]+\/config-option$/.test(request.path)),
    };
}
...
function filterScenarioSessions(scenario, searchParams) {
    const group = searchParams.get('group');
    const sourceType = searchParams.get('sourceType');
    const sourceSessions = sourceType
        ? scenario.sessions.filter((session) => session.sourceType === sourceType ||
...
function isDaemonPath(path) {
    return (path === '/health' ||
        ...
        /^\/workspaces\/[^/]+\/channels\/[^/]+\/pairing-requests(?:\/approve)?\/?$/.test(path) ||
        ...
        /^\/workspaces\/.+\/git\/(branches|checkout|branch|push|pull|commit|diff|log)\/?$/.test(path) ||
        /^\/workspace\/git\/(branches|checkout|branch|push|pull|commit|diff|log)\/?$/.test(path) ||
        ...
        /^\/session\/[^/]+\/(load|resume|prompt|permission\/[^/]+|context|supported-commands|events|model|config-option|approval-mode|heartbeat|cancel|detach|btw)\/?$/.test(path));
}
function isDaemonRoute(method, path) {
    ...
    if (method === 'POST' &&
        /^\/session\/[^/]+\/(load|resume|prompt|permission\/[^/]+|model|config-option|approval-mode|heartbeat|cancel|detach)\/?$/.test(path)) {
        return true;
    }
    return (method === 'GET' &&
...
async function handleDaemonRoute(route, method, path, scenario, body, searchParams = new URLSearchParams()) {
    ...
    const pairingMatch = path.match(/^\/workspaces\/[^/]+\/channels\/([^/]+)\/pairing-requests(\/approve)?\/?$/);
    if (pairingMatch) {
    ...
    const pairingApprovalsMatch = path.match(/^\/workspaces\/[^/]+\/channels\/([^/]+)\/pairing-approvals\/?$/);
    if (pairingApprovalsMatch) {
        ...
        if (method === 'DELETE') {
            ...
            const remainingSenders = senderId
                ...
                : senderIds;
            const remainingGroups = groupId && !senderId
    ...
    const channelMutationMatch = path.match(/^\/workspaces\/[^/]+\/channels\/([^/]+)\/?$/);
    ...
    if (method === 'GET' && /^\/workspaces\/.+\/git\/?$/.test(path)) {
        await json(route, scenario.gitStatus ?? {
            v: 2,
    ...
    if (method === 'GET' && /^\/workspaces\/.+\/github\/prs\/?$/.test(path)) {
        await json(route, scenario.gitHubPrs ?? {
            v: 1,
            workspaceCwd: scenario.workspaceCwd,
            available: true,
            pullRequests: [],
    ...
    if (method === 'GET' &&
        ...
        await json(route, scenario.gitBranches ?? {
            v: 1,
            workspaceCwd: scenario.workspaceCwd,
            available: true,
            local: [
    ...
    if (method === 'GET' &&
        ...
        await json(route, scenario.gitDiff ?? {
            v: 1,
            workspaceCwd: scenario.workspaceCwd,
            available: true,
            files: [
    ...
    if (method === 'GET' &&
        ...
        await json(route, scenario.gitLog ?? {
            v: 1,
            workspaceCwd: scenario.workspaceCwd,
            available: true,
            entries: [
    ...
    if (method === 'POST' &&
        /^\/(workspaces\/.+\/|workspace\/)?git\/(checkout|branch|push|pull|commit)\/?$/.test(path)) {
    ...
    if (method === 'POST' &&
        ...
        await json(route, { url: 'https://github.com/example/repo/pull/42', number: 42 }, 201);
        return;
    }
    if (method === 'POST' && /^\/session\/[^/]+\/btw\/?$/.test(path)) {
        await json(route, {
            sessionId: path.split('/')[2],
            answer: scenario.btwAnswer ??
    ...
    if (sessionMatch) {
        ...
        if (action === 'config-option') {
            ...
            const configOptions = Array.isArray(scenario.state.configOptions)
                ? scenario.state.configOptions.map((option) => isRecord(option) && option['id'] === configId
...
function sessionArtifactsEnvelope(scenario, sessionId) {
    return {
        v: 1,
        sessionId,
...
function workspaceFile(path, content) {
