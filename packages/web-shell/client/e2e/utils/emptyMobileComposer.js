...
export async function gotoEmptyMobileWelcomeHarness(page, options = {}) {
    const params = new URLSearchParams({ emptyMobileWelcome: 'true' });
    if (options.welcomeFooter === false)
        params.set('welcomeFooter', 'false');
    if (options.customFooter === true)
        params.set('customFooter', 'true');
    await page.goto(`/e2e/composer-layout-harness.html?${params.toString()}`);
    await expect(page.locator('[data-web-shell-root]')).toBeVisible();
}
export async function expectEmptyMobileWelcomeChromeVisible(page, options = {}) {
    const composer = page.locator(emptyMobileComposerSelectors.composerSurface);
    const welcomeHeader = page.locator(emptyMobileComposerSelectors.welcomeHeader);
    const dotField = page.locator(emptyMobileComposerSelectors.dotField);
    await expect(composer).toBeVisible();
    await expect(welcomeHeader).toBeVisible();
    if (options.requireWelcomeFooter !== false) {
        await expect(page.locator(`${emptyMobileComposerSelectors.welcomeFooter}:visible`)).toBeVisible();
    }
    else {
        await expect(page.locator(emptyMobileComposerSelectors.welcomeFooter)).toHaveCount(0);
    }
    await expect(dotField.locator('canvas')).toBeVisible();
}
export async function emptyMobileComposerLayout(page, options = {}) {
    return page.getByTestId('chat-pane-container').evaluate((chatPane, { selectors, requireWelcomeFooter }) => {
        ...
        const welcomeFooter = Array.from(chatPane.querySelectorAll(selectors.welcomeFooter)).find((candidate) => candidate.getClientRects().length > 0);
        const dotField = chatPane.querySelector(selectors.dotField);
        if (!composer) {
        ...
        const chatView = Array.from(chatPane.children).find((child) => child.contains(composer));
        if (!chatView)
            throw new Error('Expected the composer chat view.');
        const composerShell = composer.closest('[data-web-shell-composer]');
        if (!composerShell)
            throw new Error('Expected the composer shell.');
        let footer;
        for (let ancestor = composerShell.parentElement; ancestor && ancestor !== chatPane; ancestor = ancestor.parentElement) {
        ...
        if (!footer)
            throw new Error('Expected the composer footer.');
        if (footer.getClientRects().length === 0) {
            throw new Error('The composer footer has no box in the custom footer welcome variant (it renders display: contents by design), so anchored layout measurements do not apply there.');
        }
        ...
        return {
            ...
            chatViewIsPaneFlexItem: chatView.parentElement === chatPane &&
            ...
            dotFieldCoversChatPane: Math.abs(dotFieldRect.top - chatPaneRect.top) <= 1 &&
...
export function expectEmptyMobileComposerAnchored(layout, options = {}) {
    expect(layout.footerPosition).toBe('absolute');
    expect(Math.abs(layout.footerBottom - layout.chatPaneBottom)).toBeLessThanOrEqual(1);
    ...
    expect(Number(layout.chatViewZIndex)).toBeGreaterThan(Number(layout.dotFieldZIndex));
    if (options.requireWelcomeFooter !== false) {
        if (layout.welcomeFooterTop === null ||
            layout.welcomeFooterBottom === null) {
            throw new Error('Expected a visible welcome footer.');
        }
        expect(layout.welcomeHeaderBottom).toBeLessThanOrEqual(layout.welcomeFooterTop);
        expect(layout.welcomeFooterBottom).toBeLessThanOrEqual(layout.composerTop);
    }
}
//# sourceMappingURL=emptyMobileComposer.js.map