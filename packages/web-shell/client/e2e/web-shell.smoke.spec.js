...
import { assistantTextEvent, createWebShellDaemonScenario, installMockDaemon, permissionRequestEvent, replayCompleteEvent, turnCompleteEvent, userTextEvent, } from './utils/mockDaemon';
import { emptyMobileComposerLayout, emptyMobileComposerSelectors, expectEmptyMobileComposerAnchored, expectEmptyMobileWelcomeChromeVisible, gotoEmptyMobileWelcomeHarness, } from './utils/emptyMobileComposer';
const COMPOSER_VIEWPORT_HEIGHTS = [1000, 800, 600];
test('loads replayed transcript and connects to fake daemon @smoke', async ({ page, }, testInfo) => {
    ...
    await expect(page.locator('[data-web-shell-message-list]')).toContainText('Hello from replay');
    await expect(page.locator('[data-web-shell-message-list]')).toContainText('Hello from fake daemon');
...
test('submits a prompt and renders a streamed assistant response @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    await gotoSession(page, scenario, daemon);
    ...
    expectPromptBodyToContainText(requestBodyRecord(promptRequest), 'Ping from browser smoke');
    ...
    await expect(page.locator('[data-web-shell-message-list]')).toContainText('Pong from fake SSE');
});
test('configures qwen3.8-max reasoning from the model popover @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario({
    ...
    expect(requestBodyRecord(firstRequest(daemon.configOptionRequests()))).toEqual({ configId: 'reasoning_effort', value: 'medium' });
    ...
    await expect(page.locator('[data-web-shell-model-submenu] input[type="search"]')).toBeVisible();
});
test('uploads an Extension archive from the manager @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    let uploadUrl = '';
    let uploadHeaders = {};
    ...
    await page.route('**/workspace/extensions/install-archive?*', async (route) => {
        ...
        await route.fulfill({
            ...
            body: JSON.stringify(rejectUpload
                ? { error: 'Archive rejected for test' }
                : { accepted: true, operationId: 'op-upload' }),
        });
    });
    await page.route('**/workspace/extensions/operations/op-upload', async (route) => {
        await route.fulfill({
            contentType: 'application/json',
            body: JSON.stringify({
                v: 1,
    ...
    await expect(page.getByText('Select a .zip or .tar.gz Extension archive with a valid filename up to 255 bytes.')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Install' })).toBeDisabled();
    await archiveInput.setInputFiles({
    ...
    await expect(page.getByText('The selected Extension archive is empty.')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Install' })).toBeDisabled();
    await archiveInput.setInputFiles({
    ...
    await expect(page.getByText('Extension archives must be 10 MB or smaller.')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Install' })).toBeDisabled();
    await archiveInput.setInputFiles({
        name: 'demo.tar.gz',
        mimeType: 'application/gzip',
    ...
    await expect(page.getByRole('heading', { name: 'Add Extension' })).toBeVisible();
    await expect(page.getByText('Selected archive: demo.zip')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Install' })).toBeEnabled();
    uploadUrl = '';
    uploadHeaders = {};
    ...
    await expect
        .poll(() => uploadUrl)
        .toContain('/workspace/extensions/install-archive?filename=demo.zip&consent=true');
    ...
    await expect(page.getByRole('heading', { name: 'Add Extension' })).toHaveCount(0);
...
test('pastes long plain text as editable composer content @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    ...
    await page.locator('[data-web-shell-composer-submit]').click();
    await expect.poll(() => daemon.promptRequests().length).toBe(1);
    expectPromptBodyToContainText(requestBodyRecord(firstRequest(daemon.promptRequests())), edited);
});
test('keeps later SSE connections alive when an earlier one is cancelled @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    await gotoSession(page, scenario, daemon);
    await page.reload();
    ...
    await page.locator('[data-web-shell-composer-submit]').click();
    await expect.poll(() => daemon.promptRequests().length).toBe(1);
    await daemon.sse.split(assistantTextEvent('Reconnect-safe SSE payload', { id: 20 }));
    await daemon.sendEvent(turnCompleteEvent('prompt-reconnect', { id: 21 }));
    await expect(page.locator('[data-web-shell-message-list]')).toContainText('Reconnect-safe SSE payload');
});
test('clears fake SSE connection records when streams close or error @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    const baseURL = String(testInfo.project.use.baseURL);
    await page.goto('data:text/html,<html></html>');
    await openRawSseConnection(page, baseURL, scenario.sessionId);
    const firstConnection = await daemon.sse.waitForConnection(scenario.sessionId);
    ...
    await expect
        ...
        .toBe(0);
    await openRawSseConnection(page, baseURL, scenario.sessionId);
    const secondConnection = await daemon.sse.waitForConnection(scenario.sessionId);
...
test('submits permission decisions through the fake daemon @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario({
    ...
    expect(permissionRequest.path).toBe(`/session/${scenario.sessionId}/permission/perm-1`);
    expect(requestBodyRecord(permissionRequest)).toEqual({
        outcome: { outcome: 'selected', optionId: 'allow_once' },
    });
});
test('opens slash menu, resume dialog, model dialog, and theme dialog @smoke', async ({ page, }, testInfo) => {
    ...
    await page
        .locator(`[data-web-shell-resume-session][data-session-id="${resumedSessionId}"]`)
        .click();
    ...
    await submitLocalCommand(page, '/model');
    await expect(page.locator('[data-web-shell-model-dialog]')).toBeVisible();
    await expect(page.locator('[data-web-shell-model-option][data-model-id="qwen-test-alt"]')).toHaveAttribute('aria-selected', 'true');
...
test('selects and scrolls scheduled-task prompt references @smoke', async ({ page, }, testInfo) => {
    ...
    await expect
        .poll(() => extensionsButton.evaluate((element) => {
        const style = getComputedStyle(element);
    ...
    await expect
        .poll(() => picker.evaluate((element) => element.scrollHeight > element.clientHeight + 1))
        .toBe(true);
    await picker.hover();
    ...
    const tag = prompt.locator('[data-prompt-tag-serialized="@ext:extension-20"]');
    await expect(tag).toBeVisible();
    const promptBox = await prompt.boundingBox();
    if (!promptBox)
        throw new Error('Prompt editor has no bounding box');
    const blankPosition = {
    ...
    await expect(remove.evaluate((element) => element.matches(':hover'))).resolves.toBe(false);
    await prompt.click({ position: blankPosition });
    await expect(tag).toBeVisible();
    await remove.click();
    await expect(tag).toHaveCount(0);
});
test('gates voice dictation on the workspace voice setting @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario({
        voice: {
            enabled: false,
        },
...
test('loads Voice status from the active secondary workspace @smoke', async ({ page, }, testInfo) => {
    ...
    await expect
        .poll(() => daemon.requests.filter((request) => request.method === 'GET' &&
        request.path === '/workspaces/secondary/voice').length)
        .toBeGreaterThan(0);
    expect(daemon.requests.some((request) => request.method === 'GET' && request.path === '/workspace/voice')).toBe(false);
});
test('anchors the empty mobile composer to the chat pane across the breakpoint @smoke', async ({ page, }, testInfo) => {
    ...
    if (wideLayout.welcomeFooterTop === null ||
        wideLayout.welcomeFooterBottom === null) {
        throw new Error('Expected a visible welcome footer above the breakpoint.');
    }
    expect(wideLayout.welcomeFooterBottom).toBeGreaterThan(wideLayout.welcomeFooterTop);
    await page.setViewportSize({ width: 760, height: 900 });
    await expect
        .poll(() => emptyMobileComposerLayout(page))
        .toMatchObject({
...
test('anchors the empty mobile composer without a welcome footer @smoke', async ({ page, }, testInfo) => {
    await page.setViewportSize({ width: 760, height: 900 });
    const scenario = createWebShellDaemonScenario();
    await installScenario(page, scenario, testInfo);
...
test('keeps the bottom status panel visible in the custom footer mobile welcome variant @smoke', async ({ page, }, testInfo) => {
    await page.setViewportSize({ width: 760, height: 900 });
    const scenario = createWebShellDaemonScenario();
    await installScenario(page, scenario, testInfo);
    ...
    expect(statusPanelBox.y + statusPanelBox.height).toBeLessThanOrEqual(chatPaneBox.y + chatPaneBox.height);
    ...
    await expect(emptyMobileComposerLayout(page)).rejects.toThrow(/custom footer welcome variant/);
});
test('anchors the empty mobile composer with a custom footer but no welcome footer @smoke', async ({ page, }, testInfo) => {
    await page.setViewportSize({ width: 760, height: 900 });
    const scenario = createWebShellDaemonScenario();
    await installScenario(page, scenario, testInfo);
    await gotoEmptyMobileWelcomeHarness(page, {
...
for (const viewportHeight of COMPOSER_VIEWPORT_HEIGHTS) {
    test(`grows long text to the responsive composer cap at ${viewportHeight}px @smoke`, async ({ page, }, testInfo) => {
        ...
        await replaceComposerText(page, Array.from({ length: 10 }, (_, index) => `Visible line ${index + 1}`).join('\n'));
        await expect
            .poll(() => composerHeight(page))
            .toBeGreaterThan(initialHeight);
        await replaceComposerText(page, Array.from({ length: 80 }, (_, index) => `Capped line ${index + 1}`).join('\n'));
        ...
        const modeDropdown = page.locator('[data-web-shell-toolbar-popover][data-state="open"]');
        ...
        await expect(page.locator('[data-web-shell-toolbar-popover] input[type="search"]')).toBeVisible();
        await modeButton.click();
        await expect(modeDropdown).toBeVisible();
...
for (const viewportHeight of COMPOSER_VIEWPORT_HEIGHTS) {
    test(`bounds shared attachments and long text at ${viewportHeight}px @smoke`, async ({ page, }, testInfo) => {
        await page.setViewportSize({ width: 1280, height: viewportHeight });
        const scenario = createWebShellDaemonScenario({
        ...
        const images = page.locator('[data-web-shell-composer-attachments] img[src^="data:image/png;base64,"]');
        ...
        await expect
            .poll(() => attachments.evaluate((element) => element.scrollHeight > element.clientHeight + 1))
            .toBe(true);
        if (viewportHeight === 600) {
            ...
            const tooltip = portalRoot.locator('[data-web-shell-composer-tag-tooltip]');
            await expect(tooltip).toBeVisible();
            await expect
                .poll(async () => tooltip.evaluate((element) => {
        ...
        await expect
            ...
            if (!attachmentsBox || !imageBox)
                return false;
            const tolerance = 1;
            return (imageBox.y >= attachmentsBox.y - tolerance &&
...
test('lets a pasted image grow the composer without collapsing the text viewport @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    await gotoSession(page, scenario, daemon);
    const initialHeight = await composerHeight(page);
    await pasteComposerImages(page, 1);
    const image = page.locator('[data-web-shell-composer-surface] img[src^="data:image/png;base64,"]');
...
test('drops ordered PNG and BMP images and submits them without text @smoke', async ({ page, }, testInfo) => {
    const scenario = createWebShellDaemonScenario();
    const daemon = await installScenario(page, scenario, testInfo);
    await gotoSession(page, scenario, daemon);
    const surface = page.locator('[data-web-shell-composer-surface]');
    ...
    const images = surface.locator('[data-web-shell-composer-attachments] img[src^="data:image/"]');
    ...
    await submit.click();
    await expect.poll(() => daemon.promptRequests().length).toBe(1);
    const prompt = requestBodyRecord(firstRequest(daemon.promptRequests()))['prompt'];
    ...
    const transcriptImages = page.locator('[data-web-shell-message-list] img[src^="data:image/"]');
    await expect(transcriptImages).toHaveCount(2);
    await expectImagesDecoded(transcriptImages);
});
async function installScenario(page, scenario, testInfo) {
    return installMockDaemon(page, scenario, {
        baseURL: String(testInfo.project.use.baseURL),
    });
}
async function gotoSession(page, scenario, daemon) {
    await page.goto(`/session/${encodeURIComponent(scenario.sessionId)}`);
    await expect(page.locator('[data-web-shell-root]')).toBeVisible();
    await completeReplay(page, daemon, scenario.sessionId, scenario.events.length);
}
async function gotoComposerLayoutHarness(page, scenario, daemon) {
    await page.goto(`/e2e/composer-layout-harness.html?sessionId=${encodeURIComponent(scenario.sessionId)}`);
    await expect(page.locator('[data-web-shell-root]')).toBeVisible();
    await completeReplay(page, daemon, scenario.sessionId);
}
async function completeReplay(page, daemon, sessionId, replayedCount = 0) {
    const connection = await daemon.sse.waitForConnection(sessionId);
    await daemon.sendEvent(replayCompleteEvent({
...
async function pasteComposerText(page, text) {
...
async function pasteComposerImages(page, count) {
    const editor = page.locator('[data-web-shell-composer-editor] .cm-content');
    await editor.evaluate((element, imageCount) => {
        ...
        for (let index = 0; index < imageCount; index += 1) {
            clipboard.items.add(new File([pngBytes], `pasted-${index + 1}.png`, { type: 'image/png' }));
        }
...
async function dragComposerFileOver(surface) {
    await surface.evaluate((element) => {
        const transfer = new DataTransfer();
        transfer.items.add(new File([new Uint8Array([0])], 'dragged.png', { type: 'image/png' }));
...
async function dropComposerImages(surface) {
    await surface.evaluate((element) => {
        const pngBase64 = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=';
        const binary = atob(pngBase64);
        const pngBytes = Uint8Array.from(binary, (byte) => byte.charCodeAt(0));
        ...
        transfer.items.add(new File([bmpBytes], 'second.bmp', { type: 'image/bmp' }));
...
async function expectImagesDecoded(images) {
    await expect
        .poll(() => images.evaluateAll((elements) => elements.every((element) => element instanceof HTMLImageElement &&
        element.complete &&
        element.naturalWidth > 0 &&
        element.naturalHeight > 0)))
        .toBe(true);
}
async function expectCappedComposerLayout(page, viewportHeight) {
    ...
    await expect
        ...
        if (!surfaceBox || !toolbarBox)
            return false;
        return (toolbarBox.y >= surfaceBox.y - 1 &&
            toolbarBox.y + toolbarBox.height <= surfaceBox.y + surfaceBox.height + 1);
    })
        .toBe(true);
    await expect
        .poll(() => editorArea.evaluate((element) => getComputedStyle(element).overflowY))
        .toBe('clip');
    await expect
        .poll(() => editorHost.evaluate((element) => getComputedStyle(element).overflowY))
        .toBe('clip');
    await expect
        .poll(() => scroller.evaluate((element) => getComputedStyle(element).overflowY))
        .toBe('auto');
    await expect
        .poll(() => editorArea.evaluate((element) => element.scrollHeight <= element.clientHeight + 1))
        .toBe(true);
    await expect
        .poll(() => editorHost.evaluate((element) => element.scrollHeight <= element.clientHeight + 1))
        .toBe(true);
    await expect
...
async function composerHeight(page) {
    const box = await page
        .locator('[data-web-shell-composer-surface]')
        .boundingBox();
    if (!box)
        throw new Error('Expected the composer surface to be visible.');
    return box.height;
}
async function submitLocalCommand(page, text) {
    await fillComposer(page, text);
    await page.locator('[data-web-shell-composer-submit]').click();
}
async function openRawSseConnection(page, baseURL, sessionId) {
    await page.evaluate(async ({ baseURL, sessionId }) => {
        const response = await fetch(`${baseURL}/session/${encodeURIComponent(sessionId)}/events`);
        const holder = window;
        holder.__webShellRawSseResponses ??= [];
        holder.__webShellRawSseResponses.push(response);
    }, { baseURL, sessionId });
}
function firstRequest(requests) {
    const request = requests[0];
    if (!request)
        throw new Error('Expected a recorded daemon request.');
    return request;
}
function requestBodyRecord(request) {
    expect(typeof request.body).toBe('object');
    expect(request.body).not.toBeNull();
    expect(Array.isArray(request.body)).toBe(false);
    return request.body;
}
function expectPromptBodyToContainText(body, text) {
    const prompt = body['prompt'];
    expect(Array.isArray(prompt)).toBe(true);
    const blocks = prompt;
    expect(blocks.some((block) => isRecord(block) && block['type'] === 'text' && block['text'] === text)).toBe(true);
}
function isRecord(value) {
    return typeof value === 'object' && value !== null;
}
//# sourceMappingURL=web-shell.smoke.spec.js.map