...
for (const kind of ['bash', 'execute']) {
    test(`keeps collapsible ${kind} output inside the message layout @smoke`, async ({ page, }) => {
        ...
        expect(collapsed.outputScrollWidth).toBeGreaterThan(collapsed.outputClientWidth);
        ...
        expect(toggleBox.x + toggleBox.width).toBeLessThanOrEqual(messagesBox.x + messagesBox.width);
        await toggle.click();
        await expect(toggle).toHaveAttribute('aria-expanded', 'true');
        const expanded = await readOverflowMetrics(messages, output);
        expect(expanded.outputScrollWidth).toBeGreaterThan(expanded.outputClientWidth);
...
async function readOverflowMetrics(messages, output) {
...
//# sourceMappingURL=webui-tool-output-layout.spec.js.map