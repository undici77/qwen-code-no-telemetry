...
function voiceSetting(effective) {
    return {
        key: 'voiceModel',
        type: 'string',
        label: 'Voice model',
...
function sessionWorkflowSetting() {
    return {
...
const { mockCollectSystemInfo, mockConnection, mockSessionActions, mockWorkspace, mockWorkspaceActions, mockMcp, mockStore, mockFollowup, testState, rawEnqueuePrompt, queuedTexts, editLastQueuedPrompt, clearQueuedPrompts, editorClear, editorCommit, editorFocus, editorInsertText, editorRestoreInputAnnotations, settingsReload, settingsSetValue, qualifiedWorkspaceSettings, rootWorkspaceProviders, qualifiedWorkspaceProviders, qualifiedSetWorkspaceSetting, sessionCatalogController, } = vi.hoisted(() => {
    const connection = {
        status: 'connected',
        sessionId: 'session-1',
        clientId: 'client-1',
    ...
    const workspaceClient = {
        ...
        sessionStatus: vi.fn(() => Promise.resolve({ workspaceCwd: '/tmp/project' })),
    ...
    return {
        ...
        testState: {
            ...
            latestChatEditorProps: null,
            latestToastHostElevated: false,
            latestStatusBarTasks: null,
            latestStatusBarOnOpenTasks: null,
            latestMessageListProps: null,
            latestBtwMessageProps: null,
            latestAddWorkspaceDialogProps: null,
            latestToolApprovalKeyboardActive: null,
            toolApprovalKeyboardActiveHistory: [],
            latestToolApprovalPlanTodos: [],
            latestAskUserQuestionKeyboardActive: null,
            askUserKeyboardActiveHistory: [],
            latestTodoPanelTodos: [],
            latestTodoPanelOnOpen: null,
            latestAskUserQuestionOnError: null,
            latestBackgroundTasksRefreshTrigger: null,
            backgroundTasks: [],
            latestMonitorDetailsOnOpen: null,
            latestTasksStatusProps: null,
            settings: [],
            latestSettingsState: null,
            latestModelManagement: null,
            latestScheduledTasksProps: null,
            latestGoalsProps: null,
        },
...
vi.mock('@qwen-code/webui/daemon-react-sdk', () => {
    ...
    return {
        ...
        DaemonSessionProvider: ({ children, clientId, }) => (clientId?.startsWith('side-task:') ? null : children),
...
vi.mock('@qwen-code/sdk/daemon', () => ({
    ...
    isDaemonTurnError: (error) => typeof error === 'object' &&
        error !== null &&
...
vi.mock('./hooks/useBackgroundTasks', () => ({
    useBackgroundTasks: (_sessionId, _taskActivityKey, _connected, refreshTrigger = 0) => {
...
vi.mock('./hooks/useAnimationFrameValue', () => ({
    useAnimationFrameValue: (value) => value,
}));
...
vi.mock('./components/ChatEditor', async () => {
    ...
    return {
        ChatEditor: React.memo(React.forwardRef(function ChatEditor(props, ref) {
            ...
            React.useEffect(() => {
                onAttachmentsChange?.(Boolean(testState.promptImages?.length ||
            ...
            React.useImperativeHandle(ref, () => ({
                ...
                hasAttachments: () => Boolean(testState.promptImages?.length ||
                    testState.inputAnnotations?.length),
                ...
                submit: (input) => {
                    const accepted = props.onSubmit(input?.text ?? testState.prompt, testState.promptImages, editorCommit, testState.inputAnnotations
            ...
            return React.createElement('div', {
                ...
                'data-file-upload-enabled': customization.fileUploadEnabled === undefined
                    ? undefined
                ...
                type: 'button',
            }, 'submit'), props.newSessionSuggestion
                ? React.createElement('div', { 'data-testid': 'new-session-suggestion' }, React.createElement('button', {
...
vi.mock('./components/NewSessionDotField', () => ({
    NewSessionDotField: () => (_jsx("div", { "data-web-shell-new-session-dot-field": true, "aria-hidden": "true" })),
}));
vi.mock('./components/MessageList', async () => {
    ...
    return {
        MessageList: React.forwardRef(function MessageList(props, ref) {
            testState.latestMessageListProps = props;
            React.useImperativeHandle(ref, () => ({ scrollToBottom: vi.fn() }));
            return React.createElement('div', { 'data-testid': 'messages' }, props.welcomeHeader ?? null, React.createElement(InteractionBlockerProbe), props.showRetryHint
                ? React.createElement('button', {
                    'data-testid': 'retry',
                    onClick: props.onRetryClick,
                    type: 'button',
                }, 'retry')
                : null, props.failedPromptMessageId
                ? React.createElement('button', {
...
vi.mock('./components/messages/SettingsMessage', async () => {
    ...
    return {
        SettingsMessage: (props) => {
            ...
            return React.createElement('div', { 'data-testid': 'settings-message' }, React.createElement('button', {
...
vi.mock('./components/dialogs/ModelDialog', async () => {
    const React = await import('react');
    return {
        ModelDialog: (props) => React.createElement('button', {
...
vi.mock('./components/dialogs/GitDiffDialog', async () => {
    ...
    return {
        GitDiffDialog: () => React.createElement(DialogShell, null, 'changes dialog'),
...
vi.mock('./components/dialogs/DialogShell', async () => {
    const React = await import('react');
    return {
        DialogShell: (props) => React.createElement('div', {
...
vi.mock('./components/sidebar/WebShellSidebar', async () => {
    ...
    return {
        WebShellSidebar: (props) => {
            ...
            }, 'add workspace'), React.createElement('button', {
            ...
            }, 'load session'), React.createElement('button', {
                'data-testid': 'open-plugins',
                type: 'button',
                onClick: props.onOpenPlugins,
            }, 'plugins'), React.createElement('button', {
            ...
            }, 'daemon status'), React.createElement('button', {
                'data-testid': 'open-sessions-overview',
                type: 'button',
                onClick: props.onOpenSessions,
            }, 'sessions overview'), React.createElement('button', {
...
vi.mock('./session-catalog/session-catalog-store', async (importOriginal) => {
    ...
    return {
        ...actual,
        loadSessionCatalogOnce: async (client, query) => {
            ...
            if (client.listWorkspaceSessionsPage) {
                return client.listWorkspaceSessionsPage(query.workspaceCwd, query.options);
            }
            return {
                sessions: client.listWorkspaceSessions
                    ? await client.listWorkspaceSessions(query.workspaceCwd, query.options)
                    : [],
            };
        },
    };
...
function mockComponent(path, exportName) {
...
vi.doMock('./components/StreamingStatus', async () => {
    const React = await import('react');
    return {
        StreamingStatus: ({ startedAt }) => React.createElement('div', {
...
vi.doMock('./components/channels/ChannelsManagerPage', async () => {
    const React = await import('react');
    return {
        ChannelsManagerPage: () => React.createElement('div', { 'data-testid': 'channels-manager-page' }),
    };
});
vi.doMock('./components/SplitView', async () => {
    ...
    return {
        SplitView: (props) => {
            ...
            return React.createElement('div', { 'data-testid': 'split-view-mock' }, 
            // Surface the seed so a test can assert the App preserved / restored it.
            React.createElement('span', { 'data-testid': 'split-initial' }, (props.sessionIds ?? []).join(',')), React.createElement('span', { 'data-testid': 'split-voice-workspaces' }, props.voiceWorkspaces === undefined
                ? 'legacy'
            ...
            React.createElement('button', {
                ...
                type: 'button',
                onClick: () => props.onPaneArtifactsChange?.('pane-session', [artifact]),
            }, 'artifact'), React.createElement('button', {
                'data-testid': 'split-report-updated-artifact',
                type: 'button',
                onClick: () => props.onPaneArtifactsChange?.('pane-session', [updatedArtifact]),
            }, 'updated artifact'), React.createElement('button', {
                'data-testid': 'split-report-changed-artifact',
                type: 'button',
                onClick: () => props.onPaneArtifactsChange?.('pane-session', [changedArtifact]),
            }, 'changed artifact'), React.createElement('button', {
                'data-testid': 'split-clear-artifacts',
                type: 'button',
                onClick: () => props.onPaneArtifactsChange?.('pane-session', []),
            }, 'clear artifacts'), React.createElement('button', {
            ...
            }, 'open artifact'), React.createElement('button', {
            ...
            }, 'open main artifact'), React.createElement('button', {
            ...
            }, 'open scheduled task'), React.createElement('button', {
            ...
            }, 'open review'), React.createElement('button', {
                ...
                }, 'pane-session', mockSessionActions),
            }, 'open monitor'), React.createElement('button', {
                'data-testid': 'split-back',
                type: 'button',
                onClick: props.onExit,
            }, 'back'), React.createElement('span', { 'data-testid': 'split-has-header-actions' }, props.renderPaneHeaderActions ? 'yes' : 'no'));
        },
    };
});
...
vi.doMock('./components/extensions/ExtensionsManagerPage', async () => {
    const React = await import('react');
    return {
        ExtensionsManagerPage: (props) => React.createElement('div', { 'data-testid': 'extensions-manager-page' }, React.createElement('h1', {
            ref: props.initialFocusRef,
            tabIndex: -1,
            'data-testid': 'extensions-manager-heading',
        }, 'Manage extensions'), React.createElement('button', {
...
mockComponent('./components/dialogs/DeleteSessionDialog', 'DeleteSessionDialog');
mockComponent('./components/dialogs/ReleaseSessionDialog', 'ReleaseSessionDialog');
...
vi.doMock('./components/messages/ToolApproval', async () => {
    ...
    return {
        ToolApproval: (props) => {
            ...
            testState.toolApprovalKeyboardActiveHistory.push(props.keyboardActive ?? null);
...
vi.doMock('./components/messages/TasksStatusMessage', async () => {
    ...
    return {
        ...
        ShellTaskDetail: (props) => React.createElement('div', null, `${props.task.command} ${props.task.cwd}`),
    };
});
...
const { App, getTaskActivityKey, getEnvironmentAgentTasks, mergeMonitorTaskSnapshot, mergeSideTaskCatalog, } = await import('./App');
globalThis.IS_REACT_ACT_ENVIRONMENT = true;
const mounted = [];
describe('mergeSideTaskCatalog', () => {
    const listed = (sessionId) => ({ sessionId, title: sessionId });
    it('replaces the catalog when the parent session changes', () => {
        const next = mergeSideTaskCatalog({ parentSessionId: 'parent-a', items: [listed('stale')], loaded: true }, 'parent-b', [listed('b1')], new Set(['stale']));
        expect(next).toEqual({
    ...
    it('keeps a locally created draft the listing has not echoed yet', () => {
        const next = mergeSideTaskCatalog({
            ...
            loaded: true,
        }, 'parent-a', [listed('kept')], new Set(['draft']));
        expect(next.items.map((item) => item.sessionId)).toEqual(['kept', 'draft']);
    });
    it('does not duplicate a draft once the listing confirms it', () => {
        const next = mergeSideTaskCatalog({ parentSessionId: 'parent-a', items: [listed('draft')], loaded: true }, 'parent-a', [listed('draft')], new Set(['draft']));
...
describe('task activity key', () => {
    ...
    it('opens environment information for /tasks without a dialog', async () => {
        ...
        await flush();
        expect(container.querySelector('[data-testid="environment-panel"]:not([hidden])')).not.toBeNull();
    ...
    it('opens a monitor tool from the transcript in the right panel', async () => {
        ...
        expect(container.querySelector('button[title="watch server log"]')).not.toBeNull();
        expect(testState.latestBackgroundTasksRefreshTrigger).toBe(1);
        act(() => {
            container
    ...
    it('expands the right panel to fullscreen and shrinks it back from the toolbar or Escape', async () => {
        ...
        await flush();
        expect(container.querySelector('button[title="watch server log"]')).not.toBeNull();
        expect(document.querySelector('[class*="artifactPanelFullscreen"]')).toBeNull();
        const dockedAside = container.querySelector('aside[aria-label="Right panel"]');
        expect(dockedAside).not.toBeNull();
        const enterFullscreen = container.querySelector('button[aria-label="Fullscreen"]');
        expect(enterFullscreen).not.toBeNull();
        await act(async () => {
            enterFullscreen?.click();
            await Promise.resolve();
        });
        const fullscreenOverlay = document.querySelector('[class*="artifactPanelFullscreen"]');
        ...
        expect(fullscreenOverlay?.closest('[data-web-shell-portal-root]')).not.toBeNull();
        ...
        const exitFullscreenButton = fullscreenAside?.querySelector('button[aria-label="Exit fullscreen"]');
        ...
        expect(container.querySelector('aside[aria-label="Right panel"]')).toBe(dockedAside);
        ...
        expect(container.querySelector('[class*="artifactPanelDockNoOpenAnimation"]')).not.toBeNull();
        await act(async () => {
            enterFullscreen?.click();
            await Promise.resolve();
        });
        expect(document.querySelector('[class*="artifactPanelFullscreen"]')).not.toBeNull();
        await act(async () => {
            document
    ...
    it('merges a reopened monitor into its existing tab', async () => {
        ...
        await flush();
        let tabs = container.querySelectorAll('button[role="tab"]');
    ...
    it('updates an open monitor tab from background task polling', async () => {
        ...
        rerender();
        await flush();
        expect(container.querySelector('button[title="updated watch"]')).not.toBeNull();
    });
    ...
    it('ignores a monitor task response after switching sessions', async () => {
        let resolveTasks;
        mockSessionActions.getTasks.mockReturnValue(new Promise((resolve) => {
        ...
        expect(container.querySelector('button[title="watch old session"]')).toBeNull();
        expect(testState.latestBackgroundTasksRefreshTrigger).toBe(0);
    });
});
describe('artifact panel fullscreen', () => {
    it('drops fullscreen when the panel closes and reopens docked', async () => {
        ...
        await flush();
        expect(container.querySelector('aside[aria-label="Right panel"]')).not.toBeNull();
        await act(async () => {
            container
                .querySelector('button[aria-label="Fullscreen"]')
                ?.click();
            await Promise.resolve();
        });
        await flush();
        ...
        await act(async () => {
            document
                .querySelector('aside button[aria-label="Toggle right panel"]')
                ?.click();
            await Promise.resolve();
        });
        await flush();
        expect(container.querySelector('aside[aria-label="Right panel"]')).toBeNull();
        ...
        expect(container.querySelector('[class*="artifactPanelDockNoOpenAnimation"]')).toBeNull();
    });
    ...
    it('expands the floating drawer to fullscreen without remounting the panel', async () => {
        ...
        const drawerAside = document.querySelector(`${portal} aside[aria-label="Right panel"]`);
        expect(drawerAside).not.toBeNull();
        const drawerContent = document.querySelector(`${portal} [data-slot="drawer-content"]`);
        ...
        const fullscreenAside = document.querySelector(`${portal} aside[aria-label="Right panel"]`);
        expect(fullscreenAside).toBe(drawerAside);
        expect(fullscreenAside?.className).toContain('panelFullscreen');
        const fullscreenContent = document.querySelector(`${portal} [data-slot="drawer-content"]`);
        ...
        expect(fullscreenAside?.querySelector('button[aria-label="Exit fullscreen"]')).not.toBeNull();
        ...
        await act(async () => {
            drawerAside?.dispatchEvent(new KeyboardEvent('keydown', {
                key: 'Escape',
                bubbles: true,
                cancelable: true,
            }));
        ...
        await flush();
        const restoredAside = document.querySelector(`${portal} aside[aria-label="Right panel"]`);
        expect(restoredAside).toBe(drawerAside);
        expect(restoredAside?.className).not.toContain('panelFullscreen');
        const restoredContent = document.querySelector(`${portal} [data-slot="drawer-content"]`);
    ...
    it('leaves an IME-composition Escape to the composer in the fullscreen drawer', async () => {
        ...
        expect(document.querySelector(`${portal} aside[aria-label="Right panel"]`)).toBe(drawerAside);
        expect(drawerAside?.className).toContain('panelFullscreen');
    ...
    it('preserves the docked width across a fullscreen round-trip', async () => {
        ...
        expect(container
            ...
            ?.getAttribute('aria-valuenow')).toBe('400');
        ...
        await flush();
        const fullscreenAside = document.querySelector('[class*="artifactPanelFullscreen"] aside');
    ...
    it('shrinks fullscreen when a tool approval becomes pending', async () => {
        ...
        expect(container.querySelector('aside[aria-label="Right panel"]')).not.toBeNull();
        expect(container.querySelector('[role="separator"]')).not.toBeNull();
        const approvalOverlay = document.querySelector('[data-testid="approval-overlay"]');
    ...
    it('leaves a docked-fullscreen IME Escape to the window handler guard', async () => {
        ...
        const fullscreenSurface = document.querySelector('[class*="artifactPanelFullscreen"]');
    ...
    it('re-arms the dock open animation after a floating interlude', async () => {
        ...
        Object.defineProperty(window, 'matchMedia', {
            configurable: true,
            value: vi.fn().mockImplementation((query) => ({
                matches: query === dockQuery ? dockMatches : query.includes('min-width'),
                media: query,
                addEventListener: vi.fn((type, handler) => {
                    if (type === 'change' && query === dockQuery) {
                        dockChangeListeners.add(handler);
                    }
                }),
                removeEventListener: vi.fn((type, handler) => {
                    if (type === 'change' && query === dockQuery) {
                        dockChangeListeners.delete(handler);
                    }
                }),
            })),
        });
        ...
        expect(document.querySelector('[data-web-shell-portal-root] aside[aria-label="Right panel"]')).not.toBeNull();
        ...
        await flush();
        expect(container.querySelector('[class*="artifactPanelDock"]')).not.toBeNull();
        expect(container.querySelector('[class*="artifactPanelDockNoOpenAnimation"]')).toBeNull();
    });
    it('keeps the dock animation suppressed across a fullscreen floating interlude', async () => {
        ...
        await flush();
        expect(document.querySelector('[class*="artifactPanelFullscreen"]')).toBeNull();
        expect(document.querySelector('[data-web-shell-portal-root] aside[class*="panelFullscreen"]')).not.toBeNull();
    ...
    it('suppresses the dock animation when a floating fullscreen hands back to the dock', async () => {
        ...
        const drawerAside = () => document.querySelector('[data-web-shell-portal-root] aside[aria-label="Right panel"]');
        expect(drawerAside()).not.toBeNull();
        await act(async () => {
            drawerAside()
                ?.querySelector('button[aria-label="Fullscreen"]')
    ...
    it('does not dismiss the floating environment panel on pointerdown while fullscreen', async () => {
        ...
        vi.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockImplementation(function () {
            if (this.dataset['testid'] !== 'context-body')
                return new DOMRect();
        ...
        expect(container
            ...
            ?.getAttribute('data-floating')).toBe('true');
        act(() => {
            container
                .querySelector('button[aria-label="Fullscreen"]')
                ?.click();
    ...
    it('restores the app to AT when a locked floating fullscreen hands back to the dock', async () => {
        ...
        await act(async () => {
            document
                .querySelector('[data-web-shell-portal-root] aside[aria-label="Right panel"] button[aria-label="Fullscreen"]')
                ?.click();
            await Promise.resolve();
        });
        await flush();
    ...
    it('keeps an open modal focus trap active when the docked panel mounts', async () => {
        ...
        try {
            act(() => {
                trapRoot.render(_jsx(FocusScope, { trapped: true, children: _jsx("button", { type: "button", children: "inside dialog" }) }));
            });
            const trappedButton = trapContainer.querySelector('button');
            ...
            const outside = container.querySelector('[data-testid="submit"]');
    ...
    it('wraps Tab at the fullscreen surface edges', async () => {
        ...
        await flush();
        const surface = document.querySelector('[class*="artifactPanelFullscreen"]');
        expect(surface).not.toBeNull();
        const tabbables = Array.from(surface.querySelectorAll('button:not([disabled]), [tabindex]:not([tabindex="-1"])'));
    ...
    it('pulls focus back into the docked fullscreen surface when it escapes', async () => {
        ...
        const exitFullscreen = surface.querySelector('button[aria-label="Exit fullscreen"]');
    ...
    it('keeps the docked fullscreen surface keyboard-operable in shadow-DOM portal mode', async () => {
        ...
        await flush();
        const portalHost = document.querySelector('[data-web-shell-shadow-host="portals"]');
        const shadowRoot = portalHost?.shadowRoot;
        const surface = shadowRoot?.querySelector('[class*="artifactPanelFullscreen"]');
        expect(surface).not.toBeNull();
    ...
    it('shrinks fullscreen with one Escape while a forced session drawer is open', async () => {
        ...
        expect(container.querySelector('[data-sidebar-shell][role="dialog"]')?.className).toContain('mobileDrawerForced');
        await act(async () => {
            container
                .querySelector('button[aria-label="Toggle right panel"]')
                ?.click();
        ...
        expect(container.querySelector('[data-sidebar-shell][role="dialog"]')).not.toBeNull();
    });
    ...
    it('reveals the covered shells in the commit that closes the last fullscreen tab', async () => {
        ...
        await act(async () => {
            document
                .querySelector('aside button[aria-label="Close watch server log"]')
                ?.click();
            await Promise.resolve();
            expect(document.querySelector('[class*="artifactPanelFullscreen"]')).toBeNull();
            expect(container.querySelector('aside[aria-label="Right panel"]')).toBeNull();
    ...
    it('keeps the floating drawer Escape-to-close intact while not fullscreen', async () => {
        ...
        await flush();
        expect(document.querySelector(`${portal} aside[aria-label="Right panel"]`)).toBeNull();
    });
});
...
function renderApp(props = {}) {
    ...
    const doRender = (nextProps = props) => {
        act(() => {
            root.render(_jsx(App, { sidebar: { enabled: true }, header: {}, ...nextProps }));
        });
    };
    doRender(props);
...
async function triggerAutoRecap() {
    const recap = deferred();
...
beforeEach(() => {
    ...
    mockWorkspace.refreshCapabilities.mockResolvedValue(mockWorkspace.capabilities);
    ...
    mockWorkspace.client.createSideTaskSession.mockImplementation(() => new Promise(() => undefined));
    ...
    mockWorkspace.client.resolveSubagentSession.mockRejectedValue(new Error('Subagent details unavailable'));
...
describe('App compact mode', () => {
    ...
    it('uses Ctrl+O and persists the existing workspace setting', async () => {
        ...
        expect(settingsSetValue).toHaveBeenCalledWith('workspace', 'ui.compactMode', true);
        await toggleCompactMode();
        expect(settingsSetValue).toHaveBeenLastCalledWith('workspace', 'ui.compactMode', false);
    });
    it('restores compact mode from the workspace setting', async () => {
        ...
        renderApp();
        await toggleCompactMode();
        expect(settingsSetValue).toHaveBeenCalledWith('workspace', 'ui.compactMode', false);
    });
});
describe('App plan todos', () => {
    it('gates the exit-plan workflow on the experimental setting', async () => {
        ...
        rerender();
        await flush();
        expect(testState.latestToolApprovalPlanTodos.map((todo) => todo.id)).toEqual(['prepare', 'ship']);
    });
    ...
    it('opens the workflow dialog with plan todos and linked agents', async () => {
        ...
        expect(testState.latestTasksStatusProps?.planTodos?.map((todo) => todo.id)).toEqual(['work']);
        expect(testState.latestTasksStatusProps?.agentTools?.map((tool) => tool.callId)).toEqual(['agent-call']);
    });
    it('keeps the tasks dialog plain when Session Workflow is off', async () => {
        ...
        expect(container
            ...
            ?.getAttribute('data-dialog-title')).toBe('Background tasks');
    });
...
describe('App composer footer renderer', () => {
    it('passes composer state and keeps the header, composer, and footers ordered', async () => {
        const composerFooterProps = [];
        const ComposerFooter = (props) => {
            composerFooterProps.push(props);
            return _jsx("div", { "data-testid": "composer-footer", children: "composer footer" });
        };
        const { container } = renderApp({
            renderComposerHeader: () => (_jsx("div", { "data-testid": "composer-header", children: "composer header" })),
            renderComposerFooter: ComposerFooter,
            renderFooter: () => _jsx("div", { "data-testid": "shell-footer", children: "shell footer" }),
        });
        await flush();
        ...
        const composerHeader = container.querySelector('[data-testid="composer-header"]');
        const composerFooter = container.querySelector('[data-testid="composer-footer"]');
        ...
        expect(container.querySelector('[data-web-shell-new-session-dot-field]')).toBeNull();
    });
    it('updates composer footer state and renders it in the empty welcome state', async () => {
        const composerFooterProps = [];
        const ComposerFooter = (props) => {
            composerFooterProps.push(props);
            return _jsx("div", { "data-testid": "composer-footer" });
        };
        const { container, rerender } = renderApp({
            renderComposerFooter: ComposerFooter,
        });
        ...
        expect(container.querySelector('[data-web-shell-new-session-dot-field]')).not.toBeNull();
        expect(composerFooterProps.at(-1)).toEqual({
            disabled: false,
            isRunning: false,
            currentMode: 'plan',
    ...
    it('does not add composer footer DOM when omitted or when the renderer returns null', async () => {
        ...
        await flush();
        const composer = container.querySelector('[data-web-shell-composer]');
        const composerChildren = Array.from(composer?.parentElement?.children ?? []);
        ...
        const nullComposerChildren = Array.from(nullComposer?.parentElement?.children ?? []);
        expect(nullComposer?.nextElementSibling).toBeNull();
        expect(nullComposerChildren).toHaveLength(composerChildren.length);
        expect(nullComposerChildren.map((child) => child.getAttribute('data-web-shell-composer'))).toEqual(composerChildren.map((child) => child.getAttribute('data-web-shell-composer')));
    });
});
describe('App shell command queueing', () => {
    it('lazily creates a session for ! shell commands in a new task', async () => {
        ...
        let accepted;
        await act(async () => {
            accepted = testState.latestChatEditorProps?.onSubmit('!echo hi', undefined, editorCommit);
            await vi.waitFor(() => {
    ...
    it('runs the ! command in a new task even when the session-id render commits before attach resolves', async () => {
        ...
        let releaseAttach;
        const attachGate = new Promise((r) => {
            releaseAttach = r;
        });
        ...
        await flush();
        await act(async () => {
            testState.latestChatEditorProps?.onSubmit('!echo hi', undefined, editorCommit);
            await Promise.resolve();
        });
    ...
    it('returns true and clears editor for ! commands with an existing session', async () => {
        ...
        expect(sessionCatalogController.invalidateWorkspace).toHaveBeenCalledWith('/tmp/project');
    });
    it('blocks duplicate ! submission while session creation is in flight', async () => {
        mockConnection.sessionId = undefined;
        let resolveCreate;
        const createDone = new Promise((r) => {
            resolveCreate = r;
        });
        ...
        expect(mockSessionActions.sendShellCommand).toHaveBeenCalledWith('git push');
    });
    it('releases isPreparing after session creation, not after command completion', async () => {
        ...
        expect(mockSessionActions.sendShellCommand).toHaveBeenCalledWith('npm run build');
    ...
    it('reports an error and skips the command when session creation fails', async () => {
        mockConnection.sessionId = undefined;
        mockSessionActions.createSession.mockRejectedValueOnce(new Error('no session'));
        const onToast = vi.fn();
        renderApp({ onToast });
        await flush();
        let accepted;
    ...
    it('queues a ! command during a turn and drains it when the turn ends', async () => {
        ...
        await act(async () => {
            await vi.waitFor(() => {
                expect(mockSessionActions.sendShellCommand).toHaveBeenCalledWith('echo queued');
            });
        });
    });
    ...
    it('drops the whole queue when the drain bails, preserving FIFO integrity', async () => {
        ...
        expect(onToast).toHaveBeenCalledWith('warning', '2 queued shell commands will not run.');
    ...
    it('drains multiple queued commands in FIFO order', async () => {
        ...
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(1, 'aaa');
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(2, 'bbb');
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(3, 'ccc');
    });
    it('keeps draining when streamingState changes between commands', async () => {
        ...
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(1, 'first');
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(2, 'second');
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(3, 'third');
    });
    it('does not drop queued commands when a new command is queued mid-drain', async () => {
        ...
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(2, 'bbb');
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(3, 'ccc');
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(4, 'ddd');
    });
    it('continues draining after a command fails', async () => {
        ...
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(1, 'fail');
        expect(mockSessionActions.sendShellCommand).toHaveBeenNthCalledWith(2, 'ok');
        expect(onToast).toHaveBeenCalledWith('error', expect.any(String));
    });
    ...
    it('does not drop queued commands when the UI language changes', async () => {
        ...
        await flush();
        expect(mockSessionActions.sendShellCommand).not.toHaveBeenCalled();
        const warningCalls = onToast.mock.calls.filter((c) => c[0] === 'warning');
        ...
        await act(async () => {
            await vi.waitFor(() => {
                expect(mockSessionActions.sendShellCommand).toHaveBeenCalledWith('deploy prod');
            });
        });
    });
    ...
    it('allows new shell commands after cancel during session creation', async () => {
        ...
        expect(mockSessionActions.sendShellCommand).toHaveBeenCalledWith('git status');
    });
    it('runs a retry submitted while session creation is still in flight after cancel', async () => {
        ...
        mockSessionActions.createSession.mockImplementation(() => gate.then(() => {
            mockConnection.sessionId = 'session-1';
            return { sessionId: 'session-1' };
        }));
        renderApp({});
    ...
    it('reports an error when sendShellCommand rejects with an existing session', async () => {
        mockSessionActions.sendShellCommand.mockRejectedValueOnce(new Error('daemon rejected'));
        ...
        expect(accepted).toBe(true);
        expect(consoleError).toHaveBeenCalledWith('[web-shell]', 'daemon rejected', expect.anything());
...
describe('App read-only local commands mid-turn', () => {
    ...
    it('reports /stats load failures instead of swallowing them', async () => {
        ...
        await act(async () => {
            ...
            await vi.waitFor(() => {
                expect(consoleError).toHaveBeenCalledWith('[web-shell]', expect.stringContaining('stats unavailable'), expect.anything());
            });
        });
        consoleError.mockRestore();
    });
    it('reports /about load failures instead of swallowing them', async () => {
        ...
        await act(async () => {
            testState.latestChatEditorProps?.onSubmit('/about');
            await vi.waitFor(() => {
                expect(consoleError).toHaveBeenCalledWith('[web-shell]', expect.stringContaining('status unavailable'), expect.anything());
            });
        });
        consoleError.mockRestore();
    });
...
describe('App session callbacks', () => {
    ...
    it('keeps primary Voice model reads and writes on legacy routes', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="model-select"]')
                ?.click();
            await Promise.resolve();
        });
        expect(settingsSetValue).toHaveBeenCalledWith('workspace', 'voiceModel', 'fast-model-x');
    ...
    it('uses qualified providers and workspace settings for secondary Voice models', async () => {
        ...
        const select = container.querySelector('[data-testid="model-select"]');
        expect(select).not.toBeNull();
        await act(async () => {
            select?.click();
            await Promise.resolve();
        });
        expect(qualifiedSetWorkspaceSetting).toHaveBeenCalledWith('workspace', 'voiceModel', 'fast-model-x');
    ...
    it('never exposes the primary Voice setting while secondary settings load', async () => {
        ...
        let resolveSettings = () => undefined;
        qualifiedWorkspaceSettings.mockReturnValue(new Promise((resolve) => {
        ...
        await clickSubmit(container);
        await flush();
        expect(testState.latestSettingsState?.settings.find((setting) => setting.key === 'voiceModel')).toBeUndefined();
        await act(async () => {
            resolveSettings({
                v: 1,
        ...
        await flush();
        expect(testState.latestSettingsState?.settings.find((setting) => setting.key === 'voiceModel')?.values.effective).toBe('secondary-voice');
    });
    ...
    it('closes an open Voice picker when its capability gate is lost', async () => {
        ...
        expect(container.querySelector('[data-testid="model-select"]')).not.toBeNull();
        mockWorkspace.capabilities = {
            ...mockWorkspace.capabilities,
    ...
    it('waits for session loading to finish before requesting status', async () => {
        ...
        rerender();
        await vi.waitFor(() => {
            expect(mockWorkspace.client.sessionStatus).toHaveBeenCalledWith('session-1');
        });
    });
    it('uses the session catalog title when the connection has no display name', async () => {
        ...
        const { container } = renderApp();
        await vi.waitFor(() => {
            expect(container.querySelector('[data-testid="chat-context-header"]')
                ?.textContent).toContain('Real session title');
        });
    });
    it('does not expose cached metadata after a same-id workspace switch', async () => {
        mockConnection.displayName = undefined;
        const sourceStatus = deferred();
        mockWorkspace.client.sessionStatus.mockReturnValueOnce(sourceStatus.promise);
        const sourceList = deferred();
        mockWorkspace.client.listWorkspaceSessions.mockReturnValueOnce(sourceList.promise);
        const targetStatus = deferred();
        mockWorkspace.client.sessionStatus.mockReturnValueOnce(targetStatus.promise);
        const { container, rerender } = renderApp();
        await act(async () => {
        ...
        await vi.waitFor(() => {
            expect(container.querySelector('[data-testid="chat-context-header"]')
                ?.textContent).toContain('Session A title');
        });
        ...
        rerender();
        expect(container.querySelector('[data-testid="chat-context-header"]')
            ?.textContent).not.toContain('Session A title');
        await act(async () => {
    ...
    it('keeps the persistent chat header opt-in for existing integrations', () => {
        ...
        expect(container.querySelector('[data-testid="chat-context-header"]')).toBeNull();
    });
    it('lets a custom renderer replace the complete persistent chat header', () => {
        ...
        const renderChatHeader = vi.fn(() => (_jsx("header", { "data-testid": "custom-chat-header", children: "Custom session header" })));
        const { container } = renderApp({
            header: undefined,
            renderChatHeader,
        });
        expect(container.querySelector('[data-testid="chat-context-header"]')).toBeNull();
        expect(container.querySelector('[data-testid="custom-chat-header"]')
            ?.textContent).toContain('Custom session header');
        expect(renderChatHeader).toHaveBeenCalledWith(expect.objectContaining({
            sessionId: 'session-1',
            sessionName: 'Session One',
            workspaceCwd: '/tmp/project',
        ...
        expect(testState.latestChatEditorProps?.visibleToolbarActions).toContain('gitBranch');
    });
    it('wires the composer context ring from the connection and opens /context on click', async () => {
        const usageConnection = mockConnection;
        usageConnection.tokenCount = 338;
        usageConnection.contextWindow = 1000;
        renderApp();
        await flush();
        expect(testState.latestChatEditorProps?.visibleToolbarActions).toContain('contextUsage');
    ...
    it('keeps legacy task status for a custom header without explicit header configuration', () => {
        ...
        renderApp({
            header: undefined,
            renderChatHeader: () => _jsx("header", { children: "Custom session header" }),
        });
    ...
    it('controls the built-in chat header actions through header items', () => {
        ...
        expect(container.querySelector('button[aria-label="Toggle environment information"]')).not.toBeNull();
        expect(container.querySelector('button[aria-label="Toggle right panel"]')).toBeNull();
        expect(container.querySelector('[data-testid="chat-context-header"]')
            ?.textContent).not.toContain('Session One');
    });
    ...
    it('opens environment information without restoring composer Git information', () => {
        ...
        const { container } = renderApp();
        const rightPanelButton = container.querySelector('button[aria-label="Toggle right panel"]');
        ...
        expect(container.querySelector('[data-testid="environment-panel"]:not([hidden])')).not.toBeNull();
        expect(testState.latestChatEditorProps?.visibleToolbarActions).not.toContain('gitBranch');
    });
    it('keeps the right-panel action visible and opens a review-only empty state', () => {
        ...
        const emptyActions = container.querySelector('[data-testid="right-panel-empty-actions"]');
        const actions = Array.from(emptyActions?.querySelectorAll('button') ?? []);
        expect(actions).toHaveLength(1);
        expect(actions[0]?.textContent).toContain('Changes');
        expect(actions[0]?.disabled).toBe(true);
        expect(container.querySelector('button[aria-label="Add panel"]')).toBeNull();
        const header = container.querySelector('[data-testid="chat-context-header"]');
        expect(header?.querySelector('button[aria-label="Toggle right panel"]')).toBeNull();
        expect(container
            .querySelector('aside[aria-label="Right panel"]')
            ?.querySelector('button[aria-label="Toggle right panel"]')).not.toBeNull();
        const artifactDock = container.querySelector('[role="separator"]')?.parentElement;
        // The dock renders through a display:contents slot, so the .appShell flex
        // parent sits one level above the wrapper itself.
        expect(artifactDock?.parentElement?.parentElement).toBe(header?.parentElement?.parentElement?.parentElement);
    ...
    it('opens the latest reviewable turn from the empty right panel', () => {
        ...
        const changes = Array.from(container.querySelectorAll('[data-testid="right-panel-empty-actions"] button')).find((button) => button.textContent?.startsWith('Changes'));
    ...
    it('floats environment information in ultrawide mode', () => {
        ...
        const { container } = renderApp();
        const environmentButton = container.querySelector('button[aria-label="Toggle environment information"]');
        act(() => {
            testState.latestChatEditorProps?.onChatWidthModeChange?.('wide');
            environmentButton?.click();
        });
        const environmentPanel = container.querySelector('[data-testid="environment-panel"]:not([hidden])');
        expect(environmentPanel?.getAttribute('data-floating')).toBe('true');
        expect(environmentPanel?.parentElement?.contains(container.querySelector('[data-testid="chat-pane-container"]'))).toBe(true);
    });
    it('closes environment information at the dock breakpoint and reopens it floating', async () => {
        ...
        expect(container.querySelector('[data-testid="environment-panel"]:not([hidden])')).toBeNull();
        act(() => {
            container
                .querySelector('button[aria-label="Toggle environment information"]')
                ?.click();
    ...
    it('opens an environment monitor in the right panel', () => {
        ...
        const backgroundTasksButton = Array.from(container.querySelectorAll('button[aria-expanded]')).find((button) => button.textContent?.includes('Background tasks'));
        act(() => backgroundTasksButton?.click());
        const monitorButton = Array.from(container.querySelectorAll('[data-testid="environment-panel"] ul button')).find((button) => button.textContent?.includes('watch server log'));
    ...
    it('opens an environment shell task in the right panel', () => {
        ...
        const backgroundTasksButton = Array.from(container.querySelectorAll('button[aria-expanded]')).find((button) => button.textContent?.includes('Background tasks'));
        act(() => backgroundTasksButton?.click());
        const shellButton = Array.from(container.querySelectorAll('[data-testid="environment-panel"] ul button')).find((button) => button.textContent?.includes('npm run dev'));
        act(() => shellButton?.click());
        expect(container.querySelector('[data-testid="environment-panel"]:not([hidden])')).not.toBeNull();
        expect(container.querySelector('button[title="npm run dev"]')).not.toBeNull();
    ...
    it('keeps environment information open with its subagent panel', async () => {
        ...
        const subagentsButton = Array.from(container.querySelectorAll('button[aria-expanded]')).find((button) => button.textContent?.includes('Subagents'));
        act(() => subagentsButton?.click());
        const environmentButton = container.querySelector('button[aria-label="Toggle environment information"]');
        act(() => environmentButton?.click());
        expect(container.querySelector('[data-testid="environment-panel"]:not([hidden])')).toBeNull();
        act(() => environmentButton?.click());
        expect(Array.from(container.querySelectorAll('[data-testid="environment-panel"]:not([hidden]) button[aria-expanded="true"]')).some((button) => button.textContent?.includes('Subagents'))).toBe(true);
        const agentButton = Array.from(container.querySelectorAll('[data-testid="environment-panel"]:not([hidden]) ul button')).find((button) => button.textContent?.includes('Inspect repository'));
        ...
        expect(container.querySelector('[data-testid="environment-panel"]:not([hidden])')).toBeNull();
        const environmentToggle = container.querySelector('button[aria-label="Toggle environment information"]');
    ...
    it('loads an out-of-band fork after the right panel opens', async () => {
        ...
        const subagentsButton = Array.from(container.querySelectorAll('button[aria-expanded]')).find((button) => button.textContent?.includes('Subagents'));
        act(() => subagentsButton?.click());
        const forkButton = Array.from(container.querySelectorAll('[data-testid="environment-panel"] ul button')).find((button) => button.textContent?.includes('Review current changes'));
        ...
        expect(container.querySelector('[data-testid="environment-panel"]:not([hidden])')).not.toBeNull();
        expect(container.querySelector('button[title="Agent: Review current changes"]')).not.toBeNull();
        const dock = container.querySelector('[class*="artifactPanelDock"]');
        ...
        expect(mockWorkspace.client.resolveSubagentSession).toHaveBeenCalledExactlyOnceWith('session-1', 'fork-agent-1');
    });
    it('loads an out-of-band fork after the floating drawer opens with reduced motion', async () => {
        ...
        const dispatchAnimationEnd = (target, animationName) => {
        ...
        await act(async () => {
            ...
            if (panel)
                dispatchAnimationEnd(panel, 'child-animation');
            await Promise.resolve();
        });
        expect(mockWorkspace.client.resolveSubagentSession).not.toHaveBeenCalled();
        await act(async () => {
            if (drawer)
                dispatchAnimationEnd(drawer, 'slideFromRight');
            await Promise.resolve();
        });
        expect(mockWorkspace.client.resolveSubagentSession).toHaveBeenCalledExactlyOnceWith('session-1', 'fork-agent-1');
    });
    it('updates the header when session metadata supplies a generated title', () => {
        ...
        const { container, rerender } = renderApp();
        expect(container.querySelector('[data-testid="chat-context-header"]')
            ?.textContent).toContain('New session');
        mockConnection.displayName = 'Investigate task failures';
        rerender();
        expect(container.querySelector('[data-testid="chat-context-header"]')
            ?.textContent).toContain('Investigate task failures');
    });
    it('refreshes the generated title after the first turn completes', async () => {
        ...
        expect(container.querySelector('[data-testid="chat-context-header"]')
            ?.textContent).not.toContain('Generated session title');
        await act(async () => {
            await vi.advanceTimersByTimeAsync(2000);
        });
        expect(container.querySelector('[data-testid="chat-context-header"]')
            ?.textContent).toContain('Generated session title');
    });
    it('defers title refresh while the page is hidden', async () => {
        ...
        expect(mockWorkspace.client.listWorkspaceSessions).not.toHaveBeenCalled();
        expect(container.querySelector('[data-testid="chat-context-header"]')
            ?.textContent).not.toContain('Visible session title');
        Object.defineProperty(document, 'hidden', {
            configurable: true,
            value: false,
        });
        ...
        expect(container.querySelector('[data-testid="chat-context-header"]')
            ?.textContent).toContain('Visible session title');
    });
    it('submits through a disconnected session when prompt SSE restart is enabled', async () => {
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('recover connection', expect.objectContaining({ images: undefined }));
    });
    it('reports the current workspace id and path', async () => {
        ...
        await flush();
        expect(onSessionIdChange).toHaveBeenCalledWith('session-1', 'secondary', '/work/secondary');
    });
    it('reports the selected workspace, not the stale connection workspace, when no session is active', async () => {
        ...
        await flush();
        expect(onSessionIdChange).toHaveBeenCalledWith(undefined, undefined, '/workspace');
        expect(onSessionIdChange).not.toHaveBeenCalledWith(undefined, 'secondary', '/work/secondary');
    });
    ...
    it('keeps reporting the active session workspace despite a conflicting next-session selection', async () => {
        ...
        await flush();
        expect(onSessionIdChange).toHaveBeenCalledWith('session-1', 'secondary', '/work/secondary');
        expect(onSessionIdChange).not.toHaveBeenCalledWith('session-1', undefined, '/workspace');
    });
    it('notifies the host on each deferred workspace switch while no session is active', async () => {
        ...
        renderApp({ onSessionIdChange });
        await flush();
        expect(onSessionIdChange).toHaveBeenLastCalledWith(undefined, undefined, '/workspace');
        act(() => {
            testState.latestChatEditorProps?.onSelectWorkspace?.('/work/tertiary');
        });
        await flush();
        expect(onSessionIdChange).toHaveBeenLastCalledWith(undefined, 'tertiary', '/work/tertiary');
    });
    it('creates scratch once, accepts refreshed capabilities, and opens a fresh chat', async () => {
        ...
        let resolveScratch;
        mockWorkspaceActions.addScratchWorkspace.mockReturnValue(new Promise((resolve) => {
        ...
        expect(mockWorkspace.client.workspaceByCwd).toHaveBeenCalledWith('/managed/scratch-Ab3');
    });
    it('opens one App-owned Add workspace dialog from both entry points', async () => {
        ...
        expect(container.querySelectorAll('[data-testid="add-workspace-dialog"]')).toHaveLength(1);
        ...
        expect(testState.latestAddWorkspaceDialogProps?.onSuggest).toBe(mockWorkspaceActions.suggestWorkspacePaths);
        rerender();
        expect(testState.latestAddWorkspaceDialogProps?.onSuggest).toBe(mockWorkspaceActions.suggestWorkspacePaths);
        ...
        await expect(testState.latestAddWorkspaceDialogProps?.onPick?.()).resolves.toBe('/tmp/selected');
        act(() => {
            testState.latestAddWorkspaceDialogProps?.onClose();
        });
        expect(container.querySelector('[data-testid="add-workspace-dialog"]')).toBeNull();
        act(() => {
            container
    ...
    it('forwards a supported workspace display name through the shared mutation lane', async () => {
        ...
        await act(async () => {
            await testState.latestAddWorkspaceDialogProps?.onAdd('/tmp/payments', true, 'Payments API');
        });
        expect(mockWorkspaceActions.addWorkspace).toHaveBeenCalledWith('/tmp/payments', { persist: true, displayName: 'Payments API' });
        expect(mockWorkspace.refreshCapabilities).toHaveBeenCalledOnce();
    });
    it('omits unsupported persistence and display-name options', async () => {
        ...
        await act(async () => {
            await testState.latestAddWorkspaceDialogProps?.onAdd('/tmp/local', true, 'Ignored name');
        });
        expect(mockWorkspaceActions.addWorkspace).toHaveBeenCalledWith('/tmp/local', { persist: false });
    });
    it('rejects when the daemon does not confirm persistent registration', async () => {
        ...
        await expect(testState.latestAddWorkspaceDialogProps?.onAdd('/tmp/payments', true)).rejects.toThrow('The daemon did not confirm persistent workspace registration');
    });
    it('surfaces an inline error when an added folder cannot refresh capabilities', async () => {
        ...
        mockWorkspaceActions.addWorkspace.mockResolvedValue(added);
        mockWorkspace.refreshCapabilities.mockRejectedValueOnce(new Error('refresh failed'));
        renderApp();
        await flush();
        act(() => {
            testState.latestChatEditorProps?.onOpenExistingWorkspace?.();
        });
        await expect(testState.latestAddWorkspaceDialogProps?.onAdd('/tmp/payments', false)).rejects.toThrow('Workspace added, but the workspace list could not be refreshed');
    ...
    it('retries only capability refresh after a committed scratch cannot reconcile', async () => {
        ...
        expect(mockWorkspaceActions.addScratchWorkspace).toHaveBeenCalledOnce();
        const refreshButton = Array.from(document.querySelectorAll('button')).find((button) => button.textContent === 'Refresh workspace list');
        await act(async () => {
            refreshButton?.click();
            await vi.waitFor(() => {
    ...
    it('locks scratch creation after an unknown outcome until acknowledged', async () => {
        ...
        mockWorkspaceActions.addScratchWorkspace.mockRejectedValue(new Error('Add scratch workspace timed out'));
        mockWorkspace.refreshCapabilities.mockResolvedValue(mockWorkspace.capabilities);
        renderApp();
        await flush();
        await act(async () => {
        ...
        const ackButton = Array.from(document.querySelectorAll('button')).find((button) => button.textContent === 'I checked the workspace list');
        ...
        await act(async () => {
            ackButton?.click();
            await vi.waitFor(() => {
                expect(document.body.textContent).not.toContain('I checked the workspace list');
            });
        });
        await act(async () => {
            testState.latestChatEditorProps?.onCreateScratchWorkspace?.();
            await vi.waitFor(() => {
                expect(mockWorkspaceActions.addScratchWorkspace).toHaveBeenCalledTimes(2);
            });
        });
    });
    it('reports a definitive 4xx rejection without locking scratch creation', async () => {
        ...
        const { DaemonHttpError: MockDaemonHttpError } = await import('@qwen-code/sdk/daemon');
        mockWorkspaceActions.addScratchWorkspace.mockRejectedValue(new MockDaemonHttpError(403, {}, 'Forbidden'));
        const consoleError = vi
            .spyOn(console, 'error')
            .mockImplementation(() => { });
        renderApp();
        ...
        await flush();
        expect(consoleError).toHaveBeenCalledWith('[web-shell]', expect.stringContaining('Forbidden'), expect.anything());
        expect(document.body.textContent).not.toContain('I checked the workspace list');
        mockWorkspaceActions.addScratchWorkspace.mockResolvedValue({
            id: 'scratch-3',
    ...
    it('falls back to primary when the draft workspace becomes untrusted', async () => {
        ...
        expect(testState.latestChatEditorProps?.selectedWorkspaceCwd).toBe('/work/secondary');
        mockWorkspace.capabilities = {
            workspaces: [
                {
                    id: 'primary',
        ...
        view.rerender();
        await flush();
        expect(testState.latestChatEditorProps?.selectedWorkspaceCwd).toBeUndefined();
        await act(async () => {
        ...
        expect(mockSessionActions.createSession).toHaveBeenCalledWith(expect.objectContaining({ workspaceCwd: '/tmp/project' }));
        const promptOptions = mockSessionActions.sendPrompt.mock.calls.at(-1)?.[1];
        act(() => promptOptions?.onAdmitted?.());
        expect(sessionCatalogController.promptAdmitted).toHaveBeenCalledWith('/tmp/project', expect.any(String));
        expect(sessionCatalogController.promptAdmitted).not.toHaveBeenCalledWith('/work/secondary', expect.any(String));
    });
    ...
    it('keeps composer workspace props stable across an equivalent list refresh', async () => {
        ...
        const changedProps = Object.keys(previousProps ?? {}).filter((key) => !VOICE_OWNED_PROPS.has(key) &&
            previousProps[key] !==
                testState.latestChatEditorProps[key]);
        expect(changedProps).toEqual([]);
        expect(testState.latestChatEditorProps?.workspaces).toBe(previousProps?.workspaces);
        expect(testState.latestChatEditorProps?.onSelectWorkspace).toBe(previousProps?.onSelectWorkspace);
        expect(testState.latestChatEditorProps?.onCreateScratchWorkspace).toBe(previousProps?.onCreateScratchWorkspace);
        expect(testState.latestChatEditorProps?.onOpenExistingWorkspace).toBe(previousProps?.onOpenExistingWorkspace);
    });
    it('labels the Live composer workspace without exposing its backing name', async () => {
        ...
        renderApp();
        await flush();
        expect(testState.latestChatEditorProps?.workspaces?.find((entry) => entry.id === 'live')).toMatchObject({ label: 'Live' });
        expect(testState.latestChatEditorProps?.workspaces?.some((entry) => entry.label === 'Conversations')).toBe(false);
    });
    it('keeps composer git status stable across an equivalent refresh', async () => {
        ...
        expect(testState.latestChatEditorProps?.gitStatus).toBe(previousProps?.gitStatus);
        expect(testState.latestChatEditorProps?.onOpenGitDiff).toBe(previousProps?.onOpenGitDiff);
    });
    it('creates new sessions in the locked workspace without a selector', async () => {
        ...
        expect(testState.latestChatEditorProps?.atWorkspaceCwd).toBe('/work/secondary');
        await act(async () => {
        ...
        expect(mockSessionActions.createSession).toHaveBeenCalledWith(expect.objectContaining({ workspaceCwd: '/work/secondary' }));
    });
    ...
    it('clears the git mode intent when starting a new session from the sidebar', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="new-session"]')
                ?.click();
            await Promise.resolve();
        });
        ...
        const arg = mockSessionActions.createSession.mock.calls[0]?.[0];
    ...
    it('hides the git mode chip when the workspace is not trusted', async () => {
        ...
        expect(testState.latestChatEditorProps?.onGitModeIntentChange).toBeUndefined();
    });
    ...
    it('skips the wait:true fresh path for worktree sessions', async () => {
        ...
        const cwdIndex = calls.findIndex((arg) => arg?.cwd === worktreePath);
        const waitAfter = calls
            .slice(cwdIndex + 1)
            .filter((arg) => arg?.wait === true);
    ...
    it('forwards the branch intent to createSession when submitting a prompt', async () => {
        ...
        expect(mockSessionActions.createSession).toHaveBeenCalledWith(expect.objectContaining({ branch: { name: 'feat/test' } }));
    });
    it('forwards the worktree intent to createSession when submitting a prompt', async () => {
        ...
        expect(mockSessionActions.createSession).toHaveBeenCalledWith(expect.objectContaining({ worktree: { slug: 'feat-a' } }));
    });
    it('reloads skills from the target workspace when starting a new session', async () => {
        ...
        expect(mockWorkspace.client.workspaceByCwd).toHaveBeenCalledWith('/work/secondary');
    });
    it('uses a registered capability fallback while the workspace list is stale', async () => {
        ...
        expect(testState.latestScheduledTasksProps?.lockedWorkspace).toEqual(lockedWorkspaceCapability);
    });
    it('uses configured composer placeholders by state and falls back for blank values', async () => {
        ...
        await flush();
        expect(testState.latestChatEditorProps?.placeholderText).toBe('Ask a question');
        testState.streamingState = 'responding';
        rerender({ composerPlaceholders });
        await flush();
        expect(testState.latestChatEditorProps?.placeholderText).toBe('Working on it');
        rerender({ composerPlaceholders: { idle: 'Ask a question' } });
        await flush();
        expect(testState.latestChatEditorProps?.placeholderText).toBe('Processing. New messages will be queued.');
        mockConnection.catchingUp = true;
        rerender({ composerPlaceholders });
        await flush();
        expect(testState.latestChatEditorProps?.placeholderText).toBe('Preparing chat');
        mockConnection.catchingUp = false;
        testState.streamingState = 'idle';
        rerender({ composerPlaceholders: { idle: '   ' } });
        await flush();
        expect(testState.latestChatEditorProps?.placeholderText).toBe('Type a message or @ file path');
    });
    ...
    it('reloads skills when starting a new session', async () => {
        ...
        expect(testState.latestChatEditorProps?.commands).toEqual(expect.arrayContaining([expect.objectContaining({ name: 'review' })]));
        mockWorkspaceActions.loadSkillsStatus.mockResolvedValue({
        ...
        expect(testState.latestChatEditorProps?.skills).toEqual([]);
        expect(testState.latestChatEditorProps?.commands).not.toEqual(expect.arrayContaining([expect.objectContaining({ name: 'review' })]));
    ...
    it('adds an enabled skill command when starting a new session', async () => {
        ...
        expect(testState.latestChatEditorProps?.commands).toEqual(expect.arrayContaining([
            expect.objectContaining({
                name: 'review',
                argumentHint: '<path>',
                source: 'skill',
    ...
    it.each([404, 410])('shows a missing-session empty state with a new-session action for %d', async (status) => {
        mockConnection.status = 'disconnected';
        mockConnection.sessionId = undefined;
    ...
    it('keeps an automatic recap when an existing-session switch fails', async () => {
        const { recap } = await triggerAutoRecap();
        mockSessionActions.loadSession.mockRejectedValueOnce(new Error('target restore failed'));
        await act(async () => {
            window.dispatchEvent(new CustomEvent('qwen:open-session', { detail: 'session-2' }));
            await Promise.resolve();
        });
        await act(async () => {
            recap.resolve({ sessionId: 'session-1', recap: 'Current session recap' });
    ...
    it('does not finish a same-id workspace switch before commit', async () => {
        ...
        editorFocus.mockClear();
        await act(async () => {
            window.dispatchEvent(new CustomEvent('qwen:open-session', {
        ...
        expect(testState.latestChatEditorProps?.disabled).toBe(true);
        expect(testState.latestChatEditorProps?.onSubmit('must stay on A')).toBe(false);
    ...
    it('opens a Live session in its owning Conversations workspace', async () => {
        ...
        expect(mockSessionActions.loadSession).toHaveBeenCalledWith('live-coordinator', {
            workspaceCwd: '/Users/test/Documents/Qwen Code/Conversations',
        });
    });
    it('does not steal focus when an approval appears before deferred session focus', async () => {
        ...
        expect(editorFocus).not.toHaveBeenCalled();
        expect(document.querySelector('[data-testid="approval-overlay"]')).not.toBeNull();
        expect(testState.latestToolApprovalKeyboardActive).toBe(true);
    });
    it('does not show missing-session state for non-404/410 errors', async () => {
        ...
        await flush();
        expect(container.textContent).not.toContain('Current session does not exist');
    });
    ...
    it('preserves active goal for the same session and clears it after session changes', async () => {
        ...
        await flush();
        await act(async () => {
            window.dispatchEvent(new CustomEvent('web-shell-goal-status-active', {
                detail: {
                    active: true,
                    condition: 'ship it',
                    setAt: 123,
    ...
    it('gates direct submissions and dispatches compatible submit events', async () => {
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('hello', expect.objectContaining({ retry: undefined }));
        ...
        const promptOptions = mockSessionActions.sendPrompt.mock.calls.at(-1)?.[1];
        act(() => promptOptions?.onAdmitted?.());
        expect(sessionCatalogController.promptAdmitted).toHaveBeenCalledWith('/tmp/project', 'session-1');
    });
    ...
    it('dispatches direct and queued submit events for image-only prompts', async () => {
        ...
        await flush();
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('', expect.objectContaining({ images }));
        expect(onSessionChange).toHaveBeenCalledWith({
            type: 'submit',
            sessionId: 'session-1',
            prompt: '',
        ...
        expect(rawEnqueuePrompt).toHaveBeenCalledWith('', images, undefined, undefined);
        expect(onSessionChange).toHaveBeenCalledWith({
            type: 'submit',
            sessionId: 'session-1',
            prompt: '',
    ...
    it('cancels an approved direct submission after the session changes', async () => {
        let approve;
        const onSubmitBefore = vi.fn(() => new Promise((resolve) => {
            approve = resolve;
        }));
        const onSessionChange = vi.fn();
        const { container, rerender } = renderApp({
    ...
    it('cancels an approved submission as soon as a workspace switch starts', async () => {
        ...
        mockSessionActions.clearSession.mockImplementation(() => new Promise((resolve) => {
            finishClear = resolve;
        }));
        mockWorkspace.capabilities = {
            workspaces: [
    ...
    it('does not rerender the composer while a draft waits for intent classification', async () => {
        ...
        act(() => {
            testState.latestChatEditorProps?.onInputTextChange?.('Help me brainstorm a separate Web Shell design proposal');
    ...
    it('does not suggest a new session for obvious follow-up prompts', async () => {
        ...
        await flush();
        expect(container.querySelector('[data-testid="new-session-suggestion"]')).toBeNull();
    ...
    it('suggests starting a new session for a new-topic prompt and auto-sends it in the fresh session', async () => {
        ...
        const suggestedPrompt = 'Help me brainstorm Web Shell interaction ideas on top of this interface for a design doc';
        ...
        mockSessionActions.generateSessionContent.mockImplementation(async function* () {
            yield {
                type: 'delta',
                requestId: 'req-1',
                seq: 0,
        ...
        await flush();
        expect(container.querySelector('[data-testid="new-session-suggestion"]')
            ?.textContent).toContain('This looks like a new topic');
        await act(async () => {
            container
                .querySelector('[data-testid="new-session-suggestion-start"]')
                ?.click();
            await Promise.resolve();
        });
        expect(mockSessionActions.clearSession).toHaveBeenCalledTimes(1);
        ...
        await flush();
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith(suggestedPrompt, expect.objectContaining({
    ...
    it('suggests sending a side question with BTW and clears the accepted draft', async () => {
        ...
        await flush();
        expect(container.querySelector('[data-testid="btw-suggestion"]')?.textContent).toContain('side question');
        await act(async () => {
            container
                .querySelector('[data-testid="btw-suggestion-send"]')
                ?.click();
            await Promise.resolve();
        });
        expect(mockSessionActions.btwSession).toHaveBeenCalledWith(sideQuestion, expect.objectContaining({ signal: expect.any(AbortSignal) }));
        expect(editorCommit).toHaveBeenCalledTimes(1);
        ...
        await flush();
        expect(container.querySelector('[data-testid="new-session-suggestion"]')).not.toBeNull();
    });
    it('refuses a visible BTW suggestion when an inline tag is added before acceptance', async () => {
        ...
        await flush();
        expect(container.querySelector('[data-testid="btw-suggestion"]')).not.toBeNull();
    ...
    it('waits for the current session to detach before auto-submitting the suggested new-session draft', async () => {
        ...
        const delayedPrompt = 'Help me brainstorm Web Shell interaction ideas on top of this interface for a design doc';
        ...
        await flush();
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith(delayedPrompt, expect.any(Object));
    });
    ...
    it('cancels a default submission when navigation occurs during session preparation', async () => {
        ...
        let secondAccepted;
        act(() => {
            secondAccepted = testState.latestChatEditorProps?.onSubmit('second', undefined, secondCommit);
    ...
    it('keeps the first prompt retryable through its lazy session commit', async () => {
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledTimes(2);
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('first', expect.objectContaining({ retry: true }));
    });
    it('refreshes unknown-workspace retry ownership after accepting a prompt', async () => {
        ...
        expect(container.querySelector('[data-testid="streaming-status"]')).not.toBeNull();
    });
    it('commits the first prompt after creating its session', async () => {
        ...
        act(() => {
            accepted = testState.latestChatEditorProps?.onSubmit('first prompt', undefined, commitAccepted);
        });
        expect(accepted).toBe(false);
        await vi.waitFor(() => {
            expect(mockSessionActions.sendPrompt).toHaveBeenCalled();
        ...
        expect(sessionCatalogController.sessionCreated).toHaveBeenCalledWith('/workspace', 'session-created');
    });
    it('lets a selected session bypass a stale preparation promise', async () => {
        ...
        expect(mockSessionActions.attachSession).not.toHaveBeenCalled();
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('second', expect.any(Object));
        await act(async () => {
            callbackFinished.resolve();
            await vi.waitFor(() => {
                expect(mockSessionActions.releaseSession).toHaveBeenCalledWith('session-created');
            });
        });
        expect(mockSessionActions.attachSession).not.toHaveBeenCalled();
        expect(mockSessionActions.clearSession).not.toHaveBeenCalled();
    ...
    it('lets a selected session bypass creation before its id is allocated', async () => {
        mockConnection.sessionId = undefined;
        const creationFinished = deferred();
        mockSessionActions.createSession.mockImplementation(() => creationFinished.promise);
        const { container, rerender } = renderApp();
        await flush();
        act(() => {
            testState.latestChatEditorProps?.onSubmit('first');
    ...
    it('cancels direct submissions when onSubmitBefore rejects and preserves retry state', async () => {
        const onSubmitBefore = vi.fn((params) => params.prompt === 'blocked'
        ...
        await clickSubmit(container);
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('first', expect.objectContaining({ retry: undefined }));
        act(() => {
            testState.blocks = [
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('first', expect.objectContaining({ retry: true }));
    });
    it('hides a turn-error retry while a newer prompt awaits admission', async () => {
        vi.spyOn(console, 'warn').mockImplementation(() => { });
        const rejectedAdmission = deferred();
        const approvedAdmission = deferred();
        ...
        const onSubmitBefore = vi.fn(() => {
            ...
            if (admissionCount === 2)
                return rejectedAdmission.promise;
    ...
    it('restores a turn-error retry after switching away during admission', async () => {
        ...
        await clickSubmit(container);
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledOnce();
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('second', expect.objectContaining({ retry: undefined }));
        await act(async () => {
            approveRetry?.();
            await Promise.resolve();
        });
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledTimes(2);
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('second', expect.objectContaining({
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('first', expect.objectContaining({
            optimisticUserMessage: false,
            retry: true,
        }));
    });
    it('defers retry restoration until navigation commits', async () => {
        const retryApproval = deferred();
        let admissionCount = 0;
        const onSubmitBefore = vi.fn(() => {
            admissionCount += 1;
    ...
    it('does not restore a stale turn-error retry after the source advances', async () => {
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledTimes(3);
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('newer', expect.objectContaining({ retry: true }));
    });
    it('preserves the newer cancelled retry when admissions settle out of order', async () => {
        const oldRetryApproval = deferred();
        const newerRetryApproval = deferred();
        let admissionCount = 0;
        const onSubmitBefore = vi.fn(() => {
            admissionCount += 1;
            if (admissionCount === 2)
                return oldRetryApproval.promise;
            if (admissionCount === 4)
                return newerRetryApproval.promise;
            return Promise.resolve();
        });
        const oldError = {
            kind: 'error',
    ...
    it('allows manual retry after a model stream interrupted turn error', async () => {
        ...
        await clickSubmit(container);
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('recover this stream', expect.objectContaining({ retry: undefined }));
        mockSessionActions.sendPrompt.mockClear();
        act(() => {
            testState.blocks = [
                {
        ...
        expect(container.querySelector('[data-testid="retry"]')).not.toBeNull();
        mockSessionActions.sendPrompt.mockImplementationOnce(() => retrySend.promise);
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('recover this stream', expect.objectContaining({
            optimisticUserMessage: false,
            retry: true,
        }));
        testState.streamingState = 'responding';
        ...
        expect(testState.latestMessageListProps?.activeTurnStartedAt).toBeUndefined();
        const retryOptions = mockSessionActions.sendPrompt.mock.calls.at(-1)?.[1];
        act(() => retryOptions?.onAdmitted?.());
        expect(testState.latestMessageListProps?.isResponding).toBe(true);
        expect(testState.latestMessageListProps?.activeTurnStartedAt).toBeGreaterThanOrEqual(retryStartedAt);
        await act(async () => {
            retrySend.resolve();
            testState.streamingState = 'idle';
            rerender();
    ...
    it.each([
    ...
    ])('reoffers a turn-error retry when the retried turn fails with %s', async (_label, nextPromptId) => {
        const retrySend = deferred();
        mockSessionActions.sendPrompt
            .mockResolvedValueOnce(undefined)
        ...
        await act(async () => {
            retrySend.reject(Object.assign(new Error('retried turn failed'), {
        ...
        await flush();
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledTimes(3);
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('recover this stream', expect.objectContaining({ retry: true }));
    });
    it('restores a turn-error retry when resend fails before admission starts', async () => {
        ...
        mockSessionActions.sendPrompt.mockRejectedValueOnce(new TypeError('network unavailable'));
    ...
    it('preserves the turn-error retry while session writes are blocked', async () => {
        ...
        await flush();
        const unblockedRetry = container.querySelector('[data-testid="retry"]');
    ...
    it('does not settle a turn-error retry into a different workspace', async () => {
        ...
        expect(container.querySelector('[data-testid="prompt-admission-unknown"]')).toBeNull();
        expect(warn).not.toHaveBeenCalledWith('[WebShell] post-turn retry admission outcome is unknown', expect.anything());
        warn.mockRestore();
    });
    it('locks an image retry when its admission response is lost', async () => {
        ...
        expect(container.querySelector('[data-testid="prompt-admission-unknown"]')).not.toBeNull();
        expect(testState.latestChatEditorProps?.disabled).toBe(true);
        expect(sessionCatalogController.promptAdmissionUncertain).toHaveBeenCalledWith('/tmp/project');
        expect(sessionCatalogController.promptAdmitted).not.toHaveBeenCalled();
        warn.mockRestore();
    });
    it('gates queued submissions and only enqueues after approval', async () => {
        ...
        expect(rawEnqueuePrompt).toHaveBeenCalledWith('queued', undefined, undefined, undefined);
        expect(onSessionChange).toHaveBeenCalledWith({
            type: 'submit',
            sessionId: 'session-1',
            prompt: 'queued',
    ...
    it('refreshes background tasks after /fork launches', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(mockSessionActions.forkSession).toHaveBeenCalledWith('Review current changes');
        expect(testState.latestBackgroundTasksRefreshTrigger).toBe(1);
    });
    it('keeps /btw as a lightweight side question when side tasks are available', async () => {
        ...
        expect(mockSessionActions.btwSession).toHaveBeenCalledWith('explain the current implementation', expect.objectContaining({ signal: expect.any(AbortSignal) }));
    ...
    it('settles visible recap after a same-id attachment replacement', async () => {
        ...
        await clickSubmit(container);
        expect(testState.latestMessageListProps?.messages?.some((message) => message.content?.includes('Generating recap'))).toBe(true);
        act(() => {
            testState.ownerVersion += 1;
            rerender();
        });
        ...
        expect(testState.latestMessageListProps?.messages?.some((message) => message.content?.includes('Reconnect-safe recap'))).toBe(true);
    });
    ...
    it('keeps /btw side as a lightweight question without the capability', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(mockSessionActions.btwSession).toHaveBeenCalledWith('side explain the current implementation', expect.objectContaining({ signal: expect.any(AbortSignal) }));
        expect(container.querySelector('button[title="Side task"]')).toBeNull();
    });
    ...
    it('notifies the host before forwarding a slash command', async () => {
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('/Deploy staging', expect.any(Object));
    });
    ...
    it('does not treat an absolute path as a slash command', async () => {
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('/usr/local/bin/tool', expect.any(Object));
    });
    ...
    it('forwards input annotations for /plan prompts in active sessions', async () => {
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('@.husky/ explain', expect.objectContaining({
    ...
    it('dispatches turn_complete only for the session that was streaming', async () => {
        ...
        expect(sessionCatalogController.turnCompleted).toHaveBeenCalledWith('/tmp/project');
        onSessionChange.mockClear();
        act(() => {
            testState.streamingState = 'responding';
            rerender({ onSessionChange });
        ...
        expect(onSessionChange).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'turn_complete' }));
    ...
    it('prefers the active retry when its workspace becomes available', async () => {
        const oldRetryApproval = deferred();
        const activeRetryApproval = deferred();
        let admissionCount = 0;
        const onSubmitBefore = vi.fn(() => {
            admissionCount += 1;
            if (admissionCount === 2)
                return oldRetryApproval.promise;
            if (admissionCount === 4)
                return activeRetryApproval.promise;
            return Promise.resolve();
        });
        const oldError = {
            kind: 'error',
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledTimes(3);
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('second', expect.objectContaining({ retry: true }));
    });
    ...
    it('does not open the extensions manager page with /extension manage', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(container.querySelector('[data-testid="extensions-manager-page"]')).toBeNull();
    });
    it('opens the extensions manager page with /extensions manage', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(container.querySelector('[data-testid="extensions-manager-page"]')).not.toBeNull();
        const backButton = container.querySelector('[data-testid="extensions-manager-back"]');
        expect(document.activeElement).not.toBe(backButton);
        expect(document.activeElement).toBe(container.querySelector('[data-testid="extensions-manager-heading"]'));
        editorFocus.mockClear();
        await act(async () => {
            container
                .querySelector('[data-testid="extensions-manager-back"]')
                ?.click();
            await Promise.resolve();
        });
        expect(container.querySelector('[data-testid="extensions-manager-page"]')).toBeNull();
    ...
    it.each(['/skills', '/skills detail', '/skills details'])('opens the Skill manager page with %s', async (command) => {
        const { container } = renderApp();
        await flush();
        ...
        expect(container
            ...
            ?.getAttribute('aria-label')).toBe('Skills');
        expect(mockSessionActions.sendPrompt).not.toHaveBeenCalled();
    });
    it('converts /skills arguments to a direct skill command', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('/bugfix', expect.any(Object));
        expect(container.querySelector('[data-testid="inline-panel"]')).toBeNull();
    });
    it('opens plugin management tabs from the sidebar', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="open-plugins"]')
                ?.click();
            await Promise.resolve();
        });
        await flush();
        const panel = container.querySelector('[data-testid="inline-panel"]');
        const extensionsTab = panel?.querySelector('button[role="tab"]');
        ...
        expect(panel
            ?.querySelectorAll('button[role="tab"]')[2]
    ...
    it('opens Channel management from the sidebar', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="open-channels"]')
                ?.click();
            await Promise.resolve();
        });
        const panel = container.querySelector('[data-testid="inline-panel"]');
        expect(panel?.getAttribute('aria-label')).toBe('Channels');
        expect(panel?.querySelector('[data-testid="channels-manager-page"]')).not.toBeNull();
    });
    it('shadow-isolates the unified plugin manager body when plugins is enabled', async () => {
        ...
        await flush();
        const panel = container.querySelector('[data-testid="inline-panel"]');
        const host = panel?.querySelector('[data-web-shell-shadow-host="plugins"]');
        const extensionsTab = host?.shadowRoot?.querySelector('button[role="tab"]');
        ...
        expect(document.querySelector('[data-web-shell-portal-root]')).not.toBeNull();
    });
    it.each([
        ['/extensions manage', 'Manage Extensions'],
        ['/mcp', 'MCP Servers'],
        ['/skills details', 'Skills'],
    ])('shadow-isolates the %s compatibility page when plugins is enabled', async (command, panelLabel) => {
        mockWorkspaceActions.loadMcpStatus.mockResolvedValue({
            initialized: true,
            discoveryState: 'completed',
            servers: [],
        ...
        expect(host?.shadowRoot).not.toBeNull();
        expect(host?.shadowRoot?.querySelector('[data-web-shell-shadow-root="plugins"]')).not.toBeNull();
    ...
    it('uses one shadow root for all portals without moving plugin content', async () => {
        ...
        await flush();
        const portalHost = document.querySelector('[data-web-shell-shadow-host="portals"]');
        const portalRoot = portalHost?.shadowRoot?.querySelector('[data-web-shell-portal-root]');
        expect(portalRoot).not.toBeNull();
        expect(portalHost?.style.zIndex).toBe('var(--web-shell-portal-root-z-index, 1000)');
        expect(portalHost?.style.getPropertyPriority('z-index')).toBe('important');
        expect(portalHost?.style.getPropertyValue('--web-shell-portal-root-z-index')).toBe('2345');
        ...
        expect(Array.from(portalHost?.shadowRoot?.querySelectorAll('style') ?? []).some((style) => style.textContent?.includes('.consumer-shadow-content'))).toBe(true);
        await act(async () => {
            container
                .querySelector('[data-testid="open-plugins"]')
                ?.click();
        ...
        expect(panel?.querySelector('[data-web-shell-shadow-host="plugins"]')).toBeNull();
    });
    it('only shows server startup progress during MCP discovery', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(container.textContent).toContain('MCP servers are starting up (1 initializing)');
        expect(container.textContent).not.toContain('Loading MCP tools...');
        expect(container.querySelector('[role="button"][aria-label="filesystem"]')).toHaveProperty('tabIndex', 0);
    });
    it('shows server operations without duplicating tools and resources tabs', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[aria-label="filesystem"]')
                ?.click();
            await Promise.resolve();
        });
        await flush();
        await act(async () => {
            container
                .querySelector('[data-testid="mcp-server-actions"]')
        ...
        await act(async () => {
            document
                .querySelector('[data-testid="mcp-server-action-reconnect"]')
                ?.click();
            await Promise.resolve();
        });
        await flush();
    ...
    it('polls workspace MCP status until browser authentication completes', async () => {
        ...
        mockMcp.reload
            ...
            workspaceCwd: '/workspace',
            servers: [
                { ...disconnectedServer, authenticationState: 'pending' },
            ],
        })
            .mockResolvedValueOnce({
            initialized: true,
        ...
        await act(async () => {
            container
                .querySelector('[aria-label="yuque"]')
                ?.click();
            await Promise.resolve();
        });
        await flush();
        ...
        await act(async () => {
            document
                .querySelector('[data-testid="mcp-server-action-authenticate"]')
                ?.click();
            await Promise.resolve();
        });
        expect(container.textContent).toContain('Open the browser to authenticate.');
    ...
    it('opens the Daemon Status panel and auto-closes it on a pending approval', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="open-daemon-status"]')
                ?.click();
            await Promise.resolve();
        });
        expect(container.querySelector('[data-testid="inline-panel"]')).not.toBeNull();
    ...
    it('opens the Session Overview panel from the sidebar', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="open-sessions-overview"]')
                ?.click();
            await Promise.resolve();
        });
        const panel = container.querySelector('[data-testid="inline-panel"]');
    ...
    it('opens the split view from the sidebar', async () => {
        ...
        expect(container.querySelector('[data-testid="split-view-page"]')).not.toBeNull();
    ...
    it('preserves the legacy split Voice workspace fallback', async () => {
        ...
        const { container } = renderApp();
        await flush();
        expect(container.querySelector('[data-testid="split-voice-workspaces"]')
    ...
    it('restores a persisted split on load (survives a refresh)', async () => {
        ...
        await flush();
        expect(container.querySelector('[data-testid="split-view-page"]')).not.toBeNull();
        expect(container.querySelector('[data-testid="split-initial"]')?.textContent).toBe('s1,s2');
    });
    ...
    it('clears the persisted split when the user leaves the split view', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-back"]')
                ?.click();
            await Promise.resolve();
        });
    ...
    it('syncs the split view from external session ids without the sidebar', async () => {
        ...
        expect(container.querySelector('[data-testid="split-view-page"]')).not.toBeNull();
        expect(container.querySelector('[data-testid="split-initial"]')?.textContent).toBe('s1');
        expect(container.querySelector('[data-testid="split-has-header-actions"]')
            ?.textContent).toBe('yes');
    ...
    it('dedupes and caps external split session ids', async () => {
        ...
        await flush();
        expect(container.querySelector('[data-testid="split-initial"]')?.textContent).toBe('s1,s2,s3,s4,s5,s6');
    });
    it('does not reopen controlled split view when the same ids get a new array reference', async () => {
        ...
        expect(container
            .querySelector('[data-testid="inline-panel"]')
            ?.getAttribute('aria-label')).toBe('Session Overview');
        rerender({ sidebar: false, splitSessionIds: ['s1', 's2'] });
        await flush();
        expect(container.querySelector('[data-testid="split-view-page"]')).toBeNull();
        expect(container
    ...
    it('notifies external callers when split session ids change inside WebShell', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-report-panes"]')
                ?.click();
            await Promise.resolve();
        });
        ...
        await flush();
        expect(container.querySelector('[data-testid="split-initial"]')?.textContent).toBe('s1,s2,s3');
    });
    ...
    it('opens the split view from the external shell ref like the sidebar button', async () => {
        ...
        expect(container.querySelector('[data-testid="split-view-page"]')).not.toBeNull();
        expect(container.querySelector('[data-testid="split-initial"]')?.textContent).toBe('session-1');
    });
    ...
    it('creates a side task from the external shell ref', async () => {
        ...
        expect(container.querySelector('button[title="Side task"]')).not.toBeNull();
        await flush();
        expect(sessionCatalogController.sessionCreated).toHaveBeenCalledWith('/tmp/project', 'side-session-1');
    });
    ...
    it('forces the compact session drawer from the external shell ref', async () => {
        ...
        const drawer = container.querySelector('[data-sidebar-shell][role="dialog"]');
        expect(drawer).not.toBeNull();
    ...
    it('dismisses a forced compact drawer before opening split view', async () => {
        ...
        expect(container.querySelector('[data-testid="split-view-page"]')).not.toBeNull();
        expect(container.querySelector('[data-sidebar-shell][role="dialog"]')).toBeNull();
        expect(container.querySelector('[data-sidebar-shell]')?.className).not.toContain('mobileDrawerForced');
    ...
    it('returns a forced compact drawer to viewport control when dismissed', async () => {
        ...
        expect(container.querySelector('[data-sidebar-shell]')?.className).toContain('mobileDrawerForced');
        await act(async () => {
            container
                .querySelector('[data-sidebar-shell] > div[aria-hidden="true"]')
                ?.click();
            await Promise.resolve();
        });
        expect(container.querySelector('[data-sidebar-shell]')?.className).not.toContain('mobileDrawerForced');
    ...
    it('reports a failed external new-session attempt through its boolean result', async () => {
        ...
        expect(mockSessionActions.clearSession).toHaveBeenCalledTimes(1);
        expect(errorSpy).toHaveBeenCalledWith('[web-shell]', 'boom', expect.any(Error));
    });
    ...
    it('updates an open artifact tab from pane snapshots and keeps it after the pane clears', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-report-artifact"]')
                ?.click();
            await Promise.resolve();
        });
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-report-updated-artifact"]')
                ?.click();
            await Promise.resolve();
        });
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-report-changed-artifact"]')
                ?.click();
            await Promise.resolve();
        });
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-clear-artifacts"]')
                ?.click();
            await Promise.resolve();
        });
    ...
    it('routes a split pane scheduled task through its stamped workspace identity', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-open-scheduled-task"]')
                ?.click();
            await Promise.resolve();
        });
        expect(mockWorkspaceActions.listScheduledTasks).toHaveBeenCalledWith('pane-ws');
        expect(document.body.textContent).toContain('Pane task');
        expect(document.body.textContent).not.toContain('This workspace may have been removed');
    });
    it('routes a split pane review download through its stamped workspace identity', async () => {
        ...
        mockWorkspace.client.workspaceByCwd.mockImplementation(() => paneWorkspaceClient);
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-open-review"]')
                ?.click();
            await Promise.resolve();
        });
        const download = Array.from(document.body.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Download');
        ...
        expect(mockWorkspace.client.workspaceByCwd).toHaveBeenCalledWith('/tmp/pane');
        expect(paneFileStat).toHaveBeenCalledWith('notes.md');
        expect(paneReadBytes).toHaveBeenCalledWith('notes.md', expect.objectContaining({ offset: 0 }));
    });
    it('keeps a main-session artifact tab renderable across a live-list gap', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-open-main-artifact"]')
                ?.click();
            await Promise.resolve();
        });
    ...
    it('opens a split pane monitor in the right panel', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="split-open-monitor"]')
                ?.click();
            await Promise.resolve();
        });
        expect(document.body.querySelector('button[title="watch pane logs"]')).not.toBeNull();
        expect(container.querySelector('[data-testid="split-view-page"]')).not.toBeNull();
    });
    ...
    it('lets controlled split session ids take precedence over a ?split= URL', async () => {
        ...
        try {
            ...
            await flush();
            expect(container.querySelector('[data-testid="split-initial"]')?.textContent).toBe('s3');
            expect(window.location.search).toBe('');
        }
        finally {
            window.history.pushState({}, '', '/');
    ...
    it('keeps the split view open when an approval becomes pending (unlike the scheduled-tasks page)', async () => {
        ...
        expect(container.querySelector('[data-testid="approval-overlay"]')).toBeNull();
    });
    it('surfaces the outer approval as a split notice and returns to chat when clicked', async () => {
        ...
        const notice = container.querySelector('[data-testid="split-approval-notice"]');
        ...
        expect(container.querySelector('[data-testid="split-view-page"]')).toBeNull();
        expect(container.querySelector('[data-testid="approval-overlay"]')).not.toBeNull();
    });
    ...
    it('dismisses the Scheduled Tasks page when an approval becomes pending', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(container.querySelector('[data-testid="scheduled-tasks-page"]')).not.toBeNull();
        await act(async () => {
            testState.blocks = [makePendingPermissionBlock()];
            rerender();
            await Promise.resolve();
        });
        expect(container.querySelector('[data-testid="scheduled-tasks-page"]')).toBeNull();
    });
    ...
    it('keeps the composer dormant (dialogOpen) while an approval overlay is up', async () => {
        ...
        const voiceStatusRevision = testState.latestChatEditorProps?.voiceStatusRevision;
        ...
        expect(testState.latestChatEditorProps?.voiceStatusRevision).toBe(voiceStatusRevision);
    });
    ...
    it('routes AskUserQuestion submission errors to an error toast', async () => {
        ...
        act(() => {
            testState.latestAskUserQuestionOnError?.(new Error('Submit option is unavailable'), 'Failed to submit answer');
        });
        expect(onToast).toHaveBeenCalledWith('error', 'Submit option is unavailable');
    });
    ...
    it('blocks app-level shortcuts while an external modal is registered', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="interaction-blocker"]')
                ?.click();
            await Promise.resolve();
        });
        expect(testState.latestChatEditorProps?.dialogOpen).toBe(true);
    ...
    it('closes the panel and restores composer focus on Back button click', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="panel-back"]')
                ?.click();
            await Promise.resolve();
        });
        expect(container.querySelector('[data-testid="inline-panel"]')).toBeNull();
    ...
    it('closes the panel, sends /model --fast, and reloads settings on fast-model pick', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="open-fast-model"]')
                ?.click();
            await Promise.resolve();
        });
        expect(container.querySelector('[data-testid="dialog-shell"]')).not.toBeNull();
        ...
        await flush();
        expect(mockSessionActions.sendPrompt.mock.calls.some(
        ...
        (c) => c[0] === '/model --fast fast-model-x --project')).toBe(true);
        expect(container.querySelector('[data-testid="inline-panel"]')).toBeNull();
    ...
    it('does not let an A-to-B-to-A deletion clear a newer selection', async () => {
        ...
        await clickSubmit(container);
        await flush();
        act(() => testState.latestModelManagement?.onDeleteModel?.({
            authType: 'api-key',
            modelId: 'old-model',
        }));
        act(() => {
    ...
    it('sends /model --fast with --global when the fast-model picker is opened from the User tab', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="open-fast-model-user"]')
                ?.click();
            await Promise.resolve();
        });
        await act(async () => {
        ...
        await flush();
        expect(mockSessionActions.sendPrompt.mock.calls.some((c) => c[0] === '/model --fast fast-model-x --global')).toBe(true);
    });
    it('keeps a secondary Voice user-scope write on the shared legacy setting route', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="open-voice-model-user"]')
                ?.click();
            await Promise.resolve();
        });
        await act(async () => {
        ...
        await flush();
        expect(settingsSetValue).toHaveBeenCalledWith('user', 'voiceModel', 'fast-model-x');
        expect(qualifiedSetWorkspaceSetting).not.toHaveBeenCalled();
    });
    it('sends /language ui --project for a workspace-scoped language change from Settings', async () => {
        ...
        await act(async () => {
            container
                .querySelector('[data-testid="change-language-workspace"]')
                ?.click();
            await Promise.resolve();
        });
        await flush();
        expect(mockSessionActions.sendPrompt.mock.calls.some((c) => c[0] === '/language ui en --project')).toBe(true);
    });
    it('resynchronizes the catalog when a settings prompt admission is ambiguous', async () => {
        ...
        expect(sessionCatalogController.promptAdmissionUncertain).toHaveBeenCalledOnce();
        expect(sessionCatalogController.promptAdmissionUncertain).toHaveBeenCalledWith('/tmp/project');
    });
    ...
    it('dispatches rename only after the current session name changes', async () => {
        ...
        const { rerender } = renderApp({ onSessionChange });
        await flush();
        expect(onSessionChange).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'rename' }));
        act(() => {
        ...
        expect(sessionCatalogController.renamed).toHaveBeenCalledWith('/tmp/project', 'session-1', 'Renamed Session');
        onSessionChange.mockClear();
        act(() => {
            rerender({ onSessionChange });
        });
    ...
    it('patches and resynchronizes the catalog after a confirmed /rename', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(mockSessionActions.renameSession).toHaveBeenCalledWith('Catalog title');
        expect(sessionCatalogController.renamed).toHaveBeenCalledWith('/tmp/project', 'session-1', 'Catalog title');
        expect(onSessionChange).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'rename' }));
        act(() => {
    ...
    it('reconciles a confirmed rename after its source attachment is replaced', async () => {
        ...
        await clickSubmit(container);
        await vi.waitFor(() => {
            expect(mockSessionActions.renameSession).toHaveBeenCalledWith('Delayed title');
        });
        act(() => {
            testState.ownerVersion += 1;
            mockConnection.sessionId = 'session-2';
        ...
        expect(sessionCatalogController.renamed).toHaveBeenCalledWith('/tmp/project', 'session-1', 'Delayed title');
    });
    it('reconciles a name reused after the session loaded a different title', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(sessionCatalogController.renamed).toHaveBeenCalledWith('/tmp/project', 'session-1', 'Reused title');
    });
});
describe('App prompt send failure retry', () => {
    it('does not mark delivery unknown when lazy session creation fails before prompt admission', async () => {
        vi.spyOn(console, 'error').mockImplementation(() => { });
        mockConnection.sessionId = undefined;
        mockSessionActions.createSession.mockRejectedValueOnce(new Error('session creation failed'));
        renderApp();
        await flush();
        act(() => {
            testState.latestChatEditorProps?.onSubmit('hello', undefined, editorCommit);
        });
        await flush();
        expect(mockSessionActions.sendPrompt).not.toHaveBeenCalled();
        expect(editorCommit).not.toHaveBeenCalled();
        expect(document.querySelector('[data-testid="prompt-admission-unknown"]')).toBeNull();
        expect(testState.latestChatEditorProps?.disabled).toBe(false);
    });
    it('keeps an unknown lazy-session admission scoped to its allocated session', async () => {
        ...
        expect(document.querySelector('[data-testid="prompt-admission-unknown"]')).not.toBeNull();
        expect(sessionCatalogController.promptAdmissionUncertain).toHaveBeenCalledWith('/workspace');
    });
    it('locks duplicate submission when prompt admission is unknown', async () => {
        ...
        const notice = document.querySelector('[data-testid="prompt-admission-unknown"]');
        expect(notice).not.toBeNull();
        expect(document.querySelector('[data-testid="failed-prompt-retry"]')).toBeNull();
        expect(testState.latestChatEditorProps?.disabled).toBe(true);
        act(() => {
            testState.streamingState = 'responding';
            rerender();
    ...
    it('restores direct prompt annotations after uncertain admission', async () => {
        ...
        await flush();
        act(() => {
            testState.latestChatEditorProps?.onSubmit('@file.ts fix', undefined, editorCommit, { inputAnnotations });
        });
        await vi.waitFor(() => {
            expect(mockSessionActions.sendPrompt).toHaveBeenCalledOnce();
        });
        ...
        expect(editorRestoreInputAnnotations).toHaveBeenCalledWith(inputAnnotations);
        confirm.mockRestore();
        warn.mockRestore();
    });
    it('marks the failed message and retries its original payload without a duplicate', async () => {
        ...
        expect(document.querySelector('[data-testid="failed-prompt-retry"]')
            ?.textContent).toBe('u1');
        await act(async () => {
            document
        ...
        expect(document.querySelector('[data-testid="failed-prompt-retry"]')).toBeNull();
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('hello', expect.objectContaining({
            images,
            inputAnnotations,
            optimisticUserMessage: false,
        }));
    ...
    it('keeps a failed-prompt retry visible through background notifications', async () => {
        ...
        expect(container.querySelector('[data-testid="failed-prompt-retry"]')).not.toBeNull();
        act(() => {
            testState.blocks = [
        ...
        await flush();
        expect(container.querySelector('[data-testid="failed-prompt-retry"]')
            ?.textContent).toBe('u1');
    });
    it('keeps a failed-prompt retry when its optimistic block is cloned', async () => {
        vi.spyOn(console, 'error').mockImplementation(() => { });
        const firstSend = deferred();
        const optimisticUser = { id: 'u1', kind: 'user', text: 'hello' };
        mockSessionActions.sendPrompt.mockImplementationOnce(() => {
    ...
    it('rehydrates a rejected failed-prompt retry after attachment reset', async () => {
        ...
        await vi.waitFor(() => {
            expect(mockStore.appendLocalUserMessage).toHaveBeenCalledWith('hello', undefined, undefined);
        });
        act(() => {
    ...
    it('drops a known-workspace failed retry when a replacement reuses its local user id', async () => {
        ...
        await flush();
        expect(container.querySelector('[data-testid="failed-prompt-retry"]')).toBeNull();
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledTimes(1);
    });
    it('drops an uncertain failed retry response after a known-workspace transcript replacement', async () => {
        ...
        expect(container.querySelector('[data-testid="prompt-admission-unknown"]')).toBeNull();
        expect(warn).not.toHaveBeenCalledWith('[WebShell] prompt retry admission outcome is unknown', expect.anything());
    });
    ...
    it('restores a failed-prompt retry after switching away during admission', async () => {
        ...
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledTimes(2);
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('hello', expect.objectContaining({ optimisticUserMessage: false }));
    });
    ...
    it('restores the retry action when resending fails again', async () => {
        ...
        const firstSend = deferred();
        const retrySend = deferred();
        const secondRetrySend = deferred();
        mockSessionActions.sendPrompt
            .mockImplementationOnce(() => {
            testState.blocks = [{ id: 'u1', kind: 'user' }];
            return firstSend.promise;
        ...
        testState.streamingState = 'responding';
        rerender();
        expect(container.querySelector('[data-testid="streaming-status"]')).toBeNull();
        expect(testState.latestMessageListProps?.isResponding).toBe(false);
        expect(testState.latestMessageListProps?.activeTurnStartedAt).toBeUndefined();
        await act(async () => {
    ...
    it('shows processing only after retry admission and restarts its timer', async () => {
        ...
        expect(Number(status?.getAttribute('data-started-at'))).toBeGreaterThanOrEqual(retryStartedAt);
        expect(testState.latestMessageListProps?.isResponding).toBe(true);
        expect(testState.latestMessageListProps?.activeTurnStartedAt).toBeGreaterThanOrEqual(retryStartedAt);
        await act(async () => {
            retrySend.resolve();
    ...
    it('hides the old retry action when a newer user message appears', async () => {
        ...
        expect(document.querySelector('[data-testid="failed-prompt-retry"]')).not.toBeNull();
        act(() => {
            testState.latestChatEditorProps?.onSubmit('second');
        });
        testState.blocks = [
...
describe('App /goal command', () => {
    it('opens the Goals page for a bare /goal instead of sending a prompt', async () => {
        ...
        await clickSubmit(container);
        await flush();
        expect(container.querySelector('[data-testid="goals-page"]')).not.toBeNull();
        expect(mockSessionActions.sendPrompt).not.toHaveBeenCalled();
    });
    ...
    it('starts a goal in a fresh session from the Goals page', async () => {
        ...
        if (!onCreateGoal)
            throw new Error('onCreateGoal was not captured');
        mockSessionActions.clearSession.mockClear();
        mockSessionActions.sendPrompt.mockClear();
        await act(async () => {
        ...
        expect(mockSessionActions.clearSession).toHaveBeenCalledTimes(1);
        expect(mockSessionActions.sendPrompt).toHaveBeenCalledWith('/goal all tests pass', expect.anything());
    });
    it('keeps the Goals page mounted across createNewSession, not just after it', async () => {
        ...
        if (!onCreateGoal)
            throw new Error('onCreateGoal was not captured');
        mockSessionActions.sendPrompt.mockRejectedValueOnce(new Error('daemon says no'));
        await act(async () => {
            await expect(onCreateGoal('all tests pass')).rejects.toThrow('daemon says no');
        });
    ...
    it("opens a goal's session in the chat view", async () => {
        ...
        if (!onOpenSession)
            throw new Error('onOpenSession was not captured');
        mockSessionActions.loadSession.mockClear();
        await act(async () => {
        ...
        // is about.
        expect(mockSessionActions.loadSession.mock.calls[0][0]).toBe('goal-session-9');
    ...
    it("reports a failure to open a goal's session instead of swallowing it", async () => {
        ...
        if (!onOpenSession)
            throw new Error('onOpenSession was not captured');
        mockSessionActions.loadSession.mockRejectedValueOnce(new Error('session is gone'));
        const consoleError = vi
            .spyOn(console, 'error')
            .mockImplementation(() => { });
        await act(async () => {
        ...
        expect(consoleError).toHaveBeenCalledWith('[web-shell]', expect.stringContaining('session is gone'), expect.anything());
        consoleError.mockRestore();
    });
    it('reuses the empty session a failed goal attempt left behind', async () => {
        ...
        expect(mockSessionActions.clearSession).toHaveBeenCalledTimes(1);
        expect(mockSessionActions.sendPrompt).toHaveBeenLastCalledWith('/goal all tests pass', expect.anything());
    });
    it('forgets the stranded session once the user leaves the Goals page', async () => {
        ...
        const back = container.querySelector('[data-testid="goals-page"] button[aria-label="back"]');
        if (!back)
    ...
    it('starts a fresh session again once a goal has actually been sent', async () => {
        ...
        mockSessionActions.sendPrompt.mockRejectedValueOnce(new Error('daemon says no'));
        await act(async () => {
            await expect(onCreateGoal('first goal')).rejects.toThrow('daemon says no');
        });
        await act(async () => {
    ...
    it('does not drop the goal into the current session when the new session fails', async () => {
        ...
        if (!onCreateGoal)
            throw new Error('onCreateGoal was not captured');
        mockSessionActions.clearSession.mockRejectedValueOnce(new Error('daemon unreachable'));
        mockSessionActions.sendPrompt.mockClear();
        await act(async () => {
            await onCreateGoal('all tests pass');
        });
...
describe('App manual-run orchestration (scheduled tasks)', () => {
    ...
    async function openRunHandler(container) {
        testState.prompt = '/schedule';
        await clickSubmit(container);
        await flush();
        const handler = testState.latestScheduledTasksProps?.onRunPrompt;
        if (!handler)
            throw new Error('onRunPrompt was not captured');
        return handler;
    }
    // Make sendPrompt admit the prompt (fire onAdmitted) then resolve, the normal
    // "daemon accepted it" path.
    const admitOnSend = () => mockSessionActions.sendPrompt.mockImplementation((_text, opts) => {
        opts?.onAdmitted?.();
    ...
    it('rejects an unbound run that settles without admitting (cancel path)', async () => {
        ...
        const run = await openRunHandler(container);
        await act(async () => {
            await expect(run('do the thing', null)).rejects.toThrow(/cancelled before it started/);
        });
    });
    ...
    it('supersedes an older pending bound run with a newer one', async () => {
        ...
        expect(firstErr?.message).toMatch(/superseded/);
        vi.clearAllTimers();
        void second;
    });
    it('does not let an old same-target failure clear a newer bound run', async () => {
        ...
        expect(firstError?.message).toMatch(/superseded/);
        await act(async () => {
    ...
    it('does not apply the catch-up timeout while restore is pending', async () => {
        ...
        expect(err?.message).toBe('restore timed out');
    });
    it('starts the 30 second timeout only after commit while catching up', async () => {
        ...
        expect(err?.message).toMatch(/session replay/);
    });
    it('"create via chat" starts a fresh session and primes the composer', async () => {
        ...
        await clickSubmit(container);
        await flush();
        const onCreateViaChat = testState.latestScheduledTasksProps?.onCreateViaChat;
        if (!onCreateViaChat)
            throw new Error('onCreateViaChat was not captured');
        mockSessionActions.clearSession.mockClear();
        editorInsertText.mockClear();
        await act(async () => {
            onCreateViaChat();
