...
function readStoredDaemonToken() {
    try {
...
function persistDaemonToken(token) {
    try {
...
export function waitForDaemonTokenMessage(timeoutMs = DEFAULT_TOKEN_MESSAGE_TIMEOUT_MS) {
    ...
    return new Promise((resolve) => {
        ...
        const onMessage = (event) => {
            ...
            if (data?.type !== DAEMON_AUTH_MESSAGE_TYPE)
                return;
...
export function removeDaemonTokenFromUrl() {
    if (typeof window === 'undefined')
        return;
...
function getAllowedDaemonOrigin(raw) {
    try {
        ...
        const daemonPort = parsed.port || (parsed.protocol === 'https:' ? '443' : '80');
        if (daemonPort !== pagePort)
            return '';
        return parsed.origin;
    }
    catch {
