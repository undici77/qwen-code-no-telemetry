...
const EN = {
  ...
  'notification.monitor.cancelled': (v) =>
    `Monitor "${v?.description ?? ''}" was cancelled. (${v?.events ?? 0} events${
      v?.droppedLines
        ? `, ${v.droppedLines} lines dropped due to throttling`
        : ''
    })`,
  ...
  'extensions.action.failed': (v) =>
    `Extension action failed${v?.name ? ` for "${v.name}"` : v?.source ? ` from "${v.source}"` : ''}: ${v?.error ?? 'Unknown error'}`,
  'extensions.commands.refreshFailed': 'Failed to refresh extension commands.',
  'extensions.install.failed': (v) =>
    `Failed to install extension${v?.source ? ` from "${v.source}"` : ''}: ${v?.error ?? 'Unknown error'}`,
  ...
  'extensions.manage.refreshFailed': (v) =>
    `Extension action succeeded, but session refresh failed${v?.error ? `: ${v.error}` : '.'}`,
...
const ZH = {
  ...
  'notification.monitor.completed': (v) =>
    `监控器已完成（${v?.events ?? 0} 个事件${v?.droppedLines ? `，因限流丢弃 ${v.droppedLines} 行` : ''}）：${v?.description ?? ''}`,
  'notification.monitor.failed': (v) =>
    `监控器执行失败（${v?.events ?? 0} 个事件${v?.droppedLines ? `，因限流丢弃 ${v.droppedLines} 行` : ''}）：${v?.description ?? ''}`,
  'notification.monitor.cancelled': (v) =>
    `监控器已取消（${v?.events ?? 0} 个事件${v?.droppedLines ? `，因限流丢弃 ${v.droppedLines} 行` : ''}）：${v?.description ?? ''}`,
  'notification.agent.completed': (v) =>
  ...
  'extensions.label': '扩展',
  'extensions.action.failed': (v) =>
    `扩展操作失败${v?.name ? `（${v.name}）` : v?.source ? `（${v.source}）` : ''}：${v?.error ?? '未知错误'}`,
  'extensions.commands.refreshFailed': '刷新扩展命令失败。',
  'extensions.install.failed': (v) =>
    `安装扩展失败${v?.source ? `（${v.source}）` : ''}：${v?.error ?? '未知错误'}`,
