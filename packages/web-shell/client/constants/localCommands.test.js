...
import { getLocalCommands, localizeBuiltinDescriptions, skillDescriptionKey, } from './localCommands';
...
describe('getLocalCommands', () => {
    it('translates fallback command descriptions to the active language', () => {
        ...
        expect(getLocalCommands(en).every((c) => c.source === 'builtin-command')).toBe(true);
        expect(byName.get('fork')?.argumentHint).toBe('<directive>');
        expect(byName.get('btw')?.argumentHint).toBe('<your question>');
        expect(getLocalCommands(en, { sideTaskAvailable: true }).find((command) => command.name === 'btw')?.argumentHint).toBe('[side] <your question>');
    });
});
describe('localizeBuiltinDescriptions (commands)', () => {
    it('re-localizes a built-in the daemon advertised in another language', () => {
        ...
        expect(localizeBuiltinDescriptions(commands, zh)[0].description).toBe('提交错误报告');
        expect(localizeBuiltinDescriptions(commands, en)[0].description).toBe('Submit a bug report');
    });
    it('leaves a custom command that shadows a built-in name untouched', () => {
        ...
        expect(localizeBuiltinDescriptions(commands, zh)[0].description).toBe('my project exporter');
    });
    it('translates a built-in name even when source is missing', () => {
        const commands = [
            { name: 'export', description: 'Export conversation to a file' },
        ];
        expect(localizeBuiltinDescriptions(commands, zh)[0].description).toBe('将当前会话历史导出到文件');
    });
    it('does not touch built-ins that are not in the map', () => {
        ...
        expect(localizeBuiltinDescriptions(commands, zh)[0].description).toBe('Clear the screen');
    });
    it('does not localize skills (that happens in the skill-tagging step)', () => {
        ...
        expect(localizeBuiltinDescriptions(commands, zh)[0].description).toBe('Design guidance for charts…');
    });
...
describe('skillDescriptionKey', () => {
    it('maps bundled and project skills to i18n keys', () => {
        ...
        expect(zh(skillDescriptionKey('dataviz'))).toBe('图表与数据可视化设计指南');
        expect(zh(skillDescriptionKey('bugfix'))).toBe('按先复现流程修复 GitHub issue 中的 bug');
    });
...
describe('App command pipeline', () => {
    ...
    function pipeline(daemon, skills) {
        const skillNames = new Set(skills);
        return localizeBuiltinDescriptions(mergeCommands(daemon, getLocalCommands(zh)), zh).map((command) => {
    ...
    it('localizes commands and both bundled + project skills, no session needed', () => {
        ...
        const byName = new Map(pipeline(daemon, ['dataviz', 'bugfix', 'my-skill']).map((c) => [
            c.name,
            c.description,
        ]));
...
//# sourceMappingURL=localCommands.test.js.map