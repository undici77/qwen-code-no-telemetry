...
describe('Web Shell Voice development proxy', () => {
    it('proxies only qualified Voice stream upgrades', () => {
        ...
        expect(qualified && typeof qualified !== 'string' ? qualified.ws : false).toBe(true);
        expect(new RegExp(QUALIFIED_VOICE_STREAM_PROXY).test('/workspaces/id/voice/stream')).toBe(true);
        expect(new RegExp(QUALIFIED_VOICE_STREAM_PROXY).test('/voice/voiceModels.ts')).toBe(false);
    });
});
describe('Web Shell client source proxy bypass', () => {
    it('serves session catalog source modules instead of proxying them', () => {
        ...
        const options = sessionProxy;
        const request = {
            method: 'GET',
            url: '/session-catalog/session-catalog-hooks.ts',
            headers: { 'sec-fetch-dest': 'script' },
        };
        expect(options.bypass?.(request, {}, options)).toBe(request.url);
    });
});
//# sourceMappingURL=vite-config.test.js.map