...
let connectionState;
let streamingStateValue;
let pendingPermission;
let latestOnSubmit;
let latestChatEditorProps;
let latestFollowupAccept;
...
const setApprovalMode = vi.fn(async (mode) => ({ mode }));
...
vi.mock('./MessageList', () => ({
    MessageList: (props) => (_jsxs("div", { "data-testid": "pane-messages", "data-approval": props.pendingApproval ? 'yes' : 'no', children: [props.messages.length, _jsx("button", { "data-testid": "pane-open-turn-output", type: "button", onClick: () => props.onTurnOutputOpen?.({
...
vi.mock('./StreamingStatus', () => ({
    StreamingStatus: (props) => (_jsx("div", { "data-testid": "pane-streaming", "data-started-at": props.startedAt === undefined ? 'none' : String(props.startedAt), "data-show-phrase": String(props.showPhrase) })),
}));
vi.mock('./ChatEditor', () => ({
    ChatEditor: forwardRef(function MockChatEditor(props, ref) {
        latestOnSubmit = props.onSubmit;
        latestChatEditorProps = props;
        useImperativeHandle(ref, () => ({
            insertText,
        }));
        return (_jsxs("div", { "data-web-shell-composer": true, children: [_jsx("button", { "data-testid": "pane-submit", onClick: () => props.onSubmit('hello there'), children: "send" }), _jsx("button", { "data-testid": "pane-cancel", onClick: props.onCancel, children: "cancel" }), _jsx("button", { "data-testid": "pane-pick-mode", onClick: () => props.onSelectMode?.('yolo'), children: "mode" }), _jsx("button", { "data-testid": "pane-pick-badmode", onClick: () => props.onSelectMode?.('totally-bogus'), children: "badmode" }), _jsx("button", { "data-testid": "pane-pick-model", onClick: () => props.onSelectModel?.('gpt-x'), children: "model" }), _jsx("span", { "data-testid": "pane-running", children: String(props.isRunning) }), _jsx("span", { "data-testid": "pane-dialogopen", children: String(props.dialogOpen) }), _jsx("span", { "data-testid": "pane-toolbar", children: JSON.stringify(props.visibleToolbarActions ?? null) }), _jsx("span", { "data-testid": "pane-commands", children: String((props.commands ?? []).length) }), _jsx("span", { "data-testid": "pane-mode", children: String(props.currentMode) }), _jsx("span", { "data-testid": "pane-model", children: String(props.currentModel) }), _jsx("span", { "data-testid": "pane-models", children: JSON.stringify(props.availableModels ?? null) }), _jsx("span", { "data-testid": "pane-queued-messages", children: JSON.stringify(props.queuedMessages ?? null) }), _jsx("span", { "data-testid": "pane-followup", children: String(props.followupState?.suggestion ?? '') })] }));
    }),
}));
vi.mock('./QueuedPromptDisplay', () => ({
    QueuedPromptDisplay: (props) => (_jsx("div", { "data-testid": "pane-queue", "data-can-mutate-mid-turn": String(props.canMutateMidTurn), children: String(props.prompts.length) })),
}));
vi.mock('./messages/ToolApproval', () => ({
    ToolApproval: (props) => (_jsx("button", { "data-testid": "tool-approval", "data-keyboard-active": String(props.keyboardActive), "data-plan-todos": JSON.stringify(props.planTodos?.map((todo) => todo.id) ?? []), onClick: () => props.onConfirm(props.request.id, 'proceed'), children: "approve" })),
}));
vi.mock('./messages/AskUserQuestion', () => ({
    AskUserQuestion: (props) => (_jsx("button", { "data-testid": "ask-approval", "data-keyboard-active": String(props.keyboardActive), onClick: () => props.onConfirm(props.request.id, 'opt'), children: "ask" })),
}));
...
beforeEach(() => {
    ...
    getTasks.mockReset();
    sendPrompt.mockImplementation(async (_text, options) => {
...
function render(props = {}, customization = EMPTY_CUSTOMIZATION) {
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);
    act(() => root.render(_jsx(WebShellCustomizationProvider, { value: customization, children: _jsx(I18nProvider, { language: "en", children: _jsx(ChatPane, { ...props }) }) })));
}
function rerender(props = {}, customization = EMPTY_CUSTOMIZATION) {
    act(() => root.render(_jsx(WebShellCustomizationProvider, { value: customization, children: _jsx(I18nProvider, { language: "en", children: _jsx(ChatPane, { ...props }) }) })));
}
function testid(id) {
...
describe('ChatPane', () => {
    it('opens a pane monitor in the shared right panel', async () => {
        ...
        expect(onOpenMonitor).toHaveBeenCalledWith(monitor, 'sess-1', daemonActions);
    });
    ...
    it('renders the custom composer footer directly after the pane editor', () => {
        ...
        const ComposerFooter = (props) => {
            ...
            return _jsx("div", { "data-testid": "pane-composer-footer", children: "pane footer" });
        };
    ...
    it('updates the custom composer footer with pane-scoped state', () => {
        ...
        const ComposerFooter = (props) => {
            footerProps.push(props);
            return _jsx("div", { "data-testid": "pane-composer-footer" });
        };
    ...
    it('adds no composer footer DOM when omitted or returning null', () => {
        ...
        expect(Array.from(nullComposer?.parentElement?.children ?? [])).toHaveLength(children.length);
    });
    ...
    it('adds no workspace toolbar chip on a single-workspace daemon', () => {
        ...
        expect(latestChatEditorProps.visibleToolbarActions).not.toContain('workspace');
    ...
    it('omits the header workspace tag on a single-workspace daemon', () => {
        render({ title: 'Refactor core', workspaceCwd: '/w' });
        expect(container.querySelector('[data-web-shell-pane-workspace]')).toBeNull();
    });
    ...
    it('suppresses the rotating loading phrase in its compact status', () => {
        render();
        expect(testid('pane-streaming')?.getAttribute('data-show-phrase')).toBe('false');
    });
    it('sends an idle prompt directly so the pane enters loading immediately', () => {
        render();
        act(() => testid('pane-submit').dispatchEvent(new MouseEvent('click', { bubbles: true })));
    ...
    it('reports a host slash command error and continues default handling', () => {
        ...
        expect(onError).toHaveBeenCalledWith(error, 'onSlashCommand callback failed');
        expect(sendPrompt).toHaveBeenCalledWith('/deploy staging', {
            onAdmissionStarted: expect.any(Function),
            onAdmitted: expect.any(Function),
        });
    ...
    it('commits an idle prompt only after daemon admission', () => {
        ...
        act(() => sendPromptAdmit());
        expect(catalogController.promptAdmitted).toHaveBeenCalledWith('/w', 'sess-1');
    ...
    it('queues a prompt while the pane is already running', () => {
        ...
        expect(returned).toBe(true);
        expect(enqueuePrompt).toHaveBeenCalledWith('queued next', undefined, undefined, undefined, expect.any(Function));
    ...
    it('names a side task when its first text prompt is admitted from the queue', () => {
        ...
        const onAdmitted = enqueuePrompt.mock.calls[0]?.[4];
    ...
    it('captures a pane workspace that becomes available mid-turn', () => {
        ...
        expect(catalogController.turnCompleted).not.toHaveBeenCalledWith('/primary');
    });
    it('forwards composer annotations with a queued prompt', () => {
        ...
        expect(enqueuePrompt).toHaveBeenCalledWith('queue @.husky/', undefined, undefined, inputAnnotations, expect.any(Function));
        expect(sendPrompt).not.toHaveBeenCalled();
    });
    it('forwards images with a queued prompt', () => {
        ...
        expect(enqueuePrompt).toHaveBeenCalledWith('queued image', images, undefined, undefined, expect.any(Function));
    });
    ...
    it('submits while disconnected when prompt SSE restart is enabled', () => {
        ...
        expect(sendPrompt).toHaveBeenCalledWith('hi', expect.objectContaining({ onAdmitted: expect.any(Function) }));
    });
    ...
    it('locks the pane when idle prompt admission outcome is unknown', async () => {
        ...
        expect(onImageIngestionNotice).toHaveBeenCalledWith('warning', expect.stringContaining('uncertain'));
        const notice = testid('pane-prompt-admission-unknown');
        expect(notice).not.toBeNull();
        expect(latestChatEditorProps.disabled).toBe(true);
        expect(catalogController.promptAdmissionUncertain).toHaveBeenCalledWith('/w');
        expect(catalogController.promptAdmitted).not.toHaveBeenCalled();
    ...
    it('does not mark a turn error unknown after admission', async () => {
        ...
        expect(latestChatEditorProps.disabled).toBe(false);
        expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: 'turn failed' }), 'Failed to send prompt');
    });
    ...
    it('keeps the pane editable after a definite 413 rejection', async () => {
        const onError = vi.fn();
        sendPrompt.mockRejectedValueOnce(new DaemonHttpError(413, undefined, 'Too large'));
        render({ onError });
        await act(async () => {
            latestOnSubmit('hi');
            await Promise.resolve();
        });
        expect(onError).toHaveBeenCalledWith(expect.any(DaemonHttpError), 'Failed to send prompt');
        expect(latestChatEditorProps.disabled).toBe(false);
    });
    it('keeps pane approvals click-only (no global keyboard shortcuts)', () => {
        pendingPermission = { id: 'perm-1', toolName: 'write_file', rawInput: {} };
        render();
        expect(testid('tool-approval')?.getAttribute('data-keyboard-active')).toBe('false');
    });
    it('passes this pane workflow to its exit-plan approval', () => {
        ...
        expect(testid('tool-approval')?.getAttribute('data-plan-todos')).toBe('["prepare","ship"]');
    });
    ...
    it('renders a tool approval and resolves it via submitPermission', () => {
        ...
        act(() => testid('tool-approval').dispatchEvent(new MouseEvent('click', { bubbles: true })));
    ...
    it('routes an AskUserQuestion permission to the AskUserQuestion overlay', () => {
        ...
        // pane the user is in.
        expect(testid('ask-approval')?.getAttribute('data-keyboard-active')).toBe('false');
    ...
    it('invokes onToggleMaximize from the header maximize button', () => {
        ...
        const maximizeBtn = container.querySelector('[aria-label="Maximize pane"]');
        ...
        act(() => maximizeBtn.dispatchEvent(new MouseEvent('click', { bubbles: true })));
    ...
    it('renders host header actions scoped to the pane session', () => {
        const renderHeaderActions = vi.fn((info) => (_jsxs("button", { type: "button", "data-testid": "host-pane-action", children: [info.sessionId, ":", info.workspaceCwd ?? ''] })));
        render({
            title: 'Refactor core',
            workspaceCwd: '/work/api',
            renderHeaderActions,
    ...
    it('cancels the active turn via the composer cancel action', () => {
        render();
        act(() => testid('pane-cancel').dispatchEvent(new MouseEvent('click', { bubbles: true })));
    ...
    it('passes queued prompts and queue editing controls to the pane editor', () => {
        ...
        expect(testid('pane-queued-messages')?.textContent).toBe(JSON.stringify(['queued next']));
    ...
    it('anchors the streaming timer to the active turn (last user message time)', () => {
        ...
        expect(testid('pane-streaming')?.getAttribute('data-started-at')).toBe('2000');
    });
    it('passes no explicit start time while idle', () => {
        streamingStateValue = 'idle';
        render();
        expect(testid('pane-streaming')?.getAttribute('data-started-at')).toBe('none');
    });
    it('enables the interactive composer controls (approval mode, model, voice)', () => {
        render();
        expect(testid('pane-toolbar')?.textContent).toBe(JSON.stringify(['approvalMode', 'model', 'voice']));
    });
    ...
    it('hides internal composer models and labels the rest', () => {
        ...
        expect(models.map((m) => m.id)).toEqual(['qwen-max']);
    });
    it("drives THIS pane's approval mode when one is picked", () => {
        render();
        act(() => testid('pane-pick-mode').dispatchEvent(new MouseEvent('click', { bubbles: true })));
        expect(setApprovalMode).toHaveBeenCalledWith('yolo');
    });
    it("switches THIS pane's model when one is picked", () => {
        render();
        act(() => testid('pane-pick-model').dispatchEvent(new MouseEvent('click', { bubbles: true })));
    ...
    it('reports a failed model switch to onError', async () => {
        ...
        render({ onError });
        await act(async () => {
            testid('pane-pick-model').dispatchEvent(new MouseEvent('click', { bubbles: true }));
            await Promise.resolve();
            await Promise.resolve();
        });
    ...
    it('reports a failed approval mode switch to onError', async () => {
        ...
        render({ onError });
        await act(async () => {
            testid('pane-pick-mode').dispatchEvent(new MouseEvent('click', { bubbles: true }));
            await Promise.resolve();
            await Promise.resolve();
        });
        expect(onError).toHaveBeenCalled();
    ...
    it('rejects an invalid approval mode without calling the daemon', () => {
        const onError = vi.fn();
        render({ onError });
        act(() => testid('pane-pick-badmode').dispatchEvent(new MouseEvent('click', { bubbles: true })));
...
//# sourceMappingURL=ChatPane.test.js.map