...
describe('WorkspaceIndicator', () => {
    it('reveals the full cwd via a hover tooltip, not a native title', () => {
        ...
        act(() => {
            root.render(_jsx(WorkspaceIndicator, { name: name, title: title, ariaLabel: ariaLabel }));
        });
        const chip = container.querySelector(`[aria-label="${ariaLabel}"]`);
        if (!chip)
            throw new Error('workspace chip was not rendered');
    ...
    it('keeps the full cwd discoverable when the name collapses in compact mode', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(WorkspaceIndicator, { name: "api", title: "/work/api", ariaLabel: "Workspace: api", compact: true }));
        });
        ...
        openTooltip(chip);
        expect(document.querySelector('[role="tooltip"]')?.textContent).toBe('/work/api');
        act(() => root.unmount());
        container.remove();
        vi.useRealTimers();
    });
    it('tints the chip with the workspace accent so it stays distinct when compact', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(WorkspaceIndicator, { name: "api", title: "/work/api", ariaLabel: "Workspace: api", color: "blue", compact: true }));
        });
        const chip = container.querySelector('[data-web-shell-workspace]');
        if (!chip)
            throw new Error('workspace chip was not rendered');
    ...
    it('applies the accent in expanded mode too, not only when compact', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(WorkspaceIndicator, { name: "infra", title: "/work/infra", ariaLabel: "Workspace: infra", color: "purple" }));
        });
        const chip = container.querySelector('[data-web-shell-workspace]');
        if (!chip)
            throw new Error('workspace chip was not rendered');
    ...
    it('adds no accent classes when no color is given', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(WorkspaceIndicator, { name: "api", title: "/work/api", ariaLabel: "ws" }));
        });
        const chip = container.querySelector('[data-web-shell-workspace]');
    ...
    it('localizes the accessible workspace label', () => {
        expect(getTranslator('en')('workspace.paneLabel', { name: 'api' })).toBe('Workspace: api');
        expect(getTranslator('zh-CN')('workspace.paneLabel', { name: 'api' })).toBe('工作区：api');
    });
});
//# sourceMappingURL=WorkspaceIndicator.test.js.map