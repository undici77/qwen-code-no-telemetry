...
import { DaemonSessionProvider, useConnection, } from '@qwen-code/webui/daemon-react-sdk';
import { useI18n } from '../i18n';
import { ChatPane } from './ChatPane';
import { ErrorBoundary } from './ErrorBoundary';
import { MAX_SPLIT_PANES } from '../utils/splitUrl';
import { SESSION_LIST_PAGE_SIZE, SESSION_ORGANIZATION_FEATURE, WEB_SHELL_HISTORY_PAGE_SIZE, WEB_SHELL_MAX_TRANSCRIPT_BLOCKS, } from '../constants/sessions';
...
import { hasMultipleWorkspaces, mergeSessionsById, workspaceLabelForCwd, } from '../utils/workspace';
 ...
 */
export function SplitView({ sessionIds, onPanesChange, onExit, onError, onImageIngestionNotice, onSlashCommand, onRightPanelOpen, onOpenMonitor, onPaneArtifactsChange, messageTurnOutputs, renderPaneHeaderActions, includeOtherWorkspaces = true, workspaceCwd, restartSseOnPrompt, historyPageSize = WEB_SHELL_HISTORY_PAGE_SIZE, voiceUserRevision = 0, voiceWorkspaceRevisions = {}, voiceWorkspaces, sessionWorkflowEnabled = false, }) {
    const { t } = useI18n();
    const connection = useConnection();
    const currentSessionId = connection.sessionId;
    const organizationEnabled = connection.capabilities?.features?.includes(SESSION_ORGANIZATION_FEATURE) ??
        false;
    ...
    const { sessions: otherSessions, reload: reloadOther } = useOtherWorkspaceSessions(includeOtherWorkspaces && !workspaceCwd);
    const allSessions = useMemo(() => mergeSessionsById(sessions, otherSessions), [sessions, otherSessions]);
    const multiWorkspace = !workspaceCwd &&
    ...
    const normalizedSessionIds = useMemo(() => Array.from(new Set((sessionIds ?? []).filter(Boolean))).slice(0, MAX_PANES), [sessionIds]);
    ...
    const [instanceId] = useState(() => typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function'
        ? crypto.randomUUID()
        : Math.random().toString(36).slice(2));
    useEffect(() => {
        if (!sessionIdsControlled)
            return;
        setPaneIds((prev) => prev.length === normalizedSessionIds.length &&
    ...
    const titleById = useMemo(() => {
        ...
        for (const session of allSessions) {
            map.set(session.sessionId, session.displayName?.trim() || session.sessionId.slice(0, 8));
        }
        return map;
    }, [allSessions]);
    ...
    const addPane = useCallback((sessionId) => {
        const currentPaneIds = paneIdsRef.current;
        if (currentPaneIds.includes(sessionId) ||
    ...
    const removePane = useCallback((sessionId) => {
        const currentPaneIds = paneIdsRef.current;
        if (!currentPaneIds.includes(sessionId))
            return;
    ...
    useEffect(() => {
        ...
        const onKeyDown = (event) => {
            if (event.key !== 'Escape' || event.defaultPrevented)
                return;
            if (pickerOpen || isEditableTarget(event.target))
                return;
            setMaximizedPaneId(null);
        };
        document.addEventListener('keydown', onKeyDown);
    ...
    const available = useMemo(() => allSessions.filter((session) => !paneIds.includes(session.sessionId)), [allSessions, paneIds]);
    ...
    return (_jsxs("div", { className: styles.split, "data-testid": "split-view", children: [_jsxs("header", { className: styles.toolbar, children: [_jsx("button", { type: "button", className: styles.backButton, onClick: onExit, "aria-label": t('common.back'), title: t('common.back'), children: _jsx("svg", { viewBox: "0 0 24 24", width: "18", height: "18", "aria-hidden": "true", children: _jsx("path", { d: "M15 18l-6-6 6-6", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round" }) }) }), _jsx("span", { className: styles.title, children: t('splitView.title') }), _jsx("span", { className: styles.count, children: t('splitView.count', { count: paneIds.length }) }), _jsxs("div", { className: styles.addWrap, ref: addWrapRef, children: [_jsxs("button", { type: "button", className: styles.addButton, disabled: !canAdd, "aria-haspopup": "listbox", "aria-expanded": pickerOpen, onClick: () => setPickerOpen((open) => !open), children: ["+ ", t('splitView.addPane')] }), pickerOpen && available.length > 0 && (_jsx("ul", { className: styles.picker, role: "listbox", children: available.map((session) => (_jsx("li", { role: "option", "aria-selected": "false", children: _jsxs("button", { type: "button", className: styles.pickerItem, onClick: () => addPane(session.sessionId), children: [_jsx("span", { className: styles.pickerItemLabel, children: titleById.get(session.sessionId) ??
                                                    session.sessionId.slice(0, 8) }), multiWorkspace && (_jsx("span", { className: styles.pickerItemWorkspace, title: session.workspaceCwd, children: workspaceLabelForCwd(session.workspaceCwd, connection.capabilities?.workspaces) }))] }) }, session.sessionId))) }))] })] }), _jsx("div", { className: styles.panes, children: paneIds.length === 0 ? (_jsx("div", { className: styles.empty, children: t('splitView.empty') })) : (paneIds.map((sessionId) => {
                    ...
                    return (_jsx("div", { className: styles.paneSlot, "data-pane-hidden": isHidden ? '' : undefined, children: _jsx(ErrorBoundary, { label: `split-pane:${sessionId}`, resetKeys: [sessionId], fallback: (error) => (_jsxs("div", { className: styles.paneError, role: "alert", children: [_jsx("div", { className: styles.paneErrorTitle, children: titleById.get(sessionId) ?? sessionId.slice(0, 8) }), _jsxs("div", { className: styles.paneErrorMessage, children: [t('splitView.paneError'), ": ", error.message] }), _jsx("button", { type: "button", className: styles.paneErrorClose, onClick: () => removePane(sessionId), children: t('splitView.closePane') })] })), children: _jsx(DaemonSessionProvider, { sessionId: sessionId, 
                                ...
                                clientId: `split-pane:${instanceId}:${sessionId}`, historyPageSize: historyPageSize, subagentTranscriptMode: "summary", maxBlocks: WEB_SHELL_MAX_TRANSCRIPT_BLOCKS, suppressOwnUserEcho: true, restartEventStreamOnPrompt: restartSseOnPrompt, children: _jsx(ChatPane, { title: titleById.get(sessionId), workspaceCwd: paneWorkspaceCwd, reportCatalogTurnCompletion: sessionId !== currentSessionId ||
                                        paneWorkspaceCwd !== connection.workspaceCwd, renderHeaderActions: renderPaneHeaderActions, hidden: isHidden, voiceUserRevision: voiceUserRevision, voiceWorkspaceRevisions: voiceWorkspaceRevisions, voiceWorkspaces: voiceWorkspaces, onClose: () => removePane(sessionId), onToggleMaximize: canMaximize
                                        ? () => toggleMaximize(sessionId)
                                        : undefined, isMaximized: isMaximized, onError: onError, onImageIngestionNotice: onImageIngestionNotice, onSlashCommand: onSlashCommand, onRightPanelOpen: onRightPanelOpen, onOpenMonitor: onOpenMonitor, onPaneArtifactsChange: onPaneArtifactsChange, messageTurnOutputs: messageTurnOutputs, restartSseOnPrompt: restartSseOnPrompt, sessionWorkflowEnabled: sessionWorkflowEnabled }) }) }) }, scopePanesByWorkspace
