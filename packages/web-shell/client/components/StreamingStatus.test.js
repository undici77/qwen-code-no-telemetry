...
import { getLoadingPhrases, PHRASE_CHANGE_INTERVAL_MS, } from '../constants/loadingPhrases';
...
const mocks = vi.hoisted(() => ({ streamingState: 'responding' }));
...
function render(customization = {}, props = {}) {
    const container = document.createElement('div');
    document.body.appendChild(container);
    const root = createRoot(container);
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: customization, children: _jsx(StreamingStatus, { ...props }) }) }));
    });
...
function pinPhraseSelection() {
    vi.spyOn(Math, 'random').mockReturnValue(0);
}
function labelText(container) {
...
describe('StreamingStatus loading phrases', () => {
    ...
    it('hides the phrase when the resolver returns an empty array', () => {
        ...
        expect(container.firstElementChild?.querySelectorAll('span').length).toBe(2);
    });
    ...
    it('does not reset the phrase when the resolver identity changes mid-stream', () => {
        ...
        document.body.appendChild(container);
        const root = createRoot(container);
        const tree = (resolver) => (_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: { loadingPhrases: resolver }, children: _jsx(StreamingStatus, {}) }) }));
    ...
    it('re-shows the phrase when the resolver switches from empty to non-empty on the next tick', () => {
        ...
        try {
            ...
            const root = createRoot(container);
            let phrases = [];
            const tree = () => (_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: { loadingPhrases: () => phrases }, children: _jsx(StreamingStatus, {}) }) }));
...
//# sourceMappingURL=StreamingStatus.test.js.map