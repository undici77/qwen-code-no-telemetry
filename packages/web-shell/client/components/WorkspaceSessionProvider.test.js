...
vi.mock('../App', () => ({
    App: (props) => {
        ...
        return (_jsx("output", { children: String(props['initialSelectedWorkspaceCwd'] ?? '') }));
    },
}));
...
describe('WorkspaceSessionProvider transactional targets', () => {
    ...
    async function renderTarget(sessionId, workspaceCwd, onSessionIdChange = vi.fn()) {
        await act(async () => {
            root.render(_jsx(WorkspaceSessionProvider, { sessionId: sessionId, workspaceCwd: workspaceCwd, webShellProps: { onSessionIdChange } }));
        });
    ...
    it('rolls back a primary-workspace target when workspace props are omitted', async () => {
        ...
        await act(async () => {
            root.render(_jsx(WorkspaceSessionProvider, { sessionId: "session-a", webShellProps: { onSessionIdChange } }));
        });
        onSessionIdChange.mockClear();
        await act(async () => {
            root.render(_jsx(WorkspaceSessionProvider, { sessionId: "session-b", webShellProps: { onSessionIdChange } }));
        });
        mocks.connection = {
            status: 'connected',
            sessionId: 'session-a',
    ...
    it('does not remount when an unknown daemon resolves as modern', async () => {
        ...
        await act(async () => {
            root.render(_jsx(WorkspaceSessionProvider, { sessionId: "session-a", webShellProps: {} }));
        });
        expect(mocks.providerMounts).toBe(1);
        mocks.workspace = {
            ...mocks.workspace,
