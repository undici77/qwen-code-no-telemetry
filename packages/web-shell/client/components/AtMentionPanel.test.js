...
function mount(menu, handlers = {}) {
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(AtMentionPanel, { menu: menu, anchorRef: { current: anchor }, panelRef: { current: null }, onSelect: vi.fn(), onAccept: vi.fn(), onBack: vi.fn(), onSearch: vi.fn(), onSelectTab: vi.fn(), ...handlers }) }));
    });
}
afterEach(() => {
    act(() => root?.unmount());
...
describe('AtMentionPanel', () => {
    it('renders provider categories in a listbox', () => {
        ...
        expect(document.body
            ...
            ?.getAttribute('aria-label')).toBe('Reference menu');
        expect(document.body
    ...
    it('dispatches item-search keyboard actions', () => {
        ...
        expect(input.getAttribute('aria-activedescendant')).toBe('at-mention-option-0');
        act(() => {
            input.blur();
        });
        expect(input.getAttribute('aria-activedescendant')).toBe('at-mention-option-0');
    ...
    it('dispatches search input changes', () => {
        ...
        const input = document.body.querySelector('input');
        act(() => {
            Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value')?.set?.call(input, 'read');
    ...
    it('renders the upload item with an upload icon', () => {
        ...
        expect(document.body.textContent).toContain('Upload a file into this folder');
    ...
    it('guards mask icon sources', () => {
        ...
        expect(icons[0]?.getAttribute('style')).toContain('data:image/png;base64,iVBOR');
    ...
    it('falls back when a custom item renderer throws', () => {
        ...
        expect(warn).toHaveBeenCalledWith('[WebShell] at mention item render failed', error);
        warn.mockRestore();
    });
...
//# sourceMappingURL=AtMentionPanel.test.js.map