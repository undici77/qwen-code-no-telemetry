...
import { DAEMON_APPROVAL_MODES, useOptionalWorkspace, } from '@qwen-code/webui/daemon-react-sdk';
...
import { GitBranchChipContent, GitBranchIndicator, gitBranchAriaLabel, } from './GitBranchIndicator';
import { GitModePopover } from './GitModePopover';
import { BranchPickerPopover } from './BranchPickerPopover';
import { WorkspaceIndicator } from './WorkspaceIndicator';
import { ChevronDownIcon, ChevronRightIcon, FolderClosedIcon, LoaderCircleIcon, UploadIcon, XIcon, } from 'lucide-react';
...
function TypewriterPlaceholder({ text }) {
    ...
    return (_jsxs("span", { className: styles.typewriterPlaceholder, "data-web-shell-composer-typewriter": true, "aria-hidden": "true", children: [visibleText, !reducedMotion && (_jsx("span", { className: `${styles.typewriterCaret} ${finished ? styles.typewriterCaretFinished : ''}`, children: "_" }))] }));
}
...
function ContextUsageRing({ pct }) {
    ...
    return (_jsxs("svg", { viewBox: "0 0 16 16", "aria-hidden": "true", children: [_jsx("circle", { cx: "8", cy: "8", r: CONTEXT_RING_RADIUS, fill: "none", strokeWidth: "2.5", className: styles.contextRingTrack }), _jsx("circle", { cx: "8", cy: "8", r: CONTEXT_RING_RADIUS, fill: "none", strokeWidth: "2.5", strokeLinecap: "round", strokeDasharray: CONTEXT_RING_CIRCUMFERENCE, strokeDashoffset: CONTEXT_RING_CIRCUMFERENCE * (1 - capped / 100), transform: "rotate(-90 8 8)", className: valueClass })] }));
}
function ModelIcon() {
...
function ToolbarPopover({ open, items, activeId, onOpenChange, onSelect, trigger, tooltip, showCheck = false, searchable = false, searchLabel, noResultsLabel, header, submenu, }) {
    ...
    const dropdownItems = (_jsxs("div", { className: `${styles.dropdownList} ${hasRichItems
            ? styles.dropdownRich
            : showCheck
                ? styles.dropdownCheck
                : ''} ${searchable ? styles.dropdownListConstrained : ''}`, children: [visibleItems.map((item) => (_jsx("button", { type: "button", className: `${styles.dropdownItem} ${item.id === activeId ? styles.dropdownItemActive : ''}`, title: item.label, onClick: () => {
    ...
    const searchableItems = (_jsxs(_Fragment, { children: [searchable && (_jsx(Input, { ref: searchInputRef, type: "search", value: searchQuery, "aria-label": searchLabel, placeholder: searchLabel, autoComplete: "off", onChange: (event) => setSearchQuery(event.target.value), onKeyDown: (event) => {
                    if (!submenu || event.key !== 'ArrowLeft' || searchQuery)
                        return;
                    event.preventDefault();
    ...
    return (_jsxs(Popover, { open: open, onOpenChange: (nextOpen) => {
            ...
            onOpenChange(nextOpen);
        }, children: [tooltip ? (_jsx(TooltipProvider, { delayDuration: 300, children: _jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx(PopoverTrigger, { ref: triggerRef, asChild: true, children: trigger }) }), _jsx(TooltipContent, { side: "top", children: tooltip })] }) })) : (_jsx(PopoverTrigger, { ref: triggerRef, asChild: true, children: trigger })), _jsx(PopoverContent, { side: "top", align: "start", collisionPadding: 8, collisionBoundary: collisionBoundary ?? undefined, "data-web-shell-toolbar-popover": true, "data-web-shell-reasoning-popover": submenu ? '' : undefined, onClick: (event) => event.stopPropagation(), onOpenAutoFocus: (event) => {
                    if (!submenu)
                        return;
                    event.preventDefault();
                    submenuTriggerRef.current?.focus();
                    ...
                    event.preventDefault();
                    selectionRef.current = false;
                }, children: submenu ? (_jsxs(_Fragment, { children: [header, _jsxs("div", { className: styles.dropdownSubmenuSection, children: [_jsx("div", { className: styles.reasoningSectionTitle, children: submenu.sectionLabel }), _jsxs(Popover, { open: submenuOpen, onOpenChange: (nextOpen) => {
                                        setSubmenuOpen(nextOpen);
                                        if (!nextOpen)
                                            setSearchQuery('');
                                    }, children: [_jsx(PopoverTrigger, { asChild: true, children: _jsxs("button", { type: "button", ref: submenuTriggerRef, className: `${styles.dropdownItem} ${styles.dropdownSubmenuTrigger}`, "data-web-shell-model-submenu-trigger": true, "aria-haspopup": "dialog", "aria-expanded": submenuOpen, "aria-label": submenu.triggerAriaLabel, onKeyDown: (event) => {
                                                    if (event.key !== 'ArrowRight')
                                                        return;
                                                    event.preventDefault();
                                                    setSubmenuOpen(true);
                                                }, children: [_jsx("span", { title: submenu.triggerLabel, children: submenu.triggerLabel }), _jsx(ChevronRightIcon, { "aria-hidden": "true" })] }) }), _jsx(PopoverContent, { side: "right", align: "end", alignOffset: -10, sideOffset: 15, collisionPadding: 8, collisionBoundary: collisionBoundary ?? undefined, "data-web-shell-toolbar-popover": true, "data-web-shell-model-submenu": true, onClick: (event) => event.stopPropagation(), children: searchableItems })] })] })] })) : (_jsxs(_Fragment, { children: [header, searchableItems] })) })] }));
}
function ModelReasoningControls({ reasoning, onSelect, }) {
    ...
    return (_jsxs("div", { className: styles.reasoningOptions, "data-web-shell-model-reasoning": true, children: [_jsx("div", { className: styles.reasoningSectionTitle, children: t('reasoning.options') }), _jsxs("div", { className: styles.reasoningThinkingRow, children: [_jsx("span", { children: t('reasoning.thinking') }), _jsx(Switch, { checked: reasoning.enabled, disabled: busy, "aria-label": t('reasoning.thinking'), "data-web-shell-thinking-toggle": true, onCheckedChange: (enabled) => void select(enabled ? reasoning.effort : 'none') })] }), _jsx("div", { className: styles.reasoningDivider }), _jsx("div", { className: styles.reasoningSectionTitle, children: t('reasoning.effort') }), reasoning.efforts.map((effort) => (_jsxs("button", { type: "button", className: styles.reasoningEffortRow, "aria-pressed": reasoning.effort === effort, "data-web-shell-effort": effort, disabled: !reasoning.enabled || busy, onClick: () => void select(effort), children: [_jsx("span", { children: t(`reasoning.effort.${effort}`) }), _jsx("span", { className: styles.dropdownItemCheck, children: reasoning.effort === effort ? _jsx(CheckIcon, {}) : null })] }, effort)))] }));
}
function SlashCommandPanel({ menu, anchorRef, panelRef, detailRef, onClose, onSelect, onAccept, }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Popover, { open: true, onOpenChange: (open) => {
                    if (!open)
                        onClose();
                }, children: [_jsx(PopoverAnchor, { virtualRef: anchorRef }), _jsx(PopoverContent, { ref: panelRef, side: "top", align: "start", alignOffset: 16, sideOffset: 8, avoidCollisions: false, collisionPadding: 12, collisionBoundary: collisionBoundary ?? undefined, className: "duration-0 data-open:animate-none data-closed:animate-none", role: "listbox", "data-web-shell-slash-menu": true, onOpenAutoFocus: (event) => event.preventDefault(), onCloseAutoFocus: (event) => event.preventDefault(), onInteractOutside: (event) => {
                            const target = event.target;
                            ...
                            setHoverDetail(null);
                                        ...
                                        return (_jsxs("div", { className: styles.slashEntry, children: [plan.showHeader && (_jsxs(_Fragment, { children: [plan.showDivider && (_jsx("div", { className: styles.slashSection })), _jsxs("div", { className: styles.slashSectionHeader, children: [_jsx("span", { children: item.section }), plan.count > 0 ? (_jsx("span", { className: styles.slashSectionCount, children: plan.count })) : null] })] })), _jsxs("button", { ref: (node) => {
                                                        ...
                                                        const side = right - rowRect.right >= detailWidth + 8
                                                            ? 'right'
                    ...
                    if (!open)
                        setHoverDetail(null);
                }, children: [_jsx(PopoverAnchor, { virtualRef: hoverAnchorRef }), hoverDetail && (_jsx(PopoverContent, { ref: detailRef, side: hoverDetail.side, align: "start", sideOffset: 8, collisionPadding: 12, collisionBoundary: collisionBoundary ?? undefined, className: "duration-0 data-open:animate-none data-closed:animate-none", "data-web-shell-slash-detail": true, onOpenAutoFocus: (event) => event.preventDefault(), onCloseAutoFocus: (event) => event.preventDefault(), onMouseLeave: (event) => {
                            const nextTarget = event.relatedTarget;
                            if (nextTarget instanceof Node &&
...
function QuickActionsPanel({ actions, onRun, onPressKey, showKeyHints = true, }) {
    const { t } = useI18n();
...
export const ChatEditor = memo(forwardRef(function ChatEditor(props, ref) {
    const { onSubmit, onInputTextChange, onAttachmentsChange, onCycleMode, onToggleShortcuts, onCancel, isRunning = false, isPreparing = false, cancelArmed = false, disabled = false, placeholderText = 'Type a message...', animatePlaceholder = true, commands, skills = [], slashCommandCategoryOrder, queuedMessages = [], onPopQueuedMessages, currentMode = 'default', sessionWorkflowEnabled = false, currentModel = '', gitBranch, gitWorktree, gitCwd, gitModeIntent, onGitModeIntentChange, gitStatus, onOpenGitDiff, onOpenCommit, workspaceName, workspaceTitle, workspaceColor, chatWidthMode = '1000', showChatWidthToggle = true, chatWidthToggleMin, visibleToolbarActions, tokenCount = 0, contextWindow = 0, onShowContextUsage, availableModels = [], onSelectMode, onSelectModel, reasoning, onSelectReasoningEffort, workspaces, selectedWorkspaceCwd, workspaceSelectionDisabled = false, onSelectWorkspace, scratchWorkspaceSupported = false, existingFolderWorkspaceSupported = false, workspaceMutationBusy = false, onCreateScratchWorkspace, onOpenExistingWorkspace, atWorkspaceCwd, onChatWidthModeChange, onFocusFooter, dialogOpen = false, followupState, onAcceptFollowup, onDismissFollowup, sessionId, sessionName, composerInput, composerInputVersion, builtinAtProviders, atProviders, composerTagIcons, voiceTarget, voiceStatusRevision, onImageIngestionNotice, onImagePreview, } = props;
    const { renderComposerToolbarStart: ToolbarStart, renderComposerToolbarEnd: ToolbarEnd, renderComposerToolbarRight: ToolbarRight, renderComposerTag, renderComposerTagTooltip, onComposerTagClick, parseUserMessageContent, fileUploadEnabled, } = useWebShellCustomization();
    ...
    const uploadTarget = useMemo(() => {
        ...
        if (!features.includes('workspace_file_upload'))
            return undefined;
        if (atWorkspaceCwd) {
            if (!features.includes('workspace_qualified_rest_core'))
                return undefined;
            // The selected workspace must be present exactly once and trusted.
            const matches = (uploadWorkspace.capabilities?.workspaces ?? []).filter((w) => w.cwd === atWorkspaceCwd);
        ...
        const primaryMatches = (uploadWorkspace.capabilities?.workspaces ?? []).filter((workspace) => workspace.primary);
    ...
    const maxUploadBytes = uploadWorkspace?.capabilities?.limits?.maxWorkspaceFileUploadBytes ??
        50 * 1024 * 1024;
    ...
    const triggerFilePicker = useCallback((targetDir, restoreQuery) => {
    ...
    const insertUploadReference = useCallback((path) => {
    ...
    const showTypewriterPlaceholder = animatePlaceholder &&
        !disabled &&
    ...
    const modeItems = useMemo(() => DAEMON_APPROVAL_MODES.map((id) => ({
        id,
        label: id === 'plan' && sessionWorkflowEnabled
    ...
    const { searchMode, searchQuery, searchMatches, searchActiveIndex, searchInputRef, searchUiRef, closeSearch, restoreSearchMatch, handleSearchKeyDown, handleSearchInput, handleSearchCompositionEnd, } = core.searchState;
    ...
    // Mode display label
    const modeLabel = currentMode === 'plan' && sessionWorkflowEnabled
    ...
    const workspaceSelectVisible = Boolean(workspaces &&
        onSelectWorkspace &&
        (workspaces.length > 1 ||
    ...
    return (_jsxs("div", { className: `${styles.editorShell} ${modeDropdownOpen || modelDropdownOpen
            ...
            : ''}`, "data-composer": true, "data-web-shell-composer": true, onDragOver: cancelShellFileDrag, onDrop: cancelShellFileDrag, children: [fileUpload.uploads.length > 0 && (_jsxs("div", { className: styles.uploadStrip, "data-web-shell-upload-strip": true, children: [_jsx("div", { className: styles.srOnly, role: "status", "aria-live": "polite", children: fileUpload.uploads
                            .filter((upload) => upload.status === 'done' || upload.status === 'error')
                            .map((upload) => `${upload.file.name}: ${uploadStatusText(upload)}`)
                            .join(' \u2014 ') }), fileUpload.uploads.map((upload) => {
                        const busy = upload.status === 'pending' || upload.status === 'uploading';
                        return (_jsxs("div", { className: styles.uploadRow, "data-status": upload.status, children: [busy ? (_jsx(LoaderCircleIcon, { className: styles.uploadRowSpinner, "aria-hidden": "true" })) : (_jsx(UploadIcon, { "aria-hidden": "true" })), _jsx("span", { className: styles.uploadRowName, children: upload.file.name }), _jsx("span", { className: styles.uploadRowStatus, children: uploadStatusText(upload) }), _jsx("button", { type: "button", className: styles.uploadRowAction, "aria-label": busy
                                        ? t('composer.upload.cancel')
                                        : t('composer.upload.dismiss'), onClick: () => fileUpload.removeUpload(upload.id), children: _jsx(XIcon, { "aria-hidden": "true" }) })] }, upload.id));
                    })] })), _jsxs("div", { ref: containerRef, className: styles.container, "data-web-shell-composer-surface": true, "data-upload-drag-active": uploadDragActive || undefined, "data-typewriter-visible": showTypewriterPlaceholder || undefined, "data-image-drag-active": (core.imageDragActive && !uploadDragActive) || undefined, "aria-busy": core.pendingImageBatchCount > 0 || undefined, ...core.imageTransferHandlers, onDragEnter: handleUploadDragEnter, onDragOver: handleUploadDragOver, onDragLeave: handleUploadDragLeave, onDropCapture: handleUploadDrop, onClick: () => {
                    setModeDropdownOpen(false);
                    setModelDropdownOpen(false);
                    setQuickActionsOpen(false);
                    core.focus();
                }, children: [_jsx(SpecularComposerEffect, { targetRef: containerRef }), uploadEnabled && (_jsx("input", { ref: fileInputRef, type: "file", multiple: true, className: styles.hiddenUploadInput, "data-web-shell-upload-input": true, onChange: handleUploadPickerChange, tabIndex: -1, "aria-hidden": "true" })), searchMode && (_jsxs("div", { ref: searchUiRef, className: styles.searchPanel, onMouseDown: (event) => event.stopPropagation(), onClick: (event) => event.stopPropagation(), children: [_jsxs("div", { className: styles.searchBar, children: [_jsx("span", { className: styles.searchLabel, children: t('editor.searchLabel') }), _jsx("input", { ref: searchInputRef, className: styles.searchInput, value: searchQuery, onChange: handleSearchInput, onCompositionEnd: handleSearchCompositionEnd, onKeyDown: handleSearchKeyDown, placeholder: t('editor.searchPlaceholder') })] }), searchMatches.length > 0 && (_jsx("div", { className: styles.searchResults, children: searchMatches.map((match, matchIndex) => {
                                    return (_jsxs("button", { type: "button", className: `${styles.searchResult} ${matchIndex === searchActiveIndex
                                            ...
                                            closeSearch(false);
                                                ...
                                                return (_jsx(TopComposerTag, { tag: tag, content: renderComposerTagContent(tag), tooltip: tooltip, onActivate: onComposerTagClick
                                                        ...
                                                        : undefined }, tag.id));
                                            }) }) })), core.pastedImages.length > 0 && (_jsx("div", { className: styles.images, children: core.pastedImages.map((img, i) => {
                                            const src = `data:${img.media_type};base64,${img.data}`;
                                            return (_jsxs("div", { className: styles.imageThumb, children: [_jsx("img", { src: src, alt: "", onClick: onImagePreview
                                                            ? () => onImagePreview(src)
                                                            : undefined }), _jsx("button", { type: "button", className: styles.imageRemove, disabled: disabled, onClick: (e) => {
                                                            e.stopPropagation();
                                                            if (disabled)
                                                                return;
                                                            core.removeImage(i);
                                                        }, "aria-label": "Remove image", children: _jsx("svg", { width: "8", height: "8", viewBox: "0 0 10 10", fill: "none", "aria-hidden": "true", children: _jsx("path", { d: "M2 2l6 6M8 2l-6 6", stroke: "currentColor", strokeWidth: "1.5", strokeLinecap: "round" }) }) })] }, i));
                                        }) }))] })), uploadDragActive && (_jsxs("div", { className: styles.uploadDropOverlay, "data-web-shell-upload-drop-overlay": true, children: [_jsx(UploadIcon, { "aria-hidden": "true" }), _jsx("span", { children: t('composer.upload.drop') })] })), core.slashMenu && (_jsx(SlashCommandPanel, { menu: core.slashMenu, anchorRef: containerRef, panelRef: slashPanelRef, detailRef: slashDetailRef, onClose: core.closeSlashMenu, onSelect: core.selectSlashCompletion, onAccept: core.acceptSlashCompletion })), core.atMenu && (_jsx(AtMentionPanel, { menu: core.atMenu, anchorRef: containerRef, panelRef: atPanelRef, onSelect: core.selectAtCompletion, onAccept: core.acceptAtCompletion, onBack: () => {
                                    const result = core.backAtCategories();
                                    if (result === 'categories') {
                                        window.setTimeout(() => core.focus(), 0);
                                    }
                                    return Boolean(result);
                                }, onSearch: core.updateAtSearch, onSelectTab: core.selectAtTab })), _jsxs("div", { className: styles.editorArea, onPointerDownCapture: () => setTypewriterSuppressed(true), onKeyDownCapture: () => setTypewriterSuppressed(true), onBlurCapture: (event) => {
                                    if (!event.currentTarget.contains(event.relatedTarget)) {
                                        setTypewriterSuppressed(false);
                                    }
                                }, children: [showTypewriterPlaceholder && (_jsx(TypewriterPlaceholder, { text: placeholderText })), core.shellMode && (_jsx("span", { className: styles.shellPrefix, "aria-hidden": "true", children: "!" })), core.mobileComposer ? (
                                    ...
                                    _jsx("textarea", { ref: core.mobileComposer.textareaRef, className: styles.mobileTextarea, value: core.mobileComposer.value, onChange: core.mobileComposer.onChange, onBlur: core.mobileComposer.onBlur, placeholder: core.mobileComposer.placeholder, disabled: core.disabled, rows: 1, enterKeyHint: "enter", autoCapitalize: "off", autoCorrect: "off", spellCheck: false, "data-web-shell-composer-editor": true })) : (_jsx("div", { ref: core.containerRef, "data-web-shell-composer-editor": true }))] }), _jsxs("div", { ref: toolbarRef, className: styles.toolbar, "data-mobile-voice-active": mobileVoiceActive || undefined, children: [_jsxs("div", { ref: toolbarLeadingRef, className: styles.toolbarLeading, "data-web-shell-toolbar-leading": true, children: [ToolbarStart && (_jsx("div", { ref: toolbarStartRef, className: styles.toolbarStart, children: _jsx(ToolbarStart, { disabled: disabled, isRunning: isRunning, currentMode: currentMode, currentModel: currentModel, sessionName: sessionName }) })), _jsxs("div", { className: styles.toolbarLeft, children: [workspaceSelectVisible &&
                                                        workspaces &&
                                                        onSelectWorkspace && (_jsx(WorkspaceSelector, { workspaces: workspaces, selectedWorkspaceCwd: selectedWorkspaceCwd, disabled: workspaceSelectionDisabled, busy: workspaceMutationBusy, scratchSupported: scratchWorkspaceSupported, existingFolderSupported: existingFolderWorkspaceSupported, className: `${styles.toolBtn} ${styles.workspaceSelectTrigger} ${showWorkspaceSelectLabel
                                                            ? ''
                                                            : styles.workspaceSelectTriggerCompact}`, onSelectWorkspace: onSelectWorkspace, onCreateScratch: onCreateScratchWorkspace ?? (() => { }), onOpenExistingFolder: onOpenExistingWorkspace ?? (() => { }) })), workspaceIndicatorVisible && workspaceName && (_jsx(WorkspaceIndicator, { name: workspaceName, title: workspaceTitle ?? workspaceName, color: workspaceColor, compact: !showWorkspaceLabel, ariaLabel: t('workspace.paneLabel', {
                                                            name: workspaceName,
                                                        }) })), gitBranchVisible &&
                                                        gitBranch &&
                                                        (gitModeIntent && onGitModeIntentChange ? (_jsx(GitModePopover, { branch: gitBranch, compact: !showGitBranchLabel, intent: gitModeIntent, onIntentChange: onGitModeIntentChange })) : (_jsx(BranchPickerPopover, { open: branchPickerOpen, onOpenChange: setBranchPickerOpen, workspaceCwd: selectedWorkspace?.cwd ?? '', gitCwd: gitCwd, onOpenDiff: onOpenGitDiff, onOpenCommit: onOpenCommit, children: _jsx("button", { type: "button", className: styles.gitBranchChipButton, "aria-label": gitBranchAriaLabel(gitBranch, gitStatus, t), children: _jsx(GitBranchIndicator, { branch: gitBranch, status: gitStatus, compact: !showGitBranchLabel, worktree: gitWorktree }) }) }))), showModeAction && (_jsx("div", { className: `${styles.dropdownWrapper} ${showModeLabel ? '' : styles.dropdownWrapperCompact}`, children: _jsx(ToolbarPopover, { open: modeDropdownOpen, items: modeItems, activeId: currentMode, onOpenChange: (open) => {
                                                                ...
                                                                if (open)
                                                                    setModeDropdownOpen(false);
                                                            }, onSelect: handleModelSelect, tooltip: modelLabel, showCheck: true, searchable: true, searchLabel: t('common.search'), noResultsLabel: (query) => t('model.noMatch', { query }), submenu: showReasoningOptions
                                                                ? {
                                                                    triggerLabel: modelLabel,
                                                                    triggerAriaLabel: `${t('model.select')}: ${modelLabel}`,
                                                                    sectionLabel: t('model.section'),
                                                                }
                                                                : undefined, header: showReasoningOptions &&
                                                                reasoning &&
                                                                onSelectReasoningEffort ? (_jsx(ModelReasoningControls, { reasoning: reasoning, onSelect: onSelectReasoningEffort })) : undefined, trigger: _jsxs("button", { className: `${styles.toolBtn} ${styles.modelToolBtn} ${showModelLabel ? '' : styles.toolBtnCompact}`, "data-web-shell-model-button": true, "data-web-shell-toolbar-popover-trigger": true, onClick: (e) => {
                                                                    e.stopPropagation();
                                                                    core.closeSlashMenu();
                                                                    core.closeAtMenu();
                                                                    setQuickActionsOpen(false);
                                                                }, "aria-label": `${t('model.select')}: ${normalizedModelChipLabel}`, title: normalizedModelChipLabel, children: [_jsx("span", { className: styles.toolBtnModelIcon, children: _jsx(ModelIcon, {}) }), showModelLabel && (_jsx("span", { className: styles.toolBtnText, children: normalizedModelChipLabel })), _jsx("span", { className: styles.toolBtnArrow, children: _jsx(ChevronDownIcon, {}) })] }) }) })), ToolbarEnd && (_jsx("div", { ref: toolbarEndRef, className: styles.toolbarEnd, children: _jsx(ToolbarEnd, { disabled: disabled, isRunning: isRunning, currentMode: currentMode, currentModel: currentModel, sessionName: sessionName }) }))] })] }), _jsxs("div", { ref: toolbarRightRef, className: styles.toolbarRight, children: [showQuickActions && quickActions.length > 0 && (_jsx("button", { className: `${styles.toolBtn} ${styles.quickActionsBtn}`, "data-hide-during-mobile-voice": true, onClick: (e) => {
                                                    e.stopPropagation();
                                                    core.closeSlashMenu();
                                                    core.closeAtMenu();
                                                    setModeDropdownOpen(false);
                                                ...
                                                }, "aria-expanded": quickActionsOpen, "aria-label": t('quickActions.open'), title: t('quickActions.open'), "data-tooltip": t('quickActions.open'), children: _jsx("span", { className: styles.toolBtnIcon, children: _jsx(QuickActionsIcon, {}) }) })), ToolbarRight && (_jsx("div", { ref: toolbarRightCustomRef, className: styles.toolbarRightCustom, "data-hide-during-mobile-voice": true, children: _jsx(ToolbarRight, { disabled: disabled, isRunning: isRunning, currentMode: currentMode, currentModel: currentModel, sessionName: sessionName }) })), showChatWidthToggle &&
                                                widthToggleFits &&
                                                showToolbarAction('widthMode') && (_jsx("button", { className: `${styles.toolBtn} ${styles.widthModeBtn}`, "data-hide-during-mobile-voice": true, onClick: (e) => {
                                                    e.stopPropagation();
                                                    ...
                                                    ? t('settings.option.ui.chatWidth.1000')
                                                    : t('settings.option.ui.chatWidth.wide'), children: _jsx("span", { className: styles.toolBtnIcon, children: _jsx(WidthModeIcon, { mode: chatWidthMode }) }) })), showToolbarAction('contextUsage') &&
                                                contextWindow > 0 &&
                                                tokenCount > 0 && (_jsx(TooltipProvider, { delayDuration: 300, children: _jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx("button", { className: `${styles.toolBtn} ${styles.contextUsageBtn}`, "data-hide-during-mobile-voice": true, "data-web-shell-context-usage": true, onClick: (e) => {
                                                                    e.stopPropagation();
                                                                    onShowContextUsage?.();
                                                                }, disabled: !onShowContextUsage, "aria-label": t('status.contextUsed', {
                                                                    pct: ((tokenCount / contextWindow) * 100).toFixed(1),
                                                                }), children: _jsx("span", { className: styles.toolBtnIcon, children: _jsx(ContextUsageRing, { pct: (tokenCount / contextWindow) * 100 }) }) }) }), _jsx(TooltipContent, { side: "top", children: formatContextUsageDetail(tokenCount, contextWindow) })] }) })), showToolbarAction('voice') && (_jsxs(_Fragment, { children: [_jsx(LiveVoiceButton, {}), _jsx(VoiceButton, { disabled: disabled, onActiveChange: setVoiceActive, target: voiceTarget, statusRevision: voiceStatusRevision, onInsert: (text) => {
                                                            const existing = core.getText();
                                                            const sep = existing && !/\s$/.test(existing) ? ' ' : '';
                                                            core.insertText(`${sep}${text} `);
                                                            core.focus();
                                                        } })] })), _jsx("button", { className: composerPreparing || showCancelButton
                                                    ? `${styles.sendBtn} ${styles.sendBtnRunning}${cancelArmed ? ` ${styles.sendBtnArmed}` : ''}`
                                                    : styles.sendBtn, disabled: composerPreparing
                                                    ? true
                                                    : showCancelButton
                                                        ? !onCancel
                                                        : !core.canSubmit, "data-web-shell-composer-submit": true, onClick: (e) => {
                                                    e.stopPropagation();
