    ...
    return {
        ...
        useComposerCore: (options) => {
            ...
            return {
                ...
                hasAttachments: mockComposerCoreState.pastedImages.length > 0 ||
...
vi.mock('../voice/VoiceButton', () => ({
    VoiceButton: ({ onActiveChange, }) => {
        voiceButtonState.onActiveChange = onActiveChange;
        return _jsx("span", { "data-testid": "voice-button" });
    },
}));
vi.mock('../live/LiveVoiceButton', () => ({
    LiveVoiceButton: () => _jsx("span", { "data-testid": "live-voice-button" }),
}));
vi.mock('./SpecularComposerEffect', () => ({
    SpecularComposerEffect: () => (_jsx("span", { "data-web-shell-composer-specular": true, "aria-hidden": "true" })),
}));
const mounted = [];
afterEach(() => {
...
function renderChatEditorInto(root, portalRoot, props) {
    const { composerTags, pastedImages, customization, renderComposerTagTooltip, onComposerTagClick, ...chatEditorProps } = props;
    if (composerTags) {
    ...
    act(() => {
        root.render(_jsx(WebShellPortalRootContext.Provider, { value: portalRoot, children: _jsx(WebShellCustomizationProvider, { value: {
                    ...customization,
...
function rerenderChatEditor(container, props) {
    const entry = mounted.find((m) => m.container === container);
    if (!entry)
        throw new Error('container is not mounted by renderChatEditor');
...
describe('ChatEditor voice toolbar integration', () => {
    it('keeps dictation and Live together when the host toolbar allows voice', () => {
        ...
        for (const container of [defaults, voiceOnly]) {
            expect(container.querySelector('[data-testid="voice-button"]')).not.toBeNull();
        ...
        expect(hidden.querySelector('[data-testid="live-voice-button"]')).toBeNull();
    });
});
describe('ChatEditor context usage ring', () => {
    const ring = (container) => container.querySelector('[data-web-shell-context-usage]');
    it('renders in toolbarRight before the voice actions', () => {
        ...
        const liveVoice = container.querySelector('[data-testid="live-voice-button"]');
        // The ring sits immediately left of the voice cluster.
        expect(button.compareDocumentPosition(liveVoice) &
    ...
    it('opens the context breakdown when clicked', () => {
        ...
        act(() => {
            ring(container).dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }));
        });
    ...
    it('escalates the arc color at the /context panel thresholds', () => {
        const arcClass = (container) => ring(container).querySelectorAll('circle')[1].getAttribute('class') ??
            '';
...
describe('ChatEditor animation layers', () => {
    it('mounts the inert specular layer without replacing composer controls', () => {
        const container = renderChatEditor({});
        expect(container.querySelector('[data-web-shell-composer-specular]')).not.toBeNull();
        expect(container.querySelector('[data-web-shell-composer-editor]')).not.toBeNull();
        expect(container.querySelector('[data-web-shell-composer-submit]')).not.toBeNull();
    });
    ...
    it('plays the typewriter twice and then keeps the completed text', () => {
        ...
        const typewriter = () => container.querySelector('[data-web-shell-composer-typewriter]');
    ...
    it('does not mount the typewriter for an empty placeholder', () => {
        ...
        expect(container.querySelector('[data-web-shell-composer-typewriter]')).toBeNull();
        expect(container.querySelector('[data-typewriter-visible]')).toBeNull();
    });
    ...
    it('hides the typewriter when disabled, in shell mode, or during a followup', () => {
        const typewriterOf = (container) => container.querySelector('[data-web-shell-composer-typewriter]');
        expect(typewriterOf(renderChatEditor({ placeholderText: 'abc' }))).not.toBeNull();
        expect(typewriterOf(renderChatEditor({ placeholderText: 'abc', disabled: true }))).toBeNull();
        composerCoreState.shellMode = true;
        expect(typewriterOf(renderChatEditor({ placeholderText: 'abc' }))).toBeNull();
        composerCoreState.shellMode = false;
        expect(typewriterOf(renderChatEditor({
            placeholderText: 'abc',
...
describe('ChatEditor attachment reporting', () => {
    it('reports whether the composer has tags or pasted images', () => {
        ...
        expect(disabled.querySelector('img')?.nextElementSibling).toHaveProperty('disabled', true);
    });
...
describe('ChatEditor composer tag icons', () => {
    it('renders built-in icons for top composer tags', () => {
        const kinds = ['extension', 'file', 'mcp', 'skill'];
        const container = renderChatEditor({
        ...
        expect(container.querySelectorAll('[style*="--composer-tag-icon-url"]')).toHaveLength(kinds.length);
    });
    it('rejects unsafe custom icon URLs for top composer tags', () => {
        ...
        expect(container.querySelector('[style*="--composer-tag-icon-url"]')).toBeNull();
    });
});
describe('ChatEditor git branch toolbar integration', () => {
    it('shows the git branch indicator when the branch action is visible', () => {
        ...
        expect(container.querySelector('[aria-label="Current Git branch: feature/web-shell"]')).not.toBeNull();
    });
    it('hides the git branch indicator without a branch or visible action', () => {
        expect(renderChatEditor({
            visibleToolbarActions: ['gitBranch'],
        }).querySelector('[aria-label^="Current Git branch:"]')).toBeNull();
        expect(renderChatEditor({
            gitBranch: 'main',
            visibleToolbarActions: [],
        }).querySelector('[aria-label^="Current Git branch:"]')).toBeNull();
...
describe('ChatEditor workspace toolbar integration', () => {
    it('shows the workspace indicator when the workspace action is visible', () => {
        ...
        expect(chip?.getAttribute('data-web-shell-workspace-title')).toBe('/work/api');
        expect(chip?.getAttribute('title')).toBeNull();
        expect(container.querySelector('[data-web-shell-workspace]')).not.toBeNull();
    });
    it('falls back to the workspace name for the tooltip when no title is given', () => {
        ...
        expect(container
            ...
            ?.getAttribute('data-web-shell-workspace-title')).toBe('api');
    });
    ...
    it('renders the workspace chip before the git branch chip', () => {
        ...
        expect(ws.compareDocumentPosition(git) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
    });
});
describe('ChatEditor top composer tag tooltip', () => {
    it('activates the plain tag from click and keyboard with the outer tag rect', () => {
        ...
        const tag = container.querySelector('[data-web-shell-composer-tag]');
        const trigger = tag.querySelector('[data-web-shell-composer-tag-trigger]');
        const outerRect = { width: 200 };
        const innerRect = { width: 120 };
    ...
    it('removes a tag without activating it', () => {
        ...
        const remove = container.querySelector('[aria-label="Remove orders"]');
        act(() => {
            remove.dispatchEvent(new MouseEvent('click', { bubbles: true }));
            remove.dispatchEvent(new KeyboardEvent('keydown', { key: 'Backspace', bubbles: true }));
    ...
    it('falls back to a plain tag when custom tooltip rendering throws', () => {
        ...
        expect(container.querySelector('[role="tooltip"]')).toBeNull();
        expect(warn).toHaveBeenCalledWith('[WebShell] composer tag tooltip render failed', error);
        warn.mockRestore();
    });
    it('opens custom content from a top tag in the configured portal root', () => {
        ...
        const portalRoot = document.body.querySelector('[data-web-shell-portal-root]');
        const tag = container.querySelector('[data-web-shell-composer-tag]');
        const trigger = tag?.querySelector('[data-web-shell-composer-tag-trigger]');
        ...
        act(() => trigger?.focus());
        const content = portalRoot?.querySelector('[data-web-shell-composer-tag-tooltip]');
        const accessibleTooltip = portalRoot?.querySelector('[role="tooltip"]');
        ...
        expect(trigger?.getAttribute('aria-describedby')).toBe(accessibleTooltip?.id);
...
describe('ChatEditor Session Workflow mode rename', () => {
    it('renames only the plan entry in the mode dropdown while enabled', () => {
        ...
        const labels = Array.from(popover?.querySelectorAll('button') ?? []).map((button) => button.textContent ?? '');
        expect(labels.some((label) => label.includes('Plan & Review (plan)'))).toBe(true);
        expect(labels.some((label) => label.includes('Ask Approval (default)'))).toBe(true);
    ...
    it('renames the active plan mode chip while enabled', () => {
        ...
        expect(withWorkflow
            ...
            ?.textContent?.includes('Plan & Review')).toBe(true);
        const withoutWorkflow = renderChatEditor({ currentMode: 'plan' });
        expect(withoutWorkflow
            .querySelector('[data-toolbar-measure="mode:expanded"]')
            ?.textContent?.includes('Plan & Review')).toBe(false);
    });
});
describe('ChatEditor toolbar popovers', () => {
    it('opens the approval mode popover and restores editor focus after selection', async () => {
        ...
        const yolo = Array.from(popover?.querySelectorAll('button') ?? []).find((button) => button.textContent?.includes('(yolo)'));
        await act(async () => {
            yolo?.click();
        ...
        expect(document.querySelector('[data-web-shell-toolbar-popover]')).toBeNull();
    });
    it('observes custom toolbar render roots when measuring available width', () => {
        ...
        try {
            renderChatEditor({
                visibleToolbarActions: [],
                customization: {
                    renderComposerToolbarStart: () => (_jsx("span", { "data-test-toolbar-start": true, children: "start" })),
                    renderComposerToolbarEnd: () => (_jsx("span", { "data-test-toolbar-end": true, children: "end" })),
                    renderComposerToolbarRight: () => (_jsx("span", { "data-test-toolbar-right": true, children: "right" })),
                },
            });
            expect(observed.has(document.querySelector('[data-test-toolbar-start]'))).toBe(true);
            expect(observed.has(document.querySelector('[data-test-toolbar-end]'))).toBe(true);
            expect(observed.has(document.querySelector('[data-test-toolbar-right]'))).toBe(true);
        }
        finally {
    ...
    it('opens a searchable model popover and selects the filtered model', () => {
        ...
        act(() => {
            container
                .querySelector('[data-web-shell-model-button]')
                ?.click();
        });
        const search = document.querySelector('[data-web-shell-toolbar-popover] input[type="search"]');
        ...
        const options = Array.from(document.querySelectorAll('[data-web-shell-toolbar-popover] button'));
    ...
    it('displays the model label instead of an opaque route id', () => {
        ...
        const button = container.querySelector('[data-web-shell-model-button]');
    ...
    it('exposes the complete model name on dropdown items for hover', () => {
        const modelLabel = 'Qwen Very Long Model Name For Web Shell Reproduction 2026';
        const container = renderChatEditor({
            visibleToolbarActions: ['model'],
        ...
        const option = Array.from(document.querySelectorAll('[data-web-shell-toolbar-popover] button')).find((button) => button.textContent?.includes(modelLabel));
    ...
    it('switches between sibling toolbar popovers without dismissing the target', async () => {
        ...
        const modelButton = container.querySelector('[data-web-shell-model-button]');
        ...
        expect(document.querySelector('[data-web-shell-toolbar-popover] input[type="search"]')).not.toBeNull();
        await act(async () => {
            modeButton?.click();
            await new Promise((resolve) => setTimeout(resolve, 0));
        });
        expect(modeButton?.getAttribute('aria-expanded')).toBe('true');
        expect(document.querySelector('[data-web-shell-toolbar-popover] input[type="search"]')).toBeNull();
    });
});
describe('ChatEditor slash command popovers', () => {
    it('uses shadcn popovers for the command panel and hover detail', () => {
        ...
        expect(panel
            ...
            ?.hasAttribute('data-has-description')).toBe(false);
        ...
        const command = panel?.querySelector('[role="option"]');
        act(() => {
...
describe('ChatEditor mobile composer quick actions', () => {
    ...
    function openQuickActions(container) {
    ...
    it('maps the history quick action to the search UI on the mobile composer', () => {
        withTouchDevice(() => {
            ...
            const historyButton = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'Question history');
    ...
    it('hides the keyboard shortcut hints grid on the mobile composer', () => {
        withTouchDevice(() => {
            ...
            expect(Array.from(mobileContainer.querySelectorAll('button')).some((button) => button.textContent === 'Tab')).toBe(false);
            composerCoreState.mobileComposer = null;
            const desktopContainer = renderChatEditor({});
            openQuickActions(desktopContainer);
            expect(Array.from(desktopContainer.querySelectorAll('button')).some((button) => button.textContent === 'Tab')).toBe(true);
        });
    });
    it('hides other toolbar actions while mobile voice capture is active', () => {
        withTouchDevice(() => {
            ...
            expect(container.querySelector('[data-web-shell-toolbar-leading]')).toBeTruthy();
            expect(container.querySelector('button[aria-label="more actions"][data-hide-during-mobile-voice]')).toBeTruthy();
            expect(container.querySelector('[data-web-shell-composer-submit]')).toBeTruthy();
            act(() => {
                voiceButtonState.onActiveChange?.(true);
            });
            expect(container.querySelector('[data-mobile-voice-active="true"]')).toBeTruthy();
            act(() => {
                voiceButtonState.onActiveChange?.(false);
            });
            expect(container.querySelector('[data-mobile-voice-active="true"]')).toBeFalsy();
        });
    });
});
describe('ChatEditor file upload gating', () => {
    const makeWorkspace = (features, trusted = true, maxUploadBytes = 50 * 1024 * 1024) => ({
    ...
    const dispatchDrag = (target, type, types, files = [], items) => {
    ...
    it('enables the upload entry point when the capability is present', () => {
        ...
        const container = renderChatEditor({});
        expect(container.querySelector('[data-web-shell-upload-input]')).not.toBeNull();
    });
    ...
    it('stays disabled for an untrusted legacy-primary workspace', () => {
        uploadWorkspaceState.current = makeWorkspace(['workspace_file_upload'], false);
        const container = renderChatEditor({});
        expect(container.querySelector('[data-web-shell-upload-input]')).toBeNull();
        expect(composerCoreState.onFileUploadRequest).toBeUndefined();
    });
    it('does not intercept non-file drops', () => {
        uploadWorkspaceState.current = makeWorkspace(['workspace_file_upload']);
        const container = renderChatEditor({});
        const surface = container.querySelector('[data-web-shell-composer-surface]');
    ...
    it('clears the file-drop overlay when dragleave omits data-transfer types', () => {
        ...
        expect(container.querySelector('[data-web-shell-upload-drop-overlay]')).not.toBeNull();
        dispatchDrag(surface, 'dragleave', []);
        expect(container.querySelector('[data-web-shell-upload-drop-overlay]')).toBeNull();
    });
    ...
    it('ignores file drag-and-drop while the composer is disabled', () => {
        ...
        expect(container.querySelector('[data-web-shell-upload-drop-overlay]')).toBeNull();
        const drop = dispatchDrag(editor, 'drop', ['Files'], [new File(['abc'], 'report.txt')]);
    ...
    it('keeps capability-off file drops on the legacy image lane', () => {
        ...
        uploadWorkspaceState.current = workspace;
        composerCoreState.imageDropCapture.mockImplementation((event) => {
            event.preventDefault();
            event.stopPropagation();
        });
        const container = renderChatEditor({});
        const editor = container.querySelector('[data-web-shell-composer-editor]');
        const drop = dispatchDrag(editor, 'drop', ['Files'], [new File(['x'], 'notes.txt')]);
        expect(drop.defaultPrevented).toBe(true);
    ...
    it('routes mixed image-and-file drops to the upload path', async () => {
        ...
        const container = renderChatEditor({});
        const editor = container.querySelector('[data-web-shell-composer-editor]');
        const drop = dispatchDrag(editor, 'drop', ['Files'], [
            new File(['png'], 'photo.png', { type: 'image/png' }),
            new File(['csv'], 'data.csv', { type: 'text/csv' }),
        ]);
        expect(drop.defaultPrevented).toBe(true);
        expect(composerCoreState.imageDropCapture).not.toHaveBeenCalled();
        expect(container.querySelector('[data-web-shell-upload-strip]')).not.toBeNull();
        await act(async () => { });
    ...
    it('intercepts file drops before the editor and renders status above it', () => {
        ...
        const drop = dispatchDrag(editor, 'drop', ['Files'], [new File(['xx'], 'large.txt')]);
    ...
    it('cancels file drops on the upload strip so the tab cannot navigate', () => {
        ...
        const drop = dispatchDrag(strip, 'drop', ['Files'], [new File(['yy'], 'other.txt')]);
        expect(drop.defaultPrevented).toBe(true);
        expect(workspace.client.uploadWorkspaceFile).not.toHaveBeenCalled();
    });
    it('uploads picker selections into the captured directory and inserts a tag', async () => {
        ...
        uploadWorkspaceState.current = workspace;
        const container = renderChatEditor({});
        const input = container.querySelector('[data-web-shell-upload-input]');
        ...
        expect(workspace.client.uploadWorkspaceFile.mock.calls[0][0].path).toBe('docs/notes.txt');
    ...
    it('keeps supported image drops on the image attachment path', () => {
        ...
        const container = renderChatEditor({});
        const editor = container.querySelector('[data-web-shell-composer-editor]');
        const drop = dispatchDrag(editor, 'drop', ['Files'], [new File(['png'], 'photo.png', { type: 'image/png' })]);
        expect(drop.defaultPrevented).toBe(true);
        expect(composerCoreState.imageDropCapture).toHaveBeenCalledTimes(1);
        expect(workspace.client.uploadWorkspaceFile).not.toHaveBeenCalled();
    });
    it('inserts a successful upload as an inline file tag', async () => {
        ...
        const row = container.querySelector('[data-web-shell-upload-strip] [data-status="done"]');
    ...
    it('falls back to the 50 MiB default when a capable daemon omits the limit', async () => {
        ...
        dispatchDrag(editor, 'drop', ['Files'], [oversized, new File(['ok'], 'small.txt')]);
        await act(async () => { });
        const strip = container.querySelector('[data-web-shell-upload-strip]');
        expect(strip?.textContent).toContain('File exceeds the 50 MiB upload limit');
        expect(workspace.client.uploadWorkspaceFile).toHaveBeenCalledTimes(1);
        expect(workspace.client.uploadWorkspaceFile.mock.calls[0][0].path).toBe('small.txt');
    });
    it('skips dropped folders instead of publishing phantom files', async () => {
        ...
        const drop = dispatchDrag(editor, 'drop', ['Files'], [folder, file], [{ file: folder, isDirectory: true }, { file }]);
        await act(async () => { });
        expect(drop.defaultPrevented).toBe(true);
        expect(workspace.client.uploadWorkspaceFile).toHaveBeenCalledTimes(1);
        expect(workspace.client.uploadWorkspaceFile.mock.calls[0][0].path).toBe('data.csv');
    ...
    it('routes folder-only drops to the image lane without uploading', () => {
        ...
        const editor = container.querySelector('[data-web-shell-composer-editor]');
        const folder = new File([], 'myfolder');
        dispatchDrag(editor, 'drop', ['Files'], [folder], [{ file: folder, isDirectory: true }]);
        expect(composerCoreState.imageDropCapture).toHaveBeenCalledTimes(1);
        expect(workspace.client.uploadWorkspaceFile).not.toHaveBeenCalled();
    });
    it('gates the composer submit while an upload is in flight', async () => {
        const workspace = makeWorkspace(['workspace_file_upload']);
        let resolveUpload;
        workspace.client.uploadWorkspaceFile.mockReturnValue(new Promise((resolve) => {
    ...
    describe('qualified upload targeting', () => {
        const makeQualifiedWorkspace = (features, workspaces) => {
        ...
        it('routes the upload through the qualified client for a trusted cwd match', async () => {
            const { workspace, qualifiedClient, workspaceByCwd } = makeQualifiedWorkspace(qualifiedFeatures, primaryAndSecondary);
            uploadWorkspaceState.current = workspace;
        ...
        it('stays disabled without the qualified-rest capability', () => {
            const { workspace } = makeQualifiedWorkspace(['workspace_file_upload'], primaryAndSecondary);
            uploadWorkspaceState.current = workspace;
            const container = renderChatEditor({ atWorkspaceCwd: '/secondary' });
            expect(container.querySelector('[data-web-shell-upload-input]')).toBeNull();
        });
        ...
        it('blocks a picker upload after the target workspace switched', async () => {
            const { workspace, qualifiedClient } = makeQualifiedWorkspace(qualifiedFeatures, primaryAndSecondary);
            uploadWorkspaceState.current = workspace;
            const container = renderChatEditor({});
            const input = container.querySelector('[data-web-shell-upload-input]');
...
//# sourceMappingURL=ChatEditor.test.js.map