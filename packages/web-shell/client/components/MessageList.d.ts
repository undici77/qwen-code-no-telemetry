...
import { type TurnOutputFileChange, type TurnOutputOpenRequest, type TurnOutputScheduledTask } from './artifacts/TurnOutputs';
...
export type TurnTimelineNodeKind = 'thought' | 'commentary' | 'tool' | 'agents' | 'plan' | 'status' | 'none';
...
export declare function attachTurnOutputs(items: DisplayItem[], isResponding: boolean, turnFileChanges?: ReadonlyMap<string, readonly TurnOutputFileChange[]>, turnArtifacts?: ReadonlyMap<string, readonly DaemonSessionArtifact[]>, turnScheduledTasks?: ReadonlyMap<string, readonly TurnOutputScheduledTask[]>): DisplayItem[];
export declare function pinActiveParallelAgentsToTurnEnd(items: DisplayItem[], automaticallyExpandedKeys?: ReadonlySet<string>): DisplayItem[];
...
export declare function getTurnTimelineNode(item: DisplayItem, t?: (key: string, vars?: Record<string, string | number>) => string): TurnTimelineNode;
export declare function getSessionTimelineEntries(messages: readonly Message[], t?: (key: string, vars?: Record<string, string | number>) => string): SessionTimelineEntry[];
export declare function getSessionTimelineSignature(messages: readonly Message[]): string;
/**
 ...
 */
export declare function findTurnIdForIndex(items: readonly DisplayItem[], index: number): string | null;
export declare function getTurnIdByDisplayIndex(items: readonly DisplayItem[]): Array<string | null>;
export declare function getSessionTimelineRangeForIndexes(visibleItems: readonly DisplayItem[], visibleItemIndexes: readonly number[], entryIndexById: ReadonlyMap<string, number>, currentItemIndex?: number | null, turnIdByDisplayIndex?: readonly (string | null)[]): SessionTimelineRange | null;
export declare function applyTurnCollapse(items: DisplayItem[], { overrides, isResponding, activeTurnStartedAt, backgroundSummaryGraceActive, automaticallyExpandedAgentKeys, pendingApprovalCallId, includeSubagentToolUsageInMetrics, enabled, }: ApplyTurnCollapseOptions): DisplayItem[];
/**
 ...
 */
export declare function findDisplayItemIndex(items: readonly DisplayItem[], messageId: string, callId?: string): number;
...
export declare function shouldUseVirtualScroll(totalCount: number, threshold?: number): boolean;
export declare function shouldAdjustVirtualScrollPosition(itemEnd: number, scrollOffset: number): boolean;
export declare const MessageList: import("react").NamedExoticComponent<MessageListProps & import("react").RefAttributes<MessageListHandle>>;
export {};
