...
describe('WebShellTranscript DOM integration', () => {
    it('renders representative transcript blocks without daemon providers', () => {
        ...
        const { container } = render(_jsx(WebShellTranscript, { blocks: blocks, collapseCompletedTurns: false, language: "en" }));
        expect(container.textContent).toContain('Inspect the project');
        expect(container.textContent).not.toContain('Thinking through it');
        const thinkingToggle = container.querySelector('button[title="Expand thinking"]');
    ...
    it('derives task detail from todo_write transcript snapshots', () => {
        const todoSnapshot = (id, status, timestamp, stats) => block({
            id,
            kind: 'tool',
            toolCallId: `${id}-call`,
            title: 'Updated Plan',
        ...
        const { container } = render(_jsx(WebShellTranscript, { blocks: [
            ...
            ], collapseCompletedTurns: false, language: "en" }));
        const summaries = Array.from(container.querySelectorAll('button')).filter((button) => button.textContent?.includes('Updated task list'));
        expect(summaries).toHaveLength(2);
        act(() => {
            summaries[1].click();
        });
        const completedSnapshot = summaries[1].parentElement;
        const detailButton = completedSnapshot?.querySelector('button[title="Show task detail"]');
    ...
    it('derives the transition introduced by each plan snapshot', () => {
        const planSnapshot = (id, firstStatus, secondStatus, timestamp) => block({
            id,
            kind: 'status',
            text: `plan: ${JSON.stringify({
                sessionUpdate: 'plan',
        ...
        const planHeaders = Array.from(container.querySelectorAll('button')).filter((button) => button.textContent?.includes('Plan'));
    ...
    it('renders the historical result when AskUserQuestion has a tool block', () => {
        ...
        const { container } = render(_jsx(WebShellTranscript, { blocks: blocks, collapseCompletedTurns: false }));
    ...
    it('suppresses session and goal events while preserving their text', () => {
        ...
        const sessionText = Array.from(container.querySelectorAll('span')).find((element) => element.textContent === 'Open child');
    ...
    it('mounts a themed scoped portal root and removes it on unmount', () => {
        const { container, unmount } = render(_jsx(WebShellTranscript, { blocks: [], theme: "light", language: "zh-CN" }));
        ...
        unmount();
        expect(document.body.querySelector('[data-web-shell-portal-root]')).toBeNull();
    });
