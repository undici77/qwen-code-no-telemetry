...
export function SpecularComposerEffect({ targetRef, }) {
    ...
    useEffect(() => {
        ...
        const isEditorTarget = (eventTarget) => {
            const editor = target.querySelector('[data-web-shell-composer-editor]');
            return (eventTarget instanceof Node && Boolean(editor?.contains(eventTarget)));
        };
        const startLoop = () => {
            if (!running && !disposed) {
                running = true;
        ...
        const draw = (now) => {
            ...
            const targetAngle = focused
                ...
                : pointerAngle === null
                    ? angle
                    : pointerAngle;
            const difference = ((((targetAngle - angle + Math.PI) % (Math.PI * 2)) + Math.PI * 2) %
                (Math.PI * 2)) -
                Math.PI;
            ...
            gl.uniform3f(lineColor, theme === WebShellThemeId.Light ? 0.2 : 0.403922, theme === WebShellThemeId.Light ? 0.360784 : 0.521569, 1);
    ...
    return (_jsx("span", { ref: hostRef, className: styles.specularEffect, "data-web-shell-composer-specular": true, "data-theme": theme, "aria-hidden": "true" }));
}
//# sourceMappingURL=SpecularComposerEffect.js.map