...
describe('composer visual effects fallback', () => {
    it('keeps both animation layers inert when canvas contexts are unavailable', () => {
        ...
        function Harness() {
            ...
            return (_jsxs(ThemeProvider, { value: WebShellThemeId.Light, children: [_jsxs("div", { ref: composerRef, "data-web-shell-composer-surface": true, children: [_jsx(SpecularComposerEffect, { targetRef: composerRef }), _jsx("div", { "data-web-shell-composer-editor": true })] }), _jsx(NewSessionDotField, {})] }));
        }
        ...
        expect(container.querySelector('[data-web-shell-composer-specular] canvas')).toBeNull();
        expect(container.querySelector('[data-web-shell-new-session-dot-field] canvas')).not.toBeNull();
    ...
    it('erases dots for the pointer glow regardless of the background color', () => {
        ...
        Object.defineProperty(contextStub, 'globalCompositeOperation', {
            ...
            set: (value) => compositeOperations.push(value),
        });
        Object.defineProperty(contextStub, 'globalAlpha', {
            configurable: true,
            get: () => globalAlphas.at(-1) ?? 1,
            set: (value) => globalAlphas.push(value),
        });
        ...
        const root = createRoot(container);
        mounted.push({ container, root });
        act(() => root.render(_jsx(ThemeProvider, { value: WebShellThemeId.Light, children: _jsx(NewSessionDotField, {}) })));
    ...
    it('cancels the dot field interval and animation frame on unmount', () => {
        ...
        document.body.appendChild(container);
        const root = createRoot(container);
        act(() => root.render(_jsx(ThemeProvider, { value: WebShellThemeId.Dark, children: _jsx(NewSessionDotField, {}) })));
        act(() => root.unmount());
        container.remove();
...
describe('specular effect idle bail-out', () => {
    it('stops the rAF loop only after brightness decays below the threshold', () => {
        ...
        function Harness() {
            const composerRef = useRef(null);
            return (_jsx(ThemeProvider, { value: WebShellThemeId.Dark, children: _jsxs("div", { ref: composerRef, "data-web-shell-composer-surface": true, children: [_jsx(SpecularComposerEffect, { targetRef: composerRef }), _jsx("div", { "data-web-shell-composer-editor": true })] }) }));
        }
        const container = document.createElement('div');
        document.body.appendChild(container);
        const root = createRoot(container);
        ...
        const drainFrames = (count, stepMs) => {
        ...
        expect(clear.mock.calls.length).toBeGreaterThan(drawArrays.mock.calls.length);
...
describe('specular effect WebGL cleanup', () => {
    it('removes a lost canvas and releases WebGL resources on unmount', () => {
        ...
        const root = createRoot(container);
        act(() => root.render(_jsx(Harness, {})));
        expect(container.querySelector('[data-web-shell-composer-specular] canvas')).not.toBeNull();
        const removeWindowListenerSpy = vi.spyOn(window, 'removeEventListener');
        const removeDocumentListenerSpy = vi.spyOn(document.documentElement, 'removeEventListener');
        const canvas = container.querySelector('[data-web-shell-composer-specular] canvas');
        ...
        expect(removeDocumentListenerSpy).toHaveBeenCalledWith('pointerleave', expect.any(Function));
        expect(container.querySelector('[data-web-shell-composer-specular] canvas')).toBeNull();
        requestAnimationFrameSpy.mockClear();
        act(() => {
            window.dispatchEvent(new MouseEvent('pointermove', { clientX: 100, clientY: 100 }));
...
describe('dot field static grid and idle loop', () => {
    const mockDotsRect = () => vi.spyOn(Element.prototype, 'getBoundingClientRect').mockReturnValue({
        width: 30,
        height: 30,
        top: 0,
        left: 0,
...
//# sourceMappingURL=SpecularComposerEffect.test.js.map