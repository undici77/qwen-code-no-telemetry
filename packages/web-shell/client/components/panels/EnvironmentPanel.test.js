...
vi.mock('../BranchPickerPopover', async () => {
    ...
    return {
        BranchPickerPopover: ({ children, open, onOpenChange, side, }) => createElement('div', {
...
function mount(props = {}) {
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(EnvironmentPanel, { workspaceCwd: "/work/qwen-code", branch: "feat/context-panels", gitStatus: {
                    v: 2,
                    ...
                    unstaged: 2,
                    ahead: 1,
                }, tasks: [], onOpenGitDiff: vi.fn(), onOpenAgent: vi.fn(), onOpenTask: vi.fn(), ...props }) }));
    });
    return container;
}
function toggleSection(view, label) {
    const button = Array.from(view.querySelectorAll('button[aria-expanded]')).find((candidate) => candidate.textContent?.includes(label));
    act(() => button?.click());
}
describe('EnvironmentPanel', () => {
    ...
    it('opens the branch actions without dismissing a floating panel', () => {
        ...
        const branchButton = Array.from(view.querySelectorAll('button')).find((button) => button.textContent?.includes('feat/context-panels'));
    ...
    it('shows agent, shell, and monitor task activity', () => {
        ...
        expect(view.querySelectorAll('button[aria-expanded="false"]')).toHaveLength(2);
        expect(view.textContent).not.toContain('Explore code');
        toggleSection(view, 'Subagents');
        toggleSection(view, 'Background tasks');
        expect(view.querySelectorAll('button[aria-expanded="true"] svg')).toHaveLength(0);
        ...
        const agentItem = Array.from(view.querySelectorAll('ul button')).find((button) => button.textContent?.includes('Explore code'));
        expect(agentItem?.firstElementChild?.querySelector('svg')).toBeNull();
        expect(agentItem?.querySelector('[data-status="completed"] svg.lucide-circle-check')).not.toBeNull();
        const completedItem = Array.from(view.querySelectorAll('ul button')).find((button) => button.textContent?.includes('npm run build'));
        expect(completedItem?.querySelector('svg.lucide-square-terminal')).not.toBeNull();
        expect(completedItem?.querySelector('[title="npm run build"]')).not.toBeNull();
        expect(completedItem?.querySelector('[data-status="failed"] svg.lucide-circle-x')).not.toBeNull();
        const monitorItem = Array.from(view.querySelectorAll('ul button')).find((button) => button.textContent?.includes('Development server'));
        expect(monitorItem?.querySelector('svg.lucide-square-activity')).not.toBeNull();
        expect(monitorItem?.querySelector('[data-status="running"] svg.lucide-loader-circle')).not.toBeNull();
        const cancelledItem = Array.from(view.querySelectorAll('ul button')).find((button) => button.textContent?.includes('sleep 300'));
        expect(cancelledItem?.querySelector('[data-status="cancelled"] svg.lucide-circle-stop')).not.toBeNull();
    });
    it('opens an out-of-band fork without a tool call id', () => {
        ...
        toggleSection(view, 'Subagents');
        const item = Array.from(view.querySelectorAll('ul button')).find((button) => button.textContent?.includes('Review current changes'));
    ...
    it('shows the configured subagent color as a leading dot', () => {
        ...
        toggleSection(view, 'Subagents');
        const color = view.querySelector('[data-agent-color="purple"]');
    ...
    it('shows a gray dot when the subagent has no configured color', () => {
        ...
        toggleSection(view, 'Subagents');
        const color = view.querySelector('[data-agent-color="default"]');
        expect(color).not.toBeNull();
    ...
    it('opens the existing environment actions', () => {
        ...
        act(() => {
            for (const button of view.querySelectorAll('button[aria-expanded="false"]')) {
                button.click();
            }
        });
        act(() => {
            for (const button of view.querySelectorAll('ul button')) {
                button.click();
            }
        });
...
//# sourceMappingURL=EnvironmentPanel.test.js.map