...
vi.mock('../BranchPickerPopover', async () => {
  ...
  return {
    BranchPickerPopover: ({ children, open, onOpenChange, side }) =>
      createElement(
        'div',
        {
...
function mount(props = {}) {
  ...
  act(() => {
    root.render(
      _jsx(I18nProvider, {
        language: 'en',
        children: _jsx(EnvironmentPanel, {
...
function toggleSection(view, label) {
  const button = Array.from(
    view.querySelectorAll('button[aria-expanded]'),
...
describe('EnvironmentPanel', () => {
  ...
  it('opens an out-of-band fork without a tool call id', () => {
    ...
    toggleSection(view, 'Subagents');
    const item = Array.from(view.querySelectorAll('ul button')).find((button) =>
      button.textContent?.includes('Review current changes'),
    );
