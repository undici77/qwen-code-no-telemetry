...
describe('TodoPanel bottom status items', () => {
    it('opens plan execution from the progress control', () => {
        const onOpen = vi.fn();
        const container = render(_jsx(TodoPanel, { todos: [todo('1', 'in_progress')], onOpen: onOpen }));
    ...
    it('renders status items beside todo progress with a separator', () => {
        const onClick = vi.fn();
        const container = render(_jsx(TodoPanel, { todos: [todo('1', 'in_progress'), todo('2', 'pending')], statusItems: [
                {
                    id: 'changed-files',
        ...
        const button = container.querySelector('button[title="Open changed files"]');
    ...
    it('can render status-only content without todo progress or detail tooltip', () => {
        const container = render(_jsx(TodoPanel, { todos: [], statusItems: [
                {
                    id: 'changed-files',
        ...
        expect(container.querySelector('section')?.getAttribute('aria-label')).toBe('Open changed files');
