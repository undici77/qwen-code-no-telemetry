...
vi.mock('./ChatPane', () => ({
  ChatPane: (props) => {
    ...
    return _jsxs('div', {
      ...
      'data-report-catalog-turn-completion': props.reportCatalogTurnCompletion
        ? 'true'
        : 'false',
      ...
      children: [
        _jsx('span', { 'data-testid': 'pane-title', children: props.title }),
        props.renderHeaderActions &&
          _jsx('span', {
            'data-testid': 'pane-header-slot',
            children: props.renderHeaderActions({
...
function render(props = {}) {
  ...
  act(() =>
    root.render(
      _jsx(I18nProvider, {
        language: 'en',
        children: _jsx(SplitView, { onExit: () => {}, ...props }),
      }),
    ),
  );
}
  ...
  it('syncs panes when session ids change after mount', () => {
    ...
    act(() =>
      root.render(
        _jsx(I18nProvider, {
          language: 'en',
          children: _jsx(SplitView, { onExit: () => {}, sessionIds: [] }),
        }),
      ),
    );
  ...
  it('drops maximize when a controlled sync removes the maximized pane', () => {
    ...
    act(() =>
      root.render(
        _jsx(I18nProvider, {
          language: 'en',
          children: _jsx(SplitView, { onExit: () => {}, sessionIds: ['s2'] }),
        }),
      ),
    );
    ...
    const rerender = (ids) =>
      act(() =>
        root.render(
          _jsx(I18nProvider, {
            language: 'en',
            children: _jsx(SplitView, { onExit: () => {}, sessionIds: ids }),
          }),
        ),
      );
    render({ sessionIds: ['s1', 's2'] });
  ...
  it('passes renderPaneHeaderActions through to each ChatPane', () => {
    render({
      ...
      renderPaneHeaderActions: (info) =>
        _jsx('button', {
          type: 'button',
          'data-testid': `host-${info.sessionId}`,
          children: 'host',
        }),
    });
    expect(
