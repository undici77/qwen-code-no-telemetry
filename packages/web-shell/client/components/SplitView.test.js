...
vi.mock('@qwen-code/webui/daemon-react-sdk', () => ({
    DaemonSessionProvider: (props) => (_jsx("div", { "data-session": props.sessionId, "data-clientid": props.clientId, "data-workspace": props.workspaceCwd, "data-restart-sse": props.restartEventStreamOnPrompt ? 'true' : 'false', children: props.children })),
    ...
    useSessions: () => {
        const [sessions, setSessions] = React.useState(() => sessionsState);
...
vi.mock('../hooks/useScopedSessions', () => ({
    useScopedSessions: (workspaceCwd) => {
        const [sessions, setSessions] = React.useState(() => workspaceCwd ? [] : sessionsState);
        React.useEffect(() => {
            if (!workspaceCwd)
                return;
...
vi.mock('./ChatPane', () => ({
    ChatPane: (props) => {
        ...
        return (_jsxs("div", { "data-testid": "chat-pane", "data-pane-workspace": props.workspaceCwd, "data-maximized": props.isMaximized ? 'true' : 'false', "data-pane-restart-sse": props.restartSseOnPrompt ? 'true' : 'false', "data-slash-handler": props.onSlashCommand ? 'true' : 'false', "data-hidden": props.hidden ? 'true' : 'false', "data-report-catalog-turn-completion": props.reportCatalogTurnCompletion ? 'true' : 'false', "data-voice-user-revision": String(props.voiceUserRevision ?? 0), "data-voice-workspace-count": String(props.voiceWorkspaces?.length ?? 0), children: [_jsx("span", { "data-testid": "pane-title", children: props.title }), props.renderHeaderActions && (_jsx("span", { "data-testid": "pane-header-slot", children: props.renderHeaderActions({
                        sessionId: 'from-props',
                        workspaceCwd: props.workspaceCwd,
                    }) })), props.onToggleMaximize && (_jsx("button", { "data-testid": "pane-maximize", onClick: props.onToggleMaximize, children: "max" })), props.onClose && (_jsx("button", { "data-testid": "pane-close", onClick: props.onClose, children: "x" }))] }));
    },
}));
...
beforeEach(() => {
    ...
    workspaceClient = {
        listWorkspaceSessionsPage: vi.fn(async (cwd) => ({
            sessions: otherWorkspaceSessions[cwd] ?? [],
        })),
        workspaceByCwd: vi.fn((cwd) => ({
...
function render(props = {}) {
    ...
    act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SplitView, { onExit: () => { }, ...props }) })));
}
function panes() {
    return Array.from(container.querySelectorAll('[data-testid="chat-pane"]'));
}
function titles() {
    return Array.from(container.querySelectorAll('[data-testid="pane-title"]')).map((el) => el.textContent ?? '');
}
function pickerOptions() {
    return Array.from(container.querySelectorAll('[role="option"] button')).map((el) => (el.textContent ?? '').trim());
}
function openPicker() {
    const addButton = container.querySelector('button[aria-haspopup="listbox"]');
...
describe('SplitView', () => {
    ...
    it('leaves the outer session as the sole catalog turn-completion owner', () => {
        ...
        expect(panes()[0]?.getAttribute('data-report-catalog-turn-completion')).toBe('false');
        expect(panes()[1]?.getAttribute('data-report-catalog-turn-completion')).toBe('true');
    });
    it('reports completion for the same session id in another workspace', async () => {
        ...
        await flushAsync();
        expect(panes()[0]?.getAttribute('data-report-catalog-turn-completion')).toBe('true');
    });
    it('passes the prompt SSE restart option to pane providers', () => {
        ...
        expect(container
            ...
            ?.getAttribute('data-restart-sse')).toBe('true');
        expect(container
            .querySelector('[data-session="s1"] [data-testid="chat-pane"]')
            ?.getAttribute('data-pane-restart-sse')).toBe('true');
    });
    it('passes the host slash command handler to every pane', () => {
        ...
        expect(panes().every((pane) => pane.getAttribute('data-slash-handler') === 'true')).toBe(true);
    });
    ...
    it('syncs panes when session ids change after mount', () => {
        ...
        act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SplitView, { onExit: () => { }, sessionIds: ['s1', 's2'] }) })));
        expect(titles()).toEqual(['One', 'Two']);
        act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SplitView, { onExit: () => { }, sessionIds: [] }) })));
    ...
    it('requests pane changes without mutating local panes when controlled', () => {
        ...
        expect(titles()).toEqual(['One']);
        act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SplitView, { onExit: () => { }, sessionIds: ['s1', 's2'], onPanesChange: onPanesChange }) })));
        expect(titles()).toEqual(['One', 'Two']);
    ...
    it('keeps the picker open on a click inside it', () => {
        ...
        act(() => listbox.dispatchEvent(new MouseEvent('mousedown', { bubbles: true })));
        expect(addButton.getAttribute('aria-expanded')).toBe('true');
        expect(container.querySelector('[role="listbox"]')).not.toBeNull();
    });
    ...
    it('exits via the back button', () => {
        ...
        const back = container.querySelector('header button');
    ...
    function maximizeButtons() {
        return Array.from(container.querySelectorAll('[data-testid="pane-maximize"]'));
    }
    ...
    it('maximizing a pane hides the others but keeps them all mounted', () => {
        ...
        act(() => maximizeButtons()[0].dispatchEvent(new MouseEvent('click', { bubbles: true })));
        ...
        expect(maximized?.querySelector('[data-testid="pane-title"]')?.textContent).toBe('One');
    });
    it('forwards Voice revision state to every pane', () => {
        ...
        expect(container.querySelectorAll('[data-voice-user-revision="4"]')).toHaveLength(2);
    });
    it('forwards the merged Voice workspace list to every pane', () => {
        ...
        expect(container.querySelectorAll('[data-voice-workspace-count="2"]')).toHaveLength(2);
    });
    ...
    it('moves maximize to another pane when its toggle is clicked', () => {
        ...
        act(() => maximizeButtons()[1].dispatchEvent(new MouseEvent('click', { bubbles: true })));
        maximized = container.querySelector('[data-maximized="true"]');
        expect(maximized?.querySelector('[data-testid="pane-title"]')?.textContent).toBe('Two');
        // Exactly one pane maximized, the other two hidden.
        expect(container.querySelectorAll('[data-maximized="true"]')).toHaveLength(1);
        expect(hiddenSlots()).toHaveLength(2);
    });
    ...
    it('drops maximize when a controlled sync removes the maximized pane', () => {
        ...
        act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SplitView, { onExit: () => { }, sessionIds: ['s2'] }) })));
    ...
    it('clears a stale maximize when a controlled split shrinks to one pane then regrows', () => {
        const rerender = (ids) => act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SplitView, { onExit: () => { }, sessionIds: ids }) })));
        render({ sessionIds: ['s1', 's2'] });
        act(() => maximizeButtons()[0].dispatchEvent(new MouseEvent('click', { bubbles: true })));
    ...
    it('offers other trusted workspaces’ sessions in the picker, tagged by workspace', async () => {
        ...
        expect(options.some((o) => o.includes('Beta') && o.includes('Payments API'))).toBe(true);
    ...
    it('attaches an added other-workspace pane under its own workspace cwd', async () => {
        ...
        openPicker();
        await flushAsync(); // opening the picker re-fires reload()/reloadOther()
        const betaButton = Array.from(container.querySelectorAll('[role="option"] button')).find((el) => (el.textContent ?? '').includes('Beta'));
        act(() => betaButton.dispatchEvent(new MouseEvent('click', { bubbles: true })));
        const providerFor = (id) => Array.from(container.querySelectorAll('[data-session]')).find((el) => el.getAttribute('data-session') === id);
        ...
        expect(providerFor('b1')
            ?.querySelector('[data-testid="chat-pane"]')
            ?.getAttribute('data-pane-workspace')).toBe('/wsB');
    });
    it('loads and attaches only the locked secondary workspace', async () => {
        ...
        expect(container
            ...
            ?.getAttribute('data-workspace')).toBe('/wsB');
        openPicker();
        expect(pickerOptions().some((option) => option.includes('One'))).toBe(false);
    });
    ...
    it('re-queries other workspaces when the picker opens', async () => {
        ...
        expect(workspaceClient.listWorkspaceSessionsPage.mock.calls.length).toBeGreaterThan(before);
    });
    it('passes renderPaneHeaderActions through to each ChatPane', () => {
        render({
            sessionIds: ['s1', 's2'],
            renderPaneHeaderActions: (info) => (_jsx("button", { type: "button", "data-testid": `host-${info.sessionId}`, children: "host" })),
        });
        expect(container.querySelectorAll('[data-testid="pane-header-slot"]')).toHaveLength(2);
        ...
        expect(container.querySelectorAll('[data-testid="host-from-props"]')).toHaveLength(2);
    });
});
