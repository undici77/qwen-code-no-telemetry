...
import { forwardRef, memo, useContext, useEffect, useImperativeHandle, useLayoutEffect, useRef, useCallback, useMemo, useState, useTransition, } from 'react';
...
import { isAskUserQuestionToolName, isActiveToolStatus, toolContainsCallId, } from './messages/toolFormatting';
...
function isStandaloneToolGroup(msg) {
    return (msg.role === 'tool_group' &&
        msg.tools.some((tool) => isSubAgentToolCall(tool) ||
...
export function pinActiveParallelAgentsToTurnEnd(items, automaticallyExpandedKeys) {
    const shouldPin = (item) => item.type === 'parallel_agents' &&
...
function isFinalContentCandidate(item, includeBackgroundNotifications) {
...
function findFinalAnswerIndex(items, start, end, includeBackgroundNotifications = true) {
...
export function getSessionTimelineEntries(messages, t) {
    ...
    const pushTurn = () => {
        ...
        const finalAssistantIndex = findFinalAnswerIndex(timelineItems, -1, timelineItems.length - 1, false);
...
function subagentUsage(tool) {
    if (tool.parentToolCallId || !isSubAgentToolCall(tool))
        return undefined;
    const raw = tool.rawOutput && typeof tool.rawOutput === 'object'
        ? tool.rawOutput
        : undefined;
    const summary = raw?.['executionSummary'] && typeof raw['executionSummary'] === 'object'
...
function itemSubagentUsages(item) {
    if (item.type === 'parallel_agents') {
...
function someTurnToolCall(items, start, end, pred) {
    for (let i = start; i <= end; i++) {
        const item = items[i];
        if (item.type === 'parallel_agents') {
            if (item.agents.some(pred))
                return true;
        }
        else if (item.type === 'message' && item.message.role === 'tool_group') {
            if (item.message.tools.some(pred))
                return true;
        }
    }
    return false;
...
function turnOwnsCallId(items, start, end, callId) {
    if (!callId)
        return false;
    return someTurnToolCall(items, start, end, (tool) => toolContainsCallId(tool, callId));
}
function turnHasActiveAgent(items, start, end) {
    return someTurnToolCall(items, start, end, (tool) => isSubAgentToolCall(tool) && isActiveToolStatus(tool.status));
}
function turnHasAutomaticallyExpandedAgent(items, start, end, automaticallyExpandedAgentKeys) {
    if (!automaticallyExpandedAgentKeys?.size)
        return false;
    for (let i = start; i <= end; i++) {
        const item = items[i];
        if (item.type === 'parallel_agents' &&
...
function backgroundAgentCallIds(item) {
    ...
    if (item.type === 'parallel_agents') {
        return item.agents
            .filter((agent) => isBackgroundSubAgentToolCall(agent) && agent.status !== 'failed')
            .map((agent) => agent.callId);
    }
    if (item.type === 'message' && item.message.role === 'tool_group') {
        return item.message.tools
            .filter((tool) => isBackgroundSubAgentToolCall(tool) && tool.status !== 'failed')
            .map((tool) => tool.callId);
    }
    return [];
}
function backgroundAgentCompletion(item) {
    if (item.type !== 'message' ||
        item.message.role !== 'system' ||
        item.message.source !== 'background_notification') {
        return null;
    }
    const identifiesAgent = item.message.content
        ?.trimStart()
        .toLowerCase()
    ...
    const { kind, toolUseId } = data;
    if (kind !== 'agent' && !(kind === undefined && identifiesAgent))
        return null;
    return typeof toolUseId === 'string' ? { callId: toolUseId } : {};
}
function turnAwaitsBackgroundSummary(items, start, end, agentNotificationsOnly = false) {
    let lastNotificationIndex = -1;
    let latestNotificationAgentCallId;
    for (let i = end; i > start; i--) {
        const item = items[i];
        if (item.type !== 'message' || item.message.role !== 'system')
            continue;
        if (item.message.source === 'turn_error' ||
        ...
        if (item.message.source === 'background_notification') {
            ...
            if (agentNotificationsOnly && !completion)
                continue;
            if (lastNotificationIndex < 0) {
    ...
    if (latestNotificationAgentCallId) {
        for (let i = end; i >= 0; i--) {
            if (backgroundAgentCallIds(items[i]).includes(latestNotificationAgentCallId)) {
    ...
    if (latestAgentLaunchIndex >= 0) {
        ...
        const unmatchedAgentCallIds = new Set();
        const anonymousCompletionCandidates = [];
        ...
        for (const candidates of anonymousCompletionCandidates) {
            for (const callId of candidates) {
                if (!unmatchedAgentCallIds.has(callId))
                    continue;
...
export function applyTurnCollapse(items, { overrides, isResponding, activeTurnStartedAt, backgroundSummaryGraceActive = true, automaticallyExpandedAgentKeys, pendingApprovalCallId, includeSubagentToolUsageInMetrics = true, enabled, }) {
    ...
    for (let k = 0; k < userIdxs.length; k++) {
        ...
        const hasAutomaticallyExpandedAgent = turnHasAutomaticallyExpandedAgent(items, start, end, automaticallyExpandedAgentKeys);
        const awaitsBackgroundSummary = isLastTurn &&
        ...
        let thinkingCount = 0;
        let hasUsage = false;
        const countedSubagents = new Set();
        ...
        for (let i = start + 1; i <= end; i++) {
            ...
            if (includeSubagentToolUsageInMetrics) {
                for (const subagent of itemSubagentUsages(item)) {
                    if (countedSubagents.has(subagent.callId))
                        continue;
        ...
        const liveStartedAt = isActiveTurn || awaitsBackgroundSummary
        ...
        // always wins.
        const shouldStayOpen = isActiveTurn ||
            hasActiveAgent ||
...
export function shouldAdjustVirtualScrollPosition(itemEnd, scrollOffset) {
...
function tokenMetricText(collapse, t) {
    ...
    const cached = cachedTokens > 0 && collapse.inputTokens > 0
        ? ` (${formatContextTokens(cachedTokens)} ${t('turn.cached')}, ${Math.round((cachedTokens / collapse.inputTokens) * 100)}%)`
        : '';
    return `↑${formatContextTokens(collapse.inputTokens)}${cached} ↓${formatContextTokens(collapse.outputTokens)}`;
}
...
export const MessageList = memo(forwardRef(function MessageList({ messages, pendingApproval, onShowContextDetail, onImagePreview, loadingTranscript, catchingUp, hasOlderHistory = false, loadingOlderHistory = false, historyCapacityReached = false, historyPaginationError = false, onLoadOlderHistory, transcriptBlockCount = 0, transcriptActivity, onReloadTranscript, isResponding = false, activeTurnStartedAt, welcomeHeader, centerWelcomeHeader = false, workspaceCwd, tailContent, tailKey = 'tail', virtualScrollThreshold = VIRTUAL_SCROLL_THRESHOLD, autoScrollTailIntoView = false, bottomOverlayInset = 0, hideSessionTimeline = false, hideFirstUserMessage = false, firstTurnMetrics, includeSubagentToolUsageInMetrics = true, showRetryHint = false, onRetryClick, failedPromptMessageId, onRetryFailedPrompt, onBranchSession, onCanScrollToBottomChange, turnFileChanges, turnArtifacts, turnScheduledTasks, onReviewChanges, onOpenArtifact, onOpenScheduledTask, onTurnOutputOpen, onError, generateContent, }, ref) {
    const { t } = useI18n();
    ...
    const [backgroundNotificationBaselineId, setBackgroundNotificationBaselineId,] = useState(latestBackgroundNotificationId);
    const loadingBackgroundNotificationHistory = Boolean(loadingTranscript || catchingUp);
    const wasLoadingBackgroundNotificationHistory = useRef(loadingBackgroundNotificationHistory);
    const hasEstablishedBackgroundNotificationBaseline = useRef(mergedMessages.length > 0);
    const firstMessageBatchEstablishesBackgroundNotificationBaseline = !hasEstablishedBackgroundNotificationBaseline.current &&
        mergedMessages.length > 0;
    const establishingBackgroundNotificationBaseline = loadingBackgroundNotificationHistory ||
    ...
    const backgroundSummaryGraceActive = !establishingBackgroundNotificationBaseline &&
    ...
    const latestTurnAwaitsAgentSummary = useMemo(() => backgroundSummaryGraceActive &&
        turnAwaitsBackgroundSummary(displayItems, latestTurnStartIndex, displayItems.length - 1, true), [backgroundSummaryGraceActive, displayItems, latestTurnStartIndex]);
    ...
    const backgroundSummaryAgentContext = useMemo(() => {
        ...
        for (let i = notificationIndex - 1; i >= 0; i -= 1) {
            ...
            if (!callId || item.agents.some((agent) => agent.callId === callId)) {
                return {
                    key: item.key,
                    agentNotificationIsLatestBackground: notificationIndex === latestBackgroundNotificationIndex,
                    latestBackgroundNotificationInLatestTurn: notificationIndex > latestTurnStartIndex,
                };
            }
        }
        return {
    ...
    const [automaticallyExpandedAgentKeys, setAutomaticallyExpandedAgentKeys] = useState(() => new Set());
    const handleAutomaticAgentExpansionChange = useCallback((key, expanded) => {
    ...
    const turnLayoutRowTops = useRef(new Map());
    const turnLayoutAnimationTimer = useRef(undefined);
    const turnLayoutAnimations = useRef([]);
    ...
    const olderHistoryAnchorFrame = useRef(undefined);
    const olderHistoryAnchorWaitFrame = useRef(undefined);
    const olderHistoryTopCheckFrame = useRef(undefined);
    const pendingOlderHistoryTopLoad = useRef(undefined);
    const reloadTranscriptTimer = useRef(undefined);
    const reloadTranscriptAbort = useRef(undefined);
    const transcriptReloadBaseline = useRef(undefined);
    ...
    const [suppressOlderHistoryLoadingStatus, setSuppressOlderHistoryLoadingStatus,] = useState(false);
    useEffect(() => {
        if (!hasOlderHistory) {
    ...
    const visibleItems = useMemo(() => {
        ...
        const itemsWithMetrics = firstTurnMetrics
            ? collapsedItems.map((item) => {
                if (metricsApplied || item.type !== 'turn_collapse')
                    return item;
                metricsApplied = true;
                return {
                    ...item,
            ...
            : collapsedItems;
        const pinnedItems = pinActiveParallelAgentsToTurnEnd(itemsWithMetrics, automaticallyExpandedAgentKeys);
        if (!hideFirstUserMessage)
            return pinnedItems;
        const firstUserId = mergedMessages.find((message) => message.role === 'user')?.id;
        return firstUserId
            ? pinnedItems.filter((item) => item.type !== 'message' || item.message.id !== firstUserId)
            : pinnedItems;
    }, [
        displayItems,
        collapseOverrides,
    ...
    const hasVisibleRowKey = useCallback((key) => visibleItemsRef.current.some((item) => String(getDisplayItemVirtualKey(item)) === key), []);
    ...
    const uncollapsedTotalCount = headerOffset + displayItems.length + (hasTailContent ? 1 : 0);
    const useVirtualScroll = shouldUseVirtualScroll(uncollapsedTotalCount, virtualScrollThreshold);
            ...
            void onReloadTranscript(controller.signal)
                ...
                transcriptReloadBaseline.current = {
                    ...(snapshot?.lastEventId !== undefined
                        ...
                        : {}),
                    blockCount: snapshot?.blocks?.length ?? transcriptBlockCountRef.current,
                };
            })
                .catch((error) => {
    ...
    const handleToggleCollapse = useCallback((turnId, nextExpanded) => {
        ...
        containerRef.current
            ?.querySelectorAll('[data-message-row-key]')
            .forEach((row) => {
            const key = row.dataset.messageRowKey;
            if (key) {
                turnLayoutRowTops.current.set(key, row.getBoundingClientRect().top);
            }
        });
    ...
    useLayoutEffect(() => {
        if (turnLayoutPending || !turnLayoutTransitionStarted.current)
            return;
        ...
        if (!reduceMotion) {
            containerRef.current
                ...
                if (previousTop === undefined) {
                    turnLayoutAnimations.current.push(row.animate([{ opacity: 0 }, { opacity: 1 }], {
                ...
                if (Math.abs(delta) < 1)
                    return;
                turnLayoutAnimations.current.push(row.animate([{ translate: `0 ${delta}px` }, { translate: '0 0' }], {
                    duration: TURN_LAYOUT_ANIMATION_MS,
                    easing: 'ease-out',
                }));
            });
    ...
    const scrollToBottom = useCallback((behavior = 'auto') => {
        ...
        const releaseCooldown = () => {
            ...
            if (!pendingBottomFollowAfterCooldown.current)
                return;
            pendingBottomFollowAfterCooldown.current = false;
            ...
            scheduleScrollOverflowReport();
            lastScrollTop.current = Math.max(0, current.scrollHeight - current.clientHeight);
            reportCanScrollToBottom();
        };
        if (behavior === 'smooth') {
    ...
    virtualizer.shouldAdjustScrollPositionOnItemSizeChange = (item, _delta, instance) => shouldAdjustVirtualScrollPosition(item.end, containerRef.current?.scrollTop ?? instance.scrollOffset ?? 0);
    const measureVirtualRow = useCallback((node) => {
        virtualizer.measureElement(node);
        if (!node || !restoringOlderHistoryRef.current)
            return;
    ...
    useLayoutEffect(() => {
        ...
        if (olderHistoryAnchor.rowKey &&
            !hasVisibleRowKey(olderHistoryAnchor.rowKey)) {
            if (olderHistoryAnchor.generation === olderHistoryLoadGeneration.current) {
        ...
        if (unchanged) {
            ...
            olderHistoryAnchorFrame.current = requestAnimationFrame(() => {
                ...
                olderHistoryLoadInFlight.current = false;
                setOlderHistoryAnchor((anchor) => anchor === olderHistoryAnchor ? null : anchor);
            });
            return;
        }
        if (olderHistoryAnchorFrame.current !== undefined) {
        ...
        olderHistoryAnchorFrame.current = requestAnimationFrame(() => {
            ...
            const anchorRow = olderHistoryAnchor.rowKey
                ? Array.from(current.querySelectorAll('[data-message-row-key]')).find((row) => row.dataset.messageRowKey === olderHistoryAnchor.rowKey)
                : undefined;
    ...
    const loadOlderHistory = useCallback(async (allowRetry = false, force = false) => {
        const el = containerRef.current;
        if (!el ||
            !onLoadOlderHistory ||
            loadingOlderHistory ||
        ...
        if (useVirtualScroll) {
            ...
            virtualAnchor = await new Promise((resolve) => {
                const waitForTopRange = () => {
                    ...
                    const firstMessageRow = Array.from(el.querySelectorAll('[data-message-row-key]')).find((row) => row.dataset.messageRowKey === firstMessageKey);
                    if (firstMessageRow) {
                        resolve({
        ...
        const anchorRow = virtualAnchor
            ? undefined
            : Array.from(el.querySelectorAll('[data-message-row-key]')).find((row) => {
                const key = row.dataset.messageRowKey;
        ...
        try {
            ...
            if (generation === olderHistoryLoadGeneration.current) {
                setOlderHistoryAnchor((anchor) => anchor?.generation === generation
                    ? { ...anchor, settled: true }
                    : anchor);
            }
        }
    ...
    useEffect(() => {
        ...
        const loadOlderHistoryAtTop = () => {
            ...
            if (el.scrollTop > LOAD_OLDER_HISTORY_THRESHOLD_PX)
                return;
        ...
        const scheduleOlderHistoryTopCheck = () => {
            ...
            olderHistoryTopCheckFrame.current = requestAnimationFrame(loadOlderHistoryAtTop);
        };
        ...
        const renderDisplayItem = (displayItem, isLatest) => {
            if (displayItem.type === 'parallel_agents') {
                return (_jsx(MessageTimestamp, { timestamp: displayItem.timestamp, children: _jsx("div", { className: displayItemMatchesLocateTarget(displayItem, flashTarget)
                            ? flashStyles.flash
                            : undefined, children: _jsx(ParallelAgentsGroup, { agents: displayItem.agents, autoManageExpansion: transcriptRenderMode === 'interactive' && !catchingUp, automaticCollapseDelayMs: backgroundSummaryAgentContext.key === displayItem.key &&
                                !latestTurnAwaitsAgentSummary
                                ? AGENT_SUMMARY_COLLAPSE_DELAY_MS
                                : undefined, deferAutomaticCollapse: (latestTurnParallelAgentKeys.has(displayItem.key) &&
                                isResponding &&
                                ...
                                (backgroundSummaryAgentContext.key === displayItem.key &&
                                    latestTurnAwaitsAgentSummary), expandActiveWhenLive: isResponding &&
                                latestTurnParallelAgentKeys.has(displayItem.key), onAutomaticExpansionChange: (expanded) => handleAutomaticAgentExpansionChange(displayItem.key, expanded), pendingApproval: pendingApproval }) }) }));
            }
            ...
            return (_jsx(MessageItem, { message: displayItem.message, pendingApproval: pendingApproval, onShowContextDetail: onShowContextDetail, onImagePreview: onImagePreview, workspaceCwd: workspaceCwd, isLatest: isLatest, showRetryHint: showRetryHint, onRetryClick: onRetryClick, sendFailed: displayItem.message.role === 'user' &&
                    displayItem.message.id === failedPromptMessageId, onRetrySend: onRetryFailedPrompt, onBranchSession: onBranchSession, showAssistantActions: displayItem.message.role === 'assistant' &&
    ...
    return (_jsxs("div", { ref: containerRef, className: joinClassNames(styles.list, hasHeader && centerWelcomeHeader
            ...
            : undefined), "data-web-shell-message-list": true, onClickCapture: handleDisclosureClickCapture, children: [showLoadingSkeleton && (_jsx(LoadingTranscriptSkeleton, { label: t('editor.sessionLoading') })), loadingOlderHistory &&
                !showLoadingSkeleton &&
                !suppressOlderHistoryLoadingStatus && (_jsx("div", { className: styles.historyStatus, role: "status", children: t('history.loadingEarlier') })), historyCapacityReached && !showLoadingSkeleton && (_jsx("div", { className: styles.historyStatus, role: "status", children: t('history.capacityReached') })), historyPaginationError &&
                !showLoadingSkeleton &&
                !historyCapacityReached && (_jsxs("div", { className: styles.historyStatus, children: [_jsx("span", { role: "status", children: t('history.paginationError') }), onLoadOlderHistory && (_jsx("button", { type: "button", className: styles.historyRetryButton, onClick: retryOlderHistory, children: t('history.retry') }))] })), _jsx(SessionTimeline, { entries: sessionTimelineEntries, currentTurnId: currentTimelineTurnId, currentRange: sessionTimelineRange, hidden: !isSessionTimelineVisible || !hasEnoughSessionTimelineEntries, onSelect: scrollToMessage }), useVirtualScroll ? (_jsx("div", { ref: virtualizer.containerRef, className: styles.virtualSizer, children: virtualItems.map((virtualRow) => (_jsx("div", { "data-index": virtualRow.index, ref: measureVirtualRow, className: joinClassNames(styles.virtualRow, getRowClassName(visibleItems[virtualRow.index - headerOffset])), "data-message-row-key": String(getItemKey(virtualRow.index)), "data-web-shell-message-row": true, style: {
                        position: 'absolute',
                        top: 0,
                        left: 0,
                        right: 0,
                ...
                const item = visibleItems[index - headerOffset];
                return (_jsx("div", { "data-index": index, className: getRowClassName(item), "data-message-row-key": String(key), "data-web-shell-message-row": true, children: renderVirtualItem(index) }, key));
            }))] }));
}));
