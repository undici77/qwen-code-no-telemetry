...
vi.mock('./ui/popover', async () => {
    ...
    return {
        ...
        PopoverContent: ({ children }) => createElement('div', { 'data-test-popover-content': '' }, children),
    };
});
const { workspaceGitBranches, workspaceGitCreateBranch, workspaceClient } = vi.hoisted(() => {
...
function mount(overrides = {}) {
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(BranchPickerPopover, { open: true, onOpenChange: overrides.onOpenChange ?? vi.fn(), workspaceCwd: "/repo", onOpenDiff: overrides.onOpenDiff, onOpenCommit: overrides.onOpenCommit, children: _jsx("button", { type: "button", children: "trigger" }) }) }));
    });
}
function clickButton(label) {
    const button = Array.from(document.body.querySelectorAll('button')).find((b) => b.textContent?.includes(label));
...
describe('BranchPickerPopover actions', () => {
    ...
    it('explains an invalid branch name instead of silently returning', async () => {
        ...
        await flush();
        const input = document.body.querySelector('input[placeholder="Branch name"]');
...
//# sourceMappingURL=BranchPickerPopover.test.js.map