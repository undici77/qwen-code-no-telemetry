...
describe('TurnOutputs artifact downloads', () => {
  ...
  it('stamps secondary ownership onto every panel open request', () => {
    ...
    act(() => {
      container.querySelector('button[title="changed.ts"]')?.click();
      container.querySelector('button[title="Secondary artifact"]')?.click();
      container.querySelector('button[title="Secondary schedule"]')?.click();
    });
