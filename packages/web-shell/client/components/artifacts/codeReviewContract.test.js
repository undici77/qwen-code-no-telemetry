...
const fixture = readFileSync(fileURLToPath(new URL('./__fixtures__/code-review-artifact-v1.json', import.meta.url)), 'utf8');
describe('code review artifact contract', () => {
    it('parses the CLI-generated document without losing fields', () => {
        ...
        expect(reviewDocument.markdownReportPath).toBe('.qwen/reviews/contract-v1.md');
        ...
        expect(new Set(findings.map((finding) => finding.source))).toEqual(new Set(['review', 'build', 'test', 'probe', 'lint']));
        expect(new Set(findings.map((finding) => finding.severity))).toEqual(new Set(['Critical', 'Suggestion', 'Nice to have']));
        expect(new Set(findings.map((finding) => finding.confidence))).toEqual(new Set(['high', 'low']));
        expect(new Set(findings.map((finding) => finding.outcome))).toEqual(new Set(['fixed', 'skipped', 'no_change_needed']));
    ...
    it('fails closed on a vocabulary value the renderer does not know yet', () => {
        ...
        const drifted = JSON.parse(fixture);
        drifted.findings[0].source = 'typecheck';
        expect(() => parseCodeReviewDocument(JSON.stringify(drifted))).toThrow('findings[0].source has an unsupported value.');
    });
});
//# sourceMappingURL=codeReviewContract.test.js.map