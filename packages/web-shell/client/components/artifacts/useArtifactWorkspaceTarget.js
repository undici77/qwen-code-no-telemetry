import { useWorkspace, useWorkspaceActions, } from '@qwen-code/webui/daemon-react-sdk';
...
export function resolveArtifactWorkspaceOwner(capabilities, workspaceCwd) {
    ...
    if (capabilities.workspaceCwd !== workspaceCwd)
        return undefined;
    return {
        cwd: workspaceCwd,
        primary: true,
...
function isSameWorkspaceOwner(current, expected) {
    return (current?.cwd === expected.cwd &&
...
export function useArtifactWorkspaceTarget(workspaceCwd) {
    ...
    const owner = useMemo(() => resolveArtifactWorkspaceOwner(workspace.capabilities, workspaceCwd), [workspace.capabilities, workspaceCwd]);
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            queueMicrotask(() => {
                if (authorityLifetimeRef.current === lifetime &&
    ...
    const actions = useMemo(() => {
        ...
        return {
            ...
            async updateScheduledTask(id, update, workspaceId) {
                requireScheduledTaskOwner(workspaceId);
                const result = await primaryActions.updateScheduledTask(id, update, workspaceId);
                requireScheduledTaskOwner(workspaceId);
                return result;
            },
