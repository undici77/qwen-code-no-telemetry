...
const { animationFrameBlocks, connection, messagesFromBlocks, workspaceActions, workspaceClient, latestMessageListProps, messages, } = vi.hoisted(() => ({
...
it('opens subagent and fork transcript outputs in source-scoped panel tabs', async () => {
    ...
    await act(async () => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SubagentDetail, { sessionId: "parent-session", rootToolCallId: "agent-1", initialRootTool: {
                    ...messages[0]
                        .tools[0],
                    startTime: 40_000,
                }, workspaceCwd: "/work/project", onRightPanelOpen: onRightPanelOpen }) }));
    ...
    expect(messagesFromBlocks).toHaveBeenCalledWith(expect.any(Function), animationFrameBlocks);
