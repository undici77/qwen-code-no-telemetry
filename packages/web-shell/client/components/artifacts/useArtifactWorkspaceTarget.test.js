...
function capabilities(workspaces) {
    return {
        v: 1,
        mode: 'http-bridge',
        features: [],
...
describe('resolveArtifactWorkspaceOwner', () => {
    it('resolves one exact trusted workspace owner', () => {
        expect(resolveArtifactWorkspaceOwner(capabilities([primary, secondary]), '/secondary')).toMatchObject({
            cwd: '/secondary',
            id: 'secondary-id',
            primary: false,
        });
    ...
    it('supports only an exact primary match for legacy capabilities', () => {
        expect(resolveArtifactWorkspaceOwner(capabilities(), '/primary')).toMatchObject({ cwd: '/primary', primary: true });
        expect(resolveArtifactWorkspaceOwner(capabilities(), '/secondary')).toBeUndefined();
    });
    it.each([
        {
            name: 'unknown cwd',
...
//# sourceMappingURL=useArtifactWorkspaceTarget.test.js.map