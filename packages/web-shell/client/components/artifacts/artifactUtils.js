...
export function getArtifactImageMimeType(artifact) {
...
export function getImageMimeTypeFromPath(path) {
...
export function getReviewDownloadMimeType(value) {
    return /\.html?$/i.test(value) ? 'text/html' : 'text/markdown';
}
export async function readWorkspaceFileAsBlob(readFileBytes, filePath, mimeType, options) {
    const chunks = [];
    ...
    while (true) {
        ...
        if (offset >= initialStat.sizeBytes) {
            ...
            if (finalStat.sizeBytes !== initialStat.sizeBytes ||
...
export async function downloadWorkspaceFile(workspaceActions, workspacePath, mimeType = 'application/octet-stream', isCancelled) {
    const blob = await readWorkspaceFileAsBlob((filePath, opts) => workspaceActions.readFileBytes(filePath, opts), workspacePath, mimeType, {
...
const ARTIFACT_PREVIEW_CSP = "default-src 'none'; base-uri 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data: blob:;";
