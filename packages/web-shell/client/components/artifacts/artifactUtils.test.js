...
import { downloadWorkspaceFile, getArtifactImageMimeType, getArtifactTypeLabel, getReviewDownloadMimeType, normalizePath, readWorkspaceFileAsBlob, withArtifactPreviewCsp, } from './artifactUtils';
describe('artifactUtils', () => {
    ...
    it('resolves parent path segments', () => {
        ...
        expect(normalizePath('/workspace/app/../app/src/./main.ts')).toBe('/workspace/app/src/main.ts');
    ...
    it('stops at the file size even when byte windows stay truncated', async () => {
        ...
        const blob = await readWorkspaceFileAsBlob(readFileBytes, 'photo.jpg', 'image/jpeg', { statFile });
    ...
    it('stops reading image chunks after cancellation', async () => {
        ...
        await expect(readWorkspaceFileAsBlob(readFileBytes, 'photo.jpg', 'image/jpeg', {
            statFile,
    ...
    it('rejects downloads larger than the default Blob limit', async () => {
        ...
        await expect(downloadWorkspaceFile({ readFileBytes, stat }, 'video.mp4')).rejects.toThrow('too large');
        expect(readFileBytes).not.toHaveBeenCalled();
    });
...
//# sourceMappingURL=artifactUtils.test.js.map