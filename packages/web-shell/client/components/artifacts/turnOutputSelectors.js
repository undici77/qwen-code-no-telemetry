...
function getToolCallIds(tool) {
    const ids = new Set();
    if (tool.callId)
        ids.add(tool.callId);
    if (tool.parentToolCallId)
        ids.add(tool.parentToolCallId);
    for (const subTool of tool.subTools ?? []) {
        for (const id of getToolCallIds(subTool))
            ids.add(id);
    }
    return Array.from(ids);
}
export function getArtifactsByTurn(messages, artifacts, workspaceCwd) {
    const toolCallTurn = new Map();
    const recordArtifactReferences = [];
    ...
    for (const message of messages) {
        ...
        for (const tool of message.tools) {
            ...
            collectRecordArtifactReferences(tool, currentTurnId, recordArtifactReferences);
        }
    }
    const byTurn = new Map();
    for (const artifact of artifacts) {
        const turnIds = getRecordArtifactTurnIds(recordArtifactReferences, artifact, workspaceCwd);
...
function collectRecordArtifactReferences(tool, turnId, references) {
...
function getRecordArtifactTurnIds(references, artifact, workspaceCwd) {
    const turnIds = new Set();
    for (const reference of references) {
        if (reference.workspacePath &&
            artifact.workspacePath &&
            isSameWorkspacePath(reference.workspacePath, artifact.workspacePath, workspaceCwd)) {
...
export function getFileChangesByTurn(messages, artifactsByTurn, workspaceCwd) {
    const byTurn = new Map();
    let currentTurnId = null;
    for (const message of messages) {
        if (message.role === 'user' || message.role === 'user_shell') {
        ...
        if (message.role !== 'tool_group')
            continue;
        const artifactPaths = new Set((artifactsByTurn.get(currentTurnId) ?? [])
            .map((artifact) => normalizePath(artifact.workspacePath))
            .filter((path) => Boolean(path)));
        for (const tool of message.tools) {
            collectFileChanges(tool, currentTurnId, artifactPaths, byTurn, workspaceCwd);
        }
    }
    return byTurn;
}
export function getScheduledTasksByTurn(messages) {
    const byTurn = new Map();
    let currentTurnId = null;
    for (const message of messages) {
        if (message.role === 'user' || message.role === 'user_shell') {
...
function collectScheduledTasks(tool, turnId, byTurn) {
...
function getScheduledTask(tool) {
    if (tool.status !== 'completed')
        return null;
    if (tool.toolName.toLowerCase() !== 'cron_create')
        return null;
    ...
    if (!cron || !prompt)
        return null;
    const outputText = getStringField(raw, 'returnDisplay') ??
...
function collectFileChanges(tool, turnId, artifactPaths, byTurn, workspaceCwd) {
...
function getFileChange(tool, artifactPaths, workspaceCwd) {
    if (tool.status !== 'completed')
        return null;
    ...
    return {
        ...
        status,
        toolCallId: tool.callId,
        isArtifact: Array.from(artifactPaths).some((artifactPath) => isSameWorkspacePath(normalizedPath, artifactPath, workspaceCwd)),
        ...(lineStats
...
function getToolFilePath(tool) {
    const fromArgs = getStringContentField(tool.args, 'file_path', 'filePath', 'path');
    if (fromArgs !== undefined && fromArgs.length > 0)
        return fromArgs;
...
function getStringContentField(record, ...keys) {
    if (!record)
        return undefined;
    for (const key of keys) {
        const value = record[key];
...
function getRawOutputRecord(tool) {
...
function getCronTaskId(text) {
    const match = text.match(/\bScheduled\s+(?:(?:recurring job|one-shot task)\s+([a-z0-9_-]+)\b|([a-z0-9_-]+)\s*\()/i);
    return match?.[1] ?? match?.[2];
}
function getScheduledTaskTitle(prompt) {
    const firstLine = getFirstLine(prompt) ?? prompt.trim();
    return firstLine.length > 36 ? `${firstLine.slice(0, 36)}...` : firstLine;
}
function upsertFileChange(list, change) {
    const normalizedChangePath = normalizePath(change.path);
    const index = list.findIndex((existing) => normalizePath(existing.path) === normalizedChangePath);
    if (index < 0) {
        list.push(change);
        return;
    }
    const existing = list[index];
    const { additions: _existingAdditions, deletions: _existingDeletions, ...existingWithoutLineStats } = existing;
    ...
    list[index] = {
        ...
        status: existing.status === 'created' || change.status === 'created'
            ? 'created'
            : 'modified',
...
function mergeFileDiffs(existingDiffs, nextDiffs) {
    const diffs = [...existingDiffs, ...nextDiffs];
    if (!diffs.every((diff) => diff.fullContent))
        return diffs;
    const firstFullDiff = diffs[0];
    const lastFullDiff = diffs.at(-1);
    if (!firstFullDiff || !lastFullDiff)
        return diffs;
    return [
        {
...
function getFinalFullContentDiff(diffs) {
...
function isSameWorkspacePath(left, right, workspaceCwd) {
    return isSamePath(left, right, workspaceCwd);
}
//# sourceMappingURL=turnOutputSelectors.js.map