...
import { ChevronRightIcon, CirclePlusIcon, ImageIcon, Maximize2Icon, MessageCirclePlusIcon, Minimize2Icon, PanelRightIcon, PlusIcon, SquareActivityIcon, SquareTerminalIcon, } from 'lucide-react';
...
import { artifactKindLabel, downloadWorkspaceFile, formatArtifactSize, getArtifactLocation, getArtifactImageMimeType, getImageMimeTypeFromPath, getReviewDownloadMimeType, normalizePath, readWorkspaceFileAsBlob, withArtifactPreviewCsp, } from './artifactUtils';
import { displayPath, isDownloadableReviewFilePath, isRenderedFilePath, } from './TurnOutputs';
...
import { MonitorTaskDetail, ShellTaskDetail, } from '../messages/TasksStatusMessage';
...
const ignoreSideTaskTitleChange = (_tabId, _title, _fromFirstPrompt) => undefined;
const rejectMissingSideTaskCreate = () => Promise.reject(new Error('Side-task session creation is unavailable'));
...
export function ArtifactPanel({ artifacts, tabs, activeTabId, reviewChanges, selectedReviewPath, panelWidth, workspaceCwd, loading, error, onSelectTab, onCloseTab, onOpenFilePreview, latestReviewAvailable = false, onOpenLatestReview, items = DEFAULT_RIGHT_PANEL_ITEMS, sideTaskAvailable = false, sideTasks = [], sideTasksLoading = false, onCreateSideTask, onOpenSideTask, onCreateSideTaskSession, onSideTaskCreated, onSideTaskTitleChange, onNestedRightPanelOpen, onNestedArtifactsChange, onError, sessionWorkflowEnabled, onImageIngestionNotice, deferSubagentMount = false, onClose, variant = 'docked', fullscreen = false, onToggleFullscreen, }) {
    const { t } = useI18n();
    ...
    const showReviewMenuItem = items.includes('review') && !tabs.some((tab) => tab.kind === 'review');
    const showSideTaskMenuItems = items.includes('sideTask') &&
        sideTaskAvailable &&
        Boolean(onCreateSideTask);
    const showAddMenu = Boolean(activeTab) && (showReviewMenuItem || showSideTaskMenuItems);
    const activeWorkspaceIdentity = activeTab && isWorkspaceScopedTab(activeTab)
        ? {
            workspaceCwd: activeTab.workspaceCwd,
            workspaceId: activeTab.workspaceId,
        }
        : undefined;
    const activeWorkspaceTarget = useArtifactWorkspaceTarget(activeWorkspaceIdentity?.workspaceCwd);
    const activeWorkspaceActions = activeWorkspaceTarget?.workspaceId === activeWorkspaceIdentity?.workspaceId
        ? activeWorkspaceTarget?.actions
        : undefined;
    return (_jsxs("aside", { className: `${styles.panel} ${variant === 'drawer' ? styles.panelDrawer : ''} ${fullscreen ? styles.panelFullscreen : ''}`, style: variant === 'docked' && panelWidth && !fullscreen
            ? { flexBasis: panelWidth, width: panelWidth }
            : undefined, "aria-label": "Right panel", children: [_jsxs("div", { className: styles.header, children: [tabs.length > 0 && (_jsx("div", { className: styles.tabs, role: "tablist", "aria-label": "Right panel", children: tabs.map((tab) => (_jsxs("div", { className: [
                                styles.tabItem,
                                tab.id === activeTab?.id ? styles.tabActive : '',
                            ]
                                .filter(Boolean)
                                .join(' '), children: [_jsxs("button", { type: "button", role: "tab", "aria-selected": tab.id === activeTab?.id, className: styles.tab, onClick: () => onSelectTab(tab.id), title: tab.title, children: [_jsx("span", { className: styles.tabIcon, "aria-hidden": "true", children: tab.kind === 'review' ? (_jsx(TabReviewIcon, {})) : tab.kind === 'artifact' || tab.kind === 'file' ? (_jsx(TabArtifactIcon, {})) : tab.kind === 'subagent' ? (_jsx(TabSubagentIcon, {})) : tab.kind === 'monitor' ? (_jsx(SquareActivityIcon, { className: styles.tabIconSvg, strokeWidth: 1.6 })) : tab.kind === 'shell' ? (_jsx(SquareTerminalIcon, { className: styles.tabIconSvg, strokeWidth: 1.6 })) : tab.kind === 'side_task' ? (_jsx(MessageCirclePlusIcon, { className: styles.tabIconSvg, strokeWidth: 1.6 })) : tab.kind === 'image' ? (_jsx(ImageIcon, { className: styles.tabIconSvg, strokeWidth: 1.6 })) : (_jsx(TabScheduledTaskIcon, {})) }), _jsx("span", { className: styles.tabTitle, children: tab.title })] }), _jsx("button", { type: "button", className: styles.tabCloseButton, onClick: () => onCloseTab(tab.id), "aria-label": `Close ${tab.title}`, title: "Close", children: _jsx(CloseIcon, {}) })] }, tab.id))) })), _jsxs("div", { className: styles.headerActions, children: [showAddMenu && (_jsxs(DropdownMenu, { modal: false, children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsx("button", { type: "button", className: `${styles.iconButton} ${styles.addButton}`, "aria-label": t('rightPanel.add'), title: t('rightPanel.add'), children: _jsx(PlusIcon, { className: styles.toolbarIcon }) }) }), _jsxs(DropdownMenuContent, { align: "end", className: "w-64", children: [showReviewMenuItem && (_jsxs(DropdownMenuItem, { disabled: !latestReviewAvailable || !onOpenLatestReview, onSelect: onOpenLatestReview, children: [_jsx(TabReviewIcon, {}), _jsx("span", { className: styles.sideTaskListTitle, children: t('turnOutputs.review') })] })), showReviewMenuItem && showSideTaskMenuItems && (_jsx(DropdownMenuSeparator, {})), showSideTaskMenuItems && (_jsxs(DropdownMenuItem, { disabled: sideTasksLoading, onSelect: onCreateSideTask, children: [_jsx(MessageCirclePlusIcon, { className: styles.sideTaskNewIcon, strokeWidth: 1.6, "aria-hidden": "true" }), _jsx("span", { className: styles.sideTaskListTitle, children: t('sideTask.create') })] }))] })] })), onToggleFullscreen && (_jsx("button", { type: "button", className: `${styles.iconButton} ${styles.fullscreenButton} ${fullscreen ? styles.iconButtonActive : ''}`, onClick: onToggleFullscreen, "aria-label": t(fullscreen ? 'common.exitFullscreen' : 'common.fullscreen'), "aria-pressed": fullscreen, title: t(fullscreen ? 'common.exitFullscreen' : 'common.fullscreen'), children: fullscreen ? (_jsx(Minimize2Icon, { className: styles.toolbarIcon, "aria-hidden": true })) : (_jsx(Maximize2Icon, { className: styles.toolbarIcon, "aria-hidden": true })) })), _jsx("button", { type: "button", className: `${styles.iconButton} ${styles.panelToggleButton}`, onClick: onClose, "aria-label": t('chatHeader.toggleRightPanel'), "aria-pressed": "true", title: t('chatHeader.toggleRightPanel'), children: _jsx(PanelRightIcon, { className: styles.panelToggleIcon }) })] })] }), _jsx("div", { className: `${styles.body} ${activeTab?.kind === 'side_task' ? styles.bodySideTask : ''}`.trim(), children: !activeTab ? (_jsxs("div", { className: styles.emptyActions, "data-testid": "right-panel-empty-actions", children: [items.includes('review') && (_jsxs("button", { type: "button", className: styles.emptyAction, disabled: !latestReviewAvailable || !onOpenLatestReview, onClick: onOpenLatestReview, children: [_jsx("span", { className: styles.emptyActionIcon, "aria-hidden": "true", children: _jsx(TabReviewIcon, {}) }), _jsx("span", { className: styles.emptyActionTitle, children: t('turnOutputs.review') }), _jsx("span", { className: styles.emptyActionHint, children: t('turnOutputs.reviewLatest') }), _jsx(ChevronRightIcon, { className: styles.emptyActionChevron, strokeWidth: 1.6, "aria-hidden": "true" })] })), items.includes('sideTask') &&
                            sideTaskAvailable &&
                            onCreateSideTask &&
                            (sideTasks.length === 0 ? (_jsxs("button", { type: "button", className: styles.emptyAction, disabled: sideTasksLoading, "aria-busy": sideTasksLoading, onClick: onCreateSideTask, children: [_jsx("span", { className: styles.emptyActionIcon, "aria-hidden": "true", children: _jsx(MessageCirclePlusIcon, { strokeWidth: 1.6 }) }), _jsx("span", { className: styles.emptyActionTitle, children: t('sideTask.title') }), _jsx("span", { className: styles.emptyActionHint, children: t('sideTask.description') }), _jsx(ChevronRightIcon, { className: styles.emptyActionChevron, strokeWidth: 1.6, "aria-hidden": "true" })] })) : (_jsxs(DropdownMenu, { open: sideTaskMenuOpen, onOpenChange: setSideTaskMenuOpen, modal: false, children: [_jsx(DropdownMenuTrigger, { asChild: true, children: _jsxs("button", { type: "button", className: styles.emptyAction, "aria-expanded": sideTaskMenuOpen, onMouseEnter: openSideTaskMenu, onMouseLeave: scheduleSideTaskMenuClose, children: [_jsx("span", { className: styles.emptyActionIcon, "aria-hidden": "true", children: _jsx(MessageCirclePlusIcon, { strokeWidth: 1.6 }) }), _jsx("span", { className: styles.emptyActionTitle, children: t('sideTask.title') }), _jsx("span", { className: styles.emptyActionHint, children: t('sideTask.description') }), _jsx(ChevronRightIcon, { className: styles.emptyActionChevron, strokeWidth: 1.6, "aria-hidden": "true" })] }) }), _jsxs(DropdownMenuContent, { align: "end", className: "w-80", onMouseEnter: openSideTaskMenu, onMouseLeave: scheduleSideTaskMenuClose, children: [sideTasks.map((sideTask) => (_jsxs(DropdownMenuItem, { onSelect: () => onOpenSideTask?.(sideTask), children: [_jsx("span", { className: styles.sideTaskListTitle, children: sideTask.title }), sideTask.updatedAt && (_jsx("span", { className: styles.sideTaskListTime, children: formatRelativeTime(sideTask.updatedAt, t) }))] }, sideTask.sessionId))), _jsx(DropdownMenuSeparator, {}), _jsxs(DropdownMenuItem, { onSelect: onCreateSideTask, children: [_jsx(CirclePlusIcon, { className: styles.sideTaskNewIcon, strokeWidth: 1.6, "aria-hidden": "true" }), _jsx("span", { className: styles.sideTaskListTitle, children: t('sideTask.new') })] })] })] })))] })) : isWorkspaceScopedTab(activeTab) &&
                    (activeTab.kind !== 'scheduled_task' || activeTab.task.durable) &&
                    !activeWorkspaceActions ? (_jsx("div", { className: styles.empty, role: "alert", children: t('workspace.notFoundDescription') })) : activeTab.kind === 'review' ? (_jsx(ReviewChanges, { changes: activeTab.changes ?? reviewChanges, selectedPath: activeTab.selectedPath ?? selectedReviewPath, workspaceCwd: activeTab.workspaceCwd ?? workspaceCwd, onOpenFilePreview: (change) => onOpenFilePreview(change, activeTab.workspaceCwd ?? workspaceCwd, activeTab.workspaceId), onDownloadFile: (change, isCancelled) => downloadWorkspaceFile(activeWorkspaceActions, change.path, getReviewDownloadMimeType(change.path), isCancelled), onDownloadError: (downloadError) => {
                    ...
                    } })) : activeTab.kind === 'file' ? (_jsx(WorkspaceFilePreview, { workspacePath: activeTab.workspacePath, workspaceActions: activeWorkspaceActions, previewContent: activeTab.previewContent }, activeTab.id)) : activeTab.kind === 'artifact' ? (_jsx(ArtifactDetailTab, { artifacts: artifacts, artifactId: activeTab.artifactId, workspaceActions: activeWorkspaceActions, previewContent: activeTab.previewContent, loading: loading, error: error }, activeTab.id)) : activeTab.kind === 'subagent' ? (deferSubagentMount ? null : (_jsx(SubagentDetail, { sessionId: activeTab.sessionId, rootToolCallId: activeTab.rootToolCallId, initialRootTool: activeTab.rootTool, workspaceCwd: activeTab.workspaceCwd ?? workspaceCwd, onRightPanelOpen: onNestedRightPanelOpen, onArtifactsChange: onNestedArtifactsChange, onError: onError }))) : activeTab.kind === 'monitor' ? (_jsx(MonitorTaskDetail, { task: activeTab.task, actions: activeTab.sessionActions }, activeTab.id)) : activeTab.kind === 'shell' ? (_jsx(ShellTaskDetail, { task: activeTab.task, actions: activeTab.sessionActions }, activeTab.id)) : activeTab.kind === 'side_task' ? (_jsx(SideTaskPanel, { tabId: activeTab.id, sessionId: activeTab.sessionId, parentSessionId: activeTab.parentSessionId, workspaceCwd: activeTab.workspaceCwd ?? workspaceCwd, title: activeTab.title, shouldNameFromFirstPrompt: activeTab.nameFromFirstPrompt, initialPrompt: activeTab.initialPrompt, createSession: onCreateSideTaskSession ?? rejectMissingSideTaskCreate, onCreated: onSideTaskCreated ?? ignoreSideTaskCreated, onTitleChange: onSideTaskTitleChange ?? ignoreSideTaskTitleChange, onRightPanelOpen: onNestedRightPanelOpen, onArtifactsChange: onNestedArtifactsChange, onError: onError, sessionWorkflowEnabled: sessionWorkflowEnabled, onImageIngestionNotice: onImageIngestionNotice }, activeTab.id)) : activeTab.kind === 'image' ? (_jsxs("div", { className: styles.imagePreviewWrap, children: [_jsx("img", { src: activeTab.src, alt: activeTab.alt ?? activeTab.title, className: styles.imagePreview }), _jsx("a", { className: styles.imageDownloadButton, href: activeTab.src, download: imageDownloadName(activeTab.src), "aria-label": t('common.download'), title: t('common.download'), children: _jsx(DownloadIcon, { size: 16, strokeWidth: 1.8 }) })] })) : (_jsx(ScheduledTaskDetail, { task: activeTab.task, actions: activeWorkspaceActions }, activeTab.id)) })] }));
}
function TabSubagentIcon() {
    return (_jsxs("svg", { className: styles.tabIconSvg, viewBox: "0 0 24 24", fill: "none", focusable: "false", "aria-hidden": "true", children: [_jsx("circle", { cx: "12", cy: "8", r: "3", stroke: "currentColor", strokeWidth: "1.6" }), _jsx("path", { d: "M6.5 19c.7-3.1 2.5-4.7 5.5-4.7s4.8 1.6 5.5 4.7", stroke: "currentColor", strokeWidth: "1.6", strokeLinecap: "round" })] }));
}
function CloseIcon() {
...
function ScheduledTaskDetail({ task, actions, }) {
    ...
    const isCurrentRequest = useCallback((request, requestActions, taskId, workspaceId) => {
    ...
    const isCurrentLoad = useCallback((request, requestActions, taskId, workspaceId) => {
        const scope = requestScopeRef.current;
        return (request === loadRequestRef.current &&
            scope.actions === requestActions &&
            scope.taskId === taskId &&
            scope.workspaceId === workspaceId);
    }, []);
...
function ReviewChanges({ changes, selectedPath, workspaceCwd, onOpenFilePreview, onDownloadFile, onDownloadError, }) {
    const { t } = useI18n();
    ...
    const downloadFile = async (change) => {
        if (downloadingPaths.has(change.path))
            return;
    ...
    return (_jsxs("div", { className: styles.review, children: [_jsxs("div", { className: styles.reviewToolbar, children: [_jsxs("div", { className: styles.reviewToolbarTitle, children: [_jsx("span", { children: t('turnOutputs.previousTurn') }), _jsx(LineStats, { additions: totals?.additions, deletions: totals?.deletions, className: styles.lineStats, additionsClassName: styles.additions, deletionsClassName: styles.deletions })] }), _jsxs("div", { className: styles.reviewToolbarActions, children: [_jsxs("button", { type: "button", className: styles.reviewTotalsButton, onClick: () => setIsFileListOpen((value) => !value), "aria-expanded": isFileListOpen, children: [_jsx("span", { children: t('turnOutputs.fileCount', { count: changes.length }) }), _jsx("span", { className: [
                    ...
                    '--review-list-width': `${reviewListWidth}px`,
                            ...
                            return (_jsxs("div", { className: [
                                    ...
                                    .filter(Boolean)
                                    .join(' '), children: [_jsxs("div", { className: styles.reviewRow, "data-selected": change.path === selectedPath || undefined, children: [_jsx("button", { type: "button", className: styles.reviewRowToggle, onClick: () => toggleDiff(change.path), "aria-label": change.path, "aria-expanded": isExpanded }), _jsx("span", { className: styles.fileIcon, children: fileExtensionLabel(change.path) }), _jsxs("span", { className: styles.reviewFileName, children: [_jsx(PathText, { path: displayPath(change.path, workspaceCwd), title: change.path }), canOpenPreview && (_jsx("button", { type: "button", className: styles.reviewOpenButton, onClick: () => onOpenFilePreview(change), title: `${t('turnOutputs.preview')} ${change.path}`, children: t('turnOutputs.preview') })), canDownload && (_jsx("button", { type: "button", className: styles.reviewOpenButton, onClick: () => void downloadFile(change), title: `${t('common.download')} ${change.path}`, disabled: downloadingPaths.has(change.path), children: t(downloadingPaths.has(change.path)
                                                            ? 'common.downloading'
                                                            : 'common.download') }))] }), _jsx(LineStats, { additions: change.additions, deletions: change.deletions, className: styles.lineStats, additionsClassName: styles.additions, deletionsClassName: styles.deletions }), _jsx("span", { className: [
                                                    styles.chevron,
...
function ArtifactDetail({ artifact, workspaceActions, previewContent, }) {
    ...
    if (isCodeReview) {
        ...
        return (_jsx(CodeReviewArtifactDetail, { workspacePath: artifact.workspacePath, artifactVersion: `${artifact.status}:${artifact.updatedAt}`, workspaceActions: workspaceActions }));
    }
    if (canPreviewWorkspaceFile && artifact.workspacePath) {
        return (_jsx(WorkspaceFilePreview, { workspacePath: artifact.workspacePath, artifactVersion: artifact.updatedAt, workspaceActions: workspaceActions, previewContent: previewContent, imageMimeType: imageMimeType, previewKind: isHtmlArtifact(artifact)
                ? 'html'
...
function CodeReviewUnavailable({ status }) {
    const { t } = useI18n();
    return (_jsx("div", { className: styles.previewError, role: "alert", children: t('codeReview.unavailable', { status }) }));
}
function CodeReviewWorkspaceRequired() {
    const { t } = useI18n();
    return (_jsx("div", { className: styles.previewError, role: "alert", children: t('codeReview.workspaceRequired') }));
}
...
function WorkspaceFilePreview({ workspacePath, artifactVersion, workspaceActions, previewContent, imageMimeType, previewKind, }) {
    const path = workspacePath.toLowerCase();
    const resolvedImageMimeType = imageMimeType ?? getImageMimeTypeFromPath(workspacePath);
    ...
    if (resolvedPreviewKind === 'html') {
        return (_jsx(HtmlArtifactPreview, { workspacePath: workspacePath, artifactVersion: artifactVersion, workspaceActions: workspaceActions, previewContent: previewContent }));
    }
    if (resolvedPreviewKind === 'markdown') {
        return (_jsx(MarkdownArtifactPreview, { workspacePath: workspacePath, artifactVersion: artifactVersion, workspaceActions: workspaceActions, previewContent: previewContent }));
    }
    if (resolvedPreviewKind === 'image' && resolvedImageMimeType) {
        return (_jsx(ImageArtifactPreview, { workspacePath: workspacePath, artifactVersion: artifactVersion, workspaceActions: workspaceActions, mimeType: resolvedImageMimeType }));
    }
    return (_jsx(FileArtifactPreview, { workspacePath: workspacePath, artifactVersion: artifactVersion, workspaceActions: workspaceActions }));
}
function ImageArtifactPreview({ workspacePath, artifactVersion, workspaceActions, mimeType, }) {
    ...
    useEffect(() => {
        ...
        setSrc(null);
        setError(null);
        readWorkspaceFileAsBlob((filePath, opts) => workspaceActions.readFileBytes(filePath, opts), workspacePath, mimeType, {
    ...
    return (_jsxs("div", { className: styles.imagePreviewWrap, children: [src ? (_jsxs(_Fragment, { children: [_jsx("img", { className: styles.imagePreview, src: src, alt: fileName(workspacePath) }), _jsx("a", { className: styles.imageDownloadButton, href: src, download: fileName(workspacePath), "aria-label": `Download ${fileName(workspacePath)}`, title: "Download", children: _jsx(DownloadIcon, { size: 16, strokeWidth: 1.8 }) })] })) : !error ? (_jsx("div", { className: styles.empty, children: "Loading image..." })) : null, error && _jsx("div", { className: styles.previewError, children: error })] }));
}
function useWorkspaceFileContent({ workspacePath, artifactVersion, workspaceActions, previewContent, truncatedMessage, }) {
...
function HtmlArtifactPreview({ workspacePath, artifactVersion, workspaceActions, previewContent, }) {
    ...
    return (_jsxs("div", { className: styles.htmlPreviewWrap, children: [content === null ? (_jsx("div", { className: styles.empty, children: "Loading preview..." })) : (_jsx("iframe", { className: styles.htmlPreview, referrerPolicy: "no-referrer", sandbox: "allow-scripts", srcDoc: withArtifactPreviewCsp(content), title: `Preview ${workspacePath}` })), error && _jsx("div", { className: styles.previewError, children: error })] }));
}
function FileArtifactPreview({ workspacePath, artifactVersion, workspaceActions, }) {
    ...
    return (_jsxs("div", { className: styles.filePreviewWrap, children: [content === null ? (_jsx("div", { className: styles.empty, children: "Loading file..." })) : (_jsx("div", { ref: hostRef, className: styles.codeMirrorFile })), (error || renderError) && (_jsx("div", { className: styles.previewError, children: error || renderError }))] }));
}
function MarkdownArtifactPreview({ workspacePath, artifactVersion, workspaceActions, previewContent, }) {
    const { content, error } = useWorkspaceFileContent({
        workspacePath,
        artifactVersion,
        workspaceActions,
    ...
    return (_jsxs("div", { className: styles.markdownPreviewWrap, children: [content === null ? (_jsx("div", { className: styles.empty, children: "Loading preview..." })) : (_jsx(Markdown, { content: content })), error && _jsx("div", { className: styles.previewError, children: error })] }));
}
