...
const { connection, providerProps, latestChatPaneProps, renameSession, sendPrompt, transcript, catalogController, } = vi.hoisted(() => ({
    connection: {
        status: 'idle',
        sessionId: undefined,
...
vi.mock('../ChatPane', () => ({
    ChatPane: (props) => {
        ...
        return _jsx("div", { "data-testid": "side-task-chat" });
    },
}));
...
let container = null;
let root = null;
function renderSideTask(props = {}) {
    root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SideTaskPanel, { tabId: "side-task:side-session-1", sessionId: "side-session-1", parentSessionId: "parent-session", workspaceCwd: "/work/project", title: "Side task", createSession: vi.fn(), onCreated: vi.fn(), onTitleChange: vi.fn(), ...props }) }));
}
afterEach(() => {
    act(() => root?.unmount());
    container?.remove();
...
it('creates a side task and reports the new session id', async () => {
    ...
    await act(async () => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SideTaskPanel, { tabId: "side-task:draft:1", parentSessionId: "parent-session", workspaceCwd: "/work/project", title: "Side task", createSession: createSession, onCreated: onCreated, onTitleChange: onTitleChange }) }));
        await Promise.resolve();
        await Promise.resolve();
    });
    expect(createSession).toHaveBeenCalledWith('side-task:draft:1', 'parent-session', 'Side task');
...
it('does not retry creation after a prop change until the user requests it', async () => {
    ...
    document.body.appendChild(container);
    root = createRoot(container);
    const renderDraft = (title) => (_jsx(I18nProvider, { language: "en", children: _jsx(SideTaskPanel, { tabId: "side-task:draft:1", parentSessionId: "parent-session", workspaceCwd: "/work/project", title: title, createSession: createSession, onCreated: onCreated, onTitleChange: vi.fn() }) }));
    await act(async () => {
    ...
    expect(createSession).toHaveBeenLastCalledWith('side-task:draft:1', 'parent-session', 'Renamed task');
    expect(onCreated).toHaveBeenCalledWith('side-task:draft:1', 'side-session-1');
});
it('renders a restored side task as a full chat pane', () => {
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SideTaskPanel, { tabId: "side-task:side-session-1", sessionId: "side-session-1", parentSessionId: "parent-session", workspaceCwd: "/work/project", title: "Side task", createSession: vi.fn(), onCreated: vi.fn(), onTitleChange: onTitleChange, onRightPanelOpen: onRightPanelOpen, onArtifactsChange: onArtifactsChange }) }));
    });
    expect(container.querySelector('[data-testid="side-task-chat"]')).not.toBeNull();
    ...
    expect(latestChatPaneProps.current?.['onFirstPromptAdmitted']).toBeUndefined();
...
it('names a restored empty side task from its first prompt', async () => {
    ...
    await act(async () => {
        (latestChatPaneProps.current?.['onFirstPromptAdmitted'])('Investigate restored task');
        await Promise.resolve();
    });
    expect(renameSession).toHaveBeenCalledWith('Investigate restored task');
    expect(catalogController.renamed).toHaveBeenCalledWith('/work/project', 'side-session-1', 'Investigate restored task');
    expect(onTitleChange).toHaveBeenCalledWith('side-task:side-session-1', 'Investigate restored task', true);
});
it('keeps first-text naming available after an image-only user prompt', () => {
    ...
    expect(latestChatPaneProps.current?.['onFirstPromptAdmitted']).toEqual(expect.any(Function));
});
it('truncates the first-prompt title by code point, not code unit', async () => {
    ...
    await act(async () => {
        (latestChatPaneProps.current?.['onFirstPromptAdmitted'])(longPrompt);
        await Promise.resolve();
    });
    const expected = `${'a'.repeat(199)}\u{1F600}`;
    expect(renameSession).toHaveBeenCalledWith(expected);
    expect(onTitleChange).toHaveBeenCalledWith('side-task:side-session-1', expected, true);
});
it('sends the /btw question as the first side-task prompt', async () => {
    ...
    expect(sendPrompt).toHaveBeenCalledWith('Explain the current implementation', expect.objectContaining({ onAdmitted: expect.any(Function) }));
    expect(renameSession).toHaveBeenCalledWith('Explain the current implementation');
    expect(catalogController.promptAdmitted).toHaveBeenCalledWith('/work/project', 'side-session-1');
    expect(onTitleChange).toHaveBeenCalledWith('side-task:side-session-1', 'Explain the current implementation', true);
});
...
it('names a newly created side task from its first prompt', async () => {
    ...
    const onTitleChange = vi.fn();
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SideTaskPanel, { tabId: "side-task:draft:1", sessionId: "side-session-1", parentSessionId: "parent-session", workspaceCwd: "/work/project", title: "Side task", shouldNameFromFirstPrompt: true, createSession: vi.fn(), onCreated: vi.fn(), onTitleChange: onTitleChange }) }));
    });
    await act(async () => {
        (latestChatPaneProps.current?.['onFirstPromptAdmitted'])('Investigate cache invalidation');
        await Promise.resolve();
    });
    expect(onTitleChange).toHaveBeenCalledWith('side-task:draft:1', 'Investigate cache invalidation', true);
    ...
    transcript.blocks = [{ kind: 'user' }];
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SideTaskPanel, { tabId: "side-task:draft:1", sessionId: "side-session-1", parentSessionId: "parent-session", workspaceCwd: "/work/project", title: "Investigate cache invalidation", shouldNameFromFirstPrompt: false, createSession: vi.fn(), onCreated: vi.fn(), onTitleChange: onTitleChange }) }));
    });
    expect(latestChatPaneProps.current?.['onFirstPromptAdmitted']).toBeUndefined();
});
it('retries the first-prompt title before marking it complete', async () => {
    ...
    await act(async () => {
        (latestChatPaneProps.current?.['onFirstPromptAdmitted'])('Retry this title');
        await Promise.resolve();
        await Promise.resolve();
    });
    ...
    expect(onTitleChange).toHaveBeenCalledWith('side-task:draft:1', 'Retry this title', true);
...
it('bounds first-prompt title retries and reports the final failure', async () => {
    ...
    await act(async () => {
        (latestChatPaneProps.current?.['onFirstPromptAdmitted'])('Keep this title');
        await Promise.resolve();
        await Promise.resolve();
        await Promise.resolve();
    });
    expect(renameSession).toHaveBeenCalledTimes(3);
    expect(onTitleChange).not.toHaveBeenCalledWith('side-task:draft:1', 'Keep this title', true);
    expect(onError).toHaveBeenCalledWith(failure, 'Failed to name side task');
});
//# sourceMappingURL=SideTaskPanel.test.js.map