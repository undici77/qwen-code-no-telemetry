...
import { getArtifactsByTurn, getFileChangesByTurn, getScheduledTasksByTurn, } from './turnOutputSelectors';
function userMessage(id, content) {
    return { id, role: 'user', content };
}
function toolGroup(id, tools) {
...
describe('turnOutputSelectors', () => {
    it('groups artifacts by the turn that recorded them', () => {
        ...
        expect(getArtifactsByTurn(messages, artifacts).get('u1')).toEqual(artifacts);
    });
    ...
    it('shows accurate stats for a large file with a small edit', () => {
        const oldContent = Array.from({ length: 2000 }, (_, index) => `line ${index}`)
            .join('\n')
            .concat('\n');
        const newContent = [
            ...oldContent.split('\n').slice(0, 1000),
    ...
    it('keeps artifacts when the visible transcript starts with a tool group', () => {
        ...
        expect(getArtifactsByTurn(messages, artifacts).get('tg1')).toEqual(artifacts);
    });
    ...
    it('does not match two different relative paths by suffix', () => {
        ...
        const change = getFileChangesByTurn(messages, artifactsByTurn).get('u1')?.[0];
    ...
    it('matches artifact paths through the workspace cwd', () => {
        ...
        const artifactsByTurn = getArtifactsByTurn(messages, artifacts, '/workspace/project');
        expect(artifactsByTurn.get('u1')).toEqual(artifacts);
        const change = getFileChangesByTurn(messages, artifactsByTurn, '/workspace/project').get('u1')?.[0];
        expect(change).toMatchObject({
...
//# sourceMappingURL=turnOutputSelectors.test.js.map