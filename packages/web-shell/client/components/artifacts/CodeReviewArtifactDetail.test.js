    ...
    findings: [
        {
            ...
            failureScenario: 'Open workspace B and submit a request from workspace A.',
...
function renderWith(content, report = '# Durable report', language = 'en') {
    const actions = {
        readWorkspaceFile: vi.fn((path) => Promise.resolve({
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: language, children: _jsx(CodeReviewArtifactDetail, { workspacePath: ".qwen/reviews/pr-123.json", workspaceActions: actions }) }));
    });
...
describe('CodeReviewArtifactDetail', () => {
    it('renders authoritative verdict, counts and finding details without recomputing them', async () => {
        ...
        await flush();
        expect(container.textContent).toContain('Verdict: Request changes — C=9 S=4');
        ...
        expect(container.textContent).toContain('The empty state could be shorter.');
        ...
        expect(container.textContent).toContain('skipped — Outside the reviewed diff.');
        expect(container.textContent).toContain('src/write.ts:42');
        expect(container.textContent).toContain('Held back from Critical by measurement');
    ...
    it('localizes the review chrome without translating canonical finding data', async () => {
        const { container } = renderWith(JSON.stringify(reviewDocument), '# Durable report', 'zh-CN');
        await flush();
        ...
        expect(container.textContent).toContain('The request can overwrite another workspace.');
    });
    it('filters findings by severity and confidence while preserving authoritative counts', async () => {
        const { container } = renderWith(JSON.stringify(reviewDocument));
        await flush();
        const severity = container.querySelector('select[aria-label="Severity"]');
        await act(async () => {
            if (severity) {
        ...
        expect(container.textContent).toContain('9Critical');
        const confidence = container.querySelector('select[aria-label="Confidence"]');
        await act(async () => {
            if (confidence) {
    ...
    it('allows only safe evidence links', async () => {
        const { container } = renderWith(JSON.stringify(reviewDocument));
        await flush();
        const safe = container.querySelector('a[href="https://example.com/evidence.png"]');
    ...
    it('opens the Markdown report through workspace actions', async () => {
        ...
        await flush();
        const button = Array.from(container.querySelectorAll('button')).find((item) => item.textContent === 'Open Markdown report');
        await act(async () => {
            button?.click();
            await Promise.resolve();
            await Promise.resolve();
        });
        expect(actions.readWorkspaceFile).toHaveBeenCalledWith('.qwen/reviews/pr-123.md');
        expect(container.textContent).toContain('Durable report');
        expect(container.querySelector('a[href=".qwen/reviews/pr-123.md"]')).toBeNull();
    });
    it('ignores a stale Markdown response after switching artifacts', async () => {
        ...
        mounted.push({ root, container });
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(CodeReviewArtifactDetail, { workspacePath: ".qwen/reviews/pr-123.json", artifactVersion: "1", workspaceActions: actions }) }));
        });
        await flush();
        act(() => {
        ...
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(CodeReviewArtifactDetail, { workspacePath: ".qwen/reviews/pr-456.json", artifactVersion: "2", workspaceActions: actions }) }));
        });
        await flush();
        await act(async () => {
        ...
        expect(container.textContent).toContain('Verdict: Approve — current artifact');
    ...
    it('resets filters when switching artifacts', async () => {
        ...
        expect(container.querySelector('select[aria-label="Severity"]')?.value).toBe('all');
        expect(container.querySelector('select[aria-label="Confidence"]')?.value).toBe('all');
    });
    it('renders local assetFiles as workspace images and bare paths', async () => {
        ...
        mounted.push({ root, container });
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(CodeReviewArtifactDetail, { workspacePath: ".qwen/reviews/pr-123.json", workspaceActions: actions }) }));
        });
        await flush();
        const image = container.querySelector('img[alt=".qwen/tmp/shots/evidence.png"]');
        expect(image?.src).toBe('blob:evidence-0');
        expect(actions.readFileBytes).toHaveBeenCalledWith('.qwen/tmp/shots/evidence.png', expect.objectContaining({ offset: 0 }));
    ...
    it('offers the derived Markdown report when the artifact is invalid', async () => {
        ...
        await flush();
        expect(container.querySelector('[role="alert"]')?.textContent).toContain('Malformed code review JSON');
        const fallback = Array.from(container.querySelectorAll('button')).find((item) => item.textContent === 'Open Markdown report');
    ...
    it('offers the derived Markdown report when the artifact is truncated', async () => {
        ...
        await flush();
        expect(container.querySelector('[role="alert"]')?.textContent).toContain('truncated');
        const fallback = Array.from(container.querySelectorAll('button')).find((item) => item.textContent === 'Open Markdown report');
        await act(async () => {
            fallback?.click();
            await Promise.resolve();
    ...
    it('does not re-read the artifact or reset filters on a language switch', async () => {
        ...
        expect(container.textContent).toContain('严重级别');
        expect(container.querySelector('select[aria-label="严重级别"]')?.value).toBe('Critical');
    });
...
//# sourceMappingURL=CodeReviewArtifactDetail.test.js.map