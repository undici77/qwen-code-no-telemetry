...
function renderWith(content, report = '# Durable report', language = 'en') {
  ...
  act(() => {
    root.render(
      _jsx(I18nProvider, {
        language: language,
        children: _jsx(CodeReviewArtifactDetail, {
...
describe('CodeReviewArtifactDetail', () => {
  ...
  it('resets filters when switching artifacts', async () => {
    ...
    expect(
      container.querySelector('select[aria-label="Severity"]')?.value,
    ).toBe('all');
    expect(
      container.querySelector('select[aria-label="Confidence"]')?.value,
    ).toBe('all');
  });
  ...
  it('does not re-read the artifact or reset filters on a language switch', async () => {
    ...
    expect(container.textContent).toContain('严重级别');
    expect(
      container.querySelector('select[aria-label="严重级别"]')?.value,
    ).toBe('Critical');
  });
