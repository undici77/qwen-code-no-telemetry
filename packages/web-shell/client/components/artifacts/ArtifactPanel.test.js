...
const { mockActions, mockWorkspace, mockWorkspaceActions, mockSecondaryWorkspaceActions, } = vi.hoisted(() => {
...
const { useArtifactWorkspaceTarget } = await import('./useArtifactWorkspaceTarget');
globalThis.IS_REACT_ACT_ENVIRONMENT = true;
const mounted = [];
let latestArtifactWorkspaceTarget;
function ArtifactWorkspaceTargetProbe({ revision }) {
    latestArtifactWorkspaceTarget = useArtifactWorkspaceTarget('/secondary');
    return _jsx("span", { "data-revision": revision });
}
function monitorPanel(task, sessionActions) {
    return (_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                {
            ...
            ], activeTabId: "monitor:monitor-1", reviewChanges: [], selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
}
function shellPanel(task) {
    return (_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                {
                    id: 'shell:shell-1',
                    kind: 'shell',
            ...
            ], activeTabId: "shell:shell-1", reviewChanges: [], selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
}
function codeReviewArtifact(patch = {}) {
    return {
        id: 'review-artifact',
        kind: 'other',
        storage: 'workspace',
...
function artifactPanel(artifact, owner = {
    workspaceCwd: '/primary',
    workspaceId: 'primary-id',
}) {
    return (_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [artifact], tabs: [
                {
            ...
            ], activeTabId: "artifact:review-artifact", reviewChanges: [], selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
}
const secondaryScheduledTask = {
    id: 'cron-secondary',
    name: 'Secondary task',
    cron: '0 9 * * *',
...
function scheduledTaskPanel(options = {}) {
    ...
    return (_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
            ...
            ], activeTabId: "scheduled-task:secondary:cron-call", reviewChanges: [], selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
}
afterEach(() => {
...
describe('artifact workspace authority', () => {
    it('keeps an in-flight read across an equivalent capabilities refresh', async () => {
        let resolveRead;
        mockSecondaryWorkspaceActions.readWorkspaceFile.mockReturnValue(new Promise((resolve) => {
            resolveRead = resolve;
        }));
        ...
        act(() => root.render(_jsx(ArtifactWorkspaceTargetProbe, { revision: 0 })));
        ...
        act(() => root.render(_jsx(ArtifactWorkspaceTargetProbe, { revision: 1 })));
    ...
    it('does not revive an old read after the same owner is removed and re-added', async () => {
        ...
        act(() => root.render(_jsx(ArtifactWorkspaceTargetProbe, { revision: 2 })));
        ...
        await expect(read).rejects.toThrow('Workspace artifact owner is no longer available');
    });
    it('revokes every pending file read when its owner is removed', async () => {
        ...
        mockSecondaryWorkspaceActions.readWorkspaceFileBytes.mockReturnValue(new Promise((resolve) => {
            resolveBytes = resolve;
        }));
        mockSecondaryWorkspaceActions.fileStat.mockReturnValue(new Promise((resolve) => {
            resolveStat = resolve;
        }));
        const container = document.createElement('div');
        document.body.appendChild(container);
        ...
        await expect(textRead).rejects.toThrow('Workspace artifact owner is no longer available');
        await expect(bytesRead).rejects.toThrow('Workspace artifact owner is no longer available');
        await expect(statRead).rejects.toThrow('Workspace artifact owner is no longer available');
    });
});
function openAddMenu(container) {
    const add = container.querySelector('button[aria-label="Add panel"]');
    act(() => {
        add?.dispatchEvent(new MouseEvent('pointerdown', { bubbles: true, button: 0 }));
    });
    return add;
}
...
describe('ArtifactPanel code review artifacts', () => {
    it('fails closed when an artifact tab has no workspace owner', async () => {
        ...
        await flush();
        expect(container.textContent).toContain('This workspace may have been removed or the link is no longer valid.');
    ...
    it('fails closed when a file tab has no workspace owner', async () => {
        ...
        const root = createRoot(container);
        mounted.push({ root, container });
        act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                    {
                ...
                ], activeTabId: "file:missing-owner", reviewChanges: [], selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) })));
        await flush();
        expect(container.querySelector('[role="alert"]')?.textContent).toContain('This workspace may have been removed');
    ...
    it('dispatches an available workspace artifact to the dedicated renderer', async () => {
        ...
        expect(mockWorkspaceActions.readWorkspaceFile).toHaveBeenCalledWith('.qwen/reviews/review.json');
    });
    it('loads an artifact under StrictMode effect replay', async () => {
        ...
        const root = createRoot(container);
        mounted.push({ root, container });
        act(() => root.render(_jsx(StrictMode, { children: artifactPanel(codeReviewArtifact()) })));
        await flush();
        expect(container.textContent).toContain('Authoritative verdict');
        expect(container.textContent).not.toContain('Workspace artifact owner is no longer available');
    });
    it.each(['changed', 'missing'])('does not render a %s artifact as authoritative', async (status) => {
        const container = document.createElement('div');
        document.body.appendChild(container);
        const root = createRoot(container);
        mounted.push({ root, container });
        ...
        await flush();
        expect(container.querySelector('[role="alert"]')?.textContent).toContain(status);
    ...
    it('requires code review artifacts to use workspace storage', async () => {
        ...
        await flush();
        expect(container.querySelector('[role="alert"]')?.textContent).toContain('workspace files');
        expect(mockWorkspaceActions.readWorkspaceFile).not.toHaveBeenCalled();
    });
    ...
    it('discards a pending read when its workspace owner is replaced', async () => {
        ...
        await flush();
        expect(mockWorkspace.client.workspaceByCwd).toHaveBeenCalledWith('/secondary');
        mockWorkspace.capabilities = {
            ...mockWorkspace.capabilities,
            workspaces: [
                mockWorkspace.capabilities.workspaces[0],
...
describe('ArtifactPanel scheduled-task ownership', () => {
    it('loads a durable task through its secondary workspace route', async () => {
        ...
        await flush();
        expect(mockWorkspaceActions.listScheduledTasks).toHaveBeenCalledWith('secondary-id');
    });
    it('updates and deletes only through the task workspace id', async () => {
        ...
        act(() => root.render(scheduledTaskPanel()));
        await flush();
        const disable = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Disable');
        await act(async () => {
            disable?.click();
            await Promise.resolve();
            await Promise.resolve();
        });
        expect(mockWorkspaceActions.updateScheduledTask).toHaveBeenCalledWith('cron-secondary', { enabled: false }, 'secondary-id');
        const openDelete = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Delete');
        ...
        expect(mockWorkspaceActions.deleteScheduledTask).toHaveBeenCalledWith('cron-secondary', 'secondary-id');
    });
    ...
    it('keeps legacy single-workspace scheduled-task routes unqualified', async () => {
        ...
        await flush();
        expect(mockWorkspaceActions.listScheduledTasks).toHaveBeenCalledWith(undefined);
        const disable = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Disable');
        await act(async () => {
            disable?.click();
            await Promise.resolve();
        });
        expect(mockWorkspaceActions.updateScheduledTask).toHaveBeenCalledWith('cron-secondary', { enabled: false }, undefined);
        const openDelete = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Delete');
        act(() => openDelete?.click());
        const confirmDelete = Array.from(document.body.querySelectorAll('button'))
            .filter((button) => button.textContent?.trim() === 'Delete')
        ...
        expect(mockWorkspaceActions.deleteScheduledTask).toHaveBeenCalledWith('cron-secondary', undefined);
    });
    it.each(['save', 'toggle', 'delete'])('settles a pending reload after a %s mutation', async (mutation) => {
        let resolveReload;
        ...
        act(() => root.render(scheduledTaskPanel()));
        await flush();
        act(() => root.render(scheduledTaskPanel({ task: { prompt: `reload ${mutation}` } })));
        await flush();
        expect(container.textContent).toContain('Loading…');
        if (mutation === 'save') {
            const edit = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Edit');
            act(() => edit?.click());
            const save = Array.from(document.body.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Save');
            await act(async () => {
                save?.click();
                await Promise.resolve();
            });
            expect(mockWorkspaceActions.updateScheduledTask).toHaveBeenCalledWith('cron-secondary', expect.objectContaining({ prompt: expect.any(String) }), 'secondary-id');
        }
    ...
    it('discards a pending mutation when the task scope changes', async () => {
        ...
        let resolveStaleMutation;
        mockWorkspaceActions.listScheduledTasks
            .mockResolvedValueOnce([secondaryScheduledTask])
            .mockResolvedValueOnce([replacementTask]);
        mockWorkspaceActions.updateScheduledTask.mockReturnValueOnce(new Promise((resolve) => {
        ...
        const replacementDisable = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Disable');
        await act(async () => {
            replacementDisable?.click();
            await Promise.resolve();
        });
        expect(mockWorkspaceActions.updateScheduledTask).toHaveBeenLastCalledWith('cron-replacement', { enabled: false }, 'secondary-id');
    });
});
describe('ArtifactPanel add menu', () => {
    it('keeps the disabled review action on the empty page and hides the add button', () => {
        ...
        mounted.push({ root, container });
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [], activeTabId: null, reviewChanges: [], selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: onClose }) }));
        });
        const review = Array.from(container.querySelectorAll('[data-testid="right-panel-empty-actions"] button')).find((button) => button.textContent?.includes('Changes'));
        ...
        const panelToggle = container.querySelector('button[aria-label="Toggle right panel"]');
    ...
    it('filters empty-page actions through right-panel items', () => {
        ...
        mounted.push({ root, container });
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [], activeTabId: null, reviewChanges: [], selectedReviewPath: null, items: ['sideTask'], sideTaskAvailable: true, onCreateSideTask: vi.fn(), onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
        });
        const emptyText = container.querySelector('[data-testid="right-panel-empty-actions"]')?.textContent;
    ...
    it('supports opening an existing side task or creating one', () => {
        ...
        mounted.push({ root, container });
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [], activeTabId: null, reviewChanges: [], selectedReviewPath: null, sideTaskAvailable: true, sideTasks: [
                        {
                            sessionId: 'side-1',
                            title: 'Investigate flaky tests',
                            workspaceCwd: '/work/project',
                        },
                    ], onCreateSideTask: onCreateSideTask, onOpenSideTask: onOpenSideTask, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
        });
        const sideTask = Array.from(container.querySelectorAll('[data-testid="right-panel-empty-actions"] button')).find((button) => button.textContent?.includes('Side task'));
        act(() => {
            sideTask?.dispatchEvent(new MouseEvent('mouseover', { bubbles: true, cancelable: true }));
        });
        const existing = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((button) => button.textContent === 'Investigate flaky tests');
        const create = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((button) => button.textContent === 'New');
        ...
        const reopenedCreate = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((button) => button.textContent === 'New');
    ...
    it('creates a side task directly from the empty page when there is no history', () => {
        ...
        mounted.push({ root, container });
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [], activeTabId: null, reviewChanges: [], selectedReviewPath: null, sideTaskAvailable: true, onCreateSideTask: onCreateSideTask, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
        });
        const sideTask = Array.from(container.querySelectorAll('[data-testid="right-panel-empty-actions"] button')).find((button) => button.textContent?.includes('Side task'));
        act(() => sideTask?.click());
        expect(onCreateSideTask).toHaveBeenCalledOnce();
    ...
    it('does not create a side task before its history finishes loading', () => {
        ...
        mounted.push({ root, container });
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [], activeTabId: null, reviewChanges: [], selectedReviewPath: null, sideTaskAvailable: true, sideTasksLoading: true, onCreateSideTask: onCreateSideTask, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
        });
        const sideTask = Array.from(container.querySelectorAll('[data-testid="right-panel-empty-actions"] button')).find((button) => button.textContent?.includes('Side task'));
    ...
    it('opens the latest review from the empty page', () => {
        ...
        mounted.push({ root, container });
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [], activeTabId: null, reviewChanges: [], selectedReviewPath: null, latestReviewAvailable: true, onOpenLatestReview: onOpenLatestReview, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
        });
        const review = Array.from(container.querySelectorAll('[data-testid="right-panel-empty-actions"] button')).find((button) => button.textContent?.includes('Changes'));
    ...
    it('hides review from the add menu when a review tab is already open', () => {
        ...
        mounted.push({ root, container });
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                        {
                            id: 'review',
                            kind: 'review',
                            title: 'Review',
                    ...
                    ], activeTabId: "review", reviewChanges: [], selectedReviewPath: null, latestReviewAvailable: true, sideTaskAvailable: true, onOpenLatestReview: vi.fn(), onCreateSideTask: vi.fn(), onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
        });
    ...
    it('shows review and side-task actions in the add menu for a non-empty panel', () => {
        ...
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                    ...
                    ], activeTabId: "artifact", reviewChanges: [], selectedReviewPath: null, latestReviewAvailable: true, sideTaskAvailable: true, sideTasks: [
                        {
                            sessionId: 'side-1',
                            title: 'Existing side task',
                            workspaceCwd: '/work/project',
                        },
                    ], onOpenLatestReview: vi.fn(), onCreateSideTask: vi.fn(), onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
        });
        openAddMenu(container);
        const menuText = document.body.querySelector('[role="menu"]')?.textContent;
...
describe('ArtifactPanel review downloads', () => {
    it('shows the requested actions and reports download failures through toast', async () => {
        ...
        const onError = vi.fn();
        let rejectStat;
        mockWorkspaceActions.stat.mockReturnValue(new Promise((_resolve, reject) => {
            rejectStat = reject;
        }));
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                    ...
                    ], activeTabId: "review", reviewChanges: changes, selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onError: onError, onClose: () => { } }) }));
        });
        ...
        expect(actionLabels.filter((label) => label === 'Download')).toHaveLength(2);
    ...
    it('keeps other rows downloadable while one review file downloads', () => {
        ...
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                    ...
                    ], activeTabId: "review", reviewChanges: changes, selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
        });
        const downloads = Array.from(container.querySelectorAll('button')).filter((button) => button.textContent?.trim() === 'Download');
    ...
    it('cancels the download and skips the error toast when the panel unmounts mid-download', async () => {
        ...
        const onError = vi.fn();
        let resolveStat;
        mockWorkspaceActions.stat.mockReturnValue(new Promise((resolve) => {
            resolveStat = resolve;
        }));
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
...
describe('ArtifactPanel monitor tab', () => {
    ...
    it('shows the monitor snapshot in a dedicated right-panel tab', () => {
        ...
        expect(container.querySelector('svg.lucide-square-activity')).not.toBeNull();
        expect(container.textContent).toContain('watch server log');
        expect(container.querySelector('[data-status="running"]')?.textContent).toBe('Running');
    ...
    it('stops a running monitor from its detail tab', async () => {
        ...
        expect(Array.from(container.querySelectorAll('button')).some((button) => button.textContent === 'Stop')).toBe(false);
        act(() => {
            root.render(monitorPanel({ ...task }));
        });
        expect(container.textContent).toContain('Stopped');
    ...
    it('keeps an in-flight stop response scoped to its monitor tab', async () => {
        ...
        let resolveCancel;
        mockActions.cancelTask.mockReturnValue(new Promise((resolve) => {
        ...
        const root = createRoot(container);
        mounted.push({ root, container });
        const renderPanel = (activeTabId) => (_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                    {
                        id: 'monitor:monitor-1',
                        kind: 'monitor',
                ...
                ], activeTabId: activeTabId, reviewChanges: [], selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) }));
        act(() => {
        ...
        const secondStopButton = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'Stop');
    ...
    it('shows a cancel error when the stop request throws', async () => {
        ...
        const stopButtonAfter = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'Stop');
...
describe('ArtifactPanel shell tab', () => {
    it('shows shell task details in a dedicated right-panel tab', () => {
        ...
        expect(container.querySelector('[data-status="failed"]')?.textContent).toBe('Failed');
...
describe('ArtifactPanel fullscreen toggle', () => {
    function renderPanel(props) {
        ...
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                    ...
                    ], activeTabId: "monitor:monitor-1", reviewChanges: [], selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { }, fullscreen: props.fullscreen, onToggleFullscreen: props.onToggleFullscreen }) }));
        });
        return container;
    }
    it('shows a fullscreen toggle and reports clicks', () => {
        ...
        const toggle = container.querySelector('button[aria-label="Fullscreen"]');
    ...
    it('marks the panel full-bleed and flips the toggle when fullscreen', () => {
        ...
        const toggle = container.querySelector('button[aria-label="Exit fullscreen"]');
        expect(toggle).not.toBeNull();
    ...
    it('omits the toggle when fullscreen is unsupported', () => {
        ...
        expect(container.querySelector('button[aria-label="Exit fullscreen"]')).toBeNull();
    });
});
describe('ArtifactPanel image preview tabs', () => {
    it('renders one preview tab per image and shows the active image', () => {
        ...
        act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ArtifactPanel, { artifacts: [], tabs: [
                ...
                ], activeTabId: "image:a", reviewChanges: [], selectedReviewPath: null, onSelectTab: () => { }, onCloseTab: () => { }, onOpenFilePreview: () => { }, onClose: () => { } }) })));
        expect(container.querySelectorAll('[role="tab"]')).toHaveLength(2);
        const preview = container.querySelector('img[class*="imagePreview"]');
        ...
        const download = container.querySelector('a[class*="imageDownloadButton"]');
