...
function setup(overrides = {}, customization = {}) {
    const handlers = {
        onDelete: vi.fn(),
        onEdit: vi.fn(),
    ...
    const container = render(_jsx(WebShellCustomizationProvider, { value: customization, children: _jsx(QueuedPromptDisplay, { prompts: prompts, t: t, canMutateMidTurn: true, ...handlers, ...overrides }) }));
...
describe('QueuedPromptDisplay', () => {
    ...
    it('shows server queue status without an insert action', () => {
        ...
        expect(container.querySelector('[class*="queuedPromptSpinner"]')).toBeNull();
    ...
    it('allows deleting but not editing a summary-only server row', () => {
        ...
        const deleteButton = container.querySelector(`[aria-label="${t('queue.delete')}"]`);
        const editButton = container.querySelector(`[aria-label="${t('queue.edit')}"]`);
    ...
    it('keeps a mid-turn prompt queued until injection', () => {
        ...
        expect(status?.nextElementSibling?.querySelectorAll('button')).toHaveLength(2);
    ...
    it('keeps the spinner while a prompt is still submitting', () => {
        ...
        expect(container.querySelector('[class*="queuedPromptSpinner"]')).toBeTruthy();
    });
    it('requires confirmation before restoring an unknown local payload', () => {
        ...
        const restore = container.querySelector(`[aria-label="${t('queue.restoreUnknown')}"]`);
        const discard = container.querySelector(`[aria-label="${t('queue.discardUnknown')}"]`);
        act(() => restore?.click());
    ...
    it('parses the complete legacy queued prompt before rendering its tag', () => {
        ...
        const parser = vi.fn(() => [
            ...
            { type: 'text', text: ' explain the table' },
        ]);
        const { container } = setup({ prompts: [{ id: 1, text }] }, { parseUserMessageContent: parser });
    ...
    it('falls back to raw queued text when a tag field is malformed', () => {
        ...
        const { container } = setup({ prompts: [{ id: 1, text }] }, { parseUserMessageContent: malformedParser });
        expect(container.textContent).toContain(text);
    });
    it('omits an atomic tag that exceeds the visible preview budget', () => {
        ...
        const { container } = setup({ prompts: [{ id: 1, text: serialized }] }, {
            parseUserMessageContent: () => [
                {
                    type: 'tag',
    ...
    it('truncates a text-only queued prompt at the visible preview budget', () => {
        ...
        const { container } = setup({ prompts: [{ id: 1, text }] });
        expect(container.querySelector('[class*="queuedPromptText"]')?.textContent).toBe(`${text.slice(0, 240)}...`);
    });
    it('truncates trailing text after an atomic tag consumes the visible preview budget', () => {
        ...
        const { container } = setup({ prompts: [{ id: 1, text: `${serialized}${trailingText}` }] }, {
            parseUserMessageContent: () => [
                {
                    type: 'tag',
                    tag: { id: 'orders', value: visibleTag, serialized },
        ...
        expect(container.querySelector('[class*="queuedPromptText"]')?.textContent).toBe(`${visibleTag}...`);
    });
    it('falls back to raw queued text when parsing throws', () => {
        const warn = vi.spyOn(console, 'warn').mockImplementation(() => { });
        const { container } = setup({ prompts: [{ id: 1, text: 'raw <broken /> content' }] }, {
            parseUserMessageContent: () => {
