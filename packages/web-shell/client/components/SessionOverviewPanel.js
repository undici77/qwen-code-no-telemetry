...
import { useConnection, useStatusReport, } from '@qwen-code/webui/daemon-react-sdk';
import { useI18n } from '../i18n';
import { formatRelativeTime } from '../utils/formatRelativeTime';
import { buildSplitUrl, MAX_SPLIT_PANES } from '../utils/splitUrl';
import { hasMultipleWorkspaces, isNonPrimaryWorkspaceSession, mergeSessionsById, workspaceLabelForCwd, } from '../utils/workspace';
 ...
 */
export function deriveSessionCards(sessions, statusSessions, currentSessionId, primaryCwd) {
    const statusById = new Map(statusSessions.map((session) => [session.sessionId, session]));
    const cards = sessions.map((session) => {
        ...
        return {
            ...
            isNonPrimary: isNonPrimaryWorkspaceSession(session.workspaceCwd, primaryCwd),
        };
    });
    cards.sort((a, b) => {
...
function SessionOverviewPanelInner({ onOpenSession, onOpenSplit, includeOtherWorkspaces, workspaceCwd, }) {
    const { t } = useI18n();
    ...
    const { sessions: otherSessions, reload: reloadOther } = useOtherWorkspaceSessions(includeOtherWorkspaces && !workspaceCwd, LIST_POLL_MS);
    const mergedSessions = useMemo(() => mergeSessionsById(sessions, otherSessions), [sessions, otherSessions]);
    ...
    useEffect(() => {
        const timer = window.setInterval(() => {
            if (document.hidden || statusInFlight.current)
                return;
    ...
    const cards = useMemo(() => deriveSessionCards(mergedSessions, statusReport?.full?.sessions ?? [], currentSessionId, primaryCwd), [mergedSessions, statusReport, currentSessionId, primaryCwd]);
    ...
    if (cards.length === 0) {
        return (_jsx("div", { className: styles.panel, children: _jsx("div", { className: styles.empty, children: loading
                    ? t('sessionsOverview.loading')
                    : error
                        ? `${t('sessionsOverview.loadFailed')}: ${error.message}`
                        : t('sessionsOverview.empty') }) }));
    }
    return (_jsxs("div", { className: styles.panel, children: [_jsxs("div", { className: styles.toolbar, children: [_jsx("span", { className: styles.count, children: t('sessionsOverview.count', { count: cards.length }) }), _jsxs("label", { className: styles.selectAll, children: [_jsx("input", { type: "checkbox", checked: allSelected, onChange: toggleSelectAll }), t('sessionsOverview.selectAll')] }), _jsx("button", { type: "button", className: styles.actionButton, disabled: selectedCount === 0, onClick: openSelectedInNewTab, title: t('sessionsOverview.openInTabHint'), children: t('sessionsOverview.openInTab') }), onOpenSplit && (_jsx("button", { type: "button", className: styles.actionButton, disabled: selectedCount === 0, onClick: () => onOpenSplit(splitIds), title: t('sessionsOverview.openInSplitHint'), children: t('sessionsOverview.openInSplit') })), _jsx("button", { type: "button", className: styles.refreshButton, onClick: refresh, children: t('sessionsOverview.refresh') })] }), overCap && (_jsx("div", { className: styles.notice, role: "status", children: t('sessionsOverview.splitCap', { max: MAX_SPLIT_PANES }) })), popupBlocked && (_jsx("div", { className: styles.notice, role: "alert", children: t('sessionsOverview.popupBlocked') })), error && (_jsxs("div", { className: styles.notice, role: "alert", children: [t('sessionsOverview.loadFailed'), ": ", error.message] })), _jsx("ul", { className: styles.grid, children: cards.map((card) => (_jsxs("li", { className: cx(styles.card, statusClass(card.status), card.isCurrent && styles.cardCurrent), children: [_jsxs("div", { className: styles.cardTop, children: [_jsx("input", { type: "checkbox", className: styles.cardCheckbox, checked: selected.has(card.sessionId), onChange: () => toggleSelected(card.sessionId), "aria-label": t('sessionsOverview.selectSession', {
                                        name: card.label,
                                    }) }), card.color && (_jsx("span", { className: cx(styles.colorDot, colorDotClass(card.color)), "aria-hidden": "true" })), _jsx("button", { type: "button", className: styles.cardLabel, onClick: () => onOpenSession(card.sessionId), title: card.label, children: card.label }), card.isCurrent && (_jsx("span", { className: styles.currentBadge, children: t('sessionsOverview.current') }))] }), _jsxs("div", { className: styles.cardMeta, children: [_jsx("span", { className: cx(styles.statusBadge, statusClass(card.status)), children: t(`sessionsOverview.status.${card.status}`) }), multiWorkspace && (_jsx("span", { className: cx(styles.workspaceBadge, card.isNonPrimary && styles.workspaceBadgeOther), title: card.workspaceCwd, children: workspaceLabelForCwd(card.workspaceCwd, connection.capabilities?.workspaces) })), card.model && (_jsx("span", { className: styles.metaItem, title: card.model, children: card.model })), card.clientCount > 0 && (_jsx("span", { className: styles.metaItem, title: t('sessionsOverview.openElsewhere'), children: t('common.clients', { count: card.clientCount }) })), card.updatedAt && (_jsx("span", { className: styles.metaItem, children: formatRelativeTime(card.updatedAt, t) }))] })] }, card.sessionId))) })] }));
}
/**
 ...
 */
export function SessionOverviewPanel({ onOpenSession, onOpenSplit, includeOtherWorkspaces = true, workspaceCwd, }) {
    const { t } = useI18n();
    return (_jsx(ErrorBoundary, { label: "session-overview", fallback: (fallbackError) => (_jsx("div", { className: styles.panel, children: _jsxs("div", { className: styles.empty, children: [t('sessionsOverview.loadFailed'), ": ", fallbackError.message] }) })), children: _jsx(SessionOverviewPanelInner, { onOpenSession: onOpenSession, onOpenSplit: onOpenSplit, includeOtherWorkspaces: includeOtherWorkspaces, workspaceCwd: workspaceCwd }) }));
}
