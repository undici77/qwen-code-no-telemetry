...
import { DaemonSessionProvider, useConnection, useWorkspace, useWorkspaceActions, } from '@qwen-code/webui/daemon-react-sdk';
import { App } from '../App';
...
export function WorkspaceSessionProvider({ sessionId, workspaceId, workspaceCwd, lockWorkspaceCwd, clientId, restartSseOnPrompt, historyPageSize = WEB_SHELL_HISTORY_PAGE_SIZE, webShellProps, }) {
    ...
    const desiredWorkspace = targetWorkspace ??
        (!effectiveWorkspaceCwd && !effectiveWorkspaceId
            ? workspace.capabilities?.workspaces?.find((entry) => entry.primary || entry.cwd === workspace.capabilities?.workspaceCwd)
            : undefined);
    ...
    if (workspace.capabilities) {
        transactionalRef.current = workspace.capabilities.features.includes(CLIENT_IDENTITY_FEATURE);
    }
    ...
    if (pendingHostCommitKeyRef.current !== undefined &&
    ...
    const commitTarget = useCallback((target) => {
    ...
    const canKeepCommitted = transactional && committedTargetRef.current !== undefined;
    const desiredTargetResolved = (!effectiveWorkspaceCwd && !effectiveWorkspaceId) ||
        targetWorkspace !== undefined;
    const failureLatchRef = useRef(undefined);
    const desiredTargetFailed = workspace.status === 'error' ||
    ...
    const reportCommittedTarget = useCallback(() => {
        ...
        if (!committed)
            return;
        const workspaceId = workspace.capabilities?.workspaces?.find((entry) => entry.cwd === committed.workspaceCwd)?.id;
        onSessionIdChange?.(committed.sessionId, workspaceId, committed.workspaceCwd);
    }, [onSessionIdChange, workspace.capabilities?.workspaces]);
    const observeSessionState = useCallback((connection) => {
        if (transactional &&
    ...
    useEffect(() => {
        ...
        if (!desiredTargetFailed || failureLatchRef.current === desiredKey)
            return;
        failureLatchRef.current = desiredKey;
        reportCommittedTarget();
    }, [
    ...
    const keepCommittedTarget = canKeepCommitted &&
    ...
    const visibleWorkspaceCwd = canKeepCommitted
        ? committedTargetRef.current.workspaceCwd
        : desiredWorkspace?.cwd;
    const visibleWorkspace = (desiredWorkspace?.cwd === visibleWorkspaceCwd
        ? desiredWorkspace
        : undefined) ??
        workspace.capabilities?.workspaces?.find((entry) => entry.cwd === visibleWorkspaceCwd) ??
    ...
    return (_jsxs(DaemonSessionProvider, { sessionId: providerSessionId, workspaceCwd: providerWorkspaceCwd, clientId: clientId, historyPageSize: historyPageSize, subagentTranscriptMode: "summary", maxBlocks: WEB_SHELL_MAX_TRANSCRIPT_BLOCKS, suppressOwnUserEcho: true, restartEventStreamOnPrompt: restartSseOnPrompt, onSessionTransitionCommit: commitTarget, children: [_jsx(SessionStateObserver, { onChange: observeSessionState }), _jsx(App, { ...webShellProps, desiredSessionTargetPending: desiredTargetPending, historyPageSize: historyPageSize, restartSseOnPrompt: restartSseOnPrompt, initialSelectedWorkspaceCwd: !lockWorkspaceCwd ? visibleWorkspaceCwd : undefined, lockedWorkspaceCwd: lockWorkspaceCwd ? visibleWorkspaceCwd : undefined, lockedWorkspaceCapability: lockWorkspaceCwd ? visibleWorkspace : undefined })] }, transactionalRef.current !== false
