import type { DaemonSettingDescriptor, DaemonSettingUpdateResult, DaemonWorkspaceSettingsStatus } from '@qwen-code/webui/daemon-react-sdk';
...
export interface SettingsMessageSettingsState {
    ...
    setValue: (scope: 'workspace' | 'user', key: string, value: unknown) => Promise<DaemonSettingUpdateResult>;
    liveSetup?: UseLiveVoiceSetupResult;
}
type Scope = 'user' | 'workspace';
type Translator = (key: string, vars?: Record<string, string | number>) => string;
export declare function formatSettingLabel(setting: DaemonSettingDescriptor, t: Translator): string;
export type FlatRow = {
    type: 'header';
    category: string;
} | {
...
export declare function nextSettingIdx(rows: FlatRow[], current: number, dir: 1 | -1): number;
export declare function SettingsMessage({ settingsState, onLanguageChange, onSubDialog, onThemeChange, chatWidthMode, onChatWidthModeChange, modelManagement, embedded, }: SettingsMessageProps): import("react/jsx-runtime").JSX.Element;
export {};
