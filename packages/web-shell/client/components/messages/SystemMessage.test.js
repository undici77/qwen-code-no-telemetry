...
describe('SystemMessage — background notification label', () => {
    it('labels background task notifications and preserves display text', () => {
        const container = render(_jsx(SystemMessage, { content: 'Background agent "worker" completed.', variant: "info", source: "background_notification", data: { status: 'completed' } }));
        expect(container.textContent).toContain('Background agent "worker" completed.');
        ...
        expect(icon?.nextElementSibling?.textContent).toContain('Background agent "worker" completed.');
        expect(icon?.querySelector('svg')).not.toBeNull();
    });
    it('localizes the label', () => {
        const container = render(_jsx(SystemMessage, { content: "\u540E\u53F0\u4EE3\u7406\u5DF2\u5B8C\u6210\u3002", variant: "info", source: "background_notification", data: { status: 'completed' } }), 'zh-CN');
        expect(container
            .querySelector('[role="img"][data-tone="success"]')
            ?.getAttribute('aria-label')).toBe('后台任务执行完成');
    });
    it.each([
        ['failed', '后台任务执行失败', 'error'],
        ['cancelled', '后台任务已取消', 'neutral'],
        ['unknown', '后台任务通知', 'neutral'],
    ])('uses the matching label and tone for %s status', (status, label, tone) => {
        const container = render(_jsx(SystemMessage, { content: "\u4EFB\u52A1\u7ED3\u679C", variant: "info", source: "background_notification", data: { status } }), 'zh-CN');
    ...
    it('does not label generic system information', () => {
        const container = render(_jsx(SystemMessage, { content: "Connected.", variant: "info" }));
...
describe('SystemMessage — background notification i18n body', () => {
    it('renders shell notifications with structured command via i18n', () => {
        const container = render(_jsx(SystemMessage, { content: 'Background shell "npm test" completed.', variant: "info", source: "background_notification", data: {
                status: 'completed',
                kind: 'shell',
                commandLabel: 'npm test',
            } }), 'zh-CN');
        expect(container.textContent).toContain('后台 Shell 已完成：npm test');
        expect(container.textContent).not.toContain('Background shell "npm test" completed.');
    });
    it('renders agent notifications with structured description via i18n', () => {
        const container = render(_jsx(SystemMessage, { content: 'Background agent "worker" completed.', variant: "info", source: "background_notification", data: {
                status: 'completed',
                kind: 'agent',
                description: 'worker',
            } }), 'zh-CN');
    ...
    it('renders monitor notifications with the event count in English', () => {
        const container = render(_jsx(SystemMessage, { content: "raw daemon text", variant: "info", source: "background_notification", data: {
                status: 'completed',
                kind: 'monitor',
                description: 'logs',
                eventCount: 5,
            } }));
        expect(container.textContent).toContain('Monitor "logs" completed. (5 events)');
        expect(container.textContent).not.toContain('raw daemon text');
    });
    it('renders the dropped-lines clause when throttling dropped output', () => {
        ...
        expect(container.textContent).toContain('Monitor "logs" completed. (5 events, 2 lines dropped due to throttling)');
    });
    it('renders the dropped-lines clause in zh-CN when throttling dropped output', () => {
        ...
        expect(container.textContent).toContain('监控器已完成（5 个事件，因限流丢弃 2 行）：logs');
    });
    it('renders failed monitor notifications with the event count in zh-CN', () => {
        ...
        expect(container.textContent).toContain('监控器执行失败（42 个事件）：logs');
    });
    ...
    it('renders failed agent notifications via i18n in English', () => {
        const container = render(_jsx(SystemMessage, { content: "raw daemon text", variant: "info", source: "background_notification", data: { status: 'failed', kind: 'agent', description: 'worker' } }));
        expect(container.textContent).toContain('Background agent "worker" failed.');
    });
    it('falls back to raw content for unknown terminal statuses', () => {
        const container = render(_jsx(SystemMessage, { content: 'Monitor "logs" timed out.', variant: "info", source: "background_notification", data: { status: 'timeout', kind: 'monitor', description: 'logs' } }));
    ...
    it('falls back to raw content when structured fields are absent', () => {
        const container = render(_jsx(SystemMessage, { content: 'Background shell "npm test" completed.', variant: "info", source: "background_notification", data: { status: 'completed', kind: 'shell' } }));
        expect(container.textContent).toContain('Background shell "npm test" completed.');
    });
    it('renders the fallback through Markdown so session links stay clickable', () => {
        ...
        const container = render(_jsx(SystemMessage, { content: "Sub-session [\uD83E\uDDF5 abc12345](qwen-session://abc12345-full) completed.", variant: "info", source: "background_notification", data: { status: 'completed', kind: 'agent' } }));
