...
const multipleQuestionsRequest = {
    ...request,
    id: 'req-multiple',
    rawInput: {
        questions: [
...
function rerender(keyboardActive, req = request) {
    act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(AskUserQuestion, { request: req, onConfirm: onConfirm, onError: onError, keyboardActive: keyboardActive }) })));
}
function render(keyboardActive, req = request) {
...
function optionButtons() {
    return Array.from(container.querySelectorAll('[data-web-shell-ask-option]'));
}
function submitButton() {
    return (Array.from(container.querySelectorAll('button')).find((b) => b.textContent === 'Submit' ||
        b.textContent === 'Submitting...' ||
        b.textContent === '提交' ||
        b.textContent === '提交中...') ?? null);
}
...
describe('AskUserQuestion accessibility', () => {
    ...
    it('names the expanded dialog with both the tool name and the question', () => {
        ...
        expect(referenced.some((el) => el.textContent.includes('Pick a color'))).toBe(true);
    });
    it('does not steal focus when keyboardActive is false (split-view panes)', () => {
        render(false);
        expect(optionButtons().some((o) => o === document.activeElement)).toBe(false);
    });
    ...
    it('handles a shorter new request while viewing a later question', () => {
        ...
        expect(() => rerender(undefined, { ...request, id: 'req-shorter' })).not.toThrow();
    ...
    it('exits custom-input editing on Escape, then cancels on a second Escape', () => {
        ...
        const input = container.querySelector('input');
        act(() => {
            Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value')?.set?.call(input, 'Purple');
    ...
    it('keeps keyboard navigation scoped after clicking dialog content', () => {
        render(undefined);
        const panel = container.querySelector('[data-web-shell-ask-panel]');
        const questionText = Array.from(panel.querySelectorAll('p')).find((element) => element.textContent === 'Pick a color');
        act(() => {
            questionText.dispatchEvent(new MouseEvent('mousedown', { bubbles: true }));
        });
    ...
    it('restores focus to the custom answer after Enter submission fails', async () => {
        ...
        const other = optionButtons()[2];
        expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: 'network unavailable' }), 'Failed to submit answer');
        expect(other.textContent).toContain('Purple');
        expect(document.activeElement).toBe(other);
    });
    ...
    it('reports an unaccepted submit and allows retrying', async () => {
        ...
        expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: 'Failed to submit answer' }), 'Failed to submit answer');
        expect(submitButton().disabled).toBe(false);
        act(() => {
            submitButton().click();
        });
    ...
    it('reports a missing submit option without calling onConfirm', () => {
        ...
        expect(onConfirm).not.toHaveBeenCalled();
        expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: 'Submit option is unavailable' }), 'Submit option is unavailable');
    });
});
describe('AskUserQuestion multiple questions', () => {
    ...
    ])('submits from an intermediate custom input with %s+Enter', (_modifier, modifierInit) => {
        render(undefined, multipleQuestionsRequest);
        act(() => optionButtons()[2].click());
        const input = container.querySelector('input');
        act(() => {
    ...
    it('associates global shortcuts with their action buttons', () => {
        render();
        const ignore = Array.from(container.querySelectorAll('button')).find((button) => button.textContent === 'Ignore');
    ...
    it('shows contextual keyboard hints', () => {
        ...
        const singleHint = Array.from(container.querySelectorAll('p')).find((element) => element.textContent?.includes('Enter submit'));
        expect(singleHint?.parentElement).toBe(submitButton()?.parentElement);
        act(() => optionButtons()[2].click());
        expect(container.textContent).toContain('Type an answer · Esc stop editing');
        const input = container.querySelector('input');
        act(() => {
            Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value')?.set?.call(input, 'Purple');
            input.dispatchEvent(new Event('input', { bubbles: true }));
    ...
    it('keeps the shortcut footer hidden while the dialog is collapsed', () => {
        render();
        const panel = container.querySelector('[data-web-shell-ask-panel]');
        const collapse = container.querySelector('[aria-label="Collapse"]');
        act(() => collapse.click());
        expect(panel.getAttribute('aria-labelledby')).toBeNull();
        expect(container.textContent).not.toContain('Enter submit');
        const expand = container.querySelector('[aria-label="Expand"]');
        act(() => expand.click());
    ...
    it('submits a custom answer with Enter on the last question', () => {
        ...
        const input = container.querySelector('input');
        act(() => {
            Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value')?.set?.call(input, 'Medium');
            input.dispatchEvent(new Event('input', { bubbles: true }));
        });
        pressKey(input, 'Enter');
        expect(onConfirm).toHaveBeenCalledWith('req-multiple', 'submit', {
...
describe('AskUserQuestion multi-select', () => {
    ...
    it('does not advertise the global submit shortcut on a final multi-select question', () => {
        const requestWithMultiFinal = {
            ...multipleQuestionsRequest,
            rawInput: {
                questions: [
                    ...request.rawInput?.questions,
                    ...multiRequest.rawInput?.questions,
                ],
            },
        };
        render(undefined, requestWithMultiFinal);
        pressKey(optionButtons()[0], 'Enter');
        expect(container.textContent).toContain('↑↓ move · Space select/deselect · Enter select & submit');
        expect(container.textContent).not.toContain('⌘/Ctrl+Enter');
    });
    ...
    it('submits regular and custom multi-select answers together with Enter', () => {
        ...
        const input = container.querySelector('input');
        act(() => {
            Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value')?.set?.call(input, 'Custom option');
            input.dispatchEvent(new Event('input', { bubbles: true }));
        });
        pressKey(input, 'Enter');
        expect(onConfirm).toHaveBeenCalledWith('req-multi', 'submit', {
    ...
    it('selects an unselected option with Enter before advancing', () => {
        ...
        render(undefined, requestWithNextQuestion);
        expect(container.textContent).toContain('↑↓ move · Space select/deselect · Enter select & next');
        expect(container.textContent).not.toContain('⌘/Ctrl+Enter');
        expect(container.textContent).not.toContain('Esc ignore');
