...
import { getPlanNodeState, layerPlanTodos, nestedAgentToolsForTool, nestedTasksForTool, PlanExecutionView, } from './PlanExecutionView';
const todos = [
...
describe('PlanExecutionView', () => {
    it('layers dependent todos in topological order', () => {
        expect(layerPlanTodos(todos).map((layer) => layer.map((todo) => todo.id))).toEqual([['research'], ['build'], ['verify']]);
    });
    it('layers deep dependency chains without recursive traversal', () => {
        const deepTodos = Array.from({ length: 3_000 }, (_, index) => ({
            id: `todo-${index}`,
            content: `Todo ${index}`,
            status: 'pending',
        ...
        const states = deepTodos.map((todo) => getPlanNodeState(todo, deepTodosById, [], []));
    ...
    it('uses live execution state before todo and dependency state', () => {
        expect(getPlanNodeState(todos[1], todosById, [agentTool('build')], [])).toEqual({
            status: 'running',
            attention: false,
        });
        expect(getPlanNodeState(todos[1], todosById, [agentTool('build')], [task('paused')])).toEqual({
            status: 'paused',
            attention: false,
        });
    ...
    it('keeps root running precedence while surfacing a failed live descendant', () => {
        ...
        expect(getPlanNodeState(todos[1], todosById, [agentTool('build')], [root, child])).toEqual({ status: 'running', attention: true });
    });
    ...
    it('keeps nested agents under their linked root execution', () => {
        ...
        expect(nestedTasksForTool(agentTool('build'), [grandchild, root, child]).map(({ task: nested, depth }) => [nested.id, depth])).toEqual([
            ['agent-child', 1],
            ['agent-grandchild', 2],
        ]);
    });
    it('keeps the first task registered for a tool call', () => {
        ...
        expect(nestedTasksForTool(agentTool('build'), [
            firstRoot,
            firstChild,
            laterRoot,
            laterChild,
        ]).map(({ task: nested }) => nested.id)).toEqual(['agent-first-child']);
        expect(getPlanNodeState(todos[1], todosById, [agentTool('build')], [firstRoot, firstChild, laterRoot, laterChild])).toEqual({ status: 'running', attention: false });
    });
    it('rebuilds the nested agent tree from transcript tools', () => {
        ...
        expect(nestedAgentToolsForTool(root).map(({ tool, depth }) => [
            tool.callId,
            depth,
        ])).toEqual([
            ['child', 1],
    ...
    it('opens a live nested agent through its transcript tool call', () => {
        ...
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: todos, tools: [rootTool], tasks: [rootTask, childTask], onOpenSubagent: onOpen }) }));
        });
        const childButton = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.includes('Child agent'));
    ...
    it('opens a live nested agent from its task tool call id', () => {
        ...
        const childButton = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.includes('Live nested agent'));
        expect(childButton?.hasAttribute('data-plan-interactive')).toBe(true);
        act(() => childButton?.click());
    ...
    it('keeps persisted nested agents beside still-live siblings', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: todos, tools: [rootTool], tasks: [rootTask, liveChild] }) }));
        });
    ...
    it('groups executions by todo and keeps missing links unassigned', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: todos, tools: [agentTool('build'), agentTool()], tasks: [
                        task('running'),
                        task('running', {
                            id: 'agent-child',
                            label: 'Child agent',
        ...
        const step = container.querySelector('[data-plan-node-id="build"]');
        ...
        const button = Array.from(details?.querySelectorAll('button') ?? []).find((candidate) => candidate.textContent?.includes('Agent build'));
        act(() => button?.click());
    ...
    it('renders every fork and join dependency as a directed workflow edge', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: branchedTodos, tools: [], tasks: [] }) }));
        });
    ...
    it('normalizes measured coordinates when the workflow is CSS-scaled', () => {
        const scaledRect = (left, top, width, height) => ({
            x: left,
            y: top,
            left,
            top,
        ...
        const rectSpy = vi
            ...
            if (this.tagName === 'ARTICLE') {
                const [left, top] = positions[this.querySelector('span').textContent];
        ...
        expect(container
            ...
            ?.getAttribute('d')).toBe('M 210 50 C 255 50, 255 50, 300 50');
        act(() => root.unmount());
        container.remove();
        rectSpy.mockRestore();
    ...
    it('routes a cross-layer dependency below intervening nodes', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: crossLayerTodos, tools: [], tasks: [] }) }));
        });
        expect(container
            .querySelector('[data-from="docs"][data-to="release"]')
            ?.getAttribute('d')).toBe('M 500 160 H 528 V 216 H 852 V 50 H 880');
        act(() => root.unmount());
        container.remove();
        rectSpy.mockRestore();
        widthSpy.mockRestore();
    ...
    it('does not synchronously remeasure unchanged topology on task polling', () => {
        ...
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: branchedTodos, tools: [], tasks: [task('running')] }) }));
        });
    ...
    it('can receive a branched plan after mounting without todos', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: [], tools: [], tasks: [] }) }));
        });
    ...
    it('clears the selected step when the active plan is cleared', () => {
        ...
        document.body.appendChild(container);
        const root = createRoot(container);
        const renderTodos = (nextTodos) => {
            act(() => {
                root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: nextTodos, tools: [], tasks: [] }) }));
            });
        };
        renderTodos(todos);
        act(() => container
    ...
    it('recomputes edges when a later plan revises the topology', () => {
        ...
        const revisedTodos = branchedTodos.map((todo) => todo.id === 'verify' ? { ...todo, blockedBy: ['build-api'] } : todo);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: revisedTodos, tools: [], tasks: [] }) }));
        });
        const revisedEdges = Array.from(container.querySelectorAll('[data-plan-edge]'))
            .map((edge) => JSON.stringify([
            edge.getAttribute('data-from'),
            edge.getAttribute('data-to'),
        ]))
    ...
    it('skips SVG edge materialization for an excessively dense plan', () => {
        const denseTodos = Array.from({ length: 33 }, (_, index) => ({
            id: `dense-${index}`,
            content: `Dense ${index}`,
            ...
            ...(index === 0
                ? {}
                : {
                    blockedBy: Array.from({ length: index }, (__, dependencyIndex) => `dense-${dependencyIndex}`),
                }),
        }));
        const container = document.createElement('div');
        document.body.appendChild(container);
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(PlanExecutionView, { todos: denseTodos, tools: [], tasks: [] }) }));
        });
        expect(container.querySelector('[data-plan-workflow]')).not.toBeNull();
...
//# sourceMappingURL=PlanExecutionView.test.js.map