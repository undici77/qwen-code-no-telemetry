...
export function MemoryMessage({ refreshSignal = 0, addSignal = 0, addScope = 'workspace', onMessage, }) {
    const { t } = useI18n();
    ...
    const selectedEntry = entries.find((entry) => entry.scope === selectedScope) ?? entries[0];
    const loadContent = useCallback((entry, nextMode) => {
    ...
    useEffect(() => {
        ...
        reload()
            ...
            .catch((refreshError) => {
            const text = refreshError instanceof Error
    ...
    useEffect(() => {
        if (addSignal <= 0)
            return;
        if (handledAddSignalRef.current === addSignal)
            return;
    ...
    return (_jsxs("div", { className: styles.page, children: [_jsx("nav", { className: styles.tabs, "aria-label": t('memory.menu'), children: entries.map((entry) => {
                    const active = entry.scope === selectedScope;
                    return (_jsx("button", { type: "button", className: `${styles.memoryCard} ${active ? styles.memoryCardActive : ''}`, onClick: () => handleSelect(entry), children: _jsx("span", { className: styles.memoryTitle, children: entry.title }) }, entry.scope));
                }) }), _jsxs("section", { className: styles.detail, children: [_jsxs("header", { className: styles.detailHeader, children: [_jsx("div", { className: styles.detailTitleWrap, children: _jsx("div", { className: styles.detailPath, children: path }) }), _jsx("div", { className: styles.actions, children: _jsx("button", { type: "button", className: styles.actionButton, onClick: handleEdit, children: t('settings.action.edit') }) })] }), statusText && _jsx("div", { className: styles.status, children: statusText }), mode === 'edit' ? (_jsxs(_Fragment, { children: [_jsx("textarea", { ref: editorRef, className: styles.editor, value: draft, disabled: saving || contentLoading, onChange: (event) => setDraft(event.target.value), placeholder: t('memory.placeholder', {
                                    scope: selectedEntry ? scopeLabel(selectedEntry.scope, t) : '',
                                }) }), _jsxs("div", { className: styles.editorFooter, children: [_jsx("button", { type: "button", className: styles.secondaryButton, disabled: saving, onClick: () => {
                                            setDraft(content);
                                            setMode('view');
                                        }, children: t('common.cancel') }), _jsx("button", { type: "button", className: styles.primaryButton, disabled: saving || contentLoading, onClick: handleSave, children: saving ? t('memory.saving') : t('memory.save') })] })] })) : (_jsx("pre", { className: styles.content, children: contentLoading
