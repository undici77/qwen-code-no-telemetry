...
function renderCompletedThinking(durationMs, language = 'en') {
  ...
  const root = createRoot(container);
  mounted.push({ root, container });
  const tree = (isStreaming) =>
    _jsx(I18nProvider, {
      language: language,
      children: _jsx(ThinkingMessage, {
...
describe('AssistantMessage thinking logic', () => {
  ...
  it('only offers translation when the UI language is Chinese', () => {
    ...
    act(() =>
      container.querySelector('button[title="Expand thinking"]')?.click(),
    );
  ...
  it('shows a failure when generation completes without translated text', async () => {
    ...
    await act(async () => {
      container.querySelector('button[title="翻译"]')?.click();
    });
  ...
  it('shows a thinking status when generation emits thinking', async () => {
    ...
    act(() => container.querySelector('button[title="展开思考"]')?.click());
    await act(async () => {
      container.querySelector('button[title="翻译"]')?.click();
    });
...
describe('AssistantMessage streaming markdown', () => {
  it('limits intermediate renders and flushes final content immediately', () => {
    ...
    const root = createRoot(container);
    mounted.push({ root, container });
    const tree = (content, isStreaming) =>
      _jsx(I18nProvider, {
        language: 'en',
        children: _jsx(AssistantMessage, {
          content: content,
          isStreaming: isStreaming,
        }),
      });
...
describe('AssistantMessage markdown tables', () => {
  ...
  it('uses advanced tables when configured', () => {
    const container = render(
      _jsx(WebShellCustomizationProvider, {
        value: { markdownTableMode: 'advanced' },
        children: _jsx(AssistantMessage, { content: tableMarkdown }),
      }),
    );
  ...
  it('keeps streaming assistant tables plain', () => {
    const container = render(
      _jsx(AssistantMessage, { content: tableMarkdown, isStreaming: true }),
    );
