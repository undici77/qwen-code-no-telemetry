...
const { __resetForTesting, getCachedHtml, getCodeHighlighter, highlightToHtmlSync, SHIKI_CACHE_MAX, } = await import('./codeHighlighter');
...
describe('codeHighlighter retry/cleanup contracts', () => {
    ...
    it('records a failed language and does not retry the load on the next call', async () => {
        ...
        await expect(getCodeHighlighter('python')).rejects.toThrow(/previously failed/);
    ...
    it('highlightToHtmlSync returns null when codeToHtml throws (warm but failing)', async () => {
        ...
        expect(highlightToHtmlSync('const x = 1;', 'typescript', 'github-dark-default')).toBeNull();
    ...
    it('evicts the oldest cache entry once past SHIKI_CACHE_MAX', async () => {
        ...
        expect(getCachedHtml(`code-${SHIKI_CACHE_MAX}`, 'typescript', THEME)).not.toBeNull();
    });
});
//# sourceMappingURL=codeHighlighter.retry.test.js.map