...
function renderTableContent(children, language = 'en', fallback) {
    ...
    act(() => {
        root.render(_jsx(WebShellPortalRootContext.Provider, { value: portalRoot, children: _jsx(I18nProvider, { language: language, children: _jsx(EnhancedMarkdownTable, { fallback: fallback, children: children }) }) }));
    });
...
function renderTable(language = 'en') {
    return renderTableContent([
        _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Team" }), _jsx("th", { children: "Score" })] }) }, "head"),
        _jsxs("tbody", { children: [_jsxs("tr", { children: [_jsx("td", { children: "Alpha" }), _jsx("td", { children: "10" })] }), _jsxs("tr", { children: [_jsx("td", { children: "Beta" }), _jsx("td", { children: "2" })] }), _jsxs("tr", { children: [_jsx("td", { children: "Gamma" }), _jsx("td", { children: "30" })] })] }, "body"),
    ], language);
}
function renderWideTable() {
    return renderTableContent([
        _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Team" }), _jsx("th", { children: "Region" }), _jsx("th", { children: "Score" })] }) }, "head"),
        _jsxs("tbody", { children: [_jsxs("tr", { children: [_jsx("td", { children: "Alpha" }), _jsx("td", { children: "US" }), _jsx("td", { children: "10" })] }), _jsxs("tr", { children: [_jsx("td", { children: "Beta" }), _jsx("td", { children: "EMEA" }), _jsx("td", { children: "2" })] })] }, "body"),
    ]);
}
function click(el) {
    act(() => {
...
function textButton(container, text) {
    const el = [...container.querySelectorAll('button')].find((button) => button.textContent === text);
    expect(el).not.toBeNull();
    return el;
}
function openCustomColumns(container) {
    if (container.textContent?.includes('Columns shown in the table'))
        return;
...
function textButtonContaining(container, text) {
    const el = [...container.querySelectorAll('button')].find((button) => button.textContent?.includes(text));
    expect(el).not.toBeNull();
    return el;
}
...
function columnGroup(container, visibleColumnIndex) {
    const col = [...container.querySelectorAll('col')].slice(1)[visibleColumnIndex];
...
function dragColumnElements(from, to) {
    ...
    act(() => {
        from.dispatchEvent(Object.assign(new Event('dragstart', { bubbles: true }), {
            dataTransfer,
        }));
        to.dispatchEvent(Object.assign(new Event('dragover', { bubbles: true, cancelable: true }), {
            dataTransfer,
        }));
        to.dispatchEvent(Object.assign(new Event('drop', { bubbles: true, cancelable: true }), {
            dataTransfer,
        }));
...
describe('EnhancedMarkdownTable', () => {
    it('sorts numeric columns from header clicks', () => {
        ...
        expect(button(container, 'Sort by Score, ascending')
            .closest('th')
        ...
        expect(button(container, 'Sort by Score, descending')
            .closest('th')
    ...
    it('filters rows from a column value menu', () => {
        ...
        const search = container.querySelector('input[name="markdown-table-option-search-0"]');
        expect(search?.placeholder).toBe('Search filter values');
        expect(container.textContent).toContain('Select current results');
        const beta = container.querySelector('[data-name="markdown-table-filter-option-0-1"]');
        expect(beta).not.toBeNull();
        click(beta);
        click([...container.querySelectorAll('button')].find((el) => el.textContent === 'Confirm'));
    ...
    it('mounts filter overlays in the Web Shell portal root', () => {
        ...
        click(container.querySelector('[data-name="markdown-table-text-operator-0"]'));
        ...
        expect(select?.dataset.markdownTableFilterOwner).toBe(popover?.dataset.markdownTableFilterOwner);
    });
    it('applies a custom number filter', () => {
        const container = renderTable();
        click(button(container, 'Filter Score'));
        const numberFilter = container.querySelector('input[name="markdown-table-number-filter-1"]');
    ...
    it.each([
        ...
        const container = renderTable();
        click(button(container, 'Filter Score'));
        selectValue(container.querySelector('[data-name="markdown-table-number-operator-1"]'), operator);
        inputValue(container.querySelector('input[name="markdown-table-number-filter-1"]'), '10');
        click(textButton(container, 'Confirm'));
    ...
    it('applies text filter operators and reset', () => {
        const container = renderTable();
        click(button(container, 'Filter Team'));
        expect(container.querySelector('input[name="markdown-table-option-search-0"]')).toBe(document.activeElement);
        selectValue(container.querySelector('[data-name="markdown-table-text-operator-0"]'), 'equals');
        inputValue(container.querySelector('input[name="markdown-table-text-filter-0"]'), 'Alpha');
        click(textButton(container, 'Confirm'));
        ...
        expect(rowTexts(container)).toEqual(['Alpha|10', 'Beta|2', 'Gamma|30']);
        click(button(container, 'Filter Team'));
        selectValue(container.querySelector('[data-name="markdown-table-text-operator-0"]'), 'startsWith');
        inputValue(container.querySelector('input[name="markdown-table-text-filter-0"]'), 'Ga');
        click(textButton(container, 'Confirm'));
        expect(rowTexts(container)).toEqual(['Gamma|30']);
        click(button(container, 'Filter Team'));
        click(textButton(container, 'Reset'));
        click(button(container, 'Filter Team'));
        selectValue(container.querySelector('[data-name="markdown-table-text-operator-0"]'), 'endsWith');
        inputValue(container.querySelector('input[name="markdown-table-text-filter-0"]'), 'ta');
        click(textButton(container, 'Confirm'));
        expect(rowTexts(container)).toEqual(['Beta|2']);
        click(button(container, 'Filter Team'));
        click(textButton(container, 'Reset'));
        click(button(container, 'Filter Team'));
        selectValue(container.querySelector('[data-name="markdown-table-text-operator-0"]'), 'notEquals');
        inputValue(container.querySelector('input[name="markdown-table-text-filter-0"]'), 'Beta');
        click(textButton(container, 'Confirm'));
        expect(rowTexts(container)).toEqual(['Alpha|10', 'Gamma|30']);
    });
    it('applies a between number filter', () => {
        const container = renderTable();
        click(button(container, 'Filter Score'));
        selectValue(container.querySelector('[data-name="markdown-table-number-operator-1"]'), 'between');
        inputValue(container.querySelector('input[name="markdown-table-number-filter-1"]'), '10');
        inputValue(container.querySelector('input[name="markdown-table-number-filter-to-1"]'), '2');
        click(textButton(container, 'Confirm'));
    ...
    it('sorts decimal values without leading zero numerically', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Value" }) }) }, "head"),
            _jsxs("tbody", { children: [_jsx("tr", { children: _jsx("td", { children: ".5" }) }), _jsx("tr", { children: _jsx("td", { children: "-.75" }) }), _jsx("tr", { children: _jsx("td", { children: ".123" }) })] }, "body"),
        ]);
    ...
    it('sorts currency values numerically', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Amount" }) }) }, "head"),
            _jsxs("tbody", { children: [_jsx("tr", { children: _jsx("td", { children: "$100" }) }), _jsx("tr", { children: _jsx("td", { children: "$20" }) }), _jsx("tr", { children: _jsx("td", { children: "$3" }) })] }, "body"),
        ]);
    ...
    it('filters percentage values as fractions', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Ratio" }) }) }, "head"),
            _jsxs("tbody", { children: [_jsx("tr", { children: _jsx("td", { children: "40%" }) }), _jsx("tr", { children: _jsx("td", { children: "0.5" }) }), _jsx("tr", { children: _jsx("td", { children: "75%" }) })] }, "body"),
        ]);
        click(button(container, 'Filter Ratio'));
        inputValue(container.querySelector('input[name="markdown-table-number-filter-0"]'), '.45');
        click(textButton(container, 'Confirm'));
        expect(rowTexts(container)).toEqual(['0.5', '75%']);
    });
    it('preserves table alignment without applying it to row details', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { style: { textAlign: 'right' }, children: "Amount" }), _jsx("th", { style: { textAlign: 'center' }, children: "Status" })] }) }, "head"),
            _jsx("tbody", { children: _jsxs("tr", { children: [_jsx("td", { style: { textAlign: 'right' }, children: "$10" }), _jsx("td", { style: { textAlign: 'center' }, children: "Done" })] }) }, "body"),
        ]);
        const headerCells = [
        ...
        const detailElements = [
            ...detailPanel.querySelectorAll('div'),
        ];
        expect(detailElements.find((element) => element.textContent === '$10')?.style
            .textAlign).toBe('');
        expect(detailElements.find((element) => element.textContent === 'Done')?.style
            .textAlign).toBe('');
    });
    ...
    it('mounts the cell value dialog in the Web Shell portal root', () => {
        ...
        expect(portalRoot?.querySelector('[data-slot="dialog-overlay"]')).not.toBe(null);
    });
    ...
    it('sanitizes the current cell value copied from the dialog', async () => {
        const writeText = mockClipboard();
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Formula" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: "=IMPORTXML(\"https://example.com\")" }) }) }, "body"),
        ]);
        doubleClick(dataCell(container, 0, 0));
        await act(async () => {
            textButton(document.body, 'Copy').click();
        ...
        expect(writeText).toHaveBeenCalledWith('\'=IMPORTXML("https://example.com")');
    });
    it('keeps the cell value dialog in sync with table updates', async () => {
        ...
        const render = (value) => {
            act(() => {
                root.render(_jsx(I18nProvider, { language: "en", children: _jsxs(EnhancedMarkdownTable, { children: [_jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Team" }) }) }), _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: value }) }) })] }) }));
            });
        };
    ...
    it('copies an empty cell value from the dialog', async () => {
        const writeText = mockClipboard();
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Team" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", {}) }) }, "body"),
        ]);
        doubleClick(dataCell(container, 0, 0));
        await act(async () => {
            textButton(document.body, 'Copy').click();
    ...
    it('keeps table Escape handling from running behind the cell dialog', () => {
        ...
        expect(button(container, 'Sort by Team, ascending').closest('th')?.className).toContain('activeHeaderCell');
        doubleClick(dataCell(container, 0, 0));
    ...
    it('does not open the cell dialog when double clicking an interactive target', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Link" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: _jsx("a", { href: "#details", children: "Open details" }) }) }) }, "body"),
        ]);
    ...
    it('quick copies the visible sorted table', () => {
        ...
        expect(writeText).toHaveBeenCalledWith(['Team\tScore', 'Beta\t2', 'Alpha\t10', 'Gamma\t30'].join('\n'));
    });
    it('sanitizes spreadsheet formulas when copying TSV', () => {
        ...
        const container = renderTableContent([
            _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Name" }), _jsx("th", { children: "Formula" })] }) }, "head"),
            _jsxs("tbody", { children: [_jsxs("tr", { children: [_jsx("td", { children: "Alpha" }), _jsx("td", { children: "=1+1" })] }), _jsxs("tr", { children: [_jsx("td", { children: "Beta" }), _jsx("td", { children: "-10" })] }), _jsxs("tr", { children: [_jsx("td", { children: "Gamma" }), _jsx("td", { children: hiddenFormula })] }), _jsxs("tr", { children: [_jsx("td", { children: "Delta" }), _jsx("td", { children: spacedFormula })] })] }, "body"),
        ]);
        click(textButton(container, 'Copy table'));
        expect(writeText).toHaveBeenCalledWith([
            'Name\tFormula',
    ...
    it('quick copy skips hidden columns', () => {
        ...
        toggleTableColumn(container, 'Team');
        click(textButton(container, 'Copy table'));
        expect(writeText).toHaveBeenCalledWith(['Score', '10', '2', '30'].join('\n'));
    });
    ...
    it('stops resizing a column when the window blurs', () => {
        ...
        act(() => {
            window.dispatchEvent(new MouseEvent('mousemove', { bubbles: true, clientX: 220 }));
        });
    ...
    it('uses a filler column after every visible column is manually sized', () => {
        ...
        for (const column of ['Team', 'Score']) {
            act(() => {
                button(container, `Resize ${column}`).dispatchEvent(new KeyboardEvent('keydown', {
                    bubbles: true,
                    key: 'ArrowRight',
                }));
            });
        ...
        expect(table.querySelector('thead th:last-child')?.className).toContain('fillerHeaderCell');
        expect(table.querySelector('tbody td:last-child')?.className).toContain('fillerCell');
    });
    it('subtracts fixed widths from the flexible column calc()', () => {
        ...
        expect(initialColumns[1]?.style.width).toContain('0.5 * (100cqw - 40px - 0px)');
        expect(initialColumns[2]?.style.width).toContain('0.5 * (100cqw - 40px - 0px)');
        act(() => {
            button(container, 'Resize Team').dispatchEvent(new KeyboardEvent('keydown', {
                bubbles: true,
                key: 'ArrowRight',
            }));
        });
    ...
    it('reorders columns and quick copies in the visible order', () => {
        ...
        click(textButton(container, 'Copy table'));
        expect(writeText).toHaveBeenCalledWith(['Score\tTeam\tRegion', '10\tAlpha\tUS', '2\tBeta\tEMEA'].join('\n'));
    });
    ...
    it('only reorders columns when dropped within custom columns', () => {
        const container = renderWideTable();
        openCustomColumns(container);
        dragColumnElements(button(container, 'Move Score'), button(container, 'Sort by Team'));
        expect(rowTexts(container)).toEqual(['Alpha|US|10', 'Beta|EMEA|2']);
    });
    ...
    it('toggles sticky classes for the action column and first visible column', () => {
        ...
        click(textButton(container, 'Freeze first column'));
        expect(container.querySelector('[class*="tableShell"]')?.className).toContain('hasFrozenColumn');
        expect(container.textContent).not.toContain('Unfreeze first column');
        expect(container.querySelector('thead th')?.className).toContain('stickyActionHeaderCell');
        expect(button(container, 'Sort by Team').closest('th')?.className).toContain('frozenHeaderCell');
        expect(dataCell(container, 0, 0).className).toContain('frozenCell');
        click(button(container, 'Sort by Team'));
        expect(button(container, 'Sort by Team, ascending').closest('th')?.className).toContain('activeHeaderCell');
        expect(button(container, 'Sort by Team, ascending').closest('th')?.className).toContain('frozenHeaderCell');
        dragColumn(container, 'Move Score', 'Move Team');
        expect(button(container, 'Sort by Score').closest('th')?.className).toContain('frozenHeaderCell');
        openColumnMenu(container, 'Score');
        click(textButton(container, 'Unfreeze first column'));
        expect(container.textContent).not.toContain('Unfreeze first column');
        expect(container.querySelector('div')?.className).not.toContain('hasFrozenColumn');
        expect(button(container, 'Sort by Score').closest('th')?.className).not.toContain('frozenHeaderCell');
    });
    it('positions the frozen shadow from the rendered column edge', () => {
        ...
        expect(container.querySelector('[class*="frozenColumnShadow"]')
            ?.style.left).toBe('280px');
    });
    ...
    it('dismisses the first-column context menu without clearing the active column', () => {
        ...
        expect(button(container, 'Sort by Team').closest('th')?.className).toContain('activeHeaderCell');
    ...
    it('keeps the active column while clicking inside the first-column context menu', () => {
        ...
        openColumnMenu(container, 'Team');
        act(() => {
            textButton(container, 'Freeze first column').dispatchEvent(new MouseEvent('mousedown', { bubbles: true, button: 0 }));
        });
        expect(button(container, 'Sort by Team').closest('th')?.className).toContain('activeHeaderCell');
    });
    it('dismisses the first-column context menu on outside click, scroll, and resize', () => {
        ...
        expect(container.textContent).toContain('Freeze first column');
        act(() => {
            dataCell(container, 0, 0).dispatchEvent(new MouseEvent('mousedown', { bubbles: true, button: 0 }));
        });
        expect(container.textContent).not.toContain('Freeze first column');
        openColumnMenu(container, 'Team');
        expect(container.textContent).toContain('Freeze first column');
    ...
    it('collapses long cells with a tooltip and expands them from the toolbar', () => {
        const longText = 'This is a long operational note with enough content to exceed the table preview threshold and prove that the full value remains available.';
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Note" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: longText }) }) }, "body"),
        ]);
    ...
    it('treats one-line long values as expandable text', () => {
        const longText = 'A single line note can still be visually long enough to need the collapsed table preview.';
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Note" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: longText }) }) }, "body"),
        ]);
    ...
    it('copies the full long cell value instead of the expanded preview text', () => {
        const writeText = mockClipboard();
        const longText = 'A long cell value that should be copied in full even though the visual table shows an expand affordance for readability.';
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Note" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: longText }) }) }, "body"),
        ]);
    ...
    it('flattens cell-internal newlines when copying a selection', () => {
        ...
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Note" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: multilineText }) }) }, "body"),
        ]);
        dragCells(dataCell(container, 0, 0), dataCell(container, 0, 0));
        click(textButton(container, 'Copy TSV'));
    ...
    it('flattens cell-internal tabs when copying a selection', () => {
        ...
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Note" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: tabbedText }) }) }, "body"),
        ]);
        dragCells(dataCell(container, 0, 0), dataCell(container, 0, 0));
        click(textButton(container, 'Copy TSV'));
    ...
    it('only shows the long text toolbar action for visible rows', () => {
        const longText = 'A long visible note that should make the table show a toolbar expand text action before filtering.';
        const container = renderTableContent([
            _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Team" }), _jsx("th", { children: "Note" })] }) }, "head"),
            _jsxs("tbody", { children: [_jsxs("tr", { children: [_jsx("td", { children: "Long" }), _jsx("td", { children: longText })] }), _jsxs("tr", { children: [_jsx("td", { children: "Short" }), _jsx("td", { children: "ok" })] })] }, "body"),
        ]);
        expect(textButton(container, 'Expand text')).toBeDefined();
        click(button(container, 'Filter Team'));
        selectValue(container.querySelector('[data-name="markdown-table-text-operator-0"]'), 'equals');
        inputValue(container.querySelector('input[name="markdown-table-text-filter-0"]'), 'Short');
        click(textButton(container, 'Confirm'));
    ...
    it('only shows the long text toolbar action for visible columns', () => {
        const longText = 'A hidden long note should not keep the expand text toolbar action visible.';
        const container = renderTableContent([
            _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Team" }), _jsx("th", { children: "Note" })] }) }, "head"),
            _jsx("tbody", { children: _jsxs("tr", { children: [_jsx("td", { children: "Alpha" }), _jsx("td", { children: longText })] }) }, "body"),
        ]);
        expect(textButton(container, 'Expand text')).toBeDefined();
    ...
    it('selects display density from the toolbar', () => {
        ...
        expect(button(container, 'Table density').textContent).toContain('Standard density');
        expect(columns()[0]?.style.width).toBe('40px');
        expect(columns()[1]?.style.width).toContain('160px');
        selectValue(button(container, 'Table density'), 'compact');
        expect(shell?.className).toContain('densityCompact');
        expect(button(container, 'Table density').textContent).toContain('Compact density');
        expect(columns()[0]?.style.width).toBe('40px');
        ...
        expect(button(container, 'Table density').textContent).toContain('Comfortable density');
        expect(columns()[0]?.style.width).toBe('40px');
        expect(columns()[1]?.style.width).toContain('160px');
    });
    it('renders compact row details with blank values and globally expandable long values', () => {
        ...
        const container = renderTableContent([
            _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Summary" }), _jsx("th", { children: "Owner" })] }) }, "head"),
            _jsx("tbody", { children: _jsxs("tr", { children: [_jsx("td", { children: longText }), _jsx("td", {})] }) }, "body"),
        ]);
        click(button(container, 'View details for row 1'));
        expect(container.textContent).toContain('Row details');
    ...
    it('clears a selection when updated rows no longer contain it', () => {
        ...
        const render = (rows) => {
            act(() => {
                root.render(_jsx(I18nProvider, { language: "en", children: _jsxs(EnhancedMarkdownTable, { children: [_jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Value" }) }) }), _jsx("tbody", { children: rows.map((value) => (_jsx("tr", { children: _jsx("td", { children: value }) }, value))) })] }) }));
            });
        };
        mounted.push({ root, container });
        render(['10', '20', '30']);
    ...
    it('uses numeric cells only for mixed-selection arithmetic', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Label" }), _jsx("th", { children: "Value" }), _jsx("th", { children: "Extra" })] }) }, "head"),
            _jsxs("tbody", { children: [_jsxs("tr", { children: [_jsx("td", { children: "Alpha" }), _jsx("td", { children: "10" }), _jsx("td", {})] }), _jsxs("tr", { children: [_jsx("td", { children: "Beta" }), _jsx("td", { children: "2" }), _jsx("td", { children: "-4" })] })] }, "body"),
        ]);
    ...
    it('hides arithmetic statistics when the selection has no numbers', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Value" }) }) }, "head"),
            _jsxs("tbody", { children: [_jsx("tr", { children: _jsx("td", { children: "Alpha" }) }), _jsx("tr", { children: _jsx("td", {}) })] }, "body"),
        ]);
    ...
    it('preserves percent formatting for percent-only selections', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Ratio" }) }) }, "head"),
            _jsxs("tbody", { children: [_jsx("tr", { children: _jsx("td", { children: "40%" }) }), _jsx("tr", { children: _jsx("td", { children: "60%" }) })] }, "body"),
        ]);
        dragCells(dataCell(container, 0, 0), dataCell(container, 1, 0));
    ...
    it('preserves a shared currency symbol for currency-only selections', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Amount" }) }) }, "head"),
            _jsxs("tbody", { children: [_jsx("tr", { children: _jsx("td", { children: "$10" }) }), _jsx("tr", { children: _jsx("td", { children: "$20" }) })] }, "body"),
        ]);
        dragCells(dataCell(container, 0, 0), dataCell(container, 1, 0));
    ...
    it('normalizes negative zero in selection statistics', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Value" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: "-0" }) }) }, "body"),
        ]);
        dragCells(dataCell(container, 0, 0), dataCell(container, 0, 0));
    ...
    it('copies reordered selections from the keyboard copy event', () => {
        ...
        expect(event.defaultPrevented).toBe(true);
        expect(setData).toHaveBeenCalledWith('text/plain', ['10\tAlpha', '2\tBeta'].join('\n'));
    });
    ...
    it('keeps copy feedback working under StrictMode effect replay', async () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(StrictMode, { children: _jsx(I18nProvider, { language: "en", children: _jsxs(EnhancedMarkdownTable, { children: [_jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Team" }) }) }), _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: "Alpha" }) }) })] }) }) }));
        });
        mounted.push({ root, container });
        await act(async () => {
            textButton(container, 'Copy table').click();
    ...
    it('resets interactive state when table columns change', () => {
        ...
        const render = (children) => {
            act(() => {
                root.render(_jsx(WebShellPortalRootContext.Provider, { value: portalRoot, children: _jsx(I18nProvider, { language: "en", children: _jsx(EnhancedMarkdownTable, { children: children }) }) }));
            });
        };
        render([
            _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Team" }), _jsx("th", { children: "Score" })] }) }, "head"),
            _jsx("tbody", { children: _jsxs("tr", { children: [_jsx("td", { children: "Alpha" }), _jsx("td", { children: "10" })] }) }, "body"),
        ]);
        click(textButton(container, 'Custom columns'));
        click(customColumnCheckbox(document.body, 'Team'));
        expect(rowTexts(container)).toEqual(['10']);
        render([
            _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Name" }), _jsx("th", { children: "Value" })] }) }, "head"),
            _jsx("tbody", { children: _jsxs("tr", { children: [_jsx("td", { children: "Beta" }), _jsx("td", { children: "20" })] }) }, "body"),
        ]);
    ...
    it('copies selected cells from the focused keyboard copy event', () => {
        ...
        expect(event.defaultPrevented).toBe(true);
        expect(setData).toHaveBeenCalledWith('text/plain', ['Alpha\t10', 'Beta\t2'].join('\n'));
    ...
    it('selects cells with touch drag', () => {
        ...
        act(() => {
            from.dispatchEvent(touchEvent('touchstart', [{ clientX: 1, clientY: 1 }]));
    ...
    it('resets filter menu draft state when switching columns', async () => {
        const container = renderWideTable();
        click(button(container, 'Filter Team'));
        const teamSearch = container.querySelector('input[name="markdown-table-option-search-0"]');
        ...
        const scoreSearch = container.querySelector('input[name="markdown-table-option-search-2"]');
    ...
    it('lets an outside header action close the filter and run immediately', async () => {
        ...
        act(() => {
            scoreSort.dispatchEvent(new MouseEvent('pointerdown', { bubbles: true, button: 0 }));
        });
        act(() => {
            scoreSort.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, button: 0 }));
            scoreSort.dispatchEvent(new MouseEvent('mouseup', { bubbles: true, button: 0 }));
            scoreSort.dispatchEvent(new MouseEvent('click', { bubbles: true, button: 0 }));
        });
        expect(container.textContent).not.toContain('Custom filter');
        expect(rowTexts(container)).toEqual(['Beta|2', 'Alpha|10', 'Gamma|30']);
    });
    ...
    it('keeps focus within the filter popover', () => {
        ...
        const focusableElements = Array.from(menu.querySelectorAll('button, input, select')).filter((element) => !element.hasAttribute('disabled'));
        expect(focusableElements.length).toBeGreaterThan(1);
        focusableElements[0].focus();
        act(() => {
            focusableElements[0].dispatchEvent(new KeyboardEvent('keydown', {
                bubbles: true,
                cancelable: true,
                key: 'Tab',
                shiftKey: true,
    ...
    it('closes the filter menu on scroll', () => {
        ...
        click(button(container, 'Filter Team'));
        expect(container.textContent).toContain('Custom filter');
        const filterMenu = container.querySelector('[data-markdown-table-filter-owner]');
        act(() => {
    ...
    it('falls back for oversized tables', () => {
        const rows = Array.from({ length: 501 }, (_, index) => (_jsx("tr", { children: _jsx("td", { children: index }) }, index)));
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Value" }) }) }, "head"),
            _jsx("tbody", { children: rows }, "body"),
        ], 'en', _jsx("table", { children: _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: "plain fallback" }) }) }) }));
        expect(container.textContent).toContain('plain fallback');
        expect(container.textContent).not.toContain('Copy table');
    });
    it('falls back when a table has no parsed columns', () => {
        const container = renderTableContent(_jsx("tbody", { children: _jsx("tr", {}) }), 'en', _jsx("table", { children: _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: "plain fallback" }) }) }) }));
        expect(container.textContent).toContain('plain fallback');
        expect(container.textContent).not.toContain('Copy table');
    });
    ...
    it('keeps footer rows visible', () => {
        const container = renderTableContent([
            _jsx("thead", { children: _jsx("tr", { children: _jsx("th", { children: "Item" }) }) }, "head"),
            _jsx("tbody", { children: _jsx("tr", { children: _jsx("td", { children: "Alpha" }) }) }, "body"),
            _jsx("tfoot", { children: _jsx("tr", { children: _jsx("td", { children: "Total" }) }) }, "foot"),
        ]);
        expect(rowTexts(container)).toEqual(['Alpha', 'Total']);
    });
    it('parses direct table row children', () => {
        const container = renderTableContent([
            _jsxs("tr", { children: [_jsx("th", { children: "Team" }), _jsx("th", { children: "Score" })] }, "head"),
            _jsxs("tr", { children: [_jsx("td", { children: "Alpha" }), _jsx("td", { children: "10" })] }, "alpha"),
        ]);
        expect(container.textContent).toContain('Copy table');
        expect(rowTexts(container)).toEqual(['Alpha|10']);
    });
    it('localizes the new controls', () => {
        const longText = '这是一段足够长的中文备注，用来触发表格里的长文本折叠和统一展开控制，并验证本地化文案在工具栏和行详情中都能正确显示，避免后续回归。';
        const container = renderTableContent([
            _jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Team" }), _jsx("th", { children: "Note" })] }) }, "head"),
            _jsxs("tbody", { children: [_jsxs("tr", { children: [_jsx("td", { children: "Alpha" }), _jsx("td", { children: longText })] }), _jsxs("tr", { children: [_jsx("td", { children: "Beta" }), _jsx("td", {})] })] }, "body"),
        ], 'zh-CN');
