...
function renderSection(overrides = {}) {
  ...
  const container = render(
    _jsx(I18nProvider, {
      language: 'en',
      children: _jsx(ModelManagementSection, { ...props }),
    }),
  );
...
describe('ModelManagementSection', () => {
  ...
  it('marks the current model and hides its "set current" button', () => {
    const { container } = renderSection();
    const setButtons = Array.from(container.querySelectorAll('button')).filter(
      (b) => b.textContent?.trim() === 'Set current',
    );
  ...
  it('marks only the qualified variant current when base ids collide across endpoints', () => {
    ...
    const currentBadges = Array.from(container.querySelectorAll('span')).filter(
      (s) => s.textContent?.trim() === 'Current',
    );
  ...
  it('labels each row action with the model identity for screen readers', () => {
    ...
    const setCurrent = container.querySelector(
  ...
  it('omits baseUrl from the delete target for a model without one', () => {
    ...
    const arg = props.onDeleteModel.mock.calls[0][0];
