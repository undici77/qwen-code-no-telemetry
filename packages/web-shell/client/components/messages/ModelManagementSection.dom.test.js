...
function renderSection(overrides = {}) {
    ...
    const container = render(_jsx(I18nProvider, { language: "en", children: _jsx(ModelManagementSection, { ...props }) }));
...
describe('ModelManagementSection', () => {
    ...
    it('marks the current model and hides its "set current" button', () => {
        const { container } = renderSection();
        const setButtons = Array.from(container.querySelectorAll('button')).filter((b) => b.textContent?.trim() === 'Set current');
    ...
    it('deletes a model after confirmation, passing base id + baseUrl', () => {
        ...
        const deletes = Array.from(container.querySelectorAll('button')).filter((b) => b.textContent?.trim() === 'Delete');
    ...
    it('cancels the delete confirmation without deleting and restores the Delete button', () => {
        ...
        const deletesAfter = Array.from(container.querySelectorAll('button')).filter((b) => b.textContent?.trim() === 'Delete');
        expect(deletesAfter).toHaveLength(2);
        expect(Array.from(container.querySelectorAll('button')).some((b) => b.textContent?.trim() === 'Confirm')).toBe(false);
    });
    ...
    it('does not offer delete for runtime models', () => {
        ...
        const nameEl = Array.from(container.querySelectorAll('span')).find((s) => s.textContent === 'Runtime Model');
    ...
    it('marks only the qualified variant current when base ids collide across endpoints', () => {
        ...
        const currentBadges = Array.from(container.querySelectorAll('span')).filter((s) => s.textContent?.trim() === 'Current');
    ...
    it('labels each row action with the model identity for screen readers', () => {
        ...
        const setCurrent = container.querySelector('button[aria-label="Set current DeepSeek V4"]');
        const del = container.querySelector('button[aria-label="Delete DeepSeek V4"]');
    ...
    it('omits baseUrl from the delete target for a model without one', () => {
        ...
        const arg = props.onDeleteModel.mock
            .calls[0][0];
        expect('baseUrl' in arg).toBe(false);
    });
});
//# sourceMappingURL=ModelManagementSection.dom.test.js.map