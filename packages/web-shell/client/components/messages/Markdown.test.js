...
import { MAX_HIGHLIGHT_LINE_CHARS, __resetForTesting, getCodeHighlighter, } from './codeHighlighter';
import { isSafeHref, isSafeImageSrc, Markdown, markdownUrlTransform, resolveFenceLanguage, } from './Markdown';
...
describe('qwen-session:// links', () => {
    function renderMd(content) {
        ...
        act(() => {
            root.render(createElement(I18nProvider, { language: 'en' }, createElement(Markdown, { content })));
        });
        container.__unmount = () => act(() => root.unmount());
        return container;
    }
...
describe('Markdown enhanced tables', () => {
    it('uses enhanced table rendering when configured', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(createElement(I18nProvider, { language: 'en' }, createElement(Markdown, {
                content: '| A |\n| --- |\n| 1 |',
                tableMode: 'advanced',
            })));
        });
    ...
    it('keeps enhanced table when source customizes table rendering', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(createElement(I18nProvider, { language: 'en' }, createElement(WebShellCustomizationProvider, {
                value: {
                    markdown: {
                        components: {
                            table({ children }) {
                                return createElement('table', { 'data-custom-table': 'true' }, children);
                            },
                        },
                    },
                },
    ...
    it('renders the plain table fallback when enhancement throws', () => {
        ...
        expect(container.textContent).not.toContain('Copy table');
        expect(consoleError).toHaveBeenCalledWith('[web-shell] enhanced markdown table failed:', expect.any(Error), expect.any(String));
        act(() => root.unmount());
        container.remove();
    });
});
...
describe('Markdown custom code block rendering', () => {
    it('lets host renderers replace assistant fenced code blocks', async () => {
        ...
        document.body.appendChild(container);
        const root = createRoot(container);
        const renderCodeBlock = vi.fn((info) => {
            if (info.language !== 'echarts-fulldata')
                return undefined;
            return createElement('div', { 'data-chart-theme': info.theme }, `${info.source}:${info.isStreaming}:${info.code}`);
        });
        await act(async () => {
            root.render(createElement(ThemeProvider, { value: 'dark' }, createElement(WebShellCustomizationProvider, { value: { markdown: { renderCodeBlock } } }, createElement(Markdown, {
        ...
        expect(container.textContent).toContain('assistant:true:const option = {};');
    ...
    it('marks only the unterminated tail fence as incomplete', async () => {
        ...
        document.body.appendChild(container);
        const root = createRoot(container);
        const renderCodeBlock = vi.fn(() => createElement('div', { 'data-custom-code': 'true' }));
        await act(async () => {
            root.render(createElement(WebShellCustomizationProvider, { value: { markdown: { renderCodeBlock } } }, createElement(Markdown, {
                content: '```first-chart\n{"value":1}\n```\n\ntext\n\n```second-chart\n{"value":',
                source: 'assistant',
                isStreaming: true,
            })));
        });
        const infos = renderCodeBlock.mock.calls.map(([info]) => info);
    ...
    it('falls back to the default code block when the host renderer declines', async () => {
        ...
        await act(async () => {
            root.render(createElement(ThemeProvider, { value: 'light' }, createElement(WebShellCustomizationProvider, { value: { markdown: { renderCodeBlock } } }, createElement(Markdown, {
                content: '```custom-chart\nconst option = {};\n```',
                source: 'assistant',
                isStreaming: false,
            }))));
        });
        expect(renderCodeBlock).toHaveBeenCalledWith(expect.objectContaining({
        ...
        expect(container.querySelector('pre code')?.textContent).toContain('const option = {};');
        await act(async () => {
            root.unmount();
        });
        container.remove();
    ...
    it('passes punctuation language aliases to host renderers', async () => {
        ...
        expect(container.querySelector('pre code')?.textContent).toContain('std::cout << "hello";');
        await act(async () => {
            root.unmount();
        });
        container.remove();
    ...
    it('falls back to the default code block when the host renderer throws', async () => {
        ...
        try {
            ...
            expect(errorSpy).toHaveBeenCalledWith('[web-shell] custom code block renderer call failed (lang=%s):', 'echarts-fulldata', expect.any(Error));
            expect(container.textContent).toContain('echarts-fulldata');
            expect(container.querySelector('pre code')?.textContent).toContain('const option = {};');
        }
        finally {
    ...
    it('falls back to the default code block when custom rendered content throws', async () => {
        ...
        try {
            ...
            expect(renderCodeBlock).toHaveBeenCalled();
            expect(errorSpy).toHaveBeenCalledWith('[web-shell] custom code block component render (lang=echarts-fulldata) failed:', expect.any(Error), expect.any(String));
            expect(container.textContent).toContain('echarts-fulldata');
            expect(container.querySelector('pre code')?.textContent).toContain('const option = {};');
        }
        finally {
    ...
    it('retries custom rendered content after the error boundary reset key changes', async () => {
        ...
        try {
            ...
            expect(errorSpy).toHaveBeenCalledWith('[web-shell] custom code block component render (lang=echarts-fulldata) failed:', expect.any(Error), expect.any(String));
            expect(container.querySelector('[data-custom-code="good"]')).not.toBeNull();
            expect(container.querySelector('pre code')).toBeNull();
        }
        finally {
            errorSpy.mockRestore();
    ...
    it('retries custom rendered content when streaming settles', async () => {
        ...
        try {
            ...
            expect(container.querySelector('pre code')?.textContent).toContain('final');
            await act(async () => {
                root.render(createElement(WebShellCustomizationProvider, { value: { markdown: { renderCodeBlock } } }, createElement(Markdown, {
                    content: '```echarts-fulldata\nfinal\n```',
                    source: 'assistant',
            ...
            expect(errorSpy).toHaveBeenCalledWith('[web-shell] custom code block component render (lang=echarts-fulldata) failed:', expect.any(Error), expect.any(String));
            expect(container.querySelector('[data-custom-code="settled"]')).not.toBeNull();
            expect(container.querySelector('pre code')).toBeNull();
        }
        finally {
            errorSpy.mockRestore();
    ...
    it('retries custom rendered content when source or theme changes', async () => {
        ...
        const renderCodeBlock = vi.fn((info) => {
            ...
            return createElement('div', { 'data-custom-code': `${info.source}:${info.theme}` }, info.code);
        });
        const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => { });
        const tree = (source, theme) => createElement(ThemeProvider, { value: theme }, createElement(WebShellCustomizationProvider, { value: { markdown: { renderCodeBlock } } }, createElement(Markdown, {
            content: '```echarts-fulldata\nsame\n```',
            source,
        })));
        try {
            ...
            expect(container.querySelector('pre code')?.textContent).toContain('same');
            await act(async () => {
                root.render(tree('thinking', 'light'));
            });
            expect(errorSpy).toHaveBeenCalledWith('[web-shell] custom code block component render (lang=echarts-fulldata) failed:', expect.any(Error), expect.any(String));
            expect(container.querySelector('[data-custom-code="thinking:light"]')).not.toBeNull();
            expect(container.querySelector('pre code')).toBeNull();
        }
        finally {
            errorSpy.mockRestore();
    ...
    it('lets custom code components take precedence over renderCodeBlock', async () => {
        ...
        const renderCodeBlock = vi.fn(() => createElement('div', null, 'custom'));
        await act(async () => {
            root.render(createElement(WebShellCustomizationProvider, {
                value: {
                    markdown: {
                        renderCodeBlock,
                        components: {
                            code({ children }) {
                                return createElement('code', { 'data-custom-code-component': 'true' }, children);
                            },
                        },
                    },
                },
        ...
        expect(renderCodeBlock).not.toHaveBeenCalled();
        expect(container.querySelector('[data-custom-code-component="true"]')).not.toBeNull();
        await act(async () => {
            root.unmount();
        });
        container.remove();
    ...
    it('applies transformMarkdown customization before rendering', async () => {
        ...
        document.body.appendChild(container);
        const root = createRoot(container);
        const transformMarkdown = vi.fn((content) => content.replace('raw chart', 'transformed chart'));
        await act(async () => {
            root.render(createElement(WebShellCustomizationProvider, { value: { markdown: { transformMarkdown } } }, createElement(Markdown, {
                content: '**raw chart**',
                source: 'assistant',
            })));
        });
