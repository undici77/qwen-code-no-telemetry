...
import { __resetForTesting, getCachedHtml, getCodeHighlighter, highlightToHtmlSync, isTooLargeToHighlight, MAX_HIGHLIGHT_LINE_CHARS, MAX_HIGHLIGHT_TOTAL_CHARS, } from './codeHighlighter';
...
describe('isTooLargeToHighlight', () => {
    ...
    it('bails on any single line over the per-line limit (anywhere in the block)', () => {
        expect(isTooLargeToHighlight('x'.repeat(MAX_HIGHLIGHT_LINE_CHARS + 1))).toBe(true);
        // A long line earlier in the block (not just the trailing one) also bails.
        expect(isTooLargeToHighlight('x'.repeat(MAX_HIGHLIGHT_LINE_CHARS + 1) + '\nshort')).toBe(true);
    });
    it('bails when the whole block exceeds the total limit (many short lines)', () => {
        const line = 'a'.repeat(80) + '\n';
        const block = line.repeat(Math.ceil(MAX_HIGHLIGHT_TOTAL_CHARS / line.length) + 5);
...
describe('codeHighlighter', () => {
    ...
    it('does not persist streaming intermediates when persist=false', async () => {
        ...
        expect(highlightToHtmlSync('SELECT 2', 'sql', THEME, false)).toContain('shiki');
