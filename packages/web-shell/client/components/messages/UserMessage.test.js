...
describe('UserMessage', () => {
  ...
  it.each(['extension', 'file', 'mcp', 'skill'])(
  ...
  it('does not render inline email addresses as chips', () => {
    const container = render(
      _jsx(UserMessage, { content: 'mail me at a@b.test' }),
    );
  ...
  it('keeps references as text without input annotations', () => {
    const container = render(
      _jsx(UserMessage, { content: 'open @dataset:users' }),
    );
        ...
        value: {
          renderUserMessageContent: ({ content, inputAnnotations }) =>
            _jsxs('span', {
              ...
              children: [content.slice(1), ':', inputAnnotations?.length ?? 0],
            }),
        },
        children: _jsx(UserMessage, {
