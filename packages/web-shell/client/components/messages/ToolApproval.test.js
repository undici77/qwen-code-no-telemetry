...
describe('parseTitle', () => {
    ...
    it('does not split descriptive titles that contain prose colons', () => {
        const title = 'Fetching content from https://www.aliyun.com/activity (format: auto) and processing with prompt: "请列出阿里云官网当前正在进行的所有活动，包括活动名称、主要内容、优惠信息和链接"';
...
describe('ToolApproval', () => {
    it('localizes the tool name in Chinese', () => {
        ...
        act(() => {
            root.render(createElement(I18nProvider, { language: 'zh-CN' }, createElement(ToolApproval, {
                request,
                onConfirm: () => undefined,
            })));
        });
    ...
    it('orders permission options according to the product hierarchy', () => {
        ...
        act(() => {
            root.render(createElement(I18nProvider, { language: 'en' }, createElement(ToolApproval, {
                request,
                onConfirm: () => undefined,
            })));
        });
        const ids = Array.from(container.querySelectorAll('[data-option-id]')).map((el) => el.getAttribute('data-option-id'));
        expect(ids).toEqual([
            'cancel',
            'proceed_always_user',
        ...
        const labels = Array.from(container.querySelectorAll('[data-web-shell-option-label]')).map((el) => el.textContent);
