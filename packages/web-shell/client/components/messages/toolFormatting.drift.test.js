...
const toolNamesSource = readFileSync(fileURLToPath(new URL('../../../../core/src/tools/tool-names.ts', import.meta.url)), 'utf8');
function coreWireToolNames() {
    const block = toolNamesSource.match(/export const ToolNames = \{([\s\S]*?)\} as const;/);
    if (!block) {
    ...
    // here too.
    return [...block[1].matchAll(/^\s*[A-Z0-9_]+:\s*'([^']+)',/gm)].map((m) => m[1]);
}
...
//# sourceMappingURL=toolFormatting.drift.test.js.map