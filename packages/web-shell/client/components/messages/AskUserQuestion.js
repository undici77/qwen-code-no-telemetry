...
function hasCustomAnswer(value) {
    return Boolean(value?.trim());
}
export function AskUserQuestion({ request, onConfirm, onError, variant = 'inline', keyboardActive = true, }) {
    const submitShortcutLabel = typeof navigator !== 'undefined' &&
        /Mac|iPhone|iPad|iPod/i.test(navigator.platform ?? '')
        ? '⌘↵'
        : 'Ctrl↵';
    const { t } = useI18n();
    const questions = useMemo(() => Array.isArray(request.rawInput?.questions)
    ...
    const selectedIdxByQuestionRef = useRef({});
    ...
    useEffect(() => {
        ...
        setAnswers(firstQuestion && !firstQuestion.multiSelect && firstQuestion.options[0]
    ...
    const rememberSelectedIndex = useCallback((idx) => {
        selectedIdxByQuestionRef.current[currentIdx] = idx;
        selectedIdxRef.current = idx;
        setSelectedIdx(idx);
    }, [currentIdx]);
    const buildResult = useCallback((multiSelections = selectedMulti) => {
        const result = {};
    ...
    const submitDecision = useCallback(async (optionId, submittedAnswers) => {
        ...
        setSubmitting(true);
        try {
            const accepted = await onConfirm(request.id, optionId, submittedAnswers);
            if (!accepted)
                throw new Error(t('askUser.submitFailed'));
        }
        catch (error) {
            if (submissionAttemptRef.current !== attempt)
                return;
            submittedRef.current = false;
            setSubmitting(false);
    ...
    const handleSubmit = useCallback((submittedAnswers) => {
        if (submittedRef.current)
            return;
    ...
    const handleCancel = useCallback(() => {
        if (submittedRef.current)
            return;
        const cancelOption = request.options.find((o) => o.kind === 'reject_once' || o.kind === 'reject_always');
        if (!cancelOption)
            return;
        void submitDecision(cancelOption.id);
    }, [request.options, submitDecision]);
    const focusCustomInput = useCallback((initialValue) => {
        if (initialValue !== undefined) {
            setCustomInputs((prev) => ({ ...prev, [currentIdx]: initialValue }));
        }
        if (!isMulti) {
            setAnswers((prev) => {
                if (!(currentIdx in prev))
                    return prev;
                const next = { ...prev };
    ...
    const chooseOption = useCallback((idx) => {
        if (!current)
            return;
    ...
    const getSelectedIndexForQuestion = useCallback((questionIdx) => {
        ...
        if (!question.multiSelect) {
            ...
            const answerIdx = question.options.findIndex((option) => option.label === answer);
            if (answerIdx >= 0)
                return answerIdx;
        }
        return 0;
    }, [answers, customInputs, questions]);
    const selectQuestion = useCallback((nextIdx) => {
    ...
    const advanceQuestion = useCallback((submittedAnswers) => {
        setCustomFocused(false);
        if (currentIdx < questions.length - 1) {
            selectQuestion(currentIdx + 1);
            return;
    ...
    useEffect(() => {
        if (!focusAfterQuestionChangeRef.current || !current)
            return;
    ...
    useEffect(() => {
        if (customFocused || !focusCustomTriggerAfterEditRef.current)
            return;
    ...
    const handleKeyDown = useCallback((e) => {
        ...
        const isOptionTarget = e.target.closest('[data-web-shell-ask-option]');
        if (!isOptionTarget) {
            if (!current)
                return;
            if (e.key === 'ArrowLeft') {
    ...
    useEffect(() => {
        ...
        if (requestChanged && currentIdx !== 0)
            return;
    ...
    const isEmptyCustomTrigger = current !== undefined &&
        !customFocused &&
        selectedIdx === current.options.length &&
        !hasCurrentCustomAnswer;
    const contextualShortcutHint = customFocused && !hasCurrentCustomAnswer
    ...
    const shortcutHint = currentIdx > 0 && !customFocused
        ? `${t('askUser.shortcuts.previous')} · ${contextualShortcutHint}`
        : contextualShortcutHint;
    return (_jsxs("div", { className: `${styles.question} ${variant === 'floating' ? styles.floating : ''} ${collapsed ? styles.collapsed : ''}`, "data-web-shell-ask-panel": true, role: "dialog", tabIndex: -1, "aria-label": localizeToolDisplayName('ask_user_question', t), "aria-labelledby": collapsed ? undefined : `${headingId} ${questionTextId}`, onMouseDown: (e) => {
            const target = e.target;
            if (isEditableTarget(target) || target.closest('button, a[href]')) {
                return;
            }
            e.currentTarget.focus();
        }, onKeyDown: handleKeyDown, children: [_jsxs("div", { className: styles.titleLine, children: [_jsx("span", { className: styles.icon, "aria-hidden": "true", children: "?" }), _jsx("span", { className: styles.toolName, id: headingId, children: localizeToolDisplayName('ask_user_question', t) }), _jsx("span", { className: styles.toolDesc, children: t('askUser.progress', {
                            current: displayIdx + 1,
                            total: questions.length,
                        }) }), _jsxs("div", { className: styles.topRight, children: [_jsx("div", { className: styles.tabs, children: questions.map((_, i) => (_jsx("span", { className: `${styles.tab} ${i === currentIdx ? styles.tabActive : ''}`, "aria-hidden": "true" }, i))) }), _jsx("button", { type: "button", className: styles.collapseButton, onClick: () => setCollapsed((c) => !c), "aria-expanded": !collapsed, "aria-label": collapsed ? t('common.expand') : t('common.collapse'), title: collapsed ? t('common.expand') : t('common.collapse'), children: _jsx("svg", { viewBox: "0 0 16 16", className: `${styles.collapseIcon} ${collapsed ? styles.collapseIconCollapsed : ''}`, children: _jsx("path", { d: "M4 6l4 4 4-4", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5" }) }) })] })] }), !collapsed && (_jsxs(_Fragment, { children: [current ? (
                    /* Question content */
                    _jsxs(_Fragment, { children: [_jsxs("p", { className: styles.text, id: questionTextId, children: [current.question, isMulti && (_jsxs("span", { className: styles.multiHint, children: [' ', "(", t('askUser.multiHint'), ")"] }))] }), _jsx("p", { className: styles.description, children: t('askUser.selectAnswer') }), _jsxs("div", { className: styles.options, role: isMulti ? 'group' : 'radiogroup', "aria-labelledby": questionTextId, children: [current.options.map((opt, i) => {
                                        ...
                                        return (_jsxs("button", { type: "button", ref: (el) => {
                                            ...
                                            }, className: `${styles.option} ${isSelected ? styles.optionSelected : ''}`, "data-web-shell-ask-option": true, tabIndex: isActive ? 0 : -1, role: isMulti ? undefined : 'radio', "aria-checked": isMulti ? undefined : isSelected, "aria-pressed": isMulti ? isSelected : undefined, "aria-keyshortcuts": i < 9 ? String(i + 1) : undefined, disabled: submitting, onClick: () => chooseOption(i), onFocus: () => rememberSelectedIndex(i), children: [_jsx("span", { className: styles.pointer, "aria-hidden": "true", children: isActive ? '›' : ' ' }), _jsx("span", { className: styles.optionNum, "aria-hidden": "true", children: i + 1 }), _jsxs("span", { className: styles.optionContent, children: [_jsx("span", { className: styles.optionLabel, children: opt.label }), opt.description && (_jsx("span", { className: styles.optionDesc, children: opt.description }))] })] }, opt.label));
                                    }), (() => {
                                        const isCustomActive = selectedIdx === current.options.length;
                                        const hasCustomValue = hasCustomAnswer(customInputs[currentIdx]);
                                        return (_jsxs("div", { className: `${styles.option} ${hasCustomValue ? styles.optionSelected : ''}`, 
                                            ...
                                            onClick: () => {
                                                if (!submitting)
                                                    chooseOption(current.options.length);
                                            }, onMouseDown: (e) => {
                                                ...
                                                e.preventDefault();
                                                e.stopPropagation();
                                            }, children: [_jsx("span", { className: styles.pointer, "aria-hidden": "true", children: isCustomActive ? '›' : ' ' }), _jsx("span", { className: styles.editIcon, "aria-hidden": "true", children: _jsx("svg", { viewBox: "0 0 16 16", children: _jsx("path", { d: "M3.2 10.9 4 7.8 10.8 1l3.2 3.2-6.8 6.8-3 .8zM10 1.8l3.2 3.2M3 14h10", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }) }) }), customFocused ? (_jsx("input", { type: "text", className: styles.customInput, placeholder: t('askUser.typePlaceholder'), value: customInputs[currentIdx] || '', "aria-label": t('askUser.typePlaceholder'), disabled: submitting, onChange: (e) => setCustomInputs({
                                                        ...customInputs,
                                                    ...
                                                    // option choice.
                                                    onClick: (e) => e.stopPropagation(), onFocus: () => rememberSelectedIndex(current.options.length), onBlur: () => setCustomFocused(false), onKeyDown: handleCustomInputKeyDown, autoFocus: true })) : (_jsx("button", { type: "button", ref: customRef, className: `${styles.customTrigger} ${hasCustomValue ? '' : styles.optionPlaceholder}`, "data-web-shell-ask-option": true, tabIndex: isCustomActive ? 0 : -1, role: isMulti ? undefined : 'radio', "aria-checked": isMulti ? undefined : hasCustomValue, "aria-pressed": isMulti ? hasCustomValue : undefined, "aria-keyshortcuts": current.options.length < 9
                                                        ? String(current.options.length + 1)
                                                        : undefined, disabled: submitting, onFocus: () => rememberSelectedIndex(current.options.length), children: hasCustomValue
                                                        ? customInputs[currentIdx]
                                                        : t('askUser.typePlaceholder') }))] }));
                                    })()] })] })) : null, _jsxs("div", { className: styles.actions, children: [_jsx("p", { className: styles.shortcuts, title: shortcutHint, children: shortcutHint }), _jsx("button", { type: "button", className: styles.ignoreButton, disabled: submitting, "aria-keyshortcuts": "Escape", "data-shortcut": "Esc", onClick: handleCancel, children: t('askUser.ignore') }), questions.length > 1 && (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", className: styles.button, disabled: submitting || currentIdx <= 0, onClick: handlePrevious, children: t('common.previous') }), _jsx("button", { type: "button", className: styles.button, disabled: submitting || currentIdx >= questions.length - 1, onClick: handleNext, children: t('common.next') })] })), _jsx("button", { type: "button", className: `${styles.button} ${styles.submitButton}`, disabled: submitting, "aria-busy": submitting, "aria-keyshortcuts": "Control+Enter Meta+Enter", "data-shortcut": submitShortcutLabel, onClick: () => handleSubmit(), children: submitting ? (_jsxs(_Fragment, { children: [_jsx(Spinner, { className: styles.submitSpinner, "aria-hidden": "true" }), t('askUser.submitting')] })) : (t('askUser.submit')) })] })] }))] }));
}
