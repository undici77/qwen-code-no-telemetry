...
const { buildUnifiedDiff, extractDiff, fencedCodeBlock, formatSingleToolSummary, formatToolGroupSummary, getActiveTool, getRawFileDiff, getToolHeaderKind, hasExpandableContent, isWebFetchToolName, languageForPath, shouldAutoExpand, ToolGroup, ToolLine, } = await import('./ToolGroup');
...
function renderToolLine(tool, props = {}, customization = {}) {
    const container = document.createElement('div');
    document.body.appendChild(container);
    const root = createRoot(container);
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: customization, children: _jsx(ToolLine, { tool: tool, ...props }) }) }));
    });
    mounted.push({ root, container });
    return container;
}
function renderToolGroup(tools, customization = {}) {
    const container = document.createElement('div');
    document.body.appendChild(container);
    const root = createRoot(container);
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: customization, children: _jsx(ToolGroup, { tools: tools }) }) }));
    });
    mounted.push({ root, container });
    return container;
}
...
const zhT = (key, values) => {
    if (key === 'toolName.readfile')
        return '读取文件';
    return t(key, values);
};
describe('tool group summary logic', () => {
    ...
    it('keeps workspace-relative paths in multi-tool summaries', () => {
        ...
        expect(formatToolGroupSummary(tools, t, '/workspace')).toBe('Running 2 tools: ReadFile src/index.ts · ReadFile test/index.ts');
    });
    it('excludes a running background agent from a multi-tool summary', () => {
        ...
        const summary = formatToolGroupSummary(tools, t);
        expect(summary).toBe("Running 2 tools: ReadFile package.json · Grep 'ToolGroup' in path './'");
    });
    ...
    it('summarizes completed tool groups by common action type', () => {
        ...
        expect(formatToolGroupSummary(tools, t)).toBe('Edited 1 files Ran 1 commands Read 1 files Searched 1 times Updated todos 1 times Asked 2 questions');
    });
    it('formats a single shell summary as only the semantic description', () => {
        expect(formatSingleToolSummary(makeTool({
    ...
    it('falls back to command text for shell summaries without descriptions', () => {
        ...
        }), t)).toBe('Shell npm run build');
    });
    it('uses only skill names in single tool summaries', () => {
        expect(formatSingleToolSummary(makeTool({
            toolName: 'skill',
            title: 'Skill: Use skill: "qc-helper" with args: "weather in Hangzhou next 5 days"',
            args: {
                skill: 'qc-helper',
                args: 'weather in Hangzhou next 5 days',
            },
        }), t)).toBe('Skill qc-helper');
    });
    it('uses action summaries for single todo and ask-user tools', () => {
        expect(formatSingleToolSummary(makeTool({ toolName: 'todo_write' }), t)).toBe('Updated todos 1 times');
        expect(formatSingleToolSummary(makeTool({
            toolName: 'ask_user_question',
            args: { questions: [{}, {}, {}] },
        }), t)).toBe('Asked 3 questions');
        expect(formatSingleToolSummary(makeTool({
            toolName: 'ask_user_question',
            status: 'in_progress',
            args: { questions: [{}, {}, {}] },
        }), t)).toBe('Provide information');
    });
    it('counts legacy or empty AskUserQuestion inputs as one question', () => {
        expect(formatSingleToolSummary(makeTool({ toolName: 'ask_user_question' }), t)).toBe('Asked 1 question');
        expect(formatSingleToolSummary(makeTool({
            toolName: 'ask_user_question',
            args: { questions: [] },
        }), t)).toBe('Asked 1 question');
    });
    it('truncates long single tool descriptions in the chat summary', () => {
        const summary = formatSingleToolSummary(makeTool({
            toolName: 'Shell',
    ...
    it('lets custom tool header extras render single-tool chat summaries', () => {
        const container = renderToolGroup([
            ...
            renderToolHeaderExtra: (info) => (_jsxs("span", { "data-testid": "custom-summary", children: [info.kind, ":", info.description] })),
        });
    ...
    it('uses action descriptions for shell rows inside grouped summaries', () => {
        const container = renderToolGroup([
            makeTool({
                callId: 'shell',
                toolName: 'run_shell_command',
                title: 'Shell: dataworks-infra workspace list [timeout: 30000ms] (查询用户工作空间列表)',
                args: {
                    command: 'dataworks-infra workspace list',
                    description: '查询用户工作空间列表',
                    timeout: 30000,
        ...
        expect(container.textContent).not.toContain('dataworks-infra workspace list');
        expect(container.textContent).not.toContain('timeout: 30000ms');
    });
});
describe('tool output session links', () => {
    function renderSessionLinkTool(readonly) {
        const container = document.createElement('div');
        document.body.appendChild(container);
        const root = createRoot(container);
...
describe('tool expandability', () => {
    it('only marks tools with actual detail views as expandable by output', () => {
        expect(hasExpandableContent(makeTool({
            toolName: 'Shell',
...
describe('tool kind logic', () => {
    it('classifies common tool names for summary icons', () => {
        ...
        expect(getToolHeaderKind(makeTool({ toolName: 'web_fetch' }))).toBe('fetch');
        expect(getToolHeaderKind(makeTool({ toolName: 'ReadFile' }))).toBe('read');
        expect(getToolHeaderKind(makeTool({ toolName: 'edit' }))).toBe('edit');
        expect(getToolHeaderKind(makeTool({ toolName: 'write_file' }))).toBe('write');
        expect(getToolHeaderKind(makeTool({ toolName: 'todo_write' }))).toBe('todo');
        expect(getToolHeaderKind(makeTool({ toolName: 'ask_user_question' }))).toBe('ask');
    });
    ...
    it('auto-expands verbose tools only while active or failed', () => {
        expect(shouldAutoExpand(makeTool({ toolName: 'Shell', status: 'in_progress' }))).toBe(true);
        expect(shouldAutoExpand(makeTool({ toolName: 'edit', status: 'failed' }))).toBe(true);
        expect(shouldAutoExpand(makeTool({ toolName: 'Shell', status: 'completed' }))).toBe(false);
        expect(shouldAutoExpand(makeTool({ toolName: 'glob', status: 'in_progress' }))).toBe(false);
    });
});
describe('tool row rendering', () => {
    it('renders the aggregate summary for a multi-tool group', () => {
        ...
        expect(container.querySelector('button')?.textContent).toContain('package.json');
        expect(container.querySelector('button')?.textContent).toContain('ToolGroup');
    });
    it('does not show elapsed time in a running summary', () => {
        ...
        expect(container.querySelector('button')?.textContent).not.toMatch(/\d+[sm]/);
    });
    it('keeps elapsed time updating in a running tool row', () => {
        vi.useFakeTimers();
        vi.setSystemTime(6_000);
        try {
            const container = renderToolLine(makeTool({
                toolName: 'ReadFile',
                status: 'in_progress',
                startTime: 1_000,
            }));
    ...
    it('shows live elapsed time after expanding a single-tool group', () => {
        ...
        try {
            const container = renderToolGroup([
                ...
                renderToolHeaderExtra: (info) => info.elapsed ? _jsxs("span", { children: ["custom ", info.elapsed] }) : null,
            });
            const summary = container.querySelector('button');
            expect(summary?.textContent).not.toContain('5s');
            act(() => summary?.click());
            const content = container.querySelector('[class*="chatSummaryContentClip"]');
    ...
    it('keeps completed elapsed data available to custom header renderers', () => {
        const container = renderToolLine(makeTool({
            ...
            startTime: 1_000,
            endTime: 6_000,
        }), {}, { renderToolHeaderExtra: (info) => _jsx("span", { children: info.elapsed }) });
        expect(container.textContent).toContain('5s');
    });
    it.each([
        ['completed', undefined],
        ['failed', 'Agent process failed'],
    ])('shows meta for a %s agent', (status, reason) => {
        const container = renderToolLine(makeTool({
            toolName: 'Task',
            status,
            startTime: 1_000,
    ...
    it('shows a tool-kind icon on every expanded group row', () => {
        ...
        const summary = container.querySelector('button');
        act(() => summary?.click());
        const rows = container.querySelectorAll('[class*="chatSummaryGroup"] [class*="lineMain"]');
        expect(rows).toHaveLength(2);
        for (const row of rows) {
            expect(row.querySelector('svg[class*="chatSummaryToolIcon"]')).not.toBeNull();
        }
    });
    ...
    it('opens a single foreground agent from the tool summary', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SubagentDetailsProvider, { onOpen: onOpen, children: _jsx(ToolGroup, { tools: [tool] }) }) }));
        });
        mounted.push({ root, container });
        const summary = container.querySelector('button');
    ...
    it('opens a running background agent from the tool summary', () => {
        ...
        expect(container.querySelector('[class*="chatSummaryTextActive"]')).toBeNull();
    ...
    it('opens a single monitor from the tool summary', async () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(MonitorDetailsProvider, { onOpen: onOpen, children: _jsx(ToolGroup, { tools: [tool] }) }) }));
        });
        mounted.push({ root, container });
        const summary = container.querySelector('button');
        expect(summary.hasAttribute('aria-expanded')).toBe(false);
        ...
        expect(container.querySelector('[class*="chatChevronRight"]')).not.toBeNull();
    });
    it('opens a monitor tool line from a mixed tool group', async () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(MonitorDetailsProvider, { onOpen: onOpen, children: _jsx(ToolLine, { tool: tool }) }) }));
        });
        mounted.push({ root, container });
        const line = container.querySelector('[class*="lineExpandable"]');
        await act(async () => {
            line.click();
            await Promise.resolve();
        });
        ...
        expect(container.querySelector('[class*="lineChevronRight"]')).not.toBeNull();
    });
    ...
    it('resets summary expansion when the monitor identity changes', async () => {
        ...
        expect(container.querySelector('[class*="chatChevronDown"]')).not.toBeNull();
        ...
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(MonitorDetailsProvider, { onOpen: onOpen, children: _jsx(ToolGroup, { tools: [nextTool] }) }) }));
        });
        expect(container.querySelector('[class*="chatChevronDown"]')).toBeNull();
        expect(container.querySelector('[class*="chatChevronRight"]')).not.toBeNull();
    });
    ...
    it('falls back to the original tool-line expansion when monitor details are unavailable', async () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(MonitorDetailsProvider, { onOpen: onOpen, children: _jsx(ToolLine, { tool: tool, forceExpandable: true }) }) }));
        });
        mounted.push({ root, container });
        const line = container.querySelector('[class*="lineExpandable"]');
        await act(async () => {
    ...
    it('keeps a mixed group animated while a foreground tool is active', () => {
        ...
        expect(container.querySelector('[class*="chatSummaryTextActive"]')).not.toBeNull();
    });
    it('opens on-demand agent details without mounting inline content', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SubagentDetailsProvider, { onOpen: onOpen, children: _jsx(ToolLine, { tool: tool }) }) }));
        });
        mounted.push({ root, container });
        expect(container.textContent).not.toContain('large hidden result');
        expect(container.querySelector('[class*="lineExpandable"]')?.tagName).toBe('BUTTON');
        act(() => {
            container.querySelector('[class*="lineExpandable"]').click();
        });
        expect(onOpen).toHaveBeenCalledWith(tool);
    });
    it('respects hideHeader for agent tools inside SubagentDetailsProvider', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SubagentDetailsProvider, { onOpen: onOpen, children: _jsx(ToolLine, { tool: tool, hideHeader: true, forceExpanded: true }) }) }));
        });
        mounted.push({ root, container });
    ...
    it('keeps glob details visible in the header after expanding', () => {
        const pattern = '**/very-long-component-pattern-that-crosses-the-expand-threshold-*.tsx';
        const container = renderToolLine(makeTool({
            toolName: 'glob',
            args: {
                pattern,
    ...
    it('shows complete skill content in the expanded card body', () => {
        ...
        expect(output?.textContent).toBe('Base directory for this skill: /repo\n# Code Review');
    });
...
describe('tool output logic', () => {
    it('sanitizes read-file languages before building markdown fences', () => {
        ...
        expect(fencedCodeBlock('tsx', 'const fence = "~~~";')).toBe('~~~~tsx\nconst fence = "~~~";\n~~~~');
    });
    it('suppresses truncated session diffs from raw output', () => {
        const fullDiff = '--- a/file.ts\n+++ b/file.ts\n@@ -1 +1 @@\n-old\n+new';
        expect(getRawFileDiff(makeTool({
            toolName: 'edit',
...
//# sourceMappingURL=ToolGroup.test.js.map