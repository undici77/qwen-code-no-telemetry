...
import { formatToolDisplayName, getAgentCurrentToolHint, getToolDescription, getToolResultSummary, getToolSummaryDescription, localizeToolDisplayName, sanitizeControlChars, TOOL_DISPLAY_NAMES, } from './toolFormatting';
...
describe('toolFormatting', () => {
    ...
    describe('sanitizeControlChars', () => {
        ...
        it('neutralizes an ANSI color sequence via its ESC byte', () => {
            expect(sanitizeControlChars('\x1b[31mred\x1b[0m')).toBe('\\u001b[31mred\\u001b[0m');
        });
        it('leaves ordinary text, tabs, and newlines untouched', () => {
            expect(sanitizeControlChars('git log --oneline')).toBe('git log --oneline');
    ...
    it('falls back to a workspace-relative file path', () => {
        expect(getToolDescription(tool({ args: { file_path: '/workspace/project/src/index.ts' } }), '/workspace/project')).toBe('src/index.ts');
    });
    it('falls back to the basename when workspace cwd does not match', () => {
        expect(getToolDescription(tool({
            args: {
                file_path: '/Users/ytahdn/Documents/Codes/alishu/qwen-code/README.md',
            },
        }), '/Users/ytahdn/Documents/Codes/qwen/qwen-code')).toBe('README.md');
    });
    it('normalizes absolute paths embedded in title descriptions', () => {
        expect(getToolDescription(tool({
            title: 'WriteFile: Writing to /workspace/project/src/index.ts',
            toolName: 'write_file',
        }), '/workspace/project')).toBe('Writing to src/index.ts');
    });
    it('matches CLI-style grep fallback descriptions', () => {
        ...
        }), '/workspace/project')).toBe("'TODO' in path '/workspace/project/src' (filter: '*.ts')");
    });
    it('matches CLI-style glob fallback descriptions', () => {
        expect(getToolDescription(tool({
            toolName: 'glob',
            args: { pattern: '**/*.ts', path: '/Users/ytahdn/.qwen' },
        }), '/workspace/project')).toBe("'**/*.ts' in path '/Users/ytahdn/.qwen'");
    });
    it('matches CLI-style glob result summaries', () => {
        expect(getToolResultSummary(tool({
            toolName: 'glob',
    ...
    it('matches CLI-style shell fallback descriptions', () => {
        ...
        }), '/workspace/project')).toBe('npm test [in packages/web-shell] [timeout: 1000ms]');
    });
    ...
    it('uses semantic shell descriptions for summaries', () => {
        ...
        expect(getToolDescription(shellTool)).toBe('dataworks-infra workspace list [timeout: 30000ms] (查询用户工作空间列表)');
    });
    it('falls back to shell commands in summaries without timeout metadata', () => {
        expect(getToolSummaryDescription(tool({
            toolName: 'run_shell_command',
            args: {
                command: 'npm test',
                timeout: 1000,
    ...
    it('keeps long shell commands in full instead of capping at one line', () => {
        ...
        expect(getToolDescription(tool({ toolName: 'run_shell_command', args: { command } }))).toBe(command);
    });
    it('still bounds a pathologically long description', () => {
        const result = getToolDescription(tool({
            toolName: 'run_shell_command',
    ...
    describe('localizeToolDisplayName', () => {
        ...
        it('falls back to the raw wire name for unknown tools', () => {
            expect(localizeToolDisplayName('mystery_tool', getTranslator('zh-CN'))).toBe('mystery_tool');
        });
        it('has a zh translation for every tool in the display-name map', () => {
            ...
            const untranslated = Object.keys(TOOL_DISPLAY_NAMES).filter((wire) => !keepEnglish.has(wire) &&
        ...
        it('localizes the tool name in the agent activity hint', () => {
            ...
            expect(getAgentCurrentToolHint(agent, getTranslator('zh-CN'))).toContain('运行命令');
            expect(getAgentCurrentToolHint(agent, getTranslator('en'))).toContain('Shell');
        });
    });
});
//# sourceMappingURL=toolFormatting.test.js.map