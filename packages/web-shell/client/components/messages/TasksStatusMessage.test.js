...
function agentTask(id, overrides = {}) {
    return {
        kind: 'agent',
        id,
        label: `label-${id}`,
...
function renderPanel(tasks, options = {}) {
    const snapshot = {
        v: 1,
        sessionId: 'session-1',
        now: 10_000,
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(TasksStatusMessage, { message: { snapshot }, embedded: options.embedded, manageActiveEvent: false, planTodos: options.planTodos, agentTools: options.agentTools, onOpenSubagent: options.onOpenSubagent, onOpenMonitor: options.onOpenMonitor }) }));
    });
    return container;
}
describe('TasksStatusMessage monitor details', () => {
    it('opens an embedded monitor in the right-panel callback', () => {
        ...
        const label = Array.from(container.querySelectorAll('span')).find((node) => node.textContent === '[monitor] watch server log');
...
describe('TasksStatusMessage nested-agent tree', () => {
    it('leaves workflow and subagent buttons in control of their keyboard input', async () => {
        ...
        const container = renderPanel([agentTask('build', { toolUseId: tool.callId })], {
            planTodos: [
        ...
        const node = container.querySelector('[data-plan-node-id="build"]');
        ...
        const execution = Array.from(details.querySelectorAll('button')).find((button) => button.textContent?.includes('Build agent'));
    ...
    it('marks nested rows with the ↳ marker and indents by visible depth', () => {
        ...
        const indented = container.querySelector('span[style*="padding-left"]');
...
//# sourceMappingURL=TasksStatusMessage.test.js.map