...
// are in the DOM.
function renderExpandedGroup(agents) {
    const container = document.createElement('div');
    document.body.appendChild(container);
    const root = createRoot(container);
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ParallelAgentsGroup, { agents: agents }) }));
    });
...
function renderManagedGroup(agents, options = {}) {
    const container = document.createElement('div');
    document.body.appendChild(container);
    const root = createRoot(container);
    const render = (nextAgents, nextOptions = options) => {
        act(() => {
            const group = (_jsx(I18nProvider, { language: "en", children: _jsx(ParallelAgentsGroup, { agents: nextAgents, autoManageExpansion: nextOptions.autoManageExpansion, automaticCollapseDelayMs: nextOptions.automaticCollapseDelayMs, deferAutomaticCollapse: nextOptions.deferAutomaticCollapse, expandActiveWhenLive: nextOptions.expandActiveWhenLive, onAutomaticExpansionChange: nextOptions.onAutomaticExpansionChange, pendingApproval: nextOptions.pendingApproval }) }));
            root.render(nextOptions.strictMode ? _jsx(StrictMode, { children: group }) : group);
        });
    };
    render(agents);
    mounted.push({ root, container });
...
function groupSummary(container) {
    return container.querySelector('[aria-expanded]');
}
describe('ParallelAgentsGroup activity rendering', () => {
    it('renders stable active, completed, and failed activity rows without a timeline', () => {
        ...
        expect(Array.from(container.querySelectorAll('[data-agent-status]')).map((row) => row.getAttribute('data-agent-status'))).toEqual(['active', 'completed', 'failed']);
    ...
    it('keeps task, current activity, and metrics in compact stable fields', () => {
        ...
        try {
            ...
            expect(container.querySelector('[class*="rowTask"]')?.textContent).toBe('Inspect the message list');
            expect(container.querySelector('[class*="rowType"]')?.textContent).toBe('reviewer:');
            expect(container.querySelector('[class*="rowActivity"]')?.textContent).toContain('MessageList.tsx');
            expect(container.querySelector('[class*="rowStats"]')?.textContent).toBe('8s');
            expect(container.querySelector('[class*="rowAction"]')).not.toBeNull();
            act(() => vi.advanceTimersByTime(2_000));
            expect(container.querySelector('[class*="rowStats"]')?.textContent).toBe('10s');
        }
        finally {
            vi.useRealTimers();
        }
    ...
    it('does not duplicate a non-default type when the task has no description', () => {
        ...
        expect(container.querySelector('[class*="rowType"]')).toBeNull();
        expect(container.querySelector('[class*="rowTask"]')?.textContent).toBe('reviewer');
    });
    ...
    it('caps a long agent type label at the row width limit', () => {
        ...
        expect(container.querySelector('[class*="rowType"]')?.textContent).toBe(`${longType.slice(0, 50)}...:`);
    });
    it('keeps completed duration and tokens separate from the task text', () => {
        ...
        expect(container.querySelector('[class*="rowStats"]')?.textContent).toBe('8s · 214 tokens');
        expect(container.querySelector('[class*="rowTask"]')?.textContent).toBe('Audit the session route');
    });
    it('shows tokens when completed output has no duration', () => {
        ...
        expect(container.querySelector('[class*="rowStats"]')?.textContent).toBe('214 tokens');
    });
    it('keeps a cancellation reason in the activity field', () => {
        ...
        expect(container.querySelector('[class*="rowActivity"]')?.textContent).toBe('(Cancelled by user)');
        expect(container.querySelector('[class*="rowStats"]')?.textContent).toBe('8s · 214 tokens');
    });
    it('auto-expands active agents and collapses 1.5s after completion', () => {
        ...
        try {
            ...
            render(active.map((item) => ({ ...item, status: 'completed' })));
            ...
            render([
                ...active.map((item) => ({ ...item, status: 'completed' })),
    ...
    it('keeps automatically collapsed details mounted during their exit animation', () => {
        ...
        try {
            ...
            expect(groupSummary(container).getAttribute('aria-expanded')).toBe('false');
            expect(container.querySelector('[data-agent-collapse-exit="true"]')).not.toBeNull();
            expect(container
                .querySelector('[data-agent-collapse-exit="true"]')
                ?.hasAttribute('inert')).toBe(true);
            expect(groupSummary(container).getAttribute('aria-disabled')).toBe('true');
            expect(container.querySelectorAll('[data-agent-status]')).toHaveLength(2);
            expect(onAutomaticExpansionChange).toHaveBeenLastCalledWith(true);
            act(() => vi.advanceTimersByTime(179));
            expect(container.querySelector('[data-agent-collapse-exit="true"]')).not.toBeNull();
            act(() => vi.advanceTimersByTime(1));
            expect(container.querySelector('[data-agent-collapse-exit="true"]')).toBeNull();
    ...
    it('releases automatic expansion when the group unmounts', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ParallelAgentsGroup, { agents: [agent({ callId: 'a1', status: 'pending' })], autoManageExpansion: true, onAutomaticExpansionChange: onAutomaticExpansionChange }) }));
        });
        expect(onAutomaticExpansionChange).toHaveBeenLastCalledWith(true);
        act(() => root.unmount());
        container.remove();
    ...
    it('stops auto-managing expansion after the user toggles the summary', () => {
        ...
        try {
            ...
            expect(onAutomaticExpansionChange).toHaveBeenLastCalledWith(false);
            expect(onAutomaticExpansionChange).toHaveBeenCalledTimes(emissionsAfterMount + 1);
            expect(groupSummary(container).getAttribute('aria-expanded')).toBe('false');
            render(active.map((item) => ({ ...item, status: 'completed' })));
            render(active);
            expect(groupSummary(container).getAttribute('aria-expanded')).toBe('false');
    ...
    it('treats active agents seen while auto-management is disabled as baseline', () => {
        ...
        expect(groupSummary(container).getAttribute('aria-expanded')).toBe('false');
        render(active.map((item) => ({ ...item, status: 'completed' })), { autoManageExpansion: true });
        render(active, { autoManageExpansion: true });
        expect(groupSummary(container).getAttribute('aria-expanded')).toBe('true');
    });
    ...
    it('hands focus to the summary when the automatic exit starts under a focused row', () => {
        ...
        try {
            ...
            const row = container.querySelector('[data-agent-status="active"]');
            row.focus();
    ...
    it('shows live progress while background agents are pending', () => {
        ...
        try {
            ...
            expect(container.querySelectorAll('[data-agent-status="active"]')).toHaveLength(2);
        }
        finally {
            vi.useRealTimers();
        }
    ...
    it('keeps the header clock monotonic when the earliest agent finishes', () => {
        ...
        try {
            ...
            act(() => {
                root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ParallelAgentsGroup, { agents: [early, late] }) }));
            });
            mounted.push({ root, container });
            ...
            expect(container.textContent).toContain('Parallel agents 2m 31s·0/2 done');
            act(() => {
                root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ParallelAgentsGroup, { agents: [
            ...
            expect(container.textContent).toContain('Parallel agents 2m 32s·1/2 done');
        }
        finally {
            vi.useRealTimers();
        }
    ...
    it('re-anchors the header clock when a second wave of agents starts', () => {
        ...
        try {
            ...
            act(() => {
                root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ParallelAgentsGroup, { agents: [first, second] }) }));
            });
            mounted.push({ root, container });
            act(() => {
                vi.advanceTimersByTime(1_000);
            ...
            const finished = [
                { ...first, status: 'completed', endTime: 21_000 },
                { ...second, status: 'completed', endTime: 21_500 },
            ];
            act(() => {
                root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ParallelAgentsGroup, { agents: finished }) }));
            });
    ...
    it('opens nested agents through the details provider when available', () => {
        ...
        const root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SubagentDetailsProvider, { onOpen: onOpen, children: _jsx(ParallelAgentsGroup, { agents: [nested] }) }) }));
        });
        mounted.push({ root, container });
        act(() => container.querySelector('[aria-expanded]').click());
        const row = container.querySelector('[class*="row"]');
...
//# sourceMappingURL=ParallelAgentsGroup.test.js.map