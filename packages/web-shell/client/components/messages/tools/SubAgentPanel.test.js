...
function renderPanel(tool) {
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SubAgentPanel, { tool: tool, defaultExpanded: true, inline: true, hideHeader: true }) }));
    });
    mounted.push({ root, container });
    return container;
}
function makeAgentWithSubTool(subTool) {
    return {
        callId: 'agent-1',
        toolName: 'Task',
        status: 'completed',
...
describe('SubAgentPanel sub-tool timestamps', () => {
    it('renders completed result content through assistant markdown', () => {
        ...
        const markdown = container.querySelector('[data-markdown-source="assistant"]');
    ...
    it('renders each sub-tool start time, like the main transcript rows', () => {
        ...
        const container = renderPanel(makeAgentWithSubTool({
            callId: 'sub-1',
            toolName: 'Read',
            status: 'completed',
            startTime,
    ...
    it('keeps sub-tools expandable while hiding their collapsed output summary', () => {
        ...
        const row = Array.from(container.querySelectorAll('[role="button"]')).find((el) => el.textContent?.includes('Shell'));
    ...
    it('renders a completed agent stream text as the conclusion, not the live stream', () => {
        ...
        const conclusion = container.querySelector('[data-markdown-source="assistant"]');
    ...
    it('hides non-standard sub-tool summaries until the row is expanded', () => {
        ...
        const row = Array.from(container.querySelectorAll('[role="button"]')).find((el) => el.textContent?.includes('ListFiles'));
        expect(row).toBeDefined();
...
//# sourceMappingURL=SubAgentPanel.test.js.map