...
const { serialize: serializeGoalStatusMessage, parse: parseRawGoalStatusMessage, } = createSentinelSerializer(DAEMON_GOAL_STATUS_SENTINEL_PREFIX);
...
function parseGoalStatusMessage(content) {
    const parsed = typeof content === 'string' ? parseRawGoalStatusMessage(content) : content;
...
function normalizeGoalStatus(value) {
    ...
    if (typeof kind !== 'string' || !VALID_GOAL_KINDS.has(kind))
        return null;
    ...
    const lastReason = typeof record.lastReason === 'string' ? record.lastReason : undefined;
    return {
        kind: kind,
        condition,
...
function getTitle(status, t) {
    switch (status.kind) {
        case 'checking':
            return {
                title: `${t('goal.check')}${status.iterations && status.iterations > 0
...
export function GoalStatusMessage({ status, activateFooter = false, }) {
    ...
    useEffect(() => {
        if (!activateFooter || renderMode === 'readonly')
            return;
        const active = status.kind === 'set' || status.kind === 'checking';
        window.dispatchEvent(new CustomEvent(GOAL_STATUS_ACTIVE_EVENT, {
            detail: {
                active,
    ...
    const showReason = (status.kind === 'checking' ||
        status.kind === 'achieved' ||
        status.kind === 'failed' ||
        status.kind === 'aborted' ||
        status.kind === 'paused') &&
        status.lastReason?.trim();
    const reasonLabel = status.kind === 'checking' ? t('goal.judge') : t('goal.lastCheck');
    return (_jsx("div", { className: styles.message, children: _jsxs("div", { className: styles.body, children: [_jsxs("div", { className: `${styles.title} ${title.colorClass}`, children: [title.title, subtitle && _jsx("span", { className: styles.muted, children: subtitle })] }), _jsxs("div", { className: styles.row, children: [_jsxs("span", { className: styles.label, children: [t('goal.label'), ":"] }), _jsx("span", { className: styles.value, children: status.condition })] }), showReason && (_jsxs("div", { className: styles.muted, children: [reasonLabel, ": ", showReason] }))] }) }));
}
