...
export function ToolApproval({ request, onConfirm, variant = 'inline', keyboardActive = true, planTodos = [], }) {
    const { t } = useI18n();
    const displayOptions = useMemo(() => prepareDisplayOptions(request.options), [request.options]);
    const safeDefaultIndex = useMemo(() => getSafeDefaultIndex(displayOptions), [displayOptions]);
    ...
    return (_jsxs("div", { className: variant === 'floating'
            ...
            .join(' '), onKeyDown: handleKeyDown, children: [_jsxs("div", { className: styles.header, children: [_jsx("span", { className: styles.icon, "aria-hidden": "true", children: "?" }), _jsx("span", { className: styles.name, id: headingId, children: toolName })] }), descriptionText && (_jsx("div", { className: styles.desc, id: descId, title: descriptionText, children: descriptionText })), isExec && command ? (_jsx("div", { className: styles.code, children: _jsx("pre", { className: styles.codeBlock, id: commandId, title: command, children: command }) })) : contentText && contentText !== request.title ? (_jsx("pre", { className: `${styles.content}${isExitPlanApproval ? ` ${styles.planContent}` : ''}`, id: commandId, title: contentText, children: contentText })) : null, showsPlanWorkflow && (_jsx("div", { className: styles.workflow, children: _jsx(PlanExecutionView, { todos: planTodos, tools: [], tasks: [] }) })), _jsx("div", { className: styles.question, id: questionId, children: questionText }), _jsx("div", { className: styles.options, role: "radiogroup", children: displayOptions.map((option, i) => {
                    ...
                    return (_jsxs("button", { type: "button", ref: (el) => {
                            ...
                            setSelected(i);
                        }, children: [_jsx("span", { className: styles.pointer, "aria-hidden": "true", children: isSelected ? '›' : ' ' }), _jsxs("span", { className: styles.num, "aria-hidden": "true", children: [i + 1, "."] }), _jsx("span", { className: styles.label, "data-web-shell-option-label": true, children: label })] }, option.id));
                }) })] }));
}
