...
import { Fragment, createElement, isValidElement, useMemo, useRef, } from 'react';
...
import { MarkdownChartBlock, MarkdownChartProvider, createMarkdownChartComponents, isRegisteredChartLanguage, } from '@datafe-open/markdown-chart-react';
...
export function createMarkdownChartRegistry(options = {}) {
    const { resolveDataRef, validateDataRef = isSupportedLegacyDataRef, ...rendererOptions } = options;
    ...
    const boundedResolveDataRef = resolveDataRef
        ? async (ref, context) => {
            ...
            return withTimeout(Promise.resolve(resolveDataRef(resolvedRef, context)), 'Chart data reference resolution');
        }
        : undefined;
...
export function createWebShellMarkdownChartPre(registry, options = {}) {
    ...
    if (!sharedPre) {
        throw new Error('markdown-chart React adapter did not provide a pre renderer');
    }
...
export function WebShellMarkdownChartProvider({ customization, source, streaming, theme, children, }) {
    const labels = useWebShellMarkdownChartLabels(customization.labels);
    const { t } = useI18n();
    return (_jsx(MarkdownChartProvider, { registry: customization.registry, source: source, streaming: streaming, theme: theme, loadingLabel: customization.loadingLabel ?? t('echartsChart.rendering'), labels: labels, onError: customization.onError, children: children }));
}
function useWebShellMarkdownChartLabels(overrides) {
    ...
    return useMemo(() => ({
        ...
        tableNotice: ({ visibleRows, totalRows, visibleColumns, totalColumns }) => t('echartsChart.tableNotice', {
            visibleRows,
            totalRows,
            visibleColumns,
            totalColumns,
...
function withTimeout(promise, label) {
    return new Promise((resolve, reject) => {
        const timeout = globalThis.setTimeout(() => reject(new Error(`${label} timed out.`)), DATA_REF_TIMEOUT_MS);
        promise
            .then(resolve, reject)
...
function adaptLegacyRuntimeLoader(loadEcharts) {
    ...
    return async () => {
        const runtime = await withTimeout(Promise.resolve().then(loadEcharts), 'Chart runtime load');
        return {
            init(container, theme) {
                return runtime.init(container, typeof theme === 'string' ? theme : undefined);
            },
        };
    };
}
...
function MarkdownChartCodeBlock({ registry, language, source, streaming, theme, }) {
    const { t } = useI18n();
    const labels = useWebShellMarkdownChartLabels();
    return (_jsx(MarkdownChartProvider, { registry: registry, theme: theme, streaming: streaming, loadingLabel: t('echartsChart.rendering'), labels: labels, onError: reportLegacyMarkdownChartError, children: _jsx(MarkdownChartBlock, { language: language, source: source, streaming: streaming, style: { minHeight: 360 } }) }));
}
function LegacyMarkdownChartCodeBlock({ options, language, source, streaming, theme, }) {
    const registry = useMemo(() => createLegacyRegistry(options), [options]);
    return (_jsx(MarkdownChartCodeBlock, { registry: registry, language: language, source: source, streaming: streaming, theme: theme }));
}
/**
...
export function createEchartsFullDataRenderer(options = {}) {
    return function renderEchartsFullDataBlock(info) {
        ...
        return (_jsx(LegacyMarkdownChartCodeBlock, { options: options, language: normalized.language, source: normalized.source, streaming: info.isIncomplete, theme: info.theme }));
    };
}
/**
...
export function EchartsFullDataBlock({ option, parseError, isStreaming = false, theme, loadEcharts, }) {
    ...
    const stableLoadEcharts = useMemo(() => hasLoadEcharts
        ? () => {
        ...
        : undefined, [hasLoadEcharts]);
    const registry = useMemo(() => createLegacyRegistry({ loadEcharts: stableLoadEcharts }), [stableLoadEcharts]);
    if (parseError && !isStreaming) {
        return _jsx("div", { role: "alert", children: parseError });
    }
    return (_jsx(MarkdownChartCodeBlock, { registry: registry, language: "markdown-chart", source: JSON.stringify({
            version: 1,
            renderer: 'echarts',
            spec: option ?? {},
        }), streaming: isStreaming, theme: theme }));
}
