...
export function AuthMessage({ onMessage, onClose }) {
    ...
    const advancedOptionValues = useMemo(() => modality
        ? ['thinking', 'modality', 'image', 'video', 'audio', 'pdf', 'context']
        : ['thinking', 'modality', 'context'], [modality]);
    const advancedContextIndex = advancedOptionValues.indexOf('context');
    const isInputStep = currentStep === 'apiKey' ||
        currentStep === 'models' ||
    ...
    const startProvider = useCallback((nextProvider, backView = 'providers') => {
        ...
        const nextProtocol = nextProvider.protocolOptions?.[0] ?? nextProvider.protocol;
    ...
    const save = useCallback(() => {
        ...
        workspaceActions
            ...
            advancedConfig: thinking || modality || (Number.isFinite(ctxSize) && ctxSize > 0)
                ? {
    ...
    const activateAdvancedOption = useCallback((value) => {
        switch (value) {
            case 'thinking':
    ...
    const activate = useCallback(() => {
        if (view === 'groups') {
            ...
            if (group.id === 'custom') {
                const customProvider = group.providerIds
                    ...
                    .find((item) => !!item);
                if (customProvider)
                    startProvider(customProvider, 'groups');
                return;
            }
            setGroupId(group.id);
            setProviderIndex(0);
        ...
        if (view === 'providers') {
            ...
            if (selected)
                startProvider(selected, 'providers');
            return;
        }
        if (view === 'review') {
            save();
        ...
        if (currentStep === 'protocol') {
            const value = (provider.protocolOptions ?? [provider.protocol])[optionIndex];
            if (value) {
                setProtocol(value);
                if (!provider.baseUrl)
    ...
    const activateAtIndex = useCallback((index) => {
        if (view === 'groups') {
    ...
    const renderOptions = (options, selected, onSelect) => (_jsx("div", { className: styles.options, children: options.map((option, index) => (_jsx("button", { type: "button", className: `${styles.option} ${selected === index ? styles.optionActive : ''}`, onClick: () => {
                onSelect(index);
                activateAtIndex(index);
            }, children: _jsxs("div", { className: styles.optionText, children: [_jsx("div", { className: styles.label, children: option.label }), option.description && (_jsx("div", { className: styles.description, children: option.description }))] }) }, option.value))) }));
    const renderStep = () => {
        if (!provider || !currentStep)
            return null;
        if (currentStep === 'protocol') {
            const allowed = provider.protocolOptions ?? [provider.protocol];
            return renderOptions(getProtocolOptions(t).filter((option) => allowed.includes(option.value)), optionIndex, setOptionIndex);
        }
        if (currentStep === 'baseUrl') {
            if (Array.isArray(provider.baseUrl)) {
                return renderOptions(provider.baseUrl.map((option) => ({
                    value: option.url,
                    label: option.label,
                    description: option.url,
                })), optionIndex, setOptionIndex);
            }
            return (_jsxs(_Fragment, { children: [_jsx("div", { className: styles.text, children: t('auth.baseUrlPrompt') }), _jsx("input", { className: styles.input, value: baseUrl, placeholder: defaultBaseUrl(protocol), onChange: (event) => {
                        ...
                        }, autoFocus: true }), provider.documentationUrl && (_jsx("a", { className: styles.link, href: provider.documentationUrl, target: "_blank", rel: "noreferrer", children: t('auth.documentation') }))] }));
        }
        if (currentStep === 'apiKey') {
            return (_jsxs(_Fragment, { children: [provider.documentationUrl && (_jsxs("a", { className: styles.link, href: provider.documentationUrl, target: "_blank", rel: "noreferrer", children: [t('auth.documentation'), ": ", provider.documentationUrl] })), _jsx("input", { className: styles.input, type: "password", value: apiKey, placeholder: provider.apiKeyPlaceholder ?? t('auth.apiKeyPlaceholder'), onChange: (event) => {
        ...
        if (currentStep === 'models') {
            ...
            return (_jsxs(_Fragment, { children: [defaultIds && (_jsx("div", { className: styles.muted, children: t('auth.modelsPrompt', { modelIds: defaultIds }) })), _jsx("input", { className: styles.input, value: models, placeholder: defaultIds || t('auth.modelsPlaceholder'), onChange: (event) => {
        ...
        return (_jsxs(_Fragment, { children: [_jsx("div", { className: styles.text, children: t('auth.advanced.prompt') }), renderOptions(advancedOptions, optionIndex, setOptionIndex), _jsx("input", { className: styles.input, value: contextWindow, placeholder: t('common.auto'), onChange: (event) => setContextWindow(event.target.value.replace(/[^0-9]/g, '')), onFocus: () => setOptionIndex(advancedContextIndex), onKeyDown: (event) => {
                        if (event.key === 'Enter') {
                            event.preventDefault();
                            event.stopPropagation();
                            goNext();
    ...
    const activeStep = useMemo(() => {
        ...
        if (view === 'providers')
            return 2;
        if (view === 'step')
            return Math.min(3 + stepIndex, stepItems.length);
        return stepItems.length;
    }, [stepIndex, stepItems.length, view]);
    const body = (() => {
        if (loading)
            return _jsx("div", { className: styles.muted, children: t('common.loading') });
        if (view === 'groups') {
            return (_jsxs(_Fragment, { children: [renderOptions(groups.map((group) => ({
                        value: group.id,
                        label: t(group.label),
                        description: t(group.description),
                    })), groupIndex, setGroupIndex), _jsxs("div", { className: styles.terms, children: [_jsxs("div", { children: [t('auth.termsTitle'), ":"] }), _jsx("a", { className: styles.link, href: "https://qwenlm.github.io/qwen-code-docs/en/users/support/tos-privacy/", target: "_blank", rel: "noreferrer", children: "https://qwenlm.github.io/qwen-code-docs/en/users/support/tos-privacy/" })] })] }));
        }
        if (view === 'providers') {
            ...
            })), providerIndex, setProviderIndex);
        }
        if (view === 'step')
            return renderStep();
        return (_jsxs(_Fragment, { children: [_jsx("div", { className: styles.text, children: t('auth.reviewText') }), _jsx("div", { className: styles.preview, children: review })] }));
    })();
    const primaryAction = () => {
        if (view === 'review') {
            save();
    ...
    return (_jsxs("div", { className: styles.panel, children: [_jsx("div", { className: styles.steps, children: stepItems.map((label, index) => {
                    const stepNumber = index + 1;
                    return (_jsxs("div", { className: `${styles.stepPill} ${stepNumber === activeStep ? styles.stepPillActive : ''} ${stepNumber < activeStep ? styles.stepPillDone : ''}`, children: [_jsx("span", { className: styles.stepNumber, children: stepNumber }), _jsx("span", { className: styles.stepLabel, children: label })] }, `${stepNumber}:${label}`));
                }) }), _jsx("div", { className: styles.body, children: body }), error && _jsx("div", { className: styles.error, children: error }), _jsxs("div", { className: styles.actions, children: [_jsx("button", { type: "button", className: styles.actionButton, onClick: goBack, disabled: view === 'groups' || loading || saving, children: t('common.previous') }), _jsx("button", { type: "button", className: styles.actionButton, onClick: primaryAction, disabled: loading || saving, children: view === 'review'
                            ? saving
                                ? t('auth.saving')
                                : t('auth.save')
                            : t('common.next') })] })] }));
