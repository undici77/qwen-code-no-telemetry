...
describe('RootErrorFallback', () => {
    it('renders English copy by default', () => {
        const { container } = mount(_jsx(RootErrorFallback, { error: new Error('boom detail'), onRetry: () => { } }));
    ...
    it('shows the raw error message only in dev, never in production', () => {
        vi.stubEnv('DEV', true);
        const dev = mount(_jsx(RootErrorFallback, { error: new Error('boom detail'), onRetry: () => { } }));
        expect(dev.container.textContent).toContain('boom detail');
        vi.stubEnv('DEV', false);
        const prod = mount(_jsx(RootErrorFallback, { error: new Error('boom detail'), onRetry: () => { } }));
    ...
    it('renders zh-CN copy when language is zh-CN', () => {
        const { container } = mount(_jsx(RootErrorFallback, { error: new Error('x'), onRetry: () => { }, language: "zh-CN" }));
    ...
    it('catches an App-level render crash instead of white-screening', () => {
        ...
        const { container } = mount(_jsx(RootBoundary, { children: _jsx(BrokenApp, {}) }));
        const alert = container.querySelector('[role="alert"]');
    ...
    it('recovers when the user clicks "Try again" after the cause is gone', () => {
        ...
        const { container } = mount(_jsx(RootBoundary, { children: _jsx(FlakyApp, {}) }));
        ...
        expect(container.querySelector('[data-testid="app"]')?.textContent).toBe('recovered');
