...
export function ToastHost({ toasts, onDismiss, elevated = false, }) {
    ...
    if (toasts.length === 0)
        return null;
    const host = (_jsx("div", { className: `${styles.host} ${elevated ? styles.hostElevated : ''}`, role: "status", "aria-live": "polite", "data-web-shell-toast-host": true, children: toasts.map((toast) => (_jsx(ToastItem, { toast: toast, onDismiss: onDismiss }, toast.id))) }));
    ...
    if (elevated && portalRoot)
        return createPortal(host, portalRoot);
    return host;
}
