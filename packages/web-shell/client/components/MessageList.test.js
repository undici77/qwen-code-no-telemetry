...
import { attachTurnOutputs, applyTurnCollapse, findDisplayItemIndex, findTurnIdForIndex, getSessionTimelineEntries, getSessionTimelineRangeForIndexes, getSessionTimelineSignature, getTurnTimelineNode, getDisplayItemVirtualKey, getTurnIdByDisplayIndex, groupParallelAgents, pinActiveParallelAgentsToTurnEnd, shouldAdjustVirtualScrollPosition, shouldUseVirtualScroll, VIRTUAL_SCROLL_THRESHOLD, } from './MessageList';
...
function collapseOf(items, idxOrTurnId) {
    const idx = typeof idxOrTurnId === 'number'
        ? idxOrTurnId
    ...
    if (next && next.type === 'turn_collapse')
        return next.turnCollapse;
    return undefined;
}
function messageById(items, id) {
    const item = items.find((item) => item.type === 'message' && item.message.id === id);
    if (!item)
        throw new Error(`expected message row ${id}`);
    return messageRow(item);
}
function makeThinkingMessage(id, content = 'pondering') {
    return {
        id,
        role: 'thinking',
        content,
...
function makeSystemMessage(id) {
    return { id, role: 'system', content: 'heads up', variant: 'error' };
}
function makeBackgroundNotification(id, toolUseId) {
    return {
        id,
        role: 'system',
...
function makePlanMessage(id) {
    return { id, role: 'plan', todos: [] };
}
function makeAgentToolGroup(id, toolName = 'Agent', timestamp) {
    return {
        id,
        role: 'tool_group',
        tools: [
...
function makeBackgroundAgentToolGroup(id, status = 'pending') {
    return {
        id,
        role: 'tool_group',
        tools: [
...
function makeMultiToolGroup(id) {
    return {
        id,
        role: 'tool_group',
        tools: [
...
function makeUserShellMessage(id) {
...
function makeThoughtMessage(id) {
    return {
        id,
        role: 'thinking',
...
describe('attachTurnOutputs', () => {
    it('keeps outputs for a transcript that starts before a user turn', () => {
        ...
        const items = attachTurnOutputs([{ type: 'message', key: message.id, message }], false, new Map([[message.id, changes]]));
        expect(items).toHaveLength(2);
    ...
    it('keeps outputs for a leading grouped parallel-agent row', () => {
        ...
        const outputItems = attachTurnOutputs(items, false, new Map([['x1', changes]]));
...
describe('pinActiveParallelAgentsToTurnEnd', () => {
    const keys = (items) => items.map((item) => (item.type === 'message' ? item.message.id : item.key));
    ...
    it('keeps an automatically expanded terminal group pinned until it closes', () => {
        ...
        expect(keys(pinActiveParallelAgentsToTurnEnd(items, new Set(['par-a1'])))).toEqual(['u1', 'answer', 'par-a1']);
    });
    ...
    it('flushes an automatically expanded group before the next turn starts', () => {
        ...
        expect(keys(pinActiveParallelAgentsToTurnEnd(items, new Set(['par-a1'])))).toEqual(['u1', 'answer', 'par-a1', 'u2', 'next-answer']);
    });
    it.each([
        ['user', makeUserMessage('u2')],
        ['user shell', makeUserShellMessage('shell')],
    ])('does not move an active group across the next %s turn', (_label, next) => {
        const items = groupParallelAgents([
            makeUserMessage('u1'),
            makeBackgroundAgentToolGroup('a1'),
            makeBackgroundAgentToolGroup('a2'),
    ...
    it('keeps attached turn outputs above the pinned active group', () => {
        ...
        const withOutputs = attachTurnOutputs(grouped, false, new Map([['u1', changes]]));
...
describe('getTurnTimelineNode', () => {
    ...
    it('classifies thinking blocks as thought nodes', () => {
        expect(getTurnTimelineNode(item(makeThinkingMessage('think')))).toMatchObject({
            kind: 'thought',
        });
    });
    it('classifies mid-turn assistant text as commentary nodes', () => {
        expect(getTurnTimelineNode(item(makeAssistantMessage('assistant')))).toMatchObject({
            kind: 'commentary',
        });
    });
    it('does not add a node to the final assistant answer', () => {
        expect(getTurnTimelineNode({
    ...
    it('classifies tool groups and plans', () => {
        expect(getTurnTimelineNode(item(makeMultiToolGroup('tools')))).toMatchObject({
            kind: 'tool',
            label: '2 tool calls',
        });
    ...
    it('classifies mid-turn status and ignores plain user rows', () => {
        expect(getTurnTimelineNode(item({
            id: 'status',
            role: 'system',
            content: 'inserted',
            variant: 'info',
...
describe('getSessionTimelineEntries', () => {
    ...
    it('builds one entry per user turn', () => {
        expect(getSessionTimelineEntries([
            makeUserMessage('u1'),
    ...
    it('cleans markdown markers from timeline details', () => {
        const [entry] = getSessionTimelineEntries([
            ...
            {
                ...makeAssistantMessage('a1'),
                content: '**agent-reproduce-align** – 对齐测试技能\n\n**用途：** 在 [Qwen Code](https://example.com) 中运行参考代码，中文*强调*·*范围*—*引用*「_下划线_，保留 snake_case。',
            },
        ]);
        expect(entry?.label).toBe('介绍下 `agent-reproduce-align`');
        expect(entry?.detail).toBe('agent-reproduce-align – 对齐测试技能 用途： 在 Qwen Code 中运行参考代码，中文强调·范围—引用「下划线，保留 snake_case。');
    });
    it('cleans preview markdown without rewriting code text', () => {
        ...
        expect(entry?.detail).toBe('Title quoted item gone alt text *literal* **code**');
    });
...
describe('getSessionTimelineRangeForIndexes', () => {
    it('maps visible rows to a timeline range and current turn', () => {
        ...
        expect(getSessionTimelineRangeForIndexes(visibleItems, [1, 2, 3, 4], entryIndexById, 3, turnIdByDisplayIndex)).toEqual({
            startIndex: 0,
            endIndex: 1,
            currentIndex: 1,
        });
    ...
    it('returns null when visible rows do not belong to a turn', () => {
        const entryIndexById = new Map([['u1', 0]]);
        expect(getSessionTimelineRangeForIndexes([], [0], entryIndexById, 0)).toBeNull();
    });
    it('ignores out-of-bounds rows when mapping the visible range', () => {
        ...
        const entryIndexById = new Map(entries.map((entry, index) => [entry.id, index]));
        const visibleItems = groupParallelAgents(messages);
        expect(getSessionTimelineRangeForIndexes(visibleItems, [-1, 0, 99], entryIndexById, 99)).toEqual({
            startIndex: 0,
            endIndex: 0,
            currentIndex: 0,
        });
...
describe('getDisplayItemVirtualKey', () => {
    it('keeps message and grouped rows in separate key namespaces', () => {
        expect(getDisplayItemVirtualKey({
            type: 'message',
            key: 'header',
...
function collapseItems(items, opts = {}) {
...
function rowIds(items) {
    return items.map((item) => item.type === 'message' ? item.message.id : item.key);
}
describe('applyTurnCollapse', () => {
    ...
    it('does not await a background launch core rejected before it started', () => {
        ...
        const collapseState = (messages) => collapseOf(collapseItems(groupParallelAgents(messages)), 'u1');
    ...
    it('keeps the turn open while a completed background agent awaits its summary', () => {
        ...
        const collapseState = (messages, isResponding) => collapseOf(collapseItems(groupParallelAgents(messages), { isResponding }), 'u1');
    ...
    it('does not consume an agent completion for a metadata-less monitor', () => {
        const monitorNotification = makeBackgroundNotification('notification-monitor');
    ...
    ])('releases the pending background summary on %s', (source, collapsed) => {
        const items = groupParallelAgents([
            { ...makeUserMessage('u1'), timestamp: 1_000 },
            makeBackgroundAgentToolGroup('a1', 'completed'),
            makeBackgroundAgentToolGroup('a2', 'completed'),
