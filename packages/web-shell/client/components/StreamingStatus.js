...
import { PHRASE_CHANGE_INTERVAL_MS, getLoadingPhrases, } from '../constants/loadingPhrases';
...
export function StreamingStatus({ startedAt, showPhrase = true, }) {
    const streamingState = useStreamingState();
    const { estimatedOutputTokens, isReceivingContent } = useStreamingLoadingMetrics();
    ...
    const resolvePhrases = useCallback((lang) => {
        try {
            return customLoadingPhrasesRef.current?.(lang) ?? getLoadingPhrases(lang);
        }
        catch (error) {
            console.warn('[web-shell] loadingPhrases resolver threw; using defaults', error);
    ...
    const [loadingPhrase, setLoadingPhrase] = useState(() => resolvePhrases(language)[0] ?? '');
    ...
    const tokenStr = estimatedOutputTokens > 0
        ? ` · ${arrow} ${t('stream.tokens', { count: formatTokenCount(estimatedOutputTokens) })}`
        : '';
    return (_jsxs("div", { className: styles.status, children: [_jsx("span", { className: styles.spinner, children: spinnerChar }), showPhrase && loadingPhrase && (_jsx("span", { className: styles.label, children: loadingPhrase })), _jsxs("span", { className: styles.meta, children: ["(", timeStr, tokenStr, " \u00B7 ", t('stream.cancel'), ")"] })] }));
}
...
//# sourceMappingURL=StreamingStatus.js.map