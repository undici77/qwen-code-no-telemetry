...
function mount(props = {}) {
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(StatusBar, { onSelectMode: vi.fn(), onSelectModel: vi.fn(), onShowContext: vi.fn(), onOpenSettings: vi.fn(), tasks: [], ...props }) }));
    });
    return container;
}
const goalButton = () => document.querySelector('button[aria-label^="Goals"]');
describe('StatusBar goal pill', () => {
    it('names the active goal in its accessible label', () => {
        ...
        expect(goalButton()?.getAttribute('aria-label')).toBe('Goals: all tests pass');
    ...
    it('opens the Goals page when activated', () => {
        ...
        act(() => {
            goalButton()?.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }));
        });
