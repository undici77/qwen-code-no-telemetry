...
let container;
let root;
async function renderRequests({ channelName = 'release-bot', list = vi.fn().mockResolvedValue(PENDING), approve = vi.fn(), listApprovals = vi.fn().mockResolvedValue({ senderIds: ['paired-user'] }), revokeApproval = vi.fn(), staticAllowedUsers = [], language = 'en', } = {}) {
    await act(async () => {
        root.render(_jsx(I18nProvider, { language: language, children: _jsx(ChannelPairingRequests, { channelName: channelName, listRequests: list, approveRequest: approve, listApprovals: listApprovals, revokeApproval: revokeApproval, staticAllowedUsers: staticAllowedUsers }) }));
    });
...
describe('ChannelPairingRequests', () => {
    it('loads and displays pending requests for the selected Channel', async () => {
        ...
        expect(container.textContent).toContain('Approvals take effect immediately; Save and Cancel do not undo them.');
        const approve = Array.from(container.querySelectorAll('button')).find((item) => item.textContent?.trim() === 'Approve');
        expect(approve?.getAttribute('aria-label')).toBe('Approve Ada, code ABCD1234');
    });
    it('identifies the group and requesting member for a group pairing request', async () => {
        ...
        const button = container.querySelector('button[aria-label="Approve Group: Release Team, code GROUP123"]');
        await act(async () => {
            button?.click();
        });
        expect(approve).toHaveBeenCalledWith('release-bot', 'GROUP123');
        expect(container.textContent).toContain('Group: Release Team can now use this Channel.');
        ...
        expect(container.querySelector('button[aria-label="Revoke Group: group-7"]')).not.toBeNull();
    });
    it('shows pairing approvals and distinguishes configured allowlist access', async () => {
        ...
        expect(container.textContent).toContain('Configured allowlist users remain allowed after a pairing approval is revoked.');
    ...
    it('retries after loading pairing approvals fails', async () => {
        ...
        const retry = Array.from(container.querySelectorAll('button')).find((item) => item.textContent?.trim() === 'Try again');
        await act(async () => {
            retry?.click();
        });
        expect(listApprovals).toHaveBeenCalledTimes(2);
        expect(container.querySelector('button[aria-label="Revoke paired-user"]')).not.toBeNull();
    });
    it('confirms and revokes only the selected pairing approval', async () => {
        ...
        const revoke = Array.from(container.querySelectorAll('button')).find((item) => item.getAttribute('aria-label') === 'Revoke paired-user');
        await act(async () => {
            revoke?.click();
        });
        expect(document.body.textContent).toContain('Revoke pairing approval for paired-user?');
        expect(revokeApproval).not.toHaveBeenCalled();
        const confirm = Array.from(document.body.querySelectorAll('button')).find((item) => item.textContent?.trim() === 'Revoke approval');
        await act(async () => {
            confirm?.click();
        });
        ...
        expect(container.querySelector('button[aria-label="Revoke paired-user"]')).toBeNull();
        expect(container.textContent).toContain('second-user');
        expect(container.textContent).toContain('Pairing approval for paired-user was revoked.');
    });
    it('lists and revokes a group pairing approval by group ID', async () => {
        ...
        const revoke = container.querySelector('button[aria-label="Revoke Group: group-7"]');
        await act(async () => {
            revoke?.click();
        });
        expect(document.body.textContent).toContain('Revoke pairing approval for Group: group-7?');
        const confirm = Array.from(document.body.querySelectorAll('button')).find((item) => item.textContent?.trim() === 'Revoke approval');
        await act(async () => {
            confirm?.click();
        });
        ...
        expect(container.querySelector('button[aria-label="Revoke Group: group-7"]')).toBeNull();
        expect(container.querySelector('button[aria-label="Revoke Group: group-8"]')).not.toBeNull();
        expect(container.querySelector('button[aria-label="Revoke paired-user"]')).not.toBeNull();
        expect(container.textContent).toContain('Pairing approval for Group: group-7 was revoked.');
    });
    it('does not approve a request while a revoke is in flight', async () => {
        ...
        const revoke = container.querySelector('button[aria-label="Revoke paired-user"]');
        await act(async () => {
            revoke?.click();
        });
        const confirm = Array.from(document.body.querySelectorAll('button')).find((item) => item.textContent?.trim() === 'Revoke approval');
        ...
        const approveButton = Array.from(container.querySelectorAll('button')).find((item) => item.textContent?.trim() === 'Approve');
    ...
    it('cancels the revoke confirmation without revoking the approval', async () => {
        ...
        const cancel = Array.from(document.body.querySelectorAll('button')).find((item) => item.textContent?.trim() === 'Cancel');
        await act(async () => {
            cancel?.click();
        });
        expect(revokeApproval).not.toHaveBeenCalled();
    ...
    it('approves a request and replaces the list with the daemon response', async () => {
        ...
        const button = Array.from(container.querySelectorAll('button')).find((item) => item.textContent?.trim() === 'Approve');
        await act(async () => {
            button?.click();
        });
        ...
        expect(listApprovals).toHaveBeenCalledTimes(2);
        expect(container.querySelector('button[aria-label="Revoke user-42"]')).not.toBeNull();
    });
    it('refreshes pairing approvals when a revoke target is already gone', async () => {
        ...
        expect(container.textContent).not.toContain('Pairing approval was not found.');
        expect(container.querySelector('button[aria-label="Revoke paired-user"]')).toBeNull();
    });
    ...
    ])('uses an actionable message when the daemon does not respond', async (error) => {
        const list = vi.fn().mockRejectedValue(error);
        await renderRequests({ list });
        expect(container.textContent).toContain('Pairing requests are temporarily unavailable. Try again.');
    ...
    it('uses an actionable message when approval cannot reach the daemon', async () => {
        const error = Object.assign(new Error('POST /workspaces/:workspace/channels/:name/pairing-requests/approve: HTTP 500'), { status: 500, body: undefined });
        const approve = vi.fn().mockRejectedValue(error);
        await renderRequests({ approve });
        const button = Array.from(container.querySelectorAll('button')).find((item) => item.textContent?.trim() === 'Approve');
        await act(async () => {
    ...
    it('uses an actionable message for malformed daemon errors', async () => {
        const error = Object.assign(new Error('POST /workspaces/:workspace/channels/:name/pairing-requests/approve: [object Object]'), {
            status: 502,
    ...
    it('refreshes the list when an approval request has expired', async () => {
        ...
        const error = Object.assign(new Error('POST /workspaces/:workspace/channels/:name/pairing-requests/approve: Pairing request was not found or has expired.'), {
            status: 404,
            body: {
        ...
        expect(container.textContent).toContain('Pairing request was not found or has expired.');
        expect(container.textContent).not.toContain('ABCD1234');
    });
    ...
    it('uses an actionable message for transport failures', async () => {
        const error = Object.assign(new Error('GET /workspaces/:workspace/channels/:name/pairing-requests: HTTP 500'), { status: 500, body: undefined });
        const list = vi.fn().mockRejectedValue(error);
        await renderRequests({ list });
        expect(container.textContent).toContain('Pairing requests are temporarily unavailable. Try again.');
        expect(container.textContent).not.toContain('GET /workspaces/:workspace/channels/:name/pairing-requests');
    });
    it('uses consistent approval wording in Chinese', async () => {
        await renderRequests({ language: 'zh-CN' });
        expect(container.textContent).toContain('批准会立即生效，保存或取消都不会撤销已批准的访问。');
    ...
    it('ignores an approval response after the selected Channel changes', async () => {
        let resolveApproval;
        const approve = vi.fn(() => new Promise((resolve) => {
        ...
        expect(container.textContent).toContain('WXYZ5678');
        expect(container.textContent).not.toContain('Ada can now use this Channel.');
    });
...
//# sourceMappingURL=ChannelPairingRequests.test.js.map