...
function isMissingField(field, draft) {
    ...
    if (field.kind === 'record') {
        ...
        try {
            ...
            if (!isRecord(parsed))
                return true;
            return Object.values(parsed).every((v) => typeof v !== 'string' || !v.trim());
        }
        catch {
            return true;
        }
...
export function validateChannelEditorDraft(descriptor, draft, existingNames) {
    ...
    for (const field of descriptor.fields) {
        ...
        if (field.required && isMissingField(field, draft)) {
            ...
            else if (field.exclusiveMinimum !== undefined &&
...
function assignField(config, field, rawValue) {
    ...
    if (field.kind === 'number') {
        ...
        try {
            ...
            const filtered = Object.fromEntries(Object.entries(parsed).filter(([, v]) => typeof v === 'string' && v.trim()));
...
export function buildChannelUpsertRequest(descriptor, draft, expectedRevision, instance) {
    const config = {
        ...(instance?.config ?? {}),
