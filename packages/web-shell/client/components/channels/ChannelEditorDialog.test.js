...
const OPTIONAL_SECRET = {
    ...DINGTALK,
    fields: DINGTALK.fields.map((field) => field.key === 'clientSecret' ? { ...field, required: false } : field),
};
const GROUP_POLICY_DESCRIPTOR = {
    type: 'github',
    displayName: 'GitHub',
    manageable: true,
    fields: [
...
async function renderDialog(props = {}) {
    await act(async () => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ChannelEditorDialog, { open: true, descriptor: DINGTALK, expectedRevision: "revision-1", existingNames: [], onOpenChange: vi.fn(), onSave: vi.fn().mockResolvedValue(undefined), onReload: vi.fn().mockResolvedValue(undefined), listPairingRequests: vi.fn().mockResolvedValue({ requests: [] }), approvePairingRequest: vi.fn(), listPairingApprovals: vi.fn().mockResolvedValue({ senderIds: [] }), revokePairingApproval: vi.fn(), ...props }) }));
    });
}
...
describe('ChannelEditorDialog', () => {
    ...
    it('preserves a stored secret until Replace is explicitly selected', async () => {
        ...
        const replace = Array.from(document.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Replace');
        await act(async () => {
            replace?.click();
        });
    ...
    it('explains that pairing requests appear after a new Channel is saved', async () => {
        ...
        expect(document.body.textContent).toContain('Pending requests will appear here after this Channel is saved in pairing mode.');
    });
    it('keeps the dialog open and offers a reload after a stale write', async () => {
        const onSave = vi
            .fn()
            .mockRejectedValue(new Error('Channel settings changed; reload before trying again.'));
        await renderDialog({ instance: INSTANCE, onSave });
        const save = Array.from(document.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Save');
        await act(async () => {
            save?.click();
        });
        expect(document.body.textContent).toContain('Channel settings changed; reload before trying again.');
    ...
    it('keeps the dialog open when reloading the latest configuration fails', async () => {
        ...
        const reload = Array.from(document.querySelectorAll('button')).find((button) => button.textContent?.trim() === 'Reload latest');
        await act(async () => {
            reload?.click();
        });
        expect(document.body.textContent).toContain('Reload is temporarily unavailable.');
        expect(document.querySelector('[role="dialog"]')).not.toBeNull();
    });
    ...
    it('shows the out-of-range message for a number at the exclusive minimum', async () => {
        ...
        expect(document.body.textContent).toContain('Enter a number greater than 0.');
    });
    it('shows pairing affordance from a descriptor-driven groupPolicy draft', async () => {
        const groupPolicyInstance = {
            ...INSTANCE,
            config: {
                ...INSTANCE.config,
                senderPolicy: 'open',
        ...
        const option = Array.from(document.querySelectorAll('[role="option"]')).find((item) => item.textContent?.trim() === 'Pairing');
...
//# sourceMappingURL=ChannelEditorDialog.test.js.map