...
import { buildChannelUpsertRequest, createChannelEditorDraft, validateChannelEditorDraft, } from './channel-editor-state';
const DINGTALK = {
    type: 'dingtalk',
...
describe('Channel editor state', () => {
    ...
    it('preserves hidden public settings and stored secrets when editing', () => {
        ...
        expect(buildChannelUpsertRequest(DINGTALK, draft, 'revision-2', instance)).toEqual({
    ...
    it('supports explicitly clearing a stored secret', () => {
        ...
        expect(buildChannelUpsertRequest(DINGTALK, draft, 'revision-3', instance)
            .secrets).toEqual({
    ...
    it('does not change whitespace in a replacement secret', () => {
        ...
        expect(buildChannelUpsertRequest(DINGTALK, draft, 'revision-4').secrets).toEqual({
            clientSecret: {
                operation: 'replace',
                value: '  exact-secret  ',
            },
    ...
    it('does not clear a required secret from a blank replacement', () => {
        ...
        expect(buildChannelUpsertRequest(DINGTALK, draft, 'revision-5').secrets).toEqual({
    ...
    it('treats a whitespace-only number draft as empty instead of invalid', () => {
        ...
        expect(validateChannelEditorDraft(descriptor, draft, [])).toEqual({});
        expect(buildChannelUpsertRequest(descriptor, draft, 'revision-1').config).toEqual({ type: 'example', senderPolicy: 'pairing' });
    });
...
describe('Descriptor-driven senderPolicy', () => {
    ...
    it('clears an existing optional secret from a blank replacement', () => {
        ...
        const draft = createChannelEditorDraft(GITHUB, instance);
        draft.secrets.token = { operation: 'replace', value: '  ' };
        expect(buildChannelUpsertRequest(GITHUB, draft, 'rev-2', instance).secrets).toEqual({ token: { operation: 'clear' } });
    });
    it('replaces an existing optional secret with a non-blank value', () => {
        ...
        expect(buildChannelUpsertRequest(GITHUB, draft, 'rev-6', instance).secrets).toEqual({ token: { operation: 'replace', value: 'ghp_new' } });
    });
...
//# sourceMappingURL=channel-editor-state.test.js.map