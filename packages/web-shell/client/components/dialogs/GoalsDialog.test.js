...
async function mount(goals, opts = {}) {
    ...
    await act(async () => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(GoalsDialog, { onCreateGoal: opts.onCreateGoal ?? vi.fn(), onOpenSession: opts.onOpenSession ?? vi.fn(), onError: opts.onError ?? vi.fn() }) }));
    });
    await flush();
}
const baseGoal = (over = {}) => ({
...
describe('GoalsDialog', () => {
    ...
    it('warns that the list is incomplete when sessions could not be probed', async () => {
        ...
        expect(document.querySelector('[data-testid="goals-dropped"]')).not.toBeNull();
        expect(document.body.textContent).toContain('2 sessions could not be reached');
    });
    ...
    it("disables a goal's clear button while its clear is in flight", async () => {
        ...
        let release;
        actions.clearGoal.mockImplementation(() => new Promise((resolve) => {
            release = () => resolve({ cleared: true });
        }));
        const clearButton = () => document.querySelector('button[aria-label="Clear goal"]');
    ...
    it('never lets a slow /goals poll overlap itself', async () => {
        ...
        vi.useFakeTimers();
        let release;
        actions.listGoals.mockImplementation(() => new Promise((resolve) => {
        ...
        root = createRoot(container);
        await act(async () => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(GoalsDialog, { onCreateGoal: vi.fn(), onOpenSession: vi.fn(), onError: vi.fn() }) }));
        });
    ...
    it('routes a creation failure to a toast when the page closed mid-flight', async () => {
        const onError = vi.fn();
        let reject;
        const onCreateGoal = vi.fn(() => new Promise((_resolve, rj) => {
            reject = rj;
        }));
    ...
    it('renders the load error and keeps the list usable', async () => {
        ...
        expect(document.querySelector('[role="alert"]')?.textContent).toContain('daemon unreachable');
    });
...
//# sourceMappingURL=GoalsDialog.test.js.map