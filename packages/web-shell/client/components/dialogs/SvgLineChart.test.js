...
describe('SvgLineChart', () => {
    it('renders a polyline path, current value, peak, and end dot for a populated series', () => {
        const el = render(_jsx(SvgLineChart, { series: [
    ...
    it('renders no line and no peak for an empty series', () => {
        const el = render(_jsx(SvgLineChart, { series: [{ label: 'x', values: [], color: 'var(--a)' }] }));
    ...
    it('places a single-point series mid-width so its dot still renders', () => {
        const el = render(_jsx(SvgLineChart, { series: [{ label: 'x', values: [42], color: 'var(--a)' }] }));
...
//# sourceMappingURL=SvgLineChart.test.js.map