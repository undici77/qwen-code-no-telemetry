...
describe('TokenHeatmap', () => {
    it('renders a ~12-month grid of day cells', () => {
        render(_jsx(TokenHeatmap, { heatmap: { [todayKey()]: { tokens: 1_000, cacheReadRate: 0 } }, days: 365 }));
    ...
    it('shows a custom tooltip (ISO date + tokens + cache) on hover', () => {
        const key = todayKey();
        render(_jsx(TokenHeatmap, { heatmap: { [key]: { tokens: 1_395_800_000, cacheReadRate: 0.96 } }, days: 365 }));
    ...
    it('localizes the month labels to the app language (zh-CN)', () => {
        render(_jsx(TokenHeatmap, { heatmap: { [todayKey()]: { tokens: 1_000, cacheReadRate: 0 } }, days: 365 }), 'zh-CN');
