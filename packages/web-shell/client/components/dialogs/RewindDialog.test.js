...
let container = null;
let root = null;
async function mount(rewind) {
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);
    await act(async () => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(RewindDialog, { blocks: blocks, loadSnapshots: () => Promise.resolve({ snapshots }), rewind: rewind, onError: vi.fn(), onClose: vi.fn() }) }));
    });
...
describe('RewindDialog keyboard', () => {
    function rewindButton() {
        return Array.from(container.querySelectorAll('button')).find((el) => /rewind/i.test(el.textContent || ''));
    }
    ...
    it('disables keyboard navigation while a rewind is in flight', async () => {
        ...
        const rewind = vi.fn().mockReturnValue(new Promise(() => { }));
        await mount(rewind);
...
//# sourceMappingURL=RewindDialog.test.js.map