...
import { getCodeHighlighter, isTooLargeToHighlight, } from '../messages/codeHighlighter';
...
function renderContent(row) {
    if (!row.tokens || row.tokens.length === 0)
        return row.text;
    return row.tokens.map((token, index) => (_jsx("span", { style: token.color ? { color: token.color } : undefined, children: token.content }, index)));
}
function DiffHunks({ hunks, path }) {
    ...
    if (failed) {
        return (_jsx("div", { className: styles.filePlaceholder, children: t('gitDiff.fileError') }));
    }
    ...
    if (rows === null) {
        return _jsx("div", { className: styles.filePlaceholder, children: t('gitDiff.loading') });
    }
    return (_jsx("div", { className: styles.diffLines, children: (rows ?? []).map((row, index) => (_jsxs("div", { className: `${styles.diffLine} ${ROW_CLASS[row.type]}`, children: [_jsx("span", { className: styles.diffOldNo, children: row.oldNo ?? '' }), _jsx("span", { className: styles.diffNewNo, children: row.newNo ?? '' }), _jsx("span", { className: styles.diffMarker, children: row.type === 'add'
                        ? '+'
                        : row.type === 'del'
                            ? '-'
                            : row.type === 'meta'
                                ? ''
                                : ' ' }), _jsx("code", { className: styles.diffContent, children: renderContent(row) })] }, index))) }));
}
function DiffFileRow({ workspaceCwd, gitCwd, file, }) {
    ...
    return (_jsxs("div", { className: styles.file, children: [_jsxs("button", { type: "button", className: styles.fileHeader, onClick: toggle, "aria-expanded": open, "aria-label": t(open ? 'gitDiff.collapse' : 'gitDiff.expand', {
                    path: file.oldPath
                        ? `${sanitizeControlChars(file.oldPath)} → ${displayName}`
                        : displayName,
                }), children: [_jsx("span", { className: styles.fileStats, children: file.isBinary ? (_jsx("span", { className: styles.fileBinary, children: t('gitDiff.binary') })) : (_jsxs(_Fragment, { children: [_jsxs("span", { className: styles.statAdd, children: ["+", file.added ?? 0] }), _jsxs("span", { className: styles.statDel, children: ["-", file.removed ?? 0] })] })) }), _jsxs("span", { className: styles.filePath, title: displayName, children: [file.oldPath ? (_jsxs(_Fragment, { children: [_jsx("span", { className: styles.fileOldPath, children: sanitizeControlChars(file.oldPath) }), ' → '] })) : null, displayName] }), file.isUntracked && (_jsx("span", { className: styles.fileTag, children: t('gitDiff.untracked') })), file.isDeleted && (_jsx("span", { className: styles.fileTag, children: t('gitDiff.deleted') }))] }), open && (_jsx("div", { className: styles.fileBody, children: file.isBinary ? (_jsx("div", { className: styles.filePlaceholder, children: t('gitDiff.binary') })) : loading ? (_jsx("div", { className: styles.filePlaceholder, children: t('gitDiff.loading') })) : error ? (_jsx("div", { className: styles.filePlaceholder, children: t('gitDiff.fileError') })) : hunks && hunks.length > 0 ? (_jsxs(_Fragment, { children: [_jsx(DiffHunks, { hunks: hunks, path: file.path }), truncated && (_jsx("div", { className: styles.filePlaceholder, role: "note", children: t('gitDiff.truncated') }))] })) : (_jsx("div", { className: styles.filePlaceholder, children: t('gitDiff.noDiff') })) }))] }));
}
export function GitDiffContent({ workspaceCwd, gitCwd, onSubtitleChange, }) {
    ...
    const subtitle = diff && diff.available
        ? t('gitDiff.summary', {
    ...
    let body;
    if (loading) {
        body = _jsx("div", { className: styles.placeholder, children: t('gitDiff.loading') });
    }
    else if (error) {
        body = _jsx("div", { className: styles.placeholder, children: t('gitDiff.error') });
    }
    else if (!diff || !diff.available) {
        body = _jsx("div", { className: styles.placeholder, children: t('gitDiff.unavailable') });
    }
    else if (diff.files.length === 0) {
        body = _jsx("div", { className: styles.placeholder, children: t('gitDiff.empty') });
    }
    else {
        body = (_jsxs("div", { className: styles.fileList, children: [diff.files.map((file) => (_jsx(DiffFileRow
                ...
                , { workspaceCwd: workspaceCwd, gitCwd: gitCwd, file: file }, `${workspaceCwd}:${gitCwd ?? ''}:${file.path}`))), diff.hiddenCount > 0 && (_jsx("div", { className: styles.hiddenNote, children: t('gitDiff.hidden', { count: diff.hiddenCount }) }))] }));
    }
    return _jsx("div", { className: styles.content, children: body });
}
export function GitDiffDialog({ workspaceCwd, onClose, }) {
    const { t } = useI18n();
    return (_jsx(DialogShell, { title: t('gitDiff.title'), size: "xl", allowFullscreen: true, onClose: onClose, children: _jsx(GitDiffContent, { workspaceCwd: workspaceCwd }) }));
}
