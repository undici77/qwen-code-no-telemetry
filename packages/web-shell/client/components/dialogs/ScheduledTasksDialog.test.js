...
let portalRoot = null;
let root = null;
async function mount(tasks, opts = {}) {
    ...
    await act(async () => {
        root.render(_jsx(WebShellPortalRootContext.Provider, { value: portalRoot, children: _jsx(I18nProvider, { language: "en", children: _jsx(ScheduledTasksDialog, { onRunPrompt: opts.onRunPrompt ?? vi.fn(), onCreateViaChat: vi.fn(), onOpenSession: opts.onOpenSession, onError: opts.onError ?? vi.fn() }) }) }));
    });
    await flush();
}
...
function findFrequencySelect() {
    return Array.from(document.querySelectorAll('select')).find((s) => !!s.querySelector('option[value="weekdays"]'));
}
function deferred() {
    let resolve;
...
describe('ScheduledTasksDialog editing', () => {
    it('keeps the prompt placeholder outside the editable textbox', async () => {
        ...
        expect(prompt?.getAttribute('aria-placeholder')).toBe('What should this task do?');
    ...
    it('renders saved prompt references as inline tags when editing', async () => {
        ...
        click(document.querySelector('[aria-label="Edit"]'));
        const tags = Array.from(document.querySelectorAll('[data-prompt-tag-serialized]'));
        ...
        click(findButton('Save'));
        await flush();
        expect(actions.updateScheduledTask).toHaveBeenCalledWith('t1', expect.objectContaining({
            prompt: promptText,
        }), undefined);
    });
    it('removes a prompt reference tag and its adjacent spacer', async () => {
        ...
        expect(removeButtons[0]?.getAttribute('aria-label')).toBe('Remove clickhouse');
    ...
    it('copies and cuts prompt references as serialized tokens', async () => {
        ...
        expect(copyClipboard.setData).toHaveBeenCalledWith('text/plain', promptText);
    ...
    it('does not render filesystem paths as skill tags', async () => {
        ...
        expect(document.querySelector('[role="textbox"]')?.textContent).toContain('/var/log');
    });
    it('does not hang on escaped slash-reference near-misses', async () => {
        const promptText = `check /${'\\a'.repeat(30)}/ after`;
        await mount([baseTask({ prompt: promptText })]);
        click(document.querySelector('[aria-label="Edit"]'));
        expect(document.querySelector('[role="textbox"]')?.textContent).toContain(promptText);
    });
    it('inserts extension, skill, and MCP references into the created prompt', async () => {
        ...
        await flush();
        const extensionTag = document.querySelector('[data-prompt-tag-serialized="@ext:alibabacloud-compute-suite"]');
        expect(extensionTag?.textContent).toBe('Alibaba Cloud');
        expect(extensionTag?.dataset.promptTagSerialized).toBe('@ext:alibabacloud-compute-suite');
        click(findButton('Create'));
        await flush();
        expect(actions.createScheduledTask).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('drops plain text only into the prompt editor', async () => {
        ...
        Object.defineProperty(drop, 'dataTransfer', {
            value: {
                getData: (type) => type === 'text/plain' ? 'plain text' : '<b>html</b>',
            },
        });
        act(() => {
    ...
    it('does not preserve empty contenteditable leftovers before an inserted reference', async () => {
        ...
        click(findButtonContaining('alibabacloud-compute-suite'));
        await flush();
        const extensionTag = prompt.querySelector('[data-prompt-tag-serialized="@ext:alibabacloud-compute-suite"]');
    ...
    it('an unrepresentable cron lands in the custom field, losslessly', async () => {
        ...
        expect(actions.updateScheduledTask).toHaveBeenCalledWith('t1', expect.objectContaining({ cron: '0 9 * * 1,3,5' }), undefined);
    });
});
...
describe('ScheduledTasksDialog run now', () => {
    ...
    it('re-checks server state and does NOT enqueue a task disabled after load', async () => {
        ...
        const onRunPrompt = vi.fn();
        await mount([baseTask({ enabled: true, sessionId: 'sess-9', prompt: 'do it' })], {
            onRunPrompt,
        });
    ...
    it('consumes a bound ONE-SHOT before enqueuing (record → enqueue)', async () => {
        ...
        await mount([baseTask({ recurring: false, enabled: true, sessionId: 'sess-9' })], { onRunPrompt });
    ...
    it('surfaces a consumed-but-failed error when a ONE-SHOT delivery fails', async () => {
        ...
        expect(onRunPrompt).toHaveBeenCalledWith('do it', 'sess-9');
        expect(onError).toHaveBeenCalledWith(expect.any(Error), expect.stringContaining('never ran'));
    });
    it('records a RECURRING task after enqueuing (enqueue → record)', async () => {
        ...
        await mount([baseTask({ recurring: true, enabled: true, sessionId: 'sess-9' })], { onRunPrompt });
        actions.runScheduledTask.mockImplementation(async () => {
            order.push('record');
            return baseTask({});
        });
    ...
    it('serializes run now: a second click while one is pending is ignored', async () => {
        ...
        let resolveRun;
        const onRunPrompt = vi.fn().mockImplementation(() => new Promise((resolve) => {
            resolveRun = resolve;
        }));
        await mount([baseTask({ sessionId: 'sess-9', prompt: 'do it' })], {
            onRunPrompt,
...
describe('ScheduledTasksDialog next-run countdown', () => {
    it('renders a countdown pill for a task with a next run', async () => {
        ...
        const pill = document.querySelector('[data-testid="scheduled-task-next-run"]');
    ...
    it('omits the countdown when there is no next run (disabled task)', async () => {
        ...
        expect(document.querySelector('[data-testid="scheduled-task-next-run"]')).toBeNull();
    });
});
describe('ScheduledTasksDialog multi-workspace', () => {
    ...
    async function mountMulti(byWorkspace, ws = WORKSPACES, lockedWorkspace) {
        actions.listScheduledTasks.mockImplementation(async (wsId) => wsId === undefined
            ? (byWorkspace['primary'] ?? [])
            : (byWorkspace[wsId] ?? []));
        actions.createScheduledTask.mockResolvedValue(baseTask({}));
        ...
        root = createRoot(container);
        await act(async () => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ScheduledTasksDialog, { onRunPrompt: vi.fn(), onCreateViaChat: vi.fn(), workspaces: ws, lockedWorkspace: lockedWorkspace, onError: vi.fn() }) }));
        });
        await flush();
    }
    const findWorkspaceSelect = () => Array.from(document.querySelectorAll('select')).find((s) => s.querySelector('option[value="id-other"]'));
    ...
    it('creates a task in the workspace chosen in the picker', async () => {
        ...
        expect(Array.from(wsSelect.querySelectorAll('option')).map((o) => o.textContent)).toEqual(['Main Workspace', 'Payments API']);
        ...
        click(findButton('Create'));
        await flush();
        expect(actions.createScheduledTask).toHaveBeenCalledWith(expect.objectContaining({ prompt: 'do secondary work' }), 'id-other');
    });
    it('lists and creates tasks in a locked secondary workspace', async () => {
        ...
        click(findButton('Create'));
        await flush();
        expect(actions.createScheduledTask).toHaveBeenCalledWith(expect.objectContaining({ prompt: 'do locked work' }), 'id-other');
    });
    it('defaults a new task to the primary workspace', async () => {
        ...
        expect(actions.createScheduledTask).toHaveBeenCalledWith(expect.objectContaining({ prompt: 'primary work' }), undefined);
    });
    it('threads the task’s own workspace through toggle and delete', async () => {
        ...
        await flush();
        expect(actions.updateScheduledTask).toHaveBeenCalledWith('s1', { enabled: false }, 'id-other');
    ...
    it('keeps an untrusted primary in the aggregate and picker (trust-free route)', async () => {
        ...
        click(findButton('New scheduled task'));
        const wsSelect = findWorkspaceSelect();
        const values = Array.from(wsSelect.querySelectorAll('option')).map((o) => o.value);
...
//# sourceMappingURL=ScheduledTasksDialog.test.js.map