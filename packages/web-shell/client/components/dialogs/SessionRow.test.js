...
describe('SessionRow', () => {
    it('renders an option with the title and metadata', () => {
        mount(_jsx(SessionRow, { session: session, active: false, current: false, onClick: vi.fn(), onActivate: vi.fn() }));
    ...
    it('defaults aria-selected to `current` (not the roving highlight), explicit value wins', () => {
        ...
        mount(_jsx(SessionRow, { session: session, active: true, current: false, onClick: vi.fn(), onActivate: vi.fn() }));
        expect(row().getAttribute('aria-selected')).toBe('false');
        expect(row().getAttribute('aria-current')).toBeNull();
        act(() => root?.unmount());
        container?.remove();
        mount(_jsx(SessionRow, { session: session, active: false, current: true, onClick: vi.fn(), onActivate: vi.fn() }));
        expect(row().getAttribute('aria-selected')).toBe('true');
        expect(row().getAttribute('aria-current')).toBe('true');
        act(() => root?.unmount());
        container?.remove();
        // Multi-select: an explicit ariaSelected (checked state) wins over current.
        mount(_jsx(SessionRow, { session: session, active: true, ariaSelected: false, current: true, onClick: vi.fn(), onActivate: vi.fn() }));
        expect(row().getAttribute('aria-selected')).toBe('false');
    });
    it('marks the current session and exposes the label as a tooltip', () => {
        mount(_jsx(SessionRow, { session: session, active: false, current: true, currentLabel: "current", onClick: vi.fn(), onActivate: vi.fn() }));
        expect(row().className).toContain(dp('dialog-current'));
        expect(row().getAttribute('title')).toBe('current');
    });
    it('renders leading and trailing slots', () => {
        mount(_jsx(SessionRow, { session: session, active: false, current: false, leading: _jsx("span", { "data-testid": "lead", children: "[x]" }), trailing: _jsx("span", { "data-testid": "trail", children: "inactive" }), onClick: vi.fn(), onActivate: vi.fn() }));
    ...
    it('fires onClick and onActivate', () => {
        ...
        mount(_jsx(SessionRow, { session: session, active: false, current: false, onClick: onClick, onActivate: onActivate }));
        act(() => {
...
//# sourceMappingURL=SessionRow.test.js.map