 ...
 */
export function parseHhmm(time) {
...
export function buildCron(state) {
    ...
    if (state.frequency === 'minutes') {
        ...
        if (!Number.isFinite(n) || n < 1 || n > 30 || 60 % n !== 0)
            return null;
        return `*/${n} * * * *`;
    }
 ...
 */
export function parseCronToBuilder(cron) {
    const raw = cron.trim();
    const custom = {
        ...DEFAULT_BUILDER,
        frequency: 'custom',
 ...
 */
export function formatCountdown(msRemaining, t) {
    if (msRemaining <= 0)
        return t('scheduledTasks.dueNow');
    const totalSec = Math.floor(msRemaining / 1000);
    const d = Math.floor(totalSec / 86_400);
    const h = Math.floor((totalSec % 86_400) / 3_600);
    const m = Math.floor((totalSec % 3_600) / 60);
    const s = totalSec % 60;
    const unit = (key, n) => `${n}${t(`scheduledTasks.dur.${key}`)}`;
    let parts;
    if (d > 0)
        parts = h > 0 ? [unit('d', d), unit('h', h)] : [unit('d', d)];
    else if (h > 0)
        parts = m > 0 ? [unit('h', h), unit('m', m)] : [unit('h', h)];
    else if (m > 0)
        parts = s > 0 ? [unit('m', m), unit('s', s)] : [unit('m', m)];
    else
        parts = [unit('s', s)];
    return parts.join(' ');
}
...
export function describeLastRun(task, t) {
...
//# sourceMappingURL=scheduledTasksSchedule.js.map