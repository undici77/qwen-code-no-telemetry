...
export function AddWorkspaceDialog({ onClose, onAdd, displayNameEnabled = false, onSuggest, onPick, persistenceSupported = true, }) {
    const { t } = useI18n();
    ...
    useEffect(() => {
        const handler = (event) => {
            if (event.key !== 'Escape' || !listOpenRef.current)
                return;
    ...
    const handleSubmit = useCallback(async (e) => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : t('sidebar.addWorkspaceError'));
        }
        finally {
            setSubmitting(false);
        }
    ...
    return (_jsx(DialogShell, { title: t('sidebar.addWorkspaceTitle'), size: "md", onClose: onClose, children: _jsxs("form", { className: "flex flex-col gap-6", onSubmit: handleSubmit, children: [_jsxs(FieldGroup, { children: [_jsxs(Field, { "data-invalid": error ? true : undefined, children: [_jsx(FieldLabel, { htmlFor: "add-workspace-path", children: t('sidebar.addWorkspacePath') }), _jsxs("div", { className: "relative", children: [_jsxs("div", { className: "flex gap-2", children: [_jsx(Input, { ref: inputRef, id: "add-workspace-path", type: "text", placeholder: "/absolute/path/to/project", value: path, onChange: (e) => {
                                                        setPath(e.target.value);
                                                        if (error)
                                                            setError(null);
                                                    }, onKeyDown: handleInputKeyDown, onFocus: cancelBlurDismiss, onBlur: () => {
                                                    ...
                                                    }, disabled: submitting || browsing, autoCapitalize: "off", autoCorrect: "off", autoComplete: "off", spellCheck: false, role: "combobox", "aria-expanded": showList, "aria-controls": showList ? LISTBOX_ID : undefined, "aria-activedescendant": showList && highlight >= 0
                                                        ? `${LISTBOX_ID}-${highlight}`
                                                        : undefined, "aria-describedby": error ? `${ERROR_ID} ${HINT_ID}` : HINT_ID, "aria-invalid": error ? true : undefined }), onPick && (_jsxs(Button, { type: "button", variant: "outline", onMouseDown: (event) => event.preventDefault(), onClick: () => void pickDirectory(), disabled: submitting || browsing, children: [_jsx(FolderOpenIcon, { "aria-hidden": "true" }), t('sidebar.addWorkspaceBrowse')] }))] }), showList && (_jsx("ul", { id: LISTBOX_ID, role: "listbox", "aria-label": t('sidebar.addWorkspaceSuggestions'), className: "absolute inset-x-0 top-full z-50 mt-1 max-h-56 overflow-y-auto rounded-md border bg-popover py-1 text-popover-foreground shadow-md", children: suggestions.map((suggestion, index) => (_jsxs("li", { id: `${LISTBOX_ID}-${index}`, role: "option", "aria-selected": index === highlight, className: `cursor-pointer truncate px-3 py-1.5 ${index === highlight
                                                    ? 'bg-accent text-accent-foreground'
                                                    : 'hover:bg-accent/50'}`, onMouseDown: (event) => {
                                                    // Keep focus in the input; blur would close the list
                                                    // before click lands.
                                                    event.preventDefault();
                                                    acceptSuggestion(suggestion);
                                                }, onMouseEnter: () => setHighlight(index), children: [suggestion.name, _jsx("span", { className: "text-muted-foreground", children: hostSep })] }, suggestion.path))) }))] }), _jsx(FieldDescription, { id: HINT_ID, children: t('sidebar.addWorkspaceHint') }), error && _jsx(FieldError, { id: ERROR_ID, children: error })] }), displayNameEnabled && (_jsxs(Field, { children: [_jsx(FieldLabel, { htmlFor: "add-workspace-display-name", children: t('sidebar.addWorkspaceDisplayName') }), _jsx(Input, { id: "add-workspace-display-name", type: "text", value: displayName, onChange: (event) => setDisplayName(event.target.value), disabled: submitting, maxLength: 256, autoComplete: "off", "aria-describedby": DISPLAY_NAME_HINT_ID }), _jsx(FieldDescription, { id: DISPLAY_NAME_HINT_ID, children: t('sidebar.addWorkspaceDisplayNameHint') })] })), persistenceSupported && (_jsxs(Field, { orientation: "horizontal", children: [_jsxs(FieldContent, { children: [_jsx(FieldLabel, { htmlFor: "add-workspace-persist", children: t('sidebar.addWorkspacePersist') }), _jsx(FieldDescription, { children: t('sidebar.addWorkspacePersistHint') })] }), _jsx(Switch, { id: "add-workspace-persist", checked: persist, onCheckedChange: setPersist, disabled: submitting })] }))] }), _jsxs("div", { className: "flex justify-end gap-2", children: [_jsx(Button, { type: "button", variant: "outline", onClick: onClose, disabled: submitting, children: t('sidebar.addWorkspaceCancel') }), _jsx(Button, { type: "submit", disabled: submitting || !path.trim(), children: submitting
