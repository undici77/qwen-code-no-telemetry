...
function mount() {
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(ReleaseSessionDialog, { onReleased: vi.fn(), onError: vi.fn(), onClose: vi.fn() }) }));
    });
}
function rows() {
...
function dangerButton() {
    return Array.from(container.querySelectorAll('button')).find((b) => b.className.includes(dp('dialog-danger-button')));
}
...
const isCursor = (el) => el.className.includes(dp('selected'));
const isConfirmed = (el) => el.className.includes(dp('picker-item-confirmed'));
afterEach(() => {
    act(() => root?.unmount());
    container?.remove();
    root = null;
...
//# sourceMappingURL=ReleaseSessionDialog.test.js.map