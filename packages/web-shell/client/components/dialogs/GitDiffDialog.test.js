...
const { workspaceGitDiff, workspaceGitDiffFile, workspaceClient, shikiState } = vi.hoisted(() => {
...
vi.mock('../messages/codeHighlighter', () => ({
    getCodeHighlighter: vi.fn(() => shikiState.highlighter
...
function mount(workspaceCwd = '/repo', strict = false) {
    ...
    const dialog = (_jsx(I18nProvider, { language: "en", children: _jsx(GitDiffDialog, { workspaceCwd: workspaceCwd, onClose: vi.fn() }) }));
    act(() => {
        root.render(strict ? _jsx(StrictMode, { children: dialog }) : dialog);
    });
}
...
describe('GitDiffDialog', () => {
    ...
    it('loads and renders a file diff when expanded', async () => {
        ...
        mount();
        await flush();
        const header = document.body.querySelector('button[aria-expanded="false"]');
        expect(header).not.toBeNull();
        await act(async () => {
            header.click();
        });
        await flush();
        expect(workspaceGitDiffFile).toHaveBeenCalledWith('src/a.ts', undefined, undefined);
    ...
    it('forwards the pre-rename oldPath when expanding a renamed file', async () => {
        ...
        expect(workspaceGitDiffFile).toHaveBeenCalledWith('src/new.ts', 'src/old.ts', undefined);
    });
    it('shows a placeholder when git is unavailable', async () => {
        workspaceGitDiff.mockResolvedValue(diffPayload({ available: false, files: [] }));
        mount();
        await flush();
    ...
    it('renders Shiki tokens per side when highlighting succeeds', async () => {
        ...
        await flush();
        const colored = Array.from(document.body.querySelectorAll('span[style]')).filter((el) => el.style.color !== '');
    ...
    it('forwards gitCwd to the diff list and file SDK calls', async () => {
        ...
        root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(GitDiffContent, { workspaceCwd: "/repo", gitCwd: "/worktrees/wt" }) }));
        });
        await flush();
        ...
        await flush();
        expect(workspaceGitDiffFile).toHaveBeenCalledWith('src/a.ts', undefined, '/worktrees/wt');
    });
});
//# sourceMappingURL=GitDiffDialog.test.js.map