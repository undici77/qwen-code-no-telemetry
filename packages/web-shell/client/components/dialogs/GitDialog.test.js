...
const { workspaceGitDiff, workspaceGitLog, workspaceGitHubPullRequests, workspaceGitCommit, workspaceGitPush, workspaceGitBranches, workspaceGitHubDefaultBranch, workspaceGit, workspaceGitHubCreatePullRequest, workspaceClient, mockState, } = vi.hoisted(() => {
    ...
    const mockState = { capabilities: undefined };
    return {
        workspaceGitDiff,
        workspaceGitLog,
        workspaceGitHubPullRequests,
...
function mount(initialView = 'diff', gitCwd) {
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(GitDialog, { workspaceCwd: "/repo", gitCwd: gitCwd, initialView: initialView, onClose: vi.fn() }) }));
    });
}
function rerender(initialView = 'diff') {
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(GitDialog, { workspaceCwd: "/repo", initialView: initialView, onClose: vi.fn() }) }));
    });
}
afterEach(() => {
    act(() => root.unmount());
...
describe('GitDialog', () => {
    it('switches views inside one dialog with complete tab semantics', async () => {
        ...
        await flush();
        expect(document.body.querySelectorAll('[data-web-shell-dialog]')).toHaveLength(1);
    ...
    it('passes gitCwd to commit and push SDK calls in the commit view', async () => {
        ...
        root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(GitDialog, { workspaceCwd: "/repo", gitCwd: "/worktrees/wt", initialView: "commit", onClose: vi.fn() }) }));
        });
        await flush();
        const textarea = document.body.querySelector('[data-web-shell-dialog] textarea');
        ...
        await flush();
        // Click "Commit & Push".
        const buttons = document.body.querySelectorAll('[data-web-shell-dialog] button');
        const commitPushBtn = Array.from(buttons).find((b) => b.textContent?.includes('Commit and Push'));
        expect(commitPushBtn).toBeTruthy();
        await act(async () => {
            commitPushBtn.click();
        });
        await flush();
        expect(workspaceGitCommit).toHaveBeenCalledWith('fix: test commit', { all: true }, '/worktrees/wt');
        expect(workspaceGitPush).toHaveBeenCalledWith({ setUpstream: true }, '/worktrees/wt');
    });
    it('clamps to the diff view when the capability is withdrawn mid-session', async () => {
        ...
        expect(document
            ...
            ?.getAttribute('aria-labelledby')).toBe('git-dialog-tab-diff');
    });
    it('lets arrow keys leave the commit tab back to the regular tabs', async () => {
        ...
        root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(GitDialog, { workspaceCwd: "/repo", initialView: "commit", onClose: vi.fn() }) }));
        });
        await flush();
        ...
        // being swallowed.
        await act(async () => {
            commitTab?.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowLeft', bubbles: true }));
        });
        await flush();
        expect(document
    ...
    it('offers a retry when the PR base-branch list fails to load', async () => {
        ...
        const createPrBtn = Array.from(document.body.querySelectorAll('[data-web-shell-dialog] button')).find((b) => b.textContent?.includes('Create Pull Request'));
        ...
        // permanent "Loading…".
        const branchTrigger = document.body.querySelector('button[aria-label="Base branch"]');
    ...
    it('commits without pushing when only the Commit button is clicked', async () => {
        ...
        await flush();
        const buttons = document.body.querySelectorAll('[data-web-shell-dialog] button');
        const commitBtn = Array.from(buttons).find((b) => b.textContent?.includes('Commit') &&
        ...
        await flush();
        expect(workspaceGitCommit).toHaveBeenCalledWith('fix: commit only', { all: true }, undefined);
    ...
    it('creates a pull request via the SDK', async () => {
        ...
        const titleInput = document.body.querySelector('[data-web-shell-dialog] input');
        ...
        await flush();
        // Submit the PR.
        const submitBtn = Array.from(document.body.querySelectorAll('[data-web-shell-dialog] button')).find((b) => b.textContent?.includes('Create') &&
...
//# sourceMappingURL=GitDialog.test.js.map