...
const input = () => document.querySelector('#add-workspace-path');
const displayNameInput = () => document.querySelector('#add-workspace-display-name');
const alert = () => document.querySelector('[role="alert"]');
const submitButton = () => Array.from(document.querySelectorAll('button')).find((b) => b.getAttribute('type') === 'submit');
const browseButton = () => Array.from(document.querySelectorAll('button')).find((button) => button.textContent === 'Browse…');
...
function typeDisplayName(value) {
...
describe('AddWorkspaceDialog', () => {
    it('focuses the path input when opened', () => {
        mount(_jsx(AddWorkspaceDialog, { onClose: vi.fn(), onAdd: vi.fn() }));
    ...
    it('rejects a non-absolute path with an accessible error and does not call onAdd', () => {
        ...
        mount(_jsx(AddWorkspaceDialog, { onClose: vi.fn(), onAdd: onAdd }));
        type('relative/path');
        submit();
        const err = alert();
        expect(err?.textContent).toBe('Path must be absolute');
        expect(input().getAttribute('aria-invalid')).toBe('true');
        expect(input().getAttribute('aria-describedby')).toBe('add-workspace-error add-workspace-hint');
    ...
    it('submits a trimmed absolute path and closes on success', async () => {
        const onAdd = vi.fn().mockResolvedValue(undefined);
        const onClose = vi.fn();
        mount(_jsx(AddWorkspaceDialog, { onClose: onClose, onAdd: onAdd }));
        type('  /abs/project  ');
        submit();
    ...
    it('submits an optional trimmed display name', async () => {
        const onAdd = vi.fn().mockResolvedValue(undefined);
        const onClose = vi.fn();
        mount(_jsx(AddWorkspaceDialog, { onClose: onClose, onAdd: onAdd, displayNameEnabled: true }));
        expect(displayNameInput().maxLength).toBe(256);
        expect(displayNameInput().getAttribute('aria-describedby')).toBe('add-workspace-display-name-hint');
        type('/abs/project');
    ...
    it('submits a display name with persist=false', async () => {
        ...
        const sw = document.querySelector('#add-workspace-persist');
        act(() => {
        ...
        expect(onAdd).toHaveBeenCalledWith('/abs/project', false, 'Local workspace');
        expect(onClose).toHaveBeenCalledTimes(1);
    });
    it('hides persistence and always submits false when unsupported', async () => {
        const onAdd = vi.fn().mockResolvedValue(undefined);
        mount(_jsx(AddWorkspaceDialog, { onClose: vi.fn(), onAdd: onAdd, persistenceSupported: false }));
    ...
    describe('path autocomplete', () => {
        ...
        it('debounces a suggestion fetch for an absolute prefix and lists directories', async () => {
            ...
            mount(_jsx(AddWorkspaceDialog, { onClose: vi.fn(), onAdd: vi.fn(), onSuggest: onSuggest }));
            type('/home/me/co');
        ...
        it('opens the system picker and fills the selected absolute path', async () => {
            ...
            const onSuggest = vi.fn().mockResolvedValue(SUGGESTIONS);
            mount(_jsx(AddWorkspaceDialog, { onClose: vi.fn(), onAdd: vi.fn(), onPick: onPick, onSuggest: onSuggest }));
            await act(async () => {
                browseButton().click();
                await Promise.resolve();
            });
        ...
        it('keeps suggestions closed when a lookup finishes after blur', async () => {
            ...
            const onSuggest = vi.fn(() => new Promise((resolve) => {
        ...
        it('opens suggestions for a focused input in a shadow-DOM portal root', async () => {
            ...
            act(() => {
                root.render(_jsx(WebShellPortalRootContext.Provider, { value: portalRoot, children: _jsx(I18nProvider, { language: "en", children: _jsx(AddWorkspaceDialog, { onClose: vi.fn(), onAdd: vi.fn(), onSuggest: onSuggest }) }) }));
            });
            const shadowInput = shadowRoot.querySelector('#add-workspace-path');
        ...
        it('leaves the path unchanged when the system picker is cancelled', async () => {
            ...
            mount(_jsx(AddWorkspaceDialog, { onClose: vi.fn(), onAdd: vi.fn(), onPick: onPick }));
            await act(async () => {
                browseButton().click();
                await Promise.resolve();
            });
        ...
        it('opens suggestions on the first edit after a cancelled picker', async () => {
            let resolvePick;
            const onPick = vi.fn(() => new Promise((resolve) => {
                resolvePick = resolve;
            }));
            const onSuggest = vi.fn().mockResolvedValue(SUGGESTIONS);
            mount(_jsx(AddWorkspaceDialog, { onClose: vi.fn(), onAdd: vi.fn(), onPick: onPick, onSuggest: onSuggest }));
        ...
        it('shows an error when the system picker fails', async () => {
            ...
            expect(alert()?.textContent).toContain('Unable to open the system folder picker');
        ...
        it('accepts the highlighted entry with Enter without submitting the form', async () => {
            const onSuggest = vi.fn().mockResolvedValue(SUGGESTIONS);
            const onAdd = vi.fn().mockResolvedValue(undefined);
            mount(_jsx(AddWorkspaceDialog, { onClose: vi.fn(), onAdd: onAdd, onSuggest: onSuggest }));
            type('/home/me/co');
            await settle();
            keydown('ArrowDown');
        ...
        it('closes only the list on Escape, keeping the dialog open', async () => {
            const onSuggest = vi.fn().mockResolvedValue(SUGGESTIONS);
            const onClose = vi.fn();
            mount(_jsx(AddWorkspaceDialog, { onClose: onClose, onAdd: vi.fn(), onSuggest: onSuggest }));
            type('/home/me/co');
            await settle();
            expect(listbox()).not.toBeNull();
            keydown('Escape');
...
//# sourceMappingURL=AddWorkspaceDialog.test.js.map