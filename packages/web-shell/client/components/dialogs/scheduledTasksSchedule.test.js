...
import { buildCron, describeCron, describeLastRun, formatCountdown, parseCronToBuilder, parseHhmm, } from './scheduledTasksSchedule';
...
describe('buildCron', () => {
    it('builds each frequency shape', () => {
        expect(buildCron(builder({ frequency: 'daily', time: '09:30' }))).toBe('30 9 * * *');
        expect(buildCron(builder({ frequency: 'weekdays', time: '12:30' }))).toBe('30 12 * * 1-5');
        expect(buildCron(builder({ frequency: 'weekly', time: '10:00', weekday: 1 }))).toBe('0 10 * * 1');
        expect(buildCron(builder({ frequency: 'hourly', time: '00:07' }))).toBe('7 * * * *');
        expect(buildCron(builder({ frequency: 'minutes', minuteInterval: 15 }))).toBe('*/15 * * * *');
        expect(buildCron(builder({ frequency: 'custom', customCron: '0 9 * * 1-5' }))).toBe('0 9 * * 1-5');
    });
    it('returns null for invalid inputs', () => {
        expect(buildCron(builder({ frequency: 'custom', customCron: '   ' }))).toBeNull();
        expect(buildCron(builder({ frequency: 'minutes', minuteInterval: 0 }))).toBeNull();
        expect(buildCron(builder({ frequency: 'minutes', minuteInterval: 60 }))).toBeNull();
        // Non-divisors of 60 are rejected — */45 would not mean "every 45 minutes".
        expect(buildCron(builder({ frequency: 'minutes', minuteInterval: 45 }))).toBeNull();
        expect(buildCron(builder({ frequency: 'daily', time: '25:00' }))).toBeNull();
    });
});
describe('describeCron', () => {
    it('labels the builder-emitted shapes', () => {
        expect(describeCron('0 9 * * *', t)).toBe('scheduledTasks.human.daily(time=09:00)');
        expect(describeCron('30 12 * * 1-5', t)).toBe('scheduledTasks.human.weekdays(time=12:30)');
        expect(describeCron('0 9 * * 1', t)).toBe('scheduledTasks.human.weekly(day=Mon,time=09:00)');
        expect(describeCron('0 9 * * 0', t)).toBe('scheduledTasks.human.weekly(day=Sun,time=09:00)');
        // Cron allows 7 as an alternate notation for Sunday.
        expect(describeCron('0 9 * * 7', t)).toBe('scheduledTasks.human.weekly(day=Sun,time=09:00)');
        expect(describeCron('15 * * * *', t)).toBe('scheduledTasks.human.hourly(min=15)');
        expect(describeCron('*/30 * * * *', t)).toBe('scheduledTasks.human.everyMinutes(n=30)');
    });
...
describe('describeLastRun', () => {
    ...
    it('reads as "never" for a fresh (creation-minute) or null stamp', () => {
        expect(describeLastRun({ createdAt: created, lastFiredAt: null }, t)).toBe('scheduledTasks.never');
        expect(describeLastRun({ createdAt: created, lastFiredAt: createdMinute }, t)).toBe('scheduledTasks.never');
    });
    it('reads as a real run once lastFiredAt advances past the creation minute', () => {
        const label = describeLastRun({ createdAt: created, lastFiredAt: createdMinute + 60_000 }, t);
        expect(label.startsWith('scheduledTasks.lastFired(')).toBe(true);
    });
});
//# sourceMappingURL=scheduledTasksSchedule.test.js.map