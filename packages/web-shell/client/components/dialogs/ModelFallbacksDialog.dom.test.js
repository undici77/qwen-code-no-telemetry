...
import { ModelFallbacksDialog, } from './ModelFallbacksDialog';
...
function renderDialog(overrides = {}) {
    ...
    const container = render(_jsx(I18nProvider, { language: "en", children: _jsx(ModelFallbacksDialog, { ...props }) }));
    return { container, props };
}
function options(container) {
    return Array.from(container.querySelectorAll('button[aria-pressed]'));
}
function clickConfirm(container) {
    const confirm = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.trim() === 'Confirm');
    if (!confirm)
...
describe('ModelFallbacksDialog', () => {
    ...
    it('enforces the max selection by disabling further options', () => {
        ...
        expect(options(container).filter((o) => o.getAttribute('aria-pressed') === 'true').length).toBe(3);
    });
...
//# sourceMappingURL=ModelFallbacksDialog.dom.test.js.map