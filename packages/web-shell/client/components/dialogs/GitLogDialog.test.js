...
const { workspaceGitLog, workspaceGitCommitDetail, workspaceClient } = vi.hoisted(() => {
...
let container;
let root;
function mount(workspaceCwd = '/repo', language = 'en') {
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);
    act(() => {
        root.render(_jsx(I18nProvider, { language: language, children: _jsx(GitLogDialog, { workspaceCwd: workspaceCwd, onClose: vi.fn() }) }));
    });
}
...
function logPayload(entries, hasMore = false, available = true) {
    return { v: 1, workspaceCwd: '/repo', available, entries, hasMore };
}
describe('GitLogDialog', () => {
    it('renders the commit list with author and relative time', async () => {
        workspaceGitLog.mockResolvedValue(logPayload([entry({ subject: 'first change', authorName: 'Ada' })]));
        mount();
        await flush();
    ...
    it('localizes relative time and the copy action', async () => {
        ...
        expect(document.body.querySelector('button[aria-label^="复制提交"]')).toBeTruthy();
    });
    ...
    it('paginates: Load more fetches the next page at the current offset and appends', async () => {
        ...
        mount();
        await flush();
        const loadMore = Array.from(document.body.querySelectorAll('button')).find((b) => b.textContent === 'Load more');
    ...
    it('deduplicates overlapping pages while advancing the server offset', async () => {
        ...
        workspaceGitLog
            ...
            .mockResolvedValueOnce(logPayload([duplicate, entry({ subject: 'older' })], true))
            .mockResolvedValueOnce(logPayload([], false));
        mount();
        await flush();
        const loadMore = () => Array.from(document.body.querySelectorAll('button')).find((button) => button.textContent === 'Load more');
        await act(async () => {
            loadMore().click();
        });
        await flush();
    ...
    it('expands a commit and loads its detail (body + file stats)', async () => {
        ...
        mount();
        await flush();
        const row = document.body.querySelector('button[aria-expanded="false"]');
        await act(async () => {
            row.click();
        });
        await flush();
    ...
    it('shows an error when an expanded commit reports available:false', async () => {
        ...
        // error instead.
        expect(document.body.textContent).toContain('Failed to load commit details');
    });
    ...
    it('forwards gitCwd to the log list, pagination, and commit-detail SDK calls', async () => {
        ...
        root = createRoot(container);
        act(() => {
            root.render(_jsx(I18nProvider, { language: "en", children: _jsx(GitLogContent, { workspaceCwd: "/repo", gitCwd: "/worktrees/wt" }) }));
        });
        await flush();
        ...
        await flush();
        expect(workspaceGitCommitDetail).toHaveBeenCalledWith(e.sha, '/worktrees/wt');
    });
});
//# sourceMappingURL=GitLogDialog.test.js.map