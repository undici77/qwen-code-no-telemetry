...
export function RewindDialog({
  ...
  if (loading) {
    return _jsx('div', {
      ...
      children: t('rewind.loading'),
    });
  }
  if (items.length === 0) {
    return _jsx('div', {
      className: dp('picker-empty'),
      children: t('rewind.empty'),
    });
  }
  return _jsxs('div', {
    ...
    children: [
      _jsx('div', {
        ...
        children: items.map(({ snapshot, promptText }, index) => {
          ...
          return _jsxs(
            'div',
            {
              ...
              className: `${styles.item} ${isCursor ? styles.itemCursor : ''} ${isSelected ? styles.itemSelected : ''} ${isRewinding ? styles.itemDisabled : ''}`,
              onClick: () => {
