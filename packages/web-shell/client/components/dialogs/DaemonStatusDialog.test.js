...
function summaryWithSeries(count) {
    const series = Array.from({ length: count }, (_, i) => makeBucket(1000 + i * 5000));
    return {
        ...summaryReport,
...
function mount(language = 'en') {
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: language, children: _jsx(DaemonStatusDialog, {}) }));
    });
}
...
function topBadgeText() {
    return Array.from(container.querySelectorAll('span'))
        .map((s) => s.textContent?.trim() ?? '')
        .find((label) => label === 'OK' || label === 'Warning' || label === 'Error');
}
...
function openDiagnostics() {
    ...
    if (!btn)
        throw new Error('Diagnostics tab not found');
    act(() => {
...
describe('DaemonStatusDialog', () => {
    it('renders live summary counters with the full-detail rollup badge', () => {
        ...
        expect(text).toContain('2 permission requests are waiting for a client response');
    ...
    it('fetches both summary and full detail; diagnostics live under the Diagnostics tab', () => {
        ...
        const buttonLabels = Array.from(container.querySelectorAll('button')).map((el) => el.textContent);
    ...
    it('renders charts on the Metrics tab from the series and hides Overview', () => {
        ...
        expect(container.querySelectorAll('svg[role="img"]').length).toBeGreaterThanOrEqual(12);
    ...
    it('gives every metrics chart a hover help affordance with self-explaining copy', () => {
        ...
        const helpButtons = Array.from(container.querySelectorAll('button[aria-label]')).filter((b) => (b.getAttribute('aria-label') ?? '').length > 20);
        ...
        expect(container.querySelector('button[aria-label*="Provider-side LLM failures"]')).not.toBeNull();
    });
    ...
    it('moves between tabs with arrow / Home / End keys (roving tabindex)', () => {
        ...
        act(() => {
            tabs[3].dispatchEvent(new KeyboardEvent('keydown', {
                key: 'Home',
                bubbles: true,
                cancelable: true,
            }));
        ...
        expect(tabs[0].getAttribute('aria-controls')).toBe('daemon-tabpanel-overview');
    });
    ...
    it('skips a poll tick while the previous summary reload is still in flight', async () => {
        vi.useFakeTimers();
        let release = () => { };
        summaryReload.mockImplementationOnce(() => new Promise((resolve) => {
    ...
    it('manual refresh reloads both summary and full', () => {
        mount();
        const refreshButton = Array.from(container.querySelectorAll('button')).find((el) => el.textContent === 'Refresh');
    ...
    it('renders the workspace empty-state when no sections are reported', () => {
        ...
        mount();
        openDiagnostics();
        expect(container.textContent ?? '').toContain('No workspace diagnostics reported');
    });
...
//# sourceMappingURL=DaemonStatusDialog.test.js.map