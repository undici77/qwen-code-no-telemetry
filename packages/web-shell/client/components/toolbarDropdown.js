...
export function getToolbarItemVisibility({ availableWidth, items, }) {
    const visibility = Object.fromEntries(items.map((item) => [item.id, item.ready !== false]));
    let usedWidth = items.reduce((total, item) => total + (visibility[item.id] ? item.expansionWidth : 0), 0);
...
export function getToolbarItemVisibilityWithHysteresis({ availableWidth, items, currentVisibility, expansionMargin, }) {
...
export function getToolbarExpansionBudget({ toolbarWidth, leadingWidth, rightWidth, currentExpansionWidth, gap, }) {
...
export function resolveToolbarModelLabel({ currentModelLabel, lastConfirmedModelLabel, }) {
