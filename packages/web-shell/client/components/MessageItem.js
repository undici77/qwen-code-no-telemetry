...
export const MessageItem = memo(function MessageItem({
  ...
  const body = (() => {
    switch (message.role) {
      ...
      case 'plan':
        return _jsx(PlanMessage, {
          id: message.id,
          todos: message.todos,
          isLocateFlashing: isLocateFlashing,
        });
      case 'system':
        return _jsx(SystemMessage, {
          content: message.content,
      ...
      case 'user_shell':
        return _jsx(UserShellMessage, {
          command: message.command,
          output: message.output,
        });
      case 'btw':
        return _jsx(BtwMessage, {
          question: message.question,
          answer: message.answer,
          isPending: message.isPending,
        });
      case 'insight_progress':
        return _jsx(InsightProgress, {
          progress: {
            stage: message.stage,
      ...
      case 'insight_error':
        return _jsx('div', {
          style: { color: 'var(--error-color, #e06c75)' },
  ...
  const safeBody = _jsx(ErrorBoundary, {
    label: `message:${message.role}`,
    resetKeys: [message],
    fallback: _jsx(MessageRenderError, {
  ...
  // affect it.
  const selectableSafeBody = _jsx('div', {
    'data-user-selectable': 'true',
