...
vi.mock('./MessageItem', async () => {
    ...
    return {
        MessageItem: ({ message, showAssistantActions, isLocateFlashing, assistantTurnFooterInfo, sendFailed, onRetrySend, }) => {
            ...
            return React.createElement('div', {
                ...
                'data-tool-ids': message.role === 'tool_group'
...
vi.mock('@tanstack/react-virtual', () => ({
    useVirtualizer: ({ count, enabled, getItemKey, }) => {
        const virtualItems = enabled && virtualizerTestState.renderItems
...
const standaloneToolMsg = (id, toolName) => ({
    id,
    role: 'tool_group',
...
const backgroundNotificationMsg = (id, toolUseId) => ({
    id,
    role: 'system',
...
const monitorNotificationMsg = (id) => ({
    id,
    role: 'system',
...
const describedAgentMsg = (id, description) => ({
    id,
    role: 'tool_group',
    tools: [
        {
...
function mount(messages, ref, opts = {}) {
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: opts.customization ?? {}, children: _jsx(CompactModeContext.Provider, { value: opts.compactMode ?? false, children: _jsx(TranscriptRenderModeProvider, { value: opts.transcriptRenderMode ?? 'interactive', children: _jsx(MessageList, { ref: ref, messages: messages, pendingApproval: null, hideSessionTimeline: opts.hideSessionTimeline, loadingTranscript: opts.loadingTranscript, catchingUp: opts.catchingUp, hasOlderHistory: opts.hasOlderHistory, loadingOlderHistory: opts.loadingOlderHistory, historyCapacityReached: opts.historyCapacityReached, historyPaginationError: opts.historyPaginationError, onLoadOlderHistory: opts.onLoadOlderHistory, transcriptBlockCount: opts.transcriptBlockCount, transcriptActivity: opts.transcriptActivity, onReloadTranscript: opts.onReloadTranscript, isResponding: opts.isResponding, hideFirstUserMessage: opts.hideFirstUserMessage, firstTurnMetrics: opts.firstTurnMetrics, includeSubagentToolUsageInMetrics: opts.includeSubagentToolUsageInMetrics, onCanScrollToBottomChange: opts.onCanScrollToBottomChange, failedPromptMessageId: opts.failedPromptMessageId, onRetryFailedPrompt: opts.onRetryFailedPrompt }) }) }) }) }));
    });
    mounted.push({
        root,
        container,
...
function rerenderMessages(container, messages, opts = {}) {
    const entry = mounted.find((item) => item.container === container);
    if (!entry)
        throw new Error('Expected mounted MessageList root');
    act(() => {
        entry.root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: {}, children: _jsx(CompactModeContext.Provider, { value: entry.compactMode, children: _jsx(TranscriptRenderModeProvider, { value: entry.transcriptRenderMode, children: _jsx(MessageList, { messages: messages, pendingApproval: null, loadingTranscript: opts.loadingTranscript, catchingUp: opts.catchingUp, isResponding: opts.isResponding }) }) }) }) }));
    });
}
function parallelAgentsSummary(container) {
    return (Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.includes('Parallel agents')) ?? null);
}
...
const isCollapsed = (c, id) => c.querySelector(`[data-testid="msg-${id}"]`) === null;
...
describe('MessageList — failed prompt retry', () => {
    it('marks only the matching user message and forwards retry', () => {
        ...
        expect(container
            ...
            ?.getAttribute('data-send-failed')).toBe('true');
        expect(container
            .querySelector('[data-testid="msg-u2"]')
            ?.getAttribute('data-send-failed')).toBeNull();
        act(() => container
            .querySelector('[data-testid="retry-u1"]')
            ?.click());
...
describe('MessageList — compact mode', () => {
    it('hides thinking rows without removing surrounding transcript content', () => {
        const container = mount([userMsg('u1'), thinkingMsg('t1'), asstMsg('a1')], undefined, {
            compactMode: true,
    ...
    it('merges tool groups separated only by hidden thinking', () => {
        ...
        expect(container
            ...
            ?.getAttribute('data-timestamp')).toBe('1000');
        expect(container
            .querySelector('[data-testid="msg-g1"]')
            ?.getAttribute('data-tool-ids')).toBe('call-g1,call-g2');
        expect(container.querySelector('[data-testid="msg-a1"]')).not.toBeNull();
    ...
    it('keeps visible thinking and tool groups in transcript order', () => {
        const container = mount([
            ...
            toolMsg('g2'),
            asstMsg('a1'),
        ], undefined, { customization: { collapseCompletedTurns: false } });
        ...
        expect(container.querySelector('[data-testid="msg-a1"]')).not.toBeNull();
        expect(Array.from(container.querySelectorAll('[data-testid^="msg-"]')).map((element) => element.getAttribute('data-testid'))).toEqual(['msg-u1', 'msg-g1', 'msg-t1', 'msg-g2', 'msg-a1']);
    });
    ...
    it.each(['TodoWrite', 'AskUserQuestion'])('keeps %s groups separate across hidden thinking', (toolName) => {
        const container = mount([
            toolMsg('g1'),
            thinkingMsg('t1'),
        ...
        expect(container.querySelector('[data-testid="msg-g1"]')).not.toBeNull();
        expect(container.querySelector('[data-testid="msg-special"]')).not.toBeNull();
    });
    it.each([
        ...
        const container = mount([special, thinkingMsg('t1'), toolMsg('g2')], undefined, {
            compactMode: true,
            customization: { collapseCompletedTurns: false },
        });
        expect(container.querySelector('[data-testid="msg-special"]')).not.toBeNull();
        expect(container
            ...
            ?.getAttribute('data-tool-ids')).toBe('call-g2');
    });
});
describe('MessageList — turn collapse (DOM)', () => {
    ...
    it('aborts an in-flight transcript reload when the reader leaves the tail', async () => {
        ...
        let reloadSignal;
        const onReloadTranscript = vi.fn((signal) => {
            reloadSignal = signal;
    ...
    it('keeps the final answer when active agents are pinned after it', () => {
        ...
        expect(c
            ...
            ?.getAttribute('data-assistant-actions')).toBe('true');
        click(toggle(c, 'u1'));
        expect(has(c, 'a1')).toBe(true);
    ...
    it('keeps an automatically expanded terminal group mounted until its delay expires', () => {
        ...
        expect(parallelAgentsSummary(c)?.getAttribute('aria-expanded')).toBe('true');
        rerenderMessages(c, [
            userMsg('u1'),
            agentMsg('agent-1'),
            agentMsg('agent-2'),
        ...
        expect(parallelAgentsSummary(c)?.getAttribute('aria-expanded')).toBe('false');
        expect(toggleRow(c, 'u1').getAttribute('aria-expanded')).toBe('true');
        expect(parallelAgentsSummary(c)?.getAttribute('aria-disabled')).toBe('true');
    ...
    it('keeps an earlier turn group collapsed when an unrelated response starts', () => {
        ...
        expect(parallelAgentsSummary(c)?.getAttribute('aria-expanded')).toBe('false');
        rerenderMessages(c, [...messages, userMsg('u2'), thinkingMsg('u2-thinking')], { catchingUp: false, isResponding: true });
        expect(parallelAgentsSummary(c)?.getAttribute('aria-expanded')).toBe('false');
    });
    it('does not reopen a background notification loaded with transcript history', () => {
        ...
        rerenderMessages(c, [userMsg('u1'), asstMsg('a1'), backgroundNotificationMsg('bg1')], { loadingTranscript: false });
        expect(has(c, 'a1')).toBe(false);
        expect(has(c, 'bg1')).toBe(true);
    });
    it('does not flash a grace window when catch-up delivers a notification', () => {
        ...
        rerenderMessages(c, [...initialMessages, backgroundNotificationMsg('bg-new')], { catchingUp: false });
        expect(has(c, 'a1')).toBe(false);
    ...
    it('starts collapsing when summary thinking begins', () => {
        ...
        expect(parallelAgentsSummary(c)?.getAttribute('aria-expanded')).toBe('true');
        rerenderMessages(c, [...completedMessages, thinkingMsg('summary-thinking')], { isResponding: true });
        expect(has(c, 'bg1')).toBe(true);
        expect(has(c, 'bg2')).toBe(true);
        expect(has(c, 'bg3')).toBe(true);
        expect(parallelAgentsSummary(c)?.getAttribute('aria-expanded')).toBe('true');
    ...
    it('does not defer a completed agent group for an unrelated new turn', () => {
        ...
        expect(parallelAgentsSummary(c)?.getAttribute('aria-expanded')).toBe('true');
        rerenderMessages(c, [...completedTurn, userMsg('u2'), thinkingMsg('u2-thinking')], { isResponding: true });
    ...
    it('does not defer an agent group for a non-agent notification', () => {
        ...
        act(() => vi.advanceTimersByTime(200));
        expect(parallelAgentsSummary(c)?.getAttribute('aria-expanded')).toBe('true');
        rerenderMessages(c, [...completedMessages, monitorNotificationMsg('monitor')], { isResponding: true });
    ...
    it('defers only the group that owns the awaited agent notification', () => {
        ...
        const summaries = () => Array.from(c.querySelectorAll('button')).filter((button) => button.textContent?.includes('Parallel agents'));
        expect(summaries()).toHaveLength(2);
        expect(summaries().every((b) => b.getAttribute('aria-expanded') === 'true')).toBe(true);
        rerenderMessages(c, [
            userMsg('u1'),
    ...
    it('defers every latest-turn group while the response awaits the agent summary', () => {
        ...
        agentB1.tools[0].status = 'pending';
        agentB2.tools[0].status = 'pending';
        const c = mount([userMsg('u1'), agentA1, agentA2, asstMsg('narration'), agentB1, agentB2], undefined, { isResponding: true });
        const summaries = () => Array.from(c.querySelectorAll('button')).filter((button) => button.textContent?.includes('Parallel agents'));
        expect(summaries()).toHaveLength(2);
        const completedMessages = [
            userMsg('u1'),
    ...
    it('does not add tool summary usage when full transcript usage includes it', () => {
        ...
        ], undefined, { includeSubagentToolUsageInMetrics: false });
    ...
    it('drops a pending history anchor when the transcript changes', async () => {
        ...
        const onLoadOlderHistory = vi.fn(() => new Promise((resolve) => {
        ...
        const root = createRoot(container);
        mounted.push({ root, container, transcriptRenderMode: 'interactive' });
        const render = (messages) => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(MessageList, { messages: messages, pendingApproval: null, hasOlderHistory: true, onLoadOlderHistory: onLoadOlderHistory }) }));
    ...
    it('releases pagination when a virtual anchor never mounts', async () => {
        ...
        const root = createRoot(container);
        mounted.push({ root, container, transcriptRenderMode: 'interactive' });
        const render = (nextMessages) => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(MessageList, { messages: nextMessages, pendingApproval: null, hasOlderHistory: true, onLoadOlderHistory: onLoadOlderHistory }) }));
        ...
        act(() => render([...messages]));
        await nextFrame();
        expect(container.querySelectorAll('[data-message-row-key]').length).toBeGreaterThan(0);
        list.scrollTop = 0;
        act(() => {
            list.dispatchEvent(new WheelEvent('wheel', { deltaY: -1 }));
            list.dispatchEvent(new Event('scroll'));
    ...
    it('suppresses the loading status during automatic pagination', async () => {
        ...
        let resolveLoad;
        const onLoadOlderHistory = vi.fn(() => new Promise((resolve) => (resolveLoad = resolve)));
        const c = mount([userMsg('u1')], undefined, {
            hasOlderHistory: true,
            onLoadOlderHistory,
        });
    ...
    it('shows a persistent error when history pagination fails', () => {
        ...
        expect(c.querySelector('[role="status"]')?.textContent).toBe('Earlier history could not be loaded.');
    });
    ...
    it('retries loading older history with force when the retry button is clicked', async () => {
        ...
        const button = Array.from(c.querySelectorAll('button')).find((el) => el.textContent === 'Retry');
    ...
    it('finishes cooldown following unless the user scrolls up', () => {
        ...
        scrollHeight = 1800;
        renderInto(root, [userMsg('u1'), { ...thinkingMsg('t1'), content: 'thinking more' }], ref, {
            catchingUp: false,
            isResponding: true,
        });
        expect(scrollTop).toBe(600);
        ...
        scrollHeight = 2400;
        renderInto(root, [userMsg('u1'), { ...thinkingMsg('t1'), content: 'thinking even more' }], ref, {
            catchingUp: false,
            isResponding: true,
        });
        const list = container.querySelector('[data-web-shell-message-list]');
        act(() => {
            list?.dispatchEvent(new WheelEvent('wheel', { bubbles: true, deltaY: -10 }));
            scrollTop = 900;
