    ...
    root.render(
      _jsx(I18nProvider, {
        ...
        children: _jsx(WebShellCustomizationProvider, {
          value: opts.customization ?? {},
          children: _jsx(CompactModeContext.Provider, {
            value: opts.compactMode ?? false,
            children: _jsx(TranscriptRenderModeProvider, {
    ...
    entry.root.render(
      _jsx(I18nProvider, {
        ...
        children: _jsx(WebShellCustomizationProvider, {
          value: {},
          children: _jsx(CompactModeContext.Provider, {
            value: entry.compactMode,
            children: _jsx(TranscriptRenderModeProvider, {
              value: entry.transcriptRenderMode,
              children: _jsx(MessageList, {
                messages: messages,
                pendingApproval: null,
                loadingTranscript: opts.loadingTranscript,
...
const assistantActions = (c, id) =>
  c
    .querySelector(`[data-testid="msg-${id}"]`)
    ?.getAttribute('data-assistant-actions');
const isCollapsed = (c, id) =>
  c.querySelector(`[data-testid="msg-${id}"]`) === null;
const queryToggle = (c, turnId) =>
  c.querySelector(`[data-testid="toggle-${turnId}"]`);
const toggle = (c, turnId) => queryToggle(c, turnId);
const disclosure = (c, id) =>
  c.querySelector(`[data-testid="disclosure-${id}"]`);
...
const mockMessageListWidth = (width) =>
...
describe('MessageList — failed prompt retry', () => {
  it('marks only the matching user message and forwards retry', () => {
    ...
    act(() => container.querySelector('[data-testid="retry-u1"]')?.click());
...
describe('MessageList — turn collapse (DOM)', () => {
  ...
  it('renders custom footer on the completed turn final assistant message', () => {
    const renderAssistantTurnFooter = vi.fn(({ turnId, message }) =>
      _jsxs('span', {
        ...
        children: [turnId, ':', message.id, ':', message.content],
      }),
    );
  ...
  it('maps each completed turn footer to its own turn id', () => {
    const renderAssistantTurnFooter = vi.fn(({ turnId, message }) =>
      _jsxs('span', {
        'data-testid': `assistant-turn-footer-${message.id}`,
        children: [turnId, ':', message.id],
      }),
    );
    const c = mount(
  ...
  it('renders the session timeline in the left gutter without expanding turns', async () => {
    ...
    const buttons = Array.from(
      c.querySelectorAll('[data-testid="session-timeline-entry"] button'),
    );
