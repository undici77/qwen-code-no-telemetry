...
vi.mock('./MessageTimestamp', async () => {
    ...
    return {
        MessageTimestamp: ({ children, toolGroupSpacing, }) => React.createElement('div', { 'data-tool-group-spacing': String(toolGroupSpacing === true) }, children),
        formatTimestamp: () => '',
    };
});
    ...
    return {
        AssistantMessage: ({ content, customFooterInfo, }) => {
            ...
            const customFooter = customFooterInfo
                ...
                : undefined;
            return React.createElement('div', { 'data-testid': 'assistant-ok' }, content, customFooter);
        },
        ThinkingMessage: ({ generateContent }) => React.createElement('div', {
...
const assistantMsg = (id, content) => ({ id, role: 'assistant', content, timestamp: 0 });
const thinkingMsg = (id, content) => ({ id, role: 'thinking', content, timestamp: 0 });
const toolMsg = (id) => ({
    id,
    role: 'tool_group',
    tools: [],
...
describe('MessageItem error isolation', () => {
    it('renders a healthy message normally (no fallback)', () => {
        const container = render(_jsx(I18nProvider, { language: "en", children: item(userMsg('1', 'hello')) }));
        expect(container.querySelector('[data-testid="user-ok"]')?.textContent).toBe('hello');
        expect(container.querySelector('[role="alert"]')).toBeNull();
    });
    it('degrades a crashing message to an inline notice while a sibling survives', () => {
        vi.spyOn(console, 'error').mockImplementation(() => { });
        const container = render(_jsxs(I18nProvider, { language: "en", children: [item(userMsg('ok', 'hello')), item(userMsg('bad', '__BOOM__'))] }));
        ...
        expect(container.querySelector('[role="alert"]')?.textContent).toContain(RENDER_ERROR);
    });
    it('right-aligns the fallback for a user message', () => {
        vi.spyOn(console, 'error').mockImplementation(() => { });
        const container = render(_jsx(I18nProvider, { language: "en", children: item(userMsg('1', '__BOOM__')) }));
    ...
    it('left-aligns the fallback for an assistant message', () => {
        vi.spyOn(console, 'error').mockImplementation(() => { });
        const container = render(_jsx(I18nProvider, { language: "en", children: item(assistantMsg('1', '__BOOM__')) }));
        const alert = container.querySelector('[role="alert"]');
        expect(alert).not.toBeNull();
...
describe('MessageItem tool group spacing', () => {
    it('uses larger row spacing only in compact mode', () => {
        const compact = render(_jsx(I18nProvider, { language: "en", children: _jsx(CompactModeContext.Provider, { value: true, children: item(toolMsg('compact')) }) }));
        const regular = render(_jsx(I18nProvider, { language: "en", children: _jsx(CompactModeContext.Provider, { value: false, children: item(toolMsg('regular')) }) }));
        const compactAssistant = render(_jsx(I18nProvider, { language: "en", children: _jsx(CompactModeContext.Provider, { value: true, children: item(assistantMsg('assistant', 'answer')) }) }));
        const defaultTool = render(_jsx(I18nProvider, { language: "en", children: item(toolMsg('default')) }));
        expect(compact.firstElementChild?.getAttribute('data-tool-group-spacing')).toBe('true');
        expect(regular.firstElementChild?.getAttribute('data-tool-group-spacing')).toBe('false');
        expect(compactAssistant.firstElementChild?.getAttribute('data-tool-group-spacing')).toBe('false');
        expect(defaultTool.firstElementChild?.getAttribute('data-tool-group-spacing')).toBe('false');
    });
});
describe('MessageItem generation updates', () => {
    it('rerenders a thinking message when generation becomes available', () => {
        ...
        const { root, container } = renderWithRoot(_jsx(I18nProvider, { language: "en", children: _jsx(MessageItem, { message: message }) }));
        expect(container
            .querySelector('[data-testid="thinking"]')
            ?.getAttribute('data-has-generator')).toBe('false');
        const generateContent = async function* () { };
        act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(MessageItem, { message: message, generateContent: generateContent }) })));
        expect(container
            .querySelector('[data-testid="thinking"]')
            ?.getAttribute('data-has-generator')).toBe('true');
    });
});
describe('MessageItem assistant turn footer', () => {
    const customization = (renderAssistantTurnFooter) => ({ renderAssistantTurnFooter });
    ...
    it('passes custom footer info to assistant messages', () => {
        const renderAssistantTurnFooter = vi.fn(({ turnId }) => (_jsx("div", { "data-testid": "assistant-footer", children: turnId })));
        const container = render(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: customization(renderAssistantTurnFooter), children: _jsx(MessageItem, { message: assistantMsg('1', 'hello'), assistantTurnFooterInfo: footerInfo('u1') }) }) }));
        expect(renderAssistantTurnFooter).toHaveBeenCalledWith(footerInfo('u1'));
        expect(container.querySelector('[data-testid="assistant-footer"]')?.textContent).toBe('u1');
    });
    it('updates custom footer content when only footer info changes', () => {
        ...
        const { root, container } = renderWithRoot(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: value, children: _jsx(MessageItem, { message: message, assistantTurnFooterInfo: footerInfo('u1') }) }) }));
        expect(container.querySelector('[data-testid="assistant-footer"]')?.textContent).toBe('u1');
        act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellCustomizationProvider, { value: value, children: _jsx(MessageItem, { message: message, assistantTurnFooterInfo: footerInfo('u2') }) }) })));
        expect(container.querySelector('[data-testid="assistant-footer"]')?.textContent).toBe('u2');
    });
