...
function render(props, language = 'en') {
    ...
    act(() => {
        root.render(_jsx(I18nProvider, { language: language, children: _jsx(GitBranchIndicator, { ...props }) }));
    });
}
afterEach(() => {
...
function chip() {
    ...
    if (!el)
        throw new Error('branch indicator was not rendered');
    return el;
}
describe('GitBranchIndicator', () => {
    ...
    it('localizes the accessible branch label', () => {
        expect(getTranslator('en')('git.currentBranch', { branch: 'main' })).toBe('Current Git branch: main');
        expect(getTranslator('zh-CN')('git.currentBranch', { branch: 'main' })).toBe('当前 Git 分支：main');
    });
...
//# sourceMappingURL=GitBranchIndicator.test.js.map