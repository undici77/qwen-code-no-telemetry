...
import { canModifyAgent, filterAgents, isOverridden, preserveAgentSelection, } from './agents-manager-logic';
const agents = [
    {
        kind: 'agent',
        name: 'code-reviewer',
...
describe('agents manager logic', () => {
    ...
    it('preserves only a selection that still exists', () => {
        expect(preserveAgentSelection({ name: 'Explore', level: 'builtin' }, agents)).toBe(agents[2]);
        expect(preserveAgentSelection({ name: 'removed', level: 'project' }, agents)).toBeNull();
        expect(preserveAgentSelection(null, agents)).toBeNull();
    });
    it('preserves the selected level when names are shadowed', () => {
        expect(preserveAgentSelection({ name: 'code-reviewer', level: 'user' }, agents)).toBe(agents[1]);
    });
...
//# sourceMappingURL=agents-manager-logic.test.js.map