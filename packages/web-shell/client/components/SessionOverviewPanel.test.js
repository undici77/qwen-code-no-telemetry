...
vi.mock('../hooks/useScopedSessions', () => ({
    useScopedSessions: (_workspaceCwd, options = {}) => {
...
const { SessionOverviewPanel, deriveSessionCards } = await import('./SessionOverviewPanel');
...
function statusSession(id, extra = {}) {
    return {
        sessionId: id,
        workspaceCwd: '/w',
...
function render(props = {}) {
    ...
    act(() => root.render(_jsx(I18nProvider, { language: "en", children: _jsx(SessionOverviewPanel, { onOpenSession: onOpenSession, ...props }) })));
}
...
function cardLabels() {
    return Array.from(container.querySelectorAll('ul li')).map((li) => li.querySelectorAll('button')[0]?.textContent?.trim() ?? '');
}
function selectAllCheckbox() {
    return container.querySelector('input[type="checkbox"]');
}
function tabButton() {
    return Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.includes('Open in new tab'));
}
describe('deriveSessionCards', () => {
    ...
    it('needs-approval wins even when the prompt is also active (blocked turn)', () => {
        ...
        expect(deriveSessionCards(sessions, status, undefined)[0].status).toBe('needsApproval');
    });
    ...
    it('carries model and client count from the status report', () => {
        const cards = deriveSessionCards([session('s', { clientCount: 2 })], [statusSession('s', { currentModelId: 'qwen-max', clientCount: 2 })], undefined);
...
describe('SessionOverviewPanel', () => {
    ...
    it('switches the current session when a card label is clicked', () => {
        ...
        render();
        const label = container.querySelector('ul li button');
    ...
    it('opens the selected sessions as a split in ONE new tab (?split=…)', () => {
        ...
        act(() => selectAll.click());
        const tabButton = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.includes('Open in new tab'));
        expect(tabButton.disabled).toBe(false);
        act(() => tabButton.dispatchEvent(new MouseEvent('click', { bubbles: true })));
    ...
    it('severs window.opener on the new split tab (token is in its URL)', () => {
        ...
        const opened = { focus: vi.fn(), opener: {} };
    ...
    it('opens the selected sessions in split, in ranked order', () => {
        ...
        const selectAll = container.querySelector('input[type="checkbox"]');
        act(() => selectAll.click());
        const splitButton = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.includes('Open in split'));
    ...
    it('toggling a card checkbox selects it without switching sessions', () => {
        sessionsState.sessions = [session('s-run', { displayName: 'Alpha' })];
        render();
        const cardCheckbox = container.querySelector('ul li input[type="checkbox"]');
    ...
    it('caps the split to 6 sessions and warns when more are selected', () => {
        sessionsState.sessions = Array.from({ length: 8 }, (_, i) => session(`s${i}`, { displayName: `S${i}` }));
        const onOpenSplit = vi.fn();
        render({ onOpenSplit });
        ...
        act(() => tabButton().dispatchEvent(new MouseEvent('click', { bubbles: true })));
        const split = new URL(String(openSpy.mock.calls[0][0])).searchParams.get('split');
...
describe('SessionOverviewPanel polling', () => {
    ...
    it('does not overlap polls while one is still in flight', async () => {
        ...
        sessionsReload.mockImplementation(() => new Promise(() => { }));
        vi.useFakeTimers();
        try {
            render();
            sessionsReload.mockClear();
    ...
    it('re-queries other workspaces on each list poll (multi-workspace)', async () => {
        ...
        try {
            ...
            expect(workspaceClient.listWorkspaceSessionsPage).toHaveBeenCalledWith('/wsB', expect.objectContaining({ archiveState: 'active' }));
        }
        finally {
            vi.useRealTimers();
        }
...
//# sourceMappingURL=SessionOverviewPanel.test.js.map