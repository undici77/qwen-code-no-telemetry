...
const { connection, workspace, workspaceActions, active, pinned, archived } = vi.hoisted(() => {
    const makeSessions = () => {
        const state = {
            sessions: [],
            loading: false,
...
vi.mock('../../session-catalog/session-catalog-hooks', () => ({
    useWebShellSessions: (options) => {
        const state = options?.archiveState === 'archived'
            ? archived
...
function makeSession(sessionId, over = {}) {
    return {
        sessionId,
        workspaceCwd: '/tmp/project',
...
function renderSidebar() {
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellSidebar, { collapsed: false, onCollapsedChange: () => { }, onOpenSettings: () => { }, onOpenDaemonStatus: () => { }, onOpenScheduledTasks: () => { }, onOpenGoals: () => { }, onOpenSessions: () => { }, onOpenSplitView: () => { }, onNewSession: () => false, onLoadSession: () => { }, onError: () => { } }) }));
    });
}
...
describe('WebShellSidebar collapsed session group persistence', () => {
    it('shows the complete session name in a native tooltip', async () => {
        renderSidebar();
        await flushSidebar();
        const sessionName = container.querySelector('[title="API review"]');
    ...
    it('shows the complete archived session name in a native tooltip', async () => {
        ...
        renderSidebar();
        await flushSidebar();
        const archivedHeader = Array.from(container.querySelectorAll('button[aria-expanded]')).find((btn) => btn.textContent?.includes('Archived'));
        expect(archivedHeader).not.toBeNull();
        act(() => click(archivedHeader));
        await flushSidebar();
        const sessionName = container.querySelector('[title="Archived task"]');
    ...
    it('writes collapsed section ids with the qwen-code-web-shell-* key', async () => {
        ...
        expect(window.localStorage.getItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY)).toBe(JSON.stringify(['group:group-1']));
    });
    it('keeps a collapsed named group collapsed across remount', async () => {
        window.localStorage.setItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY, JSON.stringify(['group:group-1']));
        renderSidebar();
        await flushSidebar();
        expect(groupHeader('Backend').getAttribute('aria-expanded')).toBe('false');
        const backend = container.querySelector('section[aria-label="Backend"]');
    ...
    it('keeps an expanded group expanded across remount after clearing collapse', async () => {
        ...
        expect(window.localStorage.getItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY)).toBe(JSON.stringify([]));
        act(() => root.unmount());
        container.remove();
        container = document.createElement('div');
        document.body.appendChild(container);
    ...
    it('tolerates corrupt localStorage data', async () => {
        window.localStorage.setItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY, 'not valid json');
        renderSidebar();
        await flushSidebar();
        expect(groupHeader('Backend').getAttribute('aria-expanded')).toBe('true');
        expect(container.textContent).toContain('API review');
    ...
    it('auto-collapses a brand-new section that appears mid-session', async () => {
        ...
        expect(container.querySelector('section[aria-label="Red"]')?.textContent).not.toContain('Release notes');
        expect(groupHeader('Backend').getAttribute('aria-expanded')).toBe('true');
    });
    it('restores multiple section kinds and keeps sibling ids when one is removed', async () => {
        ...
        active.data = active.sessions;
        window.localStorage.setItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY, JSON.stringify(['color:red', 'group:group-1', 'recent']));
        renderSidebar();
        await flushSidebar();
        expect(groupHeader('Backend').getAttribute('aria-expanded')).toBe('false');
        expect(groupHeader('Ungrouped').getAttribute('aria-expanded')).toBe('false');
        ...
        expect(groupHeader('Ungrouped').getAttribute('aria-expanded')).toBe('false');
        expect(groupHeader('Red').getAttribute('aria-expanded')).toBe('false');
        expect(JSON.parse(window.localStorage.getItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY) ??
            '[]')).toEqual(['color:red', 'recent']);
    });
    it('does not clobber workspace-scoped collapse ids when primary toggles', async () => {
        window.localStorage.setItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY, JSON.stringify([
            'group:group-1',
            'ws:other|group:g2',
            'ws:other|ungrouped',
        ]));
...
//# sourceMappingURL=WebShellSidebar.collapse-persist.test.js.map