...
describe('isSidebarToggleShortcut', () => {
    ...
    it('ignores other keys with the modifier held', () => {
        expect(isSidebarToggleShortcut(key({ metaKey: true, key: 'k' }))).toBe(false);
    });
    it('leaves Shift/Alt variants to the browser and other bindings', () => {
        expect(isSidebarToggleShortcut(key({ metaKey: true, shiftKey: true }))).toBe(false);
        expect(isSidebarToggleShortcut(key({ ctrlKey: true, altKey: true }))).toBe(false);
    });
    it('rejects the ambiguous Cmd+Ctrl combination', () => {
        expect(isSidebarToggleShortcut(key({ metaKey: true, ctrlKey: true }))).toBe(false);
    });
});
//# sourceMappingURL=sidebarToggleShortcut.test.js.map