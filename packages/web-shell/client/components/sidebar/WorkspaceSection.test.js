...
const { workspaceGit, workspaceGitBranches, workspaceGitCheckout, pickerWorkspaceClient, } = vi.hoisted(() => {
...
function renderSection(overrides = {}) {
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WorkspaceSection, { workspace: overrides.workspace ?? trustedWorkspace, client: overrides.client ?? makeClient(), reloadToken: overrides.reloadToken ?? 0, expanded: overrides.expanded, untrustedLabel: "Untrusted", readOnlyLabel: "Read-only", trustToOpenLabel: "Trust to open", noSessionsLabel: "No sessions", loadErrorLabel: "Load failed", organizationEnabled: overrides.organizationEnabled ?? false, sourceType: overrides.sourceType, channelGroupingEnabled: overrides.channelGroupingEnabled, ungroupedLabel: "Ungrouped", formatTime: () => '', renderSession: (session) => (_jsx("div", { children: session.displayName }, session.sessionId)), onOpenGitDiff: overrides.onOpenGitDiff }) }));
    });
}
...
function gitChip() {
    return container.querySelector('[data-web-shell-git-branch]');
}
beforeEach(() => {
...
describe('WorkspaceSection label', () => {
    ...
    it('shows the complete read-only session name in a native tooltip', async () => {
        ...
        await flush();
        expect(container.querySelector('[title="A very long session name"]')).not.toBeNull();
    });
    it('does not render sessions loaded for the previous source', async () => {
        let resolveChannel = () => { };
        const channelPage = new Promise((resolve) => {
            resolveChannel = resolve;
        });
        let resolveDefault = () => { };
        const defaultPage = new Promise((resolve) => {
            resolveDefault = resolve;
        });
        const listWorkspaceSessionsPage = vi.fn((options) => options?.sourceType === 'channel' ? channelPage : defaultPage);
        const client = {
            workspaceByCwd: vi.fn(() => ({
                workspaceGit,
                listWorkspaceSessionsPage,
    ...
    it('does not carry a load error across a source switch', async () => {
        const listWorkspaceSessionsPage = vi.fn((options) => options?.sourceType === 'channel'
            ? new Promise(() => { })
        ...
        expect(listWorkspaceSessionsPage).toHaveBeenCalledWith(expect.objectContaining({ sourceType: 'channel' }));
        expect(listWorkspaceSessionsPage.mock.calls.filter(([options]) => options?.sourceType ===
            'channel')).toHaveLength(1);
    });
    it('does not flash the empty notice while a fresh source settles', async () => {
        const client = {
            workspaceByCwd: vi.fn(() => ({
                workspaceGit,
                listWorkspaceSessionsPage: vi.fn(() => new Promise(() => { })),
                listSessionGroups: vi.fn().mockResolvedValue({ groups: [] }),
            })),
        };
        renderSection({ client, expanded: true, sourceType: 'channel' });
    ...
    it('groups a secondary workspace with its own channel catalog', async () => {
        ...
        await flush();
        expect(container.querySelector('section[aria-label="DingTalk"]')?.textContent).toContain('DingTalk secondary');
        expect(container.querySelector('section[aria-label="Feishu"]')?.textContent).toContain('Feishu secondary');
        expect(container.querySelector('section[aria-label="Organization group"]')).toBeNull();
    ...
    it('ignores a stale channel catalog response', async () => {
        ...
        await flush();
        expect(container.querySelector('section[aria-label="Feishu"]')).not.toBeNull();
        resolveStale({
            revision: 'old',
            instances: {
                instance: {
    ...
    it('refreshes the channel catalog on the session poll tick', async () => {
        ...
        const poll = setIntervalSpy.mock.calls.findLast(([, timeout]) => timeout === 10_000);
...
describe('WorkspaceSection git chip', () => {
    ...
    it('re-fetches git status right after a picker checkout instead of waiting for the poll', async () => {
        ...
        const mainItem = Array.from(document.body.querySelectorAll('button')).find((button) => button.textContent === 'main');
...
//# sourceMappingURL=WorkspaceSection.test.js.map