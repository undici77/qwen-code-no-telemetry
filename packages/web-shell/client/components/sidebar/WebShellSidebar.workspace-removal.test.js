...
const {
  ...
  const useSessions = vi.fn((options) =>
...
vi.mock('../../session-catalog/session-catalog-hooks', () => {
  ...
  return {
    useWebShellSessions: (options) => {
      ...
      return {
        ...state,
        data: 'data' in state ? state.data : state.sessions,
        catalogQuery,
      };
    },
...
function workspaceAction(cwd) {
  return Array.from(
    container.querySelectorAll('button[aria-label="Workspace actions"]'),
  ).find((button) =>
...
function archiveButtonFor(label) {
  return Array.from(
    container.querySelectorAll('button[aria-label="Archive"]'),
  ).find((button) =>
...
async function expandArchived() {
  const button = Array.from(container.querySelectorAll('button')).find(
    (candidate) => candidate.textContent?.includes('Archived'),
  );
  expect(button).toBeDefined();
  await act(async () => {
    click(button);
...
async function openSessionSearch() {
  const searchButton = Array.from(container.querySelectorAll('button')).find(
    (button) => button.getAttribute('aria-label') === 'Search sessions',
  );
...
function sessionAction(label) {
  return Array.from(
    container.querySelectorAll('button[aria-label="More actions"]'),
  ).find((button) =>
    button.closest('[class*="sessionRow"]')?.textContent?.includes(label),
  );
}
function inlineSessionAction(label, action) {
  return Array.from(
    container.querySelectorAll(`button[aria-label="${action}"]`),
  ).find((button) =>
    button.closest('[class*="sessionRow"]')?.textContent?.includes(label),
  );
}
...
describe('WebShellSidebar workspace removal', () => {
  ...
  it('gives a locked trusted secondary active row normal actions through its workspace', async () => {
    ...
    const makeClient = (cwd, actions) => ({
      listWorkspaceSessions: async (options) =>
  ...
  it('does not assign a created group after its locked workspace unlocks', async () => {
    ...
    const createGroup = Array.from(
      document.body.querySelectorAll('button[role="menuitem"]'),
  ...
  it('routes locked secondary delete, archive, color, and pinned mutations only to its client', async () => {
    ...
    const secondaryOrganization = vi.fn().mockResolvedValue({});
    const makeClient = (cwd, actions) => ({
      listWorkspaceSessions: vi.fn().mockImplementation((options) =>
        cwd !== '/tmp/other'
          ? []
  ...
  it('keeps a trusted locked workspace New Task available without session organization', async () => {
    ...
    const newTaskButtons = Array.from(
      container.querySelectorAll('button[aria-label="New task"]'),
    );
  ...
  it('does not infer secondary active export from session_export', async () => {
    ...
    expect(
      Array.from(document.body.querySelectorAll('[role="menuitem"]')).some(
        (item) => item.textContent?.includes('Export'),
      ),
    ).toBe(false);
  });
  ...
  it('honors a missing rename item for double-click editing', async () => {
    ...
    const row = Array.from(container.querySelectorAll('[role="button"]')).find(
      (candidate) => candidate.textContent?.includes('Current primary'),
    );
  ...
  it('allows organization but keeps destructive actions conservative for unlocked secondary sessions', async () => {
    ...
    const secondaryCreateGroup = Array.from(
      container.querySelectorAll('button[aria-label="Create group"]'),
    ).find((button) =>
      button.closest('[class*="headerRow"]')?.textContent?.includes('other'),
    );
  ...
  it('uses custom workspace row content only when the workspace is locked', async () => {
    const render = vi.fn((ws) =>
      _jsxs('span', {
        'data-testid': 'custom-workspace',
        children: ['Custom ', ws.cwd],
      }),
    );
...
describe('WebShellSidebar session source switch', () => {
  ...
  it('preserves channel completion state while the tasks source is active', async () => {
    ...
    renderSidebar();
    const row = Array.from(container.querySelectorAll('[role="button"]')).find(
      (candidate) => candidate.textContent?.includes('Channel session'),
    );
    expect(row).toBeDefined();
  ...
  it('groups channel sessions by platform type and toggles each group', async () => {
    ...
    const dingTalkGroup = container.querySelector(
...
describe('WebShellSidebar session list notices', () => {
  ...
  it('offers a retry when the first page load failed', async () => {
    ...
    const retry = Array.from(container.querySelectorAll('button')).find(
      (button) => button.textContent?.includes('Failed to load sessions'),
    );
...
describe('WebShellSidebar Live group', () => {
  it('shows Live sessions without exposing the backing Conversations workspace', async () => {
    ...
    const liveToggle = Array.from(container.querySelectorAll('button')).find(
      (button) => button.textContent?.includes('Live'),
    );
