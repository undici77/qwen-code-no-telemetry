...
const { connection, workspace, workspaceActions, active, archived, useSessions, useChannels, listWorkspaceSessions, archiveSessionsData, unarchiveSessionsData, deleteSessionsData, updateSessionOrganization, exportSession, exportArchivedSession, sessionActions, channelState, invalidateSessionCatalog, renameSessionCatalog, refreshSessionCatalogQueries, useSessionCatalogPollingSpy, } = vi.hoisted(() => {
    const makeSessions = () => ({
        sessions: [],
        loading: false,
        error: null,
    ...
    const useSessions = vi.fn((options) => (options?.archiveState === 'archived' ? archived : active));
...
vi.mock('../../session-catalog/session-catalog-hooks', () => {
    ...
    return {
        ...
        useSessionCatalogQuery: (client, query, options) => {
            ...
            React.useEffect(() => {
                if (options.enabled === false)
                    return;
                const listener = (workspaceCwd) => {
                    if (workspaceCwd === query.workspaceCwd)
                        void reload();
                };
        ...
        useSessionCatalogQueries: (client, queries, options) => {
...
const { COLLAPSED_SESSION_SECTIONS_STORAGE_KEY } = await import('./collapsedSessionSections');
...
function renderSidebar(overrides = {}) {
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx(WebShellSidebar, { collapsed: false, onCollapsedChange: () => { }, onOpenSettings: () => { }, onOpenDaemonStatus: () => { }, onOpenScheduledTasks: () => { }, onOpenGoals: overrides.onOpenGoals ?? (() => { }), onOpenSessions: () => { }, onOpenSplitView: () => { }, onNewSession: overrides.onNewSession ?? (() => false), onLoadSession: () => { }, onError: overrides.onError ?? (() => { }), selectedWorkspaceCwd: overrides.selectedWorkspaceCwd, onSelectWorkspace: overrides.onSelectWorkspace, onOpenAddWorkspace: overrides.onOpenAddWorkspace, workspaces: overrides.workspaces, lockedWorkspaceCwd: overrides.lockedWorkspaceCwd, lockedWorkspace: overrides.lockedWorkspace, sessionActions: overrides.sessionActions }) }));
    });
}
function workspaceAction(cwd) {
    return Array.from(container.querySelectorAll('button[aria-label="Workspace actions"]')).find((button) => button.parentElement?.parentElement?.textContent?.includes(cwd.split('/').at(-1)));
}
...
async function expandWorkspace(name) {
    const button = Array.from(container.querySelectorAll('button')).find((candidate) => candidate.textContent?.includes(name));
...
async function ensureWorkspaceExpanded(name) {
    const button = Array.from(container.querySelectorAll('button')).find((candidate) => candidate.textContent?.includes(name));
    expect(button).toBeDefined();
...
function archiveButtonFor(label) {
    return Array.from(container.querySelectorAll('button[aria-label="Archive"]')).find((button) => button.closest('[role="button"]')?.textContent?.includes(label));
}
async function expandArchived() {
    const button = Array.from(container.querySelectorAll('button')).find((candidate) => candidate.textContent?.includes('Archived'));
    expect(button).toBeDefined();
    await act(async () => {
        click(button);
        await Promise.resolve();
...
async function switchSessionSource(label) {
    const tab = Array.from(container.querySelectorAll('[role="tab"]')).find((button) => button.textContent?.trim() === label);
...
function enableChannelOrganization() {
...
async function settleGroupsCatalog() {
    await act(async () => {
...
async function openSessionSearch() {
    const searchButton = Array.from(container.querySelectorAll('button')).find((button) => button.getAttribute('aria-label') === 'Search sessions');
...
function sessionAction(label) {
    return Array.from(container.querySelectorAll('button[aria-label="More actions"]')).find((button) => button
        .closest('[class*="sessionRow"]')
        ?.textContent?.includes(label));
}
function inlineSessionAction(label, action) {
    return Array.from(container.querySelectorAll(`button[aria-label="${action}"]`)).find((button) => button
        .closest('[class*="sessionRow"]')
        ?.textContent?.includes(label));
}
async function selectSessionMenuItem(label, itemLabel) {
    ...
    const item = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((candidate) => candidate.textContent?.includes(itemLabel));
...
async function openSessionMenuItems(label) {
    const trigger = sessionAction(label);
    expect(trigger).toBeDefined();
    await act(async () => {
        click(trigger);
...
function useWorkspaceSessionCatalog(resolve) {
    workspace.client.workspaceByCwd.mockImplementation((cwd) => ({
...
function openRemoval(cwd) {
    const trigger = workspaceAction(cwd);
    expect(trigger).toBeDefined();
    act(() => click(trigger));
    const item = document.body.querySelector(`[aria-label="Remove workspace: ${cwd}"]`);
    expect(item).not.toBeNull();
    act(() => click(item));
}
function dialogButton(label) {
    const button = Array.from(document.body.querySelectorAll('button')).find((candidate) => candidate.textContent === label);
    expect(button).toBeDefined();
    return button;
}
beforeEach(() => {
    ...
    useSessions.mockImplementation((options) => options?.archiveState === 'archived' ? archived : active);
...
describe('WebShellSidebar workspace removal', () => {
    it('delegates Add workspace to the App-owned dialog', () => {
        ...
        const addButton = container.querySelector('button[aria-label="Add workspace"]');
    ...
    it('scopes pinned and archived sessions to a locked secondary workspace', async () => {
        ...
        const listSecondarySessions = vi.fn(async (options) => {
        ...
        const pinnedCallIndex = listSecondarySessions.mock.calls.findIndex(([options]) => options?.group === 'pinned');
        ...
        const archivedButton = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.includes('Archived'));
        ...
        expect(refreshSessionCatalogQueries).not.toHaveBeenCalledWith(expect.arrayContaining([
            expect.objectContaining({ routeKind: 'legacy' }),
        ]));
        const archivedCallIndex = listSecondarySessions.mock.calls.findIndex(([options]) => options?.archiveState === 'archived');
        ...
        expect(useSessions.mock.calls.every(([options]) => options?.sourceType === 'default')).toBe(true);
        expect(listSecondarySessions.mock.calls.every(([options]) => options?.sourceType === 'default')).toBe(true);
    });
    it('gives a locked trusted secondary active row normal actions through its workspace', async () => {
        ...
        const makeClient = (cwd, actions) => ({
            listWorkspaceSessions: async (options) => cwd === '/tmp/other' && options?.archiveState === 'active'
                ? [
                    {
        ...
        workspace.client.workspaceByCwd.mockImplementation((cwd) => cwd === '/tmp/other' ? secondaryClient : primaryClient);
        vi.stubGlobal('URL', {
            ...URL,
    ...
    it('allows only the current locked-secondary session to rename', async () => {
        ...
        workspace.client.workspaceByCwd.mockImplementation((cwd) => ({
            listWorkspaceSessions: vi.fn().mockResolvedValue(cwd === '/tmp/other'
                ? [
                    {
        ...
        expect(inlineSessionAction('Locked current', 'Delete')?.disabled).toBe(true);
        expect(inlineSessionAction('Locked current', 'Archive')?.disabled).toBe(true);
        expect(inlineSessionAction('Locked other', 'Rename')?.disabled).toBe(true);
        const currentRow = Array.from(container.querySelectorAll('[role="button"]')).find((row) => row.textContent?.includes('Locked current'));
        await act(async () => {
        ...
        expect(sessionActions.renameSession).toHaveBeenCalledWith('Renamed locked current');
        expect(renameSessionCatalog).toHaveBeenCalledWith('/tmp/other', 'locked-current', 'Renamed locked current');
    });
    it('does not assign a created group after its locked workspace unlocks', async () => {
        let resolveCreate;
        const createSessionGroup = vi.fn(() => new Promise((resolve) => {
        ...
        const groupItem = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((item) => item.textContent?.includes('Group'));
        ...
        const createGroup = Array.from(document.body.querySelectorAll('button[role="menuitem"]')).find((button) => button.textContent?.includes('Create group'));
        expect(createGroup).toBeDefined();
        await act(async () => {
            click(createGroup);
            await Promise.resolve();
        });
        const groupName = document.body.querySelector('#session-group-name');
        ...
        expect(primaryOrganization).not.toHaveBeenCalled();
        expect(document.body.querySelector('#session-group-name')).toBeNull();
        expect(onError).toHaveBeenCalledWith(expect.any(Error), 'Group created, but failed to move session into it');
        await act(async () => {
            groupForm.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }));
            await Promise.resolve();
        });
    ...
    it('routes locked secondary delete, archive, color, and pinned mutations only to its client', async () => {
        ...
        const makeClient = (cwd, actions) => ({
            listWorkspaceSessions: vi
                .fn()
                .mockImplementation((options) => cwd !== '/tmp/other'
                ? []
        ...
        const blue = Array.from(container.querySelectorAll('[role="menuitemradio"]')).find((button) => button.textContent?.includes('Blue'));
        ...
        await selectSessionMenuItem('Locked color', 'Group');
        const namedGroup = Array.from(container.querySelectorAll('[role="menuitemradio"]')).find((button) => button.textContent?.includes('Secondary group'));
    ...
    it('requires workspace_qualified_rest_core for locked secondary destructive and organization controls', async () => {
        ...
        expect(inlineSessionAction('Without rest capability', 'Pin')).toBeUndefined();
        expect(inlineSessionAction('Without rest capability', 'Archive')).toBeUndefined();
        expect(inlineSessionAction('Without rest capability', 'Delete')).toBeUndefined();
    ...
    it('keeps a trusted locked workspace New Task available without session organization', async () => {
        connection.capabilities = {
            ...capabilities,
            features: capabilities.features.filter((feature) => feature !== 'session_organization'),
        };
        const onNewSession = vi.fn(() => true);
        renderSidebar({ lockedWorkspaceCwd: '/tmp/other', onNewSession });
        await expandWorkspace('other');
        expect(container.querySelector('button[aria-label="Create session group"]')).toBeNull();
        const newTaskButtons = Array.from(container.querySelectorAll('button[aria-label="New task"]'));
    ...
    it('applies items and inlineItems consistently to locked normal, pinned, and archived rows', async () => {
        ...
        expect(container.querySelector('button[aria-label="Create group"]')).not.toBeNull();
        ...
        expect(archivedItems).toEqual(expect.arrayContaining([
            'Details',
            'Export conversation record',
            'Restore',
            'Delete',
    ...
    it('renders locked normal, pinned, and archived rows action-free when no items are configured', async () => {
        ...
        expect(container.querySelector('button[aria-label="Create group"]')).toBeNull();
        expect(container.querySelector('button[aria-label="Rename group"]')).toBeNull();
        expect(container.querySelector('button[aria-label="Delete group"]')).toBeNull();
        await expandArchived();
        await act(async () => {
            await Promise.resolve();
            await Promise.resolve();
    ...
    it('gates primary group header controls on the live organization policy', async () => {
        ...
        expect(container.querySelector('button[aria-label="Rename group"]')).not.toBeNull();
        expect(container.querySelector('button[aria-label="Delete group"]')).not.toBeNull();
        ...
        connection.capabilities = {
            ...
            features: organizedCapabilities.features.filter((feature) => feature !== 'session_organization'),
        };
    ...
    it('keeps equal-id active export and current state isolated by workspace cwd', async () => {
        ...
        const secondaryExport = vi.fn(() => new Promise((resolve) => {
        ...
        workspace.client.workspaceByCwd.mockImplementation((cwd) => ({
            listWorkspaceSessions: async () => cwd === '/tmp/other'
                ? [
                    {
                        sessionId: 'shared-active',
                        workspaceCwd: cwd,
        ...
        await selectSessionMenuItem('Secondary shared active', 'Export conversation record');
        expect(secondaryExport).toHaveBeenCalledOnce();
        connection.workspaceCwd = '/tmp/project';
        renderSidebar();
        const primaryRow = Array.from(container.querySelectorAll('[role="button"]')).find((row) => row.textContent?.includes('Primary shared active'));
        expect(primaryRow?.getAttribute('aria-current')).toBe('page');
        await selectSessionMenuItem('Primary shared active', 'Export conversation record');
        await act(async () => {
    ...
    it('keeps primary active export on the primary action under session_export only', async () => {
        ...
        active.exportSession.mockResolvedValue(exportResult);
        const secondaryExport = vi.fn().mockResolvedValue(exportResult);
        workspace.client.workspaceByCwd.mockImplementation((_cwd) => ({
    ...
    it('does not infer secondary active export from session_export', async () => {
        ...
        renderSidebar({ lockedWorkspaceCwd: '/tmp/other' });
        await expandWorkspace('other');
        expect(inlineSessionAction('Secondary legacy export', 'Export')).toBeUndefined();
        ...
        expect(Array.from(document.body.querySelectorAll('[role="menuitem"]')).some((item) => item.textContent?.includes('Export'))).toBe(false);
    });
    it('applies configured, trusted archived actions without a stale-cwd fallback', async () => {
        ...
        expect(archived.unarchiveSession).toHaveBeenCalledWith('primary-archived-controlled');
        ...
        const staleMenuItems = Array.from(document.body.querySelectorAll('[role="menuitem"]')).map((item) => item.textContent);
    ...
    it('clears a secondary delete candidate after its workspace disappears', async () => {
        ...
        workspace.client.workspaceByCwd.mockImplementation((cwd) => ({
            ...
            archiveSessionsData,
            unarchiveSessionsData,
            deleteSessionsData: cwd === '/tmp/other' ? secondaryDelete : deleteSessionsData,
            updateSessionOrganization,
            exportSession,
            exportArchivedSession,
        }));
        ...
        const catalogWithoutSecondary = {
            ...capabilities,
            workspaces: capabilities.workspaces.filter((entry) => entry.cwd !== '/tmp/other'),
        };
    ...
    it('keeps legacy primary actions when capabilities omit the workspace catalog', async () => {
        ...
        expect(inlineSessionAction('Legacy primary', 'Export conversation record')).toBeDefined();
        expect(inlineSessionAction('Legacy primary', 'Delete')?.disabled).toBe(false);
    ...
    it('fails closed for an explicit primary cwd that disappears from the catalog', async () => {
        ...
        const mutationActions = {
            items: ['rename', 'pin', 'group', 'archive', 'export', 'delete'],
            inlineItems: ['rename', 'pin', 'archive', 'export', 'delete'],
        };
        ...
        expect(inlineSessionAction('Explicit primary stale', 'Pin')).toBeUndefined();
        expect(inlineSessionAction('Explicit primary stale', 'Delete')).toBeUndefined();
        expect(archiveButtonFor('Explicit primary stale')).toBeUndefined();
        expect(inlineSessionAction('Explicit primary stale', 'Export')).toBeUndefined();
        expect(inlineSessionAction('Implicit primary fallback', 'Delete')).toBeDefined();
    ...
    it('treats a no-cwd primary row as current when the connection cwd is omitted', async () => {
        ...
        useWorkspaceSessionCatalog(async (cwd, options) => cwd === '/tmp/other' && options?.archiveState === 'active'
            ? [
                {
                    sessionId: 'current-no-cwd',
                    workspaceCwd: cwd,
    ...
    it('deduplicates pending no-cwd primary renames and clears the busy identity afterward', async () => {
        ...
        await expandWorkspace('project');
        expect(inlineSessionAction('Rename no-cwd primary', 'Rename')?.disabled).toBe(false);
        await act(async () => {
    ...
    it('honors a missing rename item for double-click editing', async () => {
        ...
        const row = Array.from(container.querySelectorAll('[role="button"]')).find((candidate) => candidate.textContent?.includes('Current primary'));
    ...
    it('allows pin on an unlocked secondary workspace but keeps destructive actions restricted', async () => {
        ...
        useWorkspaceSessionCatalog(async (cwd, options) => cwd === '/tmp/other' && options?.group === 'pinned'
            ? [
                {
    ...
    it('allows organization but keeps destructive actions conservative for unlocked secondary sessions', async () => {
        ...
        const secondaryCreateGroup = Array.from(container.querySelectorAll('button[aria-label="Create group"]')).find((button) => button
            .closest('[class*="headerRow"]')
    ...
    it('does not carry an active rename edit across equal session ids in another workspace', async () => {
        ...
        const secondaryRow = Array.from(container.querySelectorAll('[role="button"]')).find((row) => row.textContent?.includes('Secondary shared'));
        expect(secondaryRow).toBeDefined();
        await act(async () => {
            secondaryRow?.dispatchEvent(new MouseEvent('dblclick', { bubbles: true }));
            await Promise.resolve();
        });
    ...
    it('shows only the locked workspace without registration controls', () => {
        ...
        expect(container.querySelector('button[aria-label="Add workspace"]')).toBeNull();
    ...
    it('uses custom workspace row content only when the workspace is locked', async () => {
        const render = vi.fn((ws) => (_jsxs("span", { "data-testid": "custom-workspace", children: ["Custom ", ws.cwd] })));
        ...
        expect(render).toHaveBeenLastCalledWith(expect.objectContaining({ id: 'secondary', cwd: '/tmp/other' }), { expanded: false });
        expect(container.querySelector('[data-testid="custom-workspace"]')?.textContent).toBe('Custom /tmp/other');
        expect(container
            .querySelector('[data-testid="custom-workspace"]')
            ?.closest('button')
            ?.parentElement?.querySelectorAll('button')).toHaveLength(1);
        expect(container.textContent).not.toContain('project');
        await act(async () => {
            click(container
                .querySelector('[data-testid="custom-workspace"]')
        ...
        expect(render).toHaveBeenLastCalledWith(expect.objectContaining({ id: 'secondary', cwd: '/tmp/other' }), { expanded: true });
    });
    ...
    it('exposes removal for an untrusted removable workspace', () => {
        ...
        act(() => click(trigger));
        const item = document.body.querySelector('[aria-label="Remove workspace: /tmp/danger"]');
    ...
    it('shows activity and blocks force for the current session workspace', async () => {
        ...
        connection.workspaceCwd = '/tmp/other';
        workspaceActions.removeWorkspace.mockRejectedValueOnce(new DaemonHttpError(409, {
            code: 'workspace_busy',
            activity: {
                sessions: 1,
                activePrompts: 1,
        ...
        expect(document.body.textContent).toContain('Switch to another workspace or close the current session');
...
describe('WebShellSidebar non-primary archive', () => {
    it('archives a trusted secondary session and reconciles its workspace catalogs', async () => {
        ...
        const archiveItem = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((item) => item.textContent === 'Archive');
        ...
        expect(listWorkspaceSessions.mock.calls.filter(([cwd, options]) => cwd === '/tmp/other' && options?.archiveState === 'active').length).toBeGreaterThan(1);
        expect(listWorkspaceSessions.mock.calls.some(([cwd, options]) => cwd === '/tmp/other' && options?.archiveState === 'archived')).toBe(true);
    });
    it('surfaces a partial restore error and reconciles every catalog', async () => {
        ...
        await expandWorkspace('other');
        await expandArchived();
        const moreButton = Array.from(container.querySelectorAll('button[aria-label="More actions"]')).find((button) => button
            .closest('[class*="sessionRow"]')
            ?.textContent?.includes('Secondary archived'));
        expect(moreButton).toBeDefined();
        act(() => click(moreButton));
        const restoreItem = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((item) => item.textContent?.includes('Restore'));
        ...
        expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: 'scheduled task restore failed' }), 'Failed to restore session');
        expect(active.reload).not.toHaveBeenCalled();
        expect(archived.reload).not.toHaveBeenCalled();
        expect(listWorkspaceSessions.mock.calls.filter(([cwd, options]) => cwd === '/tmp/other' && options?.archiveState === 'active').length).toBeGreaterThan(1);
    });
    it('surfaces secondary batch errors and still reconciles catalogs', async () => {
        ...
        expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: 'agent close failed' }), 'Failed to archive session');
        expect(active.reload).not.toHaveBeenCalled();
        expect(archived.reload).not.toHaveBeenCalled();
        expect(listWorkspaceSessions.mock.calls.filter(([cwd, options]) => cwd === '/tmp/other' && options?.archiveState === 'active').length).toBeGreaterThan(1);
    });
    it('allows primary archive but gates secondary archive without the plural capability', async () => {
        ...
        await expandArchived();
        expect(listWorkspaceSessions.mock.calls.some(([, options]) => options?.archiveState === 'archived')).toBe(false);
    });
    ...
    it('keeps a locked untrusted secondary active row read-only', async () => {
        ...
        useWorkspaceSessionCatalog(async (cwd, options) => cwd === '/tmp/danger' && options?.archiveState === 'active'
            ? [
                {
    ...
    it('keeps an untrusted primary active row read-only and action-free', async () => {
        ...
        connection.capabilities = {
            ...
            workspaces: capabilities.workspaces.map((entry) => entry.primary ? { ...entry, trusted: false } : entry),
        };
        workspace.capabilities = connection.capabilities;
        active.sessions.push({
        ...
        expect(inlineSessionAction('Untrusted primary active', 'Pin')).toBeUndefined();
        expect(inlineSessionAction('Untrusted primary active', 'Delete')).toBeUndefined();
        expect(inlineSessionAction('Untrusted primary active', 'Export conversation record')).toBeUndefined();
        expect(inlineSessionAction('Untrusted primary active', 'Rename')).toBeUndefined();
    ...
    it('surfaces a locked-secondary delete item error without falling back to primary', async () => {
        ...
        expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: 'daemon denied delete' }), 'Failed to delete session');
        expect(active.deleteSession).not.toHaveBeenCalled();
        expect(active.reload).not.toHaveBeenCalled();
        expect(archived.reload).not.toHaveBeenCalled();
        expect(inlineSessionAction('Locked delete error', 'Delete')?.disabled).toBe(false);
    });
    ...
    it('keeps equal-id archive busy state scoped to its workspace', async () => {
        let finishArchive;
        archiveSessionsData.mockReturnValue(new Promise((resolve) => {
...
describe('WebShellSidebar goals entry', () => {
    it('renders the Goals footer button and invokes onOpenGoals', () => {
        ...
        const button = container.querySelector('button[aria-label="Goals"]');
...
describe('WebShellSidebar primary workspace header', () => {
    it('does not tag the primary workspace with a redundant "Primary" badge', () => {
        ...
        renderSidebar();
        const primaryBadges = Array.from(container.querySelectorAll('span')).filter((el) => el.textContent === 'Primary');
...
describe('WebShellSidebar session source switch', () => {
    ...
    it('preserves channel completion state while the tasks source is active', async () => {
        ...
        useSessions.mockImplementation((options) => {
            if (options?.archiveState === 'archived')
                return archived;
        ...
        renderSidebar();
        const row = Array.from(container.querySelectorAll('[role="button"]')).find((candidate) => candidate.textContent?.includes('Channel session'));
        expect(row).toBeDefined();
        ...
        const restoredRow = Array.from(container.querySelectorAll('[role="button"]')).find((candidate) => candidate.textContent?.includes('Channel session'));
        expect(restoredRow).toBeDefined();
        expect(restoredRow.querySelector('[class*="sessionStatusDot"]')).not.toBeNull();
    });
    it('applies the source switch to the archived list', async () => {
        ...
        expect(useSessions.mock.calls.findLast(([options]) => options?.archiveState === 'archived')?.[0]?.sourceType).toBe('channel');
    ...
    it('switches primary and workspace-qualified lists from tasks to channels', async () => {
        renderSidebar();
        const sourceTabs = Array.from(container.querySelectorAll('[role="tab"]'));
        const tasksTab = sourceTabs.find((button) => button.textContent?.trim() === 'Tasks');
        const channelsTab = sourceTabs.find((button) => button.textContent?.trim() === 'Channels');
        expect(tasksTab?.getAttribute('data-state')).toBe('active');
        expect(channelsTab).toBeDefined();
        expect(useSessions.mock.calls.find(([options]) => options?.archiveState === 'active' && options.group !== 'pinned')?.[0]?.sourceType).toBe('default');
        await switchSessionSource('Channels');
        expect(channelsTab?.getAttribute('data-state')).toBe('active');
        expect(useSessions.mock.calls.findLast(([options]) => options?.archiveState === 'active' && options.group !== 'pinned')?.[0]?.sourceType).toBe('channel');
        await expandWorkspace('other');
        expect(listWorkspaceSessions.mock.calls.some(([options]) => options?.sourceType === 'channel')).toBe(true);
    });
    it('hides the switch and keeps legacy session requests unfiltered', async () => {
        connection.capabilities = {
            ...capabilities,
            features: capabilities.features.filter((feature) => feature !== 'session_source_metadata'),
        };
        renderSidebar();
        expect(container.querySelector('[aria-label="Session source"]')).toBeNull();
        expect(useSessions.mock.calls.every(([options]) => options?.sourceType === undefined)).toBe(true);
        await expandWorkspace('other');
        expect(listWorkspaceSessions).toHaveBeenCalled();
        expect(listWorkspaceSessions.mock.calls.every(([options]) => options?.sourceType === undefined)).toBe(true);
    });
    it('polls channel sessions on the active-session interval', async () => {
        ...
        renderSidebar();
        await ensureWorkspaceExpanded('project');
        expect(useSessionCatalogPollingSpy).not.toHaveBeenCalledWith(expect.anything(), expect.anything(), 2_000);
        expect(setIntervalSpy.mock.calls.some(([, timeout]) => timeout === 2_000)).toBe(false);
        ...
        expect(useSessionCatalogPollingSpy).toHaveBeenCalledWith(workspace.client, expect.anything(), 2_000);
        // ...and the channel catalog rides its own interval at the same cadence.
        const activePoll = setIntervalSpy.mock.calls.findLast(([, timeout]) => timeout === 2_000);
    ...
    it('backs off the channel catalog poll while the channels hook errors', async () => {
        ...
        expect(setIntervalSpy.mock.calls.some(([, timeout]) => timeout === 2_000)).toBe(false);
        expect(setIntervalSpy.mock.calls.some(([, timeout]) => timeout === 30_000)).toBe(true);
    });
    ...
    it('groups channel sessions by platform type and toggles each group', async () => {
        ...
        const dingTalkGroup = container.querySelector('section[aria-label="DingTalk"]');
        ...
        expect(container.querySelector('section[aria-label="Feishu"]')?.textContent).toContain('Feishu one');
        expect(container.querySelector('section[aria-label="Other channels"]')
            ?.textContent).toContain('Legacy channel');
        const toggle = dingTalkGroup?.querySelector('button[aria-expanded="true"]');
    ...
    it('starts channel sections expanded on the first Channels visit with organization enabled', async () => {
        ...
        expect(dingTalkGroup.querySelector('button[aria-expanded="true"]')).not.toBeNull();
        expect(window.localStorage.getItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY) ?? '').not.toContain('channel-type:');
    });
    it('keeps channel sections expanded when the catalog settles before the sessions page', async () => {
        ...
        expect(container.querySelector('section[aria-label="DingTalk"]')).toBeNull();
        expect(window.localStorage.getItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY) ?? '').not.toContain('channel-type:');
        channelPage = [
            {
                sessionId: 'ding-one-session',
    ...
    it('does not register the first channel catalog against a search filter', async () => {
        ...
        const feishuGroup = container.querySelector('section[aria-label="Feishu"]');
        expect(feishuGroup).not.toBeNull();
        expect(feishuGroup.querySelector('button[aria-expanded="true"]')).not.toBeNull();
        expect(window.localStorage.getItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY) ?? '').not.toContain('channel-type:');
    });
    it('does not reconcile channel sections against the previous source page', async () => {
        ...
        for (const label of ['DingTalk', 'Feishu']) {
            const group = container.querySelector(`section[aria-label="${label}"]`);
            expect(group).not.toBeNull();
            expect(group.querySelector('button[aria-expanded="true"]')).not.toBeNull();
        }
        expect(window.localStorage.getItem(COLLAPSED_SESSION_SECTIONS_STORAGE_KEY) ?? '').not.toContain('channel-type:');
    });
});
describe('WebShellSidebar session list notices', () => {
    ...
    it('offers a retry when the first page load failed', async () => {
        ...
        const retry = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.includes('Failed to load sessions'));
...
describe('WebShellSidebar Live group', () => {
    it('shows Live sessions without exposing the backing Conversations workspace', async () => {
        ...
        useWorkspaceSessionCatalog(async (cwd) => cwd === liveWorkspace.cwd
        ...
        const liveToggle = Array.from(container.querySelectorAll('button')).find((button) => button.textContent?.includes('Live'));
        expect(liveToggle).toBeDefined();
        expect(liveToggle?.parentElement?.querySelector('[aria-label="New task"]')).toBeNull();
        await expandWorkspace('Live');
        expect(container.textContent).toContain('Voice check');
        expect(listWorkspaceSessions).toHaveBeenCalledWith(liveWorkspace.cwd, expect.objectContaining({ archiveState: 'active' }));
    });
});
describe('WebShellSidebar archived session export', () => {
    ...
    it('hides export for an untrusted archived workspace', async () => {
        ...
        const archivedItems = Array.from(document.body.querySelectorAll('[role="menuitem"]')).map((item) => item.textContent);
    ...
    it('exports equal-id archived rows through their owning workspaces', async () => {
        ...
        let releasePrimary;
        const primaryExport = vi.fn(() => new Promise((resolve) => {
            releasePrimary = resolve;
        }));
        const secondaryExport = vi.fn().mockResolvedValue(exportResult);
        workspace.client.workspaceByCwd.mockImplementation((cwd) => ({
            listWorkspaceSessions: async (options) => cwd === '/tmp/other' && options?.archiveState === 'archived'
                ? [
                    {
                        sessionId: 'same-session',
            ...
            archiveSessionsData,
            unarchiveSessionsData,
            exportArchivedSession: cwd === '/tmp/project' ? primaryExport : secondaryExport,
        }));
        renderSidebar();
        await expandArchived();
        ...
        try {
            ...
            const secondaryItem = Array.from(document.body.querySelectorAll('[role="menuitem"]')).find((item) => item.textContent?.includes('Export'));
...
//# sourceMappingURL=WebShellSidebar.workspace-removal.test.js.map