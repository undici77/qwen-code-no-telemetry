...
vi.mock('../ui/dropdown-menu', () => {
    function DropdownMenuSub({ children, open = false, onOpenChange = () => { }, }) {
    ...
    const DropdownMenuSubTrigger = forwardRef(function DropdownMenuSubTrigger({ onClick, ...props }, ref) {
        return (_jsx("button", { ...props, ref: ref, type: "button", onClick: (event) => {
                onClick?.(event);
                const nextOpen = !dropdownMenu.open;
                dropdownMenu.onOpenChange?.(nextOpen);
            } }));
    });
    function DropdownMenuSubContent({ children, avoidCollisions, collisionBoundary, collisionPadding, updatePositionStrategy, className, }) {
        ...
        return (_jsx("div", { "data-slot": "dropdown-menu-sub-content", className: className, children: children }));
    }
...
const session = {
    sessionId: 'session-id-that-remains-complete-when-the-visible-value-is-truncated',
...
function render(sessionOverride = session) {
    act(() => {
        root.render(_jsx(I18nProvider, { language: "en", children: _jsx("div", { ref: (element) => {
                ...
                }, "data-web-shell-root": true, children: _jsx("aside", { children: _jsx(SessionDetailsSubmenu, { session: sessionOverride, label: "Long running session", completedUnread: true, onError: onError, getCollisionBoundary: getCollisionBoundary }) }) }) }));
    });
}
function getMenuItem(label) {
    const item = portalRoot.querySelector(`[role="menuitem"][aria-label="${label}"]`);
...
async function openDetails() {
    const details = Array.from(portalRoot.querySelectorAll('button')).find((button) => button.textContent?.includes('Details'));
...
describe('SessionDetailsSubmenu', () => {
    ...
    it('reports an unavailable clipboard through the existing error path', async () => {
        ...
        await selectCopy();
        expect(getStatus().textContent).toBe('');
        expect(onError).toHaveBeenCalledWith(expect.objectContaining({ message: 'Clipboard API is unavailable' }), 'Failed to copy session ID');
    });
});
