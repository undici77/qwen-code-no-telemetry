...
export function groupSessionsByChannelType(sessions, catalog, instances, otherLabel) {
    const labels = new Map(catalog.map((descriptor) => [descriptor.type, descriptor.displayName]));
    ...
    for (const session of sessions) {
        ...
        const instance = session.sourceId && Object.hasOwn(instances, session.sourceId)
            ? instances[session.sourceId]
            : undefined;
        const configuredType = instance?.config['type'];
        const type = typeof configuredType === 'string'
