...
export type WebShellSidebarFooterItem = 'settings' | 'version' | 'theme' | 'sessionsOverview' | 'splitView' | 'daemonStatus' | 'collapse';
...
export type WebShellSidebarPrimaryNavItem = 'newTask' | 'plugins' | 'channels' | 'scheduledTasks' | 'goals';
...
export type WebShellSidebarSessionActionItem = 'details' | 'rename' | 'group' | 'export' | 'delete' | 'pin' | 'archive';
/** Subset of action items that have working inline (hover-button) handlers. */
export type WebShellSidebarSessionInlineActionItem = 'pin' | 'archive' | 'rename' | 'export' | 'delete';
...
interface WebShellSidebarProps {
    ...
    onSessionRenameConfirmed?: (workspaceCwd: string, sessionId: string, displayName: string) => void;
