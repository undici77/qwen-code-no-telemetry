...
describe('buildSessionPathname', () => {
    ...
    it('preserves a sub-path deployment base', () => {
        expect(buildSessionPathname('/app/session/old', 'new')).toBe('/app/session/new');
    });
...
describe('build/parse round-trip', () => {
    it('reads back the written session id', () => {
        for (const base of ['/', '/app', '/app/', '/app/session/old']) {
            expect(parseSessionId(buildSessionPathname(base, 'real-id'))).toBe('real-id');
        }
    });
...
//# sourceMappingURL=sessionPath.test.js.map