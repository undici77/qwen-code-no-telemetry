...
describe('copyFromLastAssistantMessage', () => {
    it('returns an info message when there is no assistant output', async () => {
        ...
        const result = await copyFromLastAssistantMessage([user('hello')], '', writeText);
    ...
    it('copies the last assistant text by default', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant('first'), user('again'), assistant('second')], '', writeText);
    ...
    it('copies the last fenced code block with /copy code', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant(content)], 'code', writeText);
    ...
    it('copies a numbered fenced code block with /copy code 2', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant(content)], 'code 2', writeText);
    ...
    it('copies a numbered fenced code block with a bare index', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant(content)], '2', writeText);
        expect(writeText).toHaveBeenCalledWith('{"second": true}');
        expect(result.message).toBe('Code block 2 copied to the clipboard');
    });
    it('reports a missing block for a bare index that is out of range', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant(content)], '9', writeText);
        expect(writeText).not.toHaveBeenCalled();
        expect(result.message).toBe('No matching code block found in the last AI output.');
    });
    it('copies the last matching language code block', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant(content)], 'code mermaid', writeText);
    ...
    it('copies a numbered matching language block without the code token', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant(content)], 'mermaid 1', writeText);
    ...
    it('copies display LaTeX blocks', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant(content)], 'latex 1', writeText);
    ...
    it('copies inline LaTeX expressions', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant(content)], 'inline-latex 1', writeText);
        expect(writeText).toHaveBeenCalledWith('x = \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}');
        expect(result.message).toBe('Inline LaTeX expression 1 copied to the clipboard');
    });
    ...
    it('reports missing code blocks', async () => {
        const writeText = vi.fn();
        const result = await copyFromLastAssistantMessage([assistant('No code here.')], 'code', writeText);
        expect(result).toEqual({
            status: 'info',
    ...
    it('reports clipboard copy failures', async () => {
        ...
        const result = await copyFromLastAssistantMessage([assistant('copy me')], '', writeText);
        expect(result).toEqual({
            status: 'error',
