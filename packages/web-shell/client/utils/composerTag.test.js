...
import { createInputAnnotationsFromComposerTags, getComposerTagIconUrl, getComposerTagViewModel, isBuiltinComposerTagIconUrl, parseUserMessageContentSafely, splitComposerTagContentByAnnotations, } from './composerTag';
...
describe('composer tag icon URLs', () => {
    it('uses registered icons for custom tag kinds', () => {
        expect(getComposerTagIconUrl('table', { table: '/icons/table.svg' })).toBe('/icons/table.svg');
    });
    ...
    it('recognizes only exact built-in tag icon URLs', () => {
        for (const kind of ['extension', 'file', 'mcp', 'skill']) {
        ...
        expect(isBuiltinComposerTagIconUrl('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" />')).toBe(false);
        expect(isBuiltinComposerTagIconUrl('javascript:alert(1)')).toBe(false);
    });
    it('ignores inherited object properties', () => {
        const icons = Object.create({ table: '/icons/table.svg' });
...
describe('getComposerTagViewModel', () => {
    it('returns display fields for custom tags', () => {
        expect(getComposerTagViewModel({
            id: 'custom:1',
            label: '  Dataset  ',
            value: '  users.csv  ',
        })).toEqual({
    ...
    it('uses custom icon maps when provided', () => {
        expect(getComposerTagViewModel({
            ...
            kind: 'file',
            value: 'src/index.ts',
        }, { file: '/custom-file.svg' }).iconUrl).toBe('/custom-file.svg');
    });
...
describe('parseUserMessageContentSafely', () => {
    it('rejects an empty parser result', () => {
        expect(parseUserMessageContentSafely('original', () => [], 'test warning')).toBeNull();
    });
    it('rejects a non-array parser result', () => {
        const parser = (() => 'not an array');
        expect(parseUserMessageContentSafely('original', parser, 'test warning')).toBeNull();
    });
    it('rejects a text part with non-string text', () => {
        const parser = (() => [{ type: 'text', text: 1 }]);
        expect(parseUserMessageContentSafely('original', parser, 'test warning')).toBeNull();
    });
    ...
    it('allows valid non-round-tripping parts when source preservation is omitted', () => {
        const parts = parseUserMessageContentSafely('original', () => [{ type: 'text', text: 'rewritten' }], 'test warning');
    ...
    it('rejects valid non-round-tripping parts when source preservation is required', () => {
        const parts = parseUserMessageContentSafely('original', () => [{ type: 'text', text: 'rewritten' }], 'test warning', { requireSourcePreservation: true });
        expect(parts).toBeNull();
    });
});
describe('composer tag input annotations', () => {
    it('creates reference annotations using ranges from final prompt text', () => {
        expect(createInputAnnotationsFromComposerTags('@dataset:users\n\nshow rows', [
            {
                id: 'dataset:users',
                kind: 'dataset',
                label: 'Dataset',
    ...
    it('creates annotations for extensionless file references', () => {
        expect(createInputAnnotationsFromComposerTags('@Makefile @LICENSE @src/Makefile', [
            {
                id: 'file:@Makefile',
                kind: 'file',
                value: 'Makefile',
    ...
    it('returns no annotations for empty tags', () => {
        expect(createInputAnnotationsFromComposerTags('show @Makefile', [])).toEqual([]);
    });
    it('skips tags whose serialized text is absent from the prompt', () => {
        expect(createInputAnnotationsFromComposerTags('show @Makefile', [
            {
                id: 'file:@LICENSE',
                kind: 'file',
                value: 'LICENSE',
    ...
    it('matches repeated serialized references in order', () => {
        expect(createInputAnnotationsFromComposerTags('@file @file', [
            {
                id: 'file:first',
                kind: 'file',
                value: 'first',
    ...
    it('uses annotations for custom provider references', () => {
        expect(splitComposerTagContentByAnnotations('open @dataset:users now', [
            {
                type: 'reference',
                start: 5,
                end: 19,
    ...
    it('uses annotations for extensionless file references', () => {
        ...
        expect(splitComposerTagContentByAnnotations(content, [
...
//# sourceMappingURL=composerTag.test.js.map