...
import { GOAL_CLEAR_KEYWORDS, goalArgOf, isGoalClearCommand, isGoalClearKeyword, } from './goalCondition';
describe('goalArgOf', () => {
...
describe('the CLI is the authority on the clear keywords', () => {
    ...
    it('agrees with goalCommand.ts on the clear keywords', () => {
        ...
        const literal = /const CLEAR_KEYWORDS = new Set\(\[([^\]]*)\]\)/.exec(source);
        expect(literal, 'CLEAR_KEYWORDS literal not found in goalCommand.ts').not.toBeNull();
        const cliKeywords = [...literal[1].matchAll(/'([^']+)'/g)].map((m) => m[1]);
        expect(cliKeywords.length).toBeGreaterThan(0);
        expect([...cliKeywords].sort()).toEqual([...GOAL_CLEAR_KEYWORDS].sort());
    });
});
//# sourceMappingURL=goalCondition.test.js.map