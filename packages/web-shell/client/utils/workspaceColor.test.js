...
describe('workspaceAccentColor', () => {
    ...
    it('returns a stable color per cwd and distinct colors for different workspaces', () => {
        ...
        expect(workspaceAccentColor('/work/web', caps)).toBe(workspaceAccentColor('/work/web', caps));
        expect(workspaceAccentColor('/work/web', caps)).not.toBe(workspaceAccentColor('/work/api', caps));
    });
    it('cycles the six-color palette once there are more workspaces', () => {
        ...
        expect(workspaceAccentColor('/w6', caps)).toBe(workspaceAccentColor('/w0', caps));
    });
    ...
    it('returns undefined for a missing cwd so callers can skip the accent', () => {
        expect(workspaceAccentColor(undefined, capabilities(['/w0']))).toBeUndefined();
        expect(workspaceAccentColor('', capabilities(['/w0']))).toBeUndefined();
    });
});
//# sourceMappingURL=workspaceColor.test.js.map