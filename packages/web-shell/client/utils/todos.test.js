...
import { computeTodoDetails, computeTodoTimeline, extractTodoStats, extractTodosFromToolCall, getAgentToolsForPlan, getFloatingTodos, getActiveTodosForPlanRevision, getTodoStatusIcon, getTodoWindow, isExitPlanApprovalRequest, isTodoWriteToolName, todoDetailSignature, todoStateKey, todoTimelineSignature, } from './todos';
...
function planMessage(id, todos) {
    return { id, role: 'plan', todos };
}
function todoWriteMessage(id, todos, stats, planId) {
    const tool = {
        callId: `call-${id}`,
        toolName: 'todo_write',
        status: 'completed',
...
function toolMessage(id, startTime, endTime, toolName = 'read') {
    return {
        id,
        role: 'tool_group',
        tools: [
...
describe('getFloatingTodos', () => {
    it('returns the empty state when no messages carry todos', () => {
        expect(getFloatingTodos([userMessage('u1'), assistantMessage('a1')])).toEqual({
            todos: [],
            planId: null,
            allCompleted: false,
...
describe('getActiveTodosForPlanRevision', () => {
    ...
    it('rejects missing and mismatched revisions but preserves terminal ones', () => {
        ...
        expect(getActiveTodosForPlanRevision([todoWriteMessage('m1', [todo('1', 'pending')], undefined, 'plan-1')], undefined)).toEqual([]);
        expect(getActiveTodosForPlanRevision([todoWriteMessage('m1', [todo('1', 'pending')], undefined, 'plan-1')], { planId: 'plan-other', sourceCallId: 'call-m1' })).toEqual([]);
        const completed = [todo('1', 'completed')];
        expect(getActiveTodosForPlanRevision([todoWriteMessage('done', completed, undefined, 'plan-1')], { planId: 'plan-1', sourceCallId: 'call-done' })).toEqual(completed);
    });
});
describe('getAgentToolsForPlan', () => {
    const planUpdate = (messageId, planId) => ({
        id: messageId,
        role: 'tool_group',
        tools: [
            {
    ...
    const completedPlanUpdate = (messageId, planId) => ({
        id: messageId,
        role: 'tool_group',
        tools: [
            {
    ...
    const clearedPlanUpdate = (messageId, planId) => ({
        id: messageId,
        role: 'tool_group',
        tools: [
            {
    ...
    const agentUpdate = (messageId, callId, parentToolCallId) => ({
        id: messageId,
        role: 'tool_group',
        tools: [
            {
    ...
    it('collects only top-level Agent calls from the current stable plan', () => {
        ...
        expect(getAgentToolsForPlan(messages, {
            planId: 'new',
            sourceMessageId: 'new-plan',
        }).map((tool) => tool.callId)).toEqual(['new-agent']);
        expect(getAgentToolsForPlan(messages, {
            planId: 'old',
            sourceMessageId: 'old-plan',
        }).map((tool) => tool.callId)).toEqual(['old-agent']);
    });
    it('keeps collecting agents after an update to the same plan', () => {
        ...
        }).map((tool) => tool.callId)).toEqual(['first-agent', 'second-agent']);
    });
    it('stops collecting as soon as a plan is completed', () => {
        ...
        expect(getAgentToolsForPlan(messages, {
            planId: 'stable',
            sourceMessageId: 'plan-start',
        }).map((tool) => tool.callId)).toEqual(['plan-agent']);
    });
    ...
    it('stops an ACP plan at the next plan message', () => {
        ...
        }).map((tool) => tool.callId)).toEqual(['first-agent']);
    });
    it('returns immediately when there is no active plan source', () => {
        expect(getAgentToolsForPlan([agentUpdate('agent', 'agent')], {
            planId: null,
            sourceMessageId: null,
        })).toEqual([]);
    });
...
describe('isTodoWriteToolName', () => {
    it.each(['todo_write', 'todowrite', 'TodoWrite', 'TODO_WRITE'])('matches %s', (name) => {
...
describe('isExitPlanApprovalRequest', () => {
    it('matches a switch_mode exit_plan_mode request', () => {
        expect(isExitPlanApprovalRequest({
...
describe('extractTodosFromToolCall', () => {
    ...
    it('reads todos from args', () => {
        const todos = extractTodosFromToolCall(toolCall({ args: { todos: [item('1', 'A', 'pending')] } }));
        expect(todos).toEqual([{ id: '1', content: 'A', status: 'pending' }]);
    });
    it('reads todos from rawOutput.todos', () => {
        const todos = extractTodosFromToolCall(toolCall({ rawOutput: { todos: [item('1', 'A', 'in_progress')] } }));
        expect(todos?.map((t) => t.status)).toEqual(['in_progress']);
    });
    it('reads todos from rawOutput.entries', () => {
        const todos = extractTodosFromToolCall(toolCall({ rawOutput: { entries: [item('1', 'A', 'completed')] } }));
    ...
    it('does not treat an empty output from another tool as a plan clear', () => {
        expect(extractTodosFromToolCall(toolCall({
...
describe('getTodoWindow', () => {
    const statuses = (list) => list.map((status, i) => todo(String(i + 1), status));
...
function stats(promptTokens, cachedTokens, candidateTokens, apiTimeMs) {
...
describe('extractTodoStats', () => {
    ...
    it('reads a complete stats snapshot from rawOutput', () => {
        expect(extractTodoStats(toolCall({
            stats: {
                promptTokens: 1,
                cachedTokens: 2,
                candidateTokens: 3,
...
describe('computeTodoDetails', () => {
    ...
    it('derives token and API resources from the diff of the boundary snapshots', () => {
        const details = computeTodoDetails([
            at(todoWriteMessage('m1', [todo('1', 'in_progress')], stats(100, 10, 20, 500)), 1000),
            at(todoWriteMessage('m2', [todo('1', 'completed')], stats(300, 40, 80, 1500)), 5000),
        ]);
        expect(details.get(todoStateKey(todo('1', 'pending')))).toEqual({
            startTs: 1000,
            endTs: 5000,
    ...
    it('clamps an individually shrinking token field to zero', () => {
        ...
        const details = computeTodoDetails([
            at(todoWriteMessage('m1', [todo('1', 'in_progress')], stats(300, 40, 80, 1500)), 1000),
            at(todoWriteMessage('m2', [todo('1', 'completed')], stats(100, 10, 20, 1600)), 5000),
        ]);
    ...
    it('treats an all-zero token diff as not captured, keeping the timestamps', () => {
        ...
        const details = computeTodoDetails([
            at(todoWriteMessage('m1', [todo('1', 'in_progress')], stats(300, 40, 80, 1500)), 1000),
            at(todoWriteMessage('m2', [todo('1', 'completed')], stats(100, 10, 20, 500)), 5000),
        ]);
    ...
    it('omits API time when the diff did not advance it (resume path)', () => {
        ...
        const details = computeTodoDetails([
            at(todoWriteMessage('m1', [todo('1', 'in_progress')], stats(100, 0, 0, 500)), 1000),
            at(todoWriteMessage('m2', [todo('1', 'completed')], stats(300, 0, 0, 500)), 5000),
        ]);
        const resources = details.get(todoStateKey(todo('1', 'pending')))?.resources;
    ...
    it('spans intermediate snapshots while a task stays in progress', () => {
        ...
        const details = computeTodoDetails([
            at(todoWriteMessage('m1', [todo('1', 'in_progress')], stats(100, 0, 0, 100)), 1000),
            at(todoWriteMessage('m2', [todo('1', 'in_progress')], stats(300, 0, 0, 500)), 3000),
            at(todoWriteMessage('m3', [todo('1', 'completed')], stats(500, 0, 0, 900)), 5000),
        ]);
        const detail = details.get(todoStateKey(todo('1', 'pending')));
        expect(detail?.startTs).toBe(1000);
        expect(detail?.endTs).toBe(5000);
    ...
    it('starts a fresh window when a completed id+content is reused by a new task', () => {
        ...
        const details = computeTodoDetails([
            at(todoWriteMessage('a1', [item('1', 'Run tests', 'in_progress')], stats(100, 0, 0, 100)), 1000),
            at(todoWriteMessage('a2', [item('1', 'Run tests', 'completed')], stats(200, 0, 0, 300)), 2000),
            userMessage('u1'),
            at(todoWriteMessage('b1', [item('1', 'Run tests', 'in_progress')], stats(900, 0, 0, 5000)), 8000),
            at(todoWriteMessage('b2', [item('1', 'Run tests', 'completed')], stats(1000, 0, 0, 5200)), 9000),
        ]);
        const detail = details.get(todoStateKey(item('1', 'Run tests', 'completed')));
    ...
    it('resets the window when a task reopens via pending (completed → pending → in_progress)', () => {
        ...
        const details = computeTodoDetails([
            at(todoWriteMessage('a1', [item('1', 'Build', 'in_progress')], stats(100, 0, 0, 100)), 1000),
            at(todoWriteMessage('a2', [item('1', 'Build', 'completed')], stats(200, 0, 0, 300)), 2000),
            at(todoWriteMessage('a3', [item('1', 'Build', 'pending')]), 3000),
            at(todoWriteMessage('a4', [item('1', 'Build', 'in_progress')], stats(900, 0, 0, 5000)), 8000),
            at(todoWriteMessage('a5', [item('1', 'Build', 'completed')], stats(1000, 0, 0, 5200)), 9000),
        ]);
    ...
    it('upgrades a stats-less start baseline to a later snapshot that has stats', () => {
        ...
        const details = computeTodoDetails([
            ...
            at(todoWriteMessage('m1', [item('1', 'X', 'in_progress')], stats(100, 0, 0, 500)), 3000),
            at(todoWriteMessage('m2', [item('1', 'X', 'completed')], stats(300, 0, 0, 900)), 5000),
        ]);
...
describe('plan stats contract (SDK normalizer → extractTodoStats)', () => {
    it('round-trips an agent-stamped _meta.stats into a defined snapshot', () => {
        ...
        const rawOutput = events[0].rawOutput;
        const tool = {
            callId: 'c',
            toolName: 'TodoWrite',
            status: 'completed',
