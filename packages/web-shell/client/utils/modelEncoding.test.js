...
import { encodeVisionModelForSetting, extractBareModelId, decodeVisionModelForPicker, } from './modelEncoding';
describe('encodeVisionModelForSetting', () => {
    it('encodes standard ACP format', () => {
        expect(encodeVisionModelForSetting('qwen-max(qwen-oauth)')).toBe('qwen-oauth:qwen-max');
    });
    ...
    it('handles nested parentheses by matching outermost ACP pattern', () => {
        ...
        expect(encodeVisionModelForSetting('model(name)(extra)')).toBe('extra:model(name)');
    });
    it('passes through already-encoded colon format unchanged', () => {
        ...
        expect(encodeVisionModelForSetting('qwen-oauth:qwen-vl-max')).toBe('qwen-oauth:qwen-vl-max');
    });
...
describe('decodeVisionModelForPicker', () => {
    it('decodes authType:modelId back to ACP format', () => {
        expect(decodeVisionModelForPicker('qwen-oauth:qwen-max')).toBe('qwen-max(qwen-oauth)');
    });
    it('handles colon-bearing model IDs — splits on first colon only', () => {
        expect(decodeVisionModelForPicker('openai:gpt-4o:online')).toBe('gpt-4o:online(openai)');
    });
    ...
    it('passes through ACP-formatted values unchanged', () => {
        expect(decodeVisionModelForPicker('qwen-max(qwen-oauth)')).toBe('qwen-max(qwen-oauth)');
    });
    it('passes through empty string unchanged', () => {
    ...
    it('strips \\0baseUrl suffix before decoding', () => {
        expect(decodeVisionModelForPicker('qwen-oauth:qwen-vl-max\0https://api.example.com')).toBe('qwen-vl-max(qwen-oauth)');
    });
    it('handles colon-bearing ID with \\0baseUrl suffix', () => {
        expect(decodeVisionModelForPicker('openai:gpt-4o:online\0https://api.openai.com')).toBe('gpt-4o:online(openai)');
    });
    it('passes through bare ID with \\0baseUrl suffix but no colon', () => {
        expect(decodeVisionModelForPicker('plain-model\0https://api.example.com')).toBe('plain-model');
    });
});
...
//# sourceMappingURL=modelEncoding.test.js.map