...
import { extractImageTransfer, normalizeImageMediaType, readImageTransfer, } from './imageIngestion';
...
describe('image ingestion', () => {
    ...
    it('uses files as the authoritative source without duplicating item files', () => {
        ...
        const result = extractImageTransfer(transfer({
            files: [file],
    ...
    it('reads supported files in selection order and reports lifecycle settlement', async () => {
        ...
        const extracted = extractImageTransfer(transfer({ files: [first, second], types: ['Files'] }), 'drop');
        const created = [];
        const settled = [];
    ...
    it('limits concurrent file readers', async () => {
        const files = Array.from({ length: 10 }, (_, index) => new File([`image-${index}`], `${index}.png`, { type: 'image/png' }));
        const extracted = extractImageTransfer(transfer({ files, types: ['Files'] }), 'drop');
        let activeReaders = 0;
...
//# sourceMappingURL=imageIngestion.test.js.map