...
import { decideEscapeIntent } from './escapeIntent';
const base = {
    blocked: false,
    streaming: false,
    hasInput: false,
...
describe('decideEscapeIntent', () => {
    ...
    it('confirms cancel on the second Esc while streaming', () => {
        expect(decideEscapeIntent({ ...base, streaming: true, armed: 'cancel' })).toEqual({ kind: 'cancel' });
    });
    it('re-arms cancel (not clear) when a clear-armed press lands while streaming', () => {
        expect(decideEscapeIntent({ ...base, streaming: true, armed: 'clear' })).toEqual({ kind: 'arm', action: 'cancel' });
    });
    it('prioritises streaming over composer text', () => {
        expect(decideEscapeIntent({ ...base, streaming: true, hasInput: true })).toEqual({ kind: 'arm', action: 'cancel' });
    });
    ...
    it('confirms clear on the second Esc with text', () => {
        expect(decideEscapeIntent({ ...base, hasInput: true, armed: 'clear' })).toEqual({ kind: 'clear' });
    });
    it('re-arms clear when a stale cancel-armed press lands with text', () => {
        expect(decideEscapeIntent({ ...base, hasInput: true, armed: 'cancel' })).toEqual({ kind: 'arm', action: 'clear' });
    });
...
//# sourceMappingURL=escapeIntent.test.js.map