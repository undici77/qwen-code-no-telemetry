...
export function normalizeImageMediaType(mediaType, fileName = '') {
...
export function hasFileTransferPayload(dataTransfer) {
...
export function extractImageTransfer(dataTransfer, source) {
    const candidates = [];
    const rejected = [];
    ...
    if (dataTransfer.files.length > 0) {
        ...
        for (const item of Array.from(dataTransfer.items)) {
            ...
            const mediaType = normalizeImageMediaType(file.type || item.type, file.name);
            if (mediaType) {
                candidates.push({ file, mediaType });
            }
            else {
...
function readImage(candidate, lifecycle) {
...
export async function readImageTransfer(transfer, lifecycle = {}) {
    const candidates = [];
    const rejected = [...transfer.rejected];
    let estimatedEncodedBytes = 0;
    const maxEncodedBytes = lifecycle.maxEncodedBytes ?? MAX_IMAGE_ATTACHMENT_DATA_BYTES;
    ...
    await Promise.all(Array.from({ length: Math.min(MAX_CONCURRENT_IMAGE_READERS, candidates.length) }, readNext));
    const accepted = [];
...
//# sourceMappingURL=imageIngestion.js.map