...
function xterm256(index) {
    ...
    if (index < 8)
        return ANSI_COLORS[30 + index];
    if (index < 16)
        return ANSI_COLORS[90 + (index - 8)];
    if (index < 232) {
        const v = index - 16;
        return toHex(CUBE_LEVELS[Math.floor(v / 36)], CUBE_LEVELS[Math.floor(v / 6) % 6], CUBE_LEVELS[v % 6]);
    }
