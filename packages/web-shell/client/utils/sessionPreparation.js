...
export async function createAndAttachSessionForPrompt({ sessionActions, modelId, modeId, workspaceCwd, worktree, branch, onSessionCreated, onSessionAllocated, getCurrentSessionId, warn = console.warn, }) {
    ...
    const approvalMode = modeId && isDaemonApprovalMode(modeId) ? modeId : undefined;
    const { sessionId, worktree: worktreeInfo, branch: branchInfo, } = await sessionActions.createSession({
        workspaceCwd,
    ...
    try {
            ...
            try {
                await Promise.race([
                    ...
                    new Promise((_, reject) => {
                        timeout = setTimeout(() => reject(new Error('onSessionCreated timed out')), SESSION_CREATED_CALLBACK_TIMEOUT_MS);
                    }),
                ]);
            }
            finally {
        ...
        if (sessionIdBeforeAttach !== undefined &&
            sessionIdBeforeAttach !== sessionId) {
            throw new Error(`Session changed before attach: expected ${sessionId}, found ${sessionIdBeforeAttach}`);
        }
        ...
        if (sessionIdAfterAttach !== undefined &&
            sessionIdAfterAttach !== sessionId) {
            throw new Error(`Session changed while attaching: expected ${sessionId}, found ${sessionIdAfterAttach}`);
        }
    }
    catch (error) {
        ...
        if (currentSessionId === undefined || currentSessionId === sessionId) {
            ...
            warn(`[WebShell] skipping clearSession: expected ${sessionId}, found ${currentSessionId}`);
        }
        throw error;
    }
