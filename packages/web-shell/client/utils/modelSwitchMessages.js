...
function filterDuplicateModelSwitchMessages(messages) {
    ...
    for (const message of messages) {
        if (!isModelSwitchSummaryMessage(message))
            continue;
        const model = getModelSwitchSummaryMessageModel(message);
        if (model)
            summarizedModels.add(model);
    }
    if (summarizedModels.size === 0)
        return [...messages];
    return messages.filter((message) => {
        if (message.role !== 'system' || message.variant !== 'info')
            return true;
...
function isModelSwitchMessage(message) {
    if (message.role !== 'system' || message.variant !== 'info')
        return false;
    return (parseModelSwitchStatusModel(message.content) !== null ||
        isModelSwitchSummaryMessage(message));
}
function filterLeadingModelSwitchMessages(messages) {
    const firstContentIndex = messages.findIndex((message) => !isModelSwitchMessage(message));
    return firstContentIndex < 0 ? [] : messages.slice(firstContentIndex);
}
export function filterModelSwitchMessages(messages) {
    return filterLeadingModelSwitchMessages(filterDuplicateModelSwitchMessages(messages));
}
