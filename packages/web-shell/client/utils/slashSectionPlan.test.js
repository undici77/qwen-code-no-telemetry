...
describe('planSlashSectionRows', () => {
    ...
    it('shows a header but no divider on the first row', () => {
        const plans = planSlashSectionRows([{ section: 'Skill commands' }, { section: 'System commands' }], 'command');
    ...
    it('never groups subcommand menus', () => {
        const plans = planSlashSectionRows([{ section: 'Skill commands' }, { section: 'System commands' }], 'subcommand');
        expect(plans.every((p) => !p.showHeader && p.count === 0)).toBe(true);
    });
    it('shows no headers when items carry no section (search results)', () => {
        const plans = planSlashSectionRows([{ section: undefined }, { section: undefined }], 'command');
        expect(plans.every((p) => !p.showHeader)).toBe(true);
    });
});
//# sourceMappingURL=slashSectionPlan.test.js.map