...
import { getComposerPlaceholderKey, getComposerPlaceholderState, shouldBlockComposerSubmit, shouldDisableComposerInput, } from './composerInputState';
describe('composer input state', () => {
    it('keeps the composer editable while the SSE connection is disconnected', () => {
        expect(shouldDisableComposerInput({
            catchingUp: false,
            pendingApproval: false,
            isPreparingPrompt: false,
        })).toBe(false);
        expect(getComposerPlaceholderKey({
            catchingUp: false,
            isPreparingPrompt: false,
            isStreaming: false,
    ...
    it('exposes the semantic placeholder state independently of i18n keys', () => {
        expect(getComposerPlaceholderState({
            catchingUp: false,
            isPreparingPrompt: false,
            isStreaming: false,
        })).toBe('idle');
    ...
    it('blocks submit only after the connection reaches a failed state', () => {
        expect(shouldBlockComposerSubmit({
...
//# sourceMappingURL=composerInputState.test.js.map