...
describe('createAndAttachSessionForPrompt', () => {
    ...
    it('warns but resolves when the post-create model switch fails', async () => {
        ...
        expect(order).toEqual(['create', 'attach', 'model']);
        expect(warn).toHaveBeenCalledWith('[WebShell] failed to set model for new session:', error);
    });
    ...
    it('releases and clears the created session when attach fails', async () => {
        ...
        expect(actions.setModel).not.toHaveBeenCalled();
        expect(warn).toHaveBeenCalledWith('[WebShell] failed to attach new session:', error);
    });
    it('preserves the original error when release and clear both fail', async () => {
        ...
        expect(actions.releaseSession).toHaveBeenCalledWith('session-1');
        expect(actions.clearSession).toHaveBeenCalledOnce();
        expect(warn).toHaveBeenCalledWith('[WebShell] failed to release unattached session:', releaseError);
        expect(warn).toHaveBeenCalledWith('[WebShell] failed to clear unattached session:', clearError);
    });
    ...
    it('releases only the created session when the current session changes', async () => {
        ...
        callbackFinished.resolve();
        await expect(result).rejects.toThrow('Session changed before attach: expected session-1, found session-2');
        expect(actions.attachSession).not.toHaveBeenCalled();
        expect(actions.releaseSession).toHaveBeenCalledWith('session-1');
        expect(actions.clearSession).not.toHaveBeenCalled();
        expect(warn).toHaveBeenCalledWith('[WebShell] skipping clearSession: expected session-1, found session-2');
    });
    it('does not set the model when the session changes during attach', async () => {
        let currentSessionId = 'session-1';
        const attachFinished = createDeferred();
        const actions = createActions({
        ...
        await expect(result).rejects.toThrow('Session changed while attaching: expected session-1, found session-2');
        expect(actions.setModel).not.toHaveBeenCalled();
        expect(actions.releaseSession).toHaveBeenCalledWith('session-1');
        expect(actions.clearSession).not.toHaveBeenCalled();
        expect(warn).toHaveBeenCalledWith('[WebShell] skipping clearSession: expected session-1, found session-2');
    ...
    it('cleans up the created session when onSessionCreated rejects', async () => {
        ...
        expect(actions.clearSession).toHaveBeenCalledOnce();
        expect(actions.setModel).not.toHaveBeenCalled();
        expect(warn).toHaveBeenCalledWith('[WebShell] failed to run onSessionCreated:', error);
    });
...
//# sourceMappingURL=sessionPreparation.test.js.map