...
const COMPONENT_SCOPE = ':where([data-web-shell-root][data-web-shell-shadcn], [data-web-shell-portal-root][data-web-shell-shadcn], [data-web-shell-root][data-web-shell-shadcn] *, [data-web-shell-portal-root][data-web-shell-shadcn] *)';
const COMPONENT_ROOT_SCOPE = ':is([data-web-shell-root]:where([data-web-shell-shadcn]), [data-web-shell-portal-root]:where([data-web-shell-shadcn]))';
function scopeComponentCss(css) {
    const root = postcss.parse(css);
    const scopeNode = selectorParser().astSync(COMPONENT_SCOPE).first.first;
    if (!scopeNode)
        throw new Error('Invalid WebShell component CSS scope');
    ...
    const sortedPropertyNames = [...propertyNames].sort(([left], [right]) => right.length - left.length);
    root.walkDecls((declaration) => {
        const scopedProperty = propertyNames.get(declaration.prop);
        if (scopedProperty)
            declaration.prop = scopedProperty;
        ...
        for (const [original, scoped] of keyframeNames) {
            declaration.value = declaration.value.replace(new RegExp(`(?<![\\w-])${escapeRegExp(original)}(?![\\w-])`, 'g'), scoped);
        }
    });
    root.walkRules((rule) => {
        ...
        if (rule.selectors.every((selector) => selector === ':root' || selector === ':host')) {
