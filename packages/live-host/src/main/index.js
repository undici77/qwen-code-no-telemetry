...
import { isValidInputAudioFrame, } from '../shared/protocol.ts';
import { overlayPosition } from './overlay-position.ts';
import { shouldActivateNativeServices, shouldDeactivateNativeServices, } from './native-service-policy.ts';
import { isRecoverableOverlayLoadFailure, OverlayRecoveryController, } from './overlay-recovery.ts';
import { canToggleLive, isActiveLiveCall, projectLiveStatusForCapture, shouldCaptureLiveAudio, shouldStopLiveOnToggle, } from './live-state-policy.ts';
...
let readinessReconnectTimer;
let microphonePermissionTimer;
let overlayHideTimer;
let pointerInteractive = false;
let captureReadyEpoch;
...
let connection = { phase: 'disconnected' };
let live = {
    v: 1,
    available: false,
    state: 'unavailable',
...
let hostAudioCapture;
let hostInputCapture;
function openHostAudioCapture(source, epoch) {
...
function appendHostAudio(audio, epoch) {
    if (!hostAudioCapture) {
        hostAudioCapture = openHostAudioCapture('host-output', epoch);
    }
    if (hostAudioCapture)
        appendAudioCapture(hostAudioCapture, audio);
}
function closeHostAudioCapture(reason) {
    const capture = hostAudioCapture;
    hostAudioCapture = undefined;
    closeAudioCapture(capture, reason);
}
function appendHostInputAudio(audio, epoch) {
    ...
    if (hostInputCapture)
        appendAudioCapture(hostInputCapture, audio);
}
function closeHostInputCapture(reason) {
    const capture = hostInputCapture;
    hostInputCapture = undefined;
    closeAudioCapture(capture, reason);
}
function hostReadinessBlocker() {
    ...
    if (!selfChecks.audioInput)
        return 'audio_input';
    if (!selfChecks.audioOutput)
        return 'audio_output';
    if (!selfChecks.globalShortcut)
        return 'global_shortcut';
    if (!selfChecks.appshot)
        return 'appshot';
    return undefined;
}
...
function showOverlay() {
    ...
    const position = overlayPosition(display.workArea, bounds.width, bounds.height);
...
function scheduleReadinessReconnect() {
    ...
    if (readinessReconnectTimer)
        clearTimeout(readinessReconnectTimer);
...
function sendAudioCommand(channel, value) {
    if (!overlayReady ||
        !overlay ||
        overlay.isDestroyed() ||
        overlay.webContents.isDestroyed()) {
...
function failRequiredAction(action) {
...
function sendRequiredAction(action) {
    let sent = false;
    try {
    ...
    if (!sent)
        failRequiredAction(action.action);
    return sent;
}
function stopLive() {
    liveStartPending = false;
...
function failClosedForReadinessLoss() {
    if (isActiveLiveCall(live))
        stopLive();
    else
        stopLocalAudio();
}
function failAudioAndRecheck(reason) {
    if (!nativeServicesActive)
        return;
    audioTransportFailed = true;
    selfChecks.audioInput = false;
...
function applyLiveStatus(status) {
    ...
    const captureEnabled = !audioTransportFailed && shouldCaptureLiveAudio(status, isHostReady());
    ...
    if (!captureEnabled || status.state === 'stopping') {
        closeHostInputCapture(status.state === 'stopping' ? 'call_stopping' : 'capture_disabled');
        sendAudioCommand('live:audio:clear');
    }
    if (captureEnabled)
        showOverlay();
...
function toggleLive() {
    ...
    showOverlay();
    if (!canToggleLive(live, connection.phase === 'ready', isHostReady()))
        return;
    if (sendRequiredAction({
        type: 'host.action',
        action: 'toggle',
        epoch: daemon.getEpoch(),
    })) {
...
function newConversation() {
    showOverlay();
    if (connection.phase !== 'ready' || !live.available || !isHostReady())
        return;
    if (sendRequiredAction({
        type: 'host.action',
        action: 'new',
...
function beginMicrophonePermissionMonitor() {
    if (microphonePermissionTimer)
        return;
    microphonePermissionTimer = setInterval(() => {
        if (!nativeServicesActive)
            return;
        const next = microphonePermission();
        if (next === permissions.microphone)
            return;
...
function activateNativeServices() {
    if (nativeServicesActive)
        return;
    ...
    sendAudioCommand('live:audio:initialize', permissions.microphone === 'granted');
}
function deactivateNativeServices() {
    if (!nativeServicesActive)
        return;
    ...
    readinessReconnectTimer = undefined;
    if (microphonePermissionTimer)
        clearInterval(microphonePermissionTimer);
...
function isTrustedSender(event) {
    return Boolean(overlay && !overlay.isDestroyed() && event.sender === overlay.webContents);
}
function registerIpc() {
    ...
    ipcMain.handle('live:open-web-shell-permission', async (event) => {
        if (!isTrustedSender(event) || !live.pendingPermission)
            return;
        const url = daemon.getWebShellSessionUrl(live.pendingPermission);
        if (url)
            await shell.openExternal(url);
    });
    ipcMain.handle('live:set-input-muted', (event, muted) => {
        if (!isTrustedSender(event) || typeof muted !== 'boolean')
            return;
        const outputMuted = live.outputMuted ?? false;
        live = { ...live, inputMuted: muted };
        sendAudioCommand('live:audio:set-capture', {
            enabled: !audioTransportFailed && shouldCaptureLiveAudio(live, isHostReady()),
            muted,
            epoch: daemon.getEpoch(),
        });
        sendRequiredAction({
    ...
    ipcMain.handle('live:set-output-muted', (event, muted) => {
        if (!isTrustedSender(event) || typeof muted !== 'boolean')
            return;
    ...
    ipcMain.handle('live:request-permission', async (event, permission) => {
        if (!isTrustedSender(event) ||
    ...
    ipcMain.on('live:audio:input', (event, value) => {
        if (!isTrustedSender(event) ||
            !nativeServicesActive ||
        ...
        const frame = new Uint8Array(valueView.buffer, valueView.byteOffset, valueView.byteLength);
    ...
    ipcMain.on('live:audio:diagnostic', (event, value) => {
        if (!isTrustedSender(event) ||
            typeof value !== 'object' ||
            value === null ||
            Array.isArray(value)) {
        ...
        writeLiveDiagnostic(record.event, record.details);
    });
    ipcMain.on('live:audio:capture-ready', (event, value) => {
        if (!isTrustedSender(event) ||
            !rendererAudioEventsEnabled ||
            typeof value !== 'object' ||
            value === null ||
    ...
    ipcMain.on('live:audio:self-check', (event, value) => {
        if (!isTrustedSender(event) ||
            !nativeServicesActive ||
            !rendererAudioEventsEnabled ||
            typeof value !== 'object' ||
        ...
        const changed = selfChecks.audioInput !== nextInput ||
    ...
    ipcMain.on('live:pointer-interactivity', (event, interactive) => {
        if (!isTrustedSender(event) || typeof interactive !== 'boolean')
            return;
...
function createOverlay() {
    ...
    window.webContents.on('did-finish-load', () => {
        if (window !== overlay || !rendererLoadHealthy)
            return;
...
function trayIconPath() {
    return app.isPackaged
        ...
        : join(app.getAppPath(), '..', 'electron', 'resources', 'brands', 'qwen-code', 'icon.png');
}
...
void app.whenReady().then(() => {
    ...
    appshotReadiness = new AppshotReadinessMonitor((state) => {
        if (!nativeServicesActive)
            return;
        const changed = permissions.accessibility !== state.accessibility ||
        ...
        onSnapshot: (snapshot) => {
            ...
            if (shouldDeactivateNativeServices(snapshot.phase)) {
                ...
                live = {
                    ...
                    available: false,
                    state: 'unavailable',
                    blocker: snapshot.phase === 'incompatible'
                        ? 'host_version'
                        : 'host_disconnected',
                };
            }
            if (snapshot.status)
                applyLiveStatus(snapshot.status);
            else
                publishState();
        },
        onOutputAudio: (audio) => {
