...
describe('AppshotReadinessMonitor', () => {
    it('checks the in-process native module once without polling', async () => {
        ...
        const monitor = new AppshotReadinessMonitor((state) => states.push(state), () => native);
        monitor.start();
        monitor.start();
    ...
    it('fails closed when the built-in module cannot load', () => {
        const states = [];
        const monitor = new AppshotReadinessMonitor((state) => states.push(state), () => {
    ...
    it('does nothing after the monitor stops', () => {
        ...
        const monitor = new AppshotReadinessMonitor(() => undefined, () => native);
        monitor.start();
        monitor.stop();
        monitor.requestPermission('accessibility');
        monitor.refresh();
...
//# sourceMappingURL=appshot-readiness.test.js.map