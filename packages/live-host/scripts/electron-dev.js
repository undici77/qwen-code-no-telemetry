...
import { downloadUv } from './build/common';
...
async function ensureQwenRuntimeForDev() {
    ...
    const requestedExternalArtifact = Boolean(process.env.QWEN_CODE_TARBALL || process.env.QWEN_CODE_VERSION);
    ...
    if (!env.QWEN_CODE_TARBALL && !env.QWEN_CODE_VERSION) {
        ...
        if (!defaultVersion) {
            throw new Error('No Qwen Code CLI runtime configured. Set QWEN_CODE_VERSION, QWEN_CODE_TARBALL, QWEN_CODE_ROOT, or qwenCodeRuntime.version in package.json.');
        }
...
async function ensureBundledUvForCurrentPlatform() {
...
function detectInstance() {
    ...
    console.log(`🔢 ${instance.source} detected (${instance.label}): ` +
...
async function buildMcpServers() {
    ...
    const sessionResult = await runEsbuild('packages/session-mcp-server/src/index.ts', 'packages/session-mcp-server/dist/index.js', {}, { packagesExternal: true });
...
async function runEsbuild(entryPoint, outfile, defines = {}, options = {}) {
    try {
        await esbuild.build({
            ...
            external: ['electron'],
            ...(options.packagesExternal ? { packages: 'external' } : {}),
...
async function verifyJsFile(filePath) {
...
async function waitForFileStable(filePath, timeoutMs = 10000) {
...
async function main() {
    ...
    if (existsSync(preloadCjsPath))
        rmSync(preloadCjsPath);
    if (existsSync(toolbarPreloadCjsPath))
        rmSync(toolbarPreloadCjsPath);
    // Build main and preload entries in parallel
    const [mainResult, preloadResult, toolbarPreloadResult] = await Promise.all([
        runEsbuild('apps/electron/src/main/index.ts', 'apps/electron/dist/main.cjs', oauthDefines, { alias: MAIN_PROCESS_ALIAS }),
        runEsbuild('apps/electron/src/preload/bootstrap.ts', 'apps/electron/dist/bootstrap-preload.cjs'),
        runEsbuild('apps/electron/src/preload/browser-toolbar.ts', 'apps/electron/dist/browser-toolbar-preload.cjs'),
    ]);
...
//# sourceMappingURL=electron-dev.js.map