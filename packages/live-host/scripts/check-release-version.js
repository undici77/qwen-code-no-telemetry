...
import { desktopReleasePackageSources, normalizeReleaseVersion, readPackageVersion, } from './desktop-release-version.ts';
...
function appendGithubOutput(outputPath, outputs) {
    if (!outputPath)
        return;
    const lines = Object.entries(outputs).map(([key, value]) => `${key}=${value}`);
    appendFileSync(outputPath, `${lines.join('\n')}\n`);
}
function appendGithubSummary(summaryPath, params) {
    if (!summaryPath)
        return;
    const lines = [
        ...
        '| Source | Version |',
        '| --- | --- |',
        ...params.packageVersions.map(({ source, version }) => `| ${source.path} | ${version} |`),
    ];
...
function main() {
    ...
    if (!args.version) {
        throw new Error('Release version is required. Pass --version or RELEASE_VERSION.');
    }
...
//# sourceMappingURL=check-release-version.js.map