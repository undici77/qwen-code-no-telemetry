import { desktopReleasePackageSources, normalizeReleaseVersion, readPackageVersion, writePackageVersion, } from './desktop-release-version.ts';
...
function main() {
    ...
    const changes = desktopReleasePackageSources.map((source) => ({
        currentVersion: readPackageVersion(source.path),
        path: source.path,
    }));
    const pendingChanges = changes.filter((change) => change.currentVersion !== version);
    if (args.dryRun) {
        console.log(`Desktop version dry run: ${version} (${tag})`);
        for (const change of changes) {
            const suffix = change.currentVersion === version
                ? 'already set'
...
//# sourceMappingURL=bump-desktop-version.js.map