 ...
 */
export function getBunDownloadName(platform, arch) {
    const archMap = {
        x64: 'x64',
        arm64: 'aarch64',
    };
 ...
 */
export function getUvDownloadName(platform, arch) {
 ...
 */
export async function downloadBun(config) {
 ...
 */
export async function downloadUv(config) {
    const { platform, arch, electronDir } = config;
 ...
 */
export function cleanBuildArtifacts(config) {
 ...
 */
export function copySessionServer(config) {
...
export function buildMcpServers(config) {
    ...
    execSync(`bun build ${join(sessionDir, 'src', 'index.ts')} --outfile ${sessionOut} --target node --format cjs`, { cwd: rootDir, stdio: 'inherit', shell: true });
 ...
 */
export function buildWhatsAppWorker(config) {
    const { rootDir } = config;
 ...
 */
export function verifyMcpServersExist(config) {
    const { electronDir } = config;
 ...
 */
export async function buildElectronApp(config) {
    const { rootDir } = config;
 ...
 */
export async function createManifest(config) {
    const { rootDir, electronDir } = config;
 ...
 */
export async function loadEnvFile(config) {
 ...
 */
export function getArtifactName(platform, arch) {
    switch (platform) {
        case 'darwin':
