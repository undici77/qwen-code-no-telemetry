...
describe('package.json command metadata', () => {
    ...
    it('describes focusChat as focusing the chat view', () => {
        ...
        const command = manifest.contributes.commands.find((item) => item.command === 'qwen-code.focusChat');
        expect(command?.title).toBe('Qwen Code: Focus Chat View');
    });
    it('keeps the Activity Bar chat entry visible without runtime context', () => {
        const manifest = readManifest();
        const sidebarContainer = manifest.contributes.viewsContainers.activitybar.find((item) => item.id === 'qwen-code-sidebar');
        const sidebarView = manifest.contributes.views['qwen-code-sidebar']?.find((item) => item.id === 'qwen-code.chatView.sidebar');
