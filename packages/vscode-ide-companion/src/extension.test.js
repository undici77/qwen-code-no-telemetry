...
describe('activate', () => {
    ...
    it('should show the info message on first activation', async () => {
        ...
        expect(showInformationMessageMock).toHaveBeenCalledWith('Qwen Code Companion extension successfully installed.');
    });
    ...
    it('should launch the Qwen Code when the user clicks the button', async () => {
        const showInformationMessageMock = vi
            .mocked(vscode.window.showInformationMessage)
            .mockResolvedValue('Run Qwen Code');
        vi.mocked(context.globalState.get).mockReturnValue(undefined);
        vi.mocked(vscode.extensions.getExtension).mockReturnValue({
            packageJSON: { version: '1.1.0' },
        });
    ...
    describe('update notification', () => {
        ...
        it('should show an update notification if a newer version is available', async () => {
            ...
            await activate(context);
            expect(showInformationMessageMock).toHaveBeenCalledWith('A new version (1.2.0) of the Qwen Code Companion extension is available.', 'Update to latest version');
        });
        ...
        it('should execute the install command when the user clicks "Update"', async () => {
            ...
            expect(executeCommandMock).toHaveBeenCalledWith('workbench.extensions.installExtension', 'qwenlm.qwen-code-vscode-ide-companion');
        });
