 ...
 */
import { IdeDiffAcceptedNotificationSchema, IdeDiffClosedNotificationSchema, } from '@qwen-code/qwen-code-core';
...
export class DiffManager {
    ...
    pendingDelayTimers = new Map();
    static DEDUPE_WINDOW_MS = 1500;
    ...
    suppressUntil = null;
    constructor(log, diffContentProvider, shouldDelay, shouldSuppress) {
        this.log = log;
     ...
     */
    hasExistingDiff(filePath, oldContent, newContent) {
     ...
     */
    async focusExistingDiff(filePath) {
    ...
    async showDiff(filePath, a, b) {
        ...
        if (this.hasExistingDiff(normalizedPath, oldContent, newContent)) {
            ...
            if (now - last < DiffManager.DEDUPE_WINDOW_MS) {
                ...
                this.log(`Duplicate showDiff suppressed for ${filePath} (within ${DiffManager.DEDUPE_WINDOW_MS}ms)`);
                return;
            }
        ...
        await vscode.commands.executeCommand('setContext', 'qwen.diff.isVisible', true);
    ...
    async onActiveEditorChange(editor) {
        ...
        await vscode.commands.executeCommand('setContext', 'qwen.diff.isVisible', isVisible);
    }
    ...
    async closeDiffEditor(rightDocUri) {
        const diffInfo = this.diffDocuments.get(rightDocUri.toString());
        await vscode.commands.executeCommand('setContext', 'qwen.diff.isVisible', false);
        if (diffInfo) {
    ...
    async closeAll() {
        ...
        const uris = Array.from(this.diffDocuments.keys()).map((k) => vscode.Uri.parse(k));
    ...
    async readOldContentFromFs(filePath) {
        try {
    ...
    makeKey(filePath, oldContent, newContent) {
