...
import { isFileUri, isNotebookFileUri, isNotebookCellUri, removeFile, renameFile, getNotebookUriFromCellUri, } from './services/open-files-manager/utils.js';
import { addOrMoveToFront, updateActiveContext, } from './services/open-files-manager/text-handler.js';
import { addOrMoveToFrontNotebook, updateNotebookActiveContext, updateNotebookCellSelection, } from './services/open-files-manager/notebook-handler.js';
/**
    ...
    constructor(context) {
        ...
        const editorWatcher = vscode.window.onDidChangeActiveTextEditor((editor) => {
            if (editor && isFileUri(editor.document.uri)) {
                ...
                if (notebookUri && isNotebookFileUri(notebookUri)) {
                    ...
                    const notebookEditor = vscode.window.visibleNotebookEditors.find((nbEditor) => nbEditor.notebook.uri.toString() === notebookUri.toString());
                    if (notebookEditor) {
        ...
        const selectionWatcher = vscode.window.onDidChangeTextEditorSelection((event) => {
            if (isFileUri(event.textEditor.document.uri)) {
                ...
                updateNotebookCellSelection(this.openFiles, event.textEditor, event.selections);
                this.fireWithDebounce();
            }
        });
        // Add notebook cell selection watcher for .ipynb files if the API is available
        let notebookCellSelectionWatcher;
        ...
        let notebookCloseWatcher;
        if (vscode.workspace.onDidCloseNotebookDocument) {
            notebookCloseWatcher = vscode.workspace.onDidCloseNotebookDocument((document) => {
        ...
        if (vscode.window.activeNotebookEditor &&
            ...
            addOrMoveToFrontNotebook(this.openFiles, vscode.window.activeNotebookEditor);
        }
    }
    fireWithDebounce() {
