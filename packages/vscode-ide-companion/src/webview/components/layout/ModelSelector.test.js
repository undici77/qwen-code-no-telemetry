...
function renderModelSelector(props) {
    ...
    const onClose = vi.fn();
    act(() => {
        root.render(_jsx(ModelSelector, { visible: props.visible ?? true, models: props.models, currentModelId: props.currentModelId ?? null, onSelectModel: onSelectModel, onClose: onClose }));
    });
    const handle = { container, root, onSelectModel, onClose };
    handles.push(handle);
    return handle;
}
afterEach(() => {
...
describe('ModelSelector — discontinued state (Issue #3745)', () => {
    ...
    it('replaces description with the migration hint for discontinued models', () => {
        ...
        expect(container.textContent).toContain('Discontinued — switch to Coding Plan or API Key');
        expect(container.textContent).not.toContain('Original description should be replaced');
    });
    it('does NOT mark a runtime Qwen OAuth snapshot as discontinued', () => {
        ...
        expect(container.querySelector('[data-testid="discontinued-badge"]')).toBeNull();
    });
    it('blocks click selection on a discontinued model and surfaces an inline error', () => {
        ...
        const blocked = container.querySelector('[data-testid="model-selector-blocked"]');
        expect(blocked?.textContent).toContain('Qwen OAuth free tier was discontinued on 2026-04-15');
    });
    ...
    it('clears a stale blocked message when hovering another row', () => {
        ...
        const discontinuedRow = container.querySelector('[data-discontinued="true"]');
        const otherRow = container.querySelectorAll('[data-index]')[1];
        act(() => {
            discontinuedRow.click();
        });
        expect(container.querySelector('[data-testid="model-selector-blocked"]')).not.toBeNull();
        ...
        // listener.
        act(() => {
            otherRow.dispatchEvent(new MouseEvent('mouseover', { bubbles: true, relatedTarget: null }));
        });
        expect(container.querySelector('[data-testid="model-selector-blocked"]')).toBeNull();
    });
