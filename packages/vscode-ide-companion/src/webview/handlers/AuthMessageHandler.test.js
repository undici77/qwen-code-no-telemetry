...
describe('AuthMessageHandler', () => {
    ...
    it('sends authCancelled when the provider picker is dismissed', async () => {
        ...
        const handler = new AuthMessageHandler({}, {}, null, sendToWebView);
        await handler.handle({ type: 'auth' });
        expect(mockShowQuickPick).toHaveBeenCalledWith(expect.any(Array), expect.objectContaining({ ignoreFocusOut: true }));
    ...
    it('sends authCancelled when the api key input is dismissed mid-flow', async () => {
        ...
        const handler = new AuthMessageHandler({}, {}, null, sendToWebView);
        await handler.handle({ type: 'auth' });
        expect(mockShowInputBox).toHaveBeenCalledWith(expect.objectContaining({ ignoreFocusOut: true }));
        expect(sendToWebView).toHaveBeenCalledWith({ type: 'authCancelled' });
    });
    it('drives a fixed-baseUrl third-party provider through to authInteractiveHandler', async () => {
        ...
        expect(authInteractiveHandler).toHaveBeenCalledWith(expect.objectContaining({ id: 'deepseek' }), expect.objectContaining({
    ...
    it('shows a baseUrl picker for providers with BaseUrlOption arrays', async () => {
        ...
        expect(baseUrlPickerCall?.[0]).toEqual(expect.arrayContaining([
    ...
    it('falls back to the protocol-specific default when custom URL input is blank', async () => {
        ...
        handler.setAuthInteractiveHandler(authInteractiveHandler);
        await handler.handle({ type: 'auth' });
        expect(authInteractiveHandler).toHaveBeenCalledWith(expect.objectContaining({ id: 'custom-openai-compatible' }), expect.objectContaining({
...
//# sourceMappingURL=AuthMessageHandler.test.js.map