...
export class FileMessageHandler extends BaseMessageHandler {
    ...
    async getOrCreateFileSearch(rootPath) {
        ...
        try {
            ...
            logger.error('[FileMessageHandler] Failed to initialize file search:', error);
            return null;
        }
    }
    clearFileSearchCache(rootPath) {
        ...
        crawlCache.clear();
        logger.log('[FileMessageHandler] Cleared file search cache, trigger:', rootPath);
    }
    toZeroBasedEditorPosition(value) {
        if (!value) {
            return 0;
        }
    ...
    async handleShowContextPicker() {
        try {
            ...
            logger.error('[FileMessageHandler] Failed to show context picker:', error);
            const errorMsg = getErrorMessage(error);
            this.sendToWebView({
                type: 'error',
    ...
    async handleGetWorkspaceFiles(query, requestId) {
        try {
            ...
            if (query) {
                logger.log('[FileMessageHandler] Searching workspace files with fuzzy search for query', query);
                const workspaceFolders = vscode.workspace.workspaceFolders;
                if (workspaceFolders) {
                    for (const folder of workspaceFolders) {
                        const rootPath = folder.uri.fsPath;
                ...
                try {
                    ...
                    logger.log('[FileMessageHandler] Sent initial workspaceFiles (open tabs/active)', files.length);
                }
                catch (e) {
                    logger.warn('[FileMessageHandler] Failed sending initial response', e);
                }
            ...
            logger.log('[FileMessageHandler] Sent final workspaceFiles', files.length);
        }
        catch (error) {
            logger.error('[FileMessageHandler] Failed to get workspace files:', error);
            const errorMsg = getErrorMessage(error);
            this.sendToWebView({
                type: 'error',
    ...
    async handleCreateAndOpenTempFile(data) {
        if (!data) {
            logger.warn('[FileMessageHandler] No data provided for createAndOpenTempFile');
            return;
        }
        try {
            ...
            if (foundExistingTab) {
                ...
                logger.log('[FileMessageHandler] Focused on existing readonly file:', uri.toString(), 'in viewColumn:', existingViewColumn);
                return;
            }
            ...
            logger.log('[FileMessageHandler] Created and opened readonly file:', uri.toString(), 'in viewColumn:', targetViewColumn ?? 'Beside');
        }
        catch (error) {
            logger.error('[FileMessageHandler] Failed to create and open temporary file:', error);
