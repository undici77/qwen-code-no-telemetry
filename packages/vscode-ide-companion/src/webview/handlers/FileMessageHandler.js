...
import { findLeftGroupOfChatWebview, findRightGroupOfChatWebview, } from '../../utils/editorGroupUtils.js';
import { ReadonlyFileSystemProvider } from '../../services/readonlyFileSystemProvider.js';
import { crawlCache, FileDiscoveryService, FileSearchFactory, } from '@qwen-code/qwen-code-core';
...
export class FileMessageHandler extends BaseMessageHandler {
    fileDiscoveryServices = new Map();
    fileSearchInstances = new Map();
    fileSearchInitializing = new Map();
    fileWatchers = new Map();
    globSpecialChars = new Set([
        '\\',
        '*',
        '?',
        '[',
    ...
    async getOrCreateFileSearch(rootPath) {
        ...
        try {
            ...
            console.error('[FileMessageHandler] Failed to initialize file search:', error);
            return null;
        }
    }
    clearFileSearchCache(rootPath) {
        this.fileSearchInstances.delete(rootPath);
        this.fileSearchInitializing.delete(rootPath);
        crawlCache.clear();
        console.log('[FileMessageHandler] Cleared file search cache, trigger:', rootPath);
    }
    ...
    disposeWatcherForFolder(rootPath) {
    ...
    setupFileWatchers() {
        ...
        const foldersChangeListener = vscode.workspace.onDidChangeWorkspaceFolders((e) => {
    ...
    async handle(message) {
        ...
        switch (message.type) {
            ...
            case 'getWorkspaceFiles':
                await this.handleGetWorkspaceFiles(data?.query, data?.requestId);
                break;
            case 'openFile':
                await this.handleOpenFile(data?.path);
                break;
            case 'openDiff':
            ...
            case 'createAndOpenTempFile':
                ...
                break;
            default:
                console.warn('[FileMessageHandler] Unknown message type:', message.type);
                break;
        }
    }
    /**
    ...
    async handleShowContextPicker() {
        try {
            ...
            console.error('[FileMessageHandler] Failed to show context picker:', error);
            const errorMsg = getErrorMessage(error);
            this.sendToWebView({
                type: 'error',
     ...
     * Get workspace files
     */
    async handleGetWorkspaceFiles(query, requestId) {
        try {
            ...
            const addFile = (uri, isCurrentFile = false) => {
                ...
                if (workspaceFolder) {
                    ...
                    if (discovery.shouldIgnoreFile(uri.fsPath, {
            ...
            if (query) {
                console.log('[FileMessageHandler] Searching workspace files with fuzzy search for query', query);
                const workspaceFolders = vscode.workspace.workspaceFolders;
                if (workspaceFolders) {
                    for (const folder of workspaceFolders) {
                        const rootPath = folder.uri.fsPath;
                ...
                try {
                    ...
                    console.log('[FileMessageHandler] Sent initial workspaceFiles (open tabs/active)', files.length);
                }
                catch (e) {
                    console.warn('[FileMessageHandler] Failed sending initial response', e);
                }
                // If not enough files, add some workspace files (bounded)
                if (files.length < 10) {
                    const recentUris = await vscode.workspace.findFiles('**/*', '**/{.git,node_modules}/**', 20);
            ...
            console.log('[FileMessageHandler] Sent final workspaceFiles', files.length);
        }
        catch (error) {
            console.error('[FileMessageHandler] Failed to get workspace files:', error);
            const errorMsg = getErrorMessage(error);
            this.sendToWebView({
                type: 'error',
    ...
    async handleOpenFile(filePath) {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Failed to open file: ${getErrorMessage(error)}`);
        }
    }
    /**
     * Open diff view
     */
    async handleOpenDiff(data) {
        if (!data) {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Failed to open diff: ${getErrorMessage(error)}`);
        }
    }
    /**
     * Create and open temporary readonly file
     */
    async handleCreateAndOpenTempFile(data) {
        if (!data) {
            console.warn('[FileMessageHandler] No data provided for createAndOpenTempFile');
            return;
        }
        try {
            const content = data.content || '';
            const fileName = data.fileName || 'temp';
            ...
            if (foundExistingTab) {
                ...
                console.log('[FileMessageHandler] Focused on existing readonly file:', uri.toString(), 'in viewColumn:', existingViewColumn);
                return;
            }
            ...
            const targetViewColumn = findLeftGroupOfChatWebview() ??
            ...
            console.log('[FileMessageHandler] Created and opened readonly file:', uri.toString(), 'in viewColumn:', targetViewColumn ?? 'Beside');
        }
        catch (error) {
            console.error('[FileMessageHandler] Failed to create and open temporary file:', error);
            vscode.window.showErrorMessage(`Failed to create and open temporary file: ${getErrorMessage(error)}`);
        }
    }
