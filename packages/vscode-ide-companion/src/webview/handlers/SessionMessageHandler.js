...
import { getDisplayableImageMimeType, } from '../../utils/imageSupport.js';
import { processImageAttachments, buildPromptBlocks, } from '../utils/imageHandler.js';
import { isAuthenticationRequiredError } from '../../utils/authErrors.js';
import { getErrorMessage } from '../../utils/errorMessage.js';
import { stripZeroWidthSpaces } from '@qwen-code/webui';
import { exportSessionToFile, parseExportSlashCommand, } from '../../services/sessionExportService.js';
import { DISCONTINUED_MESSAGES, isDiscontinuedModel, } from '../utils/discontinuedModel.js';
function formatExportSuccessMessage(formatLabel, filename, filePath) {
...
export class SessionMessageHandler extends BaseMessageHandler {
    ...
    authHandler = null;
    isTitleSet = false; // Flag to track if title has been set
    ...
    async handle(message) {
        ...
        switch (message.type) {
            case 'sendMessage':
                await this.handleSendMessage(data?.text || '', data?.context, data?.fileContext, data?.attachments);
                break;
            case 'editMessage':
                await this.handleSendMessage(data?.text || '', data?.context, data?.fileContext, data?.attachments, typeof data?.targetTurnIndex === 'number'
            ...
            case 'getQwenSessions':
                await this.handleGetQwenSessions(data?.cursor ?? undefined, data?.size ?? undefined);
                break;
            case 'resumeSession':
                await this.handleResumeSession(data?.sessionId || '');
                break;
            case 'deleteQwenSession':
                await this.handleDeleteQwenSession(data?.sessionId || '');
                break;
            case 'renameQwenSession':
                await this.handleRenameQwenSession(data?.sessionId || '', data?.title || '');
                break;
            case 'openNewChatTab':
                ...
                try {
                    ...
                    logger.error('[SessionMessageHandler] Failed to open new chat tab:', error);
            ...
            case 'setApprovalMode':
                await this.handleSetApprovalMode(message.data);
                break;
            case 'setModel':
                await this.handleSetModel(message.data);
                break;
            default:
                logger.warn('[SessionMessageHandler] Unknown message type:', message.type);
                break;
        }
    }
    /**
    ...
    async captureConversationSnapshot(conversationId) {
        if (!conversationId) {
            return null;
        }
        ...
        const getSessionMessages = this.agentManager.getSessionMessages;
        if (!getSessionMessages) {
            return null;
        }
        const messages = await getSessionMessages.call(this.agentManager, conversationId);
    ...
    async restoreConversationSnapshot(snapshot) {
        if (!snapshot) {
            return;
        }
        const restored = await this.conversationStore.replaceMessages(snapshot.id, snapshot.messages);
        if (!restored) {
            logger.warn('[SessionMessageHandler] Failed to restore conversation snapshot; conversation not found:', snapshot.id);
        }
     ...
     */
    sendStreamEnd(reason, forRequestId) {
     ...
     */
    async promptAuthOrOffline(message) {
        const selection = await vscode.window.showWarningMessage(message, 'Configure', 'View Offline');
    ...
    shouldPromptAuth(error) {
        return isAuthenticationRequiredError(error);
    }
    async resolveSessionWorkingDir(sessionId) {
        try {
            const sessions = await this.agentManager.getSessionList();
            const match = sessions.find((session) => session.sessionId === sessionId || session.id === sessionId);
            if (typeof match?.cwd === 'string' && match.cwd.length > 0) {
                return match.cwd;
            }
        }
        catch (error) {
            logger.warn('[SessionMessageHandler] Failed to resolve export session cwd:', error);
        }
    ...
    async handleExportCommand(format) {
        ...
        const sessionId = this.agentManager.currentSessionId ?? this.currentConversationId;
        if (!sessionId) {
        ...
        try {
            ...
            this.sendToWebView({
                ...
                data: {
                    role: 'assistant',
                    content: formatExportSuccessMessage(formatLabel, result.filename, result.uri.fsPath),
                    timestamp: Date.now(),
                },
            });
        }
     ...
     */
    async handleSendMessage(text, context, fileContext, attachments, editTargetTurnIndex) {
        ...
        const { formattedText, displayText: updatedDisplayText, savedImageCount, promptImages, } = await processImageAttachments(promptText, attachments);
        ...
        if (hasAttachments && !trimmedText && savedImageCount === 0) {
            const errorMsg = 'Failed to attach the pasted image. Nothing was sent. Please paste the image again.';
        ...
        if (!this.currentConversationId) {
            logger.log('[SessionMessageHandler] No active conversation, creating one...');
            try {
        ...
        if (!this.currentConversationId) {
            const errorMsg = 'Failed to create conversation. Please restart the extension.';
            logger.error('[SessionMessageHandler]', errorMsg);
            vscode.window.showErrorMessage(errorMsg);
            this.sendToWebView({
                type: 'error',
        ...
        if (editTargetTurnIndex !== undefined) {
            ...
            if (!this.agentManager.isConnected) {
                await this.promptAuth('You need to configure your provider to use Qwen Code.');
                return;
            }
            if (!this.agentManager.currentSessionId) {
                try {
                    ...
                    logger.error('[SessionMessageHandler] Failed to create session before editing message:', createErr);
            ...
            try {
                editRestoreSnapshot = await this.captureConversationSnapshot(this.currentConversationId);
            }
            catch (error) {
                logger.error('[SessionMessageHandler] Failed to capture edit restore snapshot:', error);
                const errorMsg = this.getErrorMessage(error);
            ...
            if (!editRestoreSnapshot) {
                logger.warn('[SessionMessageHandler] Local conversation snapshot missing before edit; continuing with ACP rewind only.');
            }
            try {
                if (editRestoreSnapshot) {
                    const truncated = await this.conversationStore.truncateFromUserTurn(this.currentConversationId, editTargetTurnIndex);
                    if (!truncated) {
                        throw new Error('Conversation not found for edit target.');
                    }
                    editStoreMutationApplied = true;
                }
                const rewindResult = await this.agentManager.rewindSession(editTargetTurnIndex);
                ...
                if (editAcpMutationApplied && editAcpHistorySnapshot) {
                    try {
                        ...
                        logger.warn('[SessionMessageHandler] Failed to restore ACP history after rewind failure:', restoreError);
                    }
                }
                ...
                const errorMsg = this.getErrorMessage(error);
                logger.error('[SessionMessageHandler] Failed to rewind session:', error);
                vscode.window.showErrorMessage(`Failed to edit message: ${errorMsg}`);
                this.sendToWebView({
                    type: 'error',
                    data: { message: errorMsg },
        ...
        if (editTargetTurnIndex !== undefined) {
            ...
            try {
                const conversation = await this.conversationStore.getConversation(this.currentConversationId);
                isFirstMessage = !conversation || conversation.messages.length === 0;
            }
            catch (error) {
                logger.error('[SessionMessageHandler] Failed to check conversation:', error);
            }
        }
        ...
        try {
            await this.conversationStore.addMessage(this.currentConversationId, userMessage);
        }
        catch (error) {
            logger.error('[SessionMessageHandler] Failed to save user message:', error);
            if (editAcpMutationApplied && editAcpHistorySnapshot) {
                try {
                    await this.agentManager.restoreSessionHistory(editAcpHistorySnapshot);
                }
                catch (restoreError) {
                    logger.warn('[SessionMessageHandler] Failed to restore ACP history after user message save failure:', restoreError);
                }
            }
            if (editStoreMutationApplied) {
                await this.restoreConversationSnapshot(editRestoreSnapshot);
        ...
        if (!this.agentManager.currentSessionId) {
            try {
                ...
                logger.error('[SessionMessageHandler] Failed to create session before sending message:', createErr);
                const errorMsg = this.getErrorMessage(createErr);
                if (this.shouldPromptAuth(createErr)) {
                    await this.promptAuth('Your session has expired or is invalid. Please configure your provider to continue using Qwen Code.');
                    return;
        ...
        try {
            ...
            await this.agentManager.sendMessage(buildPromptBlocks(promptText, promptImages));
            ...
            if (this.currentStreamContent && this.currentConversationId) {
                ...
                await this.conversationStore.addMessage(this.currentConversationId, assistantMessage);
            }
            ...
            if (acpSessionId && acpSessionId !== this.currentConversationId) {
                ...
                if (previousConversationId) {
                    const renamed = await this.conversationStore.renameConversationId(previousConversationId, acpSessionId);
                    if (!renamed) {
                        logger.warn('[SessionMessageHandler] Failed to align conversation store with ACP session id:', previousConversationId, acpSessionId);
                        return;
                    }
                }
                ...
                this.sendToWebView({
                    ...
                    data: {
                        ...
                        title: displayText.substring(0, 50) +
            ...
            if (editAcpMutationApplied && editAcpHistorySnapshot) {
                try {
                    ...
                    logger.warn('[SessionMessageHandler] Failed to restore ACP history after send failure:', restoreError);
                }
            }
            if (editStoreMutationApplied) {
                await this.restoreConversationSnapshot(editRestoreSnapshot);
            ...
            const isAbortLike = (err && err.name === 'AbortError') ||
            ...
            if (errorMsg.includes('Session not found') ||
                ...
                const isTimeoutError = lower.includes('timeout') || lower.includes('timed out');
                if (isTimeoutError) {
                    ...
                    logger.warn('[SessionMessageHandler] Request timed out; suppressing popup');
                    const timeoutMessage = {
                        role: 'assistant',
                        content: 'Request timed out. This may be due to a network issue. Please try again.',
                        timestamp: Date.now(),
                    };
    ...
    async handleNewQwenSession() {
        try {
            ...
            logger.error('[SessionMessageHandler] Failed to create new session:', error);
            // Safely convert error to string
            const errorMsg = this.getErrorMessage(error);
            // Check for authentication/session expiration errors
            if (this.shouldPromptAuth(error)) {
                // Show a more user-friendly error message for expired sessions
                await this.promptAuth('Your session has expired or is invalid. Please configure your provider to create a new session.');
                // Send a specific error to the webview for better UI handling
                this.sendToWebView({
                    type: 'sessionExpired',
                    data: { message: 'Session expired. Please authenticate again.' },
     ...
     */
    async handleSwitchQwenSession(sessionId) {
        try {
            ...
            if (!this.agentManager.isConnected) {
                const choice = await this.promptAuthOrOffline('You are not authenticated. Configure your provider to fully restore this session, or view it offline.');
                if (choice === 'offline') {
                    ...
                    vscode.window.showInformationMessage('Showing cached session content. Configure your provider to interact with the AI.');
                    return;
                }
                else if (choice !== 'auth') {
            ...
            try {
                ...
                sessionDetails =
                    allSessions.find((s) => s.id === sessionId || s.sessionId === sessionId) || null;
            }
            catch (err) {
                logger.log('[SessionMessageHandler] Could not get session details:', err);
            }
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            const workingDir = workspaceFolder?.uri.fsPath || process.cwd();
            ...
            try {
                ...
                const loadResponse = await this.agentManager.loadSessionViaAcp(sessionId, sessionDetails?.cwd || undefined);
                logger.log('[SessionMessageHandler] session/load succeeded (per ACP spec result is null; actual history comes via session/update):', loadResponse);
                ...
                logger.warn('[SessionMessageHandler] session/load failed, using fallback:', loadError);
                // Check for authentication/session expiration errors
                if (this.shouldPromptAuth(loadError)) {
                    // Show a more user-friendly error message for expired sessions
                    await this.promptAuth('Your session has expired or is invalid. Please configure your provider to switch sessions.');
                    // Send a specific error to the webview for better UI handling
                    this.sendToWebView({
                        type: 'sessionExpired',
                        data: { message: 'Session expired. Please authenticate again.' },
                ...
                if (this.agentManager.isConnected) {
                    try {
                        ...
                        if (loadError &&
                            ...
                            !('result' in loadError)) {
                            vscode.window.showWarningMessage('Session restored from local cache. Some context may be incomplete.');
                        }
                    }
                    catch (createError) {
                        logger.error('[SessionMessageHandler] Failed to create session:', createError);
                    ...
                    vscode.window.showWarningMessage('Showing cached session content. Configure your provider to interact with the AI.');
                }
            }
        }
        catch (error) {
     ...
     */
    async handleGetQwenSessions(cursor, size) {
        try {
            ...
            if (this.shouldPromptAuth(error)) {
                // Show a more user-friendly error message for expired sessions
                await this.promptAuth('Your session has expired or is invalid. Please configure your provider to view sessions.');
                // Send a specific error to the webview for better UI handling
                this.sendToWebView({
                    type: 'sessionExpired',
                    data: { message: 'Session expired. Please authenticate again.' },
        ...
        try {
            ...
            try {
                ...
                if (this.shouldPromptAuth(acpError)) {
                    // Show a more user-friendly error message for expired sessions
                    await this.promptAuth('Your session has expired or is invalid. Please configure your provider to resume sessions.');
                    // Send a specific error to the webview for better UI handling
                    this.sendToWebView({
                        type: 'sessionExpired',
                        data: { message: 'Session expired. Please authenticate again.' },
     ...
     */
    async handleDeleteQwenSession(sessionId) {
        try {
     ...
     */
    async handleRenameQwenSession(sessionId, title) {
        try {
            ...
            const success = await this.agentManager.renameSession(sessionId, trimmedTitle);
            if (success) {
                this.sendToWebView({
                    type: 'sessionRenamed',
     ...
     */
    async handleSetApprovalMode(data) {
        try {
    ...
    async handleSetModel(data) {
        try {
            ...
            if (isDiscontinuedModel(modelId)) {
                logger.warn('[SessionMessageHandler] Rejected discontinued model', modelId);
            ...
            void vscode.window.showInformationMessage(`Model switched to: ${modelId}`);
        }
        catch (error) {
            const errorMsg = this.getErrorMessage(error);
...
//# sourceMappingURL=SessionMessageHandler.js.map