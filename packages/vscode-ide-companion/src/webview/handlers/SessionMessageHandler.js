...
export class SessionMessageHandler extends BaseMessageHandler {
        ...
        switch (message.type) {
            ...
            case 'openNewChatTab':
                ...
                try {
                    ...
                    console.error('[SessionMessageHandler] Failed to open new chat tab:', error);
            ...
            case 'setModel':
                ...
                break;
            default:
                console.warn('[SessionMessageHandler] Unknown message type:', message.type);
                break;
        }
    }
    /**
    ...
    async restoreConversationSnapshot(snapshot) {
        ...
        if (!restored) {
            console.warn('[SessionMessageHandler] Failed to restore conversation snapshot; conversation not found:', snapshot.id);
        }
    ...
    async resolveSessionWorkingDir(sessionId) {
        try {
            ...
            console.warn('[SessionMessageHandler] Failed to resolve export session cwd:', error);
        }
    ...
    async handleSendMessage(text, context, fileContext, attachments, editTargetTurnIndex) {
        ...
        if (!this.currentConversationId) {
            console.log('[SessionMessageHandler] No active conversation, creating one...');
            try {
        ...
        if (editTargetTurnIndex !== undefined) {
            ...
            if (!this.agentManager.currentSessionId) {
                try {
                    ...
                    console.error('[SessionMessageHandler] Failed to create session before editing message:', createErr);
            ...
            try {
                ...
                console.error('[SessionMessageHandler] Failed to capture edit restore snapshot:', error);
                const errorMsg = this.getErrorMessage(error);
            ...
            if (!editRestoreSnapshot) {
                console.warn('[SessionMessageHandler] Local conversation snapshot missing before edit; continuing with ACP rewind only.');
            }
            try {
                if (editRestoreSnapshot) {
                ...
                if (editAcpMutationApplied && editAcpHistorySnapshot) {
                    try {
                        ...
                        console.warn('[SessionMessageHandler] Failed to restore ACP history after rewind failure:', restoreError);
                    }
                }
                ...
                const errorMsg = this.getErrorMessage(error);
                console.error('[SessionMessageHandler] Failed to rewind session:', error);
                vscode.window.showErrorMessage(`Failed to edit message: ${errorMsg}`);
                this.sendToWebView({
                    type: 'error',
                    data: { message: errorMsg },
        ...
        if (editTargetTurnIndex !== undefined) {
            ...
            try {
                ...
                console.error('[SessionMessageHandler] Failed to check conversation:', error);
            }
        }
        ...
        try {
            ...
            console.error('[SessionMessageHandler] Failed to save user message:', error);
            if (editAcpMutationApplied && editAcpHistorySnapshot) {
                try {
                    await this.agentManager.restoreSessionHistory(editAcpHistorySnapshot);
                }
                catch (restoreError) {
                    console.warn('[SessionMessageHandler] Failed to restore ACP history after user message save failure:', restoreError);
                }
            }
            if (editStoreMutationApplied) {
                await this.restoreConversationSnapshot(editRestoreSnapshot);
        ...
        if (!this.agentManager.currentSessionId) {
            try {
                ...
                console.error('[SessionMessageHandler] Failed to create session before sending message:', createErr);
                const errorMsg = this.getErrorMessage(createErr);
                if (this.shouldPromptAuth(createErr)) {
                    await this.promptAuth('Your session has expired or is invalid. Please configure your provider to continue using Qwen Code.');
                    return;
        ...
        try {
            ...
            if (acpSessionId && acpSessionId !== this.currentConversationId) {
                ...
                if (previousConversationId) {
                    ...
                    if (!renamed) {
                        console.warn('[SessionMessageHandler] Failed to align conversation store with ACP session id:', previousConversationId, acpSessionId);
                        return;
                    }
                }
            ...
            if (editAcpMutationApplied && editAcpHistorySnapshot) {
                try {
                    ...
                    console.warn('[SessionMessageHandler] Failed to restore ACP history after send failure:', restoreError);
                }
            }
            if (editStoreMutationApplied) {
                await this.restoreConversationSnapshot(editRestoreSnapshot);
            ...
            if (errorMsg.includes('Session not found') ||
                ...
                if (isTimeoutError) {
                    ...
                    console.warn('[SessionMessageHandler] Request timed out; suppressing popup');
    ...
    async handleNewQwenSession() {
        try {
            ...
            console.error('[SessionMessageHandler] Failed to create new session:', error);
            // Safely convert error to string
            const errorMsg = this.getErrorMessage(error);
    ...
    async handleSwitchQwenSession(sessionId) {
        try {
            ...
            try {
                ...
                console.log('[SessionMessageHandler] Could not get session details:', err);
            }
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            const workingDir = workspaceFolder?.uri.fsPath || process.cwd();
            ...
            try {
                ...
                console.log('[SessionMessageHandler] session/load succeeded (per ACP spec result is null; actual history comes via session/update):', loadResponse);
                ...
                console.warn('[SessionMessageHandler] session/load failed, using fallback:', loadError);
                // Check for authentication/session expiration errors
                ...
                if (this.agentManager.isConnected) {
                    try {
                        ...
                        console.error('[SessionMessageHandler] Failed to create session:', createError);
    ...
    async handleSetModel(data) {
        try {
            ...
            if (isDiscontinuedModel(modelId)) {
                console.warn('[SessionMessageHandler] Rejected discontinued model', modelId);
