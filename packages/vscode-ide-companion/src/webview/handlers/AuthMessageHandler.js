...
import { ALL_PROVIDERS, ALIBABA_PROVIDERS, AuthType, THIRD_PARTY_PROVIDERS, shouldShowStep, resolveBaseUrl, getDefaultBaseUrlForProtocol, getDefaultModelIds, } from '@qwen-code/qwen-code-core';
/**
...
export class AuthMessageHandler extends BaseMessageHandler {
     ...
     */
    async pick(items, title, placeHolder) {
     ...
     */
    async handleAuthInteractive() {
        try {
            // Build grouped provider menu
            const items = [];
            const addGroup = (label, providers) => {
            ...
            const customProviders = ALL_PROVIDERS.filter((p) => p.uiGroup === 'custom');
            ...
            const selectedId = await this.pick(items, 'Qwen Code: Select Provider', 'Choose how to connect');
            if (!selectedId)
                return;
    ...
    // ---------------------------------------------------------------------------
    async runProviderSetupFlow(provider) {
        const flowTitle = provider.uiLabels?.flowTitle ?? `Qwen Code: ${provider.label}`;
        // Step 0: Protocol (only for providers offering multiple, e.g. custom)
        let protocol;
        if (shouldShowStep(provider, 'protocol') &&
            ...
            const selected = await this.pick(provider.protocolOptions.map((p) => ({
                label: protocolLabels[String(p)] ?? String(p),
                value: String(p),
            })), `${flowTitle}: Protocol`, 'Select API protocol');
            if (!selected)
                return;
            protocol = selected;
        }
        ...
        if (shouldShowStep(provider, 'baseUrl')) {
            if (Array.isArray(provider.baseUrl)) {
                const options = provider.baseUrl;
                const stepTitle = provider.uiLabels?.baseUrlStepTitle ?? 'Endpoint';
                const selected = await this.pick(options.map((opt) => ({
                    label: opt.label,
                    description: opt.url,
                    value: opt.url,
                })), `${flowTitle}: ${stepTitle}`, `Select ${stepTitle.toLowerCase()}`);
                if (!selected)
                    return;
                baseUrl = selected;
            }
        ...
        if (shouldShowStep(provider, 'advancedConfig')) {
            ...
            ], `${flowTitle}: Advanced Config`, 'Enable thinking mode?');
            if (!enableThinking)
                return;
            advancedConfig = {
        ...
        if (!this.authInteractiveHandler) {
            console.error('[AuthMessageHandler] authInteractiveHandler not set; cannot apply provider config.');
            // No authCancelled — see the base-URL validation note above.
            this.sendToWebView({
                type: 'authError',
                data: {
                    message: 'Auth handler not initialized. Please reopen the panel and try again.',
                },
            });
            return;
        }
