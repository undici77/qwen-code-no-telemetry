...
export class EditorMessageHandler extends BaseMessageHandler {
    ...
    async handle(message) {
        switch (message.type) {
            ...
            case 'focusActiveEditor':
                ...
                break;
            default:
                console.warn('[EditorMessageHandler] Unknown message type:', message.type);
                break;
        }
    }
    /**
    ...
    async handleGetActiveEditor() {
        try {
            ...
            console.error('[EditorMessageHandler] Failed to get active editor:', error);
        }
    }
    /**
     * Focus on active editor
     */
    async handleFocusActiveEditor() {
        try {
            const activeEditor = vscode.window.activeTextEditor;
            if (activeEditor) {
            ...
            console.error('[EditorMessageHandler] Failed to focus active editor:', error);
            vscode.window.showErrorMessage(`Failed to focus editor: ${getErrorMessage(error)}`);
        }
    }
}
