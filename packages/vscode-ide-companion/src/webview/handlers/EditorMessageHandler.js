...
export class EditorMessageHandler extends BaseMessageHandler {
    ...
    async handle(message) {
        switch (message.type) {
            ...
            case 'focusActiveEditor':
                ...
                break;
            default:
                logger.warn('[EditorMessageHandler] Unknown message type:', message.type);
                break;
        }
    }
    /**
    ...
    async handleGetActiveEditor() {
        try {
            ...
            logger.error('[EditorMessageHandler] Failed to get active editor:', error);
        }
    }
    /**
    ...
    async handleFocusActiveEditor() {
        try {
            ...
            logger.error('[EditorMessageHandler] Failed to focus active editor:', error);
