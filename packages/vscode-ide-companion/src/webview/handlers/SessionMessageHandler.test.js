...
const { mockProcessImageAttachments, mockShowErrorMessage, mockExportSessionToFile, } = vi.hoisted(() => ({
...
vi.mock('vscode', () => ({
    ...
    Uri: {
        file: (fsPath) => ({
            fsPath,
            toString: () => `file://${encodeURI(fsPath.replace(/\\/g, '/')).replace(/#/g, '%23')}`,
        }),
    },
}));
...
describe('SessionMessageHandler', () => {
    ...
    it('does not create conversation state or send an empty prompt when all pasted images fail to materialize', async () => {
        ...
        const handler = new SessionMessageHandler(agentManager, conversationStore, 'conversation-1', sendToWebView);
        await handler.handle({
            type: 'sendMessage',
            data: {
                text: '',
    ...
    it('sends formatted prompt text so session restore can reconstruct pasted images', async () => {
        ...
        const sendToWebView = vi.fn();
        const handler = new SessionMessageHandler(agentManager, conversationStore, null, sendToWebView);
        await handler.handle({
            type: 'sendMessage',
            data: {
                text: '这是什么内容',
    ...
    it('sends image file context as prompt image blocks', async () => {
        mockProcessImageAttachments.mockImplementation(async (promptText) => ({
        ...
        const handler = new SessionMessageHandler(agentManager, conversationStore, 'conversation-1', vi.fn());
        await handler.handle({
            type: 'sendMessage',
            data: {
                text: 'describe it',
    ...
    it('keeps the conversation store aligned with the ACP session id before editing', async () => {
        ...
        const conversationStore = {
            ...
            getConversation: vi.fn(async (id) => conversation.id === id ? conversation : null),
            addMessage: vi.fn(async (id, message) => {
                if (conversation.id === id) {
                    conversation.messages.push(message);
                }
            }),
            renameConversationId: vi.fn(async (fromId, toId) => {
        ...
        expect(conversationStore.renameConversationId).toHaveBeenCalledWith('conversation-1', 'session-1');
        expect(conversationStore.getConversation).toHaveBeenCalledWith('session-1');
        expect(conversationStore.truncateFromUserTurn).toHaveBeenCalledWith('session-1', 0);
    ...
    it('does not switch to a colliding ACP session id when rename fails', async () => {
        ...
        const syncCurrentConversationId = vi.fn((id) => {
            handlerRef.current?.setCurrentConversationId(id);
        });
        const handler = new SessionMessageHandler(agentManager, conversationStore, null, sendToWebView, syncCurrentConversationId);
    ...
    it('rewinds the active ACP session before sending an edited message', async () => {
        ...
        const sendToWebView = vi.fn();
        const handler = new SessionMessageHandler(agentManager, conversationStore, 'session-1', sendToWebView);
        await handler.handle({
            type: 'editMessage',
            data: {
                text: 'edited prompt',
        ...
        expect(conversationStore.truncateFromUserTurn).toHaveBeenCalledWith('session-1', 1);
        ...
        expect(conversationStore.truncateFromUserTurn.mock.invocationCallOrder[0]).toBeLessThan(agentManager.rewindSession.mock.invocationCallOrder[0]);
        expect(agentManager.rewindSession.mock.invocationCallOrder[0]).toBeLessThan(agentManager.sendMessage.mock.invocationCallOrder[0]);
    });
    it('restores the edited conversation snapshot when replacement send fails', async () => {
        ...
        expect(conversationStore.replaceMessages).toHaveBeenCalledWith('session-1', originalConversation.messages);
        expect(sendToWebView).toHaveBeenCalledWith({
    ...
    it('restores store and ACP history when saving the edited user message fails', async () => {
        ...
        expect(agentManager.restoreSessionHistory).toHaveBeenCalledWith(historyBeforeRewind);
        expect(conversationStore.replaceMessages).toHaveBeenCalledWith('session-1', originalConversation.messages);
        expect(agentManager.sendMessage).not.toHaveBeenCalled();
        expect(sendToWebView).toHaveBeenCalledWith({
            type: 'error',
    ...
    it('intercepts /export html and uses the VSCode export flow instead of sending a prompt', async () => {
        ...
        expect(sendToWebView).toHaveBeenCalledWith({
            ...
            data: expect.objectContaining({
                role: 'assistant',
                content: 'Session exported to HTML: [export.html](file:///workspace/export.html)',
            }),
        });
    });
    it('prefers the active ACP session id over the local conversation id when exporting', async () => {
        ...
        const sendToWebView = vi.fn();
        const handler = new SessionMessageHandler(agentManager, conversationStore, 'conv_local_123', sendToWebView);
        await handler.handle({
            type: 'sendMessage',
            data: {
                text: '/export html',
    ...
    it('encodes exported file links before rendering markdown', async () => {
        ...
        expect(sendToWebView).toHaveBeenCalledWith({
            ...
            data: expect.objectContaining({
                role: 'assistant',
                content: 'Session exported to HTML: [export (#1).html](file:///workspace/export%20(%231).html)',
            }),
        });
    });
    describe('handleSetModel — discontinued model defensive validation (Issue #3745)', () => {
        it('rejects a non-runtime Qwen OAuth model and surfaces an error', async () => {
            ...
            const sendToWebView = vi.fn();
            const handler = new SessionMessageHandler(agentManager, {}, null, sendToWebView);
            await handler.handle({
                type: 'setModel',
                data: { modelId: 'qwen3-coder-plus(qwen-oauth)' },
            });
            expect(setModelFromUi).not.toHaveBeenCalled();
            expect(mockShowErrorMessage).toHaveBeenCalledWith(expect.stringContaining('Qwen OAuth free tier was discontinued on 2026-04-15'));
            expect(sendToWebView).toHaveBeenCalledWith({
                type: 'error',
                data: expect.objectContaining({
        ...
        it('allows a runtime Qwen OAuth snapshot to pass through', async () => {
            ...
            expect(setModelFromUi).toHaveBeenCalledWith('$runtime|qwen-oauth|qwen3-coder-plus(qwen-oauth)');
...
//# sourceMappingURL=SessionMessageHandler.test.js.map