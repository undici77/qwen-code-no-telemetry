...
export function getLocalResourceRoots(extensionUri, workspaceFolders) {
    const roots = [
...
export class PanelManager {
    ...
    panelGroupViewColumn = null;
    constructor(extensionUri, onPanelDispose) {
        this.extensionUri = extensionUri;
        this.onPanelDispose = onPanelDispose;
    }
    /**
    ...
    async createPanel() {
        ...
        if (existingGroup) {
            ...
            console.log('[PanelManager] Found existing Qwen Code group, creating panel in same group');
            this.panel = vscode.window.createWebviewPanel('qwenCode.chat', 'Qwen Code', { viewColumn: existingGroup.viewColumn, preserveFocus: false }, {
                enableScripts: true,
            ...
            try {
                ...
                console.warn('[PanelManager] Failed to create right editor group (continuing):', error);
                // Fallback: create in current group
                const activeColumn = vscode.window.activeTextEditor?.viewColumn || vscode.ViewColumn.One;
                this.panel = vscode.window.createWebviewPanel('qwenCode.chat', 'Qwen Code', { viewColumn: activeColumn, preserveFocus: false }, {
                    enableScripts: true,
                    retainContextWhenHidden: true,
                    localResourceRoots: getLocalResourceRoots(this.extensionUri, vscode.workspace.workspaceFolders),
                });
            ...
            this.panel = vscode.window.createWebviewPanel('qwenCode.chat', 'Qwen Code', { viewColumn: newGroupColumn, preserveFocus: false }, {
                enableScripts: true,
                retainContextWhenHidden: true,
                localResourceRoots: getLocalResourceRoots(this.extensionUri, vscode.workspace.workspaceFolders),
            });
        ...
        this.panel.iconPath = vscode.Uri.joinPath(this.extensionUri, 'assets', 'icon.png');
     ...
     */
    findExistingQwenCodeGroup() {
        for (const group of vscode.window.tabGroups.all) {
            for (const tab of group.tabs) {
                const input = tab.input;
                const isWebviewInput = (inp) => !!inp && typeof inp === 'object' && 'viewType' in inp;
                if (isWebviewInput(input) &&
     ...
     */
    revealPanel(preserveFocus = true) {
        if (this.panel) {
            // Prefer revealing in the currently tracked group to avoid reflowing groups.
            const trackedColumn = this.panelTab?.group?.viewColumn;
            const targetColumn = trackedColumn ??
    ...
    captureTab() {
        ...
        setTimeout(() => {
            ...
            try {
                const groupViewColumn = this.panelTab?.group?.viewColumn;
                if (groupViewColumn !== null) {
                    this.panelGroupViewColumn = groupViewColumn;
                }
            }
            catch {
     ...
     */
    registerDisposeHandler(disposables) {
        if (!this.panel) {
            return;
        }
        this.panel.onDidDispose(() => {
            ...
            this.panelTab?.group?.viewColumn ??
                    ...
                    try {
                        ...
                        if (group && group.tabs.length === 0) {
                            ...
                            try {
                                ...
                                try {
                                    await vscode.commands.executeCommand('workbench.action.removeActiveEditorGroup');
                                }
                                catch (err) {
                                    console.warn('[PanelManager] Failed to close empty group after Qwen panel disposed:', err);
                                }
                            }
                        }
                    }
                    catch (err) {
                        console.warn('[PanelManager] Error while trying to close empty Qwen group:', err);
                    }
                }, 50);
            }
        }, null, disposables);
     ...
     */
    async focusGroupByColumn(target) {
        const maxHops = 20; // safety guard for unusual layouts
        let hops = 0;
        while (vscode.window.tabGroups.activeTabGroup.viewColumn !== target &&
            hops < maxHops) {
     ...
     * @param disposables Array used to store Disposable objects
     */
    registerViewStateChangeHandler(disposables) {
        if (!this.panel) {
            return;
        }
