...
import {} from '../../types/approvalModeValueTypes.js';
import { isAuthenticationRequiredError } from '../../utils/authErrors.js';
import { getErrorMessage } from '../../utils/errorMessage.js';
import { applyProviderInstallPlanToFile, snapshotSettingsForRollback, restoreSettingsSnapshot, writeCodingPlanConfig, readQwenSettingsForVSCode, clearPersistedAuth, } from '../../services/settingsWriter.js';
import { buildInstallPlan, parseInsightMessage, } from '@qwen-code/qwen-code-core';
...
export function resolveQwenCliEntryPath(extensionUri, extensionMode) {
    if (extensionMode === vscode.ExtensionMode.Development) {
        const devCliEntry = path.resolve(extensionUri.fsPath, '..', '..', 'scripts', 'dev.js');
...
function isInsightCommand(command) {
...
export class WebViewProvider {
    ...
    conversationStore;
    disposables = [];
    agentInitialized = false; // Track if agent has been initialized
    isSyncingToVSCode = false; // Guard to prevent config change loop
    ...
    pendingAskUserQuestionRequest = null;
    ...
    static lastContextMenuProvider = null;
    ...
    constructor(context, extensionUri) {
        ...
        this.messageHandler = new MessageHandler(this.agentManager, this.conversationStore, null, (message) => this.sendMessageToWebView(message));
        // Set auth interactive handler — interactive auth flow (QuickPick → InputBox → write settings → reconnect)
        this.messageHandler.setAuthInteractiveHandler(async (providerConfig, inputs) => {
        ...
        const configChangeDisposable = vscode.workspace.onDidChangeConfiguration(async (e) => {
            const authSettingsChanged = AUTH_RELATED_QWEN_SETTINGS.some((setting) => e.affectsConfiguration(setting));
            if (authSettingsChanged && !this.isSyncingToVSCode) {
                logger.log('[WebViewProvider] Auth-related qwen-code settings changed by user, syncing...');
                ...
                if (synced && this.agentInitialized) {
                    ...
                    try {
                        ...
                        logger.error('[WebViewProvider] Reconnect after settings change failed:', e);
                    }
                }
                else if (!synced &&
                    ...
                    const apiKey = vscode.workspace
                        ...
                        .get('apiKey', '');
                    if (!apiKey) {
                        logger.log('[WebViewProvider] apiKey cleared — de-authenticating and clearing persisted credentials');
                        clearPersistedAuth();
                        this.agentManager.disconnect();
                        this.agentInitialized = false;
            ...
            if (message.source?.startsWith('background_notification')) {
                ...
                if (conversationId) {
                    void this.conversationStore
                        ...
                        .catch((error) => {
                        logger.warn('[WebViewProvider] Failed to persist background notification:', error);
                    });
                }
            }
            this.sendMessageToWebView({
        ...
        this.agentManager.onAvailableModels((models) => {
            logger.log('[WebViewProvider] onAvailableModels received, sending to webview:', models);
        ...
        this.agentManager.onToolCall((update) => {
            ...
            let messageType = updateData.sessionUpdate;
            if (!messageType) {
            ...
            const pendingToolCallId = this.pendingPermissionRequest?.toolCall?.toolCallId;
            const updateToolCallId = updateData.toolCallId;
            const updateStatus = updateData.status;
            if (typeof updateToolCallId === 'string' &&
        ...
        this.agentManager.onPermissionRequest(async (request) => {
            // Notify the user immediately (dot + optional system notification)
            const toolTitle = request.toolCall
                ?.title;
            ...
            return new Promise((resolve) => {
                ...
                const handler = (message) => {
                    ...
                    const isSwitchMode = request.toolCall?.kind ===
                        'switch_mode';
                    const isWorkflowApproval = request.toolCall?._meta?.workflowApproval === true;
                        ...
                        void (async () => {
                            if (!isSwitchMode && !isWorkflowApproval) {
                                try {
                                    ...
                                    logger.warn('[WebViewProvider] cancelCurrentPrompt error:', err);
                                }
                            ...
                            try {
                                const toolCallId = request.toolCall
                                    ?.toolCallId || '';
                                const title = request.toolCall
                                    ?.title || '';
                                let kind = (request.toolCall?.kind || 'execute');
                                if (!kind && title) {
                                ...
                                logger.warn('[WebViewProvider] failed to synthesize failed tool_call_update:', err);
                            }
                        })();
                    }
                    else {
        ...
        this.agentManager.onAskUserQuestion(async (request) => {
            // Notify the user immediately (dot + optional system notification)
        ...
        this.agentManager.onDisconnected((code, signal) => {
            logger.log(`[WebViewProvider] Agent disconnected (code: ${code}, signal: ${signal})`);
    ...
    async openInsightReport(path) {
        await vscode.env.openExternal(vscode.Uri.file(path));
    }
    async handleOpenInsightReportMessage(message) {
     ...
     */
    async attachToView(webviewView, viewType) {
        logger.log(`[WebViewProvider] Attaching to WebviewView (viewType=${viewType})`);
        const webview = webviewView.webview;
        // Configure webview options
        webview.options = {
            enableScripts: true,
            localResourceRoots: getLocalResourceRoots(this.extensionUri, vscode.workspace.workspaceFolders),
        };
        ...
        logger.log('[WebViewProvider] Attempting to restore auth state and connection for view...');
    ...
    async show() {
        ...
        newPanel.onDidChangeViewState(() => {
            logger.log('[WebViewProvider] Panel view state changed, triggering serialization check');
        }, null, this.disposables);
        ...
        newPanel.webview.html = WebViewContent.generate(newPanel.webview, this.extensionUri);
        // Handle messages from WebView
        newPanel.webview.onDidReceiveMessage(async (message) => {
            ...
            if (message.type === 'updatePanelTitle') {
                const title = String(message.data?.title ?? '').trim();
        ...
        logger.log('[WebViewProvider] Attempting to restore auth state and connection...');
        await this.attemptAuthStateRestoration();
    }
    /**
     ...
     */
    async syncVSCodeSettingsToQwenConfig() {
        ...
        try {
            ...
            if (provider !== 'coding-plan') {
                logger.log('[WebViewProvider] Skipping VSCode settings sync for api-key provider; interactive auth owns provider details');
                return false;
            }
            const region = config.get('codingPlanRegion', 'china');
            writeCodingPlanConfig(region, apiKey);
            logger.log(`[WebViewProvider] Synced VSCode settings → ~/.qwen/settings.json (provider=${provider})`);
            return true;
        }
        catch (error) {
     ...
     */
    async syncQwenConfigToVSCodeSettings() {
        try {
            const qwenSettings = readQwenSettingsForVSCode();
            if (!qwenSettings) {
                return;
            }
            logger.log('[WebViewProvider] Syncing ~/.qwen/settings.json → VSCode settings');
            // Set guard to prevent onDidChangeConfiguration from triggering a write-back
            const config = vscode.workspace.getConfiguration('qwen-code');
            const target = vscode.ConfigurationTarget.Global;
            const updates = [];
            if (config.get('provider', 'coding-plan') !== qwenSettings.provider) {
                updates.push(config.update('provider', qwenSettings.provider, target));
            }
            if (config.get('codingPlanRegion', 'china') !==
                qwenSettings.codingPlanRegion) {
                updates.push(config.update('codingPlanRegion', qwenSettings.codingPlanRegion, target));
            }
            if (updates.length === 0) {
                logger.log('[WebViewProvider] VSCode settings already match ~/.qwen/settings.json');
                return;
            }
            ...
            logger.error('[WebViewProvider] Failed to sync qwen config to VSCode settings:', error);
        }
    }
    /**
     * Attempt to restore authentication state and initialize connection.
     ...
     */
    async attemptAuthStateRestoration() {
        ...
        this.initializationPromise = (async () => {
            try {
                ...
                logger.error('[WebViewProvider] Error in attemptAuthStateRestoration:', error);
     ...
     */
    async doInitializeAgentConnection(options) {
        const autoAuthenticate = options?.autoAuthenticate ?? true;
        const run = async () => {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            const workingDir = workspaceFolder?.uri.fsPath || process.cwd();
            logger.log('[WebViewProvider] Starting initialization, workingDir:', workingDir);
            logger.log(`[WebViewProvider] Using CLI-managed authentication (autoAuth=${autoAuthenticate})`);
            const cliEntry = resolveQwenCliEntryPath(this.extensionUri, this.context.extensionMode);
            try {
                logger.log('[WebViewProvider] Connecting to agent...');
                // Pass the detected CLI path to ensure we use the correct installation
                const connectResult = await this.agentManager.connect(workingDir, cliEntry, options);
                ...
                if (connectResult.requiresAuth && !autoAuthenticate) {
                    logger.log('[WebViewProvider] Authentication required, launching auth flow...');
                    this.sendMessageToWebView({
                        type: 'authState',
                        data: { authenticated: false },
                    });
                ...
                if (sessionReady) {
                    ...
                    logger.log('[WebViewProvider] Session creation deferred until user logs in.');
                }
            }
            catch (_error) {
                const errorMsg = getErrorMessage(_error);
                logger.error('[WebViewProvider] Agent connection error:', _error);
                vscode.window.showWarningMessage(`Failed to connect to Qwen CLI: ${errorMsg}\nYou can still use the chat UI, but messages won't be sent to AI.`);
     ...
     */
    async handleAuthInteractive(providerConfig, inputs) {
        if (!inputs.apiKey) {
            this.sendMessageToWebView({
                type: 'authError',
        ...
        logger.log(`[WebViewProvider] authInteractive: provider=${providerConfig.id}, host=${baseUrlHost}`);
        ...
        const safeRollback = () => {
            try {
                ...
                logger.error('[WebViewProvider] settings rollback failed:', rollbackErr);
            }
        };
        ...
        try {
            ...
            if (this.authState === true) {
                ...
                this.sendMessageToWebView({
                    type: 'authError',
                    data: {
                        message: 'Connection established but authentication failed. Please check your credentials.',
                    },
                });
            }
        }
    ...
    async attemptAutoReconnect() {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            logger.log(`[WebViewProvider] Auto-reconnect attempt ${attempt}/${maxAttempts}`);
            ...
            try {
                ...
                logger.error(`[WebViewProvider] Auto-reconnect attempt ${attempt} failed:`, error);
            }
        }
        ...
        this.sendMessageToWebView({
            type: 'agentConnectionError',
            data: {
                message: 'Lost connection to Qwen agent and auto-reconnect failed. Please use the refresh button to try again.',
            },
        });
    }
    /**
     ...
     */
    async loadCurrentSessionMessages(options) {
        const autoAuthenticate = options?.autoAuthenticate ?? true;
        let sessionReady = false;
        try {
            logger.log('[WebViewProvider] Initializing with new session (skipping restoration)');
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            const workingDir = workspaceFolder?.uri.fsPath || process.cwd();
            // avoid creating another session if connect() already created one.
            if (!this.agentManager.currentSessionId) {
                if (!autoAuthenticate) {
                    logger.log('[WebViewProvider] Skipping ACP session creation until user logs in.');
                    this.sendMessageToWebView({
                        type: 'authState',
                        data: { authenticated: false },
                    });
                    ...
                    try {
                        ...
                        if (requiresAuth && !autoAuthenticate) {
                            logger.log('[WebViewProvider] ACP session requires authentication; waiting for explicit login.');
                            this.sendMessageToWebView({
                                type: 'authState',
                                data: { authenticated: false },
                            });
                            ...
                            logger.error('[WebViewProvider] Failed to create ACP session:', sessionError);
                            vscode.window.showWarningMessage(`Failed to create ACP session: ${errorMsg}. You may need to authenticate first.`);
                        }
                    }
                }
            }
            else {
                logger.log('[WebViewProvider] Existing ACP session detected, skipping new session creation');
                sessionReady = true;
            }
            if (sessionReady) {
            ...
            const errorMsg = getErrorMessage(_error);
            logger.error('[WebViewProvider] Failed to load session messages:', _error);
            vscode.window.showErrorMessage(`Failed to load session messages: ${errorMsg}`);
            await this.initializeEmptyConversation();
            return false;
        }
        return sessionReady;
    }
    async applyInitialModelSelection() {
        ...
        try {
            ...
            logger.warn('[WebViewProvider] Failed to apply initial model selection:', error);
        }
    }
    /**
     ...
     */
    async initializeEmptyConversation() {
        try {
            ...
            logger.log('[WebViewProvider] Empty conversation initialized:', this.messageHandler.getCurrentConversationId());
        }
        catch (_error) {
            logger.error('[WebViewProvider] Failed to initialize conversation:', _error);
     ...
     */
    updateAuthStateFromMessage(message) {
    ...
    handleWebviewReady() {
        ...
        if (this.cachedAvailableModels && this.cachedAvailableModels.length > 0) {
            logger.log('[WebViewProvider] Sending cached availableModels on webviewReady:', this.cachedAvailableModels.map((m) => m.modelId));
            this.sendMessageToWebView({
                type: 'availableModels',
     ...
     */
    handleNewChatByContext(message) {
     ...
     */
    async handleCommonWebviewMessage(message, webview) {
        if (message.type === 'log') {
            const data = message.data;
            if (isLogLevel(data?.level) && typeof data.message === 'string') {
                const logMessage = data.message.length > MAX_WEBVIEW_LOG_LENGTH
                    ? `${data.message.slice(0, MAX_WEBVIEW_LOG_LENGTH)}...[truncated]`
                    : data.message;
                logger[data.level]('[Webview]', logMessage.replace(/\r\n|\r|\n/g, '\\n'));
            }
            return true;
        }
    ...
    setTabDot(color) {
        const config = vscode.workspace.getConfiguration('qwen-code');
        if (!config.get('dotIndicator', true)) {
            return;
        }
        ...
        panel.iconPath = vscode.Uri.joinPath(this.extensionUri, 'assets', DOT_ICON[color]);
    }
    ...
    clearTabDot() {
        ...
        panel.iconPath = vscode.Uri.joinPath(this.extensionUri, 'assets', DOT_ICON.default);
    }
    /**
    ...
    playNotificationSound() {
        const onError = (err) => {
            if (err) {
                logger.warn('[WebViewProvider] Notification sound failed:', err.message);
            }
        };
        if (process.platform === 'darwin') {
            ...
            execFile('powershell', ['-c', '[System.Media.SystemSounds]::Asterisk.Play()'], onError);
        }
        else {
            // canberra-gtk-play is the most "native" option; fall back to paplay.
            execFile('canberra-gtk-play', ['--id=bell'], (err) => {
                if (err) {
                    execFile('paplay', ['/usr/share/sounds/freedesktop/stereo/complete.oga'], (paErr) => {
                        if (paErr) {
                            logger.warn('[WebViewProvider] paplay fallback failed:', paErr.message);
                        }
                    });
                }
            });
     ...
     */
    handleAgentNeedsAttention(detail) {
        const panel = this.panelManager.getPanel();
        const panelActive = panel?.active ?? false;
        if (!panelActive) {
    ...
    handleResolveImagePaths(data, targetWebview) {
        ...
        const resolveImagePaths = createImagePathResolver({
            workspaceRoots,
            toWebviewUri: (filePath) => webview.asWebviewUri(vscode.Uri.file(filePath)).toString(),
        });
    ...
    respondToPendingPermission(choice) {
        ...
        const pickByKind = (substr, preferOnce = false) => {
            const lc = substr.toLowerCase();
            const filtered = options.filter((o) => (o.kind || '').toLowerCase().includes(lc));
            if (preferOnce) {
                const once = filtered.find((o) => (o.optionId || '').toLowerCase().includes('once'));
                if (once) {
        ...
        const pickByOptionId = (substr) => options.find((o) => (o.optionId || '').toLowerCase().includes(substr))
            ?.optionId;
        let optionId;
        ...
        try {
            ...
            logger.warn('[WebViewProvider] respondToPendingPermission failed:', _error);
        }
    }
    /**
    ...
    async restorePanel(panel) {
        ...
        try {
            ...
            logger.warn('[WebViewProvider] Failed to reset restored panel title/icon:', e);
        }
        panel.webview.html = WebViewContent.generate(panel.webview, this.extensionUri);
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            ...
            if (message.type === 'askUserQuestionResponse') {
                const askUserQuestionMsg = message;
        ...
        const selectionChangeDisposableRestore = vscode.window.onDidChangeTextEditorSelection((event) => {
            const editor = event.textEditor;
            if (editor === vscode.window.activeTextEditor) {
                const filePath = editor.document.uri.fsPath || null;
                const fileName = filePath ? getFileName(filePath) : null;
        ...
        // Attempt to restore authentication state and initialize connection
        logger.log('[WebViewProvider] Attempting to restore auth state and connection after restore...');
        await this.attemptAuthStateRestoration();
    }
    /**
    ...
    getState() {
        ...
        logger.log('[WebViewProvider] Current conversationId:', this.messageHandler.getCurrentConversationId());
        logger.log('[WebViewProvider] Current agentInitialized:', this.agentInitialized);
        const state = {
    ...
    restoreState(state) {
        ...
        this.authState = null;
        logger.log('[WebViewProvider] State restored. agentInitialized:', this.agentInitialized);
    ...
    async createNewSession() {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Failed to create new session: ${getErrorMessage(_error)}`);
        }
    }
    /**
