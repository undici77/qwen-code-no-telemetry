...
export class WebViewProvider {
    ...
    constructor(context, extensionUri) {
        ...
        const configChangeDisposable = vscode.workspace.onDidChangeConfiguration(async (e) => {
            ...
            if (authSettingsChanged && !this.isSyncingToVSCode) {
                console.log('[WebViewProvider] Auth-related qwen-code settings changed by user, syncing...');
                ...
                if (synced && this.agentInitialized) {
                    ...
                    try {
                        ...
                        console.error('[WebViewProvider] Reconnect after settings change failed:', e);
                    }
                }
                else if (!synced &&
                    ...
                    const apiKey = vscode.workspace
                        ...
                        .get('apiKey', '');
                    if (!apiKey) {
                        console.log('[WebViewProvider] apiKey cleared — de-authenticating and clearing persisted credentials');
                        clearPersistedAuth();
                        this.agentManager.disconnect();
                        this.agentInitialized = false;
        ...
        this.agentManager.onAvailableModels((models) => {
            console.log('[WebViewProvider] onAvailableModels received, sending to webview:', models);
                        ...
                        void (async () => {
                            if (!isSwitchMode) {
                                try {
                                    ...
                                    console.warn('[WebViewProvider] cancelCurrentPrompt error:', err);
                                }
                            ...
                            try {
                                ...
                                console.warn('[WebViewProvider] failed to synthesize failed tool_call_update:', err);
                            }
                        })();
                    }
                    else {
        ...
        this.agentManager.onDisconnected((code, signal) => {
            console.log(`[WebViewProvider] Agent disconnected (code: ${code}, signal: ${signal})`);
    ...
    async attachToView(webviewView, viewType) {
        console.log(`[WebViewProvider] Attaching to WebviewView (viewType=${viewType})`);
        ...
        console.log('[WebViewProvider] Attempting to restore auth state and connection for view...');
    ...
    async show() {
        ...
        newPanel.onDidChangeViewState(() => {
            console.log('[WebViewProvider] Panel view state changed, triggering serialization check');
        }, null, this.disposables);
        ...
        console.log('[WebViewProvider] Attempting to restore auth state and connection...');
        await this.attemptAuthStateRestoration();
    }
    /**
    ...
    async syncVSCodeSettingsToQwenConfig() {
        ...
        try {
            ...
            if (provider !== 'coding-plan') {
                console.log('[WebViewProvider] Skipping VSCode settings sync for api-key provider; interactive auth owns provider details');
                return false;
            }
            const region = config.get('codingPlanRegion', 'china');
            writeCodingPlanConfig(region, apiKey);
            console.log(`[WebViewProvider] Synced VSCode settings → ~/.qwen/settings.json (provider=${provider})`);
            return true;
        }
        catch (error) {
    ...
    async syncQwenConfigToVSCodeSettings() {
        try {
            ...
            console.log('[WebViewProvider] Syncing ~/.qwen/settings.json → VSCode settings');
            ...
            if (updates.length === 0) {
                console.log('[WebViewProvider] VSCode settings already match ~/.qwen/settings.json');
                return;
            }
            ...
            console.error('[WebViewProvider] Failed to sync qwen config to VSCode settings:', error);
        }
    }
    /**
     * Attempt to restore authentication state and initialize connection.
    ...
    async attemptAuthStateRestoration() {
        ...
        this.initializationPromise = (async () => {
            try {
                ...
                console.error('[WebViewProvider] Error in attemptAuthStateRestoration:', error);
    ...
    async doInitializeAgentConnection(options) {
        ...
        const run = async () => {
            ...
            console.log('[WebViewProvider] Starting initialization, workingDir:', workingDir);
            console.log(`[WebViewProvider] Using CLI-managed authentication (autoAuth=${autoAuthenticate})`);
            ...
            try {
                ...
                if (connectResult.requiresAuth && !autoAuthenticate) {
                    console.log('[WebViewProvider] Authentication required, launching auth flow...');
                    this.sendMessageToWebView({
                        type: 'authState',
                        data: { authenticated: false },
                    });
                ...
                if (sessionReady) {
                    ...
                    console.log('[WebViewProvider] Session creation deferred until user logs in.');
                }
            }
            catch (_error) {
    ...
    async handleAuthInteractive(providerConfig, inputs) {
        ...
        console.log(`[WebViewProvider] authInteractive: provider=${providerConfig.id}, host=${baseUrlHost}`);
        ...
        const safeRollback = () => {
            try {
                ...
                console.error('[WebViewProvider] settings rollback failed:', rollbackErr);
            }
        };
    ...
    async attemptAutoReconnect() {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            console.log(`[WebViewProvider] Auto-reconnect attempt ${attempt}/${maxAttempts}`);
            ...
            try {
                ...
                console.error(`[WebViewProvider] Auto-reconnect attempt ${attempt} failed:`, error);
            }
        }
    ...
    async refreshConnection() {
        ...
        try {
            ...
            console.log('[WebViewProvider] Connection refresh completed successfully');
    ...
    async loadCurrentSessionMessages(options) {
        ...
        try {
            console.log('[WebViewProvider] Initializing with new session (skipping restoration)');
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            const workingDir = workspaceFolder?.uri.fsPath || process.cwd();
            // avoid creating another session if connect() already created one.
            if (!this.agentManager.currentSessionId) {
                if (!autoAuthenticate) {
                    console.log('[WebViewProvider] Skipping ACP session creation until user logs in.');
                    this.sendMessageToWebView({
                        type: 'authState',
                        data: { authenticated: false },
                    });
                    ...
                    try {
                        ...
                        if (requiresAuth && !autoAuthenticate) {
                            console.log('[WebViewProvider] ACP session requires authentication; waiting for explicit login.');
                            this.sendMessageToWebView({
                                type: 'authState',
                                data: { authenticated: false },
                            });
                ...
                console.log('[WebViewProvider] Existing ACP session detected, skipping new session creation');
                sessionReady = true;
            }
            if (sessionReady) {
            ...
            const errorMsg = getErrorMessage(_error);
            console.error('[WebViewProvider] Failed to load session messages:', _error);
    ...
    async applyInitialModelSelection() {
        ...
        try {
            ...
            console.warn('[WebViewProvider] Failed to apply initial model selection:', error);
        }
    }
    /**
    ...
    async initializeEmptyConversation() {
        try {
            ...
            console.log('[WebViewProvider] Empty conversation initialized:', this.messageHandler.getCurrentConversationId());
        }
        catch (_error) {
            console.error('[WebViewProvider] Failed to initialize conversation:', _error);
    ...
    handleWebviewReady() {
        ...
        if (this.cachedAvailableModels && this.cachedAvailableModels.length > 0) {
            console.log('[WebViewProvider] Sending cached availableModels on webviewReady:', this.cachedAvailableModels.map((m) => m.modelId));
            this.sendMessageToWebView({
                type: 'availableModels',
    ...
    playNotificationSound() {
        const onError = (err) => {
            if (err) {
                console.warn('[WebViewProvider] Notification sound failed:', err.message);
            }
        };
            ...
            execFile('canberra-gtk-play', ['--id=bell'], (err) => {
                if (err) {
                    execFile('paplay', ['/usr/share/sounds/freedesktop/stereo/complete.oga'], (paErr) => {
                        if (paErr) {
                            console.warn('[WebViewProvider] paplay fallback failed:', paErr.message);
                        }
                    });
                }
            });
    ...
    respondToPendingPermission(choice) {
        ...
        try {
            ...
            console.warn('[WebViewProvider] respondToPendingPermission failed:', _error);
        }
    }
    /**
    ...
    async restorePanel(panel) {
        ...
        console.log('[WebViewProvider] Using CLI-managed authentication in restore');
        ...
        try {
            ...
            console.warn('[WebViewProvider] Failed to reset restored panel title/icon:', e);
        }
        ...
        // Attempt to restore authentication state and initialize connection
        console.log('[WebViewProvider] Attempting to restore auth state and connection after restore...');
        await this.attemptAuthStateRestoration();
    }
    /**
    ...
    getState() {
        ...
        console.log('[WebViewProvider] Current conversationId:', this.messageHandler.getCurrentConversationId());
        console.log('[WebViewProvider] Current agentInitialized:', this.agentInitialized);
        const state = {
    ...
    restoreState(state) {
        ...
        this.authState = null;
        console.log('[WebViewProvider] State restored. agentInitialized:', this.agentInitialized);
