...
const { mockConfigChangeHandlers, availableCommandsCallbackRef, mockCreateImagePathResolver, mockConfigGet, mockConfigUpdate, mockGetGlobalTempDir, mockGetPanel, mockMessageHandlerInstances, mockOnDidChangeConfiguration, mockOnDidChangeActiveTextEditor, mockOnDidChangeTextEditorSelection, mockOpenExternal, mockReadQwenSettingsForVSCode, mockWriteCodingPlanConfig, mockWriteModelProvidersConfig, mockClearPersistedAuth, mockApplyProviderInstallPlanToFile, mockSnapshotSettingsForRollback, mockRestoreSettingsSnapshot, slashCommandNotificationCallbackRef, endTurnCallbackRef, streamChunkCallbackRef, toolCallCallbackRef, permissionRequestCallbackRef, askUserQuestionCallbackRef, mockShowInformationMessage, mockWindowState, mockQwenAgentManagerInstances, mockClipboardWriteText, } = vi.hoisted(() => ({
    ...
    mockGetPanel: vi.fn(() => null),
    mockMessageHandlerInstances: [],
    mockOnDidChangeConfiguration: vi.fn((handler) => {
    ...
    mockSnapshotSettingsForRollback: vi.fn(() => null),
...
vi.mock('../../services/qwenAgentManager.js', () => ({
    QwenAgentManager: class {
        ...
        onAskUserQuestion = vi.fn((callback) => {
...
vi.mock('./PanelManager.js', async (importOriginal) => {
    ...
    return {
        ...actual,
        PanelManager: class {
            constructor(_extensionUri, _onPanelDispose) { }
            getPanel() {
                return mockGetPanel();
            }
            setPanel = vi.fn();
...
vi.mock('./MessageHandler.js', () => ({
    MessageHandler: class {
        constructor(_agentManager, _conversationStore, _currentConversationId, _sendToWebView) {
            mockMessageHandlerInstances.push(this);
        }
        setAuthInteractiveHandler = vi.fn();
        permissionHandler;
        setPermissionHandler = vi.fn((handler) => {
...
import { truncatePanelTitle, MAX_PANEL_TITLE_LENGTH, } from '../utils/panelTitleUtils.js';
import { logger } from '../../utils/logger.js';
import { mkdtempSync, mkdirSync, rmSync, writeFileSync } from 'node:fs';
import { tmpdir } from 'node:os';
import * as path from 'node:path';
const createConfigChangeEvent = (...affectedSections) => ({
    affectsConfiguration: (section) => affectedSections.includes(section),
});
describe('resolveQwenCliEntryPath', () => {
    it('uses the source dev entry when the extension runs in development mode', () => {
        ...
        try {
            const extensionRoot = path.join(tempRoot, 'packages', 'vscode-ide-companion');
            ...
            expect(resolveQwenCliEntryPath({ fsPath: extensionRoot }, 2)).toBe(devEntry);
        }
        finally {
    ...
    it('uses the bundled CLI outside development mode', () => {
        expect(resolveQwenCliEntryPath({ fsPath: '/extension-root' }, undefined)).toBe('/extension-root/dist/qwen-cli/cli.js');
    });
});
/**
...
async function setupAttachedProvider(options) {
    ...
    const provider = new WebViewProvider({ subscriptions: [] }, { fsPath: '/extension-root' });
...
describe('WebViewProvider.attachToView', () => {
    beforeEach(() => {
        ...
        mockConfigGet.mockImplementation((_key, defaultValue) => defaultValue);
        availableCommandsCallbackRef.current = undefined;
        slashCommandNotificationCallbackRef.current = undefined;
        mockCreateImagePathResolver.mockReturnValue((paths) => paths.map((entry) => ({
            path: entry,
            src: `webview:${entry}`,
        })));
        vi.spyOn(WebViewProvider.prototype, 'initializeAgentConnection').mockResolvedValue(undefined);
    });
    it('configures sidebar views with workspace/temp roots and resolves image paths through the attached webview', async () => {
        ...
        expect(mockCreateImagePathResolver).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('writes webview log messages through the extension host logger', async () => {
        ...
        expect(error).toHaveBeenCalledWith('[Webview]', `${message.slice(0, 10_000)}...[truncated]`.replace('\n', '\\n'));
        error.mockRestore();
    });
    ...
    it('routes structured insight progress markers into the attached webview', async () => {
        ...
        slashCommandNotificationCallbackRef.current?.({
            ...
            command: '/insight',
            messageType: 'info',
            message: '{"insight_progress":{"stage":"Analyzing sessions","progress":42,"detail":"21/50"}}',
        });
        expect(postMessage).toHaveBeenCalledWith({
    ...
    it('replays available skills to the webview after webviewReady', async () => {
        ...
        const agentManager = provider.agentManager;
...
describe('WebViewProvider settings sync', () => {
    ...
    it('does not report success for api-key settings without interactive auth data', async () => {
        ...
        const provider = new WebViewProvider({ subscriptions: [] }, { fsPath: '/extension-root' });
        const synced = await provider.syncVSCodeSettingsToQwenConfig();
    ...
    it('only syncs non-secret VS Code settings from ~/.qwen/settings.json', async () => {
        ...
        const provider = new WebViewProvider({ subscriptions: [] }, { fsPath: '/extension-root' });
        await provider.syncQwenConfigToVSCodeSettings();
        expect(mockConfigUpdate).toHaveBeenCalledTimes(2);
        expect(mockConfigUpdate).toHaveBeenCalledWith('provider', 'coding-plan', expect.anything());
        expect(mockConfigUpdate).toHaveBeenCalledWith('codingPlanRegion', 'global', expect.anything());
        expect(mockConfigUpdate).not.toHaveBeenCalledWith('apiKey', 'sk-updated', expect.anything());
    });
    it('ignores non-auth qwen-code setting changes', async () => {
        const provider = new WebViewProvider({ subscriptions: [] }, { fsPath: '/extension-root' });
        const syncSpy = vi
            .spyOn(provider, 'syncVSCodeSettingsToQwenConfig')
    ...
    it('reacts to auth-related qwen-code setting changes', async () => {
        ...
        const configChangeHandler = mockConfigChangeHandlers.at(-1);
        expect(configChangeHandler).toBeDefined();
        await configChangeHandler?.(createConfigChangeEvent('qwen-code', 'qwen-code.apiKey'));
    ...
    it('clears persisted credentials and disconnects when apiKey is emptied', async () => {
        ...
        provider.agentInitialized =
            true;
        // syncVSCodeSettingsToQwenConfig returns false because apiKey is empty
        vi.spyOn(provider, 'syncVSCodeSettingsToQwenConfig').mockResolvedValue(false);
        ...
        expect(provider.agentInitialized).toBe(false);
    });
    it('does not de-auth when non-apiKey auth settings change on an api-key provider', async () => {
        ...
        await configChangeHandler?.(createConfigChangeEvent('qwen-code', 'qwen-code.codingPlanRegion'));
        ...
        expect(provider.agentInitialized).toBe(true);
    });
});
describe('WebViewProvider.createNewSession', () => {
    it('forces a fresh ACP session for the sidebar new-session action', async () => {
        const provider = new WebViewProvider({ subscriptions: [] }, { fsPath: '/extension-root' });
        const agentManager = provider.agentManager;
        const messageHandler = provider.messageHandler;
        await provider.createNewSession();
        expect(agentManager.createNewSession).toHaveBeenCalledWith('/workspace-root', { forceNew: true });
...
describe('WebViewProvider initial model inheritance', () => {
    ...
    it('applies the requested initial model after creating a new session', async () => {
        ...
        await provider.loadCurrentSessionMessages();
        expect(agentManager.createNewSession).toHaveBeenCalledWith('/workspace-root', { autoAuthenticate: true });
...
describe('Notification & dot indicator', () => {
    ...
    it('shows orange dot and notification when a long task completes while panel is not active', async () => {
        ...
        mockGetPanel.mockReturnValue(mockPanel);
        ...
        expect(mockPanel.iconPath).toEqual(expect.objectContaining({
            fsPath: expect.stringContaining('icon-orange.png'),
        }));
        // Notification should be shown
        expect(mockShowInformationMessage).toHaveBeenCalledWith('Qwen Code: Waiting for your input.', 'Show');
    });
    ...
    it('shows blue dot and notification for permission requests when panel is not active', async () => {
        ...
        expect(mockShowInformationMessage).toHaveBeenCalledWith('Qwen Code: Needs your permission to use Bash.', 'Show');
    });
    ...
    it('does not show idle notification for background notification turns', async () => {
        ...
        expect(mockShowInformationMessage).not.toHaveBeenCalledWith('Qwen Code: Waiting for your input.', 'Show');
    });
...
describe('WebViewProvider.handleAuthInteractive credential rollback', () => {
    ...
    it('restores the snapshot when the reconnect leaves authState !== true', async () => {
        ...
        provider.doInitializeAgentConnection = vi.fn(async () => {
            provider.authState = false;
        });
    ...
    it('disconnects the agent after rolling back rejected credentials', async () => {
        ...
        provider.agentManager = { disconnect };
        provider.agentInitialized =
            true;
        provider.doInitializeAgentConnection = vi.fn(async () => {
    ...
    it('swallows a rollback write failure so the authError message still sends', async () => {
        ...
        const provider = makeProvider();
        const sendToWebView = provider.sendMessageToWebView;
        provider.doInitializeAgentConnection = vi.fn(async () => {
            provider.authState = false;
        });
        await expect(provider.handleAuthInteractive(providerConfig, inputs)).resolves.toBeUndefined();
        // The rollback throw must not prevent the user-facing authError.
        expect(sendToWebView).toHaveBeenCalledWith(expect.objectContaining({ type: 'authError' }));
    });
    it('rolls back + disconnects + reports authError when doInitializeAgentConnection throws (outer catch)', async () => {
        ...
        expect(disconnect).toHaveBeenCalled();
        expect(provider.agentInitialized).toBe(false);
        expect(sendToWebView).toHaveBeenCalledWith(expect.objectContaining({
            type: 'authError',
            data: expect.objectContaining({
