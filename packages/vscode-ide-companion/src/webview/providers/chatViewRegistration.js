...
import { CHAT_VIEW_ID_SECONDARY, CHAT_VIEW_ID_SIDEBAR, } from '../../constants/viewIds.js';
...
export function detectSecondarySidebarSupport(vscodeVersion) {
...
export function registerChatViewProviders(params) {
    const { context, createViewProvider, vscodeVersion = vscode.version, } = params;
    ...
    void vscode.commands.executeCommand('setContext', SECONDARY_SIDEBAR_CONTEXT_KEY, supportsSecondarySidebar);
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(CHAT_VIEW_ID_SIDEBAR, sidebarViewProvider, { webviewOptions: { retainContextWhenHidden: true } }), vscode.window.registerWebviewViewProvider(CHAT_VIEW_ID_SECONDARY, secondaryViewProvider, { webviewOptions: { retainContextWhenHidden: true } }));
