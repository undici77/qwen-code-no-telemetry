...
import { shouldSendMessage, useMessageSubmit, } from './hooks/useMessageSubmit.js';
...
import {} from '../types/completionItemTypes.js';
import { useCompletionTrigger } from './hooks/useCompletionTrigger.js';
import { AssistantMessage, UserMessage, ThinkingMessage, WaitingMessage, InterruptedMessage, FileIcon, PermissionDrawer, AskUserQuestionDialog, InsightProgressCard, ImageMessageRenderer, ImagePreview, ZERO_WIDTH_SPACE, CloseSmallIcon, 
// Layout components imported directly from webui
EmptyState, ChatHeader, SessionSelector, } from '@qwen-code/webui';
...
import { SKILL_ITEM_ID_PREFIX, isSkillsSecondaryQuery, shouldOpenSkillsSecondaryPicker, } from './utils/completionUtils.js';
import { buildSlashCommandItems, isExpandableSlashCommand, } from './utils/slashCommandUtils.js';
const consumeUserTurnIndex = (msg, counter) => {
...
const MessageList = React.memo(({ allMessages, onFileClick, onEditUserMessage, canEditMessages, childIndexMap, }) => {
    let imageIndex = 0;
    const userTurnCounter = { next: 0 };
    ...
    const elements = allMessages.map((item, index) => {
        ...
        switch (item.type) {
            case 'message': {
                ...
                if (msg.kind === 'image' && msg.imagePath) {
                    ...
                    imageIndex += 1;
                    child = (_jsx(ImageMessageRenderer, { msg: msg, imageIndex: imageIndex }));
                    break;
                }
                if (msg.role === 'thinking') {
                    child = (_jsx(ThinkingMessage, { content: msg.content || '', timestamp: msg.timestamp || 0, onFileClick: onFileClick }));
                    break;
                }
                if (msg.role === 'user') {
                    const targetTurnIndex = consumeUserTurnIndex(msg, userTurnCounter);
                    const canEditThisMessage = canEditMessages && targetTurnIndex === lastUserTurnIndex;
                    child = (_jsx(UserMessage, { content: msg.content || '', timestamp: msg.timestamp || 0, onFileClick: onFileClick, fileContext: msg.fileContext, onEdit: canEditThisMessage
                ...
                }
                {
                    ...
                    if (content === 'Interrupted' || content === 'Tool interrupted') {
                        child = _jsx(InterruptedMessage, { text: content });
                        break;
                    }
                    child = (_jsx(AssistantMessage, { content: content, timestamp: msg.timestamp || 0, onFileClick: onFileClick }));
                }
                break;
            }
            ...
            case 'completed-tool-call': {
                ...
                child = _jsx(ToolCall, { toolCall: tc });
                break;
            }
            default:
                child = null;
        ...
        mapping.push(index);
        return _jsx(React.Fragment, { children: child }, `msg-${index}`);
    });
 ...
 */
function findMessageIndex(target, container, childIndexMap) {
...
export const App = () => {
    ...
    const { inProgressToolCalls, completedToolCalls, handleToolCallUpdate, clearToolCalls, rewindToolCallsToTimestamp, } = useToolCalls();
    // UI state
    const [inputText, setInputText] = useState('');
    const [permissionRequest, setPermissionRequest] = useState(null);
    const [askUserQuestionRequest, setAskUserQuestionRequest] = useState(null);
    ...
    const [insightProgress, setInsightProgress] = useState(null);
    const [insightReportPath, setInsightReportPath] = useState(null);
    ...
    const getCompletionItems = React.useCallback(async (trigger, query) => {
        if (trigger === '@') {
            ...
            const slashCommandItems = buildSlashCommandItems(query, availableCommands);
            ...
            // Filter by query
            return allCommands.filter((cmd) => cmd.label.toLowerCase().includes(query.toLowerCase()) ||
                (cmd.description &&
                    cmd.description.toLowerCase().includes(query.toLowerCase())));
        }
    }, [fileContext, availableCommands, availableSkills, modelInfo?.name]);
    const completion = useCompletionTrigger(inputFieldRef, getCompletionItems);
    const { isOpen: completionIsOpen, triggerChar: completionTriggerChar, query: completionQuery, items: completionItems, closeCompletion, openCompletion, refreshCompletion, } = completion;
    ...
    const { attachedImages, handleRemoveImage, clearImages, handlePaste } = useImagePaste({
        onError: (error) => {
    ...
    const setComposerText = useCallback((text) => {
        ...
        inputElement.setAttribute('data-empty', text.trim().length === 0 ? 'true' : 'false');
        inputElement.focus();
    ...
    const handleEditUserMessage = useCallback((targetTurnIndex, content) => {
    ...
    useEffect(() => {
        const clearEditingOnRestoreOrFailure = (event) => {
        ...
        return () => window.removeEventListener('message', clearEditingOnRestoreOrFailure);
    }, []);
    ...
    const handleAskUserQuestionResponse = useCallback((answers) => {
    ...
    const handleCompletionSelect = useCallback((item, fillOnly) => {
        ...
        if (item.type === 'command') {
            ...
            const clientActions = {
                ...
                account: () => vscode.postMessage({ type: 'getAccountInfo', data: {} }),
            ...
            const isSkillsCmd = shouldOpenSkillsSecondaryPicker(item, availableSkills);
            if (serverCmd &&
                !isSkillsCmd &&
        ...
        const zeroWidthBeforeCursor = (rawText.substring(0, rawCursorPos).match(/\u200B/g) || []).length;
        ...
        const slashPos = completionTriggerChar === '/' ? textBeforeCursor.lastIndexOf('/') : -1;
    ...
    const handleSubmitWithScroll = useCallback((e, explicitText) => {
    ...
    const buildFence = useCallback((content) => {
    ...
    const formatToolCallForCopy = useCallback((tc, wrapCodeBlock = false) => {
        const parts = [];
        if (tc.content) {
    ...
    const contextMenuMsgIdxRef = useRef(-1);
    useEffect(() => {
        const trackTarget = (e) => {
            const container = messagesContainerRef.current;
            if (container && e.target instanceof Element) {
                contextMenuMsgIdxRef.current = findMessageIndex(e.target, container, childIndexMapRef.current);
            }
        ...
        const handler = (event) => {
            ...
            if (action === 'copyMessage') {
                ...
                if (idx >= 0 && idx < allMessages.length) {
                    ...
                    else if (item.type === 'completed-tool-call' ||
                        item.type === 'in-progress-tool-call') {
                        copyToClipboard(formatToolCallForCopy(item.data));
                    }
                }
            }
                ...
                for (const item of allMessages) {
                    if (item.type === 'message') {
                        const msg = item.data;
                        const content = msg.kind === 'image' && msg.imagePath
                            ? `![image](${msg.imagePath})`
                            : (msg.content || '').trim();
                        if (!content) {
                            continue;
    ...
    return (_jsxs("div", { className: "chat-container relative", children: [(isLoading || sessionManagement.isSwitchingSession) && (_jsx("div", { className: "bg-background/80 absolute inset-0 z-50 flex items-center justify-center backdrop-blur-sm", children: _jsxs("div", { className: "text-center", children: [_jsx("div", { className: "border-primary mx-auto mb-2 h-8 w-8 animate-spin rounded-full border-b-2" }), _jsx("p", { className: "text-muted-foreground text-sm", children: sessionManagement.isSwitchingSession
                ...
                }, onRenameSession: sessionManagement.handleRenameSession, onDeleteSession: sessionManagement.handleDeleteSession, onClose: () => sessionManagement.setShowSessionSelector(false), hasMore: sessionManagement.hasMore, isLoading: sessionManagement.isLoading, onLoadMore: sessionManagement.handleLoadMoreSessions }), _jsx(ChatHeader, { currentSessionTitle: sessionManagement.currentSessionTitle, onLoadSessions: sessionManagement.handleLoadQwenSessions, onNewSession: () => sessionManagement.handleNewQwenSession(modelInfo?.modelId ?? null) }), _jsx("div", { ref: messagesContainerRef, className: "chat-messages messages-container flex-1 overflow-y-auto overflow-x-hidden pt-5 pr-5 pl-5 pb-[140px] flex flex-col relative min-w-0 focus:outline-none [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:rounded-sm [&::-webkit-scrollbar-thumb]:hover:bg-white/30 [&>*]:flex [&>*]:gap-0 [&>*]:items-start [&>*]:text-left [&>*]:py-2 [&>*:not(:last-child)]:pb-[8px] [&>*]:flex-col [&>*]:relative [&>*]:animate-[fadeIn_0.2s_ease-in]", "data-vscode-context": hasContent ? '{"webviewSection": "chat-messages"}' : undefined, children: !hasContent && !isLoading && !sessionManagement.isSwitchingSession ? (isAuthenticated === false ? (_jsx(Onboarding, {})) : isAuthenticated === null ? (_jsxs("div", { className: "flex flex-col items-center justify-center h-full gap-3", children: [_jsx("span", { className: "inline-block w-6 h-6 animate-spin rounded-full border-2", style: {
                                borderColor: 'var(--app-secondary-foreground)',
                                borderTopColor: 'transparent',
                            } }), _jsx("span", { className: "text-sm", style: { color: 'var(--app-secondary-foreground)' }, children: "Preparing Qwen Code..." })] })) : (_jsx(EmptyState, { isAuthenticated: true }))) : (_jsxs(_Fragment, { children: [_jsx(MessageList, { allMessages: allMessages, onFileClick: handleFileClick, onEditUserMessage: handleEditUserMessage, canEditMessages: !messageHandling.isStreaming &&
                                !messageHandling.isWaitingForResponse, childIndexMap: childIndexMapRef }), insightProgress && (_jsx(InsightProgressCard, { stage: insightProgress.stage, progress: insightProgress.progress, detail: insightProgress.detail })), insightReportPath && (_jsxs("div", { className: "px-[30px] py-2", children: [_jsx("div", { className: "text-sm text-[var(--vscode-descriptionForeground)]", children: "Insight report generated at:" }), _jsx("a", { href: "#", className: "mt-1 inline-block break-all text-sm text-[var(--vscode-textLink-foreground)] underline decoration-[color-mix(in_srgb,var(--vscode-textLink-foreground)_55%,transparent)] underline-offset-2 hover:text-[var(--vscode-textLink-activeForeground)]", onClick: (event) => {
                                        event.preventDefault();
                                        handleOpenInsightReport();
                                    }, children: insightReportPath })] })), messageHandling.isWaitingForResponse &&
                ...
                }, onAttachContext: handleAttachContextClick, onPaste: handlePaste, completionIsOpen: completionIsOpen, completionItems: completionItems, onCompletionSelect: handleCompletionSelect, onCompletionFill: (item) => handleCompletionSelect(item, true), onCompletionClose: closeCompletion, canSubmit: canSubmit, extraContent: editingMessage || attachedImages.length > 0 ? (_jsxs(_Fragment, { children: [editingMessage && (_jsxs("div", { className: "flex items-center justify-between gap-2 border-t border-[var(--app-input-border)] px-2 py-1 text-xs text-[var(--app-secondary-foreground)]", children: [_jsx("span", { className: "truncate", children: "Editing message" }), _jsx("button", { type: "button", className: "btn-icon-compact h-6 w-6", title: "Cancel editing", "aria-label": "Cancel editing", onClick: clearEditingMessage, children: _jsx(CloseSmallIcon, {}) })] })), attachedImages.length > 0 ? (_jsx(ImagePreview, { images: attachedImages, onRemove: handleRemoveImage })) : null] })) : null, showModelSelector: showModelSelector, availableModels: availableModels, currentModelId: modelInfo?.modelId, onSelectModel: handleModelSelect, onCloseModelSelector: () => setShowModelSelector(false) })), isAuthenticated && permissionRequest && (_jsx(PermissionDrawer, { isOpen: !!permissionRequest, options: permissionRequest.options, toolCall: permissionRequest.toolCall, onResponse: handlePermissionResponse, onClose: () => setPermissionRequest(null) })), isAuthenticated && askUserQuestionRequest && (_jsx(AskUserQuestionDialog, { questions: askUserQuestionRequest.questions, onSubmit: handleAskUserQuestionResponse, onCancel: handleAskUserQuestionCancel })), accountInfo && (_jsx(AccountInfoDialog, { info: accountInfo, onClose: () => setAccountInfo(null) }))] }));
};
