 ...
 */
function isValidWebviewUrl(url) {
    try {
 ...
 * ```
 */
export function generateResourceUrl(relativePath) {
    ...
    if (!isValidWebviewUrl(extensionUri)) {
        console.error('[resourceUrl] Invalid extension URI - possible security risk:', extensionUri);
        return '';
    }
 ...
 */
export function generateIconUrl(iconPath) {
