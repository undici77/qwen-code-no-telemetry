 ...
 */
export function isSkillsSecondaryQuery(query) {
 ...
 */
export function shouldOpenSkillsSecondaryPicker(item, availableSkills) {
 ...
 */
export function resolveCompletionTrigger(text, cursorPosition) {
    const textBeforeCursor = text.substring(0, cursorPosition);
    const lastAtMatch = textBeforeCursor.lastIndexOf('@');
    const lastSlashMatch = textBeforeCursor.lastIndexOf('/');
    const isAtWordBoundary = (pos) => pos === 0 || text[pos - 1] === ' ' || text[pos - 1] === '\n';
    let pos = -1;
    let char = null;
