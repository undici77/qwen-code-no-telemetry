...
import { normalizeImageAttachment, escapePath, unescapePath, } from '../../utils/imageSupport.js';
...
import { processImageAttachments, saveImageToFile, buildPromptBlocks, } from './imageHandler.js';
describe('imageHandler', () => {
    ...
    it('decodes base64 data URL and writes correct buffer to disk', async () => {
        const filePath = await saveImageToFile('data:image/png;base64,YWJj', 'image/png');
        expect(filePath).toBeTruthy();
        expect(mockMkdir).toHaveBeenCalledWith(path.join('/mock/tmp', 'clipboard'), { recursive: true });
        expect(mockWriteFile).toHaveBeenCalledOnce();
        const [writtenPath, buffer] = mockWriteFile.mock.calls[0];
        expect(buffer).toEqual(Buffer.from('abc'));
        expect(path.basename(writtenPath)).toMatch(/^clipboard-\d+-[a-f0-9-]+\.png$/);
    });
    ...
    it('returns null when file write throws', async () => {
        ...
        const result = await saveImageToFile('data:image/png;base64,YWJj', 'image/png');
...
describe('buildPromptBlocks', () => {
    it('builds ACP resource_link blocks from saved image attachments', () => {
        expect(buildPromptBlocks('Please inspect this screenshot.', [
            {
...
//# sourceMappingURL=imageHandler.test.js.map