...
import { DISCONTINUED_MESSAGES, isDiscontinuedModel, parseAcpModelId, QWEN_OAUTH_AUTH_TYPE, RUNTIME_PREFIX, } from './discontinuedModel.js';
...
describe('parseAcpModelId', () => {
    ...
    it('marks runtime snapshots and still strips the trailing wrapper', () => {
        expect(parseAcpModelId('$runtime|qwen-oauth|qwen3-coder-plus(qwen-oauth)')).toEqual({
...
describe('isDiscontinuedModel', () => {
    ...
    it('does NOT flag a runtime Qwen OAuth snapshot as discontinued', () => {
        expect(isDiscontinuedModel('$runtime|qwen-oauth|qwen3-coder-plus(qwen-oauth)')).toBe(false);
    });
    ...
    it('returns false for empty / non-string ids', () => {
        ...
        expect(isDiscontinuedModel(undefined)).toBe(false);
        expect(isDiscontinuedModel(null)).toBe(false);
    });
...
//# sourceMappingURL=discontinuedModel.test.js.map