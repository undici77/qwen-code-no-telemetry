...
import { isSkillsSecondaryQuery, resolveCompletionTrigger, shouldOpenSkillsSecondaryPicker, } from './completionUtils.js';
const skillsCommandItem = {
    id: 'skills',
    label: '/skills',
    type: 'command',
    value: 'skills',
...
describe('completionUtils', () => {
    ...
    describe('shouldOpenSkillsSecondaryPicker', () => {
        it('opens the secondary picker only when skills are available', () => {
            expect(shouldOpenSkillsSecondaryPicker(skillsCommandItem, ['review', 'test'])).toBe(true);
            expect(shouldOpenSkillsSecondaryPicker(skillsCommandItem, [])).toBe(false);
        });
        it('does not open for non-/skills commands', () => {
            expect(shouldOpenSkillsSecondaryPicker({
                id: 'compress',
                label: '/compress',
                type: 'command',
                value: 'compress',
    ...
    describe('resolveCompletionTrigger', () => {
        const at = (text) => resolveCompletionTrigger(text, text.length);
...
//# sourceMappingURL=completionUtils.test.js.map