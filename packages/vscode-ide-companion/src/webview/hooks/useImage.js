...
import { MAX_IMAGE_SIZE, MAX_TOTAL_IMAGE_SIZE, isDisplayableImagePath, isSupportedPastedImageMimeType, getImageExtensionForMimeType, unescapePath, } from '../../utils/imageSupport.js';
...
export function splitMessageContentForImages(content) {
    if (!content) {
...
function parseImageReferences(content) {
    const references = [];
    let currentIndex = 0;
    while (currentIndex < content.length) {
        ...
        while (pathEndIndex < content.length) {
            ...
            if (inEscape) {
                ...
                const nextChar = pathEndIndex + 1 < content.length ? content[pathEndIndex + 1] : '';
...
export function applyImageResolution(messages, resolutions) {
...
function generatePastedImageName(mimeType) {
    const now = new Date();
...
export function useImagePaste({ onError, } = {}) {
    ...
    const handlePaste = useCallback(async (event) => {
        ...
        try {
            for (let i = 0; i < clipboardData.files.length; i++) {
                ...
                if (!isWithinSizeLimit(file)) {
                    errors.push(`Image "${file.name || 'pasted image'}" is too large (${formatFileSize(file.size)}). Maximum size is ${formatFileSize(MAX_IMAGE_SIZE)}.`);
                    continue;
                }
                if (runningTotal + file.size > MAX_TOTAL_IMAGE_SIZE) {
                    errors.push(`Skipping image "${file.name || 'pasted image'}" – total attachment size would exceed ${formatFileSize(MAX_TOTAL_IMAGE_SIZE)}.`);
                    continue;
                }
                try {
                    // Clipboard pastes default to "image.png"; generate a timestamped name instead.
                    const imageFile = file.name && file.name !== 'image.png'
                        ? file
                    ...
                    errors.push(`Failed to process image "${file.name || 'pasted image'}"`);
                }
            }
            if (errors.length > 0) {
                onError?.(errors.join('\n'));
...
// ======================== useImageResolution ========================
export function useImageResolution({ vscode, }) {
    const imageResolutionRef = useRef(new Map());
    const pendingImagePathsRef = useRef(new Set());
    const imageRequestIdRef = useRef(0);
    const expandMessages = useCallback((messages) => {
        const expanded = [];
        const allImagePaths = [];
    ...
    const applyCurrentImageResolutions = useCallback((messages) => applyImageResolution(messages, imageResolutionRef.current), []);
    const requestImageResolutions = useCallback((imagePaths) => {
        if (imagePaths.length === 0) {
            return;
        }
        const pending = imagePaths.filter((p) => !imageResolutionRef.current.has(p) &&
    ...
    const materializeMessages = useCallback((messages) => {
    ...
    const materializeMessage = useCallback((message) => {
        const expanded = message.role === 'user'
            ? expandUserMessageWithImages(message)
            : { messages: [message], imagePaths: [] };
        requestImageResolutions(expanded.imagePaths);
        return applyCurrentImageResolutions(expanded.messages);
    }, [applyCurrentImageResolutions, requestImageResolutions]);
    const mergeResolvedImages = useCallback((messages, resolved) => {
        for (const item of resolved) {
            pendingImagePathsRef.current.delete(item.path);
            imageResolutionRef.current.set(item.path, item.src === null || item.src === undefined ? null : item.src);
        }
