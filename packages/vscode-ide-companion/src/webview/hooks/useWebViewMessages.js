 ...
 */
export function liftToolNameFromMeta(toolCall) {
    if (toolCall &&
    ...
    const handleMessage = useCallback((event) => {
        ...
        switch (message.type) {
            ...
            case 'streamEnd': {
                ...
                if (activeRequestIdRef.current) {
                    if (endRequestId !== activeRequestIdRef.current) {
                        console.log('[useWebViewMessages] Ignoring stale/untagged streamEnd:', endRequestId, 'active:', activeRequestIdRef.current, 'source:', endData?.source);
                        break;
                    }
                }
            ...
            case 'permissionRequest': {
                ...
                liftToolNameFromMeta(message.data?.toolCall);
            ...
            case 'fileAttached': {
                ...
                if (inputFieldRef.current) {
                    ...
                    if (referenceText !== attachment.name) {
                        handlers.fileContext.addFileReference(referenceText, attachment.value);
                    }
