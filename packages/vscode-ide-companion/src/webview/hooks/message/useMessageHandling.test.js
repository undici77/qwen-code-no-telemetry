...
describe('useMessageHandling', () => {
    ...
    it('assigns the second assistant segment a newer timestamp so a tool call can sort between the two segments', () => {
        ...
        const assistantMessages = rendered.api.messages.filter((message) => message.role === 'assistant');
    ...
    it('keeps thinking after the user message when streamStart shares the same timestamp', () => {
        ...
        const sorted = [...rendered.api.messages].sort((a, b) => a.timestamp - b.timestamp);
    ...
    it.fails('keeps every assistant segment of a turn before a user message that was sent between segments (#3273)', () => {
...
//# sourceMappingURL=useMessageHandling.test.js.map