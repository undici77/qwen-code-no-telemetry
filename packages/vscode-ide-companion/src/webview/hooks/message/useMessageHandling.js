...
export const useMessageHandling = () => {
     ...
     */
    const startStreaming = useCallback((timestamp) => {
        ...
        setMessages((prev) => {
            ...
            const maxExistingTimestamp = prev.reduce((max, message) => Math.max(max, message.timestamp || 0), 0);
            const placeholderTimestamp = Math.max(typeof timestamp === 'number' ? timestamp : Date.now(), maxExistingTimestamp + 2);
            return [
                ...prev,
                {
                    role: 'assistant',
     ...
     */
    const setWaitingForResponse = useCallback((message) => {
        ...
        appendThinkingChunk: (chunk) => {
            ...
            setMessages((prev) => {
                ...
                if (idx === null) {
                    ...
                    const assistantTs = assistantIdx !== null &&
                        assistantIdx >= 0 &&
