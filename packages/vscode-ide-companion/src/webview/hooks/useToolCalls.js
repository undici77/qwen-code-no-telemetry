...
export const useToolCalls = () => {
     ...
     */
    const resolveTimestamp = (update, existing) => {
    ...
    const handleToolCallUpdate = useCallback((update) => {
        setToolCalls((prevToolCalls) => {
            ...
            const isTodoWrite = (kind) => (kind || '').toLowerCase() === 'todo_write' ||
                (kind || '').toLowerCase() === 'todowrite' ||
                (kind || '').toLowerCase() === 'update_todos';
            const normTitle = (t) => typeof t === 'string' ? t.trim().toLowerCase() : '';
            const isTodoTitleMergeable = (t) => {
            ...
            const isSameOrSupplement = (prevText, nextText) => {
    ...
    const rewindToolCallsToTimestamp = useCallback((cutoffTimestamp) => {
        setToolCalls((prevToolCalls) => {
            const next = new Map();
     ...
     */
    const inProgressToolCalls = Array.from(toolCalls.values()).filter((toolCall) => toolCall.status === 'pending' || toolCall.status === 'in_progress');
    /**
     * Get completed tool calls
     */
    const completedToolCalls = Array.from(toolCalls.values()).filter((toolCall) => toolCall.status === 'completed' || toolCall.status === 'failed');
    return {
        toolCalls,
        inProgressToolCalls,
        completedToolCalls,
...
//# sourceMappingURL=useToolCalls.js.map