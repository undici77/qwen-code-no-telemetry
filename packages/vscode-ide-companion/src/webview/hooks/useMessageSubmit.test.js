...
describe('shouldSendMessage', () => {
    ...
    it('returns false when streaming', () => {
        expect(shouldSendMessage({ ...defaults, inputText: 'hello', isStreaming: true })).toBe(false);
    });
...
describe('useMessageSubmit', () => {
    ...
    it('resolves raw image picker paths with spaces on submit', () => {
        ...
        props.fileContext.getFileReference = vi.fn((name) => name === imagePath ? imagePath : undefined);
        const rendered = renderHookHarness(props);
        root = rendered.root;
        container = rendered.container;
        act(() => {
    ...
    it('resolves multiple file references in one message', () => {
        ...
        props.fileContext.getFileReference = vi.fn((name) => {
            if (name === imagePath) {
                return imagePath;
            }
            if (name === notePath) {
    ...
    it('does not resolve file references from strict token prefixes', () => {
        ...
        props.fileContext.getFileReference = vi.fn((name) => name === 'data.csv' ? '/workspace/data.csv' : undefined);
        const rendered = renderHookHarness(props);
        root = rendered.root;
        container = rendered.container;
        act(() => {
...
//# sourceMappingURL=useMessageSubmit.test.js.map