 ...
 */
export function useCompletionTrigger(inputRef, getCompletionItems) {
    // Show immediate loading and provide a timeout fallback for slow sources
    const LOADING_ITEM = useMemo(() => ({
        id: 'loading',
        label: 'Loading…',
        type: 'info',
    }), []);
    const TIMEOUT_ITEM = useMemo(() => ({
        id: 'timeout',
        label: 'Timeout',
        type: 'info',
    }), []);
    ...
    const openCompletion = useCallback(async (trigger, query, position) => {
    ...
    const refreshCompletion = useCallback(async () => {
        ...
        const requestId = requestIdRef.current + 1;
        requestIdRef.current = requestId;
        const items = await getCompletionItems(currentState.triggerChar, currentState.query);
        if (requestIdRef.current !== requestId) {
            return;
        }
    ...
    useEffect(() => {
        ...
        const getCursorPosition = () => {
            ...
            try {
                ...
                console.error('[useCompletionTrigger] Error getting cursor position:', error);
                const inputRect = inputElement.getBoundingClientRect();
                return {
                    top: inputRect.top,
                    left: inputRect.left,
        ...
        const handleInput = async () => {
            ...
            const effectiveCursorPosition = Math.min(cursorPosition === 0 && text.length > 0 ? text.length : cursorPosition, text.length);
...
//# sourceMappingURL=useCompletionTrigger.js.map