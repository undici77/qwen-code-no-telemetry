...
import { CHAT_VIEW_ID_SIDEBAR, CHAT_VIEW_ID_SECONDARY, } from '../constants/viewIds.js';
 ...
 */
export function registerNewCommands(context, log, diffManager, getWebViewProviders, createWebViewProvider, outputChannel, supportsSecondarySidebar = true) {
    const disposables = [];
    // Open Chat: show the most recent editor tab or create a new one
    disposables.push(vscode.commands.registerCommand(openChatCommand, async () => {
    ...
    disposables.push(vscode.commands.registerCommand(showDiffCommand, async (args) => {
        try {
            let absolutePath = args.path;
            if (!args.path.startsWith('/') && !args.path.match(/^[a-zA-Z]:/)) {
                const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
                if (workspaceFolder) {
                    absolutePath = vscode.Uri.joinPath(workspaceFolder.uri, args.path).fsPath;
                }
            }
    ...
    disposables.push(vscode.commands.registerCommand(openNewChatTabCommand, async (args) => {
        const provider = createWebViewProvider();
        provider.setInitialModelId(args?.initialModelId);
        await provider.show();
    }));
    disposables.push(vscode.commands.registerCommand(authCommand, async () => {
        const providers = getWebViewProviders();
    ...
    disposables.push(vscode.commands.registerCommand(focusChatCommand, async () => {
    ...
    disposables.push(vscode.commands.registerCommand(newConversationCommand, async () => {
        const provider = createWebViewProvider();
        await provider.show();
    }));
    // Show Logs: reveal the output channel
    disposables.push(vscode.commands.registerCommand(showLogsCommand, async () => {
        if (outputChannel) {
            outputChannel.show(true);
        }
        else {
            vscode.window.showWarningMessage('Qwen Code Companion log channel is not available.');
        }
    }));
