...
import { authCommand, focusChatCommand, openNewChatTabCommand, registerNewCommands, } from './index.js';
const { registerCommand, executeCommand, showWarningMessage, showInformationMessage, } = vi.hoisted(() => ({
...
describe('registerNewCommands', () => {
    ...
    it('focusChat focuses the secondary sidebar when it is supported', async () => {
        registerNewCommands(context, log, diffManager, () => [], vi.fn(), undefined, true);
        await getRegisteredHandler(focusChatCommand)();
        expect(executeCommand).toHaveBeenCalledWith('qwen-code.chatView.secondary.focus');
    });
    it('focusChat falls back to the primary sidebar when secondary sidebar is unavailable', async () => {
        registerNewCommands(context, log, diffManager, () => [], vi.fn(), undefined, false);
        await getRegisteredHandler(focusChatCommand)();
