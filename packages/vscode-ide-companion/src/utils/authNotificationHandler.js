 ...
 */
export function handleAuthenticateUpdate(data) {
    const authUri = data._meta.authUri;
    // Store reference to the current notification
    currentNotification = vscode.window.showInformationMessage(`Qwen Code needs authentication. Click an action below:`, 'Open in Browser', 'Copy Link', 'Dismiss');
    currentNotification.then((selection) => {
        if (selection === 'Open in Browser') {
            ...
            vscode.window.showInformationMessage('Opening authentication page in your browser...');
        }
        else if (selection === 'Copy Link') {
            // Copy the authentication URI to clipboard
            vscode.env.clipboard.writeText(authUri);
            vscode.window.showInformationMessage('Authentication link copied to clipboard!');
        }
        // Clear the notification reference after user interaction
        currentNotification = null;
    });
}
//# sourceMappingURL=authNotificationHandler.js.map