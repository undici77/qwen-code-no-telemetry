...
import { buildProviderTemplate, codingPlanProvider, computeModelListVersion, getDefaultModelIds, tokenPlanProvider, } from '@qwen-code/qwen-code-core';
import { CodingPlanRegion, getSubscriptionPlanConfig, } from './subscriptionPlanDefinitions.js';
describe('subscription plan definitions', () => {
    it('keeps Token Plan on its dedicated model list, sourced from the core preset', () => {
        ...
        expect(tokenPlan.template.map((model) => model.id)).toEqual(getDefaultModelIds(tokenPlanProvider));
        expect(codingPlan.template.map((model) => model.id)).toEqual(getDefaultModelIds(codingPlanProvider));
        expect(tokenPlan.template.map((model) => model.id)).not.toEqual(codingPlan.template.map((model) => model.id));
        expect(codingPlan.template.map((model) => model.id)).not.toContain('qwen3.7-max');
    });
    ...
    it('defaults Token Plan to China and supports the Singapore region', () => {
        ...
        expect(china.baseUrl).toBe('https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1');
        ...
        expect(global.baseUrl).toBe('https://token-plan.ap-southeast-1.maas.aliyuncs.com/compatible-mode/v1');
...
//# sourceMappingURL=subscriptionPlanDefinitions.test.js.map