...
export class AcpConnection {
    ...
    async setupChildProcessHandlers() {
        ...
        this.child.on('exit', (code, signal) => {
            logger.error(`[ACP qwen] Process exited with code: ${code}, signal: ${signal}`);
        ...
        this.sdkConnection = new ClientSideConnection((_agent) => ({
            ...
            extNotification: async (method, params) => this.handleExtNotification(method, params),
        }), stream);
        ...
        logger.log('[ACP] Initialization response protocol:', initResponse.protocolVersion);
        try {
    ...
    async authenticate(methodId) {
        ...
        logger.log('[ACP] Sending authenticate request with methodId:', authMethodId);
    ...
    async loadSession(sessionId, cwdOverride) {
        ...
        try {
            ...
            logger.error('[ACP] Session load request failed:', error instanceof Error ? error.message : String(error));
            throw error;
        }
    }
    ...
    async switchSession(sessionId) {
        ...
        this.sessionId = sessionId;
        logger.log('[ACP] Session ID updated locally (switch not supported by CLI)');
    }
    async cancelSession() {
        const conn = this.ensureConnection();
        if (!this.sessionId) {
