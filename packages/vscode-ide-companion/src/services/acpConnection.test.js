...
describe('AcpConnection readTextFile error mapping', () => {
    it('maps ENOENT to RESOURCE_NOT_FOUND RequestError', () => {
        ...
        expect(() => conn.mapReadTextFileError(enoent, '/tmp/missing.txt')).toThrowError(expect.objectContaining({
            code: ACP_ERROR_CODES.RESOURCE_NOT_FOUND,
        }));
    });
    it('keeps non-ENOENT RequestError unchanged', () => {
        const conn = createConnection();
        const requestError = new RequestError(ACP_ERROR_CODES.INTERNAL_ERROR, 'Internal error');
        expect(conn.mapReadTextFileError(requestError, '/tmp/file.txt')).toBe(requestError);
    });
...
describe('AcpConnection extension notifications', () => {
    it('parses end_turn reason and source', () => {
        ...
        expect(onEndTurn).toHaveBeenCalledWith('end_turn', 'background_notification');
    });
});
//# sourceMappingURL=acpConnection.test.js.map