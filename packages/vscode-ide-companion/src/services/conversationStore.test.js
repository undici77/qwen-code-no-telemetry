...
function createStore(initialConversations) {
    let conversations = initialConversations.map((conversation) => ({
        ...conversation,
        messages: conversation.messages.map((message) => ({ ...message })),
    }));
    const update = vi.fn(async (_key, value) => {
        conversations = value ?? [];
    });
    const context = {
        globalState: {
...
describe('ConversationStore', () => {
    ...
    it('replaceMessages replaces messages with cloned entries', async () => {
        ...
        await expect(store.replaceMessages('conversation-1', replacement)).resolves.toBe(true);
    ...
    it('renameConversationId updates a conversation id and current id', async () => {
        ...
        await expect(store.renameConversationId('conversation-1', 'session-1')).resolves.toBe(true);
    ...
    it('renameConversationId returns false when the target id already exists', async () => {
        ...
        await expect(store.renameConversationId('conversation-1', 'session-1')).resolves.toBe(false);
    ...
    it('upsertConversation replaces an existing conversation', async () => {
        ...
        await store.upsertConversation({
            id: 'session-1',
            title: 'New',
            messages: [{ role: 'assistant', content: 'new', timestamp: 2 }],
            createdAt: 1,
            updatedAt: 2,
        });
        expect(conversations).toHaveLength(1);
    ...
    it('truncateFromUserTurn truncates from the matching user turn', async () => {
        ...
        await expect(store.truncateFromUserTurn('conversation-1', 1)).resolves.toBe(true);
        expect(conversations[0]?.messages).toEqual([
            { role: 'user', content: 'first', timestamp: 1 },
            { role: 'assistant', content: 'reply', timestamp: 2 },
        ]);
    ...
    it('truncateFromUserTurn returns false when the target turn is missing', async () => {
        ...
        await expect(store.truncateFromUserTurn('conversation-1', 4)).resolves.toBe(false);
        expect(conversations[0]?.messages).toEqual([
            { role: 'user', content: 'first', timestamp: 1 },
            { role: 'assistant', content: 'reply', timestamp: 2 },
        ]);
...
//# sourceMappingURL=conversationStore.test.js.map