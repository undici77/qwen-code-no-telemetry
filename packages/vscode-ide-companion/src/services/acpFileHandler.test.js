...
const { mockGetText, mockPositionAt, mockSave, mockApplyEdit, mockOpenTextDocument, mockCreateDirectory, mockStatFile, mockWriteFile, } = vi.hoisted(() => {
    const mockGetText = vi.fn();
    const mockPositionAt = vi.fn((offset) => ({ offset }));
...
describe('AcpFileHandler', () => {
    ...
    describe('handleReadTextFile', () => {
        ...
        it('propagates ENOENT errors', async () => {
            ...
            await expect(handler.handleReadTextFile({
    ...
    describe('handleWriteTextFile', () => {
        ...
        it('writes bytes directly for new (non-existing) file', async () => {
            ...
            expect(result).toBeNull();
            expect(mockCreateDirectory).toHaveBeenCalled();
            expect(mockWriteFile).toHaveBeenCalledWith(expect.objectContaining({ fsPath: '/test/dir/newfile.txt' }), expect.any(Uint8Array));
        });
    });
});
//# sourceMappingURL=acpFileHandler.test.js.map