 ...
 */
export function extractSessionListItems(response) {
...
export class QwenAgentManager {
    ...
    baselineModeId = 'default';
    baselineAvailableModes;
    baselineModelInfo = null;
    ...
    constructor() {
        ...
        this.connection.onSessionUpdate = (data) => {
            ...
            try {
                ...
                if (targetId &&
                    ...
                    const timestamp = typeof metaObj['timestamp'] === 'number'
                        ? metaObj['timestamp']
                        : Date.now();
        ...
        this.connection.onPermissionRequest = async (data) => {
            if (this.callbacks.onPermissionRequest) {
                ...
                return {
                    optionId: this.resolvePermissionOptionId(data, optionId) ||
        ...
        this.connection.onAskUserQuestion = async (data) => {
        ...
        this.connection.onEndTurn = (reason, source) => {
            try {
        ...
        this.connection.onAuthenticateUpdate = (data) => {
            try {
                ...
                logger.warn('[QwenAgentManager] onAuthenticateUpdate callback error:', err);
            }
        };
        this.connection.onSlashCommandNotification = (data) => {
        ...
        this.connection.onDisconnected = (code, signal) => {
            logger.log(`[QwenAgentManager] Process disconnected (code: ${code}, signal: ${signal})`);
    ...
    async connect(workingDir, cliEntryPath, options) {
        ...
        if (res.availableModels && res.availableModels.length > 0) {
            ...
            logger.log('[QwenAgentManager] Emitting availableModels from connect():', res.availableModels.map((m) => m.modelId));
     ...
     */
    async reconnect(cliEntryPath, options) {
    ...
    async setModelFromUi(modelId) {
        try {
            ...
            const modelInfo = this.baselineAvailableModels.find((model) => model.modelId === confirmedModelId) ?? {
    ...
    async getSessionList() {
        logger.log('[QwenAgentManager] Getting session list with version-aware strategy');
        try {
            logger.log('[QwenAgentManager] Attempting to get session list via ACP method');
            ...
            if (items.length > 0) {
                ...
                logger.log('[QwenAgentManager] Sessions retrieved via ACP:', sessions.length);
                return sessions;
            }
        }
        catch (error) {
            logger.warn('[QwenAgentManager] ACP session list failed, falling back to file system method:', error);
        }
        ...
        try {
            ...
            logger.log('[QwenAgentManager] Session list from file system (all projects):', sessions.length);
            ...
            logger.log('[QwenAgentManager] Sessions retrieved from file system:', result.length);
            return result;
        }
        catch (error) {
            logger.error('[QwenAgentManager] Failed to get session list from file system:', error);
            return [];
        }
    }
    /**
    ...
    async getSessionMessages(sessionId) {
        try {
            try {
                ...
                logger.log('[QwenAgentManager] Session list item for filePath lookup:', item);
    ...
    async readJsonlMessages(filePath) {
        ...
        try {
            ...
            logger.log('[QwenAgentManager] JSONL records read:', records.length, filePath);
            ...
            logger.log('[QwenAgentManager] JSONL messages reconstructed:', msgs.length);
            return msgs;
        }
        catch (err) {
    ...
    async loadSessionViaAcp(sessionId, cwdOverride) {
        try {
            ...
            logger.log('[QwenAgentManager] Rehydration start for session:', sessionId);
            logger.log('[QwenAgentManager] Attempting session/load via ACP for session:', sessionId);
            const response = await this.connection.loadSession(sessionId, cwdOverride);
            logger.log('[QwenAgentManager] Session load succeeded for session:', sessionId);
            ...
            logger.error('[QwenAgentManager] Session load via ACP failed for session:', sessionId);
            ...
            if (error && typeof error === 'object') {
                ...
                if ('error' in error) {
                    ...
                    if (acpError.error) {
                        logger.error('[QwenAgentManager] ACP error code:', acpError.error.code);
                        logger.error('[QwenAgentManager] ACP error message:', acpError.error.message);
                    }
                }
                else {
    ...
    async loadSession(sessionId) {
        logger.log('[QwenAgentManager] Loading session with version-aware strategy:', sessionId);
        try {
            logger.log('[QwenAgentManager] Attempting to load session via ACP method');
            ...
            logger.warn('[QwenAgentManager] ACP session load failed, falling back to file system method:', error);
        }
        // Always fall back to file system method
        try {
            logger.log('[QwenAgentManager] Loading session messages from file system');
            const messages = await this.loadSessionMessagesFromFile(sessionId);
            logger.log('[QwenAgentManager] Session messages loaded successfully from file system');
            return messages;
        }
        catch (error) {
            logger.error('[QwenAgentManager] Failed to load session messages from file system:', error);
            return null;
        }
    }
    /**
    ...
    async loadSessionMessagesFromFile(sessionId) {
        try {
            logger.log('[QwenAgentManager] Loading session from file system:', sessionId);
            // Load session from file system
            const session = await this.sessionManager.loadSession(sessionId, this.currentWorkingDir);
            if (!session) {
                logger.log('[QwenAgentManager] Session not found in file system:', sessionId);
                return null;
            }
            ...
            logger.error('[QwenAgentManager] Session load from file system failed:', error);
            throw error;
        }
    }
    /**
    ...
    async createNewSession(workingDir, options) {
        ...
        if (!forceNew && this.connection.currentSessionId) {
            logger.log('[QwenAgentManager] createNewSession: reusing existing session', this.connection.currentSessionId);
            return this.connection.currentSessionId;
        }
        // Deduplicate concurrent session/new attempts
        if (this.sessionCreateInFlight) {
            logger.log('[QwenAgentManager] createNewSession: session creation already in flight');
            if (!forceNew) {
        ...
        this.sessionCreateInFlight = (async () => {
            try {
                ...
                try {
                    ...
                    logger.log('[QwenAgentManager] newSession returned session:', createdSession.sessionId);
                }
                catch (err) {
                    const requiresAuth = isAuthenticationRequiredError(err);
                    if (requiresAuth) {
                        if (!autoAuthenticate) {
                            logger.warn('[QwenAgentManager] session/new requires authentication but auto-auth is disabled. Deferring until user logs in.');
                            throw err;
                        }
                        logger.warn('[QwenAgentManager] session/new requires authentication. Retrying with authenticate...');
                        try {
                            // Let CLI handle authentication - it's the single source of truth
                            await this.connection.authenticate(authMethod);
                            logger.log('[QwenAgentManager] createNewSession Authentication successful. Retrying session/new...');
                            // Add a slight delay to ensure auth state is settled
                            await new Promise((resolve) => setTimeout(resolve, 300));
                            newSessionResult = await this.connection.newSession(workingDir);
                        }
                        catch (reauthErr) {
                            logger.error('[QwenAgentManager] Re-authentication failed:', reauthErr);
                            throw reauthErr;
                        }
                    }
                    else {
                ...
                logger.log('[QwenAgentManager] New session created with ID:', newSessionId);
                return newSessionId;
            }
            finally {
    ...
    onSlashCommandNotification(callback) {
    ...
    applySessionStateFromResult(result) {
    ...
    restoreBaselineSessionStateAfterLoad(result) {
        const obj = (result || {});
...
//# sourceMappingURL=qwenAgentManager.js.map