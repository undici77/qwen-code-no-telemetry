...
export class QwenAgentManager {
    ...
    constructor() {
        ...
        this.connection.onAuthenticateUpdate = (data) => {
            try {
                ...
                console.warn('[QwenAgentManager] onAuthenticateUpdate callback error:', err);
            }
        };
        ...
        this.connection.onDisconnected = (code, signal) => {
            console.log(`[QwenAgentManager] Process disconnected (code: ${code}, signal: ${signal})`);
    ...
    async connect(workingDir, cliEntryPath, options) {
        ...
        if (res.availableModels && res.availableModels.length > 0) {
            ...
            console.log('[QwenAgentManager] Emitting availableModels from connect():', res.availableModels.map((m) => m.modelId));
    ...
    async getSessionList() {
        console.log('[QwenAgentManager] Getting session list with version-aware strategy');
        try {
            console.log('[QwenAgentManager] Attempting to get session list via ACP method');
            const response = await this.connection.listSessions();
            console.log('[QwenAgentManager] ACP session list response:', response);
            const res = response;
            const items = extractSessionListItems(res);
            console.log('[QwenAgentManager] Sessions retrieved via ACP:', res, items.length);
            if (items.length > 0) {
                ...
                console.log('[QwenAgentManager] Sessions retrieved via ACP:', sessions.length);
                return sessions;
            }
        }
        catch (error) {
            console.warn('[QwenAgentManager] ACP session list failed, falling back to file system method:', error);
        }
        ...
        try {
            ...
            console.log('[QwenAgentManager] Session list from file system (all projects):', sessions.length);
            ...
            console.log('[QwenAgentManager] Sessions retrieved from file system:', result.length);
            return result;
        }
        catch (error) {
            console.error('[QwenAgentManager] Failed to get session list from file system:', error);
            return [];
        }
    }
    /**
    ...
    async getSessionMessages(sessionId) {
        try {
            try {
                ...
                console.log('[QwenAgentManager] Session list item for filePath lookup:', item);
    ...
    async loadSessionViaAcp(sessionId, cwdOverride) {
        try {
            ...
            console.log('[QwenAgentManager] Rehydration start for session:', sessionId);
            console.log('[QwenAgentManager] Attempting session/load via ACP for session:', sessionId);
            const response = await this.connection.loadSession(sessionId, cwdOverride);
            console.log('[QwenAgentManager] Session load succeeded. Response:', JSON.stringify(response).substring(0, 200));
            ...
            console.error('[QwenAgentManager] Session load via ACP failed for session:', sessionId);
            ...
            if (error && typeof error === 'object') {
                ...
                if ('error' in error) {
                    ...
                    if (acpError.error) {
                        console.error('[QwenAgentManager] ACP error code:', acpError.error.code);
                        console.error('[QwenAgentManager] ACP error message:', acpError.error.message);
                    }
                }
                else {
    ...
    async loadSession(sessionId) {
        console.log('[QwenAgentManager] Loading session with version-aware strategy:', sessionId);
        try {
            console.log('[QwenAgentManager] Attempting to load session via ACP method');
            ...
            console.warn('[QwenAgentManager] ACP session load failed, falling back to file system method:', error);
        }
        // Always fall back to file system method
        try {
            console.log('[QwenAgentManager] Loading session messages from file system');
            const messages = await this.loadSessionMessagesFromFile(sessionId);
            console.log('[QwenAgentManager] Session messages loaded successfully from file system');
            return messages;
        }
        catch (error) {
            console.error('[QwenAgentManager] Failed to load session messages from file system:', error);
            return null;
        }
    }
    /**
    ...
    async loadSessionMessagesFromFile(sessionId) {
        try {
            console.log('[QwenAgentManager] Loading session from file system:', sessionId);
            // Load session from file system
            const session = await this.sessionManager.loadSession(sessionId, this.currentWorkingDir);
            if (!session) {
                console.log('[QwenAgentManager] Session not found in file system:', sessionId);
                return null;
            }
            ...
            console.error('[QwenAgentManager] Session load from file system failed:', error);
            throw error;
        }
    }
    /**
    ...
    async createNewSession(workingDir, options) {
        ...
        if (!forceNew && this.connection.currentSessionId) {
            console.log('[QwenAgentManager] createNewSession: reusing existing session', this.connection.currentSessionId);
            return this.connection.currentSessionId;
        }
        // Deduplicate concurrent session/new attempts
        if (this.sessionCreateInFlight) {
            console.log('[QwenAgentManager] createNewSession: session creation already in flight');
            if (!forceNew) {
        ...
        this.sessionCreateInFlight = (async () => {
            try {
                ...
                try {
                    ...
                    console.log('[QwenAgentManager] newSession returned:', JSON.stringify(newSessionResult, null, 2));
                }
                catch (err) {
                    const requiresAuth = isAuthenticationRequiredError(err);
                    if (requiresAuth) {
                        if (!autoAuthenticate) {
                            console.warn('[QwenAgentManager] session/new requires authentication but auto-auth is disabled. Deferring until user logs in.');
                            throw err;
                        }
                        console.warn('[QwenAgentManager] session/new requires authentication. Retrying with authenticate...');
                        try {
                            // Let CLI handle authentication - it's the single source of truth
                            await this.connection.authenticate(authMethod);
                            console.log('[QwenAgentManager] createNewSession Authentication successful. Retrying session/new...');
                            // Add a slight delay to ensure auth state is settled
                            await new Promise((resolve) => setTimeout(resolve, 300));
                            newSessionResult = await this.connection.newSession(workingDir);
                        }
                        catch (reauthErr) {
                            console.error('[QwenAgentManager] Re-authentication failed:', reauthErr);
                            throw reauthErr;
                        }
                    }
                    else {
                ...
                console.log('[QwenAgentManager] New session created with ID:', newSessionId);
                return newSessionId;
            }
            finally {
