...
import { deactivateCurrentActiveFile, enforceMaxFiles, truncateSelectedText, getNotebookUriFromCellUri, } from './utils.js';
export function addOrMoveToFrontNotebook(openFiles, notebookEditor) {
    // Deactivate previous active file
    deactivateCurrentActiveFile(openFiles);
    // Remove if it exists
    const index = openFiles.findIndex((f) => f.path === notebookEditor.notebook.uri.fsPath);
    if (index !== -1) {
...
export function updateNotebookActiveContext(openFiles, notebookEditor) {
    const file = openFiles.find((f) => f.path === notebookEditor.notebook.uri.fsPath);
    ...
    if (combinedSelectedText) {
        ...
        file.selectedText = truncateSelectedText(combinedSelectedText, MAX_SELECTED_TEXT_LENGTH);
    }
    else {
        file.selectedText = undefined;
    }
}
export function updateNotebookCellSelection(openFiles, cellEditor, selections) {
    ...
    if (selectedText) {
        ...
        file.selectedText = truncateSelectedText(selectedText, MAX_SELECTED_TEXT_LENGTH);
    }
    else {
        file.selectedText = undefined;
    }
}
//# sourceMappingURL=notebook-handler.js.map