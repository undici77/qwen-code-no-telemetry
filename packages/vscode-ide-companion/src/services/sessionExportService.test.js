...
const { mockLoadSession, mockCollectSessionData, mockNormalizeSessionData, mockToHtml, mockToMarkdown, mockToJson, mockToJsonl, mockGenerateExportFilename, mockWriteFile, mockShowSaveDialog, } = vi.hoisted(() => ({
...
import { exportSessionToFile, parseExportSlashCommand, } from './sessionExportService.js';
describe('sessionExportService', () => {
    beforeEach(() => {
        ...
        mockGenerateExportFilename.mockImplementation((format) => `qwen-export.${format}`);
    });
    describe('parseExportSlashCommand', () => {
        ...
        it('requires an explicit subcommand for bare /export', () => {
            expect(() => parseExportSlashCommand('/export')).toThrow("Command '/export' requires a subcommand.");
            expect(() => parseExportSlashCommand('/export   ')).toThrow("Command '/export' requires a subcommand.");
        });
        ...
        it('rejects unsupported export arguments', () => {
            expect(() => parseExportSlashCommand('/export csv')).toThrow('Unsupported /export format. Use /export html, /export md, /export json, or /export jsonl.');
            expect(() => parseExportSlashCommand('/export md extra')).toThrow('Unsupported /export format. Use /export html, /export md, /export json, or /export jsonl.');
        });
    ...
    describe('exportSessionToFile', () => {
        it('writes the exported session to the user-chosen path', async () => {
            ...
            expect(mockCollectSessionData).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 'session-1' }), expect.anything());
            expect(mockNormalizeSessionData).toHaveBeenCalled();
            expect(mockToHtml).toHaveBeenCalled();
            expect(mockShowSaveDialog).toHaveBeenCalledWith(expect.objectContaining({
                title: 'Export Session as HTML',
            }));
            expect(mockWriteFile).toHaveBeenCalledWith(chosenPath, '<html>export</html>', 'utf-8');
...
//# sourceMappingURL=sessionExportService.test.js.map