...
export class DaemonIdeConnection {
    ...
    async cancelSession() {
        ...
        if (!session) {
            logger.debug('[DaemonIdeConnection] cancelSession ignored without active session');
            return;
        }
    ...
    async pumpEvents(session, signal, generation) {
        try {
            ...
            if (!signal.aborted) {
                logger.warn('[DaemonIdeConnection] Event stream failed:', toSafeErrorMessage(error));
    ...
    async handlePermissionRequest(data, signal) {
        ...
        const session = this.session;
        if (!session) {
            logger.warn('[DaemonIdeConnection] Dropping permission request: not connected', { requestId });
            return true;
        }
        ...
        if (this.session !== session) {
            logger.warn('[DaemonIdeConnection] Permission response dropped: session changed', {
                requestId,
        ...
        try {
            ...
            if (!accepted) {
                logger.warn('[DaemonIdeConnection] Permission response rejected by daemon for request:', requestId);
            ...
            logger.warn('[DaemonIdeConnection] Permission response failed:', toSafeErrorMessage(error));
            return false;
        }
    }
    async resolvePermissionResponseUntilAbort(request, signal) {
        ...
        const responsePromise = this.resolvePermissionResponse(request).catch((error) => {
            logger.warn('[DaemonIdeConnection] Permission handler failed:', toSafeErrorMessage(error));
            return {
    ...
    async resolvePermissionResponse(request) {
        ...
        if (isAskUserQuestion) {
            ...
            if (!optionId) {
                logger.warn('[DaemonIdeConnection] AskUserQuestion option not advertised; cancelling', {
        ...
        if (!optionId) {
            logger.warn('[DaemonIdeConnection] Permission option not advertised; cancelling', {
                requestId: request.requestId,
    ...
    handleSessionDied(data) {
        const eventSessionId = isRecord(data) && typeof data['sessionId'] === 'string'
            ...
            : undefined;
        if (!this.session) {
            logger.debug('[DaemonIdeConnection] session_died received with no active session', { eventSessionId });
            return;
        }
