...
export class QwenConnectionHandler {
    ...
    async connect(connection, workingDir, cliEntryPath, options) {
        ...
        if (proxyUrl) {
            ...
            console.log('[QwenAgentManager] Using proxy from VSCode settings:', proxyUrl);
        }
        ...
        for (let attempt = 1; attempt <= maxConnectAttempts; attempt++) {
            try {
                console.log(`[QwenAgentManager] Connecting to ACP process (attempt ${attempt}/${maxConnectAttempts})...`);
                ...
                console.error(`[QwenAgentManager] Connect attempt ${attempt} failed:`, getErrorMessage(connectError));
        ...
        if (!sessionRestored) {
            console.log('[QwenAgentManager] no sessionRestored, Creating new session...');
            try {
                console.log('[QwenAgentManager] Creating new session (letting CLI handle authentication)...');
                ...
                if (modelState?.availableModels &&
                    ...
                    console.log('[QwenAgentManager] Extracted availableModels from session/new:', availableModels.map((m) => m.modelId));
                }
                ...
                if (needsAuth) {
                    requiresAuth = true;
                    console.log('[QwenAgentManager] Session creation requires authentication; waiting for user-triggered login.');
                }
                else {
    ...
    async newSessionWithRetry(connection, workingDir, maxRetries, authMethod, autoAuthenticate) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                console.log(`[QwenAgentManager] Creating session (attempt ${attempt}/${maxRetries})...`);
                ...
                console.error(`[QwenAgentManager] Session creation attempt ${attempt} failed:`, errorMessage);
                ...
                if (requiresAuth) {
                    if (!autoAuthenticate) {
                        console.log('[QwenAgentManager] Authentication required but auto-authentication is disabled. Propagating error.');
                        throw error;
                    }
                    console.log('[QwenAgentManager] Qwen requires authentication. Authenticating and retrying session/new...');
                    try {
                        ...
                        console.log('[QwenAgentManager] newSessionWithRetry Authentication successful');
                        // Retry immediately after successful auth
                        const res = await connection.newSession(workingDir);
                        console.log('[QwenAgentManager] Session created successfully after auth');
                        return res;
                    }
                    catch (authErr) {
                        console.error('[QwenAgentManager] Re-authentication failed:', authErr);
