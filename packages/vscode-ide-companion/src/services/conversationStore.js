...
export class ConversationStore {
    ...
    async replaceMessages(conversationId, messages) {
        ...
        const conversation = conversations.find((c) => c.id === conversationId);
        if (!conversation) {
            console.warn('[ConversationStore] replaceMessages: conversation not found:', conversationId);
            return false;
        }
    ...
    async renameConversationId(fromConversationId, toConversationId) {
        ...
        if (sourceIndex < 0) {
            console.warn('[ConversationStore] renameConversationId: source conversation not found:', fromConversationId);
            return false;
        }
        if (conversations.some((c) => c.id === toConversationId)) {
            console.warn('[ConversationStore] renameConversationId: target conversation already exists:', toConversationId);
            return false;
        }
    ...
    async truncateFromUserTurn(conversationId, targetTurnIndex) {
        ...
        const conversation = conversations.find((c) => c.id === conversationId);
        if (!conversation) {
            console.warn('[ConversationStore] truncateFromUserTurn: conversation not found:', conversationId);
            return false;
        }
        let userTurnIndex = 0;
        let truncateAt = -1;
        ...
        if (truncateAt < 0) {
            console.warn('[ConversationStore] truncateFromUserTurn: target turn not found:', targetTurnIndex);
            return false;
        }
