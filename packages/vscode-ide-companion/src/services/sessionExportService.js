...
import { collectSessionData, generateExportFilename, normalizeSessionData, toHtml, toJson, toJsonl, toMarkdown, } from '@qwen-code/qwen-code/export';
import { EXPORT_SESSION_FORMATS, getExportSubcommandRequiredMessage, isSessionExportFormat, } from '../utils/exportSlashCommand.js';
...
export function parseExportSlashCommand(text) {
    ...
    throw new Error('Unsupported /export format. Use /export html, /export md, /export json, or /export jsonl.');
}
function renderExportContent(format, normalizedData) {
    switch (format) {
        case 'html':
        ...
        default: {
            const unreachableFormat = format;
...
export async function exportSessionToFile(options) {
    ...
    const exportData = await collectSessionData(sessionData.conversation, EXPORT_CONFIG);
    const normalizedData = normalizeSessionData(exportData, sessionData.conversation.messages, EXPORT_CONFIG);
