...
describe('DaemonIdeConnection', () => {
    ...
    it('sends prompts through the bound daemon session', async () => {
        ...
        expect(session.prompt).toHaveBeenLastCalledWith({ prompt: blocks }, expect.any(AbortSignal));
        session.prompt.mockRejectedValueOnce(new Error('prompt failed'));
        await expect(connection.sendPrompt('will fail')).rejects.toThrow('prompt failed');
    ...
    it('responds to daemon permission requests with the selected option id', async () => {
        ...
        await waitFor(() => expect(session.respondToPermission).toHaveBeenCalledWith('request-1', {
    ...
    it('cancels permission requests by default and for reject options', async () => {
        ...
        await waitFor(() => expect(session.respondToPermission).toHaveBeenCalledWith('request-2', {
            outcome: { outcome: 'cancelled' },
        }));
        connection.onPermissionRequest = vi
            .fn()
        ...
        await waitFor(() => expect(session.respondToPermission).toHaveBeenCalledWith('request-3', {
            outcome: { outcome: 'cancelled' },
        }));
        events.close();
        await connection.disconnect();
    ...
    it('cancels permission requests when the handler throws', async () => {
        ...
        await waitFor(() => expect(session.respondToPermission).toHaveBeenCalledWith('request-throws', {
            outcome: { outcome: 'cancelled' },
        }));
        expect(warn).toHaveBeenCalledWith('[DaemonIdeConnection] Permission handler failed:', 'permission UI failed');
        events.close();
        await connection.disconnect();
        warn.mockRestore();
    });
    it('cancels permission requests when no option id is preferred', async () => {
        ...
        events.push({ id: 4, v: 1, type: 'permission_request', data: request });
        await waitFor(() => expect(session.respondToPermission).toHaveBeenCalledWith('request-fallback', {
            outcome: { outcome: 'cancelled' },
        }));
        events.close();
        await connection.disconnect();
    ...
    it('disconnects without waiting for an in-flight permission callback', async () => {
        ...
        const session = createFakeSession(events);
        const connection = new DaemonIdeConnection();
        const onPermissionRequest = vi.fn(() => new Promise(() => { }));
        connection.onPermissionRequest = onPermissionRequest;
        await connection.connect({
            baseUrl: 'http://127.0.0.1:4170',
            sessionFactory: vi.fn().mockResolvedValue(session),
    ...
    it('forwards ask-user-question answers and cancels invalid selections', async () => {
        ...
        await waitFor(() => expect(session.respondToPermission).toHaveBeenCalledWith('ask-1', {
            outcome: { outcome: 'selected', optionId: 'proceed_once' },
            answers: { q1: 'A' },
        }));
        ...
        await waitFor(() => expect(session.respondToPermission).toHaveBeenCalledWith('ask-2', {
            outcome: { outcome: 'cancelled' },
        }));
        ...
        await waitFor(() => expect(session.respondToPermission).toHaveBeenCalledWith('ask-3', {
            outcome: { outcome: 'cancelled' },
        }));
        events.close();
        await connection.disconnect();
    ...
    it('does not route non-question permission requests through ask-user-question UI', async () => {
        ...
        await waitFor(() => expect(session.respondToPermission).toHaveBeenCalledWith('tool-approval-1', {
            outcome: { outcome: 'selected', optionId: 'proceed_once' },
        }));
    ...
    it('surfaces session_died as a disconnect', async () => {
        ...
        await waitFor(() => expect(onDisconnected).toHaveBeenCalledWith(null, 'agent exited'));
        expect(connection.isConnected).toBe(false);
        events.close();
    });
    ...
    it('does not advance replay state when permission responses fail', async () => {
        ...
        const events = new EventQueue();
        const session = createFakeSession(events);
        session.respondToPermission.mockRejectedValueOnce(new Error('permission response failed'));
        const connection = new DaemonIdeConnection();
        connection.onPermissionRequest = vi
            .fn()
            .mockResolvedValue({ optionId: 'proceed_once' });
        ...
        await waitFor(() => expect(warn).toHaveBeenCalledWith('[DaemonIdeConnection] Permission response failed:', 'permission response failed'));
    ...
    it('surfaces event stream failures and normal stream completion', async () => {
        ...
        await waitFor(() => expect(failedDisconnected).toHaveBeenCalledWith(null, 'daemon_error'));
        expect(failedConnection.isConnected).toBe(false);
        expect(warn).toHaveBeenCalledWith('[DaemonIdeConnection] Event stream failed:', 'network down');
        ...
        endedEvents.close();
        await waitFor(() => expect(endedDisconnected).toHaveBeenCalledWith(null, 'stream_ended'));
    ...
    it('continues after handler failures while advancing replay state', async () => {
        ...
        expect(warn).toHaveBeenCalledWith('[DaemonIdeConnection] Event handler failed:', {
            sessionId: 'session-1',
    ...
    it('validates daemon base URLs before connecting', async () => {
        ...
        await expect(connection.connect({
            ...
            sessionFactory: vi.fn(),
        })).rejects.toThrow('Daemon baseUrl must target a loopback address, got "example.com"');
    });
});
//# sourceMappingURL=daemonIdeConnection.test.js.map