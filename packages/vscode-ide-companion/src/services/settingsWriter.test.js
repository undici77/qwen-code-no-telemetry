...
import { AuthType, CODING_PLAN_GLOBAL_BASE_URL, } from '@qwen-code/qwen-code-core';
import { CODING_PLAN_ENV_KEY } from './subscriptionPlanDefinitions.js';
import { applyProviderInstallPlanToFile, clearPersistedAuth, readQwenSettingsForVSCode, restoreSettingsSnapshot, snapshotSettingsForRollback, writeCodingPlanConfig, writeModelProvidersConfig, } from './settingsWriter.js';
describe('settingsWriter', () => {
    ...
    it('clears stale coding plan metadata when writing api-key providers', () => {
        ...
        const openaiModels = modelProviders[AuthType.USE_OPENAI];
        ...
        const preserved = openaiModels.filter((m) => m.envKey === CODING_PLAN_ENV_KEY);
    ...
    describe('applyProviderInstallPlanToFile', () => {
        ...
        it('rejects __proto__ in install-plan env keys (prototype-pollution guard)', async () => {
            ...
            await expect(applyProviderInstallPlanToFile(plan)).rejects.toThrow(/reserved segment/);
        ...
        it('rejects writes that would overwrite an intermediate scalar segment', async () => {
            ...
            fs.mkdirSync(path.dirname(settingsPath), { recursive: true });
            fs.writeFileSync(settingsPath, JSON.stringify({ env: 'legacy-string' }), 'utf-8');
            const plan = {
                providerId: 'test',
                authType: AuthType.USE_OPENAI,
                env: { NEW_KEY: 'value' },
            };
            await expect(applyProviderInstallPlanToFile(plan)).rejects.toThrow(/segment "env" is a string/);
  ...
  "API_KEY": "sk-abc\\u0022,\\n\\"INJECTED\\": \\"pwned",
        ...
        it('wipes preset, custom, and subscription-plan env keys without touching unrelated env', () => {
            ...
            const initial = {
                env: {
                    ...
                    QWEN_CUSTOM_API_KEY_OPENAI_HTTPS_API_FOO_COM_ABC123DEF456: 'sk-custom-1',
                    QWEN_CUSTOM_API_KEY_ANTHROPIC_HTTPS_API_BAR_COM_DEAD0BEEF000: 'sk-custom-2',
    ...
    describe('snapshotSettingsForRollback / restoreSettingsSnapshot', () => {
        it('round-trips: snapshot → mutate → restore brings the old state back', () => {
            ...
            fs.writeFileSync(settingsPath, JSON.stringify({ env: { OPENAI_API_KEY: 'sk-bad' } }, null, 2), 'utf-8');
...
//# sourceMappingURL=settingsWriter.test.js.map