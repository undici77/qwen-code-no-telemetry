...
describe('QwenConnectionHandler', () => {
    ...
    describe('proxy configuration', () => {
        it('passes --proxy argument when http.proxy is set', async () => {
            ...
            const connectArgs = mockConnection.connect
                .mock.calls[0];
        ...
        it('prefers http.proxy over https.proxy', async () => {
            ...
            expect(connectArgs[2]).not.toContain('http://https-proxy.example.com:8080');
        });
    ...
    describe('connect retry logic', () => {
        ...
        it('retries connect on spawn failure and succeeds on second attempt', async () => {
            mockConnection.connect
                .mockRejectedValueOnce(new Error('Qwen ACP process failed to start (exit code: null, signal: SIGTERM)'))
                .mockResolvedValueOnce(undefined);
            const connectPromise = handler.connect(mockConnection, '/workspace', '/path/to/cli.js');
        ...
        it('throws after exhausting all connect retry attempts (3 attempts)', async () => {
            const spawnError = new Error('Qwen ACP process failed to start (exit code: null, signal: SIGTERM)');
            mockConnection.connect.mockRejectedValue(spawnError);
            const connectPromise = handler.connect(mockConnection, '/workspace', '/path/to/cli.js');
...
//# sourceMappingURL=qwenConnectionHandler.test.js.map