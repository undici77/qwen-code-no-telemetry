...
export class AcpFileHandler {
    ...
    async handleReadTextFile(params) {
        ...
        try {
            ...
            console.log(`[ACP] Successfully read file: ${params.path} (${content.length} chars)`);
            ...
            if (errorCode === 'ENOENT' || errorCode === 'FileNotFound') {
                ...
                const enoent = new Error(`ENOENT: no such file or directory, open '${params.path}'`);
                enoent.code = 'ENOENT';
    ...
    async handleWriteTextFile(params) {
        console.log(`[ACP] fs/write_text_file request received for: ${params.path}`);
...
//# sourceMappingURL=acpFileHandler.js.map