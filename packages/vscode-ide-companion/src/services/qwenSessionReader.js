...
export class QwenSessionReader {
     ...
     */
    async getAllSessions(workingDir, allProjects = false) {
        try {
            const sessions = [];
            if (!allProjects && workingDir) {
                // Current project only
                const projectHash = getProjectHash(workingDir);
                const chatsDir = path.join(this.runtimeDir, 'tmp', projectHash, 'chats');
     ...
     */
    async readSessionsFromDir(chatsDir) {
        const sessions = [];
        if (!fs.existsSync(chatsDir)) {
            return sessions;
        }
        const files = fs.readdirSync(chatsDir);
        const jsonSessionFiles = files.filter((f) => f.startsWith('session-') && f.endsWith('.json'));
        const jsonlSessionFiles = files.filter((f) => /^[0-9a-fA-F-]{32,36}\.jsonl$/.test(f));
        for (const file of jsonSessionFiles) {
            ...
            try {
                ...
                console.error('[QwenSessionReader] Failed to read session file:', filePath, error);
            }
        }
        ...
        for (const file of jsonlSessionFiles) {
            ...
            try {
                ...
                console.error('[QwenSessionReader] Failed to read JSONL session file:', filePath, error);
            }
        }
        return sessions;
    }
     ...
     */
    async getSession(sessionId, _workingDir) {
     ...
     */
    async readJsonlSession(filePath, includeMessages) {
        try {
            ...
            for await (const line of rl) {
                ...
                if (type === 'system' &&
                    ...
                    const payload = obj.systemPayload;
            ...
            console.error('[QwenSessionReader] Failed to parse JSONL session:', error);
            return null;
        }
    }
     ...
     * Delete session file
     */
    async deleteSession(sessionId, workingDir) {
        try {
     ...
     */
    async renameSession(sessionId, title, workingDir) {
        try {
            const session = await this.getSession(sessionId, workingDir);
            if (!session || !session.filePath) {
                return false;
...
//# sourceMappingURL=qwenSessionReader.js.map