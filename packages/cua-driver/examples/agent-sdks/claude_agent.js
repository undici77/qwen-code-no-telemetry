...
function argumentsFromCli() {
    ...
    if (!task)
        throw new Error('usage: npm run claude -- [--route native|mcp] "<task>"');
    return { route, task };
}
...
async function driverCall(binary, name, args) {
    await execFileAsync(binary, ['call', name, JSON.stringify(args)], { timeout: 30_000 });
}
async function runAgent(options, prompt) {
    let result;
...
function nativeServer(runtime) {
    return createSdkMcpServer({
        name: 'cua_native',
        version: '1.0.0',
        tools: [
            tool('observe_desktop', 'Capture the whole desktop. Use before every action and to verify its result.', {}, async () => await runtime.observe()),
            tool('click_desktop', 'Click an absolute desktop coordinate grounded in the latest screenshot.', { x: z.number(), y: z.number() }, async ({ x, y }) => await runtime.click(x, y)),
            tool('type_text', 'Type text into the focused desktop control, then re-observe.', { text: z.string() }, async ({ text }) => await runtime.typeText(text)),
            tool('press_key', 'Press one named key in the desktop session, then re-observe.', { key: z.string() }, async ({ key }) => await runtime.pressKey(key)),
        ],
    });
}
function taskPrompt(task, route, session) {
