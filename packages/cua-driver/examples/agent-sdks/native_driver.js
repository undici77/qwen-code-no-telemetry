...
import { CaptureScope, CuaDriver, DesktopScope, EndSessionInput, GetDesktopStateInput, PressKeyInput, StartSessionInput, TypeTextInput, } from '@trycua/cua-driver';
...
try {
    await bounded(driver.startSession(StartSessionInput.new({
        session,
        captureScope: CaptureScope.Desktop,
    })), 'start_session');
    started = true;
    const before = await bounded(driver.getDesktopState(GetDesktopStateInput.new({ session })), 'get_desktop_state');
    ...
    try {
        const typed = await bounded(driver.typeText(TypeTextInput.new({
            text: token,
            scope: DesktopScope.Desktop,
            session,
        })), 'type_text');
        if (typed.isError)
            throw new Error(typed.text);
        const submitted = await bounded(driver.pressKey(PressKeyInput.new({
            key: 'ENTER',
            scope: DesktopScope.Desktop,
            session,
        })), 'press_key');
        if (submitted.isError)
            throw new Error(submitted.text);
    }
    catch {
    ...
    const after = await bounded(driver.getDesktopState(GetDesktopStateInput.new({ session })), 'post-action get_desktop_state');
...
//# sourceMappingURL=native_driver.js.map