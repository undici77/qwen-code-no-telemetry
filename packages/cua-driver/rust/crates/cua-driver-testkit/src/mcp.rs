...
impl McpDriver {
    ...
    ///
    /// A raw `target/debug/qwen-cua-driver` MCP process initializes AppKit for the
    ...
    fn spawn_macos_daemon_proxy_internal(recording_label: Option<&str>) -> Option<Self> {
        ...
        let socket = std::env::var("CUA_E2E_MACOS_DAEMON_SOCKET").unwrap_or_else(|_| {
            format!("{home}/Library/Caches/qwen-cua-driver-local/qwen-cua-driver-local.sock")
        });
        if std::os::unix::net::UnixStream::connect(&socket).is_err() {
            eprintln!(
                "[testkit] QwenCuaDriverLocal daemon not listening at {socket} — \
                 run `./scripts/install-local.sh` and `open -n -g -a QwenCuaDriverLocal --args serve`; skipping"
            );
            return None;
        }
