...
fn configure_startup_permission_mode(
    ...
    {
        eprintln!(
            "DANGER: Qwen Cua Driver is running in unrestricted mode. Runtime approval prompts are disabled; prompt injection or unintended input may act with every capability allowed by the built-in, managed, and user policy ceilings. Use only in a disposable or fully trusted environment."
        );
    }
    Ok(())
}
        ...
        Ok(backend) => {
            ...
            eprintln!(
                "⚗️  PiP preview enabled (experimental — macOS only today; \
                 see https://github.com/QwenLM/qwen-code/issues for follow-up)"
            );
        }
        Err(e) => {
...
fn main() {
    ...
    if let Some(generation) = private_worker::requested_generation() {
        ...
        std::thread::spawn(move || {
            let code = match private_worker::run(generation, Some(initialized_tx)) {
                Ok(()) => 0,
                Err(error) => {
                    eprintln!("qwen-cua-driver private worker: {error}");
                    1
                }
            };
    ...
    match command {
        ...
        cli::Command::Serve {
            ...
            //
            // The gate's `wait_for_grants` blocks while the Qwen bundle
