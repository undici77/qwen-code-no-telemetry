...
pub const RELEASE_CLI_NAME: &str = "qwen-cua-driver";
pub const LOCAL_CLI_NAME: &str = "qwen-cua-driver-local";

pub const RELEASE_APP_NAME: &str = "QwenCuaDriver";
pub const LOCAL_APP_NAME: &str = "QwenCuaDriverLocal";
pub const RELEASE_BUNDLE_ID: &str = "com.qwencode.cua-driver";
pub const LOCAL_BUNDLE_ID: &str = "com.qwencode.cua-driver.local";
...
pub(crate) fn path_is_local(path: &Path) -> bool {
    ...
    file_name == LOCAL_CLI_NAME
        ...
        || path
            .components()
            .any(|component| component.as_os_str().to_str() == Some("QwenCuaDriverLocal.app"))
}
    ...
    std::env::current_exe()
        ...
        .is_some_and(|path| {
            path.to_str().is_some_and(|path| {
                path.contains("/QwenCuaDriver.app/Contents/MacOS/")
                    || path.contains("/QwenCuaDriverLocal.app/Contents/MacOS/")
            })
        })
}
...
mod tests {
    ...
    fn local_identity_requires_the_explicit_local_product_name() {
        ...
        assert!(path_is_local(Path::new(
            "/dev/.qwen-cua-driver-local/qwen-cua-driver-local.exe"
        )));
        assert!(!path_is_local(Path::new(
            "/Applications/QwenCuaDriver.app/Contents/MacOS/qwen-cua-driver"
        )));
        assert!(!path_is_local(Path::new("/tmp/qwen-cua-driver-local-test")));
    }
