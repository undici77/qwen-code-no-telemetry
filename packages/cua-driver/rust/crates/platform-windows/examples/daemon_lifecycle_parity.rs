...
fn main() {
    let exe = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .parent()
        .unwrap()
        .join("target/debug/qwen-cua-driver.exe");
    ...
    assert_eq!(
        lines[0], "Qwen Cua Driver daemon is running",
        "first line wrong: {:?}",
        lines[0]
    );
    assert!(
    ...
    assert!(
        stderr3.contains("Qwen Cua Driver daemon is not running"),
    ...
    assert!(
        stderr4.contains("Qwen Cua Driver daemon is not running"),
