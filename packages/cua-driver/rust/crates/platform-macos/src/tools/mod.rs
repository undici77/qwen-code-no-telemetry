...
/// that `qwen-cua-driver config set capture_mode vision` (CLI) carries over into
...
///
/// The qwen-cua-driver daemon is one shared process: every `qwen-cua-driver mcp` proxy
...
pub fn register_all(
    ...
    // not have to know qwen-cua-driver internals. Provider is platform-specific; tool plumbing is in
