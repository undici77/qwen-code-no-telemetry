...
pub(crate) fn check_binary_version() -> CheckEntry {
    ...
    CheckEntry::pass(
        NAME_BINARY_VERSION,
        format!("qwen-cua-driver {}", env!("CARGO_PKG_VERSION")),
    )
}
    ...
    let (message, hint) = match bid.as_deref() {
        None | Some("") => (
            ...
            "Run the binary inside QwenCuaDriver.app so TCC grants attribute correctly. \
             ...
             connect via `qwen-cua-driver mcp`."
                .to_owned(),
        ),
        Some(other) => (
            format!("Bundle is {other}, not {CANONICAL_BUNDLE_ID}."),
            format!(
                "TCC grants will be attributed to {other}, not the qwen-cua-driver daemon. \
                 Run via `qwen-cua-driver mcp` (auto-relaunches inside QwenCuaDriver.app) or \
...
fn check_tcc_accessibility() -> CheckEntry {
    ...
    CheckEntry::fail(
        ...
        "Grant Accessibility to QwenCuaDriver.app in System Settings → Privacy & Security → \
         ...
         the grant must target the responsible app — restart via `qwen-cua-driver mcp` to relaunch \
...
fn check_screen_capture_capability() -> CheckEntry {
    ...
    CheckEntry::skip(
        NAME_SCREEN_CAPTURE_CAPABILITY,
        "Direct ScreenCaptureKit readiness was not probed because health_report is read-only; run `qwen-cua-driver permissions grant` to request and verify it explicitly.",
    )
}
...
mod tests {
    ...
    fn binary_version_always_passes() {
        ...
        assert!(
            entry.message.contains("qwen-cua-driver "),
