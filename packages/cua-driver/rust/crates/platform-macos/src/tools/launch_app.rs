        ...
        if let Ok(Ok((pid, _, _))) = &launch_result {
            if let Some(prior) = prior_frontmost {
                if *pid != prior {
                    ...
                    // long-running `qwen-cua-driver mcp` / `qwen-cua-driver serve`
                    // daemon modes. The one-shot `qwen-cua-driver call` mode
                    ...
                    // the agent is driving" — `qwen-cua-driver call` doesn't
...
fn is_cua_driver_bundle_id(bundle_id: &str) -> bool {
    matches!(
        bundle_id,
        "com.qwencode.cua-driver" | "com.qwencode.cua-driver.local"
    )
}
...
fn protected_host_launch_refusal() -> ToolResult {
    structured_launch_error(
        ...
        "launch_app cannot launch Qwen Cua Driver's protected host; operating-system permission UI must originate outside the agent tool stream".to_owned(),
        serde_json::json!({}),
    )
}
...
mod tests {
    ...
    fn recognizes_release_and_local_protected_host_bundle_ids() {
        assert!(is_cua_driver_bundle_id("com.qwencode.cua-driver"));
        assert!(is_cua_driver_bundle_id("com.qwencode.cua-driver.local"));
    ...
    async fn launch_app_cannot_reach_private_permission_host_entrypoint() {
        ...
        let result = LaunchAppTool
            .invoke(json!({ "bundle_id": "com.qwencode.cua-driver" }))
            .await;
        assert_eq!(result.is_error, Some(true));
        assert_eq!(
            result.structured_content.unwrap()["error"],
