...
fn profile_root() -> Result<PathBuf, BrowserRefusal> {
    ...
    let root = std::env::var_os("LOCALAPPDATA")
        .map(PathBuf::from)
        .map(|path| path.join("QwenCuaDriver").join("BrowserProfiles"));
    #[cfg(target_os = "macos")]
    ...
    let root = std::env::var_os("XDG_STATE_HOME")
        ...
        .map(|path| path.join("qwen-cua-driver").join("browser-profiles"));
    root.ok_or_else(|| {
        refusal(
    ...
    async fn attach_existing_profile(
        ...
        let protected_consent = if matches!(consent_path, ConsentPath::Protected) {
            ...
            let approval_request = self.approval_broker.request(
                ...
                format!(
                    "Attach Qwen Cua Driver to {:?} window {} using its logged-in profile",
