...
```bash
qwen-cua-driver start_session '{"session":"browser-run-1"}'
qwen-cua-driver list_windows '{"pid":4242}'
qwen-cua-driver get_browser_state \
...
`qwen-cua-driver serve`. An embedding application may instead provide
...
```bash
qwen-cua-driver browser_navigate \
  '{"target_id":"<target>","tab_id":"<tab>",
...
```bash
qwen-cua-driver describe browser_type
```
...
```bash
qwen-cua-driver browser_pointer \
