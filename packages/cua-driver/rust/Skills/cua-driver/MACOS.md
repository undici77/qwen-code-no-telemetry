...
`qwen-cua-driver launch_app` is the one correct path for process
...
1. `qwen-cua-driver` is on `$PATH` (`which qwen-cua-driver`). If not, point the
...
2. Start the daemon with `open -n -g -a CuaDriver --args serve` (the
   ...
   the process to QwenCuaDriver.app). `qwen-cua-driver serve &` also works;
   ...
   Cursor, VS Code, Conductor). Verify with `qwen-cua-driver status`.
3. Run `qwen-cua-driver permissions status --json`. This
   ...
   - If Accessibility is `false`, stop. AX reads and actions cannot work;
     tell the user to run `qwen-cua-driver permissions grant` and approve it.
   ...
   - If the task materially needs pixels, stop and ask the user to run
     `qwen-cua-driver permissions grant`. That command explains and deliberately
     ...
     the user should click **+**, add `/Applications/QwenCuaDriver.app` (or
     `/Applications/QwenCuaDriverLocal.app`), enable it, and rerun the command.
...
- **`check_permissions.prompt` is macOS-only and public calls are
  ...
  install, the human-run `qwen-cua-driver permissions grant` command launches a
  ...
  private embedded host, or `qwen-cua-driver mcp --direct`, the embedding host owns
...
2. `CGEvent.post(tap: .cghidEventTap)` with a leading `mouseMoved`
   event (~30 ms before the click). `qwen-cua-driver click` when the
