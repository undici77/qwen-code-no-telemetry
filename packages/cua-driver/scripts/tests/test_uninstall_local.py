...
UNINSTALL_LOCAL = REPO_ROOT / "packages/cua-driver/scripts/uninstall-local.sh"
...
def test_unix_local_uninstall_removes_owned_links_and_preserves_release(tmp_path: Path) -> None:
    ...
    local_home = home / ".qwen-cua-driver-local"
    release_home = home / ".cua-driver"
    local_bin = home / ".local/bin"
    local_cache = home / ".cache/qwen-cua-driver-local"
    release_cache = home / ".cache/qwen-cua-driver"
    for path in (fake_bin, local_home, release_home, local_bin, local_cache, release_cache):
        path.mkdir(parents=True, exist_ok=True)

    local_marker = local_home / "packages/current/qwen-cua-driver-local"
    ...
    local_cli = local_bin / "qwen-cua-driver-local"
    local_cli.symlink_to(local_home / "packages/current/qwen-cua-driver-local")
    release_cli = local_bin / "qwen-cua-driver"
    release_cli.symlink_to(release_home / "packages/current/qwen-cua-driver")

    local_skill = home / ".agents/skills/cua-driver"
    local_skill.parent.mkdir(parents=True)
    local_skill.symlink_to(local_home / "skills/cua-driver")

    local_unit = home / ".config/systemd/user/qwen-cua-driver-local.service"
    release_unit = home / ".config/systemd/user/qwen-cua-driver.service"
...
def test_local_uninstall_contract_is_explicit_on_both_platforms() -> None:
    ...
    for token in (
        "/Applications/QwenCuaDriverLocal.app",
        "com.qwencode.cua-driver.local",
        ".qwen-cua-driver-local",
        "qwen-cua-driver-local.service",
        "com.qwencode.qwen-cua-driver-local.plist",
    ):
        assert token in unix
    for token in (
        "qwen-cua-driver-local-serve",
        "qwen-cua-driver-uia-local",
        ".qwen-cua-driver-local",
        "Programs\\Qwen\\qwen-cua-driver-local\\bin",
    ):
