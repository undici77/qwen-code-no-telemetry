#!/bin/bash
#
# cua-driver local/debug installer — Swift backend (macOS only).
#
# Private helper — invoked by install-local.sh (the multi-backend
# dispatcher) when the user is on macOS and does not pass
# --experimental-rust / --backend=rust. Do not invoke directly. Builds
# from the current source tree at libs/cua-driver/swift/ and installs
# the resulting CuaDriver.app + cua-driver CLI onto the developer's
# machine. Mirrors lume's scripts/install-local.sh shape.
#
...
# Swift sources live under libs/cua-driver/swift/ (parent of scripts/ +
# `/swift`). build-app.sh / uninstall.sh / CuaDriver.entitlements stay
# in $SCRIPT_DIR (the cross-cutting libs/cua-driver/scripts/ dir).
...
cd "$CUA_DRIVER_DIR"
"$SHARED_SCRIPTS_DIR/build-app.sh" "$BUILD_CONFIG"
echo ""
...
Uninstall:  $SHARED_SCRIPTS_DIR/uninstall.sh
