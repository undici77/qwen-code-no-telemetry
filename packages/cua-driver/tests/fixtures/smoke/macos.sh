...
#
# Spawns the built AppKit harness app from packages/cua-driver/rust/test-apps/harness-appkit
# (source lives under packages/cua-driver/tests/fixtures/apps/macos),
...
#
# Prereqs:
#   - packages/cua-driver/tests/fixtures/build/macos.sh must have been run
...
DRIVER="$ROOT/packages/cua-driver/rust/target/release/qwen-cua-driver"
[[ -x "$DRIVER" ]] || DRIVER="$ROOT/packages/cua-driver/rust/target/debug/qwen-cua-driver"
HARNESS_APP="$ROOT/packages/cua-driver/rust/test-apps/harness-appkit/CuaTestHarness.AppKit.app"
...
if [[ ! -x "$DRIVER" ]]; then
    ...
    echo "       Run: cd packages/cua-driver/rust && cargo build --release -p cua-driver" >&2
    exit 1
fi

if [[ ! -x "$HARNESS_EXE" ]]; then
    echo "ERROR: AppKit harness not built at $HARNESS_EXE" >&2
    echo "       Run: packages/cua-driver/tests/fixtures/build/macos.sh" >&2
    exit 1
fi
