...
function makeConfig(overrides = {}) {
    return {
        ...
        action_prompt_template: {
            mentioned: 'Project: %project% | URL: %project_url% | Author: %author% | Type: %target_type% | IID: %iid% | Title: %title% | TodoID: %todo_id%',
        },
        ...overrides,
    };
}
...
describe('GitlabChannel', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(Gitlab).mockImplementation(() => mockApi);
        channel = new TestableGitlabChannel('test-gitlab', makeConfig(), makeBridge());
    });
    afterEach(() => {
    ...
    describe('connect', () => {
        ...
        it('constructs Gitlab client with correct host', async () => {
            ...
            await ch.connect();
            expect(Gitlab).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it('throws when bot identity fails', async () => {
            mockApi.Users.showCurrentUser.mockRejectedValue(new Error('network error'));
        ...
        it('does not warn about groupPolicy when pairing is configured', async () => {
            ...
            try {
                ...
                await ch.connect();
                ch.disconnect();
                expect(stderr.mock.calls.some((call) => String(call[0]).includes('groupPolicy is'))).toBe(false);
            }
            finally {
                stderr.mockRestore();
            }
    ...
    describe('pollOnce', () => {
        ...
        it('skips polling when action_prompt_template is missing', async () => {
            const config = makeConfig();
            delete config.action_prompt_template;
            const ch = new TestableGitlabChannel('test-gl', config, makeBridge());
            await ch.connect();
            ch.disconnect();
            ch.cursor = { lastProcessedId: 0, initialized: true };
        ...
        it('drains a very large backlog without dispatching', async () => {
            await channel.connect();
            channel.disconnect();
            const many = Array.from({ length: 150_000 }, (_, i) => makeTodo({ id: i + 1 }));
        ...
        it('fetches description for non-note mention (no #note_ anchor)', async () => {
            ...
            await pollOnce();
            expect(channel.inboundEnvelopes).toHaveLength(1);
            expect(channel.inboundEnvelopes[0].text).toContain('Full issue description');
        ...
        it('fetches pending todos', async () => {
            ...
            mockApi.TodoLists.all.mockResolvedValueOnce([]);
            await pollOnce();
            expect(mockApi.TodoLists.all).toHaveBeenCalledWith(expect.objectContaining({ state: 'pending' }));
        });
    ...
    describe('sendThreadMessage', () => {
        it('posts a note to an issue', async () => {
            ...
            expect(mockApi.IssueNotes.create).toHaveBeenCalledWith('owner/repo', 42, 'reply');
        });
        it('posts a note to a merge request', async () => {
            await initWithoutLoop();
            await channel.testSendThreadMessage('owner/repo', 'mr:99', 'reply');
            expect(mockApi.MergeRequestNotes.create).toHaveBeenCalledWith('owner/repo', 99, 'reply');
        });
        it('throws on invalid threadId format', async () => {
            await initWithoutLoop();
            await expect(channel.testSendThreadMessage('owner/repo', 'invalid', 'reply')).rejects.toThrow('invalid threadId format');
        });
        it('throws on undefined threadId', async () => {
            await initWithoutLoop();
            await expect(channel.testSendThreadMessage('owner/repo', undefined, 'x')).rejects.toThrow('requires a threadId');
        });
    });
    describe('cursor validation', () => {
        it('accepts valid cursor', () => {
            const result = channel.validateCursor({ lastProcessedId: 42, initialized: true });
            expect(result).toEqual({ lastProcessedId: 42, initialized: true });
        });
        it('rejects cursor with invalid lastProcessedId', () => {
            const result = channel.validateCursor({ lastProcessedId: 'not-a-number', initialized: true });
            expect(result).toBeNull();
        });
        it('rejects cursor with missing fields', () => {
            const result = channel.validateCursor({});
            expect(result).toBeNull();
        });
    });
    describe('template rendering', () => {
        it('replaces all known variables', async () => {
            ...
            await pollOnce();
            const env = channel.inboundEnvelopes[0];
            expect(env.metadata).toBe('Project: owner/repo | URL: https://gitlab.com/owner/repo | Author: alice | Type: Issue | IID: 42 | Title: Test Issue | TodoID: 100');
        });
        it('preserves unknown variables as-is', async () => {
            await initWithoutLoop();
            channel.config.action_prompt_template = {
                mentioned: 'Known: %project% Unknown: %nonexistent%',
            };
            const todo = makeTodo();
            mockApi.TodoLists.all.mockResolvedValueOnce([todo]);
            await pollOnce();
            expect(channel.inboundEnvelopes[0].metadata).toBe('Known: owner/repo Unknown: %nonexistent%');
        });
    });
    ...
    describe('working reaction', () => {
        ...
        class LiveGitlabChannel extends GitlabChannel {
            setReactionForTest(messageId, entry) {
        ...
        it('drives award/remove through real pollOnce path for note mention', async () => {
            const channel = new ReactingGitlabChannel('test-gitlab', makeConfig(), makeBridge());
            await channel.connect();
            channel.disconnect();
            channel.cursor = {
                lastProcessedId: 0,
            ...
            await channel.pollOnce();
            expect(mockApi.IssueNoteAwardEmojis.award).toHaveBeenCalledWith('owner/repo', 42, 1001, 'eyes');
            await vi.waitFor(() => expect(mockApi.IssueNoteAwardEmojis.remove).toHaveBeenCalledWith('owner/repo', 42, 1001, 9000));
        });
        ...
        it('acknowledges a note mention with an eyes award emoji', async () => {
            const liveChannel = new LiveGitlabChannel('test-gitlab', makeConfig(), makeBridge());
        ...
        it('uses MergeRequestNoteAwardEmojis for MR note mentions', async () => {
            ...
            liveChannel.startPromptForTest('owner/repo', 'session-1', '100');
            expect(mockApi.MergeRequestNoteAwardEmojis.award).toHaveBeenCalledWith('owner/repo', 99, 2001, 'eyes');
        });
        ...
        it('handles award failure as best-effort', async () => {
            mockApi.IssueNoteAwardEmojis.award.mockRejectedValueOnce(new Error('403'));
            const liveChannel = new LiveGitlabChannel('test-gitlab', makeConfig(), makeBridge());
            await liveChannel.connect();
            liveChannel.disconnect();
            liveChannel.setReactionForTest('100', {
            ...
            liveChannel.startPromptForTest('owner/repo', 'session-1', '100');
            await vi.waitFor(() => expect(mockApi.IssueNoteAwardEmojis.award).toHaveBeenCalled());
            liveChannel.endPromptForTest('owner/repo', 'session-1', '100');
            await Promise.resolve();
            expect(mockApi.IssueNoteAwardEmojis.remove).not.toHaveBeenCalled();
        });
        it('handles remove failure as best-effort', async () => {
            mockApi.IssueNoteAwardEmojis.remove.mockRejectedValueOnce(new Error('403'));
            const liveChannel = new LiveGitlabChannel('test-gitlab', makeConfig(), makeBridge());
            await liveChannel.connect();
            liveChannel.disconnect();
            liveChannel.setReactionForTest('100', {
            ...
            await Promise.resolve();
            liveChannel.endPromptForTest('owner/repo', 'session-1', '100');
            await vi.waitFor(() => expect(mockApi.IssueNoteAwardEmojis.remove).toHaveBeenCalled());
        });
        it('waits for pending award before removing', async () => {
            const { promise: awardPending, resolve: resolveAward } = Promise.withResolvers();
            ...
            await awardPending;
            await vi.waitFor(() => expect(mockApi.IssueNoteAwardEmojis.remove).toHaveBeenCalledWith('owner/repo', 42, 1001, 9001));
        });
    });
});
//# sourceMappingURL=GitlabAdapter.test.js.map