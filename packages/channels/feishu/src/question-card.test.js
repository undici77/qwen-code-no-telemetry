...
import { buildQuestionCard, buildQuestionTerminalCard, parseQuestionAction, parseQuestionAnswers, } from './question-card.js';
const questions = [
    {
        answerKey: 'region',
        header: 'Region',
...
describe('Feishu question cards', () => {
    it('projects all questions into one Card V2 form', () => {
        ...
        const selects = elements.filter((element) => ['select_static', 'multi_select_static'].includes(element.tag));
    ...
    it('includes correlated submit and cancel actions', () => {
        ...
        const submit = elements.find((element) => element.name === 'qwen_ask_submit_request-1');
        const cancel = elements.find((element) => element.value?.['action'] === 'qwen_ask_cancel');
    ...
    it('renders option descriptions as Markdown notes', () => {
        ...
        expect(elements.find((element) => element.tag === 'markdown')).toMatchObject({ text_size: 'notation' });
    });
    ...
    it('normalizes single and multi-select answers', () => {
        expect(parseQuestionAnswers(questions, {
            region: 'Beijing',
            sources: ['Logs', 'Metrics'],
...
//# sourceMappingURL=question-card.test.js.map