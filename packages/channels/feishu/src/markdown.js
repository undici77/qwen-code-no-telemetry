 ...
 */
export function buildCardContent(markdown, options) {
    const elements = [];
    // Main content + streaming indicator in one markdown block
    const statusLabel = options?.statusLabel ?? (options?.isStreaming ? '生成中...' : undefined);
...
export function splitChunks(text) {
    ...
    for (const line of lines) {
        ...
        if (buf.length + line.length + 1 + reserve > CHUNK_LIMIT &&
            buf.length > 0) {
            if (inCode) {
                buf += '\n```';
            }
