...
import { ChannelBase, ChannelProactiveDeliveryError, isChannelProactiveDeliveryError, isTerminalTaskLifecycleType, sanitizeSenderName, } from '@qwen-code/channel-base';
...
const FEISHU_TERMINAL_STATUS_LABELS = {
    completed: '已完成',
    cancelled: '已取消',
    failed: '已失败，请重试',
};
...
export class FeishuChannel extends ChannelBase {
    ...
    tokenRefreshHasCoreWaiters = false;
    observedUserNames = new Map();
    observedChatNames = new Map();
    observedUserLookups = new Map();
    observedChatLookups = new Map();
    observedContactWrites = new Map();
    hydratedObservedNames = false;
    collapsible;
    collapsibleThreshold;
    constructor(name, config, bridge, options) {
        super(name, config, bridge, options);
        if (!config.clientId || !config.clientSecret) {
            throw new Error(`Channel "${name}" requires clientId (appId) and clientSecret (appSecret) for Feishu.`);
        }
        const feishuCfg = config;
        this.collapsible = feishuCfg['collapsible'] || false;
        this.collapsibleThreshold =
            feishuCfg['collapsibleThreshold'] || 500;
        this.questionCardController = new FeishuQuestionCardController({
            timeoutMs: 270_000,
            sendCard: (chatId, card) => this.sendInteractiveCard(chatId, card),
            patchCard: (messageId, card) => this.patchInteractiveCard(messageId, card),
            sendFallback: (chatId, text) => this.sendMessageInternal(chatId, text, true),
            onError: (operation, error) => {
                process.stderr.write(`[Feishu:${this.name}] ${operation} error: ${error instanceof Error ? error.message : error}\n`);
            },
        });
    }
            ...
            'card.action.trigger': (data) => {
                ...
                if (question.kind === 'handled') {
                    ...
                    if (execute) {
                        setImmediate(() => {
                            execute().catch((error) => {
                                process.stderr.write(`[Feishu:${this.name}] question action execution error: ${error instanceof Error ? error.message : error}\n`);
                            });
                        });
                    }
    ...
    async connect() {
        ...
        const feishuConfig = this.config;
        const webhookPort = feishuConfig['webhookPort'];
        const verificationToken = feishuConfig['verificationToken'];
        const encryptKey = feishuConfig['encryptKey'];
        if (webhookPort) {
            if (!verificationToken) {
                throw new Error(`Channel "${this.name}" webhook mode requires verificationToken for request authentication.`);
            }
            if (!encryptKey) {
                throw new Error(`Channel "${this.name}" webhook mode requires encryptKey for HMAC request authentication. Without it, the Lark SDK skips signature verification and any client can forge events.`);
            }
            // HTTP Webhook mode
            ...
            if (!token) {
                throw new Error(`Channel "${this.name}" failed to authenticate Feishu credentials.`);
            }
        ...
        this.dedupTimer = setInterval(() => {
            ...
            for (const [msgId, state] of this.cardSessions) {
                if (state.creating && now - state.lastUpdateAt > CREATING_TIMEOUT_MS) {
                    ...
                    process.stderr.write(`[Feishu:${this.name}] WARNING: card creation timed out for msg=${msgId} (accumulated ${state.accumulatedText.length} chars dropped)\n`);
                ...
                if (now - state.lastUpdateAt > STALE_MS &&
                    !state.creating &&
                    !state.finalizing &&
                    !state.completed) {
                    this.cleanupCard(msgId);
    ...
    async connectWebhook(port, verificationToken, encryptKey) {
        const dispatcher = new lark.EventDispatcher({
            verificationToken: verificationToken || '',
            encryptKey: encryptKey || '',
        });
        dispatcher.register(this.buildHandlerMap());
        const feishuCfg = this.config;
        ...
        this.httpServer = createServer((req, res) => {
            if (req.method === 'POST') {
                req.on('error', (err) => {
                    ...
                    process.stderr.write(`[Feishu:${this.name}] Webhook request error: ${err.message}\n`);
                });
                const bodyChunks = [];
                let bodySize = 0;
                let exceeded = false;
                ...
                req.on('end', () => {
                    ...
                    try {
                        ...
                        dispatcher
                            ...
                            .catch((err) => {
                            process.stderr.write(`[Feishu:${this.name}] Webhook dispatch error: ${err instanceof Error ? err.message : err}\n`);
                            res.writeHead(500);
                            res.end('Internal Server Error');
                        });
                    }
                    catch (err) {
                        process.stderr.write(`[Feishu:${this.name}] Webhook JSON parse error: ${err instanceof Error ? err.message : err}\n`);
                        res.writeHead(400);
                        res.end('Bad Request');
                    }
                });
        ...
        const host = feishuCfg['webhookHost'] || '127.0.0.1';
    ...
    async fetchBotInfo() {
        try {
            ...
            if (resp.ok) {
                ...
                process.stderr.write(`[Feishu:${this.name}] Bot open_id: ${this.botOpenId}\n`);
            }
            else {
                process.stderr.write(`[Feishu:${this.name}] WARNING: Failed to fetch bot info (HTTP ${resp.status}). @mention detection in groups will not work.\n`);
            }
        }
        catch (err) {
            process.stderr.write(`[Feishu:${this.name}] WARNING: Failed to fetch bot info: ${err}. @mention detection in groups will not work.\n`);
        }
    }
    /**
    ...
    async fetchMessageContent(messageId) {
        ...
        try {
            const resp = await fetch(`${BASE_URL}/im/v1/messages/${messageId}?user_id_type=open_id&card_msg_content_type=user_card_content`, {
                headers: { Authorization: `Bearer ${token}` },
                signal: AbortSignal.timeout(15_000),
            });
            ...
            const isFromBot = item?.sender?.sender_type === 'app' ||
            ...
            if (item.msg_type === 'interactive') {
                ...
                const langPost = (typeof firstValue === 'object' && firstValue !== null
                    ? firstValue
                    : content);
                const lines = [];
                if (langPost?.title)
            ...
            process.stderr.write(`[Feishu:${this.name}] fetchMessageContent error: ${err}\n`);
            return { isFromBot: false };
        }
    }
    /**
    ...
    async getTenantAccessToken(options) {
        ...
        if (!options?.silent)
            this.tokenRefreshHasCoreWaiters = true;
        ...
        try {
            ...
            if (!resp.ok) {
                if (this.tokenRefreshHasCoreWaiters) {
                    process.stderr.write(`[Feishu:${this.name}] getTenantAccessToken failed: HTTP ${resp.status}\n`);
                }
                if (resp.status === 401)
                    this.tokenCache = undefined;
                return undefined;
            ...
            if (this.tokenRefreshHasCoreWaiters) {
                process.stderr.write(`[Feishu:${this.name}] getTenantAccessToken error: ${err}\n`);
            }
            return undefined;
        }
    }
    hydrateObservedNames() {
        if (this.hydratedObservedNames)
            return;
        ...
        const newestUser = new Map();
        const newestChat = new Map();
        const consider = (best, id, label, at) => {
            if (label === id)
                return;
            const current = best.get(id);
            if (!current || at >= current.at)
                best.set(id, { label, at });
        };
    ...
    observedUserName(userId) {
        ...
        return this.observedNameLookup({
            ...
            id: userId,
            request: (token) => fetch(`${BASE_URL}/contact/v3/users/basic_batch?user_id_type=${userIdType}`, {
                method: 'POST',
                headers: {
    ...
    observedChatName(chatId) {
        return this.observedNameLookup({
            lookups: this.observedChatLookups,
            names: this.observedChatNames,
            id: chatId,
            request: (token) => fetch(`${BASE_URL}/im/v1/chats/${encodeURIComponent(chatId)}`, {
                headers: { Authorization: `Bearer ${token}` },
                signal: AbortSignal.timeout(15_000),
            }),
            extractName: (body) => {
    ...
    observedContactKey(envelope) {
        return envelope.isGroup
            ? `${envelope.senderId}\u0000${envelope.chatId}\u0000${envelope.threadId ?? ''}`
            : envelope.senderId;
    }
    async enrichObservedContact(envelope) {
    ...
    async pushProactiveDelivery(target, text) {
        try {
            await this.sendMessageInternal(target.chatId, text, true, target.isGroup === false ? 'open_id' : 'chat_id');
        }
        catch (error) {
    ...
    async sendMessageInternal(chatId, text, throwOnFailure, receiveIdType = 'chat_id') {
        const token = await this.getTenantAccessToken();
        if (!token) {
            process.stderr.write(`[Feishu:${this.name}] Cannot send: no access token.\n`);
            if (throwOnFailure) {
                throw new ChannelProactiveDeliveryError('transient', 'Feishu sendMessage failed: no access token');
            }
            return;
        }
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            const title = i === 0 ? extractTitle(text) : `${extractTitle(text)} (cont.)`;
            ...
            try {
                ...
                if (!resp.ok) {
                    ...
                    if (throwOnFailure) {
                        throw new ChannelProactiveDeliveryError(resp.status === 408 || resp.status === 429 || resp.status >= 500
                            ? 'transient'
                            : 'permanent', `Feishu sendMessage failed: HTTP ${resp.status}`);
                    }
                }
            }
            catch (err) {
                ...
                if (throwOnFailure) {
                    throw new ChannelProactiveDeliveryError('transient', 'Feishu sendMessage failed: network error', { cause: err });
                }
            }
        }
    }
    ...
    async sendInteractiveCard(chatId, card) {
        const token = await this.getTenantAccessToken();
        if (!token)
            throw new FeishuCardDeliveryError('Feishu card delivery failed: no access token');
        ...
        if (!resp.ok) {
            ...
            throw new FeishuCardDeliveryError(`Feishu card delivery failed: ${errorDetail}`, errorDetail);
        }
        const data = (await resp.json());
        const messageId = data.data?.message_id;
        if (!messageId) {
            throw new FeishuCardDeliveryError('Feishu card delivery returned no message id');
        }
        return messageId;
    }
        ...
        try {
            ...
            if (!resp.ok) {
                if (resp.status === 401)
                    this.tokenCache = undefined;
                const detail = await resp.text().catch(() => '');
                process.stderr.write(`[Feishu:${this.name}] updateCard failed: HTTP ${resp.status} ${detail}\n`);
                return false;
            }
            return true;
        }
    ...
    async createStreamingCard(chatId, text, title, inboundMsgId) {
        const cardTitle = title || (inboundMsgId && this.msgToQuestion.get(inboundMsgId)) || 'Qwen';
        ...
        try {
            ...
            if (err instanceof FeishuCardDeliveryError) {
                process.stderr.write(`[Feishu:${this.name}] createStreamingCard failed: ${err.detail ?? err.message}\n`);
                return { messageId: '', success: false };
            }
            process.stderr.write(`[Feishu:${this.name}] createStreamingCard error: ${err}\n`);
            return { messageId: '', success: false };
        }
    }
    async updateCard(messageId, text, finished = false, inboundMsgId, statusLabel) {
    ...
    async presentUserInputRequest(context) {
        if (!context.precedingSegmentId) {
            ...
            if (inboundMsgId && cardState) {
                ...
                await this.endOutputCardBeforeInputRequest(context.target.chatId, inboundMsgId, cardState);
            }
        }
        return this.questionCardController.present(context);
    }
    /** Delete a card message from Feishu to prevent orphaned "思考中..." cards. */
    async deleteCard(messageId) {
        const token = await this.getTenantAccessToken();
        if (!token || !FEISHU_ID_RE.test(messageId))
            return false;
        try {
            ...
            if (!resp.ok) {
                if (resp.status === 401)
                    this.tokenCache = undefined;
                const detail = await resp.text().catch(() => '');
                process.stderr.write(`[Feishu:${this.name}] deleteCard failed: HTTP ${resp.status} msg=${messageId} ${detail}\n`);
                return false;
            }
            return true;
        }
        catch (err) {
            process.stderr.write(`[Feishu:${this.name}] deleteCard error: msg=${messageId} ${err instanceof Error ? err.message : err}\n`);
            return false;
        }
    }
    onResponseChunk(chatId, chunk, sessionId) {
        ...
        if (!inboundMsgId) {
            process.stderr.write(`[Feishu:${this.name}] onResponseChunk: no inboundMsgId for session ${sessionId}\n`);
            return;
        }
        if (this.stoppedMessages.has(inboundMsgId))
            return;
            ...
            cardState.creationTimer = setTimeout(async () => {
                try {
                    ...
                    const displayContent = atPrefix
                        ? `${atPrefix}\n\n${cs.accumulatedText}`
                        : cs.accumulatedText;
                    const result = await this.createStreamingCard(chatId, displayContent, undefined, inboundMsgId);
                    if (cs.stopped || this.stoppedMessages.has(inboundMsgId)) {
                        ...
                        if (result.success) {
                            const prefix = cs.atPrefix || this.msgToSenderName.get(inboundMsgId) || '';
                            this.updateCard(result.messageId, prefix, true, inboundMsgId, this.stopLabelFor(cs.terminalStatus, cs.userStopped ?? false)).catch(() => { });
                        }
                        cs.creating = false;
                        this.cleanupCard(inboundMsgId);
                        return;
                    ...
                    cs.cardCreationFailed = true;
                    process.stderr.write(`[Feishu:${this.name}] card create error: ${err}\n`);
                }
                cs.creating = false;
            }, 0);
            return;
    ...
    async runThrottledCardUpdate(inboundMsgId, cs) {
        if (cs.stopped || cs.finalizing)
            return;
        try {
            const atPrefix = this.msgToSenderName.get(inboundMsgId);
            const displayContent = this.truncateCardText(atPrefix ? `${atPrefix}\n\n${cs.accumulatedText}` : cs.accumulatedText);
            const ok = await this.updateCard(cs.messageId, displayContent, false, inboundMsgId);
    ...
    async onOutputSegmentEnd(chatId, sessionId, _segment, reason) {
    ...
    async endOutputCardBeforeInputRequest(chatId, inboundMsgId, cardState) {
        ...
        try {
            if (cardState.created && cardState.messageId && !cardState.stopped) {
                ...
                try {
                    ...
                    // run 已完成.
                    if (await this.finalizeStoppedCardUpdate(inboundMsgId, cardState, chatId)) {
                        return;
                    }
                    updated = await this.updateCard(cardState.messageId, finalText, true, inboundMsgId, this.statusLabelFor('completed'));
                    if (!updated) {
                        // Mirror onResponseComplete: retry without tables (Feishu card
                        // table-count limit) before giving up on the card.
                        const noTableText = this.stripTables(finalText, '(表格内容请查看原文)');
                        updated = await this.updateCard(cardState.messageId, noTableText, true, inboundMsgId, this.statusLabelFor('completed'));
                    }
                    ...
                    process.stderr.write(`[Feishu:${this.name}] input-request card finalization error: ${error instanceof Error ? error.message : error}\n`);
                }
                if (!updated) {
    ...
    isKnownInboundMessageId(messageId) {
        return (this.msgToQuestion.has(messageId) ||
    ...
    knownInboundMessageId(sessionId, messageId) {
    ...
    statusLabelFor(terminalStatus) {
        return terminalStatus === undefined
            ? FEISHU_RUNNING_STATUS_LABEL
            : FEISHU_TERMINAL_STATUS_LABELS[terminalStatus];
    }
    stopLabelFor(terminalStatus, userInitiated = false) {
        if (userInitiated || terminalStatus === undefined) {
            return FEISHU_STOPPED_STATUS_LABEL;
        }
        return this.statusLabelFor(terminalStatus);
    }
    async finalizeStoppedCardUpdate(inboundMsgId, cardState, chatId) {
        if (!cardState ||
            (!cardState.stopped && !this.stoppedMessages.has(inboundMsgId))) {
            return false;
        }
        if (cardState.created && cardState.messageId) {
            const prefix = cardState.atPrefix || this.msgToSenderName.get(inboundMsgId) || '';
            const stopLabel = `*${this.stopLabelFor(cardState.terminalStatus, this.stoppedMessages.has(inboundMsgId))}*`;
            const contentPart = cardState.accumulatedText.trim()
                ? cardState.accumulatedText + '\n\n---\n' + stopLabel
                : stopLabel;
            const finalText = prefix ? `${prefix}\n\n${contentPart}` : contentPart;
            const updated = await this.updateCard(cardState.messageId, finalText, true, inboundMsgId);
            if (!updated) {
                await this.deleteCard(cardState.messageId);
    ...
    onTaskLifecycle(event) {
        ...
        if (event.runId) {
            ...
            this.questionCardController.cancelRun(event.runId, event.type === 'cancelled' &&
                (event.reason === 'cancel_command' || event.reason === 'clear')
                ? 'cancelled'
                : 'expired');
        }
        const inboundMsgId = this.knownInboundMessageId(event.sessionId, event.messageId);
        if (!inboundMsgId)
            return;
        const cardState = this.cardSessions.get(inboundMsgId);
        if (!cardState)
        ...
        if (cardState.terminalStatus && cardState.terminalStatus !== event.type) {
            process.stderr.write(`[Feishu:${this.name}] conflicting terminal event ${event.type} after ${cardState.terminalStatus} for inbound=${inboundMsgId}\n`);
        }
        cardState.terminalStatus ??= event.type;
    }
    async onResponseComplete(chatId, fullText, sessionId) {
        const inboundMsgId = this.sessionToInboundMsg.get(sessionId);
        if (!inboundMsgId) {
            process.stderr.write(`[Feishu:${this.name}] onResponseComplete: no inboundMsgId for session ${sessionId}, fallback to sendMessage\n`);
        ...
        if (cardState?.created) {
            const updated = await this.updateCard(cardState.messageId, displayText, true, inboundMsgId, completedLabel);
            if (await this.finalizeStoppedCardUpdate(inboundMsgId, cardState, chatId)) {
                return;
            }
            if (!updated) {
                // Fallback: try without tables (card table number limit, code-fence aware)
                const noTableText = this.stripTables(displayText, '(表格内容请查看原文)');
                const retried = await this.updateCard(cardState.messageId, noTableText, true, inboundMsgId, completedLabel);
                if (await this.finalizeStoppedCardUpdate(inboundMsgId, cardState, chatId)) {
                    return;
                }
                if (!retried) {
                    ...
                    if (!lastResort) {
                        ...
                        await this.deleteCard(cardState.messageId);
                        this.cleanupCard(inboundMsgId);
                        await this.sendMessage(chatId, atSender ? `${atSender}\n\n${fullText}` : fullText);
                        return;
                    }
                }
            }
        ...
        const result = await this.createStreamingCard(chatId, displayText, undefined, inboundMsgId);
        if (result.success) {
            const finalized = await this.updateCard(result.messageId, displayText, true, inboundMsgId, completedLabel);
            if (await this.finalizeStoppedCardUpdate(inboundMsgId, cardState, chatId)) {
                return;
            }
            if (finalized) {
    ...
    onPromptStart(_chatId, sessionId, messageId) {
        const inboundMsgId = messageId && this.isKnownInboundMessageId(messageId)
            ? messageId
            : undefined;
        if (inboundMsgId) {
    ...
    async onPromptEnd(_chatId, sessionId, messageId) {
        ...
        if (inboundMsgId) {
            ...
            if (cs && !cs.stopped && !cs.completed) {
                if (cs.creating) {
                    ...
                    const atPrefix = this.msgToSenderName.get(inboundMsgId) || '';
                    const terminalStatus = cs.terminalStatus ?? (cs.cancelling ? 'cancelled' : 'failed');
                    // userStopped: a Stop click may lose the race against this wind-down
                    // path — still render 已停止生成 rather than 已取消/已失败.
                    const terminalLabel = this.stopLabelFor(terminalStatus, cs.userStopped ?? false);
                    ...
                    await this.updateCard(cs.messageId, text, true, inboundMsgId, terminalLabel).catch(() => { });
                    this.cleanupCard(inboundMsgId);
                }
                else {
                    ...
                    if (cs.accumulatedText) {
                        ...
                        process.stderr.write(`[Feishu:${this.name}] onPromptEnd: no card and no accumulated text for inbound=${inboundMsgId}, sent error fallback\n`);
                    }
                    this.cleanupCard(inboundMsgId);
                }
            }
    ...
    async addReaction(messageId, emojiType) {
        ...
        if (!token || !FEISHU_ID_RE.test(messageId))
            return;
        try {
            const resp = await fetch(`${BASE_URL}/im/v1/messages/${messageId}/reactions`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
            ...
            process.stderr.write(`[Feishu:${this.name}] addReaction failed: ${err instanceof Error ? err.message : err}\n`);
        }
    }
    async removeReaction(messageId, emojiType) {
        ...
        try {
            ...
            const resp = await fetch(`${BASE_URL}/im/v1/messages/${messageId}/reactions?reaction_type=${emojiType}&user_id_type=open_id`, {
                headers: { Authorization: `Bearer ${token}` },
                signal: AbortSignal.timeout(15_000),
            });
            if (!resp.ok) {
            ...
            for (const item of items) {
                if (item.reaction_id &&
                    ...
                    await fetch(`${BASE_URL}/im/v1/messages/${messageId}/reactions/${item.reaction_id}`, {
                        method: 'DELETE',
                        headers: { Authorization: `Bearer ${token}` },
                        signal: AbortSignal.timeout(15_000),
                    });
            ...
            process.stderr.write(`[Feishu:${this.name}] removeReaction failed: ${err instanceof Error ? err.message : err}\n`);
        }
    }
    ...
    onCardAction(data) {
        try {
            ...
            if (!targetInboundMsgId) {
                process.stderr.write(`[Feishu:${this.name}] Stop: no card session for messageId=${messageId}\n`);
                return false;
            }
            ...
            if (!operatorId || !originalSender || operatorId !== originalSender) {
                process.stderr.write(`[Feishu:${this.name}] Stop rejected: operator=${operatorId ?? 'n/a'} sender=${originalSender ?? 'n/a'}\n`);
                return false;
            }
            ...
            const handleStop = async () => {
                ...
                const cancelSucceeded = sessionId
                    ? await this.requestActivePromptCancellation(sessionId, 'cancel_command')
                    : true;
                ...
                if (cardState.created && cardState.messageId) {
                    const prefix = cardState.atPrefix || this.msgToSenderName.get(inboundId) || '';
                    ...
                    const finalText = prefix
                        ? contentPart
                            ...
                            : prefix
                        : contentPart;
                    const updated = await this.updateCard(cardState.messageId, finalText, cancelSucceeded, inboundId, stopLabel);
            ...
            process.stderr.write(`[Feishu:${this.name}] Failed to parse card action: ${err}\n`);
            return false;
        }
    }
    disconnect() {
    ...
    truncateCardText(text, reservedChars = 0) {
        const MAX_CARD_CHARS = 20_000;
        if (text.length + reservedChars <= MAX_CARD_CHARS)
            return text;
        const marker = '\n\n_(内容过长，已截断早期内容)_';
        const fenceReserve = 4; // potential '```\n' prepend for fence rebalancing
        let truncated = text.slice(-(MAX_CARD_CHARS - marker.length - fenceReserve - reservedChars)) + marker;
    ...
    releaseOutputCard(inboundMsgId) {
        const cardState = this.cardSessions.get(inboundMsgId);
        if (!cardState)
            return;
        if (cardState.pendingUpdateTimer) {
        ...
        try {
            ...
            if (msg.mentions && msg.mentions.length > 0) {
                for (const mention of msg.mentions) {
                    const mentionId = mention.id.open_id || mention.id.user_id || mention.id.union_id;
                    if (mentionId === this.botOpenId) {
                        isMentioned = true;
                    }
                    // Replace @mention placeholder in text
                    cleanText = cleanText.replaceAll(mention.key, () => `@${mention.name}`);
                }
            ...
            const processMessage = async () => {
                ...
                if (msg.parent_id) {
                    const { content: quotedContent, isFromBot } = await this.fetchMessageContent(msg.parent_id);
                    if (quotedContent) {
                ...
                const questionTitle = cleanText.length > 20 ? cleanText.slice(0, 20) + '...' : cleanText;
                ...
                if (content.imageKey) {
                    const token = await this.getTenantAccessToken();
                    if (token) {
                        const media = await downloadMedia(msgId, content.imageKey, 'image', token);
                        if (media) {
                ...
                let downloadedFileDir;
                if (content.fileKey && content.fileName) {
                    const token = await this.getTenantAccessToken();
                    if (token) {
                        const media = await downloadMedia(msgId, content.fileKey, 'file', token);
                        if (media) {
                            const dir = join(tmpdir(), 'channel-files', randomUUID());
                            mkdirSync(dir, { recursive: true });
                            const rawName = basename(content.fileName).replace(/\0/g, '');
                            const safeName = rawName.replace(/[^\w.-]/g, '_').replace(/^\.+/, '_') ||
            ...
            processMessage().catch((err) => {
                ...
                process.stderr.write(`[Feishu:${this.name}] Error handling message: ${err}\n`);
                ...
                if (existingCard.created && existingCard.messageId) {
                    this.updateCard(existingCard.messageId, '处理消息时出错，请重试。', true, msgId).catch(() => { });
                    this.cleanupCard(msgId);
                }
                else {
            ...
            process.stderr.write(`[Feishu:${this.name}] Failed to parse message: ${err}\n`);
        }
    }
    /**
     * Extract text and media keys from Feishu message content.
     */
    extractContent(messageType, contentJson) {
        try {
            ...
            switch (messageType) {
                ...
                case 'post': {
                    ...
                    const langPost = (typeof firstVal === 'object' && firstVal !== null ? firstVal : post);
                    if (langPost?.title) {
                        lines.push(langPost.title);
                    }
                    if (langPost?.content) {
                ...
                case 'image':
                    return {
                        text: '(image)',
                        imageKey: content.image_key || undefined,
                    };
                case 'file':
                    return {
                        text: `(file: ${content.file_name || 'file'})`,
                        fileKey: content.file_key || undefined,
            ...
            process.stderr.write(`[Feishu:${this.name}] extractContent parse error (type=${messageType}): ${err instanceof Error ? err.message : err}\n`);
            return { text: '' };
        }
    }
}
