...
function createContext(requestId = 'request-1', overrides = {}) {
...
describe('FeishuQuestionCardController presentation', () => {
    ...
    it('reserves the request before delivery and does not reactivate it after settlement', async () => {
        ...
        expect(patchCard).toHaveBeenCalledWith('om_1', expect.objectContaining({ schema: '2.0' }));
        expect(JSON.stringify(patchCard.mock.calls[0]?.[1])).toContain('已过期');
        expect(JSON.stringify(patchCard.mock.calls[0]?.[1])).not.toContain('已取消');
    ...
    it('falls back visibly and cancels when native delivery fails', async () => {
        ...
        expect(sendFallback).toHaveBeenCalledWith('oc_1', expect.stringContaining('互动问题卡片投递失败'));
        expect(sendFallback).toHaveBeenCalledWith('oc_1', expect.stringContaining('Which region?'));
        expect(respond).toHaveBeenCalledWith({ outcome: { outcome: 'cancelled' } });
        expect(onError).toHaveBeenCalledWith('question card delivery', expect.any(Error));
        sendCard.mockResolvedValue('om_retry');
        await expect(controller.present(createContext('request-retry').context)).resolves.toEqual({ kind: 'presented' });
    });
    ...
    it('rejects a second same-scope request while the first delivery is in flight', async () => {
        ...
        await expect(controller.present(createContext('second-in-flight').context)).resolves.toEqual({ kind: 'unsupported' });
        delivery.resolve('om_first');
        await expect(presenting).resolves.toEqual({ kind: 'presented' });
        expect(controller.claim(validCancel('first-in-flight', { messageId: 'om_first' }))).toMatchObject({ kind: 'handled', execute: expect.any(Function) });
    });
    ...
    it('keeps concurrent owners in one shared session and chat independent', async () => {
        ...
        expect(controller.claim(validCancel('request-shared-1', { messageId: 'om_one' }))).toMatchObject({ kind: 'handled', execute: expect.any(Function) });
        expect(controller.claim(validCancel('request-shared-2', {
            operatorId: 'owner-2',
            messageId: 'om_two',
...
describe('FeishuQuestionCardController callbacks', () => {
    it('claims a valid owner submit synchronously and responds only from execute', async () => {
        ...
        await controller.present(context);
        const claimed = controller.claim(submit('request-submit', { '0': 'Beijing', '1': ['Logs', 'Metrics'] }));
        ...
        expect(patchCard).toHaveBeenCalledWith('om_1', expect.objectContaining({ schema: '2.0' }));
        expect(JSON.stringify(patchCard.mock.calls.at(-1)?.[1])).toContain('Logs, Metrics');
        expect(JSON.stringify(patchCard.mock.calls.at(-1)?.[1])).toContain('已提交');
        expect(sendFallback).not.toHaveBeenCalled();
        await expect(controller.present(createContext('request-after-submit').context)).resolves.toEqual({ kind: 'presented' });
    });
    it('claims a valid owner cancellation and responds from execute', async () => {
        ...
        expect(patchCard).not.toHaveBeenCalled();
        expect(sendFallback).not.toHaveBeenCalled();
        await expect(controller.present(createContext('request-after-cancel').context)).resolves.toEqual({ kind: 'presented' });
    });
    it('correlates callbacks with the chat id captured at delivery time', async () => {
        ...
        expect(controller.claim(validCancel('request-captured-chat'))).toMatchObject({
            kind: 'handled',
    ...
    it('projects the terminal fallback to the chat id captured at delivery time', async () => {
        ...
        await controller.present(context);
        context.target.chatId = 'oc_changed';
        const claimed = controller.claim(submit('request-captured-fallback', { '0': 'Beijing' }));
        if (claimed.kind !== 'handled' || !claimed.execute) {
            throw new Error('Expected claimed callback execution');
        }
        await claimed.execute();
        expect(sendFallback).toHaveBeenCalledWith('oc_1', expect.stringContaining('已提交'));
    });
    it.each([
    ...
    ])('fails closed for %s without claiming the request', async (_name, data) => {
        const { controller, patchCard } = createHarness();
        const { context, respond } = createContext();
        await controller.present(context);
    ...
    it('keeps a malformed form pending for a later valid owner submission', async () => {
        ...
        expect(patchCard).not.toHaveBeenCalled();
        expect(controller.claim(submit('request-1', { '0': 'Beijing' }))).toMatchObject({
            kind: 'handled',
            execute: expect.any(Function),
        });
    });
    ...
    it.each([
    ...
    ])('keeps the callback-delivered cancel card when the responder %s', async (_name, respond) => {
        ...
        await claimed.execute();
        expect(onError).toHaveBeenCalledWith('question response', expect.any(Error));
        ...
        await expect(controller.present(createContext('request-after-reject').context)).resolves.toEqual({ kind: 'presented' });
    });
    it.each([
        ['returns false', vi.fn().mockResolvedValue(false)],
        ['throws', vi.fn().mockRejectedValue(new Error('response failed'))],
    ])('terminalizes an unaccepted submission as expired when the responder %s', async (_name, respond) => {
        const { controller, onError, patchCard } = createHarness();
        const { context } = createContext('request-submit-failed', { respond });
        await controller.present(context);
        const claimed = controller.claim(submit('request-submit-failed', { '0': 'Beijing' }));
        if (claimed.kind !== 'handled' || !claimed.execute) {
            throw new Error('Expected claimed callback execution');
        }
        await claimed.execute();
        ...
        expect(controller.claim(submit('request-submit-failed', { '0': 'Beijing' }))).toEqual({
            kind: 'handled',
            response: {
                toast: { type: 'warning', content: '该问题已过期或已处理。' },
            },
        });
        await expect(controller.present(createContext('request-after-submit-failure').context)).resolves.toEqual({ kind: 'presented' });
    });
    it('falls back to text when an unaccepted submission cannot patch the card', async () => {
        ...
        await controller.present(context);
        const claimed = controller.claim(submit('request-submit-expiry', { '0': 'Beijing' }));
        if (claimed.kind !== 'handled' || !claimed.execute) {
            throw new Error('Expected claimed callback execution');
        }
        await claimed.execute();
        expect(sendFallback).toHaveBeenCalledWith('oc_1', expect.stringContaining('已过期'));
        expect(sendFallback).toHaveBeenCalledWith('oc_1', expect.stringContaining('Region: Which region?'));
    });
    it('does not reopen an accepted response when its terminal patch fails', async () => {
        ...
        await controller.present(context);
        const claimed = controller.claim(submit('request-patch', { '0': 'Beijing' }));
        if (claimed.kind !== 'handled' || !claimed.execute) {
            throw new Error('Expected claimed callback execution');
        }
        await claimed.execute();
        ...
        expect(onError).toHaveBeenCalledWith('question card finalization', expect.any(Error));
        expect(sendFallback).toHaveBeenCalledWith('oc_1', expect.stringContaining('已提交'));
        expect(sendFallback).toHaveBeenCalledWith('oc_1', expect.stringContaining('Region: Beijing'));
    ...
    it('falls back to text when the terminal patch rejects', async () => {
        ...
        expect(onError).toHaveBeenCalledWith('question card finalization', expect.any(Error));
        expect(sendFallback).toHaveBeenCalledWith('oc_1', expect.stringContaining('已取消'));
        expect(respond).not.toHaveBeenCalled();
    });
    it('contains a rejected terminal fallback without leaking a rejection', async () => {
        ...
        controller.cancelRun('run-1');
        await vi.waitFor(() => {
            expect(onError).toHaveBeenCalledWith('question terminal fallback delivery', expect.any(Error));
        });
        expect(onError).toHaveBeenCalledWith('question card finalization', expect.any(Error));
    });
    it('includes submitted answers in the patch-failure fallback', async () => {
        ...
        await controller.present(context);
        const claimed = controller.claim(submit('request-answer-fallback', { '0': 'Beijing' }));
        if (claimed.kind !== 'handled' || !claimed.execute) {
            throw new Error('Expected claimed callback execution');
        }
        await claimed.execute();
    ...
    it('keeps concurrent sessions and owners independent', async () => {
        ...
        const firstClaimed = controller.claim(validCancel('request-one', { messageId: 'om_one' }));
        const secondClaimed = controller.claim(validCancel('request-two', {
            operatorId: 'owner-2',
            chatId: 'oc_2',
            messageId: 'om_two',
        }));
        const thirdClaimed = controller.claim(validCancel('request-three', {
            chatId: 'oc_3',
            messageId: 'om_three',
        }));
...
describe('FeishuQuestionCardController terminal cleanup', () => {
    ...
    it('expires locally before cancelling the original request', async () => {
        ...
        expect(controller.byRequest
            .size).toBe(0);
        expect(controller
            .activeByScope.size).toBe(0);
    });
    it('keeps the expiry timer alive after a malformed-form rejection', async () => {
        ...
        try {
            ...
            await controller.present(context);
            expect(controller.claim(submit('request-malformed-expiry', { '0': 'Unknown' }))).toEqual({
                kind: 'handled',
                response: {
                    toast: { type: 'warning', content: '请完整选择有效答案。' },
                },
    ...
    it('cancels every live request for a run exactly once', async () => {
        ...
        expect(patchCard).toHaveBeenCalledWith('om_one', expect.objectContaining({ schema: '2.0' }));
        expect(patchCard).toHaveBeenCalledWith('om_two', expect.objectContaining({ schema: '2.0' }));
        expect(JSON.stringify(patchCard.mock.calls[0]?.[1])).toContain('已取消');
        expect(JSON.stringify(patchCard.mock.calls[1]?.[1])).toContain('已取消');
        expect(controller.claim(validCancel('request-run-other', {
            operatorId: 'owner-3',
            chatId: 'oc_3',
            messageId: 'om_three',
        }))).toMatchObject({ kind: 'handled', execute: expect.any(Function) });
        expect(controller.claim(validCancel('request-run-one', { messageId: 'om_one' }))).toEqual({
            kind: 'handled',
            response: {
                toast: { type: 'warning', content: '该问题已过期或已处理。' },
            },
        });
        expect(controller.claim(validCancel('request-run-two', {
            operatorId: 'owner-2',
            chatId: 'oc_2',
            messageId: 'om_two',
        }))).toEqual({
    ...
    it('patches a terminal card after late delivery without reactivating it', async () => {
        ...
        await expect(presenting).resolves.toEqual({ kind: 'presented' });
        expect(patchCard).toHaveBeenCalledWith('om_late', expect.objectContaining({ schema: '2.0' }));
        expect(JSON.stringify(patchCard.mock.calls[0]?.[1])).toContain('已取消');
        expect(respond).not.toHaveBeenCalled();
        expect(controller.claim(validCancel('request-late', { messageId: 'om_late' }))).toEqual({
            kind: 'handled',
            response: {
                toast: { type: 'warning', content: '该问题已过期或已处理。' },
            },
    ...
    it('patches a terminal card when delivery lands across disposal', async () => {
        ...
        expect(JSON.stringify(patchCard.mock.calls[0]?.[1])).toContain('已过期');
        expect(respond).not.toHaveBeenCalled();
        expect(controller.claim(validCancel('request-dispose-late', { messageId: 'om_late' }))).toEqual({
            kind: 'handled',
            response: {
                toast: { type: 'warning', content: '该问题已过期或已处理。' },
            },
    ...
    it('terminalizes a claimed callback settled before execute starts', async () => {
        const { controller, patchCard } = createHarness();
        const { context, respond, settle } = createContext('request-claimed-settled');
        await controller.present(context);
    ...
    it('absorbs the settlement echo of an in-flight accepted response', async () => {
        ...
        await controller.present(context);
        const claimed = controller.claim(submit('request-echo', { '0': 'Beijing' }));
        if (claimed.kind !== 'handled' || !claimed.execute) {
            throw new Error('Expected claimed callback execution');
        }
    ...
    it('lets an accepted in-flight submission override run cancellation', async () => {
        ...
        await controller.present(context);
        const claimed = controller.claim(submit('request-in-flight', { '0': 'Beijing' }));
        if (claimed.kind !== 'handled' || !claimed.execute) {
            throw new Error('Expected claimed callback execution');
        }
        const executing = claimed.execute();
    ...
    it('keeps the run-cancelled label when an in-flight response settles not-accepted', async () => {
        ...
        await controller.present(context);
        const claimed = controller.claim(submit('request-race-expiry', { '0': 'Beijing' }));
        if (claimed.kind !== 'handled' || !claimed.execute) {
            throw new Error('Expected claimed callback execution');
        }
        const executing = claimed.execute();
    ...
    it('cleans an in-flight record on dispose and ignores its late projection', async () => {
        ...
        await controller.present(context);
        const claimed = controller.claim(submit('request-in-flight-dispose', { '0': 'Beijing' }));
        if (claimed.kind !== 'handled' || !claimed.execute) {
            throw new Error('Expected claimed callback execution');
        }
        const executing = claimed.execute();
        ...
        controller.dispose();
        await vi.waitFor(() => expect(patchCard).toHaveBeenCalledOnce());
        await expect(controller.present(createContext('replacement').context)).resolves.toEqual({ kind: 'unsupported' });
        response.resolve(true);
        await executing;
        expect(patchCard).toHaveBeenCalledTimes(1);
        expect(JSON.stringify(patchCard.mock.calls[0]?.[1])).toContain('已过期');
...
function validCancel(requestId = 'request-1', options = {}) {
...
function submit(requestId, formValue, options = {}) {
    const operatorId = Object.hasOwn(options, 'operatorId')
        ? options.operatorId
        : 'owner-1';
    const chatId = Object.hasOwn(options, 'chatId') ? options.chatId : 'oc_1';
...
//# sourceMappingURL=question-card-controller.test.js.map