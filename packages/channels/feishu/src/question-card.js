...
export function buildQuestionCard(context) {
    const elements = [];
...
export function buildQuestionTerminalCard(questions, state, answers) {
...
export function parseQuestionAction(data) {
    ...
    const chatId = nonEmptyString(context?.['open_chat_id']) ??
        nonEmptyString(payload?.['open_chat_id']);
    const messageId = nonEmptyString(context?.['open_message_id']) ??
        nonEmptyString(payload?.['open_message_id']);
    const correlation = {
        ...(chatId ? { chatId } : {}),
        ...(messageId ? { messageId } : {}),
    };
    if (actionId === 'qwen_ask_submit' &&
