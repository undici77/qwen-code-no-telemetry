...
function createMockBridge() {
    return {
        ...
        loadSession: vi.fn().mockImplementation((id) => id),
    };
}
...
class TestableFeishuChannel extends FeishuChannel {
    pushLoop(target, text) {
        return this.pushProactive(target, text);
    }
}
function createTestableChannel(configOverrides) {
    const config = createConfig(configOverrides);
    const bridge = createMockBridge();
...
function createObservedContactChannel(observe, list) {
    const bridge = createMockBridge();
    const channel = new ObservedContactFeishuChannel('test', createConfig({
        blockStreaming: 'on',
        groupPolicy: 'open',
        groups: { '*': { requireMention: false } },
    }), bridge, { observedContacts: { observe, ...(list ? { list } : {}) } });
...
function feishuGroupMessage(messageId) {
    return {
        message: {
...
function feishuDmMessage(messageId) {
    return {
        message: {
            message_id: messageId,
            chat_id: 'oc_dm',
...
function getPrivateMethod(instance, method) {
...
describe('FeishuChannel', () => {
    describe('constructor', () => {
        it('throws if clientId is missing', () => {
            expect(() => createChannel({ clientId: undefined })).toThrow(/requires clientId/);
        });
        it('throws if clientSecret is missing', () => {
            expect(() => createChannel({ clientSecret: undefined })).toThrow(/requires clientId.*clientSecret/);
        });
        ...
        it('logs message debug payloads from the shared handler map', () => {
            ...
            const buildHandlerMap = getPrivateMethod(channel, 'buildHandlerMap').bind(channel);
            const payload = {
                message: {
                    message_id: 'debug-m1',
                    chat_id: 'chat-1',
        ...
        it('routes handled question actions before Stop and executes asynchronously', async () => {
            ...
            const handler = getPrivateMethod(channel, 'buildHandlerMap').call(channel)['card.action.trigger'];
            const payload = {
                action: {
                    value: {
        ...
        it('contains question action execution failures with a labeled error', async () => {
            ...
            try {
                ...
                expect(handler?.(payload)).toBe(response);
                await vi.waitFor(() => {
                    expect(stderr).toHaveBeenCalledWith(expect.stringContaining('question action execution error: boom'));
                });
            }
            finally {
                stderr.mockRestore();
        ...
        it('delegates user input presentation to the question controller', async () => {
            ...
            await expect(getPrivateMethod(channel, 'presentUserInputRequest').call(channel, context)).resolves.toBe(result);
        ...
        it('wires the real question controller through the native card helpers', async () => {
            ...
            const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValue(new Response(JSON.stringify({ data: { message_id: 'om_question' } }), {
                status: 200,
            }));
            const context = {
            ...
            await expect(getPrivateMethod(channel, 'presentUserInputRequest').call(channel, context)).resolves.toEqual({ kind: 'presented' });
            expect(fetchSpy).toHaveBeenCalledWith(expect.stringContaining('/im/v1/messages?receive_id_type=chat_id'), expect.objectContaining({
                method: 'POST',
            ...
            channel.questionCardController.dispose();
            // Drain the dispose-time terminal projection so the PATCH can never
            // reach the real network once the spy is restored.
            await new Promise((resolve) => setTimeout(resolve, 0));
            expect(fetchSpy).toHaveBeenLastCalledWith(expect.stringContaining('/im/v1/messages/om_question'), expect.objectContaining({ method: 'PATCH' }));
        ...
        it('does not send an extra terminal message after a direct question', async () => {
            ...
            getPrivateMethod(channel, 'msgToQuestion').set('inbound_1', 'question?');
            getPrivateMethod(channel, 'onPromptStart').call(channel, 'oc_chat_id', 'session_1', 'inbound_1');
            const context = {
                requestId: 'request-direct',
                sessionId: 'session_1',
                target: { chatId: 'oc_chat_id' },
            };
            await getPrivateMethod(channel, 'presentUserInputRequest').call(channel, context);
            getPrivateMethod(channel, 'onTaskLifecycle').call(channel, {
                channelName: 'feishu',
                chatId: 'oc_chat_id',
                sessionId: 'session_1',
                runId: 'run-1',
            ...
            await getPrivateMethod(channel, 'onPromptEnd').call(channel, 'oc_chat_id', 'session_1', 'inbound_1');
            expect(present).toHaveBeenCalledWith(context);
    ...
    describe('extractContent', () => {
        ...
        beforeEach(() => {
            ...
            extractContent = getPrivateMethod(channel, 'extractContent').bind(channel);
        });
        ...
        it('handles image messages', () => {
            const result = extractContent('image', JSON.stringify({ image_key: 'img_key_123' }));
            expect(result.text).toBe('(image)');
            expect(result.imageKey).toBe('img_key_123');
        });
        it('handles file messages', () => {
            const result = extractContent('file', JSON.stringify({ file_key: 'file_key_456', file_name: 'doc.pdf' }));
        ...
        it('handles media (video) messages', () => {
            const result = extractContent('media', JSON.stringify({ file_key: 'vid_key', file_name: 'video.mp4' }));
    ...
    describe('observed contact enrichment', () => {
        it('resolves labels once and reuses them on later observations', async () => {
            ...
            const onMessage = getPrivateMethod(channel, 'onMessage').bind(channel);
            try {
                ...
                expect(fetchSpy).toHaveBeenCalledWith(expect.stringContaining('user_id_type=open_id'), expect.objectContaining({
                    method: 'POST',
                    body: JSON.stringify({ user_ids: ['ou_user'] }),
                }));
                expect(fetchSpy).toHaveBeenCalledWith(expect.stringContaining('/im/v1/chats/oc_group'), expect.objectContaining({
        ...
        it('does not block inbound processing and silently caches lookup failures', async () => {
            ...
            let rejectLookup = () => { };
            const lookup = new Promise((_resolve, reject) => {
                rejectLookup = reject;
            });
            const fetchSpy = vi
                .spyOn(global, 'fetch')
        ...
        it('repairs labels from a delayed message with a stale snapshot', async () => {
            ...
            const chatLookup = deferred();
            const quotedMessage = deferred();
            ...
            try {
                ...
                reply['message']['parent_id'] =
                    'om_parent';
                onMessage(reply);
                await vi.waitFor(() => expect(fetchSpy).toHaveBeenCalledTimes(3));
                userLookup.resolve(jsonResponse({
                    code: 0,
                    data: { users: [{ name: 'Alice' }] },
                }));
                chatLookup.resolve(jsonResponse({ code: 0, data: { name: 'Project Group' } }));
                await vi.waitFor(() => expect(observe).toHaveBeenCalledTimes(2));
                quotedMessage.resolve(jsonResponse({
                    data: {
                        items: [
                            {
                                msg_type: 'text',
                ...
                expect(fetchSpy.mock.calls.filter(([input]) => String(input).includes('/contact/v3/users/basic_batch'))).toHaveLength(1);
                expect(fetchSpy.mock.calls.filter(([input]) => String(input).includes('/im/v1/chats/oc_group'))).toHaveLength(1);
            }
            finally {
                fetchSpy.mockRestore();
            }
        ...
        it('retries a label lookup when token acquisition fails before the request', async () => {
            ...
            const stderrSpy = vi
                .spyOn(process.stderr, 'write')
                .mockImplementation(() => true);
            const observedUserName = getPrivateMethod(channel, 'observedUserName').bind(channel);
            try {
        ...
        it('retries a chat lookup when token acquisition fails before the request', async () => {
            ...
            const fetchSpy = vi
                ...
                .mockResolvedValueOnce(jsonResponse({ code: 0, data: { name: 'Project Group' } }));
            const stderrSpy = vi
                .spyOn(process.stderr, 'write')
                .mockImplementation(() => true);
            const observedChatName = getPrivateMethod(channel, 'observedChatName').bind(channel);
            try {
                await expect(observedChatName('oc_group')).resolves.toBeUndefined();
                await expect(observedChatName('oc_group')).resolves.toBe('Project Group');
                expect(fetchSpy).toHaveBeenCalledTimes(3);
                expect(stderrSpy).not.toHaveBeenCalled();
            }
            finally {
        ...
        it('sanitizes resolved display names before caching them', async () => {
            ...
            try {
                ...
                expect(channel['observedUserNames']).toEqual(new Map([['ou_user', 'Evil Name']]));
            }
            finally {
                fetchSpy.mockRestore();
            }
        ...
        it('reuses a cached label without a new lookup after lookup entries evict', async () => {
            ...
            try {
                ...
                channel['observedUserLookups'].clear();
                await expect(observedUserName('ou_user')).resolves.toBe('Alice');
                expect(fetchSpy).toHaveBeenCalledTimes(1);
            }
            finally {
        ...
        it('keeps the ID label when a resolved name sanitizes to unknown', async () => {
            ...
            try {
                await expect(observedUserName('ou_user')).resolves.toBeUndefined();
                expect(channel['observedUserNames']).toEqual(new Map());
            }
            finally {
                fetchSpy.mockRestore();
            }
        ...
        it('enriches direct-chat senders without issuing chat lookups', async () => {
            ...
            try {
                ...
                expect(fetchSpy).toHaveBeenCalledTimes(1);
                expect(fetchSpy.mock.calls.some(([input]) => String(input).includes('/im/v1/chats/'))).toBe(false);
            }
            finally {
                fetchSpy.mockRestore();
            }
        ...
        it('maps non-open_id senders to the matching user_id_type', async () => {
            ...
            try {
                ...
                unionOnly['sender']['sender_id'] = {
                    union_id: 'on_user',
                };
                onMessage(unionOnly);
                await vi.waitFor(() => expect(fetchSpy).toHaveBeenCalledWith(expect.stringContaining('user_id_type=union_id'), expect.objectContaining({
                    body: JSON.stringify({ user_ids: ['on_user'] }),
                })));
                const userIdOnly = feishuGroupMessage('message_user');
                userIdOnly['sender']['sender_id'] = {
                    user_id: 'user_1',
                };
                onMessage(userIdOnly);
                await vi.waitFor(() => expect(fetchSpy).toHaveBeenCalledWith(expect.stringContaining('user_id_type=user_id'), expect.objectContaining({
                    body: JSON.stringify({ user_ids: ['user_1'] }),
                })));
                expect(fetchSpy.mock.calls.some(([input]) => String(input).includes('user_id_type=open_id'))).toBe(false);
            }
            finally {
                fetchSpy.mockRestore();
            }
        ...
        it('shares one in-flight lookup across concurrent messages', async () => {
            ...
            try {
                ...
                userLookup.resolve(jsonResponse({ code: 0, data: { users: [{ name: 'Alice' }] } }));
                chatLookup.resolve(jsonResponse({ code: 0, data: { name: 'Project Group' } }));
                await vi.waitFor(() => expect(observe).toHaveBeenCalledTimes(3));
                expect(observe).toHaveBeenLastCalledWith('test', {
                    user: { id: 'ou_user', label: 'Alice' },
        ...
        it('hydrates label caches from persisted observations after a restart', async () => {
            ...
            try {
                ...
                foreign['message']['chat_id'] =
                    'oc_foreign';
                foreign['sender']['sender_id'] = {
                    open_id: 'ou_foreign',
                };
                onMessage(foreign);
                await vi.waitFor(() => {
                ...
                expect(bridge.prompt).toHaveBeenNthCalledWith(3, expect.any(String), expect.stringContaining('[ou_foreign]'), expect.anything());
                expect(list).toHaveBeenCalledTimes(1);
                expect(fetchSpy).toHaveBeenCalledTimes(2);
            }
            finally {
        ...
        it('attaches cached labels to later envelopes and prompts', async () => {
            ...
            try {
                ...
                expect(bridge.prompt).toHaveBeenNthCalledWith(1, expect.any(String), expect.stringContaining('[ou_user]'), expect.anything());
                expect(bridge.prompt).toHaveBeenNthCalledWith(2, expect.any(String), expect.stringContaining('[Alice]'), expect.anything());
                expect(observe).toHaveBeenNthCalledWith(3, 'test', {
                    user: { id: 'ou_user', label: 'Alice' },
                    group: { id: 'oc_group', label: 'Project Group' },
                });
        ...
        it('logs token errors for core callers that join a silent-initiated refresh', async () => {
            ...
            const stderrSpy = vi
                .spyOn(process.stderr, 'write')
                .mockImplementation(() => true);
            const getTenantAccessToken = getPrivateMethod(channel, 'getTenantAccessToken').bind(channel);
            try {
        ...
        it('prefers the newest persisted label when hydrating overlapping contacts', async () => {
            ...
            try {
                ...
                expect(bridge.prompt).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('[New Name]'), expect.anything());
                expect(observe).toHaveBeenNthCalledWith(1, 'test', {
        ...
        it('bounds runtime label caches to the observed-contact retention cap', () => {
            const observe = vi.fn();
            const { channel } = createObservedContactChannel(observe);
            const capObservedCache = getPrivateMethod(channel, 'capObservedCache').bind(channel);
            const cache = new Map();
            for (let i = 0; i < 525; i++) {
        ...
        it('re-hydrates persisted labels after in-lifetime cache eviction', async () => {
            ...
            try {
                ...
                const names = channel['observedUserNames'];
                for (let i = 0; i < 500; i++) {
    ...
    describe('extractCardText', () => {
        ...
        beforeEach(() => {
            channel = createChannel();
            extractCardText = getPrivateMethod(channel, 'extractCardText').bind(channel);
        });
    ...
    describe('state machine: dedup', () => {
        ...
        it('deduplicates messages with same ID within TTL', () => {
            ...
            const cardSessions = getPrivateMethod(channel, 'cardSessions');
    ...
    describe('state machine: cleanupCard', () => {
        ...
        beforeEach(() => {
            channel = createChannel();
            cleanupCard = getPrivateMethod(channel, 'cleanupCard').bind(channel);
        });
        it('cleans up all maps for a given inbound message', () => {
            const cardSessions = getPrivateMethod(channel, 'cardSessions');
            const sessionToInboundMsg = getPrivateMethod(channel, 'sessionToInboundMsg');
            const msgToQuestion = getPrivateMethod(channel, 'msgToQuestion');
            const msgToSenderName = getPrivateMethod(channel, 'msgToSenderName');
            // Populate all maps
    ...
    describe('prompt hook inbound IDs', () => {
        it('ignores loop job ids that were not registered by processMessage', async () => {
            ...
            channel.createStreamingCard = createStreamingCard;
            channel.addReaction =
                addReaction;
            channel.removeReaction = removeReaction;
            getPrivateMethod(channel, 'msgToQuestion').set('inbound_1', 'question?');
            getPrivateMethod(channel, 'onPromptStart').call(channel, 'oc_chat_id', 'session_1', 'job-1');
            await getPrivateMethod(channel, 'onPromptEnd').call(channel, 'oc_chat_id', 'session_1', 'job-1');
            expect(getPrivateMethod(channel, 'sessionToInboundMsg')
                .size).toBe(0);
        ...
        it('waits for the first response chunk before creating a card', async () => {
            ...
            try {
                ...
                expect(getPrivateMethod(channel, 'sessionToInboundMsg').get('session_1')).toBe('inbound_1');
                expect(createStreamingCard).not.toHaveBeenCalled();
                // A prompt-start creation regression may only schedule the card;
                // flush timers so the scheduled shape is caught as well.
                await vi.runAllTimersAsync();
                expect(createStreamingCard).not.toHaveBeenCalled();
                getPrivateMethod(channel, 'onResponseChunk').call(channel, 'oc_chat_id', 'first visible answer', 'session_1');
                await vi.runAllTimersAsync();
                ...
                expect(createStreamingCard.mock.calls[0]?.[1]).toContain('first visible answer');
        ...
        it('releases only output-card state at an input request and creates a second card later', async () => {
            ...
            try {
                ...
                const msgToQuestion = getPrivateMethod(channel, 'msgToQuestion');
                const msgToSenderName = getPrivateMethod(channel, 'msgToSenderName');
                const msgToSenderId = getPrivateMethod(channel, 'msgToSenderId');
                ...
                await getPrivateMethod(channel, 'onOutputSegmentEnd').call(channel, 'oc_chat_id', 'session_1', segment, 'input_requested');
                expect(updateCard).toHaveBeenCalledWith('om_first_output', '@sender\n\nbefore question', true, 'inbound_1', '已完成');
                ...
                getPrivateMethod(channel, 'onResponseChunk').call(channel, 'oc_chat_id', 'after answer', 'session_1');
                await vi.runAllTimersAsync();
                expect(createStreamingCard).toHaveBeenCalledOnce();
                expect(createStreamingCard.mock.calls[0]?.[0]).toBe('oc_chat_id');
                expect(createStreamingCard.mock.calls[0]?.[1]).toBe('@sender\n\nafter answer');
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it.each([
        ...
        ])('compensates when input-request card finalization %s', async (_name, result) => {
            const channel = createChannel();
            ...
            getPrivateMethod(channel, 'cardSessions').set('inbound_1', {
                messageId: 'om_first_output',
                created: true,
                creating: false,
                stopped: false,
            ...
            getPrivateMethod(channel, 'sessionToInboundMsg').set('session_1', 'inbound_1');
            getPrivateMethod(channel, 'msgToSenderName').set('inbound_1', '@sender');
            await getPrivateMethod(channel, 'onOutputSegmentEnd').call(channel, 'oc_chat_id', 'session_1', {}, 'input_requested');
            expect(deleteCard).toHaveBeenCalledWith('om_first_output');
            expect(sendMessage).toHaveBeenCalledWith('oc_chat_id', '@sender\n\nbefore question');
            expect(getPrivateMethod(channel, 'cardSessions').get('inbound_1')).toMatchObject({
                created: false,
                creating: false,
                accumulatedText: '',
            });
        ...
        it('caps the input-request table-stripped retry at the card size limit', async () => {
            ...
            await getPrivateMethod(channel, 'onOutputSegmentEnd').call(channel, 'oc_chat_id', 'session_1', {}, 'input_requested');
            expect(updateCard).toHaveBeenCalledTimes(2);
            const retryText = updateCard.mock.calls[1]?.[1];
        ...
        it.each([
            ...
            getPrivateMethod(channel, 'cardSessions').set('inbound_1', seeded);
            getPrivateMethod(channel, 'sessionToInboundMsg').set('session_1', 'inbound_1');
            getPrivateMethod(channel, 'msgToSenderName').set('inbound_1', '@sender');
            await getPrivateMethod(channel, 'onOutputSegmentEnd').call(channel, 'oc_chat_id', 'session_1', {}, 'input_requested');
            expect(sendMessage).toHaveBeenCalledWith('oc_chat_id', '@sender\n\nbefore question');
            ...
            expect(getPrivateMethod(channel, 'cardSessions').get('inbound_1')).toMatchObject({ created: false, creating: false, accumulatedText: '' });
        });
    });
    describe('state machine: stop button during card creation', () => {
        ...
        it('marks card as stopped even when still creating', async () => {
            ...
            channel.requestActivePromptCancellation = cancelPromptSpy;
            ...
            const onCardAction = getPrivateMethod(channel, 'onCardAction').bind(channel);
            onCardAction({
                action: { value: { action: 'stop' } },
            ...
            expect(cancelPromptSpy).toHaveBeenCalledWith('session_abc', 'cancel_command');
        ...
        it('keeps user stop label when cancellation lifecycle marks the card cancelled', async () => {
            ...
            channel.updateCard =
                updateCard;
            channel.requestActivePromptCancellation = vi
                .fn()
            ...
            getPrivateMethod(channel, 'sessionToInboundMsg').set('session_abc', 'inbound_1');
            getPrivateMethod(channel, 'msgToSenderId').set('inbound_1', 'user_open_id');
            getPrivateMethod(channel, 'onCardAction').call(channel, {
                action: { value: { action: 'stop' } },
                context: { open_message_id: 'card_1' },
                operator: { open_id: 'user_open_id' },
            });
    ...
    describe('connect: WebSocket', () => {
        ...
        it('rejects invalid credentials before starting WebSocket', async () => {
            ...
            await expect(channel.connect()).rejects.toThrow('failed to authenticate Feishu credentials');
    ...
    describe('disconnect', () => {
        it('closes wsClient on disconnect', () => {
            ...
            expect(channel['wsClient']).toBeUndefined();
        });
        it('clears dedup timer on disconnect', () => {
            const channel = createChannel();
            const clearIntervalSpy = vi.spyOn(global, 'clearInterval');
            const timer = setInterval(() => { }, 60000);
            channel['dedupTimer'] = timer;
            channel.disconnect();
    ...
    describe('extractContent: post at-node mentions', () => {
        it('extracts @mention user_name from post at nodes', () => {
            const channel = createChannel();
            const extractContent = getPrivateMethod(channel, 'extractContent').bind(channel);
            const post = {
                zh_cn: {
                    title: '',
                    content: [
    ...
    describe('onCardAction: cancelSession failure', () => {
        it('shows stop failure status when cancelSession throws', async () => {
            ...
            channel.requestActivePromptCancellation = vi.fn().mockResolvedValue(false);
            ...
            const cardText = updateCardSpy.mock.calls[0][1];
        ...
        it('uses divider status shape when stopped empty-card fallback sends a message', async () => {
            ...
            const config = createConfig();
            const channel = new FeishuChannel('test', config, bridge);
            channel.requestActivePromptCancellation = vi.fn().mockResolvedValue(true);
            const cardSessions = getPrivateMethod(channel, 'cardSessions');
            cardSessions.set('inbound_1', {
                messageId: 'card_1',
                created: true,
            ...
            getPrivateMethod(channel, 'msgToSenderId').set('inbound_1', 'original_user');
            getPrivateMethod(channel, 'sessionToInboundMsg').set('session_1', 'inbound_1');
            channel['updateCard'] = vi
                .fn()
                .mockResolvedValue(false);
            channel['deleteCard'] = vi
                .fn()
            ...
            await vi.waitFor(() => {
                expect(sendMessage).toHaveBeenCalledWith('oc_chat_id', '---\n*已停止生成*');
            });
        });
        it('awaits an in-flight streaming PATCH before the stop patch', async () => {
            ...
            try {
                ...
                getPrivateMethod(channel, 'msgToSenderId').set('inbound_1', 'original_user');
                getPrivateMethod(channel, 'msgToSenderName').set('inbound_1', '@sender');
                getPrivateMethod(channel, 'onResponseChunk').call(channel, 'oc_chat_id', ' more', 'session_1');
    ...
    describe('deleteCard', () => {
        it('returns true on successful deletion', async () => {
            ...
            const deleteCard = getPrivateMethod(channel, 'deleteCard').bind(channel);
            const result = await deleteCard('om_test_msg_id');
            expect(result).toBe(true);
            expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('/im/v1/messages/om_test_msg_id'), expect.objectContaining({ method: 'DELETE' }));
        });
        it('returns false when token is unavailable', async () => {
            ...
            const fetchMock = vi
                .fn()
                .mockResolvedValue(new Response(JSON.stringify({ code: -1 }), { status: 500 }));
            vi.spyOn(global, 'fetch').mockImplementation(fetchMock);
            const deleteCard = getPrivateMethod(channel, 'deleteCard').bind(channel);
            const result = await deleteCard('om_test_msg_id');
            expect(result).toBe(false);
        ...
        it('clears token cache on 401', async () => {
            ...
            expect(channel['tokenCache']).toBeUndefined();
        });
    });
    describe('question card native helpers', () => {
        ...
        function createWiringContext(requestId) {
        ...
        function getWiringController(channel) {
            return channel.questionCardController;
        }
        it('sends an arbitrary interactive card and returns its message id', async () => {
            ...
            await expect(getPrivateMethod(channel, 'sendInteractiveCard').call(channel, 'oc_chat', card)).resolves.toBe('om_question');
            expect(fetchSpy).toHaveBeenCalledWith(expect.stringContaining('/im/v1/messages?receive_id_type=chat_id'), expect.objectContaining({
                method: 'POST',
                headers: expect.objectContaining({
        ...
        it('rejects interactive-card delivery without an access token', async () => {
            ...
            await expect(getPrivateMethod(channel, 'sendInteractiveCard').call(channel, 'oc_chat', {})).rejects.toThrow('no access token');
        });
        it('rejects HTTP failures and clears a rejected cached token', async () => {
            ...
            await expect(getPrivateMethod(channel, 'sendInteractiveCard').call(channel, 'oc_chat', {})).rejects.toThrow('HTTP 401');
            expect(channel['tokenCache']).toBeUndefined();
        });
        it('rejects a successful response without a message id', async () => {
            ...
            await expect(getPrivateMethod(channel, 'sendInteractiveCard').call(channel, 'oc_chat', {})).rejects.toThrow('no message id');
        });
        it('patches an arbitrary interactive card in place', async () => {
            ...
            const card = { schema: '2.0', body: { elements: [] } };
            await expect(getPrivateMethod(channel, 'patchInteractiveCard').call(channel, 'om_question', card)).resolves.toBe(true);
            expect(fetchSpy).toHaveBeenCalledWith(expect.stringContaining('/im/v1/messages/om_question'), expect.objectContaining({
                method: 'PATCH',
                headers: expect.objectContaining({
                    Authorization: 'Bearer tenant-token',
                }),
        ...
        it('rejects card patch HTTP failures and clears a rejected cached token', async () => {
            ...
            vi.spyOn(global, 'fetch').mockResolvedValue(new Response('unauthorized', { status: 401 }));
            await expect(getPrivateMethod(channel, 'patchInteractiveCard').call(channel, 'om_question', {})).resolves.toBe(false);
            expect(channel['tokenCache']).toBeUndefined();
        });
        it('rejects card patches for invalid message ids without fetching', async () => {
            ...
            const fetchSpy = vi
                .spyOn(global, 'fetch')
                .mockResolvedValue(new Response(JSON.stringify({}), { status: 200 }));
            await expect(getPrivateMethod(channel, 'patchInteractiveCard').call(channel, 'om invalid id', {})).resolves.toBe(false);
        ...
        it('keeps streaming cards on the generic send and patch paths', async () => {
            ...
            await expect(getPrivateMethod(channel, 'createStreamingCard').call(channel, 'oc_chat', 'answer')).resolves.toEqual({ messageId: 'om_stream', success: true });
            await expect(getPrivateMethod(channel, 'updateCard').call(channel, 'om_stream', 'updated')).resolves.toBe(true);
            await expect(getPrivateMethod(channel, 'updateCard').call(channel, 'om_stream', 'final answer', true, undefined, '已完成')).resolves.toBe(true);
            expect(sendInteractiveCard).toHaveBeenCalledWith('oc_chat', expect.objectContaining({ schema: '2.0' }));
            expect(patchInteractiveCard).toHaveBeenCalledWith('om_stream', expect.objectContaining({ schema: '2.0' }));
            const streamingCard = JSON.stringify(patchInteractiveCard.mock.calls[0]?.[1]);
        ...
        it('preserves streaming-card failure reporting around the rejecting helper', async () => {
            ...
            vi.spyOn(global, 'fetch').mockResolvedValue(new Response('server down', { status: 500 }));
            const stderr = vi
                .spyOn(process.stderr, 'write')
                .mockImplementation(() => true);
            await expect(getPrivateMethod(channel, 'createStreamingCard').call(channel, 'oc_chat', 'answer')).resolves.toEqual({ messageId: '', success: false });
            expect(stderr).toHaveBeenCalledWith(expect.stringContaining('createStreamingCard failed: HTTP 500 server down'));
        });
        it('wires the real question-card controller through the card transport', async () => {
            ...
            const fetchSpy = vi
                .spyOn(global, 'fetch')
                .mockResolvedValue(new Response(JSON.stringify({ code: 0, data: { message_id: 'om_question' } }), { status: 200 }));
            ...
            await new Promise((resolve) => setTimeout(resolve, 0));
            expect(fetchSpy).toHaveBeenCalledTimes(2);
            expect(fetchSpy).toHaveBeenLastCalledWith(expect.stringContaining('/im/v1/messages/om_question'), expect.objectContaining({
                method: 'PATCH',
                headers: expect.objectContaining({
                    Authorization: 'Bearer tenant-token',
                }),
    ...
    describe('sendMessage: token failure logging', () => {
        it('logs and returns early when token is unavailable', async () => {
            ...
            expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('Cannot send: no access token'));
            stderrSpy.mockRestore();
            fetchSpy.mockRestore();
        });
        it('rejects proactive sends when token is unavailable', async () => {
            ...
            const fetchSpy = vi
                .spyOn(global, 'fetch')
                .mockResolvedValue(new Response(null, { status: 500 }));
            await expect(channel.pushLoop({
                channelName: 'test',
                senderId: 'ou_user',
                chatId: 'oc_chat_id',
            }, 'hello')).rejects.toThrow('Feishu sendMessage failed: no access token');
            expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('Cannot send: no access token'));
            stderrSpy.mockRestore();
            fetchSpy.mockRestore();
        });
        it('rejects proactive sends when Feishu returns an error', async () => {
            ...
            await expect(channel.pushLoop({
                ...
                senderId: 'ou_user',
                chatId: 'oc_chat_id',
            }, 'hello')).rejects.toEqual(expect.objectContaining({
                disposition: 'transient',
                message: 'Feishu sendMessage failed: HTTP 500',
            }));
            expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('sendMessage failed: HTTP 500'));
            stderrSpy.mockRestore();
        });
        it('classifies non-retryable proactive HTTP failures as permanent', async () => {
            ...
            vi.spyOn(global, 'fetch').mockResolvedValue(new Response('permission denied', { status: 403 }));
            vi.spyOn(process.stderr, 'write').mockImplementation(() => true);
            await expect(channel.deliverProactive({ channelName: 'test', type: 'user', id: 'ou_user' }, 'direct result')).rejects.toEqual(expect.objectContaining({
        ...
        it('maps typed chat and user deliveries to Feishu receive ID types', async () => {
            ...
            const fetchSpy = vi
                .spyOn(global, 'fetch')
                .mockResolvedValue(new Response('{}', { status: 200 }));
            await channel.deliverProactive({ channelName: 'test', type: 'chat', id: 'oc_group' }, 'group result');
            await channel.deliverProactive({ channelName: 'test', type: 'user', id: 'ou_user' }, 'direct result');
            expect(fetchSpy.mock.calls[0][0]).toBe('https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=chat_id');
            expect(fetchSpy.mock.calls[1][0]).toBe('https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=open_id');
        });
        it('classifies proactive network failures as transient', async () => {
            ...
            vi.spyOn(global, 'fetch').mockRejectedValueOnce(new Error('socket token=secret'));
            await expect(channel.deliverProactive({ channelName: 'test', type: 'user', id: 'ou_user' }, 'direct result')).rejects.toEqual(expect.objectContaining({
                disposition: 'transient',
    ...
    describe('onPromptEnd: error recovery branches', () => {
        it('cancels question cards for the exact terminal run even without output-card state', () => {
            ...
            const onTaskLifecycle = getPrivateMethod(channel, 'onTaskLifecycle').bind(channel);
            const baseEvent = {
                channelName: 'feishu',
                chatId: 'oc_chat_id',
        ...
        it('keeps a local card state so failures before the first chunk stay visible', async () => {
            ...
            await getPrivateMethod(channel, 'onPromptEnd').call(channel, 'oc_chat_id', 'session_1', 'inbound_1');
            expect(sendMessage).toHaveBeenCalledWith('oc_chat_id', expect.stringContaining('已失败'));
        });
        it('sends error fallback when card creation failed and no accumulated text', async () => {
            ...
            channel['sendMessage'] =
                sendMessageSpy;
            const onPromptEnd = getPrivateMethod(channel, 'onPromptEnd').bind(channel);
            const sessionToInboundMsg = getPrivateMethod(channel, 'sessionToInboundMsg');
            sessionToInboundMsg.set('session_1', 'inbound_1');
            await onPromptEnd('oc_chat_id', 'session_1');
            // Should send error fallback message
            expect(sendMessageSpy).toHaveBeenCalledWith('oc_chat_id', expect.stringContaining('出错了'));
        });
        it('sends accumulated text via sendMessage when card creation failed', async () => {
            ...
            sessionToInboundMsg.set('session_1', 'inbound_1');
            await onPromptEnd('oc_chat_id', 'session_1');
            expect(sendMessageSpy).toHaveBeenCalledWith('oc_chat_id', expect.stringContaining('partial response text'));
        });
        it('clears accumulated card text at response boundary', () => {
            ...
            getPrivateMethod(channel, 'sessionToInboundMsg').set('session_1', 'inbound_1');
            getPrivateMethod(channel, 'onResponseBoundary').call(channel, 'oc_chat_id', 'session_1');
        ...
        it('cancels pending card updates at response boundary', () => {
            ...
            try {
                ...
                getPrivateMethod(channel, 'sessionToInboundMsg').set('session_1', 'inbound_1');
                getPrivateMethod(channel, 'onResponseBoundary').call(channel, 'oc_chat_id', 'session_1');
                expect(cardSessions.get('inbound_1')?.pendingUpdateTimer).toBeUndefined();
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it('forwards response_boundary segment ends and snapshots the cleared text', async () => {
            ...
            getPrivateMethod(channel, 'sessionToInboundMsg').set('session_1', 'inbound_1');
            await getPrivateMethod(channel, 'onOutputSegmentEnd').call(channel, 'oc_chat_id', 'session_1', {}, 'response_boundary');
            expect(cardSessions.get('inbound_1')).toEqual({
                accumulatedText: '',
                stopped: false,
                updateQueued: false,
        ...
        it('keeps the first terminal lifecycle state for prompt-end card finalization', async () => {
            ...
            channel.updateCard =
                updateCard;
            const lifecycle = getPrivateMethod(channel, 'onTaskLifecycle');
            const baseEvent = {
                channelName: 'feishu',
                chatId: 'oc_chat_id',
                sessionId: 'session_1',
            ...
            expect(stderr).toHaveBeenCalledWith(expect.stringContaining('conflicting terminal event cancelled after completed'));
            stderr.mockRestore();
        });
        ...
        it('finalizes creating cards as failed instead of stopped after prompt end', async () => {
            ...
            const createCardPromise = new Promise((resolve) => {
            ...
            getPrivateMethod(channel, 'msgToQuestion').set('inbound_1', 'question?');
            getPrivateMethod(channel, 'onPromptStart').call(channel, 'oc_chat_id', 'session_1', 'inbound_1');
            getPrivateMethod(channel, 'onResponseChunk').call(channel, 'oc_chat_id', 'partial answer', 'session_1');
    ...
    describe('onResponseComplete: stopped card cleanup', () => {
        it('cleans up and returns early when card was stopped', async () => {
            ...
            channel['sendMessage'] =
                sendMessageSpy;
            const onResponseComplete = getPrivateMethod(channel, 'onResponseComplete').bind(channel);
        ...
        it('marks completed cards with the completed status label', async () => {
            ...
            channel.updateCard =
                updateCard;
            await getPrivateMethod(channel, 'onResponseComplete').call(channel, 'oc_chat_id', 'final answer', 'session_1');
        ...
        it('awaits an in-flight throttled streaming PATCH before the completion patch', async () => {
            ...
            try {
                ...
                await vi.advanceTimersByTimeAsync(1_500);
                expect(updateCard).toHaveBeenCalledTimes(1);
                const completing = getPrivateMethod(channel, 'onResponseComplete').call(channel, 'oc_chat_id', 'final answer', 'session_1');
        ...
        it('lets the pending creation timer complete instead of demoting to plain text', async () => {
            ...
            try {
                ...
                getPrivateMethod(channel, 'msgToQuestion').set('inbound_1', 'question?');
                getPrivateMethod(channel, 'onPromptStart').call(channel, 'oc_chat_id', 'session_1', 'inbound_1');
                getPrivateMethod(channel, 'onResponseChunk').call(channel, 'oc_chat_id', 'first chunk', 'session_1');
        ...
        it('keeps stop status when user stops during final card update', async () => {
            ...
            channel.updateCard =
                updateCard;
            channel.requestActivePromptCancellation = vi.fn().mockResolvedValue(true);
            const complete = getPrivateMethod(channel, 'onResponseComplete').call(channel, 'oc_chat_id', 'final answer', 'session_1');
            await vi.waitFor(() => {
                expect(updateCard).toHaveBeenCalledTimes(1);
            });
            getPrivateMethod(channel, 'onCardAction').call(channel, {
            ...
            await complete;
            expect(updateCard).toHaveBeenCalledTimes(2);
            const stoppedCard = updateCard.mock.calls[1][1];
        ...
        it('reserves final card space for the completed status label', async () => {
            ...
            channel.updateCard =
                updateCard;
            await getPrivateMethod(channel, 'onResponseComplete').call(channel, 'oc_chat_id', 'x'.repeat(20_000), 'session_1');
    ...
    describe('webhook: JSON parse error logging', () => {
        it('logs error message on malformed JSON body', async () => {
            ...
            const channel = createChannel();
            const connectWebhook = getPrivateMethod(channel, 'connectWebhook').bind(channel);
    ...
    describe('question flow output-card handoff', () => {
        it('finalizes a streamed card before a question when the boundary closed the segment first', async () => {
            ...
            try {
                const channel = createChannel();
                let resolveFinalPatch;
                const updateCard = vi.fn().mockImplementation(() => new Promise((resolve) => {
                ...
                getPrivateMethod(channel, 'msgToQuestion').set('inbound_1', 'question?');
                getPrivateMethod(channel, 'onPromptStart').call(channel, 'oc_chat_id', 'session_1', 'inbound_1');
                getPrivateMethod(channel, 'onResponseChunk').call(channel, 'oc_chat_id', 'pre-question text', 'session_1');
                await vi.runAllTimersAsync();
                await getPrivateMethod(channel, 'onOutputSegmentEnd').call(channel, 'oc_chat_id', 'session_1', {}, 'response_boundary');
                const context = {
                ...
                const presenting = getPrivateMethod(channel, 'presentUserInputRequest').call(channel, context);
                await vi.advanceTimersByTimeAsync(0);
                ...
                await presenting;
                expect(updateCard).toHaveBeenCalledWith('om_first_output', 'pre-question text', true, 'inbound_1', '已完成');
                expect(getPrivateMethod(channel, 'cardSessions').get('inbound_1')).toMatchObject({
                    created: false,
                    creating: false,
                    accumulatedText: '',
                ...
                expect(present).toHaveBeenCalledWith(context);
                expect(updateCard.mock.invocationCallOrder[0]).toBeLessThan(present.mock.invocationCallOrder[0]);
            }
            finally {
                vi.useRealTimers();
            }
        ...
        it('surfaces a post-answer failure after the output card was released', async () => {
            ...
            getPrivateMethod(channel, 'msgToQuestion').set('inbound_1', 'question?');
            getPrivateMethod(channel, 'onPromptStart').call(channel, 'oc_chat_id', 'session_1', 'inbound_1');
            await getPrivateMethod(channel, 'presentUserInputRequest').call(channel, {
        ...
        it('anchors the creating-timeout at post-answer card creation start', async () => {
            ...
            try {
                ...
                expect(cardSessions.get('inbound_1')?.lastUpdateAt).toBe(startedAt + 5 * 60 * 1000);
                ...
                getPrivateMethod(channel, 'onResponseChunk').call(channel, 'oc_chat_id', 'post-answer text', 'session_1');
                const state = cardSessions.get('inbound_1');
        ...
        it('awaits an in-flight throttled streaming PATCH before the final patch', async () => {
            ...
            try {
                ...
                expect(updateCard).toHaveBeenCalledTimes(1);
                expect(updateCard.mock.calls[0]?.[2]).toBe(false);
                const finalizing = getPrivateMethod(channel, 'onOutputSegmentEnd').call(channel, 'oc_chat_id', 'session_1', {}, 'input_requested');
                await Promise.resolve();
        ...
        it('leaves the output card to the stop flow when stop is in flight before a question', async () => {
            const channel = createChannel();
            let resolveCancel;
            channel.requestActivePromptCancellation = vi.fn().mockImplementation(() => new Promise((resolve) => {
        ...
        it('renders the stop label when Stop settles during input-request finalization', async () => {
            ...
            getPrivateMethod(channel, 'msgToSenderId').set('inbound_1', 'original_user');
            getPrivateMethod(channel, 'msgToSenderName').set('inbound_1', '@sender');
            const ending = getPrivateMethod(channel, 'onOutputSegmentEnd').call(channel, 'oc_chat_id', 'session_1', {}, 'input_requested');
            await Promise.resolve();
            expect(updateCard).not.toHaveBeenCalled();
            ...
            await vi.waitFor(() => expect(cardSessions.get('inbound_1')?.['stopped']).toBe(true));
            resolveDrain();
            await ending;
            expect(updateCard).toHaveBeenCalledTimes(1);
        ...
        it('renders the stop label when Stop settles during the final patch await', async () => {
            const channel = createChannel();
            let resolveFinalPatch;
            const finalPatch = new Promise((resolve) => {
                resolveFinalPatch = resolve;
            });
        ...
        it('coalesces throttled updates queued behind a stalled PATCH', async () => {
            ...
            try {
                ...
                getPrivateMethod(channel, 'sessionToInboundMsg').set('session_1', 'inbound_1');
                const chunk = getPrivateMethod(channel, 'onResponseChunk');
        ...
        it('carries a concurrent terminal status into the released card entry', async () => {
            ...
            await ending;
            await getPrivateMethod(channel, 'onPromptEnd').call(channel, 'oc_chat_id', 'session_1', 'inbound_1');
            expect(sendMessage).toHaveBeenCalledWith('oc_chat_id', expect.stringContaining('已取消'));
        });
        it('deletes a late card when the input request lands mid-creation', async () => {
            ...
            try {
                const channel = createChannel();
                let resolveCreate;
                const createStreamingCard = vi.fn().mockImplementation(() => new Promise((resolve) => {
                ...
                getPrivateMethod(channel, 'msgToQuestion').set('inbound_1', 'question?');
                getPrivateMethod(channel, 'msgToSenderName').set('inbound_1', '@sender');
                getPrivateMethod(channel, 'msgToSenderId').set('inbound_1', 'owner-1');
                getPrivateMethod(channel, 'onPromptStart').call(channel, 'oc_chat_id', 'session_1', 'inbound_1');
                getPrivateMethod(channel, 'onResponseChunk').call(channel, 'oc_chat_id', 'pre-question text', 'session_1');
                ...
                expect(updateCard).not.toHaveBeenCalled();
                expect(getPrivateMethod(channel, 'msgToQuestion').get('inbound_1')).toBe('question?');
                expect(getPrivateMethod(channel, 'msgToSenderName').get('inbound_1')).toBe('@sender');
                expect(getPrivateMethod(channel, 'msgToSenderId').get('inbound_1')).toBe('owner-1');
                expect(getPrivateMethod(channel, 'sessionToInboundMsg').get('session_1')).toBe('inbound_1');
            }
            finally {
                vi.useRealTimers();
        ...
        it('truncates oversized input-request card finalization', async () => {
            ...
            getPrivateMethod(channel, 'sessionToInboundMsg').set('session_1', 'inbound_1');
            await getPrivateMethod(channel, 'onOutputSegmentEnd').call(channel, 'oc_chat_id', 'session_1', {}, 'input_requested');
            const finalText = updateCard.mock.calls[0]?.[1];
            const marker = '\n\n_(内容过长，已截断早期内容)_';
