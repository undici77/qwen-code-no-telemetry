...
describe('FeishuChannel', () => {
  describe('constructor', () => {
    ...
    it('logs message debug payloads from the shared handler map', () => {
      ...
      const buildHandlerMap = getPrivateMethod(channel, 'buildHandlerMap').bind(
        channel,
      );
      const payload = {
        message: {
    ...
    it('routes handled question actions before Stop and executes asynchronously', async () => {
      ...
      const handler = getPrivateMethod(channel, 'buildHandlerMap').call(
        channel,
      )['card.action.trigger'];
      const payload = {
        action: {
    ...
    it('delegates user input presentation to the question controller', async () => {
      ...
      await expect(
        getPrivateMethod(channel, 'presentUserInputRequest').call(
          channel,
          context,
        ),
    ...
    it('does not send an extra terminal message after a direct question', async () => {
      ...
      getPrivateMethod(channel, 'onPromptStart').call(
        channel,
        'oc_chat_id',
        'session_1',
        'inbound_1',
      ...
      await getPrivateMethod(channel, 'presentUserInputRequest').call(
        channel,
        context,
      );
      ...
      await getPrivateMethod(channel, 'onPromptEnd').call(
        channel,
        'oc_chat_id',
        'session_1',
        'inbound_1',
  ...
  describe('observed contact enrichment', () => {
    ...
    it('repairs labels from a delayed message with a stale snapshot', async () => {
      ...
      try {
        ...
        reply['message']['parent_id'] = 'om_parent';
        onMessage(reply);
    ...
    it('sanitizes resolved display names before caching them', async () => {
      ...
      try {
        ...
        expect(channel['observedUserNames']).toEqual(
          new Map([['ou_user', 'Evil Name']]),
        );
      } finally {
        fetchSpy.mockRestore();
      }
    ...
    it('hydrates label caches from persisted observations after a restart', async () => {
      ...
      try {
        ...
        foreign['message']['chat_id'] = 'oc_foreign';
  ...
  describe('state machine: cleanupCard', () => {
    ...
    it('cleans up all maps for a given inbound message', () => {
      const cardSessions = getPrivateMethod(channel, 'cardSessions');
      const sessionToInboundMsg = getPrivateMethod(
        channel,
        'sessionToInboundMsg',
      );
  ...
  describe('prompt hook inbound IDs', () => {
    it('ignores loop job ids that were not registered by processMessage', async () => {
      ...
      channel.addReaction = addReaction;
      ...
      expect(getPrivateMethod(channel, 'sessionToInboundMsg').size).toBe(0);
    ...
    it('waits for the first response chunk before creating a card', async () => {
      ...
      try {
        ...
        getPrivateMethod(channel, 'msgToQuestion').set(
          'inbound_1',
          'question?',
        );
        getPrivateMethod(channel, 'onPromptStart').call(
        ...
        expect(
          getPrivateMethod(channel, 'sessionToInboundMsg').get('session_1'),
        ).toBe('inbound_1');
        expect(createStreamingCard).not.toHaveBeenCalled();
        ...
        expect(createStreamingCard).not.toHaveBeenCalled();
        getPrivateMethod(channel, 'onResponseChunk').call(
          channel,
          'oc_chat_id',
    ...
    it('releases only output-card state at an input request and creates a second card later', async () => {
      ...
      try {
        ...
        await getPrivateMethod(channel, 'onOutputSegmentEnd').call(
          channel,
          'oc_chat_id',
          'session_1',
          segment,
    ...
    it.each([
      ...
      async (_name, result) => {
        ...
        getPrivateMethod(channel, 'sessionToInboundMsg').set(
          'session_1',
          'inbound_1',
        );
        getPrivateMethod(channel, 'msgToSenderName').set(
          'inbound_1',
          '@sender',
        );
        await getPrivateMethod(channel, 'onOutputSegmentEnd').call(
        ...
        expect(
          getPrivateMethod(channel, 'cardSessions').get('inbound_1'),
        ).toMatchObject({
          created: false,
          creating: false,
          accumulatedText: '',
  ...
  describe('state machine: stop button during card creation', () => {
    ...
    it('marks card as stopped even when still creating', async () => {
      ...
      channel['updateCard'] = updateCardMock;
      ...
      const onCardAction = getPrivateMethod(channel, 'onCardAction').bind(
        channel,
      );
      onCardAction({
        action: { value: { action: 'stop' } },
    ...
    it('keeps user stop label when cancellation lifecycle marks the card cancelled', async () => {
      ...
      getPrivateMethod(channel, 'msgToSenderId').set(
        'inbound_1',
        'user_open_id',
      );
  ...
  describe('extractContent: post at-node mentions', () => {
    it('extracts @mention user_name from post at nodes', () => {
      const channel = createChannel();
      const extractContent = getPrivateMethod(channel, 'extractContent').bind(
        channel,
      );
      const post = {
        zh_cn: {
  ...
  describe('onCardAction: cancelSession failure', () => {
    it('shows stop failure status when cancelSession throws', async () => {
      ...
      channel['updateCard'] = updateCardSpy;
      const onCardAction = getPrivateMethod(channel, 'onCardAction').bind(
        channel,
      );
      onCardAction({
    ...
    it('uses divider status shape when stopped empty-card fallback sends a message', async () => {
      ...
      channel['updateCard'] = vi.fn().mockResolvedValue(false);
      channel['deleteCard'] = vi.fn().mockResolvedValue(undefined);
      const sendMessage = vi.fn().mockResolvedValue(undefined);
  ...
  describe('question card native helpers', () => {
    ...
    it('sends an arbitrary interactive card and returns its message id', async () => {
      ...
      await expect(
        getPrivateMethod(channel, 'sendInteractiveCard').call(
          channel,
          'oc_chat',
          card,
        ),
    ...
    it('patches an arbitrary interactive card in place', async () => {
      ...
      const card = { schema: '2.0', body: { elements: [] } };
      await expect(
        getPrivateMethod(channel, 'patchInteractiveCard').call(
          channel,
          'om_question',
          card,
        ),
    ...
    it('keeps streaming cards on the generic send and patch paths', async () => {
      ...
      await expect(
        getPrivateMethod(channel, 'createStreamingCard').call(
          channel,
          'oc_chat',
          'answer',
        ),
      ...
      await expect(
        getPrivateMethod(channel, 'updateCard').call(
          channel,
          'om_stream',
          'updated',
        ),
  ...
  describe('onPromptEnd: error recovery branches', () => {
    it('cancels question cards for the exact terminal run even without output-card state', () => {
      ...
      const onTaskLifecycle = getPrivateMethod(channel, 'onTaskLifecycle').bind(
        channel,
      );
      const baseEvent = {
        channelName: 'feishu',
    ...
    it('sends error fallback when card creation failed and no accumulated text', async () => {
      ...
      channel['sendMessage'] = sendMessageSpy;
      const onPromptEnd = getPrivateMethod(channel, 'onPromptEnd').bind(
        channel,
      );
      const sessionToInboundMsg = getPrivateMethod(
        channel,
    ...
    it('clears accumulated card text at response boundary', () => {
      ...
      getPrivateMethod(channel, 'onResponseBoundary').call(
        channel,
        'oc_chat_id',
        'session_1',
      );
  ...
  describe('onResponseComplete: stopped card cleanup', () => {
    ...
    it('marks completed cards with the completed status label', async () => {
      ...
      const updateCard = vi.fn().mockResolvedValue(true);
      channel.updateCard = updateCard;
      await getPrivateMethod(channel, 'onResponseComplete').call(
        channel,
        'oc_chat_id',
        'final answer',
        'session_1',
    ...
    it('awaits an in-flight throttled streaming PATCH before the completion patch', async () => {
      ...
      try {
        ...
        await vi.advanceTimersByTimeAsync(1_500);
        expect(updateCard).toHaveBeenCalledTimes(1);
        const completing = getPrivateMethod(channel, 'onResponseComplete').call(
          channel,
          'oc_chat_id',
          'final answer',
          'session_1',
    ...
    it('keeps stop status when user stops during final card update', async () => {
      ...
      channel.updateCard = updateCard;
      channel.requestActivePromptCancellation = vi.fn().mockResolvedValue(true);
      const complete = getPrivateMethod(channel, 'onResponseComplete').call(
        channel,
        'oc_chat_id',
        'final answer',
        'session_1',
  ...
  describe('webhook: JSON parse error logging', () => {
    it('logs error message on malformed JSON body', async () => {
      ...
      const channel = createChannel();
      const connectWebhook = getPrivateMethod(channel, 'connectWebhook').bind(
        channel,
      );
  ...
  describe('question flow output-card handoff', () => {
    ...
    it('awaits an in-flight throttled streaming PATCH before the final patch', async () => {
      ...
      try {
        ...
        expect(updateCard).toHaveBeenCalledTimes(1);
        expect(updateCard.mock.calls[0]?.[2]).toBe(false);
        const finalizing = getPrivateMethod(channel, 'onOutputSegmentEnd').call(
          channel,
          'oc_chat_id',
          'session_1',
          {},
    ...
    it('leaves the output card to the stop flow when stop is in flight before a question', async () => {
      const channel = createChannel();
      let resolveCancel;
      channel.requestActivePromptCancellation = vi.fn().mockImplementation(
        () =>
          new Promise((resolve) => {
    ...
    it('renders the stop label when Stop settles during input-request finalization', async () => {
      ...
      getPrivateMethod(channel, 'msgToSenderName').set('inbound_1', '@sender');
      const ending = getPrivateMethod(channel, 'onOutputSegmentEnd').call(
        channel,
        'oc_chat_id',
        'session_1',
        {},
    ...
    it('deletes a late card when the input request lands mid-creation', async () => {
      ...
      try {
        ...
        expect(updateCard).not.toHaveBeenCalled();
        expect(
          getPrivateMethod(channel, 'msgToQuestion').get('inbound_1'),
        ).toBe('question?');
        expect(
          getPrivateMethod(channel, 'msgToSenderName').get('inbound_1'),
        ).toBe('@sender');
        expect(
          getPrivateMethod(channel, 'msgToSenderId').get('inbound_1'),
        ).toBe('owner-1');
        expect(
          getPrivateMethod(channel, 'sessionToInboundMsg').get('session_1'),
        ).toBe('inbound_1');
