...
describe('downloadMedia', () => {
    ...
    it('should download file successfully', async () => {
        ...
        const result = await downloadMedia('om_valid_msg', 'file_valid_key', 'image', 'valid_token');
        expect(result).not.toBeNull();
        expect(result?.buffer).toEqual(Buffer.from(mockData));
        expect(result?.mimeType).toBe('image/png');
    });
    it('should reject invalid messageId (path traversal)', async () => {
        const result = await downloadMedia('../../../etc/passwd', 'file_key', 'file', 'token');
        expect(result).toBeNull();
        expect(fetchSpy).not.toHaveBeenCalled();
    });
    it('should reject invalid fileKey (path traversal)', async () => {
        const result = await downloadMedia('om_msg', '../../../etc/passwd', 'file', 'token');
        expect(result).toBeNull();
        expect(fetchSpy).not.toHaveBeenCalled();
    });
