import { buildQuestionCard, buildQuestionTerminalCard, parseQuestionAction, parseQuestionAnswers, terminalLabels, } from './question-card.js';
export class FeishuQuestionCardController {
    ...
    async present(context) {
        ...
        try {
            const messageId = await this.options.sendCard(record.chatId, buildQuestionCard(context));
            if (!messageId)
            ...
            if (this.isTerminated(record))
                return { kind: 'presented' };
            this.options.onError?.('question card delivery', error);
            await this.finalize(record, 'cancelled');
            try {
                await this.options.sendFallback(record.chatId, this.fallbackText(record.context));
            }
            catch (fallbackError) {
    ...
    claim(data) {
        ...
        if (action.kind === 'submit') {
            const answers = parseQuestionAnswers(record.context.questions, action.formValue);
            if (!answers) {
                return {
                    kind: 'handled',
                    response: {
            ...
            return {
                ...
                response: {
                    ...
                    card: {
                        type: 'raw',
                        data: buildQuestionTerminalCard(record.context.questions, 'processing', answers),
                    },
                },
        ...
        return {
            ...
            response: {
                ...
                card: {
                    type: 'raw',
                    data: buildQuestionTerminalCard(record.context.questions, 'cancelled'),
                },
            },
    ...
    cancelRun(runId, terminalState = 'cancelled') {
        for (const record of [...this.byRequest.values()]) {
            if (record.runId === runId)
                void this.finalize(record, terminalState);
        }
    }
    dispose() {
        this.disposed = true;
    ...
    async respond(record, terminalState, answers) {
        ...
        try {
            ...
            if (!accepted) {
                this.options.onError?.('question response', new Error('Question response was no longer accepted'));
            }
    ...
    async completeResponse(record, terminalState) {
        if (record.state !== 'terminal') {
            await this.finalize(record, terminalState);
            return;
        }
        if (record.ignoreResponseProjection)
            return;
        ...
        if (terminalState === 'expired' && record.terminalState)
            return;
        record.terminalState = terminalState;
        await this.projectTerminal(record);
    }
    projectTerminal(record, messageId = record.messageId) {
        if (!messageId || !record.terminalState)
            return Promise.resolve();
        if (record.terminalDeliveredByClaim)
            return Promise.resolve();
        const terminalState = record.terminalState;
        const answers = record.terminalAnswers;
        const card = buildQuestionTerminalCard(record.context.questions, terminalState, answers);
        const projection = (record.projection ?? Promise.resolve()).then(async () => {
            let patched = false;
            try {
                patched = await this.options.patchCard(messageId, card);
                if (!patched) {
                    this.options.onError?.('question card finalization', new Error('Feishu card patch was not accepted'));
                }
            }
            catch (error) {
            ...
            if (!patched) {
                const details = record.context.questions
                    ...
                    .join('\n');
                try {
                    await this.options.sendFallback(record.chatId, `${terminalLabels[terminalState]}\n${details}`);
                }
                catch (fallbackError) {
                    this.options.onError?.('question terminal fallback delivery', fallbackError);
                }
            }
        });
