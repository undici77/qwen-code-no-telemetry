 ...
 */
export async function downloadMedia(messageId, fileKey, resourceType, accessToken) {
    if (!messageId ||
        !fileKey ||
        !accessToken ||
    ...
    try {
        ...
        if (!resp.ok) {
            ...
            process.stderr.write(`[Feishu] downloadMedia failed: HTTP ${resp.status} ${detail}\n`);
            return null;
        }
        ...
        if (contentLength && parseInt(contentLength, 10) > MAX_DOWNLOAD_BYTES) {
            ...
            process.stderr.write(`[Feishu] downloadMedia rejected: size ${contentLength} exceeds ${MAX_DOWNLOAD_BYTES} byte limit\n`);
            return null;
        }
        ...
        while (true) {
            ...
            if (totalSize > MAX_DOWNLOAD_BYTES) {
                ...
                // above).
                await reader.cancel();
                process.stderr.write(`[Feishu] downloadMedia rejected: actual size exceeds ${MAX_DOWNLOAD_BYTES} byte limit\n`);
                return null;
            }
        ...
        process.stderr.write(`[Feishu] downloadMedia error: ${err instanceof Error ? err.message : err}\n`);
        return null;
    }
}
