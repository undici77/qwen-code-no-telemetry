...
import { DingTalkMediaUploadError, findImageMarkers, readValidatedImage, replaceImageMarkers, sanitizeStreamingImageMarkers, stripPartialImageMarker, uploadDingTalkImage, } from './outbound-image.js';
...
describe('outbound image markers', () => {
    ...
    it('replaces repeated markers by source position', () => {
        ...
        const markers = findImageMarkers(text);
        expect(replaceImageMarkers(text, markers, ['first', 'second'])).toBe(['`[IMAGE: /tmp/same.png]`', 'first', 'second'].join('\n'));
    });
});
describe('partial image markers', () => {
    ...
    it('still strips partial IMAGE prefixes', () => {
        ...
        expect(stripPartialImageMarker('see [IMAGE: /tmp/pic.png')).toBe('see [Image pending]');
    });
});
describe('streaming image markers', () => {
    it('hides complete and incomplete visible image paths', () => {
        expect(sanitizeStreamingImageMarkers('before [IMAGE: /Users/ben/private/image.png] after')).toBe('before [Image pending] after');
        expect(sanitizeStreamingImageMarkers('before [IMAGE: /Users/ben/private/image')).toBe('before [Image pending]');
        expect(sanitizeStreamingImageMarkers('before [IMAGE: /Users/ben/[private/image')).toBe('before [Image pending]');
    });
    it('preserves image-like text inside code', () => {
        expect(sanitizeStreamingImageMarkers([
...
describe('readValidatedImage', () => {
    it('reads a regular image inside the workspace', () => {
        ...
        expect(readValidatedImage(imagePath, {
    ...
    it('rejects a symlink that escapes the allowed directories', () => {
        ...
        expect(() => readValidatedImage(linkedImage, {
            workspaceDir: workspace,
            temporaryDir: workspace,
...
describe('uploadDingTalkImage', () => {
    it('uploads a validated image and returns its MediaID', async () => {
        const fetchSpy = vi
            ...
            .mockResolvedValue(new Response(JSON.stringify({ errcode: 0, media_id: '@lAL-test-media-id' }), { status: 200 }));
        await expect(uploadDingTalkImage({
            data: PNG_DATA,
            fileName: 'image.png',
            mimeType: 'image/png',
        }, 'access-token')).resolves.toBe('@lAL-test-media-id');
        ...
        expect(media.name).toBe('image.png');
    });
    it.each([40014, 42001])('marks DingTalk token error %s as retryable authentication failure', async (errcode) => {
        vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ errcode, errmsg: 'expired' }), {
            status: 200,
        }));
    ...
    it('does not include the access token in upload errors', async () => {
        ...
        await expect(uploadDingTalkImage({
            ...
            fileName: 'image.png',
            mimeType: 'image/png',
        }, 'secret-access-token')).rejects.not.toThrow(/secret-access-token/);
    });
    it('does not include a credential-bearing request URL in network errors', async () => {
        vi.spyOn(globalThis, 'fetch').mockRejectedValue(new Error('request to https://oapi.dingtalk.com/media/upload?access_token=secret-access-token failed'));
        await expect(uploadDingTalkImage({
            data: PNG_DATA,
            fileName: 'image.png',
            mimeType: 'image/png',
...
//# sourceMappingURL=outbound-image.test.js.map