...
import { STATUS_CARD_TEMPLATE_ID, } from './interactive-card-client.js';
...
export class StatusCardController {
    options;
    recordsBySegment = new Map();
    recordsByOutTrack = new Map();
    segmentIdsByRun = new Map();
    ...
    createRecord(segment, target, initialContent = '') {
        ...
        const segmentIds = this.segmentIdsByRun.get(record.runId) ?? new Set();
    ...
    complete(segmentId, text, retainedContent) {
        return this.finalize(segmentId, boundContent(text), 'Completed', false, retainedContent);
    }
    ...
    claimStop(outTrackId, actorId) {
        ...
        return {
            ...
            execute: async () => {
                ...
                if (this.recordsByOutTrack.get(outTrackId) !== record ||
                    record.terminal) {
                    return;
                }
                if (!cancelled) {
    ...
    scheduleFlush(record) {
        if (record.flushTimer || record.inFlight || record.terminal)
            return;
    ...
    async finalize(segmentId, content, state, isError, retainedContent) {
        ...
        try {
            ...
            const finalContent = boundContent(sanitizeStreamingImageMarkers(retained));
            await this.options.client.openOrUpdateStream({
                outTrackId: record.outTrackId,
                key: 'content',
                content: '',
    ...
    statusLine(record, state) {
        const second = Math.max(0, Math.floor((Date.now() - record.startedAt) / 1000));
    ...
    async updateRunningStatus(record) {
        if (record.terminal || record.streamFailed)
            return;
    ...
    scheduleStatusRefresh(record, intervalMs = STATUS_REFRESH_INTERVAL_MS) {
        if (record.terminal || record.streamFailed || record.statusTimer)
            return;
...
//# sourceMappingURL=status-card-controller.js.map