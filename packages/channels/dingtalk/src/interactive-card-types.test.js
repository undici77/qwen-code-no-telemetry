...
import { DINGTALK_INTERACTIVE_CARD_TIMEOUT_EXCLUSIVE_MINIMUM, DINGTALK_INTERACTIVE_CARD_TIMEOUT_MAXIMUM_MS, parseDingtalkCardCallback, parseDingtalkInteractiveCardConfig, } from './interactive-card-types.js';
...
describe('interactive card config', () => {
    it('keeps management descriptor values compatible with the runtime parser', () => {
        const interactiveCards = plugin.management?.fields.find((field) => field.key === 'interactiveCards');
        const questionCard = interactiveCards?.properties?.find((field) => field.key === 'questionCard');
        const timeout = questionCard?.properties?.find((field) => field.key === 'timeoutMs');
        expect(DINGTALK_INTERACTIVE_CARD_TIMEOUT_EXCLUSIVE_MINIMUM).toBe(0);
        expect(timeout?.kind === 'number' ? timeout.exclusiveMinimum : undefined).toBe(DINGTALK_INTERACTIVE_CARD_TIMEOUT_EXCLUSIVE_MINIMUM);
    ...
    it('supports explicit and independent disabling', () => {
        expect(parseDingtalkInteractiveCardConfig({
            enabled: true,
            statusCard: { enabled: false },
    ...
    it('rejects invalid nested values and timeouts', () => {
        ...
        expect(() => parseDingtalkInteractiveCardConfig({ statusCard: { enabled: 'yes' } })).toThrow('statusCard.enabled');
    });
    it('clamps question timeouts at the setTimeout maximum delay', () => {
        ...
        }).questionCard.timeoutMs).toBe(DINGTALK_INTERACTIVE_CARD_TIMEOUT_MAXIMUM_MS);
    });
});
describe('card callback parser', () => {
    it('extracts a trusted actor from an otherwise incomplete callback', () => {
        const parseActorId = interactiveCardTypes.parseDingtalkCardActorId;
    ...
    it('normalizes embedded payloads, owner, action, and form data', () => {
        expect(parseDingtalkCardCallback({
            userId: ' owner-1 ',
...
//# sourceMappingURL=interactive-card-types.test.js.map