...
import { DingtalkInteractiveCardClient, QUESTION_CARD_TEMPLATE_ID, STATUS_CARD_TEMPLATE_ID, } from './interactive-card-client.js';
...
describe('DingtalkInteractiveCardClient', () => {
    ...
    it('creates and delivers a group status card with string parameters', async () => {
        ...
        expect(url).toBe('https://api.dingtalk.com/v1.0/card/instances/createAndDeliver');
    ...
    it('rejects HTTP and per-recipient delivery failures', async () => {
        ...
        await expect(createClient().updateInstance({
            outTrackId: 'status-1',
            cardParamMap: {},
        ...
        await expect(createClient().createAndDeliver({
            templateId: QUESTION_CARD_TEMPLATE_ID,
            outTrackId: 'question-1',
            target: { chatId: 'user-1', isGroup: false },
            cardParamMap: {},
...
//# sourceMappingURL=interactive-card-client.test.js.map