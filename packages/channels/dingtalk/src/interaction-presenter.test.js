...
import { QUESTION_CARD_TEMPLATE_ID, STATUS_CARD_TEMPLATE_ID, } from './interactive-card-client.js';
...
function acceptedExecution(value) {
...
function segment(segmentId, overrides = {}) {
    return {
...
function questionContext(precedingSegmentId, requestId = 'request-1') {
...
function createHarness() {
    ...
    const client = {
        createAndDeliver: vi.fn().mockImplementation(async (request) => {
            projectionOrder.push(request.templateId === STATUS_CARD_TEMPLATE_ID
                ? 'create:status'
                : 'create:question');
        }),
        openOrUpdateStream: vi.fn().mockResolvedValue(undefined),
        updateInstance: vi.fn().mockImplementation(async (request) => {
            if (typeof request.cardParamMap.statusLine === 'string' &&
    ...
    const questionCards = new QuestionCardController({
        ...
        timeoutMs: 300_000,
        sendFallback,
        reserveRunProjection: (runId) => presenterRef.current?.reserveProjection(runId),
    });
...
describe('DingtalkInteractionPresenter', () => {
    it('creates the running card as soon as the run starts', async () => {
        ...
        await vi.waitFor(() => {
            expect(client.createAndDeliver).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('adds the group sender only to the final model output', async () => {
        ...
        await presenter.closeOutput('segment-1', '@衍*星\n\n@衍\\*星\n\n最终答案', 'completed');
        expect(client.updateInstance).toHaveBeenLastCalledWith(expect.objectContaining({
            cardParamMap: expect.objectContaining({
                content: '@衍\\*星\n\n最终答案',
            }),
        }));
    ...
    it('fails an eagerly created running card before any output is emitted', async () => {
        const { client, presenter } = createHarness();
        presenter.startStatusCard('run-1');
        presenter.terminalizeRun('run-1', 'failed', 'private failure at /Users/ben/private');
        await vi.waitFor(() => {
            expect(client.updateInstance).toHaveBeenLastCalledWith(expect.objectContaining({
                cardParamMap: expect.objectContaining({
        ...
        expect(JSON.stringify(vi.mocked(client.updateInstance).mock.calls)).not.toContain('/Users/ben/private');
    });
    it('finalizes an eagerly created card when a run completes without output', async () => {
        ...
        await vi.waitFor(() => {
            expect(client.updateInstance).toHaveBeenCalledWith(expect.objectContaining({
                cardParamMap: expect.objectContaining({
                    flowStatus: 3,
        ...
        expect(vi.mocked(client.updateInstance).mock.calls.length).toBe(callsAfterTerminal);
    });
    ...
    it('presents a direct question without creating a status card', async () => {
        ...
        expect(vi
            ...
            .mock.calls.map(([request]) => request.templateId)).toEqual([QUESTION_CARD_TEMPLATE_ID]);
    });
    ...
    it('does not create a status card when direct question delivery fails', async () => {
        const { client, presenter } = createHarness();
        vi.mocked(client.createAndDeliver).mockRejectedValueOnce(new Error('question template unavailable'));
        const context = questionContext();
        await expect(presenter.presentInput(context)).resolves.toEqual({
            kind: 'handled',
        });
    ...
    it.each(['input_requested', 'completed'])('hides local image paths when output ends with %s', async (reason) => {
        const { client, presenter } = createHarness();
        presenter.appendOutput(segment('segment-1'), 'before [IMAGE: /Users/ben/private/image.png] after');
        ...
        await vi.waitFor(() => {
            ...
            expect(JSON.stringify(terminalPayload)).not.toContain('/Users/ben/private');
        });
    });
    it('does not expose buffered model text when a run fails', async () => {
        const { client, presenter } = createHarness();
        presenter.appendOutput(segment('segment-1'), '我来查看 [IMAGE: /Users/ben/private/image.png]');
    ...
    it('streams original model text across response boundaries and replaces it with the final answer', async () => {
        ...
        const statusCards = vi
            .mocked(client.createAndDeliver)
            .mock.calls.filter(([request]) => request.templateId === STATUS_CARD_TEMPLATE_ID);
    ...
    it('drains the pending card snapshot at response boundaries', async () => {
        ...
        expect(vi
            .mocked(client.openOrUpdateStream)
            .mock.calls.map(([request]) => request.content)).toContain('segment one');
    ...
    it('falls back at response boundaries after the card stream fails', async () => {
        ...
        await vi.waitFor(() => expect(client.openOrUpdateStream).toHaveBeenCalledOnce());
        ...
        expect(client.openOrUpdateStream).toHaveBeenCalledWith(expect.objectContaining({ content: 'intermediate result' }));
        vi.mocked(client.openOrUpdateStream).mockRejectedValueOnce(new Error('stream blip'));
        presenter.appendOutput(segment('segment-1'), ' updated');
        await vi.waitFor(() => expect(client.openOrUpdateStream).toHaveBeenCalledTimes(2));
        await expect(presenter.closeOutput('segment-1', '', 'response_boundary')).resolves.toBe(true);
        expect(sendFallback).toHaveBeenCalledWith('cid-1', 'intermediate result updated', 'session-1');
    });
    it.each(['failed', 'input_requested'])('attributes the terminal card to the group sender on %s output close', async (reason) => {
        const { client, presenter } = createHarness();
        presenter.registerRun('run-1', 'owner-1', target, 'session-1', {
            senderName: '衍*星',
        });
        presenter.appendOutput(segment('segment-1'), 'Explanation');
        await presenter.closeOutput('segment-1', '', reason);
        const expectedContent = reason === 'failed'
            ? '@衍\\*星\n\n本次处理失败，请稍后重试。'
            : '@衍\\*星\n\nExplanation';
        await vi.waitFor(() => {
            const terminalPayload = vi
    ...
    ])('attributes the terminal card to the group sender when the run is %s', async (terminal, detail, expectedBody) => {
        const { client, presenter } = createHarness();
        presenter.registerRun('run-1', 'owner-1', target, 'session-1', {
            senderName: '衍*星',
        });
    ...
    it('re-delivers boundary content when the run later fails', async () => {
        ...
        expect(sendFallback).toHaveBeenCalledWith('cid-1', 'intermediate result', 'session-1');
    });
    ...
    it('neutralizes partial image markers in text fallbacks', async () => {
        ...
        presenter.registerRun('run-1', 'owner-1', target);
        presenter.appendOutput(segment('segment-1'), 'photo [IMAGE: /Users/ben/priv');
        await expect(presenter.closeOutput('segment-1', '', 'response_boundary')).resolves.toBe(true);
        expect(sendFallback).toHaveBeenCalledWith('cid-1', 'photo [Image pending]', 'session-1');
    });
    it('keeps a bare trailing bracket out of text fallbacks', async () => {
        ...
        await expect(presenter.closeOutput('segment-1', '', 'response_boundary')).resolves.toBe(true);
        expect(sendFallback).toHaveBeenCalledWith('cid-1', 'result: arr[', 'session-1');
    });
    it('keeps complete image markers uploadable in text fallbacks', async () => {
        const sendFallback = vi.fn().mockResolvedValue(undefined);
        const presenter = new DingtalkInteractionPresenter({ sendFallback });
        presenter.registerRun('run-1', 'owner-1', target);
        presenter.appendOutput(segment('segment-1'), 'photo [IMAGE: /tmp/image.png] ready');
        await presenter.closeOutput('segment-1', '', 'response_boundary');
        expect(sendFallback).toHaveBeenCalledWith('cid-1', 'photo [IMAGE: /tmp/image.png] ready', 'session-1');
    });
    ...
    it('falls back at a boundary when the in-flight card creation fails', async () => {
        ...
        creation.resolve();
        await expect(closed).resolves.toBe(true);
        expect(sendFallback).toHaveBeenCalledWith('cid-1', 'segment one', 'session-1');
    });
    it.each([
        ['cancel_command', 'Stopped'],
        ['steer', 'Cancelled'],
    ])('hides local image paths when output becomes %s', async (reason, expectedState) => {
        const { client, presenter } = createHarness();
        presenter.appendOutput(segment('segment-1'), 'before [IMAGE: /Users/ben/private/image.png] after');
        ...
        await vi.waitFor(() => {
            ...
            expect(terminalPayload).toMatchObject({
                ...
                copy_content: reason === 'cancel_command' ? '任务已停止' : '任务已取消',
                statusLine: expect.stringMatching(new RegExp(`^${expectedState} · \\d+s$`)),
            });
            expect(JSON.stringify(terminalPayload)).not.toContain('/Users/ben/private');
        });
    });
    it('falls back to text before presenting a question when output finalization fails', async () => {
        ...
        presenter.appendOutput(segment('segment-1'), 'Explanation');
        await expect(presenter.closeOutput('segment-1', '', 'input_requested')).resolves.toBe(true);
        await presenter.presentInput(questionContext('segment-1'));
        expect(sendFallback).toHaveBeenCalledWith('cid-1', 'Explanation', 'session-1');
        expect(client.createAndDeliver).toHaveBeenLastCalledWith(expect.objectContaining({ templateId: QUESTION_CARD_TEMPLATE_ID }));
    });
    it('keeps the card sender prefix out of a non-card fallback', async () => {
        ...
        presenter.appendOutput(segment('segment-1'), 'Final answer');
        await presenter.closeOutput('segment-1', '', 'completed');
        expect(sendFallback).toHaveBeenCalledWith('cid-1', 'Final answer', 'session-1');
    ...
    it('surfaces a failed text fallback to the shared delivery boundary', async () => {
        ...
        presenter.appendOutput(segment('segment-1'), 'Explanation');
        await expect(presenter.closeOutput('segment-1', '', 'input_requested')).rejects.toThrow('fallback unavailable');
    });
    ...
    it('propagates the run session into the card stop action', async () => {
        ...
        const outTrackId = vi.mocked(client.createAndDeliver).mock.calls[0][0]
            .outTrackId;
        const execute = acceptedExecution(statusCards.claimStop(outTrackId, 'owner-1'));
        await execute();
    ...
    it('preserves the lifecycle cancellation reason for the status card', async () => {
        ...
        expect(client.updateInstance).not.toHaveBeenCalledWith(expect.objectContaining({
            cardParamMap: expect.objectContaining({
    ...
    it('expires a pending question when a new message supersedes the run', async () => {
        ...
        const questionOutTrackId = vi
            .mocked(client.createAndDeliver)
            .mock.calls.find(([request]) => request.templateId === QUESTION_CARD_TEMPLATE_ID)[0].outTrackId;
        presenter.terminalizeRun('run-1', 'cancelled', 'steer');
        await vi.waitFor(() => expect(client.updateInstance).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('does not let an old run terminal event expire a newer question', async () => {
        ...
        const questionOutTrackIds = vi
            .mocked(client.createAndDeliver)
            .mock.calls.filter(([request]) => request.templateId === QUESTION_CARD_TEMPLATE_ID)
            .map(([request]) => request.outTrackId);
        const secondOutTrackId = questionOutTrackIds[1];
        vi.mocked(client.updateInstance).mockClear();
        presenter.terminalizeRun('run-1', 'cancelled', 'steer');
        expect(client.updateInstance).not.toHaveBeenCalledWith(expect.objectContaining({ outTrackId: secondOutTrackId }));
        const executeSecond = acceptedExecution(questionCards.claim({
    ...
    it('creates a new status card after the question is submitted', async () => {
        const { client, presenter, projectionOrder, questionCards } = createHarness();
        presenter.appendOutput(segment('segment-1'), 'Explanation');
        await presenter.closeOutput('segment-1', '', 'input_requested');
        await presenter.presentInput(questionContext('segment-1'));
        const questionOutTrackId = vi
            .mocked(client.createAndDeliver)
            .mock.calls.find(([request]) => request.templateId === QUESTION_CARD_TEMPLATE_ID)[0].outTrackId;
        await acceptedExecution(questionCards.claim({
            outTrackId: questionOutTrackId,
            actionId: 'submit',
            actorId: 'owner-1',
            formData: { '0': 'Beijing' },
        ...
        expect(projectionOrder.indexOf('update:question:submitted')).toBeLessThan(projectionOrder.lastIndexOf('create:status'));
    });
    ...
    it('does not create continuation output before the question terminal update finishes', async () => {
        ...
        const response = acceptedExecution(questionCards.claim({
            outTrackId: questionOutTrackId,
            actionId: 'submit',
            actorId: 'owner-1',
            formData: { '0': 'Beijing' },
        }))();
        await vi.waitFor(() => expect(projectionOrder).toContain('update:question:submitted'));
        presenter.appendOutput(segment('segment-2'), 'Continuation');
        await new Promise((resolve) => setTimeout(resolve, 0));
        expect(vi
            .mocked(client.createAndDeliver)
            .mock.calls.filter(([request]) => request.templateId === STATUS_CARD_TEMPLATE_ID)).toHaveLength(1);
        terminalUpdate.resolve();
        await response;
        await vi.waitFor(() => expect(vi
            .mocked(client.createAndDeliver)
            .mock.calls.filter(([request]) => request.templateId === STATUS_CARD_TEMPLATE_ID)).toHaveLength(2));
    });
    it('does not block a subsequent question on the previous terminal card update', async () => {
        ...
        const firstResponse = acceptedExecution(questionCards.claim({
        ...
        const secondPresentation = presenter.presentInput(questionContext(undefined, 'request-2'));
        await new Promise((resolve) => setTimeout(resolve, 0));
        const questionCreatesBeforeTerminalUpdate = vi
            .mocked(client.createAndDeliver)
            .mock.calls.filter(([request]) => request.templateId === QUESTION_CARD_TEMPLATE_ID).length;
        terminalUpdate.resolve();
        await firstResponse;
...
//# sourceMappingURL=interaction-presenter.test.js.map