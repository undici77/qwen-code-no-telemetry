...
describe('QuestionCardController', () => {
    ...
    it('renders all normalized questions in one form card', async () => {
        ...
        await expect(controller.present(context, { chatId: 'cid-1', isGroup: true })).resolves.toEqual({ kind: 'presented' });
    ...
    it('falls back visibly and cancels when card delivery fails', async () => {
        ...
        vi.mocked(client.createAndDeliver).mockRejectedValue(new Error('template unavailable'));
        const { context, respond } = createContext();
        await expect(controller.present(context, { chatId: 'cid-1', isGroup: true })).resolves.toEqual({ kind: 'handled' });
        expect(sendFallback).toHaveBeenCalledWith('cid-1', expect.stringContaining('request was cancelled'));
    ...
    it('claims one owner callback and submits validated answers', async () => {
        ...
        const outTrackId = vi.mocked(client.createAndDeliver).mock.calls[0][0]
            .outTrackId;
        expect(callbackResult(controller.claim({
            outTrackId,
            actionId: 'submit',
            actorId: 'other',
        ...
        const execute = acceptedExecution(controller.claim({
            outTrackId,
            actionId: 'submit',
            actorId: 'owner-1',
    ...
    it('submits the built-in cancel callback without form answers', async () => {
        ...
        const outTrackId = vi.mocked(client.createAndDeliver).mock.calls[0][0]
            .outTrackId;
        await acceptedExecution(controller.claim({
            outTrackId,
            actionId: 'request-1',
            actorId: 'owner-1',
            formData: {},
    ...
    it('keeps pending requests in different sessions independently claimable', async () => {
        ...
        const executeSecond = acceptedExecution(controller.claim({
    ...
    it('keeps the first card active when the same run requests another question', async () => {
        ...
        const { client, controller } = createHarness();
        vi.mocked(client.createAndDeliver).mockReturnValueOnce(firstDelivery.promise);
        const first = createContext('request-1');
        const second = createContext('request-2');
        ...
        const firstOutTrackId = vi.mocked(client.createAndDeliver).mock.calls[0][0]
            .outTrackId;
        await expect(controller.present(second.context, {
            chatId: 'cid-1',
            isGroup: true,
        })).resolves.toEqual({ kind: 'unsupported' });
        expect(client.createAndDeliver).toHaveBeenCalledOnce();
        expect(client.updateInstance).not.toHaveBeenCalledWith(expect.objectContaining({ outTrackId: firstOutTrackId }));
        expect(first.respond).not.toHaveBeenCalled();
        expect(second.respond).not.toHaveBeenCalled();
    ...
    it('does not roll back an accepted answer when terminal projection fails', async () => {
        ...
        const outTrackId = vi.mocked(client.createAndDeliver).mock.calls[0][0]
            .outTrackId;
        vi.mocked(client.updateInstance).mockRejectedValueOnce(new Error('projection failed'));
        await expect(acceptedExecution(controller.claim({
            outTrackId,
            actionId: 'submit',
            actorId: 'owner-1',
            formData: { '0': 'Beijing', '1': ['Logs'] },
