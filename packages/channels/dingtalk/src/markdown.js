...
export function splitChunks(text) {
    ...
    const appendLine = (line, needsLineBreak, closesCodeFence, leavesCodeFenceOpen) => {
        let remaining = line;
        ...
        while (remaining.length > 0 || prefixPending) {
            ...
            const fitsAsFinalPiece = remaining.length <= CHUNK_LIMIT - buf.length - prefix.length;
            const closeFenceOverhead = (inCode && !(closesCodeFence && fitsAsFinalPiece)) ||
                (!inCode && leavesCodeFenceOpen)
                ? '\n```'.length
                : 0;
            const available = CHUNK_LIMIT - closeFenceOverhead - buf.length - prefix.length;
            if (available <= 0) {
            ...
            if (pieceLength < remaining.length) {
                for (let fenceStart = Math.max(0, pieceLength - 2); fenceStart < pieceLength; fenceStart++) {
                    if (remaining.slice(fenceStart, fenceStart + 3) === '```' &&
...
export function normalizeDingTalkMarkdown(text) {
