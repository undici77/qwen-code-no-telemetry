...
function createManager(initialClient, overrides = {}) {
...
describe('DingtalkConnectionManager', () => {
    ...
    it('disconnects an initial client that opens after stop', async () => {
        const pendingConnect = deferredPromise();
        const initialClient = new FakeClient();
        let open = false;
    ...
    it('times out the complete connection attempt', async () => {
        ...
        await vi.advanceTimersByTimeAsync(10_000);
        expect(startupError).toEqual(new Error('Timed out connecting to DingTalk Stream.'));
        pendingConnect.resolve();
        await vi.advanceTimersByTimeAsync(0);
    });
...
//# sourceMappingURL=DingtalkConnectionManager.test.js.map