...
export class DingtalkChannel extends ChannelBase {
    ...
    if (this.interactiveCardConfig.enabled) {
      ...
      if (this.statusCardController || this.questionCardController) {
        this.interactionPresenter = new DingtalkInteractionPresenter({
          ...
          ...(config.blockStreaming !== 'on'
            ? {
                sendFallback: (chatId, text, sessionId) =>
  ...
  onDownStream(raw, client) {
    ...
    try {
      ...
      process.stderr.write(
        `[DingTalk:${this.name}] Failed to parse downstream: ${sanitizeLogText(String(err), 200)}\n`,
      );
      return;
    }
    const headers =
    ...
    process.stderr.write(
      `[DingTalk:${this.name}] downstream type=${sanitizeLogText(type, 40)} topic=${sanitizeLogText(topic, 80)} messageId=${sanitizeLogText(messageId, 80)} bytes=${decoded.bytes}\n`,
    );
    ...
    switch (type) {
      ...
      default:
        process.stderr.write(
          `[DingTalk:${this.name}] Ignoring downstream type ${sanitizeLogText(type || 'unknown', 40)}.\n`,
        );
    }
  }
  callDownStreamHandler(client, method, msg) {
    try {
      ...
      process.stderr.write(
        `[DingTalk:${this.name}] ${method} failed: ${sanitizeLogText(String(err), 200)}\n`,
      );
    }
  }
  ...
  async prepareOutgoingText(text) {
    ...
    for (const marker of markers) {
      ...
      try {
        ...
        process.stderr.write(
          `[DingTalk:${this.name}] outbound image upload failed (${sanitizeLogText(fileName, 100)}): ${sanitizeLogText(error instanceof Error ? error.message : String(error), 300)}\n`,
        );
  ...
  onMessage(downstream) {
    try {
      ...
      if (DingtalkChannel.isUnroutableGroupMessage(isGroup, conversationId)) {
        ...
        process.stderr.write(
          `[DingTalk:${this.name}] Group message has no conversationId, skipping (msgId=${msgId || 'unknown'}, sender=${sanitizeSenderName(senderNick || senderStaffId || 'unknown')})\n`,
        );
        return;
      }
      ...
      process.stderr.write(
        `[DingTalk:${this.name}] message msgId=${sanitizeLogText(msgId || 'unknown', 80)} conversationId=${sanitizeLogText(conversationId || '', 120)} isGroup=${isGroup} isMentioned=${isMentioned} senderNick=${sanitizeLogText(senderNick || '', 80)} senderStaffId=${sanitizeLogText(senderStaffId || '', 80)} senderId=${sanitizeLogText(senderIdValue || '', 80)}\n`,
      );
