...
function segment(segmentId = 'segment-1', overrides = {}) {
    return {
...
describe('StatusCardController', () => {
    ...
    it('creates and opens a status card on the first content snapshot', async () => {
        ...
        await vi.waitFor(() => expect(client.openOrUpdateStream).toHaveBeenCalledWith(expect.objectContaining({
            content: 'first',
            finalize: false,
        })));
    });
    it('includes replacement content in the initial card delivery', async () => {
        ...
        await vi.waitFor(() => expect(client.createAndDeliver).toHaveBeenCalledWith(expect.objectContaining({
            cardParamMap: expect.objectContaining({
                content: '@Alice',
            }),
        })));
        expect(client.openOrUpdateStream).toHaveBeenCalledWith(expect.objectContaining({
            content: '@Alice',
            finalize: false,
        }));
    });
    it('coalesces bounded full snapshots with one write in flight', async () => {
        ...
        controller.replace(segment(), target, 'a'.repeat(19_000) + 'b'.repeat(2_000));
    ...
    it('hides streamed image paths in status snapshots', async () => {
        vi.useFakeTimers();
        const { client, controller } = createHarness();
        controller.replace(segment(), target, 'before [IMAGE: /Users/ben/private/image.png] after');
    ...
    it('hides image paths when a streaming card is cancelled', async () => {
        ...
        const terminalPayload = JSON.stringify(vi.mocked(client.updateInstance).mock.calls.at(-1)?.[0].cardParamMap);
    ...
    it('keeps content streaming after a metadata update fails', async () => {
        ...
        controller.replace(segment(), target, 'first');
        await vi.advanceTimersByTimeAsync(0);
        vi.mocked(client.updateInstance).mockRejectedValueOnce(new Error('metadata failed'));
        vi.setSystemTime(1_200);
        await vi.advanceTimersByTimeAsync(1_000);
        expect(onError).toHaveBeenCalledWith('status card metadata', expect.any(Error));
        vi.mocked(client.openOrUpdateStream).mockClear();
    ...
    it('stops status refreshes after repeated metadata failures', async () => {
        ...
        controller.replace(segment(), target, 'first');
        await vi.advanceTimersByTimeAsync(0);
        vi.mocked(client.updateInstance).mockRejectedValue(new Error('metadata down'));
    ...
    it('keeps two segments from the same run independent', async () => {
        ...
        await vi.waitFor(() => expect(client.createAndDeliver).toHaveBeenCalledTimes(2));
        ...
        await expect(controller.complete('segment-1', 'first answer')).resolves.toBe(true);
        await expect(controller.complete('segment-2', 'second answer')).resolves.toBe(true);
        expect(client.updateInstance).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('cancels every live segment from the exact run only', async () => {
        ...
        controller.replace(segment('other-segment', {
            runId: 'run-2',
            sessionId: 'session-2',
        }), target, 'other');
        await vi.waitFor(() => expect(client.createAndDeliver).toHaveBeenCalledTimes(3));
        const [firstOutTrackId, secondOutTrackId, otherOutTrackId] = vi
            .mocked(client.createAndDeliver)
            .mock.calls.map(([request]) => request.outTrackId);
        controller.cancelRun('run-1', 'cancel_command');
        await vi.waitFor(() => expect(client.updateInstance).toHaveBeenCalledTimes(2));
        expect(vi
            .mocked(client.updateInstance)
            .mock.calls.map(([request]) => request.outTrackId)).toEqual(expect.arrayContaining([firstOutTrackId, secondOutTrackId]));
        expect(client.updateInstance).not.toHaveBeenCalledWith(expect.objectContaining({ outTrackId: otherOutTrackId }));
        await expect(controller.complete('other-segment', 'other')).resolves.toBe(true);
    });
    it('commits final content through V2 instance fields and rejects late snapshots', async () => {
        const { client, controller } = createHarness();
        controller.replace(segment(), target, 'answer');
        await expect(controller.complete('segment-1', 'answer')).resolves.toBe(true);
        expect(client.openOrUpdateStream).toHaveBeenLastCalledWith(expect.objectContaining({
            content: '',
            finalize: true,
        }));
        expect(client.updateInstance).toHaveBeenLastCalledWith(expect.objectContaining({
    ...
    it('allows only the owner to stop the exact current run', async () => {
        ...
        const execute = acceptedExecution(controller.claimStop(outTrackId, 'owner-1'));
    ...
    it('drains a snapshot queued while the boundary drain write is in flight', async () => {
        ...
        expect(vi
            .mocked(client.openOrUpdateStream)
            .mock.calls.map(([request]) => request.content)).toContain('first more second');
    });
    ...
    it('does not claim delivery for a card whose creation failed', async () => {
        const { client, controller } = createHarness();
        vi.mocked(client.createAndDeliver).mockRejectedValueOnce(new Error('status template unavailable'));
        controller.ensure(segment(), target);
    ...
    it('stops status refreshes after the content stream fails', async () => {
        ...
        gate.resolve();
        await vi.advanceTimersByTimeAsync(0);
        expect(onError).toHaveBeenCalledWith('status card streaming', expect.any(Error));
    ...
    it('does not retry the content stream after a latched failure', async () => {
        ...
        await vi.advanceTimersByTimeAsync(500);
        expect(client.openOrUpdateStream).toHaveBeenCalledTimes(2);
        vi.mocked(client.openOrUpdateStream).mockRejectedValueOnce(new Error('stream died'));
    ...
    it('resets the status failure breaker after a successful push', async () => {
        ...
        vi.mocked(client.updateInstance).mockRejectedValue(new Error('metadata blip'));
        vi.mocked(client.updateInstance).mockClear();
        await vi.advanceTimersByTimeAsync(3_000);
        expect(client.updateInstance).toHaveBeenCalledTimes(3);
    });
...
//# sourceMappingURL=status-card-controller.test.js.map