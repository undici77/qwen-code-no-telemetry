...
export function findImageMarkers(text) {
...
export function replaceImageMarkers(text, markers, replacements) {
...
export function stripPartialImageMarker(text) {
    const visibleText = maskCode(text);
...
export function sanitizeStreamingImageMarkers(text) {
...
export function readValidatedImage(imagePath, options) {
...
export async function uploadDingTalkImage(image, accessToken) {
    ...
    if (!response.ok || (errcode !== undefined && errcode !== 0)) {
        ...
        throw new DingTalkMediaUploadError(`DingTalk media upload failed: HTTP ${response.status}${errcode === undefined ? '' : ` errcode=${errcode}`}${detail ? ` ${detail}` : ''}`, response.status === 401 ||
            (errcode !== undefined && AUTH_ERROR_CODES.has(errcode)));
    }
    const mediaId = typeof payload['media_id'] === 'string'
        ? payload['media_id']
...
//# sourceMappingURL=outbound-image.js.map