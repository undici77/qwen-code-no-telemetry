...
export const STATUS_CARD_TEMPLATE_ID = '675cde2f-f526-40cb-b828-f5b2b57b8b77.schema';
export const QUESTION_CARD_TEMPLATE_ID = 'c2a6355b-9724-4f7e-9653-d33fcb3311bb.schema';
...
export class DingtalkInteractiveCardClient {
    ...
    async createAndDeliver(input) {
        ...
        if (Array.isArray(deliveries)) {
            const failure = deliveries.find((entry) => entry !== null &&
                typeof entry === 'object' &&
                entry.success === false);
            if (failure) {
                throw new Error(`${input.templateId}: ${typeof failure.errorMsg === 'string' && failure.errorMsg.trim()
    ...
    async request(path, method, body, templateId) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`DingTalk Card OpenAPI ${method} ${path} failed${templateId ? ` for ${templateId}` : ''}: HTTP ${response.status}${detail ? ` ${detail}` : ''}`);
        }
        return response.json().catch(() => undefined);
    }
}
//# sourceMappingURL=interactive-card-client.js.map