    ...
    execFile(
      ...
      (error, stdout, stderr) => {
        if (error) {
          ...
          reject(
            new Error(
              `[Channel:${channelName}] ${message}${stderrHint ? ` gh stderr: ${stderrHint}` : ''}`,
            ),
          );
          return;
        }
...
export class GithubChannel extends PollingChannelBase {
    ...
    const retry = this.retryPendingFinalDeliveries(retryAbort.signal)
      .catch((err) => {
        if (!retryAbort.signal.aborted) {
          process.stderr.write(
            `[Channel:${this.name}] pending GitHub delivery retry failed: ${sanitizeLogText(err instanceof Error ? err.message : String(err), 200)}\n`,
          );
        }
      })
      .finally(() => {
  ...
  async retryPendingFinalDeliveries(signal) {
    ...
    for (const record of pending) {
      ...
      if (this.hasPostedPublicationAudit(record.id)) {
        process.stderr.write(
          `[Channel:${this.name}] dropping pending GitHub delivery already recorded as posted: ${sanitizeLogText(record.id, 80)}\n`,
        );
        if (
  ...
  updatePendingFinalDeliveries(update) {
    try {
      ...
      process.stderr.write(
        `[Channel:${this.name}] failed to update pending GitHub deliveries: ${sanitizeLogText(err instanceof Error ? err.message : String(err), 200)}\n`,
      );
      return false;
    }
  }
  removeReplyPendingInboundTask(delivery) {
    try {
      ...
      process.stderr.write(
        `[Channel:${this.name}] failed to clean up inbound task: ${sanitizeLogText(cleanupErr instanceof Error ? cleanupErr.message : String(cleanupErr), 200)}\n`,
      );
    }
  }
  ...
  readPendingFinalDeliveries(strict = false) {
    try {
      ...
      if (err.code !== 'ENOENT') {
        process.stderr.write(
          `[Channel:${this.name}] failed to read pending GitHub deliveries: ${sanitizeLogText(err instanceof Error ? err.message : String(err), 200)}\n`,
        );
        if (strict) throw err;
      }
      return [];
  ...
  readInboundTasks() {
    try {
      ...
      this.inboundPersistenceBlocked = true;
      process.stderr.write(
        `[Channel:${this.name}] failed to read GitHub inbound tasks: ${sanitizeLogText(err instanceof Error ? err.message : String(err), 200)}\n`,
      );
      throw err;
    }
  }
  ...
  recordPublicationAudit(record) {
    try {
      ...
      process.stderr.write(
        `[Channel:${this.name}] publication audit write failed: ${sanitizeLogText(err instanceof Error ? err.message : String(err), 200)}\n`,
      );
    }
  }
  migrateLegacyPublicationState() {
    try {
      ...
      process.stderr.write(
        `[Channel:${this.name}] legacy GitHub state migration failed: ${sanitizeLogText(err instanceof Error ? err.message : String(err), 200)}\n`,
      );
    }
  }
