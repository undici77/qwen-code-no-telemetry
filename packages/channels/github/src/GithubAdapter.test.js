...
import { getWorkspaceScopeDirName, PairingStore, } from '@qwen-code/channel-base';
...
const mockOctokit = octokitModule.__mockOctokit;
...
function inboundTaskPath(cwd = '/tmp/test', channelName = 'test-github') {
    const nameHash = createHash('sha256')
        .update(channelName)
        .digest('hex')
        .slice(0, 16);
    return join(process.env.QWEN_HOME, 'channels', getWorkspaceScopeDirName(cwd), `${channelName}-${nameHash}-github-inbound-tasks.json`);
}
function makeInboundTaskRecord(overrides = {}) {
    const envelope = {
...
function writeInboundTasks(records) {
    const path = inboundTaskPath();
    mkdirSync(join(path, '..'), { recursive: true });
    writeFileSync(path, `${JSON.stringify(records)}\n`, 'utf-8');
}
function readInboundTasks() {
    return JSON.parse(readFileSync(inboundTaskPath(), 'utf-8'));
}
...
class TestableGithubChannel extends GithubChannel {
    inboundEnvelopes = [];
    handleInboundError = null;
    usePreflight = false;
    sourceMessageId;
    sourceSenderId;
    sourceMetadata;
    handleInboundHook;
    getResponseMessageId(_sessionId) {
        return this.sourceMessageId;
    }
    getResponseSenderId(_sessionId) {
        return this.sourceSenderId;
    }
    getResponseMetadata(_sessionId) {
        return this.sourceMetadata;
    }
    async handleInbound(envelope) {
        await this.handleInboundHook?.(envelope);
        if (this.handleInboundError)
            throw this.handleInboundError;
        if (this.usePreflight && !(await this.preflightInbound(envelope)))
            return;
        this.inboundEnvelopes.push(envelope);
    }
    triggerTaskLifecycleForTest(event) {
        this.onTaskLifecycle(event);
    }
    async testSendThreadMessage(chatId, threadId, text) {
        return this.sendThreadMessage(chatId, threadId, text);
    }
}
class LiveGithubChannel extends GithubChannel {
    setCursorForTest(lastProcessedAt) {
        this.cursor = { lastProcessedAt };
    }
    async pollForTest() {
        await this.pollOnce();
    }
    startPromptForTest(chatId, sessionId, messageId) {
        this.onPromptStart(chatId, sessionId, messageId);
    }
    endPromptForTest(chatId, sessionId, messageId) {
...
describe('GithubChannel', () => {
    ...
    beforeEach(() => {
        ...
        vi.clearAllMocks();
        channel = new TestableGithubChannel('test-github', makeConfig(), makeBridge());
    ...
    async function initWithoutLoop(configOverrides) {
        if (configOverrides) {
            channel = new TestableGithubChannel('test-github', makeConfig(configOverrides), makeBridge());
        }
    ...
    describe('connect', () => {
        it('resolves bot username', async () => {
            ...
            try {
                ...
                expect(stderr).toHaveBeenCalledWith('[Channel:test-github] using configured token\n');
        ...
        it('sanitizes the authenticated login in the stderr audit line', async () => {
            ...
            try {
                await channel.connect();
                expect(stderr).toHaveBeenCalledWith('[Channel:test-github] authenticated as "bot\\nforged-line"\n');
            }
            finally {
                channel.disconnect();
                stderr.mockRestore();
        ...
        it('requires explicit opt-in before using local gh authentication', async () => {
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '' }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('configure a GitHub token or enable local GitHub CLI authentication');
        ...
        it('rejects a quoted useLocalGh value from hand-edited settings', async () => {
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: 'true' }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('[Channel:test-github] useLocalGh must be a boolean.');
            expect(mockExecFile).not.toHaveBeenCalled();
        });
        it('rejects a non-boolean useLocalGh even when a token is configured', async () => {
            channel = new TestableGithubChannel('test-github', makeConfig({ token: 'test-token', useLocalGh: 'true' }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('useLocalGh must be a boolean');
            expect(mockOctokitConstructor).not.toHaveBeenCalled();
        });
        it('falls back to local gh for a whitespace-only token', async () => {
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(null, 'local-gh-token\n'));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: ' ', useLocalGh: true }), makeBridge());
            mockOctokit.paginate.mockResolvedValue([]);
            await channel.connect();
            expect(mockExecFile).toHaveBeenCalled();
            expect(mockOctokitConstructor).toHaveBeenCalledWith(expect.objectContaining({ auth: 'local-gh-token' }));
            channel.disconnect();
        });
        it('uses local gh authentication when explicitly enabled', async () => {
            ...
            try {
                await channel.connect();
                expect(mockExecFile).toHaveBeenCalledWith('gh', ['auth', 'token', '--hostname', 'github.com'], expect.objectContaining({
                    encoding: 'utf8',
                    maxBuffer: 64 * 1024,
                    timeout: 10_000,
                    windowsHide: true,
                ...
                expect(mockOctokitConstructor).toHaveBeenCalledWith(expect.objectContaining({ auth: 'local-gh-token' }));
                expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining('local-gh-token'));
            }
            finally {
                channel.disconnect();
                stderr.mockRestore();
        ...
        it('uses the enterprise host for local gh authentication', async () => {
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(null, 'enterprise-token\n'));
            channel = new TestableGithubChannel('test-github', makeConfig({
                token: '',
                useLocalGh: true,
            ...
            try {
                await channel.connect();
                expect(mockExecFile).toHaveBeenCalledWith('gh', ['auth', 'token', '--hostname', 'ghe.example.com'], expect.any(Object), expect.any(Function));
                expect(stderr).toHaveBeenCalledWith('[Channel:test-github] using local gh credential for ghe.example.com\n');
                expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining('enterprise-token'));
            }
            finally {
                channel.disconnect();
                stderr.mockRestore();
        ...
        it('rejects an insecure API URL before resolving local gh credentials', async () => {
            ...
            await expect(channel.connect()).rejects.toThrow('local GitHub CLI authentication requires an HTTPS baseUrl');
            expect(mockExecFile).not.toHaveBeenCalled();
            expect(mockOctokitConstructor).not.toHaveBeenCalled();
        });
        it('reports a malformed baseUrl before resolving local gh credentials', async () => {
            ...
            await expect(channel.connect()).rejects.toThrow('[Channel:test-github] baseUrl is not a valid URL: ghe.example.com/api/v3');
            expect(mockExecFile).not.toHaveBeenCalled();
            expect(mockOctokitConstructor).not.toHaveBeenCalled();
        });
        it('reports a scheme-less baseUrl with a port as malformed', async () => {
            ...
            await expect(channel.connect()).rejects.toThrow('[Channel:test-github] baseUrl is not a valid URL: ghe.example.com:8443/api/v3');
            expect(mockExecFile).not.toHaveBeenCalled();
            expect(mockOctokitConstructor).not.toHaveBeenCalled();
        });
        it('rejects a baseUrl hostname that begins with a dash before spawning gh', async () => {
            ...
            await expect(channel.connect()).rejects.toThrow('[Channel:test-github] baseUrl hostname is invalid: --evil');
            expect(mockExecFile).not.toHaveBeenCalled();
            expect(mockOctokitConstructor).not.toHaveBeenCalled();
        });
        it('rejects a baseUrl hostname outside the gh hostname allowlist', async () => {
            ...
            await expect(channel.connect()).rejects.toThrow('[Channel:test-github] baseUrl hostname is invalid: ghe.example_company.com');
            expect(mockExecFile).not.toHaveBeenCalled();
            expect(mockOctokitConstructor).not.toHaveBeenCalled();
        });
        it('preserves explicit token support for an HTTP base URL', async () => {
            ...
            await channel.connect();
            expect(mockExecFile).not.toHaveBeenCalled();
            expect(mockOctokitConstructor).toHaveBeenCalledWith(expect.objectContaining({
                auth: 'test-token',
                baseUrl: 'http://ghe.example.com/api/v3',
            }));
            channel.disconnect();
        ...
        it('reports an empty token returned by GitHub CLI', async () => {
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(null, ' \n'));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: true }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('GitHub CLI returned an empty token for github.com');
        });
        ...
        it('reports when GitHub CLI is unavailable', async () => {
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(Object.assign(new Error('secret missing-cli failure'), {
                code: 'ENOENT',
            }), ''));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: true }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('GitHub CLI (gh) is not installed on the daemon host');
            await expect(channel.connect()).rejects.not.toThrow('secret missing-cli failure');
        });
        it('reports when the selected gh host is not authenticated', async () => {
            ...
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(Object.assign(new Error('secret stderr'), { code: 1 }), ''));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: true }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('gh auth login --hostname github.com');
            await expect(channel.connect()).rejects.toThrow('gh config dir: /home/test-user/.config/gh');
        ...
        it('names the gh config dir when the host is not authenticated', async () => {
            vi.stubEnv('GH_CONFIG_DIR', '/tmp/test-gh-config');
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(Object.assign(new Error('exit 1'), { code: 1 }), ''));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: true }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('gh config dir: /tmp/test-gh-config');
        });
        it('prefers XDG_CONFIG_HOME over HOME in the gh config dir hint', async () => {
            ...
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(Object.assign(new Error('exit 1'), { code: 1 }), ''));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: true }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('gh config dir: /tmp/test-xdg-config/gh');
        });
        ...
        it('prefers the Windows AppData gh config dir on win32', async () => {
            ...
            try {
                await expect(channel.connect()).rejects.toThrow('gh config dir: C:\\Users\\test\\AppData\\Roaming\\GitHub CLI');
            }
            finally {
        ...
        it('surfaces bounded gh stderr in the authentication failure', async () => {
            ...
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(Object.assign(new Error('exit 1'), { code: 1 }), '', rawStderr));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: true }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('gh auth login --hostname github.com');
        ...
        it('reports when the GitHub CLI authentication lookup times out', async () => {
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(Object.assign(new Error('secret timeout failure'), {
                killed: true,
            }), ''));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: true }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('authentication lookup for github.com timed out after 10 seconds');
            await expect(channel.connect()).rejects.not.toThrow('secret timeout failure');
        });
        it('treats a killed lookup that also exited as a timeout', async () => {
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(Object.assign(new Error('exit 1'), { code: 1, killed: true }), ''));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: true }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('authentication lookup for github.com timed out after 10 seconds');
        });
        it('reports when GitHub CLI authentication cannot execute', async () => {
            mockExecFile.mockImplementation((_file, _args, _options, callback) => callback(Object.assign(new Error('secret failure'), { code: 'EACCES' }), ''));
            channel = new TestableGithubChannel('test-github', makeConfig({ token: '', useLocalGh: true }), makeBridge());
            await expect(channel.connect()).rejects.toThrow('authentication lookup for github.com failed to execute');
            await expect(channel.connect()).rejects.not.toThrow('secret failure');
        });
        it('throws when bot identity fails', async () => {
            mockOctokit.rest.users.getAuthenticated.mockRejectedValue(new Error('bad token'));
            await expect(channel.connect()).rejects.toThrow('failed to resolve bot identity');
        });
        ...
        it('rejects an allowlist containing only the authenticated GitHub account', async () => {
            ...
            mockOctokit.paginate.mockResolvedValue([]);
            try {
                await expect(channel.connect()).rejects.toThrow('allowlist only contains the authenticated GitHub account "test-bot"');
            }
            finally {
                channel.disconnect();
            }
        ...
        it('warns when the authenticated GitHub account is part of a mixed allowlist', async () => {
            ...
            try {
                await channel.connect();
                expect(stderr).toHaveBeenCalledWith('[Channel:test-github] warning: authenticated GitHub account "test-bot" is allowlisted but cannot trigger this channel; use a separate operator account.\n');
            }
            finally {
                channel.disconnect();
                stderr.mockRestore();
    ...
    describe('poll and process', () => {
        ...
        it('skips non-issue/PR notifications', async () => {
            ...
            await pollOnce();
            expect(channel.inboundEnvelopes).toHaveLength(0);
            expect(mockOctokit.rest.activity.markNotificationsAsRead).toHaveBeenCalledWith(expect.objectContaining({ read: true }));
        });
        ...
        it('marks notifications as read after accepted work completes', async () => {
            ...
            mockOctokit.paginate
                ...
                .mockResolvedValueOnce([makeComment()]);
            let releaseInbound;
            channel.handleInboundHook = () => new Promise((resolve) => {
                releaseInbound = resolve;
            });
            const poll = pollOnce();
            await vi.waitFor(() => expect(mockOctokit.paginate).toHaveBeenCalledTimes(2));
            expect(mockOctokit.rest.activity.markNotificationsAsRead).not.toHaveBeenCalled();
            releaseInbound();
            await poll;
            ...
            const markOrder = mockOctokit.rest.activity.markNotificationsAsRead.mock.invocationCallOrder.at(-1);
            const commentOrder = mockOctokit.paginate.mock.invocationCallOrder.at(-1);
        ...
        it('aborts the poll cycle without advancing cursor when markNotificationsAsRead fails', async () => {
            ...
            mockOctokit.rest.activity.markNotificationsAsRead.mockRejectedValue(new Error('server error'));
        ...
        it('uses cursor as enumeration window lower bound', async () => {
            ...
            expect(mockOctokit.paginate).toHaveBeenNthCalledWith(2, expect.anything(), expect.objectContaining({ since: '2026-07-01T00:00:00.000Z' }));
        });
    ...
    describe('reason routing', () => {
        it('dispatches review_requested from PR metadata', async () => {
            ...
            expect(channel.inboundEnvelopes[0].metadata).toContain('Branch: divide → main');
            expect(channel.inboundEnvelopes[0].metadata).toContain('Title: feat: divide');
            expect(channel.inboundEnvelopes[0].metadata).toContain('Author: alice | State: open | Draft: false');
            expect(channel.inboundEnvelopes[0].text).toBe('Return a formal review summary with verified actionable findings, or a concise no-blocker result.');
            expect(channel.inboundEnvelopes[0].displayText).toBe('Review requested: feat: divide');
            expect(channel.inboundEnvelopes[0].metadata).toContain('For review_requested, return a formal review summary');
        });
        it('dispatches late direct events once without muting newer events', async () => {
            ...
            const reviewNotification = (updated_at) => makeNotification({
                reason: 'review_requested',
                updated_at,
            ...
            await pollOnce();
            await pollOnce();
            expect(channel.inboundEnvelopes.map((envelope) => envelope.messageId)).toEqual(['event-2001', 'event-2002']);
        ...
        it('dispatches assign from issue metadata', async () => {
            ...
            expect(channel.inboundEnvelopes[0].metadata).toContain('Title: broken build');
            expect(channel.inboundEnvelopes[0].metadata).toContain('Author: alice | State: open');
        });
        it.each(['author', 'comment'])('aggregates new comments for %s notifications', async (reason) => {
            await initWithoutLoop();
            channel.usePreflight = true;
            mockOctokit.paginate
                .mockResolvedValueOnce([
            ...
            expect(channel.inboundEnvelopes[0].text).toContain('output exactly <no-reply/> if no public reply is needed');
            expect(channel.inboundEnvelopes[0].text).toContain('@alice: first');
            expect(channel.inboundEnvelopes[0].text).toContain('@bob: second');
            expect(channel.inboundEnvelopes[0].displayText).toBe('- @alice: first\n- @bob: second');
        });
        ...
        it('dispatches directed follow-ups from an approved paired repo on the aggregate lane', async () => {
            ...
            const created = store.createGroupRequest('owner/repo', 'owner/repo', 'alice', 'Alice');
            if (!('code' in created)) {
        ...
        it('does not turn ambient comments into pairing requests under senderPolicy open', async () => {
            ...
            expect(channel.inboundEnvelopes).toHaveLength(0);
            expect(mockOctokit.rest.issues.createComment).not.toHaveBeenCalled();
            expect(new PairingStore('test-github', '/tmp/test').listPending()).toEqual([]);
        });
        it('posts one pairing comment when assign and body mention both trigger pairing', async () => {
            ...
            expect(new PairingStore('test-github', '/tmp/test').listPending()).toHaveLength(1);
        });
        ...
        it('sanitizes crafted comment bodies in the aggregate display projection', async () => {
            ...
            expect(channel.inboundEnvelopes[0].text).toContain(displayText.slice('- @alice: '.length));
        });
        ...
        it('records aggregated comments that exceed the summary cap', async () => {
            await initWithoutLoop();
            const comments = Array.from({ length: 25 }, (_, index) => makeComment({
                id: 1001 + index,
            ...
            expect(channel.inboundEnvelopes[0].text).not.toContain('- @alice: comment 1\n');
            expect(channel.inboundEnvelopes[0].text).toContain('comment 25');
        });
        it('records aggregated comments before dispatching them', async () => {
            vi.spyOn(channel, 'handleInbound').mockRejectedValueOnce(new Error('agent down'));
            await initWithoutLoop();
            mockOctokit.paginate
                .mockResolvedValueOnce([
                makeNotification({
        ...
        it('uses the generic fallback for other reasons', async () => {
            ...
            expect(channel.inboundEnvelopes[0].metadata).toContain('Trigger: subscribed.');
            expect(channel.inboundEnvelopes[0].metadata).toContain('GitHub publication policy:');
        });
    ...
    describe('reasonFilter', () => {
        function connectWithReasonFilter(reasonFilter) {
            channel = new TestableGithubChannel('test-github', makeConfig({ reasonFilter }), makeBridge());
        ...
        it('skips notifications whose reason is not in the allowlist', async () => {
            ...
            try {
                ...
                await pollOnce();
                expect(channel.inboundEnvelopes).toHaveLength(1);
                expect(channel.inboundEnvelopes[0].metadata).toContain('Trigger: mention.');
                expect(mockOctokit.rest.activity.markNotificationsAsRead).toHaveBeenCalledWith({
                    last_read_at: '2026-07-02T10:00:00.000Z',
                    read: true,
                });
                expect(stderrWrite).toHaveBeenCalledWith(expect.stringContaining('skipping notification (reason=comment not in reasonFilter, subject=https://api.github.com/repos/owner/repo/issues/42)'));
            }
            finally {
        ...
        it('rejects unrecognized reasonFilter values', async () => {
            await expect(connectWithReasonFilter(['mentions'])).rejects.toThrow('Unrecognized reasonFilter values for channel test-github: mentions');
        });
        it('rejects non-array reasonFilter values', async () => {
            await expect(connectWithReasonFilter('mention')).rejects.toThrow('reasonFilter for channel test-github must be an array of GitHub notification reasons.');
        });
        it('rejects non-string reasonFilter entries', async () => {
            await expect(connectWithReasonFilter([42])).rejects.toThrow('reasonFilter entries for channel test-github must be strings.');
        });
        it('accepts documented security notification reasons', async () => {
            await expect(connectWithReasonFilter(['security_alert'])).resolves.toBeUndefined();
            channel.disconnect();
        });
    ...
    describe('publication contract', () => {
        const nameHash = createHash('sha256')
            ...
            .digest('hex')
            .slice(0, 16);
        function pendingPath(cwd = '/tmp/test') {
            return join(process.env.QWEN_HOME, 'channels', getWorkspaceScopeDirName(cwd), `test-github-${nameHash}-github-pending-deliveries.json`);
        }
        function auditPath() {
            return join(process.env.QWEN_HOME, 'channels', getWorkspaceScopeDirName('/tmp/test'), `test-github-${nameHash}-github-audit.jsonl`);
        }
        function stateDir() {
            return join(process.env.QWEN_HOME, 'channels', getWorkspaceScopeDirName('/tmp/test'));
        }
        ...
        async function retryPendingForTest() {
            const privateChannel = channel;
            privateChannel.octokit = mockOctokit;
        ...
        it('suppresses the exact no-reply sentinel and audits the outcome', async () => {
            ...
            const publish = channel.publishFinalResponse.bind(channel);
            await publish('owner/repo', 'issue:42', ' \n<no-reply/>\t', 'session-publication');
            expect(mockOctokit.rest.issues.createComment).not.toHaveBeenCalled();
            const audit = readFileSync(auditPath(), 'utf-8');
            expect(audit).toContain('"outcome":"suppressed"');
            expect(audit).not.toContain('<no-reply/>');
        });
        it.each(['<NO-REPLY/>', '<no-reply />', '```text\n<no-reply/>\n```'])('suppresses no-reply sentinel variant %s', async (response) => {
            await connectForPublication();
            const publish = channel.publishFinalResponse.bind(channel);
        ...
        it('posts one final comment and audits only its digest and metadata', async () => {
            ...
            expect(audit).toContain(createHash('sha256').update(response).digest('hex'));
        ...
        it('uses the active prompt thread for final delivery', async () => {
            ...
            const sendResponse = channel.sendResponseMessage.bind(channel);
            vi.spyOn(channel, 'getResponseThreadId').mockReturnValue('pr:99');
        ...
        it('does not retry an ambiguous failed final delivery', async () => {
            ...
            const publish = channel.publishFinalResponse.bind(channel);
            await expect(publish('owner/repo', 'issue:42', 'Final reply', 'session-publication')).rejects.toMatchObject({
        ...
        ])('retries final delivery when GitHub definitely did not write', async (error) => {
            await connectForPublication();
            const sleep = vi.fn().mockResolvedValue(undefined);
            channel.abortableSleep = sleep;
            mockOctokit.rest.issues.createComment
                .mockRejectedValueOnce(error)
                .mockResolvedValueOnce({ data: {} });
            const publish = channel.publishFinalResponse.bind(channel);
            await publish('owner/repo', 'issue:42', 'Final reply', 'session-publication');
        ...
        it('deduplicates repeated pending final deliveries', async () => {
            ...
            mockOctokit.rest.issues.createComment.mockRejectedValue(error);
            channel.abortableSleep = vi.fn().mockResolvedValue(undefined);
            channel.sourceMessageId = 'source-message';
            const publish = channel.publishFinalResponse.bind(channel);
            await expect(publish('owner/repo', 'issue:42', 'Final reply', 'session-publication')).rejects.toThrow('rate limited');
            await expect(publish('owner/repo', 'issue:42', 'Final reply', 'session-publication')).rejects.toThrow('rate limited');
        ...
        it('does not collapse same-body pending finals without a source message id', async () => {
            ...
            await expect(publish('owner/repo', 'issue:42', 'Final reply', 'session-publication')).rejects.toThrow('rate limited');
            await expect(publish('owner/repo', 'issue:42', 'Final reply', 'session-publication')).rejects.toThrow('rate limited');
            const records = JSON.parse(readFileSync(pendingPath(), 'utf-8'));
        ...
        it('preserves concurrent delivery while recovering a pending final', async () => {
            ...
            mockOctokit.rest.issues.createComment.mockRejectedValue(error);
            const publish = channel.publishFinalResponse.bind(channel);
            await expect(publish('owner/repo', 'issue:42', 'Final reply', 'session-publication')).rejects.toMatchObject({ message: 'rate limited', cause: error });
            ...
            let resolveRetry;
            mockOctokit.rest.issues.createComment.mockReturnValue(new Promise((resolve) => {
        ...
        it('keeps a pending final when retry still definitely did not write', async () => {
            ...
            mockOctokit.rest.issues.createComment.mockRejectedValue(Object.assign(new Error('rate limited'), {
                status: 429,
                response: { headers: { 'x-ratelimit-remaining': '0' } },
            }));
        ...
        it('drops and audits an ambiguous pending final retry failure', async () => {
            ...
            mockOctokit.rest.issues.createComment.mockRejectedValue(new Error('ambiguous'));
            await retryPendingForTest();
            expect(mockOctokit.rest.issues.createComment).toHaveBeenCalledTimes(1);
            expect(existsSync(pendingPath())).toBe(false);
        ...
        it('continues retrying pending finals when a per-record update fails', async () => {
            ...
            mockOctokit.rest.issues.createComment
                ...
                data: {
                    id: 2004,
                    html_url: 'https://github.com/owner/repo/issues/43#issuecomment-2004',
                },
            });
            try {
                await retryPendingForTest();
                expect(stderr).toHaveBeenCalledWith(expect.stringContaining('failed to update pending GitHub deliveries'));
            }
            finally {
                stderr.mockRestore();
            }
        ...
        it('does not replay an in-flight pending final on reconnect', async () => {
            ...
            const pendingRetry = channel.pendingFinalDeliveryRetry;
            await pendingRetry;
            expect(mockOctokit.rest.issues.createComment).toHaveBeenCalledTimes(1);
            expect(existsSync(pendingPath())).toBe(false);
            channel.disconnect();
        ...
        it('does not burn retry budget when reconnect aborts cooldown', async () => {
            ...
            await channel.connect();
            expect(mockOctokit.rest.issues.createComment).toHaveBeenCalledTimes(1);
            const previousRetry = channel.pendingFinalDeliveryRetry;
            channel.disconnect();
            await previousRetry;
            await channel.connect();
            expect(mockOctokit.rest.issues.createComment).toHaveBeenCalledTimes(2);
        ...
        it('keeps bare 429 publication failures single-shot', async () => {
            ...
            mockOctokit.rest.issues.createComment.mockRejectedValue(error);
            const publish = channel.publishFinalResponse.bind(channel);
            await expect(publish('owner/repo', 'issue:42', 'Final reply', 'session-publication')).rejects.toMatchObject({ cause: error });
            expect(mockOctokit.rest.issues.createComment).toHaveBeenCalledTimes(1);
            expect(existsSync(pendingPath())).toBe(false);
        });
        it('migrates legacy pending finals before retrying', async () => {
            const legacyPath = join(process.env.QWEN_HOME, 'channels', 'test-github-github-pending-deliveries.json');
            const legacyAuditPath = join(process.env.QWEN_HOME, 'channels', 'test-github-github-audit.jsonl');
            ...
            expect(readFileSync(auditPath(), 'utf-8')).toContain('{"outcome":"posted"}\n');
        ...
        it('isolates pending finals by workspace', () => {
            const other = new TestableGithubChannel('test-github', makeConfig({ cwd: '/tmp/other-workspace' }), makeBridge());
            const otherPath = other.pendingFinalDeliveriesPath();
        ...
        it('skips a pending final already recorded as posted', async () => {
            writePending([pendingRecord()]);
            mkdirSync(join(auditPath(), '..'), { recursive: true });
            writeFileSync(auditPath(), `{"partial"\n${JSON.stringify({
                at: '2026-07-30T00:01:00.000Z',
                type: 'github_publication',
                outcome: 'posted',
                channel: 'test-github',
        ...
        it('distinguishes pre-delivery validation from ambiguous failures', async () => {
            await connectForPublication();
            mockOctokit.rest.issues.createComment.mockRejectedValue(new Error('ambiguous transport failure'));
            channel.abortableSleep = vi.fn().mockResolvedValue(undefined);
            const publish = channel.publishFinalResponse.bind(channel);
            vi.spyOn(channel, 'handleInbound').mockImplementation(async () => {
                await publish('owner/repo', 'issue:42', 'Final reply', 'session-publication');
            });
            const handled = await channel.dispatchEnvelope({
                channelName: 'test-github',
                senderId: 'alice',
                senderName: 'alice',
                chatId: 'owner/repo',
            ...
            let validationError;
            try {
                await publish('owner/repo', undefined, 'Final reply', 'session-publication');
            }
            catch (error) {
                validationError = error;
            }
            expect(validationError).toBeInstanceOf(Error);
            expect(validationError.constructor).toBe(Error);
        });
        ...
        it('keeps successful publication when its audit write fails', async () => {
            ...
            await expect(publish('owner/repo', 'issue:42', 'Final reply', 'session-publication')).resolves.toBeUndefined();
            expect(mockOctokit.rest.issues.createComment).toHaveBeenCalledTimes(1);
        });
    });
    describe('sendThreadMessage', () => {
        it('throws on invalid threadId format', async () => {
            await expect(channel.testSendThreadMessage('owner/repo', 'discussion:42', 'text')).rejects.toThrow('invalid threadId format');
        });
    });
    describe('first contact (new issue body)', () => {
        it.each(['subscribed', 'mention'])('feeds a mentioned issue body for %s notifications', async (reason) => {
            await initWithoutLoop();
            mockOctokit.paginate
                .mockResolvedValueOnce([
        ...
        it('evicts oldest dispatchedBodies entries beyond the limit', async () => {
            ...
            channel.cursor.dispatchedBodies = Array.from({ length: 500 }, (_, i) => `owner/repo|issue:${i}`);
            mockOctokit.rest.issues.get.mockResolvedValue({
                data: {
            ...
            expect(channel.cursor.dispatchedBodies).not.toContain('owner/repo|issue:0');
        ...
        it('does not suppress first-contact body when mention is from a disallowed sender', async () => {
            channel = new TestableGithubChannel('test-github', makeConfig({ senderPolicy: 'allowlist', allowedUsers: ['bob'] }), makeBridge());
            mockOctokit.paginate.mockResolvedValueOnce([]);
            await channel.connect();
            channel.disconnect();
            channel.cursor = { lastProcessedAt: '2026-07-01T00:00:00.000Z' };
            ...
            await pollOnce();
            const bodyEnvelope = channel.inboundEnvelopes.find((e) => e.messageId.startsWith('issue-body-'));
    ...
    describe('error handling', () => {
        ...
        it('bounds failed task recovery and avoids duplicate error comments', async () => {
            ...
            expect(mockOctokit.rest.issues.createComment.mock.calls.filter((call) => String(call[0]?.body).includes('Failed to process'))).toHaveLength(1);
            expect(channel.cursor.lastProcessedAt).toBe('2026-07-02T10:00:00.000Z');
            expect(mockOctokit.rest.activity.markNotificationsAsRead).toHaveBeenCalledWith({
                last_read_at: '2026-07-02T10:00:00.000Z',
                read: true,
        ...
        it('retries the error comment when the first post fails', async () => {
            ...
            mockOctokit.paginate
                .mockResolvedValueOnce([makeNotification()])
                .mockResolvedValueOnce([makeComment()]);
            mockOctokit.rest.issues.createComment.mockRejectedValue(new Error('comment creation outage'));
            await pollOnce();
            expect(readInboundTasks()).toEqual([
                expect.objectContaining({
                    state: 'failed',
            ...
            expect(mockOctokit.rest.issues.createComment.mock.calls.filter((call) => String(call[0]?.body).includes('Failed to process'))).toHaveLength(4);
        });
        it('reuses an existing inbound task record instead of creating a duplicate', async () => {
            ...
            expect(mockOctokit.rest.issues.createComment.mock.calls.filter((call) => String(call[0]?.body).includes('Failed to process'))).toHaveLength(0);
        });
        ...
        it('persists cancellation as a terminal task state', async () => {
            ...
            const privateChannel = channel;
            privateChannel.activeInboundTaskIdsByMessage.set('owner/repo|1001', 'inbound-task-1');
        ...
        it('does not turn cancellation into a retryable failure', async () => {
            ...
            expect(mockOctokit.rest.issues.createComment).not.toHaveBeenCalledWith(expect.objectContaining({ body: expect.stringContaining('Failed') }));
        });
        it('does not transition a cancelled task to reply_pending on FinalPublicationError', async () => {
            ...
            channel.handleInboundHook = async (envelope) => {
                ...
                await channel.publishFinalResponse(envelope.chatId, envelope.threadId, 'cancelled response', 'session-1');
            };
        ...
        it('continues polling after inbound recovery failure', async () => {
            await initWithoutLoop();
            writeInboundTasks([makeInboundTaskRecord({ state: 'running' })]);
            const pendingPath = inboundTaskPath().replace('github-inbound-tasks.json', 'github-pending-deliveries.json');
        ...
        it('does not re-run a task whose reply was posted before a crash', async () => {
            writeInboundTasks([makeInboundTaskRecord({ state: 'running' })]);
            const auditFilePath = inboundTaskPath().replace('github-inbound-tasks.json', 'github-audit.jsonl');
            mkdirSync(join(auditFilePath, '..'), { recursive: true });
            writeFileSync(auditFilePath, `${JSON.stringify({
        ...
        it('keeps the inbound envelope recoverable when pending delivery persistence fails', async () => {
            ...
            channel.sourceSenderId = 'alice';
            channel.sourceMetadata = 'Trigger: mention.';
            vi.spyOn(channel, 'postErrorComment').mockResolvedValue(true);
            privateChannel.abortableSleep = vi.fn().mockResolvedValue(undefined);
            channel.handleInboundHook = async (envelope) => {
                await channel.publishFinalResponse(envelope.chatId, envelope.threadId, 'completed response', 'session-1');
            };
            await privateChannel.runInboundTask(makeInboundTaskRecord({ state: 'running' }));
            const [persistedTask] = readInboundTasks();
            expect(persistedTask).toMatchObject({
                state: 'failed',
                envelope: { messageId: '1001' },
            });
            expect(String(persistedTask?.error)).toContain('failed to persist pending GitHub delivery');
        });
        ...
        it('does not rerun recovered work when publication audit is unreadable', async () => {
            writeInboundTasks([makeInboundTaskRecord({ state: 'running' })]);
            const auditPath = inboundTaskPath().replace('github-inbound-tasks.json', 'github-audit.jsonl');
            mkdirSync(auditPath, { recursive: true });
            const privateChannel = channel;
            privateChannel.octokit = mockOctokit;
            mockOctokit.paginate.mockResolvedValueOnce([]);
        ...
        it('removes a reply-pending task when a posted pending delivery is reconciled', async () => {
            ...
            writeFileSync(auditPath, `${JSON.stringify({ outcome: 'posted', pendingId: pending.id })}\n`, 'utf-8');
            const privateChannel = channel;
            privateChannel.octokit = mockOctokit;
            await privateChannel.retryPendingFinalDeliveries();
            expect(existsSync(pendingPath)).toBe(false);
        ...
        it('does not rerun a task whose final reply is already pending delivery', async () => {
            ...
            await privateChannel.retryPendingFinalDeliveries();
            expect(mockOctokit.rest.issues.createComment).toHaveBeenCalledWith(expect.objectContaining({ body: 'completed response' }));
            expect(existsSync(inboundTaskPath())).toBe(false);
        });
        it('posts only one error comment when dispatch fails on a new thread', async () => {
            ...
            await pollOnce();
            const errorComments = mockOctokit.rest.issues.createComment.mock.calls.filter((call) => call[0]?.body?.includes('Failed to process'));
        ...
        it('continues processing comments after one dispatch failure', async () => {
            ...
            vi.spyOn(channel, 'handleInbound').mockImplementation(async (envelope) => {
                if (envelope.messageId === '1002')
    ...
    describe('working reaction', () => {
        it('acknowledges an accepted comment with an eyes reaction', async () => {
            const liveChannel = new LiveGithubChannel('test-github', makeConfig(), makeBridge());
            mockOctokit.paginate.mockResolvedValueOnce([]);
        ...
        it('does not wait for the acknowledgment before replying', async () => {
            const { promise: reactionPending, resolve: resolveReaction } = Promise.withResolvers();
            mockOctokit.rest.reactions.createForIssueComment.mockReturnValue(reactionPending);
            const liveChannel = new LiveGithubChannel('test-github', makeConfig(), makeBridge());
            mockOctokit.paginate.mockResolvedValueOnce([]);
            await liveChannel.connect();
            liveChannel.disconnect();
            ...
            mockOctokit.paginate
                ...
                .mockResolvedValueOnce([makeComment()]);
            await liveChannel.pollForTest();
            expect(mockOctokit.rest.issues.createComment).toHaveBeenCalledWith(expect.objectContaining({ body: 'response' }));
        ...
        it('does not create a duplicate reaction while one is pending', async () => {
            ...
            expect(mockOctokit.rest.reactions.createForIssueComment).toHaveBeenCalledTimes(1);
            resolveReaction({ data: { id: 9000 } });
            await reactionPending;
        });
        it('removes the working reaction when the prompt finishes', async () => {
            ...
            expect(mockOctokit.rest.reactions.deleteForIssueComment).not.toHaveBeenCalled();
        ...
        it('handles direct working reaction removal failures', async () => {
            mockOctokit.rest.reactions.deleteForIssueComment.mockRejectedValue(new Error('403'));
            const liveChannel = new LiveGithubChannel('test-github', makeConfig(), makeBridge());
            await liveChannel.connect();
            liveChannel.disconnect();
            liveChannel.startPromptForTest('owner/repo', 'session-1', '1001');
            ...
            await Promise.resolve();
            liveChannel.endPromptForTest('owner/repo', 'session-1', '1001');
            await vi.waitFor(() => expect(mockOctokit.rest.reactions.deleteForIssueComment).toHaveBeenCalledTimes(3));
            expect(mockOctokit.rest.reactions.deleteForIssueComment).toHaveBeenCalledWith({
                owner: 'owner',
                repo: 'repo',
                comment_id: 1001,
        ...
        it('retries acknowledgement after a create failure', async () => {
            ...
            liveChannel.disconnect();
            liveChannel.startPromptForTest('owner/repo', 'session-1', '1001');
            await vi.waitFor(() => expect(mockOctokit.rest.reactions.createForIssueComment).toHaveBeenCalledTimes(3));
            await Promise.resolve();
            await Promise.resolve();
            liveChannel.startPromptForTest('owner/repo', 'session-2', '1001');
            await vi.waitFor(() => expect(mockOctokit.rest.reactions.createForIssueComment).toHaveBeenCalledTimes(4));
        });
    ...
    describe('sendThreadMessage', () => {
        it('posts comment on the correct issue', async () => {
            mockOctokit.paginate.mockResolvedValue([]);
            await channel.connect();
            await channel.sendThreadMessage('owner/repo', 'issue:42', 'Here is my response');
            expect(mockOctokit.rest.issues.createComment).toHaveBeenCalledWith({
                owner: 'owner',
                repo: 'repo',
                issue_number: 42,
        ...
        it('falls through to sendMessage when threadId is undefined', async () => {
            mockOctokit.paginate.mockResolvedValue([]);
            await channel.connect();
            await expect(channel.sendThreadMessage('owner/repo', undefined, 'response')).rejects.toThrow('createIssueComment requires a threadId');
            expect(mockOctokit.rest.issues.createComment).not.toHaveBeenCalled();
            channel.disconnect();
        });
    });
    describe('sendMessage', () => {
        it('throws', async () => {
            await expect(channel.sendMessage('owner/repo', 'text')).rejects.toThrow('requires a threadId');
        });
    });
    describe('pollInterval', () => {
        it('respects configured pollInterval', () => {
            const ch = new TestableGithubChannel('test', makeConfig({ pollInterval: 30000 }), makeBridge());
            expect(ch.pollInterval).toBe(30000);
        });
        it('defaults to 60000 when not configured', () => {
            const ch = new TestableGithubChannel('test', makeConfig(), makeBridge());
            expect(ch.pollInterval).toBe(60000);
        });
        it.each([0, -1, NaN, Infinity, '60000'])('falls back to 60000 for invalid pollInterval %s', (value) => {
            const ch = new TestableGithubChannel('test', makeConfig({ pollInterval: value }), makeBridge());
            expect(ch.pollInterval).toBe(60000);
        });
    });
    describe('plugin', () => {
        ...
        it('allows explicit local gh authentication without a configured token', async () => {
            const { plugin } = await import('./index.js');
            const tokenField = plugin.management?.fields.find((field) => field.key === 'token');
            const localGhField = plugin.management?.fields.find((field) => field.key === 'useLocalGh');
        ...
        it.each([
            ...
            const { plugin } = await import('./index.js');
            expect(plugin.management?.validateConfig?.(config)).toBe('Channel requires a token or local GitHub CLI authentication (useLocalGh).');
        });
        it.each([
    ...
    describe('validateCursor', () => {
        function validate(parsed) {
            return channel.validateCursor(parsed);
        }
        it.each([
            'dispatchedBodies',
            'dispatchedComments',
            'dispatchedEvents',
        ])('normalizes non-array %s values', (field) => {
    ...
    describe('githubApi retry backoff', () => {
        function githubApi(fn, retries = 3) {
            return channel.githubApi(fn, 'test-op', retries);
        }
        function stubSleep() {
            const sleep = vi.fn().mockResolvedValue(undefined);
            channel.abortableSleep = sleep;
    ...
    describe('webOrigin', () => {
        async function connectAndReadWebOrigin(config) {
            const ch = new TestableGithubChannel('test-ghe', config, makeBridge());
            mockOctokit.paginate.mockResolvedValue([]);
            await ch.connect();
            const origin = ch.webOrigin;
            ch.disconnect();
            return origin;
        }
        it('defaults to https://github.com when no baseUrl is set', async () => {
            await expect(connectAndReadWebOrigin(makeConfig())).resolves.toBe('https://github.com');
        });
        it('rewrites the api.github.com baseUrl to github.com', async () => {
            await expect(connectAndReadWebOrigin(makeConfig({ baseUrl: 'https://api.github.com' }))).resolves.toBe('https://github.com');
        });
        it('strips /api/v3 from a GitHub Enterprise baseUrl', async () => {
            await expect(connectAndReadWebOrigin(makeConfig({ baseUrl: 'https://github.example.com/api/v3' }))).resolves.toBe('https://github.example.com');
        });
