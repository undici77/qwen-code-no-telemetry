...
import { ChannelLoopScheduler, ChannelLoopSkippedError, } from './ChannelLoopScheduler.js';
describe('ChannelLoopScheduler', () => {
    ...
    const runOptions = (timeoutMs = 300_000) => expect.objectContaining({
        timeoutMs,
    ...
    beforeEach(() => {
        ...
        store = {
            ...
            disable: vi.fn(async (id) => {
                jobs = jobs.map((job) => job.id === id ? { ...job, enabled: false } : job);
                return true;
            }),
        };
    ...
    it('clears stale running state and logs startup diagnostics', async () => {
        ...
        try {
            ...
            expect(writeSpy).toHaveBeenCalledWith(expect.stringContaining('[scheduler] started, tick interval 1000ms, jobs 2, enabled 1, cleared stale running 1'));
        }
        finally {
            vi.useRealTimers();
            scheduler.stop();
    ...
    it('truncates long result previews before storing success', async () => {
        ...
        await scheduler.tick();
        await vi.waitFor(() => {
            expect(store.update).toHaveBeenCalledWith('job-1', expect.objectContaining({
                lastResultPreview: 'x'.repeat(500),
            }));
        });
    });
    it('does not replay a recurring job again after recording the catch-up fire', async () => {
        const nextFireTime = vi.fn((_, after) => {
            if (after.getTime() < nowMs)
    ...
    it('does not let stopped in-flight loops write stale lifecycle state', async () => {
        ...
        runLoopPrompt
            .mockImplementationOnce(() => new Promise((resolve) => void (finishFirst = resolve)))
        ...
        await vi.waitFor(() => expect(store.update).toHaveBeenCalledWith('job-1', expect.objectContaining({ lastResultPreview: 'second result' })));
        finishFirst('stale result');
        await Promise.resolve();
        expect(store.update).not.toHaveBeenCalledWith('job-1', expect.objectContaining({ lastResultPreview: 'stale result' }));
        expect(store.update).toHaveBeenCalledWith('job-1', expect.objectContaining({
    ...
    it('uses lastFinishedAt as the recurrence anchor', async () => {
        ...
        const nextFireTime = vi.fn((_, after) => new Date(after.getTime() + 60_000));
        const scheduler = new ChannelLoopScheduler({
            store,
            channels: new Map([['feishu-main', { runLoopPrompt }]]),
        ...
        expect(nextFireTime).toHaveBeenCalledWith(baseJob.cron, new Date('2026-06-30T01:05:00.000Z'));
    });
    ...
    it('marks a loop as running before awaiting the channel loop', async () => {
        let finish;
        runLoopPrompt.mockImplementation(() => new Promise((resolve) => void (finish = resolve)));
        const scheduler = new ChannelLoopScheduler({
            store,
            channels: new Map([['feishu-main', { runLoopPrompt }]]),
            now: () => new Date(nowMs),
    ...
    it('records failures and disables a job after repeated errors', async () => {
        ...
        expect(store.disable).not.toHaveBeenCalled();
        expect(writeSpy).toHaveBeenCalledWith(expect.stringContaining('[scheduler] loop job-1 auto-disabled after 5 consecutive failures'));
        writeSpy.mockRestore();
    });
    it('clears an in-flight loop aborted by bridge recovery instead of failing it', async () => {
        ...
        const failureCountWrites = store.update.mock.calls.filter(([, patch]) => patch.consecutiveFailures !==
            undefined);
    ...
    it('records a genuine failure when recovery starts before the prompt runs', async () => {
        let releaseUpdate;
        const updateBlocked = new Promise((r) => (releaseUpdate = r));
        store.update = vi.fn(async (id, patch) => {
            if (patch.runningSince) {
                await updateBlocked;
            }
            jobs = jobs.map((job) => (job.id === id ? { ...job, ...patch } : job));
            return true;
    ...
    it('disables one-shot loops after a failed attempt', async () => {
        ...
        await scheduler.tick();
        await vi.waitFor(() => {
            expect(store.update).toHaveBeenCalledWith('job-1', expect.objectContaining({ lastStatus: 'error', enabled: false }));
        });
    });
    ...
    it('continues firing other due jobs when one job is still running', async () => {
        ...
        runLoopPrompt.mockImplementation((job) => {
            if (job.id === 'job-1')
    ...
    it('logs success persistence failures without recording a job failure', async () => {
        ...
        expect(writeSpy).toHaveBeenCalledWith(expect.stringContaining('succeeded but status persist failed'));
        writeSpy.mockRestore();
    });
    it('clears running state when failure persistence fails', async () => {
        ...
        expect(writeSpy).toHaveBeenCalledWith(expect.stringContaining('failure persist failed'));
        writeSpy.mockRestore();
    });
    it('clears running state when failure-state reload fails', async () => {
        ...
        try {
            ...
            expect(store.update).not.toHaveBeenCalledWith('job-1', expect.objectContaining({ lastStatus: 'error' }));
            expect(store.disable).not.toHaveBeenCalled();
            expect(writeSpy).toHaveBeenCalledWith(expect.stringContaining('[scheduler] findJob failed in catch for loop job-1: cron unreadable'));
        }
        finally {
            writeSpy.mockRestore();
        }
    ...
    it('ignores a malformed cron job and still fires later due jobs', async () => {
        const secondJob = { ...baseJob, id: 'job-2' };
        jobs = [{ ...baseJob }, secondJob];
        const nextFireTime = vi.fn((cron) => {
            if (cron === baseJob.cron)
                throw new Error('impossible cron');
            return new Date(nowMs - 60_000);
        });
        ...
        await scheduler.tick();
        expect(runLoopPrompt).toHaveBeenCalledWith(expect.objectContaining({ id: 'job-2' }), runOptions());
    });
});
//# sourceMappingURL=ChannelLoopScheduler.test.js.map