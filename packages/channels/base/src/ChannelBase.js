...
import { sanitizeSenderName, sanitizeQuotedText, sanitizePromptText, sanitizePromptPath, sanitizeLogText, sanitizeDisplayText, truncateCodePoints, PROMPT_UNSAFE_INVISIBLES, } from './sanitize.js';
...
import { CHANNEL_MEMORY_RECALL_MAX_ENTRIES, createChannelMemoryRecallIndex, selectRelevantChannelMemory, selectRelevantChannelMemoryFromIndex, } from './channel-memory-recall.js';
/**
...
const GROUP_HISTORY_CONTEXT_MARKER = '[Chat messages since your last reply - for context]';
...
const PARSE_COMMAND_RE = new RegExp(`^\\/([${COMMAND_TOKEN_CHARS}]+)(?:@\\S+)?\\s*(.*)`, 's');
...
export class ChannelBase {
    ...
    groupPairingNotified = new Map();
    loopController;
    observedContacts;
    observedContactEnvelopes = new WeakSet();
    instructedSessions = new Set();
    unattendedMemorySessions = new Set();
    channelMemoryReads = new Map();
    channelMemoryRecallCache = new Map();
    commands = new Map();
    ...
    pendingChannelMemoryMutations = new Map();
    pendingChannelMemoryMutationDeliveries = new Map();
    ...
    preflightedEnvelopes = new WeakSet();
    bridgeToolCallListener = (event) => {
        this.dispatchToolCall(event);
    };
    bridgeBackgroundResponseListener = (sessionId, text) => {
        void this.dispatchBackgroundResponse(sessionId, text).catch((err) => {
    ...
    bridgeSessionDiedListener = (event) => {
        this.onSessionDied(event.sessionId);
    };
    bridgePermissionRequestListener = (event) => {
    ...
    bridgePermissionResolvedListener = (event) => {
        this.dispatchPermissionResolved(event);
    };
    pendingPermissions = new Map();
    pendingPermissionsByChat = new Map();
    channelLoopToolHandler = {
        canHandle: (sessionId) => this.router.getTarget(sessionId)?.channelName === this.name,
        create: (sessionId, input) => this.createLoopFromTool(sessionId, input),
        list: (sessionId) => this.listLoopsFromTool(sessionId),
        cancel: (sessionId, id) => this.cancelLoopFromTool(sessionId, id),
    };
    ...
    async dispatchBackgroundResponse(sessionId, text) {
    ...
    normalizeUserQuestions(pending) {
        ...
        const identifiedLegacy = meta?.['toolName'] === 'ask_user_question' ||
    ...
    async respondToUserInput(pending, response) {
        ...
        pending.responsePromise = Promise.resolve()
            ...
            .then((accepted) => {
            this.removePendingPermission(pending.requestId, accepted
    ...
    constructor(name, config, bridge, options) {
        ...
        this.groupHistory = new GroupHistoryStore(options?.groupHistoryPath ??
            join(getGlobalQwenDir(), 'channels', `${encodeURIComponent(name)}-group-history.jsonl`));
        ...
        const pairingStore = config.senderPolicy === 'pairing' || config.groupPolicy === 'pairing'
    ...
    async deliverProactive(target, text) {
        ...
        if ((target.type !== 'user' && target.type !== 'chat') ||
    ...
    async prepareUnattendedSessionContext(sessionId, target, taskLabel) {
        ...
        const shouldReadUnattendedMemory = channelMemory !== undefined &&
    ...
    setCachedChannelMemoryRecallIndex(key, revision, index) {
        ...
        if (this.channelMemoryRecallCache.size >
    ...
    async runLoopPrompt(job, options = {}) {
        ...
        const current = prev.then(async () => {
            ...
            const acceptedUnattendedMemory = unattendedMemory?.generation === unattendedMemory?.state.generation
                ? unattendedMemory
                : undefined;
            const context = [
            ...
            const promptToSend = context.length > 0
                ? `${context.join('\n\n')}\n\n${promptText}`
                : promptText;
            if (shouldClaimStaticContext) {
            ...
            try {
                const response = await this.runLoopBridgePrompt(promptBridge, sessionId, promptToSend, job.prompt, promptState, job.id, options.timeoutMs);
        ...
        this.sessionQueues.set(sessionId, current.then(() => undefined).catch(() => { }));
        return current;
    }
    ...
    async runWebhookTask(task, options = {}) {
        ...
        const current = prev.then(async () => {
            ...
            promptBridge.on('textChunk', onChunk);
            try {
                const response = await this.runLoopBridgePrompt(promptBridge, sessionId, promptToSend, displayText, promptState, taskId, options.timeoutMs);
                await this.settleCancelRequested(promptState);
                if (promptState.cancelled) {
        ...
        this.sessionQueues.set(sessionId, current.then(() => undefined).catch(() => undefined));
        return await current;
    }
    webhookRoutingThreadId(task, target) {
        return `webhook:${task.source}:${target.threadId ?? target.chatId}`;
    }
    async runLoopBridgePrompt(promptBridge, sessionId, promptText, displayText, promptState, jobId, timeoutMs) {
    ...
    requestActivePromptCancellation(sessionId, reason = 'cancel_command') {
        ...
        const cancelRequested = active.cancelRequested ??
            this.bridge.cancelSession(sessionId).then(() => true, (err) => {
                this.logCancelSessionFailure(sessionId, err);
    ...
    getResponseThreadId(sessionId) {
        return (this.activePrompts.get(sessionId)?.threadId ??
     ...
     */
    async onResponseComplete(chatId, fullText, sessionId, _segment) {
    ...
    registerCancelCommand(name = 'cancel') {
        this.registerCommand(name, async (envelope) => {
            ...
            await this.sendThreadMessage(envelope.chatId, envelope.threadId, cancelSucceeded
    ...
    canEnvelopeAnswerPendingPermission(envelope, pending) {
        return (pending.target.chatId === envelope.chatId &&
    ...
    approvalOptionId(pending) {
        ...
        return (options.find((option) => option.kind === 'allow_once')?.optionId ??
            options.find((option) => option.optionId === 'proceed_once' &&
                option.kind === undefined)?.optionId);
    }
    approvalAlwaysOption(pending) {
        const options = pending.request.options.filter((option) => option.kind === 'allow_always');
        const option = this.findScopedAlwaysOption(options, 'project') ??
    ...
    denialResponse(pending) {
        const option = pending.request.options.find((candidate) => candidate.kind === 'reject_once') ??
            pending.request.options.find((candidate) => candidate.optionId === 'cancel' &&
    ...
    async handlePermissionResponseCommand(envelope, args, decision) {
        ...
        if (lookup.kind === 'none') {
            await this.sendThreadMessage(envelope.chatId, envelope.threadId, lookup.explicit
        ...
        if (pending.userInputPresented && decision !== 'deny') {
            await this.sendMessage(envelope.chatId, 'Submit this question through its interactive card, or use /deny [request-id] to cancel it.');
            return true;
        }
        const response = (() => {
            if (decision === 'deny') {
            ...
            const optionId = decision === 'approve'
        ...
        if (!response) {
            await this.sendThreadMessage(envelope.chatId, envelope.threadId, decision === 'approve-always'
        ...
        this.removePendingPermission(pending.requestId);
        await this.sendThreadMessage(envelope.chatId, envelope.threadId, accepted
            ? decision === 'approve'
                ? 'Permission approved.'
    ...
    registerSharedCommands() {
                ...
                for (const id of removedIds) {
                    ...
                    if (active) {
                        ...
                        if (!settled) {
                            ...
                            try {
                                this.onPromptEnd(active.chatId, id, active.loopPrompt ? undefined : active.messageId);
                            }
                            catch (err) {
        ...
        const clearHandler = async (envelope, args) => {
            if (!this.isAuthorizedForSharedSession(envelope)) {
        ...
        this.registerCommand('who', async (envelope) => {
            ...
            const scopeNote = this.config.sessionScope === 'single'
                ? ' (shared channel-wide)'
    ...
    async handleLoopAdd(envelope, args) {
        ...
        if (this.loopController.createForTarget) {
            ...
            if (existingJobs.filter((existingJob) => existingJob.enabled).length <
    ...
    isStoredLoopTargetAuthorized(target, senderName) {
        ...
        return (this.groupGate.check(envelope, { createPairingRequest: false }).allowed &&
    ...
    invalidateUnattendedMemory(envelope) {
        ...
        for (const entry of this.router.getAll()) {
            if (entry.target.channelName === target.channelName &&
    ...
    renderChannelMemoryCandidate(entry) {
        const preview = truncateCodePoints(sanitizePromptText(entry.text)
            .replace(/[\r\n]+/gu, ' ')
            .trim(), CHANNEL_MEMORY_PREVIEW_CODE_POINT_LIMIT);
    ...
    async handleChannelMemoryIntent(envelope, intent, options = {}) {
        ...
        if (intent.kind === 'update_confirm' || intent.kind === 'remove_confirm') {
            const pending = intent.kind === 'update_confirm'
                ? this.takePendingChannelMemoryMutation(envelope, 'update')
                : this.takePendingChannelMemoryMutation(envelope, 'remove');
            if (!pending) {
                await this.sendThreadMessage(envelope.chatId, envelope.threadId, intent.kind === 'update_confirm'
            ...
            try {
                if (isUpdate) {
                    ({ changed } = await channelMemory.updateChannelMemoryEntry(this.channelMemoryTarget(envelope), {
                        id: pending.id,
                    ...
                    ({ changed } = await channelMemory.removeChannelMemoryEntries(this.channelMemoryTarget(envelope), {
                        ids: [pending.id],
                ...
                await this.sendThreadMessage(envelope.chatId, envelope.threadId, message === 'Channel memory entry changed'
        ...
        if (intent.kind === 'remember') {
            ...
            if (result.added.length > 0) {
                ...
                await this.sendThreadMessage(envelope.chatId, envelope.threadId, result.duplicateIds.length > 0
        ...
        if (intent.kind === 'list' || intent.kind === 'inspect') {
            ...
            if (intent.kind === 'inspect') {
                ...
                await this.sendThreadMessage(envelope.chatId, envelope.threadId, entry
    ...
    shouldClassifyChannelMemoryIntent(text) {
        ...
        return (this.channelMemory !== undefined &&
    ...
    async classifyChannelMemoryIntent(envelope) {
        ...
        try {
            classified =
                await this.memoryIntentClassifier.classifyChannelMemoryIntent(envelope.text, entries);
        }
        catch (error) {
        ...
        try {
            ...
            if (intent === 'remember') {
                ...
                if (hasMemory) {
                    ...
                    if (Array.isArray(memories)) {
                        ...
                        if (snapshot.every((memory) => typeof memory === 'string')) {
            ...
            if (!targetIdSnapshot.every((id) => typeof id === 'string' && entryById.has(id)) ||
    ...
    isRecognizedCommand(text, sessionId) {
        ...
        return this.getAgentCommandsForSession(sessionId).some((cmd) => cmd.name === token ||
    ...
    groupHistoryLimit(envelope) {
        ...
        const configured = groupCfg?.groupHistoryLimit ??
    ...
    prependGroupHistoryContext(promptText, entries) {
        ...
        const lines = this.config.groupPolicy === 'pairing'
            ? entries
    ...
    preflightInbound(envelope) {
        ...
        if (!groupResult.allowed) {
            if (groupResult.pairing !== undefined) {
                ...
                return this.onGroupPairingRequired(envelope.chatId, groupResult.pairing, envelope.threadId)
                    .then(() => false)
                    .catch((err) => {
        ...
        if (!result.allowed) {
            if (result.pairing !== undefined) {
                ...
                return this.onPairingRequired(envelope.chatId, result.pairing, envelope.threadId)
                    .then(() => false)
                    .catch((err) => {
    ...
    async processInbound(envelope) {
        ...
        const recognizedSlashCommand = this.isSlashCommand(envelope.text) &&
        ...
        if (active) {
            ...
            switch (mode) {
                case 'collect': {
                    ...
                    const bufferedDisplayText = (envelope.isGroup || this.config.sessionScope === 'single') &&
                        !envelope.alreadyPrefixed &&
                        !recognizedSlashCommand
        ...
        const current = prev.then(async () => {
            ...
            if (!recognizedSlashCommand &&
                ...
                try {
                    ...
                    this.observeChannelMemoryRecall(recallStartedAt, selection.cache, stale
                        ? 'stale'
                        : (selection.result ??
                            (relevantEntries.length > 0 ? 'selected' : 'empty')), relevantEntries.length);
            ...
            const acceptedRecallContext = recallRead?.generation === recallRead?.state.generation
                ? recallContext
                : undefined;
            ...
            const streamer = useBlockStreaming
                ? new BlockStreamer({
                    ...
                    send: (text) => this.sendResponseMessage(envelope.chatId, text, sessionId),
                })
                : null;
