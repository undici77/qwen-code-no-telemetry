...
export class ChannelBase {
  ...
  drainCollectBufferForCurrentPrompt(sessionId, stillCurrent, taskLabel) {
    ...
    this.processInbound(syntheticEnvelope).catch((err) => {
      process.stderr.write(
        `[${this.name}] dropped ${lost} buffered message(s) after ${taskLabel} for session ${sessionId} (last sender ${lastEnvelope.senderId}): ${err instanceof Error ? err.message : String(err)}\n`,
      );
    });
  }
      ...
      try {
        ...
        if (!promptState.clearEvicted) {
          try {
            ...
            process.stderr.write(
              `[${safeChannel}] onPromptEnd threw in webhook ${safeTaskId} for session ${safeSessionId}: ${err instanceof Error ? err.message : err}\n`,
            );
          }
        }
        if (stillCurrent) {
  ...
  async cancelTimedOutLoopPrompt(promptBridge, sessionId, jobId) {
    ...
    try {
      ...
      process.stderr.write(
        `[${this.name}] cancelSession failed for timed out loop ${jobId} in session ${sessionId}: ${cancelErr instanceof Error ? cancelErr.message : cancelErr}\n`,
      );
    } finally {
  ...
  userInputSettlementReason(pending, outcome) {
    ...
    if (outcome?.outcome === 'selected') {
      ...
      if (
        ...
        (selected?.optionId === 'cancel' && selected.kind === undefined)
      ) {
        return 'cancelled';
      }
    }
  ...
  approvalOptionId(pending) {
    ...
    return (
      ...
      options.find(
        (option) =>
          option.optionId === 'proceed_once' && option.kind === undefined,
      )?.optionId
    );
  }
  ...
  denialResponse(pending) {
    const option =
      ...
      pending.request.options.find(
        (candidate) =>
          candidate.optionId === 'cancel' && candidate.kind === undefined,
      );
    if (option) {
  ...
  async classifyChannelMemoryIntent(envelope) {
    ...
    try {
      ...
      process.stderr.write(
        `[${this.name}] channel memory intent classifier failed: ${sanitizeLogText(this.channelMemoryErrorMessage(error), 200)}\n`,
      );
      return null;
    }
    try {
      ...
      if (
        !targetIdSnapshot.every(
          (id) => typeof id === 'string' && entryById.has(id),
        ) ||
      ...
      process.stderr.write(
        `[${this.name}] channel memory intent validation failed: ${sanitizeLogText(this.channelMemoryErrorMessage(error), 200)}\n`,
      );
      return null;
    }
  }
  ...
  logChannelMemoryError(action, envelope, message) {
    process.stderr.write(
      `[${this.name}] channel memory ${action} failed for sender=${sanitizeLogText(envelope.senderId, 80)} chat=${sanitizeLogText(envelope.chatId, 80)} thread=${sanitizeLogText(envelope.threadId ?? '', 80)}: ${sanitizeLogText(message, 200)}\n`,
    );
  }
  /**
    ...
    if (!groupResult.allowed) {
      if (groupResult.pairing !== undefined) {
        ...
        return this.onGroupPairingRequired(
          ...
          .catch((err) => {
            process.stderr.write(
              `[Channel:${this.name}] group pairing notification failed: ${sanitizeLogText(err instanceof Error ? err.message : String(err), 200)}\n`,
            );
            return false;
          });
      }
    ...
    if (!result.allowed) {
      if (result.pairing !== undefined) {
        ...
        return this.onPairingRequired(
          ...
          .catch((err) => {
            process.stderr.write(
              `[Channel:${this.name}] pairing notification failed: ${sanitizeLogText(err instanceof Error ? err.message : String(err), 200)}\n`,
            );
            return false;
          });
      }
  ...
  logPreflightRejected(reason) {
    process.stderr.write(
      `[Channel:${this.name}] preflight rejected reason=${sanitizeLogText(reason, 80)}\n`,
    );
  }
  logDebugPayload(platform, payload) {
    ...
    try {
      process.stderr.write(
        `${prefix} ${sanitizeLogText(JSON.stringify(payload, redactPayloadValue), DEBUG_PAYLOAD_LIMIT)}\n`,
      );
    } catch {
    ...
    const current = prev.then(async () => {
      ...
      try {
        ...
        if (stillCurrent && buffer && buffer.length > 0) {
          ...
          this.processInbound(syntheticEnvelope).catch((err) => {
            process.stderr.write(
              `[${this.name}] dropped ${lost} buffered message(s) on collect re-entry for session ${sessionId} (last sender ${lastEnvelope.senderId}): ${err instanceof Error ? err.message : String(err)}\n`,
            );
          });
        }
      }
