...
export type UserInputSettlementReason = 'resolved_outside_presenter' | 'cancelled' | 'run_cancelled';
...
export type ChannelOutputSegmentEndReason = 'response_boundary' | 'input_requested' | 'completed' | 'failed' | 'cancelled';
...
export type ChannelTaskCancellationReason = 'cancel_command' | 'clear' | 'steer' | 'timeout' | 'dropped';
export type ChannelTaskLifecycleEvent = (ChannelTaskLifecycleBase & {
    type: 'started';
})
...
export declare function isTerminalTaskLifecycleType(type: ChannelTaskLifecycleEvent['type']): type is 'completed' | 'cancelled' | 'failed';
...
export interface ChannelMemoryCallbacks {
    ...
    listChannelMemoryEntries(target: ChannelMemoryTarget): Promise<ChannelMemoryEntry[]>;
    addChannelMemoryEntries(target: ChannelMemoryTarget, texts: readonly string[], createdBy?: string): Promise<{
        changed: boolean;
        added: ChannelMemoryEntry[];
        duplicateIds: string[];
    }>;
    updateChannelMemoryEntry(target: ChannelMemoryTarget, mutation: {
        id: string;
        text: string;
        expectedText?: string;
    }): Promise<{
    ...
    removeChannelMemoryEntries(target: ChannelMemoryTarget, mutation: {
        ids: readonly string[];
...
export type ChannelConfigFieldKind = 'string' | 'secret' | 'boolean' | 'number' | 'enum' | 'string-list' | 'record' | 'object';
...
export type ChannelConfigNestedFieldDescriptor = (Omit<ChannelConfigValueFieldDescriptor, 'kind' | 'envResolvable'> & {
    kind: Exclude<ChannelConfigFieldKind, 'secret' | 'enum' | 'number' | 'object'>;
    envResolvable?: never;
...
export type ChannelConfigFieldDescriptor = ChannelConfigValueFieldDescriptor | ChannelConfigPlainValueFieldDescriptor | ChannelConfigEnumFieldDescriptor | ChannelConfigNumberFieldDescriptor | ChannelConfigObjectFieldDescriptor;
export interface ChannelManagementDescriptor {
     ...
     */
    validateConfig?: (config: Readonly<Record<string, unknown>>) => string | undefined;
}
/**
...
export interface ChannelPlugin {
    ...
    createChannel(name: string, config: ChannelConfig & Record<string, unknown>, bridge: ChannelAgentBridge, options?: ChannelBaseOptions): ChannelBase;
}
export {};
