...
import { ACP_PRIVATE_PARENT_CAPABILITY_ENV, ACP_PRIVATE_PARENT_CAPABILITY_META_KEY, CHANNEL_PROMPT_DISPLAY_TEXT_META_KEY, CHANNEL_PROMPT_META_KEY, } from './ChannelAgentBridge.js';
import { CHANNEL_LOOP_MCP_SERVER_NAME, CLIENT_MCP_MESSAGE_METHOD, CLIENT_MCP_OVER_WS_CONFIG_FLAG, ChannelLoopMcpServer, WORKSPACE_MCP_RUNTIME_ADD_METHOD, } from './ChannelLoopTools.js';
...
const ACP_EVENT_LOOP_STALL_RE = /^\[perf\] acp agent event loop stall: max=(\d+(?:\.\d+)?)ms/m;
/**
...
export function readAvailableCommandAltNames(raw) {
    ...
    const fromMeta = typeof meta === 'object' && meta !== null
        ? meta['altNames']
        : undefined;
...
export class AcpBridge extends EventEmitter {
            ...
            case 'agent_message_chunk': {
                ...
                if (content?.type === 'text' && content.text) {
                    this.emit(meta?.['source'] === 'slash_command'
                        ? 'slashCommandOutput'
                        : 'textChunk', sessionId, content.text);
                }
                break;
            }
            case 'tool_call': {
    ...
    requestPermission(request) {
        ...
        const sessionId = typeof request.sessionId === 'string' && request.sessionId.length > 0
            ? request.sessionId
    ...
    resolveChannelLoopToolHandler(sessionId) {
        if (this.channelLoopToolHandlers.length === 1 &&
        ...
        if (handler)
            return handler;
        throw new Error(this.channelLoopToolHandlers.length === 0
