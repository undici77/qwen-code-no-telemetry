...
describe('PairingStore workspace scoping (#7017)', () => {
    ...
    it('approves a group without approving the member who requested it', () => {
        ...
        const result = store.createGroupRequest('group-1', 'Release Team', 'sender-1', 'Alice');
        expect(result).toEqual({ code: expect.any(String) });
    ...
    it('reuses one pending request per group regardless of the initiating sender', () => {
        const store = new PairingStore('support-bot', workspaceA);
        const first = store.createGroupRequest('group-1', 'Release Team', 'alice', 'Alice');
        const second = store.createGroupRequest('group-1', 'Release Team', 'bob', 'Bob');
    ...
    it('limits each sender to one pending request across subjects', () => {
        ...
        expect(store.createGroupRequest('group-2', 'Platform Team', 'alice', 'Alice')).toEqual({ rejected: 'sender_pending' });
        ...
        expect(store.createGroupRequest('group-1', 'Release Team', 'alice', 'Alice')).toEqual(first);
        // Other senders are unaffected until the shared cap (3) is reached.
        expect(store.createGroupRequest('group-2', 'Platform Team', 'bob', 'Bob')).toEqual({ code: expect.any(String) });
    ...
    it('frees the sender slot once their pending request is approved', () => {
        const store = new PairingStore('support-bot', workspaceA);
        const code = codeOf(store.createGroupRequest('group-1', 'Release Team', 'alice', 'Alice'));
        expect(store.createRequest('alice', 'Alice')).toEqual({
            rejected: 'sender_pending',
        });
        store.approve(code);
    ...
    it('isolates group approvals and revocation by workspace', () => {
        ...
        for (const store of [storeA, storeB]) {
            const code = codeOf(store.createGroupRequest('group-1', 'Release Team', 'sender-1', 'Alice'));
            store.approve(code);
        }
    ...
    it('keeps group approvals separate from similarly named channel user approvals', () => {
        ...
        const code = codeOf(groupStore.createGroupRequest('shared-id', 'Release Team', 'sender-1', 'Alice'));
    ...
    it('maps equivalent spellings of the same workspace to the same store', () => {
        const store = new PairingStore('support-bot', workspaceA);
        const sameViaRelativeHop = new PairingStore('support-bot', path.join(workspaceA, 'sub', '..'));
    ...
    it('writes scoped files under channels/<workspace-scope>/, not the global dir', () => {
        ...
        const scopeDir = path.join(channelsRoot(), getWorkspaceScopeDirName(workspaceA));
        expect(fs.existsSync(path.join(scopeDir, 'support-bot-pairing.json'))).toBe(true);
        expect(fs.existsSync(path.join(channelsRoot(), 'support-bot-pairing.json'))).toBe(false);
    });
    it('keeps the legacy global layout when no workspace is given', () => {
        const store = new PairingStore('support-bot');
        const code = codeOf(store.createRequest('sender-1', 'Sender One'));
        store.approve(code);
        expect(fs.existsSync(path.join(channelsRoot(), 'support-bot-allowlist.json'))).toBe(true);
    });
    describe('legacy migration (grandfathering)', () => {
        const seedLegacy = () => {
            fs.mkdirSync(channelsRoot(), { recursive: true });
            fs.writeFileSync(path.join(channelsRoot(), 'support-bot-allowlist.json'), JSON.stringify(['legacy-sender']));
            fs.writeFileSync(path.join(channelsRoot(), 'support-bot-pairing.json'), JSON.stringify([
                {
        ...
        it('copies the legacy group allowlist into the scoped store', () => {
            fs.mkdirSync(channelsRoot(), { recursive: true });
            fs.writeFileSync(path.join(channelsRoot(), 'support-bot-groups.json'), JSON.stringify(['legacy-group']));
            const store = new PairingStore('support-bot', workspaceA);
        ...
        it('diverges after migration: post-migration approvals stay per-workspace', () => {
            ...
            const legacy = JSON.parse(fs.readFileSync(path.join(channelsRoot(), 'support-bot-allowlist.json'), 'utf-8'));
        ...
        it('does not resurrect senders revoked by deleting the scoped allowlist file', () => {
            ...
            const scopedDir = path.join(channelsRoot(), getWorkspaceScopeDirName(workspaceA));
        ...
        it('does not absorb a legacy file that appears after the scope is in use', () => {
            ...
            fs.writeFileSync(path.join(channelsRoot(), 'support-bot-allowlist.json'), JSON.stringify(['late-legacy-sender']));
            const reopened = new PairingStore('support-bot', workspaceA);
        ...
        it('migrates every channel of a workspace, not only the first one constructed', () => {
            ...
            fs.mkdirSync(channelsRoot(), { recursive: true });
            fs.writeFileSync(path.join(channelsRoot(), 'chan-a-allowlist.json'), JSON.stringify(['sender-a']));
            fs.writeFileSync(path.join(channelsRoot(), 'chan-b-allowlist.json'), JSON.stringify(['sender-b']));
        ...
        it('migrates legacy files written under a raw channel name that encodes differently', () => {
            ...
            fs.mkdirSync(channelsRoot(), { recursive: true });
            fs.writeFileSync(path.join(channelsRoot(), 'my channel-allowlist.json'), JSON.stringify(['spaced-sender']));
        ...
        it('retries a partially failed migration instead of locking in incomplete state', () => {
            ...
            const badAllowlist = path.join(channelsRoot(), 'support-bot-allowlist.json');
...
describe('group allowlist durability', () => {
    ...
    const groupsPath = () => path.join(qwenHome, 'channels', getWorkspaceScopeDirName(workspace), 'support-bot-groups.json');
    it('refuses to rebuild an unreadable group allowlist on approve', () => {
        const store = new PairingStore('support-bot', workspace);
        const code = codeOf(store.createGroupRequest('group-a', 'Group A', 'sender-1', 'Alice'));
        ...
        expect(fs.readFileSync(groupsPath(), 'utf-8')).toBe('["group-b", "group-c"');
    });
    it('fails closed and logs when the group allowlist is unreadable', () => {
        ...
        try {
            ...
            expect(store.getGroupAllowlist()).toEqual([]);
            expect(stderr).toHaveBeenCalledWith(expect.stringContaining('group allowlist'));
        }
        finally {
            stderr.mockRestore();
        }
...
//# sourceMappingURL=PairingStore.test.js.map