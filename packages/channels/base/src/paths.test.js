...
import { canonicalizeWorkspacePath, getGlobalQwenDir, getWorkspaceScopeDirName, resolvePath, } from './paths.js';
...
describe('canonicalizeWorkspacePath', () => {
    ...
    it('collapses a symlinked spelling to the same scope as the real path', () => {
        ...
        try {
            expect(canonicalizeWorkspacePath(link)).toBe(canonicalizeWorkspacePath(real));
            expect(getWorkspaceScopeDirName(link)).toBe(getWorkspaceScopeDirName(real));
        }
        finally {
            fs.unlinkSync(link);
    ...
    it('collapses trailing-separator and dot-dot spellings of a nonexistent path', () => {
        ...
        expect(getWorkspaceScopeDirName(`${missing}${path.sep}`)).toBe(getWorkspaceScopeDirName(missing));
        expect(getWorkspaceScopeDirName(path.join(missing, '..', 'qwen-scope-missing-norm'))).toBe(getWorkspaceScopeDirName(missing));
    });
    it('keeps the resolved spelling for a path that does not exist (ENOENT fallback)', () => {
        ...
        expect(getWorkspaceScopeDirName(missing)).toBe(getWorkspaceScopeDirName(resolvePath(missing)));
    });
});
