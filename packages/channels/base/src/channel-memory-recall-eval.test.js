...
import { CHANNEL_MEMORY_RECALL_MAX_CODE_POINTS, CHANNEL_MEMORY_RECALL_MAX_ENTRIES, selectRelevantChannelMemory, } from './channel-memory-recall.js';
const fixtureUrl = new URL('./__fixtures__/channel-memory-recall-eval.json', import.meta.url);
const RECALL_AT = 3;
...
function assertKnownFields(value, allowedFields, field) {
...
function parseStringArray(value, field) {
    ...
    const ids = value.map((item, index) => parseNonEmptyString(item, `${field}[${index}]`));
    if (new Set(ids).size !== ids.length) {
        throw new Error(`${field} must contain unique IDs`);
    }
    return ids;
}
function assertReferencesBelongToEntries(references, entryIds, field) {
...
function parseEntry(value, caseId) {
    if (!isRecord(value)) {
...
function parseCase(value) {
    ...
    const relevantIds = parseStringArray(value['relevantIds'], `${id}.relevantIds`);
    assertReferencesBelongToEntries(relevantIds, entryIds, `${id}.relevantIds`);
    const expectedSelectedIds = value['expectedSelectedIds'] === undefined
        ? undefined
        : parseStringArray(value['expectedSelectedIds'], `${id}.expectedSelectedIds`);
    if (expectedSelectedIds) {
        assertReferencesBelongToEntries(expectedSelectedIds, entryIds, `${id}.expectedSelectedIds`);
    }
    const expectedTopId = value['expectedTopId'] === undefined
        ? undefined
...
function loadCases() {
    const parsed = JSON.parse(readFileSync(fixtureUrl, 'utf8'));
...
function evaluate(cases) {
    ...
    const selectedIdsByCase = {};
    for (const testCase of cases) {
        const selected = selectRelevantChannelMemory(testCase.message, testCase.entries);
        ...
        maxSelectedCodePoints = Math.max(maxSelectedCodePoints, selected.reduce((sum, entry) => sum + Array.from(entry.text).length, 0));
        if (testCase.relevantIds.length > 0) {
            const hits = testCase.relevantIds.filter((id) => selectedIds.slice(0, RECALL_AT).includes(id)).length;
    ...
    return {
        summary: {
            ...
            noResultPrecision: noResultPredictions === 0
                ? 0
...
describe('channel memory recall evaluation', () => {
    ...
    it('loads the complete synthetic multilingual fixture', () => {
        ...
        expect(cases.map((testCase) => testCase.id).sort()).toEqual([...requiredCaseIds].sort());
        const categoryCounts = cases.reduce((counts, testCase) => {
        ...
        expect(new Set(cases.map((testCase) => testCase.category))).toEqual(categories);
    });
    ...
    it('keeps every evaluation result within the existing prompt budgets', () => {
        ...
        expect(CHANNEL_MEMORY_RECALL_MAX_CODE_POINTS).toBe(MAX_SELECTED_CODE_POINTS);
        expect(summary.maxSelectedEntries).toBeLessThanOrEqual(RECALL_AT);
        expect(summary.maxSelectedCodePoints).toBeLessThanOrEqual(MAX_SELECTED_CODE_POINTS);
    });
    it('asserts the required ordered selected IDs', () => {
        const cases = loadCases();
        const { selectedIdsByCase } = evaluate(cases);
        for (const [caseId, expectedSelectedIds] of Object.entries(requiredSelectedIds)) {
...
//# sourceMappingURL=channel-memory-recall-eval.test.js.map