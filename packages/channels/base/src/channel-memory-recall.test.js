...
import { CHANNEL_MEMORY_RECALL_FALLBACK_CODE_POINTS, CHANNEL_MEMORY_RECALL_MAX_CODE_POINTS, CHANNEL_MEMORY_RECALL_MAX_ENTRIES, createChannelMemoryRecallIndex, selectRelevantChannelMemory, selectRelevantChannelMemoryFromIndex, } from './channel-memory-recall.js';
function entry(id, text) {
    return { id, text };
}
function longFact(text) {
    return `${text} ${'z'.repeat(CHANNEL_MEMORY_RECALL_FALLBACK_CODE_POINTS + 1)}`;
}
describe('selectRelevantChannelMemory', () => {
    it('reuses an immutable prepared index with equivalent selection', () => {
        ...
        expect(selectRelevantChannelMemoryFromIndex('deploy staging', index)).toEqual(selectRelevantChannelMemory('deploy staging', entries));
        entries[0].text = 'changed after indexing';
        entries[1].text = 'production only';
        expect(selectRelevantChannelMemoryFromIndex('deploy staging', index)).toEqual([
            entry('relevant', longFact('deploy staging')),
            entry('fallback', 'short preference'),
        ]);
    });
    it('matches NFKC-normalized, lowercased Latin terms', () => {
        ...
        expect(selectRelevantChannelMemory('ＤＥＰＬＯＹ to STAGING', [matching])).toEqual([matching]);
    });
    it('does not score Latin or decimal terms shorter than two code points', () => {
        ...
        expect(selectRelevantChannelMemory('x 7 42', [shortTerms, decimalTerm])).toEqual([decimalTerm]);
    });
    it.each([
        ['Han', '数据治理', '项目治理规范'],
        ...
        ['Katakana', 'カタカナ', 'ナカナミ'],
        ['Hangul', '데이터관리', '품질관리자'],
    ])('matches sliding adjacent bigrams within longer %s runs', (_script, message, fact) => {
    ...
    it('treats punctuation, whitespace, and unsafe invisibles as separators', () => {
        const joined = entry('joined', longFact('deploytarget buildnext releasecandidate'));
        const separated = entry('separated', longFact('deploy target build next release candidate'));
        expect(selectRelevantChannelMemory('deploy,target\tbuild\u200bnext\u2028release-candidate', [joined, separated])).toEqual([separated]);
    });
    it('ranks positive overlap by unique term count with stable ties', () => {
        ...
        expect(selectRelevantChannelMemory('alpha alpha beta gamma', [
            oneTerm,
            firstTie,
            secondTie,
    ...
    it('places short no-overlap fallbacks after positives and excludes long ones', () => {
        const fallbackAtLimit = entry('fallback', 'x'.repeat(CHANNEL_MEMORY_RECALL_FALLBACK_CODE_POINTS));
        const longUnrelated = entry('long-unrelated', 'y'.repeat(CHANNEL_MEMORY_RECALL_FALLBACK_CODE_POINTS + 1));
        const positive = entry('positive', longFact('deploy'));
        expect(selectRelevantChannelMemory('deploy', [
            fallbackAtLimit,
            longUnrelated,
            positive,
    ...
    it('measures fallback length after normalization by code point', () => {
        const normalizedAtLimit = entry('at-limit', 'ﬃ'.repeat(CHANNEL_MEMORY_RECALL_FALLBACK_CODE_POINTS / 3));
        const normalizedOverLimit = entry('over-limit', 'ﬃ'.repeat(CHANNEL_MEMORY_RECALL_FALLBACK_CODE_POINTS / 3 + 1));
        expect(selectRelevantChannelMemory('unrelated', [
            normalizedAtLimit,
            normalizedOverLimit,
    ...
    it('returns at most three entries', () => {
        ...
        expect(selectRelevantChannelMemory('unrelated', entries)).toEqual(entries.slice(0, CHANNEL_MEMORY_RECALL_MAX_ENTRIES));
    });
    it('skips entries that exceed the remaining fact-text budget and keeps later complete entries', () => {
        ...
        expect(selectRelevantChannelMemory('aa bb cc', [first, doesNotFit, laterFit])).toEqual([first, laterFit]);
        expect(Array.from(first.text).length + Array.from(laterFit.text).length).toBe(CHANNEL_MEMORY_RECALL_MAX_CODE_POINTS);
    });
    it('truncates a relevant entry that alone exceeds the fact-text budget', () => {
        const relevant = entry('relevant', `deploy runbook ${'x'.repeat(CHANNEL_MEMORY_RECALL_MAX_CODE_POINTS)}`);
        ...
        expect(Array.from(selected[0]?.text ?? '')).toHaveLength(CHANNEL_MEMORY_RECALL_MAX_CODE_POINTS);
...
//# sourceMappingURL=channel-memory-recall.test.js.map