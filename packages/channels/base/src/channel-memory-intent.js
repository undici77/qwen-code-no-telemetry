...
const INSPECT_PATTERNS = [/^查看记忆\s+(\S+)$/u, /^show memory\s+(\S+)$/iu];
...
const CLEAR_CONFIRM_PATTERNS = [
  /^确认清空记忆$/u,
  /^确认清除记忆$/u,
  /^confirm clear memory$/iu,
];
const UPDATE_CONFIRM_PATTERNS = [
  /^确认更新记忆$/u,
  /^confirm memory update$/iu,
];
const REMOVE_CONFIRM_PATTERNS = [
  /^确认删除记忆$/u,
  /^confirm memory removal$/iu,
];
export function parseChannelMemoryIntent(text) {
...
//# sourceMappingURL=channel-memory-intent.js.map
