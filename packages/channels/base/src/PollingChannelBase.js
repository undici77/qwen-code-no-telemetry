...
export class PollingChannelBase extends ChannelBase {
    ...
    async runLoop() {
        ...
        while (this.running && !signal.aborted) {
            try {
                ...
                process.stderr.write(`[Channel:${this.name}] poll error (attempt ${this.consecutiveErrors}), backing off ${backoff}ms: ${err}\n`);
    ...
    cursorPath() {
        ...
        const hash = createHash('sha256')
            ...
            .digest('hex')
            .slice(0, 16);
        return join(getGlobalQwenDir(), 'channels', `${encoded}-${hash}-poll-cursor.json`);
    }
}
