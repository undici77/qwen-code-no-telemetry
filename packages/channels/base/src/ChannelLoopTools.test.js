...
describe('ChannelLoopMcpServer', () => {
    ...
    it.each(['false', 0])('treats serialized false recurring value %s as one-shot', async (recurring) => {
        const create = vi.fn().mockResolvedValue('Loop job-1: * * * * *');
        const server = new ChannelLoopMcpServer({
            create,
            list: vi.fn(),
    ...
    it.each(['true', 1])('treats serialized true recurring value %s as recurring', async (recurring) => {
        const create = vi.fn().mockResolvedValue('Loop job-1: * * * * *');
        const server = new ChannelLoopMcpServer({
            create,
            list: vi.fn(),
    ...
    it('does not respond to JSON-RPC notifications', async () => {
        ...
        await expect(server.handleMessage({ jsonrpc: '2.0', method: 'notifications/initialized' }, {})).resolves.toBeUndefined();
    });
...
//# sourceMappingURL=ChannelLoopTools.test.js.map