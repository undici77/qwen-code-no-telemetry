...
function writePersistedSession(persistPath, key = 'key1') {
...
function invalidationMetadataSize(router) {
    const state = router;
    return (state.routeTokens ?? state.routeGenerations)?.size ?? 0;
}
function daemonSession(sessionId, detach) {
    return {
        sessionId,
        workspaceCwd: '/tmp',
        prompt: vi.fn().mockResolvedValue({}),
        events: vi.fn(async function* (options) {
...
describe('SessionRouter', () => {
    ...
    describe('routing key scopes', () => {
        ...
        it('passes channel approval mode when creating sessions', async () => {
            ...
            expect(bridge.newSession).toHaveBeenCalledWith('/tmp', { approvalMode: 'yolo', sourceId: 'ch' }, expect.any(Object));
        });
        it('stamps channel name as sourceId when creating sessions', async () => {
            const router = new SessionRouter(bridge, '/tmp');
            await router.resolve('dingtalk-main', 'alice', 'chat1');
            expect(bridge.newSession).toHaveBeenCalledWith('/tmp', { sourceId: 'dingtalk-main' }, expect.any(Object));
        });
        ...
        it('thread scope: never downgrades group target metadata', async () => {
            const router = new SessionRouter(bridge, '/tmp', 'thread');
            const sessionId = await router.resolve('ch', 'alice', 'chat1', 'thread1', undefined, true);
            await router.resolve('ch', 'bob', 'chat1', 'thread1');
            expect(router.getTarget(sessionId)).toMatchObject({
                senderId: 'alice',
                chatId: 'chat1',
    ...
    describe('resolve', () => {
        it('passes cwd to bridge.newSession', async () => {
            ...
            expect(bridge.newSession).toHaveBeenCalledWith('/custom', { sourceId: 'ch' }, expect.any(Object));
        });
        it('uses defaultCwd when no cwd provided', async () => {
            const router = new SessionRouter(bridge, '/default');
            await router.resolve('ch', 'alice', 'chat1');
            expect(bridge.newSession).toHaveBeenCalledWith('/default', { sourceId: 'ch' }, expect.any(Object));
        });
        ...
        it('deduplicates concurrent session creation for the same route', async () => {
            ...
            const newSession = vi.fn(() => new Promise((resolve) => {
        ...
        it('retries if a new session dies before the route is stored', async () => {
            ...
            await expect(router.resolve('ch', 'alice', 'chat1')).resolves.toBe('live-session');
        ...
        it('does not store a route if session creation keeps dying', async () => {
            ...
            await expect(router.resolve('ch', 'alice', 'chat1')).rejects.toThrow('Session dead-session died before routing completed (2/2 attempts, key ch:alice:chat1)');
            expect(newSession).toHaveBeenCalledTimes(2);
        ...
        it.each([
            ...
            const router = new SessionRouter(mockBridge(), '/default');
            const newSession = vi.fn(async () => sessionId);
            router.setBridge({
                ...mockBridge(),
                newSession,
            });
            await expect(router.resolve('ch', 'alice', 'chat1')).rejects.toThrow('Invalid session ID from bridge');
            expect(router.getSession('ch', 'alice', 'chat1')).toBeUndefined();
            expect(router.getAll()).toEqual([]);
        });
    });
    ...
    describe('getSession', () => {
        it('returns the session for the configured scope without creating one', async () => {
            ...
            expect(router.getSession('ch', 'bob', 'chat1', 'thread2')).toBeUndefined();
            expect(bridge.newSession).toHaveBeenCalledTimes(1);
        });
    ...
    describe('removeSession', () => {
        ...
        it('releases invalidation metadata for cleared and failed routes', async () => {
            ...
            for (let index = 0; index < 20; index++) {
                await expect(router.resolve('ch', `failed-${index}`, `chat-${index}`)).rejects.toThrow('unavailable');
            }
            expect(router.getAll()).toEqual([]);
    ...
    describe('removeSessionId', () => {
        ...
        it('updates persisted target metadata when reusing a restored session', async () => {
            ...
            const sid = await router.resolve('ch', 'alice', 'chat1', undefined, '/tmp', true);
    ...
    describe('restoreSessions', () => {
        it('passes channel approval mode when restoring sessions', async () => {
            ...
            expect(bridge.loadSession).toHaveBeenCalledWith('old-session', '/tmp', { approvalMode: 'yolo' }, expect.any(Object));
        });
        ...
        it('drops malformed persisted routes from existing eager state', async () => {
            ...
            const restartedBridge = {
                ...
                loadSession: vi
                    .fn()
                    .mockImplementation((sessionId) => Promise.resolve(sessionId)),
            };
            router.setBridge(restartedBridge);
            await expect(router.restoreSessions()).resolves.toEqual({
                restored: 1,
            ...
            expect(restartedBridge.loadSession).toHaveBeenCalledWith(aliceSession, '/tmp', undefined, expect.any(Object));
        ...
        it('persists replacement ids returned by loadSession', async () => {
            ...
            expect(router.getSession('ch', 'alice', 'chat1')).toBe('replacement-session');
            expect(JSON.parse(readFileSync(persistPath, 'utf-8'))).toEqual({
        ...
        it('shares an in-flight restore with concurrent resolve for the same route', async () => {
            ...
            const loadSession = vi.fn(() => new Promise((resolve) => {
            ...
            expect(router.getSession('ch', 'alice', 'chat1')).toBe('restored-session');
            expect(router.getAll()).toHaveLength(1);
        });
        it.each(['removeSession', 'removeSessionId'])('invalidates a restore waiter when %s runs after reservation resolution', async (removal) => {
            const dir = mkdtempSync(join(tmpdir(), 'qwen-router-'));
            tempDirs.push(dir);
            const persistPath = join(dir, 'sessions.json');
            writePersistedSession(persistPath, 'ch:alice:chat1');
            ...
            bridge = {
                ...mockBridge(),
                loadSession: vi.fn(() => new Promise((resolve) => {
                    resolveLoadSession = resolve;
                })),
            };
            const router = new SessionRouter(bridge, '/tmp', 'user', persistPath);
    ...
    describe('persistence safety', () => {
        it('quarantines invalid JSON and starts with no routes', () => {
            ...
            expect(readdirSync(dir).some((name) => name.startsWith('routes.json.corrupt-'))).toBe(true);
        });
        ...
        it('persists through a same-directory temporary file and rename', async () => {
            ...
            expect(mockRenameSync).toHaveBeenCalledWith(expect.stringMatching(/\.tmp$/), persistPath);
    ...
    describe('lazy recovery', () => {
        it('rejects route restoration outside lazy recovery mode', () => {
            ...
            writePersistedSession(persistPath, 'ch:alice:chat1');
            const router = new SessionRouter(bridge, '/tmp', 'user', persistPath);
            expect(() => router.restoreRoutes()).toThrow('restoreRoutes requires lazy recovery mode');
        });
        function createLazyRouter(persistPath, customBridge = bridge) {
        ...
        it.each(['detach', 'cancel'])('does not wait for a hanging %s while rejecting invalidated creation', async (cleanup) => {
            const dir = mkdtempSync(join(tmpdir(), 'qwen-router-'));
            tempDirs.push(dir);
            const persistPath = join(dir, 'routes.json');
            const neverSettles = vi.fn(() => new Promise(() => undefined));
            const session = daemonSession('late-session', cleanup === 'detach' ? neverSettles : undefined);
            if (cleanup === 'cancel') {
        ...
        it.each(['removeSession', 'removeSessionId'])('rejects a dormant load invalidated by %s', async (removal) => {
            const dir = mkdtempSync(join(tmpdir(), 'qwen-router-'));
            tempDirs.push(dir);
            const persistPath = join(dir, 'routes.json');
            writePersistedSession(persistPath, 'ch:alice:chat1');
        ...
        it('does not install a replacement created after route removal', async () => {
            ...
            const lazyBridge = {
                ...
                newSession: vi.fn(() => new Promise((resolve) => {
        ...
        it('does not retry an invalidated shared recovery operation', async () => {
            ...
            const lazyBridge = {
                ...mockBridge(),
                loadSession: vi.fn(() => new Promise((_resolve, reject) => {
                    failLoad = reject;
                })),
        ...
        it.each(['dormant load', 'absent creation'])('rejects a late %s after disposal', async (operation) => {
            const dir = mkdtempSync(join(tmpdir(), 'qwen-router-'));
            tempDirs.push(dir);
            const persistPath = join(dir, 'routes.json');
            let finish;
        ...
        it('replaces a route only after fallback creation succeeds', async () => {
            ...
            const router = createLazyRouter(persistPath, lazyBridge);
            router.restoreRoutes();
            await expect(router.resolve('ch', 'alice', 'chat1')).resolves.toBe('replacement-session');
            // Load-failure replacement also stamps the channel name as sourceId.
            expect(lazyBridge.newSession).toHaveBeenCalledWith('/tmp', { sourceId: 'ch' }, expect.any(Object));
            expect(JSON.parse(readFileSync(persistPath, 'utf-8'))).toEqual({
                'ch:alice:chat1': expect.objectContaining({
                    sessionId: 'replacement-session',
                }),
        ...
        it('retains the dormant route when load and fallback creation both fail', async () => {
            ...
            const router = createLazyRouter(persistPath, lazyBridge);
            router.restoreRoutes();
            await expect(router.resolve('ch', 'alice', 'chat1')).rejects.toThrow('at capacity');
            expect(router.getSession('ch', 'alice', 'chat1')).toBe('old-session');
            expect(JSON.parse(readFileSync(persistPath, 'utf-8'))).toEqual({
        ...
        it('does not eagerly load route counts above the daemon live-session cap', () => {
            ...
            tempDirs.push(dir);
            const persistPath = join(dir, 'routes.json');
            const entries = Object.fromEntries(Array.from({ length: 25 }, (_, index) => [
        ...
        it('clears a dormant route destructively', async () => {
            ...
            expect(JSON.parse(readFileSync(persistPath, 'utf-8'))).toEqual({});
            await expect(router.resolve('ch', 'alice', 'chat1')).resolves.toBe('session-1');
            expect(bridge.loadSession).not.toHaveBeenCalled();
        });
...
//# sourceMappingURL=SessionRouter.test.js.map