...
describe('ChannelLoopStore', () => {
    ...
    it('lists only jobs for the current channel target', async () => {
        ...
        await expect(store.listForTarget('feishu-main', input.target)).resolves.toEqual([first]);
    });
    it('matches targets after group context is promoted', async () => {
        const created = await store.create(input);
        await expect(store.listForTarget('feishu-main', {
            ...input.target,
            isGroup: true,
    ...
    it('creates for a target without counting disabled jobs against the cap', async () => {
        ...
        await expect(store.listForTarget('feishu-main', input.target)).resolves.toHaveLength(2);
    });
    it('enforces the enabled target cap inside the serialized write', async () => {
        ...
        await expect(cappedStore.listForTarget('feishu-main', input.target)).resolves.toHaveLength(1);
    });
    it('matches legacy one-to-one targets that omit isGroup', async () => {
        await fs.mkdir(path.join(tmpDir, 'channels'), { recursive: true });
        await fs.writeFile(path.join(tmpDir, 'channels', 'loops.json'), JSON.stringify([
            {
                ...input,
                id: 'old-job',
                target: {
        ...
        await expect(store.listForTarget('feishu-main', input.target)).resolves.toMatchObject([{ id: 'old-job' }]);
    });
    ...
    it.skipIf(process.platform === 'win32')('writes loop files with private permissions', async () => {
    ...
    it('skips invalid loop entries while preserving valid loops', async () => {
        ...
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('Invalid channel loop at index 0'));
        stderr.mockRestore();
    });
...
//# sourceMappingURL=ChannelLoopStore.test.js.map