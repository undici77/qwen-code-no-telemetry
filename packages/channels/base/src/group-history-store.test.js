...
function filePath() {
    return join(mkdtempSync(join(tmpdir(), 'qwen-group-history-')), 'history.jsonl');
}
function entry(text, senderId = 'u1') {
    return {
        senderId,
        senderName: senderId,
        text,
...
describe('GroupHistoryStore', () => {
    ...
    it('replays only the latest entries from disk', () => {
        ...
        store.record('k', entry('b'), 2);
        store.record('k', entry('c'), 2);
        expect(new GroupHistoryStore(path).drain('k', 2).map((item) => item.text)).toEqual(['b', 'c']);
    });
...
//# sourceMappingURL=group-history-store.test.js.map