...
export class PairingStore {
    ...
    constructor(channelName, workspaceCwd) {
        ...
        this.allowlistPath = path.join(this.dir, `${safeChannelName}-allowlist.json`);
        this.groupAllowlistPath = path.join(this.dir, `${safeChannelName}-groups.json`);
        this.migratedSentinelPath = path.join(this.dir, `${safeChannelName}.migrated`);
        if (workspaceCwd) {
        ...
        try {
            ...
            for (const [legacyPath, scopedPath] of legacyPairs) {
                try {
                    ...
                    allSucceeded = false;
                    process.stderr.write(`[PairingStore] legacy migration of ${path.basename(legacyPath)} ` +
                        `failed for channel "${channelName}": ` +
                        `${err?.message}; will retry on next start\n`);
                }
            }
            ...
            process.stderr.write(`[PairingStore] legacy migration failed for channel ` +
                `"${channelName}": ${err?.message}; scoped store starts empty\n`);
        }
    }
    isApproved(senderId) {
    ...
    createSubjectRequest(subject, senderId, senderName) {
        ...
        const existing = active.find((request) => request.subject.type === subject.type &&
    ...
    readGroupAllowlist(strict = false) {
        try {
            ...
            if (strict && exists) {
                ...
                throw new Error(`refusing to rewrite unreadable group allowlist ` +
                    `"${path.basename(this.groupAllowlistPath)}": ` +
                    `${err?.message}`);
            }
            if (exists) {
                process.stderr.write(`[PairingStore] group allowlist ` +
                    `"${path.basename(this.groupAllowlistPath)}" is unreadable ` +
                    `(${err?.message}); treating as empty — stored group ` +
