...
import { ACP_EVENT_LOOP_STALL_RESTART_MS, ACP_PERMISSION_RESPONSE_TIMEOUT_MS, ACP_START_TIMEOUT_MS, AcpBridge, } from './AcpBridge.js';
import { CHANNEL_LOOP_MCP_SERVER_NAME } from './ChannelLoopTools.js';
import { ACP_PRIVATE_PARENT_CAPABILITY_ENV, ACP_PRIVATE_PARENT_CAPABILITY_META_KEY, CHANNEL_PROMPT_META_KEY, } from './ChannelAgentBridge.js';
const child = vi.hoisted(() => {
    ...
    let initializeImplementation = () => Promise.resolve();
    return {
        instances: [],
        clients: [],
        connections: [],
        ...
        setInitializeImplementation: (implementation) => {
...
describe('AcpBridge', () => {
    ...
    it('times out bridge initialization and stops the child', async () => {
        ...
        try {
            ...
            const rejection = expect(start).rejects.toThrow(`ACP initialization timed out after ${ACP_START_TIMEOUT_MS}ms`);
    ...
    it('performs the private-parent capability handshake with the spawned child', async () => {
        ...
        await bridge.start();
        const spawnOptions = child.spawn.mock.calls[0][2];
        ...
        const initializeParams = child.connections[0].initialize.mock
            .calls[0][0];
        expect(initializeParams._meta?.[ACP_PRIVATE_PARENT_CAPABILITY_META_KEY]).toBe(capability);
    });
    it('registers the channel loop MCP server once across concurrent calls', async () => {
        const pending = [];
        const extMethod = vi.fn(() => new Promise((resolve) => pending.push(resolve)));
        const bridge = new AcpBridge({
            cliEntryPath: '/tmp/qwen',
            cwd: '/tmp',
        });
    ...
    it('returns a synthetic payload ack for MCP notifications', async () => {
        ...
        await expect(bridge.handleClientMcpMessage({
    ...
    it('handles mid-turn queue drain requests from the ACP child', async () => {
        ...
        await expect(bridge.handleExtMethod('craft/drainMidTurnQueue', {
            sessionId: 's-1',
    ...
    it('claims Guard continuations only for a session owned by this bridge', async () => {
        ...
        await expect(bridge.handleExtMethod('craft/claimTodoStopGuardContinuation', {
            sessionId: 's-1',
    ...
    it('returns only the final turn text after tool calls', async () => {
        ...
        await expect(bridge.prompt('s-1', 'question')).resolves.toBe('Final answer.');
    ...
    it('returns only the final slash-command output', async () => {
        ...
        await expect(bridge.prompt('s-1', 'question')).resolves.toBe('Context compressed.');
    });
    ...
    it('preserves text when tool calls are not pending', async () => {
        ...
        await expect(bridge.prompt('s-1', 'question')).resolves.toBe('Before. After.');
    });
    ...
    it('rejects channel loop tool calls when no handler matches the session', () => {
        ...
        expect(() => bridge.resolveChannelLoopToolHandler('s-2')).toThrow('No channel loop handler matched session s-2.');
    });
    ...
    it('kills the ACP child when it reports a large event loop stall', async () => {
        ...
        proc.stderr.write(`[perf] acp agent event loop stall: max=${ACP_EVENT_LOOP_STALL_RESTART_MS + 1000}ms\n`);
    ...
    it('ignores non-perf stderr that mentions an event loop stall', async () => {
        ...
        await bridge.start();
        const proc = child.instances[0];
        proc.stderr.write(`debug: acp agent event loop stall: max=${ACP_EVENT_LOOP_STALL_RESTART_MS + 1000}ms\n`);
        expect(proc.kill).not.toHaveBeenCalled();
    });
    ...
    it('relays ACP permission requests instead of auto-approving them', async () => {
        ...
        await expect(bridge.respondToPermission(event.requestId, response)).resolves.toBe(true);
        await expect(pending).resolves.toEqual(response);
        expect(permissionResolved).toHaveBeenCalledWith({
            requestId: event.requestId,
            outcome: response.outcome,
        });
        await expect(bridge.respondToPermission(event.requestId, response)).resolves.toBe(false);
    });
    ...
    it('resolves matching pending permissions as cancelled when a session is cancelled', async () => {
        ...
        await expect(bridge.respondToPermission(secondEvent.requestId, response)).resolves.toBe(true);
    ...
    it('resolves pending permissions as cancelled after the response timeout', async () => {
        ...
        try {
            ...
            expect(stderr.mock.calls.join('')).toContain(`[AcpBridge] permission request ${event.requestId} timed out after ${ACP_PERMISSION_RESPONSE_TIMEOUT_MS}ms (session=session-1)`);
        }
        finally {
            stderr.mockRestore();
            vi.useRealTimers();
...
//# sourceMappingURL=AcpBridge.test.js.map