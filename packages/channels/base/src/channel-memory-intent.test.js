...
describe('parseChannelMemoryIntent', () => {
    ...
    it('parses deterministic item intents', () => {
        ...
        expect(parseChannelMemoryIntent('把 m-a31f0d82c7e4 改成默认使用 production')).toEqual({
            kind: 'update',
            id: 'm-a31f0d82c7e4',
            text: '默认使用 production',
        });
        expect(parseChannelMemoryIntent('update m-a31f0d82c7e4 to use production')).toEqual({
            kind: 'update',
            id: 'm-a31f0d82c7e4',
            text: 'use production',
        });
        expect(parseChannelMemoryIntent('更新 m-a31f0d82c7e4 为默认使用 production')).toEqual({
            kind: 'update',
            id: 'm-a31f0d82c7e4',
            text: '默认使用 production',
        });
        expect(parseChannelMemoryIntent('change m-a31f0d82c7e4 to use production')).toEqual({
            kind: 'update',
            id: 'm-a31f0d82c7e4',
            text: 'use production',
        });
    ...
    it('parses item updates before broad clear requests', () => {
        expect(parseChannelMemoryIntent('把 m-a31f0d82c7e4 改成默认的记忆清空')).toEqual({
            kind: 'update',
            id: 'm-a31f0d82c7e4',
            text: '默认的记忆清空',
        });
    ...
    it('leaves ambiguous prose alone', () => {
        ...
        expect(parseChannelMemoryIntent('Remember that bug we fixed last week?')).toBeNull();
        expect(parseChannelMemoryIntent('remember this might be tricky later')).toBeNull();
        expect(parseChannelMemoryIntent('/remember-channel use staging')).toBeNull();
    });
});
//# sourceMappingURL=channel-memory-intent.test.js.map