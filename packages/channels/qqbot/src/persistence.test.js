...
vi.mock('node:fs', () => ({
    ...
    existsSync: vi.fn((path) => !!fsExists[path] || path in fsStore),
...
vi.mock('@qwen-code/channel-base', () => ({
    ...
    sanitizeLogText: (text, maxLen) => {
        const sanitized = Array.from(text, (c) => {
            ...
            if (cp === 0x9b)
                return ' '; // C1
            if (cp === 0x85 || cp === 0x2028 || cp === 0x2029)
                return ' '; // line/paragraph sep
            if (cp === 0x202e)
                return ' '; // RLO
            if (cp === 0x0a || cp === 0x0d)
                return '\n';
...
const sessionsPath = join('/tmp/test-qwen', 'channels', 'test-bot-sessions.json');
const globalSessionsPath = join('/tmp/test-qwen', 'channels', 'sessions.json');
const sessionsBackupPath = join('/tmp/test-qwen', 'channels', 'test-bot-sessions-backup.json');
beforeEach(() => {
    vi.useFakeTimers();
    fsStore = {};
    fsExists = {};
...
describe('saveQQState', () => {
    ...
    it('persists chatTypeMap, replyMsgId, msgSeqMap, groupActiveMsgEnabled', () => {
        ...
        const botOpenIdByGroup = ch.botOpenIdByGroup;
...
describe('flushQQState', () => {
    it('writes immediately (skips debounce)', () => {
        const ch = makeChannel();
        ch.flushQQState();
        expect(writeFileSync).toHaveBeenCalledWith(statePath + '.tmp', expect.any(String), { mode: 0o600 });
    });
...
describe('backupGlobalSessions / restoreGlobalSessions', () => {
    it('backupGlobalSessions copies sessions.json to backup path on disconnect', () => {
        ...
        const ch = makeChannel();
        ch.disconnect();
        expect(fsStore[sessionsBackupPath]).toBe(JSON.stringify({ key: 'session-data' }));
    });
...
describe('fixRestoredSessions', () => {
    it('fixes undefined sessionIds in SessionRouter maps', () => {
        const toSession = new Map();
        const toTarget = new Map();
        const toCwd = new Map();
        const entryKey = 'some_key';
        toSession.set(entryKey, undefined);
        const sessionsData = {};
        ...
        expect(toTarget.has(undefined)).toBe(false);
    });
