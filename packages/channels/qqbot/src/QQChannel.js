  ...
  async sendMessage(chatId, text) {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        if (resp.status !== 429) {
          process.stderr
            .write(`[QQ:${this.name}] Send failed (HTTP ${resp.status}: ${errBody})\n
`);
        }
  ...
  saveQQState() {
    ...
    this.saveTimer = setTimeout(() => {
      ...
      try {
        ...
        process.stderr
          .write(`[QQ:${this.name}] saveQQState write failed: ${sanitizeLogText(e instanceof Error ? e.message : String(e), 200)}\n
`);
      }
    }, 500);
  ...
  flushQQState() {
    ...
    try {
      ...
      process.stderr
        .write(`[QQ:${this.name}] flushQQState write failed: ${sanitizeLogText(e instanceof Error ? e.message : String(e), 200)}\n
`);
    }
  }
  /**
  ...
  backupGlobalSessions() {
    try {
      ...
      process.stderr
        .write(`[QQ:${this.name}] backupGlobalSessions failed: ${sanitizeLogText(e instanceof Error ? e.message : String(e), 200)}\n
`);
    }
  }
  restoreGlobalSessions() {
    try {
      ...
      process.stderr
        .write(`[QQ:${this.name}] restoreGlobalSessions failed: ${sanitizeLogText(e instanceof Error ? e.message : String(e), 200)}\n
`);
    }
  }
  /**
  ...
  fixRestoredSessions() {
    try {
      ...
      process.stderr
        .write(`[QQ:${this.name}] fixRestoredSessions failed: ${sanitizeLogText(e instanceof Error ? e.message : String(e), 200)}\n
`);
    }
  }
  // ── ReplyMsgId helpers ────────────────────────────────────────
    ...
    if (delay > 0) {
      ...
      this.tokenRefreshTimer = setTimeout(() => {
        this.fetchToken().catch((e) => {
          ...
          process.stderr
            .write(`[QQ:${this.name}] Token refresh failed: ${sanitizeLogText(e instanceof Error ? e.message : String(e), 200)}, will retry\n
`);
          ...
          const retry = () => {
            ...
            if (++retryCount > 10) {
              process.stderr
                .write(`[QQ:${this.name}] FATAL: token refresh exhausted, reconnecting\n
`);
            ...
            this.tokenRefreshTimer = setTimeout(() => {
              this.fetchToken().catch((e2) => {
                if (this.disposed || this._reconnectId !== tokenReconnectId)
                  return;
                process.stderr
                  .write(`[QQ:${this.name}] Token refresh retry failed (attempt ${retryCount}): ${sanitizeLogText(e2 instanceof Error ? e2.message : String(e2), 200)}\n
`);
                retry();
              });
            }, 60_000);
    ...
    switch (op) {
      case OpCode.HELLO: {
        this.heartbeatInterval = Math.max(
          msg['d']?.['heartbeat_interval'] || 45000,
          5000,
        );
        this.sendIdentify();
        break;
