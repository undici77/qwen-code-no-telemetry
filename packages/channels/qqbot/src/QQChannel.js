 ...
 */
import { ChannelBase, SessionRouter, getGlobalQwenDir, sanitizeSenderName, sanitizePromptText, sanitizeLogText, truncateCodePoints, } from '@qwen-code/channel-base';
import WebSocket from 'ws';
import { readFileSync, writeFileSync, existsSync, mkdirSync, renameSync, unlinkSync, } from 'node:fs';
import { join } from 'node:path';
import { OpCode, Intent } from './types.js';
import { getCredsFilePath, loadCredentials, saveCredentials, } from './accounts.js';
import { qrCodeLogin } from './login.js';
import { fetchAccessToken, fetchGatewayUrl, getApiBase, sendQQMessage, } from './api.js';
...
export class QQChannel extends ChannelBase {
    ...
    static IDLE_FLUSH_BACKOFF_MS = 4000;
    ...
    _lastKeywordNoMatchLog = new Map();
    ...
    // ── Streaming state ───────────────────────────────────────────
    streamState = new Map();
    flushingSessions = new Set();
    pendingStreamDelete = new Set();
    _reconnectId = 0;
    blockStreaming = false;
    flushedSessions = new Set();
    qqStatePath;
    /**
    ...
    constructor(name, config, bridge, options) {
        ...
        const qqCfg = config;
        if ((qqCfg.groupAllPolicy === 'keyword' || qqCfg.groupAllPolicy === 'all') &&
        ...
        if (raw !== undefined &&
            ...
            process.stderr.write(`[QQ:${this.name}] WARNING: invalid bufferFlushLength=${raw}, using default ${QQChannel.MAX_BUFFER_LENGTH}\n`);
        ...
        this.globalSessionsPath = options?.router
            ...
            : sessionsPath;
        this.sessionsBackupPath = join(stateDir, `${safeName}-sessions-backup.json`);
    ...
    handleCronTextChunk(sessionId, text) {
        ...
        setImmediate(() => {
            ...
            entry.buffer += text;
            const limit = this.qqConfig.bufferFlushLength ?? QQChannel.MAX_BUFFER_LENGTH;
                        ...
                        this.sendMessage(target.chatId, toFlush)
                            ...
                            entry.timer = setTimeout(() => {
                                ...
                                this.sendMessage(retryTarget.chatId, toFlush)
                                    ...
                                    .catch((retryErr) => {
                                    const retryCode = retryErr instanceof DeliveryError
                                        ? retryErr.code
                                        : null;
                                    ...
                                    entry.timer = setTimeout(() => {
                                        ...
                                        this.sendMessage(retryTarget2.chatId, toFlush)
                                            ...
                                            .catch((err2) => {
                                            const code2 = err2 instanceof DeliveryError
                                                ? err2.code
                                                : null;
    ...
    async sendMessage(chatId, text) {
        ...
        const msgId = entry && Date.now() - entry.timestamp < QQChannel.REPLY_MSG_ID_TTL_MS
            ? entry.msgId
            : undefined;
        if (entry && !msgId) {
...
`);
    ...
    onResponseChunk(chatId, chunk, sessionId) {
        if (this.blockStreaming)
            return;
        let state = this.streamState.get(sessionId);
        if (!state) {
            state = { chatId, buffer: chunk, timer: null, retryCount: 0 };
            ...
            if (this.pendingStreamDelete.has(sessionId)) {
                ...
                if (current === state) {
                    ...
                    if (this.maxFlushRetries <= 0 ||
                        ...
                        const reconnectId = this._reconnectId;
                        const delay = current.retryCount > 1
...
`);
    ...
    _checkGroupAllPolicyRequireMention() {
        ...
        const anyFalse = groups && Object.values(groups).some((g) => g.requireMention === false);
        if (!anyFalse) {
...
`);
    ...
    restoreQQState() {
        try {
            if (!existsSync(this.qqStatePath))
                return false;
            ...
            if (raw.chatTypeMap) {
                ...
                this.chatTypeMap = new Map(Array.isArray(arr)
                    ? arr.filter(([k, v]) => typeof k === 'string' &&
                        k.length <= 256 &&
                        (v === 'c2c' || v === 'group'))
                    : []);
            ...
            if (raw.replyMsgId) {
                ...
                const totalRaw = Array.isArray(arr) ? arr.length : 0;
                this.replyMsgId = new Map(Array.isArray(arr)
                    ? arr
            ...
            if (raw.msgSeqMap) {
                ...
                const totalRaw = Array.isArray(arr) ? arr.length : 0;
                this.msgSeqMap = new Map(Array.isArray(arr)
                    ? arr.filter(([k, v]) => typeof k === 'string' &&
                        k.length <= 256 &&
                        typeof v === 'number' &&
            ...
            if (raw.groupActiveMsgEnabled) {
                ...
                const totalRaw = Array.isArray(arr) ? arr.length : 0;
                this.groupActiveMsgEnabled = new Map(Array.isArray(arr)
                    ? arr.filter(([k, v]) => typeof k === 'string' &&
                        k.length <= 256 &&
                        typeof v === 'boolean')
                    : []);
            ...
            if (raw.botOpenIdByGroup) {
                ...
                const totalRaw = Array.isArray(arr) ? arr.length : 0;
                this.botOpenIdByGroup = new Map(Array.isArray(arr)
                    ? arr.filter(([k, v]) => typeof k === 'string' &&
                        k.length <= 256 &&
                        typeof v === 'string' &&
                        QQ_OPENID_RE.test(v))
    ...
    restoreGlobalSessions() {
        try {
            if (!existsSync(this.globalSessionsPath) &&
                ...
                writeFileSync(this.globalSessionsPath, readFileSync(this.sessionsBackupPath, 'utf-8'), { mode: 0o600 });
            }
        }
        catch (e) {
...
`);
    ...
    dialGateway(url, resolve, reject) {
        ...
        this.ws.on('close', (code) => {
            ...
            this.ws = null;
            this._ready = false;
            const shouldReconnect = this.serverRequestedReconnect ||
                (code !== 1000 &&
    ...
    handleGatewayMessage(msg, onReady) {
        ...
        switch (op) {
            case OpCode.HELLO: {
                this.heartbeatInterval = Math.max(msg['d']?.['heartbeat_interval'] || 45000, 5000);
                this.sendIdentify();
                break;
            }
    ...
    sendIdentify() {
        ...
        const needsGroupMsg = this.qqConfig.groupAllPolicy === 'keyword' ||
    ...
    prepareGroupMessage(event, chatId, { forceAtMention } = {}) {
        ...
        const displayContent = content
            .replace(/<@[^>]{1,64}>/g, (mention) => event.mentions?.[mentionIndex++]?.is_you ? '' : mention)
            .trim();
        ...
        const openIdSuffix = this.qqConfig.allowMention !== false && groupBotOpenId
        ...
        const showSenderOpenId = this.qqConfig.allowMention !== false &&
            !!senderOpenId &&
    ...
    handleGroup(event) {
        ...
        if (!result)
            return;
        const { isSlash, text, displayText, senderName, safeName, cleanText } = result;
        ...
        if (this.isDuplicate(event.id))
            return;
        if (isSlash) {
            process.stderr.write(`[QQ:${this.name}] Slash cmd from ${sanitizeLogText(safeName, 64)} (${sanitizeLogText(chatId, 64)}): ${sanitizeLogText(cleanText.split(/\s/)[0], 64)}\n`);
        }
    ...
    handleGroupAll(event) {
        ...
        if (!result)
            return;
        const { isSlash, text, displayText, senderName, isAtBot, safeName, cleanText, } = result;
