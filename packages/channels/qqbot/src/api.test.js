...
const { fetchAccessToken, fetchGatewayUrl, getApiBase, sendQQMessage, validateGatewayUrl, } = await import('./api.js');
...
describe('sendQQMessage', () => {
    ...
    it('sends POST with JSON body and correct headers', async () => {
        await sendQQMessage('https://api.example.com', '/v2/users/abc/messages', 'token-123', { content: 'hello', msg_type: 0 });
        expect(mockFetch).toHaveBeenCalledTimes(1);
        const [url, init] = mockFetch.mock.calls[0];
        expect(url).toBe('https://api.example.com/v2/users/abc/messages');
        expect(init?.method).toBe('POST');
        expect(init?.headers?.['Content-Type']).toBe('application/json');
        expect(init?.headers?.['Authorization']).toBe('QQBot token-123');
    ...
    it('returns the fetch Response directly', async () => {
        ...
        const resp = await sendQQMessage('https://api.example.com', '/v2/groups/g123/messages', 'tok', { content: 'hi', msg_type: 0 });
...
describe('fetchAccessToken', () => {
    ...
    it('defaults expiresIn to 7200 when missing', async () => {
        mockFetch.mockResolvedValue(mockResponse(true, 200, { access_token: 'tok-no-exp' }));
        const result = await fetchAccessToken('app-id', 'secret');
    ...
    it('throws on HTTP error with exact status-only message (no body leak)', async () => {
        ...
        await expect(fetchAccessToken('bad', 'bad')).rejects.toThrow(/^QQ Bot token request failed \(HTTP 401\)$/);
    });
    it('throws when response is missing access_token', async () => {
        mockFetch.mockResolvedValue(mockResponse(true, 200, { other: 'data' }));
        await expect(fetchAccessToken('app-id', 'secret')).rejects.toThrow('QQ Bot token response missing access_token');
    });
...
describe('fetchGatewayUrl', () => {
    ...
    it('returns WebSocket URL from production gateway', async () => {
        mockFetch.mockResolvedValue(mockResponse(true, 200, { url: 'wss://gateway.qq.com/ws' }));
        const url = await fetchGatewayUrl('tok', false);
        expect(url).toBe('wss://gateway.qq.com/ws');
        expect(mockFetch).toHaveBeenCalledWith('https://api.sgroup.qq.com/gateway', expect.objectContaining({
            headers: { Authorization: 'QQBot tok' },
        }));
    });
    it('uses sandbox gateway when sandbox is true', async () => {
        mockFetch.mockResolvedValue(mockResponse(true, 200, { url: 'wss://sandbox.gateway.qq.com/ws' }));
        await fetchGatewayUrl('tok', true);
        expect(mockFetch).toHaveBeenCalledWith('https://sandbox.api.sgroup.qq.com/gateway', expect.anything());
    });
    it('throws on HTTP error', async () => {
        mockFetch.mockResolvedValue(mockResponse(false, 500, 'server error'));
        await expect(fetchGatewayUrl('tok', false)).rejects.toThrow('QQ Bot gateway request failed (HTTP 500)');
    });
    it('throws when response is missing url field', async () => {
        mockFetch.mockResolvedValue(mockResponse(true, 200, {}));
        await expect(fetchGatewayUrl('tok', false)).rejects.toThrow('QQ Bot gateway response missing WebSocket URL');
    });
});
describe('validateGatewayUrl', () => {
    it('rejects https URLs', () => {
        expect(() => validateGatewayUrl('https://gateway.qq.com/ws')).toThrow('wss://');
    });
    it('rejects http URLs', () => {
        expect(() => validateGatewayUrl('http://gateway.qq.com/ws')).toThrow('wss://');
    });
    it('rejects ws URLs', () => {
        expect(() => validateGatewayUrl('ws://gateway.qq.com/ws')).toThrow('wss://');
    });
    ...
    it('rejects invalid URLs', () => {
        expect(() => validateGatewayUrl('not a valid url')).toThrow('not a valid URL');
    });
...
describe('fetchGatewayUrl + validateGatewayUrl integration', () => {
    ...
    it('rejects when API returns a non-wss URL (http://)', async () => {
        mockFetch.mockResolvedValue(mockResponse(true, 200, { url: 'http://gateway.qq.com/ws' }));
        await expect(fetchGatewayUrl('tok', false)).rejects.toThrow('wss://');
    });
    it('rejects when API returns a non-wss URL (ws://)', async () => {
        mockFetch.mockResolvedValue(mockResponse(true, 200, { url: 'ws://gateway.qq.com/ws' }));
        await expect(fetchGatewayUrl('tok', false)).rejects.toThrow('wss://');
    });
    it('rejects when API returns a *.tencentcs.com wss:// URL', async () => {
        ...
        await expect(fetchGatewayUrl('tok', false)).rejects.toThrow('unexpected hostname');
    });
    it('rejects when API returns an invalid URL', async () => {
        mockFetch.mockResolvedValue(mockResponse(true, 200, { url: 'not a valid url' }));
        await expect(fetchGatewayUrl('tok', false)).rejects.toThrow('not a valid URL');
    });
    it('accepts when API returns a valid wss:// URL', async () => {
        mockFetch.mockResolvedValue(mockResponse(true, 200, { url: 'wss://api.sgroup.qq.com/ws' }));
        await expect(fetchGatewayUrl('tok', false)).resolves.toBe('wss://api.sgroup.qq.com/ws');
    });
});
