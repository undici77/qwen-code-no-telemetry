...
const { getCredsFilePath, loadCredentials, saveCredentials } = await import('./accounts.js');
describe('getCredsFilePath', () => {
    it('returns path under channels dir with credentials suffix', () => {
        expect(getCredsFilePath('mybot')).toBe(join('/tmp/test-qwen', 'channels', 'mybot-credentials.json'));
    });
...
describe('loadCredentials', () => {
    ...
    it('returns null when file is missing appId', () => {
        ...
        mockReadFileSync.mockReturnValue(JSON.stringify({ appSecret: 'secret-only' }));
        expect(loadCredentials('/path/to/creds.json')).toBeNull();
    });
    ...
    it('returns credentials when both fields are present', () => {
        mockExistsSync.mockReturnValue(true);
        mockReadFileSync.mockReturnValue(JSON.stringify({ appId: 'my-app', appSecret: 'my-secret' }));
...
describe('saveCredentials', () => {
    it('creates dir and writes file with 0o600 permissions', () => {
        ...
        expect(mockMkdirSync).toHaveBeenCalledWith(join('/tmp/test-qwen', 'channels'), { recursive: true });
        expect(mockWriteFileSync).toHaveBeenCalledWith('/path/to/creds.json', JSON.stringify({ appId: 'app-id', appSecret: 'app-secret' }), { mode: 0o600 });
    });
});
