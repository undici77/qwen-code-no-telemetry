...
const { mockSendQQMessage, mockFetchAccessToken, mockHandleInbound } = vi.hoisted(() => ({
...
function makeChannel(configOverrides) {
...
function makeC2CEvent(overrides) {
    return {
        id: 'msg-c2c-001',
        author: {
...
function makeGroupAllEvent(overrides) {
    return {
        id: 'msg-groupall-001',
        author: {
...
describe('isDuplicate', () => {
    ...
    it('自动启动 seenCleanupTimer', () => {
        const ch = makeChannel();
        const pvt = ch;
        expect(ch['seenCleanupTimer']).toBeNull();
        pvt['isDuplicate']('evt-001');
        expect(ch['seenCleanupTimer']).not.toBeNull();
    });
    it('不同 ID 不重复', () => {
        const ch = makeChannel();
        const pvt = ch;
...
describe('handleC2C', () => {
    it('设置 chatTypeMap 为 c2c', () => {
        ...
        const chatTypeMap = ch['chatTypeMap'];
    ...
    it('触发 handleInbound 带正确参数', async () => {
        ...
        const env = mockHandleInbound.mock.calls[0][0];
    ...
    it('missing author 时不触发 handleInbound', async () => {
        const ch = makeChannel();
        const pvt = ch;
        pvt['handleC2C'](makeC2CEvent({ author: undefined }));
        await vi.advanceTimersByTimeAsync(600);
        expect(mockHandleInbound).not.toHaveBeenCalled();
    });
    it('drops bot C2C messages', async () => {
        const ch = makeChannel();
        const pvt = ch;
        pvt['handleC2C'](makeC2CEvent({ author: { bot: true, user_openid: 'bot-1' } }));
        await vi.advanceTimersByTimeAsync(600);
        expect(mockHandleInbound).not.toHaveBeenCalled();
    });
});
...
describe('handleGroup', () => {
    ...
    it('触发 handleInbound 带正确参数：isGroup=true, isMentioned=true', async () => {
        ...
        expect(env['text']).toBe('[atMention=true] [Bob(ABCDEF0123456789ABCDEF0123456789)]: <@OPENID_BOT> 你好');
    ...
    it('does not duplicate an OPENID as both name and mention tag', async () => {
        ...
        await vi.advanceTimersByTimeAsync(600);
        const env = mockHandleInbound.mock.calls[0][0];
        expect(env['text']).toBe('[atMention=true] [QQ User(ABCDEF0123456789ABCDEF0123456789)]: hello');
    });
    ...
    it('groupActiveMsgEnabled=false 时 @bot 消息仍能通过（被动回复）', async () => {
        const ch = makeChannel();
        const pvt = ch;
        const groupActiveMsgEnabled = ch['groupActiveMsgEnabled'];
    ...
    it('senderOpenId 非 32 位 hex 时显示 8 位片段并打 stderr 日志', async () => {
        ...
        expect(env['text']).toBe('[atMention=true] [Bob(ZZZ12345…)]: <@OPENID_BOT> 你好');
        const calls = stderrSpy.mock.calls.map((c) => String(c[0]));
        expect(calls.some((c) => c.includes('Unexpected senderOpenId format') &&
            c.includes('ZZZ12345'))).toBe(true);
    ...
    it('31 位纯 hex senderOpenId 只违反长度约束时显示 8 位片段并告警', async () => {
        ...
        const env = mockHandleInbound.mock.calls[0][0];
        // 校验不通过的 openid 显示 8 位片段（截断后加 …），而非完整 openid
        expect(env['text']).toBe('[atMention=true] [Bob(ABCDEF01…)]: <@OPENID_BOT> 你好');
        const calls = stderrSpy.mock.calls.map((c) => String(c[0]));
        expect(calls.some((c) => c.includes('Unexpected senderOpenId format') &&
            c.includes('ABCDEF0123456789ABCDEF012345678'))).toBe(true);
        stderrSpy.mockRestore();
    });
    ...
    it('caps malformed-sender warning keys at 64 Unicode code points', async () => {
        ...
        await vi.advanceTimersByTimeAsync(600);
        const warningKeys = pvt['warnedSenderOpenIds'];
    ...
    it('allowMention=false 时同昵称不同发送者用前 8 位消歧', async () => {
        ...
        await vi.advanceTimersByTimeAsync(600);
        expect(mockHandleInbound).toHaveBeenCalledTimes(2);
        const texts = mockHandleInbound.mock.calls.map((c) => c[0]['text']);
    ...
    it('allowMention=false 时非法 senderOpenId 仍显示 8 位片段且不告警', async () => {
        ...
        const calls = stderrSpy.mock.calls.map((c) => String(c[0]));
        expect(calls.some((c) => c.includes('Unexpected senderOpenId format'))).toBe(false);
        stderrSpy.mockRestore();
    });
    ...
    it('uses legacy author.id as the positional identity tag', async () => {
        ...
        expect(env['text']).toBe('[atMention=true] [Eve(0123456789ABCDEF0123456789ABCDEF)(legacy-u…)]: hello');
    });
    it('uses a legacy-only author id as identity without an OPENID warning', async () => {
        ...
        expect(stderrSpy.mock.calls.some(([message]) => String(message).includes('Unexpected senderOpenId format'))).toBe(false);
        stderrSpy.mockRestore();
    });
...
describe('handleGroupAll', () => {
    ...
    it('policy=keyword 时只有匹配关键词才触发', async () => {
        ...
        expect(mockHandleInbound).not.toHaveBeenCalled();
        mockHandleInbound.mockClear();
        pvt['handleGroupAll'](makeGroupAllEvent({ id: 'msg-002', content: '我需要 HELP' }));
        await vi.advanceTimersByTimeAsync(600);
        expect(mockHandleInbound).toHaveBeenCalledTimes(1);
    });
    ...
    it('policy=keyword word-boundary: 不含关键词子串不触发 (e.g. "helpful" ≠ "help")', async () => {
        ...
        const pvt = ch;
        pvt['handleGroupAll'](makeGroupAllEvent({ content: 'this is helpful info' }));
        await vi.advanceTimersByTimeAsync(600);
        expect(mockHandleInbound).not.toHaveBeenCalled();
    });
    ...
    it('isAtBot=false 时的 text 格式正确', async () => {
        const ch = makeChannel({ groupAllPolicy: 'all' });
        const pvt = ch;
        pvt['handleGroupAll'](makeGroupAllEvent({ content: 'hello world', mentions: [] }));
        await vi.advanceTimersByTimeAsync(600);
        const env = mockHandleInbound.mock.calls[0][0];
        // isAtBot=false → no botOpenId, no suffix, but senderOpenId displayed
        expect(env['text']).toBe('[atMention=false] [Charlie(FEDCBA9876543210FEDCBA9876543210)]: hello world');
    });
...
describe('群管理事件', () => {
    ...
    describe('handleGroupDelRobot', () => {
        ...
        it('清理 cron buffer/timer（cron-msg-experimental 启用）', () => {
            ...
            const streamState = ch['streamState'];
            const cronBuffer = ch['cronBuffer'];
    ...
    describe('sendIdentify intent subscription', () => {
        it('includes GROUP_MESSAGE intent when groupAllPolicy=keyword', () => {
            ...
            expect(parsed.d.intents & Intent.GROUP_MESSAGE).toBe(Intent.GROUP_MESSAGE);
        });
    ...
    describe('INVALID_SESSION recovery', () => {
        it('sets readyTimeout on re-IDENTIFY and triggers ws.close(4002) + connectReject if READY never arrives', () => {
            ...
            expect(connectReject).toHaveBeenCalledWith(new Error('Timed out waiting for READY'));
        ...
        it('readyTimeout does not fire if READY arrives before 30s', () => {
            ...
            pvt['handleGatewayMessage']({ op: 0, t: 'READY', s: 1, d: { session_id: 'sess-new' } }, () => { });
...
describe('Gateway message handling', () => {
    ...
    it('INVALID_SESSION flushes state and sends re-IDENTIFY', () => {
        ...
        expect(wsSend).toHaveBeenCalled();
        const sent = wsSend.mock.calls[0][0];
    ...
    it('READY cold start: calls restoreQQState and restoreSessions', async () => {
        ...
        const restoreQQSpy = vi
            .spyOn(ch, 'restoreQQState')
            .mockReturnValue(true);
        const restoreSessionsSpy = vi
            .spyOn(chp['router'], 'restoreSessions')
            .mockResolvedValue(undefined);
        await pvt['handleGatewayMessage']({ op: 0, t: 'READY', s: 1, d: { session_id: 'sess-cold' } }, () => { });
    ...
    it('READY warm reconnect: skips restore but calls finalizeReady', async () => {
        ...
        const restoreSessionsSpy = vi
            .spyOn(chp['router'], 'restoreSessions')
            .mockResolvedValue(undefined);
        await pvt['handleGatewayMessage']({ op: 0, t: 'READY', s: 1, d: { session_id: 'sess-warm' } }, () => { });
...
describe('Token refresh exhaustion', () => {
    it('calls disconnect after 10 consecutive token refresh failures', async () => {
        ...
        const disconnectSpy = vi.spyOn(ch, 'disconnect');
        // Spy on reconnectWithRetry
        const reconnectSpy = vi
            .spyOn(ch, 'reconnectWithRetry')
            .mockResolvedValue(undefined);
    ...
    it('stale _reconnectId discards token refresh retry callbacks', async () => {
        ...
        chp['tokenExpiresAt'] = Date.now() + 20_000;
        chp['accessToken'] = 'test-token';
        const originalReconnectId = chp['_reconnectId'];
        mockFetchAccessToken.mockRejectedValue(new Error('token endpoint down'));
        // Spy on disconnect
        const disconnectSpy = vi.spyOn(ch, 'disconnect');
        chp['scheduleTokenRefresh'].call(ch);
...
describe('resolveRoute chatTypes fallback', () => {
    it('uses chatTypes config when chatTypeMap has no entry', async () => {
        ...
        chp['tokenExpiresAt'] = Date.now() + 3600_000;
        const pvt = ch;
        const result = await pvt['resolveRoute']('config-chat-id');
    ...
    it('prefers chatTypeMap over chatTypes config', async () => {
        ...
        chp['accessToken'] = 'test-token';
        chp['tokenExpiresAt'] = Date.now() + 3600_000;
        chp['chatTypeMap'].set('dual-chat-id', 'group');
        const pvt = ch;
        const result = await pvt['resolveRoute']('dual-chat-id');
        expect(result).not.toBeNull();
    ...
    it('returns null when neither chatTypeMap nor chatTypes has entry', async () => {
        ...
        chp['tokenExpiresAt'] = Date.now() + 3600_000;
        const pvt = ch;
        const result = await pvt['resolveRoute']('unknown-chat');
...
describe('extractBotOpenId', () => {
    it('returns empty string when no self-mention found', () => {
        const ch = makeChannel();
        const pvt = ch;
        const result = pvt['extractBotOpenId']([], 'group-1');
        expect(result).toBe('');
    });
    it('returns empty string for invalid botOpenId format', () => {
        const ch = makeChannel();
        const pvt = ch;
        const result = pvt['extractBotOpenId']([
            {
    ...
    it('returns botOpenId and saves to botOpenIdByGroup when valid', () => {
        ...
        const chp = ch;
        const pvt = ch;
        const botOpenIdByGroup = chp['botOpenIdByGroup'];
        const result = pvt['extractBotOpenId']([
            {
                member_openid: 'ABCDEF0123456789ABCDEF0123456789',
                is_you: true,
        ...
        expect(botOpenIdByGroup.get('group-openid-1')).toBe('ABCDEF0123456789ABCDEF0123456789');
    });
    ...
    describe('WS close handler abnormal disconnect', () => {
        it('sets coldStart=true and tryResume=false on abnormal WS close (code 4009 session timeout)', () => {
            ...
            chp['dialGateway']('wss://gateway.test', vi.fn(), vi.fn());
            const ws = chp['ws'];
            ws.emit('close', 4009);
            expect(chp['tryResume']).toBe(false);
            expect(chp['coldStart']).toBe(true);
