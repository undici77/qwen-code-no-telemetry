...
class TestTelegramChannel extends TelegramChannel {
    ...
    buildTestEnvelope(msg, text, entities) {
        return this.buildEnvelope(msg, text, entities);
    }
    pushTestProactive(target, text) {
        return this.pushProactive({ channelName: 'telegram', senderId: '1', ...target }, text);
    }
    sendTestResponse(chatId, text, sessionId) {
        return this.sendResponseMessage(chatId, text, sessionId);
    }
    sendTestResponseFromThread(threadId, chatId, text, sessionId) {
        const inboundRoute = this.inboundRoute;
        const route = threadId === undefined ? {} : { threadId };
        return inboundRoute.run(route, () => this.sendResponseMessage(chatId, text, sessionId));
    }
}
const config = {
    type: 'telegram',
...
function createChannel(configOverrides = {}, router = {}) {
    return new TestTelegramChannel('telegram', { ...config, ...configOverrides }, {}, {
        router: router,
    });
}
...
function installFakeBot(channel) {
    const bot = {
        token: 'token',
        api: {
...
describe('TelegramChannel', () => {
    ...
    it('registers the Telegram command menu before polling starts', async () => {
        ...
        expect(bot.api.setMyCommands.mock.invocationCallOrder[0]).toBeLessThan(bot.start.mock.invocationCallOrder[0]);
    ...
    it('continues startup when Telegram command menu registration fails', async () => {
        ...
        await channel.connect();
        expect(bot.start).toHaveBeenCalledWith({ drop_pending_updates: true });
        expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to register bot commands'));
    });
    it('handles /start locally', async () => {
        const channel = createChannel();
        const bot = installFakeBot(channel);
        await channel.handleInbound(envelope({ text: '/start' }));
        expect(bot.api.sendMessage).toHaveBeenCalledWith('chat-1', expect.stringContaining('Qwen Code Telegram bot'), { parse_mode: 'HTML' });
    });
    it('sends command replies back to the Telegram forum topic', async () => {
        ...
        expect(bot.api.sendMessage).toHaveBeenCalledWith('2', expect.stringContaining('Qwen Code Telegram bot'), { parse_mode: 'HTML', message_thread_id: 42 });
    });
    ...
    it('prefers the current inbound topic over a stale session route', async () => {
        ...
        const channel = createChannel({}, router);
        const bot = installFakeBot(channel);
        await channel.sendTestResponseFromThread('43', '2', 'new topic response', 'session-1');
        await channel.sendTestResponseFromThread(undefined, '2', 'general response', 'session-1');
        expect(bot.api.sendMessage).toHaveBeenNthCalledWith(1, '2', expect.any(String), { parse_mode: 'HTML', message_thread_id: 43 });
        expect(bot.api.sendMessage).toHaveBeenNthCalledWith(2, '2', expect.any(String), { parse_mode: 'HTML' });
    });
    it('only treats addressed Telegram bot commands as mentions in groups', () => {
        const channel = createChannel();
        installFakeBot(channel);
        channel.botUsername = 'qwen_bot';
        const directCommand = channel.buildTestEnvelope({
            from: { id: 1, first_name: 'User' },
            chat: { id: 2, type: 'group' },
        }, '/cancel', [{ type: 'bot_command', offset: 0, length: 7 }]);
        const addressedCommand = channel.buildTestEnvelope({
            from: { id: 1, first_name: 'User' },
            chat: { id: 2, type: 'group' },
        }, '/cancel@qwen_bot', [{ type: 'bot_command', offset: 0, length: 16 }]);
        const otherBotCommand = channel.buildTestEnvelope({
            from: { id: 1, first_name: 'User' },
            chat: { id: 2, type: 'group' },
        }, '/cancel@other_bot', [{ type: 'bot_command', offset: 0, length: 17 }]);
    ...
    it('sends proactive messages back to the Telegram forum topic', async () => {
        const channel = createChannel();
        const bot = installFakeBot(channel);
        await channel.pushTestProactive({ chatId: '2', threadId: '42' }, 'topic response');
        expect(bot.api.sendMessage).toHaveBeenCalledWith('2', expect.any(String), {
            parse_mode: 'HTML',
            message_thread_id: 42,
        });
    ...
    it('does not let bare bot commands pass mention-gated groups', async () => {
        ...
        await channel.handleInbound(channel.buildTestEnvelope(groupMessage, '/cancel', [
            { type: 'bot_command', offset: 0, length: 7 },
        ]));
        expect(router.getSession).not.toHaveBeenCalled();
        expect(bot.api.sendMessage).not.toHaveBeenCalled();
        await channel.handleInbound(channel.buildTestEnvelope(groupMessage, '/cancel@qwen_bot', [
            { type: 'bot_command', offset: 0, length: 16 },
        ]));
        expect(router.getSession).toHaveBeenCalledWith('telegram', '1', '2', undefined);
        expect(bot.api.sendMessage).toHaveBeenCalledWith('2', 'No request is currently running.', { parse_mode: 'HTML' });
    });
});
