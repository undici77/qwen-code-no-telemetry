import { chmodSync, constants, existsSync, mkdirSync, mkdtempSync, readdirSync, realpathSync, rmSync, writeFileSync, } from 'node:fs';
...
const mocks = vi.hoisted(() => {
    ...
    const decryptFile = vi.fn((buffer, _aesKey) => buffer);
    const open = vi.fn(async (_path, _flags) => {
        const handle = {
    ...
    const readFile = vi.fn(async (_path) => Buffer.from([0x89, 0x50, 0x4e, 0x47]));
    const writeFile = vi.fn(async (_path, _data, _options) => { });
    const httpResponse = {
        statusCode: 200,
        headers: {},
        body: Buffer.from('downloaded'),
    };
    const httpsRequest = vi.fn((_url, _options, callback) => {
            ...
            on: vi.fn(() => request),
                ...
                queueMicrotask(() => {
                    ...
                    queueMicrotask(() => {
                        ...
                        if (state.mediaResponseNeverEnds)
                            return;
                        emit('end');
                    });
                });
    ...
    class MockWSClient {
        ...
        connect = vi.fn(() => {
            ...
            if (state.kickAfterConnectsRemaining > 0) {
                ...
                queueMicrotask(() => this.emit('event.disconnected_event', 'kicked again'));
            }
            return this;
        });
        disconnect = vi.fn();
        sendMessage = vi.fn(async (_chatId, _message) => ({
            headers: { req_id: 'req-1' },
        }));
        uploadMedia = vi.fn(async (_data, _options) => ({
            media_id: 'media-1',
        }));
        sendMediaMessage = vi.fn(async (_chatId, _mediaType, _mediaId) => ({
            headers: { req_id: 'media-req-1' },
        }));
        downloadFile = vi.fn(async (_url, _aesKey) => ({
...
class FailingProcessWeComChannel extends WeComChannel {
    processes = vi.fn(async (_envelope) => {
...
class BlockingAliceProcessWeComChannel extends WeComChannel {
    releaseAliceProcess;
    aliceBlocked = new Promise((resolve) => {
...
function lastClient() {
    ...
    if (!client)
        throw new Error('missing mock client');
    return client;
}
function channelFileDirs() {
...
describe('WeComChannel', () => {
    ...
    beforeEach(() => {
        ...
        vi.clearAllMocks();
        mocks.decryptFile.mockImplementation((buffer) => buffer);
        mocks.readFile.mockResolvedValue(Buffer.from([0x89, 0x50, 0x4e, 0x47]));
        mocks.writeFile.mockImplementation(async (path, data, _options) => {
    ...
    it('requires botId and secret', () => {
        expect(() => new WeComChannel('bot', makeConfig({ botId: '' }), makeBridge())).toThrow('requires botId and secret');
        expect(() => new WeComChannel('bot', makeConfig({ secret: '' }), makeBridge())).toThrow('requires botId and secret');
        expect(() => new WeComChannel('bot', makeConfig({ wsUrl: 'ws://example.invalid/ws' }), makeBridge())).toThrow('requires wsUrl to use wss://');
    });
    ...
    it('connects the official SDK with bot credentials', async () => {
        ...
        const channel = new WeComChannel('bot', makeConfig({ wsUrl: 'wss://example.invalid/ws' }), makeBridge());
        ...
        const logger = client.options['logger'];
        ...
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] SDK warn: No aesKey provided:\n');
        expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining('https://example.invalid/file'));
        client.emit('error', {
            config: {
        ...
        expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining('nested-token'));
        expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining('nested-secret'));
        stderr.mockRestore();
    });
    it('waits for SDK authentication before reporting connected', async () => {
        ...
        expect(stderr).not.toHaveBeenCalledWith('[WeCom:bot] Connected via smart bot.\n');
        client.emit('authenticated', {});
        await connecting;
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] Connected via smart bot.\n');
        stderr.mockRestore();
    });
    ...
    it('drops messages received before authentication completes', async () => {
        ...
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] dropping message before authentication.\n');
        client.emit('authenticated', {});
        await connecting;
        stderr.mockRestore();
    });
    it('keeps the active SDK client when the websocket disconnects', async () => {
        ...
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] WebSocket closed; waiting for SDK reconnect.\n');
    ...
    it('preserves structured websocket disconnect reasons in logs', async () => {
        ...
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('code=1006 reason=abnormal closure wasClean=false'));
        stderr.mockRestore();
    });
    it('redacts sensitive fields in structured websocket disconnect reasons', async () => {
        ...
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('[REDACTED]'));
        expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining('nested-secret'));
        expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining('token-value'));
        stderr.mockRestore();
    });
    ...
    it('logs when kick reconnect is superseded before retrying', async () => {
        ...
        const channel = new WeComChannel('bot', makeConfig(), makeBridge());
        await channel.connect();
        const inspectable = channel;
        lastClient().emit('event.disconnected_event', 'kicked');
        inspectable.disconnectGeneration += 1;
        await vi.advanceTimersByTimeAsync(1_000);
        await vi.waitFor(() => expect(stderr).toHaveBeenCalledWith('[WeCom:bot] reconnect after server kick abandoned: connection generation changed.\n'));
    ...
    it('keeps retrying later after kick reconnect attempts are exhausted', async () => {
        ...
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] reconnect after server kick gave up after 3 attempts; retrying later.\n');
    ...
    it('schedules a long retry when kick reconnect fails unexpectedly', async () => {
        ...
        await vi.waitFor(() => expect(inspectable.kickReconnectRetry).toBeDefined());
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] kick-reconnect failed: state corrupt\n');
        stderr.mockRestore();
    });
    it('resets retry cycles for long retry after unexpected kick reconnect failure', async () => {
        ...
        inspectable.startKickReconnect('kicked', 'server kick');
        await vi.waitFor(() => expect(stderr).toHaveBeenCalledWith('[WeCom:bot] kick-reconnect failed: state corrupt\n'));
    ...
    it('falls back to adapter reconnect when SDK disconnect stalls', async () => {
        ...
        await vi.waitFor(() => expect(mocks.instances).toHaveLength(2));
        expect(oldClient.disconnect).toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] SDK reconnect did not recover after WebSocket closed; reconnecting adapter.\n');
        channel.disconnect();
        stderr.mockRestore();
    });
    it('reconnects when the SDK stays silent past the activity watchdog', async () => {
        ...
        await vi.waitFor(() => expect(mocks.instances).toHaveLength(2));
        expect(oldClient.disconnect).toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] no SDK activity for 5 minutes; reconnecting adapter.\n');
        channel.disconnect();
        stderr.mockRestore();
    });
    ...
    it('keeps kick reconnect alive when SDK disconnect throws', async () => {
        ...
        await vi.advanceTimersByTimeAsync(1_000);
        await vi.waitFor(() => expect(mocks.instances).toHaveLength(2));
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] client.disconnect() threw: socket already closed\n');
        channel.disconnect();
        stderr.mockRestore();
    });
    ...
    it('continues kick reconnect after repeated retry cycles are exhausted', async () => {
        ...
        for (let cycle = 0; cycle < 3; cycle += 1) {
            ...
            await vi.advanceTimersByTimeAsync(2_000);
            await vi.advanceTimersByTimeAsync(4_000);
            await vi.waitFor(() => expect(mocks.instances).toHaveLength(4 + cycle * 3));
            if (cycle < 2) {
                await vi.advanceTimersByTimeAsync(5 * 60 * 1000);
            }
        }
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] reconnect after server kick exhausted 3 retry cycles; next attempt in 15 minutes.\n');
        await vi.advanceTimersByTimeAsync(15 * 60 * 1000);
        await vi.advanceTimersByTimeAsync(1_000);
    ...
    it('resets the kick reconnect attempt budget after a successful reconnect', async () => {
        ...
        for (let kick = 0; kick < 4; kick += 1) {
            ...
            await vi.waitFor(() => expect(channel.reconnectingAfterKick).toBe(false));
        }
        channel.disconnect();
    });
    it('retries failed kick reconnect attempts with exponential backoff', async () => {
        ...
        await vi.advanceTimersByTimeAsync(4_000);
        await vi.waitFor(() => expect(mocks.instances).toHaveLength(4));
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('reconnect after server kick attempt 1 failed'));
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('reconnect after server kick attempt 2 failed'));
        expect(lastClient().connect).toHaveBeenCalledTimes(1);
        channel.disconnect();
        stderr.mockRestore();
    });
    ...
    it('clears pending kick reconnect state on disconnect', async () => {
        const channel = new WeComChannel('bot', makeConfig(), makeBridge());
        await channel.connect();
        channel.pendingKickReconnect = true;
        channel.disconnect();
        expect(channel.pendingKickReconnect).toBe(false);
    });
    ...
    it('does not schedule reconnect reset after disconnect races with connect success', async () => {
        ...
        expect(channel.kickReconnectReset).toBeUndefined();
    });
    ...
    it('does not clear adapter state when reconnecting after a kick', async () => {
        ...
        bridge.prompt.mockImplementationOnce(() => new Promise(() => { }));
    ...
    it('fails pending authentication promptly on SDK errors', async () => {
        ...
        await expect(connecting).rejects.toThrow('WeCom authentication failed: Error: auth failed');
        stderr.mockRestore();
    });
    it('fails pending authentication when the timeout elapses', async () => {
        ...
        const channel = new WeComChannel('bot', makeConfig(), makeBridge());
        const connecting = channel.connect();
        const rejection = expect(connecting).rejects.toThrow('WeCom authentication timed out.');
        await vi.advanceTimersByTimeAsync(30_000);
        await rejection;
    });
    ...
    it('rejects sends when no SDK client is active', async () => {
        const channel = new WeComChannel('bot', makeConfig(), makeBridge());
        await expect(channel.sendMessage('chat-1', 'hello')).rejects.toThrow('[WeCom:bot] No active SDK client, cannot send.');
    });
    ...
    it('logs malformed message payloads without processing them', async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        expect(channel.envelopes).toHaveLength(0);
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] dropping message with unrecognized payload structure.\n');
        stderr.mockRestore();
    });
    it('normalizes group, voice, mixed, quote, and file messages', async () => {
        const channel = new TestWeComChannel('bot', makeConfig({
            groupPolicy: 'open',
            groups: { '*': {} },
        }), makeBridge());
        await channel.connect();
        ...
        expect(mocks.httpsRequest).toHaveBeenCalledWith('https://example.invalid/image', expect.objectContaining({ method: 'GET' }), expect.any(Function));
        expect(mocks.httpsRequest).toHaveBeenCalledWith('https://example.invalid/file', expect.objectContaining({ method: 'GET' }), expect.any(Function));
    ...
    it('preserves distinct unicode inbound filenames', async () => {
        ...
        expect(new Set(attachments.map((attachment) => attachment.filePath)).size).toBe(2);
    });
    ...
    it('does not download attachments for messages rejected by group policy', async () => {
        const channel = new TestWeComChannel('bot', makeConfig({ groupPolicy: 'disabled' }), makeBridge());
        await channel.connect();
        const client = lastClient();
        client.emit('message.image', {
    ...
    it('drops malformed group messages without falling back to sender chat', async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        expect(channel.envelopes).toHaveLength(0);
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('missing chatId'));
        stderr.mockRestore();
    });
    it('does not create a session for local commands before base dispatch', async () => {
        ...
        expect(client.sendMessage).toHaveBeenCalledWith('alice', expect.objectContaining({
    ...
    it('does not create a session for refused group shell commands', async () => {
        const bridge = makeBridge();
        const channel = new WeComChannel('bot', makeConfig({ groupPolicy: 'open', groups: { '*': {} } }), bridge);
        await channel.connect();
        const client = lastClient();
        client.emit('message.text', {
            msgid: 'msg-shell',
        ...
        expect(bridge.newSession).not.toHaveBeenCalled();
        expect(bridge.prompt).not.toHaveBeenCalled();
        expect(client.sendMessage).toHaveBeenCalledWith('group-1', expect.objectContaining({
            markdown: expect.objectContaining({
    ...
    it('rolls back message dedup when preflight work fails', async () => {
        const stderr = vi
            .spyOn(process.stderr, 'write')
            .mockImplementation(() => true);
        const channel = new FailingPreflightWeComChannel('bot', makeConfig(), makeBridge());
        await channel.connect();
        const client = lastClient();
        const payload = {
    ...
    it('allows messages rejected by preflight to be retried', async () => {
        const stderr = vi
            ...
            .mockImplementation(() => true);
        const bridge = makeBridge();
        const channel = new RejectingPreflightWeComChannel('bot', makeConfig(), bridge);
        await channel.connect();
        const client = lastClient();
        const payload = {
        ...
        await vi.waitFor(() => expect(channel.preflights).toHaveBeenCalledTimes(2));
        await vi.waitFor(() => {
            const rejectLogs = stderr.mock.calls.filter(([message]) => String(message).includes('preflight rejected'));
    ...
    it('allows messages interrupted during attachment download to be retried', async () => {
        ...
        const client = lastClient();
        let releaseLookup;
        mocks.lookup.mockImplementationOnce(async () => await new Promise((resolve) => {
        ...
        await vi.waitFor(() => expect(stderr).toHaveBeenCalledWith('[WeCom:bot] dropping message msg-download-interrupted: connection changed during attachment download.\n'));
        expect(channel.envelopes).toHaveLength(0);
        channel.disconnectClientOnly(new Error('test reconnect'));
        await channel.connect();
    ...
    it('deduplicates repeated messages while preflight is pending', async () => {
        const channel = new BlockingPreflightWeComChannel('bot', makeConfig(), makeBridge());
        await channel.connect();
        const client = lastClient();
        const payload = {
    ...
    it('keeps message dedup when processing fails after side effects start', async () => {
        const stderr = vi
            .spyOn(process.stderr, 'write')
            .mockImplementation(() => true);
        const channel = new FailingProcessWeComChannel('bot', makeConfig(), makeBridge());
        await channel.connect();
        const client = lastClient();
        const payload = {
        ...
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] dropping duplicate message msg-process-fails (already seen).\n');
        stderr.mockRestore();
    });
    it('logs duplicate messages that are already in flight', async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        expect(channel.preflights).toHaveBeenCalledTimes(1);
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] dropping duplicate message msg-in-flight (already in flight).\n');
        channel.releasePreflights();
        stderr.mockRestore();
    });
    it('does not download attachments from unsafe media URLs', async () => {
        ...
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('unsafe media URL'));
        stderr.mockRestore();
    });
    ...
    it('blocks media URLs with embedded credentials before probing them', async () => {
        ...
        expect(mocks.lookup).not.toHaveBeenCalled();
        expect(mocks.httpsRequest).not.toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('URL contains embedded credentials'));
        stderr.mockRestore();
    });
    ...
    it('blocks non-canonical IPv6 unspecified addresses before probing them', async () => {
        ...
        expect(channel.envelopes[0]?.attachments).toBeUndefined();
        expect(mocks.httpsRequest).not.toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('unsafe media URL (private address ::)'));
        stderr.mockRestore();
    });
    ...
    it('blocks media hostnames that resolve to private addresses', async () => {
        ...
        expect(client.downloadFile).not.toHaveBeenCalled();
        expect(mocks.httpsRequest).not.toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('unsafe media URL (metadata.example.com resolved to private address 169.254.169.254)'));
        stderr.mockRestore();
    });
    it('includes DNS lookup errors when rejecting media hostnames', async () => {
        ...
        expect(client.downloadFile).not.toHaveBeenCalled();
        expect(mocks.httpsRequest).not.toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('unsafe media URL (DNS lookup failed for metadata.example.com: queryA ETIMEDOUT)'));
        stderr.mockRestore();
    });
    ...
    it('returns all resolved addresses when Node requests lookup all mode', async () => {
        ...
        const options = mocks.httpCalls[0]?.options;
    ...
    it('does not follow redirects while probing inbound media', async () => {
        ...
        await vi.waitFor(() => expect(channel.envelopes).toHaveLength(1));
        expect(mocks.httpsRequest).toHaveBeenCalledWith('https://example.invalid/redirect', expect.objectContaining({
            method: 'GET',
            lookup: expect.any(Function),
        }), expect.any(Function));
        expect(mocks.httpCalls[0]?.request.setTimeout).toHaveBeenCalledWith(10_000, expect.any(Function));
        expect(mocks.httpCalls[0]?.request.destroy).toHaveBeenCalled();
        expect(client.downloadFile).not.toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('redirected media URL'));
        stderr.mockRestore();
    });
    it('skips inbound attachments that exceed the media size cap', async () => {
        ...
        expect(mocks.httpCalls[0]?.request.destroy).toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('oversized attachment'));
        expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining('https://example.invalid/large-image'));
        stderr.mockRestore();
    });
    ...
    it('aborts inbound attachments on absolute download timeout', async () => {
        ...
        expect(channel.envelopes[0]?.attachments).toBeUndefined();
        expect(mocks.httpCalls[0]?.request.destroy).toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('media download absolute timeout'));
        stderr.mockRestore();
    });
    it('skips inbound attachments when the media request returns non-success', async () => {
        ...
        expect(channel.envelopes[0]?.attachments).toBeUndefined();
        expect(mocks.httpCalls[0]?.request.destroy).toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('media download failed: HTTP 500'));
        stderr.mockRestore();
    });
    it('destroys the media request when the response stream errors', async () => {
        ...
        expect(channel.envelopes[0]?.attachments).toBeUndefined();
        expect(mocks.httpCalls[0]?.request.destroy).toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('skipping image attachment: response failed'));
        stderr.mockRestore();
    });
    it('skips inbound attachments when the media response is empty', async () => {
        ...
        await vi.waitFor(() => expect(channel.envelopes).toHaveLength(1));
        expect(channel.envelopes[0]?.attachments).toBeUndefined();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('empty media response'));
        stderr.mockRestore();
    });
    it('rejects private addresses during request-time lookup validation', async () => {
        ...
        expect(callback.mock.calls[0]?.[0]).toEqual(new Error('unsafe resolved media address: example.invalid resolved to private address 169.254.169.254'));
    ...
    it('rejects uncompressed mapped private addresses during request-time lookup validation', async () => {
        ...
        const callback = vi.fn();
        options.lookup?.('example.invalid', { all: true }, callback);
        await vi.waitFor(() => expect(callback).toHaveBeenCalledWith(expect.any(Error), '', 0));
    });
    ...
    it('stores inbound file attachments in private temp files asynchronously', async () => {
        ...
        expect(filePath).toBeDefined();
        expect(mocks.writeFile).toHaveBeenCalledWith(filePath, Buffer.from('downloaded'), { mode: 0o600 });
    });
    it('skips inbound file attachments when temp file writing fails', async () => {
        ...
        try {
            ...
            await vi.waitFor(() => expect(channel.envelopes).toHaveLength(1));
            expect(channel.envelopes[0]?.attachments).toBeUndefined();
            expect(stderr).toHaveBeenCalledWith(expect.stringContaining('skipping file attachment: ENAMETOOLONG'));
        }
        finally {
            stderr.mockRestore();
        }
    ...
    it('removes file attachment dirs created during disconnect', async () => {
        let finishWrite;
        mocks.writeFile.mockImplementationOnce(() => new Promise((resolve) => {
            finishWrite = resolve;
        }));
        const bridge = makeBridge();
        const channel = new WeComChannel('bot', makeConfig(), bridge);
        ...
        const filePath = mocks.writeFile.mock.calls[0]?.[0];
    ...
    it('allows public IPv4 addresses adjacent to documentation ranges', async () => {
        ...
        await vi.waitFor(() => expect(channel.envelopes).toHaveLength(1));
        expect(mocks.httpsRequest).toHaveBeenCalledWith('https://192.0.32.1/image', expect.objectContaining({ method: 'GET' }), expect.any(Function));
    ...
    it.skipIf(process.platform === 'win32')('continues attachment cleanup when one dir removal fails', async () => {
        const parent = join(tmpdir(), 'channel-files');
        ...
        try {
            ...
            expect(stderr).toHaveBeenCalledWith(expect.stringContaining('failed to remove attachment dir'));
        }
        finally {
            stderr.mockRestore();
    ...
    it('removes session attachment dirs when prompt end has no message id', async () => {
        ...
        await vi.waitFor(() => expect(existsSync(dirname(attachmentPath))).toBe(false));
    });
    ...
    it('keeps no-message-id attachment dirs scoped to their session while another session completes', async () => {
        const bridge = makeBridge();
        const channel = new BlockingAliceProcessWeComChannel('bot', makeConfig(), bridge);
        await channel.connect();
        const client = lastClient();
        client.emit('message.file', {
            msgtype: 'file',
        ...
        const aliceDir = channelFileDirs().find((dir) => existsSync(join(dir, 'alice.txt')));
    ...
    it('sanitizes message ids before writing drop logs', async () => {
        ...
        await vi.waitFor(() => expect(stderr).toHaveBeenCalledWith('[WeCom:bot] dropping message msg-1\\nfake log: missing senderId.\n'));
        stderr.mockRestore();
    });
    it('cleans untracked session dirs without scanning every message dir', () => {
        ...
        const inspectable = channel;
        const messageDirs = new Map([['msg-1', [trackedDir]]]);
    ...
    it('removes downloaded file attachments when no prompt starts', async () => {
        ...
        await vi.waitFor(() => expect(client.sendMessage).toHaveBeenCalledWith('alice', expect.objectContaining({ msgtype: 'markdown' })));
        expect(bridge.prompt).not.toHaveBeenCalled();
        await vi.waitFor(() => expect(channelFileDirs()).toHaveLength(0));
    });
    it('removes every collected file attachment after the coalesced prompt finishes', async () => {
        ...
        const channel = new WeComChannel('bot', makeConfig({ dispatchMode: 'collect' }), bridge);
        await channel.connect();
        const client = lastClient();
        client.emit('message.file', {
            msgid: 'msg-active',
    ...
    it('keeps buffered attachment files until their coalesced prompt runs', async () => {
        ...
        const bufferedDir = channelFileDirs().find((dir) => existsSync(join(dir, 'buffered.txt')));
    ...
    it('tracks attachment dirs for messages without msgid by synthetic message id', async () => {
        const bridge = makeBridge();
        bridge.prompt.mockImplementation(() => new Promise(() => { }));
        const channel = new WeComChannel('bot', makeConfig(), bridge);
        await channel.connect();
        ...
        expect(filePath).toBeDefined();
        expect(existsSync(filePath)).toBe(true);
        expect(Array.from(channel.attachmentDirsByMessage.keys()).some((messageId) => messageId.startsWith('synthetic-'))).toBe(true);
        expect(channel.attachmentDirsWithoutMessageByRoute.size).toBe(0);
        channel.disconnect();
    });
    it('sends markdown text and local media through the SDK', async () => {
        ...
        await channel.connect();
        const client = lastClient();
        await channel.sendMessage('chat-1', `result\n[IMAGE: ${imagePath}]\n\n\`[IMAGE: /tmp/example.png]\``);
        expect(client.sendMessage).toHaveBeenCalledWith('chat-1', {
            msgtype: 'markdown',
        ...
        expect(client.sendMediaMessage).toHaveBeenCalledWith('chat-1', 'image', 'media-1');
    });
    ...
    it('splits long markdown responses before sending', async () => {
        ...
        const first = client.sendMessage.mock.calls[0]?.[1];
        const second = client.sendMessage.mock.calls[1]?.[1];
        expect(Buffer.byteLength(first.markdown.content, 'utf8')).toBeLessThan(4096);
        expect(Buffer.byteLength(second.markdown.content, 'utf8')).toBeLessThan(4096);
        expect(first.markdown.content + second.markdown.content).toBe('a'.repeat(3900));
    });
    it('splits long markdown responses without array-copying the remaining line', async () => {
        ...
        await channel.connect();
        await channel.sendMessage('chat-1', 'a'.repeat(3900));
        expect(arrayFrom.mock.calls.some(([value]) => typeof value === 'string' && value.length > 100)).toBe(false);
    ...
    it('resolves relative outbound image paths from channel cwd', async () => {
        ...
        const channel = new WeComChannel('bot', makeConfig({ cwd: dir }), makeBridge());
        await channel.connect();
        const client = lastClient();
        await channel.sendMessage('chat-1', '[IMAGE: out.png]');
        expect(mocks.open).toHaveBeenCalledWith(realpathSync(join(dir, 'out.png')), constants.O_RDONLY | constants.O_NOFOLLOW);
    ...
    it('logs and sends later media when one upload returns no media id', async () => {
        ...
        mkdirSync(parent, { recursive: true });
        const dir = mkdtempSync(join(parent, 'wecom-test-'));
        writeFileSync(join(dir, 'first.png'), Buffer.from([0x89, 0x50, 0x4e, 0x47]));
        writeFileSync(join(dir, 'second.png'), Buffer.from([0x89, 0x50, 0x4e, 0x47]));
        const channel = new WeComChannel('bot', makeConfig({ cwd: dir }), makeBridge());
        await channel.connect();
        const client = lastClient();
        ...
        await channel.sendMessage('chat-1', '[IMAGE: first.png]\n[IMAGE: second.png]');
        expect(client.uploadMedia).toHaveBeenCalledTimes(2);
        expect(client.sendMediaMessage).toHaveBeenCalledTimes(1);
        expect(client.sendMediaMessage).toHaveBeenCalledWith('chat-1', 'image', 'media-2');
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('upload returned no media_id'));
        stderr.mockRestore();
    });
    it('logs and sends later media when one media send fails', async () => {
        ...
        expect(client.sendMediaMessage).toHaveBeenLastCalledWith('chat-1', 'image', 'media-2');
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('media send failed for image: errcode=45009 errmsg=api freq out of limit'));
        expect(stderr).toHaveBeenCalledWith('[WeCom:bot] 1 media send(s) failed (markdown text may already be delivered): image: errcode=45009 errmsg=api freq out of limit\n');
        stderr.mockRestore();
    });
    ...
    it('skips model-emitted image paths outside the channel file directory', async () => {
        ...
        expect(client.uploadMedia).not.toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('outside allowed outbound directory'));
        expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining(secretPath));
        stderr.mockRestore();
    });
    it('reports unsafe channel file directory setup failures', async () => {
        ...
        expect(client.uploadMedia).not.toHaveBeenCalled();
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('Cannot prepare outbound media directory'));
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('channel-files'));
        stderr.mockRestore();
    });
...
//# sourceMappingURL=WeComAdapter.test.js.map