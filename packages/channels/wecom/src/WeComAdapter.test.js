...
const mocks = vi.hoisted(() => {
  ...
  const readFile = vi.fn(async (_path) =>
...
describe('WeComChannel', () => {
  ...
  it('keeps files buffered during a coalesced prompt for the next prompt', async () => {
    ...
    const prompt = bridge.prompt.mock.calls[2][1];
  ...
  it('tracks attachment dirs for messages without msgid by synthetic message id', async () => {
    ...
    expect(existsSync(filePath)).toBe(true);
    expect(
      Array.from(channel.attachmentDirsByMessage.keys()).some((messageId) =>
        messageId.startsWith('synthetic-'),
      ),
    ).toBe(true);
