...
export class WeComChannel extends ChannelBase {
    ...
    attachmentDirsByMessage = new Map();
    attachmentMessageByDir = new Map();
    attachmentDirsBySession = new Map();
    attachmentDirsWithoutMessageByRoute = new Map();
    bufferedAttachmentMessages = new Set();
    coalescedAttachmentMessages = new Map();
    dedupTimer;
    kickReconnectReset;
    kickReconnectRetry;
    ...
    async openClient() {
        ...
        try {
            ...
            const connectedPromise = isPromiseLike(connected)
                ? withTimeout(Promise.resolve(connected).then(() => { }), AUTHENTICATION_TIMEOUT_MS, 'WeCom SDK connect timed out.')
                : Promise.resolve();
    ...
    startActivityWatchdog(disconnectGeneration) {
        ...
        this.activityWatchdog = setInterval(() => {
            ...
            if (Date.now() - this.lastActivityAt < ACTIVITY_STALE_MS)
                return;
            process.stderr.write(`[WeCom:${this.name}] no SDK activity for ${ACTIVITY_STALE_MS / 60_000} minutes; reconnecting adapter.\n`);
            this.kickReconnectAttempts = 0;
            this.kickReconnectRetryCycles = 0;
    ...
    async reconnectAfterKick(reason, reconnectReason = 'server kick') {
        ...
        try {
            ...
            if (this.kickReconnectRetryCycles >= KICK_RECONNECT_MAX_RETRY_CYCLES) {
                process.stderr.write(`[WeCom:${this.name}] reconnect after ${reconnectReason} exhausted ${this.kickReconnectRetryCycles} retry cycles; next attempt in ${KICK_RECONNECT_LONG_RETRY_MS / 60_000} minutes.\n`);
            ...
            const shouldRetryPendingKick = this.pendingKickReconnect &&
...
function waitForAuthentication(client) {
    ...
    let finish = () => { };
    const onAuth = () => finish();
    const onError = (err) => finish(new Error(`WeCom authentication failed: ${sanitizeLogText(String(err), 200)}`));
    const onKicked = (reason) => finish(new Error(`WeCom authentication interrupted by server kick: ${formatDisconnectReason(reason)}`));
...
function collectInboundMediaRefs(body, depth = 0, seenUrls = new Set()) {
    if (depth > 3)
        return [];
    const refs = [];
...
function splitMarkdownChunks(text) {
    ...
    let current = '';
    let codeFence;
    const fits = (value, nextCodeFence = codeFence) => Buffer.byteLength(nextCodeFence ? `${value}\n${nextCodeFence}` : value, 'utf8') <= MARKDOWN_CHUNK_BYTES;
    const flush = (closeCode = true) => {
        if (!current)
            return;
    ...
    for (const line of text.split('\n')) {
        ...
        for (let index = 0; index < line.length;) {
            ...
            if (!token)
                break;
            const nextCodeFence = token === codeFence
                ? undefined
...
const safePublicLookup = (hostname, options, callback) => {
    lookup(hostname, { all: true })
        .then((records) => {
        const unsafeRecord = records.find((record) => !isPublicIpAddress(record.address));
...
function isPublicIpAddress(address) {
    ...
    if (ipVersion === 6) {
        ...
        const hasLowZeroPrivateIpv4 = groups.slice(0, 5).every((group) => group === 0) &&
...
function extractMediaId(value) {
    ...
    return (getString(record, 'media_id') ||
...
//# sourceMappingURL=WeComAdapter.js.map