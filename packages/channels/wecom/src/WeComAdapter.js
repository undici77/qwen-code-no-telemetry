...
export class WeComChannel extends ChannelBase {
  ...
  async openClient() {
    ...
    const messageHandler = (payload) => {
      ...
      this.onMessage(payload, connectionGeneration).catch((err) => {
        ...
        process.stderr.write(
          `[WeCom:${this.name}] message handling failed for ${logMessageId}: ${sanitizeLogText(formatSdkError(err), 200)}\n`,
        );
      });
    };
  ...
  async sendMessage(chatId, text) {
    ...
    if (chunks.length === 0 && media.length === 0) {
      process.stderr.write(
        `[WeCom:${this.name}] sendMessage produced empty payload for chatId=${sanitizeLogText(chatId, 100)}.\n`,
      );
      return;
    }
  ...
  async onMessage(payload, connectionGeneration) {
    ...
    if (!chatId || !senderId) {
      process.stderr.write(
        `[WeCom:${this.name}] dropping message ${logMessageId}: missing ${!senderId ? 'senderId' : 'chatId'}.\n`,
      );
      return;
    }
    if (rawMessageId) {
  ...
  async downloadAttachments(
    ...
    for (const ref of refs) {
      ...
      try {
        ...
        process.stderr.write(
          `[WeCom:${this.name}] skipping ${ref.type} attachment: ${sanitizeLogText(err instanceof Error ? err.message : String(err), 160)}.\n`,
        );
        continue;
      }
      if (this.disconnectGeneration !== connectionGeneration)
  ...
  disconnectClientOnly(err) {
    ...
    try {
      ...
      process.stderr.write(
        `[WeCom:${this.name}] client.disconnect() threw: ${sanitizeLogText(formatSdkError(e), 200)}\n`,
      );
    }
  }
    ...
    try {
      ...
      while (this.kickReconnectAttempts < KICK_RECONNECT_MAX_ATTEMPTS) {
        ...
        try {
          ...
          process.stderr.write(
            `[WeCom:${this.name}] reconnect after ${reconnectReason} attempt ${attempt} failed: ${sanitizeLogText(formatSdkError(err), 200)}\n`,
          );
        }
      }
  ...
  startKickReconnect(reason, reconnectReason = 'server kick') {
    void this.reconnectAfterKick(reason, reconnectReason).catch((err) => {
      process.stderr.write(
        `[WeCom:${this.name}] kick-reconnect failed: ${sanitizeLogText(formatSdkError(err), 200)}\n`,
      );
...
function waitForAuthentication(client) {
  ...
  const onKicked = (reason) =>
    finish(
      new Error(
        `WeCom authentication interrupted by server kick: ${formatDisconnectReason(reason)}`,
      ),
    );
...
function cleanupAttachmentDirs(dirs) {
  for (const dir of dirs) {
    try {
      ...
      process.stderr.write(
        `[WeCom] failed to remove attachment dir ${sanitizeLogText(dir, 200)}: ${sanitizeLogText(formatSdkError(err), 200)}.\n`,
      );
    }
  }
}
...
function splitMarkdownChunks(text) {
  ...
  let current = '';
  let codeFence;
  const fits = (value, nextCodeFence = codeFence) =>
    Buffer.byteLength(
      nextCodeFence ? `${value}\n${nextCodeFence}` : value,
      'utf8',
