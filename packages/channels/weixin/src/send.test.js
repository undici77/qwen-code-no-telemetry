...
const { mockReadFileSync, mockStatSync, mockRealpathSync, mockGetUploadUrl, mockUploadToCdn, mockSendMessage, mockRandomBytes, } = vi.hoisted(() => ({
    ...
    mockStatSync: vi.fn(),
    mockRealpathSync: vi.fn((p) => p),
    mockGetUploadUrl: vi.fn(),
    mockUploadToCdn: vi.fn(),
    mockSendMessage: vi.fn(),
    mockRandomBytes: vi.fn((size) => Buffer.alloc(size, 0x42)),
}));
...
const { encryptAesEcb, computeMd5 } = await vi.importActual('./media.js');
const { sendImage, detectImageMime, validateImagePath } = await import('./send.js');
...
describe('validateImagePath', () => {
    ...
    it('rejects disallowed extensions', () => {
        expect(() => validateImagePath('/tmp/screenshot.txt', workspaceDirs)).toThrow('Image extension not allowed');
    });
    it('rejects non-existent files', () => {
        mockRealpathSync.mockImplementation(() => {
            throw new Error('ENOENT: no such file');
        });
        expect(() => validateImagePath('/tmp/missing.png', workspaceDirs)).toThrow('Image file not found');
    });
    it('rejects non-regular files (directories etc.)', () => {
        ...
        expect(() => validateImagePath('/tmp/some-dir.png', workspaceDirs)).toThrow('Not a regular file');
    });
    it('rejects files exceeding 20 MB cap', () => {
        ...
        expect(() => validateImagePath('/tmp/huge.png', workspaceDirs)).toThrow('Image too large');
    });
...
describe('sendImage', () => {
    ...
    it('completes the four-step upload and send flow', async () => {
        ...
        expect(mockGetUploadUrl).toHaveBeenCalledWith('https://api.example.com', 'token-abc', 'user-123', expectedFilekey, fakeImageData.length, computeMd5(fakeImageData), encryptedSize, expectedAesKeyHex);
