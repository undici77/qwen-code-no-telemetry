...
import { appendFileSync, chmodSync, existsSync, lstatSync, mkdtempSync, readFileSync, readdirSync, renameSync, rmSync, statSync, symlinkSync, writeFileSync, } from 'node:fs';
...
afterEach(() => {
    if (previous === undefined)
        ...
        process.env['WEIXIN_STATE_DIR'] = previous;
...
describe('saveAccount', () => {
    ...
    it('never creates the credential file at umask-default permissions', () => {
        ...
        const tokenWrites = vi
            ...
            .mock.calls.filter(([, contents]) => String(contents).includes(DATA.token));
    ...
    posixOnly('never leaves account.json as a symlink', () => {
        ...
        saveAccount(DATA);
        expect(lstatSync(join(stateDir, 'account.json')).isSymbolicLink()).toBe(false);
        expect(readFileSync(victim, 'utf-8')).toBe('not the token');
        expect(loadAccount()).toEqual(DATA);
    });
    ...
    it('removes the partial tmp file and re-throws when the write fails', () => {
        ...
        vi.mocked(writeFileSync).mockImplementationOnce((path) => {
            appendFileSync(path, `{"token":"${DATA.token.slice(0, 8)}`);
    ...
    it('replaces the previous account rather than appending', () => {
        saveAccount(DATA);
        const next = { ...DATA, token: 'rotated', userId: 'user-2' };
        saveAccount(next);
        expect(loadAccount()).toEqual(next);
        expect(() => JSON.parse(readFileSync(join(stateDir, 'account.json'), 'utf-8'))).not.toThrow();
    });
});
...
describe('clearAccount', () => {
    ...
    it('sweeps a temp file orphaned by a killed save', () => {
        ...
        saveAccount(DATA);
        writeFileSync(join(stateDir, 'account.json.a1b2c3d4e5f6.tmp'), JSON.stringify(DATA), 'utf-8');
