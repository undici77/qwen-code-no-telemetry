...
import { NdJsonIncompleteFrameError, NdJsonQueueLimitError, ndJsonStream, } from './ndJsonStream.js';
...
async function writeOne(writable, msg) {
...
describe('ndJsonStream', () => {
    it('round-trips one message', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(line)]));
    ...
    it('parses multiple messages from one chunk', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), byteStream([
    ...
    it('parses a large message split across many chunks', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), byteStream(chunks));
        await expect(readAll(stream.readable)).resolves.toEqual([sent]);
    });
    it('preserves multibyte UTF-8 characters across chunk boundaries', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), byteStream([bytes.slice(0, split), bytes.slice(split)]));
        await expect(readAll(stream.readable)).resolves.toEqual([sent]);
    });
    it('skips empty and CRLF lines', async () => {
        const sent = message('crlf');
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(`\n\r\n${JSON.stringify(sent)}\r\n`)]));
        await expect(readAll(stream.readable)).resolves.toEqual([sent]);
    });
    it('logs invalid JSON and continues with later messages', async () => {
        const stderr = vi.spyOn(console, 'error').mockImplementation(() => { });
        const sent = message('after-error');
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(`{bad json}\n${JSON.stringify(sent)}\n`)]));
        await expect(readAll(stream.readable)).resolves.toEqual([sent]);
        expect(stderr).toHaveBeenCalledWith('Failed to parse JSON message:', '{bad json}', expect.any(SyntaxError));
        stderr.mockRestore();
    });
    ...
    it('reports received and sent payload byte counts without newlines', async () => {
        ...
        const outputChunks = [];
        const stream = ndJsonStream(new WritableStream({
            write(chunk) {
        ...
        expect(new TextDecoder().decode(outputChunks[0])).toBe(`${JSON.stringify(sent)}\n`);
    });
    it('reports observed sent and received messages without changing byte hooks', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(`${JSON.stringify(received)}\r\n`)]), { onMessageReceived, onMessageSent, onMessageObserved });
        await expect(readAll(stream.readable)).resolves.toEqual([received]);
        await writeOne(stream.writable, sent);
        expect(onMessageReceived).toHaveBeenCalledWith(receivedBytes);
    ...
    it('propagates output write errors without reporting sent bytes', async () => {
        ...
        }), byteStream([]), { onMessageSent, onMessageObserved });
        await expect(writeOne(stream.writable, sent)).rejects.toThrow('output closed');
    ...
    it('accepts an inbound frame exactly at the configured byte limit', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), byteStream([frame.slice(0, 3), frame.slice(3)]), undefined, limits({
    ...
    it('counts CRLF and resets the bounded accumulator between frames', async () => {
        ...
        const maxFrameBytes = Math.max(firstFrame.byteLength, secondFrame.byteLength);
        const stream = ndJsonStream(new WritableStream(), byteStream([
        ...
        const rejected = ndJsonStream(new WritableStream(), byteStream([firstFrame]), { onTransportError }, limits({ maxFrameBytes: firstFrame.byteLength - 1 }));
    ...
    it('rejects an oversized inbound frame before parsing or reporting it', async () => {
        ...
        const onTransportError = vi.fn();
        const stderr = vi.spyOn(console, 'error').mockImplementation(() => { });
        const stream = ndJsonStream(new WritableStream(), byteStream([frame.slice(0, 5), frame.slice(5)]), { onMessageReceived, onTransportError }, limits({ maxFrameBytes: frame.byteLength - 1 }));
    ...
    it('rejects an incomplete final frame only on the bounded path', async () => {
        ...
        const onTransportError = vi.fn();
        const stream = ndJsonStream(new WritableStream(), byteStream([partial]), { onTransportError }, limits());
        await expect(readAll(stream.readable)).resolves.toEqual([]);
        expect(onTransportError).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('bounds the decoded queue by message count for a stalled consumer', async () => {
        ...
        const onMessageReceived = vi.fn();
        const onTransportError = vi.fn();
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(frames)]), { onMessageReceived, onTransportError }, limits({
            maxFrameBytes: 200,
            maxQueuedMessages: 2,
            maxQueuedBytes: 200,
        }));
        await vi.waitFor(() => expect(onTransportError).toHaveBeenCalledWith(expect.any(NdJsonQueueLimitError)));
        expect(onTransportError).toHaveBeenCalledOnce();
    ...
    it('bounds the decoded queue by retained wire bytes', async () => {
        ...
        const onMessageReceived = vi.fn();
        const onTransportError = vi.fn();
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(first + second)]), { onMessageReceived, onTransportError }, limits({
            maxFrameBytes: 200,
            maxQueuedMessages: 100,
            maxQueuedBytes: firstBytes + 1,
        }));
        await vi.waitFor(() => expect(onTransportError).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('bounds requests retained by the ACP SDK while responses are blocked', async () => {
        ...
        const input = new ReadableStream({
            start(controller) {
                ...
                timer = setInterval(() => {
                    ...
                    inputController.enqueue(encoder.encode(`${JSON.stringify(request)}\n`));
                }, 0);
            },
            cancel,
        });
        ...
        const onTransportError = vi.fn();
        const stream = ndJsonStream(new WritableStream({ write: outputWrite }), input, { onTransportError }, limits({
            maxFrameBytes: 1024,
            maxQueuedMessages: 2,
            maxQueuedBytes: 4096,
        }));
    ...
    it('keeps bounded parse-error logs free of input and parser text', async () => {
        ...
        const stderr = vi.spyOn(console, 'error').mockImplementation(() => { });
        const onTransportError = vi.fn();
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(`${payload}\n`)]), { onTransportError }, limits());
        await expect(readAll(stream.readable)).resolves.toEqual([]);
        expect(onTransportError).toHaveBeenCalledOnce();
        expect(onTransportError).toHaveBeenCalledWith(expect.objectContaining({ code: 'ndjson_parse_error' }));
    ...
    it('rejects malformed JSON-RPC envelopes without logging their payloads', async () => {
        ...
        for (const line of invalidLines) {
            const onTransportError = vi.fn();
            const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(`${line}\n`)]), { onMessageReceived, onTransportError }, limits({ maxQueuedMessages: 8, maxQueuedBytes: 8192 }));
            await expect(readAll(stream.readable)).resolves.toEqual([]);
            expect(onTransportError).toHaveBeenCalledOnce();
            expect(onTransportError).toHaveBeenCalledWith(expect.objectContaining({ code: 'ndjson_invalid_message' }));
        }
        expect(onMessageReceived).not.toHaveBeenCalled();
    ...
    it('accepts bounded JSON-RPC string ids', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(`${JSON.stringify(request)}\n`)]), undefined, limits());
    ...
    it('accepts a string response id only when an outbound request owns it', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), input, undefined, limits());
    ...
    it('allows an owned bulk response beyond inbound structure limits', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), input, undefined, limits({ maxFrameBytes: 128_000, maxQueuedBytes: 128_000 }));
        const writer = stream.writable.getWriter();
        await writer.write({
            jsonrpc: '2.0',
            id: 41,
    ...
    it('rejects oversized request structures without materializing values', async () => {
        const params = Object.fromEntries(Array.from({ length: 10_001 }, (_, index) => [`k${index}`, index]));
        const request = {
            jsonrpc: '2.0',
            id: 1,
            method: 'client/request',
        ...
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(line)]), { onTransportError }, limits({ maxFrameBytes: 256_000, maxQueuedBytes: 256_000 }));
        await expect(readAll(stream.readable)).resolves.toEqual([]);
        expect(onTransportError).toHaveBeenCalledOnce();
        expect(onTransportError).toHaveBeenCalledWith(expect.objectContaining({ code: 'ndjson_invalid_message' }));
    ...
    it('bounds outbound requests that never receive a response', async () => {
        const input = new ReadableStream();
        const onTransportError = vi.fn();
        const stream = ndJsonStream(new WritableStream(), input, { onTransportError }, limits({ maxQueuedMessages: 2, maxQueuedBytes: 4096 }));
        const writer = stream.writable.getWriter();
        await writer.write({
            jsonrpc: '2.0',
            id: 1,
    ...
    it('rejects log-amplifying protocol scalars before ACP SDK dispatch', async () => {
        ...
        for (const line of invalidLines) {
            const onTransportError = vi.fn();
            const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode(`${line}\n`)]), { onMessageReceived, onTransportError }, limits({ maxFrameBytes: 4096, maxQueuedBytes: 16_384 }));
            const connection = new ClientSideConnection(() => ({}), stream);
            await expect(connection.closed).resolves.toBeUndefined();
            expect(onTransportError).toHaveBeenCalledOnce();
            expect(onTransportError).toHaveBeenCalledWith(expect.objectContaining({ code: 'ndjson_invalid_message' }));
        ...
        expect(renderedErrors.join('\n')).not.toMatch(/SECRET_METHOD|SECRET_ID|SECRET_ERROR/u);
        stderr.mockRestore();
    });
    it('checks outbound frame bytes including the newline', async () => {
        ...
        const outputChunks = [];
        const exact = ndJsonStream(new WritableStream({
            write(chunk) {
                outputChunks.push(chunk);
            },
        }), byteStream([]), undefined, limits({ maxFrameBytes: payloadBytes + 1 }));
        await expect(writeOne(exact.writable, sent)).resolves.toBeUndefined();
        expect(outputChunks[0]?.byteLength).toBe(payloadBytes + 1);
        const onTransportError = vi.fn();
        const rejected = ndJsonStream(new WritableStream(), byteStream([]), { onTransportError }, limits({ maxFrameBytes: payloadBytes }));
    ...
    it('reports bounded outbound serialization failures exactly once', async () => {
        ...
        for (const invalid of invalidMessages) {
            const onTransportError = vi.fn();
            const stream = ndJsonStream(new WritableStream(), byteStream([]), { onTransportError }, limits());
            await expect(writeOne(stream.writable, invalid)).rejects.toBeInstanceOf(TypeError);
            expect(onTransportError).toHaveBeenCalledOnce();
    ...
    it('closes the ACP SDK connection without rejecting on inbound fatal', async () => {
        const onTransportError = vi.fn();
        const stream = ndJsonStream(new WritableStream(), byteStream([encoder.encode('x'.repeat(17))]), { onTransportError }, limits({ maxFrameBytes: 16 }));
        const connection = new ClientSideConnection(() => ({}), stream);
        await expect(connection.closed).resolves.toBeUndefined();
        expect(connection.signal.aborted).toBe(true);
        expect(onTransportError).toHaveBeenCalledOnce();
        expect(onTransportError).toHaveBeenCalledWith(expect.objectContaining({ code: 'ndjson_frame_too_large' }));
    });
    it('delivers complete admitted frames before a later inbound fatal', async () => {
        ...
        const input = encoder.encode(`${JSON.stringify(first)}\n${JSON.stringify(second)}\n${'x'.repeat(maxFrameBytes + 1)}`);
        const stream = ndJsonStream(new WritableStream(), byteStream([input]), { onTransportError }, limits({
            maxFrameBytes,
            maxQueuedMessages: 4,
    ...
    it('redacts bounded valid messages when the ACP SDK logs handler errors', async () => {
        ...
        const stream = ndJsonStream(new WritableStream(), byteStream([
            encoder.encode(`${input.map((value) => JSON.stringify(value)).join('\n')}\n`),
        ]), undefined, limits({ maxQueuedMessages: 4, maxQueuedBytes: 4096 }));
        const connection = new ClientSideConnection(() => ({}), stream);
        await expect(connection.closed).resolves.toBeUndefined();
        await vi.waitFor(() => expect(renderedErrors).toHaveLength(2));
        expect(renderedErrors.join('\n')).toContain('payloadOmitted: true');
        expect(renderedErrors.join('\n')).not.toMatch(/unknown-secret|params-secret/u);
        stderr.mockRestore();
    });
});
