...
import { ACTIVE_WORK_HEARTBEAT_VERSION, ACTIVE_WORK_HOLD_CATEGORIES, ACTIVE_WORK_MAX_SESSION_HOLDS, ACTIVE_WORK_MAX_SNAPSHOT_SESSIONS, ACTIVE_WORK_NOTIFICATION_METHOD, MID_TURN_RECONCILIATION_RING_SIZE, MID_TURN_QUEUE_DRAIN_METHOD, TODO_STOP_GUARD_CONTINUATION_CLAIM_METHOD, } from './bridgeTypes.js';
import { SERVE_CONTROL_EXT_METHODS } from './status.js';
import { isValidExternalToolGuardDenialReason } from './externalToolGuard.js';
import { CHANNEL_DELIVERY_ERROR_CODES, LIVE_TASK_TOOL_NAMES, MAX_LIVE_SCREEN_CONTEXT_TEXT_CHARS, MAX_LIVE_SPEAK_TO_USER_MESSAGE_CHARS, MAX_SUB_SESSION_NAME_CHARS, MAX_SUB_SESSION_PROMPT_CHARS, } from './bridgeOptions.js';
...
function parseActiveWorkSnapshot(params) {
    ...
    if (params['v'] !== ACTIVE_WORK_HEARTBEAT_VERSION ||
        typeof seq !== 'number' ||
    ...
    for (const raw of sessions) {
        ...
        for (const rawHold of holds) {
            if (typeof rawHold !== 'object' || rawHold === null)
                return undefined;
            const hold = rawHold;
...
function isChannelDeliveryTarget(value) {
    if (!isRecord(value))
        return false;
    return (isBoundedChannelDeliveryString(value['channelName']) &&
        (value['type'] === 'user' || value['type'] === 'chat') &&
        isBoundedChannelDeliveryString(value['id']) &&
        Object.keys(value).every((key) => key === 'channelName' || key === 'type' || key === 'id'));
}
function normalizeChannelDeliveryHostResult(value) {
    ...
    const error = typeof value.error === 'string' && value.error.length > 0
        ? value.error.slice(0, MAX_CHANNEL_DELIVERY_ERROR_CHARS)
        : 'Channel delivery failed.';
    return { status: 'failed', code: code, error };
}
...
function pendingInteractionFromRequest(requestId, params) {
    ...
    const options = pendingInteractionOptions(Array.isArray(params.options) ? params.options : []);
    ...
    if (isUserQuestion) {
        ...
        return {
            ...
            ...(typeof toolCall['title'] === 'string'
                ...
                : {}),
            questions: rawQuestions.flatMap((question, index) => isRecord(question) ? [{ ...question, answerKey: String(index) }] : []),
            options,
        };
    }
    return {
...
function extractCappedArtifactInputs(rawArtifacts, limit, sessionId, source, toInput) {
    ...
    for (let index = 0; index < rawArtifacts.length; index++) {
        ...
        if (!isRecord(artifact)) {
            writeStderrLine(`[artifacts] session=${sessionId} action=dropped reason=malformed source=${source} index=${index}`);
            continue;
        }
        if (artifacts.length >= limit) {
            writeStderrLine(`[artifacts] session=${sessionId} action=dropped reason="artifact batch limit exceeded" source=${source} dropped=${rawArtifacts.length - index}`);
            break;
        }
...
function extractSessionUpdateArtifacts(params, updateMeta, limit, sessionId) {
    ...
    const toolName = typeof updateMeta?.['toolName'] === 'string'
        ? updateMeta['toolName']
        : undefined;
    return extractCappedArtifactInputs(rawArtifacts, limit, sessionId, 'tool', (artifact) => ({
...
export const KNOWN_APPROVAL_MODES = new Set([
    'plan',
    'default',
    'auto-edit',
    'auto',
...
export class BridgeClient {
     ...
     */
    resolveEntry, resolvePendingRestoreEvents, 
     ...
     */
    clientMcpSender, ownsSession = () => true, 
    /**
    ...
    onWorkspaceGenerationEvent, onChannelDelivery, 
    /** Permits pre-registration client-MCP discovery without trusting its id. */
    hasSessionSpawnInFlight = () => false, getLiveScreenContextCaptureHandler = () => undefined, getLiveTaskToolRequestHandler = () => undefined, getLiveSpeakToUserHandler = () => undefined, 
    /**
     ...
     */
    externalToolGuard, onActiveWork) {
    ...
    async requestPermission(params) {
        ...
        const allowedOptionIds = new Set(options.map((o) => String(o.optionId ?? '')));
        ...
        if (interaction)
            entry.pendingInteractions.set(requestId, interaction);
        try {
            const record = {
                requestId,
                sessionId: entry.sessionId,
            ...
            const resolution = await this.mediator.request(record, this.permissionTimeoutMs);
    ...
    async sessionUpdate(params) {
        ...
        if (!this.ownsSession(params.sessionId) &&
            !this.inFlightRestoreIds.has(params.sessionId)) {
            writeStderrLine(`[demux] session=${params.sessionId} type=session_update action=dropped reason=session_not_owned`);
            return;
        }
        const entry = this.resolveEntry(params.sessionId);
    ...
    prepareSessionUpdateFrames(params, entry) {
        ...
        const updateMeta = params.update
            ._meta;
        const originalTs = updateMeta?.['serverTimestamp'] ?? updateMeta?.['timestamp'];
        const serverTimestamp = typeof originalTs === 'number' && Number.isFinite(originalTs)
            ? originalTs
            : undefined;
        const artifacts = entry?.artifacts
            ? extractSessionUpdateArtifacts(params, updateMeta, entry.artifacts.inputBatchLimit(), entry.sessionId)
            : [];
    ...
    async seedSessionUpdates(entry, updates, options = {}) {
        ...
        for (const update of updates) {
            const prepared = this.prepareSessionUpdateFrames({ sessionId: entry.sessionId, update }, entry);
    ...
    recordLiveTokenUsage(params, entry) {
        ...
        this.onTokenUsage(typeof inputTokens === 'number' ? inputTokens : 0, typeof outputTokens === 'number' ? outputTokens : 0, typeof durationMs === 'number' ? durationMs : undefined, typeof apiErrors === 'number' ? apiErrors : 0, typeof apiRetries === 'number' ? apiRetries : 0);
    }
    /**
    ...
    async extMethod(method, params) {
        ...
        if (drained.length > 0) {
            ...
            if (entry.settledMidTurnMessageIds.length >
                MID_TURN_RECONCILIATION_RING_SIZE) {
                entry.settledMidTurnMessageIds.splice(0, entry.settledMidTurnMessageIds.length -
                    MID_TURN_RECONCILIATION_RING_SIZE);
            }
        }
        ...
        const hasQueuedPrompt = entry.pendingPromptList.some((prompt) => prompt.state === 'queued' && !prompt.abortController.signal.aborted);
        if (echoed.length > 0) {
    ...
    async handleExternalToolGuardPrepare(params) {
        if (!this.externalToolGuard) {
            throw RequestError.methodNotFound(SERVE_CONTROL_EXT_METHODS.externalToolGuardPrepare);
        }
        ...
        if (typeof sessionId !== 'string' ||
            ...
            toolName.length === 0 ||
            !isRecord(args)) {
            throw RequestError.invalidParams(undefined, 'Invalid external tool guard request');
        }
        if (!this.ownsSession(sessionId)) {
            throw RequestError.invalidParams(undefined, 'External tool guard session is not owned by this connection');
        }
        const entry = this.resolveEntry(sessionId);
        if (!entry || !entry.promptActive || entry.activePromptId !== promptId) {
            throw RequestError.invalidParams(undefined, 'External tool guard prompt is not the active prompt');
        }
        ...
        if (!this.ownsSession(sessionId) ||
            ...
            throw RequestError.invalidParams(undefined, 'External tool guard prompt is no longer active');
        }
    ...
    handleTodoStopGuardContinuationClaim(params) {
        ...
        if (!entry)
            return { claimed: false, hasQueuedPrompt: false };
        const livePrompts = entry.pendingPromptList.filter((prompt) => !prompt.abortController.signal.aborted);
        const promptId = typeof params['promptId'] === 'string' && params['promptId'].length > 0
            ? params['promptId']
            : undefined;
        if (promptId) {
            const ownsRunningPrompt = entry.activePromptId === promptId &&
                livePrompts.some((prompt) => prompt.promptId === promptId && prompt.state === 'running');
            const hasCompetingRunningPrompt = livePrompts.some((prompt) => prompt.promptId !== promptId && prompt.state === 'running');
            if (!ownsRunningPrompt || hasCompetingRunningPrompt) {
                return { claimed: false, hasQueuedPrompt: false };
            }
            const hasQueuedPrompt = livePrompts.some((prompt) => prompt.state === 'queued');
            if (hasQueuedPrompt) {
    ...
    async handleChannelDelivery(params) {
        if (!this.onChannelDelivery) {
            throw RequestError.methodNotFound(SERVE_CONTROL_EXT_METHODS.channelDelivery);
        }
        const sessionId = params['sessionId'];
        if (typeof sessionId !== 'string' ||
            sessionId.length === 0 ||
            !this.ownsSession(sessionId)) {
            throw RequestError.invalidParams(undefined, '`sessionId` must name a session owned by this connection');
        }
        const entry = this.resolveEntry(sessionId);
        if (!entry) {
        ...
        if (!isBoundedChannelDeliveryString(deliveryId) ||
            (source !== 'prompt' && source !== 'scheduled') ||
            typeof text !== 'string' ||
            text.length > MAX_CHANNEL_DELIVERY_TEXT_CHARS ||
            !isChannelDeliveryTarget(rawTarget)) {
            throw RequestError.invalidParams(undefined, 'Invalid channel delivery request');
        }
        const promptId = params['promptId'];
        const taskId = params['taskId'];
        const firedAt = params['firedAt'];
        const allowedKeys = source === 'prompt'
            ? new Set([
                'sessionId',
                'deliveryId',
                'source',
        ...
        const correlationValid = source === 'prompt'
        ...
        if (!correlationValid ||
            ...
            throw RequestError.invalidParams(undefined, 'Invalid channel delivery correlation');
        }
        const info = {
            sessionId,
            deliveryId,
            ...
            target: rawTarget,
            text,
            ...(source === 'prompt' ? { promptId: promptId } : {}),
            ...(source === 'scheduled'
                ? { taskId: taskId, firedAt: firedAt }
                : {}),
        };
        let result;
        try {
            result = normalizeChannelDeliveryHostResult(await this.onChannelDelivery(info));
        }
        catch {
            result = {
                status: 'failed',
    ...
    async handleClientMcpMessage(params) {
        if (!this.clientMcpSender) {
            throw RequestError.methodNotFound(SERVE_CONTROL_EXT_METHODS.clientMcpMessage);
        }
        const server = params['server'];
        if (typeof server !== 'string' || server.length === 0) {
            throw RequestError.invalidParams(undefined, '`server` must be a non-empty string');
        }
        const payload = params['payload'];
        if (payload === null || typeof payload !== 'object') {
            throw RequestError.invalidParams(undefined, '`payload` must be a JSON-RPC message object');
        }
        const send = this.clientMcpSender(server);
        if (!send) {
            // The client that hosted this server is gone (WS closed / unregistered).
            throw RequestError.invalidParams(undefined, `client-hosted MCP server '${server}' is not currently connected`);
        }
        const sessionId = params['sessionId'];
        if (sessionId !== undefined &&
            (typeof sessionId !== 'string' || sessionId.length === 0)) {
            throw RequestError.invalidParams(undefined, '`sessionId` must be a non-empty string when provided');
        }
        const ownsSession = typeof sessionId === 'string' && this.ownsSession(sessionId);
        if (typeof sessionId === 'string' &&
            !ownsSession &&
            !this.hasSessionSpawnInFlight()) {
            throw RequestError.invalidParams(undefined, `Session not owned by this channel: ${sessionId}`);
        }
        if (typeof sessionId === 'string' && !ownsSession) {
            writeStderrLine(`[demux] session=${sessionId} type=client_mcp_message action=forwarded_without_session reason=session_registration_pending`);
        }
        const response = await send(payload, typeof sessionId === 'string' && ownsSession ? { sessionId } : undefined);
    ...
    async handleCreateSubSession(params) {
        if (!this.onCreateSubSession) {
            throw RequestError.methodNotFound(SERVE_CONTROL_EXT_METHODS.createSubSession);
        }
        const prompt = params['prompt'];
        if (typeof prompt !== 'string' || prompt.length === 0) {
            throw RequestError.invalidParams(undefined, '`prompt` must be a non-empty string');
        }
        ...
        if (prompt.length > MAX_SUB_SESSION_PROMPT_CHARS) {
            throw RequestError.invalidParams(undefined, `\`prompt\` exceeds the ${MAX_SUB_SESSION_PROMPT_CHARS}-character limit`);
        }
        const completion = params['completion'];
        if (completion !== 'sent' && completion !== 'first-turn') {
            throw RequestError.invalidParams(undefined, "`completion` must be 'sent' or 'first-turn'");
        }
        const name = params['name'];
        if (typeof name === 'string' && name.length > MAX_SUB_SESSION_NAME_CHARS) {
            throw RequestError.invalidParams(undefined, `\`name\` exceeds the ${MAX_SUB_SESSION_NAME_CHARS}-character limit`);
        }
        ...
        if (typeof callerSessionId !== 'string' ||
            ...
            throw RequestError.invalidParams(undefined, '`callerSessionId` is required and must name a session owned by this connection');
        }
    ...
    async handleLiveScreenContextCapture(params) {
        ...
        if (!handler) {
            throw RequestError.methodNotFound(SERVE_CONTROL_EXT_METHODS.liveCaptureScreenContext);
        }
        const callerSessionId = params['callerSessionId'];
        if (typeof callerSessionId !== 'string' ||
            callerSessionId.length === 0 ||
    ...
    async handleLiveTaskTool(params) {
        ...
        if (typeof callerSessionId !== 'string' ||
            ...
            !this.ownsSession(callerSessionId) ||
            typeof name !== 'string' ||
            !LIVE_TASK_TOOL_NAMES.includes(name) ||
            typeof args !== 'object' ||
            args === null ||
            Array.isArray(args)) {
            throw RequestError.invalidParams(undefined, 'Invalid Live task-tool request.');
        }
        return handler({
            callerSessionId,
            name: name,
    ...
    async handleLiveSpeakToUser(params) {
        ...
        if (!handler) {
            throw RequestError.methodNotFound(SERVE_CONTROL_EXT_METHODS.liveSpeakToUser);
        }
        const callerSessionId = params['callerSessionId'];
        ...
        if (typeof callerSessionId !== 'string' ||
            ...
            throw RequestError.invalidParams(undefined, 'Invalid Live speak-to-user request.');
        }
    ...
    async extNotification(method, params) {
        ...
        if (method === ACTIVE_WORK_NOTIFICATION_METHOD) {
            ...
            if (snapshot) {
                ...
                this.onActiveWork?.({
                    ...
                    seq: snapshot.seq,
                    sessions: snapshot.sessions.filter((session) => this.ownsSession(session.sessionId)),
                });
            }
            return;
        }
        ...
        if (method === 'qwen/notify/session/recording-degraded') {
            ...
            if (params['v'] !== 1 ||
                ...
                writeStderrLine(`[demux] session=${typeof sessionId === 'string' ? sessionId : '<missing>'} type=session_recording_degraded action=dropped reason=malformed`);
                return;
            }
            const entry = this.resolveEntry(sessionId);
            if (entry && !this.ownsSession(sessionId)) {
                writeStderrLine(`[demux] session=${sessionId} type=session_recording_degraded action=dropped reason=session_not_owned`);
                return;
            }
            if (entry)
        ...
        if (method === 'qwen/notify/session/prompt-suggestion') {
            ...
            if (typeof sessionId !== 'string' ||
                ...
                typeof promptId !== 'string') {
                writeStderrLine(`[demux] session=${typeof sessionId === 'string' ? sessionId : '<missing>'} type=prompt_suggestion action=dropped reason=malformed`);
                return;
            }
            const entry = this.resolveEntry(sessionId);
            if (!entry)
    ...
    async handleArtifactEvent(params) {
        ...
        if (typeof sessionId !== 'string' || !Array.isArray(rawArtifacts)) {
            writeStderrLine(`[demux] session=${typeof sessionId === 'string' ? sessionId : '<missing>'} type=artifact_event action=dropped reason=malformed`);
            return;
        }
        if (!this.ownsSession(sessionId) &&
            !this.inFlightRestoreIds.has(sessionId)) {
            writeStderrLine(`[demux] session=${sessionId} type=artifact_event action=dropped reason=session_not_owned`);
            return;
        }
        const entry = this.resolveEntry(sessionId);
        if (!entry) {
            writeStderrLine(`[demux] session=${sessionId} type=artifact_event action=dropped reason=session_not_found`);
            return;
        }
        const hookEventName = typeof params['hookEventName'] === 'string'
            ? params['hookEventName']
            : undefined;
        const toolName = typeof params['toolName'] === 'string' ? params['toolName'] : undefined;
        const toolCallId = typeof params['toolCallId'] === 'string'
            ? params['toolCallId']
            : undefined;
        const artifacts = extractCappedArtifactInputs(rawArtifacts, entry.artifacts.inputBatchLimit(), entry.sessionId, 'hook', (artifact) => ({
            ...artifactPayloadFields(artifact),
            source: 'hook',
            hookEventName,
            toolName,
    ...
    async upsertAndPublishArtifacts(entry, artifacts, options, turn = {}) {
        try {
            ...
            writeStderrLine(`[artifacts] session=${entry.sessionId} action=dropped reason=${JSON.stringify(artifactIngestionErrorReason(error))}`);
        }
    }
    ...
    handleInSessionModelUpdate(params) {
        ...
        if (!entry) {
            ...
            writeStderrLine(`[demux] session=${sessionId} type=current_model_update action=dropped reason=no_entry`);
            return;
        }
        if (entry.modelRoundtripInFlight) {
            // Bridge owns this change and will publish model_switched itself.
            writeStderrLine(`[demux] session=${sessionId} type=current_model_update action=suppressed reason=bridge_roundtrip_in_flight`);
            return;
        }
        if (this.onModelPromoted) {
            this.onModelPromoted(entry, currentModelId, entry.activePromptOriginatorClientId);
        }
        else {
        ...
        writeStderrLine(`[demux] session=${sessionId} type=current_model_update action=promoted model=${currentModelId}`);
    }
    /**
    ...
    handleInSessionModeUpdate(params) {
        ...
        if (!KNOWN_APPROVAL_MODES.has(currentModeId)) {
            writeStderrLine(`[demux] session=${sessionId} type=current_mode_update action=dropped reason=unknown_mode mode=${currentModeId}`);
            return;
        }
        const entry = this.resolveEntry(sessionId);
        if (!entry) {
            writeStderrLine(`[demux] session=${sessionId} type=current_mode_update action=dropped reason=no_entry`);
            return;
        }
        if (entry.approvalModeRoundtripInFlight) {
            writeStderrLine(`[demux] session=${sessionId} type=current_mode_update action=suppressed reason=bridge_roundtrip_in_flight`);
            return;
        }
        if (this.onModePromoted) {
            this.onModePromoted(entry, currentModeId, entry.activePromptOriginatorClientId);
        }
        else {
        ...
        if (params['legacyFrameSent'] === true) {
            writeStderrLine(`[demux] session=${sessionId} type=current_mode_update action=promoted mode=${currentModeId} legacy_frame=skipped`);
            return;
        }
        ...
        writeStderrLine(`[demux] session=${sessionId} type=current_mode_update action=promoted mode=${currentModeId}`);
    }
    /**
    ...
    async readTextFile(params) {
        ...
        if (typeof params.limit === 'number' &&
            ...
            throw RequestError.invalidParams(undefined, `\`limit\` must be a positive integer, got ${params.limit}`);
        }
        if (typeof params.line === 'number' &&
            params.line > 0 &&
            !Number.isSafeInteger(params.line)) {
            throw RequestError.invalidParams(undefined, `\`line\` must be a positive integer, got ${params.line}`);
        }
 ...
 * Exported for unit testing.
 */
export function splitA2uiText(raw) {
...
function surfaceIdOf(c) {
    const cmd = c;
    return (cmd?.createSurface?.surfaceId ??
 ...
 * Exported for unit testing.
 */
export function extractA2uiToolUpdate(params) {
    const update = params.update;
    if (!update || update['sessionUpdate'] !== 'tool_call_update')
        return null;
    const meta = update['_meta'];
    if (!isA2uiToolMeta(meta))
        return null;
    ...
    for (const c of commands) {
        ...
        if (!sid) {
            const shape = c && typeof c === 'object'
                ? Object.keys(c).join(',') || 'empty object'
                : typeof c;
            writeStderrLine(`a2ui: dropping command with unrecognized shape (${shape})`);
            continue;
        }
        if (!grouped.has(sid)) {
            grouped.set(sid, []);
    ...
    const sanitizedContent = content.map((block, i) => i === hitIndex
        ? {
            ...block,
            content: {
                ...(block.content ?? {}),
    ...
    return {
        surfaces,
        callId: typeof update['toolCallId'] === 'string'
            ? update['toolCallId']
            : undefined,
        sanitizedParams: {
            ...params,
