 ...
 * `cli/src/serve/`.
 */
import type { ApprovalMode, DaemonBridgeTelemetryMetrics } from '@qwen-code/qwen-code-core';
 ...
 */
export type DiagnosticLineSink = (line: string, level?: 'info' | 'warn' | 'error') => void;
...
export type BridgeFreshSessionAdmission = (context: BridgeFreshSessionAdmissionContext) => BridgeFreshSessionReservation | undefined;
...
export type BridgeSessionLifecycle = (event: BridgeSessionLifecycleEvent) => void;
/**
...
export type ExternalToolGuardHandler = (request: ExternalToolGuardPrepareRequest) => Promise<ExternalToolGuardPrepareResult>;
/**
...
export interface DaemonStatusProvider {
     ...
     */
    getEnvStatus(boundWorkspace: string, acpChannelLive: boolean): Promise<ServeWorkspaceEnvStatus>;
    /**
     ...
     */
    getDaemonPreflightCells(boundWorkspace: string): Promise<ServePreflightCell[]>;
}
export type BridgeTelemetryAttributes = Record<string, string | number | boolean>;
...
export interface BridgeOptions {
     ...
     * of the request flag.
     */
    persistApprovalMode?: (boundWorkspace: string, mode: ApprovalMode) => Promise<void>;
    /**
...
export type ClientMcpMessageSender = (serverName: string) => ((payload: unknown, context?: ClientMcpMessageContext) => Promise<unknown>) | undefined;
...
export declare const MAX_SUB_SESSION_NAME_CHARS = 200;
/**
...
export type CreateSubSessionHandler = (info: CreateSubSessionInfo) => Promise<CreateSubSessionResult>;
export declare const MAX_LIVE_SCREEN_CONTEXT_TEXT_CHARS = 32000;
...
export type LiveScreenContextCaptureHandler = (info: LiveScreenContextCaptureInfo) => Promise<LiveScreenContextCaptureResult>;
...
export type LiveTaskToolRequestHandler = (info: LiveTaskToolRequestInfo) => Promise<Record<string, unknown>>;
export declare const MAX_LIVE_SPEAK_TO_USER_MESSAGE_CHARS = 32000;
export interface LiveSpeakToUserInfo {
    callerSessionId: string;
    message: string;
}
export type LiveSpeakToUserHandler = (info: LiveSpeakToUserInfo) => Promise<void>;
export type ChannelDeliveryErrorCode = 'channel_worker_unavailable' | 'channel_delivery_timeout' | 'channel_delivery_invalid' | 'channel_delivery_rejected' | 'channel_delivery_queue_full' | 'channel_delivery_failed';
export declare const CHANNEL_DELIVERY_ERROR_CODES: ReadonlySet<string>;
...
export type ChannelDeliveryHandler = (info: ChannelDeliveryInfo) => Promise<ChannelDeliveryHostResult>;
