...
import { DAEMON_TRACEPARENT_META_KEY, DAEMON_TRACESTATE_META_KEY, INVOCATION_CONTEXT_META_KEY, PRIVATE_ACP_CAPABILITY_ENV, PRIVATE_PARENT_CAPABILITY_META_KEY, SESSION_ARTIFACT_PERSISTENCE_VERSION, SESSION_TRANSCRIPT_MAX_LIMIT, TrustGateError, normalizeSnapshotPayload, ShellExecutionService, } from '@qwen-code/qwen-code-core';
import { EventBus, DEFAULT_RING_SIZE, EVENT_SCHEMA_VERSION, } from './eventBus.js';
import { normalizeCompactedReplayMaxBytes, normalizeMaxJournalBytes, normalizeMaxJournalEvents, TurnBoundaryCompactionEngine, } from './compactionEngine.js';
import { BridgeChannelClosedError, BridgeTimeoutError, SessionRestoreTimeoutError, createIdleWorkspaceExtensionsStatus, createIdleWorkspaceHooksStatus, SERVE_CONTROL_EXT_METHODS, SERVE_STATUS_EXT_METHODS, STATUS_SCHEMA_VERSION, } from './status.js';
import { EXTERNAL_TOOL_GUARD_READY_META_KEY, EXTERNAL_TOOL_GUARD_REQUIRED_VALUE, } from './externalToolGuard.js';
import { BranchWhilePromptActiveError, CdWhilePromptActiveError, SessionNotFoundError, RestoreInProgressError, InvalidSessionScopeError, SessionLimitExceededError, PromptQueueFullError, WorkspaceMismatchError, InvalidClientIdError, SessionShellClientRequiredError, SessionShellDisabledError, 
...
InvalidPermissionOptionError, InvalidSessionMetadataError, isNotCurrentlyGeneratingCancelError, SessionBusyError, InvalidRewindTargetError, PromptDeadlineExceededError, BridgeChannelQuarantinedError, } from './bridgeErrors.js';
import { resolveSessionRestoreTimeoutMs, restoreRetryAfterSeconds, } from './session-restore-timeout.js';
import { canonicalizeWorkspace, translateAndCheckAbsoluteWorkspacePath, } from './workspacePaths.js';
import { parseSessionSource, SESSION_SOURCE_META_KEY, } from './session-source.js';
import { ACTIVE_WORK_CLOSE_IF_UNHELD_PARAM, ACTIVE_WORK_CLOSE_TIMEOUT_MS, ACTIVE_WORK_HEARTBEAT_INTERVAL_MS, ACTIVE_WORK_HEARTBEAT_META_KEY, ACTIVE_WORK_HEARTBEAT_VERSION, ACTIVE_WORK_HOLD_CATEGORIES, ACTIVE_WORK_STALE_INTERVALS, clampActiveWorkIntervalMs, CHANNEL_STARTUP_PROFILE_META_KEY, CHANNEL_STARTUP_PROFILE_VERSION, DAEMON_CHANNEL_DELIVERY_META_KEY, DAEMON_MODEL_PROMPT_META_KEY, DAEMON_PROMPT_DISPLAY_TEXT_META_KEY, LOAD_REPLAY_BULK_MODE, LOAD_REPLAY_HIDE_INHERITED_META_KEY, LOAD_REPLAY_META_KEY, LOAD_REPLAY_MODE_META_KEY, LOAD_REPLAY_PAGE_SIZE_META_KEY, LOAD_REPLAY_VERSION, MID_TURN_RECONCILIATION_RING_SIZE, PROMPT_CANCEL_METHOD, REQUESTED_SESSION_ID_META_KEY, TODO_STOP_GUARD_QUEUE_RELEASE_METHOD, WORKTREE_MCP_DEFER_META_KEY, isValidTrustedModelPrompt, } from './bridgeTypes.js';
...
import { CANCEL_VOTE_SENTINEL, createNoOpPermissionAuditPublisher, MultiClientPermissionMediator, } from './permissionMediator.js';
import { PermissionForbiddenError } from './bridgeErrors.js';
import { SessionArtifactStore, isArtifactRestoreFailureWarning, publicArtifactsEqual, } from './sessionArtifacts.js';
const NOOP_BRIDGE_TELEMETRY = {
...
function logSafeAcpErrorDetails(error) {
    ...
    if (!details)
        return undefined;
    return {
        details: details.length <= MAX_LOG_SAFE_ACP_ERROR_DETAILS_CHARS
            ? details
...
function logSafeRequestErrorData(data) {
    ...
    return {
        errorKind: boundedAcpErrorString(data['errorKind'], MAX_LOG_SAFE_ACP_ERROR_KIND_CHARS),
...
function estimateAcpHandlerBytes(value, limitBytes) {
    ...
    while (stack.length > 0) {
        ...
        if (current === null) {
            ...
            if (bytes + current.length > limitBytes)
                return limitBytes + 1;
...
class AcpInboundHandlerAdmission {
    ...
    async run(params, operation) {
        ...
        const requiredBytes = envelopeBytes +
            estimateAcpHandlerBytes(params, Math.max(0, this.guard.maxActiveHandlerBytes - envelopeBytes));
        const availableBytes = Math.max(0, this.guard.maxActiveHandlerBytes - this.activeBytes);
        if (this.activeHandlers >= this.guard.maxActiveHandlers ||
            requiredBytes > availableBytes) {
            const error = new AcpInboundHandlerLimitError(this.guard.maxActiveHandlers, this.guard.maxActiveHandlerBytes, requiredBytes, availableBytes);
...
async function withLogSafeAcpError(operation, reservePreparedResponse) {
    try {
        ...
        if (error instanceof RequestError) {
            ...
            throw new LogSafeAcpRequestError(code, logSafeRequestErrorMessage(code), logSafeRequestErrorData(error.data), reservePreparedResponse);
        }
        throw new LogSafeAcpRequestError(-32603, 'Internal error', logSafeAcpErrorDetails(error), reservePreparedResponse);
    }
}
function createLogSafeAcpClient(client, transportGuard) {
    const admission = new AcpInboundHandlerAdmission(transportGuard);
    const runNotification = (params, operation) => withLogSafeAcpError(() => admission.run(params, operation));
    const runRequest = (params, operation) => withLogSafeAcpError(() => admission.run(params, async () => {
        const result = await operation();
        transportGuard.reservePreparedResponse(result ?? null);
        return result;
    }), transportGuard.reservePreparedResponse);
    return {
        requestPermission: (params) => runRequest(params, () => client.requestPermission(params)),
        sessionUpdate: (params) => runNotification(params, () => client.sessionUpdate(params)),
        ...
        releaseTerminal: client.releaseTerminal
            ? (params) => runRequest(params, async () => (await client.releaseTerminal(params)) ?? {})
            : undefined,
        waitForTerminalExit: client.waitForTerminalExit
            ? (params) => runRequest(params, () => client.waitForTerminalExit(params))
            : undefined,
        killTerminal: client.killTerminal
            ? (params) => runRequest(params, async () => (await client.killTerminal(params)) ?? {})
            : undefined,
        extMethod: client.extMethod
            ? (method, params) => runRequest(params, () => client.extMethod(method, params))
            : undefined,
        extNotification: client.extNotification
            ? (method, params) => runNotification(params, () => client.extNotification(method, params))
            : undefined,
    };
}
const OUTBOUND_GUARDED_CONNECTION_METHODS = new Set([
    'initialize',
    'newSession',
    'loadSession',
...
function extractLoadReplayResponse(state) {
    ...
    if (rawUpdates.length > MAX_BULK_REPLAY_UPDATES) {
        throw new Error(`qwen.session.loadReplay updates exceed limit ` +
            `(${rawUpdates.length} > ${MAX_BULK_REPLAY_UPDATES})`);
    }
    const partial = replay['partial'];
    if (partial !== undefined && partial !== true) {
        throw new Error(`Invalid qwen.session.loadReplay partial ` +
    ...
    if (replayError !== undefined && typeof replayError !== 'string') {
        throw new Error(`Invalid qwen.session.loadReplay replayError ` +
    ...
    if (hasMore !== undefined && typeof hasMore !== 'boolean') {
        throw new Error(`Invalid qwen.session.loadReplay hasMore ` +
            `(version=${LOAD_REPLAY_VERSION}, hasMore=${describeLoadReplayValue(hasMore)})`);
    }
    const invalidUpdateIndex = rawUpdates.findIndex((update) => !isBulkReplayUpdate(update));
    if (invalidUpdateIndex !== -1) {
        ...
        const discriminator = isRecord(invalidUpdate)
            ...
            : undefined;
        throw new Error(`Invalid qwen.session.loadReplay update at index ${invalidUpdateIndex} ` +
...
function parseWorkspaceMemoryForgetMatch(value) {
    ...
    const record = value;
    if (!isBridgeAutoMemoryTopic(record['topic']) ||
...
function parseWorkspaceMemoryForgetResult(response) {
    ...
    return {
        ...
        touchedScopes: touchedScopes === undefined
...
function echoPromptToSessionBus(entry, req, promptId, originatorClientId, displayText) {
    ...
    if (displayText &&
        ...
        if (textPart)
            echoPrompt[echoPrompt.length - 1] = textPart;
    }
...
function broadcastPromptCancelledOnce(entry, sessionId, promptId, originatorClientId, reason) {
    if ((promptId !== undefined && entry.cancelBroadcastPromptId === promptId) ||
        ...
        writeStderrLine(`broadcastPromptCancelledOnce: suppressed duplicate cancel for session ${sessionId} prompt=${promptId ?? 'none'}`);
        return;
    }
    ...
    broadcastPromptCancelled(entry, sessionId, promptId, originatorClientId, reason);
}
...
export function classifyTurnErrorKind(message) {
...
function broadcastTurnError(entry, sessionId, err, promptId, originatorClientId, mutateTurnState) {
    ...
    if (errorKind) {
        writeServeDebugLine(`turn_error classified session=${JSON.stringify(sessionId)} ` +
...
function publishPromptTerminal(entry, pendingEntry, terminal) {
    if (pendingEntry.terminalPublished) {
        ...
        writeServeDebugLine(`publishPromptTerminal: suppressed duplicate ${terminal.kind} terminal ` +
    ...
    if (terminal.kind === 'complete') {
        broadcastTurnComplete(entry, entry.sessionId, terminal.result, pendingEntry.promptId, originatorClientId);
    }
    else if (terminal.kind === 'cancelled') {
        broadcastTurnComplete(entry, entry.sessionId, { stopReason: 'cancelled' }, pendingEntry.promptId, originatorClientId);
    }
    else {
        broadcastTurnError(entry, entry.sessionId, terminal.err, pendingEntry.promptId, originatorClientId, 
...
function flushPromptTerminals(entry, code, message) {
    for (const pending of [...entry.pendingPromptList]) {
        ...
        try {
            pending.abortController.abort(new DOMException('Prompt aborted', 'AbortError'));
        }
        catch {
...
export function createAcpSessionBridge(opts) {
    ...
    const assertFreshSessionsAvailable = () => {
        ...
        if (blocker) {
            throw new BridgeChannelQuarantinedError(blocker.reason, abandonedRestoreRetryAfterSeconds);
        }
    };
    const reserveFreshSession = (context) => {
        assertFreshSessionsAvailable();
        return opts.freshSessionAdmission?.(context);
    };
    const releaseFreshSessionReservation = (reservation) => {
        if (!reservation)
            return;
        try {
            reservation.release();
    ...
    const compactedReplayMaxBytes = normalizeCompactedReplayMaxBytes(opts.compactedReplayMaxBytes);
    ...
    const abandonedRestoreRetryAfterSeconds = restoreRetryAfterSeconds(sessionRestoreTimeoutMs);
    ...
    if (maxPendingPromptsRaw === 0 ||
        maxPendingPromptsRaw === Number.POSITIVE_INFINITY) {
        maxPendingPromptsPerSession = Infinity;
    }
    else if (!Number.isInteger(maxPendingPromptsRaw) ||
        maxPendingPromptsRaw < 0) {
        throw new TypeError(`Invalid maxPendingPromptsPerSession: ${maxPendingPromptsRaw}. ` +
    ...
    let idleTimer;
    const sessionReapIntervalMs = resolvePositiveFiniteMs(opts.sessionReapIntervalMs, DEFAULT_SESSION_REAP_INTERVAL_MS);
    const sessionIdleTimeoutMs = resolvePositiveFiniteMs(opts.sessionIdleTimeoutMs, DEFAULT_SESSION_IDLE_TIMEOUT_MS);
    let sessionReaper;
    ...
    function childHoldsAreFresh(entry, capability) {
        if (entry.childHoldsAt === null)
            return false;
        return (Date.now() - entry.childHoldsAt <=
    ...
    function childReportsHeldWork(entry) {
        ...
        if (!owner?.activeWork)
            return false;
        if (!childHoldsAreFresh(entry, owner.activeWork))
            return false;
    ...
    function entryHasActiveWork(entry) {
        return (entryHasLocalWork(entry) ||
    ...
    function entryIsAutoCloseCandidate(entry) {
        if (byId.get(entry.sessionId) !== entry)
            return false;
        if (isClosingOrAuthorizingClose(entry))
            return false;
        if (entry.events.subscriberCount > 0)
            return false;
        if (entryHasLocalWork(entry))
            return false;
        ...
        const owner = channelInfoForEntry(entry);
        if (owner?.pendingRestoreIds.has(entry.sessionId))
            return false;
    ...
    async function maybeCloseIdleSession(entry, reason) {
        if (!entryIsAutoCloseCandidate(entry))
            return;
     ...
     */
    async function closeIfChildUnheld(entry, opts) {
        entry.activeWorkCloseInFlight = true;
        try {
            if (!(await confirmChildUnheld(entry)))
                return;
            ...
            const agentCloseTimeoutMs = condemnedOwner?.isQuarantined === true ||
            ...
            await closeSessionImpl(entry.sessionId, undefined, {
                ...
                writeStderrLine(`qwen serve: deferred close (${opts.trigger}) failed for ` +
    ...
    async function confirmChildUnheld(entry) {
        ...
        if (info.isQuarantined || info.restoreSettlementOverdue)
            return true;
        try {
            const response = await withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionClose, {
                sessionId: entry.sessionId,
                [ACTIVE_WORK_CLOSE_IF_UNHELD_PARAM]: true,
            }), ACTIVE_WORK_CLOSE_TIMEOUT_MS, SERVE_CONTROL_EXT_METHODS.sessionClose);
            ...
            if (Array.isArray(holds)) {
                ...
                for (const hold of holds) {
                    ...
                    if (typeof id === 'string' &&
                        ...
                        ACTIVE_WORK_HOLD_CATEGORIES.includes(category)) {
            ...
            writeStderrLine(`qwen serve: close-if-unheld for session ${JSON.stringify(entry.sessionId)} ` +
    ...
    function applyActiveWorkSnapshot(info, snapshot) {
        if (!info.activeWork || info.isDying)
            return;
        ...
        // lost before it.
        if (snapshot.seq <= info.activeWork.seq)
            return;
        ...
        for (const session of snapshot.sessions) {
            ...
            for (const hold of session.holds)
                holds.set(hold.id, hold.category);
        ...
        for (const sessionId of Array.from(info.sessionIds)) {
            ...
            if (!entry || entry.channel !== info.channel)
                continue;
            ...
            if (holds.size === 0) {
                void maybeCloseIdleSession(entry, reported.has(sessionId) ? 'child_idle' : 'child_dropped');
            }
        }
    }
    /**
    ...
    function settleActivePromptState(entry, promptId) {
        if (entry.activePromptId !== promptId)
            return;
    ...
    async function killChannelWithLog(ci, context) {
        ...
        await ci.channel.kill().catch((err) => {
            writeStderrLine(`qwen serve: channel kill failed${context ? ` (${context})` : ''}: ${String(err)}`);
        });
    }
    ...
    async function startIdleTimer(ci, context) {
        ...
        idleTimer = setTimeout(() => {
            ...
            if (hasNoChannelWork(ci)) {
                writeStderrLine(`qwen serve: idle timeout (${timeoutMs}ms) expired, killing channel`);
                void killChannelWithLog(ci, 'idle timeout');
            }
        }, timeoutMs);
        idleTimer.unref();
    }
    function hasNoChannelWork(ci, opts) {
        const inFlightSpawnCount = ci.sessionSpawnsInFlight -
            (opts?.ignoreCurrentSessionSpawn === true ? 1 : 0);
        const pendingRestoreCount = ci.pendingRestoreIds.size -
            (opts?.ignoreRestoreId !== undefined &&
                ci.pendingRestoreIds.has(opts.ignoreRestoreId)
                ? 1
                : 0);
        return (ci.sessionIds.size === 0 &&
    ...
    function armRestoreSettlementGrace(ci, sessionId, action) {
        if (ci.restoreSettlementTimers.has(sessionId))
            return;
        const timer = setTimeout(() => {
            ci.restoreSettlementTimers.delete(sessionId);
            if (!ci.unsettledAbandonedRestores.has(sessionId))
                return;
            if (ci.isDying || !aliveChannels.has(ci))
                return;
            ci.restoreSettlementOverdue = true;
            writeStderrLine(`qwen serve: abandoned session/${action} for ${JSON.stringify(sessionId)} has not settled ` +
    ...
    async function reapPendingEmptyChannel(ci) {
        if (!channelShouldReapWhenIdle(ci) || !hasNoChannelWork(ci))
            return;
    ...
    async function withWorkspaceControl(ci, fn) {
        ...
        try {
            ...
            ci.workspaceControlInFlight = Math.max(0, ci.workspaceControlInFlight - 1);
    ...
    function startSessionReaper() {
        ...
        writeStderrLine(`qwen serve: session reaper started ` +
        ...
        sessionReaper = setInterval(() => {
            ...
            for (const [id, entry] of byId) {
                ...
                if (idle < sessionIdleTimeoutMs)
                    continue;
                writeStderrLine(`qwen serve: reaping idle session ${JSON.stringify(id)} ` +
    ...
    const byId = new Map();
    const forwardRunningPromptCancel = async (entry, pending, notification) => {
        ...
        const initial = (async () => {
            try {
                ...
                if (typeof result['cancelled'] !== 'boolean') {
                    throw new Error(`${PROMPT_CANCEL_METHOD} returned an invalid acknowledgement`);
                }
            }
            catch (error) {
    ...
    const clearInFlightExtensionRefreshes = (connection) => {
    ...
    if (permissionConsensusQuorum !== undefined &&
        (!Number.isInteger(permissionConsensusQuorum) ||
            permissionConsensusQuorum < 1)) {
        throw new Error(`BridgeOptions.permissionConsensusQuorum must be a positive integer; ` +
    ...
    const permissionMediator = new MultiClientPermissionMediator(opts.permissionPolicy ?? 'first-responder', {
    ...
    // registration window.
    const inFlightRequestedSessionSpawns = new Map();
    ...
    const recordAttachRef = (entry, clientId) => {
    ...
    const releaseAttachRef = (entry, clientId) => {
    ...
    const rollbackAttachRegistration = async (entry, clientId, attachCountDelta = 1) => {
        ...
        entry.attachCount = Math.max(0, entry.attachCount - (released + (attachCountDelta - 1)));
    ...
    async function ensureChannel() {
        ...
        const promise = (async () => {
            ...
            const channel = await telemetry.withSpan('channel.spawn', {
            ...
            try {
                ...
                // unreachable.
                opts.clientMcpSender, (sessionId) => sessionIds.has(sessionId), 
                ...
                (inputTokens, outputTokens, durationMs, apiErrors, apiRetries) => telemetry.metrics?.tokenUsage?.(inputTokens, outputTokens, durationMs, apiErrors, apiRetries), 
                ...
                opts.onCreateSubSession, (sessionId, event) => {
                    const request = generationRequests.get(event.requestId);
                    if (!request || request.sessionId !== sessionId)
                        return;
                    if (request.queue.push(event))
                        return;
                    ...
                    void request.connection
                        ...
                        .catch(() => undefined);
                }, opts.onChannelDelivery, () => channelInfo?.sessionIds === sessionIds &&
                    channelInfo.sessionSpawnsInFlight > 0, () => liveScreenContextCaptureHandler, () => liveTaskToolRequestHandler, () => liveSpeakToUserHandler, opts.externalToolGuard, (snapshot) => {
                    const currentInfo = infoRef.current;
                    if (!currentInfo)
                        return;
                    applyActiveWorkSnapshot(currentInfo, snapshot);
                });
                const rawConnection = new ClientSideConnection(() => channel.transportGuard
                ...
                try {
                    ...
                    throw new AggregateError([error, teardownError], 'ACP channel construction and teardown failed');
                }
                throw error;
            }
            ...
            const markTransportFailed = (error) => {
                if (!info.isDying) {
            ...
            void channel.transportFailed?.then(markTransportFailed, markTransportFailed);
            ...
            void channel.exited.then((exitInfo) => {
                ...
                if (!shuttingDown) {
                    ...
                    writeStderrLine(`qwen serve: channel exited (code=${exitInfo?.exitCode ?? 'none'}, signal=${exitInfo?.signalCode ?? 'none'}, transport=${info.transportFailed ? (info.transportFailureCode ?? 'failed') : 'ok'}, ${sessions.length} session(s) torn down)`);
                }
                for (const sid of sessions) {
                    ...
                    flushPromptTerminals(sessEntry, 'channel_closed', 'agent channel exited before the prompt completed');
                    try {
                ...
                await telemetry.withSpan('channel.initialize', {
                    ...
                    if (opts.externalToolGuard) {
                        ...
                        if (guardAck !== EXTERNAL_TOOL_GUARD_REQUIRED_VALUE) {
                            throw new Error(`ACP child did not acknowledge the required external tool guard (received: ${JSON.stringify(guardAck)}).`);
                        }
                    }
                    const activeWorkCapability = isRecord(response._meta)
                        ? response._meta[ACTIVE_WORK_HEARTBEAT_META_KEY]
                        : undefined;
                    if (isRecord(activeWorkCapability) &&
                        ...
                        info.activeWork = {
                            ...
                            categories: Array.isArray(advertised)
                                ? ACTIVE_WORK_HOLD_CATEGORIES.filter((category) => advertised.includes(category))
                                : [],
                            seq: 0,
                        };
                    }
                    try {
                        const attributes = getChannelStartupProfileAttributes(response, Date.now(), initTimeoutMs);
    ...
    async function doSpawn(modelServiceId, effectiveScope, approvalMode, requestedClientId, onSessionRegistered, parentSessionId, sourceType, sourceId, worktree, branch, requestedSessionId) {
        ...
        const channelPath = channelInfo && !channelInfo.isDying
            ? 'reused'
            : inFlightChannelSpawn
                ? 'joined'
                : 'spawned_on_request';
        const ci = await telemetry.withSpan('channel.wait', {
        ...
        try {
            try {
                newSessionResp = await telemetry.withSpan('session.new', {
                    ...
                    const response = await withTimeout(Promise.race([
                        ci.connection.newSession(worktree
                            ? {
                                ...request,
                                _meta: {
            ...
            const entry = createSessionEntry(ci, newSessionResp.sessionId, boundWorkspace, undefined, { parentSessionId, sourceType, sourceId, worktree, branch });
                ...
                for (let attempt = 1; attempt <= MAX_PARENT_PERSIST_ATTEMPTS; attempt++) {
                    ...
                    try {
                        const parentResult = await Promise.race([
                            withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionParent, {
                                sessionId: entry.sessionId,
                                parentSessionId: entry.parentSessionId,
                            }), remaining, 'sessionParent'),
                            getTransportClosedReject(entry),
                        ]);
                        ...
                        const terminal = err instanceof BridgeTimeoutError ||
            ...
            if (entry.sourceType) {
                try {
                    const sourceResult = await Promise.race([
                        withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionSource, {
                            sessionId: entry.sessionId,
                            sourceType: entry.sourceType,
                            ...(entry.sourceId !== undefined
                                ? { sourceId: entry.sourceId }
                                : {}),
                        }), initTimeoutMs, 'sessionSource'),
                        getTransportClosedReject(entry),
                    ]);
                    sourcePersisted =
                        sourceResult?.persisted ===
                            true;
                }
                catch (err) {
            ...
            if (!byId.has(entry.sessionId)) {
                throw new Error(`Session ${entry.sessionId} died during session initialization`);
            }
            ...
            if (requestedSessionId !== undefined) {
                ...
                if (initializedSessionId !== requestedSessionId &&
            ...
            if (!sessionRegistered) {
                ...
                await reapPendingEmptyChannel(ci);
                if (!ci.isDying) {
                    await startIdleTimer(ci, `approval-mode initialization failure "${initializedSessionId}"`);
                }
            }
            else if (sessionRegistered && hasNoChannelWork(ci) && !ci.isDying) {
                await startIdleTimer(ci, `session orphaned during initialization "${initializedSessionId}"`);
            }
        }
    }
    /**
        ...
        const work = entry.modelChangeQueue.then(async () => {
            ...
            try {
                ...
                if (succeeded) {
                    ...
                    writeStderrLine(`[reconcile] session=${entry.sessionId} target=model action=skipped reason=roundtrip_failed`);
                }
            }
        });
        ...
        const approvalWork = entry.approvalModeQueue.then(async () => {
            ...
            try {
                const response = (await Promise.race([
                    withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionApprovalMode, { sessionId: entry.sessionId, mode }), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.sessionApprovalMode),
                    getTransportClosedReject(entry),
                ]));
                if (typeof response.current !== 'string' ||
                    !KNOWN_APPROVAL_MODES.has(response.current)) {
                    throw new Error(`Agent returned unknown approval mode: ${JSON.stringify(response.current)}`);
                }
                let persisted = false;
                if (persist) {
                    try {
                        await withTimeout(persistApprovalMode?.(boundWorkspace, mode) ?? Promise.resolve(), PERSIST_TIMEOUT_MS, 'persistApprovalMode');
                ...
                publishApprovalModeChanged(entry, {
                ...
                if (succeeded) {
                    ...
                    writeStderrLine(`[reconcile] session=${entry.sessionId} target=approvalMode action=skipped reason=roundtrip_failed`);
                }
            }
        });
        entry.approvalModeQueue = approvalWork.then(() => undefined, () => undefined);
        try {
    ...
    async function rollbackApprovalModeForRejectedAttach(entry, previous, clientId) {
        ...
        try {
            ...
            writeStderrLine(`attach: failed to restore approval mode for session ${entry.sessionId}: ${extractErrorMessage(err)}`);
        }
    }
    /**
    ...
    const getTransportClosedReject = (entry) => {
        if (!entry.transportClosedReject) {
            entry.transportClosedReject = channelUnavailableReject(entry.channel, `mid-request (session ${entry.sessionId})`);
        }
        return entry.transportClosedReject;
    };
    const resolveWorkspaceKey = (rawWorkspaceCwd) => {
        ...
        if (workspaceCwd === null) {
            throw new Error(`workspaceCwd must be an absolute path; got "${rawWorkspaceCwd}"`);
        }
    ...
    const channelInfoForEntry = (entry) => {
        if (channelInfo?.channel === entry.channel)
            return channelInfo;
    ...
    const assertAttachableSessionEntry = (sessionId, entry) => {
        if (byId.get(sessionId) !== entry) {
            throw new SessionNotFoundError(sessionId, 'The session channel is unavailable; retry after teardown completes');
        }
        if (isClosingOrAuthorizingClose(entry)) {
            throw new SessionNotFoundError(sessionId, 'The session is closing; retry after close completes', 'session_closing');
        }
        const owner = channelInfoForEntry(entry);
    ...
    const assertLivePromptEntry = (sessionId, entry) => {
        const info = channelInfoForEntry(entry);
    ...
    const getChannelClosedReject = (info) => {
        if (!info.statusClosedReject) {
            info.statusClosedReject = channelUnavailableReject(info.channel, 'mid-request (workspace status)');
        }
        return info.statusClosedReject;
    };
    const cacheWorkspaceMcpDetails = async (info, status) => {
        if (!Array.isArray(status.servers))
            return;
        const serverNames = status.servers.flatMap((server) => isRecord(server) &&
            typeof server['name'] === 'string' &&
            server['mcpStatus'] === 'connected'
            ? [server['name']]
            : []);
        const cacheDetail = async (serverName, method, cache) => {
            try {
        ...
        await Promise.all(serverNames.flatMap((serverName) => [
            cacheDetail(serverName, SERVE_STATUS_EXT_METHODS.workspaceMcpTools, workspaceMcpToolsCache),
            cacheDetail(serverName, SERVE_STATUS_EXT_METHODS.workspaceMcpResources, workspaceMcpResourcesCache),
        ]));
    };
    const mergeManagedWorkspaceMcpStatus = (serverNames, previous, current) => {
        if (Array.isArray(current.servers) &&
            ...
            if (current.servers.length === 0)
                return previous;
            const currentServers = new Map(current.servers.map((server) => [server.name, server]));
            const previousNames = new Set(previous.servers.map((server) => server.name));
            const servers = previous.servers.map((server) => serverNames.has(server.name)
    ...
    const requestWorkspaceStatus = async (method, idle, params = {}, managedServerNames) => {
        ...
        let response = await withTimeout(Promise.race([
        ...
        if (method === SERVE_STATUS_EXT_METHODS.workspaceMcp) {
            ...
            if (effectiveManagedServerNames.size > 0 ||
                (workspaceMcpStatusCache?.discoveryState === 'completed' &&
                    ...
                    rawServers.length === 0)) {
                response = mergeManagedWorkspaceMcpStatus(effectiveManagedServerNames, workspaceMcpStatusCache, rawStatus);
            }
            const status = response;
            if (status.discoveryState === 'completed') {
                await cacheWorkspaceMcpDetails(info, effectiveManagedServerNames.size > 0
                    ? {
                        servers: rawServers.filter((server) => effectiveManagedServerNames.has(server.name)),
                    }
                    : status);
            }
            if (info.workspaceMcpDiscoveryInFlight &&
            ...
            for (const serverName of info.workspaceMcpAuthenticationServerNames) {
                const server = rawServers.find((candidate) => candidate.name === serverName);
    ...
    const refreshChildResource = async () => {
        if (childResourceRefreshing)
            return;
        const info = liveChannelInfo();
        if (!info)
            return;
        ...
        try {
            ...
            teeServeDebugLine(`child-resource refresh failed: ${err instanceof Error ? err.message : String(err)}`);
        }
        finally {
    ...
    const getChildResourceSnapshot = () => {
        const info = liveChannelInfo();
        if (!info || info.childResourceAt === undefined)
            return undefined;
    ...
    const requestSessionStatus = async (sessionId, method, params = {}, timeoutMs = initTimeoutMs) => {
        const entry = byId.get(sessionId);
        if (!entry)
            throw new SessionNotFoundError(sessionId);
        const info = channelInfoForEntry(entry);
        if (!info || info.isDying)
            throw new SessionNotFoundError(sessionId);
        const response = await Promise.race([
            withTimeout(entry.connection.extMethod(method, { ...params, sessionId }), timeoutMs, method),
            getTransportClosedReject(entry),
        ]);
        return response;
    };
    const notifyAgentSessionClose = async (entry, ci, label, opts) => {
        if (!ci || ci.channel !== entry.channel) {
            if (opts?.throwOnFailure === true) {
                writeStderrLine(`qwen serve: ${label} ACP session close channel unavailable ` +
                    `for session ${JSON.stringify(entry.sessionId)}; agent close skipped`);
                throw new Error(`ACP session close channel unavailable for ${entry.sessionId}`);
            }
            return false;
        }
        try {
            const closeRequest = entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionClose, {
                sessionId: entry.sessionId,
            ...
            const response = await Promise.race([
                opts?.throwOnFailure === true
                    ...
                    : withTimeout(observedCloseRequest, initTimeoutMs, SERVE_CONTROL_EXT_METHODS.sessionClose),
                getTransportClosedReject(entry),
            ]);
            return response['closed'] === true;
        }
        catch (err) {
            writeStderrLine(`qwen serve: ${label} ACP session close notification failed ` +
                `for session ${JSON.stringify(entry.sessionId)}: ${String(err instanceof Error ? err.message : err)}`);
            if (opts?.throwOnFailure === true) {
                throw err;
            }
            return false;
     ...
     * its own bus).
     */
    const broadcastWorkspaceEvent = (envelope, skipSessionId) => {
        ...
        for (const entry of sessions) {
            ...
            try {
                ...
                if (published === undefined) {
                    failureCount += 1;
                    teeServeDebugLine(`broadcastWorkspaceEvent: publish on session ${entry.sessionId} no-op (bus closed or unserializable)`);
                }
                else {
                    successCount += 1;
                }
                ...
                failureCount += 1;
                const detail = `broadcastWorkspaceEvent: bus publish failed for session ` +
        ...
        if (eligible > 0 && successCount === 0 && !shuttingDown) {
            writeStderrLine(`qwen serve: broadcastWorkspaceEvent type=${envelope.type} dropped on ALL ${failureCount} session bus(es); SSE subscribers will miss this event (GET fallback still authoritative)`);
        }
    };
    const createSessionEventBus = (sessionId) => new EventBus(eventRingSize, undefined, new TurnBoundaryCompactionEngine({
        maxReplayBytes: compactedReplayMaxBytes,
        maxJournalEvents,
        maxJournalBytes,
        onReplayWindowEviction: (eviction) => {
            teeServeDebugLine(`replay window evicted ${JSON.stringify(eviction)}`);
        },
    }), {
        ...
        onCompactionError: (err) => {
            writeStderrLine(`qwen serve: compaction degraded for session=${JSON.stringify(sessionId)}; replay snapshot may lag behind live events: ${err instanceof Error ? err.message : String(err)}`);
        },
    });
    ...
    const publishModelSwitched = (entry, modelId, originatorClientId) => {
    ...
    const publishApprovalModeChanged = (entry, payload, originatorClientId) => {
    ...
    const reconcileAfterRoundtrip = async (entry, target) => {
        const flagKey = target === 'model'
            ? 'modelReconciliationInFlight'
            : 'approvalModeReconciliationInFlight';
        const genOf = () => target === 'model'
            ? entry.modelPublishGeneration
            : entry.approvalModePublishGeneration;
        if (entry[flagKey])
            return;
        ...
        let rerun = false;
        try {
            const status = await requestSessionStatus(entry.sessionId, SERVE_STATUS_EXT_METHODS.sessionContext);
            if (genOf() !== genBefore) {
                ...
                rerun = true;
                writeStderrLine(`[reconcile] session=${entry.sessionId} target=${target} action=skipped reason=generation_changed genBefore=${genBefore} genAfter=${genOf()}`);
                return;
            }
            if (target === 'model') {
                ...
                if (typeof actual === 'string' &&
                    ...
                    writeStderrLine(`[reconcile] session=${entry.sessionId} target=model action=corrected cached=${entry.currentModelId ?? '<unset>'} actual=${actual}`);
                ...
                if (actual && !KNOWN_APPROVAL_MODES.has(actual)) {
                    writeStderrLine(`[reconcile] session=${entry.sessionId} target=approvalMode action=dropped reason=unknown_mode mode=${actual}`);
                }
                else if (actual && actual !== entry.currentApprovalMode) {
                    writeStderrLine(`[reconcile] session=${entry.sessionId} target=approvalMode action=corrected cached=${entry.currentApprovalMode ?? '<unset>'} actual=${actual}`);
                    publishApprovalModeChanged(entry, {
            ...
            writeStderrLine(`[reconcile] session=${entry.sessionId} target=${target} action=failed error=${err instanceof Error ? err.message : String(err)}`);
        }
        finally {
            entry[flagKey] = false;
            if (rerun)
                void reconcileAfterRoundtrip(entry, target);
        }
    };
    const createSessionEntry = (ci, sessionId, workspaceCwd, events = createSessionEventBus(sessionId), options = {}) => {
        const entry = {
            sessionId,
            workspaceCwd,
    ...
    const publishArtifactChanges = (entry, changes, originatorClientId) => {
    ...
    const artifactReseedChanges = (before, after) => {
    ...
    const makeClientArtifactInput = (artifact, clientId) => {
        const input = {
    ...
    function createSessionArtifactPersistence(connection, sessionId) {
        return {
            recordEvent: async (payload) => {
                await connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionArtifactsPersist, {
                    sessionId,
                    kind: 'event',
                    payload,
                });
    ...
    const seedSnapshotCaches = (entry, resp) => {
        const model = resp.models?.currentModelId;
        if (typeof model === 'string' && model.length > 0) {
            entry.currentModelId = model;
        }
        else if (model != null) {
            writeStderrLine(`[seed] session=${entry.sessionId} target=model action=dropped value=${JSON.stringify(model)} reason=invalid_type`);
        }
        ...
        if (typeof mode === 'string' && KNOWN_APPROVAL_MODES.has(mode)) {
            ...
            writeStderrLine(`[seed] session=${entry.sessionId} target=approvalMode action=dropped value=${JSON.stringify(mode)} reason=${typeof mode !== 'string' ? 'invalid_type' : 'unknown_mode'}`);
        }
    };
    ...
    const replayFieldsFor = (entry, action) => {
    ...
    const restoredArtifactSnapshotFromState = (state) => {
        const candidate = state.artifactSnapshot;
        const warnings = [];
        const snapshot = normalizeSnapshotPayload(candidate, warnings);
        if (!snapshot)
            return undefined;
        const snapshotWarnings = isRecord(candidate) && Array.isArray(candidate['warnings'])
            ? candidate['warnings']
                .filter((warning) => typeof warning === 'string' && warning.length <= 1000)
                .slice(-500)
            : [];
        return {
    ...
    const artifactSnapshotUnavailableReason = (state) => {
        const reason = state.artifactSnapshotUnavailable;
        return typeof reason === 'string' && reason ? reason : undefined;
    };
    const publicRestoreState = (state) => {
        const { artifactSnapshot: _artifactSnapshot, artifactSnapshotUnavailable: _artifactSnapshotUnavailable, ...publicState } = state;
        return publicState;
    };
    async function requestSessionTranscriptPage(req) {
        const info = await ensureChannel();
        try {
            const response = await withWorkspaceControl(info, () => withTimeout(Promise.race([
                info.connection.extMethod(SERVE_STATUS_EXT_METHODS.sessionTranscript, { ...req, cwd: boundWorkspace }),
                getChannelClosedReject(info),
            ]), Math.max(initTimeoutMs, SESSION_TRANSCRIPT_TIMEOUT_MS), SERVE_STATUS_EXT_METHODS.sessionTranscript));
            return response;
        }
        catch (err) {
    ...
    async function refreshedReplayFieldsFor(entry, historyPageSize) {
    ...
    function transcriptEventRecordId(event) {
        ...
        const meta = update && typeof update === 'object' && !Array.isArray(update)
            ? update['_meta']
            : rec['_meta'];
        if (!meta || typeof meta !== 'object' || Array.isArray(meta))
            return undefined;
    ...
    async function resolveHistoryAnchorRecordId(entry, replayFields) {
        ...
        const hasRecordId = events.some((e) => transcriptEventRecordId(e) !== undefined ||
    ...
    async function restoreSession(action, req, options = {}) {
        ...
        if (req.approvalMode !== undefined &&
            ...
            throw new Error(`Invalid approvalMode: ${JSON.stringify(req.approvalMode)}`);
        }
        const historyReplay = action === 'load' ? (req.historyReplay ?? 'stream') : 'stream';
        const historyPageSize = action === 'load' && historyReplay === 'response'
            ? req.historyPageSize
            : undefined;
        if (historyPageSize !== undefined &&
            (!Number.isSafeInteger(historyPageSize) ||
                historyPageSize < 1 ||
                historyPageSize > SESSION_TRANSCRIPT_MAX_LIMIT)) {
            throw new Error(`Invalid historyPageSize; expected 1..${SESSION_TRANSCRIPT_MAX_LIMIT}`);
        }
        ...
        if (existing) {
            ...
            const replayFields = historyPageSize !== undefined
            ...
            const historyAnchorRecordId = action === 'load'
            ...
            if (req.approvalMode) {
                const previousApprovalMode = await applyApprovalModeForAttach(existing, req.approvalMode, clientId);
                try {
                    assertAttachableSessionEntry(req.sessionId, existing);
                }
                catch (error) {
                    await rollbackApprovalModeForRejectedAttach(existing, previousApprovalMode, clientId);
        ...
        if (inFlight) {
            ...
            if (inFlight.lifecycle.phase === 'abandoned' ||
                ...
                throw new RestoreInProgressError(req.sessionId, inFlight.action, action, abandoned
                    ? {
            ...
            if (req.approvalMode) {
                const previousApprovalMode = await applyApprovalModeForAttach(entry, req.approvalMode, clientId);
                try {
                    assertAttachableSessionEntry(restored.sessionId, entry);
                }
                catch (error) {
                    await rollbackApprovalModeForRejectedAttach(entry, previousApprovalMode, clientId);
                    await rollbackAttachRegistration(entry, clientId);
                    throw error;
                }
            }
        ...
        const admission = options.skipFreshSessionAdmission === true
            ? undefined
            : reserveFreshSession({
                operation: action,
        ...
        const settleAbandonedRestore = async (channel, lateResult) => {
            ...
            if (usurper) {
                writeStderrLine(`qwen serve: skipping abandoned session/${action} cleanup for ${JSON.stringify(req.sessionId)}: the id is now owned by a live session`);
                ...
                const reclaimedTimer = channel.restoreSettlementTimers.get(req.sessionId);
            ...
            try {
                ...
                try {
                    await Promise.race([
                        withTimeout(channel.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionClose, {
                            sessionId: req.sessionId,
                            drainTimeoutMs: Math.max(1, Math.floor(initTimeoutMs * 0.8)),
                        }), initTimeoutMs, 'abandonedRestoreClose'),
                    ...
                    writeStderrLine(`qwen serve: quarantining ACP channel after timed-out session/${action} cleanup failed for ${JSON.stringify(req.sessionId)}: ${extractErrorMessage(error)}`);
                    telemetry.event('session.restore.cleanup', {
                        'qwen-code.daemon.session_restore.action': action,
                    ...
                    if (hasNoChannelWork(channel)) {
                        void killChannelWithLog(channel, `abandoned session/${action} cleanup`);
                    }
                    await channel.channel.exited;
                }
            }
        ...
        const promise = (async () => {
            ...
            const transportClosed = channelUnavailableReject(restoreChannel.channel, `during session/${action}`);
            ...
            let replayHasMore;
            try {
                const rawRestore = telemetry.withSpan('session.restore', {
                    ...
                    return await restoreChannel.connection.unstable_resumeSession(request);
                });
                ...
                state = await new Promise((resolve, reject) => {
                    const timer = setTimeout(() => {
                        ...
                        writeStderrLine(`qwen serve: session/${action} timed out after ${sessionRestoreTimeoutMs}ms for ${JSON.stringify(req.sessionId)} on channel ${restoreChannel.id}; decision=${channelWasEmpty ? 'kill_empty' : 'fence_shared'}`);
                        if (channelWasEmpty) {
                            void killChannelWithLog(restoreChannel, `timed-out session/${action} on empty channel`);
                        }
                        else {
                            armRestoreSettlementGrace(restoreChannel, req.sessionId, action);
                        }
                        reject(new SessionRestoreTimeoutError(req.sessionId, action, sessionRestoreTimeoutMs));
                    }, sessionRestoreTimeoutMs);
                    timer.unref();
                    void observedRestore.then((value) => {
                        if (restoreLifecycle.phase === 'active') {
                            clearTimeout(timer);
                            resolve(value);
                            return;
            ...
            if (racedEntry) {
                ...
                if (req.approvalMode) {
                    ...
                    try {
                        const result = await applyApprovalMode(racedEntry, req.approvalMode, false, clientId);
                        previousApprovalMode = result.previous;
                    }
                    catch (err) {
                        await rollbackAttachRegistration(racedEntry, clientId, 1 + coalesceState.count);
                        throw err;
                    }
                    try {
                        assertAttachableSessionEntry(req.sessionId, racedEntry);
                        ...
                        await rollbackApprovalModeForRejectedAttach(racedEntry, previousApprovalMode, clientId);
                        await rollbackAttachRegistration(racedEntry, clientId, 1 + coalesceState.count);
                        throw error;
                    }
                }
            ...
            const artifactRestoreWarnings = await entry.artifacts.restore(restoredArtifactSnapshot);
            for (const warning of artifactRestoreWarnings) {
                writeStderrLine(`[artifacts] session=${entry.sessionId} action=restore_warning warning=${JSON.stringify(warning)}`);
            }
            const artifactRestoreFailed = artifactRestoreWarnings.some((warning) => isArtifactRestoreFailureWarning(warning));
            if (replayUpdates.length > 0) {
                await ci.client.seedSessionUpdates(entry, replayUpdates, {
                    ingestArtifacts: restoredArtifactSnapshot === undefined || artifactRestoreFailed,
                });
                if (historyPageSize !== undefined &&
                    entry.events
                        .snapshotReplay()
            ...
            if (restoreLifecycle.phase === 'abandoned')
                return;
            releaseAdmissionOnce();
    ...
    async function closeSessionImpl(sessionId, context, closeOpts) {
        ...
        if (!entry)
            throw new SessionNotFoundError(sessionId);
        if (entry.closing) {
            throw new SessionNotFoundError(sessionId, 'The session is already closing', 'session_closing');
        }
        ...
        if (!ci) {
            writeStderrLine(`qwen serve: closeSession channelInfoForEntry returned undefined ` +
        ...
        try {
            ...
            agentSessionClosed = await notifyAgentSessionClose(entry, ci, 'closeSession', {
                throwOnFailure: true,
            ...
            if (isDefinitiveAcpRequestError(error)) {
                ...
                await killChannelWithLog(ci, `recover unknown close outcome for session ${JSON.stringify(sessionId)}`);
            }
            else {
                entry.closing = false;
            }
        ...
        flushPromptTerminals(entry, 'session_closed', 'session closed before the prompt completed');
        try {
            entry.events.publish({
                type: 'session_closed',
                data: {
        ...
        if (!agentSessionClosed) {
            try {
                await telemetry.withSpan('session.close.cancel_active_prompt', {
    ...
    startSessionReaper();
    const rememberMidTurnId = (ring, messageId) => {
        ring.push(messageId);
        if (ring.length > MID_TURN_RECONCILIATION_RING_SIZE) {
            ring.splice(0, ring.length - MID_TURN_RECONCILIATION_RING_SIZE);
        }
    };
    const promoteMidTurnMessage = (entry, messageId, text, originatorClientId) => {
        void bridgeApi
            .sendPrompt(entry.sessionId, {
            sessionId: entry.sessionId,
            ...
            .catch((error) => {
            writeStderrLine(`[mid-turn] session=${JSON.stringify(entry.sessionId)} failed to run promoted message ${JSON.stringify(messageId)}: ${JSON.stringify(error instanceof Error ? error.message : String(error))}`);
        });
    };
    const bridgeApi = {
        ...
        getDaemonStatusSnapshot() {
            return {
                limits: {
                    ...
                    maxPendingPromptsPerSession: maxPendingPromptsPerSession === Infinity
                        ? null
        ...
        get activeWorkCoverage() {
            ...
            for (const entry of byId.values()) {
                ...
                if (ACTIVE_WORK_HOLD_CATEGORIES.some((category) => !capability.categories.includes(category))) {
                    continue;
                }
                if (!childHoldsAreFresh(entry, capability))
                    continue;
                covered++;
        ...
        async spawnOrAttach(req) {
            ...
            const effectiveScope = req.sessionId !== undefined
                ? 'thread'
            ...
            if (effectiveScope === 'single') {
                ...
                if (existing) {
                    ...
                    let previousApprovalMode;
                    if (req.approvalMode) {
                        previousApprovalMode = await applyApprovalModeForAttach(existing, req.approvalMode, clientId);
                    }
                    try {
                        assertAttachableSessionEntry(existing.sessionId, existing);
                    }
                ...
                if (inFlight) {
                    ...
                    let previousApprovalMode;
                    if (req.approvalMode) {
                        previousApprovalMode = await applyApprovalModeForAttach(attachedEntry, req.approvalMode, clientId);
                    }
                    try {
                        assertAttachableSessionEntry(session.sessionId, attachedEntry);
                    }
                        ...
                        if (previousApprovalMode !== undefined) {
                            await rollbackApprovalModeForRejectedAttach(attachedEntry, previousApprovalMode, clientId);
                        }
            ...
            if (req.sessionId !== undefined) {
                ...
                if (restoreOwner) {
                    ...
                    throw new RestoreInProgressError(req.sessionId, restoreOwner.action, 'spawn', abandoned
                        ? {
                            reason: 'awaiting_abandoned_cleanup',
                            retryAfterSeconds: abandonedRestoreRetryAfterSeconds,
                        }
            ...
            const requestedSessionRegistrationOwner = req.sessionId !== undefined ? Symbol(req.sessionId) : undefined;
            if (req.sessionId !== undefined &&
                requestedSessionRegistrationOwner !== undefined) {
                inFlightRequestedSessionSpawns.set(req.sessionId, requestedSessionRegistrationOwner);
            }
            ...
            const promise = doSpawn(req.modelServiceId, effectiveScope, req.approvalMode, req.clientId, releaseAdmissionOnce, req.parentSessionId, source.sourceType, source.sourceId, req.worktree, req.branch, req.sessionId);
        ...
        sendPrompt(sessionId, req, signal, context) {
            opts.onDiagnosticLine?.(`qwen serve: bridge sendPrompt for session=${sessionId}`, 'info');
            ...
            if (isClosingOrAuthorizingClose(entry)) {
                return Promise.reject(new SessionNotFoundError(sessionId, 'The session is closing; retry after close completes', 'session_closing'));
            }
            ...
            if (modelPrompt !== undefined &&
                ...
                throw new TypeError('Bridge modelPrompt must be a non-empty bounded string.');
            }
            ...
            if (!isPromotedMidTurn &&
                entry.pendingPromptCount >= maxPendingPromptsPerSession) {
                throw new PromptQueueFullError(maxPendingPromptsPerSession, entry.pendingPromptCount, sessionId);
            }
            ...
            if (signal) {
                if (signal.aborted) {
                    ...
                    signal.addEventListener('abort', () => pendingAbort.abort(signal.reason), { once: true });
                }
            }
            const channelDisplayText = getChannelPromptDisplayText(entry, context?.promptDisplayText);
            const pendingText = channelDisplayText === undefined
                ? extractPromptText(req.prompt)
                : channelDisplayText ||
                    (req.prompt.some((block) => isRecord(block) && block['type'] === 'image')
                        ? '[image]'
                        : '');
            const pendingEntry = {
                promptId,
            ...
            try {
                ...
                opts.onDiagnosticLine?.(`qwen serve: prompt admission observer failed for session=${sessionId}: ${error instanceof Error ? error.message : String(error)}`, 'warn');
            }
            ...
            const hasDeadline = typeof deadlineMs === 'number' &&
            ...
            if (hasDeadline) {
                ...
                pendingEntry.cancelForwardDeadline = deadlinePromise.then(() => undefined, () => undefined);
                const onDeadline = () => {
                    if (pendingEntry.terminalPublished)
                        return;
                    const deadlineErr = new PromptDeadlineExceededError(deadlineMs);
                    writeStderrLine(`sendPrompt: prompt ${promptId} exceeded ${deadlineMs}ms deadline ` +
            ...
            if (isQueued) {
                pendingAbort.signal.addEventListener('abort', () => {
                    if (pendingEntry.state !== 'queued')
                        return;
                    const waitingOwnerPromptId = entry.todoStopGuardAwaitingQueuedPromptOwnerPromptId;
                    if (!waitingOwnerPromptId)
                        return;
                    const hasAnotherQueuedPrompt = entry.pendingPromptList.some((candidate) => candidate !== pendingEntry &&
                        candidate.state === 'queued' &&
                        !candidate.abortController.signal.aborted);
                    if (hasAnotherQueuedPrompt)
                        return;
            ...
            // wrong agent process.
            const result = entry.promptQueue.then(() => telemetry.runWithContext(capturedContext, async () => {
                ...
                try {
                    return await telemetry.withSpan('prompt.dispatch', {
                        ...
                        const promptRequest = (() => {
                            ...
                            const meta = copy._meta && typeof copy._meta === 'object'
                                ? { ...copy._meta }
                                : {};
                        ...
                        try {
                            ...
                            if (!isRetry && !isContinue) {
                                echoPromptToSessionBus(entry, promptRequest, pendingEntry.promptId, originatorClientId, channelDisplayText);
                            }
                        }
                        catch (echoErr) {
                            settleActivePromptState(entry, pendingEntry.promptId);
                        ...
                        void racedPromise
                            ...
                            if (err instanceof DOMException &&
                                ...
                                writeStderrLine(`sendPrompt: queued prompt removed before agent forward for session ${sessionId}`);
                                return;
                            }
                            writeStderrLine(`sendPrompt: forward failed for session ${sessionId}: ${extractErrorMessage(err)}`);
                            broadcastPromptCancelledOnce(entry, sessionId, pendingEntry.promptId, originatorClientId, 'forward_failed');
                        ...
                        const onAbort = () => {
                            broadcastPromptCancelledOnce(entry, sessionId, pendingEntry.promptId, originatorClientId);
                            cancelPendingForSession(sessionId);
                            if (byId.get(sessionId) === entry) {
                                void forwardRunningPromptCancel(entry, pendingEntry, {
                                    sessionId,
                                }).catch((err) => {
                                    writeStderrLine(`[pending-prompt] cancel forward failed after removePendingPrompt session=${sessionId}: ${extractErrorMessage(err)}`);
                                });
                            }
                        };
            ...
            const drainCancelForwarding = async () => {
                try {
            ...
            entry.promptQueue = result.then(drainCancelForwarding, drainCancelForwarding);
            result
                .finally(() => {
                ...
                const shouldSettleMidTurnQueue = entry.pendingPromptCount === 1 &&
                    !entry.closing &&
                ...
                for (const message of undrainedMessages) {
                    if (message.queueOnly) {
                        try {
                            ...
                            writeStderrLine(`[mid-turn] session=${JSON.stringify(entry.sessionId)} failed to hand undrained queue-only message ${JSON.stringify(message.messageId)} back to its caller: ${JSON.stringify(error instanceof Error ? error.message : String(error))}`);
                        }
                        continue;
                    }
                    promoteMidTurnMessage(entry, message.messageId, message.text, message.originatorClientId);
                }
        ...
        async cancelSession(sessionId, req, context) {
            opts.onDiagnosticLine?.(`qwen serve: bridge cancelSession for session=${sessionId}`, 'info');
            const entry = byId.get(sessionId);
            if (!entry)
                throw new SessionNotFoundError(sessionId);
            const cancelOriginatorClientId = resolveTrustedClientId(entry, context?.clientId);
            const runningPrompt = entry.pendingPromptList.find((pending) => pending.state === 'running' && !pending.terminalPublished);
            ...
            broadcastPromptCancelledOnce(entry, sessionId, entry.activePromptId ?? runningPrompt?.promptId, cancelOriginatorClientId);
            ...
            await telemetry.withSpan('session.cancel', {
                'qwen-code.daemon.bridge.operation': 'session.cancel',
                'session.id': sessionId,
            }, async () => {
                if (runningPrompt) {
                    const forwarding = runningPrompt.dispatched === true &&
                        entry.activePromptId === runningPrompt.promptId
                        ? forwardRunningPromptCancel(entry, runningPrompt, notif)
                        : Promise.resolve();
                    runningPrompt.abortController.abort(new DOMException('Prompt cancelled before dispatch', 'AbortError'));
                    await forwarding;
                    return;
                }
                try {
        ...
        respondToPermission(requestId, response, context) {
            ...
            if (sessionId === undefined || !byId.has(sessionId)) {
                ...
                writeStderrLine(`qwen serve: legacy permission vote ${JSON.stringify(requestId)} ` +
                    `has no live session (peek returned ${JSON.stringify(sessionId)}); ` +
                    `returning 404.`);
                return false;
            }
            return this.respondToSessionPermission(sessionId, requestId, response, context);
        },
        respondToSessionPermission(sessionId, requestId, response, context) {
            ...
            if (actualSessionId !== undefined && actualSessionId !== sessionId) {
                teeServeDebugLine(`rejected permission vote ${JSON.stringify(requestId)} ` +
            ...
            if (actualSessionId === undefined) {
                writeStderrLine(`qwen serve: rejected permission vote ${JSON.stringify(requestId)} ` +
            ...
            const optionId = response.outcome.outcome === 'selected'
            ...
            switch (outcome.kind) {
                ...
                case 'unknown_request':
                    ...
                    return false;
                case 'forbidden':
                    throw new PermissionForbiddenError(requestId, sessionId, outcome.reason);
                default: {
                    const _exhaustive = outcome;
                    throw new Error(`unreachable PermissionVoteOutcome: ${JSON.stringify(_exhaustive)}`);
                }
            }
        },
        async branchSession(sessionId, req, context) {
            ...
            const branchResult = (concurrentSideTask ? Promise.resolve() : entry.promptQueue).then(async () => {
                ...
                try {
                    ...
                    const ci = channelInfoForEntry(entry);
                    const result = (await withTimeout(Promise.race([
                        ci.connection.extMethod(isSideTask
                        ...
                        getTransportClosedReject(entry),
                    ]), initTimeoutMs, isSideTask ? 'createSideTaskSession' : 'branchSession'));
                    if (!result || typeof result.newSessionId !== 'string') {
                        throw new Error(`branchSession: agent returned invalid response: ${JSON.stringify(result)}`);
                    }
                    const rawBranchName = result.displayName ?? result.title;
                    const branchDisplayName = typeof rawBranchName === 'string'
                        ? rawBranchName
                    ...
                    try {
                        ...
                        try {
                            if (!ci.isDying) {
                                ...
                                ]), initTimeoutMs, 'branchSession cleanup');
                            }
                        }
                        catch (cleanupErr) {
                    ...
                    if (newEntry)
                        newEntry.displayName = branchDisplayName;
                    let sourcePersisted;
                    if (newEntry?.sourceType) {
                        try {
                            const sourceResult = await withTimeout(newEntry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionSource, {
                            ...
                            sourcePersisted = false;
                            writeStderrLine(`qwen serve: source metadata for branched session ${result.newSessionId} was not persisted: ${error instanceof Error ? error.message : String(error)}`);
                        }
                    }
                    if (!isSideTask) {
                        const eventData = {
            ...
            if (!concurrentSideTask) {
                entry.promptQueue = branchResult.then(() => undefined, () => undefined);
            }
            return branchResult;
        },
        ...
        async changeSessionCwd(sessionId, req, context) {
            ...
            const cdPromise = entry.promptQueue.then(async () => {
                ...
                const extResult = raw;
                if (typeof extResult?.previousCwd !== 'string' ||
                    typeof extResult?.newCwd !== 'string' ||
                    !Array.isArray(extResult?.warnings)) {
                    throw new Error(`changeSessionCwd: unexpected response shape from agent: ${JSON.stringify(raw)}`);
                }
            ...
            entry.promptQueue = cdPromise.then(() => undefined, () => undefined);
            entry.cwdChangeQueue = cdPromise.then(() => undefined, () => undefined);
            ...
            const result = await withTimeout(cdPromise, Math.max(initTimeoutMs, 30_000), 'changeSessionCwd');
            writeStderrLine(`qwen serve: session ${sessionId} cwd changed: ` +
        ...
        updateSessionMetadata(sessionId, metadata, context) {
            ...
            const metadataOriginatorClientId = context?.clientId !== undefined
                ? resolveTrustedClientId(entry, context.clientId)
                : undefined;
                ...
                if (entry.displayName !== nextDisplayName) {
                    ...
                    if (nextDisplayName) {
                        entry.connection
                            ...
                            .catch((err) => {
                            writeStderrLine(`qwen serve: failed to persist displayName for ${sessionId}: ${err instanceof Error ? err.message : String(err)}`);
                        });
                    }
                    try {
                        entry.events.publish({
        ...
        publishWorkspaceEvent(event) {
            ...
            for (const entry of sessions) {
                try {
                    ...
                    if (published === undefined) {
                        failureCount += 1;
                        teeServeDebugLine(`publishWorkspaceEvent: publish on session ${entry.sessionId} no-op (bus closed or unserializable)`);
                    }
                    else {
                        successCount += 1;
                    }
        ...
        async invokeWorkspaceCommand(method, params, invokeOpts) {
            ...
            try {
                ...
                const invoke = () => withTimeout(Promise.race([
                ...
                if (method === SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart &&
                    ...
                    await requestWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcp, () => {
                        throw new BridgeChannelClosedError('workspace MCP restart status refresh');
        ...
        async isWorkspaceMemoryRememberAvailable() {
            ...
            try {
                const response = await withWorkspaceControl(info, () => withTimeout(Promise.race([
                    info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMemoryRememberAvailability, { cwd: boundWorkspace }),
                    getChannelClosedReject(info),
                ]), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.workspaceMemoryRememberAvailability));
        ...
        async runWorkspaceMemoryRemember(request) {
            ...
            try {
                const response = await withWorkspaceControl(info, () => withTimeout(Promise.race([
                    info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMemoryRemember, { ...request, cwd: boundWorkspace }),
                    getChannelClosedReject(info),
                ]), WORKSPACE_MEMORY_REMEMBER_TIMEOUT_MS, SERVE_CONTROL_EXT_METHODS.workspaceMemoryRemember));
        ...
        async runWorkspaceMemoryForget(request) {
            ...
            try {
                const response = await withWorkspaceControl(info, () => withTimeout(Promise.race([
                    info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMemoryForget, { ...request, cwd: boundWorkspace }),
                    getChannelClosedReject(info),
                ]), WORKSPACE_MEMORY_REMEMBER_TIMEOUT_MS, SERVE_CONTROL_EXT_METHODS.workspaceMemoryForget));
        ...
        async runWorkspaceMemoryDream() {
            ...
            try {
                const response = await withWorkspaceControl(info, () => withTimeout(Promise.race([
                    info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMemoryDream, { cwd: boundWorkspace }),
                    getChannelClosedReject(info),
                ]), WORKSPACE_MEMORY_REMEMBER_TIMEOUT_MS, SERVE_CONTROL_EXT_METHODS.workspaceMemoryDream));
        ...
        async getWorkspaceMcpToolsStatus(serverName) {
            const result = await requestWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcpTools, () => {
        ...
        async getWorkspaceMcpResourcesStatus(serverName) {
            const result = await requestWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcpResources, () => {
        ...
        async getWorkspaceToolsStatus() {
            return requestWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceTools, () => ({
                v: STATUS_SCHEMA_VERSION,
                workspaceCwd: boundWorkspace,
                initialized: true,
                acpChannelLive: false,
        ...
        async getSessionContextUsageStatus(sessionId, opts) {
            return requestSessionStatus(sessionId, SERVE_STATUS_EXT_METHODS.sessionContextUsage, { detail: opts?.detail === true });
        },
        ...
        async getSessionTasksStatus(sessionId) {
            return requestSessionStatus(sessionId, SERVE_STATUS_EXT_METHODS.sessionTasks);
        },
        async getSessionLspStatus(sessionId) {
            return requestSessionStatus(sessionId, SERVE_STATUS_EXT_METHODS.sessionLspStatus);
        },
        ...
        async cancelSessionTask(sessionId, taskId, taskKind) {
            return requestSessionStatus(sessionId, SERVE_CONTROL_EXT_METHODS.sessionTaskCancel, { taskId, taskKind });
        },
        async clearSessionGoal(sessionId) {
            return requestSessionStatus(sessionId, SERVE_CONTROL_EXT_METHODS.sessionGoalClear);
        },
        async getSessionGoal(sessionId) {
            return requestSessionStatus(sessionId, SERVE_CONTROL_EXT_METHODS.sessionGoalGet);
        },
        async continueSession(sessionId, context) {
            ...
            const promptPromise = bridgeApi.sendPrompt(sessionId, { sessionId, prompt: [] }, undefined, {
            ...
            promptPromise.catch((err) => {
                teeServeDebugLine(`continueSession: continuation turn failed for ${sessionId}: ` +
                    `${err instanceof Error ? err.message : String(err)}`);
            });
            return {
                ...decision,
        ...
        async getSessionStatsStatus(sessionId) {
            return requestSessionStatus(sessionId, SERVE_STATUS_EXT_METHODS.sessionStats);
        },
        async getWorkspaceHooksStatus() {
            return requestWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceHooks, () => createIdleWorkspaceHooksStatus(boundWorkspace));
        },
        async getSessionHooksStatus(sessionId) {
            return requestSessionStatus(sessionId, SERVE_STATUS_EXT_METHODS.sessionHooks);
        },
        async getWorkspaceExtensionsStatus() {
            return requestWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceExtensions, () => createIdleWorkspaceExtensionsStatus(boundWorkspace));
        },
        async refreshExtensionsForAllSessions(data) {
            const sessions = Array.from(byId.values());
            const bootstrapRefreshConnections = new Set();
            const refreshSession = async (entry, refreshBootstrap) => {
                ...
                if (!inFlight ||
                    ...
                    const promise = (async () => {
                        await entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceExtensionsRefresh, {
                            sessionId: entry.sessionId,
                    ...
                    const unavailable = new Promise((_resolve, reject) => {
                        rejectUnavailable = () => reject(new BridgeChannelClosedError('refreshExtensionsForAllSessions'));
                    });
                    inFlight = {
                        connection: entry.connection,
                        promise,
                        wait: Promise.race([
                            withTimeout(promise, 30_000, SERVE_CONTROL_EXT_METHODS.workspaceExtensionsRefresh),
                            unavailable,
                        ]),
                        rejectUnavailable,
                        refreshBootstrap,
                ...
                if (created)
                    void inFlight.promise.then(clear, clear);
                await inFlight.wait;
            };
            const results = await Promise.all(sessions.map(async (entry) => {
                const info = channelInfoForEntry(entry);
                if (!info || info.isDying) {
                    return {
                        refreshed: 0,
                ...
                const refreshBootstrap = !bootstrapRefreshConnections.has(entry.connection);
                ...
                try {
                    ...
                    writeServeDebugLine(`refreshExtensions: session ${entry.sessionId} failed: ` +
                        `${err instanceof Error ? err.message : String(err)}`);
            ...
            await Promise.all(results
                ...
                const retry = results.find((result) => result.refreshed > 0 &&
                ...
                try {
                    ...
                    writeServeDebugLine(`refreshExtensions: bootstrap retry via session ${retry.entry.sessionId} failed: ` +
                        `${err instanceof Error ? err.message : String(err)}`);
                }
            }));
            const refreshed = results.reduce((sum, result) => sum + result.refreshed, 0);
        ...
        async setSessionConfigOption(sessionId, req) {
            ...
            const work = entry.modelChangeQueue.then(() => Promise.race([
                withTimeout(entry.connection.setSessionConfigOption(normalized), initTimeoutMs, 'setSessionConfigOption'),
                transportClosed,
            ]));
            entry.modelChangeQueue = work.then(() => undefined, () => undefined);
            return (await work);
        ...
        async setSessionLanguage(sessionId, params, context) {
            ...
            const originatorClientId = resolveTrustedClientId(entry, context?.clientId);
            const result = (await Promise.race([
                withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionLanguage, {
                    sessionId,
                    language: params.language,
                    syncOutputLanguage: params.syncOutputLanguage,
                }), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.sessionLanguage),
                getTransportClosedReject(entry),
            ]));
            try {
                entry.events.publish({
                ...
                writeServeDebugLine(`language_changed event publish failed: ${err instanceof Error ? err.message : String(err)}`);
            }
            return {
                language: result.language,
                outputLanguage: result.outputLanguage ?? null,
        ...
        async setSessionLiveConversationActive(sessionId, active) {
            await requestSessionStatus(sessionId, SERVE_CONTROL_EXT_METHODS.sessionLiveConversation, { active });
        },
        async appendSessionLiveTranscript(sessionId, entries, model) {
            await requestSessionStatus(sessionId, SERVE_CONTROL_EXT_METHODS.sessionLiveTranscript, { entries, model });
        },
        async setSessionApprovalMode(sessionId, mode, opts, context) {
            ...
            if (!info || info.isDying)
                throw new SessionNotFoundError(sessionId);
            const originatorClientId = resolveTrustedClientId(entry, context?.clientId);
            return await applyApprovalMode(entry, mode, opts.persist, originatorClientId);
        },
        async generateSessionRecap(sessionId, _context) {
            ...
            if (!info || info.isDying)
                throw new SessionNotFoundError(sessionId);
            opts.onDiagnosticLine?.(`qwen serve: bridge generateSessionRecap dispatching ext-method for session=${sessionId}`, 'info');
            const response = (await Promise.race([
                withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionRecap, {
                    sessionId,
                }), SESSION_RECAP_TIMEOUT_MS, SERVE_CONTROL_EXT_METHODS.sessionRecap),
                getTransportClosedReject(entry),
            ]));
            opts.onDiagnosticLine?.(`qwen serve: bridge generateSessionRecap completed for session=${sessionId} recap=${response.recap ? `len=${response.recap.length}` : 'null'}`, 'info');
            return {
                sessionId: entry.sessionId,
        ...
        generateSessionContent(sessionId, prompt, signal, context) {
            ...
            const queue = new GenerationStreamQueue(GENERATION_STREAM_QUEUE_CAPACITY);
            const request = {
                sessionId,
                connection: entry.connection,
                queue,
            ...
            void Promise.race([
                withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionGenerationStart, { sessionId, requestId, prompt }), SESSION_GENERATION_TIMEOUT_MS, SERVE_CONTROL_EXT_METHODS.sessionGenerationStart),
                getTransportClosedReject(entry),
            ])
                .then((raw) => {
                if (request.settled)
                ...
                if (accepted)
                    ...
                    queue.fail(new Error('Generation stream consumer too slow'));
            })
                .catch((error) => {
                if (!request.settled)
                    queue.fail(error);
        ...
        removePendingPrompt(sessionId, promptId, context) {
            ...
            resolveTrustedClientId(entry, context?.clientId);
            const idx = entry.pendingPromptList.findIndex((p) => p.promptId === promptId);
            if (idx === -1)
            ...
            if (target.removed)
                return { removed: false };
            writeStderrLine(`[pending-prompt] session=${sessionId} removing promptId=${promptId} state=${target.state}`);
            ...
            target.abortController.abort(new DOMException('Prompt removed by user', 'AbortError'));
        ...
        enqueueMidTurnMessage(sessionId, message, context, requestedMessageId, options) {
            ...
            if (trimmed.length === 0) {
                writeStderrLine(`[mid-turn] session=${entry.sessionId} rejected: empty`);
                return { accepted: false };
            }
            if (requestedMessageId !== undefined) {
                const existing = entry.midTurnMessageQueue.find((queued) => queued.messageId === requestedMessageId);
                if (existing) {
                    if (existing.text === trimmed) {
                        return { accepted: true, messageId: requestedMessageId };
                    }
                    writeStderrLine(`[mid-turn] session=${JSON.stringify(entry.sessionId)} rejected id ${JSON.stringify(requestedMessageId)}: text mismatch`);
                    return { accepted: false };
                }
                const promoted = entry.pendingPromptList.find((pending) => pending.promptId === requestedMessageId);
                if (promoted) {
                    if (promoted.text === trimmed) {
                        return { accepted: true, messageId: requestedMessageId };
                    }
                    writeStderrLine(`[mid-turn] session=${JSON.stringify(entry.sessionId)} rejected promoted id ${JSON.stringify(requestedMessageId)}: text mismatch`);
                    return { accepted: false };
                }
            ...
            if (isClosingOrAuthorizingClose(entry)) {
                writeStderrLine(`[mid-turn] session=${JSON.stringify(entry.sessionId)} rejected: session closing`);
                return { accepted: false };
            }
            ...
            if (entry.pendingPromptCount === 0) {
                ...
                if (options?.queueOnly) {
                    writeStderrLine(`[mid-turn] session=${JSON.stringify(entry.sessionId)} rejected id ${JSON.stringify(messageId)}: session idle`);
                    return { accepted: false };
                }
            ...
            if (entry.midTurnMessageQueue.length >= MAX_MID_TURN_QUEUE_DEPTH) {
                writeStderrLine(`[mid-turn] session=${entry.sessionId} rejected: queue full (depth ${entry.midTurnMessageQueue.length} >= ${MAX_MID_TURN_QUEUE_DEPTH})`);
                return { accepted: false };
            }
            const queuedMessage = {
                messageId,
        ...
        removeMidTurnMessage(sessionId, messageId, context) {
            ...
            if (!entry)
                throw new SessionNotFoundError(sessionId);
            resolveTrustedClientId(entry, context?.clientId);
            const index = entry.midTurnMessageQueue.findIndex((message) => message.messageId === messageId);
            if (index === -1) {
                const isPromoted = entry.pendingPromptList.some((pending) => pending.promptId === messageId && pending.promotedMidTurn === true);
                if (!isPromoted) {
                    writeStderrLine(`[mid-turn] session=${JSON.stringify(entry.sessionId)} remove missed messageId=${JSON.stringify(messageId)} (already drained or completed)`);
                    return { removed: false };
                }
                const promoted = bridgeApi.removePendingPrompt(sessionId, messageId, context);
                if (promoted.removed) {
        ...
        async enqueueBackgroundNotification(sessionId, notification) {
            ...
            try {
                const response = await Promise.race([
                    withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionBackgroundNotification, { sessionId, ...notification }), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.sessionBackgroundNotification),
                    getTransportClosedReject(entry),
                ]);
                return { sessionId, accepted: response['accepted'] === true };
            }
            finally {
                entry.pendingAgentNotificationCount = Math.max(0, entry.pendingAgentNotificationCount - 1);
        ...
        async generateSessionBtw(sessionId, question, signal, _context) {
            ...
            const races = [
                withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionBtw, {
                    sessionId,
                    question,
                }), SESSION_BTW_TIMEOUT_MS, SERVE_CONTROL_EXT_METHODS.sessionBtw),
                getTransportClosedReject(entry),
            ];
            let cleanupAbort;
            if (signal) {
        ...
        async launchSessionForkAgent(sessionId, directive, context) {
            ...
            if (entry.pendingPromptCount > 0 || entry.promptActive) {
                throw new SessionBusyError(sessionId, 'Cannot fork while a response or tool call is in progress');
            }
            return entry.promptQueue.then(async () => {
                if (entry.pendingPromptCount > 0 || entry.promptActive) {
                    throw new SessionBusyError(sessionId, 'Cannot fork while a response or tool call is in progress');
                }
                opts.onDiagnosticLine?.(`qwen serve: launchSessionForkAgent requested for session=${sessionId}`, 'info');
                let response;
                try {
                    response = (await Promise.race([
                        withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionForkAgent, {
                            sessionId,
                            directive: trimmed,
                        }), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.sessionForkAgent),
                        getTransportClosedReject(entry),
                    ]));
                }
                catch (error) {
                    opts.onDiagnosticLine?.(`qwen serve: launchSessionForkAgent failed for session=${sessionId}: ${error instanceof Error ? error.message : String(error)}`, 'warn');
                    throw error;
                }
                const result = {
                    sessionId: entry.sessionId,
                ...
                opts.onDiagnosticLine?.(`qwen serve: launchSessionForkAgent completed for session=${sessionId} launched=${result.launched}`, 'info');
                return result;
            });
        },
        async executeShellCommand(sessionId, command, signal, context) {
            opts.onDiagnosticLine?.(`qwen serve: bridge executeShellCommand for session=${sessionId}`, 'info');
            ...
            try {
                const handle = await ShellExecutionService.execute(command, cwd, (event) => {
                    if (event.type === 'data') {
                        const chunk = typeof event.chunk === 'string'
                            ? event.chunk
                            : event.chunk
                ...
                }, abort.signal, false, { terminalWidth: 120, terminalHeight: 40 }, { streamStdout: true });
                ...
                const historyOutput = output.length > MAX_SHELL_OUTPUT_FOR_HISTORY
                    ? output.substring(0, MAX_SHELL_OUTPUT_FOR_HISTORY) +
                        '\n... (truncated)'
                    : output;
                try {
                    await withTimeout(Promise.race([
                        entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionShellHistory, { sessionId, command, output: historyOutput, exitCode }),
                        getTransportClosedReject(entry),
                    ]), initTimeoutMs, 'sessionShellHistory');
                }
                catch (err) {
                    writeServeDebugLine(`shell history injection failed for session ${sessionId}: ${err instanceof Error ? err.message : String(err)}`);
                }
        ...
        async getRewindSnapshots(sessionId) {
            return requestSessionStatus(sessionId, SERVE_STATUS_EXT_METHODS.sessionRewindSnapshots);
        },
        async rewindSession(sessionId, req, context) {
            ...
            try {
                response = (await Promise.race([
                    withTimeout(entry.connection.extMethod(SERVE_CONTROL_EXT_METHODS.sessionRewind, {
                        sessionId,
                        promptId: req.promptId,
                        rewindFiles: req.rewindFiles !== false,
                    }), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.sessionRewind),
                    getTransportClosedReject(entry),
                ]));
            }
            catch (err) {
            ...
            const artifactSnapshot = restoredArtifactSnapshotFromState(response);
            const artifactSnapshotUnavailable = artifactSnapshotUnavailableReason(response);
            const beforeArtifacts = (await entry.artifacts.list()).artifacts;
            const shouldRestoreArtifactSnapshot = artifactSnapshot !== undefined &&
                artifactSnapshotUnavailable === undefined;
            const artifactRestoreWarnings = artifactSnapshotUnavailable !== undefined
                ? [
                ...
                : shouldRestoreArtifactSnapshot
                    ...
                    : [];
            const artifactRestoreFailed = artifactRestoreWarnings.some(isArtifactRestoreFailureWarning);
            ...
            for (const warning of artifactRestoreWarnings) {
                writeStderrLine(`[artifacts] session=${entry.sessionId} action=rewind_restore_warning warning=${JSON.stringify(warning)}`);
            }
            for (const warning of artifactSnapshotWarnings) {
                writeStderrLine(`[artifacts] session=${entry.sessionId} action=rewind_snapshot_warning warning=${JSON.stringify(warning)}`);
            }
            const afterArtifacts = (await entry.artifacts.list()).artifacts;
            publishArtifactChanges(entry, artifactReseedChanges(beforeArtifacts, afterArtifacts), originatorClientId);
            try {
                entry.events.publish({
        ...
        async manageMcpServer(serverName, action, originatorClientId) {
            ...
            try {
                return await withWorkspaceControl(info, async () => {
                    const timeout = action === 'authenticate'
                        ? MCP_OAUTH_TIMEOUT_MS
                        : MCP_RESTART_TIMEOUT_MS;
                    const response = (await Promise.race([
                        withTimeout(info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMcpManage, { serverName, action, originatorClientId }), timeout, SERVE_CONTROL_EXT_METHODS.workspaceMcpManage),
                        getChannelClosedReject(info),
                    ]));
                    if (action === 'authenticate' && response.pending) {
                        ...
                        const timer = setTimeout(() => {
                            ...
                            info.workspaceMcpAuthenticationTimers.delete(serverName);
                            if (hasNoChannelWork(info)) {
                                void startIdleTimer(info, 'workspace MCP authentication timeout');
                            }
                        }, MCP_OAUTH_TIMEOUT_MS);
                        timer.unref();
                    ...
                    await requestWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcp, () => {
                        throw new BridgeChannelClosedError('workspace MCP management status refresh');
                    }, {}, new Set([serverName]));
                    broadcastWorkspaceEvent({
        ...
        async initializeWorkspaceMcp() {
            ...
            try {
                const result = (await Promise.race([
                    withTimeout(info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMcpInitialize, { cwd: boundWorkspace }), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.workspaceMcpInitialize),
                    getChannelClosedReject(info),
                ]));
                if (result.accepted) {
        ...
        async reloadWorkspaceMcp(options) {
            ...
            try {
                const result = (await Promise.race([
                    withTimeout(info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMcpReload, { cwd: boundWorkspace, ...options }), initTimeoutMs, SERVE_CONTROL_EXT_METHODS.workspaceMcpReload),
                    getChannelClosedReject(info),
                ]));
                if (result.accepted) {
                    beginWorkspaceMcpDiscovery(info);
        ...
        async generateWorkspaceAgent(description, _originatorClientId) {
            ...
            return (await Promise.race([
                withTimeout(info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceAgentGenerate, { description }), MCP_RESTART_TIMEOUT_MS, SERVE_CONTROL_EXT_METHODS.workspaceAgentGenerate),
                getChannelClosedReject(info),
            ]));
        },
            ...
            void (async () => {
                ...
                try {
                    ...
                    await withWorkspaceControl(channelInfo, async () => {
                        ...
                        const raw = await Promise.race([
                            withTimeout(channelInfo.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceGenerationStart, {
                                requestId,
                                prompt,
                                purpose: 'text',
                            }), SESSION_GENERATION_TIMEOUT_MS, SERVE_CONTROL_EXT_METHODS.workspaceGenerationStart),
        ...
        async addRuntimeMcpServer(name, config, originatorClientId) {
            ...
            const info = liveChannelInfo();
            if (!info) {
                throw Object.assign(new Error(`No live ACP channel for runtime MCP add: ${name}`), { data: { errorKind: 'acp_channel_unavailable' } });
            }
            const response = (await Promise.race([
                withTimeout(info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMcpRuntimeAdd, { name, config, originatorClientId }), MCP_RESTART_SERVER_DEADLINE_MS, SERVE_CONTROL_EXT_METHODS.workspaceMcpRuntimeAdd),
                getChannelClosedReject(info),
            ]));
        ...
        async removeRuntimeMcpServer(name, originatorClientId) {
            ...
            const info = liveChannelInfo();
            if (!info) {
                throw Object.assign(new Error(`No live ACP channel for runtime MCP remove: ${name}`), { data: { errorKind: 'acp_channel_unavailable' } });
            }
            const response = (await Promise.race([
                withTimeout(info.connection.extMethod(SERVE_CONTROL_EXT_METHODS.workspaceMcpRuntimeRemove, { name, originatorClientId }), MCP_RESTART_SERVER_DEADLINE_MS, SERVE_CONTROL_EXT_METHODS.workspaceMcpRuntimeRemove),
                getChannelClosedReject(info),
            ]));
            // Emit event on success (non-skip)
        ...
        async addSessionRuntimeMcpServer(sessionId, name, config, originatorClientId) {
            return requestSessionStatus(sessionId, SERVE_CONTROL_EXT_METHODS.sessionMcpRuntimeAdd, { name, config, originatorClientId }, MCP_RESTART_SERVER_DEADLINE_MS);
        },
        async removeSessionRuntimeMcpServer(sessionId, name, originatorClientId) {
            return requestSessionStatus(sessionId, SERVE_CONTROL_EXT_METHODS.sessionMcpRuntimeRemove, { name, originatorClientId }, MCP_RESTART_SERVER_DEADLINE_MS);
        },
        async killSession(sessionId, opts) {
            ...
            if (entry.closing) {
                ...
                if (!closingChannel)
                    return false;
                await killChannelWithLog(closingChannel, `force kill closing session ${JSON.stringify(sessionId)}`);
                return true;
            }
            entry.closing = true;
            const ci = channelInfoForEntry(entry);
            if (!ci) {
                writeStderrLine(`qwen serve: killSession channelInfoForEntry returned undefined ` +
                    `for session ${JSON.stringify(sessionId)} — channel cleanup skipped (entry's channel already torn down)`);
            }
            // Resolve permission waits before asking the agent to drain active turns;
            ...
            try {
                ...
                if (ci) {
                    await killChannelWithLog(ci, `force kill session ${JSON.stringify(sessionId)}`);
                    return true;
                }
                entry.closing = false;
                throw error;
            ...
            flushPromptTerminals(entry, 'session_killed', 'session killed before the prompt completed');
            try {
                entry.events.publish({
                    type: 'session_died',
        ...
        shutdown(options) {
            ...
            void (async () => {
                ...
                for (const e of entries) {
                    ...
                    flushPromptTerminals(e, 'daemon_shutdown', 'daemon shut down before the prompt completed');
                    try {
                        e.events.publish({
                            type: 'session_died',
                ...
                const inFlightRestoreAwaits = Array.from(inFlightRestores.values()).map((restore) => restore.settlementPromise.then(() => undefined, () => undefined));
                ...
                const teardownFailures = teardownResults.flatMap((result) => result.status === 'rejected' ? [result.reason] : []);
                if (teardownFailures.length === 1)
                    throw teardownFailures[0];
                if (teardownFailures.length > 1) {
                    throw new AggregateError(teardownFailures, 'ACP bridge shutdown failed');
                }
        ...
        async preheat() {
            if (shuttingDown)
                return;
            await telemetry.withSpan('channel.preheat', { 'qwen-code.daemon.bridge.operation': 'channel.preheat' }, async () => {
