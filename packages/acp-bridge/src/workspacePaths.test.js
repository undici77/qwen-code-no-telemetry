...
import { _setSandboxMountExistsForTest, canonicalizeWorkspaces, translateAndCheckAbsoluteWorkspacePath, translateWindowsWorkspaceForPosixSandbox, } from './workspacePaths.js';
const scratches = [];
...
describe('translateAndCheckAbsoluteWorkspacePath', () => {
    it.skipIf(process.platform === 'win32')('returns the translated mount for Windows-shaped input in a sandbox', () => {
        vi.stubEnv('SANDBOX', 'qwen-code-sandbox-0');
        _setSandboxMountExistsForTest((p) => p === '/c/qwen-repro');
        try {
            expect(translateAndCheckAbsoluteWorkspacePath('C:\\qwen-repro')).toBe('/c/qwen-repro');
        }
        finally {
            vi.unstubAllEnvs();
    ...
    it('returns null for non-absolute input (untranslated Windows shape included)', () => {
        vi.stubEnv('SANDBOX', '');
        try {
            expect(translateAndCheckAbsoluteWorkspacePath('C:\\qwen-repro')).toBe(process.platform === 'win32' ? 'C:\\qwen-repro' : null);
...
describe('translateWindowsWorkspaceForPosixSandbox', () => {
    const sandboxOpts = (exists) => ({
        platform: 'linux',
        sandboxEnv: 'qwen-code-sandbox-0',
        exists: () => exists,
    });
    it('maps a Windows-absolute path to its bind-mount location', () => {
        expect(translateWindowsWorkspaceForPosixSandbox('C:\\qwen-repro', sandboxOpts(true))).toBe('/c/qwen-repro');
        expect(translateWindowsWorkspaceForPosixSandbox('D:/Work/proj sub', sandboxOpts(true))).toBe('/d/Work/proj sub');
        expect(translateWindowsWorkspaceForPosixSandbox('C:\\nested\\dir\\leaf', sandboxOpts(true))).toBe('/c/nested/dir/leaf');
    });
    it('leaves the path alone when the translated mount does not exist', () => {
        expect(translateWindowsWorkspaceForPosixSandbox('C:\\qwen-repro', sandboxOpts(false))).toBe('C:\\qwen-repro');
    });
    it('refuses ..-laden input that escapes the drive mount', () => {
        ...
        expect(translateWindowsWorkspaceForPosixSandbox('C:\\..\\..\\etc', sandboxOpts(true))).toBe('C:\\..\\..\\etc');
        // In-mount .. that stays under the drive prefix is still fine.
        expect(translateWindowsWorkspaceForPosixSandbox('C:\\work\\..\\proj', sandboxOpts(true))).toBe('/c/work/../proj');
    });
    it('leaves non-Windows-shaped paths alone', () => {
        for (const p of ['/c/qwen-repro', 'relative/dir', 'C:', 'CC:\\x', '']) {
            expect(translateWindowsWorkspaceForPosixSandbox(p, sandboxOpts(true))).toBe(p);
        }
    });
    it('is inert on Windows hosts, outside sandboxes, and under seatbelt', () => {
        expect(translateWindowsWorkspaceForPosixSandbox('C:\\qwen-repro', {
            platform: 'win32',
            sandboxEnv: 'qwen-code-sandbox-0',
            exists: () => true,
        })).toBe('C:\\qwen-repro');
...
//# sourceMappingURL=workspacePaths.test.js.map