...
import { DEFAULT_MAX_QUEUED_BYTES, EventBus, EVENT_SCHEMA_VERSION, } from './eventBus.js';
...
describe('EventBus', () => {
    ...
    it('rejects invalid maxQueuedBytes options', () => {
        expect(() => new EventBus(100, undefined, undefined, { maxQueuedBytes: 0 })).toThrow(TypeError);
        expect(() => new EventBus(100, undefined, undefined, {
    ...
    it('routes contextual subscriber diagnostics without duplicating legacy stderr', async () => {
        ...
        expect(diagnostics[0]?.data.triggerEventBytes).toBe(new TextEncoder().encode(thresholdJson).byteLength);
        expect(diagnostics[1]?.data.triggerEventBytes).toBe(new TextEncoder().encode(overflowingJson).byteLength);
        expect(process.stderr.write).not.toHaveBeenCalledWith(expect.stringContaining('EventBus slow_client_warning'));
        expect(process.stderr.write).not.toHaveBeenCalledWith(expect.stringContaining('EventBus subscriber evicted'));
    });
    it.each([
        ['returns false', () => false],
        [
        ...
        expect(process.stderr.write).toHaveBeenCalledWith(expect.stringContaining('EventBus slow_client_warning'));
        expect(process.stderr.write).toHaveBeenCalledWith(expect.stringContaining('EventBus subscriber evicted'));
    });
    ...
    it('reports frames_and_bytes when both warning thresholds are crossed', async () => {
        ...
        for (let i = 0; i < 4; i++)
            collected.push((await it.next()).value);
        ...
        expect((warning?.data).queuedBytes).toBeGreaterThanOrEqual(900);
        abort.abort();
    });
    ...
    it('rearms slow_client_warning after queue drains below the hysteresis threshold', async () => {
        ...
        expect(firstEpisode.filter((e) => e.type === 'slow_client_warning')).toHaveLength(1);
        ...
        expect(secondEpisode.filter((e) => e.type === 'slow_client_warning')).toHaveLength(1);
        abort.abort();
    });
    it('does not rearm slow_client_warning until frame and byte backlogs both reset', async () => {
        ...
        const partiallyDrained = [];
        for (let i = 0; i < 3; i++) {
            partiallyDrained.push((await it.next()).value);
        }
        expect(partiallyDrained.filter((e) => e.type === 'slow_client_warning')).toHaveLength(0);
        for (let i = 7; i <= 9; i++) {
            bus.publish({
                type: 'foo',
                data: 'y'.repeat(2400),
        ...
        const afterPartialDrain = [];
        for (let i = 0; i < 7; i++) {
            afterPartialDrain.push((await it.next()).value);
        }
        expect(afterPartialDrain.filter((e) => e.type === 'slow_client_warning')).toHaveLength(1);
        bus.publish({
            type: 'foo',
            data: 'small',
        ...
        const afterFullReset = [];
        for (let i = 0; i < 7; i++) {
            afterFullReset.push((await it.next()).value);
        }
        expect(afterFullReset.filter((e) => e.type === 'slow_client_warning')).toHaveLength(1);
        abort.abort();
    });
    ...
    it('keeps sibling subscribers alive after one subscriber is byte-evicted', async () => {
        ...
        const slowEvents = [];
        for await (const e of slowIter)
            slowEvents.push(e);
        expect(slowEvents.find((e) => e.type === 'client_evicted')?.data).toMatchObject({
            reason: 'queue_bytes_overflow',
            droppedAfter: 2,
        });
    ...
    it('allows one oversized live event on an empty queue before byte eviction', async () => {
        ...
        expect(warning.data.queuedBytes).toBeGreaterThan(256);
        const evicted = collected.find((e) => e.type === 'client_evicted');
        expect(evicted?.data).toMatchObject({
            reason: 'queue_bytes_overflow',
            droppedAfter: 2,
    ...
    it('warn-at-back forced frame does NOT skew the live cap for subsequent publishes (codex P2)', async () => {
        ...
        const drained = [];
        for (let i = 0; i < 7; i++)
            drained.push((await it.next()).value);
        expect(drained.filter((e) => e.type === 'slow_client_warning')).toHaveLength(1);
    ...
    it('a live publish AFTER a large replay does NOT evict the resumed subscriber', async () => {
        ...
        const bus = new EventBus();
        for (let i = 1; i <= 10; i++)
            bus.publish({ type: 'replay', data: i });
        const abort = new AbortController();
    ...
    describe('state_resync_required (#4175 F4 prereq, Ilya0527 issue #15)', () => {
        ...
        it('epoch_reset replays the WHOLE fresh ring (stale cursor must not filter new low ids)', async () => {
            ...
            expect(out[0]?.type).toBe('state_resync_required');
            expect((out[0]?.data).reason).toBe('epoch_reset');
    ...
    describe('serializability gate (DAEMON-011)', () => {
        it('rejects an unserializable event without burning an id', async () => {
            ...
            expect(process.stderr.write).toHaveBeenCalledWith(expect.stringContaining('EventBus event sizing failed'));
            ...
            const replayed = await collect(bus.subscribe({ lastEventId: 0, signal: replayAbort.signal }), 3);
    ...
    describe('replay budget (DAEMON-011)', () => {
        ...
        it('truncates an over-budget replay burst and demands a resync', async () => {
            ...
            const frames = [];
            for (let i = 0; i < 4; i++) {
                frames.push((await it.next()).value);
            }
        ...
        it('replays a single frame larger than the budget (first-item rule)', async () => {
            ...
            bus.publish({ type: 'payload', data: bigPayload() });
            const abort = new AbortController();
            const frames = await collect(bus.subscribe({ lastEventId: 0, signal: abort.signal }), 2);
            expect(frames[0]?.id).toBe(1);
        ...
        it('does not emit a resync frame when the replay fits the budget', async () => {
            ...
            const abort = new AbortController();
            const frames = await collect(bus.subscribe({ lastEventId: 0, signal: abort.signal }), 6);
            expect(frames.some((e) => e.type === 'state_resync_required')).toBe(false);
            expect(frames[5]?.type).toBe('replay_complete');
            expect((frames[5]?.data).replayedCount).toBe(5);
            abort.abort();
        });
    });
    ...
    describe('compaction degradation (DAEMON-008)', () => {
        ...
        it('marks the snapshot degraded and fires onCompactionError exactly once when ingest throws', () => {
            ...
            expect(onCompactionError.mock.calls[0][0].message).toBe('boom');
        ...
        it('marks the snapshot degraded when seedReplayEvents throws', () => {
            ...
            const bus = new EventBus(10, undefined, engine, { onCompactionError });
            expect(() => bus.seedReplayEvents([{ type: 'foo', data: 1 }])).not.toThrow();
            expect(onCompactionError).toHaveBeenCalledTimes(1);
            expect(bus.snapshotReplay()?.degraded).toBe(true);
        });
