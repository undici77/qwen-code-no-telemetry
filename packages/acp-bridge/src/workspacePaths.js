...
let sandboxMountExistsOverrideForTest;
export function _setSandboxMountExistsForTest(fn) {
    sandboxMountExistsOverrideForTest = fn;
}
export function translateWindowsWorkspaceForPosixSandbox(p, opts = {}) {
    ...
    if (resolvedTranslated !== `/${match[1].toLowerCase()}` &&
...
export function canonicalizeWorkspaces(paths) {
    const out = [];
    const seen = new Set();
