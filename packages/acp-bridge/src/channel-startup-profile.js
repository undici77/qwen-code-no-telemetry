...
function addDurationAttributes(attributes, group, values) {
    for (const [key, value] of Object.entries(values)) {
        if (group === 'phase' && key === 'unattributedMs')
            continue;
        const attributeKey = key.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`);
...
export function getChannelStartupProfileAttributes(response, receivedAtEpochMs, initializeTimeoutMs) {
    ...
    const validResponseEpoch = typeof responseBuiltAtEpochMs === 'number' &&
        Number.isFinite(responseBuiltAtEpochMs) &&
        responseBuiltAtEpochMs >= 0;
    const effectiveComplete = childComplete &&
        phases.complete &&
        config.complete &&
...
//# sourceMappingURL=channel-startup-profile.js.map