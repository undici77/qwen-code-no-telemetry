...
describe('bridge spawnOrAttach inside a POSIX container sandbox (#7139)', () => {
    ...
    it.skipIf(process.platform === 'win32')('accepts a Windows-shaped workspaceCwd bound to its mount location', async () => {
    ...
    it.skipIf(process.platform === 'win32')('still rejects a Windows-shaped workspaceCwd outside a sandbox', async () => {
        vi.stubEnv('SANDBOX', '');
        const handle = makeChannel();
        const bridge = makeBridge({
            boundWorkspace: '/c/qwen-repro',
        ...
        try {
            await expect(bridge.spawnOrAttach({ workspaceCwd: 'C:\\qwen-repro' })).rejects.toThrow('workspaceCwd must be an absolute path');
        }
        finally {
            await bridge.shutdown();
        }
...
//# sourceMappingURL=bridge.sandbox.test.js.map