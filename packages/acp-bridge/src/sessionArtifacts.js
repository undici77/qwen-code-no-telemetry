...
import { isPrototypeMetadataKey, isReservedWorkspaceMetadataKey, metadataBudgetBytes, SESSION_ARTIFACT_PERSISTENCE_VERSION, stableSessionArtifactId, WORKSPACE_CONTENT_MTIME_MS_METADATA_KEY, WORKSPACE_CONTENT_SHA256_METADATA_KEY, } from '@qwen-code/qwen-code-core';
...
const RESTORE_PARTIAL_FAILED_WARNING_PREFIX = 'artifact snapshot restore partially failed';
export function isArtifactRestoreFailureWarning(warning) {
    return (warning.startsWith(RESTORE_FAILED_WARNING_PREFIX) ||
...
export class SessionArtifactStore {
    ...
    async upsertMany(inputs, options = {}) {
        return this.enqueue(async () => {
            ...
            for (const input of inputs) {
                try {
                    normalizedResults.push(await this.normalizeInput(input, ++this.receivedSeq, options.trustedPublisher === true));
                }
                catch (error) {
                    if (validationStrict) {
                        throw error;
                    ...
                    writeStderrLine(`[artifacts] session=${this.sessionId} action=dropped reason=${JSON.stringify(message)}`);
                }
            }
            const changes = [];
            try {
                for (const normalized of coalesceByIdentity(normalizedResults)) {
                    ...
                    if (this.shouldSuppressTombstonedUpsert(artifact)) {
                        writeStderrLine(`[artifacts] session=${this.sessionId} action=tombstone_replay_suppressed artifactId=${artifact.id}`);
                        continue;
                    }
                    ...
                    if (updated.changed) {
                        if (updated.artifact.id !== existing.id) {
                            const removeChange = {
                                ...
                                reason: 'explicit',
                                durableTombstoneRequired: existing.durableTombstoneRequired ||
                ...
                changes.push(...(await this.evictOverflow(createdIds, changes, persistenceStrict)));
                const hasStrictDurableTransition = changes.some(shouldCommitBeforeDurablePersistence);
                ...
                if (hasStrictDurableTransition && !persistenceStrict) {
                    try {
                        ...
                        writeStderrLine(`[artifacts] session=${this.sessionId} action=upsert_rollback warning=${JSON.stringify(warning)} artifactIds=${JSON.stringify(artifactIds)} error=${JSON.stringify(error instanceof Error ? error.message : String(error))}`);
                        return {
                            v: 1,
                            sessionId: this.sessionId,
                            changes: [],
                    ...
                    persistenceWarnings = await this.persistChanges(changes, persistenceStrict);
                }
                warnings.push(...persistenceWarnings);
                warningDetails.push(...detailsForPersistenceWarnings(persistenceWarnings, changes, 'upsert'));
    ...
    findPublishedUpgradeTarget(artifact) {
        if (artifact.storage !== 'published' ||
            !artifact.trustedPublisher ||
            !artifact.managedId ||
            !artifact.url) {
            return undefined;
        }
        const byUrl = this.artifacts.get(stableSessionArtifactId(this.sessionId, `url:${artifact.url}`));
        if (byUrl &&
            (byUrl.storage !== 'published' || byUrl.managedId === artifact.managedId)) {
            return byUrl;
        }
        for (const existing of this.artifacts.values()) {
            if ((existing.storage === 'workspace' &&
    ...
    findPublishedWorkspaceTarget(artifact) {
        ...
        const managedId = managedIdForWorkspacePath(this.workspaceCwd, artifact.workspacePath);
        for (const existing of this.artifacts.values()) {
            if (existing.storage === 'published' &&
    ...
    async remove(artifactId, options) {
        return this.enqueue(async () => {
            ...
            stripDurableTombstoneMarkers(changes);
            const warningDetails = detailsForPersistenceWarnings(warnings, changes, 'remove');
            return {
                v: 1,
                sessionId: this.sessionId,
                changes,
    ...
    async restore(snapshot, options = {}) {
        ...
        return this.enqueue(async () => {
            ...
            if (snapshot) {
                ...
                for (const id of snapshot.stickyEphemeralIds.slice(-MAX_STICKY_EPHEMERAL_IDS)) {
                ...
                for (const artifact of snapshot.markerArtifacts ?? []) {
                    if (!markerIds.has(artifact.id))
                        continue;
                    const markerArtifact = await this.normalizeRestoredMarkerArtifact(artifact, warnings);
                    if (markerArtifact)
            ...
            for (const artifact of snapshot?.artifacts ?? []) {
                try {
                    ...
                    if (input.retention === 'pinned') {
                        ...
                        warnings.push(`pinned artifact ${artifact.id} downgraded to restorable; runtime does not support pinned retention`);
                    }
                    let normalized = await this.normalizeInput(input, ++this.receivedSeq, artifact.storage === 'published' &&
                    ...
                    if (this.stickyEphemeralIds.has(normalized.id) &&
                    ...
                    const stored = {
                        ...
                        persistenceWarning,
                        durableTombstoneRequired: retention !== 'ephemeral' ? true : undefined,
                        insertSeq: ++this.insertSeq,
                    };
                    this.artifacts.set(stored.id, stored);
                    restoredCount++;
                    ...
                    warnings.push(`skipped artifact restore: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            ...
            if (previousState.artifacts.size > 0 &&
    ...
    async recordSnapshot() {
        ...
        return this.enqueue(async () => {
            ...
            try {
                await persistence.recordSnapshot(this.buildSnapshotPayload(recordedAt, sequence));
                ...
                writeStderrLine(`[artifacts] session=${this.sessionId} action=snapshot_failed reason=${JSON.stringify(error instanceof Error ? error.message : String(error))}`);
    ...
    async normalizeRestoredMarkerArtifact(artifact, warnings) {
        try {
            ...
            if (input.retention === 'pinned') {
                input.retention = 'restorable';
                warnings.push(`pinned marker artifact ${artifact.id} downgraded to restorable; runtime does not support pinned retention`);
            }
            const normalized = await this.normalizeInput(input, ++this.receivedSeq, artifact.storage === 'published' && !isFileArtifactUrl(artifact.url), {
                metadataBudget: 'persisted',
                workspaceExpected: workspaceExpectedFromArtifact(artifact),
                hashWorkspaceContent: false,
            });
            if (normalized.id !== artifact.id) {
                warnings.push(`skipped marker artifact with mismatched id ${artifact.id}`);
                return undefined;
            }
            ...
            warnings.push(`skipped marker artifact ${artifact.id}: ${error instanceof Error ? error.message : String(error)}`);
            return undefined;
        }
    }
    cloneState() {
        return {
            artifacts: new Map(Array.from(this.artifacts.entries()).map(([id, artifact]) => [
                id,
    ...
    async persistChanges(changes, strict = false) {
        ...
        if (!this.persistence) {
            if (strict) {
                throw new SessionArtifactValidationError('artifact persistence is unavailable', 'retention');
            }
        ...
        const payload = {
            ...
            sequence,
            recordedAt,
            changes: durableChanges.map((change) => toPersistedChange(change, recordedAt)),
        };
        try {
            ...
            writeStderrLine(`[artifacts] session=${this.sessionId} action=persist_failed sequence=${payload.sequence} artifactIds=${JSON.stringify(artifactIds)} reason=${JSON.stringify(reason)}`);
            const warnings = this.downgradeDurableChanges(durableChanges);
            this.applyDurableMarkers(durableChanges);
            return warnings;
        }
    ...
    async maybeRecordSnapshot(recordedAt) {
        ...
        const snapshotThreshold = SNAPSHOT_AFTER_DURABLE_EVENTS *
            Math.min(MAX_SNAPSHOT_BACKOFF_MULTIPLIER, 2 ** this.consecutiveSnapshotFailures);
        if (this.durableEventsSinceSnapshot < snapshotThreshold) {
            return;
        }
        const sequence = this.persistenceSeq + 1;
        try {
            await this.persistence.recordSnapshot(this.buildSnapshotPayload(recordedAt, sequence));
            this.persistenceSeq = sequence;
            this.durableEventsSinceSnapshot = 0;
            this.consecutiveSnapshotFailures = 0;
        }
        catch (error) {
            this.consecutiveSnapshotFailures = Math.min(this.consecutiveSnapshotFailures + 1, Math.log2(MAX_SNAPSHOT_BACKOFF_MULTIPLIER));
            writeStderrLine(`[artifacts] session=${this.sessionId} action=snapshot_failed reason=${JSON.stringify(error instanceof Error ? error.message : String(error))}`);
        }
    }
    buildSnapshotPayload(recordedAt, sequence) {
        ...
        const stickyEphemeralIds = Array.from(this.stickyEphemeralIds).filter((id) => this.artifacts.has(id) || this.markerArtifacts.has(id));
        const markerArtifacts = this.buildMarkerArtifacts(recordedAt, stickyEphemeralIds);
        return {
            v: SESSION_ARTIFACT_PERSISTENCE_VERSION,
            sessionId: this.sessionId,
            sequence,
    ...
    downgradeDurableChanges(changes) {
        ...
        const warnings = [];
        if (downgraded) {
            warnings.push('artifact persistence unavailable; durable artifacts kept ephemeral');
        }
    ...
    rememberStickyEphemeral(change) {
        ...
        if (change.artifact) {
            this.markerArtifacts.set(artifactId, toPersistedArtifact(change.artifact, change.artifact.updatedAt));
        }
    ...
    rememberTombstone(change) {
        ...
        this.tombstonedClientIds.set(change.artifactId, change.removedClientId ??
            this.artifacts.get(change.artifactId)?.clientId);
        if (change.artifact) {
            this.markerArtifacts.set(change.artifactId, toPersistedArtifact(change.artifact, change.artifact.updatedAt));
        }
        while (this.tombstonedIds.size > MAX_TOMBSTONED_IDS) {
            const oldest = this.tombstonedIds.values().next().value;
            if (oldest === undefined)
                break;
            writeStderrLine(`[artifacts] session=${this.sessionId} action=tombstone_evicted artifactId=${JSON.stringify(oldest)} limit=${MAX_TOMBSTONED_IDS}`);
    ...
    denyCrossClientMutation(action, artifactId, existing, options) {
        ...
        writeStderrLine(`[artifacts] session=${this.sessionId} action=${action}_denied artifactId=${artifactId} owner=${existing.clientId} requester=${options?.clientId ?? '<anonymous>'}`);
    ...
    async normalizeInput(input, receivedSeq, trustedPublisherFromCaller, options = {}) {
        ...
        const description = normalizeString(input.description, 'description', 1000, false);
        const source = input.source ?? 'tool';
        if (source !== 'tool' && source !== 'hook' && source !== 'client') {
            throw new SessionArtifactValidationError('source must be tool, hook, or client', 'source');
        }
        const trustedPublisher = trustedPublisherFromCaller;
        const workspacePath = input.workspacePath
            ? normalizeWorkspacePath(input.workspacePath, await this.getRealWorkspaceCwdForValidation())
            : undefined;
        ...
        const workspaceStatus = workspacePath
            ? await this.getInitialWorkspaceStatus(workspacePath, options.workspaceExpected, { hashContent: options.hashWorkspaceContent !== false })
            : undefined;
        if (workspaceStatus?.escaped) {
            throw new SessionArtifactValidationError('workspacePath must stay inside the workspace', 'workspacePath');
        }
        const metadata = withWorkspaceContentHashMetadata(normalizeMetadata(input.metadata, {
            budget: options.metadataBudget ?? 'user',
        }), workspaceStatus);
        const kind = normalizeKind(input.kind ?? inferKind({ storage, workspacePath, url }));
        ...
        return {
            ...
            sizeBytes: workspaceStatus?.sizeBytes !== undefined
            ...
            hookEventName: normalizeString(input.hookEventName, 'hookEventName', 200, false),
    ...
    shouldSuppressTombstonedUpsert(artifact) {
        ...
        return !(artifact.retentionExplicit &&
    ...
    async refreshWorkspaceStatuses() {
        ...
        await runInBatches(staleWorkspaceArtifacts, WORKSPACE_STATUS_REFRESH_BATCH_SIZE, (artifact) => this.refreshWorkspaceStatus(artifact, {
            onError: 'missing',
            now,
        }));
    }
    applyStickyEphemeralOverride(artifact) {
        if (!this.stickyEphemeralIds.has(artifact.id) ||
    ...
    async getInitialWorkspaceStatus(workspacePath, expected, options = { hashContent: true }) {
        try {
            return await getWorkspaceStatus(workspacePath, this.getRealWorkspaceCwd(), expected, options);
        }
        catch (error) {
            const reason = error instanceof Error ? error.message : String(error);
            throw new SessionArtifactValidationError(`workspacePath could not be inspected: ${reason}`, 'workspacePath');
        }
    }
    async refreshWorkspaceStatus(artifact, options) {
        ...
        try {
            const status = await getWorkspaceStatus(artifact.workspacePath, this.getRealWorkspaceCwd(), {
            ...
            writeStderrLine(`[artifacts] session=${this.sessionId} action=status_refresh_failed artifactId=${artifact.id} reason=${JSON.stringify(error instanceof Error ? error.message : String(error))}`);
    ...
    async evictOverflow(createdIds, changes, strict = false) {
        ...
        const candidates = Array.from(this.artifacts.values()).filter((artifact) => !createdInThisBatch.has(artifact.id));
        const now = Date.now();
        const staleWorkspaceCandidates = candidates
            .filter((artifact) => artifact.workspacePath)
            .filter((artifact) => shouldRefreshWorkspaceStatus(artifact, now));
        await runInBatches(staleWorkspaceCandidates, WORKSPACE_STATUS_REFRESH_BATCH_SIZE, (artifact) => this.refreshWorkspaceStatus(artifact, { onError: 'preserve', now }));
        ...
        if (strict &&
            ...
            throw new SessionArtifactValidationError('artifact store is full; no eviction candidate is available', 'artifactId');
        }
        for (const artifact of overflowCreated) {
            ...
            this.artifacts.delete(artifact.id);
            writeStderrLine(`[artifacts] session=${this.sessionId} action=dropped reason="max artifacts exceeded" artifactId=${artifact.id}`);
            removePriorChange(changes, artifact.id);
        }
        return removed;
    }
...
function coalesceByIdentity(artifacts) {
    ...
    return Array.from(byId.values()).sort((a, b) => a.receivedSeq - b.receivedSeq);
}
function mergeBatchArtifact(existing, next) {
    const publishedUpgrade = existing.storage !== 'published' &&
...
function mergeArtifact(existing, incoming) {
    ...
    const publishedRefresh = existing.storage === 'published' &&
        incoming.storage === 'published' &&
        incoming.trustedPublisher &&
    ...
    const next = {
        ...
        status: existing.storage === 'published' && !publishedUpdate
            ? existing.status
            : incoming.status,
        ...
        workspacePath: publishedUpdate || existing.storage === 'published'
            ? undefined
            : (existing.workspacePath ?? incoming.workspacePath),
        hideWorkspacePath: publishedUpdate ||
            existing.storage === 'published' ||
        ...
        mimeType: publishedUpdate
            ...
            : existing.mimeType,
        sizeBytes: existing.storage === 'published' && !publishedUpdate
            ? existing.sizeBytes
            : mergeSizeBytes(existing, incoming),
        metadata: existing.storage === 'published' && !publishedUpdate
            ? existing.metadata
            : mergeMetadata(existing, incoming),
        retention: mergeRetention(existing, incoming),
        restoreState: 'live',
        persistenceWarning: incoming.retentionExplicit && incoming.retention !== 'ephemeral'
            ? incoming.persistenceWarning
            : (existing.persistenceWarning ?? incoming.persistenceWarning),
        persistedAt: incoming.retentionExplicit && incoming.retention === 'ephemeral'
            ? undefined
        ...
        lastStatAt: publishedUpdate || existing.storage === 'published'
            ? undefined
    ...
    const changed = !publicArtifactsEqual(toPublicArtifact(existing), toPublicArtifact(next));
    if (changed) {
        next.updatedAt = now;
    }
...
function shouldRecordEphemeralUnpin(existing, incoming) {
    return (incoming.retentionExplicit &&
        incoming.retention === 'ephemeral' &&
        (existing.retention !== 'ephemeral' ||
            existing.persistedAt !== undefined ||
            existing.durableTombstoneRequired === true));
}
export function publicArtifactsEqual(a, b) {
    return (a.id === b.id &&
        a.kind === b.kind &&
        a.storage === b.storage &&
        a.source === b.source &&
...
function mergeMetadata(existing, incoming) {
    ...
    if (!isMetadataWithinLimit(merged)) {
        writeStderrLine(`[artifacts] action=metadata_merge_dropped artifactId=${incoming.id} reason="metadata limit exceeded"`);
        return existing.metadata;
    }
    return merged;
}
...
function shouldRefreshWorkspaceStatus(artifact, now) {
    return (artifact.lastStatAt === undefined ||
        now - artifact.lastStatAt >= WORKSPACE_STATUS_REFRESH_TTL_MS);
}
function selectEvictionCandidate(candidates, sourceCounts) {
    return (oldest(candidates, (artifact) => artifact.status === 'missing' && !artifact.clientRetained) ??
...
function toPublicArtifact(artifact) {
    ...
    const { id, kind, storage, source, status, title, description, workspacePath, managedId, url, mimeType, sizeBytes, metadata, retention, restoreState, persistenceWarning, persistedAt, clientRetained, createdAt, updatedAt, toolCallId, toolName, hookEventName, } = artifact;
    return {
        id,
        kind,
        storage,
...
function isArtifactSnapshotCompletenessWarning(warning) {
    if (warning.startsWith('skipped stale event sequence '))
        return false;
...
function persistenceFailureDetail(options) {
    const unavailable = options.error instanceof SessionArtifactValidationError &&
...
function inferStorage(requested, locators) {
    if (requested !== undefined && typeof requested !== 'string') {
        throw new SessionArtifactValidationError('storage must be a string', 'storage');
    }
    if (requested) {
        if (requested !== 'workspace' &&
            ...
            requested !== 'managed' &&
            requested !== 'published') {
            throw new SessionArtifactValidationError('storage must be workspace, external_url, managed, or published', 'storage');
        }
        return requested;
    }
    if (locators.workspacePath)
        return 'workspace';
    if (locators.managedId)
        return 'managed';
    if (locators.url && locators.trustedPublisher)
        return 'published';
    return 'external_url';
}
function normalizeKind(kind) {
    ...
    throw new SessionArtifactValidationError('kind must be a supported artifact kind', 'kind');
}
function validateLocator(storage, locators) {
    if (storage === 'published') {
        if (!locators.trustedPublisher) {
            throw new SessionArtifactValidationError('published artifacts are reserved for trusted publishers', 'storage');
        }
        if (!locators.url) {
            throw new SessionArtifactValidationError('published artifacts require url', 'url');
        }
        if (locators.workspacePath) {
            throw new SessionArtifactValidationError('published artifacts cannot include workspacePath', 'workspacePath');
        }
        return;
    }
    const locatorCount = [
    ...
    if (locatorCount !== 1) {
        throw new SessionArtifactValidationError('provide exactly one of workspacePath, managedId, or url');
    }
    if (storage === 'workspace' && !locators.workspacePath) {
        throw new SessionArtifactValidationError('workspace storage requires workspacePath', 'workspacePath');
    }
    if (storage === 'managed' && !locators.managedId) {
        throw new SessionArtifactValidationError('managed storage requires managedId', 'managedId');
    }
    if (storage === 'external_url' && !locators.url) {
        throw new SessionArtifactValidationError('external_url storage requires url', 'url');
    }
}
function buildIdentityKey(input) {
    ...
    if (input.managedId)
        return `managed:${input.managedId}`;
    if (input.url)
        return `url:${input.url}`;
    throw new SessionArtifactValidationError('artifact identity requires workspacePath, managedId, or url');
}
...
function normalizeString(value, field, maxLength, required) {
    ...
    if (typeof value !== 'string') {
        throw new SessionArtifactValidationError(`${field} must be a string`, field);
    }
    ...
    if (trimmed.length > maxLength) {
        throw new SessionArtifactValidationError(`${field} exceeds ${maxLength} characters`, field);
    }
    if (hasControlCharacter(trimmed, field === 'description')) {
        throw new SessionArtifactValidationError(`${field} contains control characters`, field);
    }
    if (isDisplayField(field) && hasUnsafeDisplayPayload(trimmed)) {
        throw new SessionArtifactValidationError(`${field} contains unsafe markup`, field);
    }
    return trimmed;
}
function normalizeManagedId(value) {
    ...
    if (!managedId)
        return undefined;
    if (managedId.includes('/') ||
        managedId.includes('\\') ||
        managedId.includes('..') ||
        path.isAbsolute(managedId) ||
        path.win32.isAbsolute(managedId)) {
        throw new SessionArtifactValidationError('managedId must be an opaque managed resource id', 'managedId');
    }
    return managedId;
}
...
function normalizeWorkspacePath(raw, workspaceCwd) {
    ...
    if (path.isAbsolute(trimmed)) {
        throw new SessionArtifactValidationError('workspacePath must be relative to the workspace', 'workspacePath');
    }
...
function normalizeArtifactUrl(raw, allowFile) {
    ...
    if (parsed.username || parsed.password) {
        throw new SessionArtifactValidationError('url must not include credentials', 'url');
    }
    if (hasSecretLikeUrlComponent(parsed)) {
        throw new SessionArtifactValidationError('url must not include secret-like components', 'url');
    }
    if (parsed.protocol !== 'http:' &&
        parsed.protocol !== 'https:' &&
        !(allowFile && parsed.protocol === 'file:')) {
        throw new SessionArtifactValidationError(allowFile
...
function normalizeMetadata(metadata, options = {}) {
    ...
    if (typeof metadata !== 'object' ||
        ...
        throw new SessionArtifactValidationError('metadata must be an object', 'metadata');
    }
    const normalized = {};
    for (const [key, value] of Object.entries(metadata)) {
        ...
        if (!key) {
            throw new SessionArtifactValidationError('metadata keys must not be empty', 'metadata');
        }
        if (key.length > 120) {
            throw new SessionArtifactValidationError('metadata keys must be 120 characters or fewer', 'metadata');
        }
        if (hasControlCharacter(key) || hasUnsafeDisplayPayload(key)) {
            throw new SessionArtifactValidationError('metadata keys contain unsafe content', 'metadata');
        }
        if (isSecretLikeUrlText(key)) {
            throw new SessionArtifactValidationError('metadata keys must not contain secret-like names', 'metadata');
        }
        if (value !== null &&
            typeof value !== 'string' &&
            typeof value !== 'number' &&
            typeof value !== 'boolean') {
            throw new SessionArtifactValidationError('metadata values must be primitive', 'metadata');
        }
        if (typeof value === 'number' && !Number.isFinite(value)) {
            throw new SessionArtifactValidationError('metadata numbers must be finite', 'metadata');
        }
        if (typeof value === 'string' &&
            (hasControlCharacter(value) || hasUnsafeDisplayPayload(value))) {
            throw new SessionArtifactValidationError('metadata string values contain unsafe content', 'metadata');
        }
        if (typeof value === 'string' &&
            !isReservedWorkspaceMetadataKey(key) &&
            isSecretLikeMetadataValue(value)) {
            throw new SessionArtifactValidationError('metadata string values must not contain secret-like tokens', 'metadata');
        }
    ...
    if (metadataBudgetBytes(normalized, options.budget ?? 'user') > 4096) {
        throw new SessionArtifactValidationError('metadata must be 4096 bytes or fewer', 'metadata');
    }
    return normalized;
}
function normalizeRetention(value, options) {
    ...
    if (value === 'pinned') {
        throw new SessionArtifactValidationError('pinned retention is not supported by session_artifacts_persistence', 'retention');
    }
    throw new SessionArtifactValidationError('retention must be ephemeral or restorable', 'retention');
}
function normalizeSizeBytes(value) {
    if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) {
        throw new SessionArtifactValidationError('sizeBytes must be a non-negative safe integer', 'sizeBytes');
    }
    return value;
}
function inferKind(input) {
    if (input.storage === 'published')
        return 'html';
    if (input.url)
        return 'link';
...
async function getWorkspaceStatus(workspacePath, realWorkspaceCwd, expected, options = { hashContent: true }) {
    ...
    try {
        ...
        try {
            ...
            if (stat.isFile()) {
                const expectedMtimeMs = typeof expected?.mtimeMs === 'number' ? expected.mtimeMs : undefined;
                const expectedSha256 = typeof expected?.sha256 === 'string' ? expected.sha256 : undefined;
                const unchanged = expected?.sizeBytes === stat.size && expectedMtimeMs === stat.mtimeMs;
                const sizeChanged = expected?.sizeBytes !== undefined && expected.sizeBytes !== stat.size;
                if (sizeChanged) {
                    return {
                        status: 'changed',
                        sizeBytes: stat.size,
...
//# sourceMappingURL=sessionArtifacts.js.map