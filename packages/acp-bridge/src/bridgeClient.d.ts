 ...
 */
import type { Client, ReadTextFileRequest, ReadTextFileResponse, RequestPermissionRequest, RequestPermissionResponse, SessionNotification, SessionUpdate, WriteTextFileRequest, WriteTextFileResponse } from '@agentclientprotocol/sdk';
...
import type { ChannelDeliveryHandler, ClientMcpMessageSender, CreateSubSessionHandler, ExternalToolGuardHandler, LiveScreenContextCaptureHandler, LiveSpeakToUserHandler, LiveTaskToolRequestHandler } from './bridgeOptions.js';
import type { BridgeFileSystem } from './bridgeFileSystem.js';
import type { PermissionMediator } from './permission.js';
import type { SessionArtifactInput, SessionArtifactStore } from './sessionArtifacts.js';
...
export declare class BridgeClient implements Client {
     ...
     * silent drops if ACP grows a no-sessionId call.
     */
    resolveEntry: (sessionId?: string) => BridgeClientSessionEntry | undefined, resolvePendingRestoreEvents: (sessionId?: string) => EventBus | undefined, 
    /** The multi-client permission coordinator. Owns ALL pending +
     * resolved permission state; this client just plumbs
     * `requestPermission` into `mediator.request` and forwards
     * the resolution to the agent. Strategy dispatch and audit/emit
     ...
     * cache update + generation bump stays atomic in one place.
     */
    onModelPromoted?: ((entry: BridgeClientSessionEntry, modelId: string, originatorClientId: string | undefined) => void) | undefined, 
    /**
     * §2.3 / A2 callback: centralised `approval_mode_changed` publish.
     * Called by the A2 `current_mode_update` demux when the agent
     * switches approval mode in-session (exit_plan_mode, ProceedAlways,
     * /mode). `previous` is read from the bridge state cache.
     */
    onModePromoted?: ((entry: BridgeClientSessionEntry, modeId: string, originatorClientId: string | undefined) => void) | undefined, 
    /**
     * Reverse tool channel (issue #5626, Phase 2). Resolves the
     * `sendSdkMcpMessage`-shaped sender for a client-hosted MCP server name so
     * the `qwen/control/client_mcp/message` ext-method (child → parent) can
     ...
     * `methodNotFound` (no client-hosted server can exist without it).
     */
    clientMcpSender?: ClientMcpMessageSender | undefined, ownsSession?: (sessionId: string) => boolean, 
    /**
     * Optional daemon token-usage hook. Called once per model round with the
     * per-round input/output token increments read from
     * `agent_message_chunk._meta.usage` at {@link sessionUpdate} (the single
     ...
     * round had none), for the daemon's model-API-health charts.
     */
    onTokenUsage?: ((inputTokens: number, outputTokens: number, durationMs?: number, apiErrors?: number, apiRetries?: number) => void) | undefined, 
    /**
     * Daemon-host seam for the `create_sub_session` tool. Invoked from the
     * `extMethod` dispatch (a child→daemon REQUEST, so it returns a Promise the
     * child awaits) with the prompt, completion mode, and optional model/name;
    ...
    /** Request-scoped generation events are routed to a private bridge queue,
     * never to the session EventBus. */
    onGenerationEvent?: ((sessionId: string, event: BridgeGenerationNotificationEvent) => void) | undefined, 
    /** Workspace generation events are session-less and routed to a
     * private bridge queue keyed by requestId. */
    onWorkspaceGenerationEvent?: ((event: BridgeWorkspaceGenerationNotificationEvent) => void) | undefined, onChannelDelivery?: ChannelDeliveryHandler | undefined, 
    /** Permits pre-registration client-MCP discovery without trusting its id. */
    hasSessionSpawnInFlight?: () => boolean, getLiveScreenContextCaptureHandler?: () => LiveScreenContextCaptureHandler | undefined, getLiveTaskToolRequestHandler?: () => LiveTaskToolRequestHandler | undefined, getLiveSpeakToUserHandler?: () => LiveSpeakToUserHandler | undefined, 
    /**
     * Managed tool guard hosted by the daemon. Kept after the Live handlers so
     * existing direct BridgeClient constructors remain source-compatible.
     */
    externalToolGuard?: ExternalToolGuardHandler | undefined, onActiveWork?: ((snapshot: ActiveWorkSnapshotV1) => void) | undefined);
    requestPermission(params: RequestPermissionRequest): Promise<RequestPermissionResponse>;
    sessionUpdate(params: SessionNotification): Promise<void>;
    prepareSessionUpdateFrames(params: SessionNotification, entry?: BridgeClientSessionEntry): PreparedSessionUpdateFrames;
    seedSessionUpdates(entry: BridgeClientSessionEntry, updates: SessionUpdate[], options?: {
 ...
 * Exported for unit testing.
 */
export declare function extractA2uiToolUpdate(params: SessionNotification): A2uiExtraction | null;
export {};
