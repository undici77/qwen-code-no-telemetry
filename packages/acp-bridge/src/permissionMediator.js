 ...
 */
export const CANCEL_VOTE_SENTINEL = '__cancelled__';
/**
 ...
 */
export function createNoOpPermissionAuditPublisher() {
    return {
        recordRequested() { },
        recordVoted() { },
        recordForbidden() { },
 ...
 */
export class MultiClientPermissionMediator {
    policy;
    deps;
    pending = new Map();
    resolved = new Map();
    ...
    request(record, timeoutMs) {
        ...
        if (record.allowedOptionIds.has(CANCEL_VOTE_SENTINEL)) {
            throw new CancelSentinelCollisionError(record.requestId, CANCEL_VOTE_SENTINEL);
        }
        return new Promise((resolve) => {
            ...
            this.safeAudit(() => this.deps.audit.recordRequested(record, policy, votersAtIssue));
            ...
            if (policy === 'consensus' && votersAtIssue.size === 0) {
                try {
                    process.stderr.write(`permissionMediator: consensus request ${record.requestId} ` +
            ...
            if (timeoutMs > 0) {
                pending.timer = setTimeout(() => {
                    ...
                    if (this.pending.get(record.requestId) !== pending)
                        return;
                    ...
                    try {
                        process.stderr.write(`qwen serve: permission ${record.requestId} ` +
                    ...
                    this.resolveEntry(pending, { kind: 'cancelled', reason: 'timeout' }, {
                        type: 'timeout',
    ...
    vote(vote) {
        ...
        if (!pending) {
            ...
            if (prior && prior.sessionId === vote.sessionId) {
                ...
                const optionId = prior.resolution.kind === 'option'
        ...
        if (vote.optionId === CANCEL_VOTE_SENTINEL) {
            ...
            this.safeAudit(() => this.deps.audit.recordVoted(this.toRecord(pending), vote, outcome));
            this.resolveEntry(pending, { kind: 'cancelled', reason: 'agent_cancelled' }, {
    ...
    forgetSession(sessionId) {
        ...
        for (const id of requestIds) {
            ...
            if (!pending)
                continue;
            this.resolveEntry(pending, { kind: 'cancelled', reason: 'session_closed' }, { type: 'session-closed' }, undefined);
        }
    }
    /**
    ...
    voteDesignated(pending, vote) {
        ...
        if (vote.clientId !== pending.originatorClientId) {
            return this.rejectForbidden(pending, vote, 'designated_mismatch', 'designated_mismatch (voter is not the prompt originator)');
        }
        return this.resolveWithVote(pending, vote, {
    ...
    voteConsensus(pending, vote) {
        if (vote.clientId === undefined ||
            !pending.votersAtIssue.has(vote.clientId)) {
            return this.rejectForbidden(pending, vote, 'designated_mismatch', 'designated_mismatch (voter not in consensus votersAtIssue snapshot)');
        }
        for (const [originalOptionId, set] of pending.tallies.entries()) {
            if (vote.clientId !== undefined && set.has(vote.clientId)) {
                ...
                this.safeAudit(() => this.deps.audit.recordVoted(this.toRecord(pending), { ...vote, optionId: originalOptionId }, outcome));
                return outcome;
            }
        }
    ...
    voteLocalOnly(pending, vote) {
        if (!vote.fromLoopback) {
            return this.rejectForbidden(pending, vote, 'remote_not_allowed', 'remote_not_allowed (local-only policy; vote not from loopback)');
        }
        return this.resolveWithVote(pending, vote, {
    ...
    resolveWithVote(pending, vote, decisionReason) {
        ...
        }, decisionReason, vote.clientId);
        return outcome;
    }
    rejectForbidden(pending, vote, reason, stderrDetail) {
        this.safeAudit(() => this.deps.audit.recordForbidden(this.toRecord(pending), vote, reason));
        this.safeEmit(pending.sessionId, {
    ...
    consensusQuorumFor(pending) {
        ...
        if (override !== undefined) {
            ...
            if (capped < override && !pending.consensusQuorumCapNoted) {
                ...
                try {
                    process.stderr.write(`permissionMediator: consensusQuorum override ${override} ` +
    ...
    resolveEntry(pending, resolution, decisionReason, resolverClientId) {
        ...
        if (decisionReason !== undefined) {
            this.safeAudit(() => this.deps.audit.recordResolved(this.toRecord(pending), resolution, decisionReason));
        }
    ...
    safeEmit(sessionId, event) {
        try {
            ...
            try {
                process.stderr.write(`permissionMediator: emit failed for session=${JSON.stringify(sessionId)} type=${JSON.stringify(event.type)}: ${stringifyError(err)}\n`);
            }
            catch {
                // Stderr unavailable — drop the breadcrumb and continue.
            }
    ...
    writeForbiddenStderr(pending, vote, reasonDetail) {
        try {
            const voterDescriptor = vote.clientId === undefined
                ? '<anonymous>'
                : JSON.stringify(vote.clientId);
            process.stderr.write(`qwen serve: permission ${pending.requestId} ` +
    ...
    safeAudit(fn) {
        try {
            ...
            // Promise unsettled.
            try {
                process.stderr.write(`permissionMediator: audit publisher threw: ${stringifyError(err)}\n`);
            }
            catch {
                // Stderr unavailable — drop the breadcrumb and continue.
            }
...
//# sourceMappingURL=permissionMediator.js.map