...
describe('redactLogCredentials', () => {
    ...
    it('redacts Bearer tokens', () => {
        expect(redactLogCredentials('Authorization: Bearer eyJhbGciOi.xyz.abc')).toBe(`Authorization: ${R}`);
    });
    it('redacts bare Bearer (no "Authorization:" prefix)', () => {
        expect(redactLogCredentials('header Bearer t-abc123_def.456')).toBe(`header Bearer ${R}`);
    });
    ...
    it('redacts QQBot tokens', () => {
        expect(redactLogCredentials('token: QQBot abcdef123456')).toBe(`token: QQBot ${R}`);
    });
    // ── Authorization header catch-all ────────────────────────────────
    it('redacts Authorization with Basic scheme', () => {
        expect(redactLogCredentials('Authorization: Basic dXNlcjpwYXNz')).toBe(`Authorization: ${R}`);
    });
    it('redacts Authorization with Digest scheme', () => {
        expect(redactLogCredentials('Authorization: Digest username="user"')).toBe(`Authorization: ${R}`);
    });
    // ── DingTalk access token header ──────────────────────────────────
    it('redacts DingTalk access token header', () => {
        expect(redactLogCredentials('x-acs-dingtalk-access-token: abc123def456')).toBe(`x-acs-dingtalk-access-token: ${R}`);
    });
    // ── API key prefixes ──────────────────────────────────────────────
    ...
    it('redacts API_KEY: assignments', () => {
        expect(redactLogCredentials('api_key: sk-longenoughvalue123')).toBe(`api_key: ${R}`);
    });
    it('redacts password= assignments', () => {
        expect(redactLogCredentials('password=mysecretpassword123')).toBe(`password=${R}`);
    });
    it('redacts secret= assignments', () => {
        expect(redactLogCredentials('secret=verylongsecretvalue')).toBe(`secret=${R}`);
    });
    ...
    it('redacts URL credentials', () => {
        expect(redactLogCredentials('proxy: https://user:pass@proxy.local:8080')).toBe(`proxy: https://${R}@proxy.local:8080`);
    });
    it('redacts multiple userinfo segments', () => {
        expect(redactLogCredentials('http://a:b@c:d@host/path')).toBe(`http://${R}@host/path`);
    });
    // ── Mixed patterns ────────────────────────────────────────────────
    it('redacts multiple credentials on one line', () => {
        const line = 'Authorization: Bearer eyJtoken calling https://user:pass@api.example.com/v1';
        const result = redactLogCredentials(line);
...
//# sourceMappingURL=logRedaction.test.js.map