...
import { DAEMON_ACP_NDJSON_LIMITS, createSpawnChannelFactory, createStderrForwarder, getAcpMemoryArgs, scrubChildEnv, } from './spawnChannel.js';
...
describe('createSpawnChannelFactory env policy', () => {
    ...
    let originalSimple;
    let originalServerToken;
    let originalGuardToken;
    let originalCliEntry;
    let originalRuntimeOnlyForTest;
    beforeEach(() => {
        mockSpawn.mockReset();
    ...
    it('scrubs daemon-only env vars from the spawned ACP child', async () => {
        ...
        expect(spawnOptions?.env).not.toHaveProperty('QWEN_CODE_EXTERNAL_TOOL_GUARD_TOKEN');
    ...
    it('threads NDJSON pipe hooks through daemon-side spawned channels', async () => {
        ...
        child.stdout.write(`${JSON.stringify({ jsonrpc: '2.0', method: 'child-to-daemon' })}\n`);
        await expect(reader.read()).resolves.toMatchObject({
            value: { jsonrpc: '2.0', method: 'child-to-daemon' },
            done: false,
        });
        expect(onMessageSent).toHaveBeenCalledWith(Buffer.byteLength(JSON.stringify({ jsonrpc: '2.0', method: 'daemon-to-child' })));
        expect(onMessageReceived).toHaveBeenCalledWith(Buffer.byteLength(JSON.stringify({ jsonrpc: '2.0', method: 'child-to-daemon' })));
        reader.releaseLock();
        writer.releaseLock();
    });
    ...
    it('rejects invalid known-method params before ACP SDK validation', async () => {
        ...
        const connection = new ClientSideConnection(() => ({}), channel.stream);
        ...
        expect(JSON.stringify(stderr.mock.calls)).not.toContain('Error handling request');
        stderr.mockRestore();
    });
    ...
    it('terminates the tracked child when outbound serialization fails', async () => {
        ...
        cyclic['self'] = cyclic;
        await expect(writer.write(cyclic)).rejects.toBeInstanceOf(TypeError);
        await vi.waitFor(() => expect(child.kill).toHaveBeenCalledWith('SIGTERM'));
        writer.releaseLock();
    });
    it('holds prepared response bytes until the response is written', async () => {
        ...
        releaseOutbound?.();
        expect(() => channel.transportGuard?.reservePreparedResponse(first)).not.toThrow();
        await writer.write({ jsonrpc: '2.0', id: 1, result: first });
        expect(() => channel.transportGuard?.reservePreparedResponse(second)).not.toThrow();
        expect(() => channel.transportGuard?.reservePreparedResponse(third)).toThrow('NDJSON decoded queue is full');
        await expect(channel.transportFailed).resolves.toMatchObject({
    ...
    it('stops estimating a large response once its byte budget is exceeded', async () => {
        ...
        let elementReads = 0;
        const response = new Proxy(Array.from({ length: 10_000 }, () => 0), {
        ...
        expect(() => channel.transportGuard?.reservePreparedResponse(response)).toThrow('NDJSON decoded queue is full');
    ...
    it('keeps the default factory unbounded and validates opt-in limits early', () => {
        ...
        expect(() => createSpawnChannelFactory({
            pipeLimits: {
                maxFrameBytes: 0,
                maxQueuedMessages: 1,
                maxQueuedBytes: 1,
...
describe('createSpawnChannelFactory child-heap observation', () => {
    ...
    it('leaves argv byte-identical while counting what it would have refused', async () => {
        ...
        for (let i = 0; i < limit + 2; i++)
            await factory(`/tmp/w${i}`);
        const observed = mockSpawn.mock.calls.at(-1)?.[1];
        mockSpawn.mockClear();
        await createSpawnChannelFactory({ processRegistry: new ProcessRegistry() })('/tmp/w0');
        const bare = mockSpawn.mock.calls[0]?.[1];
...
describe('createStderrForwarder', () => {
    ...
    it('redacts credentials from forwarded lines', () => {
        ...
        expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('<redacted>'));
        expect(stderrSpy).not.toHaveBeenCalledWith(expect.stringContaining('eyJsecret123'));
        stderrSpy.mockRestore();
    });
...
//# sourceMappingURL=spawnChannel.test.js.map