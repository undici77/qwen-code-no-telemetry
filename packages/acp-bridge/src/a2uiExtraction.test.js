...
import { splitA2uiText, isA2uiToolMeta, extractA2uiToolUpdate, } from './bridgeClient.js';
...
const CMD = (surfaceId, kind = 'updateComponents') => `{"version":"v0.9","${kind}":{"surfaceId":"${surfaceId}","components":[]}}`;
describe('splitA2uiText', () => {
    ...
    it('handles nested arrays and escaped quotes inside strings', () => {
        const text = '[{"version":"v0.9","updateDataModel":{"surfaceId":"s1","path":"/","value":{"rows":[[1,2],[3,4]],"note":"a \\"quoted\\" ] bracket"}}}] tail';
...
describe('isA2uiToolMeta', () => {
    it('matches when serverId contains "a2ui" regardless of tool name', () => {
        expect(isA2uiToolMeta({ serverId: 'a2ui-ui', toolName: 'anything' })).toBe(true);
        expect(isA2uiToolMeta({
            serverId: 'dq-A2UI',
    ...
    it('rejects unrelated tools and missing meta', () => {
        expect(isA2uiToolMeta({ serverId: 'github', toolName: 'create_issue' })).toBe(false);
...
describe('extractA2uiToolUpdate', () => {
    it('ignores non tool_call_update notifications and non-a2ui tools', () => {
        expect(extractA2uiToolUpdate(toolUpdate({
            toolName: 'present_ui',
            serverId: 'a2ui-ui',
            text: `[${CMD('s1')}]`,
            sessionUpdate: 'tool_call',
        }))).toBeNull();
        expect(extractA2uiToolUpdate(toolUpdate({ toolName: 'run_shell_command', text: `[${CMD('s1')}]` }))).toBeNull();
    });
    it('extracts commands, groups by surface, and sanitizes the original frame', () => {
        const text = `[${CMD('s1', 'createSurface')},${CMD('s1')},${CMD('s2')}]\nfallback summary`;
        const result = extractA2uiToolUpdate(toolUpdate({
            serverId: 'a2ui-ui',
        ...
        const sanitized = result.sanitizedParams;
        ...
        expect(sanitized.update.content[0].content.text).not.toContain('createSurface');
    });
    it('accepts updateDataModel-only results and uses the placeholder when fallback is empty', () => {
        ...
        const result = extractA2uiToolUpdate(toolUpdate({ serverId: 'a2ui-ui', text }));
        expect(result).not.toBeNull();
        ...
        const sanitized = result.sanitizedParams;
        expect(sanitized.update.content[0].content.text).toBe('[A2UI surface rendered]');
    });
    ...
    it('returns null when text is not a2ui', () => {
        expect(extractA2uiToolUpdate(toolUpdate({ serverId: 'a2ui-ui', text: 'plain text result' }))).toBeNull();
    });
});
//# sourceMappingURL=a2uiExtraction.test.js.map