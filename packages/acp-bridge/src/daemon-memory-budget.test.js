...
import { detectAvailableMemoryMb, formatMemoryBudgetStderr, isValidMemoryBudgetMb, legacyChildCeilingMb, MAX_CHILD_HEAP_MB, MAX_MEMORY_BUDGET_MB, MIN_CHILD_HEAP_MB, MIN_MEMORY_BUDGET_MB, normalizeMemoryBudgetMb, recommendedChildShareMb, resolveDaemonMemoryBudget, } from './daemon-memory-budget.js';
const MB = 1024 * 1024;
afterEach(() => {
    vi.restoreAllMocks();
});
describe('resolveDaemonMemoryBudget', () => {
    ...
    ])('derives the figures from %i MB of available memory', (availableMemoryMb, configuredBudgetMb, effectiveBudgetMb, rootReserveMb, childPoolMb, legacyChildCeilingMb) => {
    ...
    it('never reports a reserve larger than the budget it comes out of', () => {
        for (const availableMemoryMb of [128, 256, 512, 768]) {
            const budget = resolveDaemonMemoryBudget({ availableMemoryMb });
            expect(budget.rootReserveMb).toBeLessThanOrEqual(budget.effectiveBudgetMb);
    ...
    it('caps an explicit budget at host memory rather than believing it', () => {
        expect(resolveDaemonMemoryBudget({
    ...
    it('honors an explicit budget below host memory', () => {
        expect(resolveDaemonMemoryBudget({ budgetMb: 2_048, availableMemoryMb: 32_768 })).toMatchObject({
    ...
    it('rejects an out-of-range explicit budget', () => {
        expect(() => resolveDaemonMemoryBudget({ budgetMb: 512 })).toThrow(TypeError);
    });
...
describe('normalizeMemoryBudgetMb', () => {
    it.each([0, -1, 1.5, Number.NaN, 1_023, MAX_MEMORY_BUDGET_MB + 1])('rejects %p with a message naming the valid range', (value) => {
        expect(() => normalizeMemoryBudgetMb(value)).toThrow(new RegExp(`\\[${MIN_MEMORY_BUDGET_MB}, ${MAX_MEMORY_BUDGET_MB}\\]`));
    });
    it('accepts the boundaries', () => {
        expect(normalizeMemoryBudgetMb(MIN_MEMORY_BUDGET_MB)).toBe(MIN_MEMORY_BUDGET_MB);
        expect(normalizeMemoryBudgetMb(MAX_MEMORY_BUDGET_MB)).toBe(MAX_MEMORY_BUDGET_MB);
    });
});
describe('recommendedChildShareMb', () => {
    ...
    it('never exceeds the ceiling a child gets today', () => {
        for (const availableMemoryMb of [768, 1_024, 2_048, 8_192, 32_768]) {
            ...
            expect(recommendedChildShareMb(scoped, 1)).toBeLessThanOrEqual(legacyChildCeilingMb(availableMemoryMb));
        }
    });
...
describe('detectAvailableMemoryMb', () => {
    it('prefers a cgroup constraint below the host total', () => {
        ...
        vi.spyOn(process, 'constrainedMemory').mockReturnValue(4_096 * MB);
    ...
    it('ignores a cgroup v1 "unlimited" sentinel above the host total', () => {
        vi.spyOn(os, 'totalmem').mockReturnValue(32_768 * MB);
        vi.spyOn(process, 'constrainedMemory').mockReturnValue(Number.MAX_SAFE_INTEGER);
        expect(detectAvailableMemoryMb()).toEqual({
            memoryMb: 32_768,
            source: 'host',
        });
    ...
    it('treats a cgroup limit exactly equal to the host total as unconstrained', () => {
        ...
        vi.spyOn(process, 'constrainedMemory').mockReturnValue(16_384 * MB);
        expect(detectAvailableMemoryMb()).toEqual({
            memoryMb: 16_384,
            source: 'host',
        });
...
describe('isValidMemoryBudgetMb', () => {
    ...
    it.each([0, -1, 1.5, Number.NaN, 1_023, MAX_MEMORY_BUDGET_MB + 1])('rejects %p', (value) => {
...
describe('formatMemoryBudgetStderr', () => {
    it('formats a plain derived budget above the minimum', () => {
        const budget = resolveDaemonMemoryBudget({ availableMemoryMb: 32_768 });
        expect(formatMemoryBudgetStderr(budget)).toBe('qwen serve: memory budget 16384 MB (derived, 32768 MB available via host)');
    });
    it('formats the capped-down branch', () => {
        ...
        expect(formatMemoryBudgetStderr(budget)).toContain('; capped down from the configured');
    });
    it('names the host floor on the derived insufficient path', () => {
        ...
        expect(message).toContain('a derived budget needs a host with at least ~2048 MB');
...
//# sourceMappingURL=daemon-memory-budget.test.js.map