...
import { DEFAULT_SESSION_RESTORE_TIMEOUT_MS, MAX_SESSION_RESTORE_TIMEOUT_MS, resolveSessionRestoreTimeoutMs, restoreRetryAfterSeconds, } from './session-restore-timeout.js';
...
describe('restoreRetryAfterSeconds', () => {
    ...
    ])('clamps a $budgetMs ms budget to $expected s', ({ budgetMs, expected }) => {
...
describe('resolveSessionRestoreTimeoutMs', () => {
    it('accepts exactly the maximum restore timeout', () => {
        ...
        expect(resolveSessionRestoreTimeoutMs({
    ...
    it('uses the restore default when no related timeout is configured', () => {
        expect(resolveSessionRestoreTimeoutMs({})).toBe(DEFAULT_SESSION_RESTORE_TIMEOUT_MS);
    });
    it('lets a longer initialize timeout raise the restore budget', () => {
        expect(resolveSessionRestoreTimeoutMs({ initializeTimeoutMs: 90_000 })).toBe(90_000);
    });
    it('never lets an initialize timeout lower the restore budget', () => {
        ...
        expect(resolveSessionRestoreTimeoutMs({ initializeTimeoutMs: 10_000 })).toBe(DEFAULT_SESSION_RESTORE_TIMEOUT_MS);
    });
    ...
    it.each(INVALID_TIMEOUTS)('rejects invalid restore timeout %s', (sessionRestoreTimeoutMs) => {
        expect(() => resolveSessionRestoreTimeoutMs({ sessionRestoreTimeoutMs })).toThrow(TypeError);
    });
    it.each(INVALID_TIMEOUTS)('rejects invalid initialize timeout fallback %s', (initializeTimeoutMs) => {
        expect(() => resolveSessionRestoreTimeoutMs({ initializeTimeoutMs })).toThrow(TypeError);
    });
});
//# sourceMappingURL=session-restore-timeout.test.js.map