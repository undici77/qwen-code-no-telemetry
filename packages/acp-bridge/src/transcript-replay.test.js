...
import { createTranscriptReplayMachine, MISSING_TRANSCRIPT_TOOL_RESULT_MESSAGE, } from './transcript-replay.js';
const GOAL = {
    goalId: 'goal-1',
    revision: 3,
    objective: 'ship it',
...
function record(uuid, type, overrides = {}) {
    return {
        uuid,
        parentUuid: null,
        sessionId: 'session-1',
...
function updates(machine, item) {
    return [...machine.project(item)].map((emission) => emission.update);
}
function goalStateRecord(uuid, cause, goal) {
...
function goalCardRecord(uuid, ...items) {
    return record(uuid, 'system', {
...
describe('createTranscriptReplayMachine', () => {
    it('does not replay internal Goal runtime prompts as user messages', () => {
        expect(updates(createTranscriptReplayMachine(), record('goal-runtime', 'user', {
            subtype: 'goal_runtime',
            message: { role: 'user', parts: [{ text: 'Continue working.' }] },
        }))).toEqual([]);
    });
    it('projects goal_state through v2-first metadata', () => {
        const projected = updates(createTranscriptReplayMachine(), goalStateRecord('goal-create', 'create', GOAL));
    ...
    it('tracks Goal state across replay pages', () => {
        ...
        const projected = updates(second, goalStateRecord('goal-clear', 'clear', null));
        expect(projected[0]?._meta).toMatchObject({
    ...
    it('replays a legacy paused goal card instead of leaving the set card newest', () => {
        ...
        expect(updates(machine, goalCardRecord('goal-set', {
            type: 'goal_status',
            kind: 'set',
            condition: GOAL.objective,
        }))).toHaveLength(1);
        const projected = updates(machine, goalCardRecord('goal-paused', {
            type: 'goal_status',
            kind: 'paused',
            condition: GOAL.objective,
            iterations: 4,
    ...
    it('emits legacy goalTerminal metadata for a terminal goal_state', () => {
        const projected = updates(createTranscriptReplayMachine(), goalStateRecord('goal-complete', 'complete', {
            ...GOAL,
            status: 'complete',
        }));
        expect(projected[0]?._meta).toMatchObject({
    ...
    it('skips checkpoint bookkeeping goal_state records during replay', () => {
        const machine = createTranscriptReplayMachine();
        expect(updates(machine, goalStateRecord('goal-create', 'create', GOAL))).toHaveLength(1);
        const turned = {
            ...GOAL,
        ...
        expect(updates(machine, goalStateRecord('goal-turn', 'turn_finished', turned))).toHaveLength(1);
        const checkpointed = {
            ...turned,
        ...
        expect(updates(machine, goalStateRecord('goal-checkpoint', 'checkpoint', checkpointed))).toEqual([]);
        const rejected = {
            ...checkpointed,
            lastReason: 'More work remains',
        };
        expect(updates(machine, goalStateRecord('goal-reject', 'verifier_reject', rejected))).toHaveLength(1);
        const recommitted = {
            ...rejected,
            activeTimeMs: 2900,
            updatedAt: 500,
        };
        expect(updates(machine, goalStateRecord('goal-reject-checkpoint', 'verifier_reject', recommitted))).toEqual([]);
    ...
    it('persists goalCause so bookkeeping suppression survives a page boundary', () => {
        ...
        expect(updates(first, goalStateRecord('goal-reject', 'verifier_reject', rejected))).toHaveLength(1);
        ...
        expect(updates(second, goalStateRecord('goal-reject-checkpoint', 'verifier_reject', recommitted))).toEqual([]);
    ...
    it('emits a repeated verifier rejection that follows an empty turn', () => {
        ...
        expect(updates(machine, goalStateRecord('goal-turn-1', 'turn_finished', turnedOnce))).toHaveLength(1);
        const rejectedOnce = {
            ...turnedOnce,
            lastReason: 'More work remains',
            activeTimeMs: 2200,
        ...
        expect(updates(machine, goalStateRecord('goal-reject-1', 'verifier_reject', rejectedOnce))).toHaveLength(1);
        const turnedTwice = {
            ...rejectedOnce,
        ...
        expect(updates(machine, goalStateRecord('goal-turn-2', 'turn_finished', turnedTwice))).toHaveLength(1);
        ...
        expect(updates(machine, goalStateRecord('goal-reject-2', 'verifier_reject', rejectedTwice))).toHaveLength(1);
        const recommitted = {
            ...rejectedTwice,
            activeTimeMs: 2500,
            updatedAt: 340,
        };
        expect(updates(machine, goalStateRecord('goal-reject-2-checkpoint', 'verifier_reject', recommitted))).toEqual([]);
        expect(machine.snapshot().goalState?.goal).toEqual(recommitted);
    });
    ...
    it('preserves task notification metadata during replay', () => {
        const machine = createTranscriptReplayMachine();
        const projected = updates(machine, record('notification-1', 'user', {
    ...
    it('preserves cron display text and source metadata during replay', () => {
        const projected = updates(createTranscriptReplayMachine(), record('cron-1', 'user', {
            subtype: 'cron',
            message: {
                role: 'user',
    ...
    it('uses clean user display metadata while preserving image parts', () => {
        const machine = createTranscriptReplayMachine();
        const projected = updates(machine, record('user-1', 'user', {
            message: {
                role: 'user',
                parts: [
                    {
    ...
    it('strips only a complete final tag-only context part', () => {
        const projected = updates(createTranscriptReplayMachine(), record('user-1', 'user', {
            message: {
                role: 'user',
                parts: [
                    { text: 'user prompt' },
    ...
    it('preserves Live dialogue boundaries and source during replay', () => {
        const machine = createTranscriptReplayMachine();
        const projected = updates(machine, record('realtime-1', 'assistant', {
    ...
    describe('UserPromptSubmit hook context provenance', () => {
        ...
        it('appends displayText after an image-only record', () => {
            ...
            const projected = updates(createTranscriptReplayMachine(), record('user-img-only', 'user', {
                message: {
                    role: 'user',
                    parts: [
                        {
        ...
        it('does not append empty displayText after an image-only record', () => {
            const onDiagnostic = vi.fn();
            const projected = updates(createTranscriptReplayMachine({ onDiagnostic }), record('user-img-only-empty-display', 'user', {
                message: {
                    role: 'user',
                    parts: [
                        {
        ...
        it('strips a trailing whole-part tagged block when displayText is absent', () => {
            const projected = updates(createTranscriptReplayMachine(), record('user-2', 'user', {
                message: {
                    role: 'user',
        ...
        it('uses released single-field displayText when the final tag proves provenance', () => {
            const projected = updates(createTranscriptReplayMachine(), record('user-single-field-display', 'user', {
                message: {
                    role: 'user',
                    parts: [
                        {
        ...
        it('does not trust bare displayText on plain user records', () => {
            const projected = updates(createTranscriptReplayMachine(), record('user-bare-display', 'user', {
                message: {
                    role: 'user',
                    parts: [
                        {
        ...
        it('treats paired empty displayText as authoritative', () => {
            const projected = updates(createTranscriptReplayMachine(), record('user-empty-display', 'user', {
                message: {
                    role: 'user',
                    parts: [
                        { text: 'expanded model prompt' },
        ...
        it('keeps a sole part that matches the tag shape', () => {
            const projected = updates(createTranscriptReplayMachine(), record('user-3', 'user', {
                message: {
                    role: 'user',
                    parts: [{ text: tagged }],
                },
    ...
    it('projects ordered message parts with source metadata', () => {
        const machine = createTranscriptReplayMachine();
        const projected = updates(machine, record('assistant-1', 'assistant', {
            message: {
                role: 'model',
    ...
    it('correlates an id-less result only to one same-name pending call', () => {
        const machine = createTranscriptReplayMachine();
        updates(machine, record('assistant-1', 'assistant', {
            message: {
                role: 'model',
                parts: [
        ...
        const result = updates(machine, record('result-1', 'tool_result', {
            message: {
                role: 'user',
                parts: [
                    {
    ...
    it('carries versioned state across pages and rejects unknown versions', () => {
        const first = createTranscriptReplayMachine();
        updates(first, record('assistant-1', 'assistant', {
            message: {
                role: 'model',
                parts: [
                    { functionCall: { name: 'read_file', args: {}, id: 'call-1' } },
        ...
        expect(() => createTranscriptReplayMachine({
            initialState: { v: 2 },
    ...
    it('emits gaps, todo plans, and cumulative usage deterministically', () => {
        ...
        const assistant = updates(machine, record('assistant-1', 'assistant', {
        ...
        const plan = updates(machine, record('todo-result', 'tool_result', {
            message: {
                role: 'user',
...
//# sourceMappingURL=transcript-replay.test.js.map