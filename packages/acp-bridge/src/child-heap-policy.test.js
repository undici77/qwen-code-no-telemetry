...
import { MIN_CHILD_HEAP_MB, resolveDaemonMemoryBudget, } from './daemon-memory-budget.js';
describe('createChildHeapPolicy', () => {
    it.each([2_048, 8_192, 32_768, 262_144])('models a partition whose total fits the pool (%i MB host)', (availableMemoryMb) => {
        ...
        const { maxConcurrentChildren, perChildCeilingMb } = createChildHeapPolicy({ budget: b, mode: 'observe' }).snapshot();
        ...
        expect(maxConcurrentChildren * perChildCeilingMb).toBeLessThanOrEqual(b.childPoolMb);
    });
    it('admits no child when the pool cannot cover one, and offers no ceiling', () => {
        ...
        expect(createChildHeapPolicy({ budget: empty, mode: 'observe' }).snapshot()).toMatchObject({ maxConcurrentChildren: 0, perChildCeilingMb: null });
    });
    ...
    it.each([768, 900, 1_000, 1_023])('models no partition when the capped ceiling would fall under the floor (%i MB host, explicit budget)', (availableMemoryMb) => {
    ...
    it('still models the partition at the first budget the floor allows', () => {
        ...
        expect(createChildHeapPolicy({ budget, mode: 'observe' }).snapshot()).toMatchObject({ maxConcurrentChildren: 1, perChildCeilingMb: 512 });
    });
    it('sizes an 8 GB host for seven children, and a large host by the workspace cap', () => {
        ...
        expect(createChildHeapPolicy({
            budget: resolveDaemonMemoryBudget({ availableMemoryMb: 8_192 }),
            mode: 'observe',
        }).snapshot()).toMatchObject({ maxConcurrentChildren: 7, perChildCeilingMb: 526 });
        expect(createChildHeapPolicy({
            budget: resolveDaemonMemoryBudget({ availableMemoryMb: 32_768 }),
            mode: 'observe',
        }).snapshot()).toMatchObject({ maxConcurrentChildren: 25, perChildCeilingMb: 614 });
    });
...
//# sourceMappingURL=child-heap-policy.test.js.map