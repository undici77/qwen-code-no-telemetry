...
import { BridgeChannelClosedError, BridgeTimeoutError, MissingCliEntryError, SessionRestoreTimeoutError, SERVE_ERROR_KINDS, mapDomainErrorToErrorKind, } from './status.js';
...
describe('BridgeChannelClosedError', () => {
    it('preserves the legacy "agent channel closed …" wording per context', () => {
        const sessionErr = new BridgeChannelClosedError('mid-request (session abc-123)');
        expect(sessionErr.name).toBe('BridgeChannelClosedError');
        expect(sessionErr.message).toBe('agent channel closed mid-request (session abc-123)');
        expect(sessionErr.context).toBe('mid-request (session abc-123)');
        expect(sessionErr).toBeInstanceOf(Error);
        expect(new BridgeChannelClosedError('mid-request (workspace status)').message).toBe('agent channel closed mid-request (workspace status)');
        expect(new BridgeChannelClosedError('during session/load').message).toBe('agent channel closed during session/load');
    });
});
...
describe('mapDomainErrorToErrorKind', () => {
    it('classifies BridgeTimeoutError as init_timeout', () => {
        expect(mapDomainErrorToErrorKind(new BridgeTimeoutError('init', 100))).toBe('init_timeout');
    });
    it('classifies restore timeout before its BridgeTimeoutError parent', () => {
        expect(mapDomainErrorToErrorKind(new SessionRestoreTimeoutError('session-1', 'load', 60_000))).toBe('restore_timeout');
    });
    it('classifies SkillError(PARSE_ERROR / INVALID_CONFIG / INVALID_NAME) as parse_error', () => {
        expect(mapDomainErrorToErrorKind(new SkillError('bad yaml', 'PARSE_ERROR'))).toBe('parse_error');
        expect(mapDomainErrorToErrorKind(new SkillError('bad meta', 'INVALID_CONFIG'))).toBe('parse_error');
        expect(mapDomainErrorToErrorKind(new SkillError('bad name', 'INVALID_NAME'))).toBe('parse_error');
    });
    it('classifies SkillError(FILE_ERROR / NOT_FOUND) as missing_file', () => {
        expect(mapDomainErrorToErrorKind(new SkillError('cannot read', 'FILE_ERROR'))).toBe('missing_file');
        expect(mapDomainErrorToErrorKind(new SkillError('absent', 'NOT_FOUND'))).toBe('missing_file');
    });
    ...
    it('classifies SyntaxError as parse_error', () => {
        expect(mapDomainErrorToErrorKind(new SyntaxError('bad json'))).toBe('parse_error');
    });
    ...
    it('classifies BridgeChannelClosedError as protocol_error', () => {
        expect(mapDomainErrorToErrorKind(new BridgeChannelClosedError('mid-request (session abc)'))).toBe('protocol_error');
        expect(mapDomainErrorToErrorKind(new BridgeChannelClosedError('mid-request (workspace status)'))).toBe('protocol_error');
        expect(mapDomainErrorToErrorKind(new BridgeChannelClosedError('during session/load'))).toBe('protocol_error');
    });
    it('classifies MissingCliEntryError as missing_binary', () => {
        expect(mapDomainErrorToErrorKind(new MissingCliEntryError())).toBe('missing_binary');
    });
    it('does NOT classify foreign errors that merely contain bridge phrases', () => {
        ...
        expect(mapDomainErrorToErrorKind(new Error('wrapped: agent channel closed mid-request'))).toBe(undefined);
        expect(mapDomainErrorToErrorKind(new Error('Cannot determine CLI entry path for some other reason'))).toBe(undefined);
    });
    it('returns undefined for unrelated or non-Error values', () => {
        expect(mapDomainErrorToErrorKind(new Error('something else'))).toBe(undefined);
