...
import { LEGACY_CHILD_HEAP_FRACTION, legacyChildCeilingMb, MAX_CHILD_HEAP_MB, } from './daemon-memory-budget.js';
...
describe('spawn-path constant parity', () => {
    it('getAcpMemoryArgs uses the same fraction as legacyChildCeilingMb (unsaturated)', () => {
        ...
        expect(expected).toBe(Math.min(Math.floor(availableMb * LEGACY_CHILD_HEAP_FRACTION), MAX_CHILD_HEAP_MB));
        expect(expected).toBeLessThan(MAX_CHILD_HEAP_MB);
        expect(args).toContain(`--max-old-space-size=${expected}`);
    });
});
//# sourceMappingURL=daemon-memory-budget-parity-fraction.test.js.map