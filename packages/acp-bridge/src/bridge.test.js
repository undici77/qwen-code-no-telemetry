...
import { InvalidClientIdError, BridgeChannelQuarantinedError, InvalidPermissionOptionError, InvalidSessionMetadataError, InvalidSessionScopeError, NOT_CURRENTLY_GENERATING_CANCEL_MESSAGE, PromptDeadlineExceededError, PromptQueueFullError, RestoreInProgressError, SessionShellClientRequiredError, SessionShellDisabledError, SessionBusyError, SessionNotFoundError, TotalSessionLimitExceededError, WorkspaceMismatchError, } from './bridgeErrors.js';
import { MAX_WORKSPACE_PATH_LENGTH } from './workspacePaths.js';
import { SESSION_SOURCE_META_KEY } from './session-source.js';
import { classifyTurnErrorKind, extractErrorMessage, extractErrorCode, } from './bridge.js';
import { BridgeChannelClosedError, BridgeTimeoutError, SessionRestoreTimeoutError, SERVE_CONTROL_EXT_METHODS, SERVE_STATUS_EXT_METHODS, } from './status.js';
import { EXTERNAL_TOOL_GUARD_READY_META_KEY, EXTERNAL_TOOL_GUARD_REQUIRED_VALUE, } from './externalToolGuard.js';
import { createInMemoryChannel } from './inMemoryChannel.js';
import { EventBus } from './eventBus.js';
import { TurnBoundaryCompactionEngine } from './compactionEngine.js';
import { ACTIVE_WORK_HEARTBEAT_INTERVAL_MS, ACTIVE_WORK_CLOSE_TIMEOUT_MS, ACTIVE_WORK_HEARTBEAT_META_KEY, ACTIVE_WORK_HEARTBEAT_VERSION, ACTIVE_WORK_HOLD_CATEGORIES, ACTIVE_WORK_CLOSE_IF_UNHELD_PARAM, ACTIVE_WORK_MAX_SESSION_HOLDS, ACTIVE_WORK_MAX_SNAPSHOT_SESSIONS, ACTIVE_WORK_NOTIFICATION_METHOD, ACTIVE_WORK_STALE_INTERVALS, gradeActiveWorkCoverage, CHANNEL_STARTUP_PROFILE_META_KEY, CHANNEL_STARTUP_PROFILE_VERSION, DAEMON_MODEL_PROMPT_META_KEY, WORKTREE_MCP_DEFER_META_KEY, LOAD_REPLAY_HIDE_INHERITED_META_KEY, } from './bridgeTypes.js';
import { ApprovalMode, SESSION_ARTIFACT_PERSISTENCE_VERSION, ShellExecutionService, stableSessionArtifactId, ToolNames, } from '@qwen-code/qwen-code-core';
import { FakeAgent, makeBridge, makeChannel, WS_A, WS_B, SESS_A, } from './internal/testUtils.js';
import { SessionArtifactAuthorizationError } from './sessionArtifacts.js';
import { REQUESTED_SESSION_ID_META_KEY, MID_TURN_QUEUE_DRAIN_METHOD, MID_TURN_RECONCILIATION_RING_SIZE, PROMPT_CANCEL_METHOD, TODO_STOP_GUARD_CONTINUATION_CLAIM_METHOD, TODO_STOP_GUARD_QUEUE_RELEASE_METHOD, } from './bridgeTypes.js';
function deferred() {
    let resolve;
    let reject;
...
async function advanceRestoreDeadline(timeoutMs) {
    await vi.advanceTimersByTimeAsync(0);
    await vi.advanceTimersByTimeAsync(timeoutMs);
}
async function sendActiveWorkSnapshot(handle, seq, sessions) {
    await handle.agentConnection.extNotification(ACTIVE_WORK_NOTIFICATION_METHOD, {
...
const activeWorkCloseImpl = async (method) => method === SERVE_CONTROL_EXT_METHODS.sessionClose
    ? { closed: true, holds: [] }
    : {};
function activeWorkInitializeResponse(overrides = {}) {
    return {
...
function agentHold(id) {
    return { category: 'agent', id };
}
/**
...
describe('createAcpSessionBridge', () => {
    describe('active work', () => {
        ...
        it('grades a child that omits a category as partial', async () => {
            const handle = makeChannel({
                initializeImpl: () => activeWorkInitializeResponse({ categories: ['agent'] }),
            });
            const bridge = makeBridge({ channelFactory: async () => handle.channel });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            await sendActiveWorkSnapshot(handle, 1, [
        ...
        it('does not tear down a dropped session while an SSE subscriber watches', async () => {
            ...
            await vi.waitFor(() => expect(bridge.getDaemonStatusSnapshot().sessions[0]?.subscriberCount).toBe(1));
            await bridge.detachClient(session.sessionId, session.clientId);
            expect(bridge.sessionCount).toBe(1);
            await sendActiveWorkSnapshot(handle, 1, []);
            await new Promise((resolve) => setTimeout(resolve, 20));
        ...
        it('refuses new prompts while a conditional close is in flight', async () => {
            ...
            await expect(bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'race the teardown' }],
            })).rejects.toThrow(/closing/);
            await expect(bridge.rewindSession(session.sessionId, { promptId: 'whatever' })).rejects.toThrow(/closing/);
        ...
        it('refuses a restore-path attach while a conditional close is in flight', async () => {
            ...
            await expect(bridge.loadSession({
                sessionId: session.sessionId,
                workspaceCwd: WS_A,
            })).rejects.toThrow(/closing/);
            closeResponse.resolve({ closed: true, holds: [] });
        ...
        it('discards an oversized snapshot whole rather than applying part of it', async () => {
            ...
            await sendActiveWorkSnapshot(handle, 2, [
                {
                    sessionId: session.sessionId,
                    holds: Array.from({ length: ACTIVE_WORK_MAX_SESSION_HOLDS + 1 }, (_unused, index) => agentHold(`h${index}`)),
                },
            ]);
            expect(bridge.activeWork).toBe(true);
            ...
            await sendActiveWorkSnapshot(handle, 3, [
                { sessionId: session.sessionId, holds: [] },
                ...Array.from({ length: ACTIVE_WORK_MAX_SNAPSHOT_SESSIONS }, (_unused, index) => ({ sessionId: `filler-${index}`, holds: [] })),
            ]);
            expect(bridge.activeWork).toBe(true);
    ...
    it('streams workspace content without requiring a session', async () => {
        ...
        const stream = bridge.generateWorkspaceContent('say hello', new AbortController().signal, undefined);
    ...
    it('starts a workspace channel for MCP management without a session', async () => {
        ...
        await expect(bridge.manageMcpServer('aone', 'disable', undefined)).resolves.toMatchObject({
            serverName: 'aone',
            action: 'disable',
            ok: true,
            changed: true,
        ...
        await expect(bridge.queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcp, () => ({ discoveryState: 'not_started', servers: [] }))).resolves.toMatchObject({
            discoveryState: 'completed',
    ...
    it('starts a workspace channel when reconnecting an MCP server', async () => {
        const handle = makeChannel({
            extMethodImpl: async (method, params) => method === SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart
                ? {
                    serverName: params.serverName,
                    restarted: true,
                }
        ...
        const channelFactory = vi.fn().mockResolvedValue(handle.channel);
        const bridge = makeBridge({ channelFactory });
        await expect(bridge.invokeWorkspaceCommand(SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart, { serverName: 'aone' })).resolves.toEqual({ serverName: 'aone', restarted: true });
        expect(channelFactory).toHaveBeenCalledTimes(1);
        expect(handle.agent.extMethodCalls).toContainEqual({
    ...
    it('serves completed MCP status without restarting an idle channel', async () => {
        const makeMcpChannel = () => makeChannel({
            extMethodImpl: async (method) => {
        ...
        await bridge.queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcp, () => ({ discoveryState: 'not_started', servers: [] }));
        expect(handle.killed).toBe(true);
        await expect(bridge.queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcp, () => ({ discoveryState: 'not_started', servers: [] }))).resolves.toMatchObject({ discoveryState: 'completed' });
        expect(channelFactory).toHaveBeenCalledTimes(1);
        expect(handle.agent.extMethodCalls.filter((call) => call.method === SERVE_CONTROL_EXT_METHODS.workspaceMcpInitialize)).toHaveLength(1);
        await bridge.shutdown();
    });
                        ...
                        servers: [
                            {
                                ...
                                authenticationState: statusRequests === 1 ? 'pending' : 'succeeded',
                            },
    ...
    it('preserves other MCP states after restarting in a fresh child', async () => {
        ...
        await bridge.initializeWorkspaceMcp();
        await bridge.queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcp, () => ({ discoveryState: 'not_started', servers: [] }));
        await bridge.invokeWorkspaceCommand(SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart, { serverName: 'aone' });
        await expect(bridge.queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcp, () => ({ discoveryState: 'not_started', servers: [] }))).resolves.toMatchObject({
            discoveryState: 'completed',
            servers: [
    ...
    it('serves discovered MCP tools after an idle workspace channel is reaped', async () => {
            ...
            extMethodImpl: async (method) => {
                ...
                if (method === SERVE_STATUS_EXT_METHODS.workspaceMcpTools) {
                    ...
                    return {
                        ...
                        initialized: true,
                        acpChannelLive: true,
                        tools: toolsRequests === 1 ? [] : [{ name: 'search', isValid: true }],
                    };
                }
        ...
        await bridge.initializeWorkspaceMcp();
        await expect(bridge.getWorkspaceMcpToolsStatus('aone')).resolves.toEqual(expect.objectContaining({
            acpChannelLive: true,
            tools: [],
        }));
        await bridge.queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcp, () => ({ discoveryState: 'not_started', servers: [] }));
    ...
    it('reaps a workspace channel when MCP discovery is never polled again', async () => {
        vi.useFakeTimers();
        const handle = makeChannel({
            extMethodImpl: async (method) => method === SERVE_CONTROL_EXT_METHODS.workspaceMcpInitialize
                ? { accepted: true }
                : {},
        });
        const bridge = makeBridge({
    ...
    it('releases a queued cwd change when its session transport fails', async () => {
        ...
        const handle = makeChannel({
            extMethodImpl: (method) => method === SERVE_CONTROL_EXT_METHODS.sessionCd
                ? new Promise(() => { })
                : {},
        });
        handle.channel = {
        ...
        await vi.waitFor(() => expect(handle.agent.extMethodCalls).toContainEqual(expect.objectContaining({
            method: SERVE_CONTROL_EXT_METHODS.sessionCd,
        })));
        failure.resolve(Object.assign(new Error('transport queue failed'), {
    ...
    it('does not fail session lifecycle when the lifecycle callback throws', async () => {
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        await expect(bridge.closeSession(session.sessionId)).resolves.toBe(undefined);
        await bridge.shutdown();
    });
    it('memoizes shutdown and keeps the first workspace-removal reason', async () => {
        ...
        const iterator = bridge
            .subscribeEvents(session.sessionId)[Symbol.asyncIterator]();
    ...
    it('rejects an invalid eventRingSize at construction time', () => {
        expect(() => makeBridge({ eventRingSize: 0 })).toThrow(/Invalid eventRingSize/);
        expect(() => makeBridge({ eventRingSize: -1 })).toThrow(/Invalid eventRingSize/);
        expect(() => makeBridge({ eventRingSize: 1.5 })).toThrow(/Invalid eventRingSize/);
        expect(() => makeBridge({ eventRingSize: Number.NaN })).toThrow(/Invalid eventRingSize/);
        expect(() => makeBridge({ eventRingSize: Number.POSITIVE_INFINITY })).toThrow(/Invalid eventRingSize/);
        // Upper-bound typo defense (1M cap). `80_000_000` here mimics the
        // common shell typo `--event-ring-size 80000000` vs `8000000`.
        expect(() => makeBridge({ eventRingSize: 80_000_000 })).toThrow(/Invalid eventRingSize/);
    });
    it('accepts and rejects BridgeOptions.compactedReplayMaxBytes at construction time', () => {
        ...
        expect(() => makeBridge({ compactedReplayMaxBytes: 4 * 1024 * 1024 })).not.toThrow();
        expect(() => makeBridge({ compactedReplayMaxBytes: 0 })).toThrow(/compactedReplayMaxBytes/);
        expect(() => makeBridge({ compactedReplayMaxBytes: -1 })).toThrow(/compactedReplayMaxBytes/);
        expect(() => makeBridge({ compactedReplayMaxBytes: 1.5 })).toThrow(/compactedReplayMaxBytes/);
        expect(() => makeBridge({ compactedReplayMaxBytes: Number.POSITIVE_INFINITY })).toThrow(/compactedReplayMaxBytes/);
        expect(() => makeBridge({ compactedReplayMaxBytes: 512 * 1024 * 1024 })).toThrow(/compactedReplayMaxBytes/);
    });
    it('sanitizes client artifact provenance fields', async () => {
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        try {
            await bridge.addSessionArtifact(session.sessionId, {
                title: 'Client link',
    ...
    it('keeps client artifacts owned by the issuing client', async () => {
        ...
        const second = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        try {
            const created = await bridge.addSessionArtifact(first.sessionId, {
                title: 'Client link',
                url: 'https://example.com/client',
            }, { clientId: first.clientId });
            const artifactId = created.changes[0].artifactId;
            await expect(bridge.getSessionArtifacts(first.sessionId, {
                clientId: 'forged-client',
            })).rejects.toBeInstanceOf(InvalidClientIdError);
            await expect(bridge.removeSessionArtifact(first.sessionId, artifactId, {
                clientId: second.clientId,
            })).rejects.toBeInstanceOf(SessionArtifactAuthorizationError);
            await expect(bridge.removeSessionArtifact(first.sessionId, artifactId)).rejects.toBeInstanceOf(SessionArtifactAuthorizationError);
            await expect(bridge.getSessionArtifacts(first.sessionId)).resolves.toMatchObject({
    ...
    it('rejects invalid client artifact records instead of dropping them', async () => {
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        try {
            await expect(bridge.addSessionArtifact(session.sessionId, {
                title: 'x'.repeat(201),
                url: 'https://example.com/client',
            }, { clientId: session.clientId })).rejects.toThrow(/title/);
        }
        finally {
            await bridge.shutdown();
        }
    ...
    it('uses bridge telemetry for channel/session/prompt dispatch and prompt metadata injection', async () => {
        ...
        const telemetry = {
            ...
            async runWithContext(captured, fn) {
                events.push(`run:${captured?.captured === true}`);
                return await fn();
            },
    ...
    it('does not fail initialization when span enrichment throws', async () => {
        ...
        await expect(bridge.spawnOrAttach({ workspaceCwd: WS_A })).resolves.toMatchObject({ workspaceCwd: WS_A });
        await bridge.shutdown();
    });
    ...
    it('ends the channel wait span and propagates channel spawn failures', async () => {
        ...
        await expect(bridge.spawnOrAttach({ workspaceCwd: WS_A })).rejects.toBe(channelError);
        expect(events).toEqual([
            'channel.wait:start',
            'channel.spawn:start',
            'channel.spawn:end',
    ...
    it('forwards childEnvOverrides to the channelFactory at spawn time (#4247 R6 line 216)', async () => {
        ...
        expect(seenEnvs[0]).toEqual({
            QWEN_SERVE_MCP_CLIENT_BUDGET: '5',
            QWEN_SERVE_MCP_BUDGET_MODE: 'enforce',
            QWEN_CODE_PRIVATE_ACP_CAPABILITY: expect.stringMatching(/^[A-Za-z0-9_-]{43}$/u),
        });
        ...
        const capabilities = seenEnvs.map((env) => env?.['QWEN_CODE_PRIVATE_ACP_CAPABILITY']);
        ...
        expect(handles[0]?.agent.initializeCalls[0]?._meta?.['qwen-code/private-parent-capability']).toBe(capabilities[0]);
        expect(handles[1]?.agent.initializeCalls[0]?._meta?.['qwen-code/private-parent-capability']).toBe(capabilities[1]);
    ...
    it('rejects a required Guard channel whose child does not acknowledge enforcement', async () => {
        ...
        await expect(bridge.spawnOrAttach({ workspaceCwd: WS_A })).rejects.toThrow('did not acknowledge');
    ...
    it('accepts a required Guard channel only after the child acknowledges enforcement', async () => {
        ...
        await expect(bridge.spawnOrAttach({ workspaceCwd: WS_A })).resolves.toMatchObject({ sessionId: SESS_A });
    ...
    it('requests session status through the existing ACP channel', async () => {
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        await expect(bridge.getSessionContextStatus(session.sessionId)).resolves.toMatchObject({
            sessionId: session.sessionId,
            state: {},
        });
        await expect(bridge.getSessionSupportedCommandsStatus(session.sessionId)).resolves.toMatchObject({
            sessionId: session.sessionId,
            availableCommands: [],
            availableSkills: [],
        });
        await expect(bridge.getSessionTasksStatus(session.sessionId)).resolves.toMatchObject({
            sessionId: session.sessionId,
            tasks: [],
        });
        await expect(bridge.getSessionLspStatus(session.sessionId)).resolves.toMatchObject({
            sessionId: session.sessionId,
            enabled: true,
            configuredServers: 1,
            servers: [
    ...
    it('rejects malformed workspace memory remember responses', async () => {
        ...
        await expect(bridge.runWorkspaceMemoryRemember({
            content: 'Remember the workspace uses vitest.',
            contextMode: 'workspace',
    ...
    it('rejects malformed workspace memory forget responses', async () => {
        ...
        await expect(bridge.runWorkspaceMemoryForget({
            query: 'old preference',
    ...
    it('maps a missing transcript session (resourceNotFound) to SessionNotFoundError', async () => {
        ...
        const bridge = makeBridge({ channelFactory: async () => handle.channel });
        await expect(bridge.getSessionTranscriptPage({ sessionId: 'missing-transcript' })).rejects.toMatchObject({
    ...
    it('rejects malformed workspace memory dream responses', async () => {
        ...
        await expect(bridge.runWorkspaceMemoryDream()).rejects.toThrow('Malformed workspace memory dream response');
        expect(handles[0]?.agent.newSessionCalls).toHaveLength(0);
        expect(handles[0]?.agent.loadSessionCalls).toHaveLength(0);
        expect(handles[0]?.agent.resumeSessionCalls).toHaveLength(0);
        await bridge.shutdown();
    ...
    it('keeps the channel alive when availability probes overlap a remember run', async () => {
        const rememberRelease = deferred();
        const handles = [];
        const bridge = makeBridge({
            channelFactory: async () => {
                const h = makeChannel({
        ...
        await expect(bridge.isWorkspaceMemoryRememberAvailable()).resolves.toBe(true);
    ...
    it('keeps the channel alive when availability probes overlap session creation', async () => {
        const newSessionRelease = deferred();
        const handles = [];
        const bridge = makeBridge({
            channelFactory: async () => {
                const h = makeChannel({
    ...
    it('allows client MCP discovery during session pre-registration only', async () => {
        ...
        const handle = makeChannel({
            newSessionImpl: async (params) => {
                ...
                await expect(handles[0].agentConnection.extMethod(SERVE_CONTROL_EXT_METHODS.clientMcpMessage, {
                    server: 'channel-loop',
                    sessionId,
        ...
        await handle.agentConnection.extMethod(SERVE_CONTROL_EXT_METHODS.clientMcpMessage, {
            server: 'channel-loop',
            sessionId: session.sessionId,
            payload: { jsonrpc: '2.0', id: 2, method: 'tools/list' },
        });
        expect(contexts).toEqual([undefined, { sessionId: session.sessionId }]);
        await expect(handle.agentConnection.extMethod(SERVE_CONTROL_EXT_METHODS.clientMcpMessage, {
            server: 'channel-loop',
    ...
    it('redacts client request errors before the ACP SDK logs them', async () => {
        ...
        try {
            ...
            expect(reservePreparedResponse).toHaveBeenCalledWith(expect.objectContaining({ code: -32602 }));
        }
        finally {
            stderr.mockRestore();
            await bridge.shutdown();
    ...
    it('fails the bounded channel when active ACP handlers reach the cap', async () => {
        ...
        const send = vi
            .fn()
            .mockImplementation((_payload, _context) => {
            const gate = sendGates[send.mock.calls.length - 1];
            return gate?.promise ?? Promise.resolve({});
        });
        const handle = makeChannel({});
        const fail = vi.fn((error) => transportFailure.resolve(error));
        handle.channel = {
            ...handle.channel,
            transportFailed: transportFailure.promise,
            transportGuard: {
        ...
        try {
            await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            const calls = [1, 2, 3].map((id) => handle.agentConnection
            ...
            expect(fail).toHaveBeenCalledWith(expect.objectContaining({ code: 'acp_handler_limit_exceeded' }));
            expect(send).toHaveBeenCalledTimes(2);
    ...
    it('bounds outbound operations before they enter the ACP SDK write queue', async () => {
        ...
        try {
            ...
            await expect(bridge.sendPrompt(second.sessionId, {
                sessionId: second.sessionId,
                prompt: [{ type: 'text', text: 'second' }],
            })).rejects.toBeDefined();
            expect(fail).toHaveBeenCalledWith(expect.objectContaining({ code: 'ndjson_queue_limit_exceeded' }));
            expect(handle.agent.promptCalls).toHaveLength(1);
            await expect(firstPrompt).resolves.toBeInstanceOf(BridgeChannelClosedError);
        }
        finally {
    ...
    it('bounds a hung session extension refresh', async () => {
        ...
        const handle = makeChannel({
            extMethodImpl: async (method) => method === SERVE_CONTROL_EXT_METHODS.workspaceExtensionsRefresh
        ...
        try {
            ...
            expect(handle.agent.extMethodCalls.filter((call) => call.method ===
                SERVE_CONTROL_EXT_METHODS.workspaceExtensionsRefresh)).toHaveLength(1);
            await vi.advanceTimersByTimeAsync(30_000);
            ...
            expect(handle.agent.extMethodCalls.filter((call) => call.method ===
                SERVE_CONTROL_EXT_METHODS.workspaceExtensionsRefresh)).toHaveLength(2);
        }
        finally {
            refreshGate.resolve({});
            vi.useRealTimers();
    ...
    it('rejects session status requests for unknown sessions', async () => {
        ...
        await expect(bridge.getSessionContextStatus('missing')).rejects.toBeInstanceOf(SessionNotFoundError);
        await expect(bridge.getSessionSupportedCommandsStatus('missing')).rejects.toBeInstanceOf(SessionNotFoundError);
        await expect(bridge.getSessionTasksStatus('missing')).rejects.toBeInstanceOf(SessionNotFoundError);
        await expect(bridge.getSessionLspStatus('missing')).rejects.toBeInstanceOf(SessionNotFoundError);
    });
    ...
    it('detachClient unregisters only the detached client id', async () => {
        ...
        await expect(bridge.sendPrompt(first.sessionId, {
            sessionId: first.sessionId,
            prompt: [{ type: 'text', text: 'still valid' }],
        }, undefined, { clientId: first.clientId })).resolves.toMatchObject({ stopReason: 'end_turn' });
        expect(() => bridge.sendPrompt(second.sessionId, {
            sessionId: second.sessionId,
            prompt: [{ type: 'text', text: 'detached' }],
        }, undefined, { clientId: second.clientId })).toThrow(InvalidClientIdError);
    ...
    describe('recordHeartbeat', () => {
        ...
        it('records per-client timestamps when a trusted client id is supplied', async () => {
            ...
            const state = bridge.getHeartbeatState(session.sessionId);
            expect(state?.sessionLastSeenAt).toBe(result.lastSeenAt);
            expect(state?.clientLastSeenAt.get(session.clientId)).toBe(result.lastSeenAt);
            await bridge.shutdown();
        });
        it('throws SessionNotFoundError on unknown sessions', async () => {
            ...
            expect(() => bridge.recordHeartbeat('missing')).toThrow(SessionNotFoundError);
        ...
        it('rejects an unknown client id without bumping any timestamp', async () => {
            ...
            expect(() => bridge.recordHeartbeat(session.sessionId, { clientId: 'forged' })).toThrow(InvalidClientIdError);
            const state = bridge.getHeartbeatState(session.sessionId);
        ...
        it('returns a snapshot map that callers cannot use to mutate live state', async () => {
            ...
            snapshot.clientLastSeenAt.set('attacker', 0);
    ...
    it('loads an existing ACP session and registers it for daemon routes', async () => {
        ...
        expect(bridge.sessionCount).toBe(1);
        await expect(bridge.sendPrompt('persisted-1', {
            sessionId: 'ignored',
    ...
    it('restores artifact snapshots that omit marker arrays after fork remap', async () => {
        ...
        await expect(bridge.getSessionArtifacts(loaded.sessionId)).resolves.toMatchObject({
            warnings: ['skipped corrupt artifact record'],
            artifacts: [
                {
                    id: artifactId,
    ...
    it('clears durable artifacts but keeps live ephemerals when rewind returns an empty artifact snapshot', async () => {
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        const durable = await bridge.addSessionArtifact(session.sessionId, {
            title: 'Abandoned durable artifact',
            url: 'https://example.com/later-durable',
        }, { clientId: session.clientId });
        const ephemeral = await bridge.addSessionArtifact(session.sessionId, {
        ...
        await expect(bridge.rewindSession(session.sessionId, { promptId: 'prompt-1' }, { clientId: session.clientId })).resolves.toMatchObject({ targetTurnIndex: 0 });
        ...
        expect(artifacts.some((artifact) => artifact.id === durable.changes[0]?.artifactId)).toBe(false);
    ...
    it('keeps durable artifacts when rewind cannot rebuild an artifact snapshot', async () => {
        ...
        await expect(bridge.rewindSession(session.sessionId, { promptId: 'prompt-1' }, { clientId: session.clientId })).resolves.toMatchObject({ targetTurnIndex: 0 });
        await expect(bridge.getSessionArtifacts(session.sessionId)).resolves.toEqual(expect.objectContaining({
            artifacts: [
                expect.objectContaining({
    ...
    it('restores and persists artifact snapshots returned by rewind', async () => {
        ...
        const rewoundArtifactId = stableSessionArtifactId(SESS_A, `url:${rewoundUrl}`);
        const stickyArtifactId = stableSessionArtifactId(SESS_A, `url:${stickyUrl}`);
        const persistedSnapshots = [];
        const bridge = makeBridge({
            channelFactory: async () => makeChannel({
                extMethodImpl: (method, params) => {
        ...
        const artifactChanges = (async () => {
            ...
            for await (const event of iter) {
                if (event.type !== 'artifact_changed')
                    continue;
                changes.push(event.data.change);
                if (changes.length === 2)
                    return changes;
            }
            return changes;
        ...
        expect(persistedSnapshots[0].artifacts).toHaveLength(1);
        await bridge.shutdown();
    });
    it('surfaces rewind artifact snapshot persistence warnings', async () => {
        ...
        await expect(bridge.rewindSession(session.sessionId, { promptId: 'prompt-1' }, { clientId: session.clientId })).resolves.toMatchObject({
            targetTurnIndex: 0,
    ...
    it('loads history replay from response metadata when requested', async () => {
        ...
        expect(loaded.compactedReplay?.[0]?._meta?.['serverTimestamp']).toBe(1_700_000_000_000);
        const iterator = bridge
            .subscribeEvents(loaded.sessionId, { lastEventId: 0 })[Symbol.asyncIterator]();
    ...
    it('skips empty persisted pages when refreshing an attached load', async () => {
        ...
        expect(JSON.stringify(refreshed.compactedReplay)).toContain('visible prompt');
        await bridge.shutdown();
    });
    ...
    it('falls back to the live replay when a bounded refresh stays unstable', async () => {
        ...
        expect(JSON.stringify(refreshed.compactedReplay)).not.toContain('bounded page');
    ...
    it('falls back to the live replay when a bounded refresh read fails', async () => {
        ...
        expect(JSON.stringify(refreshed.compactedReplay)).toContain('initial prompt');
        await bridge.shutdown();
    });
    it('preserves session_closing when bounded refresh overlaps channel teardown', async () => {
        ...
        transportFailure.resolve(Object.assign(new Error('transport queue failed'), {
            code: 'ndjson_queue_limit_exceeded',
        }));
    ...
    it('bounds response-mode load replay and emits a history_truncated marker', async () => {
        ...
        const retained = loaded.compactedReplay?.[1]?.data;
    ...
    it('attributes live journal markers across queued prompts to the active prompt', async () => {
        ...
        const secondPromptGate = deferred();
        const handle = makeChannel({
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        const firstPrompt = bridge.sendPrompt(session.sessionId, {
            sessionId: session.sessionId,
            prompt: [{ type: 'text', text: 'first turn' }],
        }, undefined, { clientId: session.clientId, promptId: 'prompt-first' });
        const secondPrompt = bridge.sendPrompt(session.sessionId, {
            sessionId: session.sessionId,
            prompt: [{ type: 'text', text: 'second turn' }],
        }, undefined, { clientId: session.clientId, promptId: 'prompt-second' });
        await vi.waitFor(() => expect(handle.agent.promptCalls).toHaveLength(1));
        ...
        expect(duringTurn.liveJournal?.find((event) => event.type === 'history_truncated')).toMatchObject({
        ...
        expect(duringSecondTurn.liveJournal?.find((event) => event.type === 'history_truncated')).toMatchObject({
        ...
        expect(afterTurn.liveJournal).not.toContainEqual(expect.objectContaining({ type: 'history_truncated' }));
        expect(afterTurn.compactedReplay).not.toContainEqual(expect.objectContaining({ type: 'history_truncated' }));
        expect(JSON.stringify(afterTurn.compactedReplay)).toContain('first-afirst-bfirst-c');
        expect(JSON.stringify(afterTurn.compactedReplay)).toContain('second-asecond-bsecond-c');
        await bridge.shutdown();
    });
    ...
    it('rejects oversized response-mode load replay payloads', async () => {
        ...
        await expect(bridge.loadSession({
            ...
            workspaceCwd: WS_A,
            historyReplay: 'response',
        })).rejects.toThrow('qwen.session.loadReplay updates exceed limit (10001 > 10000)');
        await bridge.shutdown();
    });
    it('removes a response-mode restore entry if replay seeding throws', async () => {
        ...
        try {
            ...
            expect(handles.reduce((count, handle) => count + handle.agent.loadSessionCalls.length, 0)).toBe(2);
        }
        finally {
            seedSpy.mockRestore();
            await bridge.shutdown();
    ...
    it('reports the bad index when response-mode load replay is invalid', async () => {
        ...
        await expect(bridge.loadSession({
            ...
            workspaceCwd: WS_A,
            historyReplay: 'response',
        })).rejects.toThrow(/Invalid qwen\.session\.loadReplay update at index 1 .*count=2.*future_update_type/);
        await bridge.shutdown();
    });
    ...
    it('buffers load replay events until the restored session is registered', async () => {
        ...
        const next = await Promise.race([
            ...
            new Promise((_, reject) => {
                timer = setTimeout(() => reject(new Error('timed out waiting for replay event')), 500);
            }),
        ]);
        if (timer)
            clearTimeout(timer);
    ...
    it('propagates the original ACP state to coalesced restore waiters', async () => {
        let releaseLoad;
        const factory = async () => makeChannel({
            loadSessionImpl: () => new Promise((resolve) => {
                releaseLoad = resolve;
            }),
        }).channel;
        const bridge = makeBridge({ channelFactory: factory });
    ...
    it.each([0, 501, 1.5])('rejects invalid response history page %s before restore', async (historyPageSize) => {
        const handle = makeChannel();
        const bridge = makeBridge({
            channelFactory: async () => handle.channel,
        });
    ...
    ])('rejects response restore coalescing with %s', async (_label, historyPageSize) => {
        const load = deferred();
        const handle = makeChannel({ loadSessionImpl: () => load.promise });
        const bridge = makeBridge({
            channelFactory: async () => handle.channel,
    ...
    it('ignores history pages when coalescing streamed loads', async () => {
        ...
        expect(handle.agent.loadSessionCalls).toHaveLength(1);
        expect(handle.agent.loadSessionCalls[0]?._meta).not.toHaveProperty('qwen.session.loadReplayPageSize');
        await bridge.shutdown();
    });
    it('ignores history pages when coalescing resumes', async () => {
        ...
        for (let i = 0; i < 50 && handle.agent.resumeSessionCalls.length !== 1; i++) {
            await new Promise((resolve) => setTimeout(resolve, 10));
        }
    ...
    it('does not kill the channel when restore fails while a spawn is pending', async () => {
        ...
        for (let i = 0; i < 50 && handles[0]?.agent.newSessionCalls.length !== 1; i++) {
            await new Promise((r) => setTimeout(r, 10));
        }
        expect(handles).toHaveLength(1);
        expect(handles[0].agent.newSessionCalls).toHaveLength(1);
    ...
    it('keeps a shared channel usable when restore fails beside a live session', async () => {
        ...
        expect(handles[0].killed).toBe(false);
        await expect(bridge.setSessionLanguage(live.sessionId, {
            language: 'en',
    ...
    it('rejects load while a resume for the same session is in flight', async () => {
        let releaseResume;
        const factory = async () => makeChannel({
            resumeSessionImpl: () => new Promise((resolve) => {
    ...
    it('rejects resume while a load for the same session is in flight (mirror of load-on-resume)', async () => {
        ...
        await expect(bridge.resumeSession({
            sessionId: 'persisted-mirror',
            workspaceCwd: WS_A,
        })).rejects.toBeInstanceOf(RestoreInProgressError);
        releaseLoad?.();
    ...
    it('does not kill a shared channel when one of multiple pending restores fails', async () => {
        ...
        for (let i = 0; i < 50 && handles[0]?.agent.loadSessionCalls.length !== 1; i++) {
            await new Promise((r) => setTimeout(r, 10));
        }
        expect(handles[0]?.agent.loadSessionCalls[0]?.sessionId).toBe('good-restore');
        await expect(bridge.loadSession({
    ...
    it('shutdown awaits in-flight restores before resolving', async () => {
        ...
        const restoreFirst = restore
            .catch(() => undefined)
            .then(() => 'restore');
        const shutdownFirst = bridge.shutdown().then(() => 'shutdown');
    ...
    it('times out an empty restore channel without stopping the bridge', async () => {
        ...
        try {
            ...
            const outcome = restore.catch((error) => error);
    ...
    it.each(['load', 'resume'])('traces and injects context into session/%s', async (action) => {
        const handle = makeChannel();
        const spans = [];
        const timeline = [];
        const injectPromptContext = vi.fn((request) => {
            const meta = request._meta ?? {};
            return {
                ...request,
        ...
        const telemetry = {
            ...
            event: (name, attributes) => {
                if (name === 'session.restore.public_result') {
                    timeline.push(`public:${String(attributes['qwen-code.daemon.session_restore.result'])}`);
                }
            },
            injectPromptContext,
        };
    ...
    it('lets transport close win when it settles before the restore deadline', async () => {
        ...
        await vi.waitFor(() => expect(handle.agent.loadSessionCalls).toHaveLength(1));
        handle.crash({ exitCode: 1, signalCode: null });
    ...
    it('does not reap a timed-out restore channel during workspace control', async () => {
        vi.useFakeTimers();
        const lateRestore = deferred();
        const workspaceControl = deferred();
        const handle = makeChannel({
            loadSessionImpl: () => lateRestore.promise,
            extMethodImpl: (method) => method === SERVE_CONTROL_EXT_METHODS.workspaceMemoryRememberAvailability
        ...
        try {
            ...
            lateRestore.resolve({});
            await vi.advanceTimersByTimeAsync(0);
            expect(handle.agent.extMethodCalls.filter((call) => call.method === SERVE_CONTROL_EXT_METHODS.sessionClose &&
                call.params['sessionId'] === 'restore-timeout-workspace-control')).toHaveLength(0);
        }
        finally {
            await bridge.shutdown();
            vi.useRealTimers();
    ...
    it('fences a shared timed-out restore until one late close settles', async () => {
        ...
        const bridge = makeBridge({
            ...
            channelFactory: async () => handle.channel,
            telemetry: restoreEvents.telemetry,
            freshSessionAdmission: (context) => context.operation === 'load'
        ...
        try {
            ...
            const outcome = timedOut.catch((error) => error);
            await advanceRestoreDeadline(20);
            expect(await outcome).toBeInstanceOf(SessionRestoreTimeoutError);
            expect(handle.killed).toBe(false);
            ...
            await expect(bridge.spawnOrAttach({
                workspaceCwd: WS_A,
                sessionScope: 'thread',
            })).rejects.toThrow(/session limit/i);
            await expect(bridge.sendPrompt(sibling.sessionId, {
                sessionId: sibling.sessionId,
                prompt: [{ type: 'text', text: 'still alive' }],
            })).resolves.toMatchObject({ stopReason: 'end_turn' });
            expect(() => bridge.getSessionSummary('restore-timeout-shared')).toThrow(SessionNotFoundError);
            lateRestore.resolve({});
            await vi.advanceTimersByTimeAsync(0);
            expect(handle.agent.extMethodCalls.filter((call) => call.method === SERVE_CONTROL_EXT_METHODS.sessionClose &&
                call.params['sessionId'] === 'restore-timeout-shared')).toHaveLength(1);
            await vi.advanceTimersByTimeAsync(0);
    ...
    it('drains a settlement-overdue channel without waiting on its wedged child', async () => {
        ...
        const handle = makeChannel({
            ...
            extMethodImpl: (method) => method === SERVE_CONTROL_EXT_METHODS.sessionClose
                ? new Promise(() => { })
                : Promise.resolve({}),
        });
        const bridge = makeBridge({
    ...
    it('refuses a caller-supplied id that a restore still owns', async () => {
        ...
        try {
            ...
            expect(duringActive.message).toContain('retry the spawn');
            await advanceRestoreDeadline(20);
            expect(await timedOut).toBeInstanceOf(SessionRestoreTimeoutError);
    ...
    it('reserves a caller-supplied id against concurrent restores until spawn settles', async () => {
        const requestedSpawn = deferred();
        const handle = makeChannel({
            newSessionImpl: (params) => {
                const requestedSessionId = typeof params._meta === 'object' && params._meta !== null
    ...
    it('hands a settled abandoned id to a caller-supplied spawn before newSession events', async () => {
        ...
        const handle = makeChannel({
            ...
            newSessionImpl: async (params) => {
                ...
                if (requestedSessionId === 'settled-retry') {
                    await handle.agentConnection.extNotification('qwen/notify/session/mcp-budget-event', {
                        v: 1,
        ...
        try {
            ...
            const events = bridge
                .subscribeEvents(retried.sessionId, { lastEventId: 0 })[Symbol.asyncIterator]();
    ...
    it('never closes or fences an id a live session has reclaimed', async () => {
        ...
        const handle = makeChannel({
            loadSessionImpl: () => lateRestore.promise,
            newSessionImpl: (p) => p.cwd === WS_A && handle.agent.newSessionCalls.length === 2
                ? { sessionId: 'reclaimed' }
                : {
        ...
        try {
            ...
            // child sends it.
            const events = bridge
                .subscribeEvents('reclaimed', { lastEventId: 0 })[Symbol.asyncIterator]();
            await expect(events.next()).resolves.toMatchObject({
                value: { type: 'replay_complete' },
            });
            ...
            await vi.advanceTimersByTimeAsync(0);
            expect(handle.agent.extMethodCalls.filter((call) => call.method === SERVE_CONTROL_EXT_METHODS.sessionClose &&
                call.params['sessionId'] === 'reclaimed')).toHaveLength(0);
    ...
    it('cleans a late restore failure exactly once before releasing its fence', async () => {
        ...
        let loadAttempt = 0;
        const handle = makeChannel({
            loadSessionImpl: () => ++loadAttempt === 1 ? lateRestore.promise : Promise.resolve({}),
            extMethodImpl: (method, params) => {
                if (method === SERVE_CONTROL_EXT_METHODS.sessionClose) {
                    throw RequestError.resourceNotFound(`session:${String(params['sessionId'])}`);
                }
                return {};
            },
        });
        ...
        try {
            ...
            await vi.advanceTimersByTimeAsync(0);
            expect(handle.agent.extMethodCalls.filter((call) => call.method === SERVE_CONTROL_EXT_METHODS.sessionClose &&
                call.params['sessionId'] === 'restore-late-failure')).toHaveLength(1);
            await vi.advanceTimersByTimeAsync(0);
            expect(() => bridge.getSessionSummary('restore-late-failure')).toThrow(SessionNotFoundError);
            await expect(bridge.loadSession({
                sessionId: 'restore-late-failure',
                workspaceCwd: WS_A,
    ...
    it('allows a different restore id while an abandoned id remains fenced', async () => {
        vi.useFakeTimers();
        const abandonedRestore = deferred();
        const handle = makeChannel({
            loadSessionImpl: (params) => params.sessionId === 'restore-abandoned-id'
        ...
        try {
            ...
            await vi.advanceTimersByTimeAsync(0);
            expect(handle.agent.extMethodCalls.filter((call) => call.method === SERVE_CONTROL_EXT_METHODS.sessionClose &&
                call.params['sessionId'] === 'restore-abandoned-id')).toHaveLength(1);
        }
        finally {
            await bridge.shutdown();
            vi.useRealTimers();
    ...
    it('does not reap a timed-out restore while another restore is pending', async () => {
        ...
        const handle = makeChannel({
            loadSessionImpl: (params) => params.sessionId === 'restore-first'
        ...
        try {
            ...
            await vi.advanceTimersByTimeAsync(0);
            expect(handle.agent.extMethodCalls.filter((call) => call.method === SERVE_CONTROL_EXT_METHODS.sessionClose &&
                call.params['sessionId'] === 'restore-first')).toHaveLength(1);
            await expect(bridge.loadSession({
                sessionId: 'restore-first',
                workspaceCwd: WS_A,
    ...
    it('does not reap a timed-out restore while a session spawn is pending', async () => {
        ...
        try {
            ...
            await vi.advanceTimersByTimeAsync(0);
            expect(handle.agent.extMethodCalls.filter((call) => call.method === SERVE_CONTROL_EXT_METHODS.sessionClose &&
                call.params['sessionId'] === 'restore-timeout-pending-spawn')).toHaveLength(1);
            expect(handle.killed).toBe(false);
        }
        finally {
            await bridge.shutdown();
    ...
    it('quarantines only fresh work when late restore cleanup fails', async () => {
        ...
        try {
            ...
            expect(() => bridge.recordHeartbeat(sibling.sessionId, {
            ...
            await expect(bridge.branchSession(sibling.sessionId, {
    ...
    it('rejects cross-workspace requests with WorkspaceMismatchError (#3803 §02)', async () => {
        ...
        expect(err.bound).toBe(WS_A);
        expect(err.requested).toBe(WS_B);
    ...
    it('WorkspaceMismatchError truncates oversized `requested` to MAX_WORKSPACE_PATH_LENGTH (defense-in-depth)', () => {
        ...
        expect(err.requested.length).toBeLessThanOrEqual(MAX_WORKSPACE_PATH_LENGTH + 32);
        ...
        expect(err.message.length).toBeLessThan(MAX_WORKSPACE_PATH_LENGTH * 2 + 1024);
    ...
    it('WorkspaceMismatchError passes through `requested` shorter than MAX_WORKSPACE_PATH_LENGTH untouched', () => {
        ...
        expect(err.message).toContain('will not fall back to the primary workspace');
    });
    ...
    it('rejects an invalid per-request sessionScope with InvalidSessionScopeError', async () => {
        ...
        expect(err.sessionScope).toBe('bogus');
        expect(err.message).toMatch(/Invalid sessionScope/);
    });
    it('rejects relative workspace paths', async () => {
        ...
        await expect(bridge.spawnOrAttach({ workspaceCwd: 'relative/path' })).rejects.toThrow(/absolute path/);
    });
    ...
    it('kills the spawned channel and rejects when initialize fails', async () => {
        ...
        const err = await bridge.spawnOrAttach({ workspaceCwd: WS_A }).then(() => null, (e) => e);
    ...
    it('times out a stuck initialize', async () => {
        ...
        await expect(bridge.spawnOrAttach({ workspaceCwd: WS_A })).rejects.toThrow(/initialize timed out/);
        expect(handles[0]?.killed).toBe(true);
        expect(bridge.sessionCount).toBe(0);
    });
    ...
    it('waits for every live channel before reporting a shutdown failure', async () => {
        const handles = [];
        const teardowns = [deferred(), deferred()];
        const factory = async () => {
    ...
    it('transport failure marks the channel dying before process exit', async () => {
        ...
        const factory = async () => {
            const handle = makeChannel({
                ...
                loadSessionImpl: () => new Promise(() => { }),
                extMethodImpl: (method) => method === SERVE_STATUS_EXT_METHODS.sessionTranscript
                    ? new Promise(() => { })
                    : {},
            });
            const failure = deferred();
        ...
        await vi.waitFor(() => expect(handles[0].agent.promptCalls).toHaveLength(1));
        ...
        await vi.waitFor(() => expect(handles[0].agent.extMethodCalls).toContainEqual(expect.objectContaining({
            method: SERVE_STATUS_EXT_METHODS.sessionTranscript,
        })));
        failures[0].resolve(Object.assign(new Error('bounded transport failed'), {
        ...
        await expect(bridge.spawnOrAttach({ workspaceCwd: WS_A })).rejects.toThrow(/channel is unavailable/u);
        expect(handles).toHaveLength(1);
        const second = await bridge.spawnOrAttach({
            workspaceCwd: WS_A,
            sessionScope: 'thread',
        ...
        await vi.waitFor(() => expect(handles[1].agent.loadSessionCalls).toHaveLength(1));
        failures[1].resolve(Object.assign(new Error('bounded transport queue failed'), {
            code: 'ndjson_queue_limit_exceeded',
        }));
        await expect(restore).rejects.toBeInstanceOf(BridgeChannelClosedError);
        ...
        await vi.waitFor(() => {
            ...
            expect(event).toHaveBeenCalledWith('channel.exited', expect.objectContaining({
                'qwen-code.daemon.channel.transport_failed': true,
                'qwen-code.daemon.channel.transport_failure_initiated_teardown': true,
                'qwen-code.daemon.channel.transport_error_code': 'ndjson_frame_too_large',
            }));
        });
        bridge.killAllSync();
    });
    it('does not publish a channel whose transport fails during initialize', async () => {
        ...
        const bridge = makeBridge({ channelFactory: async () => handle.channel });
        await expect(bridge.spawnOrAttach({ workspaceCwd: WS_A })).rejects.toBeInstanceOf(BridgeChannelClosedError);
        expect(handle.killed).toBe(true);
        expect(handle.agent.newSessionCalls).toHaveLength(0);
    });
    it('keeps a planned teardown expected when the pipe fails secondarily', async () => {
        ...
        failure.resolve(Object.assign(new Error('EOF during planned termination'), {
    ...
    it('reaps an empty failed channel after overlapping thread spawns drain', async () => {
        const handles = [];
        const failures = [deferred(), deferred()];
        let failureIndex = 0;
        const factory = async () => {
    ...
    describe('sendPrompt', () => {
        ...
        it('rebuilds authoritative invocation metadata after client validation', async () => {
            ...
            expect(() => bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'rejected' }],
                _meta: { 'qwen-code/invocation': forgedInvocation },
            }, undefined, { clientId: 'invalid-client', promptId: 'rejected-prompt' })).toThrow(InvalidClientIdError);
            expect(handle.agent.promptCalls).toHaveLength(0);
            expect(() => bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'invalid internal prompt' }],
            }, undefined, { clientId: session.clientId, modelPrompt: '   ' })).toThrow('Bridge modelPrompt must be a non-empty bounded string');
            expect(handle.agent.promptCalls).toHaveLength(0);
            await bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
            ...
            expect(handle.agent.promptCalls[0]?._meta?.['qwen-code/private-parent-capability']).toBeUndefined();
            await bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'spoof-only' }],
                _meta: { [DAEMON_MODEL_PROMPT_META_KEY]: forgedModelPrompt },
            }, undefined, { clientId: session.clientId, promptId: 'spoof-only-prompt' });
            expect(handle.agent.promptCalls[1]?._meta?.[DAEMON_MODEL_PROMPT_META_KEY]).toBeUndefined();
            await bridge.shutdown();
        });
        ...
        it('strips channel display metadata from non-channel sessions', async () => {
            ...
            expect(handle.agent.promptCalls[0]?._meta?.['qwen.daemon.promptDisplayText']).toBeUndefined();
            await bridge.shutdown();
        });
        it('strips spoofed delivery metadata and injects only trusted context', async () => {
            ...
            expect(handle.agent.promptCalls[0]?._meta?.['qwen.daemon.channelDelivery']).toEqual({ deliveryId: 'prompt-1', target });
            await bridge.shutdown();
        });
        ...
        it('rejects continueSession with an unregistered client id before running the pre-check', async () => {
            ...
            await expect(bridge.continueSession(session.sessionId, {
        ...
        it('adds structured errorKind when a turn ends with terminated', async () => {
            ...
            }, undefined, { promptId: 'prompt-error' })).rejects.toThrow('terminated');
        ...
        it('echoes user_message_chunk to ALL session subscribers (cross-client sync)', async () => {
            ...
            const factory = async () => makeChannel({ promptImpl: () => ({ stopReason: 'end_turn' }) }).channel;
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            ...
            const firstUserChunk = async (iter) => {
            ...
            for (const chunk of [aChunk, bChunk]) {
                const update = chunk.data.update;
                ...
                expect(update._meta?.source).toBe('bridge-echo');
                expect(update._meta?.inputAnnotations).toEqual(inputAnnotations);
                expect(update._meta?.privateRequestId).toBeUndefined();
            }
            abortA.abort();
            abortB.abort();
            await bridge.shutdown();
        ...
        it('echoes only channel display text while forwarding full model context', async () => {
            ...
            }, undefined, { promptDisplayText: 'hello' });
        ...
        it('reserves an echo slot for channel display text after the block cap', async () => {
            ...
            }, undefined, { promptDisplayText: 'visible text' });
        ...
        it('ignores promptDisplayText for non-channel sessions', async () => {
            ...
            }, undefined, { promptDisplayText: 'hidden' });
            await vi.waitFor(() => {
            ...
            expect(handle.agent.promptCalls[0]?._meta).not.toHaveProperty('qwen.daemon.promptDisplayText');
        ...
        it('broadcasts prompt_cancelled with originator attribution on cancelSession', async () => {
            ...
            expect(evt.data.sessionId).toBe(session.sessionId);
        ...
        it('broadcasts prompt_cancelled to peers when the originator SSE aborts mid-prompt', async () => {
            ...
            }, promptAbort.signal, { clientId: session.clientId, promptId: 'prompt-cancelled' })
                .catch(() => {
        ...
        it('emits prompt_cancelled at most once when cancelSession races the SSE abort (D2)', async () => {
            ...
            const promptPromise = bridge
                ...
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'long running' }],
            }, promptAbort.signal, { clientId: session.clientId })
                .catch(() => { });
            await new Promise((r) => setTimeout(r, 10));
            // Both cancel routes fire for the same active prompt.
            await bridge.cancelSession(session.sessionId, { sessionId: session.sessionId }, { clientId: session.clientId });
            promptAbort.abort();
            releasePrompt?.();
            await promptPromise;
            await new Promise((r) => setTimeout(r, 10));
        ...
        it('resets the cancel-broadcast latch per prompt (a second prompt re-broadcasts)', async () => {
            ...
            const releasers = [];
            const factory = async () => makeChannel({
                promptImpl: async () => new Promise((res) => {
        ...
        it('emits a compensating prompt_cancelled{forward_failed} when the prompt forward rejects (C3)', async () => {
            ...
            const evt = await peerCancel;
            expect(evt.type).toBe('prompt_cancelled');
            expect(evt.data.reason).toBe('forward_failed');
        ...
        it('stamps envelope originatorClientId on session_closed', async () => {
            const factory = async () => makeChannel().channel;
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            const abort = new AbortController();
            const iter = bridge.subscribeEvents(session.sessionId, {
            ...
            expect(evt.data.closedBy).toBe(session.clientId);
            abort.abort();
            await bridge.shutdown();
        });
        it('stamps envelope originatorClientId on session_metadata_updated', async () => {
            ...
            const firstMeta = (async () => {
                for await (const e of iter) {
                    if (e.type === 'session_metadata_updated')
                        return e;
                }
                throw new Error('no session_metadata_updated observed');
            })();
            bridge.updateSessionMetadata(session.sessionId, { displayName: 'renamed session' }, { clientId: session.clientId });
            const evt = await firstMeta;
            expect(evt.originatorClientId).toBe(session.clientId);
            expect(evt.data.displayName).toBe('renamed session');
            abort.abort();
            await bridge.shutdown();
        });
        it('overrides a stale sessionId in the body with the routing id', async () => {
            ...
            expect(handles[0]?.agent.promptCalls[0]?.sessionId).toBe(session.sessionId);
            await bridge.shutdown();
        });
        it('signals prompt admission only after the pending queue owns the prompt', async () => {
            ...
            await expect(bridge.sendPrompt('missing-session', {
                sessionId: 'missing-session',
                prompt: [{ type: 'text', text: 'not admitted' }],
            }, undefined, { onPromptAdmitted: admitted })).rejects.toBeInstanceOf(SessionNotFoundError);
            expect(admitted).not.toHaveBeenCalled();
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            await bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'admitted' }],
            }, undefined, { onPromptAdmitted: admitted });
        ...
        it('rejects prompts past the default per-session pending cap synchronously', async () => {
            ...
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            const accepted = Array.from({ length: 5 }, (_, i) => bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
        ...
        it.each([[0], [Infinity]])('does not cap pending prompts when maxPendingPromptsPerSession is %s', async (maxPendingPromptsPerSession) => {
            let releaseFirst;
            const factory = async () => makeChannel({
                promptImpl: async (p) => {
                    const text = p.prompt[0]?.text ?? '';
            ...
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            const accepted = Array.from({ length: 6 }, (_, i) => bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: i === 0 ? 'hold' : `queued-${i}` }],
            }));
            await vi.waitFor(() => expect(releaseFirst).toBeDefined());
        ...
        it('does not count pre-aborted prompts against the pending cap', async () => {
            ...
            }, aborted.signal)).toThrow(/Prompt aborted/);
            await vi.waitFor(() => expect(releaseFirst).toBeDefined());
            releaseFirst();
            await active;
            await expect(bridge.sendPrompt(session.sessionId, {
        ...
        it('publishes session_branched only on the new session stream', async () => {
            ...
            const sourceIter = bridge
                .subscribeEvents(session.sessionId, { signal: sourceAbort.signal })[Symbol.asyncIterator]();
            ...
            const branchedIter = bridge
                .subscribeEvents(branch.sessionId, { lastEventId: 0 })[Symbol.asyncIterator]();
        ...
        it('throws SessionNotFoundError for unknown session ids', async () => {
            ...
            await expect(bridge.sendPrompt('unknown', {
                sessionId: 'unknown',
                prompt: [{ type: 'text', text: 'x' }],
            })).rejects.toBeInstanceOf(SessionNotFoundError);
        });
    ...
    describe('pendingPromptList', () => {
        it('tracks a single prompt without publishing a pending_prompt_added event', async () => {
            ...
            }, undefined, { promptId: 'prompt-single' });
            const addedEvents = events.filter((e) => e.type === 'pending_prompt_added');
            ...
            await vi.waitFor(() => {
                ...
                expect(completed?.data?.promptId).toBe('prompt-single');
            });
            expect(bridge.getPendingPrompts(session.sessionId)).toHaveLength(0);
            sub.catch(() => { });
            await bridge.shutdown();
        ...
        it('publishes pending_prompt_added only for queued prompts', async () => {
            ...
            }, undefined, { promptId: 'prompt-first' });
            const p2 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [
            ...
            }, undefined, { promptId: 'prompt-second', promptDisplayText: '' });
            await new Promise((r) => setTimeout(r, 20));
            const addedEvents = events.filter((e) => e.type === 'pending_prompt_added');
            expect(addedEvents).toHaveLength(1);
            expect(addedEvents[0]?.promptId).toBe('prompt-second');
            expect(addedEvents[0].data.text).toBe('[image]');
            ...
            await p2;
            await vi.waitFor(() => {
                expect(events.filter((e) => e.type === 'pending_prompt_started')).toHaveLength(1);
                expect(events.filter((e) => e.type === 'pending_prompt_completed' &&
                    e.data.state ===
                        'completed')).toHaveLength(1);
            });
            const startedEvents = events.filter((e) => e.type === 'pending_prompt_started');
            expect(startedEvents).toHaveLength(1);
            expect(startedEvents[0]?.promptId).toBe('prompt-second');
            expect(startedEvents[0].data
                .text).toBe('[image]');
            expect(handle.agent.promptCalls[1]?.prompt).toEqual([
                { type: 'image', data: 'aW1hZ2U=', mimeType: 'image/png' },
                { type: 'text', text: 'hidden channel instructions' },
            ]);
            const completedEvents = events.filter((e) => e.type === 'pending_prompt_completed');
        ...
        it('clears the Guard wait owner when a queued prompt is promoted', async () => {
            ...
            const first = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'first' }],
            }, undefined, { promptId: 'promotion-owner' });
            const second = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'second' }],
            }, undefined, { promptId: 'promoted-prompt' });
            await vi.waitFor(() => {
                expect(bridge.getPendingPrompts(session.sessionId)).toMatchObject([
            ...
            await expect(handle.agentConnection.extMethod(TODO_STOP_GUARD_CONTINUATION_CLAIM_METHOD, {
                sessionId: session.sessionId,
            ...
            const third = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'third' }],
            }, undefined, { promptId: 'removed-after-promotion' });
            await vi.waitFor(() => {
                expect(bridge.getPendingPrompts(session.sessionId)).toHaveLength(2);
            });
            expect(bridge.removePendingPrompt(session.sessionId, 'removed-after-promotion')).toEqual({ removed: true });
            expect(handle.agent.extMethodCalls.some((call) => call.method === TODO_STOP_GUARD_QUEUE_RELEASE_METHOD)).toBe(false);
            resolveSecond();
            await second;
        ...
        it('removePendingPrompt aborts a queued prompt and removes it from the list', async () => {
            ...
            }, undefined, { promptId: 'prompt-running' });
            const p2 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'to be removed' }],
            }, undefined, { promptId: 'prompt-removed' });
            await new Promise((r) => setTimeout(r, 20));
            const pending = bridge.getPendingPrompts(session.sessionId);
            expect(pending).toHaveLength(2);
            ...
            await expect(handle.agentConnection.extMethod(MID_TURN_QUEUE_DRAIN_METHOD, {
                sessionId: session.sessionId,
            ...
            try {
                ...
                await vi.waitFor(() => {
                    expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('Todo Stop Guard queued-prompt release failed'));
                });
            }
            finally {
                stderrSpy.mockRestore();
            }
            const again = bridge.removePendingPrompt(session.sessionId, 'nonexistent');
            ...
            await vi.waitFor(() => {
                const removed = events.find((event) => event.type === 'pending_prompt_completed' &&
        ...
        it('removePendingPrompt aborts a running prompt and triggers cancel', async () => {
            const events = [];
            let rejectPrompt;
            const promptPromise = new Promise((_resolve, reject) => {
                rejectPrompt = reject;
            });
            const handle = makeChannel({
            ...
            const completedEvents = events.filter((e) => e.type === 'pending_prompt_completed');
            const removedEvent = completedEvents.find((e) => e.data.promptId ===
                runningId &&
                e.data.state ===
                    'removed');
        ...
        it('keeps later prompts queued while a removed running prompt is still settling', async () => {
            ...
            const handle = makeChannel({
                promptImpl: async (req) => {
                    if (req.prompt[0].text === 'slow running prompt') {
                        await firstDone;
                    }
                    return { stopReason: 'end_turn' };
                },
            ...
            const runningId = bridge.getPendingPrompts(session.sessionId)[0]
                ?.promptId;
            expect(runningId).toBeDefined();
            expect(bridge.removePendingPrompt(session.sessionId, runningId)).toEqual({ removed: true });
            const p2 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'next prompt' }],
            });
        ...
        it('removePendingPrompt throws SessionNotFoundError for unknown sessions', () => {
            const bridge = makeBridge();
            expect(() => bridge.removePendingPrompt('unknown', 'some-id')).toThrow(SessionNotFoundError);
        });
        it('getPendingPrompts throws SessionNotFoundError for unknown sessions', () => {
            const bridge = makeBridge();
            expect(() => bridge.getPendingPrompts('unknown')).toThrow(SessionNotFoundError);
        });
    });
    describe('prompt terminal exactly-once (DAEMON-002/003/004/005)', () => {
        /** All formal turn terminals published for one promptId. */
        const terminalsFor = (events, promptId) => events.filter((e) => (e.type === 'turn_complete' || e.type === 'turn_error') &&
            e.promptId === promptId);
        ...
        const wedgeChannel = () => makeChannel({
            promptImpl: async (req) => {
                if (req.prompt[0].text === 'wedge') {
                    await new Promise(() => { });
                }
                return { stopReason: 'end_turn' };
            },
        });
        const subscribe = (bridge, sessionId, events) => {
            const sub = (async () => {
        ...
        it('publishes exactly one cancelled terminal when a queued prompt is removed (DAEMON-004)', async () => {
            ...
            const p1 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'blocker' }],
            }, undefined, { promptId: 'prompt-a' });
            const p2 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'queued victim' }],
            }, undefined, { promptId: 'prompt-b' });
            await new Promise((r) => setTimeout(r, 20));
            expect(bridge.removePendingPrompt(session.sessionId, 'prompt-b')).toEqual({ removed: true });
            ...
            await vi.waitFor(() => {
                ...
                expect((terms[0]?.data).stopReason).toBe('cancelled');
            });
            // The pending_prompt_completed{removed} bookkeeping event is unchanged.
            expect(events.some((e) => e.type === 'pending_prompt_completed' &&
                e.data.state === 'removed')).toBe(true);
            releaseFirst();
        ...
        it('publishes a deadline turn_error, unlocks the FIFO, and clears active state when the agent wedges (DAEMON-003)', async () => {
            ...
            }, undefined, { promptId: 'prompt-wedged', deadlineMs: 50 });
            ...
            await vi.waitFor(() => {
                ...
                expect((terms[0]?.data).code).toBe('prompt_deadline_exceeded');
            });
            ...
            expect(bridge.getSessionSummary(session.sessionId).hasActivePrompt).toBe(false);
            ...
            const followup = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'after the wedge' }],
            }, undefined, { promptId: 'prompt-after' });
        ...
        it('does not publish a stale deadline terminal for a prompt that completed in time (DAEMON-003)', async () => {
            ...
            }, undefined, { promptId: 'prompt-fast', deadlineMs: 60 });
        ...
        it('publishes a deadline terminal for a prompt that expires while still queued (DAEMON-003)', async () => {
            ...
            const p1 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'wedge' }],
            }, undefined, { promptId: 'prompt-head' });
            p1.catch(() => { });
            const p2 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'never dispatches' }],
            }, undefined, { promptId: 'prompt-queued-deadline', deadlineMs: 50 });
            p2.catch(() => { });
            await vi.waitFor(() => {
            ...
            expect(bridge.getSessionSummary(session.sessionId).turnError).toBeUndefined();
            ...
            expect((headTerms[0]?.data).code).toBe('daemon_shutdown');
        ...
        it('keeps a detached session draining until the last pending prompt settles (DAEMON-005)', async () => {
            ...
            await vi.waitFor(() => {
                expect(() => bridge.getSessionSummary(session.sessionId)).toThrow(SessionNotFoundError);
            });
            await bridge.shutdown();
        });
        it('flushes error terminals for active and queued prompts before session_closed (DAEMON-005)', async () => {
            ...
            for (const promptId of ['prompt-a', 'prompt-b']) {
                ...
                expect(terms).toHaveLength(1);
                expect(terms[0]?.type).toBe('turn_error');
                expect((terms[0]?.data).code).toBe('session_closed');
        ...
        it('still publishes a terminal for a removed RUNNING prompt when the session closes before the agent cooperates', async () => {
            ...
            const p1 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'wedge' }],
            }, undefined, { promptId: 'prompt-removed-running' });
            p1.catch(() => { });
            await new Promise((r) => setTimeout(r, 20));
            // Remove the RUNNING prompt — the wedged agent ignores the cancel,
            // so no terminal has been published yet.
            expect(bridge.removePendingPrompt(session.sessionId, 'prompt-removed-running')).toEqual({ removed: true });
            // The API no longer shows the prompt…
            expect(bridge.getPendingPrompts(session.sessionId)).toHaveLength(0);
            // …and a repeat removal is a no-op.
            expect(bridge.removePendingPrompt(session.sessionId, 'prompt-removed-running')).toEqual({ removed: false });
        ...
        it('does not publish a duplicate completed event when a promoted-then-removed running prompt settles', async () => {
            ...
            let releaseSecond;
            const secondDone = new Promise((r) => {
                releaseSecond = r;
            });
            const handle = makeChannel({
                promptImpl: async (req) => {
            ...
            const p1 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'blocker' }],
            }, undefined, { promptId: 'prompt-blocker' });
            const p2 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'queued then running' }],
            }, undefined, { promptId: 'prompt-promoted' });
            await new Promise((r) => setTimeout(r, 20));
            // Prompt 2 is queued behind prompt 1.
            expect(bridge
                .getPendingPrompts(session.sessionId)
                .find((p) => p.promptId === 'prompt-promoted')?.state).toBe('queued');
            ...
            expect(bridge
                .getPendingPrompts(session.sessionId)
                .find((p) => p.promptId === 'prompt-promoted')?.state).toBe('running');
            // Remove the now-running prompt 2.
            expect(bridge.removePendingPrompt(session.sessionId, 'prompt-promoted')).toEqual({ removed: true });
            ...
            const completedForPromoted = events.filter((e) => e.type === 'pending_prompt_completed' &&
                e.data.promptId ===
                    'prompt-promoted');
            expect(completedForPromoted).toHaveLength(1);
            expect(completedForPromoted[0]
                .data.state).toBe('removed');
        ...
        it('flushes error terminals for active and queued prompts before session_died on killSession (DAEMON-005)', async () => {
            ...
            for (const promptId of ['prompt-a', 'prompt-b']) {
                ...
                expect(terms).toHaveLength(1);
                expect(terms[0]?.type).toBe('turn_error');
                expect((terms[0]?.data).code).toBe('session_killed');
        ...
        it('publishes exactly one terminal per prompt under cancel + remove + deadline races (DAEMON-002)', async () => {
            ...
            const p1 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'wedge' }],
            }, undefined, { promptId: 'prompt-a', deadlineMs: 60 });
            p1.catch(() => { });
            const p2 = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'queued combatant' }],
            }, undefined, { promptId: 'prompt-b', deadlineMs: 60 });
            p2.catch(() => { });
            await new Promise((r) => setTimeout(r, 20));
    ...
    describe('cancelSession', () => {
        it('cancels an admitted prompt before agent dispatch', async () => {
            ...
            }, undefined, { promptId: 'prompt-admitted' });
            await bridge.cancelSession(session.sessionId);
            await expect(prompt).rejects.toMatchObject({ name: 'AbortError' });
            expect(handle.agent.promptCalls).toHaveLength(0);
            expect(handle.agent.cancelCalls).toHaveLength(0);
            await vi.waitFor(() => {
                const terminals = events.filter((event) => (event.type === 'turn_complete' || event.type === 'turn_error') &&
        ...
        it('keeps cancelling while the child admits the prompt', async () => {
            ...
            }, undefined, { promptId: 'prompt-child-admission' });
        ...
        it('drains a late cancel retry before dispatching the queued successor', async () => {
            ...
            const second = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'second' }],
            }, undefined, { promptId: 'prompt-second' });
        ...
        it('falls back once to standard ACP cancellation for custom agents', async () => {
            ...
            }, undefined, { promptId: 'prompt-standard-cancel' });
            await promptEntered.promise;
            await bridge.cancelSession(session.sessionId);
        ...
        it('broadcasts immediate cancellation once for each prompt', async () => {
            ...
            await vi.waitFor(() => {
                expect(events
                    ...
                    .map((event) => event.promptId)).toEqual(['prompt-immediate-1', 'prompt-immediate-2']);
            });
            expect(handle.agent.promptCalls).toHaveLength(0);
            await bridge.shutdown();
        });
        it('forwards a cancel notification with the routing id', async () => {
            ...
            expect(handles[0]?.agent.cancelCalls[0]?.sessionId).toBe(session.sessionId);
            await bridge.shutdown();
        });
        it('throws SessionNotFoundError for unknown session ids', async () => {
            const bridge = makeBridge({
            ...
            await expect(bridge.cancelSession('unknown')).rejects.toBeInstanceOf(SessionNotFoundError);
        });
        it('treats idle agent cancel as success', async () => {
            ...
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            await expect(bridge.cancelSession(session.sessionId)).resolves.toBeUndefined();
            expect(handles[0]?.agent.cancelCalls).toHaveLength(1);
            await bridge.shutdown();
        });
    ...
    describe('permission flow', () => {
        ...
        it('publishes a permission_request event with a generated requestId and awaits a vote', async () => {
            ...
            const respPromise = conn.requestPermission({
                sessionId: session.sessionId,
            ...
            expect(bridge.listWorkspaceSessions(WS_A)[0].pendingInteractions).toEqual([]);
            subAbort.abort();
            await bridge.shutdown();
        });
        it('forwards permission vote metadata back to the agent response', async () => {
            ...
            const accepted = bridge.respondToPermission(payload.requestId, responseWithAnswers);
            expect(accepted).toBe(true);
            const response = await respPromise;
        ...
        it('forwards session-scoped permission answers without arbitrary metadata', async () => {
            ...
            const accepted = bridge.respondToSessionPermission(session.sessionId, payload.requestId, responseWithAnswers, { clientId: session.clientId });
            expect(accepted).toBe(true);
            const response = await respPromise;
            expect(response).toMatchObject({
                outcome: { outcome: 'selected', optionId: 'proceed_once' },
        ...
        it('returns false (not InvalidClientIdError) when session exists but requestId is unknown and clientId is unregistered (#4335 / 3271978329 / 3272493792 / 3273077272)', async () => {
            ...
            try {
                ...
                const result = bridge.respondToSessionPermission(session.sessionId, 'unknown-req-id', { outcome: { outcome: 'cancelled' } }, { clientId: 'fabricated-client-id' });
                expect(result).toBe(false);
                expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('rejected permission vote'));
                expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('unknown-req-id'));
                await bridge.shutdown();
            }
            finally {
                stderrSpy.mockRestore();
        ...
        it('rejects cancel sentinel injection via {selected,"__cancelled__"} (#4335 / 3271420267)', async () => {
            ...
            // mediator.
            expect(() => bridge.respondToSessionPermission(session.sessionId, payload.requestId, {
        ...
        it('rejects votes whose optionId was not in the agent-offered set (BkwQI)', async () => {
            ...
            expect(() => bridge.respondToPermission(payload.requestId, {
        ...
        it('publishes a permission_resolved event when a vote lands', async () => {
            ...
            const requestId = reqEvt.data.requestId;
        ...
        it('publishes permission_already_resolved when a scoped vote loses the race', async () => {
            ...
            const reqEvt = (await it.next()).value;
            const requestId = reqEvt.data.requestId;
            const accepted = bridge.respondToSessionPermission(session.sessionId, requestId, {
                outcome: { outcome: 'selected', optionId: 'allow' },
            }, { clientId: session.clientId });
            expect(accepted).toBe(true);
            const resolvedEvt = (await it.next()).value;
            expect(resolvedEvt.type).toBe('permission_resolved');
            const second = bridge.respondToSessionPermission(session.sessionId, requestId, { outcome: { outcome: 'cancelled' } }, { clientId: session.clientId });
            expect(second).toBe(false);
        ...
        it('session-scoped permission votes cannot resolve another session request', async () => {
            ...
            const accepted = bridge.respondToSessionPermission(wrongSession.sessionId, requestId, { outcome: { outcome: 'selected', optionId: 'allow' } }, { clientId: wrongSession.clientId });
            expect(accepted).toBe(false);
            expect(bridge.pendingPermissionCount).toBe(1);
            expect(bridge.respondToSessionPermission(wrongSession.sessionId, requestId, { outcome: { outcome: 'cancelled' } }, { clientId: 'client-not-issued' })).toBe(false);
            expect(bridge.pendingPermissionCount).toBe(1);
            bridge.respondToPermission(requestId, {
                outcome: { outcome: 'cancelled' },
            });
        ...
        it('session-scoped duplicate votes do not validate clients against another session', async () => {
            ...
            const reqEvt = (await it.next()).value;
            const requestId = reqEvt.data.requestId;
            expect(bridge.respondToSessionPermission(session.sessionId, requestId, {
                outcome: { outcome: 'selected', optionId: 'allow' },
            }, { clientId: session.clientId })).toBe(true);
            const wrongSession = await bridge.spawnOrAttach({
                workspaceCwd: WS_A,
                sessionScope: 'thread',
            });
        ...
        it('respondToSessionPermission throws SessionNotFoundError for unknown sessions', async () => {
            ...
            expect(() => bridge.respondToSessionPermission('missing-session', 'req-1', {
                outcome: { outcome: 'cancelled' },
        ...
        it('rejects scoped votes whose optionId was not in the agent-offered set', async () => {
            ...
            }, { clientId: session.clientId })).toThrow(InvalidPermissionOptionError);
            expect(bridge.pendingPermissionCount).toBe(1);
            bridge.respondToSessionPermission(session.sessionId, payload.requestId, {
                outcome: { outcome: 'selected', optionId: 'allow' },
            }, { clientId: session.clientId });
        ...
        it('rejects permission votes with unregistered client ids', async () => {
            ...
            const reqEvt = (await it.next()).value;
            const requestId = reqEvt.data.requestId;
            expect(() => bridge.respondToPermission(requestId, {
                outcome: { outcome: 'selected', optionId: 'allow' },
            }, { clientId: 'client-not-issued' })).toThrow(InvalidClientIdError);
            subAbort.abort();
            await bridge.shutdown();
        });
        ...
        it('returns false uniformly for unknown permission votes regardless of clientId registration (#4335 / 3272493777)', async () => {
            ...
            expect(bridge.respondToPermission('does-not-exist', {
                outcome: { outcome: 'cancelled' },
            }, { clientId: 'client-not-issued' })).toBe(false);
            // Registered clientId — also `false`.
            expect(bridge.respondToPermission('does-not-exist', {
                outcome: { outcome: 'cancelled' },
            }, { clientId: session.clientId })).toBe(false);
    ...
    describe('modelServiceId honored at session create', () => {
        ...
        function setup(opts = {}) {
            ...
            const factory = async () => {
                ...
                new AgentSideConnection(() => augmented, agentStream);
                return {
                    stream: clientStream,
                    exited: new Promise(() => { }),
                    kill: async () => { },
    ...
    describe('model-change FIFO + failure recovery', () => {
        it('publishes model_switch_failed and surfaces the error when the agent rejects', async () => {
            ...
            expect(failed.value?.data?.requestedModelId).toBe('rejected');
            abort.abort();
            await bridge.shutdown();
        });
        ...
        it('serializes concurrent model-change calls (FIFO)', async () => {
            ...
            // one enters.
            const noEnter = callOrder.findIndex((s, i) => s.startsWith('enter:') &&
                i > 0 &&
    ...
    describe('sendPrompt fail-fast on transport close', () => {
        ...
        it('does not promote mid-turn messages after channel exit removes the session', async () => {
            const handle = makeChannel({
            ...
            try {
                ...
                await vi.waitFor(() => expect(handle.agent.promptCalls).toHaveLength(1));
                expect(bridge.enqueueMidTurnMessage(session.sessionId, 'must not target a dead entry')).toMatchObject({ accepted: true });
                handle.crash({ exitCode: 1, signalCode: null });
                await expect(prompt).rejects.toThrow(/channel closed/i);
                await vi.waitFor(() => expect(bridge.sessionCount).toBe(0));
                await Promise.resolve();
                expect(stderr.mock.calls
                    .map(([line]) => String(line))
                    .join('')
                    .includes('failed to run promoted message')).toBe(false);
            }
            finally {
                await bridge.shutdown();
                stderr.mockRestore();
    ...
    describe('opts validation', () => {
        ...
        it('rejects a non-positive initializeTimeoutMs', () => {
            expect(() => makeBridge({ initializeTimeoutMs: 0 })).toThrow(/initializeTimeoutMs/);
            expect(() => makeBridge({ initializeTimeoutMs: -1 })).toThrow(/initializeTimeoutMs/);
        });
        it('rejects NaN maxSessions (BRApy: silent fail-OPEN guard)', () => {
            ...
            expect(() => makeBridge({ maxSessions: NaN })).toThrow(/maxSessions: NaN/);
        ...
        it.each([
            ...
            expect(() => makeBridge({ maxPendingPromptsPerSession: value })).toThrow(/maxPendingPromptsPerSession/);
        });
        it('accepts disabled maxPendingPromptsPerSession sentinels', () => {
            expect(() => makeBridge({ maxPendingPromptsPerSession: 0 })).not.toThrow();
            expect(() => makeBridge({ maxPendingPromptsPerSession: Infinity })).not.toThrow();
        });
    });
    describe('concurrent spawn coalescing (single scope)', () => {
        ...
        it('clears the in-flight slot on rejection so the next call can retry', async () => {
            ...
            const bridge = makeBridge({ channelFactory: factory });
            await expect(bridge.spawnOrAttach({ workspaceCwd: WS_A })).rejects.toBeTruthy();
    ...
    describe('BridgeClient file proxy (Stage 1: same-host trust)', () => {
        ...
        async function setupForFs() {
            ...
            const factory = async () => {
                const { clientStream, agentStream } = createInMemoryChannel();
                capturedConn = new AgentSideConnection(() => new FakeAgent(), agentStream);
                return {
                    stream: clientStream,
                    exited: new Promise(() => { }),
                    kill: async () => { },
        ...
        it('writeTextFile writes to local fs', async () => {
            ...
            const tmp = path.join(os.tmpdir(), `qwen-bridge-write-${randomBytes(8).toString('hex')}.txt`);
            try {
        ...
        it('writeTextFile leaves no .tmp turd in the target directory (BSA0D)', async () => {
            ...
            const { bridge, conn } = await setupForFs();
            const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-bridge-atomic-'));
        ...
        it('readTextFile rejects files past the size cap (BSA0E)', async () => {
            ...
            const { bridge, conn } = await setupForFs();
            const sparse = path.join(os.tmpdir(), `qwen-bridge-sparse-${randomBytes(8).toString('hex')}.bin`);
            ...
            try {
                ...
                await expect(conn.readTextFile({ sessionId: 'unused', path: sparse })).rejects.toMatchObject({
                    data: {
        ...
        it('readTextFile rejects non-regular files even when size=0 (BX8YO)', async () => {
            ...
            if (process.platform === 'win32')
                return;
            const { bridge, conn } = await setupForFs();
            const fifoPath = path.join(os.tmpdir(), `qwen-bridge-fifo-${randomBytes(8).toString('hex')}`);
            ...
            try {
                await expect(conn.readTextFile({ sessionId: 'unused', path: fifoPath })).rejects.toMatchObject({
        ...
        it('writeTextFile preserves symlinks (BX8Yw)', async () => {
            ...
            const { bridge, conn } = await setupForFs();
            const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-bridge-symlink-'));
            ...
            try {
                ...
                expect(await fsp.readFile(target, 'utf8')).toBe('updated through symlink');
                ...
                expect(await fsp.readFile(link, 'utf8')).toBe('updated through symlink');
            }
            finally {
                await fsp.rm(dir, { recursive: true, force: true });
                await bridge.shutdown();
        ...
        it('writeTextFile preserves dangling symlinks (BfFvO)', async () => {
            ...
            const { bridge, conn } = await setupForFs();
            const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-bridge-dangling-'));
            ...
            try {
                ...
                expect(await fsp.readFile(target, 'utf8')).toBe('created through dangling symlink');
        ...
        it('readTextFile returns full content by default', async () => {
            const { bridge, conn } = await setupForFs();
            const tmp = path.join(os.tmpdir(), `qwen-bridge-read-${randomBytes(8).toString('hex')}.txt`);
            await fsp.writeFile(tmp, 'line one\nline two\nline three\nline four', 'utf8');
            try {
                const result = (await conn.readTextFile({ sessionId: 'unused', path: tmp }));
        ...
        it('readTextFile slices via line/limit (ACP 1-based line)', async () => {
            const { bridge, conn } = await setupForFs();
            const tmp = path.join(os.tmpdir(), `qwen-bridge-slice-${randomBytes(8).toString('hex')}.txt`);
            await fsp.writeFile(tmp, 'a\nb\nc\nd\ne', 'utf8');
            try {
                // line:1, limit:2 means "first two lines" per ACP spec (1-based).
                const first = (await conn.readTextFile({
                    sessionId: 'unused',
                    path: tmp,
                    line: 1,
                    limit: 2,
                ...
                const middle = (await conn.readTextFile({
                    sessionId: 'unused',
                    path: tmp,
                    line: 3,
                    limit: 2,
    ...
    describe('listWorkspaceSessions', () => {
        it('returns sessions matching the bound workspace cwd', async () => {
            ...
            expect(aList.map((s) => s.sessionId).sort()).toEqual([a1.sessionId, a2.sessionId].sort());
    ...
    describe('getSessionSummary', () => {
        ...
        it('throws SessionNotFoundError for an unknown session id', async () => {
            ...
            expect(() => bridge.getSessionSummary('missing')).toThrow(SessionNotFoundError);
            await bridge.shutdown();
        });
    });
    describe('session source metadata', () => {
        it('persists and returns source metadata in status and list summaries', async () => {
            ...
            const bridge = makeBridge({
                channelFactory: async () => {
                    const handle = makeChannel({
                        extMethodImpl: async (method) => method === SERVE_CONTROL_EXT_METHODS.sessionSource
                            ? { persisted: true }
                            : {},
                    });
                    handles.push(handle);
        ...
        it('creates a side task while the parent prompt is active', async () => {
            ...
            await vi.waitFor(() => expect(handle.agent.promptCalls).toHaveLength(1));
            await expect(bridge.createSideTaskSession(parent.sessionId, { name: 'Side task' })).resolves.toMatchObject({
        ...
        it('passes source metadata in the resume request without a second control call', async () => {
            ...
            expect(handle.agent.extMethodCalls).not.toContainEqual(expect.objectContaining({
                method: SERVE_CONTROL_EXT_METHODS.sessionSource,
            }));
    ...
    describe('Live transcript persistence', () => {
        it('forwards direct dialogue to the owning ACP child', async () => {
            ...
            await bridge.appendSessionLiveTranscript(session.sessionId, entries, 'qwen3.5-omni-plus-realtime');
            expect(handle.agent.extMethodCalls).toContainEqual({
    ...
    describe('parentSessionId', () => {
        ...
        it('leaves parentSessionId undefined when the spawn omits it', async () => {
            ...
            expect(bridge.getSessionSummary(session.sessionId).parentSessionId).toBeUndefined();
            const fromList = bridge
                .listWorkspaceSessions(WS_A)
                .find((s) => s.sessionId === session.sessionId);
        ...
        it('sends the sessionParent ext-method to the child when spawned with a parent', async () => {
            ...
            await vi.waitFor(() => {
                expect(handles[0]?.agent.extMethodCalls.find((c) => c.method === SERVE_CONTROL_EXT_METHODS.sessionParent)).toEqual({
        ...
        it('does not send the sessionParent ext-method when spawned without a parent', async () => {
            ...
            expect(handles[0]?.agent.extMethodCalls.some((c) => c.method === SERVE_CONTROL_EXT_METHODS.sessionParent)).toBe(false);
            await bridge.shutdown();
        });
        it('still succeeds when the child reports the parent link was not persisted', async () => {
            ...
            const bridge = makeBridge({
                channelFactory: async () => {
                    const h = makeChannel({
                        ...
                        extMethodImpl: (method) => method === SERVE_CONTROL_EXT_METHODS.sessionParent
                            ? { persisted: false }
                            : {},
                    });
                    handles.push(h);
            ...
            expect(handles[0]?.agent.extMethodCalls.some((c) => c.method === SERVE_CONTROL_EXT_METHODS.sessionParent)).toBe(true);
            await bridge.shutdown();
        });
        ...
        it('reports parentSessionPersisted:false and does NOT retry a definitive persisted:false', async () => {
            ...
            expect(handles[0]?.agent.extMethodCalls.filter((c) => c.method === SERVE_CONTROL_EXT_METHODS.sessionParent)).toHaveLength(1);
        ...
        it('retries a throwing sessionParent (MAX_PARENT_PERSIST_ATTEMPTS) then reports false', async () => {
            ...
            expect(handles[0]?.agent.extMethodCalls.filter((c) => c.method === SERVE_CONTROL_EXT_METHODS.sessionParent)).toHaveLength(3);
                ...
                channelFactory: async () => {
                    const h = makeChannel({
                        ...
                        extMethodImpl: (method, _params, self) => {
                            if (method !== SERVE_CONTROL_EXT_METHODS.sessionParent)
                                return {};
                            const calls = self.extMethodCalls.filter((c) => c.method === SERVE_CONTROL_EXT_METHODS.sessionParent).length;
                            if (calls < 3)
    ...
    describe('setSessionModel', () => {
        ...
        it('rejects unregistered client ids on session-scoped requests', async () => {
            ...
            expect(() => bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'hi' }],
            }, undefined, { clientId: 'client-not-issued' })).toThrow(InvalidClientIdError);
            expect(bridge.activePromptCount).toBe(0);
            await expect(bridge.cancelSession(session.sessionId, undefined, {
                clientId: 'client-not-issued',
            })).rejects.toBeInstanceOf(InvalidClientIdError);
            await expect(bridge.setSessionModel(session.sessionId, {
                sessionId: session.sessionId,
                modelId: 'qwen3-coder',
            }, { clientId: 'client-not-issued' })).rejects.toBeInstanceOf(InvalidClientIdError);
            await bridge.shutdown();
        });
        it('throws SessionNotFoundError for unknown session ids', async () => {
            const bridge = makeBridge({
            ...
            await expect(bridge.setSessionModel('unknown', {
                sessionId: 'unknown',
                modelId: 'qwen3-coder',
            })).rejects.toBeInstanceOf(SessionNotFoundError);
        });
    ...
    describe('executeShellCommand permission policy', () => {
        ...
        it('rejects direct shell by default before executing the command', async () => {
            ...
            await expect(disabledBridge.executeShellCommand(session.sessionId, 'echo hi')).rejects.toBeInstanceOf(SessionShellDisabledError);
        ...
        it('requires a client id before checking whether the session exists', async () => {
            ...
            await expect(bridge.executeShellCommand('unknown-session', 'echo hi')).rejects.toBeInstanceOf(SessionShellClientRequiredError);
            expect(shellSpy).not.toHaveBeenCalled();
            await bridge.shutdown();
            shellSpy.mockRestore();
        });
        it('rejects unregistered client ids when direct shell is enabled', async () => {
            const shellSpy = mockShellExecute();
            const { bridge, session } = await setupShellSession();
            await expect(bridge.executeShellCommand(session.sessionId, 'echo hi', undefined, {
                clientId: 'client-not-issued',
            })).rejects.toBeInstanceOf(InvalidClientIdError);
            expect(shellSpy).not.toHaveBeenCalled();
            await bridge.shutdown();
        ...
        it('executes and stamps events when the client id belongs to the session', async () => {
            ...
            const result = await bridge.executeShellCommand(session.sessionId, 'echo hello', undefined, { clientId: session.clientId });
            expect(result).toEqual({
                exitCode: 0,
                output: 'hello\n',
                aborted: false,
            ...
            expect(shellSpy).toHaveBeenCalledWith('echo hello', WS_A, expect.any(Function), expect.any(AbortSignal), false, { terminalWidth: 120, terminalHeight: 40 }, { streamStdout: true });
        ...
        it('does not hang a completed shell on transport-failed history injection', async () => {
            ...
            const handle = makeChannel({
                extMethodImpl: (method) => method === SERVE_CONTROL_EXT_METHODS.sessionShellHistory
                    ? new Promise(() => { })
                    : {},
            });
            handle.channel = {
            ...
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            const shell = bridge.executeShellCommand(session.sessionId, 'echo done', undefined, { clientId: session.clientId });
            await vi.waitFor(() => expect(handle.agent.extMethodCalls).toContainEqual(expect.objectContaining({
                method: SERVE_CONTROL_EXT_METHODS.sessionShellHistory,
            })));
            failure.resolve(Object.assign(new Error('transport frame failed'), {
                code: 'ndjson_frame_too_large',
            }));
        ...
        it('executes direct shell in each session effective cwd', async () => {
            ...
            await Promise.all([
                bridge.executeShellCommand(firstSession.sessionId, 'echo first', undefined, { clientId: firstSession.clientId }),
                bridge.executeShellCommand(secondSession.sessionId, 'echo second', undefined, { clientId: secondSession.clientId }),
            ]);
            expect(shellSpy).toHaveBeenCalledTimes(2);
            expect(shellSpy.mock.calls.map(([command, cwd]) => [command, cwd])).toEqual(expect.arrayContaining([
                ['echo first', WS_A],
                ['echo second', WS_B],
            ]));
            await bridge.shutdown();
        ...
        it('waits for a pending cwd change before executing direct shell', async () => {
            ...
            await vi.waitFor(() => expect(handle.agent.extMethodCalls).toContainEqual({
                method: SERVE_CONTROL_EXT_METHODS.sessionCd,
                params: {
                    sessionId: session.sessionId,
                    path: WS_B,
            ...
            const shell = bridge.executeShellCommand(session.sessionId, 'echo after-cd', undefined, { clientId: session.clientId });
            await Promise.resolve();
            expect(shellSpy).not.toHaveBeenCalled();
        ...
        it('executes direct shell in previous cwd when a pending cd fails', async () => {
            ...
            const shell = bridge.executeShellCommand(session.sessionId, 'echo after-failed-cd', undefined, { clientId: session.clientId });
            await Promise.resolve();
            expect(shellSpy).not.toHaveBeenCalled();
        ...
        it('returns an aborted direct shell without waiting for a hung cwd change', async () => {
            ...
            const abort = new AbortController();
            const shell = bridge.executeShellCommand(session.sessionId, 'echo aborted-cd', abort.signal, { clientId: session.clientId });
            await Promise.resolve();
            expect(shellSpy).not.toHaveBeenCalled();
            abort.abort();
    ...
    describe('setSessionApprovalMode (#4175 Wave 4 PR 17)', () => {
        ...
        function rejectingApprovalModeFactory() {
            return async () => {
                const { clientStream, agentStream } = createInMemoryChannel();
                const agent = new FakeAgent({
                    extMethodImpl: (method) => {
        ...
        function deferredApprovalModeFactory() {
            ...
            return {
                ...
                rejectApprovalMode: (error = new Error('trust gate rejected')) => {
                    ...
                    rejectApprovalMode(Object.assign(error, { data: { errorKind: 'trust_gate' } }));
                },
            };
        }
        ...
        it('does not publish a failing approval-mode spawn as the default session', async () => {
            const { factory, waitForApprovalMode, rejectApprovalMode } = deferredApprovalModeFactory();
            const bridge = makeBridge({
                channelFactory: factory,
                sessionScope: 'single',
            });
        ...
        it('restores approval mode when an attach loses a close race', async () => {
            ...
            expect(bridge.getDaemonStatusSnapshot().sessions[0]?.currentApprovalMode).toBe(ApprovalMode.DEFAULT);
            closeResult.resolve({
                closed: false,
        ...
        it('throws BEFORE the ACP roundtrip when persist:true but no callback wired', async () => {
            ...
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            await expect(bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.YOLO, { persist: true }, undefined)).rejects.toThrow(/persistApprovalMode/);
            expect(getCalls().some((c) => c.method === 'qwen/control/session/approval_mode')).toBe(false);
            await bridge.shutdown();
        });
        it('persist:false bypasses the guard regardless of callback wiring', async () => {
            ...
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            const res = await bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.YOLO, { persist: false }, undefined);
        ...
        it('serializes concurrent approval-mode changes through the per-session queue (A3)', async () => {
            ...
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            await Promise.all([
                bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.YOLO, { persist: false }, undefined),
                bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.DEFAULT, { persist: false }, undefined),
            ]);
        ...
        it('serializes persist + publish too, not just the extMethod (A3, persist:true)', async () => {
            ...
            const persistDelay = { yolo: 30, default: 1 };
            const bridge = makeBridge({
                channelFactory: factory,
                persistApprovalMode: async (_ws, mode) => {
            ...
            const collecting = (async () => {
                for await (const e of iter) {
                    if (e.type === 'approval_mode_changed') {
                        published.push(e.data.next);
                    }
                }
            })();
            await Promise.all([
                bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.YOLO, { persist: true }, undefined),
                bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.DEFAULT, { persist: true }, undefined),
            ]);
            await new Promise((r) => setTimeout(r, 20));
            abort.abort();
            await collecting;
        ...
        it('a failed approval-mode change does not poison the queue (A3 tail-swallow)', async () => {
            ...
            await expect(bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.YOLO, { persist: false }, undefined)).rejects.toThrow();
            // Queue not poisoned — the next change still resolves.
            const res = await bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.DEFAULT, { persist: false }, undefined);
        ...
        it('echoPromptToSessionBus tolerates a non-array prompt (D6 guard)', async () => {
            ...
            const caught = await bridge
                .sendPrompt(session.sessionId, { sessionId: session.sessionId, prompt: undefined }, undefined, { clientId: session.clientId })
                .catch((e) => e);
            await new Promise((r) => setTimeout(r, 10));
            abort.abort();
            await collecting;
        ...
        it('broadcasts approval_mode_changed to peer sessions when persisted (#4282 fold-in 4 S2)', async () => {
            ...
            const itA = bridge
                .subscribeEvents(a.sessionId, { signal: aborts[0].signal })[Symbol.asyncIterator]();
            const itB = bridge
                .subscribeEvents(b.sessionId, { signal: aborts[1].signal })[Symbol.asyncIterator]();
            await bridge.setSessionApprovalMode(a.sessionId, ApprovalMode.YOLO, { persist: true }, undefined);
        ...
        it('does NOT broadcast to peers when persisted is false', async () => {
            ...
            const itB = bridge
                .subscribeEvents(b.sessionId, { signal: aborts[1].signal })[Symbol.asyncIterator]();
            await bridge.setSessionApprovalMode(a.sessionId, ApprovalMode.YOLO, { persist: false }, undefined);
            const aFirst = await itA.next();
            expect(aFirst.value?.type).toBe('approval_mode_changed');
            // Race the peer subscriber against a 50ms timer. Without a
            // timeout the test would hang because no event is expected.
            const timed = await Promise.race([
                itB.next().then((v) => ({ kind: 'event', v })),
                new Promise((r) => setTimeout(r, 50)).then(() => ({
                    kind: 'timeout',
                })),
            ]);
    ...
    describe('generateSessionRecap (#4175 follow-up)', () => {
        ...
        it('forwards through the ACP child and returns the recap verbatim', async () => {
            const recapText = 'Refactoring the auth middleware. Next: regenerate the integration fixtures.';
            let observedParams;
            const bridge = makeBridge({
        ...
        it('throws SessionNotFoundError for unknown sessionId', async () => {
            ...
            await expect(bridge.generateSessionRecap('does-not-exist')).rejects.toBeInstanceOf(SessionNotFoundError);
            await bridge.shutdown();
        });
    });
    describe('addRuntimeMcpServer (T2.8 #4514)', () => {
         ...
         */
        function runtimeAddFactory(respond) {
            return async () => {
                const { clientStream, agentStream } = createInMemoryChannel();
                const agent = new FakeAgent({
                    extMethodImpl: (method, params) => {
        ...
        it('returns the success shape and broadcasts mcp_server_added', async () => {
            ...
            const abort = new AbortController();
            const it = bridge
                .subscribeEvents(session.sessionId, { signal: abort.signal })[Symbol.asyncIterator]();
            const result = await bridge.addRuntimeMcpServer('test-server', { command: 'node', args: ['server.js'] }, 'client-1');
            expect(result).toEqual({
                name: 'test-server',
                transport: 'stdio',
                replaced: false,
        ...
        it('throws with errorKind acp_channel_unavailable when no ACP channel is live', async () => {
            ...
            const bridge = makeBridge({});
            const err = await bridge
                .addRuntimeMcpServer('test-server', { command: 'node', args: ['server.js'] }, 'client-1')
                .catch((e) => e);
            expect(err).toBeInstanceOf(Error);
            expect(err.data?.errorKind).toBe('acp_channel_unavailable');
            await bridge.shutdown();
        });
        it('stamps mcp_server_added with the originator clientId', async () => {
            ...
            const it = bridge
                .subscribeEvents(session.sessionId, { signal: abort.signal })[Symbol.asyncIterator]();
            await bridge.addRuntimeMcpServer('my-mcp', { url: 'http://localhost:3000/sse' }, session.clientId);
            const next = await it.next();
            expect(next.value?.originatorClientId).toBe(session.clientId);
            abort.abort();
            await bridge.shutdown();
    ...
    describe('session-scoped runtime MCP servers', () => {
        it('routes add and remove to the selected live session only', async () => {
            ...
            await bridge.addSessionRuntimeMcpServer(target.sessionId, 'channel-loop', { type: 'sdk' }, 'daemon-channel');
            await bridge.removeSessionRuntimeMcpServer(target.sessionId, 'channel-loop', 'daemon-channel');
        ...
        it('rejects an unknown session without falling back to a live channel', async () => {
            ...
            await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            await expect(bridge.addSessionRuntimeMcpServer('missing-session', 'channel-loop', {
                type: 'sdk',
            })).rejects.toBeInstanceOf(SessionNotFoundError);
            expect(extMethodImpl).not.toHaveBeenCalledWith(SERVE_CONTROL_EXT_METHODS.sessionMcpRuntimeAdd, expect.anything());
            await bridge.shutdown();
        });
        it('allows session MCP mutations to outlive the initialize timeout', async () => {
            ...
            vi.useFakeTimers();
            try {
                const addResult = bridge.addSessionRuntimeMcpServer(target.sessionId, 'channel-loop', {
                    type: 'sdk',
                });
                ...
                const removeResult = bridge.removeSessionRuntimeMcpServer(target.sessionId, 'channel-loop');
                await vi.advanceTimersByTimeAsync(6);
                remove.resolve({
                    name: 'channel-loop',
                    removed: true,
    ...
    describe('removeRuntimeMcpServer (T2.8 #4514)', () => {
         ...
         */
        function runtimeRemoveFactory(respond) {
            return async () => {
                const { clientStream, agentStream } = createInMemoryChannel();
                const agent = new FakeAgent({
                    extMethodImpl: (method, params) => {
        ...
        it('returns the removed shape and broadcasts mcp_server_removed', async () => {
            ...
            const it = bridge
                .subscribeEvents(session.sessionId, { signal: abort.signal })[Symbol.asyncIterator]();
            const result = await bridge.removeRuntimeMcpServer('test-server', 'client-2');
            expect(result).toEqual({
                name: 'test-server',
                removed: true,
                wasShadowingSettings: false,
    ...
    describe('subscribeEvents', () => {
        it('throws SessionNotFoundError for unknown session ids', () => {
            ...
            expect(() => bridge.subscribeEvents('unknown')).toThrow(SessionNotFoundError);
        });
        ...
        it('splits a2ui tool updates and publishes a sanitized original frame', async () => {
            ...
            const text = '[{"version":"v0.9","createSurface":{"surfaceId":"s1","components":[]}},{"version":"v0.9","updateComponents":{"surfaceId":"s2","components":[]}}]\nfallback summary';
            void capturedConn.sessionUpdate({
                sessionId: session.sessionId,
                update: {
                    sessionUpdate: 'tool_call_update',
    ...
    describe('extNotification — MCP budget events (PR 14b)', () => {
        ...
        it('stamps terminal_sequence with the active promptId', async () => {
            ...
            const terminalEvent = (async () => {
                for await (const event of iter) {
                    if (event.type === 'terminal_sequence')
                        return event;
                }
                throw new Error('terminal_sequence event not observed');
            })();
            const promptDone = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'keep the prompt active' }],
            }, undefined, { clientId: session.clientId, promptId: 'prompt-terminal' });
            await vi.waitFor(() => {
        ...
        it('drops unknown extNotification methods, kinds, and missing sessionIds silently', async () => {
            ...
            void capturedConn.extNotification('qwen/notify/session/mcp-budget-event', { kind: 'budget_warning' });
            // Unknown kind — drop.
            void capturedConn.extNotification('qwen/notify/session/mcp-budget-event', { sessionId: session.sessionId, kind: 'mystery_kind' });
        ...
        it('purges buffered guardrail events when restore fails so retry-success does not replay stale frames (codex round 7 fix)', async () => {
            ...
            await expect(bridge.loadSession({ sessionId, workspaceCwd: WS_A })).rejects.toThrow();
    ...
    describe('extNotification — in-session model update (A1, #4511)', () => {
        ...
        it('suppresses current_model_update while a bridge model roundtrip is in flight', async () => {
            ...
            const factory = async () => {
                ...
                capturedConn = new AgentSideConnection(() => augmented, agentStream);
                return {
                    stream: clientStream,
                    exited: new Promise(() => { }),
                    kill: async () => { },
            ...
            const modelChange = bridge
                .setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'qwen-max' }, undefined)
                .catch(() => { });
            await new Promise((r) => setTimeout(r, 10));
            ...
            expect(collected[0]?.type).toBe('model_switched');
            expect((collected[0]?.data).modelId).toBe('qwen-max');
            abort.abort();
            await bridge.shutdown();
        });
    ...
    describe('extNotification — followup_suggestion', () => {
        it('publishes followup_suggestion when the child fires a prompt-suggestion notification', async () => {
            ...
            void capturedConn.extNotification('qwen/notify/session/prompt-suggestion', {
                v: 1,
                sessionId: session.sessionId,
        ...
        it('drops malformed prompt-suggestion payloads', async () => {
            ...
            void capturedConn.extNotification('qwen/notify/session/prompt-suggestion', { v: 1, sessionId: session.sessionId, suggestion: '' });
            void capturedConn.extNotification('qwen/notify/session/prompt-suggestion', { v: 1, sessionId: session.sessionId, promptId: 'p1' });
            void capturedConn.extNotification('qwen/notify/session/prompt-suggestion', { v: 1 });
            void capturedConn.extNotification('qwen/notify/session/prompt-suggestion', {
                v: 1,
                sessionId: session.sessionId,
                suggestion: 123,
    ...
    describe('extNotification — session title update', () => {
        const titleFactory = (capture) => async () => {
            const { clientStream, agentStream } = createInMemoryChannel();
    ...
    describe('extNotification — recording degradation', () => {
        const recordingFactory = (capture) => async () => {
            const { clientStream, agentStream } = createInMemoryChannel();
            capture(new AgentSideConnection(() => new FakeAgent(), agentStream));
            return {
                stream: clientStream,
        ...
        it('publishes the live event and persists it in session snapshots', async () => {
            ...
            expect(collected.find((event) => event.type === 'session_snapshot')
                .data.recordingDegraded).toBe(true);
            abort.abort();
            await bridge.shutdown();
        });
        it('drops malformed recording degradation notifications', async () => {
            ...
            expect(collected.some((event) => event.type === 'session_recording_degraded')).toBe(false);
            const snapshot = collected.find((event) => event.type === 'session_snapshot');
            expect(snapshot?.type).toBe('session_snapshot');
            expect((snapshot?.data).recordingDegraded).toBe(false);
            abort.abort();
            await bridge.shutdown();
        });
        it('drains an early recording event into both replay and snapshot state', async () => {
            ...
            const factory = async () => {
                const { clientStream, agentStream } = createInMemoryChannel();
                capturedConn = new AgentSideConnection(() => new FakeAgent({ sessionIdPrefix: 'recording-buffer' }), agentStream);
                return {
                    stream: clientStream,
                    exited: new Promise(() => { }),
                    kill: async () => { },
    ...
    describe('maxSessions cap (chiga0 Rec 3)', () => {
        it('refuses NEW spawns past the cap with SessionLimitExceededError', async () => {
            let n = 0;
            const factory = async () => makeChannel({ sessionIdPrefix: `s${n++}` }).channel;
            const bridge = makeBridge({
                channelFactory: factory,
                maxSessions: 2,
            ...
            expect(bridge.sessionCount).toBe(2);
            // Third hits the cap.
            await expect(bridge.spawnOrAttach({ workspaceCwd: WS_A })).rejects.toMatchObject({
        ...
        it('attach to an existing session under single scope is NOT counted toward the cap', async () => {
            ...
            await expect(bridge.spawnOrAttach({ workspaceCwd: WS_B })).rejects.toBeInstanceOf(WorkspaceMismatchError);
            await bridge.shutdown();
        });
        it('killSession({requireZeroAttaches:true}) skips reap when another client attached (BQ9tV)', async () => {
            ...
            await expect(bridge.killSession(a.sessionId, { requireZeroAttaches: true })).resolves.toBe(false);
        ...
        it('killSession({requireZeroAttaches:true}) DOES reap when no other client attached (BQ9tV)', async () => {
            ...
            await expect(bridge.killSession(a.sessionId, { requireZeroAttaches: true })).resolves.toBe(true);
            expect(bridge.sessionCount).toBe(0);
        ...
        it('Stage 1.5 multi-session: N sessions on same workspace share ONE channel', async () => {
            ...
            const sessions = await Promise.all(Array.from({ length: 5 }, () => bridge.spawnOrAttach({ workspaceCwd: WS_A })));
            // 5 distinct sessions...
        ...
        it('releases fresh spawn admission once the session is live', async () => {
            const modelSwitchGate = deferred();
            let releaseCount = 0;
            const factory = async () => {
                const { clientStream, agentStream } = createInMemoryChannel();
                const fakeAgent = new FakeAgent();
        ...
        it('releases branchSession admission when the branch extMethod fails', async () => {
            ...
            contexts.length = 0;
            releases.length = 0;
            await expect(bridge.branchSession(session.sessionId, {
                name: 'Branch 1',
            })).rejects.toThrow();
            expect(contexts).toMatchObject([
        ...
        it('releases branch admission when transport fails during restore cleanup', async () => {
            ...
            await vi.waitFor(() => expect(handle.agent.loadSessionCalls).toHaveLength(1));
            failure.resolve(Object.assign(new Error('bounded frame failed'), {
                code: 'ndjson_frame_too_large',
            }));
    ...
    describe('closeSession', () => {
        it('publishes session_closed and removes session from maps', async () => {
            ...
            expect((closedEvent?.data).reason).toBe('client_close');
            await bridge.shutdown();
        });
        it('throws SessionNotFoundError for unknown session', async () => {
            const bridge = makeBridge();
            await expect(bridge.closeSession('nonexistent')).rejects.toThrow(SessionNotFoundError);
            await bridge.shutdown();
        });
        it('preserves bridge state when required agent close fails so retry can flush', async () => {
            ...
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            await expect(bridge.closeSession(session.sessionId, undefined, {
            ...
            expect(bridge.sessionCount).toBe(1);
            expect(() => bridge.recordHeartbeat(session.sessionId, {
                clientId: session.clientId,
            })).not.toThrow();
        ...
        it('resolves pending permissions as cancelled', async () => {
            ...
            expect(bridge.pendingPermissionCount).toBe(0);
            const resolvedIndex = events.findIndex((e) => e.type === 'permission_resolved');
        ...
        it('resolves pending permissions before waiting for agent close during kill', async () => {
                ...
                const fakeAgent = new FakeAgent({
                    extMethodImpl: async (method) => {
                        if (method !== 'qwen/control/session/close')
                            return {};
                        const result = (await permissionResponse.current);
                        expect(result.outcome.outcome).toBe('cancelled');
                        return {};
                    },
                });
            ...
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            permissionResponse.current = capturedConn.requestPermission({
                sessionId: session.sessionId,
        ...
        it('force-kills the channel when kill follows a stuck acknowledged close', async () => {
            ...
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            const closeResult = bridge.closeSession(session.sessionId).then(() => 'resolved', () => 'rejected');
            await closeStarted.promise;
            await expect(bridge.killSession(session.sessionId)).resolves.toBe(true);
    ...
    describe('updateSessionMetadata', () => {
        it('publishes session_metadata_updated event', async () => {
            ...
            await new Promise((r) => setImmediate(r));
            const metaEvent = events.find((e) => e.type === 'session_metadata_updated');
            expect(metaEvent).toBeDefined();
            expect((metaEvent?.data).displayName).toBe('Test Session');
            await bridge.closeSession(session.sessionId);
            await drain;
            await bridge.shutdown();
        });
        it('keeps the optimistic update and logs a generic persistence failure', async () => {
            ...
            try {
                const bridge = makeBridge({
                    channelFactory: async () => makeChannel({
                        extMethodImpl: (method) => method === SERVE_CONTROL_EXT_METHODS.sessionTitle
                            ? { persisted: false }
                            : {},
                    }).channel,
                });
                ...
                await vi.waitFor(() => expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining(`displayName for ${session.sessionId} was not persisted`)));
                expect(stderrSpy).not.toHaveBeenCalledWith(expect.stringContaining('recording service unavailable'));
                await bridge.shutdown();
            }
            finally {
                stderrSpy.mockRestore();
        ...
        it('rejects displayName values with control characters', async () => {
            ...
            const bridge = makeBridge({ channelFactory: factory });
            const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
            expect(() => bridge.updateSessionMetadata(session.sessionId, {
                displayName: 'bad\nname',
        ...
        it('throws SessionNotFoundError for unknown session', () => {
            const bridge = makeBridge();
            expect(() => bridge.updateSessionMetadata('nonexistent', {
                displayName: 'test',
            })).toThrow(SessionNotFoundError);
        });
    });
    describe('enriched listWorkspaceSessions', () => {
        ...
        it('reports active prompt state when attaching to an existing session', async () => {
            ...
            const bridge = makeBridge({
                channelFactory: async () => makeChannel({
                    promptImpl: () => new Promise((resolve) => {
    ...
    describe('publishWorkspaceEvent + knownClientIds (issue #4175 PR 16)', () => {
        it('fans out a workspace event onto every active session bus', async () => {
            ...
            const aFrames = [];
            const bFrames = [];
            const collect = async (sessionId, target, signal) => {
...
describe('onDiagnosticLine', () => {
    ...
    afterEach(() => {
        if (originalDebug === undefined)
            ...
            process.env['QWEN_SERVE_DEBUG'] = originalDebug;
    });
    it('receives writeServeDebugLine output when QWEN_SERVE_DEBUG=1', async () => {
        ...
        void capturedConn.requestPermission({
        ...
        const accepted = bridge.respondToSessionPermission(sessionB.sessionId, payload.requestId, { outcome: { outcome: 'cancelled' } });
        expect(accepted).toBe(false);
        // Verify the onDiagnosticLine callback received the debug line.
        expect(captured.some((e) => e.line.includes('qwen serve debug: '))).toBe(true);
        expect(captured.some((e) => e.line.includes('rejected permission vote'))).toBe(true);
        expect(captured.every((e) => e.level === undefined || e.level === 'info')).toBe(true);
        subAbort.abort();
        await bridge.shutdown();
    });
...
describe('extractErrorMessage', () => {
    ...
    it('extracts details from Error subclasses with JSON-RPC data', () => {
        expect(extractErrorMessage(new RequestError(-32603, 'Internal error', {
            details: 'session not found',
        }))).toBe('session not found');
    });
    it('extracts string data from Error subclasses with JSON-RPC data', () => {
        expect(extractErrorMessage(new RequestError(-32603, 'Internal error', 'session not found'))).toBe('session not found');
    });
    ...
    it('falls back to message when data.details is missing', () => {
        expect(extractErrorMessage({ code: -32600, message: 'Invalid Request' })).toBe('Invalid Request');
    });
...
describe('classifyTurnErrorKind', () => {
    it('classifies bare terminated errors as model stream interruptions', () => {
        expect(classifyTurnErrorKind('terminated')).toBe('model_stream_interrupted');
        expect(classifyTurnErrorKind('Terminated')).toBe('model_stream_interrupted');
        expect(classifyTurnErrorKind(' terminated ')).toBe('model_stream_interrupted');
    });
...
describe('createHttpAcpBridge — side-channel state layer (#4511)', () => {
    describe('publish helpers cache + generation', () => {
        it('publishModelSwitched updates cache and publishes model_switched', async () => {
            ...
            await bridge.setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'qwen-max' }, undefined);
            const it2 = iter[Symbol.asyncIterator]();
            const next = await it2.next();
            expect(next.value?.type).toBe('model_switched');
            expect((next.value?.data).modelId).toBe('qwen-max');
            abort.abort();
            await bridge.shutdown();
        });
        it('publishModelSwitched stamps the active promptId', async () => {
            ...
            const promptDone = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'keep the prompt active' }],
            }, undefined, { clientId: session.clientId, promptId: 'prompt-model' });
            await vi.waitFor(() => {
                expect(releasePrompt).toBeTypeOf('function');
            });
            await bridge.setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'qwen-max' }, undefined);
        ...
        it('publishApprovalModeChanged publishes approval_mode_changed on setSessionApprovalMode', async () => {
            ...
            await bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.YOLO, { persist: false });
            const it2 = iter[Symbol.asyncIterator]();
            const next = await it2.next();
            expect(next.value?.type).toBe('approval_mode_changed');
            expect((next.value?.data).next).toBe(ApprovalMode.YOLO);
            abort.abort();
            await bridge.shutdown();
        });
        it('publishApprovalModeChanged stamps the active promptId', async () => {
            ...
            const promptDone = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'keep the prompt active' }],
            }, undefined, { clientId: session.clientId, promptId: 'prompt-approval' });
            await vi.waitFor(() => {
                expect(releasePrompt).toBeTypeOf('function');
            });
            await bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.YOLO, { persist: false });
    ...
    describe('extNotification — in-session mode update (A2)', () => {
        it('promotes current_mode_update to approval_mode_changed when no bridge roundtrip is in flight', async () => {
            ...
            expect((collected[0]?.data).next).toBe('auto-edit');
            abort.abort();
            await bridge.shutdown();
        });
                ...
                const fakeAgent = new FakeAgent({
                    extMethodImpl: (method) => {
                        if (method.includes('approval_mode')) {
                            return new Promise((res) => {
                                releaseMode = () => res({ previous: 'default', current: 'yolo' });
                            });
                        }
                        return {};
                    },
        ...
        it('dual-emits a legacy session_update on the setMode path (no legacyFrameSent)', async () => {
            ...
            const update = (collected[1]?.data).update;
    ...
    describe('A5 — session snapshot on attach', () => {
        it('yields session_snapshot after replay_complete when snapshot=true', async () => {
            ...
            expect(snap.data.currentModelId).toBe('qwen-turbo');
            abort.abort();
            await bridge.shutdown();
        });
        it('carries currentApprovalMode in the snapshot when an approval-mode change was promoted', async () => {
            ...
            expect(snap).toBeDefined();
            expect(snap.data
                .currentApprovalMode).toBe('auto-edit');
            abort.abort();
            await bridge.shutdown();
        });
        it('does NOT yield session_snapshot when snapshot is not set', async () => {
            ...
            for await (const e of iter) {
                ...
                if (collected.some((c) => c.type === 'replay_complete') &&
                    collected.some((c) => c.type === 'model_switched'))
                    break;
            }
            expect(collected.find((e) => e.type === 'session_snapshot')).toBeUndefined();
            abort.abort();
            await bridge.shutdown();
        });
        it('yields session_snapshot up front on a fresh connection (no Last-Event-ID)', async () => {
            ...
            expect((first.value?.data).currentModelId).toBe('qwen-turbo');
            abort.abort();
            await bridge.shutdown();
        });
        it('seeds snapshot from newSession response without any intermediate notification (cold attach)', async () => {
            ...
            const factory = async () => {
                const { clientStream, agentStream } = createInMemoryChannel();
                const fakeAgent = new FakeAgent({
                    newSessionImpl: (p) => Promise.resolve({
                ...
                new AgentSideConnection(() => fakeAgent, agentStream);
                return {
                    stream: clientStream,
                    exited: new Promise(() => { }),
                    kill: async () => { },
    ...
    describe('§2.2 — post-roundtrip reconciliation', () => {
        const makeReconcileFactory = (sessionContextModelId, opts = {}) => async () => {
            const { clientStream, agentStream } = createInMemoryChannel();
            const fakeAgent = new FakeAgent({
                extMethodImpl: (method) => {
                    if (method === 'qwen/status/session/context') {
        ...
        it('publishes a corrective model_switched when the agent state drifted from cache', async () => {
            ...
            for await (const e of iter) {
                ...
                if (seen.filter((s) => s.type === 'model_switched').length === 2)
                    break;
            }
            ...
            const requestedSwitchIndex = seen.findIndex((s) => s.type === 'model_switched' && s.modelId === 'qwen-max');
            const settingsChangedIndex = seen.findIndex((s) => s.type === 'settings_changed' && s.value === 'qwen-max');
            const correctiveSwitchIndex = seen.findIndex((s) => s.type === 'model_switched' && s.modelId === 'qwen-turbo');
        ...
        it('does NOT publish a corrective event when agent state matches cache', async () => {
            ...
            await bridge.setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'qwen-plus' }, undefined);
            const switches = [];
            for await (const e of iter) {
                if (e.type === 'model_switched') {
                    switches.push(e.data.modelId);
                    if (switches.includes('qwen-plus'))
                        break;
                }
            }
        ...
        it('swallows a failed status read without crashing or masking the original change', async () => {
            ...
            await expect(bridge.setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'qwen-max' }, undefined)).resolves.toBeDefined();
            const it2 = iter[Symbol.asyncIterator]();
            const next = await it2.next();
        ...
        it('does NOT reconcile when the model roundtrip itself fails', async () => {
            ...
            await expect(bridge.setSessionModel(session.sessionId, { sessionId: session.sessionId, modelId: 'qwen-max' }, undefined)).rejects.toThrow();
            const it2 = iter[Symbol.asyncIterator]();
            const next = await it2.next();
        ...
        it('re-runs reconcile when a newer change publishes during the status read (generation rerun)', async () => {
            ...
            const firstStatusGate = new Promise((res) => {
            ...
            for await (const e of iter) {
                if (e.type === 'model_switched') {
                    switches.push(e.data.modelId);
                    if (switches.includes('qwen-turbo'))
                        break;
                }
            }
        ...
        it('publishes a corrective approval_mode_changed when the agent mode drifted from cache', async () => {
            ...
            await bridge.setSessionApprovalMode(session.sessionId, ApprovalMode.YOLO, { persist: false }, undefined);
            const nexts = [];
            for await (const e of iter) {
                if (e.type === 'approval_mode_changed') {
                    nexts.push(e.data.next);
                    if (nexts.length === 2)
                        break;
                }
            }
        ...
        it('syncs peer session cache on persisted approval-mode change (snapshot reflects new mode)', async () => {
            ...
            await bridge.setSessionApprovalMode(sessionA.sessionId, ApprovalMode.YOLO, { persist: true }, undefined);
...
describe('channelIdleTimeoutMs', () => {
    it('kills the channel immediately when timeout is 0 (default)', async () => {
        const handle = makeChannel();
        const factory = async () => handle.channel;
        const bridge = makeBridge({ channelFactory: factory });
        const session = await bridge.spawnOrAttach({
            workspaceCwd: WS_A,
            sessionScope: 'thread',
...
describe('session idle reaper', () => {
    ...
    it('publishes session_closed with reason idle_timeout via closeSession opts', async () => {
        ...
        expect(closedEv.data.reason).toBe('idle_timeout');
        await bridge.shutdown();
    });
    it('closeSession defaults to reason client_close', async () => {
        ...
        const closedEv = events.find((e) => e.type === 'session_closed');
        expect(closedEv).toBeDefined();
        expect(closedEv.data.reason).toBe('client_close');
        await bridge.shutdown();
    });
...
describe('activePromptCount and lastActivityAt', () => {
    ...
    it('activePromptCount tracks multiple concurrent sessions', async () => {
        ...
        await vi.waitFor(() => {
            const totalPrompts = handles.reduce((sum, h) => sum + h.agent.promptCalls.length, 0);
    ...
    it('activePromptCount does not go negative when closeSession cancels an active prompt', async () => {
        let rejectPrompt;
        const handle = makeChannel({
            promptImpl: () => new Promise((_resolve, reject) => {
                rejectPrompt = reject;
            }),
            cancelImpl: () => {
                rejectPrompt?.(new Error('cancelled'));
    ...
    it('queued prompt rejects when the channel crashes before it starts', async () => {
        ...
        expect(resultB.error.name).toBe('AbortError');
        await vi.waitFor(() => {
            expect(bridge.activePromptCount).toBe(0);
            expect(bridge.sessionCount).toBe(0);
        });
...
describe('createAcpSessionBridge — background notifications', () => {
    it('counts a notification while the child is accepting it', async () => {
        ...
        const handle = makeChannel({
            extMethodImpl: async (method) => method === SERVE_CONTROL_EXT_METHODS.sessionBackgroundNotification
                ? acceptance.promise
                : {},
        });
        const bridge = makeBridge({ channelFactory: async () => handle.channel });
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        const notification = bridge.enqueueBackgroundNotification(session.sessionId, {
    ...
    it('forwards a daemon-owned worker completion to the live parent session', async () => {
        const handle = makeChannel({
            extMethodImpl: async (method, params) => method === SERVE_CONTROL_EXT_METHODS.sessionBackgroundNotification
                ? { sessionId: params['sessionId'], accepted: true }
                : {},
        });
        const bridge = makeBridge({ channelFactory: async () => handle.channel });
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        await expect(bridge.enqueueBackgroundNotification(session.sessionId, {
            displayText: 'Worker completed.',
            modelText: '<task-notification />',
            taskId: 'worker-1',
            status: 'completed',
    ...
    it('rejects a worker completion for an unknown parent session', async () => {
        ...
        await expect(bridge.enqueueBackgroundNotification('missing', {
            displayText: 'Worker completed.',
            modelText: '<task-notification />',
            taskId: 'worker-1',
            status: 'completed',
...
describe('createAcpSessionBridge — mid-turn message queue (enqueueMidTurnMessage)', () => {
    ...
    it('owns and promotes a request without a caller-supplied id', async () => {
        ...
        await new Promise((r) => setTimeout(r, 10));
        release();
        await vi.waitFor(() => expect(bridge.getPendingPrompts(session.sessionId)).toEqual([]));
        await bridge.shutdown();
    });
    it('rejects admission while the session is closing', async () => {
        ...
        await closeStarted.promise;
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'too late', { clientId: session.clientId }, 'closing-message')).toEqual({ accepted: false });
        closeGate.resolve({});
        await close;
        await bridge.shutdown();
    });
    it('promotes a stable-id request that reaches an idle session', async () => {
        ...
        const bridge = makeBridge({ channelFactory: factory });
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'late request', { clientId: session.clientId }, 'client-mid-late')).toEqual({ accepted: true, messageId: 'client-mid-late' });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'late request', { clientId: session.clientId }, 'client-mid-late')).toEqual({ accepted: true, messageId: 'client-mid-late' });
        expect(bridge.getPendingPrompts(session.sessionId)).toEqual([
            expect.objectContaining({
    ...
    it('throws SessionNotFoundError for an unknown session', async () => {
        ...
        expect(() => bridge.enqueueMidTurnMessage('nope', 'hi')).toThrow(SessionNotFoundError);
        await bridge.shutdown();
    });
    it('lets another attached client remove an undrained session message', async () => {
        ...
        const promptPromise = bridge
            ...
            .catch(() => { });
        await new Promise((r) => setTimeout(r, 10));
        const admission = bridge.enqueueMidTurnMessage(session.sessionId, 'remove me', { clientId: session.clientId });
        expect(admission).toEqual({
            accepted: true,
            messageId: expect.any(String),
        });
        const attached = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(bridge.removeMidTurnMessage(session.sessionId, admission.messageId, {
    ...
    it('does not recreate a deleted stable id when its admission is retried', async () => {
        ...
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'delete me', { clientId: session.clientId }, 'stable-deleted')).toEqual({ accepted: true, messageId: 'stable-deleted' });
        const attached = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(bridge.removeMidTurnMessage(session.sessionId, 'stable-deleted', {
            clientId: attached.clientId,
        })).toEqual({ removed: true });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'delete me', { clientId: session.clientId }, 'stable-deleted')).toEqual({ accepted: true, messageId: 'stable-deleted' });
        expect(bridge.getMidTurnMessages(session.sessionId, {
            clientId: session.clientId,
        })).toEqual({
            messages: [],
        ...
        await vi.waitFor(() => {
            expect(events.some((event) => event.type === 'pending_prompt_added' &&
                event.promptId === 'stable-deleted')).toBe(true);
            expect(events.some((event) => event.type === 'pending_prompt_completed' &&
                event.promptId === 'stable-deleted' &&
                event.data.state === 'removed')).toBe(true);
        });
        abort.abort();
        await collecting;
        release();
    ...
    it('mints distinct stable ids so two same-text messages remove independently', async () => {
        ...
        const second = bridge.enqueueMidTurnMessage(session.sessionId, 'same text', { clientId: session.clientId });
        ...
        expect(bridge.removeMidTurnMessage(session.sessionId, second.messageId, {
            clientId: session.clientId,
        })).toEqual({ removed: true });
        // The first entry survives its sibling's removal and is independently
        // removable by its own id.
        expect(bridge.removeMidTurnMessage(session.sessionId, first.messageId, {
            clientId: session.clientId,
        })).toEqual({ removed: true });
        expect(bridge.removeMidTurnMessage(session.sessionId, first.messageId, {
            clientId: session.clientId,
    ...
    it('drains the queue through the child connection; a second drain is empty', async () => {
        ...
        const drained = await handle.agentConnection.extMethod('craft/drainMidTurnQueue', { sessionId: session.sessionId });
        expect(drained).toEqual({
            messages: ['m1', 'm2'],
            hasQueuedPrompt: false,
        });
        // Spliced out, so the next batch's drain is empty.
        expect(await handle.agentConnection.extMethod('craft/drainMidTurnQueue', {
            sessionId: session.sessionId,
    ...
    it('promotes every undrained message at settle', async () => {
        ...
        const bridge = makeBridge({ channelFactory: async () => handle.channel });
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        const send = (text) => bridge
            .sendPrompt(session.sessionId, { sessionId: session.sessionId, prompt: [{ type: 'text', text }] }, undefined, { clientId: session.clientId })
            .catch(() => { });
        const t1 = send('t1');
        await new Promise((r) => setTimeout(r, 10));
        const admission = bridge.enqueueMidTurnMessage(session.sessionId, 'leftover');
        expect(admission).toEqual({
            accepted: true,
            messageId: expect.any(String),
        });
    ...
    it('promotes messages after their client detaches without reapplying the cap', async () => {
        ...
        const first = bridge.sendPrompt(session.sessionId, {
            sessionId: session.sessionId,
            prompt: [{ type: 'text', text: 'first' }],
        }, undefined, { clientId: session.clientId, promptId: 'first-id' });
        await vi.waitFor(() => expect(prompts).toEqual(['first']));
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'follow up', { clientId: session.clientId }, 'webui-follow-up')).toEqual({ accepted: true, messageId: 'webui-follow-up' });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'follow up', { clientId: session.clientId }, 'webui-follow-up')).toEqual({ accepted: true, messageId: 'webui-follow-up' });
        await bridge.detachClient(session.sessionId, session.clientId);
        releases[0]();
        await first;
    ...
    it('removes a promoted message through the shared mid-turn API', async () => {
        ...
        await vi.waitFor(() => expect(prompts).toEqual(['first']));
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'follow up', { clientId: session.clientId }, 'promoted-delete')).toEqual({ accepted: true, messageId: 'promoted-delete' });
        releases[0]();
        await first;
        await vi.waitFor(() => expect(prompts).toEqual(['first', 'follow up']));
        const attached = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(bridge.removeMidTurnMessage(session.sessionId, 'promoted-delete', {
            clientId: attached.clientId,
        })).toEqual({ removed: true });
        expect(bridge.getMidTurnMessages(session.sessionId, {
            clientId: session.clientId,
        }).settledMessageIds).toContain('promoted-delete');
        releases[1]();
        await vi.waitFor(() => expect(bridge.getPendingPrompts(session.sessionId)).toEqual([]));
        await bridge.shutdown();
    });
    it('does not remove an ordinary pending prompt through the mid-turn API', async () => {
        ...
        const ordinary = bridge.sendPrompt(session.sessionId, {
            sessionId: session.sessionId,
            prompt: [{ type: 'text', text: 'ordinary queued prompt' }],
        }, undefined, { clientId: session.clientId, promptId: 'ordinary-pending' });
        expect(bridge.removeMidTurnMessage(session.sessionId, 'ordinary-pending', {
            clientId: session.clientId,
        })).toEqual({ removed: false });
        expect(bridge.getPendingPrompts(session.sessionId)).toEqual(expect.arrayContaining([
    ...
    it('keeps the queue across a back-to-back prompt FIFO, clearing only at true idle', async () => {
        ...
        const bridge = makeBridge({ channelFactory: async () => handle.channel });
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        const send = (text, promptId) => bridge
            .sendPrompt(session.sessionId, { sessionId: session.sessionId, prompt: [{ type: 'text', text }] }, undefined, { clientId: session.clientId, promptId })
            .catch(() => { });
        const abort = new AbortController();
        const iter = bridge.subscribeEvents(session.sessionId, {
            signal: abort.signal,
        ...
        const injected = (async () => {
            for await (const event of iter) {
                if (event.type === 'mid_turn_message_injected')
                    return event;
            }
        ...
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'x', {
            clientId: session.clientId,
        })).toEqual({
            accepted: true,
            messageId: expect.any(String),
    ...
    it('switches session-update attribution between queued turns and clears it at idle', async () => {
        ...
        }, undefined, { clientId: session.clientId, promptId: 'prompt-1' });
        await vi.waitFor(() => expect(handle.agent.promptCalls).toHaveLength(1));
        await handle.agentConnection.sessionUpdate({
            sessionId: session.sessionId,
            update: {
        ...
        }, undefined, { clientId: session.clientId, promptId: 'prompt-2' });
        releases[0]();
        await p1;
        await vi.waitFor(() => expect(handle.agent.promptCalls).toHaveLength(2));
        await handle.agentConnection.sessionUpdate({
    ...
    it('rejects a non-member client id (mirrors /prompt and /btw authorization)', async () => {
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(() => bridge.enqueueMidTurnMessage(session.sessionId, 'sneaky', {
            clientId: 'client-not-issued',
    ...
    it('publishes a session-wide injection frame', async () => {
        ...
        const prompt = bridge
            ...
            sessionId: session.sessionId,
            prompt: [{ type: 'text', text: 'go' }],
        }, undefined, { clientId: session.clientId, promptId: 'prompt-mid-turn' })
            .catch(() => { });
        await new Promise((r) => setTimeout(r, 10));
    ...
    it('rejects past MAX_MID_TURN_QUEUE_DEPTH (20) — the DoS bound', async () => {
        ...
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'overflow')).toEqual({ accepted: false });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'stable overflow', { clientId: session.clientId }, 'stable-overflow')).toEqual({ accepted: false });
    ...
    it('uses a caller message id idempotently', async () => {
        ...
        const prompt = bridge
            ...
            .catch(() => { });
        await new Promise((r) => setTimeout(r, 10));
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'note', { clientId: session.clientId }, 'client-mid-1')).toEqual({ accepted: true, messageId: 'client-mid-1' });
        const attached = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'different note', { clientId: attached.clientId }, 'client-mid-1')).toEqual({ accepted: false });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'note', { clientId: attached.clientId }, 'client-mid-1')).toEqual({ accepted: true, messageId: 'client-mid-1' });
    ...
    it('trims the message before queuing (drain returns the trimmed text)', async () => {
        ...
        const prompt = bridge
            ...
            .catch(() => { });
        await new Promise((r) => setTimeout(r, 10));
        expect(bridge.enqueueMidTurnMessage(session.sessionId, '   hello   ')).toEqual({ accepted: true, messageId: expect.any(String) });
        const drained = await handle.agentConnection.extMethod('craft/drainMidTurnQueue', { sessionId: session.sessionId });
        expect(drained).toEqual({
            messages: ['hello'],
            hasQueuedPrompt: false,
    ...
    it('getMidTurnMessages returns every shared message', async () => {
        ...
        const prompt = bridge
            ...
            .catch(() => { });
        await new Promise((r) => setTimeout(r, 10));
        const admission = bridge.enqueueMidTurnMessage(session.sessionId, 'still waiting', { clientId: session.clientId }, 'client-mid-waiting');
        expect(admission.accepted).toBe(true);
        const generated = bridge.enqueueMidTurnMessage(session.sessionId, 'server-generated id', { clientId: session.clientId });
    ...
    it('getMidTurnMessages moves drained stable ids into the settled ring', async () => {
        ...
        const prompt = bridge
            ...
            .catch(() => { });
        await new Promise((r) => setTimeout(r, 10));
        const admission = bridge.enqueueMidTurnMessage(session.sessionId, 'note', { clientId: session.clientId }, 'client-mid-drained');
        expect(admission.accepted).toBe(true);
        await handle.agentConnection.extMethod('craft/drainMidTurnQueue', {
            sessionId: session.sessionId,
        });
    ...
    it('getMidTurnMessages throws for unknown sessions and unbound client ids', async () => {
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(() => bridge.getMidTurnMessages('nope')).toThrow(SessionNotFoundError);
        expect(() => bridge.getMidTurnMessages(session.sessionId, {
    ...
    it('answers a retry from the promoted ring after the promoted prompt completes', async () => {
        ...
        const bridge = makeBridge({ channelFactory: async () => handle.channel });
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'promote once', { clientId: session.clientId }, 'stable-promoted')).toEqual({ accepted: true, messageId: 'stable-promoted' });
    ...
    it('promotes multiple undrained messages at settle in FIFO order', async () => {
        ...
        const m3 = bridge.enqueueMidTurnMessage(session.sessionId, 'leftover three');
        releases[0]();
        await anchor;
    ...
    it('bounds the settled ring across many removals', async () => {
        ...
        for (let i = 0; i < total; i++) {
            expect(bridge.enqueueMidTurnMessage(session.sessionId, `m${i}`, { clientId: session.clientId }, `ring-${i}`).accepted).toBe(true);
            expect(bridge.removeMidTurnMessage(session.sessionId, `ring-${i}`, {
                clientId: session.clientId,
            })).toEqual({ removed: true });
        }
        const snapshot = bridge.getMidTurnMessages(session.sessionId, {
        ...
        expect(snapshot.settledMessageIds).toHaveLength(MID_TURN_RECONCILIATION_RING_SIZE);
    ...
    it('bounds the promoted ring across many idle promotions', async () => {
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        const burstSize = 20;
        const bursts = Math.ceil((MID_TURN_RECONCILIATION_RING_SIZE + 1) / burstSize);
        for (let burst = 0; burst < bursts; burst++) {
            for (let i = 0; i < burstSize; i++) {
                expect(bridge.enqueueMidTurnMessage(session.sessionId, `p-${burst}-${i}`, { clientId: session.clientId }, `p-${burst}-${i}`).accepted).toBe(true);
            }
            await vi.waitFor(() => {
        ...
        const snapshot = bridge.getMidTurnMessages(session.sessionId);
        expect(snapshot.promotedMessageIds).toHaveLength(MID_TURN_RECONCILIATION_RING_SIZE);
        expect(snapshot.promotedMessageIds).not.toContain('p-0-0');
        expect(snapshot.promotedMessageIds).toContain(`p-${bursts - 1}-${burstSize - 1}`);
        await bridge.shutdown();
    });
    it('rejects mid-turn admission while a conditional close is being confirmed', async () => {
        ...
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'already owned', undefined, 'stable-owned')).toEqual({ accepted: true, messageId: 'stable-owned' });
        await vi.waitFor(() => expect(bridge.getPendingPrompts(session.sessionId)).toEqual([]));
        await sendActiveWorkSnapshot(handle, 1, [
            { sessionId: session.sessionId, holds: [] },
        ]);
        ...
        await closeRequested.promise;
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'already owned', undefined, 'stable-owned')).toEqual({ accepted: true, messageId: 'stable-owned' });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'race the teardown')).toEqual({ accepted: false });
        expect(bridge.getPendingPrompts(session.sessionId)).toEqual([]);
        closeResponse.resolve({ closed: true, holds: [] });
        await detached;
        await vi.waitFor(() => expect(bridge.sessionCount).toBe(0));
    ...
    it('publishes pending_prompt_started for a promoted message that starts immediately', async () => {
        ...
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'settled late', { clientId: session.clientId }, 'late-started')).toEqual({ accepted: true, messageId: 'late-started' });
        await vi.waitFor(() => expect(prompts).toEqual(['settled late']));
        await vi.waitFor(() => expect(events.some((event) => event.type === 'pending_prompt_started' &&
            event.promptId === 'late-started' &&
            event.data.text === 'settled late' &&
            event.originatorClientId === session.clientId)).toBe(true));
        // An immediate start never queues: the added event would be unpaired.
        expect(events.filter((event) => event.type === 'pending_prompt_added' &&
            event.promptId === 'late-started')).toEqual([]);
        releases[0]();
        await vi.waitFor(() => expect(bridge.getPendingPrompts(session.sessionId)).toEqual([]));
        abort.abort();
        await collecting;
    ...
    it('rejects a queueOnly enqueue on an idle session instead of promoting', async () => {
        ...
        const bridge = makeBridge({ channelFactory: async () => handle.channel });
        const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'steering', { clientId: session.clientId }, 'steer-idle', { queueOnly: true })).toEqual({ accepted: false });
        expect(promptCalls).toBe(0);
        expect(bridge.getPendingPrompts(session.sessionId)).toEqual([]);
        expect(bridge.getMidTurnMessages(session.sessionId).promotedMessageIds).toEqual([]);
        // The default path still promotes on idle.
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'steering', { clientId: session.clientId }, 'steer-idle')).toEqual({ accepted: true, messageId: 'steer-idle' });
    ...
    it('still queues a queueOnly enqueue while the session is busy', async () => {
        ...
        const onSettledWithoutDrain = vi.fn(() => {
            fallback = bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'steer busy' }],
            }, undefined, { promptId: 'steer-next' });
        });
        const prompt = bridge
            .sendPrompt(session.sessionId, {
            sessionId: session.sessionId,
            ...
            .catch(() => { });
        await vi.waitFor(() => expect(prompts).toEqual(['go']));
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'steer busy', undefined, 'steer-busy', { queueOnly: true, onSettledWithoutDrain })).toEqual({ accepted: true, messageId: 'steer-busy' });
        expect(bridge.getMidTurnMessages(session.sessionId).messages).toEqual([]);
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'ordinary after steering', { clientId: session.clientId }, 'ordinary-after-steering')).toEqual({
            accepted: true,
            messageId: 'ordinary-after-steering',
        });
        release.resolve();
        await prompt;
        await vi.waitFor(() => expect(onSettledWithoutDrain).toHaveBeenCalledOnce());
        await vi.waitFor(() => expect(prompts).toEqual(['go', 'steer busy', 'ordinary after steering']));
        await fallback;
        expect(bridge.getMidTurnMessages(session.sessionId).promotedMessageIds).toEqual(['ordinary-after-steering']);
        expect(events.filter((event) => (event.type === 'pending_prompt_added' ||
            event.type === 'pending_prompt_started') &&
            event.promptId === 'steer-next')).toEqual([]);
        const steeringUserEvents = events.filter((event) => event.type === 'session_update' &&
    ...
    it('keeps anonymous enqueues off the shared queue surface', async () => {
        ...
        const anonymous = bridge.enqueueMidTurnMessage(session.sessionId, '<realtime_delegation>internal steering</realtime_delegation>');
        expect(anonymous.accepted).toBe(true);
        expect(bridge.enqueueMidTurnMessage(session.sessionId, 'from the ui', { clientId: session.clientId }, 'ui-visible')).toEqual({ accepted: true, messageId: 'ui-visible' });
        await vi.waitFor(() => expect(events.some((event) => event.type === 'pending_prompt_added' &&
            event.promptId === 'ui-visible')).toBe(true));
        expect(events.filter((event) => event.type === 'pending_prompt_added' &&
            event.promptId === anonymous.messageId)).toEqual([]);
        expect(bridge.getMidTurnMessages(session.sessionId).messages).toEqual([
...
describe('createAcpSessionBridge — child-resource refresh', () => {
    it('single-flights the refresh so a slow child cannot pile up concurrent polls', async () => {
        ...
        const bridge = makeBridge({ channelFactory: async () => handle.channel });
        const wrCalls = () => handle.agent.extMethodCalls.filter((c) => c.method === SERVE_STATUS_EXT_METHODS.workspaceResource).length;
        try {
            await bridge.spawnOrAttach({ workspaceCwd: WS_A });
    ...
    it('ages the snapshot, and drops it entirely once past the staleness window', async () => {
        const handle = makeChannel({
            extMethodImpl: async (method) => method === SERVE_STATUS_EXT_METHODS.workspaceResource
