 ...
 */
async function send(writable, msg) {
...
async function recvOne(readable) {
...
describe('createInMemoryChannel', () => {
    ...
    it('preserves order across multiple frames in one direction', async () => {
        ...
        reader.releaseLock();
        expect(out.map((m) => m.params.i)).toEqual([
            1, 2, 3,
        ]);
    });
    it('isolates client→agent direction (client write does not echo to client.readable)', async () => {
        ...
        expect(onAgent.method).toBe('client-only');
        ...
        try {
            const winner = await Promise.race([
                reader.read().then(() => 'leaked', () => null),
                new Promise((res) => setTimeout(() => res('isolated'), 50)),
            ]);
    ...
    it('isolates agent→client direction (agent write does not echo to agent.readable)', async () => {
        ...
        expect(onClient.method).toBe('agent-only');
        const reader = agentStream.readable.getReader();
        try {
            const winner = await Promise.race([
                reader.read().then(() => 'leaked', () => null),
    ...
    it('abort() settles pending readers on both sides (teardown invariant)', async () => {
        ...
        const wrap = (p) => Promise.race([
            ...
            new Promise((res) => setTimeout(() => res('hung'), 100)),
        ]);
    ...
    it('abort() with no reason still tears down', async () => {
        ...
        const reader = agentStream.readable.getReader();
        const readP = reader.read().then(() => 'settled', () => 'settled');
        abort();
...
//# sourceMappingURL=inMemoryChannel.test.js.map