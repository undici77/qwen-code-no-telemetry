...
import { SessionArtifactAuthorizationError, SessionArtifactStore, SessionArtifactValidationError, } from './sessionArtifacts.js';
import { stableSessionArtifactId, } from '@qwen-code/qwen-code-core';
...
describe('SessionArtifactStore', () => {
    ...
    function managedIdForWorkspacePath(workspacePath) {
    ...
    it('keeps live artifact when durable tombstone persistence fails', async () => {
        ...
        const created = await store.upsertMany([{ title: 'Delete me', url: 'https://example.com/delete-me' }], { strict: true });
        const artifactId = created.changes[0].artifactId;
        failWrites = true;
        await expect(store.remove(artifactId)).resolves.toMatchObject({
            changes: [],
    ...
    it('gets artifacts and refreshes stale workspace status', async () => {
        ...
        await fs.writeFile(path.join(workspace, 'report.txt'), 'HELLO');
        await fs.utimes(path.join(workspace, 'report.txt'), new Date('2026-07-06T00:00:00.000Z'), new Date('2026-07-06T00:00:00.000Z'));
        vi.useFakeTimers();
    ...
    it('does not count injected workspace hash metadata against the user metadata limit', async () => {
        ...
        const created = await store.upsertMany([{ title: 'Budget', workspacePath: 'budget.txt', metadata }], { strict: true });
    ...
    it('prevents one client from removing another client retained artifact', async () => {
        ...
        try {
            await expect(store.remove(artifactId, { clientId: 'client-b' })).rejects.toBeInstanceOf(SessionArtifactAuthorizationError);
            await expect(store.remove(artifactId)).rejects.toBeInstanceOf(SessionArtifactAuthorizationError);
        ...
        await expect(store.remove(artifactId, { clientId: 'client-a' })).resolves.toMatchObject({
            changes: [{ action: 'removed', artifactId, reason: 'explicit' }],
        });
    });
    it('prevents one client from upserting another client retained artifact', async () => {
        ...
        ], { strict: true })).rejects.toBeInstanceOf(SessionArtifactAuthorizationError);
        await expect(store.list()).resolves.toMatchObject({
            artifacts: [
                {
                    id: artifactId,
    ...
    it('keeps client ids live while omitting them from durable artifact records', async () => {
        ...
        const listed = await store.list();
        expect(listed.artifacts[0]).not.toHaveProperty('clientId');
        await expect(store.remove(created.changes[0].artifactId, { clientId: 'client-b' })).rejects.toBeInstanceOf(SessionArtifactAuthorizationError);
        await expect(store.remove(created.changes[0].artifactId, { clientId: 'client-a' })).resolves.toMatchObject({
            changes: [
                {
                    action: 'removed',
        ...
        expect(events[0]?.changes[0]?.artifact).not.toHaveProperty('persistenceWarning');
        await expect(store.upsertMany([
            {
                title: 'Client artifact',
                url,
        ...
        ], { strict: true })).resolves.toMatchObject({
            changes: [
                {
                    action: 'created',
                    artifactId: created.changes[0].artifactId,
    ...
    it('drops legacy client ownership from restored durable artifact records', async () => {
        ...
        expect(listed.artifacts[0]).not.toHaveProperty('clientId');
        await expect(store.remove(artifactId, { clientId: 'client-b' })).resolves.toMatchObject({
            changes: [{ action: 'removed', artifactId, reason: 'explicit' }],
        });
    });
    it('rolls back received sequence when strict upsert persistence fails', async () => {
        ...
        const sequenceState = store;
        await store.upsertMany([{ title: 'First', url: 'https://example.com/first' }], { strict: true });
        expect(sequenceState.receivedSeq).toBe(1);
        fail = true;
        await expect(store.upsertMany([{ title: 'Second', url: 'https://example.com/second' }], { strict: true })).rejects.toThrow('persist failed');
        expect(sequenceState.receivedSeq).toBe(1);
        fail = false;
        await store.upsertMany([{ title: 'Third', url: 'https://example.com/third' }], { strict: true });
    ...
    it('does not consume persistence sequence when strict event writes fail', async () => {
        ...
        fail = true;
        await expect(store.upsertMany([{ title: 'First', url: 'https://example.com/sequence-first' }], { strict: true })).rejects.toThrow('persist failed');
        fail = false;
        await store.upsertMany([{ title: 'Second', url: 'https://example.com/sequence-second' }], { strict: true });
    ...
    it('rejects untrusted published artifacts and allows trusted published upgrades', async () => {
        ...
        ], { strict: true })).rejects.toBeInstanceOf(SessionArtifactValidationError);
    ...
    it('upgrades a workspace artifact when the artifact tool publishes the same path', async () => {
        ...
        const created = await store.upsertMany([{ title: 'Draft', workspacePath: 'reports/dashboard.html' }], { strict: true });
        const upgraded = await store.upsertMany([
            {
    ...
    it('keeps published artifacts detached when their original workspace path is recorded again', async () => {
        ...
        const artifactUrl = pathToFileURL(artifactPath).href;
        await fs.writeFile(artifactPath, 'hello');
        await store.upsertMany([{ title: 'Draft', workspacePath: 'reports/dashboard.html' }], { strict: true });
        const upgraded = await store.upsertMany([
            {
                title: 'Published dashboard',
                storage: 'published',
        ...
        const repeated = await store.upsertMany([{ title: 'Draft again', workspacePath: 'reports/dashboard.html' }], { strict: true });
    ...
    it('accepts trusted published file urls outside the workspace', async () => {
        ...
        try {
            ...
            ], { strict: true, trustedPublisher: true })).resolves.toMatchObject({
                changes: [
                    {
                        artifact: {
                            storage: 'published',
    ...
    it('rejects untrusted file urls', async () => {
        ...
        ], { strict: true })).rejects.toMatchObject({ field: 'url' });
    });
    it('evicts non-retained old artifacts before client-retained artifacts', async () => {
        ...
        expect(overflow.changes).toContainEqual(expect.objectContaining({
            action: 'removed',
            artifactId: tool.changes[0]?.artifactId,
            reason: 'eviction',
        }));
        expect((await store.list()).artifacts.map((a) => a.id)).toContain(client.changes[0]?.artifactId);
    });
    it('evicts the oldest client-retained artifact when no other overflow candidate exists', async () => {
        ...
        expect(overflow.changes.find((change) => change.artifactId === first.changes[0]?.artifactId)).not.toHaveProperty('durableTombstoneRequired');
        expect((await store.list()).artifacts.map((artifact) => artifact.id)).toEqual([second.changes[0]?.artifactId, overflow.changes[0]?.artifactId]);
    });
    it('writes eviction removals to durable persistence', async () => {
        ...
        const first = await store.upsertMany([{ title: 'First', url: 'https://example.com/first' }], { strict: true });
        await store.upsertMany([{ title: 'Second', url: 'https://example.com/second' }], { strict: true });
        expect(events.at(-1)?.changes).toContainEqual(expect.objectContaining({
            action: 'removed',
            artifactId: first.changes[0]?.artifactId,
            reason: 'eviction',
        }));
    ...
    it('rejects workspace path traversal', async () => {
        ...
        await expect(store.upsertMany([{ title: 'Escape', workspacePath: '../outside.txt' }], {
            strict: true,
    ...
    it('accepts workspace entries whose names start with two dots', async () => {
        ...
        await expect(store.upsertMany([{ title: 'Projected volume', workspacePath: '..data/report.html' }], { strict: true })).resolves.toMatchObject({
            changes: [
                {
                    artifact: {
    ...
    it('keeps first display fields and only enriches missing metadata keys', async () => {
        ...
        expect(Object.hasOwn(repeatedTool.changes[0]?.artifact?.metadata ?? {}, 'toString')).toBe(true);
        expect(repeatedTool.changes[0]?.artifact?.metadata).not.toHaveProperty('hookKey');
    });
    ...
    it('does not count injected workspace hash metadata against the merge limit', async () => {
        ...
        const updated = await store.upsertMany([{ title: 'Budget update', workspacePath }], { strict: true });
    ...
    it('does not merge client metadata into a published tool artifact', async () => {
        ...
        expect(clientUpdate.changes[0]?.artifact?.metadata).not.toHaveProperty('injected');
    });
    ...
    it('persists explicit durable to ephemeral updates as sticky markers', async () => {
        ...
        const artifactId = created.changes[0].artifactId;
        const updated = await store.upsertMany([{ title: 'Durable', url, retention: 'ephemeral' }], { strict: true });
    ...
    it('rejects unsafe display markup in title and description', async () => {
        ...
        await expect(store.upsertMany([{ title: '<img src=x onerror=alert(1)>', url: 'https://example.com' }], { strict: true })).rejects.toMatchObject({ field: 'title' });
        await expect(store.upsertMany([{ title: 'onload=alert(1)', url: 'https://example.com/onload' }], { strict: true })).rejects.toMatchObject({ field: 'title' });
        await expect(store.upsertMany([
            {
        ...
        ], { strict: true })).rejects.toMatchObject({ field: 'description' });
        await expect(store.upsertMany([
            {
                title: '<style>body{background:url(https://example.com/x)}</style>',
                url: 'https://example.com/style',
            },
        ], { strict: true })).rejects.toMatchObject({ field: 'title' });
        await expect(store.upsertMany([
            {
                title: 'Workspace payload',
                workspacePath: '<img src=x onerror=alert(1)>.html',
            },
        ], { strict: true })).rejects.toMatchObject({ field: 'workspacePath' });
        await expect(store.upsertMany([
            {
                title: 'Managed payload',
                managedId: '<script>alert(1)</script>',
            },
        ], { strict: true })).rejects.toMatchObject({ field: 'managedId' });
        for (const managedId of ['../secret', 'folder/item', 'folder\\item']) {
            await expect(store.upsertMany([{ title: 'Managed path', managedId }], {
                strict: true,
        ...
        ], { strict: true })).rejects.toMatchObject({ field: 'metadata' });
        await expect(store.upsertMany([
            {
        ...
        ], { strict: true })).rejects.toMatchObject({ field: 'mimeType' });
    });
    ...
    it('filters prototype metadata keys without changing object prototype', async () => {
        ...
        const metadata = JSON.parse('{"__proto__":null,"constructor":"blocked","prototype":"blocked","safe":"ok"}');
        const created = await store.upsertMany([
            {
                title: 'Link',
        ...
        expect(Object.prototype.hasOwnProperty.call(normalized, 'prototype')).toBe(false);
    });
    ...
    it('rejects existing workspace symlinks that escape the workspace', async () => {
        ...
        try {
            ...
            await expect(store.upsertMany([{ title: 'Escape', workspacePath: 'escape.txt' }], {
                strict: true,
            })).rejects.toMatchObject({ field: 'workspacePath' });
        }
        finally {
    ...
    it('rejects workspace symlinks that resolve to the workspace root', async () => {
        ...
        await expect(store.upsertMany([{ title: 'Root', workspacePath: 'root-link' }], {
            strict: true,
        })).rejects.toMatchObject({ field: 'workspacePath' });
    });
    it('rejects absolute dangling symlinks that point outside the workspace', async () => {
        ...
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-outside-'));
        try {
            await fs.symlink(path.join(outside, 'missing.txt'), path.join(workspace, 'escape.txt'));
            await expect(store.upsertMany([{ title: 'Escape', workspacePath: 'escape.txt' }], {
                strict: true,
            })).rejects.toMatchObject({ field: 'workspacePath' });
        }
    ...
    it('marks dangling symlinks that point inside the workspace as missing', async () => {
        ...
        await fs.symlink('missing.txt', path.join(workspace, 'internal-missing.txt'));
        await expect(store.upsertMany([
            {
    ...
    it('marks a stored workspace artifact missing if it later escapes by symlink', async () => {
        ...
        try {
            ...
            await fs.rm(path.join(workspace, 'report.txt'));
            await fs.symlink(path.join(outside, 'secret.txt'), path.join(workspace, 'report.txt'));
            vi.useFakeTimers();
            vi.setSystemTime(new Date(Date.now() + 6_000));
            const artifact = (await store.list()).artifacts[0];
            expect(artifact).toMatchObject({
            ...
            const snapshot = store.buildSnapshotPayload('2026-07-06T00:00:00.000Z', 1);
    ...
    it('restores workspacePath when a healed artifact is recorded again', async () => {
        ...
        try {
            await fs.writeFile(path.join(workspace, 'report.txt'), 'hello');
            await fs.writeFile(path.join(outside, 'secret.txt'), 'secret');
            const created = await store.upsertMany([{ title: 'Report', workspacePath: 'report.txt' }], { strict: true });
            await fs.rm(path.join(workspace, 'report.txt'));
            await fs.symlink(path.join(outside, 'secret.txt'), path.join(workspace, 'report.txt'));
            vi.useFakeTimers();
            vi.setSystemTime(new Date(Date.now() + 6_000));
            expect((await store.list()).artifacts[0]).not.toHaveProperty('workspacePath');
            await fs.rm(path.join(workspace, 'report.txt'));
            await fs.writeFile(path.join(workspace, 'report.txt'), 'healed');
            const healed = await store.upsertMany([
                { title: 'Report', workspacePath: 'report.txt' },
            ]);
            expect(healed.changes).toContainEqual(expect.objectContaining({
                action: 'updated',
                artifactId: created.changes[0]?.artifactId,
    ...
    it('resets cached workspace realpath after a refresh failure', async () => {
        ...
        try {
            await expect(store.upsertMany([{ title: 'Denied', workspacePath: 'denied.txt' }], {
                strict: true,
    ...
    it('caches the workspace root realpath across artifact refreshes', async () => {
        ...
        try {
            ...
            await store.list();
            expect(realpathSpy.mock.calls.filter(([target]) => target === workspace)).toHaveLength(1);
        }
        finally {
            realpathSpy.mockRestore();
        }
    ...
    it('rejects workspace paths that become symlinks before open', async () => {
        ...
        try {
            await expect(store.upsertMany([{ title: 'Loop', workspacePath: 'loop.txt' }], {
                strict: true,
            })).rejects.toMatchObject({ field: 'workspacePath' });
        }
        finally {
    ...
    it('rejects relative dangling symlinks that point outside the workspace', async () => {
        ...
        await fs.symlink('../outside-missing.txt', path.join(workspace, 'dangling'));
        await expect(store.upsertMany([{ title: 'Dangling', workspacePath: 'dangling' }], {
            strict: true,
        })).rejects.toMatchObject({ field: 'workspacePath' });
    });
    ...
    it('refreshes stale missing candidates before eviction', async () => {
        ...
        expect((await store.list()).artifacts.map((artifact) => artifact.id)).toContain(restored.changes[0]?.artifactId);
    });
    ...
    it('emits one net removed change when an updated artifact is evicted', async () => {
        ...
        expect(overflow.changes.filter((change) => change.artifactId === firstId)).toEqual([
            expect.objectContaining({
                action: 'removed',
                artifactId: firstId,
                reason: 'eviction',
    ...
    it('records durable artifact events through the persistence hook', async () => {
        ...
        const created = await store.upsertMany([{ title: 'Durable', url: 'https://example.com/durable' }], { strict: true });
        expect(created.changes[0]?.artifact).toMatchObject({
            retention: 'restorable',
            restoreState: 'live',
        });
    ...
    it('records periodic snapshots after durable artifact events', async () => {
        ...
        await store.upsertMany([{ title: 'Durable 50', url: 'https://example.com/durable-50' }], { strict: true });
    ...
    it('snapshots the post-delete state for strict durable removals', async () => {
        ...
        expect(snapshots[0]?.artifacts).not.toEqual(expect.arrayContaining([expect.objectContaining({ id: targetId })]));
    });
    ...
    it('rolls back identity-changing ephemeral replacements when tombstone persistence fails', async () => {
        ...
        ], { trustedPublisher: true })).resolves.toMatchObject({
            changes: [],
            warnings: [
                'artifact durable removal not persisted; live changes rolled back',
            ],
    ...
    it('keeps explicit tombstones in periodic snapshots', async () => {
        ...
        const created = await store.upsertMany([{ title: 'Deleted', url: 'https://example.com/deleted' }], { strict: true });
        ...
        expect(snapshots[0]?.artifacts.some((artifact) => artifact.id === deletedId)).toBe(false);
    ...
    it('logs when tombstone LRU eviction drops the oldest id', async () => {
        ...
        try {
            for (let index = 0; index < 501; index++) {
                ...
                const created = await store.upsertMany([{ title: `Deleted ${index}`, url }], { strict: true });
            ...
            expect(logged).toContain(stableSessionArtifactId(sessionId, 'url:https://example.com/tombstone-lru-0'));
        }
        finally {
            stderr.mockRestore();
        }
    ...
    it('keeps live artifacts when rewind restore has no snapshot', async () => {
        ...
        const durable = await store.upsertMany([{ title: 'Durable', url: 'https://example.com/durable-rewind' }], { strict: true });
        ...
        await expect(store.restore(undefined, { preserveLiveEphemeral: true })).resolves.toEqual([]);
        await expect(store.list()).resolves.toMatchObject({
            artifacts: [
                {
    ...
    it('omits orphaned sticky markers after live eviction removes an artifact', async () => {
        ...
        await source.upsertMany([{ title: 'Sticky', url: 'https://example.com/sticky' }], { strict: true });
    ...
    it('applies sticky ephemeral markers while restoring durable artifacts', async () => {
        ...
        await source.upsertMany([{ title: 'Sticky', url: 'https://example.com/sticky-restore' }], { strict: true });
    ...
    it('does not mark restored ephemeral artifacts as sticky without a sticky marker', async () => {
        ...
        expect((await restored.list()).artifacts[0]).not.toHaveProperty('persistenceWarning');
    });
        ...
        expect(result.warningDetails).toEqual([
            {
                ...
                retryable: false,
                message: 'artifact persistence unavailable; durable artifacts kept ephemeral',
            },
        ]);
        expect(result.changes[0]?.artifact).toMatchObject({
            retention: 'ephemeral',
    ...
    it('rolls back strict mutations when persistence fails', async () => {
        ...
        await expect(store.upsertMany([{ title: 'Rollback', url: 'https://example.com/rollback' }], { strict: true })).rejects.toThrow('disk full');
    ...
    it('keeps explicit removal live when tombstone persistence fails', async () => {
        ...
        const created = await store.upsertMany([{ title: 'Sensitive', url: 'https://example.com/sensitive' }], { strict: true });
        await expect(store.remove(created.changes[0].artifactId)).resolves.toMatchObject({
            changes: [],
            warnings: ['artifact removal not persisted; live artifact kept'],
            warningDetails: [
                {
    ...
    it('writes a tombstone when deleting a downgraded durable artifact', async () => {
        ...
        const created = await store.upsertMany([{ title: 'Durable', url: 'https://example.com/downgraded' }], { strict: true });
        failNext = true;
    ...
    it('writes an eviction tombstone when evicting a downgraded durable artifact', async () => {
        ...
        const created = await store.upsertMany([{ title: 'Durable', url: 'https://example.com/downgraded-eviction' }], { strict: true });
        failNext = true;
        await store.upsertMany([
            {
                title: 'Durable',
        ...
        await store.upsertMany([{ title: 'Overflow', url: 'https://example.com/overflow' }], { strict: true });
        expect(events.at(-1)?.changes).toContainEqual(expect.objectContaining({
            action: 'removed',
            artifactId: created.changes[0]?.artifactId,
            reason: 'eviction',
    ...
    it('restores rebuilt durable artifacts as metadata-only restored entries', async () => {
        ...
        await source.upsertMany([{ title: 'Restored', url: 'https://example.com/restored' }], { strict: true });
    ...
    it('restores workspace artifacts with stat-only content checks', async () => {
        ...
        const id = stableSessionArtifactId('s11-restore-workspace-stat-only', `workspace:${workspacePath}`);
        const store = new SessionArtifactStore({
    ...
    it('keeps live artifacts when an empty restore snapshot has warnings', async () => {
        ...
        expect(listed.warningDetails).toEqual([
            ...
            },
            {
                ...
                durability: 'unavailable',
                retryable: true,
                message: 'artifact snapshot restore failed; kept existing live artifacts',
            },
        ]);
    });
    ...
    it('keeps live artifacts when a normalized snapshot has completeness warnings', async () => {
        ...
        await source.upsertMany([{ title: 'Good', url: 'https://example.com/restore-normalized-good' }], { strict: true });
        const good = sourceEvents[0].changes[0].artifact;
        const store = new SessionArtifactStore({
            sessionId: 's11-restore-normalized-warning',
            workspaceCwd: workspace,
    ...
    it('applies rebuilt snapshots when only stale event warnings are present', async () => {
        ...
        await source.upsertMany([{ title: 'Fresh', url: 'https://example.com/restore-stale-fresh' }], { strict: true });
        ...
        const staleWarning = 'skipped stale event sequence 1 at or before snapshot sequence 10';
        const warnings = await store.restore({
            v: 2,
            sessionId: 's11-restore-stale-event-warning',
            sequence: 10,
    ...
    it('prunes over-limit restored artifacts and records eviction tombstones', async () => {
        ...
        await expect(restored.restore(snapshot)).resolves.toContain('restored artifact list pruned to live limit');
        await expect(restored.list()).resolves.toMatchObject({
            artifacts: [{ title: 'Two' }],
        });
        expect(prunedEvents[0]?.changes).toContainEqual(expect.objectContaining({
            action: 'removed',
    ...
    it('downgrades legacy pinned content refs to metadata-only restore', async () => {
        ...
        const created = await source.upsertMany([{ title: 'Legacy pinned', url: 'https://example.com/legacy-pinned' }], { strict: true });
        const artifactId = created.changes[0].artifactId;
        const persisted = {
    ...
    it('restores workspace metadata near the user budget without replacing the persisted hash', async () => {
        ...
        const artifactId = stableSessionArtifactId(sessionId, `workspace:${workspacePath}`);
        const store = new SessionArtifactStore({
            sessionId,
            workspaceCwd: workspace,
        });
...
//# sourceMappingURL=sessionArtifacts.test.js.map