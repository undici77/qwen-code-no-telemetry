...
import { CANCEL_VOTE_SENTINEL, MultiClientPermissionMediator, } from './permissionMediator.js';
import {} from './permission.js';
import {} from './eventBus.js';
import { CancelSentinelCollisionError, InvalidPermissionOptionError, } from './bridgeErrors.js';
...
function makeRecord(overrides = {}) {
    return {
        ...
        originatorClientId: 'originatorClientId' in overrides
            ? overrides.originatorClientId
            : 'client_A',
        allowedOptionIds: overrides.allowedOptionIds ??
...
function makeMediator(policy = 'first-responder', voters = new Set(['client_A', 'client_B', 'client_C'])) {
...
describe('MultiClientPermissionMediator — first-responder', () => {
    ...
    it('resolves on first valid vote and emits permission_resolved with voter clientId as originator (O8)', async () => {
        ...
        const decisionReason = resolvedCall.args[2];
    ...
    it('returns already_resolved on a duplicate vote and re-emits the SSE notification', async () => {
        ...
        const outcome = mediator.vote(makeVote({ clientId: 'client_C', optionId: 'proceed_always' }));
        expect(outcome).toEqual({
    ...
    it('throws InvalidPermissionOptionError when optionId is not in the allow set', () => {
        ...
        const record = makeRecord();
        void mediator.request(record, 5_000);
        expect(() => mediator.vote(makeVote({ optionId: 'proceed_always_forged' }))).toThrow(InvalidPermissionOptionError);
    });
});
describe('MultiClientPermissionMediator — voter cancel sentinel', () => {
    ...
    it('resolves cancelled on cancel sentinel regardless of policy', async () => {
        for (const policy of [
            ...
            const record = makeRecord();
            const promise = mediator.request(record, 5_000);
            const outcome = mediator.vote(makeVote({ optionId: CANCEL_VOTE_SENTINEL }));
            expect(outcome).toEqual({
                kind: 'resolved',
    ...
    it('does NOT validate cancel sentinel against allowedOptionIds', () => {
        ...
        void mediator.request(record, 5_000);
        expect(() => mediator.vote(makeVote({ optionId: CANCEL_VOTE_SENTINEL }))).not.toThrow();
    });
    ...
    it('cancel sentinel resolves under `designated` even when voter is NOT the originator', async () => {
        ...
        const outcome = mediator.vote(makeVote({ clientId: 'client_B', optionId: CANCEL_VOTE_SENTINEL }));
        expect(outcome).toEqual({
            kind: 'resolved',
            resolvedOptionId: CANCEL_VOTE_SENTINEL,
        });
    ...
    it('cancel sentinel resolves under `consensus` even when voter is NOT in votersAtIssue', async () => {
        const { mediator, events } = makeMediator('consensus', 
        ...
        const outcome = mediator.vote(makeVote({
    ...
    it('rejects request() at issue time when allowedOptionIds collides with cancel sentinel', () => {
        ...
        expect(() => mediator.request(record, 5_000)).toThrow(CancelSentinelCollisionError);
...
describe('MultiClientPermissionMediator — timeout', () => {
    ...
    it('resolves cancelled when the timer fires before any vote', async () => {
        ...
        expect(decisionReason.firedAtMs).toBe(1_000_000);
    });
    ...
    it('writes a stderr breadcrumb when the timer fires', async () => {
        ...
        try {
            ...
            vi.advanceTimersByTime(5_000);
            await promise;
            const breadcrumb = writes.find((w) => w.includes('timed out after 5000ms'));
...
describe('MultiClientPermissionMediator — consensus', () => {
    ...
    it('resolves on first option to reach quorum (M=3, default N=2)', async () => {
        const { mediator, events, calls } = makeMediator('consensus', new Set(['client_A', 'client_B', 'client_C']));
    ...
    it('keeps the original vote on idempotent re-vote (no tally change, no partial_vote re-emit)', async () => {
        const { mediator, events } = makeMediator('consensus', new Set(['client_A', 'client_B', 'client_C']));
        const record = makeRecord();
        const promise = mediator.request(record, 5_000);
        mediator.vote(makeVote({ clientId: 'client_A', optionId: 'proceed_once' }));
        expect(events).toHaveLength(1);
        const v2 = mediator.vote(makeVote({ clientId: 'client_A', optionId: 'proceed_always' }));
    ...
    it('records the original optionId in audit on idempotent re-vote (3271041464)', async () => {
        const { mediator, calls } = makeMediator('consensus', new Set(['client_A', 'client_B', 'client_C']));
        const record = makeRecord();
        const promise = mediator.request(record, 5_000);
        ...
        mediator.vote(makeVote({ clientId: 'client_A', optionId: 'proceed_always' }));
        ...
        expect(votedCalls[0].args[1].optionId).toBe('proceed_once');
        ...
        expect(votedCalls[1].args[1].optionId).toBe('proceed_once');
    });
    ...
    it('rejects voter not in votersAtIssue snapshot', () => {
        const { mediator, events } = makeMediator('consensus', new Set(['client_A', 'client_B']));
        const record = makeRecord();
        void mediator.request(record, 5_000);
    ...
    it('writes stderr breadcrumbs for all 3 forbidden-vote paths', () => {
        ...
        try {
            ...
            {
                const { mediator } = makeMediator('consensus', new Set(['client_A', 'client_B']));
                void mediator.request(makeRecord(), 5_000);
            ...
            // 3. local-only — non-loopback voter rejected.
            {
                ...
                void mediator.request(makeRecord(), 5_000);
                mediator.vote(makeVote({ clientId: 'client_remote', fromLoopback: false }));
                mediator.forgetSession('sess-1');
            }
    ...
    it('M=4 N=3 split 2-2 never resolves and times out', async () => {
        ...
        const { mediator } = makeMediator('consensus', new Set(['client_A', 'client_B', 'client_C', 'client_D']));
        const record = makeRecord();
        const promise = mediator.request(record, 5_000);
        mediator.vote(makeVote({ clientId: 'client_A', optionId: 'proceed_once' }));
        mediator.vote(makeVote({ clientId: 'client_B', optionId: 'proceed_once' }));
        mediator.vote(makeVote({ clientId: 'client_C', optionId: 'proceed_always' }));
        const v4 = mediator.vote(makeVote({ clientId: 'client_D', optionId: 'proceed_always' }));
    ...
    it('property-style: enumerate vote interleavings for M=3 N=2 — first option to N wins', async () => {
        ...
        for (let assignmentMask = 0; assignmentMask < 8; assignmentMask++) {
            const assignments = voters.map((_, idx) => ((assignmentMask >> idx) & 1) === 1 ? options[0] : options[1]);
            const orderings = [
                [0, 1, 2],
                [0, 2, 1],
                [1, 0, 2],
            ...
            for (const order of orderings) {
                ...
                const mediatorWinner = recordedOutcomes.find((o) => o.kind === 'resolved');
                if (referenceWinner !== null) {
...
describe('MultiClientPermissionMediator — N2 cleanup ordering', () => {
    ...
    it('resolves the Promise even when emit throws', async () => {
        ...
        expect(mediator.peekSessionFor('req-1')).toBe('sess-1');
        const dupOutcome = mediator.vote(makeVote());
...
//# sourceMappingURL=permissionMediator.test.js.map