 ...
 */
import type { ApprovalMode, GoalSnapshotV2, SessionGroupPresetColor } from '@qwen-code/qwen-code-core';
import type { CancelNotification, LoadSessionResponse, PromptRequest, PromptResponse, RequestPermissionResponse, ResumeSessionResponse, SetSessionModelRequest, SetSessionModelResponse, SetSessionConfigOptionRequest, SetSessionConfigOptionResponse, SessionUpdate } from '@agentclientprotocol/sdk';
import type { BridgeEvent, SessionReplaySnapshot, SubscribeOptions } from './eventBus.js';
import type { PermissionPolicy } from './permission.js';
import type { SessionArtifactInput, SessionArtifactMutationResult, SessionArtifactsEnvelope } from './sessionArtifacts.js';
import type { ServeSessionContextStatus, ServeSessionHooksStatus, ServeSessionLspStatus, ServeSessionSupportedCommandsStatus, ServeSessionTasksStatus, ServeWorkspaceExtensionsStatus, ServeWorkspaceHooksStatus, ServeWorkspaceMcpToolsStatus, ServeWorkspaceMcpResourcesStatus, ServeWorkspaceToolsStatus, ServeSessionContextUsageStatus, ServeSessionStatsStatus } from './status.js';
...
export declare const LOAD_REPLAY_MODE_META_KEY = "qwen.session.loadReplayMode";
export declare const LOAD_REPLAY_META_KEY = "qwen.session.loadReplay";
export declare const LOAD_REPLAY_PAGE_SIZE_META_KEY = "qwen.session.loadReplayPageSize";
export declare const LOAD_REPLAY_HIDE_INHERITED_META_KEY = "qwen.session.loadReplayHideInherited";
export declare const LOAD_REPLAY_BULK_MODE = "bulk";
export declare const LOAD_REPLAY_VERSION: 1;
export declare const REQUESTED_SESSION_ID_META_KEY = "qwen-code/sessionId";
...
export declare const ACTIVE_WORK_HEARTBEAT_META_KEY = "qwen.daemon.activeWorkHeartbeat";
export declare const ACTIVE_WORK_HEARTBEAT_VERSION: 1;
...
export declare const ACTIVE_WORK_HEARTBEAT_INTERVAL_MS = 15000;
export declare const ACTIVE_WORK_HEARTBEAT_MIN_INTERVAL_MS = 5000;
export declare const ACTIVE_WORK_HEARTBEAT_MAX_INTERVAL_MS = 60000;
/** A channel's cached snapshot goes stale after this many report intervals. */
export declare const ACTIVE_WORK_STALE_INTERVALS = 3;
export declare const ACTIVE_WORK_NOTIFICATION_METHOD = "qwen/notify/channel/active-work";
export declare const ACTIVE_WORK_CLOSE_IF_UNHELD_PARAM = "onlyIfUnheld";
 ...
 *  snapshot to settle. */
export declare const ACTIVE_WORK_CLOSE_TIMEOUT_MS = 10000;
...
export declare const ACTIVE_WORK_MAX_SNAPSHOT_SESSIONS = 1024;
export declare const ACTIVE_WORK_MAX_SESSION_HOLDS = 1024;
export declare const WORKTREE_MCP_DEFER_META_KEY = "qwen.session.deferMcpDiscovery";
/**
...
export declare const ACTIVE_WORK_HOLD_CATEGORIES: readonly ActiveWorkHoldCategory[];
 ...
 */
export declare function clampActiveWorkIntervalMs(raw: unknown): number;
/**
 ...
 */
export declare function gradeActiveWorkCoverage(totals: {
    total: number;
    covered: number;
...
export type BridgeAutoMemoryTopic = 'user' | 'feedback' | 'project' | 'reference';
...
export type BridgePendingInteraction = BridgePendingPermissionInteraction | BridgePendingUserQuestionInteraction;
...
export declare const DAEMON_MODEL_PROMPT_META_KEY = "qwen.daemon.modelPrompt";
export declare const MAX_TRUSTED_MODEL_PROMPT_CHARS: number;
export declare function isValidTrustedModelPrompt(value: unknown): value is string;
export declare const DAEMON_CHANNEL_DELIVERY_META_KEY = "qwen.daemon.channelDelivery";
export declare const DAEMON_PROMPT_DISPLAY_TEXT_META_KEY = "qwen.daemon.promptDisplayText";
/**
 ...
 */
export declare const MID_TURN_QUEUE_DRAIN_METHOD = "craft/drainMidTurnQueue";
/**
 * Cap on each per-session mid-turn reconciliation ring.
 */
export declare const MID_TURN_RECONCILIATION_RING_SIZE = 200;
/**
 ...
 */
export declare const TODO_STOP_GUARD_CONTINUATION_CLAIM_METHOD = "craft/claimTodoStopGuardContinuation";
 ...
 */
export declare const TODO_STOP_GUARD_QUEUE_RELEASE_METHOD = "craft/todoStopGuardQueueReleased";
export interface TodoStopGuardQueueReleasedRequest {
    sessionId: string;
    promptId: string;
}
/** Parent-to-agent request that acknowledges prompt cancellation handling. */
export declare const PROMPT_CANCEL_METHOD = "craft/cancelPendingPrompt";
/**
...
export type BridgeGenerationNotificationEvent = Exclude<BridgeGenerationStreamEvent, {
    type: 'done';
}>;
...
export type BridgeWorkspaceGenerationNotificationEvent = Exclude<BridgeWorkspaceGenerationStreamEvent, {
    type: 'done';
}>;
...
export interface AcpSessionBridge {
     ...
     */
    setLiveScreenContextCaptureHandler?(handler: import('./bridgeOptions.js').LiveScreenContextCaptureHandler | undefined): void;
    /** Installs the daemon-owned handler for the five Codex-parity Live task tools. */
    setLiveTaskToolRequestHandler?(handler: import('./bridgeOptions.js').LiveTaskToolRequestHandler | undefined): void;
    /** Installs the daemon-owned handler for the backend-only Live speech tool. */
    setLiveSpeakToUserHandler?(handler: import('./bridgeOptions.js').LiveSpeakToUserHandler | undefined): void;
    /**
     ...
     * this daemon.
     */
    resumeSession(req: BridgeRestoreSessionRequest): Promise<BridgeRestoredSession>;
    /**
     ...
     */
    branchSession(sessionId: string, req: BridgeBranchSessionRequest, context?: BridgeClientRequestContext): Promise<BridgeBranchedSession>;
    /** Create a persisted side task with a snapshot of the parent's context. */
    createSideTaskSession(sessionId: string, req: BridgeSideTaskSessionRequest, context?: BridgeClientRequestContext): Promise<BridgeSideTaskSession>;
    /**
     ...
     */
    changeSessionCwd(sessionId: string, req: ChangeSessionCwdRequest, context?: BridgeClientRequestContext): Promise<ChangeSessionCwdResult>;
    /**
     ...
     */
    setSessionWorktree(sessionId: string, worktree: {
        slug: string;
        path: string;
        branch: string;
    }): void;
     ...
     */
    sendPrompt(sessionId: string, req: PromptRequest, signal?: AbortSignal, context?: BridgeClientRequestContext): Promise<PromptResponse>;
    /**
     ...
     */
    getPendingPrompts(sessionId: string, context?: BridgeClientRequestContext): readonly PendingPromptSummary[];
    /**
     ...
     * unknown session ids.
     */
    removePendingPrompt(sessionId: string, promptId: string, context?: BridgeClientRequestContext): {
        removed: boolean;
    };
    /**
     ...
     */
    cancelSession(sessionId: string, req?: CancelNotification, context?: BridgeClientRequestContext): Promise<void>;
    /**
     * Subscribe to the session's event stream. Throws
     * `SessionNotFoundError` when the id is unknown.
     */
    subscribeEvents(sessionId: string, opts?: SubscribeOptions & {
     ...
     */
    getSessionReplaySnapshot(sessionId: string): SessionReplaySnapshot | undefined;
    /**
     ...
     */
    closeSession(sessionId: string, context?: BridgeClientRequestContext, opts?: CloseSessionOpts): Promise<void>;
    /**
     * Update mutable session metadata. Currently supports `displayName` only.
     * Throws `SessionNotFoundError` for unknown ids.
     */
    updateSessionMetadata(sessionId: string, metadata: SessionMetadataUpdate, context?: BridgeClientRequestContext): SessionMetadataUpdate;
    /**
     * List the structured artifacts registered for a live session. Throws
     * `SessionNotFoundError` when the id is unknown.
     */
    getSessionArtifacts(sessionId: string, context?: BridgeClientRequestContext): Promise<SessionArtifactsEnvelope>;
    /**
     ...
     */
    addSessionArtifact(sessionId: string, artifact: SessionArtifactInput, context?: BridgeClientRequestContext): Promise<SessionArtifactMutationResult>;
    /**
     ...
     */
    removeSessionArtifact(sessionId: string, artifactId: string, context?: BridgeClientRequestContext): Promise<SessionArtifactMutationResult>;
    /**
     * Cast a vote on a pending `permission_request` (first-responder wins).
     */
    respondToPermission(requestId: string, response: RequestPermissionResponse, context?: BridgeClientRequestContext): boolean;
    /**
     * Cast a vote scoped to an explicit session route.
     */
    respondToSessionPermission(sessionId: string, requestId: string, response: RequestPermissionResponse, context?: BridgeClientRequestContext): boolean;
    /**
     ...
     */
    recordHeartbeat(sessionId: string, context?: BridgeClientRequestContext): BridgeHeartbeatResult;
    /**
     ...
     */
    invokeWorkspaceCommand<T>(method: string, params?: Record<string, unknown>, opts?: {
        timeoutMs?: number;
    }): Promise<T>;
    /**
     ...
     */
    runWorkspaceMemoryRemember(request: BridgeWorkspaceMemoryRememberRequest): Promise<BridgeWorkspaceMemoryRememberResult>;
    /**
     * Run a hidden workspace-level managed-memory forget task. This
     * ensures the ACP child exists but must not create/load/resume an ACP
     * session or touch the per-session prompt queue.
     */
    runWorkspaceMemoryForget(request: BridgeWorkspaceMemoryForgetRequest): Promise<BridgeWorkspaceMemoryForgetResult>;
    /**
     ...
     */
    initializeWorkspaceMcp(): Promise<{
        accepted: boolean;
    }>;
     ...
     */
    getWorkspaceMcpToolsStatus(serverName: string): Promise<ServeWorkspaceMcpToolsStatus>;
    /**
     ...
     */
    getWorkspaceMcpResourcesStatus(serverName: string): Promise<ServeWorkspaceMcpResourcesStatus>;
    /**
    ...
    getSessionContextStatus(sessionId: string): Promise<ServeSessionContextStatus>;
    /** Read structured context-window usage for a live session. */
    getSessionContextUsageStatus(sessionId: string, opts?: {
        detail?: boolean;
    }): Promise<ServeSessionContextUsageStatus>;
    /** Read slash-command/skill command availability for a live session. */
    getSessionSupportedCommandsStatus(sessionId: string): Promise<ServeSessionSupportedCommandsStatus>;
     ...
     */
    getSessionTranscriptPage(req: BridgeSessionTranscriptPageRequest): Promise<BridgeSessionTranscriptPage>;
    /** Cancel a background task in a live session. */
    cancelSessionTask(sessionId: string, taskId: string, taskKind: 'agent' | 'shell' | 'monitor'): Promise<{
        cancelled: boolean;
    }>;
    /** Clear an active goal in a live session without cancelling the running prompt. */
    clearSessionGoal(sessionId: string): Promise<{
        cleared: boolean;
        condition?: string;
    }>;
    /**
     ...
     */
    continueSession(sessionId: string, context?: BridgeClientRequestContext): Promise<{
        accepted: boolean;
     ...
     */
    refreshExtensionsForAllSessions(data?: Omit<BridgeExtensionsChangedData, 'refreshed' | 'failed'>): Promise<{
        refreshed: number;
        failed: number;
    }>;
     ...
     */
    setSessionModel(sessionId: string, req: SetSessionModelRequest, context?: BridgeClientRequestContext): Promise<SetSessionModelResponse>;
    /** Change one advertised ACP configuration option for a live session. */
    setSessionConfigOption(sessionId: string, req: SetSessionConfigOptionRequest): Promise<SetSessionConfigOptionResponse>;
    /**
     ...
     */
    setSessionLanguage(sessionId: string, params: {
        language: string;
        syncOutputLanguage: boolean;
    }, context?: BridgeClientRequestContext): Promise<{
        language: string;
        outputLanguage: string | null;
        refreshed: boolean;
    }>;
    /** Apply Codex's realtime-active world-state transition to one session. */
    setSessionLiveConversationActive(sessionId: string, active: boolean): Promise<void>;
    /** Persist Realtime-owned dialogue without starting a backend model turn. */
    appendSessionLiveTranscript(sessionId: string, entries: ReadonlyArray<{
        role: 'user' | 'assistant';
        text: string;
     ...
     */
    setSessionApprovalMode(sessionId: string, mode: ApprovalMode, opts: {
        persist: boolean;
    }, context?: BridgeClientRequestContext): Promise<{
        sessionId: string;
        mode: ApprovalMode;
     ...
     * down) throw as usual.
     */
    generateSessionRecap(sessionId: string, context?: BridgeClientRequestContext): Promise<{
        sessionId: string;
        recap: string | null;
    }>;
    /**
     ...
     */
    generateSessionContent?(sessionId: string, prompt: string, signal: AbortSignal, context?: BridgeClientRequestContext): AsyncIterable<BridgeGenerationStreamEvent>;
    /**
     ...
     */
    generateSessionBtw(sessionId: string, question: string, signal?: AbortSignal, context?: BridgeClientRequestContext): Promise<{
        sessionId: string;
        answer: string | null;
    }>;
    /**
     ...
     */
    launchSessionForkAgent(sessionId: string, directive: string, context?: BridgeClientRequestContext): Promise<BridgeForkAgentResult>;
    /**
     ...
     */
    enqueueMidTurnMessage(sessionId: string, message: string, context?: BridgeClientRequestContext, messageId?: string, options?: {
        queueOnly?: boolean;
        onSettledWithoutDrain?: () => void;
    }): {
        accepted: boolean;
    ...
    removeMidTurnMessage(sessionId: string, messageId: string, context?: BridgeClientRequestContext): {
        removed: boolean;
    };
    /**
     ...
     */
    enqueueBackgroundNotification(sessionId: string, notification: BridgeBackgroundNotification): Promise<{
        sessionId: string;
        accepted: boolean;
    }>;
    /**
     ...
     */
    getMidTurnMessages(sessionId: string, context?: BridgeClientRequestContext): BridgeMidTurnMessagesSnapshot;
    /**
     ...
     */
    executeShellCommand(sessionId: string, command: string, signal?: AbortSignal, context?: BridgeClientRequestContext): Promise<ShellCommandResult>;
    /**
     ...
     */
    rewindSession(sessionId: string, req: RewindRequest, context?: BridgeClientRequestContext): Promise<RewindResponse>;
    /**
     ...
     */
    addRuntimeMcpServer(name: string, config: Record<string, unknown>, originatorClientId?: string): Promise<RuntimeMcpServerAddResult>;
    /**
     ...
     */
    removeRuntimeMcpServer(name: string, originatorClientId?: string): Promise<RuntimeMcpServerRemoveResult>;
    /**
     ...
     * event.
     */
    addSessionRuntimeMcpServer(sessionId: string, name: string, config: Record<string, unknown>, originatorClientId?: string): Promise<RuntimeMcpServerAddResult>;
    /** Remove a runtime MCP server from one live session only. */
    removeSessionRuntimeMcpServer(sessionId: string, name: string, originatorClientId?: string): Promise<RuntimeMcpServerRemoveResult>;
    manageMcpServer(serverName: string, action: 'approve' | 'enable' | 'disable' | 'authenticate' | 'clear-auth', originatorClientId: string | undefined): Promise<{
        serverName: string;
    ...
    generateWorkspaceAgent(description: string, originatorClientId: string | undefined): Promise<{
        name: string;
        description: string;
        systemPrompt: string;
    }>;
    /** Run stateless, tool-free generation in the resolved workspace runtime. */
    generateWorkspaceContent?(prompt: string, signal: AbortSignal, originatorClientId: string | undefined): AsyncIterable<BridgeWorkspaceGenerationStreamEvent>;
    /**
     ...
     */
    killSession(sessionId: string, opts?: {
