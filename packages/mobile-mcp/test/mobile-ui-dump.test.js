...
(0, test_1.test)('mobile_ui_dump preserves absolute bounds from UIAutomator', async () => {
    const originalGetConnectedDevices = android_1.AndroidDeviceManager.prototype.getConnectedDevices;
    const originalDumpUiHierarchy = android_1.AndroidRobot.prototype.dumpUiHierarchy;
    const originalCoordinateSpace = process.env.MOBILE_MCP_COORDINATE_SPACE;
    const xml = '<hierarchy><node text="OK" bounds="[12,34][1080,2160]"/></hierarchy>';
    const outputDirectory = process.platform === 'darwin' ? '/private/tmp' : node_os_1.default.tmpdir();
    const outputPath = node_path_1.default.join(outputDirectory, `mobile-ui-dump-${node_crypto_1.default.randomUUID()}.xml`);
    ...
    try {
        ...
        (0, test_1.expect)(result.content).toContainEqual({ type: 'text', text: xml });
        await client.callTool({
            name: 'mobile_ui_dump',
            arguments: { device: 'test-device', output_path: outputPath },
        });
        (0, test_1.expect)(node_fs_1.default.readFileSync(outputPath, 'utf-8')).toBe(xml);
    }
    finally {
        ...
        android_1.AndroidRobot.prototype.dumpUiHierarchy = originalDumpUiHierarchy;
