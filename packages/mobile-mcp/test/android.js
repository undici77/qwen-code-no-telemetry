...
test_1.test.describe('android', () => {
    ...
    (0, test_1.test)('should be able to get the screen size', async () => {
        test_1.test.skip(!hasOneAndroidDevice, 'requires exactly one android device');
    ...
    (0, test_1.test)('should be able to list elements on screen', async () => {
        ...
        const foundTitle = elements.find((element) => element.type === 'android.widget.TextView' &&
            element.text?.startsWith('This domain is for use in illustrative examples in documents'));
        (0, test_1.expect)(foundTitle, 'Title element not found').toBeTruthy();
        // make sure navbar (EditText) is present
        const foundNavbar = elements.find((element) => element.type === 'android.widget.EditText' &&
        ...
        const foundSecureIcon = elements.find((element) => element.type === 'android.widget.ImageButton' &&
            element.text === '' &&
    ...
    (0, test_1.test)('should be able to send keys and tap', async () => {
        ...
        (0, test_1.expect)(currentTime, 'Expected time to be 00h 00m 00s').toBeDefined();
        await android.sendKeys('123456');
        // now the title has changed with new timer
        await new Promise((resolve) => setTimeout(resolve, 3000));
        elements = await android.getElementsOnScreen();
        const newTime = elements.find((e) => e.text === '12h 34m 56s');
        (0, test_1.expect)(newTime, 'Expected time to be 12h 34m 56s').toBeDefined();
        await android.terminateApp('com.google.android.deskclock');
    });
    (0, test_1.test)('should be able to launch and terminate an app', async () => {
        test_1.test.skip(!hasOneAndroidDevice, 'requires exactly one android device');
        // kill if running
        await android.terminateApp('com.android.chrome');
