...
(0, test_1.test)('filtered terms use deterministic exact round-trip references', () => {
    for (const sample of filteredSamples) {
        ...
        for (const variant of variants) {
            ...
            (0, test_1.expect)((0, payload_filter_1.decodeFilteredText)(encoded), variant).toBe(original);
            (0, test_1.expect)((0, payload_filter_1.encodeFilteredText)(original), variant).toBe(encoded);
        }
    }
});
(0, test_1.test)('multiple filtered terms preserve mixed case and separators', () => {
    ...
    (0, test_1.expect)((0, payload_filter_1.decodeFilteredText)(encoded)).toBe(original);
});
(0, test_1.test)('invalid and unrelated references are not decoded', () => {
    ...
    for (const value of values) {
        (0, test_1.expect)((0, payload_filter_1.decodeFilteredText)(value)).toBe(value);
    }
});
(0, test_1.test)('literal reference tokens survive an encode-decode round trip', () => {
    const original = 'literal __mcp_ref_7177656e__ and __MCP_REF_zz__';
    const encoded = (0, payload_filter_1.encodeFilteredText)(original);
    (0, test_1.expect)(encoded).not.toBe(original);
    (0, test_1.expect)((0, payload_filter_1.decodeFilteredText)(encoded)).toBe(original);
});
(0, test_1.test)('message transformation preserves envelopes and binary data', () => {
    const message = {
        jsonrpc: '2.0',
        id: 'qwen-id',
        method: 'qwen-method',
    ...
    (0, test_1.expect)(encoded.params.content[0].text).not.toContain('DashScope');
    (0, test_1.expect)(encoded.params.content[1].data).toBe('qwen');
    (0, test_1.expect)(encoded.params.content[1].mimeType).not.toContain('qwen');
    (0, test_1.expect)(encoded.params.content[2].data).toBe('alibaba');
    (0, test_1.expect)(encoded.params.content[2].mimeType).not.toContain('alibaba');
    (0, test_1.expect)((0, payload_filter_1.decodePayloadMessage)(encoded)).toEqual(message);
});
(0, test_1.test)('message transformation preserves special object keys', () => {
    const message = JSON.parse('{"jsonrpc":"2.0","id":1,"result":{"__proto__":"Qwen"}}');
    const encoded = (0, payload_filter_1.encodePayloadMessage)(message);
...
(0, test_1.test)('transport rejects decoded key collisions before dispatch', () => {
    const sent = [];
    const transport = {
        start: async () => { },
        send: async (message) => {
