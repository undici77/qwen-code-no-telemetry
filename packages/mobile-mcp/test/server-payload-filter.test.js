...
(0, test_1.test)(
  'MCP boundary preserves payloads when filtering is disabled',
  async () => {
    ...
    try {
      ...
      (0, test_1.expect)(probe?.description).toBe(
        'Qwen talks to Alibaba through Dash-Scope',
      );
...
(0, test_1.test)(
  'MCP boundary filters lists, successes, errors, and preserves round trips',
  async () => {
    ...
    const originalGetScreenSize =
      android_1.AndroidRobot.prototype.getScreenSize;
    ...
    try {
      ...
      (0, test_1.expect)(success.structuredContent).not.toHaveProperty(
        'Alibaba-key',
      );
      ...
      (0, test_1.expect)(apps.content).toContainEqual({
        type: 'text',
        text: test_1.expect.stringContaining(
          (0, payload_filter_1.encodeFilteredText)(packageName),
        ),
      });
      ...
      const actionableFailure = await client.callTool({
        ...
        arguments: {
          device: 'test-device',
          packageName: (0, payload_filter_1.encodeFilteredText)(
            'com.alibaba.failure',
          ),
        },
      });
