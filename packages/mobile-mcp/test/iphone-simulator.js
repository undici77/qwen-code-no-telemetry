...
test_1.test.describe('iphone-simulator', () => {
    ...
    const device = new mobile_device_1.MobileDevice(bootedSimulators?.[0]?.id || '');
    ...
    (0, test_1.test)('should be able to swipe', async () => {
        ...
        (0, test_1.expect)(elements1.findIndex((e) => e.name === 'com.apple.settings.general')).not.toBe(-1);
        ...
        (0, test_1.expect)(elements2.findIndex((e) => e.name === 'com.apple.settings.general')).toBe(-1);
        // swipe down
        await device.swipe('down');
        // make sure "General" is visible again
        const elements3 = await device.getElementsOnScreen();
        (0, test_1.expect)(elements3.findIndex((e) => e.name === 'com.apple.settings.general')).not.toBe(-1);
    });
    (0, test_1.test)('should be able to send keys and press enter', async () => {
        ...
        (0, test_1.expect)(newElement, 'should have found New Reminder element').toBeDefined();
        ...
        const elements2 = await device.getElementsOnScreen();
        (0, test_1.expect)(elements2.findIndex((e) => e.value === random1)).not.toBe(-1);
        (0, test_1.expect)(elements2.findIndex((e) => e.value === random2)).not.toBe(-1);
    });
    (0, test_1.test)('should be able to get the screen size', async () => {
        ...
        (0, test_1.expect)(Object.keys(screenSize).length, 'screenSize should have exactly 3 properties').toBe(3);
    });
    (0, test_1.test)('should be able to get screenshot', async () => {
        ...
        (0, test_1.expect)(Math.ceil(pngSize.width / screenSize.scale)).toBe(screenSize.width);
        (0, test_1.expect)(Math.ceil(pngSize.height / screenSize.scale)).toBe(screenSize.height);
    });
    (0, test_1.test)('should be able to open url', async () => {
        ...
        const addressBar = elements.find((element) => element.type === 'TextField' &&
    ...
    (0, test_1.test)('should be able to get elements on screen', async () => {
        ...
        (0, test_1.expect)(element, 'should have News app in home screen').toBeDefined();
    });
    ...
    /*
      ...
      */
    (0, test_1.test)('should throw an error if button is not supported', async () => {
        test_1.test.skip(!hasOneSimulator, 'requires a booted ios simulator');
        await (0, test_1.expect)(device.pressButton('NOT_A_BUTTON')).rejects.toThrow('unsupported button: NOT_A_BUTTON');
    });
});
