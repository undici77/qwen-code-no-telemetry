...
exports.coordinateScale = coordinateScale;
exports.normToPx = normToPx;
exports.cacheScreenSize = cacheScreenSize;
exports.invalidateScreenSize = invalidateScreenSize;
exports.getCachedScreenSize = getCachedScreenSize;
exports.denormalizeArgs = denormalizeArgs;
exports.hasCoordFields = hasCoordFields;
exports.ingestScreenSizeFromResult = ingestScreenSizeFromResult;
exports.rewriteDescription = rewriteDescription;
exports.coordParamDesc = coordParamDesc;
...
// ── Scalar conversion ────────────────────────────────────────────────────────
function normToPx(norm, dim, scale) {
    return Math.round((norm / scale) * dim);
}
const INPUT_COORD_FIELDS = {
...
const screenSizeCache = new Map();
function cacheScreenSize(deviceId, width, height) {
    screenSizeCache.set(deviceId, { width, height });
}
function invalidateScreenSize(deviceId) {
    screenSizeCache.delete(deviceId);
}
function getCachedScreenSize(deviceId) {
    return screenSizeCache.get(deviceId);
}
// ── Input: denormalize 0–scale → pixels/points ──────────────────────────────
function denormalizeArgs(toolName, args, screenWidth, screenHeight) {
    ...
    for (const { field, isX } of fields) {
        if (typeof args[field] === 'number') {
            if (args[field] < 0 || args[field] > scale) {
                throw new robot_1.ActionableError(`Coordinate '${field}' value ${args[field]} is out of the normalized range [0, ${scale}]. ` +
    ...
    if (toolName === 'mobile_swipe_on_screen' &&
        typeof args.distance === 'number') {
        if (args.distance < 0 || args.distance > scale) {
            throw new robot_1.ActionableError(`Swipe 'distance' value ${args.distance} is out of the normalized range [0, ${scale}]. ` +
...
function rewriteDescription(description) {
    const scale = coordinateScale();
    return description.replace(/\bin pixels\b/g, `in 0-${scale} normalized coordinates`);
}
function coordParamDesc(baseDesc) {
    if (!isNormalized())
        return baseDesc;
    return rewriteDescription(baseDesc);
}
//# sourceMappingURL=coord-norm.js.map