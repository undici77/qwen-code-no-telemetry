...
import { clearBudgetStop, expectedAdmissionSeconds, readRoundStamps, reverseAuditBudgetExhausted, reverseAuditBudgetMessage, roundCapStopEntry, stampRound, verifyBudgetExhausted, verifyBudgetMessage, writeBudgetStop, writeRoundCapStop, } from './lib/deadline.js';
import { READ_FILE_CHAR_CAP, chunkIdsProblem, } from './lib/diff-plan.js';
import { recordPrompt, writeBrief, writeFindingsFile, } from './lib/prompt-record.js';
import { scheduleReverseAuditRound, } from './lib/retirement.js';
import { BRIEFS, isRepositoryContextRoleId, MODELED_SYSTEM_EXECUTION_LENS, } from './lib/agent-briefs.js';
...
import { requiredAgents, reviewMode, } from './lib/roster.js';
/**
 ...
 */
export function buildChunkAgentPrompt(report, id, rules) {
    ...
    parts.push('', 'You may also `read_file` the **full source files** above from the worktree whenever a ' +
        ...
        'from its first screenful.', '', '## What to review', '', 'For your territory only, you own every dimension: line-by-line correctness, the ' +
        ...
        'not conclude a deletion is unreplaced merely because its replacement is not in your range.', '', FINDING_FORMAT, '', SEVERITY, '', EXCLUSIONS);
 ...
 */
export function buildWholeDiffBlock(report, rules) {
...
export function buildRoleBrief(report, role, opts = {}) {
    ...
    if (role === '0') {
        ...
        const ctx = opts.planPath
            ...
            : null;
        parts.push('', `**This PR:** #${pr} of \`${repo}\`. Use exactly that number and repo — a bare ` +
    ...
    if (role === '7') {
        ...
        if (buildTree && opts.planPath) {
            ...
            parts.push('', '**Build and test what the diff changed.** Give this one call a long tool ' +
                ...
                'command exists to prevent, one level up). Invoke it with `timeout: 600000`:', '', '```bash', 
            ...
            `"\${QWEN_CODE_CLI:-qwen}" review build-test \\`, `  --plan ${resolve(opts.planPath)} \\`, `  --worktree ${resolve(buildTree)} \\`, `  --out ${resolve(dirname(opts.planPath), outName)}`, '```');
        }
        if (typeof base === 'string' && base && pr !== undefined && opts.planPath) {
            ...
            parts.push('', '**Then run the test-efficacy probe.** A green suite says the tests pass. It does ' +
                ...
                'that ceiling:', '', '```bash', `"\${QWEN_CODE_CLI:-qwen}" review test-efficacy ${resolve(opts.planPath)} \\`, `  --worktree ${typeof wt === 'string' ? resolve(wt) : '<worktree>'} \\`, `  --base ${base} \\`, `  --out ${resolve(dirname(opts.planPath), `qwen-review-pr-${pr}-efficacy.json`)}`, '```', '', 'Read its `findings[]`. `kind: "unreachable"` is a test the project\'s test command ' +
...
function buildLaunch(report, planPath, spec, rules) {
    ...
    const id = spec.chunk;
    const key = `chunk-${id}`;
    const briefFile = writeBrief(planPath, key, buildChunkAgentPrompt(report, id, rules));
...
function runRoster(report, planPath, rules) {
    ...
    const blocks = roster.map((req, i) => {
        const { key, prompt } = buildLaunch(report, planPath, req.role === 'chunk'
            ? { chunk: req.chunk }
            : { role: req.role, file: req.file }, rules);
...
function runAllChunks(report, planPath, role, findingsContent, rules, round) {
    ...
    if (role === 'reverse-audit' &&
        ...
        try {
            schedule = scheduleReverseAuditRound(planPath, chunks.map((c) => c.id), round, process.env, typeof report.diffPathAbsolute === 'string'
    ...
    if (role === 'reverse-audit' &&
        !admitReverseAuditRound(planPath, round, reverseAuditRoundCap(report.budget), chunks.length)) {
        return;
    }
    ...
    const blocks = dueChunks.map((c, i) => {
        ...
        const { prompt } = buildLaunch(report, planPath, { role, chunk: c.id, key, round }, rules);
...
function runAgentPrompt(args) {
    ...
    if (args.role === 'reverse-audit' &&
        !hasChunk &&
        !args.allChunks &&
        !admitReverseAuditRound(args.plan, args.round, reverseAuditRoundCap(report.budget), 1)) {
        return;
    }
    ...
    if (args.role === 'reverse-audit' &&
        ...
        if (args.round !== undefined) {
            let schedule = null;
            try {
                schedule = scheduleReverseAuditRound(args.plan, planChunkIds, args.round, process.env, typeof report.diffPathAbsolute === 'string'
                    ? report.diffPathAbsolute
                    : undefined);
            }
            catch {
        ...
        if (!admitReverseAuditRound(args.plan, args.round, reverseAuditRoundCap(report.budget), planChunkIds.length))
            return;
    }
    if (args.allChunks && args.role && findingsContent !== undefined) {
        runAllChunks(report, args.plan, args.role, findingsContent, rules, args.round);
        return;
    }
    let prompt;
    let key;
