...
import { REVIEW_FOOTER_RE, footerVersion, reviewFooter, } from './lib/review-footer.js';
...
function normalizeInlineComments(comments, modelId, cliVersion) {
 ...
 * disagree with it.
 */
function compose(payload, cliVersion) {
...
function inconsistencies(payload, event) {
    const problems = [];
    const comments = payload.comments ?? [];
    if (!EVENTS.has(event)) {
...
export function runSubmit(args, cliVersion = 'unknown') {
    setGhHost(args.host);
    ...
    payload = {
        ...payload,
        comments: normalizeInlineComments(payload.comments ?? [], payload.state?.modelId, cliVersion),
    };
    ...
    if (args.dryRun) {
        writeStderrLine(`Authorised (${auth.why}) and the payload is consistent. ` +
            `--dry-run: not posting.`);
        writeStdoutLine(JSON.stringify({ posted: false, wouldPost: true, target, event, cappedBy }, null, 2));
        return;
    }
...
export const submitCommand = {
    command: 'submit',
    describe: 'Post the review to GitHub — the ONLY write in this skill. Refuses unless the run is authorised to publish.',
    builder: (yargs) => yargs
        .option('pr', {
        type: 'number',
        demandOption: true,
        ...
        .option('skill-args', {
        type: 'string',
        describe: "Path to the CLI-written record of the review's invocation arguments (defaults to .qwen/tmp/qwen-skill-args-review.txt). Its `--comment` is what authorises a post. Deliberately NOT the parser's JSON output: that is a document the caller writes, and a caller that wants to post can write anything in it.",
    })
