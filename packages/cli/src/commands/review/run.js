...
async function runReview(args) {
    ...
    const captureTimer = setInterval(() => {
        if (capturedVerdict !== null)
            return;
        const path = newestArtifactSince(REVIEW_TMP_DIR, COMPOSED_PATTERN, cutoffMs);
        if (path === null)
            return;
    ...
    if (composed === null) {
        composedPath = newestArtifactSince(REVIEW_TMP_DIR, COMPOSED_PATTERN, cutoffMs);
