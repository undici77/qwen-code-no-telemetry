...
import { parseLedger, stripLedgerMarker } from './lib/ledger.js';
/**
 ...
 */
export function classifyInlineThreads(inline) {
    ...
    const repliedBlockerRoots = roots.filter((c) => repliesByRoot.has(c.id) && carriesBlockerSignal(c.body));
    const openBlockerRoots = roots.filter((c) => !repliesByRoot.has(c.id) && carriesBlockerSignal(c.body));
    const repliedRoots = roots.filter((c) => repliesByRoot.has(c.id) && !carriesBlockerSignal(c.body));
    const openRoots = roots.filter((c) => !repliesByRoot.has(c.id) && !carriesBlockerSignal(c.body));
    return {
        openRoots,
        openBlockerRoots,
        repliedBlockerRoots,
 ...
 * prevent.
 */
export function latestOwnLedger(reviews, login) {
    if (!login)
        return null;
    let best = null;
...
export function buildMarkdown(prNumber, ownerRepo, meta, inline, issue, reviews, prevLedger = null) {
    const { openRoots, openBlockerRoots, repliedBlockerRoots, repliedRoots, repliesByRoot, } = classifyInlineThreads(inline);
...
async function runPrContext(args) {
    ...
    if (prevLedger) {
        ...
        try {
            ...
            writeFileSync(join(dirname(out), `qwen-review-pr-${prNumber}-prev-ledger.json`), JSON.stringify(prevLedger, null, 2));
        }
        catch {
    ...
    const md = buildMarkdown(prNumber, ownerRepo, meta, inline, issue, reviews, prevLedger);
    mkdirSync(dirname(out), { recursive: true });
