...
export function parseReviewArgs(raw) {
    ...
    for (const tok of targetTokens) {
        ...
        if (classified === 'invalid-url') {
            warnings.push(`Unrecognized URL ${JSON.stringify(tok)} — not a GitHub PR URL (expected …/pull/<number>); refusing to guess a target from it.`);
            extraTokens.push(tok);
            continue;
        }
    ...
    if (commentEffective && effort !== 'high') {
        ...
        warnings.push('`--comment` requires a verified review; running at high effort.');
    }
...
export const parseArgsCommand = {
    ...
    describe: 'Parse the /review skill argument string (--comment, --fix, --effort, target disambiguation) and emit the verdict as JSON; pass the string on stdin via --stdin (a positional that begins with a dash never reaches this handler — yargs rejects it as an unknown flag)',
    builder: (yargs) => yargs
        .positional('raw', {
        type: 'string',
