...
import { startMockProvider, chunksFor, completionFor, messagesText, replyProblem, anthropicStream, anthropicMessage, anthropicText, mockProviderCommand, } from './mock-provider.js';
let stop = null;
afterEach(async () => {
    await stop?.();
    stop = null;
...
const post = (base, body) => fetch(`${base}/chat/completions`, {
    method: 'POST',
...
describe('the wire', () => {
    it('streams a text reply as SSE chunks ending in [DONE]', async () => {
        ...
        const body = await (await post(report.baseUrl, { stream: true, messages: [] })).text();
    ...
    it('answers a NON-stream request as a completion, with a usage block', async () => {
        ...
        const json = (await (await post(report.baseUrl, { stream: false, messages: [] })).json());
...
describe('what it refuses to answer', () => {
    ...
    it('answers the three shapes it does serve', async () => {
        ...
        for (const p of ['/chat/completions', '/messages']) {
            const res = await fetch(`${report.baseUrl}${p}`, J({ stream: false, messages: [] }));
            expect(res.status).toBe(200);
        }
...
describe('the bounds', () => {
    it('numbers only the requests the RESPONDER saw — across every skip path', async () => {
        ...
        await fetch(`${report.baseUrl}/messages`, J({ stream: false, messages: [] }));
        await fetch(`${report.baseUrl}/models`);
        await fetch(`${report.baseUrl}/chat/completions`, J({ stream: false, messages: [] }));
...
describe('the Anthropic wire', () => {
    ...
    const anth = (base, body) => fetch(`${base}/messages`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify(body),
    });
    it('streams the six events, in order, and does NOT send [DONE]', async () => {
        ...
        const body = await (await anth(report.baseUrl, { stream: true, messages: [] })).text();
    ...
    it('answers a non-stream call with a block ARRAY and the four usage fields', async () => {
        ...
        const json = (await (await anth(report.baseUrl, { stream: false, messages: [] })).json());
    ...
    it('gives the responder the top-level `system`, which Anthropic puts outside messages', () => {
        ...
        expect(anthropicText({
            system: 'you are a verifier',
            messages: [{ content: 'hi' }],
        })).toBe('you are a verifier\nhi');
        expect(anthropicText({ system: [{ text: 'blocks' }], messages: [] })).toBe('blocks');
    });
    it('...and actually hands that text to the responder on this wire', async () => {
        ...
        expect(anthropicText({ messages: [{ content: 'only user' }] })).toBe('only user');
    });
...
describe("the responder is the caller's module, and is treated as one", () => {
    ...
    it('refuses a status body that cannot be serialised, as the tool branch does', async () => {
        ...
        expect(replyProblem({ status: 500, body: circular })).toContain('cannot be serialised');
        const { report } = await serve(() => ({ status: 500, body: circular }));
...
describe('the entry points nothing had driven', () => {
    ...
    it('loads a responder exported as `respond`, and one exported as default', async () => {
        ...
        for (const [file, want] of [
            ...
            const json = (await (await fetch(`${report.baseUrl}/chat/completions`, {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
    ...
    it('names the module when it exports neither', async () => {
        const { log } = scratch();
        await expect(startMockProvider({
            log,
            ttl: 20,
    ...
    it('the CLI handler writes the report, and its TTL is in SECONDS', async () => {
        ...
        await mockProviderCommand.handler({
...
describe('the invariants, over a mixed sequence', () => {
    ...
    it('holds all of them across 120 mixed requests', async () => {
        ...
        const { report, log } = await serve((r) => {
            seen.push(r.n);
            if (r.n % 7 === 0)
                return { status: 503, body: { e: 'x' } };
            if (r.n % 5 === 0)
                return { tool: 't', args: { n: r.n } };
        ...
        const paths = [
            [
                'oai-stream',
                () => fetch(`${B}/chat/completions`, J({ stream: true, messages: [{ content: 'a' }] })),
                true,
            ],
            [
                'oai-plain',
                () => fetch(`${B}/chat/completions`, J({ stream: false, messages: [{ content: 'b' }] })),
                true,
            ],
            [
                'anth-stream',
                () => fetch(`${B}/messages`, J({ stream: true, system: 's', messages: [{ content: 'c' }] })),
                true,
            ],
            [
                'anth-plain',
                () => fetch(`${B}/messages`, J({ stream: false, messages: [{ content: 'd' }] })),
                true,
            ],
            ...
            [
                'huge',
                () => fetch(`${B}/chat/completions`, J({ stream: false, messages: [{ content: 'z'.repeat(300_000) }] })),
                true,
            ],
        ];
        ...
        expect(recs.every((r) => r.wire === 'openai' || r.wire === 'anthropic')).toBe(true);
    }, 120_000);
});
...
//# sourceMappingURL=mock-provider.test.js.map