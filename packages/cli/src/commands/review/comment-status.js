 ...
 * of the same data.
 */
export function buildThreadStatuses(comments, prAuthor, probe) {
    const byId = new Map();
...
async function runCommentStatus(args) {
    ...
    try {
        ...
        const threads = buildThreadStatuses(comments, prAuthor, makeGitProbe(worktree));
        if (worktreeStale) {
        ...
        if (worktreeMissing) {
            writeStdoutLine(`warning: no worktree at ${worktree} — run \`qwen review fetch-pr\` first. ` +
