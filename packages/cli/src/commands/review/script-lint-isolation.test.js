...
afterAll(() => {
    for (const v of TMP_VARS) {
        if (savedTmp[v] === undefined)
            delete process.env[v];
        else
            process.env[v] = savedTmp[v];
    }
});
describe('script-lint — hadolint fails closed when config isolation is unavailable', () => {
    it('adds no --config when a private neutral config cannot be created', async () => {
        ...
        expect(buildToolInvocation('hadolint', '/w/Dockerfile').argv).not.toContain('--config');
    });
    it('errors a Dockerfile (fail closed) instead of linting it unisolated', async () => {
        ...
        writeFileSync(plan, JSON.stringify({ files: [{ path: 'Dockerfile', kind: 'source' }] }));
...
//# sourceMappingURL=script-lint-isolation.test.js.map