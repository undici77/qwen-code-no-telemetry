...
import { transcriptPaths, listAgentTranscriptFiles, TranscriptsUnavailableError, textOf, } from './lib/transcripts.js';
 ...
 */
function readUsage(file, floorMs) {
...
export function computeLedger(planPath, env = process.env) {
    ...
    const main = mainEvents.length > 0 ? foldEvents('main', 'main loop', mainEvents) : null;
    let files;
    try {
    ...
    const wallSeconds = totals.firstAt !== null && totals.lastAt !== null
        ? Math.max(0, Math.round((Date.parse(totals.lastAt) - Date.parse(totals.firstAt)) / 1000))
        : 0;
