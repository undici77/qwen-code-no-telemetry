...
import { runDrive, wrapScript, sentinelExitCode, trimCapture, shellQuote, driveCommand, DRIVE_SENTINEL, } from './drive.js';
...
describe.skipIf(process.platform === 'win32')('the wrapper, driven for real', () => {
    ...
    it('capping the STREAM never yields the true exit code — measured, not assumed', () => {
        ...
        writeFileSync(sh, wrapScript('for i in $(seq 1 20000); do echo padding-line-$i-aaaaaaaaaaaaaaaaaaaa; done; exit 5', rc));
        spawnSync('bash', ['-c', `bash ${sh} 2>&1 | head -c 4096 > ${join(dir, 'log')}`], { encoding: 'utf8' });
...
describe('cleanup', () => {
    it('kills a stale server BEFORE starting, and says it did', () => {
        ...
        expect(h.log.findIndex((l) => l.includes('kill-server'))).toBeLessThan(h.log.findIndex((l) => l.includes('new-session')));
    ...
    it('kills the server even when the drive never finishes', () => {
        ...
        expect(h.log.filter((l) => l.includes('kill-server')).length).toBeGreaterThanOrEqual(2);
    });
});
...
describe('drive warns when the bundle is not built from these sources', () => {
    ...
    let repo;
    let argv1;
    const exec = (cmd, args) => cmd === 'tmux' && args[0] === '-V'
    ...
    const stamp = (digest) => stampDigest(realFs, repo, digest);
    const run = () => {
        const originalArgv = process.argv[1];
        const originalExit = process.exitCode;
        process.argv[1] = argv1;
        try {
            driveCommand.handler({
                script: 'true',
                cwd: repo,
                readyTimeout: 1,
                timeout: 1,
    ...
    it('says it could not check when sources exist but the stamp does not', () => {
        ...
        run();
        const line = vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0];
