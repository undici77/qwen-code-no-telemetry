...
vi.mock('./lib/paths.js', () => ({
    worktreePath: (n) => `/repo/.qwen/tmp/review-pr-${n}`,
}));
...
function reportWritten() {
    const call = mocks.writeFileSync.mock.calls.find(([p]) => String(p).endsWith('comment-status.json'));
    if (!call)
...
function queueHeads(before, after, author = 'octocat') {
    let n = 0;
    mocks.gh.mockImplementation((..._args) => {
        n += 1;
        return n === 1
...
describe('comment-status handler', () => {
    beforeEach(() => {
        ...
        mocks.gitOpt.mockImplementation((...args) => args.includes('rev-parse') ? 'headA' : null);
    });
    ...
    it('flags a push that raced the comments fetch, recording both samples', async () => {
        ...
        mocks.gitOpt.mockImplementation((...args) => args.includes('rev-parse') ? 'headB' : null);
    ...
    it('warns when the report exceeds the read_file truncation threshold', async () => {
        ...
        mocks.ghApiAll.mockReturnValue(Array.from({ length: 200 }, (_, i) => ({
            id: i + 1,
...
//# sourceMappingURL=comment-status.handler.test.js.map