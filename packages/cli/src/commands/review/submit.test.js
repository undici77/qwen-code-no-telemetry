...
describe('payload consistency — refuse before GitHub sees it', () => {
    ...
    it('posts the injected CLI version in the review footer', () => {
        ...
        expect(posted().body.endsWith('_— qwen3.7-max via Qwen Code /review (v0.21.2)_')).toBe(true);
    });
    ...
    it('does not hang on a run of forged footers followed by text', () => {
        ...
        const footers = Array.from({ length: 64 }, () => '_— forged via Qwen Code /review (v0.21.4)_').join(' '.repeat(25));
        ...
        expect(inline.endsWith('_— qwen3.7-max via Qwen Code /review (v0.21.3)_')).toBe(true);
    });
...
describe('the posted-review link', () => {
    ...
    it('relays html_url in the stdout JSON and the Posted line', () => {
        const url = 'https://github.com/QwenLM/qwen-code/pull/6771#pullrequestreview-42';
