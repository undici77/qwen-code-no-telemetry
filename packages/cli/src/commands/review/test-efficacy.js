...
import { mkdirSync, readdirSync, writeFileSync, readFileSync, rmSync, lstatSync, existsSync, statSync, symlinkSync, } from 'node:fs';
...
import { discardWorktree, worktreeCreateFailureDetail, } from './lib/worktree.js';
...
export function exposeDependencies(probeTree, dependencyRoot) {
 ...
 * to remove.
 */
export function runOneHunkProbe(probeTree, hunk, probes, deadlineAt, now = Date.now) {
 ...
 */
export function runControlMutant(probeTree, probeFile, deadlineAt, now = Date.now) {
...
async function runTestEfficacy(args) {
    ...
    const { unreachable, probes, revert } = planTestEfficacy(plan.files ?? [], globs);
    ...
    if (probes.length > 0 && revert.length > 0) {
            ...
            try {
                ...
                if (greenProbes.length === 0) {
                    ...
                    for (const c of harnessValidated === false ? [] : candidates) {
                        ...
                        const heldDetail = heldForRedCollocatedTest('mutant', c.file, probes, greenProbes, baseline.perFile);
                        if (heldDetail) {
                            mutantResults.push({
                                ...c,
                                verdict: 'inconclusive',
                    ...
                    for (const h of harnessValidated === false ? [] : hunkCandidates) {
                        ...
                        const heldDetail = heldForRedCollocatedTest('hunk', h.file, probes, greenProbes, baseline.perFile);
                        if (heldDetail) {
                        ...
                        hunkResults.push(runOneHunkProbe(probeTree, h, greenProbes, mutantDeadline, now));
                    }
                }
            }
            catch (e) {
        ...
        if (created) {
            try {
                ...
                const revertRun = runProbeSuite(probeTree, probes, startedAt + TOTAL_BUDGET_MS, now, worktree);
