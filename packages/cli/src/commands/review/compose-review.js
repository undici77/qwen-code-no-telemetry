...
import { BUDGET_STOP_PHRASE, ROUND_CAP_PHRASE, budgetStopDisclosure, roundCapStopDisclosure, readBudgetStop, } from './lib/deadline.js';
...
import { serializeLedger, } from './lib/ledger.js';
import { CRITICAL_PREFIX, SUGGESTION_PREFIX, countInlineFindings, severityOf, unmarkedComments, } from './lib/inline-counts.js';
import { FOOTER_MARKER, REVIEW_FOOTER_RE, footerVersion, isFooterSafeModelId, reviewFooter, } from './lib/review-footer.js';
/**
 ...
 */
function formatCannotTell(cannotTell, pr) {
    const parsed = cannotTell.map((raw) => {
        ...
        const unmarked = raw.startsWith(CRITICAL_PREFIX)
            ...
            : raw;
        const line = linkifyCommentRefs(unmarked.includes('\n')
            ? unmarked
                .split('\n')
                .map((seg) => seg.trim())
                .filter((seg) => seg !== '')
    ...
    for (const { reason, heads } of groups) {
        if (heads.length === 1) {
            lines.push(`- ${CRITICAL_PREFIX} ${heads[0]}${reason === null ? '' : ` — ${reason}`}`);
        }
        else {
            lines.push(`- ${CRITICAL_PREFIX} ${heads.length} entries — ${reason}:`, ...heads.map((head) => `  - ${head}`));
        }
    }
    return {
...
function stripReviewFooter(entry) {
...
export function composeReview(input, cliVersion = 'unknown') {
 ...
 */
function ledgerMarkerFor(input) {
    try {
        if (!input.planPath)
            return null;
        const plan = JSON.parse(readFileSync(input.planPath, 'utf8'));
        const pr = plan?.prNumber;
        const isPr = (typeof pr === 'number' && Number.isInteger(pr) && pr > 0) ||
            (typeof pr === 'string' && /^\d+$/.test(pr));
        if (!isPr)
            return null;
        let prevRound = 0;
        try {
            const prev = JSON.parse(readFileSync(join(dirname(input.planPath), `qwen-review-pr-${pr}-prev-ledger.json`), 'utf8'));
        ...
        return serializeLedger(buildLedger(prevRound + 1, (input.draftedComments ?? []), toStringList(input.bodyCriticals, 'bodyCriticals')
...
function composeReviewBody(input, cliVersion) {
    ...
    const cannotTell = toStringList(input.cannotTellCriticals, 'cannotTellCriticals')
        .map(stripReviewFooter)
        .filter((entry) => entry.trim() !== '');
    ...
    if (input.planPath) {
        ...
        if (stop !== null) {
            ...
            budgetEntry = isRoundCap
                ? roundCapStopDisclosure(typeof stop.cap === 'number' ? stop.cap : MAX_REVERSE_AUDIT_ROUNDS)
    ...
    const nonDeterministicBodyCriticals = modelBodyCriticals.filter((x) => !/\[(?:build|test|probe)\]/i.test(x)).length;
    ...
    if (!input.planPath) {
        ...
        try {
            ...
            if (cov.blindAgents.length > 0) {
                remediation.push('blind agents: rebuild each prompt with `"${QWEN_CODE_CLI:-qwen}" ' +
            ...
            if (cov.rewrittenPrompts.length > 0) {
                remediation.push('rewritten launches: re-run `"${QWEN_CODE_CLI:-qwen}" review ' +
    ...
    if (missingReceipts.length > 0) {
        ...
        // to prevent.
        remediation.push('chunks nobody read: build each with `"${QWEN_CODE_CLI:-qwen}" review ' +
...
export function scriptLintGate(planPath) {
    ...
    try {
        ...
        if (owed) {
            unreviewed.push('the executable-script lint — `qwen review script-lint` produced no report');
        }
        return { criticals, unreviewed, disclosed };
    }
    ...
    if (!planDiffHash || report.diffHash !== planDiffHash) {
        unreviewed.push('the executable-script lint — the report is stale or its diff could not be verified; re-run `qwen review script-lint`');
        return { criticals, unreviewed, disclosed };
    }
...
function scriptLintReportName(pr) {
    const positive = (typeof pr === 'number' && Number.isInteger(pr) && pr > 0) ||
...
export function testPlanGate(planPath) {
    ...
    let report;
    try {
        report = JSON.parse(readFileSync(join(dirname(planPath), `qwen-review-pr-${pr}-test-plan.json`), 'utf8'));
    }
    catch {
        return { notes };
    }
...
export function buildLedger(round, drafted, bodyCriticals) {
    ...
    for (const c of drafted) {
        ...
        const { id: carried, title } = titleOf(body.slice(marker.length).replace(/^:?\s*/, ''));
