...
import { runExtractStep, expressionsOf, invokedCommandsOf, } from './extract-step.js';
/**
 ...
 */
const expectVerbatimBody = (scriptPath, body, allowedDirectives) => {
...
describe('runExtractStep', () => {
    ...
    it('says in the SCRIPT where it must run, labelled with the level', () => {
        ...
        const script = readFileSync(meta.scriptPath, 'utf8');
        expect(script).toContain('# working-directory [workflow] (run FROM here): repo-root');
...
`;
    ...
    it('carries the env VERBATIM as comments — never half-substituted exports', () => {
        ...
        const script = readFileSync(meta.scriptPath, 'utf8');
        expect(script).toContain('# env [step] GH_TOKEN=${{ secrets.GITHUB_TOKEN }}');
    ...
    it('lists every ${{ }} site as the stub list, evaluating none', () => {
        ...
        expect(readFileSync(meta.scriptPath, 'utf8')).toContain('repos/${{ github.repository }}/issues');
    });
...
`;
        ...
        expect(runExtractStep({ workflow: wf, job: 'j', step: '1', out: join(dir, 's') })
            .index).toBe(1);
    });
...
`;
        ...
        expectVerbatimBody(runExtractStep({
            workflow: write(declared),
            job: 'j',
            step: 'Run',
            out: join(dir, 'a.sh'),
        }).scriptPath, 'a | b', ['set -eo pipefail']);
        const defaulted = `
name: t
on: [push]
jobs:
...
`;
    ...
    it.skipIf(!hasBash)('emits a script bash will parse, even with a multi-line env value', () => {
        const meta = runExtractStep({
            workflow: write(WF_BLOCK_ENV),
            job: 'fix',
            step: 'Run',
        ...
        expect(() => execFileSync('bash', ['-n', meta.scriptPath], { stdio: 'pipe' })).not.toThrow();
    });
});
...
describe('expressionsOf / invokedCommandsOf', () => {
    ...
    it('still enumerates a real site below a MALFORMED one', () => {
        ...
        expect(expressionsOf('echo ${{ github.event.issue.title }\nrun: echo ${{ github.event.comment.body }}')).toEqual(['${{ github.event.comment.body }}']);
    });
    ...
    it('captures an expression that CONTAINS a brace, and the site after it', () => {
        ...
        expect(expressionsOf("ref: ${{ github.ref || format('refs/pull/{0}/head', github.event.number) }} then ${{ github.repository }}")).toEqual([
    ...
    it.each([
        ...
        expect(expressionsOf(input)).toEqual(expected);
    });
    it('reads pipeline segments and skips keywords, builtins, and VAR= prefixes', () => {
        expect(invokedCommandsOf('if true; then\n  FOO=1 gh api x | jq .id && curl -s y\nfi\necho done')).toEqual(['curl', 'gh', 'jq']);
    });
    ...
    it('treats a heredoc body as data, not as a list of commands', () => {
        expect(invokedCommandsOf('cat <<EOF > f\nCI Evidence PR and agent\nEOF\njq .')).toEqual(['cat', 'jq']);
    ...
    it('does not mistake a QUOTED `<<EOF` for a heredoc opener', () => {
        ...
        expect(invokedCommandsOf('echo "write <<EOF for a heredoc"\ncurl x\njq .')).toEqual(['curl', 'jq']);
    });
    ...
    it.each([
        ...
        expect(invokedCommandsOf(script)).toEqual(expected);
    });
    it('reads a backslash-continued command as ONE command', () => {
        ...
        expect(invokedCommandsOf('apt-get install -y \\\n  libx11-dev \\\n  libxext-dev')).toEqual(['apt-get']);
    ...
    it('does not read the inside of a quoted assignment as the command', () => {
        ...
        // value, not after it.
        expect(invokedCommandsOf("EVIDENCE_SECTION=$'### Evidence images\\n'")).toEqual([]);
    ...
    it('carries an unterminated quote across lines, and ends it at a trailing comment', () => {
        ...
        expect(invokedCommandsOf('msg="line one\ncurl evil\nline three"\njq .')).toEqual(['jq']);
        // ...but an apostrophe in a trailing comment must not open one.
        expect(invokedCommandsOf("gh api x # don't treat this as a quote\njq .")).toEqual(['gh', 'jq']);
    });
});
//# sourceMappingURL=extract-step.test.js.map