...
import { DIGESTED_EXTENSIONS, NOT_BUNDLED_DIR, NOT_BUNDLED_FILE, NOT_BUNDLED_RE, } from './stale-bundle.js';
const repoRoot = resolve(import.meta.dirname, '..', '..', '..', '..', '..', '..');
const reviewDir = join(repoRoot, 'packages', 'cli', 'src', 'commands', 'review');
...
const isTest = (f) => NOT_BUNDLED_RE.test(basename(f));
const isFixture = (f) => relative(reviewDir, f)
    .split(sep)
...
describe('the staleness digest covers only what the bundle can contain', () => {
    ...
    const digestedFiles = files.filter((f) => DIGESTED_EXTENSIONS.code.has(extname(f)) &&
        !isTest(f) &&
        !isFixture(f) &&
    ...
    const reachable = new Set();
    {
        ...
        while (queue.length > 0) {
            ...
            if (reachable.has(from))
                continue;
            reachable.add(from);
            for (const dep of localImports(from))
                queue.push(dep);
        }
    }
    it('folds in no module that only tests import', () => {
        ...
        const unreachable = digestedFiles.filter((f) => !reachable.has(f) && !entryPoints.has(f));
        expect(unreachable.map((f) => relative(repoRoot, f)), 'an unimported file here is either test-only support or a scratch file — or the closure missed it: the scanner sees only single-quoted relative import literals and is seeded from review.ts, so widen the seed set before believing this finding').toEqual([]);
    });
...
//# sourceMappingURL=review-digest-covers-only-bundled.test.js.map