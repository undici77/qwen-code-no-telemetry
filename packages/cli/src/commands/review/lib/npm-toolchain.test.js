...
describe('npm toolchain adapter', () => {
    ...
    it('fails closed when more than one adapter applies', () => {
        writeFileSync(join(root, 'package.json'), JSON.stringify({ scripts: { build: 'exit 0' } }));
    ...
    it('returns the unchanged report shape for a supported single-root package', () => {
        ...
        expect(npmToolchainAdapter.run({
            root,
            ...
            ok: true,
            timedOut: [],
            note: 'Built 1 of 1 workspaces (the 1 the diff changes, plus what they compile ' +
    ...
    it('keeps an unmodeled npm layout on the structured unsupported path', () => {
        ...
        expect(report).toEqual({
            ...
            ok: true,
            timedOut: [],
            note: 'This repo uses a workspace glob shape this command does not model ' +
...
//# sourceMappingURL=npm-toolchain.test.js.map