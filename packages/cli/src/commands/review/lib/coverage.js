...
import { readTranscripts, wasGivenTheDiff, TranscriptsUnavailableError, } from './transcripts.js';
import { readRecordedPrompts, wasDeliveredVerbatim, briefPath, findingsPointerOf, } from './prompt-record.js';
...
export function coverageFromTranscripts(planPath, env = process.env) {
    ...
    for (const req of roster) {
        ...
        // still caps.
        const opened = pick.successfulCallArgs.some((a) => a.includes(JSON.stringify(brief)));
        if (!opened) {
 ...
 */
const rebuildFix = (role, noun) => `build the prompt with \`"\${QWEN_CODE_CLI:-qwen}" review agent-prompt ` +
...
export function verificationGaps(planPath, opts, env = process.env) {
    ...
    const deliveryOf = (key) => {
        ...
        const readTheFindings = (r) => {
            if (findingsPointer === null)
                return true;
