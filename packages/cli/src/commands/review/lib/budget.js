...
export function reviewBudget(input) {
    ...
    return {
        ...
        reverseAuditRounds: effective >= HUGE_DIFF_FLOOR
 ...
 */
export function reverseAuditRoundCap(budget) {
    const v = budget
        ?.reverseAuditRounds;
 ...
 * them linear too.
 */
const PLACEHOLDER_GAP_RE = /^(?:<[^>]*>$|[-—*_~`]+$|(?:none|n\/a|nothing|no (?:gaps?|checks?))\b(?:[.!…,;:\s]*$|\s+(?:skipped|found|to report)\b[.!…,;:\s]*$|\s*[-—–]\s*(?:stayed\s+(?:under|within|below)\s+budget\b[.!…,;:\s]*$|(?:all|every(?:thing)?|planned|further|no further)\b(?:(?!\b(?:but|except|excepting|excluding)\b).)*(?<!\b(?:none|nothing|no|zero|never|not)\s)\b(?:complete[ds]?|done|finished|covered)\b[.!…,;:\s]*$)|\s*\(\s*(?:all|every(?:thing)?)\b(?:(?!\b(?:but|except|excepting|excluding)\b)[^()])*(?<!\b(?:none|nothing|no|zero|never|not)\s)\b(?:complete[ds]?|done|finished|covered)\b[.!…,;:\s]*\)\s*$))/i;
...
export function budgetGapDisclosures(finalText) {
    ...
    for (const line of finalText.split(/\r?\n/)) {
        ...
        const unparenthesized = normalized.startsWith('(') && normalized.endsWith(')')
