...
import { MAX_INLINE_ANGLES, MIN_INLINE_ANGLES, VERIFY_SHARD, budgetGapDisclosures, stripBudgetGapLines, launchToolBudget, reverseAuditRoundCap, reviewBudget, } from './budget.js';
...
describe('reviewBudget — the reverse-audit round cap', () => {
    it('runs the full five rounds below the huge floor, three at it and above', () => {
        ...
        expect(reviewBudget({ srcDiffLines: 100, diffLines: 100 }).reverseAuditRounds).toBe(5);
        expect(reviewBudget({ srcDiffLines: 2999, diffLines: 2999 }).reverseAuditRounds).toBe(5);
        expect(reviewBudget({ srcDiffLines: 3000, diffLines: 3000 }).reverseAuditRounds).toBe(3);
        expect(reviewBudget({ srcDiffLines: 10_000, diffLines: 12_000 })
            .reverseAuditRounds).toBe(3);
    });
    it('keys on effective lines, not raw source — a huge lockfile diff caps too', () => {
        ...
        expect(reviewBudget({ srcDiffLines: 100, diffLines: 30_000 }).reverseAuditRounds).toBe(3);
