    ...
    it('fails closed when merged fields or glob lists outgrow the wire bound', () => {
        ...
        expect(() => provide(worktree, ['src/change.ts'], manifest({
            rules: [
                {
                    paths: ['src/**'],
                    domains: Array.from({ length: 128 }, (_, index) => `domain-a-${String(index).padStart(3, '0')}`),
                },
                {
                    paths: ['src/**'],
                    domains: Array.from({ length: 128 }, (_, index) => `domain-b-${String(index).padStart(3, '0')}`),
                },
            ],
        ...
        expect(() => provide(worktree, ['src/change.ts'], manifest({
            rules: [
                {
                    paths: ['src/**'],
                    relatedPaths: Array.from({ length: 128 }, (_, index) => `p-a/${index}.ts`),
                },
                {
                    paths: ['src/**'],
                    relatedPaths: Array.from({ length: 128 }, (_, index) => `p-b/${index}.ts`),
                },
            ],
