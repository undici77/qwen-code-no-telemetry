...
import { workspaceDirFor, isWorkspaceMember, isNegationExcluded, affectedWorkspaces, buildSetFor, hasUnmodeledWorkspaceGlob, readWorkspaceGlobs, readRootPackage, readWorkspacePackages, scriptFansOut, } from './workspaces.js';
...
describe('workspaceDirFor', () => {
    it('gives `packages/*` one path segment, not the whole subtree', () => {
        expect(workspaceDirFor('packages/cli/src/commands/review/submit.ts', GLOBS)).toBe('packages/cli');
    });
    it('lets an explicitly-listed nested workspace win over the star that also matches it', () => {
        ...
        expect(workspaceDirFor('packages/channels/qqbot/src/x.ts', GLOBS)).toBe('packages/channels/qqbot');
    });
    it('honours a negation, so a separate bun workspace is not a member', () => {
        ...
        expect(workspaceDirFor('packages/desktop/apps/electron/src/main.ts', GLOBS)).toBeNull();
    ...
    it('re-includes what a negation excluded when a later glob matches again', () => {
        ...
        expect(workspaceDirFor('packages/desktop/src/a.ts', globs)).toBe('packages/desktop');
    });
    it('falls back to the surviving OUTER member when a negation excludes a nested one', () => {
        ...
        expect(workspaceDirFor('packages/desktop/src/x.test.ts', globs)).toBe('packages/desktop');
        expect(isNegationExcluded('packages/desktop/src/x.test.ts', globs)).toBe(false);
    });
    it('treats a ./-prefixed glob like its bare form', () => {
        ...
        expect(workspaceDirFor('packages/cli/src/a.ts', ['./packages/*'])).toBe('packages/cli');
    ...
    it('tolerates a `./` prefix', () => {
        expect(workspaceDirFor('./packages/cli/src/a.ts', GLOBS)).toBe('packages/cli');
    });
});
describe('readWorkspaceGlobs', () => {
    it('reads the array form', () => {
        const root = mkdtempSync(join(tmpdir(), 'ws-'));
        writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'r', workspaces: ['packages/*', 'apps/web'] }));
    ...
    it('reads the object form `{ workspaces: { packages: [...] } }` npm also accepts', () => {
        const root = mkdtempSync(join(tmpdir(), 'ws-'));
        writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'r', workspaces: { packages: ['packages/*'] } }));
...
describe('readRootPackage', () => {
    ...
    it('returns null when the root has no build or test script', () => {
        const root = mkdtempSync(join(tmpdir(), 'ws-'));
        writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'solo', scripts: { lint: 'eslint' } }));
...
describe('isNegationExcluded', () => {
    it('is true when a positive glob claims the file but a negation excludes it', () => {
        expect(isNegationExcluded('packages/desktop/src/main.rs', GLOBS)).toBe(true);
    });
    ...
    it('keeps a member owned under a partial negation (`!packages/desktop/*`)', () => {
        ...
        expect(workspaceDirFor('packages/desktop/src/main.ts', globs)).toBe('packages/desktop');
        expect(isNegationExcluded('packages/desktop/src/main.ts', globs)).toBe(false);
    });
});
describe('readWorkspacePackages', () => {
    ...
    const setup = (globs) => {
        ...
        writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'r', workspaces: globs }));
    };
    const teardown = () => {
        rmSync(root, { recursive: true, force: true });
    };
    const write = (dir, body) => {
        ...
        writeFileSync(join(root, dir, 'package.json'), typeof body === 'string' ? body : JSON.stringify(body));
    };
    ...
    it('includes a SYMLINKED member — npm links it as a workspace all the same', () => {
        ...
        symlinkSync(join(root, 'shared', 'linked-pkg'), join(root, 'packages', 'linked'));
        const { packages } = readWorkspacePackages(root);
...
describe('affectedWorkspaces', () => {
    it('dedupes and sorts the workspaces a change set touches', () => {
        expect(affectedWorkspaces([
            'packages/cli/src/a.ts',
            'packages/cli/src/b.ts',
            'packages/core/src/c.ts',
            'README.md',
        ], GLOBS)).toEqual(['packages/cli', 'packages/core']);
    });
...
describe('buildSetFor', () => {
    it('orders dependencies before the package that needs them', () => {
        ...
        expect(set.indexOf('packages/core')).toBeLessThan(set.indexOf('packages/bridge'));
        expect(set.indexOf('packages/bridge')).toBeLessThan(set.indexOf('packages/leaf'));
    });
    it('does not build the siblings of a leaf change', () => {
        // The whole point: `other` cannot have been broken by a change to `leaf`.
        expect(buildSetFor(['packages/leaf'], PKGS)).not.toContain('packages/other');
    });
    ...
    it('puts `alsoBuild` FIRST — no declared edge can order it', () => {
        ...
        expect(set.indexOf('packages/templates')).toBeLessThan(set.indexOf('packages/leaf'));
    });
