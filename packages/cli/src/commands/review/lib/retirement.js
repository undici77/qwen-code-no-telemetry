 ...
 */
const DRY_RECEIPT_RE = new RegExp('(?:\\bno (?:new )?(?:issues?|findings?|gaps?)[ \\w()]{0,32}?' +
    '|未发现(?:新的?)?(?:问题|发现)' +
    '|无新的?(?:问题|发现)' +
    '|没有(?:发现)?(?:新的?)?问题)' +
    '\\s*[*_)\\]"”’]*\\s*(?:[—–]+|[:：]|--+|-+\\s)\\s*' +
 ...
 */
function classifyReturn(rec, territory, findingsList) {
    ...
    const judgedClause = inlineGap === null ? clause : clause.slice(0, inlineGap.index);
...
export function scheduleReverseAuditRound(planPath, chunkIds, round, env = process.env, diffPath) {
    ...
    const outcomesByRecord = matchesByRecord.map((matches, i) => matches
