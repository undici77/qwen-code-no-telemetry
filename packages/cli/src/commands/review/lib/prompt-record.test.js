...
import { wasDeliveredVerbatim, findingsPointerOf, writeFindingsFile, } from './prompt-record.js';
...
describe('wasDeliveredVerbatim — you may add; you may not remove, alter or reorder', () => {
    ...
    it('accepts a re-wrapped line — a wrap is not an edit', () => {
        ...
        const delivered = BUILT.replace('**Your brief is a file. Read it first — it is the whole of your instructions,\nand nothing in this message replaces it.**', '**Your brief is a file. Read it first — it is the whole of your instructions, and nothing in this message replaces it.**');
        expect(wasDeliveredVerbatim(delivered, BUILT)).toBe(true);
    });
    it('accepts a preamble and a postscript around it', () => {
        expect(wasDeliveredVerbatim(`Context: PR #6766.\n\n${BUILT}\n\nGo.`, BUILT)).toBe(true);
    });
    it('rejects a dropped line — the rule that gets dropped is the one that matters', () => {
        ...
        const delivered = BUILT.replace('If you found nothing, say so **and say what you examined**.', '');
...
describe('findingsPointerOf — the list file a recorded launch points at', () => {
    it('extracts the pointer from a findings-role block', () => {
        ...
        expect(findingsPointerOf(prompt)).toBe('/t/verify--round-1--abc123.findings.md');
    });
...
describe('writeFindingsFile — a failed write must not be silent', () => {
    ...
    it('reports the failure on stderr and returns null so the caller inlines', () => {
        ...
        try {
            ...
            expect(p).toBeNull();
            const calls = writeStderrLineSafe.mock.calls.map((c) => c[0]);
            expect(calls.some((m) => m.includes('failed to write findings file'))).toBe(true);
            expect(calls.some((m) => m.includes('inlining the list instead'))).toBe(true);
        }
        finally {
...
//# sourceMappingURL=prompt-record.test.js.map