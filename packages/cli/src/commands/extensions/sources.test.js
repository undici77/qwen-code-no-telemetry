...
import { sourcesCommand, handleSourcesAdd, handleSourcesRemove, handleSourcesList, handleSourcesUpdate, } from './sources.js';
...
describe('extensions sources command', () => {
    it('should parse the sources subcommands', () => {
        ...
        const parse = (command) => yargs([]).command(sourcesCommand).fail(false).locale('en').parse(command);
...
describe('handleSourcesAdd', () => {
    ...
    it('adds a marketplace and reports its name', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('Added marketplace "my-marketplace".');
    });
...
describe('handleSourcesRemove', () => {
    ...
    it('removes a marketplace by name', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('Removed marketplace "my-marketplace".');
    });
    it('errors when the marketplace is unknown', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith('Marketplace "missing" not found.');
        expect(processSpy).toHaveBeenCalledWith(1);
        processSpy.mockRestore();
    });
});
describe('handleSourcesList', () => {
    ...
    it('prints a message when no sources are configured', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('No marketplace sources added yet.');
    });
    it('lists configured sources with source and type', async () => {
        ...
        await handleSourcesList();
        const output = mockWriteStdoutLine.mock.calls[0][0];
...
describe('handleSourcesUpdate', () => {
    ...
    it('re-fetches the marketplace and reports the plugin count', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('Updated marketplace "market-a".');
    ...
    it('errors when the marketplace cannot be loaded', async () => {
        ...
        await handleSourcesUpdate({ name: 'market-a' });
        expect(mockWriteStderrLine).toHaveBeenCalledWith('Could not load this marketplace.');
        expect(processSpy).toHaveBeenCalledWith(1);
        processSpy.mockRestore();
    });
});
