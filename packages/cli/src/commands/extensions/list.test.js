...
describe('handleList', () => {
    ...
    it('should display message when no extensions are installed', async () => {
        ...
        await handleList();
        expect(mockWriteStdoutLine).toHaveBeenCalledWith(t('No extensions installed.'));
    ...
    it('should list installed extensions', async () => {
        ...
        mockToOutputString.mockImplementation((ext) => `${ext.name} (${ext.version})`);
        await handleList();
        expect(mockGetLoadedExtensions).toHaveBeenCalled();
        expect(mockToOutputString).toHaveBeenCalledTimes(2);
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('extension-1 (1.0.0)\n\nextension-2 (2.0.0)');
        processExitSpy.mockRestore();
    });
