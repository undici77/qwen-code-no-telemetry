...
import { ChannelLoopScheduler, ChannelLoopStore, DaemonChannelBridge, isChannelProactiveDeliveryError, sanitizeLogText, SessionRouter, } from '@qwen-code/channel-base';
...
export function createDaemonSessionFactory({ client, DaemonSessionClient, clientId, }) {
    return async (req) => {
        ...
        if (req.sessionId) {
            return await DaemonSessionClient.load(client, req.sessionId, daemonReq, clientId);
        }
...
async function loadDaemonSdk() {
    return (await import('@qwen-code/sdk/daemon'));
}
...
export const daemonWorkerCommand = {
    ...
    handler: async (argv) => {
        ...
        try {
            ...
            scrubAndReportInheritedLoaderEnv(process.env, 'qwen channel daemon-worker', 'channel daemon worker');
            const send = process.send;
