...
describe('ObservedChannelContactStore', () => {
    ...
    let now;
    beforeEach(() => {
        filePath = join(mkdtempSync(join(tmpdir(), 'qwen-observed-contacts-')), 'observed-contacts.json');
        now = new Date('2026-07-17T12:00:00.000Z');
    });
    function createStore(maxObservations = 500) {
    ...
    it('returns an empty graph when the registry is missing', () => {
        expect(createStore().list({ freshWithinSeconds: 7 * 24 * 60 * 60 })).toEqual({ users: [], groups: [] });
    });
    ...
    it('keeps the newest bounded relationship observations', () => {
        ...
        expect(store.list({ freshWithinSeconds: 604800 }).users.map((user) => user.id)).toEqual(['3', '2']);
    });
    ...
    it('does not split a Unicode surrogate pair when truncating labels', () => {
        ...
        expect(store.list({ freshWithinSeconds: 604800 }).users[0]?.label).toBe('x'.repeat(255));
    });
    ...
    it('rejects malformed or unsupported registries', () => {
        ...
        expect(() => createStore().list({ freshWithinSeconds: 604800 })).toThrow('Unsupported observed contact registry version');
        writeFileSync(filePath, '{');
        expect(() => createStore().list({ freshWithinSeconds: 604800 })).toThrow('Invalid observed contact registry');
    });
});
//# sourceMappingURL=observed-contact-store.test.js.map