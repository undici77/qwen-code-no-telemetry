...
describe('channel startup failure formatting', () => {
            ...
            startupFailures: [
                {
                    ...
                    phase: 'connect',
                    code: 'ECONNREFUSED',
                    message: 'Authorization: Bea\u200b\u0085rer secret-token; Bearer \u001b]0;window-title\u0007 runtime-secret https://user:pass@example.com/path',
                },
            ],
        });
    ...
    it('caps output at 64 failures and appends one truncation line', () => {
        ...
        expect(lines[64]).toBe('[Channel] Additional startup failures were truncated.');
    ...
    it('ignores malformed entries and throwing getters without exposing values', () => {
        ...
        expect(formatChannelStartupFailures({ startupFailures: throwingArray })).toEqual([]);
    });
    it('reads only channel worker start error bodies and safely formats errors', () => {
        expect(channelStartupFailureBody({
            body: { code: 'channel_worker_start_failed' },
        })).toEqual({ code: 'channel_worker_start_failed' });
        expect(channelStartupFailureBody({ body: { code: 'unrelated_error' } })).toBeUndefined();
        expect(safeChannelCommandErrorMessage(new Error('Authorization: Bearer command-secret'))).not.toContain('command-secret');
        expect(sanitizeChannelCommandValue('https://user:pass@example.com')).not.toContain('user:pass');
        const throwingBody = {};
...
//# sourceMappingURL=startup-failure-format.test.js.map