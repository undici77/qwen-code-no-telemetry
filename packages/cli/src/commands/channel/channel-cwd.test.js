...
describe('resolveChannelCwd', () => {
    it('defaults to the loading workspace', () => {
        expect(resolveChannelCwd(undefined, '/workspace')).toBe(path.resolve('/workspace'));
    });
    it('resolves home-relative paths from the user home', () => {
        expect(resolveChannelCwd('~/channels', '/workspace')).toBe(path.join(os.homedir(), 'channels'));
    });
    it('resolves ordinary relative paths from the loading workspace', () => {
        expect(resolveChannelCwd('../channels', '/workspace/project')).toBe(path.resolve('/workspace/channels'));
    });
...
//# sourceMappingURL=channel-cwd.test.js.map