...
describe('sessions list command', () => {
    ...
    it('should display sessions in human-readable table format', async () => {
        ...
        expect(calls.some((c) => c.includes('a1b2c3d4') && c.includes('React 组件开发'))).toBe(true);
    });
    it('should display dash for missing git branch', async () => {
        ...
        await handleList({});
        const calls = mockWriteStdoutLine.mock.calls.map((c) => c[0]);
        const dataLine = calls.find((c) => c.includes('a1b2c3d4') && !c.includes('SESSION ID'));
    ...
    it('should output JSON Lines format when --json is set', async () => {
        ...
        const jsonLines = calls.filter((c) => c[0] !== undefined && c[0].trim().startsWith('{'));
    ...
    it('should show hasMore hint when there are more sessions', async () => {
        ...
        await handleList({});
        expect(mockWriteStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Use --limit to show more'));
    });
    it('should not show hasMore hint when hasMore is false', async () => {
        ...
        await handleList({});
        const calls = mockWriteStdoutLine.mock.calls.map((c) => c[0]);
        expect(calls.some((c) => c.includes('Use --limit to show more'))).toBe(false);
    });
    ...
    it('should emit hasMore hint to stderr in JSON mode', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('Use --limit to show more'));
    });
    ...
    it('should handle initSessionService failure', async () => {
        ...
        await handleList({});
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('initialize session service'));
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('settings not found'));
        expect(process.exit).toHaveBeenCalledWith(1);
    });
    it('should handle listSessions failure', async () => {
        mockListSessions.mockRejectedValue(new Error('disk full'));
        await handleList({});
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('failed to list sessions'));
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('disk full'));
        expect(process.exit).toHaveBeenCalledWith(1);
    });
});
