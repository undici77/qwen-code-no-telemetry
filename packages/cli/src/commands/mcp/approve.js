...
async function setProjectServerStatus(name, status, all) {
    ...
    if (names.length === 0) {
        writeStdoutLine('No approval-requiring MCP servers found (looked in .mcp.json and .qwen/settings.json).');
        return;
    }
...
export const approveCommand = {
    ...
    describe: 'Approve a gated MCP server (.mcp.json or workspace .qwen/settings.json)',
    builder: (yargs) => yargs
...
export const rejectCommand = {
    command: 'reject [name]',
    describe: 'Reject a gated MCP server (.mcp.json or workspace .qwen/settings.json)',
    builder: (yargs) => yargs
