...
vi.mock('@qwen-code/qwen-code-core', () => ({
    ...
    runWithTimeout: (task, timeoutMs, _label) => new Promise((resolve, reject) => {
...
const mockedAssembleMcpServers = assembleMcpServers;
const mockedLoadMcpApprovals = loadMcpApprovals;
...
describe('mcp list command', () => {
    ...
    it('passes explicit untrusted workspace state to the extension manager', async () => {
        ...
        await listMcpServers();
        expect(MockedExtensionManager).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should disconnect when transport startup exceeds the timeout', async () => {
        ...
        try {
            ...
            expect(mockWriteStdoutLine).toHaveBeenCalledWith(expect.stringContaining('slow-server: https://example.com/sse (sse) - Disconnected (timed out after 5000ms)'));
        }
        finally {
            vi.useRealTimers();
        }
    ...
    it('shows a pending project server without connecting', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith(expect.stringContaining('project-server: node server.js (stdio) - Pending approval'));
    });
    it('shows a rejected workspace server without connecting', async () => {
        ...
        await listMcpServers();
        expect(mockedCreateTransport).not.toHaveBeenCalled();
        expect(mockWriteStdoutLine).toHaveBeenCalledWith(expect.stringContaining('workspace-server: https://example.com/mcp (http) - Rejected'));
    });
});
