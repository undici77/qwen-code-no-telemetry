...
describe('qwen mcp approve / reject', () => {
    let dir;
    let cwdSpy;
    const output = () => mockWriteStdoutLine.mock.calls.map((c) => c[0]).join('\n');
    ...
    const writeMcpJson = (servers) => fs.writeFileSync(path.join(dir, '.mcp.json'), JSON.stringify({ mcpServers: servers }));
    const stateOf = (name) => {
        resetMcpApprovalsForTesting();
        const { servers } = loadProjectMcpServers(dir);
        return loadMcpApprovals().getState(dir, name, servers[name]);
    };
    const writeWorkspaceSettings = (servers) => {
        const qwenDir = path.join(dir, '.qwen');
        fs.mkdirSync(qwenDir, { recursive: true });
        fs.writeFileSync(path.join(qwenDir, 'settings.json'), JSON.stringify({ mcpServers: servers }));
    };
    /** Read the persisted approval status straight off disk (scope-agnostic). */
    const persistedStatus = (name) => {
        const raw = fs.readFileSync(process.env['QWEN_CODE_MCP_APPROVALS_PATH'], 'utf-8');
