...
describe('mcp add command', () => {
    ...
    it('should respect explicit transport even when commandOrUrl is a URL', async () => {
        await parser.parseAsync('add --transport sse sse-server https://example.com/sse-endpoint');
        expect(mockSetValue).toHaveBeenCalledWith(SettingScope.User, 'mcpServers', {
            'sse-server': {
    ...
    describe('when handling scope and directory', () => {
        ...
        describe('when in the home directory', () => {
            ...
            it('should show an error when --scope=project is used explicitly', async () => {
                ...
                expect(mockWriteStderrLine).toHaveBeenCalledWith('Error: Please use --scope user to edit settings in the home directory.');
    ...
    describe('OAuth configuration', () => {
        it('should add OAuth config when OAuth options are provided', async () => {
            await parser.parseAsync('add oauth-server https://example.com/mcp --transport http ' +
        ...
        it('should add OAuth config with only redirect URI', async () => {
            await parser.parseAsync('add oauth-server https://example.com/mcp --transport sse ' +
                '--oauth-redirect-uri https://example.com/oauth/callback');
            expect(mockSetValue).toHaveBeenCalledWith(SettingScope.User, 'mcpServers', expect.objectContaining({
                'oauth-server': expect.objectContaining({
        ...
        it('should not include oauth field when no OAuth options are provided', async () => {
            await parser.parseAsync('add my-server https://example.com/mcp --transport http');
            expect(mockSetValue).toHaveBeenCalledWith(SettingScope.User, 'mcpServers', expect.objectContaining({
                'my-server': expect.objectContaining({
                    httpUrl: 'https://example.com/mcp',
                }),
        ...
        it('should reject OAuth options when transport is stdio', async () => {
            ...
            await expect(parser.parseAsync('add stdio-server /usr/bin/my-server --transport stdio ' +
                '--oauth-client-id id')).rejects.toThrow('process.exit called');
            expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('OAuth options (--oauth-*) are only supported with --transport sse or --transport http.'));
            expect(mockProcessExit).toHaveBeenCalledWith(1);
            expect(mockSetValue).not.toHaveBeenCalled();
        });
