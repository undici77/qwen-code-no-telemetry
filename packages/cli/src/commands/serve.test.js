...
import { localControlUrls, maybeOpenWebShellBrowser, serveCommand, } from './serve.js';
...
describe('serve command args', () => {
    ...
    it('parses --local-control and requires its generated token and Web Shell', () => {
        ...
        expect(() => buildParser().parseSync('--local-control --token fixed')).toThrow(/generates its own token/);
        expect(() => buildParser().parseSync('--local-control --allow-origin http://localhost:3000')).toThrow(/manages its browser origins/);
        expect(() => buildParser().parseSync('--local-control --no-web')).toThrow(/Local Control requires the Web Shell/);
        expect(() => buildParser().parseSync('--local-control --port 0')).toThrow(/Local Control requires a fixed port/);
        for (const port of ['-1', '1.5', '65536']) {
            expect(() => buildParser().parseSync(`--local-control --port ${port}`)).toThrow(/Local Control requires a fixed port/);
        }
        expect(() => buildParser().parseSync('--local-control --hostname 192.168.1.2')).toThrow(/Local Control manages its hostname/);
    });
...
describe('serve rate limit env parsing', () => {
    ...
    it('starts Local Control with a fresh token, QR pairing, and sleep inhibition', async () => {
        ...
        expect(mockSleepInhibitor.acquire).toHaveBeenCalledWith('Qwen Code Local Control is active');
        expect(mockQr.setErrorLevel).toHaveBeenCalledWith('Q');
        expect(String(mockQr.generate.mock.calls[0]?.[0])).toContain(`#token=${options.token}`);
        expect(String(mockQr.generate.mock.calls[0]?.[0])).toMatch(/^https:/);
        expect(options.allowOrigins).toContain(new URL(String(mockQr.generate.mock.calls[0]?.[0])).origin);
        ...
        expect(stdoutWrites.join('')).toContain('Traffic is encrypted only when --tls-cert and --tls-key are set');
        await vi.waitFor(() => expect(mockOpenBrowserSecurely).toHaveBeenCalledWith(expect.stringMatching(/^https:\/\/127\.0\.0\.1\/#token=[A-Za-z0-9_-]{43}$/)));
    });
