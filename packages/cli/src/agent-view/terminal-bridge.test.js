...
describe('bridgeAgentViewTerminal', () => {
    it('writes AsyncIterable stdin bytes into the PTY', async () => {
        ...
        await expect(bridgeAgentViewTerminal({
    ...
    it('preserves Uint8Array subarray offsets when writing stdin bytes', async () => {
        const pty = new FakeTerminalPty();
        const chunk = new Uint8Array([0, 104, 101, 108, 108, 111, 0]).subarray(1, 6);
        await expect(bridgeAgentViewTerminal({
            stdin: bytes([chunk]),
    ...
    it('drains pending stdout writes when the input pump fails', async () => {
        ...
        const stdin = {
            [Symbol.asyncIterator]: () => ({
                next: () => Promise.reject(new Error('stdin failed')),
            }),
        };
...
async function* delayedInput(registerRelease) {
...
class DeferredWritable extends Writable {
    ...
    flush() {
        for (const cb of this.pending.splice(0))
            cb();
    }
    output() {
        return Buffer.concat(this.chunks).toString('utf8');
...
//# sourceMappingURL=terminal-bridge.test.js.map