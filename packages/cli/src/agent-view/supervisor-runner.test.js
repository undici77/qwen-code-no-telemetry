...
import { connectExistingAgentViewSupervisor, INTERNAL_AGENT_VIEW_SUPERVISOR_ARG, ensureAgentViewSupervisor, runAgentViewSupervisor, } from './supervisor-runner.js';
import { createAgentViewSupervisorServer } from './supervisor-server.js';
import { readAgentViewSupervisor, writeAgentViewSessionState, } from './supervisor-store.js';
const cleanupDirs = [];
const cleanupServers = [];
afterEach(async () => {
    await Promise.allSettled(cleanupServers.splice(0).map((server) => server.close()));
...
describe('Agent View supervisor runner', () => {
    it('returns undefined when no existing supervisor is reachable', async () => {
        ...
        await expect(connectExistingAgentViewSupervisor({ globalDir })).resolves.toBeUndefined();
    });
    it('connects to an existing supervisor handle', async () => {
        ...
        if (!handle)
            throw new Error('Expected existing supervisor handle.');
    ...
    it('rejects promptly when the spawned supervisor exits before readiness', async () => {
        ...
        await expect(result).rejects.toThrow('Agent View supervisor exited before becoming ready with code 1.');
        expect(startedProcess.unref).toHaveBeenCalledOnce();
    });
    it('routes handle status/list/dispatch/shutdown calls through IPC', async () => {
        ...
        await expect(handle.dispatch('write tests', '/workspace/project')).resolves.toEqual({ sessionId: 'session-2' });
        expect(handler.dispatch).toHaveBeenCalledWith({
            prompt: 'write tests',
            cwd: '/workspace/project',
        });
        await expect(handle.adopt({
            sessionId: 'session-2',
    ...
    it('closes the supervisor server when shutdown is requested', async () => {
        ...
        await expect(callAgentViewSupervisor(socketPath, 'shutdown', undefined, {
            authToken,
        })).resolves.toEqual({
            shuttingDown: true,
            workersStopped: 0,
...
function createFakeSupervisor(socketPath, handler) {
...
async function makeSupervisorPath() {
    const globalDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-agent-view-runner-'));
...
async function writeHibernatedSessionForTest(globalDir, sessionId) {
...
async function waitForSupervisor(socketPath, globalDir) {
...
async function readAuthToken(globalDir) {
...
async function expectSupervisorUnreachable(socketPath, authToken) {
    for (let attempt = 0; attempt < 20; attempt++) {
        try {
            await callAgentViewSupervisor(socketPath, 'status', undefined, {
                authToken,
...
//# sourceMappingURL=supervisor-runner.test.js.map