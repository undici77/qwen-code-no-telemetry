...
import { getAgentViewSessionPaths, writeAgentViewSessionState, } from './supervisor-store.js';
describe('getAgentViewSupervisorSocketPath', () => {
    ...
    it('falls back to the runtime dir for long unix paths', () => {
        ...
        expect(socketPath.startsWith(`${path.join(runtimeDir)}${path.sep}`)).toBe(true);
    ...
    it('is deterministic for the same global dir', () => {
        ...
        expect(getAgentViewSupervisorSocketPath({ globalDir, platform: 'linux' })).toBe(getAgentViewSupervisorSocketPath({ globalDir, platform: 'linux' }));
    });
    it('isolates the tmpdir fallback socket in a per-uid directory', () => {
        ...
        expect(socketPath).toContain(`${path.sep}qwen-agent-view-${uid}${path.sep}`);
        expect(socketPath.endsWith('.sock')).toBe(true);
    });
});
describe('createAgentViewSupervisorHandler', () => {
    ...
    async function makeGlobalDir() {
        const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-agent-view-process-'));
        cleanupDirs.push(dir);
        return dir;
    }
    async function writeSession(globalDir, overrides = {}) {
    ...
    it('reports session count and socket path in status', async () => {
        ...
        expect(status.socketPath).toBe(getAgentViewSupervisorSocketPath({
            globalDir,
...
//# sourceMappingURL=supervisor-process.test.js.map