...
describe('buildCurrentQwenCliArgv', () => {
    ...
    it('throws when DEV=true with a TypeScript entrypoint but tsx is missing', async () => {
        const entryRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-entry-argv-'));
        ...
        const cwdTsxCli = path.join(cwdRoot, 'node_modules', 'tsx', 'dist', 'cli.mjs');
        await fs.mkdir(path.dirname(entrypoint), { recursive: true });
        ...
        try {
            ...
            expect(() => buildCurrentQwenCliArgv(['agents'])).toThrow(/tsx was not found/);
        }
        finally {
...
//# sourceMappingURL=current-cli-argv.test.js.map