...
import { getAgentViewSessionPaths, getAgentViewStorePaths, listAgentViewSessionSnapshots, listAgentViewSessionStates, readAgentViewRoster, readAgentViewSessionState, removeAgentViewRosterEntry, updateAgentViewRosterEntry, upsertAgentViewRosterEntry, writeAgentViewActivity, writeAgentViewLaunch, writeAgentViewSessionState, writeAgentViewSupervisor, writeAgentViewWorker, readAgentViewActivity, readAgentViewLaunch, readAgentViewSupervisor, readAgentViewWorker, } from './supervisor-store.js';
describe('agent view supervisor store', () => {
    ...
    it('writes credential files without following a pre-placed symlink', async () => {
        ...
        expect(fs.readFileSync(paths.supervisorPath, 'utf8')).toContain('secret-token');
    });
    it('uses daemon and jobs directories under the global qwen dir', () => {
        ...
        expect(getAgentViewSessionPaths('../bad/id', { globalDir: tempDir })).toEqual({
    ...
    it('upserts, sorts, and removes roster entries while preserving fields', async () => {
        await upsertAgentViewRosterEntry(rosterEntry('one', {
            updatedAt: '2026-07-16T00:00:00.000Z',
            custom: 'keep',
        }), { globalDir: tempDir });
        await upsertAgentViewRosterEntry(rosterEntry('two', {
            pinned: true,
        ...
        await upsertAgentViewRosterEntry(rosterEntry('three', {
            pinned: false,
    ...
    it('updates roster entries and keeps pinned entries first', async () => {
        ...
        await expect(updateAgentViewRosterEntry('missing', (entry) => ({ ...entry, displayName: 'Missing' }), { globalDir: tempDir })).resolves.toBeUndefined();
        await expect(updateAgentViewRosterEntry('one', (entry) => ({
            ...entry,
            pinned: true,
            displayName: 'Pinned',
            updatedAt: '2026-07-16T00:00:02.000Z',
        }), { globalDir: tempDir })).resolves.toMatchObject({
            sessionId: 'one',
            displayName: 'Pinned',
            pinned: true,
        });
    ...
    it('writes session files and preserves unknown fields on updates', async () => {
        ...
        await expect(readAgentViewSessionState('session-1', { globalDir: tempDir })).resolves.toMatchObject({
            sessionId: 'session-1',
            sessionState: 'completed',
            processState: 'alive',
            customState: 'keep',
        });
        expect(fs.existsSync(getAgentViewSessionPaths('session-1', { globalDir: tempDir }).tmpDir)).toBe(true);
    });
    it('lists valid session states sorted by most recent update', async () => {
        await writeAgentViewSessionState(sessionState('older', {
            updatedAt: '2026-07-16T00:00:00.000Z',
        }), { globalDir: tempDir });
        await writeAgentViewSessionState(sessionState('newer', {
            updatedAt: '2026-07-16T00:00:01.000Z',
        }), { globalDir: tempDir });
    ...
    it('isolates an unreadable session entry instead of failing the list', async () => {
        await writeAgentViewSessionState(sessionState('healthy', { updatedAt: '2026-07-16T00:00:01.000Z' }), { globalDir: tempDir });
        ...
        await expect(listAgentViewSessionStates({ globalDir: tempDir })).resolves.toMatchObject([{ sessionId: 'healthy' }]);
    });
    it('trusts the session directory name over the state file contents', async () => {
        ...
        fs.writeFileSync(path.join(sessionDir, 'state.json'), JSON.stringify(sessionState('victim-session')));
        const states = await listAgentViewSessionStates({ globalDir: tempDir });
    ...
    it('includes roster entries in session snapshots', async () => {
        ...
        await upsertAgentViewRosterEntry(rosterEntry('session-1', {
    ...
    it('round trips launch, activity, worker, and supervisor files', async () => {
        ...
        await expect(readAgentViewLaunch('session-1', { globalDir: tempDir })).resolves.toMatchObject(launch);
        await expect(readAgentViewActivity('session-1', { globalDir: tempDir })).resolves.toMatchObject(activity);
        await expect(readAgentViewWorker('session-1', { globalDir: tempDir })).resolves.toMatchObject(worker);
        await expect(readAgentViewSupervisor({ globalDir: tempDir })).resolves.toMatchObject(supervisor);
    });
    it('normalizes session ids for case-insensitive filesystems', () => {
        expect(getAgentViewSessionPaths('ABC123', { globalDir: tempDir })).toEqual(getAgentViewSessionPaths('abc123', { globalDir: tempDir }));
    });
    it('sanitizes a dot-only session id instead of escaping the jobs dir', () => {
        expect(getAgentViewSessionPaths('..', { globalDir: tempDir }).sessionDir).toBe(path.join(tempDir, 'jobs', '_'));
    });
    it('joins roster entries to snapshots case-insensitively', async () => {
        await writeAgentViewSessionState(sessionState('ABC123'), {
            globalDir: tempDir,
        });
        await upsertAgentViewRosterEntry(rosterEntry('ABC123', { displayName: 'Upper', pinned: true }), { globalDir: tempDir });
        const snapshots = await listAgentViewSessionSnapshots({
            globalDir: tempDir,
        });
    ...
    it('preserves unknown fields from a prior writer when merging a write', async () => {
        ...
        fs.writeFileSync(paths.statePath, JSON.stringify({ ...sessionState('session-1'), futureField: 'keep' }));
        await writeAgentViewSessionState(sessionState('session-1'), {
            globalDir: tempDir,
        });
    ...
    it('throws when a transient read error hits the roster during a mutation', async () => {
        ...
        await expect(upsertAgentViewRosterEntry(rosterEntry('one'), { globalDir: tempDir })).rejects.toThrow();
        await expect(removeAgentViewRosterEntry('one', { globalDir: tempDir })).rejects.toThrow();
        await expect(updateAgentViewRosterEntry('one', (e) => e, { globalDir: tempDir })).rejects.toThrow();
    });
    it('strips wrong-typed optional fields during activity normalization', async () => {
        ...
        fs.mkdirSync(paths.sessionDir, { recursive: true });
        fs.writeFileSync(paths.activityPath, JSON.stringify({
            lastActivityAt: '2026-07-16T00:00:00.000Z',
            summary: 42,
            waitingFor: true,
            lastResult: ['wrong'],
    ...
    it('strips a wrong-typed authToken during supervisor normalization', async () => {
        const paths = getAgentViewStorePaths({ globalDir: tempDir });
        fs.mkdirSync(paths.daemonDir, { recursive: true });
        fs.writeFileSync(paths.supervisorPath, JSON.stringify({
            pid: 123,
    ...
    it('updates roster entries case-insensitively', async () => {
        ...
        const updated = await updateAgentViewRosterEntry('MYSESSION', (entry) => ({ ...entry, displayName: 'Updated' }), { globalDir: tempDir });
        expect(updated?.displayName).toBe('Updated');
    });
});
function rosterEntry(sessionId, overrides = {}) {
    return {
        sessionId,
...
function sessionState(sessionId, overrides = {}) {
    return {
        schemaVersion: 1,
        sessionId,
        ownership: 'managed',
...
//# sourceMappingURL=supervisor-store.test.js.map