...
describe('Agent View supervisor server', () => {
    it('serves status/list/shutdown requests through the JSON IPC client', async () => {
        ...
        await server.listen();
        try {
            await expect(requestAgentViewSupervisor(socketPath, {
                id: 'status-request',
                op: 'status',
                params: { verbose: true },
            ...
            await expect(callAgentViewSupervisor(socketPath, 'list')).resolves.toEqual([{ sessionId: 'session-1' }]);
            expect(handler.list).toHaveBeenCalledWith(undefined);
            await expect(callAgentViewSupervisor(socketPath, 'shutdown')).resolves.toEqual({ shuttingDown: true });
    ...
    it('requires the supervisor auth token when configured', async () => {
        ...
        await server.listen();
        try {
            await expect(callAgentViewSupervisor(socketPath, 'status')).rejects.toMatchObject({
                code: 'unauthorized',
            });
            await expect(callAgentViewSupervisor(socketPath, 'status', undefined, {
    ...
    it('rejects worker sideband operations when no authorizer is registered', async () => {
        ...
        try {
            ...
            await expect(callAgentViewSupervisor(socketPath, 'workerEvent', {
                type: 'heartbeat',
                sessionId: 'session-1',
    ...
    it('serves peek requests through the JSON IPC client', async () => {
        ...
        await server.listen();
        try {
            await expect(callAgentViewSupervisor(socketPath, 'peek', {
                sessionId: 'session-1',
    ...
    it('serves internal pin and rename requests through the JSON IPC client', async () => {
        ...
        await server.listen();
        try {
            await expect(callAgentViewSupervisor(socketPath, 'pin', {
                sessionId: 'session-1',
                pinned: true,
            ...
            await expect(callAgentViewSupervisor(socketPath, 'rename', {
                sessionId: 'session-1',
                displayName: 'Build Fix',
            })).resolves.toEqual({
                sessionId: 'session-1',
    ...
    it('serves send and answer requests through the JSON IPC client', async () => {
        ...
        await server.listen();
        try {
            await expect(callAgentViewSupervisor(socketPath, 'send', {
                sessionId: 'session-1',
                text: 'next',
            ...
            await expect(callAgentViewSupervisor(socketPath, 'answer', {
                sessionId: 'session-1',
                text: 'yes',
    ...
    it('keeps subscribe sockets open and forwards changed events', async () => {
        ...
        const handler = {
            ...
            list: vi.fn(() => []),
            shutdown: vi.fn(() => ({})),
            subscribe: vi.fn((_params, socket) => {
                socket.write(`${JSON.stringify({
    ...
    it('forwards attach bytes that arrive with the attachStream request', async () => {
        ...
        const handler = {
            ...
            list: vi.fn(() => []),
            shutdown: vi.fn(() => ({})),
            attachStream: vi.fn((_params, socket, requestId) => {
                socket.write(`${JSON.stringify({
                    id: requestId,
                    ok: true,
    ...
    it('preserves coalesced attach bytes for an async streaming handler', async () => {
        ...
        const handler = {
            ...
            list: vi.fn(() => []),
            shutdown: vi.fn(() => ({})),
            attachStream: vi.fn(async (_params, socket, requestId) => {
                socket.write(`${JSON.stringify({
                    id: requestId,
                    ok: true,
                    result: { attached: true },
    ...
    it('binds class-based streaming handlers so subscribe succeeds', async () => {
        ...
        try {
            ...
            socket.setEncoding('utf8');
            await new Promise((resolve) => socket.once('connect', resolve));
            socket.write(`${JSON.stringify({ id: 'subscribe-request', op: 'subscribe' })}\n`);
    ...
    it('delivers attach stream bytes without utf8 transcoding', async () => {
        ...
        try {
            ...
            const requestLine = Buffer.from(`${JSON.stringify({
                id: 'attach-request',
                op: 'attachStream',
                params: { sessionId: 'session-1' },
            })}\n`, 'utf8');
            socket.write(Buffer.concat([requestLine, payload]));
            await waitFor(() => Buffer.concat(received).byteLength >= payload.byteLength);
            expect(Buffer.concat(received).subarray(0, payload.byteLength)).toEqual(payload);
            socket.destroy();
        }
        finally {
            await server.close();
    ...
    it('rejects with a timeout error when the server accepts but never responds', async () => {
        ...
        await new Promise((resolve) => silentServer.listen(socketPath, resolve));
        try {
            await expect(requestAgentViewSupervisor(socketPath, { id: 'timeout-test', op: 'status' }, { timeoutMs: 50 })).rejects.toMatchObject({ code: 'timeout' });
        }
        finally {
...
//# sourceMappingURL=supervisor-server.test.js.map