...
import { mkdtempSync, rmSync, readFileSync, existsSync, mkdirSync, writeFileSync, symlinkSync, statSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { dirname, join, isAbsolute, relative, resolve } from 'node:path';
import { skillArgsPath, writeSkillArgs, clearSkillArgs, } from './skill-args-file.js';
let dir;
let cwd;
beforeEach(() => {
...
describe('writeSkillArgs', () => {
    ...
    it.skipIf(process.platform === 'win32')('writes the file mode 0600 — arguments can carry a secret', () => {
    ...
    it('refuses to write through a symlinked session directory', () => {
        ...
        try {
            ...
            expect(readFileSync(join(victim, 'qwen-skill-args-review.txt'), 'utf8')).toBe('precious');
        }
        finally {
            if (prev === undefined)
                delete process.env['QWEN_CODE_SESSION_ID'];
...
//# sourceMappingURL=skill-args-file.test.js.map