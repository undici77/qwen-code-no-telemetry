...
import { GITHUB_WORKFLOW_PATHS, MAX_WORKFLOW_DOWNLOAD_BYTES, SetupGithubError, setupGithub, updateGitignore, } from './setup-github.js';
const mockProxyAgent = vi.hoisted(() => vi.fn((proxy) => ({ proxy })));
...
describe('setupGithub service', () => {
    ...
    beforeEach(async () => {
        vi.resetAllMocks();
        mockProxyAgent.mockImplementation((proxy) => ({ proxy }));
        scratchDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'setup-github-service-'));
    ...
    it('downloads latest release workflows with configured proxy', async () => {
        ...
        expect(gitUtils.getLatestGitHubRelease).toHaveBeenCalledWith('http://proxy.local:8080');
        ...
        expect(dispatchers.every((dispatcher) => dispatcher === dispatchers[0])).toBe(true);
        expect(result.releaseTag).toBe('v1.2.3');
        expect(result.secretsUrl).toBe('https://github.com/owner/repo/settings/secrets/actions');
        expect(result.gitignore.status).toBe('created');
        for (const workflow of GITHUB_WORKFLOW_PATHS) {
            const target = path.join(scratchDir, '.github', 'workflows', path.basename(workflow));
            await expect(fsp.readFile(target, 'utf8')).resolves.toContain(path.basename(workflow));
        }
    });
    it('fails before writing when release lookup fails', async () => {
        vi.mocked(gitUtils.getLatestGitHubRelease).mockRejectedValue(new Error('offline'));
        const fetchImpl = vi.fn();
        await expect(setupGithub({ cwd: scratchDir, workspaceRoot: scratchDir, fetchImpl })).rejects.toMatchObject({
            code: 'github_release_lookup_failed',
            status: 502,
        });
        await expect(fsp.access(path.join(scratchDir, '.github'))).rejects.toBeDefined();
    ...
    it('checks write permission before release lookup and downloads', async () => {
        ...
        const fileOps = {
            assertCanWrite: vi.fn(() => {
                throw new SetupGithubError('github_setup_untrusted_workspace', 'workspace is not trusted', 403);
            }),
    ...
    it('aborts sibling workflow downloads after the first download failure', async () => {
        ...
        await expect(setupGithub({ cwd: scratchDir, workspaceRoot: scratchDir, fetchImpl })).rejects.toThrow(/qwen-invoke\.yml/);
        expect(abortedEndpoints.length).toBeGreaterThan(0);
    });
    it('reports partial workflow write failure after updating gitignore', async () => {
        const fetchImpl = vi.fn(async () => okResponse('workflow'));
    ...
    it('preserves a closed-generation error from the gitignore commit', async () => {
        ...
        await expect(setupGithub({
            cwd: scratchDir,
            workspaceRoot: scratchDir,
            fetchImpl: vi.fn(async () => okResponse('workflow')),
            fileOps,
    ...
    it('updates gitignore idempotently', async () => {
        ...
        await expect(fsp.readFile(gitignorePath, 'utf8')).resolves.toBe('.qwen/\nnode_modules/\n\ngha-creds-*.json\n');
    });
    it('rejects when git root is outside bound workspace', async () => {
        const otherWorkspace = await fsp.mkdtemp(path.join(os.tmpdir(), 'setup-github-other-'));
        try {
            await expect(setupGithub({
                cwd: scratchDir,
...
//# sourceMappingURL=setup-github.test.js.map