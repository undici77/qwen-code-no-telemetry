...
import { getCommandSourceBadge, getCommandSourceGroup, formatSupportedModes, getCommandDisplayName, getCommandSubcommandNames, formatCommandSourceLabel, } from './commandMetadata.js';
...
describe('getCommandSourceBadge', () => {
    it('returns null for builtin-command', () => {
        expect(getCommandSourceBadge(makeCmd({ source: 'builtin-command' }))).toBeNull();
    });
    it('returns [Skill] for bundled-skill', () => {
        expect(getCommandSourceBadge(makeCmd({ source: 'bundled-skill' }))).toBe('[Skill]');
    });
    it('returns [Custom] for skill-dir-command with User label but no source detail', () => {
        expect(getCommandSourceBadge(makeCmd({ source: 'skill-dir-command', sourceLabel: 'User' }))).toBe('[Custom]');
    });
    it('returns [User] for localized skill-dir-command with user source detail', () => {
        expect(getCommandSourceBadge(makeCmd({
    ...
    it('returns [Custom] for skill-dir-command with Project label but no source detail', () => {
        expect(getCommandSourceBadge(makeCmd({ source: 'skill-dir-command', sourceLabel: 'Project' }))).toBe('[Custom]');
    });
    ...
    it('returns [Custom] for skill-dir-command with other label', () => {
        expect(getCommandSourceBadge(makeCmd({ source: 'skill-dir-command', sourceLabel: 'Other' }))).toBe('[Custom]');
    });
    it('returns [Plugin] for plugin-command with Extension: prefix but no source detail', () => {
        expect(getCommandSourceBadge(makeCmd({ source: 'plugin-command', sourceLabel: 'Extension: my-ext' }))).toBe('[Plugin]');
    });
    ...
    it('returns [Plugin] for plugin-command without Extension: prefix', () => {
        expect(getCommandSourceBadge(makeCmd({ source: 'plugin-command', sourceLabel: 'My Plugin' }))).toBe('[Plugin]');
    });
    it('returns [MCP] for mcp-prompt', () => {
        expect(getCommandSourceBadge(makeCmd({ source: 'mcp-prompt' }))).toBe('[MCP]');
    });
    it('returns null for unknown source (default branch)', () => {
        expect(getCommandSourceBadge(makeCmd({ source: 'unknown-source' }))).toBeNull();
    });
});
// ---------------------------------------------------------------------------
...
describe('getCommandSourceGroup', () => {
    ...
    it('returns other group for unknown source', () => {
        const g = getCommandSourceGroup(makeCmd({ source: 'unknown-source' }));
...
describe('getCommandDisplayName', () => {
    ...
    it('appends matched alias when provided', () => {
        ...
        expect(getCommandDisplayName(cmd, { prefix: '/', matchedAlias: 'usage' })).toBe('/stats (alias: usage)');
    });
    ...
    it('returns names of non-hidden subCommands', () => {
        const cmd = makeCmd({
            subCommands: [
                ...
                { name: 'remove', hidden: true },
                { name: 'list', hidden: false },
            ],
        });
...
//# sourceMappingURL=commandMetadata.test.js.map