...
import { buildWorkspaceVoiceSettingsWrites, hasConfiguredBatchVoiceTranscriptionModel, listAvailableVoiceModels, transcribeWorkspaceVoiceAudio, validateWorkspaceVoiceConfig, validateWorkspaceVoiceModel, validateWorkspaceVoiceState, WorkspaceVoiceError, } from './voice-service.js';
...
function makeSettings(opts) {
    return new LoadedSettings(settingsFile({}), settingsFile({}), settingsFile(opts.user ?? {}), settingsFile(opts.workspace ?? {}), opts.isTrusted ?? true, new Set());
}
function expectWorkspaceVoiceError(action, code) {
    let caught;
    try {
        action();
    }
...
describe('voice service', () => {
    ...
    it('builds settings writes using voice and model persistence scopes', () => {
        ...
        expect(buildWorkspaceVoiceSettingsWrites(settings, {
            enabled: true,
            mode: 'tap',
            language: 'english',
    ...
    it('uses user scope for voice settings when workspace trust is not explicit', () => {
        ...
        expect(buildWorkspaceVoiceSettingsWrites(settings, {
            ...
            language: 'english',
            enabled: true,
        }, { workspaceTrusted: false })).toEqual([
            {
    ...
    it('uses user scope when a trusted workspace does not own voice enabled', () => {
        ...
        expect(buildWorkspaceVoiceSettingsWrites(settings, { mode: 'tap' }, { workspaceTrusted: true })).toEqual([
            {
                scope: SettingScope.User,
                key: 'general.voice.mode',
                value: 'tap',
    ...
    it('applies a workspace scope override to every voice setting', () => {
        ...
        expect(buildWorkspaceVoiceSettingsWrites(settings, {
            ...
            language: 'english',
            enabled: true,
        }, { scopeOverride: SettingScope.Workspace })).toEqual([
            {
                scope: SettingScope.Workspace,
                key: 'voiceModel',
                value: 'qwen3-asr-flash',
    ...
    it('requires an effective voice model before enabling voice', () => {
        const settings = makeSettings({ user: {} });
        expect(() => validateWorkspaceVoiceState(settings, { enabled: true })).toThrowError(WorkspaceVoiceError);
    });
    it('allows disabling voice without a configured voice model', () => {
        const settings = makeSettings({ user: {} });
        expect(() => validateWorkspaceVoiceState(settings, { enabled: false })).not.toThrow();
    });
    ...
    it('resolves voice models in providerProtocol-mapped custom provider groups', () => {
        ...
        expect(hasConfiguredBatchVoiceTranscriptionModel(settings)).toBe(true);
        expect(listAvailableVoiceModels(settings).map((model) => model.id)).toContain('qwen3-asr-flash');
    });
    it('rejects unknown, duplicate, and unsupported voice model selections', () => {
        ...
        expectWorkspaceVoiceError(() => validateWorkspaceVoiceModel(settings, 'not-configured'), 'unknown_voice_model');
        expectWorkspaceVoiceError(() => validateWorkspaceVoiceModel(settings, 'qwen3-asr-flash'), 'ambiguous_voice_model');
        expectWorkspaceVoiceError(() => validateWorkspaceVoiceModel(settings, 'gpt-4o-mini'), 'unsupported_voice_model');
    });
    it('wraps invalid voice model configuration errors', () => {
        ...
        expectWorkspaceVoiceError(() => validateWorkspaceVoiceConfig(settings, 'qwen3-asr-flash'), 'invalid_voice_model');
    });
    it('uses the supplied runtime environment for validation and transcription', async () => {
        ...
        expectWorkspaceVoiceError(() => validateWorkspaceVoiceState(settings, { enabled: true, voiceModel: 'qwen3-asr-flash' }, { env: runtimeEnv }), 'invalid_voice_model');
        await expect(transcribeWorkspaceVoiceAudio({
