...
describe('CommandService', () => {
    ...
    it('should exclude non-user-invocable commands from user command modes', async () => {
        ...
        const service = await CommandService.create([new MockCommandLoader([userCommand, modelOnlyCommand])], new AbortController().signal);
        expect(service.getCommands().map((cmd) => cmd.name)).toEqual([
            'user-command',
            'model-only-command',
        ]);
        expect(service.getCommandsForMode('interactive').map((cmd) => cmd.name)).toEqual(['user-command']);
    ...
    it('should exclude commands that are neither user nor model invocable', async () => {
        ...
        const service = await CommandService.create([new MockCommandLoader([hiddenCommand])], new AbortController().signal);
        expect(service.getCommands().map((cmd) => cmd.name)).toEqual([
            'hidden-command',
        ]);
    ...
    describe('disabled commands (disabledNames parameter)', () => {
        it('should exclude commands whose names are in the disabledNames set', async () => {
            ...
            const service = await CommandService.create([mockLoader], new AbortController().signal, new Set(['command-a']));
            const commands = service.getCommands();
            expect(commands).toHaveLength(2);
        ...
        it('should match disabled names case-insensitively', async () => {
            const mockLoader = new MockCommandLoader([mockCommandA, mockCommandB]);
            const service = await CommandService.create([mockLoader], new AbortController().signal, new Set(['COMMAND-A', 'Command-B']));
            const commands = service.getCommands();
            expect(commands).toHaveLength(0);
        });
        it('should not filter any commands when disabledNames is empty', async () => {
            const mockLoader = new MockCommandLoader([mockCommandA, mockCommandB]);
            const service = await CommandService.create([mockLoader], new AbortController().signal, new Set());
            expect(service.getCommands()).toHaveLength(2);
        });
        it('should not filter any commands when disabledNames is undefined', async () => {
            const mockLoader = new MockCommandLoader([mockCommandA, mockCommandB]);
            const service = await CommandService.create([mockLoader], new AbortController().signal, undefined);
            expect(service.getCommands()).toHaveLength(2);
        });
    });
});
