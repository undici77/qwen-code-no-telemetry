...
export async function setupGithub(options = {}) {
    ...
    try {
        ...
        writeStderrLine(`qwen setup-github: failed to determine latest qwen-code-action release: ${error instanceof Error ? error.message : String(error)}`);
        debugLogger.debug('Failed to determine latest qwen-code-action release:', error);
        throw new SetupGithubError('github_release_lookup_failed', 'Unable to determine the latest qwen-code-action release on GitHub.', 502);
    }
...
async function downloadWorkflows(options) {
    ...
    try {
        ...
        throw new SetupGithubError('github_workflow_download_failed', `Unable to download qwen-code-action workflows from GitHub. ${message}`, 502);
    }
}
