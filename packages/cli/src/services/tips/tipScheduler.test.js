...
const tempPaths = [];
function tmpPath() {
    const p = join(tmpdir(), `test-scheduler-${Date.now()}-${Math.random().toString(36).slice(2)}.json`);
    tempPaths.push(p);
    return p;
}
afterEach(() => {
...
describe('selectTip', () => {
    it('returns null for empty tips', () => {
        const result = selectTip('post-response', createContext(), [], createHistory());
        expect(result).toBeNull();
    });
    it('filters by trigger type', () => {
        const history = createHistory();
        const result = selectTip('startup', createContext(), [tipA, tipB, tipC], history);
        expect(result).not.toBeNull();
        expect(result.id).toBe('tip-c');
    });
    it('selects highest priority tip', () => {
        const history = createHistory();
        const result = selectTip('post-response', createContext(), [tipA, tipB], history);
        expect(result.id).toBe('tip-b');
    });
    it('uses LRU for equal priority — prefers never-shown tip', () => {
        const history = createHistory();
        const tipX = { ...tipA, id: 'tip-x', priority: 10 };
        const tipY = { ...tipA, id: 'tip-y', priority: 10 };
    ...
    it('skips tips where isRelevant returns false', () => {
        ...
        const result = selectTip('post-response', createContext(), [irrelevant, tipA], history);
    ...
    it('handles isRelevant throwing an error', () => {
        ...
        const result = selectTip('post-response', createContext(), [broken, tipA], history);
        expect(result.id).toBe('tip-a');
    });
});
//# sourceMappingURL=tipScheduler.test.js.map