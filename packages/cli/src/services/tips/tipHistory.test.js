...
const tempPaths = [];
function tmpPath() {
    const p = join(tmpdir(), `test-tip-history-${Date.now()}-${Math.random().toString(36).slice(2)}.json`);
    tempPaths.push(p);
    return p;
}
afterEach(() => {
...
function createHistory(sessionCount = 1) {
...
describe('TipHistory', () => {
    ...
    describe('getLastShown', () => {
        ...
        it('returns high score after recordShown (session-shown offset)', () => {
            ...
            expect(history.getLastShown('tip-a')).toBe(Number.MAX_SAFE_INTEGER - 1_000_000 + 7);
        });
        it('updates on subsequent recordShown calls', () => {
            const history = createHistory();
            history.recordShown('tip-a', 3);
            history.recordShown('tip-a', 10);
            expect(history.getLastShown('tip-a')).toBe(Number.MAX_SAFE_INTEGER - 1_000_000 + 10);
        });
        ...
        it('session-shown tips always sort after cross-session-only tips', () => {
            ...
            expect(history.getLastShown('tip-old')).toBeLessThan(history.getLastShown('tip-new'));
        });
    });
...
//# sourceMappingURL=tipHistory.test.js.map