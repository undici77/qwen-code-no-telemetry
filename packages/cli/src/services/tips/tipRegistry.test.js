...
import { tipRegistry } from './tipRegistry.js';
const baseCtx = {
...
describe('context-* tip thresholds align with computeThresholds', () => {
    it('compress-intro fires between warn and auto', () => {
        ...
        expect(t.isRelevant({ ...baseCtx, lastPromptTokenCount: 100_000 })).toBe(false);
        expect(t.isRelevant({ ...baseCtx, lastPromptTokenCount: 150_000 })).toBe(true);
        expect(t.isRelevant({ ...baseCtx, lastPromptTokenCount: 168_000 })).toBe(false);
    });
    it('context-high fires between auto and hard', () => {
        const t = tipById('context-high');
        expect(t.isRelevant({ ...baseCtx, lastPromptTokenCount: 150_000 })).toBe(false);
        expect(t.isRelevant({ ...baseCtx, lastPromptTokenCount: 170_000 })).toBe(true);
        expect(t.isRelevant({ ...baseCtx, lastPromptTokenCount: 178_000 })).toBe(false);
    });
    it('context-critical fires at or above hard', () => {
        const t = tipById('context-critical');
        expect(t.isRelevant({ ...baseCtx, lastPromptTokenCount: 170_000 })).toBe(false);
        expect(t.isRelevant({ ...baseCtx, lastPromptTokenCount: 178_000 })).toBe(true);
    });
...
//# sourceMappingURL=tipRegistry.test.js.map