...
This is a test prompt from markdown.`;
    ...
    it('should load markdown commands without frontmatter', async () => {
        ...
        try {
            const commands = await loader.loadCommands(new AbortController().signal);
            const simpleCommand = commands.find((cmd) => cmd.name === 'simple-command');
    ...
    it('should load markdown commands with BOM and CRLF frontmatter', async () => {
        ...
        try {
            const commands = await loader.loadCommands(new AbortController().signal);
            const windowsCommand = commands.find((cmd) => cmd.name === 'windows-command');
...
Markdown prompt`;
        await fs.writeFile(path.join(tempDir, 'toml-cmd.toml'), tomlContent, 'utf-8');
...
//# sourceMappingURL=FileCommandLoader-markdown.test.js.map