...
Third line of prompt.`;
        const result = parseMarkdownCommand(content);
        expect(result.prompt).toBe('First line of prompt.\nSecond line of prompt.\nThird line of prompt.');
    });
...
Prompt content.`;
    ...
    it('should parse frontmatter in CR-only files', () => {
        const content = '---\rdescription: Old mac command\r---\r\rLine 1\rLine 2\r';
        const result = parseMarkdownCommand(content);
        expect(result).toEqual({
            frontmatter: {
...
//# sourceMappingURL=markdown-command-parser.test.js.map