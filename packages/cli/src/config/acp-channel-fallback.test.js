...
import { QWEN_CODE_DESKTOP_ENV, QWEN_CODE_SERVE_ENV, resolveAcpChannelFallback, } from './acp-channel-fallback.js';
describe('resolveAcpChannelFallback', () => {
    ...
    it('reports daemon for daemon-spawned children', () => {
        expect(resolveAcpChannelFallback({ [QWEN_CODE_SERVE_ENV]: '1' })).toBe('daemon');
    });
    it('reports desktop for the Tauri desktop shell', () => {
        expect(resolveAcpChannelFallback({ [QWEN_CODE_DESKTOP_ENV]: '1' })).toBe('desktop');
        // Tauri sessions are daemon-spawned too; the launcher identity wins.
        expect(resolveAcpChannelFallback({
...
//# sourceMappingURL=acp-channel-fallback.test.js.map