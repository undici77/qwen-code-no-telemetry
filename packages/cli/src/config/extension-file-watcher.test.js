...
const { mockWatchers, mockWatch, mockExistsSync, mockReadFileSync } = vi.hoisted(() => {
    const mockWatchers = [];
...
function createRefreshState() {
    return {
        ...
        beginSuppression: vi.fn((onSettle) => () => onSettle?.()),
    };
}
...
describe('ExtensionFileWatcher', () => {
    ...
    it('watches the extensions directory and linked extension sources', () => {
        ...
        const watcher = new ExtensionFileWatcher(configWithExtensions([
            {
                path: linkedSource,
                config: {},
                installMetadata: {
        ...
        expect(mockWatchers[0].options).toEqual(expect.objectContaining({
            ignoreInitial: true,
    ...
    it('marks refresh needed for manual extension add and remove events', () => {
        const refreshState = createRefreshState();
        const watcher = new ExtensionFileWatcher(configWithExtensions([]), extensionsDir, refreshState);
        watcher.startWatching();
    ...
    it('marks refresh needed when another process commits a store generation', () => {
        ...
        const watcher = new ExtensionFileWatcher(configWithExtensions([]), extensionsDir, refreshState);
        watcher.startWatching();
        fireAllEvent(0, 'change', path.join(path.dirname(extensionsDir), 'extension-store', 'state.json'));
        expect(refreshState.markExtensionsChanged).toHaveBeenCalledWith('extension store generation changed');
    });
    ...
    it('retries a generation suppressed after an internal mutation', () => {
        ...
        const manager = {
            addMutationListener: vi.fn((listener) => {
        ...
        try {
            ...
            mockReadFileSync.mockReturnValue('{"generation":2}');
            fireAllEvent(mockWatchers.length - 1, 'change', path.join(path.dirname(extensionsDir), 'extension-store', 'state.json'));
    ...
    it('treats content events as stale when the extension manifest is gone', () => {
        mockExistsSync.mockImplementation((filePath) => !filePath.endsWith(path.join('alpha', 'qwen-extension.json')));
        const refreshState = createRefreshState();
        const watcher = new ExtensionFileWatcher(configWithExtensions([]), extensionsDir, refreshState);
        watcher.startWatching();
    ...
    it('ignores unrelated files', () => {
        ...
        expect(refreshState.markExtensionsChanged).not.toHaveBeenCalled();
        const ignored = mockWatchers[0].options['ignored'];
        expect(ignored('C:/Users/me/.qwen/extensions/alpha/.git/config')).toBe(true);
        expect(ignored('C:/Users/me/.qwen/extensions/alpha/node_modules/pkg/index.js')).toBe(true);
    });
    it('bootstraps on the parent when the extensions directory does not exist', () => {
        ...
        expect(mockWatchers[1].options).toEqual(expect.objectContaining({
            ignoreInitial: true,
            followSymlinks: false,
            depth: 0,
        }));
...
//# sourceMappingURL=extension-file-watcher.test.js.map