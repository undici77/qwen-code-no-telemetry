...
describe('createLoadedSettingsAdapter', () => {
    it('forwards setValue to LoadedSettings.setValue with the resolved scope', () => {
        ...
        const adapter = createLoadedSettingsAdapter(settings, SettingScope.User);
        adapter.setValue('env.MY_KEY', 'val');
        expect(setValue).toHaveBeenCalledWith(SettingScope.User, 'env.MY_KEY', 'val');
    });
    it('rejects prototype-pollution keys before reaching LoadedSettings', () => {
        const { settings, setValue } = makeSettings();
        const adapter = createLoadedSettingsAdapter(settings, SettingScope.User);
        expect(() => adapter.setValue('__proto__.polluted', 'x')).toThrow(/reserved segment/);
        expect(() => adapter.setValue('foo.constructor.bar', 'x')).toThrow(/reserved segment/);
        expect(() => adapter.setValue('prototype.x', 'x')).toThrow(/reserved segment/);
...
//# sourceMappingURL=loadedSettingsAdapter.test.js.map