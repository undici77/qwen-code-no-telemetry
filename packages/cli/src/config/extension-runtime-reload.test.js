...
import { refreshExtensionContentRuntime, reloadPluginsRuntime, } from './extension-runtime-reload.js';
describe('reloadPluginsRuntime', () => {
    ...
    it('rejects without refreshing extensions in safe mode', async () => {
        ...
        await expect(reloadPluginsRuntime({ config, reloadCommands })).rejects.toThrow('Extension reload is disabled in safe mode.');
...
describe('refreshExtensionContentRuntime', () => {
    ...
    it('continues refreshing content before reporting refresh failures', async () => {
        ...
        await expect(refreshExtensionContentRuntime({ config, reloadCommands })).rejects.toThrow('cache failed; skills failed');
        expect(refreshCache).toHaveBeenCalledOnce();
        expect(refreshSkillCache).toHaveBeenCalledOnce();
        expect(refreshSubagentCache).toHaveBeenCalledOnce();
        expect(reloadCommands).toHaveBeenCalledOnce();
    ...
    it('continues refreshing content when subagent manager is not initialized', async () => {
        ...
        await expect(refreshExtensionContentRuntime({ config, reloadCommands })).resolves.toBeUndefined();
        expect(refreshCache).toHaveBeenCalledOnce();
        expect(refreshSkillCache).toHaveBeenCalledOnce();
        expect(reloadCommands).toHaveBeenCalledOnce();
    });
...
//# sourceMappingURL=extension-runtime-reload.test.js.map