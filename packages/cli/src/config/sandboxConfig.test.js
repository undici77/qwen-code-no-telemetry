...
const { loadSandboxConfig, resetSandboxProbeCacheForTest } = await import('./sandboxConfig.js');
 ...
 * never answers.
 */
function probes(byCommand) {
    spawnSync.mockImplementation((cmd, args, options) => {
        const result = byCommand[cmd];
        if (!result)
            throw new Error(`unexpected probe of '${cmd}'`);
        expect(args).toEqual(['version']);
        expect(options?.timeout).toBeGreaterThan(0);
        return result;
    });
}
function installed(...commands) {
    commandExistsSync.mockImplementation((cmd) => commands.includes(cmd));
}
describe('loadSandboxConfig sandbox command selection', () => {
    ...
    it('falls back to podman when docker is installed but its daemon is unreachable', async () => {
        ...
        expect(spawnSync).toHaveBeenCalledWith('docker', ['version'], expect.objectContaining({ timeout: expect.any(Number) }));
    });
    ...
    it('probes each runtime once and reuses the result across calls', async () => {
        ...
        await loadSandboxConfig({}, { sandbox: true });
        const dockerProbes = spawnSync.mock.calls.filter(([cmd]) => cmd === 'docker').length;
    ...
    it('names the runtime that broke when no installed runtime can run', async () => {
        ...
        await expect(loadSandboxConfig({}, { sandbox: true })).rejects.toThrow(/docker.*cannot run.*Cannot connect to the Docker daemon/s);
    });
    it('names the first broken runtime when every installed runtime fails', async () => {
        ...
        const error = await loadSandboxConfig({}, { sandbox: true }).catch((e) => e);
    ...
    it('strips ANSI and control characters from the runtime failure', async () => {
        ...
        const error = await loadSandboxConfig({}, {}).catch((e) => e);
        const message = error.message;
    ...
    it('does not blame QWEN_SANDBOX when --sandbox enabled the auto-detect path', async () => {
        installed('docker');
        probes({ docker: daemonDown() });
        const failure = await loadSandboxConfig({}, { sandbox: true }).catch((error) => error);
        expect(failure.message).toContain("'docker' is installed");
        expect(failure.message).toContain('--sandbox');
        expect(failure.message).not.toContain('QWEN_SANDBOX is true');
    });
    it('says QWEN_SANDBOX is true when the env var enabled the sandbox', async () => {
        process.env['QWEN_SANDBOX'] = 'true';
        installed('docker');
        probes({ docker: daemonDown() });
        await expect(loadSandboxConfig({}, {})).rejects.toThrow(/QWEN_SANDBOX is true and 'docker' is installed but cannot run/);
    });
    it('keeps the generic message when nothing is installed at all', async () => {
        installed();
        probes({});
        await expect(loadSandboxConfig({}, { sandbox: true })).rejects.toThrow(/failed to determine command for sandbox/);
    });
    it('does not silently override an explicit QWEN_SANDBOX choice', async () => {
        process.env['QWEN_SANDBOX'] = 'docker';
        installed('docker', 'podman');
        probes({ docker: daemonDown(), podman: healthy() });
        await expect(loadSandboxConfig({}, {})).rejects.toThrow(/'docker' \(from QWEN_SANDBOX\) is installed but cannot run/);
    });
    it('does not blame QWEN_SANDBOX for a command that came from --sandbox', async () => {
        ...
        const failure = await loadSandboxConfig({}, { sandbox: 'docker' }).catch((error) => error);
        expect(failure.message).toContain("'docker' is installed");
        expect(failure.message).not.toContain('QWEN_SANDBOX');
    });
    it('names QWEN_SANDBOX when a missing command really did come from it', async () => {
        process.env['QWEN_SANDBOX'] = 'podman';
        installed('docker');
        probes({});
        await expect(loadSandboxConfig({}, {})).rejects.toThrow(/Missing sandbox command 'podman' \(from QWEN_SANDBOX\)/);
    });
