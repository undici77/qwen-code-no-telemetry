...
function decision(config, location) {
    return resolveTrustDecision(buildTrustPrecedenceRules(Object.entries(config).map(([path, trustLevel]) => ({
        path,
        trustLevel,
...
describe('trust precedence', () => {
    it('uses the deepest matching rule in either direction', () => {
        const config = {
            '/projects': 'TRUST_FOLDER',
            '/projects/evil': 'DO_NOT_TRUST',
            '/projects/good': 'TRUST_FOLDER',
        };
    ...
    it('collapses TRUST_PARENT before applying specificity', () => {
        const config = {
            '/projects': 'DO_NOT_TRUST',
            '/projects/good/marker': 'TRUST_PARENT',
        };
        expect(decision(config, '/projects/good/src')).toBe(true);
        expect(decision(config, '/projects/other/src')).toBe(false);
    });
    it('returns undefined when no rule contains the location', () => {
        expect(decision({ '/projects': 'TRUST_FOLDER' }, '/other/project')).toBeUndefined();
    });
});
//# sourceMappingURL=trust-precedence.test.js.map