...
describe('settings/trustedFolders path getters are lazy', () => {
    ...
    afterEach(() => {
        ...
        if (originalTrustedPath === undefined)
            ...
            process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'] = originalTrustedPath;
    });
    it('getUserSettingsPath() reflects QWEN_HOME set after module load', () => {
        ...
        expect(getUserSettingsPath()).toBe(path.join('/tmp/qwen-lazy-test', 'settings.json'));
    });
    ...
    it('getTrustedFoldersPath() reflects QWEN_HOME set after module load', () => {
        expect(getTrustedFoldersPath()).toBe(path.join(homedir(), '.qwen', 'trustedFolders.json'));
        process.env['QWEN_HOME'] = '/tmp/qwen-lazy-test';
        expect(getTrustedFoldersPath()).toBe(path.join('/tmp/qwen-lazy-test', 'trustedFolders.json'));
    });
});
//# sourceMappingURL=path-freshness.test.js.map