...
import { computeWorkspaceSkillListUpdates, resolveSkillSettings, updateWorkspaceSkillSettingLists, } from './skill-settings.js';
function fakeSettings({ merged, system = {}, systemDefaults = {}, user = {}, workspace = {}, }) {
    const byScope = {
...
describe('resolveSkillSettings', () => {
    it('lets a workspace opt-in override a user default case-insensitively', () => {
        const result = resolveSkillSettings(fakeSettings({
            merged: {
...
describe('updateWorkspaceSkillSettingLists', () => {
    it('persists a canonical opt-in for a default-disabled skill', () => {
        expect(updateWorkspaceSkillSettingLists({ disabled: ['orphan'], enabled: [] }, 'Review', true, true)).toEqual({ disabled: ['orphan'], enabled: ['Review'] });
    });
    it('keeps unrelated entries and canonicalizes matching entries', () => {
        expect(updateWorkspaceSkillSettingLists({
            disabled: ['orphan', ' REVIEW ', 'review'],
            enabled: ['other', 'ReViEw'],
        }, 'review', false, true)).toEqual({ disabled: ['orphan', 'review'], enabled: ['other'] });
    });
    it('does not add a redundant opt-in for an ordinary skill', () => {
        expect(updateWorkspaceSkillSettingLists({ disabled: ['review'], enabled: [] }, 'review', true, false)).toEqual({ disabled: [], enabled: [] });
    });
    it('does not reorder an already canonical hard disable', () => {
        expect(updateWorkspaceSkillSettingLists({ disabled: ['review', 'orphan'], enabled: [] }, 'review', false, false)).toEqual({ disabled: ['review', 'orphan'], enabled: [] });
    });
});
describe('computeWorkspaceSkillListUpdates', () => {
    it('preserves orphaned workspace disables the picker does not manage', () => {
        ...
        // 'review' is toggled.
        const result = computeWorkspaceSkillListUpdates(['orphan', 'review'], new Set(), [], [
            {
                name: 'review',
                wasEnabled: false,
                isEnabled: true,
    ...
    it('drops locked higher-scope entries so they are not re-emitted', () => {
        ...
        const result = computeWorkspaceSkillListUpdates(['locked', 'orphan', 'review'], new Set(['locked']), [], [
            {
                name: 'review',
                wasEnabled: false,
                isEnabled: true,
    ...
    it('reports no change when nothing toggled and lists already match', () => {
        const result = computeWorkspaceSkillListUpdates(['orphan'], new Set(), [], [
            {
                name: 'review',
                wasEnabled: true,
                isEnabled: true,
    ...
    it('records an explicit opt-in when enabling a default-disabled skill', () => {
        const result = computeWorkspaceSkillListUpdates([], new Set(), [], [
            {
                name: 'Review',
                wasEnabled: false,
                isEnabled: true,
...
//# sourceMappingURL=skill-settings.test.js.map