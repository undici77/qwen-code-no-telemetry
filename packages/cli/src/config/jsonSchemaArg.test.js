...
describe('resolveJsonSchemaArg', () => {
    ...
    it('parses an inline JSON literal into a schema object', () => {
        const schema = resolveJsonSchemaArg('{"type":"object","properties":{"summary":{"type":"string"}}}');
    ...
    it('throws when the parsed value is not an object', () => {
        ...
        expect(() => resolveJsonSchemaArg('"just a string"')).toThrow(/must be a JSON object/);
    });
    it('throws when the referenced file does not exist', () => {
        expect(() => resolveJsonSchemaArg('@/this/path/does/not/exist.json')).toThrow(/could not read/);
    });
    it('rejects @path that resolves to a directory', () => {
        ...
        try {
            expect(() => resolveJsonSchemaArg(`@${tmp}`)).toThrow(/must be a regular file/);
        }
        finally {
            fs.rmSync(tmp, { recursive: true, force: true });
        }
    ...
    it('rejects @path schema files that exceed the size cap', () => {
        ...
        try {
            expect(() => resolveJsonSchemaArg(`@${file}`)).toThrow(/Refusing to read/);
        }
        finally {
            fs.rmSync(tmp, { recursive: true, force: true });
        }
    ...
    it('throws when schema is syntactically JSON but invalid as a JSON Schema', () => {
        ...
        expect(() => resolveJsonSchemaArg('{"properties":{"foo":{"type":42}}}')).toThrow(/not a valid JSON Schema/);
    });
    ...
    it('accepts a draft-2020-12 schema', () => {
        const schema = resolveJsonSchemaArg('{"$schema":"https://json-schema.org/draft/2020-12/schema","type":"object"}');
        expect(schema).toBeDefined();
    });
    it('rejects a schema whose root type is not object', () => {
        expect(() => resolveJsonSchemaArg('{"type":"array"}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"type":"string"}')).toThrow(/must accept object-typed values/);
    });
    ...
    it('rejects root anyOf where no branch accepts object', () => {
        expect(() => resolveJsonSchemaArg('{"anyOf":[{"type":"array"},{"type":"string"}]}')).toThrow(/must accept object-typed values/);
    });
    it('rejects root oneOf where no branch accepts object', () => {
        expect(() => resolveJsonSchemaArg('{"oneOf":[{"type":"number"},{"type":"boolean"}]}')).toThrow(/must accept object-typed values/);
    });
    it('accepts root anyOf when at least one branch accepts object', () => {
        const schema = resolveJsonSchemaArg('{"anyOf":[{"type":"object"},{"type":"string"}]}');
        expect(schema).toBeDefined();
    });
    it('accepts nested anyOf/oneOf chains where a deep branch accepts object', () => {
        ...
        const schema = resolveJsonSchemaArg('{"anyOf":[{"oneOf":[{"type":"object"}]},{"type":"string"}]}');
        expect(schema).toBeDefined();
    });
    it('rejects type:"object" combined with an anyOf that excludes object', () => {
        ...
        expect(() => resolveJsonSchemaArg('{"type":"object","anyOf":[{"type":"string"},{"type":"number"}]}')).toThrow(/must accept object-typed values/);
    });
    it('accepts type:"object" combined with anyOf where one branch admits object', () => {
        const schema = resolveJsonSchemaArg('{"type":"object","anyOf":[{"type":"object","properties":{"a":{"type":"string"}}},{"type":"object","properties":{"b":{"type":"number"}}}]}');
        expect(schema).toBeDefined();
    });
    it('rejects any root $ref, even with a sibling type:"object" anchor', () => {
        ...
        expect(() => resolveJsonSchemaArg('{"$ref":"#/$defs/Foo","$defs":{"Foo":{"type":"array"}}}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"type":"object","$ref":"#/$defs/Foo","$defs":{"Foo":{"type":"array"}}}')).toThrow(/must accept object-typed values/);
        ...
        expect(() => resolveJsonSchemaArg('{"type":"object","$ref":"#/$defs/Foo","$defs":{"Foo":{"type":"object","properties":{"a":{"type":"string"}}}}}')).toThrow(/must accept object-typed values/);
    });
    it('rejects allOf where any branch forbids object at the root', () => {
        ...
        expect(() => resolveJsonSchemaArg('{"allOf":[{"type":"object"},{"type":"string"}]}')).toThrow(/must accept object-typed values/);
    });
    it('accepts allOf where every branch admits object', () => {
        const schema = resolveJsonSchemaArg('{"allOf":[{"type":"object","properties":{"a":{"type":"string"}}},{"type":"object","required":["a"]}]}');
        expect(schema).toBeDefined();
    });
    it('rejects a root `not` that directly forbids object', () => {
        ...
        expect(() => resolveJsonSchemaArg('{"not":{"type":"object"}}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"not":{"type":["object","null"]}}')).toThrow(/must accept object-typed values/);
    });
    ...
    it('accepts root `not:{type:"object", ...narrowing}` because narrowing keywords leave some objects satisfiable', () => {
        ...
        // Ajv at runtime.
        expect(resolveJsonSchemaArg('{"not":{"type":"object","required":["error"]}}')).toBeDefined();
        expect(resolveJsonSchemaArg('{"not":{"type":"object","properties":{"k":{"type":"string"}},"required":["k"]}}')).toBeDefined();
        expect(resolveJsonSchemaArg('{"not":{"type":"object","minProperties":1}}')).toBeDefined();
    });
    it('rejects a root `const` whose value is not an object', () => {
        expect(() => resolveJsonSchemaArg('{"const":1}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"const":"hello"}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"const":[1,2]}')).toThrow(/must accept object-typed values/);
    });
    it('accepts a root `const` whose value is an object', () => {
        const schema = resolveJsonSchemaArg('{"const":{"summary":"hello","risk":"low"}}');
        expect(schema).toBeDefined();
    });
    it('rejects a root `enum` with no object members', () => {
        expect(() => resolveJsonSchemaArg('{"enum":[1,2,"three"]}')).toThrow(/must accept object-typed values/);
        // Empty enum admits nothing — also reject.
        expect(() => resolveJsonSchemaArg('{"enum":[]}')).toThrow(/must accept object-typed values/);
    });
    it('accepts a root `enum` when at least one member is an object', () => {
        const schema = resolveJsonSchemaArg('{"enum":[{"summary":"a","risk":"low"},{"summary":"b","risk":"high"}]}');
        expect(schema).toBeDefined();
    });
    it('rejects an empty root anyOf / oneOf as unsatisfiable', () => {
        expect(() => resolveJsonSchemaArg('{"anyOf":[]}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"oneOf":[]}')).toThrow(/must accept object-typed values/);
    });
    ...
    it('rejects anyOf where every branch is `false`', () => {
        ...
        expect(() => resolveJsonSchemaArg('{"anyOf":[false]}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"anyOf":[false,false]}')).toThrow(/must accept object-typed values/);
    });
    it('accepts $ref nested inside anyOf / oneOf / allOf branches', () => {
        ...
        const a = resolveJsonSchemaArg('{"anyOf":[{"$ref":"#/$defs/Foo"},{"type":"string"}],"$defs":{"Foo":{"type":"object"}}}');
        expect(a).toBeDefined();
        const b = resolveJsonSchemaArg('{"oneOf":[{"$ref":"#/$defs/A"},{"$ref":"#/$defs/B"}],"$defs":{"A":{"type":"object"},"B":{"type":"object"}}}');
        expect(b).toBeDefined();
        const c = resolveJsonSchemaArg('{"allOf":[{"$ref":"#/$defs/Bar"},{"type":"object"}],"$defs":{"Bar":{"type":"object"}}}');
        expect(c).toBeDefined();
    });
    it('handles boolean subschemas in allOf', () => {
        ...
        expect(() => resolveJsonSchemaArg('{"allOf":[false,{"type":"object"}]}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"allOf":[false]}')).toThrow(/must accept object-typed values/);
    });
    it('rejects if/then/else when the decidable branch admits no objects', () => {
        ...
        expect(() => resolveJsonSchemaArg('{"if":true,"then":{"type":"string"}}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"if":true,"then":false}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"if":false,"else":{"type":"array"}}')).toThrow(/must accept object-typed values/);
        expect(() => resolveJsonSchemaArg('{"if":false,"else":false}')).toThrow(/must accept object-typed values/);
    });
    it('accepts if/then/else when the decidable branch admits objects', () => {
        ...
        expect(resolveJsonSchemaArg('{"if":true,"then":{"type":"object"}}')).toBeDefined();
        // `if: false` + object-compatible `else`.
        expect(resolveJsonSchemaArg('{"if":false,"else":{"type":"object"}}')).toBeDefined();
        ...
        expect(resolveJsonSchemaArg('{"if":{"type":"object","properties":{"k":{"const":"x"}}},"then":{"type":"object","properties":{"v":{"type":"string"}}}}')).toBeDefined();
...
//# sourceMappingURL=jsonSchemaArg.test.js.map