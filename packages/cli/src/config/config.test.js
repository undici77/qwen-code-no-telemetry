...
import { ToolNames, DEFAULT_QWEN_MODEL, OutputFormat, NativeLspService, Storage, } from '@qwen-code/qwen-code-core';
import { isValidSessionId, loadCliConfig, parseArguments, SessionIdConflictError, } from './config.js';
...
const mockSessionServiceCtor = vi.hoisted(() => vi.fn(() => mockSessionServiceInstance));
...
describe('parseArguments', () => {
    ...
    it('rejects --json-schema combined with --acp', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('--json-schema cannot be used with --acp'));
        mockExit.mockRestore();
    });
    ...
    it('rejects --json-schema combined with --prompt-interactive (-i)', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('structured output only terminates the non-interactive flow'));
        mockExit.mockRestore();
    });
    it('rejects --json-schema combined with --input-format stream-json', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('first structured_output call ends the session'));
        mockExit.mockRestore();
    });
    ...
    it('should reject --fork-session without --resume or --continue', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('--fork-session must be used with --resume or --continue'));
        mockExit.mockRestore();
    });
    ...
    it('should reject --json-schema with no prompt source when stdin is a TTY', async () => {
        ...
        try {
            await expect(parseArguments()).rejects.toThrow('process.exit called');
            expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('--json-schema only applies to non-interactive mode'));
        }
        finally {
            mockExit.mockRestore();
    ...
    it('should throw when --json-schema is combined with --input-format stream-json', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('--json-schema cannot be used with --input-format stream-json'));
        mockExit.mockRestore();
    });
...
describe('loadCliConfig', () => {
    ...
    describe('usage statistics', () => {
        it.each([
            ...
            process.argv = ['node', 'script.js'];
            const argv = await parseArguments();
            const settings = settingValue === undefined
                ? {}
    ...
    it('should use settings fallback models when the CLI flag is absent', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({ modelFallbacks: ' settings-a , settings-b ' }, argv);
    ...
    it('passes agents.maxParallelAgents from settings to core config', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({ agents: { maxParallelAgents: 2 } }, argv);
    ...
    it('passes agents.maxParallelAgentsByModel from settings to core config', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({ agents: { maxParallelAgentsByModel: { 'weak-model': 1 } } }, argv);
    ...
    it('passes tools.shell.defaultTimeoutMs from settings to core config', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({ tools: { shell: { defaultTimeoutMs: 300000 } } }, argv);
    ...
    it('passes agents.builtin.exploreModel from settings to core config', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({ agents: { builtin: { exploreModel: 'fast' } } }, argv);
    ...
    it('should enable artifacts by default', async () => {
        ...
        const argv = await parseArguments();
        await loadCliConfig({}, argv);
        expect(mockConfigConstructorParams).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('places session-injected (ACP/IDE) MCP servers at the top precedence tier', async () => {
        ...
        const config = await loadCliConfig(settings, argv, process.cwd(), undefined, undefined, undefined, sessionMcpServers);
    ...
    it('keeps session-injected MCP servers ungated in non-interactive runs', async () => {
        ...
        }, argv, process.cwd(), undefined, undefined, undefined, {
            'ide-only': new ServerConfig.MCPServerConfig('ide-cmd'),
        });
        expect(config.isMcpServerPendingApproval('workspace-server')).toBe(true);
        expect(config.isMcpServerPendingApproval('ide-only')).toBe(false);
    ...
    it('should fork and load a new session when --resume is combined with --fork-session', async () => {
        ...
        mockSessionServiceInstance.loadSession.mockImplementation(async (sessionId) => {
        ...
        expect(mockSessionServiceInstance.forkSession).toHaveBeenCalledWith(sourceSessionId, config.getSessionId());
        expect(config.getSessionId()).toBe(mockSessionServiceInstance.forkSession.mock.calls[0]?.[1]);
        expect(mockSessionServiceInstance.loadSession).toHaveBeenCalledWith(config.getSessionId());
    });
    it('should explain when --fork-session fails to copy the source session', async () => {
        ...
        mockSessionServiceInstance.forkSession.mockRejectedValue(new Error('source session belongs to another project'));
        const mockExit = vi.spyOn(process, 'exit').mockImplementation(() => {
            throw new Error('process.exit called');
        });
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith(`Failed to fork session ${sourceSessionId}: source session belongs to another project`);
        expect(mockExit).toHaveBeenCalledWith(1);
    });
    it('should explain when --continue --fork-session has no saved session to fork', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith('Cannot use --fork-session with --continue: no saved session found to fork.');
        expect(mockExit).toHaveBeenCalledWith(1);
    });
    it('should exit when a caller-supplied sessionId already exists (default CLI behavior)', async () => {
        ...
        await expect(loadCliConfig({}, { sessionId })).rejects.toThrow('process.exit called');
        expect(mockExit).toHaveBeenCalledWith(1);
    });
    it('should throw SessionIdConflictError instead of exiting when throwOnSessionIdConflict is set', async () => {
        ...
        const promise = loadCliConfig({}, { sessionId }, undefined, undefined, undefined, undefined, undefined, undefined, true);
    ...
    it('should not throw for a fresh caller-supplied sessionId when throwOnSessionIdConflict is set', async () => {
        const sessionId = '123e4567-e89b-12d3-a456-426614174000';
        mockSessionServiceInstance.sessionExistsInAnyState.mockResolvedValue(false);
        const config = await loadCliConfig({}, { sessionId }, undefined, undefined, undefined, undefined, undefined, undefined, true);
    ...
    it('should reject direct use of the internal sandbox session ID flag', async () => {
        ...
        process.argv = ['node', 'script.js', '--sandbox-session-id', sessionId];
        const argv = await parseArguments();
        await expect(loadCliConfig({}, argv)).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith('--sandbox-session-id is for internal sandbox use only.');
        expect(mockExit).toHaveBeenCalledWith(1);
        expect(mockSessionServiceInstance.sessionExists).not.toHaveBeenCalled();
    });
    ...
    describe('web search settings resolution', () => {
        const loadWithSettings = async (settings) => {
            process.argv = ['node', 'script.js'];
            const argv = await parseArguments();
        ...
        it('disables web search in safe mode', async () => {
            process.argv = ['node', 'script.js', '--safe-mode'];
            const argv = await parseArguments();
            const config = await loadCliConfig({ tools: { webSearch: { enabled: true, apiKey: 'sk-test' } } }, argv);
            expect(config.getWebSearchSettings()).toBeUndefined();
        });
...
describe('loadCliConfig telemetry', () => {
    ...
    it('should default sensitiveSpanAttributeMaxLength to 1MiB', async () => {
        ...
        const settings = { telemetry: { enabled: true } };
        const config = await loadCliConfig(settings, argv);
        expect(config.getTelemetrySensitiveSpanAttributeMaxLength()).toBe(1024 * 1024);
    });
...
describe('Approval mode tool exclusion logic', () => {
    ...
    it('should throw an error for invalid approval mode values in loadCliConfig', async () => {
        ...
        const settings = {};
        await expect(loadCliConfig(settings, invalidArgv, undefined, [])).rejects.toThrow('Invalid approval mode: invalid_mode. Valid values are: plan, default, auto-edit, auto, yolo');
    });
});
...
describe('loadCliConfig approval mode', () => {
    ...
    it('should throw when approval mode in settings is invalid', async () => {
        ...
        await expect(loadCliConfig(settings, argv, undefined, [])).rejects.toThrow('Invalid approval mode: invalid_mode. Valid values are: plan, default, auto-edit, auto, yolo');
    });
...
describe('loadCliConfig fileFiltering', () => {
    ...
    it('should pass customIgnoreFiles from settings to config', async () => {
        ...
        expect(config.getFileService().getQwenIgnoreFileNamesDisplay()).toBe('.qwenignore, .cursorignore');
    });
});
...
describe('Telemetry configuration via environment variables', () => {
    ...
    it("should treat QWEN_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES='false' as false", async () => {
        ...
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({ telemetry: { includeSensitiveSpanAttributes: true } }, argv, undefined, []);
        expect(config.getTelemetryIncludeSensitiveSpanAttributes()).toBe(false);
    });
});
...
describe('loadCliConfig plansDirectory', () => {
    ...
    it('should reject plansDirectory values outside cwd', async () => {
        ...
        await expect(loadCliConfig(settings, argv, cwd)).rejects.toThrow('plansDirectory must resolve within the project root');
    });
});
...
describe('loadCliConfig skills.disabledLevels', () => {
    ...
    it('passes valid disabled skill levels to core and ignores invalid values', async () => {
        ...
        const settings = {
            skills: {
                disabledLevels: ['bundled', 'invalid', 42, 'user'],
            },
        };
        const config = await loadCliConfig(settings, argv);
        expect(config.getDisabledSkillLevels()).toEqual(new Set(['bundled', 'user']));
    });
    ...
    it('ignores skills.disabledLevels in safe mode', async () => {
        process.argv = ['node', 'script.js', '--safe-mode'];
        const argv = await parseArguments();
        const settings = { skills: { disabledLevels: ['bundled'] } };
        const config = await loadCliConfig(settings, argv);
        expect(config.getDisabledSkillLevels()).toEqual(new Set());
    });
