...
describe('buildDisabledSkillNamesProvider', async () => {
    ...
    function fakeSettings(disabled, defaultDisabled, enabled) {
        return {
    ...
    it('returns a normalized set from a normal array', () => {
        const provider = buildDisabledSkillNamesProvider(fakeSettings(['Foo', ' BAR ', 'baz']));
    ...
    it('filters non-string elements from a mixed-type array', () => {
        const provider = buildDisabledSkillNamesProvider(fakeSettings([42, null, 'valid', undefined, true, '  TRIMMED  ']));
        expect(provider()).toEqual(new Set(['valid', 'trimmed']));
    });
    it('excludes empty-after-trim strings', () => {
        const provider = buildDisabledSkillNamesProvider(fakeSettings(['  ', '', 'keep']));
        expect(provider()).toEqual(new Set(['keep']));
    });
    it('applies explicit enables between defaults and hard disables', () => {
        const provider = buildDisabledSkillNamesProvider(fakeSettings(['hard'], ['soft', 'hard'], ['SOFT', 'HARD']));
