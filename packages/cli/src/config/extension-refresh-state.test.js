...
describe('extension refresh state', () => {
    ...
    it('deduplicates refresh notifications until cleared', () => {
        ...
        try {
            expect(refreshState.markExtensionsChanged('extension files changed')).toBe(true);
            expect(refreshState.needsExtensionRefresh()).toBe(true);
            expect(listener).toHaveBeenCalledWith('extension files changed');
            expect(refreshState.markExtensionsChanged('extension files changed again')).toBe(false);
            expect(listener).toHaveBeenCalledTimes(1);
            refreshState.clearExtensionsChanged();
            expect(refreshState.needsExtensionRefresh()).toBe(false);
            expect(refreshState.markExtensionsChanged('extension files changed again')).toBe(true);
    ...
    it('suppresses watcher notifications during known mutations', async () => {
        ...
        try {
            await refreshState.suppressNotifications(async () => {
                expect(refreshState.markExtensionsChanged('extension files changed')).toBe(false);
                expect(refreshState.markExtensionContentChanged('extension content changed')).toBe(false);
            });
            expect(refreshState.needsExtensionRefresh()).toBe(false);
    ...
    it('clears the post-mutation suppression window after reload', async () => {
        ...
        try {
            await refreshState.suppressNotifications(async () => { });
            expect(refreshState.markExtensionsChanged('during suppress window')).toBe(false);
            refreshState.clearExtensionsChanged();
    ...
    it('preserves stale state for changes that arrive during reload', () => {
        ...
        try {
            ...
            expect(refreshState.needsExtensionRefresh()).toBe(true);
            expect(listener).toHaveBeenCalledTimes(2);
            expect(listener).toHaveBeenLastCalledWith('extension files changed during reload');
            refreshState.clearExtensionsChanged();
            expect(refreshState.needsExtensionRefresh()).toBe(false);
        }
        finally {
    ...
    it('defers content changes that arrive during reload', () => {
        ...
        try {
            refreshState.notifyExtensionsReloadStarted();
            expect(refreshState.markExtensionContentChanged('content changed during reload')).toBe(false);
            expect(contentListener).not.toHaveBeenCalled();
            refreshState.clearExtensionsChanged();
            expect(refreshState.needsExtensionRefresh()).toBe(false);
            expect(contentListener).toHaveBeenCalledOnce();
            expect(contentListener).toHaveBeenCalledWith('extension content files changed during reload');
        }
        finally {
            refreshState.off(AppEvent.ExtensionContentChanged, contentListener);
        }
    ...
    it('lets stale changes during reload take priority over content changes', () => {
        ...
        try {
            refreshState.notifyExtensionsReloadStarted();
            expect(refreshState.markExtensionContentChanged('content changed')).toBe(false);
            expect(refreshState.markExtensionsChanged('manifest changed')).toBe(false);
            refreshState.clearExtensionsChanged();
            expect(refreshState.needsExtensionRefresh()).toBe(true);
            expect(staleListener).toHaveBeenCalledOnce();
            expect(staleListener).toHaveBeenCalledWith('extension files changed during reload');
            expect(contentListener).not.toHaveBeenCalled();
        }
        finally {
            refreshState.off(AppEvent.ExtensionRefreshNeeded, staleListener);
...
//# sourceMappingURL=extension-refresh-state.test.js.map