...
describe('MigrationScheduler', () => {
    ...
    const createMockMigration = (fromVersion, toVersion, shouldMigrateResult) => ({
        fromVersion,
        toVersion,
    ...
    it('should pass updated settings to each migration', () => {
        ...
        expect(migration2.shouldMigrate).toHaveBeenCalledWith(expect.objectContaining({ $version: 2, transformed: true }));
    });
    ...
    it('should throw error when migration fails', () => {
        ...
        const scheduler = new MigrationScheduler([migration1], 'user');
        expect(() => scheduler.migrate({ $version: 1 })).toThrow('Migration failed');
    });
