...
describe('V3ToV4Migration', () => {
    ...
    describe('shouldMigrate', () => {
        it('returns true for V3 settings', () => {
            expect(migration.shouldMigrate({
                $version: 3,
        ...
        it.each([
            ...
            ['number', 42],
            ['null', null],
        ])('returns true for versionless settings with invalid gitCoAuthor (%s)', (_label, value) => {
            expect(migration.shouldMigrate({
    ...
    describe('migrate', () => {
        it('expands legacy boolean true into { commit: true, pr: true }', () => {
            ...
            const { settings, warnings } = migration.migrate(input, 'user');
            expect(settings['general']['gitCoAuthor']).toEqual({ commit: true, pr: true });
        ...
        it('expands legacy boolean false into { commit: false, pr: false }', () => {
            ...
            const { settings } = migration.migrate(input, 'user');
            expect(settings['general']['gitCoAuthor']).toEqual({ commit: false, pr: false });
        });
        it('leaves an already-object value untouched', () => {
            ...
            const { settings, warnings } = migration.migrate(input, 'user');
            expect(settings['general']['gitCoAuthor']).toEqual({ commit: false, pr: true });
            expect(warnings).toEqual([]);
        });
        it('bumps version when gitCoAuthor is absent', () => {
            ...
            const { settings, warnings } = migration.migrate(input, 'user');
            expect(settings['$version']).toBe(4);
            expect(settings['general']['gitCoAuthor']).toBeUndefined();
            expect(warnings).toEqual([]);
        });
        ...
        ])('maps string %s to %j (warn=%s)', (_label, str, expected, expectWarn) => {
            const input = { $version: 3, general: { gitCoAuthor: str } };
            const { settings, warnings } = migration.migrate(input, 'user');
            expect(settings['general']['gitCoAuthor']).toEqual(expected);
            if (expectWarn) {
        ...
        it.each([
            ...
            ['array', []],
            ['number', 42],
        ])('treats %s as invalid and resets to disabled with a warning', (_label, bad) => {
        ...
        it('leaves a partially-specified object unchanged', () => {
            ...
            const { settings, warnings } = migration.migrate(input, 'user');
            expect(settings['general']['gitCoAuthor']).toEqual({ commit: false });
            expect(warnings).toEqual([]);
        });
...
//# sourceMappingURL=v3-to-v4.test.js.map