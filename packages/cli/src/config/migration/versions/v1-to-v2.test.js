...
describe('V1ToV2Migration', () => {
    ...
    describe('migrate', () => {
        it('should migrate flat V1 keys to nested V2 structure', () => {
            ...
            const { settings: result } = migration.migrate(v1Settings, 'user');
        ...
        it('should drop non-boolean non-consolidated disable* values', () => {
            ...
            const { settings: result } = migration.migrate(v1Settings, 'user');
            expect(result['$version']).toBe(2);
            expect(result['ui']?.['accessibility']).toBeUndefined();
            expect(result['context']?.['fileFiltering']?.['disableFuzzySearch']).toBeUndefined();
        });
...
//# sourceMappingURL=v1-to-v2.test.js.map