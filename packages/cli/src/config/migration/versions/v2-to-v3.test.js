...
describe('V2ToV3Migration', () => {
    ...
    describe('migrate', () => {
        it('should migrate disableAutoUpdate to enableAutoUpdate with inverted value', () => {
            ...
            const { settings: result } = migration.migrate(v2Settings, 'user');
            expect(result['$version']).toBe(3);
            expect(result['general']['enableAutoUpdate']).toBe(false);
            expect(result['general']['disableAutoUpdate']).toBeUndefined();
        });
        it('should migrate disableLoadingPhrases to enableLoadingPhrases', () => {
            ...
            const { settings: result } = migration.migrate(v2Settings, 'user');
            expect(result['$version']).toBe(3);
            expect(result['ui']['accessibility']).toEqual({
        ...
        it('should migrate disableFuzzySearch to enableFuzzySearch', () => {
            ...
            const { settings: result } = migration.migrate(v2Settings, 'user');
            expect(result['$version']).toBe(3);
            expect(result['context']['fileFiltering']).toEqual({
        ...
        it('should migrate disableCacheControl to enableCacheControl', () => {
            ...
            const { settings: result } = migration.migrate(v2Settings, 'user');
            expect(result['$version']).toBe(3);
            expect(result['model']['generationConfig']).toEqual({
        ...
        it('should handle consolidated disableAutoUpdate and disableUpdateNag', () => {
            ...
            expect(result['general']['enableAutoUpdate']).toBe(false);
            expect(result['general']['disableAutoUpdate']).toBeUndefined();
            expect(result['general']['disableUpdateNag']).toBeUndefined();
        });
        it('should set enableAutoUpdate to true when both disable* are false', () => {
            ...
            const { settings: result } = migration.migrate(v2Settings, 'user');
            expect(result['$version']).toBe(3);
            expect(result['general']['enableAutoUpdate']).toBe(true);
        });
        it('should preserve other settings during migration', () => {
            ...
            expect(result['model']['name']).toBe('gemini');
            expect(result['ui']['accessibility']).toEqual({
                enableLoadingPhrases: false,
            });
        });
        ...
        it('should throw error for non-object input', () => {
            expect(() => migration.migrate(null, 'user')).toThrow('Settings must be an object');
            expect(() => migration.migrate('string', 'user')).toThrow('Settings must be an object');
        });
        ...
        it('should coerce string "true" and remove deprecated key', () => {
            ...
            const { settings: result, warnings } = migration.migrate(v2Settings, 'user');
            expect(result['$version']).toBe(3);
            expect(result['general']['disableAutoUpdate']).toBeUndefined();
            expect(result['general']['enableAutoUpdate']).toBe(false);
        ...
        it('should remove number value and emit warning', () => {
            ...
            expect(result['$version']).toBe(3);
            expect(result['general']['disableAutoUpdate']).toBeUndefined();
            expect(result['general']['enableAutoUpdate']).toBeUndefined();
        ...
        it('should remove disableCacheControl number value and emit warning', () => {
            ...
            expect(result['$version']).toBe(3);
            expect(result['model']['generationConfig']).toEqual({});
            expect(result['model']['generationConfig']['enableCacheControl']).toBeUndefined();
            expect(warnings).toHaveLength(1);
            expect(warnings[0]).toContain('model.generationConfig.disableCacheControl');
        });
...
//# sourceMappingURL=v2-to-v3.test.js.map