 ...
 */
import { getNestedProperty, setNestedPropertySafe, } from '../../../utils/settingsUtils.js';
    ...
    migrate(settings, scope) {
        ...
        if (typeof value === 'boolean') {
            ...
            if (enableIntent.includes(lowered)) {
                ...
                warnings.push(`Reset '${GIT_CO_AUTHOR_PATH}' in ${scope} settings to {commit: false, pr: false} because the stored string '${value}' was not a recognized boolean form.`);
            }
        }
            ...
            warnings.push(`Reset '${GIT_CO_AUTHOR_PATH}' in ${scope} settings to {commit: false, pr: false} because the stored value was not a boolean or object.`);
        }
