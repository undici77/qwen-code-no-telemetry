...
import { executeToolCall, shutdownTelemetry, isTelemetrySdkInitialized, GeminiEventType, FatalInputError, promptIdContext, OutputFormat, InputFormat, LoopType, ToolNames, uiTelemetryService, parseAndFormatApiError, createDebugLogger, SendMessageType, restoreWorktreeContext, } from '@qwen-code/qwen-code-core';
...
export async function runNonInteractive(config, settings, input, prompt_id, options = {}) {
    return promptIdContext.run(prompt_id, async () => {
        ...
        try {
             ...
             */
            const processToolCallBatch = async (batchRequests, setModelOverride) => {
                ...
                if (config.getJsonSchema() &&
                    ...
                    requestsToExecute = batchRequests.filter((r) => r.name === ToolNames.STRUCTURED_OUTPUT);
                }
                ...
                const unexecutedCalls = batchRequests.filter((r) => !executedCallIds.has(r.callId));
            ...
            while (true) {
                ...
                if (toolCallRequests.length > 0) {
                    ...
                    const toolResponseParts = await processToolCallBatch(toolCallRequests, (override) => {
                    ...
                    const drainOneItem = async () => {
                        ...
                        while (true) {
                            ...
                            if (itemToolCallRequests.length > 0) {
                                ...
                                // `modelOverride`.
                                const itemToolResponseParts = await processToolCallBatch(itemToolCallRequests, (override) => {
