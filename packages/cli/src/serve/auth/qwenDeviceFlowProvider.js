 ...
 */
import { cacheQwenCredentials, generatePKCEPair, isDeviceAuthorizationSuccess, isDeviceTokenPending, isDeviceTokenSuccess, QwenOAuth2Client, QwenOAuthPollError, } from '@qwen-code/qwen-code-core';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { brandSecret, unsafeRevealSecret, UpstreamDeviceFlowError, } from './deviceFlow.js';
 ...
 */
function sanitizeForStderr(value) {
...
export class QwenOAuthDeviceFlowProvider {
    ...
    async start(opts) {
        ...
        try {
            ...
            writeStderrLine(`[serve] qwen device-flow start failed (raw): ${truncateForStderr(detail)}`);
            throw new UpstreamDeviceFlowError('Qwen IdP device authorization request failed');
        }
        ...
        if (!isDeviceAuthorizationSuccess(auth)) {
            ...
            writeStderrLine(truncateForStderr(`[serve] qwen device-flow start error envelope (raw): error=${errorData?.error ?? 'unknown'} description=${errorData?.error_description ?? '(none)'}`));
            throw new UpstreamDeviceFlowError('Qwen IdP rejected the device authorization request');
        }
        return {
    ...
    async poll(state, opts) {
        ...
        try {
            ...
            if (!aborted) {
                ...
                writeStderrLine(`[serve] qwen device-flow poll failed (errorKind=${errorKind}): ${truncateForStderr(safeDetail)}`);
            }
            return {
                kind: 'error',
                errorKind,
...
//# sourceMappingURL=qwenDeviceFlowProvider.js.map