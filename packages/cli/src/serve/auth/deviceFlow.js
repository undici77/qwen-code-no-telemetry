...
export const DEVICE_FLOW_SUPPORTED_PROVIDERS = ['qwen-oauth'];
/**
...
export class DeviceFlowRegistry {
    ...
    async runPollTick(entry, provider) {
        ...
        try {
            result = await new Promise((resolve, reject) => {
                pollTimer = this.schedule(DEVICE_FLOW_POLL_TIMEOUT_MS, () => {
                    pollTimedOut = true;
                    try {
                        entry.cancelController.abort(new DeviceFlowPollTimeoutError(DEVICE_FLOW_POLL_TIMEOUT_MS));
                    }
                    catch {
                        // best-effort
                    }
        ...
        if (pollTimedOut && providerPollPromise !== undefined) {
            ...
            void tracked.then((latePollResult) => {
                this.deps.audit?.record({
                    deviceFlowId: entry.deviceFlowId,
                    providerId: entry.providerId,
                    clientId: entry.initiatorClientId,
                ...
                if (lateErr instanceof DeviceFlowPollTimeoutError)
                    return;
                const detail = lateErr instanceof Error ? lateErr.message : String(lateErr);
                this.deps.audit?.record({
                    deviceFlowId: entry.deviceFlowId,
                    providerId: entry.providerId,
                    clientId: entry.initiatorClientId,
