...
import { canonicalizeWorkspace, createHttpAcpBridge, } from './httpAcpBridge.js';
...
export async function runQwenServe(optsIn, deps = {}) {
    ...
    try {
        ...
        if (!stats.isDirectory()) {
            throw new Error(`Invalid --workspace "${rawWorkspace}": exists but is not a directory.`);
        }
    }
    catch (err) {
        if (err && typeof err === 'object' && 'code' in err) {
            const code = err.code;
            if (code === 'ENOENT') {
                throw new Error(`Invalid --workspace "${rawWorkspace}": directory does not exist.`);
            }
            ...
            if (code === 'EACCES' || code === 'EPERM') {
                throw new Error(`Invalid --workspace "${rawWorkspace}": permission denied ` +
    ...
    const bridge = deps.bridge ??
        createHttpAcpBridge({
            ...
            persistDisabledTools: (workspace, toolName, enabled) => withSettingsLock(workspace, async () => {
                const fresh = loadSettings(workspace);
                ...
                if (enabled)
                    ...
                    next.add(toolName);
                fresh.setValue(SettingScope.Workspace, 'tools.disabled', [...next].sort());
            }),
        });
    ...
    if (opts.maxConnections !== undefined) {
        if (Number.isNaN(opts.maxConnections)) {
            throw new TypeError('Invalid maxConnections: NaN. Must be >= 0 ' +
        ...
        const server = app.listen(opts.port, listenHostname, () => {
            ...
            const onSignal = async (signal) => {
                if (shuttingDown) {
                    ...
                    writeStderrLine(`qwen serve: received ${signal} during drain — forcing exit`);
                    try {
                        bridge.killAllSync();
                    }
                    catch (err) {
                        writeStderrLine(`qwen serve: force-kill error: ${err instanceof Error ? err.message : String(err)}`);
                    }
                    process.exit(1);
                    return;
                }
                    ...
                    closePromise = new Promise((res, rej) => {
                        ...
                        if (deviceFlowRegistry) {
                            try {
                                ...
                                writeStderrLine(`qwen serve: device-flow registry dispose error: ${err instanceof Error ? err.message : String(err)}`);
                            }
                        }
                        bridge
                            .shutdown()
                            .catch((err) => {
                            writeStderrLine(`qwen serve: bridge shutdown error: ${String(err)}`);
                            bridgeShutdownError =
                            ...
                            const forceTimer = setTimeout(() => {
                                writeStderrLine(`qwen serve: ${SHUTDOWN_FORCE_CLOSE_MS}ms listener-drain timeout reached; force-closing remaining connections`);
                                ...
                                secondaryTimer = setTimeout(() => {
                                    writeStderrLine(`qwen serve: server.close did not fire ${SECONDARY_DEADLINE_MS}ms after force-close; resolving anyway`);
                                    finish();
            ...
            server.on('error', (err) => {
                writeStderrLine(`qwen serve: server error: ${err instanceof Error ? err.message : String(err)}`);
            });
            resolve(handle);
        });
...
//# sourceMappingURL=runQwenServe.js.map