...
import { createHttpAcpBridge, InvalidClientIdError, InvalidPermissionOptionError, InvalidSessionMetadataError, InvalidSessionScopeError, MAX_WORKSPACE_PATH_LENGTH, RestoreInProgressError, SessionNotFoundError, WorkspaceInitConflictError, WorkspaceMismatchError, } from './httpAcpBridge.js';
...
describe('createHttpAcpBridge', () => {
    ...
    it('answers /workspace/env from process state without consulting ACP, idle or live', async () => {
        ...
        expect(handles).toHaveLength(1);
        expect(handles[0]?.agent.extMethodCalls.some((c) => c.method.includes('/workspace/env'))).toBe(false);
        await bridge.shutdown();
    });
    ...
    it('detachClient unregisters only the detached client id', async () => {
        ...
        }, undefined, { clientId: second.clientId })).rejects.toBeInstanceOf(InvalidClientIdError);
        await bridge.shutdown();
    });
    ...
    describe('setSessionModel', () => {
        ...
        it('rejects unregistered client ids on session-scoped requests', async () => {
            ...
            await expect(bridge.sendPrompt(session.sessionId, {
                sessionId: session.sessionId,
                prompt: [{ type: 'text', text: 'hi' }],
            }, undefined, { clientId: 'client-not-issued' })).rejects.toBeInstanceOf(InvalidClientIdError);
    ...
    describe('setSessionApprovalMode (#4175 Wave 4 PR 17)', () => {
        ...
        it('broadcasts approval_mode_changed to peer sessions when persisted (#4282 fold-in 4 S2)', async () => {
            ...
            expect(aTypes.filter((t) => t === 'approval_mode_changed').length).toBe(2);
    ...
    describe('setWorkspaceToolEnabled (#4175 Wave 4 PR 17)', () => {
        it('throws when no persistDisabledTools callback is wired', async () => {
            const bridge = makeBridge();
            await expect(bridge.setWorkspaceToolEnabled('Bash', false, undefined)).rejects.toThrow(/persistDisabledTools/);
        });
        it('invokes the persist callback with the workspace + name + enabled flag', async () => {
            ...
            const result = await bridge.setWorkspaceToolEnabled('Bash', false, undefined);
        ...
        it('fan-outs tool_toggled events to every live session bus', async () => {
            const factory = async () => {
                const { clientStream, agentStream } = createInMemoryChannel();
                new AgentSideConnection(() => new FakeAgent(), agentStream);
                return {
                    stream: clientStream,
                    exited: new Promise(() => { }),
                    kill: async () => { },
    ...
    describe('initWorkspace (#4175 Wave 4 PR 17)', () => {
        ...
        it('throws WorkspaceInitConflictError when content exists and force is omitted', async () => {
            ...
            expect(err.existingSize).toBe(Buffer.byteLength(original, 'utf8'));
...
//# sourceMappingURL=httpAcpBridge.test.js.map