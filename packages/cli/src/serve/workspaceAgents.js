...
import { APPROVAL_MODES, BuiltinAgentRegistry, SubagentError, SubagentErrorCode, SubagentManager, } from '@qwen-code/qwen-code-core';
...
import { STATUS_SCHEMA_VERSION, } from './status.js';
export function mountWorkspaceAgentsRoutes(app, deps) {
    ...
    app.get('/workspace/agents', async (_req, res) => {
        try {
            ...
            writeStderrLine(`qwen serve: GET /workspace/agents failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
    ...
    app.post('/workspace/agents', deps.mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`qwen serve: POST /workspace/agents failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
    ...
    app.get('/workspace/agents/:agentType', async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`qwen serve: GET /workspace/agents/${safeLogValue(agentType)} failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
    ...
    app.post('/workspace/agents/:agentType', deps.mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`qwen serve: POST /workspace/agents/${safeLogValue(agentType)} failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
    ...
    app.delete('/workspace/agents/:agentType', deps.mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`qwen serve: DELETE /workspace/agents/${safeLogValue(agentType)} failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
        ...
        if (remaining.length > 0) {
            writeStderrLine(`qwen serve: DELETE /workspace/agents/${safeLogValue(agentType)} partial — ` +
...
function parseAgentConfig(body, level, res) {
    ...
    if (typeof body['approvalMode'] === 'string') {
        if (!APPROVAL_MODES.includes(body['approvalMode'])) {
            res.status(422).json({
 ...
 * incorrect data.
 */
export function createDaemonSubagentManager(boundWorkspace) {
    const stub = {
        getSdkMode: () => false,
    ...
    const guarded = new Proxy(stub, {
        get(target, prop) {
            ...
            if (prop === 'then')
                return undefined;
            throw new Error(`qwen serve workspace agents: SubagentManager touched Config.` +
        ...
        has(target, prop) {
            ...
            if (prop === 'then')
                return false;
            throw new Error(`qwen serve workspace agents: SubagentManager probed Config.` +
...
//# sourceMappingURL=workspaceAgents.js.map