...
export const LIVE_HOST_OSS_BASE_URL = 'https://qwen-code-assets.oss-cn-hangzhou.aliyuncs.com/live-host';
export const LIVE_HOST_RELEASE_BASE_URL = 'https://github.com/QwenLM/qwen-code/releases/download/live-host-latest';
...
export async function downloadLiveHostRelease(currentArchitecture, destination, onProgress, fetchImpl = fetch) {
    ...
    await fsp.rm(destination, { force: true });
    throw new AggregateError(errors, `Qwen Live Host download failed. ${errors
        .map((error) => error.message)
        .join(' ')}`);
}
async function installLatestHost(currentArchitecture, onStatus) {
    const temporaryDirectory = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-live-host-'));
