...
function createHarness(options = {}) {
    ...
    const persistSettings = vi.fn(async (writes) => {
        ...
        for (const write of writes) {
            ...
            if (write.value === undefined)
                delete liveVoice[property];
            else
                liveVoice[property] = write.value;
        }
        settings = {
            ...settings,
    ...
    const setEnabled = vi.fn(async (next) => {
        enabled = next;
    });
    ...
    const coordinator = new LiveHostCoordinator({
        getProviderReadiness: () => enabled ? { state: 'ready' } : { state: 'unavailable' },
    });
...
describe('LiveSetupController', () => {
    ...
    it('validates, persists, hot-enables, and starts installation', async () => {
        ...
        expect(harness.settings().experimental?.liveVoice?.apiKey).toBe('realtime-secret');
        await vi.waitFor(() => expect(harness.installLatest).toHaveBeenCalledOnce());
    });
    it('does not persist or enable when credential validation fails', async () => {
        ...
        await expect(harness.controller.update({
            enabled: true,
    ...
    it('requires a dedicated key before enablement', async () => {
        const harness = createHarness();
        await expect(harness.controller.update({ enabled: true })).rejects.toMatchObject({
...
//# sourceMappingURL=live-setup-controller.test.js.map