...
import { createLiveTaskTools, LIVE_TASK_TOOL_NAMES, } from './live-task-tools.js';
describe('createLiveTaskTools', () => {
    it('registers exactly the five Codex Live task operations', () => {
        ...
        expect(tools.find((tool) => tool.name === 'create_thread')?.description).toContain('The active Live conversation is not that separate task');
        expect(tools.find((tool) => tool.name === 'wait_threads')?.description).toContain('does not mean that a task failed');
    });
    ...
    it('returns an observation timeout as a completed tool result', async () => {
        ...
        const tool = createLiveTaskTools(execute).find((candidate) => candidate.name === 'wait_threads');
...
//# sourceMappingURL=live-task-tools.test.js.map