...
import { DEFAULT_LIVE_ENDPOINT, DEFAULT_LIVE_SHORTCUT, DEFAULT_LIVE_VOICE, DEFAULT_LIVE_VOICE_MODEL, LiveProviderConfigError, readLiveVoiceConfiguration, resolveLiveProviderCredential, } from './provider-credentials.js';
...
describe('Live provider credentials', () => {
    it('applies the documented Live defaults', () => {
        expect(readLiveVoiceConfiguration({})).toEqual({
            enabled: false,
    ...
    it('preserves an empty shortcut as Off', () => {
        expect(readLiveVoiceConfiguration({
    ...
    it('accepts a one-shot key override while validating enablement', () => {
        const resolved = resolveLiveProviderCredential(settings({ experimental: { liveVoice: { enabled: false } } }), {
    ...
    it('requires the dedicated key instead of a chat-provider env key', () => {
        ...
        expect(() => resolveLiveProviderCredential(input)).toThrow(/not configured/);
    });
    it.each([
        [
        ...
        expect(() => resolveLiveProviderCredential(input)).toThrow(LiveProviderConfigError);
    });
...
//# sourceMappingURL=provider-credentials.test.js.map