...
import { getLiveDiscoveryPath, LiveDiscoveryOwnerActiveError, removeLiveDiscoveryFile, writeLiveDiscoveryFile, } from './discovery.js';
...
describe('Live discovery file', () => {
    ...
    it('only removes a record owned by the matching daemon pid and nonce', async () => {
        ...
        expect(await removeLiveDiscoveryFile(runtime, {
            pid: current.pid,
    ...
    it('reclaims only a valid record whose owner process is stale', async () => {
        ...
        expect(JSON.parse(await fs.readFile(getLiveDiscoveryPath(runtime), 'utf8'))).toEqual(replacement);
    });
    it('reclaims a valid legacy-protocol record whose owner is stale', async () => {
        ...
        expect(JSON.parse(await fs.readFile(discoveryPath, 'utf8'))).toEqual(replacement);
