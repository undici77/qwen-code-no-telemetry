...
export class LiveHostCoordinator {
    ...
    setShortcut(shortcut) {
        ...
        if (!host?.hello || !this.isLeaseHealthy(host)) {
            return Promise.reject(new Error('Qwen Live Host must be connected to change the shortcut.'));
        }
        const requestId = randomUUID();
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                this.rejectPendingShortcut(new Error('Qwen Live Host did not confirm the shortcut change.'));
            ...
            if (!this.sendHost({
                ...
                this.rejectPendingShortcut(new Error('Qwen Live Host is unavailable for shortcut changes.'));
            }
        });
    }
    ...
    handleShortcutResult(message) {
        ...
        if (message.shortcut !== pending.shortcut) {
            pending.reject(new Error('Qwen Live Host returned a mismatched shortcut.'));
            return;
        }
        if (!message.success) {
