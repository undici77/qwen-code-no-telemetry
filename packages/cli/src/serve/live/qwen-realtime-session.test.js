...
import { deriveQwenOmniRealtimeUrl, openQwenRealtimeSession, QWEN_REALTIME_LIMITS, } from './qwen-realtime-session.js';
class FakeSocket {
    OPEN = 1;
...
function sentJsonEntry(entry) {
    return JSON.parse(String(entry));
}
function commitFinalInput(socket, itemId, transcript) {
    socket.message({
...
function responseCreated(socket, responseId) {
    socket.message({
...
function responseDone(socket, responseId, status = 'completed') {
    socket.message({
        type: 'response.done',
...
function functionCall(socket, responseId, callId, name, argumentsText) {
    socket.message({
...
async function connect(socket, callbacks = {}) {
    const opening = openQwenRealtimeSession({
        ...
        callEpoch: 7,
        voice: 'Tina',
    }, callbacks, { createWebSocket: () => socket });
...
describe('qwen-realtime-session', () => {
    it('derives a model-qualified WebSocket URL', () => {
        expect(deriveQwenOmniRealtimeUrl('https://dashscope.aliyuncs.com/compatible-mode/v1', 'qwen3.5-omni-plus-realtime')).toBe('wss://dashscope.aliyuncs.com/api-ws/v1/realtime?model=qwen3.5-omni-plus-realtime');
        expect(deriveQwenOmniRealtimeUrl('wss://example.test/custom/api-ws/v1/realtime?tenant=one', 'model/with spaces')).toBe('wss://example.test/custom/api-ws/v1/realtime?tenant=one&model=model%2Fwith+spaces');
    });
    it('configures the Codex V2 conversation and handoff tools', async () => {
        ...
        expect(tools[0]?.function.description).toContain('Before invoking this function for the first time in a user turn, first output one short, concrete assistant speech sentence');
        expect(tools[0]?.function.description).toContain('never creates a separate user-visible task, session, or conversation by itself');
        expect(String(session['instructions'])).toContain('Respond directly only when the request is clearly self-contained');
        expect(String(session['instructions'])).toContain('Always use the backend when the user asks about the current screen');
        expect(String(session['instructions'])).toContain('Never answer capability questions based only on your own realtime-model capabilities');
        expect(String(session['instructions'])).toContain("Call `background_agent` with the user's exact words");
        expect(String(session['instructions'])).toContain('A handoff is not the requested separate task');
        expect(String(session['instructions'])).toContain('Messages from the backend are prefixed with [BACKEND] .');
        expect(String(session['instructions'])).toContain('[BACKEND] messages are silent context');
        expect(String(session['instructions'])).toContain('speak exactly the text after the prefix, verbatim');
        expect(String(session['instructions'])).toContain('If the user explicitly requests frequent or detailed updates');
    });
    it('lets Realtime answer an ordinary turn directly', async () => {
        ...
        expect(callbacks.onResponseCreated).toHaveBeenCalledWith(expect.objectContaining({
            responseId: 'response-direct',
            inputItemId: 'input-direct',
            authority: 'direct',
        }));
        expect(callbacks.onOutputTextDone).toHaveBeenCalledWith(expect.objectContaining({ text: '你好！', source: 'audio_transcript' }));
        expect(callbacks.onOutputAudioDelta).toHaveBeenCalledWith(expect.objectContaining({ audio: new Uint8Array([1, 0, 2, 0]) }));
        expect(callbacks.onResponseDone).toHaveBeenCalledWith(expect.objectContaining({ responseId: 'response-direct' }));
    ...
    it('uses the active response when response.done omits its identifier', async () => {
        ...
        expect(callbacks.onResponseDone).toHaveBeenCalledWith(expect.objectContaining({
            responseId: 'response-direct',
            inputItemId: 'input-direct',
            status: 'completed',
        }));
    ...
    it('ignores an idless duplicate response.done after a completed response', async () => {
        ...
        expect(callbacks.onIgnoredEvent).toHaveBeenCalledWith(expect.objectContaining({
            type: 'response.done',
            reason: 'stale_response',
        }));
        commitFinalInput(socket, 'input-handoff', '检查当前页面');
        responseCreated(socket, 'response-handoff');
        functionCall(socket, 'response-handoff', 'call-handoff', 'background_agent', JSON.stringify({ prompt: '检查当前页面' }));
    ...
    it('rejects an idless response.done when no response has completed', async () => {
        ...
        expect(callbacks.onError).toHaveBeenCalledWith(expect.objectContaining({ code: 'invalid_response' }));
    ...
    it('keeps a handoff open while sending backend Agent messages in order', async () => {
        ...
        responseCreated(socket, 'response-handoff');
        functionCall(socket, 'response-handoff', 'call-handoff', 'background_agent', JSON.stringify({ prompt: '查看当前仓库' }));
        expect(callbacks.onDelegateCall).toHaveBeenCalledWith(expect.objectContaining({
            callId: 'call-handoff',
            request: '查看当前仓库',
        ...
        expect(session.takeTranscriptTail()).toEqual([]);
        expect(session.sendHandoffUpdate({
            callEpoch: 7,
            callId: 'call-handoff',
            output: '正在检查仓库。',
        })).toBe(true);
        ...
        expect(session.completeHandoff({ callEpoch: 7, callId: 'call-handoff' })).toBe(true);
    ...
    it('keeps a second handoff silent while routing its text as steering', async () => {
        ...
        functionCall(socket, 'response-first', 'call-first', 'background_agent', JSON.stringify({ prompt: '开始检查' }));
        responseDone(socket, 'response-first');
        commitFinalInput(socket, 'input-steer', '只看测试目录');
        responseCreated(socket, 'response-steer');
        functionCall(socket, 'response-steer', 'call-steer', 'background_agent', JSON.stringify({ prompt: '只看测试目录' }));
        expect(callbacks.onDelegateCall).toHaveBeenNthCalledWith(2, expect.objectContaining({
            callId: 'call-steer',
            request: '只看测试目录',
        }));
        ...
        expect(sentTypes(socket).filter((type) => type === 'response.create')).toHaveLength(0);
        expect(session.sendHandoffUpdate({
            callEpoch: 7,
            callId: 'call-first',
            output: '已按新要求完成。',
        })).toBe(true);
        expect(session.completeHandoff({ callEpoch: 7, callId: 'call-first' })).toBe(true);
        expect(sentTypes(socket).filter((type) => type === 'response.create')).toHaveLength(0);
    });
    it('handles remain_silent without creating backend work or another response', async () => {
        ...
        functionCall(socket, 'response-silent', 'call-silent', 'remain_silent', '{}');
    ...
    it('ignores unknown Realtime tools while preserving an active handoff', async () => {
        ...
        functionCall(socket, 'response-screen', 'call-screen', 'background_agent', JSON.stringify({ prompt: '查看屏幕' }));
        ...
        expect(session.completeHandoff({ callEpoch: 7, callId: 'call-screen' })).toBe(true);
    });
    it('continues direct Realtime conversation after handoff completion', async () => {
        ...
        functionCall(socket, 'response-work', 'call-work', 'background_agent', JSON.stringify({ prompt: '执行任务' }));
        ...
        expect(callbacks.onDelegateCall).toHaveBeenCalledTimes(1);
        expect(callbacks.onResponseCreated).toHaveBeenLastCalledWith(expect.objectContaining({
            responseId: 'response-chat',
            inputItemId: 'input-chat',
            authority: 'direct',
        }));
        expect(callbacks.onOutputTextDone).toHaveBeenLastCalledWith(expect.objectContaining({ text: '不客气。' }));
    ...
    it('keeps the replacement response alive when VAD interrupts audio', async () => {
        ...
        expect(callbacks.onBargeIn).toHaveBeenCalledWith(expect.objectContaining({ responseId: 'response-first' }));
        ...
        expect(callbacks.onResponseDone).toHaveBeenNthCalledWith(1, expect.objectContaining({
            responseId: 'response-first',
            status: 'cancelled',
        }));
        expect(callbacks.onOutputAudioDelta).toHaveBeenLastCalledWith(expect.objectContaining({
            responseId: 'response-second',
            audio: new Uint8Array([2, 0]),
        }));
        expect(callbacks.onOutputTextDone).toHaveBeenLastCalledWith(expect.objectContaining({
            responseId: 'response-second',
            text: '第二个问题的回答。',
        }));
        expect(callbacks.onResponseDone).toHaveBeenLastCalledWith(expect.objectContaining({
            responseId: 'response-second',
            status: 'completed',
        }));
    });
    it('finalizes a superseded response when the provider omits its done event', async () => {
        ...
        expect(callbacks.onResponseDone).toHaveBeenNthCalledWith(2, expect.objectContaining({
            responseId: 'response-second',
            inputItemId: 'input-second',
            status: 'completed',
        }));
    ...
    it('serializes multiple explicit speech requests without combining them', async () => {
        ...
        expect(sentTypes(socket).filter((type) => type === 'conversation.item.create')).toHaveLength(0);
        responseDone(socket, 'response-active');
        await Promise.resolve();
    ...
    it('preserves audio backpressure and frame bounds', async () => {
        ...
        expect(() => session.pushAudio(new Uint8Array(QWEN_REALTIME_LIMITS.maxInputAudioFrameBytes + 2))).toThrow(RangeError);
...
//# sourceMappingURL=qwen-realtime-session.test.js.map