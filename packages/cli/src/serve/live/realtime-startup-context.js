...
async function loadRecentThreads(workspaceRegistry) {
    try {
        const pages = await Promise.all(workspaceRegistry.list().map(async (runtime) => {
            const page = await new SessionService(runtime.workspaceCwd).listSessions({
