...
import { assertExactLiveConversationRoot, getLiveConversationRootPath, LiveConversationWorkspace, revalidateLiveConversationRoot, } from './conversation-workspace.js';
const cleanup = [];
afterEach(async () => {
...
describe('Live conversation workspace root', () => {
    ...
    it('rejects symlink, non-directory, permissive, and foreign-owned roots', async () => {
        ...
        await expect(new LiveConversationWorkspace({ homeDir: fileHome }).getRoot()).rejects.toThrow(/non-symlink/);
        ...
        await expect(new LiveConversationWorkspace({ homeDir: permissiveHome }).getRoot()).rejects.toThrow(/only to its owner/);
        ...
        try {
            await expect(new LiveConversationWorkspace({ homeDir: ownerHome }).getRoot()).rejects.toThrow(/owned by the daemon user/);
        }
        finally {
    ...
    it('accepts only the exact configured or canonical root identity', async () => {
        ...
        expect(await assertExactLiveConversationRoot(identity, identity.canonicalRoot)).toBe(identity);
