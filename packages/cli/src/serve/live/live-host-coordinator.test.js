...
import { LiveHostCoordinator, LiveUnavailableError, } from './live-host-coordinator.js';
...
function connectReady(value, hello = readyHello()) {
    const socket = new FakeSocket();
    value.attachHost(socket, value.daemonInstanceNonce);
    socket.receive(hello);
    return socket;
}
afterEach(() => {
    for (const value of coordinators.splice(0))
        value.dispose();
    vi.useRealTimers();
});
describe('LiveHostCoordinator', () => {
    it('routes one correlated Appshot only for the active Live session', async () => {
        ...
        await expect(value.captureScreenContext('worker-1')).rejects.toThrow('active Live session');
    ...
    it('lets the active Live session finish Appshot during stop drain', async () => {
        ...
        expect(value.setCoordinator(call.epoch, {
            workspaceCwd: '/conversations/live-1',
            sessionId: 'coordinator-1',
        })).toBe(true);
        const capture = value.captureScreenContext('coordinator-1');
    ...
    it('bounds a Host capture that never answers', async () => {
        ...
        const capture = value.captureScreenContext('coordinator-1');
        const settled = capture.catch((error) => error);
    ...
    it('requires the discovery nonce before accepting a Host', () => {
        const value = coordinator();
        const socket = new FakeSocket();
        value.attachHost(socket, 'wrong_nonce_value_0000');
    ...
    it('welcomes one compatible, fully-authorized Host', () => {
        ...
        value.attachHost(duplicate, value.daemonInstanceNonce);
    ...
    it('keeps the stopping call epoch until the session drain succeeds', async () => {
        let finishStop;
        const onStop = vi.fn(() => new Promise((resolve) => {
            finishStop = resolve;
        }));
    ...
    it('lets an explicit stop cancel a pending replacement', async () => {
        ...
        value.stop();
        finishStop();
        await vi.waitFor(() => expect(value.getStatus()).toMatchObject({ state: 'idle' }));
        expect(onStart).toHaveBeenCalledOnce();
    });
...
//# sourceMappingURL=live-host-coordinator.test.js.map