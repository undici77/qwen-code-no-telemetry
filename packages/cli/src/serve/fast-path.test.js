...
describe('CLI entry import boundary', () => {
    ...
    it('keeps headless yolo warning helper free of runtime core imports', () => {
        ...
        expect(helperSource).not.toMatch(/import\s+(?!type\b)[^;]*from ['"]@qwen-code\/qwen-code-core['"]/);
    });
    ...
    it('keeps runQwenServe from statically loading the full server and ACP runtime', () => {
        ...
        expect(runServeSource).not.toMatch(/from ['"]@qwen-code\/acp-bridge\/bridge['"]/);
        expect(runServeSource).not.toMatch(/from ['"]@qwen-code\/acp-bridge\/spawnChannel['"]/);
    ...
    it('keeps request helpers from value-importing the ACP compatibility shim', () => {
        ...
        expect(requestHelpersSource).toContain("import type { AcpSessionBridge } from '@qwen-code/acp-bridge/bridgeTypes';");
        ...
        expect(requestHelpersSource).toMatch(/import \{[^}]*\bMAX_WORKSPACE_PATH_LENGTH\b[^}]*\} from '@qwen-code\/acp-bridge\/workspacePaths';/);
