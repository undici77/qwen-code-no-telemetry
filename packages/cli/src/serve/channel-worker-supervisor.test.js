...
import { ChannelWorkerStartupError, createChannelWorkerSupervisor, } from './channel-worker-supervisor.js';
...
import { CHANNEL_DELIVERY_IPC_TIMEOUT_MS, MAX_CHANNEL_DELIVERIES_IN_FLIGHT, } from '../runtime/channel-delivery-ipc.js';
...
class FakeChild extends EventEmitter {
    ...
    stdout;
    stderr;
    constructor(emitExitOnKill = true) {
        super();
    ...
    send = vi.fn((_message, _callback) => true);
}
const webhookTask = {
...
describe('createChannelWorkerSupervisor', () => {
    ...
    it('accepts loop MCP registration before the worker ready signal', async () => {
        ...
        await vi.waitFor(() => expect(registerChannelLoopMcp).toHaveBeenCalledWith({
            sessionId: 'session-early',
            ownerId: expect.stringMatching(/^channel-worker:/),
            sendMessage: expect.any(Function),
        }));
        await vi.waitFor(() => expect(child.send).toHaveBeenCalledWith({
    ...
    it('correlates exact-session loop MCP traffic and unregisters on exit', async () => {
        ...
        let reverseSender;
        let ownerId;
        const registerChannelLoopMcp = vi.fn(async (request) => {
        ...
        const request = child.send.mock.calls
            .map(([message]) => message)
            .find((message) => message.type === 'channel_loop_mcp_message');
        ...
        await supervisor.stop();
        await vi.waitFor(() => expect(unregisterChannelLoopMcp).toHaveBeenCalledWith('session-1', ownerId));
    });
    it('passes daemon connection details through env without putting token in argv', async () => {
        ...
        const child = new FakeChild();
        const spawnWorker = vi.fn((_execPath, _argv, _options) => child);
        const supervisor = createChannelWorkerSupervisor({
            cliEntryPath: '/repo/dist/index.js',
            daemonUrl: 'http://127.0.0.1:4170',
        ...
        await started;
        expect(spawnWorker).toHaveBeenCalledWith(process.execPath, [
            '/repo/dist/index.js',
            'channel',
            'daemon-worker',
            '--channel',
        ...
        const env = spawnWorker.mock.calls[0][2]
            .env;
        ...
        expect(isChannelWorkerPromptAuthorized(promptAuthorization, '/workspace')).toBe(true);
        expect(isChannelWorkerPromptAuthorized(promptAuthorization, '/other')).toBe(false);
        ...
        expect(isChannelWorkerPromptAuthorized(promptAuthorization, '/workspace')).toBe(false);
    });
    ...
    it('stores and acknowledges startup failures before exposing a deep-copied running snapshot', async () => {
        ...
        expect(child.send).toHaveBeenCalledWith({ type: 'channel_startup_report_ack' }, expect.any(Function));
        child.emit('message', {
            type: 'ready',
            channels: ['feishu'],
            requestedChannels: ['telegram', 'feishu'],
        ...
        expect(supervisor.snapshot().startupFailures[0].message).toBe('connection refused');
    });
    it('retains accepted and redacted startup failures when the worker exits before ready', async () => {
        ...
        child.emit('message', {
            ...
            failure: {
                ...
                phase: 'connect',
                code: 'daemon-secret',
                message: 'prefix \tprovider-secret Authorization: Bearer abcdef123456 failed',
            },
        });
        ...
        expect(error.startupFailures[0].message).not.toContain('provider-secret');
        expect(error.startupFailures[0].message).not.toContain('abcdef123456');
        expect(supervisor.snapshot().startupFailures).toEqual(error.startupFailures.map(({ workspaceCwd: _workspaceCwd, ...failure }) => failure));
    });
    ...
    it('terminates startup on malformed reports and acknowledgement failures', async () => {
        ...
        await expect(malformedStart).rejects.toThrow('Channel worker startup IPC protocol error: invalid startup report.');
        ...
        const ackError = await ackStart.catch((value) => value);
        expect(ackError).toBeInstanceOf(ChannelWorkerStartupError);
        expect(ackError.message).toContain('acknowledgement failed');
        expect(ackError.startupFailures).toHaveLength(1);
    ...
    it('accepts exactly 64 failures followed by one truncation marker', async () => {
        ...
        expect(supervisor.snapshot().startupFailures).toHaveLength(MAX_CHANNEL_STARTUP_FAILURES);
    ...
    it('rejects a 65th startup failure without a truncation marker', async () => {
        ...
        const error = await started.catch((value) => value);
        expect(error).toBeInstanceOf(ChannelWorkerStartupError);
        expect(error.message).toContain('too many startup failures.');
        expect(error.startupFailures).toHaveLength(MAX_CHANNEL_STARTUP_FAILURES);
    ...
    it('clears startup failure details when a new generation starts', async () => {
        ...
        expect(supervisor.snapshot()).not.toHaveProperty('startupFailuresTruncated');
    });
    ...
    it('rejects heartbeat timeouts that cannot exceed the worker heartbeat interval', () => {
        expect(() => createChannelWorkerSupervisor({
            cliEntryPath: '/repo/dist/index.js',
            daemonUrl: 'http://127.0.0.1:4170',
            workspace: '/workspace',
            selection: { mode: 'names', names: ['telegram'] },
            heartbeatTimeoutMs: CHANNEL_WORKER_HEARTBEAT_INTERVAL_MS,
        })).toThrow(`heartbeatTimeoutMs (${CHANNEL_WORKER_HEARTBEAT_INTERVAL_MS}) must exceed the worker heartbeat interval (${CHANNEL_WORKER_HEARTBEAT_INTERVAL_MS}ms) or be 0 to disable.`);
    });
    ...
    it('rejects startup when the worker never becomes ready', async () => {
        ...
        await expect(supervisor.start()).rejects.toThrow('Channel worker did not become ready within 1ms.');
        expect(supervisor.snapshot()).toMatchObject({
            enabled: true,
            state: 'failed',
    ...
    it('sanitizes the pre-ready error when the worker exits after an error', async () => {
        ...
        const started = supervisor.start();
        const unsafeMessage = `spawn error secret-token https://proxy-user:telegram-secret@proxy.example:8080\n` +
    ...
    it('revokes the worker prompt authorization when spawn throws', async () => {
        ...
        const supervisor = createChannelWorkerSupervisor({
            ...
            workspace: '/workspace',
            selection: { mode: 'names', names: ['telegram'] },
            spawnWorker: vi.fn((_execPath, _argv, options) => {
    ...
    it('restarts a ready worker after unexpected exit within budget', async () => {
        ...
        expect(spawnWorker).toHaveBeenCalledTimes(2);
        expect(onReady).toHaveBeenLastCalledWith(expect.objectContaining({
            state: 'running',
            pid: 22222,
            restartCount: 1,
            requestedChannels: ['telegram'],
    ...
    it('stops restarting after restart budget is exhausted', async () => {
        ...
        expect(supervisor.snapshot()).toMatchObject({
            ...
            state: 'failed',
            restartCount: 1,
            error: expect.stringContaining('Channel worker restart budget exhausted. Last error: Channel worker exited before ready'),
        });
    });
    ...
    it('does not restart when a pre-ready worker never exits after SIGKILL', async () => {
        ...
        await vi.advanceTimersByTimeAsync(2_000);
        expect(onExit).toHaveBeenLastCalledWith(expect.objectContaining({
            state: 'failed',
        ...
        await expect(supervisor.start()).rejects.toThrow('Channel worker stop is not yet confirmed.');
        expect(spawnWorker).toHaveBeenCalledTimes(2);
    });
    ...
    it('forwards worker stdout and stderr lines with secrets redacted', async () => {
        ...
        child.stdout.emit('data', Buffer.from('benign true wayland authenticated user\n'));
        child.stdout.emit('end');
        child.stderr.emit('data', Buffer.from('proxy http://proxy-user:p@ssword@proxy.example:8080/path\n'));
        child.emit('message', {
            type: 'ready',
            pid: 12345,
            channels: ['telegram'],
        ...
        expect(onLog.mock.calls.flatMap((call) => call[0].line).join('\n')).not.toContain('ssword');
    });
    it('decodes Uint8Array worker log chunks and preserves separation', async () => {
        ...
        const started = supervisor.start();
        child.stdout.emit('data', new Uint8Array(Buffer.from('\tat stack frame\nmetric\tvalue\t42\n')));
        child.emit('message', {
            type: 'ready',
            pid: 12345,
            channels: ['telegram'],
    ...
    it('flushes oversized worker log buffers without waiting for a newline', async () => {
        ...
        expect(onLog).not.toHaveBeenCalledWith(expect.objectContaining({ line: 'discarded oversized tail' }));
        expect(onLog).not.toHaveBeenCalledWith(expect.objectContaining({ line: 'still oversized tail' }));
    ...
    it('does not throw when worker log forwarding bookkeeping fails', async () => {
        ...
        const started = supervisor.start();
        expect(() => child.stderr?.emit('data', Buffer.from('line\n'))).not.toThrow();
        child.emit('message', {
            type: 'ready',
            pid: 12345,
            channels: ['telegram'],
    ...
    it('does not throw when a worker log pipe emits an error', async () => {
        ...
        expect(() => child.stderr?.emit('error', new Error('pipe failed'))).not.toThrow();
        child.emit('message', {
            type: 'ready',
            pid: 12345,
            channels: ['telegram'],
    ...
    it('does not release or restart a worker whose SIGKILL exit is unconfirmed', async () => {
        ...
        expect(child.kill).toHaveBeenCalledWith('SIGKILL');
        await vi.advanceTimersByTimeAsync(2_000);
        await expect(stopped).rejects.toThrow('Channel worker did not exit after SIGKILL.');
        expect(supervisor.snapshot()).toMatchObject({
            enabled: true,
            state: 'failed',
        ...
        await vi.advanceTimersByTimeAsync(10_000);
        await vi.advanceTimersByTimeAsync(2_000);
        await expect(retriedStop).rejects.toThrow('Channel worker did not exit after SIGKILL.');
        await expect(supervisor.start()).rejects.toThrow('Channel worker stop is not yet confirmed.');
        expect(spawnWorker).toHaveBeenCalledTimes(1);
    ...
    it('delivers a channel message through a running worker', async () => {
        ...
        const sent = child.send.mock.calls[0][0];
    ...
    it('rejects channel delivery when the supervisor is full', async () => {
        ...
        await started;
        const pending = Array.from({ length: MAX_CHANNEL_DELIVERIES_IN_FLIGHT }, () => supervisor.deliverChannelMessage(deliveryRequest));
        const overflow = supervisor.deliverChannelMessage(deliveryRequest).catch((error) => error);
        ...
        const first = child.send.mock.calls[0][0];
        child.emit('message', {
            type: 'channel_delivery_result',
            id: first.id,
            ok: true,
        ...
        expect(child.send).toHaveBeenCalledTimes(MAX_CHANNEL_DELIVERIES_IN_FLIGHT + 1);
        child.emit('exit', 1, null);
    ...
    it('rejects channel delivery when the worker is not running', async () => {
        ...
        await expect(supervisor.deliverChannelMessage(deliveryRequest)).rejects.toMatchObject({
    ...
    it('rejects channel delivery when IPC send throws synchronously', async () => {
        ...
        await started;
        const rejected = supervisor.deliverChannelMessage(deliveryRequest).then(() => undefined, (error) => error);
        await vi.advanceTimersByTimeAsync(30_000);
        const error = await rejected;
        expect(error).toBeInstanceOf(Error);
        expect(error.message).toBe('Channel worker IPC send failed: send boom');
        expect(error.code).toBe('channel_worker_unavailable');
    });
    it('rejects channel delivery when the IPC send callback reports an error', async () => {
        ...
        const error = await rejected;
        expect(error).toBeInstanceOf(Error);
        expect(error.message).toBe('Channel worker IPC send failed: callback boom');
        expect(error.code).toBe('channel_worker_unavailable');
    });
    it('times out delivery and rejects pending work when the worker exits', async () => {
        ...
        await started;
        const timedOut = supervisor.deliverChannelMessage(deliveryRequest).catch((error) => error);
    ...
    it('rejects webhook tasks when the worker is not running', async () => {
        ...
        await expect(supervisor.enqueueWebhookTask(webhookTask)).rejects.toThrow('Channel worker is not running.');
    });
    ...
    it('rejects webhook tasks when IPC send throws synchronously', async () => {
        ...
        await started;
        const rejected = supervisor.enqueueWebhookTask(webhookTask).then(() => undefined, (error) => error);
        await vi.advanceTimersByTimeAsync(30_000);
        const error = await rejected;
        expect(error).toBeInstanceOf(Error);
        expect(error.message).toBe('Channel worker IPC send failed: send boom');
    ...
    it('rejects webhook tasks when IPC result times out', async () => {
        ...
        await started;
        const accepted = supervisor.enqueueWebhookTask(webhookTask).then(() => undefined, (error) => error);
        await vi.advanceTimersByTimeAsync(30_000);
        const error = await accepted;
        expect(error).toBeInstanceOf(Error);
        expect(error.message).toBe('Channel webhook task IPC timed out.');
    });
    ...
    describe('restart()', () => {
        const waitForCalls = async (getCount, count) => {
...
//# sourceMappingURL=channel-worker-supervisor.test.js.map