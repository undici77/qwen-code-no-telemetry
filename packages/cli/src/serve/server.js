...
export function createServeApp(opts, getPort = () => opts.port, deps = {}) {
    ...
    if (!injectedWorkspaceRegistry &&
        ...
        process.stderr.write('qwen serve: createServeApp default fsFactory uses trusted=false ' +
    ...
    const bindLiveAppshotHandler = (runtime) => {
        ...
        setHandler.call(runtime.bridge, ({ callerSessionId }) => liveCoordinator.captureScreenContext(callerSessionId));
    ...
    const ensureLiveConversationRuntime = () => {
        ...
        const pending = (async () => {
            ...
            if (liveRuntime) {
                ...
                const entry = workspaceRegistry.getEntryByWorkspaceCwd(root.canonicalRoot);
                if (entry?.state !== 'active' ||
                    entry.current?.runtime !== liveRuntime ||
                    liveRuntime.provenance !== 'live-conversation' ||
                    !liveRuntime.trusted ||
                    liveRuntime.removable !== false) {
                    throw new Error('Live conversation runtime is no longer an active owned runtime.');
                }
            ...
            if (existing) {
                if (existing.provenance !== 'live-conversation' ||
                    !existing.trusted ||
                    existing.removable !== false) {
                    throw new Error('Live conversation root is already registered without Live provenance.');
                }
            ...
            const created = await runtimePublisher.publishOwnedRuntime(root.canonicalRoot, 'live-conversation', async (candidate) => {
                await conversationWorkspace.assertExactRoot(candidate.workspaceCwd);
                if (candidate.provenance !== 'live-conversation' ||
                    !candidate.trusted ||
                    candidate.removable !== false) {
                    throw new Error('Live conversation runtime failed its ownership gate.');
                }
            });
            liveRuntime = created;
            ...
            if (liveRuntimePromise === pending)
                liveRuntimePromise = undefined;
        });
