...
import { computeDaemonMemoryPressure, CRITICAL_PRESSURE_RATIO, HARD_PRESSURE_RATIO, SOFT_PRESSURE_RATIO, } from './daemon-memory-pressure.js';
...
function pressureAtBytes(rssBytes) {
...
describe('computeDaemonMemoryPressure', () => {
    ...
    it.each([Number.NaN, Number.POSITIVE_INFINITY, -1])('treats a %p gauge as unmeasured rather than classifying on it', (bad) => {
        ...
        // to make impossible.
        expect(computeDaemonMemoryPressure({
            rssBytes: bad,
            heapUsedBytes: bad,
            availableBytes: 8 * GB,
            heapLimitBytes: 4 * GB,
    ...
    it("falls back to this process's real V8 ceiling when none is given", () => {
        ...
        expect(result.heapRatio).toBeCloseTo((1 * GB) / getHeapStatistics().heap_size_limit, 10);
    });
...
//# sourceMappingURL=daemon-memory-pressure.test.js.map