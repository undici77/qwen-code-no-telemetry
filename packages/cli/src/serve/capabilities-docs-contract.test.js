...
describe('conditional serve capability documentation', () => {
    it('documents exactly the conditional feature registry keys', async () => {
        const protocol = await readFile(resolve(process.cwd(), '../../docs/developers/qwen-serve-protocol.md'), 'utf8');
        ...
        const table = protocol.slice(protocol.indexOf(START) + START.length, protocol.indexOf(END));
        const documented = [...table.matchAll(/^\|\s*`([^`]+)`\s*\|/gm)].map(([, tag]) => tag);
        expect(documented).toHaveLength(new Set(documented).size);
        expect([...documented].sort()).toEqual([...CONDITIONAL_SERVE_FEATURES.keys()].sort());
    });
});
//# sourceMappingURL=capabilities-docs-contract.test.js.map