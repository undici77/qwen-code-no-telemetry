...
import { createManagedScratchDirectory, isManagedScratchChild, isScratchRootCompatible, prepareManagedScratchRoot, } from './managed-scratch-workspace.js';
const cleanup = [];
afterEach(async () => {
...
describe('managed scratch workspace boundary', () => {
    it('accepts only disjoint workspaces and scratch-* direct children', () => {
        ...
        expect(isScratchRootCompatible('/managed/scratch-Ab3', '/managed')).toBe(true);
        ...
        expect(isScratchRootCompatible('/managed/scratch-Ab3/nested', '/managed')).toBe(false);
    });
    ...
    it('creates unique children for independent daemon root handles', async () => {
        ...
        expect(isManagedScratchChild(first, firstDaemonRoot.canonicalRoot)).toBe(true);
        expect(isManagedScratchChild(second, secondDaemonRoot.canonicalRoot)).toBe(true);
    });
    it('rejects symlink and writable roots', async () => {
        ...
        expect(() => prepareManagedScratchRoot(join(parent, 'link'), [])).toThrow(/non-symlink/);
        await chmod(target, 0o722);
        expect(() => prepareManagedScratchRoot(target, [])).toThrow(/only to its owner/);
    });
    it('rejects a root that conflicts with a startup workspace', async () => {
        const parent = await mkdtemp(join(realpathSync.native(tmpdir()), 'qws-'));
        cleanup.push(parent);
        const root = join(parent, 'root');
        expect(() => prepareManagedScratchRoot(root, [parent])).toThrow(/conflicts/);
    });
    it('fails closed if the accepted root is replaced', async () => {
        ...
        await expect(createManagedScratchDirectory(root)).rejects.toThrow(/identity changed/);
    });
});
//# sourceMappingURL=managed-scratch-workspace.test.js.map