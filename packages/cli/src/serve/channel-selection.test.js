...
import { channelSelectionNames, isAllChannelSelectionName, normalizeServeChannelSelection, } from './channel-selection.js';
describe('normalizeServeChannelSelection', () => {
    ...
    it('trims and de-duplicates repeated channel names', () => {
        expect(normalizeServeChannelSelection([' telegram ', 'feishu', 'telegram'])).toEqual({
            mode: 'names',
    ...
    it('rejects empty channel values', () => {
        expect(() => normalizeServeChannelSelection(['telegram', ' '])).toThrow('--channel requires a non-empty channel name.');
    });
    it('rejects all mixed with explicit channel names', () => {
        expect(() => normalizeServeChannelSelection(['all', 'telegram'])).toThrow('--channel all cannot be combined with channel names.');
    });
});
...
//# sourceMappingURL=channel-selection.test.js.map