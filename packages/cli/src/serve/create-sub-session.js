...
import { createDebugLogger, escapeXml, SessionService, stripTerminalControlSequences, } from '@qwen-code/qwen-code-core';
    ...
    try {
        ...
        void awaitRecoveredParentNotification(bridge, parentSessionId, notification, lastEventId, eventEpoch, stopSignal).then((continuationCompleted) => {
            if (!continuationCompleted && !stopSignal.aborted) {
                writeStderrLine(`qwen serve: restored parent ${parentSessionId} accepted completion ` +
        ...
        try {
                ...
                void (async () => {
                    try {
                        ...
                        try {
                            ...
                            if (!stopAc.signal.aborted) {
                                writeStderrLine(`qwen serve: sub-session ${sessionId} completion could not be returned to parent ${info.callerSessionId}: ${notificationError instanceof Error ? notificationError.message : String(notificationError)}`);
                            }
                        }
                    }
                    finally {
            ...
            if (spawnedSession !== undefined && isolatedWorkspace) {
                ...
                if (sessionClosed) {
                    if (!promptDispatched) {
                        try {
                            await new SessionService(boundWorkspace).removeSession(spawnedSession.sessionId);
                        }
                        catch (cleanupError) {
