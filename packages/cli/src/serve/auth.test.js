...
import { allowOriginCors, createMutationGate, denyBrowserOriginCors, InvalidAllowOriginPatternError, parseAllowOriginPatterns, } from './auth.js';
function invokeGate(handler, req = {}) {
    ...
    handler({ headers: req.headers ?? {} }, response, next);
    return { status, body, headers, nextCalled };
}
function invokeGatedRoute(deps, gateOpts) {
...
describe('createMutationGate (#4175 PR 15)', () => {
    it('passes through when --require-auth is on (global bearerAuth handles enforcement)', () => {
        ...
        const res = invokeGatedRoute({ tokenConfigured: true, requireAuth: true }, { strict: true });
    ...
    it('passes through when a token is configured (global bearerAuth handles enforcement)', () => {
        const res = invokeGatedRoute({ tokenConfigured: true, requireAuth: false }, { strict: true });
        expect(res.nextCalled).toBe(true);
        expect(res.status).toBeUndefined();
    });
    it('passes through on loopback no-token default for non-strict routes', () => {
        ...
        const res = invokeGatedRoute({ tokenConfigured: false, requireAuth: false });
        expect(res.nextCalled).toBe(true);
        expect(res.status).toBeUndefined();
    });
    it('refuses strict routes with token_required on loopback no-token default', () => {
        ...
        const res = invokeGatedRoute({ tokenConfigured: false, requireAuth: false }, { strict: true });
        expect(res.nextCalled).toBe(false);
...
function invokeAllowOrigin(handler, req = {}) {
    let status;
    let body;
    let nextCalled = false;
    let ended = false;
...
describe('parseAllowOriginPatterns (T2.4 #4514)', () => {
    ...
    it('rejects mixed-case host in the input (URL.origin normalizes, so the round-trip fails)', () => {
        ...
        expect(() => parseAllowOriginPatterns(['http://Localhost:3000'])).toThrow(InvalidAllowOriginPatternError);
    });
    ...
    it('accepts a browser-extension opaque-origin scheme (chrome-extension://<id>)', () => {
        ...
        expect(out.allowAny).toBe(false);
        expect(out.origins.has('chrome-extension://idkijaaipeeinemigojbjkmfmabokbdk')).toBe(true);
    });
    it('still rejects a trailing slash on an extension origin', () => {
        expect(() => parseAllowOriginPatterns([
            'chrome-extension://idkijaaipeeinemigojbjkmfmabokbdk/',
    ...
    it('rejects path components — origins do not carry paths', () => {
        expect(() => parseAllowOriginPatterns(['https://app.example.com/foo'])).toThrow(InvalidAllowOriginPatternError);
    });
    it('rejects userinfo — leaks credentials in capability metadata', () => {
        expect(() => parseAllowOriginPatterns(['http://user:pass@example.com'])).toThrow(InvalidAllowOriginPatternError);
    });
    it('rejects values that are not parseable URLs', () => {
        expect(() => parseAllowOriginPatterns(['not-a-url'])).toThrow(InvalidAllowOriginPatternError);
    });
    it('rejects URLs with empty hostname (http://:3000)', () => {
        ...
        expect(() => parseAllowOriginPatterns(['http://:3000'])).toThrow(InvalidAllowOriginPatternError);
    });
...
describe('allowOriginCors (T2.4 #4514)', () => {
    const middleware = allowOriginCors(parseAllowOriginPatterns(['http://localhost:3000']));
    ...
    it('matches an allowlisted origin, sets CORS headers, and calls next()', () => {
        ...
        expect(res.headers.get('access-control-allow-headers')).toMatch(/X-Qwen-Event-Epoch/);
        expect(res.headers.get('access-control-max-age')).toBe('86400');
        expect(res.headers.get('access-control-expose-headers')).toBe('Retry-After, X-Qwen-Event-Epoch, X-Qwen-SSE-Stream-Id');
    });
