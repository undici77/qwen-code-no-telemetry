...
import { registerA2uiActionRoutes, buildTransport, callA2uiAction, extractA2uiActionResult, findFromSettingsFile, usableServerConfig, } from './a2ui-action.js';
...
describe('POST /session/:id/a2ui-action', () => {
    ...
    it('falls back to workspace settings when daemon status is unavailable', async () => {
        ...
        await fsp.writeFile(path.join(ws, '.qwen', 'settings.json'), JSON.stringify({
...
describe('helpers', () => {
    ...
    it('keeps empty arrays and ignores invalid or non-array resources', () => {
        expect(extractA2uiActionResult({
            content: [
                {
                    type: 'resource',
                    resource: {
    ...
    it('throws MCP tool errors with text details when present', () => {
        ...
        expect(() => extractA2uiActionResult({ isError: true })).toThrow('a2ui action tool returned error: unknown error');
    });
    it('usableServerConfig accepts stdio or streamable-http shapes only', () => {
        ...
        expect(usableServerConfig({ command: 'node', httpUrl: 'not a url' })).toBe(false);
    ...
    it('buildTransport creates streamable HTTP transports', () => {
        ...
        const transport = sdkMocks.state.httpTransports[0];
    ...
    it('buildTransport uses the scrubbed runtime env for stdio transports', () => {
        ...
        const withEnv = sdkMocks.state.stdioTransports[0];
        const withoutEnv = sdkMocks.state.stdioTransports[1];
    ...
    it('callA2uiAction connects, calls the action tool, and closes resources', async () => {
        ...
        const client = sdkMocks.state.clients[0];
        expect(result).toEqual({ commands: null, fallback: 'ok' });
        expect(client.connect).toHaveBeenCalledWith(transport, {
            timeout: 15_000,
        });
        expect(client.callTool).toHaveBeenCalledWith({ name: 'action', arguments: { name: 'tap' } }, undefined, { timeout: 15_000 });
    ...
    it('callA2uiAction closes resources when connect fails', async () => {
        ...
        await expect(callA2uiAction({ command: 'node' }, { name: 'tap' })).rejects.toThrow('connect failed');
        const transport = sdkMocks.state.stdioTransports[0];
        const client = sdkMocks.state.clients[0];
...
//# sourceMappingURL=a2ui-action.test.js.map