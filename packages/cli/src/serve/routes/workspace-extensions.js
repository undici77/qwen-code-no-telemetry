...
import { parseInstallSource, redactUrlCredentials, getErrorMessage, SettingScope, } from '@qwen-code/qwen-code-core';
...
const validateExtensionSourceHost = (source, res) => {
...
export function registerWorkspaceExtensionRoutes(app, deps) {
    ...
    const mutationClientBridges = (runtimes) => (typeof runtimes === 'function'
        ? runtimes()
        : (runtimes ?? workspaceRegistry?.list()))?.map((runtime) => runtime.bridge) ?? [bridge];
        ...
        const reconcileExternalGeneration = async () => {
            ...
            try {
                ...
                if (runtimes.length === 0)
                    return;
                const results = await runtimeReconciliationQueue.run(async () => await Promise.allSettled(runtimes.map(async (runtime) => {
                ...
                results.forEach((result, index) => {
                    if (result.status === 'fulfilled') {
                        ...
                        writeStderrLine(`qwen serve: extension generation reconciliation failed for workspace ${runtimes[index].workspaceId}: ${redactUrlCredentials(result.reason instanceof Error
                ...
                writeStderrLine(`qwen serve: extension generation reconciliation failed: ${redactUrlCredentials(error instanceof Error ? error.message : String(error))}`);
            }
            finally {
                reconciling = false;
            }
    ...
    const registerFor = (base, resolve) => {
        ...
        app.post(`${base}/install-archive`, mutate({ strict: true }), extensionArchiveUploadMetadata, extensionArchiveBodyParser, async (req, res) => {
            ...
            try {
                ...
                ctrl.runQueuedExtensionMutation('install', { source }, res, async (extensionManager, _signal, context, operationId) => {
                    ...
                    let failure;
                    try {
                        uploadDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-extension-upload-'));
        ...
        app.post(`${base}/install`, mutate({ strict: true }), async (req, res) => {
            ...
            try {
                ...
                ctrl.runQueuedExtensionMutation('install', { source: sourceValue }, res, async (extensionManager, _signal, context, operationId) => {
                    const prepared = await context.prepare(async (signal) => {
    ...
    app.post('/extensions/install', mutate({ strict: true }), (req, res) => {
        ...
        const manager = primaryController.createExtensionManager(boundWorkspace, true);
        sendOperation(req, res, 'POST /extensions/install', manager, 'install', { source }, async (extensionManager, _signal, context) => {
            const prepared = await context.prepare(async (signal) => {
    ...
    app.post('/extensions/:extensionId/update', mutate({ strict: true }), (req, res) => {
        ...
        sendOperation(req, res, 'POST /extensions/:extensionId/update', manager, 'update', { name: extensionId }, async (extensionManager, _signal, context) => {
            ...
            if (extension.installMetadata?.type !== 'git' &&
                ...
                throw new Error(`Extension "${extension.name}" is not remotely updatable.`);
            }
