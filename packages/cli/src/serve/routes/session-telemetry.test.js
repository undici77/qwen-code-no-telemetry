...
function bridgeWithSessions(sessionIds = []) {
    return {
...
describe('special session resolver telemetry publication', () => {
    ...
    it('publishes the runtime root for creation before later validation', async () => {
        ...
        expect(telemetryMocks.setDaemonTelemetryWorkspace).toHaveBeenCalledWith(expect.anything(), primaryCwd);
    });
    it('publishes the secondary runtime selected for creation', async () => {
        ...
        expect(telemetryMocks.setDaemonTelemetryWorkspace).toHaveBeenCalledWith(expect.anything(), secondaryCwd);
    });
    ...
    it('publishes the single transcript runtime before storage lookup', async () => {
        ...
        const res = await request(makeApp([primary])).get('/session/missing/transcript');
    ...
    it('publishes the live transcript owner in a multi-workspace daemon', async () => {
        ...
        const res = await request(makeApp([primary, secondary])).get('/session/secondary-session/transcript');
        expect(res.status).toBe(200);
        expect(archiveMocks.assertSessionLoadable).toHaveBeenCalledTimes(1);
        expect(archiveMocks.assertSessionLoadable).toHaveBeenCalledWith(secondaryCwd, 'secondary-session', path.join(secondaryCwd, '.runtime'));
        expect(telemetryMocks.setDaemonTelemetryWorkspace).toHaveBeenCalledTimes(1);
        expect(telemetryMocks.setDaemonTelemetryWorkspace).toHaveBeenCalledWith(expect.anything(), secondaryCwd);
    });
    it('publishes the sole active transcript runtime after storage lookup', async () => {
        archiveMocks.assertSessionLoadable.mockImplementation(async (workspaceCwd, _sessionId, runtimeBaseDir) => runtimeBaseDir === path.join(secondaryCwd, '.runtime') &&
            workspaceCwd === secondaryCwd
            ? 'active'
            : undefined);
        const primary = runtime({
        ...
        const res = await request(makeApp([primary, secondary])).get('/session/stored-secondary/transcript');
        expect(res.status).toBe(200);
        expect(archiveMocks.assertSessionLoadable).toHaveBeenCalledTimes(2);
        expect(archiveMocks.assertSessionLoadable).toHaveBeenCalledWith(primaryCwd, 'stored-secondary', path.join(primaryCwd, '.runtime'));
        expect(archiveMocks.assertSessionLoadable).toHaveBeenCalledWith(secondaryCwd, 'stored-secondary', path.join(secondaryCwd, '.runtime'));
        expect(telemetryMocks.setDaemonTelemetryWorkspace).toHaveBeenCalledTimes(1);
        expect(telemetryMocks.setDaemonTelemetryWorkspace).toHaveBeenCalledWith(expect.anything(), secondaryCwd);
    });
    it('does not publish a workspace for ambiguous transcript storage matches', async () => {
        ...
        const res = await request(makeApp([primary, secondary])).get('/session/shared-storage/transcript');
        expect(res.status).toBe(500);
...
//# sourceMappingURL=session-telemetry.test.js.map