...
import { translateAndCheckAbsoluteWorkspacePath, MAX_WORKSPACE_PATH_LENGTH, } from '@qwen-code/acp-bridge/workspacePaths';
...
export function registerWorkspaceManagementRoutes(app, deps) {
    const { workspaceRegistry, mutate, safeBody, createWorkspaceRuntime, managedScratchRoot, validateWorkspaceRuntimeForPublication, runWorkspaceTrustOperation, workspaceRegistrationStore, getAcpHandle, runtimeRemoval, pickWorkspaceDirectory: pickWorkspaceDirectoryOverride, } = deps;
    ...
    const publishOwnedRuntime = async (canonicalCwd, provenance, validate) => {
        ...
        try {
            ...
            const publish = async () => {
                ...
                try {
                    ...
                    try {
                        writeStderrLine(`qwen serve: workspace runtime adapter notification failed after registry add: ${error instanceof Error ? error.message : String(error)}`);
                    }
                    catch {
    ...
    const createScratchWorkspace = async (res) => {
        ...
        if (workspaceRegistry
            .listManaged()
            .some((runtime) => !isScratchRootCompatible(runtime.workspaceCwd, managedScratchRoot.canonicalRoot)) ||
        ...
        try {
            ...
            writeStderrLine(`qwen serve: scratch workspace registration failed: ${err instanceof Error ? err.message : String(err)}${canonical ? `; retained directory: ${canonical}` : ''}`);
            if (!res.headersSent) {
                res.status(500).json({
    ...
    app.post('/workspace-directory-picker', mutate(), async (req, res) => {
        ...
        try {
            ...
            if (error instanceof NativeDirectoryPickerUnavailableError) {
                writeStderrLine(`qwen serve: native directory picker unavailable: ${detail}`);
                res.status(501).json({
            ...
            writeStderrLine(`qwen serve: native directory picker failed: ${detail}`);
            res.status(500).json({
    ...
    app.patch('/workspaces/:workspace', mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            writeStderrLine(`qwen serve: failed to persist workspace display name: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.delete('/workspaces/:workspace', mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            try {
                ...
                logCleanupFailure(`qwen serve: failed to commit workspace registry drain: ${err instanceof Error ? err.message : String(err)}`);
            }
                ...
                entries: snapshot.workspaces.map((cwd) => {
                    ...
                    return {
                        ...
                        ...(snapshot.displayNames?.[registrationId] !== undefined
                            ...
                            : {}),
                        active: runtime !== undefined || registrationIsActive(registrationId),
                        persisted: true,
                    };
                }),
            });
