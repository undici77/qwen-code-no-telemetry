...
describe('usage-stats route (cache + range + clamping)', () => {
    ...
    it('clamps the heatmapDays query', async () => {
        ...
        const fractional = await request(app).get('/usage/dashboard?heatmapDays=1.5');
        expect(fractional.body.heatmapDays).toBe(183);
        const trailingText = await request(app).get('/usage/dashboard?heatmapDays=7junk');
...
describe('usage-stats route (real loader against seeded history)', () => {
    ...
    it('returns real aggregated totals from usage_record.jsonl', async () => {
        const now = Date.now();
        fs.writeFileSync(path.join(process.env['QWEN_HOME'], 'usage_record.jsonl'), JSON.stringify(rec({
            sessionId: 's1',
            timestamp: now,
            totalTokens: 1200,
            inputTokens: 1000,
...
//# sourceMappingURL=usage-stats.test.js.map