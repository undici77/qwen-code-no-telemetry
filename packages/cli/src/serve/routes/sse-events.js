...
import { addDaemonRequestAttribute, captureDaemonTelemetryContext, emitDaemonLog, runWithDaemonTelemetryContext, } from '@qwen-code/qwen-code-core';
import { mapDomainErrorToErrorKind } from '@qwen-code/acp-bridge';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { SubscriberLimitExceededError, } from '@qwen-code/acp-bridge/eventBus';
...
function formatSseFrame(event) {
    ...
    const existingMeta = event._meta;
...
export function registerSseEventsRoutes(app, deps) {
    ...
    app.get('/session/:id/events', async (req, res) => {
        ...
        const telemetryBaseAttributes = {
            ...
            ...(previousStreamId
                ? {
                    'qwen-code.daemon.sse.client_reported_previous_stream_id': previousStreamId,
                }
                : {}),
        };
            ...
            if (diagnostic.type === 'slow_client_warning') {
                ...
                try {
                    if (daemonLog) {
                        ...
                        writeStderrLine(`qwen serve: SSE slow client warning ${JSON.stringify(context)}`);
                    }
                    handled = true;
                }
                catch {
                ...
                emitLifecycleLog('qwen-code.daemon.sse.slow_client_warning', 'Daemon SSE slow client warning.', {
                    ...telemetryCommon,
                ...
                try {
                    if (daemonLog) {
                        ...
                        writeStderrLine(`qwen serve: SSE client evicted ${JSON.stringify(context)}`);
                    }
                    handled = true;
                }
                catch {
                ...
                emitLifecycleLog('qwen-code.daemon.sse.client_evicted', 'Daemon SSE client evicted.', {
                    ...telemetryCommon,
                    'qwen-code.daemon.sse.threshold': threshold,
                    'qwen-code.daemon.sse.event_bus_eviction_reason': eventBusEvictionReason,
                    'qwen-code.daemon.sse.dropped_after_event_id': diagnostic.data.droppedAfter,
                    ...(diagnostic.data.eventBytes !== undefined
                        ? {
                            'qwen-code.daemon.sse.rejected_event_bytes': diagnostic.data.eventBytes,
                        }
                        : {}),
                });
            }
        ...
        try {
            ...
            if (err instanceof SubscriberLimitExceededError) {
                writeStderrLine(`qwen serve: subscriber limit reached for session ${diagnosticSessionId} (limit=${err.limit}); rejecting new SSE client with 429`);
        ...
        const finalize = () => {
            ...
            const closeAttributes = {
                ...
                'qwen-code.daemon.sse.event_frames_write_settled': eventFramesWriteSettled,
                'qwen-code.daemon.sse.backpressure_count': backpressureCount,
                'qwen-code.daemon.sse.max_drain_wait_ms': maxDrainWaitMs,
                'qwen-code.daemon.sse.max_live_publish_to_write_settled_ms': maxLivePublishToWriteSettledMs,
            ...
            void runWithDaemonTelemetryContext(telemetryContext, async () => {
                ...
                emitDaemonLog('Daemon SSE stream closed.', { ...telemetryBaseAttributes, ...closeAttributes }, { eventName: 'qwen-code.daemon.sse.closed' });
            }).catch(() => { });
            try {
        ...
        emitLifecycleLog('qwen-code.daemon.sse.opened', 'Daemon SSE stream opened.', {
            ...
            idleTimer = setInterval(() => {
                ...
                try {
                    if (daemonLog) {
                        ...
                        writeStderrLine(`qwen serve: evicting SSE client (session ${diagnosticSessionId}) — ` +
                ...
                emitLifecycleLog('qwen-code.daemon.sse.client_evicted', 'Daemon SSE client evicted by writer idle timeout.', {
                    'qwen-code.daemon.sse.writer_idle_for_ms': idleForMs,
                    'qwen-code.daemon.sse.writer_idle_timeout_ms': writerIdleTimeoutMsValue,
        ...
        res.on('error', (err) => {
            ...
            try {
                if (daemonLog) {
                    ...
                    writeStderrLine(`qwen serve: SSE socket error (session ${diagnosticSessionId}): ${err.message} ` +
                        `(streamId=${streamId}${clientId ? `, clientId=${clientId}` : ''})`);
                }
            }
            catch {
        ...
        void (async () => {
            try {
                    ...
                    if (next.value.type === 'state_resync_required') {
                        ...
                        try {
                            if (daemonLog) {
                                ...
                                writeStderrLine(`qwen serve: SSE ring eviction detected (session ${diagnosticSessionId}): ` +
                                    `lastEventId=${data.lastDeliveredId ?? '?'}, ` +
                                    `earliestInRing=${data.earliestAvailableId ?? '?'}, ` +
                                    `gap=${gap ?? '?'} events, ` +
                        ...
                        emitLifecycleLog('qwen-code.daemon.sse.state_resync_required', 'Daemon SSE state resync required.', {
                            ...
                            ...(typeof data.lastDeliveredId === 'number'
                                ? {
                                    'qwen-code.daemon.sse.resync_last_delivered_id': data.lastDeliveredId,
                                }
                                : {}),
                            ...(typeof data.earliestAvailableId === 'number'
                                ? {
                                    'qwen-code.daemon.sse.resync_earliest_available_id': data.earliestAvailableId,
                                }
                                : {}),
                            ...(gap !== undefined
                ...
                if (!res.writableEnded && closeReason !== 'socket_error') {
                    ...
                    writeStderrLine(`qwen serve: bridge iterator error (session ${diagnosticSessionId}): ` +
                        `${errorMessage(err)}` +
                        (errorKind ? ` [${errorKind}]` : ''));
