...
async function makeHarness(opts = {}) {
    const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `qwen-workspace-voice-${randomBytes(4).toString('hex')}-`));
    ...
    const app = createServeApp(serveOpts, undefined, deps);
...
async function writeVoiceModelSettings(h) {
...
async function writeVoiceProviderSettings(h, opts = {}) {
    await writeJson(path.join(h.home, 'settings.json'), {
        modelProviders: {
            openai: [
                {
...
async function writeWorkspaceVoiceEnabled(h, enabled) {
...
describe('workspace voice routes', () => {
    ...
    it('GET skips workspace voice settings for an untrusted runtime without an env snapshot', async () => {
        ...
        await fsp.writeFile(path.join(h.workspace, '.env'), 'VOICE_TRUST_LEAK=project-value', 'utf8');
        const app = express();
        registerWorkspaceVoiceRoutes(app, {
            boundWorkspace: h.workspace,
            mutate: () => (_req, _res, next) => next(),
    ...
    it('POST updates voice settings only after validating the resulting state', async () => {
        ...
        expect(res.status).toBe(200);
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'voiceModel', 'qwen3-asr-flash', expect.any(Function));
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.mode', 'hold', expect.any(Function));
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.language', 'english', expect.any(Function));
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.enabled', true, expect.any(Function));
    });
    it('POST writes initial trusted workspace voice settings to user scope', async () => {
        ...
        expect(res.status).toBe(200);
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.mode', 'tap', expect.any(Function));
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.enabled', true, expect.any(Function));
    });
    it('POST writes trusted workspace voice settings to workspace scope after workspace opt-in', async () => {
        ...
        expect(res.status).toBe(200);
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.Workspace, 'general.voice.mode', 'tap', expect.any(Function));
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.Workspace, 'general.voice.enabled', true, expect.any(Function));
    });
    ...
    it('POST rejects requests without recognized voice update fields', async () => {
        ...
        expect(res.body).toMatchObject({
            ...
            error: 'At least one of `enabled`, `mode`, `language`, or `voiceModel` must be provided',
        });
    ...
    it('POST broadcasts settings_changed for persisted voice settings', async () => {
        ...
        expect(broadcastSettingsChanged).toHaveBeenNthCalledWith(1, 'voiceModel', 'qwen3-asr-flash', 'user', 'client-1');
        expect(broadcastSettingsChanged).toHaveBeenNthCalledWith(2, 'general.voice.mode', 'hold', 'user', 'client-1');
        expect(broadcastSettingsChanged).toHaveBeenNthCalledWith(3, 'general.voice.language', 'english', 'user', 'client-1');
        expect(broadcastSettingsChanged).toHaveBeenNthCalledWith(4, 'general.voice.enabled', true, 'user', 'client-1');
    });
    it('POST broadcasts committed batch voice writes when batch persistence partially fails', async () => {
        ...
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('voiceModel', 'qwen3-asr-flash', 'user', 'client-1');
    });
    ...
    it('POST rejects enabling voice when the selected model config is unusable', async () => {
        ...
        expect(missingKey.body.error).toContain('requires QWEN_VOICE_TEST_MISSING_KEY');
        await writeJson(path.join(h.home, 'settings.json'), {
            modelProviders: {
                openai: [
                    {
    ...
    it('POST allows disabling voice without a selected model', async () => {
        const res = await request(h.app)
            ...
            .send({ enabled: false });
        expect(res.status).toBe(200);
        expect(h.persistSetting).toHaveBeenCalledWith(h.workspace, SettingScope.User, 'general.voice.enabled', false, expect.any(Function));
    });
    ...
    it('POST /workspace/voice/transcribe accepts binary audio and delegates transcription', async () => {
        ...
        const call = h.transcribe.mock.calls[0]?.[0];
    ...
    it('POST /workspace/voice/transcribe works without settings persistence', async () => {
        ...
        const res = await request(h.app)
            ...
            .send(Buffer.from([1, 2, 3, 4]));
        expect(res.status).toBe(200);
        expect(h.transcribe).toHaveBeenCalledWith(expect.objectContaining({ voiceModel: 'qwen3-asr-flash' }));
        expect(h.persistSetting).not.toHaveBeenCalled();
    });
    ...
    it('registers transcription as a strict mutation route', () => {
        const mutate = vi.fn(() => (_req, _res, next) => next());
    ...
    it('POST /workspace/voice/transcribe rejects duplicate voiceModel query parameters', async () => {
        await writeVoiceModelSettings(h);
        const res = await request(h.app)
            .post('/workspace/voice/transcribe?voiceModel=qwen3-asr-flash&voiceModel=other')
            .set('Host', hostHeader)
            .set('Authorization', 'Bearer secret')
            .set('Content-Type', 'audio/wav')
            .send(Buffer.from([1]));
    ...
    it('POST /workspace/voice/transcribe rejects empty audio and missing models', async () => {
        ...
        expect(h.transcribe).not.toHaveBeenCalledWith(expect.objectContaining({ voiceModel: expect.any(String) }));
    });
    ...
    it('POST /workspace/voice/transcribe redacts unexpected ASR errors', async () => {
        await writeVoiceModelSettings(h);
        h.transcribe.mockRejectedValueOnce(new Error('upstream failed with Authorization: ApiKey top-secret and sk-secret'));
        const res = await request(h.app)
            .post('/workspace/voice/transcribe')
            .set('Host', hostHeader)
            .set('Authorization', 'Bearer secret')
    ...
    it('POST /workspace/voice/transcribe reports draining when an active lease is aborted', async () => {
        ...
        const release = vi.fn();
        const transcribe = vi.fn(async (input) => await new Promise((_resolve, reject) => {
    ...
    it('retains the lease after disconnect until transcription cleanup settles', async () => {
        ...
        let operationSignal;
        let settle;
        const transcribe = vi.fn(async (input) => await new Promise((resolve) => {
...
//# sourceMappingURL=workspace-voice.test.js.map