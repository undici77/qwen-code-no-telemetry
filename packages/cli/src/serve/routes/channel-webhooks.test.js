...
function appHarness(opts) {
    ...
    const enqueueWebhookTask = opts?.enqueueWebhookTask ??
        vi.fn(async () => ({
            accepted: true,
        }));
...
describe('channel webhook routes', () => {
    ...
    it.each(['string payload', 123, true, ['array']])('rejects non-object payload values: %s', async (payload) => {
        const h = appHarness();
        const res = await request(h.app)
            .post('/channels/dingtalk-main/webhooks/github-ci')
            .set('x-qwen-webhook-secret', 'secret-value')
    ...
    it('rate limits pre-auth attempts with a bounded key', async () => {
        ...
        expect(rateLimiter.checkRate).toHaveBeenCalledWith(expect.stringMatching(/^webhook:preauth:/u), 'mutation');
    });
    it('rate limits authenticated requests by channel and source', async () => {
        ...
        expect(res.status).toBe(202);
        expect(rateLimiter.checkRate).toHaveBeenNthCalledWith(1, expect.stringMatching(/^webhook:preauth:/u), 'mutation');
        expect(rateLimiter.checkRate).toHaveBeenNthCalledWith(2, 'webhook:dingtalk-main:github-ci', 'mutation');
    });
    ...
    it.each(['eventType', 'targetRef', 'title'])('rejects missing required string field %s', async (field) => {
        const h = appHarness();
        const res = await request(h.app)
            .post('/channels/dingtalk-main/webhooks/github-ci')
            .set('x-qwen-webhook-secret', 'secret-value')
    ...
    it('returns 503 when the worker is unavailable', async () => {
        const h = appHarness({
            enqueueWebhookTask: vi.fn(async () => {
                throw new ChannelWebhookEnqueueError('channel_worker_unavailable', 'worker unavailable');
            }),
        });
        const res = await request(h.app)
            .post('/channels/dingtalk-main/webhooks/github-ci')
    ...
    it('returns 503 when the worker webhook queue is full', async () => {
        const h = appHarness({
            enqueueWebhookTask: vi.fn(async () => {
                throw new ChannelWebhookEnqueueError('channel_webhook_queue_full', 'queue full');
            }),
        });
        const res = await request(h.app)
            .post('/channels/dingtalk-main/webhooks/github-ci')
    ...
    it('returns 409 when the target cannot accept webhook work', async () => {
        const h = appHarness({
            enqueueWebhookTask: vi.fn(async () => {
                throw new ChannelWebhookEnqueueError('channel_webhook_target_unavailable', 'target unavailable');
            }),
        });
        const res = await request(h.app)
            .post('/channels/dingtalk-main/webhooks/github-ci')
    ...
    it('returns 400 when the worker rejects an invalid webhook task', async () => {
        const h = appHarness({
            enqueueWebhookTask: vi.fn(async () => {
                throw new ChannelWebhookEnqueueError('channel_webhook_invalid_task', 'invalid task');
            }),
        });
        const res = await request(h.app)
            .post('/channels/dingtalk-main/webhooks/github-ci')
    ...
    it('returns 504 when enqueueing the webhook task times out', async () => {
        const h = appHarness({
            enqueueWebhookTask: vi.fn(async () => {
                throw new ChannelWebhookEnqueueError('channel_webhook_enqueue_timeout', 'timed out');
            }),
        });
        const res = await request(h.app)
            .post('/channels/dingtalk-main/webhooks/github-ci')
...
//# sourceMappingURL=channel-webhooks.test.js.map