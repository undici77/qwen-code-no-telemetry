...
import { registerWorkspaceGitRoutes, registerWorkspaceQualifiedGitRoutes, } from './workspace-git.js';
...
describe('workspace Git routes', () => {
    ...
    it('uses the selected trusted workspace runtime', async () => {
        ...
        expect(getStatus).toHaveBeenCalledWith(secondary.workspaceCwd, secondary.bridge, { wait: false });
    });
    it('passes wait:true through to the qualified workspace git state', async () => {
        ...
        expect(response.status).toBe(200);
        expect(getStatus).toHaveBeenCalledWith(primary.workspaceCwd, primary.bridge, {
            wait: true,
        });
    });
...
//# sourceMappingURL=workspace-git.test.js.map