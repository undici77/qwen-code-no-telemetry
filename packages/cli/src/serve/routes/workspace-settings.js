...
export function registerWorkspaceSettingsRoutes(app, deps) {
    ...
    app.get('/workspace/settings', (_req, res) => {
        try {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`qwen serve: GET /workspace/settings error: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.post('/workspace/settings', mutate({ strict: true }), async (req, res) => {
        ...
        try {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`qwen serve: POST /workspace/settings persist error (key=${key}, scope=${scope}, workspace=${boundWorkspace}): ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
        ...
        try {
            ...
            writeStderrLine(`qwen serve: POST /workspace/settings broadcast error (key=${key}, scope=${scope}): ${err instanceof Error ? err.message : String(err)}`);
        }
        res.status(200).json({
            key,
            scope,
