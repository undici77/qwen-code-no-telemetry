...
function makeApp(overrides = {}) {
    ...
    registerWorkspaceSettingsRoutes(app, {
        ...
        safeBody: (req) => req.body && typeof req.body === 'object' ? req.body : {},
        persistSetting,
...
describe('POST /workspace/settings', () => {
    it('exposes the Live shortcut as user-global and rejects generic writes', async () => {
        ...
        const shortcut = read.body.settings.find((setting) => setting.key === 'experimental.liveVoice.shortcut');
    ...
    it('exposes disabled Live setup settings on the supported WebShell surface', async () => {
        const { app } = makeApp();
        const read = await request(app).get('/workspace/settings');
        expect(read.body.settings.map((setting) => setting.key)).toEqual(expect.arrayContaining([
    ...
    it.each([0, 30])('accepts general.cleanupPeriodDays=%s', async (value) => {
        ...
        expect(persistSetting).toHaveBeenCalledWith('/workspace', expect.any(String), 'general.cleanupPeriodDays', value);
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('general.cleanupPeriodDays', value, 'workspace', undefined);
    });
    it('persists to the user scope (~/.qwen/settings.json)', async () => {
        ...
        expect(persistSetting).toHaveBeenCalledWith('/workspace', 'User', 'general.cleanupPeriodDays', 7);
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('general.cleanupPeriodDays', 7, 'user', undefined);
    });
    ...
    it.each(['ui.mouseTracking', 'ui.showScrollbar'])('rejects a TUI-only key (%s) that has no effect in the web shell', async (key) => {
    ...
    it.each(['workspace', 'user'])('accepts %s mcpServers for the MCP manager', async (scope) => {
        const { app, persistSetting, broadcastSettingsChanged } = makeApp();
        const value = {
            filesystem: {
                command: 'npx',
        ...
        expect(persistSetting).toHaveBeenCalledWith('/workspace', expect.any(String), 'mcpServers', value);
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('mcpServers', value, scope, undefined);
    });
    it('atomically adds one MCP server without replacing existing servers', async () => {
        ...
        expect(res.status).toBe(200);
        expect(persistSetting).toHaveBeenCalledWith('/workspace', expect.any(String), 'mcpServers', {
    ...
    it('atomically removes only the named MCP server', async () => {
        ...
        expect(res.status).toBe(200);
        expect(persistSetting).toHaveBeenCalledWith('/workspace', expect.any(String), 'mcpServers', { keep: { command: 'keep-server' } });
    });
    it('redacts MCP secrets in reads and restores them on writes', async () => {
        ...
        const redacted = read.body.settings.find((setting) => setting.key === 'mcpServers').values.workspace;
        const write = await request(app).post('/workspace/settings').send({
            scope: 'workspace',
            key: 'mcpServers',
            value: redacted,
        ...
        expect(persistSetting).toHaveBeenCalledWith('/workspace', 'Workspace', 'mcpServers', existing);
        expect(JSON.stringify(broadcastSettingsChanged.mock.calls)).not.toContain('env-secret');
    });
    ...
    it.each([1, 5])('accepts general.sessionRecapAwayThresholdMinutes=%s', async (value) => {
        const { app, persistSetting, broadcastSettingsChanged } = makeApp();
        const res = await request(app).post('/workspace/settings').send({
            scope: 'workspace',
            key: 'general.sessionRecapAwayThresholdMinutes',
        ...
        expect(persistSetting).toHaveBeenCalledWith('/workspace', expect.any(String), 'general.sessionRecapAwayThresholdMinutes', value);
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('general.sessionRecapAwayThresholdMinutes', value, 'workspace', undefined);
    });
});
//# sourceMappingURL=workspace-settings.test.js.map