...
import { MAX_READ_BYTES, isFsError, } from '../fs/index.js';
/**
...
export function sendFsError(res, err, route) {
    ...
    writeStderrLine(`qwen serve: ${route} unexpected error: ${err instanceof Error ? err.message : String(err)}`);
    res.status(500).json({
...
function requireStringQuery(res, raw, name, route) {
    if (typeof raw !== 'string' || raw.length === 0) {
        ...
        writeStderrLine(`qwen serve: ${route} rejected request missing required ?${name}`);
        return null;
    }
    return raw;
}
...
export function workspaceRelative(req, resolved) {
    const boundWorkspace = req.app.locals
        .boundWorkspace;
    if (!boundWorkspace) {
...
//# sourceMappingURL=workspaceFileRead.js.map