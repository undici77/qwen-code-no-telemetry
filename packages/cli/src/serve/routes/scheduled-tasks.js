...
import { readCronTasks, updateCronTasks, generateCronTaskId, appendCronRun, taskHasLegacyCondition, parseCron, nextFireTime, nextDurableFireMs, SessionService, Storage, stripTerminalControlSequences, MAX_JOBS, } from '@qwen-code/qwen-code-core';
...
async function rollbackCronMutation(target, before, after, route) {
    ...
    await runWithScheduledTaskTarget(target, () => updateCronTasks(target.workspaceCwd, (tasks) => isDeepStrictEqual(tasks, after) ? before : tasks)).catch((error) => {
        writeStderrLine(`qwen serve: ${route} failed to roll back a stale task mutation: ${error instanceof Error ? error.message : String(error)}`);
    });
}
...
function runWithScheduledTaskTarget(target, fn) {
...
function registerScheduledTaskCrudRoutes(app, deps) {
    ...
    app.get(base, async (req, res) => {
        ...
        try {
            ...
            // behind a silent [].
            writeStderrLine(`qwen serve: GET ${base} failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.post(base, mutate(), async (req, res) => {
        ...
        if (bridge) {
            ...
            try {
                ...
                if (sendGenerationClosedError(res, err))
                    return;
                writeStderrLine(`qwen serve: POST ${base} failed to create the task's session: ${err instanceof Error ? err.message : String(err)}`);
                res.status(500).json({
        ...
        try {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`qwen serve: POST ${base} failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.patch(`${base}/:id`, mutate(), async (req, res) => {
        ...
        try {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`qwen serve: PATCH ${base}/${id} failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.delete(`${base}/:id`, mutate(), async (req, res) => {
        ...
        try {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`qwen serve: DELETE ${base}/${id} failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.post(`${base}/:id/run`, mutate(), async (req, res) => {
        ...
        try {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`qwen serve: POST ${base}/${id}/run failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
