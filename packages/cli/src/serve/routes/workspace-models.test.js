...
function readWorkspaceSettings() {
    return JSON.parse(fs.readFileSync(path.join(workspace, '.qwen', 'settings.json'), 'utf8'));
}
function makeApp(overrides = {}) {
    ...
    const mutate = vi.fn((_opts) => (_req, _res, next) => next());
    // Real persistence: mirrors the daemon's batch persist (setValues).
    const persistSettings = vi.fn(async (ws, writes) => {
    ...
    registerWorkspaceModelsRoutes(app, {
        ...
        parseAndValidateClientId: overrides.parseAndValidateClientId ?? (() => undefined),
...
describe('DELETE /workspace/models', () => {
    ...
    it('removes a model from ~/.qwen/settings.json and keeps siblings', async () => {
        ...
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('modelProviders', { openai: [{ id: 'deepseek-v4' }] }, 'user', undefined);
    ...
    it('writes to the workspace scope when the workspace owns modelProviders', async () => {
        ...
        const res = await request(app)
            ...
            .send({ authType: 'openai', modelId: 'gpt-4o' });
        expect(res.status).toBe(200);
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('modelProviders', { openai: [{ id: 'deepseek-v4' }] }, 'workspace', undefined);
    ...
    it('forwards the validated client id to broadcasts so the origin is skipped', async () => {
        ...
        const res = await request(app)
            ...
            .send({ authType: 'openai', modelId: 'gpt-4o' });
        expect(res.status).toBe(200);
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('modelProviders', { openai: [] }, 'user', 'client-123');
    });
    ...
    it('broadcasts committed writes on a partial persistence failure', async () => {
        ...
        persistSettings.mockImplementationOnce(async (_ws, writes) => {
            ...
            throw new WorkspaceSettingsPartialPersistError('partial', [writes[0]], new Error('disk full'));
        });
        const res = await request(app)
            .delete('/workspace/models')
            .send({ authType: 'openai', modelId: 'gpt-4o' });
        ...
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('modelProviders', { openai: [] }, 'user', undefined);
    });
    ...
    it('scrubs the deleted model from modelFallbacks', async () => {
        ...
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('modelFallbacks', 'deepseek-v4', 'user', undefined);
    });
    it('clears a workspace-scoped active selection when providers are user-owned', async () => {
        ...
        expect(readWorkspaceSettings()['model'].name).toBe('');
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('model.name', '', 'workspace', undefined);
    });
    it('clears the active model when the stored baseUrl carries credentials the request sanitized', async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.body.clearedActiveModel).toBe(true);
        const model = readUserSettings()['model'];
        expect(model.name).toBe('');
        expect(model.baseUrl).toBe('');
    });
    it('scrubs modelFallbacks in its own owning scope, not the providers scope', async () => {
        ...
        expect(broadcastSettingsChanged).toHaveBeenCalledWith('modelFallbacks', 'deepseek-v4', 'workspace', undefined);
    });
    it('keeps a fallback when another provider still has the same base id', async () => {
        ...
        expect(broadcastSettingsChanged).not.toHaveBeenCalledWith('modelFallbacks', expect.anything(), expect.anything(), expect.anything());
    });
});
//# sourceMappingURL=workspace-models.test.js.map