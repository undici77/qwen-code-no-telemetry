...
import { registerWorkspaceGitBranchRoutes, registerWorkspaceQualifiedGitBranchRoutes, } from './workspace-git-branches.js';
...
describe('workspace Git branch routes', () => {
    it.each(['-evil', '-f', '--output=/tmp/pwn'])('rejects a dash-prefixed branch name %s with 400 invalid_branch_name', async (name) => {
...
function makeRepo() {
    const dir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-gitbranch-route-')));
    tmpRoots.push(dir);
    git(dir, 'init', '-q');
...
describe('workspace Git branch routes against a real repo (R10 #2)', () => {
    ...
    it('classifies a pull with no tracking information as no_upstream', async () => {
        const dir = makeRepo();
        const remote = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-gitbranch-remote-')));
    ...
    it('does not misclassify a non-dirty error when the workspace path contains "dirty"', async () => {
        const parent = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-dirty-utils-')));
...
describe('workspace qualified Git branch routes (generation guard)', () => {
    function qualifiedRuntime(workspaceId, workspaceCwd, trusted) {
        return {
            workspaceId,
            workspaceCwd,
...
describe('workspace qualified Git branch routes (input validation)', () => {
    function trustedRuntime(workspaceCwd) {
        return {
            workspaceId: 'primary',
            workspaceCwd,
            primary: true,
    ...
    it('lists branches from a real repo via the qualified route', async () => {
        ...
        const names = response.body.local.map((b) => b.name);
        expect(names).toContain(response.body.head);
        expect(names).toContain('feature-x');
    });
});
//# sourceMappingURL=workspace-git-branches.test.js.map