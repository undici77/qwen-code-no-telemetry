...
import { canonicalizeWorkspace, InvalidClientIdError, PromptQueueFullError, SessionArtifactValidationError, SessionArchivedError, SessionConflictError, SessionNotFoundError, SessionShellClientRequiredError, SessionShellDisabledError, } from '../acp-session-bridge.js';
...
export function registerSessionRoutes(app, deps) {
    const { boundWorkspace, bridge, workspaceRegistry, archiveCoordinator, mutate, sendBridgeError, daemonLog, promptDeadlineMs, sessionShellCommandEnabled, virtualSubagentSessions, } = deps;
    ...
    const captureRuntimeGenerationAssertion = (runtime) => {
        const registeredGeneration = workspaceRegistry.getEntryByWorkspaceId(runtime.workspaceId)?.current;
    ...
    const handleSessionExport = async (req, res, target) => {
        ...
        try {
            const result = await archiveCoordinator.runSharedMany([sessionId], () => runWithWorkspaceRuntimeStorage(target.runtime, async () => {
                if (target.archiveState === 'archived') {
                    await assertSessionArchived(target.runtime.workspaceCwd, sessionId, target.runtime.sessionRuntimeBaseDir);
                }
                else {
                    await assertSessionLoadable(target.runtime.workspaceCwd, sessionId, target.runtime.sessionRuntimeBaseDir);
                }
    ...
    const resolveRuntimeForSessionRestore = (body, res, route, sessionId) => {
        const cwd = parseOptionalWorkspaceCwd(body, boundWorkspace, res);
        if (cwd === undefined)
            return undefined;
        let key;
        ...
        const runtime = workspaceRegistry.resolveWorkspaceCwd('cwd' in body ? key : undefined);
        if (!runtime) {
            logSessionRoutingFailure(route, 'workspace_mismatch', {
                requestedWorkspace: key,
            });
    ...
    const resolveTranscriptSessionRuntime = async (res, route, sessionId, hasCursor) => {
                ...
                if (loadError === undefined ||
                    ...
                    if (loadError !== undefined &&
                        ...
                        logSessionRoutingFailure(route, 'transcript_resolution_error_superseded', {
                            sessionId,
                            supersededError: loadError instanceof Error
                                ? loadError.name
                                : String(loadError),
    ...
    const serializeSessionErrors = (errors) => errors.map((e) => ({
        sessionId: e.sessionId,
        error: e.error instanceof Error ? e.error.message : String(e.error),
    }));
    const withPrimaryOnlyMutableSession = (route, handler) => {
            ...
            if (worktreeMeta) {
                try {
                    ...
                    if (createRepoTop && createRepoTop !== workspaceCwd) {
                        createAllowedRoots.push(path.join(createRepoTop, '.qwen', 'worktrees'));
                    }
    ...
    const restoreSessionHandler = (action) => async (req, res) => {
        ...
        let resolvedRuntime;
        try {
            resolvedRuntime = resolveRuntimeForSessionRestore(body, res, route, sessionId);
        }
        catch (err) {
            sendBridgeError(res, err, { route, sessionId });
            return;
        ...
        try {
            const session = await archiveCoordinator.runSharedMany([sessionId], async () => {
                await assertSessionLoadable(workspaceCwd, sessionId, runtime.sessionRuntimeBaseDir);
            ...
            if (runtime.provenance !== 'live-conversation' && !session.worktree) {
                const sidecar = await readWorktreeSession(createWorkspaceRuntimeSessionService(runtime).getWorktreeSessionPath(sessionId)).catch(() => null);
                if (sidecar) {
    ...
    app.post('/session/:id/branch', mutate(), withPrimaryOnlyMutableSession('POST /session/:id/branch', async (req, res, sessionId, runtime) => {
        const body = safeBody(req);
        ...
        if (clientId === null)
            return;
        const result = await runtime.bridge.branchSession(sessionId, { name }, { clientId });
        try {
            runtime.generationGuard?.assertOpen();
        }
        catch (error) {
        ...
        if (!res.writable) {
            if (!result.attached) {
                void runWithWorkspaceRuntimeStorage(runtime, () => deleteDaemonSessionIfOrphan({
                    sessionId: result.sessionId,
                    service: createWorkspaceRuntimeSessionService(runtime),
                    bridge: runtime.bridge,
                    coordinator: archiveCoordinator,
    ...
    app.post('/session/:id/side-task', mutate(), withPrimaryOnlyMutableSession('POST /session/:id/side-task', async (req, res, sessionId, runtime) => {
        const body = safeBody(req);
        let name = typeof body?.['name'] === 'string' ? body['name'] : undefined;
        if (name) {
            // eslint-disable-next-line no-control-regex
        ...
        if (!res.writable) {
            if (!result.attached) {
                runtime.bridge
                    ...
                    if (!killed)
                        return undefined;
                    return createWorkspaceRuntimeSessionService(runtime).removeSession(result.sessionId);
                })
                    .catch(() => { });
            }
            else {
    ...
    app.post('/session/:id/fork', mutate(), withPrimaryOnlyMutableSession('POST /session/:id/fork', async (req, res, sessionId, runtime) => {
        const body = safeBody(req);
    ...
    app.post('/session/:id/cd', mutate(), withPrimaryOnlyMutableSession('POST /session/:id/cd', async (req, res, sessionId, runtime) => {
        const body = safeBody(req);
    ...
    app.get('/workspaces/:workspace/session/:id/transcript', async (req, res) => {
        ...
        try {
            const result = await runWithoutDebugLogSession(() => archiveCoordinator.runSharedMany([sessionId], () => runWithWorkspaceRuntimeStorage(runtime, async () => {
    ...
    app.post('/session/:id/prompt', mutate(), withOwnerMutableSession('POST /session/:id/prompt', async (req, res, sessionId, runtime) => {
        ...
        const trustedPromptDisplayText = typeof promptDisplayText === 'string' &&
            isChannelWorkerPromptAuthorized(promptAuthorization, runtime.workspaceCwd)
            ? promptDisplayText
            : undefined;
    ...
    app.post('/sessions/delete', mutate(), async (req, res) => {
        ...
        try {
            ...
            const service = createWorkspaceRuntimeSessionService(runtime);
            const result = await runWithSessionListInvalidation(runtime, ['active', 'archived'], () => runWithWorkspaceRuntimeStorage(runtime, () => deleteDaemonSessions({
    ...
    app.post('/sessions/archive', mutate(), async (req, res) => {
        ...
        try {
            const result = await runWithSessionListInvalidation(runtime, ['active', 'archived'], () => runWithWorkspaceRuntimeStorage(runtime, () => archiveDaemonSessions({
                sessionIds: uniqueIds,
                service,
                bridge,
                coordinator: archiveCoordinator,
    ...
    app.post('/sessions/unarchive', mutate(), async (req, res) => {
        ...
        try {
            const result = await runWithSessionListInvalidation(runtime, ['active', 'archived'], () => runWithWorkspaceRuntimeStorage(runtime, () => unarchiveDaemonSessions({
                sessionIds: uniqueIds,
                service,
                coordinator: archiveCoordinator,
            })));
    ...
    const handleSessionOrganizationUpdate = async (req, res, target) => {
        ...
        if (sessionId === null)
            return;
        try {
            await archiveCoordinator.runSharedMany([sessionId], () => runWithWorkspaceRuntimeStorage(target.runtime, async () => {
                ...
                const sessionService = createWorkspaceRuntimeSessionService(target.runtime);
                let exists = await sessionService.sessionExistsInAnyState(sessionId);
                if (!exists) {
                    try {
                        const summary = target.runtime.bridge.getSessionSummary(sessionId);
                ...
                const organization = await createSessionOrganizationService(target.runtime.workspaceCwd).updateSessionOrganization(sessionId, {
        ...
        try {
            ...
            try {
                ...
                const result = usePersisted
                    ? await runWorkspaceInspectionWithLogPolicy(runtime, () => listWorkspaceSessionsForResponse(runtime.bridge, key, options, {
