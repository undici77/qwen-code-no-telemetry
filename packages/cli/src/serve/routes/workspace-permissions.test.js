...
import { MAX_PERMISSION_RULES_COUNT, MAX_PERMISSION_RULE_LENGTH, } from '../../config/permission-settings.js';
...
async function makeHarness(opts) {
    const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `qwen-permission-routes-${randomBytes(4).toString('hex')}-`));
    ...
    const setWorkspacePermissionRules = opts?.setWorkspacePermissionRules ??
        vi.fn(async () => {
...
describe('workspace permissions routes', () => {
    ...
    it('POST rejects oversized rule lists before invoking ACP', async () => {
        const tooManyRules = await request(h.app)
            ...
            scope: 'user',
            ruleType: 'allow',
            rules: Array.from({ length: MAX_PERMISSION_RULES_COUNT + 1 }, (_, index) => `Bash(echo ${index})`),
        });
...
//# sourceMappingURL=workspace-permissions.test.js.map