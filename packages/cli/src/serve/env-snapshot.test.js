...
import { ENV_NONSECRET_VARS, ENV_PROXY_VARS, ENV_SECRET_VARS, buildEnvStatusFromEnv, buildEnvStatusFromProcess, readProxyVar, snapshotProcessEnv, } from './env-snapshot.js';
const TRACKED_ENV = [
    ...ENV_SECRET_VARS,
    ...ENV_NONSECRET_VARS,
    ...ENV_PROXY_VARS,
...
describe('buildEnvStatusFromProcess', () => {
    ...
    it('can build a snapshot from an injected runtime env without reading process.env', () => {
        ...
        const key = status.cells.find((c) => c.kind === 'env_var' && c.name === 'DASHSCOPE_API_KEY');
        expect(key).toMatchObject({ present: true, status: 'ok' });
        const proxy = status.cells.find((c) => c.kind === 'proxy' && c.name === 'HTTPS_PROXY');
    ...
    it('emits a runtime cell whose value matches the actual runtime version', () => {
        ...
        const expected = runtime.name === 'bun'
    ...
    it('marks SANDBOX disabled when unset and ok with the profile name when set', () => {
        ...
        let cell = status.cells.find((c) => c.kind === 'sandbox' && c.name === 'SANDBOX');
        ...
        cell = status.cells.find((c) => c.kind === 'sandbox' && c.name === 'SANDBOX');
    ...
    it('redacts user:pass from proxy URLs and surfaces only the host:port', () => {
        ...
        const status = buildEnvStatusFromProcess('/ws', false);
        const cell = status.cells.find((c) => c.kind === 'proxy' && c.name === 'HTTPS_PROXY');
        expect(cell.present).toBe(true);
    ...
    it('falls back to a scrubbed authority for unparseable proxy values rather than the raw input', () => {
        ...
        const status = buildEnvStatusFromProcess('/ws', false);
        const cell = status.cells.find((c) => c.kind === 'proxy' && c.name === 'HTTP_PROXY');
        expect(cell.present).toBe(true);
    ...
    it('readProxyVar uses ?? not || so an explicit empty string disables fallthrough', () => {
        ...
        const explicitlyDisabled = readProxyVar({ HTTPS_PROXY: '', https_proxy: 'http://proxy.parent:3128' }, 'HTTPS_PROXY');
        ...
        const lowercaseFallback = readProxyVar({ https_proxy: 'http://proxy.parent:3128' }, 'HTTPS_PROXY');
    ...
    it('passes NO_PROXY through redaction without URL parsing', () => {
        ...
        const status = buildEnvStatusFromProcess('/ws', false);
        const cell = status.cells.find((c) => c.kind === 'proxy' && c.name === 'NO_PROXY');
        expect(cell.present).toBe(true);
    ...
    it('emits env_var cells presence-only — never includes a value field', () => {
        ...
        for (const cell of status.cells) {
            if (cell.kind !== 'env_var')
                continue;
            expect('value' in cell).toBe(false);
        }
        const apiKey = status.cells.find((c) => c.kind === 'env_var' && c.name === 'OPENAI_API_KEY');
        expect(apiKey.present).toBe(true);
        expect(apiKey.status).toBe('ok');
        const baseUrl = status.cells.find((c) => c.kind === 'env_var' && c.name === 'ANTHROPIC_BASE_URL');
    ...
    it('does not enumerate non-whitelisted secrets even when set', () => {
        ...
        const status = buildEnvStatusFromProcess('/ws', false);
        expect(status.cells.some((c) => c.name === 'SOME_OTHER_SECRET_KEY' || c.value === 'leak-me')).toBe(false);
    });
...
//# sourceMappingURL=env-snapshot.test.js.map