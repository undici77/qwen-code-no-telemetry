...
function renderCtx(ctx) {
    ...
    const extraKeys = Object.keys(ctx)
        .filter((key) => !FIXED_CTX_SET.has(key) && ctx[key] !== undefined && ctx[key] !== null)
        .sort();
...
function createStderrOnlyLogger(input) {
    const tee = (level, message, ctx, err) => {
        input.stderr(buildDaemonLogLine({
            level,
            message,
            now: input.now(),
            ctx,
...
function createConcreteLogger(input) {
    ...
    const tee = (level, message, ctx, err) => {
        ...
        submitFileRecord(capUtf8Record(buildDaemonFileLogLine({ level, message, now: timestamp, ctx, err }, input.runId, input.pid), input.policy.maxRecordBytes));
    };
