...
describe('rateLimit', () => {
    describe('token bucket - continuous drip', () => {
        ...
        it('includes Retry-After header on 429', () => {
            ...
            expect(Number(res.setHeader.mock.calls.find((c) => c[0] === 'Retry-After')?.[1])).toBeGreaterThan(0);
            limiter.dispose();
        });
    ...
    describe('tier resolution', () => {
        ...
        it('classifies POST .../prompt as prompt tier', () => {
            const next = vi.fn();
            limiter.middleware(mockReq({ path: '/session/x/prompt' }), mockRes(), next);
        ...
        it('classifies POST /session as mutation tier', () => {
            const next = vi.fn();
            limiter.middleware(mockReq({ method: 'POST', path: '/session' }), mockRes(), next);
            expect(next).toHaveBeenCalled();
            const res = mockRes();
            limiter.middleware(mockReq({ method: 'POST', path: '/session' }), res, vi.fn());
            expect(res.body).toMatchObject({ tier: 'mutation' });
        });
        it('classifies GET as read tier', () => {
            const next = vi.fn();
            limiter.middleware(mockReq({ method: 'GET', path: '/workspace/mcp' }), mockRes(), next);
            expect(next).toHaveBeenCalled();
            const res = mockRes();
            limiter.middleware(mockReq({ method: 'GET', path: '/workspace/mcp' }), res, vi.fn());
            expect(res.body).toMatchObject({ tier: 'read' });
        });
        it('classifies plural workspace session listing as read tier', () => {
            const next = vi.fn();
            limiter.middleware(mockReq({
                method: 'GET',
        ...
        it('exempts GET /health', () => {
            const next = vi.fn();
            for (let i = 0; i < 5; i++) {
                limiter.middleware(mockReq({ method: 'GET', path: '/health' }), mockRes(), next);
            }
        ...
        it('exempts POST .../heartbeat', () => {
            const next = vi.fn();
            for (let i = 0; i < 5; i++) {
                limiter.middleware(mockReq({ method: 'POST', path: '/session/x/heartbeat' }), mockRes(), next);
            }
            expect(next).toHaveBeenCalledTimes(5);
        });
        it('exempts GET .../events (SSE)', () => {
            const next = vi.fn();
            for (let i = 0; i < 5; i++) {
                limiter.middleware(mockReq({ method: 'GET', path: '/session/x/events' }), mockRes(), next);
            }
            expect(next).toHaveBeenCalledTimes(5);
        });
        it('exempts /acp routes', () => {
            const next = vi.fn();
            for (let i = 0; i < 5; i++) {
                limiter.middleware(mockReq({ method: 'POST', path: '/acp' }), mockRes(), next);
            }
            expect(next).toHaveBeenCalledTimes(5);
        });
        it('exempts OPTIONS requests', () => {
            const next = vi.fn();
            for (let i = 0; i < 5; i++) {
                limiter.middleware(mockReq({ method: 'OPTIONS', path: '/session/x/prompt' }), mockRes(), next);
            }
            expect(next).toHaveBeenCalledTimes(5);
        });
    ...
    describe('key extraction', () => {
        it('uses client-id on loopback', () => {
            ...
            const req = mockReq({
                get: vi
                    .fn()
                    .mockImplementation((h) => h === 'x-qwen-client-id' ? 'my-client' : undefined),
            });
        ...
        it('uses IP on non-loopback', () => {
            ...
            expect(extractor(req)).toBe('10.0.0.5');
        });
        ...
        it('combines IP and client-id on non-loopback', () => {
            ...
            const req = mockReq({
                get: vi
                    .fn()
                    .mockImplementation((h) => h === 'x-qwen-client-id' ? 'web-1' : undefined),
                socket: { remoteAddress: '10.0.0.5' },
            });
            expect(extractor(req)).toBe('10.0.0.5:web-1');
        });
        it('rejects invalid client-id', () => {
            ...
            const req = mockReq({
                get: vi
                    .fn()
                    .mockImplementation((h) => h === 'x-qwen-client-id' ? 'bad id with spaces' : undefined),
            });
            expect(extractor(req)).toBe('anonymous');
        });
    });
    describe('fail-open', () => {
        it('passes through when bucket map is at capacity', () => {
            ...
            for (let i = 0; i < 10_001; i++) {
                ...
                limiter.middleware(req, mockRes(), vi.fn());
            }
            ...
            limiter.middleware(req, mockRes(), next);
            expect(next).toHaveBeenCalled();
            limiter.dispose();
        });
    });
    ...
    describe('getHitCounts', () => {
        it('tracks hits per tier', () => {
            ...
            limiter.middleware(mockReq(), mockRes(), vi.fn());
            // Trigger read hit
            limiter.middleware(mockReq({ method: 'GET', path: '/capabilities' }), mockRes(), vi.fn());
            limiter.middleware(mockReq({ method: 'GET', path: '/capabilities' }), mockRes(), vi.fn());
