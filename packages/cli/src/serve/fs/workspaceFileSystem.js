...
import { StandardFileSystemService, decodeBufferWithEncodingInfo, detectLineEnding, encodeTextFileContent, loadIgnoreRules, isWithinRoot, } from '@qwen-code/qwen-code-core';
...
import { MAX_READ_BYTES, assertTrustedForIntent, detectBinary, enforceReadSize, enforceWriteSize, shouldIgnore, } from './policy.js';
/**
...
class WorkspaceFileSystemImpl {
    ...
    async resolve(input, intent) {
        try {
            return await resolveWithinWorkspace(input, this.deps.boundWorkspace, intent);
        }
        catch (err) {
    ...
    async readText(p, opts = {}) {
        ...
        try {
            ...
            const ignoreVerdict = shouldIgnore(p, this.deps.boundWorkspace, this.deps.ignore, 'file');
    ...
    async list(p, opts = {}) {
        ...
        try {
            ...
            for await (const d of dir) {
                ...
                const verdict = shouldIgnore(childAbs, this.deps.boundWorkspace, this.deps.ignore, kind === 'directory' ? 'directory' : 'file');
    ...
    async glob(pattern, opts = {}) {
        ...
        try {
            ...
            // walk root.
            const cwd = opts.cwd ?? this.deps.boundWorkspace;
            let cwdReal;
            ...
            for (const hit of matches) {
                ...
                const verdict = shouldIgnore(canonical, this.deps.boundWorkspace, this.deps.ignore, kind);
                if (verdict.ignored && !opts.includeIgnored)
                    continue;
                out.push(canonical);
            }
    ...
    async writeTextAtomic(p, content, opts) {
        ...
        try {
            ...
            const out = await this.deps.pathLocks.runExclusive(p, async () => {
                ...
                const verdict = shouldIgnore(p, this.deps.boundWorkspace, this.deps.ignore, 'file');
                if (verdict.ignored)
    ...
    async edit(p, oldText, newText, opts) {
        ...
        try {
            ...
            if (st.size > MAX_READ_BYTES) {
                throw new FsError('file_too_large', `file of ${st.size} bytes exceeds edit cap of ${MAX_READ_BYTES} bytes`, {
            ...
            const editVerdict = shouldIgnore(p, this.deps.boundWorkspace, this.deps.ignore, 'file');
            this.deps.audit.recordAccess(this.deps.ctx, {
                intent: 'edit',
                absolute: p,
                durationMs: performance.now() - start,
     ...
     */
    recordAndWrap(err, intent, input) {
...
function validateWriteTextAtomicOptions(opts) {
    if (opts.mode !== 'create' && opts.mode !== 'replace') {
        throw new FsError('parse_error', 'mode must be either "create" or "replace"');
    }
...
async function readTextSnapshotFromResolvedFile(p, opts = {}) {
    const pre = await fsp.lstat(p);
    ...
    if (sizeOutcome.truncated) {
        ...
        if (buf.length > sizeOutcome.bytesToRead) {
            content = safeUtf8Truncate(buf, sizeOutcome.bytesToRead).toString('utf-8');
...
async function atomicWriteTextResolvedFile(input) {
    ...
    const tmpPath = path.join(parent, `.${path.basename(target)}.${process.pid}.${randomBytes(6).toString('hex')}.tmp`);
    let tempLive = false;
    let tempHandle;
    let tempStat;
    try {
...
async function writeEncodedTextTemp(input) {
    const buf = encodeTextFileContent(input.targetPath, input.content, buildWriteMeta(input.meta));
...
async function assertInodeStableAfterRead(p, preIno) {
    ...
    const preNum = typeof preIno === 'bigint' ? preIno : BigInt(preIno);
    const postNum = typeof post.ino === 'bigint' ? post.ino : BigInt(post.ino);
...
//# sourceMappingURL=workspaceFileSystem.js.map