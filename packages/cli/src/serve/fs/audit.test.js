...
import { FS_ACCESS_EVENT_TYPE, FS_DENIED_EVENT_TYPE, createAuditPublisher, } from './audit.js';
...
describe('createAuditPublisher', () => {
    ...
    it('emits fs.access with hashed path and originatorClientId', () => {
        ...
        const absolute = path.join(workspace, 'src', 'index.ts');
    ...
    it('attaches relPath when includeRawPaths is true', () => {
        ...
        const absolute = path.join(workspace, 'src', 'index.ts');
        publisher.recordAccess({ originatorClientId: 'c', route: 'GET /file' }, { intent: 'read', absolute, durationMs: 1 });
        expect(events[0].data.relPath).toBe(path.join('src', 'index.ts'));
    });
    it('omits truncated/matchedIgnore when not provided', () => {
        const { events, publisher, workspace } = setup();
        const absolute = path.join(workspace, 'a.ts');
        publisher.recordAccess({ route: 'GET /file' }, { intent: 'read', absolute, durationMs: 0 });
    ...
    it('preserves truncated and matchedIgnore when set', () => {
        const { events, publisher, workspace } = setup();
        const absolute = path.join(workspace, 'big.txt');
        publisher.recordAccess({ route: 'GET /file' }, {
            intent: 'read',
            absolute,
            durationMs: 5,
            truncated: true,
    ...
    it('emits fs.denied with errorKind and hashed probe path', () => {
        const { events, publisher, workspace } = setup();
        publisher.recordDenied({ originatorClientId: 'c', route: 'GET /file' }, {
            intent: 'read',
            input: '../escape',
    ...
    it('emits fs.denied even when hint is absent', () => {
        ...
        publisher.recordDenied({ route: 'POST /file/edit' }, {
            intent: 'edit',
            input: '/etc/passwd',
    ...
    it('attaches pattern field for fs.access on glob intent in raw-paths mode', () => {
        ...
        const { events, publisher, workspace } = setup({ includeRawPaths: true });
        publisher.recordAccess({ route: 'GET /glob' }, {
            intent: 'glob',
            absolute: workspace,
            durationMs: 7,
            sizeBytes: 12,
    ...
    it('attaches pattern field for fs.denied on glob intent in raw-paths mode', () => {
        ...
        publisher.recordDenied({ route: 'GET /glob' }, {
            intent: 'glob',
            input: '../../**',
    ...
    it('omits pattern when not provided', () => {
        ...
        publisher.recordAccess({ route: 'GET /file' }, {
            intent: 'read',
            absolute: path.join(workspace, 'a.ts'),
            durationMs: 0,
        });
        expect(events[0].data).not.toHaveProperty('pattern');
    });
    it('respects QWEN_AUDIT_RAW_PATHS=1 via env when includeRawPaths is unset', () => {
        ...
        try {
            ...
            publisher.recordAccess({ route: 'GET /file' }, {
                intent: 'read',
                absolute: path.join(workspace, 'foo'),
                durationMs: 0,
            });
            expect(events[0].data.relPath).toBe('foo');
        }
        finally {
            if (original === undefined)
                delete process.env['QWEN_AUDIT_RAW_PATHS'];
            else
                process.env['QWEN_AUDIT_RAW_PATHS'] = original;
        }
    });
    it('substitutes <cross-drive> sentinel when path.relative cannot produce a relative form', () => {
        ...
        publisher.recordDenied({ route: 'GET /file' }, {
            intent: 'read',
        ...
        if (data.relPath !== undefined) {
            expect(data.relPath === '<cross-drive>' || !path.isAbsolute(data.relPath)).toBe(true);
        }
    });
});
