...
import { canonicalizeWorkspace, hasSuspiciousPathPattern, resolveWithinWorkspace, } from './paths.js';
...
describe('resolveWithinWorkspace', () => {
    ...
    beforeEach(async () => {
        const id = randomBytes(6).toString('hex');
        scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `qwen-fs-resolve-${id}-`));
    ...
    it('resolves an absolute path inside any workspace root', async () => {
        ...
        const out = await resolveWithinWorkspace(target, [workspace, secondWorkspace], 'read');
        expect(out).toBe(realpathSync.native(target));
    });
    it('resolves a missing absolute write target inside a secondary root', async () => {
        ...
        const out = await resolveWithinWorkspace(target, [workspace, secondWorkspace], 'write');
        expect(out).toBe(path.join(realpathSync.native(secondWorkspace), 'new', 'file.txt'));
    });
    it('resolves relative writes against the first workspace root', async () => {
        const secondWorkspace = path.join(scratch, 'second-workspace');
        await fsp.mkdir(secondWorkspace);
        await fsp.mkdir(path.join(secondWorkspace, 'src'));
        const out = await resolveWithinWorkspace('src/new-file.txt', [workspace, secondWorkspace], 'write');
        expect(out).toBe(path.join(realpathSync.native(workspace), 'src', 'new-file.txt'));
    });
    it('rejects a `..` traversal that lands outside the workspace', async () => {
        await expect(resolveWithinWorkspace('../escape', workspace, 'read')).rejects.toMatchObject({ kind: 'path_outside_workspace' });
    });
    it('rejects an absolute path outside the workspace', async () => {
        const outside = path.join(scratch, 'outside.txt');
        await fsp.writeFile(outside, 'x');
        await expect(resolveWithinWorkspace(outside, workspace, 'read')).rejects.toMatchObject({ kind: 'path_outside_workspace' });
    });
    it('rejects a symlink whose target escapes the workspace', async () => {
        ...
        await expect(resolveWithinWorkspace('leak', workspace, 'read')).rejects.toMatchObject({ kind: 'symlink_escape' });
    });
    ...
    it('allows a symlink target inside another bound workspace', async () => {
        ...
        await fsp.symlink(real, link, 'file');
        const out = await resolveWithinWorkspace('link-to-second.txt', [workspace, second], 'read');
        expect(out).toBe(realpathSync.native(real));
    });
    it('tolerates ENOENT for write intent and resolves via existing ancestor', async () => {
        ...
        expect(out).toBe(path.join(realpathSync.native(workspace), 'newdir', 'leaf.txt'));
    });
    it('tolerates ENOENT for stat intent and resolves via existing ancestor', async () => {
        ...
        const out = await resolveWithinWorkspace('newdir/leaf.txt', workspace, 'stat');
        expect(out).toBe(path.join(realpathSync.native(workspace), 'newdir', 'leaf.txt'));
    });
    it('rejects ENOENT under read intent with path_not_found', async () => {
        const err = await resolveWithinWorkspace('does-not-exist', workspace, 'read').catch((e) => e);
    ...
    it('rejects ENOENT under read intent through an absolute workspace alias', async () => {
        ...
        const err = await resolveWithinWorkspace(path.join(aliasWorkspace, 'does-not-exist'), workspace, 'read').catch((e) => e);
        expect(isFsError(err)).toBe(true);
        expect(err.kind).toBe('path_not_found');
    });
    it('rejects suspicious patterns before any I/O', async () => {
        ...
        await expect(resolveWithinWorkspace('//server/share', workspace, 'read')).rejects.toMatchObject({ kind: 'path_outside_workspace' });
        if (process.platform === 'win32') {
            await expect(resolveWithinWorkspace('GIT~1', workspace, 'read')).rejects.toMatchObject({ kind: 'path_outside_workspace' });
        }
    });
    it('rejects empty/non-string input with parse_error', async () => {
        await expect(resolveWithinWorkspace('', workspace, 'read')).rejects.toMatchObject({ kind: 'parse_error' });
    });
    it('rejects a dangling symlink whose target escapes the workspace (write intent)', async () => {
        ...
        const err = await resolveWithinWorkspace('escape', workspace, 'write').catch((e) => e);
        expect(isFsError(err)).toBe(true);
    ...
    it('rejects a dangling symlink escape reached through an absolute workspace alias', async () => {
        ...
        const err = await resolveWithinWorkspace(aliasEscape, workspace, 'write').catch((e) => e);
        expect(isFsError(err)).toBe(true);
        expect(err.kind).toBe('symlink_escape');
    });
    it('allows a dangling symlink whose (not-yet-existing) target stays inside workspace', async () => {
        ...
        const out = await resolveWithinWorkspace('pending-link', workspace, 'write');
    ...
    it('allows a dangling symlink target inside another bound workspace', async () => {
        ...
        const out = await resolveWithinWorkspace('pending-link-to-second', [workspace, second], 'write');
    ...
    it('rejects a multi-hop dangling symlink chain that escapes the workspace', async () => {
        ...
        const err = await resolveWithinWorkspace('leak', workspace, 'write').catch((e) => e);
        expect(isFsError(err)).toBe(true);
        expect(err.kind).toBe('symlink_escape');
    });
    it('detects a symlink cycle and rejects with symlink_escape', async () => {
        ...
        const err = await resolveWithinWorkspace('a', workspace, 'write').catch((e) => e);
        expect(isFsError(err)).toBe(true);
        expect(err.kind).toBe('symlink_escape');
    });
    it('returns a value usable as ResolvedPath brand', async () => {
        ...
        const out = await resolveWithinWorkspace('b.txt', workspace, 'read');
    ...
    it('canonicalizes the boundWorkspace once so symlinked workspaces resolve correctly', async () => {
        ...
        const out = await resolveWithinWorkspace('inside.txt', aliasWorkspace, 'read');
        expect(out).toBe(realpathSync.native(target));
    });
});
