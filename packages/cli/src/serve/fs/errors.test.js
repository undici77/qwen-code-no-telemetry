...
describe('wrapAsFsError', () => {
    function errno(code, message = `${code} message`) {
    ...
    it('falls back to internal_error (not permission_denied) for unknown errnos and non-errno errors', () => {
        ...
        expect(wrapAsFsError(new TypeError('null deref')).kind).toBe('internal_error');
        // Caller can still override.
        expect(wrapAsFsError(errno('EWHATEVER'), 'parse_error').kind).toBe('parse_error');
    });
