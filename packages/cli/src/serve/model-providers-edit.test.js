...
describe('removeModelFromProviders', () => {
    ...
    it('resolves a custom provider id to its protocol', () => {
        ...
        const result = removeModelFromProviders(providers, { idealab: 'openai' }, { authType: 'openai', modelId: 'idea-model' });
        expect(result.removed).toBe(true);
...
describe('isActiveModelSelection', () => {
    it('matches by id when no active baseUrl is pinned', () => {
        expect(isActiveModelSelection('gpt-4o', undefined, {
            authType: 'openai',
            modelId: 'gpt-4o',
        })).toBe(true);
    });
    it('requires baseUrl agreement when the active selection is pinned', () => {
        expect(isActiveModelSelection('gpt-4o', 'https://a.example', {
            authType: 'openai',
            modelId: 'gpt-4o',
            baseUrl: 'https://b.example',
        })).toBe(false);
    ...
    it('does not match a different active model', () => {
        expect(isActiveModelSelection('other', undefined, {
            authType: 'openai',
            modelId: 'gpt-4o',
        })).toBe(false);
    });
});
//# sourceMappingURL=model-providers-edit.test.js.map