...
import { ClientMcpSenderRegistry, createClientMcpServerProvider, } from './client-mcp-sender-registry.js';
const msg = (id) => ({
    jsonrpc: '2.0',
    id,
    method: 'ping',
...
describe('ClientMcpSenderRegistry', () => {
    ...
    it('delete is idempotent and a no-op for an unknown name', () => {
        const reg = new ClientMcpSenderRegistry();
        reg.set('srv', vi.fn(async () => msg(1)), 'connA');
    ...
    it('keeps session deletion scoped to the current worker owner', async () => {
        ...
        expect(reg.deleteSession('channel_loop', 'session-a', 'worker-a')).toBe(false);
        await reg.lookup('channel_loop')(msg(7), { sessionId: 'session-a' });
        expect(senderB).toHaveBeenCalled();
        expect(reg.deleteSession('channel_loop', 'session-a', 'worker-b')).toBe(true);
    });
    it('never falls back to a global sender for a reserved session server', async () => {
        ...
        reg.setSession('channel_loop', 'session-a', vi.fn(async () => msg(2)), 'worker');
        await expect(reg.lookup('channel_loop')(msg(7), { sessionId: 'session-b' })).rejects.toThrow(/No session-scoped MCP sender/);
        await expect(reg.lookup('channel_loop')(msg(8))).rejects.toThrow(/requires a session context/);
        expect(globalSender).not.toHaveBeenCalled();
    });
});
describe('createClientMcpServerProvider', () => {
    function setupBridge(addResult) {
        return {
    ...
    it('rolls back the sender when bridge add is skipped', async () => {
        ...
        await expect(provider.registerClientMcpServer('srv', vi.fn(async () => msg(1)))).rejects.toThrow(/runtime MCP add skipped: disabled/);
    ...
    it('removes the runtime server and rolls back the sender when settings are shadowed', async () => {
        ...
        const provider = createClientMcpServerProvider(registry, bridge, 'connA');
        await expect(provider.registerClientMcpServer('srv', vi.fn(async () => msg(1)))).rejects.toThrow(/conflicts with a configured MCP server/);
    ...
    it('rolls back the sender when bridge add throws', async () => {
        ...
        const provider = createClientMcpServerProvider(registry, bridge, 'connA');
        await expect(provider.registerClientMcpServer('srv', vi.fn(async () => msg(1)))).rejects.toThrow('boom');
        expect(registry.serverNames()).toEqual([]);
        expect(bridge.removeRuntimeMcpServer).not.toHaveBeenCalled();
    });
    it('does not unregister a server now owned by another connection', async () => {
        ...
        registry.set('srv', vi.fn(async () => msg(2)), 'connB');
        const provider = createClientMcpServerProvider(registry, bridge, 'connA');
...
//# sourceMappingURL=client-mcp-sender-registry.test.js.map