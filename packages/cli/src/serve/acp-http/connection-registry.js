...
export class AcpConnection {
    ...
    idCounter = 0;
    constructor(connectionId, fromLoopback, onAbandonPending, onDetachSession) {
    ...
    attachConnStream(stream) {
        ...
        if (this.connStream && this.connStream !== stream)
            this.connStream.close();
        this.connStream = stream;
        for (const frame of this.connBuffer.splice(0))
            void stream.send(frame);
    }
    /**
    ...
    sendSession(sessionId, frame, id) {
        ...
        if (binding.stream && !binding.stream.isClosed) {
            ...
            pushCapped(binding.buffer, { frame, id }, `session ${sessionId}`, (e) => e.id);
        }
    }
    /**
    ...
    sendSessionReply(sessionId, frame, anchorId) {
        ...
        pushCapped(binding.buffer, { frame, id: undefined, anchorId }, `session ${sessionId}`, (e) => e.id);
    }
    /**
    ...
    destroy() {
        ...
        for (const binding of this.sessions.values()) {
            try {
                ...
                writeStderrLine(`qwen serve: /acp teardownBinding(${logSafe(binding.sessionId)}) failed during destroy: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        this.sessions.clear();
    ...
    abandonPendingForSession(sessionId, clientId) {
        for (const [id, req] of this.pending) {
            ...
            if (!cancelled) {
                writeStderrLine(`qwen serve: /acp MEDIATOR STUCK: abandonPendingForSession(${logSafe(sessionId)}) cancel failed for ${logSafe(req.bridgeRequestId)}`);
            }
        }
    }
}
function pushCapped(buf, frame, label = 'stream', getId) {
    if (buf.length >= MAX_BUFFERED_FRAMES) {
        ...
        if (replayable === -1) {
            ...
            if (buf.length >= HARD_BUFFERED_FRAMES_CAP) {
                buf.shift();
                writeStderrLine(`qwen serve: /acp HARD buffer cap breached (${label}) — dropping ` +
                    `oldest id-less reply (its caller may hang); buffer was ${buf.length + 1}`);
            }
                ...
                writeStderrLine(`qwen serve: /acp pre-attach buffer over soft cap (${label}) — ` +
            ...
            writeStderrLine(`qwen serve: /acp pre-attach buffer full (${label}), dropped frame` +
...
export class ConnectionRegistry {
    ...
    byId = new Map();
    sweepTimer;
    constructor(onAbandonPending, onDetachSession, maxConnections = DEFAULT_MAX_CONNECTIONS, idleTtlMs = 30 * 60_000) {
        this.onAbandonPending = onAbandonPending;
        this.onDetachSession = onDetachSession;
    ...
    create(fromLoopback) {
        ...
        const conn = new AcpConnection(undefined, fromLoopback, this.onAbandonPending, this.onDetachSession);
    ...
    sweep() {
        ...
        for (const [id, conn] of this.byId) {
            ...
            writeStderrLine(`qwen serve: /acp reaping idle connection ${id} ` +
