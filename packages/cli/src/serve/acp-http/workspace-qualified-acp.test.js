...
describe('workspace-qualified ACP (/workspaces/:workspace/acp)', () => {
    ...
    it('reserves Live creation and relocates compatible ACP restores before exposing them', async () => {
        ...
        expect(materializeLiveConversationDirectory).toHaveBeenCalledWith('generic-session');
        ...
        const foreignWorkspace = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-live-foreign-workspace-'));
        try {
            await writeStoredSession('foreign-live-session', foreignWorkspace, {
                sourceType: 'default',
                sourceId: 'realtime_voice:p1:h1:a1:foreign-call',
            });
            const rejectedForeignLoad = await sendWsRequest('/workspaces/live-id/acp', {
                jsonrpc: '2.0',
                id: 47,
                method: 'session/load',
                params: {
        ...
        expect(materializeLiveConversationDirectory).toHaveBeenCalledWith('live-session');
        expect(loadSession).toHaveBeenCalledWith(expect.objectContaining({
        ...
        expect(materializeLiveConversationDirectory.mock.invocationCallOrder[0]).toBeLessThan(loadSession.mock.invocationCallOrder[0]);
        expect(loadSession.mock.invocationCallOrder[0]).toBeLessThan(changeSessionCwd.mock.invocationCallOrder[0]);
        activeLiveSessionIds.add('live-session');
        const blockedClose = await new Promise((resolve, reject) => {
            const ws = new WebSocket(`ws://127.0.0.1:${port}/workspaces/live-id/acp`, { handshakeTimeout: 2000 });
            ws.on('open', () => ws.send(INITIALIZE));
            ws.on('message', (data) => {
                const message = JSON.parse(data.toString());
                if (message['id'] === 1) {
        ...
        expect(changeSessionCwd).not.toHaveBeenCalledWith('live-active', expect.anything());
        ...
        expect(liveBridge.detachClient).toHaveBeenCalledWith('live-active-root', 'live-client');
        expect(liveBridge.killSession).not.toHaveBeenCalledWith('live-active-root', expect.anything());
    });
