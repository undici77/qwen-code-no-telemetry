...
import { RUNTIME_MCP_IF_ABSENT_CONFIG_FLAG, Storage, } from '@qwen-code/qwen-code-core';
...
export function mountAcpHttp(app, bridge, opts) {
    ...
    const registry = new ConnectionRegistry((req, clientId) => {
        ...
        if (!dispatcherRef.current) {
            writeStderrLine('qwen serve: /acp abandonPending called before dispatcher initialized (skipped)');
            return false;
        }
    ...
    (sessionId, clientId) => {
        void bridge.detachClient(sessionId, clientId).catch((err) => {
            writeStderrLine(`qwen serve: /acp detachClient(${sessionId}) failed: ${err instanceof Error ? err.message : String(err)}`);
        });
    }, opts.maxConnections);
    ...
    async function removeChromeDevToolsMcp(originatorClientId) {
        ...
        try {
            ...
            writeStderrLine(`qwen serve: failed to remove ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    function removeChromeDevToolsMcpIfUnused(originatorClientId) {
        void (async () => {
            ...
            writeStderrLine(`qwen serve: failed to clean up ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP: ${err instanceof Error ? err.message : String(err)}`);
        });
    }
    function ensureChromeDevToolsMcpRegistered(localPort, originatorClientId) {
        if (opts.token) {
            if (!cdpMcpTerminalSkipLogged) {
                writeStderrLine(`qwen serve: ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP skipped because /cdp requires bearer auth`);
        ...
        cdpMcpRegistering = (async () => {
            try {
                ...
                if (result.skipped) {
                    writeStderrLine(`qwen serve: ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP skipped: ${result.reason ?? 'unknown'}`);
                    return;
                }
                if (result.shadowedSettings) {
                    await bridge
                        ...
                        .catch(() => { });
                    cdpMcpTerminalSkipLogged = true;
                    writeStderrLine(`qwen serve: ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP skipped because settings already define it`);
                    return;
                }
                ...
                writeStderrLine(`qwen serve: failed to add ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP: ${err instanceof Error ? err.message : String(err)}`);
            }
            finally {
    ...
    const handleAcpDelete = (mount, req, res) => {
        ...
        if (existed) {
            writeStderrLine(`qwen serve: ${mount.routeLabel} connection deleted ${connectionId.slice(0, 8)} (remaining=${mount.registry.size})`);
        }
        res.status(202).end();
    };
    // ── POST /acp ──────────────────────────────────────────────────────
    const handleAcpPost = async (mount, req, res) => {
        ...
        if (!parsed.ok) {
            writeStderrLine(`qwen serve: ${mount.routeLabel} malformed request from ${req.socket?.remoteAddress}: ${parsed.error.error.message}`);
        ...
        if (isRequest(message) && message.method === 'initialize') {
            ...
            if (!conn) {
                ...
                writeStderrLine(`qwen serve: ${mount.routeLabel} connection cap reached (max=${mount.registry.connectionCap}), rejecting initialize`);
            ...
            writeStderrLine(`qwen serve: ${mount.routeLabel} connection established ${conn.connectionId.slice(0, 8)} ` +
        ...
        await mount.dispatcher
            ...
            .catch((err) => {
            writeStderrLine(`qwen serve: ${mount.routeLabel} handle error: ${err instanceof Error ? err.message : String(err)}`);
        });
    };
    ...
    const handleAcpGet = (mount, req, res) => {
        ...
        if (!sessionId) {
            ...
            const stream = new SseStream(res, () => {
                writeStderrLine(`qwen serve: ${mount.routeLabel} connection stream closed (${connId.slice(0, 8)})`);
                ...
                const reapConnIfDead = () => {
                    if (mount.registry.get(connId) === conn &&
                        ...
                        writeStderrLine(`qwen serve: ${mount.routeLabel} reaping connection ${connId.slice(0, 8)} (conn stream gone, no live session stream)`);
        ...
        const eventEpoch = parseEventEpochHeader(headerOf(req, 'x-qwen-event-epoch'), '/acp ');
        ...
        const onPumpSettled = () => {
            if (stream.isClosed) {
                ...
                writeStderrLine(`qwen serve: ${mount.routeLabel} session stream pump ended while open ` +
                ...
                writeStderrLine(`qwen serve: ${mount.routeLabel} session stream pump settled for a superseded ` +
        ...
        void mount.dispatcher
            ...
            writeStderrLine(`qwen serve: ${mount.routeLabel} event pump error (${logSafe(sessionId)}, lastEventId=${lastEventId ?? 'none'}): ${logSafe(err instanceof Error ? err.message : String(err))}`);
            onPumpSettled();
        });
    };
    ...
    const createSecondaryAcpMount = (rt) => {
        ...
        const secondaryRegistry = new ConnectionRegistry((req, clientId) => {
            ...
            void rt.bridge
                ...
                .catch((err) => {
                writeStderrLine(`qwen serve: /workspaces/${rt.workspaceId}/acp detachClient(${sessionId}) failed: ${err instanceof Error ? err.message : String(err)}`);
            });
        }, opts.maxConnections);
    ...
    function setupWebSocket(httpServer) {
        ...
        upgradeListener = (req, socket, head) => {
            ...
            const logReject = (reason) => {
                writeStderrLine(`qwen serve: WebSocket upgrade rejected (${reason}) from ${rawAddr}`);
            };
            ...
            wss.handleUpgrade(req, socket, head, (ws) => {
                ...
                const initTimer = setTimeout(() => {
                    if (!initialized) {
                        writeStderrLine(`qwen serve: ${activeMount.routeLabel} WS initialize timeout (30s) from ${rawAddr}`);
                ...
                const wsKey = rawAddr.startsWith('::ffff:')
                    ...
                    : rawAddr;
                ws.on('error', (err) => {
                    writeStderrLine(`qwen serve: ${activeMount.routeLabel} WS error: ${err instanceof Error ? err.message : String(err)}`);
                });
                ...
                ws.on('message', (rawData) => {
                    messageQueue = messageQueue
                        ...
                        .catch((err) => {
                        writeStderrLine(`qwen serve: ${activeMount.routeLabel} WS message handler error: ${err instanceof Error ? err.message : String(err)}`);
                    });
                });
                async function handleWsMessage(rawData) {
                    ...
                    if (opts.clientMcpOverWs === true &&
                        ...
                        if (dispatchOffQueue) {
                            ...
                            if (clientMcpInflightDispatch >= MAX_INFLIGHT_MCP_DISPATCH) {
                                writeStderrLine(`qwen serve: ${activeMount.routeLabel} client-MCP inflight cap hit (${MAX_INFLIGHT_MCP_DISPATCH}); rejecting ${String(frameType)} frame`);
                                safeWsSend(ws, JSON.stringify({
                                    type: 'mcp_error',
                        ...
                        const handleP = clientMcp
                            ...
                            writeStderrLine(`qwen serve: ${activeMount.routeLabel} client-mcp frame error: ${message}`);
                    ...
                    if (!initialized) {
                        ...
                        const stream = new WsStream(ws, () => {
                            writeStderrLine(`qwen serve: ${activeMount.routeLabel} WS closed (${conn.connectionId.slice(0, 8)})`);
                        ...
                        clearTimeout(initTimer);
                        connRef = conn;
                        writeStderrLine(`qwen serve: ${activeMount.routeLabel} WS established ${conn.connectionId.slice(0, 8)} (loopback=${fromLoopback}, active=${activeMount.registry.size})`);
                        ...
                        if (activeMount.primary &&
                            ...
                            writeStderrLine(`qwen serve: ${activeMount.routeLabel} connection ${conn.connectionId.slice(0, 8)} registered as CDP bridge`);
                        }
                        return;
                    }
                    // Subsequent messages
                        ...
                        if (typeof sid === 'string' && conn.ownsSession(sid)) {
                            ...
                            if (binding &&
                                ...
                                void activeMount.dispatcher
                                    ...
                                    writeStderrLine(`qwen serve: ${activeMount.routeLabel} WS pump error (${sid}): ${err instanceof Error ? err.message : String(err)}`);
                                    cleanupSession();
                                });
                            }
                        }
                    ...
                    const dispatchP = activeMount.dispatcher
                        ...
                        .catch((err) => {
                        writeStderrLine(`qwen serve: ${activeMount.routeLabel} WS handle error: ${err instanceof Error ? err.message : String(err)}`);
                    });
                    if (!isPrompt)
                        await dispatchP;
                }
