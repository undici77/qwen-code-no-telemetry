 ...
 */
import { APPROVAL_MODES, BTW_MAX_INPUT_LENGTH, createDebugLogger, GROUP_COLOR_OPTIONS, Storage, SessionService, SessionOrganizationError, SESSION_WRITER_RPC_CODES, BuiltinAgentRegistry, SubagentError, WorkspaceMemoryFileTooLargeError, WorkspaceMemoryWriteTimeoutError, writeWorkspaceContextFile, IMAGE_CAPABILITY, } from '@qwen-code/qwen-code-core';
...
import { CancelSentinelCollisionError, InvalidPermissionOptionError, PermissionForbiddenError, PermissionPolicyNotImplementedError, SessionArchivingError, } from '../acp-session-bridge.js';
...
import { isReservedLiveSessionSource, readLoadableLiveConversationMetadata, } from '../live/session-source.js';
import { translateAndCheckAbsoluteWorkspacePath, canonicalizeWorkspace, } from '@qwen-code/acp-bridge/workspacePaths';
import { SessionNotFoundError, SessionShellClientRequiredError, SessionShellDisabledError, WorkspaceMismatchError, } from '@qwen-code/acp-bridge/bridgeErrors';
import { SessionArtifactAuthorizationError, SessionArtifactValidationError, } from '@qwen-code/acp-bridge/sessionArtifacts';
...
function parsePermissionResponse(params) {
    ...
    if (answers !== undefined) {
        if (isObject(answers) &&
            ...
            writeStderrLine('qwen serve: /acp session/permission dropping invalid `answers` (expected an object map of string values)');
        }
    }
...
export class AcpDispatcher {
    ...
    removeOrphanSession(sessionId, removePersistedSession = false, runtime = this.getSessionRuntimeContext()) {
        ...
        return cleanup.catch((err) => {
            writeStderrLine(`qwen serve: /acp orphan killSession(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`);
            return undefined;
        });
    }
    killOrphanSession(sessionId, removePersistedSession = false, runtime) {
    ...
    async configOptionsFor(sessionId, bridge = this.bridge) {
        try {
            ...
            writeStderrLine(`qwen serve: /acp configOptionsFor(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`);
            return undefined;
        }
    }
    /**
    ...
    cancelAbandonedPermission(req, clientId) {
        try {
            ...
            if (/not found|unknown session/i.test(msg))
                return true;
            writeStderrLine(`qwen serve: /acp cancelAbandonedPermission(${logSafe(req.sessionId)}) failed: ${logSafe(msg)}`);
            return false;
        }
    }
    /**
    ...
    requireOwned(conn, sessionId, id) {
        ...
        if (id === undefined) {
            ...
            writeStderrLine(`qwen serve: /acp notification for unowned session ${logSafe(sessionId)} (dropped)`);
            return false;
        }
            ...
            switch (method) {
                    ...
                    try {
                        ...
                        const rollbackSession = async () => {
                            if (session.attached) {
                                ...
                                await this.removeOrphanSession(session.sessionId, true, sessionRuntime);
                            }
                        };
                        try {
                            assertGenerationOpen?.();
                ...
                case 'session/resume': {
                    ...
                    try {
                        const restored = await this.archiveCoordinator.runSharedMany([sessionId], async () => {
                            assertGenerationOpen?.();
                            await assertSessionLoadable(cwd, sessionId, sessionRuntime.sessionRuntimeBaseDir);
                            ...
                            const metadata = this.liveSessionIsolation
                                ? await readLoadableLiveConversationMetadata(sessionId, (candidateId) => sessionService.readCreationMetadata(candidateId))
                        ...
                        if (conn.destroyed || closeRaced) {
                            void rollbackRestore().catch((err) => writeStderrLine(`qwen serve: /acp orphan ${restored.attached ? 'detach' : 'kill'}(${logSafe(sessionId)}) teardown-race: ${logSafe(errMsg(err))}`));
                ...
                case 'session/close': {
                    ...
                    const closeLocalSessionStream = () => {
                        try {
                            ...
                            writeStderrLine(`qwen serve: /acp session/close local teardown failed (${logSafe(sessionId)}): ${logSafe(errMsg(teardownErr))}`);
                        }
                    };
                ...
                case 'session/fork': {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        const result = await this.bridge.branchSession(sessionId, {
                ...
                case 'session/permission': {
                    ...
                    if (!requestId) {
                        ...
                        writeStderrLine(`qwen serve: /acp session/permission rejected: \`requestId\` is required`);
                        if (id !== undefined) {
                    ...
                    try {
                        ...
                        if (err instanceof AcpParamError) {
                            writeStderrLine(`qwen serve: /acp session/permission invalid params (requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                    ...
                    if (sessionIdParam &&
                        ...
                        writeStderrLine(`qwen serve: /acp session/permission vote rejected: requestId ${logSafe(requestId)} does not belong to session ${logSafe(sessionIdParam)}`);
                        if (id !== undefined) {
                    ...
                    if (this.registry && !pendingRef) {
                        writeStderrLine(`qwen serve: /acp session/permission vote dropped: no pending request for requestId ${logSafe(requestId)}`);
                        if (id !== undefined) {
                    ...
                    // this connection).
                    if (!conn.ownsSession(sessionId)) {
                        writeStderrLine(`qwen serve: /acp session/permission vote rejected: session ${logSafe(sessionId)} not owned by this connection (requestId ${logSafe(requestId)})`);
                        if (id !== undefined) {
                    ...
                    try {
                        ...
                        if (err instanceof InvalidPermissionOptionError) {
                            writeStderrLine(`qwen serve: /acp session/permission invalid option (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, err.message, {
                                    httpStatus: 400,
                        ...
                        if (err instanceof PermissionForbiddenError) {
                            writeStderrLine(`qwen serve: /acp session/permission forbidden (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, err.message, {
                                    httpStatus: 403,
                        ...
                        if (err instanceof PermissionPolicyNotImplementedError) {
                            ...
                            writeStderrLine(`qwen serve: /acp session/permission policy not implemented (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                        ...
                        if (err instanceof CancelSentinelCollisionError) {
                            ...
                            writeStderrLine(`qwen serve: /acp session/permission cancel-sentinel collision (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INTERNAL_ERROR, err.message, {
                                    httpStatus: 500,
                        ...
                        // error for the wire.
                        writeStderrLine(`qwen serve: /acp session/permission vote failed (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(errMsg(err))}`);
                    ...
                    if (!accepted) {
                        ...
                        writeStderrLine(`qwen serve: /acp session/permission vote not accepted by bridge (${logSafe(sessionId)}, requestId ${logSafe(requestId)})`);
                        if (id !== undefined) {
                    ...
                    writeStderrLine(`qwen serve: /acp session/permission vote accepted (${logSafe(sessionId)}, requestId ${logSafe(requestId)}, connection ${logSafe(conn.connectionId.slice(0, 8))})`);
                    this.replyConn(conn, id, {});
                    return;
                }
                ...
                case `${QWEN_METHOD_NS}session/shell`: {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        writeStderrLine(`qwen serve: /acp session/shell session=${logSessionId} client=${logClientId} cmd=${logCommand}`);
                ...
                case `${QWEN_METHOD_NS}workspace/memory/write`: {
                    ...
                    writeStderrLine(`qwen serve: /acp workspace/memory/write scope=${scope} mode=${mode} client=${conn.clientId?.slice(0, 8)} bytes=${Buffer.byteLength(content, 'utf8')}`);
                ...
                case `${QWEN_METHOD_NS}sessions/delete`: {
                    ...
                    const result = await this.runWithSessionListInvalidation(['active', 'archived'], () => deleteDaemonSessions({
                        ...
                        onError: ({ phase, sessionId, error }) => {
                            ...
                            writeStderrLine(`qwen serve: /acp sessions/delete ${phase}Session(${safeSessionId}) failed: ${safeMessage}`);
                        },
                    }));
                    this.replyConn(conn, id, result);
                    return;
            ...
            if (conn.hasInitialReplayPending(sessionId)) {
                ...
                if (snapshot) {
                    if (snapshot.degraded) {
                        ...
                        writeStderrLine(`qwen serve: /acp initial replay used a DEGRADED snapshot (compaction failure) session=${logSafe(sessionId)}; replay may be incomplete`);
                    }
                    ...
                    writeStderrLine(`qwen serve: /acp initial replay skipped (no snapshot) session=${logSafe(sessionId)}`);
            ...
            for await (const event of iterable) {
                ...
                if (event.type === 'replay_complete') {
                    ...
                    const replayedCount = event.data
                        ?.replayedCount;
                    writeStderrLine(`qwen serve: /acp replay complete (${logSafe(sessionId)}) ` +
            ...
            case 'permission_request': {
                ...
                if (!binding?.stream || binding.stream.isClosed) {
                    ...
                    if (binding?.graceTimer) {
                        writeStderrLine(`qwen serve: /acp permission cancel during reconnect grace ` +
                    ...
                    if (!cancelled) {
                        writeStderrLine(`qwen serve: /acp permission cancel FAILED for ${logSafe(sessionId)} (mediator stuck; no automatic recovery)`);
                    }
                    return;
                }
    ...
    resolveClientResponse(conn, msg, fromLoopback) {
        ...
        if (pendingConn !== conn && !conn.ownsSession(pending.sessionId)) {
            ...
            writeStderrLine(`qwen serve: /acp permission vote dropped: responding connection ${logSafe(conn.connectionId.slice(0, 8))} does not own session ${logSafe(pending.sessionId)} (requestId ${logSafe(pending.bridgeRequestId)})`);
            return;
        }
        ...
        try {
            ...
            writeStderrLine(`qwen serve: /acp permission vote failed (${logSafe(pending.sessionId)}): ${logSafe(errMsg(err))}`);
    ...
    async handlePrompt(conn, sessionId, id, params, fromLoopback) {
        ...
        try {
            ...
            if (id !== undefined) {
                ...
                writeStderrLine(`qwen serve: /acp prompt error (${logSafe(sessionId)}, notification): ${logSafe(errMsg(err))}`);
            }
        }
        finally {
    ...
    replySession(conn, sessionId, id, result, errorFrame) {
        ...
        if (conn.sessions.has(sessionId)) {
            ...
            try {
                ...
                anchorId = undefined;
                writeStderrLine(`qwen serve: /acp replySession(${logSafe(sessionId)}) ` +
            ...
            // session teardown.
            writeStderrLine(`qwen serve: /acp replySession(${logSafe(sessionId)}) binding gone mid-flight, ` +
