...
import { SESSION_TRANSCRIPT_MAX_INDEX_BYTES, SessionService, Storage, createDebugLogger, resetDebugLoggingState, setDebugLogSession, } from '@qwen-code/qwen-code-core';
...
function makeBridge(workspaceCwd, summaries = [], options = {}) {
    ...
    const bridge = {
        ...
        async launchSessionForkAgent(sessionId) {
...
describe('multi-workspace session dispatch', () => {
    ...
    it('restores cold Live load and resume sessions into their server-derived conversation directories', async () => {
        await withStoredLiveCoordinators(['live-cold-load', 'live-cold-resume'], async () => {
            const operationLog = [];
    ...
    it('loads a projectless task created from Live in the Conversations runtime', async () => {
        await withStoredProjectlessLiveTasks(['live-projectless-task'], async () => {
            ...
            expect(materializeConversationDirectory).toHaveBeenCalledWith('live-projectless-task');
        });
    });
    it('opens attached active Live workers without waiting for cwd relocation', async () => {
        await withStoredLiveWorkers(['live-active-load', 'live-active-resume'], async () => {
            const materializeConversationDirectory = vi.fn(async (sessionId) => path.join(SECONDARY_CWD, `conversation-${sessionId}`));
            for (const action of ['load', 'resume']) {
    ...
    it('exports the selected trusted workspace without falling back or starting ACP', async () => {
        await withRuntimeDir(async () => {
            ...
            expect(secondary.headers['content-disposition']).toMatch(/^attachment; filename="qwen-code-export-.+\.html"$/);
