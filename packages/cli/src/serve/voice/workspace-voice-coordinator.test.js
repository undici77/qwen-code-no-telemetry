...
import { MAX_CONCURRENT_VOICE_SESSIONS, VoiceLeaseAbortError, WorkspaceVoiceCoordinator, } from './workspace-voice-coordinator.js';
function runtime(id) {
...
describe('WorkspaceVoiceCoordinator', () => {
    it('shares capacity across runtimes and releases it exactly once', () => {
        ...
        const leases = Array.from({ length: MAX_CONCURRENT_VOICE_SESSIONS }, () => coordinator.acquire(first));
    ...
    it('keeps a re-added runtime generation independent from old leases', async () => {
        ...
        const disposal = coordinator.disposeRuntime(oldRuntime, 'workspace_removed');
        ...
        if (newAdmission.kind === 'admitted')
            newAdmission.lease.release();
    });
...
//# sourceMappingURL=workspace-voice-coordinator.test.js.map