...
import { daemonTelemetryMiddleware, legacySessionTelemetryRoutes, resolveDaemonTelemetryRoute, setDaemonTelemetryWorkspace, } from './telemetry.js';
...
function mockReq(method, path) {
    return { method, path, get: () => undefined };
}
...
describe('daemonTelemetryMiddleware — recordRequest seam', () => {
    ...
    it('defers singular owner-routed workspace attribution until the handler selects a runtime', () => {
        ...
        for (const [method, path, route] of [
            ...
            expect(coreMocks.spanSetAttribute).toHaveBeenLastCalledWith('qwen-code.workspace.hash', 'hash:/workspace/secondary');
        }
    });
    ...
    it('resolves workspace hash per request instead of closing over the primary workspace', () => {
        ...
        expect(coreMocks.spanSetAttribute).toHaveBeenNthCalledWith(1, 'qwen-code.workspace.hash', 'hash:/workspace/one');
        expect(coreMocks.spanSetAttribute).toHaveBeenNthCalledWith(2, 'qwen-code.workspace.hash', 'hash:/workspace/two');
    });
    ...
    it('settles a published workspace after its runtime is removed', () => {
        ...
        expect(coreMocks.spanSetAttribute).toHaveBeenCalledWith('qwen-code.workspace.hash', 'hash:/workspace/secondary');
    });
    it('uses first-selection-wins and clears deferred context after settlement', () => {
        ...
        expect(coreMocks.spanSetAttribute).toHaveBeenCalledWith('qwen-code.workspace.hash', 'hash:/workspace/first');
    });
...
describe('legacy session telemetry route catalog', () => {
    it('contains 54 unique routes with the audited 47/7 attribution split', () => {
        ...
        expect(legacySessionTelemetryRoutes.filter(({ attribution }) => attribution === 'handler_resolved')).toHaveLength(47);
        expect(legacySessionTelemetryRoutes.filter(({ attribution }) => attribution === 'pre_resolved')).toHaveLength(7);
