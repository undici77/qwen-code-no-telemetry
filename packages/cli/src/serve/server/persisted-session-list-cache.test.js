...
describe('PersistedSessionListCache', () => {
    it('single-flights concurrent loads for the same scope', async () => {
        ...
        const lookups = Array.from({ length: 5 }, () => cache.lookup(SCOPE, loader));
        ...
        await expect(Promise.all(lookups.map((lookup) => lookup.promise))).resolves.toHaveLength(5);
    ...
    it('rejects an already-aborted caller without creating a slot or load', async () => {
        ...
        controller.abort(reason);
        const loader = vi.fn(async () => snapshot());
        expect(() => cache.lookup(SCOPE, loader, { signal: controller.signal })).toThrow(reason);
        expect(loader).not.toHaveBeenCalled();
        const lookup = cache.lookup(SCOPE, loader);
    ...
    it('isolates archive state, runtime root, and workspace', async () => {
        ...
        await Promise.all(scopes.map((scope) => cache.lookup(scope, loader).promise));
    ...
    it('reclaims retained-summary capacity when evicting a snapshot', async () => {
        ...
        expect(cache.lookup(secondScope, async () => snapshot(2)).status).toBe('cache_hit');
        expect(cache.lookup(thirdScope, async () => snapshot(1)).status).toBe('cache_hit');
        cache.clear();
    });
    ...
    it('unrefs expiry timers and clears retained values', async () => {
        ...
        await cache.lookup(SCOPE, async () => snapshot()).promise;
        const timer = timerSpy.mock.results.at(-1)?.value;
...
//# sourceMappingURL=persisted-session-list-cache.test.js.map