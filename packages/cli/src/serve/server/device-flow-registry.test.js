...
import { brandSecret, } from '../auth/device-flow.js';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
vi.mock('../../utils/stdioHelpers.js', () => ({ writeStderrLine: vi.fn() }));
const PROVIDER_ID = 'qwen-oauth';
...
describe('createDeviceFlowRegistry device-flow event fan-out', () => {
    ...
    it('fans a device-flow event out to every resolved bridge', async () => {
        ...
        for (const bridge of [a, b]) {
            expect(bridge.publishWorkspaceEvent).toHaveBeenCalledWith(expect.objectContaining({ type: 'auth_device_flow_started' }));
        }
    });
    it('keeps delivering to the other bridges when one throws', async () => {
        ...
        expect(b.publishWorkspaceEvent).toHaveBeenCalledWith(expect.objectContaining({ type: 'auth_device_flow_started' }));
        expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('bridge A delivery failed'));
    });
    it('falls back to the single bridge when no resolver is provided', async () => {
        ...
        dispose = () => deviceFlowRegistry.dispose();
        await deviceFlowRegistry.start({ providerId: PROVIDER_ID });
        expect(only.publishWorkspaceEvent).toHaveBeenCalledWith(expect.objectContaining({ type: 'auth_device_flow_started' }));
    });
});
//# sourceMappingURL=device-flow-registry.test.js.map