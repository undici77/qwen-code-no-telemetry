...
export async function waitForServeRuntimeOrExit(handle) {
    try {
        ...
        writeStderrLine(`qwen serve: runtime startup failed after listener was ready: ${err instanceof Error ? err.message : String(err)}`);
...
export async function tryRunServeFastPath(rawArgv = process.argv.slice(2)) {
    ...
    try {
        ...
        writeStderrLine(`qwen serve: fast-path bootstrap failed, falling back to full startup: ${err instanceof Error ? err.message : String(err)}`);
        return false;
    }
