...
import { ChannelWorkerControlError, createChannelWorkerManager, } from './channel-worker-manager.js';
import { ChannelWorkerStartupError } from './channel-worker-supervisor.js';
const PRIMARY = '/ws/primary';
const SECONDARY = '/ws/secondary';
function workspaceGroups(selection) {
    return [{ workspaceCwd: PRIMARY, selection }];
}
function splitWorkspaceGroups(selection) {
    ...
    const primary = selection.names.filter((name) => !name.startsWith('secondary-'));
    const secondary = selection.names.filter((name) => name.startsWith('secondary-'));
    return [
        ...(primary.length > 0
            ? [
                {
                    workspaceCwd: PRIMARY,
                    selection: { mode: 'names', names: primary },
                },
            ]
            : []),
        ...(secondary.length > 0
            ? [
                {
                    ...
                    selection: { mode: 'names', names: secondary },
                },
            ]
            : []),
    ];
...
function fakeGroup(overrides = {}) {
    ...
    return {
        ...
        isHealthy: vi.fn(() => snapshots.every((worker) => worker.state === 'running')),
        killAllSync: vi.fn(),
        ...
        deliverChannelMessage: vi.fn(async () => ({ delivered: true })),
        enqueueWebhookTask: vi.fn(async () => ({ accepted: true })),
        ...overrides,
    };
}
function setup(group = fakeGroup()) {
    ...
    const resolveGroups = vi.fn(async (selection) => workspaceGroups(selection));
...
describe('createChannelWorkerManager', () => {
    ...
    it('serializes concurrent owner-scoped channel starts', async () => {
        ...
        test.resolveGroups.mockImplementation(async (selection) => splitWorkspaceGroups(selection));
        await manager.setSelection({ mode: 'names', names: ['telegram'] });
        await Promise.all([
            manager.setChannelEnabled({ name: 'primary-bot', workspaceCwd: PRIMARY }, true),
            manager.setChannelEnabled({ name: 'secondary-bot', workspaceCwd: SECONDARY }, true),
        ]);
    ...
    it('serializes an owner-scoped start with a concurrent stop', async () => {
        ...
        await Promise.all([
            manager.setChannelEnabled({ name: 'secondary-bot', workspaceCwd: SECONDARY }, true),
            manager.setChannelEnabled({ name: 'telegram', workspaceCwd: PRIMARY }, false),
        ]);
    ...
    it('enables a disabled manager and makes an equal healthy PUT idempotent', async () => {
        ...
        expect(test.onCommittedSelection).toHaveBeenCalledWith(selection, workspaceGroups(selection));
        expect(test.onStateChange).toHaveBeenCalledWith(expect.objectContaining({
            enabled: false,
            pendingSelection: selection,
            transition: 'starting',
        }));
        expect(test.onStateChange).toHaveBeenLastCalledWith(expect.objectContaining({
            enabled: true,
            selection,
            transition: 'idle',
        }));
    ...
    it('refreshes workspace topology without forcing unchanged workers', async () => {
        ...
        expect(test.group.reconcile).toHaveBeenCalledWith(workspaceGroups(selection));
    ...
    it('restores idle and classifies workspace topology reconcile failures', async () => {
        ...
        await test.manager.setSelection(selection);
        vi.mocked(test.group.reconcile).mockRejectedValueOnce(new ChannelWorkerReconcileError('secondary failed', {
            rolledBack: true,
        }));
    ...
    it('restores idle when workspace topology resolution fails', async () => {
        ...
        await expect(test.manager.refreshWorkspaces()).rejects.toThrow('settings invalid');
        expect(test.manager.state()).toMatchObject({
            transition: 'idle',
            selection,
        });
    ...
    it('does not reconcile after forced shutdown interrupts workspace refresh', async () => {
        ...
        await test.manager.setSelection(selection);
        let releaseGroups;
        test.resolveGroups.mockImplementationOnce(() => new Promise((resolve) => {
    ...
    it('applies an existing workspace drain before a newly created group starts', async () => {
        ...
        expect(vi.mocked(test.group.beginWorkspaceDrain).mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(test.group.start).mock.invocationCallOrder[0]);
    });
    it('keeps the boot-time lease reserved when group construction fails', async () => {
        ...
        await expect(test.manager.startInitial({ mode: 'names', names: ['telegram'] })).rejects.toMatchObject({
            code: 'channel_worker_start_failed',
            rolledBack: false,
        });
    ...
    it('marks only the first concurrent enable as created', async () => {
        ...
        const group = fakeGroup({
            start: vi.fn(() => new Promise((resolve) => {
    ...
    it('reports partial readiness without treating it as a failed enable', async () => {
        ...
        const test = setup(group);
        await expect(test.manager.setSelection({
            mode: 'names',
    ...
    it('reconciles replacements without reacquiring the existing lease', async () => {
        ...
        expect(test.reserveLease).toHaveBeenCalledTimes(1);
        expect(test.group.reconcile).toHaveBeenCalledWith(workspaceGroups({ mode: 'names', names: ['discord'] }), { onRollingBack: expect.any(Function) });
    });
    ...
    it('force-reconciles reload and returns the primary snapshot', async () => {
        ...
        expect(test.resolveGroups).toHaveBeenLastCalledWith(selection, 'reload');
        expect(group.reconcile).toHaveBeenLastCalledWith(workspaceGroups(selection), { force: true, onRollingBack: expect.any(Function) });
    });
    it('reloads only the requested workspace worker', async () => {
        ...
        await expect(test.manager.reloadWorkspace(PRIMARY, 'telegram')).resolves.toEqual(primary);
        expect(group.reconcile).toHaveBeenLastCalledWith(targetGroups, {
            forceWorkspaceCwd: PRIMARY,
            onRollingBack: expect.any(Function),
        });
        expect(test.onCommittedSelection).toHaveBeenLastCalledWith({ mode: 'names', names: ['telegram', 'feishu'] }, initialGroups);
    });
    it.each([
        {
            ...
            targetGroups: [
                ...
                {
                    workspaceCwd: '/ws/secondary',
                    selection: { mode: 'names', names: ['feishu', 'bot'] },
                },
            ],
        },
        {
    ...
    ])('rejects targeted reload when the edited channel $label', async ({ targetGroups }) => {
        const initialGroups = [
            {
                workspaceCwd: PRIMARY,
        ...
        await expect(test.manager.reloadWorkspace(PRIMARY, 'bot')).rejects.toMatchObject({ code: 'channel_runtime_owner_mismatch' });
        expect(test.group.reconcile).not.toHaveBeenCalled();
        expect(test.onCommittedSelection).toHaveBeenLastCalledWith({ mode: 'names', names: ['bot', 'other', 'feishu'] }, initialGroups);
    });
    it('rejects a required owner mismatch before reconciling selection', async () => {
        ...
        await expect(test.manager.setSelection({ mode: 'names', names: ['bot'] }, { name: 'bot', workspaceCwd: PRIMARY })).rejects.toMatchObject({ code: 'channel_runtime_owner_mismatch' });
    ...
    it('preserves workspace-attributed startup failures from reload', async () => {
        ...
        await test.manager.setSelection(selection);
        vi.mocked(test.group.reconcile).mockRejectedValueOnce(new ChannelWorkerReconcileError('reload failed', {
            rolledBack: true,
            startupFailures: [
                {
                    workspaceCwd: '/ws/secondary',
    ...
    it('does not reconcile after forced shutdown interrupts reload resolution', async () => {
        ...
        await vi.waitFor(() => expect(test.resolveGroups).toHaveBeenLastCalledWith(selection, 'reload'));
        test.manager.killAllSync();
        releaseGroups();
    ...
    it('keeps the old committed selection when reconcile rolls back', async () => {
        ...
        vi.mocked(group.reconcile).mockRejectedValueOnce(new ChannelWorkerReconcileError('discord failed', {
            rolledBack: true,
        }));
        await expect(test.manager.setSelection({ mode: 'names', names: ['discord'] })).rejects.toMatchObject({
            code: 'channel_worker_start_failed',
            rolledBack: true,
        });
        expect(test.manager.state()).toMatchObject({
    ...
    it('publishes rolling_back while a failed replacement restores old workers', async () => {
        ...
        await test.manager.setSelection({ mode: 'names', names: ['telegram'] });
        let releaseRollback;
        vi.mocked(group.reconcile).mockImplementationOnce(async (_groups, options) => {
    ...
    it('retains a failed first-start group and lease until DELETE confirms stop', async () => {
        const group = fakeGroup();
        vi.mocked(group.start).mockRejectedValueOnce(new ChannelWorkerStartupError('spawn failed', {
            workspaceCwd: PRIMARY,
            startupFailures: [
                {
                    channel: 'telegram',
        ...
        const test = setup(group);
        await expect(test.manager.setSelection({ mode: 'names', names: ['telegram'] })).rejects.toMatchObject({
            code: 'channel_worker_start_failed',
            rolledBack: false,
    ...
    it('returns attempted startup failures while current state reflects successful rollback', async () => {
        ...
        error.startupFailures[0].message =
            'mutated';
        expect(startupError.startupFailures[0].message).toBe('connection refused');
    });
    it('does not replace attempted failures with a reconcile rollback error', async () => {
        const group = fakeGroup();
        const test = setup(group);
        await test.manager.setSelection({ mode: 'names', names: ['telegram'] });
        vi.mocked(group.reconcile).mockRejectedValueOnce(new ChannelWorkerReconcileError('replacement failed', {
            rolledBack: false,
    ...
    it('rejects webhook work after shutdown latches', async () => {
        ...
        await expect(test.manager.enqueueWebhookTask({
    ...
    it('rejects delivery while shutdown is draining workers', async () => {
        ...
        await expect(test.manager.deliverChannelMessage(PRIMARY, {
    ...
    it('routes delivery through the committed group and exact workspace', async () => {
        ...
        await expect(test.manager.deliverChannelMessage(PRIMARY, delivery)).resolves.toEqual({ delivered: true });
    ...
    it('finishes mutations queued before shutdown in FIFO order', async () => {
        ...
        await test.manager.setSelection({ mode: 'names', names: ['telegram'] });
        let releaseFirst;
        vi.mocked(group.reconcile).mockImplementationOnce(() => new Promise((resolve) => {
            releaseFirst = () => resolve({ changed: true, workers: group.snapshots() });
        }));
    ...
    it('does not create a worker after forced shutdown interrupts group resolution', async () => {
        ...
        let releaseGroups;
        test.resolveGroups.mockImplementationOnce(() => new Promise((resolve) => {
            releaseGroups = () => resolve(workspaceGroups({ mode: 'names', names: ['telegram'] }));
        }));
        const enabling = test.manager.setSelection({
            mode: 'names',
            names: ['telegram'],
...
//# sourceMappingURL=channel-worker-manager.test.js.map