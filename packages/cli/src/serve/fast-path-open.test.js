...
describe('serve fast path --open import boundary', () => {
    ...
    function useTempQwenHome() {
        tempQwenHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-fast-path-open-')));
        process.env['QWEN_HOME'] = tempQwenHome;
        process.env['QWEN_CODE_SYSTEM_SETTINGS_PATH'] = path.join(tempQwenHome, 'system-settings.json');
        process.env['QWEN_CODE_SYSTEM_DEFAULTS_PATH'] = path.join(tempQwenHome, 'system-defaults.json');
        process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'] = path.join(tempQwenHome, 'trustedFolders.json');
    }
    afterEach(() => {
        vi.restoreAllMocks();
    ...
    it('defers importing the full serve command opener until runtime is ready', async () => {
        ...
        expect(runQwenServe).toHaveBeenCalledWith(expect.any(Object), expect.objectContaining({ deferRuntimeUntilFirstHealth: false }));
...
//# sourceMappingURL=fast-path-open.test.js.map