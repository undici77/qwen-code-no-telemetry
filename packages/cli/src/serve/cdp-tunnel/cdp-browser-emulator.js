    ...
    emitTabEvent(method, params) {
        ...
        if (!this.autoAttachActive && this.attachedPageSessions.size === 0) {
            ...
            if (this.droppedTabEvents === 1 || this.droppedTabEvents % 100 === 0) {
                this.cb.log?.(`qwen serve: /cdp tab event "${method}" dropped (${this.droppedTabEvents} total) — no active page session (lazy-attach clients are command-only until Target.setAutoAttach)`);
            }
        }
    }
}
