...
function setup(forward) {
    ...
    const emu = new CdpBrowserEmulator({ reply: (f) => replies.push(f), forwardToTab, log }, { url: 'https://example.com/', title: 'Mock Page' });
...
describe('CdpBrowserEmulator (Plan C #5626)', () => {
    ...
    it('emits two targetCreated (tab + page) on setDiscoverTargets', async () => {
        ...
        expect(created.map((c) => c.params.targetInfo.type)).toEqual(['tab', 'page']);
    ...
    it('browser-level setAutoAttach attaches the TAB session', async () => {
        ...
        const attached = replies.find((r) => r.method === 'Target.attachedToTarget');
    ...
    it('creates and forwards an explicitly attached page session', async () => {
        ...
        const attached = replies.find((reply) => reply.method === 'Target.attachedToTarget');
    ...
    it('rejects attachToTarget for a target that is not the page', async () => {
        ...
        expect(replies.find((reply) => reply.method === 'Target.attachedToTarget')).toBeUndefined();
        expect(replies.at(-1)).toMatchObject({
            id: 15,
    ...
    it('keeps the auto-attach session silent when autoAttach is false', async () => {
        ...
        expect(replies.filter((r) => r.method === 'Target.attachedToTarget')).toHaveLength(0);
        replies.length = 0;
        emu.emitTabEvent('Network.requestWillBeSent', { requestId: 'r-off' });
        expect(replies.filter((reply) => reply.sessionId === 'qwen-cdp-page-session')).toHaveLength(0);
    });
    it('skips the auto-attach session when no setAutoAttach handshake occurred', async () => {
        ...
        expect(replies.filter((reply) => reply.sessionId === 'qwen-cdp-page-session')).toHaveLength(0);
        expect(replies.filter((reply) => reply.sessionId === sessionId)).toHaveLength(1);
    });
    ...
    it('delivers each event once to a client that attached through a single path', async () => {
        ...
        const explicit = replies.find((reply) => reply.method === 'Target.attachedToTarget' &&
            reply.params.sessionId !==
                'qwen-cdp-page-session');
        const explicitSessionId = explicit?.params
            ?.sessionId;
        ...
        expect(replies.filter((reply) => reply.sessionId === 'qwen-cdp-page-session')).toHaveLength(1);
        expect(replies.filter((reply) => reply.sessionId === explicitSessionId)).toHaveLength(1);
    });
    ...
    it('does not emit detachedFromTarget for a never-attached session', async () => {
        ...
        expect(replies.find((reply) => reply.method === 'Target.detachedFromTarget')).toBeUndefined();
...
//# sourceMappingURL=cdp-browser-emulator.test.js.map