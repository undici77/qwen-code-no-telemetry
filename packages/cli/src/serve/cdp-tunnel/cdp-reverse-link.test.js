...
import { CdpReverseLink, isCdpInboundFrameType, } from './cdp-reverse-link.js';
function setup() {
    const sent = [];
    const link = new CdpReverseLink((f) => sent.push(f));
    return { link, sent };
}
async function markAttached(link, sent) {
    const p = link.attach();
    const attachId = sent.at(-1).id;
...
describe('CdpReverseLink (Plan C #5626)', () => {
    ...
    it('routes cdp_event to the bound emulator as a tab event', () => {
        ...
        link.bindEmulator({ emitTabEvent });
    ...
    it('dispose rejects pending commands and refuses new ones', async () => {
        ...
        await expect(link.forwardToTab('Runtime.enable', undefined)).rejects.toMatchObject({ message: 'CDP tunnel closed' });
    });
...
//# sourceMappingURL=cdp-reverse-link.test.js.map