...
import { createPermissionAuditPublisher, DEFAULT_PERMISSION_AUDIT_RING_SIZE, PermissionAuditRing, } from './permission-audit.js';
const RECORD = {
    requestId: 'req-1',
    sessionId: 'sess-1',
...
describe('PermissionAuditRing', () => {
    ...
    it('throws on non-positive-integer capacity', () => {
        ...
        expect(() => new PermissionAuditRing(Number.NaN)).toThrow(/positive integer/);
    });
    it('evicts the oldest entry on overflow (FIFO)', () => {
        ...
        publisher.recordRequested({ ...RECORD, requestId: 'req-1' }, 'first-responder', new Set());
        publisher.recordRequested({ ...RECORD, requestId: 'req-2' }, 'first-responder', new Set());
        publisher.recordRequested({ ...RECORD, requestId: 'req-3' }, 'first-responder', new Set());
        expect(ring.size).toBe(3);
        publisher.recordRequested({ ...RECORD, requestId: 'req-4' }, 'first-responder', new Set());
        expect(ring.size).toBe(3);
    ...
    it('snapshot honors limit and rejects invalid limits', () => {
        ...
        for (let i = 0; i < 10; i++) {
            publisher.recordRequested({ ...RECORD, requestId: `req-${i}` }, 'first-responder', new Set());
        }
    ...
    it('snapshotForSession filters by sessionId', () => {
        const ring = new PermissionAuditRing();
        const publisher = createPermissionAuditPublisher({ ring, now: () => 0 });
        publisher.recordRequested({ ...RECORD, sessionId: 'A' }, 'first-responder', new Set());
        publisher.recordRequested({ ...RECORD, sessionId: 'B' }, 'first-responder', new Set());
        publisher.recordRequested({ ...RECORD, sessionId: 'A' }, 'first-responder', new Set());
...
describe('createPermissionAuditPublisher', () => {
    it('writes the requested entry shape with timestamp and votersAtIssue', () => {
        ...
        publisher.recordRequested(RECORD, 'consensus', new Set(['client_A', 'client_B']));
    ...
    it('writes the resolved entry with structured decisionReason', () => {
        const ring = new PermissionAuditRing();
        const publisher = createPermissionAuditPublisher({ ring, now: () => 0 });
        publisher.recordResolved(RECORD, { kind: 'option', optionId: 'proceed_once' }, { type: 'first-responder', resolverClientId: 'client_A' });
        const [entry] = ring.snapshot();
        expect(entry).toMatchObject({
...
//# sourceMappingURL=permission-audit.test.js.map