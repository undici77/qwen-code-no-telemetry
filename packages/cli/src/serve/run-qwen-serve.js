...
import { DEFAULT_COMPACTED_REPLAY_MAX_BYTES, DEFAULT_MAX_JOURNAL_BYTES, DEFAULT_MAX_JOURNAL_EVENTS, normalizeCompactedReplayMaxBytes, normalizeMaxJournalBytes, normalizeMaxJournalEvents, } from '@qwen-code/acp-bridge/replayWindowLimits';
...
import { formatMemoryBudgetStderr, resolveDaemonMemoryBudget, } from '@qwen-code/acp-bridge/daemonMemoryBudget';
import { createChildHeapPolicy, } from '@qwen-code/acp-bridge/childHeapPolicy';
import { canonicalizeWorkspace, translateAndCheckAbsoluteWorkspacePath, } from '@qwen-code/acp-bridge/workspacePaths';
...
import { EXTERNAL_TOOL_GUARD_REQUIRED_VALUE, EXTERNAL_TOOL_GUARD_TOKEN_ENV, PRIVATE_EXTERNAL_TOOL_GUARD_ENV, } from '@qwen-code/acp-bridge/externalToolGuard';
...
export function validatePolicyConfig(policyConfig = {}, onWarning = writeStderrLine) {
    ...
    if (permissionStrategy !== undefined &&
        ...
        throw new InvalidPolicyConfigError(`qwen serve: invalid policy.permissionStrategy ` +
    ...
    if (consensusQuorum !== undefined &&
        (typeof consensusQuorum !== 'number' ||
            ...
            consensusQuorum < 1)) {
        throw new InvalidPolicyConfigError(`qwen serve: invalid policy.consensusQuorum ` +
    ...
    if (consensusQuorum !== undefined && permissionStrategy !== 'consensus') {
        onWarning('qwen serve: policy.consensusQuorum is set but ' +
...
export function subSessionConcurrencyCapsFromSettings(serve, onWarning = writeStderrLine) {
    const asCap = (key, value) => {
        ...
        onWarning(`qwen serve: ignoring invalid ${key} (${JSON.stringify(value)}); ` +
...
async function loadServeRuntimeModules() {
    const [serverModule, bridgeModule, spawnChannelModule, processRegistryModule, workspaceModule, workspaceTypesModule, daemonStatusProviderModule, workspaceProvidersStatusModule, workspaceSkillsStatusModule, totalSessionAdmissionModule, workspaceRegistryModule,] = await Promise.all([
        import('./server.js'),
...
async function runQwenServeImpl(optsIn, deps, loggerLifecycle) {
    ...
    // environment back.
    const loaderEnvScrub = acquireInheritedLoaderEnvScrub(process.env, 'qwen serve', 'daemon');
    ...
    if (optsIn.enableSessionShell === true && token === undefined) {
        writeStderrLine(`qwen serve: --enable-session-shell ignored because no bearer token ` +
    ...
    if (opts.allowOrigins && opts.allowOrigins.length > 0) {
        ...
        writeStderrLine(`qwen serve: --allow-origin: ${opts.allowOrigins.join(', ')}` +
            (parsed.allowAny
    ...
    if (opts.allowPrivateAuthBaseUrl) {
        writeStderrLine('qwen serve: --allow-private-auth-base-url enabled; ' +
    ...
    try {
        ...
        writeStderrLine(`qwen serve: could not read settings for context.fileName / ` +
    ...
    writeStderrLine(`qwen serve: daemon log → ${daemonLog.getLogPath() || '(disabled)'}`);
    ...
    if (opts.externalToolGuard?.mode === 'required') {
        ...
        writeStderrLine('qwen serve: required external tool guard handshake succeeded.');
    }
    ...
    if (opts.serveWebShell !== false) {
        if (!webShellDir) {
            writeStderrLine('qwen serve: Web Shell assets not found; serving API only. ' +
            ...
            if (!isLoopbackBind(opts.hostname)) {
                writeStderrLine('qwen serve: Web Shell UI is served WITHOUT auth on a non-loopback ' +
                ...
                if (!opts.allowOrigins || opts.allowOrigins.length === 0) {
                    writeStderrLine('qwen serve: without --allow-origin the Web Shell is read-only on a ' +
    ...
    let runtimeFailureListenerClose;
    ...
    const buildRuntime = async () => {
        ...
        try {
            ...
            writeStderrLine(`qwen serve: managed scratch workspaces are unavailable: ${err instanceof Error ? err.message : String(err)}`);
        }
        const liveConversationWorkspace = deps.liveConversationWorkspace ?? new LiveConversationWorkspace();
        ...
        try {
            ...
            writeStderrLine(`qwen serve: could not read full settings for runtime startup ` +
        ...
        const onListening = (error) => {
            ...
            const publishLiveDiscovery = (candidateApp) => {
                ...
                liveDiscoveryPublish = loadLiveDiscoveryRuntime()
                    .then(async ({ getStableLiveDiscoveryBaseDir, LiveDiscoveryOwnerActiveError, writeLiveDiscoveryFile, }) => {
                    if (liveDiscoveryShuttingDown || !liveDiscoveryEnabled)
                        return;
                    const stableBaseDir = path.resolve(deps.liveDiscoveryStableBaseDir ??
            ...
            const removeLiveDiscoveryOwners = async () => {
                ...
                try {
                    ...
                    daemonLog.warn(`failed to load Live discovery runtime for cleanup: ${err instanceof Error ? err.message : String(err)}`);
                    return;
                }
                for (const owner of owners) {
                    try {
                        ...
                        daemonLog.warn(`failed to remove Live Host discovery at ${owner.runtimeBaseDir}: ${err instanceof Error ? err.message : String(err)}`);
                    }
                }
            };
            ...
            try {
                ...
                server.close((closeErr) => {
                    if (closeErr) {
                        daemonLog.error('server close after channel worker validation error failed', closeErr);
                    }
                    reject(error);
                });
                return;
            ...
            writeStderrLine(`qwen serve: bound to workspace ${JSON.stringify(boundWorkspace)}`);
            writeStderrLine(`qwen serve: startup timing: processToListenMs=${startup.processToListenMs} ` +
                `runQwenServeToListenMs=${startup.runQwenServeToListenMs}`);
            if (!token) {
                writeStderrLine(`qwen serve: bearer auth disabled (loopback default). Set ${QWEN_SERVER_TOKEN_ENV} to enable.`);
                ...
                writeStderrLine('qwen serve: --require-auth enabled (bearer token mandatory ' +
            ...
            const startBridgePreheat = (bridge) => {
                ...
                bridge
                    ...
                    startup.preheat.durationMs = Math.round(performance.now() - preheatStartedAt);
                    startup.preheat.error = message;
                    writeStderrLine(`qwen serve: ACP preheat failed, will retry on first session: ${message}`);
                });
            };
            const startRuntime = () => {
                if (runtimeStarting)
                    ...
                    closePromise = new Promise((res, rej) => {
                        ...
                        const finish = (err) => {
                            ...
                            void (coreRuntimePromise
                                ...
                                : Promise.resolve())
                                .catch((telemetryErr) => {
                                writeStderrLine(`qwen serve: telemetry shutdown error: ${telemetryErr instanceof Error
                                ...
                                const finalErr = err ?? bridgeShutdownError ?? channelWorkerShutdownError;
                        ...
                        Promise.resolve()
                            ...
                            .finally(() => {
                            if (!server.listening) {
                                void (runtimeFailureListenerClose ?? Promise.resolve()).then(() => finish());
                                return;
                            }
                            ...
                            const forceTimer = setTimeout(() => {
                                daemonLog.warn(`${SHUTDOWN_FORCE_CLOSE_MS}ms listener-drain timeout reached; force-closing remaining connections`);
                                server.closeAllConnections();
                                ...
                                secondaryTimer = setTimeout(() => {
                                    daemonLog.warn(`server.close did not fire ${SECONDARY_DEADLINE_MS}ms after force-close; resolving anyway`);
                                    finish();
...
//# sourceMappingURL=run-qwen-serve.js.map