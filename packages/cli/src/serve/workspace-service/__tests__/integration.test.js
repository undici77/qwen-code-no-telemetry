...
// ---------------------------------------------------------------------------
function minimalBridge(overrides = {}) {
    const knownIds = new Set(overrides.knownClientIds ?? []);
    return {
        ...
        subscribeEvents: vi.fn().mockReturnValue((async function* () {
            /* empty */
        })()),
        ...
        queryWorkspaceStatus: vi
            .fn()
            .mockImplementation((_method, idle) => idle()),
...
// ---------------------------------------------------------------------------
function mockWorkspaceService(overrides = {}) {
    return {
        getWorkspaceMcpStatus: vi.fn().mockResolvedValue({
            v: 1,
            workspaceCwd: WS_BOUND,
...
describe('workspace service REST integration', () => {
    ...
    describe('GET /workspace/mcp', () => {
        ...
        it('passes correct WorkspaceRequestContext to the service', async () => {
            ...
            const ctx = workspace.getWorkspaceMcpStatus
                .mock.calls[0][0];
    ...
    describe('GET /workspace/skills', () => {
        ...
        it('passes correct WorkspaceRequestContext to the service', async () => {
            ...
            const ctx = workspace.getWorkspaceSkillsStatus.mock.calls[0][0];
    ...
    describe('GET /workspace/env', () => {
        ...
        it('passes correct WorkspaceRequestContext to the service', async () => {
            ...
            const ctx = workspace.getWorkspaceEnvStatus
                .mock.calls[0][0];
    ...
    describe('POST /workspace/init', () => {
        ...
        it('passes force:false opts when body is empty', async () => {
            ...
            await request(app)
                ...
                .set('Authorization', 'Bearer test-secret')
                .send({});
            const [ctx, opts] = workspace.initWorkspace
                .mock.calls[0];
        ...
        it('passes force:true when body has force=true', async () => {
            ...
            await request(app)
                ...
                .set('Authorization', 'Bearer test-secret')
                .send({ force: true });
            const [_ctx, opts] = workspace.initWorkspace
                .mock.calls[0];
        ...
        it('passes client identity through WorkspaceRequestContext', async () => {
            ...
            await request(app)
                ...
                .send({});
            const ctx = workspace.initWorkspace.mock
                .calls[0][0];
