...
export function createDaemonWorkspaceService(deps) {
    ...
    const readWorkspaceSkillsStatus = async (generation) => {
        ...
        try {
            ...
            writeStderrLine(`qwen serve: getWorkspaceSkillsStatus query failed: ${err instanceof Error ? err.message : String(err)}`);
        ...
        if (workspaceSkillsStatusProvider) {
            try {
                ...
                writeStderrLine(`qwen serve: getWorkspaceSkillsStatus local provider failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        return status;
    };
    ...
    const refreshWorkspaceSkillsAfterMutation = async () => {
        ...
        try {
            ...
            if ((refreshed.configsFailed ?? 0) > 0) {
                writeStderrLine(`qwen serve: ${refreshed.configsFailed} skill cache refresh(es) failed after mutation`);
            }
        }
        catch (err) {
            if (!(err instanceof SessionNotFoundError) &&
                !(err instanceof BridgeChannelClosedError)) {
                writeStderrLine(`qwen serve: workspace skill refresh after mutation failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        finally {
            invalidateWorkspaceSkillsSnapshot();
    ...
    return {
        ...
        async preheatAcpChild(_ctx, opts) {
            ...
            if (!inFlightAcpPreheat) {
                ...
                void promise.then(() => {
                    ...
                    try {
                        writeStderrLineSafe(`qwen serve: ACP preheat failed: ${err instanceof Error ? err.message : String(err)}`);
                    }
                    finally {
                        if (inFlightAcpPreheat === promise) {
                            inFlightAcpPreheat = undefined;
            ...
            try {
                ...
                if (err instanceof TimeoutError) {
                    writeStderrLineSafe(`qwen serve: ACP preheat timed out after ${opts?.timeoutMs ?? 5_000}ms`);
                }
            ...
            const live = channelLive();
            if (!live) {
                writeStderrLineSafe('qwen serve: ACP preheat resolved without a live channel');
            }
            return live
        ...
        async getWorkspaceEnvStatus(_ctx) {
            ...
            try {
                ...
                writeStderrLine(`qwen serve: getEnvStatus failed: ${err instanceof Error ? err.message : String(err)}`);
                return createIdleEnvStatus(boundWorkspace, acpChannelLive);
            }
        },
        async getWorkspacePreflightStatus(_ctx) {
            ...
            if (statusProvider) {
                try {
                    ...
                    writeStderrLine(`qwen serve: getDaemonPreflightCells failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
        ...
        async setWorkspacePermissionRules(ctx, request) {
            ...
            try {
                const result = await invokeWorkspaceCommand('qwen/permissions/setRules', {
                    cwd: boundWorkspace,
                    scope: request.scope,
            ...
            if (persistSettings) {
                ...
                for (const write of writes) {
                    try {
                        ...
                        assertActiveGeneration();
                        writeStderrLine(`qwen serve: workspace voice partial persist error (workspace=${boundWorkspace}, committed=${committed.length}/${writes.length}, failedKey=${write.key}, failedScope=${voiceSettingsScopeToWire(write.scope)}): ${err instanceof Error ? err.message : String(err)}`);
                ...
                if (channelLive) {
                    try {
                        ...
                        if (err instanceof SessionNotFoundError ||
                            ...
                            activation = 'partial';
                            sessionsFailed = 1;
                            writeStderrLine(`qwen serve: workspace skill refresh failed: ${err instanceof Error ? err.message : String(err)}`);
                        }
                    }
                    invalidateWorkspaceSkillsSnapshot();
                }
        ...
        async restartMcpServer(ctx, serverName, opts) {
            ...
            if ('entries' in result) {
                ...
                if (!Array.isArray(result.entries)) {
                    writeStderrLine(`qwen serve: pool restart response carried 'entries' field ` +
                ...
                for (const entry of entries) {
                    if (typeof entry !== 'object' ||
                        ...
                        writeStderrLine(`qwen serve: skipping malformed pool restart entry ` +
        ...
        async reload(ctx) {
            ...
            if (deps.reloadDaemonEnv) {
                try {
                    ...
                    writeStderrLine(`qwen serve: daemon reload failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
            assertActiveGeneration();
        ...
        async refreshExtensionsForAllSessions() {
            ...
            try {
                ...
                writeStderrLine(`qwen serve: refreshExtensionsForAllSessions failed: ${err instanceof Error ? err.message : String(err)}`);
