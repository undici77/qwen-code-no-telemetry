...
import { runQwenServe } from './runQwenServe.js';
...
import { InvalidClientIdError, InvalidPermissionOptionError, InvalidSessionMetadataError, MAX_WORKSPACE_PATH_LENGTH, RestoreInProgressError, SessionLimitExceededError, SessionNotFoundError, WorkspaceInitConflictError, WorkspaceMismatchError, } from './httpAcpBridge.js';
...
function fakeBridge(opts = {}) {
    ...
    const killCalls = [];
    const detachCalls = [];
    const permissionVotes = [];
    const sessionPermissionVotes = [];
    const listCalls = [];
    ...
    const sessionSupportedCommandsCalls = [];
    const setModelCalls = [];
    const closeCalls = [];
    const updateMetadataCalls = [];
    const heartbeatCalls = [];
    const heartbeatStateCalls = [];
    let shutdownCalls = 0;
    ...
    const resumeImpl = opts.resumeImpl ??
        (async (req) => ({
            sessionId: req.sessionId,
            workspaceCwd: req.workspaceCwd,
            attached: false,
    ...
    const promptImpl = opts.promptImpl ?? (async () => ({ stopReason: 'end_turn' }));
    ...
    const workspaceMcpImpl = opts.workspaceMcpImpl ??
        (async () => ({
            v: 1,
    ...
    const workspaceSkillsImpl = opts.workspaceSkillsImpl ??
        (async () => ({
            v: 1,
            workspaceCwd: WS_BOUND,
            initialized: false,
    ...
    const workspaceProvidersImpl = opts.workspaceProvidersImpl ??
        (async () => ({
            v: 1,
            workspaceCwd: WS_BOUND,
            initialized: false,
    ...
    const workspaceEnvImpl = opts.workspaceEnvImpl ??
        (async () => ({
            v: 1,
            workspaceCwd: WS_BOUND,
            initialized: true,
    ...
    const workspacePreflightImpl = opts.workspacePreflightImpl ??
        (async () => ({
            v: 1,
            workspaceCwd: WS_BOUND,
            initialized: true,
    ...
    const sessionContextImpl = opts.sessionContextImpl ??
        (async (sessionId) => ({
            v: 1,
            sessionId,
            workspaceCwd: WS_BOUND,
    ...
    const sessionSupportedCommandsImpl = opts.sessionSupportedCommandsImpl ??
        (async (sessionId) => ({
            v: 1,
            sessionId,
            availableCommands: [],
    ...
    const setApprovalModeImpl = opts.setApprovalModeImpl ??
        (async (sessionId, mode, o) => ({
            sessionId,
            mode,
            previous: ApprovalMode.DEFAULT,
            persisted: o.persist,
        }));
    const setToolEnabledCalls = [];
    const setToolEnabledImpl = opts.setToolEnabledImpl ??
        (async (toolName, enabled) => ({
            toolName,
            enabled,
        }));
    const initWorkspaceCalls = [];
    const initWorkspaceImpl = opts.initWorkspaceImpl ??
        (async () => ({
            path: path.resolve(WS_BOUND, 'QWEN.md'),
            action: 'created',
        }));
    const restartMcpServerCalls = [];
    const restartMcpServerImpl = opts.restartMcpServerImpl ??
        (async (serverName) => ({
            serverName,
            restarted: true,
            durationMs: 42,
    ...
    const heartbeatImpl = opts.heartbeatImpl ??
        ((sessionId, context) => ({
            sessionId,
    ...
    const heartbeatStateImpl = opts.heartbeatStateImpl ??
        (() => ({
    ...
    return {
        ...
        subscribeEvents(sessionId, subOpts) {
            if (opts.subscribeImpl)
                return opts.subscribeImpl(sessionId, subOpts);
        ...
        respondToSessionPermission(sessionId, requestId, response, context) {
            const accepted = sessionRespondImpl(sessionId, requestId, response, context);
        ...
        knownClientIds() {
            ...
            return new Set(opts.knownClientIds ?? []);
        },
...
describe('createServeApp', () => {
    describe('serve capability registry', () => {
        ...
        it('advertises `require_auth` only when the runtime toggle is on (#4175 PR 15)', () => {
            ...
            expect(getAdvertisedServeFeatures(undefined, { requireAuth: true })).toContain('require_auth');
            expect(getAdvertisedServeFeatures(undefined, { requireAuth: false })).not.toContain('require_auth');
            expect(getAdvertisedServeFeatures(undefined, {})).not.toContain('require_auth');
        });
        it('honors every entry in CONDITIONAL_SERVE_FEATURES (PR #4236 review #3254467192 — drift insurance)', () => {
            ...
            for (const [feature, predicate] of CONDITIONAL_SERVE_FEATURES) {
                if (feature === 'require_auth') {
                    ...
                    expect(getAdvertisedServeFeatures(undefined, { requireAuth: true })).toContain(feature);
                    expect(getAdvertisedServeFeatures(undefined, {})).not.toContain(feature);
                    continue;
                }
                ...
                throw new Error(`CONDITIONAL_SERVE_FEATURES added "${feature}" without an ` +
        ...
        it('marks every current feature with its historical v1 origin', () => {
            ...
            expect(Object.values(SERVE_CAPABILITY_REGISTRY).map(({ since }) => since)).toEqual(EXPECTED_REGISTERED_FEATURES.map(() => 'v1'));
        });
    ...
    describe('read-only status routes', () => {
        it('returns workspace MCP status from the bridge', async () => {
            ...
            const app = createServeApp({ ...baseOpts, workspace: WS_BOUND }, undefined, { bridge });
            const res = await request(app)
                .get('/workspace/mcp')
                .set('Host', `127.0.0.1:${baseOpts.port}`);
            expect(res.status).toBe(200);
        ...
        it('returns workspace env status from the bridge', async () => {
            ...
            const envVarCell = res.body.cells.find((c) => c.kind === 'env_var');
    ...
    describe('POST /session', () => {
        ...
        it('400 when cwd is present but not a string (#3803 §02 — distinguishes omitted vs malformed)', async () => {
            ...
            const malformed = [null, 123, { foo: 'bar' }, []];
        ...
        it('passes through a valid `sessionScope` to the bridge (#4175 PR 5)', async () => {
            ...
            for (const scope of ['single', 'thread']) {
                const bridge = fakeBridge();
                const app = createServeApp(baseOpts, undefined, { bridge });
                const res = await request(app)
                    .post('/session')
        ...
        it('400 invalid_session_scope when `sessionScope` is not "single"/"thread"', async () => {
            ...
            const malformed = ['user', '', 'SINGLE', null, 123, {}];
        ...
        it('strips prototype-pollution keys from body (BZ9uv/va/vs/wD)', async () => {
            ...
            const res = await request(app)
                ...
                .set('Host', `127.0.0.1:${baseOpts.port}`)
                .set('content-type', 'application/json')
                .send('{"cwd":"/work/a","__proto__":{"polluted":true},"constructor":{"prototype":{"polluted":true}}}');
            expect(res.status).toBe(200);
        ...
        it('400s a cwd longer than MAX_WORKSPACE_PATH_LENGTH before touching the bridge', async () => {
            ...
            for (const action of ['load', 'resume']) {
                ...
                const res = await request(app)
                    ...
                    .send({ cwd: longCwd });
                expect(res.status).toBe(400);
                expect(res.body.error).toMatch(new RegExp(`exceeds the ${MAX_WORKSPACE_PATH_LENGTH}-character limit`));
                expect(bridge.loadCalls).toHaveLength(0);
                expect(bridge.resumeCalls).toHaveLength(0);
            }
        });
    ...
    describe('POST /session/:id/prompt', () => {
        ...
        it('aborting the signal mid-prompt asks the bridge to wind down', async () => {
            ...
            let promptStarted;
            const promptStartedPromise = new Promise((r) => {
                promptStarted = r;
            });
            const bridge = fakeBridge({
                promptImpl: async (_sid, _req, signal) => new Promise((resolve) => {
                    promptStarted();
            ...
            const localHandle = await runQwenServe({ hostname: '127.0.0.1', port: 0, mode: 'http-bridge' }, { bridge });
            try {
                const port = localHandle.server.address().port;
    ...
    describe('GET /workspace/:id/sessions', () => {
        it('returns the list returned by the bridge', async () => {
            ...
            const app = createServeApp({ ...baseOpts, workspace: WS_BOUND }, undefined, { bridge, boundWorkspace: WS_BOUND });
            const res = await request(app)
    ...
    describe('POST /session/:id/approval-mode (#4175 Wave 4 PR 17)', () => {
        ...
        const tokenOpts = { ...baseOpts, token: 'secret' };
        const auth = (req) => req
        ...
        it('200 with the typed result on success and persist defaults to false', async () => {
            ...
            const res = await auth(request(app).post('/session/session-A/approval-mode')).send({ mode: 'yolo' });
            expect(res.status).toBe(200);
            expect(res.body).toEqual({
                sessionId: 'session-A',
                mode: 'yolo',
        ...
        it('forwards persist:true to the bridge', async () => {
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const res = await auth(request(app).post('/session/session-A/approval-mode')).send({ mode: 'auto-edit', persist: true });
            expect(res.status).toBe(200);
        ...
        it('passes client identity context into the bridge', async () => {
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const res = await auth(request(app).post('/session/session-A/approval-mode'))
                .set('X-Qwen-Client-Id', 'client-1')
                .send({ mode: 'plan' });
            expect(res.status).toBe(200);
        ...
        it('400 on missing or unknown mode literal', async () => {
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const missing = await auth(request(app).post('/session/session-A/approval-mode')).send({});
            ...
            const unknown = await auth(request(app).post('/session/session-A/approval-mode')).send({ mode: 'super-yolo' });
        ...
        it('400 when persist is non-boolean', async () => {
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const res = await auth(request(app).post('/session/session-A/approval-mode')).send({ mode: 'yolo', persist: 'truthy' });
            expect(res.status).toBe(400);
        ...
        it('404 when bridge reports unknown session', async () => {
            ...
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const res = await auth(request(app).post('/session/missing/approval-mode')).send({ mode: 'yolo' });
            expect(res.status).toBe(404);
            expect(res.body.sessionId).toBe('missing');
        });
    });
    ...
    describe('POST /workspace/mcp/:server/restart (#4175 Wave 4 PR 17)', () => {
        ...
        it('200 with restarted:true on success', async () => {
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const res = await auth(request(app).post('/workspace/mcp/docs/restart')).send({});
            expect(res.status).toBe(200);
            expect(res.body).toEqual({
                serverName: 'docs',
                restarted: true,
        ...
        it('passes client identity into the bridge', async () => {
            ...
            await auth(request(app).post('/workspace/mcp/docs/restart'))
                .set('X-Qwen-Client-Id', 'client-1')
                .send({});
            expect(bridge.restartMcpServerCalls[0]?.originatorClientId).toBe('client-1');
        });
        ...
        it('decodes URL-encoded server names', async () => {
            ...
            const res = await auth(request(app).post(`/workspace/mcp/${encodeURIComponent('foo-bar.io')}/restart`)).send({});
            expect(res.status).toBe(200);
        ...
        it('400 when serverName exceeds 256 chars (#4282 fold-in 4 S1)', async () => {
            ...
            const res = await auth(request(app).post(`/workspace/mcp/${overlong}/restart`)).send({});
            expect(res.status).toBe(400);
    ...
    describe('POST /workspace/tools/:name/enable (#4175 Wave 4 PR 17)', () => {
        ...
        it('200 with the typed result on success (disable)', async () => {
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const res = await auth(request(app).post('/workspace/tools/Bash/enable')).send({ enabled: false });
            expect(res.status).toBe(200);
        ...
        it('200 on enable=true (re-enable a previously disabled tool)', async () => {
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const res = await auth(request(app).post('/workspace/tools/Bash/enable')).send({ enabled: true });
            expect(res.status).toBe(200);
        ...
        it('passes client identity into the bridge', async () => {
            ...
            await auth(request(app).post('/workspace/tools/Bash/enable'))
                .set('X-Qwen-Client-Id', 'client-1')
                .send({ enabled: false });
            expect(bridge.setToolEnabledCalls[0]?.originatorClientId).toBe('client-1');
        });
        it('400 invalid_client_id on unknown X-Qwen-Client-Id', async () => {
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
        ...
        it('400 when enabled is missing or non-boolean', async () => {
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const missing = await auth(request(app).post('/workspace/tools/Bash/enable')).send({});
            expect(missing.status).toBe(400);
            expect(missing.body.code).toBe('invalid_enabled_flag');
            const bad = await auth(request(app).post('/workspace/tools/Bash/enable')).send({ enabled: 'truthy' });
        ...
        it('accepts URL-encoded MCP-qualified tool names', async () => {
            ...
            const res = await auth(request(app).post('/workspace/tools/mcp__github__create_issue/enable')).send({ enabled: false });
            expect(res.status).toBe(200);
            expect(bridge.setToolEnabledCalls[0]?.toolName).toBe('mcp__github__create_issue');
        });
        it('trims surrounding whitespace before persisting (#4282 fold-in 4 C3)', async () => {
            ...
            const bridge = fakeBridge();
            const app = createServeApp(tokenOpts, undefined, { bridge });
            const res = await auth(request(app).post('/workspace/tools/%20Bash%20/enable')).send({ enabled: false });
            expect(res.status).toBe(200);
        ...
        it('400 when whitespace-only path parameter trims to empty', async () => {
            ...
            const res = await auth(request(app).post('/workspace/tools/%20%20/enable')).send({ enabled: false });
            expect(res.status).toBe(400);
    ...
    describe('POST /session/:id/permission/:requestId', () => {
        ...
        it('400 with invalid_option_id when bridge rejects a scoped option', async () => {
            const bridge = fakeBridge({
                sessionRespondImpl: () => {
                    throw new InvalidPermissionOptionError('req-1', 'ProceedAlwaysProject');
                },
            });
            const app = createServeApp(baseOpts, undefined, { bridge });
            const res = await request(app)
    ...
    describe('POST /permission/:requestId', () => {
        ...
        it('200 with cancelled outcome', async () => {
            ...
            const res = await request(app)
                ...
                .send({ outcome: { outcome: 'cancelled' } });
            expect(res.status).toBe(200);
            expect(bridge.permissionVotes[0]?.response.outcome.outcome).toBe('cancelled');
        });
    ...
    describe('PATCH /session/:id/metadata', () => {
        ...
        it('400 invalid_metadata when displayName exceeds max length', async () => {
            const bridge = fakeBridge({
                updateMetadataImpl: () => {
                    throw new InvalidSessionMetadataError('displayName', 'must be a string of at most 256 characters');
                },
            });
            const app = createServeApp(baseOpts, undefined, { bridge });
            const res = await request(app)
...
describe('runQwenServe', () => {
    ...
    it('strips brackets from `[::1]` before passing to app.listen()', async () => {
        ...
        if (typeof addr === 'object' && addr) {
            ...
            // brackets).
            expect(addr.address === '::1' || addr.address === '::ffff:127.0.0.1').toBe(true);
        }
    });
    ...
    it('rejects unbracketed host:port typo with a useful error (BU-sh)', async () => {
        ...
        await expect(runQwenServe({
            ...
            port: 0,
            mode: 'http-bridge',
        })).rejects.toThrow(/Invalid --hostname "localhost:4170".*looks like a "host:port" combination/);
        await expect(runQwenServe({
    ...
    it('--workspace flows end-to-end and surfaces on /capabilities (#3803 §02)', async () => {
        ...
        const expected = await import('node:fs').then((m) => m.realpathSync.native(process.cwd()));
    ...
    it('drains the bridge before closing the listener', async () => {
        const bridge = fakeBridge();
        handle = await runQwenServe({ hostname: '127.0.0.1', port: 0, mode: 'http-bridge' }, { bridge });
    ...
    it('wires fsFactory + emit through to the read routes (#4175 PR 19 follow-up #2)', async () => {
        ...
        const captured = [];
        const bridge = fakeBridge();
        const wsRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-runqwen-fs-'));
        ...
        try {
            ...
            expect(captured.find((e) => e.type === 'fs.access' &&
                e.data.intent === 'read')).toBeDefined();
            const bad = await fetch(`http://127.0.0.1:${port}/file?path=../escape`);
            expect(bad.status).toBe(400);
            const denied = captured.find((e) => e.type === 'fs.denied' &&
                e.data.errorKind ===
    ...
    it('trust snapshot defaults to true (operator-chosen workspace)', async () => {
        ...
        // for review.
        const bridge = fakeBridge();
        const wsRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-runqwen-trust-'));
        try {
            const captured = [];
            handle = await runQwenServe({
                hostname: '127.0.0.1',
            ...
            expect(res.status).toBe(200);
            expect(captured.find((e) => e.data.errorKind ===
                'untrusted_workspace')).toBeUndefined();
        }
        finally {
            await fsp.rm(wsRoot, { recursive: true, force: true });
        }
    ...
    it('trust snapshot=false flows through deps.trustedWorkspace into the boundary (#4175 PR 19 follow-up #2)', async () => {
        ...
        const wsRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-runqwen-untrust-'));
        try {
            ...
            const { createWorkspaceFileSystemFactory } = await import('./fs/index.js');
...
describe('GET /session/:id/events (SSE)', () => {
    ...
    async function readSseFrames(body, minFrames) {
        ...
        while (frames.length < minFrames) {
            ...
            while ((idx = buf.indexOf('\n\n')) !== -1) {
                ...
                buf = buf.slice(idx + 2);
                if (!raw || raw.startsWith(':') || raw.startsWith('retry:'))
                    continue;
                const frame = {};
    ...
    it('forwards ?maxQueued=N to the bridge when in [16, 2048]', async () => {
        ...
        handle = await runQwenServe({ hostname: '127.0.0.1', port: 0, mode: 'http-bridge' }, { bridge });
        const port = handle.server.address().port;
        const res = await fetch(`http://127.0.0.1:${port}/session/sess-A/events?maxQueued=512`);
    ...
    it('400s a present-but-empty ?maxQueued= before opening the SSE stream', async () => {
        ...
        handle = await runQwenServe({ hostname: '127.0.0.1', port: 0, mode: 'http-bridge' }, { bridge });
        const port = handle.server.address().port;
        const res = await fetch(`http://127.0.0.1:${port}/session/sess-A/events?maxQueued=`);
        expect(res.status).toBe(400);
    ...
    it('400s a non-decimal ?maxQueued before opening the SSE stream', async () => {
        ...
        handle = await runQwenServe({ hostname: '127.0.0.1', port: 0, mode: 'http-bridge' }, { bridge });
        const port = handle.server.address().port;
        const res = await fetch(`http://127.0.0.1:${port}/session/sess-A/events?maxQueued=abc`);
        expect(res.status).toBe(400);
        expect(await res.json()).toMatchObject({ code: 'invalid_max_queued' });
    });
    it('400s an out-of-range ?maxQueued before opening the SSE stream', async () => {
        ...
        for (const bad of ['0', '15', '2049', '9999']) {
            const res = await fetch(`http://127.0.0.1:${port}/session/sess-A/events?maxQueued=${bad}`);
            expect(res.status).toBe(400);
            expect(await res.json()).toMatchObject({ code: 'invalid_max_queued' });
        }
    });
    ...
    it('drops malformed Last-Event-ID values (non-numeric, negative)', async () => {
        ...
        for (const value of ['abc', '-1', '1.5e10z']) {
            const res = await fetch(`http://127.0.0.1:${port}/session/sess-A/events`, { headers: { 'Last-Event-ID': value } });
            await res.body?.cancel();
        }
...
describe('GET /demo', () => {
    ...
    it('requires bearer token on non-loopback (401 without token)', async () => {
        ...
        const app = createServeApp({ ...baseOpts, hostname: '0.0.0.0', token: 'secret' }, () => 4170, { bridge: fakeBridge() });
...
describe('createServeApp ServeAppDeps.fsFactory wiring (#4175 PR 18)', () => {
    it('parks a default WorkspaceFileSystemFactory on app.locals when none is injected', async () => {
        ...
        const fsFactory = app.locals.fsFactory;
    ...
    it('uses the injected fsFactory verbatim when supplied', async () => {
        ...
        { fsFactory: sentinel });
        expect(app.locals.fsFactory).toBe(sentinel);
    });
    it('default fsFactory is built with trusted=false (writes refused)', async () => {
        const { createServeApp } = await import('./server.js');
        const { isFsError } = await import('./fs/index.js');
        const os = await import('node:os');
        const tmp = await import('node:fs').then((m) => m.promises.mkdtemp(path.join(os.tmpdir(), 'qwen-serve-default-trust-')));
        try {
            const app = createServeApp({
                port: 0,
                hostname: '127.0.0.1',
            ...
            const err = await fs.writeText(resolved, 'x').catch((e) => e);
            expect(isFsError(err)).toBe(true);
            expect(err.kind).toBe('untrusted_workspace');
        }
        finally {
            await import('node:fs').then((m) => m.promises.rm(tmp, { recursive: true, force: true }));
        }
    });
});
...
describe('auth device-flow routes', () => {
            ...
            provider: {
                ...
                async start() {
                    ...
                    return {
                        ...
                        (await import('./auth/deviceFlow.js')).brandSecret(`device-${starts}`),
                        pkceVerifier: (await import('./auth/deviceFlow.js')).brandSecret(`pkce-${starts}`),
    ...
    function buildApp(overrides = {}, fakeProvider = makeFakeProvider()) {
        const bridge = fakeBridge();
    ...
    it('POST take-over only echoes userCode/verificationUri/initiatorClientId to caller matching the initiator (#4291 follow-up review)', async () => {
        ...
        expect(takeoverDifferent.body).not.toHaveProperty('verificationUriComplete');
    ...
    it('GET /workspace/auth/device-flow/:id returns 200 for known + 404 for unknown', async () => {
        ...
        const post = await request(app)
            ...
            .set('Host', `127.0.0.1:${baseOpts.port}`)
            .send({ providerId: 'qwen-oauth' });
        const id = post.body.deviceFlowId;
    ...
    it('GET /workspace/auth/status surfaces pending flows and supported providers', async () => {
        ...
        const start = await request(app)
            ...
            .set('Host', `127.0.0.1:${baseOpts.port}`)
            .send({ providerId: 'qwen-oauth' });
        const id = start.body.deviceFlowId;
    ...
    it('sweeper-driven auto-expiry transitions a stale entry to status:error and surfaces over GET', async () => {
        ...
        const { DeviceFlowRegistry, brandSecret } = await import('./auth/deviceFlow.js');
        const fakeProvider = {
            providerId: 'qwen-oauth',
            async start() {
                return {
        ...
        const registry = new DeviceFlowRegistry({
            ...
            schedule: (_ms, _cb) => ({ cancelled: false }),
            clearScheduled: () => { },
        ...
        const id = startRes.body.deviceFlowId;
        // Drive the clock past expiresAt and trigger the sweeper.
        now += 61_000;
        for (const interval of intervalsRegistered)
            interval.cb();
    ...
    it('POST returns 409 too_many_active_flows when registry cap is reached', async () => {
        ...
        const { TooManyActiveDeviceFlowsError } = await import('./auth/deviceFlow.js');
        const fakeRegistry = {
            start: async () => {
    ...
    it('GET /workspace/auth/device-flow/:id only echoes userCode/verificationUri/initiatorClientId to caller matching the initiator', async () => {
        ...
        expect(matchingCaller.body.verificationUri).toBe('https://idp.example/verify');
