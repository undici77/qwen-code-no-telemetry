...
const { pickNativeDirectory, NativeDirectoryPickerUnavailableError } = await import('./native-directory-picker.js');
...
describe('pickNativeDirectory', () => {
    it('returns the trimmed path on macOS', async () => {
        ...
        expect(execFileAsyncMock).toHaveBeenCalledWith('osascript', expect.any(Array), { timeout: 300_000 });
    });
    ...
    it('treats a "User canceled" cancellation as no selection on macOS', async () => {
        setPlatform('darwin');
        execFileAsyncMock.mockRejectedValue(pickerError({ stderr: 'User canceled. (-128)' }));
        await expect(pickNativeDirectory()).resolves.toBeUndefined();
    });
    it('throws unavailable for a non-cancel failure on macOS', async () => {
        setPlatform('darwin');
        execFileAsyncMock.mockRejectedValue(pickerError({ stderr: 'boom' }));
        await expect(pickNativeDirectory()).rejects.toBeInstanceOf(NativeDirectoryPickerUnavailableError);
    });
    it('returns the selected path on Windows', async () => {
        ...
        expect(execFileAsyncMock).toHaveBeenCalledWith('powershell.exe', expect.any(Array), { timeout: 300_000 });
    });
    it('sets UTF-8 output encoding in the PowerShell script', async () => {
        setPlatform('win32');
        execFileAsyncMock.mockResolvedValue({ stdout: 'C:\\code' });
        await pickNativeDirectory();
        const args = execFileAsyncMock.mock.calls[0][1];
    ...
    it('returns the trimmed path on Linux', async () => {
        ...
        expect(execFileAsyncMock).toHaveBeenCalledWith('zenity', expect.any(Array), {
            timeout: 300_000,
        });
    });
    ...
    it('treats a headless "cannot open display" failure as unavailable on Linux', async () => {
        setPlatform('linux');
        execFileAsyncMock.mockRejectedValue(pickerError({
            code: 1,
    ...
    it('forwards the abort signal to the child process', async () => {
        ...
        expect(execFileAsyncMock).toHaveBeenCalledWith('osascript', expect.any(Array), { timeout: 300_000, signal: controller.signal });
    });
});
//# sourceMappingURL=native-directory-picker.test.js.map