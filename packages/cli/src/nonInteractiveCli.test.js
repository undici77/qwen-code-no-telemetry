 ...
 */
import { executeToolCall, isTelemetrySdkInitialized, ToolErrorType, shutdownTelemetry, GeminiEventType, Kind, OutputFormat, uiTelemetryService, FatalInputError, ApprovalMode, SendMessageType, SYSTEM_REMINDER_OPEN, LoopType, CronScheduler, AUTONOMOUS_SENTINEL_CRON, AUTONOMOUS_SENTINEL_DYNAMIC, LOOP_SENTINEL_CRON, LOOP_SENTINEL_DYNAMIC, TeamEventType, ToolConfirmationOutcome, ToolNames, PLAN_MODE_ENTRY_SIBLING_SKIP_MESSAGE, createGoalRuntime, GoalPersistenceUnavailableError, } from '@qwen-code/qwen-code-core';
import { EventEmitter } from 'node:events';
import { runNonInteractive, skipHeadlessLoopSentinel, TurnInterruptedError, } from './nonInteractiveCli.js';
...
describe('skipHeadlessLoopSentinel', () => {
    ...
    it('also cleans up recurring autonomous sentinel jobs', () => {
        const scheduler = new CronScheduler();
        const cron = scheduler.create('*/5 * * * *', AUTONOMOUS_SENTINEL_CRON, true);
        const dynamic = scheduler.create('*/5 * * * *', AUTONOMOUS_SENTINEL_DYNAMIC, true);
...
describe('runNonInteractive', () => {
    ...
    beforeEach(async () => {
        ...
        mockGeminiClient = {
            ...
            getChat: vi.fn(() => ({})),
            getHistoryFunctionResponseIds: vi.fn(() => new Set()),
        };
    ...
    function setupMetricsMock(overrides) {
        ...
        vi.mocked(uiTelemetryService.getMetricsForSession).mockReturnValue(mockMetrics);
    }
    ...
    async function prepareGoalState(preparation) {
        if (preparation === 'none')
            return;
    ...
    function mockFinishedGoalWorker() {
        vi.spyOn(goalRuntime, 'finishTurn').mockResolvedValue(undefined);
        mockGeminiClient.sendMessageStream.mockImplementation(() => createStreamFromEvents([
            {
    ...
    it.each(goalControlCases)('handles plain Goal $name before ordinary model input', async (testCase) => {
        setupMetricsMock();
        mockGetCommands.mockReturnValue([goalCommand]);
        await prepareGoalState(testCase.prepare);
        mockFinishedGoalWorker();
        const exitCode = await runNonInteractive(mockConfig, mockSettings, testCase.input, `goal-plain-${testCase.name}`);
        expect(exitCode).toBe(0);
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(testCase.expectedWorkers);
        expect(processStdoutSpy).toHaveBeenCalledWith(`${testCase.expectedText}\n`);
        expect(goalRuntime.getSnapshot()).toMatchObject({
            goal: testCase.expectedStatus === null
                ? null
                : {
    ...
    it.each([
    ...
    ])('answers $input from the degraded snapshot when goals cannot be persisted', async ({ input, expectedText }) => {
        setupMetricsMock();
        mockGetCommands.mockReturnValue([goalCommand]);
        ...
        const exitCode = await runNonInteractive(mockConfig, mockSettings, input, `goal-degraded-${input}`);
        expect(exitCode).toBe(0);
    ...
    it('still fails a Goal operation that genuinely needs persistence', async () => {
        ...
        const exitCode = await runNonInteractive(mockConfig, mockSettings, '/goal ship it', 'goal-degraded-set');
    ...
    it('runs resume with the exact permit scheduled by Core', async () => {
        ...
        mockFinishedGoalWorker();
        await runNonInteractive(mockConfig, mockSettings, '/goal resume', 'goal-resume-exact');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledOnce();
        const [parts, , , options] = mockGeminiClient.sendMessageStream.mock.calls[0];
        ...
        expect(options.goalTurnKey).toBe(`goal-runtime:${options.goalPermit.turnId}`);
    ...
    it('keeps the exact Goal permit through a ToolResult continuation', async () => {
        ...
        let requestCount = 0;
        mockGeminiClient.sendMessageStream.mockImplementation((_parts, _signal, _promptId, sendOptions) => {
            requestCount += 1;
            if (requestCount === 1) {
        ...
        await runNonInteractive(mockConfig, mockSettings, '/goal resume', 'goal-resume-tool-result');
    ...
    it('ends a Goal turn without another model call after update_goal', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockImplementation((_parts, _signal, _promptId, sendOptions) => createStreamFromEvents([
            {
                type: GeminiEventType.ToolCallRequest,
                value: {
                    callId: 'update-goal-terminal',
        ...
        const exitCode = await runNonInteractive(mockConfig, mockSettings, '/goal resume', 'goal-terminal-tool-result');
        expect(exitCode).toBe(0);
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledOnce();
    ...
    it('streams Goal state changes that happen while a headless turn settles', async () => {
        ...
        const exitCode = await runNonInteractive(mockConfig, mockSettings, '/goal resume', 'goal-state-stream');
        expect(exitCode).toBe(0);
        const statuses = processStdoutSpy.mock.calls
            .map(([chunk]) => JSON.parse(String(chunk)))
    ...
    it('flushes a plain-text Goal turn before releasing its permit', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, '/goal resume', 'goal-plain-text');
        expect(flush).toHaveBeenCalled();
        expect(finishTurn).toHaveBeenCalledOnce();
        expect(flush.mock.invocationCallOrder[0]).toBeLessThan(finishTurn.mock.invocationCallOrder[0]);
    });
    it('does not charge runtime Goal segments to the generic session turn cap', async () => {
        ...
        const exitCode = await runNonInteractive(mockConfig, mockSettings, '/goal resume', 'goal-unlimited-turns');
        expect(exitCode).toBe(0);
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(2);
    ...
    it('still applies the generic turn cap to real user input during a Goal', async () => {
        ...
        vi.mocked(mockConfig.getMaxSessionTurns).mockReturnValue(0);
        let goalStatusAtExit;
        vi.mocked(process.exit).mockImplementation((code) => {
            goalStatusAtExit = goalRuntime.getSnapshot().goal?.status;
            throw new Error(`process.exit(${code}) called`);
        });
        await expect(runNonInteractive(mockConfig, mockSettings, 'A real user update', 'goal-user-turn-cap')).rejects.toThrow('process.exit(53) called');
        expect(mockGeminiClient.sendMessageStream).not.toHaveBeenCalled();
    ...
    it('keeps explicit tool-call budgets on runtime Goal work', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValueOnce(createStreamFromEvents([
            {
                type: GeminiEventType.ToolCallRequest,
                value: {
        ...
        const run = runNonInteractive(mockConfig, mockSettings, '/goal resume', 'goal-explicit-budget').catch(() => undefined);
        await vi.waitFor(() => expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('Run aborted: tool-call budget of 0 exceeded (--max-tool-calls)')));
    ...
    it('interrupts Goal settlement when the explicit wall-clock budget expires', async () => {
        ...
        vi.spyOn(goalRuntime, 'finishTurn').mockImplementation(() => new Promise((resolve) => {
        ...
        const run = runNonInteractive(mockConfig, mockSettings, '/goal resume', 'goal-wall-budget', { abortController: runAbortController }).catch(() => undefined);
        await vi.waitFor(() => expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('Run aborted: wall-clock budget of 0.01s exceeded (--max-wall-time)')));
        expect(goalRuntime.getSnapshot()).toMatchObject({
            activity: 'idle',
            goal: { status: 'paused' },
        });
    ...
    it('claims an active Goal for real user input before binding the host', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'A real user update', 'goal-real-user');
        expect(beginTurn).toHaveBeenCalledWith('goal-real-user');
        expect(beginTurn.mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(mockConfig.bindGoalTurnHost).mock.invocationCallOrder[0]);
    ...
    it('waits for an active Goal turn before admitting real user input', async () => {
        ...
        mockFinishedGoalWorker();
        const beginTurn = vi.spyOn(goalRuntime, 'beginTurn');
        const run = runNonInteractive(mockConfig, mockSettings, 'A queued user update', 'goal-queued-user');
        await vi.waitFor(() => expect(beginTurn).toHaveBeenCalledWith('goal-queued-user'));
        expect(mockGeminiClient.sendMessageStream).not.toHaveBeenCalled();
    ...
    it('emits direct Goal v2 state before the legacy partial projection', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, '/goal', 'goal-direct-order');
    ...
    it('registers and clears the stream-json workflow approval channel', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockImplementation(async function* () {
            const callback = setApprovalRequestCallback.mock.calls[0]?.[0];
            expect(callback).toBeTypeOf('function');
            await callback?.({ runId: 'wf_stream' }, {
                approvalId: 'wfap_stream',
                name: 'run_shell_command',
            }, { command: 'echo safe' }, approvalSignal);
            yield {
                type: GeminiEventType.Finished,
                value: {
                    reason: undefined,
        ...
        expect(handleWorkflowApproval).toHaveBeenCalledWith('wf_stream', expect.objectContaining({ approvalId: 'wfap_stream' }), { command: 'echo safe' }, approvalSignal);
    ...
    it('prepends the recovered background agents notice on a resumed headless prompt', async () => {
        ...
        vi.mocked(mockConfig.consumePendingRecoveredAgentsNotice).mockReturnValue('Restored 2 background agents from the previous session.');
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents([
            {
                type: GeminiEventType.Finished,
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Continue the work', 'prompt-resume-notice');
        ...
        expect(request).toEqual([
            {
                text: expect.stringContaining('Restored 2 background agents from the previous session.'),
            },
            { text: 'Continue the work' },
        ]);
        expect(request[0].text).toContain(SYSTEM_REMINDER_OPEN);
    });
    it('does not consume the recovered-agents notice on an interrupted-turn continuation', async () => {
        ...
        expect(mockConfig.consumePendingRecoveredAgentsNotice).not.toHaveBeenCalled();
        const [request] = mockGeminiClient.sendMessageStream.mock.calls[0];
        expect(request).toEqual([
            {
                functionResponse: {
    ...
    it('does not let headless YOLO bypass explicit teammate approval', async () => {
        ...
        vi.mocked(mockConfig.getTeamManager).mockReturnValue(teamManager);
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Test input', 'prompt-exact-teammate');
        await vi.waitFor(() => expect(respond).toHaveBeenCalledWith(ToolConfirmationOutcome.Cancel));
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('requires an explicit interactive approval surface'));
        expect(processStderrSpy).not.toHaveBeenCalledWith(expect.stringContaining('exact-invocation'));
    });
    it('describes the active mode when explicit teammate approval cannot be shown', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Test input', 'prompt-exact-teammate-default');
        await vi.waitFor(() => expect(respond).toHaveBeenCalledWith(ToolConfirmationOutcome.Cancel));
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining(`current approval mode (${ApprovalMode.DEFAULT})`));
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('Use --input-format stream-json --output-format stream-json'));
        expect(processStderrSpy).not.toHaveBeenCalledWith(expect.stringContaining('cannot be bypassed by YOLO mode'));
    });
    describe('continueInterrupted', () => {
        it('re-submits an orphaned trailing user prompt with Retry semantics', async () => {
            ...
            expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'do the thing' }], expect.any(AbortSignal), 'prompt-c1', expect.objectContaining({ type: SendMessageType.Retry }));
        });
        it('adds plan mode reminders to an interrupted prompt replay', async () => {
            ...
            const [request, , , options] = mockGeminiClient.sendMessageStream.mock.calls[0];
            expect(options).toEqual(expect.objectContaining({ type: SendMessageType.Retry }));
            expect(request).toEqual([
        ...
        it('closes dangling tool calls with synthesized ToolResult parts', async () => {
            ...
            const [request, , , options] = mockGeminiClient.sendMessageStream.mock.calls[0];
            expect(options).toEqual(expect.objectContaining({ type: SendMessageType.ToolResult }));
            expect(request).toEqual([
                {
                    functionResponse: {
                        id: 'call-1',
        ...
        it('adds plan mode reminders to a continued tool result without moving function responses', async () => {
            ...
            await runNonInteractive(mockConfig, mockSettings, '', 'prompt-c-tool-plan', {
                continueInterrupted: true,
            });
            const [request, , , options] = mockGeminiClient.sendMessageStream.mock.calls[0];
            expect(options).toEqual(expect.objectContaining({ type: SendMessageType.ToolResult }));
    ...
    it('returns non-zero and skips pending tool calls after loop detection', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
        const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Use a tool', 'prompt-id-loop-detected');
        expect(exitCode).toBe(1);
        expect(mockCoreExecuteToolCall).not.toHaveBeenCalled();
        expect(processStdoutSpy).not.toHaveBeenCalled();
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('Loop detection halted the run'));
    });
    it('shows the always-on hint (not the skipLoopDetection escape) for a consecutive-identical halt', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
        const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Repeat a tool', 'prompt-id-consecutive-loop');
        expect(exitCode).toBe(1);
        ...
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('always-on guard and cannot be disabled via `model.skipLoopDetection`'));
        expect(processStderrSpy).not.toHaveBeenCalledWith(expect.stringContaining('Set the `model.skipLoopDetection` setting to true'));
    });
    it('shows the skipLoopDetection escape hint for a heuristic loop type', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
        const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Repeat a tool', 'prompt-id-heuristic-loop');
        expect(exitCode).toBe(1);
        ...
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('Set the `model.skipLoopDetection` setting to true'));
        expect(processStderrSpy).not.toHaveBeenCalledWith(expect.stringContaining('always-on guard'));
    });
    it('shows the maxToolCallsPerTurn hint when the per-turn cap halts the run', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
        const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Long turn', 'prompt-id-turn-cap');
        expect(exitCode).toBe(1);
        // The cap has its own knob, so the message must point at it rather than
        // claiming the halt cannot be configured away.
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('`model.maxToolCallsPerTurn`'));
        expect(processStderrSpy).not.toHaveBeenCalledWith(expect.stringContaining('cannot be disabled'));
        expect(processStderrSpy).not.toHaveBeenCalledWith(expect.stringContaining('Set the `model.skipLoopDetection` setting to true'));
    });
    it('marks JSON output as an error when loop detection halts the run', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
        const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Test input', 'prompt-id-loop-json');
        expect(exitCode).toBe(1);
        ...
        expect(resultMessage?.error?.message).toContain('Loop detection halted the run');
    });
    it('finalizes and reports recording failure before the JSON terminal result', async () => {
        ...
        mockConfig.onChatRecordingFailure = (listener) => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Test input', 'prompt-recording-failure');
        const output = processStdoutSpy.mock.calls.at(-1)?.[0];
        const messages = JSON.parse(output);
        const warningIndex = messages.findIndex((message) => message.subtype === 'session_recording_degraded');
    ...
    it('flushes but does not finalize when the caller owns the stream adapter', async () => {
        ...
        const flush = vi.fn().mockResolvedValue(undefined);
        mockConfig.getChatRecordingService = () => ({ finalize, flush });
        const adapter = new StreamJsonOutputAdapter(mockConfig, false);
        await runNonInteractive(mockConfig, mockSettings, 'Test input', 'prompt-caller-owned-adapter', { adapter });
    ...
    it('should handle a single tool call and respond', async () => {
        ...
        expect(mockGeminiClient.recordCompletedToolCall).toHaveBeenCalledWith('testTool', { arg1: 'value1' });
        // Verify consumePendingMemoryTaskPromises is called at the end of the session.
        expect(mockGeminiClient.consumePendingMemoryTaskPromises).toHaveBeenCalled();
    });
    it('uses a tool-selected full-turn model for the next request', async () => {
        setupMetricsMock();
        const model = 'qwen3-vl-plus\0';
        mockCoreExecuteToolCall.mockImplementation(async (_config, request, _signal, options) => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Use the screenshot tool', 'prompt-tool-image');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(2, [{ text: 'Tool response with image' }, { text: 'Skill response' }], expect.any(AbortSignal), 'prompt-tool-image', { type: SendMessageType.ToolResult, modelOverride: model });
    });
    it('rejects a conflicting tool-selected full-turn model within a batch', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(2, [{ text: 'first image' }, { text: 'second image' }], expect.any(AbortSignal), 'prompt-tool-image', { type: SendMessageType.ToolResult, modelOverride: first });
    });
    it('rejects a conflicting tool-selected full-turn model in a drain item', async () => {
        ...
        mockBackgroundTaskRegistry.setNotificationCallback.mockImplementation((cb) => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Do something', 'prompt-drain');
        expect(accepted['a']).toBe(true);
        expect(accepted['b']).toBe(false);
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(4, [{ text: 'drain image a' }, { text: 'drain image b' }], expect.any(AbortSignal), 'prompt-drain/automatic/3', { type: SendMessageType.ToolResult, modelOverride: first });
    });
    describe('parallel tool execution', () => {
        ...
        function toolCallEvents(ids, name, promptId) {
        ...
        it('isolates enter_plan_mode from headless siblings without charging skipped calls to the budget', async () => {
            ...
            mockGeminiClient.sendMessageStream
                .mockReturnValueOnce(createStreamFromEvents([
                ...toolCallEvents(['write-before-entry'], ToolNames.WRITE_FILE, 'p-plan-boundary'),
                ...toolCallEvents(['enter-plan'], ToolNames.ENTER_PLAN_MODE, 'p-plan-boundary'),
                ...toolCallEvents(['read-after-entry-1', 'read-after-entry-2'], ToolNames.READ_FILE, 'p-plan-boundary'),
            ]))
                .mockReturnValueOnce(createStreamFromEvents(finishTurn));
            await runNonInteractive(mockConfig, mockSettings, 'enter plan mode', 'p-plan-boundary');
            expect(mockCoreExecuteToolCall).toHaveBeenCalledOnce();
            ...
            expect(recordToolResult.mock.calls
                .map((call) => call[1])
                .filter((metadata) => metadata.callId !== 'enter-plan')
                .map((metadata) => metadata.executionStatus)).toEqual(['not_started', 'not_started', 'not_started']);
        });
        it('runs a batch of concurrency-safe tool calls concurrently', async () => {
            ...
            // parallel.
            mockCoreExecuteToolCall.mockImplementation(async (_config, req) => {
            ...
            mockGeminiClient.sendMessageStream
                .mockReturnValueOnce(createStreamFromEvents(toolCallEvents(['tool-1', 'tool-2', 'tool-3'], 'read', 'p-parallel')))
                .mockReturnValueOnce(createStreamFromEvents(finishTurn));
        ...
        it('finalizes concurrent results in request order despite out-of-order completion', async () => {
            ...
            const resolvers = {};
            mockCoreExecuteToolCall.mockImplementation((_config, req) => new Promise((resolve) => {
                resolvers[req.callId] = () => resolve({
                    responseParts: [
                        {
                            functionResponse: {
                                id: req.callId,
            ...
            mockGeminiClient.sendMessageStream
                .mockReturnValueOnce(createStreamFromEvents(toolCallEvents(['a', 'b', 'c'], 'read', 'p-order')))
                .mockReturnValueOnce(createStreamFromEvents(finishTurn));
            ...
            await vi.waitFor(() => expect(Object.keys(resolvers).sort()).toEqual(['a', 'b', 'c']));
            resolvers['c']();
            resolvers['a']();
            resolvers['b']();
            await run;
        ...
        it('hard-caps the aggregate headless tool response before the next model turn', async () => {
            ...
            mockGeminiClient.sendMessageStream
                .mockReturnValueOnce(createStreamFromEvents(toolCallEvents(['a', 'b'], 'read', 'p-cap')))
                .mockReturnValueOnce(createStreamFromEvents(finishTurn));
            ...
            expect(recordToolResult.mock.calls.flatMap((call) => call[0])).toEqual(nextTurnParts);
        });
        it('runs side-effecting (unsafe) tool calls sequentially', async () => {
            ...
            mockCoreExecuteToolCall.mockImplementation((_config, req) => new Promise((resolve) => {
                startOrder.push(req.callId);
                resolvers.push(() => resolve({ responseParts: [{ text: `resp-${req.callId}` }] }));
            }));
            mockGeminiClient.sendMessageStream
                .mockReturnValueOnce(createStreamFromEvents(toolCallEvents(['e1', 'e2'], 'edit', 'p-seq')))
                .mockReturnValueOnce(createStreamFromEvents(finishTurn));
        ...
        it('caps a parallel batch at exactly --max-tool-calls', async () => {
            ...
            mockGeminiClient.sendMessageStream.mockReturnValueOnce(createStreamFromEvents(toolCallEvents(['t1', 't2', 't3'], 'read', 'p-budget')));
            ...
            // takes).
            const run = runNonInteractive(mockConfig, mockSettings, 'go', 'p-budget').catch(() => undefined);
            await vi.waitFor(() => expect(mockCoreExecuteToolCall).toHaveBeenCalledTimes(2));
        ...
        it('partitions a mixed batch: parallel reads, sequential edit, parallel reads', async () => {
            ...
            mockCoreExecuteToolCall.mockImplementation((_config, req) => new Promise((resolve) => {
                startOrder.push(req.callId);
                resolvers[req.callId] = () => resolve({ responseParts: [{ text: `resp-${req.callId}` }] });
            }));
            mockGeminiClient.sendMessageStream
                .mockReturnValueOnce(createStreamFromEvents([
            ...
            await vi.waitFor(() => expect(startOrder).toEqual(['r1', 'r2', 'e1', 'r3', 'r4']));
            resolvers['r3']();
            resolvers['r4']();
            await run;
        ...
        it('throttles a parallel batch to QWEN_CODE_MAX_TOOL_CONCURRENCY in flight', async () => {
            ...
            try {
                ...
                mockGeminiClient.sendMessageStream
                    .mockReturnValueOnce(createStreamFromEvents(toolCallEvents(['c1', 'c2', 'c3', 'c4'], 'read', 'p-cap')))
                    .mockReturnValueOnce(createStreamFromEvents(finishTurn));
                ...
                resolvers['c2']();
                await vi.waitFor(() => expect(startOrder).toEqual(['c1', 'c2', 'c3', 'c4']));
                resolvers['c3']();
                resolvers['c4']();
                await run;
        ...
        it('canonicalizes legacy tool aliases so they partition like the interactive path', async () => {
            ...
            // them in parallel.
            vi.mocked(mockToolRegistry.getTool).mockImplementation((name) => (name === 'grep_search'
                ? { kind: Kind.Search }
                : undefined));
            const total = 2;
            const startOrder = [];
            ...
            mockGeminiClient.sendMessageStream
                .mockReturnValueOnce(createStreamFromEvents(toolCallEvents(['s1', 's2'], 'search_file_content', 'p-alias')))
                .mockReturnValueOnce(createStreamFromEvents(finishTurn));
    ...
    it('should ignore duplicate provider tool-call ids across rounds', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Use a tool', 'prompt-id-dup');
        ...
        expect(duplicateParts[0].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "tool-1"');
        expect(processStdoutSpy).toHaveBeenCalledWith('Final answer\n');
    });
    it('should stop repeated duplicate provider tool-call responses', async () => {
        ...
        mockGeminiClient.sendMessageStream
            .mockReturnValueOnce(createStreamFromEvents([toolCallEvent]))
            .mockReturnValueOnce(createStreamFromEvents([toolCallEvent]))
            .mockReturnValueOnce(createStreamFromEvents([toolCallEvent, freshToolCallEvent]));
        const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Use a tool', 'prompt-id-dup-loop');
        expect(exitCode).toBe(1);
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(3);
        expect(mockCoreExecuteToolCall).toHaveBeenCalledTimes(1);
        expect(mockGeminiClient.recordCompletedToolCall).toHaveBeenCalledTimes(1);
        const duplicateParts = mockGeminiClient.sendMessageStream.mock.calls[2][0];
        expect(duplicateParts[0].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "tool-1"');
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining(LoopType.GLOBAL_TOOL_CALL_DUPLICATE));
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('always-on guard and cannot be disabled via `model.skipLoopDetection`'));
        expect(processStderrSpy).not.toHaveBeenCalledWith(expect.stringContaining('Set the `model.skipLoopDetection` setting to true'));
    });
    it('should stop repeated duplicate provider tool-call responses from drain items', async () => {
        setupMetricsMock();
        mockGeminiClient.getHistoryFunctionResponseIds.mockReturnValue(new Set(['tool-drain']));
        ...
        mockGeminiClient.sendMessageStream
            ...
            .mockReturnValueOnce(createStreamFromEvents([duplicateToolCallEvent, freshToolCallEvent]));
        const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Watch the logs', 'prompt-id-drain-dup-loop');
        expect(exitCode).toBe(1);
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(3);
        expect(mockCoreExecuteToolCall).not.toHaveBeenCalled();
        const drainPromptIds = mockGeminiClient.sendMessageStream.mock.calls
            .slice(1)
            .map((call) => call[2]);
        expect(new Set(drainPromptIds)).toEqual(new Set(['prompt-id-drain-dup-loop/automatic/2']));
        const duplicateParts = mockGeminiClient.sendMessageStream.mock
            .calls[2][0];
        expect(duplicateParts[0].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "tool-drain"');
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining(LoopType.GLOBAL_TOOL_CALL_DUPLICATE));
    });
    it('should ignore duplicate provider tool-call ids already present in chat history', async () => {
        setupMetricsMock();
        mockGeminiClient.getHistoryFunctionResponseIds.mockReturnValue(new Set(['tool-history']));
        const toolCallEvent = {
            type: GeminiEventType.ToolCallRequest,
            value: {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Use a tool', 'prompt-id-history-dup');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(2);
        expect(mockCoreExecuteToolCall).not.toHaveBeenCalled();
        ...
        expect(duplicateParts[0].functionResponse?.id).toBe('tool-history__qwen_dup_2');
        expect(duplicateParts[0].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "tool-history"');
        expect(processStdoutSpy).toHaveBeenCalledWith('Final answer\n');
    });
    it('should execute only the first duplicate provider tool-call id in the same batch', async () => {
        ...
        mockGeminiClient.sendMessageStream
            .mockReturnValueOnce(createStreamFromEvents([firstToolCall, duplicateToolCall]))
            .mockReturnValueOnce(createStreamFromEvents([
            { type: GeminiEventType.Content, value: 'Final answer' },
            {
                type: GeminiEventType.Finished,
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Use a tool', 'prompt-id-same-batch-dup');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(2);
        expect(mockCoreExecuteToolCall).toHaveBeenCalledTimes(1);
        expect(mockGeminiClient.recordCompletedToolCall).toHaveBeenCalledTimes(1);
        ...
        expect(toolResultParts[1].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "tool-1"');
        expect(recordToolResult).toHaveBeenCalledTimes(2);
        expect(recordToolResult.mock.calls.map((call) => call[1].status)).toEqual([
            'success',
            'error',
        ]);
        expect(recordToolResult.mock.calls.map((call) => call[1].executionStatus)).toEqual(['success', 'not_started']);
        expect(processStdoutSpy).toHaveBeenCalledWith('Final answer\n');
    });
    ...
    it('keeps an agent-capable image route for the full headless tool chain', async () => {
        ...
        mockCoreExecuteToolCall.mockImplementation(async (_config, _request, _signal, options) => {
            ...
            rejectedDifferentSelector =
                options.onToolResultFullTurnModel?.('different-model\0https://vision.example/v1\0') === false;
            return {
                responseParts: [{ text: 'tool response' }],
        ...
        await runNonInteractive(mockConfig, mockSettings, 'inspect @image.png', 'prompt-vision-route');
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(1, headlessImageParts, expect.any(AbortSignal), 'prompt-vision-route', { type: SendMessageType.UserQuery, modelOverride: selector });
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(2, [{ text: 'tool response' }], expect.any(AbortSignal), 'prompt-vision-route', { type: SendMessageType.ToolResult, modelOverride: selector });
        expect(mockCoreExecuteToolCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({ callId: 'vision-tool-1' }), expect.any(AbortSignal), expect.objectContaining({ runtimeView }));
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('Routing this image turn to vision-agent'));
    });
    it('does not leak a headless image route into a notification drain', async () => {
        ...
        mockBackgroundTaskRegistry.setNotificationCallback.mockImplementation((callback) => {
        ...
        mockGeminiClient.sendMessageStream
            ...
            .mockReturnValueOnce(createStreamFromEvents(finishedEvents));
        await runNonInteractive(mockConfig, mockSettings, 'inspect @image.png', 'prompt-drain-isolation');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(2, [{ text: 'task result' }], expect.any(AbortSignal), 'prompt-drain-isolation/automatic/2', expect.objectContaining({
            type: SendMessageType.Notification,
            modelOverride: undefined,
        }));
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(3, [{ text: 'drain tool response' }], expect.any(AbortSignal), 'prompt-drain-isolation/automatic/2', { type: SendMessageType.ToolResult, modelOverride: undefined });
        expect(mockCoreExecuteToolCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({ callId: 'drain-tool-1' }), expect.any(AbortSignal), expect.objectContaining({ runtimeView: undefined }));
    });
    it('converts headless images through a non-agent vision bridge', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(finishedEvents));
        await runNonInteractive(mockConfig, mockSettings, 'inspect @image.png', 'prompt-vision-bridge');
        expect(runVisionBridgeSpy).toHaveBeenCalledWith({
            config: mockConfig,
            parts: headlessImageParts,
            signal: expect.any(AbortSignal),
        });
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'machine transcription' }], expect.any(AbortSignal), 'prompt-vision-bridge', { type: SendMessageType.UserQuery });
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('Converted 1 image(s)'));
    });
    it('emits a stream-json system message for headless vision bridge notices', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'inspect @image.png', 'prompt-vision-bridge-json');
        const systemMessage = writes
            .join('')
            .split('\n')
            .filter(Boolean)
            .map((line) => JSON.parse(line))
            .find((message) => message.type === 'system' && message.subtype === 'vision_bridge');
    ...
    it('emits a notice when a non-agent vision bridge fails', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(finishedEvents));
        await runNonInteractive(mockConfig, mockSettings, 'inspect @image.png', 'prompt-vision-bridge-failed');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'inspect this image' }], expect.any(AbortSignal), 'prompt-vision-bridge-failed', { type: SendMessageType.UserQuery });
        expect(processStderrSpy).toHaveBeenCalledWith('Vision bridge failed; proceeding without the image(s).\n');
    });
    it('strips headless images when a non-agent vision bridge is skipped', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(finishedEvents));
        await runNonInteractive(mockConfig, mockSettings, 'inspect @image.png', 'prompt-vision-bridge-skipped');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'inspect this image' }], expect.any(AbortSignal), 'prompt-vision-bridge-skipped', { type: SendMessageType.UserQuery });
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('Vision bridge cancelled.'));
        expect(processStderrSpy).toHaveBeenCalledWith(expect.stringContaining('were sent to vision-bridge'));
    });
    it('does not select a headless image route after clamping removes the image', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(finishedEvents));
        try {
            await runNonInteractive(mockConfig, mockSettings, 'inspect @image.png', 'prompt-oversized-image');
        }
        finally {
            vi.unstubAllEnvs();
        }
        ...
        ], expect.any(AbortSignal), 'prompt-oversized-image', { type: SendMessageType.UserQuery });
    });
    it('fails closed when the headless image route cannot be resolved', async () => {
        ...
        await expect(runNonInteractive(mockConfig, mockSettings, 'inspect @image.png', 'prompt-route-failure')).rejects.toThrow('route unavailable');
    ...
    it('returns from a recoverably interrupted stream-json turn without exiting', async () => {
        ...
        const exitCode = await runNonInteractive(mockConfig, mockSettings, 'interrupt me', 'prompt-recoverable-interrupt', {
    ...
    it('emits the effective fork context mode in headless task events', async () => {
        ...
        mockBackgroundTaskRegistry.setRegisterCallback.mockImplementation((callback) => {
            callback?.({
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Launch a context-inheriting fork', 'prompt-headless-fork');
        const envelopes = writes
            .join('')
            .split('\n')
            .filter((line) => line.trim().length > 0)
    ...
    it('flushes terminal monitor notifications before the final headless result', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(2);
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(2, [{ text: notificationXml }], expect.any(AbortSignal), 'prompt-monitor/automatic/2', {
            type: SendMessageType.Notification,
            modelOverride: undefined,
    ...
    it('keeps notifications from different Todo work chains in separate batches', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Watch the logs', 'prompt-monitor-work-chains');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(3);
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(2, [{ text: firstNotificationXml }], expect.any(AbortSignal), 'prompt-monitor-work-chains/automatic/2', expect.objectContaining({
            todoWorkChainId: 'chain-1',
        }));
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(3, [{ text: secondNotificationXml }], expect.any(AbortSignal), 'prompt-monitor-work-chains/automatic/3', expect.objectContaining({
            todoWorkChainId: 'chain-2',
        }));
    });
    ...
    it('drops a queued running monitor event after cancellation', async () => {
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Start then stop a monitor', 'prompt-monitor-cancel');
        ...
        const envelopes = writes
            ...
            .filter((line) => line.trim().length > 0)
            .map((line) => JSON.parse(line));
        expect(envelopes.some((env) => env.type === 'user' &&
            Array.isArray(env.message?.content) &&
            env.message.content.some((block) => typeof block === 'object' &&
                block !== null &&
                'text' in block &&
                block.text === 'Monitor "logs" event #1: ready'))).toBe(false);
        expect(envelopes.some((env) => env.type === 'system' &&
            env.subtype === 'task_notification' &&
            env.data?.task_id === 'mon_1')).toBe(false);
    });
    ...
    it('should execute only the first duplicate tool call id in stream-json format', async () => {
        ...
        mockGeminiClient.sendMessageStream
            .mockReturnValueOnce(createStreamFromEvents([duplicateToolCall, replayedToolCall]))
            .mockReturnValueOnce(createStreamFromEvents([
            { type: GeminiEventType.Content, value: 'done' },
            {
                type: GeminiEventType.Finished,
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Duplicate tool', 'prompt-id-dup');
        ...
        const toolResultParts = mockGeminiClient.sendMessageStream.mock.calls[1][0];
        expect(toolResultParts).toHaveLength(2);
        expect(toolResultParts[0].functionResponse?.response?.['output']).toBe('first');
        expect(toolResultParts[1].functionResponse?.id).toBe('dup_id_0001');
        expect(toolResultParts[1].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "dup_id_0001"');
        const envelopes = writes
            .join('')
            .split('\n')
            .filter((line) => line.trim().length > 0)
    ...
    it('should execute every tool call with an empty call id in stream-json format', async () => {
        ...
        mockGeminiClient.sendMessageStream
            .mockReturnValueOnce(createStreamFromEvents([firstToolCall, secondToolCall]))
            .mockReturnValueOnce(createStreamFromEvents([
            { type: GeminiEventType.Content, value: 'done' },
            {
                type: GeminiEventType.Finished,
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Empty id tools', 'prompt-id-empty');
        expect(mockCoreExecuteToolCall).toHaveBeenCalledTimes(2);
        expect(mockCoreExecuteToolCall).toHaveBeenNthCalledWith(1, mockConfig, expect.objectContaining({
            callId: '',
            args: { file_path: 'a.ts' },
        }), expect.any(AbortSignal), expect.any(Object));
        expect(mockCoreExecuteToolCall).toHaveBeenNthCalledWith(2, mockConfig, expect.objectContaining({
            callId: '',
            args: { file_path: 'b.ts' },
        }), expect.any(AbortSignal), expect.any(Object));
    });
    ...
    it('installs a skipDurableFire predicate that classifies loop.md sentinels in headless mode', async () => {
        ...
        expect(predicate({ prompt: LOOP_SENTINEL_CRON })).toBe(true);
        expect(predicate({ prompt: LOOP_SENTINEL_DYNAMIC })).toBe(true);
        expect(predicate({ prompt: AUTONOMOUS_SENTINEL_CRON })).toBe(true);
        expect(predicate({ prompt: AUTONOMOUS_SENTINEL_DYNAMIC })).toBe(true);
    ...
    describe('--json-schema structured output', () => {
        ...
        // event shape.
        const extractToolResultId = (event) => {
        ...
        it('stops executing remaining tool calls from the same turn once structured_output succeeds', async () => {
            mockConfig.getJsonSchema.mockReturnValue({
                type: 'object',
            ...
            mockConfig.getBackgroundTaskRegistry.mockReturnValue({
                setNotificationCallback: vi.fn(),
                setRegisterCallback: vi.fn(),
                getAll: vi.fn().mockReturnValue([]),
                hasUnfinalizedTasks: vi.fn().mockReturnValue(false),
            ...
            const writes = [];
            processStdoutSpy.mockImplementation((chunk) => {
                writes.push(typeof chunk === 'string'
                    ? chunk
            ...
            mockGeminiClient.sendMessageStream.mockReturnValueOnce(createStreamFromEvents([structuredCall, trailingCall]));
            await runNonInteractive(mockConfig, mockSettings, 'Emit structured output', 'prompt-id-structured');
            ...
            expect(mockCoreExecuteToolCall).toHaveBeenCalledTimes(1);
            const firstCallArg = mockCoreExecuteToolCall.mock.calls[0][1];
            ...
            const result = events.find((m) => typeof m === 'object' &&
                m !== null &&
                m.type === 'result');
            ...
            const trailingToolResult = events.find((m) => extractToolResultId(m) === 'tool-trailing');
        ...
        it('skips side-effecting tool calls that precede structured_output in the same turn', async () => {
            ...
            mockGeminiClient.sendMessageStream.mockReturnValueOnce(createStreamFromEvents([leadingCall, structuredCall]));
            await runNonInteractive(mockConfig, mockSettings, 'Emit structured output', 'prompt-id-leading');
            ...
            expect(mockCoreExecuteToolCall).toHaveBeenCalledTimes(1);
            const onlyCallArg = mockCoreExecuteToolCall.mock.calls[0][1];
            ...
            const leadingToolResult = events.find((m) => {
                if (extractToolResultId(m) !== 'tool-structured') {
                    return false;
                }
                const content = m?.message?.content?.[0]?.content;
            ...
            // run already exited).
            const leadingContent = leadingToolResult?.message?.content?.[0]?.content;
        ...
        it('tries multiple structured_output calls in the same turn until one succeeds', async () => {
            ...
            mockGeminiClient.sendMessageStream.mockReturnValueOnce(createStreamFromEvents([badStructured, goodStructured]));
            ...
            await runNonInteractive(mockConfig, mockSettings, 'Emit structured output', 'prompt-id-multi-struct');
            ...
            const executedNames = mockCoreExecuteToolCall.mock.calls.map((call) => call[1].name);
            const executedIds = mockCoreExecuteToolCall.mock.calls.map((call) => call[1].callId);
        ...
        it('keeps the session running when structured_output args fail validation so the model can retry', async () => {
            ...
            await runNonInteractive(mockConfig, mockSettings, 'Emit structured output', 'prompt-id-retry');
            // Both attempts must have been executed (no early termination on the
            // first call's error).
            expect(mockCoreExecuteToolCall).toHaveBeenCalledTimes(2);
            const firstName = mockCoreExecuteToolCall.mock.calls[0][1].name;
            const secondName = mockCoreExecuteToolCall.mock.calls[1][1].name;
        ...
        it('errors with non-zero exit when model emits plain text instead of structured_output', async () => {
            ...
            mockGeminiClient.sendMessageStream.mockReturnValueOnce(createStreamFromEvents(plainTextTurn));
            const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Should call structured_output', 'prompt-id-plaintext');
            expect(exitCode).toBe(1);
            const result = writes
                .join('')
                .split('\n')
        ...
        it('synthesises tool_result for suppressed sibling calls when structured_output fails validation', async () => {
            ...
            mockGeminiClient.sendMessageStream
                .mockReturnValueOnce(createStreamFromEvents([leadingCall, badStructuredCall]))
            ...
            await runNonInteractive(mockConfig, mockSettings, 'Emit structured output', 'prompt-id-suppress-pair');
            ...
            expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(2);
            const retryParts = mockGeminiClient.sendMessageStream.mock.calls[1][0];
            ...
            const suppressed = retryPartsTyped.find((p) => p.functionResponse?.id === 'tool-leading');
            ...
            const suppressedOutput = JSON.stringify(suppressed?.functionResponse?.response);
            ...
            const failedStructured = retryPartsTyped.find((p) => p.functionResponse?.id === 'tool-structured-bad');
            expect(failedStructured?.functionResponse?.name).toBe('structured_output');
            expect(JSON.stringify(failedStructured?.functionResponse?.response)).toContain('args invalid');
            expect(JSON.stringify(failedStructured?.functionResponse?.response)).not.toMatch(/Skipped:/);
        });
        it('keeps duplicate provider responses when structured_output fails validation', async () => {
            ...
            mockGeminiClient.sendMessageStream
                ...
                .mockReturnValueOnce(createStreamFromEvents([duplicateSideEffectCall, badStructuredCall]))
                .mockReturnValueOnce(createStreamFromEvents([goodStructuredCall]));
            mockCoreExecuteToolCall
                .mockResolvedValueOnce({
                responseParts: [
            ...
            await runNonInteractive(mockConfig, mockSettings, 'Emit structured output', 'prompt-id-dup-structured');
            const executedNames = mockCoreExecuteToolCall.mock.calls.map((call) => call[1].name);
            ...
            expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(3);
            const retryParts = mockGeminiClient.sendMessageStream.mock.calls[2][0];
            const retryPartsTyped = retryParts || [];
            const duplicateResponse = retryPartsTyped.find((part) => String(part.functionResponse?.response
                ?.error).includes('Duplicate provider tool call id "tool-side"'));
            const failedStructured = retryPartsTyped.find((part) => part.functionResponse?.id === 'tool-structured-bad');
            expect(duplicateResponse?.functionResponse?.id).toBe('tool-side');
            expect(duplicateResponse?.functionResponse?.name).toBe('side_effect_tool');
            expect(failedStructured?.functionResponse?.name).toBe('structured_output');
            expect(JSON.stringify(failedStructured?.functionResponse?.response)).toContain('args invalid');
        });
        it('captures structured_output emitted from a drain-turn (queued notification)', async () => {
            ...
            const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Watch the logs', 'prompt-drain-struct');
            ...
            expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(2);
            expect(mockCoreExecuteToolCall).toHaveBeenCalledTimes(1);
            const drainCallArg = mockCoreExecuteToolCall.mock.calls[0][1];
        ...
        it('holds back for in-flight background tasks before emitting structured success', async () => {
            ...
            mockConfig.getBackgroundTaskRegistry.mockReturnValue({
                ...
                abortAll: vi.fn(() => {
                    ...
                    notificationCallback?.('Agent cancelled: bg-task-1', 'Agent bg-task-1 was cancelled', {
                        agentId: 'bg-task-1',
                        toolUseId: 'tool-bg-1',
                        status: 'cancelled',
                    });
            ...
            mockGeminiClient.sendMessageStream.mockReturnValueOnce(createStreamFromEvents([structuredCall]));
            const startedAt = Date.now();
            const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Emit structured output', 'prompt-id-holdback');
            ...
            const resultIdx = events.findIndex((m) => typeof m === 'object' &&
                m !== null &&
                m.type === 'result');
            const taskNotificationIdx = events.findIndex((m) => typeof m === 'object' &&
                m !== null &&
                m.type === 'system' &&
        ...
        it('emits structuredResult to stdout in OutputFormat.TEXT mode', async () => {
            ...
            mockGeminiClient.sendMessageStream.mockReturnValueOnce(createStreamFromEvents([structuredCall]));
            const exitCode = await runNonInteractive(mockConfig, mockSettings, 'Emit structured output (text mode)', 'prompt-id-text');
            expect(exitCode).toBe(0);
    ...
    describe('--resume with active worktree (Phase C)', () => {
        it('injects a <system-reminder> block into the user prompt when sidecar names a live worktree', async () => {
            ...
            const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'wt-headless-resume-'));
            ...
            const worktreeDir = path.join(realTmpDir, '.qwen', 'worktrees', 'worktree-real');
            ...
            mockConfig.getResumedSessionData.mockReturnValue({
            ...
            mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
            try {
                await runNonInteractive(mockConfig, mockSettings, 'continue work', 'prompt-id-resume');
        ...
        it('does not inject anything when sidecar is absent', async () => {
            ...
            mockConfig.getResumedSessionData.mockReturnValue(undefined);
            setupMetricsMock();
            const events = [
                { type: GeminiEventType.Content, value: 'ok' },
                {
            ...
            mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
            await runNonInteractive(mockConfig, mockSettings, 'plain prompt', 'prompt-id-no-resume');
            const [parts] = mockGeminiClient.sendMessageStream.mock.calls[0];
        ...
        it('cleans up the sidecar when the worktree dir is gone (stale --resume)', async () => {
            const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'wt-headless-stale-'));
            const realTmpDir = await fs.realpath(tmpDir);
            ...
            mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents(events));
            try {
                await runNonInteractive(mockConfig, mockSettings, 'hello', 'prompt-id-stale');
