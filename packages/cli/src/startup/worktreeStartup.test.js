...
import { readWorktreeSessionMarker, SessionService, Storage, writeRuntimeStatus, writeWorktreeSessionMarker, } from '@qwen-code/qwen-code-core';
...
describe('setupStartupWorktree', () => {
    ...
    it('rejects full GitHub PR URLs when origin remote is missing', async () => {
        tempRepo = await makeTempRepo();
        process.chdir(tempRepo);
        const res = await setupStartupWorktree('https://github.com/QwenLM/qwen-code/pull/4174');
        expect(res).not.toBeNull();
        expect(res.ok).toBe(false);
        if (!res.ok) {
