...
const source = readFileSync(join(import.meta.dirname, 'AppContainer.tsx'), 'utf8');
/** Extract the dependency array of the controls-height measurement effect. */
function controlsHeightEffectDeps() {
...
describe('AppContainer controls-height measurement wiring', () => {
    ...
    it('computes liveAgentPanelLayoutKey from the live agent roster', () => {
        ...
        expect(source).toMatch(/liveAgentPanelLayoutKey\s*=\s*getLiveAgentPanelLayoutKey\(\s*bgTaskEntries\s*,\s*bgLivePanelFocused\s*,?\s*\)/);
    });
});
//# sourceMappingURL=app-container-controls-dep.test.js.map