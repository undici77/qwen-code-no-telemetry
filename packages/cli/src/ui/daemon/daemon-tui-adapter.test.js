...
import { createDaemonTuiReducerState, DaemonTuiAdapter, reduceDaemonEventToTuiUpdates, } from './daemon-tui-adapter.js';
...
describe('reduceDaemonEventToTuiUpdates', () => {
    ...
    it('maps assistant, tool, model, and disconnect daemon events while suppressing thought history', () => {
        expect(reduceDaemonEventToTuiUpdates({
            id: 0,
            v: 1,
            type: 'session_update',
            data: {
...
describe('DaemonTuiAdapter', () => {
    ...
    it('emits disconnected when the event stream ends or fails', async () => {
        ...
        await waitFor(() => expect(onFailingUpdate).toHaveBeenCalledWith({
            type: 'disconnected',
            reason: 'boom',
        }));
    ...
    it('reports unsupported daemon protocol versions once and advances replay state', async () => {
        ...
        const adapter = new DaemonTuiAdapter({ session, onUpdate });
        adapter.start();
        events.push({ id: 41, v: 2, type: 'future_event', data: {} });
        events.push({ id: 42, v: 2, type: 'future_event', data: {} });
        await waitFor(() => expect(adapter.lastEventId).toBe(42));
        const unsupportedUpdates = onUpdate.mock.calls.filter(([update]) => update.type === 'history' &&
    ...
    it('forwards prompt, cancel, model switch, and permission votes', async () => {
        ...
        expect(onUpdate).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'turn_complete' }));
        const blocks = [{ type: 'text', text: 'structured' }];
    ...
    it('reports prompt failures without fabricating turn completion', async () => {
        ...
        const adapter = new DaemonTuiAdapter({ session, onUpdate });
        adapter.start();
        await expect(adapter.sendPrompt('hello daemon')).rejects.toThrow('daemon down');
    ...
    it('requires a running event pump before sending control RPCs', async () => {
        ...
        await expect(adapter.sendPrompt('hello')).rejects.toThrow('Daemon TUI adapter is not running');
        await expect(adapter.cancel()).rejects.toThrow('Daemon TUI adapter is not running');
        events.close();
    });
    ...
    it('reports daemon control failures without dropping the event stream', async () => {
        ...
        cancelAdapter.start();
        cancelSession.cancel.mockRejectedValueOnce(new Error('\x1b[31mcancel down\x1b[0m'));
        ...
        expect(cancelUpdates).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'disconnected' }));
        ...
        await expect(modelAdapter.setModel('qwen3-coder-plus')).rejects.toThrow('model down');
        expect(modelUpdates).toHaveBeenCalledWith({
            type: 'history',
            item: { type: 'error', text: 'Daemon RPC failed: model down' },
        });
        expect(modelUpdates).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'disconnected' }));
        ...
        voteAdapter.start();
        voteSession.respondToPermission.mockRejectedValueOnce(new Error('vote down'));
        await expect(voteAdapter.approvePermission('req-1', 'proceed_once')).rejects.toThrow('vote down');
        expect(voteUpdates).toHaveBeenCalledWith({
            type: 'history',
            item: { type: 'error', text: 'Daemon RPC failed: vote down' },
        });
        expect(voteUpdates).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'disconnected' }));
        cancelEvents.close();
        modelEvents.close();
        voteEvents.close();
    });
});
//# sourceMappingURL=daemon-tui-adapter.test.js.map