...
const VimModeStateContext = createContext(undefined);
const VimModeActionsContext = createContext(undefined);
// ── Provider ──
export const VimModeProvider = ({ children, settings, }) => {
    ...
    const stateValue = useMemo(() => ({ vimEnabled, vimMode }), [vimEnabled, vimMode]);
    const actionsValue = useMemo(() => ({ toggleVimEnabled, setVimMode }), [toggleVimEnabled, setVimMode]);
    return (_jsx(VimModeActionsContext.Provider, { value: actionsValue, children: _jsx(VimModeStateContext.Provider, { value: stateValue, children: children }) }));
};
// ── Hooks ──
