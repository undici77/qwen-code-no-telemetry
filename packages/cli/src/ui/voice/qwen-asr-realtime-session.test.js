...
import { deriveQwenRealtimeUrl, openQwenAsrRealtimeStream, } from './qwen-asr-realtime-session.js';
class FakeSocket {
    OPEN = 1;
...
describe('qwen-asr-realtime-session', () => {
    ...
    it('derives the Qwen realtime endpoint from the provider host', () => {
        expect(deriveQwenRealtimeUrl('https://dashscope.aliyuncs.com/compatible-mode/v1', 'qwen3-asr-flash-realtime')).toBe('wss://dashscope.aliyuncs.com/api-ws/v1/realtime?model=qwen3-asr-flash-realtime');
        expect(deriveQwenRealtimeUrl('http://localhost:8080/dashscope/v1', 'qwen3-asr-flash-realtime')).toBe('ws://localhost:8080/dashscope/api-ws/v1/realtime?model=qwen3-asr-flash-realtime');
    });
    it('aborts an upstream connection while it is opening', async () => {
        ...
        controller.abort();
        await expect(sessionPromise).rejects.toThrow('Voice stream opening was aborted.');
    ...
    it('aborts an established upstream connection', async () => {
        ...
        socket.emit('message', JSON.stringify({ type: 'session.updated', event_id: 'updated' }), false);
        const session = await sessionPromise;
        controller.abort();
        expect(socket.readyState).toBe(3);
        await expect(session.finish()).rejects.toThrow('Voice stream opening was aborted.');
    });
    it('streams PCM chunks as base64 events and resolves the completed transcript', async () => {
        ...
        }, { onInterim: interim }, { createWebSocket, abortSignal: controller.signal });
        expect(createWebSocket).toHaveBeenCalledWith('wss://dashscope.example/api-ws/v1/realtime?model=qwen3-asr-flash-realtime', { headers: { Authorization: 'Bearer sk-test' } });
        socket.emit('message', JSON.stringify({ type: 'session.created', event_id: 'created' }), false);
        ...
        socket.emit('message', JSON.stringify({ type: 'session.finished', event_id: 'finished' }), false);
    ...
    it('rejects when the server finishes before the session is ready', async () => {
        ...
        socket.emit('message', JSON.stringify({ type: 'session.finished', event_id: 'finished' }), false);
        await expect(sessionPromise).rejects.toThrow('Qwen ASR realtime session finished before it was ready.');
        expect(socket.readyState).toBe(3);
    });
    it('rejects finish when the server never sends session.finished', async () => {
        ...
        await expect(transcriptPromise).rejects.toThrow('Qwen ASR realtime finish timed out.');
        expect(socket.readyState).toBe(3);
    });
    ...
    it('notifies transcription failures that arrive while recording', async () => {
        ...
        const sessionPromise = openQwenAsrRealtimeStream({
            baseUrl: 'https://dashscope.example/v1',
            model: 'qwen3-asr-flash-realtime',
        }, { onError }, { createWebSocket: () => socket });
        socket.emit('message', JSON.stringify({ type: 'session.updated', event_id: 'updated' }), false);
        const session = await sessionPromise;
        socket.emit('message', JSON.stringify({
            type: 'conversation.item.input_audio_transcription.failed',
        ...
        expect(onError).toHaveBeenCalledWith(new Error('server failed while recording'));
        expect(socket.readyState).toBe(3);
        await expect(session.finish()).rejects.toThrow('server failed while recording');
    });
    it('notifies and rejects finish when the realtime socket closed before finish', async () => {
        ...
        const session = await sessionPromise;
        socket.emit('close');
        expect(onError).toHaveBeenCalledWith(new Error('Qwen ASR realtime connection closed unexpectedly. Transcript may be incomplete.'));
        await expect(session.finish()).rejects.toThrow('Qwen ASR realtime connection closed unexpectedly.');
    });
    it('rejects committed transcript when the socket closes before session.finished', async () => {
        ...
        const transcriptPromise = session.finish();
        socket.emit('close');
        await expect(transcriptPromise).rejects.toThrow('Qwen ASR realtime connection closed unexpectedly.');
    });
    it('sanitizes and caps realtime server error messages', async () => {
        ...
        await expect(transcriptPromise).rejects.toThrow('bad\\u001b[8mhidden\\u001b[0m');
        await expect(transcriptPromise).rejects.not.toThrow('x'.repeat(220));
    });
});
//# sourceMappingURL=qwen-asr-realtime-session.test.js.map