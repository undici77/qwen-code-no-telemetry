...
function startSession(socket, callbacks = {}) {
    const sessionPromise = openVoiceStream({
        baseUrl: 'https://dashscope.example/v1',
        model: 'paraformer-realtime-v2',
    }, callbacks, { createWebSocket: () => socket });
    socket.emit('open');
    socket.emit('message', JSON.stringify({ header: { event: 'task-started' } }), false);
...
describe('voice-stream-session', () => {
    ...
    it('derives websocket URLs from https/http base URLs with path prefixes', () => {
        expect(deriveStreamUrl('https://dashscope.aliyuncs.com/compatible-mode/v1')).toBe('wss://dashscope.aliyuncs.com/api-ws/v1/inference');
        expect(deriveStreamUrl('http://localhost:8080/dashscope/v1')).toBe('ws://localhost:8080/dashscope/api-ws/v1/inference');
    });
    it('rejects finish when the task stream closes unexpectedly', async () => {
        ...
        socket.emit('close');
        await expect(transcriptPromise).rejects.toThrow('Voice stream connection closed unexpectedly.');
    });
    it('notifies immediately when the task stream closes while recording', async () => {
        ...
        expect(onError).toHaveBeenCalledWith(expect.objectContaining({
            message: 'Voice stream connection closed unexpectedly. Transcript may be incomplete.',
        }));
    });
    ...
    it('rejects when task-finished arrives before task-started', async () => {
        ...
        socket.emit('open');
        socket.emit('message', JSON.stringify({ header: { event: 'task-finished' } }), false);
        await expect(sessionPromise).rejects.toThrow('Voice stream finished before it started.');
    });
    ...
    it('rejects finish when the task never finishes', async () => {
        ...
        await expect(transcriptPromise).rejects.toThrow('Voice stream finish timed out.');
        expect(socket.readyState).toBe(3);
    });
    it('rejects finish when the server sends task-failed', async () => {
        ...
        await expect(transcriptPromise).rejects.toThrow('wss://dashscope.example/api-ws/v1/inference');
    });
...
//# sourceMappingURL=voice-stream-session.test.js.map