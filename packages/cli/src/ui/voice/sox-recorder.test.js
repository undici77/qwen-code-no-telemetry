...
describe('createSoxRecorder', () => {
    ...
    it('rejects promptly when sox closed before stop is requested', async () => {
        ...
        const stopResult = Promise.race([
            recorder.stop().then(() => 'resolved', (error) => error instanceof Error ? `rejected:${error.message}` : 'rejected'),
    ...
    it('includes sox stderr when recording fails', async () => {
        ...
        child.emit('close', 2);
        await expect(recorder.stop()).rejects.toThrow('Voice recorder failed with exit code 2: permission denied.');
    });
    it('caps captured sox stderr used in failure messages', async () => {
        ...
        child.emit('close', 2);
        await expect(recorder.stop()).rejects.toThrow(`Voice recorder failed with exit code 2: ${'x'.repeat(4096)}.`);
    });
    it('explains how to fix a missing sox executable', async () => {
        ...
        const recorder = createSoxRecorder();
        const startPromise = Promise.resolve(recorder.start());
        const startResult = startPromise.then(() => 'resolved', (error) => error instanceof Error ? `rejected:${error.message}` : 'rejected');
        await Promise.resolve();
        const error = Object.assign(new Error('spawn sox ENOENT'), {
            code: 'ENOENT',
        });
        child.emit('error', error);
        await expect(startResult).resolves.toBe('rejected:SoX is not installed or not on PATH. Install SoX and try again.');
        expect(mocks.rm).toHaveBeenCalledWith('/tmp/qwen-voice-abc', {
            recursive: true,
            force: true,
        });
...
//# sourceMappingURL=sox-recorder.test.js.map