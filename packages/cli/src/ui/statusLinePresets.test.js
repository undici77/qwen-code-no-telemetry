...
import { aggregateModelTokens, buildStatusLinePresetData, buildStatusLinePresetLines, DEFAULT_STATUS_LINE_PRESET_CONFIG, formatModelWithReasoning, formatTokenCount, getRunStateLabel, inferPullRequestNumber, normalizeStatusLinePresetConfig, orderStatusLinePresetItems, STATUS_LINE_PRESET_ITEM_IDS, STATUS_LINE_PRESET_ITEMS, } from './statusLinePresets.js';
describe('statusLinePresets', () => {
    it('normalizes valid preset configs and orders items by priority', () => {
        expect(normalizeStatusLinePresetConfig({
            type: 'preset',
    ...
    it('keeps default preset items in priority order', () => {
        expect(DEFAULT_STATUS_LINE_PRESET_CONFIG.items).toEqual(orderStatusLinePresetItems([...DEFAULT_STATUS_LINE_PRESET_CONFIG.items].reverse()));
    });
    it('orders preset items directly', () => {
        ...
        expect(orderStatusLinePresetItems([
            'run-state',
            'model',
            'git-branch',
            'model',
    ...
    it('formats model reasoning directly', () => {
        expect(formatModelWithReasoning('qwen3-code-plus', false)).toBe('qwen3-code-plus reasoning off');
        expect(formatModelWithReasoning('qwen3-code-plus', { effort: 'high' })).toBe('qwen3-code-plus high');
        expect(formatModelWithReasoning('qwen3-code-plus', { effort: undefined })).toBe('qwen3-code-plus');
        expect(formatModelWithReasoning('qwen3-code-plus', undefined)).toBe('qwen3-code-plus');
    });
    it('labels the plain model preset as model-only', () => {
        expect(STATUS_LINE_PRESET_ITEMS.find((item) => item.id === 'model')?.label).toBe('model-only');
    });
    it('renders available preset items in priority order', () => {
        ...
        expect(buildStatusLinePresetLines({
            type: 'preset',
            items: [
                'run-state',
                'model',
    ...
    it('renders model and model-with-reasoning together', () => {
        ...
        }, data)).toEqual(['qwen3-code-plus high · qwen3-code-plus']);
    });
    it('shows when reasoning is disabled', () => {
        ...
        }, data)).toEqual(['qwen3-code-plus reasoning off']);
    });
    ...
    it('labels run states', () => {
        ...
        expect(getRunStateLabel(StreamingState.WaitingForConfirmation)).toBe('Confirm');
    });
...
//# sourceMappingURL=statusLinePresets.test.js.map