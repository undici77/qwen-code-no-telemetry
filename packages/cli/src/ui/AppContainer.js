...
export const AppContainer = (props) => {
    ...
    const { handleSlashCommand, slashCommands, recentSlashCommands, pendingHistoryItems: pendingSlashCommandHistoryItems, btwItem, setBtwItem, cancelBtw, commandContext, shellConfirmationRequest, confirmationRequest, reloadCommands, } = useSlashCommandProcessor(config, settings, historyManager.history, historyManager.addItem, historyManager.clearItems, historyManager.loadHistory, refreshStatic, toggleVimEnabled, isProcessing, setIsProcessing, isIdleRef, setGeminiMdFileCount, slashCommandActions, extensionsUpdateStateInternal, isConfigInitialized, logger, historyManager.updateItem, setSessionName, extensionRefreshState);
    ...
    useEffect(() => {
        ...
        registry.setNotificationCallback((entry) => {
            ...
            sendNotification({ message: `Workflow '${name}' ${verb}`, title: 'Qwen Code' }, terminal, workflowBellEnabled);
        });
