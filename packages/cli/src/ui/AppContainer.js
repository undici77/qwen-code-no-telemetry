...
import { IdeClient, ideContextStore, createDebugLogger, getErrorMessage, getAllGeminiMdFilenames, ShellExecutionService, Storage, SessionEndReason, generatePromptSuggestion, logPromptSuggestion, PromptSuggestionEvent, logSpeculation, SpeculationEvent, startSpeculation, acceptSpeculation, abortSpeculation, IDLE_SPECULATION, ApprovalMode, ConditionalRulesRegistry, MCPDiscoveryState, ToolConfirmationOutcome, ToolNames, clearWorktreeSession, restoreWorktreeContext, GitWorktreeService, readWorktreeSessionMarker, } from '@qwen-code/qwen-code-core';
...
import { findLastUserItemIndex, isSyntheticHistoryItem, itemsAfterAreOnlySynthetic, } from './utils/historyUtils.js';
...
export const AppContainer = (props) => {
    ...
    const { handleSlashCommand, slashCommands, recentSlashCommands, pendingHistoryItems: pendingSlashCommandHistoryItems, btwItem, setBtwItem, cancelBtw, commandContext, shellConfirmationRequest, confirmationRequest, } = useSlashCommandProcessor(config, settings, historyManager.addItem, historyManager.clearItems, historyManager.loadHistory, remountStaticHistory, toggleVimEnabled, isProcessing, setIsProcessing, isIdleRef, setGeminiMdFileCount, slashCommandActions, extensionsUpdateStateInternal, isConfigInitialized, logger, setSessionName);
    ...
    const { messageQueue, addMessage, popAllMessages, drainQueue, popNextSegment, } = useMessageQueue();
    ...
    cancelHandlerRef.current = useCallback((info) => {
        ...
        if (info?.turnProducedMeaningfulContent) {
            debugLogger.debug('auto-restore bail: turn produced meaningful content during stream/flush');
            return;
        }
        ...
        if (!itemsAfterAreOnlySynthetic(history, lastUserIdx)) {
            debugLogger.debug('auto-restore bail: meaningful content committed after last user item');
            return;
        }
        ...
        try {
            ...
            if (needsConversation) {
                if (!geminiClient) {
                    ...
                    apiTruncateIndex = computeApiTruncationIndex(historyManager.history, userItem.id, geminiClient.getHistory());
    ...
    const handleGlobalKeypress = useCallback((key) => {
        ...
        if (handleRenderModeToggleKey(key, setRenderMode)) {
            ...
            const executingShell = pendingToolCallsRef.current.find((tc) => tc.status === 'executing' &&
            ...
            debugLogger.debug(`Ctrl+B promote: no executing shell tool call; falling through ` +
    ...
    return (_jsx(UIStateContext.Provider, { value: uiState, children: _jsx(UIActionsContext.Provider, { value: uiActions, children: _jsx(ConfigContext.Provider, { value: config, children: _jsx(AppContext.Provider, { value: {
                    ...
                    }, children: _jsx(CompactModeProvider, { value: compactModeValue, children: _jsx(RenderModeProvider, { value: renderModeValue, children: _jsx(TerminalOutputProvider, { value: writeRaw, children: _jsx(ShellFocusContext.Provider, { value: isFocused, children: _jsx(App, {}) }) }) }) }) }) }) }) }));
};
