...
export const DEFAULT_STATUS_LINE_PRESET_CONFIG = {
    type: 'preset',
    useThemeColors: true,
    items: STATUS_LINE_PRESET_ITEMS.filter((item) => item.defaultSelected).map((item) => item.id),
};
export function normalizeStatusLinePresetConfig(raw) {
    ...
    const rawItems = hasItemsArray ? candidate['items'] : [];
    const items = hasItemsArray
        ? rawItems.filter((item) => typeof item === 'string' && STATUS_LINE_PRESET_ITEM_ID_SET.has(item))
        : [];
    return {
        type: 'preset',
