...
const cell = (value, fullWidth = false) => ({
    type: 'char',
    value,
    fullWidth,
    styles: [],
...
function setBoundary(frame, y, kind, joiner) {
    const row = frame.boundaries[y];
...
describe('getSelectedText', () => {
    it('extracts a partial single row', () => {
        ...
        expect(getSelectedText(frame, { sx: 0, sy: 0, ex: 4, ey: 0 })).toBe('hello');
    });
    it('joins across rows with the first/last partial', () => {
        const frame = frameFromLines(['hello', 'world']);
        expect(getSelectedText(frame, { sx: 2, sy: 0, ex: 2, ey: 1 })).toBe('llo\nwor');
    });
    ...
    it('preserves selectable source whitespace', () => {
        ...
        expect(getSelectedText(frame, { sx: 0, sy: 0, ex: 4, ey: 1 })).toBe('hi   \nbye');
    });
    it('skips non-selectable layout padding', () => {
        const frame = frameFromLines(['hi   ']);
        for (let x = 2; x < 5; x++) {
            frame.cells[0][x].selectable = false;
        }
    ...
    it('separates selectable runs split by a layout gap', () => {
        ...
        expect(getSelectedText(frame, { sx: 0, sy: 0, ex: 12, ey: 0 })).toBe('status 42%');
    });
    it('replaces a soft visual break with its source joiner', () => {
        const frame = frameFromLines(['hello', 'world']);
        setBoundary(frame, 0, 'soft', ' ');
        expect(getSelectedText(frame, { sx: 0, sy: 0, ex: 4, ey: 1 })).toBe('hello world');
    });
    ...
    it('does not duplicate whitespace across a separator carrier row', () => {
        ...
        const frame = frameFromLines(['hello', '   ', 'world']);
        for (let x = 0; x < 3; x++) {
            frame.cells[1][x].selectable = false;
        }
        setBoundary(frame, 0, 'soft', ' ');
        setBoundary(frame, 1, 'soft', '');
        expect(getSelectedText(frame, { sx: 0, sy: 0, ex: 4, ey: 2 })).toBe('hello world');
    });
});
//# sourceMappingURL=selection-text.test.js.map