...
describe('useCommandCompletion', () => {
    ...
    describe('Completion mode detection', () => {
        ...
        it('should use slash completion for mid-input model-invocable commands', async () => {
            ...
            const { result } = renderHook(() => useCommandCompletion(useTextBufferForTest('please /store'), testRootDir, [skillCommand, builtInCommand, fileCommand], mockCommandContext, false, mockConfig));
            await waitFor(() => {
        ...
        it.each([
        ...
        ])('should complete a repeated skill in $input', async ({ input, expected }) => {
            const firstSkill = {
                name: 'review',
            ...
            const { result } = renderHook(() => {
                ...
                const completion = useCommandCompletion(textBuffer, testRootDir, [firstSkill, secondSkill, userOnlySkill, fileCommand], mockCommandContext, false, mockConfig);
                return { ...completion, textBuffer };
            });
            await waitFor(() => {
                expect(result.current.showSuggestions).toBe(true);
        ...
        it('should exclude hidden and non-user-invocable skills from stacked skill completion candidates', async () => {
            ...
            renderHook(() => useCommandCompletion(useTextBufferForTest('/review /sto'), testRootDir, [
                firstSkill,
                visibleSkill,
                hiddenSkill,
                nonUserInvocableSkill,
                fileCommand,
            ], mockCommandContext, false, mockConfig));
            await waitFor(() => {
                expect(useSlashCompletion).toHaveBeenLastCalledWith(expect.objectContaining({
                    enabled: true,
                    query: '/sto',
        ...
        it.each(['/stats /sto', '/unknown /sto', '/unknown\n/sto'])('should not treat an invalid stacked prefix as mid-input completion: %s', async (input) => {
            const skillCommand = {
                name: 'store-rules',
                description: 'Store rules',
                kind: CommandKind.SKILL,
            ...
            const { result } = renderHook(() => useCommandCompletion(useTextBufferForTest(input), testRootDir, [skillCommand, builtInCommand], mockCommandContext, false, mockConfig));
            expect(result.current.midInputGhostText).toBeNull();
            await waitFor(() => {
                if (input.includes('\n')) {
                    expect(useSlashCompletion).toHaveBeenLastCalledWith(expect.objectContaining({ enabled: false }));
                }
                else {
                    expect(useSlashCompletion).toHaveBeenLastCalledWith(expect.objectContaining({
                        enabled: true,
        ...
        it.each(['/tmp/foo.txt please /sto', '// note /sto'])('should treat non-command slash-led prefixes as regular mid-input completion: %s', async (input) => {
            const skillCommand = {
                name: 'store-rules',
                description: 'Store rules',
                kind: CommandKind.SKILL,
            ...
            const { result } = renderHook(() => useCommandCompletion(useTextBufferForTest(input), testRootDir, [skillCommand], mockCommandContext, false, mockConfig));
            await waitFor(() => {
                expect(result.current.showSuggestions).toBe(true);
            });
            expect(useSlashCompletion).toHaveBeenLastCalledWith(expect.objectContaining({
        ...
        it('should keep an indented first command in line-start completion', async () => {
            ...
            renderHook(() => useCommandCompletion(useTextBufferForTest('  /sto'), testRootDir, [skillCommand, builtInCommand], mockCommandContext, false, mockConfig));
            await waitFor(() => {
    ...
    describe('handleAutocomplete', () => {
        ...
        it('should complete the mid-input slash token at the cursor', async () => {
            ...
            const { result } = renderHook(() => {
                ...
                const completion = useCommandCompletion(textBuffer, testRootDir, [
                    {
                        name: 'front-end-store-rules',
                        description: 'Store rules',
                        kind: CommandKind.SKILL,
            ...
            expect(result.current.textBuffer.text).toBe('please /review /front-end-store-rules ');
        });
