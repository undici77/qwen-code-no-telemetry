 ...
 */
async function computeUniqueBranchTitle(baseName, sessionService) {
...
export function useBranchCommand(options) {
    ...
    const handleBranch = useCallback(async (name) => {
        ...
        try {
            ...
            try {
                restoreGoalFromHistory(uiHistoryItems, config, historyManager.addItem);
            }
            catch {
            ...
            const baseName = name ?? deriveFirstPrompt(resumed.conversation.messages);
