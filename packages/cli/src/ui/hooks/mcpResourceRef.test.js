...
describe('matchMcpServerPrefix', () => {
    it('matches a configured server name prefix and returns the remainder', () => {
        expect(matchMcpServerPrefix('demo:res://welcome', new Set(['demo']))).toEqual({ serverName: 'demo', rest: 'res://welcome' });
    });
    ...
    it('picks the LONGEST matching server name (handles colons in names)', () => {
        expect(matchMcpServerPrefix('my:server:res://x', new Set(['my', 'my:server']))).toEqual({ serverName: 'my:server', rest: 'res://x' });
    });
    ...
    it('never matches inherited object keys', () => {
        ...
        expect(matchMcpServerPrefix('__proto__:x', new Set(Object.keys({})))).toBeNull();
    });
});
describe('buildMcpResourceRef', () => {
    it('builds the canonical `<server>:<uri>` reference', () => {
        expect(buildMcpResourceRef('demo', 'file:///docs/spec.md')).toBe('demo:file:///docs/spec.md');
    });
...
//# sourceMappingURL=mcpResourceRef.test.js.map