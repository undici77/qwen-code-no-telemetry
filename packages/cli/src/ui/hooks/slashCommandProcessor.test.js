...
import { SlashCommandStatus, ToolConfirmationOutcome, makeFakeConfig, } from '@qwen-code/qwen-code-core';
...
describe('useSlashCommandProcessor', () => {
    ...
    const setupProcessorHook = (builtinCommands = [], fileCommands = [], mcpCommands = [], setIsProcessing = vi.fn(), settings = mockSettings) => {
        mockBuiltinLoadCommands.mockResolvedValue(Object.freeze(builtinCommands));
        mockFileLoadCommands.mockResolvedValue(Object.freeze(fileCommands));
        mockMcpLoadCommands.mockResolvedValue(Object.freeze(mcpCommands));
        const { result } = renderHook(() => useSlashCommandProcessor(mockConfig, settings, mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), // refreshStatic
    ...
    describe('Lifecycle', () => {
        ...
        it('should reload commands when SkillManager fires a change event', async () => {
            ...
            try {
                ...
                mockFileLoadCommands.mockResolvedValue([]);
                mockMcpLoadCommands.mockResolvedValue([]);
                const { unmount } = renderHook(() => useSlashCommandProcessor(mockConfig, mockSettings, mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), vi.fn(), false, vi.fn(), { current: true }, vi.fn(), createMockActions(), new Map(), true, null));
        ...
        it('should register SkillManager listener after config initialization', async () => {
            ...
            try {
                ...
                const { rerender, unmount } = renderHook(({ isConfigInitialized }) => {
                    ...
                    return useSlashCommandProcessor(mockConfig, mockSettings, mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), vi.fn(), false, vi.fn(), { current: true }, vi.fn(), createMockActions(), new Map(), isConfigInitialized, null);
        ...
        it('should not publish model-invocable commands from an aborted reload', async () => {
            ...
            mockFileLoadCommands.mockResolvedValue([]);
            mockMcpLoadCommands.mockResolvedValue([]);
            const { rerender } = renderHook(({ isConfigInitialized }) => useSlashCommandProcessor(mockConfig, mockSettings, mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), vi.fn(), false, vi.fn(), { current: true }, vi.fn(), createMockActions(), new Map(), isConfigInitialized, null), { initialProps: { isConfigInitialized: false } });
            rerender({ isConfigInitialized: true });
