...
import { SlashCommandStatus, ToolConfirmationOutcome, makeFakeConfig, MCPServerStatus, updateMCPServerStatus, recordSkillInvocation, } from '@qwen-code/qwen-code-core';
const { logSlashCommand, recordSkillInvocationMock, recordAutoSkillUsageMock, debugLoggerMock, } = vi.hoisted(() => ({
...
function createTestCommandPath(path) {
...
describe('useSlashCommandProcessor', () => {
    ...
    const setupProcessorHook = (builtinCommands = [], fileCommands = [], mcpCommands = [], setIsProcessing = vi.fn(), settings = mockSettings, extensionRefreshState, isIdleRef = { current: true }) => {
        mockBuiltinLoadCommands.mockResolvedValue(Object.freeze(builtinCommands));
        mockFileLoadCommands.mockResolvedValue(Object.freeze(fileCommands));
        mockMcpLoadCommands.mockResolvedValue(Object.freeze(mcpCommands));
        const { result } = renderHook(() => useSlashCommandProcessor(mockConfig, settings, [], // mock history array
        mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), // refreshStatic
        vi.fn(), // toggleVimEnabled
        false, // isProcessing
        setIsProcessing, isIdleRef, vi.fn(), // setGeminiMdFileCount
        createMockActions(), new Map(), // extensionsUpdateState
        true, // isConfigInitialized
        null, // logger
        mockUpdateItem, undefined, // setSessionName
    ...
    describe('Initialization and Command Loading', () => {
        ...
        it('rebuilds commands when an MCP server connects (surfaces MCP prompts in /)', async () => {
            ...
            await waitFor(() => {
                expect(vi.mocked(McpPromptLoader).mock.calls.length).toBeGreaterThan(before);
            }, { timeout: 2000 });
        });
    ...
    describe('Command Execution Logic', () => {
        ...
        it('should let slash-prefixed file paths fall through to the model', async () => {
            ...
            let actionResult;
            await act(async () => {
                actionResult = await result.current.handleSlashCommand('/api/apiFunction/接口的实现');
            });
            expect(actionResult).toBe(false);
            let absPathResult;
            await act(async () => {
                absPathResult = await result.current.handleSlashCommand('/Users/zhoushuo/Desktop/dw-operator-skill 帮我安装');
            });
        ...
        it('should display warning message command results as warnings', async () => {
            ...
            expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.WARNING, text: 'Check diagnostics.' }, expect.any(Number));
        });
        ...
        it('leaves a mid-turn Goal control response to the active stream', async () => {
            ...
            const result = setupProcessorHook([command], [], [], vi.fn(), mockSettings, undefined, { current: false });
            await waitFor(() => expect(result.current.slashCommands).toHaveLength(1));
            await act(async () => {
                await result.current.handleSlashCommand('/goal clear');
            });
            expect(mockAddItem).not.toHaveBeenCalledWith(expect.objectContaining({ type: MessageType.GOAL_STATE }), expect.any(Number));
        });
        ...
        it('renders a mid-turn causeless /goal clear with no active goal', async () => {
            ...
            expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.INFO, text: 'No Goal set.' }, expect.any(Number));
        });
    ...
    describe('Action Result Handling', () => {
        it.each([
        ...
        ])('handles %s without adding the invocation to TUI history', async (input, name, altNames, dialog) => {
            const command = createTestCommand({
                name,
            ...
            expect(mockAddItem).not.toHaveBeenCalled();
            expect(name === 'auth' ? mockOpenAuthDialog : mockOpenSettingsDialog).toHaveBeenCalledTimes(1);
            expect(mockConfig.getChatRecordingService()?.recordSlashCommand).toHaveBeenCalledWith({
                phase: 'invocation',
                rawCommand: input,
                sentToModel: false,
        ...
        it.each(['/help', '/?'])('opens %s without adding the invocation to TUI history', async (input) => {
            const command = createTestCommand({
                name: 'help',
                altNames: ['?'],
                action: vi.fn().mockResolvedValue({
        ...
        it.each(['/diff', '/editor', '/theme'])('hides the invocation for the %s panel', async (input) => {
        ...
        it.each(['/effort', '/model', '/stats', '/statusline'])('hides the invocation for the bare %s picker', async (input) => {
        ...
        it('reveals a picker-shaped invocation when argument validation fails', async () => {
            ...
            expect(mockAddItem).toHaveBeenNthCalledWith(1, { type: MessageType.USER, text: input, sentToModel: false }, expect.any(Number));
            expect(mockConfig.getChatRecordingService()?.recordSlashCommand).toHaveBeenCalledWith({
                phase: 'invocation',
                rawCommand: input,
                sentToModel: false,
        ...
        it('keeps the invocation for /theme when NO_COLOR blocks the dialog', async () => {
            ...
            try {
                ...
                expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.USER, text: '/theme', sentToModel: false }, expect.any(Number));
                expect(mockConfig.getChatRecordingService()?.recordSlashCommand).toHaveBeenCalledWith({
                    phase: 'invocation',
                    rawCommand: '/theme',
                    sentToModel: false,
        ...
        it.each([
        ...
        ])('keeps the invocation for the direct action %s', async (input, canonicalPath) => {
            ...
            expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.USER, text: input, sentToModel: false }, expect.any(Number));
        });
        it.each(['/status paths', '/stats model'])('keeps the invocation for the %s subcommand', async (input) => {
            const command = createTestCommandPath(input.slice(1));
            const result = setupProcessorHook([command]);
            await waitFor(() => expect(result.current.slashCommands).toHaveLength(1));
            await act(async () => {
        ...
        it('shows status output without adding the invocation to TUI history', async () => {
            ...
            expect(mockAddItem).toHaveBeenCalledTimes(1);
            expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.INFO, text: 'status output' }, expect.any(Number));
        });
        it('keeps the invocation for a file command that overrides status', async () => {
            ...
            expect(mockAddItem).toHaveBeenNthCalledWith(1, { type: MessageType.USER, text: '/status', sentToModel: false }, expect.any(Number));
            expect(mockAddItem).toHaveBeenNthCalledWith(2, { type: MessageType.INFO, text: 'custom status output' }, expect.any(Number));
        });
        ...
        it('awaits direct resume session switching before returning handled', async () => {
            ...
            let resolveResume;
            actions.handleResume = vi.fn(() => new Promise((resolve) => {
            ...
            const { result } = renderHook(() => useSlashCommandProcessor(mockConfig, mockSettings, [], mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), vi.fn(), false, vi.fn(), { current: true }, vi.fn(), actions, new Map(), true, null, mockUpdateItem));
            await waitFor(() => expect(result.current.slashCommands).toHaveLength(1));
            let settled = false;
        ...
        it('shows info feedback for collapse-on-resume command', async () => {
            const historyCmd = createTestCommand({
                ...
                action: vi.fn().mockResolvedValue({
                    type: 'message',
                    messageType: 'info',
                    content: 'History will be collapsed by default for future resumed sessions.',
                }),
            });
        ...
        it('should handle "submit_prompt" action returned from a file-based command', async () => {
            ...
            expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.USER, text: '/filecmd', sentToModel: false }, expect.any(Number));
            expect(mockFireUserPromptExpansionEvent).toHaveBeenCalledWith('filecmd', '', 'The actual prompt from the TOML file.', expect.any(AbortSignal));
            expect(mockUpdateItem).toHaveBeenCalledWith(1, { sentToModel: true });
            expect(debugLoggerMock.debug).toHaveBeenCalledWith('Marked slash command invocation as model-sent: /filecmd');
            const recorder = mockConfig.getChatRecordingService();
        ...
        it('should not submit a prompt cancelled while UserPromptExpansion hook is in flight', async () => {
            let resolveHook;
            mockFireUserPromptExpansionEvent.mockImplementation(() => new Promise((resolve) => {
                resolveHook = () => resolve({
                    getBlockingError: () => ({ blocked: false }),
                    shouldStopExecution: () => false,
            ...
            await waitFor(() => expect(mockFireUserPromptExpansionEvent).toHaveBeenCalled());
            act(() => {
            ...
            expect(logSlashCommand).not.toHaveBeenCalledWith(mockConfig, expect.objectContaining({
                command: 'filecmd',
        ...
        it('should handle "submit_prompt" action returned from a mcp-based command', async () => {
            ...
            expect(mockFireUserPromptExpansionEvent).toHaveBeenCalledWith('mcpcmd', '', 'The actual prompt from the mcp command.', expect.any(AbortSignal));
            expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.USER, text: '/mcpcmd', sentToModel: false }, expect.any(Number));
            expect(mockUpdateItem).toHaveBeenCalledWith(1, { sentToModel: true });
        });
        it('should fire UserPromptExpansion hooks for model-invocable command execution', async () => {
            ...
            expect(mockFireUserPromptExpansionEvent).toHaveBeenCalledWith('filecmd', 'with args', 'The actual prompt from the TOML file.', expect.any(AbortSignal));
            expect(content).toBe('The actual prompt from the TOML file.\n\nHook context');
        });
        ...
        it('should stop model-invocable command execution when hook unmounts', async () => {
            ...
            const { result, unmount } = renderHook(() => useSlashCommandProcessor(mockConfig, mockSettings, mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), vi.fn(), false, vi.fn(), { current: true }, vi.fn(), createMockActions(), new Map(), true, null, mockUpdateItem));
            await waitFor(() => expect(result.current.slashCommands).toHaveLength(1));
            const executor = mockConfig.getModelInvocableCommandsExecutor?.();
    ...
    describe('Shell Command Confirmation Flow', () => {
        ...
        it('should not duplicate user history when a confirmed command submits a prompt', async () => {
            ...
            await act(async () => {
                result.current.shellConfirmationRequest?.onConfirm(ToolConfirmationOutcome.ProceedOnce, ['rm -rf /']);
            });
            await waitFor(() => {
                expect(mockCommandAction).toHaveBeenCalledTimes(2);
            });
            const userInvocationCalls = mockAddItem.mock.calls.filter(([item]) => item.type === MessageType.USER && item.text === '/shellcmd');
        ...
        it('should not duplicate user history when a confirmed action submits a prompt', async () => {
            ...
            const userInvocationCalls = mockAddItem.mock.calls.filter(([item]) => item.type === MessageType.USER && item.text === '/actioncmd');
            expect(userInvocationCalls).toHaveLength(1);
            expect(mockUpdateItem).toHaveBeenCalledWith(1, { sentToModel: true });
            const recorder = mockConfig.getChatRecordingService();
            expect(recorder.recordSlashCommand).toHaveBeenCalledTimes(2);
    ...
    describe('Command Precedence', () => {
        ...
        it('should add an overridden command to the history', async () => {
            ...
            expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.USER, text: '/exit', sentToModel: false }, expect.any(Number));
        });
    });
    describe('Lifecycle', () => {
        it('should abort command loading when the hook unmounts', () => {
            const abortSpy = vi.spyOn(AbortController.prototype, 'abort');
            const { unmount } = renderHook(() => useSlashCommandProcessor(mockConfig, mockSettings, [], // mock history array
            mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), // refreshStatic
            vi.fn(), // toggleVimEnabled
            false, // isProcessing
        ...
        it('auto-refreshes extension content changes after debounce', async () => {
            ...
            const result = setupProcessorHook([], [], [], vi.fn(), mockSettings, extensionRefreshState);
            await waitFor(() => {
                expect(result.current.slashCommands).toBeDefined();
            });
            act(() => {
        ...
        it('shows an error when extension content auto-refresh fails', async () => {
            vi.mocked(refreshExtensionContentRuntime).mockRejectedValueOnce(new Error('refresh failed'));
            const extensionRefreshState = new ExtensionRefreshState();
            const result = setupProcessorHook([], [], [], vi.fn(), mockSettings, extensionRefreshState);
            await waitFor(() => {
                expect(result.current.slashCommands).toBeDefined();
        ...
        it('should reload commands when SkillManager fires a change event', async () => {
            ...
            const skillManagerSpy = vi
                ...
                .mockReturnValue(fakeSkillManager);
            try {
                mockBuiltinLoadCommands.mockResolvedValue([]);
                mockFileLoadCommands.mockResolvedValue([]);
                mockMcpLoadCommands.mockResolvedValue([]);
                const { unmount } = renderHook(() => useSlashCommandProcessor(mockConfig, mockSettings, [], // mock history array
                mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), vi.fn(), false, vi.fn(), { current: true }, vi.fn(), createMockActions(), new Map(), true, null, mockUpdateItem));
                ...
                const listener = addChangeListener.mock.calls[0][0];
                await act(async () => {
                    listener();
                });
                await waitFor(() => expect(BuiltinCommandLoader).toHaveBeenCalledTimes(2));
                unmount();
        ...
        it('should skip reload when consumeSlashReloadSuppression returns true', async () => {
            ...
            try {
                ...
                mockMcpLoadCommands.mockResolvedValue([]);
                const { unmount } = renderHook(() => useSlashCommandProcessor(mockConfig, mockSettings, [], // mock history array
                mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), vi.fn(), false, vi.fn(), { current: true }, vi.fn(), createMockActions(), new Map(), true, null));
                await waitFor(() => expect(addChangeListener).toHaveBeenCalledTimes(1));
                await waitFor(() => expect(BuiltinCommandLoader).toHaveBeenCalledTimes(1));
                const listener = addChangeListener.mock.calls[0][0];
                await act(async () => {
        ...
        it('should register SkillManager listener after config initialization', async () => {
            ...
            const skillManagerSpy = vi
                .spyOn(mockConfig, 'getSkillManager')
                .mockImplementation(() => initializedForConfig
                ? fakeSkillManager
                : null);
            try {
                mockBuiltinLoadCommands.mockResolvedValue([]);
                mockFileLoadCommands.mockResolvedValue([]);
                mockMcpLoadCommands.mockResolvedValue([]);
                const { rerender, unmount } = renderHook(({ isConfigInitialized }) => {
                    initializedForConfig = isConfigInitialized;
                    return useSlashCommandProcessor(mockConfig, mockSettings, [], // mock history array
                    mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), vi.fn(), false, vi.fn(), { current: true }, vi.fn(), createMockActions(), new Map(), isConfigInitialized, null, mockUpdateItem);
        ...
        it('should not publish model-invocable commands from an aborted reload', async () => {
            ...
            mockFileLoadCommands.mockResolvedValue([]);
            mockMcpLoadCommands.mockResolvedValue([]);
            const { rerender } = renderHook(({ isConfigInitialized }) => useSlashCommandProcessor(mockConfig, mockSettings, [], // mock history array
            mockAddItem, mockClearItems, mockLoadHistory, vi.fn(), vi.fn(), false, vi.fn(), { current: true }, vi.fn(), createMockActions(), new Map(), isConfigInitialized, null, mockUpdateItem), { initialProps: { isConfigInitialized: false } });
            rerender({ isConfigInitialized: true });
            await waitFor(() => expect(mockConfig
                .getModelInvocableCommandsProvider?.()?.()
                .map((c) => c.name)).toEqual(['fresh']));
            await act(async () => {
    ...
    describe('Slash Command Logging', () => {
        ...
        it('records successful skill slash commands when they submit a prompt', async () => {
            vi.spyOn(mockConfig, 'getProjectRoot').mockReturnValueOnce('/test/project');
    ...
    describe('Stacked Skill Invocations', () => {
        const createSkillCommand = (name, body) => createTestCommand({
            name,
        ...
        it('combines two stacked skills into a single submit_prompt', async () => {
            ...
            let actionResult;
            await act(async () => {
                actionResult = await result.current.handleSlashCommand('/feat-dev /e2e-testing implement X');
            });
            expect(actionResult).toEqual({
                type: 'submit_prompt',
        ...
        it('preserves context-file refresh intent from stacked skills', async () => {
            const skillA = createTestCommand({
                name: 'remember-skill',
            ...
            let actionResult;
            await act(async () => {
                actionResult = await result.current.handleSlashCommand('/remember-skill /e2e-testing implement X');
            });
            expect(actionResult).toEqual({
                type: 'submit_prompt',
                content: expect.arrayContaining([
        ...
        it('calls recordSkillInvocation for each stacked skill', async () => {
            ...
            const recordedNames = recordSkillInvocationMock.mock.calls.map((call) => call[1].skillName);
        ...
        it('appends remaining text after all skill bodies', async () => {
            ...
            let actionResult;
            await act(async () => {
                actionResult = await result.current.handleSlashCommand('/feat-dev /review please review the auth module');
            });
        ...
        it('shows a warning when stacked skills exceed the maximum', async () => {
            const skills = Array.from({ length: 6 }, (_, i) => createSkillCommand(`skill-${i}`, `body-${i}`));
            const result = setupProcessorHook(skills);
            await waitFor(() => expect(result.current.slashCommands).toHaveLength(6));
            await act(async () => {
                await result.current.handleSlashCommand('/skill-0 /skill-1 /skill-2 /skill-3 /skill-4 /skill-5 do it');
            });
            // Should have emitted a warning message
            const warningCalls = mockAddItem.mock.calls.filter((call) => call[0].type === MessageType.WARNING);
            expect(warningCalls.length).toBeGreaterThan(0);
            expect(warningCalls[0][0].text).toContain('Only the first 5');
        });
        it('handles stacked skills with no remaining text', async () => {
            ...
            let actionResult;
            await act(async () => {
                actionResult = await result.current.handleSlashCommand('/feat-dev /e2e-testing');
            });
            const content = actionResult
                .content;
            const texts = content.map((c) => c.text);
        ...
        it('skips skill commands whose action is undefined', async () => {
            ...
            let actionResult;
            await act(async () => {
                actionResult = await result.current.handleSlashCommand('/feat-dev /no-action-skill /review do it');
            });
        ...
        it('excludes non-submit_prompt results from combined content', async () => {
            ...
            let actionResult;
            await act(async () => {
                actionResult = await result.current.handleSlashCommand('/feat-dev /error-skill /review do it');
            });
            // Error message should be surfaced via addMessage
            const errorCalls = mockAddItem.mock.calls.filter((call) => call[0].type === MessageType.ERROR);
            expect(errorCalls.length).toBeGreaterThan(0);
            expect(errorCalls[0][0].text).toContain('Something went wrong');
        ...
        it('records telemetry success=false for non-submit_prompt skill results', async () => {
            ...
            const records = recordSkillInvocationMock.mock.calls.map((call) => call[1]);
        ...
        it('propagates modelOverride from first submit_prompt skill', async () => {
            ...
            let actionResult;
            await act(async () => {
                actionResult = await result.current.handleSlashCommand('/feat-dev /review do it');
            });
            expect(actionResult).toEqual({
                type: 'submit_prompt',
