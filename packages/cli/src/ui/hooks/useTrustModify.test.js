...
describe('useTrustModify', () => {
    ...
    it('should initialize with the correct trust level', () => {
        ...
        const { result } = renderHook(() => useTrustModify(mockOnExit, mockAddItem));
    ...
    it('should clear a superseded pending trust change when a later write fails', () => {
        ...
        mockedIsWorkspaceTrusted.mockImplementation((_settings, trustConfig) => ({
            isTrusted: trustConfig?.['/test/dir'] === TrustLevel.TRUST_FOLDER ? true : false,
            source: 'file',
        }));
        const { result } = renderHook(() => useTrustModify(mockOnExit, mockAddItem));
        act(() => {
...
//# sourceMappingURL=useTrustModify.test.js.map