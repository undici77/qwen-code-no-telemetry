...
let originalColumns;
let originalRows;
function setStdoutProp(name, value) {
    Object.defineProperty(process.stdout, name, { value, configurable: true });
}
function restoreStdoutProp(name, original) {
    if (original) {
...
describe('useTerminalSize', () => {
    ...
    it('attaches exactly one resize listener no matter how many hooks mount', () => {
        ...
        const hooks = Array.from({ length: 15 }, () => renderHook(() => useTerminalSize()));
        ...
        expect(emitWarning.mock.calls.some(([warning]) => warning instanceof Error &&
            warning.name === 'MaxListenersExceededWarning')).toBe(false);
...
//# sourceMappingURL=useTerminalSize.test.js.map