...
const vpWrapper = (useTerminalBuffer) => {
    const VpWrapper = ({ children }) => (_jsx(SettingsContext.Provider, { value: { merged: { ui: { useTerminalBuffer } } }, children: _jsx(KeypressProvider, { kittyProtocolEnabled: false, children: children }) }));
    return VpWrapper;
};
const virtualViewportWrapper = (virtualViewport, rawUseTerminalBuffer) => {
    const VpWrapper = ({ children }) => (_jsx(SettingsContext.Provider, { value: {
            merged: { ui: { useTerminalBuffer: rawUseTerminalBuffer } },
        }, children: _jsx(VirtualViewportContext.Provider, { value: virtualViewport, children: _jsx(KeypressProvider, { kittyProtocolEnabled: false, children: children }) }) }));
    return VpWrapper;
};
// Mechanism tests exercise enable/disable/ref-counting independent of the VP
// gate, so they opt out via bypassVpGate.
function useTwoMouseSubscribers(firstActive, secondActive) {
...
describe('useMouseEvents', () => {
    ...
    it('keeps terminal mouse mode enabled until all subscribers are inactive', () => {
        const { rerender, unmount } = renderHook(({ firstActive, secondActive }) => useTwoMouseSubscribers(firstActive, secondActive), {
    ...
    it('upgrades to ?1003h when a hover (any) subscriber is active, then restores ?1002h', () => {
        const { rerender, unmount } = renderHook(({ buttonActive, anyActive, }) => {
            useMouseEvents(() => { }, {
    ...
    it('enables ?1003h directly when the only active subscriber wants hover', () => {
        const { unmount } = renderHook(() => useMouseEvents(() => { }, {
            isActive: true,
            tracking: 'any',
            bypassVpGate: true,
        }), { wrapper });
    ...
    describe('VP gate', () => {
        ...
        it('bypassVpGate: enables mouse mode even in non-VP (modal / VP viewport)', () => {
            renderHook(() => useMouseEvents(() => { }, { isActive: true, bypassVpGate: true }), { wrapper: vpWrapper(false) });
            expect(stdout.write).toHaveBeenCalledWith(ENABLE_MOUSE);
        });
    });
    ...
    describe('mouseTracking setting', () => {
        const mouseTrackingWrapper = (mouseTracking) => {
            const Wrapper = ({ children }) => (_jsx(SettingsContext.Provider, { value: {
                    merged: { ui: { useTerminalBuffer: true, mouseTracking } },
                }, children: _jsx(KeypressProvider, { kittyProtocolEnabled: false, children: children }) }));
            return Wrapper;
        };
        it('ui.mouseTracking: false keeps mouse mode disabled', () => {
            renderHook(() => useMouseEvents(() => { }, { isActive: true, bypassVpGate: true }), { wrapper: mouseTrackingWrapper(false) });
            expect(stdout.write).not.toHaveBeenCalledWith(ENABLE_MOUSE);
        });
        it('ui.mouseTracking: true enables mouse mode', () => {
            renderHook(() => useMouseEvents(() => { }, { isActive: true, bypassVpGate: true }), { wrapper: mouseTrackingWrapper(true) });
            expect(stdout.write).toHaveBeenCalledWith(ENABLE_MOUSE);
        });
    });
});
//# sourceMappingURL=useMouseEvents.test.js.map