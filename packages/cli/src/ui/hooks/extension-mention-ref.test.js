...
import { parseExtensionRef, buildExtensionRef, matchExtensionByRef, getExtensionSuggestions, buildExtensionContextText, EXTENSION_REF_PREFIX, } from './extension-mention-ref.js';
...
describe('buildExtensionRef', () => {
    ...
    it('uses the EXTENSION_REF_PREFIX constant', () => {
        expect(buildExtensionRef('foo').startsWith(EXTENSION_REF_PREFIX)).toBe(true);
    });
});
...
describe('getExtensionSuggestions', () => {
    ...
    it('caps results at MAX_SUGGESTIONS_TO_SHOW', () => {
        const many = Array.from({ length: 12 }, (_, i) => makeExtension({ name: `ext-${String(i).padStart(2, '0')}` }));
        const config = {
...
describe('buildExtensionContextText', () => {
    it('produces a context block for a minimal extension', () => {
        ...
        expect(text).toContain('--- Extension: minimal (untrusted third-party content) ---');
...
//# sourceMappingURL=extension-mention-ref.test.js.map