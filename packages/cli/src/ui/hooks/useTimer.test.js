...
describe('useTimer', () => {
    ...
    it('should pause elapsedTime without resetting when isPaused is true', () => {
        const { result, rerender } = renderHook(({ isPaused }) => useTimer(true, 0, isPaused), { initialProps: { isPaused: false } });
        act(() => {
            vi.advanceTimersByTime(2000);
        });
        expect(result.current).toBe(2);
    ...
    it('should reset while paused when resetKey changes', () => {
        const { result, rerender } = renderHook(({ resetKey, isPaused }) => useTimer(true, resetKey, isPaused), { initialProps: { resetKey: 0, isPaused: false } });
        act(() => {
            vi.advanceTimersByTime(1000);
        });
        expect(result.current).toBe(1);
