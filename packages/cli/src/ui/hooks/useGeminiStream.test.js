...
import { ApprovalMode, AUTONOMOUS_SENTINEL_DYNAMIC, AuthType, GeminiEventType as ServerGeminiEventType, MessageSenderType, SendMessageType, ToolErrorType, ToolConfirmationOutcome, getRuntimeContentGenerator, runWithRuntimeContentGenerator, } from '@qwen-code/qwen-code-core';
import { MessageType, StreamingState, ToolCallStatus } from '../types.js';
import { findLastSafeSplitPoint } from '../utils/markdownUtilities.js';
import { MAX_INLINE_IMAGE_ENCODED_LENGTH, MAX_INLINE_IMAGES_PER_ITEM, } from '../utils/inline-image-parts.js';
// --- MOCKS ---
...
vi.mock('@qwen-code/qwen-code-core', async (importOriginal) => {
    ...
    mockFinalizeToolResponses.mockImplementation(actualCoreModule.finalizeToolResponses);
    return {
        ...actualCoreModule,
...
describe('useGeminiStream', () => {
    ...
    const renderTestHook = (initialToolCalls = [], geminiClient, availableTerminalHeightRef, onCancelSubmit = () => { }, logger, goalQueueRef) => {
        ...
        const { result, rerender } = renderHook((props) => {
            ...
            return useGeminiStream(props.client, props.history, props.addItem, props.config, true, props.loadedSettings, props.onDebugMessage, props.handleSlashCommand, props.shellModeActive, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, onCancelSubmit, () => { }, 80, 24, undefined, // midTurnDrainRef
            logger, availableTerminalHeightRef, undefined, // terminalWidthRef
    ...
    it('sends a hidden Goal turn without user admission side effects', async () => {
        ...
        const peekNextUserBatchKey = vi.fn((goalTurnActive) => goalTurnActive ? undefined : 'message-queue:next-user');
        const { result, mockSendMessageStream: streamMock } = renderTestHook([], undefined, undefined, undefined, undefined, {
            current: { peekNextUserBatchKey },
        });
        await act(async () => {
            await result.current.submitQuery(goal.continuationContext, SendMessageType.Goal, 'prompt-id-goal', { goal });
        });
        ...
        ].join('\n'), expect.any(AbortSignal), 'prompt-id-goal', expect.objectContaining({
        ...
        const options = streamMock.mock.calls[0][3];
        ...
        expect(mockAddItem).not.toHaveBeenCalledWith(expect.objectContaining({ type: MessageType.USER }), expect.any(Number));
    ...
    it('does not copy the objective into a synthetic Goal turn', async () => {
        ...
        await act(async () => {
            await result.current.submitQuery(goal.continuationContext, SendMessageType.Goal, 'prompt-id-goal-stop-token', { goal });
        });
    ...
    it('claims a Goal only after direct user input becomes model-facing', async () => {
        ...
        expect(streamMock).toHaveBeenCalledWith('user goes first', expect.any(AbortSignal), expect.any(String), expect.objectContaining({
    ...
    it('queues background shell terminal notifications for the model loop', async () => {
        ...
        await waitFor(() => {
            expect(mockBackgroundShellRegistry.setNotificationCallback).toHaveBeenCalledWith(expect.any(Function));
        });
        ...
        await waitFor(() => {
            expect(mockAddItem).toHaveBeenCalledWith({ type: 'notification', text: displayText }, expect.any(Number));
        });
        await waitFor(() => {
            expect(mockSendMessageStream).toHaveBeenCalledWith(modelText, expect.any(AbortSignal), expect.any(String), expect.objectContaining({
    ...
    it('queues background workflow completions for the model loop', async () => {
        ...
        const modelText = '<task-notification>\n<kind>workflow</kind>\n<status>completed</status>\n</task-notification>';
        await waitFor(() => {
            expect(mockWorkflowRunRegistry.setCompletionCallback).toHaveBeenCalledWith(expect.any(Function));
        });
    ...
    it('forwards submitted prompt provenance only for UserQuery', async () => {
        ...
        await act(async () => {
            await result.current.submitQuery('<system-reminder>managed</system-reminder>\n\nreview this', SendMessageType.UserQuery, undefined, { submittedPrompt: 'review this' });
        });
        expect(mockSendMessageStream.mock.calls[0]?.[3]).toEqual(expect.objectContaining({
            type: SendMessageType.UserQuery,
            submittedPrompt: 'review this',
        }));
        mockSendMessageStream.mockClear();
        await act(async () => {
            await result.current.submitQuery('retry payload', SendMessageType.Retry, undefined, { submittedPrompt: 'must not escape' });
        });
        expect(mockSendMessageStream.mock.calls[0]?.[3]).not.toHaveProperty('submittedPrompt');
        mockSendMessageStream.mockClear();
        await act(async () => {
            await result.current.submitQuery('tool result payload', SendMessageType.ToolResult, undefined, { submittedPrompt: 'must not escape' });
        });
        expect(mockSendMessageStream.mock.calls[0]?.[3]).not.toHaveProperty('submittedPrompt');
    });
    describe('vision bridge gate', () => {
        ...
        const enableBridge = (primaryAcceptsImages = false) => {
            Object.assign(mockConfig, {
                getEffectiveInputModalities: () => primaryAcceptsImages ? { image: true } : {},
        ...
        it('runs the bridge and replaces image parts with text for text-only models', async () => {
            ...
            const { result, mockSendMessageStream } = renderTestHook();
            await act(async () => {
                await result.current.submitQuery('@img.png describe', SendMessageType.UserQuery, undefined, { submittedPrompt: '@img.png describe' });
            });
            ...
            expect(mockAddItem).toHaveBeenCalledWith(expect.objectContaining({
                type: MessageType.VISION_NOTICE,
                text: expect.stringContaining('Your image and prompt/context were sent'),
            }), expect.any(Number));
            ...
            const visionNotice = mockAddItem.mock.calls.find((c) => c[0]?.type === MessageType.VISION_NOTICE &&
                String(c[0]?.text).includes('Converted'));
            expect(String(visionNotice?.[0]?.text)).not.toContain('[transcribed image]');
        });
        it('keeps an agent-capable image route through tools and retry, then clears it', async () => {
            ...
            expect(mockScheduleToolCalls).toHaveBeenCalledWith([toolRequest], expect.any(AbortSignal), selector);
            expect(mockAddItem).toHaveBeenCalledWith(expect.objectContaining({
                type: MessageType.VISION_NOTICE,
            ...
            await act(async () => {
                await result.current.submitQuery([{ text: 'retry' }, imagePart], SendMessageType.Retry);
            });
            ...
            expect(mockSendMessageStream.mock.calls[3]?.[3].modelOverride).toBeUndefined();
        });
        it('clamps oversized agent-capable image routes before applying a full-turn override', async () => {
            ...
            await waitFor(() => expect(mockSendMessageStream).toHaveBeenCalled());
            expect(JSON.stringify(mockSendMessageStream.mock.calls[0]?.[0])).toContain('Media omitted:');
            expect(mockSendMessageStream.mock.calls[0]?.[3].modelOverride).toBeUndefined();
            expect(mockRunVisionBridge).not.toHaveBeenCalled();
        });
        it('does not let a skill tool override clobber an active full-turn route', async () => {
            ...
            mockSendMessageStream.mockClear();
            const onComplete = mockUseReactToolScheduler.mock.calls.at(-1)?.[0];
            await act(async () => {
                await onComplete?.([
                    {
                        request: {
        ...
        it('pins the tool-result full-turn selector handed to the interactive scheduler', async () => {
            ...
            const canUseToolResultFullTurnModel = mockUseReactToolScheduler.mock.calls.at(-1)?.[4];
            ...
            expect(canUseToolResultFullTurnModel('other-agent\0https://x\0')).toBe(false);
        });
        ...
        it('keeps the turn alive with text plus a note when the bridge fails', async () => {
            ...
            expect(mockAddItem).toHaveBeenCalledWith(expect.objectContaining({
                ...
                text: expect.stringContaining('Your image and prompt/context were sent to vm (vision.example.com).'),
            }), expect.any(Number));
        });
        ...
        it('skips the bridge when the primary model already accepts images', async () => {
            ...
            expect(JSON.stringify(mockSendMessageStream.mock.calls[0][0])).toContain('inlineData');
        });
        it('runs the bridge when primary model modalities are unknown', async () => {
            ...
            expect(JSON.stringify(mockSendMessageStream.mock.calls[0][0])).toContain('[transcribed image]');
            expect(JSON.stringify(mockSendMessageStream.mock.calls[0][0])).not.toContain('inlineData');
        });
    ...
    it('labels loop wakeup cron notifications as Loop', async () => {
        ...
        mockConfig.isCronEnabled.mockReturnValue(true);
        mockConfig.getCronScheduler.mockReturnValue(scheduler);
        renderTestHook();
        await waitFor(() => {
            expect(mockAddItem).toHaveBeenCalledWith({ type: 'notification', text: 'Loop: /loop check status' }, expect.any(Number));
        });
    });
    it('expands autonomous loop wakeup sentinels before queuing them', async () => {
        ...
        renderTestHook();
        await waitFor(() => {
            expect(mockAddItem).toHaveBeenCalledWith({ type: 'notification', text: 'Loop: Autonomous loop tick' }, expect.any(Number));
        });
        await waitFor(() => {
        ...
        expect(sent).toContain(`prompt set to the literal sentinel \`${AUTONOMOUS_SENTINEL_DYNAMIC}\``);
        await waitFor(() => {
    ...
    it('skips missed autonomous loop wakeup sentinels', async () => {
        ...
        expect(mockAddItem).not.toHaveBeenCalledWith({ type: 'notification', text: 'Missed: Autonomous loop tick' }, expect.any(Number));
        expect(mockSendMessageStream).not.toHaveBeenCalled();
    });
    it('renders teammate reports as a compact notification, not a raw envelope bubble', async () => {
        ...
        mockConfig.getTeamManager.mockReturnValue(mockManager);
        const { mockSendMessageStream } = renderTestHook();
        await waitFor(() => {
            expect(mockManager.setLeaderMessageCallback).toHaveBeenCalledWith(expect.any(Function));
        });
        const display = '**scout-cli** reported back';
        const modelText = '<teammate_message_abcdef0123456789 from="scout-cli">\n' +
            'a very long report that should never reach the UI verbatim\n' +
            '</teammate_message_abcdef0123456789>';
        const callback = mockManager.setLeaderMessageCallback.mock
            .calls[0][0];
        act(() => {
        ...
        await waitFor(() => {
            expect(mockAddItem).toHaveBeenCalledWith({ type: 'notification', text: display }, expect.any(Number));
        });
        ...
        const addedTexts = mockAddItem.mock.calls
            .map((c) => c[0]?.text)
    ...
    it('should submit tool responses when all tool calls are completed and ready', async () => {
        ...
        renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24));
    ...
    it('forwards one exact Goal context across a ToolResult batch', async () => {
        ...
        const makeCompletedTool = (callId) => ({
            request: {
                callId,
                name: 'testTool',
                args: {},
        ...
        const options = mockSendMessageStream.mock.calls[0][3];
    ...
    it('ignores a deduplicated tool without Goal context when forwarding a fresh Goal result', async () => {
        ...
        const makeCompletedTool = (callId, goalContext) => ({
            request: {
                callId,
                name: 'testTool',
                args: {},
        ...
        renderHook(() => useGeminiStream(client, [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24));
        await act(async () => {
            await capturedOnComplete?.([
    ...
    it('fails close when a ToolResult batch has a stale Goal context', async () => {
        ...
        const stalePermit = { ...permit, revision: 2 };
        const dispatch = vi.fn().mockResolvedValue(undefined);
        const finishTurn = vi.fn().mockResolvedValue(undefined);
        const flush = vi.fn().mockResolvedValue(undefined);
        const activeSnapshot = {
    ...
    it('records tool results with goalContext during a Goal turn', async () => {
        ...
        expect(recordToolResult).toHaveBeenCalledWith(expect.any(Array), expect.objectContaining({ callId: 'shell-goal-1', status: 'success' }), {
            goalContext: {
                goalId: 'goal-record',
                revision: 1,
                turnId: 'turn-record',
    ...
    it('waits for a background agent when its launch exhausts capacity', async () => {
        ...
        expect(mockSendMessageStream).not.toHaveBeenCalled();
        act(() => {
            notificationCallback?.('Background agent completed.', '<task-notification>done</task-notification>');
        });
        await waitFor(() => expect(mockSendMessageStream).toHaveBeenCalledOnce());
        expect(mockSendMessageStream).toHaveBeenCalledWith('<task-notification>done</task-notification>', expect.any(AbortSignal), expect.any(String), expect.objectContaining({ type: SendMessageType.Notification }));
        mockSendMessageStream.mockClear();
    ...
    it('records mid-turn queued user messages after tool results accept them', async () => {
        ...
        renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24, midTurnDrainRef));
        await act(async () => {
            if (capturedOnComplete) {
                await capturedOnComplete(completedToolCalls);
            }
        ...
        expect(recordMidTurnUserMessage).toHaveBeenCalledWith([expectedMidTurnMessage], queuedPrompt);
        const queuedPromptAddItemIndex = mockAddItem.mock.calls.findIndex(([item]) => item.type === MessageType.USER && item.text === queuedPrompt);
        expect(queuedPromptAddItemIndex).toBeGreaterThanOrEqual(0);
        expect(recordMidTurnUserMessage.mock.invocationCallOrder[0]).toBeLessThan(mockAddItem.mock.invocationCallOrder[queuedPromptAddItemIndex]);
        expect(mockSendMessageStream.mock.invocationCallOrder[0]).toBeLessThan(recordMidTurnUserMessage.mock.invocationCallOrder[0]);
        expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.USER, text: queuedPrompt, sentToModel: false }, expect.any(Number));
        expect(mockSendMessageStream).toHaveBeenCalledWith([...toolCallResponseParts, expectedMidTurnMessage], expect.any(AbortSignal), 'prompt-id-midturn', expect.objectContaining({
            type: SendMessageType.ToolResult,
    ...
    it('provides queued steer input to core at the next sampling boundary', async () => {
        ...
        const drainSteer = vi
            ...
            .mockReturnValue([]);
        const { result } = renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24, { current: drainSteer }));
        await act(async () => {
            await result.current.submitQuery('start the analysis', SendMessageType.UserQuery, 'prompt-id-steer');
        });
        expect(mockSendMessageStream).toHaveBeenCalledTimes(1);
        const sendOptions = mockSendMessageStream.mock.calls[0][3];
        expect(sendOptions.getSteerInput).toEqual(expect.any(Function));
        let steerInput;
        await act(async () => {
            steerInput = await sendOptions.getSteerInput(new AbortController().signal);
        });
        expect(steerInput?.parts).toEqual([{ text: steeredPrompt }]);
        expect(recordMidTurnUserMessage).not.toHaveBeenCalled();
        expect(mockAddItem).not.toHaveBeenCalledWith({ type: MessageType.USER, text: steeredPrompt, sentToModel: false }, expect.any(Number));
        steerInput?.accept();
        expect(recordMidTurnUserMessage).toHaveBeenCalledWith([{ text: steeredPrompt }], steeredPrompt);
        expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.USER, text: steeredPrompt, sentToModel: false }, expect.any(Number));
    });
    it('processes queued /goal clear at the next sampling boundary', async () => {
        ...
        const drainSteer = vi
            ...
            .mockReturnValue([]);
        const { result } = renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24, { current: drainSteer }, undefined, undefined, undefined, { current: restoreSteer }));
        await act(async () => {
    ...
    it('keeps ordinary queued messages while Goal controls stay out of model input', async () => {
        ...
        const sendOptions = mockSendMessageStream.mock.calls[0][3];
        const steerInput = await sendOptions.getSteerInput(new AbortController().signal);
    ...
    it('restores drained steer input when attachment resolution is cancelled', async () => {
        ...
        const restoreSteer = vi.fn();
        vi.spyOn(atCommandProcessor, 'resolveAtCommandQuery').mockImplementation(() => new Promise(() => { }));
        const drainSteer = vi
            .fn()
            .mockReturnValueOnce([steeredPrompt])
            .mockReturnValue([]);
    ...
    it('restores later messages when cancellation races with @ resolution', async () => {
        ...
        vi.spyOn(atCommandProcessor, 'resolveAtCommandQuery').mockImplementation(() => new Promise((resolve) => {
    ...
    it('resolves mid-turn @ image messages before submitting tool results', async () => {
        ...
        const resolvedImagePart = {
            inlineData: {
                mimeType: 'image/png',
                data: 'iVBORw0KGgo=',
            },
        };
        const resolvedTextPart = { text: 'inspect @/tmp/screenshot.png' };
        const transcriptPart = { text: '[mid-turn image transcript]' };
        const recordMidTurnUserMessage = vi.fn();
        ...
        const expectedMidTurnParts = [transcriptPart];
        ...
        const midTurnNotice = mockAddItem.mock.calls.find((c) => c[0]?.type === MessageType.VISION_NOTICE &&
            String(c[0]?.text).includes('to text via'));
        expect(String(midTurnNotice?.[0]?.text)).not.toContain('[mid-turn image transcript]');
        const sent = JSON.stringify(mockSendMessageStream.mock.calls[0][0]);
        expect(sent).toContain('[mid-turn image transcript]');
        expect(sent).not.toContain('inlineData');
        expect(resolveAtCommandQuerySpy).toHaveBeenCalledWith(expect.objectContaining({
            query: queuedPrompt,
            config: mockConfig,
            onDebugMessage: mockOnDebugMessage,
            signal: expect.any(AbortSignal),
        }));
        expect(recordMidTurnUserMessage).toHaveBeenCalledWith(expectedMidTurnParts, queuedPrompt);
        ...
        expect(mockSendMessageStream).toHaveBeenCalledWith([...toolCallResponseParts, ...expectedMidTurnParts], expect.any(AbortSignal), 'prompt-id-midturn-image', expect.objectContaining({ type: SendMessageType.ToolResult }));
    });
    it('forwards mid-turn text when a bridge failure returns no replacement parts', async () => {
        ...
        expect(sent).not.toContain('inlineData');
        expect(recordMidTurnUserMessage).toHaveBeenCalledWith([resolvedTextPart], queuedPrompt);
    });
    it('skips mid-turn @ injection when resolution should not proceed', async () => {
        ...
        expect(recordMidTurnUserMessage).not.toHaveBeenCalled();
        expect(mockSendMessageStream).toHaveBeenCalledWith(toolCallResponseParts, expect.any(AbortSignal), 'prompt-id-midturn-at-error', expect.objectContaining({ type: SendMessageType.ToolResult }));
    });
    it('warns and skips mid-turn @ injection when resolution fails', async () => {
        ...
        vi.spyOn(atCommandProcessor, 'resolveAtCommandQuery').mockRejectedValue(new Error('permission denied'));
        const toolCallResponseParts = [
            {
                functionResponse: {
                    id: 'call1',
        ...
        expect(recordMidTurnUserMessage).not.toHaveBeenCalled();
        expect(mockSendMessageStream).toHaveBeenCalledWith(toolCallResponseParts, expect.any(AbortSignal), 'prompt-id-midturn-at-throw', expect.objectContaining({ type: SendMessageType.ToolResult }));
    });
    it('times out stalled mid-turn @ resolution before submitting tool results', async () => {
        ...
        let resolveSignal;
        let rejectResolve;
        vi.spyOn(atCommandProcessor, 'resolveAtCommandQuery').mockImplementation(({ signal }) => {
            resolveSignal = signal;
            return new Promise((_, reject) => {
                rejectResolve = reject;
                signal.addEventListener('abort', () => reject(signal.reason instanceof Error
                    ? signal.reason
                    : new Error('aborted')), { once: true });
            });
        });
        const toolCallResponseParts = [
            {
    ...
    it('skips mid-turn @ fallback side effects when cancelled during resolution', async () => {
        ...
        const { result } = renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24, midTurnDrainRef));
        let completePromise;
        act(() => {
            if (capturedOnComplete) {
                completePromise = capturedOnComplete(completedToolCalls);
        ...
        expect(resolveSignal?.aborted).toBe(true);
        expect(recordMidTurnUserMessage).not.toHaveBeenCalled();
        expect(mockAddItem).not.toHaveBeenCalledWith({ type: MessageType.USER, text: queuedPrompt, sentToModel: false }, expect.any(Number));
        expect(mockSendMessageStream).not.toHaveBeenCalled();
    });
    ...
    it('handles mid-turn drain when chat recording is not configured', async () => {
        ...
        expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.USER, text: queuedPrompt, sentToModel: false }, expect.any(Number));
        expect(mockSendMessageStream).toHaveBeenCalledWith([...toolCallResponseParts, { text: queuedPrompt }], expect.any(AbortSignal), 'prompt-id-midturn', expect.objectContaining({ type: SendMessageType.ToolResult }));
    });
    ...
    it('does not schedule tool calls collected before a LoopDetected halt', async () => {
        ...
        const client = new MockedGeminiClientClass(mockConfig);
        const { result } = renderHook(() => useGeminiStream(client, [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24));
        await act(async () => {
    ...
    it('suppresses duplicate provider tool-call ids before TUI scheduling', async () => {
        const recordToolResult = vi.fn();
        mockConfig.getToolOutputBatchBudget = () => 10_000;
        mockConfig.getChatRecordingService = () => ({ recordToolResult });
        let capturedOnComplete = null;
        mockUseReactToolScheduler.mockImplementation((onComplete) => {
            capturedOnComplete ??= onComplete;
            return [[], mockScheduleToolCalls, mockMarkToolsAsSubmitted];
        ...
        const toolResultParts = mockSendMessageStream.mock.calls[1][0];
        expect(toolResultParts).toHaveLength(2);
        expect(toolResultParts[0].functionResponse?.response?.['output']).toContain('Tool output truncated.');
        expect(toolResultParts[1].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "tool-dup"');
        ...
        expect(recordToolResult.mock.calls.flatMap((call) => call[0])).toEqual(toolResultParts);
        expect(recordToolResult.mock.calls.map((call) => call[1].executionStatus)).toEqual(expect.arrayContaining(['success', 'not_started']));
    ...
    it('submits a synthetic response for history-paired duplicate provider ids without scheduling', async () => {
        ...
        expect(toolResultParts[0].functionResponse?.response?.['error']).toContain('Duplicate provider tool call id "tool-history"');
    ...
    it('submits a fast tool result after the stream ended but before React replaces the callback', async () => {
        ...
        const staleCompletedOnComplete = staleOnComplete;
        await act(async () => {
        ...
        expect(mockSendMessageStream).toHaveBeenNthCalledWith(2, responseParts, expect.any(AbortSignal), 'prompt-fast-after-stream', expect.objectContaining({ type: SendMessageType.ToolResult }));
        expect(mockMarkToolsAsSubmitted).toHaveBeenCalledWith([
    ...
    it('handles a mixed batch (one deduped + one non-deduped) without double-counting telemetry', async () => {
        ...
        await waitFor(() => {
            ...
            const allMarked = mockMarkToolsAsSubmitted.mock.calls.flatMap((call) => call[0]);
        ...
        const recordedCallIds = client.recordCompletedToolCall.mock.calls.map((call) => call[1].path);
    ...
    it('should not flicker streaming state to Idle between tool completion and submission', async () => {
        ...
        const { result, rerender } = renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24));
    ...
    describe('Tool-use summary generation', () => {
        ...
        const runCompletion = async (config, completedTools) => {
            ...
            renderHook(() => useGeminiStream(new MockedGeminiClientClass(config), historyWithToolGroup, mockAddItem, config, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24));
            await act(async () => {
                if (capturedOnComplete) {
        ...
        it('fires generation with tool input/output when enabled', async () => {
            ...
            const userText = options.contents[0].parts[0].text;
        ...
        it('drops a late summary when a newer tool_group has been added', async () => {
            ...
            let resolveSummary;
            const generateText = vi.fn().mockImplementation(() => new Promise((resolve) => {
            ...
            renderHook(() => useGeminiStream(new MockedGeminiClientClass(config), history, mockAddItem, config, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24));
            await act(async () => {
                if (capturedOnComplete) {
                    await capturedOnComplete(completedTools);
                }
    ...
    describe('Cancellation', () => {
        ...
        it('preserves text and inline image ordering in streamed content', async () => {
            ...
            expect(mockAddItem.mock.calls
                .map(([item]) => item)
                .filter((item) => item.type === 'gemini' || item.type === 'gemini_content')).toEqual([
                expect.objectContaining({ type: 'gemini', text: 'before' }),
                { type: 'gemini_content', text: '', images: [image] },
                { type: 'gemini_content', text: 'after' },
            ]);
        ...
        it('caps inline images for one assistant output and reports the overflow', async () => {
            ...
            const assistantItems = result.current.pendingHistoryItems.filter((item) => item.type === 'gemini' || item.type === 'gemini_content');
            expect(assistantItems.flatMap((item) => item.images ?? [])).toEqual(images.slice(0, MAX_INLINE_IMAGES_PER_ITEM));
        ...
        it('resets the inline image cap after a fresh retry', async () => {
            const failedImages = Array.from({ length: MAX_INLINE_IMAGES_PER_ITEM }, (_, index) => ({
        ...
        it('resets the inline image cap at a finished response boundary', async () => {
            const firstOutputImages = Array.from({ length: MAX_INLINE_IMAGES_PER_ITEM }, (_, index) => ({
        ...
        it('does not retain an oversized inline image payload in UI state', async () => {
            ...
            const assistantItems = result.current.pendingHistoryItems.filter((item) => item.type === 'gemini' || item.type === 'gemini_content');
            expect(assistantItems).toEqual([]);
            expect(JSON.stringify(result.current.pendingHistoryItems)).not.toContain(oversizedData);
            act(() => result.current.cancelOngoingRequest());
            await act(async () => {
                releaseStream();
            });
        ...
        it('discards every staged mixed-content run on model fallback', async () => {
            vi.useFakeTimers();
            let emitFallback;
            const waitForFallback = new Promise((resolve) => {
        ...
        it('discards staged mixed content before an explicit retry after a thrown stream', async () => {
            ...
            expect(result.current.pendingHistoryItems).toEqual(expect.arrayContaining([
                expect.objectContaining({ type: 'gemini', text: 'before' }),
            ...
            expect(result.current.pendingHistoryItems).not.toEqual(expect.arrayContaining([
        ...
        it('splits oversized streamed thoughts so the pending item stays bounded', async () => {
            ...
            vi.mocked(findLastSafeSplitPoint).mockImplementation((s, max) => max !== undefined && s.length > max ? max : s.length);
            let releaseStream;
            const holdStream = new Promise((resolve) => {
                releaseStream = resolve;
            });
            ...
            const thoughtItems = mockAddItem.mock.calls
                .map(([item]) => item)
                .filter((item) => item.type === 'gemini_thought' ||
        ...
        it('repairs the fence when an oversized thought is split inside a code block', async () => {
            ...
            const codeBody = Array.from({ length: 2000 }, (_, i) => `const x${i} = ${i};`).join('\n');
        ...
        it('splits oversized streamed content by rendered height so the pending item stays bounded', async () => {
            ...
            vi.mocked(findLastSafeSplitPoint).mockImplementation((s, cap) => cap === undefined ? s.length : Math.min(cap, s.length));
            ...
            const longContent = Array.from({ length: 15 }, (_, i) => `paragraph ${i + 1}`).join('\n\n');
            let releaseStream;
            const holdStream = new Promise((resolve) => {
                releaseStream = resolve;
            });
            ...
            const pendingLineCount = pendingText.length === 0 ? 0 : pendingText.split('\n').length;
        ...
        const streamContent = async (result, content) => {
            let releaseStream;
            const holdStream = new Promise((resolve) => {
                releaseStream = resolve;
            });
        ...
        const geminiContentItems = () => mockAddItem.mock.calls
            .map(([item]) => item)
            .filter((item) => item.type === 'gemini' || item.type === 'gemini_content');
        ...
        it('commits each completed table when a single table fills the budget (no stall-then-dump)', async () => {
            ...
            const pendingText = result.current.pendingHistoryItems[0]?.text ?? '';
            const pendingLines = pendingText.length === 0 ? 0 : pendingText.split('\n').length;
        ...
        it('commits a code block taller than the viewport incrementally (no stall-then-dump)', async () => {
            ...
            const codeLines = Array.from({ length: 40 }, (_, i) => `int v${i} = ${i};`);
            const content = [
                'Here is some C++:',
                '',
                '```cpp',
            ...
            expect(committed.some((item) => item.text.includes('int v0 = 0;'))).toBe(true);
        ...
        it('keeps a tall mermaid block whole (never splits it mid-diagram)', async () => {
            ...
            const nodes = Array.from({ length: 40 }, (_, i) => `  A${i} --> A${i + 1}`);
            const content = [
                'Here is a diagram:',
                '',
                '```mermaid',
        ...
        it('should call onCancelSubmit handler when cancelOngoingRequest is called', async () => {
            ...
            mockSendMessageStream.mockReturnValue(mockStream);
            const { result } = renderHook(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, cancelSubmitSpy, () => { }, 80, 24));
            // Start a query
            await act(async () => {
                result.current.submitQuery('test query');
            });
        ...
        it("attaches the cancelled turn's user prompt to onCancelSubmit info.lastTurnUserItem for normal UserQuery", async () => {
            ...
            mockSendMessageStream.mockReturnValue(mockStream);
            const { result } = renderHook(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, cancelSubmitSpy, () => { }, 80, 24, undefined, { logMessage: vi.fn() }));
            await act(async () => {
                result.current.submitQuery('<system-reminder>managed</system-reminder>\n\nwhat time is it?', SendMessageType.UserQuery, undefined, { submittedPrompt: 'what time is it?' });
            });
            act(() => {
                result.current.cancelOngoingRequest();
            });
        ...
        it('emits lastTurnUserItem: null for paths that do NOT add a user history item (Notification)', async () => {
            ...
            const { result } = renderHook(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, cancelSubmitSpy, () => { }, 80, 24));
            await act(async () => {
                result.current.submitQuery('background agent done', SendMessageType.Notification);
            });
            act(() => {
                result.current.cancelOngoingRequest();
            });
        ...
        it('should call setShellInputFocused(false) when cancelOngoingRequest is called', async () => {
            ...
            mockSendMessageStream.mockReturnValue(mockStream);
            const { result } = renderHook(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, vi.fn(), setShellInputFocusedSpy, // Pass the spy here
            80, 24));
            // Start a query
            await act(async () => {
                result.current.submitQuery('test query');
        ...
        it('flushes buffered stream events before snapshotting pendingItem so cancelling mid-throttle does not lose content', async () => {
            ...
            expect(info?.pendingItem).toEqual(expect.objectContaining({
                type: 'gemini',
        ...
        it('still resets streamingState to Idle when onCancelSubmit throws', async () => {
            ...
            mockSendMessageStream.mockReturnValue(mockStream);
            const { result } = renderHook(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => {
                throw new Error('boom');
            }, setShellInputFocusedSpy, 80, 24));
            await act(async () => {
                result.current.submitQuery('test query');
            });
            expect(result.current.streamingState).toBe(StreamingState.Responding);
        ...
        it('cancels pending self-paced loop wakeups and notifies on a tick-in-flight abort', async () => {
            ...
            mockConfig.getCronScheduler.mockReturnValue({
                cancelAllWakeups,
            });
            const mockStream = (async function* () {
                yield { type: 'content', value: 'Part 1' };
    ...
    describe('Slash Command Handling', () => {
        ...
        it('should call Gemini with prompt content when slash command returns a `submit_prompt` action', async () => {
            ...
            await act(async () => {
                await result.current.submitQuery('/my-custom-command', SendMessageType.UserQuery, undefined, { submittedPrompt: '/my-custom-command' });
            });
            await waitFor(() => {
                ...
                expect(localMockSendMessageStream).toHaveBeenCalledWith('This is the actual prompt from the command file.', expect.any(AbortSignal), expect.any(String), expect.objectContaining({
                    type: SendMessageType.UserQuery,
        ...
        it('should correctly handle a submit_prompt action with empty content', async () => {
            ...
            await waitFor(() => {
                ...
                expect(localMockSendMessageStream).toHaveBeenCalledWith('', expect.any(AbortSignal), expect.any(String), expect.objectContaining({ type: SendMessageType.UserQuery }));
            });
        });
        it('should not call handleSlashCommand for line comments', async () => {
            ...
            await waitFor(() => {
                expect(mockHandleSlashCommand).not.toHaveBeenCalled();
                expect(localMockSendMessageStream).toHaveBeenCalledWith('// This is a line comment', expect.any(AbortSignal), expect.any(String), expect.objectContaining({ type: SendMessageType.UserQuery }));
            });
        });
        it('should not call handleSlashCommand for block comments', async () => {
            ...
            await waitFor(() => {
                expect(mockHandleSlashCommand).not.toHaveBeenCalled();
                expect(localMockSendMessageStream).toHaveBeenCalledWith('/* This is a block comment */', expect.any(AbortSignal), expect.any(String), expect.objectContaining({ type: SendMessageType.UserQuery }));
            });
        });
        describe('inline model override', () => {
            ...
            const allowInlineModel = (modelId = 'inline-model') => {
                ...
                mockConfig.getContentGeneratorConfig = vi.fn(() => ({ authType: AuthType.QWEN_OAUTH }));
                mockConfig.getAvailableModelsForAuthType = vi.fn(() => [{ id: modelId, authType: AuthType.QWEN_OAUTH }]);
            };
            it('does not let a skill tool with modelOverride: undefined clobber an active inline override', async () => {
                ...
                const { result } = renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24));
            ...
            it('clears the inline override on retry and reverts to the session model', async () => {
                ...
                // the switch.
                await act(async () => {
                    await result.current.submitQuery('do the thing', SendMessageType.Retry);
                });
                await waitFor(() => expect(mockSendMessageStream).toHaveBeenCalled());
                expect(mockSendMessageStream.mock.calls[0][3].modelOverride).toBeUndefined();
                expect(mockAddItem).toHaveBeenCalledWith(expect.objectContaining({
            ...
            it('does not clear an active model override when a background notification drains', async () => {
                ...
                const callback = mockBackgroundShellRegistry.setNotificationCallback
                    .mock.calls[0][0];
                act(() => {
                    callback('Background shell "npm test" completed.', '<task-notification>completed</task-notification>');
                });
            ...
            it('drops a queued running monitor event after cancellation', async () => {
                ...
                mockAddItem.mockClear();
                await act(async () => {
                    callback('Monitor "logs" event #1: ready', '<task-notification>running</task-notification>', { monitorId: 'mon_1', status: 'running' });
                    monitorStatus = 'cancelled';
                });
                expect(mockSendMessageStream).not.toHaveBeenCalled();
                expect(mockAddItem).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'notification' }), expect.any(Number));
            });
            it('keeps notifications from different Todo work chains in separate turns', async () => {
                ...
                mockSendMessageStream.mockClear();
                await act(async () => {
                    callback('Monitor "logs" event #1: ready', '<task-notification>first</task-notification>', {
                        monitorId: 'mon_1',
                        status: 'completed',
                        todoWorkChainId: 'chain-1',
                    });
                    callback('Monitor "build" event #1: ready', '<task-notification>second</task-notification>', {
                        monitorId: 'mon_2',
                        status: 'completed',
                ...
                await act(async () => {
                    callback('Monitor "build" event #2: done', '<task-notification>third</task-notification>', {
                        monitorId: 'mon_2',
                        status: 'completed',
                        todoWorkChainId: 'chain-2',
                    });
            ...
            it('defers a cron notification while a Goal owns queued user messages, then delivers it exactly once', async () => {
                ...
                mockConfig.isCronEnabled.mockReturnValue(true);
                mockConfig.getCronScheduler.mockReturnValue(scheduler);
                const { rerender, client } = renderTestHook([], undefined, undefined, undefined, undefined, goalQueueRef);
                ...
                expect(mockAddItem.mock.calls.filter(([item]) => item.type === 'notification')).toHaveLength(1);
            });
        });
    });
    describe('Memory Refresh on save_memory', () => {
        it('refreshes memory without finalizing client-only tool responses', async () => {
            ...
            renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, mockPerformMemoryRefresh, false, () => { }, () => { }, () => { }, () => { }, 80, 24));
        ...
        it('refreshes managed-memory instructions after interactive memory file writes', async () => {
            ...
            expect(mockRefreshMemoryAfterManagedWrite).toHaveBeenCalledWith(mockConfig, [
                {
                    toolName: 'write_file',
                    args: { file_path: '/workspace/.qwen/memory/project.md' },
                    status: 'success',
        ...
        async function completeToolWrite(completedToolCall) {
            const onComplete = mockUseReactToolScheduler.mock.calls.at(-1)?.[0];
            expect(onComplete, 'useReactToolScheduler onComplete was never registered').toBeDefined();
            await act(async () => {
                await onComplete?.([completedToolCall]);
            });
        }
        async function submitSlashCommandAndCompleteTool(completedToolCall, refreshContextFilesOnWrite) {
        ...
        async function submitContinuationAndCompleteTool(result, text, submitType, promptId, extra) {
            await act(async () => {
            ...
            await completeToolWrite(createCompletedFileWrite({ filePath: '/test/dir/QWEN.md' }));
        ...
        it('refreshes context-file instructions after bare remember writes QWEN.md', async () => {
            await submitSlashCommandAndCompleteTool(createCompletedFileWrite({ filePath: '/test/dir/QWEN.md' }), true);
            expect(mockRefreshMemoryInstruction).toHaveBeenCalledWith(mockConfig, {
                logContext: 'interactive context-file memory tool batch',
            });
        });
        it('keeps refreshing context-file instructions for later writes in a marked turn', async () => {
            await markContextRefreshIntent();
            await completeToolWrite(createCompletedFileWrite({
        ...
        it('refreshes context-file instructions after bare remember edits QWEN.md', async () => {
            await submitSlashCommandAndCompleteTool(createCompletedFileWrite({
                toolName: 'edit',
                filePath: '/test/dir/QWEN.md',
            }), true);
            expect(mockRefreshMemoryInstruction).toHaveBeenCalledWith(mockConfig, {
        ...
        it('does not refresh context-file instructions for other file writes', async () => {
            await submitSlashCommandAndCompleteTool(createCompletedFileWrite({ filePath: '/test/dir/notes.md' }), true);
            expect(mockRefreshMemoryInstruction).not.toHaveBeenCalled();
        });
        it('does not refresh context-file instructions for ordinary QWEN.md writes', async () => {
            await submitSlashCommandAndCompleteTool(createCompletedFileWrite({ filePath: '/test/dir/QWEN.md' }), false);
            expect(mockRefreshMemoryInstruction).not.toHaveBeenCalled();
        });
        it('clears unmatched context-file refresh intent on the next ordinary turn', async () => {
            ...
            await completeToolWrite(createCompletedFileWrite({ filePath: '/test/dir/notes.md' }));
            mockHandleSlashCommand.mockResolvedValue(false);
            await act(async () => {
        ...
        ])('preserves context-file refresh intent across a %s turn', async (_label, submitType) => {
            const result = await markContextRefreshIntent();
            await submitContinuationAndCompleteTool(result, 'retry remember write', submitType);
        });
        it('preserves context-file refresh intent across a Goal turn', async () => {
            ...
            await submitContinuationAndCompleteTool(result, goal.continuationContext, SendMessageType.Goal, 'prompt-id-goal-memory-refresh', { goal });
        });
    ...
    describe('Error Handling', () => {
        it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
            ...
            const { result } = renderHook(() => useGeminiStream(new MockedGeminiClientClass(testConfig), [], mockAddItem, testConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24));
            // 2. Action
            await act(async () => {
    ...
    describe('Citation event', () => {
        ...
        it('preserves streamed text across hidden citation events', async () => {
            ...
            const { result } = renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, settingsWithCitationsHidden, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24));
            await act(async () => {
                await result.current.submitQuery('test hidden citation');
            });
            expect(mockAddItem.mock.calls
                .map(([item]) => item)
                .filter((item) => item.type === 'gemini')).toEqual([
                expect.objectContaining({ type: 'gemini', text: 'Hello world more' }),
            ]);
            expect(mockAddItem).not.toHaveBeenCalledWith(expect.objectContaining({ type: MessageType.INFO }), expect.any(Number));
        });
    });
    ...
    describe('handleFinishedEvent', () => {
        ...
        it('should add appropriate messages for other finish reasons', async () => {
            const testCases = [
                ...
                {
                    reason: 'SPII',
                    message: '⚠  Response stopped due to sensitive personally identifiable information.',
                },
            ...
            for (const { reason, message } of testCases) {
                ...
                const { result } = renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, vi.fn(), 80, 24));
                await act(async () => {
    ...
    it('should process @include commands, adding user turn after processing to prevent race conditions', async () => {
        ...
        const { result } = renderHook(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, // shellModeActive
        ...
        24));
        await act(async () => {
            await result.current.submitQuery(rawQuery, SendMessageType.UserQuery, undefined, { submittedPrompt: rawQuery });
        });
    ...
    describe('Thought Reset', () => {
        ...
        it('should count streamed thought descriptions toward the response length', async () => {
            ...
            expect(result.current.streamingResponseLengthRef.current).toBe('thinking'.length);
        });
        ...
        it('should sum multiple streamed thought chunks toward the response length', async () => {
            ...
            expect(result.current.streamingResponseLengthRef.current).toBe('thinking more'.length);
        });
        ...
        it('should commit thought and finalize dual output on UserCancelled', async () => {
            ...
            await waitFor(() => expect(mockCleanupReviewWorktreeLeases).toHaveBeenCalledWith({
                sessionId: 'test-session-id',
        ...
        it('should keep terminal error visible at turn completion', async () => {
            ...
            await waitFor(() => {
                ...
                expect(errorItem?.hint).toContain('Ctrl+Y');
            });
        });
        it('omits the Ctrl+Y retry hint for stream errors during a Goal turn', async () => {
            ...
            const { result } = renderTestHook();
            await act(async () => {
                await result.current.submitQuery(goal.continuationContext, SendMessageType.Goal, 'prompt-id-goal-error', { goal });
            });
            await waitFor(() => {
                const errorItem = result.current.pendingHistoryItems.find((item) => item.type === 'error');
                expect(errorItem).toBeDefined();
                expect(errorItem?.hint).toBeUndefined();
            });
        });
        it('should clear stale countdown error when retry succeeds without a second Retry event', async () => {
            ...
            try {
                ...
                expect(errorItem).toBeDefined();
                const countdownItem = result.current.pendingHistoryItems.find((item) => item.type === 'retry_countdown');
                ...
                const remainingError = result.current.pendingHistoryItems.find((item) => item.type === MessageType.ERROR);
                expect(remainingError).toBeUndefined();
                const remainingCountdown = result.current.pendingHistoryItems.find((item) => item.type === 'retry_countdown');
        ...
        it('should not wipe a Goal turn terminal error in post-stream cleanup', async () => {
            mockConfig.getHookSystem = vi.fn(() => null);
            mockSendMessageStream.mockReturnValue((async function* () {
                yield {
                    type: ServerGeminiEventType.Error,
            ...
            const { result } = renderTestHook();
            await act(async () => {
                await result.current.submitQuery(goal.continuationContext, SendMessageType.Goal, 'prompt-id-goal-cleanup', { goal });
            });
            await waitFor(() => {
                const errorItem = result.current.pendingHistoryItems.find((item) => item.type === 'error');
                expect(errorItem).toBeDefined();
        ...
        it('fires onDeliveryFailed (not onDelivered) when a Goal turn hits a stream error', async () => {
            ...
            const { result } = renderTestHook();
            await act(async () => {
                await result.current.submitQuery(goal.continuationContext, SendMessageType.Goal, 'prompt-id-goal-deliver', { goal, onDelivered, onDeliveryFailed });
            });
        ...
        it('should memoize pendingHistoryItems', () => {
            ...
            const { result, rerender } = renderHook(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, vi.fn(), // setShellInputFocused
            80, 24));
        ...
        it('should not dispatch queued tool calls after the request is aborted', async () => {
            let resolveStream;
            let toolCallQueued;
            const streamCanFinish = new Promise((resolve) => {
                resolveStream = resolve;
            });
            const toolCallWasQueued = new Promise((resolve) => {
            ...
            let submitPromise;
            await act(async () => {
                submitPromise = result.current.submitQuery('abort before tool dispatch');
            });
        ...
        it('commits pending retry error to history (without hint) when a new query starts', async () => {
            ...
            expect(mockAddItem).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'error' }), expect.any(Number));
            mockSendMessageStream.mockReturnValueOnce((async function* () {
                yield {
                    type: ServerGeminiEventType.Content,
            ...
            await waitFor(() => {
                expect(mockAddItem).toHaveBeenCalledWith(expect.objectContaining({ type: 'error' }), expect.any(Number));
            });
            // …and the retry hint is stripped, since it is no longer actionable.
            const errorCommit = mockAddItem.mock.calls.find(([item]) => item && typeof item === 'object' && item.type === 'error');
    ...
    describe('Concurrent Execution Prevention', () => {
        ...
        it('should keep the main request cancellable after submitting /btw in parallel', async () => {
            ...
            const { result } = renderTestHook([], undefined, undefined, cancelSubmitSpy, { logMessage: mockLogMessage });
            let mainRequest;
            await act(async () => {
                mainRequest = result.current.submitQuery('<system-reminder>managed</system-reminder>\n\nFirst query', SendMessageType.UserQuery, undefined, { submittedPrompt: 'First query' });
            });
            try {
                await waitFor(() => {
                ...
                await act(async () => {
                    await result.current.submitQuery(btwQuery, SendMessageType.UserQuery, undefined, { submittedPrompt: btwQuery });
                });
                act(() => {
                    result.current.cancelOngoingRequest();
                });
                ...
                expect(cancelSubmitSpy.mock.calls.at(-1)?.[0]?.canUndoLastLoggedUserMessage).toBe(false);
                expect(mockSendMessageStream).toHaveBeenCalledTimes(1);
                expect(mockLogMessage).toHaveBeenNthCalledWith(2, MessageSenderType.USER, btwQuery);
            }
            finally {
                resolveFirstCall();
                await mainRequest;
        ...
        it('should preserve the legacy model path for ?btw during a main response', async () => {
            ...
            const cancelSubmitSpy = vi.fn();
            const { result } = renderTestHook([], undefined, undefined, cancelSubmitSpy);
            let mainRequest;
            await act(async () => {
                mainRequest = result.current.submitQuery('<system-reminder>managed</system-reminder>\n\nFirst query', SendMessageType.UserQuery, undefined, { submittedPrompt: 'First query' });
            });
            try {
                ...
                expect(mockSendMessageStream).toHaveBeenCalledTimes(2);
                expect(mockSendMessageStream.mock.calls[1]?.[3]).toEqual(expect.objectContaining({
                    type: SendMessageType.UserQuery,
        ...
        it('should prevent concurrent submitQuery calls', async () => {
            ...
            expect(mockSendMessageStream).toHaveBeenCalledTimes(1);
            expect(mockSendMessageStream).toHaveBeenCalledWith('First query', expect.any(AbortSignal), expect.any(String), expect.objectContaining({ type: SendMessageType.UserQuery }));
        ...
        it('should allow subsequent calls after first call completes', async () => {
            ...
            expect(mockSendMessageStream).toHaveBeenCalledTimes(2);
            expect(mockSendMessageStream).toHaveBeenNthCalledWith(1, 'First query', expect.any(AbortSignal), expect.any(String), expect.objectContaining({ type: SendMessageType.UserQuery }));
            expect(mockSendMessageStream).toHaveBeenNthCalledWith(2, 'Second query', expect.any(AbortSignal), expect.any(String), expect.objectContaining({ type: SendMessageType.UserQuery }));
        });
        it('should reset execution flag even when query preparation fails', async () => {
            ...
            expect(mockSendMessageStream).toHaveBeenCalledTimes(1);
            expect(mockSendMessageStream).toHaveBeenCalledWith('Second query', expect.any(AbortSignal), expect.any(String), expect.objectContaining({ type: SendMessageType.UserQuery }));
        });
    });
    ...
    describe('cron scheduler initialization', () => {
        ...
        const renderCronHook = (scheduler, initialized) => {
            const cronConfig = {
                ...mockConfig,
                isCronEnabled: vi.fn(() => true),
                getCronScheduler: vi.fn(() => scheduler),
            };
            return renderHook((props) => useGeminiStream(new MockedGeminiClientClass(cronConfig), [], mockAddItem, cronConfig, props.initialized, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24), { initialProps: { initialized } });
        };
        ...
        it('does not start scheduler if unmounted during the enableDurable gap', async () => {
            ...
            let resolveEnable = () => { };
            const scheduler = {
                enableDurable: vi.fn().mockImplementation(() => new Promise((resolve) => {
    ...
    describe('timestamp attachment', () => {
        it('attaches a numeric timestamp to gemini items via commitItem', async () => {
            ...
            const geminiCalls = mockAddItem.mock.calls.filter((call) => call[0]?.type === 'gemini');
        ...
        it('does not attach timestamp to non-gemini items', async () => {
            ...
            const nonGeminiCalls = mockAddItem.mock.calls.filter((call) => call[0]?.type !== 'gemini');
    ...
    it('excludes sentToModel-false steer items from YOLO turn-count telemetry', async () => {
        ...
        renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), history, mockAddItem, mockConfig, true, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24, undefined, undefined, undefined));
        await waitFor(() => {
