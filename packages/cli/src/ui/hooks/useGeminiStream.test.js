...
import { ApprovalMode, AuthType, GeminiEventType as ServerGeminiEventType, SendMessageType, ToolErrorType, ToolConfirmationOutcome, } from '@qwen-code/qwen-code-core';
...
describe('useGeminiStream', () => {
    ...
    it('records mid-turn queued user messages before submitting tool results', async () => {
        ...
        renderHook(() => useGeminiStream(new MockedGeminiClientClass(mockConfig), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => { }, () => { }, 80, 24, midTurnDrainRef));
        await act(async () => {
            if (capturedOnComplete) {
                await capturedOnComplete(completedToolCalls);
            }
        ...
        expect(recordMidTurnUserMessage.mock.invocationCallOrder[0]).toBeLessThan(mockSendMessageStream.mock.invocationCallOrder[0]);
        expect(mockAddItem).toHaveBeenCalledWith({ type: MessageType.USER, text: queuedPrompt }, expect.any(Number));
        expect(mockSendMessageStream).toHaveBeenCalledWith([...toolCallResponseParts, expectedMidTurnMessage], expect.any(AbortSignal), 'prompt-id-midturn', { type: SendMessageType.ToolResult });
    });
    it('handles mid-turn drain when chat recording is not configured', async () => {
        ...
        ], expect.any(AbortSignal), 'prompt-id-midturn', { type: SendMessageType.ToolResult });
    });
    ...
    describe('Cancellation', () => {
        ...
        it('still resets streamingState to Idle when onCancelSubmit throws', async () => {
            ...
            mockSendMessageStream.mockReturnValue(mockStream);
            const { result } = renderHook(() => useGeminiStream(mockConfig.getGeminiClient(), [], mockAddItem, mockConfig, mockLoadedSettings, mockOnDebugMessage, mockHandleSlashCommand, false, () => 'vscode', () => { }, () => Promise.resolve(), false, () => { }, () => { }, () => {
    ...
    describe('StopHookLoop Event', () => {
        it('syncs active_goal events into the active goal store', async () => {
            ...
            expect(mockSetActiveGoal).toHaveBeenCalledWith('test-session-id', activeGoal);
        ...
        it('renders active goal StopHookLoop as a goal_status checking card', async () => {
            ...
            expect(mockAddItem).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'stop_hook_loop' }), expect.any(Number));
        });
