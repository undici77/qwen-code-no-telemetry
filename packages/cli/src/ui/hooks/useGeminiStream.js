...
terminalWidthRef, midTurnRestoreRef, goalQueueRef) => {
    ...
    const handledProviderToolCallIdsRef = useRef(new Set());
    ...
    const processGeminiStreamEvents = useCallback(async (stream, userMessageTimestamp, signal, submitType, turnAdmission) => {
        ...
        if (toolCallRequests.length > 0 &&
            ...
            const repeatedDuplicateRequest = findRepeatedDuplicateProviderToolCall(toolCallRequests, (request) => request.providerCallId, handledProviderIds, duplicateProviderToolCallResponseIdsRef.current);
    ...
    const submitQuery = useCallback(async (query, submitType = SendMessageType.UserQuery, prompt_id, metadata) => {
        ...
        return promptIdContext.run(prompt_id, async () => {
            ...
            try {
                ...
                const processingResult = await processGeminiStreamEvents(stream, userMessageTimestamp, processingSignal, submitType, turnAdmission);
                if (!goalBinding &&
                    turnAdmission &&
    ...
    const handleCompletedTools = useCallback(async (completedToolCallsFromScheduler) => {
        ...
        const completedCallIds = new Set(completedAndReadyToSubmitTools.map((toolCall) => toolCall.request.callId));
        ...
        const pendingDuplicateResponses = readyDuplicateBatches.flatMap((batch) => batch.duplicateResponses);
        ...
        if (terminatesGoalTurn && toolGoalBinding) {
            ...
            try {
                ...
                await failClosedGoalTurn(toolGoalBinding, `Goal turn could not finish: ${getErrorMessage(error)}`);
            }
            finally {
