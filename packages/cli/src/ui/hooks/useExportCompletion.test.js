...
import { CompletionMode, } from './useCommandCompletion.js';
import { getExportFormatFromInput, getNextExportCompletionIndex, useExportCompletion, } from './useExportCompletion.js';
const EXPORT_FORMATS = ['html', 'md', 'json', 'jsonl'];
...
describe('useExportCompletion', () => {
    it('returns null display props outside export cycling', () => {
        ...
        const { result } = renderHook(() => useExportCompletion(buffer, exportSlashCommands));
    ...
    it('does not seed cycling state from buffer text alone', () => {
        ...
        let consumed = true;
        act(() => {
            consumed = result.current.handleExportInput(createKey('down'), completion);
        });
    ...
    it('seeds cycling state after a marked user text edit', () => {
        ...
        const completion = createCompletion();
        const { result, rerender } = renderHook(({ textBuffer }) => useExportCompletion(textBuffer, exportSlashCommands), { initialProps: { textBuffer: buffer } });
        act(() => {
    ...
    it('shows export suggestions after phase-1 cycling updates the buffer', () => {
        ...
        expect(result.current.suggestionDisplayProps?.suggestions.map((s) => s.value)).toEqual(['html', 'md', 'json', 'jsonl']);
    });
...
//# sourceMappingURL=useExportCompletion.test.js.map