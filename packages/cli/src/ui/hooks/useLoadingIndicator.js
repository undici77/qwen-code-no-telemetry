...
export const useLoadingIndicator = (streamingState, customWittyPhrases, currentCandidatesTokens, currentStreamingChars, isToolExecuting = false) => {
    const [timerResetKey, setTimerResetKey] = useState(0);
    const isTimerActive = streamingState === StreamingState.Responding;
    const elapsedTimeFromTimer = useTimer(isTimerActive, timerResetKey, isTimerActive && isToolExecuting);
    ...
    const [taskStartStreamingChars, setTaskStartStreamingCharsState] = useState(0);
    const taskStartTokensRef = useRef(0);
    const taskStartStreamingCharsRef = useRef(0);
    const pauseStartTokensRef = useRef(null);
    const pauseStartStreamingCharsRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const setTaskStartTokens = (tokens) => {
        ...
        const setTaskStartStreamingChars = (chars) => {
        ...
        if (streamingState === StreamingState.Responding && isToolExecuting) {
            ...
            setTaskStartTokens(pauseBaseTaskStartTokensRef.current +
                Math.max(0, currentTokens - pauseStartTokensRef.current));
            setTaskStartStreamingChars(pauseBaseTaskStartStreamingCharsRef.current +
                Math.max(0, currentChars - (pauseStartStreamingCharsRef.current ?? 0)));
        }
        else {
