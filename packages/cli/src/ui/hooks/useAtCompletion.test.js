...
describe('useAtCompletion', () => {
    ...
    describe('File Search Logic', () => {
        ...
        it('should correctly filter the recursive list based on a pattern', async () => {
            ...
            expect(suggestionValues).toEqual(expect.arrayContaining([
                'src/',
                'src/components/',
                'src/components/Button.tsx',
                'src/index.js',
        ...
        it('should append a trailing slash to directory paths in suggestions', async () => {
            ...
            const dirSuggestion = result.current.suggestions.find((s) => s.value === 'dir/');
            const fileSuggestion = result.current.suggestions.find((s) => s.value === 'file.txt');
    ...
    describe('Filtering and Configuration', () => {
        ...
        it('should respect configured custom qwen ignore files', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, '', customIgnoreConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
            expect(result.current.suggestions.map((s) => s.value)).toEqual([
    ...
    describe('MCP resource completion', () => {
        it('suggests resource URIs for @server: when the server is configured', async () => {
            ...
            const resourceConfig = {
                ...
                getResourceRegistry: () => ({
                    getResourcesByServer: (name) => name === 'myserver'
                        ? [
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'myserver:res://a', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
        ...
        it('falls through to filesystem search when the prefix is not a configured server', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'notes', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
            // 'notes' has no ':' → not a resource pattern → filesystem search.
            expect(result.current.suggestions.map((s) => s.value)).toContain('notes.txt');
        });
        it('ranks prefix matches above mid-string matches', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'myserver:doc', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
        ...
        it('does not surface resource URIs in an untrusted folder', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'myserver:res', resourceConfig, testRootDir));
            ...
            expect(result.current.suggestions.map((s) => s.value)).not.toContain('myserver:res://secret');
        });
        it('resolves a server name containing a colon (longest-prefix)', async () => {
            ...
            const resourceConfig = {
                ...
                getResourceRegistry: () => ({
                    getResourcesByServer: (name) => name === 'my:server'
                        ? [{ uri: 'res://doc', name: 'd', serverName: 'my:server' }]
                        : [],
                }),
            };
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'my:server:res', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
            expect(result.current.suggestions.map((s) => s.value)).toEqual([
        ...
        it('matches a resource by its friendly name/title, not just the URI', async () => {
            ...
            const resourceConfig = {
                ...
                getMcpServers: () => ({ demo: {} }),
                getResourceRegistry: () => ({
                    getResourcesByServer: (name) => name === 'demo'
                        ? [
                            {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'demo:Project', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
        ...
        it('matches the URI case-insensitively', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'demo:spec', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
        ...
        it('ranks prefix above substring and URI above name across both fields', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'demo:doc', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
            expect(result.current.suggestions.map((s) => s.value)).toEqual([
    ...
    describe('MCP server mention completion', () => {
        it('suggests @mcp:<server> mentions for @mcp: queries', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'mcp:de', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
            expect(result.current.suggestions.map((s) => s.value)).toContain('mcp:demo');
            expect(result.current.suggestions.map((s) => s.value)).not.toContain('demo:');
        });
    });
    describe('MCP server discovery', () => {
        it('suggests matching servers (with resources) alongside files for a bare @<partial>', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'my', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
            ...
            const mentionSug = result.current.suggestions.find((s) => s.value === 'mcp:myserver');
            expect(mentionSug?.isDirectory).toBe(false);
            const serverSug = result.current.suggestions.find((s) => s.value === 'myserver:');
        ...
        it('suggests MCP mentions for the empty @ trigger alongside files', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, '', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
            const values = result.current.suggestions.map((s) => s.value);
        ...
        it('tags MCP server and resource suggestions with category mcp', async () => {
            ...
            const mcpSuggestions = result.current.suggestions.filter((s) => s.value === 'mcp:myserver' || s.value === 'myserver:');
    ...
    describe('Global MCP resource completion', () => {
        it('matches resources globally for a bare @<partial> with no server prefix', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'asight', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
        ...
        it('matches a resource globally by its friendly name/title', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'Project', resourceConfig, testRootDir));
            await waitFor(() => {
                expect(result.current.suggestions.length).toBeGreaterThan(0);
            });
            expect(result.current.suggestions.map((s) => s.value)).toContain('demo:file:///x/spec.md');
        });
        it('prepends globally-matched resources before file results', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'doc', resourceConfig, testRootDir));
            await waitFor(() => {
        ...
        it('does not surface global resources in an untrusted folder', async () => {
            ...
            const { result } = renderHook(() => useTestHarnessForAtCompletion(true, 'asight', resourceConfig, testRootDir));
            await new Promise((r) => setTimeout(r, 300));
            expect(result.current.suggestions.map((s) => s.value)).not.toContain('demo:asight://secret');
        });
    });
    describe('Session suggestions', () => {
        it('merges session suggestions into file search results', async () => {
            const structure = { 'file.txt': '' };
            testRootDir = await createTmpDir(structure);
            mockGetSessionSuggestions.mockResolvedValue([
                {
                    label: 'Fix auth bug',
        ...
        it('shows file results when getSessionSuggestions returns empty', async () => {
            ...
            expect(result.current.suggestions.map((s) => s.value)).toContain('file.txt');
            expect(result.current.suggestions.some((s) => s.category === 'session')).toBe(false);
        });
    });
});
