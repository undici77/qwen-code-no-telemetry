...
describe('useLoadingIndicator', () => {
    ...
    it('should pause elapsedTime while a tool is executing', async () => {
        const { result, rerender } = renderHook(({ isToolExecuting }) => useLoadingIndicator(StreamingState.Responding, undefined, 0, 0, isToolExecuting), { initialProps: { isToolExecuting: false } });
        await act(async () => {
            await vi.advanceTimersByTimeAsync(2000);
        });
    ...
    it('should exclude tokens produced while a tool is executing from the rate baseline', async () => {
        const { result, rerender } = renderHook(({ currentCandidatesTokens, currentStreamingChars, isToolExecuting }) => useLoadingIndicator(StreamingState.Responding, undefined, currentCandidatesTokens, currentStreamingChars, isToolExecuting), {
            initialProps: {
    ...
    describe('token tracking', () => {
        it('should capture token snapshot when task starts', () => {
            const { result, rerender } = renderHook(({ streamingState, currentCandidatesTokens, currentStreamingChars }) => useLoadingIndicator(streamingState, undefined, currentCandidatesTokens, currentStreamingChars), {
                initialProps: {
