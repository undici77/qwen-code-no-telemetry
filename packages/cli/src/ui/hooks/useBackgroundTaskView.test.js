...
function makeFakeRegistry() {
    ...
    return {
        ...
        setApprovalChangeCallback: vi.fn((next) => {
            approvalCb = next;
        }),
        fire: () => cb?.(),
...
function makeFakeMemoryManager() {
    ...
    const subscribe = vi.fn((next, opts) => {
        listener = next;
...
function makeConfig(opts) {
    ...
    const config = {
        ...
        getMemoryManager: () => ({
            ...
            listTasksByType: (_type, _projectRoot) => dreams(),
        }),
...
const agent = (id, startTime, overrides = {}) => ({
    id,
    kind: 'agent',
    agentId: id,
    description: 'desc',
...
const shell = (id, startTime, overrides = {}) => ({
    id,
    kind: 'shell',
    shellId: id,
    command: 'sleep 60',
...
const monitor = (id, startTime, overrides = {}) => ({
    id,
    kind: 'monitor',
    monitorId: id,
    command: 'tail -f log',
...
const dream = (id, startTimeMs, overrides = {}) => ({
    id,
    taskType: 'dream',
...
describe('useBackgroundTaskView', () => {
    ...
    it('subscribes to all registries on mount', () => {
        ...
        expect(agentReg.setApprovalChangeCallback).toHaveBeenCalledWith(expect.any(Function));
        expect(workflowReg.setApprovalChangeCallback).toHaveBeenCalledWith(expect.any(Function));
    });
    ...
    it('clears all registry subscriptions on unmount', () => {
        const { config, agentReg, shellReg, monitorReg, workflowReg, memoryMgr } = makeConfig({
            agents: () => [],
            shells: () => [],
            monitors: () => [],
        });
    ...
    it('caps retained terminal dream entries at 3 most-recent (by updatedAt) plus all running', () => {
        ...
        expect(ids).toEqual(['d-mid', 'd-newest', 'd-recent', 'd-running-now'].sort());
    });
