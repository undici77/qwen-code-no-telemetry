...
describe('useVim hook', () => {
    ...
    const createMockBuffer = (text = 'hello world', cursor = [0, 5]) => {
        ...
        return {
            ...
            vimMoveToLine: vi.fn().mockImplementation((lineNum) => {
                cursorState.pos = [
    ...
    describe('New vim commands', () => {
        describe('u (undo)', () => {
            it('should undo last operation', () => {
                ...
                const { result } = renderHook(() => useVim(buffer, mockHandleFinalSubmit));
        ...
        describe('J (join lines)', () => {
            it('should join current line with next line', () => {
                ...
                expect(buffer.replaceRange).toHaveBeenCalledWith(0, 0, 1, 5, 'hello world');
            });
        });
        ...
        describe('y (yank)', () => {
            it('should yank line with yy for later paste', () => {
                ...
                expect(buffer.replaceRange).toHaveBeenCalledWith(0, 11, 0, 11, '\nhello world');
            });
        });
        describe('Y (yank line)', () => {
            it('should yank entire line for later paste', () => {
                ...
                act(() => result.current.handleInput(makeKey('P')));
                expect(buffer.replaceRange).toHaveBeenCalledWith(0, 0, 0, 0, 'hello world\n');
            });
        });
    ...
    describe('Pending operator clearing', () => {
        it.each(['r', 'f', 'F', 't', 'T'])('should clear pending operator on Escape during char-read (%s)', (key) => {
            const buffer = createMockBuffer('hello world', [0, 0]);
            const { result } = renderHook(() => useVim(buffer, mockHandleFinalSubmit));
