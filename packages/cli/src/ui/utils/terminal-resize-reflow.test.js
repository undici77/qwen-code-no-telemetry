...
import { buildWakeRepaint, installTerminalResizeReflow, } from './terminal-resize-reflow.js';
...
function frame(width, rows, trailingNewline = false) {
...
class FakeStdout extends EventEmitter {
    ...
    written = [];
    write(chunk, cb) {
        this.written.push(typeof chunk === 'string' ? chunk : Buffer.from(chunk).toString());
...
describe('installTerminalResizeReflow', () => {
    ...
    it('amplifies the post-shrink erase to the reflowed frame height', () => {
        ...
        const { restore } = installTerminalResizeReflow(stdout);
        try {
    ...
    it('VP mode replaces the stale clear with a viewport clear', () => {
        const stdout = new FakeStdout();
        const { restore } = installTerminalResizeReflow(stdout, { virtualViewport: true });
        try {
            stdout.write(eraseLines(10) + frame(60, 10));
            stdout.columns = 30;
            stdout.emit('resize');
    ...
    it('repaint replays the last frame over a clean viewport', () => {
        const stdout = new FakeStdout();
        const { restore, repaint } = installTerminalResizeReflow(stdout, { virtualViewport: true });
        try {
            stdout.write(eraseLines(10) + frame(60, 10));
    ...
    it('QWEN_CODE_LEGACY_RESIZE_ERASE disables the wrapper', () => {
        ...
        try {
            const stdout = new FakeStdout();
            const handle = installTerminalResizeReflow(stdout);
            stdout.write(eraseLines(10) + frame(60, 10));
            stdout.columns = 30;
            stdout.emit('resize');
            stdout.write(eraseLines(10));
    ...
    it('models widths from ANSI-stripped content (SGR bytes are not cells)', () => {
        ...
        const { restore } = installTerminalResizeReflow(stdout);
        try {
            const styled = Array.from({ length: 10 }, () => `\x1b[31m${'x'.repeat(60)}\x1b[39m`).join('\n');
    ...
    it('expands tabs to 8-column stops when packing', () => {
        ...
        const { restore } = installTerminalResizeReflow(stdout);
        try {
            const tabbed = Array.from({ length: 10 }, () => '\t'.repeat(3) + 'x'.repeat(70)).join('\n');
    ...
    it('adjusts the return-to-bottom prefix when amplifying', () => {
        ...
        try {
            ...
            expect(stdout.written.at(-1)).toBe('\x1b[?25l\x1b[12B\x1b[0G' + eraseLines(20));
        }
        finally {
            restore();
        }
    ...
    it('amplifies end-to-end when stacked inside the redraw optimizer', () => {
        ...
        const optimizer = installTerminalRedrawOptimizer(stdout);
        const reflow = installTerminalResizeReflow(stdout);
        try {
            stdout.write(eraseLines(10) + frame(60, 10));
            stdout.columns = 30;
            stdout.emit('resize');
    ...
    it('wrapper restores unwind in LIFO order only', () => {
        ...
        const sync = installSynchronizedOutput(stdout, { QWEN_CODE_FORCE_SYNCHRONIZED_OUTPUT: '1' });
        const reflow = installTerminalResizeReflow(stdout);
        reflow.restore();
        sync();
        optimizer();
        ...
        const optimizer2 = installTerminalRedrawOptimizer(stdout2);
        const sync2 = installSynchronizedOutput(stdout2, { QWEN_CODE_FORCE_SYNCHRONIZED_OUTPUT: '1' });
...
//# sourceMappingURL=terminal-resize-reflow.test.js.map