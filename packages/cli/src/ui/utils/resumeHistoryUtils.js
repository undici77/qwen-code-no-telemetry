...
import { getToolResponseDisplayText, isGoalCheckpointBookkeepingRecord, parseGoalStateRecordPayloadV2, projectUserTranscriptForDisplay, } from '@qwen-code/qwen-code-core';
    ...
    for (const record of conversation.messages) {
        ...
        switch (record.type) {
            case 'user': {
                ...
                const projection = projectUserTranscriptForDisplay(record);
                const text = projection.displayText ?? extractTextFromParts(projection.parts);
                if (text) {
                    items.push({ type: 'user', text });
                }
                break;
