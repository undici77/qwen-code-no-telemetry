...
`.replace(/\n/g, eol);
        ...
        it('continues gutter numbering for a fence carrying the start-line directive', () => {
            ...
            const text = '```javascript qwen-code:start-line=17\nconst a = 1;\nconst b = 2;\n```'.replace(/\n/g, eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text }));
        ...
        it('clips a long pending message to availableTerminalHeight', () => {
            ...
            const text = Array.from({ length: 60 }, (_, i) => `line ${i + 1}`).join(eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: true, availableTerminalHeight: 10 }));
        ...
        it('does not pad a short pending message up to availableTerminalHeight', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: "just one short line", isPending: true, availableTerminalHeight: 20 }));
        ...
        it('does not clip a long committed (non-pending) message', () => {
            ...
            const text = Array.from({ length: 60 }, (_, i) => `line ${i + 1}`).join(eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: false, availableTerminalHeight: 10 }));
        ...
        it('clips a non-pending message when enforceHeightBudget is set (#6867)', () => {
            ...
            // path uses.
            const text = Array.from({ length: 60 }, (_, i) => `line ${i + 1}`).join(eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: false, availableTerminalHeight: 10, enforceHeightBudget: true }));
            const output = lastFrame() ?? '';
            const lineCount = output.split('\n').length;
        ...
        it('leaves a non-pending message full when enforceHeightBudget is false', () => {
            ...
            const text = Array.from({ length: 60 }, (_, i) => `line ${i + 1}`).join(eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: false, availableTerminalHeight: 10, enforceHeightBudget: false }));
            expect(lastFrame() ?? '').toContain('line 60');
        });
        ...
        it('does not stack a double cue for a math block near the clip boundary', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: true, availableTerminalHeight: 8 }));
            const output = lastFrame() ?? '';
            expect(output.match(/generating more/g) ?? []).toHaveLength(0);
        ...
        it('does not clip a pending message when no height budget is given', () => {
            ...
            const text = Array.from({ length: 60 }, (_, i) => `line ${i + 1}`).join(eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: true, availableTerminalHeight: undefined }));
            const output = lastFrame() ?? '';
        ...
        it('applies the minimum floor at a degenerate budget of 1', () => {
            ...
            const text = Array.from({ length: 60 }, (_, i) => `line ${i + 1}`).join(eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: true, availableTerminalHeight: 1 }));
            const output = lastFrame() ?? '';
            expect(output).toContain('line 1');
...
+ item C
`.replace(/\n/g, eol);
            const { lastFrame, stdout } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text }));
            expect(lastFrame()).toMatchSnapshot();
            expect(copiedFrame(stdout)).toContain('- item A\n* item B\n+ item C');
        });
...
`.replace(/\n/g, eol);
            const { lastFrame, stdout } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text }));
            expect(lastFrame()).toMatchSnapshot();
            expect(copiedFrame(stdout)).toContain('│ Cell 1   │  Cell 2  │');
        });
...
|---|---|
| one | tw`.replace(/\n/g, eol);
            const { lastFrame: partialFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: partial, isPending: true }));
            expect(partialFrame() ?? '').not.toContain('one');
            // Once the row terminates, the table appears complete.
            const complete = `| A | B |
|---|---|
| one | two |`.replace(/\n/g, eol);
            const { lastFrame: completeFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: complete, isPending: true }));
...
|---|
| ${tallCell} |`.replace(/\n/g, eol);
            const streaming = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: true, contentWidth: 60 })).lastFrame() ?? '';
            const committed = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: false, contentWidth: 60 })).lastFrame() ?? '';
...
| one | two |
| three | fo`.replace(/\n/g, eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: false }));
...
\`\`\`
| ZZZ | YYY |`.replace(/\n/g, eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: true, availableTerminalHeight: 20 }));
...
trailing text`.replace(/\n/g, eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: true, availableTerminalHeight: 40 }));
...
\`\`\`
`.replace(/\n/g, eol);
            const { lastFrame } = renderWithProviders(_jsx(RenderModeProvider, { value: {
                    renderMode: 'raw',
...
$$
`.replace(/\n/g, eol);
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, sourceCopyIndexOffsets: {
...
`.replace(/\n/g, eol);
    ...
    describe('pending render-height safety net', () => {
        ...
        it('draws a streaming table live (no placeholder) when it fits the viewport', () => {
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: tableText, isPending: true }));
            const output = lastFrame() ?? '';
        ...
        it('renders a completed (closed) table in full even while pending', () => {
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: `${tableText}\n\nDone.`, isPending: true }));
            const output = lastFrame() ?? '';
            expect(output).not.toContain('generating table');
            expect(output).toContain('┌');
            expect(output).toContain('one');
        ...
        it('keeps a completed table + trailing text within the viewport (no overflow)', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: true, availableTerminalHeight: 12 }));
            const output = (lastFrame() ?? '').replace(/\n+$/, '');
        ...
        it('counts wide/CJK wrapping so a pending block stays within the viewport', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(MarkdownDisplay, { ...baseProps, text: text, isPending: true, contentWidth: 20, availableTerminalHeight: 10 }));
            const output = (lastFrame() ?? '').replace(/\n+$/, '');
            expect(output.split('\n').length).toBeLessThanOrEqual(10);
        });
    });
