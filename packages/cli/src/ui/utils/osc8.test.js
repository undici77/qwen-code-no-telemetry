...
import { BARE_URL_PATTERN, HYPERLINK_ENV_KEYS, isSafeOscScheme, labelMayDeceive, osc8Close, osc8Hyperlink, osc8Open, sanitizeForOsc, supportsHyperlinks, trimTrailingUrlPunctuation, } from './osc8.js';
const ESC = '\x1b';
const BEL = '\x07';
...
describe('osc8 helpers', () => {
    ...
    describe('sanitizeForOsc', () => {
        ...
        it('strips Unicode bidi controls (label spoofing)', () => {
            ...
            expect(sanitizeForOsc('a\u200eb\u200fc\u202ad\u202be\u202cf\u202dg\u202eh\u2066i\u2067j\u2068k\u2069l')).toBe('abcdefghijkl');
        });
        ...
        it('keeps printable ASCII and unicode intact', () => {
            expect(sanitizeForOsc('https://example.com/路径?q=v')).toBe('https://example.com/路径?q=v');
        });
    });
    describe('osc8Hyperlink', () => {
        it('emits the canonical OSC 8 envelope with BEL terminators', () => {
            expect(osc8Hyperlink('https://example.com', 'click me')).toBe(`${ESC}]8;;https://example.com${BEL}click me${ESC}]8;;${BEL}`);
        });
        it('defaults the label to the url', () => {
            expect(osc8Hyperlink('https://example.com')).toBe(`${ESC}]8;;https://example.com${BEL}https://example.com${ESC}]8;;${BEL}`);
        });
        ...
        it('produces an envelope that composes from osc8Open + osc8Close', () => {
            expect(osc8Open('https://x.test') + 'label' + osc8Close()).toBe(osc8Hyperlink('https://x.test', 'label'));
        });
    });
    ...
    describe('labelMayDeceive', () => {
        it('flags labels containing a different URL than the target', () => {
            ...
            expect(labelMayDeceive('https://google.com', 'https://attacker.com')).toBe(true);
            expect(labelMayDeceive('Visit https://safe.com now', 'https://evil.com/x')).toBe(true);
        ...
        it('flags bare-host labels whose host differs from the target', () => {
            ...
            expect(labelMayDeceive('paypal.com', 'https://phish.example.org/login')).toBe(true);
            // Punycode IDN attack — label `google.com`, target is the lookalike
            // punycode host. Label host does not equal target host → flag.
            expect(labelMayDeceive('google.com', 'https://xn--googl-fsa.com')).toBe(true);
        });
        it('does NOT flag bare-host labels that match the target', () => {
            // Honest rendering: label is the same host the URL points to.
            expect(labelMayDeceive('google.com', 'https://google.com/page')).toBe(false);
            expect(labelMayDeceive('docs.example.com', 'https://docs.example.com/x')).toBe(false);
        });
        it('flags IPv4 literal labels whose host differs from the target', () => {
            ...
            expect(labelMayDeceive('Click 8.8.8.8 here', 'https://evil.com')).toBe(true);
        });
        ...
        it('does NOT flag email-style labels matching the mailto target', () => {
            ...
            expect(labelMayDeceive('support@example.com', 'mailto:support@example.com')).toBe(false);
            // …but a mismatched mailto domain IS deceptive.
            expect(labelMayDeceive('support@example.com', 'mailto:abuse@evil.com')).toBe(true);
        });
        it('flags same-host different-path labels', () => {
            ...
            expect(labelMayDeceive('https://google.com/safe', 'https://google.com/evil')).toBe(true);
        });
    ...
    describe('trimTrailingUrlPunctuation', () => {
        ...
        it('trims an unbalanced trailing close-paren', () => {
            expect(trimTrailingUrlPunctuation('https://example.com/x)')).toBe('https://example.com/x');
        });
...
//# sourceMappingURL=osc8.test.js.map