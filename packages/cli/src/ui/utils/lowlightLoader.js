 ...
 */
export function isLowlightCoolingDown() {
    return (lowlightLastFailureAt > 0 &&
...
export function loadLowlight() {
    if (lowlightInstance)
        return Promise.resolve(lowlightInstance);
    ...
    lowlightLoad = import('lowlight')
        ...
        if (typeof instance?.registered !== 'function' ||
            ...
            throw new Error('lowlight instance does not match expected API (registered/highlight/highlightAuto)');
        }
