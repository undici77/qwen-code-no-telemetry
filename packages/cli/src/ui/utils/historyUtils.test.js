...
import { buildThoughtHeadIdMap, findLastUserItemIndex, isSyntheticHistoryItem, itemsAfterAreOnlySynthetic, realUserPromptTexts, } from './historyUtils.js';
const mk = (overrides, id = 1) => ({ id, ...overrides });
describe('isSyntheticHistoryItem', () => {
    it('treats info/error/warning/success/retry/vision_notice/notification/summary/thought as synthetic', () => {
        for (const type of [
            ...
            expect(isSyntheticHistoryItem(mk({ type, text: 'x' }))).toBe(true);
        }
    });
    it('treats assistant text and tool runs as meaningful', () => {
        expect(isSyntheticHistoryItem(mk({ type: 'gemini', text: 'hi' }))).toBe(false);
        expect(isSyntheticHistoryItem(mk({ type: 'gemini_content', text: 'hi' }))).toBe(false);
        expect(isSyntheticHistoryItem(mk({
            type: 'tool_group',
            tools: [
                {
                    callId: 'a',
    ...
    it('treats regular user items as meaningful', () => {
        expect(isSyntheticHistoryItem(mk({ type: 'user', text: 'hello' }))).toBe(false);
        expect(isSyntheticHistoryItem(mk({ type: 'user', text: 'hi', sentToModel: true }))).toBe(false);
    });
    it('treats steer items (sentToModel === false) as synthetic', () => {
        expect(isSyntheticHistoryItem(mk({ type: 'user', text: 'steer', sentToModel: false }))).toBe(true);
    });
...
describe('findLastUserItemIndex', () => {
    it('returns -1 when no user item exists', () => {
        expect(findLastUserItemIndex([mk({ type: 'info', text: 'x' })])).toBe(-1);
    });
...
//# sourceMappingURL=historyUtils.test.js.map