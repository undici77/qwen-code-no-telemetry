...
describe('<RenderInline />', () => {
    ...
    it('leaves shell-style dollar variables untouched by default', () => {
        const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: "echo $HOME && echo $PATH" }));
        expect(lastFrame()).toContain('echo $HOME && echo $PATH');
    });
    it('renders inline math only when explicitly enabled', () => {
        const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: "value $\\\\alpha$", enableInlineMath: true }));
        expect(lastFrame()).toContain('α');
        expect(lastFrame()).not.toContain('$\\alpha$');
    });
    it('renders single-character and CJK-adjacent inline math', () => {
        const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: "Values $x$\u3001$\u03B1$\u3002", enableInlineMath: true }));
    ...
    it('renders escaped dollars as prose while keeping inline code verbatim', () => {
        const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: 'Literal \\$xy$ and code `$xy$ and \\$xy$`', enableInlineMath: true }));
        expect((lastFrame() ?? '').replace(/\n/g, '')).toContain('Literal $xy$ and code$xy$ and \\$xy$');
    });
...
math then literal: $x^2\$$`;
        const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: text, enableInlineMath: true }));
        const output = (lastFrame() ?? '').replace(/\n/g, '');
    ...
    it('keeps math literal inside multi-backtick code spans', () => {
        const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: 'Use ``a `$xy$` b`` then $y$', enableInlineMath: true }));
    ...
    it('measures recognized single-character math without delimiters', () => {
        ...
        expect(getPlainTextLength('code ``a `$xy$` b``', true)).toBe('code a `$xy$` b'.length);
        expect(getPlainTextLength(String.raw `literal \$x$`, true)).toBe('literal $x$'.length);
        expect(getPlainTextLength(String.raw `formula $x + \$5$`, true)).toBe('formula x + $5'.length);
        expect(getPlainTextLength(String.raw `even \\$x$`, true)).toBe('even \\x'.length);
    });
    it('does not parse ordinary dollar amounts as inline math', () => {
        const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: "cost is $5 and $10 later", enableInlineMath: true }));
    ...
    describe('markdown link OSC 8 wrapping', () => {
        ...
        it('wraps a safe http(s) link and shows only the label (no `(url)` suffix)', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `click [here](${url}) please` }));
            const out = lastFrame() ?? '';
        ...
        it('falls back to showing the URL as label when [](url) has empty label', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `go [](${url}) home` }));
            const out = lastFrame() ?? '';
            expect(out).toContain(`\x1b]8;;${url}\x07`);
        ...
        it('does not wrap dangerous schemes (javascript:, data:, file:, …)', () => {
            ...
            for (const url of [
                ...
                const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `click [bad](${url}) end` }));
                const out = lastFrame() ?? '';
        ...
        it('falls back to plain "label (url)" rendering on unsupported terminals', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `see [docs](${url})` }));
            const out = lastFrame() ?? '';
        ...
        it('wraps bare URLs in an OSC 8 envelope when supported', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `go to ${url} now` }));
            const out = lastFrame() ?? '';
            expect(out).toContain(`\x1b]8;;${url}\x07`);
            expect(out).toContain(url);
            expect(out).toContain('\x1b]8;;\x07');
        ...
        it('trims trailing sentence punctuation from the OSC 8 target only', () => {
            enableHyperlinks();
            const url = 'https://example.com/page';
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `see ${url}.` }));
            const out = lastFrame() ?? '';
        ...
        it('stops the bare-URL hyperlink at glued-on full-width CJK punctuation', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `PR：${url}（2 commits，等 CI）` }));
            const out = lastFrame() ?? '';
        ...
        it('does not treat underscores around a later URL as emphasis', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `见 ${firstUrl}（说明_1）和 ${secondUrl}_。` }));
            const out = lastFrame() ?? '';
        ...
        it('preserves dunder identifiers as visible text', () => {
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: "Python \u7684 __init__ \u65B9\u6CD5" }));
        ...
        it('leaves bare URLs unwrapped when unsupported', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `visit ${url}` }));
            const out = lastFrame() ?? '';
            expect(out).not.toContain('\x1b]8;;');
            expect(out).toContain(url);
        });
        it('refuses to emit OSC 8 inside tmux without FORCE_HYPERLINK', () => {
            ...
            const url = 'https://example.com/page';
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `[ref](${url})` }));
            const out = lastFrame() ?? '';
            expect(out).not.toContain('\x1b]8;;');
        });
        it('preserves balanced parens inside the link URL (Wikipedia-style)', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `see [wiki](${url}) ok` }));
            const out = lastFrame() ?? '';
        ...
        it('does not wrap a URL that contains whitespace', () => {
            ...
            enableHyperlinks();
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: "see [doc](https://x.com path with space) end" }));
            const out = lastFrame() ?? '';
            expect(out).not.toContain('\x1b]8;;');
        ...
        it('does not wrap a URL containing NBSP / Unicode whitespace', () => {
            ...
            enableHyperlinks();
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `[a](https://x.com\u00a0b)` }));
            const out = lastFrame() ?? '';
            expect(out).not.toContain('\x1b]8;;');
        });
        it('trims a trailing `>` from a CommonMark autolink URL target', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `see <${url}> ok` }));
            const out = lastFrame() ?? '';
            expect(out).toContain(`\x1b]8;;${url}\x07`);
            expect(out).not.toContain(`\x1b]8;;${url}>\x07`);
        });
        it('mid-stream unclosed-link state emits a well-formed envelope, not a half-envelope', () => {
            ...
            enableHyperlinks();
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: "partial [foo](https://example.com/page" }));
            const out = lastFrame() ?? '';
        ...
        it('chunked stream finalizes to a single full link envelope', () => {
            enableHyperlinks();
            const url = 'https://example.com/page';
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `done [foo](${url}) ok` }));
            const out = lastFrame() ?? '';
        ...
        it('sanitizes bidi controls in the URL when used as visible text', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `see [](${dirtyUrl})` }));
            const out = lastFrame() ?? '';
        ...
        it('sanitizes bidi controls in the visible label (anti-spoof)', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `click [${spoofLabel}](${url}) end` }));
            const out = lastFrame() ?? '';
            expect(out).toContain(`\x1b]8;;${url}\x07`);
        ...
        it('keeps the `(url)` suffix when the label looks like a mismatched URL', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `click [https://google.com/auth](${target}) end` }));
            const out = lastFrame() ?? '';
        ...
        it('keeps the `(url)` suffix for bare-host labels (e.g. `google.com`)', () => {
            ...
            enableHyperlinks();
            const target = 'https://attacker.com/phish';
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `go to [google.com](${target}) end` }));
            const out = lastFrame() ?? '';
            expect(out).toContain(`\x1b]8;;${target}\x07`);
        ...
        it('elides `(url)` when label==url (no deception risk)', () => {
            ...
            enableHyperlinks();
            const url = 'https://example.com/page';
            const { lastFrame } = renderWithProviders(_jsx(RenderInline, { text: `see [${url}](${url}) end` }));
            const out = lastFrame() ?? '';
            expect(out).toContain(`\x1b]8;;${url}\x07`);
