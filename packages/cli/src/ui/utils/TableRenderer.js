...
function updateActiveForeground(activeForeground, paramsText) {
...
function readSgrSequence(text, index) {
...
function preserveForegroundAcrossLineBreaks(text) {
 ...
 */
function renderMarkdownToAnsi(text, enableInlineMath = false) {
    ...
    for (const token of mergeInlineMathMatches(text, INLINE_MARKDOWN_REGEX, enableInlineMath)) {
        ...
        if (token.kind === 'math') {
            result += applyColor(renderInlineLatex(unescapeMarkdownDollars(token.span.content)), theme.text.accent);
        ...
        if (fullMatch.startsWith('**') &&
            ...
            rendered = ansiFmt.italic(unescapeMarkdownDollars(fullMatch.slice(1, -1)));
        }
        else if (fullMatch.startsWith('~~') &&
            fullMatch.endsWith('~~') &&
            fullMatch.length > 4) {
            rendered = ansiFmt.strikethrough(unescapeMarkdownDollars(fullMatch.slice(2, -2)));
        }
        else if (fullMatch.startsWith('`') &&
            fullMatch.endsWith('`') &&
            fullMatch.length > 1) {
            ...
            fullMatch.length > 7) {
            rendered = ansiFmt.underline(unescapeMarkdownDollars(fullMatch.slice(3, -4)));
        }
        else if (/^https?:\/\//.test(fullMatch)) {
            const visible = applyColor(fullMatch, theme.text.link);
            if (canHyperlink) {
                const trimmedUrl = trimTrailingUrlPunctuation(fullMatch, text[index + fullMatch.length]);
 ...
 */
export const TableRenderer = ({ headers, rows, contentWidth, aligns, enableInlineMath = false, isStreaming = false, maxHeight, }) => {
    ...
    function calculateMaxRowLines() {
        ...
        for (let i = 0; i < colCount; i++) {
            const headerWrapped = wrapText(headerMetrics[i].rendered, columnWidths[i], {
                hard: needsHardWrap,
            });
    ...
    const minHorizontalTableWidth = Math.max(ABSOLUTE_MIN_HORIZONTAL_TABLE_WIDTH, colCount * MIN_COLUMN_WIDTH + borderOverhead + SAFETY_MARGIN);
    ...
    const useVerticalFormat = rowMetrics.length > 0 &&
    ...
    if (useVerticalFormat) {
        return (_jsx(Box, { marginY: 1, children: _jsx(Text, { children: clampToMaxHeight(renderVerticalFormat()) }) }));
    }
    ...
    return (_jsx(Box, { flexDirection: "column", marginY: 1, children: _jsx(Text, { children: clampToMaxHeight(tableLines.join('\n')) }) }));
};
