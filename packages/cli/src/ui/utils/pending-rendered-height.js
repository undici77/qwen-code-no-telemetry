...
export const TABLE_SEPARATOR_RE = /^(?=.*\|)\s*\|?\s*(:?-+:?)\s*(\|\s*(:?-+:?)\s*)*\|?\s*$/;
...
export function estimateWrappedRows(line, width) {
    if (width <= 0)
        return 1;
...
export function isTableStart(lines, i) {
    ...
    if (header === undefined || i + 1 >= lines.length)
        return false;
    ...
    const sepCols = splitMarkdownTableRow(separator).filter((cell) => cell.length > 0).length;
 ...
 */
export function fitPendingSlice(allLines, contentWidth, budget, tableClampRows) {
    let rendered = 0;
    ...
    for (let i = 0; i < allLines.length;) {
        ...
        if (isTableStart(allLines, i)) {
            let j = i + 2;
            while (j < allLines.length && TABLE_ROW_RE.test(allLines[j]))
                j++;
            ...
            const headerCells = splitMarkdownTableRow(TABLE_ROW_RE.exec(allLines[i])[1]);
            ...
            // its floor of 3.
            const perColWidth = Math.max(3, Math.floor((contentWidth - (1 + 3 * colCount) - 4) / colCount));
            ...
            for (const label of headerCells) {
                maxRowLines = Math.max(maxRowLines, estimateWrappedRows(label, perColWidth));
            }
            ...
            for (let r = i + 2; r < j; r++) {
                const cells = splitMarkdownTableRow(TABLE_ROW_RE.exec(allLines[r])[1]);
                let rowMax = 1;
                for (let colIdx = 0; colIdx < cells.length; colIdx++) {
                    ...
                    verticalRows += estimateWrappedRows(label ? `${label}: ${value}` : value, contentWidth);
                }
                contentRows += rowMax;
                if (r === i + 2) {
...
//# sourceMappingURL=pending-rendered-height.js.map