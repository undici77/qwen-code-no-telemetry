...
import { containsCmdShellMetacharacters, getTerminalImageRenderSupport, MAX_INLINE_IMAGE_PIXELS, markKittyImageWritten, prepareInlineTerminalImage, renderTerminalImage, supportsKittyImageProtocol, TRANSMITTED_KEY_LIMIT, wasKittyImageWritten, } from './terminal-image-renderer.js';
...
describe('terminalImageRenderer', () => {
    ...
    it('validates inline PNG payloads before rendering', () => {
        for (const testCase of [
            ...
            expect(prepareInlineTerminalImage({
                data: testCase.data,
    ...
    it('exposes a stable render key on Kitty results so remounts can skip re-transmission', async () => {
        ...
        if (first.kind !== 'kitty' || second.kind !== 'kitty')
            return;
    ...
    it('tracks transmitted Kitty images for the session and bounds the set', () => {
        ...
        expect(wasKittyImageWritten(`${unique}0`)).toBe(false);
        expect(wasKittyImageWritten(`${unique}${TRANSMITTED_KEY_LIMIT}`)).toBe(true);
    });
    it.each(IMAGE_SIZE_CASES)('%s', async (_name, imageWidth, imageHeight, contentWidth, availableTerminalHeight, expectedWidth, expectedRows) => {
        ...
        expect(result.placeholder.lines[0].split('\u{10EEEE}').length - 1).toBe(expectedWidth);
    });
    it('disables native placement in tmux, SSH, and non-TTY output', () => {
        expect(supportsKittyImageProtocol({ TERM: 'xterm-kitty' }, true)).toBe(true);
        expect(supportsKittyImageProtocol({ TERM: 'xterm-kitty', TMUX: '/tmp/tmux' }, true)).toBe(false);
        expect(supportsKittyImageProtocol({ TERM: 'xterm-kitty', SSH_TTY: '/dev/pts/1' }, true)).toBe(false);
        expect(supportsKittyImageProtocol({ TERM: 'xterm-kitty', SSH_CLIENT: '10.0.0.1 51234 22' }, true)).toBe(false);
        expect(supportsKittyImageProtocol({ TERM: 'xterm-kitty' }, false)).toBe(false);
    });
    it('detects Kitty and Ghostty native placement', () => {
        expect(supportsKittyImageProtocol({ KITTY_WINDOW_ID: '1', TERM: 'xterm-256color' }, true)).toBe(true);
        expect(supportsKittyImageProtocol({ TERM_PROGRAM: 'ghostty' }, true)).toBe(true);
        expect(supportsKittyImageProtocol({ TERM_PROGRAM: 'ghostty', TMUX: '/tmp/tmux' }, true)).toBe(false);
    });
    it('does not use Kitty Unicode placeholders in Warp', () => {
        expect(supportsKittyImageProtocol({ TERM: 'xterm-256color', TERM_PROGRAM: 'WarpTerminal' }, true)).toBe(false);
        expect(supportsKittyImageProtocol({ TERM: 'xterm-kitty', TERM_PROGRAM: 'WarpTerminal' }, true)).toBe(false);
        expect(getTerminalImageRenderSupport({
            PATH: tempDir,
            TERM: 'xterm-256color',
    ...
    it.runIf(process.platform !== 'win32')('falls back to chafa symbol output', async () => {
        ...
        expect(getTerminalImageRenderSupport({
            ...
            TERM: 'xterm-256color',
            TERM_PROGRAM: 'WarpTerminal',
        }, true)).toEqual({ available: true });
    });
    it.runIf(process.platform !== 'win32')('passes inline PNG bytes to chafa over stdin', async () => {
    ...
    it.runIf(process.platform !== 'win32')('does not trust a project-local node_modules/.bin/chafa', async () => {
    ...
    it.runIf(process.platform !== 'win32')('surfaces chafa stderr when rendering fails', async () => {
        const binDir = path.join(tempDir, 'bin');
        await fs.mkdir(binDir);
        const chafaPath = path.join(binDir, 'chafa');
        ...
        expect(result.kind === 'unavailable' && result.reason).toContain('libpng: invalid IHDR data');
    });
    it.runIf(process.platform !== 'win32')('bounds a verbose chafa failure to a capped first line', async () => {
        const binDir = path.join(tempDir, 'bin');
        await fs.mkdir(binDir);
        const chafaPath = path.join(binDir, 'chafa');
    ...
    it.runIf(process.platform !== 'win32')('caches a render so resize and restore do not re-spawn chafa', async () => {
        const binDir = path.join(tempDir, 'bin');
        await fs.mkdir(binDir);
        const chafaPath = path.join(binDir, 'chafa');
    ...
    it('returns a readable fallback when chafa is unavailable', () => {
        ...
        expect(result.kind).toBe('unavailable');
        expect(result.kind === 'unavailable' && result.reason).toContain('chafa is not installed');
    ...
    it('rejects missing and invalid PNG files during restored rendering', async () => {
        ...
        expect(oversized.kind === 'unavailable' && oversized.reason).toContain('display limit');
    });
});
...
//# sourceMappingURL=terminal-image-renderer.test.js.map