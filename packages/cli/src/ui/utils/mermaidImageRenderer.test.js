...
import { buildKittyPlaceholder, detectTerminalImageProtocol, encodeITerm2InlineImage, encodeKittyImage, encodeKittyVirtualImage, readPngSize, renderMermaidImageAsync, renderMermaidImageSync, } from './mermaidImageRenderer.js';
...
function createFakeMmdc(binDir, bodyLines) {
    const fakeMmdcScript = path.join(binDir, 'fake-mmdc.cjs');
    const defaultBodyLines = [
        'const fs = require("node:fs");',
        'const out = process.argv[process.argv.indexOf("-o") + 1];',
        `fs.writeFileSync(out, Buffer.from("${PNG_1X1.toString('base64')}", "base64"));`,
    ];
    fs.writeFileSync(fakeMmdcScript, (bodyLines ?? defaultBodyLines).join('\n'), 'utf8');
    const fakeMmdc = process.platform === 'win32'
...
function createFakeChafa(binDir, bodyLines) {
    ...
    const fakeChafa = process.platform === 'win32'
...
describe('mermaid image renderer', () => {
    it('keeps external image rendering disabled unless explicitly enabled', () => {
        ...
        expect(result.kind === 'unavailable' && result.reason).toContain('disabled by default');
    });
    it('does not auto-discover repo-local renderers from the current working directory', () => {
        ...
        try {
            ...
            expect(result.kind).toBe('unavailable');
            expect(result.kind === 'unavailable' && result.reason).toContain('Mermaid CLI (mmdc) was not found');
        }
        finally {
    ...
    it('does not auto-discover node_modules renderers from PATH without opt-in', () => {
        ...
        tempDirs.push(tempDir);
        const binDir = path.join(tempDir, 'packages', 'app', 'node_modules', '.bin');
        fs.mkdirSync(binDir, { recursive: true });
    ...
    it('detects forced terminal image protocols', () => {
        expect(detectTerminalImageProtocol({
            QWEN_CODE_MERMAID_IMAGE_PROTOCOL: 'kitty',
        })).toBe('kitty');
        expect(detectTerminalImageProtocol({
    ...
    it('encodes PNG data for terminal image protocols', () => {
        ...
        expect(encodeITerm2InlineImage(PNG_1X1, 40, 10)).toContain('\u001b]1337;File=inline=1;width=40;height=10;');
        expect(encodeKittyImage(PNG_1X1, 40, 10)).toContain('\u001b_Ga=T,f=100,c=40,r=10,');
        expect(encodeKittyVirtualImage(PNG_1X1, 42, 40, 10)).toContain('\u001b_Ga=T,f=100,i=42,q=2,U=1,c=40,r=10,');
    });
    ...
    it('renders Mermaid through mmdc when terminal images are available', () => {
        ...
        expect(result.kind === 'terminal-image' && result.sequence).toContain('\u001b]1337;File=inline=1;');
    });
    ...
    it('honors the Mermaid image disable flag over chafa fallback rendering', () => {
        ...
        expect(result.kind).toBe('unavailable');
        expect(result.kind === 'unavailable' && result.reason).toContain('QWEN_CODE_DISABLE_MERMAID_IMAGES');
    });
    ...
    it('bounds retained renderer output from async command failures', async () => {
        ...
        expect(result.kind).toBe('unavailable');
        expect(result.kind === 'unavailable' && result.reason.length).toBeLessThan(17 * 1024);
        expect(result.kind === 'unavailable' && result.reason).toContain('renderer output truncated');
    });
    ...
    it('cancels async Mermaid CLI rendering when the caller aborts', async () => {
        ...
        expect(result.kind).toBe('unavailable');
        expect(result.kind === 'unavailable' && result.reason).toContain('cancelled');
        createFakeMmdc(binDir);
    ...
    it('renders Kitty terminal images as virtual placements', () => {
        ...
        expect(result.kind).toBe('terminal-image');
        expect(result.kind === 'terminal-image' && result.protocol).toBe('kitty');
        expect(result.kind === 'terminal-image' && result.sequence).toContain('q=2,U=1');
        expect(result.kind === 'terminal-image' && result.placeholder).toBeTruthy();
        expect(result.kind === 'terminal-image' && result.placeholder?.lines[0]).toContain('\u{10EEEE}');
    });
    it('rejects oversized Mermaid PNG output before reading it', () => {
        ...
        expect(result.kind).toBe('unavailable');
        expect(result.kind === 'unavailable' && result.reason).toContain('exceeded');
    });
    it('evicts Mermaid image caches by retained byte size', () => {
        ...
        createFakeMmdc(binDir, [
            ...
            'const out = process.argv[process.argv.indexOf("-o") + 1];',
            `fs.writeFileSync(out, Buffer.concat([Buffer.from("${PNG_1X1.toString('base64')}", "base64"), Buffer.alloc(7 * 1024 * 1024)]));`,
        ]);
        const env = {
            PATH: `${binDir}${path.delimiter}${process.env['PATH'] ?? ''}`,
            QWEN_CODE_MERMAID_IMAGE_RENDERING: '1',
...
//# sourceMappingURL=mermaidImageRenderer.test.js.map