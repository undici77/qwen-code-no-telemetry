...
describe('<TableRenderer />', () => {
    ...
    const foregroundAtText = (output, text) => {
        const line = output
            .split('\n')
    ...
    const expectWrappedContinuation = (output, wholeText, continuationText) => {
        ...
        expect(continuationLine, `Expected rendered output to wrap before "${continuationText}"`).toBeDefined();
    };
    ...
    it('keeps all rendered lines at the same visible width for mixed content', () => {
        const output = renderTable(['项目', 'ANSI', 'Markdown'], [['中文内容', '\u001b[31mRed\u001b[0m Blue', '**bold** and `code`']], 80, ['left', 'center', 'right']);
        expectAllLinesToHaveSameVisibleWidth(output);
    });
    ...
    it('renders CJK-heavy table that would previously be misaligned', () => {
        ...
        const output = renderTable(['对比项', 'Claude Code', 'Qwen Code'], [
            ['性能', '优秀', '优秀'],
            ['中文支持', '一般', '很好'],
            ['开源', '否', '是'],
        ], 50);
    ...
    it('preserves truecolor inline-code foreground across wrapped lines', () => {
        const tableName = 'deleted_t_spark_odps_sql_type_system2_test_view_more_times_expand_view_f44c82c06096_244650615';
    ...
    it('preserves 256-color foreground across wrapped lines', () => {
        const output = renderTable(['Color'], [['\u001b[38;5;45mabcdefghijklmnopqrstuvwxyz0123456789\u001b[39m']], 24);
        expectWrappedContinuation(output, 'abcdefghijklmnopqrstuvwxyz0123456789', 'qrstuvwxyz012345');
        expect(foregroundAtText(output, 'qrstuvwxyz012345')).toBe('38;5;45');
        expectAllLinesToHaveSameVisibleWidth(output);
    });
    it('does not preserve foreground after an explicit reset', () => {
        const output = renderTable(['Color'], [['\u001b[38;5;45mcolored\u001b[0m reset']], 18);
        expect(foregroundAtText(output, 'reset')).toBeUndefined();
        expectAllLinesToHaveSameVisibleWidth(output);
    });
    it('does not preserve foreground after an explicit foreground reset', () => {
        const output = renderTable(['Color'], [['\u001b[38;5;45mcolored\u001b[39m reset']], 18);
        expect(foregroundAtText(output, 'reset')).toBeUndefined();
        expectAllLinesToHaveSameVisibleWidth(output);
    });
    ...
    it('renders markdown links as readable plain text in cells', () => {
        const output = renderTable(['Name', 'Link'], [['Doc', '[Qwen](https://example.com/path)']], 60);
    ...
    describe('OSC 8 markdown links in cells', () => {
        ...
        it('wraps a markdown link in a cell with an OSC 8 envelope', () => {
            ...
            const output = renderTable(['Name', 'Link'], [['Docs', `[here](${url})`]], 80);
        ...
        it('does not treat underscores around a later URL as emphasis', () => {
            ...
            const output = renderTable(['PR'], [[`见 ${firstUrl}（说明_1）和 ${secondUrl}_。`]], 100);
        ...
        it('does not wrap dangerous schemes in cells', () => {
            ...
            const output = renderTable(['Name', 'Link'], [['Bad', `[click](${url})`]], 80);
            expect(output).not.toContain('\x1b]8');
        ...
        it('keeps `(url)` suffix in cells when label looks like a mismatched URL', () => {
            ...
            const output = renderTable(['Name', 'Link'], [['x', `[https://google.com](${target})`]], 80);
        ...
        it('sanitizes bidi controls in a cell label', () => {
            enableHyperlinks();
            const url = 'https://example.com/page';
            const output = renderTable(['Name', 'Link'], [['x', `[safe.com\u202emoc.live](${url})`]], 80);
            expect(output).toContain(`\x1b]8;;${url}\x07`);
    ...
    describe('maxHeight clamp (streaming preview)', () => {
        const renderWithMaxHeight = (headers, rows, maxHeight, contentWidth = 80) => {
            const { lastFrame } = renderWithProviders(_jsx(TableRenderer, { headers: headers, rows: rows, contentWidth: contentWidth, maxHeight: maxHeight }));
            return lastFrame() ?? '';
        };
        const manyRows = Array.from({ length: 40 }, (_, i) => [`r${i}`, `v${i}`]);
        it('renders unchanged when the table fits within maxHeight', () => {
            const output = renderWithMaxHeight(['A', 'B'], [
                ['1', 'one'],
                ['2', 'two'],
            ], 50);
    ...
    describe('streaming table rendering (format stability)', () => {
        it('renders a header-only box (no divider) when there are no data rows', () => {
            ...
            const output = renderWithProviders(_jsx(TableRenderer, { headers: ['Alpha', 'Beta'], rows: [], contentWidth: 80 })).lastFrame() ?? '';
        ...
        it('keeps the zero-row header box horizontal on a narrow terminal', () => {
            ...
            const output = renderWithProviders(_jsx(TableRenderer, { headers: ['Alpha', 'Beta'], rows: [], contentWidth: 20 })).lastFrame() ?? '';
            const clean = stripAnsi(output);
            expect(clean).toContain('Alpha');
            expect(clean).toContain('┌');
        });
        it('keeps the zero-row box visible when it exceeds a very narrow terminal', () => {
            ...
            const output = renderWithProviders(_jsx(TableRenderer, { headers: ['C1', 'C2', 'C3', 'C4', 'C5', 'C6'], rows: [], contentWidth: 25 })).lastFrame() ?? '';
            const clean = stripAnsi(output);
            expect(clean).toContain('┌');
        ...
        const render = (rows, isStreaming = true) => renderWithProviders(_jsx(TableRenderer, { headers: headers, rows: rows, contentWidth: 80, isStreaming: isStreaming })).lastFrame() ?? '';
        it('widens the table when a wider row is appended (redraw on wider)', () => {
            ...
            expect(topBorderWidth(render(withWiderRow))).toBeGreaterThan(topBorderWidth(render(firstRowOnly)));
        });
