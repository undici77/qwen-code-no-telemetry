 ...
 */
import { registerGoalHook, setGoalTerminalObserver, setLastGoalTerminal, unregisterGoalHook, } from '@qwen-code/qwen-code-core';
...
export function recordGoalStatusItem(config, item, rawCommand = '/goal') {
    try {
        ...
        if (!recording) {
            ...
            writeStderrLineSafe(`qwen: no chat recording service; goal_status (kind=${item.kind}) will not survive a resume.`);
            return;
        }
        ...
        writeStderrLineSafe(`qwen: failed to record goal_status (kind=${item.kind}): ${error}`);
    }
}
...
export function restoreGoalFromHistory(history, config, addItem) {
    ...
    if (goalConditionBlockedBy(restorable.condition)) {
        writeStderrLineSafe(`qwen: refusing to restore a goal for session ${sessionId}: the condition is empty.`);
        unregisterGoalHook(config, sessionId);
