...
const NODE_GAP_X = 4;
const NODE_GAP_Y = 4;
const FLOW_EDGE_OPERATOR_RE = new RegExp(String.raw `\s*(?:` +
...
function stripMermaidPunctuation(text) {
...
function sanitizePreviewSourceLine(line) {
...
function previewSourceFallback(source, contentWidth, type, warning) {
    ...
    const lines = ['```mermaid', ...sourceLines, '```'].map((line) => truncateToWidth(line, contentWidth));
    return {
...
function diagramLines(source, startRe, maxLines = MAX_GENERIC_PREVIEW_LINES) {
    const allLines = source
        .split(/\r?\n/)
        .map(normalizePreviewLine)
        .filter((line) => line.length > 0 && !LINE_COMMENT_RE.test(line) && !startRe.test(line));
    return {
        lines: allLines.slice(0, maxLines),
        truncated: allLines.length > maxLines,
    };
}
function budgetWarning(truncated, maxLines = MAX_GENERIC_PREVIEW_LINES) {
...
function parseNodeToken(rawToken) {
    const token = stripMermaidPunctuation(rawToken)
        .replace(/^\|.*?\|/, '')
        .trim();
    if (!token || /^subgraph\b|^end$/i.test(token))
        return null;
...
function parseFlowEdge(line) {
    ...
    for (const pattern of patterns) {
        ...
        if (!from || !to)
            return null;
        const label = pattern.labelIndex !== undefined
...
function parseFlowEdges(line) {
    ...
    if (operators.length <= 1)
        return singleEdge ? [singleEdge] : [];
    const tokens = [];
    const labels = [];
    let cursor = 0;
    for (const operator of operators) {
        tokens.push(line.slice(cursor, operator.index).trim());
        labels.push(stripMermaidPunctuation(operator[1] ?? operator[2] ?? '') || undefined);
...
function normalizeFlowNodeLabels(edges) {
...
function boxNode(node, width) {
    const labels = nodeLabelLines(node.label).map((line) => truncateToWidth(line, Math.max(3, width - 4)));
...
function buildFlowGraph(edges) {
    ...
    for (const edge of edges) {
        ...
        if (!incomingCount.has(edge.from.id))
            incomingCount.set(edge.from.id, 0);
    }
    const roots = Array.from(nodes.values()).filter((node) => (incomingCount.get(node.id) ?? 0) === 0);
    return {
        nodes,
        outgoing,
        incomingCount,
...
function renderNodeLines(node, maxWidth) {
...
function branchPreference(label) {
    if (!label)
        return 0;
    const normalized = label.trim().toLowerCase();
    if (/^(no|false|fail|failed|否|不|失败)$/.test(normalized))
        return -1;
    if (/^(yes|true|pass|passed|是|成功)$/.test(normalized))
        return 1;
    return 0;
}
function groupNodesByRank(graph, ranks) {
    const layers = [];
    const preferenceById = new Map();
    const parentEdgesById = new Map();
    ...
    for (const [rank, layer] of layers.entries()) {
        layer?.sort((a, b) => {
            const parentOrderDelta = parentOrder(a, rank, ranks, parentEdgesById, orderById) -
                parentOrder(b, rank, ranks, parentEdgesById, orderById);
            if (parentOrderDelta !== 0)
                return parentOrderDelta;
            const preferenceDelta = (preferenceById.get(a.id) ?? 0) - (preferenceById.get(b.id) ?? 0);
...
function parentOrder(node, rank, ranks, parentEdgesById, orderById) {
...
function mergeCanvasChar(existing, next) {
    if (existing === '')
        return existing;
    if (next === '')
        return existing;
...
function drawHorizontal(canvas, y, x1, x2) {
    const start = Math.min(x1, x2);
    const end = Math.max(x1, x2);
    for (let x = start; x <= end; x++)
        putChar(canvas, x, y, '─');
}
function drawVertical(canvas, x, y1, y2) {
    const start = Math.min(y1, y2);
    const end = Math.max(y1, y2);
    for (let y = start; y <= end; y++)
        putChar(canvas, x, y, '│');
}
function drawNode(canvas, positioned) {
    positioned.lines.forEach((line, offset) => {
        putText(canvas, positioned.x, positioned.y + offset, line, true);
    });
}
function layoutVertical(graph, contentWidth) {
    ...
    for (const layer of layers) {
        ...
        const maxNodeWidth = Math.max(8, Math.floor((contentWidth - gapCount * NODE_GAP_X) / layer.length));
        const rendered = layer.map((node) => ({
            node,
            lines: renderNodeLines(node, Math.min(28, maxNodeWidth)),
        }));
        const totalWidth = rendered.reduce((sum, item) => sum + lineWidth(item.lines[0]), 0) +
            gapCount * NODE_GAP_X;
...
function layoutHorizontal(graph, contentWidth) {
    const ranks = computeRanks(graph);
    const layers = groupNodesByRank(graph, ranks);
    const columnWidth = Math.max(10, Math.min(24, Math.floor((contentWidth - (layers.length - 1) * NODE_GAP_X) / layers.length)));
    const totalWidth = layers.length * columnWidth + (layers.length - 1) * NODE_GAP_X;
    if (totalWidth > contentWidth || layers.length === 0)
        return null;
    const positioned = [];
    let x = Math.max(0, Math.floor((contentWidth - totalWidth) / 2));
    for (const layer of layers) {
...
function drawForwardVerticalEdge(canvas, from, to, label) {
    ...
    if (Math.abs(from.centerX - to.centerX) <= 1) {
        ...
        if (label) {
            putText(canvas, Math.min(canvas[midY].length - 1, from.centerX + 2), midY, truncateToWidth(label, 14));
        }
        return;
    }
    ...
    if (label) {
        ...
        const labelX = Math.abs(to.centerX - from.centerX) > stringWidth(text) + 2
            ? Math.min(from.centerX, to.centerX) +
                Math.floor((Math.abs(to.centerX - from.centerX) - stringWidth(text)) / 2)
            : Math.min(canvas[bendY].length - stringWidth(text), from.centerX + 2);
        putText(canvas, Math.max(0, labelX), bendY, text, true);
    }
}
function drawVerticalFork(canvas, from, targets) {
    if (targets.length === 0)
        return;
    if (targets.length === 1) {
        drawForwardVerticalEdge(canvas, from, targets[0].to, targets[0].edge.label);
        return;
    }
    const sortedTargets = [...targets].sort((a, b) => a.to.centerX - b.to.centerX);
    const startY = from.y + from.height;
    const firstTargetTop = Math.min(...sortedTargets.map((target) => target.to.y));
    ...
    for (const [index, target] of sortedTargets.entries()) {
        ...
        const targetJunction = sortedTargets.length === 1
            ? '┴'
            : index === 0
                ? '┌'
        ...
        if (target.edge.label) {
            ...
            const x = Math.max(0, Math.min(canvas[labelY].length - stringWidth(label), target.to.centerX - Math.floor(stringWidth(label) / 2)));
...
function formatLoopNote(from, to, label) {
    const edgeLabel = label ? ` [${label}]` : '';
    return `${singleLineLabel(from.node.label)}${edgeLabel} ↩ to ${singleLineLabel(to.node.label)}`;
}
function drawHorizontalEdge(canvas, from, to, label) {
    ...
    if (from.centerY === to.centerY) {
        ...
        if (label) {
            ...
            putText(canvas, Math.max(0, midX - Math.floor(stringWidth(text) / 2)), from.centerY, text);
        }
        return;
    }
    ...
    if (label) {
        const text = truncateToWidth(label, 12);
        putText(canvas, Math.max(0, midX - Math.floor(stringWidth(text) / 2)), Math.min(from.centerY, to.centerY), text);
    }
}
...
function renderLayeredFlowchart(edges, contentWidth, horizontal) {
    const width = Math.max(MIN_CANVAS_WIDTH, contentWidth);
    const graph = buildFlowGraph(edges);
    const positioned = horizontal && graph.nodes.size <= 8
    ...
    const canvasHeight = Math.max(...positioned.map((node) => node.y + node.height), 1) + 2;
...
function renderFlowchart(source, contentWidth) {
    ...
    if (normalizedEdges.length === 0) {
        return previewSourceFallback(source, contentWidth, 'flowchart', 'Flowchart preview supports simple A --> B style edges; showing source instead.');
    }
    const horizontal = direction.includes('LR') || direction.includes('RL');
    const rendered = renderLayeredFlowchart(normalizedEdges, contentWidth, horizontal);
    return {
...
function renderSequence(source, contentWidth) {
    const allRawLines = source
        .split(/\r?\n/)
        .map(normalizePreviewLine)
        .filter((line) => line.length > 0 &&
    ...
    for (const line of rawLines) {
        ...
        const messageMatch = /^(.+?)(-->>|->>|-->|->|--x|-x)\s*(.+?)\s*:\s*(.+)$/.exec(line);
        if (!messageMatch)
            continue;
        ...
        messages.push(truncateToWidth(`${participants.get(from) ?? from} ${arrow} ${participants.get(to) ?? to}: ${message}`, contentWidth));
    }
    const header = participants.size > 0
        ? `Participants: ${Array.from(participants.values()).join(' | ')}`
        : 'Participants: not declared';
    if (messages.length === 0) {
        return previewSourceFallback(source, contentWidth, 'sequenceDiagram', 'Sequence preview supports A->>B: message style arrows; showing source instead.');
    }
...
function renderClassDiagram(source, contentWidth) {
    ...
    for (const line of rawLines) {
        ...
        const relationMatch = /^([A-Za-z0-9_.$:-]+)\s+([<|*o.]*--[|>*o.]*|<\|--|--\|>|\*--|o--|-->|<--|\.\.>)\s+([A-Za-z0-9_.$:-]+)(?:\s*:\s*(.+))?$/i.exec(line);
        if (relationMatch) {
            ...
            classes.add(from);
            classes.add(to);
            relationships.push(truncateToWidth(`${from} ${relation} ${to}${label ? `: ${label}` : ''}`, contentWidth));
            continue;
        }
        const inlineMemberMatch = /^([A-Za-z0-9_.$:-]+)\s*:\s*(.+)$/i.exec(line);
        if (inlineMemberMatch) {
            classes.add(inlineMemberMatch[1]);
            members.push(truncateToWidth(`${inlineMemberMatch[1]}: ${stripMermaidPunctuation(inlineMemberMatch[2])}`, contentWidth));
            continue;
        }
        if (currentClass) {
            members.push(truncateToWidth(`${currentClass}: ${stripMermaidPunctuation(line)}`, contentWidth));
        }
    }
    if (classes.size === 0 &&
        relationships.length === 0 &&
        members.length === 0) {
        return previewSourceFallback(source, contentWidth, 'classDiagram', 'Class preview found no previewable classes or relationships; showing source instead.');
    }
    const output = [
        truncateToWidth(`Classes: ${Array.from(classes).join(' | ') || 'not declared'}`, contentWidth),
        '',
        ...(relationships.length > 0
...
function renderStateDiagram(source, contentWidth) {
    ...
    for (const line of rawLines) {
        ...
        if (transitionMatch) {
            ...
            transitions.push(truncateToWidth(`${from} → ${to}${label ? `: ${label}` : ''}`, contentWidth));
            continue;
        }
        const stateMatch = /^state\s+(.+?)(?:\s+as\s+(.+))?$/i.exec(line);
        if (stateMatch) {
            declarations.push(truncateToWidth(stripMermaidPunctuation(stateMatch[2] ?? stateMatch[1]), contentWidth));
        }
    }
    if (declarations.length === 0 && transitions.length === 0) {
        return previewSourceFallback(source, contentWidth, 'stateDiagram', 'State preview found no previewable states or transitions; showing source instead.');
    }
    const output = [
...
function renderErDiagram(source, contentWidth) {
    ...
    for (const line of rawLines) {
        ...
        const relationMatch = /^([A-Za-z0-9_.$:-]+)\s+([|o}{]{1,2}--[|o}{]{1,2})\s+([A-Za-z0-9_.$:-]+)(?:\s*:\s*(.+))?$/i.exec(line);
        if (relationMatch) {
            entities.add(relationMatch[1]);
            entities.add(relationMatch[3]);
            relationships.push(truncateToWidth(`${relationMatch[1]} ${relationMatch[2]} ${relationMatch[3]}${relationMatch[4] ? `: ${stripMermaidPunctuation(relationMatch[4])}` : ''}`, contentWidth));
            continue;
        }
        if (currentEntity) {
            attributes.push(truncateToWidth(`${currentEntity}: ${stripMermaidPunctuation(line)}`, contentWidth));
        }
    }
    if (entities.size === 0 &&
        relationships.length === 0 &&
        attributes.length === 0) {
        return previewSourceFallback(source, contentWidth, 'erDiagram', 'ER preview found no previewable entities or relationships; showing source instead.');
    }
    return {
        title: 'Mermaid ER diagram',
        lines: [
            truncateToWidth(`Entities: ${Array.from(entities).join(' | ') || 'not declared'}`, contentWidth),
            '',
            ...(relationships.length > 0
                ? ['Relationships:', ...relationships]
                : ['Relationships: none previewed']),
...
function renderPieDiagram(source, contentWidth) {
    ...
    const slices = rawLines
        ...
        .map((match) => truncateToWidth(`${stripMermaidPunctuation(match[1])}: ${match[2]}`, contentWidth));
    if (slices.length === 0) {
        return previewSourceFallback(source, contentWidth, 'pie', 'Pie preview found no previewable slices; showing source instead.');
    }
    return {
...
function renderGanttDiagram(source, contentWidth) {
    const { lines: rawLines, truncated } = diagramLines(source, GANTT_START_RE);
    const output = [];
    for (const line of rawLines) {
        if (/^(dateFormat|axisFormat|tickInterval|weekday|excludes|todayMarker)\b/i.test(line)) {
            continue;
        }
        const section = /^section\s+(.+)$/i.exec(line);
        if (section) {
            output.push('');
            output.push(truncateToWidth(`Section: ${stripMermaidPunctuation(section[1])}`, contentWidth));
            continue;
        }
        const title = /^title\s+(.+)$/i.exec(line);
        if (title) {
            output.push(truncateToWidth(`Title: ${stripMermaidPunctuation(title[1])}`, contentWidth));
            continue;
        }
        output.push(truncateToWidth(`• ${stripMermaidPunctuation(line)}`, contentWidth));
    }
    if (output.length === 0) {
        return previewSourceFallback(source, contentWidth, 'gantt', 'Gantt preview found no previewable tasks; showing source instead.');
    }
    return {
...
function renderJourneyDiagram(source, contentWidth) {
    ...
    if (output.length === 0) {
        return previewSourceFallback(source, contentWidth, 'journey', 'Journey preview found no previewable steps; showing source instead.');
    }
    return {
...
function renderIndentedTreeDiagram(source, contentWidth, startRe, title, sourceType) {
    const allLines = source
        .split(/\r?\n/)
        .filter((line) => normalizePreviewLine(line).length > 0 &&
    ...
    const lines = rawLines.map((line) => {
        ...
        return truncateToWidth(`${'  '.repeat(depth)}• ${safeLine.trim()}`, contentWidth);
    });
    if (lines.length === 0) {
        return previewSourceFallback(source, contentWidth, sourceType, `${title} preview found no previewable nodes; showing source instead.`);
    }
    return {
        title,
        lines: lines.slice(0, MAX_RENDERED_LINES),
...
export function renderMermaidVisual(source, contentWidth) {
    ...
    if (MINDMAP_START_RE.test(firstLine)) {
        return renderIndentedTreeDiagram(trimmed, contentWidth, MINDMAP_START_RE, 'Mermaid mindmap', 'mindmap');
    }
    if (GIT_GRAPH_START_RE.test(firstLine)) {
        return renderIndentedTreeDiagram(trimmed, contentWidth, GIT_GRAPH_START_RE, 'Mermaid git graph', 'gitGraph');
    }
    if (REQUIREMENT_START_RE.test(firstLine)) {
        return renderIndentedTreeDiagram(trimmed, contentWidth, REQUIREMENT_START_RE, 'Mermaid requirement diagram', 'requirementDiagram');
    }
    const type = firstLine.split(/\s+/)[0] || 'unknown';
    return previewSourceFallback(trimmed, contentWidth, type);
}
//# sourceMappingURL=mermaidVisualRenderer.js.map