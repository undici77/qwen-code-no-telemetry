...
import { fitPendingSlice, splitMarkdownTableRow, TABLE_ROW_RE, TABLE_SEPARATOR_RE, CODE_FENCE_RE, } from './pending-rendered-height.js';
...
const MarkdownDisplayInternal = ({ text, isPending, availableTerminalHeight, contentWidth, textColor = theme.text.primary, sourceCopyIndexOffsets, enforceHeightBudget = false, }) => {
    ...
    const pendingRenderedBudget = (isPending || enforceHeightBudget) && availableTerminalHeight !== undefined
    ...
    if (pendingRenderedBudget !== undefined) {
        const tableClampRows = availableTerminalHeight !== undefined
            ? Math.max(2, availableTerminalHeight - TABLE_PENDING_RESERVED_ROWS)
            : Number.MAX_SAFE_INTEGER;
        const { keptLines } = fitPendingSlice(allLines, contentWidth, pendingRenderedBudget, tableClampRows);
        if (keptLines < allLines.length) {
            lines = allLines.slice(0, keptLines);
            droppedSourceLines = allLines.length - keptLines;
        }
    }
    const showTruncationCue = enforceHeightBudget && !isPending && droppedSourceLines > 0;
        ...
        if (start < lines.length) {
            ...
            if (headerCells >= 2) {
                ...
                const hasMatchingSeparator = rest.some((l) => {
                    if (!tableSeparatorRegex.test(l))
                        return false;
                    const cols = splitMarkdownTableRow(l).filter((c) => c.length > 0).length;
                ...
                const looksLikeSeparatorPrefix = lineAfterHeader !== undefined &&
                    lineAfterHeader.includes('|') &&
                    /^[\s|:-]*$/.test(lineAfterHeader);
                let couldStillBeTable = lineAfterHeader === undefined ||
                ...
                if (couldStillBeTable &&
                    lineAfterHeader !== undefined &&
                    /\|\s*$/.test(lineAfterHeader)) {
                    const sepCols = splitMarkdownTableRow(lineAfterHeader).filter((c) => c.length > 0).length;
    ...
    lines.forEach((line, index) => {
        ...
        if (codeFenceMatch) {
            ...
            if (tableHeaders.length > 0 && tableRows.length > 0) {
                addContentBlock(_jsx(RenderTable, { headers: tableHeaders, rows: tableRows, contentWidth: contentWidth, aligns: tableAligns, enableInlineMath: renderVisualBlocks, isPending: isPending, 
                    ...
                    isFrontier: false, availableTerminalHeight: availableTerminalHeight }, `table-${contentBlocks.length}`));
            }
            inTable = false;
            tableRows = [];
            tableHeaders = [];
    ...
    if (inCodeBlock) {
        addContentBlock(_jsx(RenderCodeBlock, { content: codeBlockContent, lang: codeBlockLang, codeBlockIndex: currentCodeBlockIndex, codeBlockLangIndex: currentCodeBlockLangIndex, isPending: isPending, availableTerminalHeight: availableTerminalHeight, contentWidth: contentWidth, startLineNumber: currentCodeBlockStartLine }, "line-eof"));
    }
    if (inMathBlock) {
    ...
    if (inTable && tableHeaders.length > 0 && tableRows.length > 0) {
        addContentBlock(_jsx(RenderTable, { headers: tableHeaders, rows: tableRows, contentWidth: contentWidth, aligns: tableAligns, enableInlineMath: renderVisualBlocks, isPending: isPending, 
            ...
            isFrontier: true, availableTerminalHeight: availableTerminalHeight }, `table-${contentBlocks.length}`));
    }
    ...
    if (showTruncationCue) {
        contentBlocks.push(_jsx(Text, { color: theme.text.secondary, wrap: "truncate-end", children: `... ${droppedSourceLines} more line${droppedSourceLines === 1 ? '' : 's'} not shown (viewport too small) ...` }, `truncation-cue-${contentBlocks.length}`));
    }
    return _jsx(_Fragment, { children: contentBlocks });
};
const RenderCodeBlockInternal = ({ content, lang, codeBlockIndex, codeBlockLangIndex, isPending, availableTerminalHeight, contentWidth, startLineNumber = 1, }) => {
    ...
    if (isPending && availableTerminalHeight !== undefined) {
        ...
        if (content.length > MAX_CODE_LINES_WHEN_PENDING) {
            ...
            const colorizedTruncatedCode = colorizeCode(truncatedContent.join('\n'), lang, availableTerminalHeight, contentWidth - CODE_BLOCK_PREFIX_PADDING, { settings, startLineNumber });
            return (_jsx(Box, { paddingLeft: CODE_BLOCK_PREFIX_PADDING, flexDirection: "column", children: colorizedTruncatedCode }));
        }
    }
    const colorizedCode = colorizeCode(fullContent, lang, availableTerminalHeight, contentWidth - CODE_BLOCK_PREFIX_PADDING, { settings, startLineNumber });
...
const RenderPendingMermaidBlockInternal = ({ content, availableTerminalHeight, contentWidth }) => {
    ...
    return (_jsxs(Box, { paddingLeft: CODE_BLOCK_PREFIX_PADDING, flexDirection: "column", width: contentWidth, flexShrink: 0, children: [_jsx(Text, { color: theme.text.accent, children: "Mermaid diagram is being written..." }), previewLines.map((line, index) => (_jsx(Text, { color: theme.text.secondary, wrap: "truncate-end", children: line || ' ' }, index)))] }));
};
...
const RenderTableInternal = ({ headers, rows, contentWidth, aligns, enableInlineMath = false, isPending = false, isFrontier = true, availableTerminalHeight, }) => {
    ...
    const maxHeight = isPending && availableTerminalHeight !== undefined
        ? Math.max(2, availableTerminalHeight - TABLE_PENDING_RESERVED_ROWS)
        : undefined;
    return (_jsx(TableRenderer, { headers: headers, rows: rows, contentWidth: contentWidth, aligns: aligns, enableInlineMath: enableInlineMath, isStreaming: isPending && isFrontier, maxHeight: maxHeight }));
};
