...
describe('checkForUpdates', () => {
    ...
    it('checks npm updates in the global npm context', async () => {
        ...
        await expect(fetchGlobalNpmUpdateInfo('@qwen-code/qwen-code', '1.0.0', 'latest', run)).resolves.toMatchObject({ current: '1.0.0', latest: '1.1.0' });
        const npmArgs = [
            'view',
            '@qwen-code/qwen-code',
            'dist-tags.latest',
    ...
    it('always resolves the update check via npm view, regardless of installation type (#7515)', async () => {
        ...
        expect(fetchGlobalNpm).toHaveBeenCalledWith('@qwen-code/qwen-code', '1.0.0', 'latest');
    });
    it('runs the Windows npm CLI through Node without a shell', async () => {
        ...
        await runGlobalNpm(['view', '@qwen-code/qwen-code'], run, 'win32', 'C:\\Program Files\\nodejs\\node.exe', resolveNpmCliPath);
    ...
    it('does not fall back when the global npm query fails', async () => {
        ...
        await expect(fetchGlobalNpmUpdateInfo('@qwen-code/qwen-code', '1.0.0', 'latest', run)).rejects.toThrow('npm view failed');
    });
    it('treats an empty dist-tag response as no update', async () => {
        const run = vi.fn().mockResolvedValue({ stdout: '\n', stderr: '' });
        await expect(fetchGlobalNpmUpdateInfo('@qwen-code/qwen-code', '1.0.0', 'nightly', run)).resolves.toMatchObject({ current: '1.0.0', latest: '1.0.0' });
    });
    ...
    it.each([
        ...
        await expect(fetchGlobalNpmUpdateInfo('@qwen-code/qwen-code', '1.0.0', 'latest', run)).rejects.toThrow('Invalid npm latest version response');
    });
