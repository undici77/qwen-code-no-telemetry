...
describe('latexRenderer', () => {
    it('renders common fractions, roots, scripts, and symbols', () => {
        expect(renderInlineLatex(String.raw `\frac{a}{b} + \sqrt{x^2}`)).toBe('a/b + √(x²)');
    ...
    it('preserves unknown commands instead of stripping command markers', () => {
        expect(renderInlineLatex(String.raw `\mathcal{O}(n)`)).toBe(String.raw `\mathcal{O}(n)`);
        expect(renderInlineLatex(String.raw `\mathbb{E}[X]`)).toBe(String.raw `\mathbb{E}[X]`);
    });
...
//# sourceMappingURL=latexRenderer.test.js.map