...
vi.mock('@teddyzhu/clipboard', async () => {
    if (clipboardMockState.loadDelayMs > 0) {
        await new Promise((resolve) => setTimeout(resolve, clipboardMockState.loadDelayMs));
    }
 ...
 */
function createMockChild(stdoutData, exitCode = 0) {
...
describe('clipboardUtils', () => {
    let clipboardHasImage;
    let saveClipboardImage;
    let cleanupOldClipboardImages;
    let writeOsc52;
    let isWaylandSession;
    beforeEach(async () => {
    ...
    describe('xclip / X11 path', () => {
        ...
        describe('clipboardHasImage', () => {
            it('should detect xclip as the clipboard tool on X11', async () => {
                ...
                expect(mockSpawn).toHaveBeenCalledWith('xclip', ['-selection', 'clipboard', '-t', 'TARGETS', '-o'], { stdio: ['ignore', 'pipe', 'ignore'] });
            });
    ...
    describe('writeOsc52', () => {
        ...
        it('should write OSC 52 sequence to stdout when stdout is TTY', () => {
            ...
            expect(result).toBe(true);
            expect(stdoutWriteMock).toHaveBeenCalledWith(expectedSequence, expect.any(Function));
        ...
        it('should write OSC 52 sequence to stderr when stdout is not TTY but stderr is', () => {
            ...
            const result = writeOsc52(text);
            expect(result).toBe(true);
            expect(stderrWriteMock).toHaveBeenCalledWith(expectedSequence, expect.any(Function));
