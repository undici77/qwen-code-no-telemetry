...
import { findInlineMathExpressions, INLINE_MATH_MAX_CHARS, readInlineMathSpanAt, unescapeMarkdownBeforeMath, unescapeMarkdownDollars, } from './inline-math.js';
describe('inline math recognition', () => {
    ...
    it('preserves escaped dollars, prices, variables, and adjacent spans', () => {
        expect(findInlineMathExpressions(String.raw `Literal \$xy$ and \$\alpha$`)).toEqual([]);
    ...
    it('recognizes literal dollars inside and next to formulas', () => {
        ...
        expect(findInlineMathExpressions(String.raw `Literal then math: \$$x^2$`)).toEqual(['x^2']);
        expect(findInlineMathExpressions(String.raw `Math then literal: $x^2\$$`)).toEqual([String.raw `x^2\$`]);
    });
    it('uses the full backslash-run parity around delimiters', () => {
        expect(findInlineMathExpressions(String.raw `Odd \$x$; even \\$y$`)).toEqual(['y']);
        expect(findInlineMathExpressions(String.raw `Odd $x\$; even $y\\$`)).toEqual([String.raw `y\\`]);
    });
    ...
    it('removes Markdown escaping from literal dollars by parity', () => {
        expect(unescapeMarkdownDollars(String.raw `\$ \\$ \\\$`)).toBe(String.raw `$ \$ \$`);
        expect(unescapeMarkdownBeforeMath(String.raw `prefix \\`)).toBe('prefix \\');
    });
});
//# sourceMappingURL=inline-math.test.js.map