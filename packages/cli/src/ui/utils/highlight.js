...
const HIGHLIGHT_REGEX = /(^\/[a-zA-Z][a-zA-Z0-9:_-]*)|((?<=\s)\/[a-zA-Z][a-zA-Z0-9:_-]*)|(@(?:\\ |[a-zA-Z0-9_./-])+)/g;
export function parseInputForHighlighting(text, index, slashCommands) {
    if (!text) {
        return [{ text: '', type: 'default' }];
    }
    const tokens = [];
    const validSlashTokenStarts = new Set(slashCommands
