...
import { isInteractiveTerminal, shouldUseVirtualViewport, } from './terminal-buffer.js';
describe('shouldUseVirtualViewport', () => {
    it('defaults to virtual viewport when the setting is unset', () => {
        expect(shouldUseVirtualViewport(undefined, false, true)).toBe(getSettingsSchema().ui.properties.useTerminalBuffer.default);
    });
...
describe('isInteractiveTerminal', () => {
    ...
    it('treats CI sessions as non-interactive unless CI is explicitly disabled', () => {
        ...
        expect(isInteractiveTerminal(true, { CONTINUOUS_INTEGRATION: 'true' })).toBe(false);
        ...
        expect(isInteractiveTerminal(true, { CI: 'False' })).toBe(true);
        expect(isInteractiveTerminal(true, { CONTINUOUS_INTEGRATION: '' })).toBe(true);
        expect(isInteractiveTerminal(true, { CONTINUOUS_INTEGRATION: '0' })).toBe(true);
        expect(isInteractiveTerminal(true, { CONTINUOUS_INTEGRATION: 'false' })).toBe(true);
        expect(isInteractiveTerminal(true, { CONTINUOUS_INTEGRATION: 'FALSE' })).toBe(true);
...
//# sourceMappingURL=terminal-buffer.test.js.map