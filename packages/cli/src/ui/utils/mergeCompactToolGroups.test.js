...
import { compactToggleHasVisualEffect, mergeCompactToolGroups, } from './mergeCompactToolGroups.js';
...
describe('mergeCompactToolGroups', () => {
    ...
    it.each([
        ...
        ['failed', 'failed'],
        ['cancelled', 'cancelled'],
    ])('does NOT merge tool_group with terminal subagent (%s)', (_label, status) => {
        ...
        const items = [
            createToolGroup(1, [createTool('c1', 'Shell', ToolCallStatus.Success)]),
            createToolGroup(2, [
                createTool('c2', 'Agent', status === 'failed' ? ToolCallStatus.Error : ToolCallStatus.Success, subagentResult),
            ]),
            createToolGroup(3, [createTool('c3', 'Shell', ToolCallStatus.Success)]),
        ];
        const merged = mergeCompactToolGroups(items);
