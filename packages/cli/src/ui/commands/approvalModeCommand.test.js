...
describe('approvalModeCommand', () => {
    ...
    describe('direct mode setting (session-only)', () => {
        ...
        it('should emit the entry notice when switching to auto mode', async () => {
            const result = (await approvalModeCommand.action?.(mockContext, 'auto'));
            expect(result.type).toBe('message');
            expect(mockSetApprovalMode).toHaveBeenCalledWith('auto');
            expect(mockContext.ui.addItem).toHaveBeenCalledWith(expect.objectContaining({ type: 'info' }), expect.any(Number));
        });
