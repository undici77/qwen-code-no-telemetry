...
import { ApiResponseEvent, AuthType, MAIN_SOURCE, Storage, getTokenUsageFilePath, recordTokenUsageFromApiResponse, } from '@qwen-code/qwen-code-core';
...
describe('statsCommand', () => {
    ...
    async function seedUsage() {
        ...
        const first = new ApiResponseEvent('response-1', 'model-a', 100, 'prompt-1', AuthType.USE_GEMINI, {
            promptTokenCount: 10,
        ...
        const second = new ApiResponseEvent('response-2', 'model-b', 200, 'prompt-2', AuthType.USE_VERTEX_AI, {
            promptTokenCount: 7,
    ...
    it('should open stats dialog when run with no subcommand in interactive mode', () => {
        ...
        const result = statsCommand.action(mockContext, '');
    ...
    it('should display skill stats when using the "skills" subcommand', () => {
        const skillsSubCommand = statsCommand.subCommands?.find((sc) => sc.name === 'skills');
    ...
    describe('non-interactive mode', () => {
        ...
        it('should return skill stats in text mode', async () => {
            ...
            const result = (await skillsSubCommand.action(nonInteractiveContext, ''));
            expect(result.type).toBe('message');
            expect(result.messageType).toBe('info');
    ...
    describe('historical token usage subcommands', () => {
        it('stats daily returns persisted token usage in non-interactive mode', async () => {
            await seedUsage();
            const dailySubCommand = statsCommand.subCommands?.find((sc) => sc.name === 'daily');
            if (!dailySubCommand?.action) {
                throw new Error('Subcommand has no action');
            }
            const result = (await dailySubCommand.action(createMockCommandContext({
                executionMode: 'non_interactive',
        ...
        it('stats monthly writes persisted token usage to the interactive history', async () => {
            await seedUsage();
            const monthlySubCommand = statsCommand.subCommands?.find((sc) => sc.name === 'monthly');
        ...
        it('stats export writes CSV with a default filename', async () => {
            await seedUsage();
            const exportSubCommand = statsCommand.subCommands?.find((sc) => sc.name === 'export');
            if (!exportSubCommand?.action) {
                throw new Error('Subcommand has no action');
            }
            const result = (await exportSubCommand.action(createMockCommandContext({
                executionMode: 'non_interactive',
                services: { config: mockContext.services.config },
            ...
            expect(result.content).toContain('Token usage exported to CSV: qwen-token-usage-month-2025-07.csv');
            const csv = await readFile(path.join(tempDir, 'qwen-token-usage-month-2025-07.csv'), 'utf-8');
            expect(csv).toContain('period,value,group_type,group_key,model,auth_type,source,requests,input_tokens,output_tokens,cached_tokens,thoughts_tokens,total_tokens,api_duration_ms');
        ...
        it('stats export adds interactive info and error messages', async () => {
            ...
            const infoResult = await exportSubCommand.action(mockContext, 'monthly 2025-07 --format csv');
            ...
            const errorResult = await exportSubCommand.action(errorContext, 'monthly 2025-07 --output ../outside.csv');
        ...
        it('stats export writes JSON to an explicit output path', async () => {
            ...
            const json = JSON.parse(await readFile(path.join(tempDir, 'usage', 'day.json'), 'utf-8'));
        ...
        it('stats export accepts equals syntax for format and output', async () => {
            ...
            expect(result.messageType).toBe('info');
            const json = JSON.parse(await readFile(path.join(tempDir, 'usage', 'day-equals.json'), 'utf-8'));
            expect(json.totals.totalTokens).toBe(47);
        });
        it('stats export accepts short format and output flags', async () => {
            ...
            expect(result.messageType).toBe('info');
            await expect(readFile(path.join(tempDir, 'usage', 'month-short.csv'), 'utf-8')).resolves.toContain('month,2025-07,total,total');
        });
        ...
        it('stats export rejects symlinked directories that resolve outside the working directory', async () => {
            ...
            const outsideDir = await mkdtemp(path.join(tmpdir(), 'qwen-stats-outside-'));
            try {
                ...
                expect(result.messageType).toBe('error');
                expect(result.content).toContain('within the project working directory');
                await expect(access(path.join(outsideDir, 'usage.csv'))).rejects.toThrow();
            }
            finally {
        ...
        it('stats export writes through the real output directory after validation', async () => {
            ...
            expect(result.messageType).toBe('info');
            await expect(readFile(path.join(tempDir, 'nested', 'real.csv'), 'utf-8')).resolves.toContain('month,2025-07,total,total');
        });
        it('stats export replaces existing hardlinks without mutating the linked outside file', async () => {
            ...
            try {
                ...
                expect(result.messageType).toBe('info');
                await expect(readFile(outsideFile, 'utf-8')).resolves.toBe('outside-original');
                await expect(readFile(hardlinkPath, 'utf-8')).resolves.toContain('month,2025-07,total,total');
            }
            finally {
                await rm(outsideDir, { recursive: true, force: true });
            }
        ...
        it('stats export reports a missing final target after rename', async () => {
            ...
            const { rename: realRename } = await vi.importActual('node:fs/promises');
        ...
        it('stats export retries temporary file name collisions', async () => {
            ...
            fsPromisesMock.open.mockRejectedValueOnce(Object.assign(new Error('exists'), { code: 'EEXIST' }));
            const result = (await exportSubCommand.action(createMockCommandContext({
                executionMode: 'non_interactive',
                services: { config: mockContext.services.config },
            ...
            await expect(readFile(path.join(tempDir, 'retry.csv'), 'utf-8')).resolves.toContain('month,2025-07,total,total');
        });
        it('stats export reports temporary file collision exhaustion', async () => {
            ...
            fsPromisesMock.open.mockClear();
            fsPromisesMock.open.mockRejectedValue(Object.assign(new Error('exists'), { code: 'EEXIST' }));
            const result = (await exportSubCommand.action(createMockCommandContext({
                executionMode: 'non_interactive',
                services: { config: mockContext.services.config },
            }), 'monthly 2025-07 --output exhausted.csv'));
            expect(result.messageType).toBe('error');
            expect(result.content).toContain('Could not create a temporary export file.');
        ...
        it('stats export reports invalid arguments', async () => {
            ...
            expect(result.messageType).toBe('error');
            expect(result.content).toContain('Expected --format csv or --format json');
        });
        it('stats export preserves doubled backslashes inside quoted arguments', async () => {
            ...
            expect(result.messageType).toBe('error');
            expect(result.content).toContain('Unexpected argument: usage\\\\extra.csv');
        });
        ...
        it('uses locale translations for historical token usage output', async () => {
            ...
            expect(exportSubCommand.description).toBe('将 token 使用统计信息导出为 CSV 或 JSON');
            const dailyResult = (await dailySubCommand.action(createMockCommandContext({
                executionMode: 'non_interactive',
                services: { config: mockContext.services.config },
            }), '2025-07-14'));
            ...
            expect(dailyResult.content).toContain('生成耗时（TTFT/TPS）归属于生成指标');
            const exportResult = (await exportSubCommand.action(createMockCommandContext({
                executionMode: 'non_interactive',
                services: { config: mockContext.services.config },
            }), 'monthly 2025-07 --output ../outside.csv'));
            expect(exportResult.messageType).toBe('error');
            expect(exportResult.content).toContain('Token 使用导出路径必须位于项目工作目录内');
        });
    });
});
