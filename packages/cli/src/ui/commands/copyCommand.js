...
function parseInlineLatexExpressions(markdown) {
    const expressions = [];
    const lines = markdown.split(/\r?\n/);
...
function selectInlineLatexExpression(markdown, tokens) {
...
function parseLatexBlocks(markdown) {
    const blocks = [];
    const lines = markdown.split(/\r?\n/);
    const codeFenceRegex = /^ *(`{3,}|~{3,}) *([^`]*)$/;
    const mathFenceRegex = /^ *\$\$ *$/;
...
function selectLatexBlock(markdown, args) {
    ...
    const inlineSelectorIndex = selectorTokens.findIndex((token) => token.toLowerCase() === 'inline');
    if (inlineSelectorIndex !== -1) {
        return selectInlineLatexExpression(markdown, selectorTokens.filter((_, index) => index !== inlineSelectorIndex));
    }
...
function selectCodeBlock(markdown, args) {
    const tokens = args.trim().split(/\s+/).filter(Boolean);
    const firstToken = tokens[0]?.toLowerCase();
    if (!firstToken)
        return undefined;
...
export const copyCommand = {
    name: 'copy',
    get description() {
        return t('Copy to clipboard: reply, code (by lang), LaTeX, or Mermaid. N = Nth-latest message, index = block number');
    },
        ...
        if (messageIndex !== null) {
            if (messageIndex < 1) {
                return {
                    type: 'message',
                    messageType: 'info',
                    content: 'Message index must be a positive integer (1 = last AI message).',
                };
            }
            if (messageIndex > aiMessages.length) {
                const turnLabel = aiMessages.length === 1 ? 'AI message' : 'AI messages';
                return {
                    type: 'message',
                    messageType: 'info',
        ...
        if (aiOutput) {
            try {
                ...
                if (selectedLatexBlock !== undefined) {
                    const copiedLatex = 'expression' in selectedLatexBlock
