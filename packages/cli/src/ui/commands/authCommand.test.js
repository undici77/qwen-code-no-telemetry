...
describe('authCommand', () => {
    ...
    it.each(['non_interactive', 'acp'])('should return an info message in %s mode', (executionMode) => {
        if (!authCommand.action) {
            throw new Error('The auth command must have an action.');
        }
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Authentication configuration is only available in interactive mode. To configure authentication, run Qwen Code interactively and use /auth, or set environment variables: OPENAI_API_KEY, OPENAI_BASE_URL, OPENAI_MODEL.',
        });
    });
