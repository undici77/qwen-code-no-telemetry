...
import { getActiveGoal, getLastGoalTerminal, registerGoalHook, unregisterGoalHook, } from '@qwen-code/qwen-code-core';
...
function sanitizeConditionForPrompt(condition) {
    return condition.replace(/[\r\n]+/g, ' ').replace(/"/g, "'");
}
const goalInstructionPrompt = (condition) => `A session-scoped Stop hook is now active with condition: "${sanitizeConditionForPrompt(condition)}". ` +
...
const formatTurns = (n) => `${n} ${n === 1 ? 'turn' : 'turns'}`;
function assertNeverGoalKind(kind) {
    throw new Error(`Unexpected terminal goal kind: ${kind}`);
}
function terminalGoalTitle(kind) {
    switch (kind) {
        case 'achieved':
...
function formatTerminalSummary(event) {
    ...
    if (event.iterations > 0)
        stats.push(formatTurns(event.iterations));
...
export const goalCommand = {
    ...
    action: async (context, args) => {
        ...
        if (q === '') {
            ...
            if (active) {
                const turns = active.iterations === 0
                    ? 'not yet evaluated'
                    : formatTurns(active.iterations);
                const lastReason = active.lastReason
                    ? `\nLast check: ${active.lastReason}`
                    : '';
                return infoMessage(`Goal active: ${active.condition} (${turns})${lastReason}`);
            }
            ...
            return infoMessage('No goal set. Usage: `/goal <condition>` (or `/goal clear`).');
        }
        ...
        if (q.length > MAX_GOAL_LENGTH) {
            return errorMessage(`Goal condition is limited to ${MAX_GOAL_LENGTH} characters (got ${q.length}).`);
        }
        // ── Branch 4: gates ──────────────────────────────────────────────────
        ...
        if (config.getDisableAllHooks()) {
            return errorMessage('/goal is disabled because hooks are turned off in this session (`disableAllHooks` or bare mode).');
        }
        if (!config.getHookSystem()) {
            return errorMessage('Hook system is not initialized; cannot set a /goal in this session.');
        }
        ...
        try {
            ...
            return errorMessage(`Failed to set goal: ${err instanceof Error ? err.message : String(err)}`);
        }
        const setItem = {
            type: MessageType.GOAL_STATUS,
            kind: 'set',
