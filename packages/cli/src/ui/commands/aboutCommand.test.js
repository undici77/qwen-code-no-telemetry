...
describe('aboutCommand', () => {
    ...
    it('paths subcommand should return current session file paths', async () => {
        const pathsSubCommand = aboutCommand.subCommands?.find((sc) => sc.name === 'paths');
        if (!pathsSubCommand?.action) {
            throw new Error('The paths subcommand must have an action.');
        }
        const result = (await pathsSubCommand.action(mockContext, ''));
        expect(sessionPathsUtils.collectSessionPathInfo).toHaveBeenCalledWith(mockContext);
