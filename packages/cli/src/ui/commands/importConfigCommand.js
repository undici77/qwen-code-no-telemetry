...
export function parseImportConfigArgs(args) {
...
export function resolveImportSourceForScope(source, scope, sourceExplicit) {
...
function usage() {
    return [
        ...
        '',
        t('Usage: /import-config [all|claude-code|claude-desktop] [--scope user|project]'),
        '',
        t('Examples:'),
        '  /import-config',
...
export function formatClaudeMcpImportResult(result) {
    ...
    if (result.imported.length > 0) {
        lines.push(t('Imported {{count}} MCP server(s) to {{scope}} settings: {{names}}', {
        ...
        lines.push(t('No new Claude MCP servers imported into {{scope}} settings.', {
            scope: target,
        }));
    }
    if (skippedExisting.length > 0) {
        lines.push(t('Skipped existing server(s): {{names}}', {
            names: formatNameList([...new Set(skippedExisting)]),
        }));
    }
    if (skippedReserved.length > 0) {
        lines.push(t('Skipped unsupported server name(s): {{names}}', {
    ...
    const messageType = result.errors.length > 0 || result.skipped.length > 0 ? 'warning' : 'info';
    return {
        type: 'message',
        messageType,
        content: lines.join('\n'),
...
export const importConfigCommand = {
    name: 'import-config',
    get description() {
    ...
    action: async (context, args) => {
        ...
        try {
            const importSource = resolveImportSourceForScope(parsed.source, parsed.scope, parsed.sourceExplicit);
...
//# sourceMappingURL=importConfigCommand.js.map