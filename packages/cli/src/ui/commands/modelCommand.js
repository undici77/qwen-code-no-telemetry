...
import { AuthType, resolveModelId, } from '@qwen-code/qwen-code-core';
import { parseAcpModelOption } from '../../utils/acpModelUtils.js';
function persistSetting(settings, path, value) {
...
function getAvailableModelIds(context) {
...
export const modelCommand = {
    ...
    get description() {
        return t('Switch the model for this session (--fast for suggestion model, [model-id] to switch immediately).');
    },
    ...
    completion: async (context, partialArg) => {
        if (partialArg && '--fast'.startsWith(partialArg)) {
            ...
            return getAvailableModelIds(context).filter((id) => id.startsWith(partialArg.trim()));
        }
        else {
            return null;
        }
    ...
    action: async (context, actionArgs) => {
        ...
        if (modelName) {
            ...
            const availableModels = config.getAvailableModelsForAuthType(targetAuthType);
            ...
            const effectiveModelName = await switchMainModel(config, settings, authType, modelName);
            return {
                type: 'message',
                messageType: 'info',
