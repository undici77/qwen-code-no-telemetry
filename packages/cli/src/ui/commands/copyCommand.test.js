...
describe('copyCommand', () => {
    ...
    it('should copy the last matching language code block with /copy code mermaid', async () => {
        ...
        expect(mockCopyToClipboard).toHaveBeenCalledWith('sequenceDiagram\n  A->>B: hello');
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
...
math then literal: $x^2\$$`,
    ...
    it('should copy a numbered inline LaTeX expression with /copy latex inline 1', async () => {
        ...
        expect(mockCopyToClipboard).toHaveBeenCalledWith('x = \\frac{-b \\pm \\sqrt{b^2 - 4ac}}{2a}');
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Inline LaTeX expression 1 copied to the clipboard',
    ...
    it('should not copy inline LaTeX from code fences or display math blocks', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'No matching inline LaTeX expression found in the last AI output.',
        });
        expect(mockCopyToClipboard).not.toHaveBeenCalled();
    });
