...
describe('effortCommand', () => {
    ...
    beforeEach(() => {
        ...
        let currentEffort;
        setReasoningEffort = vi.fn((effort) => {
    ...
    it('lists tiers when called with no args non-interactively', async () => {
        ...
        const res = await effortCommand.action(nonInteractive, '');
    ...
    it('sets and persists a valid tier', async () => {
        ...
        expect(setValue).toHaveBeenCalledWith(expect.anything(), 'model.reasoningEffort', 'high');
    ...
    it('reports thinking is disabled when setReasoningEffort is a no-op', async () => {
        ...
        expect(setValue).toHaveBeenCalledWith(expect.anything(), 'model.reasoningEffort', 'high');
        expect(res).toMatchObject({ messageType: 'info' });
        expect(res.content).toContain('thinking is currently disabled');
    });
    it('reports a static override while thinking is disabled', async () => {
        ...
        context.services.config['getReasoningEffortOverride'] = getReasoningEffortOverride;
        const res = await effortCommand.action(context, 'high');
        expect(res.content).toContain('thinking is currently disabled');
        expect(res.content).toContain('will still have higher priority');
    });
    it('reports a higher-priority static thinking knob', async () => {
        ...
        expect(setValue).toHaveBeenCalledWith(expect.anything(), 'model.reasoningEffort', 'max');
        expect(res).toMatchObject({ messageType: 'info' });
        expect(res.content).toContain('higher priority');
        expect(res.content).toContain('will remain effective');
    });
...
//# sourceMappingURL=effort-command.test.js.map