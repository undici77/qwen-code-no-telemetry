...
describe('directoryCommand', () => {
    ...
    it('returns usage hint when invoked without a subcommand', async () => {
        const result = await directoryCommand.action?.({}, '');
    ...
    describe('add', () => {
        ...
        it('should expand Windows-style home-relative paths before adding directories', async () => {
            ...
            expect(mockWorkspaceContext.addDirectory).toHaveBeenCalledWith(homeProject);
        });
        it('should persist added directories to workspace settings', async () => {
            ...
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'context.includeDirectories', [existingPath, newPath]);
        });
        it('should not duplicate existing workspace settings when persisting', async () => {
            ...
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'context.includeDirectories', [existingPath]);
        });
        it('should not persist directories skipped by the workspace context', async () => {
            ...
            vi.mocked(mockWorkspaceContext.addDirectory).mockImplementation(() => undefined);
            if (!addCommand?.action)
                throw new Error('No action');
            const result = (await addCommand.action(mockContext, skippedPath));
            ...
            expect(result.content).not.toContain('Successfully added directories');
        });
        it('should return already-added directories without a success message', async () => {
            ...
            expect(result.content).toContain(existingPath);
            expect(result.content).not.toContain('Successfully added');
        });
        it('should preserve env-var-form include directories when persisting', async () => {
            ...
            const resolvedExistingPath = path.normalize('/home/user/existing-project');
            const newPath = path.normalize('/home/user/new-project');
            mockContext.services.settings.workspace.settings = {
            ...
            if (!addCommand?.action)
                throw new Error('No action');
            await addCommand.action(mockContext, newPath);
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'context.includeDirectories', [originalExistingPath, newPath]);
        });
        it('should persist the directory path accepted by the workspace context', async () => {
            ...
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'context.includeDirectories', [acceptedPath]);
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining(`Successfully added directories:\n- ${acceptedPath}`),
        ...
        it('should call addDirectory for each path and return a success message for multiple paths', async () => {
            ...
            if (!addCommand?.action)
                throw new Error('No action');
            const result = await addCommand.action(mockContext, `${newPath1},${newPath2}`);
        ...
        it('should return an error if addDirectory throws an exception', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: expect.stringContaining(`Error adding '${newPath}': ${error.message}`),
            });
        });
        it('should handle a mix of successful and failed additions', async () => {
            ...
            if (!addCommand?.action)
                throw new Error('No action');
            const result = await addCommand.action(mockContext, `${validPath},${invalidPath}`);
            ...
            expect(content).toContain(`Error adding '${invalidPath}': ${error.message}`);
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'context.includeDirectories', [validPath]);
        });
        it('should warn when gemini.addDirectoryContext throws', async () => {
            ...
            const content = result.content;
            expect(content).toContain('Error notifying model of new directories: gemini unavailable');
            expect(content).toContain('Successfully added directories');
        });
    });
...
describe('getDirPathCompletions', () => {
    ...
    describe('directory completions should include isDirectory flag', () => {
        ...
        it('should complete Windows-style home-relative paths', () => {
            const homeSubdir = fs.mkdtempSync(path.join(os.homedir(), `qwen-dir-complete-${process.pid}-`));
            const partialName = path.basename(homeSubdir).slice(0, -2);
            try {
                const results = getDirPathCompletions(`~\\${partialName}`);
                expect(results.length).toBeGreaterThan(0);
                expect(results.some((suggestion) => suggestion.value === homeSubdir + path.sep)).toBe(true);
                results.forEach((suggestion) => {
                    expect(suggestion.isDirectory).toBe(true);
                    expect(path.basename(suggestion.value.slice(0, -1))).toContain(partialName);
                    expect(suggestion.value.endsWith(path.sep)).toBe(true);
                });
            }
            finally {
