...
function createMockContext(mergedSettings = {}) {
...
describe('configCommand', () => {
    ...
    describe('enum settings', () => {
        it('sets a valid enum value', async () => {
            const { ctx, setValuesMock } = createMockContext({});
            const result = await configCommand.action(ctx, 'tools.approvalMode=auto');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
        ...
        it('returns error for invalid enum value', async () => {
            const { ctx } = createMockContext({});
            const result = await configCommand.action(ctx, 'tools.approvalMode=invalid_mode');
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
    ...
    describe('string settings', () => {
        it('sets a string value', async () => {
            const { ctx, setValuesMock } = createMockContext({});
            const result = await configCommand.action(ctx, 'general.preferredEditor=vim');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
        ...
        it('shows current value when toggling string', async () => {
            ...
            const result = await configCommand.action(ctx, 'general.preferredEditor');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
    ...
    describe('number settings', () => {
        it('sets a number value', async () => {
            const { ctx, setValuesMock } = createMockContext({});
            const result = await configCommand.action(ctx, 'general.sessionRecapAwayThresholdMinutes=10');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
        ...
        it('returns error for invalid number value', async () => {
            const { ctx } = createMockContext({});
            const result = await configCommand.action(ctx, 'general.sessionRecapAwayThresholdMinutes=abc');
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
        ...
        it('returns error for empty number value', async () => {
            const { ctx } = createMockContext({});
            const result = await configCommand.action(ctx, 'general.sessionRecapAwayThresholdMinutes=');
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: expect.stringContaining('Invalid number'),
        ...
        it('returns error for Infinity', async () => {
            const { ctx } = createMockContext({});
            const result = await configCommand.action(ctx, 'general.sessionRecapAwayThresholdMinutes=Infinity');
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: expect.stringContaining('Invalid number'),
        ...
        it('shows current value when toggling number', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining('general.sessionRecapAwayThresholdMinutes = 30'),
            });
        });
    });
    ...
    describe('restart warning', () => {
        it('shows restart warning for settings that require restart', async () => {
            const { ctx } = createMockContext({});
            const result = await configCommand.action(ctx, 'proxy=http://localhost:8080');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
    ...
    describe('security-sensitive settings', () => {
        ...
        it('masks sensitive value in write confirmation message', async () => {
            const { ctx } = createMockContext({});
            const result = await configCommand.action(ctx, 'proxy=http://secret-proxy:8080');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.not.stringContaining('secret-proxy'),
    ...
    describe('security blocklist', () => {
        it('blocks setting tools.approvalMode to yolo', async () => {
            const { ctx, setValuesMock } = createMockContext({});
            const result = await configCommand.action(ctx, 'tools.approvalMode=yolo');
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
    ...
    describe('completion', () => {
        it('provides completions for partial key', async () => {
            ...
            expect(completions.some((c) => typeof c === 'string'
                ? c.includes('vimMode')
                : c.value.includes('vimMode'))).toBe(true);
        });
        it('returns null when completing after = sign', async () => {
            const { ctx } = createMockContext({});
            const completions = await configCommand.completion(ctx, 'general.vimMode=');
