...
import { getAllSettingKeys, getFlattenedSchema, getNestedProperty, getSettingDefinition, validateSettingValue, } from '../../utils/settingsUtils.js';
...
function findClosestKey(input) {
    ...
    for (const key of allKeys) {
        ...
        if (!def || !SETTABLE_TYPES.has(def.type))
            continue;
    ...
    switch (def.type) {
        ...
        case 'enum': {
            ...
            if (!validValues.includes(rawValue)) {
                return {
                    value: undefined,
                    error: t('Invalid enum value: "{{value}}". Valid values: {{options}}.', { value: String(rawValue), options: validValues.join(', ') }),
                };
            }
        ...
        case 'object':
            return {
                value: undefined,
                error: t('Setting "{{type}}" type cannot be set via /config. Edit settings.json directly.', { type: def.type }),
            };
        default:
            return {
                value: undefined,
...
function listAllSettings(context) {
    ...
    for (const key of keys) {
        ...
        if (!SETTABLE_TYPES.has(def.type))
            continue;
    ...
    action: async (context, args) => {
        ...
        if (!def) {
            ...
            return {
                ...
                messageType: 'error',
                content: suggestion
                    ? t('Unknown setting key: "{{key}}". Did you mean "{{suggestion}}"?', { key, suggestion })
                    : t('Unknown setting key: "{{key}}".', { key }),
            };
        }
        const currentValue = getNestedProperty(context.services.settings.merged, key);
        ...
        if (key === 'tools.approvalMode' && result.value === 'yolo') {
            return {
                type: 'message',
                messageType: 'error',
                content: t('Setting tools.approvalMode to "yolo" is blocked via /config for security reasons. Edit settings.json directly if you understand the risks.'),
            };
        }
        try {
            context.services.settings.setValues([
        ...
        if (isSensitiveKey(key)) {
            message +=
                '\n' +
                    t('(Security-sensitive setting — verify you are not exposing credentials.)');
        }
        return {
            type: 'message',
            messageType: 'info',
