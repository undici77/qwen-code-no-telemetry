...
describe('compressCommand', () => {
    ...
    it('should set pending item, call tryCompressChat, and add result on success', async () => {
        ...
        expect(mockTryCompressChat).toHaveBeenCalledWith(expect.stringMatching(/^compress-\d+$/), true, undefined, undefined);
    ...
    describe('custom instructions argument', () => {
        ...
        it('forwards trimmed instructions as the 4th argument', async () => {
            ...
            expect(mockTryCompressChat).toHaveBeenCalledWith(expect.stringMatching(/^compress-\d+$/), true, undefined, 'focus on auth bug');
        });
        ...
        it('caps overlong instructions at 2000 chars', async () => {
            ...
            expect(call[3].length).toBe(2000);
        });
        it('surfaces an INFO notice to the user when instructions are truncated', async () => {
            ...
            await compressCommand.action(ctx, '');
            expect(ctx.ui.addItem).toHaveBeenCalledWith(expect.objectContaining({
                type: MessageType.INFO,
        ...
        it('does NOT show a truncation notice when instructions fit under the cap', async () => {
            ...
            await compressCommand.action(ctx, '');
            const infoCalls = ctx.ui.addItem.mock.calls.filter((c) => c[0].type === MessageType.INFO &&
                typeof c[0].text === 'string' &&
                c[0].text.includes('truncated'));
