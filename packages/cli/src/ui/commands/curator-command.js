 ...
 */
import { getAutoSkillCuratorStatus, restoreArchivedAutoSkill, runAutoSkillCurator, setAutoSkillPinned, } from '@qwen-code/qwen-code-core';
...
function formatRun(result) {
    ...
    if (result.skippedCollisions.length > 0) {
        ...
        lines.push(...result.skippedCollisions.map((name) => `  ${name} — remove or rename .qwen/archived-skills/${name} to re-archive`));
    }
