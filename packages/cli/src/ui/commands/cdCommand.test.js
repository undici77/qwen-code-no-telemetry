...
import { loadTrustedFolders, resetTrustedFoldersForTesting, } from '../../config/trustedFolders.js';
...
describe('cdCommand', () => {
    ...
    let nextDir;
    let context;
    let relocateWorkingDirectory;
    let addWorkingDirectoryChangedContext;
    beforeEach(() => {
        ...
        process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'] = path.join(tmpDir, 'trustedFolders.json');
    ...
    it('has command metadata', () => {
        ...
        expect(cdCommand.description).toBe('Move this session to a new working directory');
    ...
    it('shows usage when no path is provided', async () => {
        const result = (await cdCommand.action?.(context, '   '));
    ...
    it('does not use comma-separated path completion semantics', async () => {
        const completions = await cdCommand.completion?.(context, `${currentDir}, ${nextDir.slice(0, -1)}`);
        expect(completions).toEqual([]);
    });
    it('rejects a missing directory', async () => {
        const result = (await cdCommand.action?.(context, 'missing'));
    ...
    it('rejects a file target', async () => {
        ...
        const result = (await cdCommand.action?.(context, 'file.txt'));
        expect(result.type).toBe('message');
        expect(result.messageType).toBe('error');
    ...
    it('rejects switching while a response is in progress', async () => {
        ...
        const result = (await cdCommand.action?.(context, nextDir));
        expect(result.type).toBe('message');
        expect(result.messageType).toBe('error');
    ...
    it('moves to a relative path resolved from the current target directory', async () => {
        const result = (await cdCommand.action?.(context, '../next'));
        const realCurrentDir = await realpath(currentDir);
        const realNextDir = await realpath(nextDir);
        expect(relocateWorkingDirectory).toHaveBeenCalledWith(realNextDir, realNextDir);
        expect(addWorkingDirectoryChangedContext).toHaveBeenCalledWith(realCurrentDir, realNextDir);
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
    ...
    it('resolves a Windows-style home-relative path from the home directory', async () => {
        ...
        const result = (await cdCommand.action?.(context, `~\\${missingName}`));
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
    ...
    it('moves to a Windows-style home-relative directory', async () => {
        const homeSubdir = fs.mkdtempSync(path.join(os.homedir(), `qwen-cd-ok-${process.pid}-`));
        try {
            const result = (await cdCommand.action?.(context, `~\\${path.basename(homeSubdir)}`));
            const realCurrentDir = await realpath(currentDir);
            const realHomeSubdir = await realpath(homeSubdir);
            expect(relocateWorkingDirectory).toHaveBeenCalledWith(realHomeSubdir, realHomeSubdir);
            expect(addWorkingDirectoryChangedContext).toHaveBeenCalledWith(realCurrentDir, realHomeSubdir);
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
    ...
    it('moves to a path with escaped spaces', async () => {
        ...
        const result = (await cdCommand.action?.(context, '../space\\ dir'));
        const realSpacedDir = await realpath(spacedDir);
        expect(relocateWorkingDirectory).toHaveBeenCalledWith(realSpacedDir, realSpacedDir);
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
    ...
    it('reports a successful move when model context refresh fails afterward', async () => {
        addWorkingDirectoryChangedContext.mockRejectedValue(new Error('context failed'));
        const result = (await cdCommand.action?.(context, '../next'));
        const realNextDir = await realpath(nextDir);
        expect(relocateWorkingDirectory).toHaveBeenCalledWith(realNextDir, realNextDir);
        expect(result).toEqual({
    ...
    it('drops stale trust confirmations after the pending cap', async () => {
        ...
        for (let i = 0; i <= 50; i++) {
            ...
            const result = (await cdCommand.action?.(context, `pending-${i}`));
            expect(result.type).toBe('confirm_action');
        }
        context = createMockCommandContext({
            invocation: {
        ...
        const result = (await cdCommand.action?.(context, 'pending-0'));
        expect(result.type).toBe('confirm_action');
        expect(relocateWorkingDirectory).not.toHaveBeenCalled();
    });
    it('asks again when confirmed path resolves to a different directory', async () => {
        ...
        expect(relocateWorkingDirectory).not.toHaveBeenCalled();
        expect(loadTrustedFolders().isPathTrusted(await realpath(changedDir))).toBe(undefined);
    });
    ...
    it('reports a trust write failure after a confirmed move', async () => {
        ...
        await cdCommand.action?.(context, '../next');
        context.overwriteConfirmed = true;
        fs.writeFileSync(process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'], 'invalid json');
        const result = (await cdCommand.action?.(context, '../next'));
        const realNextDir = await realpath(nextDir);
        expect(relocateWorkingDirectory).toHaveBeenCalledWith(realNextDir, realNextDir);
        expect(result).toEqual({
            type: 'message',
            messageType: 'warning',
            content: expect.stringContaining(`Moved to ${realNextDir}. Trust setting update failed:`),
        });
    });
...
//# sourceMappingURL=cdCommand.test.js.map