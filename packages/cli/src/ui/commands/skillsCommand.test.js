...
function makeContext(opts) {
    const { skills = [], workspaceDisabled = [], mergedDisabled = workspaceDisabled, isTrusted = true, executionMode = 'interactive', } = opts;
    const skillManager = {
    ...
    const disabledSet = new Set(mergedDisabled
...
describe('skillsCommand bare entry', () => {
    ...
    it('shows a clarifying message when all skills are disabled in non-interactive mode', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: expect.stringMatching(/disabled.*settings\.json|skills\.disabled/i),
        });
    });
});
