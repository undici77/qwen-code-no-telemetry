...
const renderWithMockedStats = (metrics, modelPricing, width) => {
    ...
    return render(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ModelStatsDisplay, { width: width }) }));
};
describe('<ModelStatsDisplay />', () => {
    ...
    it('keeps fixed model column widths for multiple models even when space is available', () => {
        ...
        expect(headerLine.indexOf('model-b') - headerLine.indexOf('model-a')).toBe(24);
    });
    describe('Subagent source attribution', () => {
        ...
        describe('Cost estimation', () => {
            ...
            it('should display $0.0000 rather than N/A for a model priced at zero', () => {
                const { lastFrame } = renderCostTest({ 'qwen3-coder:free': mainOnly(makeCore(1000, 2000, 0)) }, {
                    'qwen3-coder:free': {
