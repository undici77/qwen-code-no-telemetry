...
describe('SuggestionsDisplay mouse wiring', () => {
    ...
    it('mounts RowMouseController with the scroll offset + callbacks when enabled', () => {
        ...
        render(_jsx(SuggestionsDisplay, { suggestions: suggestions, activeIndex: 0, isLoading: false, width: 60, scrollOffset: 2, userInput: "", mode: "slash", mouseEnabled: true, onHoverIndex: onHoverIndex, onSelectIndex: onSelectIndex }));
    ...
    it('does not mount RowMouseController when mouse is disabled', () => {
        render(_jsx(SuggestionsDisplay, { suggestions: suggestions, activeIndex: 0, isLoading: false, width: 60, scrollOffset: 0, userInput: "", mode: "slash", mouseEnabled: false, onHoverIndex: vi.fn(), onSelectIndex: vi.fn() }));
        expect(RowMouseController).not.toHaveBeenCalled();
    });
    it('does not mount RowMouseController when the callbacks are absent', () => {
        render(_jsx(SuggestionsDisplay, { suggestions: suggestions, activeIndex: 0, isLoading: false, width: 60, scrollOffset: 0, userInput: "", mode: "slash", mouseEnabled: true }));
        expect(RowMouseController).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=SuggestionsDisplay.mouse.test.js.map