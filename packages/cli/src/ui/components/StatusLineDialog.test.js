...
describe('StatusLineDialog', () => {
    ...
    it('persists selected presets on enter', async () => {
        ...
        const onClose = vi.fn();
        const onSaved = vi.fn();
        const { stdin } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(StatusLineDialog, { settings: settings, config: config, uiState: uiState, addItem: addItem, onSaved: onSaved, onClose: onClose, availableTerminalHeight: 18 }) }));
        act(() => {
            stdin.write('\r');
        });
