...
function createManager() {
    return {
        ...
        getFavorites: vi.fn(() => []),
        getExtensionScopes: vi.fn(() => ({ demo: 'user' })),
...
describe('InstalledTab', () => {
    ...
    it('surfaces activation warnings and reloads the installed list', async () => {
        ...
        const statuses = [];
        const { lastFrame } = render(_jsx(InstalledTab, { config: config, isActive: true, onLockChange: vi.fn(), onStatus: (status) => statuses.push(status), extensionsUpdateState: new Map(), reloadSignal: 0 }));
        ...
        await waitFor(() => expect(manager.disableExtension).toHaveBeenCalledOnce());
...
//# sourceMappingURL=InstalledTab.test.js.map