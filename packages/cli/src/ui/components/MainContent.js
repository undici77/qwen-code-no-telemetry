...
export const MainContent = () => {
    ...
    useEffect(() => {
        if (replayCount >= mergedHistory.length)
            return;
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Static, { items: [
                    ...
                    ...visibleHistoryItemsWithSourceCopyOffsets.map(({ item: h, sourceCopyIndexOffsets }) => (_jsx(HistoryItemDisplay, { terminalWidth: terminalWidth, mainAreaWidth: mainAreaWidth, availableTerminalHeight: staticAreaMaxItemHeight, availableTerminalHeightGemini: MAX_GEMINI_MESSAGE_LINES, item: h, isPending: false, commands: uiState.slashCommands, compactLabel: getCompactLabel(h), summaryAbsorbed: isSummaryAbsorbed(h), sourceCopyIndexOffsets: sourceCopyIndexOffsets }, h.id))),
                ], children: (item) => item }, `${historyRemountKey}-${uiState.currentModel}`), _jsx(OverflowProvider, { children: _jsxs(Box, { flexDirection: "column", children: [pendingHistoryItemsWithSourceCopyOffsets.map(({ item, sourceCopyIndexOffsets }, i) => (_jsx(HistoryItemDisplay, { availableTerminalHeight: uiState.constrainHeight ? availableTerminalHeight : undefined, terminalWidth: terminalWidth, mainAreaWidth: mainAreaWidth, item: { ...item, id: 0 }, isPending: true, isFocused: !uiState.isEditorDialogOpen, activeShellPtyId: uiState.activePtyId, embeddedShellFocused: uiState.embeddedShellFocused, compactLabel: getCompactLabel(item), summaryAbsorbed: isSummaryAbsorbed(item), sourceCopyIndexOffsets: sourceCopyIndexOffsets }, i))), _jsx(ShowMoreLines, { constrainHeight: uiState.constrainHeight })] }) })] }));
};
