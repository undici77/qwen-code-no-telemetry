...
const renderWithWidth = (width, uiState, configOverrides = {}) => {
    useTerminalSizeMock.mockReturnValue({ columns: width, rows: 24 });
    const mockSettings = createMockSettings();
    return render(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ConfigContext.Provider, { value: createMockConfig(configOverrides), children: _jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(VimModeProvider, { settings: mockSettings, children: _jsx(UIStateContext.Provider, { value: uiState, children: _jsx(Footer, {}) }) }) }) }) }));
};
const runningShellEntry = {
    kind: 'shell',
    id: 'shell-1',
    shellId: 'shell-1',
...
const pendingApprovalAgentEntry = {
    kind: 'agent',
    id: 'agent-1',
    agentId: 'agent-1',
...
const createBackgroundTaskState = (entries) => ({
    entries,
    selectedIndex: 0,
    dialogMode: 'closed',
    dialogOpen: false,
...
const renderAtLayoutWidth = (columns, uiState, backgroundEntries = [], containerRef) => {
    ...
    const mockSettings = createMockSettings();
    const footer = backgroundEntries.length > 0 ? (_jsx(BackgroundTaskViewStateContext.Provider, { value: createBackgroundTaskState(backgroundEntries), children: _jsx(Footer, { containerRef: containerRef }) })) : (_jsx(Footer, { containerRef: containerRef }));
    const instance = inkRender(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ConfigContext.Provider, { value: createMockConfig(), children: _jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(VimModeProvider, { settings: mockSettings, children: _jsx(UIStateContext.Provider, { value: uiState, children: footer }) }) }) }) }), {
        stdout: stdout,
        stderr: stderr,
        stdin: stdin,
...
describe('<Footer />', () => {
    ...
    it('attaches the selectable-region ref to its outer box', () => {
        ...
        const { unmount } = renderAtLayoutWidth(80, createMockUIState(), [], containerRef);
    ...
    it('shows the "workflow active" indicator when the keyword trigger is armed', () => {
        const { lastFrame } = renderWithWidth(120, createMockUIState({ workflowKeywordActive: true }));
    ...
    it('shows the queued-count badge outside streaming', () => {
        const { lastFrame } = renderWithWidth(120, createMockUIState({ messageQueue: ['waiting'] }));
        expect(lastFrame()).toContain('1 queued');
    });
    it('hides the queued-count badge when the queue is empty', () => {
        const { lastFrame } = renderWithWidth(120, createMockUIState({ messageQueue: [] }));
        expect(lastFrame()).not.toContain('queued');
    });
    it.each([ApprovalMode.AUTO, ApprovalMode.DEFAULT])('keeps the hint row on one line at 80 columns with %s, a queued message, active pills, and a pending skill review', (mode) => {
        ...
        const { lastFrame, unmount } = renderAtLayoutWidth(80, createMockUIState({
            streamingState: StreamingState.Responding,
    ...
    it('shows deferred IDE connection progress', () => {
        ...
        expect(lastFrame()).toContain('IDE connecting... context may be unavailable');
    });
    it('shows deferred IDE connection failures', () => {
        ...
        expect(lastFrame()).toContain('IDE connection unavailable: ide_connect timed out after 10000ms');
    });
