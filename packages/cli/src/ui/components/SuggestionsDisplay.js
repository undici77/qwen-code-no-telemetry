...
export function SuggestionsDisplay({ suggestions, activeIndex, isLoading, width, scrollOffset, userInput, mode, expandedIndex, onHoverIndex, onSelectIndex, mouseEnabled, activeCategory = 'all', availableCategories, }) {
    ...
    const endIndex = Math.min(scrollOffset + MAX_SUGGESTIONS_TO_SHOW, filteredSuggestions.length);
    ...
    const maxLabelLength = Math.max(...filteredSuggestions.map((s) => getFullLabel(s).length));
    ...
    const labelColumnWidth = mode === 'slash'
        ...
        : describedLabelLengths.length > 0
            ...
            : 0;
    return (_jsxs(Box, { flexDirection: "column", width: width, ref: containerRef, children: [mouseEnabled && onHoverIndex && onSelectIndex && (_jsx(RowMouseController, { containerRef: containerRef, itemRefs: itemRefs, scrollOffset: startIndex, onHoverIndex: onHoverIndex, onSelectIndex: onSelectIndex })), showTabBar && availableCategories && (_jsxs(Box, { flexDirection: "row", marginBottom: 1, children: [availableCategories.map((cat, i) => {
                        const active = cat === activeCategory;
                        return (_jsx(Box, { marginLeft: i === 0 ? 0 : 1, children: _jsx(Text, { color: active ? theme.background.primary : theme.text.secondary, backgroundColor: active ? theme.text.accent : undefined, children: ` ${categoryLabel(cat)} ` }) }, cat));
                    }), _jsx(Box, { marginLeft: 2, children: _jsx(Text, { color: theme.text.secondary, children: t('(←/→ to switch)') }) })] })), scrollOffset > 0 && _jsx(Text, { color: theme.text.primary, children: "\u25B2" }), visibleSuggestions.map((suggestion, index) => {
                ...
                return (_jsxs(Box, { flexDirection: "row", ref: (node) => {
                        itemRefs.current[index] = node;
                                ...
                                : { flexShrink: 1 }), children: _jsxs(Box, { children: [_jsx(Box, { flexShrink: 0, children: labelElement }), suggestion.argumentHint && (_jsxs(Text, { color: theme.text.secondary, children: [' ', suggestion.argumentHint] })), suggestion.sourceBadge && (_jsxs(Text, { color: textColor, children: [" ", suggestion.sourceBadge] }))] }) }), suggestion.description && (_jsx(Box, { width: descriptionColumnWidth, flexGrow: 1, flexShrink: 1, paddingLeft: 2, children: _jsx(Text, { color: textColor, wrap: "truncate-end", children: normalizeDescription(suggestion.description) }) })), isActive && isLong && (_jsx(Box, { children: _jsx(Text, { color: Colors.Gray, children: isExpanded ? ' ← ' : ' → ' }) }))] }, `${suggestion.value}-${originalIndex}`));
            }), endIndex < filteredSuggestions.length && _jsx(Text, { color: "gray", children: "\u25BC" }), filteredSuggestions.length > MAX_SUGGESTIONS_TO_SHOW && (_jsxs(Text, { color: "gray", children: ["(", activeIndex + 1, "/", filteredSuggestions.length, ")"] }))] }));
}
