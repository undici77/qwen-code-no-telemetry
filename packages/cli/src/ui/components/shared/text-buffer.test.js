...
import { useTextBuffer, offsetToLogicalPos, logicalPosToOffset, textBufferReducer, findWordEndInLine, findNextWordStartInLine, isWordCharStrict, __resetWordSegmenter, } from './text-buffer.js';
...
describe('textBufferReducer', () => {
    ...
    describe('kill_line_left action', () => {
        it('should clear text to the left of cursor on the same line', () => {
            ...
            const action = { type: 'kill_line_left' };
            const state = textBufferReducer(stateWithText, action);
            expect(state).toHaveOnlyValidCharacters();
...
describe('useTextBuffer', () => {
    ...
    describe('Drag and Drop File Paths', () => {
        ...
        it('should prepend @ to multiple quoted file paths separated by spaces', () => {
            ...
            expect(getBufferState(result).text).toBe('@/path/to/file1.txt @/path/to/file2.txt @/path/to/file3.txt ');
        });
        it('should prepend @ to multiple unquoted file paths separated by spaces', () => {
            const { result } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (p) => p === '/path/to/file1.txt' ||
                    p === '/path/to/file2.txt' ||
                    p === '/path/to/file3.txt',
            }));
            const filePaths = '/path/to/file1.txt /path/to/file2.txt /path/to/file3.txt';
        ...
        it('should preserve original content when not all tokens are valid paths', () => {
            ...
            const { result: result2 } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (path) => path.includes('file1') || path.includes('file2'),
            }));
            const filePaths = "'/path/to/file1.txt' '/path/to/invalid.txt' '/path/to/file2.txt'";
        ...
        it('should transform when all tokens are valid paths', () => {
            ...
            const { result: result3 } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (path) => path.includes('file1') ||
                    path.includes('file2') ||
                    path.includes('file3'),
            }));
            const filePaths = "'/path/to/file1.txt' '/path/to/file2.txt' '/path/to/file3.txt'";
            act(() => result3.current.insert(filePaths, { paste: true }));
            expect(getBufferState(result3).text).toBe('@/path/to/file1.txt @/path/to/file2.txt @/path/to/file3.txt ');
        });
        it('should handle quoted paths with spaces via greedy matching', () => {
            ...
            const { result } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (p) => p === '/path/to/my file.txt' || p === '/path/to/another file.txt',
            }));
            act(() => result.current.insert("'/path/to/my file.txt' '/path/to/another file.txt'", { paste: true }));
            expect(getBufferState(result).text).toBe('@/path/to/my\\ file.txt @/path/to/another\\ file.txt ');
        });
        it('should handle unquoted paths with spaces via greedy matching', () => {
            // Critical 3: Test unquoted paths with spaces
            const { result } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (p) => p === '/path/to/my file.txt',
            }));
        ...
        it('should escape commas in paths for parseAllAtCommands compatibility', () => {
            ...
            const { result } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (p) => p === '/path/to/report,v2.txt',
            }));
            act(() => result.current.insert("'/path/to/report,v2.txt'", { paste: true }));
        ...
        it('should escape shell metacharacters like parentheses in paths', () => {
            ...
            const { result } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (p) => p === '/Downloads/report(v2).txt' ||
                    p === '/data[2024].csv' ||
                    p === '/report;v2.txt',
            }));
            // Test parentheses
        ...
        it('should handle relative paths like ./src/index.ts', () => {
            ...
            expect(getBufferState(result).text).toBe('@./src/index.ts @../lib/utils.ts @\\~/notes.md ');
        });
        it('should handle unquoted paths with spaces via longest-match-first greedy matching', () => {
            ...
            const { result } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (p) => p === '/tmp/a b.txt' || p === '/tmp/a' || p === 'b.txt',
            }));
        ...
        it('should handle unquoted invalid paths without crashing', () => {
            ...
            const { result } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (p) => p === '/valid/file.txt',
            }));
        ...
        it('should handle Windows drive-letter paths', () => {
            ...
            const { result } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (p) => p === 'C:\\Users\\file.txt' || p === 'D:\\data\\report.csv',
            }));
            act(() => result.current.insert('C:\\Users\\file.txt D:\\data\\report.csv', {
                paste: true,
            }));
            expect(getBufferState(result).text).toBe('@C:\\Users\\file.txt @D:\\data\\report.csv ');
        });
        it('should handle quoted Windows paths with spaces', () => {
            // Suggestion 3 (wenshao #4544): test quoted Windows paths
            const { result } = renderHook(() => useTextBuffer({
                viewport,
                isValidPath: (p) => p === 'C:\\Users\\my file.txt' || p === 'D:\\data\\report.csv',
            }));
            act(() => result.current.insert("'C:\\Users\\my file.txt' 'D:\\data\\report.csv'", {
                paste: true,
            }));
            // escapePath escapes spaces, so "my file" becomes "my\ file"
            expect(getBufferState(result).text).toBe('@C:\\Users\\my\\ file.txt @D:\\data\\report.csv ');
        });
        it('should prepend @ to a bare filename when isValidPath returns true', () => {
            ...
            const { result } = renderHook(() => useTextBuffer({ viewport, isValidPath: (p) => p === 'README.md' }));
...
describe('CJK word navigation', () => {
    ...
    describe('fallback and edge cases', () => {
        ...
        it('should handle word navigation on single character', () => {
            ...
            const fromEnd = textBufferReducer({ ...state, cursorCol: 1 }, { type: 'move', payload: { dir: 'wordLeft' } });
