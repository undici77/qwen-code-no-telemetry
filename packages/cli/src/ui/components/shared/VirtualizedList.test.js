...
const makeItems = (n) => Array.from({ length: n }, (_, i) => ({ id: i, label: `item-${i}` }));
const keyExtractor = (item) => `k-${item.id}`;
const renderItem = ({ item }) => _jsx(Text, { children: item.label });
const estimatedItemHeight = () => 1;
describe('<VirtualizedList />', () => {
    it('renders nothing visible when data is empty', () => {
        const { lastFrame } = render(_jsx(VirtualizedList, { data: [], renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, containerHeight: 10, width: 40, showScrollbar: false }));
    ...
    it('renders all items when renderStatic is true (full list, no virtualization)', () => {
        const { lastFrame } = render(_jsx(VirtualizedList, { data: makeItems(5), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, renderStatic: true, containerHeight: 20, width: 40, showScrollbar: false }));
        const frame = lastFrame() ?? '';
    ...
    it('with SCROLL_TO_ITEM_END as initialScrollIndex, anchors at the last item', () => {
        ...
        function Wrapper() {
            ...
            if (ref.current)
                listRef = ref.current;
            return (_jsx(VirtualizedList, { ref: ref, data: makeItems(20), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, containerHeight: 5, width: 40, showScrollbar: false }));
        }
        const { rerender } = render(_jsx(Wrapper, {}));
        // Force commit so ref.current is populated
        rerender(_jsx(Wrapper, {}));
    ...
    it('collapses bottom-stuck viewport to measured rows when estimates are too tall', async () => {
        ...
        const { lastFrame, rerender } = render(_jsx(VirtualizedList, { data: makeItems(20), renderItem: renderItem, estimatedItemHeight: tallEstimate, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, containerHeight: 20, width: 40, showScrollbar: false }));
        for (let i = 0; i < 2; i++) {
            rerender(_jsx(VirtualizedList, { data: makeItems(20), renderItem: renderItem, estimatedItemHeight: tallEstimate, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, containerHeight: 20, width: 40, showScrollbar: false }));
            await act(async () => { });
        }
        const frame = lastFrame() ?? '';
    ...
    it('collapses a short bottom-stuck list below the container height', async () => {
        const { lastFrame, rerender } = render(_jsx(VirtualizedList, { data: makeItems(5), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, containerHeight: 20, width: 40, showScrollbar: false }));
        rerender(_jsx(VirtualizedList, { data: makeItems(5), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, containerHeight: 20, width: 40, showScrollbar: false }));
        await act(async () => { });
        const frame = lastFrame() ?? '';
        expect(frame.split('\n')).toEqual([
            'item-0',
    ...
    it('reports zero-height shrink so collapsed items leave no blank gap', async () => {
        ...
        const BodyWhenExpanded = () => {
            ...
            if (!expanded)
                return null;
            return (_jsx(Box, { flexDirection: "column", children: Array.from({ length: 30 }, (_, i) => (_jsx(Text, { children: `thinking line ${i}` }, i))) }));
        };
        const items = [
            { id: 0, kind: 'head' },
            { id: 1, kind: 'body' },
        ...
        function Wrapper() {
            ...
            const renderItem = useCallback(({ item }) => item.kind === 'head' ? (_jsx(Text, { children: "thought head" })) : (_jsx(BodyWhenExpanded, {})), []);
            return (_jsx(ExpandedContext.Provider, { value: { expanded }, children: _jsxs(Box, { flexDirection: "column", children: [_jsx(VirtualizedList, { data: items, renderItem: renderItem, estimatedItemHeight: () => 3, keyExtractor: (item) => `t-${item.id}`, initialScrollIndex: SCROLL_TO_ITEM_END, isStaticItem: () => true, containerHeight: 40, width: 40, showScrollbar: false }), _jsx(Text, { children: "FOOTER" })] }) }));
        }
        const { lastFrame } = render(_jsx(Wrapper, {}));
        await act(async () => { });
    ...
    it('collapses after measuring changed content at full viewport height', async () => {
        ...
        const { frames, lastFrame, rerender } = render(_jsx(VirtualizedList, { data: liveItems, renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, isStaticItem: (item) => item.id >= 0, containerHeight: 5, width: 40, showScrollbar: false }));
        const shortConfirmationItems = [{ id: -1, label: 'confirm' }];
        rerender(_jsx(VirtualizedList, { data: shortConfirmationItems, renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, isStaticItem: (item) => item.id >= 0, containerHeight: 5, measureAtFullHeight: true, width: 40, showScrollbar: false }));
        await act(async () => { });
        ...
        rerender(_jsx(VirtualizedList, { data: longConfirmationItems, renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, isStaticItem: (item) => item.id >= 0, containerHeight: 5, measureAtFullHeight: true, width: 40, showScrollbar: false }));
        await act(async () => { });
        expect(frames.slice(frameCountBeforeLongContent).map((frame) => frame.trimEnd())).not.toContain('confirm');
    ...
    it('targetScrollIndex anchors to that index on first usable render', () => {
        ...
        function Wrapper() {
            ...
            if (ref.current)
                listRef = ref.current;
            return (_jsx(VirtualizedList, { ref: ref, data: makeItems(10), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, targetScrollIndex: 5, containerHeight: 4, width: 40, showScrollbar: false }));
        }
        const { rerender } = render(_jsx(Wrapper, {}));
        rerender(_jsx(Wrapper, {}));
        expect(listRef).not.toBeNull();
    ...
    it('clamps an out-of-range targetScrollIndex instead of freezing the walk-back', () => {
        ...
        function Wrapper({ target }) {
            return (_jsx(VirtualizedList, { ref: ref, data: makeItems(10), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, targetScrollIndex: target, containerHeight: 4, width: 40, showScrollbar: false }));
        }
        const { rerender } = render(_jsx(Wrapper, { target: 5 }));
        ...
        // to here.
        rerender(_jsx(Wrapper, { target: SCROLL_TO_ITEM_END }));
        expect(ref.current.getScrollIndex()).toBe(6);
        rerender(_jsx(Wrapper, { target: -5 }));
    ...
    it('walks a mount-time targetScrollIndex anchor back to the run start', () => {
        ...
        const zeroRunEstimate = (i) => (i >= 2 && i <= 5 ? 0 : 2);
        function Wrapper() {
            return (_jsx(VirtualizedList, { ref: ref, data: makeItems(10), renderItem: renderItem, estimatedItemHeight: zeroRunEstimate, keyExtractor: keyExtractor, targetScrollIndex: 4, containerHeight: 5, width: 40, showScrollbar: false }));
        }
        render(_jsx(Wrapper, {}));
        expect(ref.current).not.toBeNull();
    ...
    it('exposes scrollToEnd via imperative ref and snaps to the last item', () => {
        ...
        function Wrapper() {
            ...
            if (ref.current)
                listRef = ref.current;
            return (_jsx(VirtualizedList, { ref: ref, data: makeItems(30), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 0, containerHeight: 5, width: 40, showScrollbar: false }));
        }
        const { rerender } = render(_jsx(Wrapper, {}));
        rerender(_jsx(Wrapper, {}));
        expect(listRef).not.toBeNull();
    ...
    it('scrollToIndex moves scroll anchor to the requested index', () => {
        ...
        function Wrapper() {
            ...
            if (ref.current)
                listRef = ref.current;
            return (_jsx(VirtualizedList, { ref: ref, data: makeItems(50), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 0, containerHeight: 5, width: 40, showScrollbar: false }));
        }
        const { rerender } = render(_jsx(Wrapper, {}));
        rerender(_jsx(Wrapper, {}));
        expect(listRef).not.toBeNull();
    ...
    it('survives a renderItem that throws (isolates per-item errors)', () => {
        ...
        expect(() => render(_jsx(VirtualizedList, { data: data, renderItem: renderWithBomb, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, renderStatic: true, containerHeight: 5, width: 40, showScrollbar: false }))).not.toThrow();
    });
    it('estimator returning NaN/negative is coerced to 0 (no scroll-math poison)', () => {
        let listRef = null;
        const badEstimator = (i) => {
            if (i === 1)
                return Number.NaN;
            if (i === 2)
                return -10;
        ...
        function Wrapper() {
            ...
            if (ref.current)
                listRef = ref.current;
            return (_jsx(VirtualizedList, { ref: ref, data: makeItems(4), renderItem: renderItem, estimatedItemHeight: badEstimator, keyExtractor: keyExtractor, containerHeight: 5, width: 40, showScrollbar: false }));
        }
        expect(() => {
            const { rerender } = render(_jsx(Wrapper, {}));
            rerender(_jsx(Wrapper, {}));
    ...
    it('handles initialScrollIndex pointing past the end gracefully', () => {
        ...
        function Wrapper() {
            ...
            if (ref.current)
                listRef = ref.current;
            return (_jsx(VirtualizedList, { ref: ref, data: makeItems(5), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 9999, containerHeight: 5, width: 40, showScrollbar: false }));
        }
        const { rerender } = render(_jsx(Wrapper, {}));
        rerender(_jsx(Wrapper, {}));
        expect(listRef).not.toBeNull();
    ...
    describe('scrollBy', () => {
        ...
        it('scrollBy negative moves the viewport up and clears sticking-to-bottom', () => {
            ...
            function Wrapper() {
                ...
                if (ref.current)
                    listRef = ref.current;
                return (_jsx(VirtualizedList, { ref: ref, data: makeItems(30), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, containerHeight: 5, width: 40, showScrollbar: false }));
            }
            const { rerender } = render(_jsx(Wrapper, {}));
            rerender(_jsx(Wrapper, {}));
            expect(listRef).not.toBeNull();
        ...
        it('scrollBy clamps to 0 when scrolling past the top', () => {
            ...
            function Wrapper() {
                ...
                if (ref.current)
                    listRef = ref.current;
                return (_jsx(VirtualizedList, { ref: ref, data: makeItems(30), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 2, containerHeight: 5, width: 40, showScrollbar: false }));
            }
            const { rerender } = render(_jsx(Wrapper, {}));
            rerender(_jsx(Wrapper, {}));
            expect(listRef).not.toBeNull();
    ...
    describe('scrollTo', () => {
        ...
        it('scrollTo negative is clamped to 0', () => {
            ...
            function Wrapper() {
                ...
                if (ref.current)
                    listRef = ref.current;
                return (_jsx(VirtualizedList, { ref: ref, data: makeItems(30), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 10, containerHeight: 5, width: 40, showScrollbar: false }));
            }
            const { rerender } = render(_jsx(Wrapper, {}));
            rerender(_jsx(Wrapper, {}));
            expect(listRef).not.toBeNull();
    ...
    describe('auto-scroll during streaming', () => {
        it('auto-scrolls when at bottom and data grows', () => {
            ...
            function Wrapper() {
                ...
                if (ref.current)
                    listRef = ref.current;
                return (_jsx(VirtualizedList, { ref: ref, data: items, renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, containerHeight: 5, width: 40, showScrollbar: false }));
            }
            const { rerender } = render(_jsx(Wrapper, {}));
            rerender(_jsx(Wrapper, {}));
            expect(listRef).not.toBeNull();
...
const ThoughtItem = ({ item, index, thoughtHeadId, onRenderIndex, }) => {
    onRenderIndex?.(index);
    return (_jsx(HistoryItemDisplay, { terminalWidth: 80, mainAreaWidth: 80, availableTerminalHeight: 40, item: item, isPending: false, thoughtHeadId: thoughtHeadId }));
};
describe('<VirtualizedList /> VP collapsed thought groups', () => {
    const settings = {
        merged: { ui: { useTerminalBuffer: true, mouseTracking: false } },
    };
    const line = (n) => `thought line ${n}`.padEnd(40, '.');
    const body = (seed, rows = 12) => Array.from({ length: rows }, (_, i) => `${seed} ${line(i)}`).join('\n');
    const HEAD_ID = 1;
    const thoughtHistory = (rows, answerRows = 0, continuations = 3) => {
        const items = [
    ...
    // actually produces.
    const renderThoughtItem = (data, onRenderIndex) => {
        const headIdMap = buildThoughtHeadIdMap(data);
        const renderItem = ({ item, index, }) => (_jsx(ThoughtItem, { item: item, index: index, thoughtHeadId: headIdMap.get(item), onRenderIndex: onRenderIndex }));
        return renderItem;
    };
    const thoughtTree = (expandedHeadIds, ref, data, initialScrollIndex, onRenderIndex, targetScrollIndex) => (_jsx(SettingsContext.Provider, { value: settings, children: _jsx(VirtualViewportContext.Provider, { value: true, children: _jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(ThoughtExpandedProvider, { value: { allExpanded: false, expandedHeadIds, toggle: () => { } }, children: _jsx(VirtualizedList, { ref: ref, data: data, renderItem: renderThoughtItem(data, onRenderIndex), estimatedItemHeight: () => 3, keyExtractor: (item) => `h-${item.id}`, isStaticItem: () => true, initialScrollIndex: initialScrollIndex, targetScrollIndex: targetScrollIndex, containerHeight: 40, showScrollbar: false }) }) }) }) }));
    ...
    it('releases reserved height when a thought group collapses', async () => {
        ...
        const harness = render(thoughtTree(new Set([HEAD_ID]), ref, data, SCROLL_TO_ITEM_END));
        await tick();
        await tick();
        expect((harness.lastFrame() ?? '').includes('c3 thought line 0')).toBe(true);
    ...
    it('heals every cached-zero continuation at the viewport top in one pass', async () => {
        ...
        const rendered = [];
        harness.rerender(thoughtTree(new Set([HEAD_ID]), ref, tallData, SCROLL_TO_ITEM_END, (i) => rendered.push(i)));
        const firstPass = [];
        for (const i of rendered) {
            if (firstPass.length > 0 && i <= firstPass[firstPass.length - 1])
                break;
            firstPass.push(i);
        }
    ...
    it('anchors to the first item of a cached-zero run so re-expand stays visible', async () => {
        ...
        // above the viewport.
        harness.rerender(thoughtTree(new Set([HEAD_ID]), ref, tallData, SCROLL_TO_ITEM_END));
        await tick();
        for (let i = 0; i < 10; i++) {
            await tick();
        }
        expect((harness.lastFrame() ?? '').includes('c1 thought line 0')).toBe(true);
    });
    it('re-expands to a non-empty healed frame after collapsing while scrolled', async () => {
        ...
        harness.rerender(thoughtTree(new Set([HEAD_ID]), ref, data, SCROLL_TO_ITEM_END));
        for (let i = 0; i < 15; i++) {
            await tick();
        }
        const lines = (harness.lastFrame() ?? '').split('\n');
    ...
    it('walks a mid-run targetScrollIndex anchor back to the first item of the run', async () => {
        ...
        harness.rerender(thoughtTree(new Set(), ref, tallData, SCROLL_TO_ITEM_END, undefined, 4));
        await tick();
        await tick();
        expect(ref.current.getScrollIndex()).toBe(2);
        // Re-expand: the healed run must stay visible at the viewport top
        // instead of being stranded above it.
        harness.rerender(thoughtTree(new Set([HEAD_ID]), ref, tallData, SCROLL_TO_ITEM_END, undefined, 4));
        for (let i = 0; i < 10; i++) {
            await tick();
        }
        expect((harness.lastFrame() ?? '').includes('c1 thought line 0')).toBe(true);
