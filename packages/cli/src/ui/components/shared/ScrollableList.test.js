...
const withKeypress = (children) => (_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: children }));
...
const flushScrollFrame = () => act(async () => {
...
const leftPress = (col, row) => `${ESC}[<0;${col};${row}M`;
const leftDrag = (col, row) => `${ESC}[<32;${col};${row}M`;
const leftRelease = (col, row) => `${ESC}[<0;${col};${row}m`;
describe('<ScrollableList /> mouse scrolling', () => {
    it('routes wheel SGR events to viewport scroll (window shifts)', async () => {
        ...
        const Wrapper = () => (_jsx(ScrollableList, { hasFocus: true, data: makeItems(50), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 0, containerHeight: 5, width: 40, showScrollbar: false }));
        const { stdin, lastFrame, rerender } = render(withKeypress(_jsx(Wrapper, {})));
        rerender(withKeypress(_jsx(Wrapper, {})));
        await act(async () => { });
    ...
    it('does not crash when hasFocus is false (mouse pipeline inactive)', () => {
        const renderItem = ({ item }) => _jsx(Text, { children: item.label });
        const Wrapper = () => (_jsx(ScrollableList, { hasFocus: false, data: makeItems(10), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, containerHeight: 5, width: 40, showScrollbar: false }));
        const { stdin, rerender } = render(withKeypress(_jsx(Wrapper, {})));
        rerender(withKeypress(_jsx(Wrapper, {})));
        expect(() => {
            stdin.write(wheelUp());
        }).not.toThrow();
    ...
    it('does not move the rendered window on content-area clicks', async () => {
        const renderItem = ({ item }) => _jsx(Text, { children: item.label });
        const Wrapper = () => (_jsx(ScrollableList, { hasFocus: true, data: makeItems(20), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, containerHeight: 5, width: 40, showScrollbar: false }));
        const { stdin, lastFrame, rerender } = render(withKeypress(_jsx(Wrapper, {})));
        rerender(withKeypress(_jsx(Wrapper, {})));
        await act(async () => { });
    ...
    it('drags the scrollbar to scroll the viewport', async () => {
        const renderItem = ({ item }) => _jsx(Text, { children: item.label });
        const Wrapper = () => (_jsx(ScrollableList, { hasFocus: true, data: makeItems(50), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 0, containerHeight: 5, width: 40, showScrollbar: true }));
        const { stdin, lastFrame, rerender } = render(withKeypress(_jsx(Wrapper, {})));
        rerender(withKeypress(_jsx(Wrapper, {})));
        await act(async () => { });
        expect(lastFrame()).toContain('item-0');
    ...
    it('does not start a scrollbar drag when content fits the viewport', async () => {
        const renderItem = ({ item }) => _jsx(Text, { children: item.label });
        const Wrapper = () => (_jsx(ScrollableList, { hasFocus: true, data: makeItems(3), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 0, containerHeight: 5, width: 40, showScrollbar: true }));
        const { stdin, lastFrame, rerender } = render(withKeypress(_jsx(Wrapper, {})));
        rerender(withKeypress(_jsx(Wrapper, {})));
        await act(async () => { });
        const before = lastFrame();
...
describe('<ScrollableList /> keyboard scroll', () => {
    ...
    it('Shift+Up scrolls up by 1 line', async () => {
        const Wrapper = () => (_jsx(ScrollableList, { hasFocus: true, data: makeItems(50), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 10, containerHeight: 5, width: 40, showScrollbar: false }));
        const { stdin, lastFrame, rerender } = render(withKeypress(_jsx(Wrapper, {})));
        rerender(withKeypress(_jsx(Wrapper, {})));
        await act(async () => { });
        expect(lastFrame()).not.toContain('item-0');
    ...
    it('Shift+Down scrolls down by 1 line', async () => {
        ...
        await act(async () => {
            for (let i = 0; i < 5; i++)
                stdin.write(SHIFT_DOWN);
        });
        await act(async () => { });
        expect(lastFrame()).not.toContain('item-0');
    });
    it('PageUp scrolls up by one page (containerHeight lines)', async () => {
        const Wrapper = () => (_jsx(ScrollableList, { hasFocus: true, data: makeItems(50), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: 20, containerHeight: 5, width: 40, showScrollbar: false }));
        const { stdin, lastFrame, rerender } = render(withKeypress(_jsx(Wrapper, {})));
        rerender(withKeypress(_jsx(Wrapper, {})));
        await act(async () => { });
        expect(lastFrame()).not.toContain('item-0');
    ...
    it('Ctrl+Home scrolls to the very top', async () => {
        const Wrapper = () => (_jsx(ScrollableList, { hasFocus: true, data: makeItems(50), renderItem: renderItem, estimatedItemHeight: estimatedItemHeight, keyExtractor: keyExtractor, initialScrollIndex: SCROLL_TO_ITEM_END, containerHeight: 5, width: 40, showScrollbar: false }));
        const { stdin, lastFrame, rerender } = render(withKeypress(_jsx(Wrapper, {})));
        rerender(withKeypress(_jsx(Wrapper, {})));
        await act(async () => { });
        expect(lastFrame()).not.toContain('item-0');
