...
describe('TextInputMouseController', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(layoutRowForEvent).mockImplementation((_node, terminalRow1Based) => terminalRow1Based - 1);
    ...
    function mountAndGetHandler(buffer = makeBuffer(), visibleLineCount = 2) {
        render(_jsx(TextInputMouseController, { linesRef: ref(linesNode), buffer: buffer, visibleLineCount: visibleLineCount }));
    ...
    it('applies a negative anchor when the frame overflows the screen', () => {
        ...
        vi.mocked(layoutRowForEvent).mockImplementation((_node, terminalRow1Based, terminalHeight) => {
...
//# sourceMappingURL=TextInputMouseController.test.js.map