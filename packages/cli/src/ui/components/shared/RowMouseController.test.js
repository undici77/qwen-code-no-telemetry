...
const mockLayoutRowForEvent = (frameHeight) => vi
...
describe('RowMouseController', () => {
    const containerNode = { tag: 'container' };
    const itemNodes = [
        { tag: 'i0' },
        { tag: 'i1' },
        { tag: 'i2' },
    ...
    function mountAndGetHandler(opts) {
        render(_jsx(RowMouseController, { containerRef: ref(containerNode), itemRefs: ref(itemNodes), scrollOffset: opts?.scrollOffset ?? 0, isDisabled: opts?.isDisabled, onHoverIndex: onHoverIndex, onSelectIndex: onSelectIndex }));
...
//# sourceMappingURL=RowMouseController.test.js.map