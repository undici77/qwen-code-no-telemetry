...
import { ApprovalMode, Storage, createDebugLogger, } from '@qwen-code/qwen-code-core';
...
export const InputPrompt = ({ buffer, onSubmit, userMessages, onClearScreen, config, slashCommands, commandContext, recentSlashCommands, placeholder, focus = true, suggestionsWidth, shellModeActive, setShellModeActive, approvalMode, onEscapePromptChange, onToggleShortcuts, showShortcuts, onSuggestionsVisibilityChange, onTabConsumerChange, vimHandleInput, isEmbeddedShellFocused, promptSuggestion, onPromptSuggestionDismiss, }) => {
    ...
    !justNavigatedHistory, recentSlashCommands);
    ...
    const prefixNode = (_jsxs(Text, { color: statusColor ?? theme.text.accent, "aria-label": statusText || undefined, children: [shellModeActive ? (reverseSearchActive ? (_jsxs(Text, { color: theme.text.link, "aria-label": SCREEN_READER_USER_PREFIX, children: ["(r:)", ' '] })) : ('!')) : commandSearchActive ? (_jsx(Text, { color: theme.text.accent, children: "(r:) " })) : showYoloStyling ? ('*') : ('>'), ' '] }));
