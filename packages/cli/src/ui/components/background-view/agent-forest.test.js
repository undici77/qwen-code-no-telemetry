...
import { computeAgentTreeInfo, computeUserBlockingIds, reorderChildrenUnderParents, treeRowPrefix, } from './agent-forest.js';
...
describe('computeUserBlockingIds', () => {
    ...
    it('tags a fully-foreground chain down from the top-level session', () => {
        ...
        expect(computeUserBlockingIds(entries)).toEqual(new Set(['fg-parent', 'fg-child']));
    });
...
describe('treeRowPrefix', () => {
    it('indents by visible depth with a marker on spawned rows', () => {
        expect(treeRowPrefix(agent('root'), { visibleDepth: 0, orphaned: false })).toBe('');
        expect(treeRowPrefix(agent('child', { parentAgentId: 'root' }), {
            visibleDepth: 1,
            orphaned: false,
        })).toBe('    ↳ ');
    });
    it('clamps the indent at TREE_INDENT_MAX_LEVELS for deep trees', () => {
        ...
        expect(treeRowPrefix(agent('deep', { parentAgentId: 'p3' }), {
            visibleDepth: 4,
            orphaned: false,
        })).toBe('            ↳ ');
    });
});
//# sourceMappingURL=agent-forest.test.js.map