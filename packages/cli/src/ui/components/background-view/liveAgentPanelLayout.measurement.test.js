 ...
 */
function ControlsMeasured({ entries, wireRosterDep, report, }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", ref: ref, children: [_jsx(Text, { children: "main" }), entries.map((e) => (_jsxs(Text, { children: [e.id, " \u00B7 running"] }, e.id)))] }));
}
async function measureGrowth(wireRosterDep) {
    let availableHeight = -1;
    const report = (v) => {
        availableHeight = v;
    };
    const { rerender, unmount } = render(_jsx(ControlsMeasured, { entries: [], wireRosterDep: wireRosterDep, report: report }));
    await new Promise((r) => setTimeout(r, 20));
    const before = availableHeight;
    // Three agents launch → the controls box is now three rows taller.
    rerender(_jsx(ControlsMeasured, { entries: [agent('a1'), agent('a2'), agent('a3')], wireRosterDep: wireRosterDep, report: report }));
    await new Promise((r) => setTimeout(r, 20));
...
//# sourceMappingURL=liveAgentPanelLayout.measurement.test.js.map