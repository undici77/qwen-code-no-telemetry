...
import { BackgroundTaskViewActionsContext, BackgroundTaskViewStateContext, } from '../../contexts/BackgroundTaskViewContext.js';
...
function renderPanel(options = { entries: [] }) {
    const state = {
        entries: options.entries,
        selectedIndex: 0,
        dialogMode: options.dialogOpen ? 'list' : 'closed',
    ...
    let result;
    act(() => {
        result = render(_jsx(ConfigContext.Provider, { value: options.config, children: _jsx(BackgroundTaskViewActionsContext.Provider, { value: actions, children: _jsx(BackgroundTaskViewStateContext.Provider, { value: state, children: _jsx(LiveAgentPanel, { width: options.width, maxRows: options.maxRows }) }) }) }));
    });
    return result;
}
/**
 ...
 */
function makeRegistryConfig(agents) {
    const store = new Map();
    for (const a of agents)
        store.set(a.agentId, a);
    const config = {
...
describe('<LiveAgentPanel />', () => {
        ...
        const { lastFrame } = renderPanel({
            entries: [
                agentEntry({
                    agentId: 'narrow-1',
                    subagentType: 'researcher',
                    description: 'researcher: scan the entire repository for occurrences of TODO and FIXME markers and triage them by area',
                    startTime: -3_000,
                }),
            ],
            width: 50,
    ...
    it.each([
        ...
        ['failed', '✖'],
        ['cancelled', '✖'],
    ])('renders the %s status with the %s glyph', (status, glyph) => {
...
//# sourceMappingURL=LiveAgentPanel.test.js.map