...
import { DEFAULT_BUILTIN_SUBAGENT_TYPE as CORE_DEFAULT_SUBAGENT_TYPE, ToolDisplayNames, ToolNames, } from '@qwen-code/qwen-code-core';
...
export const LiveAgentPanel = ({ maxRows = DEFAULT_MAX_ROWS, width, }) => {
    ...
    useEffect(() => {
        if (dialogOpen)
            return;
        const needsTick = (whenMs) => entries.some((e) => {
            if (!isAgentEntry(e))
                return false;
    ...
    if (dialogOpen)
        return null;
    return (_jsx(LiveAgentPanelBody, { snapshots: liveAgentSnapshots, now: now, maxRows: maxRows, width: width }));
};
const LiveAgentPanelBody = ({ snapshots, now, maxRows, width }) => {
    const visibleAgents = snapshots
        .map((entry) => ({
        ...entry,
        expired: entry.status !== 'running' &&
            entry.status !== 'paused' &&
    ...
    // resting state.
    const activeCount = visibleAgents.filter((e) => e.status === 'running' || e.status === 'paused').length;
    ...
    return (_jsxs(Box, { flexDirection: "column", marginTop: 1, width: width, paddingX: 2, children: [_jsxs(Box, { children: [_jsx(Text, { bold: true, color: theme.text.accent, children: "Active agents" }), _jsx(Text, { color: theme.text.secondary, children: ` (${activeCount}/${visibleAgents.length})` })] }), overflow > 0 && (_jsx(Box, { children: _jsx(Text, { color: theme.text.secondary, children: `  ^ ${overflow} more above (↓ to view all)` }) })), visible.map((entry) => (_jsx(AgentRow, { entry: entry, now: now }, entry.agentId)))] }));
};
const AgentRow = ({ entry, now, }) => {
    ...
    const safeSubagentType = showType
        ...
        : '';
    const tokenSuffix = entry.stats?.totalTokens && entry.stats.totalTokens > 0
    ...
    return (_jsxs(Box, { flexDirection: "row", children: [_jsx(Box, { flexShrink: 1, children: _jsxs(Text, { wrap: "truncate-end", children: [_jsx(Text, { color: color, children: `${glyph} ` }), showType && (_jsxs(_Fragment, { children: [_jsx(Text, { bold: true, children: safeSubagentType }), _jsx(Text, { color: theme.text.secondary, children: ': ' })] })), _jsx(Text, { color: theme.text.secondary, children: label }), activity && (_jsx(Text, { color: theme.text.secondary, children: ` (${activity})` }))] }) }), _jsx(Box, { flexShrink: 0, children: _jsx(Text, { color: theme.text.secondary, children: tail }) })] }));
};
