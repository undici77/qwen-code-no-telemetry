...
function workflowApproval(approvalId) {
    return {
        approvalId,
        subagentId: 'sub-1',
        callId: 'call-1',
...
describe('getPillLabel', () => {
    ...
    it.each(['pausing', 'paused'])('keeps an active %s workflow out of the done tally', (status) => {
...
describe('hasPendingApproval', () => {
    it('is false when no agent has a parked approval', () => {
        ...
        expect(hasPendingApproval([agentEntry({ pendingApprovals: [] })])).toBe(false);
    });
    it('is true when an agent has at least one parked approval', () => {
        expect(hasPendingApproval([agentEntry({ pendingApprovals: [approval('c1')] })])).toBe(true);
    });
    it('is true when a workflow has at least one parked approval', () => {
        expect(hasPendingApproval([
            workflowEntry({
