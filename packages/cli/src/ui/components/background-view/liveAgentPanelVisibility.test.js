...
import { getLiveAgentPanelLayoutKey, getLiveAgentPanelVpMaxRows, isLiveAgentPanelVisibleEntry, TERMINAL_VISIBLE_MS, } from './liveAgentPanelVisibility.js';
...
describe('getLiveAgentPanelLayoutKey', () => {
    ...
    it('changes when an agent is removed (panel shrinks)', () => {
        const two = getLiveAgentPanelLayoutKey([agentEntry({ id: 'a1' }), agentEntry({ id: 'a2' })], false);
        const one = getLiveAgentPanelLayoutKey([agentEntry({ id: 'a1' })], false);
        expect(one).not.toBe(two);
    });
    it('changes when an agent status flips (running -> completed)', () => {
        const running = getLiveAgentPanelLayoutKey([agentEntry({ id: 'a1', status: 'running' })], false);
        const done = getLiveAgentPanelLayoutKey([agentEntry({ id: 'a1', status: 'completed', endTime: 1 })], false);
    ...
    it('changes when panel focus toggles (adds the navigation hint row)', () => {
        ...
        expect(getLiveAgentPanelLayoutKey(entries, true)).not.toBe(getLiveAgentPanelLayoutKey(entries, false));
    });
...
describe('isLiveAgentPanelVisibleEntry (eviction window)', () => {
    ...
    it('keeps running agents visible unconditionally (no endTime)', () => {
        expect(isLiveAgentPanelVisibleEntry(agentEntry({ status: 'running' }), 1000)).toBe(true);
    });
    it('keeps paused agents visible unconditionally (no endTime)', () => {
        expect(isLiveAgentPanelVisibleEntry(agentEntry({ status: 'paused' }), 1000)).toBe(true);
    });
    ...
    it('keeps a terminal agent visible within the window, evicts after', () => {
        ...
        expect(isLiveAgentPanelVisibleEntry(entry, 1000 + TERMINAL_VISIBLE_MS)).toBe(true);
        expect(isLiveAgentPanelVisibleEntry(entry, 1000 + TERMINAL_VISIBLE_MS + 1)).toBe(false);
    });
});
...
//# sourceMappingURL=liveAgentPanelVisibility.test.js.map