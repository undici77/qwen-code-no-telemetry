...
describe('ConsentPrompt', () => {
    ...
    it('renders a string prompt with MarkdownDisplay', () => {
        ...
        render(_jsx(ConsentPrompt, { prompt: prompt, onConfirm: onConfirm, terminalWidth: terminalWidth }));
    ...
    it('passes a constrained height to MarkdownDisplay when terminal height is limited', () => {
        const prompt = 'Are you sure?';
        render(_jsx(ConsentPrompt, { prompt: prompt, onConfirm: onConfirm, terminalWidth: terminalWidth, availableTerminalHeight: 12 }));
        expect(MockedMarkdownDisplay).toHaveBeenCalledWith({
            isPending: true,
            text: prompt,
            contentWidth: terminalWidth,
    ...
    it('shows a truncation notice when the prompt is reduced to one row', () => {
        ...
        const { lastFrame } = render(_jsx(ConsentPrompt, { prompt: prompt, onConfirm: onConfirm, terminalWidth: terminalWidth, availableTerminalHeight: 8 }));
        expect(MockedMarkdownDisplay).toHaveBeenCalledWith({
            isPending: true,
            text: prompt,
            contentWidth: terminalWidth,
    ...
    it('shows a truncation notice at the two-row prompt boundary', () => {
        const prompt = 'This operation needs careful review.';
        const { lastFrame } = render(_jsx(ConsentPrompt, { prompt: prompt, onConfirm: onConfirm, terminalWidth: terminalWidth, availableTerminalHeight: 9 }));
        expect(MockedMarkdownDisplay).toHaveBeenCalledWith({
            isPending: true,
            text: prompt,
            contentWidth: terminalWidth,
    ...
    it('does not show a truncation notice at the three-row prompt boundary', () => {
        const prompt = 'This operation needs careful review.';
        const { lastFrame } = render(_jsx(ConsentPrompt, { prompt: prompt, onConfirm: onConfirm, terminalWidth: terminalWidth, availableTerminalHeight: 10 }));
        expect(MockedMarkdownDisplay).toHaveBeenCalledWith({
            isPending: true,
            text: prompt,
            contentWidth: terminalWidth,
    ...
    it('renders a ReactNode prompt directly', () => {
        ...
        const { lastFrame } = render(_jsx(ConsentPrompt, { prompt: prompt, onConfirm: onConfirm, terminalWidth: terminalWidth }));
