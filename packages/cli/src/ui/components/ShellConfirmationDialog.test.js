...
describe('ShellConfirmationDialog', () => {
    ...
    function manyCommands() {
        return Array.from({ length: 10 }, (_, i) => `cmd-${String(i + 1).padStart(2, '0')}`);
    }
    ...
    it('keeps the choices visible when many commands are shown in a small terminal', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(ShellConfirmationDialog, { request: {
                commands: manyCommands(),
                onConfirm,
            }, availableTerminalHeight: availableTerminalHeight, contentWidth: 80 }));
    ...
    it('keeps choices visible when wrapped by the constrained layout box', () => {
        const availableTerminalHeight = 10;
        const { lastFrame } = renderWithProviders(_jsx(Box, { height: availableTerminalHeight, overflow: "hidden", children: _jsx(ShellConfirmationDialog, { request: {
                    commands: manyCommands(),
                    onConfirm,
                }, availableTerminalHeight: availableTerminalHeight, contentWidth: 80 }) }));
        const frame = lastFrame() ?? '';
