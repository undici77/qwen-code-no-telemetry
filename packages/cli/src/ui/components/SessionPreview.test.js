...
function fakeResumedData(assistantParts = [
...
describe('SessionPreview', () => {
    it('shows loading state before data arrives', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(SessionPreview, { sessionService: svc, sessionId: "s1", sessionTitle: "My session", onExit: vi.fn(), onResume: vi.fn() }));
    ...
    it('renders full resumed thinking content after load', async () => {
        const thinkingText = Array.from({ length: 40 }, (_, index) => `RESUMED-THINKING-LINE-${index + 1}`).join('\n');
        const svc = mockService(fakeResumedData([
        ...
        expect(frame.indexOf('My session')).toBeLessThan(frame.indexOf('RESUMED-THINKING-LINE-1'));
    });
    it('renders footer metadata (messageCount · time · branch)', async () => {
        const svc = mockService(fakeResumedData());
        const { lastFrame } = renderWithProviders(_jsx(SessionPreview, { sessionService: svc, sessionId: "s1", sessionTitle: "My session", messageCount: 42, mtime: Date.now() - 60_000, gitBranch: "feat/preview", onExit: vi.fn(), onResume: vi.fn() }));
        await wait(100);
        const frame = lastFrame() ?? '';
    ...
    it('calls onExit when Escape is pressed', async () => {
        const onExit = vi.fn();
        const svc = mockService(fakeResumedData());
        const { stdin } = renderWithProviders(_jsx(SessionPreview, { sessionService: svc, sessionId: "s1", sessionTitle: "My session", onExit: onExit, onResume: vi.fn() }));
        await wait(100);
        stdin.write('\u001B'); // ESC
        await wait(50);
    ...
    it('calls onResume(sessionId) when Enter is pressed', async () => {
        ...
        const svc = mockService(fakeResumedData());
        const { stdin } = renderWithProviders(_jsx(SessionPreview, { sessionService: svc, sessionId: "s1", sessionTitle: "My session", onExit: vi.fn(), onResume: onResume }));
        await wait(100);
        stdin.write('\r'); // Enter
        await wait(50);
