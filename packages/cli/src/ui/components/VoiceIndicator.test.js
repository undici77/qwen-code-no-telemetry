...
describe('<VoiceIndicator />', () => {
    it('sanitizes streaming interim transcript text', () => {
        const { lastFrame } = render(_jsx(VoiceIndicator, { status: "recording", interimText: 'partial\x1b[8m hidden\x1b[0m' }));
    ...
    it('renders the meter when audioLevel is NaN', () => {
        const { lastFrame } = render(_jsx(VoiceIndicator, { status: "recording", audioLevel: Number.NaN }));
        expect(lastFrame()).toContain('░'.repeat(16));
    });
});
//# sourceMappingURL=VoiceIndicator.test.js.map