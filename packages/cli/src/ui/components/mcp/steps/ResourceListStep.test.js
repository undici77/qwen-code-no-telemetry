...
describe('ResourceListStep', () => {
    ...
    it('lists resource URIs and a friendly name when it differs from the URI', () => {
        const { lastFrame } = render(_jsx(ResourceListStep, { resources: [
                resource('file:///a.md', { title: 'Spec A' }),
                resource('file:///b.md'),
            ], serverName: "server", onSelect: vi.fn(), onBack: vi.fn() }));
    ...
    it('navigates with Ctrl+N/P and selects the highlighted resource', () => {
        ...
        const { lastFrame } = render(_jsx(ResourceListStep, { resources: [
            ...
            ], serverName: "server", onSelect: onSelect, onBack: vi.fn() }));
        expect(lastFrame()).toContain('❯ file:///first.md');
        pressKey({ name: 'n', sequence: '', ctrl: true });
        expect(lastFrame()).toContain('❯ file:///second.md');
        pressKey({ name: 'return' });
        expect(onSelect).toHaveBeenCalledWith(expect.objectContaining({ uri: 'file:///second.md' }));
    });
    it('calls onBack when Escape is pressed', () => {
        const onBack = vi.fn();
        render(_jsx(ResourceListStep, { resources: [resource('file:///a.md')], serverName: "server", onSelect: vi.fn(), onBack: onBack }));
        pressKey({ name: 'escape' });
        expect(onBack).toHaveBeenCalledTimes(1);
    });
    it('shows an empty-state message when there are no resources', () => {
        const { lastFrame } = render(_jsx(ResourceListStep, { resources: [], serverName: "server", onSelect: vi.fn(), onBack: vi.fn() }));
    ...
    it('windows a long list with correct count + arrow indicators', () => {
        const resources = Array.from({ length: 15 }, (_, i) => resource(`file:///r${String(i).padStart(2, '0')}.md`));
        const { lastFrame } = render(_jsx(ResourceListStep, { resources: resources, serverName: "server", onSelect: vi.fn(), onBack: vi.fn() }));
...
//# sourceMappingURL=ResourceListStep.test.js.map