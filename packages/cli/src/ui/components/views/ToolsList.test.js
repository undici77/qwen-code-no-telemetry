...
describe('<ToolsList />', () => {
    it('renders correctly with descriptions', () => {
        const { lastFrame } = render(_jsx(ToolsList, { tools: mockTools, showDescriptions: true, contentWidth: 40 }));
        expect(lastFrame()).toMatchSnapshot();
    });
    it('renders correctly without descriptions', () => {
        const { lastFrame } = render(_jsx(ToolsList, { tools: mockTools, showDescriptions: false, contentWidth: 40 }));
        expect(lastFrame()).toMatchSnapshot();
    });
    it('renders correctly with no tools', () => {
        const { lastFrame } = render(_jsx(ToolsList, { tools: [], showDescriptions: true, contentWidth: 40 }));
        expect(lastFrame()).toMatchSnapshot();
    });
});
