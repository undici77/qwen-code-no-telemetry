...
function makeBreakdown(currentTier, overrides = {}) {
    return {
        systemPrompt: 5000,
        builtinTools: 8000,
        mcpTools: 0,
...
describe('ContextUsage — CompactionThresholds section (review #4168 R1.6)', () => {
    it('keeps a positive estimated count in the numeric usage view', () => {
        const { lastFrame } = render(_jsx(ContextUsage, { modelName: "qwen3-coder", totalTokens: 50_000, contextWindowSize: 128_000, breakdown: makeBreakdown('safe'), builtinTools: [], mcpTools: [], memoryFiles: [], skills: [], isEstimated: true }));
    ...
    it('renders the new three-tier section with all four threshold rows', () => {
        const { lastFrame } = render(_jsx(ContextUsage, { modelName: "qwen3-coder", totalTokens: 0, contextWindowSize: 128_000, breakdown: makeBreakdown('safe'), builtinTools: [], mcpTools: [], memoryFiles: [], skills: [], isEstimated: true }));
        const frame = lastFrame() ?? '';
    ...
    it('shows safe tier without any ▶ marker', () => {
        const { lastFrame } = render(_jsx(ContextUsage, { modelName: "qwen3-coder", totalTokens: 50_000, contextWindowSize: 128_000, breakdown: makeBreakdown('safe'), builtinTools: [], mcpTools: [], memoryFiles: [], skills: [] }));
        const frame = lastFrame() ?? '';
    ...
    it('places ▶ on the warn row when currentTier === warn', () => {
        const { lastFrame } = render(_jsx(ContextUsage, { modelName: "qwen3-coder", totalTokens: 80_000, contextWindowSize: 128_000, breakdown: makeBreakdown('warn'), builtinTools: [], mcpTools: [], memoryFiles: [], skills: [] }));
        const frame = lastFrame() ?? '';
    ...
    it('places ▶ on the hard row when currentTier === hard', () => {
        const { lastFrame } = render(_jsx(ContextUsage, { modelName: "qwen3-coder", totalTokens: 106_000, contextWindowSize: 128_000, breakdown: makeBreakdown('hard'), builtinTools: [], mcpTools: [], memoryFiles: [], skills: [] }));
        const frame = lastFrame() ?? '';
        const lines = frame.split('\n');
        const hardLine = lines.find((l) => l.includes('Hard threshold')) ?? '';
...
//# sourceMappingURL=ContextUsage.test.js.map