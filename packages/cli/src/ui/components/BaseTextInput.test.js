...
import { BaseTextInput, defaultRenderLine, getAbsolutePosition, getPhysicalCursorPosition, } from './BaseTextInput.js';
import { useKeypress } from '../hooks/useKeypress.js';
import { renderSoftwareCursor } from '../utils/software-cursor.js';
const mockSetCursorPosition = vi.hoisted(() => vi.fn());
const mockUseBoxMetrics = vi.hoisted(() => vi.fn(() => ({
    width: 0,
    height: 0,
    top: 0,
    left: 0,
...
function createElement(top, left, parentNode) {
    return {
        yogaNode: {
...
describe('BaseTextInput', () => {
    ...
    it('hides the physical cursor when showCursor is false', () => {
        const buffer = createBuffer();
        render(_jsx(BaseTextInput, { buffer: buffer, onSubmit: vi.fn(), showCursor: false }));
        expect(mockSetCursorPosition).toHaveBeenCalledWith(undefined);
    });
    it('leaves cursor cleanup to the useCursor hook', () => {
        const buffer = createBuffer();
        const { unmount } = render(_jsx(BaseTextInput, { buffer: buffer, onSubmit: vi.fn() }));
    ...
    it('positions the physical cursor from absolute Ink DOM position', () => {
        ...
        expect(getPhysicalCursorPosition(child, {
            hasMeasured: true,
            showCursor: true,
            cursorVisualRow: 2,
            cursorVisualCol: 3,
...
describe('defaultRenderLine', () => {
    it('renders the software cursor on the current character', () => {
        const { lastFrame } = render(_jsx(_Fragment, { children: defaultRenderLine({
                lineText: 'hello',
                isOnCursorLine: true,
                cursorCol: 2,
                showCursor: true,
