...
describe('CronPill', () => {
    it('renders nothing when there are no scheduled tasks', () => {
        const { lastFrame, unmount } = renderWithProviders(_jsx(CronPill, { count: 0 }));
        expect(lastFrame()).toBe('');
        unmount();
    });
    it('renders the active scheduled task count', () => {
        const { lastFrame, unmount } = renderWithProviders(_jsx(CronPill, { count: 2 }));
        expect(lastFrame()).toContain('◎\uFE0E 2 scheduled tasks');
        unmount();
    });
});
//# sourceMappingURL=CronPill.test.js.map