...
describe('<LoadingIndicator />', () => {
    ...
    it('should display whole elapsed seconds below one minute', () => {
        const half = renderWithContext(_jsx(LoadingIndicator, { currentLoadingPhrase: "Working...", elapsedTime: 17.5 }), StreamingState.Responding);
        expect(half.lastFrame()).toContain('(17s · esc to cancel)');
        const whole = renderWithContext(_jsx(LoadingIndicator, { currentLoadingPhrase: "Working...", elapsedTime: 18 }), StreamingState.Responding);
        expect(whole.lastFrame()).toContain('(18s · esc to cancel)');
        // Timer start / reset publishes exactly 0.
        const zero = renderWithContext(_jsx(LoadingIndicator, { currentLoadingPhrase: "Working...", elapsedTime: 0 }), StreamingState.Responding);
    ...
    it('should truncate long primary text instead of wrapping', () => {
        const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, currentLoadingPhrase: 'This is an extremely long loading phrase that should be truncated in the UI to keep the primary line concise.' }), StreamingState.Responding, 80);
    ...
    describe('token display', () => {
        it('should display output tokens inline with arrow notation', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 847 }), StreamingState.Responding);
            const output = lastFrame();
        ...
        it('should not display tokens when output tokens is 0', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 0 }), StreamingState.Responding);
            const output = lastFrame();
            expect(output).not.toContain('↓');
        ...
        it('should hide tokens in narrow terminal', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 500 }), StreamingState.Responding, 79);
            const output = lastFrame();
            expect(output).not.toContain('↓');
            expect(output).not.toContain('tokens');
            expect(output).toContain('esc to cancel');
        ...
        it('should show tokens in wide terminal with inline format', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 5400 }), StreamingState.Responding, 80);
            const output = lastFrame();
            expect(output).toContain('↓ 5.4k tokens');
        });
        it('should format tokens inline with time and cancel', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 5400 }), StreamingState.Responding, 120);
            const output = lastFrame();
            expect(output).toContain('(5s · ↓ 5.4k tokens · esc to cancel)');
        });
        it('should not show response tokens/sec by default', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 500 }), StreamingState.Responding, 120);
            const output = lastFrame();
            expect(output).toContain('↓ 500 tokens');
            expect(output).not.toContain('t/s');
        });
        it('should show response tokens/sec when enabled', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 500, showResponseTokensPerSecond: true }), StreamingState.Responding, 120);
            const output = lastFrame();
            expect(output).toContain('↓ 500 tokens');
        ...
        it('should calculate response tokens/sec from tokens produced after the timer reset', () => {
            ...
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 550, taskStartTokens: 500, taskStartStreamingChars: 200, streamingCharsRef: streamingCharsRef, isStreaming: true, showResponseTokensPerSecond: true }), StreamingState.Responding, 120);
            const output = lastFrame();
        ...
        it('should not count excluded tool tokens toward response tokens/sec', () => {
            const streamingCharsRef = { current: 400 };
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 8000, taskStartTokens: 8000, streamingCharsRef: streamingCharsRef, isStreaming: true, showResponseTokensPerSecond: true }), StreamingState.Responding, 120);
            const output = lastFrame();
        ...
        it('should format sub-10 response tokens/sec with one decimal place', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { currentLoadingPhrase: "Working...", elapsedTime: 8, candidatesTokens: 25, showResponseTokensPerSecond: true }), StreamingState.Responding, 120);
            const output = lastFrame();
            expect(output).toContain('3.1 t/s');
        });
        it('should not show response tokens/sec before content arrives', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 500, showResponseTokensPerSecond: true, isReceivingContent: false }), StreamingState.Responding, 120);
            const output = lastFrame();
            expect(output).toContain('↑ 500 tokens');
            expect(output).not.toContain('t/s');
        });
        it('should show ↑ arrow when waiting for API response', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 500, isReceivingContent: false }), StreamingState.Responding);
            const output = lastFrame();
            expect(output).toContain('↑ 500 tokens');
            expect(output).not.toContain('↓');
        });
        it('should show ↓ arrow when receiving content (default)', () => {
            const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, candidatesTokens: 500 }), StreamingState.Responding);
            const output = lastFrame();
            expect(output).toContain('↓ 500 tokens');
            expect(output).not.toContain('↑');
        });
