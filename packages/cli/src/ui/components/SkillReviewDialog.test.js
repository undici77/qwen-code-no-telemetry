...
describe('SkillReviewDialog', () => {
    ...
    beforeEach(async () => {
        ...
        mockedUseConfig.mockReturnValue({ setAutoSkillEnabled });
    ...
    function renderDialog(skillsArg, overrides = {}) {
        return render(_jsx(SkillReviewDialog, { skills: skillsArg, onAccept: vi.fn(), onReject: vi.fn(), onClose: vi.fn(), onDismiss: vi.fn(), ...overrides }));
    }
    /** Write a single staged skill and render a one-skill dialog over it. */
    async function renderPreviewSkill(dirName, content) {
    ...
    it('does not overflow a width-constrained container (right border stays visible)', () => {
        const { lastFrame } = render(_jsx(Box, { width: 60, flexDirection: "column", overflow: "hidden", children: _jsx(SkillReviewDialog, { skills: skills, onAccept: vi.fn(), onReject: vi.fn(), onClose: vi.fn(), onDismiss: vi.fn() }) }));
    ...
    it('shows a fallback when the staged file cannot be read', async () => {
        ...
        await vi.waitFor(() => expect(lastFrame()).toContain('Preview unavailable'));
    });
    it('shows loading, not the previous skill body, while the next preview loads', async () => {
        ...
        const openSpy = vi.spyOn(fs, 'open').mockImplementation((async (...args) => {
            if (String(args[0]).includes('auto-skill-beta'))
                await gate;
    ...
    it('escapes bare control bytes (e.g. BEL) that are not ANSI sequences', async () => {
        ...
        const { lastFrame } = await renderPreviewSkill('auto-skill-bel', `---\nname: x\n---\nBODY_${String.fromCharCode(7)}END\n`);
    ...
    it('sanitizes the model-generated name and description in the header', async () => {
        ...
        const stagedManifestPath = await writeSkill('auto-skill-header', '---\nname: x\n---\nHEADER_BODY\n');
        const { lastFrame } = renderDialog([
            {
                name: 'evil-\u001b[2Jname',
    ...
    it('still escapes a lone CR that is not part of a CRLF pair', async () => {
        ...
        const { lastFrame } = await renderPreviewSkill('auto-skill-cr', '---\nname: x\n---\nLONE_\rMID\n');
    ...
    it('reads only a bounded chunk of a huge preview file (no unbounded read)', async () => {
        ...
        const openSpy = vi.spyOn(fs, 'open').mockImplementation((async (...args) => {
            ...
            vi.spyOn(handle, 'read').mockImplementation(((...rargs) => {
    ...
    it('flags a byte-truncated preview even when no lines are hidden', async () => {
        ...
        const { lastFrame } = await renderPreviewSkill('auto-skill-trunc', `---\nname: trunc\n---\n${content}`);
    ...
    it('refreshes the preview with the saved edits after the editor closes', async () => {
        ...
        launchEditor.mockImplementationOnce(async (p) => {
            await fs.writeFile(p, '---\nname: auto-skill-alpha\n---\nEDITED_BODY_MARKER after save.\n');
        });
        const { lastFrame } = renderDialog(skills);
        await vi.waitFor(() => expect(lastFrame()).toContain('ALPHA_BODY_MARKER'));
        pressKey('o');
        await vi.waitFor(() => expect(launchEditor).toHaveBeenCalledWith(editedPath));
    ...
    it('auto-refreshes the preview when the staged file changes on disk', async () => {
        ...
        const { lastFrame } = renderDialog(skills);
        await vi.waitFor(() => expect(lastFrame()).toContain('ALPHA_BODY_MARKER'));
        await fs.writeFile(skills[0].stagedManifestPath, '---\nname: auto-skill-alpha\n---\nWATCHED_BODY_MARKER saved later.\n');
        await vi.waitFor(() => expect(lastFrame()).toContain('WATCHED_BODY_MARKER'), { timeout: 5000 });
        expect(lastFrame()).not.toContain('ALPHA_BODY_MARKER');
    });
    ...
    it('keeps the dialog open and the feature untouched when persisting turn-off fails', async () => {
        ...
        await vi.waitFor(() => expect(lastFrame()).toContain('Failed to save setting'));
...
//# sourceMappingURL=SkillReviewDialog.test.js.map