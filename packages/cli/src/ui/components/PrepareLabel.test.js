...
describe('PrepareLabel', () => {
    const color = 'white';
    const flat = (s) => stripAnsi(s ?? '').replace(/\n/g, '');
    it('renders plain label when no match (short label)', () => {
        const { lastFrame } = render(_jsx(PrepareLabel, { label: "simple command", userInput: "", matchedIndex: undefined, textColor: color, isExpanded: false }));
        expect(lastFrame()).toMatchSnapshot();
    });
    it('truncates long label when collapsed and no match', () => {
        const long = 'x'.repeat(MAX_WIDTH + 25);
        const { lastFrame } = render(_jsx(PrepareLabel, { label: long, userInput: "", textColor: color, isExpanded: false }));
        const out = lastFrame();
        const f = flat(out);
    ...
    it('shows full long label when expanded and no match', () => {
        ...
        const { lastFrame } = render(_jsx(PrepareLabel, { label: long, userInput: "", textColor: color, isExpanded: true }));
        const out = lastFrame();
        const f = flat(out);
