...
describe('<Header />', () => {
    ...
    it('shows a non-semver fallback like "unknown" without a bogus "v" prefix', () => {
        const { lastFrame } = render(_jsx(Header, { ...defaultProps, version: "unknown" }));
    ...
    it('displays Coding Plan auth type', () => {
        const { lastFrame } = render(_jsx(Header, { ...defaultProps, authDisplayType: AuthDisplayType.CODING_PLAN }));
        expect(lastFrame()).toContain('Coding Plan');
    });
    it('displays API Key auth type', () => {
        const { lastFrame } = render(_jsx(Header, { ...defaultProps, authDisplayType: AuthDisplayType.API_KEY }));
        expect(lastFrame()).toContain('API Key');
    });
    it('displays custom provider auth labels as-is', () => {
        const { lastFrame } = render(_jsx(Header, { ...defaultProps, authDisplayType: "OpenRouter" }));
    ...
    it('renders the custom subtitle in place of the blank spacer row', () => {
        const { lastFrame } = render(_jsx(Header, { ...defaultProps, customBannerSubtitle: "Built-in DataWorks Official Skills" }));
        const frame = lastFrame() ?? '';
    ...
    it('renders the custom banner title in place of the default brand', () => {
        const { lastFrame } = render(_jsx(Header, { ...defaultProps, customBannerTitle: "Acme CLI" }));
    ...
    it('renders the custom large tier when it fits', () => {
        const { lastFrame } = render(_jsx(Header, { ...defaultProps, customAsciiArt: { small: 'SMALL', large: 'LARGE_LOGO' } }));
    ...
    it('falls back to the small tier when the large one does not fit', () => {
        ...
        const { lastFrame } = render(_jsx(Header, { ...defaultProps, customAsciiArt: {
                small: 'sm',
                large: 'X'.repeat(60),
            } }));
    ...
    it('hides the logo column when neither custom tier fits — does NOT fall back to the default Qwen logo (preserves white-label intent)', () => {
        const { lastFrame } = render(_jsx(Header, { ...defaultProps, customAsciiArt: { small: 'X'.repeat(150), large: 'Y'.repeat(150) } }));
        expect(lastFrame()).not.toContain('██╔═══██╗');
