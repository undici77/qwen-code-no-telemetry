...
describe('<AnsiOutputText />', () => {
    it('renders a simple AnsiOutput object correctly', () => {
        ...
        const { lastFrame } = render(_jsx(AnsiOutputText, { data: data, maxWidth: 80 }));
    ...
    it('copies styled rows with hard line boundaries', () => {
        ...
        const { stdout } = render(_jsx(AnsiOutputText, { data: data, maxWidth: 80 }));
    ...
    it('respects the availableTerminalHeight prop and slices the lines correctly', () => {
        ...
        const { lastFrame } = render(_jsx(AnsiOutputText, { data: data, availableTerminalHeight: 2, maxWidth: 80 }));
        const output = lastFrame();
    ...
    it('renders a large AnsiOutput object without crashing', () => {
        ...
        const { lastFrame } = render(_jsx(AnsiOutputText, { data: largeData, maxWidth: 80 }));
    ...
    it('truncates wide lines to fit within maxWidth', () => {
        ...
        const data = [[createAnsiToken({ text: wideText })]];
        const { lastFrame } = render(_jsx(AnsiOutputText, { data: data, maxWidth: 20 }));
        const output = lastFrame();
    ...
    it('truncates multi-token wide lines (styled-column output) to maxWidth', () => {
        ...
        const maxWidth = 30;
        const { lastFrame } = render(_jsx(AnsiOutputText, { data: data, maxWidth: maxWidth }));
        const output = lastFrame();
