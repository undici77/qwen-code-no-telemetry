...
import { InputPrompt, classifyPastedImagePaths, expandPendingPastePlaceholders, } from './InputPrompt.js';
...
describe('InputPrompt', () => {
    ...
    const mockedUseBackgroundTaskViewState = vi.mocked(useBackgroundTaskViewState);
    const mockedUseBackgroundTaskViewActions = vi.mocked(useBackgroundTaskViewActions);
    ...
    it('stashes non-empty input on Ctrl+S', async () => {
        ...
        await waitFor(() => {
            expect(mockedSavePromptStash).toHaveBeenCalledWith(path.join('test', 'project', 'src'), 'draft prompt');
        });
    ...
    it('clears the stash when the prompt is submitted', async () => {
        ...
        await waitFor(() => {
            expect(mockedClearPromptStash).toHaveBeenCalledWith(path.join('test', 'project', 'src'));
    ...
    it('captures explicit provenance before clearing the input buffer', async () => {
        ...
        expect(vi.mocked(props.buffer.setText).mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(props.onSubmit).mock.invocationCallOrder[0]);
        unmount();
    });
    it('clears restored provenance before applying same-text history', async () => {
        ...
        expect(invalidateSubmittedPromptProvenance.mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(props.buffer.setText).mock.invocationCallOrder[0]);
        unmount();
    });
    ...
    it('expands large paste placeholders before stashing', () => {
        ...
        expect(expandPendingPastePlaceholders('before [Pasted Content 1200 chars] after', pending)).toBe('before full pasted content after');
    });
    ...
    it('passes a voice refinement callback when a fast model is configured', () => {
        ...
        const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
        expect(mockedUseVoiceInput).toHaveBeenCalledWith(expect.objectContaining({ refine: expect.any(Function) }));
        unmount();
    });
    it('omits voice refinement when refineTranscript is disabled', () => {
        ...
        expect(mockedUseVoiceInput).toHaveBeenCalledWith(expect.objectContaining({ refine: undefined }));
        unmount();
    });
    ...
    describe('prompt suggestions', () => {
        ...
        describe('clears promptSuggestion on accept/submit (no ghost placeholder)', () => {
            ...
            it('calls onPromptSuggestionDismiss when Tab accepts the suggestion', async () => {
                ...
                const { stdin, unmount } = renderWithProviders(_jsx(InputPrompt, { ...props, promptSuggestion: "commit this", onPromptSuggestionDismiss: onPromptSuggestionDismiss }));
                try {
                    await advanceTimers(BEFORE_SUGGESTION_VISIBLE_MS);
                    act(() => {
                        stdin.write('\t');
    ...
    describe('onTabConsumerChange reporting (issue #4171)', () => {
        ...
        it('reports true while the followup prompt suggestion is visible', async () => {
            ...
            const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props, promptSuggestion: "commit this", onTabConsumerChange: onTabConsumerChange }));
        ...
        it('reports true while mid-input ghost text offers an accept', async () => {
            ...
            const onTabConsumerChange = vi.fn();
            const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props, onTabConsumerChange: onTabConsumerChange }));
            await wait();
            expect(onTabConsumerChange).toHaveBeenCalledWith(true);
            unmount();
        });
        ...
        it('reports false again after the autocomplete dropdown is dismissed', async () => {
            ...
            const onTabConsumerChange = vi.fn();
            const { rerender, unmount } = renderWithProviders(_jsx(InputPrompt, { ...props, onTabConsumerChange: onTabConsumerChange }));
            await wait();
            ...
            mockedUseCommandCompletion.mockReturnValue(mockCommandCompletion);
            rerender(_jsx(InputPrompt, { ...props, onTabConsumerChange: onTabConsumerChange }));
            await wait();
    ...
    describe('onSuggestionsVisibilityChange stays narrow (autocomplete only)', () => {
        ...
        it('stays false when only a followup prompt suggestion is visible', async () => {
            ...
            const onTabConsumerChange = vi.fn();
            const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props, promptSuggestion: "commit this", onSuggestionsVisibilityChange: onSuggestionsVisibilityChange, onTabConsumerChange: onTabConsumerChange }));
            await wait(SUGGESTION_VISIBLE_WAIT_MS);
        ...
        it('stays false when only mid-input ghost text is present', async () => {
            ...
            const onSuggestionsVisibilityChange = vi.fn();
            const onTabConsumerChange = vi.fn();
            const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props, onSuggestionsVisibilityChange: onSuggestionsVisibilityChange, onTabConsumerChange: onTabConsumerChange }));
            await wait();
            expect(onTabConsumerChange).toHaveBeenCalledWith(true);
            expect(onSuggestionsVisibilityChange).not.toHaveBeenCalledWith(true);
            unmount();
        ...
        it('flips true only when the autocomplete dropdown is visible', async () => {
            ...
            const onSuggestionsVisibilityChange = vi.fn();
            const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props, onSuggestionsVisibilityChange: onSuggestionsVisibilityChange }));
            await wait();
    ...
    describe('clipboard image paste', () => {
        ...
        describeConditional('should handle Ctrl+V when clipboard has an image', async () => {
            ...
            vi.mocked(clipboardUtils.saveClipboardImage).mockResolvedValue('/Users/mochi/.qwen/tmp/clipboard-123.png');
            const { stdin, unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
            await wait();
            // Send Ctrl+V
        ...
        it('should handle Cmd+V when clipboard has an image', async () => {
            vi.mocked(clipboardUtils.clipboardHasImage).mockResolvedValue(true);
            vi.mocked(clipboardUtils.saveClipboardImage).mockResolvedValue('/Users/mochi/.qwen/tmp/clipboard-456.png');
            const { stdin, unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
            await wait();
        ...
        it('keeps generated attachment references out of submitted provenance', async () => {
            const imagePath = path.join('test', 'project', 'src', '.qwen', 'tmp', 'clipboard.png');
            vi.mocked(clipboardUtils.clipboardHasImage).mockResolvedValue(true);
            ...
            stdin.write('\r');
            await waitFor(() => {
                expect(props.onSubmit).toHaveBeenCalledWith(`@${path.join('.qwen', 'tmp', 'clipboard.png')}\n\ndescribe this image`, {
                    deferUntilIdle: false,
        ...
        it('should show the native clipboard error only once across remounts', async () => {
            ...
            vi.mocked(clipboardUtils.clipboardHasImage).mockImplementation(async (onUnavailable) => {
                onUnavailable?.();
                return false;
            });
            const first = renderWithProviders(_jsx(InputPrompt, { ...props, clipboardUnavailableShownRef: clipboardUnavailableShownRef }));
            await wait();
            ...
            await wait();
            first.unmount();
            const second = renderWithProviders(_jsx(InputPrompt, { ...props, clipboardUnavailableShownRef: clipboardUnavailableShownRef }));
            await wait();
    ...
    it('should hide category tabs and keep bare arrows for attachments', async () => {
        const isWindows = process.platform === 'win32';
        vi.mocked(clipboardUtils.clipboardHasImage).mockResolvedValue(true);
        vi.mocked(clipboardUtils.saveClipboardImage).mockResolvedValue(path.join('test', 'project', '.qwen', 'tmp', 'clipboard.png'));
        vi.mocked(clipboardUtils.cleanupOldClipboardImages).mockResolvedValue(undefined);
        const switchCategory = vi.fn();
        mockedUseCommandCompletion.mockReturnValue({
            ...mockCommandCompletion,
        ...
        props.buffer.setText('@');
        const { stdin, lastFrame, unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
        await wait();
        stdin.write(isWindows ? '\x1Bv' : '\x16');
        await wait();
        stdin.write('\x1b[A');
    ...
    describe('Highlighting and Cursor Display', () => {
        ...
        it('should display cursor on a blank line in a multiline block', async () => {
            ...
            expect(lines.find((l) => l.includes(renderSoftwareCursor(' ')))).not.toBeUndefined();
            unmount();
        });
    });
    ...
    describe('reverse search', () => {
        ...
        it('completes the highlighted entry on Tab and exits reverse-search', async () => {
            ...
            expect(invalidateSubmittedPromptProvenance).toHaveBeenCalledOnce();
            expect(invalidateSubmittedPromptProvenance.mock.invocationCallOrder[0]).toBeLessThan(mockHandleAutocomplete.mock.invocationCallOrder[0]);
        ...
        it('submits the highlighted entry on Enter and exits reverse-search', async () => {
            ...
            expect(invalidateSubmittedPromptProvenance).toHaveBeenCalledOnce();
            expect(invalidateSubmittedPromptProvenance.mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(props.onSubmit).mock.invocationCallOrder[0]);
            unmount();
        });
        ...
        it('invalidates provenance before submitting a command-history match', async () => {
            ...
            vi.mocked(useReverseSearchCompletion).mockImplementation((_buffer, _data, isActive) => ({
                ...
                suggestions: isActive
                    ? [
                        {
                            label: '<system-reminder>generated</system-reminder>\nuser text',
                            value: '<system-reminder>generated</system-reminder>\nuser text',
                        },
                    ]
                    : [],
                showSuggestions: !!isActive,
            ...
            stdin.write('\r');
            await waitFor(() => {
                expect(props.onSubmit).toHaveBeenCalledWith('<system-reminder>generated</system-reminder>\nuser text', {
                    deferUntilIdle: false,
                    submittedPrompt: '<system-reminder>generated</system-reminder>\nuser text',
                });
            });
            expect(invalidateSubmittedPromptProvenance).toHaveBeenCalledOnce();
            expect(invalidateSubmittedPromptProvenance.mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(props.onSubmit).mock.invocationCallOrder[0]);
        ...
        it('should expand same-size placeholders correctly when #2 appears first', async () => {
            ...
            try {
                ...
                expect(props.onSubmit).toHaveBeenCalledWith(`${secondPaste}\n${firstPaste}`, {
                    deferUntilIdle: false,
                    submittedPrompt: '[Pasted Content 1001 chars] #2\n[Pasted Content 1001 chars]',
                });
            }
            finally {
                vi.useRealTimers();
    ...
    describe('two-step edge transition for history navigation', () => {
        ...
        it('arrow Down at end-of-line walks history', async () => {
            mockInputHistory.navigateDown.mockReturnValue(true);
            mockBuffer.setText('current draft');
        ...
        it('suppresses completion menu navigation for history-restored text until edited', async () => {
            ...
            const { unmount: unmountEdited } = renderWithProviders(_jsx(InputPrompt, { ...props }));
            await wait();
...
describe('classifyPastedImagePaths', () => {
    it('treats a lone @-prefixed image path (terminal Cmd+V injection) as all-image', () => {
        const result = classifyPastedImagePaths('@/var/folders/12/T/clipboard-2026-06-24-124142-18EC6DC9.png');
    ...
    it('splits multiple space- and newline-separated image paths', () => {
        const result = classifyPastedImagePaths('/a/one.png /b/two.jpg\n/c/three.webp');
        expect(result.allImages).toBe(true);
        expect(result.imagePaths).toEqual([
            '/a/one.png',
            '/b/two.jpg',
    ...
    it('does not treat plain text or non-image paths as image paste', () => {
        ...
        expect(classifyPastedImagePaths('@/a/img.png describe this').allImages).toBe(false);
    });
});
