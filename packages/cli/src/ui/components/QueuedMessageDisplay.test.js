...
describe('QueuedMessageDisplay', () => {
    ...
    it('shows edit hint when queue has messages', () => {
        const { lastFrame } = render(_jsx(QueuedMessageDisplay, { messageQueue: ['Some message'] }));
        const output = lastFrame();
    ...
    it('hides edit hint after showing it enough times', () => {
        ...
        const { lastFrame, rerender } = render(_jsx(QueuedMessageDisplay, { messageQueue: ['msg'] }));
        expect(lastFrame()).toContain('to edit queued messages'); // 1st
        rerender(_jsx(QueuedMessageDisplay, { messageQueue: [] }));
        rerender(_jsx(QueuedMessageDisplay, { messageQueue: ['msg'] }));
