...
import { SuggestionsDisplay, normalizeDescription, } from './SuggestionsDisplay.js';
...
describe('SuggestionsDisplay', () => {
    ...
    it('renders localized loading text in zh', async () => {
        ...
        const { lastFrame } = render(_jsx(SuggestionsDisplay, { suggestions: [], activeIndex: 0, isLoading: true, width: 80, scrollOffset: 0, userInput: "/", mode: "slash" }));
        expect(lastFrame()).toContain('正在加载建议...');
    });
    it('truncates long slash command descriptions to a single line by default', () => {
        const description = 'This long command description should be truncated to a single line so it cannot fill the entire terminal window.';
        const { lastFrame } = render(_jsx(SuggestionsDisplay, { suggestions: [
                {
                    label: 'review',
                    value: 'review',
                    description,
                },
            ], activeIndex: 0, isLoading: false, width: 40, scrollOffset: 0, userInput: "/re", mode: "slash" }));
    ...
    it('keeps the full MCP resource reference on one line and truncates its description (reverse mode)', () => {
        ...
        const { lastFrame } = render(_jsx(SuggestionsDisplay, { suggestions: [
                {
                    label: a,
                    value: a,
                    description: 'Analyze PPU bubble (idle time) from a loaded trace report.',
                },
                {
                    label: b,
                    value: b,
                    description: 'Analyze PPU operator performance from a loaded trace report.',
                },
            ], activeIndex: 0, isLoading: false, width: 80, scrollOffset: 0, userInput: "asys-mcp-http:asight", mode: "reverse" }));
        const output = lastFrame() ?? '';
        ...
        expect(output.split('\n').filter((l) => l.includes('asys-mcp-http:'))).toHaveLength(2);
    });
    it('collapses newlines in multi-line descriptions so a row stays one line', () => {
        ...
        const { lastFrame } = render(_jsx(SuggestionsDisplay, { suggestions: [{ label: 'skill', value: 'skill', description }], activeIndex: 0, isLoading: false, width: 120, scrollOffset: 0, userInput: "/sk", mode: "slash" }));
        const output = lastFrame() ?? '';
    ...
    it('renders a visible marker for the active suggestion', () => {
        const { lastFrame } = render(_jsx(SuggestionsDisplay, { suggestions: [
            ...
            ], activeIndex: 1, isLoading: false, width: 80, scrollOffset: 0, userInput: "/pr", mode: "slash" }));
        ...
        const { lastFrame } = render(_jsx(SuggestionsDisplay, { suggestions: [
                {
                    label: 'review',
                    value: 'review',
                    argumentHint: '[pr-number|file-path] [--effort low|medium|high] [--comment] [--fix]',
                    sourceBadge: '[Skill]',
            ...
            ], activeIndex: 0, isLoading: false, width: 120, scrollOffset: 0, userInput: "/rev", mode: "slash" }));
        const lines = (lastFrame() ?? '').split('\n');
...
describe('SuggestionsDisplay tabs', () => {
    const mixed = [
        ...
        { label: 'Fix bug', value: 'session:id-1', category: 'session' },
    ];
    it('shows a tab bar when multiple categories are present', () => {
        const { lastFrame } = render(_jsx(SuggestionsDisplay, { suggestions: mixed, activeIndex: 0, isLoading: false, width: 80, scrollOffset: 0, userInput: "", mode: "reverse", activeCategory: "all", availableCategories: ['all', 'file', 'session'] }));
        expect(lastFrame()).toContain('Files');
        expect(lastFrame()).toContain('Sessions');
    });
    it('filters rows to the active category', () => {
        const { lastFrame } = render(_jsx(SuggestionsDisplay, { suggestions: mixed, activeIndex: 0, isLoading: false, width: 80, scrollOffset: 0, userInput: "", mode: "reverse", activeCategory: "session", availableCategories: ['all', 'file', 'session'] }));
    ...
    it('hides the tab bar for single-category (file-only) completion', () => {
        const { lastFrame } = render(_jsx(SuggestionsDisplay, { suggestions: [mixed[0]], activeIndex: 0, isLoading: false, width: 80, scrollOffset: 0, userInput: "", mode: "reverse", activeCategory: "all", availableCategories: ['all', 'file'] }));
