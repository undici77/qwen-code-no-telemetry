...
import { prepareInlineTerminalImage, renderTerminalImage, } from '../utils/terminal-image-renderer.js';
import { TerminalImage } from './TerminalImage.js';
const { writtenKeys } = vi.hoisted(() => ({ writtenKeys: new Set() }));
vi.mock('../utils/terminal-image-renderer.js', () => ({
    prepareInlineTerminalImage: vi.fn(),
    renderTerminalImage: vi.fn(),
    wasKittyImageWritten: vi.fn((key) => writtenKeys.has(key)),
    markKittyImageWritten: vi.fn((key) => {
        writtenKeys.add(key);
    }),
}));
...
function configWithWorkspaceResult(isWithinWorkspace) {
    return {
        getWorkspaceContext: () => ({
...
function renderImage(result, writeRaw = vi.fn()) {
    mockedRenderTerminalImage.mockReturnValueOnce(result);
    return render(_jsx(TerminalOutputProvider, { value: writeRaw, children: _jsx(Static, { items: [IMAGE], children: (item) => (_jsx(TerminalImage, { data: item, config: configWithWorkspaceResult(true), contentWidth: 80, availableTerminalHeight: 20 })) }) }));
}
describe('TerminalImage', () => {
    ...
    it('refuses restored paths outside the current workspace', () => {
        const { lastFrame } = render(_jsx(TerminalImage, { data: {
                type: 'terminal_image',
                filePath: '/outside/chart.png',
                mimeType: 'image/png',
            }, config: configWithWorkspaceResult(false), contentWidth: 80 }));
    ...
    it('does not re-emit the Kitty sequence when the emit effect re-runs', async () => {
        ...
        const renderWith = (writer) => (_jsx(TerminalOutputProvider, { value: writer, children: _jsx(TerminalImage, { data: IMAGE, config: configWithWorkspaceResult(true), contentWidth: 80, availableTerminalHeight: 20 }) }));
    ...
    it('renders inline image data through the shared renderer', async () => {
        ...
        const { lastFrame } = render(_jsx(TerminalOutputProvider, { value: writeRaw, children: _jsx(TerminalImage, { image: INLINE_IMAGE, contentWidth: 80 }) }));
        await vi.waitFor(() => {
            expect(writeRaw).toHaveBeenCalledWith(KITTY_RESULT.sequence);
        });
        expect(lastFrame()).toContain('placeholder');
        expect(mockedPrepareInlineTerminalImage).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('explains why an inline image renderer is unavailable', () => {
        ...
        const { lastFrame } = render(_jsx(TerminalImage, { image: INLINE_IMAGE, contentWidth: 80 }));
    ...
    it('uses the deterministic inline placeholder for screen readers', () => {
        ...
        const { lastFrame } = render(_jsx(TerminalImage, { image: INLINE_IMAGE, contentWidth: 80 }));
        expect(lastFrame()).toContain('[image: 1x1 png]');
        expect(mockedPrepareInlineTerminalImage).toHaveBeenCalledWith(expect.objectContaining({ disabled: true }));
    });
});
//# sourceMappingURL=TerminalImage.test.js.map