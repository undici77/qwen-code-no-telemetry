...
describe('TrustDialog', () => {
    ...
    it('should render the main dialog with current trust level', async () => {
        const { lastFrame } = renderWithProviders(_jsx(TrustDialog, { onExit: vi.fn(), addItem: vi.fn() }));
        await waitFor(() => {
    ...
    it('should display the inherited trust note from parent', async () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(TrustDialog, { onExit: vi.fn(), addItem: vi.fn() }));
        await waitFor(() => {
            expect(lastFrame()).toContain('currently inherits trust from a parent folder');
        });
    });
    ...
    it('should call onExit when escape is pressed', async () => {
        const onExit = vi.fn();
        const { stdin, lastFrame } = renderWithProviders(_jsx(TrustDialog, { onExit: onExit, addItem: vi.fn() }));
...
//# sourceMappingURL=TrustDialog.test.js.map