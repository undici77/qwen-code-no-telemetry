...
describe('EffortDialog', () => {
    ...
    it('renders the title and all five reasoning-effort tiers', () => {
        const { lastFrame } = renderWithProviders(_jsx(EffortDialog, { onSelect: vi.fn() }));
    ...
    it('shows the "no effort configured" hint when currentEffort is unset', () => {
        const { lastFrame } = renderWithProviders(_jsx(EffortDialog, { onSelect: vi.fn() }));
        expect(lastFrame() ?? '').toContain('No effort configured — using the model/provider default.');
    });
    it('hides the "no effort configured" hint when currentEffort is set', () => {
        const { lastFrame } = renderWithProviders(_jsx(EffortDialog, { onSelect: vi.fn(), currentEffort: "high" }));
        expect(lastFrame() ?? '').not.toContain('No effort configured');
    });
    it('registers an active Escape handler that cancels with undefined', () => {
        const onSelect = vi.fn();
        renderWithProviders(_jsx(EffortDialog, { onSelect: onSelect }));
...
//# sourceMappingURL=EffortDialog.test.js.map