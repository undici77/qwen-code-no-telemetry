...
function snapshot(status, activity = 'idle', lastReason) {
    return {
        v: 2,
        activity,
        goal: {
...
describe('<GoalStatusMessage />', () => {
    it('is wrapped in React.memo to avoid unnecessary scrollback rerenders', () => {
        expect(GoalStatusMessage.$$typeof).toBe(Symbol.for('react.memo'));
    });
    it('shows the goal and judge reason on checking cards', () => {
        const { lastFrame } = render(_jsx(GoalStatusMessage, { kind: "checking", condition: "finish the refactor", iterations: 2, lastReason: "tests are still failing" }));
    ...
    it('shows impossible goals as failed terminal cards', () => {
        const { lastFrame } = render(_jsx(GoalStatusMessage, { kind: "failed", condition: "merge a nonexistent branch", iterations: 2, durationMs: 12_000, lastReason: "the remote branch does not exist" }));
        const output = lastFrame();
    ...
    it('renders legacy pause as a non-terminal paused card', () => {
        const { lastFrame } = render(_jsx(GoalStatusMessage, { kind: "paused", condition: "finish the refactor", iterations: 2, durationMs: 12_000 }));
        const output = lastFrame();
    ...
    it.each([
        ...
        const { lastFrame } = render(_jsx(GoalStatusMessage, { snapshot: value }));
        const output = lastFrame();
...
//# sourceMappingURL=GoalStatusMessage.test.js.map