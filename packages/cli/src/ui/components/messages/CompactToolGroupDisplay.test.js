...
import { CompactToolGroupDisplay, buildToolSummary, estimateCompactToolGroupHeight, isCollapsibleTool, } from './CompactToolGroupDisplay.js';
...
describe('<CompactToolGroupDisplay /> — summary label', () => {
    ...
    it('wraps long summaries instead of truncating them', () => {
        const description = 'packages/cli/src/ui/components/messages/CompactToolGroupDisplay.tsx';
        const tool = toolCall({ name: 'ReadFile', description });
        const { lastFrame } = render(_jsx(CompactToolGroupDisplay, { toolCalls: [tool], contentWidth: 30 }));
        const frame = lastFrame();
    ...
    it('keeps a 30-file active batch to two rows at 80 columns', () => {
        const tools = Array.from({ length: 30 }, (_, index) => toolCall({
            callId: `c${index + 1}`,
            name: 'ReadFile',
            description: `packages/cli/src/ui/components/example-${String(index + 1).padStart(2, '0')}.tsx`,
            status: index === 29 ? ToolCallStatus.Executing : ToolCallStatus.Success,
        }));
        const { lastFrame } = render(_jsx(CompactToolGroupDisplay, { toolCalls: tools, contentWidth: 80 }));
        const frame = lastFrame();
    ...
    it('wraps a long inline summary without a redundant hint', () => {
        const currentPath = 'packages/cli/src/ui/components/messages/CompactToolGroupDisplay.tsx';
        const tools = [
            toolCall({ callId: 'c1', name: 'ReadFile', description: 'a.ts' }),
            toolCall({
                callId: 'c2',
        ...
        const { lastFrame } = render(_jsx(CompactToolGroupDisplay, { toolCalls: tools, contentWidth: 30 }));
        const frame = lastFrame();
...
describe('buildToolSummary', () => {
    ...
    it('multiple same-type tools show first 2 + "...and N more" when > 3', () => {
        ...
        expect(buildToolSummary(tools, false)).toBe('Read a.ts, b.ts, ... and 2 more');
    });
    ...
    it('mixed types joined with comma and lowercase verbs', () => {
        ...
        expect(buildToolSummary(tools, false)).toBe('Read a.ts, ran npm test, edited b.ts');
    });
    ...
    it('keeps file names that resemble JSON delimiters', () => {
        expect(buildToolSummary([make({ description: '[id].tsx' })], false)).toBe('Read [id].tsx');
        expect(buildToolSummary([make({ description: '{draft}.md' })], false)).toBe('Read {draft}.md');
    });
    ...
    it('mixed group shows descriptions inline per category', () => {
        ...
        expect(buildToolSummary(tools, false)).toBe('Read a.ts, b.ts, ran npm test');
    });
    it('legacy display names map to correct categories', () => {
        ...
        expect(buildToolSummary(tools, false)).toBe('Searched pattern, listed /src');
    });
});
describe('estimateCompactToolGroupHeight', () => {
    ...
    it('reserves additional width for active summary status', () => {
        ...
        expect(estimateCompactToolGroupHeight([active], 30)).toBeGreaterThan(estimateCompactToolGroupHeight([completed], 30));
    });
    it('reserves timeout label width for active shell summaries', () => {
        const description = 'npm test -- --filter packages/cli/src/ui/components/messages';
        ...
        expect(estimateCompactToolGroupHeight([activeShellWithTimeout], 55)).toBeGreaterThan(estimateCompactToolGroupHeight([activeShell], 55));
    });
