...
vi.mock('../TerminalImage.js', () => ({
    TerminalImage: ({ data, image, availableTerminalHeight, }) => (_jsx(Text, { children: image
...
vi.mock('./ToolConfirmationMessage.js', () => ({
    ToolConfirmationMessage: function MockToolConfirmationMessage({ availableTerminalHeight, }) {
        ...
        return (_jsxs(Text, { children: ["MockApprovalPrompt:height=", availableTerminalHeight ?? 'undef'] }));
    },
}));
// Mock settings
const mockSettings = {
...
const renderWithContext = (ui, streamingState) => {
    ...
    return render(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(StreamingContext.Provider, { value: contextValue, children: ui }) }));
};
describe('<ToolMessage />', () => {
    ...
    it('collapses text/ANSI result for completed collapsible tool', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "config.yaml" }), StreamingState.Idle);
    ...
    it('renders inline images returned by a tool', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, images: [{ data: 'aW1hZ2U=', mimeType: 'image/png' }] }), StreamingState.Idle);
        expect(lastFrame()).toContain('MockTerminalImage:image/png');
    });
    it('renders the number of omitted inline images', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, omittedImageCount: 2 }), StreamingState.Idle);
        expect(lastFrame()).toContain('[+2 more images]');
    });
    it('shares the tool height budget across inline images', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, availableTerminalHeight: 20, images: [
    ...
    it('always shows the vision bridge disclosure for a completed read', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "scanned.pdf", resultDisplay: {
    ...
    it('shows a tool-result vision notice without replacing its display', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, visionBridgeNotice: "Converted 1 image via qwen3-vl-plus." }), StreamingState.Idle);
        const output = lastFrame();
        expect(output).toContain('qwen3-vl-plus');
    ...
    it('keeps the vision bridge disclosure beside full read details', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "scanned.pdf", resultDisplay: {
                ...
                notice: 'Converted 4 images via qwen3-vl-plus (dashscope.aliyuncs.com).',
            }, detailedDisplay: "Page 20: transcribed content", fullDetail: true, forceShowResult: true }), StreamingState.Idle);
        const output = lastFrame();
        expect(output).toContain('Transcribed PDF pages 20-23');
        expect(output).toContain('dashscope.aliyuncs.com');
    ...
    it('shows the vision bridge disclosure when the PDF fallback is an error', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "scanned.pdf", status: ToolCallStatus.Error, resultDisplay: {
                type: 'vision_bridge_notice',
    ...
    it('collapses ANSI result for completed collapsible tool', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "config.yaml", resultDisplay: ansiResult }), StreamingState.Idle);
        const output = lastFrame();
        expect(output).toContain('ReadFile');
    ...
    it('renders structured terminal image results through TerminalImage', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "DisplayImage", resultDisplay: {
                type: 'terminal_image',
                filePath: '/workspace/chart.png',
                mimeType: 'image/png',
            } }), StreamingState.Idle);
        expect(lastFrame()).toContain('MockTerminalImage:/workspace/chart.png:image/png');
    });
    it('renders tool results directly below the header row when forced', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, contentWidth: 100, forceShowResult: true }), StreamingState.Idle);
        const lines = (lastFrame() ?? '').split('\n');
        const headerLine = lines.findIndex((line) => line.includes('test-tool'));
        const resultLine = lines.findIndex((line) => line.includes('MockMarkdown:Test result'));
    ...
    it('hides text result output for completed collapsible tools', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "Grep", description: "search pattern" }), StreamingState.Idle);
        const output = lastFrame();
        expect(output).toContain('✓');
    ...
    it('shows result when forceShowResult overrides collapse', () => {
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, forceShowResult: true }), StreamingState.Idle);
        expect(lastFrame()).toContain('MockMarkdown:Test result');
    });
    describe('fullDetail (§4.9 transcript) data-source switch', () => {
        it('swaps summary for detailedDisplay on a collapsible tool in fullDetail mode', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "config.yaml", resultDisplay: "Read 1 file", detailedDisplay: "full file contents here", fullDetail: true, forceShowResult: true }), StreamingState.Idle);
            const output = lastFrame();
        ...
        it('renders detailedDisplay as plain text, not Markdown', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "config.yaml", resultDisplay: "Read 1 file", detailedDisplay: "# heading from file\\n- list item", fullDetail: true, forceShowResult: true }), StreamingState.Idle);
            const output = lastFrame();
        ...
        it('escapes ANSI/control sequences in detailedDisplay (no raw injection)', () => {
            ...
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "evil.txt", resultDisplay: "Read 1 file", detailedDisplay: 'before\x1b[?1049lafter\x1b]52;c;ZXZpbA==\x07mid\x08\x0c\x0eend', fullDetail: true, forceShowResult: true }), StreamingState.Idle);
            const output = lastFrame() ?? '';
        ...
        it('strips Unicode bidi override chars in detailedDisplay (Trojan Source)', () => {
            ...
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "evil.txt", resultDisplay: "Read 1 file", detailedDisplay: 'safe\u202estart\u202cmid\u2066end\u2069', fullDetail: true, forceShowResult: true }), StreamingState.Idle);
            const output = lastFrame() ?? '';
        ...
        it('preserves TAB and LF in detailedDisplay (structural whitespace)', () => {
            ...
            const { lastFrame, stdout } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "table.txt", resultDisplay: "Read 1 file", detailedDisplay: 'colA\tcolB\nrow2A\trow2B', fullDetail: true, forceShowResult: true }), StreamingState.Idle);
            const output = lastFrame() ?? '';
        ...
        it('keeps the summary when forced but NOT in fullDetail mode (main-view force)', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "config.yaml", resultDisplay: "Read 1 file", detailedDisplay: "full file contents here", forceShowResult: true }), StreamingState.Idle);
            const output = lastFrame();
        ...
        it('keeps the summary for a non-collapsible tool even in fullDetail mode', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "test-tool", resultDisplay: "Test result", detailedDisplay: "should not appear", fullDetail: true, forceShowResult: true }), StreamingState.Idle);
            const output = lastFrame();
            expect(output).toContain('MockMarkdown:Test result');
            expect(output).not.toContain('should not appear');
        });
        it('falls back to the summary when fullDetail is set but no detailedDisplay exists', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "config.yaml", resultDisplay: "Read 1 file", fullDetail: true, forceShowResult: true }), StreamingState.Idle);
    ...
    describe('ToolStatusIndicator rendering', () => {
        ...
        it('hides a tool description repeated in a plain-text Hook confirmation', () => {
            ...
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: ToolCallStatus.Confirming, description: JSON.stringify({ content }), confirmationDetails: {
                    type: 'info',
                    title: 'Hook confirmation',
                    prompt: `Complete content is shown here:\n${escapedContent}`,
                    renderPromptAsPlainText: true,
                    onConfirm: vi.fn(),
                }, contentWidth: 50 }), StreamingState.Idle);
        ...
        it('does not hide a tool description absent from the Hook confirmation', () => {
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: ToolCallStatus.Confirming, description: `COMMAND_TOP ${'middle '.repeat(20)} COMMAND_TAIL`, confirmationDetails: {
                    type: 'info',
                    title: 'Hook confirmation',
        ...
        it('keeps a repeated string description when another argument is not shown', () => {
            ...
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, status: ToolCallStatus.Confirming, description: JSON.stringify({ content, destructive: true }), confirmationDetails: {
                    type: 'info',
                    title: 'Hook confirmation',
                    prompt: `Complete content is shown here:\n${JSON.stringify(content)}`,
                    renderPromptAsPlainText: true,
                    onConfirm: vi.fn(),
                }, contentWidth: 80 }), StreamingState.Idle);
    ...
    it('renders DiffRenderer for diff results', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, resultDisplay: diffResult, forceShowResult: true }), StreamingState.Idle);
    ...
    it('diff results are not collapsed for completed collapsible tools (bypass shouldCollapseResult)', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ReadFile", description: "a.ts", resultDisplay: diffResult, status: ToolCallStatus.Success }), StreamingState.Idle);
        expect(lastFrame()).toMatch(/MockDiff:--- a\/file\.txt/);
    });
    ...
    describe('subagent inline rendering (approval-only surface)', () => {
        ...
        it('reserves the header and context lines out of the confirmation height budget', () => {
            ...
            const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...buildProps({
                    ...
                    isFocused: true,
                }), availableTerminalHeight: 20 }), StreamingState.Responding);
            const output = lastFrame() ?? '';
    ...
    it('renders AnsiOutputText for AnsiOutput results', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, resultDisplay: ansiOutputDisplay, forceShowResult: true }), StreamingState.Idle);
    ...
    it('caps shell ANSI output to default 5 lines when not forced', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "Shell", status: ToolCallStatus.Executing, resultDisplay: ansiOutputDisplay, availableTerminalHeight: 100 }), StreamingState.Idle);
        const output = lastFrame();
    ...
    it('does not cap non-shell ANSI output', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "some-other-tool", resultDisplay: ansiOutputDisplay, availableTerminalHeight: 100, forceShowResult: true }), StreamingState.Idle);
        const output = lastFrame();
    ...
    it('disables cap when ui.shellOutputMaxLines is 0', () => {
        ...
        const { lastFrame } = render(_jsx(SettingsContext.Provider, { value: settingsWithDisabledCap, children: _jsx(StreamingContext.Provider, { value: StreamingState.Idle, children: _jsx(ToolMessage, { ...baseProps, name: "Shell", status: ToolCallStatus.Executing, resultDisplay: ansiOutputDisplay, availableTerminalHeight: 100 }) }) }));
        const output = lastFrame();
        expect(output).toContain('height=94');
    });
    it('respects user-configured cap value', () => {
        ...
        const { lastFrame } = render(_jsx(SettingsContext.Provider, { value: settingsWithCustomCap, children: _jsx(StreamingContext.Provider, { value: StreamingState.Idle, children: _jsx(ToolMessage, { ...baseProps, name: "Shell", status: ToolCallStatus.Executing, resultDisplay: ansiOutputDisplay, availableTerminalHeight: 100 }) }) }));
        const output = lastFrame();
    ...
    it('caps shell completed string output (returnDisplayMessage path)', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "Shell", resultDisplay: longString, status: ToolCallStatus.Executing, availableTerminalHeight: 100 }), StreamingState.Idle);
        const output = lastFrame();
    ...
    it('pre-slices large non-shell string output before MaxSizedBox layout', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "some-other-tool", resultDisplay: longString, status: ToolCallStatus.Success, availableTerminalHeight: 12, forceShowResult: true }), StreamingState.Idle);
        const output = lastFrame();
    ...
    it('pre-slices single-line output by visual width before MaxSizedBox layout', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "some-other-tool", contentWidth: 20, resultDisplay: longSingleLine, status: ToolCallStatus.Success, availableTerminalHeight: 12, forceShowResult: true }), StreamingState.Idle);
        const output = lastFrame();
    ...
    it('does not pre-slice string output that exactly fits available height', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "some-other-tool", resultDisplay: exactFitString, status: ToolCallStatus.Success, availableTerminalHeight: 12, forceShowResult: true }), StreamingState.Idle);
        const output = lastFrame();
    ...
    it.each([
        ...
        const { lastFrame } = render(_jsx(SettingsContext.Provider, { value: settingsWithBadCap, children: _jsx(StreamingContext.Provider, { value: StreamingState.Idle, children: _jsx(ToolMessage, { ...baseProps, name: "Shell", status: ToolCallStatus.Executing, resultDisplay: ansiOutputDisplay, availableTerminalHeight: 100 }) }) }));
        const output = lastFrame();
    ...
    it('does not cap non-shell string output', () => {
        const longString = Array.from({ length: 30 }, (_, i) => `line ${i + 1}`).join('\n');
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "some-other-tool", resultDisplay: longString, status: ToolCallStatus.Success, availableTerminalHeight: 100, forceShowResult: true }), StreamingState.Idle);
        const output = lastFrame();
    ...
    it('renders approved plan content with approval message', () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...baseProps, name: "ExitPlanMode", description: "Plan:", status: ToolCallStatus.Success, resultDisplay: planResultDisplay, forceShowResult: true }), StreamingState.Idle);
        const output = lastFrame();
...
describe('<ToolMessage /> localized badge', () => {
    ...
    it('shows the localized display name under the zh locale', async () => {
        ...
        const { lastFrame } = renderWithContext(_jsx(ToolMessage, { ...localizedProps }), StreamingState.Idle);
        const output = lastFrame() ?? '';
