 ...
 */
function makeRegistryConfig(entries) {
    const store = new Map();
    for (const e of entries) {
        if (e.agentId) {
            store.set(e.agentId, e);
        }
    }
    const config = {
...
function renderInline(options) {
    let result;
    act(() => {
        result = render(_jsx(ConfigContext.Provider, { value: options.config, children: _jsx(InlineParallelAgentsDisplay, { toolCalls: options.toolCalls, contentWidth: 120 }) }));
    });
    return result;
}
describe('<InlineParallelAgentsDisplay />', () => {
    ...
    it('surfaces live activity + elapsed from the registry', () => {
        ...
        let result;
        act(() => {
            result = render(_jsx(ConfigContext.Provider, { value: config, children: _jsx(InlineParallelAgentsDisplay, { toolCalls: toolCalls, contentWidth: 120 }) }));
        });
    ...
    describe('height backstop (availableTerminalHeight)', () => {
        const manyAgents = (n) => Array.from({ length: n }, (_, i) => agentToolCall({
            callId: `cap-${i}`,
            subagentName: 'general-purpose',
            taskDescription: `CapAgent ${i}`,
            status: 'completed',
        }));
        const renderCapped = (toolCalls, availableTerminalHeight) => render(_jsx(ConfigContext.Provider, { value: undefined, children: _jsx(InlineParallelAgentsDisplay, { toolCalls: toolCalls, contentWidth: 120, availableTerminalHeight: availableTerminalHeight }) }));
...
//# sourceMappingURL=InlineParallelAgentsDisplay.test.js.map