...
import { extractRememberErrorCode, extractRememberErrorDetails, extractRememberErrorStack, shouldSuppressRememberErrorDetails, workspaceMemoryFailureCode, workspaceMemoryFailureDiagnostics, } from './workspace-remember-errors.js';
describe('extractRememberErrorCode', () => {
    it('extracts remember error codes from common error shapes', () => {
        expect(extractRememberErrorCode({ code: 'remember_queue_full' })).toBe('remember_queue_full');
        expect(extractRememberErrorCode({
            data: { errorKind: 'managed_memory_unavailable' },
        })).toBe('managed_memory_unavailable');
        expect(extractRememberErrorCode({ data: { code: 'remember_path_escape' } })).toBe('remember_path_escape');
        expect(extractRememberErrorCode({
        ...
        expect(extractRememberErrorCode(new Error('boom'), 'forget_failed')).toBe('forget_failed');
    });
    ...
    it('falls back when code extraction throws', () => {
        ...
        expect(workspaceMemoryFailureCode(err, 'dream_failed', (target, cause) => extractionErrors.push({
            target,
    ...
    it('keeps fallback behavior when code extraction logging throws', () => {
        ...
        expect(workspaceMemoryFailureCode(err, 'dream_failed', () => {
...
describe('extractRememberErrorDetails', () => {
    it('extracts details from common error shapes', () => {
        ...
        expect(extractRememberErrorDetails({
            data: { details: 'agent stopped because max turns exceeded' },
        })).toBe('agent stopped because max turns exceeded');
        expect(extractRememberErrorDetails({ data: 'raw data detail' })).toBe('raw data detail');
        expect(extractRememberErrorDetails({
        ...
        expect(extractRememberErrorDetails({ cause: 'string cause reason' })).toBe('string cause reason');
        expect(extractRememberErrorDetails('raw string error')).toBe('raw string error');
    });
    ...
    it('redacts credentials before exposing details', () => {
        const details = extractRememberErrorDetails(new Error('Authorization: Bearer secret-token-value'));
    ...
    it('normalizes hidden separators before redacting credentials', () => {
        for (const separator of [
            ...
            const details = extractRememberErrorDetails(new Error(`Authorization: Bearer${separator}secret-token-value`));
            expect(details).toBe('Authorization: <redacted>');
            expect(details).not.toContain('secret-token-value');
        }
    });
    it('redacts credentials with hidden separators inside token values', () => {
        for (const separator of [
            ...
            const details = extractRememberErrorDetails(new Error(`Authorization: Bearer secret${separator}token-value`));
            expect(details).toBe('Authorization: <redacted>');
    ...
    it('redacts bearer tokens split by control characters', () => {
        const details = extractRememberErrorDetails(new Error('Authorization: Bearer secret\x01token-value'));
        expect(details).toBe('Authorization: <redacted>');
        expect(details).not.toContain('secret');
        expect(details).not.toContain('token-value');
    });
    it('redacts bearer tokens split by unicode space separators', () => {
        for (const separator of [
            ...
            const details = extractRememberErrorDetails(new Error(`Bearer sk-AAAAAAAAAA${separator}BBBBBBBBBBBBBBB`));
    ...
    it('redacts bearer tokens after unicode space separators', () => {
        for (const separator of ['\u00a0', '\u2009', '\u202f']) {
            const details = extractRememberErrorDetails(new Error(`Bearer${separator}secret-token-value`));
            expect(details).toBe('Bearer <redacted>');
            expect(details).not.toContain('secret-token-value');
        }
    });
    it('normalizes unicode space separators without collapsing words', () => {
        const details = extractRememberErrorDetails(new Error('missing column\u00a0name'));
        expect(details).toBe('missing column name');
    });
    it('redacts bare tokens split by invisible characters', () => {
        const details = extractRememberErrorDetails(new Error('OpenAI key sk-AAAAAAAAAA\u2062BBBBBBBBBBBBBBB'));
    ...
    it('redacts platform tokens split by invisible characters', () => {
        for (const prefix of [
            ...
            const details = extractRememberErrorDetails(new Error(`Platform token ${prefix}AAAAAAAAAA\u200bBBBBBBBBBBBBBBB`));
    ...
    it('redacts platform tokens split by unicode space separators', () => {
        const details = extractRememberErrorDetails(new Error('Platform token ghp_AAAAAAAAAA\u00a0BBBBBBBBBBBBBBB'));
        expect(details).toBe('Platform token <redacted>');
        expect(details).not.toContain('AAAAAAAAAA');
        expect(details).not.toContain('BBBBBBBBBBBBBBB');
    });
    it('redacts AWS access keys split by credential separators', () => {
        for (const separator of ['\x01', '\u00a0', '\u200b']) {
            const details = extractRememberErrorDetails(new Error(`AWS key AKIA12345678${separator}90123456`));
    ...
    it('redacts secret assignments split by credential separators', () => {
        for (const separator of ['\x01', '\u00a0', '\u200b']) {
            const details = extractRememberErrorDetails(new Error(`token=aaaaaaaaaa${separator}bbbbbbbb`));
    ...
    it('redacts env secret assignments split by credential separators', () => {
        const details = extractRememberErrorDetails(new Error('QWEN_DAEMON_TOKEN=AAAAAAAAAA\u00a0BBBBBBBB'));
    ...
    it('redacts URL credentials split by credential separators', () => {
        for (const separator of ['\x01', '\u00a0', '\u200b']) {
            const details = extractRememberErrorDetails(new Error(`postgresql://admin:S3cret${separator}P4ssw0rd@db.internal:5432/mydb`));
    ...
    it('redacts bare bearer tokens separated by invisible characters', () => {
        const details = extractRememberErrorDetails(new Error('Bearer\u200BeyJhbGciOiABCDEFGHIJKLMN'));
        expect(details).toBe('Bearer <redacted>');
        expect(details).not.toContain('eyJhbGciOiABCDEFGHIJKLMN');
    });
    it('redacts QQBot tokens separated by invisible characters', () => {
        const details = extractRememberErrorDetails(new Error('QQBot\u200Bsecret-token-value'));
    ...
    it('does not split surrogate pairs when capping long details', () => {
        const details = extractRememberErrorDetails(new Error(`${'x'.repeat(984)}${'😀'.repeat(100)}`));
    ...
    it('keeps the full prefix when the cut point falls before a surrogate pair', () => {
        const details = extractRememberErrorDetails(new Error(`${'x'.repeat(985)}${'😀'.repeat(100)}`));
...
describe('extractRememberErrorStack', () => {
    ...
    it('normalizes unicode space separators in stacks without collapsing words', () => {
        ...
        const stack = extractRememberErrorStack(err);
        expect(stack).toBe('Error: missing column name\n\tat handler (/workspace/file.ts:1:1)');
    });
...
describe('workspaceMemoryFailureDiagnostics', () => {
    it('falls back when detail extraction throws', () => {
        ...
        const diagnostics = workspaceMemoryFailureDiagnostics(err, (target, cause) => extractionErrors.push({
            target,
            message: cause instanceof Error ? cause.message : String(cause),
        }));
...
describe('shouldSuppressRememberErrorDetails', () => {
    it('suppresses details only for configured public errors', () => {
        expect(shouldSuppressRememberErrorDetails('managed_memory_unavailable')).toBe(true);
        expect(shouldSuppressRememberErrorDetails('remember_failed')).toBe(false);
    });
});
//# sourceMappingURL=workspace-remember-errors.test.js.map