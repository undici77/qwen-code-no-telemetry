...
import { CHANNEL_DELIVERY_IPC_TIMEOUT_MS, MAX_CHANNEL_DELIVERY_TEXT_LENGTH, ChannelDeliveryError, createChannelDeliveryMessage, isChannelDeliveryError, isChannelDeliveryMessage, isChannelDeliveryResultMessage, } from './channel-delivery-ipc.js';
const request = {
...
describe('channel delivery IPC', () => {
    it('creates a bounded request message', () => {
        ...
        expect(message.expiresAt).toBeGreaterThanOrEqual(before + CHANNEL_DELIVERY_IPC_TIMEOUT_MS);
        expect(message.expiresAt).toBeLessThanOrEqual(Date.now() + CHANNEL_DELIVERY_IPC_TIMEOUT_MS);
    ...
    it('accepts success and typed failure results', () => {
        expect(isChannelDeliveryResultMessage({
    ...
    it('recognizes public delivery errors across module instances', () => {
        expect(isChannelDeliveryError(new ChannelDeliveryError('channel_delivery_failed', 'Platform send failed.'))).toBe(true);
        expect(isChannelDeliveryError({
            code: 'channel_worker_unavailable',
            message: 'Worker stopped.',
        })).toBe(true);
        expect(isChannelDeliveryError({ code: 'unknown', message: 'nope' })).toBe(false);
    });
});
//# sourceMappingURL=channel-delivery-ipc.test.js.map