...
describe('mapSkillConfigToStatus', () => {
    it('maps an invocable skill to an ok status with its core fields', () => {
        const status = mapSkillConfigToStatus(makeSkill({ argumentHint: '[pr-number]' }));
    ...
    it('keeps a disable-model-invocation skill available for manual use', () => {
        const status = mapSkillConfigToStatus(makeSkill({ name: 'internal', disableModelInvocation: true }));
    ...
    it('only emits userInvocable when manual invocation is disabled', () => {
        expect(mapSkillConfigToStatus(makeSkill())).not.toHaveProperty('userInvocable');
        expect(mapSkillConfigToStatus(makeSkill({ userInvocable: false }))).toMatchObject({ userInvocable: false });
    });
    it('marks a settings-disabled skill as disabled', () => {
        const status = mapSkillConfigToStatus(makeSkill({ name: 'internal' }), new Map([['internal', { reason: 'hard', lockedScope: 'user' }]]));
    ...
    it('marks a forced-disabled skill as disabled', () => {
        const status = mapSkillConfigToStatus(makeSkill(), new Map([['review', { reason: 'hard', lockedScope: 'user' }]]), { disabled: true });
        expect(status.status).toBe('disabled');
        expect(status.modelInvocable).toBe(true);
    ...
    it('surfaces optional model and extensionName only when present', () => {
        ...
        expect(mapSkillConfigToStatus(makeSkill())).not.toHaveProperty('extensionName');
        const status = mapSkillConfigToStatus(makeSkill({ model: 'gpt-4o', extensionName: 'acme' }));
        expect(status.model).toBe('gpt-4o');
        expect(status.extensionName).toBe('acme');
    });
});
//# sourceMappingURL=workspace-skills-mapping.test.js.map