...
import { MAX_CHANNEL_DELIVERY_TEXT_LENGTH, normalizeChannelDelivery, normalizeChannelDeliveryText, parseChannelDelivery, } from './channel-delivery.js';
const delivery = {
    kind: 'channel',
    target: {
...
describe('channel delivery contract', () => {
    ...
    it.each([
        ...
        expect(() => parseChannelDelivery(value)).toThrow('Invalid channel delivery.');
    });
    it('normalizes and safely bounds delivery text for worker IPC', () => {
        const baseline = normalizeChannelDeliveryText('x'.repeat(MAX_CHANNEL_DELIVERY_TEXT_LENGTH + 1));
        ...
        expect(request.text.length).toBeLessThanOrEqual(MAX_CHANNEL_DELIVERY_TEXT_LENGTH);
    ...
    it('rejects empty text before worker IPC', () => {
        expect(() => normalizeChannelDelivery('prompt-1', delivery, '  ')).toThrow('Channel delivery text is empty.');
    });
});
//# sourceMappingURL=channel-delivery.test.js.map