...
import { ACP_TOOL_RESULT_TEXT_JSON_BYTE_BUDGET, ACP_TOOL_RESULT_TEXT_TRUNCATION_MARKER, projectAcpToolResultUpdate, } from './acp-tool-result-text-projection.js';
function textBlock(text) {
    return { type: 'content', content: { type: 'text', text } };
}
function toolUpdate(content, rawOutput, meta = { toolName: 'read_file' }) {
    return {
...
function asRecord(update) {
    return update;
}
function contentTexts(update) {
    return asRecord(update)['content'].map((block) => block.content.text);
}
function jsonBytes(value) {
    return Buffer.byteLength(JSON.stringify(value), 'utf8');
}
describe('ACP tool-result text projection', () => {
    it.each([65_535, 65_536, 65_537])('enforces the rawOutput boundary at %i JSON bytes', (targetBytes) => {
        ...
        expect(jsonBytes(projectedRaw)).toBeLessThanOrEqual(ACP_TOOL_RESULT_TEXT_JSON_BYTE_BUDGET);
        expect(projected === update).toBe(targetBytes <= 65_536);
    });
    it.each([65_535, 65_536, 65_537])('enforces the content boundary at %i JSON bytes', (targetBytes) => {
        const content = [textBlock('c'.repeat(targetBytes - 56))];
        expect(jsonBytes(content)).toBe(targetBytes);
        const update = toolUpdate(content);
        const projected = projectAcpToolResultUpdate(update);
        expect(jsonBytes(asRecord(projected)['content'])).toBeLessThanOrEqual(ACP_TOOL_RESULT_TEXT_JSON_BYTE_BUDGET);
        expect(projected === update).toBe(targetBytes <= 65_536);
    });
    it('keeps an approximately 20/80 head and tail preview', () => {
        const source = `HEAD-${'h'.repeat(200_000)}` + `-${'t'.repeat(200_000)}-TAIL`;
        const projected = projectAcpToolResultUpdate(toolUpdate([], source));
        const rawOutput = asRecord(projected)['rawOutput'];
        ...
        const [head, tail] = rawOutput.split(ACP_TOOL_RESULT_TEXT_TRUNCATION_MARKER);
    ...
    it('shares a deterministic budget across multiple text blocks', () => {
        ...
        expect(jsonBytes(asRecord(projected)['content'])).toBeLessThanOrEqual(65_536);
    ...
    it('bounds pathological many-block content without collapsing it', () => {
        const content = Array.from({ length: 600 }, (_, index) => textBlock(`${index}:${'x'.repeat(40_000)}`));
        const projected = projectAcpToolResultUpdate(toolUpdate(content));
        const texts = contentTexts(projected);
        expect(texts).toHaveLength(content.length);
        expect(texts.every((text) => text.includes(ACP_TOOL_RESULT_TEXT_TRUNCATION_MARKER))).toBe(true);
        expect(jsonBytes(asRecord(projected)['content'])).toBe(65_536);
    });
    ...
    it('collapses content when the minimum marker set cannot fit', () => {
        const content = Array.from({ length: 1_000 }, () => textBlock('x'.repeat(100)));
        const projected = projectAcpToolResultUpdate(toolUpdate(content));
        expect(contentTexts(projected)).toEqual([
            ACP_TOOL_RESULT_TEXT_TRUNCATION_MARKER,
        ]);
    ...
    it('reuses the stricter content preview when content and rawOutput match', () => {
        ...
        expect(jsonBytes(asRecord(projected)['content'])).toBeLessThanOrEqual(65_536);
        expect(jsonBytes(asRecord(projected)['rawOutput'])).toBeLessThanOrEqual(65_536);
    });
    it('projects eligible fields independently', () => {
        ...
        const rawProjected = projectAcpToolResultUpdate(toolUpdate(mixedContent, longRaw));
        expect(asRecord(rawProjected)['content']).toBe(mixedContent);
        expect(jsonBytes(asRecord(rawProjected)['rawOutput'])).toBeLessThanOrEqual(65_536);
        const contentProjected = projectAcpToolResultUpdate(toolUpdate(longContent, structuredRaw));
        expect(asRecord(contentProjected)['rawOutput']).toBe(structuredRaw);
        expect(jsonBytes(asRecord(contentProjected)['content'])).toBeLessThanOrEqual(65_536);
    });
    ...
    it('shares unchanged blocks and metadata when one block is projected', () => {
        ...
        const projected = projectAcpToolResultUpdate(update);
        const blocks = asRecord(projected)['content'];
    ...
    it.each([
        ...
        const projected = projectAcpToolResultUpdate(toolUpdate([textBlock(source)], source));
        expect(jsonBytes(asRecord(projected)['content'])).toBeLessThanOrEqual(65_536);
        expect(jsonBytes(asRecord(projected)['rawOutput'])).toBeLessThanOrEqual(65_536);
    });
    it('keeps an ordinary projected JSON-RPC frame below 256 KiB', () => {
        const source = 'x'.repeat(499_999);
        const update = projectAcpToolResultUpdate(toolUpdate([textBlock(source)], source));
        const frame = {
            jsonrpc: '2.0',
...
//# sourceMappingURL=acp-tool-result-text-projection.test.js.map