...
import { DaemonTodoStopGuard, TODO_STOP_GUARD_MAX_ATTEMPTS, } from './daemon-todo-stop-guard.js';
const pendingResult = {
    type: 'todo_list',
...
describe('DaemonTodoStopGuard', () => {
    it('arms only from a strict successful Todo result', () => {
        ...
        expect(guard.observeTodoWrite({ todos: pendingResult.todos }, true)).toBe(false);
        expect(guard.observeTodoWrite(JSON.stringify(pendingResult), true)).toBe(false);
        expect(guard.observeTodoWrite({ type: 'todo_list', todos: [{ status: 'pending' }] }, true)).toBe(false);
...
//# sourceMappingURL=daemon-todo-stop-guard.test.js.map