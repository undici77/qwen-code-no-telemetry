...
describe('computeInitialTurnFromHistory', () => {
    it('uses the largest numeric prompt id suffix for the current session', () => {
        expect(computeInitialTurnFromHistory([
            chatRecord({
                uuid: 'user-1',
...
describe('Session', () => {
    ...
    describe('rewindToTurn', () => {
        ...
        it('rejects unreachable user turns', () => {
            ...
            expect(() => session.rewindToTurn(2)).toThrow('Cannot rewind to the requested turn');
            expect(mockChat.truncateHistory).not.toHaveBeenCalled();
        });
        it('rejects rewinds while a cron prompt is mutating history', () => {
            session.cronProcessing = true;
            expect(() => session.rewindToTurn(0)).toThrow('Cannot rewind while a prompt is running');
            expect(mockChat.truncateHistory).not.toHaveBeenCalled();
        });
        it('rejects invalid target turn indexes', () => {
            expect(() => session.rewindToTurn(-1)).toThrow('targetTurnIndex must be a non-negative integer');
            expect(mockChat.truncateHistory).not.toHaveBeenCalled();
        });
        ...
        it('rejects rewinds while a cron abort is active', () => {
            session.cronAbortController = new AbortController();
            expect(() => session.rewindToTurn(0)).toThrow('Cannot rewind while a prompt is running');
            expect(mockChat.truncateHistory).not.toHaveBeenCalled();
        });
        ...
        it('rejects history restore while a prompt is running', () => {
            session.pendingPrompt =
                new AbortController();
            expect(() => session.restoreHistory([])).toThrow('Cannot restore history while a prompt is running');
    ...
    describe('setModel', () => {
        it('sets model via config and returns current model', async () => {
            ...
            expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'model.name', 'qwen3-coder-plus');
        ...
        it('can switch the session model without persisting a new default', async () => {
            ...
            expect(mockConfig.switchModel).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'qwen3-coder-flash', undefined);
            expect(mockSettings.setValue).not.toHaveBeenCalled();
        });
    ...
    describe('sendAvailableCommandsUpdate', () => {
        ...
        it('honors explicit no-input override for built-in commands with subCommands', async () => {
            ...
            await session.sendAvailableCommandsUpdate();
            expect(mockClient.sessionUpdate).toHaveBeenCalledWith(expect.objectContaining({
                sessionId: 'test-session-id',
    ...
    describe('prompt', () => {
        it('continues ACP prompt ids after replaying resumed history', async () => {
            ...
            expect(mockChatRecordingService.recordUserMessage).toHaveBeenCalledWith('3');
            expect(mockGeminiClient.tryCompressChat).toHaveBeenCalledWith('test-session-id########3', false, expect.any(AbortSignal));
        });
