...
export async function buildAvailableCommandsSnapshot(config, abortSignal = AbortSignal.timeout(10_000)) {
...
export class Session {
    ...
    async setModel(params, options = {}) {
        ...
        if (options.persistDefault ?? true) {
            ...
            this.settings.setValue(persistScope, 'security.auth.selectedType', selectedAuthType);
        }
    }
    /**
    ...
    async runTool(abortSignal, promptId, fc) {
        ...
        if (pm && !(await pm.isToolEnabled(fc.name))) {
            return earlyErrorResponse(new Error(`Qwen Code requires permission to use "${fc.name}", but that permission was declined.`), fc.name);
        }
        ...
        try {
            ...
            if (!autoModeAllowed &&
                ...
                if (approvalMode === ApprovalMode.AUTO_EDIT &&
                    ...
                    if (hooksEnabled && messageBus) {
                        void fireNotificationHook(messageBus, `Qwen Code needs your permission to use ${fc.name}`, NotificationType.PermissionPrompt, 'Permission needed');
                    }
                    const params = {
                        sessionId: this.sessionId,
