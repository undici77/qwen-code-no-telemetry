...
import { DAEMON_CHANNEL_DELIVERY_META_KEY, DAEMON_PROMPT_DISPLAY_TEXT_META_KEY, MID_TURN_QUEUE_DRAIN_METHOD, isValidTrustedModelPrompt, TODO_STOP_GUARD_CONTINUATION_CLAIM_METHOD, } from '@qwen-code/acp-bridge/bridgeTypes';
...
import { MessageEmitter, buildGoalStateUpdate, buildGoalStatusUpdate, } from './emitters/MessageEmitter.js';
...
function hasInlineMediaContentBlock(content) {
...
function getStructuredMidTurnDisplayText(content, displayText) {
    if (typeof displayText === 'string' && displayText.trim().length > 0) {
            ...
            return [
                {
                    kind: 'structured',
                    content,
                    displayText: getStructuredMidTurnDisplayText(content, item['displayText']),
                },
            ];
        });
    }
...
export class Session {
    ...
    async renderRecoveredGoalUpdates(replayedRecords) {
        ...
        if (publicationKey === this.lastGoalPublicationKey)
            return [];
        this.lastGoalPublicationKey = publicationKey;
        return [
            buildGoalStateUpdate(snapshot, cause, this.lastGoalSnapshot?.goal ?? null),
        ];
    }
    /**
    ...
    #goalPublicationKey(snapshot, cause) {
    ...
    primeTurnFromHistory(records) {
        for (const record of records) {
            if (record.subtype !== 'notification')
                continue;
            const backgroundTask = record.systemPayload?.backgroundTask;
            if (typeof backgroundTask?.taskId === 'string') {
                this.persistedBackgroundNotificationTaskIds.add(backgroundTask.taskId);
            }
        }
        this.turn = Math.max(this.turn, computeInitialTurnFromHistory(records, this.config.getSessionId()));
    }
    async replayHistory(records, gaps) {
    ...
    async prompt(params, invocationContext, admissionCancellation, modelPrompt, scheduledGoalTurn) {
        ...
        try {
            const result = await this.#executePrompt(params, pendingSend, channelDeliveryCapture, invocationContext, modelPrompt, goalTurn);
            promptResult = result;
            releasePendingSend();
            ...
            if (channelDelivery && result.stopReason === 'end_turn') {
                this.#scheduleChannelDelivery({
                    ...
                    text: normalizeChannelDeliveryText(channelDeliveryCapture?.finalText ?? ''),
            ...
            releasePendingSend();
            const shouldDrainAutomaticQueues = todoStopGuardPreparation.drainSupersededAutomaticQueues ||
        ...
        void (async () => {
            try {
                ...
                if (ac.signal.aborted)
                    return;
                if (r.suggestion) {
                    await this.client.extNotification('qwen/notify/session/prompt-suggestion', {
                        v: 1,
                        sessionId: this.sessionId,
    ...
    async #executePrompt(params, pendingSend, channelDeliveryCapture, invocationContext, modelPrompt, goalTurn) {
        ...
        const execute = () => runWithInvocationContext(invocationContext, () => sessionIdContext.run(sessionId, () => this.#executePromptInner(params, pendingSend, channelDeliveryCapture, modelPrompt, goalTurn)));
        return goalTurn
            ? goalTurnContext.run(goalTurn.permit, execute)
            : goalTurnContext.exit(execute);
    }
    async #executePromptInner(params, pendingSend, channelDeliveryCapture, modelPrompt, goalTurn) {
        return Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), async () => {
            ...
            return await withInteractionSpan(this.config, {
                ...
                const promptText = params.prompt
                    ...
                    .join(' ');
                const promptDisplayText = typeof promptMetadata?.[DAEMON_PROMPT_DISPLAY_TEXT_META_KEY] ===
                    'string'
                ...
                if (isContinue) {
                    ...
                    parts = await this.#processSlashCommandResult(slashCommandResult, modelPromptBlocks, pendingSend.signal, onFullTurnModel);
                ...
                let turnCount = 0;
                const toolLoopState = createDaemonToolLoopState(promptMetadata?.[CHANNEL_PROMPT_META_KEY] === true
                    ? 'off'
                ...
                try {
                    while (nextMessage !== null) {
                        ...
                        try {
                            ...
                            const channelDeliveryCheckpoint = channelDeliveryResponseBlock?.length ?? 0;
                        ...
                        commitChannelDeliveryResponseBlock(channelDeliveryCapture, channelDeliveryResponseBlock, functionCalls.length > 0);
                        if (usageMetadata) {
                        ...
                        if (functionCalls.length > 0) {
                            ...
                            const nextAfterTools = await this.#buildNextMessageAfterToolRun(toolRun, pendingSend.signal, promptId, toolLoopState, onFullTurnModel);
                    ...
                    return await this.#handleStopHookLoop(pendingSend, promptId, hooksEnabled, messageBus, true, fullTurnModelOverride, channelDeliveryCapture);
                }
                finally {
    ...
    async #handleStopHookLoop(pendingSend, promptId, hooksEnabled, messageBus, allowExternalHooks = true, modelOverride, channelDeliveryCapture) {
    ...
    async #runStopContinuation(pendingSend, streamPromptId, toolPromptId, parts, skipCompression, options = {}) {
        ...
        while (nextMessage !== null) {
            ...
            try {
                ...
                const responseStream = sendResult.responseStream;
                nextMessage = null;
                channelDeliveryResponseBlock = beginChannelDeliveryResponseBlock(options.channelDeliveryCapture);
                const channelDeliveryCheckpoint = channelDeliveryResponseBlock?.length ?? 0;
                initialSend = false;
                if (guardForThisSend) {
            ...
            commitChannelDeliveryResponseBlock(options.channelDeliveryCapture, channelDeliveryResponseBlock, functionCalls.length > 0);
            if (usageMetadata) {
                this.#recordPromptTokenCount(usageMetadata);
                const durationMs = Date.now() - streamStartTime;
                await this.messageEmitter.emitUsageMetadata(usageMetadata, '', durationMs);
            ...
            if (functionCalls.length > 0) {
                ...
                const nextAfterTools = await this.#buildNextMessageAfterToolRun(toolRun, pendingSend.signal, toolPromptId, toolLoopState, options.onFullTurnModel);
                nextMessage = nextAfterTools.message;
    ...
    async #buildNextMessageAfterToolRun(toolRun, abortSignal, promptId, toolLoopState, onFullTurnModel) {
        if (toolRun.loopDetected) {
        ...
        if (toolRun.repeatedDuplicateProviderToolCall) {
            this.todoStopGuard.suspend();
            debugLogger.debug('Stopping ACP turn after dropping repeated duplicate provider tool-call response.');
            return { message: null, hadMidTurnUserInput: false };
        }
                ...
                try {
                    ...
                    if (loopMode) {
                        ...
                        try {
                            ...
                            debugLogger.warn(`loop.md sentinel resolution failed (mode=${loopMode}, code=${code}) — check .qwen/loop.md permissions/IO`, resolveErr);
                            if (loopMode === 'dynamic' &&
                    ...
                    while (nextMessage !== null) {
                        ...
                        const responseStream = sendResult.responseStream;
                        const channelDeliveryResponseBlock = beginChannelDeliveryResponseBlock(channelDeliveryCapture);
                        const channelDeliveryCheckpoint = channelDeliveryResponseBlock?.length ?? 0;
                    ...
                    if (this.todoStopGuard.needsStopInspection) {
                        const guardStop = await this.#handleStopHookLoop(ac, promptId, false, undefined, false, undefined, channelDeliveryCapture);
    ...
    #registerBackgroundNotificationCallbacks() {
        ...
        if (typeof this.config.onChatRecordingFailure === 'function') {
            this.unsubscribeChatRecordingFailure = this.config.onChatRecordingFailure((event) => this.client.extNotification('qwen/notify/session/recording-degraded', {
                v: 1,
    ...
    async runToolCalls(abortSignal, promptId, functionCalls, toolLoopState, onFullTurnModel) {
        ...
        const batches = [];
        const handledProviderToolCallIds = new Set(this.#getCurrentChat().getHistoryFunctionResponseIds());
        const repeatedDuplicateCall = findRepeatedDuplicateProviderToolCall(dedupedFunctionCalls, (fc) => getProviderToolCallId(fc) ?? fc.id, handledProviderToolCallIds, this.duplicateProviderToolCallResponseIds);
        if (repeatedDuplicateCall) {
            const providerCallId = getProviderToolCallId(repeatedDuplicateCall) ??
                repeatedDuplicateCall.id;
            debugLogger.debug(`[Session.runToolCalls] Dropping batch after repeated duplicate provider tool-call id: ` +
                ...
                try {
                    ...
                    if (!autoModeAllowed &&
                        ...
                        if (!requiresUserInteraction &&
                            ...
                            if (hooksEnabled && messageBus) {
                                this.fireNotificationHookWithTerminalSequence(messageBus, `Qwen Code needs your permission to use ${toolName}`, NotificationType.PermissionPrompt, 'Permission needed');
                            }
                    ...
                    const heartbeatSpanAttributes = () => heartbeatCount > 0
                        ...
                        : undefined;
                    const sleepInhibitorHandle = acquireSleepInhibitor(this.config, `Qwen Code is executing tool ${toolName}`);
                    try {
                        try {
    ...
    options = {}) {
        ...
        const referenceParts = [...extensionParts, ...mcpServerParts];
    ...
    fireNotificationHookWithTerminalSequence(messageBus, message, notificationType, title) {
        void fireNotificationHook(messageBus, message, notificationType, title)
            ...
            if (!hookResult.terminalSequence)
                return;
            return this.client.extNotification('qwen/notify/session/terminal-sequence', {
                v: 1,
                sessionId: this.sessionId,
