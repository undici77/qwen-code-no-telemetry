 ...
 */
import { APPROVAL_MODE_INFO, APPROVAL_MODES, AuthType, clearCachedCredentialFile, createDebugLogger, QwenOAuth2Event, qwenOAuth2Events, MCP_BUDGET_WARN_FRACTION, MCPServerConfig, SessionService, SESSION_TITLE_MAX_LENGTH, tokenLimit, getMCPDiscoveryState, getMCPServerStatus, MCPDiscoveryState, MCPServerStatus, SessionEndReason, restoreWorktreeContext, } from '@qwen-code/qwen-code-core';
...
import { formatAcpModelId, parseAcpBaseModelId, } from '../utils/acpModelUtils.js';
import { runWithAcpRuntimeOutputDir } from './runtimeOutputDirContext.js';
import { runExitCleanup } from '../utils/cleanup.js';
import { ACP_PREFLIGHT_KINDS, STATUS_SCHEMA_VERSION, SERVE_CONTROL_EXT_METHODS, SERVE_STATUS_EXT_METHODS, mapDomainErrorToErrorKind, } from '../serve/status.js';
...
class QwenAgent {
    ...
    async loadSession(params) {
        ...
        params.mcpServers ?? [], params.sessionId, true);
        await this.ensureAuthenticated(config);
        this.setupFileSystem(config);
        const sessionData = config.getResumedSessionData();
        const session = await this.createAndStoreSession(config, sessionData?.conversation);
    ...
    async unstable_resumeSession(params) {
        ...
        const config = await this.newSessionConfig(params.cwd, params.mcpServers ?? [], params.sessionId, true);
        await this.ensureAuthenticated(config);
        this.setupFileSystem(config);
        const session = await this.createAndStoreSession(config);
        await this.#restoreWorktreeOnResume(config, session);
            ...
            return {
                ...
                ...(budgetMode !== undefined
                    ? {
                        ...
                        budgets: this.buildBudgetCells(clientCount ?? 0, clientBudget, budgetMode, Array.from(refusedSet).filter((n) => !config.isMcpServerDisabled(n)).length),
                    }
                    : {}),
            };
        }
     ...
     */
    buildBudgetCells(liveCount, budget, mode, refusedCount) {
    ...
    buildWorkspaceProvidersStatus(config) {
        try {
            ...
            const currentAcpModelId = hasCurrentModel && currentAuth
            ...
            for (const model of config.getAllConfiguredModels()) {
                ...
                const isCurrent = currentAuth === model.authType &&
                    hasCurrentModel &&
    ...
    async extMethod(method, params) {
        ...
        switch (method) {
            ...
            case 'rewindSession': {
                ...
                if (!Number.isInteger(targetTurnIndex) ||
                    targetTurnIndex < 0) {
                ...
                return {
                    success: true,
                    historyBeforeRewind,
                    ...session.rewindToTurn(targetTurnIndex),
                };
            }
    ...
    async newSessionConfig(cwd, mcpServers, sessionId, resume) {
        ...
        const wiredSessionId = typeof config.getSessionId === 'function'
            ? config.getSessionId()
            : undefined;
        if (typeof config.setMcpBudgetEventCallback === 'function' &&
    ...
    async ensureAuthenticated(config) {
        ...
        if (!selectedType) {
            throw RequestError.authRequired({ authMethods: this.pickAuthMethodsForAuthRequired() }, 'Use Qwen Code CLI to authenticate first.');
        }
        try {
    ...
    pickAuthMethodsForAuthRequired(selectedType, error) {
        ...
        if (errorMessage?.includes('qwen-oauth') ||
            errorMessage?.includes('Qwen OAuth')) {
            const qwenOAuthMethods = authMethods.filter((m) => m.id === AuthType.QWEN_OAUTH);
