 ...
 */
import { APPROVAL_MODE_INFO, APPROVAL_MODES, AuthType, BTW_MAX_INPUT_LENGTH, buildBtwCacheSafeParams, buildBtwPrompt, ALL_PROVIDERS, applyProviderInstallPlan, buildInstallPlan, clearCachedCredentialFile, createDebugLogger, generateSessionRecap, findProviderById, getAllGeminiMdFilenames, getAutoMemoryRoot, getUserAutoMemoryRoot, getDefaultBaseUrlForProtocol, getDefaultModelIds, getScopedEnvContents, QwenOAuth2Event, qwenOAuth2Events, resolveBaseUrl, MCP_BUDGET_WARN_FRACTION, MCPServerConfig, runForkedAgent, SessionService, SESSION_WRITER_RPC_CODES, SessionWriterUnavailableError, SESSION_TITLE_MAX_LENGTH, Storage, tokenLimit, getMCPDiscoveryState, getMCPServerStatus, initializeTelemetry, preloadContentGenerator, MCPDiscoveryState, MCPServerStatus, McpTransportPool, POOLED_TRANSPORTS_DEFAULT, INVOCATION_CONTEXT_META_KEY, PRIVATE_ACP_CAPABILITY_ENV, PRIVATE_PARENT_CAPABILITY_META_KEY, parseInvocationContext, findExistingProviderModels, ExtensionManager, ExtensionSettingScope, HookEventName, updateSetting, SessionEndReason, WorkspaceMcpBudget, DiscoveredMCPTool, restoreWorktreeContext, uiTelemetryService, McpBudgetWouldExceedError, McpServerSpawnFailedError, InvalidMcpConfigError, isGatedMcpScope, MCPOAuthProvider, MCPOAuthTokenStorage, InvalidSessionTranscriptCursorError, SESSION_TRANSCRIPT_MAX_LIMIT, SESSION_TRANSCRIPT_MAX_PAGE_BYTES, SessionTranscriptReader, SessionTranscriptPageTooLargeError, SessionTranscriptSnapshotUnavailableError, SessionTranscriptTooLargeError, encodeSessionTranscriptCursor, findBoundaryAtOrBefore, isReplayTurnStartType, subagentGenerator, redactUrlCredentials, computeUniqueBranchTitle, ToolNames, FORK_SUBAGENT_TYPE, runManagedAutoMemoryDream, runManagedRememberByAgent, matchesAnyServerPattern, mcpServerRequiresOAuth, IMAGE_CAPABILITY, registerAcpEventLoopLagGauge, SESSION_ARTIFACT_PERSISTENCE_VERSION, normalizeEventPayload, normalizeSnapshotPayload, startEventLoopLagMonitor, refreshMemoryInstruction, applyReasoningEffort, REASONING_EFFORT_TIERS, extractDaemonTraceContext, withDaemonSpan, emptyGoalSnapshot, GoalPersistenceUnavailableError, ApprovalMode, } from '@qwen-code/qwen-code-core';
...
import { collectHistoryReplayUpdates, copyCumulativeUsage, createReplayCumulativeUsage, replayTranscriptRecordPage, } from './session/history-replay-page.js';
...
import { ACP_PREFLIGHT_KINDS, STATUS_SCHEMA_VERSION, SERVE_CONTROL_EXT_METHODS, SERVE_STATUS_EXT_METHODS, mapDomainErrorToErrorKind, IDLE_HOOK_EVENTS, } from '@qwen-code/acp-bridge/status';
import { EXTERNAL_TOOL_GUARD_READY_META_KEY, EXTERNAL_TOOL_GUARD_REQUIRED_VALUE, EXTERNAL_TOOL_GUARD_TOKEN_ENV, isValidExternalToolGuardDenialReason, PRIVATE_EXTERNAL_TOOL_GUARD_ENV, } from '@qwen-code/acp-bridge/externalToolGuard';
import { parseSessionSource, SESSION_SOURCE_META_KEY, } from '@qwen-code/acp-bridge';
import { ACTIVE_WORK_CLOSE_IF_UNHELD_PARAM, ACTIVE_WORK_HEARTBEAT_META_KEY, ACTIVE_WORK_HEARTBEAT_VERSION, ACTIVE_WORK_HOLD_CATEGORIES, clampActiveWorkIntervalMs, CHANNEL_STARTUP_PROFILE_META_KEY, CHANNEL_STARTUP_PROFILE_VERSION, CLIENT_MCP_OVER_WS_CONFIG_FLAG, DAEMON_MODEL_PROMPT_META_KEY, DAEMON_PROMPT_DISPLAY_TEXT_META_KEY, LOAD_REPLAY_BULK_MODE, LOAD_REPLAY_HIDE_INHERITED_META_KEY, LOAD_REPLAY_META_KEY, LOAD_REPLAY_MODE_META_KEY, LOAD_REPLAY_PAGE_SIZE_META_KEY, LOAD_REPLAY_VERSION, PROMPT_CANCEL_METHOD, REQUESTED_SESSION_ID_META_KEY, TODO_STOP_GUARD_QUEUE_RELEASE_METHOD, isValidTrustedModelPrompt, WORKTREE_MCP_DEFER_META_KEY, } from '@qwen-code/acp-bridge/bridgeTypes';
...
function createAcpSessionProfiler(span, attributePrefix) {
    ...
    const recordStage = (stage, start) => {
        ...
        if (Number.isFinite(durationMs) && durationMs >= 0) {
            setAttribute(`qwen-code.daemon.${attributePrefix}.${stage}_ms`, durationMs);
        }
    };
...
const SESSION_WRITER_MESSAGES = {
    session_writer_conflict: 'This session is already open in another Qwen process.',
...
async function waitForSessionDrain(operation, timeoutMs, kind) {
    let timer;
    const timeout = new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error(`Session ${kind} timed out after ${timeoutMs}ms`)), timeoutMs);
        timer.unref();
    });
    try {
...
function isHistoryPageStart(record) {
    return (record.subtype !== 'realtime_message' &&
...
function historyOrphansToolResult(records, start, end) {
    for (let i = start; i < end; i++) {
        if (records[i].type !== 'tool_result')
            continue;
...
function selectRecentHistoryRecords(records, pageSize) {
    ...
    const aligned = findBoundaryAtOrBefore(records, start, expansionFloor, isHistoryTurnStart);
    ...
    if (start > 0 && historyOrphansToolResult(records, start, records.length)) {
        ...
        const owner = findBoundaryAtOrBefore(records, start, pairFloor, isHistoryPageStart);
        if (isHistoryPageStart(records[owner])) {
            start = owner;
        }
    }
...
const PROJECT_SKILL_DIRS = ['.qwen', '.agents'];
 ...
 */
export function createManagedExternalToolGuard(connection) {
    return async (context) => {
        ...
        if (context.toolName === ToolNames.AGENT ||
    ...
    try {
        ...
        connection = new AgentSideConnection((conn) => {
            ...
            const managedToolInvocationGuard = externalToolGuardRequired
                ...
                : undefined;
            agentInstance = new QwenAgent(config, settings, argv, conn, privateParentCapability, sessionWriterLeaseEnabledAtStartup, managedToolInvocationGuard);
    ...
    const shutdownManagedAgent = (reason, label) => {
        ...
        managedShutdownPromise = (async () => {
            ...
            try {
                ...
                writeStderrLineSafe('qwen --acp: managed session writer shutdown failed; a writer lock may be retained for safety. ' +
...
function parsePoolDrainMs(envValue) {
    ...
    if (!/^\d+$/.test(trimmed)) {
        process.stderr.write(`qwen serve: QWEN_SERVE_MCP_POOL_DRAIN_MS=${JSON.stringify(envValue)} ` +
...
class QwenAgent {
    ...
    async withLiveSessionRestore(sessionId, session, operation) {
    ...
    async cleanupAfterRequestFailure(error, cleanup) {
        try {
            await cleanup();
        }
        catch (cleanupError) {
    ...
    constructor(config, settings, argv, connection, expectedPrivateParentCapability, sessionWriterLeaseEnabledAtStartup = false, managedToolInvocationGuard) {
        this.config = config;
    ...
    async initialize(args) {
        ...
        if (activeWorkIntervalMs !== undefined) {
            this.activeWorkReporter?.dispose();
            this.activeWorkReporter = new ActiveWorkReporter((method, params) => this.connection.extNotification(method, params), () => this.sessions.values(), activeWorkIntervalMs);
        }
        const responseMeta = {
    ...
    async newSession(params) {
        ...
        try {
            ...
            return await withDaemonSpan('qwen-code.daemon.session_start', { 'qwen-code.daemon.operation': 'acp_session_new' }, async (span) => {
    ...
    async loadSession(params) {
        ...
        const parentContext = extractDaemonTraceContext(params);
        return await withDaemonSpan('qwen-code.daemon.session_restore', {
    ...
    async loadSessionWithProfiler(params, initialSessionId, profiler) {
        ...
        if (liveSession) {
            ...
            return profiler.time('live_restore', async () => {
                ...
                return this.withLiveSessionRestore(sessionId, liveSession, async (config, sessionData) => {
                    ...
                    const visibleRecords = selectVisibleHistoryRecords(records, shouldHideInheritedHistory(params));
                    if (visibleRecords.length === 0)
                        return response;
                    const bulkReplay = isBulkLoadReplayRequest(params);
                    const replayPage = bulkReplay
                        ? selectRecentHistoryRecords(visibleRecords, getLoadReplayPageSize(params))
        ...
        try {
            ...
            params.mcpServers ?? [], settings, sessionSource, sessionId, true));
            ...
            try {
                await profiler.time('auth', () => this.ensureAuthenticated(config));
                profiler.timeSync('file_system_setup', () => this.setupFileSystem(config));
                await profiler.time('session_register', () => this.createAndStoreSession(config, settings, sessionData, {
                    ...
                    beforeStartPostReplayServices: async (createdSession) => {
                        if (bulkReplay) {
                            ...
                            if (records) {
                                ...
                                const visibleRecords = selectVisibleHistoryRecords(records, shouldHideInheritedHistory(params));
                                const replayPage = selectRecentHistoryRecords(visibleRecords, replayPageSize);
                                ...
                                // open.
                                try {
                                    replayUpdates = replayUpdates.concat(await createdSession.renderRecoveredGoalUpdates(records));
                                }
                                catch (error) {
    ...
    async resumeSessionWithProfiler(params, initialSessionId, profiler) {
        ...
        if (liveSession) {
            ...
            return profiler.time('live_restore', async () => {
                await this.assertLiveSessionScope(liveConfig, settings, params.cwd);
                return this.withLiveSessionRestore(sessionId, liveSession, async (config, sessionData) => profiler.timeSync('response_build', () => ({
                    modes: this.buildModesData(config),
                    models: this.buildAvailableModels(config),
                    configOptions: this.buildConfigOptions(config),
                    ...(sessionData.artifactSnapshot
        ...
        try {
            ...
            profiler.setSessionId(sessionId);
            this.settings = settings;
            const config = await profiler.time('config_setup', () => this.newSessionConfig(params.cwd, params.mcpServers ?? [], settings, sessionSource, sessionId, true));
            try {
                await profiler.time('auth', () => this.ensureAuthenticated(config));
                profiler.timeSync('file_system_setup', () => this.setupFileSystem(config));
                await profiler.time('session_register', () => this.createAndStoreSession(config, settings, config.getResumedSessionData(), {
                    enableLiveScreenContext: isCompatibleLiveSessionSource(sessionSource ?? {}),
                    replayHistory: false,
                    beforeStartPostReplayServices: async (createdSession) => {
                        ...
                        try {
                            await createdSession.publishRecoveredGoalState(config.getResumedSessionData()?.conversation.messages);
                        }
                        catch (error) {
        ...
        switch (configId) {
            ...
            case 'reasoning_effort': {
                ...
                if (modelReasoning) {
                    ...
                    if (!selected) {
                        throw RequestError.invalidParams(undefined, `Unknown reasoning effort: ${value}. Choose one of: ${ACP_REASONING_EFFORT_NONE}, ${modelReasoning.efforts.join(', ')}`);
                    }
    ...
    async buildWorkspaceMcpStatus(config) {
        try {
            ...
            try {
                ...
                process.stderr.write(`qwen serve: pool snapshot for workspace MCP status failed: ` +
                    `${err instanceof Error ? err.message : String(err)}\n`);
            }
            ...
            if (wsBudget !== undefined) {
                ...
                try {
                    ...
                    process.stderr.write(`qwen serve: getMcpClientAccounting failed: ` +
                        `${err instanceof Error ? err.message : String(err)}\n`);
                }
            }
        ...
        switch (method) {
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart: {
                ...
                try {
                    ...
                    if (mergedDisabled !== undefined && !Array.isArray(mergedDisabled)) {
                        process.stderr.write(`qwen serve: MCP restart for ${JSON.stringify(serverName)}: ` +
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceGenerationStart: {
                ...
                try {
                    const emit = async (event) => {
                        await this.connection.extNotification('qwen/notify/workspace/generation/event', { v: 1, requestId, event });
                    };
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionGenerationStart: {
                ...
                try {
                    const result = await executeGeneration(session.getConfig(), requestId, prompt, signal, async (event) => {
                        await this.connection.extNotification('qwen/notify/session/generation/event', { v: 1, sessionId, requestId, event });
                    });
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionSideTask: {
                ...
                try {
                    ...
                    const renamed = await sessionService.renameSession(newSessionId, title, 'manual');
                    if (!renamed) {
                        throw new RequestError(-32603, `Failed to set title on forked session ${newSessionId}`, { errorKind: 'internal', sessionId: newSessionId });
                    }
                }
                catch (err) {
                    sessionService.removeSession(newSessionId).catch((rmErr) => {
                        process.stderr.write(`qwen serve: failed to clean up orphan session ${newSessionId}: ${rmErr instanceof Error ? rmErr.message : rmErr}\n`);
                    });
                    throw err;
                }
            ...
            case 'qwen/settings/setCoreValue': {
                ...
                if (typeof key !== 'string' ||
                    ...
                    throw RequestError.invalidParams(undefined, 'Unsupported Qwen setting key');
                }
                const settings = loadSettings(cwd);
    ...
    async newSessionConfig(cwd, mcpServers, settings, sessionSource, sessionId, resume, initializeOptions = {}, chatRecording) {
        try {
            ...
            return await this.runWithPinnedRuntimeBaseDir(settings, cwd, async () => {
                ...
                return this.newSessionConfigInRuntimeContext(cwd, mcpServers, settings, sessionSource, sessionId, resume, initializeOptions, chatRecording);
            });
        }
        catch (error) {
    ...
    async newSessionConfigInRuntimeContext(cwd, mcpServers, settings, sessionSource, sessionId, resume, initializeOptions = {}, chatRecording) {
        ...
        true, this.managedToolInvocationGuard
    ...
    async ensureAuthenticated(config) {
        ...
        if (!selectedType) {
            throw RequestError.authRequired({ authMethods: pickAuthMethodsForAuthRequired() }, 'Use Qwen Code CLI to authenticate first.');
        }
        try {
    ...
    async createAndStoreSession(config, settings, sessionData, options = {}) {
        ...
        const session = new Session(sessionId, config, this.connection, settings, () => this.activeWorkReporter?.notifyChanged());
    ...
    buildSelectableModelOptions(config) {
        ...
        return buildAcpModelOptions(config
            .getAllConfiguredModels()
            .filter((model) => model.authType !== AuthType.QWEN_OAUTH ||
