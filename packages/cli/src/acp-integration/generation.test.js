...
function createConfig(fastModel) {
    const generateContentStream = vi.fn(async function* (_request, _promptId) {
        yield {
    ...
    const resolveForModel = vi.fn(async (model) => ({
...
describe('executeGeneration', () => {
    it('streams a stateless, tool-free request through the fast model', async () => {
        const { config, generateContentStream, resolveForModel } = createConfig('fast-model');
        const events = [];
        const result = await executeGeneration(config, 'request-1', 'Translate this', new AbortController().signal, async (event) => {
            events.push(event);
        });
        ...
        expect(generateContentStream.mock.calls[0]?.[0].config).not.toHaveProperty('maxOutputTokens');
    ...
    it('uses the main model when no valid fast model is available', async () => {
        ...
        const result = await executeGeneration(config, 'request-2', 'Summarize this', new AbortController().signal, async () => undefined);
    ...
    it('falls back to the main model when the fast model cannot be resolved', async () => {
        ...
        const result = await executeGeneration(config, 'request-3', 'Rewrite this', new AbortController().signal, async () => undefined);
        ...
        expect(result).toEqual(expect.objectContaining({ model: 'main-model', modelSource: 'main' }));
    });
});
