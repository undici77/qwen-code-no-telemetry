...
vi.mock('./session/history-replayer.js', () => ({
    HistoryReplayer: vi.fn().mockImplementation((context) => ({
        replay: (messages, gaps) => mockHistoryReplay(context, messages, gaps),
...
import { AuthType, SessionEndReason, MCPServerConfig, SessionService, MCPDiscoveryState, MCPServerStatus, getMCPDiscoveryState, getMCPServerStatus, initializeTelemetry, tokenLimit, McpBudgetWouldExceedError, buildInstallPlan, applyProviderInstallPlan, Storage, SessionTranscriptReader, InvalidSessionTranscriptCursorError, SessionTranscriptSnapshotUnavailableError, SessionTranscriptTooLargeError, SessionTranscriptPageTooLargeError, encodeSessionTranscriptCursor, unregisterGoalHook, getActiveGoal, registerGoalHook, startEventLoopLagMonitor, registerAcpEventLoopLagGauge, SESSION_ARTIFACT_PERSISTENCE_VERSION, mcpServerRequiresOAuth, APPROVAL_MODES, ToolNames, GoalPersistenceUnavailableError, } from '@qwen-code/qwen-code-core';
...
import { SERVE_STATUS_EXT_METHODS, SERVE_CONTROL_EXT_METHODS, } from '@qwen-code/acp-bridge/status';
import { EXTERNAL_TOOL_GUARD_READY_META_KEY, EXTERNAL_TOOL_GUARD_REQUIRED_VALUE, } from '@qwen-code/acp-bridge/externalToolGuard';
...
describe('QwenAgent MCP SSE/HTTP support', () => {
    ...
    it('profiles newSession stages under the daemon trace context', async () => {
        ...
        expect(mockWithDaemonSpan).toHaveBeenCalledWith('qwen-code.daemon.session_start', { 'qwen-code.daemon.operation': 'acp_session_new' }, expect.any(Function), { parentContext });
        ...
        for (const stage of [
            ...
            expect(attributes[`qwen-code.daemon.session_start.${stage}_ms`]).toEqual(expect.any(Number));
        }
    ...
    it('records the failed newSession stage without changing the error', async () => {
        ...
        expect(mockSessionStartSpan.setAttribute).toHaveBeenCalledWith('qwen-code.daemon.session_start.failed_stage', 'config_setup');
        mockConnectionState.resolve();
        await agentPromise;
    });
    it('records a failed synchronous newSession stage', async () => {
        ...
        try {
            ...
            expect(mockSessionStartSpan.setAttribute).toHaveBeenCalledWith('qwen-code.daemon.session_start.failed_stage', 'file_system_setup');
        }
        finally {
            mockConnectionState.resolve();
            await agentPromise;
    ...
    it('extMethod preflight surfaces SkillError as parse_error errorKind', async () => {
        const skillError = new (await import('@qwen-code/qwen-code-core')).SkillError('bad frontmatter', 'PARSE_ERROR');
        mockConfig = {
            ...mockConfig,
            getTargetDir: vi.fn().mockReturnValue('/work/status'),
            getMcpServers: vi.fn().mockReturnValue({}),
    ...
    it('rejects agent-backed workspace memory operations when the managed guard is required', async () => {
        Object.assign(mockConfig, {
            isManagedMemoryAvailable: vi.fn().mockReturnValue(true),
            getProjectRoot: vi.fn().mockReturnValue('/workspace'),
        });
    ...
    it('rejects /fork before starting a nested agent when the managed guard is required', async () => {
        const sessionId = '11111111-1111-1111-1111-111111111111';
        const innerConfig = await setupSessionMocks(sessionId);
        const execute = vi.fn();
        const build = vi.fn().mockReturnValue({ execute });
...
describe('QwenAgent extMethod renameSession routing', () => {
    ...
    it('branches a live session through its pinned SessionService', async () => {
        ...
        expect(sessionService.renameSession).toHaveBeenCalledWith(expect.any(String), 'Source session (Branch)', 'manual');
        expect(result).toMatchObject({
    ...
    it('creates a side task with source metadata and no branch suffix', async () => {
        ...
        expect(sessionService.renameSession).toHaveBeenCalledWith(newSessionId, 'Side task', 'manual');
        expect(result).toMatchObject({
            title: 'Side task',
...
describe('QwenAgent loadSession / unstable_resumeSession', () => {
    ...
    async function spawnAgent() {
        const agentPromise = runAcpAgent(mockConfig, makeRestoreSettings(), mockArgv);
        await vi.waitFor(() => expect(capturedAgentFactory).toBeDefined());
        const agent = capturedAgentFactory({
            get closed() {
                return mockConnectionState.promise;
    ...
    it('loadSession throws resourceNotFound when the persisted session is missing', async () => {
        ...
        expect(mockSessionStartSpan.setAttribute).toHaveBeenCalledWith('qwen-code.daemon.session_restore.failed_stage', 'existence_check');
        mockConnectionState.resolve();
        await agentPromise;
    });
    it.each(['load', 'resume'])('profiles %s restore stages under the daemon trace context', async (action) => {
        ...
        expect(mockExtractDaemonTraceContext).toHaveBeenCalledWith(request);
        expect(mockWithDaemonSpan).toHaveBeenCalledWith('qwen-code.daemon.session_restore', {
        ...
        for (const stage of [
            ...
            expect(attributes[`qwen-code.daemon.session_restore.${stage}_ms`]).toEqual(expect.any(Number));
        }
        ...
        expect(attributes['qwen-code.daemon.session_restore.live_restore_ms']).toBeUndefined();
        mockConnectionState.resolve();
        await agentPromise;
    });
    it.each(['load', 'resume'])('profiles the live %s restore path without an existence check', async (action) => {
        ...
        try {
            ...
            expect(mockWithDaemonSpan).toHaveBeenCalledWith('qwen-code.daemon.session_restore', expect.objectContaining({
                'qwen-code.daemon.session_restore.action': action,
            }), expect.any(Function), {});
            ...
            const attributes = Object.fromEntries(mockSessionStartSpan.setAttribute.mock.calls);
            expect(attributes['qwen-code.daemon.session_restore.settings_load_ms']).toEqual(expect.any(Number));
            expect(attributes['qwen-code.daemon.session_restore.live_restore_ms']).toEqual(expect.any(Number));
            expect(attributes['qwen-code.daemon.session_restore.response_build_ms']).toEqual(expect.any(Number));
            expect(attributes['qwen-code.daemon.session_restore.existence_check_ms']).toBeUndefined();
        }
        finally {
            mockConnectionState.resolve();
            await agentPromise;
    ...
    it('serializes non-live load and resume before settings or disk work', async () => {
        ...
        const loadSession = vi
            .fn()
            .mockImplementation(() => innerConfig.getResumedSessionData());
        vi.mocked(SessionService).mockImplementation(() => ({ sessionExists, loadSession }));
        const { agent, agentPromise } = await spawnAgent();
        vi.mocked(loadSettings).mockClear();
        const params = {
            cwd: '/tmp',
    ...
    it('loadSession preserves initialization failure and retries deferred cleanup', async () => {
        ...
        expect(innerConfig.shutdown).toHaveBeenCalledOnce();
        expect(mockSessionStartSpan.setAttribute).toHaveBeenCalledWith('qwen-code.daemon.session_restore.failed_stage', 'config_setup');
    ...
    it('loadSession returns LoadSessionResponse and replays history on the session', async () => {
        ...
        expect(lastSessionMock?.replayHistory).toHaveBeenCalledWith(messages, undefined);
    ...
    it('loadSession can return bulk replay updates without streaming history', async () => {
        ...
        expect(lastSessionMock?.primeTurnFromHistory).toHaveBeenCalledWith(messages);
        expect(mockHistoryReplay).toHaveBeenCalledTimes(1);
        expect(lastSessionMock.primeTurnFromHistory.mock.invocationCallOrder[0]).toBeLessThan(mockHistoryReplay.mock.invocationCallOrder[0]);
        expect(mockHistoryReplay.mock.invocationCallOrder[0]).toBeLessThan(innerConfig.loadPausedBackgroundAgents.mock.invocationCallOrder[0]);
        expect(innerConfig.loadPausedBackgroundAgents.mock.invocationCallOrder[0]).toBeLessThan(lastSessionMock.installRewriter.mock.invocationCallOrder[0]);
        expect(lastSessionMock.installRewriter.mock.invocationCallOrder[0]).toBeLessThan(lastSessionMock.startCronScheduler.mock.invocationCallOrder[0]);
        mockConnectionState.resolve();
        await agentPromise;
    ...
    it('appends the recovered Goal card after the bulk replay page', async () => {
        ...
        expect(lastSessionMock?.renderRecoveredGoalUpdates).toHaveBeenCalledWith(messages);
    ...
    it('publishes the recovered Goal state on unstable_resumeSession', async () => {
        ...
        expect(lastSessionMock?.publishRecoveredGoalState).toHaveBeenCalledWith(messages);
        expect(lastSessionMock?.replayHistory).not.toHaveBeenCalled();
        mockConnectionState.resolve();
        await agentPromise;
    });
...
describe('createManagedExternalToolGuard', () => {
    ...
    ])('rejects unsupported nested executor %s without contacting the provider', async (toolName) => {
        const extMethod = vi.fn();
        const guard = createManagedExternalToolGuard({
            extMethod,
        });
