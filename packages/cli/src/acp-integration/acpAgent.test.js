...
vi.mock('../utils/acpModelUtils.js', () => ({
    formatAcpModelId: vi.fn((modelId, authType) => `${modelId}(${authType})`),
    parseAcpBaseModelId: vi.fn((modelId) => modelId.replace(/\([^)]+\)$/, '')),
}));
import { runAcpAgent, toStdioServer, toSseServer, toHttpServer, } from './acpAgent.js';
import { SessionEndReason, MCPServerConfig, SessionService, MCPDiscoveryState, MCPServerStatus, getMCPDiscoveryState, getMCPServerStatus, tokenLimit, } from '@qwen-code/qwen-code-core';
