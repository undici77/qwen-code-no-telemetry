...
import { RunBudgetEnforcer, parseDurationSeconds, validateMaxToolCalls, validateMaxWallTimeSetting, } from './runBudget.js';
describe('parseDurationSeconds', () => {
    ...
    it.each(['', 'abc', '10x', '-5', '5 m s', 'NaN', '0', '0s', '0ms'])('rejects invalid / non-positive input %s', (input) => {
...
describe('validateMaxWallTimeSetting', () => {
    ...
    it('rejects Infinity and NaN', () => {
        expect(() => validateMaxWallTimeSetting(Number.POSITIVE_INFINITY)).toThrow();
...
describe('RunBudgetEnforcer', () => {
    ...
    it('first-fence-wins: a later overrun does not clobber the original reason', () => {
        const ac = new AbortController();
        const enforcer = new RunBudgetEnforcer({ maxToolCalls: 0, maxWallTimeSeconds: 1 }, ac);
        enforcer.start();
        enforcer.tickToolCall();
...
//# sourceMappingURL=runBudget.test.js.map