...
import { FatalSandboxError, Storage, isSubpath, resolveBundleDir, } from '@qwen-code/qwen-code-core';
...
export async function start_sandbox(config, nodeArgs = [], cliConfig, cliArgs = [], childEnv) {
    ...
    if (process.env['BUILD_SANDBOX']) {
        if (!gcPath.includes('qwen-code/packages/')) {
            throw new FatalSandboxError('Cannot build sandbox using installed Qwen Code binary; ' +
