...
describe('memoryDiagnostics', () => {
    ...
    it('writes heap snapshots to a diagnostics directory with stable filenames', () => {
        ...
        expect(writtenPath).toBe(path.join(outputDir, `qwen-code-heap-${process.pid}-2026-05-15T12-00-00-000Z.heapsnapshot`));
    });
    ...
    it('keeps only the newest heap snapshots after writing', () => {
        ...
        const oldSnapshot = path.join(outputDir, `qwen-code-heap-${process.pid}-2026-05-15T11-00-00-000Z.heapsnapshot`);
        const newerSnapshot = path.join(outputDir, `qwen-code-heap-${process.pid}-2026-05-15T11-30-00-000Z.heapsnapshot`);
    ...
    it('orders heap snapshot cleanup by modification time across process ids', () => {
        ...
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const newerLowPidSnapshot = path.join(outputDir, 'qwen-code-heap-9-2026-05-15T12-30-00-000Z.heapsnapshot');
        const olderHighPidSnapshot = path.join(outputDir, 'qwen-code-heap-12345-2026-05-15T12-00-00-000Z.heapsnapshot');
    ...
    it('uses filename timestamps to break equal-mtime cleanup ties', () => {
        ...
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const olderSnapshot = path.join(outputDir, 'qwen-code-heap-1-2026-05-15T11-00-00-000Z.heapsnapshot');
        const newerSnapshot = path.join(outputDir, 'qwen-code-heap-1-2026-05-15T12-00-00-000Z.heapsnapshot');
    ...
    it('keeps successful heap snapshot writes when cleanup fails', () => {
        ...
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const brokenSymlink = path.join(outputDir, 'qwen-code-heap-999-2026-05-15T11-00-00-000Z.heapsnapshot');
