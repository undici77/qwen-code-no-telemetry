...
import { updateSettingsFilePreservingFormat, applyUpdates, parseJsoncObject, updateJsoncContent, } from './jsonc-editor.js';
...
\t"model": "qwen"
  ...
  "prototype": "value"
    ...
    it.each(['{ "model": "qwen"', '[]', '"hello"', '42', 'true', 'null'])('rejects malformed or non-object JSONC: %s', (content) => {
...
describe('applyUpdates', () => {
    ...
    it('should ignore prototype-pollution keys in updates', () => {
        const original = {};
        const updates = JSON.parse('{"safe":true,"__proto__":{"polluted":true},"constructor":{"prototype":{"polluted":true}},"nested":{"prototype":{"polluted":true},"keep":1}}');
        const result = applyUpdates(original, updates);
        expect(result).toEqual({
            safe: true,
            nested: {
...
describe('migration write-back via updateSettingsFilePreservingFormat', () => {
    ...
    let testFilePath;
    beforeEach(() => {
        tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'migration-writeback-test-'));
        testFilePath = path.join(tempDir, 'settings.json');
    });
    afterEach(() => {
        if (fs.existsSync(tempDir)) {
  ...
  "$version": 2
        ...
        const result = updateSettingsFilePreservingFormat(testFilePath, migratedSettings, true);
...
//# sourceMappingURL=jsonc-editor.test.js.map