...
import { RELAUNCH_EXIT_CODE, UPDATE_ON_EXIT_MESSAGE, UPDATE_RELAUNCH_EXIT_CODE, } from './processUtils.js';
import { writeStderrLine } from './stdioHelpers.js';
export async function relaunchOnExitCode(runner, options) {
    while (true) {
        try {
...
export async function relaunchAppInChildProcess(additionalNodeArgs, additionalScriptArgs, options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            child.on('close', (code) => {
                ...
                if (exitCode === 0 &&
                    ...
                    void Promise.resolve(options.onUpdateRelaunch(false)).then((updatedExitCode) => resolve(updatedExitCode), reject);
                    return;
                }
                resolve(exitCode);
            });
