...
const OSS_BASE = 'https://qwen-code-assets.oss-cn-hangzhou.aliyuncs.com/releases/qwen-code';
...
function atomicReplace(standaloneDir, newDir, lockPath) {
    ...
    if (os.platform() === 'win32') {
        ...
        const scriptPath = path.join(path.dirname(standaloneDir), 'qwen-update.bat');
...
export function ensureBinWrapper(standaloneDir, target) {
    ...
    try {
        ...
        if (target.startsWith('win')) {
            ...
            const quotedQwenBin = shellQuoteForSh(path.join(standaloneDir, 'bin', 'qwen'));
...
export async function performStandaloneUpdate(standaloneDir, newVersion) {
    ...
    if (fs.existsSync(manifestPath)) {
        ...
        throw new Error(`${standaloneDir} exists but is not a Qwen Code standalone install. Remove it manually to proceed.`);
    }
    else {
    ...
    try {
        ...
        if (!fs.existsSync(path.join(newInstallDir, 'manifest.json'))) {
            throw new Error('Extracted archive does not contain expected qwen-code directory');
        }
        ...
        if (fs.existsSync(oldDir)) {
            if (isFirstTimeMigration) {
                ...
                try {
                    ...
                    fs.writeFileSync(path.join(oldDir, '.qwen-rollback-info.json'), JSON.stringify(rollbackInfo, null, 2));
                }
                catch {
