...
import { jsonStringJsonByteLength, projectJsonStringToByteBudget, } from './json-string-byte-projection.js';
const BUDGET = 65_536;
...
describe('JSON string byte projection', () => {
    ...
    it.each([65_535, 65_536, 65_537])('enforces the JSON string boundary at %i bytes', (targetBytes) => {
    ...
    ])('projects oversized %s strings within the exact budget', (_name, value) => {
        const projected = projectJsonStringToByteBudget(value, BUDGET, MARKER);
        expect(jsonBytes(projected)).toBeLessThanOrEqual(BUDGET);
    ...
    it('keeps an approximately 20/80 head and tail preview', () => {
        const value = 'HEAD-' + 'h'.repeat(200_000) + '-' + 't'.repeat(200_000) + '-TAIL';
        const projected = projectJsonStringToByteBudget(value, BUDGET, MARKER);
    ...
    it.each(['-tail', '-tail!!'])('never splits valid surrogate pairs with suffix alignment %s', (suffix) => {
...
//# sourceMappingURL=json-string-byte-projection.test.js.map