...
export function parseDurationSeconds(input) {
    ...
    if (!match) {
        throw new Error(`Invalid duration "${input}". Use a positive number of seconds (e.g. 90) or a duration with unit (e.g. 30s, 5m, 1h, 500ms).`);
    }
    ...
    if (seconds <= 0) {
        throw new Error(`Invalid duration "${input}": must be greater than zero. Omit the flag entirely if you don't want a wall-clock budget.`);
    }
    if (seconds < MIN_WALL_TIME_SECONDS) {
        ...
        const hint = /ms\b/i.test(trimmed)
            ...
            : '';
        throw new Error(`Invalid duration "${input}": below the ${MIN_WALL_TIME_SECONDS}s minimum${hint}. Sub-second wall-clock budgets fire before any model round-trip can complete.`);
    }
    if (seconds > MAX_WALL_TIME_SECONDS) {
        throw new Error(`Invalid duration "${input}": exceeds the maximum supported wall-clock budget (${MAX_WALL_TIME_SECONDS}s ≈ 24 days). Use a smaller value.`);
    }
    return seconds;
}
/**
 ...
 */
export function validateMaxWallTimeSetting(value) {
    if (value === -1)
        return -1;
    if (!Number.isFinite(value)) {
        throw new Error(`model.maxWallTimeSeconds must be a finite number; got ${value}.`);
    }
    if (value <= 0) {
        throw new Error(`model.maxWallTimeSeconds must be > 0 (or -1 for unlimited); got ${value}. ` +
            `Use -1 to disable, not 0.`);
    }
    if (value < MIN_WALL_TIME_SECONDS) {
        throw new Error(`model.maxWallTimeSeconds ${value} is below the ${MIN_WALL_TIME_SECONDS}s minimum. Sub-second budgets fire before any model round-trip can complete.`);
    }
    if (value > MAX_WALL_TIME_SECONDS) {
        throw new Error(`model.maxWallTimeSeconds ${value} exceeds the maximum supported wall-clock budget (${MAX_WALL_TIME_SECONDS}s ≈ 24 days).`);
    }
    return value;
}
/**
 ...
 */
export function validateMaxToolCalls(value) {
    if (value === -1)
        return -1;
    if (!Number.isFinite(value)) {
    ...
    if (!Number.isInteger(value)) {
        throw new Error(`maxToolCalls must be an integer (or -1 for unlimited); got ${value}.`);
    }
    if (value < 0) {
        throw new Error(`maxToolCalls must be >= 0 (or -1 for unlimited); got ${value}. Use -1 to disable, not a negative number.`);
    }
    if (value > MAX_TOOL_CALLS) {
        throw new Error(`maxToolCalls ${value} exceeds the supported ceiling (${MAX_TOOL_CALLS}). Likely a typo — use a smaller value or -1 for unlimited.`);
    }
    return value;
}
