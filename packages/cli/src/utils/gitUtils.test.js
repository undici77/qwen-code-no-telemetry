...
import { isGitHubRepositoryAsync, getGitRepoRootAsync, getLatestGitHubRelease, getGitHubRepoInfoAsync, } from './gitUtils.js';
...
function mockExecFileError(error) {
    vi.mocked(child_process.execFile).mockImplementation(((_cmd, _args, _opts, cb) => {
        cb(error, '', '');
        return {};
    }));
...
describe('getGitHubRepoInfoAsync', async () => {
    ...
    it.each([
        ...
        await expect(getGitHubRepoInfoAsync()).rejects.toThrowError(/Owner & repo could not be extracted from remote URL/);
    });
    it.each([
...
describe('getGitRepoRootAsync', async () => {
    ...
    it('throws an error if git root is empty', async () => {
        mockExecFileStdout('');
        await expect(getGitRepoRootAsync()).rejects.toThrowError(/Git repo returned empty value/);
    });
...
describe('getLatestRelease', async () => {
    ...
    it('uses ProxyAgent when a proxy is provided', async () => {
        ...
        await expect(getLatestGitHubRelease('http://proxy.local:8080')).resolves.toBe('v1.2.4');
