...
import { formatUpdateInstructions, getInstallationInfo, getNpmCliPath, PackageManager, resolveUpdateCommand, } from './installationInfo.js';
...
describe('getInstallationInfo', () => {
    ...
    it('should fall back to npm when manifest.json is malformed', () => {
        ...
        expect(info.updateCommand).toBe('npm install -g @qwen-code/qwen-code@latest');
    });
    ...
    it('should detect global yarn installation', () => {
        ...
        expect(info.isGlobal).toBe(true);
        expect(info.updateCommand).toBe('yarn global add @qwen-code/qwen-code@latest');
        expect(info.updateMessage).toContain('Attempting to automatically update');
        // isAutoUpdateEnabled = false -> "Please run..."
        const infoDisabled = getInstallationInfo(projectRoot, false);
...
describe('resolveUpdateCommand', () => {
    it('replaces @latest with the pinned stable version', () => {
        expect(resolveUpdateCommand('npm i -g @qwen-code/qwen-code@latest', '1.2.3')).toBe('npm i -g @qwen-code/qwen-code@1.2.3');
    });
    it('replaces @latest with @nightly for nightly versions', () => {
        expect(resolveUpdateCommand('npm i -g @qwen-code/qwen-code@latest', '1.2.3-nightly.20250101')).toBe('npm i -g @qwen-code/qwen-code@nightly');
    });
});
describe('formatUpdateInstructions', () => {
    ...
    it('resolves @latest in updateMessage-only guidance for nightly versions', () => {
        expect(formatUpdateInstructions({
            packageManager: PackageManager.NPM,
            isGlobal: true,
            updateMessage: 'Update requires sudo. Please run: sudo npm i -g @qwen-code/qwen-code@latest',
