...
describe('resolvePath', () => {
    ...
    it('expands POSIX-style tilde paths', () => {
        expect(resolvePath('~/schemas/input.json')).toBe(path.join(os.homedir(), 'schemas', 'input.json'));
    });
    ...
    it('expands USERPROFILE references case-insensitively', () => {
        expect(resolvePath('%USERPROFILE%\\schemas\\input.json')).toBe(path.join(os.homedir(), 'schemas', 'input.json'));
    });
    it('normalizes relative paths without resolving them', () => {
        expect(resolvePath('nested/../schema.json')).toBe(path.normalize('schema.json'));
    });
});
