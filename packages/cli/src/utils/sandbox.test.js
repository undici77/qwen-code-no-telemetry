...
import { FatalSandboxError, PRIVATE_ACP_CAPABILITY_ENV, QWEN_DIR, } from '@qwen-code/qwen-code-core';
...
describe('start_sandbox', () => {
    it('passes child environment variables into a container sandbox', async () => {
        ...
        const result = start_sandbox({ command: 'docker', image: 'example.com/qwen-code:latest' }, [], undefined, [process.execPath, '/path/to/cli.js', '--acp'], { [PRIVATE_ACP_CAPABILITY_ENV]: capability });
...
describe('parseSandboxImageName', () => {
    it('uses the image basename and tag for container names', () => {
        expect(parseSandboxImageName('ghcr.io/qwenlm/qwen-code:0.19.2')).toBe('qwen-code-0.19.2');
    });
    it('handles registry ports without treating them as tags', () => {
        expect(parseSandboxImageName('localhost:5000/team/qwen-code-sandbox:dev')).toBe('qwen-code-sandbox-dev');
    });
    it('handles registry ports when the image is untagged', () => {
        expect(parseSandboxImageName('localhost:5000/team/qwen-code-sandbox')).toBe('qwen-code-sandbox');
    });
    it('drops digests from generated container names', () => {
        expect(parseSandboxImageName('registry.example.com/team/qwen-code-sandbox@sha256:abcdef')).toBe('qwen-code-sandbox');
    });
    it('keeps tags when dropping digests from generated container names', () => {
        expect(parseSandboxImageName('registry.example.com/team/qwen-code-sandbox:dev@sha256:abcdef')).toBe('qwen-code-sandbox-dev');
    });
});
