...
describe('standalone-update', () => {
    ...
    describe('rollbackStandaloneUpdate', () => {
        ...
        it('succeeds even with minimal manifest in .old', () => {
            ...
            fs.mkdirSync(standaloneDir);
            fs.mkdirSync(oldDir);
            fs.writeFileSync(path.join(standaloneDir, 'manifest.json'), JSON.stringify({ name: '@qwen-code/qwen-code', version: '0.17.0' }));
    ...
    describe('performStandaloneUpdate', () => {
        ...
        it('rejects directory without manifest as non-managed install', async () => {
            ...
            await expect(performStandaloneUpdate(standaloneDir, '1.0.0')).rejects.toThrow('not a Qwen Code standalone install');
        });
    ...
    describe('isSafeTarEntryPath', () => {
        ...
        it('rejects parent-directory segments and absolute paths', () => {
            ...
            expect(isSafeTarEntryPath('C:\\tmp\\qwen-code\\manifest.json')).toBe(false);
    ...
    describe('isSafeTarLinkTarget', () => {
        it('allows relative link targets inside the extraction directory', () => {
            ...
            expect(isSafeTarLinkTarget('qwen-code/bin/qwen', '../lib/cli.js', dest)).toBe(true);
            expect(isSafeTarLinkTarget('qwen-code/bin/qwen', './qwen', dest)).toBe(true);
        });
        it('allows symlink targets in child directories starting with two dots', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarLinkTarget('qwen-code/bin/qwen', '../..hidden/tool', dest)).toBe(true);
        });
        it('rejects symlink targets outside the extraction directory', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarLinkTarget('qwen-code/bin/qwen', '../../../etc/passwd', dest)).toBe(false);
            expect(isSafeTarLinkTarget('qwen-code/bin/qwen', '/etc/passwd', dest)).toBe(false);
            expect(isSafeTarLinkTarget('qwen-code/bin/qwen', 'C:\\Windows\\System32', dest)).toBe(false);
        });
        it('rejects symlink targets outside the archive root that will be installed', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarLinkTarget('qwen-code/bin/qwen', '../../shared/node', dest)).toBe(false);
            expect(isSafeTarLinkTarget('./qwen-code/bin/qwen', '../../shared/node', dest)).toBe(false);
        });
    });
    describe('isSafeTarEntry', () => {
        it('rejects hardlinks outright', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarEntry('qwen-code/bin/qwen', { type: 'Link', linkpath: '../poc.txt' }, dest)).toBe(false);
        });
        it('allows safe regular entries and safe symlinks', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarEntry('qwen-code/bin/qwen', { type: 'File' }, dest)).toBe(true);
            expect(isSafeTarEntry('qwen-code/lib', { type: 'Directory' }, dest)).toBe(true);
            expect(isSafeTarEntry('qwen-code/bin/qwen', { type: 'SymbolicLink', linkpath: '../lib/cli.js' }, dest)).toBe(true);
        });
        it('accepts fs stats entries from tar filter typing', () => {
            const dest = path.join(tempDir, 'extract');
            const filePath = path.join(tempDir, 'entry.txt');
            fs.writeFileSync(filePath, 'entry');
            expect(isSafeTarEntry('qwen-code/bin/qwen', fs.statSync(filePath), dest)).toBe(true);
        });
        it('rejects special archive entry types', () => {
            ...
            for (const type of [
                ...
                expect(isSafeTarEntry('qwen-code/bin/qwen', { type }, dest)).toBe(false);
            }
        });
    });
    describe('cleanupFirstTimeMigrationArtifacts', () => {
        it.skipIf(process.platform === 'win32')('removes the wrapper and PATH block created during a failed migration', () => {
            ...
            try {
                ...
                expect(fs.existsSync(wrapperPath)).toBe(true);
                expect(fs.readFileSync(bashrc, 'utf-8')).toContain('# Qwen Code PATH block begin');
                cleanupFirstTimeMigrationArtifacts(artifacts);
                expect(fs.existsSync(wrapperPath)).toBe(false);
                expect(fs.readFileSync(bashrc, 'utf-8')).not.toContain('# Qwen Code PATH block begin');
            }
            finally {
    ...
    describe('rollbackStandaloneUpdate — concurrent lock protection', () => {
        ...
        it('proceeds when lock has dead PID', () => {
            ...
            fs.mkdirSync(oldDir);
            fs.writeFileSync(path.join(standaloneDir, 'manifest.json'), JSON.stringify({ name: '@qwen-code/qwen-code', version: '0.17.0' }));
            fs.writeFileSync(path.join(oldDir, 'manifest.json'), JSON.stringify({ name: '@qwen-code/qwen-code', version: '0.16.0' }));
            fs.writeFileSync(lockPath, '999999999');
            const result = rollbackStandaloneUpdate(standaloneDir);
            expect(result.ok).toBe(true);
        });
    ...
    describe.skipIf(process.platform === 'win32')('ensurePathInShellRc', () => {
        ...
        it('skips if block markers already in rc file', () => {
            const binDir = path.join(tempDir, 'bin');
            const zshrc = path.join(tempDir, '.zshrc');
            fs.writeFileSync(zshrc, `# Qwen Code PATH block begin\nexport PATH='${binDir}':$PATH\n# Qwen Code PATH block end\n`);
            const origShell = process.env['SHELL'];
            const origHome = process.env['HOME'];
            process.env['SHELL'] = '/bin/zsh';
            process.env['HOME'] = tempDir;
        ...
        it('skips if legacy marker already in rc file', () => {
            const binDir = path.join(tempDir, 'bin');
            const zshrc = path.join(tempDir, '.zshrc');
            fs.writeFileSync(zshrc, `# Added by Qwen Code standalone installer\nexport PATH="${binDir}:$PATH"\n`);
            const origShell = process.env['SHELL'];
            const origHome = process.env['HOME'];
            process.env['SHELL'] = '/bin/zsh';
            process.env['HOME'] = tempDir;
        ...
        it('falls back to .bash_profile for bash when .bashrc is absent', () => {
            ...
            try {
                ensurePathInShellRc(binDir);
                expect(fs.readFileSync(profile, 'utf-8')).toContain('# Qwen Code PATH block begin');
