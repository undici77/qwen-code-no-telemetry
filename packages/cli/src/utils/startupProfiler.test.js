...
import { initStartupProfiler, profileCheckpoint, recordStartupEvent, setInteractiveMode, finalizeStartupProfile, getStartupReport, resetStartupProfiler, } from './startupProfiler.js';
vi.mock('node:fs');
describe('startupProfiler', () => {
    ...
    beforeEach(() => {
        ...
        vi.restoreAllMocks();
        saveEnv('QWEN_CODE_PROFILE_STARTUP', 'QWEN_CODE_PROFILE_STARTUP_OUTER', 'QWEN_CODE_PROFILE_STARTUP_NO_HEAP', 'SANDBOX');
    ...
    describe('when enabled (QWEN_CODE_PROFILE_STARTUP=1 + SANDBOX)', () => {
        ...
        it('drops events recorded after finalize to keep memory bounded', () => {
            ...
            expect(written.events.map((e) => e.name)).toEqual([
                'first',
            ]);
            expect(written.phases.map((p) => p.name)).toEqual([
                'main_entry',
            ]);
        });
