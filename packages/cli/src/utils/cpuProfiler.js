...
export async function stopCpuProfile(options) {
    ...
    try {
        ...
        const filePath = path.join(outputDir, `qwen-code-cpu-${process.pid}-${formatTimestamp(now)}.cpuprofile`);
        try {
...
function cleanupOldProfiles(outputDir, maxProfiles) {
    ...
    try {
        profiles = fs
            ...
            .filter((name) => name.startsWith('qwen-code-cpu-') && name.endsWith('.cpuprofile'))
