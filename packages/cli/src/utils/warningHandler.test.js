...
import { initializeWarningHandler, resetWarningHandlerForTests, } from './warningHandler.js';
const ENV_KEYS = ['NODE_ENV', 'DEBUG', 'QWEN_DEBUG'];
describe('initializeWarningHandler', () => {
    ...
    function makeWarning(name, message) {
    ...
    it('suppresses MaxListenersExceededWarning for AbortSignal in production', () => {
        ...
        emit(makeWarning('MaxListenersExceededWarning', 'Possible EventTarget memory leak detected. 1509 abort listeners added to [AbortSignal].'));
        expect(priorListener).not.toHaveBeenCalled();
    });
    it('does NOT suppress generic [EventTarget] warnings — only AbortSignal', () => {
        initializeWarningHandler();
        emit(makeWarning('MaxListenersExceededWarning', 'Possible EventTarget memory leak detected. 11 listeners added to [EventTarget].'));
    ...
    it('suppresses AbortSignal warnings with class metadata, e.g. [AbortSignal{aborted: false}]', () => {
        initializeWarningHandler();
        emit(makeWarning('MaxListenersExceededWarning', 'Possible EventTarget memory leak detected. 11 abort listeners added to [AbortSignal{aborted: false}].'));
        expect(priorListener).not.toHaveBeenCalled();
    });
    ...
    it('keeps suppressed warnings visible when DEBUG is set', () => {
        process.env['DEBUG'] = '1';
        initializeWarningHandler();
        emit(makeWarning('MaxListenersExceededWarning', 'Possible EventTarget memory leak detected. 1500 abort listeners added to [AbortSignal].'));
        expect(priorListener).toHaveBeenCalledTimes(1);
    });
...
describe('initializeWarningHandler — end-to-end stderr behavior', () => {
    ...
    it('a child process with the handler installed does not print suppressed AbortSignal warnings to stderr', async () => {
        ...
        const helperImportSpecifier = pathToFileURL(join(here, 'warningHandler.ts')).href;
...
//# sourceMappingURL=warningHandler.test.js.map