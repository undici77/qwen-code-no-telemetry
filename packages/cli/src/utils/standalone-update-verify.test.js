...
describe('standalone-update-verify', () => {
    ...
    describe('verifySignature', () => {
        it('rejects signature with wrong length', () => {
            expect(() => verifySignature('test content', 'dG9vc2hvcnQ=')).toThrow('Invalid signature length');
        });
        it('rejects invalid signature (correct length but wrong bytes)', () => {
            ...
            expect(() => verifySignature('test content', fakeSig)).toThrow('signature verification failed');
        });
        it('rejects tampered content with valid-length signature', () => {
            // Even a random 64-byte signature should fail verification
            const randomSig = Buffer.from(Array.from({ length: 64 }, () => Math.floor(Math.random() * 256))).toString('base64');
            expect(() => verifySignature('some SHA256SUMS content', randomSig)).toThrow('signature verification failed');
        });
    });
});
//# sourceMappingURL=standalone-update-verify.test.js.map