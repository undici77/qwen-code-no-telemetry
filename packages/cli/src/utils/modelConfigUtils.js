 ...
 */
import { AuthType, MODEL_GENERATION_CONFIG_FIELDS, normalizeReasoningEffort, REASONING_EFFORT_TIERS, resolveModelConfig, resolveProviderProtocol, stripRuntimeSnapshotPrefix, } from '@qwen-code/qwen-code-core';
...
function buildSkippedProviderWarnings(modelProviders, providerProtocol) {
    ...
    for (const [providerId, models] of Object.entries(modelProviders)) {
        ...
        if (protocol === AuthType.QWEN_OAUTH &&
            ...
            warnings.push(`Warning: modelProviders provider "${providerId}" maps to "qwen-oauth" via providerProtocol, but qwen-oauth uses hard-coded models only; its ${models.length} model(s) are ignored.`);
            continue;
        }
