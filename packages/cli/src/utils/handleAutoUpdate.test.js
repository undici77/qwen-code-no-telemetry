...
describe('handleAutoUpdate', () => {
    ...
    it('should show manual update message when enableAutoUpdate is false', () => {
        ...
        mockGetInstallationInfo.mockReturnValue({
            ...
            updateMessage: 'Please run npm i -g @qwen-code/qwen-code@latest to update',
            isGlobal: true,
    ...
    it('should emit "update-failed" when the update process fails', async () => {
        ...
        const update = handleAutoUpdate(mockUpdateInfo, mockSettings, '/root', mockSpawn);
    ...
    it('should use the "@nightly" tag for nightly updates', async () => {
        ...
        handleAutoUpdate(mockUpdateInfo, mockSettings, '/root', mockSpawn);
        expect(mockSpawn).toHaveBeenCalledWith(process.execPath, [process.argv[1]], {
            detached: true,
    ...
    it('runs non-npm package-manager updates through the shell', () => {
        ...
        handleAutoUpdate(mockUpdateInfo, mockSettings, '/root', mockSpawn);
        expect(mockSpawn).toHaveBeenCalledWith('bash', ['-c', 'pnpm add -g @qwen-code/qwen-code@2.0.0'], {
...
describe('handleAutoUpdate — standalone path', () => {
    ...
    it('calls performStandaloneUpdate and does NOT spawn npm', async () => {
        ...
        handleAutoUpdate(mockUpdateInfo, mockSettings, '/root', mockSpawn);
        await vi.waitFor(() => expect(emitSpy).toHaveBeenCalledWith('update-success', expect.anything()));
        expect(mockPerformStandaloneUpdate).toHaveBeenCalledWith('/home/user/.local/lib/qwen-code', '2.0.0');
        expect(mockSpawn).not.toHaveBeenCalled();
    });
    it('emits deferred message when result is "deferred"', async () => {
        ...
        await vi.waitFor(() => expect(emitSpy).toHaveBeenCalledWith('update-success', expect.anything()));
        expect(emitSpy).toHaveBeenCalledWith('update-success', {
            message: 'Update downloaded. It will be applied after you exit this session.',
        });
    });
    ...
    it('emits update-failed on rejection', async () => {
        ...
        handleAutoUpdate(mockUpdateInfo, mockSettings, '/root', mockSpawn);
        await vi.waitFor(() => expect(emitSpy).toHaveBeenCalledWith('update-failed', expect.anything()));
        expect(emitSpy).toHaveBeenCalledWith('update-failed', {
            message: 'Automatic update failed: Download failed. Re-run the installer to update manually.',
        });
        expect(mockSpawn).not.toHaveBeenCalled();
    });
});
