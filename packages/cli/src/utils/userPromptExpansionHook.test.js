...
import { appendUserPromptExpansionAdditionalContext, formatUserPromptExpansionBlockedMessage, serializeUserPromptExpansionPrompt, } from './userPromptExpansionHook.js';
describe('appendUserPromptExpansionAdditionalContext', () => {
    it('returns content unchanged when additionalContext is undefined', () => {
        expect(appendUserPromptExpansionAdditionalContext('base prompt', undefined)).toBe('base prompt');
    });
    it('appends additional context to string prompts', () => {
        const result = appendUserPromptExpansionAdditionalContext('base prompt', 'hook context');
        expect(result).toBe('base prompt\n\nhook context');
    });
    it('appends additional context to part arrays', () => {
        const result = appendUserPromptExpansionAdditionalContext([{ text: 'base prompt' }], 'hook context');
...
describe('formatUserPromptExpansionBlockedMessage', () => {
    ...
    it('sanitizes and truncates block reasons', () => {
        ...
        expect(result).toBe(`UserPromptExpansion blocked: &lt;policy&gt;${'x'.repeat(9_986)}`);
        expect(result.length).toBe('UserPromptExpansion blocked: '.length + 10_000);
    });
    it('does not leave a partial entity after truncation', () => {
        const result = formatUserPromptExpansionBlockedMessage('x'.repeat(9_999) + '<');
    ...
    it('does not leave a partial ampersand entity after truncation', () => {
        const result = formatUserPromptExpansionBlockedMessage('x'.repeat(9_998) + '&');
...
describe('serializeUserPromptExpansionPrompt', () => {
    it('returns string prompts unchanged', () => {
        expect(serializeUserPromptExpansionPrompt('plain prompt')).toBe('plain prompt');
    });
    it('serializes part arrays with verbose formatting', () => {
        expect(serializeUserPromptExpansionPrompt([
            { text: 'first' },
    ...
    it('serializes a single part object', () => {
        expect(serializeUserPromptExpansionPrompt({ text: 'single part' })).toBe('single part');
    });
    it('serializes empty part arrays to an empty string', () => {
        expect(serializeUserPromptExpansionPrompt([])).toBe('');
    });
});
//# sourceMappingURL=userPromptExpansionHook.test.js.map