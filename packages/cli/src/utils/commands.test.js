...
import { isValidStackedSkillPrefix, parseSlashCommand, parseStackedSlashCommands, } from './commands.js';
...
describe('parseSlashCommand', () => {
    ...
    it('should return the parent command if subcommand is unknown', () => {
        const result = parseSlashCommand('/config unknownsub some args', mockCommands);
...
describe('isValidStackedSkillPrefix', () => {
    it('accepts one or more leading skills and aliases', () => {
        expect(isValidStackedSkillPrefix('/feat-dev ', mockCommandsWithSkills)).toBe(true);
        expect(isValidStackedSkillPrefix('/feat-dev\n/debug ', mockCommandsWithSkills)).toBe(true);
    });
    it('rejects non-skill, unknown, and prompt-text prefixes', () => {
        expect(isValidStackedSkillPrefix('/help ', mockCommandsWithSkills)).toBe(false);
        expect(isValidStackedSkillPrefix('/unknown ', mockCommandsWithSkills)).toBe(false);
        expect(isValidStackedSkillPrefix('/feat-dev implement ', mockCommandsWithSkills)).toBe(false);
    });
    it('rejects non-user-invocable and hidden skills', () => {
        ...
        expect(isValidStackedSkillPrefix('/model-only ', restrictedSkills)).toBe(false);
        expect(isValidStackedSkillPrefix('/hidden-skill ', restrictedSkills)).toBe(false);
    });
    it('rejects a prefix that already contains the maximum skill count', () => {
        expect(isValidStackedSkillPrefix('/feat-dev /e2e-testing /bugfix /review /simplify ', mockCommandsWithSkills)).toBe(false);
    });
});
describe('parseStackedSlashCommands', () => {
    it('should return empty for a single skill (not stacked)', () => {
        const result = parseStackedSlashCommands('/feat-dev implement X', mockCommandsWithSkills);
    ...
    it('should detect two stacked skills', () => {
        const result = parseStackedSlashCommands('/feat-dev /e2e-testing implement X', mockCommandsWithSkills);
    ...
    it('should detect three stacked skills with remaining text', () => {
        const result = parseStackedSlashCommands('/feat-dev /e2e-testing /bugfix fix the login page', mockCommandsWithSkills);
    ...
    it('should detect five skills at the maximum', () => {
        const result = parseStackedSlashCommands('/feat-dev /e2e-testing /bugfix /review /simplify do it', mockCommandsWithSkills);
    ...
    it('should cap at five skills and set exceededMax when six are given', () => {
        const result = parseStackedSlashCommands('/feat-dev /e2e-testing /bugfix /review /simplify /structured-debugging do it', mockCommandsWithSkills);
        expect(result.skills).toHaveLength(5);
    ...
    it('should stop at a non-skill token', () => {
        const result = parseStackedSlashCommands('/feat-dev /help do something', mockCommandsWithSkills);
        // Only feat-dev is a skill before /help (which is BUILT_IN), so only 1 skill → not stacked
        expect(result.skills).toHaveLength(0);
    });
    it('should stop at an unknown /token', () => {
        const result = parseStackedSlashCommands('/feat-dev /nonexistent do something', mockCommandsWithSkills);
        expect(result.skills).toHaveLength(0);
    });
    it('should return empty when input does not start with /', () => {
        const result = parseStackedSlashCommands('feat-dev /e2e-testing', mockCommandsWithSkills);
        expect(result.skills).toHaveLength(0);
    ...
    it('should support alt names (aliases)', () => {
        const result = parseStackedSlashCommands('/feat-dev /debug trace the issue', mockCommandsWithSkills);
        expect(result.skills).toHaveLength(2);
        expect(result.skills[0]?.name).toBe('feat-dev');
    ...
    it('should return empty when the command list has no skills', () => {
        const result = parseStackedSlashCommands('/help /commit', mockCommands);
        expect(result.skills).toHaveLength(0);
        expect(result.remainingText).toBe('/help /commit');
    });
    it('should treat first non-skill word as remaining text (not stacked)', () => {
        const result = parseStackedSlashCommands('/feat-dev implement /e2e-testing', mockCommandsWithSkills);
    ...
    it('should handle leading whitespace in query', () => {
        const result = parseStackedSlashCommands('  /feat-dev /e2e-testing do it  ', mockCommandsWithSkills);
        expect(result.skills).toHaveLength(2);
        expect(result.skills[0]?.name).toBe('feat-dev');
        expect(result.skills[1]?.name).toBe('e2e-testing');
        expect(result.remainingText).toBe('do it');
    ...
    it('should break on empty token after slash in stacked position', () => {
        ...
        const result = parseStackedSlashCommands('/feat-dev / something', mockCommandsWithSkills);
    ...
    it('should handle exactly MAX_STACKED_SKILLS without exceededMax', () => {
        ...
        const result = parseStackedSlashCommands('/feat-dev /e2e-testing /bugfix /review /simplify', mockCommandsWithSkills);
        expect(result.skills).toHaveLength(5);
        expect(result.exceededMax).toBe(false);
        expect(result.remainingText).toBe('');
    });
    it('should keep the 6th token in remainingText when exceeded', () => {
        const result = parseStackedSlashCommands('/feat-dev /e2e-testing /bugfix /review /simplify /structured-debugging extra text', mockCommandsWithSkills);
        expect(result.skills).toHaveLength(5);
        expect(result.exceededMax).toBe(true);
