...
function writeInstallation(prefix, version) {
    const packageRoot = path.join(prefix, 'node_modules', '@qwen-code', 'qwen-code');
    fs.mkdirSync(packageRoot, { recursive: true });
    fs.writeFileSync(path.join(packageRoot, 'package.json'), JSON.stringify({ name: '@qwen-code/qwen-code', version }));
...
function writeBaseInstallation(root, version = '1.0.0') {
    const packageRoot = path.join(root, 'global', 'node_modules', '@qwen-code', 'qwen-code');
...
describe('managed npm update', () => {
    ...
    it.each([
        ...
        if (brokenBundle) {
            fs.writeFileSync(path.join(update.stagingDir, 'node_modules', '@qwen-code', 'qwen-code', 'cli.js'), 'this is not valid JavaScript !!!\n');
        }
