...
export const isStackedSkillCompletableCommand = (command) => command.kind === CommandKind.SKILL &&
 ...
 */
export const isValidStackedSkillPrefix = (prefix, commands) => {
    const trimmed = prefix.trim();
    if (!trimmed)
        return false;
    const tokens = trimmed.split(/\s+/);
    if (tokens.length >= MAX_STACKED_SKILLS)
        return false;
 ...
 */
export const parseStackedSlashCommands = (query, commands) => {
    const trimmed = query.trim();
    ...
    while (pos < commandText.length) {
        ...
        while (pos < commandText.length && /\s/.test(commandText[pos]))
            pos++;
        ...
        if (skills.length === 0) {
            ...
            if (!cmd || cmd.kind !== CommandKind.SKILL)
                break;
            skills.push(cmd);
            restPos = pos;
            continue;
