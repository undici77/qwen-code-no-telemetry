...
import { parseAllowedTerminalSequences, emitTerminalSequence, } from './terminalSequence.js';
describe('parseAllowedTerminalSequences', () => {
    ...
    describe('rejected sequences', () => {
        ...
        it('rejects OSC 8 hyperlink', () => {
            expect(parseAllowedTerminalSequences('\x1b]8;;https://example.com\x07')).toBeNull();
        });
        it('rejects OSC 52 clipboard', () => {
            expect(parseAllowedTerminalSequences('\x1b]52;c;dGVzdA==\x07')).toBeNull();
        });
        it('rejects OSC 1337', () => {
            expect(parseAllowedTerminalSequences('\x1b]1337;SetMark\x07')).toBeNull();
        });
        it('rejects OSC 4 palette change', () => {
            expect(parseAllowedTerminalSequences('\x1b]4;1;rgb:ff/00/00\x07')).toBeNull();
        });
        ...
        it('rejects OSC with nested ESC that is not ST', () => {
            expect(parseAllowedTerminalSequences('\x1b]9;he\x1b[31mllo\x07')).toBeNull();
        });
...
//# sourceMappingURL=terminalSequence.test.js.map