...
import { _resetNonInteractiveForTesting, startNonInteractiveOpenAILogHousekeeping, stopNonInteractiveOpenAILogHousekeeping, } from './scheduler.js';
const MS_PER_MINUTE = 60 * 1000;
const MS_PER_HOUR = 60 * MS_PER_MINUTE;
function makeConfig(logDir) {
    return {
...
describe('non-interactive OpenAI log housekeeping', () => {
    ...
    beforeEach(async () => {
        ...
        qwenHome = fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-noninteractive-housekeeping-'));
        ...
        mocks.runThrottledOnce.mockImplementation(async (_options, task) => (await task()) === false
            ? { status: 'incomplete' }
            : { status: 'completed' });
    });
    afterEach(async () => {
    ...
    it('deduplicates the same resolved log directory', async () => {
        ...
        startNonInteractiveOpenAILogHousekeeping(makeConfig(logDir), makeSettings());
        startNonInteractiveOpenAILogHousekeeping(makeConfig(logDir), makeSettings());
        await vi.waitFor(() => expect(mocks.cleanupOldOpenAILogs).toHaveBeenCalledOnce());
    ...
    it('serializes different directories in FIFO order', async () => {
        ...
        startNonInteractiveOpenAILogHousekeeping(makeConfig(firstDir), makeSettings());
        startNonInteractiveOpenAILogHousekeeping(makeConfig(secondDir), makeSettings());
        await vi.waitFor(() => expect(releaseFirst).toBeDefined());
        expect(mocks.cleanupOldOpenAILogs).toHaveBeenCalledTimes(1);
        releaseFirst?.({ removed: 0, errors: 0, completed: true });
        await vi.waitFor(() => expect(mocks.cleanupOldOpenAILogs).toHaveBeenCalledTimes(2));
        expect(mocks.cleanupOldOpenAILogs.mock.calls[1]?.[0].logDir).toBe(secondDir);
    });
    it('uses ModelsConfig as the fallback for the CLI logging directory', async () => {
        ...
        await vi.waitFor(() => expect(mocks.cleanupOldOpenAILogs).toHaveBeenCalledOnce());
        expect(mocks.cleanupOldOpenAILogs.mock.calls[0]?.[0].logDir).toBe(modelLogDir);
    });
    it('prefers the initialized content-generator logging directory', async () => {
        const contentGeneratorLogDir = path.join(qwenHome, 'from-content-generator-config');
        const modelLogDir = path.join(qwenHome, 'from-models-config');
        const config = {
        ...
        await vi.waitFor(() => expect(mocks.cleanupOldOpenAILogs).toHaveBeenCalledOnce());
        expect(mocks.cleanupOldOpenAILogs.mock.calls[0]?.[0].logDir).toBe(contentGeneratorLogDir);
    });
    it('retries a held lock after one minute', async () => {
        vi.useFakeTimers();
        mocks.runThrottledOnce.mockResolvedValue({ status: 'locked' });
        startNonInteractiveOpenAILogHousekeeping(makeConfig(path.join(qwenHome, 'logs')), makeSettings());
        await vi.waitFor(() => expect(mocks.runThrottledOnce).toHaveBeenCalledOnce());
    ...
    ])('clamps a fresh marker retry to the %s delay', async (_name, retryAfterMs, expectedDelayMs) => {
        vi.useFakeTimers();
        mocks.runThrottledOnce
    ...
    it('aborts the active scan during stop', async () => {
        let observedSignal;
        mocks.cleanupOldOpenAILogs.mockImplementation(({ signal }) => new Promise((resolve) => {
        ...
        startNonInteractiveOpenAILogHousekeeping(makeConfig(path.join(qwenHome, 'queued')), makeSettings());
        await stopNonInteractiveOpenAILogHousekeeping();
        expect(observedSignal?.aborted).toBe(true);
        expect(mocks.cleanupOldOpenAILogs).toHaveBeenCalledOnce();
        mocks.runThrottledOnce.mockClear();
        startNonInteractiveOpenAILogHousekeeping(makeConfig(path.join(qwenHome, 'after-stop')), makeSettings());
    ...
    it('caps stop waiting at 250ms when a filesystem task does not settle', async () => {
        vi.useFakeTimers();
        let releaseWorker;
        mocks.runThrottledOnce.mockImplementation(() => new Promise((resolve) => {
...
//# sourceMappingURL=non-interactive-scheduler.test.js.map