...
describe('runThrottledOnce', () => {
    ...
    it('runs task, writes marker, releases lock on first call', async () => {
        ...
        const ran = await runThrottledOnce({ name: 'test', markerPath, lockPath }, task);
    ...
    it('skips immediate second call (mtime gate)', async () => {
        ...
        const ran2 = await runThrottledOnce({ name: 'test', markerPath, lockPath }, task2);
    ...
    it('reports only the remaining freshness interval for an old marker', async () => {
        ...
        const task = vi.fn(async () => { });
        const result = await runThrottledOnce({ name: 'test', markerPath, lockPath }, task);
    ...
    it('skips when a fresh lock exists (lock held by another process)', async () => {
        ...
        const task = vi.fn(async () => { });
        const ran = await runThrottledOnce({ name: 'test', markerPath, lockPath, staleLockMs: MS_PER_HOUR }, task);
    ...
    it('does not write marker when task throws, but releases lock', async () => {
        ...
        await expect(runThrottledOnce({ name: 'test', markerPath, lockPath }, task)).rejects.toThrow('boom');
        expect(fs.existsSync(markerPath)).toBe(false);
        expect(fs.existsSync(lockPath)).toBe(false);
    });
...
//# sourceMappingURL=throttledOnce.test.js.map