...
import { startBackgroundHousekeeping, _resetForTesting, _needsCatchUpForTesting, _getFirstPassDelayForTesting, _runHousekeepingForTesting, _runPassForTesting, _FILE_HISTORY_MARKER_FOR_TESTING, _getSubagentMarkerPathForTesting, _getOpenAILogsMarkerPathForTesting, } from './scheduler.js';
import { noteInteraction, _resetForTesting as resetInteraction, _setLastInteractionForTesting, } from './lastInteractionAt.js';
...
function makeSettings(cleanupPeriodDays) {
    return {
        merged: {
...
function makeConfig(sessionId, projectDir) {
    return {
...
describe('_runHousekeepingForTesting', () => {
    ...
    it('whitelists the current session via lazy getSessionId()', async () => {
        ...
        await _runHousekeepingForTesting(makeConfig('current-session'), makeSettings(30));
        expect(fs.readdirSync(fileHistoryRoot)).toEqual(['current-session']);
        // Marker was written by throttledOnce.
        expect(fs.existsSync(path.join(qwenHome, _FILE_HISTORY_MARKER_FOR_TESTING))).toBe(true);
    });
    it('sweeps old subagent transcripts under <projectDir>/subagents, protecting the current session', async () => {
        ...
        await _runHousekeepingForTesting(makeConfig('current-session', projectDir), makeSettings(30));
        expect(fs.readdirSync(subagentsRoot).sort()).toEqual([
            'current-session',
            'recent-session',
        ]);
        expect(fs.existsSync(_getSubagentMarkerPathForTesting(qwenHome, projectDir))).toBe(true);
        expect(fs.existsSync(path.join(projectDir, '.subagent-cleanup'))).toBe(false);
    });
    it('throttles the subagent sweep per project (second pass does not re-sweep)', async () => {
        ...
        await _runHousekeepingForTesting(makeConfig('current', projectDir), makeSettings(30));
    ...
    it('uses default 30 days when cleanupPeriodDays is unset', async () => {
        ...
        await _runHousekeepingForTesting(makeConfig('x'), makeSettings( /* unset */));
...
describe('_runHousekeepingForTesting (openai-logs cleanup)', () => {
    ...
    function makeOpenAIConfig(openAILoggingDir, workingDir = process.cwd()) {
        return {
            getSessionId: () => 's1',
    ...
    function makeModelSettings(retentionDays, options = {}) {
        const model = {
        ...
        return {
            ...
            system: {
                settings: {
                    model: options.systemRetentionDays !== undefined
            ...
            user: {
                settings: {
                    model: options.userRetentionDays !== undefined
            ...
            workspace: {
                settings: {
                    model: options.workspaceRetentionDays !== undefined
    ...
    function mkOpenAILogAt(targetDir, ageMs, id = 'a1b2c3d4') {
    ...
    function mkOpenAILog(daysAgo, targetDir = logDir) {
    ...
    it('removes logs older than the retention and writes the per-dir marker', async () => {
        ...
        await _runHousekeepingForTesting(makeOpenAIConfig(logDir), makeModelSettings(7));
        expect(fs.existsSync(old)).toBe(false);
        expect(fs.existsSync(fresh)).toBe(true);
        expect(fs.existsSync(_getOpenAILogsMarkerPathForTesting(qwenHome, logDir))).toBe(true);
    });
    it('uses the default 7-day retention when openAILogRetentionDays is unset', async () => {
        const old = mkOpenAILog(10);
        await _runHousekeepingForTesting(makeOpenAIConfig(logDir), makeModelSettings( /* unset */));
        expect(fs.existsSync(old)).toBe(false);
    });
    it('honors a longer configured retention', async () => {
        const tenDays = mkOpenAILog(10);
        await _runHousekeepingForTesting(makeOpenAIConfig(logDir), makeModelSettings(30, { userRetentionDays: 30 }));
    ...
    it('honors retention = 0 through the scheduler', async () => {
        ...
        await _runHousekeepingForTesting(makeOpenAIConfig(logDir), makeModelSettings(0, { userRetentionDays: 0 }));
        expect(fs.existsSync(aged)).toBe(false);
        expect(fs.existsSync(fresh)).toBe(true);
    });
    it('throttles OpenAI log cleanup independently for different directories', async () => {
        const otherLogDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-openai-logs-other-'));
        try {
            const first = mkOpenAILog(10);
            const second = mkOpenAILog(10, otherLogDir);
            await _runHousekeepingForTesting(makeOpenAIConfig(logDir), makeModelSettings(7));
            await _runHousekeepingForTesting(makeOpenAIConfig(otherLogDir), makeModelSettings(7));
            expect(fs.existsSync(first)).toBe(false);
            expect(fs.existsSync(second)).toBe(false);
            expect(_getOpenAILogsMarkerPathForTesting(qwenHome, logDir)).not.toBe(_getOpenAILogsMarkerPathForTesting(qwenHome, otherLogDir));
        }
        finally {
    ...
    it('resolves the default per-CWD log dir from getWorkingDir()', async () => {
        ...
        try {
            ...
            const old = path.join(defaultLogDir, 'openai-2026-06-01T10-00-00.000Z-a1b2c3d4.json');
            fs.writeFileSync(old, '{}');
            fs.utimesSync(old, d, d);
            await _runHousekeepingForTesting(makeOpenAIConfig(undefined, workingDir), makeModelSettings(7));
            expect(fs.existsSync(old)).toBe(false);
        }
        finally {
    ...
    it('uses the settings directory when content-generator config is unavailable', async () => {
        ...
        try {
            ...
            await _runHousekeepingForTesting(config, makeModelSettings(7, {
    ...
    it('skips custom-directory cleanup for workspace-scoped retention', async () => {
        ...
        await _runHousekeepingForTesting(makeOpenAIConfig(logDir), makeModelSettings(7, { workspaceRetentionDays: 7 }));
        await _runHousekeepingForTesting(makeOpenAIConfig(logDir), makeModelSettings(30, { workspaceRetentionDays: 30 }));
        expect(fs.existsSync(old)).toBe(true);
        expect(fs.existsSync(_getOpenAILogsMarkerPathForTesting(qwenHome, logDir))).toBe(false);
    });
    ...
    it('uses catch-up delay when the OpenAI marker is missing', async () => {
        ...
        await expect(_getFirstPassDelayForTesting(makeOpenAIConfig(logDir), makeModelSettings(7))).resolves.toBe(60 * 1000);
    });
    it('uses the normal delay only when the OpenAI marker is fresh', async () => {
        ...
        await expect(_getFirstPassDelayForTesting(makeOpenAIConfig(logDir), makeModelSettings(7))).resolves.toBe(10 * 60 * 1000);
    });
    it('uses catch-up delay when the OpenAI marker is older than seven days', async () => {
        const fileHistoryMarker = path.join(qwenHome, _FILE_HISTORY_MARKER_FOR_TESTING);
    ...
    it('skips the sweep gracefully when the log dir cannot be resolved', async () => {
        ...
        await expect(_runHousekeepingForTesting(throwingConfig, makeModelSettings(7))).resolves.toBeUndefined();
        expect(fs.existsSync(fresh)).toBe(true);
    });
});
describe('_runPassForTesting (timer-chain defense)', () => {
    ...
    it('catches errors escaping runHousekeeping so the next pass still gets scheduled', async () => {
        ...
        await expect(_runPassForTesting(throwingConfig, makeSettings(30))).resolves.toBeUndefined();
    });
