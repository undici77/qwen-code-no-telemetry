...
import { HEADLESS_TOOL_RESULT_TEXT_JSON_BYTE_BUDGET, HEADLESS_TOOL_RESULT_TEXT_TRUNCATION_MARKER, } from '../nonInteractive/io/headless-tool-result-text-projection.js';
...
describe('DualOutputBridge', () => {
    ...
    describe('--json-file output', () => {
        it('emits recording failures for the affected session and unsubscribes on shutdown', async () => {
            ...
            await bridge.shutdown();
            expect(readJsonl(target)).toContainEqual(expect.objectContaining({
                type: 'system',
            ...
            expect(readJsonl(target)).not.toContainEqual(expect.objectContaining({ session_id: 'late-session' }));
        });
        ...
        it('routes permission requests + responses through the adapter', async () => {
            bridge = new DualOutputBridge(config, { filePath: target });
            bridge.emitPermissionRequest('req-1', 'shell', 'tu-1', { cmd: 'ls' }, null, [
                {
                    type: 'allow',
                    label: 'Allow Command',
    ...
    describe('buffer overflow guard', () => {
        ...
        it('disables on ERR_SYSTEM_ERROR stream error', () => {
            bridge = new DualOutputBridge(config, { filePath: target });
            expect(bridge.isConnected).toBe(true);
            bridge['stream'].emit('error', Object.assign(new Error('EAGAIN'), { code: 'ERR_SYSTEM_ERROR' }));
            expect(bridge.isConnected).toBe(false);
        });
    });
    describe.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('FIFO (named pipe) support', () => {
        let fifoPath;
        beforeEach(() => {
        ...
        it('delivers events to a reader that connects after construction', async () => {
            ...
            const testEvent = lines.find((l) => l['type'] === 'system' && l['subtype'] === 'test_event');
        ...
        it('throws actionable error when FIFO lacks read permission', () => {
            ...
            try {
                expect(() => new DualOutputBridge(config, { filePath: noReadFifo })).toThrow(/permission denied opening FIFO for read-write/);
            }
            finally {
