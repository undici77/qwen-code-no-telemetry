...
describe('QueryOptionsSchema', () => {
    ...
    it('rejects forkSession without resume or continue', () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
            expect(result.error.issues[0]?.message).toContain('forkSession requires resume or continue');
        }
    });
    ...
    it('accepts maxSubagentDepth boundary values 1 and 100', () => {
        expect(QueryOptionsSchema.safeParse({ maxSubagentDepth: 1 }).success).toBe(true);
        expect(QueryOptionsSchema.safeParse({ maxSubagentDepth: 100 }).success).toBe(true);
    });
...
//# sourceMappingURL=queryOptionsSchema.test.js.map