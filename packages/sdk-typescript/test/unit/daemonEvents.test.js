...
import { asKnownDaemonEvent, createDaemonAuthState, createDaemonSessionViewState, DAEMON_KNOWN_EVENT_TYPE_VALUES, isDaemonEventType, MID_TURN_MESSAGE_INJECTED_EVENT, PENDING_PROMPT_ADDED_EVENT, PENDING_PROMPT_STARTED_EVENT, PENDING_PROMPT_COMPLETED_EVENT, reduceDaemonAuthEvent, reduceDaemonAuthEvents, reduceDaemonSessionEvent, reduceDaemonSessionEvents, } from '../../src/daemon/events.js';
describe('MID_TURN_MESSAGE_INJECTED_EVENT (shared wire constant)', () => {
    it('is the wire literal and a registered known event type', () => {
        ...
        expect(DAEMON_KNOWN_EVENT_TYPE_VALUES).toContain(MID_TURN_MESSAGE_INJECTED_EVENT);
    });
});
describe('daemon event schema', () => {
    ...
    it('recognizes pending prompt queue events', () => {
        ...
        expect(DAEMON_KNOWN_EVENT_TYPE_VALUES).toContain(PENDING_PROMPT_ADDED_EVENT);
        expect(DAEMON_KNOWN_EVENT_TYPE_VALUES).toContain(PENDING_PROMPT_STARTED_EVENT);
        expect(DAEMON_KNOWN_EVENT_TYPE_VALUES).toContain(PENDING_PROMPT_COMPLETED_EVENT);
    });
    ...
    it('records session updates without replacing a known session id with junk', () => {
        ...
        const state = reduceDaemonSessionEvent(createDaemonSessionViewState({ sessionId: 's-1' }), event);
    ...
    it('tracks malformed known event payloads without hiding raw events', () => {
        ...
        const state = reduceDaemonSessionEvent(createDaemonSessionViewState({ lastEventId: 7 }), rawEvent);
    ...
    it('tracks unmatched permission_already_resolved without rewriting session identity', () => {
        const state = reduceDaemonSessionEvent(createDaemonSessionViewState({ sessionId: 's-current' }), {
            id: 1,
            v: 1,
            type: 'permission_already_resolved',
            data: {
    ...
    it('rejected MCP guardrail payloads route through unrecognizedKnownEventCount', () => {
        ...
        expect(state.unrecognizedKnownEventCount).toBe(1);
        expect(state.lastUnrecognizedKnownEvent?.type).toBe('mcp_child_refused_batch');
...
describe('PR 21 — auth device-flow events', () => {
    ...
    it('reduceDaemonAuthEvent: failed event always projects status:error + errorKind (aligned with daemon)', () => {
        ...
        const expired = reduceDaemonAuthEvent(reduceDaemonAuthEvent(createDaemonAuthState(), {
            id: 1,
            v: 1,
            type: 'auth_device_flow_started',
            data: {
        ...
        const denied = reduceDaemonAuthEvent(reduceDaemonAuthEvent(createDaemonAuthState(), {
            id: 3,
            v: 1,
            type: 'auth_device_flow_started',
            data: {
        ...
        const persistFailed = reduceDaemonAuthEvent(reduceDaemonAuthEvent(createDaemonAuthState(), {
            id: 5,
            v: 1,
            type: 'auth_device_flow_started',
            data: {
    ...
    it('reduceDaemonAuthEvent rejects out-of-order frames (fold-in 8 #2 monotonicity)', () => {
        ...
        expect(replayedStartedStale.flows['qwen-oauth']?.deviceFlowId).toBe('flow-A');
    ...
    describe('PR 17 mutation events', () => {
        ...
        it('approval_mode_changed: malformed payload routes to unrecognized counter', () => {
            ...
            const next = reduceDaemonSessionEvent(createDaemonSessionViewState(), malformed);
    ...
    describe('F3 permission coordination events', () => {
        ...
        it.each([
            ...
            [
                'session_closed',
                { sessionId: 'sess-1', reason: 'client_close' },
            ],
            [
                'client_evicted',
        ...
        ])('reducer clears forbiddenVotes + forbiddenVoteCount on %s (#4335 / 3272576003)', (terminalType, terminalData) => {
            const state = reduceDaemonSessionEvents([
                {
                    id: 1,
                    v: 1,
        ...
        it('reducer preserves data.originatorClientId over envelope when both present (mergeOriginator)', () => {
            ...
            expect(state.forbiddenVotes[0]?.originatorClientId).toBe('data-side-originator');
        });
    ...
    describe('state_resync_required (#4175 F4 prereq, Ilya0527 issue #15)', () => {
        it('recognizes history_truncated and records it without entering resync', () => {
            ...
            })?.type).toBe('history_truncated');
            const state = reduceDaemonSessionEvent(createDaemonSessionViewState(), event);
        ...
        it('auto-skips delta events (session_update) while awaitingResync is true', () => {
            ...
            const afterResync = reduceDaemonSessionEvent(createDaemonSessionViewState(), {
                v: 1,
                type: 'state_resync_required',
                data: {
                    reason: 'ring_evicted',
    ...
    describe('session_snapshot (A5 #4511)', () => {
        ...
        it('uses snapshot recording state when present and preserves it for old daemons', () => {
            const degraded = reduceDaemonSessionEvent(createDaemonSessionViewState(), {
                v: 1,
                type: 'session_snapshot',
                data: {
                    sessionId: 's-1',
...
//# sourceMappingURL=daemonEvents.test.js.map