...
import { appendLocalUserTranscriptMessage, createDaemonToolPreview, createDaemonTranscriptState, createDaemonTranscriptStore, daemonUiEventToTerminalText, getOutputText, isDaemonUiSensitiveKey, normalizeDaemonEvent, reduceDaemonTranscriptEvents, sanitizeTerminalText, selectPendingPermissionBlocks, selectTranscriptBlocksOrderedByEventId, } from '../../src/daemon/ui/index.js';
describe('daemon UI normalizer and transcript reducer', () => {
    ...
    it('uses the tool name when replay starts with a tool update', () => {
        ...
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState(), events);
        expect(state.blocks).toMatchObject([
    ...
    it('stores input annotations from replayed user message chunks', () => {
        ...
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), normalizeDaemonEvent({
            id: 1,
            v: 1,
            type: 'session_update',
            data: {
    ...
    it('preserves assistant message metadata on transcript blocks', () => {
        ...
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), events, { now: 2 });
        expect(state.blocks[0]).toMatchObject({
            kind: 'assistant',
            meta: {
                source: 'background_notification',
    ...
    it('keeps discrete thought messages separate with their metadata', () => {
        const makeThought = (id, text, taskId) => normalizeDaemonEvent({
            id,
            v: 1,
            type: 'session_update',
            data: {
        ...
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), [
    ...
    it('keeps discrete assistant messages separate from normal text blocks', () => {
        ...
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), [...normalBefore, ...notification, ...normalAfter], { now: 2 });
        expect(state.blocks).toMatchObject([
    ...
    it('marks assistant streaming complete only on explicit done events', () => {
        ...
        let state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), events, { now: 2 });
        expect(state.activeAssistantBlockId).toBe('assistant-1');
        expect(state.blocks).toMatchObject([
            { kind: 'assistant', text: 'done', streaming: true },
        ]);
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.done', reason: 'stop' }], { now: 3 });
    ...
    it('drops usage with no active assistant block rather than minting one', () => {
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), [{ type: 'assistant.usage', usage: { inputTokens: 9, outputTokens: 9 } }], { now: 2 });
        expect(state.blocks).toHaveLength(0);
    });
    it('finalizes scalar thought blocks when the assistant turn finishes', () => {
        const streaming = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 100 }), [{ type: 'thought.text.delta', text: 'thinking' }], { now: 100 });
        expect(streaming.blocks).toMatchObject([
            { kind: 'thought', text: 'thinking', streaming: true },
        ]);
        const finished = reduceDaemonTranscriptEvents(streaming, [{ type: 'assistant.done', reason: 'end_turn' }], { now: 200 });
        ...
        expect(events).toMatchObject([
            {
                type: 'error',
                recoverable: true,
                text: expect.stringContaining('missing toolCallId'),
            },
        ]);
    });
    ...
    it('suppresses only matching own user echoes', () => {
        ...
        expect(normalizeDaemonEvent(event, {
            clientId: 'client-a',
    ...
    it('preserves user message metadata on transcript blocks', () => {
        ...
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), events);
        expect(state.blocks[0]).toMatchObject({
            kind: 'user',
            text: 'scheduled prompt',
            meta: { source: 'cron' },
    ...
    it('carries user shell command metadata into user shell transcript blocks', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [...commandEvents, ...outputEvents], { now: 2 });
        expect(state.blocks).toMatchObject([
            { kind: 'user', text: '$ ls' },
            {
                kind: 'user_shell',
    ...
    it('projects AskUserQuestion into a semantic tool preview', () => {
        ...
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 10 }), events, { now: 10 });
        expect(state.blocks).toMatchObject([
            {
                kind: 'tool',
                toolCallId: 'ask-1',
    ...
    it('tracks pending and resolved permissions', () => {
        let state = createDaemonTranscriptState({ now: 1 });
        state = reduceDaemonTranscriptEvents(state, normalizeDaemonEvent({
            id: 4,
            v: 1,
    ...
    it('upserts tool blocks and trims stale indexes', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'status', text: 'trim tool' }], { now: 4 });
    ...
    it('bounds trimmed tool indexes while keeping recent trimmed diagnostics', () => {
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ maxBlocks: 2, now: 1 }), Array.from({ length: 8 }, (_, index) => ({
            type: 'tool.update',
    ...
    it('keeps active assistant text open when reporting trimmed tool updates', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.text.delta', text: 'streaming' }], { now: 4 });
        const assistantBlockBeforeLateToolUpdate = state.blocks.find((block) => block.kind === 'assistant');
        expect(assistantBlockBeforeLateToolUpdate).toMatchObject({
            kind: 'assistant',
            streaming: true,
        });
        expect(state.activeAssistantBlockId).toBe(assistantBlockBeforeLateToolUpdate?.id);
        state = reduceDaemonTranscriptEvents(state, normalizeDaemonEvent({
            id: 11,
            v: 1,
            type: 'session_update',
        ...
        const assistantBlockAfterLateToolUpdate = state.blocks.find((block) => block.kind === 'assistant');
        expect(assistantBlockAfterLateToolUpdate).toMatchObject({
            kind: 'assistant',
            text: 'streaming',
            streaming: true,
        });
        expect(state.activeAssistantBlockId).toBe(assistantBlockAfterLateToolUpdate?.id);
    });
    it('preserves rich tool preview and status on output-only updates', () => {
        let state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), normalizeDaemonEvent({
            id: 41,
            v: 1,
            type: 'session_update',
            data: {
    ...
    it('caps verbose tool details from raw input and output', () => {
        ...
        expect(inputEvent).toMatchObject({
            type: 'tool.update',
            details: expect.stringContaining('[truncated]'),
        });
        expect(outputEvent).toMatchObject({
            type: 'tool.update',
            details: expect.stringContaining('[truncated]'),
        });
        expect(inputEvent && 'details' in inputEvent ? inputEvent.details?.length : 0).toBeLessThan(4200);
        expect(outputEvent && 'details' in outputEvent ? outputEvent.details?.length : 0).toBeLessThan(4200);
        expect(inputEvent && 'details' in inputEvent ? inputEvent.details : '').not.toContain('input-secret');
        expect(outputEvent && 'details' in outputEvent ? outputEvent.details : '').not.toContain('output-secret');
    });
    ...
    it('caps text transcript blocks to prevent unbounded memory growth', () => {
        ...
        expect(block && 'text' in block ? block.text.length : 0).toBeLessThanOrEqual(100_000);
    ...
    it('redacts raw daemon payloads from fallback error text', () => {
        ...
        expect(event && 'text' in event ? event.text : '').not.toContain('secret-token');
    });
    it('preserves structured model stream interruption turn errors', () => {
        expect(normalizeDaemonEvent({
            id: 44,
            v: 1,
            type: 'turn_error',
            data: {
    ...
    it('keeps structured model stream interruption on transcript blocks', () => {
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 100 }), normalizeDaemonEvent({
            id: 46,
            v: 1,
            type: 'turn_error',
            data: {
    ...
    it('normalizes daemon lifecycle and control events', () => {
        ...
        expect(malformed[0].text).not.toContain('secret');
    });
    ...
    it('rewinds to the last user turn when targetTurnIndex is out of range', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.text.delta', text: 'answer one' }], { now: 3 });
        state = appendLocalUserTranscriptMessage(state, 'second', { now: 4 });
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.text.delta', text: 'answer two' }], { now: 5 });
        state = reduceDaemonTranscriptEvents(state, [
            {
    ...
    it('rewinds to an exact user turn index', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.text.delta', text: 'answer three' }], { now: 7 });
        state = reduceDaemonTranscriptEvents(state, [
            {
                type: 'session.rewound',
                promptId: 'prompt-2',
    ...
    it('caps normalized plan content before storing it in tool content', () => {
        ...
        if (block?.kind !== 'tool')
            throw new Error('expected plan tool block');
    ...
    it('recreates synthetic plan blocks after transcript trimming', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'status', text: 'trim plan block' }], { now: 3 });
        state = reduceDaemonTranscriptEvents(state, normalizeDaemonEvent({
            id: 61,
            v: 1,
            type: 'session_update',
    ...
    it('drops resolution for trimmed permission requests (wenshao Critical)', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'status', text: 'trim permission' }], { now: 3 });
    ...
    it('keeps notifying store listeners when one listener throws', async () => {
        const store = createDaemonTranscriptStore();
        const globalWithReportError = globalThis;
    ...
    it('strips terminal control and bidi spoofing sequences', () => {
        const output = sanitizeTerminalText('\u202etxt.exe\u001b[31mred\roverwrite\u001bPhidden\u001b\\ok');
    ...
    it('sanitizes unterminated terminal control sequences without swallowing output', () => {
        const output = sanitizeTerminalText(`visible\u001b]${'x'.repeat(1000)}still-visible`);
        expect(output).toContain('visible');
        expect(output).toContain('still-visible');
    });
    it('caps nested tool preview traversal depth', () => {
        let nested = { command: 'npm test' };
    ...
    it('redacts sensitive values in generic tool previews', () => {
        expect(createDaemonToolPreview({
            apiKey: 'secret-key',
    ...
    it('recognizes common secret-key aliases before rendering previews', () => {
        ...
        ].every((key) => isDaemonUiSensitiveKey(key))).toBe(true);
        expect(createDaemonToolPreview({
    ...
    it('redacts sensitive fields in tool.update rawInput and rawOutput at normalizer boundary (wenshao CRIT #2)', () => {
        ...
        expect(event.rawInput['apiKey']).toBe('[redacted]');
        expect(event.rawInput['headers']['Authorization']).toBe('[redacted]');
        expect(event.rawOutput['token']).toBe('[redacted]');
        // Non-sensitive fields survive verbatim.
        expect(event.rawInput['command']).toBe('curl https://api.example.com');
        expect(event.rawOutput['text']).toBe('OK');
    ...
    it('redacts permission tool calls at the normalizer boundary', () => {
        ...
        expect(JSON.stringify(event)).not.toContain('Bearer permission-secret-do-not-leak');
    });
...
describe('daemon UI normalizer — Wave 3/4 event coverage (PR-A)', () => {
    function envelopeOf(type, data, id = 100) {
        return { id, v: 1, type, data };
    }
    it('normalizes session_metadata_updated into a typed session-meta event', () => {
        const events = normalizeDaemonEvent(envelopeOf('session_metadata_updated', {
            sessionId: 'sess-1',
    ...
    it('normalizes approval_mode_changed with persisted flag', () => {
        const events = normalizeDaemonEvent(envelopeOf('approval_mode_changed', {
            sessionId: 's1',
            previous: 'default',
            next: 'yolo',
            persisted: true,
    ...
    it('upgrades available_commands_update from status text to typed event', () => {
        const events = normalizeDaemonEvent(envelopeOf('session_update', {
            update: {
                sessionUpdate: 'available_commands_update',
                availableCommands: [
    ...
    it('stamps debugReason on unrecognized daemon events', () => {
        const events = normalizeDaemonEvent(envelopeOf('some_future_event', { sessionId: 's1' }));
        expect(events).toEqual([
            expect.objectContaining({
                type: 'debug',
    ...
    it('classifies a session_update with no usable discriminator as malformed', () => {
        ...
        for (const update of [
            ...
            expect(normalizeDaemonEvent(envelopeOf('session_update', { update }))).toEqual([
                expect.objectContaining({
                    type: 'debug',
    ...
    it('stamps debugReason on malformed payloads of known events', () => {
        const events = normalizeDaemonEvent(envelopeOf('memory_changed', { scope: 'not-a-scope' }));
        expect(events).toEqual([
            expect.objectContaining({
                type: 'debug',
                debugReason: 'malformed_payload',
    ...
    it('carries debugReason through the reducer onto the transcript block', () => {
        ...
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), normalizeDaemonEvent(envelopeOf('some_future_event', { sessionId: 's1' })));
        expect(state.blocks).toEqual([
            expect.objectContaining({
                kind: 'debug',
                debugReason: 'unrecognized_event',
    ...
    it('normalizes memory_changed with closed-enum scope + mode', () => {
        const events = normalizeDaemonEvent(envelopeOf('memory_changed', {
            scope: 'workspace',
    ...
    it('normalizes agent_changed for create/update/delete', () => {
        for (const change of ['created', 'updated', 'deleted']) {
            const events = normalizeDaemonEvent(envelopeOf('agent_changed', {
                change,
                name: 'reviewer',
                level: 'project',
            }));
    ...
    it('normalizes tool_toggled', () => {
        const events = normalizeDaemonEvent(envelopeOf('tool_toggled', { toolName: 'Bash', enabled: false }));
        expect(events).toEqual([
            expect.objectContaining({
    ...
    it('normalizes settings_reloaded as a settings refresh signal', () => {
        const events = normalizeDaemonEvent(envelopeOf('settings_reloaded', {
        ...
        expect(events).not.toContainEqual(expect.objectContaining({ type: 'debug' }));
    });
    it('normalizes workspace_initialized actions', () => {
        const events = normalizeDaemonEvent(envelopeOf('workspace_initialized', { path: '/w', action: 'created' }));
        expect(events).toEqual([
            expect.objectContaining({
    ...
    it('normalizes trust_change_requested', () => {
        const events = normalizeDaemonEvent(envelopeOf('trust_change_requested', {
            workspaceCwd: '/w',
    ...
    it('normalizes github_setup_completed', () => {
        const events = normalizeDaemonEvent(envelopeOf('github_setup_completed', {
            releaseTag: 'v1.2.3',
            readmeUrl: 'https://github.com/QwenLM/qwen-code-action/blob/v1.2.3/README.md#quick-start',
            workflows: [
                {
    ...
    it('normalizes mcp_budget_warning with mode enum', () => {
        const events = normalizeDaemonEvent(envelopeOf('mcp_budget_warning', {
            liveCount: 6,
            reservedCount: 2,
            budget: 8,
            thresholdRatio: 0.75,
    ...
    it('normalizes mcp_child_refused_batch with refusedServers list', () => {
        const events = normalizeDaemonEvent(envelopeOf('mcp_child_refused_batch', {
            refusedServers: [
    ...
    it('normalizes mcp_server_restarted and restart_refused', () => {
        const restarted = normalizeDaemonEvent(envelopeOf('mcp_server_restarted', {
            serverName: 'github',
            durationMs: 142,
        }));
        ...
        const refused = normalizeDaemonEvent(envelopeOf('mcp_server_restart_refused', {
            serverName: 'github',
            reason: 'in_flight',
        }));
    ...
    it('normalizes extension install lifecycle details', () => {
        const installed = normalizeDaemonEvent(envelopeOf('extensions_changed', {
            refreshed: 1,
            failed: 0,
            status: 'installed',
            source: 'owner/repo',
        ...
        const updated = normalizeDaemonEvent(envelopeOf('extensions_changed', {
            refreshed: 1,
            failed: 0,
            status: 'updated',
            name: 'test-extension',
        ...
        const failed = normalizeDaemonEvent(envelopeOf('extensions_changed', {
            refreshed: 0,
            failed: 0,
            status: 'failed',
            source: 'owner/repo',
    ...
    it('normalizes auth_device_flow lifecycle (started → throttled → authorized)', () => {
        const started = normalizeDaemonEvent(envelopeOf('auth_device_flow_started', {
            deviceFlowId: 'df-1',
            providerId: 'qwen',
        ...
        const throttled = normalizeDaemonEvent(envelopeOf('auth_device_flow_throttled', {
            deviceFlowId: 'df-1',
            intervalMs: 10_000,
        }));
        ...
        const authorized = normalizeDaemonEvent(envelopeOf('auth_device_flow_authorized', {
            deviceFlowId: 'df-1',
            providerId: 'qwen',
            accountAlias: 'alice',
        }));
    ...
    it('normalizes auth_device_flow_failed with closed-enum errorKind', () => {
        const events = normalizeDaemonEvent(envelopeOf('auth_device_flow_failed', {
            deviceFlowId: 'df-1',
    ...
    it('reducer is no-op on session-meta / workspace / auth events (no transcript blocks emitted)', () => {
        let state = createDaemonTranscriptState({ now: 1 });
        state = reduceDaemonTranscriptEvents(state, [
            ...normalizeDaemonEvent(envelopeOf('memory_changed', {
                scope: 'workspace',
                filePath: '/x',
                mode: 'replace',
                bytesWritten: 1,
            })),
            ...normalizeDaemonEvent(envelopeOf('approval_mode_changed', {
                sessionId: 's',
                previous: 'default',
                next: 'plan',
                persisted: false,
            })),
            ...normalizeDaemonEvent(envelopeOf('auth_device_flow_started', {
                deviceFlowId: 'df',
                providerId: 'qwen',
                expiresAt: 1_900_000_000_000,
            })),
...
describe('daemon UI time schema (PR-B)', () => {
    it('extracts serverTimestamp from envelope _meta and propagates to blocks', () => {
        ...
        const state = reduceDaemonTranscriptEvents(createDaemonTranscriptState({ now: 1 }), eventWithMeta, { now: 2 });
        expect(state.blocks[0]).toMatchObject({
            kind: 'assistant',
            serverTimestamp: 1_900_000_000_000,
            clientReceivedAt: 2,
    ...
    it.each([1_780_905_333_596, '1780905333596', '2026-06-08T07:55:33.596Z'])('extracts transcript-page timestamp %s', (timestamp) => {
        const events = normalizeDaemonEvent({
            v: 1,
            type: 'session_update',
            data: {
    ...
    it('selectTranscriptBlocksOrderedByEventId sorts by eventId, ignoring out-of-order arrival', async () => {
        const { selectTranscriptBlocksOrderedByEventId } = await import('../../src/daemon/ui/index.js');
        let state = createDaemonTranscriptState({ now: 1 });
    ...
    it('formatBlockTimestamp prefers serverTimestamp over clientReceivedAt', async () => {
        const { formatBlockTimestamp } = await import('../../src/daemon/ui/index.js');
        const events = normalizeDaemonEvent({
            id: 1,
            v: 1,
            type: 'session_update',
...
describe('daemon UI reducer state machine (PR-E)', () => {
    ...
    it('marks in-flight tools cancelled on application-layer cancellation (reason: cancelled)', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.done', reason: 'cancelled' }], { now: 3 });
        expect(state.currentToolCallId).toBeUndefined();
        expect(state.blocks[0]).toMatchObject({
            kind: 'tool',
            status: 'cancelled',
    ...
    it('projects history truncation as status without entering resync', () => {
        ...
        expect(events).toEqual([
            expect.objectContaining({
                ...
                text: expect.stringContaining('History truncated in replay history'),
        ...
        expect(daemonUiEventToTerminalText(events[0])).toContain('History truncated');
    });
    it('describes live truncation precisely and preserves structured data', () => {
        ...
        expect(event).toMatchObject({
            ...
            source: 'history_truncated',
            data,
            text: expect.stringContaining('kept the latest 10000 source events and dropped 16371 older source events'),
        });
        expect(event.text).toContain('limits: 10000 replay entries / 8388608 bytes');
        expect(event.text).toContain('Complete content remains available after the turn finishes.');
    });
    it('does not promise recovery when a full transcript is unavailable', () => {
        ...
        expect(event.text).toContain('not available for automatic recovery');
        expect(event.text).not.toContain('remains available after the turn finishes');
    });
    it('does not infer replay ownership for a future truncation scope', () => {
        ...
        expect(event.text).toContain('History truncated:');
        expect(event.text).not.toContain('live turn');
        expect(event.text).not.toContain('replay history');
    });
    ...
    it('explicit assistant.done without reason does NOT propagate cancellation', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.done', reason: 'end_turn' }], { now: 3 });
...
describe('daemon UI tool preview taxonomy (PR-C)', () => {
    ...
    it('detects file_read from tool name + range (lineRange)', () => {
        const preview = createDaemonToolPreview({ path: '/work/x.md', lineRange: [10, 20] }, { toolName: 'Read' });
        expect(preview).toMatchObject({
            kind: 'file_read',
            path: '/work/x.md',
            range: [10, 20],
    ...
    it('detects file_read from offset/limit pair with 1-based range conversion', () => {
        ...
        const preview = createDaemonToolPreview({ path: '/work/y.md', offset: 100, limit: 50 }, { toolName: 'View' });
        expect(preview).toMatchObject({
            kind: 'file_read',
            path: '/work/y.md',
            range: [101, 150],
    ...
    it('detects mcp_invocation from mcp__<server>__<tool> naming', () => {
        const preview = createDaemonToolPreview({ arguments: { issueTitle: 'Bug report' } }, { toolName: 'mcp__github__create_issue' });
        expect(preview).toMatchObject({
            kind: 'mcp_invocation',
            serverId: 'github',
            toolName: 'create_issue',
        });
        expect(preview.argsSummary).toContain('issueTitle');
    });
...
describe('daemon UI content extraction (PR-C)', () => {
    ...
    it('extractContentPart returns undefined for unknown content type', async () => {
        const { extractContentPart } = await import('../../src/daemon/ui/index.js');
        expect(extractContentPart({ type: 'video', source: { url: 'x' } })).toBeUndefined();
    });
...
describe('daemon UI render contract (PR-D)', () => {
    it('daemonBlockToMarkdown renders user/assistant/tool/shell/permission/error', async () => {
        const { daemonBlockToMarkdown } = await import('../../src/daemon/ui/index.js');
        let state = createDaemonTranscriptState({ now: 1 });
        state = appendLocalUserTranscriptMessage(state, 'hello', { now: 2 });
        state = reduceDaemonTranscriptEvents(state, [
            ...normalizeDaemonEvent({
    ...
    it('daemonBlockToMarkdown uses longer fences for embedded backticks', async () => {
        const { daemonToolPreviewToMarkdown } = await import('../../src/daemon/ui/index.js');
    ...
    it('daemonBlockToHtml escapes special characters in user/assistant content', async () => {
        ...
        let state = createDaemonTranscriptState({ now: 1 });
        state = appendLocalUserTranscriptMessage(state, '<script>alert("xss")</script>', { now: 2 });
    ...
    it('daemonBlockToHtml emits error blocks with role=alert', async () => {
        const { daemonBlockToHtml } = await import('../../src/daemon/ui/index.js');
        let state = createDaemonTranscriptState({ now: 1 });
        state = reduceDaemonTranscriptEvents(state, [{ type: 'error', text: 'auth required', recoverable: true }], { now: 2 });
        const html = daemonBlockToHtml(state.blocks[0]);
        expect(html).toContain('role="alert"');
        expect(html).toContain('daemon-error');
    });
    it('daemonBlockToPlainText drops markdown / html, suitable for copy-paste', async () => {
        const { daemonBlockToPlainText } = await import('../../src/daemon/ui/index.js');
        let state = createDaemonTranscriptState({ now: 1 });
        state = reduceDaemonTranscriptEvents(state, [{ type: 'error', text: 'session died' }], { now: 2 });
    ...
    it('sanitizeUrls rejects unsafe protocols and parse failures', async () => {
        const { daemonToolPreviewToMarkdown } = await import('../../src/daemon/ui/index.js');
        expect(daemonToolPreviewToMarkdown({ kind: 'web_fetch', url: 'javascript:alert(1)' }, { sanitizeUrls: true })).toBe('GET `#`');
        expect(daemonToolPreviewToMarkdown({ kind: 'web_fetch', url: 'http://[bad-url' }, { sanitizeUrls: true })).toBe('GET `#`');
    });
    ...
    it('daemonBlockToMarkdown strips ANSI and bidi controls', async () => {
        const { daemonBlockToMarkdown } = await import('../../src/daemon/ui/index.js');
        let state = createDaemonTranscriptState({ now: 1 });
        state = appendLocalUserTranscriptMessage(state, '\x1b[31mred\x1b[0m \u202Eevil', { now: 2 });
        const md = daemonBlockToMarkdown(state.blocks[0]);
...
describe('daemon UI tool preview taxonomy — long-tail kinds (PR-F)', () => {
    ...
    it('detects search with object-shaped results', () => {
        ...
        expect(preview.top).toContain('src/auth.ts');
    });
    ...
    it('detects code_block from REPL toolName even without explicit language', () => {
        const preview = createDaemonToolPreview({ code: 'print("hi")' }, { toolName: 'python_repl' });
        expect(preview).toMatchObject({
            kind: 'code_block',
            code: 'print("hi")',
        });
...
describe('daemon UI adapter conformance framework (PR-G)', () => {
    it('runs the built-in fixture corpus against SDK reference adapter (markdown projection)', async () => {
        const { runAdapterConformanceSuite, reduceDaemonTranscriptEvents: reduce, createDaemonTranscriptState: create, daemonBlockToMarkdown, } = await import('../../src/daemon/ui/index.js');
    ...
    it('failed fixtures surface missing + leaked phrases for debugging', async () => {
        const { runAdapterConformanceSuite, DAEMON_UI_CONFORMANCE_FIXTURES } = await import('../../src/daemon/ui/index.js');
        ...
        const fixturesWithExpect = DAEMON_UI_CONFORMANCE_FIXTURES.filter((fx) => fx.expectedContains.length > 0);
    ...
    it('detects redaction violations (leaked phrases in malformed-payload fixture)', async () => {
        const { runAdapterConformanceSuite } = await import('../../src/daemon/ui/index.js');
        ...
        expect(result.failed[0].leakedPhrases).toContain('must-not-leak-malformed-payload');
    });
    ...
    it('plain-text reference adapter also conforms to corpus', async () => {
        const { runAdapterConformanceSuite, reduceDaemonTranscriptEvents: reduce, createDaemonTranscriptState: create, daemonBlockToPlainText, } = await import('../../src/daemon/ui/index.js');
        const result = runAdapterConformanceSuite({
            reduce(events) {
                return reduce(create({ now: 1 }), events, { now: 2 });
            },
...
describe('daemon UI subagent nesting (PR-K, post-rebase)', () => {
    ...
    it('reducer correlates parentBlockId at create time when parent already in state', async () => {
        const { selectSubagentChildBlocks } = await import('../../src/daemon/ui/index.js');
        let state = createDaemonTranscriptState({ now: 1 });
        ...
        const parentBlock = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'parent-task');
        const childBlock = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'child-grep');
    ...
    it('reducer adopts parent context on later update if not yet correlated', () => {
        ...
        const child = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'c1');
        expect(child.parentToolCallId).toBe('p1');
        expect(child.subagentType).toBe('helper');
        expect(child.parentBlockId).toBe(state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'p1').id);
    });
    it('isSubagentChildBlock discriminates tool blocks by parentToolCallId', async () => {
        const { isSubagentChildBlock } = await import('../../src/daemon/ui/index.js');
        let state = createDaemonTranscriptState({ now: 1 });
        state = reduceDaemonTranscriptEvents(state, normalizeDaemonEvent({
            id: 1,
            v: 1,
...
describe('daemon UI subagent nesting — review hardening (R1-R4)', () => {
    ...
    it('back-fills parentBlockId when parent appears AFTER child (out-of-order)', async () => {
        ...
        const childPre = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'c-orphan');
        ...
        const parent = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'p-later');
        const childPost = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'c-orphan');
    ...
    it('back-fills parentBlockId on later child update if parent now exists', () => {
        ...
        const child = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'c2');
        const parent = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'p2');
    ...
    it('nulls dangling parentBlockId after parent is trimmed by maxBlocks', () => {
        ...
        const child = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'cT');
        if (child) {
    ...
    it('subagent-nesting fixture passes for the SDK reference adapter', async () => {
        const { runAdapterConformanceSuite, DAEMON_UI_CONFORMANCE_FIXTURES, daemonBlockToMarkdown, } = await import('../../src/daemon/ui/index.js');
        const fixture = DAEMON_UI_CONFORMANCE_FIXTURES.find((f) => f.name === 'subagent-nesting');
        expect(fixture).toBeDefined();
        const result = runAdapterConformanceSuite({
            reduce: (events) => reduceDaemonTranscriptEvents(createDaemonTranscriptState(), events),
            renderToText: (state) => state.blocks
                .map((b) => daemonBlockToMarkdown(b))
                .join('\n\n'),
...
describe('transcriptBlockToTerminalText (wenshao review — coverage)', () => {
    ...
    it('renders user block with qwen label', async () => {
        const { transcriptBlockToTerminalText } = await import('../../src/daemon/ui/index.js');
    ...
    it('degrades gracefully on unknown block kind (returns error line, does NOT throw)', async () => {
        ...
        const fauxBlock = {
            ...baseFields,
            kind: 'experimental_kind_from_future_daemon',
            payload: { something: 'unknown' },
        };
        expect(() => transcriptBlockToTerminalText(fauxBlock)).not.toThrow();
        const out = transcriptBlockToTerminalText(fauxBlock);
        expect(out).toContain('error');
...
describe('KNOWN_DEVICE_FLOW_ERROR_KINDS stays in sync with public type', async () => {
    ...
    it('only contains canonical device-flow error kinds (compile-time assertion)', async () => {
        const { KNOWN_DEVICE_FLOW_ERROR_KINDS } = await import('../../src/daemon/ui/normalizer.js');
...
describe('daemonBlockToPlainText forwards opts (wenshao review 4350741340)', () => {
    it('sanitizes URL on tool preview when opts.sanitizeUrls is set', async () => {
        const { daemonBlockToPlainText, createDaemonToolPreview } = await import('../../src/daemon/ui/index.js');
        const block = {
            id: 'b',
            kind: 'tool',
            toolCallId: 't',
            title: 'fetch',
            status: 'completed',
            preview: createDaemonToolPreview({ url: 'https://api.example.com/x?token=SECRET&q=keep', method: 'GET' }, { toolName: 'WebFetch', toolKind: 'tool' }),
            clientReceivedAt: 1,
            createdAt: 1,
            updatedAt: 1,
        };
...
describe('daemonBlockToHtml — additional coverage (wenshao R3 qwen3.7-max)', () => {
    ...
    it('strips token query param + Basic Auth from web_fetch URL when sanitizeUrls:true', async () => {
        const { daemonBlockToHtml, createDaemonToolPreview } = await import('../../src/daemon/ui/index.js');
        const block = {
            ...baseFields,
            kind: 'tool',
            toolCallId: 't',
    ...
    it('protocol-validates thumbnailUrl even when sanitizeUrls:false', async () => {
        const { daemonBlockToMarkdown, createDaemonToolPreview } = await import('../../src/daemon/ui/index.js');
        const block = {
            ...baseFields,
            kind: 'tool',
            toolCallId: 't',
            title: 'gen',
            status: 'completed',
            preview: createDaemonToolPreview({ prompt: 'hi', thumbnailUrl: 'javascript:alert(1)' }, { toolName: 'image_generator', toolKind: 'tool' }),
        };
    ...
    it('renders thought / debug / status blocks', async () => {
        const { daemonBlockToHtml } = await import('../../src/daemon/ui/index.js');
        expect(daemonBlockToHtml({ ...baseFields, kind: 'thought', text: 't' })).toContain('class="daemon-block daemon-thought"');
        expect(daemonBlockToHtml({ ...baseFields, kind: 'debug', text: 'd' })).toContain('class="daemon-block daemon-debug"');
        expect(daemonBlockToHtml({ ...baseFields, kind: 'status', text: 's' })).toContain('class="daemon-block daemon-status"');
    });
});
describe('Trimmed tool cancellation propagation (wenshao R3 qwen3.7-max)', () => {
    it('skips trimmed sentinel entries without throwing', () => {
        ...
        expect(() => {
            state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.done', reason: 'cancelled' }], { now: 4 });
        }).not.toThrow();
    });
    it('does NOT cancel in-flight tools on stream_ended / reconnected reasons', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.done', reason: 'stream_ended' }], { now: 3 });
        const block = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'flight');
...
describe('Late permission.resolved after sentinel pruned (wenshao R3 qwen3.7-max)', () => {
    it('drops resolution silently when permissionBlockByRequestId entry was pruned', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'status', text: 'flush' }], { now: 100 });
        const blocksBefore = state.blocks.length;
...
describe('ensureSafeImageUrl tightened to data:image/* (audit follow-up)', () => {
    it('allows http/https/data:image/* but rejects data:text/html', async () => {
        const { daemonBlockToMarkdown, createDaemonToolPreview } = await import('../../src/daemon/ui/index.js');
        const mkBlock = (thumbnailUrl) => ({
            id: 'b',
            kind: 'tool',
            toolCallId: 't',
            title: 'gen',
            status: 'completed',
            preview: createDaemonToolPreview({ prompt: 'p', thumbnailUrl }, { toolName: 'image_generator', toolKind: 'tool' }),
            clientReceivedAt: 1,
            createdAt: 1,
            updatedAt: 1,
        });
        // https → passthrough
        expect(daemonBlockToMarkdown(mkBlock('https://cdn.example.com/x.png'))).toContain('cdn.example.com');
        // data:image/png → passthrough
        expect(daemonBlockToMarkdown(mkBlock('data:image/png;base64,iVBORw0KGgo='))).toContain('data:image/png');
        // data:text/html → rejected to '#'
        expect(daemonBlockToMarkdown(mkBlock('data:text/html,<script>alert(1)</script>'))).toContain('![image](#)');
        // javascript: → rejected to '#'
        expect(daemonBlockToMarkdown(mkBlock('javascript:alert(1)'))).toContain('![image](#)');
    });
});
describe('R5 review batch — coverage additions', () => {
    ...
    it('sanitizeUrl strips AWS / GCP / Azure SAS credential params', async () => {
        ...
        const mkBlock = (url) => ({
            ...
            title: 'fetch',
            status: 'completed',
            preview: createDaemonToolPreview({ url, method: 'GET' }, { toolName: 'WebFetch', toolKind: 'tool' }),
            clientReceivedAt: 1,
            createdAt: 1,
            updatedAt: 1,
        });
        // AWS S3 presigned
        const aws = daemonBlockToMarkdown(mkBlock('https://bucket.s3.amazonaws.com/x?AWSAccessKeyId=AKIA_LEAK&Expires=1234&Signature=SIG_LEAK'), { sanitizeUrls: true });
        expect(aws).not.toContain('AKIA_LEAK');
        expect(aws).not.toContain('SIG_LEAK');
        // GCP signed URL
        const gcp = daemonBlockToMarkdown(mkBlock('https://storage.googleapis.com/b/o?GoogleAccessId=svc_LEAK@proj.iam.gserviceaccount.com&Expires=999&Signature=GCP_LEAK'), { sanitizeUrls: true });
        expect(gcp).not.toContain('svc_LEAK');
        expect(gcp).not.toContain('GCP_LEAK');
        // Azure SAS
        const az = daemonBlockToMarkdown(mkBlock('https://acct.blob.core.windows.net/c/x?sv=2020-08-04&se=2026-12-31&sig=AZ_LEAK&sp=r'), { sanitizeUrls: true });
        expect(az).not.toContain('AZ_LEAK');
    });
    it('formatMissedRange handles no-gap / single-event / multi-event', async () => {
        const { formatMissedRange } = await import('../../src/daemon/ui/transcript.js');
    ...
    it('detectFileDiff content alias rejected for non-write tools', async () => {
        const { createDaemonToolPreview } = await import('../../src/daemon/ui/index.js');
        // `{ path, content }` with READ-like tool name → NOT file_diff
        const read = createDaemonToolPreview({ path: '/x', content: 'expected text' }, { toolName: 'read_file' });
        expect(read.kind).not.toBe('file_diff');
        // Same shape with WRITE-like tool name → IS file_diff
        const write = createDaemonToolPreview({ path: '/x', content: 'new content' }, { toolName: 'write_file' });
        expect(write.kind).toBe('file_diff');
    });
    it('writeIntent regex word-boundary: prewrite_check does NOT match write', async () => {
        const { createDaemonToolPreview } = await import('../../src/daemon/ui/index.js');
        const preview = createDaemonToolPreview({ path: '/x', content: 'data' }, { toolName: 'prewrite_check' });
    ...
    it('store.clearAwaitingResync clears latch', async () => {
        const { createDaemonTranscriptStore } = await import('../../src/daemon/ui/index.js');
        const store = createDaemonTranscriptStore();
        store.dispatch({
...
describe('R6 review batch — recovery flow + pending pointer', () => {
    it('newly-created tool block with undefined status sets currentToolCallId to its default `pending`', () => {
        ...
        const block = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'unspecified');
    ...
    it('clearAwaitingResync FIRST then dispatch new events: events flow', async () => {
        ...
        store.dispatch(normalizeDaemonEvent({
            id: 100,
            v: 1,
            type: 'session_update',
            data: {
        ...
        const text = store
            .getSnapshot()
            .blocks.map((b) => b.kind === 'assistant' ? b.text : '')
            .join('');
...
describe('R7 review batch — markdown escape + details sanitization', () => {
    it('escapeMarkdownText escapes < in metadata fields (titles/kinds) for HTML-backed pipelines', async () => {
        ...
        const block = {
            ...
            status: 'running',
            preview: createDaemonToolPreview({ command: 'echo hi' }, { toolName: 'Bash', toolKind: 'tool' }),
            clientReceivedAt: 1,
            createdAt: 1,
            updatedAt: 1,
        };
    ...
    it('markdown tool block details strips URL credentials when sanitizeUrls:true', async () => {
        ...
        const block = {
            ...
            title: 'Fetch',
            status: 'running',
            preview: createDaemonToolPreview({ url: 'https://api.example.com/v1', method: 'GET' }, { toolName: 'WebFetch', toolKind: 'tool' }),
...
describe('cross-client event recognition (prompt_cancelled / replay_complete)', () => {
    ...
    it('prompt.cancelled clears in-flight tool spinners in the reducer', () => {
        ...
        const block = state.blocks.find((b) => b.kind === 'tool' && b.toolCallId === 'running-tool');
...
describe('permission_resolved voterClientId (A4)', () => {
    ...
    it('distinguishes the prompt originator (request) from the voter (resolved) when they differ', () => {
        ...
        expect(resolved.voterClientId).not.toBe(request.originatorClientId);
    });
});
...
describe('permission_request normalization for Agent tools', () => {
    it('keeps Agent toolCall metadata on permission.request without synthesizing tool.update', () => {
        ...
        const permissionEvent = events[0];
    ...
    it('does not create SDK tool blocks from Agent permission_request', () => {
        ...
        const subToolBlocks = state.blocks.filter((b) => b.kind === 'tool' && b.toolCallId === 'call_subtool_1');
        expect(subToolBlocks).toHaveLength(1);
        expect(subToolBlocks[0]).toMatchObject({
            parentToolCallId: 'call_agent_x',
        });
        expect(subToolBlocks[0].parentBlockId).toBe(undefined);
    });
});
...
describe('parallel subAgent text interleaving fix', () => {
    it('drops subagent detail blocks while retaining parent usage', () => {
        ...
        expect(state.blocks[1].rawOutput).not.toMatchObject({
    ...
    it('T1: separates text chunks by parentToolCallId into independent blocks', () => {
        ...
        const assistantBlocks = state.blocks.filter((b) => b.kind === 'assistant');
    ...
    it('T2: scoped clearActiveText — subAgent A tool does not interrupt subAgent B text', () => {
        ...
        const bBlocks = state.blocks.filter((b) => b.kind === 'assistant' &&
            b.parentToolCallId === 'task-B');
    ...
    it('T3: finishAssistant sets streaming=false on all keyed-map blocks', () => {
        ...
        const before = state.blocks.filter((b) => b.kind === 'assistant');
        expect(before[0].streaming).toBe(true);
        expect(before[1].streaming).toBe(true);
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.done', reason: 'stop' }], { now: 3 });
        const after = state.blocks.filter((b) => b.kind === 'assistant');
    ...
    it('T3b: finishAssistant sets streaming=false on keyed thought blocks', () => {
        ...
        const before = state.blocks.filter((b) => b.kind === 'thought');
        expect(before[0].streaming).toBe(true);
        expect(before[1].streaming).toBe(true);
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.done', reason: 'stop' }], { now: 3 });
        const after = state.blocks.filter((b) => b.kind === 'thought');
        expect(after[0].streaming).toBe(false);
        expect(after[1].streaming).toBe(false);
    ...
    it('T5: keyed and scalar paths coexist without interference', () => {
        ...
        const assistantBlocks = state.blocks.filter((b) => b.kind === 'assistant');
        expect(assistantBlocks).toHaveLength(2);
        const subagentBlock = assistantBlocks.find((b) => b.parentToolCallId === 'task-X');
        const topLevelBlock = assistantBlocks.find((b) => b.parentToolCallId === undefined);
    ...
    it('T7: appendLocalUserTranscriptMessage clears active subAgent text maps', () => {
        ...
        expect(state.activeThoughtBlockByParent).toEqual({});
        expect(state.blocks.find((block) => block.kind === 'assistant' && block.parentToolCallId === 'task-Y')).toMatchObject({ streaming: false, updatedAt: 3 });
    });
    it('T8: thought evicts assistant block and finalizes streaming for same parent', () => {
        ...
        const beforeBlocks = state.blocks.filter((b) => b.kind === 'assistant');
        ...
        const afterBlocks = state.blocks.filter((b) => b.kind === 'assistant');
    ...
    it('T12: assistant evicts thought block for same parent', () => {
        ...
        expect(state.blocks.find((block) => block.kind === 'thought')).toMatchObject({ streaming: false, updatedAt: 3 });
    });
    it('T12b: scalar assistant finalizes previous scalar thought block', () => {
        let state = createDaemonTranscriptState({ now: 1 });
        state = reduceDaemonTranscriptEvents(state, [{ type: 'thought.text.delta', text: 'thinking first' }], { now: 2 });
        expect(state.activeThoughtBlockId).toBeDefined();
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.text.delta', text: 'now responding' }], { now: 3 });
    ...
    it('T12c: scalar thought finalizes previous scalar assistant block', () => {
        let state = createDaemonTranscriptState({ now: 1 });
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.text.delta', text: 'answer first' }], { now: 2 });
        expect(state.activeAssistantBlockId).toBeDefined();
        state = reduceDaemonTranscriptEvents(state, [{ type: 'thought.text.delta', text: 'thinking next' }], { now: 3 });
        expect(state.activeAssistantBlockId).toBeUndefined();
        expect(state.activeThoughtBlockId).toBeDefined();
        expect(state.blocks.find((block) => block.kind === 'assistant')).toMatchObject({ streaming: false, updatedAt: 3 });
    });
    it('T12d: scalar user text finalizes previous scalar assistant block', () => {
        let state = createDaemonTranscriptState({ now: 1 });
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.text.delta', text: 'answer first' }], { now: 2 });
        expect(state.activeAssistantBlockId).toBeDefined();
        state = reduceDaemonTranscriptEvents(state, [{ type: 'user.text.delta', text: 'new question' }], { now: 3 });
        expect(state.activeAssistantBlockId).toBeUndefined();
    ...
    it('T13: scoped clearActiveText sets streaming=false on cleared block', () => {
        ...
        const aBlocks = state.blocks.filter((b) => b.kind === 'assistant' &&
            b.parentToolCallId === 'task-A');
    ...
    it('T13b: scoped clearActiveText finalizes assistant and thought for the same parent', () => {
        ...
        expect(state.blocks.find((block) => block.kind === 'assistant' && block.parentToolCallId === 'task-A')).toMatchObject({ streaming: false });
        expect(state.blocks.find((block) => block.kind === 'thought' && block.parentToolCallId === 'task-B')).toMatchObject({ streaming: false });
    ...
    it('T14: scoped clearActiveText preserves scalar activeAssistantBlockId', () => {
        ...
        const scalarBlock = state.blocks.find((b) => b.kind === 'assistant' &&
            b.parentToolCallId === undefined);
        expect(scalarBlock?.streaming).toBe(true);
        expect(state.activeAssistantBlockId).toBeDefined();
        const keyedBlock = state.blocks.find((b) => b.kind === 'assistant' &&
            b.parentToolCallId === 'task-K');
    ...
    it('T15: finishAssistant finalizes both scalar and keyed blocks', () => {
        ...
        state = reduceDaemonTranscriptEvents(state, [{ type: 'assistant.done', reason: 'stop' }], { now: 3 });
        const allAssistant = state.blocks.filter((b) => b.kind === 'assistant');
...
//# sourceMappingURL=daemonUi.test.js.map