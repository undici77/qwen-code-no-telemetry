...
import { matchRoute, synthesizeResponse, jsonRpcErrorToHttpStatus, } from '../../src/daemon/acpTransportUtils.js';
// ---------------------------------------------------------------------------
...
describe('AcpWsTransport', () => {
    ...
    describe('constructor', () => {
        ...
        it('uses REST capabilities instead of the ACP initialize envelope', async () => {
            ...
            const restFetch = vi.fn(async () => synthesizeResponse(200, {
                v: 1,
                mode: 'http-bridge',
        ...
        it('falls back for a malformed 200 and preserves REST errors', async () => {
            ...
            const restFetch = vi
                .fn()
                .mockResolvedValueOnce(synthesizeResponse(200, { index: 'spa fallback, no features' }))
                .mockResolvedValueOnce(synthesizeResponse(401, { error: 'unauthorized' }));
            Object.defineProperty(globalThis, 'fetch', {
                configurable: true,
                value: restFetch,
            });
            ...
            try {
                ...
                const unauthorized = await transport.fetch('http://daemon/capabilities', { method: 'GET' });
    ...
    describe('dispose()', () => {
        ...
        it('fetch throws after dispose', async () => {
            const transport = new AcpWsTransport('ws://host/acp');
            transport.dispose();
            await expect(transport.fetch('http://h/capabilities', { method: 'GET' })).rejects.toThrow(DaemonTransportClosedError);
        });
...
describe('AcpWsTransport – route mapping used by fetch()', () => {
    ...
    it('POST /session/:id/load extracts sessionId', () => {
        ...
        expect(result).not.toBeNull();
        const params = result.mapping.extractParams(result.segments, { checkpoint: 5 }, 'POST');
    ...
    it('PATCH /session/:id/metadata extracts sessionId', () => {
        ...
        expect(result).not.toBeNull();
        const params = result.mapping.extractParams(result.segments, { title: 'new' }, 'PATCH');
        expect(params.sessionId).toBe('s5');
        expect(params.title).toBe('new');
    });
});
//# sourceMappingURL=AcpWsTransport.test.js.map