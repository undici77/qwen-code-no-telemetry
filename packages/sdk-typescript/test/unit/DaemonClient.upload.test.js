...
import { DaemonClient, DaemonHttpError, } from '../../src/daemon/DaemonClient.js';
...
describe('uploadWorkspaceFile', () => {
    ...
    it('POSTs octet-stream bytes with the path in the query string', async () => {
        const { fetch, calls } = recordingFetch(() => jsonResponse(201, uploadResult));
        const client = new DaemonClient({ baseUrl: 'http://daemon', fetch });
        await expect(client.uploadWorkspaceFile({ path: 'blob.bin', data: new Uint8Array([1, 2, 3, 4]) }, 'client-1')).resolves.toEqual(uploadResult);
    ...
    it('uses direct REST when an ACP transport is configured', async () => {
        const { fetch: restFetch, calls } = recordingFetch(() => jsonResponse(201, uploadResult));
        const transportFetch = vi.fn(async () => jsonResponse(404, { error: 'ACP route not found' }));
        const transport = {
            type: 'acp-http',
            supportsReplay: true,
            connected: true,
        ...
        await expect(client.uploadWorkspaceFile({
            path: 'blob.bin',
    ...
    it('rejects a non-JSON 2xx fetch body with the same labeled error as XHR', async () => {
        ...
        const { fetch } = recordingFetch(() => new Response('<html>interstitial</html>', { status: 200 }));
        const client = new DaemonClient({ baseUrl: 'http://daemon', fetch });
        await expect(client.uploadWorkspaceFile({
            path: 'a.bin',
            data: new Uint8Array([1]),
    ...
    it('uses the workspace-qualified route via workspaceByCwd', async () => {
        ...
        expect(calls[0]?.url).toBe('http://daemon/workspaces/%2Frepo/file/upload?path=a.bin');
    });
    ...
    it('inherits the client timeout when timeoutMs is omitted', 
    ...
    { timeout: 5_000 }, async () => {
        vi.useFakeTimers();
        try {
            const fetch = vi.fn(async (_input, init) => await new Promise((_resolve, reject) => {
    ...
    describe('progress uploads over XMLHttpRequest', () => {
        ...
        it('builds the request and maps upload progress events', async () => {
            ...
            expect(xhr.open).toHaveBeenCalledWith('POST', 'http://daemon/file/upload?path=a.bin');
            expect(xhr.setRequestHeader).toHaveBeenCalledWith('Content-Type', 'application/octet-stream');
            expect(xhr.setRequestHeader).toHaveBeenCalledWith('X-Qwen-Client-Id', 'client-1');
...
//# sourceMappingURL=DaemonClient.upload.test.js.map