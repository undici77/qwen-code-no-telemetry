...
import { projectChatRecordsToDaemonTranscript, TranscriptProjectionInputError, } from '../../src/daemon/transcript.js';
function record(uuid, parentUuid, overrides = {}) {
    return {
        uuid,
        parentUuid,
        sessionId: 'session-1',
...
describe('projectChatRecordsToDaemonTranscript', () => {
    ...
    it('finalizes earlier assistant blocks when record boundaries prevent merging', () => {
        ...
        const assistantBlocks = projection.blocks.filter((block) => block.kind === 'assistant');
        expect(assistantBlocks).toHaveLength(2);
        expect(assistantBlocks.every((block) => block.streaming === false)).toBe(true);
    });
    ...
    it('keeps Vision Bridge disclosure in rendered tool content', () => {
        ...
        expect(projection.blocks.find((block) => block.kind === 'tool')).toEqual(expect.objectContaining({
            content: expect.arrayContaining([
                {
                    type: 'content',
                    content: {
                        type: 'text',
                        text: 'Transcribed PDF pages 1-2\n' +
    ...
    it('treats persisted identifiers as data, including prototype names', () => {
        ...
        identifiers.forEach((callId, index) => {
            ...
            records.push(record(resultUuid, startUuid, {
                type: 'tool_result',
                message: {
                    role: 'user',
                    parts: [
        ...
        expect(projection.blocks
            .filter((block) => block.kind === 'tool')
            .map((block) => block.toolCallId)).toEqual(identifiers);
    ...
    it('preserves assistant usage when the record ends with a tool call', () => {
        ...
        expect(projection.blocks.find((block) => block.kind === 'assistant')).toMatchObject({
            kind: 'assistant',
            text: 'I will read it',
    ...
    it('preserves each todo plan snapshot as a separate block', () => {
        ...
        const plans = projection.blocks.filter((block) => block.kind === 'tool' && block.toolName === 'todo_write');
    ...
    it('reports block and text truncation without scanning visible text', () => {
        ...
        expect(blocks.diagnostics).toContainEqual(expect.objectContaining({
        ...
        expect(text.diagnostics).toContainEqual(expect.objectContaining({
        ...
        expect(boundary.diagnostics).toContainEqual(expect.objectContaining({ code: 'transcript_text_truncated' }));
    });
    it('returns an empty complete projection for empty and artifact-only input', () => {
        ...
        expect(artifact.diagnostics).toContainEqual(expect.objectContaining({ code: 'artifact_only' }));
    });
    it('rejects invalid options, mixed sessions, and missing explicit leaves', () => {
        expect(() => projectChatRecordsToDaemonTranscript([], { maxBlocks: 0 })).toThrowError(expect.objectContaining({
            code: 'invalid_max_blocks',
        }));
        expect(() => projectChatRecordsToDaemonTranscript([
            record('a', null),
            record('b', 'a', { sessionId: 'session-2' }),
        ])).toThrowError(expect.objectContaining({
            code: 'mixed_session_ids',
        }));
        expect(() => projectChatRecordsToDaemonTranscript([record('a', null)], {
            leafUuid: 'missing',
        })).toThrowError(expect.objectContaining({
            code: 'leaf_not_found',
        }));
...
//# sourceMappingURL=daemon-transcript-projection.test.js.map