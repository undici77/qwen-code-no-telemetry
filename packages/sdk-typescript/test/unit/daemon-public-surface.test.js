...
import { DAEMON_KNOWN_EVENT_TYPE_VALUES, PENDING_PROMPT_ADDED_EVENT, PENDING_PROMPT_STARTED_EVENT, PENDING_PROMPT_COMPLETED_EVENT, asKnownDaemonEvent, } from '../../src/daemon/events.js';
...
describe('public SDK entry — typed daemon event surface (#4217)', () => {
    it('exports the runtime narrow + reducer surface', () => {
        ...
        expect(Public.PENDING_PROMPT_STARTED_EVENT).toBe(PENDING_PROMPT_STARTED_EVENT);
        expect(Public.PENDING_PROMPT_COMPLETED_EVENT).toBe(PENDING_PROMPT_COMPLETED_EVENT);
    ...
    it('exposes the typed event schema types at the public entry (compile-time)', () => {
        ...
        expect(typeof Public.DaemonClient.prototype.setWorkspaceSkillsEnabled).toBe('function');
        expectTypeOf().toEqualTypeOf();
        expectTypeOf().toEqualTypeOf();
        expectTypeOf().toEqualTypeOf();
