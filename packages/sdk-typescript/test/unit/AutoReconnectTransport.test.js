...
describe('AutoReconnectTransport', () => {
    ...
    describe('reconnect', () => {
        ...
        it('propagates non-transport errors without reconnecting', async () => {
            ...
            await expect(transport.fetch('http://d/health', { method: 'GET' })).rejects.toThrow('network error');
            transport.dispose();
        });
        it('uses factory for reconnect when available', async () => {
            ...
            const factory = vi.fn(async (_type) => {
                return newTransport;
            });
            const transport = new AutoReconnectTransport({
                baseUrl: 'http://d',
        ...
        it('falls back to REST when factory fails', async () => {
            ...
            const mockFetch = vi.fn(async () => new Response('{"rest":true}', { status: 200 }));
            const transport = new AutoReconnectTransport({
                baseUrl: 'http://d',
                initial: failingInner,
                factory,
...
//# sourceMappingURL=AutoReconnectTransport.test.js.map