...
import { prepareSpawnInfo, findBundledCliPath, } from '../../src/utils/cliPath.js';
// Mock fs module
vi.mock('node:fs');
...
describe('CLI Path Utilities', () => {
    ...
    describe('prepareSpawnInfo', () => {
        ...
        describe('file path resolution', () => {
            ...
            it('should throw when file path does not exist', () => {
                mockFs.existsSync.mockReturnValue(false);
                expect(() => prepareSpawnInfo('/nonexistent/path')).toThrow('Executable file not found at');
            });
            it('should throw when path is a directory', () => {
                ...
                expect(() => prepareSpawnInfo('/path/to/directory')).toThrow('exists but is not a file');
            });
        });
    });
    ...
    describe('error cases', () => {
        it('should throw for empty string', () => {
            expect(() => prepareSpawnInfo('')).toThrow('Executable path cannot be empty');
        });
        ...
        it('should provide helpful error for missing file', () => {
            mockFs.existsSync.mockReturnValue(false);
            expect(() => prepareSpawnInfo('/missing/file')).toThrow('Executable file not found at');
            expect(() => prepareSpawnInfo('/missing/file')).toThrow('Please check the file path and ensure the file exists');
        });
    });
