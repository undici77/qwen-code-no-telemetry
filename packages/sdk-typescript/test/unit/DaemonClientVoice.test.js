...
describe('DaemonClient voice helpers', () => {
    ...
    it('POSTs workspace voice settings with client identity', async () => {
        ...
        const client = new DaemonClient({ baseUrl: 'http://daemon', fetch });
        await client.setWorkspaceVoice({ enabled: true, mode: 'tap', language: 'english' }, 'client-1');
        expect(calls[0]?.url).toBe('http://daemon/workspace/voice');
    ...
    it('POSTs binary voice audio with content type and optional voice model', async () => {
        ...
        await expect(client.transcribeWorkspaceVoice(audio, {
            mimeType: 'audio/wav',
            voiceModel: 'qwen3-asr-flash',
            clientId: 'client-1',
        })).resolves.toEqual(response);
        expect(calls[0]?.url).toBe('http://daemon/workspace/voice/transcribe?voiceModel=qwen3-asr-flash');
        expect(calls[0]?.method).toBe('POST');
    ...
    it('uses the encoded workspace selector for qualified Voice requests', async () => {
        ...
        const { fetch, calls } = recordingFetch((request) => request.url.includes('/transcribe')
        ...
        await expect(workspace.setWorkspaceVoice({ enabled: true }, 'client-2')).resolves.toEqual(status);
        await expect(workspace.transcribeWorkspaceVoice(new Uint8Array([1, 2]), {
            mimeType: 'audio/wav',
    ...
    it('uses the REST endpoint for binary voice audio when ACP transport is configured', async () => {
        ...
        await expect(client.transcribeWorkspaceVoice(new Uint8Array([1, 2, 3]), {
            mimeType: 'audio/wav',
        })).resolves.toEqual(response);
    ...
    it('uses REST for every qualified Voice method when ACP transport is configured', async () => {
        ...
        const { fetch, calls } = recordingFetch((request) => jsonResponse(200, request.url.endsWith('/transcribe') ? transcription : status));
        const acpTransport = {
            type: 'acp-http',
            supportsReplay: false,
            connected: true,
        ...
        await expect(workspace.setWorkspaceVoice({ enabled: false })).resolves.toEqual(status);
        await expect(workspace.transcribeWorkspaceVoice(new Uint8Array([1]), {
            mimeType: 'audio/wav',
        })).resolves.toEqual(transcription);
        expect(acpTransport.fetch).not.toHaveBeenCalled();
...
//# sourceMappingURL=DaemonClientVoice.test.js.map