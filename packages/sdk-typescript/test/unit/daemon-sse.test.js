...
async function collect(iter, max = 100) {
    const out = [];
...
describe('parseSseStream', () => {
    ...
    it('skips comment lines and retry directives', async () => {
        const stream = bodyFromString('retry: 3000\n\n' +
            ': heartbeat\n\n' +
    ...
    it('still parses a frame whose FIRST line is a comment / retry (BRgq-)', async () => {
        ...
        const stream = bodyFromString(': intermediary keep-alive\nid: 1\nevent: x\ndata: {"id":1,"v":1,"type":"x","data":"ok"}\n\n' +
    ...
    it('skips frames whose `id` is present but not a safe integer (BSP1-)', async () => {
        ...
        const stream = bodyFromString('data: {"id":"1","v":1,"type":"x","data":"ok"}\n\n' + // string id
    ...
    it('abort during read() returns cleanly instead of rethrowing (BlqF_)', async () => {
        ...
        const body = new ReadableStream({
            start(streamController) {
                ...
                controller.signal.addEventListener('abort', () => {
                    streamController.error(new DOMException('BodyStreamBuffer was aborted', 'AbortError'));
                });
            },
        });
        const events = [];
        ...
        if (!firstFrame.done)
            events.push(firstFrame.value.id ?? -1);
        controller.abort();
    ...
    it('flushes the TextDecoder on stream close so the last UTF-8 char is preserved', async () => {
        ...
        const events = await collect(parseSseStream(stream));
        expect(events).toHaveLength(1);
        expect(events[0]?.data).toBe('中');
    });
});
//# sourceMappingURL=daemon-sse.test.js.map