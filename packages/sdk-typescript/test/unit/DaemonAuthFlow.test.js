...
import { DaemonHttpError, } from '../../src/daemon/DaemonClient.js';
function makeFakeClient(opts) {
    const calls = { start: 0, get: [], cancel: [] };
    ...
    const fake = {
        ...
        async getDeviceFlow(deviceFlowId, callOpts = {}) {
            calls.get.push({
                deviceFlowId,
        ...
        async cancelDeviceFlow(deviceFlowId, callOpts = {}) {
            calls.cancel.push({
                deviceFlowId,
                ...(callOpts.clientId !== undefined
                    ? { clientId: callOpts.clientId }
...
describe('DaemonAuthFlow.awaitCompletion (fold-in 10 #2)', () => {
    ...
    it('rethrows non-404 DaemonHttpErrors so the SDK consumer sees the daemon-side failure', async () => {
        ...
        const auth = new DaemonAuthFlow(client);
        const handle = await auth.start({ providerId: 'qwen-oauth' });
        await expect(handle.awaitCompletion({ pollOverrideMs: 1_000 })).rejects.toBeInstanceOf(DaemonHttpError);
    });
});
...
//# sourceMappingURL=DaemonAuthFlow.test.js.map