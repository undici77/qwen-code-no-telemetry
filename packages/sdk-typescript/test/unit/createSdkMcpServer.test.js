...
describe('createSdkMcpServer', () => {
    describe('Server Creation', () => {
        ...
        it('should throw error with invalid name', () => {
            expect(() => createSdkMcpServer({ name: '', version: '1.0.0' })).toThrow('MCP server name must be a non-empty string');
        });
        it('should throw error with invalid version', () => {
            expect(() => createSdkMcpServer({ name: 'test', version: '' })).toThrow('MCP server version must be a non-empty string');
        });
    ...
    describe('Tool Registration', () => {
        it('should register single tool', () => {
            const testTool = tool('test_tool', 'A test tool', { input: z.string() }, async () => ({
    ...
    describe('Tool Handler Invocation', () => {
        it('should invoke tool handler with correct input', async () => {
            ...
            const testTool = tool('test_tool', 'A test tool', { value: z.string() }, handler);
            createSdkMcpServer({
                name: 'test-server',
                version: '1.0.0',
                tools: [testTool],
    ...
    describe('Type Safety', () => {
        it('should preserve input type in handler', async () => {
            ...
            const typedTool = tool('typed_tool', 'A typed tool', {
                name: z.string(),
                age: z.number(),
            }, handler);
            const server = createSdkMcpServer({
    ...
    describe('Error Handling in Tools', () => {
        ...
        it('should handle synchronous tool handler errors', async () => {
            ...
            const errorTool = tool('sync_error_tool', 'A tool that errors synchronously', {}, handler);
            const server = createSdkMcpServer({
                name: 'test-server',
                version: '1.0.0',
                tools: [errorTool],
    ...
    describe('Complex Tool Scenarios', () => {
        it('should support tool with complex input schema', () => {
            const complexTool = tool('complex_tool', 'A tool with complex schema', {
                query: z.string(),
                filters: z
                    .object({
        ...
        it('should support tool returning complex output', () => {
            const complexOutputTool = tool('complex_output_tool', 'Returns complex data', {}, async () => {
                return {
                    content: [
                        {
                            type: 'text',
...
//# sourceMappingURL=createSdkMcpServer.test.js.map