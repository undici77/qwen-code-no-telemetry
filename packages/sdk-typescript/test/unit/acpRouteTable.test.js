...
describe('acpRouteTable – matchRoute', () => {
    ...
    it('POST /session passes body through as params', () => {
        const result = matchRoute('/session', 'POST');
        const params = result.mapping.extractParams(result.segments, { model: 'gpt-4' }, 'POST');
        expect(params).toEqual({ model: 'gpt-4' });
    });
    it('POST /session maps sessionId into ACP metadata', () => {
        const result = matchRoute('/session', 'POST');
        const params = result.mapping.extractParams(result.segments, {
    ...
    it('POST /session with non-record body returns empty params', () => {
        const result = matchRoute('/session', 'POST');
        const params = result.mapping.extractParams(result.segments, 'not-an-object', 'POST');
        expect(params).toEqual({});
    });
    ...
    it('POST /session/:id/prompt extracts sessionId', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { message: 'hello' }, 'POST');
    ...
    it('POST /session/:id/load maps to session/load', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { resumeFrom: 5 }, 'POST');
    ...
    it('POST /session/:id/permission/:reqId maps to session/permission', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { allow: true }, 'POST');
        expect(params).toEqual({
            sessionId: 's5',
            requestId: 'req-7',
            allow: true,
    ...
    it('POST /permission/:reqId maps to session/permission', () => {
        ...
        expect(result).not.toBeNull();
        expect(result.mapping.method).toBe('session/permission');
        const params = result.mapping.extractParams(result.segments, { allow: false }, 'POST');
    ...
    it('GET /workspace/:id/sessions maps to session/list', () => {
        ...
        const params = result.mapping.extractParams(result.segments, undefined, 'GET', new URLSearchParams('size=50&archiveState=archived&cursor=123'));
        expect(params).toEqual({
            workspaceCwd: '/work/a',
            cursor: '123',
    ...
    it('GET /workspace/:id/sessions extracts organized view filters', () => {
        const result = matchRoute('/workspace/%2Fwork%2Fa/sessions', 'GET');
        expect(result).not.toBeNull();
        const params = result.mapping.extractParams(result.segments, undefined, 'GET', new URLSearchParams('view=organized&group=pinned&size=25'));
        expect(params).toEqual({
            workspaceCwd: '/work/a',
            view: 'organized',
            group: 'pinned',
    ...
    it('PATCH /session/:id/organization maps to _qwen/session/update_organization', () => {
        ...
        expect(result.mapping.extractParams(result.segments, { isPinned: true, groupId: 'g-1' }, 'PATCH')).toEqual({ sessionId: 's6', isPinned: true, groupId: 'g-1' });
    });
    it('keeps URL session id when organization body contains sessionId', () => {
        const result = matchRoute('/session/s6/organization', 'PATCH');
        expect(result).not.toBeNull();
        expect(result.mapping.extractParams(result.segments, { sessionId: 'other', isPinned: true }, 'PATCH')).toEqual({ sessionId: 's6', isPinned: true });
    });
    it('maps session group CRUD routes to _qwen workspace methods', () => {
        ...
        expect(list.mapping.extractParams(list.segments, undefined, 'GET')).toEqual({ workspaceCwd: '/work/a' });
        const create = matchRoute('/workspace/%2Fwork%2Fa/session-groups', 'POST');
        expect(create?.mapping.method).toBe('_qwen/workspace/session_groups/create');
        expect(create.mapping.extractParams(create.segments, { workspaceCwd: '/other', name: 'Frontend', color: 'blue' }, 'POST')).toEqual({ workspaceCwd: '/work/a', name: 'Frontend', color: 'blue' });
        const update = matchRoute('/workspace/%2Fwork%2Fa/session-groups/g-1', 'PATCH');
        expect(update?.mapping.method).toBe('_qwen/workspace/session_groups/update');
        expect(update.mapping.extractParams(update.segments, {
            workspaceCwd: '/other',
            groupId: 'other-group',
            name: 'UI',
        }, 'PATCH')).toEqual({ workspaceCwd: '/work/a', groupId: 'g-1', name: 'UI' });
        const remove = matchRoute('/workspace/%2Fwork%2Fa/session-groups/g-1', 'DELETE');
        expect(remove?.mapping.method).toBe('_qwen/workspace/session_groups/delete');
        expect(remove.mapping.extractParams(remove.segments, undefined, 'DELETE')).toEqual({ workspaceCwd: '/work/a', groupId: 'g-1' });
    });
    ...
    it('GET /session/:id/artifacts maps to _qwen/session/artifacts', () => {
        ...
        expect(result.mapping.extractParams(result.segments, undefined, 'GET')).toEqual({ sessionId: 's8' });
    });
    it('POST /session/:id/artifacts maps to _qwen/session/artifacts/add', () => {
        ...
        expect(result.mapping.extractParams(result.segments, {
    ...
    it('DELETE /session/:id/artifacts/:artifactId maps to _qwen/session/artifacts/remove', () => {
        ...
        expect(result.mapping.extractParams(result.segments, undefined, 'DELETE')).toEqual({
            sessionId: 's8',
            artifactId: 'art 1',
        });
        expect(result.mapping.extractParams(result.segments, {
    ...
    it('GET /session/:id/context maps to _qwen/session/context', () => {
        ...
        const params = result.mapping.extractParams(result.segments, undefined, 'GET');
    ...
    it('POST /workspace/trust/request maps to _qwen/workspace/trust/request', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { desiredState: 'trusted', reason: 'operator prompt' }, 'POST');
        expect(params).toEqual({
    ...
    it('POST /workspace/permissions maps to _qwen/workspace/permissions/set', () => {
        ...
        expect(result.mapping.extractParams(result.segments, body, 'POST')).toBe(body);
    });
    ...
    it('POST /workspace/memory maps to _qwen/workspace/memory/write', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { content: 'hi' }, 'POST');
    ...
    it('POST /workspace/memory/remember maps to _qwen/workspace/memory/remember', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { content: 'hi', contextMode: 'clean' }, 'POST');
    ...
    it('POST /workspace/memory/forget maps to _qwen/workspace/memory/forget', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { query: 'old preference' }, 'POST');
    ...
    it('POST /workspace/memory/dream maps to _qwen/workspace/memory/dream', () => {
        ...
        const params = result.mapping.extractParams(result.segments, undefined, 'POST');
        expect(params).toEqual({});
    });
    ...
    it('DELETE /workspace/agents/:agentType maps to _qwen/workspace/agents/delete', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { scope: 'workspace' }, 'DELETE');
    ...
    it('POST /workspace/mcp/servers maps to _qwen/workspace/mcp/servers/add', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { name: 'test', config: {} }, 'POST');
    ...
    it('DELETE /workspace/mcp/servers/:name maps to _qwen/workspace/mcp/servers/remove', () => {
        ...
        const params = result.mapping.extractParams(result.segments, undefined, 'DELETE');
    ...
    it('POST /workspace/mcp/:server/restart maps to _qwen/workspace/restart_mcp_server', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { entryIndex: 0 }, 'POST');
    ...
    it('POST /workspace/auth/device-flow maps to _qwen/workspace/auth/device_flow/start', () => {
        ...
        expect(result).not.toBeNull();
        expect(result.mapping.method).toBe('_qwen/workspace/auth/device_flow/start');
    });
    ...
    it('DELETE /workspace/auth/device-flow/:id maps to _qwen/workspace/auth/device_flow/cancel', () => {
        ...
        expect(result).not.toBeNull();
        expect(result.mapping.method).toBe('_qwen/workspace/auth/device_flow/cancel');
        const params = result.mapping.extractParams(result.segments, undefined, 'DELETE');
        expect(params).toEqual({ id: 'flow-1' });
    });
    // ---- File system routes -----------------------------------------------
    ...
    it('POST /file/write maps to _qwen/file/write', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { path: '/a.txt', content: 'hi' }, 'POST');
    ...
    it('POST /file/edit maps to _qwen/file/edit', () => {
        ...
        const params = result.mapping.extractParams(result.segments, { path: '/b.txt', oldText: 'a', newText: 'b' }, 'POST');
...
describe('acpRouteTable – query param coercion', () => {
    function extract(path, method) {
        const url = new URL(`http://d${path}`);
        const m = matchRoute(url.pathname, method);
        expect(m).not.toBeNull();
        return {
            method: m.mapping.method,
            params: m.mapping.extractParams(m.segments, undefined, method, url.searchParams),
        };
    }
    it('GET /file forwards typed range and cursor params', () => {
        const { method, params } = extract('/file?path=src%2Fa.ts&maxBytes=123&line=4&limit=10&cursor=next%201', 'GET');
    ...
    it('GET /file/bytes forwards path + offset/maxBytes as numbers', () => {
        const { method, params } = extract('/file/bytes?path=a.bin&offset=8&maxBytes=64', 'GET');
    ...
    it('GET context-usage coerces detail to the boolean true', () => {
        const { method, params } = extract('/session/s1/context-usage?detail=true', 'GET');
...
//# sourceMappingURL=acpRouteTable.test.js.map