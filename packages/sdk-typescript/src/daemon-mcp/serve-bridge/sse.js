 ...
 */
export function startEventStream(state, sessionId) {
    ...
    if (state.eventStreams.has(sessionId)) {
        ...
        if (!existing.abortCtrl.signal.aborted)
            return;
        ...
        try {
            for await (const event of state.client.subscribeEvents(sessionId, {
                ...
                if (update['sessionUpdate'] === 'agent_message_chunk') {
                    ...
                    process.stderr.write(`[serve-bridge] daemon error event for ${sessionId}: ${JSON.stringify(update)}\n`);
            ...
            if (!(err instanceof Error && err.name === 'AbortError')) {
                ...
                process.stderr.write(`[serve-bridge] SSE stream ended unexpectedly for session ${sessionId}: ${detail}\n`);
            }
        }
        finally {
 ...
 */
export function stopEventStream(state, sessionId) {
 ...
 */
export function startSessionCleanup(state) {
    const timer = setInterval(() => {
        const now = Date.now();
        for (const [sessionId, stream] of state.eventStreams) {
            if (now - stream.lastActivityMs > SESSION_TTL_MS) {
                process.stderr.write(`[serve-bridge] Cleaning up idle session SSE: ${sessionId}\n`);
