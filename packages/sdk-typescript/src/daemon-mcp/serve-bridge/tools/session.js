...
export function sessionTools(state) {
    return [
        tool('session_create', 'Create a new qwen-code session or attach to an existing one. The created session becomes the default for subsequent tool calls.', {
            workspace_cwd: z
                .string()
                .optional()
        ...
        tool('session_load', 'Restore a persisted session with SSE history replay. Sets the loaded session as the default.', {
        ...
        tool('session_resume', 'Restore a session without history replay. Sets the resumed session as the default.', {
        ...
        tool('session_close', 'Force-close a live session.', {
            session_id: z
                .string()
                .optional()
        ...
        tool('session_update_metadata', 'Update session metadata such as display name.', {
            session_id: z
                .string()
                .optional()
                .describe('Session ID. Uses default session if omitted.'),
        ...
        tool('session_list', 'List live sessions for a workspace.', {
            workspace_cwd: z
                .string()
        ...
        tool('session_set_model', 'Switch the active model for a session.', {
        ...
        tool('session_context', 'Get the current session model/mode/config state.', {
            session_id: z
                .string()
                .optional()
                .describe('Session ID. Uses default session if omitted.'),
