...
export function workspaceWriteTools(state) {
    return [
        tool('file_write', 'Create or replace a text file in the workspace. Supports hash-verified atomic writes.', {
            ...
            expected_hash: z
                .string()
                .optional()
                .describe('Expected SHA-256 hash for replace mode (required for replace).'),
        }, handler(async (args) => {
        ...
        tool('file_edit', 'Make a single text replacement in a file. Requires exact-once match of old_text.', {
        ...
        }, handler(async (args) => formatJsonResult(await state.client.editWorkspaceFile({
            path: args.path,
            oldText: args.old_text,
            newText: args.new_text,
            expectedHash: args.expected_hash,
        })))),
        tool('session_set_approval_mode', 'Change the approval mode of a session (plan, default, auto-edit, auto, yolo).', {
            mode: z
            ...
            if (!state.allowGlobalScope) {
                ...
                if (dangerousModes.includes(args.mode)) {
                    return formatToolError(`Approval modes '${dangerousModes.join("', '")}' are restricted for security. Set QWEN_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.`);
                }
                if (args.persist) {
                    return formatToolError('Persisting approval mode changes is restricted for security. Set QWEN_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.');
                }
            }
            const sessionId = resolveSessionId(state, args.session_id);
            return formatJsonResult(await state.client.setSessionApprovalMode(sessionId, args.mode, {
                persist: args.persist,
            }));
        })),
        tool('workspace_tool_toggle', 'Enable or disable a tool in the workspace settings.', {
            ...
            if (!state.allowGlobalScope) {
                return formatToolError('Tool toggling is restricted for security. Set QWEN_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.');
            }
            return formatJsonResult(await state.client.setWorkspaceToolEnabled(args.tool_name, args.enabled));
        })),
        tool('workspace_init', 'Scaffold an empty QWEN.md at the workspace root. No LLM invocation.', {
            force: z
                .boolean()
                .optional()
                .describe('Overwrite existing QWEN.md if present.'),
        }, handler(async (args) => formatJsonResult(await state.client.initWorkspace({ force: args.force })))),
        tool('workspace_mcp_restart', 'Restart a configured MCP server. Pre-checks budget before restarting.', {
            server_name: z.string().describe('Name of the MCP server to restart.'),
        }, handler(async (args) => {
            if (!state.allowGlobalScope) {
                return formatToolError('MCP server restart is restricted for security. Set QWEN_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.');
            }
            return formatJsonResult(await state.client.restartMcpServer(args.server_name));
        })),
        tool('workspace_memory_read', 'Read workspace memory (QWEN.md hierarchy).', {}, handler(async () => formatJsonResult(await state.client.workspaceMemory()))),
        tool('workspace_memory_write', 'Write to workspace memory (QWEN.md). Supports append or replace mode.', {
            ...
            if (args.scope === 'global' && !state.allowGlobalScope) {
                return formatToolError('Global scope is disabled for security. Set QWEN_BRIDGE_ALLOW_GLOBAL_SCOPE=true to enable.');
            }
            return formatJsonResult(await state.client.writeWorkspaceMemory({
                scope: args.scope,
                content: args.content,
                mode: args.mode,
            }));
        })),
        tool('workspace_agents_manage', 'Manage workspace agent definitions. Use action to list, get, create, update, or delete agents.', {
            action: z
...
async function handleAgentGet(state, args) {
    ...
    return formatJsonResult(await state.client.getWorkspaceAgent(args.agent_type, args.scope ? { scope: args.scope } : {}));
}
async function handleAgentCreate(state, args) {
    if (!args.name || !args.description || !args.system_prompt || !args.scope) {
        return formatToolError('name, description, system_prompt, and scope are required for create action.');
    }
    return formatJsonResult(await state.client.createWorkspaceAgent({
        name: args.name,
...
async function handleAgentUpdate(state, args) {
    ...
    const hasField = args.description !== undefined ||
    ...
    if (!hasField) {
        return formatToolError('At least one field to update must be provided (description, system_prompt, tools, disallowed_tools, model, approval_mode, permission_mode, max_turns, color, mcp_servers, hooks, or background).');
    }
    return formatJsonResult(await state.client.updateWorkspaceAgent(args.agent_type, {
        description: args.description,
        systemPrompt: args.system_prompt,
        tools: args.tools,
        disallowedTools: args.disallowed_tools,
...
//# sourceMappingURL=workspaceWrite.js.map