 ...
 */
function normalizeForRegex(dirPath) {
    return dirPath.replace(/\\/g, '/');
}
/**
 * Resolve bundled CLI using import.meta.url relative path
 */
function tryResolveCliFromImportMeta() {
    try {
        if (typeof import.meta !== 'undefined' && import.meta.url) {
 ...
 */
function getBundledCliCandidatePaths() {
    const candidates = [];
    ...
    try {
        ...
        const monorepoMatch = normalizedDir.match(/^(.+?)\/packages\/sdk-typescript/);
        if (monorepoMatch && monorepoMatch[1]) {
            const monorepoRoot = process.platform === 'win32'
 ...
 */
export function findBundledCliPath() {
...
function validateFilePath(filePath) {
    if (!fs.existsSync(filePath)) {
        throw new Error(`Executable file not found at '${filePath}'. ` +
...
export function prepareSpawnInfo(executableSpec) {
    ...
    if (!isFilePath(executableSpec)) {
        if (!/^[a-zA-Z0-9._-]+$/.test(executableSpec)) {
            throw new Error(`Invalid command name '${executableSpec}'. ` +
