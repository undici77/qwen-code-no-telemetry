 ...
 */
export function daemonBlockToMarkdown(block, opts = {}) {
    const cap = capLength(opts);
    const text = (value) => cap(sanitizeTerminalText(value));
    switch (block.kind) {
        case 'user':
            return `**You**\n\n${text(block.text)}`;
        case 'assistant':
        ...
        case 'permission': {
            const optionList = block.options
                .map((opt) => `- **${escapeMarkdownText(opt.label, opts)}**${opt.description
            ...
            const resolved = block.resolved
                ...
                : '\n\n_awaiting decision_';
            const previewMd = daemonToolPreviewToMarkdown(block.preview, opts);
            return `### Permission: ${escapeMarkdownText(block.title, opts)}\n\n${previewMd}\n\n${optionList}${resolved}`;
        }
        case 'status':
            return `*${text(block.text)}*`;
        case 'debug':
...
function renderToolHeader(block, opts = {}) {
    ...
    // `opts` is provided.
    const parts = [`### ${escapeMarkdownText(block.title, opts)}`];
    if (block.toolName)
        parts.push(inlineCode(block.toolName, opts));
    if (block.toolKind)
 ...
 */
export function daemonToolPreviewToMarkdown(preview, opts = {}) {
    const cap = capLength(opts);
    const text = (value) => cap(sanitizeTerminalText(value));
    switch (preview.kind) {
        ...
        case 'web_fetch': {
            ...
            return `${escapeMarkdownText(preview.method ?? 'GET', opts)} ${inlineCode(url, opts)}`;
        }
        case 'mcp_invocation':
            return [
                `**MCP** ${inlineCode(`${preview.serverId}::${preview.toolName}`, opts)}`,
                preview.argsSummary
        ...
        case 'code_block':
            return [
                ...
                markdownFence(escapeFenceLanguage(preview.language ?? ''), text(preview.code)),
            ]
                .filter(Boolean)
                .join('\n');
        case 'search': {
        ...
        case 'tabular': {
            ...
            const bodyRows = preview.rows.map((row) => `| ${preview.columns
                .map((_, idx) => escapeTableCell(String(row[idx] ?? ''), opts))
                .join(' | ')} |`);
            const lines = [headerRow, sepRow, ...bodyRows];
            if (preview.totalRows !== undefined &&
                preview.totalRows > preview.rows.length) {
                lines.push(`_… ${preview.totalRows - preview.rows.length} more row(s) not shown_`);
            }
            return lines.join('\n');
        }
        ...
        case 'subagent_delegation':
            return [
                ...
                preview.parentDelegationId
                    ? `_(chained from ${escapeMarkdownText(preview.parentDelegationId, opts)})_`
                    : null,
            ]
                .filter(Boolean)
                .join('\n');
        case 'key_value':
            return preview.rows
                .map((row) => `- **${escapeMarkdownText(row.label, opts)}:** ${escapeMarkdownText(row.value, opts)}`)
                .join('\n');
        case 'generic':
 ...
 */
function blockquote(raw) {
    return raw
        .split('\n')
        .map((line) => `> ${line}`)
        .join('\n');
}
function markdownFence(language, raw) {
    const maxRun = Math.max(2, ...Array.from(raw.matchAll(/`+/g), (match) => match[0].length));
 ...
 */
export function daemonBlockToPlainText(block, opts = {}) {
    ...
    const cap = capLength(opts);
    const clean = (raw) => cap(sanitizeTerminalText(raw));
    switch (block.kind) {
        case 'user':
        ...
        case 'permission': {
            ...
            const optionList = block.options
                .map((opt) => `  - ${clean(opt.label)}${opt.description ? `: ${clean(opt.description)}` : ''}`)
                .join('\n');
            const resolved = block.resolved
...
function daemonToolPreviewToPlainText(preview, opts = {}) {
    ...
    // 8192 default.
    const url = (u) => (opts.sanitizeUrls ? sanitizeUrl(u) : u);
    const cap = capLength(opts);
    switch (preview.kind) {
        case 'ask_user_question':
 ...
 */
export function daemonBlockToHtml(block, opts = {}) {
    ...
    switch (block.kind) {
        ...
        case 'tool': {
            const previewHtml = sanitizer(daemonToolPreviewToPlainText(block.preview, opts));
        ...
        case 'permission': {
            ...
            const optionList = block.options
                .map((opt) => `<li><strong>${sanitizer(cap(opt.label))}</strong>${opt.description ? ` — ${sanitizer(cap(opt.description))}` : ''}</li>`)
                .join('');
            const resolved = block.resolved
...
function sanitizeUrl(url) {
    try {
        ...
        for (const key of Array.from(u.searchParams.keys())) {
            ...
            if (/^(token|key|auth|signature|sig|access|secret|bearer|credential|session|api[_-]?key|x-amz-|x-goog-|aws|google|expires)/i.test(key) ||
 ...
 */
function ensureSafeImageUrl(url) {
    try {
