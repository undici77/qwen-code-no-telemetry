 ...
 */
import { getFirstString, isRecord, isSensitiveKey, stringifyJson, stringifyRedactedJson, } from './utils.js';
const MAX_TOOL_PREVIEW_DEPTH = 8;
export function createDaemonToolPreview(input, opts = {}, depth = 0) {
    ...
    if (isRecord(input)) {
        ...
        if (nestedInput !== undefined && nestedInput !== input) {
            const nested = createDaemonToolPreview(nestedInput, {
                title: opts.title ?? getFirstString(input, ['title']),
                toolName: opts.toolName ?? getFirstString(input, ['toolName', 'name']),
                toolKind: opts.toolKind ?? getFirstString(input, ['kind']),
            }, depth + 1);
            if (nested.kind !== 'generic' || !nested.summary)
                return nested;
        }
    }
 ...
 */
function detectFileDiff(input, opts = {}) {
    if (!isRecord(input))
        return undefined;
    ...
    const writeIntent = /(?:^|[_-])(write|create|edit|replace|save|update|overwrite|modify|patch|generate)(?:$|[_-])/.test(toolNameLower) || !!oldText;
    const contentField = explicitNewText === undefined && writeIntent
 ...
 */
function detectFileRead(input, opts) {
    if (!isRecord(input))
        return undefined;
    const path = getFirstString(input, [
        'path',
    ...
    const looksLikeRead = toolName !== undefined && /read|view|cat/i.test(toolName);
 ...
 */
function detectMcpInvocation(input, opts) {
...
function extractAskUserQuestions(input) {
    ...
    return input['questions'].filter(isRecord).map((question) => {
        ...
        const prompt = getFirstString(question, ['question', 'prompt', 'text']) ?? 'Question';
...
function collectPreviewRows(input) {
    const rows = [];
    const candidates = [
 ...
 */
function detectSubagentDelegation(input, opts) {
    const toolName = opts.toolName ?? '';
    ...
    const looksLikeDelegate = /^task$/i.test(toolName) ||
 ...
 */
function detectSearch(input, opts) {
    if (!isRecord(input))
        return undefined;
    ...
    if (!query)
        return undefined;
    const toolName = opts.toolName ?? '';
    const looksLikeSearch = /(grep|search|find|ripgrep|rg|glob|lookup)/i.test(toolName) ||
        typeof input['resultCount'] === 'number' ||
        Array.isArray(input['results']) ||
        Array.isArray(input['matches']);
    if (!looksLikeSearch)
        return undefined;
    const resultCount = typeof input['resultCount'] === 'number'
        ? input['resultCount']
        : typeof input['total'] === 'number'
            ...
            : Array.isArray(input['results'])
                ...
                : Array.isArray(input['matches'])
                    ...
                    : undefined;
    let top;
    const rawResults = input['results'] ?? input['matches'];
    if (Array.isArray(rawResults)) {
        top = rawResults
            ...
            if (typeof item === 'string')
                return item;
            if (isRecord(item)) {
                return (getFirstString(item, ['path', 'file', 'name', 'title', 'text']) ??
 ...
 */
function detectImageGeneration(input, opts) {
    const toolName = opts.toolName ?? '';
    const looksLikeImageGen = /(image[_-]?gen|generate[_-]?image|diffusion|dalle|imagen|flux|stable[_-]?diffusion|midjourney|sora)/i.test(toolName);
    if (!looksLikeImageGen)
        return undefined;
    if (!isRecord(input))
        return undefined;
 ...
 */
function detectCodeBlock(input, opts) {
    if (!isRecord(input))
        return undefined;
    ...
    const toolName = opts.toolName ?? '';
    const codeTool = /(repl|format|prettier|eslint|tsc|compile|exec[_-]?code)/i.test(toolName);
    if (!language && !codeTool)
        return undefined;
...
//# sourceMappingURL=toolPreview.js.map