...
import { getFirstString, getOutputText, getString, getTextContent, extractContentPart, isRecord, redactSensitiveFields, stringifyJson, stringifyRedactedJson, } from './utils.js';
const DAEMON_ERROR_KIND_SET = new Set(DAEMON_ERROR_KINDS);
const DEVICE_FLOW_PROVIDER_SET = new Set(['qwen', 'qwen-oauth']);
...
const SESSION_RECORDING_DEGRADED_MESSAGE = 'Session recording stopped after a write failure. New messages for the affected session will not be saved. Check disk space and permissions, then start a new session to resume recording.';
export function normalizeDaemonEvent(event, opts = {}) {
    ...
    switch (event.type) {
        ...
        case 'shell_output': {
            ...
            return text
                ? [
                    {
                        ...base,
                        type: source === 'user-shell' ? 'user.shell.output' : 'shell.output',
                        text,
                        ...(stream ? { stream } : {}),
                    },
                ]
            ...
            return [
                {
                    ...
                    type: 'error',
                    recoverable: true,
                    text: getString(event.data, 'error') ??
            ...
            return [
                {
                    ...
                    recoverable: false,
                    ...(errorKind ? { errorKind } : {}),
                    text: getString(event.data, 'reason') ??
        ...
        case 'session_snapshot': {
            ...
            if (recordingDegraded !== undefined &&
            ...
            return [
                {
                    ...
                    ...(errorKind ? { errorKind } : {}),
                    ...(promptId ? { promptId } : {}),
                    text: getString(event.data, 'message') ??
        ...
        case 'user_shell_result': {
            ...
            const aborted = isRecord(event.data) &&
        ...
        case 'replay_complete': {
            ...
            const lastReplayedEventId = numberField(event.data, 'lastReplayedEventId') ??
...
function normalizeUnrecognizedEvent(event, base) {
    return [
        {
            ...base,
            type: 'debug',
...
function normalizeStateResyncRequired(event, base) {
    ...
    if (!reason ||
        ...
        return fallbackDebug(event, base, 'malformed state_resync_required payload');
    }
    return [
        {
            ...base,
...
function normalizeHistoryTruncated(event, base) {
    ...
    if ((event.data['scope'] !== undefined && !scope) ||
    ...
    const limits = [
        ...
        .join(' / ');
    const text = scope === 'live_journal'
        ? `History truncated for live turn replay: kept the latest ${retainedEvents} source events and dropped ${truncatedEvents} older source events (limits: ${limits}). ${fullTranscriptAvailable
            ? 'Complete content remains available after the turn finishes.'
            : 'Complete content is not available for automatic recovery.'}`
        : scope === undefined
            ? `History truncated in replay history: kept the latest ${retainedEvents} events and dropped ${truncatedEvents} older replay events (limits: ${limits}). ${fullTranscriptAvailable
                ? 'Older content remains available from the full transcript.'
                : 'Older content is not available from a full transcript.'}`
            : `History truncated: kept the latest ${retainedEvents} events and dropped ${truncatedEvents} older replay events (limits: ${limits}). ${fullTranscriptAvailable
...
function normalizeSessionBranched(event, base) {
...
function normalizeFollowupSuggestion(event, base) {
    const sessionId = getString(event.data, 'sessionId');
...
function normalizeMidTurnMessageInjected(event, base) {
    if (!isRecord(event.data)) {
        return fallbackDebug(event, base, 'malformed mid_turn_message_injected');
    }
    const messages = Array.isArray(event.data['messages'])
        ? event.data['messages'].filter((message) => typeof message === 'string' && message.length > 0)
        : [];
...
function createBase(event, opts) {
 ...
 */
export function extractServerTimestamp(event) {
    const direct = event.serverTimestamp;
    if (typeof direct === 'number' && Number.isFinite(direct))
        return direct;
    const envelopeMeta = event._meta;
...
function normalizeSessionUpdate(event, base, opts) {
    ...
    switch (kind) {
        ...
        case 'tool_call_update': {
            ...
            if (getString(update, 'status') === 'in_progress' &&
        ...
        case 'available_commands_update': {
            const rawCommands = Array.isArray(update['availableCommands'])
                ...
                : [];
            const commands = rawCommands.filter(isRecord);
            return [
                {
                    ...base,
...
function extractUpdateMeta(update) {
    const meta = isRecord(update['_meta']) ? update['_meta'] : undefined;
    if (!meta)
        return undefined;
...
function extractSourceRecordIds(event) {
    if (!isRecord(event.data))
        return undefined;
    const update = getSessionUpdatePayload(event.data);
    const meta = update && isRecord(update['_meta']) ? update['_meta'] : undefined;
    const transcript = meta && isRecord(meta['qwenTranscript'])
        ? meta['qwenTranscript']
        : undefined;
 ...
 */
function extractAssistantUsage(update) {
    const meta = isRecord(update['_meta']) ? update['_meta'] : undefined;
...
function normalizeToolUpdate(update, base) {
    const metadata = isRecord(update['_meta']) ? update['_meta'] : undefined;
    const toolName = getString(update, 'toolName') ??
    ...
    const title = explicitTitle ??
        (getString(update, 'sessionUpdate') === 'tool_call'
            ? (toolName ?? toolKind)
            : undefined);
    const rawInputSource = update['rawInput'] ?? update['input'] ?? update['args'];
    const rawOutputSource = update['rawOutput'] ?? update['output'] ?? update['result'];
    ...
    const rawParentToolCallId = getString(update, 'parentToolCallId') ??
        (metadata ? getString(metadata, 'parentToolCallId') : undefined);
    const parentToolCallId = rawParentToolCallId && rawParentToolCallId !== toolCallId
        ? rawParentToolCallId
        : undefined;
    const subagentType = getString(update, 'subagentType') ??
...
function normalizePlanUpdate(update, base) {
    ...
    const transcript = meta && isRecord(meta['qwenTranscript'])
        ? meta['qwenTranscript']
        : undefined;
    const planCallId = getString(transcript, 'planToolCallId') ??
    ...
    const todoPlan = meta && isRecord(meta['qwenTodoPlan']) ? meta['qwenTodoPlan'] : undefined;
...
function getPlanEntryMarker(status) {
    switch (status) {
        case 'completed':
            return 'x';
        case 'in_progress':
 ...
 */
function extractToolProvenance(update, toolName) {
...
function asDaemonErrorKind(value) {
    if (!value)
        return undefined;
...
function normalizePermissionRequest(event, base) {
    if (!isRecord(event.data)) {
        return [
            {
                ...base,
    ...
    const toolCall = event.data['toolCall'] !== undefined
...
function normalizePermissionResolved(event, base) {
    const requestId = getString(event.data, 'requestId');
    if (!requestId) {
        return [
            {
    ...
    const voterClientId = getString(event.data, 'voterClientId') ?? base.originatorClientId;
    return [
        {
            ...base,
...
export function getSessionUpdatePayload(value) {
    if (!isRecord(value))
        return undefined;
...
function describePermissionOutcome(value) {
    if (!isRecord(value))
...
function describeToolCall(value) {
    if (!isRecord(value))
 ...
 * for operator triage.
 * ──────────────────────────────────────────────────────────────────────── */
function fallbackDebug(event, base, reason) {
    return [
        {
            ...base,
            type: 'debug',
...
function normalizeSessionMetadataUpdated(event, base) {
    const sessionId = getString(event.data, 'sessionId');
    if (!sessionId)
        return fallbackDebug(event, base, 'missing sessionId');
    const displayName = getString(event.data, 'displayName');
    return [
        {
            ...base,
...
function normalizeArtifactChanged(event, base) {
    const sessionId = getString(event.data, 'sessionId');
...
function normalizeApprovalModeChanged(event, base) {
    const sessionId = getString(event.data, 'sessionId');
    ...
    const persisted = isRecord(event.data) && typeof event.data['persisted'] === 'boolean'
...
function normalizeMemoryChanged(event, base) {
    const scope = getString(event.data, 'scope');
    if (scope === 'managed') {
        const source = getString(event.data, 'source');
        const taskId = getString(event.data, 'taskId');
        const touchedScopes = event.data?.['touchedScopes'];
        if (!(source && taskId && Array.isArray(touchedScopes)) ||
    ...
    const bytesWritten = numberField(isRecord(event.data) ? event.data : undefined, 'bytesWritten');
...
function normalizeAgentChanged(event, base) {
...
function normalizeToolToggled(event, base) {
    const toolName = getString(event.data, 'toolName');
    const enabled = isRecord(event.data) && typeof event.data['enabled'] === 'boolean'
        ? event.data['enabled']
        : undefined;
...
function normalizeSettingsChanged(event, base) {
...
function normalizeSettingsReloaded(event, base) {
    if (!isRecord(event.data)) {
...
function normalizeWorkspaceInitialized(event, base) {
    const path = getString(event.data, 'path');
    const action = getString(event.data, 'action');
    if (!path ||
        (action !== 'created' && action !== 'overwrote' && action !== 'noop')) {
        return fallbackDebug(event, base, 'malformed workspace_initialized payload');
    }
    return [{ ...base, type: 'workspace.initialized', path, action }];
}
function normalizeTrustChangeRequested(event, base) {
...
function normalizeGithubSetupCompleted(event, base) {
    ...
    if (!releaseTag || !readmeUrl || !isRecord(event.data)) {
        return fallbackDebug(event, base, 'malformed github_setup_completed payload');
    }
...
function normalizeMcpBudgetWarning(event, base) {
    if (!isRecord(event.data)) {
...
function normalizeMcpChildRefused(event, base) {
    if (!isRecord(event.data)) {
        return fallbackDebug(event, base, 'non-object payload');
    }
    const refusedServers = Array.isArray(event.data['refusedServers'])
        ? event.data['refusedServers']
            ...
            if (!name || !transport || reason !== 'budget_exhausted')
                return null;
            return {
                name,
                transport,
    ...
    const liveCount = numberField(event.data, 'liveCount');
    const reservedCount = numberField(event.data, 'reservedCount');
    if (refusedServers.length === 0 ||
        budget === undefined ||
        liveCount === undefined ||
        reservedCount === undefined) {
        return fallbackDebug(event, base, 'malformed mcp_child_refused_batch payload');
    }
    return [
        {
            ...base,
...
function normalizeMcpServerRestarted(event, base) {
...
function normalizeMcpServerRestartRefused(event, base) {
    const serverName = getString(event.data, 'serverName');
    const reason = getString(event.data, 'reason');
    if (!serverName || !reason || !MCP_RESTART_REFUSED_REASONS.has(reason)) {
        return fallbackDebug(event, base, 'malformed mcp_server_restart_refused payload');
    }
    return [
        {
            ...base,
...
function normalizeMcpServerChanged(event, base, fixedAction) {
...
function normalizeExtensionsChanged(event, base) {
...
function normalizeAuthDeviceFlowStarted(event, base) {
    ...
    if (!deviceFlowId ||
        ...
        expiresAt === undefined) {
        return fallbackDebug(event, base, 'malformed auth_device_flow_started payload');
    }
    return [
        {
            ...base,
...
function normalizeAuthDeviceFlowThrottled(event, base) {
    const deviceFlowId = getString(event.data, 'deviceFlowId');
    const intervalMs = numberField(event.data, 'intervalMs');
    if (!deviceFlowId || intervalMs === undefined) {
        return fallbackDebug(event, base, 'malformed auth_device_flow_throttled payload');
    }
    return [
        {
            ...base,
...
function normalizeAuthDeviceFlowAuthorized(event, base) {
    const deviceFlowId = getString(event.data, 'deviceFlowId');
    const providerId = getString(event.data, 'providerId');
    if (!deviceFlowId ||
        !providerId ||
        !DEVICE_FLOW_PROVIDER_SET.has(providerId)) {
        return fallbackDebug(event, base, 'malformed auth_device_flow_authorized payload');
    }
    const expiresAt = numberField(event.data, 'expiresAt');
...
function normalizeAuthDeviceFlowFailed(event, base) {
    const deviceFlowId = getString(event.data, 'deviceFlowId');
    const errorKind = getString(event.data, 'errorKind');
    if (!deviceFlowId || !isDeviceFlowErrorKind(errorKind)) {
        return fallbackDebug(event, base, 'malformed auth_device_flow_failed payload');
    }
...
function isDeviceFlowErrorKind(value) {
...
function normalizeAuthDeviceFlowCancelled(event, base) {
    const deviceFlowId = getString(event.data, 'deviceFlowId');
    if (!deviceFlowId) {
        return fallbackDebug(event, base, 'malformed auth_device_flow_cancelled payload');
    }
