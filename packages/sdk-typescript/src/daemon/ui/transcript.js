...
export function createDaemonTranscriptState(opts = {}) {
    const state = {
        blocks: [],
    ...
    if (opts.onTruncation)
        truncationCallbacks.set(state, opts.onTruncation);
    return state;
}
/**
...
export function appendLocalUserTranscriptMessage(state, text, opts = {}) {
    const next = cloneTranscriptState(state, opts);
    finishAssistant(next);
    const block = createTextBlock(next, 'user', text, undefined, undefined, opts.meta);
    if (opts.images && opts.images.length > 0) {
        block.images = [...opts.images];
    }
...
const FREEZE_TRANSCRIPT_BLOCKS = typeof process !== 'undefined' && process.env.NODE_ENV !== 'production';
export function reduceDaemonTranscriptEvents(state, events, opts = {}) {
    if (events.length === 0)
        return state;
    const next = cloneTranscriptState(state, opts);
...
export function finalizeOfflineDaemonTranscriptState(state) {
...
export function rebuildDaemonTranscriptBlockIndex(blocks) {
    const blockIndexById = createIndex();
    blocks.forEach((block, index) => {
        blockIndexById[block.id] = index;
    });
    return blockIndexById;
}
function applyDaemonTranscriptEvent(next, event) {
    ...
    if (next.awaitingResync && !RESYNC_PASSTHROUGH_TYPES.has(event.type)) {
        ...
        if (typeof console !== 'undefined') {
            ...
            console.warn?.(`[daemon-ui] dropping event \`${event.type}\` while awaitingResync; ` +
                `state may be stale until session reconnect (lastResyncRequired: ${next.lastResyncRequired
    ...
    switch (event.type) {
        ...
        case 'user.image.delta': {
            const activeUserIndex = next.activeUserBlockId
                ...
                : undefined;
            const activeUser = activeUserIndex !== undefined
                ? next.blocks[activeUserIndex]
                : undefined;
            if (activeUser?.kind !== 'user' ||
                !stringArraysEqual(activeUser.sourceRecordIds, event.sourceRecordIds)) {
                const block = createTextBlock(next, 'user', '', event.eventId, event.serverTimestamp, undefined, event.sourceRecordIds);
                ...
                const block = getWritableBlockById(next, next.activeUserBlockId);
        ...
        case 'assistant.text.delta':
            if (event.parentToolCallId && !next.retainSubagentBlocks)
                break;
            appendTextDelta(next, 'assistant', 'activeAssistantBlockId', event.text, event);
            break;
        case 'assistant.done':
        ...
        case 'thought.text.delta':
            if (event.parentToolCallId && !next.retainSubagentBlocks)
                break;
            appendTextDelta(next, 'thought', 'activeThoughtBlockId', event.text, event);
            break;
        case 'tool.update':
            if (event.parentToolCallId && !next.retainSubagentBlocks) {
        ...
        case 'permission.resolved':
            ...
            break;
        case 'model.changed':
            appendStatusBlock(next, 'status', `Model switched: ${event.modelId}`, event);
            break;
        case 'status':
        case 'debug':
        ...
        case 'session.branched':
            appendStatusBlock(next, 'status', `Branched conversation "${event.displayName}". You are now in the branch.`, event);
            break;
...
function handleStateResyncRequired(state, event) {
    ...
    appendStatusBlock(state, 'error', `State resync required: ${formatMissedRange(event.lastDeliveredId, event.earliestAvailableId)}.`, event);
}
/**
 ...
 */
export function formatMissedRange(lastDeliveredId, earliestAvailableId) {
    const first = lastDeliveredId + 1;
    const last = earliestAvailableId - 1;
    if (last < first)
        return 'no events lost (resync requested without gap)';
    if (last === first)
        return `missed 1 daemon event (id ${first})`;
    return `missed daemon events ${first}-${last}`;
}
export function selectTranscriptBlocks(state) {
    return state.blocks;
}
export function selectPendingPermissionBlocks(state) {
    return state.blocks.filter((block) => block.kind === 'permission' && block.resolved === undefined);
}
function finalizeStreamingTextBlock(state, blockId, event) {
    const block = getWritableBlockById(state, blockId);
    if (block?.kind === 'assistant' || block?.kind === 'thought') {
        block.streaming = false;
        block.updatedAt = state.now;
        if (event?.eventId !== undefined)
            block.eventId = event.eventId;
...
function clearActiveAssistant(state, event) {
 ...
 */
function applyAssistantUsage(state, event) {
...
function applySubagentUsageToParentTool(state, event) {
    if (!event.parentToolCallId)
        return;
    const block = getWritableBlockById(state, state.toolBlockByCallId[event.parentToolCallId]);
    if (block?.kind !== 'tool')
        return;
    ...
    const currentSummary = isRecord(current?.['executionSummary'])
        ...
        : undefined;
    const inputTokens = finiteNumber(currentSummary?.['inputTokens']) + event.usage.inputTokens;
    const outputTokens = finiteNumber(currentSummary?.['outputTokens']) + event.usage.outputTokens;
    const cachedTokens = finiteNumber(currentSummary?.['cachedTokens']) +
...
function clearActiveThought(state, event) {
    finalizeStreamingTextBlock(state, state.activeThoughtBlockId, event);
    state.activeThoughtBlockId = undefined;
}
function clearActiveAssistantForParent(state, parentToolCallId, event) {
    finalizeStreamingTextBlock(state, state.activeAssistantBlockByParent[parentToolCallId], event);
    delete state.activeAssistantBlockByParent[parentToolCallId];
}
function clearActiveThoughtForParent(state, parentToolCallId, event) {
    finalizeStreamingTextBlock(state, state.activeThoughtBlockByParent[parentToolCallId], event);
...
function appendTextDelta(state, kind, activeKey, text, event) {
    const parentId = kind !== 'user' && 'parentToolCallId' in event
        ? event.parentToolCallId
        : undefined;
    const parentMap = parentId != null
        ? kind === 'assistant'
            ...
            : kind === 'thought'
                ...
                : undefined
        : undefined;
    const effectiveId = parentMap && parentId != null ? parentMap[parentId] : state[activeKey];
    ...
    const block = createTextBlock(state, kind, text, event.eventId, event.serverTimestamp, 'meta' in event ? event.meta : undefined, event.sourceRecordIds);
    if (kind !== 'user')
        block.streaming = true;
    if (kind === 'thought')
        block.collapsed = true;
    if (parentId != null) {
        block.parentToolCallId = parentId;
    }
...
function canMergeTextDelta(existing, event) {
    if (existing.kind !== 'user' &&
        existing.kind !== 'assistant' &&
        existing.kind !== 'thought') {
        return false;
    }
    if (existing.meta?.qwenDiscreteMessage === true)
        return false;
...
function finishAssistant(state, event) {
...
function upsertToolBlock(state, event) {
    const compactTaskOutput = !state.retainSubagentBlocks &&
        isRecord(event.rawOutput) &&
        event.rawOutput['type'] === 'task_execution';
    let rawOutput = compactTaskExecutionOutput(event.rawOutput, state.retainSubagentBlocks);
    ...
    if (existingId === TRIMMED_TOOL_BLOCK_ID) {
        ...
        if (!state.trimmedToolNotificationByCallId[event.toolCallId]) {
            ...
            appendStatusBlock(state, 'error', `Tool ${event.toolCallId} output trimmed (max blocks reached)`, event, { clearActiveText: false });
        }
        return;
    }
    ...
    if (existing?.kind === 'tool') {
        ...
        if (event.rawInput !== undefined)
            existing.rawInput = event.rawInput;
        if (rawOutput !== undefined) {
            if (compactTaskOutput &&
                ...
                if (prevSummary && nextSummary) {
                    const inputTokens = Math.max(finiteNumber(prevSummary['inputTokens']), finiteNumber(nextSummary['inputTokens']));
                    const outputTokens = Math.max(finiteNumber(prevSummary['outputTokens']), finiteNumber(nextSummary['outputTokens']));
                    rawOutput = {
                        ...rawOutput,
                        executionSummary: {
                            ...nextSummary,
                            inputTokens,
                            outputTokens,
                            cachedTokens: Math.max(finiteNumber(prevSummary['cachedTokens']), finiteNumber(nextSummary['cachedTokens'])),
                            totalTokens: Math.max(finiteNumber(prevSummary['totalTokens']), finiteNumber(nextSummary['totalTokens']), inputTokens + outputTokens),
                        },
                    };
                }
                else if (prevSummary) {
        ...
        existing.sourceRecordIds = unionStrings(existing.sourceRecordIds, event.sourceRecordIds);
        if (event.toolName)
            existing.toolName = event.toolName;
        if (event.toolKind)
            existing.toolKind = event.toolKind;
    ...
    const parentBlockId = event.parentToolCallId &&
...
function discardToolBlock(state, toolCallId) {
    const blockId = state.toolBlockByCallId[toolCallId];
    if (!blockId || blockId === TRIMMED_TOOL_BLOCK_ID)
        return;
...
function compactTaskExecutionOutput(rawOutput, retainSubagentBlocks) {
    if (retainSubagentBlocks ||
        !isRecord(rawOutput) ||
        rawOutput['type'] !== 'task_execution') {
        return rawOutput;
    }
    const compact = { type: 'task_execution' };
    for (const key of [
        'subagentName',
        'subagentColor',
        'taskDescription',
 ...
 */
function updateCurrentToolPointer(state, toolCallId, status) {
    if (status === undefined)
        return;
...
function findLatestInFlightToolCallId(state) {
    for (let index = state.blocks.length - 1; index >= 0; index -= 1) {
        const block = state.blocks[index];
        if (block?.kind !== 'tool')
            continue;
        if (IN_FLIGHT_TOOL_STATUSES.has(block.status))
 ...
 */
function propagateCancellationToInFlightTools(state) {
    ...
    for (const blockId of Object.values(state.toolBlockByCallId)) {
        if (blockId === TRIMMED_TOOL_BLOCK_ID)
            continue;
        const block = getWritableBlockById(state, blockId);
...
function appendShellBlock(state, event) {
    if (!event.text)
        return;
    ...
    if (last?.kind === 'shell' && last.stream === event.stream) {
        ...
        if (writable?.kind === 'shell') {
            ...
            if (event.eventId !== undefined)
                writable.eventId = event.eventId;
        }
        return;
    }
...
function appendUserShellBlock(state, event) {
    if (!event.text)
        return;
    const last = state.blocks[state.blocks.length - 1];
    if (last?.kind === 'user_shell' &&
...
function upsertPermissionBlock(state, event) {
    const existingId = state.permissionBlockByRequestId[event.requestId];
    if (existingId === TRIMMED_PERMISSION_BLOCK_ID)
        return;
    const existing = getWritableBlockById(state, existingId);
...
function resolvePermissionBlock(state, event) {
...
function appendStatusBlock(state, kind, text, event, opts = {}) {
...
function appendPromptCancelledBlock(state, event) {
    const block = {
...
function createTextBlock(state, kind, text, eventId, serverTimestamp, meta, sourceRecordIds) {
    const blockId = allocateBlockId(state, kind);
    return {
        id: blockId,
        kind,
...
function cloneTranscriptState(state, opts) {
    const next = {
        ...state,
        now: opts.now ?? Date.now(),
        maxBlocks: opts.maxBlocks ?? state.maxBlocks,
        retainSubagentBlocks: opts.retainSubagentBlocks ?? state.retainSubagentBlocks,
        ...
        activeAssistantBlockByParent: createIndex(state.activeAssistantBlockByParent),
        activeThoughtBlockByParent: createIndex(state.activeThoughtBlockByParent),
        trimmedToolNotificationByCallId: createIndex(state.trimmedToolNotificationByCallId),
        ...
        // in-flight tools).
        toolProgress: createIndex(Object.fromEntries(Object.entries(state.toolProgress).map(([k, v]) => [k, { ...v }]))),
        lastResyncRequired: state.lastResyncRequired !== undefined
            ? { ...state.lastResyncRequired }
            : undefined,
    ...
    if (onTruncation)
        truncationCallbacks.set(next, onTruncation);
    return next;
}
function trimTranscriptState(state) {
    ...
    for (const [toolCallId] of Object.entries(state.trimmedToolNotificationByCallId)) {
        if (state.toolBlockByCallId[toolCallId] !== TRIMMED_TOOL_BLOCK_ID) {
            delete state.trimmedToolNotificationByCallId[toolCallId];
        }
    }
    for (const [requestId, blockId] of Object.entries(state.permissionBlockByRequestId)) {
        if (!keptIds.has(blockId)) {
    ...
    for (const [parentId, blockId] of Object.entries(state.activeAssistantBlockByParent)) {
        if (!keptIds.has(blockId)) {
            delete state.activeAssistantBlockByParent[parentId];
        }
    }
    for (const [parentId, blockId] of Object.entries(state.activeThoughtBlockByParent)) {
        if (!keptIds.has(blockId)) {
...
function shouldRecreateTrimmedToolBlock(event) {
    return (event.toolCallId === DAEMON_PLAN_TOOL_CALL_ID ||
...
function takeBlocksOwnership(state) {
    if (ownedBlocks.get(state) === state.blocks)
        return;
...
function rewindTranscriptToUserTurn(state, targetTurnIndex) {
    let userTurnIndex = 0;
    let lastUserIndex = -1;
    for (let index = 0; index < state.blocks.length; index += 1) {
        if (state.blocks[index]?.kind !== 'user')
            continue;
        lastUserIndex = index;
...
function truncateTranscriptBeforeBlock(state, blockIndex) {
    takeBlocksOwnership(state);
    state.blocks = state.blocks.slice(0, blockIndex);
    ownedBlocks.set(state, state.blocks);
    rebuildTranscriptIndexes(state);
}
function rebuildTranscriptIndexes(state) {
    state.blockIndexById = rebuildDaemonTranscriptBlockIndex(state.blocks);
...
function getWritableBlockById(state, blockId) {
    if (!blockId)
        return undefined;
...
function allocateBlockId(state, prefix) {
    const id = `${prefix}-${state.nextOrdinal}`;
    state.nextOrdinal += 1;
    return id;
}
function clearActiveText(state, parentToolCallId) {
    if (parentToolCallId) {
...
function appendBoundedText(state, block, text) {
...
function truncateText(state, blockId, sourceRecordIds, text) {
    if (text.length <= MAX_TEXT_BLOCK_LENGTH)
        return text;
    reportTextTruncation(state, blockId, sourceRecordIds);
    const keepLength = Math.max(0, MAX_TEXT_BLOCK_LENGTH - TEXT_TRUNCATED_SUFFIX.length);
    return `${text.slice(0, keepLength)}${TEXT_TRUNCATED_SUFFIX}`;
}
function reportTextTruncation(state, blockId, sourceRecordIds) {
...
function pruneTrimmedToolIndexes(state) {
 ...
 */
function pruneTrimmedPermissionIndexes(state) {
    const maxTrimmedEntries = Math.max(0, state.maxBlocks);
...
function cloneJsonLike(value, depth = 0) {
    if (depth > MAX_CLONE_DEPTH)
        return value;
    if (Array.isArray(value)) {
        return value.map((entry) => cloneJsonLike(entry, depth + 1));
    }
    if (isRecord(value)) {
...
export function selectTranscriptBlocksOrderedByEventId(state) {
    const cached = sortedBlocksCache.get(state.blocks);
    if (cached)
        return cached;
    const orderKeyByBlockId = buildEventOrderKeys(state.blocks);
    const sorted = [...state.blocks].sort((a, b) => compareBlocksByEventOrder(a, b, orderKeyByBlockId));
 ...
 * `blocks[]`.
 */
export function selectCurrentTool(state) {
 ...
 */
export function selectApprovalMode(state) {
 ...
 */
export function selectLastFollowupSuggestion(state) {
 ...
 */
export function selectToolProgress(state, toolCallId) {
...
const EMPTY_CHILD_LIST = Object.freeze([]);
function getOrBuildChildrenIndex(blocks) {
    ...
    for (const block of blocks) {
        ...
        if (list)
            ...
            mutable.set(block.parentToolCallId, [block]);
    }
...
export function selectSubagentChildBlocks(state, parentToolCallId) {
 ...
 */
export function isSubagentChildBlock(block) {
    return block.kind === 'tool' && block.parentToolCallId !== undefined;
}
function compareBlocksByEventOrder(a, b, orderKeyByBlockId) {
    const orderDelta = (orderKeyByBlockId.get(a.id) ?? 0) - (orderKeyByBlockId.get(b.id) ?? 0);
    if (orderDelta !== 0)
        return orderDelta;
...
function buildEventOrderKeys(blocks) {
    const orderKeyByBlockId = new Map();
    let lastDaemonEventId;
    blocks.forEach((block, index) => {
        ...
        const syntheticBase = lastDaemonEventId === undefined
