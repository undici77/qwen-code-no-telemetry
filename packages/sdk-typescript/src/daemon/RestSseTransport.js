    ...
    async *subscribeEvents(sessionId, opts = {}) {
        ...
        try {
            ...
            if (connectTimeoutMs && Number.isFinite(connectTimeoutMs)) {
                connectTimer = setTimeout(() => requestCtrl.abort(new DOMException('Initial connect timed out', 'TimeoutError')), connectTimeoutMs);
            ...
            opts.onSseStreamAccepted?.(responseStreamId && SSE_STREAM_ID_RE.test(responseStreamId)
