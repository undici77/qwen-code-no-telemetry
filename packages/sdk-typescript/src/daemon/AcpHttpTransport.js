...
import { matchRoute, synthesizeResponse, jsonRpcErrorToHttpStatusWithData, isRecord, composeAbortSignals, mergeHeaders, } from './acpTransportUtils.js';
/**
 ...
 */
const SESSION_STREAM_REPLY_METHODS = new Set([
    'session/prompt',
    'session/cancel',
 ...
 */
function permissionRequestToEvent(msg, busId) {
    const params = isRecord(msg['params']) ? msg['params'] : {};
    const meta = isRecord(params['_meta']) ? params['_meta'] : undefined;
    const qwenMeta = meta && isRecord(meta['qwen']) ? meta['qwen'] : undefined;
    const requestId = qwenMeta && typeof qwenMeta['requestId'] === 'string'
        ? qwenMeta['requestId']
        : undefined;
    if (!requestId)
        return undefined;
...
export class AcpHttpTransport {
     ...
     */
    activeSessionSubscriptions = new Map();
    /**
    ...
    async fetch(url, init, _opts) {
        ...
        const params = mapping.extractParams(segments, body, httpMethod, parsedUrl.searchParams);
        const response = await this.sendRequest(mapping.method, params, init.signal ?? undefined, init.headers);
        if (response.error) {
            ...
            const httpStatus = isRecord(errorData) && typeof errorData['httpStatus'] === 'number'
    ...
    async *subscribeEvents(sessionId, opts = {}) {
        ...
        this.activeSessionSubscriptions.set(sessionId, (this.activeSessionSubscriptions.get(sessionId) ?? 0) + 1);
        ...
        try {
            ...
            if (n <= 0)
                ...
                this.activeSessionSubscriptions.set(sessionId, n);
            ...
            if (!this._disposed && n <= 0 && !this.sessionReplyPumps.has(sessionId)) {
                const reason = subscriptionError ??
    ...
    async *subscribeEventsInner(sessionId, opts = {}) {
        await this.ensureInitialized();
        ...
        if (opts.connectTimeoutMs && Number.isFinite(opts.connectTimeoutMs)) {
            connectTimer = setTimeout(() => connectCtrl.abort(new DOMException('Initial connect timed out', 'TimeoutError')), opts.connectTimeoutMs);
        ...
        if (!ct.toLowerCase().includes('text/event-stream')) {
            ...
            throw Object.assign(new Error(`GET /acp (session stream): expected content-type text/event-stream, got "${ct}"`), { status: res.status, body: ct });
        }
        if (!res.body) {
        ...
        try {
            while (!signal?.aborted) {
                ...
                if (buf.length > MAX_SSE_BUF_CHARS) {
                    throw new Error(`AcpHttpTransport: unread SSE buffer exceeded ${MAX_SSE_BUF_CHARS} ` +
                ...
                for (const rawFrame of frames) {
                    ...
                    if (typeof method === 'string' && !hasId) {
                        const ev = denormalizeAcpNotification(msg);
                        if (ev) {
    ...
    openConnStream() {
        ...
        void this.pumpConnStream(headers, abort.signal)
            ...
            if (this.connStreamAbort === abort)
                this.connStreamAbort = undefined;
        });
    }
    async pumpConnStream(headers, signal) {
        const res = await this._fetch(`${this.baseUrl}/acp`, {
            headers,
            signal,
        });
        ...
        if (!res.ok || !res.body) {
            throw new Error(`Connection SSE stream failed: HTTP ${res.status} ${res.statusText}`);
        }
        const reader = res.body.getReader();
        const decoder = new TextDecoder();
        let buf = '';
    ...
    async sendNotification(method, params, callerHeaders) {
        const notification = {
            jsonrpc: '2.0',
            method,
            params,
     ...
     */
    async sendRequest(method, params, signal, callerHeaders) {
        const req = {
            jsonrpc: '2.0',
            id: this.nextId++,
            method,
        ...
        const replyScopeSessionId = sessionId && SESSION_STREAM_REPLY_METHODS.has(method)
            ? sessionId
            : undefined;
        const releaseSessionPump = replyScopeSessionId &&
     ...
     */
    ensureSessionReplyPump(sessionId) {
     ...
     */
    async pumpSessionReplies(sessionId, signal) {
        ...
        if (!res.ok || !res.body) {
            throw new Error(`session reply pump: HTTP ${res.status} ${res.statusText}`);
        }
        ...
        if (!ct.toLowerCase().includes('text/event-stream')) {
            ...
            throw new Error(`session reply pump: expected content-type text/event-stream, got "${ct}"`);
        }
        const reader = res.body.getReader();
        const decoder = new TextDecoder();
        let buf = '';
 ...
 */
function extractConnectionId(obj, path) {
    let cur = obj;
...
//# sourceMappingURL=AcpHttpTransport.js.map