...
export const QueryOptionsSchema = z
    ...
    agents: z
        ...
        val.description &&
        val.systemPrompt), {
        message: 'agents must be an array of SubagentConfig objects with non-empty name, description, and systemPrompt',
    }))
        .optional(),
    ...
    includeDirectories: z
        .array(z.string().refine((s) => !s.includes(','), {
        message: 'includeDirectories items cannot contain commas',
    }))
        .optional(),
    extraArgs: z
        .array(z.string().min(1, 'extraArgs items cannot be empty'))
        .refine((args) => !args.some((arg) => RESERVED_CLI_FLAGS.has(arg.split('=')[0] ?? '')), (args) => {
        const blocked = args.find((arg) => RESERVED_CLI_FLAGS.has(arg.split('=')[0] ?? ''));
        return {
