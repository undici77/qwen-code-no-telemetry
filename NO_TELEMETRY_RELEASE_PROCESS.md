...
- For standard releases (e.g., `v0.11.0`), append `-no-telemetry`
- For release branches (e.g., `release/v0.12.0`), use just the version number with `-no-telemetry`
- Always append `-no-telemetry` to the target branch name
...
# 4. Verify auto-update is disabled (no-telemetry requirement)
grep -r '"enableAutoUpdate"' packages/cli/src/config/settingsSchema.ts | grep "default: false" && echo "PASS: auto-update disabled" || echo "FAIL: enableAutoUpdate should be false"
...
#### 4. Auto-update must be disabled
The no-telemetry version requires auto-update to be disabled:

- In `packages/cli/src/config/settingsSchema.ts`: Set `default: false` for `enableAutoUpdate`
- In Dockerfile's default `settings.json`: Set `"enableAutoUpdate": false`

This prevents automatic update checks that would contact external servers.
...
| Auto-update | Enabled by default | Disabled by default (`enableAutoUpdate: false`) |
