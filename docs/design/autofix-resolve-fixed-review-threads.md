...
### Round-report observability

Fail closed must not mean fail silent (issue #10106: 0 of 90 threads resolved on PR #9729 across ~16 rounds, with every refusal visible only as run-log warnings). Every guard that refuses records a stable name — `salvage merge`, `missing verified_head`, `verified_head mismatch`, `live-head drift` or `live-head unreadable` for the up-front skips; `live-head drift`, `thread state unproven`, `mutation post-check ambiguous` for a mid-list stop — and the round report posted on the PR carries one host-authored line naming that guard and counting the selected, resolved, and left-behind threads. Rounds with no refusing guard report the positive count (including per-thread misses and an incomplete thread fetch), so a healthy mechanism is visible by the same line whose numbers go wrong when it dies. The counts are thread-accurate: two selected ids that sit in one thread count once, and an id whose thread was already resolved before the fetch is subtracted from the residual instead of being reported left behind — a re-listed still-holding fix must not keep the residual above zero forever. The line is composed exclusively from fixed host strings and counters; no agent-authored content enters it.
...
- **Bounded wait for the initial live-head equality:** the PR read model is eventually consistent, and a `headRefOid` read seconds after this round's own push routinely still returns the previous head — on PR #9729 every pushed round declared drift on that single stale read and silently skipped (issue #10106). The initial equality check retries briefly before declaring drift, and names the refusal `live-head unreadable` rather than `live-head drift` when no attempt returned any head, so an expired PAT or an API outage reads differently from a contributor push. The per-mutation guards stay single-shot: once the head has been observed equal, a later mismatch means it actually moved. The proof runs only when the round selected at least one thread: its premise is THIS round's own push propagating, so a pushed round waits out the window while a round that pushed nothing decides on a single read — a mismatched head there moves only further away, never back — and a round that selected nothing to resolve has no head proof to run at all.
