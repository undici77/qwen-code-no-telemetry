...
- `worktree_is_workspace` 对根在该 worktree 上**或其下**的已注册工作区都拒绝，因为无论哪一种，删掉这个 worktree 都会把它毁掉。「其下」按本平台的路径分隔符分段判断，因此名字本身以两个点开头的目录算在里面；在 POSIX 上，名字里含反斜杠的目录也算在里面。`worktree_in_use` 按 daemon 为运行中的会话记下的东西计数：bridge 摘要里写着的 worktree，或者该会话的 sidecar 里写着的 worktree——启动后才经由 `enter_worktree` 进入 worktree 的会话就记在这里，标签页也正是按这份记录把会话 chip 挂到行上的。标记为已移交的 sidecar 照样计入，因为没能停下旧会话的 worktree 重置会让它继续在检出里运行；`exit_worktree` 保留下来的 sidecar 也计入，它的 chip 同样还在，这时由强制删除来解决。sidecar 存在却读不了——是链接、目录、FIFO、daemon 无权打开的文件，或解析不了的文件——时以 `sessionsUnknown` 拦住删除，而不算作一个会话：它可能写的就是这个 worktree，但把它计成会话就是在声称一个 daemon 从未看到在这里的会话。由于不知道它写的是哪个 worktree，只要该会话仍在运行、其 sidecar 仍读不了，它就会拦住每个工作区里的每一次非强制删除。拒绝无法指出是哪个会话。标签页把该会话的 chip 放在哪里另有依据——它的 bridge 摘要，或比门禁更宽松的读取——因此 chip 可能在某一行上（包括自身拒绝说无法确定该会话位置的那一行），也可能不在任何一行上；越过它的办法是强制删除。解析不了的文件之所以在列，是因为原地重写在截断与写入之间停住多久，读取看到的就是这样的文件多久。与写入方替换或清除该文件撞在一起的读取会先重读，再决定是否拦住删除；每次读取都能解析、却都不是一条记录的文件不指向任何 worktree，也不会得到 chip。两份记录都没有、却根在该 worktree 里的会话不在它的保护范围内；该 worktree 树内任何未提交改动仍由脏检查覆盖，但被忽略的嵌套检出不算改动，因此也不在覆盖范围内。后两项继承自本功能所依赖的既有界面，此处刻意保留。所有「拦住删除」的闸门——工作区闸门、会话计数、行上的工作区标记——都按文件系统自己持有的拼写来比较 daemon 手里的路径与 git 记下的路径，因为大小写不敏感的卷和 Unicode 规范化都会让同一个目录变成两个字符串，而漏掉的闸门就是放行的闸门。prune 回退的身份校验是另一个问题：它比较的是两个都由 git 记下的路径，所以保留 git 写下的拼写。
...
- 会话在客户端按会话列表的一页（100 条）关联；会话落在该页之外的 worktree 不显示会话。关联还是按路径精确相等，而 daemon 的统计会先解析符号链接，因此当会话记录的路径与 git 的列表对同一个目录写法不同时，一行可能既不显示任何会话 chip、又被以「使用中」拒绝。会话的 sidecar 读不了，或者它写的是这个 worktree 而 bridge 摘要写的是另一个时也一样，因为列表以摘要为准。这些限制的都是标签页*显示*什么，不是 daemon 拒绝什么：删除检查在服务端统计，不分页，且跨所有已注册 runtime。这条关联早于本次改动。
...
- `packages/web-shell/client/components/dialogs/GitWorktreesDialog.test.tsx`：带徽标的行、惰性状态、会话关联与打开、主工作树与当前工作区不可删除、确认与刷新、刷新时重读工作树状态而非复用、过期行不显示任何状态、状态不可读的拒绝仍提供强制、删除失败后也刷新、紧随其后的刷新失败时拒绝仍留在屏幕上、比删除活得久的目录在列表上方被告知、刷新前那次慢探测绝不覆盖刷新后的新状态、裸仓不被探测状态、无理由的锁仍有徽标、锁被指名并可强制越过、一次确认里点清每一类损失、另一个拒绝顺带会带走嵌套仓库时一并点出、而嵌套仓库本身就是拒绝理由时只说一遍且仍给出强制按钮、落在用户已离开、已被过滤掉、或已被刷新抹掉的那一行上的拒绝——带上 daemon 给的原因，且只在那一行说不了的时候才说、挡住删除的工作区被点名、daemon 没给出名字时也仍有话说、被过滤掉的行回来时键盘仍留在过滤框里、另一个工作区对同一路径的回应落地时该行不会重新可点、旧请求的回应不会落进新请求的面板、但它报告的成功照样拿掉那一行、清掉关于同一个 worktree 的较新拒绝、并说明留下的目录，而来自另一个仓库的成功在这里什么都不改变、行数只算屏幕上还在的、在删掉的路径上重新建的 worktree 在列表说有时会显示出来、回应是拒绝时键盘从危险按钮上移开、删掉的行在刷新到来之前就从屏幕上拿掉、「已经没了」按办成处理、空的失败仍按失败显示、没能检查的事如实说出、拒绝落地时键盘留在用户放的地方、为拒绝保留的列表会说明可能已过时（过滤条件把每一行都藏起来时也一样），没有拒绝在保留列表时则不提过时、面板确实少了一个按钮时键盘仍留在面板内、未作答的确认框不会挡住过期列表、锁本会掩盖的未提交改动被一并指出、同时带两种计数的拒绝仍报出会话数、daemon 定位不了的会话绝不被说成在这里运行、脏拒绝的那句话只说一遍、游离 HEAD 的 worktree 不被承诺分支、确认框不会跟着用户进入另一个仓库、git 的原话被原样转述且旁边有强制按钮、无言的拒绝也仍有话可说、已删除的行不会在刷新失败时留在屏幕上、从未到达的列表、切换工作区时丢掉前一个仓库的行、按分支过滤、脏与使用中的拒绝及强制、原样显示的失败、过滤、新建会话入口、不可用占位。
