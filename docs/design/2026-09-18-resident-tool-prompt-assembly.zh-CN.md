...
| `# Examples` 记录                                                              | 只有 `<example>` 块调用的每个工具都已声明时才保留。按 `<example>`/`</example>` 成对匹配，而不是按空行切分：示例内部本身可能含空行，按空行切会让它后续段落里的工具调用脱离门控它的标签（实现过程中被测试抓到）。不调用任何工具的块（`user: 1 + 2`）不参与门控；只要还有块存活就保留标题；模型特定的 XML 与 JSON 格式不使用 `[tool_call: …]` 记法，因此不门控。 |
