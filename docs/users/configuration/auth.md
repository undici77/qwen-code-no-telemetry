...
- **Alibaba ModelStudio**: official recommended setup. Opens a sub-menu with **Coding Plan** (for individual developers · weekly quota included), **Token Plan** (for teams and companies · usage-based billing with a dedicated endpoint), or **Standard API Key** (connect with an existing ModelStudio API key). <!-- [no-telemetry fork] --> In this fork the built-in [`web_search` tool](../../developers/tools/web-search.md) is always backed by SerpApi and needs its own `SERPAPI_API_KEY` — no provider credential here enables it.
